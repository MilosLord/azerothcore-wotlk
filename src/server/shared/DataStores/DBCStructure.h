...
struct AchievementEntry {
    ...
    // uint32 parentAchievement;                             // 3 its
    // Achievement parent (can`t start while parent uncomplete, use its Criteria
    // if don`t have own, use its progress on begin)
    ...
    uint32 flags; // 41
    // uint32    icon;                                       // 42 icon (from
    // SpellIcon.dbc) char *titleReward[16];                                //
    // 43-58 uint32 titleReward_flags;                             // 59
    uint32 count; // 60 - need this count of completed criterias (own or
                  // referenced achievement criterias)
    uint32 refAchievement; // 61 - referenced achievement (counting of all
                           // completed criterias)
};
...
struct AchievementCriteriaEntry {
    ...
    union {
        ...
        struct {
            uint32 skillID;    // 3
            uint32 skillLevel; // 4 apprentice=1, journeyman=2, expert=3,
                               // artisan=4, master=5, grand master=6
        } learn_skill_level;
        ...
        struct {
            uint32 factionID;        // 3
            uint32 reputationAmount; // 4 Total reputation amount, so 42000 =
                                     // exalted
        } gain_reputation;
        ...
        struct {
            ...
            uint32 count;   // 4 count of emotes, always required special target
                            // or requirements
        } do_emote;
        ...
        /// @todo: are those special criteria stored in the dbc or do we have to
        /// add another sql table?
        struct {
            uint32 unused;    // 3
            uint32 killCount; // 4
        } special_pvp_kill;
...
struct AuctionHouseEntry {
    ...
    uint32
        faction; // 1 id of faction.dbc for player factions associated with city
...
struct BarberShopStyleEntry {
    ...
    // char const*   name[16];                               // 2-17 name of
    // hair style uint32  name_flags;                                   // 18
    // uint32  unk_name[16];                                 // 19-34, all empty
    // uint32  unk_flags;                                    // 35
    // float   CostMultiplier;                               // 36 values 1 and
    // 0.75
    uint32 race;    // 37 race
...
struct CharStartOutfitEntry {
    ...
    // int32 ItemDisplayId[MAX_OUTFIT_ITEMS];                // 29-52 not
    // required at server side int32 ItemInventorySlot[MAX_OUTFIT_ITEMS]; //
    // 53-76 not required at server side
};
...
struct CharTitlesEntry {
    ...
    uint32 bit_index; // 36 used in PLAYER_CHOSEN_TITLE and 1<<index in
...
struct ChrClassesEntry {
    ...
    // char const*       nameFemale[16];                     // 21-36 unused, if
    // different from base (male) case
    // 37 string flag, unused
    // char const*       nameNeutralGender[16];              // 38-53 unused, if
    // different from base (male) case
...
struct ChrRacesEntry {
    ...
    // uint32    unk_322;                                    // 13 faction (0
    // alliance, 1 horde, 2 not available?)
    char const* name[16]; // 14-29 used for DBC language detection/selection
    // 30 string flags, unused
    // char const*       nameFemale[16];                     // 31-46, if
    // different from base (male) case
    // 47 string flags, unused
    // char const*       nameNeutralGender[16];              // 48-63, if
    // different from base (male) case
    // 64 string flags, unused
    // 65-67 unused
    uint32 expansion; // 68 (0 - original race, 1 - tbc addon, ...)

    inline bool HasFlag(ChrRacesFlags flag) const
    {
        return (Flags & flag) != 0;
    }
};
...
struct CinematicCameraEntry {
    ...
    DBCPosition3D
          Origin; // 3-5  Position in map used for basis for M2 co-ordinates
    float OriginFacing; // 6    Orientation in map used for basis for M2
                        // co-ordinates
};
...
struct CreatureModelDataEntry {
    ...
    float
        MountHeight; // Used in calculation of unit collision data when mounted
    // float Unks[11]

    inline bool HasFlag(CreatureModelDataFlags flag) const
    {
        return (Flags & flag) != 0;
    }
};
...
struct CreatureTypeEntry {
    ...
    //  17       string flags
    // uint32    no_expirience;                              // 18 no exp?
    // critters, non-combat pets, gas cloud.
};
...
struct CurrencyCategoryEntry
{
    uint32    ID;                                           // 0
    uint32    Unk1;                                         // 1        0 for
known categories and 3 for unknown one (3.0.9) char const*   Name[16]; // 2-17
name
    //                                                      // 18       string
flags
};
...
struct CurrencyTypesEntry {
    ...
    // uint32    Category;                                   // 2        may be
    // category
    uint32 BitIndex; // 3        bit index in PLAYER_FIELD_KNOWN_CURRENCIES (1
                     // << (index-1))
};
...
struct DungeonEncounterEntry {
    ...
    // uint32 unk0;                                          // 3
    uint32
        encounterIndex; // 4        encounter index for creating completed mask
...
struct FactionEntry {
    ...
    float  spilloverRateIn;     // 19       Faction gains incoming rep *
                                // spilloverRateIn
    float spilloverRateOut; // 20       Faction outputs rep * spilloverRateOut
                            // as spillover reputation
    uint32 spilloverMaxRankIn; // 21       The highest rank the faction will
                               // profit from incoming spillover
    // uint32    spilloverRank_unk;                          // 22       It does
    // not seem to be the max standing at which a faction outputs spillover
    // ...so no idea
...
struct FactionTemplateEntry {
    ...
    uint32
        enemyFaction[MAX_FACTION_RELATIONS]; // 6
                                             // m_enemies[MAX_FACTION_RELATIONS]
    uint32
        friendFaction[MAX_FACTION_RELATIONS]; // 10
                                              // m_friend[MAX_FACTION_RELATIONS]
    //-------------------------------------------------------  end structure
    ...
    {
        return (factionFlags &
                FACTION_TEMPLATE_FLAG_ATTACK_PVP_ACTIVE_PLAYERS) != 0;
    }
};
...
struct GameObjectArtKitEntry {
    ...
    // m_textureVariations[3] char* AttachModel[4] // 4-8 m_attachModels[4]
};
...
// gtOCTClassCombatRatingScalar.dbc stores data for 32 ratings, look at
// MAX_COMBAT_RATING for real used amount
...
struct HolidaysEntry {
    ...
    uint32 Date[MAX_HOLIDAY_DATES]; // 11-36    m_date (dates in unix time
                                    // starting at January, 1, 2000)
    ...
    // uint32 holidayNameId;                                 // 49
    // m_holidayNameID (HolidayNames.dbc) uint32 holidayDescriptionId; // 50
    // m_holidayDescriptionID (HolidayDescriptions.dbc)
    char const* TextureFilename; // 51       m_textureFilename
    uint32      Priority;        // 52       m_priority
    int32 CalendarFilterType;    // 53       m_calendarFilterType (-1 = Fishing
                                 // Contest, 0 = Unk, 1 = Darkmoon Festival, 2 =
                                 // Yearly holiday)
...
//{
//     uint32      ID;
//     uint32      condExtendedCost;                         //
//     ItemTemplate::CondExtendedCost uint32      itemextendedcostentry; //
//     ItemTemplate::ExtendedCost uint32      arenaseason; // arena season
//     number(1-4)
// };
...
struct ItemExtendedCostEntry {
    ...
    uint32
        reqitemcount[MAX_ITEM_EXTENDED_COST_REQUIREMENTS]; // 9-14 required
                                                           // count of 1st item
...
struct ItemRandomSuffixEntry {
    ...
    // std::array<uint32, 2> UnusedEnchantment;                          //
    // 22-23
    std::array<uint32, MAX_ITEM_ENCHANTMENT_EFFECTS> AllocationPct; // 24-26
    // std::array<uint32, 2> UnusedAllocationPct;                        //
    // 27-28
};
...
struct MapEntry {
    ...
    // char const*     hordeIntro[16];                       // 23-38 text for
    // PvP Zones
    //  39 intro text flags
    // char const*     allianceIntro[16];                    // 40-55 text for
    // PvP Zones
    //  56 intro text flags
    ...
    uint32 maxPlayers; // 65 max players, fallback if not present in
                       // MapDifficulty.dbc
    ...
    {
        return map_type == MAP_INSTANCE || map_type == MAP_RAID ||
               map_type == MAP_BATTLEGROUND || map_type == MAP_ARENA;
    }
    ...
    [[nodiscard]] bool IsDynamicDifficultyMap() const
    {
        return Flags & MAP_FLAG_DYNAMIC_DIFFICULTY;
    }
};
...
struct MapDifficultyEntry {
    ...
    char const* areaTriggerText; // 3-18 text showed when transfer to map failed
...
struct PvPDifficultyEntry {
    ...
    // helpers
    PvPDifficultyEntry(uint32 mapId,
                       uint32 bracketId,
                       uint32 minLevel,
                       uint32 maxLevel,
                       uint32 difficulty)
        : mapId(mapId), bracketId(bracketId), minLevel(minLevel),
          maxLevel(maxLevel), difficulty(difficulty)
    {
    }
    [[nodiscard]] BattlegroundBracketId GetBracketId() const
    {
...
struct ScalingStatValuesEntry {
    ...
    [[nodiscard]] uint32 getssdMultiplier(uint32 mask) const
    {
        if (mask & 0x4001F) {
            if (mask & 0x00000001)
                return ssdMultiplier[0]; // Shoulder
            if (mask & 0x00000002)
                return ssdMultiplier[1]; // Trinket
            if (mask & 0x00000004)
                return ssdMultiplier[2]; // Weapon1H
            if (mask & 0x00000008)
                return ssdMultiplier2;
            if (mask & 0x00000010)
                return ssdMultiplier[3]; // Ranged
            if (mask & 0x00040000)
    ...
    [[nodiscard]] uint32 getArmorMod(uint32 mask) const
    {
        if (mask & 0x00F801E0) {
            if (mask & 0x00000020)
                return armorMod[0]; // Cloth shoulder
            if (mask & 0x00000040)
                return armorMod[1]; // Leather shoulder
            if (mask & 0x00000080)
                return armorMod[2]; // Mail shoulder
            if (mask & 0x00000100)
                return armorMod[3]; // Plate shoulder

            if (mask & 0x00080000)
                return armorMod2[0]; // cloak
            if (mask & 0x00100000)
                return armorMod2[1]; // cloth
            if (mask & 0x00200000)
                return armorMod2[2]; // leather
            if (mask & 0x00400000)
                return armorMod2[3]; // mail
            if (mask & 0x00800000)
                return armorMod2[4]; // plate
        }
        return 0;
    }
    ...
    [[nodiscard]] uint32 getDPSMod(uint32 mask) const
    {
        if (mask & 0x7E00) {
            if (mask & 0x00000200)
                return dpsMod[0]; // Weapon 1h
            if (mask & 0x00000400)
                return dpsMod[1]; // Weapon 2h
            if (mask & 0x00000800)
                return dpsMod[2]; // Caster dps 1h
            if (mask & 0x00001000)
                return dpsMod[3]; // Caster dps 2h
            if (mask & 0x00002000)
                return dpsMod[4]; // Ranged
            if (mask & 0x00004000)
                return dpsMod[5]; // Wand
        }
        return 0;
    }
...
struct SpellEntry {
    ...
    int32
        EquippedItemInventoryTypeMask; // 70       m_equippedItemInvTypes (mask)
    std::array<uint32, MAX_SPELL_EFFECTS> Effect; // 71-73    m_effect
    std::array<int32, MAX_SPELL_EFFECTS>
        EffectDieSides; // 74-76    m_effectDieSides
    std::array<float, MAX_SPELL_EFFECTS>
        EffectRealPointsPerLevel; // 77-79    m_effectRealPointsPerLevel
    std::array<int32, MAX_SPELL_EFFECTS>
        EffectBasePoints; // 80-82    m_effectBasePoints (must not be used in
                          // spell/auras explicitly, must be used cached
                          // Spell::m_currentBasePoints)
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectMechanic; // 83-85    m_effectMechanic
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectImplicitTargetA; // 86-88    m_implicitTargetA
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectImplicitTargetB; // 89-91    m_implicitTargetB
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectRadiusIndex; // 92-94    m_effectRadiusIndex - spellradius.dbc
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectApplyAuraName; // 95-97    m_effectAura
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectAmplitude; // 98-100   m_effectAuraPeriod
    std::array<float, MAX_SPELL_EFFECTS> EffectValueMultiplier; // 101-103
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectChainTarget; // 104-106  m_effectChainTargets
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectItemType; // 107-109  m_effectItemType
    std::array<int32, MAX_SPELL_EFFECTS>
        EffectMiscValue; // 110-112  m_effectMiscValue
    std::array<int32, MAX_SPELL_EFFECTS>
        EffectMiscValueB; // 113-115  m_effectMiscValueB
    std::array<uint32, MAX_SPELL_EFFECTS>
        EffectTriggerSpell; // 116-118  m_effectTriggerSpell
    std::array<float, MAX_SPELL_EFFECTS>
        EffectPointsPerComboPoint; // 119-121  m_effectPointsPerCombo
    ...
    // uint32    SpellNameFlag;                                      // 152 not
    // used
    std::array<char const*, 16> Rank; // 153-168  m_nameSubtext_lang
    // uint32    RankFlags;                                          // 169 not
    // used char const*     Description[16];                              //
    // 170-185  m_description_lang not used uint32    DescriptionFlags; // 186
    // not used char const*     ToolTip[16];                                  //
    // 187-202  m_auraDescription_lang not used uint32    ToolTipFlags; // 203
    // not used
    ...
    // m_stanceBarOrder not used
    std::array<float, MAX_SPELL_EFFECTS>
        EffectDamageMultiplier; // 216-218  m_effectChainAmplitude
    // uint32    MinFactionId;                                       // 219
    // m_minFactionID not used uint32    MinReputation; // 220 m_minReputation
    // not used uint32    RequiredAuraVision;                                 //
    ...
    // m_spellMissileID not used uint32  PowerDisplayId; // 228
    // PowerDisplay.dbc, new in 3.1
    std::array<float, MAX_SPELL_EFFECTS>
        EffectBonusMultiplier; // 229-231  3.2.0
    // uint32  SpellDescriptionVariableID;                           //
    // 232      3.2.0 uint32  SpellDifficultyId; // 233      3.3.0
};
...
struct SpellCastTimesEntry {
    uint32 ID;       // 0
    int32  CastTime; // 1
    // float     CastTimePerLevel;                           // 2 unsure / per
    // skill? int32     MinCastTime;                                // 3 unsure
};
...
struct SpellDifficultyEntry {
    uint32 ID;                      // 0
    int32  SpellID[MAX_DIFFICULTY]; // 1-4 instance modes: 10N, 25N, 10H, 25H or
                                    // Normal/Heroic if only 1-2 is set, if 3-4
                                    // is 0 then Mode-2
};
...
struct SpellRuneCostEntry {
    ...
    [[nodiscard]] bool NoRuneCost() const
    {
        return RuneCost[0] == 0 && RuneCost[1] == 0 && RuneCost[2] == 0;
    }
...
struct SpellItemEnchantmentEntry {
    ...
    uint32 type
        [MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]; // 2-4
                                              // m_effect[MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]
    uint32 amount
        [MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]; // 5-7
                                              // m_effectPointsMin[MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]
    // uint32      amount2[MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]   // 8-10
    // m_effectPointsMax[MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]
    uint32 spellid
        [MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]; // 11-13
                                              // m_effectArg[MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS]
    char const* description[16]; // 14-29    m_name_lang[16]
    // uint32      descriptionFlags;                             // 30 name
    // flags
...
struct SummonPropertiesEntry {
    uint32 Id;       // 0
    uint32 Category; // 1, 0 - can't be controlled?, 1 - something guardian?, 2
                     // - pet?, 3 - something controllable?, 4 - taxi/mount?
...
#define MAX_PET_TALENT_RANK                                                    \
    3 // use in calculations, expected <= MAX_TALENT_RANK
...
struct TalentEntry {
    ...
    // 17-18 not used
    uint32 addToSpellBook; // 19  also need disable higest ranks on reset talent
                           // tree
    // uint32  unk2;                                         // 20, all 0
    // uint64  allowForPet;                                  // 21-22 its a 64
    // bit mask for pet 1<<m_categoryEnumID in CreatureFamily.dbc
};
...
struct TotemCategoryEntry {
    ...
    uint32 categoryMask; // 19 (compatibility mask for same type: different for
                         // totems, compatible from high to low for rods)
};
...
struct VehicleEntry {
    ...
    // int32     m_powerType[3];                             //       removed
    // in 3.1 int32     m_powerToken[3];                            // removed
    // in 3.1
...
struct VehicleSeatEntry {
    ...
    [[nodiscard]] bool CanEnterOrExit() const
    {
        return ((m_flags & VEHICLE_SEAT_FLAG_CAN_ENTER_OR_EXIT) != 0 ||
                // If it has anmation for enter/ride, means it can be
                // entered/exited by logic
                (m_flags & (VEHICLE_SEAT_FLAG_HAS_LOWER_ANIM_FOR_ENTER |
                            VEHICLE_SEAT_FLAG_HAS_LOWER_ANIM_FOR_RIDE)) != 0);
    }
    [[nodiscard]] bool CanSwitchFromSeat() const
    {
        return m_flags & VEHICLE_SEAT_FLAG_CAN_SWITCH;
    }
    ...
    {
        return m_flagsB & VEHICLE_SEAT_FLAG_B_EJECTABLE;
    }
    [[nodiscard]] bool CanControl() const
    {
        return (m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL) != 0;
    }
};
...
struct WorldMapAreaEntry {
    ...
    float x1;             // 6
    float x2;             // 7
    int32 virtual_map_id; // 8 -1 (map_id have correct map) other: virtual map
                          // where zone show (map_id - where zone in fact
                          // internally)
    // int32   dungeonMap_id;                               // 9 pointer to
    // DungeonMap.dbc (owerride x1, x2, y1, y2 coordinates) uint32  parentMapID;
    // // 10
};
...
struct WorldMapOverlayEntry {
    uint32 ID; // 0
    // uint32    worldMapAreaId;                             // 1 idx in
    // WorldMapArea.dbc
...
{
    uint32    ID;                                           // 0
    uint32    map_id;                                       // 1        Can be
-1 to show up everywhere. uint32    zone; // 2        Can be zero for
"everywhere". uint32    phaseMask;                                    // 3 Phase
this WorldState is avaliable in uint32    icon; // 4        The icon that is
used in the interface. char const*     textureFilename; // 5 char const* text;
// 6-21     The worldstate text char const*     description; // 22-38    Text
shown when hovering mouse on icon uint32    worldstateID; // 39       This is
the actual ID used uint32    type;                                         // 40
0 = unknown, 1 = unknown, 2 = not shown in ui, 3 = wintergrasp uint32    unk1;
// 41 uint32    unk2;                                         // 43 uint32 unk3;
// 44-58 uint32    unk4;                                         // 59-61 Used
for some progress bars. uint32    unk7; // 62       Unused in 3.3.5a
};
*/
...
// Structures not used for casting to loaded DBC data and not required then
// packing
struct MapDifficulty {
    MapDifficulty() = default;
    MapDifficulty(uint32 _resetTime, uint32 _maxPlayers, bool _hasErrorMessage)
        : resetTime(_resetTime), maxPlayers(_maxPlayers),
...
struct TalentSpellPos {
    ...
    TalentSpellPos(uint16 _talent_id, uint8 _rank)
        : talent_id(_talent_id), rank(_rank)
    {
    }
