...
// Client expected level limitation, like as used in DBC item max levels for
// "until max player level" use as default max player level, must be fit max
// level for used client also see MAX_LEVEL and STRONG_MAX_LEVEL define
#define DEFAULT_MAX_LEVEL 80

// client supported max level for player/pets/etc. Avoid overflow or client
// stability affected. also see GT_MAX_LEVEL define
...
enum AchievementFlags {
    ...
    ACHIEVEMENT_FLAG_HIDDEN =
        0x00000002, // Not sent to client - internal use only
    ACHIEVEMENT_FLAG_STORE_MAX_VALUE =
        0x00000004, // Store only max value? used only in "Reach level xx"
    ACHIEVEMENT_FLAG_SUMM = 0x00000008, // Use summ criteria value from all
                                        // reqirements (and calculate max value)
    ...
    ACHIEVEMENT_FLAG_AVERAGE =
        0x00000040, // Show as average value (value / time_in_days) depend from
                    // other flag (by def use last criteria value)
    ACHIEVEMENT_FLAG_BAR =
        0x00000080, // Show as progress bar (value / max vale) depend from other
                    // flag (by def use last criteria value)
...
enum AchievementCriteriaCondition {
    ...
    ACHIEVEMENT_CRITERIA_CONDITION_BG_MAP =
        3, // requires you to be on specific map, reset at change
    ...
    ACHIEVEMENT_CRITERIA_CONDITION_NO_SPELL_HIT =
        9, // requires the player not to be hit by specific spell
    ACHIEVEMENT_CRITERIA_CONDITION_NOT_IN_GROUP =
        10, // requires the player not to be in group
...
enum AchievementCriteriaTimedTypes {
    ACHIEVEMENT_TIMED_TYPE_EVENT =
        1, // Timer is started by internal event with id in timerStartEvent
    ACHIEVEMENT_TIMED_TYPE_QUEST =
        2, // Timer is started by accepting quest with entry in timerStartEvent
    ACHIEVEMENT_TIMED_TYPE_SPELL_CASTER =
        5, // Timer is started by casting a spell with entry in timerStartEvent
    ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET =
        6, // Timer is started by being target of spell with entry in
           // timerStartEvent
    ACHIEVEMENT_TIMED_TYPE_CREATURE =
        7, // Timer is started by killing creature with entry in timerStartEvent
    ACHIEVEMENT_TIMED_TYPE_ITEM =
        9, // Timer is started by using item with entry in timerStartEvent
...
enum AchievementCriteriaTypes {
    ...
    ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST_DAILY =
        10, // you have to complete a daily quest x times in a row
    ...
    ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA =
        37, /// @todo: the archievements 1162 and 1163 requires a special rating
            /// which can't be found in the dbc
    ...
    ACHIEVEMENT_CRITERIA_TYPE_VISIT_BARBER_SHOP =
        48, // note: rewarded as soon as the player payed, not at taking place
            // at the seat
    ACHIEVEMENT_CRITERIA_TYPE_EQUIP_EPIC_ITEM = 49,
    ACHIEVEMENT_CRITERIA_TYPE_ROLL_NEED_ON_LOOT =
        50, /// @todo: itemlevel is mentioned in text but not present in dbc
    ...
    ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS =
        56, /// @todo: in some cases map not present, and in some cases need do
            /// without die
    ...
    ACHIEVEMENT_CRITERIA_TYPE_ON_LOGIN =
        74, /// @todo: title id is not mentioned in dbc
    ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS = 75,
    ACHIEVEMENT_CRITERIA_TYPE_WIN_DUEL               = 76,
    ACHIEVEMENT_CRITERIA_TYPE_LOSE_DUEL              = 77,
    ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE_TYPE =
        78, /// @todo: creature type (demon, undead etc.) is not stored in dbc
    ...
    ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL2 =
        110, /// @todo: target entry is missing
...
enum AreaFlags {
    ...
    AREA_FLAG_UNK1 = 0x00000002, // Razorfen Downs, Naxxramas and Acherus: The
                                 // Ebon Hold (3.3.5a)
    AREA_FLAG_UNK2 =
    ...
    AREA_FLAG_OUTLAND =
        0x00000400, // expansion zones? (only Eye of the Storm not have this
                    // flag, but have 0x00004000 flag)
    AREA_FLAG_SANCTUARY = 0x00000800, // sanctuary area (PvP disabled)
    AREA_FLAG_NEED_FLY =
        0x00001000, // Respawn alive at the graveyard without corpse
    AREA_FLAG_UNUSED1 = 0x00002000, // Unused in 3.3.5a
    AREA_FLAG_OUTLAND2 =
        0x00004000, // expansion zones? (only Circle of Blood Arena not have
                    // this flag, but have 0x00000400 flag)
    AREA_FLAG_OUTDOOR_PVP =
        0x00008000, // pvp objective area? (Death's Door also has this flag
                    // although it's no pvp object area)
    AREA_FLAG_ARENA_INSTANCE = 0x00010000, // used by instanced arenas only
    AREA_FLAG_UNUSED2        = 0x00020000, // Unused in 3.3.5a
    AREA_FLAG_CONTESTED_AREA =
        0x00040000, // On PvP servers these areas are considered contested, even
                    // though the zone it is contained in is a Horde/Alliance
                    // territory.
    ...
    AREA_FLAG_REST_ZONE_HORDE =
        0x00400000, // Instead of using areatriggers, the zone will act as one
                    // for Horde players (Warsong Hold, Acherus: The Ebon Hold,
                    // New Agamand Inn, Vengeance Landing Inn, Sunreaver
                    // Pavilion, etc)
    AREA_FLAG_REST_ZONE_ALLIANCE =
        0x00800000, // Instead of using areatriggers, the zone will act as one
                    // for Alliance players (Valgarde, Acherus: The Ebon Hold,
                    // Westguard Inn, Silver Covenant Pavilion, etc)
    AREA_FLAG_WINTERGRASP = 0x01000000, // Wintergrasp and it's subzones
    AREA_FLAG_INSIDE =
        0x02000000, // used for determinating spell related inside/outside
                    // questions in Map::IsOutdoors
    AREA_FLAG_OUTSIDE =
        0x04000000, // used for determinating spell related inside/outside
                    // questions in Map::IsOutdoors
...
#define RAID_DIFFICULTY_MASK_25MAN                                             \
    1 // since 25man difficulties are 1 and 3, we can check them like that
...
enum SpawnMask {
    SPAWNMASK_CONTINENT =
        (1 << REGULAR_DIFFICULTY), // any any maps without spawn modes

    SPAWNMASK_DUNGEON_NORMAL = (1 << DUNGEON_DIFFICULTY_NORMAL),
    SPAWNMASK_DUNGEON_HEROIC = (1 << DUNGEON_DIFFICULTY_HEROIC),
    SPAWNMASK_DUNGEON_ALL =
        (SPAWNMASK_DUNGEON_NORMAL | SPAWNMASK_DUNGEON_HEROIC),
    ...
    SPAWNMASK_RAID_NORMAL_ALL =
        (SPAWNMASK_RAID_10MAN_NORMAL | SPAWNMASK_RAID_25MAN_NORMAL),
    ...
    SPAWNMASK_RAID_HEROIC_ALL =
        (SPAWNMASK_RAID_10MAN_HEROIC | SPAWNMASK_RAID_25MAN_HEROIC),

    SPAWNMASK_RAID_ALL =
        (SPAWNMASK_RAID_NORMAL_ALL | SPAWNMASK_RAID_HEROIC_ALL),
};

enum FactionFlags {
    FACTION_FLAG_NONE    = 0x00, // no faction flag
    FACTION_FLAG_VISIBLE = 0x01, // makes visible in client (set or can be set
                                 // at interaction with target of this faction)
    ...
    FACTION_FLAG_HIDDEN =
        0x04, // hidden faction from reputation pane in client (player can gain
              // reputation, but this update not sent to client)
    FACTION_FLAG_INVISIBLE_FORCED =
        0x08, // always overwrite FACTION_FLAG_VISIBLE and hide faction in
              // rep.list, used for hide opposite team factions
    FACTION_FLAG_PEACE_FORCED =
        0x10, // always overwrite FACTION_FLAG_AT_WAR, used for prevent war with
              // own team factions
    FACTION_FLAG_INACTIVE = 0x20, // player controlled, state stored in
                                  // characters.data (CMSG_SET_FACTION_INACTIVE)
    FACTION_FLAG_RIVAL   = 0x40,  // flag for the two competing outland factions
    FACTION_FLAG_SPECIAL = 0x80   // horde and alliance home cities and their
                                  // northrend allies have this flag
};

enum FactionTemplateFlags {
    FACTION_TEMPLATE_FLAG_RESPOND_TO_CALL_FOR_HELP =
        0x0001, /// @todo: Not Yet Implemented (NYI)
    ...
    FACTION_TEMPLATE_FLAG_ASSIST_PLAYERS =
        0x0800, // Old title: FACTION_TEMPLATE_FLAG_ASSIST_PLAYERS, Old comment:
                // flagged for PvP //@todo: Should see if this is implemented
                // correctly.
    FACTION_TEMPLATE_FLAG_ATTACK_PVP_ACTIVE_PLAYERS =
        0x1000, // Old title: FACTION_TEMPLATE_FLAG_ATTACK_PVP_ACTIVE_PLAYERS,
                // Old comment: faction will attack players that were involved
                // in PvP combats //@todo: Should see if this is implemented
                // correctly.
    FACTION_TEMPLATE_FLAG_HATES_ALL_EXCEPT_FRIENDS =
        0x2000, // Old title: FACTION_TEMPLATE_FLAG_HATES_ALL_EXCEPT_FRIENDS,
                // Old comment: //@todo: Should see if this is implemented
                // correctly.
};
...
enum AbilytyLearnType {
    SKILL_LINE_ABILITY_LEARNED_ON_SKILL_VALUE =
        1, // Spell state will update depending on skill value
    SKILL_LINE_ABILITY_LEARNED_ON_SKILL_LEARN =
        2 // Spell will be learned/removed together with entire skill
};
...
enum ItemLimitCategoryMode {
    ITEM_LIMIT_CATEGORY_MODE_HAVE =
        0, // limit applied to amount items in inventory/bank
    ITEM_LIMIT_CATEGORY_MODE_EQUIP =
        1, // limit applied to amount equipped items (including used gems)
};
...
enum SkillRaceClassInfoFlags {
    ...
    SKILL_FLAG_INCLUDE_IN_SORT =
        0x80, // Spells belonging to a skill with this flag will additionally
              // compare skill ids when sorting spellbook in client
    SKILL_FLAG_NOT_TRAINABLE = 0x100,
    SKILL_FLAG_MONO_VALUE    = 0x400 // Skill always has value 1 - clientside
                                  // display flag, real value can be different
};
...
enum SummonPropGroup {
    ...
    SUMMON_PROP_GROUP_CONTROLLABLE =
        3, /// @todo: NYI 13 spells in 3.0.3, mostly controllable
...
enum SummonPropFlags {
    ...
    SUMMON_PROP_FLAG_ATTACK_SUMMONER =
        0x00000001, /// @todo: NYI 75 spells in 3.0.3, something unfriendly
    SUMMON_PROP_FLAG_ASSIST_COMBAT_SUMMON =
        0x00000002, /// @todo: NYI 616 spells in 3.0.3, something friendly
    SUMMON_PROP_FLAG_USE_LEVEL_OFFSET =
        0x00000004, /// @todo: NYI 22 spells in 3.0.3, no idea...
    SUMMON_PROP_FLAG_DESPAWN_ON_SUMMONER_DEATH =
        0x00000008, /// @todo: NYI 49 spells in 3.0.3, some mounts
    SUMMON_PROP_FLAG_ONLY_VISIBLE_TO_SUMMONER =
        0x00000010, // 25 spells in 3.0.3, quest related?
    SUMMON_PROP_FLAG_CANNOT_DISMISS_PET =
        0x00000020, /// @todo: NYI 0 spells in 3.3.5, unused
    SUMMON_PROP_FLAG_USE_DEMON_TIMEOUT =
        0x00000040, /// @todo: NYI 12 spells in 3.0.3, no idea
    SUMMON_PROP_FLAG_UNLIMITED_SUMMONS =
        0x00000080, /// @todo: NYI 4 spells in 3.0.3, no idea
    SUMMON_PROP_FLAG_USE_CREATURE_LEVEL =
        0x00000100, /// @todo: NYI 51 spells in 3.0.3, no idea, many quest
                    /// related
    SUMMON_PROP_FLAG_JOIN_SUMMONER_SPAWN_GROUP =
        0x00000200, /// @todo: NYI 51 spells in 3.0.3, something defensive
    SUMMON_PROP_FLAG_DO_NOT_TOGGLE =
        0x00000400, /// @todo: NYI 3 spells, requires something near?
    SUMMON_PROP_FLAG_DESPAWN_WHEN_EXPIRED =
        0x00000800, /// @todo: NYI 30 spells in 3.0.3, no idea
    SUMMON_PROP_FLAG_USE_SUMMONER_FACTION =
        0x00001000, /// @todo: NYI Lightwell, Jeeves, Gnomish Alarm-o-bot, Build
                    /// vehicles(wintergrasp)
    SUMMON_PROP_FLAG_DO_NOT_FOLLOW_MOUNTED_SUMMONER =
        0x00002000, /// @todo: NYI Guides, player follows
    SUMMON_PROP_FLAG_SAVE_PET_AUTOCAST =
        0x00004000, /// @todo: NYI Force of Nature, Shadowfiend, Feral Spirit,
                    /// Summon Water Elemental
    SUMMON_PROP_FLAG_IGNORE_SUMMONER_PHASE =
        0x00008000, /// @todo: NYI Light/Dark Bullet, Soul/Fiery Consumption,
                    /// Twisted Visage, Twilight Whelp. Phase related?
};
...
enum VehicleSeatFlags {
    ...
    VEHICLE_SEAT_FLAG_CAN_CAST_MOUNT_SPELL =
        0x00001000, // Can cast spells with SPELL_AURA_MOUNTED from seat
                    // (possibly 4.x only, 0 seats on 3.3.5a)
    VEHICLE_SEAT_FLAG_UNCONTROLLED =
        0x00002000, // can override !& VEHICLE_SEAT_FLAG_CAN_ENTER_OR_EXIT
    VEHICLE_SEAT_FLAG_CAN_ATTACK =
        0x00004000, // Can attack, cast spells and use items from vehicle
    ...
    VEHICLE_SEAT_FLAG_IS_USING_VEHICLE_CONTROLS =
        0x00800000, // Lua_IsUsingVehicleControls
    VEHICLE_SEAT_FLAG_ENABLE_VEHICLE_ZOOM = 0x01000000,
    VEHICLE_SEAT_FLAG_CAN_ENTER_OR_EXIT =
        0x02000000, // Lua_CanExitVehicle - can enter and exit at free will
    ...
    VEHICLE_SEAT_FLAG_UNK2 =
        0x40000000, // checked in conjunction with 0x800 in CastSpell2
...
enum VehicleSeatFlagsB {
    ...
    VEHICLE_SEAT_FLAG_B_VEHICLE_PLAYERFRAME_UI =
        0x80000000, // Lua_UnitHasVehiclePlayerFrameUI - actually checked for
                    // flagsb &~ 0x80000000
};
