...
public:
    ...
    [[nodiscard]] T const* LookupEntry(uint32 id) const
    {
        return (id >= _indexTableSize) ? nullptr : _indexTable.AsT[id];
    }
    [[nodiscard]] T const* AssertEntry(uint32 id) const
    {
    ...
    iterator end()
    {
        return iterator(_indexTable.AsT, _indexTableSize, _indexTableSize);
    }
