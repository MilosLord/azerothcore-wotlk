...
#define RACEMASK_ALLIANCE                                                      \
    ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_DWARF - 1)) |                       \
     (1 << (RACE_NIGHTELF - 1)) | (1 << (RACE_GNOME - 1)) |                    \
     (1 << (RACE_DRAENEI - 1)))
...
#define CLASSMASK_ALL_PLAYABLE                                                 \
    ((1 << (CLASS_WARRIOR - 1)) | (1 << (CLASS_PALADIN - 1)) |                 \
     (1 << (CLASS_HUNTER - 1)) | (1 << (CLASS_ROGUE - 1)) |                    \
     (1 << (CLASS_PRIEST - 1)) | (1 << (CLASS_SHAMAN - 1)) |                   \
     (1 << (CLASS_MAGE - 1)) | (1 << (CLASS_WARLOCK - 1)) |                    \
...
#define CLASSMASK_ALL_CREATURES                                                \
    ((1 << (UNIT_CLASS_WARRIOR - 1)) | (1 << (UNIT_CLASS_PALADIN - 1)) |       \
     (1 << (UNIT_CLASS_ROGUE - 1)) | (1 << (UNIT_CLASS_MAGE - 1)))

#define CLASSMASK_WAND_USERS                                                   \
    ((1 << (CLASS_PRIEST - 1)) | (1 << (CLASS_MAGE - 1)) |                     \
     (1 << (CLASS_WARLOCK - 1)))
...
uint32 constexpr QuestDifficultyColors[MAX_QUEST_DIFFICULTY] = {
    0xff40c040, 0xff808080, 0xffffff00, 0xffff8040, 0xffff2020};
...
enum SpellAttr0 : uint32 {
    SPELL_ATTR0_PROC_FAILURE_BURNS_CHARGE =
        0x00000001, // TITLE Unknown attribute 0@Attr0
    SPELL_ATTR0_USES_RANGED_SLOT =
        0x00000002, // TITLE Treat as ranged attack DESCRIPTION Use ammo, ranged
                    // attack range modifiers, ranged haste, etc.
    SPELL_ATTR0_ON_NEXT_SWING_NO_DAMAGE =
        0x00000004, // TITLE On next melee (type 1) DESCRIPTION Both "on next
                    // swing" attributes have identical handling in server &
                    // client
    SPELL_ATTR0_DO_NOT_LOG_IMMUNE_MISSES =
        0x00000008, // TITLE Replenishment (client only)
    SPELL_ATTR0_IS_ABILITY =
        0x00000010, // TITLE Treat as ability DESCRIPTION Cannot be reflected,
                    // not affected by cast speed modifiers, etc.
    SPELL_ATTR0_IS_TRADESKILL =
        0x00000020, // TITLE Trade skill recipe DESCRIPTION Displayed in recipe
                    // list, not affected by cast speed modifiers
    SPELL_ATTR0_PASSIVE = 0x00000040, // TITLE Passive spell DESCRIPTION Spell
                                      // is automatically cast on self by core
    SPELL_ATTR0_DO_NOT_DISPLAY =
        0x00000080, // TITLE Hidden in UI (client only) DESCRIPTION Not visible
                    // in spellbook or aura bar (Spellbook, Aura Icon, Combat
                    // Log)
    SPELL_ATTR0_DO_NOT_LOG =
        0x00000100, // TITLE Hidden in combat log (client only) DESCRIPTION
                    // Spell will not appear in combat logs
    SPELL_ATTR0_HELD_ITEM_ONLY =
        0x00000200, // TITLE Auto-target mainhand item (client only) DESCRIPTION
                    // Client will automatically select main-hand item as cast
                    // target
    SPELL_ATTR0_ON_NEXT_SWING =
        0x00000400, // TITLE On next melee (type 2) DESCRIPTION Both "on next
                    // swing" attributes have identical handling in server &
                    // client
    SPELL_ATTR0_WEARER_CASTS_PROC_TRIGGER =
        0x00000800, // TITLE Unknown attribute 11@Attr0
    SPELL_ATTR0_SERVER_ONLY =
        0x00001000, // TITLE Only usable during daytime (unused)
    SPELL_ATTR0_ALLOW_ITEM_SPELL_IN_PVP =
        0x00002000, // TITLE Only usable during nighttime (unused)
    ...
    SPELL_ATTR0_NOT_SHAPESHIFTED =
        0x00010000, // TITLE Not usable while shapeshifted
    SPELL_ATTR0_ONLY_STEALTHED = 0x00020000, // TITLE Only usable in stealth
    SPELL_ATTR0_DO_NOT_SHEATH =
        0x00040000, // TITLE Don't shealthe weapons (client only)
    SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL =
        0x00080000, // TITLE Scale with caster level DESCRIPTION For non-player
                    // casts, scale impact and power cost with caster's level
    SPELL_ATTR0_CANCELS_AUTO_ATTACK_COMBAT =
        0x00100000, // TITLE Stop attacking after cast DESCRIPTION After casting
                    // this, the current auto-attack will be interrupted
    SPELL_ATTR0_NO_ACTIVE_DEFENSE =
        0x00200000, // TITLE Prevent physical avoidance DESCRIPTION Spell cannot
                    // be dodged, parried or blocked
    SPELL_ATTR0_TRACK_TARGET_IN_CAST_PLAYER_ONLY =
        0x00400000, // TITLE Automatically face target during cast (client only)
    SPELL_ATTR0_ALLOW_CAST_WHILE_DEAD =
        0x00800000, // TITLE Can be cast while dead DESCRIPTION Spells without
                    // this flag cannot be cast by dead units in non-triggered
                    // contexts
    SPELL_ATTR0_ALLOW_WHILE_MOUNTED =
        0x01000000, // TITLE Can be cast while mounted
    SPELL_ATTR0_COOLDOWN_ON_EVENT =
        0x02000000, // TITLE Cooldown starts on expiry DESCRIPTION Spell is
                    // unusable while already active, and cooldown does not
                    // begin until the effects have worn off
    SPELL_ATTR0_AURA_IS_DEBUFF =
        0x04000000, // TITLE Is negative spell DESCRIPTION Forces the spell to
                    // be treated as a negative spell
    SPELL_ATTR0_ALLOW_WHILE_SITTING =
        0x08000000, // TITLE Can be cast while sitting
    SPELL_ATTR0_NOT_IN_COMBAT_ONLY_PEACEFUL =
        0x10000000, // TITLE Cannot be used in combat
    SPELL_ATTR0_NO_IMMUNITIES =
        0x20000000, // TITLE Pierce invulnerability DESCRIPTION Allows spell to
                    // pierce invulnerability, unless the invulnerability spell
                    // also has this attribute
    SPELL_ATTR0_HEARTBEAT_RESIST =
        0x40000000, // TITLE Periodic resistance checks DESCRIPTION Periodically
                    // re-rolls against resistance to potentially expire aura
                    // early
    SPELL_ATTR0_NO_AURA_CANCEL =
        0x80000000 // TITLE Aura cannot be cancelled DESCRIPTION Prevents the
                   // player from voluntarily canceling a positive aura
};

// EnumUtils: DESCRIBE THIS
enum SpellAttr1 : uint32 {
    SPELL_ATTR1_DISMISS_PET_FIRST =
        0x00000001, // TITLE Dismiss Pet on cast DESCRIPTION Without this
                    // attribute, summoning spells will fail if caster already
                    // has a pet
    SPELL_ATTR1_USE_ALL_MANA =
        0x00000002, // TITLE Drain all power DESCRIPTION Ignores listed power
                    // cost and drains entire pool instead
    SPELL_ATTR1_IS_CHANNELED =
        0x00000004, // TITLE Channeled (type 1) DESCRIPTION Both "channeled"
                    // attributes have identical handling in server & client
    SPELL_ATTR1_NO_REDIRECTION =
        0x00000008, // TITLE Ignore redirection effects DESCRIPTION Spell will
                    // not be attracted by SPELL_MAGNET auras (Grounding Totem)
    SPELL_ATTR1_NO_SKILL_INCREASE =
        0x00000010, // TITLE Unknown attribute 4@Attr1 DESCRIPTION stealth and
                    // whirlwind
    SPELL_ATTR1_ALLOW_WHILE_STEALTHED =
        0x00000020, // TITLE Does not break stealth
    SPELL_ATTR1_IS_SELF_CHANNELED =
        0x00000040, // TITLE Channeled (type 2) DESCRIPTION Both "channeled"
                    // attributes have identical handling in server & client
    SPELL_ATTR1_NO_REFLECTION =
        0x00000080, // TITLE Ignore reflection effects DESCRIPTION Spell will
                    // pierce through Spell Reflection and similar
    SPELL_ATTR1_ONLY_PEACEFUL_TARGETS =
        0x00000100, // TITLE Target cannot be in combat
    SPELL_ATTR1_INITIATE_COMBAT =
        0x00000200, // TITLE Enables Auto-Attack (client only) DESCRIPTION
                    // Caster will begin auto-attacking the target on cast
    SPELL_ATTR1_NO_THREAT =
        0x00000400, // TITLE Does not generate threat DESCRIPTION Also does not
                    // cause target to engage
    SPELL_ATTR1_AURA_UNIQUE =
        0x00000800, // TITLE Aura will not refresh its duration when recast
    SPELL_ATTR1_FAILURE_BREAKS_STEALTH =
        0x00001000, // TITLE Pickpocket (client only)
    SPELL_ATTR1_TOGGLE_FAR_SIGHT =
        0x00002000, // TITLE Farsight aura (client only)
    SPELL_ATTR1_TRACK_TARGET_IN_CHANNEL =
        0x00004000, // TITLE Track target while channeling DESCRIPTION While
                    // channeling, adjust facing to face target
    SPELL_ATTR1_IMMUNITY_PURGES_EFFECT =
        0x00008000, // TITLE Immunity cancels preapplied auras DESCRIPTION For
                    // immunity spells, cancel all auras that this spell would
                    // make you immune to when the spell is applied
    SPELL_ATTR1_IMMUNITY_TO_HOSTILE_AND_FRIENDLY_EFFECTS =
        0x00010000, // TITLE Unaffected by school immunities DESCRIPTION Will
                    // not pierce Divine Shield, Ice Block and other full
                    // invulnerabilities
    SPELL_ATTR1_NO_AUTOCAST_AI =
        0x00020000, // TITLE Cannot be autocast by pet DESCRIPTION (AI)
    SPELL_ATTR1_PREVENTS_ANIM =
        0x00040000, // TITLE NYI, auras apply
                    // UNIT_FLAG_PREVENT_EMOTES_FROM_CHAT_TEXT
    SPELL_ATTR1_EXCLUDE_CASTER = 0x00080000, // TITLE Cannot be self-cast
    SPELL_ATTR1_FINISHING_MOVE_DAMAGE =
        0x00100000, // TITLE Requires combo points (type 1)
    SPELL_ATTR1_THREAT_ONLY_ON_MISS =
        0x00200000, // TITLE Unknown attribute 21@Attr1
    SPELL_ATTR1_FINISHING_MOVE_DURATION =
        0x00400000, // TITLE Requires combo points (type 2)
    SPELL_ATTR1_IGNORE_OWNERS_DEATH =
        0x00800000, // TITLE Unknwon attribute 23@Attr1
    SPELL_ATTR1_SPECIAL_SKILLUP = 0x01000000, // TITLE Fishing (client only)
    SPELL_ATTR1_AURA_STAYS_AFTER_COMBAT =
        0x02000000, // TITLE Unknown attribute 25@Attr1
    SPELL_ATTR1_REQUIRE_ALL_TARGETS =
        0x04000000, // TITLE Unknown attribute 26@Attr1 DESCRIPTION Related to
                    // [target=focus] and [target=mouseover] macros?
    SPELL_ATTR1_DISCOUNT_POWER_ON_MISS =
        0x08000000, // TITLE Unknown attribute 27@Attr1 DESCRIPTION Melee spell?
    ...
    SPELL_ATTR1_COMBO_ON_BLOCK =
        0x40000000, // TITLE Enable at dodge DESCRIPTION (Mainline: Dispel All
                    // Stacks)
    SPELL_ATTR1_CAST_WHEN_LEARNED =
        0x80000000 // TITLE Unknown attribute 31@Attr1
};

// EnumUtils: DESCRIBE THIS
enum SpellAttr2 : uint32 {
    SPELL_ATTR2_ALLOW_DEAD_TARGET =
        0x00000001, // TITLE Can target dead players or corpses
    SPELL_ATTR2_NO_SHAPESHIFT_UI =
        0x00000002, // TITLE Unknown attribute 1@Attr2 DESCRIPTION vanish,
                    // shadowform, Ghost Wolf and other
    ...
    SPELL_ATTR2_USE_SHAPESHIFT_BAR =
        0x00000010, // TITLE Show in stance bar (client only)
    SPELL_ATTR2_AUTO_REPEAT = 0x00000020, // TITLE Ranged auto-attack spell
    SPELL_ATTR2_CANNOT_CAST_ON_TAPPED =
        0x00000040, // TITLE Cannot target others' tapped units DESCRIPTION Can
                    // only target untapped units, or those tapped by caster
    SPELL_ATTR2_DO_NOT_REPORT_SPELL_FAILURE =
        0x00000080, // TITLE Unknown attribute 7@Attr2
    SPELL_ATTR2_INCLUDE_IN_ADVANCED_COMBAT_LOG =
        0x00000100, // TITLE Unknown attribute 8@Attr2 DESCRIPTION not set
                    // in 3.0.3
    SPELL_ATTR2_ALWAYS_CAST_AS_UNIT =
        0x00000200, // TITLE Unknown attribute 9@Attr2
    SPELL_ATTR2_SPECIAL_TAMING_FLAG =
        0x00000400, // TITLE Unknown attribute 10@Attr2 DESCRIPTION Related to
                    // taming?
    SPELL_ATTR2_NO_TARGET_PER_SECOND_COST = 0x00000800, // TITLE Health Funnel
    SPELL_ATTR2_CHAIN_FROM_CASTER =
        0x00001000, // TITLE Unknown attribute 12@Attr2 DESCRIPTION Cleave,
                    // Heart Strike, Maul, Sunder Armor, Swipe
    SPELL_ATTR2_ENCHANT_OWN_ITEM_ONLY =
        0x00002000, // TITLE Enchant persists when entering arena
    SPELL_ATTR2_ALLOW_WHILE_INVISIBLE =
        0x00004000, // TITLE Unknown attribute 14@Attr2
    SPELL_ATTR2_DO_NOT_CONSUME_IF_GAINED_DURING_CAST =
        0x00008000, // TITLE Unknown attribute 15@Attr2 DESCRIPTION not set
                    // in 3.0.3
    SPELL_ATTR2_NO_ACTIVE_PETS = 0x00010000, // TITLE Tame Beast
    SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS =
        0x00020000, // TITLE Don't reset swing timer DESCRIPTION Does not reset
                    // melee/ranged autoattack timer on cast
    SPELL_ATTR2_NO_JUMP_WHILE_CAST_PENDING =
        0x00040000, // TITLE Requires dead pet
    SPELL_ATTR2_ALLOW_WHILE_NOT_SHAPESHIFTED =
        0x00080000, // TITLE Also allow outside shapeshift (caster form)
                    // DESCRIPTION Even if Stances are nonzero, allow spell to
                    // be cast outside of shapeshift (though not in a different
                    // shapeshift)
    SPELL_ATTR2_INITIATE_COMBAT_POST_CAST =
        0x00100000, // TITLE (Enables Auto-Attack)
    SPELL_ATTR2_FAIL_ON_ALL_TARGETS_IMMUNE =
        0x00200000, // TITLE Damage reduction ability DESCRIPTION Causes BG
                    // flags to be dropped if combined with
                    // ATTR1_DISPEL_AURAS_ON_IMMUNITY
    SPELL_ATTR2_NO_INITIAL_THREAD =
        0x00400000, // TITLE Unknown attribute 22@Attr2 DESCRIPTION Ambush,
                    // Backstab, Cheap Shot, Death Grip, Garrote, Judgements,
                    // Mutilate, Pounce, Ravage, Shiv, Shred
    SPELL_ATTR2_PROC_COOLDOWN_ON_FAILURE =
        0x00800000, // TITLE Arcane Concentration
    SPELL_ATTR2_ITEM_CAST_WITH_OWNER_SKILL =
        0x01000000, // TITLE Unknown attribute 24@Attr2
    SPELL_ATTR2_DONT_BLOCK_MANA_REGEN =
        0x02000000, // TITLE Unknown attribute 25@Attr2
    SPELL_ATTR2_NO_SCHOOL_IMMUNITIES =
        0x04000000, // TITLE Pierce aura application immunities DESCRIPTION
                    // Allow aura to be applied despite target being immune to
                    // new aura applications
    SPELL_ATTR2_IGNORE_WEAPONSKILL =
        0x08000000,                         // TITLE Unknown attribute 27@Attr2
    ...
    SPELL_ATTR2_ACTIVE_THREAT =
        0x40000000, // TITLE Allow triggered spell to trigger (type 1)
                    // DESCRIPTION Without this attribute, any triggered spell
                    // will be unable to trigger other auras' procs
...
enum SpellAttr3 : uint32 {
    ...
    SPELL_ATTR3_NO_PROC_EQUIP_REQUIREMENT =
        0x00000002, // TITLE 1 Ignores subclass mask check when checking proc
    SPELL_ATTR3_NO_CASTING_BAR_TEXT =
        0x00000004, // TITLE Unknown attribute 2@Attr3
    ...
    SPELL_ATTR3_NO_DURABILITY_LOSS =
        0x00000020,                        // TITLE Unknown attribute 5@Attr3
    SPELL_ATTR3_NO_AVOIDANCE = 0x00000040, // TITLE Unknown attribute 6@Attr3
    SPELL_ATTR3_DOT_STACKING_RULE =
        0x00000080, // TITLE Stack separately for each caster
    SPELL_ATTR3_ONLY_ON_PLAYER = 0x00000100, // TITLE Can only target players
    SPELL_ATTR3_NOT_A_PROC =
        0x00000200, // TITLE Allow triggered spell to trigger (type 2)
                    // DESCRIPTION Without this attribute, any triggered spell
                    // will be unable to trigger other auras' procs
    SPELL_ATTR3_REQUIRES_MAIN_HAND_WEAPON =
        0x00000400, // TITLE Require main hand weapon
    SPELL_ATTR3_ONLY_BATTLEGROUNDS =
        0x00000800, // TITLE Can only be cast in battleground
    SPELL_ATTR3_ONLY_ON_GHOSTS =
        0x00001000, // TITLE Can only target ghost players
    SPELL_ATTR3_HIDE_CHANNEL_BAR =
        0x00002000, // TITLE Do not display channel bar (client only)
    SPELL_ATTR3_HIDE_IN_RAID_FILTER = 0x00004000, // TITLE Honorless Target
    SPELL_ATTR3_NORMAL_RANGED_ATTACK =
        0x00008000, // TITLE Unknown attribute 15@Attr3 DESCRIPTION Auto Shoot,
                    // Shoot, Throw - ranged normal attack attribute?
    ...
    SPELL_ATTR3_ALWAYS_HIT =
        0x00040000, // TITLE Ignore hit result DESCRIPTION Spell cannot miss, or
                    // be dodged/parried/blocked
    SPELL_ATTR3_INSTANT_TARGET_PROCS =
        0x00080000, // TITLE Cannot trigger spells during aura proc
    SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD =
        0x00100000, // TITLE Persists through death
    SPELL_ATTR3_ONLY_PROC_OUTDOORS =
        0x00200000, // TITLE Unknown attribute 21@Attr3
    SPELL_ATTR3_CASTING_CANCELS_AUTOREPEAT =
        0x00400000, // TITLE Requires equipped Wand (Mainline: Do Not Trigger
                    // Target Stand)
    SPELL_ATTR3_NO_DAMAGE_HISTORY =
        0x00800000, // TITLE Unknown attribute 23@Attr3
    SPELL_ATTR3_REQUIRES_OFF_HAND_WEAPON =
        0x01000000, // TITLE Requires offhand weapon
    SPELL_ATTR3_TREAT_AS_PERIODIC =
        0x02000000, // TITLE Treat as periodic effect
    SPELL_ATTR3_CAN_PROC_FROM_PROCS =
        0x04000000, // TITLE Can trigger from triggered spells
    SPELL_ATTR3_ONLY_PROC_ON_CASTER = 0x08000000, // TITLE Drain Soul
    SPELL_ATTR3_IGNORE_CASTER_AND_TARGET_RESTRICTIONS =
        0x10000000, // TITLE Unknown attribute 28@Attr3
    SPELL_ATTR3_IGNORE_CASTER_MODIFIERS =
        0x20000000, // TITLE Damage dealt is unaffected by modifiers
    SPELL_ATTR3_DO_NOT_DISPLAY_RANGE =
        0x40000000, // TITLE Do not show range in tooltip (client only)
    SPELL_ATTR3_NOT_ON_AOE_IMMUNE =
        0x80000000 // TITLE Unknown attribute 31@Attr3
};
...
enum SpellAttr4 : uint32 {
    ...
    SPELL_ATTR4_CLASS_TRIGGER_ONLY_ON_TARGET =
        0x00000002, // TITLE Only proc on self-cast
    SPELL_ATTR4_AURA_EXPIRES_OFFLINE =
        0x00000004, // TITLE Buff expires while offline DESCRIPTION Debuffs
                    // (except Resurrection Sickness) will automatically do this
    SPELL_ATTR4_NO_HELPFUL_THREAT =
        0x00000008, // TITLE Unknown attribute 3@Attr4
    SPELL_ATTR4_NO_HARMFUL_THREAT =
        0x00000010, // TITLE Treat as delayed spell DESCRIPTION This will no
                    // longer cause guards to attack on use??
    SPELL_ATTR4_ALLOW_CLIENT_TARGETING =
        0x00000020, // TITLE Unknown attribute 5@Attr4
    SPELL_ATTR4_CANNOT_BE_STOLEN = 0x00000040, // TITLE Aura cannot be stolen
    SPELL_ATTR4_ALLOW_CAST_WHILE_CASTING =
        0x00000080, // TITLE Can be cast while casting DESCRIPTION Ignores
                    // already in-progress cast and still casts
    SPELL_ATTR4_IGNORE_DAMAGE_TAKEN_MODIFIERS =
        0x00000100, // TITLE Deals fixed damage
    SPELL_ATTR4_COMBAT_FEEDBACK_WHEN_USABLE =
        0x00000200, // TITLE Spell is initially disabled (client only)
    SPELL_ATTR4_WEAPON_SPEED_COST_SCALING =
        0x00000400, // TITLE Attack speed modifies cost DESCRIPTION Adds 10 to
                    // power cost for each 1s of weapon speed
    SPELL_ATTR4_NO_PARTIAL_IMMUNITY =
        0x00000800,                        // TITLE Unknown attribute 11@Attr4
    SPELL_ATTR4_AURA_IS_BUFF = 0x00001000, // TITLE Unknown attribute 12@Attr4
    SPELL_ATTR4_DO_NOT_LOG_CASTER =
        0x00002000, // TITLE Unknown attribute 13@Attr4
    SPELL_ATTR4_REACTIVE_DAMAGE_PROC =
        0x00004000, // TITLE Damage does not break auras
    SPELL_ATTR4_NOT_IN_SPELLBOOK =
        0x00008000, // TITLE Unknown attribute 15@Attr4
    SPELL_ATTR4_NOT_IN_ARENA_OR_RATED_BATTLEGROUND =
        0x00010000, // TITLE Not usable in arena DESCRIPTION Makes spell
                    // unusable despite CD <= 10min
    SPELL_ATTR4_IGNORE_DEFAULT_ARENA_RESTRICTIONS =
        0x00020000, // TITLE Usable in arena DESCRIPTION Makes spell usable
                    // despite CD > 10min
    SPELL_ATTR4_BOUNCY_CHAIN_MISSILES =
        0x00040000, // TITLE Chain area targets DESCRIPTION [NYI] Hits area
                    // targets over time instead of all at once
    SPELL_ATTR4_ALLOW_PROC_WHILE_SITTING =
        0x00080000, // TITLE Unknown attribute 19@Attr4 DESCRIPTION proc
                    // dalayed, after damage or don't proc on absorb?
    ...
    SPELL_ATTR4_ALLOW_ENETRING_ARENA =
        0x00200000, // TITLE Keep when entering arena
    SPELL_ATTR4_PROC_SUPPRESS_SWING_ANIM =
        0x00400000, // TITLE Unknown attribute 22@Attr4 DESCRIPTION Seal of
                    // Command (42058,57770) and Gymer's Smash 55426
    SPELL_ATTR4_SUPRESS_WEAPON_PROCS =
        0x00800000, // TITLE Cannot trigger item spells
    SPELL_ATTR4_AUTO_RANGED_COMBAT =
        0x01000000, // TITLE Unknown attribute 24@Attr4 DESCRIPTION Shoot-type
                    // spell?
    SPELL_ATTR4_OWNER_POWER_SCALING = 0x02000000, // TITLE Pet Scaling aura
    SPELL_ATTR4_ONLY_FLYING_AREAS =
        0x04000000, // TITLE Only in Outland/Northrend
    SPELL_ATTR4_FORCE_DISPLAY_CASTBAR =
        0x08000000, // TITLE Inherit critical chance from triggering aura
    SPELL_ATTR4_IGNORE_COMBAT_TIMERS =
        0x10000000, // TITLE Unknown attribute 28@Attr4 DESCRIPTION Aimed Shot
    SPELL_ATTR4_AURA_BOUNCE_FAILS_SPELL =
        0x20000000,                    // TITLE Unknown attribute 29@Attr4
    SPELL_ATTR4_OBSOLETE = 0x40000000, // TITLE Unknown attribute 30@Attr4
    SPELL_ATTR4_USE_FACING_FROM_SPELL =
        0x80000000 // TITLE Unknown attribute 31@Attr4 DESCRIPTION Polymorph
                   // (chicken) 228 and Sonic Boom (38052,38488)
};

// EnumUtils: DESCRIBE THIS
enum SpellAttr5 : uint32 {
    SPELL_ATTR5_ALLOW_ACTION_DURING_CHANNEL =
        0x00000001, // TITLE Can be channeled while moving/casting
    SPELL_ATTR5_NO_REAGENT_COST_WITH_AURA =
        0x00000002, // TITLE No reagents during arena preparation
    SPELL_ATTR5_REMOVE_ENTERING_ARENA =
        0x00000004, // TITLE Remove when entering arena DESCRIPTION Force this
                    // aura to be removed on entering arena, regardless of other
                    // properties
    SPELL_ATTR5_ALLOW_WHILE_STUNNED = 0x00000008, // TITLE Usable while stunned
    SPELL_ATTR5_TRIGGERS_CHANNELING =
        0x00000010, // TITLE Unknown attribute 4@Attr5
    SPELL_ATTR5_LIMIT_N =
        0x00000020, // TITLE Single-target aura DESCRIPTION Remove previous
                    // application to another unit if applied
    SPELL_ATTR5_IGNORE_AREA_EFFECT_PVP_CHECK =
        0x00000040,                         // TITLE Unknown attribute 6@Attr5
    SPELL_ATTR5_NOT_ON_PLAYER = 0x00000080, // TITLE Unknown attribute 7@Attr5
    SPELL_ATTR5_NOT_ON_PLAYER_CONTROLLED_NPC =
        0x00000100, // TITLE Cannot target player controlled units but can
                    // target players
    SPELL_ATTR5_EXTRA_INITIAL_PERIOD =
        0x00000200, // TITLE Immediately do periodic tick on apply
    SPELL_ATTR5_DO_NOT_DISPLAY_DURATION =
        0x00000400, // TITLE Do not send aura duration to client
    SPELL_ATTR5_IMPLIED_TARGETING =
        0x00000800, // TITLE Auto-target target of target (client only)
    SPELL_ATTR5_MELEE_CHAIN_TARGETING =
        0x00001000, // TITLE Unknown attribute 12@Attr5 DESCRIPTION Cleave
                    // related?
    SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC =
        0x00002000, // TITLE Duration scales with Haste Rating
    SPELL_ATTR5_NOT_AVALIABLE_WHILE_CHARMED =
        0x00004000, // TITLE Charmed units cannot cast this spell
    SPELL_ATTR5_TREAT_AS_AREA_EFFECT =
        0x00008000, // TITLE Unknown attribute 15@Attr5 DESCRIPTION Related to
                    // multi-target spells?
    SPELL_ATTR5_AURA_AFFECTS_NOT_JUST_REQ_EQUIPED_ITEM =
        0x00010000, // TITLE DESCRIPTION this allows spells with
                    // EquippedItemClass to affect spells from other items if
                    // the required item is equipped
    SPELL_ATTR5_ALLOW_WHILE_FLEEING = 0x00020000, // TITLE Usable while feared
    SPELL_ATTR5_ALLOW_WHILE_CONFUSED =
        0x00040000, // TITLE Usable while confused
    SPELL_ATTR5_AI_DOESNT_FACE_TARGET =
        0x00080000, // TITLE Do not auto-turn while casting
    SPELL_ATTR5_DO_NOT_ATTEMPT_A_PET_RESUMMON_WHEN_DISMOUNTING =
        0x00100000, // TITLE Unknown attribute 20@Attr5
    SPELL_ATTR5_IGNORE_TARGET_REQUIREMENTS =
        0x00200000,                          // TITLE Unknown attribute 21@Attr5
    SPELL_ATTR5_NOT_ON_TRIVIAL = 0x00400000, // TITLE Unknown attribute 22@Attr5
    SPELL_ATTR5_NO_PARTIAL_RESISTS =
        0x00800000, // TITLE Unknown attribute 23@Attr5
    SPELL_ATTR5_IGNORE_CASTER_REQUIREMENETS =
        0x01000000, // TITLE Unknown attribute 24@Attr5
    SPELL_ATTR5_ALWAYS_LINE_OF_SIGHT =
        0x02000000, // TITLE Unknown attribute 25@Attr5
    SPELL_ATTR5_ALWAYS_AOE_LINE_OF_SIGHT =
        0x04000000, // TITLE Ignore line of sight checks
    SPELL_ATTR5_NO_CASTER_AURA_ICON =
        0x08000000, // TITLE Don't show aura if self-cast (client only)
    SPELL_ATTR5_NO_TARGET_AURA_ICON =
        0x10000000, // TITLE Don't show aura unless self-cast (client only)
    SPELL_ATTR5_AURA_UNIQUE_PER_CASTER =
        0x20000000, // TITLE Unknown attribute 29@Attr5
    SPELL_ATTR5_ALWAYS_SHOW_GROUND_TEXTURE =
        0x40000000, // TITLE Unknown attribute 30@Attr5
    SPELL_ATTR5_ADD_MELEE_HIT_RATING =
        0x80000000 // TITLE Unknown attribute 31@Attr5 DESCRIPTION Forces nearby
                   // enemies to attack caster?
};

// EnumUtils: DESCRIBE THIS
enum SpellAttr6 : uint32 {
    SPELL_ATTR6_NO_COOLDOWN_ON_TOOLTIP =
        0x00000001, // TITLE Don't display cooldown (client only)
    SPELL_ATTR6_DO_NOT_RESET_COOLDOWN_IN_ARENA =
        0x00000002, // TITLE Only usable in arena
    SPELL_ATTR6_NOT_AN_ATTACK =
        0x00000004, // TITLE Ignore all preventing caster auras
    SPELL_ATTR6_CAN_ASSIST_IMMUNE_PC =
        0x00000008, // TITLE Ignore immunity flags when assisting
    SPELL_ATTR6_IGNORE_FOR_MOD_TIME_RATE =
        0x00000010, // TITLE Unknown attribute 4@Attr6
    SPELL_ATTR6_DO_NOT_CONSUME_RESOURCES =
        0x00000020, // TITLE Don't consume proc charges
    SPELL_ATTR6_FLOATING_COMBAT_TEXT_ON_CAST =
        0x00000040, // TITLE Generate spell_cast event instead of aura_start
                    // (client only)
    SPELL_ATTR6_AURA_IS_WEAPON_PROC =
        0x00000080, // TITLE Unknown attribute 7@Attr6
    SPELL_ATTR6_DO_NOT_CHAIN_TO_CROWD_CONTROLLED_TARGETS =
        0x00000100, // TITLE Do not implicitly target in CC DESCRIPTION Implicit
                    // targeting (chaining and area targeting) will not impact
                    // crowd controlled targets
    SPELL_ATTR6_ALLOW_ON_CHARMED_TARGETS =
        0x00000200, // TITLE Unknown attribute 9@Attr6
    SPELL_ATTR6_NO_AURA_LOG =
        0x00000400, // TITLE Can target possessed friends DESCRIPTION [NYI]
    SPELL_ATTR6_NOT_IN_RAID_INSTANCES =
        0x00000800, // TITLE Unusable in raid instances
    SPELL_ATTR6_ALLOW_WHILE_RIDING_VEHICLE =
        0x00001000, // TITLE Castable while caster is on vehicle
    SPELL_ATTR6_IGNORE_PHASE_SHIFT =
        0x00002000, // TITLE Can target invisible units
    SPELL_ATTR6_AI_PRIMARY_RANGED_ATTACK =
        0x00004000,                       // TITLE Unknown attribute 14@Attr6
    SPELL_ATTR6_NO_PUSHBACK = 0x00008000, // TITLE Unknown attribute 15@Attr6
                                          // DESCRIPTION only 54368, 67892
    SPELL_ATTR6_NO_JUMP_PATHING =
        0x00010000, // TITLE Unknown attribute 16@Attr6
    SPELL_ATTR6_ALLOW_EQUIP_WHILE_CASTING =
        0x00020000, // TITLE Unknown attribute 17@Attr6 DESCRIPTION Mount
                    // related?
    SPELL_ATTR6_ORIGINATE_FROM_CONTROLLER =
        0x00040000, // TITLE Spell is cast by charmer DESCRIPTION Client will
                    // prevent casting if not possessed, charmer will be caster
                    // for all intents and purposes
    SPELL_ATTR6_DELAY_COMBAT_TIMER_DURING_CAST =
        0x00080000, // TITLE Unknown attribute 19@Attr6 DESCRIPTION only 47488,
                    // 50782
    SPELL_ATTR6_AURA_ICON_ONLY_FOR_CASTER =
        0x00100000, // TITLE Only visible to caster (client only) (LIMIT 10)
    SPELL_ATTR6_SHOW_MECHANIC_AS_COMBAT_TEXT =
        0x00200000, // TITLE Client UI target effects (client only)
    SPELL_ATTR6_ABSORB_CANNOT_BE_IGNORED =
        0x00400000, // TITLE Unknown attribute 22@Attr6 DESCRIPTION only 72054
    SPELL_ATTR6_TAPS_IMMEDIATELY =
        0x00800000, // TITLE Unknown attribute 23@Attr6
    SPELL_ATTR6_CAN_TARGET_UNTARGETABLE =
        0x01000000, // TITLE Can target untargetable units
    SPELL_ATTR6_DOESNT_RESET_SWING_TIMER_IF_INSTANT =
        0x02000000, // TITLE Do not reset swing timer if cast time is instant
    SPELL_ATTR6_VEHICLE_IMMUNITY_CATEGORY =
        0x04000000, // TITLE Unknown attribute 26@Attr6 DESCRIPTION Player
                    // castable buff?
    SPELL_ATTR6_IGNORE_HEALTH_MODIFIERS =
        0x08000000, // TITLE Limit applicable %healing modifiers DESCRIPTION
                    // This prevents certain healing modifiers from applying -
                    // see implementation if you really care about details
    SPELL_ATTR6_DO_NOT_SELECT_TARGET_WITH_INITIATES_COMBAT =
        0x10000000, // TITLE Unknown attribute 28@Attr6 DESCRIPTION Death grip?
    SPELL_ATTR6_IGNORE_CASTER_DAMAGE_MODIFIERS =
        0x20000000, // TITLE Limit applicable %damage modifiers DESCRIPTION This
                    // prevents certain damage modifiers from applying - see
                    // implementation if you really care about details
    SPELL_ATTR6_DISABLE_TIED_EFFECT_POINTS =
        0x40000000, // TITLE Unknown attribute 30@Attr6
    SPELL_ATTR6_NO_CATEGORY_COOLDOWN_MODS =
        0x80000000 // TITLE Ignore cooldown modifiers for category cooldown
};

// EnumUtils: DESCRIBE THIS
enum SpellAttr7 : uint32 {
    SPELL_ATTR7_ALLOW_SPELL_REFLECTION =
        0x00000001, // TITLE Unknown attribute 0@Attr7 DESCRIPTION Shaman's new
                    // spells (Call of the ...), Feign Death.
    SPELL_ATTR7_NO_TARGET_DURATION_MOD =
        0x00000002, // TITLE Ignore duration modifiers
    SPELL_ATTR7_DISABLE_AURA_WHILE_DEAD =
        0x00000004, // TITLE Reactivate at resurrect (client only)
    SPELL_ATTR7_DEBUG_SPELL =
        0x00000008, // TITLE Is cheat spell DESCRIPTION Cannot cast if caster
                    // doesn't have UnitFlag2 & UNIT_FLAG2_ALLOW_CHEAT_SPELLS
    SPELL_ATTR7_TREAT_AS_RAID_BUFF =
        0x00000010, // TITLE Unknown attribute 4@Attr7 DESCRIPTION Soulstone
                    // related?
    SPELL_ATTR7_CAN_BE_MULTI_CAST =
        0x00000020, // TITLE Summons player-owned totem
    SPELL_ATTR7_DONT_CAUSE_SPELL_PUSHBACK =
        0x00000040, // TITLE Damage dealt by this does not cause spell pushback
    SPELL_ATTR7_PREPARE_FOR_VEHICLE_CONTROL_END =
        0x00000080, // TITLE Unknown attribute 7@Attr7 DESCRIPTION 66218
                    // (Launch) spell.
    ...
    SPELL_ATTR7_DISPEL_REMOVES_CHARGES =
        0x00000400, // TITLE Dispel/Spellsteal remove individual charges
    SPELL_ATTR7_CAN_CAUSE_INTERRUPT =
        0x00000800, // TITLE Only interrupt non-player casting
    SPELL_ATTR7_CAN_CAUSE_SILENCE =
        0x00001000, // TITLE Unknown attribute 12@Attr7 DESCRIPTION Not set
                    // in 3.2.2a.
    SPELL_ATTR7_NO_UI_NOT_INTERRUPTIBLE =
        0x00002000, // TITLE Unknown attribute 13@Attr7 DESCRIPTION Not set
                    // in 3.2.2a.
    SPELL_ATTR7_RECAST_ON_RESUMMON =
        0x00004000, // TITLE Unknown attribute 14@Attr7 DESCRIPTION Only 52150
                    // (Raise Dead - Pet) spell.
    SPELL_ATTR7_RESET_SWING_TIMER_AT_SPELL_START =
        0x00008000, // TITLE Unknown attribute 15@Attr7 DESCRIPTION Exorcism -
                    // guaranteed crit vs families?
    SPELL_ATTR7_ONLY_IN_SPELLBOOK_UNTIL_LEARNED =
        0x00010000, // TITLE Can restore secondary power DESCRIPTION Only spells
                    // with this attribute can replenish a non-active power type
    SPELL_ATTR7_DO_NOT_LOG_PVP_KILL =
        0x00020000, // TITLE Unknown attribute 17@Attr7 DESCRIPTION Only 27965
                    // (Suicide) spell.
    SPELL_ATTR7_ATTACK_ON_CHARGE_TO_UNIT =
        0x00040000, // TITLE Has charge effect
    SPELL_ATTR7_REPORT_SPELL_FAILURE_TO_UNIT_TARGET =
        0x00080000, // TITLE Is zone teleport
    SPELL_ATTR7_NO_CLIENT_FAIL_WHILE_STUNNED_FLEEING_CONFUSED =
        0x00100000, // TITLE Unknown attribute 20@Attr7 DESCRIPTION
                    // Invulnerability related?
    SPELL_ATTR7_RETAIN_COOLDOWN_THROUGH_LOAD =
        0x00200000, // TITLE Unknown attribute 21@Attr7
    SPELL_ATTR7_IGNORES_COLD_WEATHER_FLYING_REQUIREMENT =
        0x00400000, // TITLE Ignore cold weather flying restriction DESCRIPTION
                    // Set for loaner mounts, allows them to be used despite
                    // lacking required flight skill
    SPELL_ATTR7_NO_ATTACK_DODGE =
        0x00800000, // TITLE Spell cannot be dodged 23@Attr7 DESCRIPTION
                    // Motivate, Mutilate, Shattering Throw
    SPELL_ATTR7_NO_ATTACK_PARRY =
        0x01000000, // TITLE Spell cannot be parried 24@Attr7 DESCRIPTION
                    // Motivate, Mutilate, Perform Speech, Shattering Throw
    SPELL_ATTR7_NO_ATTACK_MISS =
        0x02000000, // TITLE Spell cannot be missed 25@Attr7
    SPELL_ATTR7_TREAT_AS_NPC_AOE =
        0x04000000, // TITLE Unknown attribute 26@Attr7
    SPELL_ATTR7_BYPASS_NO_RESURRECTION_AURA =
        0x08000000, // TITLE Bypasses the prevent resurrection aura
    SPELL_ATTR7_DO_NOT_COUNT_FOR_PVP_SCOREBOARD =
        0x10000000, // TITLE Consolidate in raid buff frame (client only)
    SPELL_ATTR7_REFLECTION_ONLY_DEFENDS =
        0x20000000, // TITLE Unknown attribute 29@Attr7 DESCRIPTION only 69028,
                    // 71237
    SPELL_ATTR7_CAN_PROC_FROM_SUPPRESSED_TARGET_PROCS =
        0x40000000, // TITLE Unknown attribute 30@Attr7 DESCRIPTION Burning
                    // Determination, Divine Sacrifice, Earth Shield, Prayer of
                    // Mending
    SPELL_ATTR7_ALWAYS_CAST_LOG =
        0x80000000 // TITLE Client indicator (client only)
};
...
enum Language {
    ...
    LANG_ADDON = 0xFFFFFFFF // used by addons, in 2.4.0 not exist, replaced by
                            // messagetype?
};
...
enum Team {
    ...
    // TEAM_OUTLAND             = 980,
    TEAM_OTHER =
        0, // if ReputationListId > 0 && Flags != FACTION_FLAG_TEAM_HEADER
};
...
enum SpellCustomErrors {
    ...
    SPELL_CUSTOM_ERROR_CUSTOM_MSG =
        1, // Something bad happened, and we want to display a custom message!
    SPELL_CUSTOM_ERROR_ALEX_BROKE_QUEST =
        2, // Alex broke your quest! Thank him later!
    SPELL_CUSTOM_ERROR_NEED_HELPLESS_VILLAGER =
        3, // This spell may only be used on Helpless Wintergarde Villagers that
           // have not been rescued.
    SPELL_CUSTOM_ERROR_NEED_WARSONG_DISGUISE =
        4, // Requires that you be wearing the Warsong Orc Disguise.
    SPELL_CUSTOM_ERROR_REQUIRES_PLAGUE_WAGON =
        5, // You must be closer to a plague wagon in order to drop off your 7th
           // Legion Siege Engineer.
    SPELL_CUSTOM_ERROR_CANT_TARGET_FRIENDLY_NONPARTY =
        6, // You cannot target friendly units outside your party.
    SPELL_CUSTOM_ERROR_NEED_CHILL_NYMPH =
        7, // You must target a weakened chill nymph.
    SPELL_CUSTOM_ERROR_MUST_BE_IN_ENKILAH =
        8, // The Imbued Scourge Shroud will only work when equipped in the
    ...
    SPELL_CUSTOM_ERROR_CANT_SUMMON_GARGOYLE =
        10, // You cannot summon another gargoyle yet.
    SPELL_CUSTOM_ERROR_NEED_CORPSE_DUST_IF_NO_TARGET =
        11, // Requires Corpse Dust if the target is not dead and humanoid.
    ...
    SPELL_CUSTOM_ERROR_MUST_TARGET_PROTO_DRAKE_EGG =
        13, // You must first select a Proto-Drake Egg.
    SPELL_CUSTOM_ERROR_MUST_BE_CLOSE_TO_TREE =
        14, // You must be close to a marked tree.
    SPELL_CUSTOM_ERROR_MUST_TARGET_TURKEY =
        15, // You must target a Fjord Turkey.
    ...
    SPELL_CUSTOM_ERROR_MUST_BE_CLOSE_TO_OIL_SLICK =
        18, // Must be used near an oil slick.
    SPELL_CUSTOM_ERROR_MUST_BE_CLOSE_TO_BOUY =
        19, // You must be closer to the buoy!
    SPELL_CUSTOM_ERROR_WYRMREST_VANQUISHER =
        20, // You may only call for the aid of a Wyrmrest Vanquisher in
            // Wyrmrest Temple, The Dragon Wastes, Galakrond's Rest or The
            // Wicked Coil.
    SPELL_CUSTOM_ERROR_MUST_TARGET_ICE_HEART_JORMUNGAR =
        21, // That can only be used on a Ice Heart Jormungar Spawn.
    SPELL_CUSTOM_ERROR_MUST_BE_CLOSE_TO_SINKHOLE =
        22, // You must be closer to a sinkhole to use your map.
    SPELL_CUSTOM_ERROR_REQUIRES_HAROLD_LANE =
        23, // You may only call down a stampede on Harold Lane.
    SPELL_CUSTOM_ERROR_REQUIRES_GAMMOTH_MAGNATAUR =
        24, // You may only use the Pouch of Crushed Bloodspore on Gammothra or
            // other magnataur in the Bloodspore Plains and Gammoth.
    SPELL_CUSTOM_ERROR_MUST_BE_IN_RESURRECTION_CHAMBER =
        25, // Requires the magmawyrm resurrection chamber in the back of the
            // Maw of Neltharion.
    SPELL_CUSTOM_ERROR_CANT_CALL_WINTERGARDE_HERE =
        26, // You may only call down a Wintergarde Gryphon in Wintergarde Keep
            // or the Carrion Fields.
    SPELL_CUSTOM_ERROR_MUST_TARGET_WILHELM =
        27, // What are you doing? Only aim that thing at Wilhelm!
    SPELL_CUSTOM_ERROR_NOT_ENOUGH_HEALTH = 28, // Not enough health!
    SPELL_CUSTOM_ERROR_NO_NEARBY_CORPSES =
        29, // There are no nearby corpses to use
    SPELL_CUSTOM_ERROR_TOO_MANY_GHOULS =
        30, // You've created enough ghouls. Return to Gothik the Harvester at
            // Death's Breach.
    SPELL_CUSTOM_ERROR_GO_FURTHER_FROM_SUNDERED_SHARD =
        31, // Your companion does not want to come here.  Go further from the
            // Sundered Shard.
    SPELL_CUSTOM_ERROR_MUST_BE_IN_CAT_FORM = 32, // Must be in Cat Form
    SPELL_CUSTOM_ERROR_MUST_BE_DEATH_KNIGHT =
        33, // Only Death Knights may enter Ebon Hold.
    SPELL_CUSTOM_ERROR_MUST_BE_IN_FERAL_FORM =
        34, // Must be in Cat Form, Bear Form, or Dire Bear Form
    SPELL_CUSTOM_ERROR_MUST_BE_NEAR_HELPLESS_VILLAGER =
        35, // You must be within range of a Helpless Wintergarde Villager.
    SPELL_CUSTOM_ERROR_CANT_TARGET_ELEMENTAL_MECHANICAL =
        36, // You cannot target an elemental or mechanical corpse.
    SPELL_CUSTOM_ERROR_MUST_HAVE_USED_DALARAN_CRYSTAL =
        37, // This teleport crystal cannot be used until the teleport crystal
            // in Dalaran has been used at least once.
    SPELL_CUSTOM_ERROR_YOU_ALREADY_HOLD_SOMETHING =
        38, // You are already holding something in your hand. You must throw
            // the creature in your hand before picking up another.
    SPELL_CUSTOM_ERROR_YOU_DONT_HOLD_ANYTHING =
        39, // You don't have anything to throw! Find a Vargul and use Gymer
            // Grab to pick one up!
    SPELL_CUSTOM_ERROR_MUST_BE_CLOSE_TO_VALDURAN =
        40, // Bouldercrag's War Horn can only be used within 10 yards of
            // Valduran the Stormborn.
    SPELL_CUSTOM_ERROR_NO_PASSENGER =
        41, // You are not carrying a passenger. There is nobody to drop off.
    SPELL_CUSTOM_ERROR_CANT_BUILD_MORE_VEHICLES =
        42, // You cannot build any more siege vehicles.
    SPELL_CUSTOM_ERROR_ALREADY_CARRYING_CRUSADER =
        43, // You are already carrying a captured Argent Crusader. You must
            // return to the Argent Vanguard infirmary and drop off your
            // passenger before you may pick up another.
    SPELL_CUSTOM_ERROR_CANT_DO_WHILE_ROOTED =
        44, // You can't do that while rooted.
    ...
    SPELL_CUSTOM_ERROR_NOT_ENOUGH_TARGETS =
        47, // No targets close enough to bluff.
    SPELL_CUSTOM_ERROR_CONSTRUCT_TOO_FAR =
        48, // Your Iron Rune Construct is out of range.
    SPELL_CUSTOM_ERROR_REQUIRES_GRAND_MASTER_ENGINEER =
        49, // Requires Grand Master Engineer
    ...
    SPELL_CUSTOM_ERROR_MAX_NUMBER_OF_RECRUITS =
        96, // You already have the max number of recruits.
    SPELL_CUSTOM_ERROR_MAX_NUMBER_OF_VOLUNTEERS =
        97, // You already have the max number of volunteers.
    ...
    SPELL_CUSTOM_ERROR_CANT_MOUNT_WITH_SHAPESHIFT =
        99, // You can't mount while affected by that shapeshift.
};
...
enum AuraStateType {
    ...
    AURA_STATE_HUNTER_PARRY =
        7, // C   |
           // AURA_STATE_UNKNOWN7                   = 7,            //  c  |
           // creature cheap shot / focused bursts spells AURA_STATE_UNKNOWN8 =
           // 8,            //    t| test spells AURA_STATE_UNKNOWN9 = 9, // |
    AURA_STATE_WARRIOR_VICTORY_RUSH =
        10, // C   | warrior victory rush
            // AURA_STATE_UNKNOWN11                  = 11,           // C  t|
            // 60348 - Maelstrom Ready!, test spells
    ...
    AURA_STATE_UNKNOWN19             = 19, //     |
                               ...
                               // c  | only (45317 Suicide) AURA_STATE_UNKNOWN21
                               // = 21,           //     | not used
...
// Used for spell 42292 Immune Movement Impairment and Loss of Control
// (0x49967ca6)
#define IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK                    \
    ((1 << MECHANIC_CHARM) | (1 << MECHANIC_DISORIENTED) |                     \
     (1 << MECHANIC_FEAR) | (1 << MECHANIC_ROOT) | (1 << MECHANIC_SLEEP) |     \
     (1 << MECHANIC_SNARE) | (1 << MECHANIC_STUN) | (1 << MECHANIC_FREEZE) |   \
     (1 << MECHANIC_KNOCKOUT) | (1 << MECHANIC_POLYMORPH) |                    \
...
#define DISPEL_ALL_MASK                                                        \
    ((1 << DISPEL_MAGIC) | (1 << DISPEL_CURSE) | (1 << DISPEL_DISEASE) |       \
     (1 << DISPEL_POISON))

// To all Immune system, if target has immunes,
// some spell that related to ImmuneToDispel or ImmuneToSchool or ImmuneToDamage
// type can't cast to it, some spell_effects that related to
// ImmuneToEffect<effect>(only this effect in the spell) can't cast to it, some
// aura(related to Mechanics or ImmuneToState<aura>) can't apply to it.
enum SpellImmunity {
    ...
    IMMUNITY_ID       = 6,
    IMMUNITY_ALLOW_ID = 7 // xinef: special type of immunity, npc is immune to
                          // everything except for those spells id
};

#define MAX_SPELL_IMMUNITY 8

// target enum name consist of:
// TARGET_[OBJECT_TYPE]_[REFERENCE_TYPE(skipped for
// caster)]_[SELECTION_TYPE(skipped for default)]_[additional
...
enum Targets {
    ...
    TARGET_UNK_DEST_AREA_UNK_107 =
        107, // not enough info - only generic spells avalible
...
#define MAX_GAMEOBJECT_TYPE                                                    \
    36 // sending to client this or greater value can crash client.
#define MAX_GAMEOBJECT_DATA                                                    \
    24 // Max number of uint32 vars in gameobject_template data field

enum GameObjectFlags : uint32 {
    GO_FLAG_IN_USE = 0x00000001, // disables interaction while animated
    GO_FLAG_LOCKED = 0x00000002, // require key, spell, event, etc to be opened.
                                 // Makes "Locked" appear in tooltip
    GO_FLAG_INTERACT_COND =
        0x00000004,                 // cannot interact (condition to interact)
    GO_FLAG_TRANSPORT = 0x00000008, // any kind of transport? Object can
                                    // transport (elevator, boat, car)
    GO_FLAG_NOT_SELECTABLE = 0x00000010, // not selectable even in GM mode
    GO_FLAG_NODESPAWN = 0x00000020, // never despawn, typically for doors, they
                                    // just change state
    GO_FLAG_TRIGGERED = 0x00000040, // typically, summoned objects. Triggered by
                                    // spell or other events
...
enum GameObjectDynamicLowFlags {
    ...
    GO_DYNFLAG_LO_NO_INTERACT =
        0x04, // appears to disable interaction (not fully verified)
...
uint32 const CREATURE_TYPEMASK_DEMON_OR_UNDEAD =
    (1 << (CREATURE_TYPE_DEMON - 1)) | (1 << (CREATURE_TYPE_UNDEAD - 1));
uint32 const CREATURE_TYPEMASK_HUMANOID_OR_UNDEAD =
    (1 << (CREATURE_TYPE_HUMANOID - 1)) | (1 << (CREATURE_TYPE_UNDEAD - 1));
uint32 const CREATURE_TYPEMASK_MECHANICAL_OR_ELEMENTAL =
    (1 << (CREATURE_TYPE_MECHANICAL - 1)) |
    (1 << (CREATURE_TYPE_ELEMENTAL - 1));
...
enum CreatureTypeFlags {
    CREATURE_TYPE_FLAG_TAMEABLE =
        0x00000001, // Makes the mob tameable (must also be a beast and have
                    // family set)
    CREATURE_TYPE_FLAG_VISIBLE_TO_GHOSTS =
        0x00000002, // Creature is also visible for not alive player. Allows
                    // gossip interaction if npcflag allows?
    CREATURE_TYPE_FLAG_BOSS_MOB =
        0x00000004, // Changes creature's visible level to "??" in the
                    // creature's portrait - Immune Knockback.
    CREATURE_TYPE_FLAG_DO_NOT_PLAY_WOUND_ANIM =
        0x00000008, // Does not play wound animation on parry
    ...
    CREATURE_TYPE_FLAG_COLLIDE_WITH_MISSILES =
        0x00080000, // Projectiles can collide with this creature - interacts
                    // with TARGET_DEST_TRAJ
    ...
    CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT =
        0x04000000, // ! Creature can be targeted by spells that require target
                    // to be in caster's party/raid
    CREATURE_TYPE_FLAG_FORCE_GOSSIP =
        0x08000000, // Allows the creature to display a single gossip option.
...
// Values from ItemPetFood (power of (value-1) used for compare with
// CreatureFamilyEntry.petDietMask
enum PetDiet {
    PET_DIET_MEAT     = 1,
    PET_DIET_FISH     = 2,
    PET_DIET_CHEESE   = 3,
...
enum AiReaction {
    ...
    AI_REACTION_HOSTILE = 2, // sent on every attack, triggers aggro sound (used
                             // in client packet handler)
    AI_REACTION_AFRAID = 3,  // seen for polymorph (when AI not in control of
                             // self?) (NOT used in client packet handler)
    AI_REACTION_DESTROY =
        4, // used on object destroy (NOT used in client packet handler)
};

// Diminishing Returns Types
enum DiminishingReturnsType {
    DRTYPE_NONE = 0, // this spell is not diminished, but may have limited it's
                     // duration to 10s
...
enum SummonCategory {
    ...
    SUMMON_CATEGORY_UNK =
        5, // as of patch 3.3.5a only Bone Spike in Icecrown Citadel
    // uses this category
};
...
enum EventId {
    ...
    /// Special charge event which is used for charge spells that have explicit
    /// targets and had a path already generated - using it in
    /// PointMovementGenerator will not create a new spline and launch it
...
enum XPColorChar : uint8 { XP_RED, XP_ORANGE, XP_YELLOW, XP_GREEN, XP_GRAY };
...
// handle the queue types and bg types separately to enable joining queue for
// different sized arenas at the same time
...
enum GroupJoinBattlegroundResult {
    ...
    ERR_GROUP_JOIN_BATTLEGROUND_FAIL =
        0, // Your group has joined a battleground queue, but you are not
           // eligible (showed for non existing BattlemasterList.dbc indexes)
    ERR_BATTLEGROUND_NONE = -1, // not show anything
    ERR_GROUP_JOIN_BATTLEGROUND_DESERTERS =
        -2, // You cannot join the battleground yet because you or one of your
            // party members is flagged as a Deserter.
    ...
    ERR_BATTLEGROUND_QUEUED_FOR_RATED =
        -6, // You cannot queue for another battle while queued for a rated
            // arena match
    ...
    ERR_BATTLEGROUND_JOIN_RANGE_INDEX =
        -10, // Cannot join the queue unless all members of your party are in
             // the same battleground level range.
    ERR_BATTLEGROUND_JOIN_TIMED_OUT =
        -11, // %s was unavailable to join the queue. (ObjectGuid guid exist in
             // client cache)
    ERR_BATTLEGROUND_JOIN_FAILED = -12, // Join as a group failed (ObjectGuid
                                        // guid doesn't exist in client cache)
    ERR_LFG_CANT_USE_BATTLEGROUND =
        -13, // You cannot queue for a battleground or arena while using the
             // dungeon system.
    ERR_IN_RANDOM_BG =
        -14, // Can't do that while in a Random Battleground queue.
    ERR_IN_NON_RANDOM_BG = -15 // Can't queue for Random Battleground while in
                               // another Battleground queue.
};
...
enum PartyResult {
    ...
    ERR_GROUP_SWAP_FAILED =
        14, // if (PartyOperation == PARTY_OP_SWAP) ERR_GROUP_SWAP_FAILED else
