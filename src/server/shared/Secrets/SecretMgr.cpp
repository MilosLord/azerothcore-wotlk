...
#define SECRET_FLAG_FOR(key, val, server)                                      \
    server##_##key = (val##ull << (16 * SERVER_PROCESS_##server))
#define SECRET_FLAG(key, val)                                                  \
    SECRET_FLAG_##key = val, SECRET_FLAG_FOR(key, val, AUTHSERVER),            \
    SECRET_FLAG_FOR(key, val, WORLDSERVER)
enum SecretFlags : uint64 { SECRET_FLAG(DEFER_LOAD, 0x1) };
...
static Optional<BigNumber> GetHexFromConfig(char const* configKey, int bits)
{
    ...
    if (!secret.SetHexStr(str.c_str())) {
        LOG_FATAL("server.loading",
                  "Invalid value for '{}' - specify a hexadecimal integer of "
                  "up to {} bits with no prefix.",
                  configKey,
                  bits);
        ABORT();
    }
    ...
    if (!((BigNumber(0) <= secret) && (secret < threshold))) {
        LOG_ERROR("server.loading",
                  "Value for '{}' is out of bounds (should be an integer of up "
                  "to {} bits with no prefix). Truncated to {} bits.",
                  configKey,
                  bits,
                  bits);
        secret %= threshold;
                            ...
                            std::unique_lock<std::mutex> const&)
{
    ...
    {
        auto* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_SEL_SECRET_DIGEST);
        stmt->SetData(0, i);
    ...
    Optional<BigNumber> currentValue =
        GetHexFromConfig(info.configKey, info.bits);

    // verify digest
    if (((!oldDigest) != (!currentValue)) || // there is an old digest, but no
                                             // current secret (or vice versa)
        (oldDigest && !Acore::Crypto::Argon2::Verify(
                          currentValue->AsHexStr(),
                          *oldDigest)) // there is an old digest, and the
                                       // current secret does not match it
    ) {
        if (info.owner != THIS_SERVER_PROCESS) {
            if (currentValue)
                LOG_MESSAGE_BODY(
                    "server.loading",
                    errorLevel,
                    "Invalid value for '{}' specified - this is not actually "
                    "the secret being used in your auth DB.",
                    info.configKey);
            else
                LOG_MESSAGE_BODY(
                    "server.loading",
                    errorLevel,
                    "No value for '{}' specified - please specify the secret "
                    "currently being used in your auth DB.",
                    info.configKey);
        ...
        if (oldDigest && info.oldKey) // there is an old digest, so there might
                                      // be an old secret (if possible)
        {
            oldSecret = GetHexFromConfig(info.oldKey, info.bits);
            if (oldSecret && !Acore::Crypto::Argon2::Verify(
                                 oldSecret->AsHexStr(), *oldDigest)) {
                LOG_MESSAGE_BODY(
                    "server.loading",
                    errorLevel,
                    "Invalid value for '{}' specified - this is not actually "
                    "the secret previously used in your auth DB.",
                    info.oldKey);
                _secrets[i].state = Secret::LOAD_FAILED;
                return;
            }
        ...
        Optional<std::string> error = AttemptTransition(
            Secrets(i), currentValue, oldSecret, static_cast<bool>(oldDigest));
        if (error) {
            LOG_MESSAGE_BODY("server.loading",
                             errorLevel,
                             "Your value of '{}' changed, but we cannot "
                             "transition your database to the new value:\n{}",
                             info.configKey,
                             error->c_str());
            _secrets[i].state = Secret::LOAD_FAILED;
            return;
        ...
        LOG_INFO("server.loading",
                 "Successfully transitioned database to new '{}' value.",
                 info.configKey);
    }
                             ...
                             bool                       hadOldSecret) const
{
    ...
    case SECRET_TOTP_MASTER_KEY: {
        QueryResult result =
            LoginDatabase.Query("SELECT id, totp_secret FROM account");
        if (result)
            do {
                ...
                if (hadOldSecret) {
                    if (!oldSecret)
                        return Acore::StringFormat(
                            "Cannot decrypt old TOTP tokens - add config key "
                            "'%s' to authserver.conf!",
                            secret_info[i].oldKey);

                    bool success = Acore::Crypto::AEDecrypt<Acore::Crypto::AES>(
                        totpSecret,
                        oldSecret->ToByteArray<
                            Acore::Crypto::AES::KEY_SIZE_BYTES>());
                    if (!success)
                        return Acore::StringFormat(
                            "Cannot decrypt old TOTP tokens - value of '%s' is "
                ...
                if (newSecret)
                    Acore::Crypto::AEEncryptWithRandomIV<Acore::Crypto::AES>(
                        totpSecret,
                        newSecret->ToByteArray<
                            Acore::Crypto::AES::KEY_SIZE_BYTES>());

                auto* updateStmt = LoginDatabase.GetPreparedStatement(
    ...
    if (hadOldSecret) {
        auto* deleteStmt =
            LoginDatabase.GetPreparedStatement(LOGIN_DEL_SECRET_DIGEST);
    ...
    if (newSecret) {
        BigNumber salt;
        salt.SetRand(128);
        Optional<std::string> hash =
            Acore::Crypto::Argon2::Hash(newSecret->AsHexStr(), salt);
        if (!hash)
            return std::string("Failed to hash new secret");

        auto* insertStmt =
            LoginDatabase.GetPreparedStatement(LOGIN_INS_SECRET_DIGEST);
