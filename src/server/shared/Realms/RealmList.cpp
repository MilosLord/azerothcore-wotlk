...
void RealmList::Close() { _updateTimer->cancel(); }
...
void RealmList::LoadBuildInfo()
{
    ...
    if (auto result = LoginDatabase.Query(
            "SELECT majorVersion, minorVersion, bugfixVersion, hotfixVersion, "
            "build, winChecksumSeed, macChecksumSeed FROM build_info ORDER BY "
            "build ASC")) {
        for (auto const& fields : *result) {
            ...
            if (hotfixVersion.length() < build.HotfixVersion.size()) {
                std::copy(hotfixVersion.begin(),
                          hotfixVersion.end(),
                          build.HotfixVersion.begin());
            }
            else {
                            ...
                            float                      population)
{
    ...
    if (!realm.ExternalAddress || *realm.ExternalAddress != address) {
        realm.ExternalAddress =
            std::make_unique<boost::asio::ip::address>(std::move(address));
    }

    if (!realm.LocalAddress || *realm.LocalAddress != localAddr) {
        realm.LocalAddress =
            std::make_unique<boost::asio::ip::address>(std::move(localAddr));
    }

    if (!realm.LocalSubnetMask || *realm.LocalSubnetMask != localSubmask) {
        realm.LocalSubnetMask =
            std::make_unique<boost::asio::ip::address>(std::move(localSubmask));
    }
...
void RealmList::UpdateRealms(boost::system::error_code const& error)
{
    ...
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_REALMLIST);
    ...
    if (result) {
        for (auto const& fields : *result) {
            try {
                ...
                std::string externalAddressString =
                ...
                Optional<boost::asio::ip::tcp::endpoint> externalAddress =
                    _resolver->Resolve(
                        boost::asio::ip::tcp::v4(), externalAddressString, "");
                if (!externalAddress) {
                    LOG_ERROR(
                        "server.authserver",
                        "Could not resolve address {} for realm \"{}\" id {}",
                ...
                Optional<boost::asio::ip::tcp::endpoint> localAddress =
                    _resolver->Resolve(
                        boost::asio::ip::tcp::v4(), localAddressString, "");
                if (!localAddress) {
                    LOG_ERROR("server.authserver",
                              "Could not resolve localAddress {} for realm "
                              "\"{}\" id {}",
                              localAddressString,
                              name,
                              realmId);
                ...
                Optional<boost::asio::ip::tcp::endpoint> localSubmask =
                    _resolver->Resolve(
                        boost::asio::ip::tcp::v4(), localSubmaskString, "");
                if (!localSubmask) {
                    LOG_ERROR("server.authserver",
                              "Could not resolve localSubnetMask {} for realm "
                              "\"{}\" id {}",
                              localSubmaskString,
                              name,
                              realmId);
                ...
                LOG_ERROR("server.authserver",
                          "Realmlist::UpdateRealms has thrown an exception: {}",
                          ex.what());
                ABORT();
            }
        }
    ...
    if (_updateInterval) {
        _updateTimer->expires_from_now(
            boost::posix_time::seconds(_updateInterval));
        _updateTimer->async_wait(
            [this](boost::system::error_code const& errorCode) {
                UpdateRealms(errorCode);
            });
    }
}
