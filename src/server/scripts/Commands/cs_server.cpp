...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"server", serverCommandTable}};
    ...
    static bool HandleServerDebugCommand(ChatHandler* handler)
    {
        ...
        {
            uint16 dbPort = 0;
            if (QueryResult res = LoginDatabase.Query(
                    "SELECT port FROM realmlist WHERE id = {}", realm.Id.Realm))
                dbPort = (*res)[0].Get<uint16>();

            if (dbPort)
                dbPortOutput = Acore::StringFormatFmt(
                    "Realmlist (Realm Id: {}) configured in port {}",
                    realm.Id.Realm,
                    dbPort);
            else
                dbPortOutput =
                    Acore::StringFormat("Realm Id: %u not found in `realmlist` "
                                        "table. Please check your setup",
                                        realm.Id.Realm);
        }
        ...
        handler->PSendSysMessage("Using Boost version: %i.%i.%i",
        ...
        handler->PSendSysMessage("Found MySQL Executable: %s",
                                 GitRevision::GetMySQLExecutable());

        handler->PSendSysMessage("Compiled on: %s",
                                 GitRevision::GetHostOSVersion());
        ...
        bool vmapLOSCheck =
            VMAP::VMapFactory::createOrGetVMapMgr()->isLineOfSightCalcEnabled();
        bool vmapHeightCheck =
            VMAP::VMapFactory::createOrGetVMapMgr()->isHeightCalcEnabled();
        ...
        if (vmapIndoorCheck || vmapLOSCheck || vmapHeightCheck) {
            handler->PSendSysMessage("VMAPs status: Enabled. LineOfSight: %i, "
                                     "getHeight: %i, indoorCheck: %i",
                                     vmapLOSCheck,
                                     vmapHeightCheck,
                                     vmapIndoorCheck);
        ...
        for (std::string const& subDir : subDirs) {
            ...
            if (!std::filesystem::exists(mapPath)) {
                handler->PSendSysMessage(
                    ...
                    subDir.c_str(),
                    mapPath.generic_string().c_str());
                continue;
            }

            auto        end        = std::filesystem::directory_iterator();
            std::size_t folderSize = std::accumulate(
                std::filesystem::directory_iterator(mapPath),
                end,
                std::size_t(0),
                [](std::size_t val, std::filesystem::path const& mapFile) {
            ...
            handler->PSendSysMessage(
                Acore::StringFormatFmt(
                    "{} directory located in {}. Total size: {} bytes",
                    subDir.c_str(),
                    mapPath.generic_string().c_str(),
                    folderSize)
                    .c_str());
        ...
        handler->PSendSysMessage(
            ...
            availableLocales.c_str());

        handler->PSendSysMessage("Using World DB: %s", sWorld->GetDBVersion());

        std::string lldb = "No updates found!";
        if (QueryResult resL = LoginDatabase.Query(
                "SELECT name FROM updates ORDER BY name DESC LIMIT 1")) {
            Field* fields = resL->Fetch();
            lldb          = fields[0].Get<std::string>();
        }
        std::string lcdb = "No updates found!";
        if (QueryResult resC = CharacterDatabase.Query(
                "SELECT name FROM updates ORDER BY name DESC LIMIT 1")) {
            Field* fields = resC->Fetch();
            lcdb          = fields[0].Get<std::string>();
        }
        std::string lwdb = "No updates found!";
        if (QueryResult resW = WorldDatabase.Query(
                "SELECT name FROM updates ORDER BY name DESC LIMIT 1")) {
            Field* fields = resW->Fetch();
            lwdb          = fields[0].Get<std::string>();
        }

        handler->PSendSysMessage("Latest LoginDatabase update: %s",
                                 lldb.c_str());
        handler->PSendSysMessage("Latest CharacterDatabase update: %s",
                                 lcdb.c_str());
        handler->PSendSysMessage("Latest WorldDatabase update: %s",
                                 lwdb.c_str());

        handler->PSendSysMessage("LoginDatabase queue size: %zu",
                                 LoginDatabase.QueueSize());
        handler->PSendSysMessage("CharacterDatabase queue size: %zu",
                                 CharacterDatabase.QueueSize());
        handler->PSendSysMessage("WorldDatabase queue size: %zu",
    ...
    static bool HandleServerInfoCommand(ChatHandler* handler)
    {
        ...
        if (!queuedSessionCount)
            handler->PSendSysMessage(
                "Connected players: %u. Characters in world: %u.",
                activeSessionCount,
                playerCount);
        else
            handler->PSendSysMessage(
                "Connected players: %u. Characters in world: %u. Queue: %u.",
                activeSessionCount,
                playerCount,
                queuedSessionCount);
        ...
        handler->PSendSysMessage(
            LANG_UPTIME,
            secsToTimeString(GameTime::GetUptime().count()).c_str());
        handler->PSendSysMessage(
            "Update time diff: %ums. Last %d diffs summary:",
            sWorldUpdateTime.GetLastUpdateTime(),
            sWorldUpdateTime.GetDatasetSize());
        handler->PSendSysMessage("- Mean: %ums",
                                 sWorldUpdateTime.GetAverageUpdateTime());
        handler->PSendSysMessage("- Median: %ums",
                                 sWorldUpdateTime.GetPercentile(50));
        handler->PSendSysMessage(
            "- Percentiles (95, 99, max): %ums, %ums, %ums",
    ...
    static bool HandleServerShutDownCommand(ChatHandler*    handler,
                                            std::string     time,
    ...
    static bool HandleServerRestartCommand(ChatHandler*    handler,
                                           std::string     time,
                                           Optional<int32> exitCode,
                                           Tail            reason)
    {
        ...
        if (exitCode && *exitCode >= 0 && *exitCode <= 125) {
            ...
            sWorld->ShutdownServ(
                delay, SHUTDOWN_MASK_RESTART, RESTART_EXIT_CODE, strReason);
        }

        return true;
    }

    static bool HandleServerIdleRestartCommand(ChatHandler*    handler,
                                               std::string     time,
                                               Optional<int32> exitCode,
                                               Tail            reason)
    {
        ...
        if (exitCode && *exitCode >= 0 && *exitCode <= 125) {
            ...
            sWorld->ShutdownServ(delay,
                                 SHUTDOWN_MASK_RESTART | SHUTDOWN_MASK_IDLE,
                                 RESTART_EXIT_CODE,
                                 strReason);
        }
    ...
    static bool HandleServerIdleShutDownCommand(ChatHandler*    handler,
                                                std::string     time,
                                                Optional<int32> exitCode,
                                                Tail            reason)
    {
        ...
        if (exitCode && *exitCode >= 0 && *exitCode <= 125) {
            ...
            sWorld->ShutdownServ(
                delay, SHUTDOWN_MASK_IDLE, SHUTDOWN_EXIT_CODE, strReason);
        }
    ...
    static bool HandleServerSetMotdCommand(ChatHandler*    handler,
                                           ...
                                           Tail            motd)
    {
        ...
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_REP_MOTD);
    ...
    static bool HandleServerSetClosedCommand(ChatHandler*          handler,
    ...
    static bool HandleServerSetLogLevelCommand(ChatHandler* /*handler*/,
                                               bool               isLogger,
...
void AddSC_server_commandscript() { new server_commandscript(); }
