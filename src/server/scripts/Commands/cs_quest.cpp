...
public:
    ...
    static bool HandleQuestAdd(ChatHandler*               handler,
                               Quest const*               quest,
                               Optional<PlayerIdentifier> playerTarget)
    {
        ...
        // check item starting quest (it can work incorrectly if added without
        // item in inventory)
        ItemTemplateContainer const* itc = sObjectMgr->GetItemTemplateStore();
        ItemTemplateContainer::const_iterator result = find_if(
            itc->begin(),
            itc->end(),
            Finder<uint32, ItemTemplate>(entry, &ItemTemplate::StartQuest));

        if (result != itc->end()) {
            handler->SendErrorMessage(
                LANG_COMMAND_QUEST_STARTFROMITEM, entry, result->second.ItemId);
            return false;
        }

        if (Player* player = playerTarget->GetConnectedPlayer()) {
            if (player->IsActiveQuest(entry)) {
                handler->SendErrorMessage(LANG_COMMAND_QUEST_ACTIVE,
            ...
            QueryResult         result =
                CharacterDatabase.Query("SELECT 1 FROM character_queststatus "
        ...
        handler->PSendSysMessage(
            LANG_COMMAND_QUEST_ADD, quest->GetTitle().c_str(), entry);
        handler->SetSentErrorMessage(false);
        return true;
    }

    static bool HandleQuestRemove(ChatHandler*               handler,
                                  Quest const*               quest,
                                  Optional<PlayerIdentifier> playerTarget)
    {
        if (!playerTarget) {
        ...
        if (Player* player = playerTarget->GetConnectedPlayer()) {
            ...
            for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot) {
                ...
                if (logQuest == entry) {
                    ...
                    // we ignore unequippable quest items in this case, its'
                    // still be equipped
                    player->TakeQuestSourceItem(logQuest, false);

                    if (quest->HasFlag(QUEST_FLAGS_FLAGS_PVP)) {
                        player->pvpInfo.IsHostile =
                            player->pvpInfo.IsInHostileArea ||
        ...
        handler->PSendSysMessage(
            LANG_COMMAND_QUEST_REMOVED, quest->GetTitle().c_str(), entry);
        handler->SetSentErrorMessage(false);
        return true;
    }

    static bool HandleQuestComplete(ChatHandler*               handler,
                                    Quest const*               quest,
                                    Optional<PlayerIdentifier> playerTarget)
    {
        if (!playerTarget) {
        ...
        if (Player* player = playerTarget->GetConnectedPlayer()) {
            ...
            for (uint8 x = 0; x < QUEST_ITEM_OBJECTIVES_COUNT; ++x) {
                ...
                uint8           msg = player->CanStoreNewItem(
                    NULL_BAG, NULL_SLOT, dest, id, count - curItemCount);
                if (msg == EQUIP_ERR_OK) {
                    Item* item = player->StoreNewItem(dest, id, true);
                    player->SendNewItem(
                        item, count - curItemCount, true, false);
                }
            }

            // All creature/GO slain/casted (not required, but otherwise it will
            // display "Creature slain 0/10")
            for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i) {
                ...
                if (creature > 0) {
                    if (CreatureTemplate const* creatureInfo =
                        ...
                        for (uint16 z = 0; z < creatureCount; ++z) {
                            player->KilledMonster(creatureInfo,
                                                  ObjectGuid::Empty);
                        }
                    }
                }
            ...
            if (uint32 repFaction = quest->GetRepObjectiveFaction2()) {
                ...
                if (curRep < repValue2) {
                    if (FactionEntry const* factionEntry =
                            sFactionStore.LookupEntry(repFaction)) {
                        player->GetReputationMgr().SetReputation(
                            factionEntry, static_cast<float>(repValue2));
                    }
                }
            }
            ...
            if (!result) {
                handler->SendErrorMessage(LANG_COMMAND_QUEST_NOT_FOUND_IN_LOG,
                                          quest->GetTitle(),
                                          entry);
                return false;
            }
            ...
            // If the quest requires reputation to complete, set the player rep
        ...
        handler->PSendSysMessage(
            LANG_COMMAND_QUEST_COMPLETE, quest->GetTitle().c_str(), entry);
        handler->SetSentErrorMessage(false);
        return true;
    }

    static bool HandleQuestReward(ChatHandler*               handler,
                                  Quest const*               quest,
                                  Optional<PlayerIdentifier> playerTarget)
    {
        if (!playerTarget) {
        ...
        if (Player* player = playerTarget->GetConnectedPlayer()) {
            ...
            // Achievement criteria updates correctly the next time a quest is
            // rewarded. Titles are already awarded correctly the next time they
            // login (only one quest awards title - 11549). Rewarded talent
            // points (Death Knights) and spells (e.g Druid forms) are also
            // granted on login. No reputation gains - too troublesome to
            // calculate them when the player is offline.

            ObjectGuid::LowType guid = playerTarget->GetGUID().GetCounter();
            uint8 charLevel          = sCharacterCache->GetCharacterLevelByGuid(
                ObjectGuid(HighGuid::Player, guid));
            CharacterDatabaseTransaction trans =
                CharacterDatabase.BeginTransaction();
            CharacterDatabasePreparedStatement* stmt;

            QueryResult result = CharacterDatabase.Query(
                "SELECT 1 FROM character_queststatus WHERE guid = {} AND quest "
                "= {} AND status = 1",
                guid,
                entry);
            ...
            if (quest->GetRewChoiceItemsCount()) {
                for (uint32 const& itemId : quest->RewardChoiceItemId) {
                    uint8 index = 0;
                    questRewardItems.push_back(std::pair(
                        itemId, quest->RewardChoiceItemCount[index++]));
                }
            }
            ...
            if (!questRewardItems.empty()) {
                ...
                MailDraft draft(quest->GetTitle(),
                                "This quest has been manually rewarded to you. "
                                "This mail contains your quest rewards.");
            ...
            if (quest->IsDaily() || quest->IsDFQuest()) {
                ...
                // We can't know which event is the quest linked to, so we can't
                // do anything about this.
                /* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_INS_CHARACTER_SEASONALQUESTSTATUS);
                stmt->SetData(0, guid);
                stmt->SetData(1, entry);
            ...
            if (charLevel >= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
                ...
                uint32 xp = uint32(quest->XPValue(charLevel) *
        ...
        handler->PSendSysMessage(
            LANG_COMMAND_QUEST_REWARDED, quest->GetTitle().c_str(), entry);
        handler->SetSentErrorMessage(false);
        return true;
    }
};

void AddSC_quest_commandscript() { new quest_commandscript(); }
