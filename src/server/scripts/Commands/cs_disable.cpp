...
public:
    ...
    static bool HandleAddDisables(ChatHandler* handler,
                                  uint32       entry,
                                  uint8        flags,
                                  ...
                                  uint8        disableType)
    {
        ...
        if (result) {
            handler->SendErrorMessage("This %s (Id: %u) is already disabled.",
                                      disableTypeStr.c_str(),
                                      entry);
            return false;
        }
        ...
        handler->PSendSysMessage("Add Disabled %s (Id: %u) for reason %s",
                                 disableTypeStr.c_str(),
                                 entry,
                                 disableComment.c_str());
        return true;
    }

    static bool HandleAddDisableSpellCommand(ChatHandler* handler,
                                             uint32       entry,
                                             uint8        flags,
                                             std::string  disableComment)
    {
        return HandleAddDisables(
            handler, entry, flags, disableComment, DISABLE_TYPE_SPELL);
    }

    static bool HandleAddDisableQuestCommand(ChatHandler* handler,
                                             uint32       entry,
                                             uint8        flags,
                                             std::string  disableComment)
    {
        return HandleAddDisables(
            handler, entry, flags, disableComment, DISABLE_TYPE_QUEST);
    }

    static bool HandleAddDisableMapCommand(ChatHandler* handler,
                                           uint32       entry,
                                           uint8        flags,
                                           std::string  disableComment)
    {
        return HandleAddDisables(
            handler, entry, flags, disableComment, DISABLE_TYPE_MAP);
    }

    static bool HandleAddDisableBattlegroundCommand(ChatHandler* handler,
                                                    uint32       entry,
                                                    uint8        flags,
                                                    std::string  disableComment)
    {
        return HandleAddDisables(
            handler, entry, flags, disableComment, DISABLE_TYPE_BATTLEGROUND);
    }

    static bool
    HandleAddDisableAchievementCriteriaCommand(ChatHandler* handler,
                                               uint32       entry,
                                               uint8        flags,
                                               std::string  disableComment)
    {
        return HandleAddDisables(handler,
                                 entry,
                                 flags,
                                 disableComment,
    ...
    static bool HandleAddDisableOutdoorPvPCommand(ChatHandler* handler,
                                                  uint32       entry,
                                                  uint8        flags,
                                                  std::string  disableComment)
    {
        HandleAddDisables(
            handler, entry, flags, disableComment, DISABLE_TYPE_OUTDOORPVP);
        return true;
    }

    static bool HandleAddDisableVmapCommand(ChatHandler* handler,
                                            uint32       entry,
                                            uint8        flags,
                                            std::string  disableComment)
    {
        return HandleAddDisables(
            handler, entry, flags, disableComment, DISABLE_TYPE_VMAP);
    }

    static bool
    HandleRemoveDisables(ChatHandler* handler, uint32 entry, uint8 disableType)
    {
        std::string disableTypeStr = "";
        ...
        PreparedQueryResult result = WorldDatabase.Query(stmt);
        if (!result) {
            handler->SendErrorMessage("This %s (Id: %u) is not disabled.",
                                      disableTypeStr.c_str(),
                                      entry);
            return false;
        }
    ...
    static bool HandleRemoveDisableSpellCommand(ChatHandler* handler,
                                                uint32       entry)
    {
        return HandleRemoveDisables(handler, entry, DISABLE_TYPE_SPELL);
    }

    static bool HandleRemoveDisableQuestCommand(ChatHandler* handler,
                                                uint32       entry)
    {
        return HandleRemoveDisables(handler, entry, DISABLE_TYPE_QUEST);
    }

    static bool HandleRemoveDisableMapCommand(ChatHandler* handler,
                                              uint32       entry)
    {
        return HandleRemoveDisables(handler, entry, DISABLE_TYPE_MAP);
    }

    static bool HandleRemoveDisableBattlegroundCommand(ChatHandler* handler,
                                                       uint32       entry)
    {
    ...
    static bool
    HandleRemoveDisableAchievementCriteriaCommand(ChatHandler* handler,
                                                  uint32       entry)
    {
        return HandleRemoveDisables(
            handler, entry, DISABLE_TYPE_ACHIEVEMENT_CRITERIA);
    }

    static bool HandleRemoveDisableOutdoorPvPCommand(ChatHandler* handler,
                                                     uint32       entry)
    {
        return HandleRemoveDisables(handler, entry, DISABLE_TYPE_OUTDOORPVP);
    }

    static bool HandleRemoveDisableVmapCommand(ChatHandler* handler,
                                               uint32       entry)
    {
...
void AddSC_disable_commandscript() { new disable_commandscript(); }
