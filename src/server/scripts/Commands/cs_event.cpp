...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"event", eventCommandTable}};
        return commandTable;
    }
    ...
    static bool HandleEventInfoCommand(ChatHandler*     handler,
                                       EventEntry const eventId)
    {
        ...
        char const* activeStr =
            active ? handler->GetAcoreString(LANG_ACTIVE) : "";

        std::string startTimeStr =
            Acore::Time::TimeToTimestampStr(Seconds(eventData.start));
        std::string endTimeStr =
            Acore::Time::TimeToTimestampStr(Seconds(eventData.end));
        ...
        std::string occurenceStr =
            secsToTimeString(eventData.occurence * MINUTE, true);
        std::string lengthStr =
            secsToTimeString(eventData.length * MINUTE, true);
    ...
    static bool HandleEventStartCommand(ChatHandler*     handler,
                                        EventEntry const eventId)
    {
        GameEventMgr::GameEventDataMap const& events =
            sGameEventMgr->GetEventMap();
        ...
        if (activeEvents.find(eventId) != activeEvents.end()) {
            handler->SendErrorMessage(LANG_EVENT_ALREADY_ACTIVE,
                                      uint16(eventId),
                                      eventData.description.c_str());
            return false;
        }
        ...
        handler->PSendSysMessage(
            LANG_EVENT_STARTED, uint16(eventId), eventData.description.c_str());
        sGameEventMgr->StartEvent(eventId, true);
        return true;
    }

    static bool HandleEventStopCommand(ChatHandler*     handler,
                                       EventEntry const eventId)
    {
        GameEventMgr::GameEventDataMap const& events =
            sGameEventMgr->GetEventMap();
        ...
        if (activeEvents.find(eventId) == activeEvents.end()) {
            handler->SendErrorMessage(LANG_EVENT_NOT_ACTIVE,
                                      uint16(eventId),
                                      eventData.description.c_str());
            return false;
        }
        ...
        handler->PSendSysMessage(
            LANG_EVENT_STOPPED, uint16(eventId), eventData.description.c_str());
