...
public:
    ...
    static bool HandleInstanceListBindsCommand(ChatHandler* handler)
    {
        ...
        for (uint8 i = 0; i < MAX_DIFFICULTY; ++i) {
            for (auto const& [mapId, bind] :
                 sInstanceSaveMgr->PlayerGetBoundInstances(player->GetGUID(),
                                                           Difficulty(i))) {
                InstanceSave const* save = bind.save;
                uint32 resetTime = bind.extended ? save->GetExtendedResetTime()
                                                 : save->GetResetTime();
                uint32 ttr       = (resetTime >= GameTime::GetGameTime().count()
                                        ? resetTime - GameTime::GetGameTime().count()
                                        : 0);
                std::string timeleft = secsToTimeString(ttr);
                handler->PSendSysMessage("map: %d, inst: %d, perm: %s, diff: "
                                         "%d, canReset: %s, TTR: %s%s",
                                         mapId,
                                         save->GetInstanceId(),
                                         bind.perm ? "yes" : "no",
                                         save->GetDifficulty(),
    ...
    static bool
    HandleInstanceUnbindCommand(ChatHandler*                           handler,
        ...
        for (uint8 i = 0; i < MAX_DIFFICULTY; ++i) {
            ...
            for (BoundInstancesMap::const_iterator itr =
                ...
                if (itr->first != player->GetMapId() &&
                    (!mapId || mapId == itr->first) &&
                    (!difficultyArg ||
                     difficultyArg == save->GetDifficulty())) {
                    uint32 resetTime = itr->second.extended
                    ...
                    handler->PSendSysMessage(
                        "unbinding map: %d, inst: %d, perm: %s, diff: %d, "
                        "canReset: %s, TTR: %s%s",
                        itr->first,
                        save->GetInstanceId(),
                    ...
                    sInstanceSaveMgr->PlayerUnbindInstance(player->GetGUID(),
                                                           itr->first,
                                                           Difficulty(i),
                                                           true,
                                                           player);
    ...
    static bool HandleInstanceStatsCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(
            "instances loaded: dungeons (%d), battlegrounds (%d), arenas (%d)",
            dungeon,
            battleground,
            arena);
        dungeon      = 0;
        ...
        arena        = 0;
        spectators   = 0;
        sMapMgr->GetNumPlayersInInstances(
            dungeon, battleground, arena, spectators);
        handler->SendErrorMessage("players in instances: dungeons (%d), "
                                  "battlegrounds (%d), arenas (%d + %d spect)",
                                  dungeon,
                                  battleground,
                                  arena,
                                  spectators);
    ...
    static bool
    HandleInstanceSetBossStateCommand(ChatHandler*               handler,
                                      uint32                     encounterId,
                                      uint32                     state,
                                      Optional<PlayerIdentifier> player)
    {
        ...
        InstanceMap* map =
            player->GetConnectedPlayer()->GetMap()->ToInstanceMap();
        if (!map) {
        ...
        map->GetInstanceScript()->SetBossState(encounterId,
                                               EncounterState(state));
        std::string stateName = InstanceScript::GetBossStateName(state);
        handler->PSendSysMessage(LANG_COMMAND_INST_SET_BOSS_STATE,
                                 encounterId,
                                 state,
                                 stateName.c_str());
        return true;
    ...
    static bool
    HandleInstanceGetBossStateCommand(ChatHandler*               handler,
                                      uint32                     encounterId,
                                      Optional<PlayerIdentifier> player)
    {
        // Character name must be provided when using this from console.
        ...
        std::string stateName = InstanceScript::GetBossStateName(state);
        handler->PSendSysMessage(LANG_COMMAND_INST_GET_BOSS_STATE,
                                 encounterId,
                                 state,
                                 stateName.c_str());
        return true;
...
void AddSC_instance_commandscript() { new instance_commandscript(); }
