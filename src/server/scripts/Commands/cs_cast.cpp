...
public:
    ...
    static bool CheckSpellCastResult(ChatHandler*    handler,
                                     SpellCastResult result)
    {
        if (result != SPELL_CAST_OK) {
            handler->PSendSysMessage(
                LANG_CMD_CAST_ERROR_CODE,
                EnumUtils::ToTitle(SpellCastResult(result)),
                result);
            return false;
        }
    ...
    static bool CheckSpellExistsAndIsValid(ChatHandler*     handler,
    ...
    static Optional<TriggerCastFlags>
    GetTriggerFlags(Optional<std::string> triggeredStr)
    {
        if (triggeredStr) {
            if (StringStartsWith("triggered",
                                 *triggeredStr)) // check if "triggered" starts
                                                 // with *triggeredStr (e.g.
                                                 // "trig", "trigger", etc.)
    ...
    static bool HandleCastCommand(ChatHandler*          handler,
                                  SpellInfo const*      spell,
                                  Optional<std::string> triggeredStr)
    {
        ...
        if (!CheckSpellCastResult(handler,
                                  handler->GetSession()->GetPlayer()->CastSpell(
                                      target, spell->Id, *triggerFlags))) {
    ...
    static bool HandleCastBackCommand(ChatHandler*          handler,
                                      SpellInfo const*      spell,
                                      Optional<std::string> triggeredStr)
    {
        ...
        if (!CheckSpellCastResult(
                handler,
                caster->CastSpell(handler->GetSession()->GetPlayer(),
                                  spell->Id,
                                  *triggerFlags))) {
            handler->SetSentErrorMessage(true);
            return false;
    ...
    static bool HandleCastDistCommand(ChatHandler*          handler,
                                      SpellInfo const*      spell,
                                      float                 dist,
                                      Optional<std::string> triggeredStr)
    {
    ...
    static bool HandleCastSelfCommand(ChatHandler*          handler,
                                      SpellInfo const*      spell,
                                      Optional<std::string> triggeredStr)
    {
        Unit* target = handler->getSelectedUnit();
        ...
        if (!CheckSpellCastResult(
                handler, target->CastSpell(target, spell->Id, *triggerFlags))) {
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    static bool HandleCastTargetCommad(ChatHandler*          handler,
                                       SpellInfo const*      spell,
                                       Optional<std::string> triggeredStr)
    {
        Creature* caster = handler->getSelectedCreature();
        ...
        if (!CheckSpellCastResult(handler,
                                  caster->CastSpell(caster->GetVictim(),
                                                    spell->Id,
                                                    *triggerFlags))) {
            handler->SetSentErrorMessage(true);
            return false;
    ...
    static bool HandleCastDestCommand(ChatHandler*          handler,
                                      SpellInfo const*      spell,
                                      float                 x,
                                      float                 y,
                                      float                 z,
