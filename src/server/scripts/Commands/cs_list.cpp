...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"list", listCommandTable}};
        return commandTable;
    }

    static bool HandleListCreatureCommand(
        ChatHandler*                               handler,
        ...
        Optional<uint32>                           countArg)
    {
        ...
        if (!cInfo) {
            handler->SendErrorMessage(LANG_COMMAND_INVALIDCREATUREID,
                                      uint32(creatureId));
            return false;
        }
        ...
        result = WorldDatabase.Query("SELECT COUNT(guid) FROM creature WHERE "
                                     "id1='{}' OR id2='{}' OR id3='{}'",
                                     uint32(creatureId),
                                     uint32(creatureId),
                                     uint32(creatureId));
        ...
        if (handler->GetSession()) {
            ...
            result         = WorldDatabase.Query(
                "SELECT guid, position_x, position_y, position_z, map, "
                        ...
                        "POW(position_z - '{}', 2)) AS order_ FROM creature WHERE "
                        "id1='{}' OR id2='{}' OR id3='{}' ORDER BY order_ ASC LIMIT {}",
                player->GetPositionX(),
                player->GetPositionY(),
                player->GetPositionZ(),
                uint32(creatureId),
            ...
            result = WorldDatabase.Query(
                ...
                "creature WHERE id1='{}' OR id2='{}' OR id3='{}' LIMIT {}",
                uint32(creatureId),
                uint32(creatureId),
                uint32(creatureId),
                count);
        ...
        if (result) {
            do {
                ...
                if (thisMap) {
                    auto const creBounds =
                        thisMap->GetCreatureBySpawnIdStore().equal_range(guid);
                    if (creBounds.first != creBounds.second) {
                        for (std::unordered_multimap<uint32,
                                                     Creature*>::const_iterator
                                 itr = creBounds.first;
                             itr != creBounds.second;) {
                            if (handler->GetSession())
                                handler->PSendSysMessage(
                                    ...
                                    z,
                                    mapId,
                                    itr->second->GetGUID().ToString().c_str(),
                                    itr->second->IsAlive() ? "*" : " ");
                            else
                                handler->PSendSysMessage(
                ...
                if (!liveFound) {
                    if (handler->GetSession())
                        ...
                        handler->PSendSysMessage(LANG_CREATURE_LIST_CONSOLE,
                                                 guid,
                                                 cInfo->Name.c_str(),
                                                 x,
                                                 y,
        ...
        handler->PSendSysMessage(LANG_COMMAND_LISTCREATUREMESSAGE,
                                 uint32(creatureId),
                                 creatureCount);
    ...
    static bool HandleListItemCommand(ChatHandler*                     handler,
                                      ...
                                      Optional<uint32>                 countArg)
    {
        ...
        if (result) {
            do {
                ...
                handler->PSendSysMessage(LANG_ITEMLIST_MAIL,
                                         itemGuid,
                                         itemSenderName.c_str(),
                                         itemSender,
                                         itemSenderAccountId,
                                         itemReceiverName.c_str(),
                                         itemReceiver,
                                         itemReceiverAccount,
                                         itemPos);
            } while (result->NextRow());
        ...
        if (result) {
            do {
                ...
                handler->PSendSysMessage(LANG_ITEMLIST_AUCTION,
                                         itemGuid,
                                         ownerName.c_str(),
                                         owner,
                                         ownerAccountId,
        ...
        if (result) {
            do {
                ...
                handler->PSendSysMessage(LANG_ITEMLIST_GUILD,
                                         itemGuid,
                                         guildName.c_str(),
                                         guildGuid,
                                         itemPos);
        ...
        handler->PSendSysMessage(LANG_COMMAND_LISTITEMMESSAGE,
                                 itemId,
                                 inventoryCount + mailCount + auctionCount +
                                     guildCount,
                                 inventoryCount,
                                 mailCount,
                                 auctionCount,
    ...
    static bool HandleListObjectCommand(
        ChatHandler*                                 handler,
        Variant<Hyperlink<gameobject_entry>, uint32> gameObjectId,
        Optional<uint32>                             countArg)
    {
        GameObjectTemplate const* gInfo =
            sObjectMgr->GetGameObjectTemplate(gameObjectId);
        if (!gInfo) {
            handler->SendErrorMessage(LANG_COMMAND_LISTOBJINVALIDID,
                                      uint32(gameObjectId));
            return false;
        }
        ...
        result             = WorldDatabase.Query(
            "SELECT COUNT(guid) FROM gameobject WHERE id='{}'",
            uint32(gameObjectId));
        if (result)
        ...
        if (handler->GetSession()) {
            ...
            result         = WorldDatabase.Query(
                "SELECT guid, position_x, position_y, position_z, map, id, "
                        "(POW(position_x - '{}', 2) + POW(position_y - '{}', 2) + "
                        "POW(position_z - '{}', 2)) AS order_ FROM gameobject WHERE id "
                        "= '{}' ORDER BY order_ ASC LIMIT {}",
                player->GetPositionX(),
                player->GetPositionY(),
                player->GetPositionZ(),
                uint32(gameObjectId),
            ...
            result = WorldDatabase.Query(
                "SELECT guid, position_x, position_y, position_z, map, id FROM "
                "gameobject WHERE id = '{}' LIMIT {}",
                uint32(gameObjectId),
                count);
        ...
        if (result) {
            do {
                ...
                if (thisMap) {
                    auto const goBounds =
                        thisMap->GetGameObjectBySpawnIdStore().equal_range(
                            guid);
                    if (goBounds.first != goBounds.second) {
                        for (std::unordered_multimap<uint32, GameObject*>::
                ...
                if (!liveFound) {
                    if (handler->GetSession())
                        ...
                        handler->PSendSysMessage(LANG_GO_LIST_CONSOLE,
                                                 guid,
                                                 gInfo->name.c_str(),
                                                 x,
                                                 y,
        ...
        handler->PSendSysMessage(
            LANG_COMMAND_LISTOBJMESSAGE, uint32(gameObjectId), objectCount);
    ...
    static bool HandleListAurasByNameCommand(ChatHandler* handler,
                                             WTail        namePart)
    {
        return ListAurasCommand(handler, {}, namePart);
    }

    static bool ListAurasCommand(ChatHandler*     handler,
                                 ...
                                 std::wstring     namePart)
    {
        ...
        for (auto const& [aurId, aurApp] : auras) {
            ...
            char const* name =
                aura->GetSpellInfo()->SpellName[handler->GetSessionDbcLocale()];

            if (!ShouldListAura(aura->GetSpellInfo(),
                                spellId,
                                namePart,
            ...
            ss_name << "|cffffffff|Hspell:" << aura->GetId() << "|h[" << name
                    << "]|h|r";

            handler->PSendSysMessage(
                LANG_COMMAND_TARGET_AURADETAIL,
                aura->GetId(),
                (handler->GetSession() ? ss_name.str().c_str() : name),
        ...
        for (uint16 i = 0; i < TOTAL_AURAS; ++i) {
            Unit::AuraEffectList const& auraList =
                unit->GetAuraEffectsByType(AuraType(i));
            if (auraList.empty())
                continue;
            ...
            for (AuraEffect const* effect : auraList) {
                if (!ShouldListAura(effect->GetSpellInfo(),
                                    spellId,
                                    namePart,
                                    handler->GetSessionDbcLocale()))
                    continue;
                ...
                if (!sizeLogged) {
                    sizeLogged = true;
                    handler->PSendSysMessage(
                        LANG_COMMAND_TARGET_LISTAURATYPE, auraList.size(), i);
                }

                handler->PSendSysMessage(LANG_COMMAND_TARGET_AURASIMPLE,
                                         effect->GetId(),
                                         effect->GetEffIndex(),
                                         effect->GetAmount());
            }
    ...
    static bool ShouldListAura(SpellInfo const* spellInfo,
                               Optional<uint32> spellId,
                               std::wstring     namePart,
                               uint8            locale)
    {
