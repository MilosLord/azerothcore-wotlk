...
public:
    ...
    static bool HandleDebugPlayCinematicCommand(ChatHandler* handler,
                                                uint32       cinematicId)
    {
        CinematicSequencesEntry const* cineSeq =
        ...
        if (std::vector<FlyByCamera> const* flyByCameras =
                GetFlyByCameras(cineSeq->cinematicCamera)) {
            handler->PSendSysMessage("Waypoints for sequence %u, camera %u",
                                     cinematicId,
                                     cineSeq->cinematicCamera);
            uint32 count = 1;
            for (FlyByCamera const& cam : *flyByCameras) {
                handler->PSendSysMessage("%02u - %7ums [%s (%f degrees)]",
                                         count,
                                         cam.timeStamp,
                                         cam.locations.ToString().c_str(),
                                         cam.locations.GetOrientation() *
                                             (180 / M_PI));
                ++count;
            }
            handler->PSendSysMessage("%u waypoints dumped",
                                     flyByCameras->size());
        }
    ...
    static bool HandleDebugPlayMovieCommand(ChatHandler* handler,
                                            uint32       movieId)
    {
    ...
    static bool HandleDebugPlaySoundCommand(ChatHandler* handler,
                                            uint32       soundId)
    {
    ...
    static bool HandleDebugPlayMusicCommand(ChatHandler* handler,
                                            uint32       musicId)
    {
    ...
    static bool HandleDebugSendSpellFailCommand(ChatHandler*     handler,
    ...
    static bool HandleDebugSendEquipErrorCommand(ChatHandler*    handler,
                                                 InventoryResult error)
    {
        handler->GetPlayer()->SendEquipError(
            InventoryResult(error), nullptr, nullptr);
        return true;
    }

    static bool HandleDebugSendSellErrorCommand(ChatHandler* handler,
                                                SellResult   error)
    {
        handler->GetPlayer()->SendSellError(
            SellResult(error), nullptr, ObjectGuid::Empty, 0);
        return true;
    }

    static bool HandleDebugSendBuyErrorCommand(ChatHandler* handler,
                                               BuyResult    error)
    {
    ...
    static bool HandleDebugSendOpcodeCommand(ChatHandler* handler)
    {
        ...
        while (!parsedStream.eof()) {
            ...
            if (type == "uint8") {
                ...
                LOG_ERROR("network.opcode",
                          "Sending opcode that has unknown type '{}'",
                          type);
                break;
            }
        }
        ...
        handler->PSendSysMessage(
            LANG_COMMAND_OPCODESENT, data.GetOpcode(), unit->GetName().c_str());
        return true;
    }

    static bool HandleDebugUpdateWorldStateCommand(ChatHandler* handler,
                                                   uint32       variable,
                                                   uint32       value)
    {
    ...
    static bool HandleDebugSendChannelNotifyCommand(ChatHandler* handler,
                                                    ChatNotify   type)
    {
    ...
    static bool HandleDebugSendChatMsgCommand(ChatHandler* handler,
                                              ChatMsg      type)
    {
        WorldPacket data;
    ...
    static bool HandleDebugSendQuestPartyMsgCommand(ChatHandler*       handler,
                                                    QuestShareMessages msg)
    {
        handler->GetPlayer()->SendPushToPartyResponse(handler->GetPlayer(),
                                                      msg);
        return true;
    }
    ...
    static bool HandleDebugGetLootRecipientCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(
            "Loot recipient for creature %s (GUID %u, SpawnID %u) is %s",
            target->GetName().c_str(),
            target->GetGUID().GetCounter(),
            target->GetSpawnId(),
            target->hasLootRecipient()
                ? (target->GetLootRecipient()
                       ? target->GetLootRecipient()->GetName().c_str()
                       : "offline")
                : "no loot recipient");
        return true;
    }

    static bool HandleDebugSendQuestInvalidMsgCommand(ChatHandler*      handler,
    ...
    static bool HandleDebugGetItemStateCommand(ChatHandler* handler,
                                               std::string  itemState)
    {
        ...
        if (!listQueue && !checkAll) {
            itemState =
                "The player has the following " + itemState + " items: ";
                ...
                if (Item* item =
                    ...
                    if (Bag* bag = item->ToBag()) {
                        for (uint8 j = 0; j < bag->GetBagSize(); ++j)
                            if (Item* item2 = bag->GetItemByPos(j))
                                if (item2->GetState() == state)
                                    handler->PSendSysMessage(
                                        "bag: 255 slot: %d %s owner: %s",
                                        item2->GetSlot(),
                                        item2->GetGUID().ToString().c_str(),
                                        item2->GetOwnerGUID()
                                            .ToString()
                                            .c_str());
                    }
                        ...
                        handler->PSendSysMessage(
                            ...
                            item->GetOwnerGUID().ToString().c_str());
                }
            }
        }
        ...
        if (listQueue) {
            ...
            for (auto const& item : updateQueue) {
                ...
                uint8 bagSlot   = container ? container->GetSlot()
                                            : uint8(INVENTORY_SLOT_BAG_0);
        ...
        if (checkAll) {
            ...
            for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; ++i) {
                ...
                if (item->GetSlot() != i) {
                    handler->PSendSysMessage("Item with slot %d and guid %d "
                                             "has an incorrect slot value: %d",
                                             i,
                                             item->GetGUID().GetCounter(),
                                             item->GetSlot());
                    error = true;
                ...
                if (item->GetOwnerGUID() != player->GetGUID()) {
                    handler->PSendSysMessage(
                        "The item with slot %d %s does have non-matching owner "
                        "guid %s and %s!",
                        item->GetSlot(),
                        item->GetGUID().ToString().c_str(),
                        item->GetOwnerGUID().ToString().c_str(),
                ...
                if (Bag* container = item->GetContainer()) {
                    handler->PSendSysMessage(
                        "The item with slot %d %s has a container (slot: %d, "
                        "%s) but shouldn't!",
                        item->GetSlot(),
                        item->GetGUID().ToString().c_str(),
                        container->GetSlot(),
                        container->GetGUID().ToString().c_str());
                    error = true;
                    continue;
                }
                ...
                if (item->IsInUpdateQueue()) {
                    ...
                    if (qp > updateQueue.size()) {
                        handler->PSendSysMessage(
                            "The item with slot %d and guid %d has its "
                            "queuepos (%d) larger than the update queue size! ",
                            item->GetSlot(),
                            item->GetGUID().GetCounter(),
                            qp);
                        error = true;
                    ...
                    if (!updateQueue[qp]) {
                        handler->PSendSysMessage(
                            "The item with slot %d and guid %d has its "
                            "queuepos (%d) pointing to NULL in the queue!",
                            item->GetSlot(),
                            item->GetGUID().GetCounter(),
                            qp);
                        error = true;
                    ...
                    if (updateQueue[qp] != item) {
                        handler->PSendSysMessage(
                            "The item with slot %d and guid %d has a queuepos "
                            "(%d) that points to another item in the queue "
                            "(bag: %d, slot: %d, guid: %d)",
                            item->GetSlot(),
                            item->GetGUID().GetCounter(),
                            qp,
                            ...
                            updateQueue[qp]->GetGUID().GetCounter());
                        error = true;
                        continue;
                    }
                }
                    ...
                    handler->PSendSysMessage(
                        "The item with slot %d and guid %d is not in queue but "
                        "should be (state: %d)!",
                        item->GetSlot(),
                        item->GetGUID().GetCounter(),
                        item->GetState());
                ...
                if (Bag* bag = item->ToBag()) {
                    for (uint8 j = 0; j < bag->GetBagSize(); ++j) {
                        ...
                        if (item2->GetSlot() != j) {
                            handler->PSendSysMessage(
                                "The item in bag %d and slot %d (guid: %d) has "
                                "an incorrect slot value: %d",
                                bag->GetSlot(),
                                j,
                                item2->GetGUID().GetCounter(),
                                item2->GetSlot());
                            error = true;
                            continue;
                        }
                        ...
                        if (item2->GetOwnerGUID() != player->GetGUID()) {
                            handler->PSendSysMessage(
                                "The item in bag %d at slot %d and %s, the "
                                "owner (%s) and the player (%s) don't match!",
                                bag->GetSlot(),
                                item2->GetSlot(),
                                item2->GetGUID().ToString().c_str(),
                                item2->GetOwnerGUID().ToString().c_str(),
                                player->GetGUID().ToString().c_str());
                            error = true;
                            continue;
                        }
                        ...
                        if (!container) {
                            handler->PSendSysMessage(
                                "The item in bag %d at slot %d %s has no "
                                "container!",
                                bag->GetSlot(),
                                item2->GetSlot(),
                                item2->GetGUID().ToString().c_str());
                        ...
                        if (container != bag) {
                            handler->PSendSysMessage(
                                "The item in bag %d at slot %d %s has a "
                                "different container(slot %d %s)!",
                                bag->GetSlot(),
                                item2->GetSlot(),
                                item2->GetGUID().ToString().c_str(),
                                container->GetSlot(),
                        ...
                        if (item2->IsInUpdateQueue()) {
                            ...
                            if (qp > updateQueue.size()) {
                                handler->PSendSysMessage(
                                    "The item in bag %d at slot %d having guid "
                                    "%d has a queuepos (%d) larger than the "
                                    "update queue size! ",
                                    bag->GetSlot(),
                                    item2->GetSlot(),
                                    item2->GetGUID().GetCounter(),
                            ...
                            if (!updateQueue[qp]) {
                                handler->PSendSysMessage(
                                    "The item in bag %d at slot %d having guid "
                                    "%d has a queuepos (%d) that points to "
                                    "NULL in the queue!",
                                    bag->GetSlot(),
                                    item2->GetSlot(),
                                    item2->GetGUID().GetCounter(),
                            ...
                            if (updateQueue[qp] != item2) {
                                handler->PSendSysMessage(
                                    "The item in bag %d at slot %d having guid "
                                    "%d has a queuepos (%d) that points to "
                                    "another item in the queue (bag: %d, slot: "
                                    "%d, guid: %d)",
                                    bag->GetSlot(),
                                    item2->GetSlot(),
                                    item2->GetGUID().GetCounter(),
                            ...
                            handler->PSendSysMessage(
                                "The item in bag %d at slot %d having guid %d "
                                "is not in queue but should be (state: %d)!",
                                bag->GetSlot(),
                                item2->GetSlot(),
                                item2->GetGUID().GetCounter(),
                                item2->GetState());
            ...
            for (auto const& item : updateQueue) {
                ...
                if (item->GetOwnerGUID() != player->GetGUID()) {
                    handler->SendSysMessage(Acore::StringFormatFmt(
                        "queue({}): For the item {}, the owner ({}) and the "
                        "player ({}) don't match!",
                        index,
                ...
                if (item->GetQueuePos() != index) {
                    handler->SendSysMessage(Acore::StringFormatFmt(
                        "queue({}): For the item {}, the queuepos doesn't "
                        "match it's position in the queue!",
                        index,
                        item->GetGUID().ToString()));
                    error = true;
                    continue;
                ...
                Item* test =
                    player->GetItemByPos(item->GetBagSlot(), item->GetSlot());

                if (!test) {
                    handler->SendSysMessage(Acore::StringFormatFmt(
                        "queue({}): The bag({}) and slot({}) values for {} are "
                        "incorrect, the player doesn't have any item at that "
                        "position!",
                        index,
                        item->GetBagSlot(),
                        item->GetSlot(),
                ...
                if (test != item) {
                    handler->SendSysMessage(Acore::StringFormatFmt(
                        "queue({}): The bag({}) and slot({}) values for the %s "
                        "are incorrect, {} is there instead!",
                        index,
                        item->GetBagSlot(),
                        item->GetSlot(),
                        item->GetGUID().ToString(),
    ...
    static bool HandleDebugThreatListCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage("Threat list of %s (%s)",
                                 target->GetName().c_str(),
        ...
        for (itr = threatList.begin(); itr != threatList.end(); ++itr) {
            ...
            if (!unit) {
                handler->PSendSysMessage("   %u.   No Unit  - threat %f",
                                         ++count,
                                         (*itr)->GetThreat());
                continue;
            }

            handler->PSendSysMessage("   %u.   %s   (%s)  - threat %f",
                                     ++count,
                                     unit->GetName().c_str(),
    ...
    static bool HandleDebugHostileRefListCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage("Hostile reference list of %s (%s)",
                                 target->GetName().c_str(),
                                 target->GetGUID().ToString().c_str());

        while (ref) {
            if (Unit* unit = ref->GetSource()->GetOwner()) {
                handler->PSendSysMessage("   %u.   %s %s   (%s)  - threat %f",
                                         ++count,
                ...
                handler->PSendSysMessage("   %u.   No Owner  - threat %f",
                                         ++count,
                                         ref->GetThreat());
            }
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler*   handler,
                                               uint32         entry,
                                               Optional<int8> seatId)
    {
        ...
        if (!entry)
            ...
            Acore::AllCreaturesOfEntryInRange check(
                handler->GetPlayer(), entry, 20.0f);
            Acore::CreatureSearcher<Acore::AllCreaturesOfEntryInRange> searcher(
                handler->GetPlayer(), passenger, check);
    ...
    static bool HandleDebugSpawnVehicleCommand(ChatHandler*     handler,
                                               uint32           entry,
                                               Optional<uint32> id)
    {
        float x, y, z, o = handler->GetPlayer()->GetOrientation();
        handler->GetPlayer()->GetClosePoint(
            x, y, z, handler->GetPlayer()->GetCombatReach());
    ...
    static bool HandleDebugSendLargePacketCommand(ChatHandler* handler)
    {
        ...
        while (ss.str().size() < 128000)
            ss << "This is a dummy string to push the packet's size beyond "
                  "128000 bytes. ";
    ...
    static bool HandleDebugSendSetPhaseShiftCommand(ChatHandler* handler,
                                                    uint32       phaseShift)
    {
    ...
    static bool HandleDebugGetItemValueCommand(ChatHandler*        handler,
                                               ObjectGuid::LowType guid,
                                               uint32              index)
    {
        Item* i = handler->GetPlayer()->GetItemByGuid(
    ...
    static bool HandleDebugSetItemValueCommand(ChatHandler*        handler,
                                               ObjectGuid::LowType guid,
                                               uint32              index,
                                               uint32              value)
    {
    ...
    static bool HandleDebugItemExpireCommand(ChatHandler*        handler,
                                             ObjectGuid::LowType guid)
    {
        Item* i = handler->GetPlayer()->GetItemByGuid(
    ...
    static bool HandleDebugAnimCommand(ChatHandler* handler, Emote emote)
    {
        ...
        handler->PSendSysMessage("Playing emote %s",
                                 EnumUtils::ToConstant(emote));
    ...
    static bool HandleDebugLoSCommand(ChatHandler* handler)
    {
        if (Unit* unit = handler->getSelectedUnit()) {
            Player* player = handler->GetSession()->GetPlayer();
            handler->PSendSysMessage("Checking LoS %s -> %s:",
    ...
    static bool HandleDebugSetAuraStateCommand(ChatHandler*            handler,
    ...
    static bool HandleDebugSetValueCommand(ChatHandler*           handler,
                                           uint32                 index,
                                           Variant<uint32, float> value)
    {
        ...
        if (value.holds_alternative<uint32>()) {
            ...
            handler->PSendSysMessage(LANG_SET_UINT_FIELD,
                                     target->GetGUID().GetCounter(),
                                     uint32(index),
                                     uint32(value));
        }
            ...
            handler->PSendSysMessage(LANG_SET_FLOAT_FIELD,
                                     target->GetGUID().GetCounter(),
    ...
    static bool
    HandleDebugGetValueCommand(ChatHandler* handler, uint32 index, bool isInt)
    {
        Unit* target = handler->getSelectedUnit();
        if (!target) {
            handler->SendErrorMessage(LANG_SELECT_CHAR_OR_CREATURE);
        ...
        if (isInt) {
            ...
            handler->PSendSysMessage(
                LANG_GET_UINT_FIELD, guid.GetCounter(), index, value);
        }
        else {
            float value = target->GetFloatValue(index);
            handler->PSendSysMessage(
                LANG_GET_FLOAT_FIELD, guid.GetCounter(), index, value);
        }

        return true;
    }

    static bool HandleDebugMod32ValueCommand(ChatHandler* handler,
                                             uint32       index,
                                             uint32       value)
    {
    ...
    static bool HandleDebugUpdateCommand(ChatHandler*     handler,
                                         uint32           index,
                                         Optional<uint32> value)
    {
        ...
        if (!value) {
            ...
            handler->PSendSysMessage(
                LANG_UPDATE, unit->GetGUID().GetCounter(), index, *value);
            return true;
        }
        ...
        handler->PSendSysMessage(
            LANG_UPDATE_CHANGE, unit->GetGUID().GetCounter(), index, *value);
        return true;
    }

    static bool
    HandleDebugSet32BitCommand(ChatHandler* handler, uint32 index, uint8 bit)
    {
        WorldObject* target = handler->getSelectedObject();
        if (!target) {
            handler->SendErrorMessage(LANG_SELECT_CHAR_OR_CREATURE);
    ...
    static bool HandleDebugMoveflagsCommand(ChatHandler*     handler,
                                            ...
                                            Optional<uint32> moveFlagsExtra)
    {
        ...
        if (!moveFlags) {
            ...
            if (*moveFlags &
                (MOVEMENTFLAG_DISABLE_GRAVITY | MOVEMENTFLAG_CAN_FLY))
            ...
            if (target->HasUnitMovementFlag(MOVEMENTFLAG_SPLINE_ENABLED) &&
            ...
            handler->PSendSysMessage(LANG_MOVEFLAGS_SET,
                                     target->GetUnitMovementFlags(),
                                     target->GetExtraUnitMovementFlags());
        }
    ...
    static bool HandleDebugUnitStateCommand(ChatHandler* handler,
                                            uint32       unitState)
    {
        Unit* target = handler->getSelectedUnit();
        if (!target)
                                   ...
                                   Optional<std::string> type)
    {
        ...
        if (!type) {
            // waypoint_data - id, point, X, Y, Z, O, delay, move_type, action,
    ...
    static bool HandleDebugObjectCountCommand(ChatHandler*     handler,
                                              Optional<uint32> mapId)
    {
        if (mapId) {
            sMapMgr->DoForAllMapsWithMapId(
                mapId.value(), [handler](Map* map) -> void {
                    HandleDebugObjectCountMap(handler, map);
                });
        }
        else {
            sMapMgr->DoForAllMaps([handler](Map* map) -> void {
                HandleDebugObjectCountMap(handler, map);
            });
        }
    ...
    public:
        ...
        {
            auto comp = [](std::pair<uint32, uint32> const& a,
                           std::pair<uint32, uint32> const& b) {
                return a.second > b.second;
            };
            std::set<std::pair<uint32, uint32>, decltype(comp)> set(
                creatureIds.begin(), creatureIds.end(), comp);
    ...
    {
        handler->PSendSysMessage(
            "Map Id: %u Name: '%s' Instance Id: %u Creatures: %u GameObjects: "
            "%u SetActive Objects: %u",
            map->GetId(),
            map->GetMapName(),
            map->GetInstanceId(),
        ...
        TypeContainerVisitor<CreatureCountWorker, MapStoredObjectTypesContainer>
            visitor(worker);
    ...
    static bool HandleDebugDummyCommand(ChatHandler* handler)
    {
        handler->SendSysMessage(
            "This command does nothing right now. Edit your local core "
            "(cs_debug.cpp) to make it do whatever you need for testing.");
        return true;
    }
};

void AddSC_debug_commandscript() { new debug_commandscript(); }
