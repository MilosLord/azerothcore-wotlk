...
public:
    ...
    /**
     * Collects all GUIDs (and related info) from deleted characters which are
     * still in the database.
     *
     * @param foundList    a reference to an std::list which will be filled with
     * info data
     * @param searchString the search string which either contains a player GUID
     * or a part fo the character-name
     * @return             returns false if there was a problem while selecting
     * the characters (e.g. player name not normalizeable)
     */
    static bool GetDeletedCharacterInfoList(DeletedInfoList& foundList,
                                            std::string      searchString)
    {
        ...
        if (!searchString.empty()) {
            ...
            stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_DEL_INFO);
            result = CharacterDatabase.Query(stmt);
        }
    ...
    /**
     * Shows all deleted characters which matches the given search string,
     ...
     */
    static void
    HandleCharacterDeletedListHelper(DeletedInfoList const& foundList,
                                     ChatHandler*           handler)
    {
        ...
        for (DeletedInfoList::const_iterator itr = foundList.begin();
             itr != foundList.end();
             ++itr) {
            std::string dateStr =
                Acore::Time::TimeToTimestampStr(Seconds(itr->deleteDate));
            ...
            if (!handler->GetSession())
                handler->PSendSysMessage(
                    ...
                    itr->lowGuid,
                    itr->name.c_str(),
                    itr->accountName.empty() ? "<Not existing>"
                                             : itr->accountName.c_str(),
                    itr->accountId,
                    dateStr.c_str());
            else
     ...
     * @see DeletedInfoList
     *
     * @param delInfo the informations about the character which will be
     * restored
     */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo,
                                                    ChatHandler*       handler)
    {
        if (delInfo.accountName.empty()) // account does not exist
        {
            handler->PSendSysMessage(LANG_CHARACTER_DELETED_SKIP_ACCOUNT,
                                     delInfo.name.c_str(),
                                     delInfo.lowGuid,
                                     delInfo.accountId);
            return;
        ...
        if (charcount >= 10) {
            handler->PSendSysMessage(LANG_CHARACTER_DELETED_SKIP_FULL,
                                     delInfo.name.c_str(),
                                     delInfo.lowGuid,
                                     delInfo.accountId);
            return;
        ...
        if (sCharacterCache->GetCharacterGuidByName(delInfo.name)) {
            handler->PSendSysMessage(LANG_CHARACTER_DELETED_SKIP_NAME,
                                     delInfo.name.c_str(),
                                     delInfo.lowGuid,
                                     delInfo.accountId);
            return;
        ...
        auto* stmt = CharacterDatabase.GetPreparedStatement(
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt)) {
            sCharacterCache->AddCharacterCacheEntry(
                ObjectGuid(HighGuid::Player, delInfo.lowGuid),
                delInfo.accountId,
                delInfo.name,
                (*result)[2].Get<uint8>(),
                (*result)[0].Get<uint8>(),
    ...
    static void HandleCharacterLevel(Player*      player,
                                     ...
                                     ChatHandler* handler)
    {
        if (player) {
            ...
            // Update level and reset XP, everything else will be updated at
            // login
    ...
    static bool HandleCharacterTitlesCommand(ChatHandler*               handler,
                                             Optional<PlayerIdentifier> player)
    {
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++) {
            ...
            if (titleInfo && target->HasTitle(titleInfo)) {
                char const* name = target->getGender() == GENDER_MALE
                                       ? titleInfo->nameMale[loc]
                                       : titleInfo->nameFemale[loc];
                if (!*name)
                    name =
                        (target->getGender() == GENDER_MALE
                             ? titleInfo
                                   ->nameMale[sWorld->GetDefaultDbcLocale()]
                             : titleInfo
                                   ->nameFemale[sWorld->GetDefaultDbcLocale()]);

                if (!*name)
                    continue;

                char const* activeStr = "";
                if (target->GetUInt32Value(PLAYER_CHOSEN_TITLE) ==
                    titleInfo->bit_index)
                    activeStr = handler->GetAcoreString(LANG_ACTIVE);

                std::string titleName =
                    Acore::StringFormat(name, player->GetName().c_str());

                // send title in "id (idx:idx) - [namedlink locale]" format
                if (handler->GetSession())
                    handler->PSendSysMessage(LANG_TITLE_LIST_CHAT,
                                             id,
                                             titleInfo->bit_index,
                                             id,
                                             titleName.c_str(),
                    ...
                    handler->PSendSysMessage(LANG_TITLE_LIST_CONSOLE,
                                             id,
                                             titleInfo->bit_index,
                                             name,
                                             localeNames[loc],
    ...
    // rename characters
    static bool
    HandleCharacterRenameCommand(ChatHandler*               handler,
                                 Optional<PlayerIdentifier> player,
                                 ...
                                 Optional<std::string_view> newNameV)
    {
        ...
        if (newNameV) {
            ...
            if (ObjectMgr::CheckPlayerName(newName, true) !=
                CHAR_NAME_SUCCESS) {
                handler->SendErrorMessage(LANG_BAD_VALUE);
                return false;
            }
            ...
            if (result) {
                handler->SendErrorMessage(LANG_RENAME_PLAYER_ALREADY_EXISTS,
                                          newName.c_str());
                return false;
            }
            ...
            if (Player* target = player->GetConnectedPlayer()) {
                ...
                ObjectAccessor::UpdatePlayerNameMapReference(player->GetName(),
                                                             target);

                if (WorldSession* session = target->GetSession())
                    session->KickPlayer("HandleCharacterRenameCommand GM "
                                        "Command renaming character");
            }
            else {
                stmt = CharacterDatabase.GetPreparedStatement(
            ...
            handler->PSendSysMessage(LANG_RENAME_PLAYER_WITH_NEW_NAME,
                                     player->GetName().c_str(),
                                     newName.c_str());
        }
        else {
            if (Player* target = player->GetConnectedPlayer()) {
                handler->PSendSysMessage(LANG_RENAME_PLAYER,
                ...
                handler->PSendSysMessage(LANG_RENAME_PLAYER_GUID,
                                         handler->playerLink(*player).c_str(),
    ...
    static bool HandleCharacterLevelCommand(ChatHandler*               handler,
                                            Optional<PlayerIdentifier> player,
                                            int16                      newlevel)
    {
        if (!player)
        ...
        uint8 oldlevel =
            player->IsConnected()
                ? player->GetConnectedPlayer()->GetLevel()
                : sCharacterCache->GetCharacterLevelByGuid(player->GetGUID());
        ...
        HandleCharacterLevel(player->GetConnectedPlayer(),
                             player->GetGUID(),
                             oldlevel,
                             newlevel,
                             handler);
        ...
        if (!handler->GetSession() ||
            (handler->GetSession()->GetPlayer() !=
             player->GetConnectedPlayer())) // including chr == NULL
            handler->PSendSysMessage(LANG_YOU_CHANGE_LVL,
                                     handler->playerLink(*player).c_str(),
                                     newlevel);
    ...
    static bool
    HandleCharacterCustomizeCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> player)
    {
        if (!player)
            player = PlayerIdentifier::FromTarget(handler);
        ...
        if (Player* target = player->GetConnectedPlayer()) {
            handler->PSendSysMessage(LANG_CUSTOMIZE_PLAYER,
                                     handler->GetNameLink(target).c_str());
            target->SetAtLoginFlag(AT_LOGIN_CUSTOMIZE);
        }
        else {
            handler->PSendSysMessage(LANG_CUSTOMIZE_PLAYER_GUID,
                                     handler->playerLink(*player).c_str(),
                                     player->GetGUID().GetCounter());
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(
    ...
    static bool
    HandleCharacterChangeFactionCommand(ChatHandler*               handler,
                                        Optional<PlayerIdentifier> player)
    {
        if (!player)
            player = PlayerIdentifier::FromTarget(handler);
    ...
    static bool
    HandleCharacterChangeRaceCommand(ChatHandler*               handler,
                                     Optional<PlayerIdentifier> player)
    {
        if (!player)
            player = PlayerIdentifier::FromTarget(handler);
    ...
    static bool
    HandleCharacterReputationCommand(ChatHandler*               handler,
                                     Optional<PlayerIdentifier> player)
    {
        if (!player)
            player = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        for (FactionStateList::const_iterator itr = targetFSL.begin();
             itr != targetFSL.end();
             ++itr) {
            FactionState const& faction = itr->second;
            FactionEntry const* factionEntry =
                sFactionStore.LookupEntry(faction.ID);
            char const* factionName =
                factionEntry ? factionEntry->name[loc] : "#Not found#";
            ReputationRank rank =
                target->GetReputationMgr().GetRank(factionEntry);
            std::string rankName =
                handler->GetAcoreString(ReputationRankStrIndex[rank]);
            std::ostringstream ss;
            if (handler->GetSession())
                ss << faction.ID << " - |cffffffff|Hfaction:" << faction.ID
                   << "|h[" << factionName << ' ' << localeNames[loc]
                   << "]|h|r";
            else
                ss << faction.ID << " - " << factionName << ' '
                   << localeNames[loc];
    ...
    /**
     * Handles the '.character deleted list' command, which shows all deleted
     * characters which matches the given search string
     *
     * @see HandleCharacterDeletedListHelper
     * @see HandleCharacterDeletedRestoreCommand
     * @see HandleCharacterDeletedDeleteCommand
     * @see DeletedInfoList
     *
     * @param args the search string which either contains a player GUID or a
     * part fo the character-name
     */

    static bool
    HandleCharacterDeletedListCommand(ChatHandler*               handler,
    ...
    /**
     * Handles the '.character deleted restore' command, which restores all
     * deleted characters which matches the given search string
     *
     * The command automatically calls '.character deleted list' command with
     * the search string to show all restored characters.
     *
     ...
     *
     * @param args the search string which either contains a player GUID or a
     * part of the character-name
     */
    static bool
    HandleCharacterDeletedRestoreCommand(ChatHandler*               handler,
                                         std::string                needle,
                                         ...
                                         Optional<AccountIdentifier> newAccount)
    {
        ...
        if (!newCharName) {
            ...
            for (DeletedInfoList::iterator itr = foundList.begin();
                 itr != foundList.end();
                 ++itr)
    ...
    /**
     * Handles the '.character deleted delete' command, which completely deletes
     * all deleted characters which matches the given search string
     *
     ...
     * part fo the character-name
     */
    static bool HandleCharacterDeletedDeleteCommand(ChatHandler* handler,
                                                    std::string  needle)
    {
        DeletedInfoList foundList;
        if (!GetDeletedCharacterInfoList(foundList, needle))
        ...
        // Call the appropriate function to delete them (current account for
        // deleted characters is 0)
        for (DeletedInfoList::const_iterator itr = foundList.begin();
             itr != foundList.end();
             ++itr)
    ...
    /**
     * Handles the '.character deleted old' command, which completely deletes
     * all deleted characters deleted with some days ago
     *
     ...
     * part of the character-name
     */
    static bool HandleCharacterDeletedPurgeCommand(ChatHandler* /*handler*/,
                                                   Optional<uint16> days)
    {
        int32 keepDays = static_cast<int32>(
            sWorld->getIntConfig(CONFIG_CHARDELETE_KEEP_DAYS));

        if (days)
            keepDays = static_cast<int32>(*days);
        else if (keepDays <=
                 0) // config option value 0 -> disabled and can't be used
            return false;
    ...
    /**
     * Handles the '.character erase' command which completly delete a character
     * from the DB
     *
     * @see Player::DeleteFromDB
     *
     ...
     * part of the character-name
     */
    static bool HandleCharacterEraseCommand(ChatHandler*     handler,
                                            PlayerIdentifier player)
    {
        ...
        Player::DeleteFromDB(
            player.GetGUID().GetCounter(), accountId, true, true);
        handler->PSendSysMessage(LANG_CHARACTER_DELETED,
                                 player.GetName().c_str(),
                                 player.GetGUID().GetCounter(),
                                 accountName.c_str(),
                                 accountId);
    ...
    static bool HandleLevelUpCommand(ChatHandler*               handler,
                                     Optional<PlayerIdentifier> player,
                                     int16                      level)
    {
        if (!player)
    ...
    static bool ValidatePDumpTarget(ChatHandler*                  handler,
                                    std::string&                  name,
                                    ...
                                    Optional<ObjectGuid::LowType> characterGUID)
    {
        ...
        if (characterGUID) {
            if (sCharacterCache->GetCharacterAccountIdByGuid(
                    ObjectGuid(HighGuid::Player, *characterGUID))) {
                handler->SendErrorMessage(LANG_CHARACTER_GUID_IN_USE,
                                          *characterGUID);
                return false;
            }
        }
    ...
    static bool
    HandlePDumpLoadCommand(ChatHandler*                  handler,
                           std::string                   fileName,
                           ...
                           Optional<ObjectGuid::LowType> characterGUID)
    {
        ...
        switch (PlayerDumpReader().LoadDumpFromFile(
            fileName, account, name, characterGUID.value_or(0))) {
        case DUMP_SUCCESS:
        ...
        case DUMP_TOO_MANY_CHARS:
            handler->SendErrorMessage(LANG_ACCOUNT_CHARACTER_LIST_FULL,
    ...
    static bool
    HandlePDumpCopyCommand(ChatHandler*                  handler,
                           PlayerIdentifier              player,
                           AccountIdentifier             account,
                           Optional<std::string_view>    characterName,
                           Optional<ObjectGuid::LowType> characterGUID)
    {
        ...
        std::string dump;
        switch (PlayerDumpWriter().WriteDumpToString(
            dump, player.GetGUID().GetCounter())) {
        case DUMP_SUCCESS:
            break;
        ...
        switch (PlayerDumpReader().LoadDumpFromString(
            dump, account, name, characterGUID.value_or(0))) {
        case DUMP_SUCCESS:
            break;
        case DUMP_TOO_MANY_CHARS:
            handler->SendErrorMessage(LANG_ACCOUNT_CHARACTER_LIST_FULL,
    ...
    static bool HandlePDumpWriteCommand(ChatHandler*     handler,
                                        std::string      fileName,
                                        PlayerIdentifier player)
    {
        switch (PlayerDumpWriter().WriteDumpToFile(
            fileName, player.GetGUID().GetCounter())) {
        case DUMP_SUCCESS:
    ...
    {
        handler->GetSession()->SendShowBank(
            handler->GetSession()->GetPlayer()->GetGUID());
        return true;
    }

    static bool HandleCharacterCheckBagCommand(ChatHandler* handler,
                                               uint8        BagSlot)
    {
        ...
        if (BagSlot == 1) {
            for (uint32 i = 23; i < 39; i++) {
                if (Item* item =
                        target->GetItemByPos(INVENTORY_SLOT_BAG_0, i)) {
                    Counter++;
                    std::ostringstream ItemString;
                    ItemString
                        << std::hex
                        << ItemQualityColors[item->GetTemplate()->Quality];

                    handler->PSendSysMessage(
                        "%u - |c%s|Hitem:%u:0:0:0:0:0:0:0:0:0|h[%s]|h|r - %u",
                        Counter,
                        ItemString.str().c_str(),
                        item->GetEntry(),
            ...
            if (Bag* bag = target->GetBagByPos(BagSlot)) {
                for (uint32 i = 0; i < bag->GetBagSize(); i++) {
                    if (Item* item = target->GetItemByPos(BagSlot, i)) {
                        ...
                        handler->PSendSysMessage(
                            "%u - |c%s|Hitem:%u:0:0:0:0:0:0:0:0:0|h[%s]|h|r - "
                            "%u",
                            Counter,
                            ItemString.str().c_str(),
                            item->GetEntry(),
    ...
    static bool HandleCharacterCheckProfessionCommand(ChatHandler* handler)
    {
        ...
        for (uint32 i = 1; i < sSkillLineStore.GetNumRows(); ++i) {
            ...
            if (player->HasSkill(SkillID)) {
                ...
                switch (SkillID) {
                case SKILL_ALCHEMY:
                    handler->PSendSysMessage("%u - Alchemy - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_BLACKSMITHING:
                    handler->PSendSysMessage("%u - Blacksmithing - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_ENCHANTING:
                    handler->PSendSysMessage("%u - Enchanting - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_ENGINEERING:
                    handler->PSendSysMessage("%u - Engineering - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_INSCRIPTION:
                    handler->PSendSysMessage("%u - Inscription - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_JEWELCRAFTING:
                    handler->PSendSysMessage("%u - Jewelcrafting - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_LEATHERWORKING:
                    handler->PSendSysMessage("%u - Leatherworking - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_TAILORING:
                    handler->PSendSysMessage("%u - Tailoring - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_SKINNING:
                    handler->PSendSysMessage("%u - Skinning - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_HERBALISM:
                    handler->PSendSysMessage("%u - Herbalism - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_MINING:
                    handler->PSendSysMessage("%u - Mining - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_COOKING:
                    handler->PSendSysMessage("%u - Cooking - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                case SKILL_FIRST_AID:
                    handler->PSendSysMessage("%u - First Aid - %u",
                                             Counter,
                                             player->GetSkillValue(SkillID));
                    break;
                default:
    ...
    static bool
    HandleCharacterChangeAccountCommand(ChatHandler*               handler,
                                        std::string                accountName,
                                        Optional<PlayerIdentifier> player)
    {
        if (!player) {
        ...
        if (uint32 accountId = AccountMgr::GetId(accountName)) {
            if (AccountMgr::GetCharactersCount(accountId) >= 10) {
                handler->SendErrorMessage(
                    LANG_ACCOUNT_CHARACTER_LIST_FULL, accountName, accountId);
                return true;
            }

            if (CharacterCacheEntry const* cache =
                    sCharacterCache->GetCharacterCacheByName(
                        player->GetName())) {
                std::string accName;
                AccountMgr::GetName(cache->AccountId, accName);
                handler->PSendSysMessage(LANG_CMD_CHAR_CHANGE_ACC_SUCCESS,
                                         player->GetName(),
                                         player->GetGUID().GetCounter(),
                                         accName,
                                         cache->AccountId,
            ...
            if (player->IsConnected()) {
                player->GetConnectedPlayer()->GetSession()->KickPlayer(
                    "CMD char changeaccount");
            }
            ...
            CharacterDatabase.Query(
                ...
                accountId,
                player->GetGUID().GetCounter());
            sCharacterCache->UpdateCharacterAccountId(player->GetGUID(),
                                                      accountId);
        }
        else {
...
void AddSC_character_commandscript() { new character_commandscript(); }
