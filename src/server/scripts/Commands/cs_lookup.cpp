...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"lookup", lookupCommandTable}};
    ...
    static bool HandleLookupAreaCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        uint32 count = 0;
        uint32 maxResults =
            sWorld->getIntConfig(CONFIG_MAX_RESULTS_LOOKUP_COMMANDS);
        ...
        for (auto areaEntry : sAreaTableStore) {
            ...
            if (locale < TOTAL_LOCALES) {
                if (maxResults && count++ == maxResults) {
                    handler->PSendSysMessage(LANG_COMMAND_LOOKUP_MAX_RESULTS,
                                             maxResults);
                    return true;
                }
    ...
    static bool HandleLookupCreatureCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (auto const& [entry, creatureTemplate] :
             *sObjectMgr->GetCreatureTemplates()) {
            uint32 id          = creatureTemplate.Entry;
            uint8  localeIndex = handler->GetSessionDbLocaleIndex();
            if (CreatureLocale const* creatureLocale =
                    sObjectMgr->GetCreatureLocale(id)) {
                if (creatureLocale->Name.size() > localeIndex &&
                    !creatureLocale->Name[localeIndex].empty()) {
                    std::string name = creatureLocale->Name[localeIndex];

                    if (Utf8FitTo(name, wNamePart)) {
                        if (maxResults && count++ == maxResults) {
                            handler->PSendSysMessage(
                                LANG_COMMAND_LOOKUP_MAX_RESULTS, maxResults);
                            return true;
                        }
            ...
            if (Utf8FitTo(name, wNamePart)) {
                ...
                if (handler->GetSession()) {
                    handler->PSendSysMessage(
                        LANG_CREATURE_ENTRY_LIST_CHAT, id, id, name.c_str());
                }
                else {
                    handler->PSendSysMessage(
                        LANG_CREATURE_ENTRY_LIST_CONSOLE, id, name.c_str());
                }
    ...
    static bool HandleLookupEventCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (uint32 id = 0; id < events.size(); ++id) {
            ...
            if (Utf8FitTo(descr, wNamePart)) {
                ...
                char const* active = activeEvents.find(id) != activeEvents.end()
                                         ? handler->GetAcoreString(LANG_ACTIVE)
                                         : "";

                if (handler->GetSession()) {
                    handler->PSendSysMessage(LANG_EVENT_ENTRY_LIST_CHAT,
                                             id,
                                             id,
                    ...
                    handler->PSendSysMessage(LANG_EVENT_ENTRY_LIST_CONSOLE,
                                             id,
                                             eventData.description.c_str(),
                                             active);
                }
    ...
    static bool HandleLookupFactionCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (auto factionEntry : sFactionStore) {
            FactionState const* factionState =
            ...
            if (locale < TOTAL_LOCALES) {
                ...
                // send faction in "id - [faction] rank reputation [visible] [at
                // war] [own team] [unknown] [invisible] [inactive]" format or
                // "id - [faction] [no reputation]" format
                std::ostringstream ss;
                if (handler->GetSession()) {
                    ss << factionEntry->ID
                       << " - |cffffffff|Hfaction:" << factionEntry->ID << "|h["
                       << name << ' ' << localeNames[locale] << "]|h|r";
                }
                else {
                    ss << factionEntry->ID << " - " << name << ' '
                       << localeNames[locale];
                }
                ...
                {
                    uint32 index =
                        target->GetReputationMgr().GetReputationRankStrIndex(
                            factionEntry);
    ...
    static bool HandleLookupItemCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (auto const& [entry, itemTemplate] :
            ...
            if (localeIndex >= 0) {
                ...
                if (ItemLocale const* il =
                    ...
                    if (il->Name.size() > ulocaleIndex &&
                        !il->Name[ulocaleIndex].empty()) {
                        ...
                        if (Utf8FitTo(name, wNamePart)) {
                            ...
                            if (handler->GetSession()) {
                                handler->PSendSysMessage(LANG_ITEM_LIST_CHAT,
                                                         itemTemplate.ItemId,
                                                         itemTemplate.ItemId,
                                                         name.c_str());
                            }
                            else {
                                handler->PSendSysMessage(LANG_ITEM_LIST_CONSOLE,
                                                         itemTemplate.ItemId,
                                                         name.c_str());
                            }
        ...
        for (uint32 id = 0; id < sItemSetStore.GetNumRows(); id++) {
            ...
            if (set) {
                ...
                if (locale < TOTAL_LOCALES) {
                    ...
                    if (handler->GetSession()) {
                        handler->PSendSysMessage(LANG_ITEMSET_LIST_CHAT,
                                                 id,
                                                 id,
                                                 name.c_str(),
                                                 localeNames[locale]);
                        ...
                        handler->PSendSysMessage(LANG_ITEMSET_LIST_CONSOLE,
                                                 id,
                                                 name.c_str(),
                                                 localeNames[locale]);
                    }
    ...
    static bool HandleLookupObjectCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (auto const& [entry, gameObjectTemplate] :
             *sObjectMgr->GetGameObjectTemplates()) {
            uint8 localeIndex = handler->GetSessionDbLocaleIndex();
            if (GameObjectLocale const* objectLocalte =
                    sObjectMgr->GetGameObjectLocale(gameObjectTemplate.entry)) {
                if (objectLocalte->Name.size() > localeIndex &&
                    !objectLocalte->Name[localeIndex].empty()) {
                    ...
                    if (Utf8FitTo(name, wNamePart)) {
                        ...
                        if (handler->GetSession()) {
                            handler->PSendSysMessage(LANG_GO_ENTRY_LIST_CHAT,
                                                     gameObjectTemplate.entry,
                                                     gameObjectTemplate.entry,
                                                     name.c_str());
                        }
                        else {
                            handler->PSendSysMessage(LANG_GO_ENTRY_LIST_CONSOLE,
                                                     gameObjectTemplate.entry,
                                                     name.c_str());
                        }
        ...
        for (auto const& [entry, qInfo] : sObjectMgr->GetQuestTemplates()) {
            ...
            if (localeIndex >= 0) {
                ...
                if (QuestLocale const* questLocale =
                        sObjectMgr->GetQuestLocale(qInfo->GetQuestId())) {
                    if (questLocale->Title.size() > ulocaleIndex &&
                        !questLocale->Title[ulocaleIndex].empty()) {
                        ...
                        if (Utf8FitTo(title, wNamePart)) {
                            ...
                            if (target) {
                                QuestStatus status =
                                    target->GetQuestStatus(qInfo->GetQuestId());

                                switch (status) {
                                case QUEST_STATUS_COMPLETE:
                                    statusStr = handler->GetAcoreString(
                            ...
                            if (handler->GetSession()) {
                                handler->PSendSysMessage(LANG_QUEST_LIST_CHAT,
                                                         qInfo->GetQuestId(),
                                                         qInfo->GetQuestId(),
                                                         qInfo->GetQuestLevel(),
                                                         title.c_str(),
            ...
            if (Utf8FitTo(title, wNamePart)) {
                ...
                if (target) {
                    ...
                    case QUEST_STATUS_INCOMPLETE:
                        statusStr =
                            handler->GetAcoreString(LANG_COMMAND_QUEST_ACTIVE);
                        break;
                    case QUEST_STATUS_REWARDED:
                        statusStr = handler->GetAcoreString(
                            LANG_COMMAND_QUEST_REWARDED);
                ...
                if (handler->GetSession()) {
                    ...
                    handler->PSendSysMessage(LANG_QUEST_LIST_CONSOLE,
                                             qInfo->GetQuestId(),
                                             title.c_str(),
                                             statusStr);
                }
        ...
        for (auto skillInfo : sSkillLineStore) {
            ...
            if (locale < TOTAL_LOCALES) {
                ...
                if (target && target->HasSkill(skillInfo->id)) {
                    ...
                    valStr = Acore::StringFormat(
                        handler->GetAcoreString(LANG_SKILL_VALUES),
                        curValue,
                        maxValue,
                        permValue,
                        tempValue);
                ...
                if (handler->GetSession()) {
                    handler->PSendSysMessage(LANG_SKILL_LIST_CHAT,
                                             skillInfo->id,
                                             skillInfo->id,
                                             name.c_str(),
                                             localeNames[locale],
                    ...
                    handler->PSendSysMessage(LANG_SKILL_LIST_CONSOLE,
                                             skillInfo->id,
                                             name.c_str(),
                                             localeNames[locale],
                                             knownStr.c_str(),
        ...
        for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); id++) {
            ...
            if (spellInfo) {
                ...
                if (locale < TOTAL_LOCALES) {
                    ...
                    bool learn = (spellInfo->Effects[0].Effect ==
                                  SPELL_EFFECT_LEARN_SPELL);

                    SpellInfo const* learnSpellInfo = sSpellMgr->GetSpellInfo(
                        spellInfo->Effects[0].TriggerSpell);
                    ...
                    // unit32 used to prevent interpreting uint8 as char at
                    // output find rank of learned spell for learning spell, or
                    // talent rank
                    uint32 rank = talentCost ? talentCost
                                  : learn && learnSpellInfo
                                      ? learnSpellInfo->GetRank()
                                      : spellInfo->GetRank();
    ...
    static bool HandleLookupSpellIdCommand(ChatHandler*     handler,
                                           SpellInfo const* spell)
    {
        ...
        if (locale < TOTAL_LOCALES) {
            ...
            SpellInfo const* learnSpellInfo =
                sSpellMgr->GetSpellInfo(spell->Effects[0].TriggerSpell);
            ...
            uint32 rank = talentCost                ? talentCost
                          : learn && learnSpellInfo ? learnSpellInfo->GetRank()
                                                    : spell->GetRank();

            // send spell in "id - [name, rank N] [talent] [passive] [learn]
            // [known]" format
            std::ostringstream ss;
            if (handler->GetSession()) {
                ss << spell->Id << " - |cffffffff|Hspell:" << spell->Id << "|h["
                   << name;
            }
            else {
                ss << spell->Id << " - " << name;
    ...
    static bool HandleLookupTaxiNodeCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (auto nodeEntry : sTaxiNodesStore) {
            ...
            if (locale < TOTAL_LOCALES) {
                ...
                if (handler->GetSession()) {
                    handler->PSendSysMessage(LANG_TAXINODE_ENTRY_LIST_CHAT,
                                             nodeEntry->ID,
                                             nodeEntry->ID,
                                             name.c_str(),
                                             localeNames[locale],
                    ...
                    handler->PSendSysMessage(LANG_TAXINODE_ENTRY_LIST_CONSOLE,
                                             nodeEntry->ID,
                                             name.c_str(),
                                             localeNames[locale],
                                             nodeEntry->map_id,
    ...
    static bool HandleLookupTeleCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        for (auto const& [id, tele] : sObjectMgr->GetGameTeleMap()) {
            ...
            if (handler->GetSession()) {
                reply << "  |cffffffff|Htele:" << id << "|h[" << tele.name
                      << "]|h|r\n";
            }
            else {
                reply << "  " << id << ' ' << tele.name << "\n";
        ...
        if (reply.str().empty()) {
            ...
            handler->PSendSysMessage(LANG_COMMAND_TELE_LOCATION,
                                     reply.str().c_str());
        }
        ...
        for (auto titleInfo : sCharTitlesStore) {
            ...
            if (locale < TOTAL_LOCALES) {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo)
                                           ? handler->GetAcoreString(LANG_KNOWN)
                                           : "";
                char const* activeStr =
                    target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) ==
                                  titleInfo->bit_index
                        ? handler->GetAcoreString(LANG_ACTIVE)
                        : "";

                std::string titleNameStr =
                    Acore::StringFormat(name, targetName);
    ...
    static bool HandleLookupMapCommand(ChatHandler* handler, Tail namePart)
    {
        ...
        uint32 maxResults =
            sWorld->getIntConfig(CONFIG_MAX_RESULTS_LOOKUP_COMMANDS);
        uint8 locale = handler->GetSession()
    ...
    static bool HandleLookupPlayerIpCommand(ChatHandler*    handler,
                                            Optional<Tail>  ip,
                                            Optional<int32> limit)
    {
        ...
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BY_IP);
        stmt->SetData(0, *ip);
    ...
    static bool HandleLookupPlayerAccountCommand(ChatHandler*    handler,
                                                 std::string     account,
                                                 Optional<int32> limit)
    {
        ...
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_LIST_BY_NAME);
    ...
    static bool HandleLookupPlayerEmailCommand(ChatHandler*    handler,
                                               std::string     email,
                                               Optional<int32> limit)
    {
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_LIST_BY_EMAIL);
    ...
    static bool LookupPlayerSearchCommand(PreparedQueryResult result,
                                          int32               limit,
                                          ChatHandler*        handler)
    {
        if (!result) {
        ...
        do {
            ...
            if (result2) {
                handler->PSendSysMessage(
                    LANG_LOOKUP_PLAYER_ACCOUNT, accountName.c_str(), accountId);
                ...
                do {
                    ...
                    bool        online =
                        ObjectAccessor::FindPlayerByLowGUID(guid) != nullptr;
                    ...
                    if (plevel > 0 && prace > 0 && prace <= RACE_DRAENEI &&
                        pclass > 0 && pclass <= CLASS_DRUID) {
                        handler->PSendSysMessage(
                            "  %s (GUID %u) - %s - %s - %u%s",
                            name.c_str(),
                            guid,
                            EnumUtils::ToTitle(Races(prace)),
                            EnumUtils::ToTitle(Classes(pclass)),
                            plevel,
                            (online ? " - [ONLINE]" : ""));
                    }
                    else {
                        handler->PSendSysMessage(
                            LANG_LOOKUP_PLAYER_CHARACTER, name.c_str(), guid);
                    }
...
void AddSC_lookup_commandscript() { new lookup_commandscript(); }
