...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"arena", arenaCommandTable}};

        return commandTable;
    }

    static bool HandleArenaCreateCommand(ChatHandler*               handler,
                                         Optional<PlayerIdentifier> captain,
                                         QuotedString               name,
                                         ArenaTeamTypes             type)
    {
        if (sArenaTeamMgr->GetArenaTeamByName(name)) {
            handler->SendErrorMessage(LANG_ARENA_ERROR_NAME_EXISTS,
                                      name.c_str());
            return false;
        }
        ...
        if (Player::GetArenaTeamIdFromDB(captain->GetGUID(), type) != 0) {
            handler->SendErrorMessage(LANG_ARENA_ERROR_SIZE,
                                      captain->GetName().c_str());
            return false;
        }

        ArenaTeam* arena = new ArenaTeam();

        if (!arena->Create(captain->GetGUID(),
                           type,
                           name,
                           4293102085,
                           101,
        ...
        handler->PSendSysMessage(LANG_ARENA_CREATE,
                                 arena->GetName().c_str(),
                                 arena->GetId(),
                                 arena->GetType(),
                                 arena->GetCaptain().GetCounter());
    ...
    static bool HandleArenaRenameCommand(ChatHandler* handler,
                                         QuotedString oldName,
                                         QuotedString newName)
    {
        ArenaTeam* arena = sArenaTeamMgr->GetArenaTeamByName(oldName);
        if (!arena) {
            handler->SendErrorMessage(LANG_ARENA_ERROR_NAME_NOT_FOUND,
                                      oldName.c_str());
            return false;
        }
        ...
        handler->PSendSysMessage(LANG_ARENA_RENAME,
                                 arena->GetId(),
                                 oldName.c_str(),
                                 newName.c_str());
    ...
    static bool HandleArenaCaptainCommand(ChatHandler*               handler,
                                          uint32                     teamId,
                                          Optional<PlayerIdentifier> target)
    {
        ...
        if (!arena->IsMember(target->GetGUID())) {
            handler->SendErrorMessage(LANG_ARENA_ERROR_NOT_MEMBER,
        ...
        if (arena->GetCaptain() == target->GetGUID()) {
            handler->SendErrorMessage(LANG_ARENA_ERROR_CAPTAIN,
                                      target->GetName().c_str(),
                                      arena->GetName().c_str());
            return false;
        }
        ...
        sCharacterCache->GetCharacterNameByGuid(arena->GetCaptain(),
                                                oldCaptainName);
        arena->SetCaptain(target->GetGUID());

        handler->PSendSysMessage(LANG_ARENA_CAPTAIN,
                                 arena->GetName().c_str(),
                                 arena->GetId(),
                                 oldCaptainName.c_str(),
                                 target->GetName().c_str());
    ...
    static bool HandleArenaLookupCommand(ChatHandler* handler, Tail needle)
    {
        ...
        for (auto [teamId, team] : sArenaTeamMgr->GetArenaTeams()) {
            if (StringContainsStringI(team->GetName(), needle)) {
                if (handler->GetSession()) {
                    handler->PSendSysMessage(LANG_ARENA_LOOKUP,
                                             team->GetName().c_str(),
                                             team->GetId(),
                                             team->GetType(),
                                             team->GetType());
        ...
        if (!found)
            handler->PSendSysMessage(LANG_ARENA_ERROR_NAME_NOT_FOUND,
...
void AddSC_arena_commandscript() { new arena_commandscript(); }
