...
public:
    ...
    static bool HandleGameObjectActivateCommand(ChatHandler*      handler,
                                                GameObjectSpawnId guidLow)
    {
        GameObject* object = handler->GetObjectFromPlayerMapByDbGuid(guidLow);
        if (!object) {
            handler->SendErrorMessage(LANG_COMMAND_OBJNOTFOUND,
                                      uint32(guidLow));
            return false;
        }

        uint32_t const autoCloseTime =
            object->GetGOInfo()->GetAutoCloseTime() ? 10000u : 0u;

        // Activate
        object->SetLootState(GO_READY);
        object->UseDoorOrButton(
            autoCloseTime, false, handler->GetSession()->GetPlayer());
    ...
    // spawn go
    static bool HandleGameObjectAddCommand(ChatHandler*    handler,
                                           ...
                                           Optional<int32> spawnTimeSecs)
    {
        ...
        GameObjectTemplate const* objectInfo =
            sObjectMgr->GetGameObjectTemplate(objectId);
        if (!objectInfo) {
            handler->SendErrorMessage(LANG_GAMEOBJECT_NOT_EXIST,
                                      uint32(objectId));
            return false;
        }
        ...
        if (objectInfo->displayId &&
            ...
            LOG_ERROR("sql.sql",
                      "Gameobject (Entry {} GoType: {}) have invalid displayId "
                      "({}), not spawned.",
                      *objectId,
                      objectInfo->type,
                      objectInfo->displayId);
            handler->SendErrorMessage(LANG_GAMEOBJECT_HAVE_INVALID_DATA,
                                      uint32(objectId));
            return false;
        }
        ...
        GameObject* object =
            sObjectMgr->IsGameObjectStaticTransport(objectInfo->entry)
                ? new StaticTransport()
                : new GameObject();
        ObjectGuid::LowType guidLow =
            map->GenerateLowGuid<HighGuid::GameObject>();

        if (!object->Create(guidLow,
                            objectInfo->entry,
                            map,
        ...
        object->SaveToDB(map->GetId(),
                         (1 << map->GetSpawnMode()),
                         player->GetPhaseMaskForSpawn());
        guidLow = object->GetSpawnId();

        // delete the old object and do a clean load from DB with a fresh new
        // GameObject instance. this is required to avoid weird behavior and
        // memory leaks
        delete object;

        object = sObjectMgr->IsGameObjectStaticTransport(objectInfo->entry)
                     ? new StaticTransport()
                     : new GameObject();
        ...
        handler->PSendSysMessage(LANG_GAMEOBJECT_ADD,
                                 uint32(objectId),
                                 objectInfo->name.c_str(),
                                 guidLow,
                                 player->GetPositionX(),
                                 player->GetPositionY(),
                                 player->GetPositionZ());
    ...
    // add go, temp only
    static bool HandleGameObjectAddTempCommand(ChatHandler*     handler,
                                               GameObjectEntry  objectId,
                                               Optional<uint64> spawntime)
    {
        ...
        player->SummonGameObject(
            objectId, x, y, z, ang, 0, 0, rot2, rot3, spawntm);
        ...
        Optional<Variant<GameObjectEntry, std::string_view>> objectId)
    {
        ...
        if (objectId) {
            if (objectId->holds_alternative<GameObjectEntry>()) {
                result = WorldDatabase.Query(
                    ...
                    "orientation, map, phaseMask, (POW(position_x - '{}', 2) + "
                    "POW(position_y - '{}', 2) + POW(position_z - '{}', 2)) AS "
                    "order_ FROM gameobject WHERE map = '{}' AND id = '{}' "
                ...
                std::string name =
                    std::string(objectId->get<std::string_view>());
                ...
                result = WorldDatabase.Query(
                    ...
                    "gameobject_template.entry = gameobject.id WHERE map = {} "
                    "AND name LIKE '%{}%' ORDER BY order_ ASC LIMIT 1",
                    player->GetPositionX(),
                    player->GetPositionY(),
                    player->GetPositionZ(),
                    player->GetMapId(),
            ...
            result = WorldDatabase.Query(
                "SELECT gameobject.guid, id, position_x, position_y, "
                "position_z, orientation, map, phaseMask, "
                "(POW(position_x - {}, 2) + POW(position_y - {}, 2) + "
                "POW(position_z - {}, 2)) AS order_ FROM gameobject "
                "LEFT OUTER JOIN game_event_gameobject on gameobject.guid = "
                "game_event_gameobject.guid WHERE map = '{}' {} ORDER BY "
                "order_ ASC LIMIT 10",
        ...
        handler->PSendSysMessage(LANG_GAMEOBJECT_DETAIL,
                                 guidLow,
                                 objectInfo->name.c_str(),
                                 guidLow,
                                 id,
    ...
    static bool HandleGameObjectDeleteCommand(ChatHandler*      handler,
                                              GameObjectSpawnId spawnId)
    {
        ...
        if (ownerGuid) {
            Unit* owner = ObjectAccessor::GetUnit(
                *handler->GetSession()->GetPlayer(), ownerGuid);
            if (!owner || !ownerGuid.IsPlayer()) {
                handler->SendErrorMessage(LANG_COMMAND_DELOBJREFERCREATURE,
        ...
        handler->PSendSysMessage(LANG_COMMAND_DELOBJMESSAGE,
                                 object->GetSpawnId());
    ...
    static bool HandleGameObjectTurnCommand(ChatHandler*      handler,
                                            GameObjectSpawnId guidLow,
                                            Optional<float>   oz,
                                            Optional<float>   oy,
                                            Optional<float>   ox)
    {
        ...
        object->Relocate(object->GetPositionX(),
                         object->GetPositionY(),
                         object->GetPositionZ(),
                         *oz);
        object->SetLocalRotationAngles(
            *oz, oy.value_or(0.0f), ox.value_or(0.0f));
        object->SaveToDB();

        // Generate a completely new spawn with new guid
        // 3.3.5a client caches recently deleted objects and brings them back to
        // life when CreateObject block for this guid is received again however
        // it entirely skips parsing that block and only uses already known
        // location
        object->Delete();
        ...
        handler->PSendSysMessage(LANG_COMMAND_TURNOBJMESSAGE,
                                 object->GetSpawnId(),
                                 object->GetGOInfo()->name.c_str(),
                                 object->GetSpawnId());
        return true;
    }

    // move selected object
    static bool HandleGameObjectMoveCommand(ChatHandler*      handler,
                                            GameObjectSpawnId guidLow,
                                            Optional<std::array<float, 3>> xyz)
    {
        ...
        if (xyz) {
            ...
            if (!MapMgr::IsValidMapCoord(object->GetMapId(), pos)) {
                handler->SendErrorMessage(LANG_INVALID_TARGET_COORD,
                                          pos.GetPositionX(),
                                          pos.GetPositionY(),
                                          object->GetMapId());
                return false;
        ...
        sObjectMgr->RemoveGameobjectFromGrid(guidLow,
                                             object->GetGameObjectData());
        object->SaveToDB();
        ...
        handler->PSendSysMessage(LANG_COMMAND_MOVEOBJMESSAGE,
                                 object->GetSpawnId(),
                                 object->GetGOInfo()->name.c_str(),
                                 object->GetSpawnId());
        return true;
    ...
    static bool HandleGameObjectSetPhaseCommand(ChatHandler*      handler,
                                                GameObjectSpawnId guidLow,
                                                uint32            phaseMask)
    {
        if (!guidLow)
    ...
    static bool HandleGameObjectNearCommand(ChatHandler*    handler,
                                            Optional<float> dist)
    {
        ...
        if (result) {
            do {
                ...
                GameObjectTemplate const* gameObjectInfo =
                    sObjectMgr->GetGameObjectTemplate(entry);

                if (!gameObjectInfo)
                    continue;

                handler->PSendSysMessage(LANG_GO_LIST_CHAT,
                                         guid,
                                         entry,
                                         guid,
                                         gameObjectInfo->name.c_str(),
                                         x,
                                         y,
                                         z,
                                         mapId,
    ...
    static bool HandleGameObjectInfoCommand(
        ...
        Variant<Hyperlink<gameobject_entry>, Hyperlink<gameobject>, uint32>
            data)
    {
        uint32      entry     = 0;
        uint32      type      = 0;
        ...
        if (isGuid || data.holds_alternative<Hyperlink<gameobject>>()) {
            spawnId = *data;
            GameObjectData const* spawnData =
        ...
        if (gameObject)
            handler->PSendSysMessage("GUID: %u",
                                     gameObject->GetGUID().GetCounter());
        handler->PSendSysMessage(LANG_GOINFO_TYPE, type);
        handler->PSendSysMessage(LANG_GOINFO_LOOTID, lootId);
        handler->PSendSysMessage(LANG_GOINFO_DISPLAYID, displayId);
        if (gameObject) {
            handler->PSendSysMessage("LootMode: %u", gameObject->GetLootMode());
            handler->PSendSysMessage("LootState: %u",
                                     gameObject->getLootState());
            handler->PSendSysMessage("GOState: %u", gameObject->GetGoState());
            handler->PSendSysMessage("PhaseMask: %u",
                                     gameObject->GetPhaseMask());
            handler->PSendSysMessage("IsLootEmpty: %u",
                                     gameObject->loot.empty());
            handler->PSendSysMessage("IsLootLooted: %u",
    ...
    static bool HandleGameObjectSetStateCommand(ChatHandler*      handler,
                                                GameObjectSpawnId guidLow,
                                                int32             objectType,
    ...
    static bool HandleGameObjectRespawn(ChatHandler*      handler,
                                        GameObjectSpawnId guidLow)
    {
        GameObject* object = handler->GetObjectFromPlayerMapByDbGuid(guidLow);
        if (!object) {
        ...
        handler->PSendSysMessage(
            LANG_CMD_GO_RESPAWN,
            object->GetNameForLocaleIdx(handler->GetSessionDbcLocale()),
            object->GetEntry(),
            object->GetSpawnId());
        return true;
    }
...
void AddSC_gobject_commandscript() { new gobject_commandscript(); }
