...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"mmap", mmapCommandTable}};
        return commandTable;
    }

    static bool HandleMmapPathCommand(ChatHandler*          handler,
                                      Optional<std::string> param)
    {
        if (!MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMesh(
        ...
        if (param) {
            ...
            if (paramValue.starts_with("line") ||
                paramValue.starts_with("ray") ||
                paramValue.starts_with("raycast")) {
                useRaycast = true;
            }
        }
        ...
        handler->PSendSysMessage("%s's path to %s:",
                                 target->GetName().c_str(),
                                 player->GetName().c_str());
        handler->PSendSysMessage("Building: %s",
                                 useStraightPath ? "StraightPath"
                                 : useRaycast    ? "Raycast"
                                                 : "SmoothPath");
        handler->PSendSysMessage(
            Acore::StringFormatFmt("Result: {} - Length: {} - Type: {}",
                                   (result ? "true" : "false"),
                                   pointPath.size(),
                                   path.GetPathType())
                .c_str());
        ...
        handler->PSendSysMessage("ActualEndPosition (%.3f, %.3f, %.3f)",
                                 actualEnd.x,
                                 actualEnd.y,
                                 actualEnd.z);
        ...
        for (auto i : pointPath)
            player->SummonCreature(VISUAL_WAYPOINT,
                                   i.x,
                                   i.y,
                                   i.z,
                                   0,
    ...
    static bool HandleMmapLocCommand(ChatHandler* handler)
    {
        ...
        dtNavMesh const* navmesh =
            MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMesh(
                handler->GetSession()->GetPlayer()->GetMapId());
        dtNavMeshQuery const* navmeshquery =
            MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMeshQuery(
                handler->GetSession()->GetPlayer()->GetMapId(),
    ...
    static bool HandleMmapLoadedTilesCommand(ChatHandler* handler)
    {
        ...
        dtNavMesh const* navmesh =
            MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMesh(mapid);
        dtNavMeshQuery const* navmeshquery =
            MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMeshQuery(
    ...
    static bool HandleMmapStatsCommand(ChatHandler* handler)
    {
        ...
        // handler->PSendSysMessage("  global mmap pathfinding is %sabled",
        // DisableMgr::IsPathfindingEnabled(mapId) ? "en" : "dis");

        MMAP::MMapMgr* manager = MMAP::MMapFactory::createOrGetMMapMgr();
        handler->PSendSysMessage(" %u maps loaded with %u tiles overall",
                                 manager->getLoadedMapsCount(),
                                 manager->getLoadedTilesCount());

        dtNavMesh const* navmesh =
            manager->GetNavMesh(handler->GetSession()->GetPlayer()->GetMapId());
        if (!navmesh) {
            handler->PSendSysMessage("NavMesh not loaded for current map.");
            return true;
        }
        ...
        handler->PSendSysMessage(" %.2f MB of data (not including pointers)",
                                 ((float)dataSize / sizeof(unsigned char)) /
                                     1048576);
    ...
    static bool HandleMmapTestArea(ChatHandler* handler)
    {
        ...
        Acore::CreatureListSearcher<Acore::AnyUnitInObjectRangeCheck> go_search(
            object, creatureList, go_check);
        Cell::VisitGridObjects(object, go_search, radius);

        if (!creatureList.empty()) {
            handler->PSendSysMessage(
                Acore::StringFormatFmt("Found {} Creatures.",
                                       creatureList.size())
                    .c_str());
...
void AddSC_mmaps_commandscript() { new mmaps_commandscript(); }
