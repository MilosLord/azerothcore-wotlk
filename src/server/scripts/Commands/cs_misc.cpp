...
{
    auto creatureTemplate =
        sObjectMgr->GetCreatureTemplate(creature->GetEntry());
...
public:
    ...
    static bool HandleSkirmishCommand(ChatHandler*                  handler,
                                      std::vector<std::string_view> args)
    {
        ...
        if (args.empty() || !tokens.size()) {
            ...
            handler->PSendSysMessage(
                "[arena] can be \"all\" or comma-separated list of possible "
                "arenas (NA, BE, RL, DS, RV).");
            handler->PSendSysMessage(
                "[XvX] can be 1v1, 2v2, 3v3, 5v5. After [XvX] specify enough "
                "nicknames for that mode.");
        ...
        BattlegroundTypeId randomizedArenaBgTypeId =
            Acore::Containers::SelectRandomContainerElement(allowedArenas);
        ...
        if (tokens.size() != uint16(count + 2)) {
            handler->SendErrorMessage(
                "Invalid number of nicknames for this bracket.");
            return false;
        }
        ...
        for (; tokensItr != tokens.end(); ++tokensItr) {
            ...
            if (hcnt > 1) {
                ...
                if (g->isRaidGroup() || g->isBGGroup() || g->isBFGroup() ||
                    g->isLFGGroup()) {
                    error = 7;
                    break;
                }
        ...
        case 1:
            ...
            break;
        case 2:
            handler->PSendSysMessage("Player %s is being teleported.",
                                     last_name.c_str());
            break;
        case 3:
            handler->PSendSysMessage(
                "Player %s is in instance/battleground/arena.",
                last_name.c_str());
            break;
        case 4:
            handler->PSendSysMessage("Player %s is in LFG system.",
                                     last_name.c_str());
            break;
        case 5:
            handler->PSendSysMessage(
                "Player %s is queued for battleground/arena.",
                last_name.c_str());
            break;
        case 6:
            handler->PSendSysMessage("Player %s is not in group.",
                                     last_name.c_str());
            break;
        case 7:
            handler->PSendSysMessage("Player %s is not in normal group.",
                                     last_name.c_str());
            break;
        case 8:
            handler->PSendSysMessage(
                "Group of player %s has invalid member count.",
                last_name.c_str());
            break;
        case 9:
            handler->PSendSysMessage("Players %s are not in the same group.",
                                     last_name.c_str());
            break;
        case 10:
            handler->PSendSysMessage("Player %s is in flight.",
                                     last_name.c_str());
            break;
        case 11:
            ...
            break;
        case 12:
            handler->PSendSysMessage("Player %s is in a group.",
                                     last_name.c_str());
            break;
        case 13:
            handler->PSendSysMessage("Player %s occurs more than once.",
                                     last_name.c_str());
            break;
        }
        ...
        Battleground* bg = sBattlegroundMgr->CreateNewBattleground(
            randomizedArenaBgTypeId,
            GetBattlegroundBracketById(bgt->GetMapId(), bgt->GetBracketId()),
            ArenaType(hcnt >= 2 ? hcnt : 2),
            false);
        if (!bg) {
            handler->SendErrorMessage("Couldn't create arena map!");
            return false;
        ...
        TeamId teamId2 =
            (teamId1 == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);
        ...
        for (uint8 i = 0; i < cnt; ++i) {
            ...
            player->SetBattlegroundId(
                bg->GetInstanceID(), bgTypeId, queueSlot, true, false, teamId);
            sBattlegroundMgr->SendToBattleground(
                player, bg->GetInstanceID(), bgTypeId);
        }

        handler->PSendSysMessage(
            "Success! Players are now being teleported to the arena.");
        return true;
    }

    static bool HandleCommentatorCommand(ChatHandler*   handler,
                                         Optional<bool> enableArg)
    {
        ...
        auto SetCommentatorMod = [&](bool enable) {
            session->SendNotification(enable ? "Commentator mode on"
        ...
        if (!enableArg) {
            if (!AccountMgr::IsPlayerAccount(session->GetSecurity()) &&
                session->GetPlayer()->IsCommentator()) {
    ...
    static bool HandleDevCommand(ChatHandler* handler, Optional<bool> enableArg)
    {
        ...
        auto SetDevMod = [&](bool enable) {
            ...
            sScriptMgr->OnHandleDevCommand(handler->GetSession()->GetPlayer(),
                                           enable);
        };

        if (!enableArg) {
            if (!AccountMgr::IsPlayerAccount(session->GetSecurity()) &&
                session->GetPlayer()->IsDeveloper()) {
                SetDevMod(true);
            }
            else {
                SetDevMod(false);
    ...
    static bool HandleGPSCommand(ChatHandler*               handler,
                                 Optional<PlayerIdentifier> target)
    {
        ...
        Cell cell(Acore::ComputeCellCoord(object->GetPositionX(),
                                          object->GetPositionY()));
        ...
        float groundZ = object->GetMapHeight(
            object->GetPositionX(), object->GetPositionY(), MAX_HEIGHT);
        float floorZ = object->GetMapHeight(object->GetPositionX(),
                                            object->GetPositionY(),
                                            object->GetPositionZ());

        GridCoord gridCoord = Acore::ComputeGridCoord(object->GetPositionX(),
                                                      object->GetPositionY());
        ...
        uint32 haveMap =
            Map::ExistMap(object->GetMapId(), gridX, gridY) ? 1 : 0;
        uint32 haveVMap =
            Map::ExistVMap(object->GetMapId(), gridX, gridY) ? 1 : 0;
        uint32 haveMMAP = MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMesh(
        ...
        handler->PSendSysMessage(
            LANG_MAP_POSITION,
            object->GetMapId(),
            (mapEntry ? mapEntry->name[handler->GetSessionDbcLocale()]
                      : "<unknown>"),
            zoneId,
            (zoneEntry ? zoneEntry->area_name[handler->GetSessionDbcLocale()]
                       : "<unknown>"),
            areaId,
            (areaEntry ? areaEntry->area_name[handler->GetSessionDbcLocale()]
                       : "<unknown>"),
            object->GetPhaseMask(),
            object->GetPositionX(),
            object->GetPositionY(),
        ...
        if (liquidData.Status) {
            handler->PSendSysMessage(LANG_LIQUID_STATUS,
                                     liquidData.Level,
                                     liquidData.DepthLevel,
                                     liquidData.Entry,
                                     liquidData.Flags,
                                     liquidData.Status);
        }
        ...
        if (object->GetTransport()) {
            handler->PSendSysMessage(
                "Transport offset: %.2f, %.2f, %.2f, %.2f",
                object->m_movementInfo.transport.pos.GetPositionX(),
                object->m_movementInfo.transport.pos.GetPositionY(),
                object->m_movementInfo.transport.pos.GetPositionZ(),
                object->m_movementInfo.transport.pos.GetOrientation());
        }
    ...
    static bool
    HandleUnAuraCommand(ChatHandler*                                handler,
                        Variant<SpellInfo const*, std::string_view> spells)
    {
        ...
        if (spells.holds_alternative<std::string_view>() &&
            spells.get<std::string_view>() == "all") {
    ...
    // Teleport to Player
    static bool HandleAppearCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> target)
    {
        if (!target) {
        ...
        if (target->IsConnected()) {
            ...
            if (map->IsBattlegroundOrArena()) {
                ...
                if (!_player->IsGameMaster()) {
                    handler->SendErrorMessage(LANG_CANNOT_GO_TO_BG_GM,
                                              nameLink.c_str());
                    return false;
                }
                ...
                _player->SetBattlegroundId(
                    targetPlayer->GetBattlegroundId(),
                    targetPlayer->GetBattlegroundTypeId(),
                ...
                if (_player->GetGroup()) {
                    // we are in group, we can go only if we are in the player
                    // group
                    if (_player->GetGroup() != targetPlayer->GetGroup()) {
                        handler->SendErrorMessage(LANG_CANNOT_GO_TO_INST_PARTY,
                                                  nameLink.c_str());
                        return false;
                    }
                }
                    ...
                    if (!_player->IsGameMaster()) {
                        handler->SendErrorMessage(LANG_CANNOT_GO_TO_INST_GM,
                                                  nameLink.c_str());
                        return false;
                    }
                }

                // if the GM is bound to another instance, he will not be bound
                // to another one
                InstancePlayerBind* bind =
                    sInstanceSaveMgr->PlayerGetBoundInstance(
                        _player->GetGUID(),
                        targetPlayer->GetMapId(),
                        targetPlayer->GetDifficulty(map->IsRaid()));
                if (!bind) {
                    if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(
                            target->GetConnectedPlayer()->GetInstanceId())) {
                ...
                if (map->IsRaid()) {
                    _player->SetRaidDifficulty(
                        targetPlayer->GetRaidDifficulty());
                }
                else {
                    _player->SetDungeonDifficulty(
                        targetPlayer->GetDungeonDifficulty());
                }
            }
            ...
            if (_player->TeleportTo(targetPlayer->GetMapId(),
                                    targetPlayer->GetPositionX(),
                                    targetPlayer->GetPositionY(),
                                    targetPlayer->GetPositionZ() + 0.25f,
            ...
            if (!Player::LoadPositionFromDB(map,
                                            x,
                                            y,
                                            z,
                                            o,
                                    ...
                                    Optional<PlayerIdentifier> target)
    {
        ...
        if (target->IsConnected()) {
            ...
            if (map->IsBattlegroundOrArena()) {
                ...
                // Allow GM to summon players or only other GM accounts inside
                // instances.
                if (!sWorld->getBoolConfig(CONFIG_INSTANCE_GMSUMMON_PLAYER)) {
                    // pussywizard: prevent unbinding normal player's perm bind
                    // by just summoning him >_>
                    if (!targetPlayer->GetSession()->GetSecurity()) {
                        handler->SendErrorMessage(
                            "Only GMs can be summoned to an instance!");
                        return false;
                    }
                }
                ...
                if (destMap->Instanceable() &&
                    destMap->GetInstanceId() != map->GetInstanceId()) {
                ...
                // we are in an instance, and can only summon players in our
                // group with us as leader
                if (!handler->GetSession()->GetPlayer()->GetGroup() ||
                    !targetPlayer->GetGroup() ||
                    (targetPlayer->GetGroup()->GetLeaderGUID() !=
                     handler->GetSession()->GetPlayer()->GetGUID()) ||
                    (handler->GetSession()
                         ->GetPlayer()
                         ->GetGroup()
                         ->GetLeaderGUID() !=
                     handler->GetSession()->GetPlayer()->GetGUID()))
                // the last check is a bit excessive, but let it be, just in
                // case
                {
            ...
            if (handler->needReportToTarget(targetPlayer)) {
                ChatHandler(targetPlayer->GetSession())
                    .PSendSysMessage(
                        LANG_SUMMONED_BY,
                        handler->playerLink(_player->GetName()).c_str());
            }
            ...
            handler->PSendSysMessage(LANG_SUMMONING,
                                     nameLink.c_str(),
                                     handler->GetAcoreString(LANG_OFFLINE));
    ...
    static bool HandleGroupSummonCommand(ChatHandler*               handler,
                                         Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        // we are in instance, and can summon only player in our group with us
        // as lead
        if (toInstance &&
            (!handler->GetSession()->GetPlayer()->GetGroup() ||
             (group->GetLeaderGUID() !=
              handler->GetSession()->GetPlayer()->GetGUID()) ||
             (handler->GetSession()->GetPlayer()->GetGroup()->GetLeaderGUID() !=
              handler->GetSession()->GetPlayer()->GetGUID())))
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
            ...
            if (toInstance) {
                ...
                if (playerMap->Instanceable() &&
                    playerMap->GetInstanceId() != gmMap->GetInstanceId()) {
            ...
            float x, y, z;
            handler->GetSession()->GetPlayer()->GetClosePoint(
                x, y, z, player->GetObjectSize());
            player->TeleportTo(handler->GetSession()->GetPlayer()->GetMapId(),
                               x,
                               y,
                               z,
            ...
            if (sWorld->getBoolConfig(CONFIG_DIE_COMMAND_MODE)) {
                if (target->GetTypeId() == TYPEID_UNIT &&
                    handler->GetSession()->GetSecurity() ==
                        SEC_CONSOLE) // pussywizard
                {
                    target->ToCreature()->LowerPlayerDamageReq(
                        target->GetMaxHealth());
                }
                Unit::Kill(handler->GetSession()->GetPlayer(), target);
            }
            else {
                Unit::DealDamage(handler->GetSession()->GetPlayer(),
                                 target,
                                 target->GetHealth(),
                                 nullptr,
                                 DIRECT_DAMAGE,
    ...
    static bool HandleReviveCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        if (target->IsConnected()) {
            ...
            targetPlayer->ResurrectPlayer(
                !AccountMgr::IsPlayerAccount(
                    targetPlayer->GetSession()->GetSecurity())
                    ? 1.0f
                    : 0.5f);
    ...
    static bool HandleCooldownCommand(ChatHandler*               handler,
                                      Optional<SpellInfo const*> spell)
    {
        ...
        if (!spell) {
            ...
            if (!SpellMgr::IsSpellValid(*spell)) {
                handler->SendErrorMessage(LANG_COMMAND_SPELL_BROKEN,
                                          spell.value()->Id);
                return false;
            }

            target->RemoveSpellCooldown(spell.value()->Id, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN,
                                     spell.value()->Id,
                                     target ==
                                             handler->GetSession()->GetPlayer()
                                         ? handler->GetAcoreString(LANG_YOU)
                                         : nameLink.c_str());
        }
        return true;
    }

    static bool HandleGetDistanceCommand(ChatHandler*               handler,
                                         Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        handler->PSendSysMessage(
            LANG_DISTANCE,
            handler->GetSession()->GetPlayer()->GetDistance(object),
            handler->GetSession()->GetPlayer()->GetDistance2d(object),
            handler->GetSession()->GetPlayer()->GetExactDist(object),
            handler->GetSession()->GetPlayer()->GetExactDist2d(object));
        return true;
    }
    // Teleport player to last position
    static bool HandleRecallCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        if (targetPlayer->IsBeingTeleported()) {
            handler->SendErrorMessage(
                LANG_IS_TELEPORTED,
                handler->playerLink(target->GetName()).c_str());
            return false;
        }
        ...
        targetPlayer->TeleportTo(targetPlayer->m_recallMap,
                                 targetPlayer->m_recallX,
                                 targetPlayer->m_recallY,
                                 targetPlayer->m_recallZ,
                                 targetPlayer->m_recallO);
        return true;
    }
    ...
    static bool HandleSaveCommand(ChatHandler* handler)
    {
        ...
        if (saveInterval == 0 ||
            (saveInterval > 20 * IN_MILLISECONDS &&
             player->GetSaveTimer() <= saveInterval - 20 * IN_MILLISECONDS)) {
            player->SaveToDB(false, false);
        }
    ...
    // kick player
    static bool HandleKickPlayerCommand(ChatHandler*               handler,
                                        Optional<PlayerIdentifier> target,
                                        Optional<std::string_view> reason)
    {
        ...
        if (sWorld->getBoolConfig(CONFIG_SHOW_KICK_IN_WORLD)) {
            ...
            handler->PSendSysMessage(LANG_COMMAND_KICKMESSAGE,
                                     target->GetName().c_str());
        }
    ...
    static bool HandleUnstuckCommand(ChatHandler*               handler,
                                     Optional<PlayerIdentifier> target,
        ...
        if (player->IsInFlight() || player->IsInCombat()) {
            ...
            if (Player* caster = handler->GetSession()->GetPlayer()) {
                Spell::SendCastResult(
                    caster, spellInfo, 0, SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW);
            }
    ...
    static bool HandleLinkGraveCommand(ChatHandler*               handler,
                                       uint32                     graveyardId,
                                       Optional<std::string_view> team)
    {
        ...
        GraveyardStruct const* graveyard =
            sGraveyard->GetGraveyard(graveyardId);

        if (!graveyard) {
            handler->SendErrorMessage(LANG_COMMAND_GRAVEYARDNOEXIST,
                                      graveyardId);
            return false;
        }
        ...
        if (!areaEntry || areaEntry->zone != 0) {
            handler->SendErrorMessage(
                LANG_COMMAND_GRAVEYARDWRONGZONE, graveyardId, zoneId);
            return false;
        }

        if (sGraveyard->AddGraveyardLink(graveyardId, zoneId, teamId)) {
            handler->PSendSysMessage(
                LANG_COMMAND_GRAVEYARDLINKED, graveyardId, zoneId);
        }
        else {
            handler->PSendSysMessage(
                LANG_COMMAND_GRAVEYARDALRLINKED, graveyardId, zoneId);
        }

        return true;
    }

    static bool HandleNearGraveCommand(ChatHandler*               handler,
                                       Optional<std::string_view> team)
    {
        TeamId teamId;
        ...
        GraveyardStruct const* graveyard =
            sGraveyard->GetClosestGraveyard(player, teamId);

        if (graveyard) {
            uint32 graveyardId = graveyard->ID;

            GraveyardData const* data =
                sGraveyard->FindGraveyardData(graveyardId, zone_id);
            if (!data) {
                handler->SendErrorMessage(LANG_COMMAND_GRAVEYARDERROR,
                                          graveyardId);
                return false;
            }

            std::string team_name =
                handler->GetAcoreString(LANG_COMMAND_GRAVEYARD_NOTEAM);
            ...
            if (data->teamId == TEAM_NEUTRAL) {
                ...
                team_name =
                    handler->GetAcoreString(LANG_COMMAND_GRAVEYARD_HORDE);
            }
            else if (data->teamId == TEAM_ALLIANCE) {
                team_name =
                    handler->GetAcoreString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
            }

            handler->PSendSysMessage(LANG_COMMAND_GRAVEYARDNEAREST,
                                     graveyardId,
                                     team_name.c_str(),
                                     zone_id);
        }
    ...
    static bool HandleShowAreaCommand(ChatHandler* handler, uint32 areaID)
    {
        ...
        uint32 currFields =
            playerTarget->GetUInt32Value(PLAYER_EXPLORED_ZONES_1 + offset);
        playerTarget->SetUInt32Value(PLAYER_EXPLORED_ZONES_1 + offset,
                                     ...
                                     Optional<int32>            _count)
    {
        if (!sObjectMgr->GetItemTemplate(itemTemplate->ItemId)) {
            handler->SendErrorMessage(LANG_COMMAND_ITEMIDINVALID,
                                      itemTemplate->ItemId);
            return false;
        }
            ...
            if (playerTarget->GetSession()->GetSecurity() == SEC_PLAYER) {
                if (!playerTarget->HasItemCount(itemId, 0)) {
                    ...
                    handler->SendErrorMessage(
                        ...
                        handler->GetNameLink(playerTarget).c_str(),
                        itemId);
                    return false;
                }

                if (!playerTarget->HasItemCount(itemId, -count)) {
                    // output that player don't have as many items that you want
                    // to destroy
                    handler->SendErrorMessage(
        ...
        {
            handler->SendErrorMessage(
                LANG_ITEM_CANNOT_CREATE, itemId, noSpaceForCount);
            return false;
        }

        Item* item = playerTarget->StoreNewItem(dest, itemId, true);

        Player* p = handler->GetSession() ? handler->GetSession()->GetPlayer()
                                          : nullptr;
    ...
    static bool
    HandleAddItemSetCommand(ChatHandler*                        handler,
                            Variant<Hyperlink<itemset>, uint32> itemSetId)
    {
        // prevent generation all items with itemset field value '0'
        if (!*itemSetId) {
            handler->SendErrorMessage(LANG_NO_ITEMS_FROM_ITEMSET_FOUND,
                                      uint32(itemSetId));
            return false;
        }
        ...
        for (auto const& [itemid, itemTemplate] :
             *sObjectMgr->GetItemTemplateStore()) {
            if (itemTemplate.ItemSet == uint32(itemSetId)) {
                found = true;
                ItemPosCountVec dest;
                InventoryResult msg = playerTarget->CanStoreNewItem(
                    NULL_BAG, NULL_SLOT, dest, itemTemplate.ItemId, 1);

                if (msg == EQUIP_ERR_OK) {
                    Item* item = playerTarget->StoreNewItem(
                        dest, itemTemplate.ItemId, true);
                    ...
                    player->SendEquipError(
                        msg, nullptr, nullptr, itemTemplate.ItemId);
                    handler->PSendSysMessage(
                        LANG_ITEM_CANNOT_CREATE, itemTemplate.ItemId, 1);
                }
            }
        }
    ...
    static bool
    HandleChangeWeather(ChatHandler* handler, uint32 type, float grade)
    {
        // Weather is OFF
    ...
    static bool HandleSetSkillCommand(ChatHandler*                      handler,
                                      ...
                                      Optional<uint16> maxPureSkill)
    {
        ...
        // If our target does not yet have the skill they are trying to add to
        // them, the chosen level also becomes the max level of the new
        // profession.
        uint16 max = maxPureSkill     ? *maxPureSkill
                     : targetHasSkill ? target->GetPureMaxSkillValue(skillID)
                                      : uint16(level);
        ...
        // If the player has the skill, we get the current skill step. If they
        // don't have the skill, we add the skill to the player's book with step
        // 1 (which is the first rank, in most cases something like 'Apprentice
        // <skill>'.
        target->SetSkill(skillID,
                         targetHasSkill ? target->GetSkillStep(skillID) : 1,
                         level,
                         max);
        handler->PSendSysMessage(
            LANG_SET_SKILL,
            skillID,
            skillLine->name[handler->GetSessionDbcLocale()],
    ...
    // show info of player
    static bool HandlePInfoCommand(ChatHandler*               handler,
                                   Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        // Guild data print variables defined so that they exist, but are not
        // necessarily used
        uint32      guildId     = 0;
        uint8       guildRankId = 0;
        ...
        if (playerTarget) {
            ...
            alive  = playerTarget->IsAlive() ? handler->GetAcoreString(LANG_YES)
                                             : handler->GetAcoreString(LANG_NO);
        ...
        if (accInfoResult) {
            ...
            if (!handler->GetSession() ||
                ...
                if (IpLocationRecord const* location =
                        sIPLocation->GetLocationRecord(lastIp)) {
                    lastIp.append(" (");
        ...
        LoginDatabasePreparedStatement* banQuery =
        ...
        if (accBannedResult) {
            ...
            banTime =
                int64(fields[1].Get<uint64>() ? 0 : fields[0].Get<uint32>());
            bannedBy  = fields[2].Get<std::string>();
            banReason = fields[3].Get<std::string>();
        }

        // Can be used to query data from World database
        WorldDatabasePreparedStatement* xpQuery =
            WorldDatabase.GetPreparedStatement(WORLD_SEL_REQ_XP);
        ...
        CharacterDatabasePreparedStatement* charXpQuery =
            CharacterDatabase.GetPreparedStatement(CHAR_SEL_PINFO_XP);
        ...
        if (charXpResult) {
            ...
            if (gguid != 0) {
                CharacterDatabasePreparedStatement* guildQuery =
                    CharacterDatabase.GetPreparedStatement(
                        CHAR_SEL_GUILD_MEMBER_EXTENDED);
                guildQuery->SetData(0, lowguid);

                PreparedQueryResult guildInfoResult =
                    CharacterDatabase.Query(guildQuery);
                if (guildInfoResult) {
        ...
        handler->PSendSysMessage(
            LANG_PINFO_PLAYER,
            playerTarget ? "" : handler->GetAcoreString(LANG_OFFLINE),
            nameLink.c_str(),
            target->GetGUID().GetCounter());
            ...
            handler->PSendSysMessage(
                ...
                banTime > 0
                    ? secsToTimeString(
                          ...
                          .c_str()
                    : handler->GetAcoreString(LANG_PERMANENTLY),
                bannedBy.c_str());
        }
        ...
        if (muteTime > 0) {
            handler->PSendSysMessage(
                LANG_PINFO_MUTED,
                muteReason.c_str(),
                secsToTimeString(muteTime - GameTime::GetGameTime().count(),
                                 true)
                    .c_str(),
                muteBy.c_str());
        }
        ...
        handler->PSendSysMessage(
            LANG_PINFO_ACC_ACCOUNT, userName.c_str(), accId, security);

        // Output VI. LANG_PINFO_ACC_LASTLOGIN
        handler->PSendSysMessage(
            LANG_PINFO_ACC_LASTLOGIN, lastLogin.c_str(), failedLogins);
        ...
        handler->PSendSysMessage(
            LANG_PINFO_ACC_REGMAILS, regMail.c_str(), eMail.c_str());

        // Output IX. LANG_PINFO_ACC_IP
        handler->PSendSysMessage(LANG_PINFO_ACC_IP,
                                 lastIp.c_str(),
                                 locked ? handler->GetAcoreString(LANG_YES)
                                        : handler->GetAcoreString(LANG_NO));

        // Output X. LANG_PINFO_CHR_LEVEL
        if (level != sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
            handler->PSendSysMessage(
                LANG_PINFO_CHR_LEVEL_LOW, level, xp, xptotal, (xptotal - xp));
        }
        else {
        ...
        handler->PSendSysMessage(
            LANG_PINFO_CHR_RACE,
            (gender == 0
                 ? handler->GetAcoreString(LANG_CHARACTER_GENDER_MALE)
        ...
        // Output XIII. LANG_PINFO_CHR_PHASE if player is not in GM mode (GM is
        // in every phase)
        if (playerTarget &&
            !playerTarget->IsGameMaster()) // IsInWorld() returns false on
                                           // loadingscreen, so it's more
        {
            handler->PSendSysMessage(
                LANG_PINFO_CHR_PHASE,
                phase); // precise than just target (safer ?).
        }

        // However, as we usually just require a target here, we use target
        // instead. Output XIV. LANG_PINFO_CHR_MONEY
        ...
        if (area) {
            ...
            AreaTableEntry const* zone =
                sAreaTableStore.LookupEntry(area->zone);
            if (zone) {
                areaName = zoneName;
        ...
        if (areaName) {
            handler->PSendSysMessage(LANG_PINFO_CHR_MAP_WITH_AREA,
                                     map->name[locale],
                                     zoneName,
                                     areaName);
        }
            ...
            handler->PSendSysMessage(
                LANG_PINFO_CHR_MAP, map->name[locale], zoneName);
        }

        // Output XVII. - XVIX. if they are not empty
        if (!guildName.empty()) {
            handler->PSendSysMessage(
                LANG_PINFO_CHR_GUILD, guildName.c_str(), guildId);
            handler->PSendSysMessage(LANG_PINFO_CHR_GUILD_RANK,
                                     guildRank.c_str(),
                                     uint32(guildRankId));

            if (!note.empty()) {
                handler->PSendSysMessage(LANG_PINFO_CHR_GUILD_NOTE,
                                         note.c_str());
            }

            if (!officeNote.empty()) {
                handler->PSendSysMessage(LANG_PINFO_CHR_GUILD_ONOTE,
                                         officeNote.c_str());
            }
        }
        ...
        handler->PSendSysMessage(
            ...
            (secsToTimeString(totalPlayerTime, true)).c_str());

        // Mail Data - an own query, because it may or may not be useful.
        // SQL: "SELECT SUM(CASE WHEN (checked & 1) THEN 1 ELSE 0 END) AS
        // 'readmail', COUNT(*) AS 'totalmail' FROM mail WHERE `receiver` = ?"
        CharacterDatabasePreparedStatement* mailQuery =
            CharacterDatabase.GetPreparedStatement(CHAR_SEL_PINFO_MAILS);
        ...
        if (mailInfoResult) {
            ...
            if (totalmail >= 1) {
                handler->PSendSysMessage(
                    LANG_PINFO_CHR_MAILS, readmail, totalmail);
            }
        }
    ...
    static bool HandleRespawnAllCommand(ChatHandler* handler)
    {
        ...
        CellCoord p(Acore::ComputeCellCoord(player->GetPositionX(),
                                            player->GetPositionY()));
        Cell      cell(p);
        cell.SetNoCreate();
        ...
        Cell::VisitGridObjects(
            player, worker, player->GetGridActivationRange());

        return true;
    }

    // mute player for some times
    static bool HandleMuteCommand(ChatHandler*               handler,
                                  Optional<PlayerIdentifier> player,
                                  ...
                                  Tail                       muteReason)
    {
        ...
        if (target) {
            ...
            if (sWorld->getBoolConfig(CONFIG_SHOW_MUTE_IN_WORLD)) {
                sWorld->SendWorldText(
                    ...
                    muteBy.c_str(),
                    nameLink.c_str(),
                    secsToTimeString(muteDuration, true).c_str(),
                    muteReasonStr.c_str());
            }
            ...
            // Target is offline, mute will be in effect starting from the next
            // login.
        ...
        if (sWorld->getBoolConfig(CONFIG_SHOW_MUTE_IN_WORLD) && !target) {
            sWorld->SendWorldText(LANG_COMMAND_MUTEMESSAGE_WORLD,
                                  muteBy.c_str(),
                                  nameLink.c_str(),
                                  secsToTimeString(muteDuration, true).c_str(),
                                  muteReasonStr.c_str());
            ...
            HashMapHolder<Player>::MapType const& m =
                ObjectAccessor::GetPlayers();
            for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin();
                 itr != m.end();
                 ++itr)
                if (itr->second->GetSession()->GetSecurity())
                    ChatHandler(itr->second->GetSession())
                        .PSendSysMessage(
                            target ? LANG_YOU_DISABLE_CHAT
                                   : LANG_COMMAND_DISABLE_CHAT_DELAYED,
                            (handler->GetSession()
                                 ? handler->GetSession()
                                       ->GetPlayerName()
                                       .c_str()
                                 : handler->GetAcoreString(LANG_CONSOLE)),
                            nameLink.c_str(),
                            secsToTimeString(muteDuration, true).c_str(),
                            muteReasonStr.c_str());
        }
    ...
    // unmute player
    static bool HandleUnmuteCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        Player* playerTarget = target->GetConnectedPlayer();
        uint32  accountId    = playerTarget
                                   ? playerTarget->GetSession()->GetAccountId()
                                   : sCharacterCache->GetCharacterAccountIdByGuid(
                                     target->GetGUID());
        ...
        if (playerTarget) {
            ChatHandler(playerTarget->GetSession())
    ...
    // mutehistory command
    static bool HandleMuteInfoCommand(ChatHandler* handler,
                                      std::string  accountName)
    {
        ...
        if (!accountId) {
            handler->PSendSysMessage(LANG_ACCOUNT_NOT_EXIST,
                                     accountName.c_str());
            return false;
        }
    ...
    static bool HandleMuteInfoHelper(ChatHandler* handler,
                                     uint32       accountId,
                                     char const*  accountName)
    {
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_MUTE_INFO);
        stmt->SetData(0, accountId);
        PreparedQueryResult result = LoginDatabase.Query(stmt);

        if (!result) {
            handler->PSendSysMessage(LANG_COMMAND_MUTEHISTORY_EMPTY,
                                     accountName);
            return true;
        }
        ...
        do {
            ...
            handler->PSendSysMessage(LANG_COMMAND_MUTEHISTORY_OUTPUT,
                                     Acore::Time::TimeToHumanReadable(
                                         Seconds(fields[0].Get<uint32>()))
                                         .c_str(),
                                     fields[1].Get<uint32>(),
                                     fields[2].Get<std::string>().c_str(),
    ...
    static bool HandleMovegensCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(
            LANG_MOVEGENS_LIST,
            (unit->GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"),
        ...
        for (uint8 i = 0; i < MAX_MOTION_SLOT; ++i) {
            ...
            case CHASE_MOTION_TYPE: {
                ...
                if (unit->GetTypeId() == TYPEID_PLAYER) {
                    target = static_cast<ChaseMovementGenerator<Player> const*>(
                                 movementGenerator)
                                 ->GetTarget();
                }
                else {
                    target =
                        static_cast<ChaseMovementGenerator<Creature> const*>(
                            movementGenerator)
                            ->GetTarget();
                }
                ...
                if (!target) {
                    ...
                    handler->PSendSysMessage(LANG_MOVEGENS_CHASE_PLAYER,
                                             target->GetName().c_str(),
                                             target->GetGUID().GetCounter());
                }
                else {
                    handler->PSendSysMessage(LANG_MOVEGENS_CHASE_CREATURE,
                                             target->GetName().c_str(),
                                             target->GetGUID().GetCounter());
                }
                break;
            ...
            case FOLLOW_MOTION_TYPE: {
                ...
                if (unit->GetTypeId() == TYPEID_PLAYER) {
                    target =
                        static_cast<FollowMovementGenerator<Player> const*>(
                            movementGenerator)
                            ->GetTarget();
                }
                else {
                    target =
                        static_cast<FollowMovementGenerator<Creature> const*>(
                            movementGenerator)
                            ->GetTarget();
                }
                ...
                if (!target) {
                    ...
                    handler->PSendSysMessage(LANG_MOVEGENS_FOLLOW_PLAYER,
                                             target->GetName().c_str(),
                                             target->GetGUID().GetCounter());
                }
                else {
                    handler->PSendSysMessage(LANG_MOVEGENS_FOLLOW_CREATURE,
                                             target->GetName().c_str(),
                                             target->GetGUID().GetCounter());
                }
                break;
            ...
            case HOME_MOTION_TYPE: {
                if (unit->GetTypeId() == TYPEID_UNIT) {
                    handler->PSendSysMessage(
                        LANG_MOVEGENS_HOME_CREATURE, x, y, z);
                }
                else {
    ...
    /*
    ComeToMe command REQUIRED for 3rd party scripting library to have access to
    PointMovementGenerator Without this function 3rd party scripting library
    will get linking errors (unresolved external) when attempting to use the
    ...
    */
    static bool HandleComeToMeCommand(ChatHandler* handler)
    {
        ...
        caster->GetMotionMaster()->MovePoint(0,
                                             player->GetPositionX(),
                                             player->GetPositionY(),
                                             player->GetPositionZ());
    ...
    static bool HandleDamageCommand(ChatHandler* handler, uint32 damage)
    {
        ...
        if (target->GetTypeId() == TYPEID_UNIT &&
            handler->GetSession()->GetSecurity() == SEC_CONSOLE) // pussywizard
        {
        ...
        if (target != handler->GetSession()->GetPlayer()) {
            handler->GetSession()->GetPlayer()->SendAttackStateUpdate(
    ...
    static bool HandleCombatStopCommand(ChatHandler*               handler,
                                        Optional<PlayerIdentifier> target)
    {
        if (!target) {
            target = PlayerIdentifier::FromTargetOrSelf(handler);
    ...
    static bool HandleFreezeCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> target)
    {
        ...
        if (playerTarget && !creatureTarget) {
            handler->PSendSysMessage(LANG_COMMAND_FREEZE,
                                     target->GetName().c_str());

            if (SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(SPELL_FREEZE)) {
                Aura::TryRefreshStackOrCreate(
                    spellInfo, MAX_EFFECT_MASK, playerTarget, playerTarget);
            }
            ...
            handler->PSendSysMessage(
                LANG_COMMAND_FREEZE,
                GetLocalizeCreatureName(creatureTarget,
            ...
            if (SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(SPELL_FREEZE)) {
                Aura::TryRefreshStackOrCreate(
                    spellInfo, MAX_EFFECT_MASK, creatureTarget, creatureTarget);
            }
    ...
    static bool HandleUnFreezeCommand(ChatHandler*               handler,
                                      Optional<PlayerIdentifier> target)
    {
        Creature* creatureTarget = handler->getSelectedCreature();
        ...
        if (!creatureTarget && playerTarget &&
            playerTarget->HasAura(SPELL_FREEZE)) {
            handler->PSendSysMessage(LANG_COMMAND_UNFREEZE,
                                     target->GetName().c_str());
    ...
    static bool HandlePlayAllCommand(ChatHandler* handler, uint32 soundId)
    {
        ...
        sWorld->SendGlobalMessage(
            WorldPackets::Misc::Playsound(soundId).Write());
    ...
    static bool
    HandleStringCommand(ChatHandler* handler, uint32 id, Optional<uint8> locale)
    {
        if (!id) {
        ...
        const char* str = sObjectMgr->GetAcoreString(
            id, locale ? static_cast<LocaleConstant>(*locale) : DEFAULT_LOCALE);
