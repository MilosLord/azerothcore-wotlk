...
public:
     ...
     *
     * This function applies a Deserter Debuff of the given type (Instance or
     * BG) to the selected player, with the provided duration in seconds.
     *
     * @param handler The ChatHandler, passed by the system.
     * @param playerName Player by name. Optional, defaults to selected or self.
     * @param time The provided duration as TimeString. Optional, defaults to
     * bg/instance default time.
                                  ...
                                  bool                  isInstance)
    {
        ...
        uint32 deserterSpell =
            isInstance ? LFG_SPELL_DUNGEON_DESERTER : BG_SPELL_DESERTER;
        ...
        if (target) {
            ...
            if (QueryResult result = CharacterDatabase.Query(
                    "SELECT remainTime FROM character_aura WHERE guid = {} AND "
                    "spell = {}",
                    guid.GetCounter(),
                    deserterSpell)) {
                ...
                if (remainTime < 0 ||
                    remainTime >= duration * IN_MILLISECONDS) {
                    handler->PSendSysMessage(
                        "Player %s already has a longer %s Deserter active.",
                        handler->playerLink(*playerName),
                        isInstance ? "Instance" : "Battleground");
            ...
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_INS_AURA);
        ...
        handler->PSendSysMessage(
            "%s of %s Deserter has been added to player %s.",
     ...
     *
     * This function removes a Deserter Debuff of the given type (Instance or
     * BG) from the selected player.
     *
     * @param handler The ChatHandler, passed by the system.
     ...
     * @endcode
     */
    static bool HandleDeserterRemove(ChatHandler*               handler,
                                     ...
                                     bool                       isInstance)
    {
        ...
        if (duration == 0) {
            handler->SendErrorMessage("Player %s does not have %s Deserter.",
        ...
        if (duration < 0) {
            handler->SendErrorMessage("Permanent %s Deserter has been removed "
                                      "from player %s (GUID %u).",
                                      isInstance ? "Instance" : "Battleground",
                                      handler->playerLink(player->GetName()),
                                      player->GetGUID().GetCounter());
            return true;
        ...
        handler->PSendSysMessage(
            "%s of %s Deserter has been removed from player %s (GUID %u).",
     ...
     *
     * This function removes a Deserter Debuff of the given type (Instance or
     * BG) from all players, online or offline.
     *
     * @param handler The ChatHandler, passed by the system.
     * @param isInstance provided by the relaying functions, so we don't have
     * to write that much code :)
     * @param maxTime Optional: The maximum remaining time of the Debuff on
     * players to be removed. Any Player with a Deserter Debuff of this time or
     * less will get their Debuff removed. Use -1 for any. Default: 15m for BG,
     * 30m for Instance.
     *
     * @return true if everything was correct, false if an error occured.
     *
     ...
     * @endcode
     */
    static bool HandleDeserterRemoveAll(ChatHandler*          handler,
                                        bool                  isInstance,
                                        Optional<std::string> maxTime)
    {
        ...
        // Optimization. Do not execute any further functions or Queries if
        // remainTime is 0.
        if (remainTime == 0) {
            handler->SendErrorMessage(LANG_BAD_VALUE);
            return false;
        ...
        if (remainTime > 0) {
            result = CharacterDatabase.Query(
                "SELECT COUNT(guid) FROM character_aura WHERE spell = {} AND "
                "remainTime <= {}",
                deserterSpell,
            ...
            result = CharacterDatabase.Query(
                "SELECT COUNT(guid) FROM character_aura WHERE spell = {}",
                deserterSpell);
        }
        ...
        if (deserterCount > 0) {
            countOnline = false;
            if (remainTime > 0) {
                CharacterDatabase.Execute("DELETE FROM character_aura WHERE "
                                          "spell = {} AND remainTime <= {}",
                                          deserterSpell,
                                          remainTime * IN_MILLISECONDS);
            }
            else {
                CharacterDatabase.Execute(
                    "DELETE FROM character_aura WHERE spell = {}",
                    deserterSpell);
            }
        }
        ...
        if (deserterCount == 0) {
            handler->PSendSysMessage("No player on this realm has %s Deserter "
                                     "with a duration of %s or less.",
                                     isInstance ? "Instance" : "Battleground",
                                     remainTimeStr);
            return true;
        }

        handler->PSendSysMessage("%s Deserter has been removed from %u "
                                 "player(s) with a duration of %s or less.",
                                 isInstance ? "Instance" : "Battleground",
                                 deserterCount,
                                 remainTimeStr);
        return true;
    ...
    static bool HandleDeserterInstanceAdd(ChatHandler*          handler,
                                          Optional<std::string> playerName,
                                          Optional<std::string> time)
    {
    ...
    /// @sa HandleDeserterAdd()
    static bool HandleDeserterBGAdd(ChatHandler*          handler,
                                    Optional<std::string> playerName,
                                    Optional<std::string> time)
    {
...
void AddSC_deserter_commandscript() { new deserter_commandscript(); }
