...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"teleport", teleCommandTable}};
        return commandTable;
    }

    static bool HandleTeleAddCommand(ChatHandler*       handler,
    ...
    static bool DoNameTeleport(ChatHandler*       handler,
                               ...
                               std::string const& locationName)
    {
        ...
        if (Player* target = player.GetConnectedPlayer()) {
            ...
            handler->PSendSysMessage(LANG_TELEPORTING_TO,
                                     chrNameLink.c_str(),
                                     "",
                                     locationName.c_str());
            ...
            Player::SavePositionInDB(
                {mapId, pos},
                sMapMgr->GetZoneId(PHASEMASK_NORMAL, {mapId, pos}),
                player.GetGUID(),
                nullptr);
        }
        ...
        if (where.index() == 1) // References target's homebind
        {
            if (Player* target = player->GetConnectedPlayer())
                ...
                if (resultDB) {
                    ...
                    Player::SavePositionInDB(
                        loc, zoneId, player->GetGUID(), nullptr);
                }
            }
    ...
    static bool HandleTeleGroupCommand(ChatHandler*    handler,
                                       GameTele const* tele)
    {
        if (!tele) {
            handler->SendErrorMessage(LANG_COMMAND_TELE_NOTFOUND);
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr;
            ...
            if (player->IsBeingTeleported()) {
                handler->PSendSysMessage(LANG_IS_TELEPORTED,
                                         plNameLink.c_str());
                continue;
            }
            ...
            if (handler->needReportToTarget(player))
                ChatHandler(player->GetSession())
                    .PSendSysMessage(LANG_TELEPORTED_TO_BY, nameLink.c_str());
    ...
    static bool HandleTeleCommand(ChatHandler* handler, GameTele const* tele)
    {
        ...
        if (!map ||
            (map->IsBattlegroundOrArena() &&
             (player->GetMapId() != tele->mapId || !player->IsGameMaster()))) {
            handler->SendErrorMessage(LANG_CANNOT_TELE_TO_BG);
            return false;
        }
        ...
        Variant<Hyperlink<creature_entry>, uint32> creatureId)
    {
        ...
        CreatureTemplate const* creatureTemplate =
            ASSERT_NOTNULL(sObjectMgr->GetCreatureTemplate(*creatureId));
        ...
        Variant<Hyperlink<creature>, ObjectGuid::LowType> spawnId)
    {
        ...
        CreatureTemplate const* creatureTemplate =
            ASSERT_NOTNULL(sObjectMgr->GetCreatureTemplate(spawnpoint->id1));
    ...
    static bool HandleTeleNameNpcNameCommand(ChatHandler*     handler,
                                             PlayerIdentifier player,
                                             Tail             name)
    {
        ...
        QueryResult result = WorldDatabase.Query(
            ...
            "c.map, ct.name FROM creature c INNER JOIN creature_template ct ON "
            "c.id1 = ct.entry WHERE ct.name LIKE '{}'",
            normalizedName);
        if (!result) {
            handler->SendErrorMessage(LANG_COMMAND_GOCREATNOTFOUND);
            return false;
...
void AddSC_tele_commandscript() { new tele_commandscript(); }
