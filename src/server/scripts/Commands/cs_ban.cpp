...
public:
    ...
    static bool HandleBanCharacterCommand(ChatHandler* handler,
                                          char const*  args)
    {
        if (!*args)
            return false;
        ...
        switch (sBan->BanCharacter(name,
                                   durationStr,
                                   reasonStr,
                                   handler->GetSession()
                                       ? handler->GetSession()->GetPlayerName()
                                       : "")) {
        case BAN_SUCCESS:
            if (atoi(durationStr) > 0) {
                if (!sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
                    handler->PSendSysMessage(
                        LANG_BAN_YOUBANNED,
                        name.c_str(),
                        secsToTimeString(TimeStringToSecs(durationStr), true)
                            .c_str(),
                        reasonStr);
            }
            else {
                if (!sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
                    handler->PSendSysMessage(
                        LANG_BAN_YOUPERMBANNED, name.c_str(), reasonStr);
            }
            break;
        case BAN_NOTFOUND: {
            handler->SendErrorMessage(
                LANG_BAN_NOTFOUND, "character", name.c_str());
            return false;
        }
        default:
            break;
    ...
    static bool HandleBanAccountByCharCommand(ChatHandler* handler,
                                              char const*  args)
    {
    ...
    static bool
    HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        if (!*args)
            return false;
        ...
        switch (mode) {
        case BAN_ACCOUNT:
            banReturn = sBan->BanAccount(
                nameOrIP,
                durationStr,
                reasonStr,
                handler->GetSession() ? handler->GetSession()->GetPlayerName()
                                      : "");
            break;
        case BAN_CHARACTER:
            banReturn = sBan->BanAccountByPlayerName(
                nameOrIP,
                durationStr,
                reasonStr,
                handler->GetSession() ? handler->GetSession()->GetPlayerName()
        ...
        case BAN_SUCCESS:
            if (atoi(durationStr) > 0) {
                ...
                if (!sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
                    handler->PSendSysMessage(
                        LANG_BAN_YOUPERMBANNED, nameOrIP.c_str(), reasonStr);
            }
            break;
        ...
        case BAN_NOTFOUND:
            ...
            default:
                handler->SendErrorMessage(
                    LANG_BAN_NOTFOUND, "account", nameOrIP.c_str());
                break;
            case BAN_CHARACTER:
                handler->SendErrorMessage(
                    LANG_BAN_NOTFOUND, "character", nameOrIP.c_str());
                break;
            case BAN_IP:
                handler->SendErrorMessage(
                    LANG_BAN_NOTFOUND, "ip", nameOrIP.c_str());
                break;
            }
            return false;
        case BAN_LONGER_EXISTS:
            handler->PSendSysMessage("Unsuccessful! A longer ban is already "
                                     "present on this account!");
            break;
        default:
            break;
        }
    ...
    static bool HandleBanInfoAccountCommand(ChatHandler* handler,
                                            char const*  args)
    {
        if (!*args)
            return false;
    ...
    static bool HandleBanInfoHelper(uint32       accountId,
                                    char const*  accountName,
                                    ChatHandler* handler)
    {
        QueryResult result = LoginDatabase.Query(
            "SELECT FROM_UNIXTIME(bandate, '%Y-%m-%d..%H:%i:%s') as bandate, "
            "unbandate-bandate, active, unbandate, banreason, bannedby FROM "
            "account_banned WHERE id = '{}' ORDER BY bandate ASC",
            accountId);
        if (!result) {
        ...
        do {
            ...
            if (fields[2].Get<bool>() &&
                (fields[1].Get<uint64>() == uint64(0) ||
                 unbanDate >= GameTime::GetGameTime().count()))
                active = true;
            bool        permanent = (fields[1].Get<uint64>() == uint64(0));
            std::string banTime =
                permanent ? handler->GetAcoreString(LANG_BANINFO_INFINITE)
                          : secsToTimeString(fields[1].Get<uint64>(), true);
            handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
                                     fields[0].Get<std::string>().c_str(),
                                     banTime.c_str(),
                                     active ? handler->GetAcoreString(LANG_YES)
                                            : handler->GetAcoreString(LANG_NO),
                                     fields[4].Get<std::string>().c_str(),
                                     fields[5].Get<std::string>().c_str());
        } while (result->NextRow());

        return true;
    }

    static bool HandleBanInfoCharacterCommand(ChatHandler* handler,
                                              char const*  args)
    {
        if (!*args)
            return false;
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_SEL_BANINFO);
    ...
    static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(
            ...
            permanent ? handler->GetAcoreString(LANG_BANINFO_NEVER)
                      : fields[2].Get<std::string>().c_str(),
            permanent ? handler->GetAcoreString(LANG_BANINFO_INFINITE)
                      : secsToTimeString(fields[3].Get<uint64>(), true).c_str(),
            fields[4].Get<std::string>().c_str(),
            fields[5].Get<std::string>().c_str());
    ...
    static bool HandleBanListAccountCommand(ChatHandler* handler,
                                            char const*  args)
    {
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_DEL_EXPIRED_IP_BANS);
    ...
    static bool HandleBanListHelper(PreparedQueryResult result,
                                    ChatHandler*        handler)
    {
        ...
        if (handler->GetSession()) {
            do {
                ...
                QueryResult banResult = LoginDatabase.Query(
                    "SELECT account.username FROM account, account_banned "
                    "WHERE account_banned.id='{}' AND "
                    "account_banned.id=account.id",
                    accountid);
                if (banResult) {
                    Field* fields2 = banResult->Fetch();
                    handler->PSendSysMessage(
                        "%s", fields2[0].Get<std::string>().c_str());
                }
            } while (result->NextRow());
        }
        // Console wide output
            ...
            do {
                ...
                QueryResult banInfo = LoginDatabase.Query(
                    "SELECT bandate, unbandate, bannedby, banreason FROM "
                    "account_banned WHERE id = {} ORDER BY unbandate",
                    accountId);
                if (banInfo) {
                    Field* fields2 = banInfo->Fetch();
                    do {
                        tm tmBan = Acore::Time::TimeBreakdown(
                            fields2[0].Get<uint32>());

                        if (fields2[0].Get<uint32>() ==
                            fields2[1].Get<uint32>()) {
                            handler->PSendSysMessage(
                                "|%-15.15s|%02d-%02d-%02d %02d:%02d|   "
                                "permanent  |%-15.15s|%-15.15s|",
                                accountName.c_str(),
                                tmBan.tm_year % 100,
                                tmBan.tm_mon + 1,
                                tmBan.tm_mday,
                                tmBan.tm_hour,
                                tmBan.tm_min,
                                fields2[2].Get<std::string>().c_str(),
                                fields2[3].Get<std::string>().c_str());
                        }
                        else {
                            tm tmUnban = Acore::Time::TimeBreakdown(
                                fields2[1].Get<uint32>());
                            handler->PSendSysMessage(
                                "|%-15.15s|%02d-%02d-%02d "
                                "%02d:%02d|%02d-%02d-%02d "
    ...
    static bool HandleBanListCharacterCommand(ChatHandler* handler,
                                              char const*  args)
    {
        if (!*args)
            return false;
        ...
        if (handler->GetSession()) {
            do {
                ...
                if (banResult)
                    handler->PSendSysMessage(
                        "%s", (*banResult)[0].Get<std::string>().c_str());
            } while (result->NextRow());
        }
        // Console wide output
        else {
            ...
            do {
                ...
                if (banInfo) {
                    ...
                    do {
                        tm tmBan = Acore::Time::TimeBreakdown(
                            banFields[0].Get<uint32>());

                        if (banFields[0].Get<uint32>() ==
                            banFields[1].Get<uint32>()) {
                            handler->PSendSysMessage(
                                "|%-15.15s|%02d-%02d-%02d %02d:%02d|   "
                                "permanent  |%-15.15s|%-15.15s|",
                                char_name.c_str(),
                                ...
                                tmBan.tm_hour,
                                tmBan.tm_min,
                                banFields[2].Get<std::string>().c_str(),
                                banFields[3].Get<std::string>().c_str());
                        }
                        else {
                            tm tmUnban = Acore::Time::TimeBreakdown(
                                banFields[1].Get<uint32>());
    ...
    static bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (filter.empty()) {
            LoginDatabasePreparedStatement* stmt2 =
                LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_BANNED_ALL);
            result = LoginDatabase.Query(stmt2);
        }
        else {
            LoginDatabasePreparedStatement* stmt2 =
                LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_BANNED_BY_IP);
            stmt2->SetData(0, filter);
            result = LoginDatabase.Query(stmt2);
        }
        ...
        // Console wide output
            ...
            do {
                ...
                if (fields[1].Get<uint32>() == fields[2].Get<uint32>()) {
                    ...
                    tm tmUnban =
                        Acore::Time::TimeBreakdown(fields[2].Get<uint32>());
                    handler->PSendSysMessage(
    ...
    static bool HandleUnBanAccountCommand(ChatHandler* handler,
                                          char const*  args)
    {
        return HandleUnBanHelper(BAN_ACCOUNT, args, handler);
    }

    static bool HandleUnBanCharacterCommand(ChatHandler* handler,
                                            char const*  args)
    {
        if (!*args)
            return false;
    ...
    static bool HandleUnBanAccountByCharCommand(ChatHandler* handler,
                                                char const*  args)
    {
    ...
    static bool
    HandleUnBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        if (!*args)
            return false;
...
void AddSC_ban_commandscript() { new ban_commandscript(); }
