...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"go", goCommandTable}};
        return commandTable;
    }

    static bool
    DoTeleport(ChatHandler* handler, Position pos, uint32 mapId = MAPID_INVALID)
    {
    ...
    static bool
    HandleGoCreatureCIdCommand(ChatHandler* handler,
    ...
    static bool HandleGoCreatureSpawnIdCommand(
        ChatHandler*                                      handler,
    ...
    static bool HandleGoCreatureNameCommand(ChatHandler* handler, Tail name)
    {
        ...
        QueryResult result = WorldDatabase.Query(
            "SELECT entry FROM creature_template WHERE name = \"{}\" LIMIT 1",
            name.data());
        if (!result) {
            handler->SendErrorMessage(LANG_COMMAND_GOCREATNOTFOUND);
            return false;
    ...
    static bool HandleGoGameObjectSpawnIdCommand(ChatHandler* handler,
                                                 uint32       spawnId)
    {
        GameObjectData const* spawnpoint =
    ...
    static bool HandleGoGraveyardCommand(ChatHandler* handler, uint32 gyId)
    {
        ...
        if (!MapMgr::IsValidMapCoord(gy->Map, gy->x, gy->y, gy->z)) {
            handler->SendErrorMessage(
                LANG_INVALID_TARGET_COORD, gy->x, gy->y, gy->Map);
            return false;
        }
        ...
        player->TeleportTo(
            gy->Map, gy->x, gy->y, gy->z, player->GetOrientation());
        return true;
    }

    // teleport to grid
    static bool HandleGoGridCommand(ChatHandler*     handler,
                                    float            gridX,
                                    float            gridY,
                                    Optional<uint32> oMapId)
    {
        ...
        float      z   = std::max(map->GetHeight(x, y, MAX_HEIGHT),
                           map->GetWaterLevel(x, y));
    ...
    static bool
    HandleGoTaxinodeCommand(ChatHandler*                         handler,
                            Variant<Hyperlink<taxinode>, uint32> nodeId)
    {
        TaxiNodesEntry const* node = sTaxiNodesStore.LookupEntry(nodeId);
        if (!node) {
            handler->SendErrorMessage(LANG_COMMAND_GOTAXINODENOTFOUND,
                                      uint32(nodeId));
            return false;
        }
    ...
    static bool
    HandleGoZoneXYCommand(ChatHandler*                               handler,
                          float                                      x,
                          float                                      y,
                          Optional<Variant<Hyperlink<area>, uint32>> areaIdArg)
    {
        ...
        AreaTableEntry const* zoneEntry =
            areaEntry->zone ? sAreaTableStore.LookupEntry(areaEntry->zone)
                            : areaEntry;
        ASSERT(zoneEntry);
        ...
        if (map->Instanceable()) {
            handler->SendErrorMessage(
                ...
                areaEntry->ID,
                areaEntry->area_name[handler->GetSessionDbcLocale()],
                map->GetId(),
                map->GetMapName());
            return false;
        }
        ...
        if (!MapMgr::IsValidMapCoord(zoneEntry->mapid, x, y)) {
            handler->SendErrorMessage(
                LANG_INVALID_TARGET_COORD, x, y, zoneEntry->mapid);
            return false;
        }
    ...
    /**
     * @brief Teleports the GM to the specified world coordinates, optionally
     * specifying map ID and orientation.
     *
     * @param handler The ChatHandler that is handling the command.
     * @param args The coordinates to teleport to in format "x y z [mapId
     * [orientation]]".
    ...
    static bool HandleGoXYZCommand(ChatHandler* handler, Tail args)
    {
        ...
        std::wsregex_iterator floatRegexIterator(
            wInputCoords.begin(), wInputCoords.end(), floatRegex);
        ...
        uint32 mapId = locationValues.size() >= 4 ? uint32(locationValues[3])
                                                  : player->GetMapId();
        ...
        if (!sMapStore.LookupEntry(mapId) ||
            !MapMgr::IsValidMapCoord(mapId, x, y)) {
            handler->SendErrorMessage(LANG_INVALID_TARGET_COORD, x, y, mapId);
            return false;
        }
        ...
        float z = locationValues.size() >= 3
                      ? locationValues[2]
                      : std::max(map->GetHeight(x, y, MAX_HEIGHT),
                                 map->GetWaterLevel(x, y));
        // map ID (locationValues[3]) already handled above
        float o = locationValues.size() >= 5 ? locationValues[4]
    ...
    static bool HandleGoQuestCommand(ChatHandler*     handler,
                                     ...
                                     Quest const*     quest)
    {
        ...
        if (type == "starter") {
            ...
            for (auto itr = qr->begin(); itr != qr->end(); ++itr) {
                if (itr->second == entry) {
                    CreatureData const* spawnpoint =
                        GetCreatureData(handler, itr->first);
                    if (!spawnpoint) {
                        handler->SendErrorMessage(LANG_COMMAND_GOCREATNOTFOUND);
                        return false;
                    }
            ...
            for (auto itr = qr->begin(); itr != qr->end(); ++itr) {
                if (itr->second == entry) {
                    GameObjectData const* spawnpoint =
                        GetGameObjectData(handler, itr->first);
                    if (!spawnpoint) {
                        handler->SendErrorMessage(LANG_COMMAND_GOOBJNOTFOUND);
                        return false;
                    }
    ...
    static CreatureData const* GetCreatureData(ChatHandler* handler,
                                               uint32       entry)
    {
    ...
    static GameObjectData const* GetGameObjectData(ChatHandler* handler,
                                                   uint32       entry)
    {
...
void AddSC_go_commandscript() { new go_commandscript(); }
