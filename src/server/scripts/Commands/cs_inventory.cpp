...
constexpr std::array<const char*, MAX_ITEM_SUBCLASS_CONTAINER>
    bagSpecsToString = {"normal",
                        "soul",
                        "herb",
                        "enchanting",
                        "engineering",
...
public:
    ...
    static bool HandleInventoryCountCommand(ChatHandler*               handler,
                                            Optional<PlayerIdentifier> player)
    {
        ...
        for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END;
            ...
            if (Bag* bag = target->GetBagByPos(i)) {
                if (ItemTemplate const* bagTemplate = bag->GetTemplate()) {
                    if (bagTemplate->Class == ITEM_CLASS_CONTAINER ||
                        bagTemplate->Class == ITEM_CLASS_QUIVER) {
                        haveFreeSlot = true;
                        freeSlotsInBags[bagTemplate->SubClass] +=
                            bag->GetFreeSlots();
                    }
                }
            }
        ...
        if (haveFreeSlot) {
            ...
            for (uint8 i = ITEM_SUBCLASS_CONTAINER;
                 i < MAX_ITEM_SUBCLASS_CONTAINER;
                 ++i) {
            ...
            str << "Player " << target->GetName()
                << " does not have free slots in their bags";
        }

        if (freeSlotsForBags) {
            str << " and also has " << freeSlotsForBags
                << " free slots for bags";
        }
...
void AddSC_inventory_commandscript() { new inventory_commandscript(); }
