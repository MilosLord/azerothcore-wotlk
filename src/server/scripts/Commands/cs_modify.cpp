...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"morph", morphCommandTable},
    ...
    static void NotifyModification(ChatHandler* handler,
                                   Unit*        target,
                                   ...
                                   Args&&... args)
    {
        if (Player* player = target->ToPlayer()) {
            handler->PSendSysMessage(resourceMessage,
                                     std::forward<Args>(args)...,
                                     handler->GetNameLink(player).c_str());

            if (handler->needReportToTarget(player)) {
                ChatHandler(player->GetSession())
                    .PSendSysMessage(resourceReportMessage,
    ...
    static bool
    CheckModifyInt32(ChatHandler* handler, Player* target, int32 modifyValue)
    {
        if (modifyValue < 1) {
    ...
    static bool HandleModifyManaCommand(ChatHandler* handler, int32 manaPoints)
    {
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_MANA,
                                 handler->GetNameLink(target).c_str(),
                                 manaPoints,
                                 manaPoints);
    ...
    // Edit Player Energy
    static bool HandleModifyEnergyCommand(ChatHandler* handler,
                                          int32        energyPoints)
    {
        Player* target = handler->getSelectedPlayer();
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_ENERGY,
                                 handler->GetNameLink(target).c_str(),
                                 energyPoints / 10,
                                 energyPoints / 10);
        ...
        LOG_DEBUG("misc",
                  handler->GetAcoreString(LANG_CURRENT_ENERGY),
                  target->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleModifyRageCommand(ChatHandler* handler, int32 ragePoints)
    {
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_RAGE,
                                 handler->GetNameLink(target).c_str(),
                                 ragePoints / 10,
                                 ragePoints / 10);
    ...
    static bool HandleModifyRunicPowerCommand(ChatHandler* handler,
                                              int32        runePoints)
    {
        Player* target = handler->getSelectedPlayer();
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_RUNIC_POWER,
                                 handler->GetNameLink(target).c_str(),
                                 runePoints / 10,
                                 runePoints / 10);
    ...
    // Edit Player Faction
    static bool HandleModifyFactionCommand(ChatHandler*     handler,
                                           ...
                                           Optional<uint32> dynamicFlagID)
    {
        ...
        if (!factionID) {
            ...
            handler->PSendSysMessage(LANG_CURRENT_FACTION,
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_FACTION,
                                 target->GetGUID().GetCounter(),
                                 factionid,
                                 flag,
                                 npcflag,
    ...
    // Edit Player Spell
    static bool HandleModifySpellCommand(ChatHandler*     handler,
                                         uint8            spellFlatID,
                                         uint8            op,
                                         uint16           val,
                                         Optional<uint16> mark)
    {
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_SPELLFLATID,
                                 spellFlatID,
                                 val,
                                 mark ? *mark : 65535,
                                 handler->GetNameLink(target).c_str());
    ...
    // Edit Player TP
    static bool HandleModifyTalentCommand(ChatHandler* handler,
                                          uint32       talentPoints)
    {
        if (!talentPoints) {
            return false;
        ...
        if (target->GetTypeId() == TYPEID_PLAYER) {
            ...
            if (owner && owner->GetTypeId() == TYPEID_PLAYER &&
                ((Pet*)target)->IsPermanentPetFor(owner->ToPlayer())) {
                // check online security
    ...
    static bool CheckModifySpeed(ChatHandler* handler,
                                 Unit*        target,
                                 float        speed,
                                 float        minimumBound,
                                 float        maximumBound,
                                 bool         checkInFlight = true)
    {
        ...
        if (Player* player = target->ToPlayer()) {
            ...
            if (player->IsInFlight() && checkInFlight) {
                handler->SendErrorMessage(LANG_CHAR_IN_FLIGHT,
                                          handler->GetNameLink(player).c_str());
                return false;
            }
        }
    ...
    static bool HandleModifyScaleCommand(ChatHandler* handler, float scale)
    {
        ...
        if (Player* player = target->ToPlayer()) {
            ...
            handler->PSendSysMessage(LANG_YOU_CHANGE_SIZE,
                                     scale,
                                     handler->GetNameLink(player).c_str());
            if (handler->needReportToTarget(player))
                ChatHandler(player->GetSession())
    ...
    // Enable Player mount
    static bool HandleModifyMountCommand(ChatHandler*    handler,
                                         ...
                                         Optional<float> speed)
    {
        ...
        NotifyModification(
            handler, target, LANG_YOU_GIVE_MOUNT, LANG_MOUNT_GIVED);
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, Tail money)
    {
        ...
        if (moneyToAdd < 0) {
            ...
            LOG_DEBUG("chat.system",
                      handler->GetAcoreString(LANG_CURRENT_MONEY),
                      targetMoney,
                      moneyToAdd,
                      newmoney);
            if (newmoney <= 0) {
                handler->PSendSysMessage(LANG_YOU_TAKE_ALL_MONEY,
                                         handler->GetNameLink(target).c_str());
                if (handler->needReportToTarget(target))
                    ChatHandler(target->GetSession())
                ...
                handler->PSendSysMessage(LANG_YOU_TAKE_MONEY,
                                         std::abs(moneyToAdd),
                                         handler->GetNameLink(target).c_str());
                if (handler->needReportToTarget(target))
                    ChatHandler(target->GetSession())
            ...
            handler->PSendSysMessage(LANG_YOU_GIVE_MONEY,
                                     moneyToAdd,
                                     handler->GetNameLink(target).c_str());
            if (handler->needReportToTarget(target))
                ChatHandler(target->GetSession())
        ...
        LOG_DEBUG("chat.system",
                  handler->GetAcoreString(LANG_NEW_MONEY),
                  targetMoney,
                  moneyToAdd,
                  target->GetMoney());
    ...
    // Edit Unit field
    static bool
    HandleModifyBitCommand(ChatHandler* handler, uint16 field, uint32 bit)
    {
        Unit* target = handler->getSelectedUnit();
        if (!target) {
            handler->SendErrorMessage(LANG_NO_CHAR_SELECTED);
        ...
        // check online security
        if (target->GetTypeId() == TYPEID_PLAYER &&
            handler->HasLowerSecurity(target->ToPlayer())) {
            return false;
        }
    ...
    static bool HandleModifyHonorCommand(ChatHandler* handler, int32 amount)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_MODIFY_HONOR,
                                 handler->GetNameLink(target).c_str(),
    ...
    static bool HandleModifyRepCommand(ChatHandler*                handler,
                                       uint32                      factionId,
                                       ...
                                       Optional<int32>             delta)
    {
        ...
        if (rank.holds_alternative<std::string>()) {
            ...
            for (; r < MAX_REPUTATION_RANK; ++r) {
                std::string rank =
                    handler->GetAcoreString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
        ...
        if (factionEntry->reputationListID < 0) {
            handler->SendErrorMessage(
                ...
                factionEntry->name[handler->GetSessionDbcLocale()],
                factionId);
            return false;
        }

        target->GetReputationMgr().SetOneFactionReputation(
            factionEntry, float(amount), false);
        target->GetReputationMgr().SendState(
            target->GetReputationMgr().GetState(factionEntry));
    ...
    static bool HandleMorphTargetCommand(ChatHandler* handler, uint32 displayID)
    {
        ...
        if (!target) {
            target = handler->GetSession()->GetPlayer();
                 ...
                 handler->HasLowerSecurity(
                     target->ToPlayer())) // check online security
        {
            return false;
        }
    ...
    static bool HandleModifyArenaCommand(ChatHandler* handler, int32 amount)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA,
                                 handler->GetNameLink(target).c_str(),
    ...
    static bool HandleModifyGenderCommand(ChatHandler* handler,
                                          Tail         genderString)
    {
        ...
        PlayerInfo const* info =
            sObjectMgr->GetPlayerInfo(target->getRace(), target->getClass());
        if (!info) {
            return false;
        }
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_GENDER,
                                 handler->GetNameLink(target).c_str(),
                                 gender_full);
...
void AddSC_modify_commandscript() { new modify_commandscript(); }
