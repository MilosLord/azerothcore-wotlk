...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"reset", resetCommandTable}};
        return commandTable;
    }

    static bool
    HandleResetAchievementsCommand(ChatHandler*,
    ...
    static bool HandleResetHonorCommand(ChatHandler*,
                                        Optional<PlayerIdentifier> target)
    {
        if (!target) {
            return false;
        ...
        playerTarget->UpdateAchievementCriteria(
    ...
    static bool HandleResetStatsOrLevelHelper(Player* player)
    {
        ...
        if (!classEntry) {
            LOG_ERROR("dbc",
                      "Class {} not found in DBC (Wrong DBC files?)",
                      player->getClass());
            return false;
        }
        ...
        player->SetUInt32Value(
            UNIT_FIELD_BYTES_0,
            ((player->getRace()) | (player->getClass() << 8) |
             (player->getGender() << 16) | (powerType << 24)));
    ...
    static bool HandleResetLevelCommand(ChatHandler*,
                                        Optional<PlayerIdentifier> target)
    {
        if (!target) {
            return false;
        ...
        // set starting level
        uint32 startLevel =
            !playerTarget->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_INIT)
    ...
    static bool HandleResetSpellsCommand(ChatHandler*               handler,
                                         Optional<PlayerIdentifier> target)
    {
        if (!target) {
        ...
        if (target) {
            ...
            if (!handler->GetSession() ||
                handler->GetSession()->GetPlayer() != playerTarget)
            ...
            handler->PSendSysMessage(LANG_RESET_SPELLS_OFFLINE,
                                     target->GetName());
        }
    ...
    static bool HandleResetStatsCommand(ChatHandler*,
                                        Optional<PlayerIdentifier> target)
    {
        if (!target) {
            return false;
    ...
    static bool HandleResetTalentsCommand(ChatHandler*               handler,
                                          Optional<PlayerIdentifier> target)
    {
        ...
        if (targetPlayer) {
            ...
            if (!handler->GetSession() ||
                handler->GetSession()->GetPlayer() != targetPlayer)
                handler->PSendSysMessage(
                    LANG_RESET_TALENTS_ONLINE,
                    handler->GetNameLink(targetPlayer).c_str());
            ...
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(
                    CHAR_UPD_ADD_AT_LOGIN_FLAG);
            stmt->SetData(
                0, uint16(AT_LOGIN_RESET_TALENTS | AT_LOGIN_RESET_PET_TALENTS));
            stmt->SetData(1, target->GetGUID().GetCounter());
            CharacterDatabase.Execute(stmt);

            std::string nameLink = handler->playerLink(target->GetName());
            handler->PSendSysMessage(LANG_RESET_TALENTS_OFFLINE,
                                     nameLink.c_str());
            return true;
        }
    }
                                      ...
                                      std::string_view caseName)
    {
        ...
        // Command specially created as single command to prevent using short
        // case names
        if (caseName == "spells") {
            ...
            atLogin = AtLoginFlags(AT_LOGIN_RESET_TALENTS |
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_UPD_ALL_AT_LOGIN_FLAGS);
        stmt->SetData(0, uint16(atLogin));
        CharacterDatabase.Execute(stmt);

        sWorld->DoForAllOnlinePlayers(
            [&](Player* player) { player->SetAtLoginFlag(atLogin); });

        return true;
    }

    static bool
    HandleResetItemsEquippedCommand(ChatHandler*               handler,
                                    Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
    ...
    static bool HandleResetItemsInBagsCommand(ChatHandler* handler,
                                              Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
    ...
    static bool
    HandleResetItemsKeyringCommand(ChatHandler*               handler,
                                   Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
    ...
    static bool
    HandleResetItemsInCurrenciesListCommand(ChatHandler*               handler,
                                            Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
    ...
    static bool HandleResetItemsInBankCommand(ChatHandler* handler,
                                              Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
    ...
    static bool
    HandleResetItemsInVendorBuyBackTabCommand(ChatHandler* handler,
                                              Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
        ...
        if (!targetPlayer) {
            ...
            int16 deletedItemsCount =
                ResetItemsInVendorBuyBackTab(targetPlayer);
            handler->PSendSysMessage(
    ...
    static bool HandleResetItemsAllCommand(ChatHandler*               handler,
                                           Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
        ...
        if (!targetPlayer) {
            ...
            int16 deletedItemsInKeyringCount =
                ResetItemsInKeyring(targetPlayer);
            int16 deletedItemsInCurrenciesListCount =
                ResetItemsInCurrenciesList(targetPlayer);
            int16 deletedItemsInVendorBuyBackTabCount =
                ResetItemsInVendorBuyBackTab(targetPlayer);
    ...
    static bool
    HandleResetItemsAllAndDeleteBagsCommand(ChatHandler*               handler,
                                            Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer =
            GetPlayerFromIdentifierOrSelectedTarget(handler, target);
        ...
        if (!targetPlayer) {
            ...
            int16 deletedItemsInVendorBuyBackTabCount =
                ResetItemsInVendorBuyBackTab(targetPlayer);
            int16 deletedItemsStandardBagsCount =
                ResetItemsDeleteStandardBags(targetPlayer);
            int16 deletedItemsBankBagsCount =
                ResetItemsDeleteBankBags(targetPlayer);
...
private:
    static Player*
    GetPlayerFromIdentifierOrSelectedTarget(ChatHandler*               handler,
                                            Optional<PlayerIdentifier> target)
    {
        Player* targetPlayer = nullptr;
    ...
    static int16 ResetItemsInBags(Player* playerTarget)
    {
        ...
        for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END;
             ++i) {
            Bag* pBag =
                (Bag*)playerTarget->GetItemByPos(INVENTORY_SLOT_BAG_0, i);
            if (pBag) {
    ...
    static int16 ResetItemsInCurrenciesList(Player* playerTarget)
    {
        ...
        int16 count = 0;
        for (uint8 i = CURRENCYTOKEN_SLOT_START; i < CURRENCYTOKEN_SLOT_END;
             ++i) {
            Item* pItem = playerTarget->GetItemByPos(INVENTORY_SLOT_BAG_0, i);
            if (pItem) {
                playerTarget->DestroyItem(INVENTORY_SLOT_BAG_0, i, true);
...
void AddSC_reset_commandscript() { new reset_commandscript(); }
