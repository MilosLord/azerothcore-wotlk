...
public:
    ...
    static bool
    HandleTitlesCurrentCommand(ChatHandler*                      handler,
                               Variant<Hyperlink<title>, uint16> titleId)
    {
        ...
        handler->PSendSysMessage(
            LANG_TITLE_CURRENT_RES, uint32(titleId), titleNameStr, tNameLink);

        return true;
    }

    static bool
    HandleTitlesAddCommand(ChatHandler*                      handler,
                           Variant<Hyperlink<title>, uint16> titleId)
    {
        Player* target = handler->getSelectedPlayer();
        if (!target) {
        ...
        target->SetTitle(titleInfo);
        handler->PSendSysMessage(
            LANG_TITLE_ADD_RES, uint32(titleId), titleNameStr, tNameLink);

        return true;
    }

    static bool
    HandleTitlesRemoveCommand(ChatHandler*                      handler,
                              Variant<Hyperlink<title>, uint16> titleId)
    {
        Player* target = handler->getSelectedPlayer();
        if (!target) {
        ...
        handler->PSendSysMessage(
            LANG_TITLE_REMOVE_RES, uint32(titleId), titleNameStr, tNameLink);
    ...
    static bool HandleTitlesSetMaskCommand(ChatHandler* handler, uint64 mask)
    {
        ...
        if (!target->HasTitle(target->GetInt32Value(PLAYER_CHOSEN_TITLE))) {
            target->SetUInt32Value(PLAYER_CHOSEN_TITLE, 0);
            handler->PSendSysMessage(LANG_CURRENT_TITLE_RESET,
...
void AddSC_titles_commandscript() { new titles_commandscript(); }
