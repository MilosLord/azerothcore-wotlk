...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"item", itemCommandTable}};
        return commandTable;
    }

    static bool HandleItemRestoreCommand(ChatHandler*     handler,
                                         uint32           restoreId,
                                         PlayerIdentifier player)
    {
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_SEL_RECOVERY_ITEM);
        stmt->SetData(0, restoreId);
        PreparedQueryResult fields = CharacterDatabase.Query(stmt);

        if (!fields || !(*fields)[1].Get<uint32>() ||
            (*fields)[3].Get<uint32>() != player.GetGUID().GetCounter()) {
        ...
        if (Player* onlinePlayer = player.GetConnectedPlayer()) {
            ...
            MailDraft  draft(
                "Recovered Item",
                "We recovered a lost item in the twisting nether and noted "
                 "that it was yours.$B$BPlease find said object enclosed.");
            ...
            draft.SendMailTo(
                trans, MailReceiver(player.GetGUID().GetCounter()), sender);
    ...
    static bool HandleItemRestoreListCommand(ChatHandler*     handler,
                                             PlayerIdentifier player)
    {
        if (!HasItemDeletionConfig()) {
            handler->SendErrorMessage(LANG_COMMAND_DISABLED);
        ...
        do {
            ...
            std::string itemName = "";
            if (ItemTemplate const* item =
                    sObjectMgr->GetItemTemplate(itemId)) {
                itemName = item->Name1;
            }

            handler->PSendSysMessage(
                LANG_ITEM_RESTORE_LIST, id, itemName, itemId, count);
    ...
    static bool
    HandleItemMoveCommand(ChatHandler* handler, uint8 srcSlot, uint8 dstSlot)
    {
        if (srcSlot == dstSlot)
            return true;

        if (!handler->GetSession()->GetPlayer()->IsValidPos(
                INVENTORY_SLOT_BAG_0, srcSlot, true))
            return false;

        if (!handler->GetSession()->GetPlayer()->IsValidPos(
                INVENTORY_SLOT_BAG_0, dstSlot, false))
            return false;
    ...
    {
        return sWorld->getBoolConfig(CONFIG_ITEMDELETE_METHOD) ||
               sWorld->getBoolConfig(CONFIG_ITEMDELETE_VENDOR);
    }

    static bool HandleItemRefundCommand(ChatHandler*     handler,
                                        PlayerIdentifier player,
                                        uint32           itemId,
                                        uint32           extendedCost)
    {
        ItemExtendedCostEntry const* iece =
            sItemExtendedCostStore.LookupEntry(extendedCost);
        if (!iece) {
        ...
        if (Player* target = player.GetConnectedPlayer()) {
            if (!target->HasItemCount(itemId, 1, true)) {
                handler->SendErrorMessage(LANG_CMD_ITEM_REFUND_NOT_FOUND,
                                          itemId);
                return false;
            }
            ...
            if (iece->reqhonorpoints) {
                ...
                if (honor > sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS)) {
                    ...
                    ChatHandler(target->GetSession())
                        .PSendSysMessage(LANG_CMD_ITEM_REFUND_HONOR_FAILED,
                                         item->Name1);
                    return false;
                }
                ...
                ChatHandler(target->GetSession())
                    .PSendSysMessage(LANG_CMD_ITEM_REFUNDED_HONOR,
                                     item->Name1,
                                     item->ItemId,
                                     iece->reqhonorpoints);
                handler->PSendSysMessage(LANG_CMD_ITEM_REFUNDED_HONOR,
                                         item->Name1,
                                         item->ItemId,
                                         iece->reqhonorpoints);
            }
            ...
            if (iece->reqarenapoints) {
                uint32 arenapoints =
                    target->GetArenaPoints() + iece->reqarenapoints;
                if (arenapoints >
                    ...
                    handler->SendErrorMessage(
                        ...
                        iece->reqarenapoints);
                    ChatHandler(target->GetSession())
                        .PSendSysMessage(LANG_CMD_ITEM_REFUND_AP_FAILED,
                                         item->Name1);
                    return false;
                }
                ...
                ChatHandler(target->GetSession())
                    .PSendSysMessage(LANG_CMD_ITEM_REFUNDED_AP,
                                     item->Name1,
                                     item->ItemId,
                                     iece->reqarenapoints);
                handler->PSendSysMessage(LANG_CMD_ITEM_REFUNDED_AP,
                                         item->Name1,
                                         item->ItemId,
                                         iece->reqarenapoints);
            }
            ...
            if (result) {
                ...
                std::string msg =
                    ...
                    " has been removed and the used currency restored. This "
                    "mail contains any items used as currency.";
                ...
                for (uint32 const& reqItem : iece->reqitem) {
                    if (reqItem) {
                        ...
                        if (Item* item = Item::CreateItem(
                                reqItem, iece->reqitemcount[count])) {
                            item->SaveToDB(trans);
                            draft.AddItem(item);
                            foundItems = true;
                        }
                ...
                if (foundItems) {
                    draft.SendMailTo(
                        trans, MailReceiver(nullptr, guid), sender);
                }
...
void AddSC_item_commandscript() { new item_commandscript(); }
