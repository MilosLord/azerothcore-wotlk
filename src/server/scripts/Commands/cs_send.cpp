...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"send", sendCommandTable}};

        return commandTable;
    }

    static bool HandleSendItemsCommand(ChatHandler*               handler,
                                       ...
                                       Tail                       items)
    {
        ...
        for (auto const& itemString : Acore::Tokenize(items, ' ', true)) {
            ...
            default:
                handler->SendSysMessage(Acore::StringFormatFmt(
                    "> Incorrect item list format for '{}'", itemString));
                continue;
            }
            ...
            if (!itemCount || (itemTemplate->MaxCount > 0 &&
                               itemCount > uint32(itemTemplate->MaxCount))) {
                handler->SendErrorMessage(
                    LANG_COMMAND_INVALID_ITEM_COUNT, itemCount, itemID);
                return false;
            }
            ...
            if (itemList.size() > MAX_MAIL_ITEMS) {
                handler->SendErrorMessage(LANG_COMMAND_MAIL_ITEMS_LIMIT,
                                          MAX_MAIL_ITEMS);
                return false;
            }
        }

        // If the message is sent from console, set it as sent by the target
        // itself, like the other Customer Support mails.
        ...
        draft.SendMailTo(trans,
                         MailReceiver(target->GetConnectedPlayer(),
                                      target->GetGUID().GetCounter()),
                         sender);
        CharacterDatabase.CommitTransaction(trans);

        handler->PSendSysMessage(
            LANG_MAIL_SENT, handler->playerLink(target->GetName()).c_str());
        return true;
    }

    static bool HandleSendMailCommand(ChatHandler*               handler,
                                      Optional<PlayerIdentifier> target,
                                      QuotedString               subject,
                                      QuotedString               text)
    {
    ...
    static bool HandleSendMessageCommand(ChatHandler*               handler,
                                         Optional<PlayerIdentifier> target,
                                         Tail                       message)
    {
        if (!target) {
        ...
        handler->PSendSysMessage(LANG_SENDMESSAGE,
    ...
    static bool HandleSendMoneyCommand(ChatHandler*               handler,
                                       Optional<PlayerIdentifier> target,
                                       QuotedString               subject,
                                       QuotedString               text,
                                       uint32                     money)
...
void AddSC_send_commandscript() { new send_commandscript(); }
