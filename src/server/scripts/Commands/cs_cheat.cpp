...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"cheat", cheatCommandTable}};
        return commandTable;
    }

    static bool HandleGodModeCheatCommand(ChatHandler*   handler,
                                          Optional<bool> enableArg)
    {
        bool enable =
            !handler->GetSession()->GetPlayer()->GetCommandStatus(CHEAT_GOD);
        if (enableArg)
            enable = *enableArg;
    ...
    static bool HandleCasttimeCheatCommand(ChatHandler*   handler,
                                           Optional<bool> enableArg)
    {
        bool enable = !handler->GetSession()->GetPlayer()->GetCommandStatus(
            CHEAT_CASTTIME);
        if (enableArg)
            enable = *enableArg;

        if (enable) {
            handler->GetSession()->GetPlayer()->SetCommandStatusOn(
                CHEAT_CASTTIME);
            handler->SendSysMessage(
                "CastTime Cheat is ON. Your spells won't have a casttime.");
        }
        else {
            handler->GetSession()->GetPlayer()->SetCommandStatusOff(
                CHEAT_CASTTIME);
            handler->SendSysMessage(
    ...
    static bool HandleCoolDownCheatCommand(ChatHandler*   handler,
                                           Optional<bool> enableArg)
    {
        bool enable = !handler->GetSession()->GetPlayer()->GetCommandStatus(
            CHEAT_COOLDOWN);
    ...
    static bool HandlePowerCheatCommand(ChatHandler*   handler,
                                        Optional<bool> enableArg)
    {
        bool enable =
            !handler->GetSession()->GetPlayer()->GetCommandStatus(CHEAT_POWER);
        if (enableArg)
            enable = *enableArg;

        if (enable) {
            handler->GetSession()->GetPlayer()->SetCommandStatusOn(CHEAT_POWER);
            handler->SendSysMessage("Power Cheat is ON. You don't need "
                                    "mana/rage/energy to use spells.");
        }
        else {
            handler->GetSession()->GetPlayer()->SetCommandStatusOff(
                CHEAT_POWER);
    ...
    static bool HandleCheatStatusCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(
            ...
            player->GetCommandStatus(CHEAT_GOD) ? enabled : disabled);
        handler->PSendSysMessage(
            LANG_COMMAND_CHEAT_CD,
            player->GetCommandStatus(CHEAT_COOLDOWN) ? enabled : disabled);
        handler->PSendSysMessage(
            LANG_COMMAND_CHEAT_CT,
            player->GetCommandStatus(CHEAT_CASTTIME) ? enabled : disabled);
        handler->PSendSysMessage(
            LANG_COMMAND_CHEAT_POWER,
            player->GetCommandStatus(CHEAT_POWER) ? enabled : disabled);
        handler->PSendSysMessage(
            LANG_COMMAND_CHEAT_WW,
            player->GetCommandStatus(CHEAT_WATERWALK) ? enabled : disabled);
        handler->PSendSysMessage(LANG_COMMAND_CHEAT_TAXINODES,
                                 player->isTaxiCheater() ? enabled : disabled);

        return true;
    }

    static bool HandleWaterWalkCheatCommand(ChatHandler*   handler,
                                            Optional<bool> enableArg)
    {
        bool enable = !handler->GetSession()->GetPlayer()->GetCommandStatus(
            CHEAT_WATERWALK);
        ...
        if (enable) {
            handler->GetSession()->GetPlayer()->SetCommandStatusOn(
                CHEAT_WATERWALK);
            handler->GetSession()->GetPlayer()->SetMovement(
                MOVE_WATER_WALK); // ON
            handler->SendSysMessage(
    ...
    static bool HandleTaxiCheatCommand(ChatHandler*   handler,
                                       Optional<bool> enableArg)
    {
        ...
        else if (handler->HasLowerSecurity(
                     chr, ObjectGuid::Empty)) // check online security
            return false;
        ...
        if (enable) {
            ...
            handler->PSendSysMessage(LANG_YOU_GIVE_TAXIS,
                                     handler->GetNameLink(chr).c_str());
            if (handler->needReportToTarget(chr))
                ChatHandler(chr->GetSession())
                    .PSendSysMessage(LANG_YOURS_TAXIS_ADDED,
                                     handler->GetNameLink().c_str());
        }
        else {
            chr->SetTaxiCheater(false);
            handler->PSendSysMessage(LANG_YOU_REMOVE_TAXIS,
                                     handler->GetNameLink(chr).c_str());
            if (handler->needReportToTarget(chr))
                ChatHandler(chr->GetSession())
    ...
    static bool HandleExploreCheatCommand(ChatHandler* handler, bool reveal)
    {
        ...
        if (reveal) {
            handler->PSendSysMessage(LANG_YOU_SET_EXPLORE_ALL,
                                     handler->GetNameLink(chr).c_str());
            if (handler->needReportToTarget(chr))
                ChatHandler(chr->GetSession())
            ...
            handler->PSendSysMessage(LANG_YOU_SET_EXPLORE_NOTHING,
                                     handler->GetNameLink(chr).c_str());
            if (handler->needReportToTarget(chr))
                ChatHandler(chr->GetSession())
...
void AddSC_cheat_commandscript() { new cheat_commandscript(); }
