...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable commandTable = {{"npc", npcCommandTable}};
        return commandTable;
    }
    ...
    static bool HandleNpcAddCommand(ChatHandler* handler, CreatureEntry id)
    {
        ...
        if (Transport* tt = chr->GetTransport())
            if (MotionTransport* trans = tt->ToMotionTransport()) {
                ...
                creature->SaveToDB(trans->GetGOInfo()->moTransport.mapID,
                                   1 << map->GetSpawnMode(),
        ...
        // otherwise old values might trigger asserts or cause undefined
        // behavior
        ...
        sObjectMgr->AddCreatureToGrid(spawnId,
                                      sObjectMgr->GetCreatureData(spawnId));
        return true;
    }

    // add item in vendorlist
    static bool HandleNpcAddVendorItemCommand(ChatHandler*        handler,
                                              ...
                                              Optional<bool>      addMulti)
    {
        ...
        uint32 vendor_entry = addMulti.value_or(false)
                                  ? handler->GetSession()->GetCurrentVendor()
                                  : vendor->GetEntry();

        if (!sObjectMgr->IsVendorItemValid(
                vendor_entry,
                itemId,
                maxcount,
                incrtime,
        ...
        sObjectMgr->AddVendorItem(
            vendor_entry, itemId, maxcount, incrtime, extendedcost);

        handler->PSendSysMessage(LANG_ITEM_ADDED_TO_LIST,
                                 itemId,
                                 item->Name1.c_str(),
                                 maxcount,
                                 incrtime,
    ...
    static bool HandleNpcAddMoveCommand(ChatHandler*    handler,
                                        CreatureSpawnId lowGuid)
    {
        // attempt check creature existence by DB data
        CreatureData const* data = sObjectMgr->GetCreatureData(lowGuid);
        if (!data) {
            handler->SendErrorMessage(LANG_COMMAND_CREATGUIDNOTFOUND,
                                      uint32(lowGuid));
            return false;
        }
    ...
    static bool HandleNpcSetEntryCommand(ChatHandler*  handler,
    ...
    {
        if (lvl < 1 ||
            lvl > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL) + 3) {
    ...
    static bool HandleNpcDeleteVendorItemCommand(ChatHandler*        handler,
                                                 ItemTemplate const* item,
                                                 Optional<bool>      addMulti)
    {
        Creature* vendor = handler->getSelectedCreature();
        ...
        uint32 itemId = item->ItemId;
        if (!sObjectMgr->RemoveVendorItem(
                addMulti.value_or(false)
                    ? handler->GetSession()->GetCurrentVendor()
                    : vendor->GetEntry(),
                itemId)) {
        ...
        handler->PSendSysMessage(
            LANG_ITEM_DELETED_FROM_LIST, itemId, item->Name1.c_str());
        return true;
    }

    // set faction of creature
    static bool HandleNpcSetFactionIdCommand(ChatHandler* handler,
                                             uint32       factionId)
    {
        ...
        // ..and DB
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_FACTION);
    ...
    static bool HandleNpcSetFactionTempIdCommand(ChatHandler* handler,
                                                 uint32       tempfaction)
    {
    ...
    static bool HandleNpcSetFlagCommand(ChatHandler* handler, uint32 npcFlags)
    {
        ...
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_NPCFLAG);
    ...
    static bool
    HandleNpcSetDataCommand(ChatHandler* handler, uint32 data_1, uint32 data_2)
    {
        Creature* creature = handler->getSelectedCreature();
        ...
        std::string AIorScript =
            !creature->GetAIName().empty()
                ? "AI type: " + creature->GetAIName()
                : (!creature->GetScriptName().empty()
                       ? "Script Name: " + creature->GetScriptName()
                       : "No AI or Script Name Set");
        handler->PSendSysMessage(LANG_NPC_SETDATA,
    ...
    static bool HandleNpcFollowCommand(ChatHandler* handler)
    {
        ...
        creature->GetMotionMaster()->MoveFollow(
            player, PET_FOLLOW_DIST, creature->GetFollowAngle());

        handler->PSendSysMessage(LANG_CREATURE_FOLLOW_YOU_NOW,
                                 creature->GetName().c_str());
        return true;
    }
    ...
    static bool HandleNpcInfoCommand(ChatHandler* handler)
    {
        ...
        int64 curRespawnDelay =
            target->GetRespawnTimeEx() - GameTime::GetGameTime().count();
        if (curRespawnDelay < 0)
            curRespawnDelay = 0;
        std::string curRespawnDelayStr =
            secsToTimeString(uint64(curRespawnDelay), true);
        std::string defRespawnDelayStr =
            secsToTimeString(target->GetRespawnDelay(), true);

        handler->PSendSysMessage(LANG_NPCINFO_CHAR,
                                 target->GetSpawnId(),
        ...
        handler->PSendSysMessage(LANG_NPCINFO_EQUIPMENT,
                                 target->GetCurrentEquipmentId(),
                                 target->GetOriginalEquipmentId());
        handler->PSendSysMessage(LANG_NPCINFO_HEALTH,
                                 target->GetCreateHealth(),
                                 target->GetMaxHealth(),
                                 target->GetHealth());
        handler->PSendSysMessage(LANG_NPCINFO_FLAGS,
                                 target->GetUnitFlags(),
                                 target->GetUnitFlags2(),
                                 target->GetDynamicFlags(),
                                 target->GetFaction());
        handler->PSendSysMessage(LANG_COMMAND_RAWPAWNTIMES,
                                 defRespawnDelayStr.c_str(),
                                 curRespawnDelayStr.c_str());
        handler->PSendSysMessage(LANG_NPCINFO_LOOT,
                                 cInfo->lootid,
                                 cInfo->pickpocketLootId,
                                 cInfo->SkinLootId);
        handler->PSendSysMessage(LANG_NPCINFO_DUNGEON_ID,
                                 target->GetInstanceId());
        handler->PSendSysMessage(LANG_NPCINFO_PHASEMASK,
                                 target->GetPhaseMask());
        handler->PSendSysMessage(LANG_NPCINFO_ARMOR, target->GetArmor());
        handler->PSendSysMessage(LANG_NPCINFO_POSITION,
                                 float(target->GetPositionX()),
                                 float(target->GetPositionY()),
                                 float(target->GetPositionZ()));
        handler->PSendSysMessage(LANG_NPCINFO_AIINFO,
                                 target->GetAIName().c_str(),
                                 target->GetScriptName().c_str());

        for (uint8 i = 0; i < NPCFLAG_COUNT; i++) {
            if (npcflags & npcFlagTexts[i].flag) {
                handler->PSendSysMessage(npcFlagTexts[i].text,
                                         npcFlagTexts[i].flag);
            }
        }

        handler->PSendSysMessage(LANG_NPCINFO_MECHANIC_IMMUNE,
                                 mechanicImmuneMask);
        for (uint8 i = 1; i < MAX_MECHANIC; ++i) {
            if (mechanicImmuneMask & (1 << (mechanicImmunes[i].flag - 1))) {
                handler->PSendSysMessage(mechanicImmunes[i].text,
                                         mechanicImmunes[i].flag);
            }
        }

        handler->PSendSysMessage(LANG_NPCINFO_SPELL_SCHOOL_IMMUNE,
                                 spellSchoolImmuneMask);
        for (uint8 i = 0; i < MAX_SPELL_SCHOOL; ++i) {
            if (spellSchoolImmuneMask & (1 << spellSchoolImmunes[i].flag)) {
                handler->PSendSysMessage(spellSchoolImmunes[i].text,
                                         spellSchoolImmunes[i].flag);
            }
        }
    ...
    static bool HandleNpcNearCommand(ChatHandler* handler, Optional<float> dist)
    {
        ...
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_SEL_CREATURE_NEAREST);
        ...
        if (result) {
            do {
                ...
                handler->PSendSysMessage(LANG_CREATURE_LIST_CHAT,
                                         guid,
                                         entry,
                                         guid,
                                         creatureTemplate->Name.c_str(),
                                         x,
                                         y,
                                         z,
                                         mapId,
    ...
    static bool HandleNpcMoveCommand(ChatHandler* handler)
    {
        ...
        if (creature) {
            ...
            if (creature->IsAlive()) // dead creature will reset movement
        ...
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_POSITION);
        stmt->SetData(0, x);
        stmt->SetData(1, y);
        stmt->SetData(2, z);
        stmt->SetData(3, o);
    ...
    static bool HandleNpcSetModelCommand(ChatHandler* handler, uint32 displayId)
    {
        ...
        if (!sCreatureDisplayInfoStore.LookupEntry(displayId)) {
            handler->SendErrorMessage(LANG_COMMAND_FACTION_INVPARAM,
                                      Acore::ToString(displayId).c_str());
            return false;
        }
     ...
     * <li> random - NPC will move randomly according to the wander_distance
     * </li> <li> way - NPC will move with given waypoints set </li>
     * </ul>
     * additional parameter: NODEL - so no waypoints are deleted, if you
     *                       change the movement type
     */
    static bool
    HandleNpcSetMoveTypeCommand(ChatHandler*                      handler,
                                Optional<CreatureSpawnId>         lowGuid,
                                Variant<EXACT_SEQUENCE("stay"),
                                        EXACT_SEQUENCE("random"),
                                        EXACT_SEQUENCE("way")>    type,
                                Optional<EXACT_SEQUENCE("nodel")> nodel)
    {
        ...
        else // case .setmovetype #creature_guid $move_type (with selected
             // creature)
        {
            lowguid = *lowGuid;
    ...
    static bool HandleNpcSetWanderDistanceCommand(ChatHandler* handler,
                                                  float        option)
    {
        if (option < 0.0f) {
        ...
        if (creature->IsAlive()) // dead creature will reset movement generator
                                 // at respawn
        {
            creature->setDeathState(DeathState::JustDied);
            creature->Respawn();
    ...
    // spawn time handling
    static bool HandleNpcSetSpawnTimeCommand(ChatHandler* handler,
                                             std::string  spawnTimeStr)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_SPAWNTIME,
                                 secsToTimeString(spawnTime, true).c_str());
    ...
    static bool HandleNpcUnFollowCommand(ChatHandler* handler)
    {
        ...
        if (/*creature->GetMotionMaster()->empty() ||*/
            creature->GetMotionMaster()->GetCurrentMovementGeneratorType() !=
            FOLLOW_MOTION_TYPE) {
            handler->SendErrorMessage(LANG_CREATURE_NOT_FOLLOW_YOU,
                                      creature->GetName().c_str());
            return false;
        }
        ...
        handler->PSendSysMessage(LANG_CREATURE_NOT_FOLLOW_YOU_NOW,
                                 creature->GetName().c_str());
        return true;
    }

    // make npc whisper to player
    static bool HandleNpcWhisperCommand(ChatHandler*       handler,
    ...
    static bool HandleNpcAddTempSpawnCommand(ChatHandler*  handler,
                                             CreatureEntry id)
    {
        Player* chr = handler->GetSession()->GetPlayer();
                                             ...
                                             ObjectGuid::LowType leaderGUID)
    {
        ...
        group_member.follow_angle =
            (creature->GetAngle(chr) - chr->GetOrientation()) * 180 / M_PI;
        group_member.follow_dist =
            sqrtf(pow(chr->GetPositionX() - creature->GetPositionX(), int(2)) +
                  pow(chr->GetPositionY() - creature->GetPositionY(), int(2)));
        ...
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_INS_CREATURE_FORMATION);
        ...
        handler->PSendSysMessage(
            "Creature %u added to formation with leader %u",
            lowguid,
            leaderGUID);
    ...
    static bool HandleNpcSetLinkCommand(ChatHandler*        handler,
                                        ObjectGuid::LowType linkguid)
    {
        ...
        if (!creature->GetSpawnId()) {
            handler->SendErrorMessage(
                "Selected creature %u isn't in creature table",
                creature->GetGUID().GetCounter());
            return false;
        }

        if (!sObjectMgr->SetCreatureLinkedRespawn(creature->GetSpawnId(),
                                                  linkguid)) {
            handler->SendErrorMessage(
        ...
        handler->PSendSysMessage(
            "LinkGUID '%u' added to creature with DBTableGUID: '%u'",
            linkguid,
            creature->GetSpawnId());
        return true;
    }
...
void AddSC_npc_commandscript() { new npc_commandscript(); }
