...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable ticketCommandTable = {
            ...
            {"viewname",
             ...
             SEC_GAMEMASTER,
             Console::Yes}};
        static ChatCommandTable commandTable = {{"ticket", ticketCommandTable}};
        return commandTable;
    }

    static bool HandleGMTicketAssignToCommand(ChatHandler* handler,
                                              uint32       ticketId,
                                              std::string  target)
    {
        ...
        uint32 targetGmLevel =
            AccountMgr::GetSecurity(targetAccountId, realm.Id.Realm);
        ...
        if (ticket->IsAssignedTo(targetGuid)) {
            handler->PSendSysMessage(LANG_COMMAND_TICKETASSIGNERROR_B,
                                     ticket->GetId());
            return true;
        }
        ...
        if (player && ticket->IsAssignedNotTo(player->GetGUID())) {
            handler->PSendSysMessage(LANG_COMMAND_TICKETALREADYASSIGNED,
                                     ticket->GetId(),
                                     target.c_str());
            return true;
        }
        ...
        ticket->SetAssignedTo(targetGuid,
                              AccountMgr::IsAdminAccount(targetGmLevel));
        ticket->SaveToDB(trans);
        sTicketMgr->UpdateLastChange();

        std::string msg = ticket->FormatMessageString(
            *handler, nullptr, target.c_str(), nullptr, nullptr);
        handler->SendGlobalGMSysMessage(msg.c_str());
        return true;
    }

    static bool HandleGMTicketCloseByIdCommand(ChatHandler* handler,
                                               uint32       ticketId)
    {
        GmTicket* ticket = sTicketMgr->GetTicket(ticketId);
        ...
        Player* player = handler->GetSession()
                             ...
                             : nullptr;
        if (player && ticket->IsAssignedNotTo(player->GetGUID())) {
            handler->PSendSysMessage(LANG_COMMAND_TICKETCANNOTCLOSE,
                                     ticket->GetId());
            return true;
        }

        sTicketMgr->ResolveAndCloseTicket(
            ticket->GetId(), player ? player->GetGUID() : ObjectGuid::Empty);
        sTicketMgr->UpdateLastChange();

        std::string msg = ticket->FormatMessageString(
            *handler,
            player ? player->GetName().c_str() : "Console",
            nullptr,
            nullptr,
            nullptr);
        handler->SendGlobalGMSysMessage(msg.c_str());
        ...
        if (Player* submitter = ticket->GetPlayer()) {
            ...
            ChatHandler(submitter->GetSession())
                .SendSysMessage(LANG_TICKET_CLOSED);
        }
        return true;
    }

    static bool HandleGMTicketCommentCommand(ChatHandler* handler,
                                             uint32       ticketId,
                                             Tail         comment)
    {
        if (comment.empty())
        ...
        // Cannot comment ticket assigned to someone else (unless done from the
        // Console)
        Player* player = handler->GetSession()
                             ? handler->GetSession()->GetPlayer()
                             : nullptr;
        ...
        std::string       msg          = ticket->FormatMessageString(
            *handler,
            assignedName.empty() ? nullptr : assignedName.c_str(),
            nullptr,
            nullptr,
            nullptr);

        msg += handler->PGetParseString(LANG_COMMAND_TICKETLISTADDCOMMENT,
    ...
    static bool HandleGMTicketCompleteCommand(ChatHandler* handler,
                                              uint32       ticketId)
    {
        GmTicket* ticket = sTicketMgr->GetTicket(ticketId);
        if (!ticket || ticket->IsClosed() || ticket->IsCompleted()) {
        ...
        if (Player* player2 = ticket->GetPlayer()) {
            ...
            ChatHandler(player2->GetSession())
                .SendSysMessage(LANG_TICKET_COMPLETED);
        }

        Player* gm = handler->GetSession() ? handler->GetSession()->GetPlayer()
                                           : nullptr;
        ...
        std::string msg = ticket->FormatMessageString(
            *handler, nullptr, nullptr, nullptr, nullptr);
        msg += handler->PGetParseString(LANG_COMMAND_TICKETCOMPLETED,
                                        gm ? gm->GetName().c_str() : "Console");
        handler->SendGlobalGMSysMessage(msg.c_str());
        sTicketMgr->UpdateLastChange();
        return true;
    }

    static bool HandleGMTicketDeleteByIdCommand(ChatHandler* handler,
                                                uint32       ticketId)
    {
        GmTicket* ticket = sTicketMgr->GetTicket(ticketId);
        if (!ticket) {
        ...
        std::string msg = ticket->FormatMessageString(
            *handler,
            ...
            nullptr,
            handler->GetSession()
                ? handler->GetSession()->GetPlayer()->GetName().c_str()
                : "Console");
        handler->SendGlobalGMSysMessage(msg.c_str());
    ...
    static bool HandleGMTicketEscalateCommand(ChatHandler* handler,
                                              uint32       ticketId)
    {
        GmTicket* ticket = sTicketMgr->GetTicket(ticketId);
        if (!ticket || ticket->IsClosed() || ticket->IsCompleted() ||
            ticket->GetEscalatedStatus() != TICKET_UNASSIGNED) {
            handler->SendSysMessage(LANG_COMMAND_TICKETNOTEXIST);
            return true;
        }
    ...
    static bool HandleToggleGMTicketSystem(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(status ? LANG_ALLOW_TICKETS
                                        : LANG_DISALLOW_TICKETS);
        return true;
    }

    static bool HandleGMTicketUnAssignCommand(ChatHandler* handler,
                                              uint32       ticketId)
    {
        GmTicket* ticket = sTicketMgr->GetTicket(ticketId);
        if (!ticket || ticket->IsClosed()) {
        ...
        if (!ticket->IsAssigned()) {
            handler->PSendSysMessage(LANG_COMMAND_TICKETNOTASSIGNED,
                                     ticket->GetId());
            return true;
        }
        ...
        // Check security
        //! If no m_session present it means we're issuing this command from the
        //! console
        uint32 mySecurity = handler->GetSession()
        ...
        std::string assignedTo =
            ticket->GetAssignedToName(); // copy assignedto name because we need
                                         // it after the ticket has been
                                         // unnassigned
        CharacterDatabaseTransaction trans =
            CharacterDatabaseTransaction(nullptr);
    ...
    static bool HandleGMTicketGetByIdCommand(ChatHandler* handler,
                                             uint32       ticketId)
    {
        GmTicket* ticket = sTicketMgr->GetTicket(ticketId);
        if (!ticket || ticket->IsClosed() || ticket->IsCompleted()) {
    ...
    static bool HandleGMTicketGetByNameCommand(ChatHandler* handler,
                                               std::string  name)
    {
    ...
    static bool _HandleGMTicketResponseAppendCommand(uint32       ticketId,
                                                     bool         newLine,
                                                     ChatHandler* handler)
    {
        char* response = strtok(nullptr, "\n");
    ...
    static bool HandleGMTicketResponseAppendCommand(ChatHandler* handler,
                                                    uint32       ticketId)
    {
        return _HandleGMTicketResponseAppendCommand(ticketId, false, handler);
    }

    static bool HandleGMTicketResponseAppendLnCommand(ChatHandler* handler,
                                                      uint32       ticketId)
    {
...
void AddSC_ticket_commandscript() { new ticket_commandscript(); }
