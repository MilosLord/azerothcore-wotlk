...
struct npc_preparations_for_war_vehicle : public NullCreatureAI {
    ...
    void InitializeAI() override
    {
        ...
        transportEntry =
            (me->GetEntry() == NPC_HAMMERHEAD ? TRANSPORT_ORGRIMS_HAMMER
        ...
        if (searchForShipTimer) {
            ...
            if (searchForShipTimer >= 3000) {
                ...
                TransportsContainer const& transports =
                    me->GetMap()->GetAllTransports();
                for (TransportsContainer::const_iterator itr =
                         transports.begin();
                     itr != transports.end();
                     ++itr) {
...
struct npc_warmage_violetstand : public ScriptedAI {
    ...
    void Reset() override { targetGUID.Clear(); }
        ...
        if (me->GetEntry() == NPC_WARMAGE_SARINA) {
            ...
            if (!targetGUID)
                if (Creature* pOrb = GetClosestCreatureWithEntry(
                        me, NPC_TRANSITUS_SHIELD_DUMMY, 32.0f))
                    targetGUID = pOrb->GetGUID();
        }
