...
const Position hadronoxSteps[4] = {{607.9f, 512.8f, 695.3f, 0.0f},
...
public:
    ...
    struct boss_hadronoxAI : public BossAI {
        boss_hadronoxAI(Creature* creature)
            : BossAI(creature, DATA_HADRONOX_EVENT)
        {
        }
        ...
        void Reset() override
        {
            ...
            BossAI::Reset();
            me->SummonCreature(
                NPC_ANUB_AR_CRUSHER, 542.9f, 519.5f, 741.24f, 2.14f);
        }
        ...
        uint32 GetData(uint32 data) const override
        {
            if (data == me->GetEntry())
                return !me->isActiveObject() ||
                       events.GetNextEventTime(EVENT_HADRONOX_MOVE4) != 0;
            return 0;
        }
        ...
        bool AnyPlayerValid() const
        {
            ...
            for (Map::PlayerList::const_iterator itr = playerList.begin();
                 itr != playerList.end();
                 ++itr)
                if (me->GetDistance(itr->GetSource()) < 130.0f &&
                    itr->GetSource()->IsAlive() &&
                    !itr->GetSource()->IsGameMaster() &&
                    me->CanCreatureAttack(itr->GetSource()))
                    return true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_HADRONOX_ACID:
                if (Unit* target =
                        SelectTarget(SelectTargetMethod::Random, 0, 100, false))
            ...
            case EVENT_HADRONOX_MOVE3:
                ...
                me->GetMotionMaster()->MoveCharge(
                    hadronoxSteps[eventId - 1].GetPositionX(),
        ...
        void JustSummoned(Creature* summon) override
        {
            if (summon->GetEntry() != me->GetEntry()) {
                ...
                summon->GetMotionMaster()->MoveFollow(
                    me, 0.1f, 0.0f + M_PI * 0.3f * summons.size());
            }
            summons.Summon(summon);
        }
            ...
            if (me->ToTempSummon())
                if (Unit* summoner = me->ToTempSummon()->GetSummonerUnit())
                    if (summoner->GetEntry() != me->GetEntry()) {
                        ...
                        me->SummonCreature(NPC_ANUB_AR_CRUSHER,
                                           519.58f,
                                           573.73f,
                                           734.30f,
                                           4.50f);
...
public:
    spell_hadronox_summon_periodic(const char* name,
                                   uint32      delay,
                                   uint32      spellEntry)
        : SpellScriptLoader(name), _delay(delay), _spellEntry(spellEntry)
    {
    }

    class spell_hadronox_summon_periodic_AuraScript : public AuraScript {
    public:
        spell_hadronox_summon_periodic_AuraScript(uint32 delay,
                                                  uint32 spellEntry)
            : _delay(delay), _spellEntry(spellEntry)
        {
        }
        ...
        {
            GetAura()
                ->GetEffect(auraEffect->GetEffIndex())
                ->SetPeriodicTimer(_delay);
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_hadronox_summon_periodic_AuraScript::HandlePeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            OnEffectApply += AuraEffectApplyFn(
                spell_hadronox_summon_periodic_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
    ...
    {
        return new spell_hadronox_summon_periodic_AuraScript(_delay,
                                                             _spellEntry);
    }
...
public:
    spell_hadronox_leech_poison()
        : SpellScriptLoader("spell_hadronox_leech_poison")
    {
    }
    ...
    class spell_hadronox_leech_poison_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_hadronox_leech_poison_AuraScript::HandleEffectRemove,
                EFFECT_0,
...
public:
    achievement_hadronox_denied()
        : AchievementCriteriaScript("achievement_hadronox_denied")
    {
    }
...
void AddSC_boss_hadronox()
{
    ...
    new spell_hadronox_summon_periodic(
        "spell_hadronox_summon_periodic_champion",
        15000,
    ...
    new spell_hadronox_summon_periodic(
        ...
        SPELL_SUMMON_ANUBAR_NECROMANCER);
    new spell_hadronox_summon_periodic(
        "spell_hadronox_summon_periodic_crypt_fiend",
        5000,
        SPELL_SUMMON_ANUBAR_CRYPT_FIEND);
