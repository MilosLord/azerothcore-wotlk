...
public:
    ...
    struct boss_anub_arakAI : public BossAI {
        boss_anub_arakAI(Creature* creature)
            : BossAI(creature, DATA_ANUBARAK_EVENT)
        {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_POUND:
                if (Unit* target =
                        SelectTarget(SelectTargetMethod::Random, 0, 10.0f)) {
            ...
            case EVENT_SUMMON_VENOMANCER:
                SummonHelpers(
                    550.34f, 316.00f, 234.30f, SPELL_SUMMON_VENOMANCER);
                break;
            }
...
class spell_azjol_nerub_carrion_beetels : public AuraScript {
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        GetUnitOwner()->CastSpell(
            GetUnitOwner(), SPELL_SUMMON_CARRION_BEETLES, true);
        GetUnitOwner()->CastSpell(
            GetUnitOwner(), SPELL_SUMMON_CARRION_BEETLES, true);
    }
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_azjol_nerub_carrion_beetels::HandleEffectPeriodic,
            EFFECT_0,
...
class spell_azjol_nerub_pound : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_azjol_nerub_pound::HandleApplyAura,
                          EFFECT_0,
...
class spell_azjol_nerub_impale_summon : public SpellScript {
    ...
    void SetDest(SpellDestination& dest)
    {
        ...
        float floorZ = GetCaster()->GetMapHeight(GetCaster()->GetPositionX(),
    ...
    void Register() override
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_azjol_nerub_impale_summon::SetDest,
            EFFECT_0,
            TARGET_DEST_CASTER);
    }
};
