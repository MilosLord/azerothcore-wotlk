...
enum Spells {
    ...
    SPELL_SUMMON_SWARM_GUARD =
        56120, // 1x NPC_AHNKAHAR_GUARDIAN_ENTRY  -- at 50%hp
    SPELL_SWARM = 56281,
...
struct boss_elder_nadox : public BossAI {
    boss_elder_nadox(Creature* creature)
        : BossAI(creature, DATA_ELDER_NADOX), guardianSummoned(false),
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        me->GetCreatureListWithEntryInGrid(
            eggs, NPC_AHNKAHAR_SWARM_EGG, 250.0f);
        if (!eggs.empty()) {
        ...
        // Guardian eggs
        me->GetCreatureListWithEntryInGrid(
            eggs, NPC_AHNKAHAR_GUARDIAN_EGG, 250.0f);
        if (!eggs.empty()) {
            for (Creature* const egg : eggs) {
                if (egg) {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 const eventId = events.ExecuteEvent()) {
            ...
            case EVENT_BROOD_RAGE: {
                if (Creature* pSwarmer =
                        me->FindNearestCreature(NPC_AHNKAHAR_SWARMER, 40, true))
            ...
            case EVENT_CHECK_HOME: {
                if (!me->HasAura(SPELL_ENRAGE) &&
                    (me->GetPositionZ() < 24.0f ||
                     !me->GetHomePosition().IsInDist(me->GetPositionX(),
                                                     me->GetPositionY(),
                                                     me->GetPositionZ(),
                                                     110.0f))) {
...
private:
    ...
    ObjectGuid previousSwarmEgg_GUID; // This will prevent casting summoning
                                      // spells on same egg twice
        ...
        if (swarm) {
            ...
            if (previousSwarmEgg_GUID) {
                std::list<ObjectGuid>::iterator itr =
                    std::find(swarmEggs2.begin(),
                              swarmEggs2.end(),
                              previousSwarmEgg_GUID);
            ...
            previousSwarmEgg_GUID =
                Acore::Containers::SelectRandomContainerElement(swarmEggs2);

            if (Creature* egg =
                    ObjectAccessor::GetCreature(*me, previousSwarmEgg_GUID)) {
                egg->CastSpell(egg,
            ...
            ObjectGuid const& guardianEggGUID =
                Acore::Containers::SelectRandomContainerElement(guardianEggs);
            if (Creature* egg =
                    ObjectAccessor::GetCreature(*me, guardianEggGUID)) {
                egg->CastSpell(egg,
                               SPELL_SUMMON_SWARM_GUARD,
                               true,
                               nullptr,
...
struct npc_ahnkahar_nerubian : public ScriptedAI {
    ...
    void Reset() override
    {
        DoCastSelf(me->GetEntry() == NPC_AHNKAHAR_GUARDIAN ? SPELL_GUARDIAN_AURA
                                                           : SPELL_SWARMER_AURA,
                   true);
        uiSprintTimer = 10000;
    }
...
class spell_ahn_kahet_swarmer_aura : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (_targetCount) {
            if (Aura* aur = caster->GetAura(SPELL_SWARM)) {
                ...
                caster->CastCustomSpell(SPELL_SWARM,
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_ahn_kahet_swarmer_aura::CountTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ALLY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_ahn_kahet_swarmer_aura::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
public:
    achievement_respect_your_elders()
        : AchievementCriteriaScript("achievement_respect_your_elders")
    {
    }
