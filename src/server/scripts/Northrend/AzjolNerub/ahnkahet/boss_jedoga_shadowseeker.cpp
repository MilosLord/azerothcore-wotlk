...
const Position JedogaPosition[3] = {
    ...
    {373.48f,
     -706.00f,
     -16.18f} // POINT_RITUAL and POINT_INITIAL. This positions also is used for
              // visual trigger used for ritual
};
...
struct boss_jedoga_shadowseeker : public BossAI {
    boss_jedoga_shadowseeker(Creature* pCreature)
        : BossAI(pCreature, DATA_JEDOGA_SHADOWSEEKER), sayPreachTimer(120000),
          combatSummonsSummoned(false), ritualTriggered(false),
          volunteerWork(true)
    {
    }
    ...
    void Reset() override
    {
        ...
        me->GetMotionMaster()->MovePoint(
            POINT_INITIAL, JedogaPosition[0], false);
        ...
        if (!tempOOCSummons.empty()) {
            for (TempSummon* summon : tempOOCSummons) {
                if (summon) {
                    summon->ApplySpellImmune(
                        0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, false);
                    summon->ApplySpellImmune(
                        0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_MAGIC, false);
    ...
    void SummonedCreatureDies(Creature* summon, Unit* killer) override
    {
        ...
        case NPC_TWILIGHT_INITIATE: {
            GuidList::iterator itr = std::find(
                oocSummons.begin(), oocSummons.end(), summon->GetGUID());
            ...
            me->GetMotionMaster()->MovePoint(
                POINT_DOWN, JedogaPosition[1], false);
            ...
            if (!combatSummonsSummoned) {
                ...
                for (uint8 i = 0; i < MAX_COMBAT_INITIATES; ++i) {
                    if (TempSummon* summon =
                            me->SummonCreature(NPC_TWILIGHT_VOLUNTEER,
                                               VolunteerSpotPositions[i][0],
                                               TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                                               5000)) {
                        summon->GetMotionMaster()->MovePoint(
                            POINT_INITIAL, VolunteerSpotPositions[i][1]);
        ...
        case NPC_TWILIGHT_VOLUNTEER: {
            if (sacraficeTarget_GUID &&
                summon->GetGUID() != sacraficeTarget_GUID) {
                break;
            }
    ...
    {
        if (!ritualTriggered && me->HealthBelowPctDamaged(55, damage) &&
            ...
            events.ScheduleEvent(
                EVENT_JEDOGA_PREPARE_RITUAL, 1s, 0, PHASE_RITUAL);
    ...
    void DoAction(int32 action) override
    {
        if (action == ACTION_SACRAFICE) {
            if (Creature* target =
                    ObjectAccessor::GetCreature(*me, sacraficeTarget_GUID)) {
                Unit::Kill(me, target);
            }
        }
    }
        ...
        case POINT_UP: {
            ...
            if (!summons.empty()) {
                ...
                if (ObjectAccessor::GetCreature(*me, sacraficeTarget_GUID)) {
                    events.ScheduleEvent(
                        EVENT_JEDGA_START_RITUAL, 3s, 0, PHASE_RITUAL);
                }
                // Something failed, let players continue but do not grant
                // achievement
                else {
                    volunteerWork = false;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 const eventId = events.ExecuteEvent()) {
            ...
            case EVENT_JEDOGA_CYCLONE: {
                DoCastSelf(
                    DUNGEON_MODE(SPELL_CYCLONE_STRIKE, SPELL_CYCLONE_STRIKE_H),
                    false);
            ...
            case EVENT_JEDOGA_LIGHTNING_BOLT: {
                if (Unit* pTarget = SelectTarget(
                    ...
                    DoCast(pTarget,
                           DUNGEON_MODE(SPELL_LIGHTNING_BOLT,
            ...
            case EVENT_JEDOGA_THUNDERSHOCK: {
                if (Unit* pTarget = SelectTarget(
                    ...
                    DoCast(
                        pTarget,
                        DUNGEON_MODE(SPELL_THUNDERSHOCK, SPELL_THUNDERSHOCK_H),
                        false);
                }
            ...
            case EVENT_JEDOGA_PREPARE_RITUAL: {
                ...
                me->GetMotionMaster()->MovePoint(POINT_RITUAL,
                                                 JedogaPosition[1]);
                break;
            }
            case EVENT_JEDOGA_MOVE_UP: {
                ...
                me->GetMotionMaster()->MoveTakeoff(
                    POINT_UP, JedogaPosition[0], 7.0f);
                break;
            }
            ...
            case EVENT_JEDGA_START_RITUAL: {
                sacraficeTarget_GUID =
                    Acore::Containers::SelectRandomContainerElement(summons);
                if (Creature* volunteer = ObjectAccessor::GetCreature(
                        *me, sacraficeTarget_GUID)) {
...
private:
    ...
    void ReschedulleCombatEvents()
    {
        ...
        events.RescheduleEvent(
            EVENT_JEDOGA_LIGHTNING_BOLT, 7s, 0, PHASE_NORMAL);
...
struct npc_twilight_volunteer : public ScriptedAI {
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        if (id == POINT_INITIAL) {
            ...
            if (Creature* jedoga =
                    pInstance->GetCreature(DATA_JEDOGA_SHADOWSEEKER)) {
                jedoga->AI()->Talk(SAY_SACRIFICE_2);
                jedoga->CastSpell(
                    nullptr, SPELL_SACRIFICE_BEAM); /// @todo: Visual is not
                                                    /// working. (cosmetic)
    ...
    void UpdateAI(uint32 diff) override
    {
        if (!events.Empty()) {
            ...
            if (events.ExecuteEvent() == EVENT_RITUAL_BEGIN_MOVE) {
                ...
                me->SetWalk(true);
                me->GetMotionMaster()->MovePoint(
                    POINT_RITUAL, JedogaPosition[2], false);

                if (Creature* jedoga =
                        pInstance->GetCreature(DATA_JEDOGA_SHADOWSEEKER)) {
                    if (Creature* ritualTrigger =
                            jedoga->SummonCreature(NPC_JEDOGA_CONTROLLER,
                                                   JedogaPosition[2],
                                                   TEMPSUMMON_TIMED_DESPAWN,
                                                   15000)) {
                        ritualTrigger->CastSpell(ritualTrigger,
...
class spell_random_lightning_visual_effect : public SpellScript {
    ...
    void ModDestHeight(SpellDestination& dest)
    {
        Position const offset = {
            frand(-15.0f, 15.0f), frand(-15.0f, 15.0f), -19.0f, 0.0f};
    ...
    void Register() override
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_random_lightning_visual_effect::ModDestHeight,
            EFFECT_0,
...
public:
    achievement_volunteer_work()
        : AchievementCriteriaScript("achievement_volunteer_work")
    {
    }
