    ...
    struct instance_ahnkahet_InstanceScript : public InstanceScript {
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_TELDARAM_PLATFORM: {
                ...
                HandleGameObject(
                    ObjectGuid::Empty, IsAllSpheresActivated(), go);
                break;
            }
            case GO_TELDARAM_SPHERE1:
            case GO_TELDARAM_SPHERE2: {
                if (GetPersistentData(go->GetEntry() == GO_TELDARAM_SPHERE1
                                          ? 0
                                          : 1) == DONE ||
        ...
        void SetData(uint32 type, uint32 /*data*/) override
        {
            ...
            if ((type == DATA_TELDRAM_SPHERE1 ||
                 type == DATA_TELDRAM_SPHERE2) &&
                GetPersistentData(index) != DONE) {
                ...
                if (Creature* taldaram = GetCreature(DATA_PRINCE_TALDARAM)) {
                    if (taldaram->IsAlive()) {
                        ...
                        if (IsAllSpheresActivated()) {
                            HandleGameObject(
                                taldaramPlatform_GUID, true, nullptr);
    ...
    private:
        ...
        bool IsAllSpheresActivated() const
        {
            return GetBossState(DATA_PRINCE_TALDARAM) == DONE ||
                   (GetPersistentData(DATA_TELDRAM_SPHERE1) == DONE &&
                    GetPersistentData(DATA_TELDRAM_SPHERE2) == DONE);
        }
    };
...
class spell_shadow_sickle_periodic_damage : public AuraScript {
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_shadow_sickle_periodic_damage::HandlePeriodic,
            EFFECT_0,
