...
struct npc_taldaram_flamesphere : public NullCreatureAI {
    npc_taldaram_flamesphere(Creature* pCreature)
        : NullCreatureAI(pCreature), instance(pCreature->GetInstanceScript()),
          uiDespawnTimer(13000), moveTimer(0)
    {
    ...
    void UpdateAI(uint32 diff) override
    {
        if (moveTimer) {
            if (moveTimer <= diff) {
                ...
                float x =
                    me->GetPositionX() + DATA_SPHERE_DISTANCE * cos(angle);
                float y =
                    me->GetPositionY() + DATA_SPHERE_DISTANCE * std::sin(angle);
                me->GetMotionMaster()->MovePoint(
                    POINT_ORB, x, y, me->GetPositionZ());
    ...
    void SetVictimPos(Position const& pos) { victimPos.Relocate(pos); }
...
struct boss_taldaram : public BossAI {
    boss_taldaram(Creature* pCreature)
        : BossAI(pCreature, DATA_PRINCE_TALDARAM), vanishDamage(0)
    {
    }
    ...
    void InitializeAI() override
    {
        ...
        // Event not started
        if (instance->GetPersistentData(DATA_TELDRAM_SPHERE1) != DONE ||
            instance->GetPersistentData(DATA_TELDRAM_SPHERE2) != DONE) {
                     ...
                     SpellSchoolMask /*school*/) override
    {
        if (vanishTarget_GUID) {
            if (me->FindCurrentSpellBySpellId(SPELL_EMBRACE_OF_THE_VAMPYR)) {
                vanishDamage += damage;
                if (vanishDamage >=
                    DUNGEON_MODE<uint32>(MAX_EMBRACE_DMG, MAX_EMBRACE_DMG_H)) {
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        case NPC_FLAME_SPHERE_2:
        case NPC_FLAME_SPHERE_3: {
            if (npc_taldaram_flamesphere* summonAI =
                    dynamic_cast<npc_taldaram_flamesphere*>(summon->AI())) {
        ...
        while (uint32 const eventId = events.ExecuteEvent()) {
            ...
            case EVENT_PRINCE_FLAME_SPHERES: {
                ...
                if (!events.GetNextEventTime(EVENT_PRINCE_VANISH)) {
                    ...
                    // Make sure that Vanish won't get triggered at same time as
                    // sphere summon
            ...
            case EVENT_PRINCE_VANISH: {
                // Count alive players
                uint8                              count = 0;
                std::list<HostileReference*> const t_list =
                    me->GetThreatMgr().GetThreatList();
                if (!t_list.empty()) {
                    for (HostileReference const* reference : t_list) {
                        if (reference) {
                            Unit const* pTarget = ObjectAccessor::GetUnit(
                                *me, reference->getUnitGuid());
                            if (pTarget &&
                ...
                if (count > 2) {
                    ...
                    if (Unit* pEmbraceTarget = SelectTarget(
            ...
            case EVENT_PRINCE_VANISH_RUN: {
                if (Unit* _vanishTarget =
                        ObjectAccessor::GetUnit(*me, vanishTarget_GUID)) {
                    vanishDamage = 0;
                    DoCast(_vanishTarget, SPELL_SHADOWSTEP);
                    me->CastSpell(
                        _vanishTarget, SPELL_EMBRACE_OF_THE_VAMPYR, false);
...
class spell_prince_taldaram_conjure_flame_sphere : public SpellScript {
    ...
    {
        return ValidateSpellInfo({SPELL_FLAME_SPHERE_SUMMON_1,
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_prince_taldaram_conjure_flame_sphere::HandleScript,
            EFFECT_0,
            SPELL_EFFECT_DUMMY);
    }
};
...
class spell_prince_taldaram_flame_sphere_summon : public SpellScript {
    ...
    void Register() override
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_prince_taldaram_flame_sphere_summon::SetDest,
            EFFECT_0,
            TARGET_DEST_CASTER);
    }
};
