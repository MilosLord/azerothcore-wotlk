...
enum Spells {
    ...
    SPELL_BUILD_SIEGE_VEHICLE_FORCE_ALLIANCE =
        56662, // Which faction uses which ?
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->HasAura(SPELL_CORPORAL)) {
            AddGossipItemFor(
                player, 9923, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        }
                        ...
                        uint32 action) override
    {
        ...
        case 1:
            ...
            break;
        case 2:
            spellId = player->GetTeamId() == TEAM_ALLIANCE
                          ? SPELL_BUILD_SIEGE_VEHICLE_FORCE_ALLIANCE
...
private:
    bool canBuild(Creature* creature)
    {
        ...
        case NPC_GOBLIN_MECHANIC:
            return (wintergrasp->GetData(BATTLEFIELD_WG_DATA_MAX_VEHICLE_H) >
                    wintergrasp->GetData(BATTLEFIELD_WG_DATA_VEHICLE_H));
        case NPC_GNOMISH_ENGINEER:
            return (wintergrasp->GetData(BATTLEFIELD_WG_DATA_MAX_VEHICLE_A) >
                    wintergrasp->GetData(BATTLEFIELD_WG_DATA_VEHICLE_A));
        default:
            return false;
        }
    }
            ...
            if (graveyard[i]->GetControlTeamId() == player->GetTeamId())
                AddGossipItemFor(
                    ...
                    sObjectMgr->GetAcoreStringForDBCLocale(
                        ((BfGraveyardWG*)graveyard[i])->GetTextId()),
                    GOSSIP_SENDER_MAIN,
                        ...
                        uint32 action) override
    {
        ...
        if (wintergrasp) {
            ...
            for (uint8 i = 0; i < gy.size(); i++)
                if (action - GOSSIP_ACTION_INFO_DEF == i &&
                    gy[i]->GetControlTeamId() == player->GetTeamId())
                    if (GraveyardStruct const* safeLoc =
                            sGraveyard->GetGraveyard(gy[i]->GetGraveyardId()))
                        player->TeleportTo(safeLoc->Map,
                                           safeLoc->x,
                                           safeLoc->y,
                                           safeLoc->z,
                                           0);
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (wintergrasp->IsWarTime()) {
            ...
            player->SendUpdateWorldState(
                4354, GameTime::GetGameTime().count() + timer);
            if (timer < 15 * MINUTE) {
                AddGossipItemFor(player,
                                 WG_GOSSIP_MENU_QUEUE,
                                 0,
            ...
            case EVENT_MAGISTER_SURDIEL_YELL:
                if (Battlefield* wintergrasp =
                        sBattlefieldMgr->GetBattlefieldByBattleId(
                            BATTLEFIELD_BATTLEID_WG)) {
                    uint32 timer = wintergrasp->GetTimer() / 1000;
                    if (!wintergrasp->IsWarTime() && timer < 5 * MINUTE &&
                        timer > 4 * MINUTE) {
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (creature->IsQuestGiver()) {
            QuestRelationBounds objectQR =
                sObjectMgr->GetCreatureQuestRelationBounds(
                    creature->GetEntry());
            ...
            for (QuestRelations::const_iterator i = objectQR.first;
                ...
                case QUEST_BONES_AND_ARROWS_ALLIANCE_ATT:
                    if (!sPoolMgr->IsSpawnedObject<Quest>(
                            QUEST_BONES_AND_ARROWS_ALLIANCE_DEF))
                        continue;
                    questRelationVector.push_back(
            ...
            for (std::vector<uint32>::const_iterator i =
                     questRelationVector.begin();
                 i != questRelationVector.end();
                 ++i) {
                uint32       questId = *i;
                Quest const* quest   = sObjectMgr->GetQuestTemplate(questId);
                switch (questId) {
    ...
    uint32 GetDialogStatus(Player* player, Creature* creature) override
    {
        ...
        for (QuestRelations::const_iterator i = qir.first; i != qir.second;
             ++i) {
...
public:
    ...
    struct npc_wg_siege_machineAI : public VehicleAI {
        ...
        bool CanControlVehicle(Unit* passenger)
        {
            ...
            if (me->GetEntry() == NPC_WINTERGRASP_CATAPULT &&
                passenger->HasAura(SPELL_CORPORAL))
                return true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (checkTimer >= 1000) {
                checkTimer = 0;
                if (me->GetVehicleKit())
                    for (SeatMap::iterator itr =
                             me->GetVehicleKit()->Seats.begin();
                         itr != me->GetVehicleKit()->Seats.end();
                         ++itr)
                        if (const VehicleSeatEntry* seatInfo =
                                itr->second.SeatInfo)
                            if (seatInfo->m_flags &
...
public:
    ...
    struct go_wg_vehicle_teleporterAI : public GameObjectAI {
        go_wg_vehicle_teleporterAI(GameObject* gameObject)
            : GameObjectAI(gameObject), _checkTimer(0)
        {
        }

        bool IsFriendly(Unit* passenger)
        {
            return ((me->GetUInt32Value(GAMEOBJECT_FACTION) ==
                         WintergraspFaction[TEAM_HORDE] &&
                     passenger->getRaceMask() & RACEMASK_HORDE) ||
                    (me->GetUInt32Value(GAMEOBJECT_FACTION) ==
                         WintergraspFaction[TEAM_ALLIANCE] &&
                     passenger->getRaceMask() & RACEMASK_ALLIANCE));
        }
                ...
                if (Vehicle* vehicle = cVeh->GetVehicleKit())
                    if (Unit* passenger = vehicle->GetPassenger(0))
                        if (IsFriendly(passenger))
                            if (Creature* teleportTrigger =
                                    passenger->SummonTrigger(
                                        me->GetPositionX() - 60.0f,
                                        me->GetPositionY(),
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_checkTimer >= 1000) {
                for (uint8 i = 0; i < MAX_WINTERGRASP_VEHICLES; i++)
                    if (Creature* vehicleCreature = me->FindNearestCreature(
                            vehiclesList[i], 3.0f, true))
                        if (Creature* teleportTrigger =
                                IsValidVehicle(vehicleCreature))
                            teleportTrigger->CastSpell(
                                vehicleCreature, SPELL_VEHICLE_TELEPORT, true);
...
public:
    spell_wintergrasp_force_building()
        : SpellScriptLoader("spell_wintergrasp_force_building")
    {
    }
    ...
    class spell_wintergrasp_force_building_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_wintergrasp_force_building_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_wintergrasp_create_vehicle()
        : SpellScriptLoader("spell_wintergrasp_create_vehicle")
    {
    }
    ...
    class spell_wintergrasp_create_vehicle_SpellScript : public SpellScript {
            ...
            if (Unit* caster = GetCaster()) {
                ...
                SummonPropertiesEntry const* properties =
                    sSummonPropertiesStore.LookupEntry(
                        GetSpellInfo()->Effects[effIndex].MiscValueB);
                if (!properties) {
                    return;
                }
        ...
        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_wintergrasp_create_vehicle_SpellScript::HandleSummon,
                EFFECT_1,
                SPELL_EFFECT_SUMMON);
        }
    };
...
public:
    ...
    class spell_wintergrasp_rp_gg_SpellScript : public SpellScript {
        ...
        void HandleFinish()
        {
            ...
            GetCaster()->CastSpell(GetExplTargetDest()->GetPositionX(),
                                   GetExplTargetDest()->GetPositionY(),
                                   GetExplTargetDest()->GetPositionZ(),
        ...
        {
            AfterCast +=
                SpellCastFn(spell_wintergrasp_rp_gg_SpellScript::HandleFinish);
        }
    };
    ...
    class spell_wintergrasp_portal_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            if (!wintergrasp || !target || target->GetLevel() < 75 ||
                (wintergrasp->GetDefenderTeam() != target->GetTeamId()))
                return;
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_wintergrasp_portal_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_wintergrasp_hide_small_elementals()
        : SpellScriptLoader("spell_wintergrasp_hide_small_elementals")
    {
    }

    class spell_wintergrasp_hide_small_elementals_AuraScript
        : public AuraScript {
...
public:
    spell_wg_reduce_damage_by_distance()
        : SpellScriptLoader("spell_wg_reduce_damage_by_distance")
    {
    }
    ...
    class spell_wg_reduce_damage_by_distance_SpellScript : public SpellScript {
        ...
        void RecalculateDamage()
        {
            ...
            float maxDistance = GetSpellInfo()->Effects[EFFECT_0].CalcRadius(
                GetCaster()); // Xinef: always stored in EFFECT_0
            float distance = std::min<float>(
                GetHitUnit()->GetDistance(*GetExplTargetDest()), maxDistance);

            int32 damage = std::max<int32>(
                0,
                int32(GetHitDamage() -
                      floor(GetHitDamage() * (distance / maxDistance))));
            SetHitDamage(damage);
        }

        void Register() override
        {
            OnHit += SpellHitFn(spell_wg_reduce_damage_by_distance_SpellScript::
                                    RecalculateDamage);
        }
    };
...
public:
    achievement_wg_didnt_stand_a_chance()
        : AchievementCriteriaScript("achievement_wg_didnt_stand_a_chance")
    {
    }
...
public:
    achievement_wg_vehicular_gnomeslaughter()
    ...
    bool OnCheck(Player* source, Unit* target, uint32 /*criteria_id*/) override
    {
        ...
        if (Unit* vehicle = source->GetVehicleBase())
            if (vehicle->GetEntry() == NPC_WINTERGRASP_SIEGE_ENGINE_ALLIANCE ||
                vehicle->GetEntry() == NPC_WINTERGRASP_SIEGE_ENGINE_HORDE ||
                vehicle->GetEntry() == NPC_WINTERGRASP_CATAPULT ||
                vehicle->GetEntry() == NPC_WINTERGRASP_DEMOLISHER ||
...
public:
    achievement_wg_within_our_grasp()
        : AchievementCriteriaScript("achievement_wg_within_our_grasp")
    {
    }

    bool OnCheck(Player* /*source*/,
                 Unit* /*target*/,
