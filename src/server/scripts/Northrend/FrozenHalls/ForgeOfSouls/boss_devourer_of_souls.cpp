...
enum eSpells {
    ...
    SPELL_WAILING_SOULS =
        68899, // target 1.0,1.0, change model, triggers 68871, cast time 3 secs
    SPELL_WAILING_SOULS_SCRIPT_EFFECT =
        68871, // target 1.0, script effect, instant
    SPELL_WAILING_SOULS_PERIODIC_DUMMY = 68875, // target 1.0, aura 226, instant
    SPELL_WAILING_SOULS_PERIODIC_DUMMY_2 =
        68876,                             // target 1.0, aura 226, instant
    SPELL_WAILING_SOULS_TARGETING = 68912, // target 22.15, aura dummy, 50000yd,
                                           // cast instant, duration 4 secs
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                ...
                if ((player->GetTeamId() == TEAM_ALLIANCE &&
                     player->GetQuestStatus(QUEST_TEMPERING_THE_BLADE_A) ==
                         QUEST_STATUS_INCOMPLETE) ||
                    (player->GetTeamId() == TEAM_HORDE &&
                     player->GetQuestStatus(QUEST_TEMPERING_THE_BLADE_H) ==
                         QUEST_STATUS_INCOMPLETE)) {
                    if (!me->FindNearestCreature(NPC_CRUCIBLE_OF_SOULS, 100.0f))
                        me->SummonCreature(NPC_CRUCIBLE_OF_SOULS,
                                           5672.29f,
                                           2520.69f,
                                           713.44f,
                                           0.96f);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (Spell* s = me->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
                if (s->m_spellInfo->Id == SPELL_MIRRORED_SOUL) {
                    ...
                    case EVENT_SPELL_PHANTOM_BLAST:
                        me->CastSpell(
                            me->GetVictim(), SPELL_PHANTOM_BLAST, false);
                        events.Repeat(5s);
                        break;
                    default:
                        events.Repeat(1s);
            ...
            case EVENT_SPELL_WAILING_SOULS:
                ...
                if (Unit* target =
                    ...
                    me->CastCustomSpell(SPELL_WAILING_SOULS_TARGETING,
...
public:
    spell_wailing_souls_periodic()
        : SpellScriptLoader("spell_wailing_souls_periodic")
    {
    }
    ...
    class spell_wailing_souls_periodic_AuraScript : public AuraScript {
        ...
        void HandlePeriodicTick(AuraEffect const* aurEff)
        {
            ...
            if (Unit* t = GetTarget()) {
                if (aurEff->GetTickNumber() < 30) {
                    ...
                    if (t->GetVictim()) {
                        t->SetGuidValue(UNIT_FIELD_TARGET,
                                        t->GetVictim()->GetGUID());
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_wailing_souls_periodic_AuraScript::HandlePeriodicTick,
                EFFECT_0,
