...
BossBoundaryData const boundaries = {
    ...
    {DATA_DEVOURER,
     new ParallelogramBoundary(Position(5663.56f, 2570.53f),
                               Position(5724.39f, 2520.45f),
                               Position(5570.36f, 2461.42f))}};

class instance_forge_of_souls : public InstanceMapScript {
public:
    instance_forge_of_souls()
        : InstanceMapScript("instance_forge_of_souls", 632)
    {
    }
    ...
    struct instance_forge_of_souls_InstanceScript : public InstanceScript {
        ...
        {
            // this will happen only after crash and loading the instance from
            // db
            if (m_auiEncounter[0] == DONE && m_auiEncounter[1] == DONE &&
                (!NPC_LeaderSecondGUID ||
                 !instance->GetCreature(NPC_LeaderSecondGUID))) {
        ...
        void HandleOutro()
        {
            if (!NPC_LeaderSecondGUID ||
                !instance->GetCreature(NPC_LeaderSecondGUID))
                if (Creature* leader = instance->SummonCreature(
                        NPC_SYLVANAS_PART2, outroSpawnPoint))
                    if (Creature* boss =
                            instance->GetCreature(NPC_DevourerGUID)) {
                        float angle = boss->GetAngle(leader);
                        leader->GetMotionMaster()->MovePoint(
                            1,
            ...
            for (int8 i = 0; outroPositions[i].entry[teamIdInInstance] != 0;
                 ++i)
                if (Creature* summon = instance->SummonCreature(
                        outroPositions[i].entry[teamIdInInstance],
                        outroPositions[i].startPosition))
                    summon->GetMotionMaster()->MovePath(
                        outroPositions[i].pathId, false);
        }
            ...
            case 12752: // Soul Power
                if (Creature* c = instance->GetCreature(NPC_BronjahmGUID)) {
                    ...
                    c->GetCreaturesWithEntryInRange(
                        L,
                        200.0f,
                        36535); // find all Corrupted Soul Fragment (36535)
...
void AddSC_instance_forge_of_souls() { new instance_forge_of_souls(); }
