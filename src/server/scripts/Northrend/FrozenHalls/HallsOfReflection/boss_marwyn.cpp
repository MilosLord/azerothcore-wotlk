...
public:
    spell_hor_shared_suffering()
        : SpellScriptLoader("spell_hor_shared_suffering")
    {
    }
    ...
    class spell_hor_shared_sufferingAuraScript : public AuraScript {
                ...
                if (Unit* caster = GetCaster())
                    if (Map* map = caster->FindMap())
                        if (Aura* a = aurEff->GetBase()) {
                            ...
                            uint32 ticks = 0;
                            uint32 dmgPerTick =
                                a->GetSpellInfo()->Effects[0].BasePoints;
                            ...
                            ticks =
                                (a->GetDuration() /
                                 int32(
                                     a->GetSpellInfo()->Effects[0].Amplitude)) +
                                1;
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_hor_shared_sufferingAuraScript::OnRemove,
                EFFECT_0,
