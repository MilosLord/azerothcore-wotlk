...
Position const NpcJainaOrSylvanasEscapeRoute[] = {
    {5601.217285f,
     2207.652832f,
     731.541931f,
     5.223304f}, // leave the throne room
    ...
    {5560.281738f,
     2104.025635f,
     731.410889f,
     4.058383f}, // attack the first icewall
    {5510.990723f,
     2000.772217f,
     734.716064f,
     3.973213f}, // attack the second icewall
    {5452.641113f,
     1905.762329f,
     746.530579f,
     4.118834f}, // attack the third icewall
    {5338.126953f,
     1768.429810f,
     767.237244f,
     3.855189f},                             // attack the fourth icewall
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (canStart) {
            QuestStatus status = player->GetQuestStatus(
                creature->GetEntry() == NPC_SYLVANAS_PART1
            ...
            // once last quest is completed, she offers this shortcut of the
            // starting event
            status = player->GetQuestStatus(
                creature->GetEntry() == NPC_SYLVANAS_PART1
    ...
    struct npc_hor_leaderAI : public NullCreatureAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_START_INTRO:
                ...
                if (Creature* pLoralen = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(NPC_DARK_RANGER_LORALEN))) {
                    pLoralen->LoadEquipment(true);
                    pLoralen->SetVisible(true);
                    pLoralen->SetWalk(true);
                    pLoralen->GetMotionMaster()->MovePoint(
                        0, LoralenMidleFollowPos);
                }
                // Begining of intro is differents between factions as the
                // speech sequence and timers are differents.
                if (me->GetEntry() == NPC_JAINA_PART1) {
            ...
            case EVENT_INTRO_A2_3:
                pInstance->HandleGameObject(
                    pInstance->GetGuidData(GO_FROSTMOURNE), true);
                me->CastSpell(me, SPELL_FROSTMOURNE_SPAWN_SOUND, true);
                // me->CastSpell(me, SPELL_ARCANE_CAST_VISUAL, false);// the
                // sniff is missing from the alliance
                events.ScheduleEvent(EVENT_INTRO_A2_4, 10s);
                break;
            case EVENT_INTRO_A2_4:
                if (Creature* pUther = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(NPC_UTHER))) {
                    pUther->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                           EMOTE_ONESHOT_NONE);
                    pUther->SetVisible(true);
                    if (Aura* a =
                            pUther->AddAura(SPELL_SHADOWMOURNE_VISUAL, pUther))
                        a->SetDuration(8000);
                }
            ...
            case EVENT_INTRO_H2_4:
                if (Creature* pUther = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(NPC_UTHER))) {
                    pUther->SetVisible(true);
            ...
            case EVENT_INTRO_LK_1:
                if (Creature* pLichKing = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(NPC_LICH_KING_EVENT))) {

                    pInstance->HandleGameObject(
                        pInstance->GetGuidData(GO_ARTHAS_DOOR), true);
                    pLichKing->SetVisible(true);

                    pLichKing->GetMotionMaster()->MovePoint(
                        0, LichKingMoveMidlelThronePos, false);
                }
            ...
            case EVENT_INTRO_LK_1_1:
                if (Creature* pLichKing = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(NPC_LICH_KING_EVENT))) {
            ...
            case EVENT_INTRO_LK_1_3:
                pInstance->HandleGameObject(
                    pInstance->GetGuidData(GO_ARTHAS_DOOR), false);
                break;
            ...
            case EVENT_INTRO_LK_4_2:
                if (Creature* pLichKing = ObjectAccessor::GetCreature(
                    ...
                    pLichKing->CastSpell(
                        pLichKing, SPELL_FROSTMOURNE_EQUIP, false);
                    pInstance->HandleGameObject(
                        pInstance->GetGuidData(GO_FROSTMOURNE), false);
            ...
            case EVENT_INTRO_LK_4_3:
                if (GameObject* go = pInstance->instance->GetGameObject(
                        pInstance->GetGuidData(GO_FROSTMOURNE)))
                    go->SetPhaseMask(2, true);
                break;
            case EVENT_INTRO_LK_5:
                if (Creature* pFalric = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(DATA_FALRIC))) {
                    pFalric->UpdatePosition(
                        5274.9f, 2039.2f, 709.319f, 5.4619f, true);
                    pFalric->StopMovingOnCurrentPos();
                    pFalric->SetVisible(true);
                    if (pFalric->IsAlive()) {
                        pFalric->GetMotionMaster()->MovePoint(0, FalricMovePos);
                        if (Aura* a = pFalric->AddAura(
                                SPELL_SHADOWMOURNE_VISUAL, pFalric))
                            a->SetDuration(8000);
                    }
                }
                if (Creature* pMarwyn = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(DATA_MARWYN))) {
                    pMarwyn->UpdatePosition(
                        5343.77f, 1973.86f, 709.319f, 2.35173f, true);
                    pMarwyn->StopMovingOnCurrentPos();
                    pMarwyn->SetVisible(true);
                    if (pMarwyn->IsAlive()) {
                        pMarwyn->GetMotionMaster()->MovePoint(0, MarwynMovePos);
                        if (Aura* a = pMarwyn->AddAura(
                                SPELL_SHADOWMOURNE_VISUAL, pMarwyn))
                            a->SetDuration(8000);
                    }
                }
            ...
            case EVENT_INTRO_LK_5_3:
                ...
                if (Creature* pLoralen = pInstance->instance->GetCreature(
                    ...
                    pLoralen->SetUInt32Value(UNIT_NPC_EMOTESTATE,
            ...
            case EVENT_INTRO_END:
                pInstance->HandleGameObject(
                    pInstance->GetGuidData(GO_ARTHAS_DOOR), false);
                pInstance->HandleGameObject(
                    pInstance->GetGuidData(GO_FRONT_DOOR), false);
                events.ScheduleEvent(EVENT_INTRO_END_SET, 3s);
                break;
            case EVENT_INTRO_END_SET:
                if (Creature* pLoralen = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(NPC_DARK_RANGER_LORALEN))) {
                    pLoralen->UpdatePosition(
                        5369.71289f, 2083.6330f, 707.695129f, 0.188739f, true);
        ...
        void DoAction(int32 a) override
        {
            if (a == 1) {
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxDistance, 0, 0.0f, true))
                    AttackStart(target);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_DARK_MENDING:
                if (Unit* target = DoSelectLowestHpFriendly(
                        35.0f, DUNGEON_MODE(20000, 35000))) {
    ...
    struct npc_phantom_mageAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_HALLUCINATION:
                ...
                me->SummonCreature(NPC_PHANTOM_HALLUCINATION,
                                   *me,
...
public:
    ...
    struct npc_phantom_hallucinationAI
        : public npc_phantom_mage::npc_phantom_mageAI {
        npc_phantom_hallucinationAI(Creature* c)
            : npc_phantom_mage::npc_phantom_mageAI(c)
        {
            numOfUpd = 2;
        }
    ...
    struct npc_shadowy_mercenaryAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SHADOW_STEP:
                if (Unit* target =
                        SelectTargetFromPlayerList(100.0f, 0, true)) {
                    DoResetThreatList();
    ...
    struct boss_frostsworn_generalAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ACTIVATE_REFLECTIONS:
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_REFLECTIONS_DUMMY, false);
...
public:
    npc_hor_spiritual_reflection()
        : CreatureScript("npc_hor_spiritual_reflection")
    {
    }

    struct npc_hor_spiritual_reflectionAI : public ScriptedAI {
        npc_hor_spiritual_reflectionAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        ...
        if (inst->GetData(DATA_FROSTSWORN_GENERAL) &&
            !inst->GetData(DATA_LK_INTRO))
...
public:
    ...
    struct npc_hor_lich_kingAI : public NullCreatureAI {
        ...
        void DoAction(int32 a) override
        {
            ...
            else if ((a == ACTION_INFORM_TRASH_DIED && reqKillCount) ||
                     a == ACTION_CHECK_TRASH_DIED) {
                if ((a == ACTION_CHECK_TRASH_DIED && reqKillCount == 0) ||
                    (a == ACTION_INFORM_TRASH_DIED && (--reqKillCount) == 0)) {
                    events.CancelEvent(
                        EVENT_DECREASE_REQ_COUNT_BY_100); // just in case, magic
                                                          // happens sometimes
                    ++currentWall;
                    pInstance->SetData(ACTION_DELETE_ICE_WALL, 1);
                    if (currentWall <= 3) {
                        events.ScheduleEvent(EVENT_LK_SUMMON_NEXT_ICE_WALL, 1s);
                        events.ScheduleEvent(EVENT_LK_SUMMON,
                                             currentWall == 3 ? 11s : 7500ms);
                    }
                    else
        ...
        void MovementInform(uint32 type, uint32 /*id*/) override
        {
            ...
            // Xinef: if type is escort and spline is finalized, it means that
            // we reached last point from the path
            if (type == ESCORT_MOTION_TYPE && me->movespline->Finalized()) {
                if (currentWall == 0) {
                    Talk(SAY_LK_IW_1);
                    me->SetOrientation(4.15f);
                    if (Creature* icewall = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(NPC_ICE_WALL_TARGET))) {
        ...
        void JustSummoned(Creature* s) override
        {
            ...
            s->GetMotionMaster()->MovePoint(
                0, PathWaypoints[WP_STOP[currentWall + 1]]);
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if (target && target->IsAlive() &&
                spell->Id == SPELL_LICH_KING_ZAP_PLAYER)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_LK_CHECK_COMBAT:
                ...
                {
                    if (Creature* leader = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(NPC_SYLVANAS_PART2)))
                        if (leader->IsAlive() &&
                            leader->GetPositionX() < 5575.0f &&
                            me->GetExactDist2d(leader) <= 12.5f &&
                            !leader->HasAura(SPELL_HARVEST_SOUL) &&
                            me->HasAura(SPELL_REMORSELESS_WINTER)) {
                    ...
                    if (pInstance->instance->HavePlayers()) {
                        ...
                        if (div2 > 1) {
                            div2 = 0;
                            if (me->HasAura(
                                    SPELL_REMORSELESS_WINTER)) // prevent going
                                                               // behind him
                                                               // during fight
                            {
                                Map::PlayerList const& pl =
                                    pInstance->instance->GetPlayers();
            ...
            case EVENT_LK_KILL_LEADER:
                if (Creature* leader = pInstance->instance->GetCreature(
                    ...
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_FURY_OF_FROSTMOURNE, false);
                }
                break;
            case EVENT_LK_START_FOLLOWING: {
                ...
                for (uint8 i = 0; i <= 2; ++i)
                    path.push_back(
                        G3D::Vector3(PathWaypoints[i].GetPositionX(),
                                     PathWaypoints[i].GetPositionY(),
                                     PathWaypoints[i].GetPositionZ()));
            ...
            case EVENT_LK_SUMMON:
                ...
                if (currentWall <= 3) {
                    reqKillCount = 100;
                    events.RescheduleEvent(EVENT_DECREASE_REQ_COUNT_BY_100,
                                           10s);
                }
                break;
            ...
            case EVENT_LK_SUMMON_GHOULS:
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_RAGING_GHOULS, false);
                break;
            case EVENT_LK_SUMMON_RWD:
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_RISEN_WITCH_DOCTOR, false);
                break;
            case EVENT_LK_SUMMON_LA:
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_LUMBERING_ABOMINATION, false);
                break;
            case EVENT_LK_SUMMON_NEXT_ICE_WALL:
                Talk(SAY_LK_IW_1 + currentWall);
                if (Creature* c =
                        pInstance->instance->GetCreature(pInstance->GetGuidData(
                            NPC_ICE_WALL_TARGET + currentWall)))
    ...
    struct npc_hor_leader_secondAI : public NullCreatureAI {
...

        {
            ...
            case ACTION_START_INTRO:
                ...
                events.ScheduleEvent(me->GetEntry() == NPC_JAINA_PART2
                                         ? EVENT_JAINA_IMMOBILIZE_LK
                                         : EVENT_SYLVANAS_IMMOBILIZE_JUMP,
                                     9s);
                break;
        ...
        void MoveToNextStopPoint()
        {
            ...
            for (uint8 i = WP_STOP[currentStopPoint - 1] +
                           (currentStopPoint == 1 ? 0 : 1);
                 i <= WP_STOP[currentStopPoint];
                 ++i)
                path.push_back(G3D::Vector3(PathWaypoints[i].GetPositionX(),
                                            PathWaypoints[i].GetPositionY(),
                                            PathWaypoints[i].GetPositionZ()));
            me->GetMotionMaster()->MoveSplinePath(&path);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_LK_SAY_AGGRO:
                if (Creature* lkboss = pInstance->instance->GetCreature(
                    ...
                    me->Attack(lkboss, true),
                        lkboss->AI()->Talk(me->GetEntry() == NPC_JAINA_PART2
                                               ? SAY_LK_AGGRO_ALLY
                                               : SAY_LK_AGGRO_HORDE);
                }
                break;
            ...
            case EVENT_LK_BATTLE_2:
                ...
                if (Creature* lkboss = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(NPC_LICH_KING_BOSS))) {
                    lkboss->SetUInt32Value(UNIT_NPC_EMOTESTATE,
            ...
            case EVENT_SYLVANAS_IMMOBILIZE_JUMP:
                if (Creature* lkboss = pInstance->instance->GetCreature(
                    ...
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_NONE);
                    me->KnockbackFrom(lkboss->GetPositionX(),
                                      lkboss->GetPositionY(),
                                      34.3f,
                                      4.0f);
                    events.ScheduleEvent(EVENT_SYLVANAS_DARK_BINDING,
                                         2s + 500ms);
                }
                break;
            ...
            case EVENT_SAY_LEAVE: {
                ...
                Talk(me->GetEntry() == NPC_JAINA_PART2 ? SAY_JAINA_AGGRO
                                                       : SAY_SYLVANA_AGGRO);
                me->SetSheath(SHEATH_STATE_MELEE);
                me->SetWalk(false);
                me->SetSpeed(MOVE_RUN, me->GetCreatureTemplate()->speed_run);
            ...
            case EVENT_ADD_GOSSIP:
                me->RemoveAura(me->GetEntry() == NPC_JAINA_PART2
            ...
            case EVENT_START_RUN: {
                ...
                if (Creature* lkboss = pInstance->instance->GetCreature(
                    ...
                    lkboss->RemoveAura(me->GetEntry() == NPC_JAINA_PART2
            ...
            case EVENT_SAY_LEADER_STOP_TEXT: {
                ...
                case 5:
                    textId =
                        (me->GetEntry() == NPC_JAINA_PART2 ? SAY_JAINA_TRAP
                                                           : SAY_SYLVANA_TRAP);
                    break;
                }
                Talk(textId);
                if (currentStopPoint <= 4)
                    ...
                    me->SetFacingTo(
                        PathWaypoints[PATH_WP_COUNT - 1].GetOrientation());
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                       EMOTE_ONESHOT_READY1H);
                }
            } break;
            case EVENT_SAY_OPENING:
                Talk(me->GetEntry() == NPC_JAINA_PART2 ? SAY_JAINA_ESCAPE_01
...
public:
    ...
    struct npc_hor_raging_ghoulAI : public ScriptedAI {
        ...
        void Reset() override { leaped = false; }
...
public:
    npc_hor_lumbering_abomination()
        : CreatureScript("npc_hor_lumbering_abomination")
    {
    }

    struct npc_hor_lumbering_abominationAI : public ScriptedAI {
        npc_hor_lumbering_abominationAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetHallsOfReflectionAI<npc_hor_lumbering_abominationAI>(
            creature);
    }
};
...
public:
    spell_hor_gunship_cannon_fire()
        : SpellScriptLoader("spell_hor_gunship_cannon_fire")
    {
    }
    ...
    class spell_hor_gunship_cannon_fireAuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Unit* caster = GetCaster())
                if (Creature* c = caster->SummonCreature(
                        WORLD_TRIGGER,
                        CannonFirePos[caster->GetEntry() == NPC_JAINA_PART2 ? 0
                                                                            : 1]
                                     [urand(0, 2)],
                        TEMPSUMMON_TIMED_DESPAWN,
                        1)) {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_hor_gunship_cannon_fireAuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
public:
    at_hor_battered_hilt_start()
        : AreaTriggerScript("at_hor_battered_hilt_start")
    {
    }
...
public:
    at_hor_battered_hilt_throw()
        : AreaTriggerScript("at_hor_battered_hilt_throw")
    {
    }
