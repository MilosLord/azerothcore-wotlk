...
public:
    ...
    struct npc_pos_leaderAI : public NullCreatureAI {
        npc_pos_leaderAI(Creature* creature)
        ...
        {
            if (type == DATA_START_INTRO &&
                pInstance->GetData(DATA_INSTANCE_PROGRESS) ==
                    INSTANCE_PROGRESS_NONE &&
                counter == 0 && !me->IsVisible()) {
                me->setActive(true);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 4:
                        ...
                        for (SummonList::iterator itr = summons.begin();
                            ...
                            if (Creature* c =
                                ...
                                if (c->GetPositionY() > 215.0f)
                                    c->GetMotionMaster()->MoveChase(
                                        n2, 0.0f, rand_norm() * 2 * M_PI);
                                else
                                    c->GetMotionMaster()->MoveChase(
                                        n1, 0.0f, rand_norm() * 2 * M_PI);
                            }
                    }
                }
                ...
                break;
            case 5:
                Talk(me->GetEntry() == NPC_JAINA_PART1 ? SAY_JAINA_INTRO_1
            ...
            case 6:
                ...
                break;
            case 7: /// @todo: (Initial RP, when zoning in the instance) is not
                    /// complete.
                if (pInstance) {
                    if (Creature* n1 = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(DATA_NECROLYTE_1_GUID)))
                        n1->AI()->DoAction(1337); // remove invincibility
                    if (Creature* n2 = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(DATA_NECROLYTE_2_GUID)))
                    ...
                    for (SummonList::iterator itr = summons.begin();
                        ...
                        if (Creature* c =
                            ...
                            c->GetMotionMaster()->MoveTakeoff(
                                0,
                                c->GetPositionX() + dist * cos(angle),
                                c->GetPositionY() + dist * std::sin(angle),
                                c->GetPositionZ() + 6.0f + (float)urand(0, 4),
                                1.5f + frand(0.0f, 1.5f));
                        }
                }
            ...
            case 9:
                ...
                break;
            case 10:
                Talk(me->GetEntry() == NPC_JAINA_PART1 ? SAY_JAINA_INTRO_2
                ...
                if (pInstance) {
                    ...
                    for (SummonList::iterator itr = summons.begin();
                        ...
                        if (Creature* c =
                            ...
                            if (c->GetPositionX() < 450.0f)
                                continue;
                            c->SetHomePosition(c->GetPositionX(),
                                               c->GetPositionY(),
                                               c->GetPositionZ(),
                                               c->GetOrientation());
                            c->Respawn(true);
                ...
                if (pInstance) {
                    for (SummonList::iterator itr = summons.begin();
                        ...
                        if (Creature* c =
                            ...
                            c->SetHomePosition(
                                me->GetPositionX() + dist * cos(angle),
                                me->GetPositionY() + dist * std::sin(angle),
                                me->GetPositionZ(),
                                0.0f);
                        }
                }
            ...
            case 14:
                if (pInstance) {
                    if (me->GetEntry() == NPC_JAINA_PART1) {
                        ...
                        for (uint8 i = 0; i < 2; ++i)
                            if (Creature* c = pInstance->instance->GetCreature(
                                    pInstance->GetGuidData(DATA_GUARD_1_GUID +
                                                           i)))
            ...
            case 15:
                ...
                break;
            case 16:
                Talk(me->GetEntry() == NPC_JAINA_PART1 ? SAY_JAINA_INTRO_4
                                                       : SAY_SYLVANAS_INTRO_3);
                if (pInstance) {
                    for (uint8 i = 0; i < 2; ++i)
                        if (Creature* c = pInstance->instance->GetCreature(
                                pInstance->GetGuidData(DATA_GUARD_1_GUID + i)))
                    ...
                    for (SummonList::iterator itr = summons.begin();
                        ...
                        if (Creature* c =
                                pInstance->instance->GetCreature(*itr)) {
                            if (c->IsAlive()) {
                                if (c->GetEntry() == NPC_KALIRA ||
                                    c->GetEntry() == NPC_ELANDRA ||
                                    c->GetEntry() == NPC_LORALEN ||
                                    c->GetEntry() == NPC_KORELN)
                                    continue;
                    ...
                    pInstance->SetData(DATA_INSTANCE_PROGRESS,
            ...
            case 17:
                me->setActive(false);
                Talk(me->GetEntry() == NPC_JAINA_PART1 ? SAY_JAINA_INTRO_5
...
public:
    npc_pos_deathwhisper_necrolyte()
        : CreatureScript("npc_pos_deathwhisper_necrolyte")
    {
    }

    struct npc_pos_deathwhisper_necrolyteAI : public ScriptedAI {
        npc_pos_deathwhisper_necrolyteAI(Creature* creature)
            : ScriptedAI(creature)
        {
            pInstance    = me->GetInstanceScript();
            isInvincible = false;
        ...
        {
            if (pInstance && pInstance->GetData(DATA_INSTANCE_PROGRESS) ==
                                 INSTANCE_PROGRESS_NONE) {
                if ((me->GetPositionX() > 490.0f &&
                     me->GetPositionX() < 504.0f &&
                     me->GetPositionY() > 192.0f &&
                     me->GetPositionY() < 206.0f) ||
                    (me->GetPositionX() > 490.0f &&
                     me->GetPositionX() < 504.0f &&
                     me->GetPositionY() > 240.0f &&
                     me->GetPositionY() < 254.0f)) {
                    isInvincible = true;
                    ...
                    if (me->GetPositionY() < 206.0f) {
                        pInstance->SetGuidData(DATA_NECROLYTE_1_GUID,
                                               me->GetGUID());
                    }
                    else {
                        pInstance->SetGuidData(DATA_NECROLYTE_2_GUID,
                                               me->GetGUID());
                    }
                }
            }
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            if (Creature* c = pInstance->instance->GetCreature(
                    pInstance->GetGuidData(DATA_LEADER_FIRST_GUID)))
                c->AI()->SetData(DATA_START_INTRO, 0);
        }

        void DamageTaken(Unit* /*doneBy*/,
                         uint32& damage,
                         DamageEffectType,
...
public:
    ...
    struct npc_pos_after_first_bossAI : public NullCreatureAI {
        npc_pos_after_first_bossAI(Creature* creature)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1: {
                ...
                while (FBSData[i].entry) {
                    if (Creature* c = me->SummonCreature(
                            FBSData[i].entry,
                            688.69f + i * 1.8f,
                            FBSSpawnPos.GetPositionY() + (float)irand(-2, 2),
                            FBSSpawnPos.GetPositionZ(),
                            3 * M_PI / 2))
                        c->GetMotionMaster()->MovePath(FBSData[i].pathId,
                                                       false);
                    ++i;
                }
            ...
            case 2:
                if (Creature* c = me->SummonCreature(NPC_TYRANNUS_VOICE,
                                                     me->GetPositionX(),
                                                     me->GetPositionY(),
...
public:
    ...
    struct npc_pos_tyrannus_eventsAI : public NullCreatureAI {
        ...
        {
            if (!me->IsAlive() || pInstance->GetData(DATA_GARFROST) != DONE ||
                pInstance->GetData(DATA_ICK) != DONE)
                return;
            if (type == 2) {
                if (id == 1)
                    if (killsLeft)
            ...
            switch (id) {
            case 1:
                if (pInstance->GetData(DATA_INSTANCE_PROGRESS) !=
            ...
            case 2:
                ...
                killsLeft = (Difficulty(me->GetMap()->GetSpawnMode()) ==
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 5:
                me->GetMotionMaster()->MoveTakeoff(
                    10,
                    me->GetPositionX() + 2.0f * cos(me->GetOrientation()),
                    me->GetPositionY() + 2.0f * std::sin(me->GetOrientation()),
                    me->GetPositionZ() + 30.0f,
                    7.0f);
                break;
            case 6:
            ...
            case 31:
                if (Creature* c =
                        pInstance->instance->GetCreature(deathbringerGUID[0]))
            ...
            case 32:
                if (Creature* c =
                        pInstance->instance->GetCreature(deathbringerGUID[1]))
                    if (c->GetMotionMaster()
                            ->GetCurrentMovementGeneratorType() !=
                        ESCORT_MOTION_TYPE) {
                        c->CastSpell(c, 69516, false);
                ...
                break;
            case 33:
                me->SummonCreature(NPC_YMIRJAR_WRATHBRINGER,
                                   919.733f,
                                   89.0972f,
                                   558.959f,
                                   3.85718f);
                me->SummonCreature(NPC_YMIRJAR_WRATHBRINGER,
                                   ...
                                   547.698f,
                                   3.735f);
                me->SummonCreature(NPC_YMIRJAR_FLAMEBEARER,
                                   909.356f,
                                   83.1684f,
                                   551.717f,
                                   3.57792f);
            ...
            case 36:
                ...
                break;
            case 60: {
                Position startPos[6] = {{927.11f, -72.60f, 592.2f, 1.52f},
                                        {922.92f, -72.64f, 592.3f, 1.52f},
                                        {930.46f, -72.57f, 592.1f, 1.52f},
                                        {934.52f, -72.52f, 592.1f, 1.52f},
                                        {934.57f, -77.66f, 592.20f, 1.52f},
                                        {927.15f, -77.07f, 592.20f, 1.52f}};
                Position endPos      = {926.10f, -46.63f, 591.2f, 1.52f};
                for (uint8 i = 0; i < 6; ++i)
                    if (Creature* s =
                            me->SummonCreature(i < 4 ? NPC_FALLEN_WARRIOR
                        ...
                        s->MovePosition(
                            finalPos,
                            startPos[i].GetExactDist(&startPos[0]),
                            Position::NormalizeOrientation(
                                startPos[i].GetAngle(&startPos[0]) + 1.52f));

                        Movement::PointsArray path;
                        path.push_back(G3D::Vector3(s->GetPositionX(),
                                                    s->GetPositionY(),
                                                    s->GetPositionZ()));
                        path.push_back(G3D::Vector3(finalPos.GetPositionX(),
                                                    finalPos.GetPositionY(),
                                                    finalPos.GetPositionZ()));

                        s->SetHomePosition(finalPos);
                        s->GetMotionMaster()->MoveSplinePath(&path);
                    }

                if (Difficulty(me->GetMap()->GetSpawnMode()) ==
                    DUNGEON_DIFFICULTY_HEROIC) {
                    Position startPos[6] = {{925.485f, -65.67f, 592.5f, 1.4f},
                                            {921.77f, -65.10f, 592.5f, 1.4f},
                                            {929.19f, -66.24f, 592.5f, 1.4f},
                                            {932.46f, -66.74f, 592.5f, 1.4f},
                                            {924.66f, -71.03f, 592.5f, 1.4f},
                                            {928.81f, -71.66f, 592.5f, 1.4f}};
                    Position middlePos   = {928.43f, -29.31f, 589.0f, 1.4f};
                    Position endPos      = {937.8f, 21.20f, 574.6f, 1.4f};
                    for (uint8 i = 0; i < 6; ++i)
                        if (Creature* s = me->SummonCreature(
                                i < 4 ? NPC_FALLEN_WARRIOR
                                      : NPC_WRATHBONE_COLDWRAITH,
                                startPos[i])) {
                            s->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                            ...
                            s->MovePosition(
                                ...
                                startPos[i].GetExactDist(&startPos[0]),
                                Position::NormalizeOrientation(
                                    startPos[i].GetAngle(&startPos[0]) + 1.4f));
                            s->MovePosition(
                                finalPos,
                                startPos[i].GetExactDist(&startPos[0]),
                                Position::NormalizeOrientation(
                            ...
                            path.push_back(G3D::Vector3(s->GetPositionX(),
                                                        s->GetPositionY(),
                                                        s->GetPositionZ()));
                            path.push_back(G3D::Vector3(midPos.GetPositionX(),
                                                        midPos.GetPositionY(),
                                                        midPos.GetPositionZ()));
                            path.push_back(
                                G3D::Vector3(finalPos.GetPositionX(),
                                             finalPos.GetPositionY(),
                                             finalPos.GetPositionZ()));
...
public:
    ...
    struct npc_pos_collapsing_icicleAI : public NullCreatureAI {
        npc_pos_collapsing_icicleAI(Creature* creature)
            : NullCreatureAI(creature)
        {
            pInstance = me->GetInstanceScript();
            timer1    = 2500;
...
public:
    npc_pos_martin_or_gorkun_second()
        : CreatureScript("npc_pos_martin_or_gorkun_second")
    {
    }

    struct npc_pos_martin_or_gorkun_secondAI : public NullCreatureAI {
        npc_pos_martin_or_gorkun_secondAI(Creature* creature)
            : NullCreatureAI(creature), summons(me)
        {
            pInstance = me->GetInstanceScript();
            ...
            if (p == 1)
                ...
                for (ObjectGuid const& guid : summons)
                    if (Creature* c = pInstance->instance->GetCreature(guid)) {
                        ...
                        c->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                          EMOTE_ONESHOT_CHEER);
                        float ang  = frand(1.92f, 2.36f);
                        float dist = urand(50, 85);
                        c->GetMotionMaster()->MovePoint(
                            0,
                            TSSpawnPos.GetPositionX() + cos(ang) * dist,
                            TSSpawnPos.GetPositionY() + std::sin(ang) * dist,
                            628.2f);
                    }
            }
            else if (p == 3) {
                if (pInstance)
                    if (Creature* c = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(DATA_SINDRAGOSA_GUID))) {
                        for (ObjectGuid const& guid : summons)
                            if (Creature* s =
                                    pInstance->instance->GetCreature(guid))
                                if (s->IsAlive())
                                    Unit::Kill(c, s);
                        if (me->IsAlive())
                            Unit::Kill(c, me);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1:
                if (TSData[i].entry) {
                    if (Creature* c =
                            me->SummonCreature(TSData[i].entry, TSSpawnPos)) {
                        c->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                          EMOTE_STATE_READY1H);
                        c->GetMotionMaster()->MovePoint(
                            0, TSData[i].x, TSData[i].y, TSHeight);
                    }
                    ++i;
                ...
                break;
            case 2:
                Talk(me->GetEntry() == NPC_MARTIN_VICTUS_2
            ...
            case 10:
                if (summons.GetEntryCount(NPC_FALLEN_WARRIOR) < 3)
                    if (Creature* c =
                            me->SummonCreature(NPC_FALLEN_WARRIOR,
                                               1060.95f,
                                               102.79f,
                                               630.2f,
                                               2.01f,
                        ...
                        c->GetMotionMaster()->MovePoint(
                            0, 1047.0f + offset, 118.0f + offset, 628.2f);
...
public:
    ...
    struct npc_pos_leader_secondAI : public NullCreatureAI {
        npc_pos_leader_secondAI(Creature* creature) : NullCreatureAI(creature)
        {
            ...
            if (pInstance) {
                if (Creature* c = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(DATA_RIMEFANG_GUID))) {
                    c->RemoveAllAuras();
                ...
                if (Creature* c = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(DATA_MARTIN_OR_GORKUN_GUID))) {
                    c->AI()->DoAction(2);
                }
            }
        }
        ...
        void DoAction(int32 p) override
        {
            if (p == 1) {
                events.RescheduleEvent(
                    1, me->GetEntry() == NPC_JAINA_PART2 ? 15s + 500ms : 18s);
                events.RescheduleEvent(
                    2, me->GetEntry() == NPC_JAINA_PART2 ? 16s + 500ms : 19s);
            }
        }

        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if ((spell->Id == SPELL_TELEPORT_JAINA ||
                 spell->Id == SPELL_TELEPORT_SYLVANAS) &&
                target && target->GetTypeId() == TYPEID_PLAYER) {
                float angle = rand_norm() * 2 * M_PI;
                float dist  = urand(1, 4);
                target->ToPlayer()->NearTeleportTo(
                    me->GetPositionX() + cos(angle) * dist,
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            switch (id) {
            case 0:
                Talk(me->GetEntry() == NPC_JAINA_PART2 ? SAY_JAINA_OUTRO_2
            ...
            case 6:
                ...
                if (GameObject* g =
                        me->FindNearestGameObject(GO_HOR_PORTCULLIS, 50.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1:
                if (pInstance)
                    if (Creature* c = me->SummonCreature(NPC_SINDRAGOSA,
                                                         TSSindragosaPos1)) {
                        c->SetCanFly(true);
                        c->SetDisableGravity(true);
            ...
            case 2:
                ...
                events.RescheduleEvent(
                    3, me->GetEntry() == NPC_JAINA_PART2 ? 7s : 8s);
                break;
            case 3:
                Talk(me->GetEntry() == NPC_JAINA_PART2 ? SAY_JAINA_OUTRO_1
                                                       : SAY_SYLVANAS_OUTRO_1);
                me->CastSpell(me,
                              me->GetEntry() == NPC_JAINA_PART2
            ...
            case 4:
                me->CastSpell(me,
                              ...
                              true);
                if (GameObject* barrier = me->SummonGameObject(203005,
                                                               1055.49f,
                                                               115.03f,
                                                               628.15f,
                                                               2.08f,
            ...
            case 5:
                if (pInstance)
                    if (Creature* x = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(
                                DATA_MARTIN_OR_GORKUN_GUID))) {
                        if (Creature* c = pInstance->instance->GetCreature(
                                pInstance->GetGuidData(DATA_SINDRAGOSA_GUID)))
                            c->CastSpell(x->GetPositionX(),
                                         x->GetPositionY(),
                                         x->GetPositionZ(),
            ...
            case 7:
                if (pInstance)
                    if (Creature* c = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(DATA_SINDRAGOSA_GUID)))
                        c->SetVisible(false);
                if (GameObject* barrier =
                        pInstance->instance->GetGameObject(barrierGUID))
                    barrier->Delete();
                barrierGUID.Clear();
            ...
            case 8:
                me->GetMotionMaster()->MovePath(
                    me->GetEntry() == NPC_JAINA_PART2 ? PATH_BEGIN_VALUE + 16
                                                      : PATH_BEGIN_VALUE + 17,
                    false);
                break;
            case 10:
...
public:
    spell_pos_empowered_blizzard()
        : SpellScriptLoader("spell_pos_empowered_blizzard")
    {
    }
    ...
    class spell_pos_empowered_blizzardAuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Unit* caster = GetCaster())
                caster->CastSpell((float)urand(447, 480),
                                  (float)urand(200, 235),
                                  528.71f,
                                  70131,
                                  true);
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_pos_empowered_blizzardAuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
const Position slaveFreePos[4] = {{699.82f, -82.68f, 512.6f, 0.0f},
    ...
    bool Execute(uint64 /*eventTime*/, uint32 /*updateTime*/) override
    {
        ...
        if (minDist < 200.0f * 200.0f)
            _owner.GetMotionMaster()->MovePoint(
                0, slaveFreePos[pointId], true, false);
        return true;
    }
...
public:
    spell_pos_slave_trigger_closest()
        : SpellScriptLoader("spell_pos_slave_trigger_closest")
    {
    }
    ...
    class spell_pos_slave_trigger_closestSpellScript : public SpellScript {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* target = GetHitUnit())
                if (target->GetUInt32Value(
                        UNIT_NPC_EMOTESTATE)) // prevent using multiple times
                {
                    if (Unit* caster = GetCaster())
                        if (Player* p = caster->ToPlayer()) {
                            p->RewardPlayerAndGroupAtEvent(36764,
                                                           caster); // alliance
                            p->RewardPlayerAndGroupAtEvent(36770,
                                                           caster); // horde
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_pos_slave_trigger_closestSpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_pos_rimefang_frost_nova()
        : SpellScriptLoader("spell_pos_rimefang_frost_nova")
    {
    }
    ...
    class spell_pos_rimefang_frost_novaSpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_pos_rimefang_frost_novaSpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
    ...
    class spell_pos_blightAuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (aurEff->GetTotalTicks() >= 0 &&
                aurEff->GetTickNumber() == uint32(aurEff->GetTotalTicks()))
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_pos_blightAuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
public:
    ...
    class spell_pos_glacial_strikeAuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_pos_glacial_strikeAuraScript::HandleEffectPeriodic,
                EFFECT_2,
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        ...
        if (inst->GetData(DATA_INSTANCE_PROGRESS) <
            INSTANCE_PROGRESS_AFTER_TUNNEL_WARN)
            return false;

        if (inst->GetData(DATA_GARFROST) == DONE &&
            inst->GetData(DATA_ICK) == DONE &&
            inst->GetData(DATA_TYRANNUS) != DONE &&
            !inst->GetGuidData(DATA_MARTIN_OR_GORKUN_GUID)) {
            if (Creature* c = inst->instance->SummonCreature(
                    NPC_GORKUN_IRONSKULL_2, TSSpawnPos))
                c->GetMotionMaster()->MovePoint(0, TSMidPos);

            inst->SetData(DATA_INSTANCE_PROGRESS,
