...
public:
    ...
    struct instance_pit_of_saron_InstanceScript : public InstanceScript {
        ...
        {
            instance->LoadGrid(LeaderIntroPos.GetPositionX(),
                               LeaderIntroPos.GetPositionY());
            if (Creature* c =
                    instance->GetCreature(GetGuidData(DATA_LEADER_FIRST_GUID)))
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_GORKUN_IRONSKULL_2:
                if (NPC_MartinOrGorkunGUID)
                    if (Creature* c =
                            instance->GetCreature(NPC_MartinOrGorkunGUID)) {
        ...
        void SetData(uint32 type, uint32 data) override
        {
                ...
                if (InstanceProgress < data) {
                    ...
                    if (InstanceProgress == INSTANCE_PROGRESS_TYRANNUS_INTRO &&
                        ...
                        DoUpdateAchievementCriteria(
                            ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 72845);
                }
                break;
            case DATA_GARFROST:
                m_auiEncounter[0] = data;
                if (data == DONE)
                    instance->SummonCreature(NPC_GORKUN_IRONSKULL_1,
                                             FBSSpawnPos);
                else // NOT_STARTED, IN_PROGRESS
                    bAchievEleven = true;
                if (data == DONE && GetData(DATA_ICK) == DONE)
                    if (GameObject* icewall =
                            instance->GetGameObject(GO_IceWallGUID))
            ...
            case DATA_TYRANNUS:
                ...
                if (data == DONE)
                    instance->SummonCreature(NPC_SYLVANAS_PART2,
                                             TSLeaderSpawnPos);
                break;
        ...
        void WriteSaveDataMore(std::ostringstream& data) override
        {
            data << m_auiEncounter[0] << ' ' << m_auiEncounter[1] << ' '
                 << m_auiEncounter[2] << ' ' << InstanceProgress;
        }
    };
...
void AddSC_instance_pit_of_saron() { new instance_pit_of_saron(); }
