    ...
    struct boss_garfrostAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->GetVictim()) {
                ...
                if (x < 600.0f || x > 770.0f || y < -270.0f || y > -137.0f ||
                    z < 514.0f || z > 550.0f) {
                    me->SetHealth(me->GetMaxHealth());
                    EnterEvadeMode(EVADE_REASON_OTHER);
                    if (CreatureGroup* f = me->GetFormation()) {
                        const CreatureGroup::CreatureGroupMemberType& m =
                            f->GetMembers();
                        for (CreatureGroup::CreatureGroupMemberType::
                                 const_iterator itr = m.begin();
                             itr != m.end();
                             ++itr)
                            if (itr->first->IsAlive() &&
                                itr->first->IsInCombat() &&
                                !itr->first->IsInEvadeMode() &&
            ...
            case EVENT_SPELL_THROW_SARONITE:
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 140.0f, true)) {
...
public:
    ...
    class spell_garfrost_permafrost_SpellScript : public SpellScript {
        ...
        void Unload() override { targetList.clear(); }

        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (Unit* caster = GetCaster()) {
                std::list<GameObject*> blockList;
                caster->GetGameObjectListWithEntryInGrid(
                    blockList, GO_SARONITE_ROCK, 100.0f);

                if (!blockList.empty()) {
                    for (std::list<WorldObject*>::iterator itrU =
                             targets.begin();
                         itrU != targets.end();
                         ++itrU)
                        if (WorldObject* target = (*itrU)) {
                            ...
                            if (!caster->IsWithinMeleeRange(target->ToUnit()))
                                for (std::list<GameObject*>::const_iterator
                                         itr = blockList.begin();
                                     itr != blockList.end();
                                     ++itr)
                            ...
                            if (valid) {
                                if (Aura* aur =
                                        target->ToUnit()->GetAura(70336))
                                    if (aur->GetStackAmount() >= 10 &&
                                        caster->GetTypeId() == TYPEID_UNIT)
                                        caster->ToCreature()->AI()->SetData(
                                            1, aur->GetStackAmount());
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_garfrost_permafrost_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_DEST_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_garfrost_permafrost_SpellScript::FilterTargetsNext,
                EFFECT_1,
                TARGET_UNIT_DEST_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_garfrost_permafrost_SpellScript::FilterTargetsNext,
