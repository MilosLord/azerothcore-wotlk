...
#define SPELL_STAGGERING_ROAR                                                  \
    DUNGEON_MODE(SPELL_STAGGERING_ROAR_N, SPELL_STAGGERING_ROAR_H)
#define SPELL_DREADFUL_ROAR                                                    \
    DUNGEON_MODE(SPELL_DREADFUL_ROAR_N, SPELL_DREADFUL_ROAR_H)
    ...
    struct boss_ingvar_the_plundererAI : public ScriptedAI {
        ...
        void JustSummoned(Creature* s) override
        {
            ...
            if (s->GetEntry() == NPC_ANNHYLDE) {
                ...
                s->SetPosition(s->GetPositionX(),
                               s->GetPositionY(),
                               s->GetPositionZ() + 35.0f,
                               s->GetOrientation());
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_VALKYR_MOVE:
                if (Creature* s = ObjectAccessor::GetCreature(*me, ValkyrGUID))
                    s->GetMotionMaster()->MovePoint(1,
                                                    s->GetPositionX(),
                                                    s->GetPositionY(),
                                                    s->GetPositionZ() - 15.0f);
                break;
            ...
            case EVENT_START_PHASE_2:
                if (Creature* c =
                        ObjectAccessor::GetCreature(*me, ValkyrGUID)) {
                    c->DespawnOrUnsummon();
                    summons.DespawnAll();
                }
                me->RemoveUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
            ...
            case EVENT_SPELL_ENRAGE_OR_SHADOW_AXE:
                if (me->GetDisplayId() == DISPLAYID_DEFAULT) {
                    ...
                    SetEquipmentSlots(
                        false, EQUIP_UNEQUIP, EQUIP_NO_CHANGE, EQUIP_NO_CHANGE);
                    events.Repeat(35s);
            ...
            case EVENT_AXE_RETURN:
                if (Creature* c = ObjectAccessor::GetCreature(*me, ThrowGUID))
                    c->GetMotionMaster()->MoveCharge(me->GetPositionX(),
                                                     me->GetPositionY(),
                                                     me->GetPositionZ());
...
void AddSC_boss_ingvar_the_plunderer() { new boss_ingvar_the_plunderer(); }
