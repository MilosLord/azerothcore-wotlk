            ...
            if (TempSummon* t = c->ToTempSummon())
                if (Unit* s = t->GetSummonerUnit()) {
                    ...
                    if (me->GetInstanceScript() &&
                        me->GetInstanceScript()->instance->IsHeroic()) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUMMON_SKELETONS:
                ...
                for (uint8 i = 0; i < 5; ++i) {
                    ...
                    if (Creature* c =
                            me->SummonCreature(NPC_SKELETON,
        ...
        void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            ...
            if (aurEff->GetTickNumber() == 1)
                if (Unit* target = GetTarget())
                    target->CastSpell(
                        (Unit*)nullptr, SPELL_FROST_TOMB_SUMMON, true);
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_frost_tombAuraScript::HandleEffectPeriodic,
                EFFECT_1,
