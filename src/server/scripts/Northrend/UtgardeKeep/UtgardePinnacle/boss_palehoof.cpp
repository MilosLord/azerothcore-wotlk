    ...
    struct boss_palehoofAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (m_pInstance) {
                ...
                // Reset statue
                if (GameObject* statisGenerator =
                        m_pInstance->instance->GetGameObject(
                            m_pInstance->GetGuidData(STATIS_GENERATOR))) {
                    statisGenerator->RemoveGameObjectFlag(
                ...
                // Reset mini bosses
                for (uint8 i = 0; i < 4; ++i) {
                    if (Creature* Animal = ObjectAccessor::GetCreature(
                            *me,
                            m_pInstance->GetGuidData(DATA_NPC_FRENZIED_WORGEN +
                                                     i))) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_UNFREEZE_MONSTER: {
                if (Creature* orb = ObjectAccessor::GetCreature(*me, OrbGUID)) {
                    if (Creature* miniBoss = ObjectAccessor::GetCreature(
                            *me,
                            m_pInstance->GetGuidData(
                                DATA_NPC_FRENZIED_WORGEN +
                                RandomUnfreeze[Counter]))) {
                        Counter++;
            ...
            case EVENT_PALEHOOF_WITHERING_ROAR: {
                me->CastSpell(me,
                              IsHeroic() ? SPELL_WITHERING_ROAR_H
            ...
            case EVENT_PALEHOOF_IMPALE: {
                if (Unit* tgt = SelectTarget(SelectTargetMethod::Random, 0))
                    me->CastSpell(tgt,
                                  IsHeroic() ? SPELL_IMPALE_H : SPELL_IMPALE_N,
                                  false);
...
public:
    ...
    struct npc_massive_jormungarAI : public ScriptedAI {
        npc_massive_jormungarAI(Creature* pCreature)
            : ScriptedAI(pCreature), summons(me)
        {
            m_pInstance = pCreature->GetInstanceScript();
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_JORMUNGAR_ACID_SPLATTER: {
                me->CastSpell(me,
                              IsHeroic() ? SPELL_ACID_SPLATTER_H
                ...
                for (uint8 i = 0; i < 6; ++i) {
                    if (Creature* pJormungarWorm = me->SummonCreature(
                            NPC_JORMUNGAR_WORM,
                            x + rand() % 10,
                            y + rand() % 10,
                            z,
            ...
            case EVENT_JORMUNGAR_POISON_BREATH: {
                if (Unit* tgt = SelectTarget(SelectTargetMethod::Random, 0))
                    me->CastSpell(tgt,
                                  IsHeroic() ? SPELL_POISON_BREATH_H
        ...
        {
            if (m_pInstance) {
                if (Creature* palehoof = ObjectAccessor::GetCreature(
                        *me, m_pInstance->GetGuidData(DATA_GORTOK_PALEHOOF)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_RHINO_GORE: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_GORE_H : SPELL_GORE_N,
                              false);
                events.Repeat(13s, 17s);
                break;
            }
            case EVENT_RHINO_WOUND: {
                if (Unit* tgt = SelectTarget(SelectTargetMethod::Random, 0))
                    me->CastSpell(tgt,
                                  IsHeroic() ? SPELL_GRIEVOUS_WOUND_H
    ...
    struct npc_ravenous_furbolgAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_FURBOLG_CHAIN: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_CHAIN_LIGHTING_H
...
public:
    ...
    bool OnGossipHello(Player* /*pPlayer*/, GameObject* go) override
    {
        ...
        Creature* pPalehoof = ObjectAccessor::GetCreature(
            *go,
            pInstance ? pInstance->GetGuidData(DATA_GORTOK_PALEHOOF)
                      : ObjectGuid::Empty);
