...
static Position TrashPosition[] = {{441.236f, -512.000f, 104.930f, 0.0f},
                                   {478.436f, -494.475f, 104.730f, 0.0f}};

static Position SkadiPosition[] = {{338.679f, -507.254f, 124.122f, 0.0f},
                                   {338.679f, -513.254f, 124.122f, 0.0f},
                                   {490.096f, -510.86f, 123.368f, 0.0f},
                                   {490.76f, -517.389f, 123.368f, 0.0f}};

enum phase { PHASE_NONE, PHASE_START, PHASE_FLIGHT, PHASE_LAND, PHASE_GROUND };
    ...
    struct boss_skadiAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (Creature* cr = me->SummonCreature(
                    NPC_GRAUF, 341.741f, -516.955f, 116.669f, 3.12414f)) {
            ...
            if (m_pInstance) {
                m_pInstance->DoStopTimedAchievement(
                    ACHIEVEMENT_TIMED_TYPE_EVENT, ACHIEV_TIMED_LODI_DODI);
        ...
        Creature* GetGrauf()
        {
            return ObjectAccessor::GetCreature(*me, GraufGUID);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SKADI_CRUSH: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_CRUSH_H : SPELL_CRUSH_N,
                              false);
                events.Repeat(8s);
                break;
            }
            case EVENT_SKADI_SPEAR: {
                if (Unit* tgt = SelectTarget(SelectTargetMethod::Random, 0))
                    me->CastSpell(tgt,
                                  IsHeroic() ? SPELL_POISONED_SPEAR_H
        ...
        void JustDied(Unit* /*pKiller*/) override
        {
            ...
            if (m_pInstance) {
                ...
                m_pInstance->HandleGameObject(
                    m_pInstance->GetGuidData(SKADI_DOOR), true);
            }
        }
...
public:
    ...
    struct boss_skadi_graufAI : public VehicleAI {
        boss_skadi_graufAI(Creature* pCreature)
            : VehicleAI(pCreature), summons(me)
        {
            m_pInstance = pCreature->GetInstanceScript();
        }
        ...
        void SpawnFlameTriggers(uint8 point)
        {
            for (uint8 j = 0; j < 50; ++j) {
                if (point == 1)
                    me->SummonCreature(NPC_BREATH_TRIGGER,
                                       480.0f - (j * 3),
                                       -518.0f + (j / 16.0f),
                                       105.0f,
                                       0,
            ...
            if (point == 0) {
                Creature* cr;
                if ((cr = me->SummonCreature(NPC_BREATH_TRIGGER,
                                             483,
                                             -484.9f,
                                             105,
                                             0,
        ...
        void RemoveSkadi(bool withEvade)
        {
            if (Unit* skadi = me->GetVehicleKit()->GetPassenger(0)) {
                ...
                if (withEvade) {
                    ...
                    skadi->UpdatePosition(
                        343.02f, -507.325f, 104.567f, M_PI, true);
                }
                else
        ...
        void CheckPlayers()
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                 itr != pList.end();
                 ++itr) {
                if (itr->GetSource()->GetPositionX() < 320.0f ||
        ...
        {
            if (Creature* Harpooner = me->SummonCreature(
                    NPC_YMIRJAR_HARPOONER,
                    TrashPosition[Spot].GetPositionX() + rand() % 5,
                    TrashPosition[Spot].GetPositionY() + rand() % 5,
                    TrashPosition[Spot].GetPositionZ(),
                    0,
            ...
            if (Creature* Second = me->SummonCreature(
                    (urand(0, 1) ? NPC_YMIRJAR_WARRIOR
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_GRAUF_START: {
                ...
                me->GetMotionMaster()->MoveTakeoff(
                    10,
                    SkadiPosition[0].GetPositionX(),
                    SkadiPosition[0].GetPositionY(),
                    SkadiPosition[0].GetPositionZ(),
                    3.0f);
            ...
            case EVENT_GRAUF_MOVE: {
                ...
                me->GetMotionMaster()->MovePoint(
                    targetPoint,
                    SkadiPosition[targetPoint].GetPositionX(),
                    SkadiPosition[targetPoint].GetPositionY(),
                    SkadiPosition[targetPoint].GetPositionZ());
                if (targetPoint <= 1) {
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, GameObject* go) override
    {
        ...
        if (m_pInstance &&
            m_pInstance->GetData(DATA_SKADI_THE_RUTHLESS) == IN_PROGRESS)
            if (m_pInstance->GetData(SKADI_IN_RANGE) == 1) {
                uint8 count = m_pInstance->GetData(SKADI_HITS) + 1;
                m_pInstance->SetData(SKADI_HITS, count);

                if (Creature* grauf = ObjectAccessor::GetCreature(
                        *pPlayer, m_pInstance->GetGuidData(DATA_GRAUF))) {
                    if (count >= 3) {
                        m_pInstance->SetData(SKADI_IN_RANGE, 0);
