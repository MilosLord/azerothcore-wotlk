...
static ActiveBoatStruct BoatStructure[4] = {{NPC_RANULF,
                                             NPC_RANULF_VISUAL,
                                             SAY_SUMMON_RANULF,
                                             404.379f,
                                             -335.335f,
...
public:
    ...
    struct boss_ymironAI : public ScriptedAI {
        boss_ymironAI(Creature* pCreature)
            : ScriptedAI(pCreature), summons(me), summons2(me)
        {
        ...
        void MovementInform(uint32 uiType, uint32 point) override
        {
            ...
            if (point == 0) {
                ...
                if (Creature* cr = me->FindNearestCreature(
                        BoatStructure[BoatOrder[BoatNum - 1]].trigger, 50.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_YMIRON_HEALTH_CHECK: {
                if (me->GetHealth() <
                    std::max(0.0f,
                             float(me->GetMaxHealth() *
                                   (1.0f - (IsHeroic() ? 0.2f : 0.334f) *
                                               float(BoatNum + 1))))) {
                    ...
                    me->GetMotionMaster()->MovePoint(
                        0,
                        BoatStructure[BoatOrder[BoatNum]].MoveX,
                        BoatStructure[BoatOrder[BoatNum]].MoveY,
                        BoatStructure[BoatOrder[BoatNum]].MoveZ);
                    me->SetUnitFlag(UNIT_FLAG_DISABLE_MOVE);
                    summons.DespawnAll();

                    // Spawn flames in previous boat if any
                    if (BoatNum) // different than 0
                        if (Creature* cr = me->SummonTrigger(
                                BoatStructure[BoatOrder[BoatNum - 1]].SpawnX,
                                BoatStructure[BoatOrder[BoatNum - 1]].SpawnY,
                                BoatStructure[BoatOrder[BoatNum - 1]].SpawnZ,
                                0,
                                1800000)) {
            ...
            case EVENT_YMIRON_BANE: {
                me->CastSpell(
                    me, IsHeroic() ? SPELL_BANE_H : SPELL_BANE_N, false);
                events.Repeat(20s, 25s);
                break;
            }
            case EVENT_YMIRON_FETID_ROT: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_FETID_ROT_H
                                         : SPELL_FETID_ROT_N,
                              false);
            ...
            case EVENT_YMIRON_DARK_SLASH: {
                ...
                me->CastCustomSpell(
                    me->GetVictim(), SPELL_DARK_SLASH, &dmg, 0, 0, false);
                events.Repeat(30s, 35s);
                break;
            }
            case EVENT_YMIRON_ACTIVATE_BOAT: {
                // Spawn it!
                if (Creature* king = me->SummonCreature(
                        BoatStructure[BoatOrder[BoatNum - 1]].npc,
                        BoatStructure[BoatOrder[BoatNum - 1]].SpawnX,
                        BoatStructure[BoatOrder[BoatNum - 1]].SpawnY,
                        BoatStructure[BoatOrder[BoatNum - 1]].SpawnZ,
                        BoatStructure[BoatOrder[BoatNum - 1]].SpawnO,
                        TEMPSUMMON_CORPSE_DESPAWN,
                        0)) {
                    me->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                    king->CastSpell(me, SPELL_CHANNEL_SPIRIT_TO_YMIRON, true);
                    summons.Summon(king);
                    king->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
                    ...
                    case 0:
                        events.ScheduleEvent(
                            EVENT_YMIRON_RANULF_ABILITY, 3s, 1);
                        break;
                    case 1:
                        events.ScheduleEvent(
                            EVENT_YMIRON_TORGYN_ABILITY, 3s, 1);
                        break;
                    case 2:
                    ...
                    case 3:
                        events.ScheduleEvent(
                            EVENT_YMIRON_HALDOR_ABILITY, 3s, 1);
                        break;
                    }
                }
            ...
            case EVENT_YMIRON_BJORN_ABILITY: {
                if (Creature* sf = me->SummonCreature(
                        NPC_SPIRIT_FOUNT,
                        385 + rand() % 10,
                        -330 + rand() % 10,
                        104.756f,
                    ...
                    sf->AddAura(IsHeroic() ? SPELL_SPIRIT_FOUNT_H
                                           : SPELL_SPIRIT_FOUNT_N,
                                sf);
                    sf->SetUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
                    sf->GetMotionMaster()->MoveFollow(
                        me->GetVictim(), 0, rand_norm() * M_PI * 2);
                }
                break;
            }
            case EVENT_YMIRON_HALDOR_ABILITY: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_SPIRIT_STRIKE_H
            ...
            case EVENT_YMIRON_TORGYN_ABILITY: {
                for (uint8 i = 0; i < 4; ++i) {
                    if (Creature* as = me->SummonCreature(
                            NPC_AVENGING_SPIRIT,
                            me->GetPositionX() + rand() % 10,
                            me->GetPositionY() + rand() % 10,
                            me->GetPositionZ(),
                            0,
                            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                            30000)) {
...
void AddSC_boss_ymiron() { new boss_ymiron(); }
