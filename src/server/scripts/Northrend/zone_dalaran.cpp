...
public:
    npc_steam_powered_auctioneer()
        : CreatureScript("npc_steam_powered_auctioneer")
    {
    }

    struct npc_steam_powered_auctioneerAI : public ScriptedAI {
        npc_steam_powered_auctioneerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    ...
    struct npc_mei_francis_mountAI : public ScriptedAI {
        ...
        {
            if (player->GetTeamId() == TEAM_ALLIANCE)
                return me->GetEntry() == 32206 || me->GetEntry() == 32335 ||
                       me->GetEntry() == 31851;
            else
                return me->GetEntry() == 32207 || me->GetEntry() == 32336 ||
                       me->GetEntry() == 31852;
        }
    };
...
public:
    ...
    struct npc_shandy_dalaranAI : public ScriptedAI {
        ...
        void SetData(uint32 type, uint32 /*data*/) override
        {
            ...
            case ACTION_SHANDY_INTRO:
                if (Creature* aquanos =
                        me->FindNearestCreature(NPC_AQUANOS_ENTRY, 30, true))
            ...
            default:
                if (_lSource == type && _canWash) {
                    _canWash = false;
                    _events.ScheduleEvent(EVENT_INTRO_DH2,
                                          type == ACTION_UNMENTIONABLES ? 4s
                                                                        : 10s);
                    Talk(SAY_SHANDY2);
                    if (Creature* aquanos =
                            ObjectAccessor::GetCreature(*me, _aquanosGUID))
                        aquanos->CastSpell(
                            aquanos, SPELL_EVOCATION_VISUAL, false);
                }
                break;
            }
        }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_SUITABLE_DISGUISE_A) ==
            ...
            if (_switchImageTimer > MINUTE * IN_MILLISECONDS) {
                ...
                for (QuestRelations::const_iterator i = objectQR.first;
                    ...
                    if (_summonGUID.GetEntry() != newEntry) {
                        if (Creature* image =
                                ObjectAccessor::GetCreature(*me, _summonGUID))
                            image->DespawnOrUnsummon();

                        float z = 653.622f;
                        if (newEntry == NPC_MALYGOS_IMAGE ||
                            newEntry == NPC_RAZORSCALE_IMAGE ||
                            newEntry == NPC_SARTHARION_IMAGE)
                            z += 3.0f;
                        me->SummonCreature(
                            newEntry, 5703.077f, 583.9757f, z, 3.926991f);
                    }
                }
            }
        }
...
public:
    ...
    struct npc_mageguard_dalaranAI : public ScriptedAI {
        npc_mageguard_dalaranAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            creature->ApplySpellImmune(
                0, IMMUNITY_DAMAGE, SPELL_SCHOOL_NORMAL, true);
            creature->ApplySpellImmune(
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!player || player->IsGameMaster() ||
                ...
                // If player has Disguise aura for quest A Meeting With The
                // Magister or An Audience With The Arcanist, do not teleport it
                // away but let it pass
                player->HasAura(SPELL_SUNREAVER_DISGUISE_FEMALE) ||
                player->HasAura(SPELL_SUNREAVER_DISGUISE_MALE) ||
            ...
            case NPC_SILVER_COVENANT_GUARDIAN_MAGE:
                if (player->GetTeamId() ==
                    TEAM_HORDE) // Horde unit found in Alliance area
                {
                    if (GetClosestCreatureWithEntry(
                            me, NPC_APPLEBOUGH_A, 32.0f)) {
                        if (me->isInBackInMap(
                                who, 12.0f)) // In my line of sight, "outdoors",
                                             // and behind me
                            DoCast(who, SPELL_TRESPASSER_A); // Teleport the
                                                             // Horde unit out
                    }
                    else // In my line of sight, and "indoors"
                        DoCast(
                            who,
                            SPELL_TRESPASSER_A); // Teleport the Horde unit out
                }
                break;
            case NPC_SUNREAVER_GUARDIAN_MAGE:
                if (player->GetTeamId() ==
                    TEAM_ALLIANCE) // Alliance unit found in Horde area
                {
                    if (GetClosestCreatureWithEntry(
                            me, NPC_SWEETBERRY_H, 32.0f)) {
                        if (me->isInBackInMap(
                                who, 12.0f)) // In my line of sight, "outdoors",
                                             // and behind me
                            DoCast(who,
                                   SPELL_TRESPASSER_H); // Teleport the Alliance
                                                        // unit out
                    }
                    else // In my line of sight, and "indoors"
                        DoCast(who, SPELL_TRESPASSER_H); // Teleport the
                                                         // Alliance unit out
                }
                break;
            }
...
public:
    ...
    struct npc_minigob_manabonkAI : public ScriptedAI {
            ...
            for (Map::PlayerList::const_iterator itr = players.begin();
                ...
                if (Player* player = itr->GetSource()->ToPlayer())
                    if (player->GetZoneId() == ZONE_DALARAN &&
                        !player->IsFlying() && !player->IsMounted() &&
                        !player->IsGameMaster())
        ...
        void SendMailToPlayer(Player* player)
        {
            ...
            int16 deliverDelay =
                irand(MAIL_DELIVER_DELAY_MIN, MAIL_DELIVER_DELAY_MAX);
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_SELECT_TARGET:
                    ...
                    if (Player* player = SelectTargetInDalaran()) {
                        me->NearTeleportTo(player->GetPositionX(),
                                           player->GetPositionY(),
                                           player->GetPositionZ(),
                                           0.0f);
                ...
                case EVENT_BLINK: {
                    ...
                    me->GetMotionMaster()->MovePoint(
                        0, pos.m_positionX, pos.m_positionY, pos.m_positionZ);
    ...
    void Reset() override
    {
        ...
        scheduler.Schedule(420ms, [this](TaskContext context) {
            if (Creature* cr = me->FindNearestCreature(
                    NPC_DND_DALARAN_TOY_STORE_PLANE_STRING_HOOK, 42.0f))
