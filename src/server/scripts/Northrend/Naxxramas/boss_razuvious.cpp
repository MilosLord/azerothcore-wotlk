...
enum Says { SAY_AGGRO = 0, SAY_SLAY = 1, SAY_TAUNTED = 2, SAY_DEATH = 3 };
...
enum Misc { NPC_DEATH_KNIGHT_UNDERSTUDY = 16803, NPC_RAZUVIOUS = 16061 };
...
public:
    ...
    struct boss_razuviousAI : public BossAI {
        explicit boss_razuviousAI(Creature* c)
            : BossAI(c, BOSS_RAZUVIOUS), summons(me)
        {
        ...
        void SpawnHelpers()
        {
            me->SummonCreature(NPC_DEATH_KNIGHT_UNDERSTUDY,
                               2762.23f,
                               -3085.07f,
                               267.685f,
                               1.95f);
        ...
        {
            // Damage done by the controlled Death Knight understudies should
            // also count toward damage done by players
            if (who && who->GetTypeId() == TYPEID_UNIT &&
                who->GetEntry() == NPC_DEATH_KNIGHT_UNDERSTUDY) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_DISRUPTING_SHOUT:
                me->CastSpell(me,
                              RAID_MODE(SPELL_DISRUPTING_SHOUT_10,
            ...
            case EVENT_JAGGED_KNIFE:
                if (Unit* target =
                        SelectTarget(SelectTargetMethod::Random, 0, 45.0f)) {
