...
const Position summoningPosition[5] = {{2728.06f, -3535.38f, 263.21f, 2.75f},
    ...
    struct boss_nothAI : public BossAI {
        ...
        void Reset() override
        {
            ...
            if (pInstance) {
                if (GameObject* go = me->GetMap()->GetGameObject(
                        pInstance->GetGuidData(DATA_NOTH_ENTRY_GATE))) {
        ...
        void JustDied(Unit* killer) override
        {
            if (me->GetPositionZ() > 270.27f) {
                me->RemoveUnitFlag(UNIT_FLAG_NOT_SELECTABLE |
                                   UNIT_FLAG_DISABLE_MOVE);
                me->NearTeleportTo(nothPosition.GetPositionX(),
                                   nothPosition.GetPositionY(),
                                   nothPosition.GetPositionZ(),
                                   nothPosition.GetOrientation(),
                                   true);
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_BLINK:
                DoResetThreatList();
                me->CastSpell(
                    me, RAID_MODE(SPELL_CRIPPLE_10, SPELL_CRIPPLE_25), false);
            ...
            case EVENT_BALCONY_SUMMON_REAL:
                me->CastSpell(
                    me, SPELL_SUMMON_PLAGUED_WARRIORS, true); // visual
...
void AddSC_boss_noth() { new boss_noth(); }
