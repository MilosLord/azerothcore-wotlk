...
enum Events { EVENT_POISON_BOLT = 1, EVENT_RAIN_OF_FIRE = 2, EVENT_FRENZY = 3 };

enum Misc { NPC_NAXXRAMAS_WORSHIPPER = 16506, NPC_NAXXRAMAS_FOLLOWER = 16505 };
...
public:
    ...
    struct boss_faerlinaAI : public BossAI {
        ...
        void SummonHelpers()
        {
            ...
            if (Is25ManRaid()) {
                me->SummonCreature(
                    NPC_NAXXRAMAS_FOLLOWER, 3347.49f, -3617.59f, 261.0f, 4.49f);
                me->SummonCreature(
                    NPC_NAXXRAMAS_FOLLOWER, 3359.64f, -3619.16f, 261.0f, 4.56f);
            }
        }
        ...
        void Reset() override
        {
            ...
            if (pInstance) {
                if (GameObject* go = me->GetMap()->GetGameObject(
                        pInstance->GetGuidData(DATA_FAERLINA_WEB))) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_POISON_BOLT:
                if (!me->HasAura(RAID_MODE(SPELL_WIDOWS_EMBRACE,
                                           SPELL_MINION_WIDOWS_EMBRACE))) {
                    me->CastCustomSpell(RAID_MODE(SPELL_POISON_BOLT_VOLLEY_10,
            ...
            case EVENT_RAIN_OF_FIRE:
                if (Unit* target =
                    ...
                    me->CastSpell(
                        target,
                        RAID_MODE(SPELL_RAIN_OF_FIRE_10, SPELL_RAIN_OF_FIRE_25),
                        false);
                }
            ...
            case EVENT_FRENZY:
                if (!me->HasAura(RAID_MODE(SPELL_FRENZY_10, SPELL_FRENZY_25))) {
                    ...
                    Talk(EMOTE_FRENZY);
                    me->CastSpell(
                        me, RAID_MODE(SPELL_FRENZY_10, SPELL_FRENZY_25), true);
                    events.Repeat(1min);
                }
                else {
                    events.Repeat(30s);
        ...
        {
            if (spell->Id ==
                RAID_MODE(SPELL_WIDOWS_EMBRACE, SPELL_MINION_WIDOWS_EMBRACE)) {
                Talk(EMOTE_WIDOWS_EMBRACE);
                if (me->HasAura(RAID_MODE(SPELL_FRENZY_10, SPELL_FRENZY_25))) {
                    me->RemoveAurasDueToSpell(
                        RAID_MODE(SPELL_FRENZY_10, SPELL_FRENZY_25));
...
void AddSC_boss_faerlina() { new boss_faerlina(); }
