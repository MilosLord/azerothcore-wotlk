...
public:
    ...
    struct boss_kelthuzadAI : public BossAI {
        explicit boss_kelthuzadAI(Creature* c)
            : BossAI(c, BOSS_KELTHUZAD), summons(me)
        {
        ...
        float NormalizeOrientation(float o)
        {
            return std::fmod(
                ...
                2.0f * static_cast<float>(
                           M_PI)); // Only positive values will be passed
        }

        void SpawnHelpers()
        {
            // spawn at gate
            me->SummonCreature(NPC_UNSTOPPABLE_ABOMINATION,
                               3656.19f,
                               -5093.78f,
                               143.33f,
                               6.08,
            ...
            me->SummonCreature(NPC_UNSTOPPABLE_ABOMINATION,
                               ...
                               TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                               2000); // abo right
            me->SummonCreature(NPC_SOUL_WEAVER,
                               3651.73f,
                               -5092.62f,
                               143.38f,
                               6.05,
                               TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                               2000); // soul behind
            me->SummonCreature(NPC_SOLDIER_OF_THE_FROZEN_WASTES,
                               3660.17f,
                               -5092.45f,
                               143.37f,
                               6.07,
            ...
            for (uint8 i = 6; i < 12; ++i) {
                for (uint8 j = 0; j < 8; ++j) {
                    ...
                    me->SummonCreature(
                        ...
                        SummonGroups[i].GetPositionX() + 6 * cos(angle),
                        SummonGroups[i].GetPositionY() + 6 * std::sin(angle),
                        SummonGroups[i].GetPositionZ(),
                        SummonGroups[i].GetOrientation(),
                        TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                        20000);
                }
            }
            for (uint8 i = 6; i < 12; ++i) {
                for (uint8 j = 1; j < 4; ++j) {
                    ...
                    float angle =
                        SummonGroups[i].GetOrientation() + M_PI * 2 / 4 * j;
                    me->SummonCreature(
                        NPC_UNSTOPPABLE_ABOMINATION,
                        SummonGroups[i].GetPositionX() + dist * cos(angle),
                        SummonGroups[i].GetPositionY() + dist * std::sin(angle),
                        SummonGroups[i].GetPositionZ(),
                        SummonGroups[i].GetOrientation(),
                        TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                        20000);
            ...
            for (uint8 i = 6; i < 12; ++i) {
                for (uint8 j = 0; j < 1; ++j) {
                    ...
                    me->SummonCreature(
                        ...
                        SummonGroups[i].GetPositionX() + 6 * cos(angle),
                        SummonGroups[i].GetPositionY() + 6 * std::sin(angle),
                        SummonGroups[i].GetPositionZ() + 0.5f,
                        SummonGroups[i].GetOrientation(),
                        TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                        20000);
                }
        ...
        void Reset() override
        {
            ...
            if (GameObject* go = me->GetMap()->GetGameObject(
                    pInstance->GetGuidData(DATA_KELTHUZAD_FLOOR))) {
                go->SetPhaseMask(1, true);
                go->SetGoState(GO_STATE_READY);
            }
            if (GameObject* go = me->GetMap()->GetGameObject(
                    pInstance->GetGuidData(DATA_KELTHUZAD_GATE))) {
                if (!_justSpawned) // Don't open the door if we just spawned and
                                   // are still doing the conversation
                {
            ...
            _justSpawned = false;
            if (GameObject* go = me->GetMap()->GetGameObject(
                    pInstance->GetGuidData(DATA_KELTHUZAD_PORTAL_1))) {
                go->SetGoState(GO_STATE_READY);
            }
            if (GameObject* go = me->GetMap()->GetGameObject(
                    pInstance->GetGuidData(DATA_KELTHUZAD_PORTAL_2))) {
                go->SetGoState(GO_STATE_READY);
            }
            if (GameObject* go = me->GetMap()->GetGameObject(
                    pInstance->GetGuidData(DATA_KELTHUZAD_PORTAL_3))) {
                go->SetGoState(GO_STATE_READY);
            }
            if (GameObject* go = me->GetMap()->GetGameObject(
                    pInstance->GetGuidData(DATA_KELTHUZAD_PORTAL_4))) {
                go->SetGoState(GO_STATE_READY);
            }
        }
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (Creature* guardian =
                    summons.GetCreatureWithEntry(NPC_GUARDIAN_OF_ICECROWN)) {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (!me->IsInCombat() && who->GetTypeId() == TYPEID_PLAYER &&
                who->IsAlive() && me->GetDistance(who) <= 50.0f)
                AttackStart(who);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_FROST_BOLT_SINGLE:
                me->CastSpell(me->GetVictim(),
                              RAID_MODE(SPELL_FROST_BOLT_SINGLE_10,
            ...
            case EVENT_FROST_BOLT_MULTI:
                me->CastSpell(me,
                              RAID_MODE(SPELL_FROST_BOLT_MULTI_10,
            ...
            case EVENT_FROST_BLAST:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, RAID_MODE(1, 0), 0, true)) {
            ...
            case EVENT_P3_LICH_KING_SAY:
                if (pInstance) {
                    if (Creature* cr = ObjectAccessor::GetCreature(
                            *me, pInstance->GetGuidData(DATA_LICH_KING_BOSS))) {
                        cr->AI()->Talk(SAY_ANSWER_REQUEST);
                    }
                }
                for (uint8 i = 0; i < RAID_MODE(2, 4); ++i) {
                    events.ScheduleEvent(EVENT_SUMMON_GUARDIAN_OF_ICECROWN,
                                         10000 + (i * 5000));
                }
                break;
            case EVENT_SUMMON_GUARDIAN_OF_ICECROWN:
                if (Creature* cr =
                        me->SummonCreature(NPC_GUARDIAN_OF_ICECROWN,
                                           SpawnPool[RAND(0, 1, 3, 4)])) {
        ...
        void AttackStart(Unit* who) override
        {
            ...
            if (callHelp) {
                ...
                me->GetCreaturesWithEntryInRange(
                    targets, 15.0f, me->GetEntry());
                for (std::list<Creature*>::const_iterator itr = targets.begin();
                     itr != targets.end();
                     ++itr) {
            ...
            if (me->GetEntry() != NPC_UNSTOPPABLE_ABOMINATION &&
                me->GetEntry() != NPC_GUARDIAN_OF_ICECROWN) {
...
public:
    spell_kelthuzad_frost_blast()
        : SpellScriptLoader("spell_kelthuzad_frost_blast")
    {
    }
    ...
    class spell_kelthuzad_frost_blast_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_kelthuzad_frost_blast_SpellScript::FilterTargets,
                EFFECT_ALL,
...
public:
    spell_kelthuzad_detonate_mana()
        : SpellScriptLoader("spell_kelthuzad_detonate_mana")
    {
    }
    ...
    class spell_kelthuzad_detonate_mana_AuraScript : public AuraScript {
        ...
        void HandleScript(AuraEffect const* aurEff)
        {
            ...
            if (auto mana = int32(target->GetMaxPower(POWER_MANA) / 10)) {
                ...
                target->CastCustomSpell(SPELL_MANA_DETONATION_DAMAGE,
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_kelthuzad_detonate_mana_AuraScript::HandleScript,
                EFFECT_0,
