...
public:
    ...
    struct boss_sapphironAI : public BossAI {
        ...
        {
            me->SummonGameObject(GO_SAPPHIRON_BIRTH,
                                 me->GetPositionX(),
                                 me->GetPositionY(),
                                 me->GetPositionZ(),
                                 0,
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            me->CastSpell(
                me, RAID_MODE(SPELL_FROST_AURA_10, SPELL_FROST_AURA_25), true);
        ...
        bool IsValidExplosionTarget(WorldObject* target)
        {
            for (ObjectGuid const& guid : blockList) {
                ...
                if (Unit* block = ObjectAccessor::GetUnit(*me, guid)) {
                    if (block->IsInBetween(me, target, 2.0f) &&
                        block->IsWithinDist(target, 10.0f))
                        return false;
                }
            }
            return true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_TAIL_SWEEP:
                me->CastSpell(
                    me,
                    RAID_MODE(SPELL_TAIL_SWEEP_10, SPELL_TAIL_SWEEP_25),
                    false);
                events.Repeat(10s);
                return;
            case EVENT_LIFE_DRAIN:
                me->CastCustomSpell(
                    RAID_MODE(SPELL_LIFE_DRAIN_10, SPELL_LIFE_DRAIN_25),
            ...
            case EVENT_BLIZZARD: {
                ...
                if (Unit* target = SelectTarget(
                    ...
                    cr = me->SummonCreature(
                        NPC_BLIZZARD, *target, TEMPSUMMON_TIMED_DESPAWN, 16000);
                }
                else {
                    cr = me->SummonCreature(
                        NPC_BLIZZARD, *me, TEMPSUMMON_TIMED_DESPAWN, 16000);
                }
                if (cr) {
            ...
            case EVENT_FLIGHT_ICEBOLT: {
                if (currentTarget) {
                    if (Unit* target =
                            ObjectAccessor::GetUnit(*me, currentTarget)) {
                        me->SummonGameObject(GO_ICE_BLOCK,
                                             target->GetPositionX(),
                                             target->GetPositionY(),
                                             target->GetPositionZ(),
                ...
                for (; i != me->GetThreatMgr().GetThreatList().end(); ++i) {
                    if ((*i)->getTarget()->GetTypeId() == TYPEID_PLAYER) {
                        bool inList = false;
                        if (!blockList.empty()) {
                            for (GuidList::const_iterator itr =
                                     blockList.begin();
                                 itr != blockList.end();
                                 ++itr) {
                ...
                if (!targets.empty() && iceboltCount) {
                    ...
                    --iceboltCount;
                    events.ScheduleEvent(EVENT_FLIGHT_ICEBOLT,
                                         (me->GetExactDist(*itr) / 13.0f) *
                                             IN_MILLISECONDS);
                }
                else {
            ...
            case EVENT_FLIGHT_START_LAND:
                if (!blockList.empty()) {
                    for (GuidList::const_iterator itr = blockList.begin();
                         itr != blockList.end();
                         ++itr) {
            ...
            case EVENT_HUNDRED_CLUB: {
                ...
                for (const auto& itr : pList) {
                    if (itr.GetSource()->GetResistance(SPELL_SCHOOL_FROST) >
                            100 &&
                        pInstance) {
    ...
    class spell_sapphiron_frost_explosion_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            for (auto& target : targets) {
                if (CAST_AI(boss_sapphiron::boss_sapphironAI,
                            caster->ToCreature()->AI())
                        ->IsValidExplosionTarget(target)) {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sapphiron_frost_explosion_SpellScript::FilterTargets,
                EFFECT_0,
