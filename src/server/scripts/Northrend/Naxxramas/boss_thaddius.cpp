...
public:
    ...
    struct boss_thaddiusAI : public BossAI {
        explicit boss_thaddiusAI(Creature* c)
            : BossAI(c, BOSS_THADDIUS), summons(me), ballLightningEnabled(false)
        {
        ...
        void Reset() override
        {
            ...
            me->SummonCreature(
                NPC_STALAGG, 3450.45f, -2931.42f, 312.091f, 5.49779f);
            me->SummonCreature(
                NPC_FEUGEN, 3508.14f, -2988.65f, 312.092f, 2.37365f);
            if (Creature* cr = me->SummonCreature(
                    NPC_TESLA_COIL, 3527.34f, -2951.56f, 318.75f, 0.0f)) {
                cr->RemoveAllAuras();
            ...
            if (Creature* cr = me->SummonCreature(
                    NPC_TESLA_COIL, 3487.04f, -2911.68f, 318.75f, 0.0f)) {
                cr->RemoveAllAuras();
                cr->InterruptNonMeleeSpells(true);
            ...
            if (GameObject* go =
                    me->FindNearestGameObject(GO_TESLA_COIL_LEFT, 100.0f)) {
                go->SetGoState(GO_STATE_ACTIVE);
            }
            if (GameObject* go =
                    me->FindNearestGameObject(GO_TESLA_COIL_RIGHT, 100.0f)) {
                go->SetGoState(GO_STATE_ACTIVE);
            }
            if (pInstance) {
                if (GameObject* go = me->GetMap()->GetGameObject(
                        pInstance->GetGuidData(DATA_THADDIUS_GATE))) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_THADDIUS_CHAIN_LIGHTNING:
                me->CastSpell(me->GetVictim(),
                              RAID_MODE(SPELL_CHAIN_LIGHTNING_10,
...
public:
    ...
    struct boss_thaddius_summonAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            me->SetControlled(false, UNIT_STATE_STUNNED);
            if (Creature* cr =
                    me->FindNearestCreature(NPC_TESLA_COIL, 150.0f)) {
                cr->CastSpell(cr,
                              me->GetEntry() == NPC_STALAGG
                                  ? SPELL_STALAGG_CHAIN
                                  : SPELL_FEUGEN_CHAIN,
                              false);
                cr->SetImmuneToPC(false);
        ...
        void JustEngagedWith(Unit* pWho) override
        {
            me->SetInCombatWithZone();
            if (Creature* cr =
                    me->FindNearestCreature(NPC_TESLA_COIL, 150.f, true)) {
                myCoil = cr->GetGUID();
            }
            ...
            if (me->GetEntry() ==
                NPC_STALAGG) // This event needs synchronisation, called for
                             // stalagg only
            {
            ...
            if (pInstance) {
                ...
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(DATA_THADDIUS_BOSS))) {
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_MAGNETIC_PULL) {
                ...
                if (!me->IsAlive()) {
                    me->Respawn();
                    me->SetInCombatWithZone();
                    Talk(me->GetEntry() == NPC_STALAGG ? EMOTE_STAL_REVIVE
                                                       : EMOTE_FEUG_REVIVE);
                }
                else {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            Talk(me->GetEntry() == NPC_STALAGG ? SAY_STAL_DEATH
                                               : SAY_FEUG_DEATH);
            Talk(me->GetEntry() == NPC_STALAGG ? EMOTE_STAL_DEATH
                                               : EMOTE_FEUG_DEATH);
            if (pInstance) {
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(DATA_THADDIUS_BOSS))) {
        ...
        void KilledUnit(Unit* who) override
        {
            ...
            if (!urand(0, 2)) {
                Talk(me->GetEntry() == NPC_STALAGG ? SAY_STAL_SLAY
                                                   : SAY_FEUG_SLAY);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_MINION_POWER_SURGE:
                me->CastSpell(
                    me,
                    RAID_MODE(SPELL_POWER_SURGE_10, SPELL_POWER_SURGE_25),
                    false);
                events.Repeat(19s);
                break;
            case EVENT_MINION_STATIC_FIELD:
                me->CastSpell(
                    me,
                    RAID_MODE(SPELL_STATIC_FIELD_10, SPELL_STATIC_FIELD_25),
                    false);
                events.Repeat(3s);
                break;
            case EVENT_MINION_MAGNETIC_PULL:
                events.Repeat(20s);
                if (pInstance) {
                    if (Creature* feugen = ObjectAccessor::GetCreature(
                            *me, pInstance->GetGuidData(DATA_FEUGEN_BOSS))) {
                        if (!feugen->IsAlive() || !feugen->GetVictim() ||
                            !me->GetVictim())
                            return;

                        float threatFeugen = feugen->GetThreatMgr().GetThreat(
                            feugen->GetVictim());
                        float threatStalagg =
                        ...
                        feugen->GetThreatMgr().ModifyThreatByPercent(tankFeugen,
                                                                     -100);
                        feugen->AddThreat(tankStalagg, threatFeugen);
                        feugen->CastSpell(
                            tankStalagg, SPELL_MAGNETIC_PULL, true);
                        feugen->AI()->DoAction(ACTION_MAGNETIC_PULL);

                        me->GetThreatMgr().ModifyThreatByPercent(tankStalagg,
                                                                 -100);
            ...
            case EVENT_MINION_CHECK_DISTANCE:
                if (Creature* cr = ObjectAccessor::GetCreature(*me, myCoil)) {
                    if (!me->GetHomePosition().IsInDist(me, 28) &&
                        me->IsInCombat()) {
                        if (!overload) {
                            overload = true;
                            cr->AI()->Talk(EMOTE_TESLA_LINK_BREAKS);
                            me->RemoveAurasDueToSpell(me->GetEntry() ==
                                                              NPC_STALAGG
                                                          ? SPELL_STALAGG_CHAIN
                                                          : SPELL_FEUGEN_CHAIN);
                            cr->InterruptNonMeleeSpells(true);
                        ...
                        if (Unit* target = SelectTarget(
                                SelectTargetMethod::Random, 0, 1000.f, true)) {
...
public:
    spell_thaddius_pos_neg_charge()
        : SpellScriptLoader("spell_thaddius_pos_neg_charge")
    {
    }
    ...
    class spell_thaddius_pos_neg_charge_SpellScript : public SpellScript {
        ...
        void HandleTargets(std::list<WorldObject*>& targets)
        {
            ...
            if (count) {
                uint32 spellId = GetSpellInfo()->Id == SPELL_POSITIVE_CHARGE
                                     ? SPELL_POSITIVE_CHARGE_STACK
                                     : SPELL_NEGATIVE_CHARGE_STACK;
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_thaddius_pos_neg_charge_SpellScript::HandleDamage,
                EFFECT_0,
                SPELL_EFFECT_SCHOOL_DAMAGE);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_thaddius_pos_neg_charge_SpellScript::HandleTargets,
                EFFECT_0,
...
public:
    spell_thaddius_polarity_shift()
        : SpellScriptLoader("spell_thaddius_polarity_shift")
    {
    }
    ...
    class spell_thaddius_polarity_shift_SpellScript : public SpellScript {
        ...
        bool Validate(SpellInfo const* /*spell*/) override
        {
            return ValidateSpellInfo(
                {SPELL_POSITIVE_POLARITY, SPELL_NEGATIVE_POLARITY});
        }
        ...
        void HandleDummy(SpellEffIndex /* effIndex */)
        {
            ...
            if (Unit* target = GetHitUnit()) {
                ...
                target->CastSpell(target,
                                  roll_chance_i(50) ? SPELL_POSITIVE_POLARITY
                                                    : SPELL_NEGATIVE_POLARITY,
                                  true,
                                  nullptr,
                                  nullptr,
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_thaddius_polarity_shift_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
            AfterCast += SpellCastFn(
                spell_thaddius_polarity_shift_SpellScript::HandleAfterCast);
        }
    };
...
public:
    ...
    public:
        ...
        void EnterEvadeMode(EvadeReason /*why*/) override {
        } // never stop casting due to evade
        void UpdateAI(uint32 /*diff*/) override {
        } // never do anything unless told
        void JustEngagedWith(Unit* /*who*/) override {}
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        ...
        if (!instance || instance->GetData(DATA_HAD_THADDIUS_GREET) ||
            instance->GetBossState(BOSS_THADDIUS) == DONE)
            return true;

        if (Creature* thaddius = ObjectAccessor::GetCreature(
                *player, instance->GetGuidData(DATA_THADDIUS_BOSS))) {
