...
enum Misc { NPC_WEB_WRAP = 16486, NPC_MAEXXNA_SPIDERLING = 17055 };

const Position PosWrap[3] = {{3546.796f, -3869.082f, 296.450f, 0.0f},
...
public:
    ...
    struct boss_maexxnaAI : public BossAI {
        explicit boss_maexxnaAI(Creature* c)
            : BossAI(c, BOSS_MAEXXNA), summons(me)
        {
        ...
        void Reset() override
        {
            ...
            if (pInstance) {
                if (GameObject* go = me->GetMap()->GetGameObject(
                        pInstance->GetGuidData(DATA_MAEXXNA_GATE))) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_POISON_SHOCK:
                me->CastSpell(
                    me->GetVictim(),
                    RAID_MODE(SPELL_POISON_SHOCK_10, SPELL_POISON_SHOCK_25),
                    false);
                events.Repeat(10s);
                break;
            ...
            case EVENT_WEB_WRAP:
                ...
                for (uint8 i = 0; i < RAID_MODE(1, 2); ++i) {
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random,
                        ...
                        uint8 pos = urand(0, 2);
                        if (Creature* wrap =
                                me->SummonCreature(NPC_WEB_WRAP,
                                                   PosWrap[pos].GetPositionX(),
                                                   PosWrap[pos].GetPositionY(),
                                                   PosWrap[pos].GetPositionZ(),
                                                   0.0f,
...
public:
    ...
    struct boss_maexxna_webwrapAI : public NullCreatureAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (me->m_spells[0] && victimGUID) {
                if (Unit* victim = ObjectAccessor::GetUnit(*me, victimGUID)) {
                    victim->RemoveAurasDueToSpell(me->m_spells[0],
                                                  me->GetGUID());
                }
            }
        }
