...
public:
    ...
    struct boss_grobbulusAI : public BossAI {
        explicit boss_grobbulusAI(Creature* c)
            : BossAI(c, BOSS_GROBBULUS), summons(me)
        {
        ...
        void PullChamberAdds()
        {
            ...
            me->GetCreaturesWithEntryInRange(
                StichedGiants, 300.0f, NPC_STICHED_GIANT);
            for (std::list<Creature*>::const_iterator itr =
                     StichedGiants.begin();
                 itr != StichedGiants.end();
                 ++itr) {
        ...
        {
            if (spellInfo->Id ==
                    RAID_MODE(SPELL_SLIME_SPRAY_10, SPELL_SLIME_SPRAY_25) &&
                target->GetTypeId() == TYPEID_PLAYER) {
                me->SummonCreature(NPC_FALLOUT_SLIME,
                                   target->GetPositionX(),
                                   target->GetPositionY(),
                                   target->GetPositionZ());
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!me->IsInCombat() && dropSludgeTimer >= 5000) {
                if (me->IsWithinDist3d(3178, -3305, 319, 5.0f) &&
                    !summons.HasEntry(NPC_SEWAGE_SLIME)) {
                    me->CastSpell(3128.96f + irand(-20, 20),
                                  -3312.96f + irand(-20, 20),
                                  293.25f,
                                  SPELL_BOMBARD_SLIME,
                                  false);
                }
...
public:
    ...
    struct boss_grobbulus_poison_cloudAI : public NullCreatureAI {
        explicit boss_grobbulus_poison_cloudAI(Creature* pCreature)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            sizeTimer += diff; // increase size to 15yd in 60 seconds, 0.00025
                               // is the growth of size in 1ms
            me->SetFloatValue(UNIT_FIELD_COMBATREACH,
                              2.0f + (0.00025f * sizeTimer));
        }
    };
};
    ...
    class spell_grobbulus_poison_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_grobbulus_poison_SpellScript::FilterTargets,
                EFFECT_0,
    ...
    class spell_grobbulus_mutating_injection_AuraScript : public AuraScript {
                          ...
                          AuraEffectHandleModes /*mode*/)
        {
            ...
            case AURA_REMOVE_BY_EXPIRE:
                if (auto caster = GetCaster()) {
                    caster->CastSpell(
                        GetTarget(), SPELL_MUTATING_EXPLOSION, true);
                }
                break;
            default:
                return;
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_grobbulus_mutating_injection_AuraScript::HandleRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
