...
// MARKS
const uint32 TABLE_SPELL_MARK[4] = {SPELL_MARK_OF_ZELIEK,
                                    SPELL_MARK_OF_BLAUMEUX,
                                    SPELL_MARK_OF_RIVENDARE,
                                    SPELL_MARK_OF_KORTHAZZ};

// PRIMARY SPELL
const uint32 TABLE_SPELL_PRIMARY_10[4] = {SPELL_ZELIEK_HOLY_BOLT_10,
                                          SPELL_BLAUMEUX_SHADOW_BOLT_10,
                                          SPELL_RIVENDARE_UNHOLY_SHADOW_10,
                                          SPELL_KORTHAZZ_METEOR_10};
const uint32 TABLE_SPELL_PRIMARY_25[4] = {SPELL_ZELIEK_HOLY_BOLT_25,
...
const uint32 TABLE_SPELL_PUNISH[4] = {
    SPELL_ZELIEK_CONDEMNATION, SPELL_BLAUMEUX_UNYIELDING_PAIN, 0, 0};

// SECONDARY SPELL
const uint32 TABLE_SPELL_SECONDARY_10[4] = {
    SPELL_ZELIEK_HOLY_WRATH_10, SPELL_BLAUMEUX_VOID_ZONE_10, 0, 0};
const uint32 TABLE_SPELL_SECONDARY_25[4] = {
    SPELL_ZELIEK_HOLY_WRATH_25, SPELL_BLAUMEUX_VOID_ZONE_25, 0, 0};
    ...
    struct boss_four_horsemenAI : public BossAI {
        ...
        void MoveToCorner()
        {
            ...
            me->GetMotionMaster()->MovePoint(
                currentWaypoint, WaypointPositions[currentWaypoint]);
        }
        ...
        void Reset() override
        {
            ...
            if ((me->GetEntry() != NPC_LADY_BLAUMEUX &&
                 me->GetEntry() != NPC_SIR_ZELIEK)) {
            ...
            if (pInstance) {
                if (GameObject* go = me->GetMap()->GetGameObject(
                        pInstance->GetGuidData(DATA_HORSEMEN_GATE))) {
                ...
                if (pInstance->GetBossState(BOSS_HORSEMAN) == DONE) {
                    if (!me->GetMap()->GetPlayers().IsEmpty()) {
                        if (Player* player = me->GetMap()
                                                 ...
                                                 .getFirst()
                                                 ->GetSource()) {
                            if (GameObject* chest = player->SummonGameObject(
                                    RAID_MODE(GO_HORSEMEN_CHEST_10,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_MARK_CAST:
                ...
                events.Repeat((me->GetEntry() == NPC_LADY_BLAUMEUX ||
                               me->GetEntry() == NPC_SIR_ZELIEK)
                                  ? 15s
                                  : 12s);
                return;
            ...
            case EVENT_PRIMARY_SPELL:
                ...
                me->CastSpell(me->GetVictim(),
                              RAID_MODE(TABLE_SPELL_PRIMARY_10[horsemanId],
                                        TABLE_SPELL_PRIMARY_25[horsemanId]),
                              false);
                events.Repeat(15s);
                return;
            case EVENT_SECONDARY_SPELL:
                me->CastSpell(me->GetVictim(),
                              RAID_MODE(TABLE_SPELL_SECONDARY_10[horsemanId],
                                        TABLE_SPELL_SECONDARY_25[horsemanId]),
                              false);
                events.Repeat(15s);
                return;
            }
            ...
            if (me->GetEntry() == NPC_LADY_BLAUMEUX ||
                ...
                if (me->IsWithinDistInMap(me->GetVictim(), 45.0f) &&
                    me->IsValidAttackTarget(me->GetVictim())) {
                    DoCastVictim(RAID_MODE(TABLE_SPELL_PRIMARY_10[horsemanId],
                                           TABLE_SPELL_PRIMARY_25[horsemanId]));
                }
                else if (!me->IsWithinDistInMap(me->GetVictim(), 45.0f) ||
                         !me->IsValidAttackTarget(me->GetVictim())) {
    ...
    class spell_four_horsemen_mark_AuraScript : public AuraScript {
                     ...
                     AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster()) {
                ...
                if (damage) {
                    caster->CastCustomSpell(SPELL_MARK_DAMAGE,
        ...
        {
            AfterEffectApply +=
                AuraEffectApplyFn(spell_four_horsemen_mark_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_DUMMY,
...
class spell_four_horsemen_consumption : public SpellScript {
    ...
    {
        uint32 damage =
            GetCaster()->GetMap()->ToInstanceMap()->GetDifficulty() ==
                    REGULAR_DIFFICULTY
                ? 2750
                : 4250;
        SetHitDamage(damage);
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_four_horsemen_consumption::HandleDamageCalc,
                          EFFECT_0,
