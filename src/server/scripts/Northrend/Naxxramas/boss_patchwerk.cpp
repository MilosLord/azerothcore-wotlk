...
enum Misc { ACHIEV_TIMED_START_EVENT = 10286 };
    ...
    struct boss_patchwerkAI : public BossAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_HATEFUL_STRIKE: {
                // Cast Hateful strike on the player with the highest amount of
                // HP within melee distance, and second threat amount
                std::list<Unit*> meleeRangeTargets;
                Unit*            finalTarget = nullptr;
                uint8            counter     = 0;
                auto             i = me->GetThreatMgr().GetThreatList().begin();
                for (; i != me->GetThreatMgr().GetThreatList().end();
                     ++i, ++counter) {
                ...
                for (itr = meleeRangeTargets.begin();
                     itr != meleeRangeTargets.end();
                     ++itr, ++counter) {
                    ...
                    {
                        if (!finalTarget ||
                            (*itr)->GetHealth() > finalTarget->GetHealth()) {
                            finalTarget = (*itr);
                        }
                        // third loop
                        if (counter >= 2)
                ...
                if (finalTarget) {
                    me->CastSpell(finalTarget,
                                  RAID_MODE(SPELL_HATEFUL_STRIKE_10,
                                            SPELL_HATEFUL_STRIKE_25),
                                  false);
                }
                events.Repeat(1s);
                break;
...
void AddSC_boss_patchwerk() { new boss_patchwerk(); }
