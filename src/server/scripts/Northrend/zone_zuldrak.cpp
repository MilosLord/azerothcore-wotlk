...
const uint32 AA_ITEM_ENTRY[24] = {38336, 39669, 38342, 38340, 38344, 38369,
                                  38396, 38398, 38338, 38386, 38341, 38384,
                                  38397, 38381, 38337, 38393, 38339, 39668,
                                  39670, 38346, 38379, 38345, 38343, 38370};
const uint32 AA_AURA_ID[24]    = {51095, 53153, 51100, 51087, 51091, 51081,
                                  51072, 51079, 51018, 51067, 51055, 51064,
                                  51077, 51062, 51057, 51069, 51059, 53150,
                                  53158, 51093, 51097, 51102, 51083, 51085};
const char*  AA_ITEM_NAME[24]  = {"Crystallized Hogsnot",
                                  "Ghoul Drool",
                                  "Trollbane",
                                  "Amberseed",
...
public:
    ...
    struct npc_finklesteinAI : public ScriptedAI {
        ...
        {
            std::map<ObjectGuid, uint32>::const_iterator itr =
                questList.find(guid);
        ...
        void RightClickCauldron(ObjectGuid guid)
        {
            ...
            if (player) {
                ...
                if (player->HasAura(auraId)) {
                    ...
                    player->SetQuestStatus(QUEST_ALCHEMIST_APPRENTICE,
                                           QUEST_STATUS_FAILED);
                }
            }
            questList.erase(itr);
        ...
        void StartNextTask(ObjectGuid playerGUID, uint32 counter)
        {
            ...
            case 5:
                me->TextEmote(
                    "Before it thickens, we must add...", player, true);
                me->TextEmote(itemName, player, true);
                me->CastSpell(player, auraId, true);
                break;
            case 6:
                me->TextEmote(
                    "It's thickening! Quickly get me some...", player, true);
                me->TextEmote(itemName, player, true);
                me->CastSpell(player, auraId, true);
                break;
            }
                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_ALCHEMIST_APPRENTICE)
            if (creature->AI() &&
                CAST_AI(npc_finklestein::npc_finklesteinAI, creature->AI()))
                CAST_AI(npc_finklestein::npc_finklesteinAI, creature->AI())
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_ALCHEMIST_APPRENTICE) ==
            QUEST_STATUS_INCOMPLETE) {
            if (creature->AI() &&
                CAST_AI(npc_finklestein::npc_finklesteinAI, creature->AI()))
                if (!CAST_AI(npc_finklestein::npc_finklesteinAI, creature->AI())
                         ->IsPlayerOnTask(player->GetGUID()))
                    AddGossipItemFor(player,
                                     GOSSIP_ICON_CHAT,
                                     "I'm ready to begin. What is the first "
...
public:
    ...
    {
        Creature* finklestein =
            go->FindNearestCreature(NPC_FINKLESTEIN, 30.0f, true);
        if (finklestein && finklestein->AI())
            CAST_AI(npc_finklestein::npc_finklesteinAI, finklestein->AI())
                ->RightClickCauldron(player->GetGUID());
    ...
    struct npc_feedin_da_goolzAI : public NullCreatureAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (findTimer) {
                findTimer += diff;
                if (findTimer >= 1000) {
                    if (Creature* ghoul = me->FindNearestCreature(
                            NPC_DECAYING_GHOUL, 30.0f, true)) {
                        ghoul->SetReactState(REACT_DEFENSIVE);
                        float o = me->GetAngle(ghoul);
                        ghoul->GetMotionMaster()->MovePoint(
                            1,
            ...
            if (checkTimer) {
                ...
                if (checkTimer >= 1500) {
                    ...
                    if (!ghoulFed) {
                        ...
                        if (GameObject* bowl =
                                me->FindNearestGameObject(GO_BOWL, 10.0f))
                            bowl->Delete();

                        if (Creature* ghoul =
                                ObjectAccessor::GetCreature(*me, ghoulFed)) {
...
public:
    npc_overlord_drakuru_betrayal()
        : CreatureScript("npc_overlord_drakuru_betrayal")
    {
    }
    ...
    struct npc_overlord_drakuru_betrayalAI : public ScriptedAI {
        npc_overlord_drakuru_betrayalAI(Creature* creature)
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (who->GetTypeId() == TYPEID_PLAYER) {
                if (playerGUID) {
                    if (who->GetGUID() != playerGUID) {
                        ...
                        if (player && player->IsWithinDistInMap(me, 80))
                            who->ToPlayer()->NearTeleportTo(
                                6143.76f, -1969.7f, 417.57f, 2.08f);
                        else {
                ...
                else if (who->ToPlayer()->GetQuestStatus(QUEST_BETRAYAL) ==
                             QUEST_STATUS_INCOMPLETE &&
        ...
        void JustSummoned(Creature* cr) override
        {
            ...
            if (cr->GetEntry() == NPC_BLIGHTBLOOD_TROLL)
                ...
                cr->GetMotionMaster()->MovePoint(
                    0,
                    me->GetPositionX() + cos(o) * 6.0f,
                    me->GetPositionY() + std::sin(o) * 6.0f,
                    me->GetPositionZ());
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_BETRAYAL_2:
                me->SummonCreature(
                    NPC_BLIGHTBLOOD_TROLL, 6184.1f, -1969.9f, 586.76f, 4.5f);
                me->SummonCreature(
                    NPC_BLIGHTBLOOD_TROLL, 6222.9f, -2026.5f, 586.76f, 2.9f);
                me->SummonCreature(
                    NPC_BLIGHTBLOOD_TROLL, 6166.2f, -2065.4f, 586.76f, 1.4f);
                me->SummonCreature(
                    NPC_BLIGHTBLOOD_TROLL, 6127.5f, -2008.7f, 586.76f, 0.0f);
            ...
            case EVENT_BETRAYAL_3:
                ...
                if (Player* player = ObjectAccessor::GetPlayer(*me, playerGUID))
                    player->CastSpell(
                        player, SPELL_SCOURGE_DISGUISE_EXPIRING, true);
            ...
            case EVENT_BETRAYAL_6:
                me->SummonCreature(NPC_LICH_KING,
                                   6142.9f,
                                   -2011.6f,
                                   590.86f,
                                   0.0f,
            ...
            case EVENT_BETRAYAL_9:
                if (Creature* lich =
                        ObjectAccessor::GetCreature(*me, lichGUID)) {
            ...
            case EVENT_BETRAYAL_13:
                if (Creature* lich =
                    ...
                    lich->GetMotionMaster()->MovePoint(
                        0, 6143.8f, -2011.5f, 590.9f);
                }
            ...
            if (me->GetFaction() == FACTION_FRIENDLY ||
                me->HasUnitState(UNIT_STATE_CASTING | UNIT_STATE_STUNNED))
                return;
...
public:
    ...
    struct npc_drakuru_shacklesAI : public NullCreatureAI {
        ...
        void Reset() override { me->SetUnitFlag(UNIT_FLAG_NOT_SELECTABLE); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (timer >= 2000) {
                ...
                if (Creature* cr =
                        me->FindNearestCreature(NPC_RAGECLAW, 10.0f)) {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_UNLOCK_SHACKLE) {
                if (caster->ToPlayer()->GetQuestStatus(
                    ...
                    QUEST_STATUS_INCOMPLETE) {
                    if (Creature* rageclaw =
                            ObjectAccessor::GetCreature(*me, _rageclawGUID)) {
                        UnlockRageclaw(caster, rageclaw);
                        caster->ToPlayer()->KilledMonster(
                            rageclaw->GetCreatureTemplate(), _rageclawGUID);
...
public:
    ...
    struct npc_captured_rageclawAI : public NullCreatureAI {
        ...
        void Reset() override
        {
            ...
            DoCast(me,
                   SPELL_KNEEL,
                   true); // Little Hack for kneel - Thanks Illy :P
        }
...
public:
    npc_released_offspring_harkoa()
        : CreatureScript("npc_released_offspring_harkoa")
    {
    }

    struct npc_released_offspring_harkoaAI : public ScriptedAI {
        npc_released_offspring_harkoaAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    ...
    {
        go->UseDoorOrButton();
        if (player->GetQuestStatus(QUEST_OUR_ONLY_HOPE) ==
            QUEST_STATUS_INCOMPLETE) {
            Creature* gymerDummy =
                go->FindNearestCreature(NPC_GYMER_DUMMY, 20.0f);
            if (gymerDummy) {
                player->KilledMonsterCredit(gymerDummy->GetEntry(),
                                            gymerDummy->GetGUID());
                gymerDummy->CastSpell(
                    gymerDummy, SPELL_GYMER_LOCK_EXPLOSION, true);
...
public:
    ...
    struct npc_storm_cloudAI : public ScriptedAI {
        ...
        void Reset() override { me->CastSpell(me, STORM_VISUAL, true); }
