...
public:
    ...
    struct npc_isle_of_conquest_turretAI : public VehicleAI {
        ...
        {
            if (spellInfo->Id == SPELL_REPAIR_TURRET_DUMMY &&
                me->GetEntry() == NPC_BROKEN_KEEP_CANNON) {
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_DESPAWN:
                    if (me->GetMap()->ToBattlegroundMap())
                        if (Battleground* bgIoC =
                                me->GetMap()->ToBattlegroundMap()->GetBG())
    ...
    struct boss_isle_of_conquestAI : public ScriptedAI {
        ...
        void CheckRageBuff()
        {
            if (!rage) {
                ...
                if (me->GetDistance(me->GetHomePosition()) < 40.0f &&
                    std::abs(me->GetPositionZ() -
                             me->GetHomePosition().GetPositionZ()) < 5.0f) {
                    rage = false;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_BRUTAL_STRIKE:
                me->CastSpell(
                    me->GetVictim(), SPELL_IOCBOSS_BRUTAL_STRIKE, false);
                events.Repeat(6s);
                break;
    ...
    class spell_ioc_repair_turret_AuraScript : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            if (GetTargetApplication()->GetRemoveMode() ==
                ...
                GetTarget()->CastSpell(
                    GetTarget(), SPELL_REPAIR_TURRET_DUMMY, true);
        }

        void Register() override
        {
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_ioc_repair_turret_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_DUMMY,
...
public:
    spell_ioc_bomb_blast_criteria()
        : SpellScriptLoader("spell_ioc_bomb_blast_criteria")
    {
    }
    ...
    class spell_ioc_bomb_blast_criteria_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_ioc_bomb_blast_criteria_SpellScript::
                                  HandleGameObjectDamage,
                              EFFECT_1,
...
public:
    ...
    class spell_ioc_gunship_portal_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
             ...
             *
             * HACK: GetWorldLocation() returns real position and not
            transportposition.
            ...
            caster->TeleportTo(GetHitCreature()->GetWorldLocation(),
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_ioc_gunship_portal_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
            OnEffectHitTarget += SpellEffectFn(
                spell_ioc_gunship_portal_SpellScript::HandleScript2,
                EFFECT_1,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
    ...
    class spell_ioc_parachute_ic_AuraScript : public AuraScript {
            ...
            void HandleTriggerSpell(AuraEffect const* /*aurEff*/)
        {
            if (Player* target = GetTarget()->ToPlayer())
                if (target->m_movementInfo.fallTime > 2500 &&
                    !target->GetTransport())
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_ioc_parachute_ic_AuraScript::HandleTriggerSpell,
                EFFECT_0,
    ...
    class spell_ioc_launch_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            if (Player* player = GetHitPlayer())
                player->AddAura(SPELL_LAUNCH_NO_FALLING_DAMAGE,
                                player); // prevents falling damage
        }
        ...
        void Launch()
        {
            ...
            if (Player* player = GetHitPlayer()) {
                ...
                player->GetMap()->PlayerRelocation(
                    player,
                ...
                float dist = position->GetExactDist2d(player->GetPositionX(),
                                                      player->GetPositionY());
                float elevation = GetSpell()->m_targets.GetElevation();
                float speedZ =
                    std::max(10.0f, float(50.0f * std::sin(elevation)));
                float speedXY = dist * 10.0f / speedZ;

                player->GetMotionMaster()->MoveJump(position->GetPositionX(),
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_ioc_launch_SpellScript::HandleScript,
                              EFFECT_1,
