...
public:
    ...
    struct boss_gal_darahAI : public BossAI {
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            if (Unit* target =
                    SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true)) {
                summon->CastSpell(target, SPELL_STAMPEDE_DMG, true);
                despawnTime =
                    (summon->GetDistance(target) / 40.0f * 1000) + 500;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_IMPALING_CHARGE:
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 1, 100.0f, true)) {
...
public:
    spell_galdarah_impaling_charge()
        : SpellScriptLoader("spell_galdarah_impaling_charge")
    {
    }
    ...
    class spell_galdarah_impaling_charge_SpellScript : public SpellScript {
        ...
        void HandleApplyAura(SpellEffIndex /*effIndex*/)
        {
            if (Unit* target = GetHitUnit())
                target->CastSpell(
                    GetCaster(), SPELL_IMPALING_CHARGE_VEHICLE, true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_galdarah_impaling_charge_SpellScript::HandleApplyAura,
                EFFECT_1,
...
public:
    ...
    class spell_galdarah_transform_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_galdarah_transform_SpellScript::HandleScriptEffect,
                EFFECT_0,
...
public:
    achievement_share_the_love()
        : AchievementCriteriaScript("achievement_share_the_love")
    {
    }
