    ...
    struct instance_gundrak_InstanceMapScript : public InstanceScript {
        ...
        void OnGameObjectCreate(GameObject* gameobject) override
        {
            ...
            case GO_ALTAR_OF_SLAD_RAN:
                ...
                gameobject->SetGoState(GetBossState(DATA_SLAD_RAN) == DONE
                                           ? GO_STATE_ACTIVE
                                           : GO_STATE_READY);
                break;
            case GO_ALTAR_OF_DRAKKARI:
                _drakkariAltarGUID = gameobject->GetGUID();
                gameobject->SetGoState(GetBossState(DATA_DRAKKARI_COLOSSUS) ==
                                               DONE
                                           ? GO_STATE_ACTIVE
                                           : GO_STATE_READY);
                break;
            case GO_ALTAR_OF_MOORABI:
                ...
                gameobject->SetGoState(GetBossState(DATA_MOORABI) == DONE
                                           ? GO_STATE_ACTIVE
                                           : GO_STATE_READY);
                break;
            case GO_STATUE_OF_SLAD_RAN:
                ...
                gameobject->SetGoState(
                    _keysInCount == 3 ? GO_STATE_ACTIVE_ALTERNATIVE
                                      : (GetBossState(DATA_SLAD_RAN) == DONE
                                             ? GO_STATE_READY
                                             : GO_STATE_ACTIVE));
                break;
            case GO_STATUE_OF_DRAKKARI:
                ...
                gameobject->SetGoState(
                    _keysInCount == 3
                        ...
                        : (GetBossState(DATA_DRAKKARI_COLOSSUS) == DONE
                               ? GO_STATE_READY
                               : GO_STATE_ACTIVE));
                break;
            case GO_STATUE_OF_MOORABI:
                _bridgeGUIDs[2] = gameobject->GetGUID();
                gameobject->SetGoState(_keysInCount == 3
                                           ? GO_STATE_ACTIVE_ALTERNATIVE
                                           : (GetBossState(DATA_MOORABI) == DONE
                                                  ? GO_STATE_READY
                                                  : GO_STATE_ACTIVE));
                break;
        ...
        void SetData(uint32 type, uint32) override
        {
            ...
            case NPC_ECK_THE_FEROCIOUS:
                ...
                break;
            case GO_ALTAR_OF_SLAD_RAN:
                if (GameObject* statue =
                        instance->GetGameObject(_bridgeGUIDs[0]))
                    statue->SetGoState(GO_STATE_READY);
                break;
            case GO_ALTAR_OF_DRAKKARI:
                if (GameObject* statue =
                        instance->GetGameObject(_bridgeGUIDs[1]))
                    statue->SetGoState(GO_STATE_READY);
                break;
            case GO_ALTAR_OF_MOORABI:
                if (GameObject* statue =
                        instance->GetGameObject(_bridgeGUIDs[2]))
                    statue->SetGoState(GO_STATE_READY);
                break;
            }
            ...
            if (type >= GO_ALTAR_OF_SLAD_RAN) {
                for (uint8 i = 0; i < 3; ++i)
                    if (GameObject* statue =
                            instance->GetGameObject(_bridgeGUIDs[i]))
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            if (!InstanceScript::SetBossState(type, state)) {
                if (state == DONE &&
                    (type == DATA_SLAD_RAN || type == DATA_MOORABI ||
                     type == DATA_DRAKKARI_COLOSSUS))
                    ++_keysInCount;
                return false;
            }
            ...
            case DATA_SLAD_RAN:
                if (GameObject* altar =
                        instance->GetGameObject(_sladRanAltarGUID))
                    altar->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                break;
            case DATA_MOORABI:
                if (GameObject* altar =
                        instance->GetGameObject(_moorabiAltarGUID))
                    altar->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                break;
            case DATA_DRAKKARI_COLOSSUS:
                if (GameObject* altar =
                        instance->GetGameObject(_drakkariAltarGUID))
                    altar->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                break;
...
void AddSC_instance_gundrak() { new instance_gundrak(); }
