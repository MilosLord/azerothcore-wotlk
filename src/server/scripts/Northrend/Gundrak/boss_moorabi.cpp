...
public:
    ...
    struct boss_moorabiAI : public BossAI {
        ...
        void SpellHitTarget(Unit* /*caster*/,
    ...
    class spell_moorabi_mojo_frenzy_AuraScript : public AuraScript {
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (GetUnitOwner()->GetMap()->IsHeroic())
                GetUnitOwner()->SetFloatValue(
                    UNIT_MOD_CAST_SPEED,
                    1.0f * (GetUnitOwner()->GetHealthPct() *
                            GetUnitOwner()->GetHealthPct() / 10000.0f));
            else
                GetUnitOwner()->SetFloatValue(
                    UNIT_MOD_CAST_SPEED,
                    1.0f * (GetUnitOwner()->GetHealthPct() / 100.0f));
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_moorabi_mojo_frenzy_AuraScript::HandlePeriodic,
                EFFECT_0,
