...
public:
    ...
    struct boss_general_zarithrianAI : public BossAI {
        boss_general_zarithrianAI(Creature* creature)
            : BossAI(creature, DATA_GENERAL_ZARITHRIAN)
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUMMON_ADDS1:
                Talk(SAY_ADDS);
                if (Creature* stalker1 = ObjectAccessor::GetCreature(
                        *me,
                        instance->GetGuidData(DATA_ZARITHRIAN_SPAWN_STALKER_1)))
                    stalker1->CastSpell(
                        stalker1, SPELL_SUMMON_FLAMECALLER, false);
                if (Creature* stalker2 = ObjectAccessor::GetCreature(
                        *me,
                        instance->GetGuidData(DATA_ZARITHRIAN_SPAWN_STALKER_2)))
                    stalker2->CastSpell(
                        stalker2, SPELL_SUMMON_FLAMECALLER, false);
...
public:
    ...
    struct npc_onyx_flamecallerAI : public npc_escortAI {
        npc_onyx_flamecallerAI(Creature* creature)
        ...
        {
            // Let Zarithrian count as summoner. _instance cant be null since we
            // got GetRubySanctumAI
            if (Creature* zarithrian = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(NPC_GENERAL_ZARITHRIAN)))
