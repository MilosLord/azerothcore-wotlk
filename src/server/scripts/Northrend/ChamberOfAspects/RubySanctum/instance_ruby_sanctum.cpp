...
BossBoundaryData const boundaries = {
    {DATA_GENERAL_ZARITHRIAN,
     new EllipseBoundary(Position(3013.409f, 529.492f), 45.0, 100.0)},
    ...
    struct instance_ruby_sanctum_InstanceMapScript : public InstanceScript {
        ...
        void OnPlayerEnter(Player* /*player*/) override
        {
            if (GetBossState(DATA_HALION_INTRO_DONE) != DONE &&
                ...
                if (Creature* halionController =
                        instance->GetCreature(HalionControllerGUID))
        ...
        ObjectGuid GetGuidData(uint32 type) const override
        {
            ...
            case DATA_ZARITHRIAN_SPAWN_STALKER_2:
                return ZarithrianSpawnStalkerGUID
                    [type - DATA_ZARITHRIAN_SPAWN_STALKER_1];
            case NPC_HALION_CONTROLLER:
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            case DATA_BALTHARUS_THE_WARBORN:
                if (GetBossState(DATA_BALTHARUS_THE_WARBORN) == DONE &&
                    GetBossState(DATA_SAVIANA_RAGEFIRE) == DONE)
                    if (Creature* zarithrian =
                            instance->GetCreature(GeneralZarithrianGUID)) {
...
public:
    spell_ruby_sanctum_rallying_shout()
        : SpellScriptLoader("spell_ruby_sanctum_rallying_shout")
    {
    }
    ...
    class spell_ruby_sanctum_rallying_shout_SpellScript : public SpellScript {
        ...
        void CountAllies()
        {
            ...
            if (count > 0)
                GetCaster()->CastCustomSpell(SPELL_RALLY,
        ...
        {
            AfterHit += SpellHitFn(
                spell_ruby_sanctum_rallying_shout_SpellScript::CountAllies);
        }
    };
