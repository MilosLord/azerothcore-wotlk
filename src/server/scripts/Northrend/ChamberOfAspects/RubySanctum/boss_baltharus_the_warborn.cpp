...
public:
    ...
    struct boss_baltharus_the_warbornAI : public BossAI {
        boss_baltharus_the_warbornAI(Creature* creature)
            : BossAI(creature, DATA_BALTHARUS_THE_WARBORN)
        {
            _introDone = false;
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_INTRO_BALTHARUS && !_introDone) {
                _introDone = true;
                me->m_Events.AddEvent(new DelayedTalk(me, SAY_BALTHARUS_INTRO),
                                      me->m_Events.CalculateTime(6000));
            }
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (Creature* xerestrasza = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(NPC_XERESTRASZA)))
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            summon->m_Events.AddEvent(new RestoreFight(summon),
...
public:
    npc_baltharus_the_warborn_clone()
        : CreatureScript("npc_baltharus_the_warborn_clone")
    {
    }
...
public:
    spell_baltharus_enervating_brand_trigger()
        : SpellScriptLoader("spell_baltharus_enervating_brand_trigger")
    {
    }

    class spell_baltharus_enervating_brand_trigger_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void CheckDistance()
        {
            if (Unit* caster = GetOriginalCaster())
                if (Unit* target = GetHitUnit())
                    if (target == GetCaster()
                        // the spell has an unlimited range, so we need this
                        // check
...
public:
    ...
    struct npc_xerestraszaAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (me->GetInstanceScript()->GetBossState(
                    DATA_BALTHARUS_THE_WARBORN) == DONE)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_XERESTRASZA_EVENT_0:
                ...
                me->GetMotionMaster()->MovePoint(
                    0, 3151.236f, 379.8733f, 86.31996f);
                break;
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            if (Creature* xerestrasza = ObjectAccessor::GetCreature(
                    *player, instance->GetGuidData(NPC_XERESTRASZA)))
                xerestrasza->AI()->DoAction(ACTION_INTRO_BALTHARUS);

            if (Creature* baltharus = ObjectAccessor::GetCreature(
                    *player, instance->GetGuidData(NPC_BALTHARUS_THE_WARBORN)))
