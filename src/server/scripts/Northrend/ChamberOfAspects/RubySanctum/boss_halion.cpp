...
enum Texts {
    // Shared
    SAY_REGENERATE =
        0, // Without pressure in both realms, %s begins to regenerate.

    // Halion
    SAY_INTRO =
        1, // Meddlesome insects! You are too late. The Ruby Sanctum is lost!
    SAY_AGGRO =
        2, // Your world teeters on the brink of annihilation. You will ALL bear
           // witness to the coming of a new age of DESTRUCTION!
    SAY_METEOR_STRIKE = 3, // The heavens burn!
    SAY_PHASE_TWO     = 4, // You will find only suffering within the realm of
                           // twilight! Enter if you dare!
    SAY_DEATH = 5,   // Relish this victory, mortals, for it will be your last!
                     // This world will burn with the master's return!
    SAY_KILL    = 6, // Another "hero" falls.
    SAY_BERSERK = 7, // Not good enough.
    EMOTE_CORPOREALITY_POT =
        8, // Your efforts force %s further out of the physical realm!
    EMOTE_CORPOREALITY_PIP =
        9, // Your companions' efforts force %s further into the physical realm!

    // Twilight Halion
    SAY_SPHERE_PULSE = 1, // Beware the shadow!
    SAY_PHASE_THREE  = 2, // I am the light and the darkness! Cower, mortals,
                          // before the herald of Deathwing!
    EMOTE_CORPOREALITY_TIT =
        3, // Your companions' efforts force %s further into the twilight realm!
    EMOTE_CORPOREALITY_TOT =
        4, // Your efforts force %s further out of the twilight realm!
...
uint32 const _corporealityReference[MAX_CORPOREALITY_STATE] = {74836,
                                                               74835,
                                                               74834,
                                                               74833,
                                                               74832,
    ...
    struct boss_halionAI : public BossAI {
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            case NPC_METEOR_STRIKE_FLAME:
                if (Is25ManRaid() && IsHeroic() && roll_chance_i(90) &&
                    summons.GetEntryCount(NPC_LIVING_EMBER) <
                        _livingEmberCount + 12)
                    if (Creature* ember =
                            me->SummonCreature(NPC_LIVING_EMBER,
                                               *summon,
            ...
            case NPC_METEOR_STRIKE_EAST:
                summon->CastSpell(
                    summon, SPELL_METEOR_STRIKE_FIRE_AURA_2, true);
        ...
        bool IsAnyPlayerValid()
        {
            ...
            for (Map::PlayerList::const_iterator itr = playerList.begin();
                 itr != playerList.end();
                 ++itr)
                if (Player* player = itr->GetSource())
                    if (!player->IsGameMaster() && player->IsAlive() &&
                        me->GetHomePosition().GetExactDist2d(player) < 52.0f &&
                        me->IsWithinLOSInMap(player) &&
                        !player->HasAuraType(SPELL_AURA_MOD_INVISIBILITY) &&
                        !player->HasAuraType(SPELL_AURA_MOD_STEALTH) &&
                        !player->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE))
                        return true;
            return false;
        }
        ...
        void EnterEvadeMode(EvadeReason why) override
        {
            ...
            if (Creature* controller = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(NPC_HALION_CONTROLLER)))
        ...
        void KilledUnit(Unit* victim) override
        {
            if (victim->GetTypeId() == TYPEID_PLAYER &&
                events.GetNextEventTime(EVENT_KILL_TALK) == 0) {
                Talk(SAY_KILL);
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (Creature* twilightHalion = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(NPC_TWILIGHT_HALION)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_HALION_VISIBILITY:
                me->SetVisible(instance->GetBossState(DATA_HALION_INTRO_DONE) ==
                               DONE);
                me->SetReactState(
                    instance->GetBossState(DATA_HALION_INTRO_DONE) == DONE
                        ? REACT_AGGRESSIVE
                        : REACT_PASSIVE);
                break;
            case EVENT_TRIGGER_BERSERK:
                ...
                if (Creature* twilightHalion = ObjectAccessor::GetCreature(
                    ...
                    twilightHalion->CastSpell(
                        twilightHalion, SPELL_BERSERK, true);
                break;
            }
            ...
            case EVENT_ACTIVATE_FIREWALL:
                instance->HandleGameObject(
                    instance->GetGuidData(GO_FLAME_RING), false, nullptr);
                instance->HandleGameObject(
                    instance->GetGuidData(GO_TWILIGHT_FLAME_RING),
                    false,
                    nullptr);
                break;
            case EVENT_METEOR_STRIKE:
                _livingEmberCount = summons.GetEntryCount(NPC_LIVING_EMBER);
                me->CastCustomSpell(SPELL_METEOR_STRIKE_TARGETING,
...
public:
    ...
    struct boss_twilight_halionAI : public ScriptedAI {
        ...
        {
            Creature* halion = ObjectAccessor::GetCreature(
                *me, _instance->GetGuidData(NPC_HALION));
            if (!halion)
                return;
        ...
        {
            if (victim->GetTypeId() == TYPEID_PLAYER &&
                _events.GetNextEventTime(EVENT_KILL_TALK) == 0) {
                Talk(SAY_KILL);
        ...
        void JustDied(Unit* killer) override
        {
            if (Creature* halion = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(NPC_HALION))) {
                // Ensure looting
            ...
            if (Creature* controller = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(NPC_HALION_CONTROLLER)))
                if (controller->IsAlive())
                    Unit::Kill(controller, controller);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SHADOW_PULSARS_SHOOT:
                if (Creature* orbCarrier = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(NPC_ORB_CARRIER)))
...
public:
    ...
    private:
        void UpdateCorporeality()
        {
            ...
            _instance->DoUpdateWorldState(WORLDSTATE_CORPOREALITY_MATERIAL,
                                          _corporeality * 10);
            _instance->DoUpdateWorldState(WORLDSTATE_CORPOREALITY_TWILIGHT,
                                          100 - _corporeality * 10);

            if (Creature* twilightHalion = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(NPC_TWILIGHT_HALION))) {
                twilightHalion->RemoveAurasDueToSpell(
                    _corporealityReference[MAX_CORPOREALITY_STATE - 1 -
                                           oldValue]);
                twilightHalion->CastSpell(
                    twilightHalion,
                    _corporealityReference[MAX_CORPOREALITY_STATE - 1 -
                                           _corporeality],
                    true);
                twilightHalion->AI()->Talk(oldValue < _corporeality
                                               ? EMOTE_CORPOREALITY_TOT
                                               : EMOTE_CORPOREALITY_TIT);
            }
            ...
            if (Creature* halion = ObjectAccessor::GetCreature(
                ...
                halion->CastSpell(
                    halion, _corporealityReference[_corporeality], true);
                halion->AI()->Talk(oldValue > _corporeality
                                       ? EMOTE_CORPOREALITY_POT
                                       : EMOTE_CORPOREALITY_PIP);
            }
        }
...
public:
    spell_halion_meteor_strike_targeting()
        : SpellScriptLoader("spell_halion_meteor_strike_targeting")
    {
    }

    class spell_halion_meteor_strike_targeting_SpellScript
        : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_halion_meteor_strike_targeting_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_halion_meteor_strike_marker()
        : SpellScriptLoader("spell_halion_meteor_strike_marker")
    {
    }
    ...
    class spell_halion_meteor_strike_marker_AuraScript : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster()) {
                if (!caster->GetInstanceScript() ||
                    !caster->GetInstanceScript()->IsEncounterInProgress())
                    return;

                caster->CastSpell(caster, SPELL_METEOR_STRIKE_AOE_DAMAGE, true);
                caster->CastSpell(
                    caster, SPELL_METEOR_STRIKE_FIRE_AURA_1, true);
                for (uint32 spellId = SPELL_SUMMON_METEOR_STRIKE1;
                     spellId <= SPELL_SUMMON_METEOR_STRIKE4;
                     ++spellId)
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_halion_meteor_strike_marker_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_halion_meteor_strike_marker_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_halion_meteor_strike_spread()
        : SpellScriptLoader("spell_halion_meteor_strike_spread")
    {
    }
    ...
    class spell_halion_meteor_strike_spread_AuraScript : public AuraScript {
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (!GetUnitOwner()->GetInstanceScript() ||
                !GetUnitOwner()->GetInstanceScript()->IsEncounterInProgress())
                return;

            GetUnitOwner()->CastSpell(GetUnitOwner(),
                                      RAND(SPELL_SUMMON_METEOR_FLAME1,
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_halion_meteor_strike_spread_AuraScript::HandlePeriodic,
                EFFECT_0,
...
public:
    ...
    class spell_halion_blazing_aura_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_halion_blazing_aura_SpellScript::HandleForceCast,
                EFFECT_1,
...
public:
    spell_halion_combustion_consumption(char const* scriptName, uint32 spell)
        : SpellScriptLoader(scriptName), _spellID(spell)
    {
    }
    ...
    public:
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            GetTarget()->RemoveAurasDueToSpell(
                _markSpell, ObjectGuid::Empty, 0, AURA_REMOVE_BY_EXPIRE);
        }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_halion_combustion_consumption_AuraScript::AddMarkStack,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE);
            AfterEffectApply += AuraEffectApplyFn(
                spell_halion_combustion_consumption_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_halion_combustion_consumption_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_halion_marks(char const* scriptName,
                       uint32      summonSpell,
                       uint32      removeSpell)
        : SpellScriptLoader(scriptName), _summonSpell(summonSpell),
    ...
    public:
        spell_halion_marks_AuraScript(uint32 summonSpell, uint32 removeSpell)
            : AuraScript(), _summonSpellId(summonSpell),
              _removeSpellId(removeSpell)
        {
        }
        ...
        void BeforeDispel(DispelInfo* dispelData)
        {
            ...
            if (Unit* dispelledUnit = GetUnitOwner())
                if (dispelledUnit->HasAura(_removeSpellId))
                    dispelledUnit->RemoveAurasDueToSpell(_removeSpellId,
                                                         ObjectGuid::Empty,
                                                         0,
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            ...
            if (!GetTarget()->GetInstanceScript() ||
                !GetTarget()->GetInstanceScript()->IsEncounterInProgress() ||
                GetTarget()->GetMapId() != 724)
                return;

            GetTarget()->CastCustomSpell(_summonSpellId,
                                         SPELLVALUE_BASE_POINT1,
                                         GetAura()->GetStackAmount(),
                                         GetTarget(),
                                         TRIGGERED_FULL_MASK,
                                         nullptr,
                                         nullptr,
        ...
        {
            OnDispel +=
                AuraDispelFn(spell_halion_marks_AuraScript::BeforeDispel);
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_halion_marks_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_DUMMY,
                                   AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_halion_damage_aoe_summon(char const* scriptName,
                                   uint32      explosionSpell,
                                   uint32      auraSpell)
        : SpellScriptLoader(scriptName), _explosionSpell(explosionSpell),
          _auraSpell(auraSpell)
    {
    }
    ...
    public:
        spell_halion_damage_aoe_summon_SpellScript(uint32 explosionSpell,
                                                   uint32 auraSpell)
            : SpellScript(), _explosionSpell(explosionSpell),
              _auraSpell(auraSpell)
        {
        }
        ...
        void HandleSummon(SpellEffIndex effIndex)
        {
            ...
            if (Creature* summon =
                    caster->GetMap()->SummonCreature(entry,
                                                     pos,
                                                     properties,
                                                     duration,
                                                     caster,
                ...
                summon->CastCustomSpell(
                    SPELL_SCALE_AURA,
                    SPELLVALUE_AURA_STACK,
                    GetSpellValue()->EffectBasePoints[EFFECT_1],
                    summon);
                summon->CastSpell(summon, _auraSpell, true);

                int32 damage =
                    int32((1500 + (GetSpellValue()->EffectBasePoints[EFFECT_1] *
                                   1250)) *
                          (heroic ? 1.25f : 1.0f) * (raid ? 1.5f : 1.0f));
                caster->CastCustomSpell(
                    _explosionSpell, SPELLVALUE_BASE_POINT0, damage, caster);
            }
        }
        ...
        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_halion_damage_aoe_summon_SpellScript::HandleSummon,
                EFFECT_0,
                SPELL_EFFECT_SUMMON);
        }
    ...
    SpellScript* GetSpellScript() const override
    {
        return new spell_halion_damage_aoe_summon_SpellScript(_explosionSpell,
                                                              _auraSpell);
    }
...
public:
    spell_halion_clear_debuffs()
        : SpellScriptLoader("spell_halion_clear_debuffs")
    {
    }
    ...
    class spell_halion_clear_debuffs_SpellScript : public SpellScript {
        ...
        {
            if (GetHitUnit()) {
                GetHitUnit()->RemoveAurasDueToSpell(
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_halion_clear_debuffs_SpellScript::HandleScriptEffect,
                EFFECT_0,
...
public:
    spell_halion_twilight_phasing()
        : SpellScriptLoader("spell_halion_twilight_phasing")
    {
    }
    ...
    class spell_halion_twilight_phasing_SpellScript : public SpellScript {
        ...
        {
            Unit* caster = GetCaster();
            caster->CastSpell(caster->GetPositionX(),
                              caster->GetPositionY(),
                              caster->GetPositionZ(),
                              SPELL_SUMMON_TWILIGHT_PORTAL,
                              true);
            caster->GetMap()->SummonCreature(
                NPC_TWILIGHT_HALION,
                caster->ToCreature()->GetHomePosition(),
                nullptr,
                0,
                caster);
        }
        ...
        {
            OnHit +=
                SpellHitFn(spell_halion_twilight_phasing_SpellScript::Phase);
        }
    };
    ...
    class spell_halion_twilight_phasing_AuraScript : public AuraScript {
        ...
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_halion_twilight_phasing_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_TRANSFORM,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_halion_twilight_phasing_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_TRANSFORM,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_halion_twilight_realm()
        : SpellScriptLoader("spell_halion_twilight_realm")
    {
    }
    ...
    class spell_halion_twilight_realm_AuraScript : public AuraScript {
                     ...
                     AuraEffectHandleModes /*handle*/)
        {
            ...
            GetTarget()->m_Events.AddEvent(
                new SendEncounterUnit(GetTarget()->ToPlayer()),
        ...
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_halion_twilight_realm_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PHASE,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_halion_leave_twilight_realm()
        : SpellScriptLoader("spell_halion_leave_twilight_realm")
    {
    }
    ...
    class spell_halion_leave_twilight_realm_AuraScript : public AuraScript {
                     ...
                     AuraEffectHandleModes /*handle*/)
        {
            ...
            target->RemoveAurasDueToSpell(SPELL_SOUL_CONSUMPTION,
                                          ObjectGuid::Empty,
                                          0,
                                          AURA_REMOVE_BY_ENEMY_SPELL);
        }
        ...
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_halion_leave_twilight_realm_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_halion_leave_twilight_realm_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_halion_twilight_cutter_periodic()
        : SpellScriptLoader("spell_halion_twilight_cutter_periodic")
    {
    }
...
public:
    TwilightCutterSelector(WorldObject* caster, WorldObject* cutterCaster)
        : _caster(caster), _cutterCaster(cutterCaster)
    {
    }
...
public:
    spell_halion_twilight_cutter()
        : SpellScriptLoader("spell_halion_twilight_cutter")
    {
    }
    ...
    class spell_halion_twilight_cutter_SpellScript : public SpellScript {
        ...
        void RemoveNotBetween(std::list<WorldObject*>& unitList)
        {
            ...
            if (Aura* cutter = GetCaster()->GetAura(SPELL_TWILIGHT_CUTTER))
                if (Unit* cutterCaster = cutter->GetCaster()) {
                    unitList.remove_if(
                        TwilightCutterSelector(GetCaster(), cutterCaster));
                    return;
                }
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_halion_twilight_cutter_SpellScript::RemoveNotBetween,
                EFFECT_0,
...
public:
    spell_halion_summon_exit_portals()
        : SpellScriptLoader("spell_halion_summon_exit_portals")
    {
    }
    ...
    class spell_halion_summon_exit_portals_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectLaunch += SpellEffectFn(
                spell_halion_summon_exit_portals_SpellScript::OnSummon,
                EFFECT_0,
...
public:
    spell_halion_twilight_division()
        : SpellScriptLoader("spell_halion_twilight_division")
    {
    }
    ...
    class spell_halion_twilight_division_SpellScript : public SpellScript {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            Creature*       controller = ObjectAccessor::GetCreature(
                *GetCaster(), instance->GetGuidData(NPC_HALION_CONTROLLER));
            Creature* halion = ObjectAccessor::GetCreature(
                *GetCaster(), instance->GetGuidData(NPC_HALION));

            if (!controller || !halion)
                return;

            GetCaster()->CastSpell(
                GetCaster(), _corporealityReference[5], true);
            halion->CastSpell(halion, _corporealityReference[5], true);

            controller->CastSpell(
                controller, SPELL_SUMMON_EXIT_PORTALS_NORMAL, true);
            ...
            if (GameObject* gobject =
                    halion->FindNearestGameObject(GO_HALION_PORTAL_1, 100.0f))
                gobject->Delete();
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_halion_twilight_division_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_halion_twilight_mending()
        : SpellScriptLoader("spell_halion_twilight_mending")
    {
    }
    ...
    class spell_halion_twilight_mending_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_halion_twilight_mending_SpellScript::HandleHealPct,
                EFFECT_ALL,
...
public:
    ...
    struct npc_living_infernoAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override { me->DespawnOrUnsummon(1); }
    };
...
void AddSC_boss_halion()
{
    ...
    new spell_halion_combustion_consumption("spell_halion_soul_consumption",
                                            SPELL_MARK_OF_CONSUMPTION);
    new spell_halion_combustion_consumption("spell_halion_fiery_combustion",
                                            SPELL_MARK_OF_COMBUSTION);
    new spell_halion_marks("spell_halion_mark_of_combustion",
                           SPELL_FIERY_COMBUSTION_SUMMON,
                           SPELL_FIERY_COMBUSTION);
    new spell_halion_marks("spell_halion_mark_of_consumption",
                           SPELL_SOUL_CONSUMPTION_SUMMON,
                           SPELL_SOUL_CONSUMPTION);
    new spell_halion_damage_aoe_summon("spell_halion_combustion_summon",
                                       SPELL_FIERY_COMBUSTION_EXPLOSION,
                                       SPELL_COMBUSTION_DAMAGE_AURA);
    new spell_halion_damage_aoe_summon("spell_halion_consumption_summon",
