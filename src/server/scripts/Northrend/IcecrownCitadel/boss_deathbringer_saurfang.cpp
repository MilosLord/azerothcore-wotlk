...
enum Spells {
    ...
    SPELL_SUMMON_BLOOD_BEAST_25_MAN =
        72356, // Additional cast, does not replace
    SPELL_FRENZY                      = 72737,
...
Position const chargePos[6] = {
    {-509.6505f,
     2211.377f,
     539.2872f,
     0.0f}, // High Overlord Saurfang/Muradin Bronzebeard
...
public:
    ...
    struct boss_deathbringer_saurfangAI : public BossAI {
        boss_deathbringer_saurfangAI(Creature* creature)
            : BossAI(creature, DATA_DEATHBRINGER_SAURFANG)
        {
            ASSERT(creature->GetVehicleKit()); // we dont actually use it, just
                                               // check if exists
        }
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            // pussywizard: without this, the aura is not recalculated the first
            // time
            me->RemoveAurasDueToSpell(SPELL_BLOOD_POWER);
            DoCast(me, SPELL_BLOOD_POWER, true);

            if (!instance->CheckRequiredBosses(DATA_DEATHBRINGER_SAURFANG,
                                               who->ToPlayer())) {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Creature* creature = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_SAURFANG_EVENT_NPC)))
        ...
        {
            if (Unit* target =
                    SelectTarget(SelectTargetMethod::Random, 1, 0.0f, true))
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            instance->HandleGameObject(
                instance->GetGuidData(GO_SAURFANG_S_DOOR), false);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_SUMMON_BLOOD_BEAST:
                    ...
                    if (IsHeroic())
                        events.ScheduleEvent(EVENT_BLOOD_BEAST_SCENT_OF_BLOOD,
                                             10s);
                    break;
                ...
                case EVENT_BLOOD_NOVA: {
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_BLOOD_NOVA_TRIGGER, false);
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_MARK_OF_THE_FALLEN_CHAMPION:
                if (Unit* target =
                    ...
                    me->CastSpell(
                        target, SPELL_MARK_OF_THE_FALLEN_CHAMPION, false);
                    me->SetPower(POWER_ENERGY, 0);
...
public:
    npc_high_overlord_saurfang_icc()
        : CreatureScript("npc_high_overlord_saurfang_icc")
    {
    }
    ...
    struct npc_high_overlord_saurfangAI : public ScriptedAI {
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_START_EVENT: {
                ...
                Creature* deathbringer = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(DATA_DEATHBRINGER_SAURFANG));
                if (!deathbringer || deathbringer->IsInEvadeMode())
                    return;

                if (_guardList.empty()) {
                    GetCreatureListWithEntryInGrid(
                        _guardList, me, NPC_SE_KOR_KRON_REAVER, 20.0f);
                    _guardList.sort(Acore::ObjectDistanceOrderPred(me));
                }
                uint32 x = 1;
                for (std::list<Creature*>::iterator itr = _guardList.begin();
                     itr != _guardList.end();
                     ++itr)
                    (*itr)->AI()->SetData(0, x++);
                ...
                _events.ScheduleEvent(
                    EVENT_INTRO_HORDE_2, 5s, 0, PHASE_INTRO_H);
                _events.ScheduleEvent(
                    EVENT_INTRO_HORDE_3, 18s + 500ms, 0, PHASE_INTRO_H);
                _instance->HandleGameObject(
                    _instance->GetGuidData(GO_SAURFANG_S_DOOR), true);

                if (GameObject* teleporter = ObjectAccessor::GetGameObject(
                        *me,
                        _instance->GetGuidData(
                            GO_SCOURGE_TRANSPORTER_SAURFANG))) {
                    _instance->HandleGameObject(
                        ObjectGuid::Empty, false, teleporter);
                ...
                deathbringer->GetMotionMaster()->MovePoint(
                    POINT_SAURFANG,
                    deathbringerPos.GetPositionX(),
                    deathbringerPos.GetPositionY(),
                    deathbringerPos.GetPositionZ());
            } break;
            case ACTION_START_OUTRO: {
                me->RemoveAurasDueToSpell(SPELL_GRIP_OF_AGONY);
                me->SetDisableGravity(false);
                me->MonsterMoveWithSpeed(
                    me->GetPositionX(), me->GetPositionY(), 539.2917f, 10.0f);
                for (std::list<Creature*>::iterator itr = _guardList.begin();
                     itr != _guardList.end();
                     ++itr)
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE) {
                ...
                case POINT_FIRST_STEP:
                    ...
                    _events.ScheduleEvent(
                        EVENT_INTRO_HORDE_4, 6500ms, 0, PHASE_INTRO_H);
                    _events.ScheduleEvent(
                        EVENT_INTRO_HORDE_5, 15s + 500ms, 0, PHASE_INTRO_H);
                    _events.ScheduleEvent(
                        EVENT_INTRO_HORDE_6, 29s + 500ms, 0, PHASE_INTRO_H);
                    _events.ScheduleEvent(
                        EVENT_INTRO_HORDE_7, 43s + 800ms, 0, PHASE_INTRO_H);
                    _events.ScheduleEvent(
                        EVENT_INTRO_HORDE_8, 47s, 0, PHASE_INTRO_H);
                    _events.ScheduleEvent(
                        EVENT_INTRO_HORDE_9, 48s + 200ms, 0, PHASE_INTRO_H);
                    _events.ScheduleEvent(
                        EVENT_INTRO_FINISH, 56s + 700ms, 0, PHASE_INTRO_H);
                    break;
                /*case POINT_CORPSE:
                    if (Creature* deathbringer =
                ObjectAccessor::GetCreature(*me,
                _instance->GetGuidData(DATA_DEATHBRINGER_SAURFANG)))
                    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INTRO_HORDE_2:
                if (Creature* deathbringer = ObjectAccessor::GetCreature(
                        *me,
                        _instance->GetGuidData(DATA_DEATHBRINGER_SAURFANG)))
            ...
            case EVENT_INTRO_HORDE_3:
                me->SetWalk(true);
                me->GetMotionMaster()->MovePoint(POINT_FIRST_STEP,
                                                 firstStepPos.GetPositionX(),
                                                 firstStepPos.GetPositionY(),
                                                 firstStepPos.GetPositionZ());
                break;
            ...
            case EVENT_INTRO_HORDE_8:
                ...
                me->GetMotionMaster()->MoveCharge(chargePos[0].GetPositionX(),
                                                  chargePos[0].GetPositionY(),
                                                  chargePos[0].GetPositionZ(),
                                                  8.5f,
                                                  POINT_CHARGE);
                break;
            ...
            case EVENT_INTRO_FINISH:
                if (Creature* deathbringer = ObjectAccessor::GetCreature(
                    ...
                    if (Player* target =
                            deathbringer->SelectNearestPlayer(100.0f))
                ...
                _instance->GetGuidData(DATA_DEATHBRINGER_SAURFANG)))
                    {
                        float x, y, z;
                        deathbringer->GetClosePoint(x, y, z,
                deathbringer->GetObjectSize()); me->SetWalk(true);
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (instance &&
            ...
            instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != IN_PROGRESS) {
            AddGossipItemFor(
                player, 10953, 0, GOSSIP_SENDER_INFO, -ACTION_START_EVENT);
            SendGossipMenuFor(player,
                              player->GetGossipTextId(10953, creature),
                              creature->GetGUID());
        }
...
public:
    npc_muradin_bronzebeard_icc()
        : CreatureScript("npc_muradin_bronzebeard_icc")
    {
    }
    ...
    struct npc_muradin_bronzebeard_iccAI : public ScriptedAI {
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_START_EVENT: {
                ...
                _events.ScheduleEvent(
                    EVENT_INTRO_ALLIANCE_2, 2500ms, 0, PHASE_INTRO_A);
                _events.ScheduleEvent(
                    EVENT_INTRO_ALLIANCE_3, 20s, 0, PHASE_INTRO_A);
                _events.ScheduleEvent(
                    EVENT_INTRO_ALLIANCE_4, 29s + 500ms, 0, PHASE_INTRO_A);
                _instance->HandleGameObject(
                    _instance->GetGuidData(GO_SAURFANG_S_DOOR), true);
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE) {
                ...
                case POINT_FIRST_STEP:
                    ...
                    _events.ScheduleEvent(
                        EVENT_INTRO_ALLIANCE_5, 5s, 0, PHASE_INTRO_A);
                    _events.ScheduleEvent(
                        EVENT_INTRO_ALLIANCE_6, 7s, 0, PHASE_INTRO_A);
                    _events.ScheduleEvent(
                        EVENT_INTRO_ALLIANCE_7, 9s, 0, PHASE_INTRO_A);
                    _events.ScheduleEvent(
                        EVENT_INTRO_FINISH, 14s, 0, PHASE_INTRO_A);
                    break;
                default:
                    break;
                }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (instance &&
            ...
            instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != IN_PROGRESS) {
            AddGossipItemFor(
                player, 10933, 0, GOSSIP_SENDER_INFO, -ACTION_START_EVENT + 1);
            SendGossipMenuFor(player,
                              player->GetGossipTextId(10933, creature),
                              creature->GetGUID());
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_CHARGE && _index) {
                ...
                me->GetMotionMaster()->MoveCharge(
                    chargePos[_index].GetPositionX(),
                    chargePos[_index].GetPositionY(),
                    chargePos[_index].GetPositionZ(),
                    13.0f,
                    POINT_CHARGE);
            }
...
public:
    spell_deathbringer_blood_link_aura()
        : SpellScriptLoader("spell_deathbringer_blood_link_aura")
    {
    }
    ...
    class spell_deathbringer_blood_link_AuraScript : public AuraScript {
        ...
        void HandlePeriodicTick(AuraEffect const* /*aurEff*/)
        {
            ...
            if (GetUnitOwner()->getPowerType() == POWER_ENERGY &&
                GetUnitOwner()->GetPower(POWER_ENERGY) ==
                    GetUnitOwner()->GetMaxPower(POWER_ENERGY))
                if (Creature* saurfang = GetUnitOwner()->ToCreature())
                    saurfang->AI()->DoAction(
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            return eventInfo.GetActor() && eventInfo.GetActionTarget() &&
                   ...
                   procSpell &&
                   procSpell->SpellIconID !=
                       2731; // Xinef: Mark of the Fallen Champion
        }
        ...
        void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            // markCount = saurfang->IsAIEnabled ?
            // saurfang->AI()->GetData(123456 /*FALLEN_CHAMPION_CAST_COUNT*/) :
            // 0;
            ...
            victim->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY,
        ...
        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(
                spell_deathbringer_blood_link_AuraScript::CheckProc);
            OnEffectProc += AuraEffectProcFn(
                spell_deathbringer_blood_link_AuraScript::HandleProc,
                EFFECT_0,
                SPELL_AURA_PROC_TRIGGER_SPELL);
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_deathbringer_blood_link_AuraScript::HandlePeriodicTick,
                EFFECT_1,
...
public:
    spell_deathbringer_blood_link_blood_beast_aura()
        : SpellScriptLoader("spell_deathbringer_blood_link_blood_beast_aura")
    {
    }

    class spell_deathbringer_blood_link_blood_beast_aura_AuraScript
        : public AuraScript {
        PrepareAuraScript(
            ...
            if (Map* map = eventInfo.GetActor()->FindMap())
                if (InstanceMap* imap = map->ToInstanceMap())
                    if (InstanceScript* isc = imap->GetInstanceScript())
                        if (ObjectGuid sguid = isc->GetGuidData(3)
            //DATA_DEATHBRINGER_SAURFANG if (Creature* saurfang =
            ObjectAccessor::GetCreature(*eventInfo.GetActor(), sguid)) markCount
            = saurfang->IsAIEnabled ? saurfang->AI()->GetData(123456) : 0;
            //FALLEN_CHAMPION_CAST_COUNT
            */
            int32 basepoints = int32(3.0f /*+ 0.5f + 0.5f*markCount*/);

            eventInfo.GetActor()->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY,
                                                  SPELLVALUE_BASE_POINT0,
                                                  basepoints,
...
public:
    spell_deathbringer_blood_link()
        : SpellScriptLoader("spell_deathbringer_blood_link")
    {
    }
    ...
    class spell_deathbringer_blood_link_SpellScript : public SpellScript {
        ...
        {
            GetHitUnit()->CastCustomSpell(SPELL_BLOOD_LINK_POWER,
                                          SPELLVALUE_BASE_POINT0,
                                          GetEffectValue(),
                                          GetHitUnit(),
                                          true);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_deathbringer_blood_link_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_deathbringer_blood_power()
        : SpellScriptLoader("spell_deathbringer_blood_power")
    {
    }
    ...
    class spell_deathbringer_blood_power_SpellScript : public SpellScript {
        ...
        {
            AfterHit += SpellHitFn(
                spell_deathbringer_blood_power_SpellScript::ModAuraValue);
        }
    };
    ...
    class spell_deathbringer_blood_power_AuraScript : public AuraScript {
        ...
        void RecalculateHook(AuraEffect const* /*aurEffect*/,
                             int32& amount,
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_deathbringer_blood_power_AuraScript::RecalculateHook,
                EFFECT_0,
...
public:
    spell_deathbringer_blood_nova_targeting()
        : SpellScriptLoader("spell_deathbringer_blood_nova_targeting")
    {
    }

    class spell_deathbringer_blood_nova_targeting_SpellScript
        : public SpellScript {
        ...
        void FilterTargetsInitial(std::list<WorldObject*>& targets)
        {
            ...
            uint32 const minTargets =
                uint32(GetCaster()->GetMap()->GetSpawnMode() & 1 ? 10 : 4);
...
public:
    spell_deathbringer_boiling_blood()
        : SpellScriptLoader("spell_deathbringer_boiling_blood")
    {
    }
    ...
    class spell_deathbringer_boiling_blood_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            if (GetSpellInfo()->Id == 72385 ||
                GetSpellInfo()->Id == 72442) // 10n, 10h
            {
                WorldObject* target =
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_deathbringer_boiling_blood_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
public:
    achievement_ive_gone_and_made_a_mess()
        : AchievementCriteriaScript("achievement_ive_gone_and_made_a_mess")
    {
    }
