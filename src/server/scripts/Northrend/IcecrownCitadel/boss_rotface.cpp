...
enum Spells {
    ...
    SPELL_GREEN_ABOMINATION_HITTIN__YA_PROC =
        70001, // prevents getting hit by infection
...
public:
    ...
    struct boss_rotfaceAI : public BossAI {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            instance->SetData(DATA_OOZE_DANCE_ACHIEVEMENT,
                              uint32(true)); // reset
            ...
            list.remove_if(
                RotfaceHeightCheck()); // remove from the list all on the ground
            for (std::list<Creature*>::const_iterator itr = list.begin();
                 itr != list.end();
                 ++itr) {
                uint32 index =
                    me->GetHomePosition().GetAngle(*itr) / (M_PI / 2.0f);
                if (index > 3)
                    index = 3;
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            case 73190:
                if (Player* p = target->ToPlayer())
                    if (p->GetQuestStatus(QUEST_RESIDUE_RENDEZVOUS_10) ==
                            QUEST_STATUS_INCOMPLETE ||
                        p->GetQuestStatus(QUEST_RESIDUE_RENDEZVOUS_25) ==
                            QUEST_STATUS_INCOMPLETE)
            ...
            case 69801: {
                ...
                if (target->GetGUID() == _oozeFloodDummyGUIDs[index][0] ||
                    target->GetGUID() == _oozeFloodDummyGUIDs[index][1])
                    target->CastSpell(
                        (Unit*)nullptr, spell->Effects[0].CalcValue(), false);
            } break;
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SLIME_SPRAY:
                if (Unit* target = SelectTarget(SelectTargetMethod::Random,
                                                0,
                                                NonTankTargetSelector(me))) {
                    if (Creature* c =
                            me->SummonCreature(NPC_OOZE_SPRAY_STALKER,
                                               *target,
            ...
            case EVENT_MUTATED_INFECTION:
                me->CastCustomSpell(SPELL_MUTATED_INFECTION,
                                    SPELLVALUE_MAX_TARGETS,
                                    1,
                                    nullptr,
                                    false);
                events.ScheduleEvent(EVENT_MUTATED_INFECTION,
                                     infectionCooldown);
                break;
            case EVENT_ROTFACE_OOZE_FLOOD:
                if (Creature* professor = ObjectAccessor::GetCreature(
                    ...
                    me->CastSpell(
                        (Unit*)nullptr, oozeFloodSpells[_oozeFloodStage], true);
            ...
            case EVENT_ROTFACE_VILE_GAS: {
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, minDist, true))
                    if (Creature* professor = ObjectAccessor::GetCreature(
                            *me,
                        ...
                        professor->CastSpell(
                            target,
                            SPELL_VILE_GAS_H,
                            true); // triggered, to skip LoS check
            }
                events.ScheduleEvent(EVENT_ROTFACE_VILE_GAS, 15s, 20s);
                break;
            default:
...
public:
    ...
    struct npc_little_oozeAI : public ScriptedAI {
        ...
        {
            firstUpdate = true;
            if (Creature* rotface = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_ROTFACE)))
    ...
    struct npc_big_oozeAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (firstUpdate) {
                ...
                me->CastSpell(
                    me, SPELL_GREEN_ABOMINATION_HITTIN__YA_PROC, true);
                events.Reset();
                events.ScheduleEvent(EVENT_STICKY_OOZE, 5s);
                DoResetThreatList();
                me->SetInCombatWithZone();
...
public:
    spell_rotface_mutated_infection()
        : SpellScriptLoader("spell_rotface_mutated_infection")
    {
    }
    ...
    class spell_rotface_mutated_infection_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_rotface_mutated_infection_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_rotface_mutated_infection_SpellScript::ReplaceTargets,
                EFFECT_1,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_rotface_mutated_infection_SpellScript::ReplaceTargets,
                ...
                TARGET_UNIT_SRC_AREA_ENEMY);
            AfterHit += SpellHitFn(
                spell_rotface_mutated_infection_SpellScript::NotifyTargets);
        }
    ...
    class spell_rotface_mutated_infection_AuraScript : public AuraScript {
        ...
        {
            GetTarget()->CastSpell(
                GetTarget(), GetSpellInfo()->Effects[2].CalcValue(), true);
        }

        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_rotface_mutated_infection_AuraScript::ExtraRemoveEffect,
                EFFECT_0,
...
public:
    spell_rotface_little_ooze_combine()
        : SpellScriptLoader("spell_rotface_little_ooze_combine")
    {
    }
    ...
    class spell_rotface_little_ooze_combine_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_rotface_little_ooze_combine_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_rotface_large_ooze_combine()
        : SpellScriptLoader("spell_rotface_large_ooze_combine")
    {
    }
    ...
    class spell_rotface_large_ooze_combine_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            ...
            if (newStack >= 5) {
                ...
                if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                    if (Creature* rotface = ObjectAccessor::GetCreature(
                            *GetCaster(), instance->GetGuidData(DATA_ROTFACE)))
                        if (rotface->IsAlive()) {
                            if (GetCaster()->GetTypeId() == TYPEID_UNIT)
                                GetCaster()->ToCreature()->AI()->Talk(
            ...
            GetHitCreature()->RemoveAurasDueToSpell(
                SPELL_LARGE_OOZE_BUFF_COMBINE);
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_rotface_large_ooze_combine_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_rotface_large_ooze_buff_combine()
        : SpellScriptLoader("spell_rotface_large_ooze_buff_combine")
    {
    }

    class spell_rotface_large_ooze_buff_combine_SpellScript
        : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_rotface_large_ooze_buff_combine_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_rotface_unstable_ooze_explosion_init()
        : SpellScriptLoader("spell_rotface_unstable_ooze_explosion_init")
    {
    }

    class spell_rotface_unstable_ooze_explosion_init_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void HandleCast(SpellEffIndex effIndex)
        {
            ...
            Creature* dummy =
                GetCaster()->SummonCreature(NPC_UNSTABLE_EXPLOSION_STALKER,
                                            x,
                                            y,
                                            z,
                                            0.0f,
                                            ...
                                            60000);
            GetCaster()->CastSpell(
                dummy, SPELL_UNSTABLE_OOZE_EXPLOSION_TRIGGER, true);
        }
...
public:
    spell_rotface_unstable_ooze_explosion()
        : SpellScriptLoader("spell_rotface_unstable_ooze_explosion")
    {
    }

    class spell_rotface_unstable_ooze_explosion_SpellScript
        : public SpellScript {
        ...
        void CheckTarget(SpellEffIndex effIndex)
        {
            ...
            if (InstanceScript* script = GetCaster()->GetInstanceScript())
                if (Creature* rotface = script->instance->GetCreature(
                        script->GetGuidData(DATA_ROTFACE)))
                    rotface->CastSpell(
                        x,
                        y,
                        z,
                        triggered_spell_id,
                        true /*, nullptr, nullptr, GetCaster()->GetGUID()*/); // caster not available on clientside, no log in such case
        }

        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_rotface_unstable_ooze_explosion_SpellScript::CheckTarget,
                EFFECT_0,
...
public:
    spell_rotface_unstable_ooze_explosion_suicide()
        : SpellScriptLoader("spell_rotface_unstable_ooze_explosion_suicide")
    {
    }
    ...
    struct npc_precious_iccAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (InstanceScript* _instance = me->GetInstanceScript())
                if (Creature* rotface = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_ROTFACE)))
                    if (rotface->IsAlive())
