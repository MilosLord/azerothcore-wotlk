...
BossBoundaryData const boundaries = {
    ...
    {DATA_LORD_MARROWGAR,
     new RectangleBoundary(-430.0f, -330.0f, 2110.0f, 2310.0f)},
    {DATA_LADY_DEATHWHISPER,
     new RectangleBoundary(-670.0f, -520.0f, 2145.0f, 2280.0f)},
    {DATA_DEATHBRINGER_SAURFANG,
     new RectangleBoundary(-565.0f, -465.0f, 2160.0f, 2260.0f)},
    ...
    {DATA_PROFESSOR_PUTRICIDE,
     new ParallelogramBoundary(Position(4356.0f, 3290.0f),
                               Position(4435.0f, 3194.0f),
                               Position(4280.0f, 3194.0f))},
    {DATA_PROFESSOR_PUTRICIDE,
     new RectangleBoundary(4280.0f, 4435.0f, 3150.0f, 4360.0f)},

    {DATA_BLOOD_PRINCE_COUNCIL,
     new EllipseBoundary(Position(4660.95f, 2769.194f), 85.0, 60.0)},
    {DATA_BLOOD_QUEEN_LANA_THEL,
     new CircleBoundary(Position(4595.93f, 2769.365f), 64.0)},

    {DATA_SISTER_SVALNA,
     new RectangleBoundary(4291.0f, 4423.0f, 2438.0f, 2653.0f)},
    {DATA_VALITHRIA_DREAMWALKER,
     new RectangleBoundary(4112.5f, 4293.5f, 2385.0f, 2585.0f)},
    {DATA_SINDRAGOSA,
     new EllipseBoundary(Position(4418.6f, 2484.0f), 110.0, 75.0)}};
...
ObjectData const creatureData[] = {{NPC_SINDRAGOSA, DATA_SINDRAGOSA}, {0, 0}};

// this doesnt have to only store questgivers, also can be used for related
// quest spawns
struct WeeklyQuest {
...
// when changing the content, remember to update SetData,
// DATA_BLOOD_QUICKENING_STATE case for NPC_ALRIN_THE_AGILE index
WeeklyQuest const WeeklyQuestData[WeeklyNPCs] = {
    {NPC_INFILTRATOR_MINCHAR,
     {QUEST_DEPROGRAMMING_10, QUEST_DEPROGRAMMING_25}}, // Deprogramming
    {NPC_KOR_KRON_LIEUTENANT,
     {QUEST_SECURING_THE_RAMPARTS_10,
      QUEST_SECURING_THE_RAMPARTS_25}}, // Securing the Ramparts
    ...
    {NPC_ALCHEMIST_ADRIANNA,
     {QUEST_RESIDUE_RENDEZVOUS_10,
      QUEST_RESIDUE_RENDEZVOUS_25}}, // Residue Rendezvous
    {NPC_ALRIN_THE_AGILE,
     {QUEST_BLOOD_QUICKENING_10,
      QUEST_BLOOD_QUICKENING_25}}, // Blood Quickening
    ...
    {NPC_VALITHRIA_DREAMWALKER_QUEST,
     {QUEST_RESPITE_FOR_A_TORMENTED_SOUL_10,
      QUEST_RESPITE_FOR_A_TORMENTED_SOUL_25}}, // Respite for a Tormented Soul
};
...
Position const SindragosaSpawnPos = {
    4818.6997f, 2483.7102f, 287.06497f, 3.286661f};

// Set position traps Spirit Alarm
std::vector<Position> GoSpiritAlarm_1 = {{-160.96f, 2210.46f, 35.24f, 0.0f},
                                         {-176.27f, 2201.93f, 35.24f, 0.0f},
                                         {-207.83f, 2207.38f, 35.24f, 0.0f}};
std::vector<Position> GoSpiritAlarm_2 = {{-178.41f, 2225.11f, 35.24f, 0.0f},
                                         {-195.23f, 2221.55f, 35.24f, 0.0f},
                                         {-209.94f, 2250.34f, 37.99f, 0.0f}};
std::vector<Position> GoSpiritAlarm_3 = {{-289.80f, 2216.60f, 42.39f, 0.0f},
                                         {-317.76f, 2216.11f, 42.57f, 0.0f},
                                         {-301.07f, 2216.62f, 42.0f, 0.0f}};
std::vector<Position> GoSpiritAlarm_4 = {{-276.07f, 2206.76f, 42.57f, 0.0f},
                                         {-304.44f, 2199.11f, 41.99f, 0.0f},
                                         {-292.82f, 2204.61f, 42.02f, 0.0f}};
...
public:
    DelayedCastMincharEvent(Creature* trigger, uint32 spellId)
        : _trigger(trigger), _spellId(spellId)
    {
    }

    bool Execute(uint64 /*time*/, uint32 /*diff*/) override
    {
        if (Creature* minchar = _trigger->FindNearestCreature(
                NPC_INFILTRATOR_MINCHAR_BQ, 50.0f, true))
    ...
    struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript {
        ...
        void FillInitialWorldStates(WorldPacket& data) override
        {
            if (instance->IsHeroic()) {
                data << uint32(WORLDSTATE_SHOW_TIMER)
                     << uint32(BloodQuickeningState == IN_PROGRESS);
                data << uint32(WORLDSTATE_EXECUTION_TIME)
                     << uint32(BloodQuickeningMinutes);
                data << uint32(WORLDSTATE_SHOW_ATTEMPTS) << uint32(1);
                data << uint32(WORLDSTATE_ATTEMPTS_REMAINING)
                     << uint32(HeroicAttempts);
                data << uint32(WORLDSTATE_ATTEMPTS_MAX)
                     << uint32(MaxHeroicAttempts);
            }
        }

        void OnPlayerAreaUpdate(Player* player,
                                uint32 /*oldArea*/,
                                uint32 newArea) override
        {
            if (newArea == 4890 /*Putricide's Laboratory of Alchemical Horrors
                                   and Fun*/
                ...
                player->SendInitWorldStates(player->GetZoneId(),
                                            player->GetAreaId());
            }
            else {
        ...
        void OnPlayerEnter(Player* player) override
        {
            ...
            if (GetBossState(DATA_LADY_DEATHWHISPER) == DONE &&
                GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) != DONE)
                SpawnGunship();

            if (GetBossState(DATA_SINDRAGOSA) != DONE &&
                IsSindragosaIntroDone && !GetCreature(DATA_SINDRAGOSA) &&
                Events.GetTimeUntilEvent(EVENT_RESPAWN_SINDRAGOSA) ==
                    Milliseconds::max()) {
            ...
            if (IsBuffAvailable) {
                ...
                for (SpellAreaForAreaMap::const_iterator itr = saBounds.first;
                     itr != saBounds.second;
                     ++itr)
                    if ((itr->second->raceMask & player->getRaceMask()) &&
                        !player->HasAura(itr->second->spellId)) {
                        if (SpellInfo const* si =
                                sSpellMgr->GetSpellInfo(itr->second->spellId)) {
                            if (si->HasAura(
                                    SPELL_AURA_MOD_INCREASE_HEALTH_PERCENT)) {
                                DoCastSpellOnPlayer(
                                    player, itr->second->spellId, false, false);
                            }
                        }
                    }
            }
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            if (GetData(DATA_BUFF_AVAILABLE) &&
                creature->GetOwnerGUID().IsPlayer() &&
                creature->HasUnitTypeMask(UNIT_MASK_MINION |
                                          UNIT_MASK_GUARDIAN |
                                          UNIT_MASK_CONTROLABLE_GUARDIAN) &&
                creature->CanHaveThreatList())
                if (Unit* owner = creature->GetOwner())
                    if (Player* plr = owner->ToPlayer()) {
                        SpellAreaForAreaMapBounds saBounds =
                            sSpellMgr->GetSpellAreaForAreaMapBounds(4812);
                        for (SpellAreaForAreaMap::const_iterator itr =
                                 saBounds.first;
                             itr != saBounds.second;
                             ++itr)
                            if ((itr->second->raceMask & plr->getRaceMask()) &&
                                !creature->HasAura(itr->second->spellId))
                                if (SpellInfo const* si =
            ...
            if (!creature->GetTransport() &&
                creature->GetPositionZ() <= 205.0f &&
                creature->GetExactDist2d(-439.0f, 2210.0f) <= 150.0f &&
                (creature->GetEntry() == 37544 ||
                 creature->GetEntry() == 37545 ||
                 creature->GetName().compare(0, name1.length(), name1) == 0 ||
                 creature->GetName().compare(0, name2.length(), name2) == 0))
                creature->AddToNotify(NOTIFY_AI_RELOCATION);

            // pussywizard: check weekly here, before possible UpdateEntry
            // allow creating all of them, because after killing Marrowgar some
            // have to appear, so just hide them
            ...
            case NPC_VALITHRIA_DREAMWALKER_QUEST:
                for (uint8 i = 0; i < WeeklyNPCs; ++i)
                    if (WeeklyQuestData[i].creatureEntry ==
                        creature->GetEntry()) {
                        WeeklyQuestNpcGUID[i] = creature->GetGUID();
                        if (WeeklyQuestId10 != WeeklyQuestData[i].questId[0])
                            creature->SetVisible(false);
                        else if (WeeklyQuestData[i].creatureEntry ==
                                     NPC_VALITHRIA_DREAMWALKER_QUEST &&
                                 GetBossState(DATA_VALITHRIA_DREAMWALKER) !=
                                     DONE)
                            creature->SetVisible(false);
                    }
                break;
            ...
            case NPC_GARROSH_HELLSCREAM:
                ...
                if (GetBossState(DATA_THE_LICH_KING) == DONE) {
                    instance->SummonCreature(NPC_LADY_JAINA_PROUDMOORE_QUEST,
                                             JainaSpawnPos);
                    instance->SummonCreature(NPC_MURADIN_BRONZEBEARD_QUEST,
                                             MuradinSpawnPos);
                    instance->SummonCreature(NPC_UTHER_THE_LIGHTBRINGER_QUEST,
                                             UtherSpawnPos);
                    instance->SummonCreature(NPC_LADY_SYLVANAS_WINDRUNNER_QUEST,
                                             SylvanasSpawnPos);
                }
                break;
            ...
            case NPC_HIGH_OVERLORD_SAURFANG_DUMMY:
                if (TeamIdInInstance == TEAM_ALLIANCE) {
                    creature->UpdateEntry(NPC_MURADIN_BRONZEBEARD_DUMMY,
                                          creature->GetCreatureData());
                    creature->LoadEquipment();
                }
                break;
            ...
            case NPC_PROFESSOR_PUTRICIDE:
                ...
                if (GetBossState(DATA_ROTFACE) == DONE &&
                    GetBossState(DATA_FESTERGUT) == DONE && !HeroicAttempts &&
                    GetData(DATA_HAS_LIMITED_ATTEMPTS) && creature->IsAlive())
                    creature->SetVisible(false);
                break;
            ...
            case NPC_BLOOD_QUEEN_LANA_THEL:
                ...
                if (!HeroicAttempts && GetData(DATA_HAS_LIMITED_ATTEMPTS) &&
                    creature->IsAlive())
                    creature->SetVisible(false);
                break;
            ...
            case NPC_CAPTAIN_RUPERT:
                CrokCaptainGUIDs[creature->GetEntry() - NPC_CAPTAIN_ARNATH] =
                    creature->GetGUID();
                break;
            ...
            case NPC_INVISIBLE_STALKER:
                ...
                if (creature->GetExactDist2d(4357.052f, 2769.421f) < 10.0f &&
                    GetBossState(DATA_PROFESSOR_PUTRICIDE) == DONE &&
                    GetBossState(DATA_BLOOD_QUEEN_LANA_THEL) == DONE &&
                    GetBossState(DATA_SINDRAGOSA) == DONE)
                    creature->CastSpell(
                        creature, SPELL_ARTHAS_TELEPORTER_CEREMONY, false);
                break;
            ...
            case NPC_MINCHAR_BEAM_STALKER:
                if (BloodQuickeningState != DONE) {
                    uint32 spellId = 0;
                    if (creature->GetPositionY() > 2790.0f &&
                        creature->GetPositionZ() > 420.0f)
                        spellId = BLOOD_BEAM_VISUAL_RHAND;
                    else if (creature->GetPositionY() < 2790.0f &&
                             creature->GetPositionZ() > 420.0f)
                    ...
                    creature->m_Events.AddEvent(
                        new DelayedCastMincharEvent(creature, spellId),
        ...
        uint32 GetCreatureEntry(ObjectGuid::LowType /*guidLow*/,
                                CreatureData const* data) override
        {
            if (TeamIdInInstance == TEAM_NEUTRAL) {
                Map::PlayerList const& players = instance->GetPlayers();
                if (!players.IsEmpty())
            ...
            case NPC_ZAFOD_BOOMBOX:
                if (GameObjectTemplate const* go =
                        sObjectMgr->GetGameObjectTemplate(GO_THE_SKYBREAKER_A))
                    if ((TeamIdInInstance == TEAM_ALLIANCE &&
                         data->mapid == go->moTransport.mapID) ||
                        (TeamIdInInstance == TEAM_HORDE &&
                         data->mapid != go->moTransport.mapID))
                        return entry;
                return 0;
        ...
        uint32 GetGameObjectEntry(ObjectGuid::LowType /*guidLow*/,
        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            case NPC_YMIRJAR_WARLORD:
                if (Creature* crok = instance->GetCreature(CrokScourgebaneGUID))
                    crok->AI()->SetGUID(creature->GetGUID(),
                                        ACTION_VRYKUL_DEATH);
                break;
            case NPC_FROSTWING_WHELP:
                ...
                if (creature->AI()->GetData(1 /*DATA_FROSTWYRM_OWNER*/) ==
                    DATA_SPINESTALKER) {
                    SpinestalkerTrash.erase(creature->GetSpawnId());
                    if (SpinestalkerTrash.empty())
                        if (Creature* spinestalk =
                                instance->GetCreature(SpinestalkerGUID))
                    ...
                    if (RimefangTrash.empty())
                        if (Creature* spinestalk =
                                instance->GetCreature(RimefangGUID))
                            spinestalk->AI()->DoAction(ACTION_START_FROSTWYRM);
                }
                break;
            case NPC_RIMEFANG:
            case NPC_SPINESTALKER: {
                ...
                if (FrostwyrmGUIDs.empty()) {
                    instance->LoadGrid(SindragosaSpawnPos.GetPositionX(),
                                       SindragosaSpawnPos.GetPositionY());
                    if (Creature* boss = instance->SummonCreature(
                            NPC_SINDRAGOSA, SindragosaSpawnPos))
                        boss->AI()->DoAction(ACTION_START_FROSTWYRM);
                }
                break;
            }
            case NPC_DEATHSPEAKER_SERVANT:
                if (Creature* c = unit->SummonCreature(WORLD_TRIGGER,
                                                       *unit,
                                                       TEMPSUMMON_TIMED_DESPAWN,
                                                       10000)) {
                    c->CastSpell(c, VOID_ZONE_VISUAL, true);
                    unit->SummonCreature(NPC_RISEN_DEATHSPEAKER_SERVANT,
                                         *unit,
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_GEIST_ALARM_2:
                go->SetPosition(
                    go->GetPositionX() +
                        urand(0, 2) * 20.0f *
                            (go->GetEntry() == GO_GEIST_ALARM_1 ? -1.0f : 1.0f),
                    go->GetPositionY(),
                    go->GetPositionZ(),
                    go->GetOrientation());
                break;
            ...
            case GO_SCIENTIST_ENTRANCE:
                ...
                HandleGameObject(PutricideEnteranceDoorGUID,
            ...
            case GO_SCIENTIST_AIRLOCK_DOOR_COLLISION:
                ...
                HandleGameObject(PutricideCollisionGUID,
                                 ((PutricideEventProgress &
            ...
            case GO_SCIENTIST_AIRLOCK_DOOR_ORANGE:
                ...
                if ((PutricideEventProgress &
                     PUTRICIDE_EVENT_FLAG_FESTERGUT_VALVE) &&
                    (PutricideEventProgress &
                     PUTRICIDE_EVENT_FLAG_ROTFACE_VALVE))
                    go->SetGoState(GO_STATE_ACTIVE_ALTERNATIVE);
                else
                    HandleGameObject(PutricideGateGUIDs[0],
                                     !(PutricideEventProgress &
                                       PUTRICIDE_EVENT_FLAG_FESTERGUT_VALVE),
                                     go);
                break;
            case GO_SCIENTIST_AIRLOCK_DOOR_GREEN:
                ...
                if ((PutricideEventProgress &
                    ...
                    HandleGameObject(PutricideGateGUIDs[1],
                                     !(PutricideEventProgress &
                                       PUTRICIDE_EVENT_FLAG_ROTFACE_VALVE),
                                     go);
                break;
            ...
            case GO_GAS_RELEASE_VALVE:
                ...
                if (GetBossState(DATA_FESTERGUT) != DONE)
                    go->SetGameObjectFlag(GO_FLAG_INTERACT_COND |
            ...
            case GO_CACHE_OF_THE_DREAMWALKER_25H:
                if (Creature* valithria =
                        instance->GetCreature(ValithriaDreamwalkerGUID))
            ...
            case GO_ARTHAS_PLATFORM:
                // this enables movement at The Frozen Throne, when printed this
                // value is 0.000000f however, when represented as integer
                // client will accept only this value
        ...
        void HandleDropAttempt(bool drop = true)
        {
            ...
            if (drop && HeroicAttempts) {
                --HeroicAttempts;
                DoUpdateWorldState(WORLDSTATE_ATTEMPTS_REMAINING,
                                   HeroicAttempts);
                SaveToDB();
            }
            if (HeroicAttempts)
                return;
            if (GetBossState(DATA_ROTFACE) == DONE &&
                GetBossState(DATA_FESTERGUT) == DONE)
                if (Creature* professor =
                        instance->GetCreature(ProfessorPutricideGUID))
        ...
        void RemoveBackPack()
        {
            for (auto const& itr : instance->GetPlayers())
                if (Player* _player = itr.GetSource())
                    _player->DestroyItemCount(
                        ITEM_GOBLIN_ROCKET_PACK,
                        _player->GetItemCount(ITEM_GOBLIN_ROCKET_PACK),
                        true);
        }
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            case DATA_LORD_MARROWGAR:
                if (state == DONE) {
                    WeeklyQuestId10 =
                        RAND(QUEST_BLOOD_QUICKENING_10,
                             QUEST_RESIDUE_RENDEZVOUS_10,
                             QUEST_RESPITE_FOR_A_TORMENTED_SOUL_10,
                             QUEST_DEPROGRAMMING_10,
                             QUEST_SECURING_THE_RAMPARTS_10);
                    SetData(DATA_WEEKLY_QUEST_ID,
                            0); // show required hidden npcs
                    if (GameObject* transporter = instance->GetGameObject(
                            ScourgeTransporterFirstGUID))
                        transporter->RemoveGameObjectFlag(
                            GO_FLAG_NOT_SELECTABLE);
                    SaveToDB();
                }
                break;
            ...
            case DATA_ICECROWN_GUNSHIP_BATTLE:
                if (state == DONE) {
                    if (GameObject* loot =
                            instance->GetGameObject(GunshipArmoryGUID)) {
                        loot->SetLootRecipient(instance);
                        loot->RemoveGameObjectFlag(GO_FLAG_LOCKED |
                                                   GO_FLAG_NOT_SELECTABLE |
                                                   GO_FLAG_NODESPAWN);
                    }
                }
            ...
            case DATA_DEATHBRINGER_SAURFANG:
                ...
                case DONE:
                    if (GameObject* loot =
                            instance->GetGameObject(DeathbringersCacheGUID)) {
                        if (Creature* deathbringer =
                                instance->GetCreature(DeathbringerSaurfangGUID))
                    ...
                    [[fallthrough]];
                case NOT_STARTED:
                    if (GameObject* teleporter =
                            instance->GetGameObject(SaurfangTeleportGUID)) {
                        HandleGameObject(
                            SaurfangTeleportGUID, true, teleporter);
            ...
            case DATA_FESTERGUT:
                if (state == DONE) {
                    if (GameObject* go =
                            instance->GetGameObject(GasReleaseValveGUID))
            ...
            case DATA_ROTFACE:
                if (state == DONE) {
                    if (GameObject* go =
                            instance->GetGameObject(OozeReleaseValveGUID))
                        go->RemoveGameObjectFlag(GO_FLAG_INTERACT_COND |
                                                 GO_FLAG_NOT_SELECTABLE);
                    if (GetBossState(DATA_FESTERGUT) == DONE)
                        HandleDropAttempt(false);
            ...
            case DATA_VALITHRIA_DREAMWALKER:
                if (state == DONE)
                    SetData(
                        ...
                        GetData(
                            DATA_WEEKLY_QUEST_ID)); // will show weekly quest
                                                    // npc if necessary
                break;
            case DATA_SINDRAGOSA:
                ...
                break;
            case DATA_THE_LICH_KING: {
                // dramatically increase visibility range during fight to seeing
                // frostmourne room
                instance->SetVisibilityRange(state == IN_PROGRESS ? 500.0f
                                                                  : 200.0f);
                ...
                if (state == DONE) {
                    if (GameObject* bolvar =
                            instance->GetGameObject(FrozenBolvarGUID))
                        bolvar->SetRespawnTime(7 * DAY);
                    if (GameObject* pillars =
                            instance->GetGameObject(PillarsChainedGUID))
                        pillars->SetRespawnTime(7 * DAY);
                    if (GameObject* pillars =
                            instance->GetGameObject(PillarsUnchainedGUID))
                        pillars->SetRespawnTime(7 * DAY);

                    instance->LoadGrid(JainaSpawnPos.GetPositionX(),
                                       JainaSpawnPos.GetPositionY());
                    instance->SummonCreature(NPC_LADY_JAINA_PROUDMOORE_QUEST,
                                             JainaSpawnPos);
                    instance->SummonCreature(NPC_MURADIN_BRONZEBEARD_QUEST,
                                             MuradinSpawnPos);
        ...
        void SpawnGunship()
        {
            if (!GunshipGUID && instance->HavePlayers()) {
                ...
                uint32 gunshipEntry = TeamIdInInstance == TEAM_HORDE
                                          ? GO_ORGRIMS_HAMMER_H
                                          : GO_THE_SKYBREAKER_A;
                if (MotionTransport* gunship = sTransportMgr->CreateTransport(
                        gunshipEntry, 0, instance)) {
        ...
        void SetData(uint32 type, uint32 data) override
        {
                ...
                if (!IsBuffAvailable) {
                    instance->DoForAllPlayers([&](Player* player) {
                        ...
                        for (Unit::ControlSet::const_iterator itr =
                            ...
                            for (Unit::AuraMap::iterator itra = am.begin();
                                 itra != am.end();)
            ...
            case DATA_WEEKLY_QUEST_ID:
                for (uint8 i = 0; i < WeeklyNPCs; ++i)
                    if (WeeklyQuestData[i].questId[0] == WeeklyQuestId10 &&
                        (WeeklyQuestData[i].creatureEntry !=
                             NPC_VALITHRIA_DREAMWALKER_QUEST ||
                         GetBossState(DATA_VALITHRIA_DREAMWALKER) ==
                             DONE) /*appears after killing valithria*/)
                        if (WeeklyQuestNpcGUID[i])
                            if (Creature* c = instance->GetCreature(
                                    WeeklyQuestNpcGUID[i]))
                                c->SetVisible(true);
                break;
            case DATA_PUTRICIDE_TRAP_STATE:
                if (data == NOT_STARTED) {
                    ...
                    if ((PutricideEventProgress &
                        ...
                        for (uint8 i = 0; i < 2; ++i)
                            if (GameObject* go = instance->GetGameObject(
                                    PutricideGateGUIDs[i]))
                                go->SetGoState(GO_STATE_ACTIVE_ALTERNATIVE);
                    }
                    else {
                        HandleGameObject(
                            PutricideGateGUIDs[0],
                            !(PutricideEventProgress &
                              PUTRICIDE_EVENT_FLAG_FESTERGUT_VALVE));
                        HandleGameObject(PutricideGateGUIDs[1],
                                         !(PutricideEventProgress &
            ...
            case DATA_BLOOD_QUICKENING_STATE: {
                ...
                case IN_PROGRESS:
                    ...
                    DoUpdateWorldState(WORLDSTATE_EXECUTION_TIME,
        ...
        bool CheckRequiredBosses(uint32        bossId,
                                 Player const* player) const override
        {
        ...
        bool CheckPlagueworks(uint32 bossId) const
        {
            ...
            case DATA_PROFESSOR_PUTRICIDE:
                if (GetBossState(DATA_FESTERGUT) != DONE ||
                    GetBossState(DATA_ROTFACE) != DONE)
                    return false;
                break;
            default:
                break;
        ...
        void CheckLichKingAvailability()
        {
            if (GetBossState(DATA_PROFESSOR_PUTRICIDE) == DONE &&
                ...
                GetBossState(DATA_SINDRAGOSA) == DONE) {
                if (GameObject* teleporter =
                        instance->GetGameObject(TheLichKingTeleportGUID)) {
                    teleporter->SetGoState(GO_STATE_ACTIVE);

                    std::list<Creature*> stalkers;
                    GetCreatureListWithEntryInGrid(
                        stalkers, teleporter, NPC_INVISIBLE_STALKER, 100.0f);
                    if (stalkers.empty())
                        return;

                    stalkers.sort(Acore::ObjectDistanceOrderPred(teleporter));
                    stalkers.front()->CastSpell(
                        (Unit*)nullptr,
                        SPELL_ARTHAS_TELEPORTER_CEREMONY,
                        false);
                    stalkers.pop_front();
                    for (std::list<Creature*>::iterator itr = stalkers.begin();
                         itr != stalkers.end();
                         ++itr)
                        (*itr)->AI()->Reset();
                }
        ...
        void WriteSaveDataMore(std::ostringstream& data) override
        {
            data << HeroicAttempts << ' ' << ColdflameJetsState << ' '
                 ...
                 << WeeklyQuestId10 << ' ' << PutricideEventProgress << ' '
        ...
        void Update(uint32 diff) override
        {
                    ...
                    if (Player* player = players.begin()->GetSource())
                        if (player->GetQuestStatus(QUEST_A_FEAST_OF_SOULS) ==
                            ...
                            if (Creature* theLichKing =
                                    instance->GetCreature(TheLichKingLhGUID)) {
                                theLichKing->AI()->Talk(
                                    SAY_SOULS_LICH_KING_RAND_WHISPER, player);
                            }
                        }
            }
            else
            ...
            if (DarkwhisperElevatorTimer <= diff) {
                DarkwhisperElevatorTimer = 3000;
                if (GetBossState(DATA_LADY_DEATHWHISPER) == DONE)
                    if (GameObject* elevator = instance->GetGameObject(
                            LadyDeathwisperElevatorGUID))
                        if (StaticTransport* trans =
                                elevator->ToStaticTransport()) {
                            if (trans->GetGoState() == GO_STATE_READY &&
                                trans->GetPathProgress() == 0)
                                trans->SetGoState(GO_STATE_ACTIVE);
                            else if (trans->GetGoState() == GO_STATE_ACTIVE &&
                                     trans->GetPathProgress() ==
                                         trans->GetPauseTime())
            ...
            while (uint32 eventId = Events.ExecuteEvent()) {
                ...
                case EVENT_UPDATE_EXECUTION_TIME: {
                    ...
                    if (BloodQuickeningMinutes) {
                        ...
                        DoUpdateWorldState(WORLDSTATE_SHOW_TIMER, 0);
                        if (Creature* bq =
                                instance->GetCreature(BloodQueenLanaThelGUID))
                ...
                case EVENT_QUAKE_SHATTER: {
                    if (GameObject* platform =
                            instance->GetGameObject(ArthasPlatformGUID))
                        platform->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
                    if (GameObject* edge =
                            instance->GetGameObject(FrozenThroneEdgeGUID))
                        edge->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* wind =
                            instance->GetGameObject(FrozenThroneWindGUID))
                        wind->SetGoState(GO_STATE_READY);
                    if (GameObject* warning =
                            instance->GetGameObject(FrozenThroneWarningGUID))
                        warning->SetGoState(GO_STATE_READY);
                    if (Creature* theLichKing =
                            instance->GetCreature(TheLichKingGUID))
                ...
                case EVENT_REBUILD_PLATFORM:
                    if (GameObject* platform =
                            instance->GetGameObject(ArthasPlatformGUID))
                        platform->SetDestructibleState(
                            GO_DESTRUCTIBLE_REBUILDING, nullptr, true);
                    if (GameObject* edge =
                            instance->GetGameObject(FrozenThroneEdgeGUID))
                ...
                case EVENT_RESPAWN_SINDRAGOSA:
                    if (!GetCreature(DATA_SINDRAGOSA)) {
                        if (Creature* sindragosa = instance->SummonCreature(
                                NPC_SINDRAGOSA, SindragosaSpawnPos)) {
                            sindragosa->setActive(true);
                            sindragosa->SetDisableGravity(true);
                            sindragosa->GetMotionMaster()->MovePath(
                                NPC_SINDRAGOSA * 10, true);

                            if (TempSummon* summon =
                                    sindragosa->ToTempSummon()) {
                                summon->SetTempSummonType(
                                    TEMPSUMMON_DEAD_DESPAWN);
                            }
                        }
                    }
                    // Could happen more than once if more than one player
                    // enters before she respawns.
        ...
        void ProcessEvent(WorldObject* source, uint32 eventId) override
        {
            ...
            case EVENT_TELEPORT_TO_FROSMOURNE: // Harvest Soul (normal mode)
                if (Creature* terenas = instance->SummonCreature(
                    ...
                    if (!triggers.empty()) {
                        triggers.sort(
                            Acore::ObjectDistanceOrderPred(terenas, false));
                        Unit* visual = triggers.front();
                        visual->CastSpell(
                            visual, SPELL_FROSTMOURNE_TELEPORT_VISUAL, true);
                    }

                    if (Creature* warden =
                            instance->SummonCreature(NPC_SPIRIT_WARDEN,
                                                     SpiritWardenSpawn,
                                                     nullptr,
                                                     65000)) {
        ...
        void SetPositionTraps(GameObject* go)
        {
            ...
            go->Relocate(
                Acore::Containers::SelectRandomContainerElement(trapPositions));
        }

    protected:
        // pussywizard:
        bool   IsBuffAvailable;
        uint32 WeeklyQuestId10; // contains id from 10man for any difficulty
                                // (for simplicity)
        ...
        ObjectGuid DeathbringerSaurfangEventGUID; // Muradin Bronzebeard or High
                                                  // Overlord Saurfang
...
void AddSC_instance_icecrown_citadel() { new instance_icecrown_citadel(); }
