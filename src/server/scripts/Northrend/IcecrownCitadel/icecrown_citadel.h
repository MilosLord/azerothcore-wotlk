...
enum PersistentData { DATA_SPIRE_FROSTWYRM = 0, MAX_DATA_INDEXES };
...
enum QuestsICC { QUEST_A_FEAST_OF_SOULS = 24547 };
...
enum ItemIds { ITEM_GOBLIN_ROCKET_PACK = 49278 };

class spell_trigger_spell_from_caster : public SpellScriptLoader {
public:
    spell_trigger_spell_from_caster(char const* scriptName, uint32 triggerId)
        : SpellScriptLoader(scriptName), _triggerId(triggerId)
    {
    }
    ...
    public:
        spell_trigger_spell_from_caster_SpellScript(uint32 triggerId)
            : SpellScript(), _triggerId(triggerId)
        {
        }
...
#define RegisterIcecrownCitadelCreatureAI(ai_name)                             \
    RegisterCreatureAIWithFactory(ai_name, GetIcecrownCitadelAI)
