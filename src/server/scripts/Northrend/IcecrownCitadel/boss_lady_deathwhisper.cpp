...
#define NPC_DARNAVAN                                                           \
    RAID_MODE<uint32>(                                                         \
        NPC_DARNAVAN_10, NPC_DARNAVAN_25, NPC_DARNAVAN_10, NPC_DARNAVAN_25)
#define NPC_DARNAVAN_CREDIT                                                    \
    RAID_MODE<uint32>(NPC_DARNAVAN_CREDIT_10,                                  \
                      NPC_DARNAVAN_CREDIT_25,                                  \
                      NPC_DARNAVAN_CREDIT_10,                                  \
                      NPC_DARNAVAN_CREDIT_25)
#define QUEST_DEPROGRAMMING                                                    \
    RAID_MODE<uint32>(QUEST_DEPROGRAMMING_10,                                  \
...
Position const SummonPositions[7] = {
    {-578.7066f,
     2154.167f,
     51.01529f,
     1.692969f}, // 1 Left Door 1 (Cult Fanatic)
    {-598.9028f,
     2155.005f,
     51.01530f,
     1.692969f}, // 2 Left Door 2 (Cult Adherent)
    {-619.2864f,
     2154.460f,
     51.01530f,
     1.692969f}, // 3 Left Door 3 (Cult Fanatic)
    {-578.6996f,
     2269.856f,
     51.01529f,
     4.590216f}, // 4 Right Door 1 (Cult Adherent)
    {-598.9688f,
     2269.264f,
     51.01529f,
     4.590216f}, // 5 Right Door 2 (Cult Fanatic)
    {-619.4323f,
     2268.523f,
     51.01530f,
     4.590216f}, // 6 Right Door 3 (Cult Adherent)
...
public:
    ...
    {
        _darnavan.GetMotionMaster()->MovePoint(POINT_DESPAWN,
                                               SummonPositions[6]);
        return true;
    }
...
public:
    ...
    struct boss_lady_deathwhisperAI : public BossAI {
        boss_lady_deathwhisperAI(Creature* creature)
            : BossAI(creature, DATA_LADY_DEATHWHISPER), _introDone(false)
        {
        }

        void Reset() override
        {
            if (Creature* darnavan =
                    ObjectAccessor::GetCreature(*me, _darnavanGUID))
        ...
        void AttackStart(Unit* victim) override
        {
            if (victim && me->Attack(victim, true) &&
                !(events.GetPhaseMask() & PHASE_ONE_MASK))
                me->GetMotionMaster()->MoveChase(victim);
        }

        void JustEngagedWith(Unit* who) override
        {
            if (!instance->CheckRequiredBosses(DATA_LADY_DEATHWHISPER,
                                               who->ToPlayer())) {
                EnterEvadeMode();
                         ...
                         SpellSchoolMask) override
        {
            if (events.GetPhaseMask() & PHASE_ONE_MASK &&
                ...
                for (ThreatContainer::StorageType::const_iterator itr =
                     ...
                     ++itr) {
                    Unit* unit =
                        ObjectAccessor::GetUnit((*me), (*itr)->getUnitGuid());
                ...
                events.ScheduleEvent(
                    EVENT_SPELL_FROSTBOLT, 10s, 12s, 0, PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SPELL_FROSTBOLT_VOLLEY, 19s, 21s, 0, PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SPELL_TOUCH_OF_INSIGNIFICANCE, 6s, 9s, 0, PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SPELL_SUMMON_SHADE, 12s, 15s, 0, PHASE_TWO);
                if (IsHeroic()) {
                    me->ApplySpellImmune(
                        0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, true);
                    me->ApplySpellImmune(
                        0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, true);
                    events.ScheduleEvent(
                        EVENT_SUMMON_WAVE_P2, 45s, 0, PHASE_TWO);
                }
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->HasUnitState(UNIT_STATE_CASTING) &&
                !(events.GetPhaseMask() & PHASE_INTRO_MASK))
                return;
            ...
            case EVENT_SPELL_DOMINATE_MIND_25: {
                ...
                for (Map::PlayerList::const_iterator itr = pList.begin();
                    ...
                    if (Player* plr = itr->GetSource())
                        if (plr->IsAlive() && !plr->IsGameMaster() &&
                            plr->GetExactDist2dSq(me) < (150.0f * 150.0f))
                            if (!me->GetVictim() ||
                                me->GetVictim()->GetGUID() != plr->GetGUID()) {
                                // shouldn't be casted on any victim of summoned
                                // mobs
                                bool valid = true;
                ...
                std::vector<Player*>::iterator begin = validPlayers.begin(),
                                               end   = validPlayers.end();

                std::random_device rd;
                std::shuffle(begin, end, std::default_random_engine{rd()});

                for (uint8 i = 0; i < RAID_MODE<uint8>(0, 1, 1, 3) &&
                                  i < validPlayers.size();
                     i++) {
            ...
            case EVENT_SPELL_TOUCH_OF_INSIGNIFICANCE:
                me->CastSpell(
                    me->GetVictim(), SPELL_TOUCH_OF_INSIGNIFICANCE, false);
                events.Repeat(6s, 9s);
                break;
            ...
            if (summon->GetEntry() == NPC_VENGEFUL_SHADE) {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                    ...
                    if (Player* p = itr->GetSource())
                        if (p != me->GetVictim() &&
                            summon->GetExactDist(p) < minrange &&
                            me->CanCreatureAttack(p)) {
                            target   = p;
        ...
        void JustDied(Unit* killer) override
        {
            ...
            for (SummonList::iterator itr = summons.begin();
                ...
                if (Unit* unit = ObjectAccessor::GetUnit(*me, *itr))
                    if (unit->IsAlive() &&
                        unit->GetEntry() != NPC_VENGEFUL_SHADE)
            ...
            if (Creature* darnavan =
                ...
                if (darnavan->IsAlive()) {
                    ...
                    darnavan->m_Events.AddEvent(
                        new DaranavanMoveEvent(*darnavan),
                        darnavan->m_Events.CalculateTime(10000));
        ...
        void DoAction(int32 action) override
        {
            ...
            if (!_introDone && !me->IsInCombat()) {
                ...
                events.ScheduleEvent(
                    EVENT_INTRO_4, 31s + 500ms, 0, PHASE_INTRO);
                events.ScheduleEvent(
                    EVENT_INTRO_5, 39s + 500ms, 0, PHASE_INTRO);
                events.ScheduleEvent(
                    EVENT_INTRO_6, 48s + 500ms, 0, PHASE_INTRO);
        ...
        void SummonWaveP1()
        {
            ...
            // Summon first add, replace it with Darnavan if weekly quest is
            // active
            if (_waveCounter || instance->GetData(DATA_WEEKLY_QUEST_ID) !=
                                    QUEST_DEPROGRAMMING_10)
                Summon(SummonEntries[addIndex], SummonPositions[addIndex * 3]);
            else
                Summon(NPC_DARNAVAN, SummonPositions[addIndex * 3]);

            Summon(SummonEntries[addIndexOther],
                   SummonPositions[addIndex * 3 + 1]);
            Summon(SummonEntries[addIndex], SummonPositions[addIndex * 3 + 2]);
            if (Is25ManRaid()) {
                Summon(SummonEntries[addIndexOther],
                       SummonPositions[addIndexOther * 3]);
                Summon(SummonEntries[addIndex],
                       SummonPositions[addIndexOther * 3 + 1]);
                Summon(SummonEntries[addIndexOther],
                       SummonPositions[addIndexOther * 3 + 2]);
        ...
        void SummonWaveP2()
        {
            if (Is25ManRaid()) {
                uint8 addIndex = _waveCounter & 1;
                Summon(SummonEntries[addIndex], SummonPositions[addIndex * 3]);
                Summon(SummonEntries[addIndex ^ 1],
                       SummonPositions[addIndex * 3 + 1]);
                Summon(SummonEntries[addIndex],
                       SummonPositions[addIndex * 3 + 2]);
            }
            else
                Summon(SummonEntries[urand(0, 1)], SummonPositions[6]);
        ...
        void Summon(uint32 entry, const Position& pos)
        {
            if (me->SummonCreature(
                    entry, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000))
                if (TempSummon* trigger = me->SummonCreature(
                        WORLD_TRIGGER, pos, TEMPSUMMON_TIMED_DESPAWN, 2000)) {
                    trigger->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
        ...
        void EmpowerCultist()
        {
            ...
            for (SummonList::iterator itr = summons.begin();
                ...
                if (Creature* cre = ObjectAccessor::GetCreature(*me, *itr))
                    if (cre->IsAlive() &&
                        (cre->GetEntry() == NPC_CULT_FANATIC ||
                         cre->GetEntry() == NPC_CULT_ADHERENT))
                        temp.push_back(cre);
            ...
            Creature* cultist =
                Acore::Containers::SelectRandomContainerElement(temp);
            if (!cultist)
                return;
            ...
            if (RAND(0, 1))
                ...
                me->CastSpell(cultist,
                              cultist->GetEntry() == NPC_CULT_FANATIC
                                  ? SPELL_DARK_TRANSFORMATION_T
                                  : SPELL_DARK_EMPOWERMENT_T,
                              true);
                Talk(uint8(cultist->GetEntry() == NPC_CULT_FANATIC
    ...
    struct npc_cult_fanaticAI : public ScriptedAI {
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
        {
            ...
            case SPELL_DARK_MARTYRDOM_FANATIC_25H:
                ...
                events.ScheduleEvent(EVENT_SPELL_CULTIST_DARK_MARTYRDOM,
                                     5ms); // Visual purposes only.
                break;
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_CULTIST_DARK_MARTYRDOM_REVIVE:
                ...
                if (Creature* ladyDeathwhisper = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_LADY_DEATHWHISPER)))
...
public:
    ...
    struct npc_cult_adherentAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            events.ScheduleEvent(
                EVENT_SPELL_ADHERENT_CURSE_OF_TORPOR, 14s, 16s);
            events.ScheduleEvent(
                EVENT_SPELL_ADHERENT_SHROUD_OF_THE_OCCULT, 32s, 39s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_ADHERENT_DEATHCHILL:
                if (me->GetEntry() == NPC_EMPOWERED_ADHERENT)
                    me->CastSpell(
                        me->GetVictim(), SPELL_DEATHCHILL_BLAST, false);
                else
                    me->CastSpell(
                        me->GetVictim(), SPELL_DEATHCHILL_BOLT, false);
                events.Repeat(9s, 13s);
                break;
...
public:
    ...
    struct npc_darnavanAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_canShatter && me->GetVictim() &&
                me->GetVictim()->IsImmunedToDamageOrSchool(
...
public:
    spell_deathwhisper_mana_barrier()
        : SpellScriptLoader("spell_deathwhisper_mana_barrier")
    {
    }
...
class spell_deathwhisper_dark_reckoning : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_deathwhisper_dark_reckoning::OnPeriodic,
                                 EFFECT_0,
...
public:
    at_lady_deathwhisper_entrance()
        : AreaTriggerScript("at_lady_deathwhisper_entrance")
    {
    }
                ...
                if (!player->IsGameMaster())
                    if (Creature* ladyDeathwhisper =
                            ObjectAccessor::GetCreature(
                                *player,
                                instance->GetGuidData(DATA_LADY_DEATHWHISPER)))
