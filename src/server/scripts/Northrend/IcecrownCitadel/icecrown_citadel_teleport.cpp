    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        if (go->GetEntry() != GO_SCOURGE_TRANSPORTER_FIRST)
            AddGossipItemFor(player,
                             gossipMenuId,
                             ICC_TELEPORT_GOSSIP_OPT_LIGHTS_HAMMER,
                             GOSSIP_SENDER_ICC_PORT,
                             LIGHT_S_HAMMER_TELEPORT); // M_PI + M_PI/6
        ...
        if (InstanceScript* instance = go->GetInstanceScript()) {
            if (instance->GetBossState(DATA_LORD_MARROWGAR) == DONE &&
                ...
                AddGossipItemFor(
                    player,
                    gossipMenuId,
                    ICC_TELEPORT_GOSSIP_OPT_ORATORY_OF_THE_DAMNED,
                    GOSSIP_SENDER_ICC_PORT,
                    ORATORY_OF_THE_DAMNED_TELEPORT); // M_PI + M_PI/6
            if (instance->GetBossState(DATA_LADY_DEATHWHISPER) == DONE &&
                go->GetEntry() != 202243)
                AddGossipItemFor(player,
                                 gossipMenuId,
                                 ICC_TELEPORT_GOSSIP_OPT_RAMPART_OF_SKULLS,
                                 GOSSIP_SENDER_ICC_PORT,
                                 RAMPART_OF_SKULLS_TELEPORT); // M_PI/6
            if (instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) == DONE &&
                go->GetEntry() != 202244)
                AddGossipItemFor(player,
                                 gossipMenuId,
                                 ICC_TELEPORT_GOSSIP_OPT_DEATHBRINGERS_RISE,
                                 GOSSIP_SENDER_ICC_PORT,
                                 DEATHBRINGER_S_RISE_TELEPORT); // M_PI/6
            if (instance->GetData(DATA_COLDFLAME_JETS) == DONE &&
                go->GetEntry() != 202235)
                AddGossipItemFor(player,
                                 gossipMenuId,
                                 ICC_TELEPORT_GOSSIP_OPT_UPPER_SPIRE,
                                 GOSSIP_SENDER_ICC_PORT,
                                 UPPER_SPIRE_TELEPORT); // M_PI/6
            if (instance->GetBossState(DATA_VALITHRIA_DREAMWALKER) == DONE &&
                instance->GetBossState(DATA_SINDRAGOSA_GAUNTLET) == DONE &&
                go->GetEntry() != 202246)
                AddGossipItemFor(
                    player,
                    gossipMenuId,
                    ICC_TELEPORT_GOSSIP_OPT_SINDRAGOSAS_LAIR,
                    GOSSIP_SENDER_ICC_PORT,
                    SINDRAGOSA_S_LAIR_TELEPORT); // M_PI*3/2 + M_PI/6
        }
                        ...
                        uint32 action) override
    {
        ...
        if (player->IsInCombat()) {
            Spell::SendCastResult(
                player, spell, 0, SPELL_FAILED_AFFECTING_COMBAT);
            return true;
        }
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        if (player->IsInCombat()) {
            if (SpellInfo const* spell =
                    sSpellMgr->GetSpellInfo(FROZEN_THRONE_TELEPORT))
                Spell::SendCastResult(
                    player, spell, 0, SPELL_FAILED_AFFECTING_COMBAT);
            return true;
        }
