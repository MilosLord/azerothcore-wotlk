...
enum Texts {
    ...
    SAY_SVALNA_CAPTAIN_DEATH =
        5, // happens when a captain resurrected by her dies
    SAY_SVALNA_DEATH           = 6,
    EMOTE_SVALNA_IMPALE        = 7,
    EMOTE_SVALNA_BROKEN_SHIELD = 8,

    SAY_CROK_INTRO_1 = 0,   // Ready your arms, my Argent Brothers. The Vrykul
                            // will protect the Frost Queen with their lives.
    SAY_ARNATH_INTRO_2 = 5, // Even dying here beats spending another day
                            // collecting reagents for that madman, Finklestein.
    SAY_CROK_INTRO_3 = 1,   // Enough idle banter! Our champions have arrived -
                            // support them as we push our way through the hall!
    SAY_SVALNA_EVENT_START =
        0, // You may have once fought beside me, Crok, but now you are nothing
           // more than a traitor. Come, your second death approaches!
    SAY_CROK_COMBAT_WP_0 = 2, // Draw them back to us, and we'll assist you.
    SAY_CROK_COMBAT_WP_1 = 3, // Quickly, push on!
    SAY_CROK_FINAL_WP    = 4, // Her reinforcements will arrive shortly, we must
                              // bring her down quickly!
    SAY_SVALNA_RESURRECT_CAPTAINS =
        2, // Foolish Crok. You brought my reinforcements with you. Arise,
           // Argent Champions, and serve the Lich King in death!
    SAY_CROK_COMBAT_SVALNA = 5, // I'll draw her attacks. Return our brothers to
                                // their graves, then help me bring her down!
    SAY_SVALNA_AGGRO = 3, // Come, Scourgebane. I'll show the master which of us
                          // is truly worthy of the title of "Champion"!
    SAY_CAPTAIN_DEATH           = 0,
...
#define SPELL_FLASH_HEAL                                                       \
    (IsUndead ? SPELL_FLASH_HEAL_UNDEAD : SPELL_FLASH_HEAL_NORMAL)
#define SPELL_POWER_WORD_SHIELD                                                \
    (IsUndead ? SPELL_POWER_WORD_SHIELD_UNDEAD : SPELL_POWER_WORD_SHIELD_NORMAL)
#define SPELL_SMITE (IsUndead ? SPELL_SMITE_UNDEAD : SPELL_SMITE_NORMAL)

// Captain Rupert
#define SPELL_FEL_IRON_BOMB                                                    \
    (IsUndead ? SPELL_FEL_IRON_BOMB_UNDEAD : SPELL_FEL_IRON_BOMB_NORMAL)
#define SPELL_MACHINE_GUN                                                      \
    (IsUndead ? SPELL_MACHINE_GUN_UNDEAD : SPELL_MACHINE_GUN_NORMAL)
#define SPELL_ROCKET_LAUNCH                                                    \
    (IsUndead ? SPELL_ROCKET_LAUNCH_UNDEAD : SPELL_ROCKET_LAUNCH_NORMAL)
...
public:
    npc_highlord_tirion_fordring_lh()
        : CreatureScript("npc_highlord_tirion_fordring_lh")
    {
    }
    ...
    struct npc_highlord_tirion_fordringAI : public ScriptedAI {
        ...
        void SetData(uint32 type, uint32 data) override
        {
            if (type == DATA_DAMNED_KILLS && data == 1) {
                if (++_damnedKills == 2) {
                    if (Creature* theLichKing = me->FindNearestCreature(
                            NPC_THE_LICH_KING_LH, 150.0f)) {
                        if (Creature* bolvarFordragon = me->FindNearestCreature(
                                NPC_HIGHLORD_BOLVAR_FORDRAGON_LH, 150.0f)) {
                            if (Creature* factionNPC = me->FindNearestCreature(
                                    _instance->GetData(
                                        DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_LK_INTRO_1:
                    ...
                    if (Creature* theLichKing =
                            ObjectAccessor::GetCreature(*me, _theLichKing))
                ...
                case EVENT_BOLVAR_INTRO_1:
                    if (Creature* bolvarFordragon = ObjectAccessor::GetCreature(
                            *me, _bolvarFordragon)) {
                ...
                case EVENT_SAURFANG_INTRO_1:
                    if (Creature* saurfang =
                            ObjectAccessor::GetCreature(*me, _factionNPC))
                ...
                case EVENT_SAURFANG_RUN:
                    if (Creature* factionNPC =
                            ObjectAccessor::GetCreature(*me, _factionNPC)) {
                        factionNPC->GetMotionMaster()->MovePath(
                            factionNPC->GetSpawnId() * 10, false);
                        factionNPC->DespawnOrUnsummon(46500);
                        std::list<Creature*> followers;
                        factionNPC->GetCreaturesWithEntryInRange(
                            followers,
                            30,
                            _instance->GetData(DATA_TEAMID_IN_INSTANCE) ==
                                    TEAM_HORDE
...
public:
    ...
    struct npc_rotting_frost_giantAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override { _events.Reset(); }
...
public:
    ...
    struct npc_crok_scourgebaneAI : public npc_escortAI {
        npc_crok_scourgebaneAI(Creature* creature)
            : npc_escortAI(creature), _instance(creature->GetInstanceScript())
        {
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_GAUNTLET) {
                ...
                if (Creature* svalna = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_SISTER_SVALNA)))
                    svalna->AI()->DoAction(ACTION_START_GAUNTLET);
                for (uint32 i = 0; i < 4; ++i)
                    if (Creature* crusader = ObjectAccessor::GetCreature(
                            *me,
                            _instance->GetGuidData(DATA_CAPTAIN_ARNATH + i)))
                ...
                _isEventDone =
                    _instance->GetBossState(DATA_SISTER_SVALNA) == DONE;
                me->setActive(false);
                _aliveTrash.clear();
                _currentWPid = 0;
                _handledWP4  = false;
            ...
            if (type == ACTION_VRYKUL_DEATH) {
                ...
                if (_aliveTrash.empty()) {
                    ...
                    if (_currentWPid == 4 && !_handledWP4) {
                        ...
                        if (Creature* svalna = ObjectAccessor::GetCreature(
                                *me,
                                _instance->GetGuidData(DATA_SISTER_SVALNA)))
        ...
        void WaypointStart(uint32 waypointId) override
        {
            ...
            if (minY) {
                ...
                Acore::CreatureListSearcher<FrostwingVrykulSearcher> searcher(
                    me, temp, check);
                Cell::VisitGridObjects(me, searcher, 150.0f);

                _aliveTrash.clear();
                for (std::list<Creature*>::iterator itr = temp.begin();
                     itr != temp.end();
                     ++itr)
                         ...
                         SpellSchoolMask) override
        {
            if (HealthBelowPct(10)) {
                if (!_didUnderTenPercentText) {
                    ...
                    if (me->GetVictim() &&
                        me->GetVictim()->GetEntry() == NPC_SISTER_SVALNA)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_wipeCheckTimer <= diff) {
                ...
                Acore::PlayerSearcher<Acore::AnyPlayerInObjectRangeCheck>
                    searcher(me, player, check);
                Cell::VisitWorldObjects(me, searcher, 140.0f);
                // wipe
                if (!player ||
                    me->GetExactDist(4357.0f, 2606.0f, 350.0f) > 125.0f) {
                    // Talk(SAY_CROK_DEATH);
                    FrostwingGauntletRespawner                        respawner;
                    Acore::CreatureWorker<FrostwingGauntletRespawner> worker(
                        me, respawner);
            ...
            case EVENT_ARNATH_INTRO_2:
                if (Creature* arnath = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_CAPTAIN_ARNATH)))
        ...
        bool CanAIAttack(Unit const* target) const override
        {
            ...
            return target->GetTypeId() != TYPEID_PLAYER &&
                   (me->GetPositionY() > 2660.0f) ==
                       (target->GetPositionY() > 2660.0f) &&
                   target->GetEntry() != NPC_SINDRAGOSA;
        }
...
public:
    ...
    struct boss_sister_svalnaAI : public BossAI {
        boss_sister_svalnaAI(Creature* creature)
            : BossAI(creature, DATA_SISTER_SVALNA)
        {
        }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Creature* crok = ObjectAccessor::GetCreature(
                    *me,
                    instance->GetGuidData(
                        DATA_CROK_SCOURGEBANE))) // _isEventDone = true,
                                                 // setActive(false)
            ...
            for (uint32 i = 0; i < 4; ++i)
                if (Creature* crusader = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(DATA_CAPTAIN_ARNATH + i)))
                    if (crusader->IsAlive()) {
                        if (crusader->GetEntry() ==
                            crusader->GetCreatureData()->id1) {
                            crusader->m_Events.AddEvent(
                                new CaptainSurviveTalk(*crusader),
                                crusader->m_Events.CalculateTime(delay));
                            delay += 6000;
                        }
                        else
        ...
        void JustEngagedWith(Unit* /*attacker*/) override
        {
            ...
            if (Creature* crok = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_CROK_SCOURGEBANE))) {
        ...
        void KilledUnit(Unit* victim) override
        {
            ...
            case TYPEID_PLAYER:
                ...
                break;
            /*case TYPEID_UNIT: // captains also say something on death and this
               causes spam switch (victim->GetEntry())
                {
                    case NPC_CAPTAIN_ARNATH:
                    case NPC_CAPTAIN_BRANDON:
                    case NPC_CAPTAIN_GRONDEL:
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_KILL_CAPTAIN:
                if (me->IsAlive())
                    me->CastCustomSpell(SPELL_CARESS_OF_DEATH,
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_HURL_SPEAR &&
                me->HasAura(SPELL_AETHER_SHIELD)) {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            case SPELL_IMPALING_SPEAR:
                if (TempSummon* summon =
                        target->SummonCreature(NPC_IMPALING_SPEAR, *target)) {
                    Talk(EMOTE_SVALNA_IMPALE, target);
                    summon->CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED,
                                            SPELLVALUE_BASE_POINT0,
                                            1,
                                            target,
                                            false);
                    summon->SetUnitFlag2(UNIT_FLAG2_HIDE_BODY |
...
public:
    ...
    {
        me->SetCorpseDelay(
            DAY); // leave corpse for a long time so svalna can resurrect
        IsUndead = (me->GetCreatureData() &&
                    me->GetCreatureData()->id1 != me->GetEntry());
    }
        ...
        if (action == ACTION_START_GAUNTLET) {
            if (Creature* crok = ObjectAccessor::GetCreature(
                ...
                me->GetMotionMaster()->MoveFollow(
                    crok, FollowDist, FollowAngle, MOTION_SLOT_IDLE);
            }
        }
    }
    ...
    {
        // do not see targets inside Frostwing Halls when we are not there
        return (me->GetPositionY() > 2660.0f) ==
                   (target->GetPositionY() > 2660.0f) &&
               (target->GetTypeId() == TYPEID_PLAYER || target->IsInCombat());
    }
...
public:
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ARNATH_PW_SHIELD: {
                std::list<Creature*> targets =
                    DoFindFriendlyMissingBuff(40.0f, SPELL_POWER_WORD_SHIELD);
                if (!targets.empty())
                    DoCast(Acore::Containers::SelectRandomContainerElement(
                               targets),
    ...
    private:
        Creature* FindFriendlyCreature() const
        {
            ...
            Acore::CreatureLastSearcher<Acore::MostHPMissingInRange> searcher(
                me, target, u_check);
        ...
        void Reset() override
        {
            ...
            if (IsUndead)
                Events.ScheduleEvent(
                    EVENT_BRANDON_HAMMER_OF_BETRAYAL, 25s, 30s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = Events.ExecuteEvent()) {
                ...
                case EVENT_BRANDON_CRUSADER_STRIKE:
                    ...
                    Events.ScheduleEvent(
                        EVENT_BRANDON_CRUSADER_STRIKE, 6s, 12s);
                    break;
                ...
                case EVENT_BRANDON_JUDGEMENT_OF_COMMAND:
                    ...
                    Events.ScheduleEvent(
                        EVENT_BRANDON_JUDGEMENT_OF_COMMAND, 8s, 13s);
                    break;
                case EVENT_BRANDON_HAMMER_OF_BETRAYAL:
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 1, 0.0f, true))
                        DoCast(target, SPELL_HAMMER_OF_BETRAYAL);
                    Events.ScheduleEvent(
                        EVENT_BRANDON_HAMMER_OF_BETRAYAL, 45s, 60s);
                    break;
                default:
                    break;
                }
...
public:
    ...
    struct npc_frostwing_vrykulAI : public ScriptedAI {
        ...
        {
            if (me->GetEntry() == NPC_YMIRJAR_DEATHBRINGER ||
                me->GetEntry() == NPC_YMIRJAR_FROSTBINDER ||
                me->GetEntry() == NPC_YMIRJAR_HUNTRESS)
                isRanged = true;
            else
                isRanged = false;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (!me->IsInCombat()) {
                ...
                case 100:
                    if (Creature* stalker = me->FindNearestCreature(
                            NPC_INVISIBLE_STALKER_3_0, 50.0f))
            ...
            if (me->HasUnitState(UNIT_STATE_CASTING) || me->isFeared() ||
                me->isFrozen() || me->HasUnitState(UNIT_STATE_STUNNED) ||
                me->HasUnitState(UNIT_STATE_CONFUSED) ||
                ((me->GetEntry() == NPC_YMIRJAR_DEATHBRINGER ||
                  me->GetEntry() == NPC_YMIRJAR_FROSTBINDER) &&
            ...
            {
                Unit* target =
                    SelectTarget(SelectTargetMethod::Random, 0, 35.0f, true);
            ...
            case 32: // Frozen Orb
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MinDistance, 0, 30.0f, true))
            ...
            if (me->GetEntry() == NPC_YMIRJAR_HUNTRESS && me->GetVictim() &&
                me->GetDistance(me->GetVictim()) > 5.0f)
        ...
        {
            if (spell->Id == 71306 &&
                c->GetTypeId() == TYPEID_UNIT) // Twisted Winds
            {
                Position myPos = me->GetPosition();
                me->NearTeleportTo(c->GetPositionX(),
                                   c->GetPositionY(),
                                   c->GetPositionZ(),
                                   c->GetOrientation());
                c->NearTeleportTo(myPos.GetPositionX(),
                                  myPos.GetPositionY(),
                                  myPos.GetPositionZ(),
                                  myPos.GetOrientation());
                const ThreatContainer::StorageType me_tl =
                    me->GetThreatMgr().GetThreatList();
                const ThreatContainer::StorageType target_tl =
                    c->GetThreatMgr().GetThreatList();
                DoResetThreatList();
                for (ThreatContainer::StorageType::const_iterator iter =
                         target_tl.begin();
                     iter != target_tl.end();
                     ++iter)
                    me->GetThreatMgr().AddThreat((*iter)->getTarget(),
                                                 (*iter)->GetThreat());
                ...
                for (ThreatContainer::StorageType::const_iterator iter =
                     ...
                     iter != me_tl.end();
                     ++iter)
                    c->GetThreatMgr().AddThreat((*iter)->getTarget(),
                                                (*iter)->GetThreat());
            }
        }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (InstanceScript* instance = creature->GetInstanceScript())
            if (instance->GetBossState(DATA_ROTFACE) == DONE &&
                instance->GetBossState(DATA_FESTERGUT) == DONE &&
                !creature->FindCurrentSpellBySpellId(
                    SPELL_HARVEST_BLIGHT_SPECIMEN) &&
                !creature->FindCurrentSpellBySpellId(
                    SPELL_HARVEST_BLIGHT_SPECIMEN25))
                if (player->HasAura(SPELL_ORANGE_BLIGHT_RESIDUE) &&
                    player->HasAura(SPELL_GREEN_BLIGHT_RESIDUE))
                    creature->CastSpell(
                        creature, SPELL_HARVEST_BLIGHT_SPECIMEN, false);
        return false;
    }
};
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        ...
        if (creature->GetExactDist2d(4357.052f, 2769.421f) < 100.0f &&
            creature->GetHomePosition().GetPositionZ() < 315.0f)
    ...
    class spell_icc_stoneform_AuraScript : public AuraScript {
                     ...
                     AuraEffectHandleModes /*mode*/)
        {
            if (Creature* target = GetTarget()->ToCreature()) {
                ...
                target->SetUInt32Value(UNIT_NPC_EMOTESTATE,
        ...
        {
            OnEffectApply +=
                AuraEffectApplyFn(spell_icc_stoneform_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_DUMMY,
                                  AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_icc_stoneform_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_DUMMY,
                                   AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    class spell_icc_sprit_alarm_SpellScript : public SpellScript {
        ...
        void HandleEvent(SpellEffIndex effIndex)
        {
            ...
            if (GameObject* trap =
                    GetCaster()->FindNearestGameObject(trapId, 5.0f)) {
            ...
            GetCaster()->GetCreatureListWithEntryInGrid(
                wards, NPC_DEATHBOUND_WARD, 150.0f);
            wards.sort(Acore::ObjectDistanceOrderPred(GetCaster()));
            for (std::list<Creature*>::iterator itr = wards.begin();
                 itr != wards.end();
                 ++itr) {
        ...
        {
            OnEffectHit +=
                SpellEffectFn(spell_icc_sprit_alarm_SpellScript::HandleEvent,
                              EFFECT_2,
    ...
    class spell_icc_geist_alarm_SpellScript : public SpellScript {
        ...
        void HandleEvent(SpellEffIndex effIndex)
        {
            ...
            if (InstanceScript* instance = GetCaster()->GetInstanceScript()) {
                ...
                if (Creature* l = instance->instance->SummonCreature(
                        NPC_VENGEFUL_FLESHREAPER, p)) {
                    ...
                    if ((target = l->SelectNearestTarget(20.0f)))
                        ...
                        l->GetMotionMaster()->MoveJump(l->GetPositionX(),
                                                       l->GetPositionY() +
                                                           55.0f,
                                                       l->GetPositionZ(),
                                                       20.0f,
        ...
        {
            OnEffectHit +=
                SpellEffectFn(spell_icc_geist_alarm_SpellScript::HandleEvent,
                              EFFECT_2,
                              SPELL_EFFECT_SEND_EVENT);
        }
    };
...
public:
    spell_frost_giant_death_plague()
        : SpellScriptLoader("spell_frost_giant_death_plague")
    {
    }
    ...
    class spell_frost_giant_death_plague_SpellScript : public SpellScript {
        ...
        void CountTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(
                Acore::ObjectGUIDCheck(GetCaster()->GetGUID(), true));
            ...
            if (kill)
                GetCaster()->CastSpell(
                    GetCaster(), SPELL_DEATH_PLAGUE_KILL, true);
            else {
                GetCaster()->CastSpell(
                    GetCaster(), SPELL_RECENTLY_INFECTED, true);
        ...
        {
            if (!GetHitUnit()->HasAura(SPELL_RECENTLY_INFECTED) &&
                !GetHitUnit()->HasAura(SPELL_DEATH_PLAGUE_AURA))
                GetHitUnit()->CastSpell(
                    GetHitUnit(), SPELL_DEATH_PLAGUE_AURA, true);
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_frost_giant_death_plague_SpellScript::CountTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ALLY);
            OnEffectHitTarget += SpellEffectFn(
                spell_frost_giant_death_plague_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_icc_harvest_blight_specimen()
        : SpellScriptLoader("spell_icc_harvest_blight_specimen")
    {
    }
    ...
    class spell_icc_harvest_blight_specimen_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_icc_harvest_blight_specimen_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
            OnEffectHitTarget +=
                SpellEffectFn(spell_icc_harvest_blight_specimen_SpellScript::
                                  HandleQuestComplete,
                              EFFECT_1,
...
public:
    spell_svalna_revive_champion()
        : SpellScriptLoader("spell_svalna_revive_champion")
    {
    }
    ...
    class spell_svalna_revive_champion_SpellScript : public SpellScript {
        ...
        void Land(SpellEffIndex /*effIndex*/)
        {
            ...
            pos.m_positionZ =
                caster->GetMap()->GetHeight(caster->GetPhaseMask(),
                                            pos.GetPositionX(),
                                            pos.GetPositionY(),
                                            caster->GetPositionZ(),
                                            true,
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_svalna_revive_champion_SpellScript::RemoveAliveTarget,
                EFFECT_0,
                TARGET_UNIT_DEST_AREA_ENTRY);
            OnEffectHit +=
                SpellEffectFn(spell_svalna_revive_champion_SpellScript::Land,
                              EFFECT_0,
                              SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    class spell_svalna_remove_spear_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_svalna_remove_spear_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    class spell_icc_soul_missile_SpellScript : public SpellScript {
        ...
        {
            OnCast +=
                SpellCastFn(spell_icc_soul_missile_SpellScript::RelocateDest);
        }
    };
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        ...
        if (!instance ||
            instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != DONE)
            return true;
        ...
        if (instance->GetData(DATA_COLDFLAME_JETS) == NOT_STARTED) {
            ...
            GetCreatureListWithEntryInGrid(
                traps, player, NPC_FROST_FREEZE_TRAP, 120.0f);
            traps.sort(Acore::ObjectDistanceOrderPred(player));
            bool instant = false;
            for (std::list<Creature*>::iterator itr = traps.begin();
                 itr != traps.end();
                 ++itr) {
...
public:
    at_icc_start_frostwing_gauntlet()
        : AreaTriggerScript("at_icc_start_frostwing_gauntlet")
    {
    }
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript())
            if (instance->GetBossState(DATA_SISTER_SVALNA) != DONE)
                if (Creature* crok = ObjectAccessor::GetCreature(
                        *player,
                        instance->GetGuidData(DATA_CROK_SCOURGEBANE))) {
                    if (!crok->IsAlive()) {
                        FrostwingGauntletRespawner respawner;
                        Acore::CreatureWorker<FrostwingGauntletRespawner>
                            worker(crok, respawner);
...
public:
    ...
    class spell_icc_web_wrap_AuraScript : public AuraScript {
        ...
        {
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_icc_web_wrap_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_MOD_ROOT,
                                   AURA_EFFECT_HANDLE_REAL);
        }
    ...
    class spell_icc_dark_reckoning_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_icc_dark_reckoning_AuraScript::OnPeriodic,
                EFFECT_0,
...
public:
    spell_stinky_precious_decimate()
        : SpellScriptLoader("spell_stinky_precious_decimate")
    {
    }
    ...
    class spell_stinky_precious_decimate_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            if (GetHitUnit()->GetHealthPct() > float(GetEffectValue())) {
                uint32 newHealth = GetHitUnit()->GetMaxHealth() *
                                   uint32(GetEffectValue()) / 100;
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_stinky_precious_decimate_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    class spell_icc_yh_volley_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_icc_yh_volley_AuraScript::HandleEffectPeriodic,
                EFFECT_1,
...
public:
    spell_icc_yd_summon_undead()
        : SpellScriptLoader("spell_icc_yd_summon_undead")
    {
    }
    ...
    class spell_icc_yd_summon_undead_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectLaunch += SpellEffectFn(
                spell_icc_yd_summon_undead_SpellScript::HandleDummyLaunch,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    npc_icc_skybreaker_hierophant()
        : CreatureScript("npc_icc_skybreaker_hierophant")
    {
    }

    struct npc_icc_skybreaker_hierophantAI : public ScriptedAI {
        npc_icc_skybreaker_hierophantAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        EventMap events;
...
public:
    npc_icc_skybreaker_marksman()
        : CreatureScript("npc_icc_skybreaker_marksman")
    {
    }
...
public:
    npc_icc_skybreaker_luminary()
        : CreatureScript("npc_icc_skybreaker_luminary")
    {
    }
...
public:
    ...
    struct npc_icc_valkyr_heraldAI : public ScriptedAI {
        ...
        void JustSummoned(Creature* s) override { summons.Summon(s); }

        void MoveInLineOfSight(Unit* who) override
        {
            if (me->IsAlive() && !me->IsInCombat() &&
                who->GetTypeId() == TYPEID_PLAYER &&
                who->GetExactDist2d(me) < 35.0f)
                AttackStart(who);
        }
                    ...
                    if (!target->IsClass(CLASS_DRUID))
                        if (Player* p = target->ToPlayer()) {
                            ...
                            target->CastSpell(
                                c,
                                60352,
                                true); // Mirror Image, clone visual appearance
                        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1: {
                ...
                for (uint8 i = 0; i < count; ++i)
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0, 37.5f, true)) {
                        casted = true;
            ...
            for (uint8 i = 0;; ++i) {
                if (sesi_spells[i].id) {
                    if (Class == sesi_spells[i].Class)
                        events.ScheduleEvent(i + 1,
                                             sesi_spells[i].cooldown_ms / 4);
                }
                else
                    break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 e = events.ExecuteEvent()) {
                ...
                if (sesi_spells[e - 1].targetType == 1)
                    ...
                    target = DoSelectLowestHpFriendly(sesi_spells[e - 1].range -
                                                      3.0f);
...
struct npc_icc_spire_frostwyrm : public ScriptedAI {
    ...
    void DoAction(int32 action) override
    {
        const Position posHordeMove = {
            -433.667084f, 2080.347412f, 191.253860f, 3.825093f};
        const Position posAllianceMove = {
            -433.589508f, 2344.564697f, 191.253616f, 2.543328f};

        bool hordeSide =
            action == HORDE_AREATRIGGER || action == HORDE_AREATRIGGER + 1;
    ...
    bool CanAIAttack(Unit const* target) const override
    {
        return me->GetPositionZ() < 225.0f &&
               me->GetHomePosition().GetExactDist(target) < 200.0f;
    }
...
public:
    npc_icc_vengeful_fleshreaper()
        : CreatureScript("npc_icc_vengeful_fleshreaper")
    {
    }

    struct npc_icc_vengeful_fleshreaperAI : public ScriptedAI {
        npc_icc_vengeful_fleshreaperAI(Creature* creature)
            : ScriptedAI(creature)
        {
        ...
        {
            if (currPipeWP != VENGEFUL_WP_COUNT &&
                (type == POINT_MOTION_TYPE || type == EFFECT_MOTION_TYPE) && id)
                needMove = true;
        }
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (currPipeWP == VENGEFUL_WP_COUNT)
                ...
                if (!me->IsInCombat() && who->GetTypeId() == TYPEID_PLAYER &&
                    me->GetExactDist2dSq(who) < 25.0f * 25.0f &&
                    me->CanSeeOrDetect(who) && me->IsValidAttackTarget(who))
                    AttackStart(who);
            }
        }
        ...
        void AttackStart(Unit* who) override
        {
            ...
            if (currPipeWP != VENGEFUL_WP_COUNT) {
                ...
                me->GetMotionMaster()->MoveJump(pos.GetPositionX(),
                                                pos.GetPositionY(),
                                                pos.GetPositionZ(),
                                                10.0f,
                                                6.0f,
        ...
        void UpdateAI(uint32 diff) override
        {
            if (needMove) {
                ...
                me->SetHomePosition(VengefulWP[currPipeWP].GetPositionX(),
            ...
            switch (events.ExecuteEvent()) {
            case 1:
                if (me->GetVictim() && !me->GetVictim()->HasAura(71163) &&
                    me->GetVictim()->GetDistance(me) > 5.0f &&
                    me->GetVictim()->GetDistance(me) < 30.0f) {
...
public:
                        ...
                        uint32 /*action*/) override
    {
        if ((creature->GetEntry() == NPC_GARROSH_HELLSCREAM &&
             player->PlayerTalkClass->GetGossipMenu().GetMenuId() == 11206) ||
            (creature->GetEntry() == NPC_KING_VARIAN_WRYNN &&
             player->PlayerTalkClass->GetGossipMenu().GetMenuId() == 11204)) {
            if (!player->GetGroup() || !player->GetGroup()->isRaidGroup() ||
                !player->GetGroup()->IsLeader(player->GetGUID())) {
                CloseGossipMenuFor(player);
                ChatHandler(player->GetSession())
                    .PSendSysMessage(
                        "Only the raid leader can turn off the buff.");
                return true;
            }
...
public:
    ...
    struct npc_icc_nerubar_broodkeeperAI : public ScriptedAI {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (!_didWebBeam && who->GetTypeId() == TYPEID_PLAYER &&
                me->GetExactDist2d(who) < 70.0f) {
                _didWebBeam = true;
        ...
        void JustEngagedWith(Unit* /*who*/) override { me->CallForHelp(15.0f); }
...
public:
    ...
    struct npc_icc_gauntlet_controllerAI : public NullCreatureAI {
        ...
        void SummonBroodling()
        {
            ...
            if (Creature* broodling = me->SummonCreature(
                    NPC_NERUBAR_BROODLING,
                    me->GetPositionX() + cos(o) * dist,
                    me->GetPositionY() + std::sin(o) * dist,
                    250.0f,
                    Position::NormalizeOrientation(o - M_PI))) {
                broodling->CastSpell(broodling, SPELL_WEB_BEAM2, false);
                broodling->GetMotionMaster()->MovePoint(
                    POINT_ENTER_COMBAT,
                    broodling->GetPositionX(),
                    broodling->GetPositionY(),
                    213.03f,
                    false);
            }
        }
        ...
        {
            for (uint8 i = 0; i < 3; ++i) {
                me->SummonCreature(i == 1 ? NPC_FROSTWARDEN_SORCERESS
        ...
        {
            me->SummonCreature(
                NPC_NERUBAR_CHAMPION, 4207.30f, 2532.00f, 256.0, 4.253f);
            me->SummonCreature(
                NPC_NERUBAR_WEBWEAVER, 4228.79f, 2510.36f, 256.0f, 3.577f);
            me->SummonCreature(
                NPC_NERUBAR_CHAMPION, 4228.34f, 2458.20f, 256.0f, 2.642f);
            me->SummonCreature(
                NPC_NERUBAR_WEBWEAVER, 4207.54f, 2437.18f, 256.0f, 2.073f);
            me->SummonCreature(
                NPC_NERUBAR_CHAMPION, 4156.20f, 2436.80f, 256.0f, 1.083f);
            me->SummonCreature(
                NPC_NERUBAR_WEBWEAVER, 4133.50f, 2459.28f, 256.0f, 0.483f);
            me->SummonCreature(
                NPC_NERUBAR_CHAMPION, 4134.28f, 2509.71f, 256.0f, 5.788f);
            me->SummonCreature(
                NPC_NERUBAR_WEBWEAVER, 4156.29f, 2532.19f, 256.0f, 5.187f);
        }
            ...
            for (SummonList::const_iterator itr = summons.begin();
                ...
                if (Creature* spider = ObjectAccessor::GetCreature(*me, *itr))
                    if (spider->GetPositionZ() > 220.0f) {
                        ...
                        spider->GetMotionMaster()->MoveLand(
                            POINT_ENTER_COMBAT,
                            spider->GetPositionX(),
                            spider->GetPositionY(),
                            213.03f,
        ...
        void SummonedCreatureDies(Creature* summon, Unit*) override
        {
            ...
            if (summon->GetEntry() != NPC_NERUBAR_BROODLING &&
                summons.GetEntryCount(NPC_NERUBAR_BROODLING) ==
                    summons.size()) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_CHECK_FIGHT: {
                ...
                for (Map::PlayerList::const_iterator itr = pList.begin();
                     itr != pList.end();
                     ++itr) {
                    if (me->GetDistance(itr->GetSource()) > 100.0f ||
...
public:
    ...
    struct npc_icc_putricades_trapAI : public NullCreatureAI {
        npc_icc_putricades_trapAI(Creature* creature)
            : NullCreatureAI(creature), summons(me)
        {
            instance = creature->GetInstanceScript();
        }
...
public:
    ...
    {
        if (InstanceScript* instance = player->GetInstanceScript())
            if (instance->GetBossState(DATA_SINDRAGOSA_GAUNTLET) ==
                    NOT_STARTED &&
                !player->IsGameMaster())
                if (Creature* gauntlet = ObjectAccessor::GetCreature(
                        *player,
                        instance->GetGuidData(NPC_SINDRAGOSA_GAUNTLET)))
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript())
            if (instance->GetData(DATA_PUTRICIDE_TRAP_STATE) == NOT_STARTED &&
                !player->IsGameMaster())
                if (Creature* trap = ObjectAccessor::GetCreature(
                        *player, instance->GetGuidData(NPC_PUTRICADES_TRAP)))
...
public:
    at_icc_spire_frostwyrm()
        : OnlyOnceAreaTriggerScript("at_icc_spire_frostwyrm")
    {
    }

    bool _OnTrigger(Player* player, AreaTrigger const* areaTrigger) override
    {
        if (player->GetInstanceScript()->GetPersistentData(
                DATA_SPIRE_FROSTWYRM) != DONE) {
            player->GetInstanceScript()->StorePersistentData(
                DATA_SPIRE_FROSTWYRM, DONE);
            const Position posHordeWyrm = {
                -375.538879f, 2120.774658f, 242.256775f, 3.714352f};
            const Position posAllianceWyrm = {
                -361.154358f, 2305.821289f, 244.771713f, 2.704335f};

            bool hordeSide = areaTrigger->entry == HORDE_AREATRIGGER ||
                             areaTrigger->entry == HORDE_AREATRIGGER + 1;

            if (Creature* frostwyrm = player->SummonCreature(
                    NPC_SPIRE_FROSTWYRM,
                    hordeSide ? posHordeWyrm : posAllianceWyrm)) {
...
void AddSC_icecrown_citadel()
{
    ...
    new spell_trigger_spell_from_caster("spell_svalna_caress_of_death",
