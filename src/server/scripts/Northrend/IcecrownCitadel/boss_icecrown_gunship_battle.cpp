...
Position const SkybreakerTeleportPortal = {
    6.666975f, 0.013001f, 20.87888f, 0.0f};
Position const OrgrimsHammerTeleportExit = {
    7.461699f, 0.158853f, 35.72989f, 0.0f};

// Alliance encounter
Position const OrgrimsHammerTeleportPortal = {
    47.550990f, -0.101778f, 37.61111f, 0.0f};
Position const SkybreakerTeleportExit = {
    -17.55738f, -0.090421f, 21.18366f, 0.0f};
...
SlotInfo const OrgrimsHammerSlotInfo[MAX_SLOTS] = {
    ...
    {NPC_KOR_KRON_ROCKETEER,
     ...
     30},
    {NPC_KOR_KRON_ROCKETEER,
     {27.62149000f, -23.48108f, 36.12708f, 1.518436f},
     30},
...
public:
    ...
    bool Execute(uint64 timer, uint32 /*diff*/) override
    {
        ...
        if (_level < (_creature->GetMap()->IsHeroic() ? 4 : 3)) {
            _creature->m_Events.AddEvent(this,
                                         timer + ExperiencedTimes[_level]);
            return false;
        }
...
uint32 const BattleExperienceEvent::ExperiencedSpells[5] = {
    0, SPELL_EXPERIENCED, SPELL_VETERAN, SPELL_ELITE, SPELL_ADDS_BERSERK};
uint32 const BattleExperienceEvent::ExperiencedTimes[5] = {
    100000, 70000, 60000, 90000, 0};

class PassengerController {
public:
    PassengerController() { ResetSlots(TEAM_HORDE, nullptr); }
    ...
    void ResetSlots(TeamId teamId, MotionTransport* t)
    {
        ...
        _spawnPoint = teamId == TEAM_HORDE ? &OrgrimsHammerAddsSpawnPos
                                           : &SkybreakerAddsSpawnPos;
        _slotInfo =
            teamId == TEAM_HORDE ? OrgrimsHammerSlotInfo : SkybreakerSlotInfo;
    }

    bool SummonCreatures(Creature*      summoner,
                         ...
                         PassengerSlots last)
    {
        ...
        for (int32 i = first; i <= last; ++i) {
            ...
            if (_controlledSlots[i]) {
                Creature* current = ObjectAccessor::GetCreature(
                    *_transport, _controlledSlots[i]);
                if (current && current->IsAlive())
                    continue;
            }

            Position spawnPos = SelectSpawnPoint();
            if (Creature* passenger =
                    summoner->SummonCreature(_slotInfo[i].Entry,
                                             spawnPos,
                                             TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                                             15000)) {
                if (!passenger->GetTransport())
                    _transport->AddPassenger(passenger, true);
                passenger->ApplySpellImmune(
                    0, IMMUNITY_ID, 49576, true); // death grip

                if (i >= SLOT_MAGE_1 &&
                    i <= SLOT_MORTAR_4) // only these npcs are pooled
                {
    ...
    void ClearSlot(PassengerSlots slot)
    {
        ...
        _respawnCooldowns[slot] =
            GameTime::GetGameTime().count() + _slotInfo[slot].Cooldown;
    }
...
private:
    Position SelectSpawnPoint() const
    {
        ...
        Position newPos;
        newPos.m_positionX =
            _spawnPoint->GetPositionX() + 2.0f * std::cos(angle);
        newPos.m_positionY =
            _spawnPoint->GetPositionY() + 2.0f * std::sin(angle);
        newPos.m_positionZ = _spawnPoint->GetPositionZ();
        newPos.SetOrientation(_spawnPoint->GetOrientation());
        _transport->CalculatePassengerPosition(newPos.m_positionX,
                                               newPos.m_positionY,
                                               newPos.m_positionZ,
                                               &(newPos.m_orientation));
        return newPos;
    }
...
public:
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        _owner->GetMotionMaster()->MovePoint(
            EVENT_CHARGE_PREPATH, x, y, z, false);
        return true;
    }
...
public:
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        _caster->GetTransport()
            ->ToMotionTransport()
            ->UnloadNonStaticPassengers();
        _caster->GetTransport()->AddObjectToRemoveList();

        if (Transport* transport =
                ObjectAccessor::GetTransport(*_caster, _otherTransport)) {
    ...
    struct npc_gunshipAI : public NullCreatureAI {
        npc_gunshipAI(Creature* creature)
            : NullCreatureAI(creature),
              ...
              _teamIdInInstance(TeamId(creature->GetInstanceScript()->GetData(
                  DATA_TEAMID_IN_INSTANCE))),
              _died(false), _summonedFirstMage(false)
        {
        ...
        {
            if (_instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) !=
                IN_PROGRESS) {
                damage = 0;
                return;
            }
            ...
            if (me->GetTransport()->GetEntry() !=
                uint32(_teamIdInInstance == TEAM_HORDE ? GO_THE_SKYBREAKER_H
                                                       : GO_ORGRIMS_HAMMER_A))
                return;
            ...
            if (Creature* captain = me->FindNearestCreature(
                    _teamIdInInstance == TEAM_HORDE
        ...
        {
            if (_died || _instance->GetBossState(
                             DATA_ICECROWN_GUNSHIP_BATTLE) != IN_PROGRESS)
                return;
            _died = true;

            bool isVictory =
                me->GetTransport()->GetEntry() == GO_THE_SKYBREAKER_H ||
                me->GetTransport()->GetEntry() == GO_ORGRIMS_HAMMER_A;
            _instance->SetBossState(DATA_ICECROWN_GUNSHIP_BATTLE,
                                    isVictory ? DONE : FAIL);
            me->GetMap()->SetZoneMusic(AREA_ICECROWN_CITADEL, 0);

            if (Creature* creature = me->FindNearestCreature(
                    me->GetEntry() == NPC_ORGRIMS_HAMMER ? NPC_THE_SKYBREAKER
                                                         : NPC_ORGRIMS_HAMMER,
                    200.0f)) {
                _instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE,
                                             creature);
            ...
            uint32 explosionSpell =
                isVictory ? SPELL_EXPLOSION_VICTORY : SPELL_EXPLOSION_WIPE;
            if (MotionTransport* t =
                    (me->GetTransport()
                         ? me->GetTransport()->ToMotionTransport()
                         : nullptr)) {
                Transport::PassengerSet const& passengers =
                    t->GetStaticPassengers();
                for (Transport::PassengerSet::const_iterator itr =
                         passengers.begin();
                     itr != passengers.end();
                     ++itr) {
                    if ((*itr)->GetTypeId() != TYPEID_UNIT ||
                        (*itr)->GetEntry() != NPC_GUNSHIP_HULL)
                        continue;
                    (*itr)->ToCreature()->CastSpell(
                        (*itr)->ToCreature(), explosionSpell, true);
                }
            }

            uint32 cannonEntry = _teamIdInInstance == TEAM_HORDE
                                     ? NPC_HORDE_GUNSHIP_CANNON
                                     : NPC_ALLIANCE_GUNSHIP_CANNON;
            if (GameObject* go = _instance->instance->GetGameObject(
                    _instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
                if (MotionTransport* t = go->ToMotionTransport()) {
                    ...
                    for (Transport::PassengerSet::const_iterator itr =
                         ...
                         ++itr) {
                        if ((*itr)->GetTypeId() != TYPEID_UNIT ||
                            (*itr)->GetEntry() != cannonEntry)
                            continue;
                        Creature* cannon = (*itr)->ToCreature();
                        cannon->CastSpell(
                            cannon, SPELL_EJECT_ALL_PASSENGERS, true);

                        WorldPacket data(SMSG_PLAYER_VEHICLE_DATA,
                                         cannon->GetPackGUID().size() + 4);
            ...
            if (Creature* creature =
                    me->FindNearestCreature(creatureEntry, 200.0f))
                creature->AI()->Talk(textId);

            if (isVictory) {
                if (Transport* transport = _instance->instance->GetTransport(
                        _instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
                    if (MotionTransport* otherTransport =
                            transport->ToMotionTransport())
                        otherTransport->EnableMovement(true);

                me->GetTransport()->ToMotionTransport()->EnableMovement(true);

                if (Creature* ship = me->FindNearestCreature(
                        _teamIdInInstance == TEAM_HORDE ? NPC_ORGRIMS_HAMMER
                                                        : NPC_THE_SKYBREAKER,
                        200.0f)) {
                    ship->CastSpell(
                        ship, SPELL_TELEPORT_PLAYERS_ON_VICTORY, true);
                    ship->CastSpell(ship, SPELL_ACHIEVEMENT, true);
                    ship->CastSpell(
                        ship, SPELL_AWARD_REPUTATION_BOSS_KILL, true);
                }

                for (uint8 i = 0; i < 2; ++i)
                    if (GameObject* go = _instance->instance->GetGameObject(
                            _instance->GetGuidData(
                                i == 0 ? DATA_ICECROWN_GUNSHIP_BATTLE
                                       : DATA_ENEMY_GUNSHIP)))
                        if (MotionTransport* t = go->ToMotionTransport()) {
                            Transport::PassengerSet const& passengers =
                                t->GetPassengers();
                            for (Transport::PassengerSet::const_iterator itr =
                                ...
                                if (c->GetEntry() == NPC_SKYBREAKER_MARINE ||
                                    c->GetEntry() == NPC_SKYBREAKER_SERGEANT ||
                                    c->GetEntry() == NPC_KOR_KRON_REAVER ||
                                    c->GetEntry() == NPC_KOR_KRON_SERGEANT)
                ...
                me->m_Events.AddEvent(
                    new ResetEncounterEvent(
                        me,
                        teleportSpellId,
                        _instance->GetGuidData(DATA_ENEMY_GUNSHIP)),
        ...
        uint32 GetData(uint32 id) const override
        {
            ...
            for (std::map<ObjectGuid, uint32>::const_iterator itr =
                     _shipVisits.begin();
                 itr != _shipVisits.end();
                 ++itr)
                if (itr->second == 0)
...
public:
    npc_high_overlord_saurfang_igb()
        : CreatureScript("npc_high_overlord_saurfang_igb")
    {
    }

    struct npc_high_overlord_saurfang_igbAI : public ScriptedAI {
        npc_high_overlord_saurfang_igbAI(Creature* creature)
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
            _events.Reset();
            _controller.ResetSlots(
                TEAM_HORDE, creature->GetTransport()->ToMotionTransport());
            me->SetRegeneratingHealth(false);
        ...
        void JustEngagedWith(Unit* /*target*/) override
        {
            if (_instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE &&
                !me->HasAura(SPELL_FRIENDLY_BOSS_DAMAGE_MOD))
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_ENEMY_GUNSHIP_TALK) {
                ...
                if (Creature* muradin = me->FindNearestCreature(
                        NPC_IGB_MURADIN_BRONZEBEARD, 200.0f))
                ...
                if (Creature* skybreaker =
                        me->FindNearestCreature(NPC_THE_SKYBREAKER, 200.0f))
                    _instance->SendEncounterUnit(
                        ENCOUNTER_FRAME_ENGAGE, skybreaker, 1);
                if (Creature* orgrimsHammer =
                        me->FindNearestCreature(NPC_ORGRIMS_HAMMER, 200.0f)) {
                    _instance->SendEncounterUnit(
                        ENCOUNTER_FRAME_ENGAGE, orgrimsHammer, 2);
                    orgrimsHammer->CastSpell(
                        orgrimsHammer, SPELL_CHECK_FOR_PLAYERS, true);
                }
            }
            else if (action == ACTION_SPAWN_MAGE) {
                time_t now = GameTime::GetGameTime().count();
                if (_firstMageCooldown > now)
                    _events.ScheduleEvent(EVENT_SUMMON_MAGE,
                                          (_firstMageCooldown - now) *
                                              IN_MILLISECONDS);
                else
                ...
                if (Is25ManRaid())
                    ...
                    _controller.SummonCreatures(
                        me, SLOT_MORTAR_1, SLOT_MORTAR_2);
                    _controller.SummonCreatures(
                        me, SLOT_RIFLEMAN_1, SLOT_RIFLEMAN_4);
                }
            }
                ...
                Movement::PointsArray path(points,
                                           points + SaurfangExitPathSize);
                me->SetWalk(true);
                         ...
                         SpellSchoolMask) override
        {
            if (me->HealthBelowPctDamaged(65, damage) &&
                !me->HasAura(SPELL_TASTE_OF_BLOOD))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INTRO_SUMMON_SKYBREAKER:
                sTransportMgr->CreateTransport(
                    GO_THE_SKYBREAKER_H, 0, me->GetMap());
                break;
            ...
            case EVENT_SUMMON_MAGE:
                ...
                _controller.SummonCreatures(
                    me, SLOT_FREEZE_MAGE, SLOT_FREEZE_MAGE);
                break;
            case EVENT_ADDS:
                Talk(SAY_SAURFANG_ENTER_SKYBREAKER);
                _controller.SummonCreatures(me, SLOT_MAGE_1, SLOT_MAGE_2);
                _controller.SummonCreatures(me,
                                            SLOT_MARINE_1,
                                            Is25ManRaid() ? SLOT_MARINE_4
                                                          : SLOT_MARINE_2);
                _controller.SummonCreatures(me,
                                            SLOT_SERGEANT_1,
                                            Is25ManRaid() ? SLOT_SERGEANT_2
                                                          : SLOT_SERGEANT_1);
                if (MotionTransport* orgrimsHammer =
                        (me->GetTransport()
                             ? me->GetTransport()->ToMotionTransport()
                             : nullptr)) {
                    float x, y, z, o;
                    ...
                    me->SummonCreature(NPC_TELEPORT_PORTAL,
                                       x,
                                       y,
                                       z,
                                       o,
            ...
            case EVENT_CHECK_RIFLEMAN:
                if (_controller.SummonCreatures(
                        me,
                        SLOT_RIFLEMAN_1,
                        Is25ManRaid() ? SLOT_RIFLEMAN_8 : SLOT_RIFLEMAN_4)) {
                    if (_axethrowersYellCooldown <
                        GameTime::GetGameTime().count()) {
                        Talk(SAY_SAURFANG_AXETHROWERS);
                        _axethrowersYellCooldown =
                            GameTime::GetGameTime().count() + 5;
                    }
                }
            ...
            case EVENT_CHECK_MORTAR:
                if (_controller.SummonCreatures(
                        me,
                        SLOT_MORTAR_1,
                        Is25ManRaid() ? SLOT_MORTAR_4 : SLOT_MORTAR_2)) {
                    if (_rocketeersYellCooldown <
                        GameTime::GetGameTime().count()) {
                        Talk(SAY_SAURFANG_ROCKETEERS);
                        _rocketeersYellCooldown =
                            GameTime::GetGameTime().count() + 5;
                    }
                }
        ...
        bool CanAIAttack(Unit const* target) const override
        {
            if (_instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) !=
                IN_PROGRESS)
                return false;
            if (target->GetEntry() == NPC_SKYBREAKER_MARINE ||
                target->GetEntry() == NPC_SKYBREAKER_SERGEANT)
                return target->ToCreature()->GetReactState() != REACT_PASSIVE;
            return target->GetTransport() == me->GetTransport() &&
                   target->GetPositionY() <
...
public:
    npc_muradin_bronzebeard_igb()
        : CreatureScript("npc_muradin_bronzebeard_igb")
    {
    }
    ...
    struct npc_muradin_bronzebeard_igbAI : public ScriptedAI {
            ...
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
            _events.Reset();
            _controller.ResetSlots(
                TEAM_ALLIANCE, creature->GetTransport()->ToMotionTransport());
            me->SetRegeneratingHealth(false);
            me->m_CombatDistance  = 70.0f;
            _firstMageCooldown    = GameTime::GetGameTime().count() + 45;
        ...
        void JustEngagedWith(Unit* /*target*/) override
        {
            if (_instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_ALLIANCE &&
                !me->HasAura(SPELL_FRIENDLY_BOSS_DAMAGE_MOD))
                me->CastSpell(me, SPELL_FRIENDLY_BOSS_DAMAGE_MOD, true);
            if (!me->HasAura(SPELL_BATTLE_FURY))
                me->CastSpell(me, SPELL_BATTLE_FURY, true);
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_ENEMY_GUNSHIP_TALK) {
                ...
                if (Creature* saurfang = me->FindNearestCreature(
                        NPC_IGB_HIGH_OVERLORD_SAURFANG, 200.0f))
                ...
                if (Creature* orgrimsHammer =
                        me->FindNearestCreature(NPC_ORGRIMS_HAMMER, 200.0f))
                    _instance->SendEncounterUnit(
                        ENCOUNTER_FRAME_ENGAGE, orgrimsHammer, 1);
                if (Creature* skybreaker =
                        me->FindNearestCreature(NPC_THE_SKYBREAKER, 200.0f)) {
                    _instance->SendEncounterUnit(
                        ENCOUNTER_FRAME_ENGAGE, skybreaker, 2);
                    skybreaker->CastSpell(
                        skybreaker, SPELL_CHECK_FOR_PLAYERS, true);
                }
            }
            else if (action == ACTION_SPAWN_MAGE) {
                time_t now = GameTime::GetGameTime().count();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INTRO_SUMMON_ORGRIMS_HAMMER:
                sTransportMgr->CreateTransport(
                    GO_ORGRIMS_HAMMER_A, 0, me->GetMap());
                break;
        ...
        bool CanAIAttack(Unit const* target) const override
        {
            if (_instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) !=
                IN_PROGRESS)
                return false;
            if (target->GetEntry() == NPC_KOR_KRON_REAVER ||
                target->GetEntry() == NPC_KOR_KRON_SERGEANT)
                return target->ToCreature()->GetReactState() != REACT_PASSIVE;
            return target->GetTransport() == me->GetTransport() &&
                   target->GetPositionY() >
                       (_instance->GetData(DATA_TEAMID_IN_INSTANCE) ==
...
public:
    ...
    struct npc_igb_ship_crewAI : public ScriptedAI {
        npc_igb_ship_crewAI(Creature* creature)
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
        }
        ...
        bool CanAIAttack(Unit const* target) const override
        {
            return _instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) ==
                       IN_PROGRESS &&
                   target->GetTransport() == me->GetTransport() &&
                   target->GetPositionZ() < 478.0f &&
                   (me->GetEntry() == NPC_SKYBREAKER_DECKHAND
                        ? (target->GetPositionY() > 2042.0f)
                        : (target->GetPositionY() < 2431.0f));
        }
...
void TriggerBurningPitch(Creature* c)
{
    ...
    uint32          spellId = i->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE
...
struct gunship_npc_AI : public ScriptedAI {
    ...
    void JustDied(Unit* /*killer*/) override
    {
        if (Slot)
            if (Creature* captain = me->FindNearestCreature(
                    Instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE
                        ? NPC_IGB_MURADIN_BRONZEBEARD
                        : NPC_IGB_HIGH_OVERLORD_SAURFANG,
                    200.0f))
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        if (type == POINT_MOTION_TYPE && pointId == EVENT_CHARGE_PREPATH &&
            ...
            me->m_Events.AddEvent(
                new BattleExperienceEvent(me),
                me->m_Events.CalculateTime(
                    BattleExperienceEvent::ExperiencedTimes[0]));
...
protected:
    ...
    {
        Index = index;
        Slot  = &((Instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE
                       ? SkybreakerSlotInfo
                       : OrgrimsHammerSlotInfo)[Index]);
    }
...
struct npc_gunship_boarding_addAI : public ScriptedAI {
    ...
    void SetData(uint32 type, uint32 data) override
    {
        if (type == ACTION_SET_SLOT) {
            ...
            me->SetReactState(REACT_PASSIVE);
            me->m_Events.AddEvent(
                new DelayedMovementEvent(me, Slot->TargetPosition),
                me->m_Events.CalculateTime(3000 * (Index - SLOT_MARINE_1)));
        }
    }
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        if (type == POINT_MOTION_TYPE && pointId == EVENT_CHARGE_PREPATH &&
            ...
            Position const& otherTransportPos =
                Instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE
            ...
            if (Transport* transport = Instance->instance->GetTransport(
                    Instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
    ...
    bool CanAIAttack(Unit const* target) const override
    {
        return Instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) ==
                   IN_PROGRESS &&
               target->GetTransport() &&
               target->GetTransport() != me->GetTransport() &&
               target->GetPositionZ() < 478.0f &&
               (me->GetEntry() == NPC_SKYBREAKER_SERGEANT ||
                        me->GetEntry() == NPC_SKYBREAKER_MARINE
                    ? (target->GetPositionY() < 2431.0f)
                    : (target->GetPositionY() > 2042.0f));
    }
...
public:
    ...
    struct npc_gunship_boarding_leaderAI : public npc_gunship_boarding_addAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (Instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) !=
                IN_PROGRESS)
                return;
...
public:
    ...
    struct npc_gunship_boarding_add_realAI : public npc_gunship_boarding_addAI {
        npc_gunship_boarding_add_realAI(Creature* creature)
            : npc_gunship_boarding_addAI(creature)
        {
        }
...
public:
    ...
    struct npc_gunship_mageAI : public gunship_npc_AI {
        ...
        void AttackStart(Unit* target) override { me->Attack(target, false); }
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
            if (type == POINT_MOTION_TYPE && pointId == EVENT_CHARGE_PREPATH &&
                ...
                case SLOT_MAGE_2:
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_SHADOW_CHANNELING, false);
                    break;
                default:
                    break;
                }
...
public:
    ...
    struct npc_gunship_gunnerAI : public gunship_npc_AI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!me->GetVictim())
                return;
            DoSpellAttackIfReady(me->GetEntry() == NPC_SKYBREAKER_RIFLEMAN
                                     ? SPELL_SHOOT
                                     : SPELL_HURL_AXE);
        }
...
public:
    ...
    struct npc_gunship_rocketeerAI : public gunship_npc_AI {
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            ...
            uint32 spellId = me->GetEntry() == NPC_SKYBREAKER_MORTAR_SOLDIER
    ...
    class spell_igb_rocket_pack_AuraScript : public AuraScript {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return ValidateSpellInfo(
                {SPELL_ROCKET_PACK_DAMAGE, SPELL_ROCKET_BURST});
        }
        ...
        {
            SpellInfo const* damageInfo =
                sSpellMgr->AssertSpellInfo(SPELL_ROCKET_PACK_DAMAGE);
            GetTarget()->CastCustomSpell(
                SPELL_ROCKET_PACK_DAMAGE,
                SPELLVALUE_BASE_POINT0,
                2 * (damageInfo->Effects[EFFECT_0].CalcValue() +
                     aurEff->GetTickNumber() * aurEff->GetAmplitude()),
                nullptr,
                true);
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_igb_rocket_pack_AuraScript::HandlePeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_igb_rocket_pack_AuraScript::HandleRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
...
public:
    spell_igb_rocket_pack_useable()
        : SpellScriptLoader("spell_igb_rocket_pack_useable")
    {
    }
    ...
    class spell_igb_rocket_pack_useable_AuraScript : public AuraScript {
        ...
        bool Load() override { return GetOwner()->GetInstanceScript(); }

        bool CheckAreaTarget(Unit* target)
        {
            return target->GetTypeId() == TYPEID_PLAYER &&
                   GetOwner()->GetInstanceScript()->GetBossState(
                       DATA_ICECROWN_GUNSHIP_BATTLE) != DONE;
        }
            ...
            if (Creature* owner = GetOwner()->ToCreature())
                if (Player* target = GetTarget()->ToPlayer())
                    if (target->HasItemCount(ITEM_GOBLIN_ROCKET_PACK, 1))
                        sCreatureTextMgr->SendChat(owner,
        ...
        void Register() override
        {
            DoCheckAreaTarget += AuraCheckAreaTargetFn(
                spell_igb_rocket_pack_useable_AuraScript::CheckAreaTarget);
            AfterEffectApply += AuraEffectApplyFn(
                spell_igb_rocket_pack_useable_AuraScript::HandleApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_igb_rocket_pack_useable_AuraScript::HandleRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_igb_teleport_to_enemy_ship()
        : SpellScriptLoader("spell_igb_teleport_to_enemy_ship")
    {
    }
    ...
    class spell_igb_teleport_to_enemy_ship_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_igb_teleport_to_enemy_ship_SpellScript::
...
public:
    ...
    class spell_igb_check_for_players_SpellScript : public SpellScript {
        ...
        {
            if (GetHitUnit()->GetPositionZ() <
                GetCaster()->GetPositionZ() - 10.0f)
                GetHitUnit()->CastSpell(
                    GetHitUnit(), SPELL_GUNSHIP_FALL_TELEPORT, true);
        }
        ...
        void Register() override
        {
            ...
            AfterCast += SpellCastFn(
                spell_igb_check_for_players_SpellScript::TriggerWipe);
            OnEffectHitTarget += SpellEffectFn(
                spell_igb_check_for_players_SpellScript::TeleportPlayer,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
...
public:
    spell_igb_gunship_fall_teleport()
        : SpellScriptLoader("spell_igb_gunship_fall_teleport")
    {
    }
    ...
    class spell_igb_gunship_fall_teleport_SpellScript : public SpellScript {
        ...
        bool Load() override { return GetCaster()->GetInstanceScript(); }

        void SelectTransport(WorldObject*& target)
        {
            if (InstanceScript* instance = target->GetInstanceScript())
                target = instance->instance->GetTransport(
                    instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE));
        }
        ...
        void RelocateDest(SpellEffIndex /*effIndex*/)
        {
            ...
            offset.m_positionZ = GetCaster()->GetInstanceScript()->GetData(
        ...
        void Register() override
        {
            OnObjectTargetSelect += SpellObjectTargetSelectFn(
                spell_igb_gunship_fall_teleport_SpellScript::SelectTransport,
                EFFECT_0,
                TARGET_DEST_NEARBY_ENTRY);
            OnEffectLaunch += SpellEffectFn(
                spell_igb_gunship_fall_teleport_SpellScript::RelocateDest,
                EFFECT_0,
                SPELL_EFFECT_TELEPORT_UNITS);
        }
    };
...
public:
    ...
    class spell_igb_explosion_main_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_igb_explosion_main_AuraScript::PeriodicTick,
                EFFECT_0,
...
public:
    ...
    class spell_igb_explosion_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_igb_explosion_SpellScript::SelectTarget,
                EFFECT_0,
...
public:
    spell_igb_teleport_players_on_victory()
        : SpellScriptLoader("spell_igb_teleport_players_on_victory")
    {
    }
    ...
    class spell_igb_teleport_players_on_victory_SpellScript
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_igb_teleport_players_on_victory_SpellScript::
                    FilterTargets,
                EFFECT_1,
                TARGET_UNIT_DEST_AREA_ENTRY);
        }
...
public:
    spell_igb_periodic_trigger_with_power_cost()
        : SpellScriptLoader("spell_igb_periodic_trigger_with_power_cost")
    {
    }

    class spell_igb_periodic_trigger_with_power_cost_AuraScript
        : public AuraScript {
        PrepareAuraScript(
...
public:
    ...
    class spell_igb_overheat_AuraScript : public AuraScript {
        ...
        {
            AfterEffectApply +=
                AuraEffectApplyFn(spell_igb_overheat_AuraScript::HandleApply,
                                  EFFECT_0,
                                  SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                  AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_igb_overheat_AuraScript::HandleRemove,
                                   EFFECT_0,
                                   SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                   AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    class spell_igb_cannon_blast_SpellScript : public SpellScript {
        ...
        void CheckEnergy()
        {
            if (GetCaster()->GetPower(POWER_ENERGY) >= 100) {
                ...
                if (Vehicle* vehicle = GetCaster()->GetVehicleKit())
                    if (Unit* passenger = vehicle->GetPassenger(0))
                        sCreatureTextMgr->SendChat(
                            GetCaster()->ToCreature(), SAY_OVERHEAT, passenger);
            }
        }
        ...
        {
            AfterHit +=
                SpellHitFn(spell_igb_cannon_blast_SpellScript::CheckEnergy);
        }
    };
...
public:
    spell_igb_incinerating_blast()
        : SpellScriptLoader("spell_igb_incinerating_blast")
    {
    }
    ...
    class spell_igb_incinerating_blast_SpellScript : public SpellScript {
        ...
        void RemoveEnergy() { GetCaster()->SetPower(POWER_ENERGY, 0); }

        void CalculateDamage(SpellEffIndex /*effIndex*/)
        {
            PreventHitEffect(EFFECT_0);
            SpellInfo const* si = sSpellMgr->GetSpellInfo(
                GetSpellInfo()->Effects[0].TriggerSpell);
            if (!si)
                return;
            ...
            int32             damage =
                si->Effects[0].CalcValue() + _energyLeft * _energyLeft * 8;
        ...
        {
            OnCast += SpellCastFn(
                spell_igb_incinerating_blast_SpellScript::StoreEnergy);
            AfterCast += SpellCastFn(
                spell_igb_incinerating_blast_SpellScript::RemoveEnergy);
            OnEffectHit += SpellEffectFn(
                spell_igb_incinerating_blast_SpellScript::CalculateDamage,
                EFFECT_0,
...
public:
    spell_igb_burning_pitch_selector()
        : SpellScriptLoader("spell_igb_burning_pitch_selector")
    {
    }
    ...
    class spell_igb_burning_pitch_selector_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(BurningPitchFilterCheck(
                teamId == TEAM_HORDE ? GO_ORGRIMS_HAMMER_H
                                     : GO_THE_SKYBREAKER_A));
            if (!targets.empty()) {
                WorldObject* target =
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            GetCaster()->CastSpell(
                GetHitUnit(), uint32(GetEffectValue()), TRIGGERED_NONE);
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_igb_burning_pitch_selector_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
            OnEffectHitTarget += SpellEffectFn(
                spell_igb_burning_pitch_selector_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    class spell_igb_burning_pitch_SpellScript : public SpellScript {
        ...
        {
            PreventHitDefaultEffect(effIndex);
            GetCaster()->CastCustomSpell(uint32(GetEffectValue()),
                                         SPELLVALUE_BASE_POINT0,
                                         8000,
                                         nullptr,
                                         TRIGGERED_FULL_MASK);
            GetHitUnit()->CastSpell(
                GetHitUnit(), SPELL_BURNING_PITCH, TRIGGERED_FULL_MASK);
        }

        void Register() override
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_igb_burning_pitch_SpellScript::HandleDummy,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    bool operator()(WorldObject* unit)
    {
        return unit->GetTypeId() != TYPEID_PLAYER ||
               unit->GetPositionZ() > 478.0f || !unit->GetTransport() ||
               unit->GetTransport()->GetEntry() != _entry ||
               unit->GetMapHeight(unit->GetPhaseMask(),
                                  unit->GetPositionX(),
                                  unit->GetPositionY(),
                                  unit->GetPositionZ()) < 465.0f;
    }
...
public:
    spell_igb_rocket_artillery()
        : SpellScriptLoader("spell_igb_rocket_artillery")
    {
    }
    ...
    class spell_igb_rocket_artillery_SpellScript : public SpellScript {
        ...
        void SelectRandomTarget(std::list<WorldObject*>& targets)
        {
            ...
            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                teamId = TeamId(instance->GetData(DATA_TEAMID_IN_INSTANCE));
            targets.remove_if(IgbArtilleryCheck(teamId == TEAM_HORDE
                                                    ? GO_ORGRIMS_HAMMER_H
                                                    : GO_THE_SKYBREAKER_A));
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            PreventHitDefaultEffect(effIndex);
            GetCaster()->CastSpell(
                GetHitUnit()->GetPositionX(),
                GetHitUnit()->GetPositionY(),
                GetHitUnit()->GetMapHeight(GetCaster()->GetPhaseMask(),
                                           GetHitUnit()->GetPositionX(),
                                           GetHitUnit()->GetPositionY(),
                                           GetHitUnit()->GetPositionZ()),
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_igb_rocket_artillery_SpellScript::SelectRandomTarget,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(
                spell_igb_rocket_artillery_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_igb_rocket_artillery_explosion()
        : SpellScriptLoader("spell_igb_rocket_artillery_explosion")
    {
    }
    ...
    class spell_igb_rocket_artillery_explosion_SpellScript
        ...
        {
            OnEffectHit += SpellEffectFn(
                spell_igb_rocket_artillery_explosion_SpellScript::DamageGunship,
                EFFECT_0,
                SPELL_EFFECT_TRIGGER_MISSILE);
        }
    };
...
public:
    ...
    class spell_igb_below_zero_SpellScript : public SpellScript {
        ...
        void RemovePassengers(SpellMissInfo missInfo)
        {
            ...
            GetHitUnit()->CastSpell(
                GetHitUnit(), SPELL_EJECT_ALL_PASSENGERS, TRIGGERED_FULL_MASK);
        }

        void Register() override
        {
            BeforeHit += BeforeSpellHitFn(
                spell_igb_below_zero_SpellScript::RemovePassengers);
        }
    };
    ...
    class spell_igb_on_gunship_deck_AuraScript : public AuraScript {
        ...
        bool Load() override
        {
            if (InstanceScript* instance = GetOwner()->GetInstanceScript())
                _teamIdInInstance =
                    TeamId(instance->GetData(DATA_TEAMID_IN_INSTANCE));
            else
        ...
        void HandleApply(AuraEffect const* /*aurEff*/,
                         AuraEffectHandleModes /*mode*/)
        {
            bool enemy = GetSpellInfo()->Id ==
                         uint32(_teamIdInInstance == TEAM_HORDE
                                    ? SPELL_ON_SKYBREAKER_DECK
                                    : SPELL_ON_ORGRIMS_HAMMER_DECK);
            if (Creature* gunship = GetOwner()->FindNearestCreature(
                    _teamIdInInstance == TEAM_HORDE ? NPC_ORGRIMS_HAMMER
                                                    : NPC_THE_SKYBREAKER,
                    200.0f))
                gunship->AI()->SetGUID(GetTarget()->GetGUID(),
                                       enemy ? ACTION_SHIP_VISITS_ENEMY
        ...
        {
            DoCheckAreaTarget += AuraCheckAreaTargetFn(
                spell_igb_on_gunship_deck_AuraScript::CheckAreaTarget);
            AfterEffectApply += AuraEffectApplyFn(
                spell_igb_on_gunship_deck_AuraScript::HandleApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    achievement_im_on_a_boat()
        : AchievementCriteriaScript("achievement_im_on_a_boat")
    {
    }
    ...
    {
        return target->GetAI() &&
               target->GetAI()->GetData(ACTION_SHIP_VISITS_ENEMY) == 1;
    }
};
...
class spell_igb_battle_experience_check : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& /*eventInfo*/) { return false; }

    void Register() override
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_igb_battle_experience_check::CheckProc);
    }
};
