...
enum Spells {
    ...
    SPELL_GASTRIC_BLOAT  = 72219, // 72214 is the proper way (with proc) but atm
                                  // procs can't have cooldown for creatures
...
public:
    ...
    struct boss_festergutAI : public BossAI {
        ...
        void Reset() override
        {
            ...
            _Reset();
            events.Reset();
            if (Creature* gasDummy =
                    me->FindNearestCreature(NPC_GAS_DUMMY, 100.0f, true)) {
        ...
        {
            if (!instance->CheckRequiredBosses(DATA_FESTERGUT,
                                               who->ToPlayer())) {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if (spell->Id == PUNGENT_BLIGHT_HELPER)
                ...
                if (((spell->Id == 69159 || spell->Id == 70136 ||
                      spell->Id == 69161 || spell->Id == 70139 ||
                      spell->Id == 69163 || spell->Id == 70469) &&
                     p->GetQuestStatus(QUEST_RESIDUE_RENDEZVOUS_10) ==
                         QUEST_STATUS_INCOMPLETE) ||
                    ((spell->Id == 70135 || spell->Id == 70138 ||
                      spell->Id == 70468 || spell->Id == 70137 ||
        ...
        {
            if (Creature* gasDummy =
                    ObjectAccessor::GetCreature(*me, _gasDummyGUID))
                for (uint8 i = 0; i < 3; ++i) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INHALE_BLIGHT:
                ...
                if (_inhaleCounter == 3) {
                    ...
                    // just cast and dont bother with target, conditions will
                    // handle it
                    ++_inhaleCounter;
                    if (_inhaleCounter < 3)
                        me->CastSpell(me,
                                      gaseousBlight[_inhaleCounter],
                                      true,
                                      nullptr,
                                      nullptr,
                                      me->GetGUID());
            ...
            case EVENT_GAS_SPORE:
                ...
                me->CastCustomSpell(SPELL_GAS_SPORE,
                                    SPELLVALUE_MAX_TARGETS,
                                    RAID_MODE<int32>(2, 3, 2, 3),
                                    me);
            ...
            case EVENT_FESTERGUT_GOO:
                if (Unit* target = SelectTarget(SelectTargetMethod::Random,
                    ...
                    if (Creature* professor = ObjectAccessor::GetCreature(
                        ...
                        professor->CastSpell(
                            target, SPELL_MALLABLE_GOO_H, true);
                events.ScheduleEvent(EVENT_FESTERGUT_GOO, 15s, 20s);
            default:
                break;
            }
...
public:
    spell_festergut_pungent_blight()
        : SpellScriptLoader("spell_festergut_pungent_blight")
    {
    }
    ...
    class spell_festergut_pungent_blight_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            ...
            uint32 blightId = sSpellMgr->GetSpellIdForDifficulty(
                uint32(GetEffectValue()), caster);
            ...
            if (InstanceScript* inst = caster->GetInstanceScript())
                if (Creature* professor = ObjectAccessor::GetCreature(
                        *caster, inst->GetGuidData(DATA_PROFESSOR_PUTRICIDE)))
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_festergut_pungent_blight_SpellScript::HandleScript,
                EFFECT_1,
...
public:
    spell_festergut_blighted_spores()
        : SpellScriptLoader("spell_festergut_blighted_spores")
    {
    }
    ...
    class spell_festergut_blighted_spores_AuraScript : public AuraScript {
        ...
        void ExtraEffect(AuraEffect const* aurEff,
                         AuraEffectHandleModes /*mode*/)
        {
            if (Aura* a = aurEff->GetBase())
                if (a->GetDuration() >
                    a->GetMaxDuration() -
                        1000) // this does not stack for different casters and
                              // previous is removed by new DoT, prevent it from
                              // giving inoculation in such case
                    return;
            uint32 inoculatedId = sSpellMgr->GetSpellIdForDifficulty(
                SPELL_INOCULATED, GetTarget());
            uint8 inoculatedStack = 1;
            if (Aura* a = GetTarget()->GetAura(inoculatedId)) {
                inoculatedStack += a->GetStackAmount();
                if (a->GetDuration() >
                    a->GetMaxDuration() -
                        10000) // player may gain only one stack at a time, no
                               // matter how many spores explode near him
                    return;
            }
            GetTarget()->CastSpell(GetTarget(), SPELL_INOCULATED, true);
            if (InstanceScript* instance = GetTarget()->GetInstanceScript())
                if (Creature* festergut = ObjectAccessor::GetCreature(
                        *GetTarget(), instance->GetGuidData(DATA_FESTERGUT)))
                    festergut->AI()->SetData(DATA_INOCULATED_STACK,
                                             inoculatedStack);
        }
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_festergut_blighted_spores_AuraScript::ExtraEffect,
                EFFECT_0,
...
public:
    spell_festergut_gastric_bloat()
        : SpellScriptLoader("spell_festergut_gastric_bloat")
    {
    }
    ...
    class spell_festergut_gastric_bloat_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            ...
            GetHitUnit()->CastSpell(
                GetHitUnit(), SPELL_GASTRIC_EXPLOSION, true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_festergut_gastric_bloat_SpellScript::HandleScript,
                EFFECT_2,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    achievement_flu_shot_shortage()
        : AchievementCriteriaScript("achievement_flu_shot_shortage")
    {
    }
    ...
    OnCheck(Player* /*source*/, Unit* target, uint32 /*criteria_id*/) override
    {
        if (target && target->GetTypeId() == TYPEID_UNIT)
            return target->ToCreature()->AI()->GetData(DATA_INOCULATED_STACK) <
                   3;
    ...
    struct npc_stinky_iccAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (InstanceScript* _instance = me->GetInstanceScript())
                if (Creature* festergut = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_FESTERGUT)))
