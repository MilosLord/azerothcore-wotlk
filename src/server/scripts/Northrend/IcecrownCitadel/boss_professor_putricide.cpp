...
enum ScriptTexts {
    ...
    SAY_TRANSFORM_1             = 7,
    SAY_TRANSFORM_2 =
        8, // always used for phase2 change, DO NOT GROUP WITH SAY_TRANSFORM_1
...
Position const festergutWatchPos = {
    4324.820f, 3166.03f, 389.3831f, 3.316126f}; // emote 432 (release gas)
Position const rotfaceWatchPos = {
    4390.371f, 3164.50f, 389.3890f, 5.497787f}; // emote 432 (release ooze)
...
public:
    ...
    bool operator()(ObjectGuid guid)
    {
        if (Unit* summon = ObjectAccessor::GetUnit(*_owner, guid)) {
            if (summon->GetEntry() == NPC_MUTATED_ABOMINATION_10 ||
                summon->GetEntry() == NPC_MUTATED_ABOMINATION_25) {
...
public:
    ...
    struct boss_professor_putricideAI : public BossAI {
        boss_professor_putricideAI(Creature* creature)
            : BossAI(creature, DATA_PROFESSOR_PUTRICIDE)
        {
            bCallEvade     = false;
        ...
        uint8  _phase;
        bool   bChangePhase;
        bool   bEnteredCombat; // needed for failing an attempt in
                               // JustReachedHome()
        ...
        uint32 GetData(uint32 type) const override
        {
            ...
            case DATA_ABOMINATION:
                return (uint32)(const_cast<SummonList*>(&summons)->HasEntry(
                                    NPC_MUTATED_ABOMINATION_10) ||
                                const_cast<SummonList*>(&summons)->HasEntry(
        ...
        {
            return me->IsVisible() && target->GetPositionZ() > 388.0f &&
                   target->GetPositionZ() < 410.0f &&
                   target->GetPositionY() > 3157.1f &&
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            if (!instance->CheckRequiredBosses(DATA_PROFESSOR_PUTRICIDE,
                                               who->ToPlayer()) ||
                me->GetExactDist2d(&homePos) > 10.0f ||
                !me->IsVisible()) // check home position because during
                                  // festergut/rotface fight, trigger missile
                                  // after their death can trigger putricide
                                  // combat
            {
                me->CombatStop();
                me->RemoveAllAuras();
            ...
            events.ScheduleEvent(
                EVENT_SLIME_PUDDLE, 10s, EVENT_GROUP_ABILITIES);
            events.ScheduleEvent(
                EVENT_UNSTABLE_EXPERIMENT, 30s, 35s, EVENT_GROUP_ABILITIES);
            if (IsHeroic())
                events.ScheduleEvent(
                    EVENT_UNBOUND_PLAGUE, 20s, EVENT_GROUP_ABILITIES);
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Is25ManRaid() && me->HasAura(SPELL_SHADOWS_FATE))
                DoCastAOE(SPELL_UNHOLY_INFUSION_CREDIT,
                          true); // ReqTargetAura in dbc
        ...
        {
            if (summon->GetEntry() != 38308 && summon->GetEntry() != 38309 &&
                (!me->IsInCombat() || me->IsInEvadeMode())) {
            ...
            case NPC_GROWING_OOZE_PUDDLE:
                // blizzard casts this spell 7 times initially (confirmed in
                // sniff)
                for (uint8 i = 0; i < 7; ++i)
            ...
            case NPC_VOLATILE_OOZE:
                ...
                summon->CastSpell(
                    summon, SPELL_OOZE_ERUPTION_SEARCH_PERIODIC, true);
                summon->SetReactState(REACT_PASSIVE);
                break;
            case NPC_CHOKING_GAS_BOMB:
                summon->CastSpell(
                    summon, SPELL_CHOKING_GAS_BOMB_PERIODIC, true);
                summon->CastSpell(
                    summon, SPELL_CHOKING_GAS_EXPLOSION_TRIGGER, true);
                return;
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            case POINT_ROTFACE:
                if (Creature* c = instance->instance->GetCreature(
                        instance->GetGuidData(DATA_ROTFACE))) {
                    if (c->IsInCombat()) {
            ...
            case POINT_TABLE_COMBAT:
                ...
                events.ScheduleEvent(EVENT_TABLE_DRINK_STUFF,
                                     IsHeroic() ? 25s : 0ms);
                break;
            }
        }
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_FESTERGUT_COMBAT:
                me->GetMotionMaster()->MoveCharge(
                    festergutWatchPos.GetPositionX(),
                    festergutWatchPos.GetPositionY(),
                    festergutWatchPos.GetPositionZ(),
                    15.0f,
                    POINT_FESTERGUT);
                break;
            ...
            case ACTION_ROTFACE_COMBAT:
                me->GetMotionMaster()->MoveCharge(
                    rotfaceWatchPos.GetPositionX(),
                    rotfaceWatchPos.GetPositionY(),
                    rotfaceWatchPos.GetPositionZ(),
                    15.0f,
                    POINT_ROTFACE);
                break;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SLIME_PUDDLE: {
                ...
                SelectTargetList(
                    targets, 2, SelectTargetMethod::Random, 0, 0.0f, true);
                if (!targets.empty())
                    for (std::list<Unit*>::iterator itr = targets.begin();
                         itr != targets.end();
                         ++itr)
                        me->CastSpell(*itr, SPELL_SLIME_PUDDLE_TRIGGER, true);
                events.ScheduleEvent(
                    EVENT_SLIME_PUDDLE, 35s, EVENT_GROUP_ABILITIES);
            } break;
            case EVENT_UNSTABLE_EXPERIMENT:
                Talk(EMOTE_UNSTABLE_EXPERIMENT);
                me->CastSpell(me, SPELL_UNSTABLE_EXPERIMENT, false);
                events.ScheduleEvent(
                    EVENT_UNSTABLE_EXPERIMENT, 35s, 40s, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_GO_TO_TABLE:
                me->CastSpell(me, SPELL_TEAR_GAS_PERIODIC_TRIGGER, true);
                me->GetMotionMaster()->MoveCharge(tablePos.GetPositionX(),
                                                  tablePos.GetPositionY(),
                                                  tablePos.GetPositionZ(),
                                                  15.0f,
                                                  POINT_TABLE_COMBAT);
                break;
            case EVENT_TABLE_DRINK_STUFF:
                ...
                case 2: {
                    SpellInfo const* spell =
                        sSpellMgr->GetSpellInfo(SPELL_CREATE_CONCOCTION);
                    me->CastSpell(me, SPELL_CREATE_CONCOCTION, false);
                    events.ScheduleEvent(
                        EVENT_PHASE_TRANSITION,
                        sSpellMgr->GetSpellForDifficultyFromSpell(spell, me)
                                ->CalcCastTime() +
                            2250);
                    break;
                }
                case 3: {
                    SpellInfo const* spell =
                        sSpellMgr->GetSpellInfo(SPELL_GUZZLE_POTIONS);
            ...
            case EVENT_PHASE_TRANSITION: {
                switch (_phase) {
                case 2:
                    if (Creature* face = me->FindNearestCreature(
                            NPC_TEAR_GAS_TARGET_STALKER, 50.0f))
            ...
            case EVENT_MALLEABLE_GOO:
                ...
                events.ScheduleEvent(
                    EVENT_MALLEABLE_GOO, 25s, 30s, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_CHOKING_GAS_BOMB:
                Talk(EMOTE_CHOKING_GAS_BOMB);
                me->CastSpell(me, SPELL_CHOKING_GAS_BOMB, false);
                events.ScheduleEvent(
                    EVENT_CHOKING_GAS_BOMB, 35s, 40s, EVENT_GROUP_ABILITIES);
                break;
            default:
                break;
            }
        ...
        void ChangePhase()
        {
            ...
            if (!IsHeroic()) {
                ...
                if (Is25ManRaid()) {
                    ...
                    // and half gets gas
                    for (std::list<Unit*>::iterator itr = targetList.begin();
                         itr != targetList.end();
                         ++itr)
            ...
            case 1:
                _phase = 2;
                events.ScheduleEvent(EVENT_MALLEABLE_GOO,
                                     urand(25000, 28000) + heroicDelay,
                                     EVENT_GROUP_ABILITIES);
                events.ScheduleEvent(EVENT_CHOKING_GAS_BOMB,
                                     urand(35000, 40000) + heroicDelay,
                                     EVENT_GROUP_ABILITIES);
                break;
            case 2:
                _phase = 3;
...
public:
    npc_putricide_oozeAI(Creature* creature, uint32 hitTargetSpellId)
        : ScriptedAI(creature), _hitTargetSpellId(hitTargetSpellId),
    ...
    void IsSummonedBy(WorldObject* /*summoner*/) override
    {
        if (InstanceScript* instance = me->GetInstanceScript())
            if (Creature* professor = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_PROFESSOR_PUTRICIDE))) {
    ...
    void SpellHitTarget(Unit* /*target*/, SpellInfo const* spell) override
    {
        if (!_newTargetSelectTimer &&
            spell->Id ==
                sSpellMgr->GetSpellIdForDifficulty(_hitTargetSpellId, me))
            SelectNewTarget();
    }
        ...
        if (!_newTargetSelectTimer) {
            if ((!me->HasUnitState(UNIT_STATE_CASTING) && !me->GetVictim()) ||
                ...
                if (me->GetVictim()->GetGUID() != targetGUID || !target ||
                    ...
                    target->GetExactDist2dSq(4356.0f, 3211.0f) >
                        80.0f * 80.0f ||
                    target->GetPositionZ() < 380.0f ||
                    target->GetPositionZ() > 405.0f)
                    SelectNewTarget();
            }
        }
...
public:
    ...
    struct npc_volatile_oozeAI : public npc_putricide_oozeAI {
        npc_volatile_oozeAI(Creature* creature)
            : npc_putricide_oozeAI(creature, SPELL_OOZE_ERUPTION)
        {
        }
...
public:
    ...
    struct npc_gas_cloudAI : public npc_putricide_oozeAI {
        npc_gas_cloudAI(Creature* creature)
            : npc_putricide_oozeAI(creature, SPELL_EXPUNGED_GAS)
        {
            _newTargetSelectTimer = 0;
        }
...
public:
    spell_putricide_slime_puddle()
        : SpellScriptLoader("spell_putricide_slime_puddle")
    {
    }
    ...
    class spell_putricide_slime_puddle_SpellScript : public SpellScript {
        ...
        void ScaleRange(std::list<WorldObject*>& targets)
        {
            targets.remove_if(Acore::AllWorldObjectsInExactRange(
                GetCaster(),
                2.5f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X),
                true));
        }

        // big hax to unlock Abomination Eat Ooze ability, requires caster aura
        // spell from difficulty X, but unlocks clientside when got base aura
            ...
            if (s1 && s2)
                if (Unit* target = GetHitUnit()) {
                    Aura::TryRefreshStackOrCreate(
                        s1, MAX_EFFECT_MASK, target, target);
                    Aura::TryRefreshStackOrCreate(
                        s2, MAX_EFFECT_MASK, target, target);
                }
        }
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_slime_puddle_SpellScript::ScaleRange,
                EFFECT_0,
            ...
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_slime_puddle_SpellScript::HandleScript,
                EFFECT_1,
...
public:
    spell_putricide_slime_puddle_spawn()
        : SpellScriptLoader("spell_putricide_slime_puddle_spawn")
    {
    }
    ...
    class spell_putricide_slime_puddle_spawn_SpellScript : public SpellScript {
        ...
        void SelectDest()
        {
            if (Position* dest =
                ...
                float destZ = 395.0f; // random number close to ground, get
                                      // exact in next call
                GetCaster()->UpdateGroundPositionZ(
                    dest->GetPositionX(), dest->GetPositionY(), destZ);
        ...
        void Register() override
        {
            BeforeCast += SpellCastFn(
                spell_putricide_slime_puddle_spawn_SpellScript::SelectDest);
        }
    };
...
public:
    spell_putricide_grow_stacker()
        : SpellScriptLoader("spell_putricide_grow_stacker")
    {
    }
    ...
    class spell_putricide_grow_stacker_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_putricide_grow_stacker_AuraScript::HandleTriggerSpell,
                EFFECT_0,
...
public:
    spell_putricide_unstable_experiment()
        : SpellScriptLoader("spell_putricide_unstable_experiment")
    {
    }
    ...
    class spell_putricide_unstable_experiment_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            GetCreatureListWithEntryInGrid(
                ...
                200.0f);
            for (std::list<Creature*>::iterator itr = creList.begin();
                 itr != creList.end();
                 ++itr)
            ...
            if (target) {
                if (Aura* aura = target->GetAura(
                        uint32(GetSpellInfo()->Effects[stage].CalcValue())))
                    if (aura->GetOwner() ==
                        target) // avoid assert(false) at any cost
                        aura->UpdateOwner(
                            5000,
                            target); // update whole aura so previous periodic
                                     // ticks before refreshed by new one
            }
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_unstable_experiment_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_putricide_tear_gas_effect()
        : SpellScriptLoader("spell_putricide_tear_gas_effect")
    {
    }
    ...
    class spell_putricide_tear_gas_effect_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_tear_gas_effect_SpellScript::FilterTargets,
                EFFECT_0,
...
public:
    spell_putricide_gaseous_bloat()
        : SpellScriptLoader("spell_putricide_gaseous_bloat")
    {
    }
        ...
        void HandleExtraEffect(AuraEffect const* /*aurEff*/)
        {
            ...
            /*if (!target->HasAura(GetId()))
                if (Unit* caster = GetCaster())
                    caster->CastCustomSpell(SPELL_GASEOUS_BLOAT,
               SPELLVALUE_AURA_STACK, 10, caster, false);*/
        }
...
public:
    spell_putricide_ooze_channel()
        : SpellScriptLoader("spell_putricide_ooze_channel")
    {
    }
    ...
    class spell_putricide_ooze_channel_SpellScript : public SpellScript {
        ...
        {
            // dbc has only 1 field for excluding, this will prevent anyone from
            // getting both at the same time
            targets.remove_if(
                Acore::UnitAuraCheck(true, SPELL_VOLATILE_OOZE_PROTECTION));
            targets.remove_if(
                Acore::UnitAuraCheck(true, SPELL_GASEOUS_BLOAT_PROTECTION));

            if (targets.empty()) {
                FinishCast(SPELL_FAILED_NO_VALID_TARGETS);
                GetCaster()->ToCreature()->DespawnOrUnsummon(
                    1); // despawn next update
                return;
            }
        ...
        void StartAttack()
        {
            ...
            GetCaster()->AddThreat(GetHitUnit(),
                                   500000000.0f); // value seen in sniff
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_ooze_channel_SpellScript::SelectTarget,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_ooze_channel_SpellScript::SetTarget,
                EFFECT_1,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_ooze_channel_SpellScript::SetTarget,
                ...
                TARGET_UNIT_SRC_AREA_ENEMY);
            AfterHit += SpellHitFn(
                spell_putricide_ooze_channel_SpellScript::StartAttack);
        }
...
public:
    spell_putricide_ooze_eruption_searcher()
        : SpellScriptLoader("spell_putricide_ooze_eruption_searcher")
    {
    }

    class spell_putricide_ooze_eruption_searcher_SpellScript
        : public SpellScript {
        PrepareSpellScript(spell_putricide_ooze_eruption_searcher_SpellScript);

        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            uint32 adhesiveId = sSpellMgr->GetSpellIdForDifficulty(
                SPELL_VOLATILE_OOZE_ADHESIVE, GetCaster());
            if (GetHitUnit()->HasAura(adhesiveId)) {
                GetHitUnit()->RemoveAurasDueToSpell(adhesiveId,
                                                    GetCaster()->GetGUID(),
                                                    0,
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_ooze_eruption_searcher_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_putricide_mutated_plague()
        : SpellScriptLoader("spell_putricide_mutated_plague")
    {
    }
    ...
    class spell_putricide_mutated_plague_AuraScript : public AuraScript {
        ...
        void HandleTriggerSpell(AuraEffect const* aurEff)
        {
            ...
            GetTarget()->CastCustomSpell(triggerSpell,
        ...
        {
            uint32 healSpell =
                uint32(GetSpellInfo()->Effects[EFFECT_0].CalcValue());
            SpellInfo const* spell = sSpellMgr->GetSpellInfo(healSpell);
            if (!spell)
                return;
            spell =
                sSpellMgr->GetSpellForDifficultyFromSpell(spell, GetTarget());
            int32 healAmount = spell->Effects[EFFECT_0].CalcValue();
            healAmount *= GetStackAmount();
            GetTarget()->CastCustomSpell(healSpell,
                                         SPELLVALUE_BASE_POINT0,
                                         healAmount,
                                         GetTarget(),
                                         TRIGGERED_FULL_MASK,
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_putricide_mutated_plague_AuraScript::HandleTriggerSpell,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_putricide_mutated_plague_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
...
public:
    spell_putricide_unbound_plague()
        : SpellScriptLoader("spell_putricide_unbound_plague")
    {
    }
    ...
    class spell_putricide_unbound_plague_SpellScript : public SpellScript {
        ...
        bool Validate(SpellInfo const* /*spell*/) override
        {
            return ValidateSpellInfo(
                {SPELL_UNBOUND_PLAGUE, SPELL_UNBOUND_PLAGUE_SEARCHER});
        }

        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (AuraEffect const* eff = GetCaster()->GetAuraEffect(
                    SPELL_UNBOUND_PLAGUE_SEARCHER, EFFECT_0)) {
            ...
            targets.remove_if(
                Acore::UnitAuraCheck(true,
                                     sSpellMgr->GetSpellIdForDifficulty(
                                         SPELL_UNBOUND_PLAGUE, GetCaster())));
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            ...
            uint32 plagueId = sSpellMgr->GetSpellIdForDifficulty(
                SPELL_UNBOUND_PLAGUE, GetCaster());

            if (!GetHitUnit()->HasAura(plagueId)) {
                if (Creature* professor = ObjectAccessor::GetCreature(
                        *GetCaster(),
                        instance->GetGuidData(DATA_PROFESSOR_PUTRICIDE))) {
                    if (Aura* oldPlague = GetCaster()->GetAura(
                            plagueId, professor->GetGUID())) {
                        if (Aura* newPlague =
                                professor->AddAura(plagueId, GetHitUnit())) {
                            newPlague->SetMaxDuration(
                                oldPlague->GetMaxDuration());
                            newPlague->SetDuration(oldPlague->GetDuration());
                            oldPlague->Remove();
                            GetCaster()->RemoveAurasDueToSpell(
                                SPELL_UNBOUND_PLAGUE_SEARCHER);
                            GetCaster()->CastSpell(
                                GetCaster(), SPELL_PLAGUE_SICKNESS, true);
                            GetCaster()->CastSpell(
                                GetCaster(),
                                SPELL_UNBOUND_PLAGUE_PROTECTION,
                                true);
                            professor->CastSpell(GetHitUnit(),
                                                 SPELL_UNBOUND_PLAGUE_SEARCHER,
                                                 true);
                        }
                    }
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_unbound_plague_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ALLY);
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_unbound_plague_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_putricide_unbound_plague_dmg()
        : SpellScriptLoader("spell_putricide_unbound_plague_dmg")
    {
    }
    ...
    class spell_putricide_unbound_plague_dmg_AuraScript : public AuraScript {
        ...
        void HandlePeriodic(AuraEffect* aurEff)
        {
            ...
            int32 dmg =
                int32(baseAmt * pow(1.25f, float(aurEff->GetTickNumber())));
        ...
        void Register() override
        {
            OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
                spell_putricide_unbound_plague_dmg_AuraScript::HandlePeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE);
        }
    };
...
public:
    spell_putricide_choking_gas_bomb()
        : SpellScriptLoader("spell_putricide_choking_gas_bomb")
    {
    }
    ...
    class spell_putricide_choking_gas_bomb_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_choking_gas_bomb_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_putricide_clear_aura_effect_value()
        : SpellScriptLoader("spell_putricide_clear_aura_effect_value")
    {
    }

    class spell_putricide_clear_aura_effect_value_SpellScript
        : public SpellScript {
        ...
        {
            PreventHitDefaultEffect(effIndex);
            uint32 auraId = sSpellMgr->GetSpellIdForDifficulty(
                uint32(GetEffectValue()), GetCaster());
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_clear_aura_effect_value_SpellScript::
                    HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
...
public:
    spell_putricide_mutation_init()
        : SpellScriptLoader("spell_putricide_mutation_init")
    {
    }
    ...
    class spell_putricide_mutation_init_SpellScript : public SpellScript {
        ...
        SpellCastResult
        CheckRequirementInternal(SpellCustomErrors& extendedError)
        {
            InstanceScript* instance = GetExplTargetUnit()->GetInstanceScript();
            if (!instance)
                return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;

            Creature* professor = ObjectAccessor::GetCreature(
                *GetExplTargetUnit(),
                instance->GetGuidData(DATA_PROFESSOR_PUTRICIDE));
            if (!professor)
                return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;

            if (professor->AI()->GetData(DATA_PHASE) == 3 ||
                !professor->IsAlive()) {
        ...
        SpellCastResult CheckRequirement()
        {
            ...
            if (result != SPELL_CAST_OK) {
                Spell::SendCastResult(GetExplTargetUnit()->ToPlayer(),
                                      GetSpellInfo(),
                                      0,
                                      result,
                                      extension);
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_putricide_mutation_init_SpellScript::CheckRequirement);
        }
    };
    ...
    class spell_putricide_mutation_init_AuraScript : public AuraScript {
        ...
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_putricide_mutation_init_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_putricide_mutated_transformation()
        : SpellScriptLoader("spell_putricide_mutated_transformation")
    {
    }

    class spell_putricide_mutated_transformation_SpellScript
        : public SpellScript {
        ...
        void HandleSummon(SpellEffIndex effIndex)
        {
            ...
            Creature* putricide = ObjectAccessor::GetCreature(
                *caster, instance->GetGuidData(DATA_PROFESSOR_PUTRICIDE));
            if (!putricide)
                return;
            ...
            SummonPropertiesEntry const* properties =
                sSummonPropertiesStore.LookupEntry(
                    uint32(GetSpellInfo()->Effects[effIndex].MiscValueB));
            uint32 duration = uint32(GetSpellInfo()->GetDuration());

            Position    pos    = caster->GetPosition();
            TempSummon* summon = caster->GetMap()->SummonCreature(
                entry, pos, properties, duration, caster, GetSpellInfo()->Id);
            if (!summon || !summon->IsVehicle())
                return;

            summon->CastSpell(
                summon, SPELL_ABOMINATION_VEHICLE_POWER_DRAIN, true);
            summon->CastSpell(
                summon, SPELL_MUTATED_TRANSFORMATION_DAMAGE, true);
            caster->CastSpell(summon, SPELL_MUTATED_TRANSFORMATION_NAME, true);

            // EnterVehicle(summon, 0);    // VEHICLE_SPELL_RIDE_HARDCODED is
            // used according to sniff, this is ok
            caster->CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED,
                                    SPELLVALUE_BASE_POINT0,
                                    1,
                                    summon,
                                    TRIGGERED_FULL_MASK);
...
public:
    spell_putricide_mutated_transformation_dismiss()
        : SpellScriptLoader("spell_putricide_mutated_transformation_dismiss")
    {
    }

    class spell_putricide_mutated_transformation_dismiss_AuraScript
        : public AuraScript {
        PrepareAuraScript(
...
public:
    spell_putricide_mutated_transformation_dmg()
        : SpellScriptLoader("spell_putricide_mutated_transformation_dmg")
    {
    }

    class spell_putricide_mutated_transformation_dmg_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_putricide_mutated_transformation_dmg_SpellScript);

        void FilterTargetsInitial(std::list<WorldObject*>& targets)
        {
            if (Unit* owner = ObjectAccessor::GetUnit(
                    *GetCaster(), GetCaster()->GetCreatorGUID()))
...
public:
    ...
    class spell_putricide_eat_ooze_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_eat_ooze_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_putricide_eat_ooze_SpellScript::SelectTarget,
                EFFECT_0,
                TARGET_UNIT_DEST_AREA_ENTRY);
        }
    };
...
public:
    spell_putricide_regurgitated_ooze()
        : SpellScriptLoader("spell_putricide_regurgitated_ooze")
    {
    }
    ...
    class spell_putricide_regurgitated_ooze_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_putricide_regurgitated_ooze_SpellScript::ExtraEffect,
                EFFECT_0,
                SPELL_EFFECT_APPLY_AURA);
        }
    };
