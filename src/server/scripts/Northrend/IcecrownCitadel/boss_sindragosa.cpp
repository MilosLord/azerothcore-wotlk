...
enum Texts {
    SAY_AGGRO = 0, // You are fools to have come to this place! The icy winds of
                   // Northrend will consume your souls!
    SAY_UNCHAINED_MAGIC =
        1, // Suffer, mortals, as your pathetic magic betrays you!
    EMOTE_WARN_BLISTERING_COLD =
        2, // %s prepares to unleash a wave of blistering cold!
    SAY_BLISTERING_COLD =
        3, // Can you feel the cold hand of death upon your heart?
    SAY_RESPITE_FOR_A_TORMENTED_SOUL =
        4,             // Aaah! It burns! What sorcery is this?!
...
Position const SpinestalkerFlyPos = {
    4418.895f, 2514.233f, 230.4864f, 3.396045f};
Position const SpinestalkerLandPos = {
    4418.895f, 2514.233f, 203.3848f, 3.396045f};
Position const SindragosaFlyInPos = {
    4420.190f, 2484.360f, 232.5150f, 3.141593f};
...
Position const SindragosaAirPosFar = {
    4525.600f, 2485.150f, 245.0820f, 3.141593f};
...
public:
    FrostBombExplosion(Creature* owner, ObjectGuid sindragosaGUID)
        : _owner(owner), _sindragosaGUID(sindragosaGUID)
    {
    }

    bool Execute(uint64 /*eventTime*/, uint32 /*updateTime*/) override
    {
        _owner->CastSpell((Unit*)nullptr,
                          SPELL_FROST_BOMB,
                          false,
                          nullptr,
                          nullptr,
...
public:
    IceTombSummonEvent(Unit* owner, ObjectGuid sindragosaGUID)
        : _owner(owner), _sindragosaGUID(sindragosaGUID)
    {
    }
...
public:
    ...
    bool operator()(Unit const* target) const
    {
        ...
        if (target->IsImmunedToDamageOrSchool(SPELL_SCHOOL_MASK_ALL) ||
            target->HasAura(SPELL_ICE_TOMB_UNTARGETABLE) ||
            target->HasAura(SPELL_ICE_TOMB_DAMAGE) ||
            target->HasAura(SPELL_TANK_MARKER_AURA) ||
...
public:
    ...
    struct boss_sindragosaAI : public BossAI {
        boss_sindragosaAI(Creature* creature)
            : BossAI(creature, DATA_SINDRAGOSA)
        {
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_HASTE_SPELLS, true);
        }
        ...
        {
            if (!instance->CheckRequiredBosses(DATA_SINDRAGOSA,
                                               who->ToPlayer()) ||
                !me->IsVisible()) {
            ...
            events.ScheduleEvent(
                EVENT_FROST_BREATH, 8s, 12s, EVENT_GROUP_LAND_PHASE);
            events.ScheduleEvent(
                EVENT_UNCHAINED_MAGIC, 9s, 14s, EVENT_GROUP_LAND_PHASE);
            events.ScheduleEvent(
                EVENT_ICY_GRIP, 33s + 500ms, EVENT_GROUP_LAND_PHASE);
        ...
        {
            return me->IsVisible() &&
                   target->GetEntry() != NPC_CROK_SCOURGEBANE;
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_FROSTWYRM && !_isLanding) {
                ...
                float moveTime = me->GetExactDist(&SindragosaFlyInPos) /
                                 (me->GetSpeed(MOVE_RUN) * 0.001f);
                me->m_Events.AddEvent(
                    new FrostwyrmLandEvent(*me, SindragosaLandPos),
                    me->m_Events.CalculateTime(uint64(moveTime) + 250));
                me->GetMotionMaster()->MovePoint(POINT_FROSTWYRM_FLY_IN,
                                                 SindragosaFlyInPos);
        ...
        void MovementInform(uint32 type, uint32 point) override
        {
            ...
            case POINT_AIR_PHASE:
                me->CastCustomSpell(SPELL_ICE_TOMB_TARGET,
                                    SPELLVALUE_MAX_TARGETS,
                                    RAID_MODE<int32>(2, 5, 2, 6),
                                    nullptr);
                me->SetFacingTo(float(M_PI));
                events.ScheduleEvent(
                    EVENT_AIR_MOVEMENT_FAR,
                    0ms); // won't be processed during cast time anyway, so 0
                         ...
                         SpellSchoolMask) override
        {
            ...
            if (!_isThirdPhase) {
                ...
                if (!HealthAbovePct(20)) {
                    _isBelow20Pct = true;
                    if (instance->GetData(DATA_WEEKLY_QUEST_ID) ==
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            if (summon->GetEntry() == NPC_FROST_BOMB)
                summon->m_Events.AddEvent(
                    new FrostBombExplosion(summon, me->GetGUID()),
                    summon->m_Events.CalculateTime(5500));
        }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if (target->GetTypeId() == TYPEID_PLAYER)
                if (uint32 spellId =
                        sSpellMgr->GetSpellIdForDifficulty(70127, me))
                    if (spellId == spell->Id)
                        if (Aura const* mysticBuffet =
                                target->GetAura(spell->Id))
                            _mysticBuffetStack =
                                std::max<uint8>(_mysticBuffetStack,
                                                mysticBuffet->GetStackAmount());
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_CLEAVE:
                ...
                events.ScheduleEvent(
                    EVENT_CLEAVE, 10s, 15s, EVENT_GROUP_LAND_PHASE);
                break;
            ...
            case EVENT_FROST_BREATH:
                ...
                me->CastSpell(me->GetVictim(),
                              _isThirdPhase ? SPELL_FROST_BREATH_P2
                                            : SPELL_FROST_BREATH_P1,
                              false);
                events.DelayEventsToMax(1, 0);
                events.ScheduleEvent(EVENT_UNROOT, 0ms);
                events.ScheduleEvent(
                    EVENT_FROST_BREATH, 20s, 25s, EVENT_GROUP_LAND_PHASE);
                break;
            case EVENT_UNROOT:
            ...
            case EVENT_BLISTERING_COLD:
                ...
                events.ScheduleEvent(
                    EVENT_BLISTERING_COLD_YELL, 5s, EVENT_GROUP_LAND_PHASE);
                if (_isThirdPhase)
            ...
            case EVENT_AIR_PHASE:
                // pussywizard: unroot may be scheduled after this event cos of
                // events (time must be unique)
                ...
                me->StopMoving();
                me->SetDisableGravity(true);
                me->GetMotionMaster()->MoveTakeoff(POINT_TAKEOFF,
                                                   me->GetPositionX(),
                                                   me->GetPositionY(),
            ...
            case EVENT_AIR_MOVEMENT:
                me->GetMotionMaster()->MovePoint(POINT_AIR_PHASE,
                                                 SindragosaAirPos);
                break;
            case EVENT_AIR_MOVEMENT_FAR:
                me->GetMotionMaster()->MovePoint(POINT_AIR_PHASE_FAR,
                                                 SindragosaAirPosFar);
                break;
            case EVENT_FROST_BOMB: {
                ...
                me->GetGameObjectListWithEntryInGrid(
                    gl, GO_ICE_BLOCK, SIZE_OF_GRIDS);
                uint8 triesLeft = 10;
                do {
                    destX = float(rand_norm()) * 75.0f + 4350.0f;
                    destY = float(rand_norm()) * 75.0f + 2450.0f;
                    destZ = 205.0f; // random number close to ground, get exact
                                    // in next call
                    ...
                    for (std::list<GameObject*>::const_iterator itr =
                         ...
                         itr != gl.end();
                         ++itr)
                        if ((*itr)->GetExactDist2dSq(destX, destY) <
                            3.0f * 3.0f) {
                            ok = false;
                            break;
                        }
                ...
                me->CastSpell(
                    destX, destY, destZ, SPELL_FROST_BOMB_TRIGGER, false);
                if (_bombCount >= 4)
            ...
            case EVENT_LAND:
                me->GetMotionMaster()->MovePoint(POINT_LAND,
                                                 SindragosaFlyInPos);
                break;
            case EVENT_LAND_GROUND:
                events.ScheduleEvent(
                    EVENT_CLEAVE, 13s, 15s, EVENT_GROUP_LAND_PHASE);
                events.ScheduleEvent(
                    EVENT_TAIL_SMASH, 19s, 23s, EVENT_GROUP_LAND_PHASE);
                events.ScheduleEvent(
                    EVENT_FROST_BREATH, 7s, 10s, EVENT_GROUP_LAND_PHASE);
                events.ScheduleEvent(
                    EVENT_UNCHAINED_MAGIC, 12s, 17s, EVENT_GROUP_LAND_PHASE);
                events.ScheduleEvent(
                    EVENT_ICY_GRIP, 35s, 40s, EVENT_GROUP_LAND_PHASE);
                me->GetMotionMaster()->MoveLand(
                    POINT_LAND_GROUND, SindragosaLandPos, 10.0f);
                break;
            ...
            case EVENT_ICE_TOMB:
                if (Unit* target =
                        SelectTarget(SelectTargetMethod::Random,
                                     0,
                                     LastPhaseIceTombTargetSelector(me))) {
                    ...
                    if (uint32 evTime = events.GetNextEventTime(EVENT_ICY_GRIP))
                        if (events.GetTimer() > evTime ||
                            evTime - events.GetTimer() < 8000)
                            events.RescheduleEvent(
                                EVENT_ICY_GRIP, 8s, EVENT_GROUP_LAND_PHASE);
                }
...
public:
    ...
    struct npc_ice_tombAI : public NullCreatureAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Player* player =
                    ObjectAccessor::GetPlayer(*me, _trappedPlayerGUID)) {
    ...
    class spell_sindragosa_s_fury_SpellScript : public SpellScript {
        ...
        bool Load() override
        {
            ...
            if (InstanceMap* instance = GetCaster()->GetMap()->ToInstanceMap())
                if (instance->GetInstanceScript())
                    if (instance->GetScriptId() ==
                        sObjectMgr->GetScriptId(ICCScriptName))
                        return true;
        ...
        void SelectDest()
        {
            if (Position* dest =
                ...
                float destZ = 205.0f; // random number close to ground, get
                                      // exact in next call
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            if (!GetHitUnit()->IsAlive() ||
                (GetHitUnit()->GetTypeId() == TYPEID_PLAYER &&
                 GetHitUnit()->ToPlayer()->IsGameMaster()) ||
                !_targetCount)
                return;

            float resistance   = float(GetHitUnit()->GetResistance(
                SpellSchoolMask(GetSpellInfo()->SchoolMask)));
            ...
            SpellNonMeleeDamage damageInfo(GetCaster(),
                                           GetHitUnit(),
                                           GetSpellInfo(),
                                           GetSpellInfo()->SchoolMask);
        ...
        {
            BeforeCast +=
                SpellCastFn(spell_sindragosa_s_fury_SpellScript::SelectDest);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sindragosa_s_fury_SpellScript::CountTargets,
                EFFECT_1,
                TARGET_UNIT_DEST_AREA_ENTRY);
            OnEffectHitTarget +=
                SpellEffectFn(spell_sindragosa_s_fury_SpellScript::HandleDummy,
                              EFFECT_1,
                              SPELL_EFFECT_DUMMY);
        }
    ...
    bool operator()(WorldObject* object) const
    {
        if (Player* p = object->ToPlayer()) {
            ...
            if ((p->IsClass(CLASS_PALADIN) && maxIndex >= 1) ||
                (p->IsClass(CLASS_SHAMAN) && maxIndex == 1) ||
                (p->IsClass(CLASS_DRUID) && maxIndex == 1))
                return true;
            if (_removeHealers ==
                ((p->IsClass(CLASS_DRUID) && maxIndex == 2) ||
                 (p->IsClass(CLASS_PALADIN) && maxIndex == 0) ||
                 (p->IsClass(CLASS_PRIEST) && maxIndex <= 1) ||
                 (p->IsClass(CLASS_SHAMAN) && maxIndex == 2)))
                return true;
...
public:
    spell_sindragosa_unchained_magic()
        : SpellScriptLoader("spell_sindragosa_unchained_magic")
    {
    }
    ...
    class spell_sindragosa_unchained_magic_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& unitList)
        {
            ...
            unitList.clear();
            uint32 maxSize =
                uint32(GetCaster()->GetMap()->GetSpawnMode() & 1 ? 3 : 1);
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sindragosa_unchained_magic_SpellScript::FilterTargets,
                EFFECT_0,
    ...
    class spell_sindragosa_unchained_magic_AuraScript : public AuraScript {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            std::map<uint32, uint32>::iterator itr =
                _lastMSTimeForSpell.find(spellInfo->Id);
        ...
        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(
                spell_sindragosa_unchained_magic_AuraScript::CheckProc);
        }
    };
...
public:
    spell_sindragosa_permeating_chill()
        : SpellScriptLoader("spell_sindragosa_permeating_chill")
    {
    }
    ...
    class spell_sindragosa_permeating_chill_AuraScript : public AuraScript {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            return eventInfo.GetProcTarget() &&
                   eventInfo.GetProcTarget()->GetEntry() == NPC_SINDRAGOSA;
        }

        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(
                spell_sindragosa_permeating_chill_AuraScript::CheckProc);
        }
    };
...
public:
    spell_sindragosa_instability()
        : SpellScriptLoader("spell_sindragosa_instability")
    {
    }
    ...
    class spell_sindragosa_instability_AuraScript : public AuraScript {
        ...
        void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            if (GetTargetApplication()->GetRemoveMode() ==
                ...
                GetTarget()->CastCustomSpell(SPELL_BACKLASH,
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_sindragosa_instability_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
    ...
    class spell_sindragosa_icy_grip_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            if (!GetHitUnit()->IsWithinLOSInMap(GetCaster()) ||
                GetHitUnit()->HasAura(SPELL_TANK_MARKER_AURA))
                return;
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_sindragosa_icy_grip_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_sindragosa_icy_grip_jump()
        : SpellScriptLoader("spell_sindragosa_icy_grip_jump")
    {
    }
    ...
    class spell_sindragosa_icy_grip_jump_SpellScript : public SpellScript {
        ...
        void HandleSpecial(SpellEffIndex effIndex)
        {
            ...
            if (GetSpellInfo()->Effects[effIndex].MiscValue)
                ...
                speedZ =
                    float(GetSpellInfo()->Effects[effIndex].MiscValueB) / 10;
            else
                speedZ = 10.0f;
        ...
        void Register() override
        {
            OnEffectLaunchTarget += SpellEffectFn(
                spell_sindragosa_icy_grip_jump_SpellScript::HandleSpecial,
                EFFECT_0,
                SPELL_EFFECT_JUMP);
        }
    };
...
public:
    spell_sindragosa_frost_beacon()
        : SpellScriptLoader("spell_sindragosa_frost_beacon")
    {
    }
    ...
    class spell_sindragosa_frost_beacon_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_sindragosa_frost_beacon_AuraScript::PeriodicTick,
                EFFECT_0,
...
public:
    ...
    {
        return unit->HasAura(SPELL_FROST_IMBUED_BLADE) ||
               unit->IsImmunedToDamageOrSchool(SPELL_SCHOOL_MASK_ALL);
    }

    bool operator()(WorldObject* object) const
    {
        return object->ToUnit() &&
               (object->ToUnit()->HasAura(SPELL_FROST_IMBUED_BLADE) ||
                object->ToUnit()->IsImmunedToDamageOrSchool(
...
public:
    spell_sindragosa_ice_tomb_filter()
        : SpellScriptLoader("spell_sindragosa_ice_tomb_filter")
    {
    }
    ...
    class spell_sindragosa_ice_tomb_filter_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sindragosa_ice_tomb_filter_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
public:
    spell_sindragosa_ice_tomb()
        : SpellScriptLoader("spell_sindragosa_ice_tomb_trap")
    {
    }
    ...
    class spell_sindragosa_ice_tomb_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sindragosa_ice_tomb_SpellScript::FilterTargets,
                EFFECT_1,
                TARGET_UNIT_DEST_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sindragosa_ice_tomb_SpellScript::FilterTargetsSubseq,
                EFFECT_2,
                TARGET_UNIT_DEST_AREA_ENEMY);
        }
    };
    ...
    class spell_sindragosa_ice_tomb_AuraScript : public AuraScript {
                        ...
                        AuraEffectHandleModes /*mode*/)
        {
            if (Unit* c = GetCaster())
                GetTarget()->m_Events.AddEvent(
                    new IceTombSummonEvent(GetTarget(), c->GetGUID()),
                    GetTarget()->m_Events.CalculateTime(500));
        }

        void ExtraRemoveEffect(AuraEffect const* /*aurEff*/,
                               AuraEffectHandleModes /*mode*/)
        {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_sindragosa_ice_tomb_AuraScript::PeriodicTick,
                EFFECT_2,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_sindragosa_ice_tomb_AuraScript::ExtraRemoveEffect,
                EFFECT_1,
                SPELL_AURA_MOD_STUN,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectApply += AuraEffectApplyFn(
                spell_sindragosa_ice_tomb_AuraScript::AfterApply,
                EFFECT_1,
                SPELL_AURA_MOD_STUN,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    bool operator()(WorldObject* unit) const
    {
        ...
        return !dTree.isInLineOfSight(unit->GetPositionX(),
                                      unit->GetPositionY(),
                                      unit->GetPositionZ() + 2.f,
                                      ox,
                                      oy,
                                      oz + 2.f,
                                      unit->GetPhaseMask(),
...
public:
    spell_sindragosa_mystic_buffet()
        : SpellScriptLoader("spell_sindragosa_mystic_buffet")
    {
    }
    ...
    class spell_sindragosa_mystic_buffet_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_sindragosa_mystic_buffet_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
public:
    spell_sindragosa_soul_preservation()
        : SpellScriptLoader("spell_sindragosa_soul_preservation")
    {
    }
    ...
    class spell_sindragosa_soul_preservation_AuraScript : public AuraScript {
        ...
        void PeriodicTick(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Unit* s = GetTarget())
                if (GetStackAmount() >=
                    (s->GetMap()->Is25ManRaid() ? 75 : 30)) {
                    s->CastSpell(s, 72466, true);
                    s->RemoveAurasDueToSpell(72424);
                    if (s->GetTypeId() == TYPEID_UNIT)
                        s->ToCreature()->SetLootMode(3);
                    SetDuration(1);
                }
        }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_sindragosa_soul_preservation_AuraScript::PeriodicTick,
                EFFECT_0,
...
public:
    achievement_all_you_can_eat()
        : AchievementCriteriaScript("achievement_all_you_can_eat")
    {
    }
...
public:
    ...
    struct npc_spinestalkerAI : public ScriptedAI {
        ...
        void InitializeAI() override
        {
            if (!me->isDead()) {
                _instance->SetData(
                    ...
                    me->GetSpawnId()); // this cannot be in Reset because reset
                                       // also happens on evade
                Reset();
            }
        }
        ...
        void JustRespawned() override
        {
            ...
            _instance->SetData(
                DATA_SINDRAGOSA_FROSTWYRMS,
                me->GetSpawnId()); // this cannot be in Reset because reset also
                                   // happens on evade
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_FROSTWYRM) {
                ...
                float moveTime = me->GetExactDist(&SpinestalkerFlyPos) /
                                 (me->GetSpeed(MOVE_RUN) * 0.001f);
                me->m_Events.AddEvent(
                    new FrostwyrmLandEvent(*me, SpinestalkerLandPos),
                    me->m_Events.CalculateTime(uint64(moveTime) + 250));
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_CLEAVE_SPINESTALKER:
                me->CastSpell(
                    me->GetVictim(), SPELL_CLEAVE_SPINESTALKER, false);
                _events.ScheduleEvent(EVENT_CLEAVE_SPINESTALKER, 10s, 15s);
                break;
    ...
    struct npc_rimefangAI : public ScriptedAI {
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_FROSTWYRM) {
                ...
                me->setActive(true);
                me->SetImmuneToPC(true);
                float moveTime = me->GetExactDist(&RimefangFlyPos) /
                                 (me->GetSpeed(MOVE_RUN) * 0.001f);
                me->m_Events.AddEvent(
                    new FrostwyrmLandEvent(*me, RimefangLandPos),
                    me->m_Events.CalculateTime(uint64(moveTime) + 250));
                me->SetDefaultMovementType(IDLE_MOTION_TYPE);
                me->GetMotionMaster()->MoveIdle();
                me->StopMoving();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_FROST_BREATH_RIMEFANG:
                if (!me->IsFlying()) {
                    ...
                    _events.ScheduleEvent(
                        EVENT_FROST_BREATH_RIMEFANG, 20s, 25s);
                }
                else
            ...
            case EVENT_ICY_BLAST: {
                ...
                me->AttackStop();
                me->SetDisableGravity(true);
                float floorZ = me->GetMapHeight(
                    me->GetPositionX(), me->GetPositionY(), me->GetPositionZ());
                float destZ;
                if (floorZ > 190.0f)
                    destZ = floorZ + 25.0f;
                else
                    destZ = me->GetPositionZ() + 25.0f;
                me->GetMotionMaster()->MoveTakeoff(0,
                                                   me->GetPositionX(),
                                                   me->GetPositionY(),
                                                   destZ,
                                                   me->GetSpeed(MOVE_RUN));
                float moveTime = std::fabs(destZ - me->GetPositionZ()) /
                                 (me->GetSpeed(MOVE_RUN) * 0.001f);
                _events.ScheduleEvent(EVENT_ICY_BLAST,
                                      uint32(moveTime) + urand(60000, 70000));
                _events.ScheduleEvent(EVENT_ICY_BLAST_CAST,
                                      uint32(moveTime) + 250);
                break;
            }
            case EVENT_ICY_BLAST_CAST:
                if (--_icyBlastCounter) {
                    ...
                    float floorZ = me->GetMapHeight(me->GetPositionX(),
                                                    me->GetPositionY(),
                                                    me->GetPositionZ());
                    float destZ;
                    if (floorZ > 190.0f)
                        ...
                        destZ = me->GetPositionZ() - 25.0f;
                    me->SendMeleeAttackStop(me->GetVictim());
                    me->GetMotionMaster()->MoveLand(POINT_LAND_GROUND,
                                                    me->GetPositionX(),
                                                    me->GetPositionY(),
                                                    destZ,
                                                    me->GetSpeed(MOVE_RUN));
    ...
    class spell_rimefang_icy_blast_SpellScript : public SpellScript {
        ...
        void HandleTriggerMissile(SpellEffIndex effIndex)
        {
            ...
            if (Position const* pos = GetExplTargetDest())
                if (TempSummon* summon = GetCaster()->SummonCreature(
                        NPC_ICY_BLAST, *pos, TEMPSUMMON_TIMED_DESPAWN, 40000))
        ...
        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_rimefang_icy_blast_SpellScript::HandleTriggerMissile,
                EFFECT_1,
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            if (!instance->GetData(DATA_SPINESTALKER))
                if (Creature* spinestalker = ObjectAccessor::GetCreature(
                        *player, instance->GetGuidData(DATA_SPINESTALKER)))
                    spinestalker->AI()->DoAction(ACTION_START_FROSTWYRM);

            if (!instance->GetData(DATA_RIMEFANG))
                if (Creature* rimefang = ObjectAccessor::GetCreature(
                        *player, instance->GetGuidData(DATA_RIMEFANG)))
                    rimefang->AI()->DoAction(ACTION_START_FROSTWYRM);

            if (!instance->GetData(DATA_SINDRAGOSA_FROSTWYRMS) &&
                instance->GetBossState(DATA_SINDRAGOSA) != IN_PROGRESS) {
                player->GetMap()->LoadGrid(SindragosaSpawnPos.GetPositionX(),
                                           SindragosaSpawnPos.GetPositionY());
                if (Creature* sindragosa = ObjectAccessor::GetCreature(
                        *player, instance->GetGuidData(DATA_SINDRAGOSA)))
...
public:
    ...
    struct npc_sindragosa_trashAI : public ScriptedAI {
        ...
        void InitializeAI() override
        {
            _frostwyrmId = (me->GetHomePosition().GetPositionY() < 2484.35f)
                               ? DATA_RIMEFANG
                               : DATA_SPINESTALKER;
            if (!me->isDead()) {
                if (me->GetEntry() == NPC_FROSTWING_WHELP)
                    _instance->SetData(
                        _frostwyrmId,
                        me->GetSpawnId()); // this cannot be in Reset because
                                           // reset also happens on evade
                Reset();
            }
        }
        ...
        void JustEngagedWith(Unit* who) override
        {
            if (me->GetEntry() == NPC_FROSTWARDEN_HANDLER) {
                ...
                GetCreatureListWithEntryInGrid(
                    unitList, me, NPC_FROSTWING_WHELP, 40.0f);
                ...
                if (Creature* c = me->FindNearestCreature(
                        NPC_FROSTWARDEN_HANDLER, 40.0f, true))
                    if (!c->IsInCombat())
...
public:
    ...
    {
        if (!creature->AI()->GetData(DATA_WHELP_MARKER) &&
            creature->AI()->GetData(DATA_FROSTWYRM_OWNER) ==
                _owner->AI()->GetData(DATA_FROSTWYRM_OWNER))
            return false;
        return true;
    }
...
public:
    spell_frostwarden_handler_order_whelp()
        : SpellScriptLoader("spell_frostwarden_handler_order_whelp")
    {
    }
    ...
    class spell_frostwarden_handler_order_whelp_SpellScript
        ...
        {
            // caster is Frostwarden Handler, target is player, caster of
            // triggered is whelp
            PreventHitDefaultEffect(effIndex);
            std::list<Creature*> unitList;
            GetCreatureListWithEntryInGrid(
                unitList, GetCaster(), NPC_FROSTWING_WHELP, 150.0f);
            ...
            Acore::Containers::SelectRandomContainerElement(unitList)
                ->CastSpell(GetHitUnit(), uint32(GetEffectValue()), true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_frostwarden_handler_order_whelp_SpellScript::
                    HandleForcedCast,
                EFFECT_0,
...
public:
    spell_frostwarden_handler_focus_fire()
        : SpellScriptLoader("spell_frostwarden_handler_focus_fire")
    {
    }
    ...
    class spell_frostwarden_handler_focus_fire_SpellScript
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_frostwarden_handler_focus_fire_SpellScript::HandleScript,
                EFFECT_1,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
    ...
    class spell_frostwarden_handler_focus_fire_AuraScript : public AuraScript {
        ...
        void PeriodicTick(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Unit* caster = GetCaster()) {
                caster->AddThreat(
                    GetTarget(),
                    -float(GetSpellInfo()->Effects[EFFECT_1].CalcValue()));
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_frostwarden_handler_focus_fire_AuraScript::PeriodicTick,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_sindragosa_frost_breath()
        : SpellScriptLoader("spell_sindragosa_frost_breath")
    {
    }
    ...
    class spell_sindragosa_frost_breath_SpellScript : public SpellScript {
        ...
        void HandleInfusion()
        {
            ...
            if (!(target->GetRaidDifficulty() & RAID_DIFFICULTY_MASK_25MAN) ||
                target->GetQuestStatus(QUEST_FROST_INFUSION) !=
                    QUEST_STATUS_INCOMPLETE)
                return;

            // Check if player has Shadow's Edge equipped and not ready for
            // infusion
            if (!target->HasAura(SPELL_UNSATED_CRAVING) ||
                target->HasAura(SPELL_FROST_IMBUED_BLADE))
                return;

            Aura* infusion =
                target->GetAura(SPELL_FROST_INFUSION, target->GetGUID());
            if (infusion && infusion->GetStackAmount() >= 3) {
                target->RemoveAura(infusion);
                target->CastSpell(
                    target, SPELL_FROST_IMBUED_BLADE, TRIGGERED_FULL_MASK);
            }
            else
                target->CastSpell(
                    target, SPELL_FROST_INFUSION, TRIGGERED_FULL_MASK);
        }

        void Register() override
        {
            AfterHit += SpellHitFn(
                spell_sindragosa_frost_breath_SpellScript::HandleInfusion);
        }
    };
...
void AddSC_boss_sindragosa()
{
    ...
    new spell_trigger_spell_from_caster("spell_sindragosa_ice_tomb",
                                        SPELL_ICE_TOMB_DUMMY);
    new spell_trigger_spell_from_caster("spell_sindragosa_ice_tomb_dummy",
                                        SPELL_FROST_BEACON);
