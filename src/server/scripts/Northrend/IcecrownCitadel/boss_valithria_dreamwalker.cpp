...
#define SUMMON_PORTAL                                                          \
    RAID_MODE<uint32>(SPELL_PRE_SUMMON_DREAM_PORTAL,                           \
                      SPELL_PRE_SUMMON_DREAM_PORTAL,                           \
                      SPELL_PRE_SUMMON_NIGHTMARE_PORTAL,                       \
                      SPELL_PRE_SUMMON_NIGHTMARE_PORTAL)
#define EMERALD_VIGOR                                                          \
    RAID_MODE<uint32>(SPELL_EMERALD_VIGOR,                                     \
                      SPELL_EMERALD_VIGOR,                                     \
...
Position const ValithriaSpawnPos = {
    4210.813f, 2484.443f, 364.9558f, 0.01745329f};

class RisenArchmageCheck {
public:
    // look for all permanently spawned Risen Archmages that are not yet in
    // combat
    bool operator()(Creature* creature)
    {
        return creature->IsAlive() &&
               creature->GetEntry() == NPC_RISEN_ARCHMAGE &&
...
public:
    ...
    bool operator()(Unit* unit) const
    {
        return unit->getPowerType() == POWER_MANA &&
               _source->GetDistance(unit) > 15.0f;
    }
...
public:
    DelayedCastEvent(Creature*  trigger,
                     uint32     spellId,
                     ObjectGuid originalCaster,
                     uint32     despawnTime)
        : _trigger(trigger), _originalCaster(originalCaster), _spellId(spellId),
    ...
    {
        _trigger->CastSpell(
            _trigger, _spellId, false, nullptr, nullptr, _originalCaster);
        if (_despawnTime)
...
public:
    ...
    struct boss_valithria_dreamwalkerAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_OBS_MOD_HEALTH, true);
            me->ApplySpellImmune(
                0, IMMUNITY_EFFECT, SPELL_EFFECT_HEAL_PCT, true);
            // Glyph of Dispel Magic - not a percent heal by effect, its cast
            // with custom basepoints
        ...
        void HealReceived(Unit* healer, uint32& heal) override
        {
            ...
            if (me->HealthAbovePctHealed(100, heal) && !_done) {
                ...
                if (Creature* trigger = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_VALITHRIA_TRIGGER)))
                    trigger->AI()->EnterEvadeMode();
                if (Creature* lichKing = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_VALITHRIA_LICH_KING)))
                    lichKing->AI()->Reset();
            }
            else if (!_over75PercentTalkDone &&
                     me->HealthAbovePctHealed(75, heal)) {
                ...
                if (Creature* trigger = ObjectAccessor::GetCreature(
                    ...
                    _instance->SetBossState(DATA_VALITHRIA_DREAMWALKER,
                                            IN_PROGRESS);
                }
            }
        }
            ...
            if (me->HealthBelowPctDamaged(25, damage)) {
                ...
                if (damage >= me->GetHealth()) {
                    ...
                    if (!_justDied) {
                        ...
                        if (Creature* trigger = ObjectAccessor::GetCreature(
                                *me,
                                _instance->GetGuidData(DATA_VALITHRIA_TRIGGER)))
                            trigger->AI()->EnterEvadeMode();
                    }
                }
            }
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_DREAM_SLIP) {
                ...
                _instance->SetData(DATA_WEEKLY_QUEST_ID,
                                   0); // show hidden npc if necessary
            }
        }
            ...
            if (summon->GetEntry() == NPC_DREAM_PORTAL_PRE_EFFECT) {
                summon->m_Events.AddEvent(
                    new DelayedCastEvent(
                        summon, SPELL_SUMMON_DREAM_PORTAL, me->GetGUID(), 6000),
                    summon->m_Events.CalculateTime(15000));
                summon->m_Events.AddEvent(
                    new AuraRemoveEvent(summon, SPELL_DREAM_PORTAL_VISUAL_PRE),
                    summon->m_Events.CalculateTime(15000));
            }
            else if (summon->GetEntry() == NPC_NIGHTMARE_PORTAL_PRE_EFFECT) {
                summon->m_Events.AddEvent(
                    new DelayedCastEvent(summon,
                                         SPELL_SUMMON_NIGHTMARE_PORTAL,
                                         me->GetGUID(),
                                         6000),
                    summon->m_Events.CalculateTime(15000));
                summon->m_Events.AddEvent(
                    new AuraRemoveEvent(summon,
        ...
        void SummonedCreatureDespawn(Creature* summon) override
        {
            if (summon->GetEntry() == NPC_DREAM_PORTAL ||
                summon->GetEntry() == NPC_NIGHTMARE_PORTAL)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_instance->GetBossState(DATA_VALITHRIA_DREAMWALKER) ==
                NOT_STARTED) {
                uint32 startingHealth = me->GetMaxHealth() * 0.5f;
                if (me->GetHealth() !=
                    startingHealth) // healing when boss cannot be engaged
                                    // (lower spire not finished, cheating)
                                    // doesn't start the fight, prevent winning
                                    // this way
...
public:
    ...
    struct npc_green_dragon_combat_triggerAI : public BossAI {
        ...
        {
            if (!instance->CheckRequiredBosses(DATA_VALITHRIA_DREAMWALKER,
                                               target->ToPlayer())) {
                me->setActive(true);
                EnterEvadeMode();
            ...
            if (Creature* valithria = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_VALITHRIA_DREAMWALKER)))
                valithria->AI()->DoAction(ACTION_ENTER_COMBAT);
            if (Creature* lichKing = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_VALITHRIA_LICH_KING)))
                lichKing->AI()->DoAction(ACTION_ENTER_COMBAT);

            std::list<Creature*>                            archmages;
            RisenArchmageCheck                              check;
            Acore::CreatureListSearcher<RisenArchmageCheck> searcher(
                me, archmages, check);
            Cell::VisitGridObjects(me, searcher, 100.0f);
            for (std::list<Creature*>::iterator itr = archmages.begin();
                 itr != archmages.end();
                 ++itr)
        ...
        void JustReachedHome() override
        {
            if (instance->GetBossState(DATA_VALITHRIA_DREAMWALKER) != DONE)
                DoAction(
                    ACTION_DEATH); // setActive(false) in ValithriaDespawner
            else
                _JustReachedHome();
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_DEATH)
                me->m_Events.AddEvent(new ValithriaDespawner(me),
                                      me->m_Events.CalculateTime(5000));
                    ...
                    for (ThreatContainer::StorageType::const_iterator itr =
                        ...
                        if (Unit* target = (*itr)->getTarget())
                            if (target->IsAlive() &&
                                ...
                                me->GetExactDist(target) < 200.0f &&
                                !target->IsImmunedToDamageOrSchool(
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetIcecrownCitadelAI<npc_green_dragon_combat_triggerAI>(
            creature);
    }
};
...
public:
    npc_the_lich_king_controller()
        : CreatureScript("npc_the_lich_king_controller")
    {
    }
    ...
    struct npc_the_lich_king_controllerAI : public ScriptedAI {
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_ENTER_COMBAT) {
                ...
                _events.Reset();
                _events.ScheduleEvent(EVENT_GLUTTONOUS_ABOMINATION_SUMMONER,
                                      5s);
        ...
        void JustSummoned(Creature* summon) override
        {
            summon->SetPhaseMask((summon->GetPhaseMask() & ~0x10),
                                 true); // must not be in dream phase
            ...
            if (summon->GetEntry() == NPC_COLUMN_OF_FROST)
                summon->m_Events.AddEvent(
                    new DelayedCastEvent(summon,
                                         ...
                                         ObjectGuid::Empty,
                                         8000),
                    summon->m_Events.CalculateTime(2000));
...
public:
    ...
    struct npc_valithria_portalAI : public NullCreatureAI {
        npc_valithria_portalAI(Creature* creature)
            : NullCreatureAI(creature), _used(false)
        {
        }
...
public:
    ...
    struct npc_valithria_cloudAI : public ScriptedAI {
        npc_valithria_cloudAI(Creature* creature)
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_EXPLODE:
                ...
                // must use originalCaster the same for all clouds to allow
                // stacking
                me->CastSpell(
                    me,
                    EMERALD_VIGOR,
                    ...
                    nullptr,
                    nullptr,
                    _instance->GetGuidData(DATA_VALITHRIA_DREAMWALKER));
...
public:
    ...
    struct npc_suppresserAI : public ScriptedAI {
        ...
        {
            if (Creature* valithria = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(DATA_VALITHRIA_DREAMWALKER)))
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            ...
            if (!me->GetVictim() ||
                me->GetVictim()->GetEntry() != NPC_VALITHRIA_DREAMWALKER)
                return;
    ...
    struct npc_gluttonous_abominationAI : public ScriptedAI {
        ...
        void JustSummoned(Creature* summon) override
        {
            if (me->GetInstanceScript() &&
                me->GetInstanceScript()->GetBossState(
                    DATA_VALITHRIA_DREAMWALKER) == DONE)
...
public:
    spell_dreamwalker_summon_portal()
        : SpellScriptLoader("spell_dreamwalker_summon_portal")
    {
    }
    ...
    class spell_dreamwalker_summon_portal_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            uint32 spellId = (GetSpellInfo()->Id == 72224
                                  ? 71301
                                  : 71977); // spell implicit target replaced to
                                            // TARGET_DEST_DEST
            float minDist    = 20.0f;
            float maxDist    = 30.0f;
            ...
            float maxAddAngle =
                ((target->GetMap()->GetSpawnMode() % 2) == 0 ? M_PI : 2 * M_PI);
            float angle = startAngle + rand_norm() * maxAddAngle;
            target->CastSpell(target->GetPositionX() + cos(angle) * dist,
                              target->GetPositionY() + std::sin(angle) * dist,
                              target->GetPositionZ(),
                              spellId,
                              true);
        }
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_dreamwalker_summon_portal_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    ...
    class spell_dreamwalker_twisted_nightmares_SpellScript
        : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            if (InstanceScript* instance = GetHitUnit()->GetInstanceScript())
                GetHitUnit()->CastSpell(
                    ...
                    nullptr,
                    nullptr,
                    instance->GetGuidData(DATA_VALITHRIA_DREAMWALKER));
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_dreamwalker_twisted_nightmares_SpellScript::HandleScript,
                EFFECT_2,
...
public:
    spell_dreamwalker_nightmare_cloud()
        : SpellScriptLoader("spell_dreamwalker_nightmare_cloud")
    {
    }
    ...
    class spell_dreamwalker_nightmare_cloud_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_dreamwalker_nightmare_cloud_AuraScript::PeriodicTick,
                EFFECT_0,
...
public:
    spell_dreamwalker_mana_void()
        : SpellScriptLoader("spell_dreamwalker_mana_void")
    {
    }
    ...
    class spell_dreamwalker_mana_void_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_dreamwalker_mana_void_AuraScript::PeriodicTick,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };
...
public:
    spell_dreamwalker_decay_periodic_timer()
        : SpellScriptLoader("spell_dreamwalker_decay_periodic_timer")
    {
    }

    class spell_dreamwalker_decay_periodic_timer_AuraScript
        : public AuraScript {
...
public:
    spell_dreamwalker_summoner()
        : SpellScriptLoader("spell_dreamwalker_summoner")
    {
    }
    ...
    class spell_dreamwalker_summoner_SpellScript : public SpellScript {
        ...
        {
            targets.remove_if(
                Acore::AllWorldObjectsInExactRange(GetCaster(), 250.0f, true));
            std::list<WorldObject*> list_copy = targets;
            targets.remove_if(
                Acore::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
            if (targets.empty()) {
                if (list_copy.empty())
                    return;
                targets = list_copy;
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_dreamwalker_summoner_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
            OnEffectHitTarget += SpellEffectFn(
                spell_dreamwalker_summoner_SpellScript::HandleForceCast,
                EFFECT_0,
                SPELL_EFFECT_FORCE_CAST);
        }
    };
...
public:
    spell_dreamwalker_summon_suppresser()
        : SpellScriptLoader("spell_dreamwalker_summon_suppresser")
    {
    }
    ...
    class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript {
        ...
        void PeriodicTick(AuraEffect const* /*aurEff*/)
        {
            ...
            caster->GetCreaturesWithEntryInRange(
                summoners, 200.0f, NPC_WORLD_TRIGGER);
            ...
            for (uint32 i = 0; i < 3; ++i)
                caster->CastSpell(
                    summoners.front(), SPELL_SUMMON_SUPPRESSER, true);
            for (uint32 i = 0; i < 3; ++i)
                caster->CastSpell(
                    summoners.back(), SPELL_SUMMON_SUPPRESSER, true);
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_dreamwalker_summon_suppresser_AuraScript::PeriodicTick,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };
...
public:
    spell_dreamwalker_summon_suppresser_effect()
        : SpellScriptLoader("spell_dreamwalker_summon_suppresser_effect")
    {
    }

    class spell_dreamwalker_summon_suppresser_effect_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_dreamwalker_summon_suppresser_effect_SpellScript);
...
public:
    spell_valithria_suppression()
        : SpellScriptLoader("spell_valithria_suppression")
    {
    }
    ...
    class spell_valithria_suppression_AuraScript : public AuraScript {
        ...
        void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            Unit::AuraApplicationMapBounds range =
                aam.equal_range(GetSpellInfo()->Id);
            uint32 count = target->GetAuraCount(GetSpellInfo()->Id);

            if (range.first == range.second)
                return;

            for (Unit::AuraApplicationMap::const_iterator itr = range.first;
                 itr != range.second;
                 ++itr)
                if (count == 1 ||
                    itr->second->GetBase()->GetEffect(EFFECT_0)->GetAmount()) {
                    itr->second->GetBase()->GetEffect(EFFECT_0)->SetAmount(
                        count * GetSpellInfo()->Effects[0].CalcValue());
                    break;
                }
        }
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            ...
            range.first->second->GetBase()->GetEffect(EFFECT_0)->SetAmount(
                count * GetSpellInfo()->Effects[0].CalcValue());
        }
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_valithria_suppression_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_MOD_HEALING_PCT,
                AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_valithria_suppression_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_MOD_HEALING_PCT,
...
public:
    achievement_portal_jockey()
        : AchievementCriteriaScript("achievement_portal_jockey")
    {
    }
    ...
    {
        return target && target->GetEntry() == NPC_VALITHRIA_DREAMWALKER &&
               !target->GetAI()->GetData(MISSED_PORTALS);
    }
};
