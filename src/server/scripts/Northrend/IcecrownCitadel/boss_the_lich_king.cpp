...
enum Spells {
    ...
    SPELL_HARVEST_SOUL_VALKYR    = 68985, // vehicle aura used by Val'kyr
                                          // Shadowguard and Strangulate Vehicle
    SPELL_CHARGE               = 74399,
...
enum Phases {
    ...
    PHASE_TRANSITION = 5,
    PHASE_FROSTMOURNE =
        6, // only set on heroic mode when all players are sent into frostmourne
    PHASE_OUTRO = 7,

    PHASE_MASK_NO_CAST_CHECK =
        (1 << PHASE_TRANSITION) | (1 << PHASE_FROSTMOURNE) | (1 << PHASE_OUTRO),
    PHASE_MASK_NO_VICTIM =
        (1 << PHASE_INTRO) | (1 << PHASE_OUTRO) | (1 << PHASE_FROSTMOURNE),
};
...
Position const LichKingIntro[3] = {{432.0851f, -2123.673f, 864.6582f, 0.0f},
...
enum MiscData {
    ...
    MUSIC_SPECIAL =
        17458, // Summon Shambling Horror, Remorseless Winter, Quake, Summon
               // Val'kyr Periodic, Harvest Soul, Vile Spirits
...
public:
    ShadowTrapLKTargetSelector(Creature* source,
                               bool      playerOnly = true,
                               bool      reqLOS     = false,
                               float     maxDist    = 0.0f)
        : _source(source), _playerOnly(playerOnly), _reqLOS(reqLOS),
          _maxDist(maxDist)
    {
    }
...
public:
    NonTankLKTargetSelector(Creature* source,
                            bool      playerOnly = true,
                            bool      reqLOS     = false,
                            float     maxDist    = 0.0f,
                            uint32    exclude1   = 0,
                            uint32    exclude2   = 0)
        : _source(source), _playerOnly(playerOnly), _reqLOS(reqLOS),
          _maxDist(maxDist), _exclude1(exclude1), _exclude2(exclude2)
    {
    }
    bool operator()(Unit const* target) const
    {
...
public:
    ...
    bool operator()(Unit const* target) const
    {
        ...
        if (target->HasAura(SPELL_HARVEST_SOUL_VALKYR) ||
            target->HasAura(SPELL_VALKYR_TARGET_SEARCH))
            return false;
...
public:
    ...
    bool Execute(uint64 /*time*/, uint32 /*diff*/) override
    {
        ...
        if (!_owner->GetVictim())
            if (Unit* target = _summoner->AI()->SelectTarget(
                    SelectTargetMethod::Random,
                    0,
                    NonTankLKTargetSelector(_summoner)))
                _owner->AI()->AttackStart(target);
...
public:
    ...
    bool Execute(uint64 /*time*/, uint32 /*diff*/) override
    {
        ...
        _owner->CastSpell(
            (Unit*)nullptr, SPELL_VILE_SPIRIT_DAMAGE_SEARCH, true);
        return true;
    }
...
public:
    ...
    bool Execute(uint64 /*time*/, uint32 /*diff*/) override
    {
        _owner->CastCustomSpell(SPELL_TRIGGER_VILE_SPIRIT_HEROIC,
        ...
        if (--_counter) {
            _owner->m_Events.AddEvent(this,
                                      _owner->m_Events.CalculateTime(3000));
            return false;
        }
...
public:
    ...
    {
        if (!unit || unit->GetTypeId() != TYPEID_PLAYER || unit == _sourceObj ||
            _sourceObj->GetVictim() == unit || !unit->isTargetableForAttack())
            return false;
        if (unit->HasAura(SPELL_PLAGUE_AVOIDANCE) ||
            unit->HasAura(SPELL_BOSS_HITTIN_YA_AURA) ||
            unit->HasAura(_notAura1) || unit->HasAura(_notAura2))
            return false;
...
public:
    ...
    {
        return (unit->GetPositionZ() - _baseObject->GetPositionZ() >
                _difference) != _reverse;
    }
...
public:
    ...
    struct boss_the_lich_kingAI : public BossAI {
        boss_the_lich_kingAI(Creature* creature)
            : BossAI(creature, DATA_THE_LICH_KING)
        {
        ...
        void Reset() override
        {
            ...
            if (!ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_HIGHLORD_TIRION_FORDRING)))
                me->SummonCreature(NPC_HIGHLORD_TIRION_FORDRING_LK,
                                   TirionSpawn,
        ...
        {
            if (!instance->CheckRequiredBosses(DATA_THE_LICH_KING,
                                               target->ToPlayer()) ||
                !me->IsVisible()) {
            ...
            me->RemoveAurasDueToSpell(
                SPELL_EMOTE_SIT_NO_SHEATH); // just to be sure

            events.ScheduleEvent(EVENT_BERSERK, 15min, EVENT_GROUP_BERSERK);
            events.ScheduleEvent(
                EVENT_SUMMON_SHAMBLING_HORROR, 15s, EVENT_GROUP_ABILITIES);
            events.ScheduleEvent(
                EVENT_SUMMON_DRUDGE_GHOUL, 10s, EVENT_GROUP_ABILITIES);
            events.ScheduleEvent(EVENT_INFEST, 5s, EVENT_GROUP_ABILITIES);
            events.ScheduleEvent(
                EVENT_NECROTIC_PLAGUE, 30s, 31s, EVENT_GROUP_ABILITIES);
            if (IsHeroic())
                events.ScheduleEvent(
                    EVENT_SHADOW_TRAP, 15s + 500ms, EVENT_GROUP_ABILITIES);
        }

        bool CanAIAttack(Unit const* target) const override
        {
            return me->IsVisible() && IsValidPlatformTarget(target) &&
                   !target->GetVehicle();
        }
        ...
        void KilledUnit(Unit* victim) override
        {
            if (victim->GetTypeId() == TYPEID_PLAYER && !me->IsInEvadeMode() &&
                _phase != PHASE_OUTRO &&
                _lastTalkTimeKill + 5 < GameTime::GetGameTime().count()) {
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_RESTORE_LIGHT:
                me->GetMap()->SetZoneOverrideLight(
                    AREA_THE_FROZEN_THRONE, 0, 5s);
                me->GetMap()->SetZoneWeather(
                    AREA_THE_FROZEN_THRONE, WEATHER_STATE_FINE, 0.5f);
                break;
            ...
            case ACTION_BREAK_FROSTMOURNE:
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_BROKEN_FROSTMOURNE, true);
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_BROKEN_FROSTMOURNE_2, false);
                SetEquipmentSlots(false, EQUIP_BROKEN_FROSTMOURNE);
                if (Creature* tirion = ObjectAccessor::GetCreature(
                        *me,
                        instance->GetGuidData(DATA_HIGHLORD_TIRION_FORDRING)))
            ...
            case ACTION_TELEPORT_BACK: {
                ...
                if (!IsHeroic() && _phase != PHASE_OUTRO && me->IsInCombat() &&
                    _lastTalkTimeBuff + 5 <= GameTime::GetGameTime().count())
                         ...
                         SpellSchoolMask) override
        {
            if (!attacker ||
                (_bFrostmournePhase &&
                 attacker->GetExactDistSq(495.708f, -2523.76f, 1049.95f) >
                     40.0f * 40.0f)) // frostmourne room, prevent exploiting
                                     // (tele hack to get back and damage him)
            {
                damage = 0;
                return;
            }

            if (_phase == PHASE_ONE && !HealthAbovePct(70) &&
                ...
                me->GetMotionMaster()->MovePoint(POINT_CENTER_1,
                                                 CenterPosition);
                return;
            }

            if (_phase == PHASE_TWO && !HealthAbovePct(40) &&
                !me->HasUnitState(UNIT_STATE_CASTING)) {
                _phase = PHASE_TRANSITION;
                me->SetReactState(REACT_PASSIVE);
                me->AttackStop();
                events.CancelEventGroup(EVENT_GROUP_ABILITIES);
                me->InterruptNonMeleeSpells(false);
                me->GetMotionMaster()->MovePoint(POINT_CENTER_2,
                                                 CenterPosition);
                return;
            }

            if (_phase == PHASE_THREE && HealthBelowPct(10) &&
                !me->HasUnitState(UNIT_STATE_CASTING)) {
                _phase = PHASE_OUTRO;
                EntryCheckPredicate pred(NPC_STRANGULATE_VEHICLE);
                summons.DoAction(ACTION_TELEPORT_BACK, pred);
                ...
                me->SetReactState(REACT_PASSIVE);
                me->AttackStop();
                me->GetMap()->SetZoneMusic(AREA_THE_FROZEN_THRONE,
            ...
            if (_phase == PHASE_OUTRO) {
                ...
                else if (damage >= me->GetHealth()) // dying...
                {
                    ...
                    if (Creature* frostmourne = me->FindNearestCreature(
                            NPC_FROSTMOURNE_TRIGGER, 50.0f))
                        frostmourne->DespawnOrUnsummon(1);
                    if (Creature* terenas = me->FindNearestCreature(
                            NPC_TERENAS_MENETHIL_OUTRO, 50.0f))
                        terenas->DespawnOrUnsummon(1);

                    me->m_Events.AddEvent(
                        new LichKingDeathEvent(*me),
                        me->m_Events.CalculateTime(
                            2500)); // die after spinning anim is over, so death
                                    // anim is visible
                    me->m_Events.AddEvent(new LichKingMovieEvent(*me),
                                          me->m_Events.CalculateTime(11500));
                }
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            case NPC_DRUDGE_GHOUL:
                ...
                summon->m_Events.AddEvent(new StartMovementEvent(me, summon),
                                          summon->m_Events.CalculateTime(5000));
                break;
            ...
            case NPC_VILE_SPIRIT: {
                ...
                if (_phase == PHASE_THREE)
                    summon->m_Events.AddEvent(
                        new VileSpiritActivateEvent(summon),
                        summon->m_Events.CalculateTime(15000));
                break;
            }
            ...
            case NPC_SHADOW_TRAP_TRIGGER:
                ...
                summon->UpdatePosition(summon->GetPositionX(),
                                       summon->GetPositionY(),
                                       summon->GetPositionZ(),
            ...
            case NPC_VALKYR_SHADOWGUARD:
                if (_phase == PHASE_THREE ||
                    events.GetNextEventTime(EVENT_QUAKE_2))
        ...
        void SummonedCreatureDies(Creature* summon, Unit*) override
        {
            ...
            case NPC_VILE_SPIRIT:
                ...
                summon->ToTempSummon()->SetTempSummonType(
        ...
        {
            if (spell->Id == HARVESTED_SOUL_BUFF && me->IsInCombat() &&
                !IsHeroic() && _phase != PHASE_OUTRO &&
                _lastTalkTimeBuff + 5 <= GameTime::GetGameTime().count()) {
        ...
        {
            if (spell->Id == REMORSELESS_WINTER_1 ||
                spell->Id == REMORSELESS_WINTER_2) {
                me->GetMap()->SetZoneOverrideLight(
                    AREA_THE_FROZEN_THRONE, LIGHT_SNOWSTORM, 5s);
                me->GetMap()->SetZoneWeather(
                    AREA_THE_FROZEN_THRONE, WEATHER_STATE_LIGHT_SNOW, 0.5f);
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
            ...
            case POINT_CENTER_1:
                ...
                // events.DelayEvents(62500, EVENT_GROUP_BERSERK); // delay
                // berserk timer, its not ticking during phase transitions,
                // 15mins on movies
                events.ScheduleEvent(EVENT_QUAKE, 62s + 500ms);
                events.ScheduleEvent(
                    EVENT_PAIN_AND_SUFFERING, 3500ms, EVENT_GROUP_ABILITIES);
                events.ScheduleEvent(
                    EVENT_SUMMON_ICE_SPHERE, 8s, EVENT_GROUP_ABILITIES);
                events.ScheduleEvent(
            ...
            case POINT_CENTER_2:
                ...
                // events.DelayEvents(62500, EVENT_GROUP_BERSERK); // delay
                // berserk timer, its not ticking during phase transitions, 15
                // mins on movies
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            // check phase first to prevent updating victim and entering evade
            // mode when not wanted
            ...
            // during Remorseless Winter phases The Lich King is channeling a
            // spell, but we must continue casting other spells
            if (me->HasUnitState(UNIT_STATE_CASTING) &&
                !((1 << _phase) & PHASE_MASK_NO_CAST_CHECK))
                return;
            ...
            case EVENT_START_ATTACK:
                ...
                if (_phase == PHASE_FROSTMOURNE) {
                    ...
                    events.RescheduleEvent(
                        EVENT_DEFILE, 0ms, EVENT_GROUP_ABILITIES);
                    events.RescheduleEvent(
                        EVENT_SOUL_REAPER, 7s, 12s, EVENT_GROUP_ABILITIES);

                    for (SummonList::iterator i = summons.begin();
                         i != summons.end();
                         ++i)
                        if (Creature* summon =
                                ObjectAccessor::GetCreature(*me, *i))
            ...
            case EVENT_QUAKE:
                ...
                events.ScheduleEvent(
                    EVENT_SUMMON_VALKYR, 20s, EVENT_GROUP_ABILITIES);
                events.ScheduleEvent(
                    EVENT_SOUL_REAPER, 40s, EVENT_GROUP_ABILITIES);
            ...
            case EVENT_QUAKE_2:
                ...
                events.ScheduleEvent(EVENT_DEFILE, 38s, EVENT_GROUP_ABILITIES);
                events.ScheduleEvent(
                    EVENT_VILE_SPIRITS, 20s, EVENT_GROUP_VILE_SPIRITS);
                events.ScheduleEvent(IsHeroic() ? EVENT_HARVEST_SOULS
                                                : EVENT_HARVEST_SOUL,
                                     14s,
                                     EVENT_GROUP_ABILITIES);
            ...
            case EVENT_SUMMON_SHAMBLING_HORROR:
                ...
                me->GetMap()->SetZoneMusic(AREA_THE_FROZEN_THRONE,
                                           MUSIC_SPECIAL);
                events.ScheduleEvent(
                    EVENT_SUMMON_SHAMBLING_HORROR, 60s, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_SUMMON_DRUDGE_GHOUL:
                me->CastSpell(me, SPELL_SUMMON_DRUDGE_GHOULS, false);
                events.ScheduleEvent(
                    EVENT_SUMMON_DRUDGE_GHOUL, 30s, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_INFEST:
                me->CastSpell(me, SPELL_INFEST, false);
                events.ScheduleEvent(
                    EVENT_INFEST, 22s + 500ms, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_NECROTIC_PLAGUE:
                if (Unit* target = SelectTarget(
                        ...
                        NecroticPlagueTargetCheck(
                            me, NECROTIC_PLAGUE_LK, NECROTIC_PLAGUE_PLR))) {
                    ...
                    events.ScheduleEvent(
                        EVENT_NECROTIC_PLAGUE, 5s, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_SHADOW_TRAP:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random,
                        0,
                        ShadowTrapLKTargetSelector(me, true, true, 100.0f)))
            ...
            case EVENT_PAIN_AND_SUFFERING:
                ...
                events.ScheduleEvent(
                    EVENT_PAIN_AND_SUFFERING,
                    (IsHeroic() ? urand(1250, 1750) : urand(1750, 2250)),
                    EVENT_GROUP_ABILITIES);
                break;
            case EVENT_SUMMON_ICE_SPHERE:
                me->CastSpell((Unit*)nullptr, SPELL_SUMMON_ICE_SPHERE, false);
                events.ScheduleEvent(
                    EVENT_SUMMON_ICE_SPHERE, 7500ms, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_SUMMON_RAGING_SPIRIT:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 100.0f, true))
                    me->CastSpell(target, SPELL_RAGING_SPIRIT, false);
                events.ScheduleEvent(EVENT_SUMMON_RAGING_SPIRIT,
                                     (!HealthAbovePct(40) ? 15s : 20s),
                                     EVENT_GROUP_ABILITIES);
                break;
            case EVENT_DEFILE: {
                uint32 evTime = events.GetNextEventTime(EVENT_SUMMON_VALKYR);
                // if defile (cast time 2sec) is less than 3 before valkyr
                // appears we've to decide
                if (evTime && (events.GetTimer() > evTime ||
                               evTime - events.GetTimer() < 5000)) {
                    // if valkyr is less than 1.5 secs after defile (cast time 2
                    // sec) then we've a sync issue, so we need to cancel it
                    // (break) and schedule a defile to be casted 5 or 4 seconds
                    // after valkyr
                    if (events.GetTimer() > evTime ||
                        evTime - events.GetTimer() < 3500) {
                        uint32 t = events.GetTimer() > evTime
                                       ? 0
                                       : evTime - events.GetTimer();
                        events.ScheduleEvent(EVENT_DEFILE,
                                             t + (Is25ManRaid() ? 5000 : 4000),
                                             EVENT_GROUP_ABILITIES);
                        break;
                    }

                    // if valkyr is coming between 1.5 and 3 seconds after
                    // defile then we've to delay valkyr just a bit
                    events.RescheduleEvent(
                        EVENT_SUMMON_VALKYR, 5s, EVENT_GROUP_ABILITIES);
                }

                if (Unit* target = SelectTarget(SelectTargetMethod::Random,
                                                0,
                                                DefileTargetSelector(me))) {
                    Talk(EMOTE_DEFILE_WARNING);
                    me->CastSpell(target, SPELL_DEFILE, false);
                    // defile has a fixed CD (from dbm) that can be variable
                    // only if no target has been found at the moment (schedule
                    // after 1 second)
                    events.ScheduleEvent(
                        EVENT_DEFILE, 32s + 500ms, EVENT_GROUP_ABILITIES);
                }
                else {
            ...
            case EVENT_SOUL_REAPER:
                if (me->IsWithinMeleeRange(me->GetVictim())) {
                    ...
                    events.ScheduleEvent(
                        EVENT_SOUL_REAPER, 30s + 500ms, EVENT_GROUP_ABILITIES);
                }
                else
                    events.ScheduleEvent(
            ...
            case EVENT_SUMMON_VALKYR: {
                ...
                events.ScheduleEvent(
                    EVENT_SUMMON_VALKYR, 45s, EVENT_GROUP_ABILITIES);

                // schedule a defile (or reschedule it) if next defile event
                // doesn't exist ( now > next defile ) or defile is coming too
                // soon
                uint32 minTime = (Is25ManRaid() ? 5000 : 4000);
                if (uint32 evTime = events.GetNextEventTime(EVENT_DEFILE))
                    if (events.GetTimer() > evTime ||
                        evTime - events.GetTimer() < minTime) {
                        events.RescheduleEvent(
                            EVENT_DEFILE, minTime, EVENT_GROUP_ABILITIES);
                    }
            } break;
            case EVENT_VILE_SPIRITS:
                ...
                events.ScheduleEvent(
                    EVENT_VILE_SPIRITS, 30s, EVENT_GROUP_VILE_SPIRITS);
                break;
            case EVENT_HARVEST_SOUL:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random,
                        0,
                        NonTankLKTargetSelector(me, true, true, 55.0f))) {
                    ...
                    events.ScheduleEvent(
                        EVENT_HARVEST_SOUL, 10s, EVENT_GROUP_ABILITIES);
                break;
            case EVENT_HARVEST_SOULS:
                ...
                events.ScheduleEvent(
                    EVENT_FROSTMOURNE_HEROIC, 6s, EVENT_GROUP_ABILITIES);
                events.ScheduleEvent(
                    EVENT_HARVEST_SOULS, 100s, 110s, EVENT_GROUP_ABILITIES);
                ...
                for (SummonList::iterator i = summons.begin();
                    ...
                    if (Creature* summon =
                        ...
                        if (summon->GetEntry() == NPC_VILE_SPIRIT) {
                            ...
                            summon->m_Events.AddEvent(
                                new VileSpiritActivateEvent(summon),
                                summon->m_Events.CalculateTime(55000));
            ...
            case EVENT_FROSTMOURNE_HEROIC:
                ...
                if (TempSummon* terenas = me->GetMap()->SummonCreature(
                        NPC_TERENAS_MENETHIL_FROSTMOURNE_H,
                        TerenasSpawnHeroic,
                        nullptr,
                        55000)) {
                    terenas->AI()->DoAction(ACTION_FROSTMOURNE_INTRO);
                    if (Creature* spawner = terenas->FindNearestCreature(
                            NPC_WORLD_TRIGGER_INFINITE_AOI, 100.0f, true)) {
                        spawner->CastSpell(spawner,
                                           SPELL_SUMMON_SPIRIT_BOMB_1,
                                           true); // summons bombs randomly
                        spawner->CastSpell(spawner,
                                           SPELL_SUMMON_SPIRIT_BOMB_2,
                                           true); // summons bombs on players
                        spawner->m_Events.AddEvent(
                            new TriggerWickedSpirit(spawner),
                            spawner->m_Events.CalculateTime(3000));
                        terenas->SetImmuneToAll(
                            true); // to avoid being healed by player trinket
                                   // procs. terenas' health doesn't matter on
                                   // heroic
                    }
                }
                break;
            ...
            if (!me->HasReactState(REACT_PASSIVE) && me->GetVictim() &&
                ...
                me->GetMotionMaster()->MoveCharge(
                    me->GetVictim()->GetPositionX(),
                    me->GetVictim()->GetPositionY(),
                    me->GetVictim()->GetPositionZ(),
    ...
    struct npc_tirion_fordringAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (_instance->GetBossState(DATA_THE_LICH_KING) == DONE ||
                (me->GetMap()->IsHeroic() &&
                 !_instance->GetData(DATA_LK_HC_AVAILABLE)))
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            case POINT_TIRION_INTRO:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_THE_LICH_KING))) {
                    if (!theLichKing->IsAlive() || !theLichKing->IsVisible())
                        break;
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                       EMOTE_STATE_READY2H);
                    theLichKing->SetStandState(UNIT_STAND_STATE_STAND);
                    theLichKing->SetSheath(SHEATH_STATE_MELEE);
                    theLichKing->RemoveAurasDueToSpell(
        ...
        {
            Creature* theLichKing = ObjectAccessor::GetCreature(
                *me, _instance->GetGuidData(DATA_THE_LICH_KING));
            if (me->GetCreatureTemplate()->GossipMenuId == sender && !action &&
                me->HasNpcFlag(UNIT_NPC_FLAG_GOSSIP) && theLichKing &&
                !theLichKing->IsInEvadeMode()) {
                if (me->GetMap()->IsHeroic() &&
                    !_instance->GetData(DATA_LK_HC_AVAILABLE))
                    return;
                me->RemoveNpcFlag(UNIT_NPC_FLAG_GOSSIP);
                me->SetWalk(true);
                me->GetMotionMaster()->MovePoint(POINT_TIRION_INTRO,
                                                 TirionIntro);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INTRO_LK_MOVE:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    path.push_back(G3D::Vector3(theLichKing->GetPositionX(),
                                                theLichKing->GetPositionY(),
                                                theLichKing->GetPositionZ()));
                    for (uint8 i = 0; i < 3; ++i)
                        path.push_back(
                            G3D::Vector3(LichKingIntro[i].GetPositionX(),
                                         LichKingIntro[i].GetPositionY(),
                                         LichKingIntro[i].GetPositionZ()));
            ...
            case EVENT_INTRO_LK_TALK_1:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    theLichKing->HandleEmoteCommand(
            ...
            case EVENT_INTRO_LK_EMOTE_CAST_SHOUT:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    theLichKing->CastSpell(
                        theLichKing, SPELL_EMOTE_SHOUT_NO_SHEATH, false);
                break;
            ...
            case EVENT_INTRO_LK_CAST_FREEZE:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    theLichKing->CastSpell(
                        (Unit*)nullptr, SPELL_ICE_LOCK, false);
            ...
            case EVENT_OUTRO_LK_TALK_1:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    theLichKing->CastSpell(
                        (Unit*)nullptr, SPELL_FURY_OF_FROSTMOURNE_NO_REZ, true);
                    Map::PlayerList const& pl = me->GetMap()->GetPlayers();
                    for (Map::PlayerList::const_iterator itr = pl.begin();
                         itr != pl.end();
                         ++itr)
            ...
            case EVENT_OUTRO_LK_TALK_2:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    theLichKing->CastSpell(
                        (Unit*)nullptr, SPELL_EMOTE_QUESTION_NO_SHEATH, false);
                }
                break;
            ...
            case EVENT_OUTRO_LK_MOVE_CENTER:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_THE_LICH_KING))) {
                    theLichKing->GetMotionMaster()->MovePoint(0,
                                                              CenterPosition);
                    uint32 travelTime =
                        1000 * theLichKing->GetExactDist(&CenterPosition) /
                            theLichKing->GetSpeed(MOVE_WALK) +
                        1000;

                    _events.ScheduleEvent(EVENT_OUTRO_LK_TALK_4,
                                          1 + travelTime);
                    _events.ScheduleEvent(EVENT_OUTRO_LK_RAISE_DEAD,
                                          1000 + travelTime);
                    _events.ScheduleEvent(EVENT_OUTRO_LK_TALK_5,
                                          29000 + travelTime);
                }
                break;
            ...
            case EVENT_OUTRO_LK_RAISE_DEAD:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                        *me, _instance->GetGuidData(DATA_THE_LICH_KING))) {
                    theLichKing->CastSpell(
                        (Unit*)nullptr, SPELL_RAISE_DEAD, false);
            ...
            case EVENT_OUTRO_LK_TALK_6:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    theLichKing->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID,
                                                EQUIP_UNEQUIP);
                    theLichKing->CastSpell((Unit*)nullptr,
                                           SPELL_SUMMON_BROKEN_FROSTMOURNE_3,
                                           true);
                    me->GetMap()->SetZoneOverrideLight(
                        AREA_THE_FROZEN_THRONE, LIGHT_SOULSTORM, 10s);
                    me->GetMap()->SetZoneWeather(
                        AREA_THE_FROZEN_THRONE, WEATHER_STATE_BLACKSNOW, 0.5f);

                    _events.ScheduleEvent(
                        EVENT_OUTRO_AFTER_SUMMON_BROKEN_FROSTMOURNE, 1s);
                    _events.ScheduleEvent(EVENT_OUTRO_KNOCK_BACK, 3s);
                    break;
                }
                break;
            case EVENT_OUTRO_AFTER_SUMMON_BROKEN_FROSTMOURNE:
                if (Creature* frostmourne =
                        me->FindNearestCreature(NPC_FROSTMOURNE_TRIGGER, 50.0f))
                    frostmourne->CastSpell(
                        (Unit*)nullptr, SPELL_BROKEN_FROSTMOURNE, true);
                break;
            case EVENT_OUTRO_KNOCK_BACK:
                if (Creature* frostmourne =
                        me->FindNearestCreature(NPC_FROSTMOURNE_TRIGGER, 50.0f))
                    frostmourne->CastSpell(
                        (Unit*)nullptr, SPELL_BROKEN_FROSTMOURNE_KNOCK, false);
                break;
            case EVENT_OUTRO_SOUL_BARRAGE:
                if (Creature* theLichKing = ObjectAccessor::GetCreature(
                    ...
                    sCreatureTextMgr->SendSound(theLichKing,
                                                SOUND_PAIN,
            ...
            case EVENT_OUTRO_SUMMON_TERENAS:
                if (Creature* frostmourne = me->FindNearestCreature(
                        NPC_FROSTMOURNE_TRIGGER, 50.0f)) {
                    frostmourne->CastSpell(
                        (Unit*)nullptr, SPELL_SUMMON_TERENAS, false);
                    if (Creature* terenas = me->FindNearestCreature(
                            NPC_TERENAS_MENETHIL_OUTRO, 50.0f))
            ...
            case EVENT_OUTRO_TERENAS_TALK_2:
                if (Creature* terenas = me->FindNearestCreature(
                    ...
                    terenas->CastSpell(
                        (Unit*)nullptr, SPELL_MASS_RESURRECTION, false);
                    if (Creature* lichKing = ObjectAccessor::GetCreature(
                            *me, _instance->GetGuidData(DATA_THE_LICH_KING))) {
            ...
            case EVENT_OUTRO_FORDRING_MOVE_1:
                me->GetMotionMaster()->MovePoint(POINT_TIRION_OUTRO,
                                                 OutroPosition1);
                break;
...
public:
    ...
    class spell_the_lich_king_quake_SpellScript : public SpellScript {
        ...
        {
            if (GameObject* platform = ObjectAccessor::GetGameObject(
                    *GetCaster(),
                    GetCaster()->GetInstanceScript()->GetGuidData(
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_the_lich_king_quake_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
            OnEffectHit += SpellEffectFn(
                spell_the_lich_king_quake_SpellScript::HandleSendEvent,
                EFFECT_1,
    ...
    class spell_the_lich_king_jump_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_the_lich_king_jump_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    spell_the_lich_king_jump_remove_aura()
        : SpellScriptLoader("spell_the_lich_king_jump_remove_aura")
    {
    }
...
public:
    spell_the_lich_king_play_movie()
        : SpellScriptLoader("spell_the_lich_king_play_movie")
    {
    }
    ...
    class spell_the_lich_king_play_movie_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_the_lich_king_play_movie_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_the_lich_king_infest()
        : SpellScriptLoader("spell_the_lich_king_infest")
    {
    }
    ...
    class spell_the_lich_king_infest_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_the_lich_king_infest_AuraScript::OnPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE);
            OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
                spell_the_lich_king_infest_AuraScript::OnUpdate,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE);
        }
    };
    ...
    class spell_the_lich_king_necrotic_plague_AuraScript : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            ...
            if (dispel)
                values.AddSpellMod(
                    ...
                    AURA_REMOVE_BY_ENEMY_SPELL); // add as marker (spell has no
                                                 // effect 1)
            GetTarget()->CastCustomSpell(SPELL_NECROTIC_PLAGUE_JUMP,
                                         values,
                                         nullptr,
                                         TRIGGERED_FULL_MASK,
                                         nullptr,
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_the_lich_king_necrotic_plague_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
...
public:
    spell_the_lich_king_necrotic_plague_jump()
        : SpellScriptLoader("spell_the_lich_king_necrotic_plague_jump")
    {
    }
    ...
    class spell_the_lich_king_necrotic_plague_SpellScript : public SpellScript {
        ...
        void CheckAura(SpellMissInfo missInfo)
        {
            ...
            else if (uint32 spellId = sSpellMgr->GetSpellIdForDifficulty(
                         SPELL_NECROTIC_PLAGUE, GetHitUnit()))
        ...
        void AddMissingStack()
        {
            if (GetHitAura() && !_hadJumpingAura) {
                uint32 spellId = sSpellMgr->GetSpellIdForDifficulty(
                    SPELL_NECROTIC_PLAGUE, GetHitUnit());
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_the_lich_king_necrotic_plague_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
            BeforeHit += BeforeSpellHitFn(
                spell_the_lich_king_necrotic_plague_SpellScript::CheckAura);
            OnHit +=
                SpellHitFn(spell_the_lich_king_necrotic_plague_SpellScript::
                               AddMissingStack);
        }
    ...
    class spell_the_lich_king_necrotic_plague_AuraScript : public AuraScript {
                     ...
                     AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
                if (caster->GetAI())
                    caster->GetAI()->SetData(DATA_PLAGUE_STACK,
                                             GetStackAmount());
        }
        ...
        void AfterDispel(AuraEffect const* aurEff,
                         AuraEffectHandleModes /*mode*/)
        {
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_the_lich_king_necrotic_plague_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
            ...
            AfterEffectRemove += AuraEffectRemoveFn(
                ...
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_the_lich_king_necrotic_plague_AuraScript::OnDispel,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
                AURA_EFFECT_HANDLE_REAPPLY);
            AfterEffectApply += AuraEffectApplyFn(
                spell_the_lich_king_necrotic_plague_AuraScript::AfterDispel,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
                AURA_EFFECT_HANDLE_REAPPLY);
        }
...
public:
    spell_the_lich_king_shadow_trap_visual()
        : SpellScriptLoader("spell_the_lich_king_shadow_trap_visual")
    {
    }

    class spell_the_lich_king_shadow_trap_visual_AuraScript
        : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            if (GetTargetApplication()->GetRemoveMode() ==
                ...
                GetTarget()->CastSpell(
                    GetTarget(), SPELL_SHADOW_TRAP_AURA, TRIGGERED_NONE);
        }

        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_the_lich_king_shadow_trap_visual_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_the_lich_king_shadow_trap_periodic()
        : SpellScriptLoader("spell_the_lich_king_shadow_trap_periodic")
    {
    }
    ...
    class spell_the_lich_king_shadow_trap_periodic_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_the_lich_king_shadow_trap_periodic_SpellScript);
        ...
        void CheckTargetCount(std::list<WorldObject*>& targets)
        {
            ...
            GetCaster()->CastSpell(
                (Unit*)nullptr, SPELL_SHADOW_TRAP_KNOCKBACK, true);
...
public:
    spell_the_lich_king_ice_burst_target_search()
        : SpellScriptLoader("spell_the_lich_king_ice_burst_target_search")
    {
    }

    class spell_the_lich_king_ice_burst_target_search_SpellScript
        : public SpellScript {
        PrepareSpellScript(
...
public:
    ...
    struct npc_icc_ice_sphereAI : public ScriptedAI {
        ...
        void SelectNewTarget()
        {
            ...
            me->GetMotionMaster()->Clear();
            me->StopMoving();
            if (Player* p = ScriptedAI::SelectTargetFromPlayerList(
                    120.0f, SPELL_ICE_PULSE, false)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!targetGUID || !me->GetVictim() ||
                !me->IsNonMeleeSpellCast(false, false, true, false, true))
                SelectNewTarget();
            else {
                Unit* target = ObjectAccessor::GetUnit(*me, targetGUID);
                if (me->GetVictim()->GetGUID() != targetGUID || !target ||
                    !me->IsValidAttackTarget(target) ||
                    target->HasUnitFlag2(UNIT_FLAG2_FEIGN_DEATH) ||
                    target->GetExactDist2dSq(&CenterPosition) > 75.0f * 75.0f ||
                    target->GetPositionZ() < 830.0f ||
                    target->GetPositionZ() > 855.0f)
                    SelectNewTarget();
            }
        }
    };
...
public:
    spell_the_lich_king_raging_spirit()
        : SpellScriptLoader("spell_the_lich_king_raging_spirit")
    {
    }
    ...
    class spell_the_lich_king_raging_spirit_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_the_lich_king_raging_spirit_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    struct npc_raging_spiritAI : public ScriptedAI {
        npc_raging_spiritAI(Creature* creature)
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
            ...
            if (TempSummon* summon = me->ToTempSummon())
                if (Unit* summoner = summon->GetSummonerUnit())
                    if (summoner->GetTypeId() == TYPEID_PLAYER &&
                        summoner->IsAlive() &&
                        !summoner->ToPlayer()->IsBeingTeleported() &&
                        summoner->FindMap() == me->GetMap()) {
                        valid = true;
                        summoner->CastSpell(
                            me, SPELL_RAGING_SPIRIT_VISUAL_CLONE, true);
                    }
            if (!valid) {
                if (Player* plr =
                        ScriptedAI::SelectTargetFromPlayerList(100.0f, 0, true))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_RAGING_SPIRIT_UNROOT: {
                ...
                if (!me->GetVictim()) {
                    ...
                    if (!valid)
                        if (Player* plr =
                                ScriptedAI::SelectTargetFromPlayerList(
                                    100.0f, 0, true))
                            AttackStart(plr);
                }
...
public:
    spell_the_lich_king_defile()
        : SpellScriptLoader("spell_the_lich_king_defile")
    {
    }
    ...
    class spell_the_lich_king_defile_SpellScript : public SpellScript {
        ...
        void CorrectRange(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(Acore::AllWorldObjectsInExactRange(
                GetCaster(),
                10.0f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X),
                true));
            uint32 strangulatedAura[4] = {68980, 74325, 74296, 74297};
            targets.remove_if(Acore::UnitAuraCheck(
                true,
                strangulatedAura[GetCaster()->GetMap()->GetDifficulty()]));
        }

        void ChangeDamageAndGrow()
        {
            SetHitDamage(int32(GetHitDamage() * GetCaster()->GetFloatValue(
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_the_lich_king_defile_SpellScript::CorrectRange,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_the_lich_king_defile_SpellScript::CorrectRange,
                ...
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnHit += SpellHitFn(
                spell_the_lich_king_defile_SpellScript::ChangeDamageAndGrow);
        }
    };
...
public:
    spell_the_lich_king_soul_reaper()
        : SpellScriptLoader("spell_the_lich_king_soul_reaper")
    {
    }
    ...
    class spell_the_lich_king_soul_reaper_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_the_lich_king_soul_reaper_AuraScript::OnPeriodic,
                EFFECT_1,
                SPELL_AURA_PERIODIC_DAMAGE);
        }
    };
...
public:
    ...
    struct npc_valkyr_shadowguardAI : public NullCreatureAI {
        npc_valkyr_shadowguardAI(Creature* creature)
            : NullCreatureAI(creature), didbelow50pct(false), dropped(false),
              _instance(creature->GetInstanceScript())
        {
            me->SetReactState(REACT_PASSIVE);
            me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, true);
            me->ApplySpellImmune(
                0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, true);
            _events.Reset();
        ...
        void GoSiphon()
        {
            ...
            _destPoint.Relocate(
                CenterPosition.GetPositionX() + dist * cos(angle),
                CenterPosition.GetPositionY() + dist * std::sin(angle),
                855.0f + frand(0.0f, 4.0f),
                0.0f);
        ...
        void DamageTaken(Unit*,
                         ...
                         SpellSchoolMask) override
        {
            if (IsHeroic() && !didbelow50pct && !dropped &&
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            case EVENT_CHARGE: {
                bool valid = false;
                if (Player* target =
                        ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
                    if (target->FindMap() == me->GetMap() &&
                        target->GetExactDist(me) < 15.0f &&
                        !target->GetVehicle())
                        if (GameObject* platform =
                                ObjectAccessor::GetGameObject(
                                    *me,
                                    _instance->GetGuidData(
                                        DATA_ARTHAS_PLATFORM))) {
                            std::list<Creature*> triggers;
                            GetCreatureListWithEntryInGrid(
                                triggers, me, NPC_WORLD_TRIGGER, 150.0f);
                            triggers.remove_if(
                                HeightDifferenceCheck(platform, 5.0f, true));
                            if (!triggers.empty()) {
                                valid = true;
                                triggers.sort(
                                    Acore::ObjectDistanceOrderPred(me));
                                ...
                                me->CastSpell(
                                    target, SPELL_VALKYR_CARRY, false);
                                _destPoint.Relocate(triggers.front());
                                _events.Reset();
                                _events.ScheduleEvent(EVENT_MOVE_TO_DROP_POS,
                                                      1s);
                            }
                        }
                if (!valid) {
            ...
            case POINT_DROP_PLAYER: {
                if (didbelow50pct || dropped)
                    break;
                if (me->GetExactDist(&_destPoint) >
                    1.5f) // movement was interrupted (probably by a stun, start
                          // again)
                {
                    _events.Reset();
                ...
                me->CastSpell(
                    (Unit*)nullptr, SPELL_EJECT_ALL_PASSENGERS, false);
            ...
            case POINT_START_SIPHON:
                ...
                me->ApplySpellImmune(
                    0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, false);
                me->ApplySpellImmune(
                    0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, false);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_GRAB_PLAYER:
                if (!_grabbedPlayer) {
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_VALKYR_TARGET_SEARCH, false);
            ...
            case EVENT_MOVE_TO_DROP_POS:
                ...
                me->SetDisableGravity(false);
                me->GetMotionMaster()->MovePoint(
                    POINT_DROP_PLAYER, _destPoint, false);
                me->SetDisableGravity(true, true);
                me->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                break;
            case EVENT_MOVE_TO_SIPHON_POS:
                me->RemoveUnitFlag(
                    UNIT_FLAG_NON_ATTACKABLE); // just in case if passenger
                                               // disappears so quickly that
                                               // EVENT_MOVE_TO_DROP_POS is
                                               // never executed
                {
                    int32 bp0 = 80;
                    me->CastCustomSpell(me, 1557, &bp0, nullptr, nullptr, true);
                }
                me->SetCanFly(true);
                me->SetDisableGravity(true);
                me->SendMovementFlagUpdate();
                me->GetMotionMaster()->MovePoint(POINT_START_SIPHON,
                                                 _destPoint);
                break;
            case EVENT_LIFE_SIPHON: {
                Unit*                       target = nullptr;
                Unit::AuraEffectList const& tauntAuras =
                    me->GetAuraEffectsByType(SPELL_AURA_MOD_TAUNT);
                if (!tauntAuras.empty())
                ...
                if (!target)
                    if (Creature* lichKing = ObjectAccessor::GetCreature(
                            *me, _instance->GetGuidData(DATA_THE_LICH_KING)))
                        target = lichKing->AI()->SelectTarget(
                            SelectTargetMethod::Random,
                            0,
                            NonTankLKTargetSelector(
                                lichKing, true, false, 100.0f));
                if (target)
...
public:
    spell_the_lich_king_summon_into_air()
        : SpellScriptLoader("spell_the_lich_king_summon_into_air")
    {
    }
    ...
    class spell_the_lich_king_summon_into_air_SpellScript : public SpellScript {
        ...
        void ModDestHeight(SpellEffIndex effIndex)
        {
            ...
            WorldLocation* dest =
                const_cast<WorldLocation*>(GetExplTargetDest());
        ...
        {
            OnEffectHit += SpellEffectFn(
                spell_the_lich_king_summon_into_air_SpellScript::ModDestHeight,
                EFFECT_0,
                SPELL_EFFECT_SUMMON);
        }
    };
...
public:
    spell_the_lich_king_teleport_to_frostmourne_hc()
        : SpellScriptLoader("spell_the_lich_king_teleport_to_frostmourne_hc")
    {
    }

    class spell_the_lich_king_teleport_to_frostmourne_hc_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void ModDest(SpellEffIndex /*effIndex*/)
        {
            ...
            Position const offset = {
                dist * cos(angle), dist * std::sin(angle), 0.0f, 0.0f};
            WorldLocation* dest =
                const_cast<WorldLocation*>(GetExplTargetDest());
            dest->RelocateOffset(offset);
            GetHitDest()->RelocateOffset(offset);
...
public:
    spell_the_lich_king_valkyr_target_search()
        : SpellScriptLoader("spell_the_lich_king_valkyr_target_search")
    {
    }
    ...
    class spell_the_lich_king_valkyr_target_search_SpellScript
        ...
        void SelectTarget(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(Acore::UnitAuraCheck(
                true, SPELL_BOSS_HITTIN_YA_AURA)); // done in dbc, but just to
                                                   // be sure xd
            targets.remove_if(
                Acore::UnitAuraCheck(true, SPELL_HARVEST_SOUL_VALKYR));
            if (InstanceScript* _instance = caster->GetInstanceScript())
                if (Creature* lichKing = ObjectAccessor::GetCreature(
                        *caster, _instance->GetGuidData(DATA_THE_LICH_KING)))
                    if (Spell* s =
                            lichKing->GetCurrentSpell(CURRENT_GENERIC_SPELL))
                        if (s->GetSpellInfo()->Id == SPELL_DEFILE &&
                            s->m_targets.GetUnitTarget())
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            PreventHitDefaultEffect(effIndex);
            if (Unit* target = GetHitUnit()) {
                GetCaster()->GetMotionMaster()->MoveCharge(
                    target->GetPositionX(),
                    target->GetPositionY(),
                    target->GetPositionZ() + 4.0f,
                    42.0f,
                    EVENT_CHARGE);
...
public:
    spell_the_lich_king_cast_back_to_caster()
        : SpellScriptLoader("spell_the_lich_king_cast_back_to_caster")
    {
    }

    class spell_the_lich_king_cast_back_to_caster_SpellScript
        : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_the_lich_king_cast_back_to_caster_SpellScript::
                    HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
...
public:
    spell_the_lich_king_life_siphon()
        : SpellScriptLoader("spell_the_lich_king_life_siphon")
    {
    }
    ...
    class spell_the_lich_king_life_siphon_SpellScript : public SpellScript {
        ...
        void TriggerHeal()
        {
            GetHitUnit()->CastCustomSpell(SPELL_LIFE_SIPHON_HEAL,
        ...
        {
            AfterHit += SpellHitFn(
                spell_the_lich_king_life_siphon_SpellScript::TriggerHeal);
        }
    };
...
public:
    spell_the_lich_king_vile_spirits()
        : SpellScriptLoader("spell_the_lich_king_vile_spirits")
    {
    }
    ...
    class spell_the_lich_king_vile_spirits_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_the_lich_king_vile_spirits_AuraScript::OnPeriodic,
                EFFECT_0,
...
public:
    spell_the_lich_king_vile_spirits_visual()
        : SpellScriptLoader("spell_the_lich_king_vile_spirits_visual")
    {
    }
    ...
    class spell_the_lich_king_vile_spirits_visual_SpellScript
        ...
        void Register() override
        {
            OnEffectLaunch += SpellEffectFn(
                spell_the_lich_king_vile_spirits_visual_SpellScript::
                    ModDestHeight,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
...
public:
    ...
    class spell_the_lich_king_vile_spirit_move_target_search_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            GetCaster()->AddThreat(GetHitUnit(),
                                   GetCaster()->GetMaxHealth() * 0.2f);
        }
...
public:
    ...
    class spell_the_lich_king_vile_spirit_damage_target_search_SpellScript
        : public SpellScript {
        PrepareSpellScript(
...
public:
    spell_the_lich_king_harvest_soul()
        : SpellScriptLoader("spell_the_lich_king_harvest_soul")
    {
    }
    ...
    class spell_the_lich_king_harvest_soul_AuraScript : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            ...
            if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_DEATH)
                GetTarget()->CastSpell(
                    ...
                    nullptr,
                    nullptr,
                    GetTarget()->GetInstanceScript()->GetGuidData(
                        DATA_THE_LICH_KING));
        }
        ...
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_the_lich_king_harvest_soul_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DAMAGE,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    struct npc_strangulate_vehicleAI : public NullCreatureAI {
        npc_strangulate_vehicleAI(Creature* creature)
            : NullCreatureAI(creature), _instance(creature->GetInstanceScript())
        {
        }
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            if (Creature* lichKing = ObjectAccessor::GetCreature(
                ...
                me->SetWalk(false);
                Movement::PointsArray path;
                path.push_back(G3D::Vector3(me->GetPositionX(),
                                            me->GetPositionY(),
                                            me->GetPositionZ()));
                path.push_back(G3D::Vector3(
                    me->GetPositionX(), me->GetPositionY(), 843.0f));
                ...
                me->CastSpell(
                    summoner->ToUnit(), SPELL_HARVEST_SOUL_VEHICLE, true);
                ...
                // this will let us easily access all creatures of this entry on
                // heroic mode when its time to teleport back
                lichKing->AI()->JustSummoned(me);
            }
        }
        ...
        void DoAction(int32 action) override
        {
            ...
            if (TempSummon* summ = me->ToTempSummon()) {
                if (Unit* summoner = summ->GetSummonerUnit()) {
                    bool buff = _instance->GetBossState(DATA_THE_LICH_KING) ==
                                    IN_PROGRESS &&
                    ...
                    if (summoner->GetTypeId() == TYPEID_PLAYER &&
                        ...
                        if (buff)
                            summoner->CastSpell(
                                ...
                                nullptr,
                                nullptr,
                                _instance->GetGuidData(DATA_THE_LICH_KING));

                        me->CastSpell(
                            summoner, SPELL_HARVEST_SOUL_TELEPORT_BACK, false);
                    }
                    else if (buff)
                        me->CastSpell(
                            (Unit*)nullptr,
                    ...
                    summoner->RemoveAurasDueToSpell(
                        IsHeroic() ? SPELL_HARVEST_SOULS_TELEPORT
            ...
            case EVENT_TELEPORT:
                ...
                if (TempSummon* summ = me->ToTempSummon())
                    if (Unit* summoner = summ->GetSummonerUnit()) {
                        if (summoner->IsAlive() &&
                            summoner->GetTypeId() == TYPEID_PLAYER) {
                            summoner->CastSpell((Unit*)nullptr,
                                                SPELL_HARVEST_SOUL_VISUAL,
                                                true);
                            summoner->ExitVehicle(summoner);
                            me->UpdatePosition(me->GetPositionX(),
                                               me->GetPositionY(),
                                               840.87f,
                                               me->GetOrientation(),
                                               true);
                ...
                if (Creature* lichKing = ObjectAccessor::GetCreature(
                    ...
                    if (me->GetExactDist(lichKing) > 8.0f) {
                        ...
                        path.push_back(G3D::Vector3(me->GetPositionX(),
                                                    ...
                                                    me->GetPositionZ()));
                        path.push_back(G3D::Vector3(
                            lichKing->GetPositionX() + dist * cos(angle),
                            lichKing->GetPositionY() + dist * std::sin(angle),
                            843.0f));
                        me->GetMotionMaster()->MoveSplinePath(&path);
                    }
                break;
...
public:
    ...
    struct npc_terenas_menethilAI : public ScriptedAI {
        npc_terenas_menethilAI(Creature* creature)
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
        }
                         ...
                         SpellSchoolMask) override
        {
            if (damage >= me->GetHealth()) {
                ...
                if (!me->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE)) {
                    ...
                    if (Creature* warden =
                            me->FindNearestCreature(NPC_SPIRIT_WARDEN, 20.0f)) {
                        warden->CastSpell(
                            (Unit*)nullptr, SPELL_DESTROY_SOUL, false);
...
public:
    spell_the_lich_king_lights_favor()
        : SpellScriptLoader("spell_the_lich_king_lights_favor")
    {
    }
    ...
    class spell_the_lich_king_lights_favor_AuraScript : public AuraScript {
        ...
        void CalculateBonus(AuraEffect const* /*aurEff*/,
                            int32& amount,
        ...
        void Register() override
        {
            ...
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_the_lich_king_lights_favor_AuraScript::CalculateBonus,
                EFFECT_1,
...
public:
    spell_the_lich_king_restore_soul()
        : SpellScriptLoader("spell_the_lich_king_restore_soul")
    {
    }
    ...
    class spell_the_lich_king_restore_soul_SpellScript : public SpellScript {
        ...
        {
            for (std::list<WorldObject*>::const_iterator itr = unitList.begin();
                 itr != unitList.end();
                 ++itr)
                if (Unit* target = (*itr)->ToUnit())
                    target->RemoveAurasDueToSpell(
                        target->GetMap()->IsHeroic()
                            ...
                            : SPELL_HARVEST_SOUL_TELEPORT);
            if (Creature* lichKing = ObjectAccessor::GetCreature(
                    *GetCaster(), _instance->GetGuidData(DATA_THE_LICH_KING)))
                lichKing->AI()->DoAction(ACTION_TELEPORT_BACK);
            if (Creature* spawner = GetCaster()->FindNearestCreature(
                    NPC_WORLD_TRIGGER_INFINITE_AOI, 50.0f, true)) {
                spawner->RemoveAllAuras();
                spawner->m_Events.KillAllEvents(true);
            }

            std::list<Creature*> spirits;
            GetCaster()->GetCreatureListWithEntryInGrid(
                spirits, NPC_WICKED_SPIRIT, 200.0f);
            for (std::list<Creature*>::iterator itr = spirits.begin();
                 itr != spirits.end();
                 ++itr) {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_the_lich_king_restore_soul_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
        }
...
public:
    ...
    struct npc_spirit_wardenAI : public ScriptedAI {
        npc_spirit_wardenAI(Creature* creature)
            : ScriptedAI(creature), _instance(creature->GetInstanceScript())
        {
        }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (Creature* terenas = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(DATA_TERENAS_MENETHIL)))
...
public:
    spell_the_lich_king_dark_hunger()
        : SpellScriptLoader("spell_the_lich_king_dark_hunger")
    {
    }
    ...
    class spell_the_lich_king_dark_hunger_AuraScript : public AuraScript {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            GetTarget()->CastCustomSpell(SPELL_DARK_HUNGER_HEAL,
                                         SPELLVALUE_BASE_POINT0,
                                         heal,
                                         GetTarget(),
                                         true,
        ...
        void Register() override
        {
            OnEffectProc += AuraEffectProcFn(
                spell_the_lich_king_dark_hunger_AuraScript::HandleProc,
                EFFECT_0,
                SPELL_AURA_DUMMY);
        }
    };
...
public:
    spell_the_lich_king_soul_rip()
        : SpellScriptLoader("spell_the_lich_king_soul_rip")
    {
    }
    ...
    class spell_the_lich_king_soul_rip_AuraScript : public AuraScript {
        ...
        void OnPeriodic(AuraEffect const* aurEff)
        {
            ...
            if (Unit* caster = GetCaster())
                caster->CastCustomSpell(SPELL_SOUL_RIP_DAMAGE,
                                        SPELLVALUE_BASE_POINT0,
                                        5000 * aurEff->GetTickNumber(),
                                        GetTarget(),
                                        true,
                                        nullptr,
                                        aurEff,
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_the_lich_king_soul_rip_AuraScript::OnPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_the_lich_king_summon_spirit_bomb()
        : SpellScriptLoader("spell_the_lich_king_summon_spirit_bomb")
    {
    }

    class spell_the_lich_king_summon_spirit_bomb_SpellScript
        : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            PreventHitDefaultEffect(effIndex);
            GetHitUnit()->CastSpell(
                (Unit*)nullptr, uint32(GetEffectValue()), true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_the_lich_king_summon_spirit_bomb_SpellScript::
                    HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
...
public:
    ...
    class spell_the_lich_king_trigger_vile_spirit_SpellScript
        : public SpellScript {
        ...
        {
            OnHit +=
                SpellHitFn(spell_the_lich_king_trigger_vile_spirit_SpellScript::
                               ActivateSpirit);
        }
    };
...
public:
    ...
    struct npc_lk_wicked_spiritAI : public ScriptedAI {
        ...
        void JustRespawned() override { me->SetReactState(REACT_PASSIVE); }

        bool CanAIAttack(Unit const* target) const override
        {
            return me->GetReactState() == REACT_AGGRESSIVE &&
                   target->GetTypeId() == TYPEID_PLAYER &&
                   target->GetExactDistSq(495.708f, -2523.76f, 1049.95f) <
                       40.0f * 40.0f;
        }
    };
...
public:
    achievement_been_waiting_long_time()
        : AchievementCriteriaScript("achievement_been_waiting_long_time")
    {
    }

    bool
    OnCheck(Player* /*source*/, Unit* target, uint32 /*criteria_id*/) override
    {
        if (!target)
            return false;
...
public:
    achievement_neck_deep_in_vile()
        : AchievementCriteriaScript("achievement_neck_deep_in_vile")
    {
    }
...
void AddSC_boss_the_lich_king()
{
    ...
    new spell_trigger_spell_from_caster("spell_the_lich_king_mass_resurrection",
