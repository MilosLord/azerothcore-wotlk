...
public:
    boss_lord_marrowgar(Creature* creature)
        : BossAI(creature, DATA_LORD_MARROWGAR)
    {
        _introDone = false;
        _boneSlice = false;
    }
    ...
    {
        if (target && (spell->Id == 69055 ||
                       spell->Id == 70814)) // Bone Slice (Saber Lash)
            for (uint8 i = 0; i < 3; ++i)
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_WARN_BONE_STORM:
            ...
            me->SetReactState(REACT_PASSIVE); // to prevent chasing another
                                              // target on UpdateVictim()
        ...
        case EVENT_BEGIN_BONE_STORM: {
            uint32 _boneStormDuration =
                RAID_MODE<uint32>(20000, 30000, 20000, 30000);
        ...
        case EVENT_BONE_STORM_MOVE: {
            ...
            events.Repeat(5s);
            Unit* unit = SelectTarget(
                SelectTargetMethod::Random, 0, BoneStormMoveTargetSelector(me));
            if (!unit) {
                if ((unit = SelectTarget(
                         SelectTargetMethod::MaxThreat, 0, 175.0f, true)))
            ...
            if (unit)
                me->GetMotionMaster()->MoveCharge(unit->GetPositionX(),
                                                  unit->GetPositionY(),
                                                  unit->GetPositionZ(),
                                                  25.0f,
                                                  1337);
        ...
        case EVENT_END_BONE_STORM:
            ...
            if (!IsHeroic())
                events.RescheduleEvent(
                    EVENT_SPELL_BONE_SPIKE_GRAVEYARD, 15s, 20s);
            break;
        case EVENT_ENRAGE:
        ...
        if (_boneSlice && me->isAttackReady() && me->GetVictim() &&
    ...
    {
        if (!_introDone && me->IsAlive() && who->GetTypeId() == TYPEID_PLAYER &&
            me->GetExactDist2dSq(who) <=
                10000.0f) // 100*100, moveinlineofsight limited to 60yd anyway
        {
            Talk(SAY_ENTER_ZONE);
            _introDone = true;
        }
    ...
    struct npc_coldflameAI : public NullCreatureAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1: {
                ...
                float nx =
                    me->GetPositionX() + 5.0f * cos(me->GetOrientation());
                float ny =
                    me->GetPositionY() + 5.0f * std::sin(me->GetOrientation());
...
public:
    ...
    struct npc_bone_spikeAI : public NullCreatureAI {
        npc_bone_spikeAI(Creature* creature)
            : NullCreatureAI(creature), hasTrappedUnit(false)
        {
        }
        ...
        void DoAction(int32 action) override
        {
            ...
            if (TempSummon* summ = me->ToTempSummon())
                if (Unit* trapped = summ->GetSummonerUnit()) {
                    Position exitPos = {me->GetPositionX(),
                                        me->GetPositionY(),
                                        60.0f,
                                        me->GetOrientation()};
                    trapped->UpdateAllowedPositionZ(exitPos.GetPositionX(),
                                                    exitPos.GetPositionY(),
                                                    exitPos.m_positionZ);
                    exitPos.m_positionZ += 1.0f;
                    if (Unit* vehBase = trapped->GetVehicleBase())
                        vehBase->RemoveAurasByType(SPELL_AURA_CONTROL_VEHICLE,
                                                   trapped->GetGUID());
                    ...
                    trapped->NearTeleportTo(exitPos.GetPositionX(),
                                            exitPos.GetPositionY(),
                                            exitPos.GetPositionZ(),
                                            exitPos.GetOrientation(),
                                            false);
                }
        ...
        void JustDied(Unit* /*killer*/) override { DoAction(-1337); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (TempSummon* summ = me->ToTempSummon()) {
                if (Unit* trapped = summ->GetSummonerUnit()) {
                    if (!trapped->IsOnVehicle(me) || !trapped->IsAlive() ||
                        !me->GetInstanceScript() ||
                        me->GetInstanceScript()->GetBossState(
                            DATA_LORD_MARROWGAR) != IN_PROGRESS ||
                        trapped->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION)) {
                        DoAction(-1337);
                        return;
                    }
                }
...
public:
    ...
    class spell_marrowgar_coldflame_SpellScript : public SpellScript {
        ...
        {
            targets.clear();
            Unit* target = GetCaster()->GetAI()->SelectTarget(
                SelectTargetMethod::Random,
                1,
                -1.0f,
                true,
                true,
                -SPELL_IMPALED); // -1.0f as it takes into account object size
            if (!target)
                target = GetCaster()->GetAI()->SelectTarget(
                    SelectTargetMethod::Random,
                    0,
                    0.0f,
                    true); // if only tank or noone outside of boss' model
            if (!target)
                return;
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_marrowgar_coldflame_SpellScript::SelectTarget,
                EFFECT_0,
                TARGET_UNIT_DEST_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(
                spell_marrowgar_coldflame_SpellScript::HandleScriptEffect,
                EFFECT_0,
...
public:
    spell_marrowgar_bone_spike_graveyard()
        : SpellScriptLoader("spell_marrowgar_bone_spike_graveyard")
    {
    }

    class spell_marrowgar_bone_spike_graveyard_SpellScript
        : public SpellScript {
        ...
        void HandleSpikes(SpellEffIndex effIndex)
        {
            ...
            if (Creature* marrowgar = GetCaster()->ToCreature()) {
                ...
                uint8       boneSpikeCount =
                    uint8(GetCaster()->GetMap()->GetSpawnMode() & 1 ? 3 : 1);

                std::vector<Player*>   validPlayers;
                Map::PlayerList const& pList =
                    marrowgar->GetMap()->GetPlayers();
                for (Map::PlayerList::const_iterator itr = pList.begin();
                     itr != pList.end();
                     ++itr)
                    if (Player* plr = itr->GetSource())
                        if (plr->IsAlive() && !plr->IsGameMaster() &&
                            plr->GetExactDist2dSq(marrowgar) <
                                (150.0f * 150.0f) &&
                            !plr->HasAura(SPELL_IMPALED))
                            if (!marrowgar->GetVictim() ||
                                marrowgar->GetVictim()->GetGUID() !=
                                    plr->GetGUID())
                                if (plr->GetGUID() != marrowgarAI->GetGUID(0) &&
                                    plr->GetGUID() != marrowgarAI->GetGUID(1) &&
                                    plr->GetGUID() !=
                                        marrowgarAI->GetGUID(
                                            2)) // not a bone slice target
                ...
                for (uint8 i = 0; i < boneSpikeCount && i < validPlayers.size();
                     ++i) {
                    Unit* target = validPlayers[i];
                    didHit       = true;
                    // target->CastCustomSpell(boneSpikeSummonId[i],
                    // SPELLVALUE_BASE_POINT0, 0, target, true);
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_marrowgar_bone_spike_graveyard_SpellScript::HandleSpikes,
                EFFECT_1,
...
public:
    spell_marrowgar_coldflame_bonestorm()
        : SpellScriptLoader("spell_marrowgar_coldflame_bonestorm")
    {
    }
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            for (uint8 i = 0; i < 4; ++i) {
                ...
                if (caster->IsWithinLOS(nx, ny, z)) {
                    ...
                    caster->CastSpell(
                        nx, ny, z, uint32(GetEffectValue() + i), true);
                }
            }
        }
...
public:
    spell_marrowgar_bone_storm()
        : SpellScriptLoader("spell_marrowgar_bone_storm")
    {
    }
    ...
    class spell_marrowgar_bone_storm_SpellScript : public SpellScript {
        ...
        {
            OnHit += SpellHitFn(
                spell_marrowgar_bone_storm_SpellScript::RecalculateDamage);
        }
    };
...
public:
    spell_marrowgar_bone_slice()
        : SpellScriptLoader("spell_marrowgar_bone_slice")
    {
    }
    ...
    class spell_marrowgar_bone_slice_SpellScript : public SpellScript {
        ...
        {
            _targetCount = std::min<uint32>(targets.size(),
                                            GetSpellInfo()->MaxAffectedTargets);
        }
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_marrowgar_bone_slice_SpellScript::CountTargets,
                EFFECT_0,
                TARGET_UNIT_DEST_AREA_ENEMY);
            OnHit +=
                SpellHitFn(spell_marrowgar_bone_slice_SpellScript::SplitDamage);
        }
