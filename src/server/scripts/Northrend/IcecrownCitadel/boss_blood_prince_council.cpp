...
public:
    ...
    struct boss_prince_kelesethAI : public ScriptedAI {
        ...
        void InitializeAI() override
        {
            ...
            if (me->IsAlive()) {
                if (Creature* c = me->SummonCreature(WORLD_TRIGGER,
                                                     me->GetHomePosition())) {
        ...
        {
            bool valid = true;
            if (Creature* keleseth = instance->instance->GetCreature(
                    instance->GetGuidData(DATA_PRINCE_KELESETH_GUID)))
                if (!keleseth->IsAlive() || keleseth->IsInEvadeMode())
                    valid = false;
            if (Creature* taldaram = instance->instance->GetCreature(
                    instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID)))
                if (!taldaram->IsAlive() || taldaram->IsInEvadeMode())
                    valid = false;
            if (Creature* valanar = instance->instance->GetCreature(
                    instance->GetGuidData(DATA_PRINCE_VALANAR_GUID)))
            ...
            if (Creature* taldaram = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID)))
                if (!taldaram->IsInEvadeMode())
                    taldaram->SetInCombatWithZone();
            if (Creature* valanar = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_PRINCE_VALANAR_GUID)))
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_STAND_UP:
                ...
                me->SetReactState(REACT_AGGRESSIVE);
                me->ForceValuesUpdateAtIndex(
                    UNIT_NPC_FLAGS); // was in sniff. don't ask why
                me->m_Events.AddEvent(new StandUpEvent(*me),
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            DoSpellAttackIfReady(_isEmpowered ? SPELL_EMPOWERED_SHADOW_LANCE
                                              : SPELL_SHADOW_LANCE);
        }
...
public:
    ...
    struct boss_prince_taldaramAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            if (Creature* keleseth = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(DATA_PRINCE_KELESETH_GUID)))
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            Unit* target =
                SelectTarget(SelectTargetMethod::Random, 0, -10.0f, true);
            if (!target)
                target =
                    SelectTarget(SelectTargetMethod::Random, 0, 0.0f, true);
            if (target) {
...
public:
    ...
    struct boss_prince_valanarAI : public BossAI {
        boss_prince_valanarAI(Creature* creature)
            : BossAI(creature, DATA_BLOOD_PRINCE_COUNCIL)
        {
            if (!instance) {
                me->IsAIEnabled = false;
                me->AddUnitState(UNIT_STATE_EVADE);
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            invocationOrder[1] =
                RAND(DATA_PRINCE_KELESETH_GUID, DATA_PRINCE_TALDARAM_GUID);
            invocationOrder[2] = DATA_PRINCE_KELESETH_GUID +
                                 DATA_PRINCE_TALDARAM_GUID - invocationOrder[1];
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            case NPC_SHOCK_VORTEX:
                summon->m_Events.AddEvent(new ShockVortexExplodeEvent(*summon),
                                          summon->m_Events.CalculateTime(4500));
                break;
            default:
                break;
            }
        ...
        bool CheckRoom()
        {
            Creature* keleseth = instance->instance->GetCreature(
                instance->GetGuidData(DATA_PRINCE_KELESETH_GUID));
            Creature* taldaram = instance->instance->GetCreature(
                instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID));
            if (keleseth && taldaram && IsInBoundary(me) &&
                IsInBoundary(keleseth) && IsInBoundary(taldaram))
                return true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INVOCATION_OF_BLOOD: {
                ...
                Creature* current =
                    instance->instance->GetCreature(instance->GetGuidData(
                        invocationOrder[currentInvocationIndex]));
                ...
                if (!visualSpellId || !current || !next ||
                    !current->IsInCombat() || !next->IsInCombat()) {
                    EnterEvadeMode(EVADE_REASON_OTHER);
                    return;
                }
            ...
            case EVENT_KINETIC_BOMB:
                ...
                events.ScheduleEvent(EVENT_KINETIC_BOMB,
                                     me->GetMap()->Is25ManRaid() ? 20s + 500ms
                                                                 : 30s + 500ms);
                break;
...
public:
    ...
    struct npc_blood_queen_lana_thelAI : public ScriptedAI {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (Creature* summon = DoSummon(NPC_FLOATING_TRIGGER,
                                            triggerPos,
                                            15000,
                                            TEMPSUMMON_TIMED_DESPAWN)) {
                summon->SetDisplayId(
                    11686); // it's a general npc, and the template has 2 models
                            // (first is an infernal)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_events.ExecuteEvent() == EVENT_INTRO_1) {
                Talk(SAY_INTRO_2);
                me->GetMotionMaster()->MovePoint(POINT_INTRO_DESPAWN,
                                                 introFinalPos);
                _events.Reset();

                if (Creature* keleseth = ObjectAccessor::GetCreature(
                        *me,
                        _instance->GetGuidData(DATA_PRINCE_KELESETH_GUID))) {
                    keleseth->AI()->DoAction(ACTION_STAND_UP);
                }
                if (Creature* taldaram = ObjectAccessor::GetCreature(
                        *me,
                        _instance->GetGuidData(DATA_PRINCE_TALDARAM_GUID))) {
                    taldaram->AI()->DoAction(ACTION_STAND_UP);
                }
                if (Creature* valanar = ObjectAccessor::GetCreature(
                        *me,
                        _instance->GetGuidData(DATA_PRINCE_VALANAR_GUID))) {
            ...
            if (timer <= diff) {
                ...
                if (Unit* victim = me->GetVictim()) {
                    if (me->GetDistance(victim) < 15.0f &&
                        !victim->HasAura(SPELL_SHADOW_RESONANCE_RESIST,
                                         me->GetGUID())) {
                        me->InterruptNonMeleeSpells(true, 0, true);
                        me->CastSpell(
                            victim, SPELL_SHADOW_RESONANCE_RESIST, false);
                        me->ClearUnitState(UNIT_STATE_CASTING);
                    }
                }
            }
...
public:
    ...
    struct npc_ball_of_flameAI : public ScriptedAI {
        ...
        void DoAction(int32 action) override
        {
            ...
            if (!target) {
                target =
                    ScriptedAI::SelectTargetFromPlayerList(150.0f, 0, true);
            }
            if (target) {
                // need to clear states now because this call is before
                // AuraEffect is fully removed
                _started = true;
                if (me->GetEntry() == NPC_BALL_OF_INFERNO_FLAME)
    ...
    struct npc_kinetic_bombAI : public NullCreatureAI {
        ...
        void Reset() override
        {
            ...
            me->GetMotionMaster()->MoveCharge(
                _x, _y, _groundZ, me->GetSpeed(MOVE_WALK));
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == SPELL_KINETIC_BOMB_EXPLOSION) {
                ...
                if (!me->HasAura(SPELL_KINETIC_BOMB_KNOCKBACK)) {
                    ...
                    me->StopMoving();
                    me->GetMotionMaster()->MoveCharge(_x,
                                                      _y,
                                                      me->GetPositionZ() +
                                                          60.0f,
...
public:
    spell_blood_council_shadow_prison()
        : SpellScriptLoader("spell_blood_council_shadow_prison")
    {
    }
    ...
    class spell_blood_council_shadow_prison_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_blood_council_shadow_prison_AuraScript::HandleDummyTick,
                EFFECT_0,
...
public:
    spell_blood_council_shadow_prison_damage()
        : SpellScriptLoader("spell_blood_council_shadow_prison_damage")
    {
    }
    ...
    class spell_blood_council_shadow_prison_SpellScript : public SpellScript {
        ...
        {
            OnHit += SpellHitFn(
                spell_blood_council_shadow_prison_SpellScript::AddExtraDamage);
        }
    };
...
public:
    spell_taldaram_glittering_sparks()
        : SpellScriptLoader("spell_taldaram_glittering_sparks")
    {
    }
    ...
    class spell_taldaram_glittering_sparks_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_taldaram_glittering_sparks_SpellScript::HandleScript,
                EFFECT_0,
...
public:
    ...
    class spell_taldaram_summon_flame_ball_SpellScript : public SpellScript {
        ...
        bool Load() override
        {
            ...
            GetCaster()->CastSpell(
                GetCaster(),
                uint32(GetSpellInfo()->Effects[0].CalcValue()),
                true);
            return true;
        }
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_taldaram_summon_flame_ball_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_taldaram_ball_of_inferno_flame()
        : SpellScriptLoader("spell_taldaram_ball_of_inferno_flame")
    {
    }

    class spell_taldaram_ball_of_inferno_flame_SpellScript
        : public SpellScript {
        ...
        {
            AfterHit += SpellHitFn(
                spell_taldaram_ball_of_inferno_flame_SpellScript::ModAuraStack);
        }
    };
...
public:
    spell_valanar_kinetic_bomb()
        : SpellScriptLoader("spell_valanar_kinetic_bomb")
    {
    }
    ...
    class spell_valanar_kinetic_bomb_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_valanar_kinetic_bomb_SpellScript::ChangeSummonPos,
                EFFECT_0,
                SPELL_EFFECT_SUMMON);
        }
    };
    ...
    class spell_valanar_kinetic_bomb_AuraScript : public AuraScript {
        ...
        void HandleDummyTick(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Creature* bomb =
                    target->FindNearestCreature(NPC_KINETIC_BOMB, 1.0f, true)) {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_valanar_kinetic_bomb_AuraScript::HandleDummyTick,
                EFFECT_1,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_valanar_kinetic_bomb_absorb()
        : SpellScriptLoader("spell_valanar_kinetic_bomb_absorb")
    {
    }
    ...
    class spell_valanar_kinetic_bomb_absorb_AuraScript : public AuraScript {
        ...
        void
        OnAbsorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
        {
            absorbAmount =
                CalculatePct(dmgInfo.GetDamage(), aurEff->GetAmount());
        ...
        void Register() override
        {
            OnEffectAbsorb += AuraEffectAbsorbFn(
                spell_valanar_kinetic_bomb_absorb_AuraScript::OnAbsorb,
                EFFECT_0);
        }
    };
...
public:
    spell_valanar_kinetic_bomb_knockback()
        : SpellScriptLoader("spell_valanar_kinetic_bomb_knockback")
    {
    }
    ...
    class spell_valanar_kinetic_bomb_knockback_SpellScript
        ...
        void Register() override
        {
            BeforeHit += BeforeSpellHitFn(
                spell_valanar_kinetic_bomb_knockback_SpellScript::KnockIntoAir);
        }
    };
...
public:
    spell_valanar_kinetic_bomb_summon()
        : SpellScriptLoader("spell_valanar_kinetic_bomb_summon")
    {
    }
    ...
    class spell_valanar_kinetic_bomb_summon_SpellScript : public SpellScript {
        ...
        void SelectDest()
        {
            if (Position* dest =
                ...
                Position offset = {
                    6.0f * cos(angle), 6.0f * std::sin(angle), 10.0f, 0.0f};
                dest->RelocateOffset(offset);
                GetCaster()->UpdateAllowedPositionZ(dest->GetPositionX(),
                                                    dest->GetPositionY(),
                                                    dest->m_positionZ);
            }
        }
        ...
        void Register() override
        {
            BeforeCast += SpellCastFn(
                spell_valanar_kinetic_bomb_summon_SpellScript::SelectDest);
        }
    };
...
public:
    spell_blood_council_summon_shadow_resonance()
        : SpellScriptLoader("spell_blood_council_summon_shadow_resonance")
    {
    }

    class spell_blood_council_summon_shadow_resonance_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void SetDest(SpellDestination& dest)
        {
            ...
            if (dest._position.GetExactDist2d(summoner) > 35.0f &&
                x > 4585.0f && y > 2716.0f && y < 2822.0f)
                return;

            for (uint8 a = 0; a < 2; ++a)
                for (uint8 i = 6; i > 0; --i) {
                    float destX = summoner->GetPositionX() +
                                  cos(angle + a * M_PI) * i * 10.0f;
                    float destY = summoner->GetPositionY() +
                                  std::sin(angle + a * M_PI) * i * 10.0f;
                    if (summoner->GetMap()->isInLineOfSight(
                        ...
                        float destZ =
                            summoner->GetMapHeight(summoner->GetPhaseMask(),
                                                   destX,
                                                   destY,
                                                   summoner->GetPositionZ());
                        if (std::fabs(destZ - summoner->GetPositionZ()) <
                            10.0f) // valid z found
                        {
            ...
            dest._position.Relocate(summoner->GetPositionX(),
                                    summoner->GetPositionY(),
                                    summoner->GetPositionZ());
        }
