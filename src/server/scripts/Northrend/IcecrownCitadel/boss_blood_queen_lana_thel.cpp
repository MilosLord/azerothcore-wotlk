...
public:
    ...
    struct boss_blood_queen_lana_thelAI : public BossAI {
        boss_blood_queen_lana_thelAI(Creature* creature)
            : BossAI(creature, DATA_BLOOD_QUEEN_LANA_THEL)
        {
        ...
        {
            if (!instance->CheckRequiredBosses(DATA_BLOOD_QUEEN_LANA_THEL,
                                               who->ToPlayer()) ||
                !me->IsVisible()) {
            ...
            _creditBloodQuickening =
                instance->GetData(DATA_BLOOD_QUICKENING_STATE) == IN_PROGRESS;
        }
            ...
            if (_creditBloodQuickening) {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                    ...
                    if (Player* p = itr->GetSource())
                        p->KilledMonsterCredit(
                            RAID_MODE(NPC_INFILTRATOR_MINCHAR_BQ,
                                      NPC_BLOOD_QUICKENING_CREDIT_25));
                if (Creature* minchar = me->FindNearestCreature(
                        NPC_INFILTRATOR_MINCHAR_BQ, 200.0f)) {
                    minchar->SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);
                    minchar->SetCanFly(false);
                    minchar->SetDisableGravity(false);
                    minchar->RemoveAllAuras();
                    minchar->GetMotionMaster()->MoveCharge(
                        4629.3711f, 2782.6089f, 401.5301f, SPEED_CHARGE / 3.0f);
                }
            }
        }
        ...
        void DoAction(int32 action) override
        {
            ...
            if (instance->GetBossState(DATA_BLOOD_QUEEN_LANA_THEL) ==
                IN_PROGRESS)
                _killMinchar = true;
            else
                GoToMinchar();
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            case POINT_CENTER:
                ...
                events.ScheduleEvent(EVENT_AIR_PHASE,
                                     Is25ManRaid() ? 100s : 120s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_VAMPIRIC_BITE: {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                     ...
                     ++itr)
                    if (Player* p = itr->GetSource())
                        if (p->IsAlive() && p != me->GetVictim() &&
                            p->GetGUID() != _offtankGUID &&
                            !p->IsGameMaster() && p->GetDistance(me) < 70.0f) {
                            float th =
                                me->GetThreatMgr().getThreatWithoutTemp(p);
                ...
                if (target) {
                    ...
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_VAMPIRIC_BITE_DUMMY, true);
            ...
            case EVENT_BLOOD_MIRROR:
                if (me->GetVictim()) {
                    ...
                    if (!myList.empty()) {
                        myList.sort(
                            Acore::ObjectDistanceOrderPred(me->GetVictim()));
                        Player* target = myList.front();
                        if (me->GetVictim()->GetGUID() != _tankGUID ||
                            target->GetGUID() != _offtankGUID) {
                            // remove manually from previous, single target flag
                            // has nothing to do with this as caster is in every
                            // case different.
                            if (_tankGUID)
                                if (Player* prevTank =
                                        ObjectAccessor::GetPlayer(*me,
                                                                  _tankGUID)) {
                                    prevTank->RemoveAurasDueToSpell(
                                        SPELL_BLOOD_MIRROR_DAMAGE);
                                    prevTank->RemoveAurasDueToSpell(
                                        SPELL_BLOOD_MIRROR_VISUAL);
                                }
                            if (_offtankGUID)
                                if (Player* prevOfftank =
                                        ObjectAccessor::GetPlayer(*me,
                                                                  _offtankGUID))
                                    prevOfftank->RemoveAurasDueToSpell(
                            ...
                            if (target->GetDistance(me->GetVictim()) > 39.0f ||
                                ...
                                _tankGUID.Clear();
                                _offtankGUID.Clear();
                                events.ScheduleEvent(EVENT_BLOOD_MIRROR,
                                                     2500ms);
                                break;
                            }
                            ...
                            target->CastSpell(me->GetVictim(),
                                              SPELL_BLOOD_MIRROR_DAMAGE,
                                              true);
                            me->GetVictim()->CastSpell(
                                target, SPELL_BLOOD_MIRROR_DUMMY, true);
                            me->CastSpell(me->GetVictim(),
                                          SPELL_BLOOD_MIRROR_VISUAL,
                                          false);

                            if (Is25ManRaid() &&
                                target->GetQuestStatus(QUEST_BLOOD_INFUSION) ==
                                    QUEST_STATUS_INCOMPLETE &&
                                target->HasAura(SPELL_UNSATED_CRAVING) &&
                                !target->HasAura(SPELL_THIRST_QUENCHED) &&
            ...
            case EVENT_DELIRIOUS_SLASH:
                if (!me->HasReactState(REACT_PASSIVE)) {
                    ...
                    if (_offtankGUID)
                        if (Unit* t =
                                ObjectAccessor::GetUnit(*me, _offtankGUID))
                            if (t->IsAlive() && t->GetDistance(me) < 10.0f)
                                target = t;
                    if (!target)
                        if (me->GetVictim() &&
                            me->GetVictim()->GetDistance(me) < 10.0f)
                            target = me->GetVictim();
                    if (!target) {
                    ...
                    for (Map::PlayerList::const_iterator itr = pl.begin();
                        ...
                        if (Player* p = itr->GetSource())
                            if (p->IsAlive() && p != me->GetVictim() &&
                                p->GetGUID() != _offtankGUID &&
                                !p->IsGameMaster() &&
                                p->GetDistance(me) < 100.0f &&
                                !p->HasAura(SPELL_UNCONTROLLABLE_FRENZY))
                                myList.push_back(p);
                    Acore::Containers::RandomResize(myList,
                                                    Is25ManRaid() ? 3 : 2);
                    if (myList.size() > 1) {
                        Talk(SAY_PACT_OF_THE_DARKFALLEN);
                        for (std::list<Player*>::iterator itr = myList.begin();
                             itr != myList.end();
                             ++itr)
                            me->CastSpell(
                                *itr, SPELL_PACT_OF_THE_DARKFALLEN, false);
                    ...
                    for (Map::PlayerList::const_iterator itr = pl.begin();
                        ...
                        if (Player* p = itr->GetSource())
                            if (p->IsAlive() && p != me->GetVictim() &&
                                p->GetGUID() != _offtankGUID &&
                                !p->IsGameMaster() &&
                                !p->HasAura(SPELL_PACT_OF_THE_DARKFALLEN) &&
                                !p->HasAura(SPELL_UNCONTROLLABLE_FRENZY))
                                myList.push_back(p);
            ...
            case EVENT_AIR_START_FLYING:
                ...
                me->GetMotionMaster()->MoveTakeoff(
                    POINT_AIR, airPos, 0.642857f * 7.0f);
                break;
            case EVENT_AIR_FLY_DOWN:
                me->GetMotionMaster()->MoveLand(
                    POINT_GROUND, centerPos, 0.642857f * 7.0f);
                break;
            }
        ...
        bool WasVampire(ObjectGuid guid) { return _vampires.count(guid) != 0; }
...
public:
    spell_blood_queen_pact_of_the_darkfallen_dmg()
        : SpellScriptLoader("spell_blood_queen_pact_of_the_darkfallen_dmg")
    {
    }

    class spell_blood_queen_pact_of_the_darkfallen_dmg_AuraScript
        : public AuraScript {
        PrepareAuraScript(
        ...
        void PeriodicTick(AuraEffect const* aurEff)
        {
            ...
            SpellInfo const* damageSpell =
                sSpellMgr->AssertSpellInfo(SPELL_PACT_OF_THE_DARKFALLEN_DAMAGE);
            int32 damage = damageSpell->Effects[EFFECT_0].CalcValue();
            float herobonus =
                ((GetTarget()->FindMap() && GetTarget()->FindMap()->IsHeroic())
                     ? 0.2f
                     : 0.0f);
            float multiplier =
                0.5f + herobonus +
                0.1f *
                    uint32(
                        ...
                        10); // do not convert to 0.01f - we need tick number/10
                             // as INT (damage increases every 10 ticks)
            damage = int32(damage * multiplier);
            GetTarget()->CastCustomSpell(SPELL_PACT_OF_THE_DARKFALLEN_DAMAGE,
...
public:
    spell_blood_queen_pact_of_the_darkfallen()
        : SpellScriptLoader("spell_blood_queen_pact_of_the_darkfallen")
    {
    }

    class spell_blood_queen_pact_of_the_darkfallen_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_blood_queen_pact_of_the_darkfallen_SpellScript);

        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if(
                Acore::UnitAuraCheck(false, SPELL_PACT_OF_THE_DARKFALLEN));

            bool                                    remove = true;
            std::list<WorldObject*>::const_iterator itr, itr2,
                itrEnd = targets.end();
...
public:
    ...
    class spell_blood_queen_pact_of_the_darkfallen_dmg_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        {
            unitList.remove_if(
                Acore::UnitAuraCheck(true, SPELL_PACT_OF_THE_DARKFALLEN));
...
public:
    spell_blood_queen_bloodbolt()
        : SpellScriptLoader("spell_blood_queen_bloodbolt")
    {
    }
    ...
    class spell_blood_queen_bloodbolt_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(BloodboltHitCheck(
                static_cast<LanaThelAI*>(GetCaster()->GetAI())));
            Acore::Containers::RandomResize(targets, targetCount);
            // mark targets now, effect hook has missile travel time delay
            // (might cast next in that time)
            for (std::list<WorldObject*>::const_iterator itr = targets.begin();
                 itr != targets.end();
                 ++itr)
                GetCaster()->GetAI()->SetGUID((*itr)->GetGUID(),
                                              GUID_BLOODBOLT);
        }
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            GetCaster()->CastSpell(
                GetHitUnit(), SPELL_TWILIGHT_BLOODBOLT_FROM_WHIRL, true);
        }
        ...
        void Register() override
        {
            ...
            OnEffectHitTarget += SpellEffectFn(
                spell_blood_queen_bloodbolt_SpellScript::HandleScript,
                EFFECT_1,
...
public:
    spell_blood_queen_frenzied_bloodthirst()
        : SpellScriptLoader("spell_blood_queen_frenzied_bloodthirst")
    {
    }
    ...
    class spell_blood_queen_frenzied_bloodthirst_AuraScript
                     ...
                     AuraEffectHandleModes /*mode*/)
        {
            if (InstanceScript* instance = GetTarget()->GetInstanceScript())
                if (Creature* bloodQueen = ObjectAccessor::GetCreature(
                        *GetTarget(),
                ...
                if (InstanceScript* instance = target->GetInstanceScript())
                    if (Creature* bloodQueen = ObjectAccessor::GetCreature(
                        ...
                        if (bloodQueen->IsAlive() && bloodQueen->IsInCombat()) {
                            // this needs to be done BEFORE charm aura or we hit
                            // an assert in Unit::SetCharmedBy
                            if (target->GetVehicleKit())
                                target->RemoveVehicleKit();

                            bloodQueen->AI()->Talk(SAY_MIND_CONTROL);
                            bloodQueen->CastSpell(
                                target, SPELL_UNCONTROLLABLE_FRENZY, true);
                        }
        }
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_blood_queen_frenzied_bloodthirst_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_OVERRIDE_SPELLS,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_blood_queen_frenzied_bloodthirst_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_OVERRIDE_SPELLS,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_blood_queen_essence_of_the_blood_queen()
        : SpellScriptLoader("spell_blood_queen_essence_of_the_blood_queen")
    {
    }

    class spell_blood_queen_essence_of_the_blood_queen_AuraScript
        : public AuraScript {
        PrepareAuraScript(
        ...
        void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            int32 heal =
                CalculatePct(static_cast<int32>(damageInfo->GetDamage()),
                             aurEff->GetAmount());
        ...
        void Register() override
        {
            OnEffectProc += AuraEffectProcFn(
                spell_blood_queen_essence_of_the_blood_queen_AuraScript::OnProc,
                EFFECT_1,
                SPELL_AURA_DUMMY);
        }
    };
...
public:
    spell_blood_queen_vampiric_bite()
        : SpellScriptLoader("spell_blood_queen_vampiric_bite")
    {
    }
    ...
    class spell_blood_queen_vampiric_bite_SpellScript : public SpellScript {
        ...
        bool Validate(SpellInfo const* /*spell*/) override
        {
            return ValidateSpellInfo({SPELL_ESSENCE_OF_THE_BLOOD_QUEEN_PLR,
        ...
        SpellCastResult CheckTarget()
        {
            ...
            if (InstanceScript* instance =
                    GetExplTargetUnit()->GetInstanceScript())
                if (instance->GetBossState(DATA_BLOOD_QUEEN_LANA_THEL) ==
                    IN_PROGRESS)
        ...
        void OnCast(SpellMissInfo missInfo)
        {
            ...
            if (GetCaster()->GetTypeId() != TYPEID_PLAYER ||
                GetCaster()->GetMapId() != 631)
                return;
            InstanceScript* instance = GetCaster()->GetInstanceScript();
            if (!instance || instance->GetBossState(
                                 DATA_BLOOD_QUEEN_LANA_THEL) != IN_PROGRESS)
                return;

            uint32 spellId = sSpellMgr->GetSpellIdForDifficulty(
                SPELL_FRENZIED_BLOODTHIRST, GetCaster());
            GetCaster()->RemoveAura(
                spellId, ObjectGuid::Empty, 0, AURA_REMOVE_BY_ENEMY_SPELL);
            ...
            if (Aura* aura = GetCaster()->GetAura(SPELL_GUSHING_WOUND)) {
                if (aura->GetStackAmount() == 3) {
                    ...
                    GetCaster()->CastSpell(
                        GetCaster(), SPELL_GUSHING_WOUND, TRIGGERED_FULL_MASK);
            }
            ...
            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                if (Creature* bloodQueen = ObjectAccessor::GetCreature(
                        *GetCaster(),
                        instance->GetGuidData(DATA_BLOOD_QUEEN_LANA_THEL)))
                    bloodQueen->AI()->SetGUID(GetHitUnit()->GetGUID(),
                                              GUID_VAMPIRE);
        }
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_blood_queen_vampiric_bite_SpellScript::CheckTarget);
            BeforeHit += BeforeSpellHitFn(
                spell_blood_queen_vampiric_bite_SpellScript::OnCast);
            OnEffectHitTarget += SpellEffectFn(
                spell_blood_queen_vampiric_bite_SpellScript::HandlePresence,
                EFFECT_1,
...
public:
    spell_blood_queen_swarming_shadows_floor_dmg()
        : SpellScriptLoader("spell_blood_queen_swarming_shadows_floor_dmg")
    {
    }

    class spell_blood_queen_swarming_shadows_floor_dmg_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if(Acore::AllWorldObjectsInExactRange(
                GetCaster(), GetSpellInfo()->Effects[0].CalcRadius(), true));
        }
...
public:
    achievement_once_bitten_twice_shy(const char* name,
                                      uint8       spawnMode,
                                      bool        wasVampire)
        : AchievementCriteriaScript(name), _spawnMode(spawnMode),
...
void AddSC_boss_blood_queen_lana_thel()
{
    ...
    new achievement_once_bitten_twice_shy(
        "achievement_once_bitten_twice_shy_n_10", 0, false);
    new achievement_once_bitten_twice_shy(
        "achievement_once_bitten_twice_shy_v_10", 0, true);
    new achievement_once_bitten_twice_shy(
        "achievement_once_bitten_twice_shy_n_25", 1, false);
    new achievement_once_bitten_twice_shy(
        "achievement_once_bitten_twice_shy_v_25", 1, true);
}
