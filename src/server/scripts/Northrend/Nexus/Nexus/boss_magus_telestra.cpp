...
public:
    ...
    struct boss_magus_telestraAI : public BossAI {
        boss_magus_telestraAI(Creature* creature)
        ...
        void Reset() override
        {
            ...
            if (IsHeroic() &&
                sGameEventMgr->IsActiveEvent(GAME_EVENT_WINTER_VEIL) &&
        ...
        {
            if (spellInfo->Id >= SPELL_FIRE_MAGUS_DEATH &&
                spellInfo->Id <= SPELL_ARCANE_MAGUS_DEATH &&
                caster->ToCreature()) {
...
class spell_boss_magus_telestra_summon_telestra_clones
    : public SpellScriptLoader {
public:
    spell_boss_magus_telestra_summon_telestra_clones()
        : SpellScriptLoader("spell_boss_magus_telestra_summon_telestra_clones")
    {
    }

    class spell_boss_magus_telestra_summon_telestra_clones_AuraScript
        : public AuraScript {
        PrepareAuraScript(
                         ...
                         AuraEffectHandleModes /*mode*/)
        {
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_FIRE_MAGUS_SUMMON, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_FROST_MAGUS_SUMMON, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_ARCANE_MAGUS_SUMMON, true);
...
public:
    spell_boss_magus_telestra_gravity_well()
        : SpellScriptLoader("spell_boss_magus_telestra_gravity_well")
    {
    }

    class spell_boss_magus_telestra_gravity_well_SpellScript
        : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_boss_magus_telestra_gravity_well_SpellScript::
                    SelectTarget,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(
                spell_boss_magus_telestra_gravity_well_SpellScript::HandlePull,
                EFFECT_0,
...
public:
    achievement_split_personality()
        : AchievementCriteriaScript("achievement_split_personality")
    {
    }
