...
public:
    ...
    struct boss_keristraszaAI : public BossAI {
        boss_keristraszaAI(Creature* creature)
            : BossAI(creature, DATA_KERISTRASZA_EVENT)
        {
        }
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_CRYSTAL_CHAINS,
                                 DUNGEON_MODE(20000, 11000));
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ACHIEVEMENT_CHECK: {
                ...
                for (Map::PlayerList::const_iterator itr = pList.begin();
                     itr != pList.end();
                     ++itr)
                    if (Aura* aur = itr->GetSource()->GetAura(
                            SPELL_INTENSE_COLD_TRIGGER))
                        if (aur->GetStackAmount() > 2)
                            aGuids.insert(
                                itr->GetSource()->GetGUID().GetCounter());
...
public:
    achievement_intense_cold()
        : AchievementCriteriaScript("achievement_intense_cold")
    {
    }
