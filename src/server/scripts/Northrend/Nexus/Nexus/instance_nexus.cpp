    ...
    struct instance_nexus_InstanceMapScript : public InstanceScript {
        ...
        void OnGameObjectCreate(GameObject* gameObject) override
        {
            ...
            case GO_TELESTRA_SPHERE:
                if (GetBossState(DATA_TELESTRA_ORB) != DONE &&
                    GetBossState(DATA_MAGUS_TELESTRA_EVENT) == DONE)
                    gameObject->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                AddDoor(gameObject);
                break;
            case GO_ANOMALUS_SPHERE:
                if (GetBossState(DATA_ANOMALUS_ORB) != DONE &&
                    GetBossState(DATA_ANOMALUS_EVENT) == DONE)
                    gameObject->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                AddDoor(gameObject);
                break;
            case GO_ORMOROK_SPHERE:
                if (GetBossState(DATA_ORMOROK_ORB) != DONE &&
                    GetBossState(DATA_ORMOROK_EVENT) == DONE)
                    gameObject->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                AddDoor(gameObject);
                break;
            }
        ...
        bool SetBossState(uint32 id, EncounterState state) override
        {
            ...
            for (DoorSet::const_iterator i =
                     bossInfo->door[DOOR_TYPE_PASSAGE].begin();
...
public:
    ...
    struct npc_crystalline_frayerAI : public ScriptedAI {
        ...
        {
            _allowDeath = me->GetInstanceScript()->GetBossState(
                              DATA_ORMOROK_EVENT) == DONE;
        }
