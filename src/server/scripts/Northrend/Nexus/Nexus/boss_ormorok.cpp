...
public:
    ...
    struct boss_ormorokAI : public BossAI {
        boss_ormorokAI(Creature* creature)
            : BossAI(creature, DATA_ORMOROK_EVENT)
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ORMOROK_SUMMON:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MinDistance, 0, 50.0f, true))
                    me->CastSpell(
                        target, SPELL_SUMMON_CRYSTALLINE_TANGLER, true);
                events.ScheduleEvent(EVENT_ORMOROK_SUMMON, 17s);
                break;
            ...
            case EVENT_ORMOROK_SUMMON_SPIKES:
                ...
                for (uint8 i = 0; i < 4; ++i) {
                    ...
                    float y =
                        me->GetPositionY() + 5.0f * _spikesCount * std::sin(o);
                    float h = me->GetMapHeight(x, y, me->GetPositionZ());

                    if (h != INVALID_HEIGHT)
                        me->SummonCreature(NPC_CRYSTAL_SPIKE,
                                           x,
                                           y,
                                           h,
                                           0,
    ...
    struct npc_crystal_spikeAI : public NullCreatureAI {
        ...
        void Reset() override
        {
            if (GameObject* gameobject =
                    me->SummonGameObject(GO_CRYSTAL_SPIKE,
                                         me->GetPositionX(),
                                         me->GetPositionY(),
                                         me->GetPositionZ(),
                                         0.0f,
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_damageTimer) {
                ...
                if (_damageTimer >= 2000) {
                    if (GameObject* gameobject =
                            ObjectAccessor::GetGameObject(*me, _gameObjectGUID))
