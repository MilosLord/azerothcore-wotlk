...
public:
    ...
    struct instance_oculus_InstanceMapScript : public InstanceScript {
        ...
        {
            if (m_auiEncounter[DATA_DRAKOS] == DONE &&
                m_auiEncounter[DATA_VAROS] != DONE) {
                player->SendUpdateWorldState(
                    WORLD_STATE_CENTRIFUGE_CONSTRUCT_SHOW, 1);
                player->SendUpdateWorldState(
                ...
                player->SendUpdateWorldState(
                    WORLD_STATE_CENTRIFUGE_CONSTRUCT_SHOW, 0);
                player->SendUpdateWorldState(
                    WORLD_STATE_CENTRIFUGE_CONSTRUCT_AMOUNT, 0);
            }
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_DRAKOS:
                ...
                if (data == DONE) {
                    DoUpdateWorldState(WORLD_STATE_CENTRIFUGE_CONSTRUCT_SHOW,
                                       1);
                    DoUpdateWorldState(WORLD_STATE_CENTRIFUGE_CONSTRUCT_AMOUNT,
                                       10 - CentrifugeCount);
            ...
            case DATA_EREGOS:
                ...
                if (data == DONE) {
                    ...
                    if (GameObject* cache =
                            instance->GetGameObject(EregosCacheGUID)) {
                        if (Creature* eregos =
                                instance->GetCreature(uiEregosGUID)) {
...
void AddSC_instance_oculus() { new instance_oculus(); }
