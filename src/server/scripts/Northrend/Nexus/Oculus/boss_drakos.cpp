...
#define SPELL_THUNDERING_STOMP                                                 \
    DUNGEON_MODE(SPELL_THUNDERING_STOMP_N, SPELL_THUNDERING_STOMP_H)
    ...
    struct boss_drakosAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (pInstance) {
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (ObjectGuid guid =
                            pInstance->GetGuidData(DATA_DCD_1 + i))
                        if (GameObject* pGo =
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUMMON: {
                for (uint8 i = 0; i < 2; ++i) {
                    ...
                    me->SummonCreature(NPC_UNSTABLE_SPHERE,
                                       me->GetPositionX() + 5.0f * cos(angle),
                                       me->GetPositionY() +
                                           5.0f * std::sin(angle),
                                       me->GetPositionZ(),
                                       0.0f,
    ...
    struct npc_oculus_unstable_sphereAI : public ScriptedAI {
        ...
        void PickNewLocation()
        {
            ...
            me->GetMotionMaster()->MovePoint(1,
                                             961.29f + dist * cos(angle),
                                             1049.0f + dist * std::sin(angle),
                                             360.0f);
        }
