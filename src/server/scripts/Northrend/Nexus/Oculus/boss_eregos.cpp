...
#define SPELL_ARCANE_VOLLEY                                                    \
    DUNGEON_MODE(SPELL_ARCANE_VOLLEY_N, SPELL_ARCANE_VOLLEY_H)
...
public:
    ...
    struct boss_eregosAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            me->SummonGameObject(GO_SPOTLIGHT,
                                 1018.06f,
                                 1051.09f,
                                 605.619019f,
                                 0.0f,
                         ...
                         SpellSchoolMask) override
        {
            ...
            if (shiftNumber <= uint32(1) &&
                uint32(me->GetHealth() * 100 / me->GetMaxHealth()) <=
                    uint32(60 - shiftNumber * 40)) {
                ++shiftNumber;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->HasAura(SPELL_PLANAR_SHIFT) ||
                me->HasAura(SPELL_DRAKE_STOP_TIME))
                return;
            ...
            case EVENT_SUMMON_WHELPS:
                for (uint8 i = 0; i < 5; ++i)
                    events.ScheduleEvent(EVENT_SUMMON_SINGLE_WHELP,
                                         urand(0, 8000));
                events.Repeat(40s);
                break;
            ...
            case EVENT_SPELL_PLANAR_SHIFT:
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (Unit* t = SelectTarget(
                            SelectTargetMethod::Random, 0, 300.0f, false))
                        if (Creature* pa =
                                me->SummonCreature(NPC_PLANAR_ANOMALY,
                                                   *me,
...
void AddSC_boss_eregos() { new boss_eregos(); }
