...
float summons[3][4] = {{NPC_PHANTASMAL_AIR,
                        NPC_PHANTASMAL_AIR,
...
float cords[4][4] = {{1177.47f, 937.722f, 527.405f, 2.21657f},
...
public:
    ...
    struct boss_uromAI : public ScriptedAI {
        ...
        {
            for (uint8 i = 0; i < 4; ++i)
                if (me->GetDistance(cords[i][0], cords[i][1], cords[i][2]) <
                    20.0f)
                    return i;
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            if (phase == 3) {
                ...
                me->SetHomePosition(
                    cords[0][0], cords[0][1], cords[0][2], cords[0][3]);
        ...
        void KilledUnit(Unit* /*victim*/) override { Talk(SAY_PLAYER_KILL); }
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
        {
            ...
            case SPELL_SUMMON_MENAGERIE_1: {
                for (uint8 i = 0; i < 4; ++i)
                    me->SummonCreature(summons[0][i],
                                       ...
                                       300000);
                uint8 phase = GetPhaseByCurrentPosition();
                me->SetHomePosition(cords[phase + 1][0],
                                    cords[phase + 1][1],
                                    cords[phase + 1][2],
                                    cords[phase + 1][3]);
            ...
            case SPELL_TELEPORT:
                ...
                // At this point we are still in casting state so we need to
                // clear it for DoCastAOE not to fail
                me->ClearUnitState(UNIT_STATE_CASTING);
                DoCastAOE(DUNGEON_MODE(SPELL_EMPOWERED_ARCANE_EXPLOSION_N,
                                       SPELL_EMPOWERED_ARCANE_EXPLOSION_H));
                me->AddUnitState(UNIT_STATE_CASTING);
                events.RescheduleEvent(EVENT_TELE_BACK,
                                       DUNGEON_MODE(9000, 7000));
            default:
                break;
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_TIME_BOMB:
                if (Unit* target = SelectTarget(
                    ...
                    DoCast(target,
                           DUNGEON_MODE(SPELL_TIME_BOMB_N, SPELL_TIME_BOMB_H));
...
void AddSC_boss_urom() { new boss_urom(); }
