...
#define HAS_ESSENCE(a)                                                         \
    ((a)->HasItemCount(ITEM_EMERALD_ESSENCE) ||                                \
     (a)->HasItemCount(ITEM_AMBER_ESSENCE) ||                                  \
     (a)->HasItemCount(ITEM_RUBY_ESSENCE))
            ...
            if (m_pInstance->GetData(DATA_DRAKOS) == DONE) {
                if (!moved) {
                    ...
                    if (timer > 3000) {
                        ...
                        switch (me->GetEntry()) {
                        case NPC_VERDISA:
                            me->GetMotionMaster()->MovePoint(POINT_MOVE_DRAKES,
                                                             VerdisaPOS);
                            break;
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (creature->GetInstanceScript()->GetData(DATA_DRAKOS) == DONE) {
            ...
            case NPC_VERDISA:
                ...
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_VERDISA1, creature->GetGUID());
                break;
            case NPC_BELGARISTRASZ:
                ...
                if (!openedMenu[player->GetGUID()]) {
                    AddGossipItemFor(player,
                                     ...
                                     GOSSIP_ACTION_INFO_DEF);
                    SendGossipMenuFor(
                        player, GOSSIP_TEXTID_DRAKES, creature->GetGUID());
                }
                else {
                    OnGossipSelect(player,
                                   creature,
                                   GOSSIP_SENDER_MAIN,
                                   GOSSIP_ACTION_INFO_DEF);
                }
            ...
            case NPC_ETERNOS:
                ...
                AddGossipItemFor(player,
                                 ...
                                 GOSSIP_ACTION_INFO_DEF + 4);
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_ETERNOS1, creature->GetGUID());
                break;
            }
        }
                        ...
                        uint32 uiAction) override
    {
        ...
        case NPC_VERDISA:
            ...
            case GOSSIP_ACTION_INFO_DEF:
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_VERDISA2, creature->GetGUID());
                return true;
            case GOSSIP_ACTION_INFO_DEF + 4:
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_VERDISA3, creature->GetGUID());
                return true;
        ...
        case NPC_BELGARISTRASZ:
            ...
            case GOSSIP_ACTION_INFO_DEF:
                ...
                AddGossipItemFor(player,
                                 ...
                                 GOSSIP_ACTION_INFO_DEF + 4);
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_BELGARISTRASZ1, creature->GetGUID());
                return true;
            case GOSSIP_ACTION_INFO_DEF + 4:
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_BELGARISTRASZ2, creature->GetGUID());
                return true;
            case GOSSIP_ACTION_INFO_DEF + 1:
                RemoveEssence(player, ITEM_AMBER_ESSENCE);
                break;
        ...
        case NPC_ETERNOS:
            ...
            case GOSSIP_ACTION_INFO_DEF:
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_ETERNOS2, creature->GetGUID());
                return true;
            case GOSSIP_ACTION_INFO_DEF + 4:
                SendGossipMenuFor(
                    player, GOSSIP_TEXTID_ETERNOS3, creature->GetGUID());
                return true;
            case GOSSIP_ACTION_INFO_DEF + 1:
                RemoveEssence(player, ITEM_EMERALD_ESSENCE);
                break;
    ...
    struct npc_oculus_drakeAI : public VehicleAI {
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            if (m_pInstance->GetBossState(DATA_EREGOS) == IN_PROGRESS)
                if (Creature* eregos =
                        me->FindNearestCreature(NPC_EREGOS, 450.0f, true))
                    eregos->DespawnOrUnsummon(); // On retail this kills abusive
                                                 // call of drake during engaged
                                                 // Eregos
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE && id == POINT_LAND)
                me->SetDisableGravity(
                    false); // Needed this for proper animation after spawn, the
                            // summon in air fall to ground bug leave no other
                            // option for now, if this isn't used the drake will
                            // only walk on move.
        }

        void
        PassengerBoarded(Unit* passenger, int8 /*seatid*/, bool add) override
        {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            for (uint8 i = 0; i < 8; ++i)
                if (me->m_spells[i] == spell->Id) {
                    if (target && target->IsAlive() && !target->CanFly() &&
                        target->IsHostileTo(me) && !spell->IsTargetingArea()) {
            ...
            if (JustSummoned) {
                ...
                if (m_pInstance) {
                    if (!m_pInstance->IsEncounterInProgress() ||
                        m_pInstance->GetData(DATA_EREGOS) == IN_PROGRESS) {
...
public:
    ...
    struct npc_centrifuge_constructAI : public ScriptedAI {
        ...
        {
            DoCast(IsHeroic() ? H_SPELL_EMPOWERING_BLOWS
...
public:
    ...
    class spell_oculus_stop_time_AuraScript : public AuraScript {
        ...
        {
            AfterEffectApply +=
                AuraEffectApplyFn(spell_oculus_stop_time_AuraScript::Apply,
                                  EFFECT_0,
                                  SPELL_AURA_MOD_STUN,
...
public:
    spell_oculus_evasive_maneuvers()
        : SpellScriptLoader("spell_oculus_evasive_maneuvers")
    {
    }
    ...
    class spell_oculus_evasive_maneuvers_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectProc += AuraEffectProcFn(
                spell_oculus_evasive_maneuvers_AuraScript::HandleProc,
                EFFECT_2,
...
public:
    ...
    class spell_oculus_shock_lance_SpellScript : public SpellScript {
        ...
        void CalcDamage()
        {
            ...
            if (Unit* target = GetHitUnit())
                if (Aura* aura = target->GetAura(
                        ...
                        GetCaster()
                            ->GetGUID())) // shock charges from same caster
                {
        ...
        {
            OnHit +=
                SpellHitFn(spell_oculus_shock_lance_SpellScript::CalcDamage);
        }
    };
...
public:
    spell_oculus_temporal_rift()
        : SpellScriptLoader("spell_oculus_temporal_rift")
    {
    }
    ...
    class spell_oculus_temporal_rift_AuraScript : public AuraScript {
            ...
            if (amount >= 15000) {
                if (Unit* caster = GetCaster())
                    for (uint8 i = 0; i < num; ++i)
                        caster->CastSpell(
                            GetTarget(), SPELL_AMBER_SHOCK_CHARGE, true);
            }
        ...
        {
            OnEffectProc += AuraEffectProcFn(
                spell_oculus_temporal_rift_AuraScript::HandleProc,
                EFFECT_2,
                SPELL_AURA_DUMMY);
        }
    };
...
public:
    spell_oculus_touch_the_nightmare()
        : SpellScriptLoader("spell_oculus_touch_the_nightmare")
    {
    }
    ...
    class spell_oculus_touch_the_nightmare_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_oculus_touch_the_nightmare_SpellScript::HandleDamageCalc,
                EFFECT_2,
...
public:
    ...
    class spell_oculus_dream_funnel_AuraScript : public AuraScript {
        ...
        void HandleEffectCalcAmount(AuraEffect const* /*aurEff*/,
                                    int32& amount,
...
public:
    spell_oculus_call_ruby_emerald_amber_drake()
        : SpellScriptLoader("spell_oculus_call_ruby_emerald_amber_drake")
    {
    }

    class spell_oculus_call_ruby_emerald_amber_drake_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void Register() override
        {
            OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
                spell_oculus_call_ruby_emerald_amber_drake_SpellScript::SetDest,
                EFFECT_0,
...
public:
    spell_oculus_ride_ruby_emerald_amber_drake_que()
        : SpellScriptLoader("spell_oculus_ride_ruby_emerald_amber_drake_que")
    {
    }

    class spell_oculus_ride_ruby_emerald_amber_drake_que_AuraScript
        : public AuraScript {
        PrepareAuraScript(
        ...
        {
            // caster of the triggered spell is wrong for an unknown reason,
            // handle it here correctly
            PreventDefaultAction();
            if (Unit* caster = GetCaster())
                GetTarget()->CastSpell(
                    caster,
...
public:
    spell_oculus_evasive_charges()
        : SpellScriptLoader("spell_oculus_evasive_charges")
    {
    }
    ...
    class spell_oculus_evasive_chargesAuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_oculus_evasive_chargesAuraScript::HandleOnEffectApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_oculus_evasive_chargesAuraScript::HandleOnEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    class spell_oculus_soarAuraScript : public AuraScript {
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            // We handle the health regen here, normal heal regen isn't
            // working....
            if (caster->GetHealth() < caster->GetMaxHealth())
                caster->SetHealth(
                    caster->GetHealth() +
                    (uint32)((double)caster->GetMaxHealth() * 0.2));
        }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_oculus_soarAuraScript::HandleEffectPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            OnEffectApply += AuraEffectApplyFn(
                spell_oculus_soarAuraScript::HandleOnEffectApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    class spell_oculus_rider_auraAuraScript : public AuraScript {
        ...
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_oculus_rider_auraAuraScript::HandleOnEffectApply,
                EFFECT_1,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            OnEffectApply += AuraEffectApplyFn(
...
public:
    ...
    class spell_oculus_drake_flagAuraScript : public AuraScript {
        ...
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_oculus_drake_flagAuraScript::HandleOnEffectApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        }
