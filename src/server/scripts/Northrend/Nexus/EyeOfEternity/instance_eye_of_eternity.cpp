...
bool EoEDrakeEnterVehicleEvent::Execute(uint64 /*eventTime*/,
                                        uint32 /*updateTime*/)
{
    if (Player* p = ObjectAccessor::GetPlayer(_owner, _playerGUID))
        if (p->IsInWorld() && !p->IsDuringRemoveFromWorld() &&
            !p->isBeingLoaded() && p->FindMap() == _owner.FindMap()) {
    ...
    struct instance_eye_of_eternity_InstanceMapScript : public InstanceScript {
        ...
        void OnPlayerEnter(Player* pPlayer) override
        {
            if (EncounterStatus == DONE) {
                // destroy platform, hide iris (actually ensure, done at
                // loading, but doesn't always work
                ...
                if (pPlayer) {
                    ...
                    if (Creature* c = pPlayer->SummonCreature(
                        ...
                        c->m_Events.AddEvent(new EoEDrakeEnterVehicleEvent(
                                                 *c, pPlayer->GetGUID()),
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_ENCOUNTER_STATUS:
                ...
                case NOT_STARTED:
                    ...
                    if (GameObject* go =
                            instance->GetGameObject(GO_ExitPortalGUID))
                        go->SetPhaseMask(1, true);
                    if (GameObject* go =
                            instance->GetGameObject(GO_PlatformGUID)) {
                        go->SetDestructibleState(
                ...
                case IN_PROGRESS:
                    bPokeAchiev =
                        (instance->GetPlayersCountExceptGMs() <
                         (instance->GetSpawnMode() == 0 ? (uint32)9
                                                        : (uint32)21));
                    break;
                case DONE:
                    bPokeAchiev = false;
            ...
            case DATA_SET_IRIS_INACTIVE:
                if (GameObject* go = instance->GetGameObject(GO_IrisGUID)) {
                    ...
                    if (Creature* c =
                            go->SummonCreature(NPC_WORLD_TRIGGER_LAOI,
                                               *go,
                                               TEMPSUMMON_TIMED_DESPAWN,
                                               10000))
            ...
            case 20158:
                if (GameObject* go = instance->GetGameObject(GO_PlatformGUID))
                    if (Creature* c = instance->GetCreature(NPC_MalygosGUID)) {
                        go->ModifyHealth(
                            -6500000,
                            c); // We have HP 6 million in the database... So we
                                // have to do at least that
                                          ...
                                          uint32 /*miscvalue1*/) override
        {
            ...
            case ACHIEV_CRITERIA_DENYIN_THE_SCION_25:
                return (source && source->GetVehicle() &&
                        source->GetVehicle()->GetVehicleInfo()->m_ID == 224);
            }
            return false;
        }
    };
...
void AddSC_instance_eye_of_eternity() { new instance_eye_of_eternity(); }
