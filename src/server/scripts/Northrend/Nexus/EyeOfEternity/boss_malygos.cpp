...
#define SPELL_ARCANE_BREATH                                                    \
    DUNGEON_MODE(SPELL_ARCANE_BREATH_N, SPELL_ARCANE_BREATH_H)
#define SPELL_ARCANE_STORM                                                     \
    DUNGEON_MODE(SPELL_ARCANE_STORM_N, SPELL_ARCANE_STORM_H)
#define SPELL_PH3_SURGE_OF_POWER                                               \
    DUNGEON_MODE(SPELL_PH3_SURGE_OF_POWER_N, SPELL_PH3_SURGE_OF_POWER_H)
...
enum Phases { PHASE_NONE = 0, PHASE_ONE, PHASE_TWO, PHASE_THREE };
    ...
    struct boss_malygosAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (pInstance) {
                ...
                pInstance->DoStopTimedAchievement(
            ...
            if (type == POINT_MOTION_TYPE) {
                ...
                case MI_POINT_INTRO_SIDE_3: {
                    ...
                    if (Creature* c = me->SummonCreature(
                            ...
                            me->GetPositionZ(),
                            FourSidesPos[id].GetOrientation(),
                ...
                case MI_POINT_VORTEX_CENTER:
                    if (Creature* c =
                            me->SummonCreature(NPC_WORLD_TRIGGER_LAOI,
                                               CenterPos,
                                               TEMPSUMMON_TIMED_DESPAWN,
                                               15000))
                ...
                case MI_POINT_CIRCLE_OUTSIDE_PH_2:
                    events.RescheduleEvent(
                        EVENT_RESUME_FLYING_CIRCLES_PH_2, 0ms, 1);
                    break;
                case MI_POINT_SURGE_OF_POWER_CENTER:
                    events.RescheduleEvent(
                        EVENT_SURGE_OF_POWER_WARNING, 0ms, 1);
                    break;
                }
            }
                ...
                case MI_POINT_CENTER_AIR_PH_2:
                    me->GetMap()->SetZoneOverrideLight(
                        AREA_EYE_OF_ETERNITY, LIGHT_ARCANE_RUNES, 5s);
                    break;
        ...
        void AttackStart(Unit* victim) override
        {
            ...
            if (me->GetVictim() &&
                me->GetVictim()->GetGUID() == victim->GetGUID() &&
                         ...
                         SpellSchoolMask) override
        {
            if (damage >= me->GetHealth() &&
                !me->HasUnitFlag(
                    UNIT_FLAG_DISABLE_MOVE)) // allow dying only in phase 3!
            {
                damage = 0;
                return;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INTRO_MOVE_CENTER: {
                ...
                me->GetMotionMaster()->MovePoint(
                    MI_POINT_INTRO_CENTER_AIR, x, y, z);
                break;
            }
            case EVENT_INTRO_LAND: {
                me->GetMotionMaster()->MoveLand(MI_POINT_INTRO_LAND,
                                                me->GetPositionX(),
                                                me->GetPositionY(),
            ...
            case EVENT_START_FIGHT: {
                if (pInstance) {
                    ...
                    pInstance->DoStartTimedAchievement(
                        ACHIEVEMENT_TIMED_TYPE_EVENT,
                        ACHIEV_YOU_DONT_HAVE_AN_ENTERNITY_EVENT);
                }
            ...
            case EVENT_SPELL_ARCANE_STORM:
                me->CastCustomSpell(SPELL_ARCANE_STORM,
            ...
            case EVENT_SUMMON_POWER_SPARK: {
                ...
                if (Creature* c = me->SummonCreature(NPC_PORTAL,
                                                     FourSidesPos[random],
                                                     TEMPSUMMON_TIMED_DESPAWN,
                                                     6000))
                    c->CastSpell(c, SPELL_PORTAL_BEAM, false);
                if (Creature* c = me->SummonCreature(NPC_POWER_SPARK,
                                                     FourSidesPos[random],
            ...
            case EVENT_START_VORTEX_0: {
                ...
                me->StopMoving();
                me->SetDisableGravity(true);
                me->GetMotionMaster()->MoveTakeoff(MI_POINT_VORTEX_TAKEOFF,
                                                   me->GetPositionX(),
                                                   me->GetPositionY(),
                                                   CenterPos.GetPositionZ() +
                                                       20.0f,
            ...
            case EVENT_VORTEX_FLY_TO_CENTER:
                me->GetMotionMaster()->MovePoint(MI_POINT_VORTEX_CENTER,
            ...
            case EVENT_START_VORTEX_REAL: {
                ...
                float    angle  = (me->GetOrientation() >= M_PI / 4
                                       ? me->GetOrientation() - M_PI / 4
                                       : 7 * M_PI / 4 + me->GetOrientation());
                pos.m_positionX = CenterPos.GetPositionX() + cos(angle) * 40.0f;
                pos.m_positionY =
                    CenterPos.GetPositionY() + std::sin(angle) * 40.0f;
                pos.m_positionZ = CenterPos.GetPositionZ() + 20.0f;
                pos.SetOrientation(pos.GetAngle(&CenterPos));

                if (Creature* vp = me->SummonCreature(NPC_WORLD_TRIGGER_LAOI,
                                                      pos,
                                                      TEMPSUMMON_TIMED_DESPAWN,
                                                      14000)) {
                    ...
                    if (!PlayerList.IsEmpty())
                        for (Map::PlayerList::const_iterator i =
                            ...
                            if (Player* pPlayer = i->GetSource()) {
                                ...
                                plrpos.m_positionX = CenterPos.GetPositionX() +
                                                     cos(playerAngle) * 5.0f;
                                plrpos.m_positionY =
                                    CenterPos.GetPositionY() +
                                    std::sin(playerAngle) * 5.0f;
                                plrpos.m_positionZ =
                                    CenterPos.GetPositionZ() + 18.0f;
                                plrpos.SetOrientation(
                                    plrpos.GetAngle(&CenterPos));
                                ...
                                if (Creature* c = me->SummonCreature(
                                        ...
                                        15000)) {
                                    pPlayer->CastSpell(
                                        pPlayer, SPELL_FREEZE_ANIM, true);
                                    pPlayer->CastSpell(
                                        c, SPELL_VORTEX_CONTROL_VEHICLE, true);
                                    if (!pPlayer
                                             ->GetVehicle()) // didn't work
                                                             // somehow, try
                                                             // again with a
                                                             // different way,
                                                             // if fails - break
                                    ...
                                    init.MoveTo(CenterPos.GetPositionX(),
                                                CenterPos.GetPositionY(),
                                                CenterPos.GetPositionZ());
                                    ...
                                    sScriptMgr->AnticheatSetUnderACKmount(
                                        pPlayer);

                                    pPlayer->SetGuidValue(PLAYER_FARSIGHT,
                                                          vp->GetGUID());
            ...
            case EVENT_VORTEX_LAND_0:
                me->GetMotionMaster()->MoveLand(
                    MI_POINT_VORTEX_LAND, CenterPos, 7.0f);
            ...
            case EVENT_START_PHASE_2:
                ...
                me->GetMotionMaster()->MoveIdle();
                me->DisableSpline();
                me->GetMotionMaster()->MovePoint(MI_POINT_CENTER_GROUND_PH_2,
                                                 CenterPos);
            ...
            case EVENT_START_PHASE_2_FLY_UP: {
                ...
                me->DisableSpline();
                me->SetDisableGravity(true);
                me->GetMotionMaster()->MoveTakeoff(MI_POINT_CENTER_AIR_PH_2,
                                                   me->GetPositionX(),
                                                   me->GetPositionY(),
                                                   me->GetPositionZ() + 32.0f,
                                                   7.0f);
                events.RescheduleEvent(
                    EVENT_START_PHASE_2_MOVE_TO_SIDE, 22s + 500ms, 1);
                break;
            }
            case EVENT_START_PHASE_2_MOVE_TO_SIDE:
                Talk(SAY_PHASE_2);
                me->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PACIFIED);
                me->GetMotionMaster()->MovePoint(MI_POINT_CIRCLE_OUTSIDE_PH_2,
                                                 Phase2NorthPos);
                ...
                for (int i = 0; i < MAX_NEXUS_LORDS; i++) {
                    float dist = 22.0f;
                    float angle =
                        M_PI / 2 + ((float)i / MAX_NEXUS_LORDS) * 2 * M_PI;
                    if (Creature* disk = me->SummonCreature(
                            NPC_HOVER_DISK,
                            CenterPos.GetPositionX() + cos(angle) * dist,
                            CenterPos.GetPositionY() + std::sin(angle) * dist,
                            CenterPos.GetPositionZ() + 30.0f,
                            0.0f,
                            TEMPSUMMON_MANUAL_DESPAWN,
                            0))
                        if (Creature* c =
                                me->SummonCreature(NPC_NEXUS_LORD,
                                                   *disk,
                                                   TEMPSUMMON_MANUAL_DESPAWN,
                                                   0)) {
                ...
                for (int i = 0; i < MAX_SCIONS_OF_ETERNITY; i++) {
                    float dist = 30.0f;
                    float angle =
                        0.0f + ((float)i / MAX_SCIONS_OF_ETERNITY) * 2 * M_PI;
                    if (Creature* disk = me->SummonCreature(
                            NPC_HOVER_DISK,
                            CenterPos.GetPositionX() + cos(angle) * dist,
                            CenterPos.GetPositionY() + std::sin(angle) * dist,
                            ...
                            0))
                        if (Creature* c =
                                me->SummonCreature(NPC_SCION_OF_ETERNITY,
                                                   *disk,
                                                   TEMPSUMMON_MANUAL_DESPAWN,
                                                   0)) {
                            c->EnterVehicle(disk, 0);
            ...
            case EVENT_RESUME_FLYING_CIRCLES_PH_2: {
                ...
                me->GetMotionMaster()->MovePoint(
                    MI_POINT_CIRCLE_OUTSIDE_PH_2,
                    CenterPos.GetPositionX() + cos(newangle) * dist,
                    CenterPos.GetPositionY() + std::sin(newangle) * dist,
                    Phase2NorthPos.GetPositionZ());
            } break;
            case EVENT_MOVE_TO_SURGE_OF_POWER: {
                ...
                me->GetMotionMaster()->MovePoint(
                    MI_POINT_SURGE_OF_POWER_CENTER,
                    CenterPos.GetPositionX() + cos(angle) * 10.0f,
                    CenterPos.GetPositionY() + std::sin(angle) * 10.0f,
                    Phase2NorthPos.GetPositionZ());
            ...
            case EVENT_SPELL_SURGE_OF_POWER:
                if (Creature* c = me->SummonCreature(NPC_SURGE_OF_POWER,
                                                     CenterPos,
                                                     TEMPSUMMON_TIMED_DESPAWN,
                                                     10000))
                ...
                events.RescheduleEvent(
                    EVENT_RESUME_FLYING_CIRCLES_PH_2, 10s, 1);
            ...
            case EVENT_CHECK_TRASH_DEAD: {
                if (me->FindNearestCreature(
                        NPC_SCION_OF_ETERNITY, 250.0f, true) ||
                    me->FindNearestCreature(NPC_NEXUS_LORD, 250.0f, true))
                    ...
                    me->GetMotionMaster()->MoveIdle();
                    me->StopMoving();
                    me->GetMotionMaster()->MovePoint(MI_POINT_START_PH_3,
                                                     CenterPos.GetPositionX(),
                                                     CenterPos.GetPositionY(),
                                                     CenterPos.GetPositionZ() +
                                                         70.0f);
            ...
            case EVENT_LIGHT_DIMENSION_CHANGE:
                me->GetMap()->SetZoneOverrideLight(
                    AREA_EYE_OF_ETERNITY, LIGHT_CHANGE_DIMENSIONS, 2s);
                break;
            case EVENT_DESTROY_PLATFORM_0:
                if (Creature* c = me->SummonCreature(NPC_WORLD_TRIGGER_LAOI,
                                                     CenterPos,
                                                     TEMPSUMMON_TIMED_DESPAWN,
                                                     3000)) {
                ...
                me->GetMap()->SetZoneOverrideLight(
                    AREA_EYE_OF_ETERNITY, LIGHT_OBSCURE_SPACE, 1s);
                events.RescheduleEvent(EVENT_MOVE_TO_PHASE_3_POSITION, 2s, 1);
                break;
            case EVENT_MOVE_TO_PHASE_3_POSITION: {
                me->SendMeleeAttackStop(me->GetVictim());
                me->GetMotionMaster()->MoveTakeoff(MI_POINT_PH_3_FIGHT_POSITION,
                                                   CenterPos.GetPositionX(),
                                                   CenterPos.GetPositionY(),
                                                   CenterPos.GetPositionZ() -
                                                       5.0f,
                ...
                me->GetThreatMgr()
                    .ClearAllThreat(); // players on vehicle are unattackable ->
                                       // leads to EnterEvadeMode() because
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin();
                        ...
                        if (Player* pPlayer = i->GetSource()) {
                            ...
                            if (Creature* c = me->SummonCreature(
                                ...
                                // pPlayer->CastCustomSpell(60683,
                                // SPELLVALUE_BASE_POINT0, 1, c, true);
                                c->m_Events.AddEvent(
                                    new EoEDrakeEnterVehicleEvent(
                                        *c, pPlayer->GetGUID()),
                                    c->m_Events.CalculateTime(500));
                                AttackStart(c);
                            }
                        }
            ...
            case EVENT_START_PHASE_3:
                ...
                me->GetMap()->SetZoneOverrideLight(
                    AREA_EYE_OF_ETERNITY, LIGHT_OBSCURE_ARCANE_RUNES, 1s);
                me->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                ...
                events.RescheduleEvent(
                    EVENT_SPELL_PH3_SURGE_OF_POWER, 4s, 7s, 1);
                events.RescheduleEvent(EVENT_SPELL_ARCANE_STORM, 12s, 15s, 1);
                break;
            ...
            case EVENT_SPELL_STATIC_FIELD:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 200.0f, false)) {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (pInstance) {
                pInstance->DoUpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, NPC_MALYGOS, 1);
        ...
        void EnterEvadeMode(EvadeReason why) override
        {
            me->SetDisableGravity(true);
            me->GetMap()->SetZoneOverrideLight(
                AREA_EYE_OF_ETERNITY, LIGHT_GET_DEFAULT_FOR_MAP, 1s);
        ...
        void HandleIntroSpeech(const uint32 diff)
        {
            ...
            if (timer2) {
                if (timer2 <= diff) {
                    ...
                    me->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                    me->GetMotionMaster()->MovePoint(
                        MI_POINT_INTRO_SIDE_0 + tmp, FourSidesPos[tmp]);
                }
                else
                    timer2 -= diff;
            }
...
public:
    ...
    struct npc_vortex_rideAI : public VehicleAI {
        npc_vortex_rideAI(Creature* pCreature) : VehicleAI(pCreature)
        {
            ...
            Position pos;
            pos.m_positionX =
                CenterPos.GetPositionX() + VORTEX_RADIUS * cos(angle);
            pos.m_positionY =
                CenterPos.GetPositionY() + VORTEX_RADIUS * std::sin(angle);
        ...
        void PassengerBoarded(Unit* pass, int8 /*seat*/, bool apply) override
        {
            if (pass && !apply && pass->GetTypeId() == TYPEID_PLAYER) {
                ...
                float   speed = plr->GetDistance(me->GetPositionX(),
                                               me->GetPositionY(),
                                               me->GetPositionZ()) /
                              (1.0f * 0.001f);
                plr->MonsterMoveWithSpeed(me->GetPositionX(),
                                          me->GetPositionY(),
                                          me->GetPositionZ(),
                                          speed);
        ...
        void UpdateAI(uint32 diff) override
        {
            /*      here: if player has some aura that should make him exit
               vehicle (eg. ice block) -> exit or make it another way (dunno
               how)                                                          */

            if (despawnTimer <= diff) {
                despawnTimer = 0;
                me->UpdatePosition(CenterPos.GetPositionX(),
                                   CenterPos.GetPositionY(),
                                   CenterPos.GetPositionZ() + 18.0f,
                                   0.0f,
                                   true);
            ...
            if (timer <= diff) {
                float angle    = CenterPos.GetAngle(me);
                float newangle = angle + 2 * M_PI /
                                             ((float)VORTEX_TRAVEL_TIME /
                                              VORTEX_DEFAULT_DIFF);
                if (newangle >= 2 * M_PI)
                    newangle -= 2 * M_PI;
                float newx =
                    CenterPos.GetPositionX() + VORTEX_RADIUS * cos(newangle);
                float newy = CenterPos.GetPositionY() +
                             VORTEX_RADIUS * std::sin(newangle);
                    ...
                    if (Unit* pass = me->GetVehicleKit()->GetPassenger(0))
                        if (Player* plr = pass->ToPlayer()) {
                            if (!bUpdatedFlying && timer) {
                                ...
                                sScriptMgr->AnticheatSetCanFlybyServer(plr,
                                                                       true);
                                sScriptMgr->AnticheatSetUnderACKmount(plr);
                            }

                            plr->SendMonsterMove(
                                me->GetPositionX() + dist * cos(arcangle),
                                me->GetPositionY() + dist * std::sin(arcangle),
                                me->GetPositionZ(),
                                VORTEX_DEFAULT_DIFF * 2,
                                SPLINEFLAG_FLYING);
                            me->Relocate(newx, newy);
                        }

                timer = (diff - timer <= VORTEX_DEFAULT_DIFF)
                            ? VORTEX_DEFAULT_DIFF - (diff - timer)
                            : 0;
            }
            else
                timer -= diff;
            ...
            if (damage >= me->GetHealth()) {
                ...
                if (!me->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE)) {
                    ...
                    me->SetPosition(me->GetPositionX(),
                                    ...
                                    CenterPos.GetPositionZ(),
                                    me->GetOrientation());
                    me->ReplaceAllUnitFlags(UNIT_FLAG_NOT_SELECTABLE |
                ...
                if (pInstance)
                    if (Creature* c = pInstance->instance->GetCreature(
                        ...
                        if (me->IsWithinDist3d(c, 12.0f)) {
                            me->CastSpell(
                                c, SPELL_POWER_SPARK_MALYGOS_BUFF, true);
                            me->DespawnOrUnsummon();
                            return;
                        }
                CheckTimer = 1000;
                ...
                if (Unit* victim = me->GetVictim()) {
                    if (timer <= diff) {
                        if (!victim->IsWithinMeleeRange(me)) {
                            float x, y, z;
                            victim->GetClosePoint(
                                x, y, z, 0.0f, 1.5f, me->GetAngle(victim));
...
public:
    ...
    struct npc_scion_of_eternityAI : public ScriptedAI {
        npc_scion_of_eternityAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            ...
            events.RescheduleEvent(EVENT_TELEPORT_VISUAL, 0ms);
            events.RescheduleEvent(
                EVENT_SCION_OF_ETERNITY_ARCANE_BARRAGE, 20s, 25s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SCION_OF_ETERNITY_ARCANE_BARRAGE: {
                ...
                if (!guids.empty())
                    if (Player* plr = ObjectAccessor::GetPlayer(
                            *me, guids.at(urand(0, guids.size() - 1))))
            ...
            if (apply) {
                if (who->GetTypeId() == TYPEID_PLAYER) {
                    who->ApplySpellImmune(
                        0, IMMUNITY_ID, SPELL_ARCANE_OVERLOAD_DMG, true);
                    who->ApplySpellImmune(
                        0, IMMUNITY_ID, SPELL_SURGE_OF_POWER_DMG, true);
                ...
                me->SetCanFly(false);
                me->GetMotionMaster()->MoveLand(
                    0, me->GetPositionX(), me->GetPositionY(), 267.24f, 10.0f);

                if (who->GetTypeId() == TYPEID_PLAYER) {
                    who->ApplySpellImmune(
                        0, IMMUNITY_ID, SPELL_ARCANE_OVERLOAD_DMG, false);
                    who->ApplySpellImmune(
                        0, IMMUNITY_ID, SPELL_SURGE_OF_POWER_DMG, false);
                }
            }
        }
...
public:
    ...
    struct npc_alexstraszaAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1:
                me->CastSpell(
                    773.98f, 1285.97f, 266.254f, SPELL_ALEXSTRASZA_GIFT, true);
                if (GameObject* chest = me->SummonGameObject(ALEXSTRASZA_GIFT,
                                                             773.98f,
                                                             1285.97f,
                                                             266.254f,
                                                             0.0f,
                ...
                if (GameObject* heart = me->SummonGameObject(HEART_OF_MAGIC,
                                                             773.98f,
                                                             1275.97f,
                                                             266.254f,
                                                             0.0f,
...
public:
    ...
    struct npc_eoe_wyrmrest_skytalonAI : public VehicleAI {
        ...
        void PassengerBoarded(Unit* pass, int8 /*seat*/, bool apply) override
        {
            ...
            else if (pass && pass->GetTypeId() == TYPEID_PLAYER &&
                     me->IsAlive()) {
                me->SetDisplayId(
                    11686); // prevents nasty falling animation at despawn
...
public:
    spell_eoe_ph3_surge_of_power()
        : SpellScriptLoader("spell_eoe_ph3_surge_of_power")
    {
    }
    ...
    class spell_eoe_ph3_surge_of_power_SpellScript : public SpellScript {
        ...
        bool Load() override
        {
            if (Unit* caster = GetCaster())
                if (Creature* c = caster->ToCreature()) {
                    ...
                    c->AI()->SelectTargetList(
                        drakes,
                        (c->GetMap()->GetSpawnMode() == 0 ? 1 : 3),
                        SelectTargetMethod::Random,
                        0,
                        0.0f,
                        false,
                        true,
                        57403 /*only drakes have this aura*/);
                    for (std::list<Unit*>::iterator itr = drakes.begin();
                         itr != drakes.end() && i < 3;
                         ++itr) {
                        ...
                        if (Vehicle* v = (*itr)->GetVehicleKit())
                            if (Unit* p = v->GetPassenger(0))
                                if (Player* plr = p->ToPlayer())
                                    c->AI()->Talk(
                                        EMOTE_SURGE_OF_POWER_WARNING_P3, plr);
                    }
                }
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (Unit* caster = GetCaster()) {
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (DrakeGUID[i])
                        if (Unit* u =
                                ObjectAccessor::GetUnit(*caster, DrakeGUID[i]))
                            targets.push_back(u);
            }
        }
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_eoe_ph3_surge_of_power_SpellScript::FilterTargets,
                EFFECT_0,
