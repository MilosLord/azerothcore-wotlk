...
public:
    spell_q11919_q11940_drake_hunt()
        : SpellScriptLoader("spell_q11919_q11940_drake_hunt")
    {
    }
    ...
    class spell_q11919_q11940_drake_hunt_AuraScript : public AuraScript {
                                ...
                                AuraEffectHandleModes /*mode*/)
        {
            ...
            owner->GetMotionMaster()->MoveFollow(
                GetCaster(), 4.0f, M_PI, MOTION_SLOT_ACTIVE);
            owner->CastSpell(owner, SPELL_SUBDUED, true);
            GetCaster()->CastSpell(
                GetCaster(), SPELL_DRAKE_HATCHLING_SUBDUED, true);
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_q11919_q11940_drake_hunt_AuraScript::HandleEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
...
public:
    ...
    struct npc_sinkhole_kill_creditAI : public NullCreatureAI {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            ...
            if (player && player->GetQuestStatus(QUEST_PLUG_THE_SINKHOLES) ==
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (phaseTimer <= diff) {
                ...
                case 1:
                    ...
                    if (GameObject* cart =
                            me->FindNearestGameObject(GO_EXPLOSIVES_CART, 3.0f))
                ...
                case 5:
                    ...
                    if (Unit* worm = me->FindNearestCreature(
                            NPC_SCOURGED_BURROWER, 3.0f)) {
                ...
                case 7:
                    DoCast(me, SPELL_EXPLODE_CART, true);
                    if (Player* caster =
                            ObjectAccessor::GetPlayer(*me, casterGuid))
                        caster->KilledMonster(me->GetCreatureTemplate(),
                                              me->GetGUID());
                    phaseTimer = 5000;
                    phase      = 8;
                    break;
...
public:
    ...
    struct npc_khunok_the_behemothAI : public ScriptedAI {
...

        {
            ...
            if (who->GetEntry() == NPC_ORPHANED_MAMMOTH_CALF &&
                me->IsWithinDistInMap(who, 10.0f)) {
                if (Unit* owner = who->GetOwner()) {
                    if (owner->GetTypeId() == TYPEID_PLAYER) {
                        owner->CastSpell(
                            owner, SPELL_MAMMOTH_CALF_ESCORT_CREDIT, true);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        SendGossipMenuFor(
            player, NPC_TEXT_THIS_YOUNG_TUSKARR, creature->GetGUID());
...
const uint32 nerubarVictims[3] = {SPELL_FREED_WARSONG_MAGE,
                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_ESCAPE_WINTERFIN_CAVERNS) {
            ...
            if (npc_escortAI* pEscortAI =
                    CAST_AI(npc_lurgglbr::npc_lurgglbrAI, creature->AI()))
                pEscortAI->Start(true, false, player->GetGUID());

            creature->SetFaction(player->GetTeamId() == TEAM_ALLIANCE
...
public:
    npc_captured_beryl_sorcerer()
        : CreatureScript("npc_captured_beryl_sorcerer")
    {
    }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_ADD_ARCANE_CHAINS:
                    if (Player* summoner =
                            me->ToTempSummon()->GetSummonerUnit()->ToPlayer()) {
                        summoner->CastSpell(
                            summoner,
...
public:
    spell_arcane_chains_character_force_cast()
        : SpellScriptLoader("spell_arcane_chains_character_force_cast")
    {
    }

    class spell_arcane_chains_character_force_cast_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_arcane_chains_character_force_cast_SpellScript);
        ...
        void HandleScriptEffect(SpellEffIndex /* effIndex */)
        {
            GetHitUnit()->CastSpell(
                ...
                TriggerCastFlags(
                    TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_SET_FACING &
                    ~TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS &
                    ~TRIGGERED_IGNORE_CAST_ITEM &
                    ~TRIGGERED_IGNORE_GCD)); // Player cast back 45626 on npc
        }
...
public:
    ...
    struct npc_mootoo_the_youngerAI : public npc_escortAI {
        ...
        void Reset() override { SetDespawnAtFar(false); }
...
public:
    ...
    struct npc_valiance_keep_cannoneerAI : public ScriptedAI {
        ...
        void Reset() override { uiTimer = urand(13000, 18000); }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (uiTimer <= diff) {
                ...
                GameObject* pCannon =
                    me->FindNearestGameObject(GO_VALIANCE_KEEP_CANNON_1, 10);
                if (!pCannon)
...
public:
    ...
    struct npc_warmage_coldarraAI : public ScriptedAI {
        ...
        void Reset() override { m_uiTimer = 0; }
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            if (m_uiTimer <= uiDiff) {
                ...
                GetCreatureListWithEntryInGrid(
                    orbList, me, NPC_TRANSITUS_SHIELD_DUMMY, 32.0f);
...
public:
    spell_q11719_bloodspore_ruination_45997()
        : SpellScriptLoader("spell_q11719_bloodspore_ruination_45997")
    {
    }

    class spell_q11719_bloodspore_ruination_45997_SpellScript
        : public SpellScript {
        ...
        void HandleEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
                if (Creature* laurith = caster->FindNearestCreature(
                        NPC_BLOODMAGE_LAURITH, 100.0f))
        ...
        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_q11719_bloodspore_ruination_45997_SpellScript::
                    HandleEffect,
                EFFECT_1,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_THASSARIAN_CAST: {
                    if (Unit* tanathal =
                            me->FindNearestCreature(NPC_TANATHAL, 10.0f)) {
...
public:
    ...
    struct npc_thassarian2AI : public ScriptedAI {
        ...
        void SetData(uint32 /*type*/, uint32 data) override
        {
            ...
            case NPC_LERYSSA: {
                if (Creature* arlos =
                        ObjectAccessor::GetCreature(*me, _arlosGUID)) {
                    arlos->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                          EMOTE_STATE_STUN);
                }
                if (Creature* leryssa =
                        ObjectAccessor::GetCreature(*me, _leryssaGUID)) {
                    leryssa->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                            EMOTE_STATE_STUN);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_THASSARIAN_SCRIPT_1:
                    ...
                    if (Creature* arthas =
                            me->SummonCreature(NPC_IMAGE_LICH_KING,
                                               3729.4614f,
                                               3520.386f,
                                               473.4048f,
                                               1.361f,
                    ...
                    if (Creature* talbot =
                            me->SummonCreature(NPC_COUNSELOR_TALBOT,
                                               3748.7627f,
                                               3614.0374f,
                                               473.4048f,
                                               4.5553f,
                ...
                case EVENT_THASSARIAN_SCRIPT_2:
                    // Arthas load path
                    if (Creature* arthas =
                            ObjectAccessor::GetCreature(*me, _arthasGUID)) {
                ...
                case EVENT_THASSARIAN_SCRIPT_3:
                    // Talbot load path
                    if (Creature* talbot =
                            ObjectAccessor::GetCreature(*me, _talbotGUID)) {
                ...
                case EVENT_THASSARIAN_SCRIPT_6:
                    ...
                    if (Creature* arlos = me->SummonCreature(
                            NPC_GENERAL_ARLOS,
                            3746.2825f,
                            3616.3699f,
                            473.4048f,
                    ...
                    if (Creature* leryssa = me->SummonCreature(
                            NPC_LERYSSA,
                            3751.0986f,
                            3614.9219f,
                            473.4048f,
                        ...
                        leryssa->RemoveNpcFlag(UNIT_NPC_FLAG_GOSSIP |
                                               UNIT_NPC_FLAG_QUESTGIVER);
                        leryssa->GetMotionMaster()->MovePath(PATH_LERYSSA,
                                                             false);
                    }
                ...
                case EVENT_THASSARIAN_SCRIPT_8:
                    ...
                    me->GetMotionMaster()->MovePoint(
                        0, 3722.527f, 3567.2583f, 477.44086f);
                ...
                case EVENT_THASSARIAN_SCRIPT_19:
                    ...
                    if (Creature* leryssa =
                            me->FindNearestCreature(NPC_LERYSSA, 50.0f, true)) {
                        _leryssaGUID = leryssa->GetGUID();
                ...
                case EVENT_THASSARIAN_SCRIPT_20:
                    ...
                    if (Creature* arlos = me->FindNearestCreature(
                            NPC_GENERAL_ARLOS, 50.0f, true)) {
                        _arlosGUID = arlos->GetGUID();
                ...
                case EVENT_THASSARIAN_SCRIPT_22:
                    ...
                    if (Creature* leryssa =
                        ...
                        leryssa->MonsterMoveWithSpeed(
                            3726.751f,
                            3568.1633f,
                            477.44086f,
                            leryssa->GetSpeed(MOVE_RUN));
                    }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (me->IsSummon()) {
                if (Unit* summoner = me->ToTempSummon()->GetSummonerUnit()) {
                    summoner->ToCreature()->AI()->SetData(1,
