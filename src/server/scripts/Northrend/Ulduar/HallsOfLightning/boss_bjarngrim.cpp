...
enum BjarngrimSpells {
    ...
    // OTHER SPELLS
    SPELL_CHARGE_UP =
        52098, // only used when starting walk from one platform to the other
...
public:
    ...
    struct boss_bjarngrimAI : public npc_escortAI {
        boss_bjarngrimAI(Creature* creature)
            : npc_escortAI(creature), summons(creature)
        {
        ...
        void Reset() override
        {
            ...
            for (uint8 i = 0; i < 2; ++i)
                if (Creature* dwarf =
                        me->SummonCreature(NPC_STORMFORGED_LIEUTENANT,
                                           me->GetPositionX() + urand(4, 12),
                                           me->GetPositionY() + urand(4, 12),
                                           me->GetPositionZ())) {
                    dwarf->GetMotionMaster()->MoveFollow(
                        me, 3, rand_norm() * 2 * 3.14f);
        ...
        void JustEngagedWith(Unit*) override
        {
            ...
            events.ScheduleEvent(
                EVENT_BJARNGRIM_REFLECTION, 8s, STANCE_DEFENSIVE);
            ...
            events.ScheduleEvent(
                EVENT_BJARNGRIM_IRONFORM, 12s, STANCE_DEFENSIVE);

            // BERSERKER STANCE
            events.ScheduleEvent(
                EVENT_BJARNGRIM_MORTAL_STRIKE, 24s, STANCE_BERSERKER);
            events.ScheduleEvent(
                EVENT_BJARNGRIM_WHIRLWIND, 26s, STANCE_BERSERKER);
            ...
            if (m_pInstance) {
                ...
                m_pInstance->SetData(
                    ...
                    me->HasAura(SPELL_TEMPORARY_ELECTRICAL_CHARGE));
            }
        }
        ...
        void RollStance(uint8 stance, uint8 force = 0)
        {
            ...
            case STANCE_DEFENSIVE:
                ...
                SetEquipmentSlots(
                    false, EQUIP_SWORD, EQUIP_SHIELD, EQUIP_NO_CHANGE);
                break;
            case STANCE_BERSERKER:
                ...
                SetEquipmentSlots(
                    false, EQUIP_SWORD, EQUIP_SWORD, EQUIP_NO_CHANGE);
                break;
            case STANCE_BATTLE:
                ...
                SetEquipmentSlots(
                    false, EQUIP_MACE, EQUIP_UNEQUIP, EQUIP_NO_CHANGE);
                break;
            }
    ...
    struct npc_stormforged_lieutenantAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_RENEW_STEEL:
                if (Creature* bjarngrim =
                        ObjectAccessor::GetCreature(*me, BjarngrimGUID))
                    if (bjarngrim->IsAlive())
                        me->CastSpell(bjarngrim,
                                      me->GetMap()->IsHeroic()
                                          ? SPELL_RENEW_STEEL_H
                                          : SPELL_RENEW_STEEL_N,
                                      true);
