...
enum Yells { SAY_AGGRO = 0, SAY_SLAY = 1, SAY_DEATH = 2 };
    ...
    struct boss_sjonnirAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (pInstance) {
                ...
                if (pInstance->GetData(BOSS_TRIBUNAL_OF_AGES) == DONE) {
                    ...
                    if (GameObject* doors = me->GetMap()->GetGameObject(
                            pInstance->GetGuidData(GO_SJONNIR_DOOR)))
                        doors->SetGoState(GO_STATE_ACTIVE);

                    if (GameObject* console = me->GetMap()->GetGameObject(
                            pInstance->GetGuidData(GO_SJONNIR_CONSOLE)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_CHECK_HEALTH: {
                if (SummonPhase == PHASE_SUMMON_UNFRIENDLY_DWARFES &&
                    HealthBelowPct(50)) {
                    ...
                    if (pInstance)
                        if (Creature* brann = ObjectAccessor::GetCreature(
                                *me, pInstance->GetGuidData(NPC_BRANN))) {
                            brann->Yell("What in the name o' Madoran did THAT "
                                        "do? Oh! Wait: I just about got it...",
                                        LANG_UNIVERSAL);
                ...
                if (HealthBelowPct(20)) {
                    if (Creature* brann = ObjectAccessor::GetCreature(
                            *me, pInstance->GetGuidData(NPC_BRANN))) {
                        brann->Yell("Ha, that did it! Help's a-comin'! Take "
                                    "this, ya glowin' iron brute!",
                                    LANG_UNIVERSAL);
            ...
            case EVENT_SHIELD: {
                me->CastSpell(me,
                              DUNGEON_MODE(SPELL_LIGHTNING_SHIELD,
            ...
            case EVENT_CHAIN_LIGHTNING: {
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 50.0f, true, 0))
                    me->CastSpell(target,
                                  DUNGEON_MODE(SPELL_CHAIN_LIGHTNING,
            ...
            case EVENT_STATIC_CHARGE: {
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 50.0f, true, 0))
                    me->CastSpell(target,
                                  DUNGEON_MODE(SPELL_STATIC_CHARGE,
            ...
            case EVENT_LIGHTNING_RING: {
                me->CastSpell(
                    me,
                    DUNGEON_MODE(SPELL_LIGHTNING_RING, SPELL_LIGHTNING_RING_H),
                    false);
            ...
            case EVENT_SUMMON_SPEACH: {
                if (Creature* brann = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(NPC_BRANN))) {
                    brann->Yell("This is a wee bit trickier that before... Oh, "
                                "bloody--incomin'!",
                                LANG_UNIVERSAL);
            ...
            case EVENT_SUMMON: {
                ...
                case PHASE_SUMMON_OOZE: {
                    for (uint8 i = POS_GEN_RIGHT; i <= POS_GEN_LEFT; i++) {
                        if (Creature* ooze = me->SummonCreature(
                                NPC_OOZE,
                                ...
                                20000)) {
                            ActivatePipe(i);
                            ooze->GetMotionMaster()->MovePoint(
                                0,
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (pInstance) {
                ...
                if (GameObject* sd = me->GetMap()->GetGameObject(
                        pInstance->GetGuidData(GO_SJONNIR_DOOR)))
        ...
        void ActivatePipe(uint8 side)
        {
            if (pInstance)
                if (GameObject* pipe =
                        me->GetMap()->GetGameObject(pInstance->GetGuidData(
                            side == POS_GEN_RIGHT ? GO_RIGHT_PIPE
                                                  : GO_LEFT_PIPE)))
        ...
        void SummonDwarfes(bool friendly)
        {
            if (friendly) {
                for (int i = 0; i < 3; i++) {
                    ...
                    if (Creature* dwarf =
                            me->SummonCreature(NPC_DWARFES_FRIENDLY,
                ...
                for (int i = 0; i < 2; i++) {
                    uint8 Pos = urand(POS_GEN_RIGHT, POS_GEN_LEFT);
                    if (Creature* dwarf = me->SummonCreature(
                            urand(0, 1) ? NPC_FORGED_IRON_TROGG
    ...
    struct boss_sjonnir_dwarfAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            ...
            DoSpellAttackIfReady((me->GetEntry() == NPC_FORGED_IRON_DWARF)
                                     ? DUNGEON_MODE(SPELL_LIGHTNING_TETHER,
                                                    SPELL_LIGHTNING_TETHER_H)
                                     : DUNGEON_MODE(SPELL_LIGHTNING_SHOCK,
    ...
    struct boss_sjonnir_iron_sludgeAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (InstanceScript* pInstance = me->GetInstanceScript())
                if (Creature* sjonnir = ObjectAccessor::GetCreature(
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_TOXIC_VOLLEY: {
                me->CastSpell(
                    me,
                    DUNGEON_MODE(SPELL_TOXIC_VOLLEY, SPELL_TOXIC_VOLLEY_H),
                    false);
                events.Repeat(5s);
                break;
            }
...
public:
    boss_sjonnir_malformed_ooze()
        : CreatureScript("boss_sjonnir_malformed_ooze")
    {
    }
            ...
            case EVENT_MALFORMED_OOZE_CHECK: {
                ...
                for (std::list<Creature*>::const_iterator itr =
                    ...
                    if ((*itr)->GetGUID() != me->GetGUID() &&
                        (*itr)->IsAlive() && me->IsAlive())
                        if (Creature* is = me->SummonCreature(
                                NPC_IRON_SLUDGE,
                                me->GetPositionX(),
                                me->GetPositionY(),
                                me->GetPositionZ(),
