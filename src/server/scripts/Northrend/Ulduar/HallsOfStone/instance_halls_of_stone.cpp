...
public:
    instance_halls_of_stone()
        : InstanceMapScript("instance_halls_of_stone", 599)
    {
    }
    ...
    struct instance_halls_of_stone_InstanceMapScript : public InstanceScript {
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            if (data == DONE) {
                isMaidenOfGriefDead =
                    type == BOSS_MAIDEN_OF_GRIEF || isMaidenOfGriefDead;
                isKrystalusDead = type == BOSS_KRYSTALLUS || isKrystalusDead;
            }

            if (isMaidenOfGriefDead && isKrystalusDead)
                if (GameObject* tribunalDoor =
                        instance->GetGameObject(goTribunalDoorGUID))
            ...
            if (type == BOSS_TRIBUNAL_OF_AGES && data == DONE) {
                ...
                if (GameObject* pF =
                        instance->GetGameObject(goSkyRoomFloorGUID))
        ...
        {
            data << Encounter[0] << ' ' << Encounter[1] << ' ' << Encounter[2]
                 << ' ' << Encounter[3] << ' ' << Encounter[4] << ' ';
        }
    };
};

void AddSC_instance_halls_of_stone() { new instance_halls_of_stone(); }
