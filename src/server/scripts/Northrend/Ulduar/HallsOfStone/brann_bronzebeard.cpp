...
#define GOSSIP_ITEM_3                                                          \
    "We dont have time for this right now, we have to keep going."
...
static Yells Conversation[] = {
    {14259,
     ...
     0},
    {14247,
     "Take a moment and relish this with me. Soon... all will be revealed. "
    ...
    {14248,
     ...
     17000},
    {13765,
     "Warning: life form pattern not recognized. Archival processing "
     "terminated. Continued interference will result in targeted response.",
     NPC_ABEDNEUM,
     20500},
    {14249,
     ...
     32000},
    {13756,
     "Security breach in progress. Analysis of historical archives transferred "
     "to lower-priority queue. Countermeasures engaged.",
     NPC_KADDRAK,
     37000},
    {14250,
    ...
    {13761,
     "Threat index threshold exceeded. Celestial archive aborted. Security "
     "level heightened.",
     NPC_MARNAK,
     105000},
    ...
    {14253,
     ...
     195000},
    {13767,
     "Critical threat index. Void analysis diverted. Initiating sanitization "
     "protocol.",
     NPC_ABEDNEUM,
     205000},
    {14254,
    ...
    {14256,
     ...
     310000},
    {13769,
     "System online. Life form pattern recognized. Welcome, Branbronzan. "
     "Query?",
     NPC_ABEDNEUM,
     321000},
    {14263,
     "Query? What do you think I'm here for, tea and biscuits? Spill the beans "
     "already!",
     NPC_BRANN,
     329000},
    {14264,
     ...
     336000},
    {13770,
     "Accessing prehistoric data... retrieved. In the beginning the earthen "
     "were created to--",
     NPC_ABEDNEUM,
     342000},
    {14265,
     "Right, right... I know the earthen were made from stone to shape the "
     "deep regions o' the world. But what about the anomalies? Matrix "
     "non-stabilizin' and what-not?",
     NPC_BRANN,
     348000},
    {13771,
     "Accessing... In the early stages of it's development cycle, Azeroth "
     "suffered infection by parasitic necrophotic symbiotes.",
     NPC_ABEDNEUM,
     360000},
    {14266,
     ...
     373500},
    {13772,
     "Designation: Old Gods. Old Gods rendered all systems, including earthen, "
     "defenseless in order to facilitate assimilation. This matrix "
     "destabilization has been termed the Curse of Flesh. Effects of "
     "destabilization increased over time.",
     NPC_ABEDNEUM,
     380000},
    {14267,
     "Old Gods, huh? So they zapped the earthen with this Curse of Flesh... "
     "and then what?",
     NPC_BRANN,
     399500},
    {13757,
     "Accessing... Creators arrived to extirpate symbiotic infection. "
     "Assessment revealed that Old God infestation had grown malignant. "
     "Excising parasites would result in loss of host--",
     NPC_KADDRAK,
     406000},
    {14268,
     ...
     424000},
    {13758,
     "Correct. Creators neutralized parasitic threat and contained it within "
     "the host. Forge of Wills and other systems were instituted to create new "
     "earthen. Safeguards were implemented, and protectors were appointed.",
     NPC_KADDRAK,
     429000},
    {14269, "What protectors?", NPC_BRANN, 449000},
    {13759,
     "Designations: Aesir and Vanir. Or in the common nomenclature, storm and "
     "earth giants. Sentinel Loken designated supreme. Dragon Aspects "
     "appointed to monitor evolution on Azeroth.",
     NPC_KADDRAK,
     452000},
    {14270,
     "Aesir and Vanir... Okay, so the Forge o' Wills started makin' new "
     "earthen... but what happened to the old ones?",
     NPC_BRANN,
     471000},
    {13762,
     "Additional background is relevant to your query: following global combat "
    ...
    {14271,
     ...
     489000},
    {13763,
     "Unknown. Data suggests that impetus for global combat originated with "
     "prime designate Loken, who neutralized all remaining Aesir and Vanir, "
     "affecting termination of conflict. Prime designate Loken then initiated "
     "stasis of several seed races, including earthen, giants and vrykul, at "
     "designated holding facilities.",
     NPC_MARNAK,
     494000},
    {14272,
     "This Loken sounds like a nasty character. Glad we don't have to worry "
     "about the likes o' him anymore. So... if I'm understandin' ye right, the "
     "original earthen eventually woke up from this stasis, and by that time "
     "the destabili-whatever had turned 'em into proper dwarves. Or at "
     "least... dwarf ancestors.",
     NPC_BRANN,
     519000},
    {13764, "Essentially that is correct.", NPC_MARNAK, 543000},
    {14273,
     "Well, now... that's a lot to digest. I'm gonna need some time to take "
     "all this in. Thank ye.",
     NPC_BRANN,
     549000},
    {13773,
     ...
     559000},
    {0,
     "I think it's time to see what's behind the door near the entrance. I'm "
     "going to sneak over there, nice and quiet. Meet me at the door and I'll "
     "get us in.",
     NPC_BRANN,
     574000},
};
    ...
    struct brann_bronzebeardAI : public npc_escortAI {
        ...
        void SwitchHeadVisaul(uint8 headMask, bool activate)
        {
            ...
            if (headMask & 0x1) // Kaddrak
                if ((go = me->GetMap()->GetGameObject(
                         pInstance->GetGuidData(GO_KADDRAK))))
                    activate ? go->SendCustomAnim(0)
                             : go->SetGoState(GO_STATE_READY);

            if (headMask & 0x2) // Marnak
                if ((go = me->GetMap()->GetGameObject(
                         pInstance->GetGuidData(GO_MARNAK))))
                    activate ? go->SendCustomAnim(0)
                             : go->SetGoState(GO_STATE_READY);

            if (headMask & 0x4) // Abedneum
                if ((go = me->GetMap()->GetGameObject(
                         pInstance->GetGuidData(GO_ABEDNEUM))))
                    activate ? go->SendCustomAnim(0)
                             : go->SetGoState(GO_STATE_READY);
        }
        ...
        void ResetEvent()
        {
            if (GameObject* tribunal = ObjectAccessor::GetGameObject(
                    *me, pInstance->GetGuidData(GO_TRIBUNAL_CONSOLE)))
        ...
        void Reset() override
        {
            ...
            if (pInstance) {
                ...
                if (pInstance->GetData(BOSS_TRIBUNAL_OF_AGES) == DONE)
                    pInstance->SetData(
                        BRANN_BRONZEBEARD,
                        (pInstance->GetData(BOSS_SJONNIR) == DONE) ? 5 : 4);
            }
        }
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_START_SJONNIR_FIGHT:
                me->SetFaction(FACTION_FRIENDLY);
                me->Yell("Don't worry! Ol' Brann's got yer back! Keep that "
                         "metal monstrosity busy, and I'll see if I can't "
                         "sweet talk this machine into helping ye!",
                         LANG_UNIVERSAL);
                me->PlayDirectSound(14274);
                SetEscortPaused(false);
                break;
            case ACTION_SJONNIR_DEAD:
                me->Yell("Loken? That's downright bothersome... We might've "
                         "neutralized the iron dwarves, but I'd lay odds "
                         "there's another machine somewhere else churnin' out "
                         "a whole mess o' these iron vrykul!",
                         LANG_UNIVERSAL);
            ...
            case ACTION_OPEN_DOOR:
                if (GameObject* door = ObjectAccessor::GetGameObject(
                        *me, pInstance->GetGuidData(GO_SJONNIR_DOOR)))
        ...
        {
            if (cr->GetEntry() == NPC_ABEDNEUM ||
                cr->GetEntry() == NPC_KADDRAK || cr->GetEntry() == NPC_MARNAK)
                cr->SetCanFly(true);
            else
                summons.Summon(cr);
        }
        ...
        void UpdateEscortAI(uint32 diff) override
        {
            ...
            case EVENT_KADDRAK_HEAD: // First
            {
                if (Creature* kaddrak = GetKaddrak()) {
                    if (Player* plr = SelectTargetFromPlayerList(100.0f))
                        kaddrak->CastSpell(
                            plr,
                            DUNGEON_MODE(SPELL_GLARE_OF_THE_TRIBUNAL,
            ...
            case EVENT_KADDRAK_SWITCH_EYE: {
                if (Creature* kaddrak = GetKaddrak()) {
                    if (urand(0, 1))
                        kaddrak->UpdatePosition(
                            927.9f, 330.9f, 219.4f, 2.4f, true);
                    else
                        kaddrak->UpdatePosition(
                            923.7f, 326.9f, 219.5f, 2.1f, true);
            ...
            case EVENT_MARNAK_HEAD: // Second
            {
                if (Creature* marnak = GetMarnak()) {
                    if (Creature* cr =
                            me->SummonCreature(NPC_DARK_MATTER_TRIGGER,
                                               marnak->GetPositionX(),
                                               marnak->GetPositionY(),
                                               marnak->GetPositionZ(),
                                               0,
                        ...
                        if (Player* plr = SelectTargetFromPlayerList(100.0f)) {
                            float speed = me->GetDistance(plr->GetPositionX(),
                                                          plr->GetPositionY(),
                                                          plr->GetPositionZ()) /
                                          (4000.0f * 0.001f);
                            cr->MonsterMoveWithSpeed(plr->GetPositionX(),
                                                     plr->GetPositionY(),
                                                     plr->GetPositionZ(),
                                                     speed);
                        }
            ...
            case EVENT_ABEDNEUM_HEAD: // Third
            {
                if (GetAbedneum()) {
                    ...
                    if (Creature* cr =
                            me->SummonCreature(NPC_SEARING_GAZE_TRIGGER,
                                               plr->GetPositionX(),
                                               plr->GetPositionY(),
                                               plr->GetPositionZ(),
                                               0,
                                               TEMPSUMMON_TIMED_DESPAWN,
                                               10000)) {
                        // summon another abedneum to create double beam,
                        // despawn just after trigger despawn
                        me->SummonCreature(NPC_ABEDNEUM,
                                           897.0f,
                                           326.9f,
                                           223.5f,
                                           0.0f,
                                           ...
                                           12000);
                        cr->CastSpell(cr,
                                      DUNGEON_MODE(SPELL_SEARING_GAZE,
            ...
            case EVENT_END: {
                ...
                me->ReplaceAllNpcFlags(UNIT_NPC_FLAG_GOSSIP |
                                       UNIT_NPC_FLAG_QUESTGIVER);
                me->Yell("I'll use the forge to make batches o' earthen to "
                         "stand guard... But our greatest challenge still "
                         "remains: find and stop Loken!",
                         LANG_UNIVERSAL);
            ...
            if (TalkEvent) {
                ...
                if (SpeechPause >= Conversation[SpeechCount].timer) {
                    ...
                    if (cs) {
                        cs->Yell(Conversation[SpeechCount].text,
                                 LANG_UNIVERSAL);
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ResetEvent();
            if (pInstance) {
                if (Creature* brann = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(NPC_BRANN))) {
...
{
    Creature* cr = nullptr;
    if ((cr = me->SummonCreature(NPC_KADDRAK,
                                 923.7f,
                                 326.9f,
                                 219.5f,
                                 2.1f,
    ...
    if ((cr = me->SummonCreature(NPC_MARNAK,
                                 895.974f,
                                 363.571f,
                                 219.337f,
                                 5.5f,
    ...
    if ((cr = me->SummonCreature(NPC_ABEDNEUM,
                                 892.25f,
                                 331.25f,
                                 223.86f,
                                 0.6f,
...
void brann_bronzebeard::brann_bronzebeardAI::WaypointReached(uint32 id)
{
    ...
    case 27:
        ...
        if (pInstance) {
            ...
            if (Creature* cr = ObjectAccessor::GetCreature(
                    *me, pInstance->GetGuidData(NPC_SJONNIR)))
    ...
    case 29:
        ...
        if (pInstance)
            if (GameObject* console = ObjectAccessor::GetGameObject(
                    *me, pInstance->GetGuidData(GO_SJONNIR_CONSOLE)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_DRS_LIGHTNING_BOLD: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_DRS_LIGHTING_BOLT_H
            ...
            case EVENT_DRS_SHADOW_WORD_PAIN: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_DRS_SHADOW_WORD_PAIN_H
    ...
    struct iron_golem_custodianAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_IGC_GROUND_SMASH: {
                me->CastSpell(me->GetVictim(),
                              IsHeroic() ? SPELL_IGC_GROUND_SMASH_H
