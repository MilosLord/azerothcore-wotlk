...
#define SPELL_BATTLE_SHOUT                                                     \
    RAID_MODE(SPELL_BATTLE_SHOUT_10, SPELL_BATTLE_SHOUT_25)
#define SPELL_LIGHTINGBOLT                                                     \
    RAID_MODE(SPELL_LIGHTINGBOLT_10, SPELL_LIGHTINGBOLT_25)
#define SPELL_CHAINLIGHTNING                                                   \
    RAID_MODE(SPELL_CHAINLIGHTNING_10, SPELL_CHAINLIGHTNING_25)
...
enum Texts {
    ...
    SAY_COMMANDER_ENGINEERS_DEAD =
        2, // Should be called when all engineers are dead, currently unused
    ...
    struct boss_razorscaleAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (Creature* commander =
                    ObjectAccessor::GetCreature(*me, CommanderGUID))
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            me->GetCreaturesWithEntryInRange(
                eeList, 300.0f, NPC_EXPEDITION_ENGINEER);
            uint8 i = 0;
            for (std::list<Creature*>::iterator itr = eeList.begin();
                 itr != eeList.end();
                 ++itr) {
                if (i > 2)
                    break;
            ...
            if (Creature* c = me->FindNearestCreature(
                    NPC_EXPEDITION_COMMANDER, 300.0f, true))
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            ...
            case SPELL_LAUNCH_CHAIN: {
                ...
                if (caster->GetGUID() ==
                    pInstance->GetGuidData(DATA_HARPOON_FIRE_STATE_1)) {
                    spellId = SPELL_CHAIN_1;
                }
                else if (caster->GetGUID() ==
                         pInstance->GetGuidData(DATA_HARPOON_FIRE_STATE_2)) {
                    spellId = SPELL_CHAIN_2;
                }
                else if (caster->GetGUID() ==
                         pInstance->GetGuidData(DATA_HARPOON_FIRE_STATE_3)) {
        ...
        {
            if (me->GetPositionZ() >
                440.0f) // protection, razorscale is attackable (so harpoons can
                        // hit him, etc.), but should not receive dmg while in
                        // air
                damage = 0;
            else if (!bGroundPhase &&
                     ((me->GetHealth() * 100) / me->GetMaxHealth() < 50) &&
                     me->HasAura(62794)) // already below 50%, but still in
                                         // chains and stunned
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            else if (type == ESCORT_MOTION_TYPE &&
                     me->movespline->Finalized() && !me->IsInCombat()) {
                startPath = true;
                return;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (startPath) {
                me->StopMoving();
                startPath = false;
                if (WaypointPath const* i_path =
                        sWaypointMgr->GetPath(me->GetWaypointPath())) {
            ...
            case EVENT_SPELL_FIREBALL:
                if (Unit* pTarget = SelectTarget(
                        SelectTargetMethod::Random, 0, 200.0f, true))
            ...
            case EVENT_SUMMON_MOLE_MACHINES: {
                ...
                for (int i = 0; i < num; ++i) {
                    ...
                    if (GameObject* drill = me->SummonGameObject(GO_DRILL,
                                                                 cords[i][0],
                                                                 cords[i][1],
                                                                 391.1f,
                                                                 M_PI / 4,
            ...
            case EVENT_SUMMON_ADDS:
                for (int i = 0; i < 4; ++i) {
                    ...
                    for (int j = 0; j < 4; ++j) {
                        ...
                        case 1:
                            if (j == 1)
                                npc_entry = NPC_DARK_RUNE_SENTINEL;
                            break;
                        case 2:
                            switch (j) {
                            case 1:
            ...
            case EVENT_FLY_UP:
                ...
                if (pInstance)
                    for (int i = 0; i < 4; ++i)
                        if (ObjectGuid guid = pInstance->GetGuidData(
                                DATA_HARPOON_FIRE_STATE_1 + i))
                            if (Creature* hfs =
                                    ObjectAccessor::GetCreature(*me, guid)) {
                                me->SummonCreature(34188,
                                                   hfs->GetPositionX(),
                                                   hfs->GetPositionY(),
                                                   hfs->GetPositionZ(),
                                                   0,
                    ...
                    50) // start phase 3
                {
                    ...
                    Unit* target = SelectTarget(
                        SelectTargetMethod::MaxDistance, 0, 0.0, true);
                    if (!target)
                    ...
                    events.ScheduleEvent(EVENT_SPELL_DEVOURING_FLAME_GROUND,
                                         5s);
        ...
        void JustReachedHome() override { startPath = true; }
...
public:
    npc_ulduar_expedition_commander()
        : CreatureScript("npc_ulduar_expedition_commander")
    {
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        Creature* razorscale = ObjectAccessor::GetCreature(
            *creature, instance->GetGuidData(TYPE_RAZORSCALE));
                        ...
                        uint32 uiAction) override
    {
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF + 1) {
            ...
            if (razorscale && !razorscale->IsInCombat()) {
                ...
                for (uint8 i = 0; i < 4; ++i)
                    if (Creature* hfs = ObjectAccessor::GetCreature(
                            *creature,
                            instance->GetGuidData(DATA_HARPOON_FIRE_STATE_1 +
                                                  i)))
                        hfs->AI()->SetData(1, 0);
                ...
                if (razorscale->AI()) {
                    ...
                    razorscale->GetMotionMaster()->MovePoint(
    ...
    struct npc_ulduar_expedition_commanderAI : public NullCreatureAI {
        npc_ulduar_expedition_commanderAI(Creature* creature)
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (who->GetTypeId() != TYPEID_PLAYER ||
                me->GetExactDist2d(who) > 15.0f)
                return;

            _introSpoken = true;
            // Talk(SAY_COMMANDER_INTRO); // No source leads to showing any text
            // messages, perhaps only SOUND ID 15647 is played?
        }
...
public:
    npc_ulduar_harpoonfirestate()
        : CreatureScript("npc_ulduar_harpoonfirestate")
    {
    }
    ...
    struct npc_ulduar_harpoonfirestateAI : public NullCreatureAI {
        npc_ulduar_harpoonfirestateAI(Creature* pCreature)
        ...
        void Reset() override { repairPoints = 0; }
        ...
        void SetData(uint32 id, uint32 value) override
        {
            ...
            case 1: // cleanup at the start of the fight
                if (pInstance) {
                    ...
                    if (GameObject* wh =
                            me->FindNearestGameObject(h_entry, 5.0f)) {
                        wh->SetRespawnTime(0);
                        wh->Delete();
                    }
                    if (GameObject* bh =
                            me->FindNearestGameObject(GO_BROKEN_HARPOON, 5.0f))
            ...
            case 2: // repairing
                if (repairPoints < REPAIR_POINTS) {
                    if (++repairPoints >= REPAIR_POINTS) {
                        if (GameObject* bh = me->FindNearestGameObject(
                                GO_BROKEN_HARPOON, 4.0f))
                            bh->SetPhaseMask(2, true);
                        if (GameObject* wh = me->SummonGameObject(
                                GetHarpoonGunIdForThisHFS(),
                                me->GetPositionX(),
                                me->GetPositionY(),
                                me->GetPositionZ(),
                                3 * M_PI / 2,
            ...
            case 3: // shoot
                if (pInstance) {
                    Creature* razorscale = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(TYPE_RAZORSCALE));
                    if (!razorscale)
                        return;
...
public:
    npc_ulduar_expedition_engineer()
        : CreatureScript("npc_ulduar_expedition_engineer")
    {
    }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (working) {
                if (timer <= diff) {
                    ...
                    if (fixingGUID) {
                        if (Creature* c =
                                ObjectAccessor::GetCreature(*me, fixingGUID))
                            if (me->GetExactDist2dSq(c) <= 25.0f) {
                                ...
                                if (std::fabs(me->GetOrientation() -
                                              me->GetAngle(c)) > M_PI / 4)
                    ...
                    if (!fixingGUID) {
                        ...
                        if (ObjectGuid rsGUID =
                                pInstance->GetGuidData(TYPE_RAZORSCALE))
                            razorscale =
                                ObjectAccessor::GetCreature(*me, rsGUID);
                        ...
                        for (int i = 0; i < 4; ++i)
                            if (ObjectGuid fs_GUID = pInstance->GetGuidData(
                                    DATA_HARPOON_FIRE_STATE_1 + i))
                                if (Creature* fs = ObjectAccessor::GetCreature(
                                        *me, fs_GUID))
                                    if (!fs->AI()->GetData(2)) {
                                        float a = rand_norm() * M_PI;
                                        me->GetMotionMaster()->MovePoint(
                                            0,
                                            fs->GetPositionX() + 3.0f * cos(a),
                                            fs->GetPositionY() +
                                                3.0f * std::sin(a),
                                            fs->GetPositionZ());
...
public:
    npc_ulduar_dark_rune_guardian()
        : CreatureScript("npc_ulduar_dark_rune_guardian")
    {
    }
    ...
    struct npc_ulduar_dark_rune_guardianAI : public ScriptedAI {
        npc_ulduar_dark_rune_guardianAI(Creature* pCreature)
        ...
        void Reset() override { timer2 = 6000; }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (timer2 == 0 && me->GetVictim() &&
                ...
                me->CastSpell(me->GetVictim(),
                              ...
                              true); // cast the same twice cus second one
                                     // requires setting offhand damage
...
public:
    npc_ulduar_dark_rune_watcher()
        : CreatureScript("npc_ulduar_dark_rune_watcher")
    {
    }
    ...
    struct npc_ulduar_dark_rune_watcherAI : public ScriptedAI {
        npc_ulduar_dark_rune_watcherAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
        }
...
public:
    npc_ulduar_dark_rune_sentinel()
        : CreatureScript("npc_ulduar_dark_rune_sentinel")
    {
    }
    ...
    struct npc_ulduar_dark_rune_sentinelAI : public ScriptedAI {
        npc_ulduar_dark_rune_sentinelAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
        }
...
public:
    achievement_quick_shave()
        : AchievementCriteriaScript("achievement_quick_shave")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetTypeId() == TYPEID_UNIT &&
               target->GetEntry() == NPC_RAZORSCALE &&
...
public:
    achievement_iron_dwarf_medium_rare()
        : AchievementCriteriaScript("achievement_iron_dwarf_medium_rare")
    {
    }
