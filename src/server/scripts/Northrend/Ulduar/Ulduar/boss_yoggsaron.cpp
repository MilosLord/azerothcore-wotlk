...
enum YoggSpells {
    ...
    SPELL_INSANE1               = 63120,
    SPELL_INSANE2               = 64464,
    SPELL_INSANE_PERIODIC = 64554, // this checks if player dc'ed and insanes
                                   // him instantly after logging in
...
#define SPELL_PSYCHOSIS                                                        \
    RAID_MODE(SPELL_SARA_PSYCHOSIS_10, SPELL_SARA_PSYCHOSIS_25)
...
static LocationsXY yoggPortalLoc[] = {{1970.48f, -9.75f, 325.5f},
...
public:
    ...
    struct boss_yoggsaron_saraAI : public ScriptedAI {
        boss_yoggsaron_saraAI(Creature* pCreature)
            : ScriptedAI(pCreature), summons(pCreature)
        {
        ...
        void JustSummoned(Creature* cr) override
        {
            ...
            if (cr->GetEntry() >= NPC_FREYA_KEEPER &&
                ...
                cr->ApplySpellImmune(
                    0, IMMUNITY_STATE, SPELL_AURA_MOD_SCALE, true);
        }
        ...
        void SpawnClouds()
        {
            for (uint8 i = 0; i < 6; ++i) {
                ...
                if (i % 2)
                    me->SummonCreature(NPC_OMINOUS_CLOUD,
                                       me->GetPositionX() + 8 + i * 7,
                                       me->GetPositionY() + 8 + i * 7,
                                       326 + Zplus,
                                       0);
                else
                    me->SummonCreature(NPC_OMINOUS_CLOUD,
        ...
        {
            me->SummonCreature(
                NPC_SANITY_WELL, 2042.56f, -40.3667f, 329.274f, 0.0f);
            me->SummonCreature(
                NPC_SANITY_WELL, 1975.89f, 40.0216f, 331.1f, 0.0f);
            me->SummonCreature(
                NPC_SANITY_WELL, 1987.12f, -91.2702f, 330.186f, 0.0f);
            me->SummonCreature(
                NPC_SANITY_WELL, 1900.48f, -51.2386f, 332.13f, 0.0f);
            me->SummonCreature(
                NPC_SANITY_WELL, 1899.94f, 0.330621f, 332.296f, 0.0f);
        }
        ...
        void Reset() override
        {
            if (!_secondPhase) // Phase 1 wipe
            {
                me->GetMap()->DoForAllPlayers([&](Player* player) {
                    if (Creature* voice = me->FindNearestCreature(
                            NPC_VOICE_OF_YOGG_SARON, 10.0f)) {
            ...
            if (m_pInstance) {
                m_pInstance->DoStopTimedAchievement(
                    ACHIEVEMENT_TIMED_TYPE_EVENT, CRITERIA_NOT_GETTING_OLDER);
                m_pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_SANITY);
                m_pInstance->SetData(TYPE_YOGGSARON, NOT_STARTED);
                if (GameObject* go = ObjectAccessor::GetGameObject(
                        *me, m_pInstance->GetGuidData(GO_YOGG_SARON_DOORS)))
        ...
        void InitFight(Unit* target)
        {
            ...
            if (m_pInstance->GetData(TYPE_VEZAX) != DONE ||
                m_pInstance->GetData(TYPE_XT002) != DONE)
                return;

            m_pInstance->DoStartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_EVENT,
            ...
            events.ScheduleEvent(
                EVENT_SARA_P1_DOORS_CLOSE, 15s, 0, EVENT_PHASE_ONE);
        ...
        void SaveKeepers()
        {
            for (uint8 i = 0; i < 4; ++i)
                if (m_pInstance->GetData(TYPE_WATCHERS) & (1 << i))
                    ...
                    case KEEPER_FREYA:
                        SpawnWeels();
                        me->SummonCreature(NPC_FREYA_KEEPER,
                                           1939.32f,
                                           42.165f,
                                           338.415f,
                                           5.17955f);
                        break;
                    case KEEPER_HODIR:
                        me->SummonCreature(NPC_HODIR_KEEPER,
                                           1939.13f,
                                           -90.8332f,
                                           338.415f,
                                           1.00123f);
                        break;
                    case KEEPER_MIMIRON:
                        me->SummonCreature(NPC_MIMIRON_KEEPER,
                                           2036.81f,
                                           25.6646f,
                                           338.415f,
                                           3.74227f);
                        break;
                    case KEEPER_THORIM:
                        me->SummonCreature(NPC_THORIM_KEEPER,
                                           2036.59f,
                                           -73.8499f,
                                           338.415f,
                                           2.34819f);
        ...
        void InformCloud()
        {
            ...
            for (SummonList::const_iterator itr = summons.begin();
                ...
                ++itr;
                if (!summon || summon->GetEntry() != NPC_OMINOUS_CLOUD ||
                    me->GetDistance(summon) < 20)
                    continue;

                if ((!cloud || (urand(0, 1) &&
                                !summon->HasAura(SPELL_SUMMON_GUARDIAN_OF_YS))))
                    cloud = summon;
            }
        ...
        void SpawnTentacle(uint32 entry)
        {
            ...
            if (Creature* cr =
                    me->SummonCreature(entry,
                                       me->GetPositionX() + dist * cos(o),
                                       me->GetPositionY() + dist * std::sin(o),
                                       327.2 + Zplus,
                                       0,
        ...
        void SummonDeathOrbs()
        {
            for (uint8 i = 0; i < 4; ++i) {
                ...
                float  o     = rand_norm() * M_PI * 2;
                float  Zplus = (dist - 38) / 6.5f;
                me->SummonCreature(NPC_DEATH_ORB,
                                   me->GetPositionX() + dist * cos(o),
                                   me->GetPositionY() + dist * std::sin(o),
                                   327.2 + Zplus,
                                   0,
        ...
        void AddPortals()
        {
            ...
            for (uint8 i = 0; i < RAID_MODE(4, 10); ++i) {
                if ((cr = me->SummonCreature(NPC_DESCEND_INTO_MADNESS,
                                             yoggPortalLoc[i].x,
                                             yoggPortalLoc[i].y,
                                             yoggPortalLoc[i].z,
                                             0,
                                             TEMPSUMMON_TIMED_DESPAWN,
                                             25000))) {
                    cr->SetUnitFlag(UNIT_FLAG_DISABLE_MOVE |
            ...
            if (_isIllusionReversed)
                _currentIllusion =
                    _currentIllusion == 3 ? 1 : (_currentIllusion + 1);
            else
                _currentIllusion =
                    _currentIllusion == 1 ? 3 : (_currentIllusion - 1);
        }
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_BRAIN_DAMAGED) {
                ...
                if (!(_keepersGUID[0] && _keepersGUID[1] && _keepersGUID[2] &&
                      _keepersGUID[3]) &&
                    me->GetMap()->Is25ManRaid())
            ...
            if (Creature* yoggb = ObjectAccessor::GetCreature(
                    *me,
                    me->GetInstanceScript()->GetGuidData(
            ...
            uint32 portalTime =
                (timer > events.GetTimer() ? timer - events.GetTimer() : 0);
            events.DelayEvents(param + 100);
            events.RescheduleEvent(
                EVENT_SARA_P2_OPEN_PORTALS, portalTime, 0, EVENT_PHASE_TWO);
            events.ScheduleEvent(
                EVENT_SARA_P2_REMOVE_STUN, param, 0, EVENT_PHASE_TWO);
                         ...
                         SpellSchoolMask) override
        {
            if (who && who->GetEntry() == NPC_GUARDIAN_OF_YS && !_secondPhase) {
                ...
                if (me->GetHealth() <= damage) {
                    ...
                    if (Creature* cr = me->SummonCreature(NPC_YOGG_SARON,
                                                          me->GetPositionX(),
                                                          me->GetPositionY(),
                                                          me->GetPositionZ(),
                                                          M_PI))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_p2TalkTimer) {
                ...
                if (_p2TalkTimer >= 4000 && _p2TalkTimer < 20000) {
                    ...
                    _p2TalkTimer = 0;
                    events.ScheduleEvent(
                        EVENT_SARA_P2_START, 500ms, 0, EVENT_PHASE_TWO);
                }
                return;
            }
            ...
            case EVENT_SARA_P1_DOORS_CLOSE:
                // Whispers of YS
                me->SummonCreature(NPC_VOICE_OF_YOGG_SARON,
                                   me->GetPositionX(),
                                   me->GetPositionY(),
                                   me->GetPositionZ());
                ...
                events.ScheduleEvent(
                    EVENT_SARA_P1_SPELLS, 0ms, 1, EVENT_PHASE_ONE);
                break;
            case EVENT_SARA_P1_SUMMON:
                events.RepeatEvent(
                    20000 -
                    (std::min(_summonedGuardiansCount, (uint8)5) * 2000));
                ++_summonedGuardiansCount;
                InformCloud();
                break;
            case EVENT_SARA_P1_SPELLS: {
                uint32 spell = RAND(SPELL_SARAS_ANGER_TARGET_SELECTOR,
                ...
                me->CastCustomSpell(
                    spell, SPELLVALUE_MAX_TARGETS, 1, nullptr, false);
                events.RepeatEvent(me->GetMap()->Is25ManRaid()
                                       ? urand(0, 3000)
                                       : 4000 + urand(0, 2000));
                break;
            }
            case EVENT_SARA_P2_START: {
                ...
                events.RescheduleEvent(EVENT_SARA_P2_SPAWN_START_TENTACLES,
                                       500,
                                       0,
                                       EVENT_PHASE_TWO);
                ...
                // Spawn Brain!
                me->SummonCreature(
                    NPC_BRAIN_OF_YOGG_SARON, 1981.3f, -25.43f, 265);
                break;
            }
            case EVENT_SARA_P2_MALADY:
                me->CastCustomSpell(SPELL_MALADY_OF_THE_MIND,
            ...
            case EVENT_SARA_P2_PSYCHOSIS:
                if ((urand(0, 9)) ==
                    0) // Rarely said (as it's casted every 3.5s)
                {
                    Talk(SAY_SARA_PSYCHOSIS_HIT);
                }
                me->CastCustomSpell(
                    SPELL_PSYCHOSIS, SPELLVALUE_MAX_TARGETS, 1, me, false);
            ...
            case EVENT_SARA_P2_BRAIN_LINK:
                me->CastCustomSpell(
                    SPELL_BRAIN_LINK, SPELLVALUE_MAX_TARGETS, 1, me, false);
                events.Repeat(30s);
                break;
            ...
            case EVENT_SARA_P2_SPAWN_START_TENTACLES:
                ...
                me->NearTeleportTo(me->GetPositionX(),
                                   me->GetPositionY(),
                                   355,
                                   me->GetOrientation());
                me->SetPosition(me->GetPositionX(),
                                me->GetPositionY(),
                                355,
                                me->GetOrientation());
                ...
                events.ScheduleEvent(
                    EVENT_SARA_P2_MALADY, 7s, 0, EVENT_PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SARA_P2_PSYCHOSIS, 3s, 0, EVENT_PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SARA_P2_DEATH_RAY, 15s, 0, EVENT_PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SARA_P2_SUMMON_T1, 50s, 60s, 0, EVENT_PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SARA_P2_SUMMON_T2, 15s, 20s, 0, EVENT_PHASE_TWO);
                events.ScheduleEvent(EVENT_SARA_P2_SUMMON_T3,
                                     30000 + urand(0, 10000),
                                     0,
                                     EVENT_PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SARA_P2_BRAIN_LINK, 0, 0, EVENT_PHASE_TWO);
                events.ScheduleEvent(
                    EVENT_SARA_P2_OPEN_PORTALS, 60000, 0, EVENT_PHASE_TWO);
                break;
            case EVENT_SARA_P1_BERSERK:
                if (me->GetInstanceScript()) {
                    if (Creature* yogg = ObjectAccessor::GetCreature(
                            *me,
                            me->GetInstanceScript()->GetGuidData(
                                TYPE_YOGGSARON))) {
...
public:
    ...
    struct boss_yoggsaron_cloudAI : public npc_escortAI {
        ...
        void JustSummoned(Creature* cr) override
        {
            ...
            _isSummoning = false;
            if (me->GetInstanceScript())
                if (Creature* sara = ObjectAccessor::GetCreature(
                        *me, me->GetInstanceScript()->GetGuidData(NPC_SARA)))
        ...
        void InitWaypoint()
        {
            ...
            if (me->GetPositionX() > Middle.GetPositionX()) {
                for (uint8 i = 0; i <= dist; ++i) {
                    ...
                    AddWaypoint(i,
                                Middle.GetPositionX() + dist * cos(angle),
                                Middle.GetPositionY() + dist * std::sin(angle),
                                me->GetPositionZ(),
                                0);
                }
            }
        ...
        void UpdateEscortAI(uint32 diff) override
        {
            ...
            if (_checkTimer >= 500 && !_isSummoning) {
                ...
                if (who && who->IsPlayer() &&
                    !me->HasAura(SPELL_SUMMON_GUARDIAN_OF_YS) &&
                    !who->HasAura(SPELL_HODIR_FLASH_FREEZE)) {
                    _isSummoning = true;
                    Talk(0, who);
                    me->CastSpell(me, SPELL_SUMMON_GUARDIAN_OF_YS, true);
                }
...
public:
    boss_yoggsaron_guardian_of_ys()
        : CreatureScript("boss_yoggsaron_guardian_of_ys")
    {
    }
    ...
    struct boss_yoggsaron_guardian_of_ysAI : public ScriptedAI {
        boss_yoggsaron_guardian_of_ysAI(Creature* pCreature)
...
public:
    ...
    struct boss_yoggsaronAI : public ScriptedAI {
            ...
            : ScriptedAI(pCreature), summons(pCreature)
        {
            ...
            uint8 _count = 4;
            me->SetLootMode(
                31); // 1 + 2 + 4 + 8 + 16, remove with watchers addition
            if (m_pInstance) {
                for (uint8 i = 0; i < 4; ++i)
                    if (m_pInstance->GetData(TYPE_WATCHERS) & (1 << i)) {
        ...
        void SummonImmortalGuardian()
        {
            ...
            float  o     = rand_norm() * M_PI * 2;
            float  Zplus = (dist - 38) / 6.5f;
            me->SummonCreature(NPC_IMMORTAL_GUARDIAN,
                               me->GetPositionX() + dist * cos(o),
                               me->GetPositionY() + dist * std::sin(o),
                               327.2 + Zplus,
                               0,
        ...
        void JustDied(Unit* /*who*/) override
        {
            ...
            if (m_pInstance) {
                ...
                if (Creature* sara = ObjectAccessor::GetCreature(
                        *me, m_pInstance->GetGuidData(NPC_SARA)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_YS_SHADOW_BEACON:
                ...
                me->CastCustomSpell(SPELL_SHADOW_BEACON,
                                    SPELLVALUE_MAX_TARGETS,
                                    RAID_MODE(1, 3),
                                    me,
                                    false);
...
public:
    ...
    struct boss_yoggsaron_brainAI : public NullCreatureAI {
        boss_yoggsaron_brainAI(Creature* pCreature)
            : NullCreatureAI(pCreature), summons(pCreature)
        {
            me->SetDisableGravity(true);
            _tentacleCount  = 0;
            _activeIllusion = 0;
        ...
        void JustSummoned(Creature* cr) override
        {
            if (cr->GetEntry() == NPC_INFLUENCE_TENTACLE) {
                // Dragons Illusion
                if (cr->GetPositionX() > 2000.0f &&
                    cr->GetPositionX() < 2150.0f)
                    cr->UpdateEntry(urand(NPC_CONSORT_FIRST, NPC_CONSORT_LAST));
                // Icecrown Illusion
                else if (cr->GetPositionY() > -150.0f &&
                         cr->GetPositionY() < -90.0f) {
        ...
        {
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 2126.13f, -65.488f, 239.721f, 1.99171f);
            me->SummonCreature(NPC_INFLUENCE_TENTACLE,
                               2141.05f,
                               -50.5146f,
                               239.751f,
                               2.72998f);
            ...
            me->SummonCreature(NPC_INFLUENCE_TENTACLE,
                               ...
                               5.96974f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 2117.31f, 14.897f, 239.731f, 4.32041f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 2136.7f, 2.43262f, 239.72f, 3.90023f);

            // Laughing Skulls
            if (urand(0, 1))
                me->SummonCreature(
                    NPC_LAUGHING_SKULL, 2139.13f, -59.0848f, 239.728f, 2.2974f);
            else
                me->SummonCreature(NPC_LAUGHING_SKULL, 2083, -25.66f, 244, 0);
            if (urand(0, 1))
                me->SummonCreature(NPC_LAUGHING_SKULL,
                                   2066.67f,
                                   -59.8984f,
                                   239.72f,
                                   0.718747f);
                ...
                me->SummonCreature(
                    NPC_LAUGHING_SKULL, 2126.22f, -25.86f, 244, 0);

            me->SummonCreature(
                NPC_LAUGHING_SKULL, 2133.09f, 15.341f, 239.72f, 4.0724f);
            me->SummonCreature(
                NPC_LAUGHING_SKULL, 2065.83f, 12.3772f, 239.792f, 5.49789f);
            ...
            // Yogg Vision
            me->SummonCreature(
                NPC_YOGG_SARON_VISION, 2109.695f, -25.09549f, 222.3250f, 0);
        }
        ...
        void PrepareIceCrownIllusion()
        {
            // Laughing Skulls
            me->SummonCreature(
                NPC_LAUGHING_SKULL, 1931.12f, -92.702f, 239.991f, 5.2819f);
            if (urand(0, 1))
                me->SummonCreature(NPC_LAUGHING_SKULL,
                                   1969.88f,
                                   -147.729f,
            ...
            if (urand(0, 1))
                ...
                me->SummonCreature(
                    NPC_LAUGHING_SKULL, 1938.45f, -116.5f, 240, 0);
            if (urand(0, 1))
                me->SummonCreature(NPC_LAUGHING_SKULL,
                                   1896.45f,
                                   -141.469f,
            ...
            // Influence
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1958.29f, -128.65f, 239.99f, 3.61293f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1957.78f, -134.368f, 239.99f, 3.35375f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1953.04f, -137.843f, 239.99f, 3.55796f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1900.31f, -93.5241f, 239.99f, 4.50043f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1895.03f, -98.0773f, 239.99f, 4.88135f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1895.19f, -104.587f, 239.99f, 5.02271f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1923.31f, -125.98f, 240, 4.2f);

            // Others
            me->SummonCreature(NPC_LICH_KING, 1906.98f, -153, 240, 4.2f);
            me->SummonCreature(
                NPC_IMMOLATED_CHAMPION, 1902.03f, -161.7f, 240, 1.07f);

            // Yogg Vision
            me->SummonCreature(
                NPC_YOGG_SARON_VISION, 1906.226f, -155.8941f, 223.4727, 0);
        }
        ...
        void PrepareStormwindIllusion()
        {
            ...
            if (urand(0, 1))
                me->SummonCreature(
                    NPC_LAUGHING_SKULL, 1916.36f, 28.05f, 239.666f, 1.30238f);
            else
                me->SummonCreature(
                    NPC_LAUGHING_SKULL, 1966.7f, 57.8f, 239.66f, 0);
            if (urand(0, 1))
                me->SummonCreature(
                    NPC_LAUGHING_SKULL, 1902, 75.1362f, 239.666f, 6.06189f);
            else
                me->SummonCreature(NPC_LAUGHING_SKULL, 1933, 91, 240, 0);
            me->SummonCreature(
                NPC_LAUGHING_SKULL, 1914.42f, 90.8465f, 239.666f, 5.25294f);
            me->SummonCreature(
                NPC_LAUGHING_SKULL, 1963.68f, 89.7549f, 239.667f, 3.70571f);

            // Influence
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1931.41f, 39.0711f, 239.66f, 1.82467f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1908.67f, 45.5867f, 239.666f, 0.72119f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1897.68f, 66.1274f, 239.666f, 6.27395f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1950.73f, 49.3446f, 239.666f, 2.63756f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1923.16f, 97.5586f, 239.666f, 4.74635f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1956.16f, 72.1403f, 239.666f, 3.19518f);
            me->SummonCreature(
                NPC_INFLUENCE_TENTACLE, 1944.81f, 92.3154f, 239.666f, 4.03556f);

            // Others
            me->SummonCreature(NPC_GARONA, 1928.58f, 65.64f, 242.37f, 2.1f);
            me->SummonCreature(
                NPC_KING_LLANE, 1925.14f, 71.74f, 242.37f, 5.17f);

            // Yogg Vision
            me->SummonCreature(
                NPC_YOGG_SARON_VISION, 1929.160f, 67.75694f, 221.7322f, 0);
        }
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_DESPAWN_ADDS) {
                ...
                if (_tentacleCount >= 7 /*TENTACLES COUNT*/) {
                    ...
                    if (me->GetInstanceScript())
                        if (Creature* sara = ObjectAccessor::GetCreature(
                                *me,
                                me->GetInstanceScript()->GetGuidData(NPC_SARA)))
                            sara->AI()->DoAction(
                                MINUTE * IN_MILLISECONDS -
                                std::min((uint32)MINUTE * IN_MILLISECONDS,
                                         _induceTimer));

                    _induceTimer = 0;
                    summons.DespawnEntry(NPC_LAUGHING_SKULL);
                    if (GameObject* go = me->FindNearestGameObject(
                            GO_CHAMBER_ILLUSION_DOORS + _activeIllusion,
                            150.0f))
                        go->SetGoState(GO_STATE_ACTIVE);
                }
                return;
            ...
            for (uint32 i = GO_CHAMBER_ILLUSION_DOORS;
                 i <= GO_STORMWIND_ILLUSION_DOORS;
                 ++i)
...
public:
    ...
    struct boss_yoggsaron_death_orbAI : public NullCreatureAI {
        boss_yoggsaron_death_orbAI(Creature* pCreature)
...
public:
    boss_yoggsaron_crusher_tentacle()
        : CreatureScript("boss_yoggsaron_crusher_tentacle")
    {
    }
    ...
    struct boss_yoggsaron_crusher_tentacleAI : public ScriptedAI {
        boss_yoggsaron_crusher_tentacleAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
        ...
        void Reset() override { me->SetInCombatWithZone(); }
...
public:
    boss_yoggsaron_corruptor_tentacle()
        : CreatureScript("boss_yoggsaron_corruptor_tentacle")
    {
    }
    ...
    struct boss_yoggsaron_corruptor_tentacleAI : public ScriptedAI {
        boss_yoggsaron_corruptor_tentacleAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            me->SetCombatMovement(false);
        }
        ...
        Unit* SelectCorruptionTarget()
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                 itr != pList.end();
                 ++itr, ++count) {
                if (me->GetDistance(itr->GetSource()) > 200 ||
                    itr->GetSource()->GetPositionZ() < 300 ||
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            ...
            if (Unit* target = SelectCorruptionTarget()) {
                uint32 spellid = RAND(SPELL_APATHY,
                                      SPELL_BLACK_PLAGUE,
...
public:
    boss_yoggsaron_constrictor_tentacle()
        : CreatureScript("boss_yoggsaron_constrictor_tentacle")
    {
    }
    ...
    struct boss_yoggsaron_constrictor_tentacleAI : public ScriptedAI {
        boss_yoggsaron_constrictor_tentacleAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            me->SetCombatMovement(false);
            _checkTimer = 1;
        ...
        Unit* SelectConstrictTarget()
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                 itr != pList.end();
                 ++itr, ++count) {
                if (me->GetDistance(itr->GetSource()) > 10 ||
                    !itr->GetSource()->IsAlive() ||
                    itr->GetSource()->IsGameMaster())
                    continue;
                if (itr->GetSource()->HasAura(SPELL_SQUEEZE) ||
                    itr->GetSource()->HasAura(SPELL_INSANE1))
                    continue;
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_checkTimer) {
                _checkTimer += diff;
                if (_checkTimer >= 1000 &&
                    !me->HasUnitState(UNIT_STATE_STUNNED)) {
...
public:
    ...
    struct boss_yoggsaron_keeperAI : public NullCreatureAI {
        ...
        void DoAction(int32 param) override
        {
            if (me->GetEntry() == NPC_THORIM_KEEPER &&
                param == ACTION_THORIM_START_STORM)
        ...
        {
            if (me->GetInstanceScript())
                if (me->GetInstanceScript()->GetData(TYPE_YOGGSARON) !=
                    IN_PROGRESS)
                    return;
...
public:
    boss_yoggsaron_descend_portal()
        : CreatureScript("boss_yoggsaron_descend_portal")
    {
    }

    struct boss_yoggsaron_descend_portalAI : public PassiveAI {
        boss_yoggsaron_descend_portalAI(Creature* creature)
            : PassiveAI(creature), _instance(creature->GetInstanceScript())
        {
        }
...
public:
    boss_yoggsaron_influence_tentacle()
        : CreatureScript("boss_yoggsaron_influence_tentacle")
    {
    }
    ...
    struct boss_yoggsaron_influence_tentacleAI : public NullCreatureAI {
        boss_yoggsaron_influence_tentacleAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
        ...
        void
        DamageTaken(Unit*, uint32&, DamageEffectType, SpellSchoolMask) override
        {
...
public:
    boss_yoggsaron_immortal_guardian()
        : CreatureScript("boss_yoggsaron_immortal_guardian")
    {
    }
    ...
    struct boss_yoggsaron_immortal_guardianAI : public ScriptedAI {
        boss_yoggsaron_immortal_guardianAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            Reset();
        }
...
public:
    spell_yogg_saron_malady_of_the_mind()
        : SpellScriptLoader("spell_yogg_saron_malady_of_the_mind")
    {
    }
    ...
    class spell_yogg_saron_malady_of_the_mind_AuraScript : public AuraScript {
        ...
        {
            GetUnitOwner()->ApplySpellImmune(SPELL_DEATH_RAY_DAMAGE_REAL,
                                             IMMUNITY_ID,
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            GetUnitOwner()->ApplySpellImmune(SPELL_DEATH_RAY_DAMAGE_REAL,
                                             ...
                                             SPELL_DEATH_RAY_DAMAGE_REAL,
                                             false);
            GetUnitOwner()->CastCustomSpell(SPELL_MALADY_OF_THE_MIND_TRIGGER,
                                            SPELLVALUE_MAX_TARGETS,
                                            1,
                                            GetUnitOwner(),
                                            true);
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_yogg_saron_malady_of_the_mind_AuraScript::OnApply,
                EFFECT_1,
                SPELL_AURA_MOD_FEAR,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_yogg_saron_malady_of_the_mind_AuraScript::OnRemove,
                EFFECT_1,
                SPELL_AURA_MOD_FEAR,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_yogg_saron_brain_link()
        : SpellScriptLoader("spell_yogg_saron_brain_link")
    {
    }
    ...
    class spell_yogg_saron_brain_link_AuraScript : public AuraScript {
        ...
        void HandleOnEffectApply(AuraEffect const* /*aurEff*/,
                                 AuraEffectHandleModes /*mode*/)
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                 itr != pList.end();
                 ++itr, ++_counter) {
                if (itr->GetSource() == GetUnitOwner() ||
                    GetUnitOwner()->GetDistance(itr->GetSource()) > 50.0f ||
                    !itr->GetSource()->IsAlive() ||
                    itr->GetSource()->IsGameMaster())
                    continue;
        ...
        void OnPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (!_target || !_target->IsAlive() ||
                std::fabs(owner->GetPositionZ() - _target->GetPositionZ()) >
                    10.0f) // Target or owner underground
            {
                SetDuration(0);
                return;
            }
        ...
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_yogg_saron_brain_link_AuraScript::HandleOnEffectApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_yogg_saron_brain_link_AuraScript::OnPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    ...
    class spell_yogg_saron_brain_link_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.clear();
            for (std::list<WorldObject*>::iterator itr = tempList.begin();
                 itr != tempList.end();
                 ++itr)
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_yogg_saron_brain_link_SpellScript::FilterTargets,
                EFFECT_0,
...
public:
    spell_yogg_saron_shadow_beacon()
        : SpellScriptLoader("spell_yogg_saron_shadow_beacon")
    {
    }
    ...
    class spell_yogg_saron_shadow_beacon_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_yogg_saron_shadow_beacon_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_yogg_saron_shadow_beacon_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_yogg_saron_destabilization_matrix()
        : SpellScriptLoader("spell_yogg_saron_destabilization_matrix")
    {
    }

    class spell_yogg_saron_destabilization_matrix_SpellScript
        : public SpellScript {
        ...
        void HandleDummyEffect(SpellEffIndex effIndex)
        {
            ...
            if (Unit* target = GetHitUnit())
                GetCaster()->CastSpell(
                    target, SPELL_DESTABILIZATION_MATRIX_ATTACK, false);
        }
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_yogg_saron_destabilization_matrix_SpellScript::
                    HandleDummyEffect,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
...
public:
    spell_yogg_saron_titanic_storm()
        : SpellScriptLoader("spell_yogg_saron_titanic_storm")
    {
    }
    ...
    class spell_yogg_saron_titanic_storm_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_yogg_saron_titanic_storm_SpellScript::HandleDummyEffect,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_yogg_saron_titanic_storm_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };
...
public:
    spell_yogg_saron_lunatic_gaze()
        : SpellScriptLoader("spell_yogg_saron_lunatic_gaze")
    {
    }
    ...
    class spell_yogg_saron_lunatic_gaze_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_yogg_saron_lunatic_gaze_SpellScript::FilterTargets,
                EFFECT_ALL,
                TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
public:
    spell_yogg_saron_protective_gaze()
        : SpellScriptLoader("spell_yogg_saron_protective_gaze")
    {
    }
    ...
    class spell_yogg_saron_protective_gaze_AuraScript : public AuraScript {
                    ...
                    uint32&     absorbAmount)
        {
            ...
            if (dmgInfo.GetDamage() < target->GetHealth() || !GetCaster() ||
                GetCaster()->ToCreature()->HasSpellCooldown(
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_yogg_saron_protective_gaze_AuraScript::CalculateAmount,
                EFFECT_0,
                SPELL_AURA_SCHOOL_ABSORB);
            OnEffectAbsorb += AuraEffectAbsorbFn(
                spell_yogg_saron_protective_gaze_AuraScript::Absorb, EFFECT_0);
        }
    };
...
public:
    spell_yogg_saron_empowered()
        : SpellScriptLoader("spell_yogg_saron_empowered")
    {
    }
    ...
    class spell_yogg_saron_empowered_AuraScript : public AuraScript {
        ...
        void OnPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            uint8 stack =
                std::min(uint8(target->GetHealthPct() / 10), (uint8)9);
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_yogg_saron_empowered_AuraScript::OnPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    ...
    class spell_yogg_saron_insane_periodic_trigger_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_yogg_saron_insane_periodic_trigger_SpellScript);
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            for (std::list<WorldObject*>::iterator itr = targets.begin();
                 ...
                 ++itr)
                if ((*itr)->IsPlayer() &&
                    !(*itr)->ToPlayer()->HasAuraType(SPELL_AURA_AOE_CHARM) &&
...
public:
    ...
    class spell_yogg_saron_insane_AuraScript : public AuraScript {
        ...
        {
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_yogg_saron_insane_AuraScript::OnRemove,
                                   EFFECT_0,
...
public:
    spell_yogg_saron_sanity_well()
        : SpellScriptLoader("spell_yogg_saron_sanity_well")
    {
    }
    ...
    class spell_yogg_saron_sanity_well_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
                spell_yogg_saron_sanity_well_AuraScript::
                    HandleEffectCalcPeriodic,
                EFFECT_0,
                SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_yogg_saron_sanity_well_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
                SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
        }
    };
...
public:
    spell_yogg_saron_sanity_reduce()
        : SpellScriptLoader("spell_yogg_saron_sanity_reduce")
    {
    }
    ...
    class spell_yogg_saron_sanity_reduce_SpellScript : public SpellScript {
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            case SPELL_INDUCE_MADNESS:
                ...
                if (target->GetPositionZ() > 300.0f)
                    ...
                    target->CastSpell(
                        ...
                        true); // else we are underground, remove illusion aura
                               // and teleport outside
                _reduceAmount = 100;
                break;
            }
            ...
            if (Aura* aur = target->GetAura(SPELL_SANITY)) {
                if ((aur->GetStackAmount() - _reduceAmount) <= 20)
                    target->CastSpell(
                        target, 63752 /*SANITY_SCREEN_EFFECT*/, true);
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_yogg_saron_sanity_reduce_SpellScript::HandleScriptEffect,
                EFFECT_FIRST_FOUND,
...
public:
    spell_yogg_saron_empowering_shadows()
        : SpellScriptLoader("spell_yogg_saron_empowering_shadows")
    {
    }
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            if (Unit* target = GetHitUnit())
                target->CastSpell(target,
                                  target->GetMap()->Is25ManRaid() ? 64486
                                                                  : 64468,
                                  true); // SPELL_EMPOWERING_SHADOWS_HEAL
        }

        void Register() override
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_yogg_saron_empowering_shadows_SpellScript::
                                  HandleScriptEffect,
                              EFFECT_0,
                              SPELL_EFFECT_SCRIPT_EFFECT);
        }
...
public:
    spell_yogg_saron_in_the_maws_of_the_old_god()
        : SpellScriptLoader("spell_yogg_saron_in_the_maws_of_the_old_god")
    {
    }

    class spell_yogg_saron_in_the_maws_of_the_old_god_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_yogg_saron_in_the_maws_of_the_old_god_SpellScript);
...
public:
    spell_yogg_saron_target_selectors()
        : SpellScriptLoader("spell_yogg_saron_target_selectors")
    {
    }
    ...
    class spell_yogg_saron_target_selectors_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_yogg_saron_target_selectors_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_yogg_saron_grim_reprisal()
        : SpellScriptLoader("spell_yogg_saron_grim_reprisal")
    {
    }
    ...
    class spell_yogg_saron_grim_reprisal_AuraScript : public AuraScript {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            int32 damage =
                CalculatePct(static_cast<int32>(damageInfo->GetDamage()), 60);
            GetTarget()->CastCustomSpell(SPELL_GRIM_REPRISAL_DAMAGE,
        ...
        void Register() override
        {
            OnEffectProc += AuraEffectProcFn(
                spell_yogg_saron_grim_reprisal_AuraScript::HandleProc,
                EFFECT_0,
                SPELL_AURA_DUMMY);
        }
    };
...
public:
    achievement_yogg_saron_drive_me_crazy()
        : AchievementCriteriaScript("achievement_yogg_saron_drive_me_crazy")
    {
    }
    ...
    {
        return target && target->GetAI()->GetData(
                             DATA_GET_DRIVE_ME_CRAZY); // target = Yogg-Saron
    }
};

class achievement_yogg_saron_darkness : public AchievementCriteriaScript {
public:
    achievement_yogg_saron_darkness(char const* name, uint32 count)
        : AchievementCriteriaScript(name), _keepersCount(count)
    {
    }

    bool OnCheck(Player* player,
                 Unit* /*target*/ /*Yogg-Saron*/,
                 uint32 /*criteria_id*/) override
    {
        if (player->GetInstanceScript())
            if (Creature* sara = ObjectAccessor::GetCreature(
                    *player,
                    player->GetInstanceScript()->GetGuidData(NPC_SARA)))
                return sara->GetAI()->GetData(DATA_GET_KEEPERS_COUNT) <=
                       _keepersCount;
...
public:
    achievement_yogg_saron_he_waits_dreaming(char const* name, uint8 illusion)
        : AchievementCriteriaScript(name), _requiredIllusion(illusion)
    {
    }
                 ...
                 uint32 /*criteria_id*/) override
    {
        if (player->GetInstanceScript())
            if (Creature* sara = ObjectAccessor::GetCreature(
                    *player,
                    player->GetInstanceScript()->GetGuidData(
                        NPC_BRAIN_OF_YOGG_SARON)))
                return sara->GetAI()->GetData(DATA_GET_CURRENT_ILLUSION) ==
                       _requiredIllusion;
...
class achievement_yogg_saron_kiss_and_make_up
    : public AchievementCriteriaScript {
public:
    achievement_yogg_saron_kiss_and_make_up()
        : AchievementCriteriaScript("achievement_yogg_saron_kiss_and_make_up")
    {
    }
    ...
    bool OnCheck(Player* /*player*/,
                 ...
                 uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_SARA && target->GetAI() &&
               target->GetAI()->GetData(DATA_GET_SARA_PHASE);
    }
};
...
void AddSC_boss_yoggsaron()
{
    ...
    new achievement_yogg_saron_darkness(
        "achievement_yogg_saron_three_lights_in_the_darkness", 3);
    new achievement_yogg_saron_darkness(
        "achievement_yogg_saron_two_lights_in_the_darkness", 2);
    new achievement_yogg_saron_darkness(
        "achievement_yogg_saron_one_light_in_the_darkness", 1);
    new achievement_yogg_saron_darkness(
        "achievement_yogg_saron_alone_in_the_darkness", 0);
    new achievement_yogg_saron_he_waits_dreaming(
        "achievement_yogg_saron_he_waits_dreaming_stormwind",
        ACTION_ILLUSION_STORMWIND);
    new achievement_yogg_saron_he_waits_dreaming(
        "achievement_yogg_saron_he_waits_dreaming_chamber",
        ACTION_ILLUSION_DRAGONS);
    new achievement_yogg_saron_he_waits_dreaming(
        "achievement_yogg_saron_he_waits_dreaming_icecrown",
