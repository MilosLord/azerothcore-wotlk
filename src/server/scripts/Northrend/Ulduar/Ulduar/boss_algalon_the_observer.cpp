...
enum EncounterPhases {
    ...
    PHASE_MASK_NO_UPDATE =
        (1 << (PHASE_ROLE_PLAY - 1)) | (1 << (PHASE_BIG_BANG - 1)),
...
Position const BrannIntroSpawnPos = {
    1676.277f, -162.5308f, 427.3326f, 3.235537f};
...
public:
    ...
    struct boss_algalon_the_observerAI : public ScriptedAI {
        ...
        {
            uint8 _count = 0;
            for (SummonList::const_iterator i = summons.begin();
                 i != summons.end();) {
                Creature* summon = ObjectAccessor::GetCreature(*me, *i++);
                if (summon && summon->GetEntry() == NPC_LIVING_CONSTELLATION &&
                    !summon->AI()->GetData(0)) {
                    ++_count;
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_START_INTRO: {
                ...
                me->GetMotionMaster()->MovePoint(POINT_ALGALON_LAND,
                                                 AlgalonLandPos);
                me->SetHomePosition(AlgalonLandPos);
                Movement::MoveSplineInit init(me);
                init.MoveTo(AlgalonLandPos.GetPositionX(),
                            AlgalonLandPos.GetPositionY(),
                            AlgalonLandPos.GetPositionZ());
                ...
                events.ScheduleEvent(
                    EVENT_INTRO_FINISH, 36s, 0, PHASE_ROLE_PLAY);
                break;
            }
        ...
        void JustEngagedWith(Unit*) override
        {
            ...
            events.ScheduleEvent(EVENT_SUMMON_COLLAPSING_STAR,
                                 16500 + introDelay);
            events.ScheduleEvent(EVENT_COSMIC_SMASH, 25000 + introDelay);
            events.ScheduleEvent(EVENT_ACTIVATE_LIVING_CONSTELLATION,
                                 50500 + introDelay);
            events.ScheduleEvent(EVENT_BIG_BANG, 90000 + introDelay);
            events.ScheduleEvent(EVENT_ASCEND_TO_THE_HEAVENS,
                                 360000 + introDelay);
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            case NPC_BLACK_HOLE:
                summon->CastSpell(
                    (Unit*)nullptr, SPELL_BLACK_HOLE_TRIGGER, true);
                summon->CastSpell(
                    summon, SPELL_CONSTELLATION_PHASE_TRIGGER, true);
                summon->CastSpell(
                    (Unit*)nullptr, SPELL_BLACK_HOLE_EXPLOSION, false);
            ...
            case NPC_ALGALON_STALKER_ASTEROID_TARGET_01:
                summon->CastSpell(
                    summon, SPELL_COSMIC_SMASH_VISUAL_STATE, true);
                break;
            case NPC_ALGALON_STALKER_ASTEROID_TARGET_02: {
                ...
                summon->m_Events.AddEvent(new CosmicSmashDamageEvent(summon),
                                          summon->m_Events.CalculateTime(4000));
                break;
            }
                         ...
                         SpellSchoolMask) override
        {
            ...
            if (!_phaseTwo && me->HealthBelowPctDamaged(20, damage)) {
                ...
                for (uint32 i = 0; i < COLLAPSING_STAR_COUNT; ++i)
                    me->SummonCreature(NPC_WORM_HOLE,
                                       CollapsingStarPos[i],
        ...
        bool IsInRoom()
        {
            if (me->GetExactDist2d(&me->GetHomePosition()) > 45.f ||
                me->GetPositionZ() < 410.f) {
                DoAction(ACTION_ASCEND);
                return false;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_INTRO_FINISH:
                ...
                if (Creature* brann = ObjectAccessor::GetCreature(
                        *me,
                        m_pInstance->GetGuidData(NPC_BRANN_BRONZBEARD_ALG)))
            ...
            case EVENT_INTRO_TIMER_DONE:
                ...
                // Hack: _IsValidTarget failed earlier due to flags, call
                // AttackStart again
                ...
                for (uint32 i = 0; i < LIVING_CONSTELLATION_COUNT; ++i)
                    me->SummonCreature(NPC_LIVING_CONSTELLATION,
                                       ConstellationPos[i],
            ...
            case EVENT_SUMMON_COLLAPSING_STAR:
                ...
                for (uint8 i = 0; i < COLLAPSING_STAR_COUNT; ++i)
                    me->SummonCreature(NPC_COLLAPSING_STAR,
                                       CollapsingStarPos[i],
            ...
            case EVENT_COSMIC_SMASH:
                ...
                me->CastCustomSpell(SPELL_COSMIC_SMASH,
            ...
            case EVENT_ASCEND_TO_THE_HEAVENS:
                ...
                me->CastSpell(
                    (Unit*)nullptr, SPELL_ASCEND_TO_THE_HEAVENS, false);
            ...
            case EVENT_OUTRO_2: {
                ...
                me->GetMotionMaster()->MovePoint(POINT_ALGALON_OUTRO,
                                                 AlgalonOutroPos);
                break;
            }
            case EVENT_OUTRO_3:
                ...
                if (GameObject* go = me->SummonGameObject(
                        RAID_MODE(GO_ALGALON_CHEST, GO_ALGALON_CHEST_HERO),
                        1632.1f,
                        -306.561f,
                        417.321f,
                        4.69494f,
            ...
            case EVENT_OUTRO_5:
                if (Creature* brann =
                        me->SummonCreature(NPC_BRANN_BRONZBEARD_ALG,
                                           BrannOutroPos[0],
...
public:
    npc_brann_bronzebeard_algalon()
        : CreatureScript("npc_brann_bronzebeard_algalon")
    {
    }
    ...
    struct npc_brann_bronzebeard_algalonAI : public CreatureAI {
        ...
        void DoAction(int32 action) override
        {
            ...
            case ACTION_FINISH_INTRO:
                ...
                break;
            case ACTION_OUTRO:
                me->GetMotionMaster()->MovePoint(POINT_BRANN_OUTRO,
                                                 BrannOutroPos[1]);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_BRANN_MOVE_INTRO:
                if (_currentPoint < MAX_BRANN_WAYPOINTS_INTRO)
                    me->GetMotionMaster()->MovePoint(
                        _currentPoint, BrannIntroWaypoint[_currentPoint]);
                break;
            case EVENT_SUMMON_ALGALON:
                if (me->GetInstanceScript() &&
                    !me->GetInstanceScript()->GetGuidData(TYPE_ALGALON))
                    if (Creature* algalon = me->GetMap()->SummonCreature(
                            NPC_ALGALON, AlgalonSummonPos))
            ...
            case EVENT_BRANN_OUTRO_2:
                me->GetMotionMaster()->MovePoint(POINT_BRANN_OUTRO_END,
                                                 BrannOutroPos[2]);
                break;
            }
        }
    ...
    struct npc_collapsing_starAI : public NullCreatureAI {
        ...
        void JustSummoned(Creature* summon) override
        {
            if (TempSummon* summ = me->ToTempSummon())
                if (Creature* algalon = ObjectAccessor::GetCreature(
                        *me, summ->GetSummonerGUID()))
...
public:
    ...
    struct npc_living_constellationAI : public ScriptedAI {
        ...
        uint32 GetData(uint32 /*param*/) const override { return _isActive; }
        ...
        {
            if (spell->Id != SPELL_CONSTELLATION_PHASE_EFFECT ||
            ...
            caster->CastSpell(
                (Unit*)nullptr, SPELL_BLACK_HOLE_CREDIT, TRIGGERED_FULL_MASK);
            caster->ToCreature()->DespawnOrUnsummon(1);
            me->DespawnOrUnsummon(1);
            if (Creature* voidZone = caster->FindNearestCreature(
                    NPC_ALGALON_VOID_ZONE_VISUAL_STALKER, 10.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ARCANE_BARRAGE:
                me->CastCustomSpell(SPELL_ARCANE_BARRAGE,
                                    SPELLVALUE_MAX_TARGETS,
                                    1,
                                    (Unit*)nullptr,
                                    true);
...
public:
    ...
    struct npc_algalon_worm_holeAI : public NullCreatureAI {
        ...
        void Reset() override { _summonTimer = urand(22000, 24000); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_summonTimer >= 30000) {
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SUMMON_UNLEASHED_DARK_MATTER, true);
                _summonTimer = 0;
            }
        }
    };
...
public:
    ...
    struct go_celestial_planetarium_accessAI : public GameObjectAI {
        ...
        {
            bool hasKey = true;
            if (LockEntry const* lock =
                    sLockStore.LookupEntry(me->GetGOInfo()->goober.lockId)) {
                hasKey = false;
            ...
            if (Creature* brann = me->SummonCreature(NPC_BRANN_BRONZBEARD_ALG,
                                                     BrannIntroSpawnPos))
                brann->AI()->DoAction(ACTION_START_INTRO);

            if (InstanceScript* instance = me->GetInstanceScript()) {
                instance->SetData(DATA_ALGALON_SUMMON_STATE, 1);
                if (GameObject* sigil = ObjectAccessor::GetGameObject(
                        *me, instance->GetGuidData(GO_DOODAD_UL_SIGILDOOR_01)))
                    sigil->SetGoState(GO_STATE_ACTIVE);

                if (GameObject* sigil = ObjectAccessor::GetGameObject(
                        *me, instance->GetGuidData(GO_DOODAD_UL_SIGILDOOR_02)))
                    sigil->SetGoState(GO_STATE_ACTIVE);
    ...
    class spell_algalon_phase_punch_AuraScript : public AuraScript {
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (GetStackAmount() != 1)
                GetTarget()->RemoveAurasDueToSpell(
                    PhasePunchAlphaId[GetStackAmount() - 2]);
            GetTarget()->CastSpell(GetTarget(),
                                   PhasePunchAlphaId[GetStackAmount() - 1],
                                   TRIGGERED_FULL_MASK);
            if (GetStackAmount() == 5)
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_algalon_phase_punch_AuraScript::HandlePeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_algalon_phase_punch_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
...
public:
    ...
    class spell_algalon_collapse_AuraScript : public AuraScript {
        ...
        {
            PreventDefaultAction();
            Unit::DealDamage(GetTarget(),
                             GetTarget(),
                             GetTarget()->CountPctFromMaxHealth(1),
                             nullptr,
                             NODAMAGE);
        }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_algalon_collapse_AuraScript::HandlePeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_algalon_trigger_3_adds()
        : SpellScriptLoader("spell_algalon_trigger_3_adds")
    {
    }
    ...
    class spell_algalon_trigger_3_adds_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_algalon_trigger_3_adds_SpellScript::SelectTarget,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
            OnEffectHitTarget += SpellEffectFn(
                spell_algalon_trigger_3_adds_SpellScript::HandleDummyEffect,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_algalon_cosmic_smash_damage()
        : SpellScriptLoader("spell_algalon_cosmic_smash_damage")
    {
    }
    ...
    class spell_algalon_cosmic_smash_damage_SpellScript : public SpellScript {
        ...
        void RecalculateDamage()
        {
            ...
            float distance = GetHitUnit()->GetDistance2d(
                GetExplTargetDest()->GetPositionX(),
                GetExplTargetDest()->GetPositionY());
            if (distance >= 10.0f)
        ...
        void Register() override
        {
            OnHit += SpellHitFn(spell_algalon_cosmic_smash_damage_SpellScript::
                                    RecalculateDamage);
        }
    };
...
public:
    ...
    class spell_algalon_big_bang_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            ...
            AfterCast +=
                SpellCastFn(spell_algalon_big_bang_SpellScript::CheckTargets);
        }
...
public:
    spell_algalon_remove_phase()
        : SpellScriptLoader("spell_algalon_remove_phase")
    {
    }
    ...
    class spell_algalon_remove_phase_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_algalon_remove_phase_AuraScript::HandlePeriodic,
                EFFECT_0,
...
public:
    spell_algalon_supermassive_fail()
        : SpellScriptLoader("spell_algalon_supermassive_fail")
    {
    }
    ...
    class spell_algalon_supermassive_fail_SpellScript : public SpellScript {
        ...
        void RecalculateDamage()
        {
            ...
            GetHitPlayer()->ResetAchievementCriteria(
        ...
        {
            OnHit += SpellHitFn(
                spell_algalon_supermassive_fail_SpellScript::RecalculateDamage);
        }
    };
...
public:
    ...
    bool
    OnCheck(Player*, Unit* target /*Algalon*/, uint32 /*criteria_id*/) override
    {
...
class achievement_algalon_herald_of_the_titans
    : public AchievementCriteriaScript {
public:
    achievement_algalon_herald_of_the_titans()
        : AchievementCriteriaScript("achievement_algalon_herald_of_the_titans")
    {
    }
