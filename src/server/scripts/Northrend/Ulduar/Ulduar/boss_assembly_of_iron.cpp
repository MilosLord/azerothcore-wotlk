...
#define SPELL_HIGH_VOLTAGE                                                     \
    RAID_MODE(SPELL_HIGH_VOLTAGE_10, SPELL_HIGH_VOLTAGE_25)
#define SPELL_FUSION_PUNCH                                                     \
    RAID_MODE(SPELL_FUSION_PUNCH_10, SPELL_FUSION_PUNCH_25)
#define SPELL_STATIC_DISRUPTION                                                \
    RAID_MODE(SPELL_STATIC_DISRUPTION_10, SPELL_STATIC_DISRUPTION_25)
#define SPELL_OVERWHELMING_POWER                                               \
    RAID_MODE(SPELL_OVERWHELMING_POWER_10, SPELL_OVERWHELMING_POWER_25)
#define SPELL_SHIELD_OF_RUNES                                                  \
    RAID_MODE(SPELL_SHIELD_OF_RUNES_10, SPELL_SHIELD_OF_RUNES_25)
#define SPELL_RUNE_OF_DEATH                                                    \
    RAID_MODE(SPELL_RUNE_OF_DEATH_10, SPELL_RUNE_OF_DEATH_25)
#define SPELL_LIGHTNING_BLAST                                                  \
...
#define SPELL_LIGHTNING_WHIRL                                                  \
    RAID_MODE(SPELL_LIGHTNING_WHIRL_10, SPELL_LIGHTNING_WHIRL_25)
#define SPELL_LIGHTNING_TENDRILS                                               \
    RAID_MODE(SPELL_LIGHTNING_TENDRILS_10, SPELL_LIGHTNING_TENDRILS_25)
...
enum Misc { ACTION_ADD_CHARGE = 1, POINT_CHANNEL_STEELBREAKER = 1 };
...
public:
    ...
    struct boss_steelbreakerAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            if (Creature* boss = ObjectAccessor::GetCreature(
                    *me,
                    pInstance->GetGuidData(DATA_STEELBREAKER + urand(0, 2)))) {
            ...
            for (uint8 i = 0; i < 3; ++i)
                if (Creature* boss = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(DATA_STEELBREAKER + i)))
                    if (!boss->IsInCombat())
        ...
        void JustDied(Unit* /*Killer*/) override
        {
            ...
            if (IsEncounterComplete(pInstance, me)) {
                ...
                RestoreAssemblyHealth(pInstance->GetGuidData(DATA_BRUNDIR),
                                      pInstance->GetGuidData(DATA_MOLGEIM),
                                      me);
...
public:
    ...
    struct boss_runemaster_molgeimAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            me->m_Events.AddEvent(new CastRunesEvent(*me),
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (IsEncounterComplete(pInstance, me)) {
                ...
                RestoreAssemblyHealth(pInstance->GetGuidData(DATA_STEELBREAKER),
...
public:
    ...
    struct boss_stormcaller_brundirAI : public ScriptedAI {
        ...
        void SpellHitTarget(Unit* /*target*/,
                            SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_CHAIN_LIGHTNING ||
                spellInfo->Id ==
                    uint32(
                        RAID_MODE(61916, 63482))) // Lightning Whirl triggered
        ...
        void MovementInform(uint32 type, uint32 point) override
        {
            if (type == POINT_MOTION_TYPE &&
                point == POINT_CHANNEL_STEELBREAKER)
        ...
        void UpdateAI(uint32 diff) override
        {
            if (!me->IsInCombat() && me->GetReactState() == REACT_AGGRESSIVE) {
                ...
                if (_channelTimer >= 10000) {
                    ...
                    me->InterruptNonMeleeSpells(false);
                    me->GetMotionMaster()->MovePoint(POINT_CHANNEL_STEELBREAKER,
                                                     1587.18f + 10.0f * cos(o),
                                                     121.02f +
                                                         10.0f * std::sin(o),
                                                     427.3f);
                }
            ...
            case EVENT_LIGHTNING_LAND: {
                float speed = me->GetDistance(me->GetPositionX(),
                                              me->GetPositionY(),
                                              me->GetPositionZ()) /
                              (1000.0f * 0.001f);
            ...
            case EVENT_LAND_LAND:
                ...
                me->SetDisableGravity(false);
                if (Unit* flyTarget =
                        ObjectAccessor::GetUnit(*me, _flyTargetGUID)) {
...
public:
    ...
    class spell_shield_of_runes_AuraScript : public AuraScript {
        ...
        {
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_shield_of_runes_AuraScript::OnRemove,
                                   EFFECT_0,
    ...
    class spell_assembly_meltdown_SpellScript : public SpellScript {
        ...
        void HandleInstaKill(SpellEffIndex /*effIndex*/)
        {
            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                if (Creature* Steelbreaker = ObjectAccessor::GetCreature(
                        *GetCaster(), instance->GetGuidData(DATA_STEELBREAKER)))
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_assembly_meltdown_SpellScript::HandleInstaKill,
                EFFECT_1,
...
public:
    spell_assembly_rune_of_summoning()
        : SpellScriptLoader("spell_assembly_rune_of_summoning")
    {
    }
    ...
    class spell_assembly_rune_of_summoning_AuraScript : public AuraScript {
        ...
        void OnPeriodic(AuraEffect const* aurEff)
        {
            ...
            if (aurEff->GetTickNumber() % 2 == 0)
                GetTarget()->CastSpell(
                    ...
                    aurEff,
                    GetTarget()->IsSummon()
                        ? GetTarget()->ToTempSummon()->GetSummonerGUID()
                        : ObjectGuid::Empty);
        }
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_assembly_rune_of_summoning_AuraScript::OnPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_assembly_rune_of_summoning_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    achievement_assembly_of_iron(char const* name, uint32 entry)
        : AchievementCriteriaScript(name), _targetEntry(entry)
    {
    }
    ...
    {
        return target && target->GetAuraCount(SPELL_SUPERCHARGE) >= 2 &&
               (!_targetEntry || target->GetEntry() == _targetEntry);
    }
...
public:
    achievement_cant_do_that_while_stunned()
        : AchievementCriteriaScript("achievement_cant_do_that_while_stunned")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        ...
        if (InstanceScript* instance = target->GetInstanceScript())
            if (Creature* cr = ObjectAccessor::GetCreature(
                    *target, instance->GetGuidData(DATA_BRUNDIR)))
...
void AddSC_boss_assembly_of_iron()
{
    ...
    new achievement_assembly_of_iron("achievement_assembly_steelbreaker",
                                     NPC_STEELBREAKER);
    new achievement_assembly_of_iron("achievement_assembly_runemaster",
                                     NPC_MOLGEIM);
    new achievement_assembly_of_iron("achievement_assembly_stormcaller",
                                     NPC_BRUNDIR);
