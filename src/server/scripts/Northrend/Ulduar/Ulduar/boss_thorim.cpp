...
#define SPELL_GREATER_HEAL                                                     \
    RAID_MODE(SPELL_GREATER_HEAL_10, SPELL_GREATER_HEAL_25)
...
#define SPELL_COLOSSUS_CHARGE                                                  \
    RAID_MODE(SPELL_COLOSSUS_CHARGE_10, SPELL_COLOSSUS_CHARGE_25)
#define SPELL_STOMP RAID_MODE(SPELL_STOMP_10, SPELL_STOMP_25)
#define SPELL_SHIELD_SMASH                                                     \
    RAID_MODE(SPELL_SHIELD_SMASH_10, SPELL_SHIELD_SMASH_25)
#define SPELL_RUNIC_LIGHTNING                                                  \
    RAID_MODE(SPELL_RUNIC_LIGHTNING_10, SPELL_RUNIC_LIGHTNING_25)
#define SPELL_RUNIC_MENDING                                                    \
    RAID_MODE(SPELL_RUNIC_MENDING_10, SPELL_RUNIC_MENDING_25)
#define SPELL_RUNIC_SHIELD                                                     \
    RAID_MODE(SPELL_RUNIC_SHIELD_10, SPELL_RUNIC_SHIELD_25)
#define SPELL_CHAIN_LIGHTNING                                                  \
    RAID_MODE(SPELL_CHAIN_LIGHTNING_10, SPELL_CHAIN_LIGHTNING_25)
...
const Position ArenaNPCs[] = {{2178.5f, -300.2f, 441.97f, 2.5f},
...
public:
    ...
    struct boss_thorimAI : public ScriptedAI {
        ...
        {
            if (m_pInstance)
                return ObjectAccessor::GetGameObject(
                    *me, m_pInstance->GetGuidData(entry));
            return nullptr;
        }
        ...
        void SpawnAllNPCs()
        {
            ...
            me->SummonCreature(NPC_JORMUNGAR_BEHEMOT,
                               2149.68f,
                               -263.477f,
                               419.679f,
                               3.12102f,
            ...
            me->SummonCreature(_isAlly ? NPC_CAPTURED_MERCENARY_SOLDIER_ALLY
            ...
            me->SummonCreature(_isAlly ? NPC_CAPTURED_MERCENARY_CAPTAIN_ALLY
            ...
            me->SummonCreature(NPC_DARK_RUNE_ACOLYTE_I,
                               2129.09f,
                               -277.142f,
                               419.756f,
                               1.22173f,
            ...
            me->SummonCreature(
                NPC_IRON_RING_GUARD, 2217.69f, -337.394f, 412.177f, 1.23918f);
            me->SummonCreature(
                NPC_IRON_RING_GUARD, 2218.38f, -297.505f, 412.176f, 1.02974f);
            me->SummonCreature(
                NPC_IRON_RING_GUARD, 2235.26f, -338.345f, 412.134f, 1.58979f);
            me->SummonCreature(
                NPC_IRON_RING_GUARD, 2235.07f, -297.985f, 412.134f, 1.61336f);

            // Dark Rune Acolyte (gauntlet) 33110
            me->SummonCreature(NPC_DARK_RUNE_ACOLYTE_G,
                               2198.29f,
                               -436.92f,
                               419.985f,
                               0.261799f);
            ...
            me->SummonCreature(
                NPC_IRON_HONOR_GUARD, 2198.05f, -428.769f, 419.985f, 6.05629f);
            me->SummonCreature(
                NPC_IRON_HONOR_GUARD, 2220.31f, -436.22f, 412.26f, 1.06465f);

            // Runic Colossus 32872
            me->SummonCreature(
                NPC_RUNIC_COLOSSUS, 2227.5f, -396.179f, 412.176f, 1.79769f);

            // Ancient Rune Giant 32873
            me->SummonCreature(NPC_ANCIENT_RUNE_GIANT,
                               2134.57f,
                               -440.318f,
                               438.331f,
                               0.226893f);
        ...
        {
            if (who && _isHitAllowed && who->GetPositionZ() > 430 &&
                ...
                events.ScheduleEvent(
                    EVENT_THORIM_UNBALANCING_STRIKE, 8s, 0, EVENT_PHASE_RING);
                events.ScheduleEvent(EVENT_THORIM_LIGHTNING_CHARGE,
                                     12s + 500ms,
                                     0,
                                     EVENT_PHASE_RING);
                events.ScheduleEvent(
                    EVENT_THORIM_CHAIN_LIGHTNING, 13s, 0, EVENT_PHASE_RING);
                events.ScheduleEvent(
                    EVENT_THORIM_BERSERK, 5min, 0, EVENT_PHASE_RING);

                me->GetMotionMaster()->MoveChase(me->GetVictim());
                me->GetMotionMaster()->MoveJump(Middle.GetPositionX(),
                                                Middle.GetPositionY(),
                                                Middle.GetPositionZ(),
                                                20,
                                                20);
                ...
                if (!me->HasAura(62565 /*TOUCH OF DOMINION TRIGGER*/)) {
                    if (m_pInstance)
                        m_pInstance->DoUpdateAchievementCriteria(
                            ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET,
                            64980 /*SIFFED ACHIEVEMENT*/);
            ...
            if (damage >= me->GetHealth()) {
                ...
                if (!_encounterFinished) {
                    ...
                    uint32 chestId = me->GetMap()->Is25ManRaid()
                                         ? GO_THORIM_CHEST_HERO
                                         : GO_THORIM_CHEST;
                    if (_hardMode)
                        chestId += 1; // hard mode offset

                    if ((go = me->SummonGameObject(chestId,
                                                   2134.73f,
                                                   -286.32f,
                                                   419.51f,
                                                   4.65f,
        ...
        void SpawnArenaNPCs()
        {
            ...
            for (uint8 i = 0; i < (_spawnCommoners ? 7 : 2); ++i) {
                rnd = urand(0, 13);
                if ((cr = me->SummonCreature((_spawnCommoners
                    ...
                    cr->GetMotionMaster()->MoveJump(
                        Middle.GetPositionX() +
                            urand(19, 24) * cos(Middle.GetAngle(cr)),
                        Middle.GetPositionY() +
                            urand(19, 24) * std::sin(Middle.GetAngle(cr)),
                        Middle.GetPositionZ(),
                        20,
                        20);
            }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_LIGHTNING_ORB_CHARGER) {
                ...
                events.RescheduleEvent(
                    EVENT_THORIM_LIGHTNING_CHARGE, 10s, 0, EVENT_PHASE_RING);
            }
        }
        ...
        Player* GetArenaPlayer()
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                ...
                if (Player* p = itr->GetSource())
                    if (p->GetPositionX() > 2085 && p->GetPositionX() < 2185 &&
                        p->GetPositionY() < -214 && p->GetPositionY() > -305 &&
                        p->IsAlive() && p->GetPositionZ() < 425)
                        return p;
            return nullptr;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_THORIM_START_PHASE1: {
                events.ScheduleEvent(
                    EVENT_THORIM_STORMHAMMER, 8s, 0, EVENT_PHASE_START);
                events.ScheduleEvent(
                    EVENT_THORIM_CHARGE_ORB, 14s, 0, EVENT_PHASE_START);
                events.ScheduleEvent(
                    EVENT_THORIM_FILL_ARENA, 0ms, 0, EVENT_PHASE_START);
                events.ScheduleEvent(
                    EVENT_THORIM_LIGHTNING_ORB,
                    5s,
                    0,
                    EVENT_PHASE_START); // checked every 5 secs if there are
                                        // players on arena
                events.ScheduleEvent(
                    EVENT_THORIM_NOT_REACH_IN_TIME, 5min, 0, EVENT_PHASE_START);
            ...
            case EVENT_THORIM_STORMHAMMER:
                me->CastCustomSpell(SPELL_STORMHAMMER,
            ...
            case EVENT_THORIM_CHARGE_ORB:
                me->CastCustomSpell(
                    SPELL_CHARGE_ORB, SPELLVALUE_MAX_TARGETS, 1, me, false);
                events.Repeat(16s);
                break;
            case EVENT_THORIM_LIGHTNING_ORB: {
                ...
                // No players found
                Talk(SAY_WIPE);
                me->SummonCreature(NPC_LIGHTNING_ORB,
                                   me->GetPositionX(),
                                   me->GetPositionY(),
                                   me->GetPositionZ());
    ...
    struct boss_thorim_sifAI : public ScriptedAI {
            ...
            if (param == ACTION_SIF_START_TALK)
                ...
                if (me->GetInstanceScript())
                    if (Creature* cr = ObjectAccessor::GetCreature(
                            *me,
                            me->GetInstanceScript()->GetGuidData(TYPE_THORIM)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SIF_BLIZZARD:
                me->SummonCreature(NPC_SIF_BLIZZARD,
                                   2108.7f,
                                   -280.04f,
                                   419.42f,
                                   0,
            ...
            case EVENT_SIF_FROST_NOVA_START:
                me->NearTeleportTo(2108 + urand(0, 42),
                                   -238 - irand(0, 46),
                                   420.02f,
                                   me->GetAngle(&Middle));
                events.Repeat(20s);
...
public:
    ...
    struct boss_thorim_lightning_orbAI : public npc_escortAI {
        boss_thorim_lightning_orbAI(Creature* pCreature)
...
public:
    ...
    struct boss_thorim_sif_blizzardAI : public npc_escortAI {
        boss_thorim_sif_blizzardAI(Creature* pCreature)
            : npc_escortAI(pCreature)
        {
            InitWaypoint();
            Reset();
        ...
        void Reset() override
        {
            ...
            me->CastSpell(
                me, RAID_MODE(SPELL_BLIZZARD_10, SPELL_BLIZZARD_25), true);
        }
    ...
    struct boss_thorim_pillarAI : public NullCreatureAI {
        ...
        void SpellHit(Unit*, SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_CHARGE_ORB)
                ...
                if (Creature* cr =
                        me->FindNearestCreature(NPC_THUNDER_ORB, 100))
    ...
    struct boss_thorim_start_npcsAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (me->GetEntry() != NPC_JORMUNGAR_BEHEMOT)
                if (Creature* cr =
                        me->FindNearestCreature(NPC_JORMUNGAR_BEHEMOT, 30.0f))
                    AttackStart(cr);
        }
        ...
        void DamageTaken(Unit* who,
                         ...
                         SpellSchoolMask) override
        {
            if (!_playerAttack && who &&
                (who->GetTypeId() == TYPEID_PLAYER ||
                 who->GetOwnerGUID().IsPlayer())) {
                if (me->GetInstanceScript())
                    if (Creature* thorim = ObjectAccessor::GetCreature(
                            *me,
        ...
        void JustDied(Unit*) override
        {
            if (me->GetInstanceScript())
                if (Creature* thorim = ObjectAccessor::GetCreature(
                        *me, me->GetInstanceScript()->GetGuidData(TYPE_THORIM)))
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            else if (me->GetEntry() == NPC_CAPTURED_MERCENARY_SOLDIER_ALLY ||
                     me->GetEntry() == NPC_CAPTURED_MERCENARY_SOLDIER_HORDE) {
            ...
            else if (me->GetEntry() == NPC_CAPTURED_MERCENARY_CAPTAIN_ALLY ||
                     me->GetEntry() == NPC_CAPTURED_MERCENARY_CAPTAIN_HORDE) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!_isCaster ||
                (me->GetPower(POWER_MANA) * 100 / me->GetMaxPower(POWER_MANA) <
                 10))
                DoMeleeAttackIfReady();
        }
    };
    ...
    struct boss_thorim_gauntlet_npcsAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            if (me->GetEntry() == NPC_IRON_RING_GUARD) {
                ...
                if (Creature* runeGiant =
                        me->FindNearestCreature(NPC_ANCIENT_RUNE_GIANT, 200.0f))
...
public:
    ...
    struct boss_thorim_runic_colossusAI : public ScriptedAI {
        boss_thorim_runic_colossusAI(Creature* pCreature)
        ...
        void SpellHit(Unit*, SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_RUNIC_SMASH_LEFT ||
                spellInfo->Id == SPELL_RUNIC_SMASH_RIGHT) {
        ...
        void RunRunicSmash(bool cast)
        {
            if (Creature* cr = ObjectAccessor::GetCreature(
                    *me,
                    _leftHand ? _triggerLeftGUID[0] : _triggerRightGUID[0])) {
                if (cast)
                    cr->CastSpell(cr, SPELL_RUNIC_SMASH_DAMAGE, true);
                cr->SetPosition(_leftHand ? 2235.0f : 2221.0f,
                                _nextTriggerPos,
                                cr->GetPositionZ(),
                                0.0f);
            ...
            if (Creature* cr = ObjectAccessor::GetCreature(
                    *me,
                    _leftHand ? _triggerLeftGUID[1] : _triggerRightGUID[1])) {
                if (cast)
                    cr->CastSpell(cr, SPELL_RUNIC_SMASH_DAMAGE, true);
                cr->SetPosition(_leftHand ? 2246.0f : 2210.0f,
                                _nextTriggerPos,
                                cr->GetPositionZ(),
                                0.0f);
                cr->StopMovingOnCurrentPos();
...
public:
    boss_thorim_ancient_rune_giant()
        : CreatureScript("boss_thorim_ancient_rune_giant")
    {
    }
    ...
    struct boss_thorim_ancient_rune_giantAI : public ScriptedAI {
        ...
        void JustDied(Unit*) override
        {
            if (InstanceScript* pInstance = me->GetInstanceScript()) {
                if (GameObject* go = ObjectAccessor::GetGameObject(
                        *me, pInstance->GetGuidData(DATA_THORIM_SECOND_DOORS)))
                    go->SetGoState(GO_STATE_ACTIVE);

                if (Creature* thorim = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(TYPE_THORIM)))
...
public:
    ...
    struct boss_thorim_arena_npcsAI : public ScriptedAI {
        ...
        {
            return target->GetPositionX() < 2180 &&
                   target->GetPositionZ() < 425;
        }
        ...
        bool SelectT()
        {
            ...
            for (Map::PlayerList::const_iterator itr = pList.begin();
                 itr != pList.end();
                 ++itr, ++count) {
                if (itr->GetSource()->GetPositionX() > 2180 ||
                    !itr->GetSource()->IsAlive() ||
                    itr->GetSource()->GetPositionZ() > 425)
                    continue;
...
public:
    ...
    {
        if (GameObject* g =
                pPlayer->FindNearestGameObject(GO_ARENA_LEVER_GATE, 50))
            g->UseDoorOrButton();
...
public:
    spell_thorim_lightning_pillar_P2()
        : SpellScriptLoader("spell_thorim_lightning_pillar_P2")
    {
    }
    ...
    class spell_thorim_lightning_pillar_P2_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_thorim_lightning_pillar_P2_AuraScript::OnPeriodic,
                EFFECT_0,
...
public:
    ...
    class spell_thorim_trash_impale_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_thorim_trash_impale_AuraScript::OnPeriodic,
                EFFECT_0,
...
public:
    achievement_thorim_stand_in_the_lightning()
        : AchievementCriteriaScript("achievement_thorim_stand_in_the_lightning")
    {
    }
    ...
    bool OnCheck(Player* player, Unit*, uint32 /*criteria_id*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript())
            if (Creature* cr = ObjectAccessor::GetCreature(
                    *player, instance->GetGuidData(TYPE_THORIM)))
...
public:
    achievement_thorim_lose_your_illusion()
        : AchievementCriteriaScript("achievement_thorim_lose_your_illusion")
    {
    }
