    ...
    struct boss_vezaxAI : public ScriptedAI {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if (target && spell && target->GetTypeId() == TYPEID_PLAYER &&
                spell->Id == SPELL_VEZAX_SHADOW_CRASH_DMG)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!berserk &&
                (me->GetPositionX() < 1720.0f || me->GetPositionX() > 1940.0f ||
            ...
            case EVENT_RESTORE_TARGET:
                if (me->GetVictim())
                    me->SetGuidValue(UNIT_FIELD_TARGET,
            ...
            case EVENT_SPELL_MARK_OF_THE_FACELESS: {
                ...
                Player* t = nullptr;
                if (outside.size() >=
                    uint8(me->GetMap()->Is25ManRaid() ? 9 : 4))
                ...
                if (vaporsCount < 6 || !hardmodeAvailable)
                    ...
                    for (ObjectGuid const& guid : summons)
                        if (Creature* sv =
                            ...
                            sv->GetMotionMaster()->MoveCharge(
                                1852.78f, 81.38f, 342.461f, 28.0f);
                        }
            ...
            case EVENT_SARONITE_VAPORS_SWIRL:
                if (summons.size()) {
                    ...
                    if (Creature* sv = ObjectAccessor::GetCreature(
                            *me, *(summons.begin())))
                        sv->CastSpell(
                            sv, SPELL_SARONITE_ANIMUS_FORMATION_VISUAL, true);

                    events.ScheduleEvent(EVENT_SPELL_SUMMON_SARONITE_ANIMUS,
                                         2s);
                    break;
                }
                break;
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (GameObject* door =
                    me->FindNearestGameObject(GO_VEZAX_DOOR, 500.0f))
...
public:
    ...
    struct npc_ulduar_saronite_vaporsAI : public NullCreatureAI {
        npc_ulduar_saronite_vaporsAI(Creature* pCreature)
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (pInstance)
                if (Creature* vezax = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(TYPE_VEZAX)))
                    vezax->AI()->DoAction(1);
        }
...
public:
    ...
    struct npc_ulduar_saronite_animusAI : public ScriptedAI {
        npc_ulduar_saronite_animusAI(Creature* pCreature)
...
public:
    ...
    class spell_aura_of_despair_AuraScript : public AuraScript {
        ...
        {
            OnEffectApply +=
                AuraEffectApplyFn(spell_aura_of_despair_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_PREVENT_REGENERATE_POWER,
                                  AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_aura_of_despair_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_PREVENT_REGENERATE_POWER,
                                   AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_mark_of_the_faceless_periodic()
        : SpellScriptLoader("spell_mark_of_the_faceless_periodic")
    {
    }
...
public:
    spell_mark_of_the_faceless_drainhealth()
        : SpellScriptLoader("spell_mark_of_the_faceless_drainhealth")
    {
    }
    ...
    class spell_mark_of_the_faceless_drainhealth_SpellScript
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_mark_of_the_faceless_drainhealth_SpellScript::
                    FilterTargets,
                EFFECT_1,
...
public:
    spell_saronite_vapors_dummy()
        : SpellScriptLoader("spell_saronite_vapors_dummy")
    {
    }
    ...
    class spell_saronite_vapors_dummy_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_saronite_vapors_dummy_AuraScript::HandleAfterEffectApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
...
public:
    spell_saronite_vapors_damage()
        : SpellScriptLoader("spell_saronite_vapors_damage")
    {
    }
    ...
    class spell_saronite_vapors_damage_SpellScript : public SpellScript {
        ...
        {
            AfterHit += SpellHitFn(
                spell_saronite_vapors_damage_SpellScript::HandleAfterHit);
        }
    };
...
public:
    achievement_smell_saronite()
        : AchievementCriteriaScript("achievement_smell_saronite")
    {
    }
    ...
    {
        return target && target->GetEntry() == NPC_VEZAX &&
...
public:
    achievement_shadowdodger()
        : AchievementCriteriaScript("achievement_shadowdodger")
    {
    }
...
public:
    go_ulduar_pure_saronite_deposit()
        : GameObjectScript("go_ulduar_pure_saronite_deposit")
    {
    }
    ...
    bool OnGossipHello(Player* plr, GameObject* go) override
    {
        ...
        if (InstanceScript* pInstance = go->GetInstanceScript())
            if (pInstance->GetData(TYPE_XT002) != DONE &&
                pInstance->GetData(TYPE_MIMIRON) != DONE &&
                pInstance->GetData(TYPE_THORIM) != DONE &&
                pInstance->GetData(TYPE_FREYA) != DONE &&
                pInstance->GetData(TYPE_HODIR) != DONE) {
                std::string accountName;
                AccountMgr::GetName(plr->GetSession()->GetAccountId(),
                                    accountName);
