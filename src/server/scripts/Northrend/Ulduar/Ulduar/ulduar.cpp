...
enum UldSpells { SPELL_SNOW_MOUND_PARTICLES = 64615 };
    ...
    class spell_ulduar_energy_sap_AuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (Unit* target = GetTarget())
                target->CastSpell(
                    target, (aurEff->GetId() == 64740) ? 64747 : 64863, true);
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_ulduar_energy_sap_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (!_activated && who->GetTypeId() == TYPEID_PLAYER) {
            if (me->GetExactDist2d(who) <= 10.0f &&
                ...
                if (GameObject* go =
                        me->FindNearestGameObject(GOBJ_SNOW_MOUND, 5.0f)) {
                    go->Delete();
                }
                ...
                _scheduler.Schedule(0s, [this](TaskContext context) {
                    _counter++;
                    float a = rand_norm() * 2 *
                              M_PI; // needs verification from sniffs
                    float d = rand_norm() * 4.0f;
                    if (Creature* jormungar = me->SummonCreature(
...
public:
    npc_ulduar_storm_tempered_keeper()
        : CreatureScript("npc_ulduar_storm_tempered_keeper")
    {
    }
    ...
    struct npc_ulduar_storm_tempered_keeperAI : public ScriptedAI {
        npc_ulduar_storm_tempered_keeperAI(Creature* creature)
            : ScriptedAI(creature)
        {
            otherGUID.Clear();
        }
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(
                1, 2s); // checking Separation Anxiety, Charged Sphere
            events.ScheduleEvent(2, 5s, 8s); // Forked Lightning
            events.ScheduleEvent(
                3,
                (me->GetEntry() == 33722 ? 20s : 50s)); // Summon Charged Sphere
            if (Creature* c = me->FindNearestCreature(
                    (me->GetEntry() == 33722 ? 33699 : 33722), 30.0f, true))
                otherGUID = c->GetGUID();
            else
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1:
                ...
                if (Creature* c = me->FindNearestCreature(33715, 2.0f, true))
                    if (c->IsSummon())
                        if (c->ToTempSummon()->GetSummonerGUID() !=
                            me->GetGUID())
...
public:
    npc_ulduar_arachnopod_destroyer()
        : CreatureScript("npc_ulduar_arachnopod_destroyer")
    {
    }
    ...
    struct npc_ulduar_arachnopod_destroyerAI : public ScriptedAI {
        npc_ulduar_arachnopod_destroyerAI(Creature* creature)
            : ScriptedAI(creature)
        {
            _spawnedMechanic = false;
            me->ApplySpellImmune(
                0, IMMUNITY_ID, 64919, true); // Ice Nova from Ice Turret
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (me->GetFaction() != FACTION_MONSTER_2) {
                if (me->IsAlive() &&
                    (me->GetExactDist2dSq(2058.0f, 42.0f) < 25.0f * 25.0f ||
                ...
                case 1:
                    me->CastSpell(
                        me->GetVictim(), RAID_MODE(64717, 65241), false);
                    events.Repeat(15s, 25s);
                    break;
                case 2:
                    me->CastSpell(
                        me->GetVictim(), RAID_MODE(64776, 65240), false);
...
public:
    spell_ulduar_arachnopod_damaged()
        : SpellScriptLoader("spell_ulduar_arachnopod_damaged")
    {
    }
...
public:
    ...
    {
        if (player->IsAlive())
            if (uint32 questId = (player->GetMap()->Is25ManRaid()
                                      ? 13816
                                      : 13607 /*QUEST_CELESTIAL_PLANETARIUM*/))
