...
#define SPELL_FROZEN_BLOWS                                                     \
    RAID_MODE(SPELL_FROZEN_BLOWS_10, SPELL_FROZEN_BLOWS_25)
#define SPELL_SHAMAN_STORM_CLOUD                                               \
    RAID_MODE(SPELL_SHAMAN_STORM_CLOUD_10, SPELL_SHAMAN_STORM_CLOUD_25)
...
public:
    ...
    struct boss_hodirAI : public ScriptedAI {
        ...
        const Position ENTRANCE_DOOR{
            1999.160034f, -297.792999f, 431.960999f, 0};
        ...
        void Reset() override
        {
            ...
            if (GameObject* go =
                    me->FindNearestGameObject(GO_HODIR_FRONTDOOR, 900.0f)) {
        ...
        void JustEngagedWith(Unit* /*pWho*/) override
        {
            ...
            if (GameObject* go =
                    me->FindNearestGameObject(GO_HODIR_FRONTDOOR, 300.0f)) {
                ...
                case EVENT_FAIL_HM:
                    if (pInstance) {
                        if (GameObject* go = pInstance->instance->GetGameObject(
                                pInstance->GetGuidData(GO_HODIR_CHEST_HARD))) {
                            go->SetGoState(GO_STATE_ACTIVE);
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            case SPELL_FLASH_FREEZE_VISUAL: {
                ...
                me->GetCreaturesWithEntryInRange(
                    fires, 200.0f, NPC_TOASTY_FIRE);
                for (std::list<Creature*>::iterator itr = fires.begin();
                     itr != fires.end();
                     ++itr)
                         ...
                         SpellSchoolMask) override
        {
            if (damage >= me->GetHealth() || me->GetHealth() < 150000) {
                ...
                if (!me->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE)) {
                    ...
                    if (GameObject* d = me->FindNearestGameObject(
                            GO_HODIR_FROZEN_DOOR, 250.0f)) {
                    ...
                    if (GameObject* d =
                            me->FindNearestGameObject(GO_HODIR_DOOR, 250.0f)) {
                        if (d->GetGoState() != GO_STATE_ACTIVE) {
                            d->SetLootState(GO_READY);
                            d->UseDoorOrButton(0, false);
                        }
        ...
        {
            if (me->GetPositionY() <= ENTRANCE_DOOR.GetPositionY() ||
                me->GetPositionY() >= EXIT_DOOR.GetPositionY()) {
            ...
            case EVENT_FLASH_FREEZE: {
                ...
                targets.remove_if(
                    Acore::ObjectTypeIdCheck(TYPEID_PLAYER, false));
                targets.remove_if(Acore::UnitAuraCheck(
                    true, SPELL_FLASH_FREEZE_TRAPPED_PLAYER));
                Acore::Containers::RandomResize(targets, (RAID_MODE(2, 3)));
                for (std::list<Unit*>::const_iterator itr = targets.begin();
                     itr != targets.end();
                     ++itr) {
                ...
                SmallIcicles(false);
                events.ScheduleEvent(EVENT_FLASH_FREEZE, 48s, 49s);
                events.ScheduleEvent(EVENT_SMALL_ICICLES_ENABLE,
                                     Is25ManRaid() ? 12s : 24s);
            ...
            case EVENT_FROZEN_BLOWS: {
                ...
                me->CastSpell(me,
                              Is25ManRaid() ? SPELL_FROZEN_BLOWS_25
                                            : SPELL_FROZEN_BLOWS_10,
                              true);
            } break;
            case EVENT_FREEZE:
                if (Player* plr = SelectTargetFromPlayerList(
                        50.0f, SPELL_FLASH_FREEZE_TRAPPED_PLAYER)) {
                    me->CastSpell(plr, SPELL_FREEZE, false);
                }
                else if (Unit* target = SelectTarget(
        ...
        Creature* GetHelper(uint8 index)
        {
            return Helpers[index]
                       ? ObjectAccessor::GetCreature(*me, Helpers[index])
                       : nullptr;
        }
        ...
        void SpawnHelpers()
        {
            ...
            if (hhd[0][0].id) {
                ...
                for (Map::PlayerList::const_iterator itr = cl.begin();
                     itr != cl.end();
                     ++itr)
                    if (!itr->GetSource()->IsGameMaster()) {
                        faction =
                            (itr->GetSource()->GetTeamId() == TEAM_ALLIANCE
                                 ? 'A'
                                 : 'H');
                        break;
                    }
            ...
            if (faction)
                for (uint8 k = 0; k < 4; ++k) {
                    if ((faction == 'A' &&
                         (k > 1 || (k == 1 && RAID_MODE(1, 0)))) ||
                        (faction == 'H' &&
                         (k < 2 || (k == 3 && RAID_MODE(1, 0)))))
                        continue;
                    ...
                    for (uint8 i = 0; i < 4; ++i) {
                        ...
                        if (Creature* h_p = me->SummonCreature(hhd[k][i].id,
                                                               hhd[k][i].x,
                                                               hhd[k][i].y,
                                                               432.69f,
                                                               M_PI / 2)) {
                            ...
                            if (Creature* c = h_p->SummonCreature(
                                    ...
                                    2000)) {
                                c->CastSpell(
                                    h_p, SPELL_FLASH_FREEZE_TRAPPED_NPC, true);
                                JustSummoned(c);
                            }
                        }
                    }
...
public:
    ...
    struct npc_ulduar_flash_freezeAI : public NullCreatureAI {
        npc_ulduar_flash_freezeAI(Creature* pCreature)
                         ...
                         SpellSchoolMask) override
        {
            if (pInstance && doneBy)
                if (pInstance->GetData(TYPE_HODIR) == NOT_STARTED)
                    if (Creature* hodir = ObjectAccessor::GetCreature(
                            *me, pInstance->GetGuidData(TYPE_HODIR)))
            ...
            if (timer <= diff) {
                ...
                if (me->IsSummon()) {
                    if (Unit* s = me->ToTempSummon()->GetSummonerUnit()) {
                        if ((s->GetTypeId() == TYPEID_PLAYER &&
                             !s->HasAura(SPELL_FLASH_FREEZE_TRAPPED_PLAYER)) ||
                            (s->GetTypeId() == TYPEID_UNIT &&
                             !s->HasAura(SPELL_FLASH_FREEZE_TRAPPED_NPC)))
                            me->DespawnOrUnsummon(2000);
                        else if (s->GetTypeId() == TYPEID_PLAYER)
                            if (InstanceScript* instanceScript =
                                    me->GetInstanceScript())
                                if (instanceScript->GetData(TYPE_HODIR) ==
                                    NOT_STARTED) {
                                    s->CastSpell(
                                        s, SPELL_FLASH_FREEZE_INSTAKILL, true);
                                    me->DespawnOrUnsummon(2000);
                                }
                    }
                    else {
...
public:
    ...
    struct npc_ulduar_toasty_fireAI : public NullCreatureAI {
        npc_ulduar_toasty_fireAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
        ...
        void DoAction(int32 a) override
        {
            if (a == 1) {
                if (GameObject* fire =
                        me->FindNearestGameObject(194300, 1.0f)) {
...
public:
    ...
    struct npc_ulduar_hodir_priestAI : public ScriptedAI {
        ...
        void AttackStart(Unit* who) override { AttackStartCaster(who, 17.0f); }
        ...
        void UpdateAI(uint32 diff) override
        {
                ...
                if (!me->HasAura(SPELL_FLASH_FREEZE_TRAPPED_NPC))
                    if (pInstance)
                        if (ObjectGuid g = pInstance->GetGuidData(TYPE_HODIR))
                            if (Creature* hodir =
                                    ObjectAccessor::GetCreature(*me, g)) {
                                AttackStart(hodir);
                                ScheduleAbilities();
                                break;
                            }
            ...
            case EVENT_PRIEST_DISPELL_MAGIC:
                me->CastCustomSpell(SPELL_PRIEST_DISPELL_MAGIC,
...
public:
    ...
    struct npc_ulduar_hodir_druidAI : public ScriptedAI {
        ...
        void AttackStart(Unit* who) override { AttackStartCaster(who, 22.0f); }
...
public:
    ...
    struct npc_ulduar_hodir_shamanAI : public ScriptedAI {
        ...
        void AttackStart(Unit* who) override { AttackStartCaster(who, 25.0f); }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if (target && spell->Id == SPELL_SHAMAN_STORM_CLOUD)
                if (Aura* a = target->GetAura(SPELL_SHAMAN_STORM_CLOUD,
                                              me->GetGUID()))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SHAMAN_STORM_CLOUD:
                if (Player* target = ScriptedAI::SelectTargetFromPlayerList(
                        35.0f, SPELL_SHAMAN_STORM_CLOUD))
...
public:
    ...
    struct npc_ulduar_hodir_mageAI : public ScriptedAI {
        ...
        void AttackStart(Unit* who) override { AttackStartCaster(who, 30.0f); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_MAGE_MELT_ICE: {
                ...
                bool                 found = false;
                me->GetCreaturesWithEntryInRange(
                    FB, 150.0f, NPC_FLASH_FREEZE_NPC);
                for (std::list<Creature*>::const_iterator itr = FB.begin();
                     itr != FB.end();
                     ++itr)
    ...
    class spell_hodir_shatter_chestSpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_hodir_shatter_chestSpellScript::destroyWinterCache,
                EFFECT_0,
            ...
            if ((aurEff->GetTickNumber() % 4) == 0)
                if (Unit* target = GetTarget())
                    if (target->GetTypeId() == TYPEID_PLAYER &&
                        ...
                        target->CastSpell(
                            target, SPELL_BITING_COLD_PLAYER_AURA, true);
        }
...
public:
    spell_hodir_biting_cold_player_aura()
        : SpellScriptLoader("spell_hodir_biting_cold_player_aura")
    {
    }
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            if (Unit* target = GetTarget()) {
                ...
                if (target->isMoving() ||
                    ...
                    if (counter >= 4) {
                        if (GetStackAmount() ==
                            2) // increasing from 2 to 3 (not checking >= to
                               // improve performance)
                            if (InstanceScript* pInstance =
                ...
                target->CastCustomSpell(
                    target, SPELL_BITING_COLD_DAMAGE, &dmg, 0, 0, true);
            }
        }
...
public:
    spell_hodir_periodic_icicle()
        : SpellScriptLoader("spell_hodir_periodic_icicle")
    {
    }
    ...
    class spell_hodir_periodic_icicle_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(
                Acore::UnitAuraCheck(true, SPELL_FLASH_FREEZE_TRAPPED_PLAYER));
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_hodir_periodic_icicle_SpellScript::FilterTargets,
                EFFECT_0,
...
public:
    ...
    class spell_hodir_flash_freeze_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_hodir_flash_freeze_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_hodir_flash_freeze_SpellScript::FilterTargets,
    ...
    class spell_hodir_flash_freeze_AuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (aurEff->GetTotalTicks() > 0 &&
                ...
                if (Aura* aur =
                        target->GetAura(target->GetTypeId() == TYPEID_PLAYER
                    ...
                    target->CastSpell(
                        target, SPELL_FLASH_FREEZE_INSTAKILL, true);
                    return;
                }
                if (target->GetTypeId() == TYPEID_PLAYER) {
                    ...
                    if (Creature* c = target->SummonCreature(
                            ...
                            5 * 60 * 1000)) {
                        c->CastSpell(
                            target, SPELL_FLASH_FREEZE_TRAPPED_PLAYER, true);
                    ...
                    if (Creature* c = target->SummonCreature(
                            ...
                            2000)) {
                        c->CastSpell(
                            target, SPELL_FLASH_FREEZE_TRAPPED_NPC, true);
                        caster->ToCreature()->AI()->JustSummoned(c);
                    }
                }
            }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_hodir_flash_freeze_AuraScript::HandleEffectPeriodic,
                EFFECT_1,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
    ...
    class spell_hodir_storm_power_AuraScript : public AuraScript {
                         ...
                         AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
                if (Aura* a =
                        caster->GetAura(GetId() == SPELL_SHAMAN_STORM_POWER_10
                                            ? SPELL_SHAMAN_STORM_CLOUD_10
                                            : SPELL_SHAMAN_STORM_CLOUD_25))
                    a->ModStackAmount(-1);
        }

        void HandleAfterEffectApply(AuraEffect const* /*aurEff*/,
                                    AuraEffectHandleModes /*mode*/)
        {
            if (Unit* target = GetTarget())
                if (target->GetTypeId() == TYPEID_PLAYER)
        ...
        {
            OnEffectApply +=
                AuraEffectApplyFn(spell_hodir_storm_power_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_MOD_CRIT_DAMAGE_BONUS,
                                  AURA_EFFECT_HANDLE_REAL);
            AfterEffectApply += AuraEffectApplyFn(
                spell_hodir_storm_power_AuraScript::HandleAfterEffectApply,
                EFFECT_0,
                SPELL_AURA_MOD_CRIT_DAMAGE_BONUS,
...
public:
    ...
    class spell_hodir_storm_cloud_AuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Unit* target = GetTarget())
                target->CastSpell((Unit*)nullptr,
                                  (GetId() == SPELL_SHAMAN_STORM_CLOUD_10
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_hodir_storm_cloud_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
public:
    achievement_cheese_the_freeze()
        : AchievementCriteriaScript("achievement_cheese_the_freeze")
    {
    }
    ...
    {
        return target && target->GetEntry() == NPC_HODIR &&
...
public:
    achievement_getting_cold_in_here()
        : AchievementCriteriaScript("achievement_getting_cold_in_here")
    {
    }
...
public:
    achievement_i_have_the_coolest_friends()
        : AchievementCriteriaScript("achievement_i_have_the_coolest_friends")
    {
    }
...
class achievement_staying_buffed_all_winter_10
    : public AchievementCriteriaScript {
public:
    achievement_staying_buffed_all_winter_10()
        : AchievementCriteriaScript("achievement_staying_buffed_all_winter_10")
    {
    }
    ...
    OnCheck(Player* player, Unit* /*target*/, uint32 /*criteria_id*/) override
    {
        return player && player->HasAura(SPELL_MAGE_TOASTY_FIRE_AURA) &&
               player->HasAura(SPELL_DRUID_STARLIGHT_AREA_AURA) &&
               player->HasAura(SPELL_SHAMAN_STORM_POWER_10);
    }
};
...
public:
    achievement_staying_buffed_all_winter_25()
        : AchievementCriteriaScript("achievement_staying_buffed_all_winter_25")
    {
    }
    ...
    OnCheck(Player* player, Unit* /*target*/, uint32 /*criteria_id*/) override
    {
        return player && player->HasAura(SPELL_MAGE_TOASTY_FIRE_AURA) &&
               player->HasAura(SPELL_DRUID_STARLIGHT_AREA_AURA) &&
               player->HasAura(SPELL_SHAMAN_STORM_POWER_25);
    }
};
...
public:
    ...
    class spell_hodir_toasty_fire_AuraScript : public AuraScript {
        ...
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_hodir_toasty_fire_AuraScript::HandleAfterEffectApply,
                EFFECT_0,
                SPELL_AURA_MOD_STAT,
                AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK);
        }
...
public:
    ...
    class spell_hodir_starlight_AuraScript : public AuraScript {
        ...
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_hodir_starlight_AuraScript::HandleAfterEffectApply,
                EFFECT_0,
