...
#define S_SCORCHED_GROUND                                                      \
    RAID_MODE(SPELL_SCORCHED_GROUND_10, SPELL_SCORCHED_GROUND_25)
...
public:
    ...
    struct npc_ulduar_iron_constructAI : public ScriptedAI {
        ...
        void JustReachedHome() override { me->CastSpell(me, 38757, true); }
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_ACTIVATE_CONSTRUCT) {
                ...
                if (InstanceScript* instance = me->GetInstanceScript())
                    if (Creature* ignis = ObjectAccessor::GetCreature(
                            *me, instance->GetGuidData(TYPE_IGNIS))) {
                        ignis->CastSpell(
                            ignis, SPELL_STRENGTH_OF_THE_CREATOR, true);
...
public:
    ...
    struct boss_ignisAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            for (std::list<Creature*>::iterator itr = icl.begin();
                 itr != icl.end();
                 ++itr) {
                if (!(*itr)->IsAlive()) {
                    (*itr)->Respawn();
            ...
            lastShatterMSTime = 0;
            events.Reset();
            events.ScheduleEvent(EVENT_ACTIVATE_CONSTRUCT,
                                 RAID_MODE(40000, 30000));
        ...
        void SetData(uint32 id, uint32 /*value*/) override
        {
            if (id == 1337) {
                if (lastShatterMSTime)
                    if (getMSTimeDiff(lastShatterMSTime,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->GetPositionX() < 490.0f || me->GetPositionX() > 690.0f ||
                me->GetPositionY() < 130.0f || me->GetPositionY() > 410.0f) {
            ...
            case EVENT_ACTIVATE_CONSTRUCT:
                Talk(SAY_SUMMON);
                me->CastCustomSpell(SPELL_ACTIVATE_CONSTRUCT,
            ...
            case EVENT_GRAB: {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                    ...
                    if (!temp->IsAlive() || temp->GetExactDist2d(me) > 90.0f)
                        continue;
                    if (me->GetVictim() &&
                        temp->GetGUID() == me->GetVictim()->GetGUID())
                        continue;
                    bool found = false;
                    for (std::list<Creature*>::iterator iterator = icl.begin();
                         iterator != icl.end();
                         ++iterator) {
                        if ((*iterator)->GetVictim() &&
                            (*iterator)->GetVictim()->GetGUID() ==
                                temp->GetGUID()) {
                            found = true;
                            break;
                        }
                ...
                if (!playerGUIDs.empty()) {
                    ...
                    if (Player* pTarget = ObjectAccessor::GetPlayer(
                            *me, playerGUIDs.at(pos))) {
                        Talk(SAY_SLAG_POT);
    ...
    class spell_ignis_scorch_AuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (aurEff->GetTotalTicks() >= 0 &&
                ...
                if (Unit* c = GetCaster())
                    if (Creature* s = c->SummonCreature(
                            NPC_SCORCHED_GROUND,
                            c->GetPositionX() +
                                20.0f * cos(c->GetOrientation()),
                            c->GetPositionY() +
                                20.0f * std::sin(c->GetOrientation()),
                            361.0f,
                            0.0f,
                            TEMPSUMMON_TIMED_DESPAWN,
                            30000)) {
                        if (!s->FindNearestCreature(
                                NPC_WATER_TRIGGER,
                                25.0f,
                                true)) // must be away from the water
                            s->CastSpell(s,
                                         (aurEff->GetId() == 62546
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_ignis_scorch_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
    ...
    class spell_ignis_grab_initial_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_ignis_grab_initial_SpellScript::HandleScript,
                EFFECT_2,
...
public:
    ...
    class spell_ignis_slag_pot_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_ignis_slag_pot_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            OnEffectApply +=
                AuraEffectApplyFn(spell_ignis_slag_pot_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_PERIODIC_DUMMY,
                                  AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_ignis_slag_pot_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_PERIODIC_DUMMY,
                                   AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    achievement_ignis_shattered()
        : AchievementCriteriaScript("achievement_ignis_shattered")
    {
    }
