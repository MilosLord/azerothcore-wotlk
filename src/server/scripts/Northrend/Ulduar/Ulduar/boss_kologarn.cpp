...
#define SPELL_PETRIFYING_BREATH                                                \
    RAID_MODE(SPELL_PETRIFYING_BREATH_10, SPELL_PETRIFYING_BREATH_25)
#define SPELL_OVERHEAD_SMASH                                                   \
    RAID_MODE(SPELL_OVERHEAD_SMASH_10, SPELL_OVERHEAD_SMASH_25)
#define SPELL_ONEARMED_OVERHEAD_SMASH                                          \
    RAID_MODE(SPELL_ONEARMED_OVERHEAD_SMASH_10,                                \
...
#define SPELL_FOCUSED_EYEBEAM                                                  \
    RAID_MODE(SPELL_FOCUSED_EYEBEAM_10, SPELL_FOCUSED_EYEBEAM_25)
#define SPELL_RUBBLE_FALL RAID_MODE(SPELL_RUBBLE_FALL_10, SPELL_RUBBLE_FALL_25)
#define SPELL_RUBBLE_ATTACK                                                    \
    RAID_MODE(SPELL_RUBBLE_ATTACK_10, SPELL_RUBBLE_ATTACK_25)
#define SPELL_RIDE_RIGHT_ARM                                                   \
    RAID_MODE(SPELL_RIDE_RIGHT_ARM_10, SPELL_RIDE_RIGHT_ARM_25)
    ...
    struct boss_kologarnAI : public ScriptedAI {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (who->GetTypeId() == TYPEID_PLAYER &&
                me->GetExactDist2d(who) < 45.0f &&
                me->getStandState() == UNIT_STAND_STATE_SUBMERGED) {
        ...
        void AttachLeftArm()
        {
            if (Unit* arm = ObjectAccessor::GetCreature(*me, _left))
                arm->SetHealth(arm->GetMaxHealth());
                         ...
                         NPC_LEFT_ARM, *me, TEMPSUMMON_MANUAL_DESPAWN)) {
                ...
                if (!me->HandleSpellClick(accessory, 0))
                    ...
                    accessory->CastSpell(
                        accessory, SPELL_ARM_RESPAWN_VISUAL, true);
                }
            }
        }
        ...
        void AttachRightArm()
        {
            if (Unit* arm = ObjectAccessor::GetCreature(*me, _right))
                arm->SetHealth(arm->GetMaxHealth());
                         ...
                         NPC_RIGHT_ARM, *me, TEMPSUMMON_MANUAL_DESPAWN)) {
                accessory->AddUnitTypeMask(UNIT_MASK_ACCESSORY);
        ...
        void Reset() override
        {
            ...
            if (m_pInstance) {
                ...
                if (GameObject* door = m_pInstance->instance->GetGameObject(
                        m_pInstance->GetGuidData(GO_KOLOGARN_DOORS)))
        ...
        void AttackStart(Unit* who) override { me->Attack(who, true); }

        void JustSummoned(Creature* cr) override
        {
            if (cr->GetEntry() != NPC_LEFT_ARM &&
                cr->GetEntry() != NPC_RIGHT_ARM)
                summons.Summon(cr);
        }
        ...
        void JustDied(Unit*) override
        {
            ...
            if (GameObject* bridge =
                    me->FindNearestGameObject(GO_KOLOGARN_BRIDGE, 100))
                bridge->SetGoState(GO_STATE_READY);

            // Summon Chest
            if (GameObject* go = me->SummonGameObject(
                    RAID_MODE(GO_KOLOGARN_CHEST, GO_KOLOGARN_CHEST_HERO),
                    1839.62f,
                    -35.98f,
                    448.81f,
                    3.6f,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SMASH:
                if (_left && _right)
                    ...
                    me->CastSpell(
                        me->GetVictim(), SPELL_ONEARMED_OVERHEAD_SMASH, false);
            ...
            case EVENT_SWEEP:
                if (_left) {
                    if (Creature* cr =
                            me->FindNearestCreature(NPC_SWEEP_TRIGGER, 300))
            ...
            case EVENT_FOCUSED_EYEBEAM: {
                ...
                if ((eyebeamTarget = SelectTarget(
                         SelectTargetMethod::MinDistance, 0, 0, true))) {
                    me->CastSpell(
                        eyebeamTarget, SPELL_FOCUSED_EYEBEAM_SUMMON, false);
                }
            ...
            // before checking distance
            if (me->isAttackReady() &&
                me->GetVictim()) // victim could die by a spell (IMPORTANT!!!)
                                 // and kologarn entered evade mode
            {
                ...
                if (breathReady)
                    me->CastSpell(
                        me->GetVictim(), SPELL_PETRIFYING_BREATH, false);
                me->resetAttackTimer();
            }
        }
    };
...
// also used for left arm, all functions except JustDied wont be used by left
// arm
...
public:
    ...
    struct boss_kologarn_armsAI : public ScriptedAI {
        ...
        void
        PassengerBoarded(Unit* /*who*/, int8 /*seatId*/, bool apply) override
        {
            if (!apply)
                _damageDone = 0;
            else {
            ...
            if (!_combatStarted)
                if (InstanceScript* instance = me->GetInstanceScript())
                    if (Creature* cr = ObjectAccessor::GetCreature(
                            *me, instance->GetGuidData(TYPE_KOLOGARN))) {
                        _combatStarted = true;
        ...
        void JustDied(Unit*) override
        {
            ...
            if (Creature* cr = me->SummonTrigger(x, y, z, 0, 5000)) {
                ...
                if (me->GetInstanceScript())
                    if (Creature* kologarn = ObjectAccessor::GetCreature(
                            *me,
                            me->GetInstanceScript()->GetGuidData(
                                TYPE_KOLOGARN)))
                        for (uint8 i = 0; i < 5; ++i)
                            if (Creature* cr2 = kologarn->SummonCreature(
                                    NPC_RUBBLE_SUMMON,
                                    cr->GetPositionX() + irand(-5, 5),
                                    cr->GetPositionY() + irand(-5, 5),
                                    cr->GetPositionZ(),
                                    0,
                                ...
                                if (Unit* target =
                                        SelectTargetFromPlayerList(100))
            ...
            if (me->GetInstanceScript())
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *me,
                        me->GetInstanceScript()->GetGuidData(TYPE_KOLOGARN)))
...
public:
    ...
    struct boss_kologarn_eyebeamAI : public ScriptedAI {
        boss_kologarn_eyebeamAI(Creature* c)
            : ScriptedAI(c), _timer(1), _damaged(false), justSpawned(true)
        {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (justSpawned) {
                ...
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *me, m_pInstance->GetGuidData(TYPE_KOLOGARN))) {
                    me->CastSpell(cr,
                                  me->GetEntry() == NPC_EYE_LEFT
...
public:
    spell_ulduar_stone_grip_cast_target()
        : SpellScriptLoader("spell_ulduar_stone_grip_cast_target")
    {
    }
    ...
    class spell_ulduar_stone_grip_cast_target_SpellScript : public SpellScript {
        ...
        void FilterTargetsInitial(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(StoneGripTargetSelector(
                GetCaster()->ToCreature(), GetCaster()->GetVictim()));
...
public:
    ...
    class spell_ulduar_stone_grip_AuraScript : public AuraScript {
        ...
        void OnRemoveStun(AuraEffect const* aurEff,
        ...
        void Register() override
        {
            OnEffectRemove += AuraEffectRemoveFn(
                spell_ulduar_stone_grip_AuraScript::OnRemoveStun,
                EFFECT_2,
                SPELL_AURA_MOD_STUN,
    ...
    class spell_ulduar_squeezed_lifeless_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_ulduar_squeezed_lifeless_SpellScript::HandleInstaKill,
                EFFECT_1,
...
public:
    spell_kologarn_stone_shout()
        : SpellScriptLoader("spell_kologarn_stone_shout")
    {
    }
    ...
    class spell_kologarn_stone_shout_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            if (m_scriptSpellId == SPELL_STONE_SHOUT_10 ||
                m_scriptSpellId == SPELL_STONE_SHOUT_25)
    ...
    class spell_kologarn_stone_shout_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            if (m_scriptSpellId != SPELL_STONE_SHOUT_10 &&
                m_scriptSpellId != SPELL_STONE_SHOUT_25)
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                    spell_kologarn_stone_shout_SpellScript::FilterTargets,
                    EFFECT_0,
                    TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
public:
    achievement_kologarn_looks_could_kill()
        : AchievementCriteriaScript("achievement_kologarn_looks_could_kill")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        if (target)
            if (InstanceScript* instance = target->GetInstanceScript())
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *target, instance->GetGuidData(TYPE_KOLOGARN)))
...
public:
    achievement_kologarn_rubble_and_roll()
        : AchievementCriteriaScript("achievement_kologarn_rubble_and_roll")
    {
    }
...
public:
    achievement_kologarn_with_open_arms()
        : AchievementCriteriaScript("achievement_kologarn_with_open_arms")
    {
    }
