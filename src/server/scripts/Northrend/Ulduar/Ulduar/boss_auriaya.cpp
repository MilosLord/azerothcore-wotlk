...
#define SPELL_SONIC_SCREECH                                                    \
    RAID_MODE(SPELL_SONIC_SCREECH_10, SPELL_SONIC_SCREECH_25)
#define SPELL_SENTINEL_BLAST                                                   \
    RAID_MODE(SPELL_SENTINEL_BLAST_10, SPELL_SENTINEL_BLAST_25)
#define SPELL_SAVAGE_POUNCE                                                    \
    RAID_MODE(SPELL_SAVAGE_POUNCE_10, SPELL_SAVAGE_POUNCE_25)
#define SPELL_RIP_FLESH RAID_MODE(SPELL_RIP_FLESH_10, SPELL_RIP_FLESH_25)
#define SPELL_FERAL_POUNCE                                                     \
    RAID_MODE(SPELL_FERAL_POUNCE_10, SPELL_FERAL_POUNCE_25)
...
public:
    ...
    struct boss_auriayaAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_EFFECT, SPELL_EFFECT_INTERRUPT_CAST, false);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUMMON_FERAL_DEFENDER:
                ...
                me->ApplySpellImmune(
                    0, IMMUNITY_EFFECT, SPELL_EFFECT_INTERRUPT_CAST, true);
...
public:
    ...
    struct npc_auriaya_sanctum_sentryAI : public ScriptedAI {
        npc_auriaya_sanctum_sentryAI(Creature* pCreature)
        ...
        void JustEngagedWith(Unit*) override
        {
            if (me->GetInstanceScript())
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *me,
                        me->GetInstanceScript()->GetGuidData(TYPE_AURIAYA)))
                    cr->SetInCombatWithZone();
        }
...
public:
    ...
    struct npc_auriaya_feral_defenderAI : public ScriptedAI {
        npc_auriaya_feral_defenderAI(Creature* pCreature)
            : ScriptedAI(pCreature), summons(pCreature)
        {
        }
        ...
        void JustDied(Unit*) override
        {
            ...
            if (me->GetInstanceScript())
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *me,
                        me->GetInstanceScript()->GetGuidData(TYPE_AURIAYA)))
                    cr->AI()->DoAction(_feralEssenceStack
                                           ? ACTION_FERAL_DEATH_WITH_STACK
                                           : ACTION_FERAL_DEATH);

            if (_feralEssenceStack) {
                if (Creature* cr = me->SummonCreature(NPC_SEEPING_FERAL_ESSENCE,
                                                      me->GetPositionX(),
                                                      me->GetPositionY(),
                                                      me->GetPositionZ(),
                                                      0.0f))
...
public:
    spell_auriaya_sentinel_blast()
        : SpellScriptLoader("spell_auriaya_sentinel_blast")
    {
    }
    ...
    class spell_auriaya_sentinel_blast_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_auriaya_sentinel_blast_SpellScript::FilterTargets,
                EFFECT_ALL,
...
public:
    achievement_auriaya_crazy_cat_lady()
        : AchievementCriteriaScript("achievement_auriaya_crazy_cat_lady")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        if (target)
            if (InstanceScript* instance = target->GetInstanceScript())
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *target, instance->GetGuidData(TYPE_AURIAYA)))
...
public:
    achievement_auriaya_nine_lives()
        : AchievementCriteriaScript("achievement_auriaya_nine_lives")
    {
    }
