...
enum Texts {
    ...
    // Vehicle Repair - Said by a spell, BroadcastTextID, same as
...
public:
    ...
    struct boss_flame_leviathanAI : public ScriptedAI {
        boss_flame_leviathanAI(Creature* pCreature)
            : ScriptedAI(pCreature), vehicle(me->GetVehicleKit()), summons(me)
        {
        ...
        {
            if (cr->GetEntry() != NPC_FLAME_LEVIATHAN_TURRET &&
                cr->GetEntry() != NPC_SEAT)
                summons.Summon(cr);
        }
        ...
        {
            if (m_pInstance &&
                m_pInstance->GetData(TYPE_LEVIATHAN) == SPECIAL) {
                me->SetHomePosition(homePos);
        ...
        void Reset() override
        {
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_USE_NORMAL_MOVEMENT_SPEED, true);
            ...
            if (m_pInstance) {
                if (m_pInstance->GetData(TYPE_LEVIATHAN) != SPECIAL) {
                    ...
                    m_pInstance->SetData(DATA_VEHICLE_SPAWN,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_startTimer) {
                ...
                if (_startTimer >= 4000) {
                    ...
                    if (me->FindNearestCreature(
                            NPC_ULDUAR_COLOSSUS, 250.0f, true) ||
                        !SelectTargetFromPlayerList(250.0f))
                        _startTimer = 1;
                    else {
                        _startTimer = 0;
                        _speakTimer = 1;
            ...
            if (_speakTimer) {
                ...
                if (_speakTimer <= 10000) {
                    ...
                    me->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                    TurnGates(true, false);
                    me->MonsterMoveWithSpeed(homePos.GetPositionX(),
...
{
    if (Creature* r = me->SummonCreature(NPC_BRANN_RADIO,
                                         me->GetPositionX() - 150,
                                         me->GetPositionY(),
                                         me->GetPositionZ(),
                                         me->GetOrientation(),
...
void boss_flame_leviathan::boss_flame_leviathanAI::ActivateTowers()
{
    _towersCount = 0;
    me->ResetLootMode();
    for (uint32 i = EVENT_TOWER_OF_LIFE_DESTROYED;
         i <= EVENT_TOWER_OF_FLAMES_DESTROYED;
         ++i) {
                                                             ...
                                                             bool _death)
{
    ...
    if (_start) {
        ...
        if ((go = ObjectAccessor::GetGameObject(
                 *me, m_pInstance->GetGuidData(DATA_LIGHTNING_WALL2))))
            go->SetGoState(GO_STATE_READY);

        if (m_pInstance->GetData(TYPE_LEVIATHAN) == NOT_STARTED)
            if ((go = ObjectAccessor::GetGameObject(
                     *me, m_pInstance->GetGuidData(GO_LEVIATHAN_DOORS))))
        ...
        if (_death)
            if ((go = ObjectAccessor::GetGameObject(
                     *me, m_pInstance->GetGuidData(DATA_LIGHTNING_WALL1))))
        ...
        if ((go = ObjectAccessor::GetGameObject(
                 *me, m_pInstance->GetGuidData(GO_LEVIATHAN_DOORS)))) {
            if (m_pInstance->GetData(TYPE_LEVIATHAN) == SPECIAL ||
                m_pInstance->GetData(TYPE_LEVIATHAN) == DONE)
                go->SetGoState(GO_STATE_ACTIVE_ALTERNATIVE);
            else
                go->SetGoState(GO_STATE_READY);
        }
...
void boss_flame_leviathan::boss_flame_leviathanAI::TurnHealStations(bool _apply)
{
    ...
    if (_apply) {
        if ((go = ObjectAccessor::GetGameObject(
                 *me, m_pInstance->GetGuidData(DATA_REPAIR_STATION1))))
            go->SetLootState(GO_READY);
        if ((go = ObjectAccessor::GetGameObject(
                 *me, m_pInstance->GetGuidData(DATA_REPAIR_STATION2))))
            go->SetLootState(GO_READY);
    }
    else {
        if ((go = ObjectAccessor::GetGameObject(
...
void boss_flame_leviathan::boss_flame_leviathanAI::SpellHit(
    Unit* /*caster*/, SpellInfo const* spellInfo)
{
    ...
    uint8 towerId)
{
    if (towerId == TOWER_OF_LIFE) {
        me->SummonCreature(NPC_FREYA_WARD_TARGET,
                           374,
                           -141,
                           411,
                           0,
        ...
        me->SummonCreature(
            NPC_FREYA_WARD, 374, -141, 411 + 40, 0, TEMPSUMMON_MANUAL_DESPAWN);
        me->SummonCreature(NPC_FREYA_WARD_TARGET,
                           382.9f,
                           74,
                           411.6f,
                           0,
                           TEMPSUMMON_MANUAL_DESPAWN);
        me->SummonCreature(NPC_FREYA_WARD,
                           382.9f,
                           74,
                           411.6f + 40,
                           0,
        ...
        me->SummonCreature(NPC_HODIRS_FURY_TARGET,
                           343.4f,
                           -77.5f,
                           409.8f,
                           0,
        ...
        me->SummonCreature(NPC_MIMIRONS_INFERNO_TARGET,
                           364.4f,
                           -9.7f,
                           409.8f,
                           0,
                           TEMPSUMMON_MANUAL_DESPAWN);
        // me->SummonCreature(NPC_MIMIRONS_INFERNO, 364.4f, -9.7f, 409.8f+40, 0,
        // TEMPSUMMON_MANUAL_DESPAWN);
    }
    else if (towerId == TOWER_OF_STORMS) {
        for (uint8 i = 0; i < 8; ++i)
            me->SummonCreature(NPC_THORIM_HAMMER_TARGET,
                               157 + rand() % 200,
                               -140 + rand() % 200,
                               409.8f,
                               0,
...
void boss_flame_leviathan::boss_flame_leviathanAI::SpellHitTarget(
    Unit* target, SpellInfo const* spell)
{
    if (spell->Id != SPELL_PURSUED)
        return;

    for (SeatMap::const_iterator itr = target->GetVehicleKit()->Seats.begin();
         itr != target->GetVehicleKit()->Seats.end();
         ++itr) {
        if (Player* passenger =
                ObjectAccessor::GetPlayer(*me, itr->second.Passenger.Guid)) {
...
public:
    ...
    struct boss_flame_leviathan_seatAI : public VehicleAI {
        boss_flame_leviathan_seatAI(Creature* creature)
            : VehicleAI(creature), vehicle(creature->GetVehicleKit())
        {
            ASSERT(vehicle);
        ...
        void Reset() override { _despawnTimer = !me->GetMap()->Is25ManRaid(); }
            ...
            if (_despawnTimer) {
                ...
                if (_despawnTimer >= 2000) {
                    ...
                    if (Vehicle* veh = me->GetVehicle())
                        if (veh->GetPassenger(0) == me ||
                            veh->GetPassenger(1) == me)
        ...
        void PassengerBoarded(Unit* who, int8 seatId, bool apply) override
        {
            ...
            who->ApplySpellImmune(
                63847, IMMUNITY_ID, 63847, apply); // SPELL_FLAME_VENTS_TRIGGER
            who->ApplySpellImmune(SPELL_MISSILE_BARRAGE,
                                  IMMUNITY_ID,
                                  SPELL_MISSILE_BARRAGE,
                                  apply);
            who->ApplySpellImmune(
                SPELL_BATTERING_RAM, IMMUNITY_ID, SPELL_BATTERING_RAM, apply);

            if (seatId == SEAT_PLAYER) {
                if (Unit* turret =
                        me->GetVehicleKit()->GetPassenger(SEAT_TURRET)) {
                    if (apply) {
...
public:
    boss_flame_leviathan_defense_turret()
        : CreatureScript("boss_flame_leviathan_defense_turret")
    {
    }
    ...
    struct boss_flame_leviathan_defense_turretAI : public TurretAI {
        ...
        void JustDied(Unit* killer) override
        {
            if (Player* player = killer->ToPlayer())
                player->UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS, 1, 0, me);

            if (Vehicle* vehicle = me->GetVehicle())
                if (Unit* device = vehicle->GetPassenger(SEAT_DEVICE))
                    device->ReplaceAllUnitFlags(UNIT_FLAG_NONE); // unselectable

            if (Creature* leviathan = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(TYPE_LEVIATHAN)))
        ...
        bool CanAIAttack(Unit const* who) const override
        {
            if (!who || who->GetTypeId() != TYPEID_PLAYER ||
                !who->GetVehicle() ||
                who->GetVehicleBase()->GetEntry() != NPC_SEAT)
                return false;
            return true;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_setHealth) {
                me->SetHealth(
                    std::min(me->GetHealth() + 1, me->GetMaxHealth()));
                _setHealth = false;
            }
        ...
        void KilledUnit(Unit* who) override
        {
            if (Player* plr = who->ToPlayer()) // make sure that there's no
                                               // death player on the seat.
                if (plr->GetVehicle())
                    plr->ExitVehicle();
        }
    };
...
public:
    boss_flame_leviathan_overload_device()
        : CreatureScript("boss_flame_leviathan_overload_device")
    {
    }
    ...
    struct boss_flame_leviathan_overload_deviceAI : public NullCreatureAI {
        ...
        void OnSpellClick(Unit* /*clicker*/, bool& result) override
        {
            ...
            if (me->GetVehicle()) {
                ...
                if (Unit* player =
                        me->GetVehicle()->GetPassenger(SEAT_PLAYER)) {
                    me->GetVehicleBase()->CastSpell(
                        player, SPELL_SMOKE_TRAIL, true);
                    player->ExitVehicle();
                }
            }
        }
...
public:
    ...
    struct npc_freya_wardAI : public NullCreatureAI {
        ...
        void Reset() override
        {
            ...
            summons.DespawnAll();
            if (Creature* cr =
                    me->FindNearestCreature(NPC_FREYA_WARD_TARGET, 60.0f, true))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_castTimer >= 29 * IN_MILLISECONDS) {
                if (Creature* cr = me->FindNearestCreature(
                        NPC_FREYA_WARD_TARGET, 60.0f, true)) {
                    me->CastSpell(cr, SPELL_FREYA_WARD, false);
                    me->CastSpell(
                        cr, 62947 /*SPELL_FREYA_WARD_SECOND_SUMMON*/, false);
                }
    ...
    struct npc_hodirs_furyAI : public NullCreatureAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_timeToHit) {
                ...
                if (_timeToHit >= 5000) {
                    if (Creature* cr =
                            me->SummonCreature(NPC_HODIRS_FURY,
                                               me->GetPositionX(),
                                               me->GetPositionY(),
                                               me->GetPositionZ() + 40,
                                               0,
            ...
            if (_switchTargetTimer >= 30000) {
                if (Unit* target = me->SelectNearbyTarget(nullptr, 200.0f)) {
                    if (target->GetVehicleBase() &&
                        target->GetVehicleBase()->GetEntry() == NPC_SEAT) {
...
public:
    ...
    struct npc_mimirons_infernoAI : public npc_escortAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_spellTimer >= 2000) {
                if (Creature* cr =
                        me->SummonCreature(NPC_MIMIRONS_INFERNO,
                                           me->GetPositionX(),
                                           me->GetPositionY(),
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_beamTimer) {
                ...
                if (_beamTimer >= _finishTime) {
                    if (Creature* cr =
                            me->SummonCreature(NPC_THORIM_HAMMER,
                                               me->GetPositionX(),
                                               me->GetPositionY(),
                                               me->GetPositionZ() + 40,
                                               0,
...
public:
    ...
    struct npc_brann_radioAI : public NullCreatureAI {
        ...
        {
            _lock     = (me->GetInstanceScript() &&
                     me->GetInstanceScript()->GetData(TYPE_LEVIATHAN) >
                         NOT_STARTED);
            _helpLock = _lock;
        }
        ...
        void Reset() override { me->SetReactState(REACT_AGGRESSIVE); }
...
public:
    boss_flame_leviathan_safety_container()
        : CreatureScript("boss_flame_leviathan_safety_container")
    {
    }
    ...
    struct boss_flame_leviathan_safety_containerAI : public NullCreatureAI {
        ...
        {
            _allowTimer += diff;
            if (_allowTimer >= 5000 && !me->GetVehicle() &&
    ...
    struct npc_mechanoliftAI : public NullCreatureAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_startTimer) {
                _startTimer -= diff;
                if (_startTimer <= 0) {
                    me->GetMotionMaster()->MovePath(3000000 + urand(0, 11),
                                                    true);
                    _startTimer = 0;
                }
            }
...
public:
    ...
    {
        Creature* trigger =
            go->FindNearestCreature(NPC_ULDUAR_GAUNTLET_GENERATOR, 15.0f, true);
        if (trigger)
...
public:
    ...
    class spell_load_into_catapult_AuraScript : public AuraScript {
        ...
        {
            OnEffectApply +=
                AuraEffectApplyFn(spell_load_into_catapult_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_CONTROL_VEHICLE,
                                  AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_load_into_catapult_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_CONTROL_VEHICLE,
...
public:
    ...
    class spell_auto_repair_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*eff*/)
        {
            ...
            // driver->TextEmote(VEHICLE_EMOTE_REPAIR, driver, true); // No
            // source

            // Actually should/could use basepoints (100) for this spell effect
            // as percentage of health, but oh well.
            vehicle->GetBase()->SetFullHealth();

            // Achievement
            if (InstanceScript* instance =
                    vehicle->GetBase()->GetInstanceScript())
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_auto_repair_SpellScript::FilterTargets,
                EFFECT_ALL,
                TARGET_UNIT_DEST_AREA_ENTRY);
            OnEffectHitTarget +=
                SpellEffectFn(spell_auto_repair_SpellScript::HandleScript,
                              EFFECT_0,
...
public:
    ...
    class spell_systems_shutdown_AuraScript : public AuraScript {
        ...
        {
            OnEffectApply +=
                AuraEffectApplyFn(spell_systems_shutdown_AuraScript::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN,
                                  AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_systems_shutdown_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN,
                                   AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    bool operator()(WorldObject* target) const
    {
        ...
        for (SeatMap::const_iterator itr = vehicle->Seats.begin();
             itr != vehicle->Seats.end() && !playerFound;
             ++itr)
...
public:
    ...
    class spell_pursue_SpellScript : public SpellScript {
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_pursue_SpellScript::FilterTargets,
                EFFECT_ALL,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget +=
                SpellEffectFn(spell_pursue_SpellScript::HandleScript,
                              EFFECT_0,
...
public:
    spell_vehicle_throw_passenger()
        : SpellScriptLoader("spell_vehicle_throw_passenger")
    {
    }
    ...
    class spell_vehicle_throw_passenger_SpellScript : public SpellScript {
        ...
        void HandleScript()
        {
            ...
            if (Vehicle* vehicle = GetCaster()->GetVehicleKit())
                if (Unit* passenger = vehicle->GetPassenger(3)) {
                    ...
                    Acore::WorldObjectSpellAreaTargetCheck check(
                        99,
                        GetExplTargetDest(),
                        GetCaster(),
                        GetCaster(),
                        ...
                        nullptr);
                    Acore::WorldObjectListSearcher<
                        Acore::WorldObjectSpellAreaTargetCheck>
                        searcher(GetCaster(), targetList, check);
                    Cell::VisitAllObjects(GetCaster(), searcher, 99.0f);
                    float minDist = 99 * 99;
                    Unit* target  = nullptr;
                    for (std::list<WorldObject*>::iterator itr =
                             targetList.begin();
                         itr != targetList.end();
                         ++itr) {
                    ...
                    if (target &&
                        target->IsWithinDist2d(
                            targets.GetDstPos(),
                            GetSpellInfo()->Effects[EFFECT_0].CalcRadius() *
                                2)) // now we use *2 because the location of the
                                    // seat is not correct
                    {
                        ...
                        passenger->GetMotionMaster()->MoveJump(
                            x, y, z, targets.GetSpeedXY(), targets.GetSpeedZ());
                    }
                }
        }
        ...
        void Register() override
        {
            AfterCast += SpellCastFn(
                spell_vehicle_throw_passenger_SpellScript::HandleScript);
        }
    };
...
public:
    ...
    class spell_tar_blaze_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic +=
                AuraEffectPeriodicFn(spell_tar_blaze_AuraScript::OnPeriodic,
                                     EFFECT_0,
    ...
    class spell_vehicle_grab_pyrite_SpellScript : public SpellScript {
            ...
            if (Unit* target = GetHitUnit())
                if (Unit* seat = GetCaster()->GetVehicleBase()) {
                    ...
                    if (Unit* parent = seat->GetVehicleBase()) {
                        GetCaster()->CastSpell(
                            parent, 62496 /*SPELL_ADD_PYRITE*/, true);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_vehicle_grab_pyrite_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_vehicle_circuit_overload()
        : SpellScriptLoader("spell_vehicle_circuit_overload")
    {
    }
    ...
    class spell_vehicle_circuit_overload_AuraScript : public AuraScript {
        ...
        void OnPeriodic(AuraEffect const* /*aurEff*/)
        {
            if (Unit* target = GetTarget())
                if (int(target->GetAppliedAuras().count(
                        SPELL_OVERLOAD_CIRCUIT)) >=
                    (target->GetMap()->Is25ManRaid() ? 4 : 2)) {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_vehicle_circuit_overload_AuraScript::OnPeriodic,
                EFFECT_1,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
    ...
    class spell_orbital_supports_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            DoCheckAreaTarget += AuraCheckAreaTargetFn(
                spell_orbital_supports_AuraScript::CheckAreaTarget);
        }
    };
    ...
    class spell_thorims_hammer_SpellScript : public SpellScript {
        ...
        void RecalculateDamage(SpellEffIndex effIndex)
        {
            ...
            if (dist <= 7.0f) {
                ...
                dist -= 6.0f;
                SetHitDamage(
                    int32(GetSpellInfo()->Effects[EFFECT_1].CalcValue() /
                          std::max(dist, 1.0f)));
            }
        }
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_thorims_hammer_SpellScript::RecalculateDamage,
                EFFECT_ALL,
...
public:
    spell_transitus_shield_beam()
        : SpellScriptLoader("spell_transitus_shield_beam")
    {
    }
    ...
    class spell_transitus_shield_beam_AuraScript : public AuraScript {
        ...
        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_transitus_shield_beam_AuraScript::HandleOnEffectApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_transitus_shield_beam_AuraScript::HandleOnEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        }
    ...
    class spell_shield_generator_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_shield_generator_AuraScript::CalculateAmount,
                EFFECT_0,
                SPELL_AURA_SCHOOL_ABSORB);
            OnEffectAbsorb += AuraEffectAbsorbFn(
                spell_shield_generator_AuraScript::Absorb, EFFECT_0);
        }
    };
...
public:
    spell_demolisher_ride_vehicle()
        : SpellScriptLoader("spell_demolisher_ride_vehicle")
    {
    }
    ...
    class spell_demolisher_ride_vehicle_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_demolisher_ride_vehicle_SpellScript::CheckCast);
        }
    };
...
public:
    achievement_flame_leviathan_towers(char const* name, uint32 count)
        : AchievementCriteriaScript(name), _towerCount(count)
    {
    }

    bool OnCheck(Player* /*player*/,
                 Unit* target /*Flame Leviathan*/,
                 uint32 /*criteria_id*/) override
    {
        return target &&
               _towerCount <= target->GetAI()->GetData(DATA_GET_TOWER_COUNT);
    }
...
public:
    achievement_flame_leviathan_shutout()
        : AchievementCriteriaScript("achievement_flame_leviathan_shutout")
    {
    }
...
public:
    achievement_flame_leviathan_garage(char const* name,
                                       uint32      entry1,
                                       uint32      entry2)
        : AchievementCriteriaScript(name), _entry1(entry1), _entry2(entry2)
    {
    }
    ...
    bool OnCheck(Player* player, Unit*, uint32 /*criteria_id*/) override
    {
        if (Vehicle* vehicle = player->GetVehicle())
            if (vehicle->GetCreatureEntry() == _entry1 ||
                vehicle->GetCreatureEntry() == _entry2)
                return true;
        return false;
    }
...
public:
    achievement_flame_leviathan_unbroken()
        : AchievementCriteriaScript("achievement_flame_leviathan_unbroken")
    {
    }
...
void AddSC_boss_flame_leviathan()
{
    ...
    // Achievements
    new achievement_flame_leviathan_towers(
        "achievement_flame_leviathan_orbital_bombardment", 1);
    new achievement_flame_leviathan_towers(
        "achievement_flame_leviathan_orbital_devastation", 2);
    new achievement_flame_leviathan_towers(
        "achievement_flame_leviathan_nuked_from_orbit", 3);
    new achievement_flame_leviathan_towers(
        "achievement_flame_leviathan_orbituary", 4);
    new achievement_flame_leviathan_shutout();
    new achievement_flame_leviathan_garage(
        "achievement_flame_leviathan_garage_chopper", NPC_VEHICLE_CHOPPER, 0);
    new achievement_flame_leviathan_garage(
        "achievement_flame_leviathan_garage_siege_engine",
        NPC_SALVAGED_SIEGE_ENGINE,
        NPC_SALVAGED_SIEGE_ENGINE_TURRET);
    new achievement_flame_leviathan_garage(
        "achievement_flame_leviathan_garage_demolisher",
