...
public:
    ...
    struct instance_ulduar_InstanceMapScript : public InstanceScript {
        ...
        Position   normalChestPosition = {
            1967.152588f, -204.188461f, 432.686951f, 5.50957f};
        Position hardChestPosition = {
            2035.94600f, -202.084885f, 432.686859f, 3.164077f};
        ...
        {
            packet << uint32(WORLD_STATE_ALGALON_TIMER_ENABLED)
                   << uint32(m_algalonTimer && m_algalonTimer <= 60);
            packet << uint32(WORLD_STATE_ALGALON_DESPAWN_TIMER)
                   << uint32(std::min<uint32>(m_algalonTimer, 60));
        }
        ...
        void OnPlayerEnter(Player* player) override
        {
            ...
            if (GameObject* MimironTram =
                    instance->GetGameObject(m_mimironTramGUID))
                player->UpdateVisibilityOf(MimironTram);

            if (!m_uiAlgalonGUID && m_algalonTimer &&
                (m_algalonTimer <= 60 ||
        ...
        bool IsEncounterInProgress() const override
        {
            ...
            // Leviathan does not use IN_PROGRESS type, instead SPECIAL is set
            // and never reset, Check if he is in combat.
        ...
        {
            // destory towers
            if (eventId >= EVENT_TOWER_OF_LIFE_DESTROYED &&
                eventId <= EVENT_TOWER_OF_FLAMES_DESTROYED)
                SetData(eventId, 0);
        }
            ...
            case RAID_DIFFICULTY_10MAN_NORMAL: // 10 man chest
            {
                if (!m_hodirNormalChest) {
                    if (GameObject* go = hodir->SummonGameObject(
                            ...
                            normalChestPosition.GetOrientation(),
                            0,
                            0,
                            0,
                            0,
                ...
                if (!m_hodirHardmodeChest) {
                    if (GameObject* go = hodir->SummonGameObject(
                            ...
                            hardChestPosition.GetOrientation(),
                            0,
                            0,
                            0,
                            0,
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_ELDER_BRIGHTLEAF:
                m_FreyaElder[creature->GetEntry() - NPC_ELDER_IRONBRANCH] =
                    creature->GetGUID();
                break;
            case NPC_SARA:
        ...
        void ShowKeeperGossip(uint8      type,
                              Creature*  cr,
                              ObjectGuid guid = ObjectGuid::Empty)
        {
            ...
            bool on = (GetData(type) == DONE &&
                       !(GetData(TYPE_WATCHERS) & (1 << (type - TYPE_FREYA))));
            cr->SetVisible(on);
        }
        ...
        void OnGameObjectCreate(GameObject* gameObject) override
        {
            ...
            case GO_ARENA_LEVER_GATE:
                m_thorimGameobjectsGUID[DATA_THORIM_LEVER_GATE -
                                        DATA_THORIM_LEVER_GATE] =
                    gameObject->GetGUID();
                break;
            case GO_ARENA_LEVER:
                m_thorimGameobjectsGUID[DATA_THORIM_LEVER -
                                        DATA_THORIM_LEVER_GATE] =
                    gameObject->GetGUID();
                break;
            case GO_ARENA_FENCE:
                m_thorimGameobjectsGUID[DATA_THORIM_FENCE -
                                        DATA_THORIM_LEVER_GATE] =
                    gameObject->GetGUID();
                break;
            case GO_FIRST_COLOSSUS_DOORS:
                m_thorimGameobjectsGUID[DATA_THORIM_FIRST_DOORS -
                                        DATA_THORIM_LEVER_GATE] =
                    gameObject->GetGUID();
                break;
            case GO_SECOND_COLOSSUS_DOORS:
                m_thorimGameobjectsGUID[DATA_THORIM_SECOND_DOORS -
                                        DATA_THORIM_LEVER_GATE] =
                    gameObject->GetGUID();
                break;
            // Yogg-Saron
            ...
            case GO_KEEPERS_GATE:
                if (GetData(TYPE_MIMIRON) == DONE &&
                    GetData(TYPE_FREYA) == DONE &&
                    GetData(TYPE_HODIR) == DONE &&
                    GetData(TYPE_THORIM) == DONE) {
        ...
        void setChestsLootable(uint32 boss)
        {
            if (boss) {
                ...
                case TYPE_HODIR:
                    if (hmHodir) {
                        if (GameObject* go =
                                instance->GetGameObject(m_hodirHardmodeChest)) {
                            go->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                            go->SetLootRecipient(instance);
                        }
                    }
                    if (GameObject* go =
                            instance->GetGameObject(m_hodirNormalChest)) {
                        go->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                        go->SetLootRecipient(instance);
                    }
                    break;
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case TYPE_LEVIATHAN:
                ...
                if (data == DONE) {
                    ...
                    for (Map::PlayerList::const_iterator itr = pList.begin();
                         itr != pList.end();
                         ++itr) {
                        if (Creature* vehicleCreature =
                                itr->GetSource()->GetVehicleCreatureBase()) {
            ...
            case TYPE_ASSEMBLY:
                if (GameObject* go =
                        instance->GetGameObject(m_assemblyDoorsGUID))
                    go->SetGoState(data == IN_PROGRESS ? GO_STATE_READY
                                                       : GO_STATE_ACTIVE);
                if (GameObject* go =
                        instance->GetGameObject(m_archivumDoorsGUID))
                    go->SetGoState(data == DONE ? GO_STATE_ACTIVE
                                                : GO_STATE_READY);
            ...
            case TYPE_FREYA:
                m_auiEncounter[type] = data;
                ShowKeeperGossip(
                    type, nullptr, m_keepersGossipGUID[type - TYPE_FREYA]);
                if (GetData(TYPE_MIMIRON) == DONE &&
                    GetData(TYPE_FREYA) == DONE &&
                    GetData(TYPE_HODIR) == DONE &&
                    GetData(TYPE_THORIM) == DONE) {
                    if (GameObject* go =
                            instance->GetGameObject(m_keepersgateGUID))
                        go->RemoveGameObjectFlag(GO_FLAG_LOCKED);
                }
                if (type == TYPE_MIMIRON &&
                    data == IN_PROGRESS) // after reaching him without tram and
                                         // starting the fight
                    m_mimironTramUsed = true;
                if (GetData(TYPE_HODIR) == DONE)
            ...
            case EVENT_TOWER_OF_FLAMES_DESTROYED: {
                instance->LoadGrid(364.0f,
                                   -16.0f); // make sure leviathan is loaded
                m_leviathanTowers[type - EVENT_TOWER_OF_LIFE_DESTROYED] = data;
                for (uint8 i = 0; i < 2; ++i) {
                    if (GameObject* gameObject = instance->GetGameObject(
                            m_leviathanVisualTowers
                                [type - EVENT_TOWER_OF_LIFE_DESTROYED][i])) {
                        gameObject->SetGoState(GO_STATE_ACTIVE);
                    }
                }
                return;
            ...
            case DATA_ALGALON_DEFEATED:
                ...
                m_algalonTimer =
                    (type == DATA_ALGALON_DEFEATED ? TIMER_ALGALON_DEFEATED
            ...
            case TYPE_ALGALON:
                ...
                if (GameObject* go = instance->GetGameObject(
                        GetGuidData(GO_DOODAD_UL_SIGILDOOR_03))) {
                    go->SetGoState(data != IN_PROGRESS ? GO_STATE_ACTIVE
                                                       : GO_STATE_READY);
                    go->EnableCollision(false);
                }
                if (GameObject* go = instance->GetGameObject(
                        GetGuidData(GO_DOODAD_UL_UNIVERSEFLOOR_01))) {
                    go->SetGoState(data != IN_PROGRESS ? GO_STATE_ACTIVE
                                                       : GO_STATE_READY);
                    go->EnableCollision(false);
                }
                if (GameObject* go = instance->GetGameObject(
                        GetGuidData(GO_DOODAD_UL_UNIVERSEFLOOR_02))) {
                    go->SetGoState(data == IN_PROGRESS ? GO_STATE_ACTIVE
                                                       : GO_STATE_READY);
                    go->EnableCollision(false);
                }
                if (GameObject* go = instance->GetGameObject(
                        GetGuidData(GO_DOODAD_UL_UNIVERSEGLOBE01))) {
                    go->SetGoState(data == IN_PROGRESS ? GO_STATE_ACTIVE
                                                       : GO_STATE_READY);
                    go->EnableCollision(false);
                }
                if (GameObject* go = instance->GetGameObject(
                        GetGuidData(GO_DOODAD_UL_ULDUAR_TRAPDOOR_03))) {
                    go->SetGoState(data == IN_PROGRESS ? GO_STATE_ACTIVE
                                                       : GO_STATE_READY);
                    go->EnableCollision(false);
                }
            ...
            case DATA_DWARFAGEDDON:
                DoStartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET,
                                        SPELL_DWARFAGEDDON);
            ...
            case DATA_CALL_TRAM:
                if (GameObject* MimironTram =
                    ...
                    if (StaticTransport* t = MimironTram->ToStaticTransport()) {
                        if (data == 0 && t->GetGoState() == GO_STATE_ACTIVE &&
                            t->GetPathProgress() == t->GetPauseTime())
                            MimironTram->SetGoState(GO_STATE_READY);
                        if (data == 1 && t->GetGoState() == GO_STATE_READY &&
                            t->GetPathProgress() == 0)
            ...
            case DATA_BRANN_MEMOTESAY:
                if (Creature* cr =
                        instance->GetCreature(m_brannBronzebeardBaseCamp)) {
            ...
            if (type == TYPE_FREYA && data == DONE) {
                ...
                if (Creature* freya =
                        instance->GetCreature(GetGuidData(TYPE_FREYA))) {
                    freya->GetGameObjectListWithEntryInGrid(
                        goList, 191019 /*Adder's Tongue*/, 333.0f);
                    freya->GetGameObjectListWithEntryInGrid(
                        goList, 190176 /*Frost Lotus*/, 333.0f);
                    freya->GetGameObjectListWithEntryInGrid(
                        goList, 190171 /*Lichbloom*/, 333.0f);
                    freya->GetGameObjectListWithEntryInGrid(
                        goList, 190170 /*Talandra's Rose*/, 333.0f);
                    freya->GetGameObjectListWithEntryInGrid(
                        goList, 189973 /*Goldclover*/, 333.0f);
            ...
            if (type > TYPE_LEVIATHAN && type < TYPE_WATCHERS &&
                data == IN_PROGRESS) {
                Map::PlayerList const& pList = instance->GetPlayers();
                for (Map::PlayerList::const_iterator itr = pList.begin();
                     itr != pList.end();
        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            if (unit->GetTypeId() == TYPEID_PLAYER)
                for (uint8 i = 0; i <= 12; ++i) {
                    ...
                    if (i == TYPE_LEVIATHAN) {
                        if (Creature* c =
                                instance->GetCreature(m_uiLeviathanGUID))
                            if (c->IsInCombat())
                                go = true;
                    }
                    else
        ...
        void ReadSaveDataMore(std::istringstream& data) override
        {
            ...
            if (m_algalonTimer && m_algalonTimer <= 60 &&
                GetData(TYPE_ALGALON) != DONE) {
                DoUpdateWorldState(WORLD_STATE_ALGALON_TIMER_ENABLED, 1);
        ...
        void WriteSaveDataMore(std::ostringstream& data) override
        {
            data << m_auiEncounter[0] << ' ' << m_auiEncounter[1] << ' '
                 ...
                 << m_auiEncounter[14] << ' ' << m_conspeedatoryAttempt << ' '
                 << m_unbrokenAchievement << ' ' << m_algalonTimer << ' '
                 << C_of_Ulduar_MASK << ' ' << m_mageBarrier;
        }
...
void instance_ulduar::instance_ulduar_InstanceMapScript::
    SpawnLeviathanEncounterVehicles(uint8 mode)
{
    ...
    if (mode < VEHICLE_POS_NONE) {
        for (uint8 i = 0; i < (instance->Is25ManRaid() ? 5 : 2); ++i) {
            if (TempSummon* veh =
                    instance->SummonCreature(NPC_SALVAGED_SIEGE_ENGINE,
                                             vehiclePositions[15 * mode + i])) {
                _leviathanVehicles.push_back(veh->GetGUID());
            }
            if (TempSummon* veh = instance->SummonCreature(
                    NPC_VEHICLE_CHOPPER, vehiclePositions[15 * mode + i + 5])) {
                _leviathanVehicles.push_back(veh->GetGUID());
            }
            if (TempSummon* veh = instance->SummonCreature(
                    NPC_SALVAGED_DEMOLISHER,
                    vehiclePositions[15 * mode + i + 10])) {
                _leviathanVehicles.push_back(veh->GetGUID());
            }
        }
    }
...
void AddSC_instance_ulduar() { new instance_ulduar(); }
