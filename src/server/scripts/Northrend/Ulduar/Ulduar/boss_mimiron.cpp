...
#define SPELL_NAPALM_SHELL                                                     \
    RAID_MODE(SPELL_NAPALM_SHELL_10, SPELL_NAPALM_SHELL_25)
#define SPELL_PLASMA_BLAST                                                     \
    RAID_MODE(SPELL_PLASMA_BLAST_10, SPELL_PLASMA_BLAST_25)
#define SPELL_MINE_EXPLOSION                                                   \
    RAID_MODE(SPELL_MINE_EXPLOSION_10, SPELL_MINE_EXPLOSION_25)
#define SPELL_PLASMA_BALL RAID_MODE(SPELL_PLASMA_BALL_10, SPELL_PLASMA_BALL_25)
#define SPELL_HAND_PULSE_R                                                     \
    RAID_MODE(SPELL_HAND_PULSE_10_R, SPELL_HAND_PULSE_25_R)
#define SPELL_HAND_PULSE_L                                                     \
    RAID_MODE(SPELL_HAND_PULSE_10_L, SPELL_HAND_PULSE_25_L)
#define SPELL_FROST_BOMB_EXPLOSION                                             \
    RAID_MODE(SPELL_FROST_BOMB_EXPLOSION_10, SPELL_FROST_BOMB_EXPLOSION_25)
...
#define GetMimiron()                                                           \
    ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(TYPE_MIMIRON))
#define GetLMK2()                                                              \
    ObjectAccessor::GetCreature(                                               \
        *me, pInstance->GetGuidData(DATA_MIMIRON_LEVIATHAN_MKII))
#define GetVX001()                                                             \
    ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(DATA_MIMIRON_VX001))
#define GetACU()                                                               \
    ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(DATA_MIMIRON_ACU))
    ...
    struct boss_mimironAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            if (!hardmode) {
                ...
                events.ScheduleEvent(EVENT_BERSERK,
                                     Is25ManRaid() ? 10min : 8min);

                events.ScheduleEvent(EVENT_COMPUTER_SAY_INITIATED, 0ms);
                events.ScheduleEvent(EVENT_COMPUTER_SAY_MINUTES, 3s);
                minutesTalkNum =
                    Is25ManRaid() ? TALK_COMPUTER_TEN : TALK_COMPUTER_EIGHT;
                for (uint32 i = 0;
                     i < uint32(TALK_COMPUTER_ZERO - minutesTalkNum - 1);
                     ++i)
                    events.ScheduleEvent(EVENT_COMPUTER_SAY_MINUTES,
                                         (i + 1) * MINUTE * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_COMPUTER_SAY_MINUTES,
                                     (TALK_COMPUTER_ZERO - minutesTalkNum) *
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->GetExactDist(&p) > 80.0f ||
                !SelectTargetFromPlayerList(150.0f)) {
                EnterEvadeMode(EVADE_REASON_OTHER);
                return;
            }
            ...
            case EVENT_COMPUTER_SAY_INITIATED:
                if (Creature* computer =
                        me->SummonCreature(NPC_COMPUTER,
                                           2746.7f,
                                           2569.44f,
                                           410.39f,
                                           0.0f,
            ...
            case EVENT_SPAWN_FLAMES_INITIAL: {
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (!pg.empty()) {
                        ...
                        float   z      = 364.35f;
                        if (!player->IsWithinLOS(
                                player->GetPositionX() + cos(angle) * 5.0f,
                                player->GetPositionY() + std::sin(angle) * 5.0f,
                                z)) {
                    ...
                    if (hardmode) {
                        ...
                        if (Vehicle* veh = LMK2->GetVehicleKit())
                            if (Unit* cannon = veh->GetPassenger(3))
                                cannon->CastSpell(
                                    cannon, SPELL_EMERGENCY_MODE, true);
                    }
                    LMK2->AI()->SetData(1, 1);
                    break;
                }
            ...
            case EVENT_ELEVATOR_INTERVAL_0:
                if (GameObject* elevator = me->FindNearestGameObject(
                        GO_MIMIRON_ELEVATOR, 100.0f)) {
            ...
            case EVENT_ELEVATOR_INTERVAL_1:
                if (me->SummonCreature(NPC_VX001,
                                       2744.65f,
                                       2569.46f,
                                       364.40f,
                                       3.14f,
            ...
            case EVENT_VX001_EMOTESTATE_DEATH:
                if (Creature* VX001 = GetVX001()) {
                    ...
                    VX001->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                          EMOTE_STATE_DROWNED);
            ...
            case EVENT_GET_OUT_VX001:
                if (Creature* VX001 = GetVX001())
                    if (Creature* ACU =
                            me->SummonCreature(NPC_AERIAL_COMMAND_UNIT,
                                               2743.91f,
                                               2568.78f,
                                               391.34f,
                                               M_PI,
                        ...
                        float speed =
                            ACU->GetDistance(2737.75f, 2574.22f, 381.34f) /
                            2.0f;
                        ACU->MonsterMoveWithSpeed(
                            2737.75f, 2574.22f, 381.34f, speed);
            ...
            case EVENT_LEVIATHAN_COME_CLOSER:
                if (Creature* LMK2 = GetLMK2()) {
                    LMK2->GetMotionMaster()->MoveCharge(
                        2755.77f, 2574.95f, 364.31f, 21.0f);
            ...
            case EVENT_LEVIATHAN_RIDE_MIDDLE: {
                ...
                LMK2->GetMotionMaster()->MoveCharge(
                    2744.65f, 2569.46f, 364.31f, 21.0f);
                VX001->SetUInt32Value(UNIT_NPC_EMOTESTATE,
            ...
            case EVENT_START_PHASE4: {
                ...
                LMK2->CastSpell(
                    LMK2,
                    SPELL_SELF_REPAIR,
                    true); // LMK2->SetHealth( LMK2->GetMaxHealth()/2 );
                VX001->CastSpell(
                    VX001,
                    SPELL_SELF_REPAIR,
                    true); // VX001->SetHealth( VX001->GetMaxHealth()/2 );
                ACU->CastSpell(
                    ACU,
                    SPELL_SELF_REPAIR,
                    true); // ACU->SetHealth( ACU->GetMaxHealth()/2 );
                if (hardmode) {
                    LMK2->CastSpell(LMK2, SPELL_EMERGENCY_MODE, true);
                    VX001->CastSpell(VX001, SPELL_EMERGENCY_MODE, true);
                    ACU->CastSpell(ACU, SPELL_EMERGENCY_MODE, true);
            ...
            case EVENT_FINISH: {
                ...
                if (pInstance)
                    for (uint16 i = 0; i < 3; ++i)
                        if (ObjectGuid guid = pInstance->GetGuidData(
                                DATA_GO_MIMIRON_DOOR_1 + i))
                            if (GameObject* door =
                                    ObjectAccessor::GetGameObject(*me, guid))
            ...
            case EVENT_SAY_VOLTRON_DEAD:
                ...
                // spawn chest
                if (uint32 chestId =
                        (hardmode ? RAID_MODE(GO_MIMIRON_CHEST_HARD,
                                              GO_MIMIRON_CHEST_HERO_HARD)
                                  : RAID_MODE(GO_MIMIRON_CHEST,
                                              GO_MIMIRON_CHEST_HERO))) {
                    if (GameObject* go = me->SummonGameObject(chestId,
                                                              2744.65f,
                                                              2569.46f,
                                                              364.397f,
                                                              0,
        ...
        void ResetGameObjects()
        {
            if (pInstance)
                for (uint16 i = 0; i < 3; ++i)
                    if (ObjectGuid guid =
                            pInstance->GetGuidData(DATA_GO_MIMIRON_DOOR_1 + i))
                        if (GameObject* door =
                                ObjectAccessor::GetGameObject(*me, guid))
                            if (door->GetGoState() != GO_STATE_ACTIVE) {
                                door->SetLootState(GO_READY);
            ...
            if (GameObject* elevator =
                    me->FindNearestGameObject(GO_MIMIRON_ELEVATOR, 200.0f)) {
                if (elevator->GetGoState() != GO_STATE_ACTIVE) {
                    elevator->SetLootState(GO_READY);
                    elevator->SetByteValue(
                        GAMEOBJECT_BYTES_1, 0, GO_STATE_ACTIVE);
                }
                elevator->EnableCollision(false);
            }

            if (GameObject* button =
                    me->FindNearestGameObject(GO_BUTTON, 200.0f))
        ...
        void SetData(uint32 /*id*/, uint32 value) override
        {
            ...
            case 6: {
                ...
                if (s1 && s2 && s3 &&
                    s1->GetSpellInfo()->Id == SPELL_SELF_REPAIR &&
                    s2->GetSpellInfo()->Id == SPELL_SELF_REPAIR &&
                    s3->GetSpellInfo()->Id == SPELL_SELF_REPAIR)
    ...
    struct npc_ulduar_leviathan_mkiiAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (Creature* c = me->SummonCreature(
                    NPC_LEVIATHAN_MKII_CANNON, *me, TEMPSUMMON_MANUAL_DESPAWN))
                c->EnterVehicle(me, 3);
            me->SetUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
            ...
            if (id == 1) // setting phase to start fighting
            {
                ...
                case 1:
                    ...
                    if (Creature* c = GetMimiron())
                        if (c->AI()->GetData(1))
                            events.ScheduleEvent(EVENT_FLAME_SUPPRESSION_50000,
                                                 60s);
                    break;
                case 4:
                    me->SetReactState(REACT_AGGRESSIVE);
                ...
                if (Phase == 1) {
                    if (!me->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE)) {
                        ...
                        me->GetMotionMaster()->MoveCharge(
                            2795.076f, 2598.616f, 364.32f, 21.0f);
                        if (Creature* c = GetMimiron())
                            c->AI()->SetData(0, 1);
                    }
                }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            Unit* cannon = GetS3();
            if (!cannon || cannon->HasUnitState(UNIT_STATE_CASTING) ||
...
public:
    ...
    struct npc_ulduar_vx001AI : public ScriptedAI {
        ...
        void SetData(uint32 id, uint32 value) override
        {
            if (id == 1) // setting phase to start fighting
            {
                ...
                case 2:
                    ...
                    if (Creature* c = GetMimiron())
                        if (c->AI()->GetData(1)) {
                            events.ScheduleEvent(EVENT_FLAME_SUPPRESSION_10,
                                                 7s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (spinningUpTimer) // executed about a second after starting
                                 // casting to ensure players can see the
                                 // correct direction
            {
                ...
                if (Vehicle* vk = me->GetVehicleKit()) {
                    for (int i = 0; i < (Phase / 2); ++i) {
                        ...
                        if (Unit* r = vk->GetPassenger(5 + index))
                            if (Player* temp =
                                    SelectTargetFromPlayerList(100.0f)) {
                                if (Creature* trigger = me->SummonCreature(
                                        NPC_ROCKET_STRIKE_N,
                                        temp->GetPositionX(),
                                        temp->GetPositionY(),
                                        temp->GetPositionZ(),
                                        ...
                                        TEMPSUMMON_TIMED_DESPAWN,
                                        6000))
                                    trigger->CastSpell(trigger,
                                                       SPELL_ROCKET_STRIKE_AURA,
                                                       true);
                                Position exitPos = r->GetPosition();
                                exitPos.m_positionX +=
                                    cos(me->GetOrientation()) * 2.35f;
                                exitPos.m_positionY +=
                                    std::sin(me->GetOrientation()) * 2.35f;
                                exitPos.m_positionZ += 2.0f * Phase;
                                r->_ExitVehicle(&exitPos);
                                me->RemoveAurasByType(
                                    SPELL_AURA_CONTROL_VEHICLE, r->GetGUID());
                                if (r->GetTypeId() == TYPEID_UNIT)
                ...
                if (Vehicle* vk = me->GetVehicleKit()) {
                    for (uint8 i = 5; i <= 6; ++i)
                        if (!vk->GetPassenger(i))
                            if (TempSummon* accessory = me->SummonCreature(
                                    NPC_ROCKET_VISUAL,
                                    me->GetPositionX(),
                                    me->GetPositionY(),
            ...
            case EVENT_HAND_PULSE:
                if (Player* p = SelectTargetFromPlayerList(80.0f)) {
                    ...
                    if (Unit* vb = me->GetVehicleBase()) {
                        ...
                        vb->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                           EMOTE_ONESHOT_NONE);

                        if (!leftarm) {
                            vb->HandleEmoteCommand(
            ...
            case EVENT_SPELL_SPINNING_UP:
                ...
                if (Player* p = SelectTargetFromPlayerList(80.0f)) {
                    ...
                    spinningUpOrientation =
                        (uint32)((angle * 100.0f) / (2 * M_PI));
                    spinningUpTimer = 1500;
                    me->SetFacingTo(angle);
                    me->CastSpell(p, SPELL_SPINNING_UP, true);
                    if (Unit* vehicle = me->GetVehicleBase()) {
                        vehicle->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                                EMOTE_STATE_CUSTOM_SPELL_01);
                        vehicle->HandleEmoteCommand(
                            EMOTE_STATE_CUSTOM_SPELL_01);
                    }
                    events.RescheduleEvent((Phase == 2 ? EVENT_SPELL_RAPID_BURST
                                                       : EVENT_HAND_PULSE),
                                           14s + 500ms);
                }
                break;
            ...
            case EVENT_FROST_BOMB:
                me->CastCustomSpell(SPELL_VX001_FROST_BOMB,
...
public:
    npc_ulduar_aerial_command_unit()
        : CreatureScript("npc_ulduar_aerial_command_unit")
    {
    }
    ...
    struct npc_ulduar_aerial_command_unitAI : public ScriptedAI {
                    ...
                    if (Creature* c = GetMimiron())
                        if (c->AI()->GetData(1))
                            events.ScheduleEvent(
                                EVENT_SUMMON_EMERGENCY_FIRE_BOTS, 0ms);
                    break;
                case 4:
                    me->SetReactState(REACT_AGGRESSIVE);
                    DoResetThreatList();
                ...
                if (Phase == 3) {
                    if (!me->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE)) {
                        ...
                        me->MonsterMoveWithSpeed(
                            ...
                            2569.46f,
                            381.34f,
                            me->GetDistance(2744.65f, 2569.46f, 381.34f));
                        me->UpdatePosition(
                            2744.65f, 2569.46f, 381.34f, M_PI, false);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (Phase == 3 && !immobilized)
                if (Unit* victim = me->GetVictim())
                    if (me->GetExactDist2d(victim) > 25.0f) {
                        float angle = victim->GetAngle(me->GetPositionX(),
                                                       me->GetPositionY());
                        me->SetOrientation(me->GetAngle(
                            victim->GetPositionX(), victim->GetPositionY()));
                        float x = victim->GetPositionX() + 15.0f * cos(angle);
                        float y =
                            victim->GetPositionY() + 15.0f * std::sin(angle);

                        // check if there's magnetic core in line of movement
                        Creature*            mc = nullptr;
                        std::list<Creature*> cl;
                        me->GetCreaturesWithEntryInRange(
                            cl, me->GetExactDist2d(victim), NPC_MAGNETIC_CORE);
                        for (std::list<Creature*>::iterator itr = cl.begin();
                             itr != cl.end();
                             ++itr) {
                            if ((*itr)->IsInBetween(me, victim, 4.0f) &&
                                (*itr)->GetExactDist2d(victim) >=
                                    10.0f) // don't come very close just because
                                           // there's a magnetic core
                            {
                                x  = (*itr)->GetPositionX();
                                y  = (*itr)->GetPositionY();
                                mc = (*itr);
                        ...
                        me->UpdatePosition(
                            x, y, 381.34f, me->GetAngle(victim), false);
                        if (mc) {
                            mc->AI()->SetData(0, 0);
                            SetData(2, 1);
                        }
                ...
                if (!immobilized) {
                    if (Phase == 3) {
                        ...
                        if (Unit* victim = SelectTarget(
                                SelectTargetMethod::Random, 0, 27.5f, true)) {
            ...
            case EVENT_SUMMON_ASSAULT_BOT:
                if (GameObject* pad = me->FindNearestGameObject(
                        RAND(194742, 194746, 194745), 200.0f))
                    if (Creature* trigger =
                            me->SummonCreature(NPC_BOT_SUMMON_TRIGGER,
                                               *pad,
                                               TEMPSUMMON_TIMED_DESPAWN,
                                               15000))
            ...
            case EVENT_SUMMON_JUNK_BOT:
                if (GameObject* pad = me->FindNearestGameObject(
                        RAND(194741, 194744, 194747), 200.0f))
                    if (Creature* trigger =
                            me->SummonCreature(NPC_BOT_SUMMON_TRIGGER,
                                               *pad,
                                               TEMPSUMMON_TIMED_DESPAWN,
            ...
            case EVENT_SUMMON_EMERGENCY_FIRE_BOTS: {
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (GameObject* pad =
                            me->FindNearestGameObject(ids[i], 200.0f))
                        if (Creature* trigger =
                                me->SummonCreature(NPC_BOT_SUMMON_TRIGGER,
                                                   *pad,
                                                   TEMPSUMMON_MANUAL_DESPAWN))
            ...
            case EVENT_MAGNETIC_CORE_PULL_DOWN:
                ...
                me->MonsterMoveWithSpeed(me->GetPositionX(),
                                         me->GetPositionY(),
                                         365.34f,
                                         me->GetExactDist(me->GetPositionX(),
                                                          me->GetPositionY(),
                                                          365.34f));
...
public:
    ...
    struct npc_ulduar_proximity_mineAI : public ScriptedAI {
        ...
        {
            if (target && spell && target->GetTypeId() == TYPEID_PLAYER &&
                spell->Id == SPELL_MINE_EXPLOSION)
...
public:
    ...
    struct npc_ulduar_mimiron_rocketAI : public NullCreatureAI {
        npc_ulduar_mimiron_rocketAI(Creature* pCreature)
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            if (!me->GetVehicle()) {
                me->SetSpeed(
                    MOVE_RUN, me->GetSpeedRate(MOVE_RUN) + 0.4f, false);
...
public:
    ...
    struct npc_ulduar_magnetic_coreAI : public NullCreatureAI {
        npc_ulduar_magnetic_coreAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
            pInstance = me->GetInstanceScript();
            if (Creature* c = GetACU())
                if (c->GetExactDist2d(me) <= 10.0f) {
                    me->SendMonsterMove(
                        c->GetPositionX(), c->GetPositionY(), 364.313f, 1);
                    me->UpdatePosition(c->GetPositionX(),
                                       c->GetPositionY(),
                                       364.313f,
                                       me->GetOrientation(),
                                       true);
...
public:
    npc_ulduar_bot_summon_trigger()
        : CreatureScript("npc_ulduar_bot_summon_trigger")
    {
    }
    ...
    struct npc_ulduar_bot_summon_triggerAI : public NullCreatureAI {
        npc_ulduar_bot_summon_triggerAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
        }
        ...
        {
            if (timer <= diff) {
                uint32 option_npcid[3] = {
                    NPC_JUNK_BOT, NPC_ASSAULT_BOT, NPC_EMERGENCY_FIRE_BOT};
                InstanceScript* pInstance = me->GetInstanceScript();
                if (Creature* ACU = GetACU()) // ACU summons for easy removing
                    if (Creature* bot =
                            ACU->SummonCreature(option_npcid[option - 1],
                                                *me,
...
public:
    ...
    class spell_mimiron_rapid_burst_AuraScript : public AuraScript {
        ...
        {
            if (Unit* c = GetCaster()) {
                uint32 id = (c->GetMap()->Is25ManRaid()
                                 ? ((aurEff->GetTickNumber() % 2)
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_mimiron_rapid_burst_AuraScript::HandleEffectPeriodic,
                EFFECT_1,
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            if (Unit* c = GetCaster()) {
                if (c->GetTypeId() != TYPEID_UNIT)
                    return;
                uint32 diff = getMSTimeDiff(lastMSTime,
                                            GameTime::GetGameTimeMS().count());
                if (lastOrientation == -1.0f) {
                    lastOrientation =
                        (c->ToCreature()->AI()->GetData(0) * 2 * M_PI) / 100.0f;
                    diff = 0;
                }
                float new_o = Position::NormalizeOrientation(
                    lastOrientation - (M_PI / 60) * (diff / 250.0f));
                lastMSTime      = GameTime::GetGameTimeMS().count();
...
public:
    go_ulduar_do_not_push_this_button()
        : GameObjectScript("go_ulduar_do_not_push_this_button")
    {
    }
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        if (InstanceScript* instance = go->GetInstanceScript()) {
            ...
            if (Creature* c = ObjectAccessor::GetCreature(
                    *go, instance->GetGuidData(TYPE_MIMIRON))) {
                c->AI()->SetData(0, 7);
...
public:
    ...
    struct npc_ulduar_flames_initialAI : public NullCreatureAI {
        npc_ulduar_flames_initialAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
            CreateTime = GameTime::GetGameTime().count();
            events.Reset();
            events.ScheduleEvent(EVENT_FLAMES_SPREAD, 5750ms);
            if (Creature* flame = me->SummonCreature(NPC_FLAMES_SPREAD,
                                                     me->GetPositionX(),
                                                     me->GetPositionY(),
                                                     364.32f,
                                                     0.0f)) {
        ...
        {
            if (Creature* flame = me->SummonCreature(
                    NPC_FLAMES_SPREAD, x, y, 364.32f, 0.0f)) {
                FlameList.push_back(flame->GetGUID());
                if (Creature* c =
                        me->FindNearestCreature(NPC_FLAMES_SPREAD, 10.0f))
                    if (c->GetExactDist2d(flame->GetPositionX(),
                                          flame->GetPositionY()) <= 4.0f)
                        return;
                flame->CastSpell(flame, SPELL_FLAMES_AURA, true);
            }
        }

        void RemoveFlame(ObjectGuid guid) { FlameList.remove(guid); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_FLAMES_SPREAD: {
                ...
                Creature* last =
                    ObjectAccessor::GetCreature(*me, FlameList.back());
                if (last) {
                    ...
                    for (Map::PlayerList::const_iterator itr = pl.begin();
                        ...
                        if (Player* plr = itr->GetSource())
                            if (plr->IsAlive() &&
                                plr->GetExactDist2d(last) < prevdist &&
                                !plr->IsGameMaster()) {
                                target   = plr;
                    ...
                    if (target &&
                        prevdist >= 4.0f) // no need to spread when player is
                                          // standing in fire, check distance
                    {
                        float angle = last->GetAngle(target->GetPositionX(),
                                                     target->GetPositionY()) -
                                      M_PI / 8 + rand_norm() * 2 * M_PI / 8;
                        SpreadFlame(last->GetPositionX() + 7.0f * cos(angle),
                                    last->GetPositionY() +
                                        7.0f * std::sin(angle));
                    }
                }
...
public:
    ...
    struct npc_ulduar_flames_spreadAI : public NullCreatureAI {
        npc_ulduar_flames_spreadAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
        }
                ...
                if (me->IsSummon())
                    if (Unit* summoner = me->ToTempSummon()->GetSummonerUnit())
                        if (Creature* c = summoner->ToCreature())
                            if (c->AI())
                                CAST_AI(npc_ulduar_flames_initial::
                                            npc_ulduar_flames_initialAI,
                                        c->AI())
                                    ->RemoveFlame(me->GetGUID());
...
public:
    npc_ulduar_emergency_fire_bot()
        : CreatureScript("npc_ulduar_emergency_fire_bot")
    {
    }
    ...
    struct npc_ulduar_emergency_fire_botAI : public ScriptedAI {
        npc_ulduar_emergency_fire_botAI(Creature* pCreature)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_EMERGENCY_BOT_CHECK:
                events.Repeat(15s);
                if (Creature* flame = me->FindNearestCreature(
                        NPC_FLAMES_SPREAD, 150.0f, true)) {
                    me->SetOrientation(me->GetAngle(flame->GetPositionX(),
                                                    flame->GetPositionY()));
                    ...
                    if (dist <= 5.0f)
                        ...
                        me->GetMotionMaster()->MovePoint(
                            1,
                            me->GetPositionX() +
                                (dist - 5.0f) * cos(me->GetOrientation()),
                            me->GetPositionY() +
                                (dist - 5.0f) * sin(me->GetOrientation()),
                            364.32f);
                }
                break;
...
public:
    npc_ulduar_rocket_strike_trigger()
        : CreatureScript("npc_ulduar_rocket_strike_trigger")
    {
    }
    ...
    struct npc_ulduar_rocket_strike_triggerAI : public NullCreatureAI {
        npc_ulduar_rocket_strike_triggerAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
        }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            if (spell->Id == 63041) {
                if (target->GetEntry() == NPC_ASSAULT_BOT)
                    me->CastSpell(
                        me, 65040, true); // achievement Not-So-Friendly Fire
...
public:
    achievement_mimiron_firefighter()
        : AchievementCriteriaScript("achievement_mimiron_firefighter")
    {
    }
    ...
    {
        return target && target->GetEntry() == NPC_MIMIRON &&
...
public:
    achievement_mimiron_set_up_us_the_bomb_11()
        : AchievementCriteriaScript("achievement_mimiron_set_up_us_the_bomb_11")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_MIMIRON &&
               target->GetTypeId() == TYPEID_UNIT &&
               !target->ToCreature()->AI()->GetData(11);
    }
};
...
class achievement_mimiron_set_up_us_the_bomb_12
    : public AchievementCriteriaScript {
public:
    achievement_mimiron_set_up_us_the_bomb_12()
        : AchievementCriteriaScript("achievement_mimiron_set_up_us_the_bomb_12")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_MIMIRON &&
               target->GetTypeId() == TYPEID_UNIT &&
               !target->ToCreature()->AI()->GetData(12);
    }
};
...
class achievement_mimiron_set_up_us_the_bomb_13
    : public AchievementCriteriaScript {
public:
    achievement_mimiron_set_up_us_the_bomb_13()
        : AchievementCriteriaScript("achievement_mimiron_set_up_us_the_bomb_13")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_MIMIRON &&
               target->GetTypeId() == TYPEID_UNIT &&
               !target->ToCreature()->AI()->GetData(13);
    }
};
