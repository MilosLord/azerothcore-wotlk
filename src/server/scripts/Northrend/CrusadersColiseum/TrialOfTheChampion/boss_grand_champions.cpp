...
enum MountSpells {
    ...
    SPELL_BOSS_CHARGE =
        68301, // triggers SPELL_MINIONS_CHARGE (should be with custom damage?)
...
#define SPELL_CHAIN_LIGHTNING                                                  \
    DUNGEON_MODE(SPELL_CHAIN_LIGHTNING_N, SPELL_CHAIN_LIGHTNING_H)
#define SPELL_HEALING_WAVE                                                     \
    DUNGEON_MODE(SPELL_HEALING_WAVE_N, SPELL_HEALING_WAVE_H)
...
#define SPELL_MORTAL_STRIKE                                                    \
    DUNGEON_MODE(SPELL_MORTAL_STRIKE_N, SPELL_MORTAL_STRIKE_H)
...
public:
    ...
    struct npc_toc5_player_vehicleAI : public NullCreatureAI {
        npc_toc5_player_vehicleAI(Creature* pCreature)
...
public:
    npc_toc5_grand_champion_minion()
        : CreatureScript("npc_toc5_grand_champion_minion")
    {
    }

    CreatureAI* GetAI(Creature* pCreature) const override
    {
        return GetTrialOfTheChampionAI<npc_toc5_grand_champion_minionAI>(
            pCreature);
    }

    struct npc_toc5_grand_champion_minionAI : public ScriptedAI {
        npc_toc5_grand_champion_minionAI(Creature* pCreature)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_MOUNT_CHARGE: {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                    ...
                    if (Player* plr = itr->GetSource()) {
                        if (me->GetExactDist(plr) < 8.0f ||
                            me->GetExactDist(plr) > 25.0f || plr->isDead())
                            continue;
                        if (!plr->GetVehicle())
                ...
                if (!LIST.empty()) {
                    ...
                    if (Unit* target =
                            ObjectAccessor::GetUnit(*me, LIST.at(rnd))) {
            ...
            case EVENT_SHIELD_BREAKER: {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                     ...
                     ++itr)
                    if (Player* plr = itr->GetSource()) {
                        if (me->GetExactDist(plr) < 10.0f ||
                            me->GetExactDist(plr) > 30.0f)
                            continue;
                ...
                if (!LIST.empty()) {
                    uint8 rnd = LIST.size() > 1 ? urand(0, LIST.size() - 1) : 0;
                    if (Unit* target =
                            ObjectAccessor::GetCreature(*me, LIST.at(rnd)))
            ...
            case EVENT_THRUST:
                if (me->GetVictim() &&
                    me->GetExactDist(me->GetVictim()) <= 5.5f)
                    me->CastSpell(
                        me->GetVictim(), SPELL_PLAYER_VEHICLE_THRUST, false);
                events.Repeat(3s, 5s);
                break;
            }
        }
...
public:
    ...
    struct boss_grand_championAI : public npc_escortAI {
        boss_grand_championAI(Creature* pCreature) : npc_escortAI(pCreature)
        {
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_MECHANIC, MECHANIC_DISORIENTED, true);
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_MECHANIC, MECHANIC_POLYMORPH, true);
        ...
        void ScheduleAbilitiesEvents()
        {
            ...
            case NPC_RUNOK:   // Runok Wildmane
                ...
                events.RescheduleEvent(
                    EVENT_SHAMAN_SPELL_EARTH_SHIELD, 30s, 35s);
                events.RescheduleEvent(EVENT_SHAMAN_SPELL_HEALING_WAVE, 12s);
                events.RescheduleEvent(
                    EVENT_SHAMAN_SPELL_HEX_OF_MENDING, 20s, 25s);
                break;
            ...
            case NPC_MOKRA: // Mokra the Skullcrusher
                events.RescheduleEvent(
                    EVENT_WARRIOR_SPELL_MORTAL_STRIKE, 8s, 12s);
                events.RescheduleEvent(
                    EVENT_WARRIOR_SPELL_BLADESTORM, 15s, 20s);
        ...
        void AddCreatureAddonAuras()
        {
            ...
            if (!cainfo->auras.empty()) {
                for (std::vector<uint32>::const_iterator itr =
                         cainfo->auras.begin();
                ...
                if (me->GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID) == 0)
                    ...
                    if (pInstance) {
                        ...
                        if (Creature* mount = me->FindNearestCreature(
                                pInstance->GetData(DATA_TEAMID_IN_INSTANCE) ==
                                        TEAM_HORDE
        ...
        void WaypointReached(uint32 i) override
        {
            ...
            if ((i == 2 && (BossOrder == 1 || BossOrder == 2)) ||
                (i == 3 && BossOrder == 0))
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (type == POINT_MOTION_TYPE) {
                ...
                {
                    if (NewMountGUID)
                        if (Creature* mount = ObjectAccessor::GetCreature(
                                *me, NewMountGUID)) {
                            mount->DespawnOrUnsummon();
                            me->SetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID,
                                               mount->GetDisplayId());
                            me->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                            me->SetHealth(50000);
                            me->CastSpell(me, SPELL_BOSS_DEFEND_PERIODIC, true);
                            ...
                            events.ScheduleEvent(
                                EVENT_MOUNT_CHARGE, 2500ms, 4000ms);
                            events.ScheduleEvent(EVENT_SHIELD_BREAKER, 5s, 8s);
                            events.ScheduleEvent(EVENT_THRUST, 3s, 5s);
                            me->SetReactState(REACT_AGGRESSIVE);
                            me->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->HasUnitState(UNIT_STATE_CASTING) ||
                ((me->GetEntry() == NPC_JACOB || me->GetEntry() == NPC_MOKRA) &&
            ...
            case EVENT_FIND_NEW_MOUNT: {
                ...
                // hackfix, trample won't hit grand champions because of
                ...
                if (Creature* mount =
                        ObjectAccessor::GetCreature(*me, NewMountGUID))
                    if (mount->IsAlive()) {
            ...
            case EVENT_THRUST:
                if (Unit* victim = me->GetVictim())
                    if (me->GetExactDist(victim) <= 6.0f)
                        me->CastSpell(
                            victim, SPELL_PLAYER_VEHICLE_THRUST, false);
                events.Repeat(3s, 5s);
                break;
                ...
                if (!UnitTargetGUID) {
                    ...
                    if (target && me->IsInRange(target, 5.0f, 30.0f, false))
                        ...
                        for (Map::PlayerList::const_iterator itr = pl.begin();
                            ...
                            if (player &&
                                me->IsInRange(player, 5.0f, 30.0f, false)) {
