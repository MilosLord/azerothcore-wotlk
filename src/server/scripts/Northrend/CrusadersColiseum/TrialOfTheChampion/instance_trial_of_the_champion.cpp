...
/**
 *  @todo: Missing dialog/RP (already populated in DB) && spawns (can use ToC25
 * locations?) for:
 *
 ...
 *   Thrall 34994
 *
 *  And possibly NPC_TIRION 33628 is wrong (should be 34996, from ToC25, needs a
 * sniff to confirm, check .h)
 */
...
public:
    instance_trial_of_the_champion()
        : InstanceMapScript("instance_trial_of_the_champion", 650)
    {
    }
    ...
    struct instance_trial_of_the_champion_InstanceMapScript
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            if (creature->GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID))
                if (const CreatureAddon* ca = creature->GetCreatureAddon())
                    if (ca->mount !=
                        creature->GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID))
                        creature->SetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID,
                                                 ca->mount);
        }
        ...
        void InstanceCleanup()
        {
                    ...
                    for (uint8 i = 0; i < 3; ++i) {
                        for (uint8 j = 0; j < 3; ++j) {
                            if (Creature* c = instance->GetCreature(
                                    NPC_GrandChampionMinionsGUID[i][j]))
                                c->DespawnOrUnsummon();
                            NPC_GrandChampionMinionsGUID[i][j].Clear();
                        }
                        if (Creature* c =
                                instance->GetCreature(NPC_GrandChampionGUID[i]))
                            c->DespawnOrUnsummon();
                        NPC_GrandChampionGUID[i].Clear();
                    }
                    if (Creature* c =
                            instance->GetCreature(NPC_AnnouncerGUID)) {
                        c->DespawnOrUnsummon();
                        c->SetHomePosition(
                            748.309f, 619.488f, 411.172f, 4.71239f);
            ...
            case INSTANCE_PROGRESS_CHAMPIONS_UNMOUNTED: {
                if (Creature* announcer =
                        instance->GetCreature(NPC_AnnouncerGUID)) {
                    announcer->DespawnOrUnsummon();
                    announcer->SetHomePosition(
                        735.81f, 661.92f, 412.39f, 4.714f);
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* c = instance->GetCreature(
                                NPC_GrandChampionGUID[i])) {
                            uint32 entry = c->GetEntry();
                            c->DespawnOrUnsummon();
                            switch (i) {
                            case 0:
                                if (Creature* pBoss = announcer->SummonCreature(
                                        entry,
                                        736.695f,
                                        650.02f,
                                        412.4f,
            ...
            case INSTANCE_PROGRESS_ARGENT_SOLDIERS_DIED:
                // revert to INSTANCE_PROGRESS_CHAMPIONS_DEAD
                {
                    for (uint8 i = 0; i < 3; ++i)
                        for (uint8 j = 0; j < 3; ++j) {
                            if (Creature* c = instance->GetCreature(
                                    NPC_ArgentSoldierGUID[i][j]))
                                c->DespawnOrUnsummon();
                            NPC_ArgentSoldierGUID[i][j].Clear();
                        }
                    if (Creature* c =
                            instance->GetCreature(NPC_ArgentChampionGUID)) {
            ...
            case INSTANCE_PROGRESS_ARGENT_CHALLENGE_DIED:
                ...
                {
                    if (Creature* c =
                            instance->GetCreature(NPC_BlackKnightVehicleGUID))
                        c->DespawnOrUnsummon();
                    NPC_BlackKnightVehicleGUID.Clear();
                    if (Creature* c =
                            instance->GetCreature(NPC_BlackKnightGUID)) {
                        c->AI()->DoAction(-1);
                        c->DespawnOrUnsummon();
                    }
        ...
        void SetData(uint32 uiType, uint32 uiData) override
        {
                ...
                case INSTANCE_PROGRESS_INITIAL:
                    if (uiData == 0) // normal intro
                    {
                        ...
                        if (Creature* announcer =
                                instance->GetCreature(NPC_AnnouncerGUID)) {
                            if (GameObject* gate =
                                    instance->GetGameObject(GO_MainGateGUID))
                    ...
                    else // short version
                    {
                        ...
                        if (Creature* announcer =
                                instance->GetCreature(NPC_AnnouncerGUID)) {
                            announcer->SetUnitMovementFlags(
                                MOVEMENTFLAG_WALKING);
                            announcer->GetMotionMaster()->MovePoint(
                                1, 735.81f, 661.92f, 412.39f);
                        }
                        events.ScheduleEvent(EVENT_GRAND_GROUP_1_MOVE_MIDDLE,
                                             10s);
                    }
                    break;
                case INSTANCE_PROGRESS_CHAMPIONS_DEAD:
                    ...
                    events.RescheduleEvent(EVENT_START_ARGENT_CHALLENGE_INTRO,
                                           0ms);
                    break;
                case INSTANCE_PROGRESS_ARGENT_CHALLENGE_DIED:
                    if (Creature* tirion =
            ...
            case DATA_MOUNT_DIED:
                switch (InstanceProgress) {
                case INSTANCE_PROGRESS_GRAND_CHAMPIONS_REACHED_DEST: // fighting
                                                                     // group
                                                                     // 1/3
                    if (++Counter >= 3) {
                        Counter = 0;
                        ...
                        uiData = DONE; // save to db
                        events.ScheduleEvent(EVENT_GRAND_GROUP_2_MOVE_MIDDLE,
                                             0ms);
                    }
                    break;
                case INSTANCE_PROGRESS_CHAMPION_GROUP_DIED_1: // fighting group
                                                              // 2/3
                    if (++Counter >= 3) {
                        Counter = 0;
                        InstanceProgress =
                        ...
                        uiData = DONE; // save to db
                        events.ScheduleEvent(EVENT_GRAND_GROUP_3_MOVE_MIDDLE,
                                             0ms);
                    }
                    break;
                case INSTANCE_PROGRESS_CHAMPION_GROUP_DIED_2: // fighting group
                                                              // 3/3
                    if (++Counter >= 3) {
                        Counter = 0;
                        InstanceProgress =
                        ...
                        uiData = DONE; // save to db
                        events.ScheduleEvent(EVENT_GRAND_CHAMPIONS_MOVE_MIDDLE,
                                             0ms);
                    }
                    break;
                case INSTANCE_PROGRESS_CHAMPION_GROUP_DIED_3: // fighting grand
                                                              // champions (on
                                                              // vehicles)
                    if (++Counter >= 3) {
                        Counter = 0;
                        ...
                        for (ObjectGuid const& guid : VehicleList)
                            if (Creature* veh = instance->GetCreature(guid))
                                veh->DespawnOrUnsummon();
                        events.ScheduleEvent(EVENT_GRAND_CHAMPIONS_MOVE_SIDE,
                                             0ms);
                    }
                    break;
                }
            ...
            case DATA_GRAND_CHAMPION_DIED:
                if (++Counter >= 3) {
                    ...
                    if (Creature* announcer =
                            instance->GetCreature(NPC_AnnouncerGUID)) {
                        announcer->GetMotionMaster()->MovePoint(
                            0, 743.14f, 628.77f, 411.2f);
                        announcer->SummonGameObject(instance->IsHeroic()
                                                        ? GO_CHAMPIONS_LOOT_H
                                                        : GO_CHAMPIONS_LOOT,
                                                    746.59f,
                                                    618.49f,
                                                    ...
                                                    0,
                                                    90000000); // [LOOT]
                        events.ScheduleEvent(EVENT_RESTORE_ANNOUNCER_GOSSIP,
                                             15s);
            ...
            case DATA_ARGENT_SOLDIER_DEFEATED:
                if (++Counter >= 9) {
                    ...
                    uiData           = DONE; // save to db
                    events.ScheduleEvent(EVENT_ARGENT_CHALLENGE_MOVE_FORWARD,
                                         0ms);
                }
                break;
            case BOSS_ARGENT_CHALLENGE: {
                ...
                if (uiData == DONE) {
                    HandleGameObject(GO_EnterGateGUID, true);
                    InstanceProgress = INSTANCE_PROGRESS_ARGENT_CHALLENGE_DIED;
                    events.ScheduleEvent(EVENT_ARGENT_CHALLENGE_RUN_MIDDLE,
                                         0ms);
                }
            } break;
        ...
        void DoSummonGrandChampion(uint32 BossNumber, uint8 BossOrder)
        {
            ...
            if (Creature* pBoss =
                    instance->SummonCreature(CHAMPION_TO_SUMMON, SpawnPos)) {
                NPC_GrandChampionGUID[BossOrder] = pBoss->GetGUID();
                pBoss->ToCreature()->SetReactState(REACT_PASSIVE);
                pBoss->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
                                   UNIT_FLAG_PACIFIED);
                pBoss->SetImmuneToAll(true);
                pBoss->ToCreature()->SetHomePosition(
                    748.309f, 619.448f, 411.3f, M_PI / 2);
                pBoss->AI()->SetData(BossOrder, (shortver ? 1 : 0));

                for (uint8 i = 0; i < 3; ++i)
                    if (Creature* pAdd = instance->SummonCreature(
                            MINION_TO_SUMMON, SpawnPos)) {
                        NPC_GrandChampionMinionsGUID[BossOrder][i] =
                            pAdd->GetGUID();
                        pAdd->SetReactState(REACT_PASSIVE);
                        pAdd->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                        pAdd->SetImmuneToAll(true);
                        pAdd->SetHomePosition(
                            748.309f, 619.448f, 411.3f, M_PI / 2);
                        pAdd->GetMotionMaster()->MoveFollow(
                            pBoss, 2.0f, (i + 1) * M_PI / 2);
                    }
            }
            ...
            if (!shortver)
                if (Creature* announcer =
                        instance->GetCreature(NPC_AnnouncerGUID)) {
                    announcer->AI()->Talk(TEXT_ID); /// @todo: Missing Argent
                                                    /// Raid Spectator cheers.
                }
        }
        ...
        void Update(uint32 diff) override
        {
            ...
            case EVENT_GRAND_GROUP_1_MOVE_MIDDLE: {
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (Creature* c = instance->GetCreature(
                            NPC_GrandChampionMinionsGUID[1][i])) {
                        float angle = rand_norm() * 2 * M_PI;
                        c->GetMotionMaster()->MovePoint(
                            0,
                            748.309f + 3.0f * cos(angle),
                            619.448f + 3.0f * std::sin(angle),
                            411.3f);
                    }
            ...
            case EVENT_GRAND_GROUP_2_MOVE_MIDDLE: {
                for (uint8 i = 0; i < 3; ++i)
                    if (Creature* c = instance->GetCreature(
                            NPC_GrandChampionMinionsGUID[0][i])) {
                        float angle = rand_norm() * 2 * M_PI;
                        c->GetMotionMaster()->MovePoint(
                            0,
                            748.309f + 3.0f * cos(angle),
            ...
            case EVENT_GRAND_GROUP_3_MOVE_MIDDLE: {
                for (uint8 i = 0; i < 3; ++i)
                    if (Creature* c = instance->GetCreature(
                            NPC_GrandChampionMinionsGUID[2][i])) {
                        float angle = rand_norm() * 2 * M_PI;
                        c->GetMotionMaster()->MovePoint(
                            0,
                            748.309f + 3.0f * cos(angle),
            ...
            case EVENT_GRAND_CHAMPIONS_MOVE_SIDE: {
                for (uint8 i = 0; i < 3; ++i)
                    if (Creature* c =
                        ...
                        case 0:
                            c->GetMotionMaster()->MovePoint(
                                5, 736.695f, 650.02f, 412.4f);
                            break;
                        case 1:
                            c->GetMotionMaster()->MovePoint(
                                5, 756.32f, 650.05f, 412.4f);
                            break;
                        case 2:
                            c->GetMotionMaster()->MovePoint(
                                5, 746.5f, 650.65f, 411.7f);
                            break;
                        }
                    }
            ...
            case EVENT_START_ARGENT_CHALLENGE_INTRO: {
                ...
                for (int8 i = 0; i < 3; ++i) {
                    ...
                    if (Creature* pTrash = instance->SummonCreature(
                            NPC_ARGENT_LIGHTWIELDER, pos)) {
                        pTrash->AI()->SetData(i, 0);
                        NPC_ArgentSoldierGUID[i][0] = pTrash->GetGUID();
                    }
                    pos.m_positionX = x + (i - 1) * 10.0f;
                    if (Creature* pTrash =
                            instance->SummonCreature(NPC_ARGENT_MONK, pos)) {
                        pTrash->AI()->SetData(i, 0);
                        NPC_ArgentSoldierGUID[i][1] = pTrash->GetGUID();
                    }
                    pos.m_positionX = x + 2.0f + (i - 1) * 10.0f;
                    if (Creature* pTrash =
                            instance->SummonCreature(NPC_PRIESTESS, pos)) {
                        pTrash->AI()->SetData(i, 0);
            ...
            case EVENT_SUMMON_ARGENT_CHALLENGE: {
                if (Creature* announcer =
                        instance->GetCreature(NPC_AnnouncerGUID))
                    announcer->GetMotionMaster()->MovePoint(
                        0, 735.81f, 661.92f, 412.39f);
                if (Creature* boss = instance->SummonCreature(
                        Counter ? NPC_EADRIC : NPC_PALETRESS, SpawnPosition))
                    boss->GetMotionMaster()->MovePoint(
                        0, 746.881f, 660.263f, 411.7f);
                events.ScheduleEvent(EVENT_CLOSE_GATE, 5s);
                events.ScheduleEvent(EVENT_ARGENT_CHALLENGE_SAY_1, 4s);
                events.ScheduleEvent(EVENT_ARGENT_SOLDIER_GROUP_ATTACK,
                                     12s + 500ms);
            } break;
            case EVENT_ARGENT_CHALLENGE_SAY_1: {
                if (Creature* ac =
                        instance->GetCreature(NPC_ArgentChampionGUID))
                    ac->AI()->Talk(Counter ? SAY_EADRIC_INTRO
            ...
            case EVENT_ARGENT_CHALLENGE_RUN_MIDDLE: {
                if (Creature* boss =
                        instance->GetCreature(NPC_ArgentChampionGUID)) {
                    boss->GetMotionMaster()->MovePoint(
                        1, 747.13f, 628.037f, 411.2f);
                    events.ScheduleEvent(EVENT_ARGENT_CHALLENGE_LEAVE_CHEST,
                                         6s);
                }
            } break;
            case EVENT_ARGENT_CHALLENGE_LEAVE_CHEST: {
                if (Creature* announcer =
                    ...
                    if (Creature* boss =
                        ...
                        uint32 chest = 0;
                        if (instance->IsHeroic())
                            chest = (boss->GetEntry() == NPC_EADRIC ||
                                     boss->GetEntry() == NPC_EADRIC_H)
                                        ? GO_EADRIC_LOOT_H
                                        : GO_PALETRESS_LOOT_H;
                        else
                            chest = (boss->GetEntry() == NPC_EADRIC ||
                                     boss->GetEntry() == NPC_EADRIC_H)
                                        ? GO_EADRIC_LOOT
                                        : GO_PALETRESS_LOOT;
                        announcer->SummonGameObject(chest,
                                                    746.59f,
                                                    618.49f,
                                                    411.09f,
                                                    1.42f,
            ...
            case EVENT_SUMMON_BLACK_KNIGHT: {
                if (Creature* announcer =
                        instance->GetCreature(NPC_AnnouncerGUID))
                    if (Creature* bk_vehicle =
                            announcer->SummonCreature(VEHICLE_BLACK_KNIGHT,
                                                      769.834f,
                                                      651.915f,
                                                      447.035f,
                                                      0.0f)) {
                        ...
                        if (Vehicle* v = bk_vehicle->GetVehicleKit())
                            if (Unit* bk = v->GetPassenger(0)) {
                                ...
                                bk->SendMovementFlagUpdate(); // put him on
                                                              // vehicle
                                                              // visually
                                if (bk->GetTypeId() == TYPEID_UNIT)
                                    bk->ToCreature()->SetReactState(
                                        REACT_PASSIVE);
                            }
            ...
            case EVENT_START_BLACK_KNIGHT_SCENE: {
                if (Creature* bk = instance->GetCreature(NPC_BlackKnightGUID)) {
                    Position exitPos = {
                        751.003357f, 638.145508f, 411.570129f, M_PI};
            ...
            case EVENT_BLACK_KNIGHT_CAST_ANNOUNCER: {
                if (Creature* announcer =
                    ...
                    if (Creature* bk =
                            instance->GetCreature(NPC_BlackKnightGUID)) {
                        bk->SetPosition(745.016f,
                                        631.506f,
                                        411.575f,
                                        bk->GetAngle(announcer));
                        ...
                        bk->AddAura(
                            68306,
                            announcer); // spell has attribute player only
                        if (Creature* tirion =
                                instance->GetCreature(NPC_TirionGUID))
            ...
            case EVENT_BLACK_KNIGHT_MOVE_FORWARD: {
                if (Creature* bk = instance->GetCreature(NPC_BlackKnightGUID)) {
                    ...
                    bk->GetMotionMaster()->MovePoint(
                        0, 746.81f, 623.15f, 411.42f);
