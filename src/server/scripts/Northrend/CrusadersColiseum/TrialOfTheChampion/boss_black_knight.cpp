...
#define SPELL_PLAGUE_STRIKE                                                    \
    DUNGEON_MODE(SPELL_PLAGUE_STRIKE_N, SPELL_PLAGUE_STRIKE_H)
#define SPELL_ICY_TOUCH DUNGEON_MODE(SPELL_ICY_TOUCH_N, SPELL_ICY_TOUCH_H)
#define SPELL_DEATH_RESPITE                                                    \
    DUNGEON_MODE(SPELL_DEATH_RESPITE_N, SPELL_DEATH_RESPITE_H)
...
#define SPELL_MARKED_DEATH                                                     \
    DUNGEON_MODE(SPELL_MARKED_DEATH_N, SPELL_MARKED_DEATH_H)
...
public:
    ...
    struct boss_black_knightAI : public ScriptedAI {
        ...
        void DoAction(int32 param) override
        {
            if (param == -1) {
                ...
                me->CastSpell(
                    ...
                    false);
                if (Creature* announcer = pInstance->instance->GetCreature(
                        pInstance->GetGuidData(DATA_ANNOUNCER)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_ANNOUNCER_SAY_ZOMBIE:
                if (pInstance && !summons.empty())
                    if (Creature* ghoul =
                            pInstance->instance->GetCreature(*summons.begin()))
                        if (urand(0, 1))
                            ghoul->Yell(
                                "[Zombie] .... . Brains ....",
                                LANG_UNIVERSAL); /// @todo: Multiple variations
                                                 /// + not always happening,
                                                 /// from video sources, needs
                                                 /// sniff to transition from
                                                 /// DB.
                break;
            ...
            case EVENT_SPELL_MARKED_DEATH:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 0.000000f, true))
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (me->ToTempSummon())
                me->ToTempSummon()->SetTempSummonType(
...
public:
    ...
    struct npc_black_knight_skeletal_gryphonAI : public npc_escortAI {
        npc_black_knight_skeletal_gryphonAI(Creature* pCreature)
    ...
    CreatureAI* GetAI(Creature* pCreature) const override
    {
        return GetTrialOfTheChampionAI<npc_black_knight_skeletal_gryphonAI>(
            pCreature);
    }
};
...
public:
    ...
    struct npc_black_knight_ghoulAI : public ScriptedAI {
        ...
        {
            events.Reset();
            if (me->GetEntry() == NPC_RISEN_JAEREN ||
                me->GetEntry() == NPC_RISEN_ARELAS)
