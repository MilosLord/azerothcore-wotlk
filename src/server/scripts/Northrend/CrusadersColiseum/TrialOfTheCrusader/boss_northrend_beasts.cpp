    ...
    struct npc_snobold_vassalAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (t->isDead()) {
                ...
                if (pInstance)
                    if (Creature* gormok = ObjectAccessor::GetCreature(
                            *me, pInstance->GetGuidData(TYPE_GORMOK)))
                        if (gormok->IsAlive())
            ...
            case EVENT_SPELL_FIRE_BOMB: {
                if (t->GetTypeId() != TYPEID_PLAYER && pInstance) {
                    ...
                    Creature*              gormok = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(TYPE_GORMOK));
                    ...
                    for (Map::PlayerList::const_iterator itr = pl.begin();
                        ...
                        if (Player* p = itr->GetSource())
                            if (p->IsAlive() && p->GetGUID() != TargetGUID &&
                                (!gormok || !p->IsWithinMeleeRange(gormok)))
                                validPlayers.push_back(p->GetGUID());
                    }

                    if (!validPlayers.empty())
                        if (Player* p = ObjectAccessor::GetPlayer(
                                *me,
                                validPlayers.at(
                                    urand(0, validPlayers.size() - 1))))
                            if (Creature* trigger =
                                ...
                                me->CastSpell(
                                    ...
                                    true); // periodic damage aura, speed 14.0f
                                me->CastSpell(
                                    trigger,
                                    SPELL_FIRE_BOMB); // visual + initial damage
                                                      // 4k
                            }
                }
    ...
    struct boss_gormokAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
                    ...
                    for (uint8 i = 0; i < 4; ++i)
                        if (Unit* snobold = vk->GetPassenger(i)) {
                            ...
                            for (Map::PlayerList::const_iterator itr =
                                 ...
                                 ++itr) {
                                if (Player* p = itr->GetSource())
                                    if (p->IsAlive() && !p->GetVehicleKit() &&
                                        !p->IsMounted() && !p->GetVehicle() &&
                                        !p->IsGameMaster())
                                        validPlayers.push_back(p->GetGUID());
                            }
                            ...
                            if (!validPlayers.empty())
                                if (Player* p = ObjectAccessor::GetPlayer(
                                        *me,
                                        validPlayers.at(urand(
                                            0, validPlayers.size() - 1)))) {
                                    snobold->ChangeSeat(4); // switch to hand
                                    me->setAttackTimer(BASE_ATTACK, 3000);
                                    PlayerGUID = p->GetGUID();
                                    events.RescheduleEvent(
                                        EVENT_RELEASE_SNOBOLD, 2500ms);
                                }
            ...
            case EVENT_RELEASE_SNOBOLD: {
                ...
                if (p && p->IsAlive() && !p->GetVehicleKit() &&
                    !p->IsMounted() && !p->GetVehicle()) {
                    if (Vehicle* vk = me->GetVehicleKit())
                        if (Unit* snobold = vk->GetPassenger(4)) {
                            if (snobold->GetTypeId() == TYPEID_UNIT) {
                                CAST_AI(
                                    npc_snobold_vassal::npc_snobold_vassalAI,
                                    snobold->ToCreature()->AI())
...
struct boss_jormungarAI : public ScriptedAI {
    ...
    void ScheduleEvents()
    {
        ...
        if (me->GetDisplayId() == _MODEL_STATIONARY) {
            ...
            events.RescheduleEvent(EVENT_SPELL_SPRAY,
                                   (me->GetEntry() == NPC_ACIDMAW ? 20s : 15s));
            events.RescheduleEvent(EVENT_SPELL_SWEEP, 15s, 30s);
        }
        else {
            me->SetAttackTime(BASE_ATTACK, 2000);
            events.RescheduleEvent(EVENT_SPELL_BITE,
                                   (me->GetEntry() == NPC_ACIDMAW ? 20s : 15s));
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_SUBMERGE: {
            ...
            // second one submerge 1.5sec after the first one, used also for
            // synchronizing
            if (pInstance)
                if (Creature* c = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(_TYPE_OTHER)))
                    c->AI()->DoAction(-1);
        ...
        case EVENT_MOVE_UNDERGROUND: {
            float angle = me->GetAngle(
                      Locs[LOC_CENTER].GetPositionX() + urand(0, 20) - 10.0f,
                      Locs[LOC_CENTER].GetPositionY() + urand(0, 20) - 10.0f),
                  dist = urand(10, 35);
            if (Creature* c = me->SummonCreature(
                    NPC_WORLD_TRIGGER, *me, TEMPSUMMON_TIMED_DESPAWN, 6000)) {
                ...
                c->GetMotionMaster()->MovePoint(
                    0,
                    Locs[LOC_CENTER].GetPositionX() + cos(angle) * dist,
                    Locs[LOC_CENTER].GetPositionY() + std::sin(angle) * dist,
                    me->GetPositionZ());
            }
            me->UpdatePosition(
                Locs[LOC_CENTER].GetPositionX() + cos(angle) * dist,
        ...
        case EVENT_SPELL_SLIME_POOL:
            if (Creature* c = me->SummonCreature(
                    NPC_SLIME_POOL, *me, TEMPSUMMON_TIMED_DESPAWN, 30000))
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        if (pInstance) {
            if (Creature* c = pInstance->instance->GetCreature(
                    pInstance->GetGuidData(_TYPE_OTHER)))
                if (c->IsAlive())
                    c->AI()->DoAction(-2);
...
public:
    ...
    struct boss_icehowlAI : public ScriptedAI {
        boss_icehowlAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            ...
            if (IsHeroic())
                me->ApplySpellImmune(
                    0, IMMUNITY_EFFECT, SPELL_EFFECT_DISPEL, true);
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_MOD_DECREASE_SPEED, true);
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_MOD_SPEED_NOT_STACK, true);
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_MOD_SPEED_ALWAYS, true);
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_MOD_SPEED_SLOW_ALL, true);
            me->ApplySpellImmune(0,
                                 IMMUNITY_STATE,
                                 SPELL_AURA_USE_NORMAL_MOVEMENT_SPEED,
                                 true); // judgement of justice
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_ARCTIC_BREATH:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 90.0f, true))
            ...
            case EVENT_JUMP_MIDDLE:
                ...
                me->SetReactState(REACT_PASSIVE);
                me->AttackStop();
                me->GetMotionMaster()->MoveJump(Locs[LOC_CENTER].GetPositionX(),
            ...
            case EVENT_GAZE:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 500.0f, true)) {
            ...
            case EVENT_JUMP_BACK: {
                ...
                me->GetMotionMaster()->MoveJump(
                    Locs[LOC_CENTER].GetPositionX() + cos(jumpangle) * 35.0f,
                    Locs[LOC_CENTER].GetPositionY() +
                        std::sin(jumpangle) * 35.0f,
                    Locs[LOC_CENTER].GetPositionZ() + 1.0f,
                    40.0f,
                    12.0f);
                ...
                if (pInstance)
                    ...
                    case RAID_DIFFICULTY_10MAN_NORMAL:
                        pInstance->DoRemoveAurasDueToSpellOnPlayers(
                            SPELL_MASSIVE_CRASH);
                        pInstance->DoCastSpellOnPlayers(
