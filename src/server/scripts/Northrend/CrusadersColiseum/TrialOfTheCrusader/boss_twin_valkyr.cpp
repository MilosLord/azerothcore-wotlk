...
/*
    (!) ACTUALLY FJOLA CONTROLLS THE WHOLE FIGHT (SPECIAL ABILITIES, SHARED
   HEALTH, ETC.) SINCE THEY DIE SIMULTANEOUSLY
*/
...
struct boss_twin_valkyrAI : public ScriptedAI {
    boss_twin_valkyrAI(Creature* pCreature) : ScriptedAI(pCreature), summons(me)
    {
        ...
        if (me->GetEntry() == NPC_LIGHTBANE) {
            if (pInstance)
                pInstance->DoStopTimedAchievement(ACHIEVEMENT_TIMED_TYPE_EVENT,
                                                  21853);
    ...
    void DoAction(int32 a) override
    {
        ...
        case -1:
            ...
            if (pInstance && me->GetEntry() == NPC_LIGHTBANE) {
                uint32 essenceId1 = 0, empoweredId1 = 0, touchId1 = 0,
                       essenceId2 = 0, empoweredId2 = 0, touchId2 = 0;
        ...
        return ObjectAccessor::GetCreature(
            *me,
            pInstance->GetGuidData(
                me->GetEntry() == NPC_DARKBANE ? NPC_LIGHTBANE : NPC_DARKBANE));
    }
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        Talk(SAY_AGGRO);
        me->CastSpell(me,
                      me->GetEntry() == NPC_LIGHTBANE ? SPELL_LIGHT_SURGE
                                                      : SPELL_DARK_SURGE,
                      true);
        ...
        if (me->GetEntry() == NPC_LIGHTBANE)
            if (Creature* twin = GetSister())
                if (twin->IsAlive() && me->IsAlive()) {
                    int32 d =
                        CAST_AI(boss_twin_valkyrAI, twin->AI())->LastSynchroHP -
                        (int32)twin->GetHealth();
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_SUMMON_BALLS_3: {
            ...
            for (uint8 i = 0; i < count; ++i) {
                ...
                if (Creature* ball = me->SummonCreature(
                        (i % 2) ? NPC_CONCENTRATED_DARK
        ...
        case EVENT_SPELL_TOUCH: {
            ...
            /*
            if( Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0.0f,
            true, essenceId) ) me->CastSpell(target,
            me->GetEntry()==NPC_LIGHTBANE ? SPELL_LIGHT_TOUCH :
            SPELL_DARK_TOUCH, false); events.RepeatEvent(urand(45000,50000));
            */
        ...
        case EVENT_SPECIAL: {
            ...
            if ((SpecialMask & 0xF) == 0xF)
                SpecialMask = 0;
            events.Repeat(45s);
            events.DelayEventsToMax(
                15000,
                1); // no touch of light/darkness during special abilities!
        } break;
    ...
    void SummonedCreatureDespawn(Creature* s) override { summons.Despawn(s); }
...
public:
    ...
    struct boss_eydisAI : public boss_twin_valkyrAI {
        boss_eydisAI(Creature* pCreature) : boss_twin_valkyrAI(pCreature)
        {
            ...
            SetEquipmentSlots(
                false, EQUIP_MAIN_2, EQUIP_OFFHAND_2, EQUIP_RANGED_2);
            if (Creature* c =
                    me->SummonCreature(NPC_DARK_ESSENCE, Locs[LOC_DARKESS_1]))
                boss_twin_valkyrAI::JustSummoned(c);
            if (Creature* c =
                    me->SummonCreature(NPC_DARK_ESSENCE, Locs[LOC_DARKESS_2]))
                boss_twin_valkyrAI::JustSummoned(c);
        }
...
public:
    ...
    struct boss_fjolaAI : public boss_twin_valkyrAI {
        boss_fjolaAI(Creature* pCreature) : boss_twin_valkyrAI(pCreature)
        {
            ...
            SetEquipmentSlots(
                false, EQUIP_MAIN_1, EQUIP_OFFHAND_1, EQUIP_RANGED_1);
            if (Creature* c =
                    me->SummonCreature(NPC_LIGHT_ESSENCE, Locs[LOC_LIGHTESS_1]))
                boss_twin_valkyrAI::JustSummoned(c);
            if (Creature* c =
                    me->SummonCreature(NPC_LIGHT_ESSENCE, Locs[LOC_LIGHTESS_2]))
                boss_twin_valkyrAI::JustSummoned(c);
        }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        case NPC_LIGHT_ESSENCE: {
            ...
            // player->RemoveAura(touchId2); // dont remove black touch here -
            // only white can have black touch - so white changing to white - so
            // no change of color player->RemoveAura(empoweredId); // apply new
            // empowered?
        ...
        case NPC_DARK_ESSENCE: {
            ...
            player->RemoveAura(essenceId);
            player->RemoveAura(effect2Id);
            // player->RemoveAura(touchId1); // dont remove white touch here -
            // only black can have white touch - so black changing to black - so
            // no change of color
    ...
    class spell_valkyr_essence_auraAuraScript : public AuraScript {
                ...
                if (Unit* owner = GetOwner()->ToUnit()) {
                    ...
                    if (Aura* aur = owner->GetAura(auraId)) {
                        if (aur->GetStackAmount() + count < 100) {
                            ...
                            if (roll_chance_i(30)) // 30% chance to gain extra
                                                   // speed for collecting
                                owner->CastSpell(
                                    owner, SPELL_SURGE_OF_SPEED, true);
                        }
                        else {
        ...
        void Register() override
        {
            AfterEffectAbsorb += AuraEffectAbsorbFn(
                spell_valkyr_essence_auraAuraScript::HandleAfterEffectAbsorb,
                EFFECT_0);
        }
    };
    ...
    class spell_valkyr_touchAuraScript : public AuraScript {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin();
                    ...
                    if (Player* plr = itr->GetSource())
                        if (plr->IsAlive() && !plr->HasAura(excludedID) &&
                            ...
                            int32 dmg = urand(2925, 3075) *
                                        (caster->GetMap()->GetDifficulty() - 1);
                            uint32 damage              = dmg;
                            int32  resilienceReduction = damage;
                            if (caster->CanApplyResilience())
                                Unit::ApplyResilience(plr,
                                                      nullptr,
                                                      &dmg,
                                                      false,
                                                      CR_CRIT_TAKEN_SPELL);
                            ...
                            if (overkill < 0)
                                overkill = 0;
                            SpellPeriodicAuraLogInfo pInfo(aurEff,
                                                           damage,
                                                           overkill,
                                                           absorb,
                                                           resist,
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_valkyr_touchAuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
public:
    spell_valkyr_ball_periodic_dummy()
        : SpellScriptLoader("spell_valkyr_ball_periodic_dummy")
    {
    }
    ...
    class spell_valkyr_ball_periodic_dummyAuraScript : public AuraScript {
            ...
            if (Unit* target = GetTarget())
                if (target->GetDisplayId() != 11686)
                    if (Creature* creature = target->ToCreature())
                        if (Player* player =
                                creature->SelectNearestPlayer(2.75f))
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_valkyr_ball_periodic_dummyAuraScript::
