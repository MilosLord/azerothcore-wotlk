...
public:
    ...
    struct boss_anubarak_trialAI : public ScriptedAI {
            ...
            : ScriptedAI(pCreature), summons(me)
        {
            ...
            bPhase3 = false;
            me->ApplySpellImmune(
                0, IMMUNITY_ID, RAID_MODE(66193, 67855, 67856, 67857), true);
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            for (uint8 i = 0; i < 4; ++i)
                if (Creature* c =
                        me->SummonCreature(NPC_BURROW, AnubLocs[i + 1]))
                    BurrowGUID[i] = c->GetGUID();
            for (uint8 i = 0; i < 6; ++i)
                if (Creature* c =
                        me->SummonCreature(NPC_FROST_SPHERE, AnubLocs[i + 5]))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_RESPAWN_SPHERE: {
                ...
                do {
                    if (Creature* c =
                            ObjectAccessor::GetCreature(*me, SphereGUID[i]))
                        if (!c->HasAura(SPELL_FROST_SPHERE)) {
                            if (Creature* c = me->SummonCreature(
                                    NPC_FROST_SPHERE, AnubLocs[i + 5]))
                                SphereGUID[i] = c->GetGUID();
                            break;
                        }
                    i = (i + 1) % 6;
            ...
            case EVENT_SPELL_PENETRATING_COLD: {
                me->CastCustomSpell(SPELL_PENETRATING_COLD,
                                    SPELLVALUE_MAX_TARGETS,
                                    RAID_MODE(2, 5, 2, 5));
                events.Repeat(18s);
            } break;
            case EVENT_SUMMON_NERUBIAN: {
                me->CastCustomSpell(SPELL_SUMMON_BURROWER,
                                    SPELLVALUE_MAX_TARGETS,
                                    RAID_MODE(1, 2, 2, 4));
                events.Repeat(45s);
            } break;
            ...
            case EVENT_SUMMON_SCARAB: {
                uint8 i = urand(0, 3);
                if (Creature* c =
                        ObjectAccessor::GetCreature(*me, BurrowGUID[i]))
        ...
        {
            if (who->GetTypeId() != TYPEID_PLAYER ||
                me->GetExactDistSq(who) > 6400.0f) // 80yd*80yd
                return;
...
public:
    ...
    struct npc_swarm_scarabAI : public ScriptedAI {
            ...
            if (me->GetFaction() == FACTION_MONSTER_2) // hostile - it's phase 2
                if (Unit* target = me->SelectNearestTarget(250.0f)) {
                    ...
                    if (Unit* t = SelectTarget(
                            SelectTargetMethod::Random, 0, 250.0f, true)) {
        ...
        {
            me->CastSpell(me,
                          RAID_MODE(SPELL_TRAITOR_KING_10,
                                    SPELL_TRAITOR_KING_25,
                                    SPELL_TRAITOR_KING_10,
                                    SPELL_TRAITOR_KING_25),
                          true);
            me->m_Events.AddEvent(new HideNpcEvent(*me),
            ...
            if (me->GetHealth() <= damage) {
                ...
                if (!me->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE)) {
                    ...
                    me->GetMotionMaster()->MoveCharge(
                        me->GetPositionX(), me->GetPositionY(), 143.0f, 20.0f);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUBMERGE:
                if (HealthBelowPct(80) &&
                    !me->HasAura(RAID_MODE(
                        66193,
                        67855,
                        67856,
                        67857))) // not having permafrost - allow submerge
                {
                    me->GetMotionMaster()->MoveIdle();
                    me->SetUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
                    me->RemoveAllAuras();
...
public:
    ...
    struct npc_anubarak_spikeAI : public ScriptedAI {
        ...
        void SelectNewTarget(bool next)
        {
            ...
            DoResetThreatList();
            if (Unit* target =
                    SelectTarget(SelectTargetMethod::Random, 0, 250.0f, true)) {
                if (!next) {
        ...
        void Reset() override { SelectNewTarget(false); }
    ...
    class spell_pursuing_spikesAuraScript : public AuraScript {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            if (Unit* target = GetTarget()) {
                if (Creature* c = target->FindNearestCreature(
                        NPC_FROST_SPHERE, 8.0f, true)) {
                    target->UpdatePosition(*c, false);
                    target->CastCustomSpell(
                        SPELL_SPIKE_FAIL, SPELLVALUE_MAX_TARGETS, 1);
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_pursuing_spikesAuraScript::HandleEffectPeriodic,
                EFFECT_0,
    ...
    class spell_gen_leeching_swarm_AuraScript : public AuraScript {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return ValidateSpellInfo(
                {SPELL_LEECHING_SWARM_DMG, SPELL_LEECHING_SWARM_HEAL});
        }
        ...
        void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (Unit* caster = GetCaster()) {
                int32 lifeLeeched =
                    GetTarget()->CountPctFromCurHealth(aurEff->GetAmount());
                if (lifeLeeched < 250)
                    lifeLeeched = 250;
                // Damage
                caster->CastCustomSpell(GetTarget(),
                                        ...
                                        0,
                                        true);
                // Heal is handled in damage spell. It has to heal the same
                // amount, but some of the dmg can be resisted.
            }
        }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_gen_leeching_swarm_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_gen_leeching_swarm_dmg()
        : SpellScriptLoader("spell_gen_leeching_swarm_dmg")
    {
    }
    ...
    class spell_gen_leeching_swarm_dmg_SpellScript : public SpellScript {
            ...
            if (Unit* caster = GetCaster())
                if (GetHitDamage() > 0) {
                    ...
                    caster->CastCustomSpell(
                        caster, SPELL_LEECHING_SWARM_HEAL, &damage, 0, 0, true);
                }
        }
        ...
        {
            AfterHit += SpellHitFn(
                spell_gen_leeching_swarm_dmg_SpellScript::HandleAfterHit);
        }
    };
