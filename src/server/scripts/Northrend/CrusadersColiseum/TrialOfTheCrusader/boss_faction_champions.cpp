...
struct boss_faction_championsAI : public ScriptedAI {
                       ...
                       Unit* target)
    {
        ...
        float unimportant_dist = (mAIType == AI_MELEE || mAIType == AI_PET
        ? 5.0f : 20.0f); if (dist > unimportant_dist) dist -= unimportant_dist;
        // compensate melee range else dist = 0.0f; float mod_dist
        = 25.0f/(25.0f + dist); float mod_armor = (mAIType == AI_MELEE ||
        mAIType == AI_PET) ? armor / 16635.0f : 0.0f; return mod_dist /
        ((0.15f+mod_armor)*mod_health);*/

        // TC:
        /*float dist_mod = (mAIType == AI_MELEE || mAIType == AI_PET) ? 15.0f /
        (15.0f + dist) : 1.0f; float armor_mod = (mAIType == AI_MELEE || mAIType
        == AI_PET) ? armor / 16635.0f : 0.0f; float eh = (health + 1) * (1.0f +
        armor_mod); return dist_mod * 30000.0f / eh;*/

        // third try:
        float unimportant_dist =
            (mAIType == AI_MELEE || mAIType == AI_PET ? 5.0f : 35.0f);
        if (dist > unimportant_dist)
            dist -= unimportant_dist;
        else
            dist = 0.0f;
        const float dist_factor =
            (mAIType == AI_MELEE || mAIType == AI_PET ? 15.0f : 25.0f);
        float mod_dist = dist_factor / (dist_factor + dist); // 0.2 .. 1.0
        float mod_health =
            health > 40000 ? 2.0f : (60000 - health) / 10000.0f; // 2.0 .. 6.0
        float mod_armor =
            (mAIType == AI_MELEE || mAIType == AI_PET)
                ? Unit::CalcArmorReducedDamage(me, target, 10000, nullptr) /
                      10000.0f
                : 1.0f;
    ...
    void RecalculateThreat()
    {
        ...
        for (ThreatContainer::StorageType::const_iterator itr = tList.begin();
            ...
            if (pUnit && pUnit->GetTypeId() == TYPEID_PLAYER &&
                me->GetThreatMgr().GetThreat(pUnit)) {
                ...
                // me->getThreatMgr().DoAddThreat(pUnit, 10000000.0f *
                // threatMod);
                if (HostileReference* ref = me->GetThreatMgr()
                                                .GetOnlineContainer()
                                                .getReferenceByTarget(pUnit))
    ...
    Creature* SelectTarget_MostHPLostFriendlyMissingBuff(uint32 spell,
                                                         float  range)
    {
        ...
        for (std::list<Creature*>::const_iterator itr = lst.begin();
             itr != lst.end();
             ++itr)
    ...
    uint32 EnemiesInRange(float distance)
    {
        ...
        uint32 count = 0;
        Unit*  target;
        for (ThreatContainer::StorageType::const_iterator iter = tList.begin();
             iter != tList.end();
             ++iter) {
    ...
    Unit* SelectEnemyCaster(bool casting, float range)
    {
        ...
        for (ThreatContainer::StorageType::const_iterator iter = tList.begin();
            ...
            if (target && target->getPowerType() == POWER_MANA &&
                (!casting || target->HasUnitState(UNIT_STATE_CASTING)) &&
                me->GetExactDist(target) <= range)
                return target;
        }
        return nullptr;
    }
    ...
    struct npc_toc_druidAI : public boss_faction_championsAI {
        ...
        bool myCanCast()
        {
            return !(me->HasUnitState(UNIT_STATE_CASTING) ||
                     me->HasUnitFlag(UNIT_FLAG_SILENCED) || IsCCed());
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_THORNS:
                if (Creature* target =
                        SelectTarget_MostHPLostFriendlyMissingBuff(SPELL_THORNS,
                                                                   30.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_HEX:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 20.0f, true))
    ...
    struct npc_toc_paladinAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_HAND_OF_PROTECTION:
                if (Creature* target =
                        SelectTarget_MostHPLostFriendlyMissingBuff(
                            SPELL_HAND_OF_PROTECTION, 40.0f)) {
            ...
            case EVENT_SPELL_HAMMER_OF_JUSTICE:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxDistance, 0, 15.0f, true)) {
    ...
    struct npc_toc_priestAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_RENEW:
                if (Creature* target =
                        SelectTarget_MostHPLostFriendlyMissingBuff(SPELL_RENEW,
                                                                   40.0f))
            ...
            case EVENT_SPELL_SHIELD:
                if (Creature* target =
                        SelectTarget_MostHPLostFriendlyMissingBuff(SPELL_SHIELD,
                                                                   40.0f))
    ...
    struct npc_toc_shadow_priestAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_HORROR:
                if (me->GetVictim() &&
                    me->GetExactDist2d(me->GetVictim()) <= 30.0f) {
    ...
    struct npc_toc_warlockAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_CURSE_OF_EXHAUSTION:
                if (me->GetVictim())
                    me->CastSpell(
                        me->GetVictim(), SPELL_CURSE_OF_EXHAUSTION, false);
                events.Repeat(10s, 20s);
                EventMapGCD(events, 1500);
                break;
            ...
            case EVENT_SPELL_UNSTABLE_AFFLICTION:
                if (me->GetVictim())
                    me->CastSpell(
                        me->GetVictim(), SPELL_UNSTABLE_AFFLICTION, false);
                events.Repeat(5s, 15s);
                EventMapGCD(events, 1500);
                break;
            }
    ...
    struct npc_toc_mageAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_ARCANE_EXPLOSION:
                if (EnemiesInRange(9.0f) >= 3) {
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_ARCANE_EXPLOSION, false);
                    events.Repeat(6s);
                    EventMapGCD(events, 1500);
                }
                else
...
public:
    ...
    struct npc_toc_hunterAI : public boss_faction_championsAI {
        ...
        {
            return !(me->HasUnitState(UNIT_STATE_CASTING) ||
                     me->HasUnitFlag2(UNIT_FLAG2_DISARM_RANGED) || IsCCed());
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_WING_CLIP:
                if (me->GetVictim() &&
                    me->GetDistance2d(me->GetVictim()) <= 5.0f)
            ...
            case EVENT_SPELL_WYVERN_STING:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 35.0f, true)) {
    ...
    struct npc_toc_boomkinAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_CYCLONE:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxDistance, 0, 20.0f, true))
    ...
    struct npc_toc_warriorAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_INTIMIDATING_SHOUT:
                if (EnemiesInRange(8.0f) >= 3) {
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_INTIMIDATING_SHOUT, false);
                    events.Repeat(2min);
                    EventMapGCD(events, 1500);
                }
                else
            ...
            case EVENT_SPELL_CHARGE:
                if (me->GetVictim() &&
                    me->GetDistance2d(me->GetVictim()) > 8.0f &&
            ...
            case EVENT_SPELL_SHATTERING_THROW:
                ...
                if (me->GetVictim() &&
                    me->GetDistance2d(me->GetVictim()) < 25.0f) {
                    me->CastSpell(
                        me->GetVictim(), SPELL_SHATTERING_THROW, false);
                    events.Repeat(5min);
                    EventMapGCD(events, 1500);
                }
                else
    ...
    struct npc_toc_dkAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_DEATH_COIL:
                if (me->GetVictim() &&
                    me->GetDistance2d(me->GetVictim()) <= 30.0f) {
            ...
            case EVENT_SPELL_DEATH_GRIP:
                if (me->GetVictim() &&
                    ...
                    pos.Relocate(x, y, z);
                    me->GetVictim()->CastSpell(pos.GetPositionX(),
                                               pos.GetPositionY(),
                                               pos.GetPositionZ(),
                                               49575,
                                               true);
            ...
            case EVENT_SPELL_ICY_TOUCH:
                if (me->GetVictim() &&
                    me->GetDistance2d(me->GetVictim()) < 20.0f) {
    ...
    struct npc_toc_rogueAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_BLIND:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MinThreat, 0, 20.0f, true)) {
            ...
            case EVENT_SPELL_SHADOWSTEP:
                if (me->GetVictim() &&
                    me->GetDistance2d(me->GetVictim()) < 40.0f &&
    ...
    struct npc_toc_enh_shamanAI : public boss_faction_championsAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_EARTH_SHOCK_ENH:
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxThreat, 0, 25.0f, true)) {
            ...
            case EVENT_SPELL_LAVA_LASH:
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxThreat, 0, 5.0f, true)) {
    ...
    struct npc_toc_retro_paladinAI : public boss_faction_championsAI {
            ...
            : boss_faction_championsAI(pCreature, AI_MELEE)
        {
            ...
            events.RescheduleEvent(
                EVENT_SPELL_HAND_OF_PROTECTION_RET, 25s, 40s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_HAMMER_OF_JUSTICE_RET:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxThreat, 0, 15.0f, true)) {
            ...
            case EVENT_SPELL_HAND_OF_PROTECTION_RET:
                if (Creature* target =
                        SelectTarget_MostHPLostFriendlyMissingBuff(
                            SPELL_HAND_OF_PROTECTION_RET, 30.0f)) {
            ...
            case EVENT_SPELL_JUDGEMENT_OF_COMMAND:
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::MaxThreat, 0, 20.0f, true)) {
...
public:
    ...
    struct npc_toc_pet_warlockAI : public boss_faction_championsAI {
        npc_toc_pet_warlockAI(Creature* pCreature)
            : boss_faction_championsAI(pCreature, AI_PET)
        {
            events.Reset();
...
public:
    ...
    struct npc_toc_pet_hunterAI : public boss_faction_championsAI {
        npc_toc_pet_hunterAI(Creature* pCreature)
            : boss_faction_championsAI(pCreature, AI_PET)
        {
            events.Reset();
...
public:
    ...
    class spell_faction_champion_warl_unstable_affliction_AuraScript
        : public AuraScript {
        PrepareAuraScript(
        ...
        void HandleDispel(DispelInfo* dispelInfo)
        {
            if (Unit* caster = GetCaster())
                caster->CastSpell(dispelInfo->GetDispeller(),
