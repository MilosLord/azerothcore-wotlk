...
/// @todo: Missing Sinclari Trigger announcements (32204) Look at its
/// creature_text for more info.
/// @todo: Activation Crystals (go_vh_activation_crystal) (193611) are
/// spammable, should be a 1 time use per crystal.
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (InstanceScript* pInstance = creature->GetInstanceScript())
            ...
            case NOT_STARTED:
                ...
                SendGossipMenuFor(
                    player, NPC_TEXT_SINCLARI_IN, creature->GetGUID());
                break;
            case IN_PROGRESS:
                AddGossipItemFor(player,
                ...
                SendGossipMenuFor(
                    player, NPC_TEXT_SINCLARI_LATE_JOIN, creature->GetGUID());
                break;
            default: // DONE or invalid
                SendGossipMenuFor(
                    player, NPC_TEXT_SINCLARI_DONE, creature->GetGUID());
            }
        return true;
    }
                        ...
                        uint32 uiAction) override
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 2:
            SendGossipMenuFor(
                player, NPC_TEXT_SINCLARI_ITEM, creature->GetGUID());
            break;
...
public:
    npc_vh_teleportation_portal()
        : CreatureScript("npc_vh_teleportation_portal")
    {
    }
    ...
    struct npc_vh_teleportation_portalAI : public NullCreatureAI {
        npc_vh_teleportation_portalAI(Creature* c)
            : NullCreatureAI(c), listOfMobs(me)
        {
            ...
            if (pInstance) {
                ...
                if (wave % 6 != 0)
                    events.RescheduleEvent(RAND(EVENT_SUMMON_KEEPER_OR_GUARDIAN,
                                                EVENT_SUMMON_ELITES),
                                           10s);
                else
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUMMON_KEEPER_OR_GUARDIAN:
                ...
                spawned = true;
                if (Creature* c =
                        DoSummon(RAND(NPC_PORTAL_GUARDIAN, NPC_PORTAL_KEEPER),
                                 me,
                                 2.0f,
                                 0,
            ...
            case EVENT_SUMMON_ELITES:
                ...
                for (uint8 i = 0; i < 2 + addValue; ++i) {
                    uint32 entry = RAND(NPC_AZURE_CAPTAIN,
                                        NPC_AZURE_RAIDER,
                                        NPC_AZURE_STALKER,
                                        NPC_AZURE_SORCEROR);
                    DoSummon(entry, me, 2.0f, 20000, TEMPSUMMON_DEAD_DESPAWN);
            ...
            if (bKorG) {
                if (!me->IsNonMeleeSpellCast(
                        false)) // keeper/guardian died => channeled spell
                                // interrupted
                {
                    // if keeper/guard lost all victims, in enterevademode
                    // linking aura is removed, restore it:
                    if (pInstance)
                        for (SummonList::iterator itr = listOfMobs.begin();
                             itr != listOfMobs.end();
                             ++itr)
                            if (Creature* c =
                                    pInstance->instance->GetCreature(*itr))
                                if (c->IsAlive() &&
                                    (c->GetEntry() == NPC_PORTAL_GUARDIAN ||
                                     c->GetEntry() == NPC_PORTAL_KEEPER)) {
                                    me->CastSpell(
                                        c, SPELL_PORTAL_CHANNEL, false);
                                    return;
                                }
        ...
        void JustSummoned(Creature* pSummoned) override
        {
            if (pSummoned) {
                ...
                pInstance->SetGuidData(DATA_ADD_TRASH_MOB,
                                       pSummoned->GetGUID());
            }
        }
        ...
        void SummonedMobDied(Creature* pSummoned)
        {
            if (pSummoned) {
                ...
                pInstance->SetGuidData(DATA_DELETE_TRASH_MOB,
                                       pSummoned->GetGUID());
            }
        }
    };
...
struct violet_hold_trashAI : public npc_escortAI {
    ...
    void ClearDoorSealAura()
    {
        if (pInstance)
            if (Creature* c = pInstance->instance->GetCreature(
                    pInstance->GetGuidData(DATA_DOOR_SEAL_GUID)))
    ...
    void JustReachedHome() override { CreatureStartAttackDoor(); }
            ...
            case 0:
                for (int i = 0; i < 6; i++)
                    AddWaypoint(i,
                                ...
                                0);
                me->SetHomePosition(FirstPortalTrashWPs[5][0],
                                    FirstPortalTrashWPs[5][1],
                                    FirstPortalTrashWPs[5][2],
                                    3.149439f);
                break;
            case 1:
                bAlt = (bool)urand(0, 1);
                if (!bAlt) {
                    for (int i = 0; i < 9; i++)
                        AddWaypoint(i,
                                    ...
                                    0);
                    me->SetHomePosition(
                        SecondPortalTrashWPs1[8][0] + irand(-1, 1),
            ...
            case 2:
                for (int i = 0; i < 8; i++)
                    AddWaypoint(i,
                                ...
                                0);
                me->SetHomePosition(ThirdPortalTrashWPs[7][0],
                                    ThirdPortalTrashWPs[7][1],
                                    ThirdPortalTrashWPs[7][2],
                                    3.149439f);
                break;
            case 3:
                for (int i = 0; i < 9; i++)
                    AddWaypoint(i,
                                ...
                                0);
                me->SetHomePosition(FourthPortalTrashWPs[8][0],
                                    FourthPortalTrashWPs[8][1],
                                    FourthPortalTrashWPs[8][2],
                                    3.149439f);
                break;
            case 4:
                for (int i = 0; i < 6; i++)
                    AddWaypoint(i,
                                ...
                                0);
                me->SetHomePosition(FifthPortalTrashWPs[5][0],
                                    FifthPortalTrashWPs[5][1],
                                    FifthPortalTrashWPs[5][2],
                                    3.149439f);
                break;
            case 5:
                for (int i = 0; i < 4; i++)
                    AddWaypoint(i,
                                ...
                                0);
                me->SetHomePosition(SixthPoralTrashWPs[3][0],
                                    SixthPoralTrashWPs[3][1],
                                    SixthPoralTrashWPs[3][2],
                                    3.149439f);
                break;
            }
    ...
    {
        if (pInstance)
            if (Creature* portal = ObjectAccessor::GetCreature(
                    (*me),
                    pInstance->GetGuidData(DATA_TELEPORTATION_PORTAL_GUID)))
                CAST_AI(
                    npc_vh_teleportation_portal::npc_vh_teleportation_portalAI,
                    portal->AI())
                    ->SummonedMobDied(me);
    }
    ...
    {
        RemoveEscortState(STATE_ESCORT_ESCORTING | STATE_ESCORT_RETURNING |
                          STATE_ESCORT_PAUSED);
    ...
    void EnterEvadeMode(EvadeReason /*why*/) override
    {
        if (!HasEscortState(STATE_ESCORT_ESCORTING)) {
            me->SetImmuneToNPC(false);
            me->SetHomePosition(1845.577759f + rand_norm() * 5 - 2.5f,
                                800.681152f + rand_norm() * 5 - 2.5f,
                                44.104248f,
                                M_PI);
        }
...
#define SPELL_ARCANE_BLAST                                                     \
    DUNGEON_MODE(SPELL_ARCANE_BLAST_N, SPELL_ARCANE_BLAST_H)
#define SPELL_CONE_OF_COLD                                                     \
    DUNGEON_MODE(SPELL_CONE_OF_COLD_N, SPELL_CONE_OF_COLD_H)
...
#define SPELL_ARCANE_BARRAGE                                                   \
    DUNGEON_MODE(SPELL_ARCANE_BARRAGE_N, SPELL_ARCANE_BARRAGE_H)
#define SPELL_ARCANE_EXPLOSION                                                 \
    DUNGEON_MODE(SPELL_ARCANE_EXPLOSION_N, SPELL_ARCANE_EXPLOSION_H)
...
#define SPELL_ARCANE_STREAM                                                    \
    DUNGEON_MODE(SPELL_ARCANE_STREAM_N, SPELL_ARCANE_STREAM_H)
#define SPELL_MANA_DETONATION                                                  \
    DUNGEON_MODE(SPELL_MANA_DETONATION_N, SPELL_MANA_DETONATION_H)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->GetEntry() == NPC_AZURE_INVADER_1) {
                ...
                if (uiImpaleTimer <= diff) {
                    Unit* pTarget =
                        SelectTarget(SelectTargetMethod::Random, 0, 5.0f, true);
                    if (pTarget)
    ...
    struct npc_azure_stalkerAI : public violet_hold_trashAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            /*if (!TacticalBlinkCasted)
            {
                ...
                {
                    Unit* pTarget = SelectTarget(SelectTargetMethod::Random,
            0, 40.0f, true); if (pTarget) DoCast(pTarget, SPELL_TACTICAL_BLINK);
            ...
            else*/
            {
                if (uiBackstabTimer <= diff) {
                    Unit* pTarget = SelectTarget(
                        SelectTargetMethod::MaxDistance, 0, 5.0f, true);
    ...
    struct npc_azure_sorcerorAI : public violet_hold_trashAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uiManaDetonationTimer <= diff && uiArcaneStreamTimer >= 1500 &&
                uiArcaneStreamTimer <=
                    uiArcaneStreamTimerStartingValueHolder / 2) {
...
public:
    ...
    struct npc_azure_saboteurAI : public npc_escortAI {
        npc_azure_saboteurAI(Creature* c) : npc_escortAI(c)
        {
            ...
            uiBoss    = 0;
            if (pInstance)
                uiBoss = pInstance->GetData(DATA_WAVE_COUNT) == 6
                             ? pInstance->GetData(DATA_FIRST_BOSS_NUMBER)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!bAddedWPs) {
                ...
                case 1:
                    for (int i = 0; i < 3; i++)
                        AddWaypoint(i,
                                    SaboteurFinalPos1[i][0],
                                    SaboteurFinalPos1[i][1],
                                    SaboteurFinalPos1[i][2],
                                    0);
                    me->SetHomePosition(SaboteurFinalPos1[2][0],
                                        SaboteurFinalPos1[2][1],
                                        SaboteurFinalPos1[2][2],
                                        4.762346f);
                    break;
                case 2:
                    for (int i = 0; i < 3; i++)
                        AddWaypoint(i,
                                    SaboteurFinalPos2[i][0],
                                    SaboteurFinalPos2[i][1],
                                    SaboteurFinalPos2[i][2],
                                    0);
                    me->SetHomePosition(SaboteurFinalPos2[2][0],
                                        SaboteurFinalPos2[2][1],
                                        SaboteurFinalPos2[2][2],
                                        1.862674f);
                    break;
                case 3:
                    for (int i = 0; i < 2; i++)
                        AddWaypoint(i,
                                    SaboteurFinalPos3[i][0],
                                    SaboteurFinalPos3[i][1],
                                    SaboteurFinalPos3[i][2],
                                    0);
                    me->SetHomePosition(SaboteurFinalPos3[1][0],
                                        SaboteurFinalPos3[1][1],
                                        SaboteurFinalPos3[1][2],
                                        5.500638f);
                    break;
                case 4:
                    AddWaypoint(0,
                                ...
                                SaboteurFinalPos4[2],
                                0);
                    me->SetHomePosition(SaboteurFinalPos4[0],
                                        SaboteurFinalPos4[1],
                                        SaboteurFinalPos4[2],
                                        3.991108f);
                    break;
                case 5:
                    AddWaypoint(0,
                                ...
                                SaboteurFinalPos5[2],
                                0);
                    me->SetHomePosition(SaboteurFinalPos5[0],
                                        SaboteurFinalPos5[1],
                                        SaboteurFinalPos5[2],
                                        1.100841f);
                    break;
                case 6:
                    for (int i = 0; i < 5; i++)
                        AddWaypoint(i,
                                    SaboteurFinalPos6[i][0],
                                    SaboteurFinalPos6[i][1],
                                    SaboteurFinalPos6[i][2],
                                    0);
                    me->SetHomePosition(SaboteurFinalPos6[4][0],
                                        SaboteurFinalPos6[4][1],
                                        SaboteurFinalPos6[4][2],
                                        0.983031f);
                    break;
                }
                SetDespawnAtEnd(false);
    ...
    class spell_destroy_door_sealAuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_destroy_door_sealAuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
struct npc_violet_hold_defense_system : public ScriptedAI {
    ...
    void Reset() override
    {
        DoCast(RAND(SPELL_DEFENSE_SYSTEM_SPAWN_EFFECT,
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_ARCANE_LIGHTNING:
            DoCastAOE(
                RAND(SPELL_ARCANE_LIGHTNING, SPELL_ARCANE_LIGHTNING_VISUAL));
