...
#define SPELL_WATER_BOLT_VOLLEY                                                \
    DUNGEON_MODE(SPELL_WATER_BOLT_VOLLEY_N, SPELL_WATER_BOLT_VOLLEY_H)
    ...
    struct boss_ichoronAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (!bIsFrenzy) {
                if (!bIsExploded) {
                    if (!me->HasAura(SPELL_PROTECTIVE_BUBBLE)) {
                        ...
                        Talk(SAY_SHATTER);
                        DoZoneInCombat();
                        IchoronDoCastToAllHostilePlayers(SPELL_WATER_BLAST,
                                                         true);
            ...
            if (!bIsExploded) {
                if (uiWaterBoltVolleyTimer <= uiDiff) {
                    me->CastSpell(
                        (Unit*)nullptr, SPELL_WATER_BOLT_VOLLEY, false);
        ...
        void JustSummoned(Creature* pSummoned) override
        {
            if (pSummoned) {
                ...
                me->CastSpell(pSummoned,
                              SPELL_CREATE_GLOBULE_VISUAL,
                              true); // triggered should ignore los
    ...
    struct npc_ichor_globuleAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            if (uiRangeCheck_Timer < uiDiff) {
                if (pInstance)
                    if (Creature* pIchoron = pInstance->instance->GetCreature(
                            pInstance->GetGuidData(DATA_ICHORON_GUID)))
                        if (me->IsWithinDist(pIchoron, 2.0f, false)) {
                            if (pIchoron->AI())
                                pIchoron->AI()->DoAction(
