    ...
    struct instance_violet_hold_InstanceMapScript : public InstanceScript {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_DEFENSE_DUMMY_TARGET:
                creature->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_ARCANE_LIGHTNING, true);
                break;
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_ACTIVATION_CRYSTAL:
                HandleGameObject(
                    ObjectGuid::Empty, false, go); // make go not used yet
                go->SetGameObjectFlag(
                    GO_FLAG_NOT_SELECTABLE); // not useable at the beginning
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_DECRASE_DOOR_HEALTH:
                ...
                DoUpdateWorldState(WORLD_STATE_VH_PRISON_STATE,
                                   (uint32)GateHealth);
                break;
        ...
        void StartBossEncounter(uint8 uiBoss)
        {
            ...
            case BOSS_EREKEM:
                ...
                if (Creature* pGuard1 =
                        instance->GetCreature(NPC_ErekemGuardGUID[0])) {
                ...
                if (Creature* pGuard2 =
                        instance->GetCreature(NPC_ErekemGuardGUID[1])) {
        ...
        void Update(uint32 diff) override
        {
            ...
            case EVENT_START_ENCOUNTER: {
                ...
                DoUpdateWorldState(WORLD_STATE_VH_PRISON_STATE,
                                   (uint32)GateHealth);
                DoUpdateWorldState(WORLD_STATE_VH_WAVE_COUNT,
                                   (uint32)WaveCount);

                for (ObjectGuid const& guid : GO_ActivationCrystalGUID)
                    if (GameObject* go = instance->GetGameObject(guid)) {
                        HandleGameObject(
                            ObjectGuid::Empty, false, go); // not used yet
                        go->RemoveGameObjectFlag(
                            GO_FLAG_NOT_SELECTABLE); // make it useable
                    }
            ...
            case EVENT_SUMMON_PORTAL:
                ...
                DoUpdateWorldState(WORLD_STATE_VH_WAVE_COUNT,
                                   (uint32)WaveCount);
                SetData(DATA_PORTAL_LOCATION,
                        (GetData(DATA_PORTAL_LOCATION) + urand(1, 5)) % 6);
                if (Creature* c = instance->GetCreature(NPC_SinclariGUID)) {
                    if (WaveCount % 6 != 0)
                        c->SummonCreature(
                            ...
                            TEMPSUMMON_CORPSE_DESPAWN);
                             ...
                             WaveCount == 12) // first or second boss
                    {
                    ...
                    else // cyanigossa
                    {
                        if (Creature* cyanigosa =
                                c->SummonCreature(NPC_CYANIGOSA,
                                                  CyanigosasSpawnLocation,
                                                  TEMPSUMMON_DEAD_DESPAWN)) {
                            cyanigosa->CastSpell(
                                cyanigosa, SPELL_CYANIGOSA_BLUE_AURA, false);
                            cyanigosa->AI()->Talk(CYANIGOSA_SAY_SPAWN);
                            cyanigosa->GetMotionMaster()->MoveJump(
                                MiddleRoomLocation.GetPositionX(),
        ...
        void OnPlayerEnter(Player* plr) override
        {
            ...
            if (EncounterStatus == IN_PROGRESS) {
                ...
                plr->SendUpdateWorldState(WORLD_STATE_VH_PRISON_STATE,
                                          (uint32)GateHealth);
                plr->SendUpdateWorldState(WORLD_STATE_VH_WAVE_COUNT,
                                          (uint32)WaveCount);
            }
            else
            ...
            for (Map::PlayerList::const_iterator itr = pl.begin();
                ...
                if (Player* plr = itr->GetSource())
                    if (plr->IsAlive() && !plr->IsGameMaster() &&
                        !plr->HasAura(27827) /*spirit of redemption aura*/)
                        ++aliveCount;
        ...
        void InstanceCleanup()
        {
            ...
            if (m_auiEncounter[MAX_ENCOUNTER - 1] !=
                DONE) // instance not finished
            {
                // close all cells
        ...
        void WriteSaveDataMore(std::ostringstream& data) override
        {
            data << m_auiEncounter[0] << ' ' << m_auiEncounter[1] << ' '
                 << m_auiEncounter[2] << ' ' << uiFirstBoss << ' '
                 << uiSecondBoss << ' ';
        }
    };
};

void AddSC_instance_violet_hold() { new instance_violet_hold(); }
