...
#define SPELL_EARTH_SHIELD                                                     \
    DUNGEON_MODE(SPELL_EARTH_SHIELD_N, SPELL_EARTH_SHIELD_H)
...
public:
    ...
    struct boss_erekemAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            if (Creature* c = pInstance->instance->GetCreature(
                    pInstance->GetGuidData(DATA_EREKEM_GUARD_2_GUID)))
                if (!c->IsInCombat())
                    c->AI()->AttackStart(who);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_CHAIN_HEAL:
                if (ObjectGuid TargetGUID = GetChainHealTargetGUID())
                    if (pInstance) {
                        if (Creature* target =
                                pInstance->instance->GetCreature(TargetGUID))
            ...
            case EVENT_SPELL_STORMSTRIKE: {
                ...
                Creature* pGuard2 = pInstance->instance->GetCreature(
                    pInstance->GetGuidData(DATA_EREKEM_GUARD_2_GUID));
                if (pGuard1 && !pGuard1->IsAlive() && pGuard2 &&
                    !pGuard2->IsAlive()) // both dead
...
public:
    ...
    struct npc_erekem_guardAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            if (Creature* c = pInstance->instance->GetCreature(
                    pInstance->GetGuidData(DATA_EREKEM_GUID)))
                if (!c->IsInCombat())
                    c->AI()->AttackStart(who);
        }
