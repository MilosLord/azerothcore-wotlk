...
enum Floppy {
    ...
    SAY_WORGRAGGRO3 =
        2, // Oh, no! Look, it's another wolf, and it's a biiiiiig one!
    SAY_WORGRAGGRO4 = 3, // He's gonna eat Mr. Floppy! You gotta help Mr.
                         // Floppy! You just gotta!
    ...
    SAY_VICTORY3 =
        7, // Mr. Floppy, you're ok! Thank you so much for saving Mr. Floppy!
    SAY_VICTORY4 =
        8, // I think I see the camp! We're almost home, Mr. Floppy! Let's go!
    TEXT_EMOTE_WP1   = 9,   // Mr. Floppy revives
    TEXT_EMOTE_AGGRO = 10,  // The Ravenous Worg chomps down on Mr. Floppy
    SAY_QUEST_ACCEPT = 11,  // Are you ready, Mr. Floppy? Stay close to me and
                            // watch out for those wolves!
    SAY_QUEST_COMPLETE = 12 // Thank you for helping me get back to the camp. Go
                            // tell Walter that I'm safe now!
};
...
public:
    ...
    struct npc_emilyAI : public npc_escortAI {
        ...
        {
            if (Creature* Mrfloppy =
                    GetClosestCreatureWithEntry(me, NPC_MRFLOPPY, 50.0f))
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 9:
                if (Creature* Mrfloppy =
                        GetClosestCreatureWithEntry(me, NPC_MRFLOPPY, 100.0f))
            ...
            case 10:
                if (ObjectAccessor::GetCreature(*me, _mrfloppyGUID)) {
                    ...
                    if (Creature* worg =
                            me->SummonCreature(NPC_HUNGRY_WORG,
                                               me->GetPositionX() + 5,
                                               me->GetPositionY() + 2,
                                               me->GetPositionZ() + 1,
                                               3.229f,
                                               ...
                                               120000))
                        if (Creature* Mrfloppy = ObjectAccessor::GetCreature(
                                *me, _mrfloppyGUID)) {
            ...
            case 11:
                if (Creature* Mrfloppy =
                        ObjectAccessor::GetCreature(*me, _mrfloppyGUID))
                    Mrfloppy->GetMotionMaster()->MoveFollow(
                        me, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
                break;
            case 17:
                if (Creature* Mrfloppy =
                        ObjectAccessor::GetCreature(*me, _mrfloppyGUID))
                    Mrfloppy->GetMotionMaster()->MovePoint(0,
                                                           me->GetPositionX(),
                                                           me->GetPositionY(),
                                                           me->GetPositionZ());
                Talk(SAY_WORGRAGGRO3);
                if (Creature* RWORG =
                        me->SummonCreature(NPC_RAVENOUS_WORG,
                                           me->GetPositionX() + 10,
                                           me->GetPositionY() + 8,
                                           me->GetPositionZ() + 2,
                                           3.229f,
            ...
            case 18:
                if (Creature* Mrfloppy =
                        ObjectAccessor::GetCreature(*me, _mrfloppyGUID)) {
                    if (Creature* RWORG =
                            ObjectAccessor::GetCreature(*me, _RavenousworgGUID))
                        RWORG->GetMotionMaster()->MovePoint(
                            0,
                            Mrfloppy->GetPositionX(),
                            Mrfloppy->GetPositionY(),
                            Mrfloppy->GetPositionZ());
            ...
            case 19:
                if (Creature* Mrfloppy =
                    ...
                    if (Mrfloppy->HasAura(SPELL_MRFLOPPY)) {
                        if (Creature* RWORG = ObjectAccessor::GetCreature(
                                *me, _RavenousworgGUID))
        ...
        void JustEngagedWith(Unit* /*Who*/) override { Talk(SAY_RANDOMAGGRO); }
                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_PERILOUS_ADVENTURE) {
            ...
            if (Creature* Mrfloppy =
                    GetClosestCreatureWithEntry(creature, NPC_MRFLOPPY, 180.0f))
                Mrfloppy->GetMotionMaster()->MoveFollow(
                    creature, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);

            if (npc_escortAI* pEscortAI =
                    CAST_AI(npc_emily::npc_emilyAI, (creature->AI())))
...
public:
    ...
    struct npc_mrfloppyAI : public ScriptedAI {
        ...
        {
            if (Creature* Emily =
                    GetClosestCreatureWithEntry(me, NPC_EMILY, 50.0f)) {
                switch (Who->GetEntry()) {
        ...
        void EnterEvadeMode(EvadeReason /*why*/) override
        {
            if (Creature* Emily =
                ...
                me->GetMotionMaster()->MoveFollow(
                    Emily, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
        }
...
class spell_q12227_outhouse_groans : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_q12227_outhouse_groans::HandleScriptEffect,
                          EFFECT_0,
...
class spell_q12227_camera_shake : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Player* player = GetCaster()->ToPlayer())
            if (Creature* target = GetClosestCreatureWithEntry(
                    player,
                    NPC_OUTHOUSE_BUNNY_GRIZZLY,
                    3.0f)) // hackfix: Outhouse bunny doesnt show in any script.
                           // But the visual of Dust Field do not show if cast
                           // by the player
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_q12227_camera_shake::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
public:
    ...
    void Initialize() { me->SetReactState(REACT_DEFENSIVE); }
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell) override
    {
        ...
        if (spell->Id == SPELL_RENEW_SKIRMISHER &&
            playerCaster->GetQuestStatus(QUEST_OVERWHELMED) ==
...
enum renewskirmisher { NPC_WOUNDED_SKIRMISHER = 27463 };
...
public:
    ...
    SpellCastResult CheckRequirement()
    {
        if (Unit* caster = GetCaster())
            if (Creature* wounded =
                    caster->FindNearestCreature(NPC_WOUNDED_SKIRMISHER, 5.0f))
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_renew_skirmisher::CheckRequirement);
    }
};
    ...
    struct npc_lake_frogAI : public ScriptedAI {
        ...
        void ReceiveEmote(Player* player, uint32 emote) override
        {
            ...
            if (emote == TEXT_EMOTE_KISS &&
                ...
                player->HasItemCount(ITEM_WARTS_B_GONE_LIP_BALM, 1, false)) {
                if (!player->HasAura(SPELL_WARTSBGONE_LIP_BALM))
                    ...
                    if (me->GetEntry() == NPC_LAKE_FROG) {
                        ...
                        me->GetMotionMaster()->MoveFollow(
                            player, 0.3f, frand(M_PI / 2, M_PI + (M_PI / 2)));
                        _following = true;
                    }
...
public:
    ...
    class spell_shredder_delivery_SpellScript : public SpellScript {
        ...
        {
            if (GetCaster()->ToCreature()->GetEntry() ==
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_shredder_delivery_SpellScript::HandleScript,
                              EFFECT_0,
                              SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    spell_infected_worgen_bite()
        : SpellScriptLoader("spell_infected_worgen_bite")
    {
    }
    ...
    class spell_infected_worgen_bite_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_infected_worgen_bite_AuraScript::HandleAfterEffectApply,
                EFFECT_1,
...
public:
    npc_rocket_propelled_warhead()
        : CreatureScript("npc_rocket_propelled_warhead")
    {
    }

    struct npc_rocket_propelled_warheadAI : public VehicleAI {
        npc_rocket_propelled_warheadAI(Creature* creature)
            : VehicleAI(creature), _faction(ALLIANCE), _finished(false)
        {
        }
        ...
        {
            if (caster && (caster->GetEntry() == NPC_HORDE_LUMBERBOAT ||
                           caster->GetEntry() == NPC_ALLIANCE_LUMBERBOAT)) {
        ...
        void FinishQuest(bool success, uint32 faction)
        {
            ...
            if (success) {
                DoCast(me,
                       faction == ALLIANCE ? SPELL_ALLIANCE_KILL_CREDIT_TORPEDO
...
public:
    spell_vehicle_warhead_fuse()
        : SpellScriptLoader("spell_vehicle_warhead_fuse")
    {
    }
    ...
    class spell_vehicle_warhead_fuse_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_vehicle_warhead_fuse_SpellScript::HandleDummy,
                EFFECT_1,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    class spell_warhead_detonate_SpellScript : public SpellScript {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return sSpellMgr->GetSpellInfo(SPELL_PARACHUTE) &&
                   sSpellMgr->GetSpellInfo(SPELL_TORPEDO_EXPLOSION);
        }
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            player->KnockbackFrom(caster->GetPositionX(),
                                  caster->GetPositionY(),
                                  horizontalSpeed,
                                  verticalSpeed);
            ...
            caster->GetCreatureListWithEntryInGrid(
                explosionBunnys, NPC_ALLIANCE_LUMBERBOAT_EXPLOSIONS, 90.0f);
            for (auto itr = explosionBunnys.begin();
                 itr != explosionBunnys.end();
                 ++itr) {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_warhead_detonate_SpellScript::HandleDummy,
                              EFFECT_0,
                              SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    private:
        ...
        {
            OnEffectApply +=
                AuraEffectApplyFn(spell_z_check_AuraScript::HandleEffectApply,
                                  EFFECT_0,
    ...
    class spell_warhead_fuse_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectRemove += AuraEffectRemoveFn(
                spell_warhead_fuse_AuraScript::HandleOnEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
class spell_frog_kiss : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_frog_kiss::HandleScript,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
