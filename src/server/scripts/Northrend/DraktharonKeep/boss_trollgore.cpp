    ...
    class spell_trollgore_consume_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_trollgore_consume_SpellScript::HandleScriptEffect,
                EFFECT_1,
...
public:
    spell_trollgore_corpse_explode()
        : SpellScriptLoader("spell_trollgore_corpse_explode")
    {
    }
    ...
    class spell_trollgore_corpse_explode_AuraScript : public AuraScript {
        ...
        void PeriodicTick(AuraEffect const* aurEff)
        {
            if (aurEff->GetTickNumber() == 2)
                if (Unit* caster = GetCaster())
                    caster->CastSpell(
                        GetTarget(), SPELL_CORPSE_EXPLODE_DAMAGE, true);
        }
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_trollgore_corpse_explode_AuraScript::PeriodicTick,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_trollgore_corpse_explode_AuraScript::HandleRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
...
public:
    ...
    class spell_trollgore_invader_taunt_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_trollgore_invader_taunt_SpellScript::HandleScriptEffect,
                EFFECT_0,
                SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    achievement_consumption_junction()
        : AchievementCriteriaScript("achievement_consumption_junction")
    {
    }
    ...
    OnCheck(Player* /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        ...
        return target->GetAuraCount(sSpellMgr->GetSpellIdForDifficulty(
                   SPELL_CONSUME_AURA, target)) < 10;
    }
};
