        ...
        void setphase(short newPhase)
        {
            ...
            case 2: {
                ...
                if (me->GetExactDist(summoner) < 50.0f) {
                    me->UpdatePosition(summoner->GetPositionX(),
...
public:
    ...
    struct npc_Apothecary_HanesAI : public npc_escortAI {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 5:
                if (Unit* Trigger =
                        me->FindNearestCreature(NPC_HANES_FIRE_TRIGGER, 10.0f))
                    Trigger->CastSpell(
                        Trigger, SPELL_COSMETIC_LOW_POLY_FIRE, false);
                SetRun(false);
                break;
            case 6:
                if (Unit* Trigger =
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_REPORTS_FROM_THE_FIELD) ==
            QUEST_STATUS_INCOMPLETE)
            switch (creature->GetEntry()) {
            case NPC_RAZAEL:
                if (!player->GetReqKillOrCastCurrentCount(
                    ...
                    SendGossipMenuFor(
                        player, GOSSIP_TEXTID_RAZAEL1, creature->GetGUID());
                    return true;
                }
                break;
            case NPC_LYANA:
                if (!player->GetReqKillOrCastCurrentCount(
                    ...
                    SendGossipMenuFor(
                        player, GOSSIP_TEXTID_LYANA1, creature->GetGUID());
                    return true;
                }
                break;
            }
                        ...
                        uint32 action) override
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            SendGossipMenuFor(
                player, GOSSIP_TEXTID_RAZAEL2, creature->GetGUID());
            player->TalkedToCreature(NPC_RAZAEL, creature->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF + 2:
            SendGossipMenuFor(
                player, GOSSIP_TEXTID_LYANA2, creature->GetGUID());
