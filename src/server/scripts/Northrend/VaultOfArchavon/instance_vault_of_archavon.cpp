...
public:
    instance_vault_of_archavon()
        : InstanceMapScript("instance_vault_of_archavon", 624)
    {
    }

    struct instance_vault_of_archavon_InstanceMapScript
        ...
        void Update(uint32 diff) override
        {
            ...
            if (checkTimer >= 60000) {
                ...
                if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldByBattleId(
                        ...
                        if (bf->GetTimer() <= (16 * MINUTE * IN_MILLISECONDS) &&
                            ...
                            if (!PlayerList.IsEmpty())
                                for (Map::PlayerList::const_iterator i =
                                    ...
                                    if (Player* player = i->GetSource())
                                        player->TextEmote(
                                            "This instance will reset in 15 "
                                            "minutes.",
                                            nullptr,
                                            true);
                        }
                                 ...
                                 bf->GetTimer() >=
                                     (9 * MINUTE * IN_MILLISECONDS)) {
                            for (uint8 i = 0; i < MAX_ENCOUNTER; ++i)
                                if (Creature* cr =
                                        instance->GetCreature(bossGUIDs[i]))
                                    if (!cr->IsInCombat()) {
                                        cr->RemoveAllAuras();
                                        if (Aura* aur = cr->AddAura(
                                                SPELL_STONED_AURA, cr)) {
                                            aur->SetMaxDuration(
                                                60 * MINUTE * IN_MILLISECONDS);
                                            aur->SetDuration(60 * MINUTE *
                                                             IN_MILLISECONDS);
                                        }
                                    }
                            ...
                            if (!PlayerList.IsEmpty())
                                for (Map::PlayerList::const_iterator i =
                                    ...
                                    if (Player* player = i->GetSource())
                                        player->TextEmote(
                                            "This instance is about to reset. "
        ...
        bool IsEncounterInProgress() const override
        {
            ...
            Battlefield* bf = sBattlefieldMgr->GetBattlefieldByBattleId(
                BATTLEFIELD_BATTLEID_WG);
            if (!bf || bf->IsWarTime() ||
                bf->GetTimer() <= 10 * MINUTE * IN_MILLISECONDS)
                return true;
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            if (data == DONE) {
                ...
                // on every death of Archavon, Emalon and Koralon check our
                // achievement
                                          ...
                                          uint32 /*miscvalue1*/) override
        {
            ...
            case CRITERIA_EARTH_WIND_FIRE_25:
                if (ArchavonDeath && EmalonDeath && KoralonDeath) {
                    // instance difficulty check is already done in db
                    // (achievement_criteria_data) int() for Visual Studio,
                    // compile errors with std::abs(time_t)
                    return (
                        std::abs(int(ArchavonDeath - EmalonDeath)) < MINUTE &&
...
void AddSC_instance_vault_of_archavon() { new instance_vault_of_archavon(); }
