    ...
    struct boss_toravonAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(
                EVENT_CAST_WHITEOUT,
                25s); // schedule FIRST whiteout event in 25 seconds -1 for
                      // compesate updateai 2seconds check delay
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_FROZEN_ORB_STALKER:
                me->CastCustomSpell(SPELL_FROZEN_ORB,
            ...
            case EVENT_CAST_WHITEOUT:
                ...
                events.ScheduleEvent(EVENT_CAST_WHITEOUT_GROUND_EFFECT,
                                     1s); // triggers after 1 sec "plus 1 from
                                          // trigger to cast visual"
                events.Repeat(
                    40s); // next whiteout instead first 25 SEC is now 45 SEC
                break;
            case EVENT_CAST_WHITEOUT_GROUND_EFFECT: // Whiteout Ground effect
                                                    // trigger
                if (Unit* whiteOutGround =
                        me->SummonCreature(NPC_WHITEOUT_GROUND_EFFECT,
                                           -43.3316,
                                           -288.708,
                                           92.2511,
                                           1.58825,
                                           ...
                                           4000))
                    whiteOutGround->CastSpell(whiteOutGround,
    ...
    struct npc_frozen_orb_stalkerAI : public NullCreatureAI {
        ...
        void JustSummoned(Creature* cr) override
        {
            if (InstanceScript* pInstance = me->GetInstanceScript())
                if (Creature* toravon = ObjectAccessor::GetCreature(
                        *me, pInstance->GetGuidData(EVENT_TORAVON)))
                    if (toravon->AI())
