...
enum Archavon {
    ...
    SPELL_CRUSHING_LEAP_25 =
        60894, // Instant (10-80yr range) -- Leaps at an enemy, inflicting 8000
               // Physical damage, knocking all nearby enemies away, and
               // creating a cloud of choking debris.
    SPELL_STOMP_10  = 58663,
    SPELL_STOMP_25  = 60880,
    SPELL_IMPALE_10 = 58666,
    SPELL_IMPALE_25 =
        60882, // Lifts an enemy off the ground with a spiked fist, inflicting
               // 47125 to 52875 Physical damage and 9425 to 10575 additional
               // damage each second for 8 sec.
    SPELL_BERSERK = 47008
};
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_CHOKING_CLOUD:
                if (Unit* target =
                    ...
                    DoCast(target,
                           RAID_MODE(SPELL_CRUSHING_LEAP_10,
                                     SPELL_CRUSHING_LEAP_25),
                           true); // 10y ~ 80y, ignore range
                }
            ...
            case EVENT_STOMP: {
                ...
                snprintf(buffer,
                         sizeof(buffer),
                         "Archavon the Stone Watcher lunges for %s!",
                         me->GetVictim()->GetName().c_str());
                me->TextEmote(buffer);
...
public:
    spell_archavon_rock_shards()
        : SpellScriptLoader("spell_archavon_rock_shards")
    {
    }
    ...
    class spell_archavon_rock_shards_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            caster->CastSpell(
                target, SPELL_ROCK_SHARDS_RIGHT_HAND_VISUAL, true);

            uint32 spellId = map->Is25ManRaid() ? SPELL_ROCK_SHARDS_DAMAGE_25
