...
public:
    ...
    struct npc_frosthoundAI : public npc_escortAI {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            switch (waypointId) {
            case 0:
                me->TextEmote("You've been seen! Use the net and Freezing "
                              "elixir to keep the dwarves away!",
                              nullptr,
                              true);
                break;
            case 19:
                me->TextEmote("The frosthound has located the thief's hiding "
                              "place. Confront him!",
                              0,
                              true);
        ...
        {
            cr->ToTempSummon()->SetTempSummonType(
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (hpTimer >= 1000) {
                if (me->HealthBelowPct(40)) {
                    ...
                    me->ApplySpellImmune(SPELL_SHATTERED_EYES,
                                         IMMUNITY_MECHANIC,
                                         MECHANIC_STUN,
                                         false);
    ...
    struct npc_time_lost_proto_drakeAI : public npc_escortAI {
        ...
        void Reset() override
        {
            ...
            if (me->HasUnitState(UNIT_STATE_EVADE))
                return;
            me->SetVisible(
                false); // pussywizard: zeby nie dostawali info o npc w miejscu
                        // spawna (kampienie z addonem npc scan)
            rollPath = true;
        }
        ...
        void RollPath()
        {
            ...
            me->UpdateEntry(roll_chance_i(25) ? NPC_TIME_LOST_PROTO_DRAKE
                                              : NPC_VYRAGOSA,
                            0,
                            false);
        }
    ...
    class spell_q13003_thursting_hodirs_spear_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_q13003_thursting_hodirs_spear_AuraScript::HandlePeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            OnEffectApply += AuraEffectApplyFn(
                spell_q13003_thursting_hodirs_spear_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_q13003_thursting_hodirs_spear_AuraScript::AfterRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
    ...
    class spell_q13007_iron_colossus_SpellScript : public SpellScript {
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            if (GetSpellInfo()->Id == SPELL_JORMUNGAR_SUBMERGE) {
                caster->CastSpell(
                    caster, SPELL_JORMUNGAR_SUBMERGE_VISUAL, true);
                caster->ApplySpellImmune(SPELL_COLOSSUS_GROUND_SLAM,
                                         IMMUNITY_ID,
                ...
                if (CreatureTemplate const* ct =
                        sObjectMgr->GetCreatureTemplate(caster->GetEntry()))
                    for (uint8 i = 0; i < MAX_CREATURE_SPELLS; ++i)
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_q13007_iron_colossus_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (creature->IsVendor())
            if (player->HasSpell(SPELL_MECHANO_HOG) ||
                ...
                if (me->GetAreaId() == AREA_VALLEY_OF_ANCIENT_WINTERS) {
                    if (Vehicle* vehicle = me->GetVehicleKit())
                        if (Unit* passenger = vehicle->GetPassenger(0)) {
                            ...
                            Movement::PointsArray pathPoints;
                            pathPoints.push_back(
                                G3D::Vector3(me->GetPositionX(),
                                             me->GetPositionY(),
                                             me->GetPositionZ()));

                            WaypointPath const* i_path =
                                sWaypointMgr->GetPath(NPC_DRAKE);
                            for (uint8 i = 0; i < i_path->size(); ++i) {
                                WaypointData const* node = i_path->at(i);
                                pathPoints.push_back(
                                    G3D::Vector3(node->x, node->y, node->z));
                            ...
                            for (uint8 i = 1; i < 4; ++i)
                                if (Unit* prisoner =
                                    ...
                                    prisoner->CastSpell(
                                        ...
                                        true);
                                    prisoner->CastSpell(
                                        prisoner, SPELL_SUMMON_LIBERATED, true);
...
class npc_hyldsmeet_protodrake : public CreatureScript {
    enum NPCs { NPC_HYLDSMEET_DRAKERIDER = 29694 };
...
public:
    ...
    public:
        npc_hyldsmeet_protodrakeAI(Creature* creature)
            : CreatureAI(creature), _accessoryRespawnTimer(0),
        ...
        void PassengerBoarded(Unit* who, int8 /*seat*/, bool apply) override
        {
            if (apply) {
                ...
                public:
                    ...
                    bool Execute(uint64 /*eventTime*/,
                                 uint32 /*updateTime*/) override
                    {
                        _owner->m_movementInfo.transport.pos.Relocate(
                            -3.5f, 0.f, -0.2f, 0.f);
                        return true;
                    }
                ...
                if (who->IsPlayer())
                    who->m_Events.AddEvent(
                        new DelayedTransportPositionOffsets(who),
                        who->m_Events.CalculateTime(500));
        ...
        void UpdateAI(uint32 diff) override
        {
            //! We need to manually reinstall accessories because the vehicle
            //! itself is friendly to players, so EnterEvadeMode is never
            //! triggered. The accessory on the other hand is hostile and
            //! killable.
            if (_accessoryRespawnTimer && _accessoryRespawnTimer <= diff &&
                _vehicleKit) {
...
public:
    ...
    class spell_close_rift_AuraScript : public AuraScript {
        ...
        void HandlePeriodic(AuraEffect const* /* aurEff */)
        {
            if (++_counter == 5)
                GetTarget()->CastSpell(
                    (Unit*)nullptr, SPELL_DESPAWN_RIFT, true);
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_close_rift_AuraScript::HandlePeriodic,
                EFFECT_0,
...
class spell_q12823_remove_collapsing_cave_aura : public SpellScript {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_q12823_remove_collapsing_cave_aura::HandleScriptEffect,
            EFFECT_0,
...
public:
    npc_vehicle_d16_propelled_delivery()
        : CreatureScript("npc_vehicle_d16_propelled_delivery")
    {
    }
        ...
        PassengerBoarded(Unit* /*who*/, int8 /*seatId*/, bool apply) override
        {
            if (apply) {
                ...
                pathPoints.push_back(G3D::Vector3(me->GetPositionX(),
                                                  ...
                                                  me->GetPositionZ()));
                WaypointPath const* i_path =
                    sWaypointMgr->GetPath(me->GetEntry() * 100);
                for (uint8 i = 0; i < i_path->size(); ++i) {
                    WaypointData const* node = i_path->at(i);
                    pathPoints.push_back(
                        G3D::Vector3(node->x, node->y, node->z));
