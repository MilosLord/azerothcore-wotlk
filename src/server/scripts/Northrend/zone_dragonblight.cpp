...
enum DepthsTexts {
    ...
    // SAY_OACHANOA_SUMMONED_4                 = 4, // Unused(no source) and no
    // BroadcastTextId
...
public:
    npc_conversing_with_the_depths_trigger()
        : CreatureScript("npc_conversing_with_the_depths_trigger")
    {
    }
    ...
    struct npc_conversing_with_the_depths_triggerAI : public ScriptedAI {
        ...
        Creature* GetOachanoa()
        {
            return ObjectAccessor::GetCreature(*me, oachanoaGUID);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (running) {
                if (Player* p = GetPlayer())
                    if (p->GetPositionZ() < 1.0f &&
                        !secondpart) // Player is in the water
                    {
                ...
                if (timer != 0) {
                    ...
                    {
                        Creature* c =
                            me->SummonCreature(NPC_OACHANOA,
                                               2406.24f,
                                               1701.98f,
                                               0.1f,
                                               0.3f,
...
public:
    go_the_pearl_of_the_depths()
        : GameObjectScript("go_the_pearl_of_the_depths")
    {
    }
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        Creature* t = player->FindNearestCreature(
            NPC_CONVERSING_WITH_THE_DEPTHS_TRIGGER, 10.0f, true);
        if (t && t->AI() &&
            CAST_AI(npc_conversing_with_the_depths_trigger::
                        npc_conversing_with_the_depths_triggerAI,
                    t->AI()))
            if (!CAST_AI(npc_conversing_with_the_depths_trigger::
                             npc_conversing_with_the_depths_triggerAI,
                         t->AI())
                     ->running)
                CAST_AI(npc_conversing_with_the_depths_trigger::
...
enum hourglassText {
    // (All are whispers) Both NPC_PAST_YOU and NPC_FUTURE_YOU share the same
    // creature_text GroupIDs Start
...
public:
    ...
    struct npc_hourglass_of_eternityAI : public ScriptedAI {
            ...
            if (me->ToTempSummon())
                if (Unit* summoner = me->ToTempSummon()->GetSummonerUnit()) {
                    ...
                    me->GetNearPoint(summoner,
                                     ...
                                     0.0f,
                                     rand_norm() * 2 * M_PI);
                    if (Creature* cr = summoner->SummonCreature(
                            (IsFuture() ? NPC_FUTURE_YOU : NPC_PAST_YOU),
                            x,
                            y,
                            z,
                            0.0f,
        ...
        Creature* GetCopy()
        {
            return ObjectAccessor::GetCreature(*me, copyGUID);
        }

        uint32 randEntry() { return NPC_INFINITE_ASSAILANT + urand(0, 2); }

        void ShowNozdormu()
        {
            if (Creature* cr =
                    me->FindNearestCreature(NPC_NOZDORMU, 100.0f, true))
            ...
            case EVENT_CHECK_FINISH: {
                ...
                if (Player* player = GetPlayer())
                    player->GroupEventHappens(
                        IsFuture() ? QUEST_MYSTERY_OF_THE_INFINITE
                                   : QUEST_MYSTERY_OF_THE_INFINITE_REDUX,
                        me);

                if (Creature* cr = GetCopy()) {
                    cr->SetFacingToObject(
                        me->FindNearestCreature(NPC_NOZDORMU, 100.0f, true));
        ...
        {
            randomTalk = urand(SAY_HOURGLASS_RANDOM_1,
                               SAY_HOURGLASS_RANDOM_8); // 3 to 10
    ...
    struct npc_future_youAI : public ScriptedAI {
        ...
        void Reset() override
        {
            if (me->ToTempSummon() && me->ToTempSummon()->GetSummonerUnit())
                me->SetFaction(
                    me->ToTempSummon()->GetSummonerUnit()->GetFaction());
        }

        void MoveInLineOfSight(Unit* who) override
        {
            if (!me->GetVictim() && !who->IsFlying() &&
                who->GetEntry() >= NPC_INFINITE_ASSAILANT &&
                who->GetEntry() <= NPC_INFINITE_TIMERENDER)
                AttackStart(who);
        }
...
public:
    ...
    struct npc_mindless_ghoulAI : public ScriptedAI {
        ...
        void JustDied(Unit*) override { me->SetCorpseDelay(1); }
    };
};
...
public:
    npc_injured_7th_legion_soldier()
        : CreatureScript("npc_injured_7th_legion_soldier")
    {
    }
...
public:
    ...
    void JustDied(Unit* /*killer*/) override { me->DespawnOrUnsummon(3s, 0s); }
    ...
    Creature* getVillager()
    {
        return ObjectAccessor::GetCreature(*me, villagerGUID);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent()) {
            ...
            case EVENT_GET_VILLAGER: {
                if (getVillager()) {
                    getVillager()->GetMotionMaster()->MovePoint(
                        0, 3660.0f, -706.4f, 215.0f);
...
class spell_q12237_rescue_villager : public SpellScript {
    ...
    SpellCastResult CheckCast()
    {
        ...
        if (GetCaster()->FindNearestGameObject(GO_TEMP_GRYPHON_STATION,
                                               15.0f)) {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_q12237_rescue_villager::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
        OnCheckCast +=
            SpellCheckCastFn(spell_q12237_rescue_villager::CheckCast);
    }
};
...
class spell_q12237_drop_off_villager : public SpellScript {
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_q12237_drop_off_villager::CheckCast);
    }
};
...
class spell_call_wintergarde_gryphon : public SpellScript {
    ...
    SpellCastResult CheckRequirement()
    {
        if (Player* playerCaster = GetCaster()->ToPlayer()) {
            if (playerCaster->GetQuestStatus(
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_call_wintergarde_gryphon::CheckRequirement);
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_call_wintergarde_gryphon::SetDest,
            EFFECT_0,
...
public:
    spell_q12478_frostmourne_cavern()
        : SpellScriptLoader("spell_q12478_frostmourne_cavern")
    {
    }
    ...
    class spell_q12478_frostmourne_cavern_SpellScript : public SpellScript {
        ...
        void HandleSendEvent(SpellEffIndex effIndex)
        {
            ...
            GetCaster()->SummonCreature(
                NPC_PRINCE_ARTHAS, 4821.3f, -580.14f, 163.541f, 4.57f);
        }

        void Register() override
        {
            OnEffectHit += SpellEffectFn(
                spell_q12478_frostmourne_cavern_SpellScript::HandleSendEvent,
                EFFECT_0,
...
public:
    spell_q12243_fire_upon_the_waters()
        : SpellScriptLoader("spell_q12243_fire_upon_the_waters")
    {
    }
    ...
    class spell_q12243_fire_upon_the_waters_AuraScript : public AuraScript {
                               ...
                               AuraEffectHandleModes /*mode*/)
        {
            ...
            GetTarget()->GetCreatureListWithEntryInGrid(
                servants, 27233 /*NPC_ONSLAUGHT_DECKHAND*/, 40.0f);
            for (std::list<Creature*>::const_iterator itr = servants.begin();
                 itr != servants.end();
                 ++itr) {
                (*itr)->SetSpeed(MOVE_RUN, 0.7f, true);
                (*itr)->GetMotionMaster()->MoveFleeing(GetTarget(),
                                                       GetDuration());
            }
        }
                                ...
                                AuraEffectHandleModes /*mode*/)
        {
            std::list<Creature*> servants;
            GetTarget()->GetCreatureListWithEntryInGrid(
                servants, 27233 /*NPC_ONSLAUGHT_DECKHAND*/, 100.0f);
            for (std::list<Creature*>::const_iterator itr = servants.begin();
                 itr != servants.end();
                 ++itr)
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_q12243_fire_upon_the_waters_AuraScript::HandleApplyEffect,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_q12243_fire_upon_the_waters_AuraScript::
                    HandleRemoveEffect,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
...
public:
    ...
    {
        if (creature->GetEntry() == NPC_SAC_WRETCHED_GHOUL &&
            creature->GetDisplayId() != 11686 && creature->IsAlive())
    ...
    struct npc_q24545_lich_kingAI : public NullCreatureAI {
        ...
        void CleanAll(bool fromReset = true)
        {
            if (Creature* c = me->FindNearestCreature(
                    NPC_SAC_LIGHTS_VENGEANCE_BUNNY, 150.0f, true))
                c->RemoveAllAuras();
            if (fromReset) {
                if (Creature* c = me->FindNearestCreature(
                        NPC_SAC_LIGHTS_VENGEANCE, 150.0f, true))
                    c->DespawnOrUnsummon(1);
                if (Creature* c = me->FindNearestCreature(
                        NPC_SAC_LIGHTS_VENGEANCE_VEH_1, 150.0f, true))
                    c->RemoveAllAuras();
            }
            if (Creature* c = me->FindNearestCreature(
                    NPC_SAC_LIGHTS_VENGEANCE_VEH_2, 150.0f, true))
                c->DespawnOrUnsummon(1);
            if (GameObject* go = me->FindNearestGameObject(
                    GO_SAC_LIGHTS_VENGEANCE_1, 150.0f))
                go->Delete();
            if (GameObject* go = me->FindNearestGameObject(
                    GO_SAC_LIGHTS_VENGEANCE_2, 150.0f))
                go->Delete();
        ...
        {
            if (playerGUID || events.GetNextEventTime(998) ||
                events.GetNextEventTime(2))
                return;
            ...
            if (data == 1)
                ...
                if (Creature* c = me->FindNearestCreature(
                    ...
                    if (Player* p = ObjectAccessor::GetPlayer(*me, playerGUID))
                        p->KnockbackFrom(
                            c->GetPositionX(), c->GetPositionY(), 5.0f, 3.0f);
                }
                ...
                if (Creature* c = me->FindNearestCreature(
                    ...
                    if (GameObject* go = me->FindNearestGameObject(
                            GO_SAC_LIGHTS_VENGEANCE_3, 150.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 999: // apply holy aura
                if (Creature* c = me->FindNearestCreature(
                        NPC_SAC_LIGHTS_VENGEANCE_VEH_1, 150.0f, true))
                    if (Creature* l =
                            me->SummonCreature(NPC_SAC_LIGHTS_VENGEANCE,
                                               *c,
            ...
            case 1: // check player
                if (playerGUID) {
                    bool valid = false;
                    if (Player* p = ObjectAccessor::GetPlayer(*me, playerGUID))
                        if (p->IsAlive() && p->GetPhaseMask() & 2 &&
                            p->GetExactDistSq(me) < 100.0f * 100.0f &&
                            !p->IsGameMaster())
                            valid = true;
                    if (!valid) {
                        Reset();
            ...
            case 8: // summon ghouls
                me->CastSpell(
                    (Unit*)nullptr, SPELL_SAC_SUMMON_GHOULS_AURA, false);
                break;
            case 9: // talk 3
                Talk(3);
            ...
            case 10: // summon vegard
                me->SummonCreature(NPC_SAC_VEGARD_1,
                                   4812.12f,
                                   -586.08f,
                                   162.49f,
                                   3.14f,
            ...
            case 11: // vagard shield
                if (Creature* c =
                        me->FindNearestCreature(NPC_SAC_VEGARD_1, 50.0f, true))
            ...
            case 15: // remove light
                if (Creature* x = me->FindNearestCreature(
                        NPC_SAC_LIGHTS_VENGEANCE_VEH_2, 150.0f, true))
                    if (Creature* c = me->FindNearestCreature(
                            NPC_SAC_LIGHTS_VENGEANCE_BUNNY, 150.0f, true)) {
                        ...
                        if (Creature* l =
                            ...
                            x->SetHover(true);
                            x->NearTeleportTo(
                                4812.09f, -585.55f, 172.03f, 3.75f);
                            l->EnterVehicle(x, 1);
        ...
        {
            if (spell->Id == SPELL_SAC_REPEL_HAMMER &&
...
public:
    at_q24545_frostmourne_cavern()
        : AreaTriggerScript("at_q24545_frostmourne_cavern")
    {
    }
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*areaTrigger*/) override
    {
        if (player->GetPhaseMask() & 2)
            if (Creature* c =
                    player->FindNearestCreature(NPC_SAC_LICH_KING, 60.0f, true))
...
public:
    ...
    struct npc_q24545_wretched_ghoulAI : public ScriptedAI {
        ...
        void Reset() override { me->SetCorpseDelay(3); }
        ...
        void Activate()
        {
            ...
            me->m_Events.AddEvent(new SACActivateEvent(me),
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            me->m_Events.AddEvent(new SACDeactivateEvent(me),
                                  me->m_Events.CalculateTime(4000));
        }

        void JustRespawned() override { Deactivate(); }
    };
};
...
public:
    ...
    {
        return _alive ^ (object->GetTypeId() != TYPEID_UNIT ||
                         ((Unit*)object)->GetDisplayId() != 11686);
    }
            ...
            if (Unit* target = GetHitUnit())
                if (target->GetTypeId() == TYPEID_UNIT)
                    target->ToCreature()->AI()->DoAction(
                        GetSpellInfo()->Id == 70790 ? -2 : -1);
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_q24545_aod_special_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENTRY);
            OnEffectHitTarget += SpellEffectFn(
                spell_q24545_aod_special_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    struct npc_q24545_vegardAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Creature* c =
                    me->FindNearestCreature(NPC_SAC_LICH_KING, 200.0f, true))
                c->AI()->SetData(3, 3);
        }
    ...
    struct npc_spiritual_insightAI : public NullCreatureAI {
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            std::string const& text = sCreatureTextMgr->GetLocalizedChatString(
                me->GetEntry(), 0, id, 0, LOCALE_enUS);
            WorldPacket data;
            ...
            if (id == 1)
                if (Aura* aura =
                        summoner->ToUnit()->GetAura(47189)) // Transform Aura
...
Position const PosTalkLocations[6] = {
    ...
    {3807.508f,
     -691.0882f,
     221.9688f,
     2.094395f}, // Death knight wing of Naxxramas
    {3797.228f,
     -690.3555f,
     222.5019f,
    ...
    {3815.097f,
     -680.2596f,
     221.9777f,
     2.86234f}, // Dread plague wing of Naxxramas
...
public:
    npc_commander_eligor_dawnbringer()
        : CreatureScript("npc_commander_eligor_dawnbringer")
    {
    }
    ...
    struct npc_commander_eligor_dawnbringerAI : public ScriptedAI {
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE) {
                if (id == 1) {
                    me->SetFacingTo(
                        PosTalkLocations[talkWing].GetOrientation());
                    TurnAudience();
                    ...
                    case 0: // Pinnacle of Naxxramas
                    {
                        switch (urand(0, 1)) {
                        case 0:
                            ChangeImage(NPC_IMAGE_OF_KELTHUZAD,
                                        MODEL_IMAGE_OF_KELTHUZAD,
                                        SAY_KELTHUZAD_1);
                            _events.ScheduleEvent(EVENT_KELTHUZAD_2, 8s);
                            break;
                        case 1:
                            ChangeImage(NPC_IMAGE_OF_SAPPHIRON,
                    ...
                    case 1: // Death knight wing of Naxxramas
                    {
                        switch (urand(0, 2)) {
                        case 0:
                            ChangeImage(NPC_IMAGE_OF_RAZUVIOUS,
                                        MODEL_IMAGE_OF_RAZUVIOUS,
                                        SAY_RAZUVIOUS);
                            break;
                        case 1:
                            ChangeImage(NPC_IMAGE_OF_GOTHIK,
                                        MODEL_IMAGE_OF_GOTHIK,
                                        SAY_GOTHIK);
                            break;
                        case 2:
                            ChangeImage(NPC_IMAGE_OF_THANE,
                    ...
                    case 2: // Blighted abomination wing of Naxxramas
                    {
                        switch (urand(0, 3)) {
                        case 0:
                            ChangeImage(NPC_IMAGE_OF_PATCHWERK,
                                        MODEL_IMAGE_OF_PATCHWERK,
                                        SAY_PATCHWERK);
                            break;
                        case 1:
                            ChangeImage(NPC_IMAGE_OF_GROBBULUS,
                                        MODEL_IMAGE_OF_GROBBULUS,
                                        SAY_GROBBULUS);
                            break;
                        case 2:
                            ChangeImage(NPC_IMAGE_OF_THADDIUS,
                                        MODEL_IMAGE_OF_THADDIUS,
                                        SAY_THADDIUS);
                            break;
                        case 3:
                            ChangeImage(NPC_IMAGE_OF_GLUTH,
                    ...
                    case 3: // Accursed spider wing of Naxxramas
                    {
                        switch (urand(0, 2)) {
                        case 0:
                            ChangeImage(NPC_IMAGE_OF_ANUBREKHAN,
                                        MODEL_IMAGE_OF_ANUBREKHAN,
                                        SAY_ANUBREKHAN);
                            break;
                        case 1:
                            ChangeImage(NPC_IMAGE_OF_FAERLINA,
                                        MODEL_IMAGE_OF_FAERLINA,
                                        SAY_FAERLINA);
                            break;
                        case 2:
                            ChangeImage(NPC_IMAGE_OF_MAEXXNA,
                    ...
                    case 4: // Dread plague wing of Naxxramas
                    {
                        switch (urand(0, 2)) {
                        case 0:
                            ChangeImage(NPC_IMAGE_OF_NOTH,
                                        MODEL_IMAGE_OF_NOTH,
                                        SAY_NOTH);
                            break;
                        case 1:
                            ChangeImage(NPC_IMAGE_OF_HEIGAN,
                                        MODEL_IMAGE_OF_HEIGAN,
                                        SAY_HEIGAN_1);
                            _events.ScheduleEvent(EVENT_HEIGAN_2, 8s);
                            break;
                        case 2:
                            ChangeImage(NPC_IMAGE_OF_LOATHEB,
        ...
        void StoreTargets()
        {
            ...
            for (uint8 ii = 0; ii < 3; ++ii) {
                ...
                GetCreatureListWithEntryInGrid(
                    creatureList, me, AudienceMobs[ii], 15.0f);
                for (std::list<Creature*>::iterator itr = creatureList.begin();
                     itr != creatureList.end();
                     ++itr) {
            ...
            if (Creature* creature = me->FindNearestCreature(
                    NPC_IMAGE_OF_KELTHUZAD, 20.0f, true))
                imageList[0] = creature->GetGUID();
            if (Creature* creature = me->FindNearestCreature(
                    NPC_IMAGE_OF_RAZUVIOUS, 20.0f, true))
                imageList[1] = creature->GetGUID();
            if (Creature* creature = me->FindNearestCreature(
                    NPC_IMAGE_OF_PATCHWERK, 20.0f, true))
                imageList[2] = creature->GetGUID();
            if (Creature* creature = me->FindNearestCreature(
                    NPC_IMAGE_OF_ANUBREKHAN, 20.0f, true))
                imageList[3] = creature->GetGUID();
            if (Creature* creature =
                    me->FindNearestCreature(NPC_IMAGE_OF_NOTH, 20.0f, true))
        ...
        {
            if (Creature* creature =
                    ObjectAccessor::GetCreature(*me, imageList[talkWing])) {
                Talk(text);
        ...
        void TurnAudience()
        {
            for (uint8 i = 0; i < 10; ++i) {
                if (Creature* creature =
                        ObjectAccessor::GetCreature(*me, audienceList[i]))
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_MOVE_TO_POINT:
                    ...
                    me->GetMotionMaster()->MovePoint(
                        1,
                        PosTalkLocations[talkWing].m_positionX,
                        PosTalkLocations[talkWing].m_positionY,
                        PosTalkLocations[talkWing].m_positionZ);
                    break;
                ...
                case EVENT_DEATH_KNIGHTS_2:
                    ...
                    if (Creature* creature = ObjectAccessor::GetCreature(
                            *me, imageList[talkWing])) {
...
class spell_q12096_q12092_dummy
    : public SpellScriptLoader // Strengthen the Ancients: On Interact Dummy to
                               // Woodlands Walker
{
public:
...
public:
    ...
    class spell_q12096_q12092_bark_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_q12096_q12092_bark_SpellScript::HandleDummy,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
...
class spell_dragonblight_corrosive_spit : public AuraScript {
    ...
    void AfterApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        if (GetTarget()->HasAura(
                aurEff->GetSpellInfo()->GetEffect(EFFECT_0).CalcValue()))
            GetAura()->Remove();
    }
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_dragonblight_corrosive_spit::AfterApply,
                              EFFECT_1,
                              SPELL_AURA_PERIODIC_DAMAGE,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_dragonblight_corrosive_spit::PeriodicTick,
            EFFECT_1,
            SPELL_AURA_PERIODIC_DAMAGE);
    }
};
