    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (creature->GetEntry() == 23484) {
            ...
            if (creature->IsVendor() &&
                (player->GetReputationRank(932) == REP_EXALTED) &&
                (player->GetReputationRank(935) == REP_EXALTED) &&
                (player->GetReputationRank(942) == REP_EXALTED)) {
        ...
        if (creature->GetEntry() == 23483) {
            // Scryers vendor
            if (creature->IsVendor() &&
                (player->GetReputationRank(934) == REP_EXALTED) &&
                (player->GetReputationRank(935) == REP_EXALTED) &&
                (player->GetReputationRank(942) == REP_EXALTED)) {
                AddGossipItemFor(player,
                                 GOSSIP_ICON_VENDOR,
...
public:
    ...
    public:
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            if (player &&
                player->GetQuestStatus(10211) == QUEST_STATUS_INCOMPLETE)
