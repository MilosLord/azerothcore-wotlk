...
public:
    spell_q10930_big_bone_worm()
        : SpellScriptLoader("spell_q10930_big_bone_worm")
    {
    }
    ...
    class spell_q10930_big_bone_worm_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
                spell_q10930_big_bone_worm_SpellScript::SetDest,
                EFFECT_1,
                TARGET_DEST_CASTER);
        }
    };
    ...
    class spell_q10930_big_bone_worm_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectRemove += AuraEffectRemoveFn(
                spell_q10930_big_bone_worm_AuraScript::HandleEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
...
public:
    ...
    class spell_q10929_fumping_SpellScript : public SpellScript {
        ...
        {
            OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
                spell_q10929_fumping_SpellScript::SetDest,
                EFFECT_1,
                TARGET_DEST_CASTER);
        }
    };
    ...
    class spell_q10929_fumping_AuraScript : public AuraScript {
        ...
        {
            OnEffectRemove += AuraEffectRemoveFn(
                spell_q10929_fumping_AuraScript::HandleEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    ...
    class spell_q10036_torgos_SpellScript : public SpellScript {
        ...
        {
            if (Creature* torgos =
                    GetCaster()->FindNearestCreature(NPC_TORGOS, 100.0f, true))
        ...
        {
            OnEffectLaunch +=
                SpellEffectFn(spell_q10036_torgos_SpellScript::HandleSendEvent,
                              EFFECT_0,
...
public:
    spell_q10923_evil_draws_near_summon()
        : SpellScriptLoader("spell_q10923_evil_draws_near_summon")
    {
    }
    ...
    class spell_q10923_evil_draws_near_summon_SpellScript : public SpellScript {
        ...
        void HandleSendEvent(SpellEffIndex /*effIndex*/)
        {
            if (Creature* auchenai = GetCaster()->FindNearestCreature(
                    NPC_AUCHENAI_DEATH_SPIRIT, 10.0f, true))
                auchenai->CastSpell(
                    auchenai, SPELL_DUSTIN_UNDEAD_DRAGON_VISUAL_AURA, true);
        }
...
public:
    spell_q10923_evil_draws_near_periodic()
        : SpellScriptLoader("spell_q10923_evil_draws_near_periodic")
    {
    }
    ...
    class spell_q10923_evil_draws_near_periodic_AuraScript : public AuraScript {
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            GetUnitOwner()->CastSpell(GetUnitOwner(),
                                      RAND(SPELL_DUSTIN_UNDEAD_DRAGON_VISUAL1,
...
public:
    spell_q10923_evil_draws_near_visual()
        : SpellScriptLoader("spell_q10923_evil_draws_near_visual")
    {
    }
    ...
    class spell_q10923_evil_draws_near_visual_SpellScript : public SpellScript {
        ...
        {
            OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
                spell_q10923_evil_draws_near_visual_SpellScript::SetDest,
                EFFECT_0,
    ...
    class spell_q10898_skywing_SpellScript : public SpellScript {
        ...
        void SetDest(SpellDestination& dest)
        {
            ...
            Position const offset = {
                frand(-7.0f, 7.0f), frand(-7.0f, 7.0f), 11.0f, 0.0f};
        ...
        {
            OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
                spell_q10898_skywing_SpellScript::SetDest,
                EFFECT_0,
    ...
    struct npc_unkor_the_ruthlessAI : public ScriptedAI {
                         ...
                         SpellSchoolMask) override
        {
            ...
            if (player && me->HealthBelowPctDamaged(30, damage)) {
                if (Group* group = player->GetGroup()) {
                    for (GroupReference* itr = group->GetFirstMember();
                        ...
                        if (groupie && groupie->IsInMap(player) &&
                            groupie->GetQuestStatus(QUEST_DONTKILLTHEFATONE) ==
                                QUEST_STATUS_INCOMPLETE &&
                            groupie->GetReqKillOrCastCurrentCount(
                                QUEST_DONTKILLTHEFATONE, 18260) == 10) {
                            groupie->AreaExploredOrEventHappens(
                ...
                else if (player->GetQuestStatus(QUEST_DONTKILLTHEFATONE) ==
                             QUEST_STATUS_INCOMPLETE &&
                         player->GetReqKillOrCastCurrentCount(
                             QUEST_DONTKILLTHEFATONE, 18260) == 10) {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (CanDoQuest) {
                if (!UnkorUnfriendly_Timer) {
                    // DoCast(me, SPELL_QUID9889);        //not using spell for
                    // now
                    DoNice();
                }
                else {
...
public:
    ...
    {
        if (quest->GetQuestId() == QUEST_EFTW_H ||
            quest->GetQuestId() == QUEST_EFTW_A) {
...
public:
    ...
    {
        if ((player->GetQuestStatus(QUEST_ADVERSARIAL_BLOOD) ==
             QUEST_STATUS_INCOMPLETE) ||
            ...
            SendGossipMenuFor(
                player, GOSSIP_MENU_TEXT_SKULL_PILE_QUEST, go->GetGUID());
        }
        else
            SendGossipMenuFor(
                player, GOSSIP_MENU_TEXT_SKULL_PILE, go->GetGUID());
    ...
    void SendActionMenu(Player* player, GameObject* go, uint32 action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            _result = player->CastSpell(
                player, SPELL_SUMMON_GEZZARAK_THE_HUNTRESS, false);
            break;
        case GOSSIP_ACTION_INFO_DEF + 2:
            _result = player->CastSpell(
                player, SPELL_SUMMON_DARKSCREECHER_AKKARAI, false);
            break;
        ...
        case GOSSIP_ACTION_INFO_DEF + 4:
            _result = player->CastSpell(
                player, SPELL_SUMMON_VAKKIZ_THE_WINDRAGER, false);
            break;
        default:
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (creature->IsVendor() &&
            player->GetReputationRank(FACTION_CONSORTIUM) >= REP_FRIENDLY) {
            AddGossipItemFor(player,
                             GOSSIP_ICON_VENDOR,
