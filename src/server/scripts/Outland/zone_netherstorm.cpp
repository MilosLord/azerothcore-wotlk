...
public:
    ...
    struct npc_captain_saeedAI : public npc_escortAI {
        npc_captain_saeedAI(Creature* creature)
        ...
        void SetGUID(ObjectGuid playerGUID, int32 type) override
        {
            if (type == DATA_START_ENCOUNTER) {
                ...
                me->GetCreaturesWithEntryInRange(
                    cl, 20.0f, NPC_PROTECTORATE_AVENGER);
                ...
                cl.clear();
                me->GetCreaturesWithEntryInRange(
                    cl, 20.0f, NPC_PROTECTORATE_DEFENDER);
                for (std::list<Creature*>::iterator itr = cl.begin();
                     itr != cl.end();
                     ++itr) {
                    summons.Summon(*itr);
        ...
        {
            float i = 0;
            for (GuidList::iterator itr = summons.begin(); itr != summons.end();
                 ++itr, i += 1.0f)
                if (Creature* cr = ObjectAccessor::GetCreature(*me, *itr)) {
                    if (!who) {
                        ...
                        cr->GetMotionMaster()->MoveFollow(
                            me,
                            2.0f,
                            M_PI / 2.0f + (i / summons.size() * M_PI));
                    }
                    else {
                        cr->SetHomePosition(cr->GetPositionX(),
                                            cr->GetPositionY(),
                                            cr->GetPositionZ(),
                                            cr->GetOrientation());
        ...
        void JustEngagedWith(Unit* who) override { SummonsAction(who); }
        ...
        void CorpseRemoved(uint32&) override { summons.DespawnAll(); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_START_FIGHT2:
                if (Creature* dimensius =
                        me->FindNearestCreature(NPC_DIMENSIUS, 50.0f)) {
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_DIMENSIUS_DEVOURING) ==
...
// Entries of Arcanist Ardonis, Commander Dawnforge, Pathaleon the Curators
// Image
...
public:
    ...
    struct npc_commander_dawnforgeAI : public ScriptedAI {
        ...
        void Turn_to_Pathaleons_Image()
        {
            ...
            Creature* pathaleon =
                ObjectAccessor::GetCreature(*me, pathaleonGUID);
        ...
        bool CanStartEvent(Player* player)
        {
            if (!isEvent) {
                Creature* ardonis =
                    me->FindNearestCreature(CreatureEntry[0], 10.0f);
                if (!ardonis)
                    return false;
            ...
            LOG_DEBUG("scripts.ai",
                      "npc_commander_dawnforge event already in progress, need "
                      "to wait.");
            return false;
        }
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        ...
        if (player->IsAlive() && player->GetQuestStatus(QUEST_INFO_GATHERING) ==
                                     QUEST_STATUS_INCOMPLETE) {
            Creature* Dawnforge =
                player->FindNearestCreature(CreatureEntry[1], 30.0f);
            if (!Dawnforge)
                return false;

            if (CAST_AI(npc_commander_dawnforge::npc_commander_dawnforgeAI,
                        Dawnforge->AI())
...
public:
    ...
    struct npc_phase_hunterAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED) ||
                me->HasUnitState(
                    UNIT_STATE_ROOT)) // if the mob is rooted/slowed by spells
                                      // eg.: Entangling Roots, Frost Nova,
                                      // Hamstring, Crippling Poison, etc. =>
                                      // remove it
            ...
            if (ManaBurnTimer <= diff) {
                std::list<HostileReference*> AggroList =
            ...
            if (Player* player = ObjectAccessor::GetPlayer(
                    *me, PlayerGUID)) // start: support for quest 10190
            {
                if (!Weak && HealthBelowPct(WeakPercent) &&
                    player->GetQuestStatus(QUEST_RECHARGING_THE_BATTERIES) ==
                        QUEST_STATUS_INCOMPLETE) {
                    Talk(EMOTE_WEAK);
                    Weak = true;
                }
...
public:
    ...
    struct npc_bessyAI : public npc_escortAI {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 7:
                Talk(SAY_BESSY_1);
                me->SummonCreature(SPAWN_SECOND,
                                   2309.64f,
                                   2186.24f,
                                   92.25f,
                                   6.06f,
...
public:
    ...
    struct npc_maxx_a_million_escortAI : public npc_escortAI {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 29:
                ...
                if (GetClosestGameObjectWithEntry(
                        me, GO_DRAENEI_MACHINE, INTERACTION_DISTANCE)) {
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_MARK_V_IS_ALIVE) {
            if (npc_maxx_a_million_escortAI* pEscortAI = CAST_AI(
                    npc_maxx_a_million_escort::npc_maxx_a_million_escortAI,
                    creature->AI())) {
                creature->SetFaction(FACTION_ESCORTEE_N_NEUTRAL_PASSIVE);
...
class spell_q10190_battery_recharging_blaster : public SpellScript {
    ...
    void Register() override
    {
        OnCheckCast += SpellCheckCastFn(
            spell_q10190_battery_recharging_blaster::CheckCast);
    }
};
...
class spell_q10190_battery_recharging_blaster_aura : public AuraScript {
    ...
    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(
            spell_q10190_battery_recharging_blaster_aura::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
...
enum Veraku { NPC_VERAKU = 18544, SPELL_CHALLENGE_VERAKU = 34895 };
...
public:
    ...
    SpellCastResult CheckRequirement()
    {
        if (Unit* caster = GetCaster())
            if (Creature* veraku =
                    caster->FindNearestCreature(NPC_VERAKU, 100.0f))
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_challenge_veraku::CheckRequirement);
    }
};
...
void AddSC_netherstorm()
{
    ...
    RegisterSpellAndAuraScriptPair(
        spell_q10190_battery_recharging_blaster,
        spell_q10190_battery_recharging_blaster_aura);
