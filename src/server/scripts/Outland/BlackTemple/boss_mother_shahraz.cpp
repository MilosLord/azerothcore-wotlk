...
public:
    ...
    struct boss_shahrazAI : public BossAI {
        boss_shahrazAI(Creature* creature)
            : BossAI(creature, DATA_MOTHER_SHAHRAZ)
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_PRISMATIC_AURA:
                me->CastSpell(me,
                              RAND(SPELL_PRISMATIC_AURA_SHADOW,
            ...
            case EVENT_SPELL_FATAL_ATTRACTION:
                Talk(SAY_SPELL);
                me->CastCustomSpell(SPELL_FATAL_ATTRACTION,
...
public:
    spell_mother_shahraz_random_periodic()
        : SpellScriptLoader("spell_mother_shahraz_random_periodic")
    {
    }
    ...
    class spell_mother_shahraz_random_periodic_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_mother_shahraz_random_periodic_AuraScript::Update,
                EFFECT_0,
...
public:
    spell_mother_shahraz_beam_periodic()
        : SpellScriptLoader("spell_mother_shahraz_beam_periodic")
    {
    }
    ...
    class spell_mother_shahraz_beam_periodic_AuraScript : public AuraScript {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_mother_shahraz_beam_periodic_AuraScript::Update,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };
...
public:
    spell_mother_shahraz_saber_lash()
        : SpellScriptLoader("spell_mother_shahraz_saber_lash")
    {
    }
    ...
    class spell_mother_shahraz_saber_lash_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(
                spell_mother_shahraz_saber_lash_AuraScript::CheckProc);
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_mother_shahraz_saber_lash_AuraScript::Update,
                EFFECT_1,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };
...
public:
    spell_mother_shahraz_fatal_attraction()
        : SpellScriptLoader("spell_mother_shahraz_fatal_attraction")
    {
    }

    class spell_mother_shahraz_fatal_attraction_SpellScript
        : public SpellScript {
        ...
        {
            targets.remove_if(
                Acore::UnitAuraCheck(true, SPELL_SABER_LASH_IMMUNITY));
            if (targets.size() <= 1)
        ...
        void SetDest(SpellDestination& dest)
        {
            ...
            for (std::list<TargetInfo>::const_iterator ihit =
                 ...
                 ++ihit)
                if (Unit* target = ObjectAccessor::GetUnit(*GetCaster(),
                                                           ihit->targetGUID)) {
        ...
        void HandleTeleportUnits(SpellEffIndex /*effIndex*/)
        {
            if (Unit* target = GetHitUnit())
                GetCaster()->CastSpell(
                    target, SPELL_FATAL_ATTRACTION_AURA, true);
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_mother_shahraz_fatal_attraction_SpellScript::
                    FilterTargets,
                EFFECT_ALL,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
                spell_mother_shahraz_fatal_attraction_SpellScript::SetDest,
                EFFECT_1,
...
public:
    spell_mother_shahraz_fatal_attraction_dummy()
        : SpellScriptLoader("spell_mother_shahraz_fatal_attraction_dummy")
    {
    }

    class spell_mother_shahraz_fatal_attraction_dummy_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* target = GetHitUnit()) {
                ...
                if (AuraEffect* aurEff = target->GetAuraEffect(
                        SPELL_FATAL_ATTRACTION_AURA, EFFECT_1))
...
public:
    spell_mother_shahraz_fatal_attraction_aura()
        : SpellScriptLoader("spell_mother_shahraz_fatal_attraction_aura")
    {
    }

    class spell_mother_shahraz_fatal_attraction_aura_AuraScript
        : public AuraScript {
        PrepareAuraScript(
            spell_mother_shahraz_fatal_attraction_aura_AuraScript);
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_mother_shahraz_fatal_attraction_aura_AuraScript::Update,
                EFFECT_1,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };
