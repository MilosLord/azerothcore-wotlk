...
SD%Complete: 100
SDComment: Quest support: 10519, 10583, 10601, 10804, 10854, 10458, 10481,
10480, 10781. Vendor Drake Dealer Hurlunk. SDCategory: Shadowmoon Valley
EndScriptData */
...
public:
    spell_q10612_10613_the_fel_and_the_furious()
        : SpellScriptLoader("spell_q10612_10613_the_fel_and_the_furious")
    {
    }

    class spell_q10612_10613_the_fel_and_the_furious_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_q10612_10613_the_fel_and_the_furious_SpellScript);
        ...
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            ...
            uint8 counter = 0;
            for (std::list<GameObject*>::const_iterator itr = gList.begin();
                 itr != gList.end();
                 ++itr, ++counter) {
                if (counter >= 10)
                    break;
                ...
                if (!go->isSpawned())
                    continue;
                Creature* cr2 = go->SummonTrigger(go->GetPositionX(),
                                                  go->GetPositionY(),
                                                  go->GetPositionZ() + 2.0f,
                                                  0.0f,
                                                  100);
                if (cr2) {
...
public:
    spell_q10563_q10596_to_legion_hold()
        : SpellScriptLoader("spell_q10563_q10596_to_legion_hold")
    {
    }
...
public:
    ...
    struct npc_invis_infernal_casterAI : public ScriptedAI {
        ...
        {
            Creature* infernal =
                me->SummonCreature(NPC_INFERNAL_ATTACKER,
                                   me->GetPositionX(),
                                   me->GetPositionY(),
                                   ground + 0.05f,
                                   0.0f,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_CAST_SUMMON_INFERNAL: {
                    if (Unit* infernal =
                            ObjectAccessor::GetUnit(*me, infernalGUID))
                        if (infernal->GetDisplayId() == MODEL_INVISIBLE)
                            me->CastSpell(
                                infernal, SPELL_SUMMON_INFERNAL, true);
...
public:
    npc_mature_netherwing_drake()
        : CreatureScript("npc_mature_netherwing_drake")
    {
    }
    ...
    struct npc_mature_netherwing_drakeAI : public ScriptedAI {
        ...
        void SpellHit(Unit* pCaster, SpellInfo const* spell) override
        {
            ...
            if (pCaster->GetTypeId() == TYPEID_PLAYER &&
                spell->Id == SPELL_PLACE_CARCASS &&
                !me->HasAura(SPELL_JUST_EATEN)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (bCanEat || bIsEating) {
                if (EatTimer <= diff) {
                    if (bCanEat && !bIsEating) {
                        if (Unit* unit =
                                ObjectAccessor::GetUnit(*me, uiPlayerGUID)) {
                            if (GameObject* go = unit->FindNearestGameObject(
                                    GO_CARCASS, 10)) {
                        ...
                        if (Player* player =
                                ObjectAccessor::GetPlayer(*me, uiPlayerGUID)) {
                            player->KilledMonsterCredit(NPC_EVENT_PINGER);

                            if (GameObject* go = player->FindNearestGameObject(
                                    GO_CARCASS, 10))
                                go->Delete();
                        }
...
public:
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell) override
    {
        ...
        if (spell->Id == SPELL_HIT_FORCE_OF_NELTHARAKU && !Tapped &&
            playerCaster->GetQuestStatus(QUEST_THE_FORCE_OF_NELTHARAKU) ==
                QUEST_STATUS_INCOMPLETE) {
            Tapped     = true;
            PlayerGUID = caster->GetGUID();

            me->SetFaction(
                FACTION_FLAYER_HUNTER); // Not sure if this is correct, it was
                                        // taken off of Mordenai.

            if (Unit* Dragonmaw =
                    me->FindNearestCreature(NPC_DRAGONMAW_SUBJUGATOR, 25.0f))
                AttackStart(Dragonmaw);
            else if (Unit* Dragonmaw =
                         me->FindNearestCreature(NPC_DRAGONMAW_WRANGLER, 25.0f))
                AttackStart(Dragonmaw);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (!UpdateVictim()) {
            ...
            case EVENT_TAKE_OFF: {
                ...
                Position pos;
                if (Unit* EscapeDummy =
                        me->FindNearestCreature(NPC_ESCAPE_DUMMY, 30))
...
public:
    ...
    struct npc_dragonmaw_peonAI : public ScriptedAI {
        ...
        void MovementInform(uint32 /*type*/, uint32 id) override
        {
            if (id == 1) {
                if (GameObject* food =
                        me->FindNearestGameObject(DELICIOUS_MUTTON, 5.0f))
        ...
        void CreditPlayer()
        {
            if (PlayerGUID) {
                ...
                if (player && player->GetQuestStatus(QUEST_A_SLOW_DEATH) ==
                                  QUEST_STATUS_INCOMPLETE)
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (creature->IsVendor() &&
            player->GetReputationRank(1015) == REP_EXALTED)
...
public:
                        ...
                        uint32 action) override
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF + 1) {
            ...
            uint8           msg = player->CanStoreNewItem(
                NULL_BAG, NULL_SLOT, dest, ITEM_FLAUNISS_PACK, 1, nullptr);
            if (msg == EQUIP_ERR_OK) {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF + 2) {
            ItemPosCountVec dest;
            uint8           msg = player->CanStoreNewItem(
                NULL_BAG, NULL_SLOT, dest, ITEM_KAGROSHS_PACK, 1, nullptr);
            if (msg == EQUIP_ERR_OK) {
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->GetQuestStatus(QUEST_THE_FATE_OF_FLANIS) ==
                QUEST_STATUS_INCOMPLETE &&
        ...
        if (player->GetQuestStatus(QUEST_THE_FATE_OF_KAGROSH) ==
                QUEST_STATUS_INCOMPLETE &&
...
npc_torloth_the_magnificent : Final Creature that players have to face before
quest is completed npc_lord_illidan_stormrage : Creature that controls the
event. go_crystal_prison : GameObject that begins the event and hands out quest
EndContentData */

#define QUEST_BATTLE_OF_THE_CRIMSON_WATCH 10781
#define EVENT_AREA_RADIUS 65 // 65yds
#define EVENT_COOLDOWN                                                         \
    30000 // in ms. appear after event completed or failed (should be = Adds
          // despawn time)
...
static TorlothCinematic TorlothAnim[] = {{0, 2000},
                                         {1, 7000},
                                         {0, 3000},
...
public:
    npc_torloth_the_magnificent()
        : CreatureScript("npc_torloth_the_magnificent")
    {
    }
    ...
    struct npc_torloth_the_magnificentAI : public ScriptedAI {
        ...
        void HandleAnimation()
        {
            ...
            case 5:
                if (Player* AggroTarget =
                        ObjectAccessor::GetPlayer(*me, AggroTargetGUID)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (Timers) {
                ...
                if (SpellTimer3 <= diff) {
                    ...
                    SpellTimer3 = SpawnCast[8].Timer2 +
                                  (rand() % 7 * 1000); // Spell Reflection
                }
                else
                    SpellTimer3 -= diff;
            }
        ...
        void JustDied(Unit* killer) override
        {
                ...
                if (Unit* owner = killer->GetOwner())
                    if (Player* player = owner->ToPlayer())
                        player->GroupEventHappens(
                            QUEST_BATTLE_OF_THE_CRIMSON_WATCH, me);
                break;
            case TYPEID_PLAYER:
                if (Player* player = killer->ToPlayer())
                    player->GroupEventHappens(QUEST_BATTLE_OF_THE_CRIMSON_WATCH,
                                              me);
                break;
            default:
                break;
            ...
            if (Creature* LordIllidan =
                    (ObjectAccessor::GetCreature(*me, LordIllidanGUID)))
        ...
        void CheckEventFail()
        {
            ...
            if (Group* EventGroup = player->GetGroup()) {
                ...
                Group::MemberSlotList const& members =
                    EventGroup->GetMemberSlots();

                for (Group::member_citerator itr = members.begin();
                     itr != members.end();
                     ++itr) {
                    Player* GroupMember =
                        ObjectAccessor::GetPlayer(*me, itr->guid);
                    if (!GroupMember)
                        continue;
                    if (!GroupMember->IsWithinDistInMap(me,
                                                        EVENT_AREA_RADIUS) &&
                        GroupMember->GetQuestStatus(
                            QUEST_BATTLE_OF_THE_CRIMSON_WATCH) ==
                            QUEST_STATUS_INCOMPLETE) {
                        GroupMember->FailQuest(
                            QUEST_BATTLE_OF_THE_CRIMSON_WATCH);
                        ++FailedMemberCount;
                    }
                    ++GroupMemberCount;
                ...
                if (GroupMemberCount == DeadMemberCount) {
                    ...
                    Failed = true;
                     ...
                     !player->IsWithinDistInMap(me, EVENT_AREA_RADIUS)) {
                ...
                if (SpellTimer1 <= diff) {
                    if (Unit* target =
                        ...
                        if (target->GetTypeId() == TYPEID_PLAYER) {
                            DoCast(target,
                                   SpawnCast[1].SpellId); // Focused Bursts
                            SpellTimer1 =
                                SpawnCast[1].Timer2 + (rand() % 5 * 1000);
                        }
                        else
                            SpellTimer1 = 2000;
                    }
...
void npc_lord_illidan_stormrage::npc_lord_illidan_stormrageAI::SummonNextWave()
{
    ...
    for (uint8 i = 0; i < count; ++i) {
        ...
        Spawn           = me->SummonCreature(WavesInfo[WaveCount].CreatureId,
                                   X,
                                   Y,
                                   Z,
                                   O,
        ...
        if (Spawn) {
            ...
            if (WaveCount < 3) // 1-3 Wave
            {
                ...
                CAST_AI(npc_illidari_spawn::npc_illidari_spawnAI, Spawn->AI())
                    ->LordIllidanGUID = me->GetGUID();
            }

            if (WavesInfo[WaveCount].CreatureId == 22076) // Torloth
            {
                CAST_AI(
                    npc_torloth_the_magnificent::npc_torloth_the_magnificentAI,
                    Spawn->AI())
                    ->LordIllidanGUID = me->GetGUID();
                if (PlayerGUID)
                    CAST_AI(npc_torloth_the_magnificent::
                                npc_torloth_the_magnificentAI,
                            Spawn->AI())
                        ->AggroTargetGUID = PlayerGUID;
            }
        }
    }
    ++WaveCount;
...
public:
                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_BATTLE_OF_THE_CRIMSON_WATCH) {
            ...
            if (Illidan &&
                !CAST_AI(
                     npc_lord_illidan_stormrage::npc_lord_illidan_stormrageAI,
                     Illidan->AI())
                     ->EventStarted) {
                CAST_AI(
                    npc_lord_illidan_stormrage::npc_lord_illidan_stormrageAI,
...
enum Enraged_Dpirits {
    ...
    // ENRAGED FIRE SPIRIT SPELLS - Will be using the enrage spell from Air
    // Spirit
...
public:
    ...
    struct npc_enraged_spiritAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            case NPC_ENRAGED_AIR_SPIRIT:
                _scheduler.Schedule(10s, [this](TaskContext context) {
                    ...
                    _scheduler.Schedule(
                        3s, 5s, [this](TaskContext /*context*/) {
                            if (UpdateVictim()) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->GetEntry() == NPC_ENRAGED_FIRE_SPIRIT ||
                me->GetEntry() == NPC_ENRAGED_AIR_SPIRIT) {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (entry != 0)
                Summoned = DoSpawnCreature(
                    entry, 0, 0, 1, 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 5000);

            // FIND TOTEM, PROCESS QUEST
            if (Summoned) {
                totemOspirits = me->FindNearestCreature(
                    ENTRY_TOTEM_OF_SPIRITS, RADIUS_TOTEM_OF_SPIRITS);
                if (totemOspirits) {
                    Summoned->SetFaction(FACTION_FRIENDLY);
                    Summoned->GetMotionMaster()->MovePoint(
                        0,
                        totemOspirits->GetPositionX(),
                        totemOspirits->GetPositionY(),
                        Summoned->GetPositionZ());
    ...
    struct npc_shadowmoon_tuber_nodeAI : public ScriptedAI {
        ...
        void SetData(uint32 id, uint32 data) override
        {
            if (id == TYPE_BOAR && data == DATA_BOAR) {
                ...
                // Despawn the tuber
                if (GameObject* tuber = me->FindNearestGameObject(
                        GO_SHADOWMOON_TUBER_MOUND, 5.0f)) {
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
        {
            if (!tapped && spell->Id == SPELL_WHISTLE) {
                if (Creature* boar =
                        me->FindNearestCreature(NPC_BOAR_ENTRY, 30.0f)) {
                    // Disable trigger and force nearest boar to walk to him
                    tapped = true;
                    boar->SetWalk(false);
                    boar->GetMotionMaster()->MovePoint(POINT_TUBER,
                                                       me->GetPositionX(),
                                                       me->GetPositionY(),
                                                       me->GetPositionZ());
                }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (tapped) {
                if (resetTimer <= diff) {
                    ...
                    if (tuberGUID)
                        if (GameObject* tuber =
                                ObjectAccessor::GetGameObject(*me, tuberGUID))
...
enum KorWild { SAY_LAND = 0, POINT_LAND = 1 };
...
class spell_calling_korkron_or_wildhammer : public SpellScript {
    ...
    void Register() override
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_calling_korkron_or_wildhammer::SetDest,
            EFFECT_0,
            TARGET_DEST_CASTER);
    }
};
...
public:
    ...
    SpellCastResult CheckRequirement()
    {
        if (Unit* caster = GetCaster())
            if (Creature* infernal =
                    caster->FindNearestCreature(NPC_INFERNAL_OVERSOUL, 100.0f))
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_disrupt_summoning_ritual::CheckRequirement);
    }
};
