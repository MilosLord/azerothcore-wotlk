...
enum Misc { ACTION_FLY_DOWN = 0, POINT_MIDDLE = 0, POINT_FLIGHT = 1 };

enum GroupPhase { GROUP_PHASE_1 = 0, GROUP_PHASE_2 = 1 };

const Position NazanPos[3] = {{-1430.37f, 1710.03f, 111.0f, 0.0f},
...
struct boss_vazruden_the_herald : public BossAI {
    ...
    void Reset() override
    {
        ...
        me->SummonCreature(
            NPC_HELLFIRE_SENTRY, -1372.56f, 1724.31f, 82.967f, 5.3058f);
        me->SummonCreature(
            NPC_HELLFIRE_SENTRY, -1383.39f, 1711.82f, 82.7961f, 5.67232f);
    }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == POINT_MOTION_TYPE && id == POINT_MIDDLE) {
            me->SetVisible(false);
            me->SummonCreature(NPC_VAZRUDEN,
                               me->GetPositionX(),
                               me->GetPositionY(),
                               81.2f,
                               5.46f);
            me->SummonCreature(NPC_NAZAN,
                               me->GetPositionX(),
                               me->GetPositionY(),
                               me->GetPositionZ(),
                               5.46f);
    ...
    void SummonedCreatureDies(Creature* summon, Unit*) override
    {
        ...
        if (summon->GetEntry() == NPC_HELLFIRE_SENTRY && summons.size() == 0) {
            ...
            me->GetMotionMaster()->MovePoint(
                POINT_MIDDLE, -1406.5f, 1746.5f, 85.0f, false);
            _JustEngagedWith();
        }
...
struct boss_nazan : public ScriptedAI {
        ...
        _scheduler
            .Schedule(5ms,
                      ...
                      [this](TaskContext context) {
                          me->GetMotionMaster()->MovePoint(
                              POINT_FLIGHT, NazanPos[urand(0, 2)], false);
    ...
    void DoAction(int32 param) override
    {
        if (param == ACTION_FLY_DOWN) {
            ...
            me->GetMotionMaster()->MovePoint(
                POINT_MIDDLE, -1406.5f, 1746.5f, 81.2f, false);
        }
    }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == POINT_MOTION_TYPE && id == POINT_MIDDLE) {
            ...
            if (IsHeroic()) {
                _scheduler.Schedule(
                    10s, GROUP_PHASE_2, [this](TaskContext context) {
...
struct boss_vazruden : public ScriptedAI {
    ...
    {
        _scheduler
            .Schedule(5s, [this](TaskContext /*context*/) { Talk(SAY_AGGRO); })
    ...
    void KilledUnit(Unit*) override
    {
        ...
        _scheduler.Schedule(
            6s, [this](TaskContext /*context*/) { _hasSpoken = false; });
    }
    ...
    void JustDied(Unit*) override { Talk(SAY_DIE); }
...
class spell_vazruden_fireball : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_vazruden_fireball::HandleScriptEffect,
                          EFFECT_1,
...
class spell_vazruden_call_nazan : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_vazruden_call_nazan::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
