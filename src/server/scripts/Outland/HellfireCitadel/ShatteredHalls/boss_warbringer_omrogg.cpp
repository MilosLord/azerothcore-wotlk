...
enum Equip { EQUIP_STANDARD = 1, EQUIP_BURNING_MAUL = 2 };
...
enum Misc { EMOTE_BURNING_MAUL = 0, DATA_BURNING_MAUL_END = 1 };
...
struct boss_warbringer_omrogg : public BossAI {
    ...
    void HandleHeadTalk(HeadYells yell)
    {
        ...
        case SAY_ON_AGGRO: {
            ...
            if (Creature* leftHead =
                    instance->GetCreature(DATA_OMROGG_LEFT_HEAD)) {
                leftHead->AI()->Talk(group);
                _headTalk.Schedule(
                    3600ms, [this, group](TaskContext /*context*/) {
                        if (Creature* rightHead =
                                instance->GetCreature(DATA_OMROGG_RIGHT_HEAD))
            ...
            if (Creature* leftHead =
                ...
                _headTalk.Schedule(3600ms, [this](TaskContext context) {
                    ...
                    context.Schedule(3600ms, [this](TaskContext context) {
                        uint8 group =
                            urand(SAY_ON_BEATDOWN_2, SAY_ON_BEATDOWN_3);
                        if (Creature* leftHead =
                                instance->GetCreature(DATA_OMROGG_LEFT_HEAD))
                            leftHead->AI()->Talk(group);
                        context.Schedule(
                            3600ms, [this, group](TaskContext /*context*/) {
                                if (Creature* rightHead = instance->GetCreature(
    ...
    void ScheduleNonBurningPhase()
    {
        scheduler
            ...
            .Schedule(
                20s, 30s, GROUP_NON_BURNING_PHASE, [this](TaskContext context) {
    ...
    {
        scheduler.Schedule(
            45s, 60s, GROUP_BURNING_PHASE, [this](TaskContext context) {
                me->AttackStop();
                me->SetReactState(REACT_PASSIVE);
...
class spell_burning_maul : public AuraScript {
    ...
    void HandleOnRemove(AuraEffect const* /* aurEff */,
    ...
    {
        OnEffectRemove += AuraEffectRemoveFn(spell_burning_maul::HandleOnRemove,
                                             EFFECT_1,
                                             SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                             AURA_EFFECT_HANDLE_REAL);
        OnEffectApply += AuraEffectApplyFn(spell_burning_maul::HandleOnApply,
                                           EFFECT_1,
                                           SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                           AURA_EFFECT_HANDLE_REAL);
    }
