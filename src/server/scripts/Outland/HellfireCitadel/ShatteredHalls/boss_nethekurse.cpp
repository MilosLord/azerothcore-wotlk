...
enum Data { SETDATA_DATA = 1, SETDATA_PEON_AGGRO = 1, SETDATA_PEON_DEATH = 2 };
...
struct boss_grand_warlock_nethekurse : public BossAI {
    boss_grand_warlock_nethekurse(Creature* creature)
        : BossAI(creature, DATA_NETHEKURSE)
    {
    ...
    void Reset() override
    {
        ScheduleHealthCheckEvent(25, [&] { DoCastSelf(SPELL_DARK_SPIN); });
    ...
    void JustReachedHome() override { me->GetMotionMaster()->Initialize(); }
    ...
    void SetData(uint32 data, uint32 value) override
    {
        ...
        if (value == SETDATA_PEON_AGGRO && PeonEngagedCount <= 4) {
            ...
            scheduler.Schedule(
                500ms, GROUP_RP, [this](TaskContext /*context*/) {
                    me->HandleEmoteCommand(EMOTE_ONESHOT_APPLAUD);
                    Talk(SAY_PEON_DIES);

                    scheduler.Schedule(
                        1s, GROUP_RP, [this](TaskContext /*context*/) {
    ...
    void IntroRP()
    {
        scheduler.Schedule(500ms, GROUP_RP, [this](TaskContext context) {
            ...
            scheduler.Schedule(
                2500ms, GROUP_RP, [this](TaskContext /*context*/) {
                    PeonRoleplay roleplayData =
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler
            ...
            .Schedule(8100ms,
                      ...
                      [this](TaskContext context) {
                          DoCastRandomTarget(
                              SPELL_SHADOW_FISSURE, 0, 60.0f, true);
    ...
    void DoAction(int32 action) override
    {
        if (action == ACTION_CANCEL_INTRO) {
            ...
            _canAggro = true;
            // Bit of a hack to make sure it can't be started with the
            // areatrigger AND the door opening
            _introStarted = true;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        // this should never be called if the action to start intro has been
        // called
        if (!_introStarted) {
            // find the door that is nearest to the entrance
            if (GameObject* nethekursedoor = GetClosestGameObjectWithEntry(
                    me, GO_GRAND_WARLOCK_CHAMBER_DOOR_1, 100.0f)) {
                // check if door is openened
                // this should only happen before the intro, if the door is
                // picked by someone
...
class spell_tsh_shadow_bolt : public SpellScript {
    ...
    void SelectRandomPlayer(WorldObject*& target)
    {
        if (Creature* caster = GetCaster()->ToCreature()) {
            if (Unit* randomTarget = caster->AI()->SelectTarget(
    ...
    void Register() override
    {
        OnObjectTargetSelect +=
            SpellObjectTargetSelectFn(spell_tsh_shadow_bolt::SelectRandomPlayer,
                                      EFFECT_0,
...
class spell_target_fissures : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_target_fissures::HandleEffect,
                                           EFFECT_0,
    ...
    bool _OnTrigger(Player* player, AreaTrigger const* /*at*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            if (instance->GetBossState(DATA_NETHEKURSE) != DONE &&
                instance->GetBossState(DATA_NETHEKURSE) != IN_PROGRESS) {
                if (Creature* nethekurse =
                        instance->GetCreature(DATA_NETHEKURSE)) {
