...
public:
    ...
    struct instance_shattered_halls_InstanceMapScript : public InstanceScript {
        ...
        {
            if (type == DATA_ENTERED_ROOM && data == DATA_ENTERED_ROOM &&
                RescueTimer == 100 * MINUTE * IN_MILLISECONDS) {
            ...
            if (RescueTimer && RescueTimer < 100 * MINUTE * IN_MILLISECONDS) {
                ...
                if ((RescueTimer / IN_MILLISECONDS) == 25 * MINUTE) {
                    ...
                    if (Creature* prisoner =
                            instance->GetCreature(prisonerGUID[0]))
                        Unit::Kill(prisoner, prisoner);
                    if (Creature* executioner =
                            instance->GetCreature(executionerGUID))
                    ...
                    if (Creature* prisoner =
                            instance->GetCreature(prisonerGUID[1]))
                        Unit::Kill(prisoner, prisoner);
                }
                    ...
                    if (Creature* prisoner =
                            instance->GetCreature(prisonerGUID[2]))
                        Unit::Kill(prisoner, prisoner);
                }
            }
        }
...
public:
    at_shattered_halls_execution()
        : AreaTriggerScript("at_shattered_halls_execution")
    {
    }
