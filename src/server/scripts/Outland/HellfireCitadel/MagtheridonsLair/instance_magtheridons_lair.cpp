...
MinionData const minionData[] = {{NPC_HELLFIRE_CHANNELER, DATA_MAGTHERIDON}};
...
public:
    ...
    struct instance_magtheridons_lair_InstanceMapScript
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_CHANNELER_COMBAT:
                if (GetBossState(DATA_MAGTHERIDON) != IN_PROGRESS)
                    if (Creature* magtheridon =
                            instance->GetCreature(_magtheridonGUID))
...
void AddSC_instance_magtheridons_lair() { new instance_magtheridons_lair(); }
