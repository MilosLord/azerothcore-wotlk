...
enum Groups { GROUP_INTERRUPT_CHECK = 0, GROUP_EARLY_RELEASE_CHECK = 1 };

enum Actions { ACTION_INCREASE_HELLFIRE_CHANNELER_DEATH_COUNT = 1 };
...
struct boss_magtheridon : public BossAI {
    ...
    void KilledUnit(Unit* /*victim*/) override
    {
        ...
        scheduler.Schedule(
            5s, [this](TaskContext /*context*/) { _recentlySpoken = false; });
    }
    ...
    void ScheduleCombatEvents()
    {
        ...
        scheduler
            ...
            .Schedule(20s,
                      [this](TaskContext context) {
                          me->CastCustomSpell(
                              SPELL_BLAZE, SPELLVALUE_MAX_TARGETS, 1);
    ...
    void DoAction(int32 action) override
    {
        if (action == ACTION_INCREASE_HELLFIRE_CHANNELER_DEATH_COUNT) {
            ...
            if (_channelersKilled >= 5 && !_magReleased) {
                ...
                Talk(SAY_FREE);
                scheduler.CancelGroup(
                    GROUP_EARLY_RELEASE_CHECK); // cancel regular countdown
                _magReleased = true;
                scheduler.Schedule(
                    3s, [this](TaskContext) { ScheduleCombatEvents(); });
            }
        }
    }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler
            .Schedule(
                ...
                [this](TaskContext /*context*/) { Talk(SAY_EMOTE_NEARLY); })
            .Schedule(120s,
                      GROUP_EARLY_RELEASE_CHECK,
                      [this](TaskContext /*context*/) {
                          Talk(SAY_EMOTE_FREE);
            ...
            .Schedule(
                123s,
                GROUP_EARLY_RELEASE_CHECK,
                [this](TaskContext /*context*/) { ScheduleCombatEvents(); });
    }
...
class spell_magtheridon_blaze : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_magtheridon_blaze::HandleScriptEffect,
                          EFFECT_0,
...
class spell_magtheridon_shadow_grasp : public AuraScript {
    ...
    void HandleDummyApply(AuraEffect const* /*aurEff*/,
                          AuraEffectHandleModes /*mode*/)
    {
        GetUnitOwner()->CastSpell(
            (Unit*)nullptr, SPELL_SHADOW_GRASP_VISUAL, false);
    }

    void HandleDummyRemove(AuraEffect const* /*aurEff*/,
                           AuraEffectHandleModes /*mode*/)
    {
        GetUnitOwner()->InterruptNonMeleeSpells(true);
    }

    void HandlePeriodicRemove(AuraEffect const* /*aurEff*/,
                              AuraEffectHandleModes /*mode*/)
    {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_magtheridon_shadow_grasp::HandleDummyApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(
            spell_magtheridon_shadow_grasp::HandleDummyRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(
            spell_magtheridon_shadow_grasp::HandlePeriodicRemove,
            EFFECT_1,
...
class spell_magtheridon_debris_target_selector : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_magtheridon_debris_target_selector::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENTRY);
        OnHit +=
            SpellHitFn(spell_magtheridon_debris_target_selector::HandleHit);
    }
};
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* /*go*/) override
    {
        if (player->HasAura(SPELL_MIND_EXHAUSTION) ||
            player->HasAura(SPELL_SHADOW_GRASP))
            return true;

        if (Creature* trigger =
                player->FindNearestCreature(NPC_HELLFIRE_RAID_TRIGGER, 10.0f))
