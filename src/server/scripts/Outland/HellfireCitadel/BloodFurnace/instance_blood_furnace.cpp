...
ObjectData const creatureData[] = {
    {NPC_BROGGOK, DATA_BROGGOK}, {NPC_KELIDAN, DATA_KELIDAN}, {0, 0}};
    ...
    struct instance_blood_furnace_InstanceMapScript : public InstanceScript {
        ...
        void OnUnitDeath(Unit* unit) override
        {
            if (unit && unit->GetTypeId() == TYPEID_UNIT &&
                unit->GetEntry() == NPC_NASCENT_FEL_ORC)
        ...
        {
            if (_prisonersCell[0].find(guid) != _prisonersCell[0].end() &&
                --_prisonerCounter[0] <= 0)
                ActivateCell(DATA_PRISON_CELL2);
            else if (_prisonersCell[1].find(guid) != _prisonersCell[1].end() &&
                     --_prisonerCounter[1] <= 0)
                ActivateCell(DATA_PRISON_CELL3);
            else if (_prisonersCell[2].find(guid) != _prisonersCell[2].end() &&
                     --_prisonerCounter[2] <= 0)
                ActivateCell(DATA_PRISON_CELL4);
            else if (_prisonersCell[3].find(guid) != _prisonersCell[3].end() &&
                     --_prisonerCounter[3] <= 0)
...
void AddSC_instance_blood_furnace() { new instance_blood_furnace(); }
