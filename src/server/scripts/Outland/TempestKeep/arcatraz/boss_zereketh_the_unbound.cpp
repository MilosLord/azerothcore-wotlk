...
enum Say { SAY_AGGRO = 0, SAY_SLAY = 1, SAY_SHADOW_NOVA = 2, SAY_DEATH = 3 };
...
class spell_zereketh_seed_of_corruption : public AuraScript {
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& /*eventInfo*/)
    {
        ...
        if (GetCaster()) {
            GetCaster()->CastCustomSpell(SPELL_CORRUPTION_PROC,
    ...
    {
        OnEffectProc +=
            AuraEffectProcFn(spell_zereketh_seed_of_corruption::HandleProc,
                             EFFECT_1,
                             SPELL_AURA_DUMMY);
    }
};
