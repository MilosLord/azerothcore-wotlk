...
struct boss_laj : public BossAI {
    boss_laj(Creature* creature)
        : BossAI(creature, DATA_LAJ), _lastTransform(LajTransform[0])
    {
    }
    ...
    {
        ScheduleTimedEvent(
            5s, [&] { DoCastVictim(SPELL_ALLERGIC_REACTION); }, 25s);
        ...
        ScheduleTimedEvent(
            ...
            [&] {
                ...
                _lastTransform =
                    Acore::Containers::SelectRandomContainerElementIf(
                        _transformContainer,
                        [&](LajTransformData data) -> bool {
...
void AddSC_boss_laj() { RegisterTheBotanicaCreatureAI(boss_laj); }
