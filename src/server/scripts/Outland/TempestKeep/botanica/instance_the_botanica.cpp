...
public:
    spell_botanica_call_of_the_falcon()
        : SpellScriptLoader("spell_botanica_call_of_the_falcon")
    {
    }
    ...
    class spell_botanica_call_of_the_falcon_AuraScript : public AuraScript {
                               ...
                               AuraEffectHandleModes /*mode*/)
        {
            ...
            GetUnitOwner()->GetCreaturesWithEntryInRange(
                creatureList, 80.0f, NPC_BLOODFALCON);
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_botanica_call_of_the_falcon_AuraScript::HandleEffectApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_botanica_call_of_the_falcon_AuraScript::
                    HandleEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
    ...
    class spell_botanica_shift_form_AuraScript : public AuraScript {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (SpellInfo const* spellInfo = eventInfo.GetSpellInfo()) {
                if ((spellInfo->GetSchoolMask() & _lastSchool) &&
                    _swapTime > GameTime::GetGameTime().count())
                    return false;
        ...
        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(
                spell_botanica_shift_form_AuraScript::CheckProc);
        }
