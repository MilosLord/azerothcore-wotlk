...
const Position triggersPos[6] = {{799.11f, -38.95f, 85.0f, 0.0f},
...
struct boss_kaelthas : public BossAI {
    ...
    {
        for (uint8 advisorData = DATA_THALADRED; advisorData <= DATA_TELONICUS;
             ++advisorData) {
        ...
        if (type == DATA_RESURRECT_CAST && data == DATA_RESURRECT_CAST) {
            summons.DoForAllSummons([&](WorldObject* summon) {
                if (Creature* summonedCreature = summon->ToCreature()) {
                    if (summonedCreature->GetEntry() >= NPC_LORD_SANGUINAR &&
                        summonedCreature->GetEntry() <= NPC_THALADRED) {
                        summonedCreature->SetReactState(REACT_PASSIVE);
                        summonedCreature->setDeathState(
    ...
    void SetRoomState(GOState state)
    {
        ...
        if (GameObject* window = ObjectAccessor::GetGameObject(
                *me, instance->GetGuidData(GO_BRIDGE_WINDOW)))
            window->SetGoState(state);
        if (GameObject* window = ObjectAccessor::GetGameObject(
                *me, instance->GetGuidData(GO_KAEL_STATUE_RIGHT)))
            window->SetGoState(state);
        if (GameObject* window = ObjectAccessor::GetGameObject(
                *me, instance->GetGuidData(GO_KAEL_STATUE_LEFT)))
            window->SetGoState(state);
    }
    ...
    {
        BossAI::Reset();
        scheduler.Schedule(1s, [this](TaskContext) { PrepareAdvisors(); });
        _phase = PHASE_NONE;
        ...
        ScheduleHealthCheckEvent(50, [&] {
            ...
            me->GetMotionMaster()->MovePoint(
                POINT_MIDDLE, me->GetHomePosition(), true, true);
    ...
    {
        if (_phase == PHASE_NONE && who->GetTypeId() == TYPEID_PLAYER &&
            ...
            ScheduleUniqueTimedEvent(
                ...
                [&] {
                    if (Creature* thaladred =
                            summons.GetCreatureWithEntry(NPC_THALADRED)) {
    ...
    void JustEngagedWith(Unit* who) override { BossAI::JustEngagedWith(who); }
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        if (summon->GetEntry() >= NPC_NETHERSTRAND_LONGBOW &&
            summon->GetEntry() <= NPC_STAFF_OF_DISINTEGRATION)
        ...
        case ACTION_START_WEAPONS:
            ...
            ScheduleUniqueTimedEvent(
                ...
                [&] {
                    ...
                    scheduler.Schedule(
                        2min, GROUP_PROGRESS_PHASE, [this](TaskContext) {
                ...
                summons.DoForAllSummons([&aliveWeapon](WorldObject* summon) {
                    if (Creature* summonedCreature = summon->ToCreature()) {
                        if (summonedCreature->IsAlive()) {
                            if (summonedCreature->GetEntry() >=
                                    NPC_NETHERSTRAND_LONGBOW &&
                                summonedCreature->GetEntry() <=
    ...
    void MovementInform(uint32 type, uint32 point) override
    {
        ...
        if (point == POINT_MIDDLE) {
            ...
            ScheduleTimedEvent(
                0ms, [&] { DoCastVictim(SPELL_FIREBALL); }, 2400ms, 7500ms);
            ScheduleTimedEvent(
                10000ms,
    ...
    void ExecuteMiddleEvent()
    {
        ...
        ScheduleUniqueTimedEvent(
            ...
            [&] {
                ...
                for (uint8 i = 0; i < 2; ++i)
                    if (Creature* trigger =
                            me->SummonCreature(WORLD_TRIGGER,
                                               triggersPos[i],
        ...
        ScheduleUniqueTimedEvent(
            ...
            [&] {
                ...
                me->SummonCreature(NPC_WORLD_TRIGGER,
                                   me->GetPositionX(),
                                   me->GetPositionY(),
        ...
        ScheduleUniqueTimedEvent(
            ...
            [&] {
                ...
                me->RemoveAurasDueToSpell(
                    SPELL_DARK_BANISH_STATE); // WRONG VISUAL
                me->GetMotionMaster()->MovePoint(
                    POINT_START_LAST_PHASE, me->GetHomePosition(), false, true);
            },
            EVENT_SCENE_16);
    }
    ...
    void IntroduceNewAdvisor(Yells talkIntroduction, KaelActions kaelAction)
    {
        ...
        scheduler.Schedule(attackStartTimer, [this, advisorNPCId](TaskContext) {
            if (Creature* advisor =
                    summons.GetCreatureWithEntry(advisorNPCId)) {
    ...
    void PhaseAllAdvisorsExecute()
    {
        ...
        ScheduleUniqueTimedEvent(
            ...
            [&] {
                ...
                summons.DoForAllSummons([&](WorldObject* summon) {
                    if (Creature* summonedCreature = summon->ToCreature()) {
                        if (summonedCreature->GetSpawnId()) {
                            ...
                            summonedCreature->RemoveUnitFlag(
                ...
                scheduler.Schedule(3min,
                                   GROUP_PROGRESS_PHASE,
                                   [this](TaskContext) { PhaseKaelExecute(); });
            },
    ...
    void PhaseKaelExecute()
    {
        ...
        ScheduleTimedEvent(
            1000ms, [&] { DoCastVictim(SPELL_FIREBALL); }, 2400ms, 7500ms);
        ScheduleTimedEvent(
            15000ms,
            [&] { DoCastRandomTarget(SPELL_FLAME_STRIKE, 0, 100.0f); },
            30250ms,
        ...
        ScheduleTimedEvent(
            ...
            [&] {
                ...
                me->CastCustomSpell(
                    SPELL_MIND_CONTROL, SPELLVALUE_MAX_TARGETS, 3, me, false);
    ...
    {
        return me->GetHomePosition().GetExactDist2d(me) > 165.0f ||
               !SelectTargetFromPlayerList(165.0f);
    }
...
struct npc_lord_sanguinar : public ScriptedAI {
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        ScheduleTimedEvent(
            0s, 2s, [&] { DoCastSelf(SPELL_BELLOWING_ROAR); }, 30s, 40s);
    }
...
struct npc_capernian : public ScriptedAI {
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        ScheduleTimedEvent(
            ...
            20500ms);
        ScheduleTimedEvent(
            3s, [&] { DoCastRandomTarget(SPELL_ARCANE_BURST, 0, 8.0f); }, 6s);
    }
...
struct npc_telonicus : public ScriptedAI {
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        ScheduleTimedEvent(
            0ms, [&] { DoCastVictim(SPELL_BOMB); }, 3600ms, 7100ms);
        ScheduleTimedEvent(
            13250ms,
        ...
        if (GetCaster()->GetTypeId() == TYPEID_UNIT)
            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                if (Creature* kael = instance->GetCreature(DATA_KAELTHAS))
                    kael->AI()->SummonedCreatureDies(GetCaster()->ToCreature(),
                                                     nullptr);
        return true;
    }
    ...
    void HandlePeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (roll_chance_i(66))
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_REMOTE_TOY_STUN, true);
    }

    void Register() override
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_kaelthas_remote_toy::HandlePeriodic,
                                 EFFECT_0,
...
class spell_kaelthas_summon_weapons : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_summon_weapons::HandleScriptEffect,
                          EFFECT_0,
...
class spell_kaelthas_resurrection : public SpellScript {
    ...
    {
        BeforeCast +=
            SpellCastFn(spell_kaelthas_resurrection::HandleBeforeCast);
    }
};
...
class spell_kaelthas_mind_control : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_kaelthas_mind_control::SelectTarget,
            EFFECT_ALL,
...
class spell_kaelthas_burn : public AuraScript {
    ...
    void HandlePeriodic(AuraEffect const* /*aurEff*/)
    {
        Unit::DealDamage(GetUnitOwner(),
                         GetUnitOwner(),
                         GetUnitOwner()->CountPctFromMaxHealth(5) + 1);
    }

    void Register() override
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_kaelthas_burn::HandlePeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_kaelthas_flame_strike : public AuraScript {
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        GetUnitOwner()->CastSpell(
            GetUnitOwner(), SPELL_FLAME_STRIKE_DAMAGE, true);
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_kaelthas_flame_strike::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
...
public:
    ...
    bool Execute(uint64 /*execTime*/, uint32 /*diff*/) override
    {
        if (_owner->IsBeingTeleportedNear())
            _owner->m_Events.AddEvent(new lapseTeleport(_owner),
                                      _owner->m_Events.CalculateTime(1));
...
class spell_kaelthas_gravity_lapse : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        if (_currentSpellId < SPELL_GRAVITY_LAPSE_TELEPORT1 + 25)
            if (Player* target = GetHitPlayer()) {
                ...
                target->m_Events.AddEvent(new lapseTeleport(target),
                                          target->m_Events.CalculateTime(1));
            }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_gravity_lapse::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_kaelthas_nether_beam : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        ThreatContainer::StorageType const& ThreatList =
            GetCaster()->GetThreatMgr().GetThreatList();
        ...
        for (ThreatContainer::StorageType::const_iterator itr =
             ...
             ++itr) {
            Unit* target =
                ObjectAccessor::GetUnit(*GetCaster(), (*itr)->getUnitGuid());
            if (target && target->GetTypeId() == TYPEID_PLAYER)
                targetList.push_back(target);
        }

        Acore::Containers::RandomResize(targetList, 5);
        for (std::list<Unit*>::const_iterator itr = targetList.begin();
             itr != targetList.end();
             ++itr)
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_nether_beam::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kaelthas_summon_nether_vapor : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        for (uint32 i = 0; i < 5; ++i)
            GetCaster()->SummonCreature(
                NPC_NETHER_VAPOR,
                GetCaster()->GetPositionX() + 6 * cos(i / 5.0f * 2 * M_PI),
                GetCaster()->GetPositionY() + 6 * std::sin(i / 5.0f * 2 * M_PI),
                GetCaster()->GetPositionZ() + 7.0f + i,
                0.0f,
                TEMPSUMMON_TIMED_DESPAWN,
                30000);
    }
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_kaelthas_summon_nether_vapor::HandleScriptEffect,
            EFFECT_0,
            SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kael_pyroblast : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (GetCaster()->GetVictim()) {
            if (Unit* victim = GetCaster()->GetVictim())
                targets.remove_if(
                    Acore::ObjectGUIDCheck(victim->GetGUID(), false));
        }
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_kael_pyroblast::FilterTargets,
                                          EFFECT_0,
                                          TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
