...
// @todo: Alar doesnt seem to move to waypoints but instead to the triggers in
// p1
...
struct boss_alar : public BossAI {
        ...
        scheduler.Schedule(0s, [this](TaskContext context) {
            if (roll_chance_i(20 * _noQuillTimes)) {
                ...
                me->GetMotionMaster()->MovePoint(
                    POINT_QUILL, alarPoints[POINT_QUILL], false, true);
                _platformMoveRepeatTimer = 16s;
            }
            else {
                if (_noQuillTimes++ > 0) {
                ...
                me->GetMotionMaster()->MovePoint(
                    POINT_PLATFORM, alarPoints[_platform], false, true);
    ...
    void JustDied(Unit* killer) override
    {
        ...
        if (Map* map = me->GetMap()) {
            map->DoForAllPlayers([&](Player* player) {
                if (player->GetQuestStatus(QUEST_RUSE_OF_THE_ASHTONGUE) ==
        ...
        if (damage >= me->GetHealth() && _platform < POINT_MIDDLE) {
            ...
            if (!_hasPretendedToDie) {
                ...
                _transitionScheduler
                    .Schedule(1s,
                              [this](TaskContext) { me->SetVisible(false); })
                    .Schedule(8s,
                              [this](TaskContext) {
    ...
    void ScheduleAbilities()
    {
        _transitionScheduler.CancelAll();
        ScheduleTimedEvent(
            57s, [&] { DoCastVictim(SPELL_MELT_ARMOR); }, 60s);
        ScheduleTimedEvent(
            10s, [&] { DoCastRandomTarget(SPELL_CHARGE, 0, 50.0f); }, 30s);
        ScheduleTimedEvent(
            20s,
            [&] {
                ...
                if (Unit* target = SelectTarget(
                    ...
                    me->SummonCreature(NPC_FLAME_PATCH,
                                       *target,
        ...
        ScheduleTimedEvent(
            ...
            [&] {
                me->GetMotionMaster()->MovePoint(
                    POINT_DIVE, alarPoints[POINT_DIVE], false, true);
                scheduler.DelayAll(15s);
            },
            50s);
        ScheduleUniqueTimedEvent(
            10min, [&] { DoCastSelf(SPELL_BERSERK); }, EVENT_SPELL_BERSERK);
        ScheduleMainSpellAttack(0s);
    }
    ...
    void SpawnPhoenixes(uint8 count, Unit* targetToSpawnAt)
    {
        if (targetToSpawnAt) {
            Position spawnPosition =
                DeterminePhoenixPosition(targetToSpawnAt->GetPosition());
    ...
    void DoDiveBomb()
    {
        ...
        scheduler
            .Schedule(
                ...
                [this](TaskContext) {
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0, 110.0f, true)) {
        ...
        if (Unit* target =
                SelectTarget(SelectTargetMethod::Random, 0, 90.0f, true)) {
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        case POINT_QUILL:
            ...
            scheduler.Schedule(
                1s, [this](TaskContext) { DoCastSelf(SPELL_FLAME_QUILLS); });
        ...
        case POINT_DIVE:
            scheduler
                .Schedule(
                    1s,
                    [this](TaskContext) { DoCastSelf(SPELL_DIVE_BOMB_VISUAL); })
                .Schedule(5s, [this](TaskContext) { DoDiveBomb(); });
            break;
        default:
            return;
        }
    ...
    {
        scheduler.Schedule(
            timer, GROUP_FLAME_BUFFET, [this](TaskContext context) {
    ...
    void ConstructWaypointsAndMove()
    {
        ...
        if (WaypointPath const* i_path =
            ...
            pathPoints.push_back(G3D::Vector3(
    ...
    Position DeterminePhoenixPosition(Position playerPosition)
    {
        ...
        if (float playerFraction = deltaX / deltaY) {
            ...
            float phoenixDeltaYPosition =
                std::cos(playerAngle) * INNER_CIRCLE_RADIUS;
            float phoenixDeltaXPosition =
                std::sin(playerAngle) * INNER_CIRCLE_RADIUS;
            // as calculations are absolute values we have to multiply in the
            // end should be negative if player position was further down than
            // centre
            ...
            finalPosition = {
                centreXPosition + signMultiplier[0] * phoenixDeltaXPosition,
                centreYPosition + signMultiplier[1] * phoenixDeltaYPosition,
                0.0f,
                0.0f};
        }
    ...
    void HandlePeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        for (uint8 i = 0; i < 21; ++i)
            GetUnitOwner()->m_Events.AddEvent(
                new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_1 + i),
                GetUnitOwner()->m_Events.CalculateTime(i * 40));
        GetUnitOwner()->m_Events.AddEvent(
            new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_2 + 0),
            GetUnitOwner()->m_Events.CalculateTime(22 * 40));
        GetUnitOwner()->m_Events.AddEvent(
            new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_2 + 1),
            GetUnitOwner()->m_Events.CalculateTime(23 * 40));
        GetUnitOwner()->m_Events.AddEvent(
            new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_2 + 2),
            GetUnitOwner()->m_Events.CalculateTime(24 * 40));
    }

    void Register() override
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_alar_flame_quills::HandlePeriodic,
                                 EFFECT_0,
        ...
        if (InstanceScript* instance = GetCaster()->GetInstanceScript()) {
            if (Creature* alar = instance->GetCreature(DATA_ALAR)) {
                if (!alar->HasAura(SPELL_MODEL_VISIBILITY))
                    Unit::DealDamage(
                        GetCaster(), alar, alar->CountPctFromMaxHealth(2));
            }
        }
    }
...
class spell_alar_dive_bomb : public AuraScript {
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_alar_dive_bomb::OnApply,
                                           EFFECT_0,
                                           SPELL_AURA_DUMMY,
