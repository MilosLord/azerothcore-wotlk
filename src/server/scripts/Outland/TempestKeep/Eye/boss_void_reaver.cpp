...
enum Groups { GROUP_ARCANE_ORB = 1 };
...
struct boss_void_reaver : public BossAI {
    boss_void_reaver(Creature* creature) : BossAI(creature, DATA_REAVER)
    {
        ...
        me->ApplySpellImmune(
            0, IMMUNITY_EFFECT, SPELL_EFFECT_HEALTH_LEECH, true);
        me->ApplySpellImmune(
            0, IMMUNITY_EFFECT, SPELL_EFFECT_POWER_DRAIN, true);
        me->ApplySpellImmune(
            0, IMMUNITY_STATE, SPELL_AURA_PERIODIC_LEECH, true);
        me->ApplySpellImmune(
            0, IMMUNITY_STATE, SPELL_AURA_PERIODIC_MANA_LEECH, true);
    }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler
            .Schedule(10min, [this](TaskContext) { DoCastSelf(SPELL_BERSERK); })
            .Schedule(8300ms,
            ...
            .Schedule(3450ms,
                      GROUP_ARCANE_ORB,
                      [this](TaskContext context) {
                          if (DoCastRandomTarget(SPELL_ARCANE_ORB, 0, -20.0f) !=
                              SPELL_CAST_OK) {
...
void AddSC_boss_void_reaver() { RegisterTheEyeAI(boss_void_reaver); }
