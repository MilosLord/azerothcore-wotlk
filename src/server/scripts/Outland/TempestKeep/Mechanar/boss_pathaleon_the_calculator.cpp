...
struct boss_pathaleon_the_calculator : public BossAI {
    boss_pathaleon_the_calculator(Creature* creature)
        : BossAI(creature, DATA_PATHALEON_THE_CALCULATOR)
    {
    }
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        scheduler
                ...
                [this](TaskContext context) {
                    if (!_isEnraged) {
                        for (uint8 i = 0; i < DUNGEON_MODE(3, 4); ++i)
                            me->CastSpell(
                                me, SPELL_SUMMON_NETHER_WRAITH_1 + i, true);
            ...
            .Schedule(10s,
                      15s,
                      [this](TaskContext context) {
                          if (DoCastRandomTarget(SPELL_DOMINATION, 1, 50.0f) ==
                              SPELL_CAST_OK) {
                              Talk(SAY_DOMINATION);
                          }
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_BRIDGE_MOB_DEATH) {
            uint8 mobCount =
                instance->GetPersistentData(DATA_BRIDGE_MOB_DEATH_COUNT);
            instance->StorePersistentData(DATA_BRIDGE_MOB_DEATH_COUNT,
                                          ++mobCount);
            ...
            if (mobCount >= 4) {
                ...
                scheduler
                    ...
                    .Schedule(25s, [this](TaskContext) { DoZoneInCombat(); });
            }
        }
    }
