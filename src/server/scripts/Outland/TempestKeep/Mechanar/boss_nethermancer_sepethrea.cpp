...
enum Spells {
    SPELL_FROST_ATTACK =
        45196, // This is definitely spell added in TBC but did it replaced both
               // 35264 and 39086 or only normal version?
...
struct boss_nethermancer_sepethrea : public BossAI {
    boss_nethermancer_sepethrea(Creature* creature)
        : BossAI(creature, DATA_NETHERMANCER_SEPRETHREA)
    {
    }
    ...
    {
        // Fails probably because target is in evade mode (yes, she kills them
        // on evade too). We'll kill them directly in their script for now
...
struct npc_raging_flames : public ScriptedAI {
    ...
    void InitializeAI() override { me->SetCorpseDelay(20); }
        ...
        if (TempSummon* summon = me->ToTempSummon())
            if (Creature* summoner = summon->GetSummonerCreatureBase())
                if (summoner->IsAIEnabled) {
                    if (Unit* target = summoner->AI()->SelectTarget(
                            SelectTargetMethod::Random, 0, 100.0f, true, false))
    ...
    void EnterEvadeMode(EvadeReason /*why*/) override { FixateRandomTarget(); }
...
class spell_ragin_flames_inferno : public AuraScript {
    ...
    {
        GetUnitOwner()->CastCustomSpell(SPELL_INFERNO_DAMAGE,
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_ragin_flames_inferno::HandlePeriodic,
                                 EFFECT_0,
