            ...
            .Schedule(5600ms,
                      [this](TaskContext context) {
                          if (Unit* target = SelectTarget(
                                  SelectTargetMethod::MaxThreat, 1, 5.0f)) {
...
struct npc_invisible_tractor_beam_source : public NullCreatureAI {
    npc_invisible_tractor_beam_source(Creature* creature)
...
class spell_gruul_ground_slam : public SpellScript {
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget +=
            SpellEffectFn(spell_gruul_ground_slam::HandleScriptEffect,
                          EFFECT_1,
...
class spell_tractor_beam_creator : public SpellScript {
    ...
    {
        if (Unit* caster = GetCaster()) {
            std::vector<uint32> tractorBeamSummons = {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_tractor_beam_creator::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gruul_ground_slam_trigger : public AuraScript {
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gruul_ground_slam_trigger::OnApply,
                               EFFECT_0,
...
class spell_gruul_shatter : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gruul_shatter::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gruul_shatter_effect : public SpellScript {
    ...
    void CalculateDamage()
    {
        ...
        if (distance > 1.0f) {
            SetHitDamage(
                int32(GetHitDamage() * ((radius - distance) / radius)));
        }
    }
