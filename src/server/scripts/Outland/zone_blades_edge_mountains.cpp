...
SD%Complete: 90
SDComment: Quest support: 10503, 10504, 10556, 10594, 10609, 10821.
Ogri'la->Skettis Flight. (npc_daranelle needs bit more work before consider
complete) SDCategory: Blade's Edge Mountains EndScriptData */
...
public:
    npc_deaths_door_fell_cannon_target_bunny()
        : CreatureScript("npc_deaths_door_fell_cannon_target_bunny")
    {
    }

    struct npc_deaths_door_fell_cannon_target_bunnyAI : public ScriptedAI {
        npc_deaths_door_fell_cannon_target_bunnyAI(Creature* creature)
            : ScriptedAI(creature), PartyTime(false)
        {
        }
                ...
                if (count > 6) {
                    if (Player* player =
                        ...
                        if (GetClosestCreatureWithEntry(
                                me, NPC_SOUTH_GATE, 200.0f))
                            player->KilledMonsterCredit(NPC_SOUTH_GATE_CREDIT);
                        else if (GetClosestCreatureWithEntry(
                                     me, NPC_NORTH_GATE, 200.0f))
                            player->KilledMonsterCredit(NPC_NORTH_GATE_CREDIT);
                        // complete quest part
                        if (Creature* bunny = GetClosestCreatureWithEntry(
                                me, NPC_EXPLOSION_BUNNY, 200.0f))
                            bunny->CastSpell(
                                nullptr, SPELL_EXPLOSION, TRIGGERED_NONE);
                        if (Creature* cannon =
                                ObjectAccessor::GetCreature(*me, CannonGUID))
        ...
        void JustSummoned(Creature* summoned) override
        {
            ...
            if (Creature* Target = GetClosestCreatureWithEntry(
                    me, NPC_DEATHS_DOOR_FEL_CANNON, 200.0f)) {
                Target->RemoveUnitFlag(
                    UNIT_FLAG_NOT_ATTACKABLE_1); // attack the cannon
...
public:
    ...
    struct npc_deaths_fel_cannonAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            ...
            if (Creature* Target = GetClosestCreatureWithEntry(
                    me, NPC_DEATHS_DOOR_FEL_CANNON_TARGET_BUNNY, 200.0f)) {
...
public:
    spell_npc22275_crystal_prison()
        : SpellScriptLoader("spell_npc22275_crystal_prison")
    {
    }
    ...
    class spell_npc22275_crystal_prison_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_npc22275_crystal_prison_AuraScript::OnPeriodic,
                EFFECT_0,
...
public:
    ...
    struct npc_nether_drakeAI : public ScriptedAI {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_T_PHASE_MODULATOR &&
                caster->GetTypeId() == TYPEID_PLAYER) {
                const uint32 entry_list[4] = {
                    ENTRY_PROTO, ENTRY_ADOLE, ENTRY_MATUR, ENTRY_NIHIL};
                int cid = rand() % (4 - 1);
    ...
    struct npc_daranelleAI : public ScriptedAI {
...

        {
            if (who->GetTypeId() == TYPEID_PLAYER) {
                if (who->HasAura(SPELL_LASHHAN_CHANNEL) &&
                    me->IsWithinDistInMap(who, 10.0f)) {
                    if (Creature* bird =
                            who->FindNearestCreature(NPC_KALIRI_TOTEM, 10.0f)) {
                        Talk(SAY_SPELL_INFLUENCE, who);
                        /// @todo Move the below to updateAI and run if this
                        /// statement == true
...
public:
    ...
    struct npc_simon_bunnyAI : public ScriptedAI {
        ...
        // Called by color clusters script (go_simon_cluster) and used for
        // knowing the button pressed by player
        ...
        // Used for getting involved player guid. Parameter id is used for
        // defining if is a large(Monument) or small(Relic) node
        ...
        /*
        Resets all variables and also find the ids of the four closests color
        clusters, since every simon node have diferent ids for clusters this is
        ...
        */
        void StartGame()
        {
            ...
            Acore::WorldObjectListSearcher<Acore::AllWorldObjectsInRange>
                searcher(me, ClusterList, objects);
            Cell::VisitAllObjects(me, searcher, searchDistance);

            for (std::list<WorldObject*>::const_iterator i =
                     ClusterList.begin();
                 i != ClusterList.end();
                 ++i) {
                if (GameObject* go = (*i)->ToGameObject()) {
                    // We are checking for displayid because all simon nodes
                    // have 4 clusters with different entries
                    if (large) {
            ...
            if (GameObject* relic = me->FindNearestGameObject(
                    large ? GO_APEXIS_MONUMENT : GO_APEXIS_RELIC,
                    searchDistance))
                relic->SetGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
        }

        // Called when despawning the bunny. Sets all the node GOs to their
        // default states.
        void ResetNode()
        {
            DoPlaySoundToSet(me, SOUND_DISABLE_NODE);

            for (uint32 clusterId = SIMON_BLUE; clusterId < SIMON_MAX_COLORS;
                 clusterId++)
                if (GameObject* cluster = me->FindNearestGameObject(
                        clusterIds[clusterId], searchDistance))
                    cluster->SetGameObjectFlag(GO_FLAG_NOT_SELECTABLE);

            for (uint32 auraId = GO_AURA_BLUE; auraId <= GO_AURA_YELLOW;
                 auraId++)
                if (GameObject* auraGo =
                        me->FindNearestGameObject(auraId, searchDistance))
        ...
        /*
        Called on every button click of player. Adds the clicked color to the
        player created sequence and checks if it corresponds to the AI created
        sequence. If so, incremente gameLevel and start a new round, if not,
        give punishment and restart current level.
        */
        void CheckPlayerSequence()
        {
            bool correct = true;
            if (playerSequence.size() <= colorSequence.size())
                for (std::list<uint8>::const_iterator
                         i = playerSequence.begin(),
                         j = colorSequence.begin();
        ...
        /*
        Generates a random sequence of colors depending on the gameLevel. We
        also copy this sequence to the playableSequence wich will be used when
        playing the sequence to the player.
        */
        void GenerateColorSequence()
        {
            ...
            for (uint8 i = 0; i <= gameLevel; i++)
                colorSequence.push_back(
                    RAND(SIMON_BLUE, SIMON_RED, SIMON_GREEN, SIMON_YELLOW));

            for (std::list<uint8>::const_iterator i = colorSequence.begin();
                 i != colorSequence.end();
                 ++i)
                playableSequence.push_back(*i);
        }

        // Remove any existant glowing auras over clusters and set clusters
        // ready for interating with them.
        ...
        /*
        Called when AI is playing the sequence for player. We cast the visual
        spell and then remove the cast color from the casting sequence.
        */
        void PlayNextColor()
        {
        ...
        After calling this function bunny is teleported to the center of the
        node.
        */
        void SetUpPreGame()
        {
            for (uint32 clusterId = SIMON_BLUE; clusterId < SIMON_MAX_COLORS;
                 clusterId++) {
                if (GameObject* cluster = me->FindNearestGameObject(
                        clusterIds[clusterId], 2.0f * searchDistance)) {
                    cluster->SetGameObjectFlag(GO_FLAG_NOT_SELECTABLE);

                    // break since we don't need glowing auras for large
                    // clusters
                    if (large)
                        break;
        ...
        // Handles the spell rewards. The spells also have the
        // QuestCompleteEffect, so quests credits are working.
        ...
        /*
        Depending on the number of failed pushes for player the damage of the
        spell scales, so we first cast the spell on the target that hits for 50
        and shows the visual and then forces the player to cast the damaging
        spell on it self with the modified basepoints. 4 fails = death. On large
        nodes punishment and reward are the same, summoning the Apexis Guardian.
        */
        void GivePunishment()
        {
            if (large) {
                if (Player* player = ObjectAccessor::GetPlayer(*me, playerGUID))
                    if (Creature* guardian = me->SummonCreature(
                            NPC_APEXIS_GUARDIAN,
                            me->GetPositionX(),
                            me->GetPositionY(),
        ...
        {
            // Cast SPELL_BAD_PRESS_DAMAGE with scaled basepoints when the
            // visual hits the target. Need Fix: When SPELL_BAD_PRESS_TRIGGER
            // hits target it triggers spell SPELL_BAD_PRESS_DAMAGE by itself so
            // player gets damage equal to calculated damage  dbc basepoints for
            // SPELL_BAD_PRESS_DAMAGE (~50)
            if (spell->Id == SPELL_BAD_PRESS_TRIGGER) {
                int32 bp =
                    (int32)((float)(fails) * 0.33f * target->GetMaxHealth());
        ...
        // Checks if player has already die or has get too far from the current
        // node
        bool CheckPlayer()
        {
            if (Player* player = ObjectAccessor::GetPlayer(*me, playerGUID)) {
...
public:
    ...
    {
        if (Creature* bunny =
                go->FindNearestCreature(NPC_SIMON_BUNNY, 12.0f, true))
                        ...
                        uint32 /*action*/) override
    {
        ...
        if (player->HasItemCount(ITEM_APEXIS_SHARD, large ? 35 : 1)) {
            player->CastSpell(player,
                              large ? SPELL_TAKE_REAGENTS_GROUP
                                    : SPELL_TAKE_REAGENTS_SOLO,
                              false);

            if (Creature* bunny = player->SummonCreature(NPC_SIMON_BUNNY,
                                                         go->GetPositionX(),
                                                         go->GetPositionY(),
                                                         go->GetPositionZ()))
...
/*######
## npc_oscillating_frequency_scanner_master_bunny used for quest 10594 "Gauging
...
public:
    npc_oscillating_frequency_scanner_master_bunny()
        : CreatureScript("npc_oscillating_frequency_scanner_master_bunny")
    {
    }

    struct npc_oscillating_frequency_scanner_master_bunnyAI
        : public ScriptedAI {
        ...
        {
            if (GetClosestCreatureWithEntry(
                    me, NPC_OSCILLATING_FREQUENCY_SCANNER_TOP_BUNNY, 25.0f))
...
public:
    ...
    class spell_oscillating_field_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_oscillating_field_SpellScript::HandleEffect,
                              EFFECT_0,
