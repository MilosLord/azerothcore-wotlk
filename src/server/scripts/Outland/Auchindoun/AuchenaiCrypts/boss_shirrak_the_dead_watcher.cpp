        ...
        scheduler
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin();
                        ...
                        if (Player* player = i->GetSource()) {
                            ...
                            if (player->IsAlive() && dist < 45.0f) {
                                Aura* aura =
                                    player->GetAura(SPELL_INHIBIT_MAGIC);
                                if (!aura) {
                                    aura = me->AddAura(SPELL_INHIBIT_MAGIC,
                                                       player);
                                }
                                else {
            ...
            .Schedule(28s,
                      [this](TaskContext context) {
                          ...
                          context.Repeat(30s);
                          scheduler.Schedule(
                              1500ms, [this](TaskContext context) {
            ...
            .Schedule(17s, [this](TaskContext context) {
                if (Unit* target = SelectTarget(
                    ...
                    if (Creature* cr =
                            me->SummonCreature(ENTRY_FOCUS_FIRE,
                                               target->GetPositionX(),
                                               target->GetPositionY(),
                                               target->GetPositionZ(),
                                               0,
                ...
                scheduler
                    .Schedule(3s,
                              [this](TaskContext /*context*/) {
                                  if (Unit* flare = ObjectAccessor::GetCreature(
                                          *me, focusGUID)) {
                                      me->CastSpell(
                                          flare, SPELL_FOCUS_CAST, true);
                                  }
                              })
                    .Schedule(3500ms,
                              [this](TaskContext /*context*/) {
...
class spell_auchenai_possess : public AuraScript {
    ...
    void Register() override
    {
        if (m_scriptSpellId == SPELL_POSSESS_CHANNELED) {
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_auchenai_possess::OnRemove,
                                   EFFECT_0,
            ...
            DoEffectCalcPeriodic +=
                AuraEffectCalcPeriodicFn(spell_auchenai_possess::CalcPeriodic,
                                         EFFECT_0,
                                         SPELL_AURA_MOD_CHARM);
            OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
                spell_auchenai_possess::Update, EFFECT_0, SPELL_AURA_MOD_CHARM);
        }
    }
};
