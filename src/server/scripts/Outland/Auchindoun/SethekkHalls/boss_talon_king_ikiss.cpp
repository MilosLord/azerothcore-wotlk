...
struct boss_talon_king_ikiss : public BossAI {
    boss_talon_king_ikiss(Creature* creature)
        : BossAI(creature, DATA_IKISS), _spoken(false)
    {
    ...
    bool CanAIAttack(Unit const* /*victim*/) const override { return _spoken; }
        ...
        scheduler
            ...
            .Schedule(8s, [this](TaskContext context) {
                SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(SPELL_POLYMORPH);
    ...
    void JustDied(Unit* /*killer*/) override
    {
        _JustDied();
        Talk(SAY_DEATH);
        if (GameObject* coffer =
                instance->GetGameObject(DATA_GO_TALON_KING_COFFER)) {
            coffer->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE |
...
class spell_talon_king_ikiss_blink : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_talon_king_ikiss_blink::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_talon_king_ikiss_blink::HandleDummyHitTarget,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
