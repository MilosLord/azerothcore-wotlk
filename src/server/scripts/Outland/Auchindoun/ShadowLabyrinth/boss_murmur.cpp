...
struct boss_murmur : public BossAI {
        ...
        me->m_Events.AddEventAtOffset(
            [this] {
                if (me->FindNearestCreature(NPC_CABAL_SPELLBINDER, 35.0f)) {
                    me->CastCustomSpell(SPELL_SUPPRESSION,
    ...
    {
        // Boss engages mobs during roleplay, this checks prevents it from
        // setting the zone in combat before players engage it.
        ...
        scheduler
            ...
            .Schedule(15s,
                      30s,
                      [this](TaskContext context) {
                          if (DoCastRandomTarget(SPELL_MAGNETIC_PULL,
                                                 0,
                                                 80.0f) == SPELL_CAST_OK) {
...
class spell_murmur_thundering_storm : public SpellScript {
    ...
    {
        targets.remove_if(
            Acore::AllWorldObjectsInExactRange(GetCaster(), 100.0f, true));
        targets.remove_if(
            Acore::AllWorldObjectsInExactRange(GetCaster(), 25.0f, false));
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_murmur_thundering_storm::SelectTarget,
            EFFECT_0,
