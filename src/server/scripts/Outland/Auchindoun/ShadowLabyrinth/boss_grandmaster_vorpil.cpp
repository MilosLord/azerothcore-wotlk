...
enum Npc { NPC_VOID_TRAVELER = 19226, NPC_VOID_PORTAL = 19224 };
...
struct boss_grandmaster_vorpil : public BossAI {
    boss_grandmaster_vorpil(Creature* creature)
        : BossAI(creature, DATA_GRANDMASTER_VORPIL), sayIntro(false)
    {
    }
    ...
    {
        for (uint8 i = 0; i < 5; ++i) {
            me->SummonCreature(NPC_VOID_PORTAL,
                               VoidPortalCoords[i][0],
                               VoidPortalCoords[i][1],
                               VoidPortalCoords[i][2],
                               VoidPortalCoords[i][3],
    ...
    {
        uint8 pos = urand(0, 4);
        me->SummonCreature(NPC_VOID_TRAVELER,
                           VoidPortalCoords[pos][0],
                           VoidPortalCoords[pos][1],
                           VoidPortalCoords[pos][2],
                           VoidPortalCoords[pos][3],
        ...
        scheduler
            ...
            .Schedule(36400ms,
                      [this](TaskContext context) {
                          ...
                          me->NearTeleportTo(VorpilPosition[0],
                                             VorpilPosition[1],
                                             VorpilPosition[2],
                                             0.0f);
            ...
            if (TempSummon* summon = me->ToTempSummon()) {
                if (Unit* vorpil = summon->GetSummonerUnit()) {
                    if (me->IsWithinMeleeRange(vorpil)) {
                        ...
                        _scheduler.Schedule(
                            1200ms, [this](TaskContext /*context*/) {
                                if (TempSummon* summon = me->ToTempSummon()) {
                                    if (Unit* vorpil =
                                            summon->GetSummonerUnit()) {
                                        ...
                                        me->CastSpell(vorpil,
                                                      ...
                                                      nullptr,
                                                      vorpil->GetGUID());
                                        vorpil->ModifyHealth(int32(
                                            vorpil->CountPctFromMaxHealth(4)));
                                    }
                                }

                                _scheduler.Schedule(
                                    100ms, [this](TaskContext /*context*/) {
                                        me->KillSelf();
                                    });
                            });
                    }
