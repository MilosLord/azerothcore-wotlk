...
const Position TotemPos[3]     = {{-115.141f, -143.317f, -2.09467f, 4.92772f},
                                  {-120.178f, -144.398f, -2.23786f, 4.92379f},
                                  {-125.277f, -145.463f, -1.95209f, 4.97877f}};
...
enum CreatureIds { NPC_AHUNITE_HAILSTONE = 25755 };
...
struct boss_ahune : public ScriptedAI {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_INVOKER_SAY_3:
            if (Player* plr = ObjectAccessor::GetPlayer(*me, InvokerGUID))
                plr->Say("Your frozen reign will not come to pass!",
                         LANG_UNIVERSAL);
            break;
        case EVENT_ATTACK:
            events.Reset();
        ...
        case EVENT_TOTEMS_ATTACK:
            for (uint8 i = 0; i < 3; ++i)
                if (Creature* bunny = me->FindNearestCreature(
                        NPC_TOTEM_BUNNY_1 + i, 150.0f, true))
        ...
        case EVENT_SUBMERGE:
            ...
            if (Creature* c = DoSummon(NPC_FROZEN_CORE,
                                       *me,
                                       24000,
        ...
        case EVENT_COMBAT_EMERGE:
            ...
            me->CastSpell(me, SPELL_EMERGE_0, false);
            // me->CastSpell(me, SPELL_AHUNE_RESURFACES, true); // done in
        ...
        case EVENT_SPELL_COLD_SLAP:
            if (Unit* target = SelectTarget(
                ...
                if (target->GetPositionZ() < me->GetPositionZ() + 6.0f) {
                    ...
                    me->CastCustomSpell(
                        target, SPELL_COLD_SLAP, &dmg, nullptr, nullptr, false);
                    float x, y, z;
                    target->GetNearPoint(target,
                                         x,
                                         y,
                                         z,
                                         target->GetObjectSize(),
                                         30.0f,
                                         target->GetAngle(me->GetPositionX(),
                                                          me->GetPositionY()) +
                                             M_PI);
                    target->GetMotionMaster()->MoveJump(
                        x, y, z + 20.0f, 10.0f, 20.0f);
                }
        ...
        case EVENT_SPELL_SUMMON_HAILSTONE: {
            ...
            me->CastSpell(MinionSummonPos.GetPositionX() + cos(angle) * dist,
                          MinionSummonPos.GetPositionY() +
                              std::sin(angle) * dist,
                          MinionSummonPos.GetPositionZ(),
        ...
        case EVENT_SPELL_SUMMON_COLDWAVE:
            for (uint8 i = 0; i < 2; ++i) {
                ...
                float angle = rand_norm() * 2 * M_PI;
                me->CastSpell(
                    MinionSummonPos.GetPositionX() + cos(angle) * dist,
                    MinionSummonPos.GetPositionY() + std::sin(angle) * dist,
                    MinionSummonPos.GetPositionZ(),
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        if (summon->GetEntry() == NPC_AHUNITE_HAILSTONE) {
            ...
            if (SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(SPELL_CHILLING_AURA)) {
                Aura::TryRefreshStackOrCreate(
                    spellInfo, MAX_EFFECT_MASK, summon, summon);
            }
        }
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (GameObject* chest =
                me->SummonGameObject(187892,
                                     MinionSummonPos.GetPositionX(),
                                     MinionSummonPos.GetPositionY(),
                                     MinionSummonPos.GetPositionZ(),
                                     M_PI / 2,
                                     0.0f,
        ...
        me->GetMap()->DoForAllPlayers([&](Player* player) {
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, 25740, 1, me);

            if (player->GetGroup() && !finished) {
                finished = true;
                sLFGMgr->FinishDungeon(player->GetGroup()->GetGUID(),
                                       lfg::LFG_DUNGEON_FROST_LORD_AHUNE,
                                       me->FindMap());
            }
        });
    }
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        if (!player->IsGameMaster()) {
            if (Group const* group = player->GetGroup()) {
                if (sLFGMgr->GetDungeon(group->GetGUID()) !=
                    lfg::LFG_DUNGEON_FROST_LORD_AHUNE) {
                    return true;
                }
            }
        ...
        AddGossipItemFor(player,
                         GOSSIP_ICON_CHAT,
                         "Disturb the stone and summon Lord Ahune.",
                         GOSSIP_SENDER_MAIN,
                         GOSSIP_ACTION_INFO_DEF + 1337);
                        ...
                        uint32 action) override
    {
        ...
        if (Creature* c = go->SummonCreature(
                NPC_AHUNE, AhuneSummonPos, TEMPSUMMON_MANUAL_DESPAWN)) {
            player->DestroyItemCount(ITEM_MAGMA_TOTEM, 1, true, false);
            player->AreaExploredOrEventHappens(
                QUEST_SUMMON_AHUNE); // auto rewarded
            ...
            if (Creature* bunny = go->SummonCreature(NPC_AHUNE_SUMMON_LOC_BUNNY,
                                                     AhuneSummonPos,
                                                     TEMPSUMMON_TIMED_DESPAWN,
                                                     12000))
                if (Creature* crystal_trigger =
                        go->SummonCreature(WORLD_TRIGGER,
                                           go->GetPositionX(),
                                           go->GetPositionY(),
                                           5.0f,
                                           0.0f,
                                           TEMPSUMMON_TIMED_DESPAWN,
                                           12000))
                    crystal_trigger->CastSpell(
                        bunny, SPELL_STARTING_BEAM, false);
        }
