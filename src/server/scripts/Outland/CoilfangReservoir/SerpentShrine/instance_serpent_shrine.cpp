...
ObjectData const gameObjectData[] = {{GO_STRANGE_POOL, DATA_STRANGE_POOL},
                                     {0, 0}};
...
BossBoundaryData const boundaries = {
    {DATA_FATHOM_LORD_KARATHRESS,
     new RectangleBoundary(456.86f, 571.56f, -602.07f, -449.59f)},
    {DATA_MOROGRIM_TIDEWALKER,
     new RectangleBoundary(304.32f, 457.59f, -786.5f, -661.3f)},
...
public:
    instance_serpent_shrine()
        : InstanceMapScript("instance_serpent_shrine", 548)
    {
    }

    struct instance_serpentshrine_cavern_InstanceMapScript
            ...
            if (type == DATA_LADY_VASHJ)
                for (uint8 i = 0; i < 4; ++i)
                    if (GameObject* gobject =
                            instance->GetGameObject(_shieldGeneratorGUID[i]))
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_SHIELD_GENERATOR4:
                _shieldGeneratorGUID[go->GetEntry() - GO_SHIELD_GENERATOR1] =
                    go->GetGUID();
                break;
            }
            ...
            case DATA_ACTIVATE_SHIELD:
                if (Creature* vashj = GetCreature(DATA_LADY_VASHJ)) {
                    for (auto const& shieldGuid : _shieldGeneratorGUID) {
                        if (GameObject* gobject =
                                instance->GetGameObject(shieldGuid)) {
                            gobject->RemoveGameObjectFlag(
                                GO_FLAG_NOT_SELECTABLE);
                            vashj->SummonTrigger(gobject->GetPositionX(),
                                                 gobject->GetPositionY(),
                                                 gobject->GetPositionZ(),
                                                 0.0f,
                                                 0);
                        }
                    }
...
class spell_serpentshrine_cavern_serpentshrine_parasite : public AuraScript {
                            ...
                            AuraEffectHandleModes /*mode*/)
    {
        if (GetTarget()->GetInstanceScript() &&
            ...
            GetTarget()->CastSpell(
                GetTarget(), SPELL_SUMMON_SERPENTSHRINE_PARASITE, true);
    }

    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_serpentshrine_cavern_serpentshrine_parasite::
                HandleEffectRemove,
            EFFECT_0,
...
class spell_serpentshrine_cavern_serpentshrine_parasite_trigger
    : public SpellScript {
    PrepareSpellScript(
        spell_serpentshrine_cavern_serpentshrine_parasite_trigger);
...
class spell_serpentshrine_cavern_infection : public AuraScript {
                            ...
                            AuraEffectHandleModes /*mode*/)
    {
        if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE &&
            ...
            values.AddSpellMod(SPELLVALUE_BASE_POINT0,
                               aurEff->GetAmount() + 500);
            values.AddSpellMod(SPELLVALUE_BASE_POINT1,
                               aurEff->GetAmount() + 500);
            GetTarget()->CastCustomSpell(SPELL_RAMPART_INFECTION,
                                         values,
                                         GetTarget(),
                                         TRIGGERED_FULL_MASK,
                                         nullptr);
    ...
    {
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_serpentshrine_cavern_infection::HandleEffectRemove,
            EFFECT_1,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_serpentshrine_cavern_coilfang_water : public AuraScript {
            ...
            if (instance->GetBossState(DATA_THE_LURKER_BELOW) != DONE)
                if (instance->GetData(DATA_ALIVE_KEEPERS) == 0)
                    GetUnitOwner()->CastSpell(
                        GetUnitOwner(), SPELL_SCALDING_WATER, true);
    }
    ...
    void HandlePeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (instance->GetBossState(DATA_THE_LURKER_BELOW) == DONE ||
            instance->GetData(DATA_ALIVE_KEEPERS) == 0 ||
            GetUnitOwner()->GetPositionZ() > -20.5f ||
            !GetUnitOwner()->IsInWater())
            return;
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_serpentshrine_cavern_coilfang_water::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_serpentshrine_cavern_coilfang_water::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);

        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_serpentshrine_cavern_coilfang_water::CalcPeriodic,
            EFFECT_0,
            SPELL_AURA_DUMMY);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_serpentshrine_cavern_coilfang_water::HandlePeriodic,
            EFFECT_0,
            SPELL_AURA_DUMMY);
    }
};
...
struct npc_rancid_mushroom : public ScriptedAI {
    ...
    enum Spells : uint32 { SPELL_GROW = 31698, SPELL_SPORE_CLOUD = 38652 };
...
class spell_rancid_spore_cloud : public AuraScript {
    ...
    {
        PreventDefaultAction();
        GetCaster()->CastSpell((Unit*)nullptr,
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_rancid_spore_cloud::HandlePeriodic,
                                 EFFECT_0,
