...
enum Misc { GO_CAGE = 185474, ACTION_RESET_ADDS = 1 };
...
struct boss_fathomlord_karathress : public BossAI {
    boss_fathomlord_karathress(Creature* creature)
        : BossAI(creature, DATA_FATHOM_LORD_KARATHRESS)
    {
    }
    ...
    void Reset() override
    {
        ...
        ScheduleHealthCheckEvent(75, [&] {
            instance->DoForAllMinions(
                DATA_FATHOM_LORD_KARATHRESS, [&](Creature* fathomguard) {
                    if (fathomguard->IsAlive()) {
                        fathomguard->CastSpell(
                            me, SPELL_BLESSING_OF_THE_TIDES, true);
                    }
                });
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        instance->DoForAllMinions(
            ...
            [&](Creature* fathomguard) { fathomguard->SetInCombatWithZone(); });
            ...
            .Schedule(
                ...
                [this](TaskContext context) {
                    if (Unit* target = SelectTarget(
                            ...
                            0,
                            PowerUsersSelector(me, POWER_MANA, 50.0f, true))) {
...
public:
    ...
    bool operator()(Unit* unit) const
    {
        return unit->getPowerType() == POWER_MANA &&
               _source->GetDistance(unit) < 50.0f;
    }
...
struct boss_fathomguard_sharkkis : public ScriptedAI {
    boss_fathomguard_sharkkis(Creature* creature)
        : ScriptedAI(creature), _summons(creature)
    {
    ...
    void JustEngagedWith(Unit* who) override
    {
        if (Creature* karathress =
                _instance->GetCreature(DATA_FATHOM_LORD_KARATHRESS)) {
        ...
        scheduler
            ...
            .Schedule(41250ms,
                      [this](TaskContext context) {
                          ...
                          _summons.DoForAllSummons([&](WorldObject* summon) {
                              me->CastSpell(summon->ToCreature(),
                                            SPELL_BESTIAL_WRATH,
                                            true);
                          });
            ...
            .Schedule(14550ms, [this](TaskContext context) {
                DoCastSelf(urand(0, 1) ? SPELL_SUMMON_FATHOM_LURKER
...
enum TotemChoice { SPITFIRE = 1, EARTHBIND = 2, CLEANSING = 3 };
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        _scheduler
            ...
            .Schedule(15800ms, [this](TaskContext context) {
                if (_entryList.size() !=
                    0) // don't summon when all totems are up
                {
...
class spell_karathress_power_of_tidalvess : public AuraScript {
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_karathress_power_of_tidalvess::OnPeriodic,
            EFFECT_0,
...
class spell_karathress_power_of_caribdis : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_karathress_power_of_caribdis::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
