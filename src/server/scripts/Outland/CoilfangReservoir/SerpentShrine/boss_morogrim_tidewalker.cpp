...
const uint32 wateryGraveIds[4]  = {SPELL_WATERY_GRAVE_1,
                                   SPELL_WATERY_GRAVE_2,
                                   SPELL_WATERY_GRAVE_3,
                                   SPELL_WATERY_GRAVE_4};
const uint32 waterGlobuleIds[4] = {SPELL_SUMMON_WATER_GLOBULE_1,
...
struct boss_morogrim_tidewalker : public BossAI {
    boss_morogrim_tidewalker(Creature* creature)
        : BossAI(creature, DATA_MOROGRIM_TIDEWALKER)
    {
        ...
        scheduler
            ...
            .Schedule(20s,
                      [this](TaskContext context) {
                          ...
                          if (me->HealthAbovePct(25)) {
                              ...
                              me->CastCustomSpell(SPELL_WATERY_GRAVE,
            ...
            .Schedule(40s, [this](TaskContext context) {
                ...
                scheduler.Schedule(8s, [this](TaskContext) {
                    Talk(SAY_SUMMON);
                    for (uint32 murlocSpellId = SPELL_SUMMON_MURLOC1;
                         murlocSpellId < SPELL_SUMMON_MURLOC1 + 11;
                         ++murlocSpellId) {
...
class spell_morogrim_tidewalker_watery_grave : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit())
            if (_targetNumber < 4)
                GetCaster()->CastSpell(
                    target, wateryGraveIds[_targetNumber++], true);
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_morogrim_tidewalker_watery_grave::FilterTargets,
            EFFECT_ALL,
            TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_morogrim_tidewalker_watery_grave::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
...
class spell_morogrim_tidewalker_water_globule_new_target : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_morogrim_tidewalker_water_globule_new_target::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget += SpellEffectFn(
            spell_morogrim_tidewalker_water_globule_new_target::HandleDummy,
            EFFECT_0,
            SPELL_EFFECT_DUMMY);
    }
};
