...
struct boss_leotheras_the_blind : public BossAI {
    boss_leotheras_the_blind(Creature* creature)
        : BossAI(creature, DATA_LEOTHERAS_THE_BLIND)
    {
    }
        ...
        scheduler
            ...
            .Schedule(24250ms,
                      ...
                      [this](TaskContext) {
                          ...
                          me->CastCustomSpell(SPELL_INSIDIOUS_WHISPER,
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (me->GetDisplayId() == me->GetNativeDisplayId()) {
            ...
            if (DoCastVictim(SPELL_CHAOS_BLAST) != SPELL_CAST_OK) {
                // Auto-attacks if there are no valid targets to cast his spell
                // on f.e pet taunted.
                DoMeleeAttackIfReady();
            }
            else {
...
struct npc_inner_demon : public ScriptedAI {
    ...
    {
        if (Unit* affectedPlayer =
                ObjectAccessor::GetUnit(*me, me->GetSummonerGUID())) {
    ...
    void OnCalculatePeriodicTickReceived(uint32& damage,
...
class spell_leotheras_whirlwind : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        if (roll_chance_i(33))
            if (Unit* target = GetCaster()->GetAI()->SelectTarget(
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_leotheras_whirlwind::HandleScriptEffect,
                          EFFECT_2,
...
class spell_leotheras_chaos_blast : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_leotheras_chaos_blast::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_leotheras_insidious_whisper : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_leotheras_insidious_whisper::FilterTargets,
            EFFECT_0,
...
class spell_leotheras_insidious_whisper_aura : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        GetUnitOwner()->CastSpell(
            GetUnitOwner(), SPELL_SUMMON_INNER_DEMON, true);
    }
                            ...
                            AuraEffectHandleModes /*mode*/)
    {
        if (GetTargetApplication()->GetRemoveMode() != AURA_REMOVE_BY_DEFAULT) {
            if (InstanceScript* instance =
                ...
                if (Creature* leotheras =
                        instance->GetCreature(DATA_LEOTHERAS_THE_BLIND)) {
                    leotheras->CastSpell(
                        GetUnitOwner(), SPELL_CONSUMING_MADNESS, true);
                }
            }
        }
    }
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_leotheras_insidious_whisper_aura::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_leotheras_insidious_whisper_aura::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_leotheras_demon_link : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_leotheras_demon_link::OnPeriodic,
                                 EFFECT_0,
...
class spell_leotheras_clear_consuming_madness : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_leotheras_clear_consuming_madness::HandleScriptEffect,
            EFFECT_1,
            SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
void AddSC_boss_leotheras_the_blind()
{
    ...
    RegisterSpellAndAuraScriptPair(spell_leotheras_insidious_whisper,
                                   spell_leotheras_insidious_whisper_aura);
