...
enum WaterElementalPathIds { PATH_CENTER = 5, PATH_END = 12 };

struct boss_hydross_the_unstable : public BossAI {
    boss_hydross_the_unstable(Creature* creature)
        : BossAI(creature, DATA_HYDROSS_THE_UNSTABLE), _recentlySpoken(false)
    {
    }
    ...
    void SummonMovementInform(Creature* summon,
                              uint32    movementType,
                              uint32    pathId) override
    {
        if (movementType == WAYPOINT_MOTION_TYPE) {
            if (pathId == PATH_CENTER) {
                ...
                // Happens even if Hydross is dead, so completely detached to
                // the spell, which is nothing but a dummy anyways.
    ...
    void SetForm(bool corrupt, bool initial)
    {
        ...
        if (corrupt) {
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_SCHOOL, SPELL_SCHOOL_MASK_FROST, false);
            me->ApplySpellImmune(
                0, IMMUNITY_SCHOOL, SPELL_SCHOOL_MASK_NATURE, true);
            ...
            scheduler
                ...
                .Schedule(
                    12150ms, GROUP_ABILITIES, [this](TaskContext context) {
                        DoCastRandomTarget(
                            SPELL_VILE_SLUDGE, 0, 0.0f, true, true);
            ...
            scheduler
                .Schedule(
                    15s,
                    GROUP_ABILITIES,
                    [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS1); })
                .Schedule(
                    30s,
                    GROUP_ABILITIES,
                    [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS2); })
                .Schedule(
                    45s,
                    GROUP_ABILITIES,
                    [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS3); })
                .Schedule(
                    60s,
                    GROUP_ABILITIES,
                    [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS4); })
                .Schedule(
                    75s,
                    GROUP_ABILITIES,
                    [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS5); })
                .Schedule(90s,
                          GROUP_ABILITIES,
                          [this](TaskContext context) {
                ...
                .Schedule(
                    12150ms, GROUP_ABILITIES, [this](TaskContext context) {
                        DoCastRandomTarget(
                            SPELL_WATER_TOMB, 0, 0.0f, true, true);
                        context.Repeat(9700ms, 32800ms);
                    });
        }
        ...
        if (corrupt) {
            ...
            for (uint32 spellId = SPELL_SUMMON_CORRUPTED1;
                 spellId <= SPELL_SUMMON_CORRUPTED4;
                 ++spellId) {
            ...
            for (uint32 spellId = SPELL_SUMMON_PURIFIED1;
                 spellId <= SPELL_SUMMON_PURIFIED4;
                 ++spellId) {
                DoCastSelf(spellId, true);
            }
        }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler
            .Schedule(1s,
                      [this](TaskContext context) {
                          if (me->HasAura(SPELL_BLUE_BEAM) ==
                              me->HasAura(SPELL_CORRUPTION)) {
                              SetForm(!me->HasAura(SPELL_BLUE_BEAM), false);
                          }
                          context.Repeat(1s);
                      })
            .Schedule(10min,
                      [this](TaskContext) { DoCastSelf(SPELL_ENRAGE, true); });
    }

    void KilledUnit(Unit* /*victim*/) override
    {
        if (!_recentlySpoken) {
            Talk(me->HasAura(SPELL_CORRUPTION) ? SAY_CORRUPT_SLAY
                                               : SAY_CLEAN_SLAY);
            _recentlySpoken = true;
        }
        scheduler.Schedule(6s,
                           [this](TaskContext) { _recentlySpoken = false; });
    }
    ...
    {
        Talk(me->HasAura(SPELL_CORRUPTION) ? SAY_CORRUPT_DEATH
                                           : SAY_CLEAN_DEATH);
...
class spell_hydross_cleansing_field_aura : public AuraScript {
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_hydross_cleansing_field_aura::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_hydross_cleansing_field_aura::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hydross_cleansing_field_command : public AuraScript {
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectApplyFn(
            spell_hydross_cleansing_field_command::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hydross_mark_of_hydross : public AuraScript {
    ...
    void HandleEffectApply(AuraEffect const* /*aurEff*/,
                           AuraEffectHandleModes /*mode*/)
    {
        GetTarget()->RemoveAurasByType(
            SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, GetCasterGUID(), GetAura());
    }

    void Register() override
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_hydross_mark_of_hydross::HandleEffectApply,
                              EFFECT_0,
