...
struct boss_the_lurker_below : public BossAI {
    boss_the_lurker_below(Creature* creature)
        : BossAI(creature, DATA_THE_LURKER_BELOW)
    {
    }
                     ...
                     SpellSchoolMask /*school*/) override
    {
        if (damage >= me->GetHealth()) {
            // Liquid state around his area is working awkwardly and doesn't
            // consider him to be swimming Therefore he falls to the bottom of
            // the lake when he dies, because he is "flying" in his pool
            // Removing this prevents him from dropping to the bottom of the
            // lake There is also a visual issue currently causing him to drop
            // to the bottom of the lake but his body not update clientside So
            // it looks as if he was floating but he has in fact dropped to the
            // bottom of the lake, and thus becomes unlootable
    ...
    void SchedulerPhaseOne(std::chrono::milliseconds spoutTimer,
        ...
        scheduler
            ...
            .Schedule(spoutTimer,
                      [this](TaskContext context) {
                          ...
                          scheduler.Schedule(3s, [this](TaskContext) {
                              ...
                              DoCastSelf(RAND(SPELL_SPOUT_PERIODIC_1,
            ...
            .Schedule(p2Timer, [this](TaskContext) {
                ...
                for (uint8 i = 0; i < MAX_SUMMONS; ++i) {
                    ...
                    me->SummonCreature(i < 6 ? NPC_COILFANG_AMBUSHER
                                             : NPC_COILFANG_GUARDIAN,
                                       positions[i].GetPositionX(),
                                       positions[i].GetPositionY(),
                                       positions[i].GetPositionZ(),
                                       positions[i].GetAngle(me),
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (me->getStandState() != UNIT_STAND_STATE_STAND ||
            !me->isAttackReady() || me->GetReactState() != REACT_AGGRESSIVE)
            return;
        ...
        if (me->IsWithinMeleeRange(me->GetVictim())) {
            ...
            for (ThreatReference const* ref : t_list) {
                if (Unit* threatTarget =
                        ObjectAccessor::GetUnit(*me, ref->getUnitGuid())) {
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        if (InstanceScript* instance = go->GetInstanceScript())
            if (roll_chance_i(instance->GetBossState(DATA_THE_LURKER_BELOW) !=
                                      DONE
                                  ? 25
                                  : 0) &&
                ...
                if (Creature* lurker =
                        go->SummonCreature(NPC_THE_LURKER_BELOW,
                                           38.4567f,
                                           -417.324f,
                                           -18.916666f,
                                           2.94960f,
...
class spell_lurker_below_spout : public AuraScript {
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        ...
        float orientation = GetSpellInfo()->Id == SPELL_SPOUT_PERIODIC_1
                                ? Position::NormalizeOrientation(
                                      caster->GetOrientation() + 0.1f)
                                : Position::NormalizeOrientation(
                                      caster->GetOrientation() - 0.1f);
    ...
    void Register() override
    {
        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_lurker_below_spout::CalcPeriodic,
                                     EFFECT_0,
                                     SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        OnEffectApply +=
            AuraEffectApplyFn(spell_lurker_below_spout::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_lurker_below_spout::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                               AURA_EFFECT_HANDLE_REAL);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_lurker_below_spout::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_lurker_below_spout_cone : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([this](WorldObject const* target) -> bool {
            return !GetCaster()->HasInLine(target, 5.0f) ||
                   !target->IsPlayer() || target->ToUnit()->IsInWater();
        });
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_lurker_below_spout_cone::FilterTargets,
            EFFECT_ALL,
