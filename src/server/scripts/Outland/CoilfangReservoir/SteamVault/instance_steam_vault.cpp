...
enum MainChambersAccessPanelSays { SAY_FAINT_ECHO = 0, SAY_LOUD_RUMBLE = 1 };
...
public:
    go_main_chambers_access_panel()
        : GameObjectScript("go_main_chambers_access_panel")
    {
    }
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        if (InstanceScript* instance = go->GetInstanceScript()) {
            Creature* doorController =
                instance->GetCreature(DATA_DOOR_CONTROLLER);
            if (go->GetEntry() == GO_ACCESS_PANEL_HYDRO) {
                ...
                if (instance->GetBossState(DATA_MEKGINEER_STEAMRIGGER) ==
                    DONE) {
                    go->SetGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                    if (doorController && doorController->IsAIEnabled) {
                        doorController->AI()->Talk(SAY_FAINT_ECHO);
            ...
            if (instance->GetBossState(DATA_HYDROMANCER_THESPIA) == DONE &&
                ...
                if (doorController) {
                    ...
                    doorController->m_Events.AddEventAtOffset(
                        [instance]() {
                            if (GameObject* mainGate = instance->GetGameObject(
                                    DATA_MAIN_CHAMBERS_DOOR)) {
                                instance->HandleGameObject(
                                    ObjectGuid::Empty, true, mainGate);
                            }
                        },
                        4s);
                }
                else if (GameObject* mainGate =
                             instance->GetGameObject(DATA_MAIN_CHAMBERS_DOOR)) {
                    instance->HandleGameObject(
                        ObjectGuid::Empty, true, mainGate);
                }
            }
