...
struct boss_warlord_kalithresh : public BossAI {
    ...
    void Reset() override
    {
        ...
        instance->DoForAllMinions(
            DATA_WARLORD_KALITHRESH, [&](Creature* minion) {
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        scheduler
            ...
            .Schedule(20s, [this](TaskContext context) {
                ...
                if (Creature* distiller =
                        me->FindNearestCreature(NPC_NAGA_DISTILLER, 8.0f)) {
                    distiller->AI()->DoCast(
                        me, SPELL_WARLORDS_RAGE_DISTILLER, true);
                    distiller->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
                                              UNIT_FLAG_NOT_SELECTABLE);
                }
                else {
                    if (Creature* distiller = me->FindNearestCreature(
                            NPC_NAGA_DISTILLER, 100.0f)) {
                        me->GetMotionMaster()->MoveFollow(
                            distiller, 8.0f, 0.0f);

                        scheduler.Schedule(
                            1s, [this](TaskContext chaseContext) {
                                if (Creature* distiller =
                                        me->FindNearestCreature(
                                            NPC_NAGA_DISTILLER, 8.0f)) {
                                    distiller->AI()->DoCast(
                                        me,
                                        SPELL_WARLORDS_RAGE_DISTILLER,
                                        true);
                                    distiller->RemoveUnitFlag(
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        instance->DoForAllMinions(
            ...
            [&](Creature* minion) { minion->KillSelf(); });
    }
...
class spell_warlords_rage : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE) {
            if (GetTarget()) {
                GetTarget()->CastSpell(
                    GetTarget(), SPELL_WARLORDS_RAGE_PROC, true);
            }
        }
    }
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_warlords_rage::HandleAfterRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
