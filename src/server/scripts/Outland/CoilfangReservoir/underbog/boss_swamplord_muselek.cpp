...
enum Text { SAY_AGGRO = 1, SAY_KILL = 2, SAY_JUST_DIED = 3 };

enum Misc { RANGED_GROUP = 1, RANGE_CHECK = 2 };
...
struct boss_swamplord_muselek : public BossAI {
    ...
    bool CanShootVictim()
    {
        ...
        if (!victim || !me->IsWithinLOSInMap(victim) ||
            !me->IsWithinRange(victim, 30.f) ||
        ...
        scheduler
            ...
            .Schedule(30s, 40s, [this](TaskContext context) {
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 40.0f, false, true)) {
                    ...
                    scheduler.Schedule(3s, [this, target](TaskContext) {
                        if (target && me->GetVictim()) {
                            if (me->IsWithinMeleeRange(me->GetVictim())) {
                                me->GetMotionMaster()->Clear();
                                me->GetMotionMaster()->MoveForwards(
                                    me->GetVictim(), 10.0f);
                                _canChase = false;
                            }
