...
SDName: Nagrand
SD%Complete: 90
SDComment: Quest support: 9868, 9874, 10085. TextId's unknown for
altruis_the_sufferer and greatmother_geyah (npc_text) SDCategory: Nagrand
EndScriptData */
...
public:
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_TOTEM_KARDASH_H) {
            if (npc_maghar_captiveAI* EscortAI =
                    dynamic_cast<npc_maghar_captiveAI*>(creature->AI())) {
                creature->SetStandState(UNIT_STAND_STATE_STAND);
                creature->SetFaction(FACTION_ESCORTEE_H_NEUTRAL_ACTIVE);
                EscortAI->Start(true, false, player->GetGUID(), quest);
                creature->AI()->Talk(SAY_MAG_START);

                creature->SummonCreature(NPC_MURK_RAIDER,
                                         m_afAmbushA[0] + 2.5f,
                                         m_afAmbushA[1] - 2.5f,
                                         m_afAmbushA[2],
                                         0.0f,
                                         ...
                                         25000);
                creature->SummonCreature(NPC_MURK_PUTRIFIER,
                                         m_afAmbushA[0] - 2.5f,
                                         m_afAmbushA[1] + 2.5f,
                                         m_afAmbushA[2],
                                         0.0f,
                                         TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                         25000);
                creature->SummonCreature(NPC_MURK_BRUTE,
                                         m_afAmbushA[0],
                                         m_afAmbushA[1],
                                         m_afAmbushA[2],
                                         0.0f,
    ...
    struct npc_maghar_captiveAI : public npc_escortAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Player* player = GetPlayerForEscort()) {
                if (player->GetQuestStatus(QUEST_TOTEM_KARDASH_H) !=
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 7:
                ...
                me->SummonCreature(NPC_MURK_PUTRIFIER,
                                   m_afAmbushB[0] - 2.5f,
                                   m_afAmbushB[1] - 2.5f,
                                   m_afAmbushB[2],
                                   0.0f,
                                   TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                   25000);
                me->SummonCreature(NPC_MURK_SCAVENGER,
                                   m_afAmbushB[0] + 2.5f,
                                   m_afAmbushB[1] + 2.5f,
                                   m_afAmbushB[2],
                                   0.0f,
        ...
        void JustSummoned(Creature* summoned) override
        {
            ...
            summoned->GetMotionMaster()->MovePoint(
                0, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ());
...
public:
    npc_creditmarker_visit_with_ancestors()
        : CreatureScript("npc_creditmarker_visit_with_ancestors")
    {
    }
    ...
    struct npc_creditmarker_visit_with_ancestorsAI : public ScriptedAI {
...

        {
            ...
            if (player &&
                player->GetQuestStatus(10085) == QUEST_STATUS_INCOMPLETE) {
                uint32 creditMarkerId = me->GetEntry();
                if (creditMarkerId >= 18840 && creditMarkerId <= 18843) {
                    // 18840: Sunspring, 18841: Laughing, 18842: Garadar, 18843:
                    // Bleeding
                    if (!player->GetReqKillOrCastCurrentCount(10085,
                                                              creditMarkerId))
                        player->KilledMonsterCredit(creditMarkerId,
                                                    me->GetGUID());
                }
            }
        }
    ...
    struct npc_kurenai_captiveAI : public npc_escortAI {
        ...
        void SetGUID(ObjectGuid guid, int32 /*questId*/) override
        {
            ...
            me->SummonCreature(NPC_KUR_MURK_RAIDER,
                               kurenaiAmbushA[0] + 2.5f,
                               kurenaiAmbushA[1] - 2.5f,
                               kurenaiAmbushA[2],
                               0.0f,
                               TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                               50000);
            me->SummonCreature(NPC_KUR_MURK_BRUTE,
                               kurenaiAmbushA[0] - 2.5f,
                               kurenaiAmbushA[1] + 2.5f,
                               kurenaiAmbushA[2],
                               0.0f,
                               TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                               50000);
            me->SummonCreature(NPC_KUR_MURK_SCAVENGER,
                               kurenaiAmbushA[0],
                               kurenaiAmbushA[1],
                               kurenaiAmbushA[2],
                               0.0f,
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Player* player = GetPlayerForEscort()) {
                if (player->GetQuestStatus(QUEST_TOTEM_KARDASH_A) !=
                    QUEST_STATUS_COMPLETE)
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 3: {
                ...
                me->SummonCreature(NPC_KUR_MURK_PUTRIFIER,
                                   kurenaiAmbushB[0] - 2.5f,
                                   kurenaiAmbushB[1] - 2.5f,
                                   kurenaiAmbushB[2],
                                   0.0f,
                                   TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                   25000);
        ...
        void JustSummoned(Creature* summoned) override
        {
            ...
            // This function is for when we summoned enemies to fight - so that
            // does NOT mean we should make our totem count in this!
            if (summoned->IsTotem())
                return;
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        if (player->GetQuestStatus(QUEST_FINDING_THE_SURVIVORS) !=
            QUEST_STATUS_INCOMPLETE)
            return false;

        if (Creature* prisoner =
                go->FindNearestCreature(NPC_MAGHAR_PRISONER, 5.0f)) {
