...
public:
    spell_q10935_the_exorcism_of_colonel_jules()
        : SpellScriptLoader("spell_q10935_the_exorcism_of_colonel_jules")
    {
    }
    ...
    class spell_q10935_the_exorcism_of_colonel_jules_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_q10935_the_exorcism_of_colonel_jules_SpellScript);
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            if (GetCaster()->IsHostileTo(target))
                ...
                GetCaster()->CastSpell(
                    target, 39322 /*SPELL_HEAL_BARADA*/, true);
        }
            ...
            case 50:
                if (Creature* ryga = me->FindNearestCreature(NPC_RYGA, 70)) {
                    if (ryga->IsAlive() && !ryga->IsInCombat()) {
                        ...
                        ryga->GetMotionMaster()->MovePoint(
                            0, 515.877991f, 3885.67627f, 190.470535f, true);
                        Reset();
                    }
                }
                break;
            ...
            case 52:
                if (Creature* ryga = me->FindNearestCreature(NPC_RYGA, 70)) {
                    if (ryga->IsAlive() && !ryga->IsInCombat()) {
                        ...
                        ryga->SetSpeed(MOVE_WALK, 1.0f);
                        ryga->GetMotionMaster()->MovePoint(
                            0, 504.59201f, 3882.12988f, 192.156006f, true);
                        Reset();
                    }
                }
                break;
...
public:
    ...
    struct npc_wounded_blood_elfAI : public npc_escortAI {
        ...
        {
            me->SetReactState(REACT_PASSIVE);
            me->FindNearestGameObject(ARAKKOA_CAGE, 10.0f)
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 9:
                ...
                DoSpawnCreature(NPC_HAALESHI_TALONGUARD,
                                -15,
                                -15,
                                0,
                                0,
            ...
            case 14:
                ...
                DoSpawnCreature(NPC_HAALESHI_WINDWALKER,
                                -15,
                                -15,
                                0,
                                0,
...
public:
    ...
    struct npc_fel_guard_houndAI : public ScriptedAI {
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (Creature* helboar =
                    ObjectAccessor::GetCreature(*me, helboarGUID)) {
        ...
        {
            if (checkTimer <= diff) {
                if (Creature* helboar = me->FindNearestCreature(
                        NPC_DERANGED_HELBOAR, 10.0f, false)) {
                    if (helboar->GetGUID() != helboarGUID &&
                        ...
                        me->GetMotionMaster()->MovePoint(
                            1,
                            helboar->GetPositionX(),
                            helboar->GetPositionY(),
                            helboar->GetPositionZ());
                    }
                }
                if (Player* owner =
        ...
        void OnStateChanged(uint32 state, Unit* /*unit*/) override
        {
            if (state == GO_ACTIVATED) {
                me->GetCreaturesWithEntryInRange(
                    creatureList, 40, NPC_STONESCHYE_WHELP);
                {
                    for (Creature* whelp : creatureList) {
                        if (whelp->IsAlive() && !whelp->IsInCombat() &&
                            whelp->GetMotionMaster()
                                    ->GetCurrentMovementGeneratorType() !=
                                HOME_MOTION_TYPE) {
                            whelp->GetMotionMaster()->MovePoint(
                                0,
                                me->GetNearPosition(4.0f,
                                                    whelp->GetOrientation()));
                        }
                    }
                }
            }
