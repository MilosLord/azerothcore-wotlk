...
enum EmeraldDragonSpells {
    SPELL_TAIL_SWEEP = 15847, // tail sweep - slap everything behind dragon (2
                              // seconds interval)
    SPELL_SUMMON_PLAYER = 24776, // teleport highest threat player in front of
                                 // dragon if wandering off
    ...
    SPELL_MARK_OF_NATURE =
        25040, // Mark of Nature trigger (applied on target death - 15 minutes
               // of being suspectible to Aura Of Nature)
    SPELL_MARK_OF_NATURE_AURA =
        25041, // Mark of Nature (passive marker-test, ticks every 10 seconds
               // from boss, triggers spellID 25042 (scripted)
...
struct emerald_dragonAI : public WorldBossAI {
    ...
    // Target killed during encounter, mark them as suspectible for Aura Of
    // Nature
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case EVENT_SEEPING_FOG:
            ...
            // given time! Despawntime is 2 minutes, so reschedule it for new
            // cast after 2 minutes + a minor "random time" (30 seconds at max)
    ...
    struct npc_dream_fogAI : public ScriptedAI {
        ...
        void ScheduleEvents()
        {
            ...
            scheduler.Schedule(1s, [this](TaskContext context) {
                ...
                // Seeping fog movement is slow enough for a player to be able
                // to walk backwards and still outpace it
                me->SetWalk(true);
        ...
        {
            if (Creature* dragon =
                    ObjectAccessor::GetCreature(*me, _dragonGUID)) {
                if (dragon->GetAI()) {
                    return dragon->GetAI()->SelectTarget(
...
public:
    ...
    struct boss_lethonAI : public emerald_dragonAI {
        ...
        {
            if (spell->Id == SPELL_DRAW_SPIRIT &&
                target->GetTypeId() == TYPEID_PLAYER) {
                Position targetPos = target->GetPosition();
                me->SummonCreature(NPC_SPIRIT_SHADE,
                                   targetPos,
...
public:
    ...
    struct boss_taerarAI : public emerald_dragonAI {
        ...
        void DamageTaken(Unit*,
                         ...
                         SpellSchoolMask) override
        {
            // At 75, 50 or 25 percent health, we need to activate the shades
            // and go "banished" Note: _stage holds the amount of times they
            // have been summoned
            if (!_banished &&
                me->HealthBelowPctDamaged(100 - (25 * _stage), damage)) {
                _banished      = true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_banished) {
                // If all three shades are dead, OR it has taken too long, end
                // the current event and get Taerar back into business
                ...
                // _banishtimer has not expired, and we still have active
                // shades:
                else
                    _banishedTimer -= diff;

                // Update the events before we return (handled under
                // emerald_dragonAI::UpdateAI(diff); if we're not inside this
                // check)
                events.Update(diff);
    ...
    private:
        ...
        uint8 _stage; // check which "shade phase" we're at (75-50-25 percentage
                      // counters)
    };
...
public:
    ...
    class spell_dream_fog_sleep_SpellScript : public SpellScript {
        ...
        void HandleEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster()) {
                if (Unit* target = GetHitUnit()) {
                    caster->GetAI()->SetGUID(target->GetGUID(),
                                             GUID_FOG_TARGET);
                }
            }
        }
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_dream_fog_sleep_SpellScript::HandleEffect,
                              EFFECT_0,
                              SPELL_EFFECT_APPLY_AURA);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_dream_fog_sleep_SpellScript::FilterTargets,
                EFFECT_0,
...
public:
    ...
    {
        // return those not tagged or already under the influence of Aura of
        // Nature
        if (Unit* unit = object->ToUnit())
            return !(unit->HasAura(SPELL_MARK_OF_NATURE) &&
                     !unit->HasAura(SPELL_AURA_OF_NATURE));
        return true;
    }
};
...
class spell_shadow_bolt_whirl : public AuraScript {
    ...
    void HandlePeriodic(AuraEffect const* aurEff)
    {
        ...
        if (tick <= 3)
            target->SetOrientation(forward + 0.75f * M_PI -
                                   tick * M_PI / 8); // Left
        else
            target->SetOrientation(forward - 0.75f * M_PI +
                                   (8 - tick) * M_PI / 8); // Right
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_shadow_bolt_whirl::HandlePeriodic,
                                 EFFECT_0,
    ...
    class spell_mark_of_nature_SpellScript : public SpellScript {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return ValidateSpellInfo(
                {SPELL_MARK_OF_NATURE, SPELL_AURA_OF_NATURE});
        }
        ...
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_mark_of_nature_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget +=
                SpellEffectFn(spell_mark_of_nature_SpellScript::HandleEffect,
                              EFFECT_0,
                              SPELL_EFFECT_APPLY_AURA);
        }
    };
