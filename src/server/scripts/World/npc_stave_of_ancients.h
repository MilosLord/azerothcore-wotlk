...
enum NPCArtorius {
    ...
    ARTORIUS_WEAKNESS_EMOTE = 9786, // broadcast_text.ID         "%s is stricken
                                    // by a virulent poison."
    ARTORIUS_SAY =
        9787, // broadcast_text.ID         "Your soul is mine, weakling."
...
enum NPCSimone {
    ...
    SIMONE_EMOTE =
        9759, // broadcast_text.ID         "%s laughs at your foolish request."
    SIMONE_SAY = 9760, // broadcast_text.ID         "As you wish, $c."
    SIMONE_WEAKNESS_EMOTE = 9762, // broadcast_text.ID         "%s is silenced
                                  // by the venomous sting."
...
enum NPCPrecious { PRECIOUS_NORMAL_ENTRY = 14528, PRECIOUS_EVIL_ENTRY = 14538 };

enum NPCNelson {
    NELSON_EVENT_DREADFUL_FRIGHT = 6,
    NELSON_EVENT_CREEPING_DOOM   = 7,

    NELSON_SAY = 9782, // broadcast_text.ID          "You dare challenge me?
                       // Prepare for an e..."
    NELSON_WEAKNESS_EMOTE =
        9785, // breadcast_text.ID          "%s is immobilized."
    NELSON_DESPAWN_SAY = 9815, // breadcast_text.ID          "Only a fool would
                               // remain in this battle..."
...
enum NPCFranklin {
    ...
    FRANKLIN_ENRAGE_EMOTE =
        9764, // broadcast_text.ID          "%s goes into a killing frenzy!"
    FRANKLIN_DESPAWN_SAY = 9815, // breadcast_text.ID          "Only a fool
                                 // would remain in this battle..."
...
struct NPCStaveQuestAI : public ScriptedAI {
    ...
    std::map<int, int> entryKeys = {{ARTORIUS_NORMAL_ENTRY, 1},
    ...
    std::map<int, std::map<std::string, int>> entryList = {
        {1, {{"normal", ARTORIUS_NORMAL_ENTRY}, {"evil", ARTORIUS_EVIL_ENTRY}}},
        {2, {{"normal", SIMONE_NORMAL_ENTRY}, {"evil", SIMONE_EVIL_ENTRY}}},
        {3, {{"normal", PRECIOUS_NORMAL_ENTRY}, {"evil", PRECIOUS_EVIL_ENTRY}}},
        {4, {{"normal", NELSON_NORMAL_ENTRY}, {"evil", NELSON_EVIL_ENTRY}}},
        {5,
         {{"normal", FRANKLIN_NORMAL_ENTRY}, {"evil", FRANKLIN_EVIL_ENTRY}}}};
