...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
    {
        if (GameObject* go = GetClosestGameObjectWithEntry(
                player, GO_COILFANG_WATERFALL, 35.0f))
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
    {
        if (!player->isDead() &&
            ...
            player->CastSpell(
                player, SPELL_CREATE_TRUE_POWER_OF_THE_TEMPEST, false);
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* trigger) override
    {
        ...
        case 5339:
            pPosition =
                WorldLocation(571, 3733.68f, 3563.25f, 290.812f, 3.665192f);
            break;
        case 5334:
        ...
        case 5340:
            if (QBT == QUEST_STATUS_INCOMPLETE)
                ...
                pPosition =
                    WorldLocation(571, 3687.91f, 3577.28f, 473.342f, 0.0f);
            break;
        default:
            return false;
        }
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
    {
        ...
        if (player->GetQuestStatus(QUEST_NATS_BARGAIN) ==
            QUEST_STATUS_INCOMPLETE) {
            if (!player->FindNearestCreature(NPC_LURKING_SHARK, 20.0f)) {
                if (Creature* shark = player->SummonCreature(
                        NPC_LURKING_SHARK,
                        -4246.243f,
                        -3922.356f,
                        -7.488f,
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
    {
        ...
        if (!player->IsAlive() || quest_status == QUEST_STATUS_NONE ||
            quest_status == QUEST_STATUS_REWARDED)
            return false;

        if (!player->FindNearestCreature(NPC_TERVOSH, 100.0f)) {
            if (Creature* tervosh =
                    player->SummonCreature(NPC_TERVOSH,
                                           -3476.51f,
                                           -4105.94f,
                                           17.1f,
                                           5.3816f,
...
public:
    AreaTrigger_at_brewfest() : AreaTriggerScript("at_brewfest")
    {
        ...
        _triggerTimes[AT_BREWFEST_DUROTAR] =
            _triggerTimes[AT_BREWFEST_DUN_MOROGH] = 0;
    }
    ...
    bool OnTrigger(Player* player, AreaTrigger const* trigger) override
    {
        ...
        if (GameTime::GetGameTime().count() - _triggerTimes[triggerId] <
        ...
        case AT_BREWFEST_DUROTAR:
            if (Creature* tapper =
                    player->FindNearestCreature(NPC_TAPPER_SWINDLEKEG, 20.0f))
                tapper->AI()->Talk(SAY_WELCOME, player);
            break;
        case AT_BREWFEST_DUN_MOROGH:
            if (Creature* ipfelkofer =
                    player->FindNearestCreature(NPC_IPFELKOFER_IRONKEG, 20.0f))
...
public:
    ...
    {
        _triggerTimes[AT_AREA_52_SOUTH]    = _triggerTimes[AT_AREA_52_NORTH] =
            _triggerTimes[AT_AREA_52_WEST] = _triggerTimes[AT_AREA_52_EAST] = 0;
    }
    ...
    bool OnTrigger(Player* player, AreaTrigger const* trigger) override
    {
        ...
        uint32 triggerId = trigger->entry;
        if (GameTime::GetGameTime().count() - _triggerTimes[trigger->entry] <
            SUMMON_COOLDOWN)
            return false;
        ...
        player->SummonCreature(
            NPC_SPOTLIGHT, x, y, z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 5000);
