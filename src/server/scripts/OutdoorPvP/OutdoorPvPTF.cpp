...
{
    SetCapturePointData(TFCapturePoints[type].entry,
                        TFCapturePoints[type].map,
                        TFCapturePoints[type].x,
                        TFCapturePoints[type].y,
                        TFCapturePoints[type].z,
                        TFCapturePoints[type].o,
                        TFCapturePoints[type].rot0,
                        TFCapturePoints[type].rot1,
                        TFCapturePoints[type].rot2,
                        TFCapturePoints[type].rot3);
}

void OPvPCapturePointTF::FillInitialWorldStates(WorldPacket& data)
{
    data << uint32(TFTowerWorldStates[m_TowerType].n)
         << uint32(bool(m_TowerState & TF_TOWERSTATE_N));
    data << uint32(TFTowerWorldStates[m_TowerType].h)
         << uint32(bool(m_TowerState & TF_TOWERSTATE_H));
    data << uint32(TFTowerWorldStates[m_TowerType].a)
         << uint32(bool(m_TowerState & TF_TOWERSTATE_A));
}
...
void OutdoorPvPTF::FillInitialWorldStates(WorldPacket& data)
{
    ...
    data << TF_UI_LOCKED_DISPLAY_NEUTRAL
         << uint32(m_IsLocked && !m_HordeTowersControlled &&
                   !m_AllianceTowersControlled);
    data << TF_UI_LOCKED_DISPLAY_HORDE
         << uint32(m_IsLocked &&
                   (m_HordeTowersControlled > m_AllianceTowersControlled));
    data << TF_UI_LOCKED_DISPLAY_ALLIANCE
         << uint32(m_IsLocked &&
                   (m_HordeTowersControlled < m_AllianceTowersControlled));
...
void OutdoorPvPTF::SendRemoveWorldStates(Player* player)
{
    ...
    player->SendUpdateWorldState(TF_UI_LOCKED_TIME_MINUTES_FIRST_DIGIT,
                                 uint32(0));
    player->SendUpdateWorldState(TF_UI_LOCKED_TIME_MINUTES_SECOND_DIGIT,
                                 uint32(0));
    ...
    for (int i = 0; i < TF_TOWER_NUM; ++i) {
        player->SendUpdateWorldState(uint32(TFTowerWorldStates[i].n),
                                     uint32(0));
        player->SendUpdateWorldState(uint32(TFTowerWorldStates[i].h),
                                     uint32(0));
        player->SendUpdateWorldState(uint32(TFTowerWorldStates[i].a),
                                     uint32(0));
    }
}
...
void OutdoorPvPTF::SaveRequiredWorldStates() const
{
    ...
    // Save expiry as unix
    uint32 const lockExpireTime =
        GameTime::GetGameTime().count() + (m_LockTimer / IN_MILLISECONDS);
...
{
    _pvp->SendUpdateWorldState(uint32(TFTowerWorldStates[m_TowerType].n),
                               uint32(bool(m_TowerState & TF_TOWERSTATE_N)));
    _pvp->SendUpdateWorldState(uint32(TFTowerWorldStates[m_TowerType].h),
                               uint32(bool(m_TowerState & TF_TOWERSTATE_H)));
    _pvp->SendUpdateWorldState(uint32(TFTowerWorldStates[m_TowerType].a),
                               uint32(bool(m_TowerState & TF_TOWERSTATE_A)));
}
    ...
    if (m_IsLocked) {
        ...
        if (m_LockTimer < diff) {
            ...
            if (m_LockTimerUpdate < diff) {
                ...
                SendUpdateWorldState(TF_UI_LOCKED_TIME_MINUTES_FIRST_DIGIT,
                                     first_digit);
                SendUpdateWorldState(TF_UI_LOCKED_TIME_MINUTES_SECOND_DIGIT,
                                     second_digit);
...
bool OutdoorPvPTF::IsLocked() const { return m_IsLocked; }
...
bool OutdoorPvPTF::SetupOutdoorPvP()
{
    ...
    if (m_IsLocked) {
        // Core shutdown while locked -- init from latest known data in
        // WorldState Convert from unix
        int32 const lockRemainingTime =
            int32((sWorld->getWorldState(TF_UI_LOCKED_TIME_HOURS) -
        ...
        if (lockRemainingTime > 0) {
            ...
            uint32 const hordeTowers =
                uint32(sWorld->getWorldState(TF_UI_TOWER_COUNT_H));
            uint32 const allianceTowers =
                uint32(sWorld->getWorldState(TF_UI_TOWER_COUNT_A));
            TeamId const controllingTeam =
                hordeTowers > allianceTowers ? TEAM_HORDE : TEAM_ALLIANCE;
...
bool OPvPCapturePointTF::Update(uint32 diff)
{
    ...
    bool canupdate =
        ((((OutdoorPvPTF*)_pvp)->GetAllianceTowersControlled() > 0) &&
         _activePlayers[0].size() > _activePlayers[1].size()) ||
        ((((OutdoorPvPTF*)_pvp)->GetHordeTowersControlled() > 0) &&
         _activePlayers[0].size() < _activePlayers[1].size());
...
void OPvPCapturePointTF::ChangeState()
{
    ...
    if (_oldState == OBJECTIVESTATE_ALLIANCE) {
        if (uint32 alliance_towers =
                ((OutdoorPvPTF*)_pvp)->GetAllianceTowersControlled())
            ((OutdoorPvPTF*)_pvp)
                ->SetAllianceTowersControlled(--alliance_towers);
        sWorld->SendZoneText(
            OutdoorPvPTFBuffZones[0],
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_TF_LOSE_A));
    }
    // if changing from controlling horde to alliance
    else if (_oldState == OBJECTIVESTATE_HORDE) {
        if (uint32 horde_towers =
                ((OutdoorPvPTF*)_pvp)->GetHordeTowersControlled())
            ((OutdoorPvPTF*)_pvp)->SetHordeTowersControlled(--horde_towers);
        sWorld->SendZoneText(
            OutdoorPvPTFBuffZones[0],
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_TF_LOSE_H));
    }
    ...
    case OBJECTIVESTATE_ALLIANCE: {
        ...
        sWorld->SendZoneText(
            OutdoorPvPTFBuffZones[0],
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_TF_CAPTURE_A));

        for (PlayerSet::iterator itr = _activePlayers[0].begin();
             itr != _activePlayers[0].end();
             ++itr)
    ...
    case OBJECTIVESTATE_HORDE: {
        ...
        sWorld->SendZoneText(
            OutdoorPvPTFBuffZones[0],
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_TF_CAPTURE_H));

        for (PlayerSet::iterator itr = _activePlayers[1].begin();
             itr != _activePlayers[1].end();
             ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
...
public:
    ...
    OutdoorPvP* GetOutdoorPvP() const override { return new OutdoorPvPTF(); }
};

void AddSC_outdoorpvp_tf() { new OutdoorPvP_terokkar_forest(); }
