...
const uint32 HP_LANG_LOSE_A[HP_TOWER_NUM] = {LANG_OPVP_HP_LOSE_BROKENHILL_A,
                                             LANG_OPVP_HP_LOSE_OVERLOOK_A,
                                             LANG_OPVP_HP_LOSE_STADIUM_A};

const uint32 HP_LANG_LOSE_H[HP_TOWER_NUM] = {LANG_OPVP_HP_LOSE_BROKENHILL_H,
...
void OPvPCapturePointHP::ChangeState()
{
    ...
    bounds =
        map->GetGameObjectBySpawnIdStore().equal_range(_objects[m_TowerType]);
    for (auto itr = bounds.first; itr != bounds.second; ++itr)
...
public:
    ...
    OutdoorPvP* GetOutdoorPvP() const override { return new OutdoorPvPHP(); }
};

void AddSC_outdoorpvp_hp() { new OutdoorPvP_hellfire_peninsula(); }
