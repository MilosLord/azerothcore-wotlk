...
void OutdoorPvPNA::HandleKill(Player* killer, Unit* killed)
{
    if (Group* group = killer->GetGroup()) {
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
            ...
            // inside objective
            if ((groupGuy->IsOutdoorPvPActive() &&
                 groupGuy->GetAreaId() == NA_HALAA_ZONE_ID) ||
                killed->GetTypeId() == TYPEID_UNIT) {
        ...
        // outdoor pvp active
        if (killer && ((killer->IsOutdoorPvPActive() &&
                        killer->ToPlayer()->GetAreaId() == NA_HALAA_ZONE_ID) ||
                       killed->GetTypeId() == TYPEID_UNIT)) {
...
void OutdoorPvPNA::HandleKillImpl(Player* player, Unit* killed)
{
    if (killed->GetTypeId() == TYPEID_PLAYER &&
        player->GetTeamId() != killed->ToPlayer()->GetTeamId()) {
                         ...
                         float               y)
{
    ...
    if (!map->Instanceable() && !map->IsRemovalGrid(x, y)) {
        ...
        if (!creature->LoadCreatureFromDB(spawnId, map, true, true)) {
            LOG_ERROR("sql.sql",
                      "AddCreature: Cannot add creature spawnId {} to map",
                      spawnId);
            delete creature;
            return;
        }
...
uint32 OPvPCapturePointNA::GetAliveGuardsCount()
{
    ...
    for (auto itr = _creatures.begin(); itr != _creatures.end(); ++itr) {
        auto bounds = _pvp->GetMap()->GetCreatureBySpawnIdStore().equal_range(
            itr->second);
        for (auto itr2 = bounds.first; itr2 != bounds.second; ++itr2)
            if (itr2->second->IsAlive() &&
                (itr2->second->GetEntry() == NA_HALAANI_GUARD_A ||
                 itr2->second->GetEntry() == NA_HALAANI_GUARD_H))
                ++count;
    }
    return count;
}
...
void OPvPCapturePointNA::SpawnNPCsForTeam(HalaaNPCS teamNPC)
{
    for (int i = 0; i < NA_HALAA_CREATURE_TEAM_SPAWN; i++) {
        ...
        if (data) {
            UpdateCreatureHalaa(
                spawnId, _pvp->GetMap(), data->posX, data->posY);
...
void OPvPCapturePointNA::SpawnGOsForTeam(TeamId teamId)
{
    ...
    for (int i = 0; i < NA_CONTROL_GO_NUM; ++i) {
        if (i == NA_ROOST_S || i == NA_ROOST_W || i == NA_ROOST_N ||
            i == NA_ROOST_E || i == NA_BOMB_WAGON_S || i == NA_BOMB_WAGON_W ||
            i == NA_BOMB_WAGON_N || i == NA_BOMB_WAGON_E)
            continue; // roosts and bomb wagons are spawned when someone uses
                      // the matching destroyed roost
        AddObject(i,
                  gos[i].entry,
                  gos[i].map,
                  gos[i].x,
...
void OPvPCapturePointNA::FactionTakeOver(TeamId teamId)
{
    if (m_ControllingFaction != TEAM_NEUTRAL)
        sGraveyard->RemoveGraveyardLink(NA_HALAA_GRAVEYARD,
    ...
    if (m_ControllingFaction == TEAM_ALLIANCE)
        sWorld->SendZoneText(
            NA_HALAA_GRAVEYARD_ZONE,
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_NA_LOSE_A));
    else if (m_ControllingFaction == TEAM_HORDE)
        sWorld->SendZoneText(
            NA_HALAA_GRAVEYARD_ZONE,
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_NA_LOSE_H));
    DespawnCreatures(GetControllingFaction() == TEAM_HORDE ? halaaNPCHorde
                                                           : halaaNPCAlly);
    m_ControllingFaction = teamId;
    if (m_ControllingFaction != TEAM_NEUTRAL)
        sGraveyard->AddGraveyardLink(NA_HALAA_GRAVEYARD,
                                     NA_HALAA_GRAVEYARD_ZONE,
                                     m_ControllingFaction,
                                     false);
    DespawnGOs();
    SpawnGOsForTeam(teamId);
    SpawnNPCsForTeam(GetControllingFaction() == TEAM_HORDE ? halaaNPCHorde
                                                           : halaaNPCAlly);
    ...
    if (teamId == TEAM_ALLIANCE) {
        ...
        sWorld->SendZoneText(
            NA_HALAA_GRAVEYARD_ZONE,
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_NA_CAPTURE_A));
    }
    else {
        ...
        sWorld->SendZoneText(
            NA_HALAA_GRAVEYARD_ZONE,
            sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_NA_CAPTURE_H));
    }
...
OPvPCapturePointNA::OPvPCapturePointNA(OutdoorPvP* pvp)
    : OPvPCapturePoint(pvp), m_capturable(true), m_GuardsAlive(0),
      m_ControllingFaction(TEAM_NEUTRAL), m_WyvernStateNorth(0),
      m_WyvernStateSouth(0), m_WyvernStateEast(0), m_WyvernStateWest(0),
      m_HalaaState(HALAA_N), m_RespawnTimer(NA_RESPAWN_TIME),
      m_GuardCheckTimer(NA_GUARD_CHECK_TIME), m_canRecap(true)
{
...
{
    //    _typeId = OUTDOOR_PVP_NA; _MUST_ be set in ctor, because of spawns
    //    cleanup
    ...
    // Remove linked graveyard at the server start to avoid players spawning in
    // halaa
    sGraveyard->RemoveGraveyardLink(
        NA_HALAA_GRAVEYARD, NA_HALAA_GRAVEYARD_ZONE, TEAM_ALLIANCE, false);
    sGraveyard->RemoveGraveyardLink(
        NA_HALAA_GRAVEYARD, NA_HALAA_GRAVEYARD_ZONE, TEAM_HORDE, false);
...
void OPvPCapturePointNA::FillInitialWorldStates(WorldPacket& data)
{
    ...
    data << NA_MAP_WYVERN_NORTH_NEU_H
         << uint32(bool(m_WyvernStateNorth & WYVERN_NEU_HORDE));
    data << NA_MAP_WYVERN_NORTH_NEU_A
         << uint32(bool(m_WyvernStateNorth & WYVERN_NEU_ALLIANCE));
    data << NA_MAP_WYVERN_NORTH_H
         << uint32(bool(m_WyvernStateNorth & WYVERN_HORDE));
    data << NA_MAP_WYVERN_NORTH_A
         << uint32(bool(m_WyvernStateNorth & WYVERN_ALLIANCE));

    data << NA_MAP_WYVERN_SOUTH_NEU_H
         << uint32(bool(m_WyvernStateSouth & WYVERN_NEU_HORDE));
    data << NA_MAP_WYVERN_SOUTH_NEU_A
         << uint32(bool(m_WyvernStateSouth & WYVERN_NEU_ALLIANCE));
    data << NA_MAP_WYVERN_SOUTH_H
         << uint32(bool(m_WyvernStateSouth & WYVERN_HORDE));
    data << NA_MAP_WYVERN_SOUTH_A
         << uint32(bool(m_WyvernStateSouth & WYVERN_ALLIANCE));

    data << NA_MAP_WYVERN_WEST_NEU_H
         << uint32(bool(m_WyvernStateWest & WYVERN_NEU_HORDE));
    data << NA_MAP_WYVERN_WEST_NEU_A
         << uint32(bool(m_WyvernStateWest & WYVERN_NEU_ALLIANCE));
    data << NA_MAP_WYVERN_WEST_H
         << uint32(bool(m_WyvernStateWest & WYVERN_HORDE));
    data << NA_MAP_WYVERN_WEST_A
         << uint32(bool(m_WyvernStateWest & WYVERN_ALLIANCE));

    data << NA_MAP_WYVERN_EAST_NEU_H
         << uint32(bool(m_WyvernStateEast & WYVERN_NEU_HORDE));
    data << NA_MAP_WYVERN_EAST_NEU_A
         << uint32(bool(m_WyvernStateEast & WYVERN_NEU_ALLIANCE));
    data << NA_MAP_WYVERN_EAST_H
         << uint32(bool(m_WyvernStateEast & WYVERN_HORDE));
    data << NA_MAP_WYVERN_EAST_A
         << uint32(bool(m_WyvernStateEast & WYVERN_ALLIANCE));
...
bool OutdoorPvPNA::Update(uint32 diff) { return m_obj->Update(diff); }
                                           ...
                                           GameObject* /*go*/)
{
    ...
    if (retval) {
        ...
        InventoryResult msg = player->CanStoreNewItem(
            NULL_BAG, NULL_SLOT, dest, NA_HALAA_BOMB, count, &noSpaceForCount);
...
bool OPvPCapturePointNA::Update(uint32 diff)
{
    ...
    Acore::PlayerListSearcher<Acore::AnyPlayerInObjectRangeCheck> searcher(
        _capturePoint, players, checker);
    Cell::VisitWorldObjects(_capturePoint, searcher, radius);

    for (Player* player : players) {
        if (player->IsOutdoorPvPActive()) {
            if (_activePlayers[player->GetTeamId()]
                    .insert(player->GetGUID())
                    .second)
    ...
    if (m_GuardCheckTimer < diff) {
        ...
        if (cnt != m_GuardsAlive) {
            ...
            if (m_GuardsAlive == 0) {
                ...
                sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE,
                                     sObjectMgr->GetAcoreStringForDBCLocale(
    ...
    if (m_capturable) {
        if (m_RespawnTimer < diff) {
            // if the guards have been killed, then the challenger has one hour
            // to take over halaa. in case they fail to do it, the guards are
            // respawned, and they have to start again.
            ...
            // we reset again the artkit, map icons, sliders and respawn Halaa
            // for controller team
            SendChangePhase();
            ChangeState();
            ...
            return true;
                 ...
                 TEAM_NEUTRAL) // Don't decrease the respawn timer if the team
                               // is not HORDE or ALLIANCE
            m_RespawnTimer -= diff;

        // get the difference of numbers
        float factDiff = ((float)_activePlayers[0].size() -
                          (float)_activePlayers[1].size()) *
                         diff / OUTDOORPVP_OBJECTIVE_UPDATE_INTERVAL;
        if (!factDiff)
            return false;
        ...
        if (factDiff < 0) {
            // horde is in majority, but it's already horde-controlled -> no
            // change
            ...
            // ally is in majority, but it's already ally-controlled -> no
            // change
        ...
        {
            if (_value < -_maxValue) // check if the m_value is lower than max,
                                     // that means horde capped point
            {
                _value = -_maxValue;
                _state = OBJECTIVESTATE_HORDE;
                _team  = TEAM_HORDE;
            ...
            {
                if (_oldState == OBJECTIVESTATE_NEUTRAL ||
                    _oldState == OBJECTIVESTATE_NEUTRAL_HORDE_CHALLENGE ||
            ...
            if (GetControllingFaction() == TEAM_ALLIANCE && !m_canRecap) {
                // When the point goes through neutral, the same faction can
                // recapture again to respawn the guards, still need check
                // blizzlike
                m_canRecap = true;
                DespawnGOs();
                DespawnCreatures(GetControllingFaction() == TEAM_HORDE
                                     ? halaaNPCHorde
                                     : halaaNPCAlly);
            }
        }
        ...
        {
            if (_value > _maxValue) // check if the m_value is bigger than max,
                                    // that means alliance capped point
            {
                _value = _maxValue;
                _state = OBJECTIVESTATE_ALLIANCE;
                _team  = TEAM_ALLIANCE;
...
void OPvPCapturePointNA::ChangeState()
{
    ...
    auto bounds =
        sMapMgr->FindMap(530, 0)->GetGameObjectBySpawnIdStore().equal_range(
...
{
    _pvp->SendUpdateWorldState(NA_MAP_HALAA_NEUTRAL,
                               uint32(bool(m_HalaaState & HALAA_N)));
    _pvp->SendUpdateWorldState(NA_MAP_HALAA_NEU_A,
                               uint32(bool(m_HalaaState & HALAA_N_A)));
    _pvp->SendUpdateWorldState(NA_MAP_HALAA_NEU_H,
                               uint32(bool(m_HalaaState & HALAA_N_H)));
    _pvp->SendUpdateWorldState(NA_MAP_HALAA_HORDE,
                               uint32(bool(m_HalaaState & HALAA_H)));
    _pvp->SendUpdateWorldState(NA_MAP_HALAA_ALLIANCE,
                               uint32(bool(m_HalaaState & HALAA_A)));
}
...
public:
    ...
    OutdoorPvP* GetOutdoorPvP() const override { return new OutdoorPvPNA(); }
};
...
struct outdoorpvp_na_halaa_creatures : public ScriptedAI {
    ...
    {
        if (halaaNPCHorde.size() != NA_HALAA_CREATURE_TEAM_SPAWN &&
            halaaNPCAlly.size() != NA_HALAA_CREATURE_TEAM_SPAWN) {
            std::list<Creature*> creatures;
            uint32               entry = 0;
            for (int i = 0; i < NA_HALAA_CREATURES; i++) {
                me->GetCreatureListWithEntryInGrid(
                    creatures, PatrolCreatureEntry[i].idPatrol, 250);
            }

            if (creatures.size() == NA_HALAA_MAX_CREATURE_SPAWN) {
                for (std::list<Creature*>::iterator itr = creatures.begin();
                     itr != creatures.end();
                     ++itr) {
                    ...
                    if (entry < NA_HALAA_CREATURE_TEAM_SPAWN) {
                        ...
                        halaaNPCAlly[entry - NA_HALAA_CREATURE_TEAM_SPAWN] =
                            c->GetSpawnId();
                    }
                    c->AddObjectToRemoveList();
                    entry++;
                    sObjectMgr->RemoveCreatureFromGrid(c->GetSpawnId(),
                                                       c->GetCreatureData());
                }
            }
        }
