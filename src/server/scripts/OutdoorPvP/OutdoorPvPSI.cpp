...
bool OutdoorPvPSI::Update(uint32 /*diff*/) { return false; }
...
bool OutdoorPvPSI::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case SI_AREATRIGGER_A:
        if (player->GetTeamId() == TEAM_ALLIANCE &&
            player->HasAura(SI_SILITHYST_FLAG)) {
            // remove aura
            ...
            if (m_Gathered_A >= SI_MAX_RESOURCES) {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0],
            ...
            player->GetReputationMgr().ModifyReputation(
                sFactionStore.LookupEntry(609), 20.f);
            // complete quest
...
bool OutdoorPvPSI::HandleDropFlag(Player* player, uint32 spellId)
{
    if (spellId == SI_SILITHYST_FLAG) {
        // if it was dropped away from the player's turn-in point, then create a
        // silithyst mound, if it was dropped near the areatrigger, then it was
        // dispelled by the outdoorpvp, so do nothing
        switch (player->GetTeamId()) {
        case TEAM_ALLIANCE: {
            AreaTrigger const* atEntry =
                sObjectMgr->GetAreaTrigger(SI_AREATRIGGER_A);
            if (atEntry) {
                // 5.0f is safe-distance
                if (player->GetDistance(atEntry->x, atEntry->y, atEntry->z) >
                    5.0f + atEntry->radius) {
                    // he dropped it further, summon mound
                    GameObject* go  = sObjectMgr->IsGameObjectStaticTransport(
                                         SI_SILITHYST_MOUND)
                                          ? new StaticTransport()
                                          : new GameObject();
        ...
        case TEAM_HORDE: {
            AreaTrigger const* atEntry =
                sObjectMgr->GetAreaTrigger(SI_AREATRIGGER_H);
            if (atEntry) {
                // 5.0f is safe-distance
                if (player->GetDistance(atEntry->x, atEntry->y, atEntry->z) >
                    5.0f + atEntry->radius) {
...
public:
    ...
    OutdoorPvP* GetOutdoorPvP() const override { return new OutdoorPvPSI(); }
};

void AddSC_outdoorpvp_si() { new OutdoorPvP_silithus(); }
