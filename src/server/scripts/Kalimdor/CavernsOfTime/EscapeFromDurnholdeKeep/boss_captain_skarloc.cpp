...
enum Text { SAY_ENTER = 0, SAY_TAUNT = 1, SAY_SLAY = 2, SAY_DEATH = 3 };
...
enum Misc { WAYPOINTS_COUNT = 4 };

const Position startPath[WAYPOINTS_COUNT] = {{2008.38f, 281.57f, 65.70f, 0.0f},
...
struct boss_captain_skarloc : public BossAI {
    boss_captain_skarloc(Creature* creature)
        : BossAI(creature, DATA_CAPTAIN_SKARLOC), summons(me)
    {
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        if (Creature* thrall = ObjectAccessor::GetCreature(
                *me, me->GetInstanceScript()->GetGuidData(DATA_THRALL_GUID))) {
        ...
        if (summons.size() == 1) {
            summon->GetMotionMaster()->MovePoint(
                0, 2060.788f, 237.301f, 63.999f);
        }
        else {
            summon->GetMotionMaster()->MovePoint(
                0, 2056.870f, 234.853f, 63.839f);
        }
    }
    ...
    void InitializeAI() override
    {
        ...
        for (uint8 i = 0; i < WAYPOINTS_COUNT; ++i) {
            path.push_back(G3D::Vector3(startPath[i].GetPositionX(),
                                        startPath[i].GetPositionY(),
                                        startPath[i].GetPositionZ()));
        }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        _JustDied();
        Talk(SAY_DEATH);
        me->GetInstanceScript()->SetData(DATA_ESCORT_PROGRESS,
