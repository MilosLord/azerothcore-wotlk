...
enum MiscIds {
    ...
    // ENCOUNTER_PROGRESS_DRAKE_KILLED     = 2, No longer used. Kept as
    // reference as DB might rely on the existing order.
...
#define RegisterOldHillsbradCreatureAI(ai_name)                                \
    RegisterCreatureAIWithFactory(ai_name, GetOldHillsbradAI)
