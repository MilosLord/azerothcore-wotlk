    ...
    struct instance_old_hillsbrad_InstanceMapScript : public InstanceScript {
        ...
        void OnPlayerEnter(Player* player) override
        {
            ...
            if (_encounterProgress < ENCOUNTER_PROGRESS_BARRELS)
                player->SendUpdateWorldState(WORLD_STATE_BARRELS_PLANTED,
                                             _barrelCount);
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_THRALL_REPOSITION:
                if (data > 1)
                    _events.ScheduleEvent(EVENT_THRALL_REPOSITION,
                                          data == 2 ? 0 : 10000);
            ...
            case DATA_BOMBS_PLACED: {
                if (_barrelCount >= 5 ||
                    _encounterProgress > ENCOUNTER_PROGRESS_NONE)
                    return;
        ...
        void Update(uint32 diff) override
        {
            ...
            case EVENT_INITIAL_BARRELS_FLAME: {
                instance->LoadGrid(instancePositions[0].GetPositionX(),
                                   instancePositions[0].GetPositionY());
                instance->LoadGrid(instancePositions[1].GetPositionX(),
                                   instancePositions[1].GetPositionY());

                for (ObjectGuid const& guid : _prisonersSet)
                    if (Creature* orc = instance->GetCreature(guid)) {
                        uint8 index =
                            orc->GetDistance(instancePositions[0]) < 80.0f ? 0
                                                                           : 1;
                        Position pos(instancePositions[index]);
                        orc->MovePosition(
                            pos,
                            frand(1.0f, 3.0f) + 15.0f * (float)rand_norm(),
            ...
            case EVENT_SUMMON_LIEUTENANT: {
                instance->LoadGrid(instancePositions[2].GetPositionX(),
                                   instancePositions[2].GetPositionY());
                instance->SummonCreature(NPC_LIEUTENANT_DRAKE,
                                         instancePositions[2]);
                break;
            }
        ...
        void Reposition(Creature* thrall)
        {
            ...
            case ENCOUNTER_PROGRESS_TARETHA_MEET:
                thrall->UpdatePosition(
                    thrallPositions[data - ENCOUNTER_PROGRESS_THRALL_ARMORED],
                    true);
                thrall->SetHomePosition(
                    thrallPositions[data - ENCOUNTER_PROGRESS_THRALL_ARMORED]);
                thrall->SetFacingTo(
                    thrallPositions[data - ENCOUNTER_PROGRESS_THRALL_ARMORED]
                        .GetOrientation());
        ...
        void EnsureGridLoaded()
        {
            for (uint8 i = 0; i < THRALL_POSITIONS_COUNT; ++i)
                instance->LoadGrid(thrallPositions[i].GetPositionX(),
                                   thrallPositions[i].GetPositionY());
        }
...
void AddSC_instance_old_hillsbrad() { new instance_old_hillsbrad(); }
