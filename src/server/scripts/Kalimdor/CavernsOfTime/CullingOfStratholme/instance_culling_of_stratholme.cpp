...
public:
    instance_culling_of_stratholme()
        : InstanceMapScript("instance_culling_of_stratholme", 595)
    {
    }
    ...
    struct instance_culling_of_stratholme_InstanceMapScript
        ...
        void OnPlayerEnter(Player* plr) override
        {
            ...
            if (plr->getRace() != RACE_HUMAN && plr->getRace() != RACE_DWARF &&
                plr->getRace() != RACE_GNOME)
                plr->CastSpell(plr,
                               ((plr->getGender() == GENDER_MALE)
                                    ? SPELL_HUMAN_MALE
                                    : SPELL_HUMAN_FEMALE),
                               true);
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_SHOW_INFINITE_TIMER:
                ...
                DoUpdateWorldState(
                    ...
                    uint32(_guardianTimer / (MINUTE * IN_MILLISECONDS)));
                if (data == 0) {
                    _guardianTimer = 0;
                    SaveToDB();
                }
                else if (!_infiniteGUID)
                    instance->SummonCreature(NPC_INFINITE,
                                             EventPos[EVENT_SRC_CORRUPTOR]);
                return;
                ...
                if (_crateCount == 5) {
                    ...
                    if (!PlayerList.IsEmpty())
                        for (Map::PlayerList::const_iterator i =
                             ...
                             i != PlayerList.end();
                             ++i)
                            i->GetSource()->KilledMonsterCredit(
        ...
        void UpdateEventState()
        {
            if (_encounterState > COS_PROGRESS_NOT_STARTED) {
                ...
                instance->SummonCreature(NPC_CHROMIE_MIDDLE,
                                         EventPos[EVENT_POS_CHROMIE]);
                instance->SummonCreature(NPC_HOURGLASS,
                                         EventPos[EVENT_POS_HOURGLASS]);
        ...
        void ChromieWhisper(uint8 textId)
        {
            if (!instance->GetPlayers().IsEmpty())
                if (Player* player =
                        instance->GetPlayers().getFirst()->GetSource()) {
                    Position pos = player->GetPosition();
                    if (Creature* cr =
                            instance->SummonCreature(NPC_CHROMIE_MIDDLE, pos)) {
                        cr->SetVisible(false);
                        cr->DespawnOrUnsummon(1000);
                        sCreatureTextMgr->SendChat(cr,
                                                   textId,
                                                   player,
                                                   CHAT_MSG_ADDON,
                                                   LANG_ADDON,
        ...
        {
            instance->LoadGrid(LeaderIntroPos1.GetPositionX(),
                               LeaderIntroPos1.GetPositionY());
            instance->LoadGrid(LeaderIntroPos2.GetPositionX(),
                               LeaderIntroPos2.GetPositionY());
            instance->LoadGrid(LeaderIntroPos3.GetPositionX(),
                               LeaderIntroPos3.GetPositionY());
            instance->LoadGrid(LeaderIntroPos4.GetPositionX(),
                               LeaderIntroPos4.GetPositionY());
            instance->LoadGrid(LeaderIntroPos5.GetPositionX(),
                               LeaderIntroPos5.GetPositionY());
            instance->LoadGrid(LeaderIntroPos6.GetPositionX(),
                               LeaderIntroPos6.GetPositionY());
        }
