...
public:
    ...
    struct boss_mal_ganisAI : public ScriptedAI {
        boss_mal_ganisAI(Creature* c) : ScriptedAI(c) { finished = false; }
            ...
            if (!finished && damage >= me->GetHealth()) {
                ...
                if (InstanceScript* pInstance = me->GetInstanceScript()) {
                    if (Creature* cr = ObjectAccessor::GetCreature(
                            *me, pInstance->GetGuidData(DATA_ARTHAS)))
                ...
                if (who)
                    if (Player* player =
                        ...
                        player->RewardPlayerAndGroupAtEvent(
                            31006,
                            player); // Royal Escort quest, Mal'ganis bunny
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_CARRION_SWARM:
                me->CastSpell(
                    me->GetVictim(),
                    DUNGEON_MODE(SPELL_CARRION_SWARM_N, SPELL_CARRION_SWARM_H),
                    false);
            ...
            case EVENT_SPELL_MIND_BLAST:
                if (Unit* target = SelectTarget(
                    ...
                    me->CastSpell(
                        target,
                        DUNGEON_MODE(SPELL_MIND_BLAST_N, SPELL_MIND_BLAST_H),
                        false);
            ...
            case EVENT_SPELL_SLEEP:
                ...
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 50.0f, true))
                    me->CastSpell(target,
                                  DUNGEON_MODE(SPELL_SLEEP_N, SPELL_SLEEP_H),
                                  false);
...
void AddSC_boss_mal_ganis() { new boss_mal_ganis(); }
