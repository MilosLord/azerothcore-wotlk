...
ObjectData const creatureData[] = {{NPC_WINTERCHILL, DATA_WINTERCHILL},
...
Milliseconds hyjalNightElfWaveTimers[1][MAX_WAVES_NIGHT_ELF]{{0ms}};
    ...
    struct instance_mount_hyjal_InstanceMapScript : public InstanceScript {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_INFER:
                ...
                if (creature->IsSummon() && _bossWave) {
                    DoUpdateWorldState(WORLD_STATE_ENEMYCOUNT,
                                       ++trash); // Update the instance wave
                                                 // count on new trash spawn
                    _encounterNPCs.insert(
                        creature->GetGUID()); // Used for despawning on wipe
                }
                break;
            }
                    ...
                    if (unit->ToCreature()->IsSummon()) {
                        if (_bossWave) {
                            DoUpdateWorldState(
                                ...
                                --trash); // Update the instance wave count on
                                          // new trash death
                            _encounterNPCs.erase(
                                unit->ToCreature()
                                    ->GetGUID()); // Used for despawning on wipe

                            if (trash ==
                                0) // It can reach negatives if trash spawned
                                   // after a retreat are killed, it shouldn't
                                   // affect anything. Also happens on retail
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_ALLIANCE_RETREAT:
                ...
                // Move all alliance NPCs near Jaina (only happens in this base,
                // not Horde's)
                if (Creature* jaina = GetCreature(DATA_JAINA)) {
                    for (ObjectGuid const& guid : _baseAlliance) {
                        if (instance->GetCreature(guid) &&
                            instance->GetCreature(guid)->IsAlive()) {
                            float x, y, z;
                            jaina->GetNearPoint(
                                instance->GetCreature(guid),
                                x,
                                y,
                                z,
                                10.f,
                                0,
                                jaina->GetAngle(instance->GetCreature(guid)));
            ...
            case DATA_SPAWN_WAVES:
                ...
                if (GetBossState(DATA_WINTERCHILL) != DONE) {
                    ...
                    ScheduleWaves(1ms,
                                  ...
                                  MAX_WAVES_STANDARD,
                                  hyjalWaveTimers[DATA_WINTERCHILL - 1]);
                }
                    ...
                    ScheduleWaves(1ms,
                                  ...
                                  MAX_WAVES_STANDARD,
                                  hyjalWaveTimers[DATA_ANETHERON - 1]);
                }
                    ...
                    ScheduleWaves(1ms,
                                  START_WAVE_KAZROGAL,
                                  MAX_WAVES_STANDARD,
                                  hyjalWaveTimers[DATA_KAZROGAL - 1]);
                }
                    ...
                    ScheduleWaves(1ms,
                                  START_WAVE_AZGALOR,
                                  MAX_WAVES_STANDARD,
                                  hyjalWaveTimers[DATA_AZGALOR - 1]);
                }
                    ...
                    if (Creature* target = instance->GetCreature(guid)) {
                        if (doubleInfernalCount > 0) {
                            target->AI()->SetData(
                                ...
                                2); // Spawns 2 infernals, as there are only 6
                                    // spawns, some summon 2
                            doubleInfernalCount--;
                        }
                        else
            ...
            // LOG_DEBUG("scripts", "Instance Hyjal: Instance data updated for
        ...
        void ScheduleWaves(Milliseconds /* time */,
                           uint8        startWaves,
                           uint8        maxWaves,
                           Milliseconds timerptr[])
        {
            // No overlapping!
            _scheduler.CancelGroup(CONTEXT_GROUP_WAVES);
            trash = 0; // Reset counter here to avoid resetting the counter from
                       // scheduled waves. Required because creatures killed for
                       // RP events counts towards the kill counter as well,
                       // confirmed in Retail.

            _scheduler.Schedule(
                1ms,
                [this, startWaves, maxWaves, timerptr](TaskContext context) {
                    ...
                    instance->SummonCreatureGroup(
                        startWaves +
                        _currentWave); // _currentWave should be 0 when this
                                       // function is first called

                    // Check if it's time to summon Infernals
                    if (GetBossState(DATA_KAZROGAL) == DONE &&
                        GetBossState(DATA_AZGALOR) != DONE) {
        ...
        void Update(uint32 diff) override { _scheduler.Update(diff); }
...
void AddSC_instance_mount_hyjal() { new instance_hyjal(); }
