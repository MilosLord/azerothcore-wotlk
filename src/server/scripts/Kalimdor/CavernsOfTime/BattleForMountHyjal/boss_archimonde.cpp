        ...
        ScheduleTimedEvent(
            ...
            [&] {
                if (Creature* archimonde =
                        _instance->GetCreature(DATA_ARCHIMONDE)) {
                    if (archimonde->HealthBelowPct(2) ||
                        !archimonde->IsAlive()) {
...
struct npc_doomfire : public ScriptedAI {
    ...
    void Reset() override { _summons.DespawnAll(); }
...
struct boss_archimonde : public BossAI {
    ...
    void Reset() override
    {
        ...
        if (Map* map = me->GetMap()) {
            map->DoForAllPlayers([&](Player* player) {
                player->ApplySpellImmune(SPELL_HAND_OF_DEATH,
                                         IMMUNITY_ID,
                                         SPELL_HAND_OF_DEATH,
                                         false);
                player->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_HAND_OF_DEATH, false);
            });
        }
            ...
            ScheduleTimedEvent(
                ...
                [&] {
                    ...
                    Position wispPosition = {
                        me->GetPositionX() + float(rand() % WISP_OFFSET),
                        me->GetPositionY() + float(rand() % WISP_OFFSET),
                        me->GetPositionZ(),
                        0.0f};
                    if (Creature* wisp = me->SummonCreature(
                            CREATURE_ANCIENT_WISP, wispPosition)) {
    ...
    void DoAction(int32 action) override
    {
        ...
        case ACTION_BECOME_ACTIVE_AND_CHANNEL:
            ...
            me->SetVisible(true);
            if (!_isChanneling) {
                if (Creature* nordrassil =
                        me->SummonCreature(CREATURE_CHANNEL_TARGET,
                                           nordrassilPosition,
                    ...
                    _isChanneling = true;
                    nordrassil->AI()->DoCast(
                        me, SPELL_DRAIN_WORLD_TREE_2, true);
                }
            }
            break;
        }
        ...
        if (Map* map = me->GetMap()) {
            map->DoForAllPlayers([&](Player* player) {
                ...
                player->ApplySpellImmune(SPELL_HAND_OF_DEATH,
                                         ...
                                         true);
                player->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_HAND_OF_DEATH, true);
            });
        }
    }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        ScheduleTimedEvent(
            ...
            20s);
        ScheduleTimedEvent(
            25s, 35s, [&] { DoCastVictim(SPELL_FEAR); }, 42s);
        ScheduleTimedEvent(
            25s,
            35s,
        ...
        ScheduleTimedEvent(
            ...
            [&] {
                ...
                if (Map* map = me->GetMap()) {
                    map->DoForAllPlayers(
                        [&noPlayersInRange, this](Player* player) {
    ...
    void GainSoulCharge(Player* player)
    {
        ...
        scheduler.Schedule(
            2s, 10s, [this](TaskContext) { UnleashSoulCharge(); });
    }
    ...
    void JustSummoned(Creature* summoned) override
    {
        ...
        if (summoned->GetEntry() == CREATURE_DOOMFIRE) {
            ...
            summoned->CastSpell(
                summoned, SPELL_DOOMFIRE, true, 0, 0, me->GetGUID());
        }
    }
    ...
    void DoCastDoomFire(Unit* target)
    {
        ...
        Talk(SAY_DOOMFIRE);
        Position spiritPosition   = {target->GetPositionX() + DOOMFIRE_OFFSET,
                                     target->GetPositionY() + DOOMFIRE_OFFSET,
                                     target->GetPositionZ(),
                                     0.0f};
        Position doomfirePosition = {target->GetPositionX() - DOOMFIRE_OFFSET,
                                     target->GetPositionY() - DOOMFIRE_OFFSET,
                                     target->GetPositionZ(),
                                     0.0f};
        if (Unit* doomfireSpirit = me->SummonCreature(CREATURE_DOOMFIRE_SPIRIT,
                                                      spiritPosition,
                                                      TEMPSUMMON_TIMED_DESPAWN,
                                                      27000)) {
            if (Unit* doomfire = me->SummonCreature(CREATURE_DOOMFIRE,
                                                    doomfirePosition,
                                                    TEMPSUMMON_TIMED_DESPAWN,
                                                    27000)) {
    ...
    void UnleashSoulCharge()
    {
        ...
        if (!_availableAuras.empty() && !_availableSpells.empty()) {
            ...
            if (urand(0, 1)) {
                _availableAuras.insert(_availableAuras.begin(),
                                       _availableAuras.back());
                _availableAuras.pop_back();
                _availableSpells.insert(_availableSpells.begin(),
                                        _availableSpells.back());
...
class spell_red_sky_effect : public SpellScript {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(spell_red_sky_effect::HandleHit,
                                           EFFECT_0,
...
class spell_finger_of_death : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_finger_of_death::HandleHit,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_hand_of_death : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_hand_of_death::HandleHit,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
