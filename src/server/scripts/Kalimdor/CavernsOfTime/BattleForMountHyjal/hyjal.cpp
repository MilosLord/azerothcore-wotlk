...
public:
    ...
    struct hyjalJainaAI : public ScriptedAI {
        ...
        {
            me->ApplySpellImmune(SPELL_DEATH_AND_DECAY,
                                 IMMUNITY_ID,
                        ...
                        uint32 /*action*/) override
    {
        ...
        if (InstanceScript* hyjal = creature->GetInstanceScript()) {
            if (hyjal->GetBossState(DATA_WINTERCHILL) != DONE ||
                hyjal->GetBossState(DATA_ANETHERON) != DONE) {
                hyjal->SetData(DATA_RESET_WAVES, 0);
                hyjal->SetData(DATA_SPAWN_WAVES, 0);
            }
            else {
                creature->AI()->Talk(SAY_SUCCESS);
                creature->GetMotionMaster()->MovePath(JAINA_RETREAT_PATH,
                                                      false);
            }
        }
        return true;
...
public:
                        ...
                        uint32 /*action*/) override
    {
        ...
        if (InstanceScript* hyjal = creature->GetInstanceScript()) {
            if (hyjal->GetBossState(DATA_KAZROGAL) != DONE ||
                hyjal->GetBossState(DATA_AZGALOR) != DONE) {
                hyjal->SetData(DATA_RESET_WAVES, 0);
                hyjal->SetData(DATA_SPAWN_WAVES, 0);
            }
            else {
...
struct npc_hyjal_ground_trash : public ScriptedAI {
        ...
        case NPC_NECRO: {
            scheduler
                ...
                .Schedule(5s,
                          10s,
                          [this](TaskContext context) {
                              // TODO: Should target corpse, and skeletons
                              // should spawn at the target
                              switch (urand(1, 3)) {
                              case 1:
                ...
                .Schedule(15s, 20s, [this](TaskContext context) {
                    if (Creature* target = GetNearbyFriendlyTrashCreature(
                            UNHOLY_FRENZY_RANGE)) {
        ...
        case NPC_STALK: {
            scheduler.Schedule(3s, 6s, [this](TaskContext context) {
                if (Unit* target = SelectTarget(
                        ...
                        0,
                        PowerUsersSelector(me, Powers(POWER_MANA), 30.f, true)))
    ...
    void DoAction(int32 action) override
    {
        ...
        case DATA_ALLIANCE_RETREAT:
            me->GetMotionMaster()->MovePath(
                urand(ALLIANCE_BASE_CHARGE_1, ALLIANCE_BASE_CHARGE_3), false);
            break;
        case DATA_KAZROGAL:
        case DATA_AZGALOR:
        case DATA_HORDE_RETREAT:
            me->GetMotionMaster()->MovePath(
                urand(HORDE_BASE_CHARGE_1, HORDE_BASE_CHARGE_3), false);
            break;
        case DATA_ARCHIMONDE:
            me->GetMotionMaster()->MovePath(
                urand(NIGHT_ELF_BASE_CHARGE_1, NIGHT_ELF_BASE_CHARGE_3), false);
            break;
        }
    }
    ...
    void PathEndReached(uint32 pathId) override
    {
        ...
        case ALLIANCE_BASE_CHARGE_3:
            me->m_Events.AddEventAtOffset(
                [this]() {
                    me->GetMotionMaster()->MovePath(
                        urand(ALLIANCE_BASE_PATROL_1, ALLIANCE_BASE_PATROL_3),
                        true);
                },
                1s);
            break;
        ...
        case HORDE_BASE_CHARGE_3:
            me->m_Events.AddEventAtOffset(
                [this]() {
                    me->GetMotionMaster()->MovePath(
                        urand(HORDE_BASE_PATROL_1, HORDE_BASE_PATROL_3), true);
                },
                1s);
            break;
        ...
        case NIGHT_ELF_BASE_CHARGE_3:
            me->m_Events.AddEventAtOffset(
                [this]() { me->GetMotionMaster()->MoveRandom(5.f); }, 1s);
            break;
        }
    }
    ...
    Creature* GetNearbyFriendlyTrashCreature(float radius)
    {
        ...
        GetCreatureListWithEntryInGrid(
            creatureList, me, NPC_SKELETON_INVADER, radius);
...
struct npc_hyjal_gargoyle : public ScriptedAI {
    ...
    void DoAction(int32 action) override
    {
        ...
        case DATA_HORDE_RETREAT:
            if (me->GetPositionX() < 5500.f)
                me->GetMotionMaster()->MovePath(
                    urand(GARGOYLE_PATH_FORTRESS_1, GARGOYLE_PATH_FORTRESS_3),
                    false);
            else
                me->GetMotionMaster()->MovePath(
    ...
    void PathEndReached(uint32 /* pathId */) override
    {
        ...
        me->m_Events.AddEventAtOffset(
            [this]() { me->GetMotionMaster()->MoveRandom(30.f); }, 1s);
    }
...
struct npc_hyjal_frost_wyrm : public ScriptedAI {
    ...
    void PathEndReached(uint32 pathId) override
    {
        if (pathId == FROST_WYRM_FORTRESS) {
            me->m_Events.AddEventAtOffset(
                [this]() {
                    me->GetMotionMaster()->MovePath(FROST_WYRM_FORTRESS_PATROL,
                                                    true);
                },
                1s);
        }
