...
static std::vector<uint32> firstWave = {
    NPC_INFINITE_ASSASSIN, NPC_INFINITE_WHELP, NPC_INFINITE_CHRONOMANCER};
static std::vector<uint32> secondWave = {NPC_INFINITE_EXECUTIONER,
                                         NPC_INFINITE_CHRONOMANCER,
                                         NPC_INFINITE_WHELP,
                                         NPC_INFINITE_ASSASSIN};
static std::vector<uint32> thirdWave  = {NPC_INFINITE_EXECUTIONER,
...
struct npc_medivh_bm : public ScriptedAI {
    npc_medivh_bm(Creature* creature) : ScriptedAI(creature)
    {
        ...
        _groundArray.push_back(G3D::Vector3(creature->GetPositionX() + 8.0f,
                                            creature->GetPositionY(),
                                            creature->GetPositionZ()));
        _airArray.push_back(G3D::Vector3(creature->GetPositionX(),
                                         creature->GetPositionY(),
                                         creature->GetPositionZ()));

        for (uint8 i = 0; i < 10; ++i) {
            _groundArray.push_back(G3D::Vector3(
                creature->GetPositionX() + 8.0f * cos(2.0f * M_PI * i / 10.0f),
                creature->GetPositionY() +
                    8.0f * std::sin(2.0f * M_PI * i / 10.0f),
                creature->GetPositionZ()));
        }

        for (uint8 i = 0; i < 40; ++i) {
            _airArray.push_back(
                G3D::Vector3(creature->GetPositionX() +
                                 i * 0.25f * cos(2.0f * M_PI * i / 10.0f),
                             creature->GetPositionY() +
                                 i * 0.25f * std::sin(2.0f * M_PI * i / 10.0f),
                             creature->GetPositionZ() + i / 4.0f));
        }

        for (uint8 i = 40; i < 80; ++i) {
            _airArray.push_back(G3D::Vector3(
                creature->GetPositionX() + 10.0f * cos(2.0f * M_PI * i / 10.0f),
                creature->GetPositionY() +
                    10.0f * std::sin(2.0f * M_PI * i / 10.0f),
                creature->GetPositionZ() + i / 4.0f));
        }
    }
    ...
    void JustSummoned(Creature* summon) override
    {
        if (summon->GetEntry() == NPC_DP_CRYSTAL_STALKER) {
            ...
            summon->CastSpell(
                summon, RAND(SPELL_BANISH_PURPLE, SPELL_BANISH_GREEN), true);
    ...
    void DoAction(int32 param) override
    {
        if (param == ACTION_OUTRO) {
            ...
            me->SummonGameObject(GO_DARK_PORTAL,
                                 -2086.0f,
                                 7125.6215f,
                                 30.5f,
                                 6.148f,
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_SUMMON_CRYSTAL:
            me->SummonCreature(NPC_DP_EMITTER_STALKER,
                               me->GetPositionX() + 8.0f,
                               me->GetPositionY(),
                               me->GetPositionZ());
            break;
        ...
        case EVENT_OUTRO_8:
            if (Creature* cr = me->FindNearestCreature(
                    NPC_SHADOW_COUNCIL_ENFORCER, 20.0f)) {
    ...
    void
    SummonOrcs(float x, float y, uint32 duration, uint32 homeTime, bool first)
    {
        for (uint8 i = 0; i < 6; ++i) {
            if (Creature* cr = me->SummonCreature(NPC_SHADOW_COUNCIL_ENFORCER,
                                                  -2091.731f,
                                                  7133.083f - 3.0f * i,
                                                  34.589f,
                                                  0.0f)) {
                cr->GetMotionMaster()->MovePoint(
                    ...
                    cr->GetPositionY() + y,
                    cr->GetMapHeight(
                        x, cr->GetPositionY() + y, cr->GetPositionZ(), true));
                cr->m_Events.AddEvent(
                    new NpcRunToHome(*cr),
                    cr->m_Events.CalculateTime(homeTime + urand(0, 2000)));
...
struct npc_time_rift : public NullCreatureAI {
    ...
    void DoSummonAtRift(uint32 entry)
    {
        ...
        if (Creature* summon = me->SummonCreature(
                entry, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 150000)) {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_SUMMON_BOSS: {
            ...
            case 6:
                entry =
                    _instance->GetBossState(DATA_CHRONO_LORD_DEJA) == DONE
                        ? (me->GetMap()->IsHeroic() ? NPC_INFINITE_CHRONO_LORD
                                                    : -NPC_CHRONO_LORD_DEJA)
                        : NPC_CHRONO_LORD_DEJA;
                break;
            case 12:
                entry =
                    _instance->GetBossState(DATA_TEMPORUS) == DONE
                        ? (me->GetMap()->IsHeroic() ? NPC_INFINITE_TIMEREAVER
                                                    : -NPC_TEMPORUS)
                        : NPC_TEMPORUS;
                break;
            case 18:
                ...
                break;
            default:
                entry = RAND(NPC_RIFT_KEEPER_WARLOCK,
            ...
            if (Creature* summon =
                    me->SummonCreature(std::abs(entry),
                                       pos,
...
class spell_black_morass_corrupt_medivh : public AuraScript {
    ...
    void PeriodicTick(AuraEffect const* /*aurEff*/)
    {
        if (++_ticks >= 3) {
            ...
            if (InstanceScript* instance =
                    GetUnitOwner()->GetInstanceScript()) {
                instance->SetData(DATA_DAMAGE_SHIELD,
                                  m_scriptSpellId == SPELL_CORRUPT_AEONUS ? 2
                                                                          : 1);
            }
        }
    }
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_black_morass_corrupt_medivh::PeriodicTick,
            EFFECT_0,
