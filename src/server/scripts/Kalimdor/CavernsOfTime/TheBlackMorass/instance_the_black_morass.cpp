...
ObjectData const creatureData[] = {{NPC_MEDIVH, DATA_MEDIVH}, {0, 0}};

class instance_the_black_morass : public InstanceMapScript {
public:
    instance_the_black_morass()
        : InstanceMapScript("instance_the_black_morass", 269)
    {
    }
    ...
    struct instance_the_black_morass_InstanceMapScript : public InstanceScript {
            ...
            if (state == DONE) {
                ...
                case DATA_AEONUS: {
                    ...
                    instance->DoForAllPlayers([&](Player* player) {
                        if (player->GetQuestStatus(QUEST_OPENING_PORTAL) ==
                            QUEST_STATUS_INCOMPLETE) {
                            player->AreaExploredOrEventHappens(
                                QUEST_OPENING_PORTAL);
                        }

                        if (player->GetQuestStatus(QUEST_MASTER_TOUCH) ==
                            QUEST_STATUS_INCOMPLETE) {
                            player->AreaExploredOrEventHappens(
                                QUEST_MASTER_TOUCH);
                        }
        ...
        void ScheduleNextPortal(Milliseconds time, Position lastPosition)
        {
            ...
            _scheduler.Schedule(
                time, [this, lastPosition](TaskContext context) {
                    if (GetCreature(DATA_MEDIVH)) {
                        // Spawning prevented: after-boss-delay or event
                        // failed/not started or last portal spawned
                        if (!_noBossSpawnDelay ||
                            _eventStatus == EVENT_PREPARE ||
                            _currentRift >= 18) {
                            return;
                        }
                        ...
                        if (!_availableRiftPositions.empty()) {
                            ...
                            DoUpdateWorldState(WORLD_STATE_BM_RIFT,
                                               ++_currentRift);

                            instance->SummonCreature(NPC_TIME_RIFT, spawnPos);

                            // queue next portal if group doesn't kill keepers
                            // fast enough
                            context.Repeat((_currentRift >= 13 ? 2min : 90s));
                        }
                        // if no rift positions are available, the next rift
                        // will be scheduled in OnCreatureRemove
                    }
        ...
        void OnCreatureRemove(Creature* creature) override
        {
            switch (creature->GetEntry()) {
            case NPC_TIME_RIFT:
                if (_currentRift < 18 && _noBossSpawnDelay &&
                    _eventStatus == EVENT_IN_PROGRESS) {
                    ...
                    if (Creature* medivh = GetCreature(DATA_MEDIVH)) {
                        if (medivh->IsAlive() && medivh->IsAIEnabled) {
                            ...
                            _scheduler.Schedule(4s, [this](TaskContext) {
                                if (Creature* medivh =
                                        GetCreature(DATA_MEDIVH)) {
                                    medivh->RemoveAllAuras();
                                }

                                // Step 2 - Medivh dies and visual effect NPCs
                                // are despawned.
                                _scheduler.Schedule(500ms, [this](TaskContext) {
                                    ...
                                    _scheduler.Schedule(
                                        2s, [this](TaskContext) {
                                            ...
                                            for (ObjectGuid const& guid :
                                                 encounterNPCSCopy) {
                                                // Don't despawn permanent
                                                // visual effect NPC twice or it
                                                // won't respawn correctly
                                                if (guid.GetEntry() ==
                                                    NPC_DP_BEAM_STALKER) {
                                                    continue;
                                                }
                                            ...
                                            // Step 4 - Schedule instance
                                            // cleanup without player
                                            // interaction
                                            _scheduler.Schedule(
                                                300s, [this](TaskContext) {
                                                    CleanupInstance();
...
void AddSC_instance_the_black_morass() { new instance_the_black_morass(); }
