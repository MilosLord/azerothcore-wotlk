...
public:
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        instance->DoStopTimedAchievement(
            ACHIEVEMENT_TIMED_TYPE_EVENT,
            ACHIEV_TIMED_START_EVENT); // just in case at reset some players
                                       // already left the instance
        instance->DoStartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_EVENT,
                                          ACHIEV_TIMED_START_EVENT);
        BossAI::JustEngagedWith(who);

        me->SummonCreature(NPC_ONYXIAN_LAIR_GUARD,
                           -167.837936f,
                           -200.549332f,
                           -66.343231f,
                           5.598287f,
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        if (summon->GetEntry() != NPC_ONYXIAN_WHELP &&
            summon->GetEntry() != NPC_ONYXIAN_LAIR_GUARD) {
            return;
        }

        if (summon->GetEntry() == NPC_ONYXIAN_LAIR_GUARD &&
            Phase < PHASE_AIRPHASE) {
            return;
        }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        if (id < 9) {
            ...
            case 13:
                ...
                me->SetDisableGravity(false);
                me->SetSpeed(
                    MOVE_RUN, me->GetCreatureTemplate()->speed_run, false);
        ...
        if (whelpSpam) {
            if (whelpCount < 40) {
                ...
                if (whelpSpamTimer <= 0) {
                    ...
                    me->CastSpell(-33.18f + cos(angle) * dist,
                                  -258.80f + std::sin(angle) * dist,
                                  -89.0f,
                                  17646,
                                  true);
                    me->CastSpell(-32.535f + cos(angle) * dist,
                                  -170.190f + std::sin(angle) * dist,
                                  -89.0f,
                                  17646,
                                  true);
                    whelpCount += 2;
    ...
    bool CheckInRoom() override
    {
        if (me->GetDistance2d(me->GetHomePosition().GetPositionX(),
                              me->GetHomePosition().GetPositionY()) > 95.0f) {
            Talk(SAY_EVADE);
            EnterEvadeMode();
            return false;
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_START_PHASE_2: {
            ...
            me->StopMoving();
            DoResetThreatList();
            me->GetMotionMaster()->MovePoint(10,
                                             OnyxiaMoveData[0].x,
                                             OnyxiaMoveData[0].y,
                                             OnyxiaMoveData[0].z);
            break;
        ...
        case EVENT_SUMMON_LAIR_GUARD: {
            me->CastSpell(
                -101.654f, -214.491f, -80.70f, SPELL_SUMMON_LAIR_GUARD, true);
        ...
        case EVENT_SPELL_FIREBALL_FIRST: {
            if (Unit* v =
                    SelectTarget(SelectTargetMethod::Random, 0, 200.0f, true)) {
        ...
        case EVENT_PHASE_2_STEP_CW: {
            ...
            me->GetMotionMaster()->MovePoint(newWP,
                                             OnyxiaMoveData[newWP].x,
                                             OnyxiaMoveData[newWP].y,
                                             OnyxiaMoveData[newWP].z);
            break;
        }
        ...
        case EVENT_START_PHASE_3: {
            me->SetSpeed(MOVE_RUN, 2.95f, false);
            me->GetMotionMaster()->MovePoint(12,
                                             OnyxiaMoveData[1].x,
                                             OnyxiaMoveData[1].y,
                                             OnyxiaMoveData[1].z);
            break;
        ...
        case EVENT_PHASE_3_ATTACK: {
            ...
            if (Unit* target =
                    SelectTarget(SelectTargetMethod::MaxThreat, 0, 0, false)) {
                AttackStart(target);
            }
        ...
        case EVENT_ERUPTION: {
            if (Creature* trigger = me->SummonCreature(
                    12758, *me, TEMPSUMMON_TIMED_DESPAWN, 1000)) {
    ...
    void SpellHitTarget(Unit* target, SpellInfo const* spell) override
    {
        if (target->IsPlayer() && spell->DurationEntry &&
            spell->DurationEntry->ID == 328 &&
            spell->Effects[EFFECT_1].TargetA.GetTarget() == 1 &&
            (spell->Effects[EFFECT_1].Amplitude == 50 ||
             spell->Effects[EFFECT_1].Amplitude == 215)) // Deep Breath
        {
