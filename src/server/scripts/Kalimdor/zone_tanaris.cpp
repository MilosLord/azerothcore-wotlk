...
SDName: Tanaris
SD%Complete: 80
SDComment: Quest support: 1560, 2954, 4005, 10277, 10279(Special flight path).
Noggenfogger vendor SDCategory: Tanaris EndScriptData */
        ...
        void SendItem(Unit* receiver)
        {
            ...
            if (player && player->HasItemCount(11169, 1, false) &&
                ...
                uint8           msg = player->CanStoreNewItem(
                    NULL_BAG, NULL_SLOT, dest, 11522, 1, nullptr);
                if (msg == EQUIP_ERR_OK)
                    player->StoreNewItem(dest, 11522, true);
            }
        }

        void JustEngagedWith(Unit* who) override { Talk(AGGRO_YELL_AQUE, who); }
    ...
    struct npc_custodian_of_timeAI : public npc_escortAI {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            if (Player* player = GetPlayerForEscort()) {
                ...
                case 24:
                    ...
                    DoCast(player, 34883);
                    // below here is temporary workaround, to be removed when
...

        {
            ...
            if (Player* player = who->ToPlayer()) {
                if (who->HasAura(34877) &&
                    player->GetQuestStatus(10277) == QUEST_STATUS_INCOMPLETE) {
                    float Radius = 10.0f;
        ...
        void UpdateAI(uint32 diff) override { npc_escortAI::UpdateAI(diff); }
    };
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(10279) == QUEST_STATUS_INCOMPLETE ||
            player->GetQuestRewardStatus(10279)) {
...
public:
    npc_stone_watcher_of_norgannon()
        : CreatureScript("npc_stone_watcher_of_norgannon")
    {
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(2954) == QUEST_STATUS_INCOMPLETE) {
            AddGossipItemFor(
                player, 57000, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        }
...
public:
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_TOOGA) {
            if (npc_toogaAI* pToogaAI =
                    CAST_AI(npc_tooga::npc_toogaAI, creature->AI()))
...

        {
            ...
            if (!me->GetVictim() &&
                ...
                if (me->IsWithinDistInMap(who, INTERACTION_DISTANCE)) {
                    ...
                    if (player && player->GetQuestStatus(QUEST_TOOGA) ==
            ...
            if (!UpdateVictim()) {
                ...
                if (HasFollowState(STATE_FOLLOW_POSTEVENT)) {
                    if (PostEventTimer <= Diff) {
                        ...
                        Creature* torta =
                            ObjectAccessor::GetCreature(*me, TortaGUID);
                        ...
                        case 6:
                            ...
                            me->GetMotionMaster()->MovePoint(POINT_ID_TO_WATER,
                                                             ToWaterLoc);
                            break;
                        }
