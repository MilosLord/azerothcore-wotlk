...
struct boss_twinemperorsAI : public BossAI {
    boss_twinemperorsAI(Creature* creature)
        : BossAI(creature, DATA_TWIN_EMPERORS), _introDone(false)
    {
    ...
    Creature* GetTwin()
    {
        return instance->GetCreature(IAmVeklor() ? DATA_VEKNILASH
                                                 : DATA_VEKLOR);
    }
    ...
    {
        if (attacker) {
            if (attacker->GetEntry() == NPC_VEKLOR ||
                attacker->GetEntry() == NPC_VEKNILASH) {
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler
            ...
            .Schedule(
                3600ms,
                [this](TaskContext context) // according to sniffs it should be
                                            // casted by both emperors.
                {
                    if (Creature* twin = GetTwin()) {
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler
            .Schedule(14s,
                      [this](TaskContext context) {
                          DoCastRandomTarget(
                              SPELL_UPPERCUT, 0, me->GetMeleeReach(), true);
    ...
    bool _OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            if (instance->GetBossState(DATA_TWIN_EMPERORS) != DONE) {
                if (Creature* mastersEye =
                        instance->GetCreature(DATA_MASTERS_EYE)) {
                    mastersEye->AI()->Talk(EMOTE_MASTERS_EYE_AT, player);
                    mastersEye->DespawnOrUnsummon(11000);
                    mastersEye->m_Events.AddEventAtOffset(
                        [mastersEye, player]() {
...
class spell_mutate_explode_bug : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([&](WorldObject const* target) -> bool {
            if (target->GetEntry() != NPC_QIRAJI_SCARAB &&
                target->GetEntry() != NPC_QIRAJI_SCORPION)
                return true;
            if (Creature const* creature = target->ToCreature())
                if (creature->HasAura(SPELL_EXPLODE_BUG) ||
                    creature->HasAura(SPELL_MUTATE_BUG))
                    return true;
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_mutate_explode_bug::FilterTargets,
            EFFECT_ALL,
