...
ObjectData const creatureData[] = {{NPC_VEM, DATA_VEM},
                                   {NPC_KRI, DATA_KRI},
                                   {NPC_YAUJ, DATA_YAUJ},
...
public:
    instance_temple_of_ahnqiraj()
        : InstanceMapScript("instance_temple_of_ahnqiraj", 531)
    {
    }
    ...
    struct instance_temple_of_ahnqiraj_InstanceMapScript
        ...
        void Initialize() override { BugTrioDeathCount = 0; }
            ...
            case NPC_CTHUN:
                if (!creature->IsAlive()) {
                    for (ObjectGuid const& guid : CThunGraspGUIDs) {
                        if (GameObject* cthunGrasp =
                                instance->GetGameObject(guid)) {
            ...
            case NPC_QIRAJI_MINDSLAYER:
                if (Creature* creature = unit->ToCreature()) {
                    if (CreatureGroup* formation = creature->GetFormation()) {
                        scheduler.Schedule(
                            100ms, [formation](TaskContext /*context*/) {
...
void AddSC_instance_temple_of_ahnqiraj() { new instance_temple_of_ahnqiraj(); }
