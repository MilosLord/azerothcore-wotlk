...
struct npc_anubisath_defender : public ScriptedAI {
    ...
    {
        DoCastSelf(urand(0, 1) ? SPELL_SHADOW_FROST_REFLECT
...
struct npc_vekniss_stinger : public ScriptedAI {
    ...
    {
        DoCast(who,
               who->HasAura(SPELL_VEKNISS_CATALYST)
        ...
        scheduler.Schedule(6s, [this](TaskContext context) {
            Unit* target =
                SelectTarget(SelectTargetMethod::Random, 0, [&](Unit* u) {
                    return u && !u->IsPet() && u->IsWithinDist2d(me, 20.f) &&
                           u->HasAura(SPELL_VEKNISS_CATALYST);
                });
            if (!target) {
                target =
                    SelectTarget(SelectTargetMethod::Random, 0, [&](Unit* u) {
            ...
            if (target) {
                DoCast(target,
                       target->HasAura(SPELL_VEKNISS_CATALYST)
                           ? SPELL_STINGER_CHARGE_BUFFED
                           : SPELL_STINGER_CHARGE_NORMAL,
                       true);
            }
...
class spell_aggro_drones : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_aggro_drones::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_nullify : public AuraScript {
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_nullify::HandleApply,
                                              EFFECT_1,
