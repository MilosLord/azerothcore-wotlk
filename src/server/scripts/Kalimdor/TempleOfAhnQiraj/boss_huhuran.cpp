    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventid = events.ExecuteEvent()) {
            ...
            case EVENT_WYVERN_STING:
                me->CastCustomSpell(
                    SPELL_WYVERN_STING, SPELLVALUE_MAX_TARGETS, 10, me, true);
...
class spell_huhuran_wyvern_sting : public AuraScript {
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetTargetApplication()->GetRemoveMode() ==
            AURA_REMOVE_BY_ENEMY_SPELL) // dispelled
        {
            if (Unit* caster = GetCaster()) {
                caster->CastCustomSpell(SPELL_WYVERN_STING_DAMAGE,
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_huhuran_wyvern_sting::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_MOD_STUN,
...
class spell_huhuran_poison_bolt : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_huhuran_poison_bolt::FilterTargets,
            EFFECT_0,
