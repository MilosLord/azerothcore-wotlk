...
public:
                     ...
                     SpellSchoolMask) override
    {
        ...
        if (me->HealthBelowPctDamaged(0, damage) &&
            instance->GetData(DATA_BUG_TRIO_DEATH) < 2 && !_dying) {
            damage = 0;
            if (_isEating)
                return;
            ...
            if (Creature* yauj = instance->GetCreature(DATA_YAUJ)) {
                if (yauj->GetGUID() != me->GetGUID()) {
                    if (yauj->IsAlive()) {
                        ...
                        yauj->GetMotionMaster()->MovePoint(
                            POINT_CONSUME, x, y, z);
                    }
                }
                else
    ...
    void DoFinalSpell()
    {
        ...
        case NPC_YAUJ:
            for (uint8 i = 0; i < 10; ++i) {
                Position randomPos;
                me->GetRandomContactPoint(me,
                                          randomPos.m_positionX,
                                          randomPos.m_positionY,
                                          randomPos.m_positionZ);
                if (Creature* summon =
                        me->SummonCreature(NPC_YAUJ_BROOD, randomPos))
                    ...
                    if (Unit* target = SelectTarget(
                            ...
                            [this](Unit* target) -> bool {
                                ...
                                if (me->IsWithinMeleeRange(target) ||
                                    target == me->GetVictim())
                                    return false;
                                if (!me->IsWithinLOS(target->GetPositionX(),
                                                     target->GetPositionY(),
                                                     target->GetPositionZ()))
                                    return false;
                      ...
                      [this](TaskContext context) {
                          if (me->GetHealthPct() <= 93.f) {
                              DoCastSelf(SPELL_HEAL);
                                       ...
                                       DoSelectLowestHpFriendly(100.f)) {
...
class spell_vem_knockback : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_vem_knockback::HandleScriptEffect,
                          EFFECT_2,
...
class spell_vem_vengeance : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([&](WorldObject const* target) -> bool {
            return target->GetEntry() != NPC_YAUJ &&
                   target->GetEntry() != NPC_VEM &&
                   target->GetEntry() != NPC_KRI;
        });
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_vem_vengeance::FilterTargets,
                                          EFFECT_ALL,
