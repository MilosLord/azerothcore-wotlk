...
enum Says { SAY_AGGRO = 0, SAY_SLAY = 1, SAY_DEATH = 2 };
        ...
        while (uint32 eventId = events.ExecuteEvent()) {
            ...
            case EVENT_SARTURA_SUNDERING_CLEAVE:
                if (me->HasReactState(REACT_PASSIVE)) {
                    Milliseconds whirlwindTimer =
                        events.GetTimeUntilEvent(EVENT_SARTURA_WHIRLWIND_END);
                    events.RescheduleEvent(EVENT_SARTURA_SUNDERING_CLEAVE,
                                           whirlwindTimer + 500ms);
                }
                else {
                    DoCastVictim(SPELL_SUNDERING_CLEAVE, false);
                    events.RescheduleEvent(
                        EVENT_SARTURA_SUNDERING_CLEAVE, 2400ms, 3s);
                }
                break;
            default:
                break;
