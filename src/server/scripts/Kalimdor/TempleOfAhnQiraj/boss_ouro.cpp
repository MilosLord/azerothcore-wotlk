...
struct npc_ouro_spawner : public ScriptedAI {
    npc_ouro_spawner(Creature* creature) : ScriptedAI(creature) { Reset(); }
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!hasSummoned && who->GetTypeId() == TYPEID_PLAYER &&
            me->IsWithinDistInMap(who, 40.0f) &&
...
struct boss_ouro : public BossAI {
    ...
    void Submerge()
    {
        ...
        if (GameObject* base =
                me->FindNearestGameObject(GO_SANDWORM_BASE, 10.f)) {
            base->Use(me);
            base->DespawnOrUnsummon(6s);
        }

        DoCastSelf(SPELL_SUMMON_OURO_MOUNDS, true);
        // According to sniffs, Ouro uses his mounds to respawn. The health
        // management could be a little scuffed.
    ...
    void Emerge()
    {
        ...
        scheduler
            .Schedule(
                ...
                [this](TaskContext context) {
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::MaxThreat, 0, 0.0f, true)) {
            ...
            .Schedule(90s,
                      ...
                      [this](TaskContext /*context*/) { Submerge(); })
    ...
    void EnterEvadeMode(EvadeReason /*why*/) override
    {
        if (me->GetThreatMgr().GetThreatList().empty()) {
            ...
            if (GameObject* base =
                    me->FindNearestGameObject(GO_SANDWORM_BASE, 200.f))
