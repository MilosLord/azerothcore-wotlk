...
enum Misc { MAX_HATCHLING_SPAWN = 4, NPC_VEKNISS_HATCHLING = 15962 };
...
const std::array<uint32, 3> entangleSpells = {
    SPELL_ENTANGLE_RIGHT, SPELL_ENTANGLE_CENTER, SPELL_ENTANGLE_LEFT};
...
struct boss_fankriss : public BossAI {
    ...
    {
        summonWormSpells = {
            SPELL_SUMMON_WORM_1, SPELL_SUMMON_WORM_2, SPELL_SUMMON_WORM_3};
        BossAI::Reset();
    }
    ...
    void SummonHatchlingWaves()
    {
        for (Position spawnPos : hatchlingsSpawnPoints) {
            for (uint8 i = 0; i < MAX_HATCHLING_SPAWN; i++) {
                ...
                me->SummonCreature(NPC_VEKNISS_HATCHLING,
                                   randSpawn,
