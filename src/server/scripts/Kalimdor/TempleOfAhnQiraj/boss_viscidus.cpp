...
std::array<uint32, MAX_GLOB_SPAWN> const spawnGlobSpells = {
...
struct boss_viscidus : public BossAI {
                     ...
                     SpellSchoolMask  spellSchoolMask) override
    {
        ...
        if (_phase != PHASE_MELEE) {
            if (_phase == PHASE_FROST && effType == DIRECT_DAMAGE &&
                (spellSchoolMask & SPELL_SCHOOL_MASK_FROST) != 0) {
                ++_hitcounter;
            }
        ...
        if (attacker->HasUnitState(UNIT_STATE_MELEE_ATTACKING) &&
            _hitcounter >= HITCOUNTER_EXPLODE) {
            ...
            me->HandleEmoteCommand(
                EMOTE_ONESHOT_FLYDEATH); // not found in sniff, this is the best
                                         // one I found
            scheduler
                .Schedule(2500ms,
    ...
    void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
    {
        ...
        if (spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
            ...
            if (Item* pItem =
                    caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK)) {
                spellSchoolMask = SpellSchoolMask(
                    1 << pItem->GetTemplate()->Damage[0].DamageType);
            }
        }

        if ((spellSchoolMask & SPELL_SCHOOL_MASK_FROST) &&
            _phase == PHASE_FROST) {
            ++_hitcounter;
    ...
    void SummonedCreatureDies(Creature* summon, Unit* killer) override
    {
        ...
        if (killer && killer->GetEntry() == NPC_GLOB_OF_VISCIDUS) {
            ...
            me->CastCustomSpell(
                me, SPELL_VISCIDUS_GROWS, &heal, nullptr, nullptr, true);
        }

        if (!summons.IsAnyCreatureWithEntryAlive(
                NPC_GLOB_OF_VISCIDUS)) // all globs were killed
        {
            SoftReset();
            InitSpells();
            me->LowerPlayerDamageReq(me->GetMaxHealth());
    ...
    void InitializeAI() override
    {
        ...
        scheduler.Schedule(2400ms, [this](TaskContext context) {
            ...
            context.Schedule(1s, [this, topSpeed](TaskContext context) {
                float newSpeed =
                    me->GetSpeedRate(MOVE_RUN) + 0.2142855f; // sniffed
                me->SetSpeed(MOVE_RUN,
                             newSpeed < topSpeed ? newSpeed : topSpeed);
                context.Repeat();
            });
        });
    }
