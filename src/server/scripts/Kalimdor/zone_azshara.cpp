...
Position const WPs[58] = {{3691.97f, -3962.41f, 35.9118f, 3.67f},
...
public:
    ...
    struct npc_rizzle_sprysprocketAI : public ScriptedAI {
        ...
        void AttackStart(Unit* who) override
        {
            ...
            if (player && player->GetQuestStatus(QUEST_CHASING_THE_MOONSTONE) ==
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!Escape) {
                ...
                if (TeleportTimer <= diff) {
                    // temp solution - unit can't be teleported by core using
                    // spelleffect 5, only players
                    me->NearTeleportTo(
                        3706.39f, -3969.15f, 35.9118f, me->GetOrientation());
            ...
            if (CheckTimer <= diff) {
                ...
                if (me->IsWithinDist(player, 10) &&
                    me->GetPositionX() > player->GetPositionX() && !Reached) {
    ...
    {
        if (player->GetQuestStatus(QUEST_CHASING_THE_MOONSTONE) !=
            QUEST_STATUS_INCOMPLETE)
            return true;
