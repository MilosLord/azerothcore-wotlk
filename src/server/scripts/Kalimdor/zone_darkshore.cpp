            ...
            if (spawnTimer >= 5000) {
                ...
                case 0:
                    if (!me->FindNearestCreature(
                            NPC_GREYMIST_WARRIOR, 80.0f, true) &&
                        !me->FindNearestCreature(
                            NPC_GREYMIST_HUNTER, 80.0f, true)) {
                    ...
                    return;
                case 1:
                    if (!me->FindNearestCreature(NPC_GREYMIST_COASTRUNNER,
                                                 80.0f)) {
                        Player* player = me->SelectNearestPlayer(100.0f);
                        if (!player)
                            return;
...
/// @todo make concept similar as "ringo" -escort. Find a way to run the
/// scripted attacks, _if_ player are choosing road.
...
public:
    ...
    struct npc_kerlonianAI : public FollowerAI {
...

        {
            ...
            if (!me->GetVictim() && !HasFollowState(STATE_FOLLOW_COMPLETE) &&
                who->GetEntry() == NPC_LILADRIS) {
                if (me->IsWithinDistInMap(who, INTERACTION_DISTANCE * 5)) {
                    if (Player* player = GetLeaderForFollower()) {
                        if (player->GetQuestStatus(QUEST_SLEEPER_AWAKENED) ==
                            QUEST_STATUS_INCOMPLETE)
                            player->GroupEventHappens(QUEST_SLEEPER_AWAKENED,
                                                      me);
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_SLEEPER_AWAKENED) {
            if (npc_kerlonianAI* pKerlonianAI =
                    CAST_AI(npc_kerlonian::npc_kerlonianAI, creature->AI())) {
                creature->SetStandState(UNIT_STAND_STATE_STAND);
                creature->AI()->Talk(SAY_KER_START, player);
                pKerlonianAI->StartFollow(
...
public:
        ...
        void WaypointReached(uint32 waypointId) override
        {
            if (Player* player = GetPlayerForEscort()) {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    DoSpawnCreature(NPC_GRAVEL_SCOUT,
                                    -10.0f,
                                    5.0f,
                                    0.0f,
                                    0.0f,
                                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                    30000);
                    DoSpawnCreature(NPC_GRAVEL_BONE,
                                    -10.0f,
                                    7.0f,
                                    0.0f,
                                    0.0f,
                ...
                case 29:
                    ...
                    DoSpawnCreature(NPC_GRAVEL_BONE,
                                    ...
                                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                    30000);
                    DoSpawnCreature(NPC_GRAVEL_GEO,
                                    -15.0f,
                                    7.0f,
                                    0.0f,
                                    0.0f,
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_ABSENT_MINDED_PT2) {
            if (npc_escortAI* pEscortAI = CAST_AI(
                    npc_prospector_remtravel::npc_prospector_remtravelAI,
                    creature->AI()))
...
public:
    ...
    struct npc_rabid_thistle_bearAI : public FollowerAI {
        ...
        void Reset() override { me->SetStandState(UNIT_STAND_STATE_STAND); }

        void SpellHit(Unit* /*caster*/, SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_BEAR_CAPTURED_IN_TRAP) {
                if (GameObject* trap =
                        me->FindNearestGameObject(OBJECT_BEAR_TRAP, 5.0f)) {
                    if (Unit* owner = trap->GetOwner()) {
                        if (Player* player = owner->ToPlayer()) {
                            ...
                            me->GetMotionMaster()->MoveFollow(
                                player,
                                1.0f,
                                PET_FOLLOW_ANGLE - (PET_FOLLOW_ANGLE / 4));
                            _events.Reset();
            ...
            if (spellInfo->Id == SPELL_THARNARIUMS_HEAL) {
                me->HandleEmoteCommand(
                    EMOTE_ONESHOT_ATTACK1H); // EMOTE_ONESHOT_WOUND
            }
        }
                ...
                case EVENT_CHECK_FOLLOWING:
                    ...
                    if (!player || me->GetDistance2d(player) > 100.0f ||
                        me->GetMotionMaster()
                            ...
                            FOLLOW_MOTION_TYPE ||
                        player->GetQuestStatus(QUEST_PLAGUED_LANDS) ==
                            QUEST_STATUS_NONE) {
        ...
        void SetGUID(ObjectGuid /*guid*/, int32 type) override
        {
            if (type == GUID_SCRIPT_INVOKER && _scriptRunning == false) {
                if (Creature* bear = me->FindNearestCreature(
                        NPC_CAPTURED_RABID_THISTLE_BEAR, 5.0f)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_scriptRunning) {
                ...
                case EVENT_POST_QUEST_ONE:
                    if (Creature* bear =
                            ObjectAccessor::GetCreature(*me, _bearGUID)) {
