...
Position const BanditLoc[6] = {{-4905.479492f, -2062.732666f, 84.352f, 0.0f},
...
public:
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_FREE_AT_LAST) {
            ...
            if (npc_lakota_windsongAI* pEscortAI = CAST_AI(
                    npc_lakota_windsong::npc_lakota_windsongAI, creature->AI()))
...
Position const WyvernLoc[3] = {{-4990.606f, -906.057f, -5.343f, 0.0f},
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_HOMEWARD) {
            ...
            if (npc_paoka_swiftmountainAI* pEscortAI =
                    CAST_AI(npc_paoka_swiftmountain::npc_paoka_swiftmountainAI,
                            creature->AI()))
                pEscortAI->Start(false, false, player->GetGUID(), quest);
        }
        return true;
...
public:
    ...
    struct npc_pluckyAI : public ScriptedAI {
        ...
        {
            if (player->GetQuestStatus(QUEST_SCOOP) ==
...
class spell_panther_cage_key : public SpellScript {
    ...
    void HandleDummy()
    {
        if (Player* player = GetCaster()->ToPlayer()) {
            if (player->GetQuestStatus(QUEST_HYPERCAPACITOR_GIZMO) ==
                QUEST_STATUS_INCOMPLETE) {
                if (Creature* panther = player->FindNearestCreature(
                        NPC_ENRAGED_PANTHER, 5.0f, true)) {
