...
enum Belnistrasz {
    ...
    SAY_AGGRO                = 6, // Combat
    SAY_WATCH_OUT =
        7, // 25% chance to target random creature and say on wave spawn
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_IDOL_ROOM_SPAWNER:
                    if (Creature* creature =
                            me->SummonCreature(NPC_IDOL_ROOM_SPAWNER,
                                               PosSummonSpawner[urand(0, 2)],
                ...
                case EVENT_COMPLETE: {
                    ...
                    for (std::list<WorldObject*>::const_iterator itr =
                        ...
                        if (Player* player = (*itr)->ToPlayer()) {
                            ...
                            if (go->GetEntry() == GO_IDOL_OVEN_FIRE ||
                                go->GetEntry() == GO_IDOL_CUP_FIRE ||
        ...
        void SetData(uint32 /*type*/, uint32 data) override
        {
            if (data < 7) {
                ...
                me->SummonCreature(
                    ...
                    me->GetPositionX() +
                        (cos(me->GetOrientation() - (M_PI / 2)) * 2),
                    me->GetPositionY() +
                        (std::sin(me->GetOrientation() - (M_PI / 2)) * 2),
                    me->GetPositionZ(),
                    me->GetOrientation());
                me->SummonCreature(
