...
enum Phases { PHASE_NONE = 0, PHASE_REGROWTH };

enum Points { POINT_RANDOM = 1 };
                     ...
                     SpellSchoolMask /*school*/) override
    {
        if (_phase != PHASE_REGROWTH && me->HealthBelowPct(80.f)) {
            ...
            _scheduler.Schedule(25s, 30s, [this](TaskContext context) {
                Position randomPos =
                    me->GetRandomPoint(me->GetPosition(), 15.f);
...
void AddSC_boss_isalien() { RegisterDireMaulCreatureAI(boss_isalien); }
