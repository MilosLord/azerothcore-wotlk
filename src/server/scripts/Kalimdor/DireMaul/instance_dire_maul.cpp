...
public:
    ...
    struct instance_dire_maul_InstanceMapScript : public InstanceScript {
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case TYPE_PYLONS_STATE:
                ...
                if (_pylonsState == ALL_PYLONS_OFF) // all five active, 31
                {
                    ...
                    if (Creature* immol =
                            instance->GetCreature(_immoltharGUID)) {
        ...
        {
            data << _eastWingProgress << ' ' << _westWingProgress << ' '
                 << _pylonsState << ' ' << _northWingProgress << ' '
                 << _northWingBosses;
        }
...
void AddSC_instance_dire_maul() { new instance_dire_maul(); }
