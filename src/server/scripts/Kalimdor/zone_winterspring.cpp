...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (creature->IsVendor() &&
            player->GetReputationRank(589) == REP_EXALTED)
...
enum Spells {
    SPELL_LIGHT_TORCH = 18953, // channeled spell by Ranshalla while waiting for
                               // the torches / altar
};
...
public:
    ...
    DialogueHelper(DialogueEntry const* dialogueArray)
        : _dialogueArray(dialogueArray), _currentEntry(nullptr), _actionTimer(0)
    {
    }
    // The array MUST be terminated by {0, 0, 0, 0, 0}

    /// Function to initialize the dialogue helper for instances. If not used
    /// with instances, GetSpeakerByEntry MUST be overwritten to obtain the
    /// speakers Set if take first entries or second entries
    ...
    void StartNextDialogueText(int32 textEntry)
    {
        ...
        for (DialogueEntry const* entry = _dialogueArray; entry->TextEntry;
             ++entry) {
...
protected:
    ...
    /// Will be called to get a speaker, MUST be implemented if not used in
    /// instances
...
const DialogueEntry introDialogue[] = {
    ...
    {SAY_PRIESTESS_ALTAR_13,
     NPC_PRIESTESS_DATA_1,
     8000}, // summon voice and guard of elune
    ...
    {SAY_PRIESTESS_ALTAR_20,
     NPC_PRIESTESS_DATA_1,
     4000}, // move the first priestess up
    {SAY_PRIESTESS_ALTAR_21,
     NPC_PRIESTESS_DATA_2,
     10000},                        // move second priestess up
...
public:
                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_GUARDIANS_ALTAR) {
            ...
            if (npc_ranshallaAI* escortAI =
                    dynamic_cast<npc_ranshallaAI*>(creature->AI()))
    ...
    struct npc_ranshallaAI : public npc_escortAI, private DialogueHelper {
        npc_ranshallaAI(Creature* creature)
            : npc_escortAI(creature), DialogueHelper(introDialogue)
        {
            Reset();
        }
        ...
        void Reset() override { _delayTimer = 0; }
        ...
        {
            // Check if we are using the fire or the altar and remove the
            // no_interact flag
            if (isAltarWaypoint) {
                if (GameObject* go = GetClosestGameObjectWithEntry(
                        me, GO_ELUNE_ALTAR, 10.0f)) {
                    go->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                    me->SetFacingToObject(go);
                    _altarGUID = go->GetGUID();
                }
            }
            else if (GameObject* go = GetClosestGameObjectWithEntry(
                         me, GO_ELUNE_FIRE, 10.0f))
                go->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
        ...
        {
            // Summon 2 Elune priestess and make each of them move to a
            // different spot
            if (Creature* priestess =
                    me->SummonCreature(NPC_PRIESTESS_ELUNE,
                                       wingThicketLocations[0].m_positionX,
                                       wingThicketLocations[0].m_positionY,
                                       wingThicketLocations[0].m_positionZ,
                                       wingThicketLocations[0].GetOrientation(),
                                       TEMPSUMMON_CORPSE_DESPAWN,
                                       0)) {
                priestess->GetMotionMaster()->MovePoint(
                    0,
                    wingThicketLocations[3].m_positionX,
                    wingThicketLocations[3].m_positionY,
                    wingThicketLocations[3].m_positionZ);
                _firstPriestessGUID = priestess->GetGUID();
            }
            if (Creature* priestess =
                    me->SummonCreature(NPC_PRIESTESS_ELUNE,
                                       wingThicketLocations[1].m_positionX,
                                       wingThicketLocations[1].m_positionY,
                                       wingThicketLocations[1].m_positionZ,
                                       wingThicketLocations[1].GetOrientation(),
                                       TEMPSUMMON_CORPSE_DESPAWN,
                                       0)) {
                // Left priestess should have a distinct move point because she
                // is the one who starts the dialogue at point reach
                priestess->GetMotionMaster()->MovePoint(
                    1,
                    wingThicketLocations[4].m_positionX,
                    wingThicketLocations[4].m_positionY,
                    wingThicketLocations[4].m_positionZ);
                _secondPriestessGUID = priestess->GetGUID();
            }
        }

        void
        SummonedMovementInform(Creature* summoned, uint32 type, uint32 pointId)
        {
            if (type != POINT_MOTION_TYPE ||
                summoned->GetEntry() != NPC_PRIESTESS_ELUNE || pointId != 1)
                return;

            // Start the dialogue when the priestess reach the altar (they
            // should both reach the point in the same time)
        ...
        void WaypointReached(uint32 pointId) override
        {
            ...
            case 41: {
                ...
                GetGameObjectListWithEntryInGrid(
                    eluneLights, me, GO_ELUNE_LIGHT, 20.0f);
        ...
        void JustDidDialogueStep(int32 entry) override
        {
            ...
            case SAY_PRIESTESS_ALTAR_8:
                ...
                if (GameObject* gem = GetClosestGameObjectWithEntry(
                        me, GO_ELUNE_GEM, 10.0f)) {
                    if (gem->isSpawned())
                        break;
            ...
            case SAY_PRIESTESS_ALTAR_9:
                ...
                if (Creature* priestess =
                        me->GetMap()->GetCreature(_firstPriestessGUID))
                    priestess->GetMotionMaster()->MovePoint(
                        0,
                        wingThicketLocations[6].m_positionX,
                        wingThicketLocations[6].m_positionY,
                        wingThicketLocations[6].m_positionZ);
                break;
            case SAY_PRIESTESS_ALTAR_13:
                // summon the Guardian of Elune
                if (Creature* guard = me->SummonCreature(
                        NPC_GUARDIAN_ELUNE,
                        wingThicketLocations[2].m_positionX,
                        wingThicketLocations[2].m_positionY,
                        wingThicketLocations[2].m_positionZ,
                        wingThicketLocations[2].GetOrientation(),
                        TEMPSUMMON_CORPSE_DESPAWN,
                        0)) {
                    guard->GetMotionMaster()->MovePoint(
                        0,
                        wingThicketLocations[5].m_positionX,
                        wingThicketLocations[5].m_positionY,
                        wingThicketLocations[5].m_positionZ);
                    _guardEluneGUID = guard->GetGUID();
                }
                // summon the Voice of Elune
                if (GameObject* altar =
                        me->GetMap()->GetGameObject(_altarGUID)) {
                    if (Creature* voice =
                            me->SummonCreature(NPC_VOICE_ELUNE,
                                               altar->GetPositionX(),
                                               altar->GetPositionY(),
                                               altar->GetPositionZ(),
                                               0,
            ...
            case SAY_VOICE_ALTAR_15:
                ...
                if (Creature* priestess =
                        me->GetMap()->GetCreature(_secondPriestessGUID)) {
                    priestess->AI()->Talk(SAY_PRIESTESS_ALTAR_14);
                    priestess->GetMotionMaster()->MovePoint(
                        0,
                        wingThicketLocations[7].m_positionX,
                        wingThicketLocations[7].m_positionY,
                        wingThicketLocations[7].m_positionZ);
                }
                break;
            case SAY_PRIESTESS_ALTAR_19:
                // make the voice of elune leave
                if (Creature* guard =
                        me->GetMap()->GetCreature(_guardEluneGUID)) {
                    guard->GetMotionMaster()->MovePoint(
                        0,
                        wingThicketLocations[2].m_positionX,
                        wingThicketLocations[2].m_positionY,
...
public:
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        ...
        if (Creature* ranshalla =
                GetClosestCreatureWithEntry(go, NPC_RANSHALLA, 10.0f)) {
            if (npc_ranshalla::npc_ranshallaAI* escortAI =
                    dynamic_cast<npc_ranshalla::npc_ranshallaAI*>(
                        ranshalla->AI()))
