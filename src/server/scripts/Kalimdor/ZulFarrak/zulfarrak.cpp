...
#define GOSSIP_BLY                                                             \
    "That's it!  I'm tired of helping you out.  It's time we settled things "  \
    "on the battlefield!"
...
public:
    ...
    struct npc_sergeant_blyAI : public ScriptedAI {
        ...
        uint32 Revenge_Timer; // this is wrong, spell should never be used
                              // unless me->GetVictim() dodge, parry or block
                              // attack. Trinity support required.
        ...
        void MovementInform(uint32 type, uint32 /*id*/) override
        {
            ...
            if (instance->GetData(DATA_PYRAMID) == PYRAMID_WAVE_3) {
                if (Creature* shadowpriest = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_SHADOWPRIEST_SEZZZIZ))) {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (postGossipStep > 0 && postGossipStep < 4) {
                if (Text_Timer <= diff) {
                    ...
                    case 1:
                        ...
                        if (Creature* pWeegli = ObjectAccessor::GetCreature(
                                *me, instance->GetGuidData(NPC_WEEGLI))) {
            ...
            if (Porthome_Timer <= diff && ableToPortHome == true) {
                if (Creature* weegli = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_WEEGLI))) {
                    weegli->CastSpell(weegli, SPELL_BLYS_BAND_ESCAPE);
                    weegli->DespawnOrUnsummon(10000);
                }
                if (Creature* raven = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_RAVEN))) {
                    raven->CastSpell(raven, SPELL_BLYS_BAND_ESCAPE);
                    raven->DespawnOrUnsummon(10000);
                }
                if (Creature* oro = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_ORO))) {
                    oro->CastSpell(oro, SPELL_BLYS_BAND_ESCAPE);
                    oro->DespawnOrUnsummon(10000);
                }
                if (Creature* murta = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_MURTA))) {
                    murta->CastSpell(murta, SPELL_BLYS_BAND_ESCAPE);
                    murta->DespawnOrUnsummon(10000);
                }
                DoCastSelf(SPELL_BLYS_BAND_ESCAPE);
                me->DespawnOrUnsummon(10000);
                Porthome_Timer = 156000; // set timer back so that the event
                                         // doesn't keep triggering
            }
            else {
                Porthome_Timer -= diff;
            }
        ...
        {
            if (Creature* crew = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(entry))) {
                if (crew->IsAlive()) {
...
public:
    ...
    struct go_troll_cageAI : public GameObjectAI {
        go_troll_cageAI(GameObject* go)
            : GameObjectAI(go), instance(go->GetInstanceScript())
        {
        }
        ...
        bool GossipHello(Player* /*player*/, bool reportUse) override
        {
            ...
            if (Creature* bly = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(NPC_BLY))) {
...
public:
    ...
    struct npc_weegli_blastfuseAI : public ScriptedAI {
                    ...
                    if (outroTimer <= diff) {
                        ...
                        case 1:
                            me->GetMotionMaster()->MovePoint(
                                2, 1871.18f, 1100.f, 8.88f);
                            Talk(SAY_WEEGLI_OUT_OF_HERE);
                            me->DespawnOrUnsummon(8000);
                            instance->SetData(DATA_PYRAMID,
        ...
        void JustReachedHome() override
        {
            ...
            else if (instance->GetData(DATA_PYRAMID) >=
        ...
        void DestroyDoor()
        {
            if (me->IsAlive()) {
                ...
                me->SetWalk(false);
                me->GetMotionMaster()->MovePoint(
                    0, {1858.57f, 1146.35f, 14.745f, 3.85f});
                           ...
                           uint32 gossipListId) override
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1) {
                CloseGossipMenuFor(player);
                // here we make him run to door, set the charge and run away off
                // to nowhere
                DestroyDoor();
            }
        }
        ...
        void sGossipHello(Player* player) override
        {
            ...
            case PYRAMID_KILLED_ALL_TROLLS:
                AddGossipItemFor(player,
                                 ...
                                 GOSSIP_ACTION_INFO_DEF + 1);
                SendGossipMenuFor(
                    player, 1514, me->GetGUID()); // if event can proceed to end
                break;
            case PYRAMID_NOT_STARTED:
                SendGossipMenuFor(
                    player, 1511, me->GetGUID()); // if event not started
                break;
            default:
                SendGossipMenuFor(
                    player, 1513, me->GetGUID()); // if event is in progress
            }
        }
    };
...
std::array<std::vector<std::pair<uint32, Position>>, 4> shadowpriestSezzizAdds =
    {{{{NPC_SANDFURY_ZEALOT, {1874.12f, 1198.90f, 8.87f}},
      ...
      {{NPC_SANDFURY_ACOLYTE, {895.26f, 1199.09f, 8.87f}},
       {NPC_SANDFURY_ACOLYTE, {895.26f, 1199.088f, 8.87f}}},
      {{NPC_SANDFURY_ZEALOT, {1874.12f, 1198.90f, 8.87f}},
       {NPC_SANDFURY_ACOLYTE, {895.26f, 1199.09f, 8.87f}},
       {NPC_SANDFURY_ACOLYTE, {895.26f, 1199.09f, 8.87f}}},
...
public:
    ...
    struct npc_shadowpriest_sezzizAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            _physicScreemTimer =
                urand(1 * IN_MILLISECONDS, 16 * IN_MILLISECONDS);
            _missingHPForRenewTimer =
                urand(10 * IN_MILLISECONDS, 15 * IN_MILLISECONDS);
            _missingHPForHealTimer =
                urand(7 * IN_MILLISECONDS, 10 * IN_MILLISECONDS);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_summonAddsTimer <= diff) {
                ...
                _summonAddsTimer =
                    urand(10 * IN_MILLISECONDS, 14 * IN_MILLISECONDS);
            }
            else {
            ...
            if (_physicScreemTimer <= diff) {
                ...
                _physicScreemTimer =
                    urand(22 * IN_MILLISECONDS, 30 * IN_MILLISECONDS);
            }
            else {
            ...
            if (_shadowBoltTimer <= diff) {
                ...
                _shadowBoltTimer =
                    urand(3 * IN_MILLISECONDS, 4 * IN_MILLISECONDS);
            }
            else {
