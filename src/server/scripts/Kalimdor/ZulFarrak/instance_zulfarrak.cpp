...
public:
    ...
    struct instance_zulfarrak_InstanceMapScript : public InstanceScript {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_BLY:
                ...
                creature->SetReactState(
                    REACT_PASSIVE); // starts out passive (in a cage)
                break;
            case NPC_RAVEN:
        ...
        void Update(uint32 diff) override
        {
                ...
                if (IsWaveAllDead()) {
                    ...
                    major_wave_Timer =
                        10 * IN_MILLISECONDS; // give players a few seconds
                                              // before wave 2 starts to rebuff
                }
                else {
            ...
            case PYRAMID_WAVE_2:
                if (IsWaveAllDead()) {
                    ...
                    major_wave_Timer =
                        5 * IN_MILLISECONDS; // give NPCs time to return to
                                             // their home spots
                }
                else {
                    if (minor_wave_Timer <= diff) {
            ...
            case PYRAMID_PRE_WAVE_3:
                if (major_wave_Timer <= diff) {
                    ...
                    MoveNPCIfAlive(
                        NPC_RAVEN, 1883.68f, 1227.95f, 9.543f, 4.78f);
                    MoveNPCIfAlive(
                        NPC_WEEGLI, 1878.02f, 1227.65f, 9.485f, 4.78f);
                    SetData(DATA_PYRAMID, PYRAMID_WAVE_3);
                    if (Creature* sergeantBlye =
                            instance->GetCreature(BlyGUID)) {
            ...
            case PYRAMID_WAVE_3:
                if (IsWaveAllDead()) // move NPCS to their final positions
                {
                    ...
                    MoveNPCIfAlive(
                        NPC_WEEGLI, 1877.52f, 1199.63f, 8.87f, 1.32f);
                }
                break;
            default:
                break;
        ...
        void SpawnPyramidWave(uint32 wave)
        {
            for (uint32 i = 0; i < pyramidSpawnTotal; i++) {
                if (pyramidSpawns[i].waveID == wave) {
                    ...
                    if (pyramidSpawns[i].creatureID ==
                            NPC_SHADOWPRIEST_SEZZZIZ ||
                        pyramidSpawns[i].creatureID == NPC_NEKRUM_GUTCHEWER) {
                        orientation = 1.32f;
                    }

                    Position pos = {pyramidSpawns[i].pos.GetPositionX(),
                                    pyramidSpawns[i].pos.GetPositionY(),
                                    8.87f,
                                    orientation};
                    if (TempSummon* ts = instance->SummonCreature(
                            pyramidSpawns[i].creatureID, pos)) {
                        addsAtBase.push_back(ts->GetGUID());

                        if (pyramidSpawns[i].creatureID !=
                                NPC_SHADOWPRIEST_SEZZZIZ &&
                            pyramidSpawns[i].creatureID !=
        ...
        bool IsWaveAllDead()
        {
            for (GuidList::iterator itr = addsAtBase.begin();
                 itr != addsAtBase.end();
                 ++itr) {
            ...
            for (GuidList::iterator itr = movedadds.begin();
                 itr != movedadds.end();
                 ++itr) {
                if (Creature* add = instance->GetCreature(((*itr)))) {
                    if (add->IsAlive()) {
        ...
        void SendAddsUpStairs(uint32 count)
        {
            ...
            for (uint32 addCount = 0; addCount < count && !addsAtBase.empty();
                 addCount++) {
                if (Creature* add =
                        instance->GetCreature(*addsAtBase.begin())) {
...
public:
    spell_zulfarrak_summon_zulfarrak_zombies()
        : SpellScriptLoader("spell_zulfarrak_summon_zulfarrak_zombies")
    {
    }
    ...
    class spell_zulfarrak_summon_zulfarrak_zombies_SpellScript
        : public SpellScript {
        PrepareSpellScript(
            spell_zulfarrak_summon_zulfarrak_zombies_SpellScript);
    ...
    class spell_zulfarrak_unlocking_SpellScript : public SpellScript {
        ...
        void HandleOpenLock(SpellEffIndex /*effIndex*/)
        {
            ...
            Acore::WorldObjectListSearcher<Acore::AllWorldObjectsInRange>
                searcher(GetCaster(), cagesList, objects);
            Cell::VisitAllObjects(GetCaster(), searcher, 15.0f);
            for (std::list<WorldObject*>::const_iterator itr =
                     cagesList.begin();
                 itr != cagesList.end();
                 ++itr) {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_zulfarrak_unlocking_SpellScript::HandleOpenLock,
                EFFECT_0,
