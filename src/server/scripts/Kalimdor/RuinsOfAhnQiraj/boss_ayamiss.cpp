...
enum Points { POINT_AIR = 0, POINT_GROUND = 2, POINT_PARALYZE = 2 };
    ...
    void ScheduleTasks() override
    {
        scheduler
            ...
            .Schedule(15s, 28s, [this](TaskContext context) {
                if (Unit* target =
                        SelectTarget(SelectTargetMethod::Random, 0, 0, true)) {
                    DoCast(target, SPELL_PARALYZE, true);
                    instance->SetGuidData(DATA_PARALYZED, target->GetGUID());
                    DoCastAOE(RAND(SPELL_SUMMON_LARVA_A, SPELL_SUMMON_LARVA_B),
                              true);
                }
                context.Repeat();
            });
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == POINT_MOTION_TYPE && id == POINT_PARALYZE) {
            if (Player* target = ObjectAccessor::GetPlayer(
                    *me, _instance->GetGuidData(DATA_PARALYZED))) {
...
struct npc_hive_zara_swarmer : public ScriptedAI {
    ...
    {
        // Delay is required because we are calling the movement generator from
        // inside the pathing hook. If we issue another call here, it will be
        // flushed before it is executed.
        me->m_Events.AddEventAtOffset(
            [this]() { DoCastSelf(SPELL_HIVEZARA_SWARMER_START_LOOP); }, 1s);
    }
};
...
class spell_ayamiss_swarmer_teleport_trigger : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        WaspTeleportData telData[5] = {
            ...
            {SPELL_HIVEZARA_SWARMER_TELEPORT_2,
             (NPC_HIVEZARA_SWARMER + 1) * 10},
            {SPELL_HIVEZARA_SWARMER_TELEPORT_3,
             (NPC_HIVEZARA_SWARMER + 2) * 10},
            {SPELL_HIVEZARA_SWARMER_TELEPORT_4,
             (NPC_HIVEZARA_SWARMER + 3) * 10},
            {SPELL_HIVEZARA_SWARMER_TELEPORT_5,
             (NPC_HIVEZARA_SWARMER + 4) * 10}};

        WaspTeleportData data =
            Acore::Containers::SelectRandomContainerElement(telData);
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_ayamiss_swarmer_teleport_trigger::HandleScript,
                          EFFECT_0,
...
class spell_ayamiss_swarmer_swarm : public SpellScript {
    ...
    bool Load() override { return GetCaster()->GetEntry() == NPC_AYAMISS; }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_ayamiss_swarmer_swarm::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_ayamiss_swarmer_start_loop : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_HIVEZARA_SWARMER_LOOP_1,
                                  SPELL_HIVEZARA_SWARMER_LOOP_2,
                                  SPELL_HIVEZARA_SWARMER_LOOP_3});
    }
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        uint32 loopSpells[3] = {SPELL_HIVEZARA_SWARMER_LOOP_1,
                                SPELL_HIVEZARA_SWARMER_LOOP_2,
                                SPELL_HIVEZARA_SWARMER_LOOP_3};
        GetCaster()->CastSpell(
            (Unit*)nullptr,
            Acore::Containers::SelectRandomContainerElement(loopSpells));
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_ayamiss_swarmer_start_loop::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
public:
    spell_gen_ayamiss_swarmer_loop(uint32 pathId)
        : SpellScript(), _pathId(pathId)
    {
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_ayamiss_swarmer_loop::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
...
void AddSC_boss_ayamiss()
{
    ...
    RegisterSpellScriptWithArgs(spell_gen_ayamiss_swarmer_loop,
                                "spell_gen_ayamiss_swarmer_loop_1",
                                (NPC_HIVEZARA_SWARMER + 5) * 10);
    RegisterSpellScriptWithArgs(spell_gen_ayamiss_swarmer_loop,
                                "spell_gen_ayamiss_swarmer_loop_2",
                                (NPC_HIVEZARA_SWARMER + 6) * 10);
    RegisterSpellScriptWithArgs(spell_gen_ayamiss_swarmer_loop,
                                "spell_gen_ayamiss_swarmer_loop_3",
                                (NPC_HIVEZARA_SWARMER + 7) * 10);
}
