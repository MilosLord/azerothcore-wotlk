...
enum Yells {
    // The time of our retribution is at hand! Let darkness reign in the hearts
    // of our enemies! Sound: 8645 Emote: 35
    SAY_DEATH         = 9,
    SAY_CHANGEAGGRO   = 10,
    SAY_KILLS_ANDOROV = 11,
    SAY_COMPLETE_QUEST =
        12 // Yell when realm complete quest 8743 for world event
...
struct boss_rajaxx : public BossAI {
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (Creature* andorov = instance->instance->GetCreature(
                instance->GetGuidData(DATA_ANDOROV))) {
        ...
        me->GetCreatureListWithEntryInGrid(
            creatureList, NPC_KALDOREI_ELITE, 200.0f);
        creatureList.remove_if(
            [&](Creature* creature) -> bool { return !creature->IsAlive(); });
...
class spell_rajaxx_thundercrash : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_rajaxx_thundercrash::HandleDamageCalc,
                          EFFECT_0,
...
struct npc_general_andorov : public npc_escortAI {
    npc_general_andorov(Creature* creature)
        : npc_escortAI(creature), _summons(me)
    {
    ...
    void InitializeAI() override
    {
        ...
        for (uint8 i = 0; i < 4; ++i) {
            if (Creature* kaldoreielitist =
                    me->SummonCreature(NPC_KALDOREI_ELITE, *me)) {
                ...
                CAST_AI(SmartAI, kaldoreielitist->AI())
                    ->SetFollow(me, 2.5f, 0.f + i * (M_PI / 2));
            }
        }
            ...
            if (!Endwaypoint) {
                ...
                for (ObjectGuid const& guid : _summons) {
                    if (Creature* kaldoreielitist =
                        ...
                        kaldoreielitist->RemoveUnitFlag(
