...
enum Actions { ACTION_TRIGGER_WEAKNESS = 1, ACTION_DESPAWN_TRIGGER = 2 };
...
Position    CrystalCoordinates[NUM_CRYSTALS] = {{-9407.7197265625f,
                                                 1960.20996093750f,
                                                 85.6390991210937f,
                                                 1.11700999736786f},
                                                {-9388.4404296875f,
...
Position VortexPositions[2] = {{-9524.06f, 1881.9224f, 85.64029f, 0.0f},
                               {-9228.479, 1925.3331f, 85.64147f, 0.0f}};

uint8 const  NUM_WEAKNESS                = 5;
uint32 const spellWeakness[NUM_WEAKNESS] = {SPELL_FIRE_WEAKNESS,
...
struct boss_ossirian : public BossAI {
    ...
    void Reset() override
    {
        ...
        if (!ObjectAccessor::GetGameObject(*me, _firstCrystalGUID)) {
            if (Creature* trigger = me->GetMap()->SummonCreature(
                    NPC_OSSIRIAN_TRIGGER, CrystalCoordinates[0])) {
                if (GameObject* crystal = me->SummonGameObject(
                        GO_OSSIRIAN_CRYSTAL,
                    ...
                    crystal->AI()->SetGUID(trigger->GetGUID(),
                                           GUID_TRIGGER_PAIR);
                }
            }
        }
    ...
    void JustSummoned(Creature* creature) override { summons.Summon(creature); }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        WorldPackets::Misc::Weather weather(WEATHER_STATE_HEAVY_SANDSTORM,
                                            1.0f);
    ...
    void SummonedCreatureDespawn(Creature* summon) override
    {
        ...
        if (summon->GetEntry() == NPC_OSSIRIAN_TRIGGER) {
            if (GameObject* crystal = GetClosestGameObjectWithEntry(
                    summon, GO_OSSIRIAN_CRYSTAL, 5.0f)) {
                crystal->Delete();
            }
        }
    }
    ...
    void SpawnNextCrystal(uint8 count = 1)
    {
        for (uint8 i = 0; i < count; ++i) {
            ...
            if (Creature* trigger =
                    me->SummonCreature(NPC_OSSIRIAN_TRIGGER,
                                       CrystalCoordinates[_crystalIterator])) {
                if (GameObject* crystal = trigger->SummonGameObject(
                        GO_OSSIRIAN_CRYSTAL,
    ...
    struct go_ossirian_crystalAI : public GameObjectAI {
        ...
        bool GossipHello(Player* /*player*/, bool reportUse) override
        {
            if (reportUse) {
                ...
                if (Creature* trigger =
                        ObjectAccessor::GetCreature(*me, _triggerGUID)) {
                    if (!trigger->HasUnitState(UNIT_STATE_CASTING)) {
                        ossirian->AI()->SetGUID(me->GetGUID(),
                                                ACTION_TRIGGER_WEAKNESS);
                        trigger->CastSpell(
                            trigger, spellWeakness[urand(0, 4)], false);
                    }
                }
            }
...
struct npc_anubisath_guardian : public ScriptedAI {
    npc_anubisath_guardian(Creature* creature) : ScriptedAI(creature)
    {
        ...
        _spells[1] =
            RAND(SPELL_REFLECT_ARCANE_FIRE, SPELL_REFLECT_FROST_SHADOW);
    }
...
class spell_crystal_weakness : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_crystal_weakness::FilterTargets,
                                          EFFECT_ALL,
...
class spell_aq_shadow_storm : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_aq_shadow_storm::FilterTargets,
                                          EFFECT_0,
