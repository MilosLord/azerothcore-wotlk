...
enum Phases { PHASE_EGG = 1, PHASE_TRANSFORM = 2 };
...
struct boss_buru : public BossAI {
    ...
    void ChaseNewVictim()
    {
        ...
        events.ScheduleEvent(EVENT_GATHERING_SPEED, 2s);
        if (Unit* victim =
                SelectTarget(SelectTargetMethod::Random, 0, 0.f, true)) {
            DoResetThreatList();
            AttackStart(victim);
...
struct npc_buru_egg : public ScriptedAI {
    ...
    void JustSummoned(Creature* who) override
    {
        if (who->GetEntry() == NPC_HATCHLING) {
            if (Creature* buru = _instance->GetCreature(DATA_BURU)) {
                if (Unit* target =
                        buru->AI()->SelectTarget(SelectTargetMethod::Random)) {
...
class spell_egg_explosion : public SpellScript {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            if (target->IsPlayer())
                damage = -16 * GetCaster()->GetDistance(target) + 500;
                     ...
                     target->HasAura(SPELL_THORNS))
                damage = target->GetMaxHealth() * 7.f / 100;

            GetCaster()->CastCustomSpell(
                target, SPELL_EXPLODE_2, &damage, nullptr, nullptr, true);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_egg_explosion::HandleDummyHitTarget,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
