...
ObjectData const creatureData[] = {{NPC_BURU, DATA_BURU},
...
std::array<uint32, 8> RajaxxWavesData[] = {{DATA_QUUEZ, 0},
                                           {DATA_TUUBID, 0},
...
public:
    ...
    struct instance_ruins_of_ahnqiraj_InstanceMapScript
        ...
        void OnPlayerEnter(Player* player) override
        {
            if (GetBossState(DATA_KURINNAXX) == DONE &&
                ...
                _rajaxWaveCounter ==
                    0 &&       // if non-zero, encounter is in progress
                !_andorovGUID) // cleared if he is dead
            {
                instance->LoadGrid(-8538.17f,
                                   1486.09f); // Andorov run path grid
                if (Creature* creature =
                        player->SummonCreature(NPC_ANDOROV,
                                               -8538.177f,
                                               1486.0956f,
                                               32.39054f,
                                               3.7638654f,
                ...
                if (CreatureGroup* formation = creature->GetFormation()) {
                    if (Creature* leader = formation->GetLeader()) {
                        ...
                        case NPC_ZERRAN:
                            scheduler.CancelAll();
                            scheduler.Schedule(
                                1s, [this, formation](TaskContext /*context*/) {
        ...
        void CallNextRajaxxLeader(bool announce = false)
        {
            ...
            if (Creature* nextLeader =
                    GetCreature(RajaxxWavesData[_rajaxWaveCounter].at(0))) {
                if (announce) {
                    if (_rajaxWaveCounter >= 2) {
                        if (Creature* rajaxx = GetCreature(DATA_RAJAXX)) {
                            rajaxx->AI()->Talk(
                                RajaxxWavesData[_rajaxWaveCounter].at(1));
                        }
                    }
                }

                if (nextLeader->IsAlive()) {
                    Creature* generalAndorov =
                        instance->GetCreature(_andorovGUID);
                    if (generalAndorov && generalAndorov->IsAlive() &&
                        generalAndorov->AI()->GetData(DATA_ANDOROV)) {
...
void AddSC_instance_ruins_of_ahnqiraj() { new instance_ruins_of_ahnqiraj(); }
