...
SDName: Moonglade
SD%Complete: 100
SDComment: Quest support: 30, 272, 5929, 5930, 10965. Special Flight Paths for
Druid class. SDCategory: Moonglade EndScriptData */
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->GetTeamId() != TEAM_HORDE) {
            if (player->IsClass(CLASS_DRUID, CLASS_CONTEXT_QUEST) &&
                player->GetQuestStatus(QUEST_SEA_LION_ALLY) ==
            ...
            if (player->IsClass(CLASS_DRUID, CLASS_CONTEXT_QUEST) &&
                player->GetQuestStatus(QUEST_SEA_LION_HORDE) ==
                    QUEST_STATUS_INCOMPLETE) {
                AddGossipItemFor(player,
                                 4042,
                                 1,
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        // ally or horde quest
        if (player->GetQuestStatus(5929) == QUEST_STATUS_INCOMPLETE ||
            player->GetQuestStatus(5930) == QUEST_STATUS_INCOMPLETE) {
            AddGossipItemFor(
                player, 3882, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
// Position const ClintarSpiritSummon  = {7459.2275f, -3122.5632f, 438.9842f,
// 0.8594f};
    ...
    public:
        ...
        void IsSummonedBy(WorldObject* /*summoner*/) override
        {
            ...
            Acore::PlayerListSearcher<Acore::AnyPlayerInObjectRangeCheck>
                searcher(me, playerOnQuestList, checker);
            Cell::VisitWorldObjects(me, searcher, 5.0f);
            for (std::list<Player*>::const_iterator itr =
                     playerOnQuestList.begin();
                 itr != playerOnQuestList.end();
                 ++itr) {
                // Check if found player target has active quest
                if (Player* player = (*itr)) {
                    if (player->GetQuestStatus(10965) ==
                        QUEST_STATUS_INCOMPLETE) {
                        StartEvent(player);
                        break;
                    }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (player &&
                player->GetQuestStatus(10965) == QUEST_STATUS_INCOMPLETE) {
        ...
        void StartEvent(Player* player)
        {
            if (player &&
                ...
                for (uint8 i = 0; i < 41; ++i) {
                    AddWaypoint(i,
                                Clintar_spirit_WP[i][0],
                                Clintar_spirit_WP[i][1],
                                Clintar_spirit_WP[i][2],
                                (uint32)Clintar_spirit_WP[i][4]);
                }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (EventOnWait && EventTimer <= diff) {
                Player* player = ObjectAccessor::GetPlayer(*me, PlayerGUID);
                if (!player ||
                    player->GetQuestStatus(10965) == QUEST_STATUS_NONE) {
                    me->setDeathState(DeathState::JustDied);
                    return;
                }
                ...
                case 20:
                    switch (Step) {
                    case 0:
                        if (Creature* mob = me->SummonCreature(
                                ASPECT_RAVEN,
                                AspectRavenSummon,
    ...
    struct npc_omenAI : public ScriptedAI {
        npc_omenAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            me->GetMotionMaster()->MovePoint(
                1, 7549.977f, -2855.137f, 456.9678f);
        }
    ...
    struct npc_giant_spotlightAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (events.ExecuteEvent() == EVENT_DESPAWN) {
                if (GameObject* trap =
                        me->FindNearestGameObject(GO_ELUNE_TRAP_1, 5.0f))
                    trap->RemoveFromWorld();

                if (GameObject* trap =
                        me->FindNearestGameObject(GO_ELUNE_TRAP_2, 5.0f))
                    trap->RemoveFromWorld();

                if (Creature* omen =
                        me->FindNearestCreature(NPC_OMEN, 5.0f, false))
