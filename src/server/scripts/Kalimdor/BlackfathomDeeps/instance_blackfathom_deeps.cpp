...
public:
    instance_blackfathom_deeps()
        : InstanceMapScript("instance_blackfathom_deeps", 48)
    {
    }
    ...
    struct instance_blackfathom_deeps_InstanceMapScript
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            if (creature->IsSummon() &&
                (creature->GetEntry() == NPC_BARBED_CRUSTACEAN ||
                 creature->GetEntry() == NPC_AKU_MAI_SERVANT ||
                 creature->GetEntry() == NPC_MURKSHALLOW_SOFTSHELL ||
                 creature->GetEntry() == NPC_AKU_MAI_SNAPJAW))
                ++_requiredDeaths;
        }
        ...
        {
            if (unit->IsSummon() &&
                (unit->GetEntry() == NPC_BARBED_CRUSTACEAN ||
                 unit->GetEntry() == NPC_AKU_MAI_SERVANT ||
                 unit->GetEntry() == NPC_MURKSHALLOW_SOFTSHELL ||
                 unit->GetEntry() == NPC_AKU_MAI_SNAPJAW)) {
        ...
        void WriteSaveDataMore(std::ostringstream& data) override
        {
            data << _encounters[0] << ' ' << _encounters[1] << ' '
                 << _encounters[2] << ' ' << _encounters[3] << ' '
                 << _encounters[4] << ' ' << _encounters[5];
        }

        bool IsFireEventDone()
        {
            return _encounters[TYPE_FIRE1] == DONE &&
                   _encounters[TYPE_FIRE2] == DONE &&
                   _encounters[TYPE_FIRE3] == DONE &&
                   _encounters[TYPE_FIRE4] == DONE;
        }
...
void AddSC_instance_blackfathom_deeps() { new instance_blackfathom_deeps(); }
