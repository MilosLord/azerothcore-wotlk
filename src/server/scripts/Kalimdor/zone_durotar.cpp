        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (events.ExecuteEvent() == EVENT_CHECK_SUMMON_AURA) {
                ...
                GetCreatureListWithEntryInGrid(
                    tigers, me, NPC_TIGER_VEHICLE, 15.0f);
                if (!tigers.empty()) {
                    for (std::list<Creature*>::iterator itr = tigers.begin();
                         itr != tigers.end();
                         ++itr) {
                        if (!(*itr)->IsSummon())
                            continue;
                        ...
                        if (Unit* summoner =
                                (*itr)->ToTempSummon()->GetSummonerUnit())
                            if (!summoner->HasAura(SPELL_NO_SUMMON_AURA) &&
                                ...
                                summoner->CastSpell(
                                    summoner, SPELL_SUMMON_MATRIARCH, true);
    ...
    struct npc_tiger_matriarchAI : public ScriptedAI {
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            if (Player* player = summoner->ToPlayer()) {
                if (Vehicle* vehicle = player->GetVehicle()) {
                    ...
                    if (Unit* tiger =
                            ObjectAccessor::GetUnit(*me, _tigerGuid)) {
        ...
        void KilledUnit(Unit* victim) override
        {
            ...
            if (Unit* vehSummoner = victim->ToTempSummon()->GetSummonerUnit()) {
                ...
                vehSummoner->RemoveAurasDueToSpell(
                         ...
                         SpellSchoolMask) override
        {
            ...
            if (HealthBelowPct(20)) {
                ...
                if (Unit* vehSummoner =
                        attacker->ToTempSummon()->GetSummonerUnit()) {
                    vehSummoner->AddAura(SPELL_SUMMON_ZENTABRA_TRIGGER,
                                         vehSummoner);
                    vehSummoner->CastSpell(
                        vehSummoner, SPELL_SUMMON_ZENTABRA, true);
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_NOSUMMON: // Reapply SPELL_NO_SUMMON_AURA
                    if (Unit* tiger =
                        ...
                        if (tiger->IsSummon())
                            if (Unit* vehSummoner =
                                    tiger->ToTempSummon()->GetSummonerUnit())
...
uint32 const trollmodel[] = {
    11665, 11734, 11750, 12037, 12038, 12042, 12049, 12849, 13529, 14759,
    15570, 15701, 15702, 1882,  1897,  1976,  2025,  27286, 2734,  2735,
    4084,  4085,  4087,  4089,  4231,  4357,  4358,  4360,  4361,  4362,
    4363,  4370,  4532,  4537,  4540,  4610,  6839,  7037,  9767,  9768};
...
public:
    ...
    struct npc_troll_volunteerAI : public ScriptedAI {
        ...
        // This is needed for mount check aura to know what mountmodel the npc
        // got stored
        uint32 GetMountId() { return _mountModel; }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_AOE_TURNIN &&
                caster->GetEntry() == NPC_URUZIN && !_complete) {
                ...
                me->GetMotionMaster()->MovePoint(POINT_URUZIN,
                                                 caster->GetPositionX(),
                                                 caster->GetPositionY(),
                                                 caster->GetPositionZ());
            }
    ...
    class spell_mount_check_AuraScript : public AuraScript {
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (owner->IsMounted() && !target->IsMounted()) {
                if (VolunteerAI* volunteerAI =
                        CAST_AI(VolunteerAI, target->GetAI()))
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_mount_check_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
...
public:
    ...
    class spell_voljin_war_drums_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_voljin_war_drums_SpellScript::HandleDummy,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    class spell_voodoo_SpellScript : public SpellScript {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            uint32 spellid = RAND(SPELL_BREW,
                                  SPELL_GHOSTLY,
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_voodoo_SpellScript::HandleDummy,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
