...
public:
    ...
    struct npc_cork_gizeltonAI : public npc_escortAI {
        ...
        void SetGUID(ObjectGuid playerGUID, int32 faction) override
        {
            ...
            if (Creature* active =
                    !headNorth ? me
                               : ObjectAccessor::GetCreature(*me, summons[0]))
        ...
        void RemoveSummons()
        {
            for (uint8 i = 0; i < MAX_CARAVAN_SUMMONS; ++i) {
                if (Creature* summon =
                        ObjectAccessor::GetCreature(*me, summons[i]))
            ...
            for (uint8 i = 0; i < MAX_CARAVAN_SUMMONS; ++i)
                if (Creature* summon =
                    ...
                    summon->GetMotionMaster()->MoveFollow(
                        me, dist, M_PI, MOTION_SLOT_ACTIVE);
                    dist += (i == 1 ? 9.5f : 3.0f);
                }
        }
        ...
        void RelocateSummons()
        {
            for (uint8 i = 0; i < MAX_CARAVAN_SUMMONS; ++i)
                if (Creature* summon =
                        ObjectAccessor::GetCreature(*me, summons[i]))
                    summon->SetHomePosition(me->GetPositionX(),
                                            me->GetPositionY(),
                                            me->GetPositionZ(),
                                            me->GetOrientation());
        }
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 103:
                if (Player* player =
                        ObjectAccessor::GetPlayer(*me, _playerGUID)) {
                    if (CheckCaravan())
                        player->GroupEventHappens(QUEST_BODYGUARD_FOR_HIRE,
                                                  player);
                    else
            ...
            case 235:
                if (Player* player =
                        ObjectAccessor::GetPlayer(*me, _playerGUID)) {
                    if (CheckCaravan())
                        player->GroupEventHappens(QUEST_GIZELTON_CARAVAN,
                                                  player);
                    else
        ...
        void UpdateEscortAI(uint32 diff) override
        {
            ...
            case EVENT_RESUME_PATH:
                SetEscortPaused(false);
                if (Creature* talker = headNorth ? me
            ...
            case EVENT_WAIT_FOR_ASSIST:
                SetEscortPaused(false);
                if (Creature* active = !headNorth ? me
                                                  : ObjectAccessor::GetCreature(
                                                        *me, summons[0]))
                    active->RemoveNpcFlag(UNIT_NPC_FLAG_QUESTGIVER);
                break;
...
public:
    ...
    struct npc_aged_dying_ancient_kodoAI : public ScriptedAI {
        ...
        {
            me->UpdateEntry(
                RAND(NPC_AGED_KODO, NPC_DYING_KODO, NPC_ANCIENT_KODO),
                nullptr,
                false);
        }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_KODO_KOMBO_ITEM) {
                if (!(caster->HasAura(SPELL_KODO_KOMBO_PLAYER_BUFF) ||
                      me->HasAura(SPELL_KODO_KOMBO_DESPAWN_BUFF)) &&
                    (me->GetEntry() == NPC_AGED_KODO ||
                    ...
                    me->GetMotionMaster()->MoveFollow(
                        caster, PET_FOLLOW_DIST, me->GetFollowAngle());

                    caster->CastSpell(
                        caster, SPELL_KODO_KOMBO_PLAYER_BUFF, true);
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->HasAura(SPELL_KODO_KOMBO_PLAYER_BUFF) &&
            creature->HasAura(SPELL_KODO_KOMBO_DESPAWN_BUFF)) {
