...
public:
    instance_wailing_caverns()
        : InstanceMapScript("instance_wailing_caverns", 43)
    {
    }
    ...
    struct instance_wailing_caverns_InstanceMapScript : public InstanceScript {
        instance_wailing_caverns_InstanceMapScript(Map* map)
            : InstanceScript(map)
        {
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            if (type != TYPE_MUTANUS &&
                _encounters[TYPE_LORD_COBRAHN] == DONE &&
                _encounters[TYPE_LORD_PYTHAS] == DONE &&
                _encounters[TYPE_LADY_ANACONDRA] == DONE &&
                _encounters[TYPE_LORD_SERPENTIS] == DONE) {
                instance->LoadGrid(-134.97f, 125.402f);
                if (Creature* disciple =
                        instance->GetCreature(DiscipleOfNaralexGUID))
...
void AddSC_instance_wailing_caverns() { new instance_wailing_caverns(); }
