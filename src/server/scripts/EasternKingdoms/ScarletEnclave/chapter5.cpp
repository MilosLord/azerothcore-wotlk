...
enum LightOfDawnGOs { GO_HOLY_LIGHTNING = 191301, GO_LIGHT_OF_DAWN = 191330 };
...
const Position LightOfDawnFightPos[] = {{2279.68f, -5256.75f, 79.79f, 4.8f},
...
public:
    DelayedSummonEvent(Unit* owner, uint32 entry, Position pos)
        : _owner(owner), _entry(entry), _pos(pos)
    {
    }

    bool Execute(uint64 /*eventTime*/, uint32 /*updateTime*/) override
    {
        _owner->SummonCreature(
            _entry, _pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000);
        return true;
    }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(12801) == QUEST_STATUS_INCOMPLETE &&
            !creature->AI()->GetData(WORLD_STATE_SOLDIERS_ENABLE))
    ...
    struct npc_highlord_darion_mograineAI : public ScriptedAI {
        ...
        {
            if (param == ACTION_START_EVENT && !startTimeRemaining &&
                events.Empty()) {
                ...
                for (Map::PlayerList::const_iterator itr = players.begin();
                    ...
                    if (Player* player = itr->GetSource())
                        if (player->GetPhaseMask() &
                            128) // Xinef: client skips players without chapter
                                 // 5 aura anyway, speedup
                            player->SendUpdateWorldState(id, state);
        }

        void SendInitialWorldStates()
        {
            SendUpdateWorldState(WORLD_STATE_DEFENDERS_COUNT,
                                 GetData(WORLD_STATE_DEFENDERS_COUNT));
            SendUpdateWorldState(WORLD_STATE_SCOURGE_COUNT,
                                 GetData(WORLD_STATE_SCOURGE_COUNT));
            SendUpdateWorldState(WORLD_STATE_SOLDIERS_ENABLE,
                                 GetData(WORLD_STATE_SOLDIERS_ENABLE));
            SendUpdateWorldState(WORLD_STATE_COUNTDOWN_ENABLE,
                                 GetData(WORLD_STATE_COUNTDOWN_ENABLE));
            SendUpdateWorldState(WORLD_STATE_COUNTDOWN_TIME,
                                 GetData(WORLD_STATE_COUNTDOWN_TIME));
            SendUpdateWorldState(WORLD_STATE_EVENT_BEGIN_ENABLE,
                                 GetData(WORLD_STATE_EVENT_BEGIN_ENABLE));
        }
        ...
        void JustSummoned(Creature* cr) override
        {
            ...
            if (me->IsInCombat() &&
                cr->GetEntry() != NPC_HIGHLORD_TIRION_FORDRING &&
                ...
                if (Unit* target = cr->SelectNearbyTarget(nullptr, 10.0f))
                    if (target->GetTypeId() == TYPEID_UNIT)
                        target->GetMotionMaster()->MoveCharge(
                            pos.GetPositionX(),
                            pos.GetPositionY(),
                            pos.GetPositionZ(),
                            me->GetSpeed(MOVE_RUN));
                cr->GetMotionMaster()->MoveCharge(pos.GetPositionX(),
                                                  pos.GetPositionY(),
                                                  pos.GetPositionZ(),
                                                  me->GetSpeed(MOVE_RUN));
            }

            if (battleStarted == ENCOUNTER_STATE_OUTRO &&
                cr->GetEntry() == NPC_DEFENDER_OF_THE_LIGHT) {
        ...
        void SummonedCreatureDies(Creature* creature, Unit*) override
        {
            ...
            me->m_Events.AddEvent(
                new DelayedSummonEvent(me, creature->GetEntry(), *creature),
        ...
        {
            if (Creature* tirion =
                    me->SummonCreature(NPC_HIGHLORD_TIRION_FORDRING,
                                       LightOfDawnPos[6],
                ...
                tirion->SummonGameObject(GO_HOLY_LIGHTNING,
                                         2254.84f,
                                         -5298.75f,
                                         82.168f,
                                         1.134f,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_START_COUNTDOWN_13: {
                ...
                for (SummonList::const_iterator itr = summons.begin();
                    ...
                    if (Creature* summon =
                        ...
                        summon->GetMotionMaster()->MovePoint(1,
                                                             pos.GetPositionX(),
                                                             pos.GetPositionY(),
                                                             pos.GetPositionZ(),
                                                             true,
            ...
            case EVENT_FINISH_FIGHT_2: {
                ...
                if (Creature* orbaz =
                        GetEntryFromSummons(NPC_ORBAZ_BLOODBANE)) {
                    orbaz->SetReactState(REACT_PASSIVE);
                    orbaz->AI()->Talk(EMOTE_LIGHT_OF_DAWN04);
                    orbaz->GetMotionMaster()->MovePoint(
                        2, LightOfDawnPos[2], true, true);
                ...
                me->SummonCreature(NPC_DEFENDER_OF_THE_LIGHT,
                                   2276.66f,
                                   -5273.60f,
                                   81.86f,
                                   5.14f,
            ...
            case EVENT_FINISH_FIGHT_3:
                if (Creature* koltira =
                        GetEntryFromSummons(NPC_KOLTIRA_DEATHWEAVER)) {
                    koltira->SetWalk(true);
                    koltira->SetHomePosition(*koltira);
                    koltira->CastSpell(koltira, SPELL_THE_LIGHT_OF_DAWN, false);
                    koltira->GetMotionMaster()->MoveCharge(
                        LightOfDawnPos[3].GetPositionX(),
                        LightOfDawnPos[3].GetPositionY(),
                        LightOfDawnPos[3].GetPositionZ(),
                        4.0f,
                        2);
                }
                break;
            case EVENT_FINISH_FIGHT_4:
                if (Creature* thassarin = GetEntryFromSummons(NPC_THASSARIAN)) {
                    ...
                    thassarin->CastSpell(
                        thassarin, SPELL_THE_LIGHT_OF_DAWN, false);
                    thassarin->GetMotionMaster()->MoveCharge(
                        LightOfDawnPos[4].GetPositionX(),
                        LightOfDawnPos[4].GetPositionY(),
                        LightOfDawnPos[4].GetPositionZ(),
                        4.0f,
                        2);
                }
                break;
            case EVENT_FINISH_FIGHT_5:
                ...
                me->GetMotionMaster()->MoveCharge(
                    LightOfDawnPos[5].GetPositionX(),
                    LightOfDawnPos[5].GetPositionY(),
                    LightOfDawnPos[5].GetPositionZ(),
                    4.0f,
                    2);

                if (Creature* tirion =
                        GetEntryFromSummons(NPC_HIGHLORD_TIRION_FORDRING))
            ...
            case EVENT_OUTRO_SCENE_6:
                if (Creature* alex =
                        me->SummonCreature(NPC_HIGHLORD_ALEXANDROS_MOGRAINE,
                                           LightOfDawnPos[7].GetPositionX(),
                                           LightOfDawnPos[7].GetPositionY(),
                                           LightOfDawnPos[7].GetPositionZ(),
                                           LightOfDawnPos[7].GetOrientation(),
                                           TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
                                           300000)) {
                    alex->SetUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
                    alex->GetMotionMaster()->MovePoint(
                        0,
                        LightOfDawnPos[8].GetPositionX(),
                        LightOfDawnPos[8].GetPositionY(),
                        LightOfDawnPos[8].GetPositionZ());
                    alex->CastSpell(
                        alex, SPELL_ALEXANDROS_MOGRAINE_SPAWN, true);
                    // alex->AI()->Talk(EMOTE_LIGHT_OF_DAWN06);
                }
                break;
            case EVENT_OUTRO_SCENE_7:
                if (Creature* alex =
                        GetEntryFromSummons(NPC_HIGHLORD_ALEXANDROS_MOGRAINE)) {
            ...
            case EVENT_OUTRO_SCENE_10:
                if (Creature* darion =
                        me->SummonCreature(NPC_DARION_MOGRAINE,
                                           LightOfDawnPos[9].GetPositionX(),
                                           LightOfDawnPos[9].GetPositionY(),
                                           LightOfDawnPos[9].GetPositionZ(),
                                           LightOfDawnPos[9].GetOrientation(),
                                           TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
                                           300000)) {
            ...
            case EVENT_OUTRO_SCENE_11:
                if (Creature* darion =
                        GetEntryFromSummons(NPC_DARION_MOGRAINE)) {
                    // darion->AI()->Talk(EMOTE_LIGHT_OF_DAWN07);
                    darion->GetMotionMaster()->MovePoint(
                        0,
                        LightOfDawnPos[10].GetPositionX(),
                        LightOfDawnPos[10].GetPositionY(),
                        LightOfDawnPos[10].GetPositionZ());
                }
                break;
            ...
            case EVENT_OUTRO_SCENE_16:
                ...
                if (Creature* tirion =
                        GetEntryFromSummons(NPC_HIGHLORD_TIRION_FORDRING))
                    tirion->GetMotionMaster()->MovePoint(
                        0,
                        LightOfDawnPos[11].GetPositionX(),
                        LightOfDawnPos[11].GetPositionY(),
                        LightOfDawnPos[11].GetPositionZ());
                break;
            ...
            case EVENT_OUTRO_SCENE_20:
                if (Creature* lk =
                        me->SummonCreature(NPC_THE_LICH_KING,
                                           LightOfDawnPos[12].GetPositionX(),
                                           LightOfDawnPos[12].GetPositionY(),
                                           LightOfDawnPos[12].GetPositionZ(),
                                           LightOfDawnPos[12].GetOrientation(),
                                           TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
                                           300000))
            ...
            case EVENT_OUTRO_SCENE_22:
                if (Creature* lk = GetEntryFromSummons(NPC_THE_LICH_KING)) {
                    ...
                    if (Creature* alex = GetEntryFromSummons(
            ...
            case EVENT_OUTRO_SCENE_25:
                if (Creature* lk = GetEntryFromSummons(NPC_THE_LICH_KING))
                    lk->GetMotionMaster()->MovePoint(
                        0,
                        LightOfDawnPos[13].GetPositionX(),
                        LightOfDawnPos[13].GetPositionY(),
                        LightOfDawnPos[13].GetPositionZ());
                break;
                        ...
                        if (Creature* summon =
                            ...
                            if (summon->GetEntry() <=
                                ...
                                summon->GetMotionMaster()->MovePoint(
                                    3,
                                    lk->GetPositionX() + 2.0f * cos(o),
                                    lk->GetPositionY() + 2.0f * std::sin(o),
                                    lk->GetPositionZ());
            ...
            case EVENT_OUTRO_SCENE_34:
                for (SummonList::const_iterator itr = summons.begin();
                    ...
                    if (Creature* summon =
                            ObjectAccessor::GetCreature(*me, *itr))
                        if (summon->GetEntry() <= NPC_RIMBLAT_EARTHSHATTER &&
            ...
            case EVENT_OUTRO_SCENE_38:
                if (Creature* tirion =
                    ...
                    tirion->CastSpell(
                        me, SPELL_REBIRTH_OF_THE_ASHBRINGER, true);
                    tirion->SummonGameObject(GO_LIGHT_OF_DAWN,
                                             tirion->GetPositionX(),
                                             tirion->GetPositionY(),
                                             tirion->GetPositionZ(),
                                             tirion->GetOrientation(),
                                             0,
                                             0,
                                             0,
                                             0,
            ...
            case EVENT_OUTRO_SCENE_43:
                if (Creature* tirion =
                    ...
                    tirion->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                           EMOTE_STATE_READY2H);
            ...
            case EVENT_OUTRO_SCENE_47:
                ...
                if (Creature* tirion =
                    ...
                    tirion->GetMotionMaster()->MovePoint(
                        4,
                        me->GetPositionX() + 2.0f * cos(o),
                        me->GetPositionY() + 2.0f * std::sin(o),
                        me->GetPositionZ(),
                        false);
                    tirion->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                           EMOTE_ONESHOT_NONE);
            ...
            case EVENT_OUTRO_SCENE_50:
                if (Creature* tirion =
                    ...
                    tirion->GetMotionMaster()->MovePoint(
                        4,
                        LightOfDawnPos[14].GetPositionX(),
                        LightOfDawnPos[14].GetPositionY(),
                        LightOfDawnPos[14].GetPositionZ());
                break;
            case EVENT_OUTRO_SCENE_51:
                if (Creature* tirion =
                    ...
                    tirion->GetMotionMaster()->MovePoint(
                        4,
                        LightOfDawnPos[15].GetPositionX(),
                        LightOfDawnPos[15].GetPositionY(),
                        LightOfDawnPos[15].GetPositionZ());
                break;
                ...
                if (!PlayerList.IsEmpty()) {
                    for (Map::PlayerList::const_iterator i = PlayerList.begin();
                        ...
                        if (i->GetSource()->IsAlive() &&
                            me->IsWithinDistInMap(i->GetSource(), 100))
...
public:
    spell_chapter5_light_of_dawn_aura()
        : SpellScriptLoader("spell_chapter5_light_of_dawn_aura")
    {
    }
    ...
    class spell_chapter5_light_of_dawn_aura_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_chapter5_light_of_dawn_aura_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_chapter5_light_of_dawn_aura_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
    ...
    class spell_chapter5_rebuke_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectLaunchTarget +=
                SpellEffectFn(spell_chapter5_rebuke_SpellScript::HandleLeapBack,
                              EFFECT_0,
