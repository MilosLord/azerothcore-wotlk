...
struct npc_eye_of_acherus : public ScriptedAI {
    ...
    {
        if (!apply) {
            me->GetCharmerOrOwner()->RemoveAurasDueToSpell(
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent()) {
            ...
            case EVENT_UNROOT:
                ...
                _events.ScheduleEvent(EVENT_LAUNCH_TOWARDS_DESTINATION,
                                      1s + 200ms);
                break;
            case EVENT_LAUNCH_TOWARDS_DESTINATION: {
                Position const EYE_DESTINATION_1 = {
                    2361.21f, -5660.45f, 496.744f, 0.0f};
                Position const EYE_DESTINATION_2 = {
                    2341.571f, -5672.797f, 538.3942f, 0.0f};
                Position const EYE_DESTINATION_3 = {
                    1957.4f, -5844.1f, 273.867f, 0.0f};
                Position const EYE_DESTINATION_4 = {
                    1758.01f, -5876.79f, 166.867f, 0.0f};
                ...
                me->GetMotionMaster()->MovePoint(EYE_POINT_DESTINATION_0,
                                                 EYE_DESTINATION_1);
                me->GetMotionMaster()->MovePoint(EYE_POINT_DESTINATION_1,
                                                 EYE_DESTINATION_2);
                me->GetMotionMaster()->MovePoint(EYE_POINT_DESTINATION_2,
                                                 EYE_DESTINATION_3);
                me->GetMotionMaster()->MovePoint(EYE_POINT_DESTINATION_3,
                                                 EYE_DESTINATION_4);
...
public:
    ...
    class spell_q12641_death_comes_from_on_high_summon_ghouls_SpellScript
        : public SpellScript {
        PrepareSpellScript(
...
public:
    ...
    {
        if (player->GetQuestStatus(QUEST_DEATH_CHALLENGE) ==
    ...
    struct npc_death_knight_initiateAI : public CombatAI {
        ...
        void SpellHit(Unit* caster, SpellInfo const* pSpell) override
        {
            if (!_duelInProgress && pSpell->Id == SPELL_DUEL) {
                ...
                timer = 600000; // clear playerGUIDs after 10 minutes if no one
                                // initiates a duel
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_duelInProgress) {
                if (me->GetVictim()->GetGUID() == _duelGUID &&
                    me->GetVictim()->HealthBelowPct(10)) {
                    me->GetVictim()->CastSpell(
                        me->GetVictim(), 7267, true); // beg
...
public:
    spell_item_gift_of_the_harvester()
        : SpellScriptLoader("spell_item_gift_of_the_harvester")
    {
    }
    ...
    class spell_item_gift_of_the_harvester_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_item_gift_of_the_harvester_SpellScript::CheckRequirement);
        }
    };
...
public:
    spell_q12698_the_gift_that_keeps_on_giving()
        : SpellScriptLoader("spell_q12698_the_gift_that_keeps_on_giving")
    {
    }

    class spell_q12698_the_gift_that_keeps_on_giving_SpellScript
        : public SpellScript {
        PrepareSpellScript(
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_GHOUL_MOVE_TO_PIT:
                me->GetMotionMaster()->MovePoint(
                    1, 2364.77f, -5776.14f, 151.36f);
                if (Creature* gothik =
                        ObjectAccessor::GetCreature(*me, gothikGUID))
            ...
            case EVENT_GHOUL_RESTORE_STATE:
                ...
                if (Player* owner = me->GetCharmerOrOwnerPlayerOrPlayerItself())
                    me->GetMotionMaster()->MoveFollow(
                        owner, PET_FOLLOW_DIST, frand(0.0f, 2 * M_PI));
...
public:
    ...
    struct npc_dkc1_gothikAI : public ScriptedAI {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!who->IsImmuneToNPC() && who->GetEntry() == NPC_GHOUL &&
                me->IsWithinDistInMap(who, 10.0f))
                if (Unit* owner = who->GetOwner())
                    if (Player* player = owner->ToPlayer()) {
                        Creature* creature = who->ToCreature();
                        if (player->GetQuestStatus(12698) ==
                            QUEST_STATUS_INCOMPLETE)
...
public:
    ...
    struct npc_scarlet_cannonAI : public VehicleAI {
        ...
        void   PassengerBoarded(Unit* /*passenger*/,
                                int8 /*seatId*/,
                                bool apply) override
        {
                ...
                if (summonAttackers >= 15000) {
                    for (uint8 i = 0; i < 15; ++i)
                        if (Creature* summon = me->SummonCreature(
                                28834 /*NPC_SCARLET_FLEET_DEFENDER*/,
...
uint32 acherus_soul_prison[12] = {191577,
                                  191580,
                                  191581,
                                  191582,
                                  191583,
            ...
            case PHASE_CHAINED:
                if (!anchorGUID) {
                    ...
                    for (uint8 i = 0; i < 12; ++i) {
                        if (GameObject* temp_prison = me->FindNearestGameObject(
                                acherus_soul_prison[i], 100)) {
            ...
            case PHASE_TO_EQUIP:
                if (wait_timer) {
                    if (wait_timer > diff)
                        ...
                        me->GetMotionMaster()->MovePoint(
                            1, anchorX, anchorY, me->GetPositionZ());
                        // LOG_DEBUG("scripts.ai", "npc_unworthy_initiateAI:
                        // move to {} {} {}", anchorX, anchorY,
                        // me->GetPositionZ());
...
public:
    ...
    struct npc_unworthy_initiate_anchorAI : public PassiveAI {
        ...
        ObjectGuid GetGUID(int32 /*id*/) const override { return prisonerGUID; }
    };
};
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        if (Creature* anchor = go->FindNearestCreature(29521, 15))
            if (ObjectGuid prisonerGUID = anchor->AI()->GetGUID())
                if (Creature* prisoner =
                        ObjectAccessor::GetCreature(*player, prisonerGUID))
                    CAST_AI(npc_unworthy_initiate::npc_unworthy_initiateAI,
                            prisoner->AI())
                        ->EventStart(anchor, player);
...
enum Spells_SM { SPELL_CART_CHECK = 54173, SPELL_CART_DRAG = 52465 };
...
public:
    ...
    struct npc_scarlet_miner_cartAI : public PassiveAI {
        npc_scarlet_miner_cartAI(Creature* creature) : PassiveAI(creature)
        {
            ...
            me->SetDisplayId(
                me->GetCreatureTemplate()->Modelid1); // Modelid2 is a horse.
        }
        ...
        void DoAction(int32 /*param*/) override
        {
            if (Creature* miner = ObjectAccessor::GetCreature(*me, minerGUID)) {
                ...
                // Not 100% correct, but movement is smooth. Sometimes miner
                // walks faster than normal, this speed is fast enough to keep
                // up at those times.
        ...
        void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply) override
        {
            ...
            if (!apply)
                if (Creature* miner =
                        ObjectAccessor::GetCreature(*me, minerGUID))
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 23:
                if (Creature* car = ObjectAccessor::GetCreature(*me, carGUID)) {
                    car->SetPosition(car->GetPositionX(),
                                     car->GetPositionY(),
                                     me->GetPositionZ() + 1,
                                     car->GetOrientation());
        ...
        void UpdateAI(uint32 diff) override
        {
            if (IntroPhase) {
                if (IntroTimer <= diff) {
                    if (IntroPhase == 1) {
                        if (Creature* car =
                                ObjectAccessor::GetCreature(*me, carGUID))
            ...
            if (Creature* miner =
                ...
                if (Creature* car = player->GetVehicleCreatureBase()) {
                    if (car->GetEntry() == 28817) {
                        ...
                        CAST_AI(npc_scarlet_miner::npc_scarlet_minerAI,
                                miner->AI())
                            ->InitCartQuest(player);
                    }
                }
            }
        }
...
class spell_death_knight_initiate_visual : public SpellScript {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_death_knight_initiate_visual::HandleScriptEffect,
            EFFECT_0,
            SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
