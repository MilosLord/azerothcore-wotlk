        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_PERSUASIVE_STRIKE &&
                ...
                if (Player* player = caster->ToPlayer()) {
                    if (player->GetQuestStatus(QUEST_HOW_TO_WIN_FRIENDS) ==
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_BREAKOUT) {
            ...
            if (npc_escortAI* pEscortAI =
                    CAST_AI(npc_koltira_deathweaver::npc_koltira_deathweaverAI,
                            creature->AI()))
    ...
    struct npc_koltira_deathweaverAI : public npc_escortAI {
        ...
        void EnterEvadeMode(EvadeReason /*why*/) override
        {
            ...
            if (HasEscortState(STATE_ESCORT_ESCORTING)) {
                ...
                LOG_DEBUG("scripts.ai",
                          "EscortAI has left combat and is now returning to "
                          "last point");
            }
            else {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 2:
                ...
                // me->UpdateEntry(NPC_KOLTIRA_ALT); //unclear if we must update
                // or not
            ...
            case 3:
                ...
                DoCast(
                    me,
                    SPELL_ANTI_MAGIC_ZONE); // cast again that makes bubble up
                break;
            case 4:
                me->ApplySpellImmune(
                    0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, false);
                SetRun(true);
                break;
            case 9:
        ...
        void SummonAcolyte(uint32 uiAmount)
        {
            for (uint32 i = 0; i < uiAmount; ++i)
                me->SummonCreature(NPC_CRIMSON_ACOLYTE,
                                   1642.329f,
                                   -6045.818f,
                                   127.583f,
                                   0.0f,
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (HasEscortState(STATE_ESCORT_PAUSED)) {
                if (m_uiWave_Timer <= uiDiff) {
                    ...
                    case 4: {
                        Creature* temp =
                            ObjectAccessor::GetCreature(*me, m_uiValrothGUID);
                        ...
                        if (!temp || !temp->IsAlive()) {
                            ...
                            m_uiWave_Timer = 2500;
                            return; // return, we don't want m_uiWave to
                                    // increment now
                        }
                        break;
                    }
        ...
        void UpdateAI(uint32 diff) override
        {
            if (uiStage && !me->IsInCombat()) {
                if (uiStage_timer <= diff) {
                    ...
                    case 1:
                        me->SetWalk(true);
                        if (GameObject* tree = me->FindNearestGameObject(
                                GO_INCONSPICUOUS_TREE, 40.0f)) {
                            Talk(SAY_TREE1);
                            float x, y, z;
...
######*/
// used by 29032, 29061, 29065, 29067, 29068, 29070, 29074, 29072, 29073, 29071
// but signed for 29032
...
public:
    ...
    struct npc_a_special_surpriseAI : public ScriptedAI {
        ...
        {
            if (PlayerGUID || who->GetTypeId() != TYPEID_PLAYER ||
                !who->IsWithinDist(me, INTERACTION_DISTANCE))
                return;
            ...
            if (PlayerGUID && !me->GetVictim() && me->IsAlive()) {
                if (ExecuteSpeech_Timer <= diff) {
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        if (Creature* Plaguefist = GetClosestCreatureWithEntry(
                                me, NPC_PLAGUEFIST, 85.0f)) {
