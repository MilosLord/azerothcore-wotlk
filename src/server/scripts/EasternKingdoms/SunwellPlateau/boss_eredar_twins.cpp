...
public:
    ...
    struct boss_sacrolashAI : public BossAI {
        boss_sacrolashAI(Creature* creature)
            : BossAI(creature, DATA_EREDAR_TWINS)
        {
        }
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_SISTER_DIED) {
                ...
                events.ScheduleEvent(EVENT_SPELL_CONFLAGRATION,
                                     timer - events.GetTimer());
            }
        }
        ...
        void EnterEvadeMode(EvadeReason why) override
        {
            ...
            if (Creature* alythess = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(NPC_GRAND_WARLOCK_ALYTHESS))) {
                if (!alythess->IsAlive())
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            else if (Creature* alythess = ObjectAccessor::GetCreature(
                         *me,
                         instance->GetGuidData(NPC_GRAND_WARLOCK_ALYTHESS)))
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            if (Unit* target =
                    SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_CONFOUNDING_BLOW:
                ...
                events.ScheduleEvent(EVENT_SPELL_CONFOUNDING_BLOW,
                                     urand(20000, 25000));
                break;
            ...
            case EVENT_SPELL_SHADOW_NOVA: {
                Unit* target =
                    SelectTarget(SelectTargetMethod::MaxThreat, 1, 100.0f);
                if (!target)
                    target = me->GetVictim();
                Talk(EMOTE_SHADOW_NOVA, target);
                Talk(YELL_SHADOW_NOVA);
                me->CastSpell(target, SPELL_SHADOW_NOVA, false);
                events.ScheduleEvent(EVENT_SPELL_SHADOW_NOVA,
                                     urand(30000, 35000));
                break;
            }
            case EVENT_SHADOW_IMAGE:
                me->SummonCreature(NPC_SHADOW_IMAGE,
                                   me->GetPositionX(),
                                   me->GetPositionY(),
                                   me->GetPositionZ(),
                                   me->GetOrientation(),
    ...
    struct boss_alythessAI : public BossAI {
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_SISTER_DIED) {
                ...
                uint32 timer =
                    events.GetNextEventTime(EVENT_SPELL_CONFLAGRATION);
        ...
        {
            BossAI::EnterEvadeMode(why);
            if (Creature* scorlash = ObjectAccessor::GetCreature(
                    *me, instance->GetGuidData(NPC_LADY_SACROLASH))) {
                if (!scorlash->IsAlive())
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            else if (Creature* scorlash = ObjectAccessor::GetCreature(
                         *me, instance->GetGuidData(NPC_LADY_SACROLASH)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPELL_FLAME_SEAR:
                me->CastCustomSpell(SPELL_FLAME_SEAR,
...
public:
    spell_eredar_twins_apply_dark_touched()
        : SpellScriptLoader("spell_eredar_twins_apply_dark_touched")
    {
    }

    class spell_eredar_twins_apply_dark_touched_SpellScript
        : public SpellScript {
        ...
        {
            AfterHit +=
                SpellHitFn(spell_eredar_twins_apply_dark_touched_SpellScript::
                               HandleApplyTouch);
        }
    };
...
public:
    spell_eredar_twins_apply_flame_touched()
        : SpellScriptLoader("spell_eredar_twins_apply_flame_touched")
    {
    }

    class spell_eredar_twins_apply_flame_touched_SpellScript
        : public SpellScript {
        ...
        {
            AfterHit +=
                SpellHitFn(spell_eredar_twins_apply_flame_touched_SpellScript::
                               HandleApplyTouch);
        }
    };
...
public:
    spell_eredar_twins_handle_touch()
        : SpellScriptLoader("spell_eredar_twins_handle_touch")
    {
    }
    ...
    class spell_eredar_twins_handle_touch_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_eredar_twins_handle_touch_SpellScript::CheckCast);
        }
    };
    ...
    class spell_eredar_twins_blaze_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_eredar_twins_blaze_SpellScript::HandleScript,
                EFFECT_1,
...
public:
    AreaTrigger_at_sunwell_eredar_twins()
        : AreaTriggerScript("at_sunwell_eredar_twins")
    {
    }
        ...
        if (InstanceScript* instance = player->GetInstanceScript())
            if (instance->GetBossState(DATA_EREDAR_TWINS_INTRO) != DONE) {
                ...
                if (Creature* creature = ObjectAccessor::GetCreature(
                        *player, instance->GetGuidData(NPC_LADY_SACROLASH)))
