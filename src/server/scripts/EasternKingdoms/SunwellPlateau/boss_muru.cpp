    ...
    struct boss_muruAI : public BossAI {
        ...
        void JustSummoned(Creature* summon) override
        {
            if (summon->GetEntry() == NPC_ENTROPIUS)
                summon->AI()->SetData(
                    DATA_ENRAGE_TIMER,
                    events.GetNextEventTime(EVENT_SPELL_ENRAGE));
            else {
                if (!summon->IsTrigger())
    ...
    struct boss_entropiusAI : public ScriptedAI {
        ...
        void EnterEvadeMode(EvadeReason why) override
        {
            if (InstanceScript* instance = me->GetInstanceScript())
                if (Creature* muru = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_MURU)))
                    ...
                    if (Player* player = itr->GetSource())
                        if (me->GetDistance2d(player) < 15.0f &&
                            ...
                            me->GetMotionMaster()->MovePoint(
                                ...
                                false,
                                true);
                            events.ScheduleEvent(EVENT_SWITCH_BLACK_HOLE_TARGET,
                                                 5000);
                            return;
                        }
...
public:
    spell_muru_summon_blood_elves_periodic()
        : SpellScriptLoader("spell_muru_summon_blood_elves_periodic")
    {
    }

    class spell_muru_summon_blood_elves_periodic_AuraScript
        : public AuraScript {
                         ...
                         AuraEffectHandleModes /*mode*/)
        {
            ...
            GetAura()
                ...
                ->SetPeriodicTimer(10000);
        }

        void OnPeriodic(AuraEffect const* /*aurEff*/)
        {
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_FURY_MAGE1, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_FURY_MAGE2, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_BERSERKER1, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_BERSERKER2, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_BERSERKER1, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_BERSERKER2, true);
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_muru_summon_blood_elves_periodic_AuraScript::HandleApply,
                EFFECT_0,
    ...
    class spell_muru_darkness_AuraScript : public AuraScript {
        ...
        void OnPeriodic(AuraEffect const* aurEff)
        {
            if (aurEff->GetTickNumber() == 3)
                for (uint8 i = 0; i < 8; ++i)
                    GetUnitOwner()->CastSpell(
                        GetUnitOwner(), SPELL_SUMMON_DARK_FIEND + i, true);
        }

        void Register() override
        {
            OnEffectPeriodic +=
                AuraEffectPeriodicFn(spell_muru_darkness_AuraScript::OnPeriodic,
                                     EFFECT_2,
                                     SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    spell_entropius_negative_energy()
        : SpellScriptLoader("spell_entropius_negative_energy")
    {
    }
    ...
    class spell_entropius_negative_energy_SpellScript : public SpellScript {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            Acore::Containers::RandomResize(
                targets,
                GetCaster()->GetAI()->GetData(DATA_NEGATIVE_ENERGY_TARGETS));
        }
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            if (Unit* target = GetHitUnit())
                GetCaster()->CastSpell(
                    target, SPELL_NEGATIVE_ENERGY_CHAIN, true);
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_entropius_negative_energy_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(
                spell_entropius_negative_energy_SpellScript::HandleScriptEffect,
                EFFECT_0,
...
public:
    spell_entropius_void_zone_visual()
        : SpellScriptLoader("spell_entropius_void_zone_visual")
    {
    }
    ...
    class spell_entropius_void_zone_visual_AuraScript : public AuraScript {
        ...
        {
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SUMMON_DARK_FIEND_ENTROPIUS, true);
        }

        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_entropius_void_zone_visual_AuraScript::HandleApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
...
public:
    spell_entropius_black_hole_effect()
        : SpellScriptLoader("spell_entropius_black_hole_effect")
    {
    }
    ...
    class spell_entropius_black_hole_effect_SpellScript : public SpellScript {
        ...
        void HandlePull(SpellEffIndex effIndex)
        {
            ...
            if (target->GetDistance(GetCaster()) < 5.0f) {
                ...
                pos.Relocate(GetCaster()->GetPositionX(),
                             GetCaster()->GetPositionY(),
                             GetCaster()->GetPositionZ() + 1.0f);

            float speedXY =
                float(GetSpellInfo()->Effects[effIndex].MiscValue) * 0.1f;
            float speedZ =
                target->GetDistance(pos) / speedXY * 0.5f * Movement::gravity;

            target->GetMotionMaster()->MoveJump(pos.GetPositionX(),
                                                pos.GetPositionY(),
                                                pos.GetPositionZ(),
                                                speedXY,
                                                speedZ);
        ...
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_entropius_black_hole_effect_SpellScript::HandlePull,
                EFFECT_0,
