...
enum Yells {
    ...
    SAY_GOOD_MADRIGOSA = 3 // Madrigosa deserved a far better fate. You did what
                           // had to be done, but this battle is far from over!
};
    ...
    struct boss_kalecgosAI : public BossAI {
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_ENRAGE || param == ACTION_ENRAGE_OTHER) {
                Talk(param == ACTION_ENRAGE ? SAY_KALEC_ENRAGE_SATH
                                            : SAY_SATH_ENRAGE_ME);
            ...
            if (me->HasAura(SPELL_BANISH) && sathBanished) {
                ...
                ClearPlayerAuras();
                if (Creature* Sath = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_SATHROVARR))) {
                    Sath->RemoveAllAuras();
                    Sath->GetMotionMaster()->MovementExpired();
                    Sath->SetReactState(REACT_PASSIVE);
                    Sath->NearTeleportTo(1696.20f,
                                         915.0f,
                                         DRAGON_REALM_Z,
                                         Sath->GetOrientation());
                }
        ...
        void JustDied(Unit* killer) override { BossAI::JustDied(killer); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SPAWN_SPECTRALS:
                me->SummonCreature(NPC_KALEC,
                                   1702.21f,
                                   931.7f,
                                   -74.56f,
                                   5.07f,
                                   TEMPSUMMON_MANUAL_DESPAWN);
                me->SummonCreature(NPC_SATHROVARR,
                                   1704.62f,
                                   927.78f,
                                   -73.9f,
                                   2.0f,
    ...
    struct boss_kalecAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (me->GetPositionY() < 750.0f) {
                ...
                me->GetMotionMaster()->MovePoint(
                    0, 1483.30f, 657.99f, 28.0f, false, true);
        ...
        void JustDied(Unit*) override
        {
            if (InstanceScript* instance = me->GetInstanceScript())
                if (Creature* kalecgos = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_KALECGOS)))
...
public:
    ...
    struct boss_sathrovarrAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override { Talk(SAY_SATH_AGGRO); }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_AGONY_CURSE:
                me->CastCustomSpell(SPELL_CURSE_OF_BOUNDLESS_AGONY,
                                    SPELLVALUE_MAX_TARGETS,
                                    1,
                                    me,
                                    false);
            ...
            case EVENT_CHECK_HEALTH:
                if (me->HealthBelowPct(10)) {
                    if (InstanceScript* instanceScript =
                        ...
                        if (Creature* kalecgos = ObjectAccessor::GetCreature(
                                *me,
                                instanceScript->GetGuidData(NPC_KALECGOS))) {
...
public:
    ...
    {
        return unit->GetPositionZ() < 50.0f ||
               unit->ToUnit()->HasAura(SPELL_SPECTRAL_EXHAUSTION) ||
               unit->GetGUID() == _victim->GetGUID();
    }
...
public:
    spell_kalecgos_spectral_blast_dummy()
        : SpellScriptLoader("spell_kalecgos_spectral_blast_dummy")
    {
    }
    ...
    class spell_kalecgos_spectral_blast_dummy_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_kalecgos_spectral_blast_dummy_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(
                spell_kalecgos_spectral_blast_dummy_SpellScript::HandleDummy,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_kalecgos_curse_of_boundless_agony()
        : SpellScriptLoader("spell_kalecgos_curse_of_boundless_agony")
    {
    }
        ...
        void OnPeriodic(AuraEffect const* aurEff)
        {
            if (aurEff->GetTickNumber() > 1 && aurEff->GetTickNumber() % 5 == 1)
                GetAura()
                    ...
                    ->SetAmount(aurEff->GetAmount() * 2);
        }

        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_kalecgos_curse_of_boundless_agony_AuraScript::OnRemove,
                EFFECT_0,
...
public:
    spell_kalecgos_spectral_realm_dummy()
        : SpellScriptLoader("spell_kalecgos_spectral_realm_dummy")
    {
    }
    ...
    class spell_kalecgos_spectral_realm_dummy_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_kalecgos_spectral_realm_dummy_SpellScript::CheckCast);
            OnEffectHitTarget +=
                SpellEffectFn(spell_kalecgos_spectral_realm_dummy_SpellScript::
                                  HandleScriptEffect,
                              EFFECT_0,
...
public:
    spell_kalecgos_spectral_realm()
        : SpellScriptLoader("spell_kalecgos_spectral_realm")
    {
    }
    ...
    class spell_kalecgos_spectral_realm_AuraScript : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_SPECTRAL_EXHAUSTION, true);
            GetUnitOwner()->CastSpell(
                GetUnitOwner(), SPELL_TELEPORT_NORMAL_REALM, true);
        }

        void Register() override
        {
            OnEffectRemove += AuraEffectRemoveFn(
                spell_kalecgos_spectral_realm_AuraScript::OnRemove,
                EFFECT_1,
