...
struct boss_kormok : public ScriptedAI {
    ...
    void JustDied(Unit* /*killer*/) override { Talk(TALK_DEATH); }
...
class spell_kormok_summon_bone_mages : public SpellScript {
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        for (uint32 i = 0; i < 2; ++i) {
            GetCaster()->CastSpell(
                GetCaster(), SummonMageSpells[urand(0, 3)], true);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kormok_summon_bone_mages::HandleScript,
                          EFFECT_0,
...
class spell_kormok_summon_bone_minions : public SpellScript {
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        for (uint32 i = 0; i < 4; ++i) {
            GetCaster()->CastSpell(
                GetCaster(), SPELL_SUMMON_BONE_MINION1 + i, true);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kormok_summon_bone_minions::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
