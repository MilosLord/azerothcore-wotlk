    ...
    struct instance_scholomance_InstanceMapScript : public InstanceScript {
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_KIRTONOS_THE_HERALD:
                ...
                case IN_PROGRESS:
                    ...
                    if (_kirtonosState == NOT_STARTED) {
                        if (Creature* kirtonos = instance->SummonCreature(
                                NPC_KIRTONOS, KirtonosSpawn)) {
                            kirtonos->AI()->DoAction(IN_PROGRESS);
                        }
                        if (GameObject* gate = instance->GetGameObject(
                                GetGuidData(GO_GATE_KIRTONOS))) {
                ...
                case FAIL:
                    ...
                    if (GameObject* brazier = instance->GetGameObject(
                            GetGuidData(GO_BRAZIER_KIRTONOS))) {
            ...
            case DATA_MINI_BOSSES:
                ++_miniBosses;
                if (_miniBosses == 6) {
                    if (Creature* Gandling =
                            instance->GetCreature(GandlingGUID)) {
                        Gandling->AI()->Talk(0);
                        Gandling->AI()->Reset();
                    }
                }
    ...
    class spell_scholomance_fixate_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectApply += AuraEffectApplyFn(
                spell_scholomance_fixate_AuraScript::HandleEffectApply,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(
                spell_scholomance_fixate_AuraScript::HandleEffectRemove,
                EFFECT_0,
                SPELL_AURA_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
...
public:
    spell_scholomance_boon_of_life()
        : SpellScriptLoader("spell_scholomance_boon_of_life")
    {
    }
    ...
    class spell_scholomance_boon_of_life_AuraScript : public AuraScript {
        ...
        {
            OnEffectRemove += AuraEffectRemoveFn(
                spell_scholomance_boon_of_life_AuraScript::OnRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
            AfterEffectApply += AuraEffectApplyFn(
                spell_scholomance_boon_of_life_AuraScript::OnApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                AURA_EFFECT_HANDLE_REAL);
        }
...
enum OccultistEntries { CASTER_ENTRY = 10472, DARK_SHADE_ENTRY = 11284 };
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 3:
                if (Unit* target = SelectTarget(
                        ...
                        0,
                        PowerUsersSelector(me, POWER_MANA, 20.0f, false))) {
