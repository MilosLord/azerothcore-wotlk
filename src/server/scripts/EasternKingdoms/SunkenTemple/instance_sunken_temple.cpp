    ...
    struct instance_sunken_temple_InstanceMapScript : public InstanceScript {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            if (creature->IsAlive() && creature->GetSpawnId() &&
                creature->GetCreatureType() == CREATURE_TYPE_DRAGONKIN &&
                creature->GetEntry() != NPC_SHADE_OF_ERANIKUS)
        ...
        void OnUnitDeath(Unit* unit) override
        {
            if (unit->GetTypeId() == TYPEID_UNIT &&
                unit->GetCreatureType() == CREATURE_TYPE_DRAGONKIN &&
            ...
            case GO_ATALAI_STATUE6:
                if (gameobject->GetEntry() < GO_ATALAI_STATUE1 + _statuePhase) {
                    instance->SummonGameObject(GO_ATALAI_LIGHT2,
                                               ...
                                               0.0f,
                                               0.0f);
                    gameobject->ReplaceAllGameObjectFlags(
                        GO_FLAG_NOT_SELECTABLE);
                }
                break;
            case GO_ATALAI_IDOL:
                if (_statuePhase == MAX_STATUE_PHASE)
                    gameobject->SummonGameObject(GO_IDOL_OF_HAKKAR,
                                                 -480.08f,
                                                 94.29f,
                                                 -189.72f,
                                                 1.571f,
            ...
            case DATA_DEFENDER_KILLED:
                ...
                if (_defendersKilled == DEFENDERS_COUNT) {
                    ...
                    if (Creature* jammal = instance->GetCreature(_jammalanGUID))
                        jammal->AI()->Talk(0);
                    if (GameObject* forcefield =
                            instance->GetGameObject(_forcefieldGUID))
            ...
            case DATA_ERANIKUS_FIGHT:
                for (ObjectGuid const& guid : _dragonkinList) {
                    if (Creature* creature = instance->GetCreature(guid))
                        if (instance->IsGridLoaded(creature->GetPositionX(),
        ...
        void Update(uint32 diff) override
        {
            ...
            case DATA_STATUES:
                ++_statuePhase;
                if (_statuePhase == MAX_STATUE_PHASE)
                    instance->SummonGameObject(GO_IDOL_OF_HAKKAR,
                                               -480.08f,
                                               94.29f,
                                               -189.72f,
                                               1.571f,
        ...
        {
            data << _encounters[0] << ' ' << _encounters[1] << ' '
                 << _encounters[2] << ' ' << _statuePhase << ' '
                 << _defendersKilled;
        }
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (player->GetInstanceScript() &&
            !player->FindNearestCreature(NPC_MALFURION_STORMRAGE, 15.0f) &&
            player->GetQuestStatus(QUEST_THE_CHARGE_OF_DRAGONFLIGHTS) ==
                QUEST_STATUS_REWARDED &&
            player->GetQuestStatus(QUEST_ERANIKUS_TYRANT_OF_DREAMS) !=
                QUEST_STATUS_REWARDED)
...
public:
    spell_temple_of_atal_hakkar_hex_of_jammal_an()
        : SpellScriptLoader("spell_temple_of_atal_hakkar_hex_of_jammal_an")
    {
    }
                      ...
                      AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
                if (caster->IsAlive() && caster->IsInCombat()) {
                    ...
                    caster->CastSpell(
                        GetTarget(), HEX_OF_JAMMAL_AN_CHARM, true);
                }
        }
...
public:
    ...
    class spell_temple_of_atal_hakkar_awaken_the_soulflayer_SpellScript
        ...
        void HandleSendEvent(SpellEffIndex effIndex)
        {
            ...
            if (!map || !instanceScript ||
                instanceScript->GetData(TYPE_HAKKAR_EVENT) != NOT_STARTED)
                return;

            Position pos = {-466.795f, 272.863f, -90.447f, 1.57f};
            if (TempSummon* summon =
                    map->SummonCreature(NPC_SHADE_OF_HAKKAR, pos)) {
