                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_ESCORTING) {
            ...
            if (npc_escortAI* pEscortAI =
                    CAST_AI(npc_deathstalker_erland::npc_deathstalker_erlandAI,
                            creature->AI()))
...
public:
                       ...
                       const Quest* quest) override
    {
        if (quest->GetQuestId() == QUEST_PYREWOOD_AMBUSH &&
            !CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())
                 ->QuestInProgress) {
            CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())
                ->QuestInProgress = true;
            CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())->Phase =
                0;
            CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())
                ->KillCount = 0;
            CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())
    ...
    struct pyrewood_ambushAI : public ScriptedAI {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (PlayerGUID)
                if (Player* player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
                    if (player->GetQuestStatus(QUEST_PYREWOOD_AMBUSH) ==
        ...
        {
            // LOG_INFO("scripts", "DEBUG: p({}) k({}) d({}) W({})", Phase,
            // KillCount, diff, WaitTimer);
...
/**
 *
 * @todo: Actual emote and BroadcastTextId need to be sniffed. Probably the
 * entire event to begin with.... There is a possibility that the unused texts
 * are chosen by random for specific parts of the speech. (making it look like
 * they are preset, when in fact, they are not)
 *
 */
...
public:
    ...
    struct npc_ravenclaw_apparitionAI : public NullCreatureAI {
        npc_ravenclaw_apparitionAI(Creature* creature)
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_APPA_OUTRO_CROWD:
                ...
                break;
            case EVENT_APPA_OUTRO_END: // Despawn for Apparition is handled via
                                       // Areatrigger SAI (5m)
                summons.DespawnAll();
                me->SetVisible(false);
                HasEnded = true;
