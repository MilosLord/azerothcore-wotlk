...
struct boss_netherspite : public BossAI {
    ...
    bool IsBetween(WorldObject* u1,
                   WorldObject* target,
                   WorldObject* u2) // the in-line checker
    {
        if (!u1 || !u2 || !target)
            return false;
        ...
        return (std::abs((xn - xp) * yh + (yp - yn) * xh - xn * yp + xp * yn) /
                    dist(xn, yn, xp, yp) <
                1.5f);
    }
    ...
    float dist(float xa,
               float ya,
               float xb,
               float yb) // auxiliary method for distance
    {
    ...
    void SummonPortals()
    {
        ...
        for (int i = 0; i < 3; ++i) {
            if (Creature* portal = me->SummonCreature(PortalID[i],
                                                      PortalCoord[pos[i]][0],
                                                      PortalCoord[pos[i]][1],
                                                      PortalCoord[pos[i]][2],
                                                      0,
    ...
    void UpdatePortals() // Here we handle the beams' behavior
    {
        ...
        {
            if (Creature* portal =
                    ObjectAccessor::GetCreature(*me, PortalGUID[j])) {
                ...
                if (Map* map = me->GetMap()) {
                    ...
                    for (Map::PlayerList::const_iterator i = players.begin();
                        ...
                        if (p && p->IsAlive() // alive
                            && (!target ||
                                target->GetDistance2d(portal) >
                                    p->GetDistance2d(
                                        portal)) // closer than current best
                            && !p->HasAura(PlayerDebuff[j]) // not exhausted
                            &&
                            !p->HasAura(
                                PlayerBuff[(j + 1) % 3]) // not on another beam
                ...
                // simple target switching isn't working -> using BeamerGUID to
                // cast (workaround)
                if (!current || target != current) {
                    BeamTarget[j] = target->GetGUID();
                    // remove currently beaming portal
                    if (Creature* beamer = ObjectAccessor::GetCreature(
                            *portal, BeamerGUID[j])) {
                    ...
                    if (Creature* beamer =
                            portal->SummonCreature(PortalID[j],
                                                   portal->GetPositionX(),
                                                   portal->GetPositionY(),
                                                   portal->GetPositionZ(),
                ...
                if (j == RED_PORTAL && me->GetVictim() != target &&
                    target->GetTypeId() == TYPEID_PLAYER) {
                    me->GetThreatMgr().AddThreat(
                        target, 100000.0f + DoGetThreat(me->GetVictim()));
                }
            }
        }
    }
    ...
    void DestroyPortals()
    {
        for (int i = 0; i < 3; ++i) {
            if (Creature* portal =
                    ObjectAccessor::GetCreature(*me, PortalGUID[i])) {
                portal->DisappearAndDie();
            }
            if (Creature* portal =
                    ObjectAccessor::GetCreature(*me, BeamerGUID[i])) {
                portal->DisappearAndDie();
            }
    ...
    void HandleDoors(bool open)
    {
        if (GameObject* door = ObjectAccessor::GetGameObject(
                *me, instance->GetGuidData(DATA_GO_MASSIVE_DOOR))) {
...
class spell_nether_portal_perseverence : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_nether_portal_perseverence::HandleApply,
                              EFFECT_2,
