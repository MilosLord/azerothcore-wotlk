...
ObjectData const gameObjectData[] = {
    {GO_SIDE_ENTRANCE_DOOR, DATA_GO_SIDE_ENTRANCE_DOOR}, {0, 0}};
    ...
    struct instance_karazhan_InstanceMapScript : public InstanceScript {
        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            case NPC_SHADOWBEAST:
                if (GetBossState(DATA_OPTIONAL_BOSS) == TO_BE_DECIDED) {
                    ++OptionalBossCount;
                    if (OptionalBossCount ==
                        OPTIONAL_BOSS_REQUIRED_DEATH_COUNT) {
                        switch (urand(NPC_HYAKISS_THE_LURKER,
                                      NPC_ROKAD_THE_RAVAGER)) {
                        case NPC_HYAKISS_THE_LURKER:
                            instance->SummonCreature(NPC_HYAKISS_THE_LURKER,
                                                     OptionalSpawn[0]);
                            break;
                        case NPC_SHADIKITH_THE_GLIDER:
                            instance->SummonCreature(NPC_SHADIKITH_THE_GLIDER,
                                                     OptionalSpawn[1]);
                            break;
                        case NPC_ROKAD_THE_RAVAGER:
                            instance->SummonCreature(NPC_ROKAD_THE_RAVAGER,
                                                     OptionalSpawn[2]);
                            break;
                        }
                    }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_CHESS_EVENT: {
                ...
                case SPECIAL: {
                    ...
                    for (ObjectGuid const& chessPieceGUID : _chessPiecesGUID) {
                        if (Creature* piece =
                                instance->GetCreature(chessPieceGUID)) {
            ...
            case DATA_CHESS_REINIT_PIECES:
                ...
                for (ObjectGuid const& medivhCheatFireGUID :
                     _medivhCheatFiresGUID) {
                    if (Creature* fire =
                            instance->GetCreature(medivhCheatFireGUID)) {
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            case DATA_OPERA_PERFORMANCE:
                if (state == DONE) {
                    ...
                    instance->UpdateEncounterState(
                        ENCOUNTER_CREDIT_KILL_CREATURE, 16812, nullptr);
                }
                else if (state == FAIL) {
                    HandleGameObject(m_uiStageDoorLeftGUID, true);
    ...
    class spell_karazhan_brittle_bones_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
                spell_karazhan_brittle_bones_AuraScript::CalcPeriodic,
                EFFECT_0,
                SPELL_AURA_DUMMY);
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_karazhan_brittle_bones_AuraScript::Update,
                EFFECT_0,
                SPELL_AURA_DUMMY);
        }
    };
...
public:
    ...
    class spell_karazhan_overload_AuraScript : public AuraScript {
        ...
        void PeriodicTick(AuraEffect const* auraEffect)
        {
            ...
            GetUnitOwner()->CastCustomSpell(
                ...
                int32(auraEffect->GetAmount() *
                      pow(2.0, auraEffect->GetTickNumber())),
                GetUnitOwner(),
                true);
        }
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_karazhan_overload_AuraScript::PeriodicTick,
                EFFECT_0,
...
public:
    ...
    class spell_karazhan_blink_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_karazhan_blink_SpellScript::HandleDummy,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
