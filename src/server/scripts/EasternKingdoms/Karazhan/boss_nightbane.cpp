...
enum Groups { GROUP_GROUND = 0, GROUP_FLYING = 1 };
...
float IntroWay[8][3] = {
    ...
    {-11163.00f, -1903.00f, 91.473f}}; // TODO: move to table
...
struct boss_nightbane : public BossAI {
    ...
    void Reset() override
    {
        ...
        ScheduleHealthCheckEvent({75, 50, 25}, [&] { TakeOff(); });
    }
    ...
    void EnterEvadeMode(EvadeReason why) override
    {
        ...
        me->GetMotionMaster()->MoveTakeoff(
            POINT_DESPAWN, -11013.246f, -1770.5212f, 166.50139f);
    }
    ...
    void ScheduleGround()
    {
        scheduler
            ...
            .Schedule(15s,
                      ...
                      [this](TaskContext context) {
                          DoCastRandomTarget(
                              SPELL_CHARRED_EARTH, 0, 100.0f, true);
                          context.Repeat(20s);
                      })
            .Schedule(10s,
                      GROUP_GROUND,
        ...
        scheduler
            .Schedule(2s,
                      ...
                      [this](TaskContext) {
                          ...
                          _skeletonscheduler.Schedule(
                              50ms, [this](TaskContext context) {
                                  // spawns skeletons every second until
                                  // skeletonCount is reached
    ...
    void DoCastOnFarAwayPlayers(uint32 spellid,
                                bool   triggered,
                                float  tresholddistance)
    {
        // resembles DoCastToAllHostilePlayers a bit/lot
        ThreatContainer::StorageType targets =
            me->GetThreatMgr().GetThreatList();
        for (ThreatContainer::StorageType::const_iterator itr = targets.begin();
             itr != targets.end();
             ++itr) {
            if (Unit* unit =
                    ObjectAccessor::GetUnit(*me, (*itr)->getUnitGuid())) {
                if (unit->IsPlayer() &&
                    !unit->IsWithinDist(me, tresholddistance, false)) {
    ...
    void TakeOff()
    {
        ...
        me->GetMotionMaster()->MovePoint(
            0, IntroWay[2][0], IntroWay[2][1], IntroWay[2][2]);
        ...
        scheduler.Schedule(45s, 60s, [this](TaskContext) {
            ...
            me->GetMotionMaster()->Clear(false);
            me->GetMotionMaster()->MovePoint(
                3, IntroWay[3][0], IntroWay[3][1], IntroWay[3][2]);

            _flying = true;
            scheduler.CancelGroup(GROUP_FLYING);
            scheduler.Schedule(2s, [this](TaskContext) { ScheduleGround(); });
        });
    }
        ...
        if (_intro) {
            if (_movePhase) {
                if (_movePhase >= 7) {
                    ...
                    me->GetMotionMaster()->MovePoint(
                        8, IntroWay[7][0], IntroWay[7][1], IntroWay[7][2]);
                }
                else {
                    me->GetMotionMaster()->MovePoint(_movePhase,
                                                     IntroWay[_movePhase][0],
                                                     IntroWay[_movePhase][1],
                                                     IntroWay[_movePhase][2]);
                }
                _movePhase = 0;
            }
            return;
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        if (InstanceScript* instance = go->GetInstanceScript()) {
            if (instance->GetData(DATA_NIGHTBANE) != DONE &&
                !go->FindNearestCreature(NPC_NIGHTBANE, 40.0f)) {
                if (Creature* nightbane =
                        instance->GetCreature(DATA_NIGHTBANE)) {
                    nightbane->GetMotionMaster()->MovePoint(
                        0, IntroWay[0][0], IntroWay[0][1], IntroWay[0][2]);
