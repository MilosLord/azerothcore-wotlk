...
struct boss_shade_of_aran : public BossAI {
    boss_shade_of_aran(Creature* creature)
        : BossAI(creature, DATA_ARAN), _atieshReaction(false)
    {
    }
    ...
    void Reset() override
    {
        ...
        // If this was omitted, the boss would start drinking on reset if the
        // mana was low on a wipe
        ...
        if (GameObject* libraryDoor = instance->instance->GetGameObject(
                instance->GetGuidData(DATA_GO_LIBRARY_DOOR))) {
    ...
    {
        return me->GetDistance2d(roomCenter.GetPositionX(),
                                 roomCenter.GetPositionY()) < 45.0f;
    }
    ...
    void KilledUnit(Unit* /*victim*/) override { Talk(SAY_KILL); }
                     ...
                     SpellSchoolMask  damageSchoolMask) override
    {
        ...
        if ((damagetype == DIRECT_DAMAGE ||
            ...
            _drinking && me->GetReactState() == REACT_PASSIVE) {
    ...
    void OnPowerUpdate(Powers /*power*/,
                       int32 /*gain*/,
                       int32 /*updateVal*/,
                       uint32 currentPower) override
    {
        // Should drink at 10%, need 10% mana for mass polymorph
        if (!_hasDrunk && me->GetMaxPower(POWER_MANA) &&
            (currentPower * 100 / me->GetMaxPower(POWER_MANA)) < 13.5) {
            _hasDrunk = true;
            ...
            _drinkScheduler
                .Schedule(0s,
                          ...
                          [this](TaskContext) {
                              ...
                              // If we set drinking earlier it will break when
                              // someone attacks aran while casting poly
                              _drinking = true;
                          })
                .Schedule(3s,
                          GROUP_DRINKING,
                ...
                .Schedule(12s, GROUP_DRINKING, [this](TaskContext) {
                    me->SetStandState(UNIT_STAND_STATE_STAND);
                    me->SetReactState(REACT_AGGRESSIVE);
                    me->SetPower(POWER_MANA,
                                 me->GetMaxPower(POWER_MANA) - 32000);
            ...
            .Schedule(
                ...
                [this](TaskContext) {
                    if (GameObject* libraryDoor =
                            instance->instance->GetGameObject(
                                instance->GetGuidData(DATA_GO_LIBRARY_DOOR))) {
            ...
            .Schedule(
                ...
                [this](TaskContext context) {
                    if (!_drinking) {
                        ...
                        std::list<uint32> normalSpells = {SPELL_ARCANE_MISSILE,
                                                          SPELL_FIREBALL,
                                                          SPELL_FROSTBOLT};
                        normalSpells.remove_if([&](uint32 spell) -> bool {
                            return !me->CanCastSpell(spell);
                        });
                        ...
                        if (!normalSpells.empty()) {
                            ...
                            // Otherwise, chase in melee range for auto attacks
                            // (and drink mana potion, if needed).
                            me->SetWalk(false);
                ...
                [this](TaskContext context) {
                    if (!_drinking) {
                        ...
                        std::copy_if(superSpells.begin(),
                                     ...
                                     [&](uint32 superSpell) -> bool {
                                         return superSpell != _lastSuperSpell;
                                     });
                        _lastSuperSpell =
                            allowedSpells[urand(0, allowedSpells.size() - 1)];

                        //  SelectRandomContainerElementIf produces unexpected
                        //  output. Reintroduce when issue is resolved: Sample
                        //  results:
                        //       Selected Super Spell: 3722304989
                        //       superSpells elements : 29969 29973 30004
                        //  _lastSuperSpell =
                        //  Acore::Containers::SelectRandomContainerElementIf(superSpells,
                        //  [&](uint32 superSpell) -> bool { return superSpell
                        //  != _lastSuperSpell; });

                        me->InterruptNonMeleeSpells(
                            true); // Super spell should have prio over normal
                                   // spells
    ...
    void HandleFinish()
    {
        for (auto const& target : _targets) {
            if (Unit* targetUnit = target->ToUnit()) {
                GetCaster()->CastSpell(
                    targetUnit, SPELL_FLAME_WREATH_RING, true);
            }
        }
    }
...
private:
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_flamewreath::FilterTargets,
                                          EFFECT_ALL,
...
class spell_flamewreath_aura : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spell*/) override
    {
        return ValidateSpellInfo(
            {SPELL_FLAME_WREATH_RAN_THRU, SPELL_FLAME_WREATH_EXPLOSION});
    }
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_DEFAULT &&
            ...
            if (Unit* target = GetTarget()) {
                if (target->IsPlayer()) {
                    target->CastSpell(
                        target, SPELL_FLAME_WREATH_RAN_THRU, true);
                    ...
                    target->m_Events.AddEventAtOffset(
                        [target] {
                            ...
                            target->CastSpell(
                                target, SPELL_FLAME_WREATH_EXPLOSION, true);
                        },
                        1s);
                }
            }
    ...
    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(spell_flamewreath_aura::OnRemove,
                                             EFFECT_0,
