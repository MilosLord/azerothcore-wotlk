...
SDName: Karazhan
SD%Complete: 100
SDComment: Support for Barnes (Opera controller) and Berthold (Doorman), Support
for Quest 9645. SDCategory: Karazhan EndScriptData */
...
static Dialogue OzDialogue[] = {{0, 6000}, {1, 18000}, {2, 9000}, {3, 15000}};

static Dialogue HoodDialogue[] = {
    {4, 6000}, {5, 10000}, {6, 14000}, {7, 15000}};

static Dialogue RAJDialogue[] = {
    {8, 5000}, {9, 7000}, {10, 14000}, {11, 14000}};
...
public:
    ...
    struct npc_barnesAI : public npc_escortAI {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 0:
                ...
                instance->HandleGameObject(
                    instance->GetGuidData(DATA_GO_STAGEDOORLEFT), true);
                break;
            case 4:
                TalkCount = 0;
                ...
                if (Creature* spotlight =
                        me->SummonCreature(NPC_SPOTLIGHT,
                                           me->GetPositionX(),
                                           me->GetPositionY(),
                                           me->GetPositionZ(),
                                           0.0f,
            ...
            case 8:
                if (m_uiEventId !=
                    EVENT_HOOD) // in red riding hood door should close when
                                // gossip with grandma is over
                {
                    instance->DoUseDoorOrButton(
                        instance->GetGuidData(DATA_GO_STAGEDOORLEFT));
                }
            ...
            case 9:
                PrepareEncounter();
                instance->DoUseDoorOrButton(
                    instance->GetGuidData(DATA_GO_CURTAINS));
                break;
            }
        }
        ...
        void PrepareEncounter()
        {
            LOG_DEBUG("scripts.ai",
                      "Barnes Opera Event - Introduction complete - preparing "
                      "encounter {}",
                      m_uiEventId);
            uint8 index = 0;
            uint8 count = 0;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (HasEscortState(STATE_ESCORT_PAUSED)) {
                if (TalkTimer <= diff) {
                    if (TalkCount > 3) {
                        if (Creature* pSpotlight = ObjectAccessor::GetCreature(
                                *me, m_uiSpotlightGUID))
                        ...
                        uint32 action) override
    {
        ...
        npc_barnesAI* pBarnesAI =
            CAST_AI(npc_barnes::npc_barnesAI, creature->AI());
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            ...
            SendGossipMenuFor(
                player, BARNES_TEXT_IS_READY2, creature->GetGUID());
            break;
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (InstanceScript* instance = creature->GetInstanceScript()) {
            ...
            if (instance->GetBossState(DATA_MOROES) == DONE &&
                instance->GetBossState(DATA_OPERA_PERFORMANCE) != DONE) {
                AddGossipItemFor(player,
                                 OZ_GOSSIP1_MID,
                                 OZ_GOSSIP1_OID,
                                 GOSSIP_SENDER_MAIN,
                ...
                if (instance->GetBossState(DATA_OPERA_PERFORMANCE) != FAIL) {
                    SendGossipMenuFor(
                        player, BARNES_TEXT_IS_READY, creature->GetGUID());
                }
                else {
                    SendGossipMenuFor(
                        player, BARNES_TEXT_WIPED, creature->GetGUID());
                }
    ...
    struct npc_image_of_medivhAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (instance && !instance->GetGuidData(DATA_IMAGE_OF_MEDIVH)) {
                Creature* Arcanagos =
                    me->SummonCreature(NPC_ARCANAGOS,
                                       ArcanagosPos[0],
                                       ArcanagosPos[1],
                                       ArcanagosPos[2],
                                       0,
        ...
        uint32 NextStep(uint32 nextStep)
        {
            ...
            case 2:
                if (Creature* arca =
                        ObjectAccessor::GetCreature((*me), ArcanagosGUID))
            ...
            case 12:
                if (Creature* arca =
                    ...
                    arca->GetMotionMaster()->MovePoint(
                        0, -11010.82f, -1761.18f, 156.47f);
            ...
            case 14:
                if (me->GetMap()->IsDungeon()) {
                    InstanceMap::PlayerList const& PlayerList =
                        me->GetMap()->GetPlayers();
                    for (InstanceMap::PlayerList::const_iterator i =
                             PlayerList.begin();
                         i != PlayerList.end();
                         ++i) {
                        if (i->GetSource()->GetQuestStatus(9645) ==
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (Step >= 7 && Step <= 8) {
                ...
                if (ATimer >= 6000) {
                    if (Unit* arca =
                            ObjectAccessor::GetUnit((*me), ArcanagosGUID))
...
public:
    at_karazhan_side_entrance()
        : OnlyOnceAreaTriggerScript("at_karazhan_side_entrance")
    {
    }
    ...
    bool _OnTrigger(Player* player, AreaTrigger const* /*at*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            if (instance->GetBossState(DATA_OPERA_PERFORMANCE) == DONE) {
                if (GameObject* door =
                        instance->GetGameObject(DATA_GO_SIDE_ENTRANCE_DOOR)) {
...
class spell_karazhan_temptation : public AuraScript {
    ...
    {
        OnEffectProc += AuraEffectProcFn(spell_karazhan_temptation::HandleProc,
                                         EFFECT_0,
