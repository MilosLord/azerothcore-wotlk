...
//                                                  ORI_W     ORI_NW      ORI_N
//                                                  ORI_NE      ORI_E
static std::array<float, MAX_ORI> orientations = {3.809080f,
                                                  3.022091f,
                                                  2.235102f,
                                                  1.448113f,
                                                  0.661124f,
...
struct npc_echo_of_medivh : public ScriptedAI {
    ...
    void HandleCellInitialData(uint8      row,
                               uint8      col,
                               Creature*  trigger,
                               BoardCell& cell)
    {
        ...
        case 0: // Alliance first row
        {
            ...
            case 7: // Rook
                if (Creature* rook =
                        trigger->FindNearestCreature(NPC_ROOK_A, 4.0f, true)) {
                    cell.SetPiece(rook);
                }
                break;
            case 1:
            case 6: // Knight
                if (Creature* knight = trigger->FindNearestCreature(
                        NPC_KNIGHT_A, 4.0f, true)) {
                    cell.SetPiece(knight);
                }
                break;
            case 2:
            case 5: // Bishop
                if (Creature* bishop = trigger->FindNearestCreature(
                        NPC_BISHOP_A, 4.0f, true)) {
                    cell.SetPiece(bishop);
                }
                break;
            case 3: // Queen
                if (Creature* queen =
                        trigger->FindNearestCreature(NPC_QUEEN_A, 4.0f, true)) {
                    cell.SetPiece(queen);
                }
                break;
            case 4: // King
                if (Creature* king =
                        trigger->FindNearestCreature(NPC_KING_A, 4.0f, true)) {
                    cell.SetPiece(king);
                }
                break;
            }
        ...
        case 1: // Alliance second row
            // All pawns
            if (Creature* pawn =
                    trigger->FindNearestCreature(NPC_PAWN_A, 4.0f, true)) {
                cell.SetPiece(pawn);
            }
            break;
        case 6: // Horde second row
            // All pawns
            if (Creature* pawn =
                    trigger->FindNearestCreature(NPC_PAWN_H, 4.0f, true)) {
                cell.SetPiece(pawn);
            }
            break;
        case 7: // Horde first row
        {
            ...
            case 7: // Rook
                if (Creature* rook =
                        trigger->FindNearestCreature(NPC_ROOK_H, 4.0f, true)) {
                    cell.SetPiece(rook);
                }
                break;
            case 1:
            ...
            case 5: // Bishop
                if (Creature* bishop = trigger->FindNearestCreature(
                        NPC_BISHOP_H, 4.0f, true)) {
                    cell.SetPiece(bishop);
                }
                break;
            case 3: // Queen
                if (Creature* queen =
                        trigger->FindNearestCreature(NPC_QUEEN_H, 4.0f, true)) {
                    cell.SetPiece(queen);
                }
                break;
            case 4: // King
                if (Creature* king =
                        trigger->FindNearestCreature(NPC_KING_H, 4.0f, true)) {
                    cell.SetPiece(king);
                }
                break;
            default:
    ...
    Creature*
    GetHostileTargetForChangeFacing(Creature*                    piece,
        ...
        for (uint8 row = 0; row < MAX_ROW; ++row) {
            for (uint8 col = 0; col < MAX_COL; ++col) {
                ...
                for (KarazhanChessOrientationType orient : orientations) {
                    ...
                    if (newRow < MAX_ROW && newCol < MAX_COL && newRow >= 0 &&
                        newCol >= 0)
                        if (Creature* targetPiece = ObjectAccessor::GetCreature(
                                *me, _boards[newRow][newCol].pieceGUID))
    ...
    void HandlePieceJustDied(Creature* piece)
    {
        ...
        case CHESS_FACTION_ALLIANCE: {
            ...
            float finalX =
                baseX + offset * deltaX +
                (_deadCount[DEAD_ALLIANCE] >= MAX_ROW ? 1 : 0) * extraX;
            float finalY =
                baseY + offset * deltaY +
                (_deadCount[DEAD_ALLIANCE] >= MAX_ROW ? 1 : 0) * extraY;
        ...
        case CHESS_FACTION_HORDE: {
            ...
            float finalX = baseX + offset * deltaX +
                           (_deadCount[DEAD_HORDE] >= MAX_ROW ? 1 : 0) * extraX;
            float finalY = baseY + offset * deltaY +
                           (_deadCount[DEAD_HORDE] >= MAX_ROW ? 1 : 0) * extraY;
        ...
        if (_instance->GetData(CHESS_EVENT_TEAM) == TEAM_HORDE) {
            ...
            case NPC_KING_A:
                if (_instance->GetData(DATA_CHESS_EVENT) == IN_PROGRESS) {
                    ...
                    _instance->SetData(DATA_CHESS_REINIT_PIECES, 0);
                    _instance->DoRespawnGameObject(
                        _instance->GetGuidData(DATA_DUST_COVERED_CHEST), DAY);
    ...
    int8 HandlePieceMove(Creature*         piece,
    ...
    bool HandlePieceMoveByAI(Creature*                    piece,
                             KarazhanChessOrientationType orientation)
    {
        uint8 pieceRow = 0;
        uint8 pieceCol = 0;
        ...
        if (orientation == ORI_SE && pieceRow == 0) {
            if (Creature* trigger = ObjectAccessor::GetCreature(
                    *me, _boards[pieceRow + 1][pieceCol].triggerGUID)) {
            ...
            if (Creature* trigger = ObjectAccessor::GetCreature(
                    *me, _boards[pieceRow][pieceCol + 1].triggerGUID)) {
                CastChangeFacing(piece, trigger);
            }
        ...
        {
            if (Creature* target =
                    GetHostileTargetForChangeFacing(piece, orientation)) {
        ...
        switch (orientation) //! Here we shouldn't be facing the edges of the
                             //! board, check in the 4 first if statements
        {
        case ORI_SE:
        case ORI_NW: {
            int32 randomRow =
                (orientation == ORI_SE) ? pieceRow - 1 : pieceRow + 1;
            int32 randomCol = pieceCol;
            switch (urand(0, 2)) {
            case 0:
                if ((me->GetEntry() == NPC_QUEEN_A ||
                     me->GetEntry() == NPC_QUEEN_H)) {
                    randomCol -= irand(1, 2);
                }
                else {
                    randomCol -= 1;
            ...
            case 2:
                if ((me->GetEntry() == NPC_QUEEN_A ||
                     me->GetEntry() == NPC_QUEEN_H)) {
                    randomRow +=
                        (orientation == ORI_SE) ? irand(-2, 0) : irand(0, 2);
                }
                else if ((me->GetEntry() == NPC_KNIGHT_A ||
            ...
            if (Creature* trigger =
                    ObjectAccessor::GetCreature(*me, cell.triggerGUID)) {
                if (!cell.pieceGUID) {
        ...
        case ORI_NE: {
            ...
            int32 randomCol =
                orientation == ORI_SW ? pieceCol - 1 : pieceCol + 1;
            switch (urand(0, 2)) {
            case 0:
                if ((me->GetEntry() == NPC_QUEEN_A ||
                     me->GetEntry() == NPC_QUEEN_H)) {
            ...
            case 2:
                if ((me->GetEntry() == NPC_QUEEN_A ||
                     me->GetEntry() == NPC_QUEEN_H)) {
                    randomCol +=
                        (orientation == ORI_SW) ? irand(-2, 0) : irand(0, 2);
                }
                else if ((me->GetEntry() == NPC_KNIGHT_A ||
                          me->GetEntry() == NPC_KNIGHT_H) &&
                         urand(0, 1)) {
    ...
    void HandleCheat()
    {
        ...
        case 0: // Heal king
        {
            if (_instance->GetData(CHESS_EVENT_TEAM) == TEAM_ALLIANCE) {
                if (Creature* king =
                        me->FindNearestCreature(NPC_KING_H, 80.0f, true)) {
                    king->SetHealth(king->GetMaxHealth());
                }
            }
            else if (_instance->GetData(CHESS_EVENT_TEAM) == TEAM_HORDE) {
                if (Creature* king =
                        me->FindNearestCreature(NPC_KING_A, 80.0f, true)) {
                    king->SetHealth(king->GetMaxHealth());
                }
            }
        ...
        case 2: // Buff
        {
            ...
            for (uint8 row = 0; row < MAX_ROW; ++row) {
                for (uint8 col = 0; col < MAX_COL; ++col) {
                    ...
                    if (Creature* piece =
                            ObjectAccessor::GetCreature(*me, cell.pieceGUID)) {
    ...
    Creature* GetPiece(Creature* sourcePiece,
                       uint8     searchType,
                       float     distance,
                       bool      hostile,
                       uint32    minHPDiff,
                       bool      checkFront)
    {
        ...
        for (uint8 row = 0; row < MAX_ROW; ++row) {
            for (uint8 col = 0; col < MAX_COL; ++col) {
                if (Creature* piece = ObjectAccessor::GetCreature(
                        *me, _boards[row][col].pieceGUID)) {
                    if (IsFriendly(piece, sourcePiece) == hostile) {
                        continue;
                    }

                    if (checkFront &&
                        !sourcePiece->HasInArc(float(M_PI) / 3.0f, piece)) {
                        continue;
                    }

                    if (minHPDiff) {
                        if (piece->GetMaxHealth() - piece->GetHealth() <=
                            minHPDiff) {
                            continue;
                        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (chessPhase != CHESS_PHASE_INPROGRESS_PVE &&
            chessPhase != CHESS_PHASE_INPROGRESS_PVP)
            return;
    ...
    void sGossipHello(Player* player) override
    {
        ...
        case CHESS_PHASE_INPROGRESS_PVP:
            AddGossipItemFor(player,
                             ...
                             "Restart",
                             GOSSIP_SENDER_MAIN,
                             MEDIVH_GOSSIP_RESTART); // We want to player
                                                     // another game against you
            break;
    ...
    {
        uint32 chessPhase = _instance->GetData(DATA_CHESS_GAME_PHASE);
        _instance->SetData(CHESS_EVENT_TEAM,
        ...
        uint32 const action =
            player->PlayerTalkClass->GetGossipOptionAction(gossipListId);
        switch (action) {
...
struct npc_chesspiece : public ScriptedAI {
    ...
    void MovementInform(uint32 movementType, uint32 /*pointId*/) override
    {
        ...
        if (!me->IsCharmed()) {
            ...
            if (!_nextMoveTimer) {
                _nextMoveTimer =
                    urand(10 * IN_MILLISECONDS, 20 * IN_MILLISECONDS);
            }

            if (Creature* medivh = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH))) {
                if (Creature* target = CAST_AI(npc_echo_of_medivh, medivh->AI())
                                           ->GetHostileTargetForChangeFacing(
                                               me, _currentOrientation)) {
                    CastChangeFacing(me, target);
                }
            }
        }
    ...
    void OnCharmed(bool apply) override
    {
        ...
        if (apply) {
            ...
            if (Player* playerCharmer = charmer->ToPlayer()) {
                ...
                for (uint32 i = 0; i < MAX_CREATURE_SPELLS; ++i) {
                    ...
                    data << uint32(
                        MAKE_UNIT_ACTION_BUTTON(spellId, ACT_ENABLED));
                }

                for (uint32 i = MAX_CREATURE_SPELLS; i < MAX_SPELL_CONTROL_BAR;
                     ++i)
                    data << uint32(0);
            ...
            if (Unit* charmer = ObjectAccessor::GetUnit(*me, _charmerGUID)) {
                ...
                charmer->NearTeleportTo(
                    -11106.92f, -1843.32f, 229.626f, 4.2331f);
            }
        ...
        if (apply) {
            ...
            else if (FactionTemplateEntry const* factionTemplateEntry =
                         me->GetFactionTemplateEntry()) {
    ...
    Creature* GetEnemyPiece(float maxDistance,
        ...
        if (!me->IsCharmed()) {
            ...
            case TEAM_ALLIANCE:
                _teamControlledByRaid =
                    me->GetFaction() == CHESS_FACTION_ALLIANCE;
                break;
            case TEAM_HORDE:
                _teamControlledByRaid = me->GetFaction() == CHESS_FACTION_HORDE;
                break;
            ...
            if (!_teamControlledByRaid) {
                if (_nextMoveTimer) {
                    if (_nextMoveTimer <= diff) {
                        if (Creature* medivh = ObjectAccessor::GetCreature(
                                *me,
                                _instance->GetGuidData(DATA_ECHO_OF_MEDIVH))) {
                            if (CAST_AI(npc_echo_of_medivh, medivh->AI())
                                    ->HandlePieceMoveByAI(
                                        me, _currentOrientation)) {
                                _nextMoveTimer = 0;
                            }
                                ...
                                _nextMoveTimer =
                                    1 *
                                    IN_MILLISECONDS; //! Re-check after a second
                            }
                        }
                    }
                    else {
            ...
            if (_combatSpellTimer) {
                if (_combatSpellTimer <= diff) {
                    _combatSpellTimer =
                        ...
                        (_teamControlledByRaid
                             ? urand(6 * IN_MILLISECONDS, 12 * IN_MILLISECONDS)
            ...
            if (_combatSpellTimer2) {
                if (_combatSpellTimer2 <= diff) {
                    ...
                    case NPC_KNIGHT_A:
                        ...
                        break;
                    case NPC_QUEEN_A:
                        if (Creature* victim = GetEnemyPiece(
                                25.0f, CHESS_PIECE_SEARCH_TYPE_RANDOM, false)) {
                            DoCast(victim, SPELL_QUEEN_A_2); //! Rain of Fire
                        }
                        break;
                    case NPC_BISHOP_A:
                        if (Creature* target =
                                GetLowestHpFriendlyPiece(25.0f, 5000)) {
    ...
    void InitializeCombatSpellsByEntry()
    {
        ...
        case NPC_PAWN_H:
            _combatSpellTimer =
                urand(7 * IN_MILLISECONDS, 27 * IN_MILLISECONDS);
            _combatSpellTimer2 =
                urand(3 * IN_MILLISECONDS, 5 * IN_MILLISECONDS);
            break;
        case NPC_KING_A:
        case NPC_KING_H:
            _combatSpellTimer =
                urand(13 * IN_MILLISECONDS, 15 * IN_MILLISECONDS);
            _combatSpellTimer2 =
                urand(3 * IN_MILLISECONDS, 5 * IN_MILLISECONDS);
            break;
        case NPC_BISHOP_A:
        case NPC_BISHOP_H:
            _combatSpellTimer =
                urand(7 * IN_MILLISECONDS, 15 * IN_MILLISECONDS);
            _combatSpellTimer2 =
                urand(15 * IN_MILLISECONDS, 90 * IN_MILLISECONDS);
            break;
        case NPC_ROOK_A:
        case NPC_ROOK_H:
        case NPC_KNIGHT_A:
    ...
    void SpellHitTarget(Unit* target, SpellInfo const* spellInfo) override
    {
        ...
        if (target->GetEntry() != NPC_CHESS_MOVE_TRIGGER ||
        ...
        case SPELL_MOVE_GENERIC:
            if (Creature* medivh = ObjectAccessor::GetCreature(
                    *me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH))) {
                int8 result =
                    CAST_AI(npc_echo_of_medivh, medivh->AI())
                        ->HandlePieceMove(me,
                                          target->GetGUID(),
                                          spellInfo->Id == SPELL_MOVE_GENERIC);
                if (result != -1) {
        ...
        case SPELL_CHANGE_FACING:
            if (Creature* medivh = ObjectAccessor::GetCreature(
                ...
                int8 result = CAST_AI(npc_echo_of_medivh, medivh->AI())
                                  ->HandlePieceRotate(me, target->GetGUID());
                if (result != -1) {
                    DoCast(SPELL_MOVE_COOLDOWN);
                    _currentOrientation = KarazhanChessOrientationType(result);
    ...
    void sGossipHello(Player* player) override
    {
        ...
        if (_instance->GetData(CHESS_EVENT_TEAM) == TEAM_ALLIANCE &&
            ...
            chessPhase < CHESS_PHASE_PVE_FINISHED) {
            SendGossipMenuFor(
                player, player->GetGossipTextId(me), me->GetGUID());
            return;
        }

        if (_instance->GetData(CHESS_EVENT_TEAM) == TEAM_HORDE &&
            me->GetFaction() != CHESS_FACTION_HORDE &&
            chessPhase < CHESS_PHASE_PVE_FINISHED) {
            SendGossipMenuFor(
                player, player->GetGossipTextId(me), me->GetGUID());
            return;
        ...
        case NPC_KING_A:
            if (chessPhase != CHESS_PHASE_INPROGRESS_PVE &&
                ...
                chessPhase != CHESS_PHASE_PVP_WARMUP) {
                ok = false;
            }
            break;
        default:
...
class spell_control_piece : public AuraScript {
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_control_piece::HandleEffectRemove,
                               EFFECT_1,
