...
public:
    npc_lady_sylvanas_windrunner()
        : CreatureScript("npc_lady_sylvanas_windrunner")
    {
    }

    bool OnQuestReward(Player*      player,
                       Creature*    creature,
                       const Quest* _Quest,
    ...
    struct npc_lady_sylvanas_windrunnerAI : public ScriptedAI {
        ...
        void SetGUID(ObjectGuid guid, int32 type) override
        {
            if (type == GUID_EVENT_INVOKER) {
                ...
                for (uint8 i = 0; i < 4; ++i)
                    me->SummonCreature(NPC_HIGHBORNE_LAMENTER,
                                       HighborneLoc[i][0],
                                       HighborneLoc[i][1],
                                       HIGHBORNE_LOC_Y,
                                       HighborneLoc[i][2],
        ...
        void JustSummoned(Creature* summoned) override
        {
            if (summoned->GetEntry() == NPC_HIGHBORNE_BUNNY) {
                ...
                float speed =
                    summoned->GetDistance(summoned->GetPositionX(),
                                          summoned->GetPositionY(),
                                          me->GetPositionZ() + 15.0f) /
                    (1000.0f * 0.001f);
                summoned->MonsterMoveWithSpeed(summoned->GetPositionX(),
                                               summoned->GetPositionY(),
                                               me->GetPositionZ() + 15.0f,
                                               speed);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_FADE:
                    ...
                    // add a blink to simulate a stealthed movement and
                ...
                case EVENT_SUNSORROW_WHISPER:
                    if (Creature* ambassador = me->FindNearestCreature(
                            NPC_AMBASSADOR_SUNSORROW, 20.0f))
                        if (Player* player =
                                ObjectAccessor::GetPlayer(*me, playerGUID))
                            ambassador->AI()->Talk(SAY_SUNSORROW_WHISPER,
                                                   player);
                    break;
                default:
                    break;
        ...
        void UpdateAI(uint32 diff) override
        {
            if (EventMove) {
                if (EventMoveTimer <= diff) {
                    ...
                    me->MonsterMoveWithSpeed(
                        me->GetPositionX(),
                        me->GetPositionY(),
                        HIGHBORNE_LOC_Y_NEW,
                        me->GetDistance(me->GetPositionX(),
                                        me->GetPositionY(),
                                        HIGHBORNE_LOC_Y_NEW) /
                            (5000 * 0.001f));
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_ID_TEST_OF_LORE) ==
                QUEST_STATUS_INCOMPLETE &&
            !player->HasAura(SPELL_MARK_OF_SHAME)) {
            ...
            SendGossipMenuFor(
                player, GOSSIP_TEXTID_TEST_OF_LORE, creature->GetGUID());
        }
        else {
            SendGossipMenuFor(
                player, GOSSIP_TEXTID_PARQUAL_FINTALLAS, creature->GetGUID());
        }
...
public:
                        ...
                        uint32 action) override
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            ...
            if (auto ai = CAST_AI(npc_varian_wrynn::npc_varian_wrynnAI,
                                  creature->AI())) {
                ai->Start(true, true, player->GetGUID());
                if (Creature* jaina =
                        GetClosestCreatureWithEntry(creature, NPC_JAINA, 50.0f))
    ...
    struct npc_varian_wrynnAI : public npc_escortAI {
        ...
        void Reset() override
        {
            if (!HasEscortState(STATE_ESCORT_ESCORTING)) {
                ...
                if (Creature* putress =
                        ObjectAccessor::GetCreature(*me, putressGUID)) {
                    putress->DespawnOrUnsummon();
                    putressGUID.Clear();
                }

                if (Creature* blightWorm =
                        ObjectAccessor::GetCreature(*me, blightWormGUID)) {
                    blightWorm->DespawnOrUnsummon();
                    blightWormGUID.Clear();
                }

                if (Creature* khanok =
                        ObjectAccessor::GetCreature(*me, khanokGUID)) {
                    khanok->DespawnOrUnsummon();
                    khanokGUID.Clear();
                }

                if (Creature* thrall =
                        ObjectAccessor::GetCreature(*me, thrallGUID)) {
                    thrall->DespawnOrUnsummon();
                    thrallGUID.Clear();
                }

                if (Creature* sylvanas =
                        ObjectAccessor::GetCreature(*me, sylvanasGUID)) {
                    sylvanas->DespawnOrUnsummon();
                    sylvanasGUID.Clear();
                }

                for (uint8 i = 0; i < GENERATOR_MAXCOUNT; ++i) {
                    if (Creature* temp = ObjectAccessor::GetCreature(
                            *me, generatorGUID[i])) {
        ...
        void JustSummoned(Creature* summonedCreature) override
        {
            ...
            case NPC_GENERATOR:
                ...
                summonedCreature->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_WRYNN_BUFF, true);
                summonedCreature->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_THRALL_BUFF, true);
                summonedCreature->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_SYLVANAS_BUFF, true);
                break;
            default:
                break;
            }
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            ...
            case NPC_PUTRESS:
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_APOTHECARIUM_FIGHT_A, 0);
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_APOTHECARIUM_DONE_A, 1);
                bStepping = true;
                break;
            default:
                break;
        ...
        void SetHoldState(bool bOnHold) { SetEscortPaused(bOnHold); }
        ...
        void SpawnWave(uint32 waveId)
        {
            switch (waveId) {
            case 0:
                me->SummonCreature(NPC_GUARDIAN,
                                   AllianceSpawn[0].x,
                                   AllianceSpawn[0].y,
                                   AllianceSpawn[0].z,
            ...
            case 1:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i) {
                    switch (urand(0, 12)) {
                    case 0:
                        me->SummonCreature(NPC_DREADLORD,
                                           AllianceSpawn[1].x + rand32() % 5,
                                           AllianceSpawn[1].y + rand32() % 5,
                                           AllianceSpawn[1].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
                    ...
                    case 5:
                    case 6:
                        me->SummonCreature(NPC_FELBEAST,
                                           AllianceSpawn[1].x + rand32() % 5,
                                           AllianceSpawn[1].y + rand32() % 5,
                                           AllianceSpawn[1].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                    ...
                    case 8:
                    case 9:
                        me->SummonCreature(NPC_BETRAYER,
                                           AllianceSpawn[1].x + rand32() % 5,
                                           AllianceSpawn[1].y + rand32() % 5,
                                           AllianceSpawn[1].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                    ...
                    case 11:
                    case 12:
                        me->SummonCreature(NPC_COLLABORATOR,
                                           AllianceSpawn[1].x + rand32() % 5,
                                           AllianceSpawn[1].y + rand32() % 5,
                                           AllianceSpawn[1].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
            ...
            case 2:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_COLLABORATOR,
                                           AllianceSpawn[2].x - rand32() % 5,
                                           AllianceSpawn[2].y - rand32() % 5,
                                           AllianceSpawn[2].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
            ...
            case 3:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_GUARDIAN,
                                           AllianceSpawn[3].x - rand32() % 5,
                                           AllianceSpawn[3].y - rand32() % 5,
                                           AllianceSpawn[3].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
                    case 2:
                        me->SummonCreature(NPC_CHEMIST,
                                           AllianceSpawn[3].x - rand32() % 5,
                                           AllianceSpawn[3].y - rand32() % 5,
                                           AllianceSpawn[3].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
            ...
            case 4:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i) {
                    switch (urand(0, 5)) {
                    case 0:
                        if (Unit* temp = me->SummonCreature(
                                NPC_DOCTOR,
                                AllianceSpawn[4].x - rand32() % 5,
                                AllianceSpawn[4].y - rand32() % 5,
                                AllianceSpawn[4].z,
                                TEMPSUMMON_DEAD_DESPAWN))
                            temp->GetMotionMaster()->MovePoint(
                                0,
                                me->GetPositionX(),
                                me->GetPositionY(),
                                me->GetPositionZ(),
                    ...
                    case 3:
                        if (Unit* temp = me->SummonCreature(
                                NPC_DOCTOR,
                                AllianceSpawn[5].x - rand32() % 5,
                                AllianceSpawn[5].y - rand32() % 5,
                                AllianceSpawn[5].z,
                                TEMPSUMMON_DEAD_DESPAWN))
                            temp->GetMotionMaster()->MovePoint(
                                0,
            ...
            case 5:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i)
                    if (Unit* temp = me->SummonCreature(
                            NPC_GUARDIAN,
                            AllianceSpawn[6].x - rand32() % 5,
                            AllianceSpawn[6].y - rand32() % 5,
                            AllianceSpawn[6].z,
                            TEMPSUMMON_DEAD_DESPAWN))
                        temp->GetMotionMaster()->MovePoint(0,
                                                           me->GetPositionX(),
                                                           me->GetPositionY(),
                                                           me->GetPositionZ(),
                                                           false);
            ...
            case 6:
                if (Unit* temp =
                        me->SummonCreature(NPC_BLIGHTWORM,
                                           AllianceSpawn[7].x,
                                           AllianceSpawn[7].y,
                                           AllianceSpawn[7].z,
                    ...
                    if (Creature* jaina =
                            ObjectAccessor::GetCreature(*me, jainaGUID)) {
                        temp->AddThreat(jaina, 100.0f);
                        jaina->AddThreat(temp, 100.0f);
                    }
                }
                if (Unit* temp =
                        me->SummonCreature(NPC_KHANOK,
                                           HordeSpawn[0].x,
                                           HordeSpawn[0].y,
                                           HordeSpawn[0].z,
                                           TEMPSUMMON_MANUAL_DESPAWN)) {
                ...
                for (uint8 i = 0; i < GENERATOR_MAXCOUNT; ++i) {
                    ...
                    case 0:
                        if (Unit* temp =
                                me->SummonCreature(NPC_GENERATOR,
                                                   1433.142212f,
                                                   402.493835f,
                                                   -80.515945f,
                                                   TEMPSUMMON_MANUAL_DESPAWN)) {
                            generatorGUID[i] = temp->GetGUID();
                            if (Creature* putress = ObjectAccessor::GetCreature(
                                    *me, putressGUID))
                                temp->CastSpell(putress,
                ...
                break;
            case 7:
                if (Unit* temp = me->SummonCreature(NPC_SW_SOLDIER,
                                                    AllianceSpawn[8].x,
                                                    AllianceSpawn[8].y,
                                                    AllianceSpawn[8].z,
                                                    0,
                    ...
                    temp->GetMotionMaster()->MovePath(NPC_SW_SOLDIER * 10,
                                                      false);
                }
                if (Unit* temp = me->SummonCreature(NPC_SW_SOLDIER,
                                                    AllianceSpawn[8].x,
                                                    ...
                                                    90000)) {
                    allianceGuardsGUID.push_back(temp->GetGUID());
                    temp->GetMotionMaster()->MovePath((NPC_SW_SOLDIER * 10) + 1,
                                                      false);
                }
                if (Unit* temp = me->SummonCreature(NPC_SW_SOLDIER,
                                                    AllianceSpawn[8].x,
                                                    ...
                                                    90000)) {
                    allianceGuardsGUID.push_back(temp->GetGUID());
                    temp->GetMotionMaster()->MovePath((NPC_SW_SOLDIER * 10) + 2,
                                                      false);
                }
                if (Unit* temp = me->SummonCreature(NPC_SW_SOLDIER,
                                                    AllianceSpawn[8].x,
                                                    ...
                                                    90000)) {
                    allianceGuardsGUID.push_back(temp->GetGUID());
                    temp->GetMotionMaster()->MovePath((NPC_SW_SOLDIER * 10) + 3,
                                                      false);
                }
                break;
            case 8:
            ...
            case 9:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i)
                    me->SummonCreature(NPC_GUARDIAN,
                                       AllianceSpawn[10].x + rand32() % 13,
                                       AllianceSpawn[10].y + rand32() % 13,
                                       AllianceSpawn[10].z,
                                       TEMPSUMMON_DEAD_DESPAWN);
                break;
            case 10:
                if (Unit* temp = me->SummonCreature(NPC_DREADLORD,
                                                    AllianceSpawn[11].x,
                                                    AllianceSpawn[11].y,
                                                    AllianceSpawn[11].z,
                                                    TEMPSUMMON_DEAD_DESPAWN)) {
                    temp->GetMotionMaster()->MovePath(NPC_DREADLORD * 10,
                                                      false);
                    temp->ApplySpellImmune(
                        0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
                    temp->ApplySpellImmune(
                        0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK_DEST, true);
                }
                break;
            case 11:
                for (uint8 i = 0; i < WAVE_MAXCOUNT; ++i) {
                    ...
                    case 0:
                        if (Unit* temp = me->SummonCreature(
                                NPC_EXPERIMENT,
                                AllianceSpawn[13].x + rand32() % 5,
                                AllianceSpawn[13].y + rand32() % 5,
                                AllianceSpawn[13].z,
                                TEMPSUMMON_DEAD_DESPAWN))
                            temp->GetMotionMaster()->MovePoint(
                                0,
                    ...
                    case 1:
                        if (Unit* temp = me->SummonCreature(
                                NPC_EXPERIMENT,
                                AllianceSpawn[14].x + rand32() % 5,
                                AllianceSpawn[14].y + rand32() % 5,
                                AllianceSpawn[14].z,
                                TEMPSUMMON_DEAD_DESPAWN))
                            temp->GetMotionMaster()->MovePoint(
                                0,
                    ...
                    case 2:
                        if (Unit* temp = me->SummonCreature(
                                NPC_EXPERIMENT,
                                AllianceSpawn[15].x + rand32() % 5,
                                AllianceSpawn[15].y + rand32() % 5,
                                AllianceSpawn[15].z,
                                TEMPSUMMON_DEAD_DESPAWN))
                            temp->GetMotionMaster()->MovePoint(
                                0,
                    ...
                    case 3:
                        if (Unit* temp = me->SummonCreature(
                                NPC_EXPERIMENT,
                                AllianceSpawn[16].x + rand32() % 5,
                                AllianceSpawn[16].y + rand32() % 5,
                                AllianceSpawn[16].z,
                                TEMPSUMMON_DEAD_DESPAWN))
                            temp->GetMotionMaster()->MovePoint(
                                0,
            ...
            case 12:
                if (Creature* temp =
                    ...
                    temp->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                         EMOTE_STATE_READY2H);
                }
                if (Creature* temp =
                        me->SummonCreature(NPC_SYLVANAS,
                                           AllianceSpawn[18].x,
                                           AllianceSpawn[18].y,
                                           AllianceSpawn[18].z,
                                           TEMPSUMMON_MANUAL_DESPAWN)) {
                ...
                for (uint8 i = 0; i < HORDE_FORCE_MAXCOUNT; ++i) {
                    if (Creature* temp =
                            me->SummonCreature(NPC_HORDE_SOLDIER,
                                               AllianceSpawn[i + 19].x,
                                               AllianceSpawn[i + 19].y,
                                               AllianceSpawn[i + 19].z,
                                               TEMPSUMMON_MANUAL_DESPAWN)) {
            ...
            case 13:
                for (uint8 i = 0; i < ALLIANCE_FORCE_MAXCOUNT; ++i) {
                    if (Unit* temp =
                            me->SummonCreature(NPC_SW_SOLDIER,
                                               AllianceSpawn[i + 25].x,
                                               AllianceSpawn[i + 25].y,
                                               AllianceSpawn[i + 25].z,
                                               AllianceSpawn[i + 25].o,
                                               TEMPSUMMON_MANUAL_DESPAWN)) {
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 45:
                ...
                if (Creature* jaina =
                        ObjectAccessor::GetCreature(*me, jainaGUID)) {
                    jaina->GetMotionMaster()->Clear();
                    jaina->GetMotionMaster()->MovePoint(
                        0, AllianceWP[6].x, AllianceWP[6].y, AllianceWP[6].z);
                }
                break;
            case 46:
                SetHoldState(true);
            ...
            if (bStepping) {
                if (phaseTimer <= diff) {
                    ...
                    case 0:
                        me->setActive(true);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_MANHUNT_COUNTDOWN_A, 1);
                    ...
                    case 4:
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_MANHUNT_COUNTDOWN_A, 0);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_MANHUNT_STARTS_A, 1);
                    ...
                    case 8:
                        if (Creature* jaina =
                                ObjectAccessor::GetCreature(*me, jainaGUID)) {
                            jaina->GetMotionMaster()->MovePath(NPC_JAINA * 10,
                                                               false);
                    ...
                    case 12:
                        SetEscortPaused(false);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_MANHUNT_STARTS_A, 0);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_SEWERS_FIGHT_A, 1);
                        JumpToNextStep(1 * IN_MILLISECONDS);
                        break;
                    case 13:
                        if (Creature* jaina =
                                ObjectAccessor::GetCreature(*me, jainaGUID)) {
                            jaina->GetMotionMaster()->MoveFollow(
                                me, 5, PET_FOLLOW_ANGLE);
                            jaina->SetReactState(REACT_AGGRESSIVE);
                    ...
                    case 30:
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_APOTHECARIUM_FIGHT_A, 1);
                        if (Creature* putress =
                                ObjectAccessor::GetCreature(*me, putressGUID))
                    ...
                    case 44:
                        ...
                        if (Creature* jaina =
                                ObjectAccessor::GetCreature(*me, jainaGUID)) {
                            jaina->GetMotionMaster()->Clear();
                            jaina->GetMotionMaster()->MovePoint(0,
                                                                AllianceWP[7].x,
                                                                AllianceWP[7].y,
                                                                AllianceWP[7].z,
                                                                false);
                    ...
                    case 73:
                        ...
                        if (Creature* sylvanas = ObjectAccessor::GetCreature(
                                *me, sylvanasGUID)) {
                    ...
                    case 76:
                        ...
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_SW_SOLDIER, 100.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_HORDE_SOLDIER, 100.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_JAINA, 100.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_SYLVANAS, 100.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_THRALL, 100.0f);
                        if (!HelperList.empty())
                            for (std::list<Creature*>::iterator itr =
                                     HelperList.begin();
                                 itr != HelperList.end();
                                 itr++)
                        ...
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_MANHUNT_STARTS_A, 0);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_SEWERS_DONE_A, 0);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_APOTHECARIUM_DONE_A, 0);
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_INFEST:
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0, 0, true))
    ...
    class spell_blight_worm_ingest_SpellScript : public SpellScript {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_blight_worm_ingest_SpellScript::HandleScript,
                EFFECT_0,
...
public:
                        ...
                        uint32 action) override
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1: {
            ...
            if (auto thrall_ai =
                    CAST_AI(npc_thrall_bfu::npc_thrall_bfuAI, creature->AI())) {
                if (Creature* sylvannas = GetClosestCreatureWithEntry(
                        creature, NPC_SYLVANAS, 50.0f)) {
    ...
    struct npc_thrall_bfuAI : public npc_escortAI {
        ...
        void Reset() override
        {
            if (!HasEscortState(STATE_ESCORT_ESCORTING)) {
                ...
                if (Creature* valimathras =
                        ObjectAccessor::GetCreature(*me, ValimathrasGUID)) {
                    valimathras->DespawnOrUnsummon();
                    ValimathrasGUID.Clear();
                }

                if (Creature* valimathrasportal = ObjectAccessor::GetCreature(
                        *me, ValimathrasPortalGUID)) {
                    valimathrasportal->DespawnOrUnsummon();
                    ValimathrasPortalGUID.Clear();
                }

                if (Creature* wrynn =
                        ObjectAccessor::GetCreature(*me, WrynnGUID)) {
                ...
                if (Creature* saurfang =
                        ObjectAccessor::GetCreature(*me, SaurfangGUID)) {
        ...
        void JustSummoned(Creature* summoned) override
        {
            ...
            case NPC_WARSONG_BATTLEGUARD:
                summoned->ApplySpellImmune(
                    0, IMMUNITY_ID, SPELL_SYLVANAS_BUFF, true);
                summoned->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                         EMOTE_STATE_READY2H);
                break;
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            ...
            case NPC_BLIGHT_ABBERATION:
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_COURTYARD_FIGHT_H, 0);
                UpdateWorldState(me->GetMap(), WORLD_STATE_COURTYARD_DONE_H, 1);
                bStepping = true;
                break;
            case NPC_KHANOK: {
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_INNER_SANKTUM_FIGHT_H, 0);
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_INNER_SANKTUM_DONE_H, 1);
                FollowThrall();
                SetEscortPaused(false);
                std::list<Creature*> SanktumList;
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_FELGUARD_MORADEUR, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_DREADLORD, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_TREACHEROUS_GUARDIAN_H, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_DOCTOR_H, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_CHEMIST_H, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_BETRAYER_H, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_FELBEAST_H, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    SanktumList, NPC_DOOMGUARD_PILLARGER, 1000.0f);
            ...
            case NPC_VARIMATHRAS: {
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_ROYAL_QUARTER_FIGHT_H, 0);
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_ROYAL_QUARTER_DONE_H, 1);
                std::list<Creature*> ThroneList;
                me->GetCreatureListWithEntryInGrid(
                    ThroneList, NPC_LEGION_OVERLORD, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    ThroneList, NPC_LEGION_INVADER, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    ThroneList, NPC_LEGION_DREADWHISPER, 1000.0f);
                me->GetCreatureListWithEntryInGrid(
                    ThroneList, NPC_VARIMATHRAS_PORTAL, 1000.0f);
                if (!ThroneList.empty())
                    for (std::list<Creature*>::iterator itr =
                             ThroneList.begin();
                         itr != ThroneList.end();
        ...
        {
            if (Creature* sylvanas =
                    ObjectAccessor::GetCreature(*me, sylvanasfollowGUID)) {
        ...
        {
            if (Creature* portal =
                    me->FindNearestCreature(NPC_VARIMATHRAS_PORTAL, 500.0f)) {
                portal->SummonCreature(NPC_LEGION_OVERLORD,
                                       ThrallSpawn[81].x + rand32() % 5,
                                       ThrallSpawn[81].y + rand32() % 5,
                                       ThrallSpawn[81].z,
                                       TEMPSUMMON_DEAD_DESPAWN);
        ...
        void SpawnWave(uint32 waveId)
        {
            ...
            case 0: // Vortex
                if (Creature* whirlwind1 =
                        me->SummonCreature(NPC_VORTEX,
                                           ...
                                           30 * IN_MILLISECONDS))
                    whirlwind1->GetMotionMaster()->MovePath(NPC_WHIRLWIND * 10,
                                                            false);
                if (Creature* whirlwind2 =
                        me->SummonCreature(NPC_VORTEX,
                                           ThrallSpawn[0].x,
                                           ThrallSpawn[0].y,
                                           ThrallSpawn[0].z,
                                           ...
                                           TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                                           30 * IN_MILLISECONDS))
                    whirlwind2->GetMotionMaster()->MovePath(NPC_WHIRLWIND * 100,
                                                            false);
                break;
            case 1:
                // BATTLING_COURTYARD Initial Spawn
                for (uint8 i = 0; i < 3; ++i)
                    me->SummonCreature(NPC_DOCTOR_H,
                                       ThrallSpawn[i + 1].x,
                                       ThrallSpawn[i + 1].y,
                                       ThrallSpawn[i + 1].z,
                                       ThrallSpawn[i + 1].o,
                                       ...
                                       300 * IN_MILLISECONDS);
                for (uint8 i = 0; i < 9; ++i)
                    me->SummonCreature(NPC_CHEMIST_H,
                                       ThrallSpawn[i + 4].x,
                                       ThrallSpawn[i + 4].y,
                                       ThrallSpawn[i + 4].z,
                                       ThrallSpawn[i + 4].o,
                                       ...
                                       300 * IN_MILLISECONDS);
                for (uint8 i = 0; i < 6; ++i)
                    me->SummonCreature(NPC_TREACHEROUS_GUARDIAN_H,
                                       ThrallSpawn[i + 13].x,
                                       ThrallSpawn[i + 13].y,
                                       ThrallSpawn[i + 14].z,
                                       ThrallSpawn[i + 14].o,
                                       ...
                                       300 * IN_MILLISECONDS);
                for (uint8 i = 0; i < 4; ++i)
                    me->SummonCreature(NPC_BLIGHT_SLINGER,
                                       ThrallSpawn[i + 19].x,
                                       ThrallSpawn[i + 19].y,
                                       ThrallSpawn[i + 19].z,
                                       ThrallSpawn[i + 19].o,
            ...
            case 2:
                // Valimathras
                if (Unit* temp =
                        me->SummonCreature(NPC_VARIMATHRAS,
                                           ThrallSpawn[23].x,
                                           ThrallSpawn[23].y,
                                           ThrallSpawn[23].z,
                                           ThrallSpawn[23].o,
            ...
            case 3:
                if (Unit* temp =
                        me->SummonCreature(NPC_VARIMATHRAS_PORTAL,
                                           ThrallSpawn[24].x,
                                           ThrallSpawn[24].y,
                                           ThrallSpawn[24].z,
                                           ThrallSpawn[24].o,
            ...
            case 4:
                ...
                for (uint8 i = 0; i < WAVE_COURTYARD_FIGHT; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_TREACHEROUS_GUARDIAN_H,
                                           ThrallSpawn[25].x + rand32() % 5,
                                           ThrallSpawn[25].y + rand32() % 5,
                                           ThrallSpawn[25].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
                        me->SummonCreature(NPC_DOCTOR_H,
                                           ThrallSpawn[26].x + rand32() % 5,
                                           ThrallSpawn[26].y + rand32() % 5,
                                           ThrallSpawn[26].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 2:
                        me->SummonCreature(NPC_CHEMIST_H,
                                           ThrallSpawn[27].x + rand32() % 5,
                                           ThrallSpawn[27].y + rand32() % 5,
                                           ThrallSpawn[27].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    }
            ...
            case 5:
                // Bossspawn 1
                if (Creature* temp =
                        me->SummonCreature(NPC_BLIGHT_ABBERATION,
                                           ThrallSpawn[28].x,
                                           ThrallSpawn[28].y,
                                           ThrallSpawn[28].z,
                                           ThrallSpawn[28].o,
                                           TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                                           900 * IN_MILLISECONDS)) {
                    temp->GetMotionMaster()->MoveJump(ThrallSpawn[62].x,
                                                      ThrallSpawn[62].y,
                                                      ThrallSpawn[62].z,
                                                      10.0f,
                                                      20.0f,
            ...
            case 6:
                ...
                if (Unit* temp =
                        me->SummonCreature(NPC_WARSONG_BATTLEGUARD,
                                           ThrallSpawn[29].x,
                                           ThrallSpawn[29].y,
                                           ThrallSpawn[29].z,
                                           ThrallSpawn[29].o,
            ...
            case 7:
                for (uint8 i = 0; i < 9; ++i)
                    me->SummonCreature(NPC_CAVE_DUMMY,
                                       ThrallSpawn[i + 46].x,
                                       ThrallSpawn[i + 46].y,
                                       ThrallSpawn[i + 46].z,
                                       ThrallSpawn[i + 46].o,
            ...
            case 9:
                ...
                for (uint8 i = 0; i < 3; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_TREACHEROUS_GUARDIAN_H,
                                           ThrallSpawn[59].x + rand32() % 2,
                                           ThrallSpawn[59].y + rand32() % 2,
                                           ThrallSpawn[59].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
            ...
            case 10:
                ...
                for (uint8 i = 0; i < 3; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_TREACHEROUS_GUARDIAN_H,
                                           ThrallSpawn[60].x + rand32() % 5,
                                           ThrallSpawn[60].y + rand32() % 5,
                                           ThrallSpawn[60].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
                        me->SummonCreature(NPC_FELGUARD_MORADEUR,
                                           ThrallSpawn[60].x + rand32() % 5,
                                           ThrallSpawn[60].y + rand32() % 5,
                                           ThrallSpawn[60].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
            ...
            case 11:
                ...
                for (uint8 i = 0; i < 3; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_FELGUARD_MORADEUR,
                                           ThrallSpawn[61].x + rand32() % 5,
                                           ThrallSpawn[61].y + rand32() % 5,
                                           ThrallSpawn[61].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
            ...
            case 14:
                for (uint8 i = 0; i < 4; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_FELGUARD_MORADEUR,
                                           ThrallSpawn[65].x + rand32() % 5,
                                           ThrallSpawn[65].y + rand32() % 5,
                                           ThrallSpawn[65].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        me->SummonCreature(NPC_BETRAYER_H,
                                           ThrallSpawn[65].x + rand32() % 5,
                                           ThrallSpawn[65].y + rand32() % 5,
                                           ThrallSpawn[65].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 6:
                        me->SummonCreature(NPC_FELBEAST_H,
                                           ThrallSpawn[65].x + rand32() % 5,
                                           ThrallSpawn[65].y + rand32() % 5,
                                           ThrallSpawn[65].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
            ...
            case 15:
                for (uint8 i = 0; i < 4; ++i) {
                    ...
                    case 0:
                        me->SummonCreature(NPC_FELGUARD_MORADEUR,
                                           ThrallSpawn[66].x + rand32() % 5,
                                           ThrallSpawn[66].y + rand32() % 5,
                                           ThrallSpawn[66].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
            ...
            // NPC_KHANOK - Inner Sunktum Spawn Top
            case 16:
                me->SummonCreature(NPC_DOOMGUARD_PILLARGER,
                                   ThrallSpawn[67].x + rand32() % 15,
                                   ThrallSpawn[67].y + rand32() % 15,
                                   ThrallSpawn[67].z + rand32() % 5,
                                   TEMPSUMMON_DEAD_DESPAWN);
                break;
            ...
            case 18:
                if (Creature* temp =
                    ...
                    temp->GetMotionMaster()->MovePath(
                        NPC_WARSONG_BATTLEGUARD * 100, false);
                }
                break;
            // Valimathras Room Preparation
            case 19:
                for (uint8 i = 0; i < 3; ++i)
                    me->SummonGameObject(GO_BLOCKED_PASSAGE,
                                         ThrallSpawn[i + 70].x,
                                         ThrallSpawn[i + 70].y,
                                         ThrallSpawn[70].z,
                                         ThrallSpawn[i + 70].o,
            ...
            case 20:
                for (uint8 i = 0; i < 3; ++i) {
                    switch (urand(0, 1)) {
                    case 0:
                        me->SummonCreature(NPC_LEGION_INVADER,
                                           ThrallSpawn[80].x + rand32() % 5,
                                           ThrallSpawn[80].y + rand32() % 5,
                                           ThrallSpawn[80].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    case 1:
                        me->SummonCreature(NPC_LEGION_DREADWHISPER,
                                           ThrallSpawn[81].x + rand32() % 5,
                                           ThrallSpawn[81].y + rand32() % 5,
                                           ThrallSpawn[81].z,
                                           TEMPSUMMON_DEAD_DESPAWN);
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 36:
                ...
                UpdateWorldState(
                    me->GetMap(), WORLD_STATE_INNER_SANKTUM_FIGHT_H, 1);
                break;
            case 46:
                SetHoldState(true);
                bStepping = true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (bStepping) {
                if (phaseTimer <= diff) {
                    ...
                    case 1:
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_BATTLE_COUNTDOWN_H, 1);
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        if (Creature* guard = me->FindNearestCreature(
                                NPC_HORDE_GUARD, 100.0f))
                    ...
                    case 11:
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_BATTLE_COUNTDOWN_H, 0);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_BATTLE_START_H, 1);
                        ...
                        SetRun(true);
                        if (Creature* sylvanas = ObjectAccessor::GetCreature(
                                *me, sylvanasfollowGUID)) {
                            sylvanas->GetMotionMaster()->MovePath(
                                NPC_SYLVANAS * 100, false);
                    ...
                    case 16: {
                        ...
                        me->GetCreatureListWithEntryInGrid(
                            PlagueList, NPC_PLAGUE_TRIGGER, 50.0f);
                        if (!PlagueList.empty())
                            for (std::list<Creature*>::iterator itr =
                                     PlagueList.begin();
                                 itr != PlagueList.end();
                    ...
                    case 20:
                        if (Creature* valimathras = ObjectAccessor::GetCreature(
                                *me, ValimathrasGUID))
                    ...
                    case 23:
                        ...
                        break;
                    case 24:
                        if (Creature* valimathrasportal =
                                ObjectAccessor::GetCreature(
                                    *me, ValimathrasPortalGUID))
                            valimathrasportal->CastSpell(
                                valimathrasportal, SPELL_VALIMATHRAS_PORTAL);
                        JumpToNextStep(12 * IN_MILLISECONDS);
                        break;
                    case 25:
                        if (Creature* valimathras = ObjectAccessor::GetCreature(
                                *me, ValimathrasGUID)) {
                            valimathras->GetMotionMaster()->MovePoint(
                                0, 1804.559f, 235.504f, 62.753f);
                    ...
                    case 26: {
                        ...
                        std::list<Creature*> HelperList;
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_SLINGER_TRIGGER, 1000.0f);
                        if (!HelperList.empty())
                            for (std::list<Creature*>::iterator itr =
                                     HelperList.begin();
                                 itr != HelperList.end();
                    ...
                    case 27:
                        ...
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_BATTLE_START_H, 0);
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_COURTYARD_FIGHT_H, 1);
                        JumpToNextStep(0);
                        break;
                    case 28: {
                        EnableAttack = true;
                        ...
                        me->GetCreatureListWithEntryInGrid(
                            ...
                            1000.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HostileEndList, NPC_DOCTOR_H, 1000.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HostileEndList, NPC_CHEMIST_H, 1000.0f);
                        if (!HostileEndList.empty())
                            for (std::list<Creature*>::iterator itr =
                                     HostileEndList.begin();
                                 itr != HostileEndList.end();
                                 itr++)
                                (*itr)->SetFaction(FACTION_MONSTER);
                        SpawnWave(4);
                        JumpToNextStep(10 * IN_MILLISECONDS);
                        break;
                    }
                    ...
                    case 46: {
                        ...
                        me->GetCreatureListWithEntryInGrid(
                            HostileList, NPC_TREACHEROUS_GUARDIAN_H, 1000.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HostileList, NPC_DOCTOR_H, 1000.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HostileList, NPC_CHEMIST_H, 1000.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HostileList, NPC_BLIGHT_SLINGER, 1000.0f);
                        if (!HostileList.empty())
                            for (std::list<Creature*>::iterator itr =
                                ...
                                (*itr)->DespawnOrUnsummon();
                        for (uint8 i = 0; i < 7; ++i)
                            me->SummonGameObject(GO_HORDE_BANNER,
                                                 ThrallSpawn[i + 37].x,
                                                 ThrallSpawn[i + 37].y,
                                                 ThrallSpawn[i + 37].z,
                                                 ThrallSpawn[i + 37].o,
                    ...
                    case 53:
                        SpawnWave(8);
                        me->GetMotionMaster()->MoveJump(ThrallSpawn[55].x,
                                                        ThrallSpawn[55].y,
                                                        ThrallSpawn[55].z,
                                                        40.0f,
                                                        40.0f,
                        ...
                        if (Creature* sylvanas = ObjectAccessor::GetCreature(
                                *me, sylvanasfollowGUID))
                            sylvanas->GetMotionMaster()->MoveJump(
                                ThrallSpawn[56].x,
                                ThrallSpawn[56].y,
                                ThrallSpawn[56].z,
                                40.0f,
                    ...
                    case 115: {
                        ...
                        GetGameObjectListWithEntryInGrid(
                            SaronitList, me, GO_BLOCKED_PASSAGE, 80.0f);
                        for (std::list<GameObject*>::const_iterator itr =
                                 SaronitList.begin();
                             itr != SaronitList.end();
                             ++itr)
                    ...
                    case 116:
                        ...
                        UpdateWorldState(
                            me->GetMap(), WORLD_STATE_ROYAL_QUARTER_FIGHT_H, 1);
                        FollowThrall();
                        SetEscortPaused(false);
                        bStepping = false;
                        JumpToNextStep(0 * IN_MILLISECONDS);
                    ...
                    case 118:
                        if (Creature* valimathras = ObjectAccessor::GetCreature(
                            ...
                            valimathras->CastSpell(
                                me, SPELL_OPENING_LEGION_PORTALS);
                        }
                        JumpToNextStep(3 * IN_MILLISECONDS);
                        break;
                    case 119:
                    ...
                    case 143:
                        if (Creature* sylvanas = ObjectAccessor::GetCreature(
                                *me, sylvanasfollowGUID)) {
                            sylvanas->GetMotionMaster()->MovePoint(
                                0, 1289.48f, 314.33f, -57.32f, true);
                            sylvanas->CastSpell(sylvanas,
                    ...
                    case 147:
                        for (uint8 i = 0; i < ALLIANCE_FORCE_MAXCOUNT; ++i) {
                            if (Creature* temp = me->SummonCreature(
                                    NPC_SW_SOLDIER,
                                    AllianceSpawn[i + 25].x,
                                    AllianceSpawn[i + 25].y,
                                    AllianceSpawn[i + 25].z,
                        ...
                        if (Creature* wrynn =
                            ...
                            wrynn->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                                  EMOTE_STATE_READY2H);
                            wrynn->SetReactState(REACT_PASSIVE);
                            wrynn->GetMotionMaster()->MovePoint(
                                0, 1302.543f, 359.472f, -67.295f, true);
                        }
                        if (Creature* jaina =
                                me->SummonCreature(NPC_JAINA,
                                                   1308.862f,
                                                   381.809f,
                                                   -66.044243f,
                                                   TEMPSUMMON_MANUAL_DESPAWN)) {
                    ...
                    case 158:
                        if (Creature* saurfang =
                                me->SummonCreature(NPC_OVERLORD_SAURFANG,
                                                   1297.574f,
                                                   347.154f,
                                                   -65.027f,
                                                   TEMPSUMMON_MANUAL_DESPAWN)) {
                            ...
                            saurfang->GetMotionMaster()->MovePoint(
                                0, 1300.862f, 353.670f, -66.187f, true);
                        }
                        JumpToNextStep(7 * IN_MILLISECONDS);
                        break;
                    case 159:
                        if (Creature* saurfang = ObjectAccessor::GetCreature(
                                *me, SaurfangGUID)) {
                    ...
                    case 163:
                        ...
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_SYLVANAS, 100.0f);
                        me->GetCreatureListWithEntryInGrid(
                            HelperList, NPC_OVERLORD_SAURFANG, 100.0f);
                        if (!HelperList.empty())
                            for (std::list<Creature*>::iterator itr =
                                     HelperList.begin();
                                 itr != HelperList.end();
                                 itr++)
                                (*itr)->DespawnOrUnsummon(120 *
                                                          IN_MILLISECONDS);
...
public:
    npc_lady_sylvanas_windrunner_bfu()
        : CreatureScript("npc_lady_sylvanas_windrunner_bfu")
    {
    }

    struct npc_lady_sylvanas_windrunner_bfuAI : public ScriptedAI {
        npc_lady_sylvanas_windrunner_bfuAI(Creature* creature)
            : ScriptedAI(creature)
        {
            Reset();
        }

        void InitializeAI() override { me->Mount(MODEL_SKELETON_MOUNT); }

        void JustRespawned() override { me->Mount(MODEL_SKELETON_MOUNT); }
        ...
        void Reset() override
        {
            ...
            me->ApplySpellImmune(
                0, IMMUNITY_STATE, SPELL_AURA_MOD_INCREASE_SPEED, true);
        }
