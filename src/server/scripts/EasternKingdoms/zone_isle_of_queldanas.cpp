...
public:
    npc_bh_thalorien_dawnseeker()
        : CreatureScript("npc_bh_thalorien_dawnseeker")
    {
    }
    ...
    struct npc_bh_thalorien_dawnseekerAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (Creature* c = me->FindNearestCreature(
                    NPC_THALORIEN_REMAINS, 100.0f, true))
        ...
        void SummonedCreatureDespawn(Creature* summon) override
        {
            ...
            if (summon->GetEntry() != NPC_SUNWELL_DEFENDER &&
                me->isActiveObject()) {
                if (summon->GetEntry() == NPC_MORLEN_COLDGRIP) {
                    me->RemoveAurasDueToSpell(67541);
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_NONE);
                    me->GetMotionMaster()->MoveCharge(11779.30f,
                                                      -7065.43f,
                                                      24.92f,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_SUMMON_MORLEN:
                if (Creature* c =
                        me->SummonCreature(NPC_MORLEN_COLDGRIP,
                                           11766.70f,
                                           -7050.57f,
                                           25.17f,
                                           5.56f,
            ...
            case EVENT_SPAWN_WAVE_3:
                if (Creature* c =
                        ObjectAccessor::GetCreature(*me, morlenGUID)) {
            ...
            case EVENT_SUMMONS_ATTACK:
                for (SummonList::const_iterator itr = summons.begin();
                     ...
                     ++itr)
                    if (Creature* c = ObjectAccessor::GetCreature(*me, *itr)) {
                        if (c->GetEntry() == NPC_MORLEN_COLDGRIP &&
                            summons.size() != 1)
                            continue;
                        else
...
public:
    spell_bh_cleanse_quel_delar()
        : SpellScriptLoader("spell_bh_cleanse_quel_delar")
    {
    }
    ...
    class spell_bh_cleanse_quel_delar_SpellScript : public SpellScript {
        ...
        void OnEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
                if (Creature* c =
                        caster->FindNearestCreature(NPC_ROMMATH, 50.0f, true))
                    c->AI()->DoAction(-1);
        }
...
public:
    ...
    struct npc_grand_magister_rommathAI : public NullCreatureAI {
        ...
        {
            if (!announced && who->GetTypeId() == TYPEID_PLAYER &&
                who->GetPositionZ() < 30.0f) {
                announced  = true;
                playerGUID = who->GetGUID();
                if (Creature* c =
                        me->FindNearestCreature(NPC_GALIROS, 100.0f, true))
                    c->AI()->Talk(0, who);
            }
        }
        ...
        void DoAction(int32 a) override
        {
            if (a == -1 && !me->isActiveObject()) {
                me->SummonGameObject(GO_QUEL_DELAR,
                                     1688.24f,
                                     621.769f,
                                     29.1745f,
                                     0.523177f,
                                     ...
                                     0.0f,
                                     86400);
                me->SummonCreature(NPC_SUNWELL_VISUAL_BUNNY,
                                   1688.24f,
                                   621.769f,
                                   29.1745f,
                                   0.523177f,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case 1:
                if (Creature* c = me->FindNearestCreature(
                        NPC_SUNWELL_HONOR_GUARD, 60.0f, true))
                    c->AI()->Talk(0);
                break;
            case 2:
                if (Creature* c =
                        me->FindNearestCreature(NPC_THERON, 60.0f, true))
                    c->AI()->Talk(0);
                break;
            case 3:
                me->SetWalk(true);
                ...
                if (Creature* c =
                        me->FindNearestCreature(NPC_AURIC, 60.0f, true)) {
                    c->SetWalk(true);
                    c->GetMotionMaster()->MovePath(c->GetEntry() * 100, false);
                }
                break;
            ...
            case 13:
                me->setActive(false);
                if (Creature* c = me->FindNearestCreature(
                        NPC_SUNWELL_VISUAL_BUNNY, 60.0f, true))
                    c->DespawnOrUnsummon(1);
                if (GameObject* go =
                        me->FindNearestGameObject(GO_QUEL_DELAR, 60.0f))
                    go->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
                me->SetWalk(true);
                if (me->GetCreatureData())
                    me->GetMotionMaster()->MovePoint(
                        0,
                        me->GetCreatureData()->posX,
                        me->GetCreatureData()->posY,
                        me->GetCreatureData()->posZ);
                if (Creature* c =
                        me->FindNearestCreature(NPC_THERON, 60.0f, true)) {
                    c->SetWalk(true);
                    if (c->GetCreatureData())
                        c->GetMotionMaster()->MovePoint(
                            0,
                            c->GetCreatureData()->posX,
                            c->GetCreatureData()->posY,
                            c->GetCreatureData()->posZ);
                }
                if (Creature* c =
                        me->FindNearestCreature(NPC_AURIC, 60.0f, true)) {
                    c->SetWalk(true);
...
public:
    ...
    struct npc_greengill_slaveAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 /*diff*/) override { DoMeleeAttackIfReady(); }
    };
};
