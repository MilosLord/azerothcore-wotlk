...
public:
    ...
    struct npc_lord_gregor_lescovarAI : public npc_escortAI {
        ...
        void EnterEvadeMode(EvadeReason /*why*/) override
        {
            ...
            if (Creature* pMarzon =
                    ObjectAccessor::GetCreature(*me, MarzonGUID)) {
                if (pMarzon->IsAlive())
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            case 16:
                SetEscortPaused(true);
                if (Creature* pMarzon =
                        me->SummonCreature(NPC_MARZON_BLADE,
                                           -8411.360352f,
                                           480.069733f,
                                           123.760895f,
                                           4.941504f,
                                           ...
                                           1000)) {
                    pMarzon->GetMotionMaster()->MovePoint(
                        0, -8408.000977f, 468.611450f, 123.759903f);
        ...
        // TO-DO: We don't have movemaps, also we can't make 2 npcs walks to one
        // point propperly (and we can not use escort ai, because they are 2
        // different spawns and with same entry), because of it we make them,
        // disappear.
        void DoGuardsDisappearAndDie()
        {
            std::list<Creature*> GuardList;
            me->GetCreatureListWithEntryInGrid(
                GuardList, NPC_STORMWIND_ROYAL, 8.0f);
            if (!GuardList.empty()) {
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            if (uiPhase) {
                if (uiTimer <= uiDiff) {
                    switch (uiPhase) {
                    case 1:
                        if (Creature* pGuard = me->FindNearestCreature(
                                NPC_STORMWIND_ROYAL, 8.0f, true))
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        if (Creature* pTyrion = me->FindNearestCreature(
                                NPC_TYRION, 20.0f, true))
...
public:
    ...
    struct npc_marzon_silent_bladeAI : public ScriptedAI {
        ...
        void Reset() override { me->RestoreFaction(); }
            ...
            if (me->IsSummon()) {
                if (Unit* summoner = me->ToTempSummon()->GetSummonerUnit()) {
                    if (summoner->GetTypeId() == TYPEID_UNIT &&
                        summoner->IsAlive() && !summoner->IsInCombat())
        ...
        void MovementInform(uint32 uiType, uint32 /*uiId*/) override
        {
            ...
            if (me->IsSummon()) {
                ...
                if (summoner && summoner->GetTypeId() == TYPEID_UNIT &&
                    summoner->IsAIEnabled) {
                    npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI* ai =
                        CAST_AI(npc_lord_gregor_lescovar::
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            if (uiPhase) {
                if (uiTimer <= uiDiff) {
                    ...
                    case 2:
                        if (Creature* pTyrion =
                                me->FindNearestCreature(NPC_TYRION, 10.0f)) {
                            if (Player* player = GetPlayerForEscort())
                    ...
                    case 5:
                        if (Creature* pGuard = me->FindNearestCreature(
                                NPC_STORMWIND_ROYAL, 10.0f, true))
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        if (Creature* pLescovar = me->FindNearestCreature(
                                NPC_LORD_GREGOR_LESCOVAR, 10.0f))
...
public:
                       ...
                       Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_THE_ATTACK) {
            if (Creature* pSpybot = creature->FindNearestCreature(
                    NPC_TYRION_SPYBOT, 5.0f, true)) {
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())
                    ->Start(false, false, player->GetGUID());
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())
                    ->SetMaxPlayerDistance(200.0f);
            }
