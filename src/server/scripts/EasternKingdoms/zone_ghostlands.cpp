...
struct npc_ranger_lilatha : public npc_escortAI {
    ...
    void WaypointReached(uint32 waypointId) override
    {
        ...
        case 33:
            ...
            Creature* CaptainHelios =
                me->FindNearestCreature(NPC_CAPTAIN_HELIOS, 50.0f);
            if (CaptainHelios)
...
struct npc_sentinel_leader : public ScriptedAI {
    ...
    void Reset() override { _helpCalled = false; }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == WAYPOINT_MOTION_TYPE) {
            ...
            case 14:
            case 17:
                Creature* SentinelSpy =
                    me->FindNearestCreature(NPC_SENTINEL_SPY, 2.0f, true);
                if (SentinelSpy) {
...
struct npc_sentinel_infiltrator : public ScriptedAI {
    ...
    void Initialize() { _path = me->GetWaypointPath(); }

    void Reset() override { _fleedForAssistance = false; }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == WAYPOINT_MOTION_TYPE) {
            ...
            case PATH_ONE:
                ...
                case 14:
                case 18:
                    Creature* SentinelInfiltrator = me->FindNearestCreature(
                        NPC_SENTINEL_INFILTRATOR, 3.5f, true);
                    if (SentinelInfiltrator) {
                        me->HandleEmoteCommand(EMOTE_ONESHOT_SALUTE);
                        SentinelInfiltrator->HandleEmoteCommand(
