...
public:
    ...
    struct boss_urok_doomhowlAI : public BossAI {
        boss_urok_doomhowlAI(Creature* creature)
            : BossAI(creature, DATA_UROK_DOOMHOWL)
        {
        }
        ...
        void InitializeAI() override
        {
            ...
            if (GameObject* challenge = instance->instance->GetGameObject(
                    instance->GetGuidData(GO_UROK_CHALLENGE))) {
                challenge->Delete();
            }

            if (GameObject* pile = instance->instance->GetGameObject(
                    instance->GetGuidData(GO_UROK_PILE))) {
...
void AddSC_boss_urok_doomhowl() { new boss_urok_doomhowl(); }
