...
std::vector<uint32> bearSpells  = {
    SPELL_DEMORALIZING_ROAR, SPELL_MAUL, SPELL_SWIPE};
std::vector<uint32> faerieSpells = {SPELL_ARCANE_EXPLOSION,
                                    SPELL_REFLECTION,
...
struct boss_mor_grayhoof : public BossAI {
    ...
    void CastRandomSpell(uint8 phase)
    {
        ...
        case PHASE_HUMAN:
            spell =
                Acore::Containers::SelectRandomContainerElement(humanSpells);
        ...
        case PHASE_FAERIE:
            spell =
                Acore::Containers::SelectRandomContainerElement(faerieSpells);
            if (spell == SPELL_SLEEP) {
                if (Unit* target = SelectTarget(
                        SelectTargetMethod::Random, 0, 50.f, true)) {
                    me->CastSpell(target, SPELL_SLEEP);

                    // Sleep can target tank, we need to drop threat temporarily
                    // on the target.
                    ...
                    _scheduler.Schedule(10s, [this](TaskContext /*context*/) {
                        if (Unit* sleepTarget = ObjectAccessor::GetUnit(
                                *me, _sleepTargetGUID)) {
                            me->GetThreatMgr().AddThreat(sleepTarget,
                                                         _sleepTargetThreat);
                        }
                    });
                }
                     ...
                     SpellSchoolMask /*school*/) override
    {
        if (_phase == PHASE_HUMAN && me->HealthBelowPct(75.f)) {
            ...
            _scheduler.Schedule(
                5s, 10s, PHASE_FAERIE, [this](TaskContext context) {
