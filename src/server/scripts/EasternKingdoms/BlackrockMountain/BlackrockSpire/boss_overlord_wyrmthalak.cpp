...
public:
    ...
    struct boss_overlordwyrmthalakAI : public BossAI {
        boss_overlordwyrmthalakAI(Creature* creature)
            : BossAI(creature, DATA_OVERLORD_WYRMTHALAK)
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!Summoned && HealthBelowPct(51)) {
                ...
                if (Unit* target = SelectTarget(
                    ...
                    if (Creature* warlord =
                            me->SummonCreature(NPC_SPIRESTONE_WARLORD,
                                               SummonLocation1,
                                               TEMPSUMMON_TIMED_DESPAWN,
                                               300 * IN_MILLISECONDS))
                        warlord->AI()->AttackStart(target);
                    if (Creature* berserker =
                            me->SummonCreature(NPC_SMOLDERTHORN_BERSERKER,
                                               SummonLocation2,
                                               TEMPSUMMON_TIMED_DESPAWN,
                                               300 * IN_MILLISECONDS))
...
void AddSC_boss_overlordwyrmthalak() { new boss_overlord_wyrmthalak(); }
