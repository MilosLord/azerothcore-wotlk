...
enum EventGroups { GROUP_THRASHER = 1, GROUP_BRAWLER, GROUP_WARMASTER };
...
public:
    ...
    struct boss_warmastervooneAI : public BossAI {
        boss_warmastervooneAI(Creature* creature)
            : BossAI(creature, DATA_WARMASTER_VOONE)
        {
        }
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(
                EVENT_THRASH, 3s, GROUP_BRAWLER, PHASE_BRAWLER);
            events.ScheduleEvent(
                EVENT_THROW_AXE, 1s, GROUP_BRAWLER, PHASE_BRAWLER);
        }
                         ...
                         SpellSchoolMask /*school*/) override
        {
            if (me->HealthBelowPctDamaged(65, damage) &&
                events.IsInPhase(PHASE_BRAWLER)) {
                events.SetPhase(PHASE_THRASHER);
                events.CancelEventGroup(GROUP_BRAWLER);
                events.ScheduleEvent(
                    EVENT_CLEAVE, 14s, GROUP_THRASHER, PHASE_THRASHER);
                events.ScheduleEvent(
                    EVENT_MORTAL_STRIKE, 12s, GROUP_THRASHER, PHASE_THRASHER);
            }
            else if (me->HealthBelowPctDamaged(40, damage) &&
                     events.IsInPhase(PHASE_THRASHER)) {
                events.SetPhase(PHASE_WARMASTER);
                events.CancelEventGroup(GROUP_THRASHER);
                events.ScheduleEvent(
                    EVENT_SNAP_KICK, 8s, GROUP_WARMASTER, PHASE_WARMASTER);
                events.ScheduleEvent(
                    EVENT_UPPERCUT, 20s, GROUP_WARMASTER, PHASE_WARMASTER);
                events.ScheduleEvent(
                    EVENT_PUMMEL, 32s, GROUP_WARMASTER, PHASE_WARMASTER);
            }
        }
...
void AddSC_boss_warmastervoone() { new boss_warmaster_voone(); }
