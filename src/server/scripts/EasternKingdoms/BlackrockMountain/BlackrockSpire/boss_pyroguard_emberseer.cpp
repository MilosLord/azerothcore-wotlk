...
enum Spells {
    ...
    SPELL_FIRE_SHIELD_TRIGGER = 13377, // Self on spawn missing from 335 dbc
                                       // triggers SPELL_FIRE_SHIELD every 3 sec
    ...
    SPELL_EMBERSEER_GROWING_TRIGGER =
        16049, // Triggered by SPELL_EMBERSEER_GROWING
...
public:
    ...
    struct boss_pyroguard_emberseerAI : public BossAI {
        boss_pyroguard_emberseerAI(Creature* creature)
            : BossAI(creature, DATA_PYROGAURD_EMBERSEER)
        {
        }
        ...
        void Reset() override
        {
            ...
            // DoCast(me, SPELL_FIRE_SHIELD_TRIGGER); // Need to find this in
            // old DBC if possible
        ...
        void SetData(uint32 /*type*/, uint32 data) override
        {
            ...
            case 2:
                ...
                if (GameObject* door1 = me->GetMap()->GetGameObject(
                        instance->GetGuidData(GO_EMBERSEER_IN)))
                    if (door1->GetGoState() == GO_STATE_ACTIVE)
                        door1->SetGoState(GO_STATE_READY);
                if (GameObject* door2 = me->GetMap()->GetGameObject(
                        instance->GetGuidData(GO_DOORS)))
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
        {
            ...
            if (spell->Id == SPELL_EMBERSEER_GROWING_TRIGGER) {
                ...
                if (me->GetAuraCount(SPELL_EMBERSEER_GROWING_TRIGGER) == 20) {
                    events.CancelEvent(
                        EVENT_FIRE_SHIELD); // temporarily cancel fire shield to
                                            // keep it from interrupting combat
                                            // start
        ...
        void OpenDoors(bool Boss_Killed)
        {
            ...
            if (Boss_Killed)
                if (GameObject* door3 = me->GetMap()->GetGameObject(
                        instance->GetGuidData(GO_EMBERSEER_OUT)))
        ...
        {
            // update all runes
            if (GameObject* rune1 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_1)))
                rune1->SetGoState(state);
            if (GameObject* rune2 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_2)))
                rune2->SetGoState(state);
            if (GameObject* rune3 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_3)))
                rune3->SetGoState(state);
            if (GameObject* rune4 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_4)))
                rune4->SetGoState(state);
            if (GameObject* rune5 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_5)))
                rune5->SetGoState(state);
            if (GameObject* rune6 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_6)))
                rune6->SetGoState(state);
            if (GameObject* rune7 = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_EMBERSEER_RUNE_7)))
...
public:
    ...
    struct npc_blackhand_incarceratorAI : public ScriptedAI {
        ...
        void Reset() override
        {
            ...
            if (Creature* Emberseer = me->FindNearestCreature(
                    NPC_PYROGAURD_EMBERSEER, 30.0f, true))
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            std::list<Creature*> creatureList;
            GetCreatureListWithEntryInGrid(
                creatureList, me, NPC_BLACKHAND_INCARCERATOR, 60.0f);
            for (std::list<Creature*>::iterator itr = creatureList.begin();
                 itr != creatureList.end();
                 ++itr) {
                if (Creature* creature = *itr)
                    creature
                        ->SetInCombatWithZone(); // AI()->AttackStart(me->GetVictim());
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_ENCAGE:
                    DoCast(
                        SelectTarget(SelectTargetMethod::Random, 0, 100, true),
                        EVENT_ENCAGE,
                        true);
