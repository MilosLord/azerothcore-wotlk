...
Position const OrcsRunawayPosition = {
    34.163567f, -536.852356f, 110.935196f, 6.056306f};
...
public:
    ...
    struct boss_thebeastAI : public BossAI {
        boss_thebeastAI(Creature* creature)
            : BossAI(creature, DATA_THE_BEAST), _beastReached(false),
              _orcYelled(false)
        {
        }
        ...
        void SetData(uint32 type, uint32 /*data*/) override
        {
                ...
                if (!_orcYelled) {
                    ...
                    for (ObjectGuid guid : _nearbyOrcsGUIDs) {
                        if (Creature* orc =
                            ...
                            orc->m_Events.AddEvent(new OrcMoveEvent(orc),
                                                   me->m_Events.CalculateTime(
                                                       3 * IN_MILLISECONDS));
                            orc->m_Events.AddEvent(new OrcDeathEvent(orc),
                                                   me->m_Events.CalculateTime(
                                                       9 * IN_MILLISECONDS));
                        }
                    }
            ...
            case DATA_BEAST_REACHED: {
                if (!_beastReached) {
                    ...
                    // There is a chance player logged in between areatriggers
                    // (realm crash or restart) executing part of script which
                    // happens when player enters boss room otherwise we will
                    // see weird behaviour when someone steps on the previous
                    // areatrigger (dead mob yelling/moving)
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_BERSERKER_CHARGE:
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0, 38.f, true)) {
                ...
                case EVENT_FIREBALL:
                    ...
                    events.ScheduleEvent(EVENT_FIREBALL, 8s, 21s);
                    if (events.GetNextEventTime(EVENT_FIREBLAST) <
                        3 * IN_MILLISECONDS) {
                ...
                case EVENT_FIREBLAST:
                    ...
                    events.ScheduleEvent(EVENT_FIREBLAST, 5s, 8s);
                    if (events.GetNextEventTime(EVENT_FIREBALL) <
                        3 * IN_MILLISECONDS) {
        ...
        void FindNearbyOrcs()
        {
            ...
            me->GetCreatureListWithEntryInGrid(
                temp, NPC_BLACKHAND_ELITE, 50.0f);
...
public:
    at_trigger_the_beast_movement()
        : AreaTriggerScript("at_trigger_the_beast_movement")
    {
    }
    ...
    bool OnTrigger(Player* player, AreaTrigger const* /*at*/) override
    {
        ...
        if (InstanceScript* instance = player->GetInstanceScript()) {
            if (Creature* beast = ObjectAccessor::GetCreature(
                    *player, instance->GetGuidData(DATA_THE_BEAST))) {
