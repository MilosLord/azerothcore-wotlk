...
uint32 const DragonspireMobs[3] = {
    NPC_BLACKHAND_DREADWEAVER, NPC_BLACKHAND_SUMMONER, NPC_BLACKHAND_VETERAN};
...
public:
    ...
    struct instance_blackrock_spireMapScript : public InstanceScript {
        ...
        uint32 SolakarState = NOT_STARTED; // there should be a global instance
                                           // encounter state, where is it?
        ...
        void CreatureLooted(Creature* creature, LootType loot) override
        {
            ...
            case NPC_THE_BEAST:
                if (loot == LOOT_SKINNING) {
                    creature->CastSpell(
                        creature, SPELL_FINKLE_IS_EINHORN, true);
                }
                break;
            }
        }
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_EMBERSEER_IN:
                ...
                HandleGameObject(ObjectGuid::Empty,
                                 GetBossState(DATA_DRAGONSPIRE_ROOM) == DONE,
                                 go);
                break;
            case GO_DOORS:
                go_doors = go->GetGUID();
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            case DATA_WARCHIEF_REND_BLACKHAND:
                if (state == FAIL) {
                    if (Creature* rend =
                            instance->GetCreature(WarchiefRendBlackhand)) {
                        rend->Respawn(true);
                    }

                    if (Creature* nefarius =
                            instance->GetCreature(LordVictorNefarius)) {
        ...
        void ProcessEvent(WorldObject* /*obj*/, uint32 eventId) override
        {
            ...
            case EVENT_PYROGUARD_EMBERSEER:
                if (GetBossState(DATA_PYROGAURD_EMBERSEER) == NOT_STARTED) {
                    if (Creature* Emberseer =
                            instance->GetCreature(PyroguardEmberseer))
            ...
            case EVENT_UROK_DOOMHOWL:
                if (GetBossState(DATA_UROK_DOOMHOWL) == NOT_STARTED) {
                    ...
                    if (GameObject* pile =
                            instance->GetGameObject(go_urokPile)) {
            ...
            case DATA_UROK_DOOMHOWL:
                if (data == FAIL) {
                    if (GetBossState(DATA_UROK_DOOMHOWL) != NOT_STARTED) {
                        ...
                        if (GameObject* challenge =
                                instance->GetGameObject(go_urokChallenge)) {
                            challenge->Delete();
                        }
                        if (GameObject* pile =
                                instance->GetGameObject(go_urokPile)) {
                        ...
                        for (const auto& circleGUID : go_urokOgreCirles) {
                            if (GameObject* circle =
                                    instance->GetGameObject(circleGUID)) {
                                circle->Delete();
                            }
                        }
                        for (const auto& mobGUID : UrokMobs) {
                            if (Creature* mob =
                                    instance->GetCreature(mobGUID)) {
        ...
        void SummonSolakarWave(uint8 number)
        {
            if (number < MAX_WAVE_COUNT) {
                if (Creature* summon = instance->SummonCreature(
                        NPC_ROOKERY_GUARDIAN, SolakarPosLeft)) {
                    SolakarSummons.push_back(summon->GetGUID());
                }

                if (Creature* summon = instance->SummonCreature(
                        NPC_ROOKERY_HATCHER, SolakarPosRight)) {
                    SolakarSummons.push_back(summon->GetGUID());
                }

                if (number == 0) {
                    if (Creature* FirstHatcher = instance->GetCreature(
                            SolakarSummons.back())) // works because we spawned
                                                    // a hatcher second
                    {
                ...
                if (Creature* summon =
                        instance->SummonCreature(NPC_SOLAKAR, SolakarPosBoss)) {
                    SolakarSummons.push_back(summon->GetGUID());
                }
            }
        }
            ...
            while (uint32 eventId = Events.ExecuteEvent()) {
                ...
                case EVENT_SOLAKAR_WAVE:
                    ...
                    if (CurrentSolakarWave < MAX_WAVE_COUNT) {
                        Events.ScheduleEvent(EVENT_SOLAKAR_WAVE,
                                             TIMER_SOLAKAR_WAVE);
                        CurrentSolakarWave++;
                    }
                    break;
        ...
        void Dragonspireroomstore()
        {
            ...
            for (uint8 i = 0; i < 7; ++i) {
                ...
                if (GameObject* rune =
                        instance->GetGameObject(go_roomrunes[i])) {
                    for (uint8 j = 0; j < 3; ++j) {
                        std::list<Creature*> creatureList;
                        GetCreatureListWithEntryInGrid(
                            creatureList, rune, DragonspireMobs[j], 15.0f);
                        for (std::list<Creature*>::iterator itr =
                                 creatureList.begin();
                             itr != creatureList.end();
                             ++itr) {
                            if (Creature* creature = *itr) {
                                runecreaturelist[i].push_back(
                                    creature->GetGUID());
                            }
                        }
                    }
        ...
        void Dragonspireroomcheck()
        {
            ...
            if (GetBossState(DATA_HALL_RUNE_1) == DONE &&
                GetBossState(DATA_HALL_RUNE_2) == DONE &&
                GetBossState(DATA_HALL_RUNE_3) == DONE &&
                GetBossState(DATA_HALL_RUNE_4) == DONE &&
                GetBossState(DATA_HALL_RUNE_5) == DONE &&
                GetBossState(DATA_HALL_RUNE_6) == DONE &&
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (player && player->IsAlive()) {
            ...
            Creature* rend;
            if (instance->GetBossState(DATA_WARCHIEF_REND_BLACKHAND) ==
                NOT_STARTED) {
                if ((rend = player->FindNearestCreature(
                         NPC_WARCHIEF_REND_BLACKHAND, 100.0f, false)))
                    rend->Respawn(true);
            }

            if ((rend = player->FindNearestCreature(NPC_WARCHIEF_REND_BLACKHAND,
                                                    100.0f))) {
...
public:
    ...
    void
    OnLootStateChanged(GameObject* go, uint32 state, Unit* /*unit*/) override
    {
        if (InstanceScript* instance = go->GetInstanceScript()) {
            if (state == GO_ACTIVATED) {
                if (instance->GetData(DATA_SOLAKAR_FLAMEWREATH) ==
                        IN_PROGRESS ||
...
public:
    near_scarshield_infiltrator()
        : AreaTriggerScript("near_scarshield_infiltrator")
    {
    }
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (player && player->IsAlive()) {
            if (Creature* creature = player->FindNearestCreature(
                    NPC_SCARSHIELD_INFILTRATOR, 100.0f, true)) {
                bool transformHasStarted = creature->AI()->GetData(0) == 1;
                if ((player->GetLevel() < 57 ||
                     !player->HasItemCount(ITEM_UNADORNED_SEAL)) &&
                    !transformHasStarted) {
                    // Send whisper if not already sent
                    std::list<ObjectGuid>::iterator itr =
                        std::find(whisperedTargets.begin(),
                                  whisperedTargets.end(),
                                  player->GetGUID());
                    if (itr == whisperedTargets.end()) {
                        creature->AI()->Talk(SAY_SCARSHIELD_INF_WHISPER,
                                             player);
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (player && player->IsAlive()) {
            if (Creature* creature = player->FindNearestCreature(
                ...
                if (player->GetLevel() >= 57 &&
                    player->HasItemCount(ITEM_UNADORNED_SEAL)) {
                    creature->AI()->SetData(0,
                                            1); // Start transform into Vaelan
                    return true;
                }
            }
        }
...
Position VaelastraszTheRedPosNorth =
    Position(168.815506f, -420.311066f, 110.472298f, 3.141593f);
Position VaelastraszTheRedPosSouth =
    Position(134.369049f, -420.311066f, 110.472298f, 6.283184f);
...
class spell_blackrock_spire_call_of_vaelastrasz : public SpellScript {
    ...
    void OnEffect(SpellEffIndex /*effIndex*/)
    {
        if (Unit* caster = GetCaster()) {
            if (InstanceScript* instance = caster->GetInstanceScript()) {
                ...
                float distanceToNorthSpawn = caster->GetDistance2d(
                    VaelastraszTheRedPosNorth.m_positionX,
                    VaelastraszTheRedPosNorth.m_positionY);
                float distanceToSouthSpawn = caster->GetDistance2d(
                    VaelastraszTheRedPosSouth.m_positionX,
                    VaelastraszTheRedPosSouth.m_positionY);
                Position spawnPosition =
                    distanceToNorthSpawn < distanceToSouthSpawn
                        ? VaelastraszTheRedPosNorth
                        : VaelastraszTheRedPosSouth;
                // despawn is called by the CreatureAI
                caster->SummonCreature(NPC_VAELASTRASZ_THE_RED,
                                       spawnPosition,
    ...
    {
        OnEffectLaunch +=
            SpellEffectFn(spell_blackrock_spire_call_of_vaelastrasz::OnEffect,
                          EFFECT_0,
...
public:
    ...
    struct npc_vaelastrasz_the_redAI : public CreatureAI {
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            if (Creature* victor =
                    me->FindNearestCreature(NPC_LORD_VICTOR_NEFARIUS, 100.0f)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_VAEL_1_START_COMBAT:
                    ...
                    if (Creature* gyth =
                            me->FindNearestCreature(NPC_GYTH, 100.0f, true)) {
                        me->AddThreat(gyth, 1000000.f);
                        me->AI()->AttackStart(gyth);
                    }
                    if (Creature* rend = me->FindNearestCreature(
                            NPC_WARCHIEF_REND_BLACKHAND, 100.0f, true)) {
                        if (!rend->IsImmuneToNPC() &&
                            rend->isTargetableForAttack()) {
                            me->AddThreat(rend, 100000.f);
                            if (!me->FindNearestCreature(
                                    NPC_GYTH, 100.0f, true)) {
                ...
                case EVENT_NEFARIUS_TALK_1:
                    if (Creature* victor =
                            ObjectAccessor::GetCreature(*me, _victorGUID)) {
            ...
            if (!_combatEnabled || !UpdateVictim() ||
