...
}; Position const Teleport1Loc = { 194.2993f, -474.0814f, 121.4505f,
-0.01225555f }; Position const Teleport2Loc = { 216.485f,  -434.93f,   110.888f,
-0.01225555f };*/
...
public:
    ...
    struct boss_rend_blackhandAI : public BossAI {
        boss_rend_blackhandAI(Creature* creature)
            : BossAI(creature, DATA_WARCHIEF_REND_BLACKHAND)
        {
        }
        ...
        void Reset() override
        {
            ...
            if (Creature* victor = me->FindNearestCreature(
                    NPC_LORD_VICTOR_NEFARIUS, 5.0f, true))
                victor->Respawn(true);

            if (GameObject* exitDoor = me->GetMap()->GetGameObject(
                    instance->GetGuidData(GO_GYTH_ENTRY_DOOR)))
                exitDoor->SetGoState(GO_STATE_ACTIVE);

            instance->SetBossState(DATA_WARCHIEF_REND_BLACKHAND, NOT_STARTED);
        }

        void SummonedCreatureDies(Creature* /*creature*/,
        ...
        void JustDied(Unit* /*killer*/) override
        {
            _JustDied();
            if (Creature* victor = me->FindNearestCreature(
                    NPC_LORD_VICTOR_NEFARIUS, 75.0f, true))
            ...
            if (type == AREATRIGGER && data == AREATRIGGER_BLACKROCK_STADIUM) {
                if (!gythEvent) {
                    ...
                    if (GameObject* portcullis =
                            me->FindNearestGameObject(GO_DR_PORTCULLIS, 50.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            if (gythEvent) {
                ...
                while (uint32 eventId = events.ExecuteEvent()) {
                    switch (eventId) {
                    case EVENT_START_1:
                        instance->SetBossState(DATA_WARCHIEF_REND_BLACKHAND,
                                               IN_PROGRESS);
                        instance->SetData(DATA_VAELASTRASZ, NOT_STARTED);

                        if (Creature* victor =
                                ObjectAccessor::GetCreature(*me, victorGUID))
                    ...
                    case EVENT_TURN_TO_PLAYER:
                        if (Creature* victor =
                                ObjectAccessor::GetCreature(*me, victorGUID))
                            if (Unit* player =
                                    victor->SelectNearestPlayer(60.0f))
                    ...
                    case EVENT_PATH_NEFARIUS:
                        if (Creature* victor =
                                ObjectAccessor::GetCreature(*me, victorGUID))
                            victor->GetMotionMaster()->MovePath(NEFARIUS_PATH_1,
                                                                true);
                        break;
                    ...
                    case EVENT_TELEPORT_1:
                        me->NearTeleportTo(
                            194.2993f, -474.0814f, 121.4505f, -0.01225555f);
                        events.ScheduleEvent(EVENT_TELEPORT_2, 13s);
                        break;
                    case EVENT_TELEPORT_2:
                        me->NearTeleportTo(
                            216.485f, -434.93f, 110.888f, -0.01225555f);
                        me->SummonCreature(NPC_GYTH,
                                           211.762f,
                                           -397.5885f,
                                           111.1817f,
                                           4.747295f);
        ...
        void SummonWave()
        {
            ...
            if (GameObject* waveDoor =
                    me->GetMap()->GetGameObject(waveDoorGUID)) {
