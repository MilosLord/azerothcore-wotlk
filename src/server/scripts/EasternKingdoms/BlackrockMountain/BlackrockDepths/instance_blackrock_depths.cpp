...
public:
    ...
    struct instance_blackrock_depths_InstanceMapScript : public InstanceScript {
        ...
        Position    GrimstonePositon =
            Position(625.559f, -205.618f, -52.735f, 2.609f);
        ...
        {
            ReplaceMoiraIfSaved(); // In case a player joins the party during
                                   // the run
        ...
        {
            ObjectGuid* GUIDToReplace =
                &PriestessGUID; // default to having Moira
            ...
            if (!lPlayers.IsEmpty()) {
                for (Map::PlayerList::const_iterator itr = lPlayers.begin();
                    ...
                    if (Player* player = itr->GetSource()) {
                        // set to false if this player hasn't saved her. Another
                        // player can't put it to true.
                        MoiraSaved =
                            MoiraSaved &&
                            ((player->GetQuestStatus(PRINCESS_QUEST_HORDE) ==
                              QUEST_STATUS_REWARDED) ||
                             (player->GetQuestStatus(PRINCESS_QUEST_ALLIANCE) ==
                              QUEST_STATUS_REWARDED));
                    }
                }
            }
            ...
            if (Creature* CreatureToReplace =
                    instance->GetCreature(*GUIDToReplace)) {
                Creature* NewSpawn = instance->SummonCreature(
                    NPCEntry, CreatureToReplace->GetPosition());
        ...
        void Initialize() override
        {
            ...
            // these are linked to the dungeon and not how many times the arena
            // started.
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_MAGMUS:
                ...
                if (!creature->IsAlive())
                    HandleGameObject(
                        GoThroneGUID,
                        true); // if Magmus is dead open door to last boss
                break;
            case NPC_WEAPON_TECHNICIAN:
            case NPC_DOOMFORGE_ARCANASMITH:
            case NPC_RAGEREAVER_GOLEM:
            case NPC_WRATH_HAMMER_CONSTRUCT:
                if (creature->IsAlive() && creature->GetPositionZ() < -51.5f &&
                    creature->GetPositionZ() > -55.f) {
            ...
            case NPC_SHADOWFORCE_CITIZEN: // both do the same
                if (creature->GetDistance2d(CenterOfRingOfLaw.GetPositionX(),
                                            CenterOfRingOfLaw.GetPositionY()) <
            ...
            case NPC_SHADOWFORGE_SENATOR:
                // keep track of Senators that are not too far from emperor.
                // Can't really use emperor as creature due to him possibly not
                // being spawned. some senators spawn at ring of law
                if (creature->GetDistance2d(EmperorSpawnPos.GetPositionX(),
                                            EmperorSpawnPos.GetPositionY()) <
        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            case NPC_SHADOWFORGE_SENATOR:
                deadSenators =
                    1; // hacky, but we cannot count the unit that just died
                       // through its state because OnUnitDeath() is called
                       // before the state is set.
                for (const auto& senatorGUID : EmperorSenatorsVector) {
                    if (Creature* senator =
                            instance->GetCreature(senatorGUID)) {
                ...
                if (Creature* emperor = instance->GetCreature(EmperorGUID)) {
                    ...
                    emperor->AI()->SetData(
                        0, (100 * deadSenators) / EmperorSenatorsVector.size());
                }
                break;
            case NPC_ANGERREL:
            case NPC_DOPEREL:
        ...
        void SetData(uint32 type, uint32 data) override
        {
            LOG_DEBUG(
                "scripts.ai",
                "Instance Blackrock Depths: SetData update (Type: {} Data {})",
                type,
                data);
            ...
            case TYPE_RING_OF_LAW:
                ...
                switch (data) {
                case IN_PROGRESS:
                    TempSummonGrimstone = instance->SummonCreature(
                        NPC_GRIMSTONE, GrimstonePositon);
                    break;
                case FAIL:
            ...
            case TYPE_LYCEUM:
                ...
                if (data == DONE) {
                    ...
                    ReplaceMoiraIfSaved(); // Need to place the correct final
                                           // boss, but we need her to be
                                           // spawned first.
                }
                break;
            case TYPE_IRON_HALL:
                ...
                case IN_PROGRESS:
                    for (int i = 0; i < 6; i++) {
                        if (Creature* ironhand =
                                instance->GetCreature(IronhandGUID[i])) {
            ...
            case DATA_OPEN_COFFER_DOORS:
                ...
                if (OpenedCoofers == 12) {
                    ...
                    if (TempSummon* summon = instance->SummonCreature(
                            NPC_WATCHMAN_DOOMGRIP, pos))
                        summon->SetTempSummonType(TEMPSUMMON_MANUAL_DESPAWN);
                }
                break;
            case DATA_GOLEM_LORD_ARGELMACH_INIT: {
                if (Creature* argelmach =
                        instance->GetCreature(ArgelmachGUID)) {
                    GuidList adds = ArgelmachAdds;
                    for (GuidList::const_iterator itr = adds.begin();
                         itr != adds.end();) {
                        if (Creature* argelmachAdd =
                                instance->GetCreature(*itr)) {
                            if (argelmachAdd->GetEntry() ==
                                NPC_WRATH_HAMMER_CONSTRUCT) {
                                argelmachAdd->RemoveAurasDueToSpell(
                                    SPELL_STONED);
                                argelmachAdd->AI()->AttackStart(
                                    argelmach->GetVictim());
                                itr = adds.erase(itr);
                            }
                            else if (argelmachAdd->GetEntry() ==
                                     NPC_RAGEREAVER_GOLEM) {
                                if (argelmachAdd->IsWithinDist2d(argelmach,
                                                                 10.f)) {
                                    argelmachAdd->RemoveAurasDueToSpell(
                                        SPELL_STONED);
                                    argelmachAdd->AI()->AttackStart(
                    ...
                    if (!adds.empty()) {
                        ...
                        argelmach->SetTarget();
                        argelmach->AI()->SetData(DATA_GOLEM_LORD_ARGELMACH_INIT,
                                                 IN_PROGRESS);
                    }
                    else {
                        argelmach->AI()->SetData(DATA_GOLEM_LORD_ARGELMACH_INIT,
            ...
            case DATA_GOLEM_LORD_ARGELMACH_ADDS: {
                if (Creature* argelmach =
                    ...
                    argelmach->m_Events.AddEvent(
                        new RestoreAttack(argelmach),
                        argelmach->m_Events.CalculateTime(3000));

                    for (ObjectGuid const& argelmachAddGUID : ArgelmachAdds) {
                        if (Creature* argelmachAdd =
                                instance->GetCreature(argelmachAddGUID)) {
        ...
        std::string GetSaveData() override { return str_data; }
        ...
        void TombOfSevenEvent()
        {
            if (GhostKillCount < 7 && TombBossGUIDs[TombEventCounter]) {
                if (Creature* boss = instance->GetCreature(
                        TombBossGUIDs[TombEventCounter])) {
                    ++TombEventCounter;
        ...
        void TombOfSevenReset()
        {
            ...
            for (uint8 i = 0; i < 7; ++i) {
                if (Creature* boss = instance->GetCreature(TombBossGUIDs[i])) {
                    if (!boss->IsAlive()) {
                        // do not call EnterEvadeMode(), it will create infinit
                        // loops
                        boss->Respawn();
                        boss->RemoveAllAuras();
        ...
        {
            bool anyBossAlive =
                false; // status of the bosses up until the current one
            for (uint8 i = 0; i < TombEventCounter; i++) {
                ...
                if (boss) {
                    ...
                    if (boss->IsAlive() && boss->IsInCombat()) {
                        tombResetTimer = TIMER_TOMB_RESET;
                        return false; // any boss in combat means we shouldn't
                                      // reset.
                    }
                }
            }
...
void AddSC_instance_blackrock_depths() { new instance_blackrock_depths(); }
