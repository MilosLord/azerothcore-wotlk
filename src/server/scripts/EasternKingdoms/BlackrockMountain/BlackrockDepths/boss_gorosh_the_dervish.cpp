...
public:
    ...
    struct boss_gorosh_the_dervishAI : public BossAI {
        boss_gorosh_the_dervishAI(Creature* creature)
            : BossAI(creature, DATA_GOROSH)
        {
        }
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case SPELL_WHIRLWIND:
                    if (me->GetDistance2d(me->GetVictim()) < 10.0f) {
                        ...
                        nextWhirlwindTime = urand(TIMER_WHIRLWIND - 2000,
                                                  TIMER_WHIRLWIND + 2000);
                    }
                    else {
                        // reschedule sooner
                        nextWhirlwindTime = 0.3 * urand(TIMER_WHIRLWIND - 2000,
                                                        TIMER_WHIRLWIND + 2000);
                    }
                ...
                case SPELL_MORTALSTRIKE:
                    ...
                    events.ScheduleEvent(
                        SPELL_MORTALSTRIKE,
                        urand(TIMER_MORTAL - 2000, TIMER_MORTAL + 2000));
                    break;
                case SPELL_BLOODLUST:
                    DoCastSelf(SPELL_BLOODLUST);
                    events.ScheduleEvent(
                        SPELL_BLOODLUST,
                        urand(TIMER_BLOODLUST - 2000, TIMER_BLOODLUST + 2000));
                    break;
                default:
                    break;
                }
...
void AddSC_boss_gorosh_the_dervish() { new boss_gorosh_the_dervish(); }
