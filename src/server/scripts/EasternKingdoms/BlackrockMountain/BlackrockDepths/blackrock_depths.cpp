...
public:
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        if (InstanceScript* instance = go->GetInstanceScript()) {
            GameObject* northBrazier = ObjectAccessor::GetGameObject(
                *go, instance->GetGuidData(DATA_SF_BRAZIER_N));
            GameObject* southBrazier = ObjectAccessor::GetGameObject(
                *go, instance->GetGuidData(DATA_SF_BRAZIER_S));
            ...
            if ((go->GetGUID() == northBrazier->GetGUID() &&
                 southBrazier->GetGoState() == GO_STATE_ACTIVE) ||
                (go->GetGUID() == southBrazier->GetGUID() &&
                 northBrazier->GetGoState() == GO_STATE_ACTIVE)) {
        ...
        void SetData(uint32 id, uint32 value) override
        {
            if (id == 0) {
                if (value == 0 || value == 1) {
                    ...
                    events.ScheduleEvent(SPELL_GOUT_OF_FLAMES,
                                         urand(1, IRONHAND_N_GROUPS) *
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            ...
            if (instance->GetData(TYPE_RING_OF_LAW) == IN_PROGRESS ||
                instance->GetData(TYPE_RING_OF_LAW) == DONE) {
                return false;
            }
            if (now - instance->GetData(DATA_TIME_RING_FAIL) <
                2 * 60) // in case of wipe, so people can rez.
            {
                return false;
            }
...
public:
    ...
    struct npc_grimstoneAI : public npc_escortAI {
            ...
            : npc_escortAI(creature), summons(me)
        {
            ...
            MobSpawnId =
                instance ? instance->GetData(DATA_ARENA_MOBS) : urand(0, 5);
            BossSpawnId =
                instance ? instance->GetData(DATA_ARENA_BOSS) : urand(0, 5);
            eventPhase    = 0;
            eventTimer    = 1000;
            resetTimer    = 0;
            theldrenEvent = false;
            ...
            case 5:
                if (instance) {
                    me->GetMap()->UpdateEncounterState(
                        ENCOUNTER_CREDIT_KILL_CREATURE, NPC_GRIMSTONE, me);
        ...
        void SummonBoss()
        {
            if (me->FindNearestGameObject(GO_BANNER_OF_PROVOCATION, 100.0f)) {
                theldrenEvent = true;
                me->SummonCreature(NPC_THELDREN,
                                   644.300f,
                                   -175.989f,
                                   -53.739f,
                                   3.418f,
                ...
                for (uint8 i = rand; i < rand + 4; ++i)
                    me->SummonCreature(theldrenTeam[i],
                                       644.300f,
                                       -175.989f,
                                       -53.739f,
                                       3.418f,
                ...
                me->SummonCreature(RingBoss[BossSpawnId],
                                   644.300f,
                                   -175.989f,
                                   -53.739f,
                                   3.418f,
        ...
        {
            // as long as the summoned creatures have someone to attack, we
            // reset the timer. once they don't find anyone, the timer will
            // count down until it is smaller than diff and reset.
            bool doReset = false;
            if (resetTimer > 0) {
                for (const auto& sum : summons) {
                    if (Creature* creature =
                            ObjectAccessor::GetCreature(*me, sum)) {
        ...
        void UpdateEscortAI(uint32 diff) override
        {
            ...
            if (eventTimer) {
                if (eventTimer <= diff) {
                    ...
                    case 9:
                        if (theldrenEvent) {
                            // All objects are removed from world once
                            // tempsummons despawn, so have a player spawn it
                            // instead.
                            Player* player = me->SelectNearestPlayer(100.0f);
                            if (GameObject* go =
                                    player->SummonGameObject(GO_ARENA_SPOILS,
                                                             596.48f,
                                                             -187.91f,
                                                             -54.14f,
                                                             4.9f,
...
public:
                        ...
                        uint32 action) override
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF + 1) {
            ...
            player->CastSpell(
                player, SPELL_CREATE_THORIUM_BROTHERHOOD_CONTRACT_DND, false);
        }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (creature->IsVendor() &&
            player->GetReputationRank(59) >= REP_FRIENDLY)
            AddGossipItemFor(
                player, 4781, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);

        if (player->GetQuestRewardStatus(QUEST_A_BINDING_CONTRACT) != 1 &&
            !player->HasItemCount(
                ITEM_THRORIUM_BROTHERHOOD_CONTRACT, 1, true) &&
...
public:
                       ...
                       uint32 /*item*/) override
    {
        ...
        if (instance->GetData(TYPE_BAR) == DONE ||
            instance->GetData(TYPE_BAR) == SPECIAL)
            return true;
        ...
        if (quest->GetQuestId() == QUEST_ALE) {
            ...
            // keep track of amount in instance script, returns SPECIAL if
            // amount ok and event in progress
            if (instance->GetData(TYPE_BAR) == SPECIAL) {
                creature->AI()->Talk(SAY_GOT_BEER);
                creature->CastSpell(creature, SPELL_DRUNKEN_RAGE, false);

                if (npc_escortAI* escortAI =
                        CAST_AI(npc_rocknot::npc_rocknotAI, creature->AI()))
    ...
    struct npc_rocknotAI : public npc_escortAI {
        ...
        {
            if (GameObject* go = instance->instance->GetGameObject(
                    instance->GetGuidData(id)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (BreakDoor_Timer) {
                if (BreakDoor_Timer <= diff) {
                    ...
                    DoGo(DATA_GO_BAR_KEG_TRAP,
                         0); // doesn't work very well, leaving code here for
                             // future
                    // spell by trap has effect61, this indicate the bar go
                    // hostile

                    if (Unit* tmp = ObjectAccessor::GetUnit(
                            *me, instance->GetGuidData(DATA_PHALANX)))
