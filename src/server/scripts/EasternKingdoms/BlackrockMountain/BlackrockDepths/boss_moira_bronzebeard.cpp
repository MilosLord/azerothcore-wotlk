...
struct boss_moira_bronzebeardAI : public BossAI {
    ...
    boss_moira_bronzebeardAI(Creature* creature, uint32 data = DATA_MOIRA)
        : BossAI(creature, data)
    {
    }
        ...
        while (uint32 eventId = events.ExecuteEvent()) {
            ...
            case SPELL_MINDBLAST:
                ...
                events.ScheduleEvent(
                    SPELL_MINDBLAST,
                    urand(TIMER_MINDBLAST - 2000, TIMER_MINDBLAST + 2000));
                break;
            case SPELL_HEAL:
    ...
    {
        Creature* emperor = ObjectAccessor::GetCreature(
            *me, instance->GetGuidData(DATA_EMPEROR));
...
// Running away when emperor dies is handled through GUID from emperor,
// therefore not relevant here.
struct boss_high_priestess_thaurissanAI : public boss_moira_bronzebeardAI {
    boss_high_priestess_thaurissanAI(Creature* creature)
        : boss_moira_bronzebeardAI(creature, DATA_PRIESTESS)
    {
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent()) {
            ...
            case SPELL_WORDPAIN:
                ...
                events.ScheduleEvent(
                    SPELL_WORDPAIN,
                    urand(TIMER_WORDPAIN - 2000, TIMER_WORDPAIN + 2000));
                break;
            case SPELL_HEAL:
                CastOnEmperorIfPossible(SPELL_HEAL, TIMER_HEAL);
                break;
            ...
            case SPELL_SHADOWBOLT:
                ...
                events.ScheduleEvent(
                    SPELL_SHADOWBOLT,
                    urand(TIMER_SHADOWBOLT - 2000, TIMER_SHADOWBOLT + 2000));
                break;
            default:
                break;
            }
...
public:
    boss_high_priestess_thaurissan()
        : CreatureScript("boss_high_priestess_thaurissan")
    {
    }
...
void AddSC_boss_moira_bronzebeard() { new boss_moira_bronzebeard(); }
