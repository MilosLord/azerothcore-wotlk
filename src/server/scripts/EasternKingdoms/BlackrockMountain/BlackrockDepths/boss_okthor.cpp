...
public:
    ...
    struct boss_okthorAI : public BossAI {
        ...
        {
            _JustEngagedWith();
            events.ScheduleEvent(SPELL_ARCANE_BOLT,
                                 0.2 * (int)TIMER_ARCANE_BOLT);
            events.ScheduleEvent(SPELL_ARCANE_EXPLOSION,
                                 0.2 * (int)TIMER_ARCANE_EXPLOSION);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case SPELL_ARCANE_BOLT:
                    ...
                    events.ScheduleEvent(SPELL_ARCANE_BOLT,
                                         urand(TIMER_ARCANE_BOLT - 2000,
                                               TIMER_ARCANE_BOLT + 2000));
                    break;
                case SPELL_ARCANE_EXPLOSION:
                    if (me->GetDistance2d(me->GetVictim()) < 50.0f) {
                        DoCast(SPELL_ARCANE_EXPLOSION);
                        nextArcaneExplosionTime =
                            urand(TIMER_ARCANE_EXPLOSION - 2000,
                                  TIMER_ARCANE_EXPLOSION + 2000);
                    }
                    else {
                        nextArcaneExplosionTime =
                            0.3 * urand(TIMER_ARCANE_EXPLOSION - 2000,
                                        TIMER_ARCANE_EXPLOSION + 2000);
                    }
                    events.ScheduleEvent(SPELL_ARCANE_EXPLOSION,
                                         nextArcaneExplosionTime);
                ...
                case SPELL_POLYMORPH:
                    ...
                    events.ScheduleEvent(
                        SPELL_POLYMORPH,
                        urand(TIMER_POLYMORPH - 2000, TIMER_POLYMORPH + 2000));
                    break;
                case SPELL_SLOW:
                    if (me->GetDistance2d(me->GetVictim()) < 50.0f) {
                        DoCast(SPELL_SLOW);
...
void AddSC_boss_okthor() { new boss_okthor(); }
