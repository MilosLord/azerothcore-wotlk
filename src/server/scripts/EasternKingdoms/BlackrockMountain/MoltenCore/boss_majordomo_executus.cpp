...
enum Spells {
    ...
    SPELL_SEPARATION_ANXIETY =
        21094, // Aura cast on himself by Majordomo Executus, if adds move out
               // of range, they will cast spell 21095 on themselves
    ...
    SPELL_RAGNAROS_SUBMERGE_EFFECT =
        21859, // Applies pacify state and applies all schools immunity
};
...
Position const MajordomoMoveRagPos = {
    ...
    -814.7055f,
    -228.9733f,
    0.0f}; // Position used at Ragnaros summoning event
Position const RagnarosSummonPos = {
    838.3082f, -831.4665f, -232.1853f, 2.199115f};
...
struct MajordomoAddData {
    ...
    MajordomoAddData() {}
    MajordomoAddData(ObjectGuid _guid,
                     uint32     _creatureEntry,
                     Position   _spawnPos)
        : guid(_guid), creatureEntry(_creatureEntry), spawnPos(_spawnPos)
    {
    }
};
...
public:
    ...
    struct boss_majordomoAI : public BossAI {
        boss_majordomoAI(Creature* creature)
            : BossAI(creature, DATA_MAJORDOMO_EXECUTUS)
        {
        }
                ...
                if (!p_summons.empty()) {
                    for (TempSummon const* summon : p_summons) {
                        if (summon) {
                            ...
                            majordomoSummonsData[summon->GetGUID()
                                                     .GetCounter()] =
                                MajordomoAddData(summon->GetGUID(),
                                                 summon->GetEntry(),
                                                 summon->GetPosition());
                        }
                    }
        ...
        void Reset() override
        {
            ...
            if (instance->GetBossState(DATA_MAJORDOMO_EXECUTUS) != DONE) {
                ...
                for (auto const& summon : majordomoSummonsData) {
                    ...
                    if (Creature* spawn =
                            me->SummonCreature(summon.second.creatureEntry,
                                               summon.second.spawnPos)) {
                        static_minionsGUIDS.erase(
                            summon.second
                                .guid); // Erase the guid from the previous, no
                                        // longer existing, spawn.
                        static_minionsGUIDS.insert(spawn->GetGUID());
                        majordomoSummonsData.erase(
                            summon.second.guid.GetCounter());
                        majordomoSummonsData[spawn->GetGUID().GetCounter()] =
                            MajordomoAddData(spawn->GetGUID(),
                                             spawn->GetEntry(),
                                             spawn->GetPosition());
                    }
                }
        ...
        void JustEngagedWith(Unit* /*attacker*/) override
        {
            ...
            events.ScheduleEvent(
                EVENT_SHIELD_REFLECTION, 30s, PHASE_COMBAT, PHASE_COMBAT);
            events.ScheduleEvent(
                EVENT_TELEPORT_RANDOM, 25s, PHASE_COMBAT, PHASE_COMBAT);
            events.ScheduleEvent(
                EVENT_TELEPORT_TARGET, 15s, PHASE_COMBAT, PHASE_COMBAT);
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            ...
            if (summon->GetEntry() == NPC_FLAMEWAKER_HEALER ||
                summon->GetEntry() == NPC_FLAMEWAKER_ELITE) {
                uint32 const remainingAdds = std::count_if(
                    aliveMinionsGUIDS.begin(),
                    aliveMinionsGUIDS.end(),
                    [](ObjectGuid const& summonGuid) {
                ...
                // 50% of adds
                    ...
                    me->GetMap()->UpdateEncounterState(
                        ENCOUNTER_CREDIT_KILL_CREATURE, me->GetEntry(), me);
                    me->SetImmuneToAll(true);
                    me->SetFaction(FACTION_MAJORDOMO_FRIENDLY);
                    EnterEvadeMode();
                    Talk(SAY_DEFEAT);
        ...
        void JustReachedHome() override
        {
            ...
            if (instance->GetBossState(DATA_MAJORDOMO_EXECUTUS) == DONE) {
                ...
                events.ScheduleEvent(EVENT_DEFEAT_OUTRO_1,
                                     7500ms,
                                     PHASE_DEFEAT_OUTRO,
                                     PHASE_DEFEAT_OUTRO);
            }
            ...
            case (1 << (PHASE_DEFEAT_OUTRO - 1)): {
                ...
                while (uint32 const eventId = events.ExecuteEvent()) {
                    ...
                    case EVENT_DEFEAT_OUTRO_1: {
                        Talk(SAY_DEFEAT_2);
                        events.ScheduleEvent(EVENT_DEFEAT_OUTRO_2,
                                             8s,
                                             PHASE_DEFEAT_OUTRO,
                                             PHASE_DEFEAT_OUTRO);
                        break;
                    ...
                    case EVENT_DEFEAT_OUTRO_2: {
                        Talk(SAY_DEFEAT_3);
                        events.ScheduleEvent(EVENT_DEFEAT_OUTRO_3,
                                             21500ms,
                                             PHASE_DEFEAT_OUTRO,
                                             PHASE_DEFEAT_OUTRO);
                        break;
                ...
                while (uint32 const eventId = events.ExecuteEvent()) {
                    ...
                    case EVENT_RAGNAROS_SUMMON_1: {
                        if (GameObject* lavaSplash =
                                ObjectAccessor::GetGameObject(
                                    *me,
                                    instance->GetGuidData(DATA_LAVA_SPLASH))) {
                        ...
                        if (GameObject* lavaSteam =
                                ObjectAccessor::GetGameObject(
                                    *me,
                                    instance->GetGuidData(DATA_LAVA_STEAM))) {
                        ...
                        me->SetWalk(true);
                        me->GetMotionMaster()->MovePoint(POINT_RAGNAROS_SUMMON,
                                                         MajordomoMoveRagPos,
                                                         true,
                                                         false);
                        break;
                    ...
                    case EVENT_RAGNAROS_SUMMON_2: {
                        ...
                        events.ScheduleEvent(EVENT_RAGNAROS_SUMMON_3,
                                             ...
                                             PHASE_RAGNAROS_SUMMONING);
                        events.ScheduleEvent(EVENT_RAGNAROS_EMERGE,
                                             15s,
                                             PHASE_RAGNAROS_SUMMONING,
                                             PHASE_RAGNAROS_SUMMONING);
                        break;
                    ...
                    case EVENT_RAGNAROS_SUMMON_3: {
                        if (Creature* ragnaros = ObjectAccessor::GetCreature(
                                *me, instance->GetGuidData(DATA_RAGNAROS))) {
                    ...
                    case EVENT_RAGNAROS_EMERGE: {
                        if (Creature* ragnaros = ObjectAccessor::GetCreature(
                                *me, instance->GetGuidData(DATA_RAGNAROS))) {
                            ragnaros->RemoveAurasDueToSpell(
                                SPELL_RAGNAROS_FADE);
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
            if (type == POINT_MOTION_TYPE && pointId == POINT_RAGNAROS_SUMMON) {
                ...
                events.ScheduleEvent(EVENT_RAGNAROS_SUMMON_2,
                                     11500ms,
                                     PHASE_RAGNAROS_SUMMONING,
                                     PHASE_RAGNAROS_SUMMONING);
            }
        ...
        {
            if (events.IsInPhase(PHASE_DEFEAT_OUTRO) &&
                spellInfo->Id == SPELL_TELEPORT_SELF) {
                me->SetNpcFlag(UNIT_NPC_FLAG_GOSSIP);
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_RAGNAROS_INTRO &&
                !events.IsInPhase(PHASE_RAGNAROS_SUMMONING)) {
                events.SetPhase(PHASE_RAGNAROS_SUMMONING);
                events.ScheduleEvent(EVENT_RAGNAROS_SUMMON_1,
                                     5s,
                                     PHASE_RAGNAROS_SUMMONING,
                                     PHASE_RAGNAROS_SUMMONING);
            }
    ...
    private:
        GuidSet static_minionsGUIDS; // contained data should be changed on
...
public:
    ...
    class spell_hate_to_zero_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_hate_to_zero_SpellScript::HandleHit,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    spell_majordomo_separation_nexiety()
        : SpellScriptLoader("spell_majordomo_separation_nexiety")
    {
    }
    ...
    class spell_majordomo_separation_nexiety_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_majordomo_separation_nexiety_AuraScript::HandlePeriodic,
                EFFECT_0,
...
public:
    ...
    class spell_summon_ragnaros_SpellScript : public SpellScript {
        ...
        {
            if (Unit* caster = GetCaster()) {
                caster->SummonCreature(NPC_RAGNAROS,
                                       RagnarosSummonPos,
        ...
        {
            AfterCast +=
                SpellCastFn(spell_summon_ragnaros_SpellScript::HandleHit);
        }
    };
