...
enum Texts {
    ...
    SAY_KNOCKBACK       = 7, // Text is used for SPELL_HAND_OF_RAGNAROS &
                             // SPELL_MIGHT_OF_RAGNAROS. "By fire be purged!"
    SAY_WRATH      = 8,
    SAY_KILL       = 9,
    SAY_MAGMABURST = 10,
};
...
enum Spells {
    ...
    SPELL_LAVA_BURST = 21908, // Randomly trigger one of server side spells
                              // (21886, 21900 - 21907) which summons Go 178088
    ...
    SPELL_RAGNAROS_SUBMERGE_EFFECT =
        21859, // Applies pacify state and applies all schools immunity (server
               // side)
    ...
    SPELL_SUMMON_SONS_FLAME =
        21108, // Trigger the eight spells summoning the Son of Flame adds
...
enum Misc {
    ...
    PHASE_SUBMERGED = 3, // events which are executed while Ragnaros is
...
public:
    ...
    struct boss_ragnarosAI : public BossAI {
        boss_ragnarosAI(Creature* creature)
            : BossAI(creature, DATA_RAGNAROS), _isIntroDone(false),
              _hasYelledMagmaBurst(false), _processingMagmaBurst(false),
              _hasSubmergedOnce(false), _isKnockbackEmoteAllowed(true)
        {
        }
        ...
        void Reset() override
        {
            ...
            if (_isIntroDone &&
                !(extraEvents.GetPhaseMask() & (1 << (PHASE_INTRO - 1)))) {
                extraEvents.Reset();
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            ...
            if (events.IsInPhase(PHASE_SUBMERGED) &&
                !summons.HasEntry(NPC_SON_OF_FLAME)) {
                HandleEmerge();
            }
        }
        ...
        void EnterEvadeMode(EvadeReason why) override
        {
            if (!me->GetThreatMgr().GetThreatList().empty()) {
                if (!_processingMagmaBurst) {
                    // Boss try to evade, but still got some targets on threat
                    // list - it means that none of these targets are in melee
                    // range - cast magma blast
                    _processingMagmaBurst = true;
                    events.ScheduleEvent(
                        EVENT_MAGMA_BLAST, 4s, PHASE_EMERGED, PHASE_EMERGED);
                }
            }
            else {
            ...
            if (!extraEvents.Empty()) {
                ...
                while (uint32 const eventId = extraEvents.ExecuteEvent()) {
                    ...
                    case EVENT_INTRO_SAY: {
                        ...
                        extraEvents.ScheduleEvent(EVENT_INTRO_MAKE_ATTACKABLE,
                                                  2500ms,
                                                  0,
                                                  PHASE_INTRO);
                        break;
                    ...
                    case EVENT_LAVA_BURST_TRIGGER: {
                        if (!_lavaBurstGUIDS.empty()) {
                            ...
                            events.RescheduleEvent(EVENT_LAVA_BURST,
                                                   10s,
                                                   PHASE_EMERGED,
                                                   PHASE_EMERGED);
                        }
            ...
            while (uint32 const eventId = events.ExecuteEvent()) {
                ...
                case EVENT_HAND_OF_RAGNAROS: {
                    ...
                    if (_isKnockbackEmoteAllowed) {
                        ...
                        extraEvents.RescheduleEvent(EVENT_RESET_KNOCKBACK_EMOTE,
                                                    5s);
                    }
                ...
                case EVENT_MIGHT_OF_RAGNAROS: {
                    if (Unit* target = SelectTarget(
                        ...
                        if (me->CastSpell(target, SPELL_MIGHT_OF_RAGNAROS) ==
                                SPELL_CAST_OK &&
                            _isKnockbackEmoteAllowed) {
                            Talk(SAY_KNOCKBACK, me);
                            _isKnockbackEmoteAllowed = false;
                            extraEvents.RescheduleEvent(
                                EVENT_RESET_KNOCKBACK_EMOTE, 5s);
                        }
                    }
                ...
                case EVENT_SUBMERGE: {
                    ...
                    Talk(_hasSubmergedOnce ? SAY_REINFORCEMENTS2
                                           : SAY_REINFORCEMENTS1);
                    ...
                    extraEvents.ScheduleEvent(
                        EVENT_EMERGE, 90s, PHASE_SUBMERGED, PHASE_SUBMERGED);
                    break;
                }
                }
    ...
    private:
        ...
        void ScheduleCombatEvents()
        {
            events.RescheduleEvent(
                EVENT_ERUPTION, 15s, PHASE_EMERGED, PHASE_EMERGED);
            events.RescheduleEvent(
                EVENT_WRATH_OF_RAGNAROS, 30s, PHASE_EMERGED, PHASE_EMERGED);
            events.RescheduleEvent(
                EVENT_HAND_OF_RAGNAROS, 25s, PHASE_EMERGED, PHASE_EMERGED);
            events.RescheduleEvent(
                EVENT_LAVA_BURST, 10s, PHASE_EMERGED, PHASE_EMERGED);
            events.RescheduleEvent(
                EVENT_SUBMERGE, 180s, PHASE_EMERGED, PHASE_EMERGED);
            events.RescheduleEvent(
                EVENT_MIGHT_OF_RAGNAROS, 11s, PHASE_EMERGED, PHASE_EMERGED);
        }
...
constexpr std::array<uint32, 8> RagnarosLavaBurstSpells = {SPELL_LAVA_BURST_A,
                                                           SPELL_LAVA_BURST_B,
                                                           SPELL_LAVA_BURST_C,
                                                           SPELL_LAVA_BURST_D,
                                                           SPELL_LAVA_BURST_E,
...
class spell_ragnaros_lava_burst_randomizer : public SpellScript {
    ...
    void HandleScript()
    {
        if (Unit* caster = GetCaster()) {
            ...
            for (uint8 i = 0; i < 3; ++i) {
                uint32 spell = Acore::Containers::SelectRandomContainerElement(
    ...
    {
        AfterCast +=
            SpellCastFn(spell_ragnaros_lava_burst_randomizer::HandleScript);
    }
};

constexpr std::array<uint32, 8> RagnarosSoFSpells = {
...
class spell_ragnaros_summon_sons_of_flame : public SpellScript {
    ...
    {
        AfterCast +=
            SpellCastFn(spell_ragnaros_summon_sons_of_flame::HandleScript);
    }
};
