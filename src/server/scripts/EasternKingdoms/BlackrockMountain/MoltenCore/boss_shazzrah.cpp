...
enum Spells {
    ...
    SPELL_SHAZZRAH_GATE_DUMMY =
        23138, // Teleports to and attacks a random target. About every 45
               // seconds Shazzrah will blink to random target causing a wipe of
               // the threat list (source: wowwwiki)
    ...
    class spell_shazzrah_gate_dummy_SpellScript : public SpellScript {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            ...
            if (caster && target) {
                ...
                if (Creature* creatureCaster = caster->ToCreature()) {
                    ...
                    creatureCaster->AI()->AttackStart(
                        target); // Attack the target which caster will teleport
                                 // to.
                }
            }
        }
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_shazzrah_gate_dummy_SpellScript::FilterTargets,
                EFFECT_0,
                TARGET_UNIT_SRC_AREA_ENEMY);
            OnEffectHitTarget += SpellEffectFn(
                spell_shazzrah_gate_dummy_SpellScript::HandleScript,
                EFFECT_0,
                SPELL_EFFECT_DUMMY);
        }
    };
