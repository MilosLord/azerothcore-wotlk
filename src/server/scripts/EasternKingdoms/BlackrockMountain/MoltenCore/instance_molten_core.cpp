...
public:
    ...
    struct instance_molten_core_InstanceMapScript : public InstanceScript {
            ...
            case GO_CIRCLE_SULFURON: {
                for (uint8 i = 0; i < MAX_MC_LINKED_BOSS_OBJ; ++i) {
                    ...
                    if (GetBossState(linkedBossObjData[i].bossId) == DONE) {
                        ...
                        _circlesGUIDs[linkedBossObjData[i].bossId] =
                            go->GetGUID();
                    }
                }
            ...
            case GO_RUNE_KORO: {
                for (uint8 i = 0; i < MAX_MC_LINKED_BOSS_OBJ; ++i) {
                    ...
                    if (GetBossState(linkedBossObjData[i].bossId) == DONE) {
                        ...
                        _runesGUIDs[linkedBossObjData[i].bossId] =
                            go->GetGUID();
                    }
                }
                break;
        ...
        bool SetBossState(uint32 bossId, EncounterState state) override
        {
            ...
            if (bossId == DATA_MAJORDOMO_EXECUTUS && state == DONE) {
                if (GameObject* cache =
                        instance->GetGameObject(_cacheOfTheFirelordGUID)) {
                ...
                case FAIL: {
                    if (!_golemaggMinionsGUIDS.empty()) {
                        for (ObjectGuid const& minionGuid :
                             _golemaggMinionsGUIDS) {
                            Creature* minion =
                                instance->GetCreature(minionGuid);
            ...
            if (bossId < DATA_MAJORDOMO_EXECUTUS && state == DONE) {
                if (GameObject* circle =
                        instance->GetGameObject(_circlesGUIDs[bossId])) {
                    circle->DespawnOrUnsummon(0ms, Seconds(WEEK));
                    _circlesGUIDs[bossId].Clear();
                }

                if (GameObject* rune =
                        instance->GetGameObject(_runesGUIDs[bossId])) {
        ...
        void SummonMajordomoExecutus()
        {
            ...
            if (GetBossState(DATA_MAJORDOMO_EXECUTUS) != DONE) {
                if (Creature* creature = instance->SummonCreature(
                        NPC_MAJORDOMO_EXECUTUS, MajordomoSummonPos)) {
                    creature->AI()->Talk(SAY_SPAWN);
                }
            }
            else {
                instance->SummonCreature(NPC_MAJORDOMO_EXECUTUS,
                                         MajordomoRagnaros);
            }
        }
    ...
    private:
        std::unordered_map<uint32 /*bossid*/, ObjectGuid /*circleGUID*/>
            _circlesGUIDs;
        std::unordered_map<uint32 /*bossid*/, ObjectGuid /*runeGUID*/>
            _runesGUIDs;
...
void AddSC_instance_molten_core() { new instance_molten_core(); }
