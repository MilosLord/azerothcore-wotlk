...
enum Spells {
    ...
    SPELL_ANTIMAGIC_PULSE =
        19492, // Dispels magic on nearby enemies, removing 1 beneficial spell
    SPELL_MAGMA_SHACKLES =
        19496, // Reduces the movement speed of nearby enemies by 60%
    SPELL_SEPARATION_ANXIETY =
        23487, // Aura cast on himself by Garr, if adds move out of range, they
               // will cast spell 23492 on themselves (server side)
    SPELL_FRENZY = 19516, // Increases the caster's attack speed by 9 + scale.
                          // Stacks up to 10 times

    // Fireworn
    SPELL_SEPARATION_ANXIETY_MINION =
        23492, // Increases damage done by 300% and applied banish immunity
    SPELL_ERUPTION =
        19497, // Deals fire aoe damage and knockbacks nearby enemies
    SPELL_MASSIVE_ERUPTION = 20483, // Deals fire aoe damage, knockbacks nearby
                                    // enemies and kills caster
    SPELL_ERUPTION_TRIGGER = 20482, // Removes banish auras and applied immunity
                                    // to banish (server side)
...
public:
    ...
    struct boss_garrAI : public BossAI {
        boss_garrAI(Creature* creature)
            : BossAI(creature, DATA_GARR), massEruptionTimer(600000) // 10 mins
        {
        }
    ...
    struct npc_garr_fireswornAI : public ScriptedAI {
                         ...
                         SpellSchoolMask /*damageSchoolMask*/) override
        {
            if (damage >= me->GetHealth()) {
                // Prevent double damage because Firesworn can kill himself with
                // Massive Erruption
                if (me != attacker) {
...
public:
    spell_garr_separation_nexiety()
        : SpellScriptLoader("spell_garr_separation_nexiety")
    {
    }
    ...
    class spell_garr_separation_nexiety_AuraScript : public AuraScript {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_garr_separation_nexiety_AuraScript::HandlePeriodic,
                EFFECT_0,
...
public:
    ...
    class spell_garr_frenzy_SpellScript : public SpellScript {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_garr_frenzy_SpellScript::HandleHit,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
    };
