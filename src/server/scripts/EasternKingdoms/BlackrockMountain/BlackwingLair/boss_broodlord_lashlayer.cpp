...
public:
    ...
    struct boss_broodlordAI : public BossAI {
        boss_broodlordAI(Creature* creature)
            : BossAI(creature, DATA_BROODLORD_LASHLAYER)
        {
        }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            GetGameObjectListWithEntryInGrid(
                _goList, me, GO_SUPPRESSION_DEVICE, 200.0f);
            for (std::list<GameObject*>::const_iterator itr = _goList.begin();
                 itr != _goList.end();
                 itr++) {
...
public:
    ...
    {
        switch (state) {
        case GO_JUST_DEACTIVATED: // This case prevents the Gameobject despawn
                                  // by Disarm Trap
