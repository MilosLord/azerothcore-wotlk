...
enum Gossip { GOSSIP_ID = 21332, GOSSIP_OPTION_ID = 0 };
...
struct ClassCallSelector : public Acore::unary_function<Unit*, bool> {
    ClassCallSelector(Unit const* unit, uint8 targetClass)
        : _me(unit), _targetClass(targetClass)
    {
    }
...
public:
    ...
    struct boss_victor_nefariusAI : public BossAI {
        boss_victor_nefariusAI(Creature* creature)
            : BossAI(creature, DATA_NEFARIAN)
        {
            Initialize();

            _nefarianLeftTunnel = instance->GetData(DATA_NEFARIAN_LEFT_TUNNEL);
            _nefarianRightTunnel =
                instance->GetData(DATA_NEFARIAN_RIGHT_TUNNEL);

            if (!_nefarianLeftTunnel || !_nefarianRightTunnel) {
                // Victor Nefarius weekly mechanic drakonid spawn
                // Pick 2 drakonids and keep them for the whole save duration
                // (the drakonids can't be repeated).
                std::vector<uint32> nefarianDrakonidSpawners = {
                    NPC_BLACK_SPAWNER,
                    NPC_BLUE_SPAWNER,
                    NPC_BRONZE_SPAWNER,
                    NPC_GREEN_SPAWNER,
                ...
                // save it to instance
                instance->SetData(DATA_NEFARIAN_LEFT_TUNNEL,
                                  _nefarianLeftTunnel);
                instance->SetData(DATA_NEFARIAN_RIGHT_TUNNEL,
                                  _nefarianRightTunnel);
            }
        }

        void Initialize() { KilledAdds = 0; }
        ...
        void Reset() override
        {
            ...
            if (me->GetMapId() == 469) {
                if (Creature* nefarian =
                        me->FindNearestCreature(NPC_NEFARIAN, 1000.0f, true)) {
                    // Nefarian is spawned and he didn't finish his intro path
                    // yet, despawn it manually.
                    if (nefarian->GetMotionMaster()
                            ->GetCurrentMovementGeneratorType() ==
                        MovementGeneratorType::WAYPOINT_MOTION_TYPE) {
        ...
        void DoAction(int32 action) override
        {
            ...
            if (action == ACTION_ADD_KILLED) {
                ...
                if (KilledAdds == MAX_DRAKONID_KILLED) {
                    if (Creature* nefarian =
                            me->SummonCreature(NPC_NEFARIAN, NefarianSpawn)) {
                        nefarian->setActive(true);
                        nefarian->SetCanFly(true);
                        nefarian->SetDisableGravity(true);
                        nefarian->GetMotionMaster()->MovePath(NEFARIAN_PATH,
                                                              false);
                    }
        ...
        void BeginEvent()
        {
            ...
            AttackStart(
                SelectTarget(SelectTargetMethod::Random, 0, 200.f, true));
            ...
            if (!UpdateVictim()) {
                ...
                while (uint32 eventId = events.ExecuteEvent()) {
                    ...
                    case EVENT_PATH_2:
                        ...
                        break;
                    case EVENT_CHAOS_1:
                        if (Creature* gyth = me->FindNearestCreature(
                                NPC_GYTH, 75.0f, true)) {
                    ...
                    case EVENT_SUCCESS_1:
                        if (Unit* player = me->SelectNearestPlayer(60.0f)) {
                            ...
                            Talk(SAY_SUCCESS);
                            if (GameObject* portcullis1 =
                                    me->FindNearestGameObject(
                                        GO_PORTCULLIS_ACTIVE, 65.0f))
                                portcullis1->SetGoState(GO_STATE_ACTIVE);
                            if (GameObject* portcullis2 =
                                    me->FindNearestGameObject(
                                        GO_PORTCULLIS_TOBOSSROOMS, 80.0f))
                ...
                while (uint32 eventId = events.ExecuteEvent()) {
                    ...
                    case EVENT_SHADOW_BOLT_VOLLEY:
                        ...
                        events.ScheduleEvent(
                            EVENT_SHADOW_BOLT_VOLLEY, 19s, 25s);
                        break;
                    case EVENT_FEAR:
                    ...
                    case EVENT_SPAWN_ADDS:
                        // Spawn the spawners.
                        me->SummonCreature(_nefarianLeftTunnel,
                                           spawnerPositions[0]);
                        me->SummonCreature(_nefarianRightTunnel,
                                           spawnerPositions[1]);
                        break;
                    }
...
struct boss_nefarian : public BossAI {
    boss_nefarian(Creature* creature)
        : BossAI(creature, DATA_NEFARIAN), _introDone(false)
    {
        Initialize();
    }

    void Initialize() { Phase3 = false; }
    ...
    void Reset() override
    {
        ...
        if (_introDone) // already in combat, reset properly.
        {
            _Reset();
            if (Creature* victor =
                    me->FindNearestCreature(NPC_VICTOR_NEFARIUS, 200.f, true)) {
                if (victor->AI()) {
                     ...
                     SpellSchoolMask) override
    {
        if (me->HealthBelowPctDamaged(20, damage) && !Phase3) {
            ...
            me->GetCreatureListWithEntryInGrid(
                constructList, NPC_BONE_CONSTRUCT, 500.0f);
            ...
            case EVENT_CLASSCALL:
                if (classesPresent.empty()) {
                    for (auto& ref : me->GetThreatMgr().GetThreatList()) {
                        if (ref->getTarget() &&
                            ref->getTarget()->GetTypeId() == TYPEID_PLAYER) {
                ...
                if (Unit* target =
                        SelectTarget(SelectTargetMethod::Random,
                                     0,
                                     ClassCallSelector(me, targetClass))) {
...
struct npc_corrupted_totem : public ScriptedAI {
    ...
    void SetAura(bool apply) const
    {
        ...
        std::vector<uint32> mobsEntries = {NPC_NEFARIAN,
                                           NPC_BONE_CONSTRUCT,
                                           NPC_BRONZE_DRAKONID,
                                           NPC_BLUE_DRAKONID,
                                           NPC_RED_DRAKONID,
    ...
    void IsSummonedBy(WorldObject* /*summoner*/) override
    {
        ...
        _scheduler
            .Schedule(1ms,
                      [this](TaskContext context) {
                          if (me->GetEntry() == NPC_TOTEM_C_FIRE_NOVA) {
                              if (!_auraAdded) {
                                  context.Schedule(
                                      4s, [this](TaskContext /*context*/) {
                                          if (me->IsAlive()) {
                                              DoCastAOE(SPELL_FIRE_NOVA_EFFECT,
                                                        true);
                                          }
                                      });
                                  _auraAdded = true;
            ...
            .Schedule(me->GetEntry() == NPC_TOTEM_C_WINDFURY ? 89s : 59s,
                      [this](TaskContext /*context*/) {
                          SetAura(false);
                          me->DespawnOrUnsummon();
                      });
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*unit*/) override
    {
        if (summon->GetEntry() != NPC_BONE_CONSTRUCT) {
            ...
            _scheduler
                .Schedule(
                    1s,
                    [this, summonGuid](TaskContext /*context*/) {
                        if (Creature* construct =
...
class spell_class_call_handler : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (SpellInfo const* spellInfo = GetSpellInfo()) {
            targets.remove_if([spellInfo](WorldObject const* target) -> bool {
                ...
                if (!player ||
                    player->IsClass(
                        CLASS_DEATH_KNIGHT)) // ignore all death knights from
                                             // whatever spell, for some reason
                                             // the condition below is not
                                             // working x.x
                {
                    return true;
                }
                ...
                if (it !=
                    classCallSpells
                        .end()) // should never happen but only to be sure.
                {
    ...
    void HandleOnHitRogue(SpellEffIndex /*effIndex*/)
    {
        ...
        target->NearTeleportTo(tp.GetPositionX(),
                               tp.GetPositionY(),
                               tp.GetPositionZ(),
                               tp.GetOrientation());
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_class_call_handler::FilterTargets,
            EFFECT_ALL,
            TARGET_UNIT_SRC_AREA_ENEMY);

        if (m_scriptSpellId == SPELL_ROGUE) {
            OnEffectLaunchTarget +=
                SpellEffectFn(spell_class_call_handler::HandleOnHitRogue,
                              EFFECT_0,
...
class aura_class_call_wild_magic : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(aura_class_call_wild_magic::HandlePeriodic,
                                 EFFECT_0,
...
class aura_class_call_siphon_blessing : public AuraScript {
    ...
    void HandlePeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Unit* target = GetTarget()) {
            if (Unit* nefarian =
                    target->FindNearestCreature(NPC_NEFARIAN, 100.f)) {
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            aura_class_call_siphon_blessing::HandlePeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_class_call_polymorph : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([&](WorldObject const* target) -> bool {
            return target->GetTypeId() != TYPEID_PLAYER ||
                   ...
                   target->ToPlayer()->HasAura(SPELL_POLYMORPH);
        });
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_class_call_polymorph::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ALLY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_class_call_polymorph::FilterTargetsEff,
            EFFECT_1,
            TARGET_UNIT_SRC_AREA_ALLY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_class_call_polymorph::FilterTargetsEff,
...
class spell_corrupted_totems : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_CORRUPTED_FIRE_NOVA_TOTEM,
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        std::list<uint32> spellList = {SPELL_CORRUPTED_FIRE_NOVA_TOTEM,
                                       SPELL_CORRUPTED_HEALING_TOTEM,
                                       SPELL_CORRUPTED_STONESKIN_TOTEM,
                                       SPELL_CORRUPTED_WINDFURY_TOTEM};
        uint32            spellId =
            Acore::Containers::SelectRandomContainerElement(spellList);
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_corrupted_totems::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_shadowblink : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_SHADOWBLINK_TRIGGERED_1,
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        Unit* target = caster->ToCreature()->AI()->SelectTarget(
            SelectTargetMethod::Random, 0, 200.f, true);
        if (!target) {
            return;
        }

        for (auto& itr : spellPos) {
            float distTarget = target->GetDistance2d(itr.second.m_positionX,
                                                     itr.second.m_positionY);
            if (distTarget <= 30.f) {
        ...
        auto spellId =
            Acore::Containers::SelectRandomContainerElement(spellPos);
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_shadowblink::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_spawn_drakonid : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_SPAWN_BLACK_DRAKONID,
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_spawn_drakonid::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
