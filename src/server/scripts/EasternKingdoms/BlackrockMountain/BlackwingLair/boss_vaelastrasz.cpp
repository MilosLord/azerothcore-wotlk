...
constexpr float aNefariusSpawnLoc[4] = {
    -7466.16f, -1040.80f, 412.053f, 2.14675f};
...
enum Spells {
    ...
    SPELL_CLEAVE = 19983, // Chain cleave is most likely named something
                          // different and contains a dummy effect
...
public:
    ...
    struct boss_vaelAI : public BossAI {
        boss_vaelAI(Creature* creature)
            : BossAI(creature, DATA_VAELASTRAZ_THE_CORRUPT)
        {
            Initialize();
        }
        ...
        void UpdateAI(uint32 diff) override
        {
                ...
                while (uint32 eventId = _eventsIntro.ExecuteEvent()) {
                    ...
                    case EVENT_SPEECH_2:
                        if (Creature* nefarius =
                                me->GetMap()->GetCreature(m_nefariusGuid)) {
                            nefarius->CastSpell(me,
                    ...
                    case EVENT_SPEECH_3:
                        if (Creature* nefarius =
                                me->GetMap()->GetCreature(m_nefariusGuid))
                            nefarius->CastSpell(
                                me, SPELL_RED_LIGHTNING, TRIGGERED_NONE);
                    ...
                    case EVENT_SPEECH_7:
                        ...
                        if (PlayerGUID &&
                            ObjectAccessor::GetUnit(*me, PlayerGUID))
                            AttackStart(
                                ObjectAccessor::GetUnit(*me, PlayerGUID));
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_BURNING_ADRENALINE: {
                    if (_burningAdrenalineCast <
                        2) // It's better to use TaskScheduler for this, but zzz
                    {
                        // selects a random target that isn't the current victim
                        // and is a mana user (selects mana users) but not pets
                        // it also ignores targets who have the aura. We don't
                        // want to place the debuff on the same target twice.
                        if (Unit* target = SelectTarget(
                                SelectTargetMethod::Random, 0, [&](Unit* u) {
                                    return u && !u->IsPet() &&
                                           u->getPowerType() == POWER_MANA &&
                                           !u->HasAura(
                                               SPELL_BURNING_ADRENALINE) &&
                                           u != me->GetVictim();
                                })) {
                            me->CastSpell(
                                target, SPELL_BURNING_ADRENALINE, true);
                        }
                        ...
                        me->CastSpell(
                            me->GetVictim(), SPELL_BURNING_ADRENALINE, true);
                        _burningAdrenalineCast = 0;
                    }
                    events.ScheduleEvent(EVENT_BURNING_ADRENALINE, 15s);
                    break;
...
class spell_vael_burning_adrenaline : public AuraScript {
    ...
    {
        return ValidateSpellInfo({SPELL_BURNING_ADRENALINE_EXPLOSION,
                      ...
                      AuraEffectHandleModes /*mode*/)
    {
        ...
        GetTarget()->CastSpell(
            GetTarget(), SPELL_BURNING_ADRENALINE_EXPLOSION, true);
        GetTarget()->CastSpell(
            GetTarget(), SPELL_BURNING_ADRENALINE_INSTAKILL, true);
    }

    void Register() override
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_vael_burning_adrenaline::HandleRemove,
                               EFFECT_2,
