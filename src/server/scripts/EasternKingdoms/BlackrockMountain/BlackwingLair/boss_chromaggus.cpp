...
enum Spells {
    ...
    SPELL_TIMELAPSE = 23310,  // Time lapse 23310, 23311(old threat mod that was
                             // removed in 2.01)
    SPELL_CORROSIVEACID = 23313, // Corrosive Acid 23313, 23314
    SPELL_IGNITEFLESH   = 23315, // Ignite Flesh 23315, 23316
    SPELL_FROSTBURN =
        23187, // Frost burn 23187, 23189
               //  Brood Affliction 23173 - Scripted Spell that cycles through
               //  all targets within 100 yards and has a chance to cast one of
               //  the afflictions on them Since Scripted spells arn't coded
               //  I'll just write a function that does the same thing
    ...
    SPELL_CHROMATIC_MUT_1 =
        23174, // Spell cast on player if they get all 5 debuffs
...
public:
    ...
    struct boss_chromaggusAI : public BossAI {
        boss_chromaggusAI(Creature* creature)
            : BossAI(creature, DATA_CHROMAGGUS)
        {
            Initialize();

            // Select the 2 breaths that we are going to use until despawned so
            // we don't end up casting 2 of the same breath.
            _breathSpells = {SPELL_INCINERATE,
                             SPELL_TIMELAPSE,
                             SPELL_CORROSIVEACID,
                             SPELL_IGNITEFLESH,
                             SPELL_FROSTBURN};
            ...
            // Hack fix: This is here to prevent him from being pulled from the
            // floor underneath, remove it once maps are fixed.
            creature->SetImmuneToAll(true);
        }

        void Initialize() { Enraged = false; }
        ...
        void SetGUID(ObjectGuid guid, int32 id) override
        {
            if (id == GUID_LEVER_USER) {
                _playerGUID = guid;
                // Hack fix: This is here to prevent him from being pulled from
                // the floor underneath, remove it once maps are fixed.
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_AFFLICTION: {
                    uint32 afflictionSpellID = RAND(SPELL_BROODAF_BLUE,
                                                    SPELL_BROODAF_BLACK,
                                                    SPELL_BROODAF_RED,
...
public:
    ...
    struct go_chromaggus_leverAI : public GameObjectAI {
        go_chromaggus_leverAI(GameObject* go)
            : GameObjectAI(go), _instance(go->GetInstanceScript())
        {
        }
            ...
            if (reportUse) {
                if (_instance->GetBossState(DATA_CHROMAGGUS) != DONE &&
                    ...
                    if (Creature* creature =
                            _instance->GetCreature(DATA_CHROMAGGUS)) {
                        creature->SetHomePosition(homePos);
                        creature->GetMotionMaster()->MovePath(
                            creature->GetEntry() * 10, false);
                        creature->AI()->SetGUID(player->GetGUID(),
                                                GUID_LEVER_USER);
                    }

                    if (GameObject* go =
                            _instance->GetGameObject(DATA_GO_CHROMAGGUS_DOOR))
                        _instance->HandleGameObject(
                            ObjectGuid::Empty, true, go);
                }
...
class spell_gen_elemental_shield : public SpellScript {
    ...
    {
        return ValidateSpellInfo({SPELL_FIRE_ELEMENTAL_SHIELD,
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Unit* caster = GetCaster()) {
            for (uint32 spell = SPELL_FIRE_ELEMENTAL_SHIELD;
                 spell <= SPELL_ARCANE_ELEMENTAL_SHIELD;
                 ++spell) {
                caster->RemoveAurasDueToSpell(spell);
            }

            caster->CastSpell(
                caster, SPELL_FIRE_ELEMENTAL_SHIELD + urand(0, 4), true);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_elemental_shield::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_brood_power : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_RED_BROOD_POWER,
    ...
    {
        if (Unit* caster = GetCaster()) {
            for (uint32 spell = SPELL_RED_BROOD_POWER;
                 spell <= SPELL_GREEN_BROOD_POWER;
                 ++spell) {
                caster->RemoveAurasDueToSpell(spell);
            }

            caster->CastSpell(caster,
                              RAND(SPELL_RED_BROOD_POWER,
                                   SPELL_BLUE_BROOD_POWER,
                                   SPELL_BRONZE_BROOD_POWER,
                                   SPELL_BLACK_BROOD_POWER,
                                   SPELL_GREEN_BROOD_POWER),
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_brood_power::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
