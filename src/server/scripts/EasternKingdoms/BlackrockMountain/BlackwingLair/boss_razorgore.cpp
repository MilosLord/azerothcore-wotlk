...
public:
    ...
    struct boss_razorgoreAI : public BossAI {
        boss_razorgoreAI(Creature* creature)
            : BossAI(creature, DATA_RAZORGORE_THE_UNTAMED)
        {
        }
        ...
        void DoChangePhase()
        {
            ...
            if (Creature* troops =
                    instance->GetCreature(DATA_NEFARIAN_TROOPS)) {
        ...
        void OnCharmed(bool apply) override
        {
            if (apply) {
                if (Unit* charmer =
                        ObjectAccessor::GetUnit(*me, _charmerGUID)) {
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        if (InstanceScript* instance = go->GetInstanceScript())
            if (instance->GetData(DATA_EGG_EVENT) != DONE &&
                !player->HasAura(SPELL_MIND_EXHAUSTION) && !player->GetPet())
