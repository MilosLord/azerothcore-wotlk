...
DoorData const doorData[] = {
    {GO_PORTCULLIS_RAZORGORE,
     ...
     DOOR_TYPE_PASSAGE}, // ID 175946 || GUID 7230
    {
        GO_PORTCULLIS_RAZORGORE_ROOM,
        DATA_RAZORGORE_THE_UNTAMED,
        DOOR_TYPE_ROOM,
    }, // ID 176964 || GUID 75158
    {GO_PORTCULLIS_VAELASTRASZ,
     DATA_VAELASTRAZ_THE_CORRUPT,
     DOOR_TYPE_PASSAGE}, // ID 175185 || GUID 7229
    {GO_PORTCULLIS_BROODLORD,
     DATA_BROODLORD_LASHLAYER,
     DOOR_TYPE_PASSAGE}, // ID 179365 || GUID 75159
    {GO_PORTCULLIS_CHROMAGGUS_EXIT,
     DATA_CHROMAGGUS,
     DOOR_TYPE_PASSAGE}, // ID 179117 || GUID 75164
    {GO_PORTCULLIS_CHROMAGGUS_EXIT,
     DATA_NEFARIAN,
     DOOR_TYPE_ROOM}, // ID 179117 || GUID 75164
...
public:
    ...
    struct instance_blackwing_lair_InstanceMapScript : public InstanceScript {
        ...
        {
            // This is required because the tempspawn at Vael overwrites his
            // GUID.
            ...
            case NPC_RED_DRAKONID:
                if (Creature* nefarius =
                        GetCreature(DATA_LORD_VICTOR_NEFARIUS)) {
            ...
            case DATA_NEFARIAN:
                ...
                case NOT_STARTED:
                    if (Creature* nefarian =
                            instance->GetCreature(nefarianGUID)) {
            ...
            if (type == DATA_EGG_EVENT) {
                ...
                case SPECIAL:
                    ...
                    if (++EggCount >= EggList.size()) {
                        if (Creature* razor =
                                instance->GetCreature(razorgoreGUID)) {
        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            case NPC_BLACKWING_DRAGON:
                --addsCount[0];
                if (EggEvent != DONE &&
                    _events.GetTimeUntilEvent(EVENT_RAZOR_SPAWN) ==
                        Milliseconds::max()) {
                    ...
                    if (EggEvent == IN_PROGRESS) {
                        ...
                        for (uint8 i = urand(2, 5); i > 0; --i) {
                            ...
                            if (dragonkinsCount >= 12) {
                                //... and more than 40 orcs - stop spawning more
                                //adds.
                                if (orcsCount >= 40) {
                                    spawnMoreAdds = false;
                                    break;
                            ...
                            else if (orcsCount >= 40 &&
                                     mobEntry != NPC_BLACKWING_DRAGON) {
                                continue;
                            }

                            if (Creature* summon = instance->SummonCreature(
                                    mobEntry, SummonPosition[urand(0, 7)])) {
    ...
    class spell_bwl_shadowflame_SpellScript : public SpellScript {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return ValidateSpellInfo(
                {SPELL_ONYXIA_SCALE_CLOAK, SPELL_SHADOW_FLAME_DOT});
        }

        void HandleEffectScriptEffect(SpellEffIndex /*effIndex*/)
        {
            // If the victim of the spell does not have "Onyxia Scale Cloak" -
            // add the Shadow Flame DoT (22682)
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(
                spell_bwl_shadowflame_SpellScript::HandleEffectScriptEffect,
                EFFECT_0,
...
enum orb_of_command_misc { QUEST_BLACKHANDS_COMMAND = 7761, MAP_BWL = 469 };
...
public:
    ...
    {
        if (!player->IsAlive() &&
            player->GetQuestRewardStatus(QUEST_BLACKHANDS_COMMAND)) {
            player->TeleportTo(MAP_BWL,
                               orbOfCommandTP.m_positionX,
                               orbOfCommandTP.m_positionY,
                               orbOfCommandTP.m_positionZ,
                               orbOfCommandTP.m_orientation);
            return true;
        }
        return false;
    }
