...
struct boss_hazzarah : public BossAI {
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        summon->m_Events.AddEventAtOffset(
            [summon]() { summon->SetVisible(true); }, 2s);
            ...
            case EVENT_CHAIN_BURN:
                if (me->GetPowerPct(POWER_MANA) > 5.f) // totally guessed
                {
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 1, [&](Unit* u) {
                                return u && !u->IsPet() &&
...
class spell_chain_burn : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([caster](WorldObject* target) -> bool {
            ...
            return !unit || unit->getPowerType() != POWER_MANA ||
                   caster->GetVictim() == unit;
        });
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_chain_burn::FilterTargets,
                                          EFFECT_0,
