...
enum Events { EVENT_FROSTBREATH = 1, EVENT_MASSIVEGEYSER = 2, EVENT_SLAM = 3 };
...
class spell_gahzranka_slam : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (Unit* caster = GetCaster()) {
            _wipeThreat =
                targets.size() < caster->GetThreatMgr().GetThreatListSize();
        }
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_gahzranka_slam::FilterTargets,
                                          EFFECT_1,
                                          TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_gahzranka_slam::HandleWipeThreat,
                          EFFECT_1,
...
class spell_pagles_point_cast : public SpellScript {
        ...
        if (Unit* caster = GetCaster()) {
            if (InstanceScript* instanceScript = caster->GetInstanceScript()) {
                if (!instanceScript->GetData(DATA_GAHZRANKA) &&
                    !caster->FindNearestCreature(NPC_GAHZRANKA, 50.0f)) {
                    caster->m_Events.AddEventAtOffset(
                        [caster]() {
                            if (GameObject* lure = caster->SummonGameObject(
                                ...
                                caster->m_Events.AddEventAtOffset(
                                    [caster]() {
                                        if (!caster->FindNearestCreature(
                                                NPC_GAHZRANKA, 50.0f)) {
                                            caster->CastSpell(
                                                caster, SPELL_SPLASH, true);
    ...
    void Register() override
    {
        OnEffectLaunch += SpellEffectFn(spell_pagles_point_cast::OnEffect,
                                        EFFECT_1,
