...
enum BatIds { NPC_BLOODSEEKER_BAT = 11368, NPC_BATRIDER = 14750 };

enum Phase { PHASE_ONE = 1, PHASE_TWO = 2 };

Position const SpawnBat[6] = {{-12291.6220f, -1380.2640f, 144.8304f, 5.483f},
...
Position const JeklikCaveHomePosition = {
    -12291.9f, -1380.08f, 144.902f, 2.28638f};

enum PathID { PATH_JEKLIK_INTRO = 145170, PATH_BATRIDER_LOOP = 147500 };

enum BatRiderMode { BATRIDER_MODE_TRASH = 1, BATRIDER_MODE_BOSS };
        ...
        scheduler
            .Schedule(10s,
                      ...
                      [this](TaskContext context) {
                          if (Unit* target =
                                  SelectTarget(SelectTargetMethod::MinDistance,
                                               0,
                                               -8.0f,
                                               false,
                                               false)) {
            ...
            .Schedule(30s, PHASE_ONE, [this](TaskContext context) {
                ...
                if (Unit* target =
                    ...
                    for (uint8 i = 0; i < 6; ++i) {
                        if (Creature* bat = me->SummonCreature(
                                NPC_BLOODSEEKER_BAT,
                                SpawnBat[i],
...
struct npc_batrider : public CreatureAI {
    ...
    npc_batrider(Creature* creature) : CreatureAI(creature)
    {
        ...
        if (me->GetEntry() == NPC_BATRIDER && me->IsSummon() &&
            me->ToTempSummon() && me->ToTempSummon()->GetSummoner() &&
            me->ToTempSummon()->GetSummoner()->GetEntry() ==
...
class spell_batrider_bomb : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_batrider_bomb::HandleScriptEffect,
                          EFFECT_1,
