...
DoorData const doorData[] = {{GO_FORCEFIELD, DATA_ARLOKK, DOOR_TYPE_ROOM},
                             {0, 0, DOOR_TYPE_ROOM}};

ObjectData const creatureData[] = {{NPC_HIGH_PRIEST_THEKAL, DATA_THEKAL},
...
Position const edgeOfMagnessSummonPos = {
    -11901.229f, -1906.366f, 65.358f, 0.942f};
...
struct go_brazier_of_madness : public GameObjectAI {
    ...
    bool GossipHello(Player* /*player*/, bool reportUse) override
    {
        ...
        uint32 bossEntry = 0;
        for (uint8 i = 0; i < 4; ++i) {
            if (sGameEventMgr->IsActiveEvent(
                    BrazierOfMadnessContainer[i].first)) {
