...
Position const PosMandokir[2] = {{-12167.8f, -1927.25f, 153.73f, 3.76991f},
...
void RevivePlayer(Unit* victim, ObjectGuid& reviveGUID)
{
    ...
    GetCreatureListWithEntryInGrid(
        chainedSpirits, victim, NPC_CHAINED_SPIRIT, 200.f);
    ...
    struct boss_mandokirAI : public BossAI {
        ...
        void Reset() override
        {
            ...
            if (me->GetPositionZ() > 140.0f) {
                ...
                if (Creature* speaker = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(NPC_VILEBRANCH_SPEAKER))) {
                    if (!speaker->IsAlive()) {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            std::list<Creature*> chainedSpirits;
            GetCreatureListWithEntryInGrid(
                chainedSpirits, me, NPC_CHAINED_SPIRIT, 200.f);
            for (Creature* spirit : chainedSpirits)
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            for (int i = 0; i < CHAINED_SPIRIT_COUNT; ++i) {
                me->SummonCreature(NPC_CHAINED_SPIRIT,
                                   PosSummonChainedSpirits[i],
        ...
        void KilledUnit(Unit* victim) override
        {
            ...
            if (++killCount == 3) {
                Talk(SAY_DING_KILL);
                if (Creature* jindo = ObjectAccessor::GetCreature(
                        *me, instance->GetGuidData(DATA_JINDO))) {
                    if (jindo->IsAlive()) {
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_REVIVE) {
                ...
                GetCreatureListWithEntryInGrid(
                    creatures, me, NPC_CHAINED_SPIRIT, 200.0f);
                if (creatures.empty())
                    return;
                ...
                for (std::list<Creature*>::iterator itr = creatures.begin();
                     itr != creatures.end();
                     ++itr) {
                    if (Creature* chainedSpirit = ObjectAccessor::GetCreature(
                            *me, (*itr)->GetGUID())) {
        ...
        void SetGUID(ObjectGuid const guid, int32 type) override
        {
            if (type == ACTION_CHARGE) {
                if (_chargeTarget.first == guid && _chargeTarget.second > 0.f) {
                    if (Unit* target =
                            ObjectAccessor::GetUnit(*me, _chargeTarget.first)) {
        ...
        void CalculateThreat(Unit*            hatedUnit,
                             float&           threat,
                             SpellInfo const* threatSpell) override
        {
            if (_chargeTarget.first == hatedUnit->GetGUID()) {
                // Do not count DOTs/HOTs
                if (!(threatSpell &&
                      (threatSpell->HasAura(SPELL_AURA_DAMAGE_SHIELD) ||
                       threatSpell->HasAttribute(
        ...
        void DoMeleeAttackIfReady(bool ignoreCasting)
        {
            ...
            if (me->isAttackReady()) {
                // xinef: prevent base and off attack in same time, delay attack
                // at 0.2 sec
                ...
                me->AttackerStateUpdate(
                    victim, BASE_ATTACK, false, ignoreCasting);
                me->resetAttackTimer();
            }

            if (me->haveOffhandWeapon() && me->isAttackReady(OFF_ATTACK)) {
                // xinef: delay main hand attack if both will hit at the same
                // time (players code)
                if (me->getAttackTimer(BASE_ATTACK) < ATTACK_DISPLAY_DELAY)
                    me->setAttackTimer(BASE_ATTACK, ATTACK_DISPLAY_DELAY);

                me->AttackerStateUpdate(
                    victim, OFF_ATTACK, false, ignoreCasting);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!UpdateVictim()) {
                if (instance->GetBossState(DATA_MANDOKIR) == NOT_STARTED ||
                    instance->GetBossState(DATA_MANDOKIR) == SPECIAL) {
                    while (uint32 eventId = events.ExecuteEvent()) {
                        switch (eventId) {
                        case EVENT_CHECK_START:
                            if (instance->GetBossState(DATA_MANDOKIR) ==
                                SPECIAL) {
                                me->GetMotionMaster()->MovePoint(
                                    0,
                                    PosMandokir[1].m_positionX,
                                    PosMandokir[1].m_positionY,
                                    PosMandokir[1].m_positionZ);
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_WATCH_PLAYER:
                    if (Unit* player = SelectTarget(
                            SelectTargetMethod::Random, 0, 100, true)) {
                ...
                case EVENT_CHARGE_PLAYER:
                    if (Unit* target = SelectTarget(
                        ...
                        if (Unit* mainTarget = SelectTarget(
                                SelectTargetMethod::MaxThreat, 0, 100.0f)) {
                            me->GetThreatMgr().ModifyThreatByPercent(mainTarget,
                                                                     -100);
                        }
                    }
                    events.ScheduleEvent(EVENT_CHARGE_PLAYER, 30s, 40s);
...
// Ohgan
enum OhganSpells { SPELL_SUNDERARMOR = 24317, SPELL_THRASH = 3391 };
...
public:
    ...
    void Reset() override { revivePlayerGUID.Clear(); }
    ...
    void DoAction(int32 action) override
    {
        if (action == ACTION_REVIVE) {
            if (Player* target =
                    ObjectAccessor::GetPlayer(*me, revivePlayerGUID)) {
                Position pos;
    ...
    void JustDied(Unit* /*killer*/) override { me->DespawnOrUnsummon(); }
...
public:
    ...
    class spell_threatening_gaze_AuraScript : public AuraScript {
                    ...
                    if (Unit* caster = GetCaster()) {
                        if (Creature* cCaster = caster->ToCreature()) {
                            if (cCaster->IsAIEnabled) {
                                cCaster->AI()->SetGUID(target->GetGUID(),
                                                       ACTION_CHARGE);
                            }
                        }
                    }
        ...
        {
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_threatening_gaze_AuraScript::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_DUMMY,
...
class spell_mandokir_charge : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_mandokir_charge::LaunchHit, EFFECT_0, SPELL_EFFECT_CHARGE);
    }
};
...
class spell_threatening_gaze_charge : public SpellScript {
    ...
    void Register() override
    {
        OnEffectLaunchTarget +=
            SpellEffectFn(spell_threatening_gaze_charge::PreventLaunchHit,
                          EFFECT_1,
                          SPELL_EFFECT_TRIGGER_SPELL);
        OnEffectHitTarget +=
            SpellEffectFn(spell_threatening_gaze_charge::LaunchHit,
                          EFFECT_1,
                          SPELL_EFFECT_TRIGGER_SPELL);
    }
};
