        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_CAUSE_INSANITY:
                    if (me->GetThreatMgr().GetThreatListSize() > 1) {
                        if (Unit* victim = SelectTarget(
                                SelectTargetMethod::MaxThreat, 0, 30.f, true)) {
                ...
                case EVENT_ASPECT_OF_MARLI:
                    if (Unit* victim = SelectTarget(
                            SelectTargetMethod::MaxThreat, 0, 5.f, true)) {
                        DoCast(victim, SPELL_ASPECT_OF_MARLI, true);
                        me->GetThreatMgr().ModifyThreatByPercent(victim,
                                                                 -100.f);
                    }
...
public:
    at_zulgurub_entrance_speech()
        : OnlyOnceAreaTriggerScript("at_zulgurub_entrance_speech")
    {
    }
    ...
    bool _OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            // Instance map's enormous, Hakkar's GRID is not loaded by the time
            // players enter. Without this, the creature never says anything,
            // because it doesn't load in time.
            player->GetMap()->LoadGrid(-11783.99f, -1655.27f);

            if (Creature* hakkar = ObjectAccessor::GetCreature(
                    *player, instance->GetGuidData(DATA_HAKKAR))) {
...
public:
    at_zulgurub_bridge_speech()
        : OnlyOnceAreaTriggerScript("at_zulgurub_bridge_speech")
    {
    }
    ...
    bool _OnTrigger(Player* player, const AreaTrigger* /*at*/) override
    {
        if (InstanceScript* instance = player->GetInstanceScript()) {
            // Instance map's enormous, Hakkar's GRID is not loaded by the time
            // players enter.
            // Without this, the creature never says anything, because it
...
public:
    at_zulgurub_temple_speech()
        : OnlyOnceAreaTriggerScript("at_zulgurub_temple_speech")
    {
    }
...
public:
    at_zulgurub_bloodfire_pit_speech()
        : OnlyOnceAreaTriggerScript("at_zulgurub_bloodfire_pit_speech")
    {
    }
...
public:
    at_zulgurub_edge_of_madness_speech()
        : OnlyOnceAreaTriggerScript("at_zulgurub_edge_of_madness_speech")
    {
    }
...
class spell_blood_siphon : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_BLOOD_SIPHON_DAMAGE, SPELL_BLOOD_SIPHON_HEAL});
    }
        ...
        if (Unit* caster = GetCaster()) {
            if (Player* player = GetHitPlayer()) {
                player->CastSpell(caster,
                                  player->HasAura(SPELL_POISONOUS_BLOOD)
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_blood_siphon::FilterTargets,
                                          EFFECT_0,
                                          TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_blood_siphon::HandleScriptEffect,
                          EFFECT_0,
