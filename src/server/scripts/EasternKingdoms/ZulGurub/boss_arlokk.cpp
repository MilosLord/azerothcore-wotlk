...
Position const PosMoveOnSpawn[1] = {{-11561.9f, -1627.868f, 41.29941f, 0.0f}};
    ...
    struct boss_arlokkAI : public BossAI {
        ...
        void Reset() override
        {
            if (events.IsInPhase(PHASE_TWO))
                me->HandleStatModifier(
                    UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, 35.0f, false); // hack
            _Reset();
            _summonCountA = 0;
            _summonCountB = 0;
            me->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 0,
                               uint32(WEAPON_DAGGER));
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            GetCreatureListWithEntryInGrid(
                triggerList, me, NPC_PANTHER_TRIGGER, 100.0f);
        ...
        {
            if (GameObject* object = ObjectAccessor::GetGameObject(
                    *me, instance->GetGuidData(GO_GONG_OF_BETHEKK)))
        ...
        void EnterEvadeMode(EvadeReason why) override
        {
            ...
            GetCreatureListWithEntryInGrid(
                panthers, me, NPC_ZULIAN_PROWLER, 200.f);
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_SHADOW_WORD_PAIN:
                    ...
                    events.ScheduleEvent(
                        EVENT_SHADOW_WORD_PAIN, 5s, 7s, 0, PHASE_ONE);
                    break;
                case EVENT_GOUGE:
                ...
                case EVENT_SUMMON_PROWLERS:
                    if (_summonCountA < MAX_PROWLERS_PER_SIDE) {
                        if (Unit* trigger = ObjectAccessor::GetUnit(
                                *me, _triggersSideAGUID[urand(0, 4)])) {
                    ...
                    if (_summonCountB < MAX_PROWLERS_PER_SIDE) {
                        if (Unit* trigger = ObjectAccessor::GetUnit(
                                *me, _triggersSideBGUID[urand(0, 4)])) {
                            trigger->CastSpell(trigger, SPELL_SUMMON_PROWLER);
                            ++_summonCountB;
                        }
                    }
                    events.ScheduleEvent(
                        EVENT_SUMMON_PROWLERS, 6s, 0, PHASE_ALL);
                    break;
                ...
                case EVENT_VANISH:
                    ...
                    me->GetMotionMaster()->MovePoint(
                        0,
                        frand(-11551.0f, -11508.0f),
                        frand(-1638.0f, -1617.0f),
                        me->GetPositionZ());
                ...
                case EVENT_VISIBLE:
                    ...
                    events.ScheduleEvent(
                        EVENT_TRANSFORM_BACK, 30s, 40s, 0, PHASE_TWO);
                    events.SetPhase(PHASE_TWO);
                    me->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND,
                                           TOTAL_PCT,
                                           35.0f,
                                           true); // hack
                    break;
                ...
                case EVENT_TRANSFORM_BACK: {
                    me->RemoveAura(
                        SPELL_PANTHER_TRANSFORM); // SPELL_AURA_TRANSFORM
                    ...
                    me->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND,
                                           ...
                                           false); // hack
                    events.ScheduleEvent(
                        EVENT_SHADOW_WORD_PAIN, 4s, 7s, 0, PHASE_ONE);
                    events.ScheduleEvent(EVENT_GOUGE, 12s, 15s, 0, PHASE_ONE);
                    events.ScheduleEvent(EVENT_TRANSFORM, 30s, 0, PHASE_ONE);
...
public:
    ...
    struct npc_zulian_prowlerAI : public ScriptedAI {
        npc_zulian_prowlerAI(Creature* creature)
        ...
        void Reset() override
        {
            ...
            if (Unit* arlokk = ObjectAccessor::GetUnit(
                    *me, _instance->GetGuidData(NPC_ARLOKK)))
        ...
        {
            if (spell->Id == SPELL_MARK_OF_ARLOKK_TRIGGER) // Should only hit if
                                                           // line of sight
            {
                AttackStart(caster);
            }
            ...
            while (uint32 eventId = _events.ExecuteEvent()) {
                ...
                case EVENT_ATTACK:
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0.0f, 100, false)) {
                        AttackStart(target);
                    }
                    break;
                default:
...
public:
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        if (go->GetInstanceScript() &&
            !go->FindNearestCreature(NPC_ARLOKK, 25.0f)) {
            go->SetGameObjectFlag(GO_FLAG_NOT_SELECTABLE);
            go->SendCustomAnim(0);
            go->SummonCreature(NPC_ARLOKK,
                               PosSummonArlokk[0],
