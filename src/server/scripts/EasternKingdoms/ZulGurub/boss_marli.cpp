    ...
    void Reset() override
    {
        if (_phase == PHASE_SPIDER) {
            ...
            me->HandleStatModifier(
                UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, 35.0f, false);
            _phase = PHASE_TROLL;
        }

        std::list<GameObject*> eggs;
        me->GetGameObjectListWithEntryInGrid(
            eggs, GO_SPIDER_EGGS, DEFAULT_VISIBILITY_INSTANCE);
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler.Schedule(1s, [this](TaskContext) {
            ...
            scheduler.Schedule(500ms,
                               [this](TaskContext) { Talk(SAY_SPIDER_SPAWN); });
...
private:
    ...
    void _schedulePhaseTroll()
    {
        ...
        scheduler
            ...
            .Schedule(10s, 20s, PHASE_TROLL, [this](TaskContext context) {
                std::list<Creature*> targets =
                    DoFindFriendlyMissingBuff(100.f, SPELL_ENLARGE);
        ...
        scheduler.Schedule(
            1min, PHASE_TROLL, [this](TaskContext) { _schedulePhaseSpider(); });
    }
    ...
    void _schedulePhaseSpider()
    {
        ...
        me->HandleStatModifier(
            UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, 35.0f, true);
        ...
        scheduler
            .Schedule(5s,
                      ...
                      [this](TaskContext context) {
                          ...
                          scheduler.Schedule(
                              500ms, PHASE_SPIDER, [this](TaskContext) {
                                  _chargePlayer();
                              });
        ...
        scheduler.Schedule(
            1min, PHASE_SPIDER, [this](TaskContext) { _schedulePhaseTroll(); });
    }

    void _chargePlayer()
    {
        Unit* target = SelectTarget(
            SelectTargetMethod::Random, 0, [this](Unit* target) -> bool {
                if (target->GetTypeId() != TYPEID_PLAYER ||
                    target->getPowerType() != Powers::POWER_MANA)
                    return false;
...
class spell_hatch_eggs : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_hatch_eggs::HandleObjectAreaTargetSelect,
            EFFECT_0,
...
class spell_marli_transform : public AuraScript {
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_marli_transform::HandleApply,
                                           EFFECT_0,
                                           SPELL_AURA_TRANSFORM,
                                           AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_marli_transform::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_TRANSFORM,
                               AURA_EFFECT_HANDLE_REAL);
    }
