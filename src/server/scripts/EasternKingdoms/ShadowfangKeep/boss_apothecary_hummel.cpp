...
public:
    ...
    struct boss_apothecary_hummelAI : public BossAI {
        boss_apothecary_hummelAI(Creature* creature)
            : BossAI(creature, DATA_APOTHECARY_HUMMEL), _deadCount(0),
              _isDead(false)
        {
        ...
        {
            if (menuId == GOSSIP_MENU_HUMMEL &&
                gossipListId == GOSSIP_OPTION_START) {
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_EVENT && _phase == PHASE_ALL) {
                ...
                scheduler
                    .Schedule(
                        1ms,
                        [this](TaskContext /*context*/) { Talk(SAY_INTRO_0); })
                    .Schedule(
                        4s,
                        [this](TaskContext /*context*/) { Talk(SAY_INTRO_1); })
                    .Schedule(
                        8s,
                        [this](TaskContext /*context*/) { Talk(SAY_INTRO_2); })
                    .Schedule(12s, [this](TaskContext context) {
                        ...
                        context
                            .Schedule(
                                6s,
                                [this](TaskContext /*context*/) // Call Baxter
                                {
                                    Talk(SAY_CALL_BAXTER);
                                    EntryCheckPredicate pred(
                            ...
                            .Schedule(
                                14s,
                                [this](TaskContext /*context*/) // Call Frye
                                {
                                    Talk(SAY_CALL_FRYE);
                                    EntryCheckPredicate pred(
                                        NPC_APOTHECARY_FRYE);
                            ...
                            .Schedule(
                                3640ms,
                                [this](TaskContext context) // Perfume spray
                                {
                            ...
                            .Schedule(
                                15s,
                                [this](TaskContext context) // Chain Reaction
                                {
                            ...
                            .Schedule(
                                15s,
                                [this](TaskContext /*context*/) // Call Crazed
                                                                // Apothecary
                                                                // (text)
        ...
        {
            if (summon->GetEntry() == NPC_APOTHECARY_FRYE ||
                summon->GetEntry() == NPC_APOTHECARY_BAXTER) {
                _deadCount++;
            }
...
struct npc_apothecary_genericAI : public ScriptedAI {
    npc_apothecary_genericAI(Creature* creature, Position pos)
        : ScriptedAI(creature), _movePos(pos)
    {
    }
...
struct npc_apothecary_frye : public npc_apothecary_genericAI {
    npc_apothecary_frye(Creature* creature)
        : npc_apothecary_genericAI(creature, FryeMovePos)
    {
    }

    void JustDied(Unit* /*killer*/) override { Talk(SAY_FRYE_DEATH); }
};

struct npc_apothecary_baxter : public npc_apothecary_genericAI {
    npc_apothecary_baxter(Creature* creature)
        : npc_apothecary_genericAI(creature, BaxterMovePos)
    {
    ...
    void JustDied(Unit* /*killer*/) override { Talk(SAY_BAXTER_DEATH); }
...
class spell_apothecary_lingering_fumes : public SpellScript {
    ...
    void HandleAfterCast()
    {
        ...
        caster->GetCreatureListWithEntryInGrid(
            triggers, NPC_VIAL_BUNNY, 100.0f);
        if (triggers.empty())
            return;

        Creature* trigger =
            Acore::Containers::SelectRandomContainerElement(triggers);
    ...
    {
        AfterCast +=
            SpellCastFn(spell_apothecary_lingering_fumes::HandleAfterCast);
        OnEffectHitTarget +=
            SpellEffectFn(spell_apothecary_lingering_fumes::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_apothecary_validate_area : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_BUNNY_LOCKDOWN, SPELL_THROW_COLOGNE, SPELL_THROW_PERFUME});
    }
    ...
    void HandleScript(SpellEffIndex /*effindex*/)
    {
        ...
        GetCaster()->CastSpell(
            GetHitUnit(), RAND(SPELL_THROW_COLOGNE, SPELL_THROW_PERFUME), true);
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_apothecary_validate_area::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_DEST_AREA_ENTRY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_apothecary_validate_area::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_apothecary_throw_cologne : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_apothecary_throw_cologne::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_apothecary_throw_perfume : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_apothecary_throw_perfume::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_apothecary_perfume_spill : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_apothecary_perfume_spill::OnPeriodic,
                                 EFFECT_0,
...
class spell_apothecary_cologne_spill : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_apothecary_cologne_spill::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_DUMMY);
    }
};
