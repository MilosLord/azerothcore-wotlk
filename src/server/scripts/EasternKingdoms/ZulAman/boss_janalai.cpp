...
float JanalainPos[1][3] = {{-33.93f, 1149.27f, 19}};

float FireWallCoords[4][4] = {{-10.13f, 1149.27f, 19, 3.1415f},
                              {-33.93f, 1123.90f, 19, 0.5f * 3.1415f},
                              {-54.80f, 1150.08f, 19, 0},
                              {-33.93f, 1175.68f, 19, 1.5f * 3.1415f}};

float hatcherway[2][5][3] = {{{-87.46f, 1170.09f, 6},
                              {-74.41f, 1154.75f, 6},
                              {-52.74f, 1153.32f, 19},
                              {-33.37f, 1172.46f, 19},
                              {-33.09f, 1203.87f, 19}},
    ...
    struct boss_janalaiAI : public ScriptedAI {
        ...
        void FireWall()
        {
            for (uint8 i = 0; i < 4; ++i) {
                ...
                for (uint8 j = 0; j < WallNum; j++) {
                    Creature* wall =
                        WallNum == 3 ? me->SummonCreature(
                                           NPC_FIRE_BOMB,
                                           FireWallCoords[i][0],
                                           FireWallCoords[i][1] + 5 * (j - 1),
                                           FireWallCoords[i][2],
                                           FireWallCoords[i][3],
        ...
        void SpawnBombs()
        {
            ...
            for (int i(0); i < 40; ++i) {
                ...
                Creature* bomb = DoSpawnCreature(NPC_FIRE_BOMB,
                                                 dx,
                                                 dy,
                                                 0,
                                                 0,
        ...
        {
            if (BombCount < 40) {
                if (Unit* FireBomb = ObjectAccessor::GetUnit(
                        *me, FireBombGUIDs[BombCount])) {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (BombTimer <= diff) {
                ...
                me->NearTeleportTo(JanalainPos[0][0],
                                   JanalainPos[0][1],
                                   JanalainPos[0][2],
                                   me->GetOrientation());
                me->StopMovingOnCurrentPos();
                DoCast(me, SPELL_FIRE_BOMB_CHANNEL, false);
                // DoTeleportPlayer(me, JanalainPos[0][0], JanalainPos[0][1],
                // JanalainPos[0][2], 0); DoCast(me, SPELL_TELE_TO_CENTER,
                // true);
                ...
                for (Map::PlayerList::const_iterator i = PlayerList.begin();
                    ...
                    if (Player* i_pl = i->GetSource())
                        if (i_pl->IsAlive())
                            DoTeleportPlayer(
                                i_pl,
                                JanalainPos[0][0] - 5 + rand() % 10,
                                JanalainPos[0][1] - 5 + rand() % 10,
                                JanalainPos[0][2],
                                0);
                // DoCast(Temp, SPELL_SUMMON_PLAYERS, true) // core bug, spell
                // does not work if too far
                return;
            }
            else
                BombTimer -= diff;
            ...
            if (!noeggs) {
                if (HealthBelowPct(35)) {
                    ...
                    if (HatchAllEggs(0)) {
                        ...
                        me->SummonCreature(NPC_AMANI_HATCHER,
                                           hatcherway[0][0][0],
                                           hatcherway[0][0][1],
                                           hatcherway[0][0][2],
                                           0,
...
public:
    ...
    struct npc_janalai_hatcherAI : public ScriptedAI {
        ...
        {
            if (!instance ||
                instance->GetData(DATA_JANALAIEVENT) != IN_PROGRESS) {
                me->DisappearAndDie();
                return;
            }
            ...
            if (!isHatching) {
                if (WaitTimer) {
                    ...
                    me->GetMotionMaster()->MovePoint(
                        0,
                        hatcherway[side][waypoint][0],
                        hatcherway[side][waypoint][1],
                        hatcherway[side][waypoint][2]);
                    ++waypoint;
                    WaitTimer = 0;
                }
            }
