...
enum UniqueEvents { EVENT_BERSERK = 0 };

enum Hal_CreatureIds { NPC_TOTEM = 24224 };
...
enum Groups { GROUP_LYNX = 0, GROUP_HUMAN = 1, GROUP_MERGE = 2 };
    ...
    void EnterPhase(PhaseHalazzi nextPhase)
    {
        ...
        case PHASE_MERGE:
            if (Creature* lynx = instance->GetCreature(DATA_SPIRIT_LYNX)) {
                ...
                ++_transformCount;
                scheduler.Schedule(
                    2s, GROUP_MERGE, [this](TaskContext context) {
                        if (Creature* lynx =
                                instance->GetCreature(DATA_SPIRIT_LYNX)) {
...
struct npc_halazzi_lynx : public ScriptedAI {
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        ScheduleTimedEvent(
            30s, 50s, [&] { DoCastSelf(SPELL_LYNX_FRENZY); }, 30s, 50s);
        ScheduleTimedEvent(
            4s, [&] { DoCastVictim(SPELL_SHRED_ARMOR); }, 4s);
    }
