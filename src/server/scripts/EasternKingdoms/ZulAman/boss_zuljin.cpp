...
public:
    ...
    struct boss_zuljinAI : public ScriptedAI {
        ...
        {
            for (uint8 i = 0; i < 4; ++i) {
                Creature* creature = me->SummonCreature(SpiritInfo[i].entry,
                                                        SpiritInfo[i].x,
                                                        SpiritInfo[i].y,
                                                        SpiritInfo[i].z,
                                                        SpiritInfo[i].orient,
        ...
        void DespawnAdds()
        {
            for (uint8 i = 0; i < 4; ++i) {
                if (SpiritGUID[i]) {
                    if (Unit* temp =
                            ObjectAccessor::GetUnit(*me, SpiritGUID[i])) {
        ...
        void EnterPhase(uint32 NextPhase)
        {
            ...
            case 4:
                me->NearTeleportTo(
                    CENTER_X, CENTER_Y, CENTER_Z, me->GetOrientation());
                DoResetThreatList();
                ...
                if (Phase > 0) {
                    if (Unit* Temp =
                            ObjectAccessor::GetUnit(*me, SpiritGUID[Phase - 1]))
                        Temp->SetUInt32Value(UNIT_FIELD_BYTES_1,
                                             UNIT_STAND_STATE_DEAD);
                }
                if (Unit* Temp =
                        ObjectAccessor::GetUnit(*me, SpiritGUID[NextPhase - 1]))
                    Temp->CastSpell(
                        me, SPELL_SIPHON_SOUL, false); // should m cast on temp
                if (NextPhase == 2) {
                    me->GetMotionMaster()->Clear();
                    DoCast(me, SPELL_ENERGY_STORM, true); // enemy aura
                    for (uint8 i = 0; i < 4; ++i) {
                        Creature* Vortex =
                            DoSpawnCreature(CREATURE_FEATHER_VORTEX,
                                            0,
                                            0,
                                            0,
                                            0,
                        ...
                        if (Vortex) {
                            Vortex->CastSpell(
                                Vortex, SPELL_CYCLONE_PASSIVE, true);
                            Vortex->CastSpell(
                                Vortex, SPELL_CYCLONE_VISUAL, true);
                            Vortex->SetUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
                            Vortex->SetSpeed(MOVE_RUN, 1.0f);
                            Vortex->AI()->AttackStart(
                        ...
                        if (Claw_Loop_Timer <= diff) {
                            ...
                            if (target) {
                                ...
                                if (me->IsWithinMeleeRange(target)) {
                                    DoCast(
                                        target, SPELL_CLAW_RAGE_DAMAGE, true);
                                    ++Claw_Counter;
                                    if (Claw_Counter == 12) {
                                        Claw_Rage_Timer = urand(15000, 20000);
                                        me->SetSpeed(MOVE_RUN, 1.2f);
                                        AttackStart(ObjectAccessor::GetUnit(
                                            *me, TankGUID));
                                        TankGUID.Clear();
                                        return;
                                    }
                        ...
                        if (target) {
                            if (me->IsWithinMeleeRange(target)) {
                                ...
                                if (Claw_Counter == 9) {
                                    ...
                                    me->SetSpeed(MOVE_RUN, 1.2f);
                                    AttackStart(
                                        ObjectAccessor::GetUnit(*me, TankGUID));
                                    TankGUID.Clear();
                                }
                                else
                                    AttackStart(SelectTarget(
...
public:
    ...
    struct npc_zuljin_vortexAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 /*diff*/) override
        {
            // if the vortex reach the target, it change his target to another
            // player
