    ...
    struct boss_nalorakkAI : public ScriptedAI {
        ...
        void Reset() override
        {
            if (MoveEvent) {
                ...
                (*me).GetMotionMaster()->MovePoint(
                    0, NalorakkWay[7][0], NalorakkWay[7][1], NalorakkWay[7][2]);
            }
            ...
            // @todo find the correct equipment id
        }
        ...
        void ResetMobs()
        {
...

            {
                ...
                TypeContainerVisitor<Acore::CreatureListSearcher<
                                         Acore::AllFriendlyCreaturesInGrid>,
            ...
            for (std::list<Creature*>::const_iterator i = templist.begin();
                 i != templist.end();
                 ++i)
                if ((*i) && me->GetGUID() != (*i)->GetGUID() &&
                    me->IsWithinDistInMap((*i), 25))
                    (*i)->AI()->Reset();
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!inBearForm) {
                ...
                if (Surge_Timer <= diff) {
                    Talk(SAY_SURGE);
                    Unit* target =
                        SelectTarget(SelectTargetMethod::Random, 1, 45, true);
                    if (target)
...
void AddSC_boss_nalorakk() { new boss_nalorakk(); }
