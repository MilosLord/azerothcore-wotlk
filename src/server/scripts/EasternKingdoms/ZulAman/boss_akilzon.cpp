    ...
    struct boss_akilzonAI : public BossAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            events.ScheduleEvent(
                ...
                urand(10000, 20000)); // 10 to 20 seconds (bosskillers)
            events.ScheduleEvent(
                EVENT_GUST_OF_WIND,
                urand(20000, 30000)); // 20 to 30 seconds(bosskillers)
            events.ScheduleEvent(
                EVENT_CALL_LIGHTNING,
                urand(
                    10000,
                    20000)); // totaly random timer. can't find any info on this
            events.ScheduleEvent(EVENT_ELECTRICAL_STORM,
                                 60000); // 60 seconds(bosskillers)
            events.ScheduleEvent(EVENT_RAIN, urand(47000, 52000));
            events.ScheduleEvent(
                EVENT_ENRAGE,
                10 * MINUTE *
                    IN_MILLISECONDS); // 10 minutes till enrage(bosskillers)
        ...
        void HandleStormSequence(Unit* Cloud) // 1: begin, 2-9: tick, 10: end
        {
            if (StormCount < 10 && StormCount > 1) {
                ...
                Acore::UnitListSearcher<
                    ...
                    searcher(me, tempUnitMap, u_check);
                Cell::VisitAllObjects(me, searcher, SIZE_OF_GRIDS);

                // deal damage
                for (std::list<Unit*>::const_iterator i = tempUnitMap.begin();
                     i != tempUnitMap.end();
                     ++i) {
                    if (Unit* target = (*i)) {
                        if (Cloud && !Cloud->IsWithinDist(target, 6, false))
                            Cloud->CastCustomSpell(target,
                                                   SPELL_ZAP,
                                                   &bp0,
                                                   nullptr,
                                                   nullptr,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_STATIC_DISRUPTION: {
                    ...
                    /*if (float dist =
                    me->IsWithinDist3d(target->GetPositionX(),
                    target->GetPositionY(), target->GetPositionZ(), 5.0f) dist
                    = 5.0f; SDisruptAOEVisual_Timer = 1000 + floor(dist / 30 *
                    1000.0f);*/
                    events.ScheduleEvent(EVENT_STATIC_DISRUPTION,
                                         urand(10000, 18000));
                    break;
                }
                case EVENT_GUST_OF_WIND: {
                    ...
                    events.ScheduleEvent(EVENT_GUST_OF_WIND,
                                         urand(20000, 30000));
                    break;
                }
                case EVENT_CALL_LIGHTNING:
                    ...
                    events.ScheduleEvent(
                        EVENT_CALL_LIGHTNING,
                        urand(12000, 17000)); // totaly random timer. can't find
                                              // any info on this
                    break;
                case EVENT_ELECTRICAL_STORM: {
                    Unit* target =
                        SelectTarget(SelectTargetMethod::Random, 0, 50, true);
                    if (!target) {
                        EnterEvadeMode();
                        return;
                    }
                    ...
                    DoCast(target,
                           ...
                           false); // storm cyclon + visual
                    float x, y, z;
                    target->GetPosition(x, y, z);
                    /// @todo: fix it in correct way, that causes player to can
                    /// fly until logout
                    /*
                    if (target)
                    {
                        target->SetDisableGravity(true);
                        target->MonsterMoveWithSpeed(x, y,
                    me->GetPositionZ()+15, 0);
                    }
                    */

                    Unit* Cloud = me->SummonTrigger(
                        x, y, me->GetPositionZ() + 16, 0, 15000);
                    if (Cloud) {
                ...
                case EVENT_STORM_SEQUENCE: {
                    ...
                    else if (Unit* Cyclone =
                                 ObjectAccessor::GetUnit(*me, CycloneGUID))
                        Cyclone->CastSpell(target,
                                           SPELL_SAND_STORM,
                                           true); // keep casting or...
                ...
                case EVENT_SUMMON_EAGLES:
                    ...
                    for (uint8 i = 0; i < 8; ++i) {
                        ...
                        if (!bird) // they despawned on die
                        {
                            ...
                            Creature* creature =
                                me->SummonCreature(NPC_SOARING_EAGLE,
                                                   x,
                                                   y,
                                                   z,
                                                   0,
