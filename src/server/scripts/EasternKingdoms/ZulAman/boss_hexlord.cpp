...
public:
    ...
    struct boss_hex_lord_malacrassAI : public ScriptedAI {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            for (uint8 i = 0; i < 4; ++i) {
                Creature* creature =
                    ObjectAccessor::GetCreature(*me, AddGUID[i]);
        ...
        void SelectAddEntry()
        {
            ...
            uint8 i = 0;
            for (std::vector<uint32>::const_iterator itr = AddList.begin();
                 itr != AddList.end();
                 ++itr, ++i)
                AddEntry[i] = *itr;
        }
        ...
        void SpawnAdds()
        {
            for (uint8 i = 0; i < 4; ++i) {
                ...
                if (!creature || !creature->IsAlive()) {
                    ...
                    creature = me->SummonCreature(AddEntry[i],
                                                  Pos_X[i],
                                                  POS_Y,
                                                  POS_Z,
                                                  ORIENT,
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (DrainPower_Timer <= diff) {
                ...
                DrainPower_Timer =
                    urand(40000, 55000); // must cast in 60 sec, or buff/debuff
                                         // will disappear
            }
            else
            ...
            if (SiphonSoul_Timer <= diff) {
                Unit* target =
                    SelectTarget(SelectTargetMethod::Random, 0, 70, true);
                Unit* trigger = DoSpawnCreature(NPC_TEMP_TRIGGER,
                                                0,
                                                0,
                                                0,
                                                0,
                ...
                if (!target || !trigger) {
                    ...
                    // me->SetGuidValue(UNIT_FIELD_CHANNEL_OBJECT,
                    // target->GetGUID());
                    // me->SetUInt32Value(UNIT_CHANNEL_SPELL,
                    // SPELL_SIPHON_SOUL);
        ...
        void UseAbility()
        {
            ...
            case ABILITY_TARGET_BUFF: {
                std::list<Creature*> templist = DoFindFriendlyMissingBuff(
                    50, PlayerAbility[PlayerClass][random].spell);
                if (!templist.empty())
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (bloodlust_timer <= diff) {
                std::list<Creature*> templist =
                    DoFindFriendlyMissingBuff(50, SPELL_BLOODLUST);
                if (!templist.empty()) {
    ...
    struct boss_alyson_antilleAI : public boss_hexlord_addAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (urand(0, 1))
            {
                ...
                me->CastSpell(SelectUnit(SelectTargetMethod::Random, 0),
            SPELL_DISPEL_MAGIC, false);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (volatileinf_timer <= diff) {
                ...
                me->GetVictim()->CastSpell(
                    me->GetVictim(), SPELL_VOLATILE_INFECTION, false);
...
public:
    spell_hexlord_unstable_affliction()
        : SpellScriptLoader("spell_hexlord_unstable_affliction")
    {
    }
