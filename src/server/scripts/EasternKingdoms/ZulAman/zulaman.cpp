...
SDName: Zulaman
SD%Complete: 90
SDComment: Forest Frog will turn into different NPC's. Workaround to prevent new
entry from running this script SDCategory: Zul'Aman EndScriptData */
...
public:
    ...
    struct npc_forest_frogAI : public ScriptedAI {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (eventTimer) {
                Player* player =
                    ObjectAccessor::GetPlayer(me->GetMap(), PlayerGUID);
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (me->GetEntry() != NPC_GUNTER &&
                        me->GetEntry() != NPC_KYREN) // vendors don't kneel?
                ...
                case 3:
                    ...
                    eventTimer = 4;
                    if (me->GetEntry() == NPC_GUNTER ||
                        me->GetEntry() == NPC_KYREN)
                        events.ScheduleEvent(
                            eventTimer,
                            5 * MINUTE *
                                IN_MILLISECONDS); // vendors wait for 5 minutes
                                                  // before running away and
                                                  // despawning
                    else
                ...
                case 5:
                    ...
                    if (me->GetPositionY() > 1290.0f)
                        me->GetMotionMaster()->MovePoint(
                            POINT_DESPAWN, 118.2742f, 1400.657f, -9.118711f);
                    else
                        me->GetMotionMaster()->MovePoint(
                            POINT_DESPAWN, 114.3155f, 1244.244f, -20.97606f);
                    eventTimer = 0;
                    break;
                }
            }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            if (spell->Id == SPELL_REMOVE_AMANI_CURSE &&
                ...
                if (roll_chance_i(2)) {
                    ...
                    me->GetMotionMaster()->MovePoint(POINT_DESPAWN,
                                                     caster->GetPositionX(),
                                                     caster->GetPositionY(),
                                                     caster->GetPositionZ());
                }
            ...
            for (uint8 i = 0; i < 4; ++i) {
                if (HostageEntry[i] == entry) {
                    creature->SummonGameObject(
                        ChestEntry[i], x - 2, y, z, 0, 0, 0, 0, 0, 0);
                    break;
                }
            }
        }
            ...
            if (_gongEvent) {
                if (_gongTimer <= diff) {
                    ...
                    case GONG_EVENT_2:
                        ...
                        break;
                    case GONG_EVENT_3:
                        if (GameObject* gong = me->GetMap()->GetGameObject(
                                instance->GetGuidData(GO_STRANGE_GONG)))
                    ...
                    case GONG_EVENT_4:
                        ...
                        // trigger or gong will need to be scripted to set
                        // IN_PROGRESS after enough hits. This is temp
                        // workaround.
                        instance->SetData(DATA_GONGEVENT,
                                          IN_PROGRESS); // to be removed.

                        if (instance->GetData(DATA_GONGEVENT) == IN_PROGRESS) {
                            // Players are Now Saved to instance at SPECIAL
                            // (Player should be notified?)
                            me->GetMotionMaster()->MovePath(HARRISON_MOVE_2,
                                                            false);
                    ...
                    case GONG_EVENT_7:
                        if (!uiTargetGUID) {
                            ...
                            GetCreatureListWithEntryInGrid(
                                targetList, me, NPC_AMANISHI_GUARDIAN, 26.0f);
                            if (!targetList.empty()) {
                                    ...
                                    if (Creature* ptarget = *itr) {
                                        if (ptarget->GetPositionX() > 120) {
                                            ptarget->SetUInt32Value(
                                                UNIT_VIRTUAL_ITEM_SLOT_ID + 0,
                                                uint32(WEAPON_SPEAR));
                                            ptarget->SetImmuneToPC(true);
                                            ptarget->SetReactState(
                                                REACT_PASSIVE);
                        ...
                        if (GameObject* gate = me->GetMap()->GetGameObject(
                                instance->GetGuidData(GO_MASSIVE_GATE)))
