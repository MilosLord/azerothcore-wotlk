...
ObjectData const creatureData[] = {{NPC_KALECGOS, DATA_KALECGOS}, {0, 0}};

ObjectData const gameobjectData[] = {{GO_ESCAPE_ORB, DATA_ESCAPE_ORB},
                                     {
                                         0,
                                         0,
                                     }};
...
public:
    instance_magisters_terrace()
        : InstanceMapScript("instance_magisters_terrace", 585)
    {
    }

    struct instance_magisters_terrace_InstanceMapScript
        : public InstanceScript {
        instance_magisters_terrace_InstanceMapScript(Map* map)
            : InstanceScript(map)
        {
        ...
        void ProcessEvent(WorldObject* /*obj*/, uint32 eventId) override
        {
            if (eventId == EVENT_SPAWN_KALECGOS) {
                if (!GetCreature(DATA_KALECGOS) &&
                    !scheduler.IsGroupScheduled(DATA_KALECGOS)) {
                    scheduler.Schedule(
                        1min, 1min, DATA_KALECGOS, [this](TaskContext) {
                            if (Creature* kalecgos = instance->SummonCreature(
                                    NPC_KALECGOS, KalecgosSpawnPos)) {
                                kalecgos->GetMotionMaster()->MovePath(
                                    PATH_KALECGOS_FLIGHT, false);
