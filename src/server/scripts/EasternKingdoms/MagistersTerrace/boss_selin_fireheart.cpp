...
struct boss_selin_fireheart : public ScriptedAI {
    ...
    void SpawnCrystals()
    {
        me->SummonCreature(NPC_FEL_CRYSTAL,
                           248.053f,
                           14.592f,
                           3.74882f,
                           3.94444f,
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        instance->SetBossState(DATA_SELIN_FIREHEART,
                               DONE); // Encounter complete!
        summons.DespawnAll();
    }
    ...
    void SelectNearestCrystal()
    {
        ...
        if (crystal) {
            Talk(SAY_ENERGY);
            float x, y, z;
            crystal->GetClosePoint(
                x, y, z, me->GetObjectSize(), CONTACT_DISTANCE);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_SPELL_DRAIN_LIFE:
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0))
                me->CastSpell(
                    target,
                    DUNGEON_MODE(SPELL_DRAIN_LIFE_N, SPELL_DRAIN_LIFE_H),
                    false);
