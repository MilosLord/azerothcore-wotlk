...
public:
    ...
    struct instance_stratholme_InstanceMapScript : public InstanceScript {
        ...
        void ProcessSlaughterEvent()
        {
            if (_slaughterProgress == 1) {
                if (Creature* baron =
                        instance->GetCreature(_baronRivendareGUID))
            ...
            if (_slaughterProgress == 2) {
                ...
                if (Creature* baron =
                        instance->GetCreature(_baronRivendareGUID))
                    if (GameObject* gate = baron->FindNearestGameObject(
                            GO_SLAUGHTER_GATE_SIDE, 200.0f))
            ...
            if (_slaughterProgress == 4) {
                ...
                if (GameObject* gate =
                        instance->GetGameObject(_zigguratDoorsGUID5))
                    gate->SetGoState(GO_STATE_ACTIVE);
            }
        }
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_GAUNTLET_GATE:
                ...
                if (_zigguratState1 == 2 && _zigguratState2 == 2 &&
                    _zigguratState3 == 2)
                    go->SetGoState(GO_STATE_ACTIVE);
                break;
        ...
        void CheckZiggurats()
        {
            if (_zigguratState1 == 2 && _zigguratState2 == 2 &&
                ...
                if (GameObject* gate =
                        instance->GetGameObject(_gauntletGateGUID))
                    gate->SetGoState(GO_STATE_ACTIVE);
                if (GameObject* gate =
                        instance->GetGameObject(_slaughterGateGUID))
                    gate->SetGoState(GO_STATE_ACTIVE);
            }
        }
        ...
        void DoSpawnPlaguedCritters(uint8 /*uiGate*/, Player* player)
        {
            ...
            for (uint8 i = 0; i < 30; ++i) {
                float          x, y, z;
                const Position pPos = {player->GetPositionX(),
                                       player->GetPositionY(),
                                       player->GetPositionZ(),
                ...
                player
                    ->SummonCreature(
                        uiEntry, x, y, z, 0, TEMPSUMMON_DEAD_DESPAWN, 0)
                    ->AI()
                    ->AttackStart(player);
            }
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case TYPE_ZIGGURAT1: {
                ...
                if (_zigguratState1 == 1)
                    if (GameObject* ziggurat =
                            instance->GetGameObject(_zigguratDoorsGUID1))
            ...
            case TYPE_ZIGGURAT2: {
                ...
                if (_zigguratState2 == 1)
                    if (GameObject* ziggurat =
                            instance->GetGameObject(_zigguratDoorsGUID2))
                        ziggurat->SetGoState(GO_STATE_ACTIVE);
            ...
            case TYPE_ZIGGURAT3: {
                ...
                if (_zigguratState3 == 1)
                    if (GameObject* ziggurat =
                            instance->GetGameObject(_zigguratDoorsGUID3))
                        ziggurat->SetGoState(GO_STATE_ACTIVE);
            ...
            case TYPE_BARON_FIGHT: {
                if (GameObject* gate =
                        instance->GetGameObject(_zigguratDoorsGUID5))
                    gate->SetGoState(data == IN_PROGRESS ? GO_STATE_READY
                                                         : GO_STATE_ACTIVE);
                return;
            }
            case TYPE_MALLOW:
        ...
        {
            data << _baronRunProgress << ' ' << _baronRunTime << ' '
                 << _zigguratState1 << ' ' << _zigguratState2 << ' '
                 << _zigguratState3 << ' ' << _slaughterProgress << ' '
                 << _postboxesOpened;
        }
        ...
        void Update(uint32 diff) override
        {
            ...
            // Loop over the two Gate traps, each one has up to three timers
            // (trap reset, gate opening delay, critters spawning delay)
            for (uint8 i = 0; i < 2; i++) {
                // if the gate is in cooldown, skip the other checks
                if (_gateTrapsCooldown[i])
                    break;

                // Check that the trap is not on cooldown, if so check if
                // player/pet is in range
                for (Map::PlayerList::const_iterator itr = players.begin();
                     itr != players.end();
                     ++itr) {
                    if (Player* player = itr->GetSource()) {
                        // should pet also trigger the trap? could not find any
                        // source for it
                        if (!player->IsGameMaster() &&
                            player->IsWithinDist2d(aGateTrap[i].m_positionX,
                                                   aGateTrap[i].m_positionY,
                                                   5.5f)) {
                            // Check if timer was not already set by another
                            // player/pet a few milliseconds before
                            if (_gateTrapsCooldown[i])
                                return;
                            ...
                            if (i == 0) {
                                ...
                                events.ScheduleEvent(EVENT_GATE1_TRAP,
                                                     30 * MINUTE *
                                                         IN_MILLISECONDS);
                                // set timer to reopen gates
                                events.ScheduleEvent(EVENT_GATE1_DELAY,
                                                     20 * IN_MILLISECONDS);
                                // set timer to spawn the plagued critters
                                events.ScheduleEvent(EVENT_GATE1_CRITTER_DELAY,
                                                     2 * IN_MILLISECONDS);
                            }
                            else if (i == 1) {
                                // set timer to reset the trap
                                events.ScheduleEvent(EVENT_GATE2_TRAP,
                                                     30 * MINUTE *
                                                         IN_MILLISECONDS);
                                // set timer to reopen gates
                                events.ScheduleEvent(EVENT_GATE2_DELAY,
                                                     20 * IN_MILLISECONDS);
                                // set timer to spawn the plagued critters
                                events.ScheduleEvent(EVENT_GATE2_CRITTER_DELAY,
                                                     2 * IN_MILLISECONDS);
                            }
                        }
                    }
            ...
            case EVENT_BARON_TIME: {
                ...
                if (baron && !baron->IsInCombat()) {
                    ...
                    case 5:
                        ...
                        if (Creature* ysida =
                                baron->FindNearestCreature(NPC_YSIDA, 50.0f))
                ...
                if (!PlayerList.IsEmpty())
                    for (Map::PlayerList::const_iterator i = PlayerList.begin();
                        ...
                        if (Player* player = i->GetSource())
                            if (player->GetDistance2d(4034.97f, -3402.13f) <
                                50.0f) {
                                ProcessSlaughterEvent();
                                return;
                            }
            ...
            case EVENT_SPAWN_BLACK_GUARD: {
                for (uint8 i = 0; i < 5; ++i)
                    if (Creature* guard = instance->SummonCreature(
                            NPC_BLACK_GUARD, BlackGuardPos[i])) {
                        guard->SetWalk(true);
                ...
                if (GameObject* gate =
                        instance->GetGameObject(_zigguratDoorsGUID4))
                    gate->SetGoState(GO_STATE_ACTIVE);
                break;
            }
            }
    ...
    private:
        ...
        void gate_critter_delay(int gate)
        {
            if (_trappedPlayerGUID) {
                if (Player* pPlayer = ObjectAccessor::GetPlayer(
                        instance, _trappedPlayerGUID)) {
...
void AddSC_instance_stratholme() { new instance_stratholme(); }
