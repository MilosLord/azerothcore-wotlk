...
const Position heirloomsPosition = {
    3423.389893f, -3055.571045f, 136.49837f, 5.707379f};
...
void HandleBothDead(Creature* creature, bool jarien, Unit* killer)
{
    ...
    if (jarien) {
        if (Creature* spirit =
                creature->FindNearestCreature(NPC_SPIRIT_OF_JARIEN, 100.f)) {
            spirit->AttackStop();
        ...
        if (Creature* spirit =
                creature->FindNearestCreature(NPC_SPIRIT_OF_SOTHOS, 100.f)) {
            spirit->AttackStop();
            spirit->SetReactState(REACT_PASSIVE);
            spirit->AI()->Talk(SAY_SPIRIT_BOTH_DEAD);
        }
    ...
    // chest spawn
    if (GameObject* chest =
            killer->SummonGameObject(GO_JARIEN_AND_SOTHOS_HEIRLOOMS,
                                     heirloomsPosition.GetPositionX(),
                                     heirloomsPosition.GetPositionY(),
                                     heirloomsPosition.GetPositionZ(),
                                     heirloomsPosition.GetOrientation(),
                                     0.f,
                                     0.f,
                                     0.f,
                                     0.f,
...
struct boss_jarien : public BossAI {
    ...
    void Reset() override
    {
        ...
        if (_sothosDied) {
            if (Creature* sothos =
                    me->FindNearestCreature(NPC_SOTHOS, 200.f, false)) {
                sothos->Respawn();
            }
        }
        _sothosDied = false;
...
struct boss_sothos : public BossAI {
    ...
    void Reset() override
    {
        ...
        if (_jarienDied) {
            if (Creature* jarien =
                    me->FindNearestCreature(NPC_JARIEN, 200.f, false)) {
                jarien->Respawn();
            }
        }
        _jarienDied = false;
