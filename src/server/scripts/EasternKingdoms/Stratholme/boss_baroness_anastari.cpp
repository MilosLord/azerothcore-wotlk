...
public:
    ...
    struct boss_baroness_anastariAI : public BossAI {
        boss_baroness_anastariAI(Creature* creature)
            : BossAI(creature, TYPE_ZIGGURAT1)
        {
        }
        ...
        void SchedulePossession()
        {
            _scheduler.Schedule(20s, 30s, [this](TaskContext context) {
                if (Unit* possessTarget = SelectTarget(
                        SelectTargetMethod::Random, 1, 0, true, false)) {
                    ...
                    // We must keep track of the possessed player, the aura
                    // falls off when their health drops below 50%. The
                    // encounter resumes when the aura falls off.
                    _scheduler.Schedule(
                        1s, [this](TaskContext possessionContext) {
                            if (Player* possessedTarget =
                                    ObjectAccessor::GetPlayer(
                                        *me, _possessedTargetGuid)) {
                                if (!possessedTarget->HasAura(
                                        SPELL_POSSESSED) ||
                                    possessedTarget->HealthBelowPct(50)) {
                                    possessedTarget->RemoveAurasDueToSpell(
                                        SPELL_POSSESS);
                                    possessedTarget->RemoveAurasDueToSpell(
                                        SPELL_POSSESSED);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            _scheduler.Update(
                diff, std::bind(&ScriptedAI::DoMeleeAttackIfReady, this));
        }
...
void AddSC_boss_baroness_anastari() { new boss_baroness_anastari; }
