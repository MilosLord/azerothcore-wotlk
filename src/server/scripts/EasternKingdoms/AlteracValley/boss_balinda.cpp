...
struct boss_balinda : public ScriptedAI {
    boss_balinda(Creature* creature)
        : ScriptedAI(creature), summons(me), _hasCastIceBlock(false)
    {
        scheduler.SetValidator(
            [this] { return !me->HasUnitState(UNIT_STATE_CASTING); });
    }

    void Reset() override { summons.DespawnAll(); }
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        ScheduleTimedEvent(
            5s, 15s, [&] { DoCastAOE(SPELL_ARCANE_EXPLOSION); }, 5s, 15s);

        ScheduleTimedEvent(
            8s, [&] { DoCastVictim(SPELL_CONE_OF_COLD); }, 10s, 20s);

        ScheduleTimedEvent(
            1s, [&] { DoCastVictim(SPELL_FIREBALL); }, 5s, 9s);

        ScheduleTimedEvent(
            4s, [&] { DoCastVictim(SPELL_FROSTBOLT); }, 4s, 12s);
        ...
        ScheduleTimedEvent(
            ...
            [&] {
                ...
                if (Creature* elemental =
                        summons.GetCreatureWithEntry(NPC_WATER_ELEMENTAL)) {
...
void AddSC_boss_balinda() { RegisterCreatureAI(boss_balinda); }
