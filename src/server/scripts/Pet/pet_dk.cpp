...
public:
    ...
    struct npc_pet_dk_ebon_gargoyleAI : ScriptedAI {
        ...
        {
            _despawnTimer =
                36000; // 30 secs + 4 fly out + 2 initial attack timer
            _despawning       = false;
        ...
        void InitializeAI() override
        {
            ...
            if (Aura* aur = me->GetAura(SPELL_DK_NIGHT_OF_THE_DEAD))
                if (AuraEffect* aurEff =
                        owner->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER,
                                             ...
                                             0)) {
                    if (aur->GetEffect(0)) {
                        aur->GetEffect(0)->SetAmount(-aurEff->GetSpellInfo()
                                                          ->Effects[EFFECT_2]
                                                          .CalcValue());
                    }
                }
            ...
            float tz = me->GetMapHeight(me->GetPositionX(),
                                        me->GetPositionY(),
                                        me->GetPositionZ(),
                                        true,
                                        MAX_FALL_DISTANCE);
            me->GetMotionMaster()->MoveCharge(
                me->GetPositionX(), me->GetPositionY(), tz, 7.0f, 1);
        ...
        void MySelectNextTarget()
        {
            ...
            if (owner && owner->GetTypeId() == TYPEID_PLAYER &&
                (!me->GetVictim() ||
                 me->GetVictim()->IsImmunedToSpell(
                     sSpellMgr->GetSpellInfo(51963)) ||
                 !me->IsValidAttackTarget(me->GetVictim()) ||
                 !owner->CanSeeOrDetect(me->GetVictim()))) {
                Unit* selection = owner->ToPlayer()->GetSelectedUnit();
                if (selection && selection != me->GetVictim() &&
                    me->IsValidAttackTarget(selection)) {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (_initialSelection) {
                ...
                Acore::UnitListSearcher<
                    ...
                    searcher(me, targets, u_check);
                Cell::VisitAllObjects(me, searcher, 50.0f);
                for (std::list<Unit*>::const_iterator iter = targets.begin();
                     iter != targets.end();
                     ++iter)
                    if ((*iter)->GetAura(SPELL_DK_SUMMON_GARGOYLE_1,
                                         me->GetOwnerGUID())) {
                        (*iter)->RemoveAura(SPELL_DK_SUMMON_GARGOYLE_1,
                                            me->GetOwnerGUID());
                        SetGazeOn(*iter);
            ...
            if (_despawnTimer > 4000) {
                ...
                if (_initialCastTimer >= 2000 &&
                    !me->HasUnitState(UNIT_STATE_CASTING |
                                      UNIT_STATE_LOST_CONTROL) &&
                    me->GetMotionMaster()->GetMotionSlotType(
                ...
                if (Unit* owner = me->GetCharmerOrOwner()) {
                    if (Player* player = owner->ToPlayer()) {
                        player->RemoveAurasDueToSpell(
                            SPELL_DK_RAISE_ALLY); // Remove Raise Ally aura
                        player->RemoveAurasDueToSpell(
                            SPELL_GHOUL_FRENZY); // Remove Frenzy aura
...
public:
    npc_pet_dk_army_of_the_dead()
        : CreatureScript("npc_pet_dk_army_of_the_dead")
    {
    }
...
public:
    npc_pet_dk_dancing_rune_weapon()
        : CreatureScript("npc_pet_dk_dancing_rune_weapon")
    {
    }

    struct npc_pet_dk_dancing_rune_weaponAI : public NullCreatureAI {
        npc_pet_dk_dancing_rune_weaponAI(Creature* creature)
        ...
        void InitializeAI() override
        {
            ...
            if (Unit* owner = me->GetOwner())
                me->GetMotionMaster()->MoveFollow(
                    owner, 0.01f, me->GetFollowAngle(), MOTION_SLOT_CONTROLLED);
...
class spell_pet_dk_gargoyle_strike : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pet_dk_gargoyle_strike::HandleDamageCalc,
                          EFFECT_0,
