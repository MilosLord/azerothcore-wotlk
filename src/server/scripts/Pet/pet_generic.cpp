...
static uint32 argentPonyService[2][3] = {{ARGENT_PONY_STATE_MAILBOX,
...
struct npc_pet_gen_argent_pony_bridle : public ScriptedAI {
        ...
        if (Unit* owner = me->GetCharmerOrOwner())
            if (Player* player = owner->ToPlayer())
                if (player->HasAchieved(ACHIEVEMENT_PONY_UP)) {
                    ...
                    aura     = (player->GetTeamId(true) == TEAM_ALLIANCE
                                    ? SPELL_AURA_TIRED_S
                                    : SPELL_AURA_TIRED_G);
                    ...
                    // Generate Banners
                    uint32 mask = player->GetTeamId(true) ? RACEMASK_HORDE
                                                          : RACEMASK_ALLIANCE;
                    for (uint8 i = 1; i < MAX_RACES; ++i)
                        if (mask & (1 << (i - 1)) &&
                            player->HasAchieved(argentBanners[i].achievement))
                            _banners[i] = true;
                }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (!creature->HasAura(player->GetTeamId(true) ? SPELL_AURA_TIRED_G
                                                       : SPELL_AURA_TIRED_S)) {
            uint8 _state = creature->AI()->GetData(0 /*GET_DATA_STATE*/);
            if (_state == ARGENT_PONY_STATE_ENCH ||
                _state == ARGENT_PONY_STATE_VENDOR)
            ...
            if (_state == ARGENT_PONY_STATE_ENCH ||
                ...
                AddGossipItemFor(player,
                                 ...
                                 GOSSIP_ACTION_BANK);
            if (_state == ARGENT_PONY_STATE_ENCH ||
                _state == ARGENT_PONY_STATE_MAILBOX)
                AddGossipItemFor(player,
                                 GOSSIP_ICON_CHAT,
                                 "Visit a mailbox.",
                                 GOSSIP_SENDER_MAIN,
        ...
        for (uint8 i = RACE_HUMAN; i < MAX_RACES; ++i)
            if (creature->AI()->GetData(i) == uint32(true))
                AddGossipItemFor(player,
                                 GOSSIP_ICON_CHAT,
                                 argentBanners[i].text,
                                 GOSSIP_SENDER_MAIN,
                                 argentBanners[i].spell);
                        ...
                        uint32 action)
    {
        ...
        case GOSSIP_ACTION_TRADE:
            ...
            spellId =
                player->GetTeamId(true) ? SPELL_AURA_SHOP_G : SPELL_AURA_SHOP_S;
        ...
        case GOSSIP_ACTION_BANK:
            ...
            spellId =
                player->GetTeamId(true) ? SPELL_AURA_BANK_G : SPELL_AURA_BANK_S;
        ...
        case GOSSIP_ACTION_MAILBOX: {
            ...
            spellId = player->GetTeamId(true) ? SPELL_AURA_POSTMAN_G
        ...
        if (spellId && !creature->HasAura(spellId)) {
            ...
            player->AddSpellCooldown(
                player->GetTeamId(true) ? SPELL_AURA_TIRED_G
                                        : SPELL_AURA_TIRED_S,
                0,
...
struct npc_pet_gen_gnomish_flame_turret : public ScriptedAI {
    ...
    void Reset() override { me->GetMotionMaster()->Clear(false); }
...
struct npc_pet_gen_valkyr_guardian : public ScriptedAI {
    ...
    {
        if (!target ||
            (me->GetVictim() && me->GetVictim()->IsAlive() &&
...
class spell_pet_gen_valkyr_guardian_smite : public SpellScript {
    ...
    {
        OnHit +=
            SpellHitFn(spell_pet_gen_valkyr_guardian_smite::RecalculateDamage);
    }
};
        ...
        if (_talkTimer >= 5000) {
            ...
            if (!_hasParty)
                Talk(0, ObjectAccessor::GetPlayer(*me, _ownerGUID));
                         ...
                         ObjectAccessor::GetPlayer(*me, _ownerGUID)) {
                uint8 limit = 0;
                if (player->GetGroup())
                    for (GroupReference* itr =
                             player->GetGroup()->GetFirstMember();
                         itr != nullptr && limit < 4;
                         itr = itr->next(), ++limit)
...
struct npc_pet_gen_plump_turkey : public PassiveAI {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (checkTimer > 3000) {
            checkTimer = 0;
            if (GameObject* go =
                    me->FindNearestGameObject(GO_BASIC_CAMPFIRE, 7.0f)) {
                goGUID = go->GetGUID();
                me->StopMoving();
                me->GetMotionMaster()->Clear(false);
        ...
        if (checkTimer >= 1000) {
            ...
            if (Creature* target = ObjectAccessor::GetCreature(*me, targetGUID))
                if (me->GetDistance2d(target) < 2.0f) {
                    ...
                    target->CastSpell(
                        target,
                        48708 /*SPELL_PET_TOY_FETCH_BALL_HAS_BALL*/,
                        true);
                    me->DespawnOrUnsummon(1);
                }
        }
...
enum Darting {
    SPELL_DARTING_ON_SPAWN =
        62586,                  // Applied on spawn via creature_template_addon
