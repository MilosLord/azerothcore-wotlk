...
#include "UnitAI.h"
/*
 * Scripts for spells with SPELLFAMILY_DEATHKNIGHT and SPELLFAMILY_GENERIC
 * spells used by deathknight players. Ordered alphabetically using scriptname.
...
enum DeathKnightSpellIcons { DK_ICON_ID_IMPROVED_DEATH_STRIKE = 2751 };

enum Misc { NPC_DK_GHOUL = 26125, NPC_RISEN_ALLY = 30230 };
...
class spell_dk_wandering_plague : public SpellScript {
    ...
    {
        static const AuraType ccAuras[] = {SPELL_AURA_MOD_CONFUSE,
                                           SPELL_AURA_MOD_FEAR,
                                           SPELL_AURA_MOD_STUN,
                                           SPELL_AURA_MOD_ROOT,
        ...
        for (std::list<WorldObject*>::iterator itr = targets.begin();
            ...
            bool skip = false;
            for (uint8 index = 0; !skip && ccAuras[index] != SPELL_AURA_NONE;
                 ++index) {
                Unit::AuraEffectList const& auras =
                    target->GetAuraEffectsByType(ccAuras[index]);
                for (Unit::AuraEffectList::const_iterator i = auras.begin();
                     i != auras.end();
                     ++i)
                    if ((*i)->GetAmount()) {
                        skip = true;
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_dk_wandering_plague::FilterTargets,
            EFFECT_0,
...
class spell_dk_raise_ally : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        if (Player* unitTarget = GetHitPlayer()) {
            ...
            if (Unit* ghoul = unitTarget->GetCharm()) {
                ...
                PetLevelInfo const* pInfo = sObjectMgr->GetPetLevelInfo(
                    ghoul->GetEntry(), ghoul->GetLevel());
                if (pInfo) // exist in DB
                {
                    ghoul->SetCreateHealth(pInfo->health);
                    ghoul->SetModifierValue(
                        UNIT_MOD_HEALTH, BASE_VALUE, pInfo->health);
                    ghoul->SetModifierValue(
                        UNIT_MOD_ARMOR, BASE_VALUE, float(pInfo->armor));
                    for (uint8 stat = 0; stat < MAX_STATS; ++stat)
                        ghoul->SetCreateStat(Stats(stat),
                                             float(pInfo->stats[stat]));
                }

                ghoul->SetBaseWeaponDamage(
                    BASE_ATTACK,
                    MINDAMAGE,
                ...
                if (Aura* aur = ghoul->AddAura(62137, ghoul))
                    if (AuraEffect* aurEff = GetCaster()->GetAuraEffect(
                            SPELL_AURA_ADD_FLAT_MODIFIER,
                            SPELLFAMILY_DEATHKNIGHT,
                            2718,
                            0))
                        if (aur->GetEffect(0))
                            aur->GetEffect(0)->SetAmount(
                                -aurEff->GetSpellInfo()
                                     ->Effects[EFFECT_2]
                                     .CalcValue());

                // Hit / Expertise scalling, warlock / hunter pets have this by
                // default
                ghoul->AddAura(SPELL_HUNTER_PET_SCALING_04, ghoul);

                // DK Ghoul haste refresh
                float val =
                    (GetCaster()->m_modAttackSpeedPct[BASE_ATTACK] - 1.0f) *
                    100.0f;
                ghoul->m_modAttackSpeedPct[BASE_ATTACK] =
                    GetCaster()->m_modAttackSpeedPct[BASE_ATTACK];
                ghoul->SetFloatValue(UNIT_FIELD_BASEATTACKTIME, 2000.0f);
                ghoul->ApplyPercentModFloatValue(
                    UNIT_FIELD_BASEATTACKTIME,
                    val,
                    true); // we want to reduce attack time
                ...
                for (uint8 i = STAT_STRENGTH; i <= STAT_STAMINA; ++i) {
                    ...
                    // Check just if owner has Ravenous Dead since it's effect
                    // is not an aura
                    AuraEffect const* aurEff = GetCaster()->GetAuraEffect(
                    ...
                    if (aurEff) {
                        SpellInfo const* spellInfo =
                            aurEff
                                ->GetSpellInfo(); // Then get the SpellProto and
                                                  // add the dummy effect value
                        AddPct(mod,
                               spellInfo->Effects[EFFECT_1]
                                   .CalcValue()); // Ravenous Dead edits the
                                                  // original scale
                    }
                    // Glyph of the Ghoul
                    aurEff = GetCaster()->GetAuraEffect(
                        SPELL_DK_GLYPH_OF_THE_GHOUL, EFFECT_0);
                    if (aurEff)
                        mod += CalculatePct(
                            1.0f,
                            aurEff->GetAmount()); // Glyph of the Ghoul adds a
                                                  // flat value to the scale mod
                ...
                // Attack Power
                ghoul->SetModifierValue(UNIT_MOD_ATTACK_POWER,
                                        BASE_VALUE,
                                        589 + ghoul->GetStat(STAT_STRENGTH) +
                                            ghoul->GetStat(STAT_AGILITY));
                ghoul->SetInt32Value(UNIT_FIELD_ATTACK_POWER,
                                     (int32)ghoul->GetModifierValue(
                                         UNIT_MOD_ATTACK_POWER, BASE_VALUE) *
                                         ghoul->GetModifierValue(
                                             UNIT_MOD_ATTACK_POWER, BASE_PCT));
                ghoul->SetInt32Value(UNIT_FIELD_ATTACK_POWER_MODS,
                                     (int32)ghoul->GetModifierValue(
                                         UNIT_MOD_ATTACK_POWER, TOTAL_VALUE));
                ghoul->SetFloatValue(
                    UNIT_FIELD_ATTACK_POWER_MULTIPLIER,
                    ghoul->GetModifierValue(UNIT_MOD_ATTACK_POWER, TOTAL_PCT) -
                        1.0f);

                // Health
                ghoul->SetModifierValue(UNIT_MOD_HEALTH,
                                        TOTAL_VALUE,
                                        (ghoul->GetStat(STAT_STAMINA) -
                                         ghoul->GetCreateStat(STAT_STAMINA)) *
                                            10.0f);

                // Power Energy
                ghoul->SetModifierValue(
                    UnitMods(UNIT_MOD_POWER_START +
                             static_cast<uint8>(POWER_ENERGY)),
                    BASE_VALUE,
                    ghoul->GetCreatePowers(POWER_ENERGY));
                ghoul->UpdateAllStats();
                ghoul->SetFullHealth();

                // Aura Immunities
                ghoul->ApplySpellImmune(0,
                                        IMMUNITY_STATE,
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_dk_raise_ally::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_dk_raise_ally_trigger : public AuraScript {
                            ...
                            AuraEffectHandleModes /*mode*/)
    {
        if (Unit* charm = GetUnitOwner()->GetCharm())
            if (GetSpellInfo()->Effects[EFFECT_0].MiscValue >= 0 &&
                charm->GetEntry() ==
                    uint32(GetSpellInfo()->Effects[EFFECT_0].MiscValue))
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_dk_raise_ally_trigger::HandleEffectRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
...
class spell_dk_aotd_taunt : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_dk_aotd_taunt::FilterTargets,
                                          EFFECT_ALL,
...
class spell_dk_death_and_decay_aura : public AuraScript {
    ...
    void HandlePeriodic(AuraEffect const* aurEff)
    {
        ...
        if (GetCaster() && GetTarget()) {
            ...
            GetCaster()->CastCustomSpell(GetTarget(),
    ...
    void Register() override
    {
        if (m_scriptSpellId != SPELL_DK_DEATH_AND_DECAY_TRIGGER)
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_dk_death_and_decay_aura::HandlePeriodic,
                EFFECT_0,
...
class spell_dk_master_of_ghouls : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_dk_master_of_ghouls::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_ADD_FLAT_MODIFIER,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_dk_master_of_ghouls::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_ADD_FLAT_MODIFIER,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_chains_of_ice : public SpellScript {
    ...
    void HandleAfterCast()
    {
        if (Unit* target = GetExplTargetUnit()) {
            std::list<TargetInfo> const* targetsInfo =
                GetSpell()->GetUniqueTargetInfo();
            for (std::list<TargetInfo>::const_iterator ihit =
                     targetsInfo->begin();
                 ihit != targetsInfo->end();
                 ++ihit)
                if (ihit->missCondition == SPELL_MISS_NONE &&
                    ihit->targetGUID == target->GetGUID())
                    GetCaster()->CastSpell(
                        target, 55095 /*SPELL_FROST_FEVER*/, true);
        }
    }
...
class spell_dk_chains_of_ice_aura : public AuraScript {
    ...
    void Register() override
    {
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_dk_chains_of_ice_aura::HandlePeriodic,
            EFFECT_1,
            SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_dk_bloodworms : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(
            spell_dk_bloodworms::HandleSummon, EFFECT_0, SPELL_EFFECT_SUMMON);
    }
};
...
class spell_dk_summon_gargoyle : public SpellScript {
    ...
    void SetDest(SpellDestination& dest)
    {
        ...
        if (GetCaster()->IsWithinLOS(dest._position.GetPositionX(),
                                     dest._position.GetPositionY(),
                                     dest._position.GetPositionZ() + 15.0f))
    ...
    void Register() override
    {
        OnDestinationTargetSelect +=
            SpellDestinationTargetSelectFn(spell_dk_summon_gargoyle::SetDest,
                                           EFFECT_0,
...
class spell_dk_improved_blood_presence_proc : public AuraScript {
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_dk_improved_blood_presence_proc::CheckProc);
    }
};
...
class spell_dk_wandering_plague_aura : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (!spellInfo || !eventInfo.GetActionTarget() ||
            !eventInfo.GetDamageInfo() || !eventInfo.GetActor())
            return false;

        if (!roll_chance_f(eventInfo.GetActor()->GetUnitCriticalChance(
                BASE_ATTACK, eventInfo.GetActionTarget())))
            return false;

        return !eventInfo.GetActor()->HasSpellCooldown(
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        eventInfo.GetActor()->AddSpellCooldown(
            SPELL_DK_WANDERING_PLAGUE_TRIGGER, 0, 1000);
        eventInfo.GetActor()->CastCustomSpell(
            SPELL_DK_WANDERING_PLAGUE_TRIGGER,
            SPELLVALUE_BASE_POINT0,
            CalculatePct<int32, int32>(eventInfo.GetDamageInfo()->GetDamage(),
                                       aurEff->GetAmount()),
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_dk_wandering_plague_aura::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_dk_wandering_plague_aura::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_dk_rune_of_the_fallen_crusader : public SpellScript {
    ...
    void RecalculateDamage()
    {
        ...
        for (std::list<TargetInfo>::iterator ihit = targetsInfo->begin();
             ihit != targetsInfo->end();
             ++ihit)
            if (ihit->targetGUID == GetCaster()->GetGUID())
                ihit->crit = roll_chance_f(
                    GetCaster()->GetFloatValue(PLAYER_CRIT_PERCENTAGE));
    }

    void Register() override
    {
        OnHit +=
            SpellHitFn(spell_dk_rune_of_the_fallen_crusader::RecalculateDamage);
    }
};
    ...
    void HandleProc(ProcEventInfo& eventInfo)
    {
        ...
        if (eventInfo.GetDamageInfo() &&
            ...
            (!eventInfo.GetSpellInfo() ||
             eventInfo.GetSpellInfo()->Dispel != DISPEL_DISEASE))
            SetDuration(0);
    }
...
class spell_dk_blood_caked_blade : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        return eventInfo.GetActionTarget() &&
               eventInfo.GetActionTarget()->IsAlive() && eventInfo.GetActor();
    }
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (roll_chance_i(75) && eventInfo.GetActor()->FindMap() &&
            !eventInfo.GetActor()->FindMap()->IsBattlegroundOrArena() &&
            eventInfo.GetActor()->HasAura(71903) &&
            !eventInfo.GetActor()->HasAura(
                SPELL_SHADOWMOURNE_CHAOS_BANE_BUFF)) {
            eventInfo.GetActor()->CastSpell(
                eventInfo.GetActor(), SPELL_SHADOWMOURNE_SOUL_FRAGMENT, true);

            // this can't be handled in AuraScript of SoulFragments because we
            // need to know victim
            if (Aura* soulFragments = eventInfo.GetActor()->GetAura(
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_dk_blood_caked_blade::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_dk_dancing_rune_weapon : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        if (!eventInfo.GetActor() || !eventInfo.GetActionTarget() ||
            !eventInfo.GetActionTarget()->IsAlive() ||
            eventInfo.GetActor()->GetTypeId() != TYPEID_PLAYER)
            return false;
        ...
        if (spellInfo->Id != SPELL_DK_DEATH_COIL_DAMAGE &&
            !eventInfo.GetActor()->ToPlayer()->HasActiveSpell(spellInfo->Id))
            return false;

        // Can't cast raise dead/ally, death grip, dark command, death pact,
        // death and decay, anti-magic shell
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        for (Unit::ControlSet::const_iterator itr =
                 player->m_Controlled.begin();
             itr != player->m_Controlled.end();
             ++itr)
            if (int32((*itr)->GetEntry()) ==
                GetSpellInfo()->Effects[EFFECT_0].MiscValue) {
        ...
        if (SpellInfo const* procSpell = eventInfo.GetSpellInfo()) {
            ...
            dancingRuneWeapon->CastSpell(target,
                                         procSpell->Id,
                                         true,
                                         nullptr,
                                         aurEff,
                                         dancingRuneWeapon->GetGUID());
            ...
            player->CalculateMeleeDamage(
                target,
                &damageInfo,
                eventInfo.GetDamageInfo()->GetAttackType());
            for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
                Unit::DealDamageMods(target,
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_dk_dancing_rune_weapon::HandleProc,
                             EFFECT_1,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_dk_dancing_rune_weapon_visual : public AuraScript {
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_dk_dancing_rune_weapon_visual::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_scent_of_blood_trigger : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        return (eventInfo.GetHitMask() & (PROC_EX_DODGE | PROC_EX_PARRY)) ||
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_dk_scent_of_blood_trigger::CheckProc);
    }
};
...
class spell_dk_pet_scaling : public AuraScript {
    ...
    void CalculateStatAmount(AuraEffect const* aurEff,
                             int32&            amount,
                             bool& /*canBeRecalculated*/)
    {
        Stats stat = Stats(
            aurEff->GetSpellInfo()->Effects[aurEff->GetEffIndex()].MiscValue);
        ...
        if (GetUnitOwner()->GetEntry() != NPC_RISEN_GHOUL) {
            ...
            if (GetUnitOwner()->GetEntry() == NPC_EBON_GARGOYLE &&
                ...
                if (Unit* owner = GetUnitOwner()->GetOwner())
                    amount = CalculatePct(
                        std::max<int32>(0, owner->GetStat(stat)), 30);
            return;
        }
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            ...
            // Check just if owner has Ravenous Dead since it's effect is not an
            // aura
            if (AuraEffect const* rdEff =
                    owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE,
                                         SPELLFAMILY_DEATHKNIGHT,
                                         3010,
                                         0)) {
                SpellInfo const* spellInfo =
                    rdEff->GetSpellInfo(); // Then get the SpellProto and add
                                           // the dummy effect value
                AddPct(
                    modifier,
                    spellInfo->Effects[EFFECT_1]
                        .CalcValue()); // Ravenous Dead edits the original scale
            }

            // xinef: Glyph of the Ghoul
            if (AuraEffect const* glyphEff =
                    owner->GetAuraEffect(SPELL_DK_GLYPH_OF_THE_GHOUL, EFFECT_0))
                modifier += glyphEff->GetAmount();

            amount = CalculatePct(std::max<int32>(0, owner->GetStat(stat)),
                                  modifier);
        }
    }

    void CalculateSPAmount(AuraEffect const* /*aurEff*/,
                           int32& amount,
                           bool& /*canBeRecalculated*/)
    {
    ...
    void CalculateHasteAmount(AuraEffect const* /*aurEff*/,
                              int32& amount,
                              bool& /*canBeRecalculated*/)
    {
        // xinef: scale haste with owners melee haste
        if (Unit* owner = GetUnitOwner()->GetOwner())
            if (owner->m_modAttackSpeedPct[BASE_ATTACK] <
                1.0f) // inherit haste only
                amount = std::min<int32>(
                    100,
                    int32(((1.0f / owner->m_modAttackSpeedPct[BASE_ATTACK]) -
                           1.0f) *
                          100.0f));
    }
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        GetUnitOwner()->ApplySpellImmune(0,
                                         IMMUNITY_STATE,
    ...
    void HandlePeriodic(AuraEffect const* aurEff)
    {
        ...
        if (aurEff->GetAuraType() == SPELL_AURA_MOD_STAT &&
            (aurEff->GetMiscValue() == STAT_STAMINA ||
             aurEff->GetMiscValue() == STAT_INTELLECT)) {
            int32 currentAmount = aurEff->GetAmount();
            int32 newAmount =
                GetEffect(aurEff->GetEffIndex())->CalculateAmount(GetCaster());
            if (newAmount != currentAmount) {
                if (aurEff->GetMiscValue() == STAT_STAMINA) {
                    uint32 actStat = GetUnitOwner()->GetHealth();
                    GetEffect(aurEff->GetEffIndex())
                        ->ChangeAmount(newAmount, false);
                    GetUnitOwner()->SetHealth(std::min<uint32>(
                        GetUnitOwner()->GetMaxHealth(), actStat));
                }
                else {
                    ...
                    GetUnitOwner()->SetPower(
                        POWER_MANA,
                        std::min<uint32>(
                            GetUnitOwner()->GetMaxPower(POWER_MANA), actStat));
                }
            }
        }
        else
    ...
    void Register() override
    {
        if (m_scriptSpellId == 54566) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_dk_pet_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);
            DoEffectCalcAmount +=
                AuraEffectCalcAmountFn(spell_dk_pet_scaling::CalculateSPAmount,
                                       EFFECT_ALL,
        ...
        if (m_scriptSpellId == 51996)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_dk_pet_scaling::CalculateHasteAmount,
                EFFECT_ALL,
                SPELL_AURA_MELEE_SLOW);

        OnEffectApply +=
            AuraEffectApplyFn(spell_dk_pet_scaling::HandleEffectApply,
                              EFFECT_ALL,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_dk_pet_scaling::CalcPeriodic, EFFECT_ALL, SPELL_AURA_ANY);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_dk_pet_scaling::HandlePeriodic, EFFECT_ALL, SPELL_AURA_ANY);
    }
};
...
class spell_dk_anti_magic_shell_raid : public AuraScript {
    ...
    void CalculateAmount(AuraEffect const* /*aurEff*/,
                         ...
                         bool& /*canBeRecalculated*/)
    {
        /// @todo: this should absorb limited amount of damage, but no info on
        /// calculation formula
        amount = -1;
    }
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_dk_anti_magic_shell_raid::CalculateAmount,
            EFFECT_0,
            SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb += AuraEffectAbsorbFn(
            spell_dk_anti_magic_shell_raid::Absorb, EFFECT_0);
    }
};
...
class spell_dk_anti_magic_shell_self : public AuraScript {
    ...
    Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        absorbAmount = std::min(CalculatePct(dmgInfo.GetDamage(), absorbPct),
                                GetTarget()->CountPctFromMaxHealth(hpPct));
    }

    void
    Trigger(AuraEffect* aurEff, DamageInfo& /*dmgInfo*/, uint32& absorbAmount)
    {
        // damage absorbed by Anti-Magic Shell energizes the DK with additional
        // runic power. This, if I'm not mistaken, shows that we get back ~20%
        // of the absorbed damage as runic power.
        int32 bp = CalculatePct(absorbAmount, 20);
        GetTarget()->CastCustomSpell(SPELL_DK_RUNIC_POWER_ENERGIZE,
                                     SPELLVALUE_BASE_POINT0,
                                     bp,
                                     GetTarget(),
                                     true,
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_dk_anti_magic_shell_self::CalculateAmount,
            EFFECT_0,
            SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb += AuraEffectAbsorbFn(
            spell_dk_anti_magic_shell_self::Absorb, EFFECT_0);
        AfterEffectAbsorb += AuraEffectAbsorbFn(
            spell_dk_anti_magic_shell_self::Trigger, EFFECT_0);

        OnEffectApply +=
            AuraEffectApplyFn(spell_dk_anti_magic_shell_self::HandleEffectApply,
                              EFFECT_1,
                              SPELL_AURA_MOD_IMMUNE_AURA_APPLY_SCHOOL,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(
            spell_dk_anti_magic_shell_self::HandleEffectRemove,
            EFFECT_1,
            SPELL_AURA_MOD_IMMUNE_AURA_APPLY_SCHOOL,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_anti_magic_zone : public AuraScript {
    ...
    void CalculateAmount(AuraEffect const* /*aurEff*/,
                         ...
                         bool& /*canBeRecalculated*/)
    {
        SpellInfo const* talentSpell =
            sSpellMgr->AssertSpellInfo(SPELL_DK_ANTI_MAGIC_SHELL_TALENT);
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_dk_anti_magic_zone::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_dk_anti_magic_zone::Absorb, EFFECT_0);
    }
};
...
class spell_dk_blood_boil : public SpellScript {
    ...
    bool Load() override
    {
        ...
        return GetCaster()->GetTypeId() == TYPEID_PLAYER &&
               GetCaster()->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_ABILITY);
    }
    ...
    void HandleAfterHit()
    {
        ...
        _executed = true;
        GetCaster()->CastSpell(
            GetCaster(), SPELL_DK_BLOOD_BOIL_TRIGGERED, true);
    }
...
class spell_dk_blood_gorged : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        GetTarget()->CastCustomSpell(SPELL_DK_BLOOD_GORGED_HEAL,
                                     SPELLVALUE_BASE_POINT0,
                                     bp,
                                     _procTarget,
                                     true,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_dk_blood_gorged::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
...
public:
    explicit CorpseExplosionCheck(ObjectGuid casterGUID, bool allowGhoul)
        : _casterGUID(casterGUID), _allowGhoul(allowGhoul)
    {
    }
    ...
    bool operator()(WorldObject* obj) const
    {
        if (Unit* target = obj->ToUnit()) {
            if ((target->isDead() ||
                 (_allowGhoul && target->GetEntry() == NPC_DK_GHOUL &&
                  target->GetOwnerGUID() == _casterGUID)) &&
...
class spell_dk_corpse_explosion : public SpellScript {
    ...
    void CheckTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (!target) {
            targets.remove_if(
                CorpseExplosionCheck(GetCaster()->GetGUID(), false));
            if (targets.empty()) {
    ...
    void HandleDamage(SpellEffIndex effIndex, Unit* target)
    {
        if (effIndex == EFFECT_0)
            ...
            GetCaster()->CastCustomSpell(
                GetEffectValue(),
                SPELLVALUE_BASE_POINT0,
                GetSpell()->CalculateSpellDamage(EFFECT_0, nullptr),
                target,
                true);
    }
        ...
        if (Unit* unitTarget = GetHitUnit()) {
            if (unitTarget->IsAlive()) // Living ghoul as a target
            {
                ...
                if (CharmInfo* charmInfo = unitTarget->GetCharmInfo())
                    charmInfo->GetGlobalCooldownMgr().CancelGlobalCooldown(
                        sSpellMgr->GetSpellInfo(SPELL_DK_GHOUL_EXPLODE));

                unitTarget->StopMoving();
                unitTarget->CastSpell(
                    unitTarget, SPELL_DK_GHOUL_EXPLODE, false);
                // Corpse Explosion (Suicide) and Set corpse look handled in
                // SpellScript of SPELL_DK_GHOUL_EXPLODE
            }
            else // Some corpse
            {
                ...
                unitTarget->CastSpell(
                    unitTarget, SPELL_DK_CORPSE_EXPLOSION_TRIGGERED, true);
                // Set corpse look
                GetCaster()->CastSpell(
                    unitTarget, SPELL_DK_CORPSE_EXPLOSION_VISUAL, true);
            }
        }
    }
    ...
    void Register() override
    {
        OnObjectTargetSelect +=
            SpellObjectTargetSelectFn(spell_dk_corpse_explosion::CheckTarget,
                                      EFFECT_0,
                                      TARGET_UNIT_TARGET_ANY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_dk_corpse_explosion::CheckTargets,
            EFFECT_1,
            TARGET_UNIT_DEST_AREA_ENTRY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_dk_corpse_explosion::HandleCorpseExplosion,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_dk_corpse_explosion::HandleCorpseExplosion,
...
class spell_dk_death_coil : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_DK_DEATH_COIL_DAMAGE, SPELL_DK_DEATH_COIL_HEAL});
    }
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            if (caster->IsFriendlyTo(target)) {
                ...
                if (AuraEffect const* auraEffect = caster->GetAuraEffect(
                        SPELL_DK_ITEM_SIGIL_VENGEFUL_HEART, EFFECT_1))
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_dk_death_coil::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_dk_death_gate : public SpellScript {
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(spell_dk_death_gate::HandleScript,
                                           EFFECT_0,
...
class spell_dk_death_grip : public SpellScript {
    ...
    SpellCastResult CheckCast()
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER &&
            caster->GetExactDist(target) <
                8.0f) // xinef: should be 8.0f, but we have to add target size
                      // (1.5f)
            return SPELL_FAILED_TOO_CLOSE;

        if (caster->HasUnitState(UNIT_STATE_JUMPING) ||
            caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING))
    ...
    void HandleBaseDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (caster != target) {
            if (targetCreature &&
                (targetCreature->isWorldBoss() ||
                 targetCreature->IsDungeonBoss()) &&
                targetCreature->GetEntry() !=
                    EntryCheck(targetCreature->GetEntry())) {
                return;
            }
            else {
                caster->CastSpell(target, 49560, true);
                SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(1766); // Rogue kick
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit())
            if (!target->HasAuraType(SPELL_AURA_DEFLECT_SPELLS) ||
                target->HasUnitState(UNIT_STATE_STUNNED)) // Deterrence
            {
                if (target != GetCaster()) {
                    SpellInfo const* spellInfo =
                        sSpellMgr->GetSpellInfo(1766); // Rogue kick
                ...
                target->CastSpell(gripPos.GetPositionX(),
                                  gripPos.GetPositionY(),
                                  gripPos.GetPositionZ(),
                                  57604,
                                  true);
            }
    }
    ...
    {
        if (m_scriptSpellId ==
            49576) // xinef: base death grip, add pvp range restriction
        {
            OnCheckCast += SpellCheckCastFn(spell_dk_death_grip::CheckCast);
            OnEffectHitTarget +=
                SpellEffectFn(spell_dk_death_grip::HandleBaseDummy,
                              EFFECT_0,
                              SPELL_EFFECT_DUMMY);
        }
        else
            OnEffectHitTarget += SpellEffectFn(
                spell_dk_death_grip::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_dk_death_pact : public SpellScript {
    ...
    {
        Unit* target = nullptr;
        for (std::list<WorldObject*>::iterator itr = targetList.begin();
             itr != targetList.end();
             ++itr) {
            if (Unit* unit = (*itr)->ToUnit())
                if (unit->GetOwnerGUID() == GetCaster()->GetGUID() &&
                    unit->GetCreatureType() == CREATURE_TYPE_UNDEAD) {
                    target = unit;
                    break;
                }
        }
    ...
    void Register() override
    {
        ...
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_dk_death_pact::FilterTargets,
                                          EFFECT_1,
...
class spell_dk_death_strike : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            int32  bp =
                int32(count *
                      caster->CountPctFromMaxHealth(int32(
                          GetSpellInfo()->Effects[EFFECT_0].DamageMultiplier)));
            // Improved Death Strike
            if (AuraEffect const* aurEff =
                    caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER,
                                          SPELLFAMILY_DEATHKNIGHT,
                                          DK_ICON_ID_IMPROVED_DEATH_STRIKE,
                                          0))
                AddPct(bp,
                       caster->CalculateSpellDamage(
                           caster, aurEff->GetSpellInfo(), 2));
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_dk_death_strike::HandleDummy, EFFECT_2, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_dk_ghoul_explode : public SpellScript {
    ...
    {
        int32 value = int32(GetCaster()->CountPctFromMaxHealth(
            GetSpellInfo()->Effects[EFFECT_2].CalcValue(GetCaster())));
    ...
    void Register() override
    {
        OnEffectLaunchTarget +=
            SpellEffectFn(spell_dk_ghoul_explode::HandleDamage,
                          EFFECT_0,
                          SPELL_EFFECT_SCHOOL_DAMAGE);
        OnEffectHitTarget += SpellEffectFn(spell_dk_ghoul_explode::Suicide,
                                           EFFECT_1,
                                           SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_dk_ghoul_thrash : public SpellScript {
    ...
    void CalcDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        Causes more damage per frenzy point:
            1 point   : (Attack power * 40 * 0.01 + Attack power * 0.05)-(Attack
        power * 40 * 0.01 + Attack power * 0.10) damage 2 points  : (Attack
        power * 40 * 0.01 + Attack power * 0.10)-(Attack power * 40 * 0.01 +
        Attack power * 0.20) damage 3 points  : (Attack power * 40 * 0.01 +
        Attack power * 0.15)-(Attack power * 40 * 0.01 + Attack power * 0.30)
        damage 4 points  : (Attack power * 40 * 0.01 + Attack power *
        0.20)-(Attack power * 40 * 0.01 + Attack power * 0.40) damage 5 points
        : (Attack power * 40 * 0.01 + Attack power * 0.25)-(Attack power * 40 *
        0.01 + Attack power * 0.50) damage
        */
        ...
        if (Aura* frenzy = GetCaster()->GetAura(SPELL_GHOUL_FRENZY)) {
            ...
            SetEffectValue(fixedDamageBonus +
                           urand(int32(APBonus), int32(APBonus * 2.f)));
    ...
    void Register() override
    {
        OnEffectLaunchTarget += SpellEffectFn(spell_dk_ghoul_thrash::CalcDamage,
                                              EFFECT_0,
                                              SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_dk_icebound_fortitude : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (Unit* caster = GetCaster()) {
            int32  value    = amount;
            uint32 defValue = uint32(
                caster->ToPlayer()->GetSkillValue(SKILL_DEFENSE) +
                caster->ToPlayer()->GetRatingBonusValue(CR_DEFENSE_SKILL));
            ...
            if (AuraEffect const* aurEff = caster->GetAuraEffect(
                    SPELL_DK_GLYPH_OF_ICEBOUND_FORTITUDE, EFFECT_0)) {
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_dk_icebound_fortitude::CalculateAmount,
                                   EFFECT_2,
...
class spell_dk_improved_blood_presence : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_DK_BLOOD_PRESENCE,
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        if ((target->HasAura(SPELL_DK_FROST_PRESENCE) ||
             target->HasAura(SPELL_DK_UNHOLY_PRESENCE)) &&
            !target->HasAura(SPELL_DK_IMPROVED_BLOOD_PRESENCE_TRIGGERED))
            target->CastCustomSpell(SPELL_DK_IMPROVED_BLOOD_PRESENCE_TRIGGERED,
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_dk_improved_blood_presence::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_dk_improved_blood_presence::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_improved_frost_presence : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        if ((target->HasAura(SPELL_DK_BLOOD_PRESENCE) ||
            ...
            target->CastCustomSpell(SPELL_DK_FROST_PRESENCE_TRIGGERED,
                                    SPELLVALUE_BASE_POINT0,
                                    aurEff->GetAmount(),
                                    target,
                                    true,
    ...
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_dk_improved_frost_presence::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_dk_improved_frost_presence::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_improved_unholy_presence : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        if (target->HasAura(SPELL_DK_UNHOLY_PRESENCE) &&
            !target->HasAura(SPELL_DK_IMPROVED_UNHOLY_PRESENCE_TRIGGERED)) {
        ...
        if ((target->HasAura(SPELL_DK_BLOOD_PRESENCE) ||
             target->HasAura(SPELL_DK_FROST_PRESENCE)) &&
            !target->HasAura(SPELL_DK_UNHOLY_PRESENCE_TRIGGERED))
            target->CastCustomSpell(SPELL_DK_UNHOLY_PRESENCE_TRIGGERED,
                                    SPELLVALUE_BASE_POINT0,
                                    aurEff->GetAmount(),
                                    target,
                                    true,
    ...
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_dk_improved_unholy_presence::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_dk_improved_unholy_presence::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_pestilence : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_dk_pestilence::HandleScriptEffect,
                          EFFECT_2,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_dk_presence : public AuraScript {
    ...
    void HandleImprovedBloodPresence(AuraEffect const* aurEff,
                                     AuraEffectHandleModes /*mode*/)
    {
        Unit* target = GetTarget();
        ...
        if (GetId() == SPELL_DK_BLOOD_PRESENCE)
            target->CastSpell(
                target, SPELL_DK_IMPROVED_BLOOD_PRESENCE_TRIGGERED, true);
        else if (AuraEffect const* impAurEff =
                     target->GetAuraEffectOfRankedSpell(
                         SPELL_DK_IMPROVED_BLOOD_PRESENCE_R1, EFFECT_0))
    ...
    void HandleImprovedFrostPresence(AuraEffect const* aurEff,
                                     AuraEffectHandleModes /*mode*/)
    {
        Unit* target = GetTarget();
        ...
        if (GetId() == SPELL_DK_FROST_PRESENCE)
            target->CastSpell(target, SPELL_DK_FROST_PRESENCE_TRIGGERED, true);
                     ...
                     target->GetAuraEffectOfRankedSpell(
                         SPELL_DK_IMPROVED_FROST_PRESENCE_R1, EFFECT_0))
    ...
    void HandleImprovedUnholyPresence(AuraEffect const* aurEff,
                                      AuraEffectHandleModes /*mode*/)
    {
        Unit* target = GetTarget();
        ...
        if (AuraEffect const* impAurEff = target->GetAuraEffectOfRankedSpell(
                SPELL_DK_IMPROVED_UNHOLY_PRESENCE_R1, EFFECT_0)) {
            if (GetId() == SPELL_DK_UNHOLY_PRESENCE) {
                // Not listed as any effect, only base points set
                int32 bp =
                    impAurEff->GetSpellInfo()->Effects[EFFECT_1].CalcValue();
                target->CastCustomSpell(
                    target,
                    SPELL_DK_IMPROVED_UNHOLY_PRESENCE_TRIGGERED,
                    &bp,
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_dk_presence::HandleImprovedBloodPresence,
                              EFFECT_0,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectApply +=
            AuraEffectApplyFn(spell_dk_presence::HandleImprovedFrostPresence,
                              EFFECT_0,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectApply +=
            AuraEffectApplyFn(spell_dk_presence::HandleImprovedUnholyPresence,
                              EFFECT_0,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_dk_presence::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_ANY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
public:
    ...
    {
        if (Unit* target = obj->ToUnit()) {
            if (!target->IsAlive() && _caster->isHonorOrXPTarget(target) &&
...
class spell_dk_raise_dead : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_DK_RAISE_DEAD_USE_REAGENT, SPELL_DK_MASTER_OF_GHOULS});
    }
    ...
    {
        /// @workaround: there is no access to castresult of other spells, check
        /// it manually
        SpellInfo const* reagentSpell =
        ...
        if (!player->CanNoReagentCast(reagentSpell)) {
            for (uint32 i = 0; i < MAX_SPELL_REAGENTS; i++) {
                ...
                if (!player->HasItemCount(reagentSpell->Reagent[i],
                                          reagentSpell->ReagentCount[i])) {
                    Spell::SendCastResult(
                        player, reagentSpell, 0, SPELL_FAILED_REAGENTS);
    ...
    void Register() override
    {
        ...
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_dk_raise_dead::CheckTargets,
                                          EFFECT_1,
                                          TARGET_UNIT_DEST_AREA_ENTRY);
        OnObjectTargetSelect += SpellObjectTargetSelectFn(
            spell_dk_raise_dead::CheckTarget, EFFECT_2, TARGET_UNIT_CASTER);
        OnCast += SpellCastFn(spell_dk_raise_dead::ConsumeReagents);
        OnEffectHitTarget += SpellEffectFn(spell_dk_raise_dead::HandleRaiseDead,
                                           EFFECT_1,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
        OnEffectHitTarget += SpellEffectFn(
            spell_dk_raise_dead::HandleRaiseDead, EFFECT_2, SPELL_EFFECT_DUMMY);
    }
...
class spell_dk_rune_tap_party : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_dk_rune_tap_party::CheckTargets,
                                          EFFECT_0,
...
class spell_dk_scent_of_blood : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            GetTarget(), SPELL_DK_SCENT_OF_BLOOD, true, nullptr, aurEff);
        GetTarget()->RemoveAuraFromStack(GetSpellInfo()->Id);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_dk_scent_of_blood::HandleProc,
                                         EFFECT_0,
...
class spell_dk_scourge_strike : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* unitTarget = GetHitUnit()) {
            uint8 mode =
                caster->GetAuraEffectDummy(SPELL_DK_GLYPH_OF_SCOURGE_STRIKE)
                    ? 2
                    : 0;
            ...
            if (AuraEffect const* aurEff = caster->GetAuraEffect(
                    SPELL_DK_ITEM_T8_MELEE_4P_BONUS, EFFECT_0))
                AddPct(disease_amt, aurEff->GetAmount());

            multiplier =
                disease_amt *
                unitTarget->GetDiseasesByCaster(caster->GetGUID(), mode) /
                100.0f;
    ...
    void HandleAfterHit()
    {
        ...
        if (Unit* unitTarget = ObjectAccessor::GetUnit(*caster, guid)) {
            ...
            caster->CastCustomSpell(unitTarget,
            ...
            // Xinef: Shadowmourne hack (scourge strike trigger proc
            // disabled...)
            if (roll_chance_i(75) && caster->FindMap() &&
                !caster->FindMap()->IsBattlegroundOrArena() &&
                caster->HasAura(71903) &&
                !caster->HasAura(SPELL_SHADOWMOURNE_CHAOS_BANE_BUFF)) {
                caster->CastSpell(
                    caster, SPELL_SHADOWMOURNE_SOUL_FRAGMENT, true);

                // this can't be handled in AuraScript of SoulFragments because
                // we need to know victim
                if (Aura* soulFragments =
                        caster->GetAura(SPELL_SHADOWMOURNE_SOUL_FRAGMENT)) {
                    if (soulFragments->GetStackAmount() >= 10) {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_dk_scourge_strike::HandleDummy, EFFECT_2, SPELL_EFFECT_DUMMY);
...
class spell_dk_spell_deflection : public AuraScript {
    ...
    Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (GetTarget()->IsNonMeleeSpellCast(false, false, true) ||
            GetTarget()->HasUnitState(UNIT_STATE_CONTROLLED))
            chance = 0.0f;
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_dk_spell_deflection::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_dk_spell_deflection::Absorb, EFFECT_0);
    }
};
...
class spell_dk_vampiric_blood : public AuraScript {
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_dk_vampiric_blood::CalculateAmount,
                                   EFFECT_1,
...
class spell_dk_will_of_the_necropolis : public AuraScript {
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        ...
        // can't use other spell than will of the necropolis due to spell_ranks
        // dependency
        ...
        if (!sSpellMgr->GetSpellWithRank(
                SPELL_DK_WILL_OF_THE_NECROPOLIS_TALENT_R1, rank, true))
            return false;
    ...
    Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        SpellInfo const* talentProto =
            sSpellMgr->AssertSpellInfo(sSpellMgr->GetSpellWithRank(
                SPELL_DK_WILL_OF_THE_NECROPOLIS_TALENT_R1, rank));

        int32 remainingHp =
            int32(GetTarget()->GetHealth() - dmgInfo.GetDamage());
        int32 minHp = int32(GetTarget()->CountPctFromMaxHealth(
            talentProto->Effects[EFFECT_0].CalcValue(GetCaster())));

        // Damage that would take you below [effect0] health or taken while you
        // are at [effect0]
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_dk_will_of_the_necropolis::CalculateAmount,
            EFFECT_0,
            SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb += AuraEffectAbsorbFn(
            spell_dk_will_of_the_necropolis::Absorb, EFFECT_0);
    }
};
...
void AddSC_deathknight_spell_scripts()
{
    ...
    RegisterSpellAndAuraScriptPair(spell_dk_death_and_decay,
                                   spell_dk_death_and_decay_aura);
    RegisterSpellScript(spell_dk_master_of_ghouls);
    RegisterSpellAndAuraScriptPair(spell_dk_chains_of_ice,
                                   spell_dk_chains_of_ice_aura);
