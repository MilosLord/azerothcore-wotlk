...
/*
 * Scripts for spells with SPELLFAMILY_WARLOCK and SPELLFAMILY_GENERIC spells
 * used by warlock players. Ordered alphabetically using scriptname. Scriptnames
 * of files in this file should be prefixed with "spell_warl_".
 */
...
class spell_warl_eye_of_kilrogg : public AuraScript {
    ...
    void HandleAuraApply(AuraEffect const* /*aurEff*/,
            ...
            if (player->HasAura(58081)) {
                if (Unit* charm = player->GetCharm()) {
                    if (charm->GetMapId() == 530 || charm->GetMapId() == 571) {
                        charm->CastSpell(
                            charm, SPELL_WARLOCK_EYE_OF_KILROGG_FLY, true);
                    }
                }
            }
        }
    ...
    void HandleAuraRemove(AuraEffect const* /*aurEff*/,
                          AuraEffectHandleModes /*mode*/)
    {
        if (Player* player = GetTarget()->ToPlayer()) {
            if (Unit* charm = player->GetCharm())
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_warl_eye_of_kilrogg::HandleAuraApply,
                              EFFECT_1,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_warl_eye_of_kilrogg::HandleAuraRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_shadowflame : public SpellScript {
    ...
    void HandleSchoolDMG(SpellEffIndex /*effIndex*/)
    {
        if (Unit* target = GetHitUnit())
            GetCaster()->CastSpell(
                target, (GetSpellInfo()->Id == 47897 ? 47960 : 61291), true);
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warl_shadowflame::HandleSchoolDMG,
                          EFFECT_0,
...
class spell_warl_seduction : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_warl_seduction::HandleAuraApply,
                              EFFECT_0,
                              SPELL_AURA_MOD_STUN,
                              AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_improved_demonic_tactics : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (aurEff->GetEffIndex() == EFFECT_0)
            amount = CalculatePct<int32, float>(
                GetUnitOwner()->ToPlayer()->GetFloatValue(
                    PLAYER_SPELL_CRIT_PERCENTAGE1 +
                    static_cast<uint8>(SPELL_SCHOOL_FROST)),
    ...
    void Register() override
    {
        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_warl_improved_demonic_tactics::CalcPeriodic,
            EFFECT_ALL,
            SPELL_AURA_DUMMY);
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_warl_improved_demonic_tactics::CalculateAmount,
            EFFECT_ALL,
            SPELL_AURA_DUMMY);
        DoEffectCalcSpellMod += AuraEffectCalcSpellModFn(
            spell_warl_improved_demonic_tactics::HandleEffectCalcSpellMod,
            EFFECT_ALL,
            SPELL_AURA_DUMMY);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_warl_improved_demonic_tactics::HandlePeriodic,
            EFFECT_ALL,
            SPELL_AURA_DUMMY);
    }
};
...
class spell_warl_ritual_of_summoning : public SpellScript {
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_warl_ritual_of_summoning::CheckCast);
    }
};
...
class spell_warl_demonic_aegis : public AuraScript {
                         ...
                         AuraEffectHandleModes /*mode*/)
    {
        ...
        GetTarget()->RemoveAurasWithFamily(
            SPELLFAMILY_WARLOCK, 0, 0x20000020, 0, ObjectGuid::Empty);
    }

    void Register() override
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_demonic_aegis::HandleAuraApply,
                               EFFECT_0,
...
class spell_warl_demonic_knowledge : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (Unit* caster = GetCaster()) {
            ...
            amount    = CalculatePct(caster->GetStat(STAT_STAMINA) +
                                      caster->GetStat(STAT_INTELLECT),
                                  pct);
        }
    }
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_warl_demonic_knowledge::CalculateAmount,
            EFFECT_0,
            SPELL_AURA_MOD_DAMAGE_DONE);
        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_warl_demonic_knowledge::CalcPeriodic,
                                     EFFECT_0,
                                     SPELL_AURA_MOD_DAMAGE_DONE);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_warl_demonic_knowledge::HandlePeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_MOD_DAMAGE_DONE);
    }
};
...
class spell_warl_generic_scaling : public AuraScript {
                                   ...
                                   bool& /*canBeRecalculated*/)
    {
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            ...
            if (owner->HasAura(SPELL_WARLOCK_PET_VOID_STAR_TALISMAN) &&
    ...
    void CalculateStatAmount(AuraEffect const* aurEff,
                             ...
                             bool& /*canBeRecalculated*/)
    {
        // xinef: by default warlock pet inherits 75% of stamina and 30% of
        // intellect
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            Stats stat     = Stats(aurEff->GetSpellInfo()
                                   ->Effects[aurEff->GetEffIndex()]
                                   .MiscValue);
    ...
    void CalculateAPAmount(AuraEffect const* /*aurEff*/,
                           ...
                           bool& /*canBeRecalculated*/)
    {
        // xinef: by default warlock pet inherits 57% of max(SP FIRE, SP SHADOW)
        // as AP
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            int32 fire =
    ...
    void CalculateSPAmount(AuraEffect const* /*aurEff*/,
                           ...
                           bool& /*canBeRecalculated*/)
    {
        // xinef: by default warlock pet inherits 15% of max(SP FIRE, SP SHADOW)
        // as SP
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            int32 fire =
                owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE);
    ...
    void Register() override
    {
        if (m_scriptSpellId != 34947)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_generic_scaling::CalculateResistanceAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_RESISTANCE);

        if (m_scriptSpellId == 34947 || m_scriptSpellId == 34956)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_generic_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);

        if (m_scriptSpellId == 34947) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_generic_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_generic_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_warl_generic_scaling::CalcPeriodic,
                                     EFFECT_ALL,
                                     SPELL_AURA_ANY);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_warl_generic_scaling::HandlePeriodic,
                                 EFFECT_ALL,
                                 SPELL_AURA_ANY);
    }
};
...
class spell_warl_infernal_scaling : public AuraScript {
    ...
    void Register() override
    {
        if (m_scriptSpellId != 36186)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_infernal_scaling::CalculateResistanceAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_RESISTANCE);

        if (m_scriptSpellId == 36186 || m_scriptSpellId == 36188)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_infernal_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);

        if (m_scriptSpellId == 36186) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_infernal_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_warl_infernal_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        OnEffectApply +=
            AuraEffectApplyFn(spell_warl_infernal_scaling::HandleEffectApply,
                              EFFECT_ALL,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_banish : public SpellScript {
    ...
    void HandleBanish(SpellMissInfo missInfo)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            if (Aura* banishAura = target->GetAura(GetSpellInfo()->Id,
                                                   GetCaster()->GetGUID())) {
                banishAura->Remove();
            }
        }
...
class spell_warl_demonic_empowerment : public SpellScript {
        ...
        if (Creature* targetCreature = GetHitCreature()) {
            if (targetCreature->IsPet()) {
                CreatureTemplate const* ci =
                    sObjectMgr->GetCreatureTemplate(targetCreature->GetEntry());
                switch (ci->family) {
                case CREATURE_FAMILY_SUCCUBUS:
                    targetCreature->CastSpell(
                        targetCreature,
                        SPELL_WARLOCK_DEMONIC_EMPOWERMENT_SUCCUBUS,
                        true);
                    break;
                case CREATURE_FAMILY_VOIDWALKER: {
                    SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(
                        SPELL_WARLOCK_DEMONIC_EMPOWERMENT_VOIDWALKER);
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warl_demonic_empowerment::HandleScriptEffect,
                          EFFECT_0,
...
class spell_warl_create_healthstone : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_WARLOCK_IMPROVED_HEALTHSTONE_R1,
    ...
    SpellCastResult CheckCast()
    {
        if (Player* caster = GetCaster()->ToPlayer()) {
            ...
            InventoryResult msg =
                caster->CanStoreNewItem(NULL_BAG,
                                        NULL_SLOT,
                                        dest,
                                        iTypes[spellRank - 1][0],
                                        1,
                                        nullptr);
            if (msg != EQUIP_ERR_OK)
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        if (Unit* unitTarget = GetHitUnit()) {
            ...
            if (AuraEffect const* aurEff = unitTarget->GetDummyAuraEffect(
                    SPELLFAMILY_WARLOCK, 284, 0)) {
                switch (aurEff->GetId()) {
                ...
                default:
                    LOG_ERROR("spells",
                              "Unknown rank of Improved Healthstone id: {}",
                              aurEff->GetId());
                    break;
                }
            }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warl_create_healthstone::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
        OnCheckCast +=
            SpellCheckCastFn(spell_warl_create_healthstone::CheckCast);
    }
};

uint32 const
    spell_warl_create_healthstone::spell_warl_create_healthstone::iTypes[8][3] =
        {
...
class spell_warl_everlasting_affliction : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Unit* unitTarget = GetHitUnit())
            ...
            if (AuraEffect* aur =
                    unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                              SPELLFAMILY_WARLOCK,
                                              0x2,
                                              0,
                                              0,
                ...
                aur->ChangeAmount(aur->CalculateAmount(aur->GetCaster()),
                                  false);
            }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warl_everlasting_affliction::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_warl_ritual_of_doom_effect : public SpellScript {
    ...
    {
        OnEffectHit +=
            SpellEffectFn(spell_warl_ritual_of_doom_effect::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warl_seed_of_corruption : public SpellScript {
        ...
        targets.remove_if([&](WorldObject const* target) {
            if (Unit const* unitTarget = target->ToUnit()) {
                if (WorldLocation const* dest = GetExplTargetDest()) {
                    if (!unitTarget->IsWithinLOS(dest->GetPositionX(),
                                                 dest->GetPositionY(),
                                                 dest->GetPositionZ())) {
                        return true;
                    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_warl_seed_of_corruption::FilterTargets,
            EFFECT_0,
...
class spell_warl_soulshatter : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            if (target->CanHaveThreatList() &&
                target->GetThreatMgr().GetThreat(caster) > 0.0f)
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_warl_soulshatter::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warl_siphon_life : public AuraScript {
    ...
    void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 amount = CalculatePct(
            static_cast<int32>(eventInfo.GetDamageInfo()->GetDamage()),
            aurEff->GetAmount());
        // Glyph of Siphon Life
        if (AuraEffect const* glyph = GetTarget()->GetAuraEffect(
                SPELL_WARLOCK_GLYPH_OF_SIPHON_LIFE, EFFECT_0))
            AddPct(amount, glyph->GetAmount());

        GetTarget()->CastCustomSpell(SPELL_WARLOCK_SIPHON_LIFE_HEAL,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_warl_siphon_life::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_warl_life_tap : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            int32 mana = int32(spellEffect + (caster->SpellBaseDamageBonusDone(
            ...
            if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(
                    SPELLFAMILY_WARLOCK, WARLOCK_ICON_ID_IMPROVED_LIFE_TAP, 0))
            ...
            if (AuraEffect const* aurEff =
                    caster->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER,
                                          SPELLFAMILY_WARLOCK,
    ...
    SpellCastResult CheckCast()
    {
        if ((int32(GetCaster()->GetHealth()) >
             int32(GetSpellInfo()->Effects[EFFECT_0].CalcValue())))
            return SPELL_CAST_OK;
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_warl_life_tap::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
...
class spell_warl_demonic_circle_summon : public AuraScript {
    ...
    {
        // If effect is removed by expire remove the summoned demonic circle
        // too.
    ...
    void HandleDummyTick(AuraEffect const* /*aurEff*/)
    {
        if (GameObject* circle = GetTarget()->GetGameObject(GetId())) {
            // Here we check if player is in demonic circle teleport range, if
            // so add WARLOCK_DEMONIC_CIRCLE_ALLOW_CAST; allowing him to cast
            // the WARLOCK_DEMONIC_CIRCLE_TELEPORT. If not in range remove the
            // WARLOCK_DEMONIC_CIRCLE_ALLOW_CAST.

            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(
                SPELL_WARLOCK_DEMONIC_CIRCLE_TELEPORT);

            if (GetTarget()->IsWithinDist(circle,
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_demonic_circle_summon::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_DUMMY,
                               AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_warl_demonic_circle_summon::HandleDummyTick,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_warl_demonic_circle_teleport : public AuraScript {
    ...
    void HandleTeleport(AuraEffect const* /*aurEff*/,
                        AuraEffectHandleModes /*mode*/)
    {
        if (Player* player = GetTarget()->ToPlayer()) {
            if (GameObject* circle = player->GetGameObject(
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_warl_demonic_circle_teleport::HandleTeleport,
            EFFECT_0,
...
class spell_warl_fel_synergy : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        return (GetTarget()->GetGuardianPet() || GetTarget()->GetCharm()) &&
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_warl_fel_synergy::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
    ...
    void HandleAfterHit()
    {
        if (Aura* aura = GetHitAura())
            if (AuraEffect* aurEff = aura->GetEffect(EFFECT_1))
                aurEff->SetAmount(
                    CalculatePct(aurEff->GetAmount(), GetHitDamage()));
    }
...
class spell_warl_haunt_aura : public AuraScript {
    ...
    void HandleRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster()) {
            ...
            GetTarget()->CastCustomSpell(caster,
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_haunt_aura::HandleRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
...
class spell_warl_unstable_affliction : public AuraScript {
    ...
    void HandleDispel(DispelInfo* dispelInfo)
    {
        if (Unit* caster = GetCaster())
            if (AuraEffect const* aurEff = GetEffect(EFFECT_0)) {
                ...
                damage = aurEff->GetSpellInfo()->Effects[EFFECT_0].CalcValue(
                             caster, &damage, nullptr) *
                         9;
    ...
    {
        AfterDispel +=
            AuraDispelFn(spell_warl_unstable_affliction::HandleDispel);
    }
};
...
class spell_warl_curse_of_doom : public AuraScript {
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_warl_curse_of_doom::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_DAMAGE,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_health_funnel : public AuraScript {
    ...
    void ApplyEffect(AuraEffect const* /*aurEff*/,
                     AuraEffectHandleModes /*mode*/)
    {
        Unit* caster = GetCaster();
        if (!caster)
        ...
        if (caster->HasAura(SPELL_WARLOCK_IMPROVED_HEALTH_FUNNEL_R2))
            target->CastSpell(
                target, SPELL_WARLOCK_IMPROVED_HEALTH_FUNNEL_BUFF_R2, true);
        else if (caster->HasAura(SPELL_WARLOCK_IMPROVED_HEALTH_FUNNEL_R1))
            target->CastSpell(
                target, SPELL_WARLOCK_IMPROVED_HEALTH_FUNNEL_BUFF_R1, true);
    }

    void RemoveEffect(AuraEffect const* /*aurEff*/,
                      AuraEffectHandleModes /*mode*/)
    {
        Unit* target = GetTarget();
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_health_funnel::RemoveEffect,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_HEAL,
                               AURA_EFFECT_HANDLE_REAL);
        OnEffectApply +=
            AuraEffectApplyFn(spell_warl_health_funnel::ApplyEffect,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_HEAL,
                              AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_shadow_ward : public AuraScript {
                         ...
                         bool&  canBeRecalculated)
    {
        ...
        if (Unit* caster = GetCaster()) {
            ...
            bonus *= caster->SpellBaseDamageBonusDone(
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_warl_shadow_ward::CalculateAmount,
                                   EFFECT_0,
...
class spell_warl_glyph_of_shadowflame : public AuraScript {
    ...
    {
        OnEffectProc +=
            AuraEffectProcFn(spell_warl_glyph_of_shadowflame::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_warl_drain_soul : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        // Drain Soul's proc tries to happen each time the warlock lands a
        // killing blow on a unit while channeling. Make sure that the dying
        // unit is afflicted by the caster's Drain Soul debuff in order to avoid
        // a false positive.
        ...
        if (caster && victim) {
            return victim->GetAuraApplicationOfRankedSpell(
                       SPELL_WARLOCK_DRAIN_SOUL_R1, caster->GetGUID()) != 0;
        }
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (Unit* caster = eventInfo.GetActor()) {
            // Improved Drain Soul.
            if (Aura const* impDrainSoul = caster->GetAuraOfRankedSpell(
                    SPELL_WARLOCK_IMPROVED_DRAIN_SOUL_R1, caster->GetGUID())) {
                int32 amount = CalculatePct(caster->GetMaxPower(POWER_MANA),
                                            impDrainSoul->GetSpellInfo()
                                                ->Effects[EFFECT_2]
                                                .CalcValue());
    ...
    void HandleTick(AuraEffect const* aurEff)
    {
        ...
        if (caster && caster->GetTypeId() == TYPEID_PLAYER &&
            ...
            if (roll_chance_i(20)) {
                caster->CastSpell(
                    caster, SPELL_WARLOCK_CREATE_SOULSHARD, aurEff);
                // Glyph of Drain Soul - chance to create an additional Soul
                // Shard.
                if (AuraEffect* aur = caster->GetAuraEffect(
                        SPELL_WARLOCK_GLYPH_OF_DRAIN_SOUL_AURA, EFFECT_0)) {
                    if (roll_chance_i(aur->GetMiscValue())) {
                        caster->CastSpell(
                            caster, SPELL_WARLOCK_CREATE_SOULSHARD, aur);
                    }
                }
            }
        }
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_drain_soul::RemoveEffect,
                               EFFECT_0,
                               SPELL_AURA_CHANNEL_DEATH_ITEM,
                               AURA_EFFECT_HANDLE_REAL);
        DoCheckProc += AuraCheckProcFn(spell_warl_drain_soul::CheckProc);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_warl_drain_soul::HandleTick,
                                 EFFECT_1,
                                 SPELL_AURA_PERIODIC_DAMAGE);
        OnEffectProc += AuraEffectProcFn(spell_warl_drain_soul::HandleProc,
                                         EFFECT_2,
...
class spell_warl_shadowburn : public AuraScript {
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_shadowburn::RemoveEffect,
                               EFFECT_0,
                               SPELL_AURA_CHANNEL_DEATH_ITEM,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_glyph_of_felguard : public AuraScript {
        ...
        if (Player* player = GetCaster()->ToPlayer()) {
            if (Pet* pet = player->GetPet()) {
                if (pet->GetEntry() == NPC_FELGUARD) {
                    pet->HandleStatModifier(UNIT_MOD_ATTACK_POWER,
                                            TOTAL_PCT,
                                            aurEff->GetAmount(),
                                            true);
                }
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_warl_glyph_of_felguard::HandleApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_glyph_of_felguard::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_warl_glyph_of_voidwalker : public AuraScript {
        ...
        if (Player* player = GetCaster()->ToPlayer()) {
            if (Pet* pet = player->GetPet()) {
                if (pet->GetEntry() == NPC_VOIDWALKER) {
                    pet->HandleStatModifier(UNIT_MOD_STAT_STAMINA,
                                            TOTAL_PCT,
                                            aurEff->GetAmount(),
                                            true);
                }
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_warl_glyph_of_voidwalker::HandleApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warl_glyph_of_voidwalker::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
