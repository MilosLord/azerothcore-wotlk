...
#include "Unit.h"
/*
 * Scripts for spells with SPELLFAMILY_SHAMAN and SPELLFAMILY_GENERIC spells
 * used by shaman players. Ordered alphabetically using scriptname. Scriptnames
 * of files in this file should be prefixed with "spell_sha_".
 */
...
class spell_sha_totem_of_wrath : public SpellScript {
    ...
    void HandleAfterCast()
    {
        if (AuraEffect* aurEff = GetCaster()->GetAuraEffect(63280, EFFECT_0))
            if (Creature* totem = GetCaster()->GetMap()->GetCreature(
                    GetCaster()->m_SummonSlot[1])) // Fire totem summon slot
                if (SpellInfo const* totemSpell =
                        sSpellMgr->GetSpellInfo(totem->m_spells[0])) {
                    int32 bp0 =
                        CalculatePct(totemSpell->Effects[EFFECT_0].CalcValue(),
                                     aurEff->GetAmount());
                    int32 bp1 =
                        CalculatePct(totemSpell->Effects[EFFECT_1].CalcValue(),
                                     aurEff->GetAmount());
...
class spell_sha_spirit_walk : public SpellScript {
    ...
    SpellCastResult CheckCast()
    {
        if (Unit* owner = GetCaster()->GetOwner())
            if (GetCaster()->IsWithinDist(
                    owner,
                    GetSpellInfo()->GetMaxRange(GetSpellInfo()->IsPositive())))
...
class spell_sha_t10_restoration_4p_bonus : public AuraScript {
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_sha_t10_restoration_4p_bonus::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_sha_t10_restoration_4p_bonus::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_sha_totemic_mastery : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_sha_totemic_mastery::HandlePeriodic,
                                 EFFECT_0,
...
class spell_sha_feral_spirit_scaling : public AuraScript {
    ...
    {
        // xinef: feral spirit inherits 40% of resistance from owner and 35% of
        // armor
                           ...
                           bool& /*canBeRecalculated*/)
    {
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            int32 modifier = 30;
            if (AuraEffect const* gofsEff = owner->GetAuraEffect(
                    SPELL_SHAMAN_GLYPH_OF_FERAL_SPIRIT, EFFECT_0))
    ...
    void Register() override
    {
        if (m_scriptSpellId == 35675)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_feral_spirit_scaling::CalculateResistanceAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_RESISTANCE);

        if (m_scriptSpellId == 35674) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_feral_spirit_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_feral_spirit_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_feral_spirit_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        OnEffectApply +=
            AuraEffectApplyFn(spell_sha_feral_spirit_scaling::HandleEffectApply,
                              EFFECT_ALL,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_sha_feral_spirit_scaling::CalcPeriodic,
            EFFECT_ALL,
            SPELL_AURA_ANY);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_sha_feral_spirit_scaling::HandlePeriodic,
                                 EFFECT_ALL,
                                 SPELL_AURA_ANY);
    }
};
...
class spell_sha_fire_elemental_scaling : public AuraScript {
    ...
    void CalculateResistanceAmount(AuraEffect const* aurEff,
                                   ...
                                   bool& /*canBeRecalculated*/)
    {
        // xinef: fire elemental inherits 40% of resistance from owner and 35%
        // of armor
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            SpellSchoolMask schoolMask =
                SpellSchoolMask(aurEff->GetSpellInfo()
                           ...
                           bool& /*canBeRecalculated*/)
    {
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            ...
            amount = CalculatePct(
                std::max<int32>(0, fire),
                (GetUnitOwner()->GetEntry() == NPC_FIRE_ELEMENTAL ? 300 : 150));
        }
    }
    ...
    void Register() override
    {
        ...
        if (m_scriptSpellId == 35666 || m_scriptSpellId == 65226)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_fire_elemental_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);

        if (m_scriptSpellId == 35665 || m_scriptSpellId == 65225) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_fire_elemental_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_sha_fire_elemental_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        OnEffectApply += AuraEffectApplyFn(
            spell_sha_fire_elemental_scaling::HandleEffectApply,
            EFFECT_ALL,
            SPELL_AURA_ANY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_sha_ancestral_awakening_proc : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (GetHitUnit())
            GetCaster()->CastCustomSpell(GetHitUnit(),
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_sha_ancestral_awakening_proc::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_CASTER_AREA_RAID);
        OnEffectHitTarget +=
            SpellEffectFn(spell_sha_ancestral_awakening_proc::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_astral_shift : public AuraScript {
    ...
    Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (GetTarget()->GetUnitFlags() &
                (UNIT_FLAG_FLEEING | UNIT_FLAG_SILENCED) ||
            (GetTarget()->GetUnitFlags() & (UNIT_FLAG_STUNNED) &&
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_sha_astral_shift::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_sha_astral_shift::Absorb, EFFECT_0);
    }
};
...
class spell_sha_bloodlust : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_sha_bloodlust::RemoveInvalidTargets,
            EFFECT_ALL,
            TARGET_UNIT_CASTER_AREA_RAID);
...
class spell_sha_chain_heal : public SpellScript {
    ...
    void HandleHeal(SpellEffIndex /*effIndex*/)
    {
        if (firstHeal) {
            ...
            if (AuraEffect* aurEff =
                    GetHitUnit()->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL,
                                                SPELLFAMILY_SHAMAN,
                                                0,
                                                0,
                                                0x10,
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_sha_chain_heal::HandleHeal, EFFECT_0, SPELL_EFFECT_HEAL);
    }
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (GetCaster() && GetHitUnit() && GetOriginalCaster())
            GetCaster()->CastCustomSpell(GetHitUnit(),
                                         ...
                                         nullptr,
                                         nullptr,
                                         GetOriginalCaster()->GetGUID());
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_sha_cleansing_totem_pulse::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_earth_shield : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (Unit* caster = GetCaster()) {
            ...
            amount           = caster->SpellHealingBonusDone(GetUnitOwner(),
                                                   GetSpellInfo(),
                                                   amount,
                                                   HEAL,
                                                   aurEff->GetEffIndex());
            // xinef: taken should be calculated at every heal
            // amount = GetUnitOwner()->SpellHealingBonusTaken(caster,
            // GetSpellInfo(), amount, HEAL);

            // Glyph of Earth Shield
            //! WORKAROUND
            //! this glyphe is a proc
            if (AuraEffect* glyphe = caster->GetAuraEffect(
    ...
    {
        PreventDefaultAction();
        GetTarget()->CastCustomSpell(SPELL_SHAMAN_EARTH_SHIELD_HEAL,
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_sha_earth_shield::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_DUMMY);
        DoCheckProc += AuraCheckProcFn(spell_sha_earth_shield::CheckProc);
        OnEffectProc += AuraEffectProcFn(
            spell_sha_earth_shield::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_sha_earthbind_totem : public AuraScript {
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Player* owner =
            ...
            if (AuraEffect* aur =
                    owner->GetDummyAuraEffect(SPELLFAMILY_SHAMAN, 2289, 0))
                if (roll_chance_i(aur->GetBaseAmount()))
                    GetTarget()->CastSpell(
                        (Unit*)nullptr, SPELL_SHAMAN_TOTEM_EARTHEN_POWER, true);
    }
    ...
    void Apply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (AuraEffect* aurEff = owner->GetAuraEffectOfRankedSpell(
                SPELL_SHAMAN_STORM_EARTH_AND_FIRE, EFFECT_1)) {
            if (roll_chance_i(aurEff->GetAmount()))
                GetCaster()->CastSpell(
                    GetCaster(), SPELL_SHAMAN_TOTEM_EARTHBIND_EARTHGRAB, false);
        }
    }
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_sha_earthbind_totem::HandleEffectPeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        OnEffectApply += AuraEffectApplyFn(spell_sha_earthbind_totem::Apply,
                                           EFFECT_0,
                                           SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                           AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_sha_earthen_power : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_sha_earthen_power::FilterTargets,
            EFFECT_0,
...
class spell_sha_earthliving_weapon : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (AuraEffect const* aurEff = caster->GetAuraEffectOfRankedSpell(
                SPELL_SHAMAN_BLESSING_OF_THE_ETERNALS_R1,
                EFFECT_1,
                caster->GetGUID())) {
            if (eventInfo.GetProcTarget()->HasAuraState(
...
class spell_sha_fire_nova : public SpellScript {
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        SpellInfo const* firstRankSpellInfo =
            sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_NOVA_R1);
        if (!firstRankSpellInfo || !spellInfo->IsRankOf(firstRankSpellInfo))
            return false;

        uint8 rank = spellInfo->GetRank();
        if (!sSpellMgr->GetSpellWithRank(
                SPELL_SHAMAN_FIRE_NOVA_TRIGGERED_R1, rank, true))
            return false;
        return true;
    }
    ...
    SpellCastResult CheckFireTotem()
    {
        ...
        if (Creature* totem =
                caster->GetMap()->GetCreature(caster->m_SummonSlot[1])) {
            if (!caster->IsWithinDistInMap(
                    totem,
                    caster->GetSpellMaxRangeForTarget(totem, GetSpellInfo())))
                return SPELL_FAILED_OUT_OF_RANGE;
            return SPELL_CAST_OK;
        }
        else {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Creature* totem =
            ...
            if (totem->IsTotem())
                caster->CastSpell(
                    totem,
                    sSpellMgr->GetSpellWithRank(
                        SPELL_SHAMAN_FIRE_NOVA_TRIGGERED_R1, rank),
                    true);
        }
    }
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_sha_fire_nova::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_flame_shock : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spell*/) override
    {
        return ValidateSpellInfo(
            {SPELL_SHAMAN_LAVA_FLOWS_R1, SPELL_SHAMAN_LAVA_FLOWS_TRIGGERED_R1});
    }
    ...
    void HandleDispel(DispelInfo* /*dispelInfo*/)
    {
        if (Unit* caster = GetCaster())
            ...
            if (AuraEffect const* aurEff =
                    caster->GetDummyAuraEffect(SPELLFAMILY_SHAMAN,
                                               SHAMAN_ICON_ID_SHAMAN_LAVA_FLOW,
                                               EFFECT_0)) {
                if (SpellInfo const* firstRankSpellInfo =
                        sSpellMgr->GetSpellInfo(SPELL_SHAMAN_LAVA_FLOWS_R1))
                ...
                caster->CastSpell(
                    caster,
                    sSpellMgr->GetSpellWithRank(
                        SPELL_SHAMAN_LAVA_FLOWS_TRIGGERED_R1, rank),
                    true);
            }
    }
...
// 52041, 52046, 52047, 52048, 52049, 52050, 58759, 58760, 58761 - Healing
// Stream Totem
class spell_sha_healing_stream_totem : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_SHAMAN_GLYPH_OF_HEALING_STREAM_TOTEM,
                ...
                if (Unit* owner = caster->GetOwner()) {
                    if (triggeringSpell)
                        damage = int32(owner->SpellHealingBonusDone(
                            target, triggeringSpell, damage, HEAL, effIndex));

                    // Restorative Totems
                    if (AuraEffect* dummy = owner->GetAuraEffect(
                            SPELL_AURA_DUMMY,
                            SPELLFAMILY_SHAMAN,
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_sha_healing_stream_totem::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_heroism : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_sha_heroism::RemoveInvalidTargets,
            EFFECT_0,
            TARGET_UNIT_CASTER_AREA_RAID);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_sha_heroism::RemoveInvalidTargets,
...
class spell_sha_item_lightning_shield : public AuraScript {
    ...
    {
        OnEffectProc +=
            AuraEffectProcFn(spell_sha_item_lightning_shield::HandleProc,
                             EFFECT_0,
...
class spell_sha_item_lightning_shield_trigger : public AuraScript {
    ...
    {
        OnEffectProc += AuraEffectProcFn(
            spell_sha_item_lightning_shield_trigger::HandleProc,
            EFFECT_0,
            SPELL_AURA_PROC_TRIGGER_SPELL);
    }
};
...
class spell_sha_item_mana_surge : public AuraScript {
    ...
    {
        PreventDefaultAction();
        int32 mana = eventInfo.GetSpellInfo()->CalcPowerCost(
            GetTarget(), eventInfo.GetSchoolMask());
        int32 damage = CalculatePct(mana, 35);

        GetTarget()->CastCustomSpell(SPELL_SHAMAN_ITEM_MANA_SURGE,
                                     SPELLVALUE_BASE_POINT0,
                                     damage,
                                     GetTarget(),
                                     true,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(spell_sha_item_mana_surge::HandleProc,
                                         EFFECT_0,
                                         SPELL_AURA_PROC_TRIGGER_SPELL);
    }
};
...
class spell_sha_item_t10_elemental_2p_bonus : public AuraScript {
                          ...
                          ProcEventInfo& /*eventInfo*/)
    {
        ...
        if (Player* target = GetTarget()->ToPlayer())
            target->ModifySpellCooldown(SPELL_SHAMAN_ELEMENTAL_MASTERY,
                                        -aurEff->GetAmount());
    }
    ...
    {
        OnEffectProc += AuraEffectProcFn(
            spell_sha_item_t10_elemental_2p_bonus::HandleEffectProc,
            EFFECT_0,
            SPELL_AURA_DUMMY);
    }
};
...
class spell_sha_lava_lash : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        if (Player* caster = GetCaster()->ToPlayer()) {
            ...
            if (caster->GetItemByPos(INVENTORY_SLOT_BAG_0,
                                     EQUIPMENT_SLOT_OFFHAND)) {
                // Damage is increased by 25% if your off-hand weapon is
                // enchanted with Flametongue.
                if (caster->GetAuraEffect(
                        SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, 0x200000, 0, 0))
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_sha_lava_lash::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_mana_spring_totem : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_sha_mana_spring_totem::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_mana_tide_totem : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_SHAMAN_GLYPH_OF_MANA_TIDE, SPELL_SHAMAN_MANA_TIDE_TOTEM});
    }
        ...
        if (Unit* caster = GetCaster())
            if (Unit* unitTarget = GetHitUnit()) {
                if (unitTarget->HasActivePowerType(POWER_MANA)) {
                    ...
                    if (Unit* owner = caster->GetOwner())
                        if (AuraEffect* dummy = owner->GetAuraEffect(
                                SPELL_SHAMAN_GLYPH_OF_MANA_TIDE, 0))
                            effValue += dummy->GetAmount();
                    // Regenerate 6% of Total Mana Every 3 secs
                    int32 effBasePoints0 = int32(CalculatePct(
                        unitTarget->GetMaxPower(POWER_MANA), effValue));
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_sha_mana_tide_totem::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_sha_sentry_totem : public AuraScript {
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_sha_sentry_totem::AfterRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_sha_thunderstorm : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_sha_thunderstorm::HandleKnockBack,
                          EFFECT_2,
...
class spell_sha_flurry_proc : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (SpellInfo const* spellInfo = eventInfo.GetSpellInfo()) {
            constexpr std::array<uint32, 2> spellIcons = {
                SPELL_SHAMAN_STORMSTRIKE, SPELL_SHAMAN_LAVA_LASH};
            const auto found = std::find(
                std::begin(spellIcons), std::end(spellIcons), spellInfo->Id);

            if ((spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN &&
                 (spellInfo->SpellFamilyFlags[0] & 0x00800000) != 0) ||
                found != std::end(spellIcons)) {
                return false;
            }
        }
...
class spell_sha_t8_electrified : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* electrifiedDot =
            sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_ELECTRIFIED);
        int32 amount =
            int32(CalculatePct(eventInfo.GetDamageInfo()->GetDamage(),
                               aurEff->GetAmount()) /
                  electrifiedDot->GetMaxTicks());
    ...
    {
        OnEffectProc += AuraEffectProcFn(
            spell_sha_t8_electrified::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
