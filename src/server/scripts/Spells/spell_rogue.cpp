...
/*
 * Scripts for spells with SPELLFAMILY_ROGUE and SPELLFAMILY_GENERIC spells used
 * by rogue players. Ordered alphabetically using scriptname. Scriptnames of
 * files in this file should be prefixed with "spell_rog_".
 */
...
class spell_rog_savage_combat : public AuraScript {
    ...
    void Register() override
    {
        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_rog_savage_combat::CalcPeriodic,
                                     EFFECT_0,
                                     SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
        OnEffectUpdatePeriodic +=
            AuraEffectUpdatePeriodicFn(spell_rog_savage_combat::Update,
                                       EFFECT_0,
                                       SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
    }
};
...
class spell_rog_blade_flurry : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        Unit* _procTarget = eventInfo.GetActor()->SelectNearbyNoTotemTarget(
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        // Unit::CalculateAOEDamageReduction (this=0x0, damage=4118,
        // schoolMask=1, caster=0x7ffdad089000)
        Unit* procTarget =
            ObjectAccessor::GetUnit(*GetTarget(), _procTargetGUID);
        ...
        if (procTarget && damageInfo) {
            ...
            values.AddSpellMod(
                ...
                int32(eventInfo.GetHitMask() & PROC_EX_CRITICAL_HIT));
            GetTarget()->CastCustomSpell(SPELL_ROGUE_BLADE_FLURRY_EXTRA_ATTACK,
                                         values,
                                         procTarget,
                                         TRIGGERED_FULL_MASK,
                                         nullptr,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(spell_rog_blade_flurry::HandleProc,
                                         EFFECT_0,
...
class spell_rog_cheat_death : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_ROGUE_CHEAT_DEATH_COOLDOWN, SPELL_ROGUE_CHEATING_DEATH});
    }
    ...
    Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (dmgInfo.GetDamage() < target->GetHealth() ||
            target->HasSpellCooldown(SPELL_ROGUE_CHEAT_DEATH_COOLDOWN) ||
        ...
        target->AddSpellCooldown(
            SPELL_ROGUE_CHEAT_DEATH_COOLDOWN, 0, MINUTE * IN_MILLISECONDS);
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_rog_cheat_death::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_rog_cheat_death::Absorb, EFFECT_0);
    }
};
            ...
            for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot) {
                ...
                for (uint8 s = 0; s < 3; ++s) {
                    ...
                    SpellInfo const* spellInfo =
                        sSpellMgr->GetSpellInfo(enchant->spellid[s]);
                    if (!spellInfo) {
                        LOG_ERROR("misc",
                                  "Player::CastItemCombatSpell Enchant {}, "
                                  "player (Name: {}, {}) cast unknown spell {}",
                                  enchant->ID,
                                  player->GetName(),
                    ...
                    if (spellInfo->SpellFamilyFlags.IsEqual(
                            0x10000, 0x80000, 0))
                        continue;

                    if (spellInfo->IsPositive())
                        player->CastSpell(
                            player, enchant->spellid[s], true, item);
                    else
                        player->CastSpell(
                            target, enchant->spellid[s], true, item);
                }
            }
        }
    }
...
public:
    ...
    class spell_rog_killing_spree_SpellScript : public SpellScript {
        ...
        SpellCastResult CheckCast()
        {
            ...
            if (GetCaster()->GetMapId() == 603 /*Ulduar*/ &&
                GetCaster()->GetDistance2d(1766.936f, -24.748f) < 50.0f)
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (targets.empty() || GetCaster()->GetVehicleBase() ||
                GetCaster()->HasUnitState(UNIT_STATE_ROOT))
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Aura* aura = GetCaster()->GetAura(SPELL_ROGUE_KILLING_SPREE)) {
                if (spell_rog_killing_spree_AuraScript* script =
                        dynamic_cast<spell_rog_killing_spree_AuraScript*>(
                            aura->GetScriptByName(KillingSpreeScriptName)))
        ...
        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(
                spell_rog_killing_spree_SpellScript::CheckCast);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_rog_killing_spree_SpellScript::FilterTargets,
                EFFECT_1,
                TARGET_UNIT_DEST_AREA_ENEMY);
            OnEffectHitTarget +=
                SpellEffectFn(spell_rog_killing_spree_SpellScript::HandleDummy,
                              EFFECT_1,
                              SPELL_EFFECT_DUMMY);
        }
    };
    ...
    class spell_rog_killing_spree_AuraScript : public AuraScript {
        ...
        {
            GetTarget()->CastSpell(
                GetTarget(), SPELL_ROGUE_KILLING_SPREE_DMG_BUFF, true);
        }
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            while (!_targets.empty()) {
                ObjectGuid guid =
                    Acore::Containers::SelectRandomContainerElement(_targets);
                if (Unit* target =
                        ObjectAccessor::GetUnit(*GetTarget(), guid)) {
                    ...
                    GetTarget()->CastSpell(
                        target, SPELL_ROGUE_KILLING_SPREE_TELEPORT, true);

                    // xinef: ensure fast coordinates switch, dont wait for
                    // client to send opcode
                    WorldLocation const& dest =
                        GetTarget()->ToPlayer()->GetTeleportDest();
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(
                spell_rog_killing_spree_AuraScript::HandleApply,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_rog_killing_spree_AuraScript::HandleEffectPeriodic,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY);
            AfterEffectRemove += AuraEffectRemoveFn(
                spell_rog_killing_spree_AuraScript::HandleRemove,
                EFFECT_0,
                SPELL_AURA_PERIODIC_DUMMY,
                AURA_EFFECT_HANDLE_REAL);
        }
    ...
    public:
        void AddTarget(Unit* target) { _targets.push_back(target->GetGUID()); }
...
class spell_rog_nerves_of_steel : public AuraScript {
    ...
    Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (GetTarget()->GetUnitFlags() & (UNIT_FLAG_FLEEING) ||
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_rog_nerves_of_steel::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_rog_nerves_of_steel::Absorb, EFFECT_0);
    }
};
...
class spell_rog_preparation : public SpellScript {
        ...
        for (PlayerSpellMap::const_iterator itr = spellMap.begin();
            ...
            if (spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE) {
                if (spellInfo->SpellFamilyFlags[1] &
                        SPELLFAMILYFLAG1_ROGUE_COLDB_SHADOWSTEP || // Cold
                                                                   // Blood,
                                                                   // Shadowstep
                    spellInfo->SpellFamilyFlags[0] &
                        SPELLFAMILYFLAG_ROGUE_VAN_EVAS_SPRINT) // Vanish,
                                                               // Evasion,
                                                               // Sprint
                {
                    ...
                    if (spellInfo->SpellFamilyFlags[1] &
                        ...
                        (spellInfo->SpellFamilyFlags[0] &
                             SPELLFAMILYFLAG_ROGUE_BLADE_FLURRY && // Blade
                                                                   // Flurry
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_rog_preparation::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_rog_prey_on_the_weak : public AuraScript {
        ...
        if (victim && (target->GetHealthPct() > victim->GetHealthPct())) {
            if (!target->HasAura(SPELL_ROGUE_PREY_ON_THE_WEAK)) {
                ...
                target->CastCustomSpell(
                    target, SPELL_ROGUE_PREY_ON_THE_WEAK, &bp, 0, 0, true);
            }
        }
        else
    ...
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_rog_prey_on_the_weak::HandleEffectPeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_rog_rupture : public AuraScript {
                         ...
                         bool&  canBeRecalculated)
    {
        if (Unit* caster = GetCaster()) {
            ...
            float const attackpowerPerCombo[6] = {
                0.0f,
                0.015f, // 1 point:  ${($m1 + $b1*1 + 0.015 * $AP) * 4} damage
                        // over 8 secs
                0.024f, // 2 points: ${($m1 + $b1*2 + 0.024 * $AP) * 5} damage
                        // over 10 secs
                0.03f,  // 3 points: ${($m1 + $b1*3 + 0.03 * $AP) * 6} damage
                        // over 12 secs
                0.03428571f, // 4 points: ${($m1 + $b1*4 + 0.03428571 * $AP) *
                             // 7} damage over 14 secs
                0.0375f // 5 points: ${($m1 + $b1*5 + 0.0375 * $AP) * 8} damage
                        // over 16 secs
            };
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_rog_rupture::CalculateAmount,
                                   EFFECT_0,
...
class spell_rog_shiv : public SpellScript {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_rog_shiv::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_rog_tricks_of_the_trade : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_ROGUE_TRICKS_OF_THE_TRADE_DMG_BOOST,
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& /*eventInfo*/)
    {
        ...
        Unit* target = GetTarget();
        target->CastSpell(
            _redirectTarget, SPELL_ROGUE_TRICKS_OF_THE_TRADE_DMG_BOOST, true);
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_rog_tricks_of_the_trade::OnRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
        DoCheckProc +=
            AuraCheckProcFn(spell_rog_tricks_of_the_trade::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_rog_tricks_of_the_trade::HandleProc,
                             EFFECT_1,
                             SPELL_AURA_DUMMY);
    }
...
class spell_rog_tricks_of_the_trade_proc : public AuraScript {
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_rog_tricks_of_the_trade_proc::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_rog_pickpocket : public SpellScript {
    ...
    SpellCastResult CheckCast()
    {
        if (!GetExplTargetUnit() || !GetCaster()->IsValidAttackTarget(
                                        GetExplTargetUnit(), GetSpellInfo()))
...
class spell_rog_vanish_purge : public SpellScript {
    ...
    void HandleRootRemove(SpellEffIndex /*effIndex*/)
    {
        if (GetCaster() &&
            !GetCaster()->HasAura(
                SPELL_PARALYZE)) // Root from Tainted Core SSC, should not be
                                 // removed by vanish.
    ...
    void Register() override
    {
        // Blizzard handles EFFECT_0 as the unroot and EFFECT_1 as unsnare.
        // Hence why they are not done in the same place.
        OnEffectHitTarget +=
            SpellEffectFn(spell_rog_vanish_purge::HandleRootRemove,
                          EFFECT_0,
                          SPELL_EFFECT_APPLY_AURA);
        OnEffectHitTarget +=
            SpellEffectFn(spell_rog_vanish_purge::HandleSnareRemove,
                          EFFECT_1,
                          SPELL_EFFECT_APPLY_AURA);
    }
};
...
class spell_rog_vanish : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_CLEAN_ESCAPE_AURA, SPELL_CLEAN_ESCAPE_HEAL});
    }
        ...
        if (GetCaster()) {
            ...
            if (!GetCaster()->HasAura(SPELL_STEALTH)) {
                ...
                if (GetCaster()->IsPlayer() &&
                    GetCaster()->HasSpellCooldown(SPELL_STEALTH))
            ...
            if (GetCaster()->HasAura(SPELL_CLEAN_ESCAPE_AURA))
                GetCaster()->CastSpell(
                    GetCaster(), SPELL_CLEAN_ESCAPE_HEAL, true);
        }
    }
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_rog_vanish::HandleEffect, EFFECT_2, SPELL_EFFECT_SANCTUARY);
    }
};
