...
/*
 * Scripts for spells with SPELLFAMILY_HUNTER, SPELLFAMILY_PET and
 * SPELLFAMILY_GENERIC spells used by hunter players. Ordered alphabetically
 * using scriptname. Scriptnames of files in this file should be prefixed with
 * "spell_hun_".
 */
...
class spell_hun_cower : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (AuraEffect* aurEff =
                GetUnitOwner()->GetAuraEffect(SPELL_AURA_DUMMY,
                                              SPELLFAMILY_PET,
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_hun_cower::CalculateAmount,
                                   EFFECT_1,
...
class spell_hun_wyvern_sting : public AuraScript {
                                ...
                                AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster())
            caster->CastSpell(
                GetTarget(),
                sSpellMgr->GetSpellWithRank(SPELL_HUNTER_WYVERN_STING_DOT,
                                            GetSpellInfo()->GetRank()),
                true);
    }
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_hun_wyvern_sting::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_MOD_STUN,
...
class spell_hun_animal_handler : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner())
            if (AuraEffect const* animalHandlerEff = owner->GetDummyAuraEffect(
                    SPELLFAMILY_HUNTER, 2234, EFFECT_1))
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_hun_animal_handler::CalculateAmount,
                                   EFFECT_0,
...
class spell_hun_generic_scaling : public AuraScript {
    ...
    void CalculateResistanceAmount(AuraEffect const* aurEff,
                                   int32&            amount,
                                   bool& /*canBeRecalculated*/)
    {
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            SpellSchoolMask schoolMask =
                SpellSchoolMask(aurEff->GetSpellInfo()
                                    ->Effects[aurEff->GetEffIndex()]
                                    .MiscValue);
            int32 modifier = schoolMask == SPELL_SCHOOL_MASK_NORMAL ? 35 : 40;
            amount         = CalculatePct(
                std::max<int32>(0, owner->GetResistance(schoolMask)), modifier);
                             ...
                             bool& /*canBeRecalculated*/)
    {
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            ...
            if (AuraEffect* wildHuntEff = GetUnitOwner()->GetDummyAuraEffect(
                    SPELLFAMILY_PET, 3748, EFFECT_0))
                AddPct(modifier, wildHuntEff->GetAmount());

            amount = CalculatePct(
                std::max<int32>(
                    0,
                    owner->GetStat(Stats(aurEff->GetSpellInfo()
                                             ->Effects[aurEff->GetEffIndex()]
                                             .MiscValue))),
                modifier);
            if (owner->HasAura(SPELL_HUNTER_PET_LEGGINGS_OF_BEAST_MASTERY)) {
    ...
    void CalculateAPAmount(AuraEffect const* /*aurEff*/,
                           int32& amount,
                           bool& /*canBeRecalculated*/)
    {
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
    ...
    void Register() override
    {
        if (m_scriptSpellId != 34902)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_hun_generic_scaling::CalculateResistanceAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_RESISTANCE);
        else {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_hun_generic_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_hun_generic_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_hun_generic_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_hun_generic_scaling::CalcPeriodic,
                                     EFFECT_ALL,
                                     SPELL_AURA_ANY);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_hun_generic_scaling::HandlePeriodic,
                                 EFFECT_ALL,
                                 SPELL_AURA_ANY);
    }
};
...
class spell_hun_taming_the_beast : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_hun_taming_the_beast::HandleOnEffectApply,
                              EFFECT_0,
                              SPELL_AURA_MOD_CHARM,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_hun_taming_the_beast::HandleOnEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_MOD_CHARM,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hun_aspect_of_the_beast : public AuraScript {
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetCaster())
            if (Player* caster = GetCaster()->ToPlayer())
                if (caster->GetPet())
                    caster->CastSpell(
                        caster, SPELL_HUNTER_ASPECT_OF_THE_BEAST_PET, true);
    }

    void OnPetApply(AuraEffect const* /*aurEff*/,
                    AuraEffectHandleModes /*mode*/)
    {
    ...
    void Register() override
    {
        if (m_scriptSpellId == 13161) {
            AfterEffectApply +=
                AuraEffectApplyFn(spell_hun_aspect_of_the_beast::OnApply,
                                  EFFECT_0,
                                  SPELL_AURA_UNTRACKABLE,
                                  AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove +=
                AuraEffectRemoveFn(spell_hun_aspect_of_the_beast::OnRemove,
                                   EFFECT_0,
                                   SPELL_AURA_UNTRACKABLE,
                                   AURA_EFFECT_HANDLE_REAL);
        }
            ...
            AfterEffectApply +=
                AuraEffectApplyFn(spell_hun_aspect_of_the_beast::OnPetApply,
                                  EFFECT_0,
                                  SPELL_AURA_UNTRACKABLE,
                                  AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hun_ascpect_of_the_viper : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& procInfo)
    {
        ...
        if (spellInfo && (spellInfo->SpellFamilyFlags[0] & 0x2000) &&
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        int32  mana    = CalculatePct(
            maxMana, GetTarget()->GetAttackTime(RANGED_ATTACK) / 1000.0f);

        if (AuraEffect const* glyph = GetTarget()->GetAuraEffect(
                SPELL_HUNTER_GLYPH_OF_ASPECT_OF_THE_VIPER, EFFECT_0))
            AddPct(mana, glyph->GetAmount());

        GetTarget()->CastCustomSpell(SPELL_HUNTER_ASPECT_OF_THE_VIPER_ENERGIZE,
    ...
    void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetTarget()->HasAura(SPELL_HUNTER_VIPER_ATTACK_SPEED))
            GetTarget()->CastSpell(
                GetTarget(), SPELL_HUNTER_VICIOUS_VIPER, true, nullptr, aurEff);
    }
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_hun_ascpect_of_the_viper::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_hun_ascpect_of_the_viper::HandleProc,
                             EFFECT_2,
                             SPELL_AURA_DUMMY);
        AfterEffectApply +=
            AuraEffectApplyFn(spell_hun_ascpect_of_the_viper::OnApply,
                              EFFECT_0,
                              SPELL_AURA_OBS_MOD_POWER,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_hun_ascpect_of_the_viper::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_OBS_MOD_POWER,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hun_chimera_shot : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* unitTarget = GetHitUnit()) {
            ...
            for (Unit::AuraApplicationMap::iterator i = Auras.begin();
                 i != Auras.end();
                 ++i) {
                ...
                if (AuraEffect* aurEff = aura->GetEffect(0)) {
                    // Serpent Sting - Instantly deals 40% of the damage done by
                    // your Serpent Sting.
                    if (familyFlag[0] & 0x4000) {
                        ...
                        basePoint = unitTarget->SpellDamageBonusTaken(
                            caster,
                            aura->GetSpellInfo(),
                            basePoint,
                            DOT,
                    ...
                    // Viper Sting - Instantly restores mana to you equal to 60%
                    // of the total amount drained by your Viper Sting.
                        ...
                        basePoint = int32(
                            CalculatePct(unitTarget->GetMaxPower(POWER_MANA),
                                         aurEff->GetAmount()));
                        int32 casterBasePoint =
                            aurEff->GetAmount() *
                            unitTarget->GetMaxPower(POWER_MANA) /
                            50; /// @todo: Caster uses unitTarget?
                    ...
                    // This effect cannot occur more than once per 1 minute.
                        ...
                        if (caster->ToPlayer()) // Scorpid Sting - Add 1 minute
                                                // cooldown
                        {
                            ...
                            caster->ToPlayer()->AddSpellCooldown(
                                SPELL_HUNTER_CHIMERA_SHOT_SCORPID, 0, 60000);
                        }
                    ...
                    aurEff->ChangeAmount(aurEff->CalculateAmount(caster),
                                         false);
                }
                break;
            }
            ...
            if (spellId)
                caster->CastCustomSpell(
                    unitTarget, spellId, &basePoint, 0, 0, true);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hun_chimera_shot::HandleScriptEffect,
                          EFFECT_0,
...
class spell_hun_improved_mend_pet : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            GetTarget(), SPELL_HUNTER_IMPROVED_MEND_PET, true, nullptr, aurEff);
    }
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_hun_improved_mend_pet::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_hun_invigoration : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Unit* unitTarget = GetHitUnit())
            if (AuraEffect* aurEff =
                    unitTarget->GetDummyAuraEffect(SPELLFAMILY_HUNTER, 3487, 0))
                if (roll_chance_i(aurEff->GetAmount()))
                    unitTarget->CastSpell(
                        unitTarget, SPELL_HUNTER_INVIGORATION_TRIGGERED, true);
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hun_invigoration::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hun_last_stand_pet : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        int32 healthModSpellBasePoints0 =
            int32(caster->CountPctFromMaxHealth(30));
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hun_last_stand_pet::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_hun_masters_call : public SpellScript {
    ...
    SpellCastResult DoCheckCast()
    {
        ...
        // Do a mini Spell::CheckCasterAuras on the pet, no other way of doing
        // this
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        GetCaster()->ToPlayer()->GetPet()->CastSpell(
            GetHitUnit(), GetEffectValue(), true);
    }

    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        GetHitUnit()->CastSpell(
            (Unit*)nullptr, SPELL_HUNTER_MASTERS_CALL_TRIGGERED, true);
    }
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_hun_masters_call::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_hun_masters_call::HandleScriptEffect,
                          EFFECT_1,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hun_readiness : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        // immediately finishes the cooldown on your other Hunter abilities
        // except Bestial Wrath
        ...
        for (const auto& [spellId, cooldown] : cooldowns) {
            ...
            if (spellInfo && spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
                ...
                if (spellInfo->RecoveryTime > 0)
                    spellsToRemove.insert(std::make_pair(
                        spellInfo->Id, cooldown.needSendToClient));
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_hun_readiness::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_hun_scatter_shot : public SpellScript {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_hun_scatter_shot::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_hun_sniper_training : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_HUNTER_SNIPER_TRAINING_R1,
    ...
    void HandlePeriodic(AuraEffect const* aurEff)
    {
        ...
        if (aurEff->GetAmount() <= 0) {
            ...
            uint32 spellId = SPELL_HUNTER_SNIPER_TRAINING_BUFF_R1 + GetId() -
                             SPELL_HUNTER_SNIPER_TRAINING_R1;
            if (SpellInfo const* triggeredSpellInfo =
                    sSpellMgr->GetSpellInfo(spellId)) {
                Unit* triggerCaster =
                    triggeredSpellInfo->NeedsToBeTriggeredByCaster(
                        GetSpellInfo())
                        ? GetCaster()
                        : target;
                triggerCaster->CastSpell(
                    target, triggeredSpellInfo, true, 0, aurEff);
            }
        }
    }
    ...
    void HandleUpdatePeriodic(AuraEffect* aurEff)
    {
        if (Player* playerTarget = GetUnitOwner()->ToPlayer()) {
            ...
            int32 amount =
                playerTarget->isMoving() || aurEff->GetAmount() <= 0
                    ? playerTarget->CalculateSpellDamage(playerTarget,
                                                         GetSpellInfo(),
                                                         aurEff->GetEffIndex(),
                                                         &baseAmount)
                    : aurEff->GetAmount() - 1;
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_hun_sniper_training::HandlePeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_hun_sniper_training::HandleUpdatePeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_hun_pet_heart_of_the_phoenix : public SpellScript {
        ...
        if (Unit* owner = caster->GetOwner())
            if (!caster->HasAura(
                ...
                owner->CastCustomSpell(
                    ...
                    true);
                caster->CastSpell(
                    caster, SPELL_HUNTER_PET_HEART_OF_THE_PHOENIX_DEBUFF, true);
            }
    }
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_hun_pet_heart_of_the_phoenix::CheckCast);
        OnEffectHitTarget +=
            SpellEffectFn(spell_hun_pet_heart_of_the_phoenix::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hun_pet_carrion_feeder : public SpellScript {
    ...
    SpellCastResult CheckIfCorpseNear()
    {
        ...
        Acore::AnyDeadUnitSpellTargetInRangeCheck check(
            caster, max_range, GetSpellInfo(), TARGET_CHECK_ENEMY);
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        Unit* caster = GetCaster();
        caster->CastSpell(
            caster, SPELL_HUNTER_PET_CARRION_FEEDER_TRIGGERED, false);
    }

    void Register() override
    {
        OnEffectHit += SpellEffectFn(spell_hun_pet_carrion_feeder::HandleDummy,
                                     EFFECT_0,
                                     SPELL_EFFECT_DUMMY);
        OnCheckCast +=
            SpellCheckCastFn(spell_hun_pet_carrion_feeder::CheckIfCorpseNear);
    }
};
...
class spell_hun_misdirection : public AuraScript {
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetTargetApplication()->GetRemoveMode() != AURA_REMOVE_BY_DEFAULT ||
            !GetTarget()->HasAura(SPELL_HUNTER_MISDIRECTION_PROC))
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            GetTarget(), SPELL_HUNTER_MISDIRECTION_PROC, true, nullptr, aurEff);
    }

    void Register() override
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_hun_misdirection::OnRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
        DoCheckProc += AuraCheckProcFn(spell_hun_misdirection::CheckProc);
        OnEffectProc += AuraEffectProcFn(
            spell_hun_misdirection::HandleProc, EFFECT_1, SPELL_AURA_DUMMY);
    }
};
...
class spell_hun_misdirection_proc : public AuraScript {
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_hun_misdirection_proc::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
    ...
    SpellCastResult CheckCast()
    {
        ...
        if (Creature* target = GetExplTargetUnit()->ToCreature()) {
            ...
            if (target->GetCreatureTemplate()->IsExotic() &&
                !player->CanTameExoticPets()) {
                player->SendTameFailure(PET_TAME_CANT_CONTROL_EXOTIC);
                return SPELL_FAILED_DONT_REPORT;
            }

            if (!target->GetCreatureTemplate()->IsTameable(
                    player->CanTameExoticPets())) {
...
class spell_hun_viper_attack_speed : public AuraScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_HUNTER_ASPECT_OF_THE_VIPER, SPELL_HUNTER_VICIOUS_VIPER});
    }
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_hun_viper_attack_speed::OnApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_hun_viper_attack_speed::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hun_glyph_of_arcane_shot : public AuraScript {
            ...
            const auto found = std::find_if(
                ...
                [&](std::pair<uint32, AuraApplication*> pair) {
                    ...
                    return ((aura->GetCasterGUID() == GetTarget()->GetGUID()) &&
                            aura->GetSpellInfo()->SpellFamilyName ==
                                SPELLFAMILY_HUNTER &&
                            aura->GetSpellInfo()->SpellFamilyFlags.HasFlag(
                                0xC000, 0x1080));
                });
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 mana =
            procSpell->CalcPowerCost(GetTarget(), procSpell->GetSchoolMask());
        ApplyPct(mana, aurEff->GetAmount());

        GetTarget()->CastCustomSpell(SPELL_HUNTER_GLYPH_OF_ARCANE_SHOT,
                                     SPELLVALUE_BASE_POINT0,
                                     mana,
                                     GetTarget());
    }
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_hun_glyph_of_arcane_shot::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_hun_glyph_of_arcane_shot::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_hun_volley_trigger : public SpellScript {
    ...
    {
        // It's here because Volley is an AOE spell so there is no specific
        // target to be attacked Let's select one of our targets
        if (!targets.empty()) {
    ...
    void HandleFinish()
    {
        ...
        for (Unit::ControlSet::iterator itr = caster->m_Controlled.begin();
             itr != caster->m_Controlled.end();
             ++itr) {
            if (Unit* pet = *itr) {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_hun_volley_trigger::SelectTarget,
            EFFECT_0,
...
enum LocknLoadSpells { SPELL_FROST_TRAP_SLOW = 67035 };
...
class spell_hun_lock_and_load : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_LOCK_AND_LOAD_TRIGGER,
    ...
    bool CheckTrapProc(ProcEventInfo& eventInfo)
    {
        ...
        if (((spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_FIRE) ||
            ...
            (spellInfo->Effects[0].ApplyAuraName ==
                 SPELL_AURA_PERIODIC_DAMAGE ||
             spellInfo->Effects[1].ApplyAuraName ==
                 SPELL_AURA_PERIODIC_DAMAGE)) {
            return true;
        }

        return IsTargetValid(spellInfo, eventInfo.GetProcTarget()) &&
               !eventInfo.GetActor()->HasAura(SPELL_LOCK_AND_LOAD_MARKER);
    }
    ...
    bool IsTargetValid(SpellInfo const* spellInfo, Unit* target)
    {
        ...
        // Don't check it for fire traps and black arrow, they proc on periodic
        // only and not spell hit. So it's wrong to check for immunity, it was
        // already checked when the spell was applied.
        if ((spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_FIRE) ||
            (spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_SHADOW)) {
            return false;
        }
        ...
        // That's because the talent is triggered by the spell that fires the
        // trap (63487)...
        ...
        if (spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_NATURE) {
            if (SpellInfo const* triggerSpell =
                    sSpellMgr->GetSpellInfo(SPELL_FROST_TRAP_SLOW)) {
    ...
    void HandleProcs(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        // Also check if the proc from the fire traps and black arrow actually
        // comes from the periodic ticks here. Normally this wouldn't be
        // required, but we are circumventing the current proc system
        // limitations.
        if (((spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_FIRE) ||
             (spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_SHADOW)) &&
            (spellInfo->Effects[0].ApplyAuraName ==
    ...
    {
        if (IsTargetValid(eventInfo.GetSpellInfo(),
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(spell_hun_lock_and_load::HandleProcs<
                                             PROC_FLAG_DONE_TRAP_ACTIVATION>,
                                         EFFECT_0,
                                         SPELL_AURA_PROC_TRIGGER_SPELL);
        OnEffectProc += AuraEffectProcFn(
            spell_hun_lock_and_load::HandleProcs<PROC_FLAG_DONE_PERIODIC>,
            EFFECT_1,
            SPELL_AURA_DUMMY);
...
class spell_hun_target_self_and_pet : public SpellScript {
    ...
    bool Load() override { return GetCaster()->IsPet(); }

    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([&](WorldObject const* target) -> bool {
            return target != GetCaster() &&
                   target != GetCaster()->ToPet()->GetOwner();
        });
    }
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_hun_target_self_and_pet::FilterTargets,
            EFFECT_ALL,
