...
#include "UnitAI.h"
/*
 * Scripts for spells with SPELLFAMILY_PALADIN and SPELLFAMILY_GENERIC spells
 * used by paladin players. Ordered alphabetically using scriptname. Scriptnames
 * of files in this file should be prefixed with "spell_pal_".
 */
...
class spell_pal_seal_of_command_aura : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (SpellInfo const* procSpell = eventInfo.GetSpellInfo()) {
            if (procSpell->SpellIconID ==
                3025) // Righteous Vengeance, should not proc SoC
            {
                return false;
            }
        }
    ...
    void Register() override
    {
        if (m_scriptSpellId == 20375) {
            DoCheckProc +=
                AuraCheckProcFn(spell_pal_seal_of_command_aura::CheckProc);
            OnEffectProc +=
                AuraEffectProcFn(spell_pal_seal_of_command_aura::HandleProc,
                                 EFFECT_0,
    ...
    void Register() override
    {
        if (m_scriptSpellId == 20424)
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_pal_seal_of_command::FilterTargets,
                EFFECT_0,
...
class spell_pal_divine_intervention : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
    {
        if (!GetTarget()->IsInCombat() &&
            ...
            GetTarget()->RemoveAurasDueToSpell(
                GetTarget()->ToPlayer()->GetTeamId() == TEAM_ALLIANCE ? 57723
                                                                      : 57724);
    }
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_pal_divine_intervention::HandleRemove,
                               EFFECT_0,
...
class spell_pal_seal_of_light : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        // xinef: skip divine storm self hit (dummy) and righteous vengeance
        // (0x20000000=
        return eventInfo.GetActor() != eventInfo.GetProcTarget() &&
               (!eventInfo.GetSpellInfo() ||
                !eventInfo.GetSpellInfo()->SpellFamilyFlags.HasFlag(
                    0x20000000));
    }
...
class spell_pal_sacred_shield_base : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (Unit* caster = GetCaster()) {
            ...
            // +75.00% from sp bonus
            amount += CalculatePct(
                caster->SpellBaseDamageBonusDone(spellInfo->GetSchoolMask()),
                75.0f);

            // Xinef: removed divine guardian because it will affect triggered
            // spell with increased amount Arena - Dampening
            if (AuraEffect const* dampening = caster->GetAuraEffect(
                    SPELL_GENERIC_ARENA_DAMPENING, EFFECT_0)) {
                AddPct(amount, dampening->GetAmount());
            }
            // Battleground - Dampening
            else if (AuraEffect const* dampening2 = caster->GetAuraEffect(
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        return !(eventInfo.GetHitMask() & PROC_EX_INTERNAL_HOT) &&
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        eventInfo.GetActionTarget()->AddSpellCooldown(
            triggered_spell_id, 0, cooldown);
        eventInfo.GetActionTarget()->CastCustomSpell(
            ...
            nullptr,
            aurEff,
            eventInfo.GetActionTarget()->GetGUID());
    }

    void Register() override
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_pal_sacred_shield_base::CalculateAmount,
            EFFECT_0,
            SPELL_AURA_DUMMY);
        DoCheckProc += AuraCheckProcFn(spell_pal_sacred_shield_base::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_pal_sacred_shield_base::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
private:
    ...
    void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (remainingHealth <= 0 &&
            !victim->ToPlayer()->HasAura(PAL_SPELL_ARDENT_DEFENDER_DEBUFF)) {
            ...
            int32 healAmount = int32(victim->CountPctFromMaxHealth(
            ...
            // Reduce damage that brings us under 35% (or full damage if we are
            // already under 35%) by x%
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_pal_ardent_defender::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_pal_ardent_defender::Absorb, EFFECT_0);
    }
};
...
class spell_pal_avenging_wrath : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_PALADIN_SANCTIFIED_WRATH,
                     ...
                     AuraEffectHandleModes /*mode*/)
    {
        ...
        if (AuraEffect const* aurEff = target->GetAuraEffectOfRankedSpell(
                SPELL_PALADIN_SANCTIFIED_WRATH_TALENT_R1, EFFECT_2)) {
            int32 basepoints = aurEff->GetAmount();
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_pal_avenging_wrath::HandleApply,
                              EFFECT_0,
                              SPELL_AURA_MOD_DAMAGE_PERCENT_DONE,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_pal_avenging_wrath::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_MOD_DAMAGE_PERCENT_DONE,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_pal_blessing_of_faith : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_PALADIN_BLESSING_OF_LOWER_CITY_DRUID,
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pal_blessing_of_faith::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_pal_blessing_of_sanctuary : public AuraScript {
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_pal_blessing_of_sanctuary::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_pal_blessing_of_sanctuary::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        DoCheckProc +=
            AuraCheckProcFn(spell_pal_blessing_of_sanctuary::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_pal_blessing_of_sanctuary::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_pal_divine_sacrifice : public AuraScript {
        ...
        if (Unit* caster = GetCaster()) {
            if (caster->GetTypeId() == TYPEID_PLAYER) {
                if (caster->ToPlayer()->GetGroup())
                    groupSize =
                        caster->ToPlayer()->GetGroup()->GetMembersCount();
                else
                    groupSize = 1;
            }
            else
    ...
    void
    Split(AuraEffect* /*aurEff*/, DamageInfo& /*dmgInfo*/, uint32& splitAmount)
    {
        remainingAmount -= splitAmount;
        // break when absorbed everything it could, or if the casters hp drops
        // below 20%
        if (Unit* caster = GetCaster())
    ...
    {
        OnEffectSplit +=
            AuraEffectSplitFn(spell_pal_divine_sacrifice::Split, EFFECT_0);
    }
};
...
class spell_pal_divine_storm : public SpellScript {
    ...
    void TriggerHeal()
    {
        ...
        if (GetHitUnit() != caster)
            caster->CastCustomSpell(SPELL_PALADIN_DIVINE_STORM_DUMMY,
                                    SPELLVALUE_BASE_POINT0,
                                    (GetHitDamage() * healPct) / 100,
                                    caster,
                                    true);
    }
...
class spell_pal_divine_storm_dummy : public SpellScript {
    ...
    {
        Acore::Containers::RandomResize(targetList,
                                        GetSpellValue()->MaxAffectedTargets);
...
private:
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pal_divine_storm_dummy::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_pal_divine_storm_dummy::CountTargets,
            EFFECT_0,
...
class spell_pal_exorcism_and_holy_wrath_damage : public AuraScript {
    ...
    void Register() override
    {
        DoEffectCalcSpellMod += AuraEffectCalcSpellModFn(
            spell_pal_exorcism_and_holy_wrath_damage::HandleEffectCalcSpellMod,
            EFFECT_0,
            SPELL_AURA_DUMMY);
    }
};
...
class spell_pal_eye_for_an_eye : public AuraScript {
    ...
    void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 damage =
            std::min(CalculatePct(static_cast<int32>(damageInfo->GetDamage()),
                                  aurEff->GetAmount()),
                     static_cast<int32>(GetTarget()->GetMaxHealth()) / 2);
        GetTarget()->CastCustomSpell(SPELL_PALADIN_EYE_FOR_AN_EYE_DAMAGE,
                                     SPELLVALUE_BASE_POINT0,
                                     damage,
    ...
    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(
            spell_pal_eye_for_an_eye::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_pal_glyph_of_holy_light : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_pal_glyph_of_holy_light::FilterTargets,
            EFFECT_0,
...
class spell_pal_guarded_by_the_light : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pal_guarded_by_the_light::HandleScriptEffect,
                          EFFECT_0,
...
class spell_pal_hand_of_sacrifice_aura : public AuraScript {
    ...
    void Register() override
    {
        OnEffectSplit += AuraEffectSplitFn(
            spell_pal_hand_of_sacrifice_aura::Split, EFFECT_0);
    }
};
...
class spell_pal_hand_of_sacrifice : public SpellScript {
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_pal_hand_of_sacrifice::CheckTarget);
    }
};
...
class spell_pal_hand_of_salvation : public AuraScript {
        ...
        if (Unit* caster = GetCaster()) {
            ...
            if (caster->GetGUID() == GetUnitOwner()->GetGUID())
                if (AuraEffect const* aurEff = caster->GetAuraEffect(
                        SPELL_PALADIN_GLYPH_OF_SALVATION, EFFECT_0))
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_pal_hand_of_salvation::CalculateAmount,
                                   EFFECT_1,
...
class spell_pal_holy_shock : public SpellScript {
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        ...
        if (!sSpellMgr->GetSpellWithRank(
                SPELL_PALADIN_HOLY_SHOCK_R1_DAMAGE, rank, true) ||
            !sSpellMgr->GetSpellWithRank(
                SPELL_PALADIN_HOLY_SHOCK_R1_HEALING, rank, true))
            return false;
        ...
        if (Unit* unitTarget = GetHitUnit()) {
            ...
            if (caster->IsFriendlyTo(unitTarget))
                caster->CastSpell(
                    unitTarget,
                    sSpellMgr->GetSpellWithRank(
                        SPELL_PALADIN_HOLY_SHOCK_R1_HEALING, rank),
                    true);
            else
                caster->CastSpell(unitTarget,
                                  sSpellMgr->GetSpellWithRank(
                                      SPELL_PALADIN_HOLY_SHOCK_R1_DAMAGE, rank),
                                  true);
        }
    }
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_pal_holy_shock::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
public:
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        Unit::AuraEffectList const& auras =
            GetCaster()->GetAuraEffectsByType(SPELL_AURA_DUMMY);
        for (Unit::AuraEffectList::const_iterator i = auras.begin();
             i != auras.end();
             ++i) {
            if ((*i)->GetSpellInfo()->GetSpellSpecific() ==
                    SPELL_SPECIFIC_SEAL &&
                (*i)->GetEffIndex() == EFFECT_2)
        ...
        if (GetCaster()->HasAura(SPELL_IMPROVED_JUDGEMENT)) {
            GetCaster()->CastSpell(
                GetCaster(), SPELL_IMPROVED_JUDGEMENT_ENERGIZE, true);
        }

        // Judgement of the Just
        if (GetCaster()->GetAuraEffect(
                SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_PALADIN, 3015, 0)) {
            if (GetCaster()->CastSpell(
                    GetHitUnit(), SPELL_JUDGEMENTS_OF_THE_JUST, true) &&
                (spellId2 == SPELL_JUDGEMENT_OF_VENGEANCE_EFFECT ||
                 spellId2 == SPELL_JUDGEMENT_OF_CORRUPTION_EFFECT)) {
                // hidden effect only cast when spellcast of judgements of the
                // just is succesful
                GetCaster()->CastSpell(GetHitUnit(),
                                       SealApplication(spellId2),
                                       true); // add hidden seal apply effect
                                              // for vengeance and corruption
            }
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pal_judgement::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_pal_judgement_of_command : public SpellScript {
    ...
    {
        if (Unit* unitTarget = GetHitUnit())
            if (SpellInfo const* spell_proto =
                    sSpellMgr->GetSpellInfo(GetEffectValue()))
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pal_judgement_of_command::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_pal_lay_on_hands : public SpellScript {
        ...
        if (Unit* target = GetExplTargetUnit())
            if (caster == target)
                if (target->HasAura(SPELL_PALADIN_FORBEARANCE) ||
                    target->HasAura(SPELL_PALADIN_AVENGING_WRATH_MARKER) ||
    ...
    void HandleScript()
    {
        ...
        if (caster == target) {
            ...
            caster->CastSpell(
                caster, SPELL_PALADIN_AVENGING_WRATH_MARKER, true);
            caster->CastSpell(caster, SPELL_PALADIN_IMMUNE_SHIELD_MARKER, true);
        }
        // Xinef: Glyph of Divinity
        else if (target && caster->HasAura(54939) &&
                 GetSpellInfo()->Id != 633 &&
                 _manaAmount > 0) // excluding first rank
        {
            _manaAmount = target->GetPower(POWER_MANA) - _manaAmount;
            if (_manaAmount > 0)
                caster->CastCustomSpell(54986 /*Energize*/,
...
class spell_pal_righteous_defense : public SpellScript {
    ...
    void HandleTriggerSpellHit(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit())
            GetCaster()->CastSpell(
                target, SPELL_PALADIN_RIGHTEOUS_DEFENSE_TAUNT, true);
    }
    ...
    void Register() override
    {
        ...
        //! so we must handle trigger spell also in hit phase (default execution
        //! in launch phase) see issue #3718
        OnEffectLaunchTarget +=
            SpellEffectFn(spell_pal_righteous_defense::HandleTriggerSpellLaunch,
                          EFFECT_1,
                          SPELL_EFFECT_TRIGGER_SPELL);
        OnEffectHitTarget +=
            SpellEffectFn(spell_pal_righteous_defense::HandleTriggerSpellHit,
                          EFFECT_1,
                          SPELL_EFFECT_TRIGGER_SPELL);
    }
};
...
class spell_pal_seal_of_righteousness : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 holy =
            GetTarget()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY);
        holy += eventInfo.GetProcTarget()->SpellBaseDamageBonusTaken(
            SPELL_SCHOOL_MASK_HOLY);

        // Xinef: Libram of Divine Purpose
        if (AuraEffect* aurEffPaladin = GetTarget()->GetDummyAuraEffect(
                SPELLFAMILY_PALADIN, 2025, EFFECT_0)) {
            holy += aurEffPaladin->GetAmount();
        }

        int32 bp = std::max<int32>(
            0,
            int32((ap * 0.022f + 0.044f * holy) *
                  GetTarget()->GetAttackTime(BASE_ATTACK) / 1000));
        GetTarget()->CastCustomSpell(SPELL_PALADIN_SEAL_OF_RIGHTEOUSNESS,
                                     SPELLVALUE_BASE_POINT0,
                                     bp,
                                     eventInfo.GetProcTarget(),
                                     true,
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_pal_seal_of_righteousness::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_pal_seal_of_righteousness::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
{
    RegisterSpellAndAuraScriptPair(spell_pal_seal_of_command,
    ...
    RegisterSpellAndAuraScriptPair(spell_pal_hand_of_sacrifice,
                                   spell_pal_hand_of_sacrifice_aura);
    RegisterSpellScript(spell_pal_hand_of_salvation);
    RegisterSpellScript(spell_pal_holy_shock);
    RegisterSpellScriptWithArgs(spell_pal_judgement,
                                "spell_pal_judgement_of_justice",
                                SPELL_PALADIN_JUDGEMENT_OF_JUSTICE);
    RegisterSpellScriptWithArgs(spell_pal_judgement,
                                "spell_pal_judgement_of_light",
                                SPELL_PALADIN_JUDGEMENT_OF_LIGHT);
    RegisterSpellScriptWithArgs(spell_pal_judgement,
                                "spell_pal_judgement_of_wisdom",
