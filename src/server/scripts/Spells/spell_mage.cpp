...
/*
 * Scripts for spells with SPELLFAMILY_MAGE and SPELLFAMILY_GENERIC spells used
 * by mage players. Ordered alphabetically using scriptname. Scriptnames of
 * files in this file should be prefixed with "spell_mage_".
 */
...
class spell_mage_arcane_blast : public SpellScript {
    ...
    {
        GetCaster()->CastSpell(
            GetCaster(), _triggerSpellId, TRIGGERED_FULL_MASK);
    }

    void Register() override
    {
        OnEffectLaunch +=
            SpellEffectFn(spell_mage_arcane_blast::HandleTriggerSpell,
                          EFFECT_1,
...
class spell_mage_burning_determination : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (!(eventInfo.GetSpellInfo()->GetAllEffectsMechanicMask() &
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_mage_burning_determination::CheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_mage_burning_determination::HandleProc,
                             EFFECT_0,
...
class spell_mage_molten_armor : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (!spellInfo ||
            (eventInfo.GetTypeMask() & PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK))
            return true;
        ...
        if (AuraEffect* aurEff = eventInfo.GetActionTarget()->GetAuraEffect(
                SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_MAGE, 16, EFFECT_0))
...
class spell_mage_mirror_image : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_mage_mirror_image::HandleEffectApply,
                              EFFECT_2,
                              SPELL_AURA_PERIODIC_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_mage_mirror_image::CalcPeriodic,
                                     EFFECT_2,
                                     SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_mage_burnout : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 mana = int32(eventInfo.GetSpellInfo()->CalcPowerCost(
            GetTarget(), eventInfo.GetSchoolMask()));
        mana       = CalculatePct(mana, aurEff->GetAmount());

        GetTarget()->CastCustomSpell(SPELL_MAGE_BURNOUT_TRIGGER,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_mage_burnout::HandleProc, EFFECT_1, SPELL_AURA_DUMMY);
    }
};
...
class spell_mage_burnout_trigger : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            GetSpell()->ExecuteLogEffectTakeTargetPower(
                effIndex, target, POWER_MANA, newDamage, 0.0f);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_mage_burnout_trigger::HandleDummy,
                          EFFECT_0,
...
class spell_mage_pet_scaling : public AuraScript {
    ...
    {
        // xinef: mage pet inherits 40% of resistance from owner and 35% of
        // armor (guessed)
                           ...
                           bool& /*canBeRecalculated*/)
    {
        ...
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            int32 frost =
                owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST);
    ...
    void Register() override
    {
        if (m_scriptSpellId != 35657)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_mage_pet_scaling::CalculateResistanceAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_RESISTANCE);

        if (m_scriptSpellId == 35657 || m_scriptSpellId == 35658)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_mage_pet_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);

        if (m_scriptSpellId == 35657) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_mage_pet_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_mage_pet_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        OnEffectApply +=
            AuraEffectApplyFn(spell_mage_pet_scaling::HandleEffectApply,
                              EFFECT_ALL,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_mage_pet_scaling::CalcPeriodic, EFFECT_ALL, SPELL_AURA_ANY);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_mage_pet_scaling::HandlePeriodic, EFFECT_ALL, SPELL_AURA_ANY);
    }
};
...
class spell_mage_glyph_of_eternal_water : public AuraScript {
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_mage_glyph_of_eternal_water::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_mage_combustion_proc : public AuraScript {
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_mage_combustion_proc::OnRemove,
                               EFFECT_0,
...
public:
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_MAGE_INCANTERS_ABSORBTION_TRIGGERED,
    ...
    Trigger(AuraEffect* aurEff, DamageInfo& /*dmgInfo*/, uint32& absorbAmount)
    {
        ...
        if (AuraEffect* talentAurEff = target->GetAuraEffectOfRankedSpell(
                SPELL_MAGE_INCANTERS_ABSORBTION_R1, EFFECT_0)) {
            int32 bp = CalculatePct(absorbAmount, talentAurEff->GetAmount());
            if (AuraEffect* currentAura =
                    target->GetAuraEffect(SPELL_AURA_MOD_DAMAGE_DONE,
                                          SPELLFAMILY_MAGE,
                                          2941,
                                          EFFECT_0)) {
                bp += int32(currentAura->GetAmount() *
                            (currentAura->GetBase()->GetDuration() /
                             (float)currentAura->GetBase()->GetMaxDuration()));
...
class spell_mage_blast_wave : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_mage_blast_wave::HandleKnockBack,
                          EFFECT_2,
...
class spell_mage_cold_snap : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        for (PlayerSpellMap::const_iterator itr = spellMap.begin();
             itr != spellMap.end();
             ++itr) {
            ...
            if (spellInfo->SpellFamilyName == SPELLFAMILY_MAGE &&
                ...
                SpellCooldowns::iterator citr =
                    caster->GetSpellCooldownMap().find(spellInfo->Id);
                if (citr != caster->GetSpellCooldownMap().end() &&
                    citr->second.needSendToClient)
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(
            spell_mage_cold_snap::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_mage_fire_frost_ward
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_MAGE_FROST_WARDING_TRIGGERED, SPELL_MAGE_FROST_WARDING_R1});
    }
    ...
    void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        Unit* target = GetTarget();
        if (AuraEffect* talentAurEff = target->GetAuraEffectOfRankedSpell(
                SPELL_MAGE_FROST_WARDING_R1, EFFECT_0)) {
            int32 chance =
                talentAurEff->GetSpellInfo()
                    ->Effects[EFFECT_1]
                    .CalcValue(); // SPELL_EFFECT_DUMMY with NO_TARGET
            ...
            if (roll_chance_i(chance)) {
                ...
                if (!dmgInfo.GetSpellInfo() ||
                    dmgInfo.GetSpellInfo()->SpellIconID != 3178)
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_mage_fire_frost_ward::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_mage_fire_frost_ward::Absorb, EFFECT_0);
        AfterEffectAbsorb +=
            AuraEffectAbsorbFn(spell_mage_fire_frost_ward::Trigger, EFFECT_0);
    }
};
...
class spell_mage_focus_magic : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            _procTarget, SPELL_MAGE_FOCUS_MAGIC_PROC, true, nullptr, aurEff);
    }

    void Register() override
    {
        DoCheckProc += AuraCheckProcFn(spell_mage_focus_magic::CheckProc);
        OnEffectProc += AuraEffectProcFn(spell_mage_focus_magic::HandleProc,
                                         EFFECT_0,
...
class spell_mage_ice_barrier_aura
                                      ...
                                      const AuraEffect* aurEff)
    {
        ...
        // Glyph of Ice Barrier: its weird having a SPELLMOD_ALL_EFFECTS here
        // but its blizzards doing :) Glyph of Ice Barrier is only applied at
        // the spell damage bonus because it was already applied to the base
        // value in CalculateSpellDamage
        bonus = caster->ApplyEffectModifiers(
            spellInfo, aurEff->GetEffIndex(), bonus);
                         ...
                         bool&             canBeRecalculated)
    {
        ...
        if (Unit* caster = GetCaster())
            amount =
                CalculateSpellAmount(caster, amount, GetSpellInfo(), aurEff);
    }

    void Register() override
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_mage_ice_barrier_aura::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_SCHOOL_ABSORB);
        AfterEffectAbsorb +=
            AuraEffectAbsorbFn(spell_mage_ice_barrier_aura::Trigger, EFFECT_0);
    }
};
...
class spell_mage_ice_barrier : public SpellScript {
    ...
    SpellCastResult CheckCast()
    {
        ...
        if (AuraEffect* aurEff = caster->GetAuraEffect(
                SPELL_AURA_SCHOOL_ABSORB,
                (SpellFamilyNames)GetSpellInfo()->SpellFamilyName,
                GetSpellInfo()->SpellIconID,
                EFFECT_0)) {
            int32 newAmount = GetSpellInfo()->Effects[EFFECT_0].CalcValue(
                caster, nullptr, nullptr);
            newAmount =
                CalculateSpellAmount(caster, newAmount, GetSpellInfo(), aurEff);
...
class spell_mage_ignite : public AuraScript {
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* igniteDot =
            sSpellMgr->AssertSpellInfo(SPELL_MAGE_IGNITE);
        int32 pct = 8 * GetSpellInfo()->GetRank();

        int32 amount =
            int32(CalculatePct(eventInfo.GetDamageInfo()->GetDamage(), pct) /
        ...
        eventInfo.GetProcTarget()->CastDelayedSpellWithPeriodicAmount(
            ...
            amount);
        // GetTarget()->CastCustomSpell(SPELL_MAGE_IGNITE,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_mage_ignite::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_mage_living_bomb : public AuraScript {
    ...
    bool Validate(SpellInfo const* spell) override
    {
        if (!sSpellMgr->GetSpellInfo(
                uint32(spell->Effects[EFFECT_1].CalcValue())))
            return false;
        return true;
    }
    ...
    void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (Unit* caster = GetCaster())
            caster->CastSpell(GetTarget(),
                              uint32(aurEff->GetAmount()),
                              true,
                              nullptr,
                              aurEff);
    }
    ...
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_mage_living_bomb::AfterRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_mage_mana_shield
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_mage_mana_shield::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_MANA_SHIELD);
        AfterEffectManaShield +=
            AuraEffectManaShieldFn(spell_mage_mana_shield::Trigger, EFFECT_0);
    }
};
...
class spell_mage_master_of_elements : public AuraScript {
    ...
    bool AfterCheckProc(ProcEventInfo& eventInfo,
                        bool           isTriggeredAtSpellProcEvent)
    {
        if (!isTriggeredAtSpellProcEvent || !eventInfo.GetActor() ||
        ...
        if (eventInfo.GetTriggerAuraEffectIndex() >= EFFECT_0) {
            if (Unit* caster = GetCaster()) {
                if (Unit* target =
                        (selectCaster ? eventInfo.GetActor()
                                      : eventInfo.GetActionTarget())) {
                    if (AuraEffect const* aurEff = target->GetAuraEffect(
                            _spellInfo->Id,
                            eventInfo.GetTriggerAuraEffectIndex(),
                            caster->GetGUID())) {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (Unit* target = GetTarget()) {
            int32 mana = int32(
                _spellInfo->CalcPowerCost(target, eventInfo.GetSchoolMask()) /
                _ticksModifier);
            mana = CalculatePct(mana, aurEff->GetAmount());

            if (mana > 0) {
                target->CastCustomSpell(SPELL_MAGE_MASTER_OF_ELEMENTS_ENERGIZE,
                                        SPELLVALUE_BASE_POINT0,
                                        mana,
                                        target,
                                        true,
    ...
    {
        DoAfterCheckProc +=
            AuraAfterCheckProcFn(spell_mage_master_of_elements::AfterCheckProc);
        OnEffectProc +=
            AuraEffectProcFn(spell_mage_master_of_elements::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
...
class spell_mage_polymorph_cast_visual : public SpellScript {
    ...
    {
        if (Unit* target =
                GetCaster()->FindNearestCreature(NPC_AUROSALIA, 30.0f))
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_mage_polymorph_cast_visual::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};

const uint32 spell_mage_polymorph_cast_visual::
    spell_mage_polymorph_cast_visual::PolymorhForms[6] = {
...
class spell_mage_summon_water_elemental : public SpellScript {
    PrepareSpellScript(spell_mage_summon_water_elemental) bool Validate(
        SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_MAGE_GLYPH_OF_ETERNAL_WATER,
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Creature* pet =
                ObjectAccessor::GetCreature(*caster, caster->GetPetGUID()))
            if (!pet->IsAlive())
        ...
        if (caster->HasAura(SPELL_MAGE_GLYPH_OF_ETERNAL_WATER))
            caster->CastSpell(
                caster, SPELL_MAGE_SUMMON_WATER_ELEMENTAL_PERMANENT, true);
        else
            caster->CastSpell(
                caster, SPELL_MAGE_SUMMON_WATER_ELEMENTAL_TEMPORARY, true);
    ...
    {
        OnEffectHit +=
            SpellEffectFn(spell_mage_summon_water_elemental::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_mage_fingers_of_frost_proc_aura : public AuraScript {
    ...
    void HandleOnEffectProc(AuraEffect const* /*aurEff*/,
                            ProcEventInfo& eventInfo)
    {
        if (eventInfo.GetSpellPhaseMask() == PROC_SPELL_PHASE_CAST) {
            ...
            if (eventInfo.GetSpellPhaseMask() == PROC_SPELL_PHASE_FINISH ||
                ((_spell && _spell->GetDelayMoment() > 0) ||
    ...
    void HandleAfterEffectProc(AuraEffect const* /*aurEff*/,
                               ProcEventInfo& eventInfo)
    {
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_mage_fingers_of_frost_proc_aura::CheckProc);
        DoAfterCheckProc += AuraAfterCheckProcFn(
            spell_mage_fingers_of_frost_proc_aura::AfterCheckProc);
        OnEffectProc += AuraEffectProcFn(
            spell_mage_fingers_of_frost_proc_aura::HandleOnEffectProc,
            EFFECT_0,
            SPELL_AURA_PROC_TRIGGER_SPELL);
        AfterEffectProc += AuraEffectProcFn(
            spell_mage_fingers_of_frost_proc_aura::HandleAfterEffectProc,
            EFFECT_0,
            SPELL_AURA_PROC_TRIGGER_SPELL);
    }
...
typedef spell_mage_fingers_of_frost_proc_aura
...
class spell_mage_fingers_of_frost_proc : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        if (Aura* aura = GetCaster()->GetAuraOfRankedSpell(
                SPELL_MAGE_FINGERS_OF_FROST)) {
            if (spell_mage_fingers_of_frost_proc_aura_script* script =
                    dynamic_cast<spell_mage_fingers_of_frost_proc_aura_script*>(
                        aura->GetScriptByName(FingersOfFrostScriptName))) {
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_mage_fingers_of_frost_proc::CheckProc);
    }
};
