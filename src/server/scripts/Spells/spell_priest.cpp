...
/*
 * Scripts for spells with SPELLFAMILY_PRIEST and SPELLFAMILY_GENERIC spells
 * used by priest players. Ordered alphabetically using scriptname. Scriptnames
 * of files in this file should be prefixed with "spell_pri_".
 */
...
class spell_pri_shadowfiend_scaling : public AuraScript {
    ...
    {
        // xinef: shadowfiend inherits 40% of resistance from owner and 35% of
        // armor (guessed)
    ...
    void CalculateStatAmount(AuraEffect const* aurEff,
                             ...
                             bool& /*canBeRecalculated*/)
    {
        // xinef: shadowfiend inherits 30% of intellect and 65% of stamina
        // (guessed)
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
    ...
    void CalculateAPAmount(AuraEffect const* /*aurEff*/,
                           ...
                           bool& /*canBeRecalculated*/)
    {
        // xinef: shadowfiend inherits 333% of SP as AP - 35.7% of damage
        // increase per hit
        if (Unit* owner = GetUnitOwner()->GetOwner()) {
            int32 shadow =
                owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW);
            amount = CalculatePct(
                std::max<int32>(0, shadow),
                300); // xinef: deacrased to 300, including 15% from self buff
        }
    }
    ...
    void Register() override
    {
        if (m_scriptSpellId != 35661)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_pri_shadowfiend_scaling::CalculateResistanceAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_RESISTANCE);

        if (m_scriptSpellId == 35661 || m_scriptSpellId == 35662)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_pri_shadowfiend_scaling::CalculateStatAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_STAT);

        if (m_scriptSpellId == 35661) {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_pri_shadowfiend_scaling::CalculateAPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_ATTACK_POWER);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_pri_shadowfiend_scaling::CalculateSPAmount,
                EFFECT_ALL,
                SPELL_AURA_MOD_DAMAGE_DONE);
        }

        OnEffectApply +=
            AuraEffectApplyFn(spell_pri_shadowfiend_scaling::HandleEffectApply,
                              EFFECT_ALL,
                              SPELL_AURA_ANY,
...
class spell_pri_circle_of_healing : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        uint32 const maxTargets =
            GetCaster()->HasAura(SPELL_PRIEST_GLYPH_OF_CIRCLE_OF_HEALING)
                ? 6
                : 5; // Glyph of Circle of Healing
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_pri_circle_of_healing::FilterTargets,
            EFFECT_0,
...
class spell_pri_divine_aegis : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 absorb = CalculatePct(int32(eventInfo.GetHealInfo()->GetHeal()),
                                    aurEff->GetAmount());

        // Multiple effects stack, so let's try to find this aura.
        if (AuraEffect const* aegis = eventInfo.GetProcTarget()->GetAuraEffect(
                SPELL_PRIEST_DIVINE_AEGIS, EFFECT_0))
            absorb += aegis->GetAmount();

        absorb = std::min(absorb, eventInfo.GetProcTarget()->GetLevel() * 125);

        GetTarget()->CastCustomSpell(SPELL_PRIEST_DIVINE_AEGIS,
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_pri_divine_aegis::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_pri_divine_hymn : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_pri_divine_hymn::FilterTargets,
                                          EFFECT_ALL,
...
class spell_pri_glyph_of_prayer_of_healing : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* triggeredSpellInfo = sSpellMgr->AssertSpellInfo(
            SPELL_PRIEST_GLYPH_OF_PRAYER_OF_HEALING_HEAL);
        int32 heal = int32(
            CalculatePct(int32(healInfo->GetHeal()), aurEff->GetAmount()) /
            triggeredSpellInfo->GetMaxTicks());
    ...
    {
        OnEffectProc +=
            AuraEffectProcFn(spell_pri_glyph_of_prayer_of_healing::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_pri_guardian_spirit : public AuraScript {
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_pri_guardian_spirit::CalculateAmount,
                                   EFFECT_1,
                                   SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_pri_guardian_spirit::Absorb, EFFECT_1);
    }
};
...
class spell_pri_hymn_of_hope : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_pri_hymn_of_hope::FilterTargets,
                                          EFFECT_ALL,
                                          TARGET_UNIT_SRC_AREA_ALLY);
    }
};
...
class spell_pri_item_greater_heal_refund : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        if (HealInfo* healInfo = eventInfo.GetHealInfo())
            if (Unit* healTarget = healInfo->GetTarget())
                if (eventInfo.GetHitMask() & PROC_EX_NO_OVERHEAL &&
                    healTarget->IsFullHealth())
                    return true;
        return false;
    }
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            GetTarget(), SPELL_PRIEST_ITEM_EFFICIENCY, true, nullptr, aurEff);
    }
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_pri_item_greater_heal_refund::HandleProc,
                             EFFECT_0,
...
class spell_pri_lightwell : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /* effIndex */)
    {
        ...
        case PRIEST_LIGHTWELL_NPC_2:
            lightwellRenew = 27873;
            break;
        case PRIEST_LIGHTWELL_NPC_3:
            lightwellRenew = 27874;
            break;
        case PRIEST_LIGHTWELL_NPC_4:
            lightwellRenew = 28276;
            break;
        case PRIEST_LIGHTWELL_NPC_5:
            lightwellRenew = 48084;
            break;
        case PRIEST_LIGHTWELL_NPC_6:
            lightwellRenew = 48085;
            break;
        }

        // proc a spellcast
        if (Aura* chargesAura =
                caster->GetAura(SPELL_PRIEST_LIGHTWELL_CHARGES)) {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pri_lightwell::HandleScriptEffect,
                          EFFECT_0,
...
class spell_pri_lightwell_renew : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (Unit* caster = GetCaster()) {
            ...
            if (AuraEffect* modHealing = caster->GetAuraEffect(
                    SPELL_PRIEST_GLYPH_OF_LIGHTWELL, EFFECT_0))
                AddPct(amount, modHealing->GetAmount());
        }
    }

    void HandleUpdateSpellclick(AuraEffect const* /*aurEff*/,
                                AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster()) {
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_pri_lightwell_renew::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_PERIODIC_HEAL);
        AfterEffectApply +=
            AuraEffectApplyFn(spell_pri_lightwell_renew::HandleUpdateSpellclick,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_HEAL,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_pri_lightwell_renew::HandleUpdateSpellclick,
            EFFECT_0,
            SPELL_AURA_PERIODIC_HEAL,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_pri_mana_burn : public SpellScript {
    ...
    void HandleAfterHit()
    {
        if (Unit* unitTarget = GetHitUnit())
            unitTarget->RemoveAurasWithMechanic((1 << MECHANIC_FEAR) |
...
class spell_pri_mana_leech : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            _procTarget, SPELL_PRIEST_MANA_LEECH_PROC, true, nullptr, aurEff);
    }
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_pri_mana_leech::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
...
class spell_pri_mind_sear : public SpellScript {
    ...
    {
        unitList.remove_if(Acore::ObjectGUIDCheck(
            GetCaster()->GetGuidValue(UNIT_FIELD_CHANNEL_OBJECT), true));
    }

    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_pri_mind_sear::FilterTargets,
                                          EFFECT_0,
...
class spell_pri_pain_and_suffering_proc : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_pri_pain_and_suffering_proc::HandleEffectScriptEffect,
            EFFECT_0,
            SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_pri_penance : public SpellScript {
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        SpellInfo const* firstRankSpellInfo =
            sSpellMgr->GetSpellInfo(SPELL_PRIEST_PENANCE_R1);
        ...
        if (!sSpellMgr->GetSpellWithRank(
                SPELL_PRIEST_PENANCE_R1_DAMAGE, rank, true))
            return false;
        if (!sSpellMgr->GetSpellWithRank(
                SPELL_PRIEST_PENANCE_R1_HEAL, rank, true))
            return false;
            ...
            if (caster->IsFriendlyTo(unitTarget))
                caster->CastSpell(unitTarget,
                                  sSpellMgr->GetSpellWithRank(
                                      SPELL_PRIEST_PENANCE_R1_HEAL, rank),
                                  false);
            else
                caster->CastSpell(unitTarget,
                                  sSpellMgr->GetSpellWithRank(
                                      SPELL_PRIEST_PENANCE_R1_DAMAGE, rank),
                                  false);
        }
    }
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_pri_penance::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
                                  ...
                                  const AuraEffect* aurEff)
{
    ...
    // Borrowed Time
    if (AuraEffect const* borrowedTime = caster->GetDummyAuraEffect(
            SPELLFAMILY_PRIEST, PRIEST_ICON_ID_BORROWED_TIME, EFFECT_1))
        bonus += CalculatePct(1.0f, borrowedTime->GetAmount());

    bonus *= caster->SpellBaseHealingBonusDone(spellInfo->GetSchoolMask());

    // Improved PW: Shield: its weird having a SPELLMOD_ALL_EFFECTS here but its
    // blizzards doing :) Improved PW: Shield is only applied at the spell
    // healing bonus because it was already applied to the base value in
    // CalculateSpellDamage
    bonus =
        caster->ApplyEffectModifiers(spellInfo, aurEff->GetEffIndex(), bonus);
    bonus *= caster->CalculateLevelPenalty(spellInfo);

    amount += int32(bonus);

    // Twin Disciplines
    if (AuraEffect const* twinDisciplines =
    ...
    if (int32 healModifier = caster->GetMaxPositiveAuraModifier(
            SPELL_AURA_MOD_HEALING_DONE_PERCENT))
        AddPct(amount, healModifier);

    // Arena - Dampening
    if (AuraEffect const* arenaDampening =
            caster->GetAuraEffect(SPELL_GENERIC_ARENA_DAMPENING, EFFECT_0)) {
        AddPct(amount, arenaDampening->GetAmount());
    }
    // Battleground - Dampening
    else if (AuraEffect const* bgDampening = caster->GetAuraEffect(
                 SPELL_GENERIC_BATTLEGROUND_DAMPENING, EFFECT_0)) {
...
class spell_pri_power_word_shield_aura : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_PRIEST_REFLECTIVE_SHIELD_TRIGGERED,
    ...
    void
    ReflectDamage(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        Unit* target = GetTarget();
        ...
        if (Unit* owner = GetUnitOwner())
            if (AuraEffect* talentAurEff = owner->GetAuraEffectOfRankedSpell(
                    SPELL_PRIEST_REFLECTIVE_SHIELD_R1, EFFECT_0)) {
                int32 bp =
                    CalculatePct(absorbAmount, talentAurEff->GetAmount());
                // xinef: prevents infinite loop!
                if (!dmgInfo.GetSpellInfo() ||
                    dmgInfo.GetSpellInfo()->Id !=
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_pri_power_word_shield_aura::CalculateAmount,
            EFFECT_0,
            SPELL_AURA_SCHOOL_ABSORB);
        AfterEffectAbsorb += AuraEffectAbsorbFn(
            spell_pri_power_word_shield_aura::ReflectDamage, EFFECT_0);
    }
};
...
class spell_pri_prayer_of_mending_heal : public SpellScript {
    ...
    void HandleHeal(SpellEffIndex /*effIndex*/)
    {
        if (Unit* caster = GetOriginalCaster()) {
            ...
            if (AuraEffect* aurEff = caster->GetAuraEffect(
                    SPELL_PRIEST_T9_HEALING_2P, EFFECT_0)) {
                AddPct(heal, aurEff->GetAmount());
            }

            if (AuraEffect* aurEff = caster->GetAuraEffectOfRankedSpell(
                    SPELL_PRIEST_TWIN_DISCIPLINE_R1, EFFECT_0)) {
                AddPct(heal, aurEff->GetAmount());
            }
            if (AuraEffect* aurEff = caster->GetAuraEffectOfRankedSpell(
                    SPELL_PRIEST_SPIRITUAL_HEALING_R1, EFFECT_0)) {
                AddPct(heal, aurEff->GetAmount());
            }
            if (AuraEffect* aurEff = caster->GetAuraEffectOfRankedSpell(
                    SPELL_PRIEST_DIVINE_PROVIDENCE_R1, EFFECT_0)) {
                AddPct(heal, aurEff->GetAmount());
            }
    ...
    void Register() override
    {
        OnEffectLaunchTarget +=
            SpellEffectFn(spell_pri_prayer_of_mending_heal::HandleHeal,
                          EFFECT_0,
                          SPELL_EFFECT_HEAL);
    }
};
...
class spell_pri_renew : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster()) {
            // Empowered Renew
            if (AuraEffect const* empoweredRenewAurEff =
                    caster->GetDummyAuraEffect(
                        SPELLFAMILY_PRIEST,
                        PRIEST_ICON_ID_EMPOWERED_RENEW_TALENT,
                        EFFECT_1)) {
                uint32 heal = GetEffect(EFFECT_0)->GetAmount();
                heal        = GetTarget()->SpellHealingBonusTaken(
                    caster, GetSpellInfo(), heal, DOT);

                int32 basepoints0 = empoweredRenewAurEff->GetAmount() *
                                    GetEffect(EFFECT_0)->GetTotalTicks() *
                                    int32(heal) / 100;
                caster->CastCustomSpell(GetTarget(),
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_pri_renew::HandleApplyEffect,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_HEAL,
...
class spell_pri_shadow_word_death : public SpellScript {
    ...
    void HandleDamage()
    {
        ...
        if (AuraEffect* aurEff = GetCaster()->GetDummyAuraEffect(
                SPELLFAMILY_PRIEST,
        ...
        GetCaster()->CastCustomSpell(
            GetCaster(), SPELL_PRIEST_SHADOW_WORD_DEATH, &damage, 0, 0, true);
    }
...
class spell_pri_vampiric_touch : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        if (!eventInfo.GetActionTarget() ||
            GetOwner()->GetGUID() != eventInfo.GetActionTarget()->GetGUID()) {
            return false;
        }
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_pri_vampiric_touch::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_pri_mind_control : public AuraScript {
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_pri_mind_control::HandleApplyEffect,
                              EFFECT_0,
                              SPELL_AURA_MOD_POSSESS,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_pri_mind_control::HandleRemoveEffect,
                               EFFECT_0,
                               SPELL_AURA_MOD_POSSESS,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_pri_t4_4p_bonus : public AuraScript {
    ...
    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_pri_t4_4p_bonus::HandleProc,
                                         EFFECT_ALL,
...
void AddSC_priest_spell_scripts()
{
    ...
    RegisterSpellAndAuraScriptPair(spell_pri_power_word_shield,
                                   spell_pri_power_word_shield_aura);
