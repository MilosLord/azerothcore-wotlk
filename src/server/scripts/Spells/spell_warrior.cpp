...
/*
 * Scripts for spells with SPELLFAMILY_WARRIOR and SPELLFAMILY_GENERIC spells
 * used by warrior players. Ordered alphabetically using scriptname. Scriptnames
...
class spell_warr_intervene : public SpellScript {
    ...
    void HandleApplyAura(SpellEffIndex /*effIndex*/)
    {
        if (Unit* target = GetHitUnit())
            target->CastSpell(
                (Unit*)nullptr, SPELL_WARRIOR_INTERVENE_TRIGGER, true);
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warr_intervene::HandleApplyAura,
                          EFFECT_1,
...
class spell_warr_improved_spell_reflection : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        return eventInfo.GetSpellInfo() && eventInfo.GetActor() &&
               eventInfo.GetSpellInfo()->Id == SPELL_WARRIOR_SPELL_REFLECTION;
    }
    ...
    void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        values.AddSpellMod(
            ...
            2000); // Base range = 100, final range = 20 value / 10000.0f = 0.2f
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_warr_improved_spell_reflection::OnProc,
                             EFFECT_1,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_warr_improved_spell_reflection_trigger : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_warr_improved_spell_reflection_trigger::FilterTargets,
            EFFECT_0,
...
class spell_warr_improved_spell_reflection_trigger_aura : public AuraScript {
                      ...
                      AuraEffectHandleModes /*mode*/)
    {
        if (!IsExpired()) {
            ...
            for (std::list<Unit*>::iterator itr = PartyMembers.begin();
                 itr != PartyMembers.end();
                 ++itr) {
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_warr_improved_spell_reflection_trigger_aura::HandleRemove,
            EFFECT_0,
...
class spell_warr_last_stand : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        int32 healthModSpellBasePoints0 =
            int32(caster->CountPctFromMaxHealth(GetEffectValue()));
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(
            spell_warr_last_stand::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_deep_wounds : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            damage = target->MeleeDamageBonusTaken(
                caster, damage, BASE_ATTACK, GetSpellInfo());
            ...
            // SPELL_WARRIOR_DEEP_WOUNDS_RANK_PERIODIC, &damage, nullptr,
            // nullptr, true);
        }
    }
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_warr_deep_wounds::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_charge : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_WARRIOR_JUGGERNAUT_CRIT_BONUS_TALENT,
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (caster->HasAura(SPELL_WARRIOR_JUGGERNAUT_CRIT_BONUS_TALENT))
            caster->CastSpell(
                caster, SPELL_WARRIOR_JUGGERNAUT_CRIT_BONUS_BUFF, true);
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_warr_charge::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_slam : public SpellScript {
    ...
    {
        if (GetHitUnit())
            GetCaster()->CastCustomSpell(SPELL_WARRIOR_SLAM,
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_warr_slam::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_damage_shield : public AuraScript {
    ...
    void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        // % of amount blocked
        int32 damage = CalculatePct(int32(GetTarget()->GetShieldBlockValue()),
                                    aurEff->GetAmount());
        GetTarget()->CastCustomSpell(SPELL_WARRIOR_DAMAGE_SHIELD_DAMAGE,
                                     SPELLVALUE_BASE_POINT0,
                                     damage,
    ...
    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(
            spell_warr_damage_shield::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_warr_execute : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_WARRIOR_EXECUTE, SPELL_WARRIOR_GLYPH_OF_EXECUTION});
    }
    ...
    void SendMiss(SpellMissInfo missInfo)
    {
        if (missInfo != SPELL_MISS_NONE) {
            if (Unit* caster = GetCaster()) {
                if (Unit* target = GetHitUnit()) {
                    caster->SendSpellMiss(
                        target, SPELL_WARRIOR_EXECUTE, missInfo);
                }
            }
        }
    }
    ...
    void HandleEffect(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            int32            rageUsed  = std::min<int32>(
                300 - spellInfo->CalcPowerCost(
                          caster, SpellSchoolMask(spellInfo->SchoolMask)),
                caster->GetPower(POWER_RAGE));
            int32 newRage =
                std::max<int32>(0, caster->GetPower(POWER_RAGE) - rageUsed);
            ...
            if (AuraEffect* aurEff =
                    caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL,
                                          ...
                                          EFFECT_0)) {
                int32 ragesave =
                    aurEff->GetSpellInfo()->Effects[EFFECT_1].CalcValue() * 10;
            ...
            if (AuraEffect* aurEff = caster->GetAuraEffect(
                    SPELL_WARRIOR_GLYPH_OF_EXECUTION, EFFECT_0))
                rageUsed += aurEff->GetAmount() * 10;

            int32 bp =
                GetEffectValue() +
                int32(rageUsed * spellInfo->Effects[effIndex].DamageMultiplier +
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget += SpellEffectFn(
            spell_warr_execute::HandleEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_concussion_blow : public SpellScript {
    ...
    {
        SetHitDamage(
            CalculatePct(GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK),
                         GetEffectValue()));
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warr_concussion_blow::HandleDummy,
                          EFFECT_2,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_bloodthirst : public SpellScript {
    ...
    void HandleDamage(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            damage = GetCaster()->SpellDamageBonusDone(target,
                                                       GetSpellInfo(),
                                                       uint32(damage),
                                                       SPELL_DIRECT_DAMAGE,
                                                       effIndex);
            damage = target->SpellDamageBonusTaken(GetCaster(),
                                                   GetSpellInfo(),
                                                   uint32(damage),
                                                   SPELL_DIRECT_DAMAGE);
        }
    ...
    void Register() override
    {
        ...
        OnEffectHit += SpellEffectFn(
            spell_warr_bloodthirst::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_warr_bloodthirst_heal : public SpellScript {
    ...
    void HandleHeal(SpellEffIndex /*effIndex*/)
    {
        if (SpellInfo const* spellInfo =
                sSpellMgr->GetSpellInfo(SPELL_WARRIOR_BLOODTHIRST_DAMAGE))
            SetEffectValue(GetCaster()->CountPctFromMaxHealth(
    ...
    void Register() override
    {
        OnEffectLaunchTarget +=
            SpellEffectFn(spell_warr_bloodthirst_heal::HandleHeal,
                          EFFECT_0,
                          SPELL_EFFECT_HEAL);
    }
};
...
class spell_warr_overpower : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_WARRIOR_UNRELENTING_ASSAULT_RANK_1,
    ...
    void HandleEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Player* target = GetHitPlayer())
            if (target->HasUnitState(UNIT_STATE_CASTING))
                target->CastSpell(
                    target, spellId, true, 0, 0, GetCaster()->GetGUID());
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_warr_overpower::HandleEffect, EFFECT_0, SPELL_EFFECT_ANY);
    }
};
...
class spell_warr_intimidating_shout : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_warr_intimidating_shout::FilterTargets,
            EFFECT_1,
...
class spell_warr_rend : public AuraScript {
                         ...
                         bool&             canBeRecalculated)
    {
        if (Unit* caster = GetCaster()) {
            ...
            for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
                mwbMin +=
                    caster->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE, i);
                mwbMax +=
                    caster->GetWeaponDamageRange(BASE_ATTACK, MAXDAMAGE, i);
            }

            float mwb = ((mwbMin + mwbMax) / 2 + ap * mws / 14000) * 0.2f;
            amount += int32(caster->ApplyEffectModifiers(
                GetSpellInfo(), aurEff->GetEffIndex(), mwb));

            // "If used while your target is above 75% health, Rend does 35%
            // more damage." as for 3.1.3 only ranks above 9 (wrong tooltip?)
            if (GetSpellInfo()->GetRank() >= 9) {
                if (GetUnitOwner()->HasAuraState(
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_warr_rend::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_PERIODIC_DAMAGE);
    }
};
...
class spell_warr_shattering_throw : public SpellScript {
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit())
            target->RemoveAurasWithMechanic(1 << MECHANIC_IMMUNE_SHIELD,
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warr_shattering_throw::HandleScript,
                          EFFECT_0,
...
class spell_warr_sweeping_strikes : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        _procTarget =
            actor->SelectNearbyNoTotemTarget(eventInfo.GetProcTarget());
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (DamageInfo* damageInfo = eventInfo.GetDamageInfo()) {
            ...
            if (spellInfo && spellInfo->Id == SPELL_WARRIOR_EXECUTE &&
                !_procTarget->HasAuraState(AURA_STATE_HEALTHLESS_20_PERCENT)) {
                // If triggered by Execute (while target is not under 20% hp)
                // deals normalized weapon damage
                GetTarget()->CastSpell(
                    _procTarget,
                    SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_2,
                    aurEff);
                ...
                if (spellInfo &&
                    spellInfo->Id == SPELL_WARRIOR_WHIRLWIND_MAIN) {
                    eventInfo.GetActor()->AddSpellCooldown(
                        SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_1, 0, 500);
                }
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_warr_sweeping_strikes::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
...
class spell_warr_vigilance : public AuraScript {
                     ...
                     AuraEffectHandleModes /*mode*/)
    {
        ...
        if (Unit* caster = GetCaster())
            target->CastSpell(
                caster, SPELL_WARRIOR_VIGILANCE_REDIRECT_THREAT, true);
    }

    void HandleAfterApply(AuraEffect const* /*aurEff*/,
                          AuraEffectHandleModes /*mode*/)
    {
        //! WORKAROUND
        //! this glyph is a proc
        if (Unit* caster = GetCaster()) {
            if (AuraEffect const* glyph = caster->GetAuraEffect(
                    SPELL_WARRIOR_GLYPH_OF_VIGILANCE, EFFECT_0))
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            _procTarget, SPELL_WARRIOR_VIGILANCE_PROC, true, nullptr, aurEff);
    }

    void Register() override
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_warr_vigilance::HandleApply,
                              EFFECT_0,
                              SPELL_AURA_PROC_TRIGGER_SPELL,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        AfterEffectApply +=
            AuraEffectApplyFn(spell_warr_vigilance::HandleAfterApply,
                              EFFECT_0,
                              SPELL_AURA_PROC_TRIGGER_SPELL,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_warr_vigilance::HandleRemove,
                               EFFECT_0,
                               SPELL_AURA_PROC_TRIGGER_SPELL,
                               AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        DoCheckProc += AuraCheckProcFn(spell_warr_vigilance::CheckProc);
        OnEffectProc += AuraEffectProcFn(spell_warr_vigilance::HandleProc,
                                         EFFECT_0,
                                         SPELL_AURA_PROC_TRIGGER_SPELL);
    }
...
class spell_warr_vigilance_trigger : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_warr_vigilance_trigger::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_warr_glyph_of_sunder_armor : public AuraScript {
                                  ...
                                  SpellModifier*&   spellMod)
    {
        if (!spellMod) {
            ...
            spellMod->mask =
                GetSpellInfo()->Effects[aurEff->GetEffIndex()].SpellClassMask;
        }
    ...
    void Register() override
    {
        DoEffectCalcSpellMod += AuraEffectCalcSpellModFn(
            spell_warr_glyph_of_sunder_armor::HandleEffectCalcSpellMod,
            EFFECT_0,
            SPELL_AURA_DUMMY);
    }
};
...
class spell_warr_t3_prot_8p_bonus : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        return eventInfo.GetActionTarget() &&
               eventInfo.GetActionTarget()->GetHealthPct() <= 20.0f;
    }
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_warr_t3_prot_8p_bonus::HandleEffectProc,
                             EFFECT_0,
                             SPELL_AURA_PROC_TRIGGER_SPELL);
    }
};
...
class spell_warr_retaliation : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        return GetTarget()->isInFront(eventInfo.GetActor(), M_PI) &&
               !GetTarget()->HasUnitState(UNIT_STATE_STUNNED);
    }
    ...
    void Register() override
    {
        ...
        OnEffectProc +=
            AuraEffectProcFn(spell_warr_retaliation::HandleEffectProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
void AddSC_warrior_spell_scripts()
{
    ...
    RegisterSpellAndAuraScriptPair(
        spell_warr_improved_spell_reflection_trigger,
        spell_warr_improved_spell_reflection_trigger_aura);
