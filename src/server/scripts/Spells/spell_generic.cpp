...
#include <array>
/*
 * Scripts for spells with SPELLFAMILY_GENERIC which cannot be included in AI
 * script file of creature using it or can't be bound to any player class.
...
class spell_gen_5000_gold : public SpellScript {
    ...
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_5000_gold::HandleScript,
                                           EFFECT_0,
...
public:
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        for (uint8 i = 0; i < 3; ++i)
            _itemId.at(i) =
                GetUnitOwner()->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i);
                            ...
                            AuraEffectHandleModes /*mode*/)
    {
        ...
        for (uint8 i = 0; i < 3; ++i)
            GetUnitOwner()->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i,
                                           _itemId.at(i));
    }
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_model_visible::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_model_visible::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_the_flag_of_ownership : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        snprintf(buff,
                 sizeof(buff),
                 "%s plants the Flag of Ownership in the corpse of %s.",
    ...
    void HandleFinish()
    {
        ...
        if (!haveTarget) {
            ...
            Spell::SendCastResult(caster->ToPlayer(),
                                  GetSpellInfo(),
                                  0,
    ...
    void Register() override
    {
        ...
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_flag_of_ownership::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_flag_of_ownership::FilterTargets,
            EFFECT_0,
...
class spell_gen_have_item_auras : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_have_item_auras::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_mine_sweeper : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_LAND_MINE_KNOCKBACK, SPELL_LANDMINE_KNOCKBACK_ACHIEVEMENT});
    }
        ...
        if (Unit* target = GetHitPlayer())
            if (Aura* aur = target->GetAura(GetSpellInfo()->Id))
                if (aur->GetStackAmount() >= 10)
                    target->CastSpell(
                        target, SPELL_LANDMINE_KNOCKBACK_ACHIEVEMENT, true);
    }
    ...
    void Register() override
    {
        if (m_scriptSpellId == 54355)
            OnEffectHitTarget +=
                SpellEffectFn(spell_gen_mine_sweeper::HandleSchoolDMG,
                              EFFECT_0,
                              SPELL_EFFECT_SCHOOL_DAMAGE);
        else
            OnEffectHitTarget +=
                SpellEffectFn(spell_gen_mine_sweeper::HandleScriptEffect,
                              EFFECT_0,
                              SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
    ...
    void RecalculateDamage()
    {
        if (Unit* target = GetHitUnit())
            if (target->GetLevel() > 60) {
                ...
                AddPct(damage,
                       -4 * int8(std::min(target->GetLevel(), uint8(85)) -
                                 60)); // prevents reduce by more than 100%
                SetHitDamage(damage);
            }
    }
                         ...
                         bool& /*canBeRecalculated*/)
    {
        if (Unit* owner = GetUnitOwner())
            if (owner->GetLevel() > 60)
                AddPct(amount,
                       -4 * int8(std::min(owner->GetLevel(), uint8(85)) -
                                 60)); // prevents reduce by more than 100%
    }
    ...
    void Register() override
    {
        if (m_scriptSpellId != 20004) // Lifestealing enchange - no aura effect
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_gen_reduced_above_60_aura::CalculateAmount,
                EFFECT_ALL,
                SPELL_AURA_ANY);
    }
};

/* 69664 - Aquanos Laundry (spell_q20438_q24556_aquantos_laundry) 38724 - Magic
   Sucker Device (Success Visual) (spell_q10838_demoniac_scryer_visual) */
class spell_gen_relocaste_dest : public SpellScript {
    PrepareSpellScript(spell_gen_relocaste_dest);

public:
    spell_gen_relocaste_dest(float x, float y, float z, float o)
        : _x(x), _y(y), _z(z), _o(o)
    {
    }
...
class spell_gen_allow_proc_from_spells_with_cost : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        // xinef: Spells with no damage class and physical school only are not
        // treated as spells
        ...
        return spellInfo->ManaCost > 0 || spellInfo->ManaCostPercentage > 0 ||
               (spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN &&
                spellInfo->SpellIconID == 156);
    }

    void Register() override
    {
        DoCheckProc += AuraCheckProcFn(
            spell_gen_allow_proc_from_spells_with_cost::CheckProc);
    }
};
...
class spell_gen_bg_preparation : public AuraScript {
                               ...
                               AuraEffectHandleModes /*mode*/)
    {
        GetTarget()->ApplySpellImmune(
            GetId(), IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, true);
    }
    ...
    {
        GetTarget()->ApplySpellImmune(
            GetId(), IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, false);
    }

    void CalcPeriodic(AuraEffect const* /*effect*/,
                      bool&  isPeriodic,
                      int32& amplitude)
    {
        isPeriodic = true;
    ...
    {
        if (Player* player =
                GetUnitOwner()->GetCharmerOrOwnerPlayerOrPlayerItself())
            if (MapEntry const* mapEntry =
                    sMapStore.LookupEntry(player->GetMapId()))
                if ((GetId() == 32727 && mapEntry->IsBattleArena()) ||
                    (GetId() == 44521 && mapEntry->IsBattleground()))
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_bg_preparation::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_MOD_POWER_COST_SCHOOL_PCT,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_bg_preparation::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_MOD_POWER_COST_SCHOOL_PCT,
                               AURA_EFFECT_HANDLE_REAL);

        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_gen_bg_preparation::CalcPeriodic,
                                     EFFECT_0,
                                     SPELL_AURA_MOD_POWER_COST_SCHOOL_PCT);
        OnEffectUpdatePeriodic +=
            AuraEffectUpdatePeriodicFn(spell_gen_bg_preparation::Update,
                                       EFFECT_0,
                                       SPELL_AURA_MOD_POWER_COST_SCHOOL_PCT);
    }
};
...
class spell_gen_disabled_above_level : public SpellScript {
    PrepareSpellScript(spell_gen_disabled_above_level)
        ...
        public : spell_gen_disabled_above_level(uint8 level)
        : SpellScript(), _level(level)
    {
    }
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_gen_disabled_above_level::CheckRequirement);
    }
...
class spell_pet_hit_expertise_scalling : public AuraScript {
    ...
    void CalculateHitAmount(AuraEffect const* /*aurEff*/,
                            int32& amount,
                            bool& /*canBeRecalculated*/)
    {
        if (Player* modOwner = GetUnitOwner()->GetSpellModOwner()) {
            if (modOwner->IsClass(CLASS_HUNTER, CLASS_CONTEXT_STATS))
                amount = CalculatePercent(
                    modOwner->m_modRangedHitChance, 8.0f, 8.0f);
            else if (modOwner->getPowerType() == POWER_MANA)
                amount = CalculatePercent(
                    modOwner->m_modSpellHitChance, 17.0f, 8.0f);
            else
                amount =
                    CalculatePercent(modOwner->m_modMeleeHitChance, 8.0f, 8.0f);
        }
    }

    void CalculateSpellHitAmount(AuraEffect const* /*aurEff*/,
                                 int32& amount,
                                 bool& /*canBeRecalculated*/)
    {
        if (Player* modOwner = GetUnitOwner()->GetSpellModOwner()) {
            if (modOwner->IsClass(CLASS_HUNTER, CLASS_CONTEXT_STATS))
                amount = CalculatePercent(
                    modOwner->m_modRangedHitChance, 8.0f, 17.0f);
            else if (modOwner->getPowerType() == POWER_MANA)
                amount = CalculatePercent(
                    modOwner->m_modSpellHitChance, 17.0f, 17.0f);
            else
                amount = CalculatePercent(
                    modOwner->m_modMeleeHitChance, 8.0f, 17.0f);
        }
    }

    void CalculateExpertiseAmount(AuraEffect const* /*aurEff*/,
                                  int32& amount,
                                  bool& /*canBeRecalculated*/)
    {
        if (Player* modOwner = GetUnitOwner()->GetSpellModOwner()) {
            if (modOwner->IsClass(CLASS_HUNTER, CLASS_CONTEXT_STATS))
                amount = CalculatePercent(
                    modOwner->m_modRangedHitChance, 8.0f, 26.0f);
            else if (modOwner->getPowerType() == POWER_MANA)
                amount = CalculatePercent(
                    modOwner->m_modSpellHitChance, 17.0f, 26.0f);
            else
                amount = CalculatePercent(
                    modOwner->m_modMeleeHitChance, 8.0f, 26.0f);
        }
    }
    ...
    void HandleEffectApply(AuraEffect const* aurEff,
                           AuraEffectHandleModes /*mode*/)
    {
        GetUnitOwner()->ApplySpellImmune(GetId(),
                                         IMMUNITY_STATE,
                                         aurEff->GetAuraType(),
                                         true,
    ...
    void CalcPeriodic(AuraEffect const* /*aurEff*/,
                      bool&  isPeriodic,
                      int32& amplitude)
    {
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_pet_hit_expertise_scalling::CalculateHitAmount,
            EFFECT_0,
            SPELL_AURA_MOD_HIT_CHANCE);
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_pet_hit_expertise_scalling::CalculateSpellHitAmount,
            EFFECT_1,
            SPELL_AURA_MOD_SPELL_HIT_CHANCE);
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_pet_hit_expertise_scalling::CalculateExpertiseAmount,
            EFFECT_2,
            SPELL_AURA_MOD_EXPERTISE);

        OnEffectApply += AuraEffectApplyFn(
            spell_pet_hit_expertise_scalling::HandleEffectApply,
            EFFECT_ALL,
            SPELL_AURA_ANY,
            AURA_EFFECT_HANDLE_REAL);
        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_pet_hit_expertise_scalling::CalcPeriodic,
            EFFECT_ALL,
            SPELL_AURA_ANY);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_pet_hit_expertise_scalling::HandlePeriodic,
            EFFECT_ALL,
            SPELL_AURA_ANY);
    }
};
...
class spell_gen_grow_flower_patch : public SpellScript {
        ...
        SpellCastResult CheckCast()
    {
        if (GetCaster()->HasAuraType(SPELL_AURA_MOD_STEALTH) ||
            GetCaster()->HasAuraType(SPELL_AURA_MOD_INVISIBILITY))
            return SPELL_FAILED_DONT_REPORT;
...
class spell_gen_rallying_cry_of_the_dragonslayer : public SpellScript {
    ...
    void SelectTarget(std::list<WorldObject*>& targets)
    {
        ...
        for (Map::PlayerList::const_iterator itr = pList.begin();
             itr != pList.end();
             ++itr)
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_gen_rallying_cry_of_the_dragonslayer::SelectTarget,
            EFFECT_ALL,
...
class spell_gen_adals_song_of_battle : public SpellScript {
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_gen_adals_song_of_battle::SelectTarget,
            EFFECT_ALL,
            TARGET_UNIT_SRC_AREA_ALLY);
    }
};
...
class spell_gen_disabled_above_63 : public AuraScript {
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_gen_disabled_above_63::CalculateAmount,
                                   EFFECT_ALL,
                                   SPELL_AURA_ANY);
    }
};
...
class spell_gen_black_magic_enchant : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (spellInfo->SpellFamilyName == SPELLFAMILY_DRUID)
            if (spellInfo->SpellIconID == 2857 /*SPELL_INFECTED_WOUNDS*/ ||
                spellInfo->SpellIconID == 147 /*SPELL_SHRED*/ ||
                spellInfo->SpellFamilyFlags[1] & 0x400 /*SPELL_MANGLE_(CAT)*/)
                return true;
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_gen_black_magic_enchant::CheckProc);
    }
};
...
class spell_gen_area_aura_select_players : public AuraScript {
    ...
    void Register() override
    {
        DoCheckAreaTarget += AuraCheckAreaTargetFn(
            spell_gen_area_aura_select_players::CheckAreaTarget);
    }
};
...
public:
    spell_gen_select_target_count(Targets effTarget, uint8 count)
        : _effTarget(effTarget), _count(count)
    {
    }
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_gen_select_target_count::FilterTargets,
            EFFECT_ALL,
            _effTarget);
    }
...
class spell_gen_use_spell_base_level_check : public SpellScript {
    ...
    void Register() override
    {
        OnCheckCast += SpellCheckCastFn(
            spell_gen_use_spell_base_level_check::CheckRequirement);
    }
};
...
class spell_gen_proc_from_direct_damage : public AuraScript {
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        return !eventInfo.GetSpellInfo() ||
               !eventInfo.GetSpellInfo()->IsTargetingArea();
    }

    void Register() override
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_gen_proc_from_direct_damage::CheckProc);
    }
};
...
class spell_gen_proc_not_self : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (Unit* caster = GetCaster())
            if (Unit* target = eventInfo.GetActionTarget()) {
                ...
                uint32     spellID    = aurEff->GetSpellInfo()
                                     ->Effects[aurEff->GetEffIndex()]
                                     .TriggerSpell;
                caster->m_Events.AddEventAtOffset(
                    [caster, targetGUID, spellID]() {
                        if (Unit* target =
                                ObjectAccessor::GetUnit(*caster, targetGUID))
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(spell_gen_proc_not_self::HandleProc,
                                         EFFECT_0,
...
class spell_gen_baby_murloc_passive : public AuraScript {
    ...
    {
        GetAura()
            ->GetEffect(aurEff->GetEffIndex())
            ->SetPeriodicTimer(urand(60000, 120000));
    }
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_baby_murloc_passive::HandleApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_gen_baby_murloc_passive::HandlePeriodicTimer,
            EFFECT_0,
            SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_gen_baby_murloc : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_baby_murloc::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_baby_murloc::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_fixate : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_fixate::HandleScriptEffect,
                                           EFFECT_2,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_fixate_aura : public AuraScript {
    ...
    {
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_fixate_aura::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_periodic_knock_away : public AuraScript {
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        ...
        if (Unit* victim = GetUnitOwner()->GetVictim())
            GetUnitOwner()->CastSpell(
                victim,
                GetSpellInfo()->Effects[aurEff->GetEffIndex()].TriggerSpell,
                true);
    }
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_periodic_knock_away::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_gen_knock_away : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit())
            if (Creature* caster = GetCaster()->ToCreature())
                caster->GetThreatMgr().ModifyThreatByPercent(
                    target, -25); // Xinef: amount confirmed by onyxia and void
                                  // reaver notes
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_knock_away::HandleScriptEffect,
                          EFFECT_2,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_mod_radius_by_caster_scale : public SpellScript {
    PrepareSpellScript(spell_gen_mod_radius_by_caster_scale)
        ...
        bool Load() override
    {
        GetSpell()->SetSpellValue(
            SPELLVALUE_RADIUS_MOD,
            10000.0f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X));
        return true;
    }
...
class spell_gen_proc_reduced_above_60 : public AuraScript {
    ...
    {
        DoCheckProc +=
            AuraCheckProcFn(spell_gen_proc_reduced_above_60::CheckProc);
    }
};
...
class spell_gen_visual_dummy_stun : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_visual_dummy_stun::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_visual_dummy_stun::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_random_target32 : public SpellScript {
    ...
    void Register() override
    {
        OnDestinationTargetSelect +=
            SpellDestinationTargetSelectFn(spell_gen_random_target32::ModDest,
                                           EFFECT_0,
...
class spell_gen_hate_to_zero : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_hate_to_zero::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_focused_bursts : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit())
            GetCaster()->CastSpell(
                target,
                uint32(GetSpellInfo()->Effects[EFFECT_0].BasePoints) +
                    urand(1, 3),
                true);
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_focused_bursts::HandleDummy,
                          EFFECT_ALL,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_flurry_of_claws : public AuraScript {
    ...
    void OnPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Unit* target = GetUnitOwner()->SelectNearbyTarget(nullptr, 7.0f))
            if (target->GetEntry() == NPC_FRENZYHEART_RAVAGER ||
                target->GetEntry() == NPC_FRENZYHEART_HUNTER) {
                int32 basePoints = irand(1400, 2200);
                GetUnitOwner()->CastCustomSpell(SPELL_FLURRY_OF_CLAWS_DAMAGE,
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_flurry_of_claws::OnPeriodic,
                                 EFFECT_0,
...
class spell_gen_throw_back : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_throw_back::HandleScriptEffect,
                          EFFECT_1,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_haunted : public SpellScript {
    ...
    void HandleOnEffectHit(SpellEffIndex effIndex)
    {
        ...
        if (Unit* caster = GetCaster()) {
            ...
            if (Creature* haunt =
                    caster->SummonCreature(NPC_SCOURGE_HAUNT,
                                           pos,
                                           TEMPSUMMON_TIMED_DESPAWN,
                                           urand(10, 20) * IN_MILLISECONDS)) {
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(spell_gen_haunted::HandleOnEffectHit,
                                     EFFECT_0,
                                     SPELL_EFFECT_SUMMON);
    }
};
...
class spell_gen_haunted_aura : public AuraScript {
    ...
    void HandleEffectCalcPeriodic(AuraEffect const* /*aurEff*/,
                                  bool&  isPeriodic,
                                  int32& amplitude)
    {
        isPeriodic = true;
                              ...
                              AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster())
            if (Creature* haunt =
                    caster->FindNearestCreature(NPC_SCOURGE_HAUNT, 5.0f, true))
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_haunted_aura::HandleEffectPeriodic,
                                 EFFECT_1,
                                 SPELL_AURA_DUMMY);
        DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
            spell_gen_haunted_aura::HandleEffectCalcPeriodic,
            EFFECT_1,
            SPELL_AURA_DUMMY);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_haunted_aura::HandleOnEffectRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_absorb0_hitlimit1 : public AuraScript {
    ...
    {
        OnEffectAbsorb +=
            AuraEffectAbsorbFn(spell_gen_absorb0_hitlimit1::Absorb, EFFECT_0);
    }
};
...
class spell_gen_adaptive_warding : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_GEN_ADAPTIVE_WARDING_FIRE,
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        // find Mage Armor
        if (!GetTarget()->GetAuraEffect(SPELL_AURA_MOD_MANA_REGEN_INTERRUPT,
                                        SPELLFAMILY_MAGE,
                                        0x10000000,
                                        0x0,
                                        0x0))
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_gen_adaptive_warding::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_gen_av_drekthar_presence : public AuraScript {
    ...
    {
        DoCheckAreaTarget += AuraCheckAreaTargetFn(
            spell_gen_av_drekthar_presence::CheckAreaTarget);
    }
};
...
class spell_gen_burn_brutallus : public AuraScript {
    ...
    {
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_gen_burn_brutallus::HandleEffectPeriodicUpdate,
            EFFECT_0,
...
class spell_gen_cannibalize : public SpellScript {
    ...
    SpellCastResult CheckIfCorpseNear()
    {
        ...
        Acore::AnyDeadUnitSpellTargetInRangeCheck check(
            caster, max_range, GetSpellInfo(), TARGET_CHECK_CORPSE);
        Acore::WorldObjectSearcher<Acore::AnyDeadUnitSpellTargetInRangeCheck>
            searcher(caster, result, check);
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(
            spell_gen_cannibalize::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        OnCheckCast +=
            SpellCheckCastFn(spell_gen_cannibalize::CheckIfCorpseNear);
    }
};
...
class spell_gen_clear_debuffs : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_clear_debuffs::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_create_lance : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_CREATE_LANCE_ALLIANCE, SPELL_CREATE_LANCE_HORDE});
    }
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        if (Player* target = GetHitPlayer()) {
            if (target->GetTeamId() == TEAM_ALLIANCE)
                GetCaster()->CastSpell(
                    target, SPELL_CREATE_LANCE_ALLIANCE, true);
            else
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_create_lance::HandleScript,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_moss_covered_feet : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (eventInfo.GetActionTarget()) {
            eventInfo.GetActionTarget()->CastSpell(
                (Unit*)nullptr, SPELL_FALL_DOWN, true, nullptr, aurEff);
        }
    }
    ...
    {
        OnEffectProc +=
            AuraEffectProcFn(spell_gen_moss_covered_feet::HandleProc,
                             EFFECT_0,
                             SPELL_AURA_DUMMY);
    }
};
...
class spell_gen_netherbloom : public SpellScript {
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            target->CastSpell(
                target, SPELL_NETHERBLOOM_POLLEN_1 + urand(0, 4), true);
        }
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_netherbloom::HandleScript,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_nightmare_vine : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_nightmare_vine::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_obsidian_armor : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_GEN_OBSIDIAN_ARMOR_HOLY,
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (GetFirstSchoolInMask(eventInfo.GetSchoolMask()) ==
            SPELL_SCHOOL_NORMAL)
            return false;
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(
            spell_gen_obsidian_armor::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
class spell_gen_parachute : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_parachute::HandleEffectPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_gen_pet_summoned : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        if (player->GetLastPetNumber() &&
            player->CanResummonPet(player->GetLastPetSpell())) {
            PetType newPetType =
                (player->IsClass(CLASS_HUNTER, CLASS_CONTEXT_PET)) ? HUNTER_PET
                                                                   : SUMMON_PET;
            Pet* newPet = new Pet(player, newPetType);
            if (newPet->LoadPetFromDB(
                    player, 0, player->GetLastPetNumber(), true, 100)) {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_pet_summoned::HandleScript,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_remove_flight_auras : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_remove_flight_auras::HandleScript,
                          EFFECT_1,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_elune_candle : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_ELUNE_CANDLE_OMEN_HEAD,
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_elune_candle::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};

/*
There are only 3 possible flags Feign Death auras can apply: UNIT_DYNFLAG_DEAD,
UNIT_FLAG2_FEIGN_DEATH and UNIT_FLAG_PREVENT_EMOTES_FROM_CHAT_TEXT. Some auras
can apply only 2 flags

spell_gen_feign_death_all_flags applies all 3 flags
spell_gen_feign_death_no_dyn_flag applies no UNIT_DYNFLAG_DEAD (does not make
the creature appear dead) spell_gen_feign_death_no_prevent_emotes applies no
UNIT_FLAG_PREVENT_EMOTES_FROM_CHAT_TEXT

REACT_PASSIVE should be handled directly in scripts since not all creatures
should be passive. Otherwise creature will be not able to aggro or execute
MoveInLineOfSight events. Removing may cause more issues than already exists
*/
...
class spell_gen_feign_death_all_flags : public AuraScript {
    ...
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_gen_feign_death_all_flags::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectApplyFn(
            spell_gen_feign_death_all_flags::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_feign_death_no_dyn_flag : public AuraScript {
    ...
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_gen_feign_death_no_dyn_flag::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectApplyFn(
            spell_gen_feign_death_no_dyn_flag::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_feign_death_no_prevent_emotes : public AuraScript {
    ...
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_gen_feign_death_no_prevent_emotes::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectApplyFn(
...
class spell_gen_teleporting : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_TELEPORT_SPIRE_UP, SPELL_TELEPORT_SPIRE_DOWN});
    }
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_teleporting::HandleScript,
                                           EFFECT_0,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_pvp_trinket_wotf_shared_cd : public SpellScript {
    ...
    {
        Player* player = GetCaster()->ToPlayer();
        // This is only needed because spells cast from spell_linked_spell are
        // triggered by default Spell::SendSpellCooldown() skips all spells with
        // TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD
        player->AddSpellAndCategoryCooldowns(
            GetSpellInfo(),
            GetCastItem() ? GetCastItem()->GetEntry() : 0,
            GetSpell());
        ...
        if (player->GetTeamId(true) == TEAM_HORDE) {
            if (GetSpellInfo()->Id ==
                SPELL_WILL_OF_THE_FORSAKEN_COOLDOWN_TRIGGER) {
                WorldPacket data;
                player->BuildCooldownPacket(
                    data,
                    SPELL_COOLDOWN_FLAG_INCLUDE_GCD,
                    7744,
                    GetSpellInfo()
                        ->CategoryRecoveryTime); // Will of the forsaken
                ...
                player->BuildCooldownPacket(
                    ...
                    SPELL_COOLDOWN_FLAG_INCLUDE_GCD,
                    SPELL_PVP_TRINKET,
                    GetSpellInfo()->CategoryRecoveryTime); // PvP Trinket spell
    ...
    {
        AfterCast +=
            SpellCastFn(spell_pvp_trinket_wotf_shared_cd::HandleScript);
    }
};
...
class spell_gen_animal_blood : public AuraScript {
    ...
    {
        // Remove all auras with spell id 46221, except the one currently being
        // applied
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_gen_animal_blood::OnApply,
                                              EFFECT_0,
                                              SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gen_animal_blood::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_spawn_blood_pool : public SpellScript {
    ...
    void SetDest(SpellDestination& dest)
    {
        ...
        LiquidData liquidStatus =
            caster->GetMap()->GetLiquidData(caster->GetPhaseMask(),
                                            caster->GetPositionX(),
                                            caster->GetPositionY(),
                                            caster->GetPositionZ(),
                                            caster->GetCollisionHeight(),
                                            MAP_ALL_LIQUIDS);

        float    level = liquidStatus.Level > INVALID_HEIGHT
                             ? liquidStatus.Level
                             : caster->GetPositionZ();
        Position pos   = Position(caster->GetPositionX(),
                                caster->GetPositionY(),
                                level,
    ...
    void Register() override
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_spawn_blood_pool::SetDest, EFFECT_0, TARGET_DEST_CASTER);
    }
};
...
class spell_gen_divine_storm_cd_reset : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_divine_storm_cd_reset::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_profession_research : public SpellScript {
    ...
    SpellCastResult CheckRequirement()
    {
        if (HasDiscoveredAllSpells(GetSpellInfo()->Id,
                                   GetCaster()->ToPlayer())) {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        if (uint32 discoveredSpellId =
                GetExplicitDiscoverySpell(spellId, caster))
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_gen_profession_research::CheckRequirement);
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_profession_research::HandleScript,
                          EFFECT_1,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_clone : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        PreventHitDefaultEffect(effIndex);
        if (GetHitUnit()) {
            GetHitUnit()->CastSpell(
                GetCaster(), uint32(GetEffectValue()), true);
        }
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_clone::HandleScriptEffect,
                                           EFFECT_1,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
        OnEffectHitTarget += SpellEffectFn(spell_gen_clone::HandleScriptEffect,
                                           EFFECT_2,
...
class spell_gen_clone_weapon : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_clone_weapon::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_clone_weapon_aura : public AuraScript {
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        case SPELL_COPY_WEAPON_3_AURA: {
            ...
            if (Player* player = caster->ToPlayer()) {
                if (Item* mainItem = player->GetItemByPos(
                ...
                target->SetUInt32Value(
                    ...
                    caster->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID));
            break;
        }
        ...
        case SPELL_COPY_OFFHAND_2_AURA: {
            ...
            if (Player* player = caster->ToPlayer()) {
                if (Item* offItem = player->GetItemByPos(
                        INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1,
                                           offItem->GetEntry());
            }
            else
                target->SetUInt32Value(
                    UNIT_VIRTUAL_ITEM_SLOT_ID + 1,
                    caster->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1));
            break;
        }
        case SPELL_COPY_RANGED_AURA: {
            ...
            if (Player* player = caster->ToPlayer()) {
                if (Item* rangedItem = player->GetItemByPos(
                        INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED))
                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2,
                                           rangedItem->GetEntry());
            }
            else
                target->SetUInt32Value(
                    UNIT_VIRTUAL_ITEM_SLOT_ID + 2,
                    caster->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2));
            break;
        }
        default:
            break;
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_clone_weapon_aura::OnApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_DUMMY,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_clone_weapon_aura::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_DUMMY,
                               AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
...
class spell_gen_seaforium_blast : public SpellScript {
    ...
    void AchievementCredit(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* originalCaster = GetOriginalCaster())
            if (GameObject* go = GetHitGObj())
                if (go->GetGOValue()->Building.Health > 0 &&
                    go->GetGOInfo()->type ==
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_seaforium_blast::AchievementCredit,
                          EFFECT_1,
...
class spell_gen_turkey_marker : public AuraScript {
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        ...
        if (_applyTimes.front() + GetMaxDuration() <
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_turkey_marker::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_DUMMY);
    }
...
class spell_gen_lifeblood : public AuraScript {
    ...
    {
        if (Unit* owner = GetUnitOwner())
            amount += int32(CalculatePct(owner->GetMaxHealth(),
                                         1.5f / aurEff->GetTotalTicks()));
    }

    void Register() override
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_gen_lifeblood::CalculateAmount,
                                   EFFECT_0,
...
class spell_gen_allow_cast_from_item_only : public SpellScript {
    ...
    {
        OnCheckCast += SpellCheckCastFn(
            spell_gen_allow_cast_from_item_only::CheckRequirement);
    }
};
...
class spell_gen_vehicle_scaling : public SpellScript {
    ...
    SpellCastResult CheckSeat()
    {
        if (Vehicle* veh = GetCaster()->GetVehicle())
            if (const VehicleSeatEntry* seatEntry =
                    veh->GetSeatForPassenger(GetCaster()))
...
class spell_gen_vehicle_scaling_aura : public AuraScript {
    ...
    bool Load() override
    {
        return GetCaster() && GetCaster()->GetTypeId() == TYPEID_PLAYER &&
               GetOwner()->GetTypeId() == TYPEID_UNIT;
    }
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_gen_vehicle_scaling_aura::CalculateAmount,
            EFFECT_0,
...
class spell_gen_oracle_wolvar_reputation : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        // Set rep to baserep + basepoints (expecting spillover for oposite
        // faction -> become hated) Not when player already has equal or higher
        // rep with this faction
        if (player->GetReputationMgr().GetReputation(factionEntry) <= repChange)
            player->GetReputationMgr().SetReputation(
                factionEntry, static_cast<float>(repChange));

        // EFFECT_INDEX_2 most likely update at war state, we already handle
        // this in SetReputation
    }

    void Register() override
    {
        OnEffectHit +=
            SpellEffectFn(spell_gen_oracle_wolvar_reputation::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_damage_reduction_aura : public AuraScript {
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (target->HasAura(SPELL_DAMAGE_REDUCTION_AURA) &&
            !(target->HasAura(SPELL_BLESSING_OF_SANCTUARY) ||
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_damage_reduction_aura::OnApply,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_gen_damage_reduction_aura::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
...
class spell_gen_dummy_trigger : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_PERSISTANT_SHIELD_TRIGGERED, SPELL_PERSISTANT_SHIELD});
    }
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_dummy_trigger::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_spirit_healer_res : public SpellScript {
    ...
    bool Load() override
    {
        return GetOriginalCaster() &&
               GetOriginalCaster()->GetTypeId() == TYPEID_PLAYER;
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_spirit_healer_res::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_gadgetzan_transporter_backfire : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /* effIndex */)
    {
        ...
        if (r < 20) // Transporter Malfunction - 1/6 polymorph
            caster->CastSpell(
                caster, SPELL_TRANSPORTER_MALFUNCTION_POLYMORPH, true);
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_gadgetzan_transporter_backfire::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_gnomish_transporter : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_TRANSPORTER_SUCCESS, SPELL_TRANSPORTER_FAILURE});
    }

    void HandleDummy(SpellEffIndex /* effIndex */)
    {
        GetCaster()->CastSpell(GetCaster(),
                               roll_chance_i(50) ? SPELL_TRANSPORTER_SUCCESS
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_gnomish_transporter::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_gryphon_wyvern_mount_check : public AuraScript {
    ...
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_gen_gryphon_wyvern_mount_check::HandleEffectPeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_gen_dalaran_disguise : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_SUNREAVER_DISGUISE_TRIGGER,
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* player = GetHitPlayer()) {
            ...
            case SPELL_SUNREAVER_DISGUISE_TRIGGER:
                spellId = gender ? SPELL_SUNREAVER_DISGUISE_FEMALE
                                 : SPELL_SUNREAVER_DISGUISE_MALE;
                break;
            case SPELL_SILVER_COVENANT_DISGUISE_TRIGGER:
                spellId = gender ? SPELL_SILVER_COVENANT_DISGUISE_FEMALE
                                 : SPELL_SILVER_COVENANT_DISGUISE_MALE;
                break;
            default:
                break;
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_dalaran_disguise::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
/* DOCUMENTATION: Break-Shield spells
    ...
    In the following script we handle the SCRIPT_EFFECT for effIndex EFFECT_0
   and EFFECT_1.
        - When handling EFFECT_0 we're in the "Spells on vehicle bar used by
   players" case and we'll trigger "Spells cast by players triggered by script"
        - When handling EFFECT_1 we're in the "Spells cast by players triggered
   by script" or "Spells cast by NPCs on players" so we'll search for the first
   defend layer and drop it.
*/
...
/* 62575, 62626, 64342, 64590, 64595, 64686, 65147, 66480, 68504 -
   Shield-Breaker 63233 - Necrocution 63825 - Sundering Thrust 64507 -
...
class spell_gen_break_shield : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        switch (effIndex) {
        case EFFECT_0: // On spells wich trigger the damaging spell (and also
                       // the visual)
        {
            uint32 spellId;
        ...
        case EFFECT_1: // On damaging spells, for removing a defend layer
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = auras.begin();
                ...
                if (Aura* aura = itr->second->GetBase()) {
                    ...
                    if (auraInfo && auraInfo->SpellIconID == 2007 &&
                        aura->HasEffectType(
                            SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN)) {
                        aura->ModStackAmount(-1, AURA_REMOVE_BY_ENEMY_SPELL);
                        // Remove dummys from rider (Necessary for updating
                        // visual shields)
                        if (Unit* rider = target->GetCharmer())
                            if (Aura* defend = rider->GetAura(aura->GetId()))
                                defend->ModStackAmount(
                                    -1, AURA_REMOVE_BY_ENEMY_SPELL);
                        break;
                    }
                }
            }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_break_shield::HandleScriptEffect,
                          EFFECT_FIRST_FOUND,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
/* DOCUMENTATION: Charge spells
    ...
    In the following script we handle the SCRIPT_EFFECT and CHARGE
        - When handling SCRIPT_EFFECT:
            + EFFECT_0: Corresponds to "Spells on vehicle bar used by players"
   and we make player's mount cast the charge effect on the current target ,
   "Spells cast by player's mounts triggered by script").
            + EFFECT_1 and EFFECT_2: Triggered when "Spells cast by player's
   mounts triggered by script" hits target, corresponding to "Spells cast by
   players on the target triggered by script" and "Spells cast by NPCs on
   players" and we check Defend layers and drop a charge of the first found.
        - When handling CHARGE:
            + Only launched for "Spells cast by player's mounts triggered by
   script", makes the player cast the damaging spell on target with a small
...
// 62563, 62874, 62960, 63003, 63010, 63661, 64591, 66481, 68282, 68284, 68321,
// 68498, 68501 - Charge
class spell_gen_mounted_charge : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
                       ...
                       // the visual)
        {
            ...
            // If target isn't a training dummy there's a chance of failing the
            // charge
    ...
    void Register() override
    {
        ...
        if (spell->HasEffect(SPELL_EFFECT_SCRIPT_EFFECT))
            OnEffectHitTarget +=
                SpellEffectFn(spell_gen_mounted_charge::HandleScriptEffect,
                              EFFECT_FIRST_FOUND,
                              SPELL_EFFECT_SCRIPT_EFFECT);

        if (spell->Effects[EFFECT_0].Effect == SPELL_EFFECT_CHARGE)
            OnEffectHitTarget +=
                SpellEffectFn(spell_gen_mounted_charge::HandleChargeEffect,
                              EFFECT_0,
                              SPELL_EFFECT_CHARGE);
    }
};
...
class spell_gen_defend : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_VISUAL_SHIELD_1,
                                  SPELL_VISUAL_SHIELD_2,
                                  SPELL_VISUAL_SHIELD_3});
    }

    void RefreshVisualShields(AuraEffect const* aurEff,
                              AuraEffectHandleModes /*mode*/)
    {
        if (GetCaster()) {
            Unit* target = GetTarget();
    ...
    void RemoveVisualShields(AuraEffect const* /*aurEff*/,
                             AuraEffectHandleModes /*mode*/)
    {
        for (uint8 i = 0; i < GetSpellInfo()->StackAmount; ++i)
    ...
    void RemoveDummyFromDriver(AuraEffect const* /*aurEff*/,
                               AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster())
    ...
    void Register() override
    {
        ...
        if (spell->Effects[EFFECT_0].ApplyAuraName ==
            SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN) {
            AfterEffectApply +=
                AuraEffectApplyFn(spell_gen_defend::RefreshVisualShields,
                                  EFFECT_0,
                                  SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN,
                                  AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_gen_defend::RemoveVisualShields,
                                   EFFECT_0,
                                   SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN,
        ...
        if (spell->Effects[EFFECT_2].ApplyAuraName ==
            SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN)
            OnEffectRemove +=
                AuraEffectRemoveFn(spell_gen_defend::RemoveDummyFromDriver,
                                   EFFECT_2,
                                   SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN,
                                   AURA_EFFECT_HANDLE_REAL);
...
class spell_gen_throw_shield : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_throw_shield::HandleScriptEffect,
                          EFFECT_1,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_tournament_duel : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_ON_TOURNAMENT_MOUNT, SPELL_MOUNTED_DUEL});
    }
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Unit* rider = GetCaster()->GetCharmer()) {
            if (Player* playerTarget = GetHitPlayer()) {
                if (playerTarget->HasAura(SPELL_ON_TOURNAMENT_MOUNT) &&
                ...
                if (unitTarget->GetCharmer() &&
                    unitTarget->GetCharmer()->GetTypeId() == TYPEID_PLAYER &&
                    unitTarget->GetCharmer()->HasAura(
                        SPELL_ON_TOURNAMENT_MOUNT))
                    rider->CastSpell(
                        unitTarget->GetCharmer(), SPELL_MOUNTED_DUEL, true);
            }
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_tournament_duel::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_summon_tournament_mount : public SpellScript {
    ...
    {
        OnCheckCast += SpellCheckCastFn(
            spell_gen_summon_tournament_mount::CheckIfLanceEquiped);
    }
};
...
class spell_gen_on_tournament_mount : public AuraScript {
    ...
    void HandleApplyEffect(AuraEffect const* /*aurEff*/,
                           AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster()) {
    ...
    void HandleRemoveEffect(AuraEffect const* /*aurEff*/,
                            AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster())
    ...
    uint32 GetPennatSpellId(Player* player, Unit* mount)
    {
        ...
        case NPC_STORMWIND_STEED: {
            ...
            else if (player->GetQuestRewardStatus(QUEST_VALIANT_OF_STORMWIND) ||
                     player->GetQuestRewardStatus(QUEST_A_VALIANT_OF_STORMWIND))
        ...
        case NPC_DARK_SPEAR_RAPTOR: {
            ...
            else if (player->GetQuestRewardStatus(QUEST_VALIANT_OF_SEN_JIN) ||
                     player->GetQuestRewardStatus(QUEST_A_VALIANT_OF_SEN_JIN))
        ...
        case NPC_DARNASSIAN_NIGHTSABER: {
            ...
            else if (player->GetQuestRewardStatus(QUEST_VALIANT_OF_DARNASSUS) ||
                     player->GetQuestRewardStatus(QUEST_A_VALIANT_OF_DARNASSUS))
        ...
        case NPC_IRONFORGE_RAM: {
            ...
            else if (player->GetQuestRewardStatus(QUEST_VALIANT_OF_IRONFORGE) ||
                     player->GetQuestRewardStatus(QUEST_A_VALIANT_OF_IRONFORGE))
        ...
        case NPC_FORSAKEN_WARHORSE: {
            ...
            else if (player->GetQuestRewardStatus(QUEST_VALIANT_OF_UNDERCITY) ||
                     player->GetQuestRewardStatus(QUEST_A_VALIANT_OF_UNDERCITY))
        ...
        case NPC_ORGRIMMAR_WOLF: {
            ...
            else if (player->GetQuestRewardStatus(QUEST_VALIANT_OF_ORGRIMMAR) ||
                     player->GetQuestRewardStatus(QUEST_A_VALIANT_OF_ORGRIMMAR))
        ...
        case NPC_ARGENT_WARHORSE: {
            if (player->HasAchieved(ACHIEVEMENT_CHAMPION_ALLIANCE) ||
                player->HasAchieved(ACHIEVEMENT_CHAMPION_HORDE))
                return player->IsClass(CLASS_DEATH_KNIGHT)
                           ? SPELL_PENNANT_EBON_BLADE_CHAMPION
                           : SPELL_PENNANT_ARGENT_CRUSADE_CHAMPION;
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_gen_on_tournament_mount::HandleApplyEffect,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        OnEffectRemove += AuraEffectRemoveFn(
            spell_gen_on_tournament_mount::HandleRemoveEffect,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
...
class spell_gen_tournament_pennant : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_tournament_pennant::HandleApplyEffect,
                              EFFECT_0,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
...
class spell_gen_ds_flush_knockback : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        // Here the target is the water spout and determines the position where
        // the player is knocked from
        if (Unit* target = GetHitUnit()) {
            if (Player* player = GetCaster()->ToPlayer()) {
                float horizontalSpeed =
                    20.0f + (40.0f - GetCaster()->GetDistance(target));
                float verticalSpeed = 8.0f;
                // This method relies on the Dalaran Sewer map disposition and
                // Water Spout position What we do is knock the player from a
                // position exactly behind him and at the end of the pipe
                player->KnockbackFrom(target->GetPositionX(),
                                      player->GetPositionY(),
                                      horizontalSpeed,
                                      verticalSpeed);
            }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_ds_flush_knockback::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
/* 20625 - Ritual of Doom Sacrifice (spell_gen_default_count_pct_from_max_hp)
   ...
   38441 - Cataclysmic Bolt (spell_gen_50pct_count_pct_from_max_hp) 66316,
   67100, 67101, 67102 - Spinning Pain Spike
   (spell_gen_50pct_count_pct_from_max_hp) 41360 - L5 Arcane Charge
   (spell_gen_100pct_count_pct_from_max_hp) 33711/38794 - Murmur's Touch
   */
class spell_gen_count_pct_from_max_hp : public SpellScript {
    PrepareSpellScript(spell_gen_count_pct_from_max_hp)

        public : spell_gen_count_pct_from_max_hp(int32 damagePct = 0)
        : SpellScript(), _damagePct(damagePct)
    {
    }
...
class spell_gen_despawn_self : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        if (GetSpellInfo()->Effects[effIndex].Effect == SPELL_EFFECT_DUMMY ||
            GetSpellInfo()->Effects[effIndex].Effect ==
                SPELL_EFFECT_SCRIPT_EFFECT)
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_despawn_self::HandleDummy, EFFECT_ALL, SPELL_EFFECT_ANY);
    }
};
...
class spell_gen_paralytic_poison : public AuraScript {
    ...
    void HandleStun(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        GetTarget()->CastSpell(
            (Unit*)nullptr, SPELL_PARALYSIS, true, nullptr, aurEff);
    }

    void Register() override
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gen_paralytic_poison::HandleStun,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_DAMAGE,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_prevent_emotes : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_prevent_emotes::HandleEffectApply,
                              EFFECT_FIRST_FOUND,
                              SPELL_AURA_ANY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(spell_gen_prevent_emotes::OnRemove,
                                             EFFECT_FIRST_FOUND,
                                             SPELL_AURA_ANY,
                                             AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_blade_warding : public AuraScript {
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo =
            sSpellMgr->GetSpellInfo(SPELL_GEN_BLADE_WARDING_TRIGGERED);
        if (!caster) {
            return;
        }
        ...
        caster->CastCustomSpell(SPELL_GEN_BLADE_WARDING_TRIGGERED,
                                SPELLVALUE_BASE_POINT0,
                                bp,
                                eventInfo.GetActor(),
                                TRIGGERED_FULL_MASK,
    ...
    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_gen_blade_warding::HandleProc,
                                         EFFECT_1,
                                         SPELL_AURA_PROC_TRIGGER_SPELL);
    }
};
...
public:
    ...
    void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetTargetApplication() &&
            ...
            GetTargetApplication()->GetRemoveMode() !=
        ...
        // final heal
        GetTarget()->CastSpell(
            GetTarget(), _spellId, true, nullptr, aurEff, GetCasterGUID());
    }

    void Register() override
    {
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gen_lifebloom::AfterRemove,
                               EFFECT_0,
                               SPELL_AURA_PERIODIC_HEAL,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
public:
    ...
    void AfterApply(AuraEffect const* /*aurEff*/,
                    AuraEffectHandleModes /*mode*/)
    {
        if (GetCaster())
    ...
    void AfterRemove(AuraEffect const* /*aurEff*/,
                     AuraEffectHandleModes /*mode*/)
    {
        if (GetCaster())
            if (Unit* owner = GetCaster()->GetOwner())
                if (owner->GetTypeId() ==
                    TYPEID_PLAYER) /// @todo this check is maybe wrong
                    owner->ToPlayer()->RemovePet(
                        nullptr, PET_SAVE_NOT_IN_SLOT, true);
    }

    void Register() override
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_gen_summon_elemental::AfterApply,
                              EFFECT_1,
                              SPELL_AURA_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gen_summon_elemental::AfterRemove,
                               EFFECT_1,
                               SPELL_AURA_DUMMY,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
public:
    spell_gen_mount(uint32 mount0,
                    uint32 mount60,
                    uint32 mount100,
                    uint32 mount150,
                    uint32 mount280,
                    uint32 mount310)
        : SpellScript(), _mount0(mount0), _mount60(mount60),
          _mount100(mount100), _mount150(mount150), _mount280(mount280),
          _mount310(mount310)
    {
    }
    ...
    void HandleMount(SpellEffIndex effIndex)
    {
        ...
        if (Player* target = GetHitPlayer()) {
            ...
            target->RemoveAurasByType(
                SPELL_AURA_MOUNTED, ObjectGuid::Empty, GetHitAura());

            // Triggered spell id dependent on riding skill and zone
            bool   canFly = false;
            uint32 map    = GetVirtualMapForMapAndZone(target->GetMapId(),
                                                    target->GetZoneId());
            if (map == 530 ||
                (map == 571 && target->HasSpell(SPELL_COLD_WEATHER_FLYING)))
                canFly = true;
            ...
            Battlefield* Bf =
                sBattlefieldMgr->GetBattlefieldToZoneId(target->GetZoneId());
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_mount::HandleMount, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_gen_upper_deck_create_foam_sword : public SpellScript {
    ...
    {
        if (Player* player = GetHitPlayer()) {
            static uint32 const itemId[5] = {ITEM_FOAM_SWORD_GREEN,
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_upper_deck_create_foam_sword::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_bonked : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_BONKED, SPELL_FOAM_SWORD_DEFEAT, SPELL_ON_GUARD});
    }
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer()) {
            ...
            if (Aura const* onGuardAura = target->GetAura(SPELL_ON_GUARD)) {
                if (Item* item =
                        target->GetItemByGuid(onGuardAura->GetCastItemGUID())) {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_gen_bonked::HandleScript,
                                           EFFECT_1,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_gift_of_naaru : public AuraScript {
                         ...
                         bool& /*canBeRecalculated*/)
    {
        ...
        case SPELLFAMILY_PRIEST:
            heal = 1.885f * float(GetCaster()->SpellBaseDamageBonusDone(
                                GetSpellInfo()->GetSchoolMask()));
            break;
        case SPELLFAMILY_PALADIN:
        case SPELLFAMILY_SHAMAN:
            heal = std::max(
                1.885f * float(GetCaster()->SpellBaseDamageBonusDone(
                             GetSpellInfo()->GetSchoolMask())),
                1.1f *
                    float(GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK)));
            break;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            heal = 1.1f *
                   float(std::max(
                       GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK),
                       GetCaster()->GetTotalAttackPowerValue(RANGED_ATTACK)));
            break;
    ...
    {
        DoEffectCalcAmount +=
            AuraEffectCalcAmountFn(spell_gen_gift_of_naaru::CalculateAmount,
                                   EFFECT_0,
                                   SPELL_AURA_PERIODIC_HEAL);
    }
};
...
class spell_gen_replenishment : public SpellScript {
    ...
    {
        return ValidateSpellInfo(
            {SPELL_REPLENISHMENT, SPELL_INFINITE_REPLENISHMENT});
    }
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_gen_replenishment::RemoveInvalidTargets,
            EFFECT_ALL,
...
class spell_gen_replenishment_aura : public AuraScript {
    ...
    bool Load() override { return GetUnitOwner()->GetPower(POWER_MANA); }
    ...
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_gen_replenishment_aura::CalculateAmount,
            EFFECT_0,
...
uint8 const EmoteArray[3] = {
    EMOTE_ONE_SHOT_CHEER, EMOTE_ONE_SHOT_EXCLAMATION, EMOTE_ONE_SHOT_APPLAUD};
...
class spell_gen_spectator_cheer_trigger : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_spectator_cheer_trigger::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_vendor_bark_trigger : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_vendor_bark_trigger::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_whisper_gulch_yogg_saron_whisper : public AuraScript {
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(
            (Unit*)nullptr, SPELL_YOGG_SARON_WHISPER_DUMMY, true);
    }
...
class spell_gen_eject_all_passengers : public SpellScript {
    ...
    {
        AfterHit +=
            SpellHitFn(spell_gen_eject_all_passengers::RemoveVehicleAuras);
    }
};
...
class spell_gen_eject_passenger : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_eject_passenger::EjectPassenger,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
/* 37727 - Touch of Darkness
   ...
   56524 - Acid Breath */
// Used for some spells cast by vehicles or charmed creatures that do not send a
// cooldown event on their own.
class spell_gen_charmed_unit_spell_cooldown : public SpellScript {
    ...
    void HandleCast()
    {
        ...
        if (Player* owner = caster->GetCharmerOrOwnerPlayerOrPlayerItself()) {
            WorldPacket data;
            caster->BuildCooldownPacket(data,
                                        SPELL_COOLDOWN_FLAG_NONE,
                                        GetSpellInfo()->Id,
                                        GetSpellInfo()->RecoveryTime);
    ...
    {
        OnCast +=
            SpellCastFn(spell_gen_charmed_unit_spell_cooldown::HandleCast);
    }
};
...
class spell_contagion_of_rot : public AuraScript {
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        while (Aura* aur = GetUnitOwner()->GetOwnedAura(
                   7103, ObjectGuid::Empty, ObjectGuid::Empty, 0, GetAura())) {
            GetUnitOwner()->RemoveOwnedAura(aur);
        }
        // 7102 => contagion of rot casted by mobs
        while (Aura* aur = GetUnitOwner()->GetOwnedAura(
                   7102, ObjectGuid::Empty, ObjectGuid::Empty, 0, GetAura())) {
            GetUnitOwner()->RemoveOwnedAura(aur);
        }
    }
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_contagion_of_rot::OnApply,
                                              EFFECT_2,
                                              SPELL_AURA_PROC_TRIGGER_SPELL,
                                              AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_silithyst : public AuraScript {
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetCaster()->IsMounted()) {
            GetCaster()->SummonGameObject(181597,
                                          ...
                                          0.0f,
                                          0.0f,
                                          10 * IN_MILLISECONDS * MINUTE);
        }
    }

    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_silithyst::OnApply,
                                              EFFECT_0,
                                              SPELL_AURA_MOD_DECREASE_SPEED,
                                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(spell_silithyst::OnRemove,
                                                EFFECT_0,
                                                SPELL_AURA_MOD_DECREASE_SPEED,
                                                AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_gen_holiday_buff_food : public AuraScript {
    ...
    void TriggerFoodBuff(AuraEffect* aurEff)
    {
        if (aurEff->GetTickNumber() == 10 && GetUnitOwner()) {
            GetUnitOwner()->CastSpell(
                GetUnitOwner(),
                SPELL_WELL_FED,
                TriggerCastFlags(TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS));
        }
    }
    ...
    {
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_gen_holiday_buff_food::TriggerFoodBuff,
            EFFECT_0,
...
class spell_gen_arcane_charge : public SpellScript {
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_gen_arcane_charge::CheckRequirement);
    }
};
...
class spell_gen_remove_impairing_auras : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_remove_impairing_auras::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
public:
    spell_gen_consume(uint32 spellId1, uint32 spellId2)
        : AuraScript(), _spellId1(spellId1), _spellId2(spellId2)
    {
    }
        ...
        if (GetTargetApplication()) {
            if (Unit* caster = GetCaster()) {
                if (GetTargetApplication()->GetRemoveMode() ==
                    ...
                    caster->CastSpell(
                        GetTarget(), SPELL_CONSUME_SPIT_OUT, true);
                }
            }
        }
    }
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_gen_consume::AfterRemove,
                                                EFFECT_1,
                                                SPELL_AURA_MOD_STUN,
                                                AURA_EFFECT_HANDLE_REAL);
        OnEffectUpdatePeriodic +=
            AuraEffectUpdatePeriodicFn(spell_gen_consume::HandleProc,
                                       EFFECT_2,
                                       SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
...
public:
    spell_gen_apply_aura_after_expiration(uint32 spellId,
                                          uint32 effect,
                                          uint32 aura)
        : AuraScript(), _spellId(spellId), _effect(effect), _aura(aura)
    {
    }
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_gen_apply_aura_after_expiration::AfterRemove,
            _effect,
            _aura,
            AURA_EFFECT_HANDLE_REAL);
    }
...
inline int32 SkillGainChance(uint32 SkillValue,
                             uint32 GrayLevel,
                             uint32 GreenLevel,
                             uint32 YellowLevel)
{
...
class spell_gen_basic_campfire : public SpellScript {
    ...
    {
        if (Unit* caster = GetCaster()) {
            if (caster->GetMap()->GetGameObjectFloor(caster->GetPhaseMask(),
                                                     caster->GetPositionX(),
                                                     caster->GetPositionY(),
                                                     caster->GetPositionZ()) ==
                -G3D::finf()) {
                float ground = caster->GetMap()->GetHeight(
                    dest._position.GetPositionX(),
                    dest._position.GetPositionY(),
                    dest._position.GetPositionZ() +
                        caster->GetCollisionHeight() * 0.5f);
    ...
    {
        OnDestinationTargetSelect +=
            SpellDestinationTargetSelectFn(spell_gen_basic_campfire::ModDest,
                                           EFFECT_0,
                                           TARGET_DEST_CASTER_SUMMON);
        OnEffectHit +=
            SpellEffectFn(spell_gen_basic_campfire::ModifyCookingSkill,
                          EFFECT_0,
...
class spell_freezing_circle : public SpellScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({SPELL_FREEZING_CIRCLE_PIT_OF_SARON_NORMAL,
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_freezing_circle::HandleDamage,
                                           EFFECT_1,
                                           SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_gen_threshalisk_charge : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(spell_gen_threshalisk_charge::HandleDummy,
                                     EFFECT_0,
                                     SPELL_EFFECT_DUMMY);
    }
};
...
class spell_gen_shriveling_gaze : public AuraScript {
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_shriveling_gaze::HandleApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_shriveling_gaze::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_gen_curse_of_pain : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_gen_curse_of_pain::OnPeriodic,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_DAMAGE);
    }
};
...
class spell_gen_spirit_of_competition_participant : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* player = GetHitPlayer()) {
            player->CastSpell(
                player, SPELL_SPIRIT_OF_COMPETITION_PARTICIPANT_EFFECT, true);
            ...
            MailDraft(MAIL_THE_COMPETITIORS_TABARD)
                ...
                .SendMailTo(trans,
                            player,
                            MailSender(NPC_SPIRIT_OF_COMPETITION),
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_gen_spirit_of_competition_participant::HandleScript,
            EFFECT_0,
            SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_spirit_of_competition_winner : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* player = GetHitPlayer()) {
            player->CastSpell(
                player, SPELL_SPIRIT_OF_COMPETITION_WINNER_EFFECT, true);
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_spirit_of_competition_winner::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_planting_scourge_banner : public SpellScript {
    ...
    {
        if (GameObject* tent =
                GetCaster()->FindNearestGameObject(GO_COMMAND_TENT, 20.0f))
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_gen_planting_scourge_banner::CheckCast);
    }
};
...
class spell_gen_jubling_cooldown : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gen_jubling_cooldown::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_gen_yehkinya_bramble : public SpellScript {
    ...
    {
        if (Unit* target = GetExplTargetUnit())
            if ((target->GetEntry() == NPC_VALE_SCREECHER ||
                 target->GetEntry() == NPC_ROGUE_VALE_SCREECHER) &&
                target->isDead())
                return SPELL_CAST_OK;
...
// 35244 - Choking Vines
enum ChokingVines { SPELL_CHOKING_VINES = 35244, SPELL_CHOKING_WOUND = 35247 };
...
class spell_gen_choking_vines : public AuraScript {
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* target = GetTarget()) {
            if (GetStackAmount() == GetSpellInfo()->StackAmount) // 5 stacks
            {
                ...
                target->CastSpell(target,
                                  SPELL_CHOKING_WOUND,
                                  true); // Unknown if it's a self cast or
                                         // casted by the source on 5th
            }
        }
    }
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_gen_choking_vines::OnApply,
                              EFFECT_0,
                              SPELL_AURA_MOD_DECREASE_SPEED,
                              AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
...
void AddSC_generic_spell_scripts()
{
    ...
    RegisterSpellAndAuraScriptPair(spell_gen_reduced_above_60,
                                   spell_gen_reduced_above_60_aura);
    RegisterSpellScriptWithArgs(spell_gen_relocaste_dest,
                                "spell_q10838_demoniac_scryer_visual",
                                0,
                                0,
                                20.0,
                                0);
    RegisterSpellScriptWithArgs(spell_gen_relocaste_dest,
                                "spell_q20438_q24556_aquantos_laundry",
                                0,
                                0,
                                7.0f,
                                0);
    ...
    RegisterSpellScriptWithArgs(
        spell_gen_disabled_above_level, "spell_gen_disabled_above_73", 73);
    RegisterSpellScriptWithArgs(
        spell_gen_disabled_above_level, "spell_gen_disabled_above_70", 70);
    ...
    RegisterSpellScriptWithArgs(spell_gen_select_target_count,
                                "spell_gen_select_target_count_15_1",
                                TARGET_UNIT_SRC_AREA_ENEMY,
                                1);
    RegisterSpellScriptWithArgs(spell_gen_select_target_count,
                                "spell_gen_select_target_count_15_2",
                                TARGET_UNIT_SRC_AREA_ENEMY,
                                2);
    RegisterSpellScriptWithArgs(spell_gen_select_target_count,
                                "spell_gen_select_target_count_15_5",
                                TARGET_UNIT_SRC_AREA_ENEMY,
                                5);
    RegisterSpellScriptWithArgs(spell_gen_select_target_count,
                                "spell_gen_select_target_count_7_1",
                                TARGET_UNIT_SRC_AREA_ENTRY,
                                1);
    RegisterSpellScriptWithArgs(spell_gen_select_target_count,
                                "spell_gen_select_target_count_24_1",
                                TARGET_UNIT_CONE_ENEMY_24,
                                1);
    RegisterSpellScriptWithArgs(spell_gen_select_target_count,
                                "spell_gen_select_target_count_30_1",
                                TARGET_UNIT_SRC_AREA_ALLY,
                                1);
    ...
    RegisterSpellAndAuraScriptPair(spell_gen_vehicle_scaling,
    ...
    RegisterSpellScriptWithArgs(spell_gen_dalaran_disguise,
                                "spell_gen_sunreaver_disguise");
    RegisterSpellScriptWithArgs(spell_gen_dalaran_disguise,
                                "spell_gen_silver_covenant_disguise");
    RegisterSpellScript(spell_gen_elune_candle);
    RegisterSpellScriptWithArgs(spell_gen_break_shield,
    ...
    RegisterSpellScriptWithArgs(spell_gen_count_pct_from_max_hp,
                                "spell_gen_default_count_pct_from_max_hp");
    RegisterSpellScriptWithArgs(spell_gen_count_pct_from_max_hp,
                                "spell_gen_10pct_count_pct_from_max_hp",
                                10);
    RegisterSpellScriptWithArgs(spell_gen_count_pct_from_max_hp,
                                "spell_gen_50pct_count_pct_from_max_hp",
                                50);
    RegisterSpellScriptWithArgs(spell_gen_count_pct_from_max_hp,
                                "spell_gen_100pct_count_pct_from_max_hp",
                                100);
    ...
    RegisterSpellScriptWithArgs(spell_gen_lifebloom,
                                "spell_hexlord_lifebloom",
                                SPELL_HEXLORD_MALACRASS_LIFEBLOOM_FINAL_HEAL);
    RegisterSpellScriptWithArgs(spell_gen_lifebloom,
                                "spell_tur_ragepaw_lifebloom",
                                SPELL_TUR_RAGEPAW_LIFEBLOOM_FINAL_HEAL);
    RegisterSpellScriptWithArgs(spell_gen_lifebloom,
                                "spell_cenarion_scout_lifebloom",
    ...
    RegisterSpellScriptWithArgs(spell_gen_summon_elemental,
                                "spell_gen_summon_fire_elemental",
                                SPELL_SUMMON_FIRE_ELEMENTAL);
    RegisterSpellScriptWithArgs(spell_gen_summon_elemental,
                                "spell_gen_summon_earth_elemental",
                                SPELL_SUMMON_EARTH_ELEMENTAL);
    RegisterSpellScriptWithArgs(spell_gen_mount,
                                "spell_big_blizzard_bear",
                                0,
    ...
    RegisterSpellScriptWithArgs(spell_gen_mount,
                                ...
                                0);
    RegisterSpellScriptWithArgs(spell_gen_mount,
                                "spell_winged_steed_of_the_ebon_blade",
                                0,
                                0,
                                0,
    ...
    RegisterSpellAndAuraScriptPair(spell_gen_replenishment,
                                   spell_gen_replenishment_aura);
    ...
    RegisterSpellScriptWithArgs(spell_gen_consume,
                                "spell_consume_aq20",
