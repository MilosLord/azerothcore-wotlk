...
class spell_hallows_end_trick : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer()) {
            ...
            switch (urand(0, 7)) {
            case 1:
                spellId = gender ? SPELL_LEPER_GNOME_COSTUME_FEMALE
                                 : SPELL_LEPER_GNOME_COSTUME_MALE;
                break;
            case 2:
                spellId = gender ? SPELL_PIRATE_COSTUME_FEMALE
                                 : SPELL_PIRATE_COSTUME_MALE;
                break;
            case 3:
                spellId = gender ? SPELL_GHOST_COSTUME_FEMALE
                                 : SPELL_GHOST_COSTUME_MALE;
                break;
            case 4:
                spellId = gender ? SPELL_NINJA_COSTUME_FEMALE
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_trick::HandleScript,
                          EFFECT_0,
...
public:
    spell_hallows_end_put_costume(uint32 maleSpell, uint32 femaleSpell)
        : _maleSpell(maleSpell), _femaleSpell(femaleSpell)
    {
    }
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer())
            GetCaster()->CastSpell(
                target, target->getGender() ? _femaleSpell : _maleSpell, true);
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_put_costume::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_hallows_end_trick_or_treat : public SpellScript {
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer()) {
            GetCaster()->CastSpell(target,
                                   roll_chance_i(50) ? SPELL_TRICK
                                                     : SPELL_TREAT,
                                   true,
                                   nullptr);
            GetCaster()->CastSpell(
                target, SPELL_TRICKED_OR_TREATED, true, nullptr);
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_trick_or_treat::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hallows_end_candy : public SpellScript {
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_hallows_end_candy::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_hallows_end_candy_pirate_costume : public AuraScript {
        ...
        if (Unit* target = GetTarget()) {
            target->CastSpell(target,
                              target->getGender() == GENDER_MALE
                                  ? SPELL_HALLOWS_END_CANDY_3_MALE
                                  : SPELL_HALLOWS_END_CANDY_3_FEMALE,
                              true);
        }
    }
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_hallows_end_candy_pirate_costume::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_MOD_INCREASE_SWIM_SPEED,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(
            spell_hallows_end_candy_pirate_costume::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_MOD_INCREASE_SWIM_SPEED,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hallows_end_tricky_treat : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_tricky_treat::HandleScript,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hallows_end_bucket_lands : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_bucket_lands::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_hallows_end_base_fire : public AuraScript {
    ...
    void Register() override
    {
        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_hallows_end_base_fire::CalcPeriodic,
                                     EFFECT_0,
                                     SPELL_AURA_PERIODIC_DUMMY);
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_hallows_end_base_fire::HandleEffectPeriodicUpdate,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DUMMY);
        OnEffectApply +=
            AuraEffectApplyFn(spell_hallows_end_base_fire::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
    }
...
struct npc_costumed_orphan_matron : public ScriptedAI {
    ...
    void DoAction(int32 param) override
    {
        if (param == ACTION_START_EVENT) {
            ...
            if (Creature* cr =
                    me->SummonCreature(NPC_SHADE_OF_HORSEMAN,
                                       x,
                                       y,
                                       z,
                                       o,
    ...
    void sGossipHello(Player* player) override
    {
        QuestRelationBounds pObjectQR =
            sObjectMgr->GetCreatureQuestRelationBounds(me->GetEntry());
        QuestRelationBounds pObjectQIR =
            sObjectMgr->GetCreatureQuestInvolvedRelationBounds(me->GetEntry());
        ...
        for (QuestRelations::const_iterator i = pObjectQR.first;
             i != pObjectQR.second;
             ++i) {
            uint32       quest_id = i->second;
    ...
    {
        if ((quest->GetQuestId() == QUEST_LET_THE_FIRES_COME_A ||
             quest->GetQuestId() == QUEST_LET_THE_FIRES_COME_H) &&
            !allowQuest) {
...
struct npc_soh_fire_trigger : public NullCreatureAI {
    ...
    void Reset() override { me->SetDisableGravity(true); }
    ...
    void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
    {
        if (spellInfo->Id == SPELL_START_FIRE) {
            ...
            if (AuraEffect* aurEff =
                    me->GetAuraEffect(SPELL_FIRE_AURA_BASE, EFFECT_0)) {
                int32 amount = 1;
...
struct npc_hallows_end_soh : public ScriptedAI {
            ...
            .Schedule(
                ...
                [this](TaskContext context) {
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0, 30.f, true)) {
                        me->CastSpell(
                            target, SPELL_HORSEMAN_CONFLAGRATION, false);
                        target->CastSpell(
                            target, SPELL_HORSEMAN_CONFLAGRATION_SOUND, true);
    ...
    void DoAction(int32 param) override { pos = param; }
    ...
    void Reset() override
    {
        ...
        for (std::list<Creature*>::const_iterator itr = temp.begin();
             itr != temp.end();
             ++itr) {
    ...
    uint32 GetData(uint32 /*type*/) const override { return playerCount; }
    ...
    void CastFires(bool intial)
    {
        ...
        if (!intial) {
            ...
            // If there are more burning triggers than players, do not cast next
            // fire
...
struct boss_headless_horseman : public ScriptedAI {
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        for (std::list<Creature*>::iterator itr = unitList.begin();
             itr != unitList.end();
             ++itr)
            (*itr)->ToCreature()->DespawnOrUnsummon(500);

        Map::PlayerList const& players = me->GetMap()->GetPlayers();
        if (!players.IsEmpty() && players.begin()->GetSource() &&
            players.begin()->GetSource()->GetGroup())
            sLFGMgr->FinishDungeon(
                players.begin()->GetSource()->GetGroup()->GetGUID(),
                lfg::LFG_DUNGEON_HEADLESS_HORSEMAN,
                me->FindMap());
    ...
    void KilledUnit(Unit* /*who*/) override { Talk(TALK_PLAYER_DEATH); }

    void DoAction(int32 param) override { health = param; }
    ...
    {
        return playerGUID ? ObjectAccessor::GetPlayer(*me, playerGUID)
                          : nullptr;
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_HH_PLAYER_TALK: {
            ...
            case 2:
                ...
                if (Creature* trigger = me->SummonTrigger(1765.28f,
                                                          1347.46f,
                                                          17.5514f,
                                                          0.0f,
                                                          15 * IN_MILLISECONDS))
...
struct boss_headless_horseman_pumpkin : public ScriptedAI {
    ...
    void SpellHit(Unit* /*caster*/, SpellInfo const* spellInfo) override
    {
        if (spellInfo->Id == SPELL_SPROUTING) {
            if (Creature* cr =
                    me->SummonCreature(NPC_PUMPKIN_FIEND,
                                       me->GetPositionX(),
                                       me->GetPositionY(),
                                       me->GetPositionZ(),
                                       0.0f,
...
public:
                       ...
                       uint32 /*opt*/) override
    {
        ...
        if (Creature* horseman =
                go->SummonCreature(NPC_HEADLESS_HORSEMAN_MOUNTED,
                                   1754.00f,
                                   1346.00f,
                                   17.50f,
                                   0.0f,
...
public:
                        ...
                        uint32 /*action*/) override
    {
        ...
        if (InstanceScript* instance = go->GetInstanceScript()) {
            if (instance->GetData(DATA_HORSEMAN_EVENT) == IN_PROGRESS ||
                instance->GetData(DATA_HORSEMAN_EVENT) == DONE)
                return true;

            if (player->FindNearestCreature(NPC_HEADLESS_HORSEMAN_MOUNTED,
                                            100.0f))
                return true;
...
void AddSC_event_hallows_end_scripts()
{
    ...
    RegisterSpellScriptWithArgs(spell_hallows_end_put_costume,
                                "spell_hallows_end_pirate_costume",
                                SPELL_PIRATE_COSTUME_MALE,
                                SPELL_PIRATE_COSTUME_FEMALE);
    RegisterSpellScriptWithArgs(spell_hallows_end_put_costume,
                                "spell_hallows_end_leper_costume",
                                SPELL_LEPER_GNOME_COSTUME_MALE,
                                SPELL_LEPER_GNOME_COSTUME_FEMALE);
    RegisterSpellScriptWithArgs(spell_hallows_end_put_costume,
                                "spell_hallows_end_ghost_costume",
                                SPELL_GHOST_COSTUME_MALE,
                                SPELL_GHOST_COSTUME_FEMALE);
    RegisterSpellScriptWithArgs(spell_hallows_end_put_costume,
                                "spell_hallows_end_ninja_costume",
                                SPELL_NINJA_COSTUME_MALE,
