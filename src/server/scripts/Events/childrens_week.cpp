...
ObjectGuid getOrphanGUID(Player* player, uint32 orphan)
{
    if (Aura* orphanOut = player->GetAura(SPELL_ORPHAN_OUT))
        if (orphanOut->GetCaster() &&
            orphanOut->GetCaster()->GetEntry() == orphan)
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (!phase && who && who->GetDistance2d(me) < 10.0f)
            if (Player* player = who->ToPlayer())
                if (player->GetQuestStatus(QUEST_PLAYMATE_ORACLE) ==
...
struct npc_the_biggest_tree : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (!phase && who && who->GetDistance2d(me) < 10.0f)
            if (Player* player = who->ToPlayer())
                if (player->GetQuestStatus(QUEST_THE_BIGGEST_TREE_EVER) ==
                    QUEST_STATUS_INCOMPLETE) {
                    playerGUID = player->GetGUID();
                    orphanGUID = getOrphanGUID(player, ORPHAN_ORACLE);
                    if (orphanGUID)
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (timer <= diff) {
            ...
            case 3:
                ...
                player->GroupEventHappens(QUEST_THE_BRONZE_DRAGONSHRINE_ORACLE,
                                          me);
                orphan->GetMotionMaster()->MoveFollow(
                    player, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
                Reset();
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (timer <= diff) {
            ...
            case 3:
                ...
                player->GroupEventHappens(QUEST_THE_BRONZE_DRAGONSHRINE_WOLVAR,
                                          me);
                orphan->GetMotionMaster()->MoveFollow(
                    player, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
                Reset();
...
/*######
## npc_the_etymidian
## TODO: A red crystal as a gift for the great one should be spawned during the
event.
######*/
struct npc_the_etymidian : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (!phase && who && who->GetDistance2d(me) < 10.0f)
            if (Player* player = who->ToPlayer())
                if (player->GetQuestStatus(QUEST_MEETING_A_GREAT_ONE) ==
                    QUEST_STATUS_INCOMPLETE) {
                    playerGUID = player->GetGUID();
                    orphanGUID = getOrphanGUID(player, ORPHAN_ORACLE);
                    if (orphanGUID)
...
struct npc_alexstraza_the_lifebinder : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (!phase && who && who->GetDistance2d(me) < 10.0f)
            if (Player* player = who->ToPlayer()) {
                if (player->GetQuestStatus(QUEST_THE_DRAGON_QUEEN_ORACLE) ==
                    QUEST_STATUS_INCOMPLETE) {
                    playerGUID = player->GetGUID();
                    orphanGUID = getOrphanGUID(player, ORPHAN_ORACLE);
                    if (orphanGUID)
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (timer <= diff) {
            ...
            case 8:
                if (Creature* krasus =
                        me->FindNearestCreature(NPC_KRASUS, 10.0f)) {
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* trigger) override
    {
        ...
        case AT_STONEWROUGHT_DAM:
            ...
            break;
        case AT_DARK_PORTAL:
            questId  = player->GetTeamId() == TEAM_ALLIANCE
                           ? QUEST_DARK_PORTAL_A
                           : QUEST_DARK_PORTAL_H;
            orphanId = player->GetTeamId() == TEAM_ALLIANCE ? ORPHAN_DRAENEI
                                                            : ORPHAN_BLOOD_ELF;
            break;
        }
                ...
                if (Player* player = who->ToPlayer())
                    if (player->HasAura(SPELL_ORPHAN_OUT)) {
                        ...
                        case NPC_CAVERNS_OF_TIME_CW_TRIGGER:
                            ...
                            orphanId = player->GetTeamId() == TEAM_ALLIANCE
                                           ? ORPHAN_DRAENEI
                                           : ORPHAN_BLOOD_ELF;
                            break;
    ...
    struct npc_grizzlemaw_cw_triggerAI : public ScriptedAI {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (who && who->GetDistance2d(me) < 10.0f)
                if (Player* player = who->ToPlayer())
                    if (player->GetQuestStatus(QUEST_HOME_OF_THE_BEAR_MEN) ==
                        QUEST_STATUS_INCOMPLETE)
                        if (Creature* orphan = ObjectAccessor::GetCreature(
                                *me, getOrphanGUID(player, ORPHAN_WOLVAR))) {
                            player->AreaExploredOrEventHappens(
