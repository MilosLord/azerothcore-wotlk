...
struct npc_pilgrims_bounty_chair : public VehicleAI {
    ...
    Creature* GetPlate()
    {
        return plateGUID ? ObjectAccessor::GetCreature(*me, plateGUID)
                         : nullptr;
    }
                ...
                if (Vehicle* table = me->GetVehicle())
                    if (Unit* plateHolder = table->GetPassenger(6)) {
                        SeatMap::const_iterator itr =
                            table->GetSeatIteratorForPassenger(me);
                        if (itr == table->Seats.end())
                            return;

                        uint8     vehicleSeatId = itr->first;
                        Creature* plate = me->SummonCreature(NPC_STRUDY_PLATE,
                                                             me->GetPositionX(),
                                                             me->GetPositionY(),
                                                             me->GetPositionZ(),
                                                             0.0f);
...
class spell_pilgrims_bounty_pass_generic : public SpellScript {
        ...
        if (Unit* target = GetHitUnit()) {
            ...
            if (target->IsVehicle() && target->ToCreature()) {
                if (Player* player =
                    ...
                    GetCaster()->CastSpell(
                        player, GetVisualThrow(GetSpellInfo()->Id, true), true);
                        ...
                        if (aur->GetBase()->GetStackAmount() >= 5) {
                            if (Player* casterPlayer =
                                ...
                                casterPlayer->UpdateAchievementCriteria(
                                    ...
                                    GetVisualThrow(GetSpellInfo()->Id, true));
                            }
                        }
                    }
                }
            ...
            // normal case
                ...
                GetCaster()->CastSpell(
                    target, GetVisualThrow(GetSpellInfo()->Id, false), true);
            }
        }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pilgrims_bounty_pass_generic::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_pilgrims_bounty_feast_on_generic : public SpellScript {
            ...
            if (AuraEffect* aur = player->GetAuraEffectDummy(spellId)) {
                if (aur->GetBase()->GetStackAmount() >= 5) {
                    ...
                    case SPELL_PLAYER_STUFFING:
                        player->CastSpell(
                            player, SPELL_WELL_FED_STUFFING, true);
                        break;
                    ...
                    case SPELL_PLAYER_CRANBERRY:
                        player->CastSpell(
                            player, SPELL_WELL_FED_CRANBERRY, true);
                        break;
                    case SPELL_PLAYER_SWEET_POTATOES:
                        player->CastSpell(
                            player, SPELL_WELL_FED_SWEET_POTATOES, true);
                        break;
                    }

                    uint8                       count = 0;
                    Unit::AuraEffectList const& dummyAuras =
                        player->GetAuraEffectsByType(SPELL_AURA_DUMMY);
                    for (Unit::AuraEffectList::const_iterator i =
                             dummyAuras.begin();
                         i != dummyAuras.end();
                         ++i) {
                        if ((*i)->GetId() >= SPELL_PLAYER_CRANBERRY &&
                            (*i)->GetId() <= SPELL_PLAYER_PIE)
                    ...
                    if (count >= 5)
                        player->CastSpell(
                            player, SPELL_SPIRIT_OF_SHARING, true);
                }
            }
        }
    }
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_pilgrims_bounty_feast_on_generic::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_pilgrims_bounty_turkey_tracker : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer()) {
            if (AuraEffect* aurEff =
                    target->GetAuraEffectDummy(SPELL_TURKEY_TRACKER)) {
                ...
                case 40:
                    ...
                    target->CastSpell(
                        target, SPELL_ACHI_TURKINATOR_CREDIT, true);
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_pilgrims_bounty_turkey_tracker::HandleScriptEffect,
            EFFECT_1,
...
class spell_pilgrims_bounty_serve_generic : public AuraScript {
    ...
    void OnAuraRemove(AuraEffect const* /*aurEff*/,
    ...
    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(
            spell_pilgrims_bounty_serve_generic::OnAuraRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
    ...
    void Register() override
    {
        if (m_scriptSpellId == 66041)
            DoEffectCalcAmount += AuraEffectCalcAmountFn(
                spell_pilgrims_bounty_food_aura::RecalculateHook,
                EFFECT_0,
...
class spell_pilgrims_bounty_food : public SpellScript {
    ...
    void Register() override
    {
        if (m_scriptSpellId == 66477)
            OnEffectHitTarget +=
                SpellEffectFn(spell_pilgrims_bounty_food::HandleScriptEffect,
                              EFFECT_0,
                              SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
public:
    achievement_pb_pilgrims_peril()
        : AchievementCriteriaScript("achievement_pb_pilgrims_peril")
    {
    }
...
public:
    achievement_pb_terokkar_turkey_time()
        : AchievementCriteriaScript("achievement_pb_terokkar_turkey_time")
    {
    }
...
void AddSC_event_pilgrims_end_scripts()
{
    ...
    RegisterSpellAndAuraScriptPair(spell_pilgrims_bounty_food,
                                   spell_pilgrims_bounty_food_aura);
