...
class spell_winter_veil_mistletoe : public SpellScript {
    ...
    {
        return ValidateSpellInfo({SPELL_CREATE_MISTLETOE,
                                  SPELL_CREATE_HOLLY,
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer()) {
            uint32 spellId = RAND(SPELL_CREATE_HOLLY,
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_winter_veil_mistletoe::HandleScript,
                          EFFECT_0,
...
class spell_winter_wondervolt_trap : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Player* target = GetHitPlayer()) {
            ...
            if (target->getGender() == GENDER_MALE) {
                spellId = target->GetTeamId() == TEAM_ALLIANCE
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_winter_wondervolt_trap::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_winter_veil_racer_rocket_slam : public SpellScript {
    ...
    void HandleTriggerSpell(SpellEffIndex /*effIndex*/)
    {
        ...
        if (!unitList.empty())
            for (std::list<Creature*>::const_iterator itr = unitList.begin();
                 itr != unitList.end();
                 ++itr)
                if (caster->HasInLine((*itr), 1.0f) &&
                    (*itr)->GetGUID() != caster->GetGUID()) {
                    target = (*itr);
                    break;
                }
        if (!target) {
        ...
        if (target) {
            ...
            float y = caster->GetPositionY() +
                      30 * std::sin(caster->GetOrientation());
            pos.Relocate(
                x, y, caster->GetMap()->GetHeight(x, y, MAX_HEIGHT) + 0.5f);
            // caster->GetFirstCollisionPosition(pos, 30.0f,
    ...
    void Register() override
    {
        OnEffectLaunch += SpellEffectFn(
            spell_winter_veil_racer_rocket_slam::HandleTriggerSpell,
            EFFECT_1,
...
class spell_winter_veil_racer_slam_hit : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Player* targetSummoner =
                target->GetCharmerOrOwnerPlayerOrPlayerItself()) {
            // targetSummoner->RemoveCriteriaProgress(sAchievementCriteriaStore.LookupEntry(RACER_ACHI_CRITERIA));
            // !ZOMG, ADD ACCESSOR
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_winter_veil_racer_slam_hit::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
    ...
    void Register() override
    {
        if (m_scriptSpellId == SPELL_AIR_RIFLE_HIT_TRIGGER)
            OnEffectHitTarget += SpellEffectFn(
                spell_winter_veil_shoot_air_rifle::HandleScriptEffect,
                EFFECT_1,
                SPELL_EFFECT_SCRIPT_EFFECT);
        else
            OnEffectHitTarget += SpellEffectFn(
