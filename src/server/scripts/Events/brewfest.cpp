...
struct npc_brewfest_keg_thrower : public ScriptedAI {
    ...
    {
        if (me->GetDistance(who) < 10.0f && who->GetTypeId() == TYPEID_PLAYER &&
            who->GetMountID() == RAM_DISPLAY_ID) {
            if (!who->ToPlayer()->HasItemCount(
                    ITEM_PORTABLE_BREWFEST_KEG)) // portable brewfest keg
        ...
        if (me->GetDistance(who) < 10.0f && who->GetTypeId() == TYPEID_PLAYER &&
                    ...
                    ITEM_PORTABLE_BREWFEST_KEG)) // portable brewfest keg
            {
                ...
                uint32 spellCooldown =
                    player->GetSpellCooldownDelay(SPELL_COOLDOWN_CHECKER) /
                    IN_MILLISECONDS;
                if (spellCooldown >
                    (HOUR * 18 - 900)) // max aproximated time - 12 minutes
                {
                    if (Aura* aur = player->GetAura(SPELL_RAM_AURA)) {
                        int32 diff = aur->GetApplyTime() -
                                     (GameTime::GetGameTime().count() -
                                      (HOUR * 18) + spellCooldown);
...
struct npc_brewfest_bark_trigger : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (me->GetDistance(who) < 10.0f && who->GetTypeId() == TYPEID_PLAYER &&
            ...
            // Kalimdor
            if (me->GetMapId() == 1) {
                if (player->GetQuestStatus(QUEST_BARK_FOR_DROHN) ==
                    QUEST_STATUS_INCOMPLETE) {
                    allow = true;
                    quest = QUEST_BARK_FOR_DROHN;
                }
                else if (player->GetQuestStatus(QUEST_BARK_FOR_VOODOO) ==
                         QUEST_STATUS_INCOMPLETE) {
                    allow = true;
                ...
                else if (player->GetQuestStatus(QUEST_BARK_FOR_THUNDERBREW) ==
                         QUEST_STATUS_INCOMPLETE) {
                    allow = true;
            ...
            if (allow) {
                QuestStatusMap::iterator itr =
                    player->getQuestStatusMap().find(quest);
                ...
                if (q_status.CreatureOrGOCount[me->GetEntry() - 24202] == 0) {
                    ...
                    player->Say(GetTextFor(me->GetEntry(), quest).c_str(),
                                LANG_UNIVERSAL,
                                player);
                }
            }
    ...
    std::string GetTextFor(uint32 /*entry*/, uint32 questId)
    {
        ...
        case QUEST_BARK_FOR_VOODOO: {
            switch (urand(0, 3)) {
            case 0:
                str = "Join with your brothers and sisters at " +
                      std::string(questId == QUEST_BARK_FOR_DROHN
                                      ? "Drohn's Distillery"
                                      : "T'chali's Voodoo Brewery") +
                      " and drink for the horde!";
                break;
            case 1:
                str = "If you think an orc can hit hard, check out their brew, "
                      "it hits even harder! See for yourself at " +
                      std::string(questId == QUEST_BARK_FOR_DROHN
                                      ? "Drohn's Distillery"
                                      : "T'chali's Voodoo Brewery") +
                      ", only at Brewfest!";
                break;
            case 2:
                str = "Celebrate Brewfest with orcs that know what a good "
                      "drink really is! Check out " +
                      std::string(questId == QUEST_BARK_FOR_DROHN
                                      ? "Drohn's Distillery"
                                      : "T'chali's Voodoo Brewery") +
                      " at Brewfest!";
                break;
            case 3:
                str = std::string(questId == QUEST_BARK_FOR_DROHN
                                      ? "Drohn's Distillery"
                                      : "T'chali's Voodoo Brewery") +
                      "  knows how to party hard! Check them out at Brewfest!";
                break;
            }
            break;
        }
        ...
        case QUEST_BARK_FOR_THUNDERBREW: {
            ...
            case 0:
                str = "Join with your brothers and sisters at " +
                      std::string(questId == QUEST_BARK_FOR_BARLEY
                                      ? "Barleybrews"
                                      : "Thunderbrews") +
                      " and drink for the alliance!";
                break;
            case 1:
                str = "If you think an dwarf can hit hard, check out their "
                      "brew, it hits even harder! See for yourself at " +
                      std::string(questId == QUEST_BARK_FOR_BARLEY
                                      ? "Barleybrews"
                                      : "Thunderbrews") +
                      ", only at Brewfest!";
                break;
            case 2:
                str = "Celebrate Brewfest with dwarves that know what a good "
                      "drink really is! Check out " +
                      std::string(questId == QUEST_BARK_FOR_BARLEY
                                      ? "Barleybrews"
                                      : "Thunderbrews") +
                ...
                break;
            case 3:
                str = std::string(questId == QUEST_BARK_FOR_BARLEY
                                      ? "Barleybrews"
                                      : "Thunderbrews") +
                      "  knows how to party hard! Check them out at Brewfest!";
                break;
...
struct npc_dark_iron_attack_generator : public ScriptedAI {
    ...
    void Reset() override
    {
        for (ObjectGuid const& guid : revelerGUIDs) {
            if (Creature* reveler = ObjectAccessor::GetCreature(*me, guid)) {
                ...
                // It's here because SmartAI::JustRespawned restores original
                // faction So we need to delay a little bit reloading auras from
                // creature_template_addon
                reveler->m_Events.AddEventAtOffset(
                    [reveler]() {
        ...
        case EVENT_SPAWN_MOLE_MACHINE: {
            if (me->GetMapId() == 1) // Kalimdor
            {
                ...
                if (Creature* cr = me->SummonCreature(
                        NPC_MOLE_MACHINE_TRIGGER, x, y, 21.3f, 0.0f))
            ...
            else if (me->GetMapId() == 0) // EK
            {
                ...
                if (Creature* cr = me->SummonCreature(
                        NPC_MOLE_MACHINE_TRIGGER, x, y, 398.11f, 0.0f))
                    cr->CastSpell(cr, SPELL_SPAWN_MOLE_MACHINE, true);
            }
        ...
        case EVENT_BARTENDER_SAY: {
            ...
            if (thrown == 3) {
                ...
                sayer->SummonCreature(
                    ...
                    sayer->GetPositionX() + 15 * cos(sayer->GetOrientation()),
                    sayer->GetPositionY() +
                ...
                if (urand(0, 1))
                    sayer->Say("Chug and chuck! Chug and chuck!",
                               LANG_UNIVERSAL);
                else
                    sayer->Say("Down the free brew and pelt the Guzzlers with "
                               "your mug!",
                               LANG_UNIVERSAL);
            }
    ...
    {
        if (Creature* herald =
                me->FindNearestCreature(NPC_DARK_IRON_HERALD, 100.0f)) {
            char amount[500];
            snprintf(amount,
                     sizeof(amount),
                     "We did it boys! Now back to the Grim Guzzler and we'll "
                     "drink to the %u that were injured!",
                     guzzlerCounter);
    ...
    void FinishAttackDueToWin()
    {
        if (Creature* herald =
            ...
            snprintf(amount,
                     sizeof(amount),
                     "RETREAT!! We've already lost %u and we can't afford to "
                     "lose any more!!",
                     guzzlerCounter);
            herald->Yell(amount, LANG_UNIVERSAL);
        }
        ...
        me->CastSpell(
            me,
            (me->GetMapId() == 1 ? SPELL_SUMMON_PLANS_H : SPELL_SUMMON_PLANS_A),
            true);
        Reset();
    }
    ...
    void PrepareEvent()
    {
        ...
        GetCreatureListWithEntryInGrid(
            revelers, me, NPC_BREWFEST_REVELER, 100.f);
        ...
        if (me->GetMapId() == 1) // Kalimdor
        {
            if ((cr = me->SummonCreature(NPC_DROHN_KEG,
                                         1183.69f,
                                         -4315.15f,
                                         21.1875f,
                                         0.750492f))) {
            ...
            if ((cr = me->SummonCreature(NPC_VOODOO_KEG,
                                         1182.42f,
                                         -4272.45f,
                                         21.1182f,
                                         -1.02974f))) {
            ...
            if ((cr = me->SummonCreature(NPC_GORDOK_KEG,
                                         1223.78f,
                                         -4296.48f,
                                         21.1707f,
                                         -2.86234f))) {
        ...
        else if (me->GetMapId() == 0) // Eastern Kingdom
        {
            ...
            if ((cr = me->SummonCreature(NPC_THUNDERBREW_KEG,
                                         -5160.05f,
                                         -632.632f,
                                         397.178f,
                                         1.39626f))) {
        ...
        if ((cr = me->SummonCreature(NPC_DARK_IRON_HERALD,
                                     me->GetPositionX(),
                                     me->GetPositionY(),
                                     me->GetPositionZ(),
                                     0.0f,
    ...
    Creature* GetRandomBartender()
    {
        ...
        switch (urand(0, 2)) {
        case 0:
            entry = (me->GetMapId() == 1 ? NPC_NORMAL_DROHN
                                         : NPC_NORMAL_THUNDERBREW);
            break;
        case 1:
            entry = (me->GetMapId() == 1 ? NPC_NORMAL_VOODOO
    ...
    void UpdateAI(uint32 diff) override
    {
        if (goTimer) {
            ...
            if (goTimer >= 3000) {
                goTimer = 0;
                summonTimer++;
                if (GameObject* drill = me->SummonGameObject(GO_MOLE_MACHINE,
                                                             me->GetPositionX(),
                                                             me->GetPositionY(),
                                                             me->GetPositionZ(),
                                                             M_PI / 4,
        ...
        if (summonTimer) {
            ...
            if (summonTimer >= 2000 && summonTimer < 10000) {
                me->SummonCreature(NPC_DARK_IRON_GUZZLER,
                                   me->GetPositionX(),
                                   me->GetPositionY(),
                                   me->GetPositionZ(),
                                   0.0f,
...
struct npc_dark_iron_guzzler : public ScriptedAI {
    ...
    void Reset() override
    {
        ...
        FindNextKeg();
        me->ApplySpellImmune(
            SPELL_ATTACK_KEG, IMMUNITY_ID, SPELL_ATTACK_KEG, true);
        SayText();
    ...
    void SayText()
    {
        if (!urand(0, 20)) {
            ...
            case 2:
                ...
                break;
            case 3:
                me->Say("No one expects the Dark Iron dwarves!",
                        LANG_UNIVERSAL);
                break;
            case 4:
                me->Say("It's not a party without some crashers!",
                        LANG_UNIVERSAL);
                break;
            }
        }
...
struct npc_brewfest_super_brew_trigger : public ScriptedAI {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (timer >= 500) {
            ...
            Acore::PlayerSearcher<Acore::AnyPlayerInObjectRangeCheck> searcher(
                me, player, checker);
...
class spell_brewfest_main_ram_buff : public AuraScript {
    ...
    void HandleEffectPeriodic(AuraEffect const* aurEff)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            // Two - three clicks to maintains speed, less to decrease, more to
            // increase
            if (stack < 3) {
                caster->CastSpell(caster, SPELL_TROT, true);
                privateLevel--;
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_brewfest_main_ram_buff::HandleEffectPeriodic,
            EFFECT_1,
            SPELL_AURA_PERIODIC_DUMMY);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_brewfest_main_ram_buff::HandleEffectRemove,
                               EFFECT_1,
                               SPELL_AURA_PERIODIC_DUMMY,
...
class spell_brewfest_ram_fatigue : public AuraScript {
    ...
    void Register() override
    {
        if (m_scriptSpellId != 43332)
            OnEffectPeriodic += AuraEffectPeriodicFn(
                spell_brewfest_ram_fatigue::HandleEffectPeriodic,
                EFFECT_1,
                SPELL_AURA_PERIODIC_DUMMY);
        else
            OnEffectRemove += AuraEffectRemoveFn(
                spell_brewfest_ram_fatigue::HandleEffectRemove,
                EFFECT_0,
...
class spell_brewfest_apple_trap : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_brewfest_apple_trap::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_brewfest_apple_trap::HandleDummyEffect,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_catch_the_wild_wolpertinger : public AuraScript {
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_catch_the_wild_wolpertinger::HandleEffectApply,
            EFFECT_0,
    ...
    void HandleAfterHit()
    {
        if (GetCaster() && GetCaster()->ToPlayer()) {
            if (Item* itemCaster = GetCastItem()) {
                ...
                case BLUE_EMPTY_KEG:
                    item = itemCaster->GetEntry() +
                           urand(1, 5); // 5 items, id in range empty+1-5
                    break;
                ...
                if (item &&
                    player->AddItem(item, 1)) // ensure filled keg is stored
                {
...
class spell_brewfest_toss_mug : public SpellScript {
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (caster->GetMapId() == 1) // Kalimdor
        {
            if (Creature* creature =
                    caster->FindNearestCreature(NPC_NORMAL_VOODOO, 40.0f)) {
                bakers.push_back(creature);
            }

            if (Creature* creature =
                    caster->FindNearestCreature(NPC_NORMAL_DROHN, 40.0f)) {
                bakers.push_back(creature);
            }

            if (Creature* creature =
                    caster->FindNearestCreature(NPC_NORMAL_GORDOK, 40.0f)) {
                bakers.push_back(creature);
            }
        }
        else // EK
        {
            if (Creature* creature = caster->FindNearestCreature(
                    NPC_NORMAL_THUNDERBREW, 40.0f)) {
                bakers.push_back(creature);
            }

            if (Creature* creature =
                    caster->FindNearestCreature(NPC_NORMAL_BARLEYBREW, 40.0f)) {
                bakers.push_back(creature);
            }
        ...
        if (!bakers.empty()) {
            std::sort(bakers.begin(),
                      bakers.end(),
                      Acore::ObjectDistanceOrderPred(caster));
    ...
    void Register() override
    {
        ...
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_brewfest_toss_mug::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENTRY);
        BeforeHit += BeforeSpellHitFn(spell_brewfest_toss_mug::HandleBeforeHit);
        OnEffectHitTarget +=
            SpellEffectFn(spell_brewfest_toss_mug::HandleScriptEffect,
                          EFFECT_0,
...
class spell_brewfest_add_mug : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_brewfest_add_mug::HandleDummyEffect,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
struct npc_brew_bubble : public NullCreatureAI {
    ...
    void DoAction(int32) override { timer = 0; }
    ...
    void MoveInLineOfSight(Unit* target) override
    {
        if (target->GetEntry() == me->GetEntry())
            if (me->IsWithinDist(target, 1.0f)) {
                ...
                uint8 stacksTarget =
                    target->GetAuraCount(SPELL_BUBBLE_BUILD_UP);
...
class spell_brewfest_reveler_transform : public AuraScript {
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_brewfest_reveler_transform::OnApply,
                              EFFECT_0,
...
class spell_brewfest_relay_race_force_cast : public SpellScript {
    ...
    SpellCastResult CheckItem()
    {
        if (Unit* target = GetExplTargetUnit()) {
            if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(
    ...
    {
        OnCheckCast +=
            SpellCheckCastFn(spell_brewfest_relay_race_force_cast::CheckItem);
    }
};
...
struct npc_coren_direbrew : public ScriptedAI {
    ...
    void
    sGossipSelect(Player* player, uint32 menuId, uint32 gossipListId) override
    {
        if (menuId != GOSSIP_ID) {
            return;
        }
    ...
    bool CanBeSeen(Player const* player) override
    {
        ...
        return group && sLFGMgr->GetDungeon(group->GetGUID()) ==
    ...
    void Reset() override
    {
        ...
        for (uint8 i = 0; i < MAX_ANTAGONISTS; ++i) {
            me->SummonCreature(
                NPC_ANTAGONIST, AntagonistPos[i], TEMPSUMMON_DEAD_DESPAWN);
        }

        std::list<GameObject*> machines;
        GetGameObjectListWithEntryInGrid(
            machines, me, GO_MACHINE_SUMMONER, 60.f);
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (!_events.IsInPhase(PHASE_ALL) ||
    ...
    {
        if (Creature* sister = me->SummonCreature(
                entry, me->GetPosition(), TEMPSUMMON_DEAD_DESPAWN)) {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent()) {
            ...
            case EVENT_SUMMON_MOLE_MACHINE: {
                me->CastCustomSpell(SPELL_MOLE_MACHINE_TARGET_PICKER,
...
struct npc_direbrew_minion : public ScriptedAI {
    ...
    {
        if (Creature* coren = ObjectAccessor::GetCreature(
                *me, _instance->GetGuidData(DATA_COREN))) {
...
class spell_direbrew_summon_mole_machine_target_picker : public SpellScript {
    ...
    {
        GetCaster()->CastSpell(
            GetHitUnit(), SPELL_MOLE_MACHINE_MINION_SUMMONER, true);
    }

    void Register() override
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_direbrew_summon_mole_machine_target_picker::
                              HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_send_mug_target_picker : public SpellScript {
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if(
            Acore::UnitAuraCheck(true, SPELL_HAS_DARK_BREWMAIDENS_BREW));

        if (targets.size() > 1) {
            targets.remove_if([caster](WorldObject* obj) {
                if (obj->GetGUID() ==
                    caster->GetAI()->GetGUID(DATA_TARGET_GUID)) {
                    return true;
                }
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_send_mug_target_picker::FilterTargets,
            EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENTRY);
        OnEffectHitTarget +=
...
class spell_request_second_mug : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_request_second_mug::HandleScriptEffect,
                          EFFECT_0,
                          SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_send_mug_control_aura : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_send_mug_control_aura::PeriodicTick,
                                 EFFECT_0,
                                 SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_barreled_control_aura : public AuraScript {
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_barreled_control_aura::PeriodicTick,
                                 EFFECT_0,
...
class spell_direbrew_disarm : public AuraScript {
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_DIREBREW_DISARM, SPELL_DIREBREW_DISARM_GROW});
    }
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_direbrew_disarm::PeriodicTick,
                                 EFFECT_1,
                                 SPELL_AURA_PERIODIC_DUMMY);
        OnEffectApply += AuraEffectRemoveFn(spell_direbrew_disarm::OnApply,
                                            EFFECT_1,
                                            SPELL_AURA_PERIODIC_DUMMY,
                                            AURA_EFFECT_HANDLE_REAL);
    }
