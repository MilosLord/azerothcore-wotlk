...
enum Quests { QUEST_PILGRIM_HORDE = 24541, QUEST_PILGRIM_ALLIANCE = 24656 };

enum SupplySentrySay { SAY_SUPPLY_SENTRY_0 = 0 };

struct npc_love_in_air_supply_sentry : public ScriptedAI {
    npc_love_in_air_supply_sentry(Creature* creature)
        : ScriptedAI(creature), lock(0)
    {
    }
    ...
    {
        if (lock > 1000 && me->GetDistance(who) < 10.0f &&
            who->GetTypeId() == TYPEID_PLAYER &&
            who->HasAura(SPELL_GOBLIN_DISGUISE) &&
    ...
    void UpdateAI(uint32 diff) override { lock += diff; }
};
...
struct npc_love_in_air_snivel : public NullCreatureAI {
    ...
    {
        uint16 slot = player->FindQuestSlot(player->GetTeamId() == TEAM_ALLIANCE
        ...
        QuestStatusData& qData = player->getQuestStatusMap()[(
            player->GetTeamId() == TEAM_ALLIANCE ? QUEST_HOT_ON_TRAIL_ALLY
                                                 : QUEST_HOT_ON_TRAIL_HORDE)];
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (delay == 0 && me->GetDistance(who) < 7.0f &&
            who->GetTypeId() == TYPEID_PLAYER) {
            Player* plr = who->ToPlayer();
            if (AllowAction(plr)) {
                delay       = 25000;
                uint8 index = plr->GetTeamId() * 3 +
                              (me->GetEntry() - NPC_SNIVEL_COUNTER);
...
class spell_love_in_air_perfume_immune : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetId() == SPELL_COLOGNE_IMMUNE) {
            target->ApplySpellImmune(SPELL_COLOGNE_PASSIVE_DAMAGE,
                                     IMMUNITY_ID,
            ...
            target->ApplySpellImmune(SPELL_PERFUME_PASSIVE_DAMAGE,
                                     IMMUNITY_ID,
                                     SPELL_PERFUME_PASSIVE_DAMAGE,
                                     true);
            target->ApplySpellImmune(SPELL_ALLURING_PERFUME_SPRAY,
                                     IMMUNITY_ID,
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(
            spell_love_in_air_perfume_immune::HandleEffectApply,
            EFFECT_2,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(
            spell_love_in_air_perfume_immune::HandleEffectRemove,
            EFFECT_2,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_item_create_heart_candy : public SpellScript {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_item_create_heart_candy::HandleScript,
                          EFFECT_0,
...
enum SpellsPicnic {
    ...
    SPELL_MEAL_PERIODIC = 45103, // Holiday - Valentine - Romantic Picnic Meal
                                 // Periodic - effect dummy
    SPELL_MEAL_EAT_VISUAL =
        45120, // Holiday - Valentine - Romantic Picnic Meal Eat Visual
    // SPELL_MEAL_PARTICLE             = 45114, // Holiday - Valentine -
    // Romantic Picnic Meal Particle - unused
    SPELL_DRINK_VISUAL =
...
class spell_love_is_in_the_air_romantic_picnic : public AuraScript {
    ...
    void OnPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        target->CastSpell(target,
                          SPELL_BASKET_CHECK,
                          false); // unknown use, it targets Romantic Basket
        target->CastSpell(
            target, RAND(SPELL_MEAL_EAT_VISUAL, SPELL_DRINK_VISUAL), false);

        bool foundSomeone = false;
        // For nearby players, check if they have the same aura. If so, cast
        // Romantic Picnic (45123) required by achievement and "hearts" visual
        std::list<Player*>                 playerList;
        Acore::AnyPlayerInObjectRangeCheck checker(target,
                                                   INTERACTION_DISTANCE * 2);
        Acore::PlayerListSearcher<Acore::AnyPlayerInObjectRangeCheck> searcher(
            target, playerList, checker);
        Cell::VisitWorldObjects(target, searcher, INTERACTION_DISTANCE * 2);
        for (std::list<Player*>::const_iterator itr = playerList.begin();
             itr != playerList.end();
             ++itr) {
            if ((*itr) != target &&
                (*itr)->HasAura(GetId())) // && (*itr)->getStandState() ==
                                          // UNIT_STAND_STATE_SIT)
            {
                if (caster) {
                    caster->CastSpell(*itr, SPELL_ROMANTIC_PICNIC_ACHIEV, true);
                    caster->CastSpell(
                        target, SPELL_ROMANTIC_PICNIC_ACHIEV, true);
                }
                foundSomeone = true;
                // break;
            }
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_love_is_in_the_air_romantic_picnic::OnApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_love_is_in_the_air_romantic_picnic::OnPeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_gen_aura_service_uniform : public AuraScript {
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_gen_aura_service_uniform::OnApply,
                              EFFECT_0,
                              SPELL_AURA_TRANSFORM,
                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gen_aura_service_uniform::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_TRANSFORM,
                               AURA_EFFECT_HANDLE_REAL);
    }
