...
static bool
    BonfireStampedOutState[COUNT_GO_BONFIRE_ALLIANCE + COUNT_GO_BONFIRE_HORDE];
...
uint32 const GoBonfireAlliance[COUNT_GO_BONFIRE_ALLIANCE] = {
    187564, 187914, 187916, 187917, 187919, 187920, 187921, 187922,
    187923, 187924, 187925, 187926, 187927, 187928, 187929, 187930,
    187931, 187932, 187933, 187934, 187935, 187936, 187937, 187938,
    187939, 187940, 187941, 187942, 187943, 187944, 187945, 187946,
    194032, 194035, 194036, 194038, 194040, 194044, 194045, 194049};
uint32 const GoBonfireHorde[COUNT_GO_BONFIRE_HORDE] = {
    187559, 187947, 187948, 187949, 187950, 187951, 187952, 187953,
    187954, 187955, 187956, 187957, 187958, 187959, 187960, 187961,
    187962, 187963, 187964, 187965, 187966, 187967, 187968, 187969,
    187970, 187971, 187972, 187973, 187974, 187975, 194033, 194034,
    194037, 194039, 194042, 194043, 194046, 194048};
...
public:
    MidsummerPlayerScript()
        : PlayerScript("MidsummerPlayerScript", {PLAYERHOOK_ON_UPDATE_ZONE})
    {
    }

    void
    OnUpdateZone(Player* player, uint32 newZone, uint32 /*newArea*/) override
    {
        if (!IsHolidayActive(HOLIDAY_FIRE_FESTIVAL))
            return;

        auto itr = BonfireStateStore.find(
            std::make_tuple(player->GetMapId(), newZone, player->GetTeamId()));
...
struct npc_midsummer_bonfire : public ScriptedAI {
    ...
    void Ignite()
    {
        ...
        if (!_spellFocus) {
            ...
            if ((_spellFocus = me->FindNearestGameObject(
                     GO_MIDSUMMER_BONFIRE_CAMPFIRE_SPELL_FOCUS, 10.0f)))
    ...
    bool InitBonfire()
    {
        ...
        for (uint32 i = 0;
             (i < COUNT_GO_BONFIRE_ALLIANCE) && (_type == BONFIRE_TYPE_NONE);
             i++) {
            if ((_bonfire =
                     me->FindNearestGameObject(GoBonfireAlliance[i], 10.0f))) {
        ...
        for (uint32 i = 0;
             (i < COUNT_GO_BONFIRE_HORDE) && (_type == BONFIRE_TYPE_NONE);
             i++) {
            if ((_bonfire =
                     me->FindNearestGameObject(GoBonfireHorde[i], 10.0f))) {
        ...
        for (uint32 i = 0;
             (i < COUNT_GO_BONFIRE_CITY) && (_type == BONFIRE_TYPE_NONE);
             i++) {
            if ((_bonfire =
                     me->FindNearestGameObject(GoBonfireCity[i], 10.0f))) {
        ...
        if ((_type == BONFIRE_TYPE_NONE) &&
            (_bonfire = me->FindNearestGameObject(GO_AHUNE_BONFIRE, 10.0f))) {
        ...
        auto itr = BonfireStateStore.find(
            std::make_tuple(me->GetMapId(), me->GetZoneId(), _teamId));
        if ((itr != BonfireStateStore.end()) && (itr->second))
            _isStampedOut = itr->second;
        else
            LOG_ERROR("scripts.midsummer",
                      "NPC {} (GUID{}) in map {}, zone {} with teamId {} can't "
                      "locate its entry within BonfireStateStore",
                      me->GetGUID().GetEntry(),
                      me->GetSpawnId(),
                      me->GetMapId(),
                      me->GetZoneId(),
...
struct npc_midsummer_torch_target : public ScriptedAI {
    ...
    void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
    {
        ...
        if (spellInfo->Id == SPELL_TORCH_TOSS_LAND &&
            caster->GetTypeId() == TYPEID_PLAYER) {
            me->CastSpell(
                me, SPELL_BRAZIERS_HIT_VISUAL, true); // hit visual anim
            if (++counter >= maxCount) {
                caster->CastSpell(caster,
                                  (caster->ToPlayer()->GetTeamId()
    ...
    void FillPositions()
    {
        ...
        me->GetGameObjectListWithEntryInGrid(
            gobjList, GO_TORCH_TARGET_BRAZIER, 30.0f);
        for (std::list<GameObject*>::const_iterator itr = gobjList.begin();
             itr != gobjList.end();
             ++itr) {
            Position pos;
            pos.Relocate(*itr);
...
class spell_fire_festival_fortitude : public SpellScript {
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_fire_festival_fortitude::SelectTargets,
            EFFECT_0,
...
class spell_bonfires_blessing : public AuraScript {
    ...
    {
        AfterEffectApply +=
            AuraEffectApplyFn(spell_bonfires_blessing::OnApply,
                              EFFECT_0,
...
class spell_gen_crab_disguise : public AuraScript {
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_gen_crab_disguise::OnApply,
                                              EFFECT_0,
                                              SPELL_AURA_FORCE_REACTION,
                                              AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove +=
            AuraEffectRemoveFn(spell_gen_crab_disguise::OnRemove,
                               EFFECT_0,
                               SPELL_AURA_FORCE_REACTION,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
struct npc_midsummer_ribbon_pole_target : public ScriptedAI {
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell) override
    {
        ...
        // prevent duplicates
        if (std::find(_dancerList.begin(), _dancerList.end(), dancer) !=
            _dancerList.end())
            return;
    ...
    {
        _bunny =
            me->FindNearestCreature(NPC_RIBBON_POLE_FIRE_SPIRAL_BUNNY, 10.0f);

        if (!_bunny)
            _bunny = DoSpawnCreature(NPC_RIBBON_POLE_FIRE_SPIRAL_BUNNY,
                                     0,
                                     0,
                                     0,
                                     0,
    ...
    void DoSpewLavaChecks()
    {
        ...
        if (_dancerList.size() >= THRESHOLD_SPEW_LAVA) {
            if (!_dancerList.empty()) {
                ...
                for (uint8 i = 0; (i < MAX_COUNT_SPEW_LAVA_TARGETS) &&
                                  (i < _dancerList.size());
                     i++) {
                    Player* dancerTarget = _dancerList[i];

                    if (dancerTarget) {
                        Creature* fireSpiralBunny =
                            dancerTarget->SummonCreature(
    ...
    void DoDancingFLameChecks()
    {
        if (_dancerList.size() >= THRESHOLD_DANCING_FLAMES) {
            ...
            me->GetCreaturesWithEntryInRange(
                crList, 20.0f, NPC_BIG_DANCING_FLAMES);
...
class spell_midsummer_ribbon_pole_firework : public SpellScript {
    ...
    void Register() override
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_midsummer_ribbon_pole_firework::ModDestHeight,
            EFFECT_0,
...
class spell_midsummer_ribbon_pole : public AuraScript {
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Unit* target = GetTarget()) {
            Creature* cr = target->FindNearestCreature(
                NPC_RIBBON_POLE_DEBUG_TARGET, 10.0f);
            if (!cr) {
            ...
            if (Aura* aur = target->GetAura(SPELL_RIBBON_POLE_XP))
                aur->SetDuration(
                    std::min(aur->GetDuration() + 3 * MINUTE * IN_MILLISECONDS,
                             60 * MINUTE * IN_MILLISECONDS));
            else {
                target->CastSpell(target, SPELL_RIBBON_POLE_XP, true);

                // Achievement
                if ((GameTime::GetGameTime().count() - GetApplyTime()) > 60 &&
            ...
            // Achievement
            if ((time(nullptr) - GetApplyTime()) > 60 &&
                target->GetTypeId() == TYPEID_PLAYER)
                target->ToPlayer()->UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET,
                    58934,
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_midsummer_ribbon_pole::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_PERIODIC_DUMMY,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_midsummer_ribbon_pole::HandleEffectPeriodic,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_midsummer_ribbon_pole_visual : public SpellScript {
        ...
        void UpdateTarget(WorldObject*& target)
    {
        ...
        std::list<Creature*> crList;
        target->GetCreaturesWithEntryInRange(
            crList, 30.0f, NPC_RIBBON_POLE_DEBUG_TARGET);
        if (crList.empty())
            return;

        for (std::list<Creature*>::const_iterator itr = crList.begin();
             itr != crList.end();
             ++itr) {
    ...
    void Register() override
    {
        OnObjectTargetSelect += SpellObjectTargetSelectFn(
            spell_midsummer_ribbon_pole_visual::UpdateTarget,
            EFFECT_0,
...
class spell_midsummer_torch_quest : public AuraScript {
                           ...
                           AuraEffectHandleModes /*mode*/)
    {
        ...
        if (Creature* cr = ar->SummonCreature(NPC_TORCH_TOSS_TARGET_BUNNY,
            ...
            CAST_AI(npc_midsummer_torch_target, cr->AI())
                ->SetPlayerGUID(
                    ar->GetGUID(),
                    (GetId() == SPELL_TORCH_TOSS_TRAINING ? 8 : 20));
        }
    }
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_midsummer_torch_quest::HandleEffectApply,
                              EFFECT_0,
                              SPELL_AURA_DETECT_AMORE,
                              AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove +=
            AuraEffectRemoveFn(spell_midsummer_torch_quest::HandleEffectRemove,
                               EFFECT_0,
                               SPELL_AURA_DETECT_AMORE,
                               AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_midsummer_fling_torch : public SpellScript {
    ...
    void ThrowNextTorch(Unit* caster)
    {
        ...
        // targets are located on a circle with fixed radius around the target
        // bunny first target is chosen randomly anywhere on the circle next
        // target is chosen on the opposite half of the circle so a minimum
        // flight duration of the torch is guaranteed
        float angle = 0.0f;
        ...
        caster->CastSpell(pos.GetPositionX(),
                          pos.GetPositionY(),
                          pos.GetPositionZ(),
                          SPELL_FLING_TORCH,
                          true);
        ...
        if (GetSpellInfo()->Id != SPELL_FLING_TORCH_DUMMY) {
            if (!handled)
                if (const WorldLocation* loc = GetExplTargetDest()) {
                    caster->CastSpell(loc->GetPositionX(),
                                      loc->GetPositionY(),
                                      loc->GetPositionZ(),
                                      SPELL_MISSED_TORCH,
                                      true);
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        if (Player* target = GetHitPlayer()) {
            ...
            if (Aura* aur = target->GetAura(SPELL_TORCH_COUNTER)) {
                ...
                uint8 count = 4;
                if (target->GetQuestStatus(target->GetTeamId()
                ...
                if (aur->GetStackAmount() >= count) {
                    // target->CastSpell(target, 46711, true); // Set Flag: all
                    // torch returning quests are complete
    ...
    void Register() override
    {
        ...
        if (m_scriptSpellId == SPELL_JUGGLE_TORCH) {
            OnEffectHitTarget +=
                SpellEffectFn(spell_midsummer_fling_torch::HandleScript,
                              EFFECT_0,
...
class spell_midsummer_torch_catch : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (player->GetQuestStatus(QUEST_TORCH_CATCHING_A) ==
                QUEST_STATUS_REWARDED ||
            player->GetQuestStatus(QUEST_TORCH_CATCHING_H) ==
                QUEST_STATUS_REWARDED) {
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_midsummer_torch_catch::HandleDummy,
                          EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
...
class spell_midsummer_summon_ahune_lieutenant : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (npcEntry)
            caster->SummonCreature(npcEntry,
                                   caster->GetPosition(),
    ...
    {
        OnEffectHit +=
            SpellEffectFn(spell_midsummer_summon_ahune_lieutenant::HandleDummy,
                          EFFECT_1,
