...
void KeepDatabaseAliveHandler(
    std::weak_ptr<Acore::Asio::DeadlineTimer> dbPingTimerRef,
    int32                                     dbPingInterval,
    boost::system::error_code const&          error);
void BanExpiryHandler(
    std::weak_ptr<Acore::Asio::DeadlineTimer> banExpiryCheckTimerRef,
...
int main(int argc, char** argv)
{
    ...
    auto configFile = fs::path(sConfigMgr->GetConfigPath() +
                               std::string(_ACORE_REALM_CONFIG));
    ...
    sRealmList->Initialize(
        *ioContext, sConfigMgr->GetOption<int32>("RealmsStateUpdateDelay", 20));

    std::shared_ptr<void> sRealmListHandle(nullptr,
                                           [](void*) { sRealmList->Close(); });
    ...
    std::shared_ptr<void> sAuthSocketMgrHandle(
        nullptr, [](void*) { sAuthSocketMgr.StopNetwork(); });
    ...
    std::shared_ptr<Acore::Asio::DeadlineTimer> dbPingTimer =
        std::make_shared<Acore::Asio::DeadlineTimer>(*ioContext);
    dbPingTimer->expires_from_now(boost::posix_time::minutes(dbPingInterval));
    dbPingTimer->async_wait(
        std::bind(&KeepDatabaseAliveHandler,
                  std::weak_ptr<Acore::Asio::DeadlineTimer>(dbPingTimer),
                  dbPingInterval,
                  std::placeholders::_1));

    int32 banExpiryCheckInterval =
        sConfigMgr->GetOption<int32>("BanExpiryCheckInterval", 60);
    std::shared_ptr<Acore::Asio::DeadlineTimer> banExpiryCheckTimer =
        std::make_shared<Acore::Asio::DeadlineTimer>(*ioContext);
    banExpiryCheckTimer->expires_from_now(
        boost::posix_time::seconds(banExpiryCheckInterval));
    banExpiryCheckTimer->async_wait(std::bind(
        &BanExpiryHandler,
...
bool StartDB()
{
    ...
    // Load databases
    // NOTE: While authserver is singlethreaded you should keep synch_threads
    // == 1. Increasing it is just silly since only 1 will be used ever.
...
{
    if (!error) {
        if (std::shared_ptr<Acore::Asio::IoContext> ioContext =
                ioContextRef.lock()) {
            ioContext->stop();
        }
    }
...
{
    if (!error) {
        if (std::shared_ptr<Acore::Asio::DeadlineTimer> dbPingTimer =
                dbPingTimerRef.lock()) {
            LOG_INFO("server.authserver",
                     "Ping MySQL to keep connection alive");
            LoginDatabase.KeepAlive();

            dbPingTimer->expires_from_now(
                boost::posix_time::minutes(dbPingInterval));
...
{
    if (!error) {
        if (std::shared_ptr<Acore::Asio::DeadlineTimer> banExpiryCheckTimer =
                banExpiryCheckTimerRef.lock()) {
            LoginDatabase.Execute(
                LoginDatabase.GetPreparedStatement(LOGIN_DEL_EXPIRED_IP_BANS));
            LoginDatabase.Execute(LoginDatabase.GetPreparedStatement(
