...
std::array<uint8, 16> VersionChallenge = {{0xBA,
                                           0xA3,
                                           0x1E,
                                           0x99,
                                           0xA0,
...
std::unordered_map<uint8, AuthHandler> AuthSession::InitHandlers()
{
    ...
    handlers[AUTH_LOGON_CHALLENGE]     = {STATUS_CHALLENGE,
                                          AUTH_LOGON_CHALLENGE_INITIAL_SIZE,
                                          &AuthSession::HandleLogonChallenge};
    handlers[AUTH_LOGON_PROOF]         = {STATUS_LOGON_PROOF,
                                          sizeof(AUTH_LOGON_PROOF_C),
                                          &AuthSession::HandleLogonProof};
    handlers[AUTH_RECONNECT_CHALLENGE] = {
        STATUS_CHALLENGE,
        AUTH_LOGON_CHALLENGE_INITIAL_SIZE,
        &AuthSession::HandleReconnectChallenge};
    handlers[AUTH_RECONNECT_PROOF] = {STATUS_RECONNECT_PROOF,
                                      sizeof(AUTH_RECONNECT_PROOF_C),
                                      &AuthSession::HandleReconnectProof};
    handlers[REALM_LIST]           = {
        STATUS_AUTHED, REALM_LIST_PACKET_SIZE, &AuthSession::HandleRealmList};
...
{
    //          0        1          2           3             4             5
    // SELECT a.id, a.username, a.locked, a.lock_country, a.last_ip,
    // a.failed_logins,
    //                                 6 7
    ...
    // FROM account a LEFT JOIN account_access aa ON a.id = aa.id LEFT JOIN
    // account_banned ab ON ab.id = a.id AND ab.active = 1 LEFT JOIN ip_banned
    // ipb ON ipb.ip = ? WHERE a.username = ?
    ...
    SecurityLevel =
        static_cast<AccountTypes>(fields[10].Get<uint8>()) > SEC_CONSOLE
            ? SEC_CONSOLE
            : static_cast<AccountTypes>(fields[10].Get<uint8>());

    // Use our own uppercasing of the account name instead of using UPPER() in
    // mysql query This is how the account was created in the first place and
    // changing it now would result in breaking login for all accounts having
    // accented characters in their name
...
void AuthSession::Start()
{
    ...
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_INFO);
    stmt->SetData(0, ip_address);

    _queryProcessor.AddCallback(
        LoginDatabase.AsyncQuery(stmt).WithPreparedCallback(std::bind(
            &AuthSession::CheckIpCallback, this, std::placeholders::_1)));
}
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result) {
        ...
        if (banned) {
            ...
            SendPacket(pkt);
            LOG_DEBUG("session",
                      "[AuthSession::CheckIpCallback] Banned ip '{}:{}' tries "
                      "to login!",
    ...
    while (packet.GetActiveSize()) {
        ...
        if (cmd == AUTH_LOGON_CHALLENGE || cmd == AUTH_RECONNECT_CHALLENGE) {
            sAuthLogonChallenge_C* challenge =
                reinterpret_cast<sAuthLogonChallenge_C*>(
                    packet.GetReadPointer());
            size += challenge->size;
...
bool AuthSession::HandleLogonChallenge()
{
    ...
    sAuthLogonChallenge_C* challenge = reinterpret_cast<sAuthLogonChallenge_C*>(
        GetReadBuffer().GetReadPointer());
    if (challenge->size - (sizeof(sAuthLogonChallenge_C) -
                           AUTH_LOGON_CHALLENGE_INITIAL_SIZE - 1) !=
        challenge->I_len)
        return false;
    ...
    _expversion = uint8(AuthHelper::IsPostBCAcceptedClientBuild(_build)
                            ? POST_BC_EXP_FLAG
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
    ...
    // If the IP is 'locked', check that the player comes indeed from the
    // correct IP address
    if (_accountInfo.IsLockedToIP) {
        LOG_DEBUG("server.authserver",
                  "[AuthChallenge] Account '{}' is locked to IP - '{}' is "
                  "logging in from '{}'",
                  _accountInfo.Login,
                  _accountInfo.LastIP,
                  ipAddress);
        ...
        if (IpLocationRecord const* location =
                sIPLocation->GetLocationRecord(ipAddress))
            _ipCountry = location->CountryCode;

        LOG_DEBUG("server.authserver",
                  "[AuthChallenge] Account '{}' is not locked to ip",
                  _accountInfo.Login);
        if (_accountInfo.LockCountry.empty() ||
            _accountInfo.LockCountry == "00")
            LOG_DEBUG("server.authserver",
                      "[AuthChallenge] Account '{}' is not locked to country",
                      _accountInfo.Login);
        else if (!_ipCountry.empty()) {
            LOG_DEBUG("server.authserver",
                      "[AuthChallenge] Account '{}' is locked to country: '{}' "
                      "Player country is '{}'",
                      _accountInfo.Login,
                      _accountInfo.LockCountry,
                      _ipCountry);
    ...
    if (_accountInfo.IsBanned) {
        if (_accountInfo.IsPermanentlyBanned) {
            ...
            LOG_INFO(
                ...
                "'{}:{}' [AuthChallenge] Banned account {} tried to login!",
                ipAddress,
                port,
                _accountInfo.Login);
            return;
            ...
            SendPacket(pkt);
            LOG_INFO("server.authserver.banned",
                     "'{}:{}' [AuthChallenge] Temporarily banned account {} "
                     "tried to login!",
                     ipAddress,
                     port,
                     _accountInfo.Login);
    ...
    if (!fields[11].IsNull()) {
        ...
        if (auto const& secret =
                sSecretMgr->GetSecret(SECRET_TOTP_MASTER_KEY)) {
            bool success = Acore::Crypto::AEDecrypt<Acore::Crypto::AES>(
                *_totpSecret, *secret);
            if (!success) {
                pkt << uint8(WOW_FAIL_DB_BUSY);
                LOG_ERROR("server.authserver",
                          "[AuthChallenge] Account '{}' has invalid ciphertext "
                          "for TOTP token key stored",
                          _accountInfo.Login);
                SendPacket(pkt);
                return;
            }
...
bool AuthSession::HandleLogonProof()
{
    ...
    // Read the packet
    sAuthLogonProof_C* logonProof =
        reinterpret_cast<sAuthLogonProof_C*>(GetReadBuffer().GetReadPointer());
    ...
    if (Optional<SessionKey> K = _srp6->VerifyChallengeResponse(
            logonProof->A, logonProof->clientM)) {
        _sessionKey = *K;
        // Check auth token
        ...
        if (sentToken && _totpSecret) {
            uint8 size =
                *(GetReadBuffer().GetReadPointer() + sizeof(sAuthLogonProof_C));
            std::string token(reinterpret_cast<char*>(
                                  GetReadBuffer().GetReadPointer() +
                                  sizeof(sAuthLogonProof_C) + sizeof(size)),
                              size);
            GetReadBuffer().ReadCompleted(sizeof(size) + size);

            uint32 incomingToken = *Acore::StringTo<uint32>(token);
            tokenSuccess =
                Acore::Crypto::TOTP::ValidateToken(*_totpSecret, incomingToken);
        ...
        if (!VerifyVersion(logonProof->A.data(),
                           logonProof->A.size(),
                           logonProof->crc_hash,
                           false)) {
            ByteBuffer packet;
        ...
        // Update the sessionkey, last_ip, last login time and reset number of
        // failed logins in the account table for this account No SQL injection
        // (escaped user name) and IP address as received by socket
        ...
        std::string address =
            sConfigMgr->GetOption<bool>(
                ...
                : "0.0.0.0";
        LoginDatabasePreparedStatement* stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_UPD_LOGONPROOF);
        ...
        Acore::Crypto::SHA1::Digest M2 =
            Acore::Crypto::SRP6::GetSessionVerifier(
                logonProof->A, logonProof->clientM, _sessionKey);
        ...
        if (_expversion & POST_BC_EXP_FLAG) // 2.x and 3.x clients
        {
            ...
            proof.error = 0;
            proof.AccountFlags =
                0x00800000; // 0x01 = GM, 0x08 = Trial, 0x00800000 = Pro pass
                            // (arena tournament)
            proof.SurveyId   = 0;
        ...
        LOG_INFO("server.authserver.hack",
                 "'{}:{}' [AuthChallenge] account {} tried to login with "
                 "invalid password!",
                 GetRemoteIpAddress().to_string(),
                 GetRemotePort(),
                 _accountInfo.Login);

        uint32 MaxWrongPassCount =
            sConfigMgr->GetOption<int32>("WrongPass.MaxCount", 0);

        // We can not include the failed account login hook. However, this is a
        // workaround to still log this.
        if (sConfigMgr->GetOption<bool>("WrongPass.Logging", false)) {
            LoginDatabasePreparedStatement* logstmt =
                LoginDatabase.GetPreparedStatement(LOGIN_INS_FALP_IP_LOGGING);
        ...
        if (MaxWrongPassCount > 0) {
            // Increment number of failed logins by one and if it reaches the
            // limit temporarily ban that account or IP
            LoginDatabasePreparedStatement* stmt =
                LoginDatabase.GetPreparedStatement(LOGIN_UPD_FAILEDLOGINS);
            stmt->SetData(0, _accountInfo.Login);
            LoginDatabase.Execute(stmt);

            if (++_accountInfo.FailedLogins >= MaxWrongPassCount) {
                uint32 WrongPassBanTime =
                    sConfigMgr->GetOption<int32>("WrongPass.BanTime", 600);
                bool WrongPassBanType =
                    sConfigMgr->GetOption<bool>("WrongPass.BanType", false);
                ...
                if (WrongPassBanType) {
                    ...
                    LOG_DEBUG("server.authserver",
                              "'{}:{}' [AuthChallenge] account {} got banned "
                              "for '{}' seconds because it failed to "
                              "authenticate '{}' times",
                              GetRemoteIpAddress().to_string(),
                              GetRemotePort(),
                              _accountInfo.Login,
                              WrongPassBanTime,
                              _accountInfo.FailedLogins);
                }
                else {
                    stmt = LoginDatabase.GetPreparedStatement(
                    ...
                    LOG_DEBUG("server.authserver",
                              "'{}:{}' [AuthChallenge] IP got banned for '{}' "
                              "seconds because account {} failed to "
                              "authenticate '{}' times",
                              GetRemoteIpAddress().to_string(),
                              GetRemotePort(),
                              WrongPassBanTime,
...
bool AuthSession::HandleReconnectChallenge()
{
    ...
    // Get the account details from the account table
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_RECONNECTCHALLENGE);
    stmt->SetData(0, GetRemoteIpAddress().to_string());
    stmt->SetData(1, login);

    _queryProcessor.AddCallback(
        LoginDatabase.AsyncQuery(stmt).WithPreparedCallback(
            std::bind(&AuthSession::ReconnectChallengeCallback,
                      this,
                      std::placeholders::_1)));
    return true;
}
...
bool AuthSession::HandleReconnectProof()
{
    ...
    sAuthReconnectProof_C* reconnectProof =
        reinterpret_cast<sAuthReconnectProof_C*>(
            GetReadBuffer().GetReadPointer());
    ...
    if (sha.GetDigest() == reconnectProof->R2) {
        if (!VerifyVersion(reconnectProof->R1,
                           sizeof(reconnectProof->R1),
                           reconnectProof->R3,
                           true)) {
            ByteBuffer packet;
        ...
        LOG_ERROR(
            ...
            "'{}:{}' [ERROR] user {} tried to login, but session is invalid.",
            GetRemoteIpAddress().to_string(),
            GetRemotePort(),
            _accountInfo.Login);
        return false;
...
bool AuthSession::HandleRealmList()
{
    ...
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_REALM_CHARACTER_COUNTS);
    stmt->SetData(0, _accountInfo.Id);

    _queryProcessor.AddCallback(
        LoginDatabase.AsyncQuery(stmt).WithPreparedCallback(std::bind(
            &AuthSession::RealmListCallback, this, std::placeholders::_1)));
...
void AuthSession::RealmListCallback(PreparedQueryResult result)
{
    ...
    // (including # of user characters in each realm)
    ByteBuffer pkt;
    ...
    for (auto const& [realmHandle, realm] : sRealmList->GetRealms()) {
        ...
        bool okBuild =
            ((_expversion & POST_BC_EXP_FLAG) && realm.Build == _build) ||
            ((_expversion & PRE_BC_EXP_FLAG) &&
             !AuthHelper::IsPreBCAcceptedClientBuild(realm.Build));
        ...
        if (!okBuild) {
            ...
            flag |= REALM_FLAG_OFFLINE |
                    REALM_FLAG_SPECIFYBUILD; // tell the client what build the
                                             // realm is for
        }
        ...
        if (_expversion & PRE_BC_EXP_FLAG && flag & REALM_FLAG_SPECIFYBUILD) {
            ...
            ss << name << " (" << buildInfo->MajorVersion << '.'
               << buildInfo->MinorVersion << '.' << buildInfo->BugfixVersion
               << ')';
            name = ss.str();
        }

        uint8 lock =
            (realm.AllowedSecurityLevel > _accountInfo.SecurityLevel) ? 1 : 0;
        ...
        pkt << name;
        pkt << boost::lexical_cast<std::string>(
            realm.GetAddressForClient(GetRemoteIpAddress()));
