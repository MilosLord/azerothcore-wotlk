...
static inline void PrintCliPrefix() { fmt::print(CLI_PREFIX); }
...
char** cli_completion(char const* text, int /*start*/, int /*end*/)
{
    ...
    vec                            = Acore::ChatCommands::GetAutoCompletionsFor(
        CliHandler(nullptr, nullptr), text);
...
void CliThread()
{
    ...
    ///- As long as the World is running (no World::m_stopEvent), get the
    ///command line and handle it
