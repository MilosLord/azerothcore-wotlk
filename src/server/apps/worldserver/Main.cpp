...
char serviceDescription[] =
    "AzerothCore World of Warcraft emulator world service";
/*
...
public:
    FreezeDetector(Acore::Asio::IoContext& ioContext, uint32 maxCoreStuckTime)
        : _timer(ioContext), _worldLoopCounter(0),
          _lastChangeMsTime(getMSTime()),
    ...
    static void Start(std::shared_ptr<FreezeDetector> const& freezeDetector)
    {
        ...
        freezeDetector->_timer.async_wait(
            std::bind(&FreezeDetector::Handler,
                      std::weak_ptr<FreezeDetector>(freezeDetector),
                      std::placeholders::_1));
    }

    static void Handler(std::weak_ptr<FreezeDetector>    freezeDetectorRef,
...
variables_map  GetConsoleArguments(int                           argc,
                                   char**                        argv,
                                   fs::path&                     configFile,
                                   [[maybe_unused]] std::string& cfg_service);
...
int main(int argc, char** argv)
{
    ...
    auto configFile =
        fs::path(sConfigMgr->GetConfigPath() + std::string(_ACORE_CORE_CONFIG));
        ...
        [&](boost::dll::shared_library* lib) {
            try {
                if (newTimerResolution)
                    lib->get<decltype(timeEndPeriod)>("timeEndPeriod")(
                        *newTimerResolution);
            }
    ...
    if (winmm->is_loaded()) {
        try {
            auto timeGetDevCapsPtr =
                winmm->get<decltype(timeGetDevCaps)>("timeGetDevCaps");
            // setup timer resolution
            TIMECAPS timeResolutionLimits;
            if (timeGetDevCapsPtr(&timeResolutionLimits, sizeof(TIMECAPS)) ==
                TIMERR_NOERROR) {
                auto timeBeginPeriodPtr =
                    winmm->get<decltype(timeBeginPeriod)>("timeBeginPeriod");
                newTimerResolution =
                    std::min(std::max(timeResolutionLimits.wPeriodMin, 1u),
                             timeResolutionLimits.wPeriodMax);
    ...
    sConfigMgr->Configure(
        configFile.generic_string(), {argv, argv + argc}, CONFIG_FILE_LIST);

    if (!sConfigMgr->LoadAppConfigs())
        return 1;

    std::shared_ptr<Acore::Asio::IoContext> ioContext =
        std::make_shared<Acore::Asio::IoContext>();

    // Init all logs
    sLog->RegisterAppender<AppenderDB>();
    // If logs are supposed to be handled async then we need to pass the
    // IoContext into the Log singleton
    sLog->Initialize(sConfigMgr->GetOption<bool>("Log.Async.Enable", false)
                         ? ioContext.get()
                         : nullptr);
    ...
    if (!pidFile.empty()) {
        if (uint32 pid = CreatePIDFile(pidFile))
            ...
            LOG_ERROR(
                "server",
                "Cannot create PID file {} (possible error: permission)\n",
                pidFile);
            return 1;
        }
    }
    ...
    sMetric->Initialize(realm.Name, *ioContext, []() {
        ...
        METRIC_VALUE("db_queue_character",
                     uint64(CharacterDatabase.QueueSize()));
    ...
    if (sConfigMgr->GetOption<bool>("SOAP.Enabled", false)) {
        soapThread.reset(
            new std::thread(
                ACSoapThread,
                sConfigMgr->GetOption<std::string>("SOAP.IP", "127.0.0.1"),
                uint16(sConfigMgr->GetOption<int32>("SOAP.Port", 7878))),
            [](std::thread* thr) {
                thr->join();
                delete thr;
            });
    ...
    std::string worldListener =
        sConfigMgr->GetOption<std::string>("BindIP", "0.0.0.0");
    ...
    if (!sWorldSocketMgr.StartWorldNetwork(
            *ioContext, worldListener, worldPort, networkThreads)) {
    ...
    std::shared_ptr<void> sWorldSocketMgrHandle(nullptr, [](void*) {
        ...
        sWorld->UpdateSessions(
            1); // real players unload required UpdateSessions call
    ...
    LoginDatabase.DirectExecute("UPDATE realmlist SET flag = flag & ~{}, "
                                "population = 0 WHERE id = '{}'",
                                REALM_FLAG_VERSION_MISMATCH,
                                realm.Id.Realm);
    realm.PopulationLevel = 0.0f;
    realm.Flags =
        RealmFlags(realm.Flags & ~uint32(REALM_FLAG_VERSION_MISMATCH));

    // Start the freeze check callback cycle in 5 seconds (cycle itself is 1
    // sec)
    std::shared_ptr<FreezeDetector> freezeDetector;
    if (int32 coreStuckTime =
            sConfigMgr->GetOption<int32>("MaxCoreStuckTime", 60)) {
        freezeDetector =
            std::make_shared<FreezeDetector>(*ioContext, coreStuckTime * 1000);
        FreezeDetector::Start(freezeDetector);
        LOG_INFO(
            "server.worldserver",
            "Starting up anti-freeze thread ({} seconds max stuck time)...",
            coreStuckTime);
    }
...
// Be kind and warn people of EOL deprecation :)
    ...
    if (MySQL::GetLibraryVersion() < 80000)
        LOG_WARN("server",
                 "WARNING: You are using MySQL version 5.7 which is soon "
                 "EOL!\nThis version will be deprecated. Consider upgrading to "
                 "MySQL 8.0 or 8.1!");
#endif
#if OPENSSL_VERSION_NUMBER < 0x30000000L
    LOG_WARN("server",
             "WARNING: You are using OpenSSL version 1.1 which is soon "
             "EOL!\nThis version will be deprecated. Consider upgrading to "
             "OpenSSL 3.0 or 3.1!");
#endif
    ...
    if (sConfigMgr->GetOption<bool>("Console.Enable", true) &&
        (m_ServiceStatus == -1) /* need disable console in service mode*/)
#else
    if (sConfigMgr->GetOption<bool>("Console.Enable", true))
#endif
    {
...
bool StartDB()
{
    ...
    DatabaseLoader loader(
        "server.worldserver", DatabaseLoader::DATABASE_NONE, AC_MODULES_LIST);
    ...
    LOG_INFO(
        "server.loading", "> Version DB world:     {}", sWorld->GetDBVersion());
...
void ClearOnlineAccounts()
{
    ...
    // pussywizard: tc query would set online=0 even if logged in on another
    // realm >_>
    LoginDatabase.DirectExecute(
        "UPDATE account SET online = 0 WHERE online = {}", realm.Id.Realm);
...
void ShutdownCLIThread(std::thread* cliThread)
{
    if (cliThread) {
        ...
        if (!CancelSynchronousIo(cliThread->native_handle())) {
            // if CancelSynchronousIo() fails, print the error and try with old
            // way
            DWORD  errorCode = GetLastError();
            LPCSTR errorBuffer;

            DWORD formatReturnCode = FormatMessage(
...
void WorldUpdateLoop()
{
    uint32 minUpdateDiff =
        uint32(sConfigMgr->GetOption<int32>("MinWorldUpdateTime", 1));
    uint32 realCurrTime = 0;
    uint32 realPrevTime = getMSTime();

    uint32 maxCoreStuckTime =
        uint32(sConfigMgr->GetOption<int32>("MaxCoreStuckTime", 60)) * 1000;
    ...
    while (!World::IsStopped()) {
        ...
        if (diff < minUpdateDiff) {
            ...
            if (sleepTime >= halfMaxCoreStuckTime)
                LOG_ERROR("server.worldserver",
                          "WorldUpdateLoop() waiting for {} ms with "
                          "MaxCoreStuckTime set to {} ms",
                          sleepTime,
                          maxCoreStuckTime);
...
{
    if (!error) {
        if (std::shared_ptr<FreezeDetector> freezeDetector =
                freezeDetectorRef.lock()) {
            ...
            // possible freeze
                ...
                uint32 msTimeDiff =
                    getMSTimeDiff(freezeDetector->_lastChangeMsTime, curtime);
                if (msTimeDiff > freezeDetector->_maxCoreStuckTimeInMs) {
                    LOG_ERROR("server.worldserver",
                              "World Thread hangs for {} ms, forcing a crash!",
                              msTimeDiff);
                    ABORT("World Thread hangs for {} ms, forcing a crash!",
                          msTimeDiff);
                }
            }

            freezeDetector->_timer.expires_from_now(
...
AsyncAcceptor* StartRaSocketAcceptor(Acore::Asio::IoContext& ioContext)
{
    ...
    std::string raListener =
        sConfigMgr->GetOption<std::string>("Ra.IP", "0.0.0.0");
...
bool LoadRealmInfo(Acore::Asio::IoContext& ioContext)
{
    ...
    Optional<boost::asio::ip::tcp::endpoint> externalAddress = resolver.Resolve(
        boost::asio::ip::tcp::v4(), fields[2].Get<std::string>(), "");
    if (!externalAddress) {
        LOG_ERROR("server.worldserver",
    ...
    realm.ExternalAddress =
        std::make_unique<boost::asio::ip::address>(externalAddress->address());

    Optional<boost::asio::ip::tcp::endpoint> localAddress = resolver.Resolve(
        boost::asio::ip::tcp::v4(), fields[3].Get<std::string>(), "");
    if (!localAddress) {
        LOG_ERROR("server.worldserver",
                  "Could not resolve address {}",
    ...
    realm.LocalAddress =
        std::make_unique<boost::asio::ip::address>(localAddress->address());

    Optional<boost::asio::ip::tcp::endpoint> localSubmask = resolver.Resolve(
        boost::asio::ip::tcp::v4(), fields[4].Get<std::string>(), "");
    if (!localSubmask) {
        LOG_ERROR("server.worldserver",
                  "Could not resolve address {}",
    ...
    realm.LocalSubnetMask =
        std::make_unique<boost::asio::ip::address>(localSubmask->address());
...
void AuctionListingRunnable()
{
    ...
    while (!World::IsStopped()) {
        ...
        if (!AsyncAuctionListingMgr::GetTempList().empty() ||
            !AsyncAuctionListingMgr::GetList().empty()) {
            {
                std::lock_guard<std::mutex> guard(
                    AsyncAuctionListingMgr::GetTempLock());

                for (auto const& delayEvent :
            ...
            for (auto itr = AsyncAuctionListingMgr::GetList().begin();
                 itr != AsyncAuctionListingMgr::GetList().end();
                 ++itr) {
    ...
    LOG_INFO("server",
             "Auction House Listing thread exiting without problems.");
}

variables_map GetConsoleArguments(int                           argc,
                                  char**                        argv,
                                  fs::path&                     configFile,
                                  [[maybe_unused]] std::string& configService)
{
