...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    if (!soap->userid || !soap->passwd) {
        LOG_DEBUG("network.soap",
                  "ACSoap: Client didn't provide login information");
        return 401;
    }
    ...
    if (!accountId) {
        LOG_DEBUG("network",
                  "ACSoap: Client used invalid username '{}'",
                  soap->userid);
        return 401;
    }
    ...
    if (!AccountMgr::CheckPassword(accountId, soap->passwd)) {
        LOG_DEBUG("network.soap",
                  "ACSoap: invalid password for account '{}'",
                  soap->userid);
        return 401;
    }
    ...
    if (AccountMgr::GetSecurity(accountId) < SEC_ADMINISTRATOR) {
        LOG_DEBUG(
            "network.soap", "ACSoap: {}'s gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    // completed
    {
        // CliCommandHolder will be deleted from world, accessing after queueing
        // is NOT save
        CliCommandHolder* cmd =
            new CliCommandHolder(&connection,
                                 command,
                                 &SOAPCommand::print,
...
struct Namespace namespaces[] = {
    {"SOAP-ENV",
     ...
     nullptr,
     nullptr}, // must be first
    {"SOAP-ENC",
     "http://schemas.xmlsoap.org/soap/encoding/",
     nullptr,
     nullptr}, // must be second
    {"xsi",
