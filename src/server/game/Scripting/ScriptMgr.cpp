...
inline void SCR_CLEAR()
{
    for (auto const& [scriptID, script] :
         ScriptRegistry<T>::ScriptPointerList) {
        delete script;
    }
...
void ScriptMgr::Initialize()
{
    ...
    ScriptRegistry<AchievementScript>::InitEnabledHooksIfNeeded(
        ACHIEVEMENTHOOK_END);
    ScriptRegistry<ArenaScript>::InitEnabledHooksIfNeeded(ARENAHOOK_END);
    ScriptRegistry<ArenaTeamScript>::InitEnabledHooksIfNeeded(
        ARENATEAMHOOK_END);
    ScriptRegistry<AuctionHouseScript>::InitEnabledHooksIfNeeded(
    ...
    ScriptRegistry<GameEventScript>::InitEnabledHooksIfNeeded(
        GAMEEVENTHOOK_END);
    ...
    ScriptRegistry<MovementHandlerScript>::InitEnabledHooksIfNeeded(
        MOVEMENTHOOK_END);
    ...
    ScriptRegistry<WorldObjectScript>::InitEnabledHooksIfNeeded(
        WORLDOBJECTHOOK_END);
    ...
    for (auto const& scriptName : sObjectMgr->GetScriptNames()) {
        if (uint32 sid = sObjectMgr->GetScriptId(scriptName)) {
            if (!ScriptRegistry<SpellScriptLoader>::GetScriptById(sid) &&
                ...
                !ScriptRegistry<AchievementCriteriaScript>::GetScriptById(
                    sid) &&
                ...
                LOG_ERROR("sql.sql",
                          "Script named '{}' is assigned in the database, but "
                          "has no code!",
                          scriptName);
            }
        }
...
void ScriptMgr::FillSpellSummary()
{
    ...
    for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i) {
        ...
        for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
            // Spell targets self.
            if (pTempSpell->Effects[j].TargetA.GetTarget() ==
                TARGET_UNIT_CASTER)
            ...
            if (pTempSpell->Effects[j].TargetA.GetTarget() ==
                ...
                pTempSpell->Effects[j].TargetA.GetTarget() ==
            ...
            if (pTempSpell->Effects[j].Effect == SPELL_EFFECT_HEAL ||
                ...
                (pTempSpell->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA &&
