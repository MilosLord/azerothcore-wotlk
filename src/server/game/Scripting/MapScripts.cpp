...
void Map::ScriptsStart(ScriptMapMap const& scripts,
                       uint32              id,
                       Object*             source,
                       Object*             target)
{
    ...
    ObjectGuid sourceGUID =
        source ? source->GetGUID()
               : ObjectGuid::Empty; // some script commands doesn't have source
    ObjectGuid targetGUID = target ? target->GetGUID() : ObjectGuid::Empty;
    ObjectGuid ownerGUID  = (source && source->GetTypeId() == TYPEID_ITEM)
    ...
    for (ScriptMap::const_iterator iter = s2->begin(); iter != s2->end();
         ++iter) {
        ScriptAction sa;
        ...
        m_scriptSchedule.insert(ScriptScheduleMap::value_type(
            time_t(GameTime::GetGameTime().count() + iter->first), sa));
        if (iter->first == 0)
            immedScript = true;
                             ...
                             Object*           target)
{
    ...
    sa.script = &script;
    m_scriptSchedule.insert(ScriptScheduleMap::value_type(
        time_t(GameTime::GetGameTime().count() + delay), sa));
...
inline Player* Map::_GetScriptPlayerSourceOrTarget(
    Object* source, Object* target, const ScriptInfo* scriptInfo) const
{
    Player* player = nullptr;
    if (!source && !target)
        LOG_ERROR("maps.script",
                  "{} source and target objects are nullptr.",
        ...
        if (!player)
            LOG_ERROR("maps.script",
                      "{} neither source nor target object is player (source: "
                      "TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, "
                      "Entry: {}, GUID: {}), skipping.",
                      scriptInfo->GetDebugInfo(),
...
inline Creature*
Map::_GetScriptCreatureSourceOrTarget(Object*           source,
                                      Object*           target,
                                      ...
                                      bool              bReverse) const
{
    ...
    if (!source && !target)
        ...
        if (!creature)
            LOG_ERROR("maps.script",
                      "{} neither source nor target are creatures (source: "
                      "TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, "
                      "Entry: {}, GUID: {}), skipping.",
                      scriptInfo->GetDebugInfo(),
                      source ? source->GetTypeId() : 0,
...
inline Unit* Map::_GetScriptUnit(Object*           obj,
                                 bool              isSource,
                                 const ScriptInfo* scriptInfo) const
{
    ...
    if (!obj)
        ...
        LOG_ERROR("maps.script",
                  "{} {} object is not unit (TypeId: {}, Entry: {}, GUID: {}), "
                  "skipping.",
                  scriptInfo->GetDebugInfo(),
                  isSource ? "source" : "target",
                  obj->GetTypeId(),
...
inline Player* Map::_GetScriptPlayer(Object*           obj,
                                     bool              isSource,
                                     const ScriptInfo* scriptInfo) const
{
    Player* player = nullptr;
...
inline Creature* Map::_GetScriptCreature(Object*           obj,
                                         bool              isSource,
                                         const ScriptInfo* scriptInfo) const
{
    Creature* creature = nullptr;
...
inline WorldObject* Map::_GetScriptWorldObject(
    Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
...
inline void Map::_ScriptProcessDoor(Object*           source,
                                    Object*           target,
                                    const ScriptInfo* scriptInfo) const
{
    bool                bOpen = false;
    ...
    int32               nTimeToToggle =
        std::max(15, int32(scriptInfo->ToggleDoor.ResetDelay));
    ...
    default:
        LOG_ERROR("maps.script",
                  "{} unknown command for _ScriptProcessDoor.",
                  scriptInfo->GetDebugInfo());
        return;
    }
    if (!guid)
        LOG_ERROR("maps.script",
                  "{} door guid is not specified.",
                  scriptInfo->GetDebugInfo());
    else if (!source)
        LOG_ERROR("maps.script",
                  "{} source object is nullptr.",
                  scriptInfo->GetDebugInfo());
    else if (!source->isType(TYPEMASK_UNIT))
        LOG_ERROR("maps.script",
                  "{} source object is not unit ({}), skipping.",
                  scriptInfo->GetDebugInfo(),
        ...
        if (!wSource)
            LOG_ERROR("maps.script",
                      "{} source object could not be casted to world object "
                      "({}), skipping.",
                      scriptInfo->GetDebugInfo(),
                      source->GetGUID().ToString());
        else {
            ...
            if (!pDoor)
                LOG_ERROR("maps.script",
                          "{} gameobject was not found (guid: {}).",
                          scriptInfo->GetDebugInfo(),
                          guid);
            else if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR)
                LOG_ERROR("maps.script",
                          "{} gameobject is not a door ({}).",
                          scriptInfo->GetDebugInfo(),
                          pDoor->GetGUID().ToString());
                ...
                if (target && target->isType(TYPEMASK_GAMEOBJECT)) {
                    ...
                    if (goTarget &&
                        goTarget->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
...
inline GameObject* Map::_FindGameObject(WorldObject*        searchObject,
                                        ObjectGuid::LowType guid) const
{
    auto bounds =
        searchObject->GetMap()->GetGameObjectBySpawnIdStore().equal_range(guid);
...
void Map::ScriptsProcess()
{
    ...
    while (!m_scriptSchedule.empty() &&
           (iter->first <= GameTime::GetGameTime().count())) {
        ...
        if (step.sourceGUID) {
            ...
            case HighGuid::Item: // as well as HIGHGUID_CONTAINER
                if (Player* player =
                        ObjectAccessor::GetPlayer(this, step.ownerGUID))
        ...
        case SCRIPT_COMMAND_TALK: {
            if (step.script->Talk.ChatType > CHAT_TYPE_WHISPER &&
                step.script->Talk.ChatType != CHAT_MSG_RAID_BOSS_WHISPER) {
                LOG_ERROR("maps.script",
                          "{} invalid chat type ({}) specified, skipping.",
                          step.script->GetDebugInfo(),
            ...
            if (step.script->Talk.Flags & SF_TALK_USE_PLAYER) {
                source =
                    _GetScriptPlayerSourceOrTarget(source, target, step.script);
            }
            else {
                source = _GetScriptCreatureSourceOrTarget(
                    source, target, step.script);
            }
            ...
            if (source) {
                ...
                if (!sourceUnit) {
                    LOG_ERROR("scripts",
                              "{} source object ({}) is not an unit, skipping.",
                              step.script->GetDebugInfo(),
                              source->GetGUID().ToString());
                    break;
                }
                ...
                case CHAT_TYPE_BOSS_EMOTE:
                    sourceUnit->TextEmote(step.script->Talk.TextID,
                                          target,
                                          step.script->Talk.ChatType ==
                ...
                case CHAT_MSG_RAID_BOSS_WHISPER: {
                    ...
                    if (!receiver)
                        LOG_ERROR("scripts",
                                  "{} attempt to whisper to non-player unit, "
                                  "skipping.",
                                  step.script->GetDebugInfo());
                    else
                        sourceUnit->Whisper(step.script->Talk.TextID,
                                            receiver,
                                            step.script->Talk.ChatType ==
        ...
        case SCRIPT_COMMAND_EMOTE:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                    source, target, step.script)) {
                if (step.script->Emote.Flags & SF_EMOTE_USE_STATE)
                    cSource->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                            step.script->Emote.EmoteID);
                else
        ...
        case SCRIPT_COMMAND_FIELD_SET:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                ...
                if (step.script->FieldSet.FieldID <= OBJECT_FIELD_ENTRY ||
                    step.script->FieldSet.FieldID >= cSource->GetValuesCount())
                    LOG_ERROR("maps.script",
                              "{} wrong field {} (max count: {}) in object "
                              "({}) specified, skipping.",
                              step.script->GetDebugInfo(),
                              step.script->FieldSet.FieldID,
                              cSource->GetValuesCount(),
                              cSource->GetGUID().ToString());
                else
                    cSource->SetUInt32Value(step.script->FieldSet.FieldID,
                                            step.script->FieldSet.FieldValue);
            }
            break;
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                ...
                if (step.script->MoveTo.TravelTime != 0) {
                    float speed =
                        ...
                        ((float)step.script->MoveTo.TravelTime * 0.001f);
        ...
        case SCRIPT_COMMAND_FLAG_SET:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                    source, target, step.script)) {
                // Validate field number.
                if (step.script->FlagToggle.FieldID <= OBJECT_FIELD_ENTRY ||
                    step.script->FlagToggle.FieldID >=
                        cSource->GetValuesCount())
                    LOG_ERROR("maps.script",
                              "{} wrong field {} (max count: {}) in object "
                              "({}) specified, skipping.",
                    ...
                    cSource->SetFlag(step.script->FlagToggle.FieldID,
                                     step.script->FlagToggle.FieldValue);
            }
            break;
        ...
        case SCRIPT_COMMAND_FLAG_REMOVE:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                ...
                if (step.script->FlagToggle.FieldID <= OBJECT_FIELD_ENTRY ||
                    ...
                    cSource->RemoveFlag(step.script->FlagToggle.FieldID,
                                        step.script->FlagToggle.FieldValue);
            }
            break;
        ...
        case SCRIPT_COMMAND_TELEPORT_TO:
            if (step.script->TeleportTo.Flags & SF_TELEPORT_USE_CREATURE) {
                // Source or target must be Creature.
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                        source, target, step.script, true))
                    cSource->NearTeleportTo(
                ...
                if (Player* player = _GetScriptPlayerSourceOrTarget(
                        source, target, step.script))
        ...
        case SCRIPT_COMMAND_QUEST_EXPLORED: {
            ...
            if (!target) {
                LOG_ERROR("maps.script",
                          "{} target object is nullptr.",
                          step.script->GetDebugInfo());
                break;
            }

            // when script called for item spell casting then target == (unit or
            // GO) and source is player
            WorldObject* worldObject;
            Player*      player = target->ToPlayer();
            if (player) {
                if (source->GetTypeId() != TYPEID_UNIT &&
                    source->GetTypeId() != TYPEID_GAMEOBJECT &&
                    source->GetTypeId() != TYPEID_PLAYER) {
                    LOG_ERROR("maps.script",
                              "{} source is not unit, gameobject or player "
                              "({}), skipping.",
                              step.script->GetDebugInfo(),
                              source->GetGUID().ToString());
                    break;
                ...
                if (player) {
                    if (target->GetTypeId() != TYPEID_UNIT &&
                        target->GetTypeId() != TYPEID_GAMEOBJECT &&
                        target->GetTypeId() != TYPEID_PLAYER) {
                        LOG_ERROR("maps.script",
                                  "{} target is not unit, gameobject or player "
                                  "({}), skipping.",
                                  step.script->GetDebugInfo(),
                    ...
                    LOG_ERROR("maps.script",
                              "{} neither source nor target is player (source: "
                              "{}; target: {}), skipping.",
                              step.script->GetDebugInfo(),
                              source->GetGUID().ToString(),
                              target->GetGUID().ToString());
            ...
            if ((worldObject->GetTypeId() != TYPEID_UNIT ||
                 ((Unit*)worldObject)->IsAlive()) &&
                (step.script->QuestExplored.Distance == 0 ||
                 worldObject->IsWithinDistInMap(
                     player, float(step.script->QuestExplored.Distance))))
                player->GroupEventHappens(step.script->QuestExplored.QuestID,
                                          worldObject);
            else
            ...
            if (Player* player = _GetScriptPlayerSourceOrTarget(
                ...
                if (step.script->KillCredit.Flags & SF_KILLCREDIT_REWARD_GROUP)
                    player->RewardPlayerAndGroupAtEvent(
                        step.script->KillCredit.CreatureEntry, player);
                else
                    player->KilledMonsterCredit(
                        step.script->KillCredit.CreatureEntry);
            }
            break;
        ...
        case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            if (!step.script->RespawnGameobject.GOGuid) {
                LOG_ERROR("maps.script",
                          "{} gameobject guid (datalong) is not specified.",
                          step.script->GetDebugInfo());
                break;
            }

            // Source or target must be WorldObject.
            if (WorldObject* pSummoner =
                    _GetScriptWorldObject(source, true, step.script)) {
                GameObject* pGO = _FindGameObject(
                    pSummoner, step.script->RespawnGameobject.GOGuid);
                if (!pGO) {
                    LOG_ERROR("maps.script",
                              "{} gameobject was not found (guid: {}).",
                              step.script->GetDebugInfo(),
                ...
                if (pGO->GetGoType() == GAMEOBJECT_TYPE_FISHINGNODE ||
                    ...
                    LOG_ERROR("maps.script",
                              "{} can not be used with gameobject of type {} "
                              "(guid: {}).",
                              step.script->GetDebugInfo(),
                              uint32(pGO->GetGoType()),
                              step.script->RespawnGameobject.GOGuid);
                    break;
                }
                ...
                if (!pGO->isSpawned()) {
                    int32 nTimeToDespawn = std::max(
                        5, int32(step.script->RespawnGameobject.DespawnDelay));
        ...
        case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE: {
            ...
            if (WorldObject* pSummoner =
                ...
                if (!step.script->TempSummonCreature.CreatureEntry)
                    LOG_ERROR("maps.script",
                              "{} creature entry (datalong) is not specified.",
                              step.script->GetDebugInfo());
                else {
                    uint32 entry =
                        step.script->TempSummonCreature.CreatureEntry;
                    ...
                    if (step.script->TempSummonCreature.CheckIfExists)
                        if (Unit* trigger =
                                pSummoner->SummonTrigger(x, y, z, o, 1))
                    ...
                    if (!pSummoner->SummonCreature(
                            ...
                            step.script->TempSummonCreature.DespawnDelay))
                        LOG_ERROR(
                            "maps.script",
                            "{} creature was not spawned (entry: {}).",
                            step.script->GetDebugInfo(),
                            step.script->TempSummonCreature.CreatureEntry);
                }
            }
        ...
        case SCRIPT_COMMAND_REMOVE_AURA: {
            ...
            bool bReverse =
                step.script->RemoveAura.Flags & SF_REMOVEAURA_REVERSE;
            if (Unit* unit = _GetScriptUnit(
                    bReverse ? source : target, bReverse, step.script))
        ...
        case SCRIPT_COMMAND_CAST_SPELL: {
            ...
            // source/target cast spell at target/source (script->datalong2: 0:
            // s->t 1: s->s 2: t->t 3: t->s
                ...
                uTarget =
                    uSource
                        ? GetClosestCreatureWithEntry(
                              uSource,
                              std::abs(step.script->CastSpell.CreatureEntry),
                              step.script->CastSpell.SearchRadius)
                        : nullptr;
                break;
            }
            ...
            if (!uSource || !uSource->isType(TYPEMASK_UNIT)) {
                LOG_ERROR("maps.script",
                          "{} no source unit found for spell {}",
                          step.script->GetDebugInfo(),
                          step.script->CastSpell.SpellID);
                break;
            }

            if (!uTarget || !uTarget->isType(TYPEMASK_UNIT)) {
                LOG_ERROR("maps.script",
                          "{} no target unit found for spell {}",
                          step.script->GetDebugInfo(),
                          step.script->CastSpell.SpellID);
                break;
            }
            ...
            bool triggered = (step.script->CastSpell.Flags != 4)
                                 ? step.script->CastSpell.CreatureEntry &
                                       SF_CASTSPELL_TRIGGERED
                                 : step.script->CastSpell.CreatureEntry < 0;
            uSource->CastSpell(
                uTarget, step.script->CastSpell.SpellID, triggered);
            break;
        }
        ...
        case SCRIPT_COMMAND_PLAY_SOUND:
            ...
            if (WorldObject* object =
                ...
                if (step.script->Playsound.Flags & SF_PLAYSOUND_DISTANCE_SOUND)
                    object->PlayDistanceSound(step.script->Playsound.SoundID,
                                              player);
                else
                    object->PlayDirectSound(step.script->Playsound.SoundID,
                                            player);
            }
            break;
        ...
        case SCRIPT_COMMAND_CREATE_ITEM:
            ...
            if (Player* pReceiver = _GetScriptPlayerSourceOrTarget(
                    source, target, step.script)) {
                ...
                InventoryResult msg = pReceiver->CanStoreNewItem(
                    ...
                    NULL_SLOT,
                    dest,
                    step.script->CreateItem.ItemEntry,
                    step.script->CreateItem.Amount);
                if (msg == EQUIP_ERR_OK) {
                    if (Item* item = pReceiver->StoreNewItem(
                            dest, step.script->CreateItem.ItemEntry, true))
                        pReceiver->SendNewItem(
                            item, step.script->CreateItem.Amount, false, true);
                }
                else
                    pReceiver->SendEquipError(
                        msg,
                        nullptr,
                        nullptr,
                        step.script->CreateItem.ItemEntry);
        ...
        case SCRIPT_COMMAND_DESPAWN_SELF:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(
                    source, target, step.script, true))
                cSource->DespawnOrUnsummon(
                    step.script->DespawnSelf.DespawnDelay);
            break;
        ...
        case SCRIPT_COMMAND_LOAD_PATH:
            ...
            if (Unit* unit = _GetScriptUnit(source, true, step.script)) {
                if (!sWaypointMgr->GetPath(step.script->LoadPath.PathID))
                    LOG_ERROR(
                        "maps.script",
                        "{} source object has an invalid path ({}), skipping.",
                        step.script->GetDebugInfo(),
                        step.script->LoadPath.PathID);
                else
                    unit->GetMotionMaster()->MovePath(
                        step.script->LoadPath.PathID,
                        step.script->LoadPath.IsRepeatable);
            }
            break;
        ...
        case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT: {
            if (!step.script->CallScript.CreatureEntry) {
                LOG_ERROR("maps.script",
                          "{} creature entry is not specified, skipping.",
                          step.script->GetDebugInfo());
                break;
            }
            if (!step.script->CallScript.ScriptID) {
                LOG_ERROR("maps.script",
                          "{} script id is not specified, skipping.",
                          step.script->GetDebugInfo());
                break;
            }

            Creature* cTarget        = nullptr;
            auto      creatureBounds = _creatureBySpawnIdStore.equal_range(
                step.script->CallScript.CreatureEntry);
            if (creatureBounds.first != creatureBounds.second) {
                // Prefer alive (last respawned) creature
                auto creatureItr = std::find_if(
                    creatureBounds.first,
                    creatureBounds.second,
                    [](Map::CreatureBySpawnIdContainer::value_type const&
                           pair) { return pair.second->IsAlive(); });
                cTarget = creatureItr != creatureBounds.second
                              ? creatureItr->second
                              : creatureBounds.first->second;
            }

            if (!cTarget) {
                LOG_ERROR("maps.script",
                          "{} target was not found (entry: {})",
                          step.script->GetDebugInfo(),
                          step.script->CallScript.CreatureEntry);
                break;
            }
            ...
            ScriptMapMap* datamap = GetScriptsMapByType(
                ScriptsType(step.script->CallScript.ScriptType));
            // if no scriptmap present...
            if (!datamap) {
                LOG_ERROR("maps.script",
                          "{} unknown scriptmap ({}) specified, skipping.",
                          step.script->GetDebugInfo(),
                          step.script->CallScript.ScriptType);
                break;
            }

            // Insert script into schedule but do not start it
            ScriptsStart(
                *datamap, step.script->CallScript.ScriptID, cTarget, nullptr);
            break;
        }
        ...
        case SCRIPT_COMMAND_ORIENTATION:
            ...
            if (Unit* sourceUnit = _GetScriptUnit(source, true, step.script)) {
                if (step.script->Orientation.Flags &
                    SF_ORIENTATION_FACE_TARGET) {
                    // Target must be Unit.
                    Unit* targetUnit =
                        _GetScriptUnit(target, false, step.script);
                    if (!targetUnit)
                        break;
                    ...
                    sourceUnit->SetFacingTo(
                        step.script->Orientation.Orientation);
            }
            break;
        ...
        case SCRIPT_COMMAND_EQUIP:
            ...
            if (Creature* cSource =
                    _GetScriptCreature(source, true, step.script))
        ...
        case SCRIPT_COMMAND_MOVEMENT:
            ...
            if (Creature* cSource =
                ...
                case RANDOM_MOTION_TYPE:
                    cSource->GetMotionMaster()->MoveRandom(
                        (float)step.script->Movement.MovementDistance);
                    break;
                case WAYPOINT_MOTION_TYPE:
                    cSource->GetMotionMaster()->MovePath(
                        step.script->Movement.Path, false);
                    break;
                }
            }
            break;
