...
{
    CALL_ENABLED_HOOKS(
        GuildScript, GUILDHOOK_ON_DISBAND, script->OnDisband(guild));
}
                                     ...
                                     bool          sendAllSlots)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanGuildSendBankList(guild, session, tabId, sendAllSlots));
}
