...
void ForeachMaps(Map* map, std::function<void(ScriptName*)> const& executeHook)
{
    ...
    if constexpr (std::is_same_v<ScriptName, WorldMapScript>) {
        ...
        static_assert(Acore::dependant_false_v<ScriptName>,
                      "Unsupported type used for ForeachMaps");
    }
...
void ScriptMgr::OnCreateMap(Map* map)
{
    ...
    ExecuteScript<AllMapScript>(
        [&](AllMapScript* script) { script->OnCreateMap(map); });

    ForeachMaps<WorldMapScript>(
        map, [&](WorldMapScript* script) { script->OnCreate(map); });

    ForeachMaps<InstanceMapScript>(map, [&](InstanceMapScript* script) {
        script->OnCreate((InstanceMap*)map);
    });

    ForeachMaps<BattlegroundMapScript>(map, [&](BattlegroundMapScript* script) {
...
void ScriptMgr::OnDestroyMap(Map* map)
{
    ...
    ExecuteScript<AllMapScript>(
        [&](AllMapScript* script) { script->OnDestroyMap(map); });

    ForeachMaps<WorldMapScript>(
        map, [&](WorldMapScript* script) { script->OnDestroy(map); });
...
void ScriptMgr::OnLoadGridMap(Map* map, GridMap* gmap, uint32 gx, uint32 gy)
{
    ...
    ForeachMaps<WorldMapScript>(map, [&](WorldMapScript* script) {
...
void ScriptMgr::OnPlayerEnterMap(Map* map, Player* player)
{
    ...
    ExecuteScript<AllMapScript>(
        [&](AllMapScript* script) { script->OnPlayerEnterAll(map, player); });

    ExecuteScript<PlayerScript>(
        [=](PlayerScript* script) { script->OnMapChanged(player); });
...
void ScriptMgr::OnPlayerLeaveMap(Map* map, Player* player)
{
    ...
    ExecuteScript<AllMapScript>(
        [&](AllMapScript* script) { script->OnPlayerLeaveAll(map, player); });
...
void ScriptMgr::OnMapUpdate(Map* map, uint32 diff)
{
    ...
    ExecuteScript<AllMapScript>(
        [&](AllMapScript* script) { script->OnMapUpdate(map, diff); });

    ForeachMaps<WorldMapScript>(
        map, [&](WorldMapScript* script) { script->OnUpdate(map, diff); });
                                             ...
                                             uint32 completedEncounterMask)
{
    ExecuteScript<AllMapScript>([&](AllMapScript* script) {
        script->OnBeforeCreateInstanceScript(
            instanceMap, instanceData, load, data, completedEncounterMask);
    });
}
