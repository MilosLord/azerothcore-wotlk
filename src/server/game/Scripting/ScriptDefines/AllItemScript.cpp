...
bool ScriptMgr::OnQuestAccept(Player* player, Item* item, Quest const* quest)
{
    ...
    auto tempScript =
        ScriptRegistry<ItemScript>::GetScriptById(item->GetScriptId());
...
bool ScriptMgr::OnItemExpire(Player* player, ItemTemplate const* proto)
{
    ...
    auto tempScript =
        ScriptRegistry<ItemScript>::GetScriptById(proto->ScriptId);
                                      ...
                                      Item*            item)
{
    ...
    auto tempScript =
        ScriptRegistry<ItemScript>::GetScriptById(item->GetScriptId());
    return tempScript ? tempScript->OnCastItemCombatSpell(
                            player, victim, spellInfo, item)
                      : true;
}
...
void ScriptMgr::OnGossipSelectCode(
    Player* player, Item* item, uint32 sender, uint32 action, const char* code)
{
    ASSERT(player);
    ASSERT(item);
