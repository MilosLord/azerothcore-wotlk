...
{
    CALL_ENABLED_BOOLEAN_HOOKS(MiscScript,
                               MISCHOOK_CAN_APPLY_SOULBOUND_FLAG,
                               !script->CanApplySoulboundFlag(item, proto));
}
...
bool ScriptMgr::CanItemApplyEquipSpell(Player* player, Item* item)
{
    CALL_ENABLED_BOOLEAN_HOOKS(MiscScript,
                               ...
                               !script->CanItemApplyEquipSpell(player, item));
}
                                    ...
                                    Creature*           creature)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanSendAuctionHello(session, guid, creature));
}
                                         ...
                                         uint8&  castFlags)
{
    CALL_ENABLED_HOOKS(MiscScript,
                       ...
                       script->ValidateSpellAtCastSpell(
                           player, oldSpellId, spellId, castCount, castFlags));
}
                                               ...
                                               uint32  spellId)
{
    CALL_ENABLED_HOOKS(MiscScript,
                       ...
                       script->ValidateSpellAtCastSpellResult(
                           player, mover, spell, oldSpellId, spellId));
}
                                           ...
                                           uint16              lootMode)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnAfterLootTemplateProcess(
            loot, tab, store, lootOwner, personal, noEmptyError, lootMode));
}
