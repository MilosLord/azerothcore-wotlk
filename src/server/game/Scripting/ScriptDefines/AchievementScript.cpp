    ...
    CriteriaProgress const*         progress)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->IsCompletedCriteria(
            mgr, achievementCriteria, achievement, progress));
}
                                 ...
                                 SystemTimePoint             completionTime)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->IsRealmCompleted(globalmgr, achievement, completionTime));
}

void ScriptMgr::OnBeforeCheckCriteria(
    AchievementMgr*                                   mgr,
    std::list<AchievementCriteriaEntry const*> const* achievementCriteriaList)
{
    CALL_ENABLED_HOOKS(
        AchievementScript,
