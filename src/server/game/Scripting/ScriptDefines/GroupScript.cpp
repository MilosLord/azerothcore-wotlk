...
void ScriptMgr::OnGroupAddMember(Group* group, ObjectGuid guid)
{
    ...
    CALL_ENABLED_HOOKS(
        GroupScript, GROUPHOOK_ON_ADD_MEMBER, script->OnAddMember(group, guid));
}
...
void ScriptMgr::OnGroupInviteMember(Group* group, ObjectGuid guid)
{
    ...
    CALL_ENABLED_HOOKS(GroupScript,
...
void ScriptMgr::OnGroupDisband(Group* group)
{
    ...
    CALL_ENABLED_HOOKS(
        GroupScript, GROUPHOOK_ON_DISBAND, script->OnDisband(group));
}
                                              ...
                                              uint32 arenaSlot)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanGroupJoinBattlegroundQueue(
            group, member, bgTemplate, MinPlayerCount, isRated, arenaSlot));
}

void ScriptMgr::OnCreate(Group* group, Player* leader)
{
    CALL_ENABLED_HOOKS(
        GroupScript, GROUPHOOK_ON_CREATE, script->OnCreate(group, leader));
}
