...
bool ScriptMgr::OnGossipHello(Player* player, Creature* creature)
{
    ...
    auto ret =
        IsValidBoolScript<AllCreatureScript>([&](AllCreatureScript* script) {
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
                               ...
                               uint32    action)
{
    ...
    auto ret =
        IsValidBoolScript<AllCreatureScript>([&](AllCreatureScript* script) {
            return script->CanCreatureGossipSelect(
                player, creature, sender, action);
        });
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    return tempScript
               ? tempScript->OnGossipSelect(player, creature, sender, action)
               : false;
}
                                   ...
                                   const char* code)
{
    ...
    auto ret =
        IsValidBoolScript<AllCreatureScript>([&](AllCreatureScript* script) {
            return script->CanCreatureGossipSelectCode(
                player, creature, sender, action, code);
        });
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    return tempScript ? tempScript->OnGossipSelectCode(
                            player, creature, sender, action, code)
                      : false;
}
                              ...
                              Quest const* quest)
{
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    ClearGossipMenuFor(player);
    return tempScript ? tempScript->OnQuestAccept(player, creature, quest)
                      : false;
}
                              ...
                              Quest const* quest)
{
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    ClearGossipMenuFor(player);
    return tempScript ? tempScript->OnQuestSelect(player, creature, quest)
                      : false;
}
                                ...
                                Quest const* quest)
{
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    ClearGossipMenuFor(player);
    return tempScript ? tempScript->OnQuestComplete(player, creature, quest)
                      : false;
}
                              ...
                              uint32       opt)
{
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    ClearGossipMenuFor(player);
    return tempScript ? tempScript->OnQuestReward(player, creature, quest, opt)
                      : false;
}
...
uint32 ScriptMgr::GetDialogStatus(Player* player, Creature* creature)
{
    ...
    auto tempScript =
        ScriptRegistry<CreatureScript>::GetScriptById(creature->GetScriptId());
    return tempScript ? tempScript->GetDialogStatus(player, creature)
...
CreatureAI* ScriptMgr::GetCreatureAI(Creature* creature)
{
    ...
    auto retAI = GetReturnAIScript<AllCreatureScript, CreatureAI>(
        [creature](AllCreatureScript* script) {
...
void ScriptMgr::OnCreatureUpdate(Creature* creature, uint32 diff)
{
    ...
    if (auto tempScript = ScriptRegistry<CreatureScript>::GetScriptById(
