                                      ...
                                      ObjectGuid::LowType          itemGuid)
{
    ...
    CALL_ENABLED_HOOKS(GlobalScript,
                                ...
                                LootStore const& store)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnAfterRefCount(
            player, LootStoreItem, loot, canRate, lootMode, maxcount, store));
}
                                                ...
                                                LootStore const& store)
{
    CALL_ENABLED_HOOKS(GlobalScript,
                       ...
                       script->OnAfterCalculateLootGroupAmount(
                                    ...
                                    LootStore const& store)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnBeforeDropAddItem(
            player, loot, canRate, lootMode, LootStoreItem, store));
}
                           ...
                           LootStore const&     store)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnItemRoll(player, lootStoreItem, chance, loot, store));
}
                                         ...
                                         LootStore const&  store)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnBeforeLootEqualChanced(player, equalChanced, loot, store));
}
...
void ScriptMgr::OnAfterUpdateEncounterState(
    Map*                        map,
    ...
    bool                        updated)
{
    CALL_ENABLED_HOOKS(GlobalScript,
                       ...
                       script->OnAfterUpdateEncounterState(map,
                                                           type,
                                                           creditEntry,
                                                           source,
                                                           difficulty_fixed,
                                                ...
                                                bool& update)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnBeforeWorldObjectSetPhaseMask(worldObject,
                                                oldPhaseMask,
                                                newPhaseMask,
                                                useCombinedPhases,
                                                update));
                                            ...
                                            SpellModifier const* mod)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnIsAffectedBySpellModCheck(affectSpell, checkSpell, mod));
}

bool ScriptMgr::OnSpellHealingBonusTakenNegativeModifiers(
    Unit const*      target,
    Unit const*      caster,
    ...
    float&           val)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        ...
        script->OnSpellHealingBonusTakenNegativeModifiers(
            target, caster, spellInfo, val));
}
                                            ...
                                            ObjectGuid    source)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        script->OnAllowedForPlayerLootCheck(player, source));
}
                                              ...
                                              ObjectGuid    source)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        script->OnAllowedToLootContainerCheck(player, source));
}

/**
 * @brief Called when an instance Id is deleted, usually because it expired or
 * no players are bound to it anymore.
 *
...
/**
 * @brief Called when any raid boss has their state updated (e.g. pull, reset,
 * kill).
 * @details Careful checks for old- and newState are required, since it can fire
 * multiple times and not only when combat starts/ends.
 *
