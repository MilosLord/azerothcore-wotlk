...
{
    CALL_ENABLED_HOOKS(
        UnitScript, UNITHOOK_ON_HEAL, script->OnHeal(healer, reciever, gain));
}

void ScriptMgr::OnDamage(Unit* attacker, Unit* victim, uint32& damage)
{
    CALL_ENABLED_HOOKS(UnitScript,
                       UNITHOOK_ON_DAMAGE,
                                              ...
                                              SpellInfo const* spellInfo)
{
    CALL_ENABLED_HOOKS(UnitScript,
                       ...
                       script->ModifyPeriodicDamageAurasTick(
                           target, attacker, damage, spellInfo));
}
                             ...
                             DamageEffectType damagetype)
{
    ...
    for (auto const& [scriptID, script] :
         ScriptRegistry<UnitScript>::ScriptPointerList) {
        auto const& dmg =
            script->DealDamage(AttackerUnit, pVictim, damage, damagetype);
        if (dmg != damage) {
            return damage;
        }
    }
...
void ScriptMgr::OnBeforeRollMeleeOutcomeAgainst(
    Unit const*      attacker,
    Unit const*      victim,
    ...
    int32&           block_chance)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnBeforeRollMeleeOutcomeAgainst(attacker,
                                                victim,
                                                attType,
...
{
    CALL_ENABLED_HOOKS(
        UnitScript, UNITHOOK_ON_AURA_APPLY, script->OnAuraApply(unit, aura));
}
                                 ...
                                 ReputationRank& repRank)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->IfNormalReaction(unit, target, repRank));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(UnitScript,
                               UNITHOOK_IS_NEEDMOD_SPELL_DAMAGE_PERCENT,
                               !script->IsNeedModSpellDamagePercent(
                                   unit, auraEff, doneTotalMod, spellProto));
}
                                            ...
                                            SpellInfo const* spellProto)
{
    CALL_ENABLED_BOOLEAN_HOOKS(UnitScript,
                               ...
                               !script->IsNeedModMeleeDamagePercent(
                                   unit, auraEff, doneTotalMod, spellProto));
}
                                     ...
                                     SpellInfo const* spellProto)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->IsNeedModHealPercent(unit, auraEff, doneTotalMod, spellProto));
}
                                ...
                                bool        update)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanSetPhaseMask(unit, newPhaseMask, update));
}
                                          ...
                                          uint16        index)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        ...
        script->IsCustomBuildValuesUpdate(
            unit, updateType, fieldBuffer, target, index));
}
                                                  ...
                                                  uint16      index)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        ...
        script->ShouldTrackValuesUpdatePosByIndex(unit, updateType, index));
}
                                    ...
                                    Player*                      target)
{
    CALL_ENABLED_HOOKS(UnitScript,
                       ...
                       script->OnPatchValuesUpdate(
                           unit, valuesUpdateBuf, posPointers, target));
}

void ScriptMgr::OnUnitUpdate(Unit* unit, uint32 diff)
{
    CALL_ENABLED_HOOKS(
        UnitScript, UNITHOOK_ON_UNIT_UPDATE, script->OnUnitUpdate(unit, diff));
}
...
{
    CALL_ENABLED_HOOKS(
        UnitScript, UNITHOOK_ON_UNIT_DEATH, script->OnUnitDeath(unit, killer));
}
