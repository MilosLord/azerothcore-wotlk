...
void ScriptMgr::OnBeforePlayerDurabilityRepair(Player*    player,
                                               ...
                                               uint8      guildBank)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       PLAYERHOOK_ON_BEFORE_DURABILITY_REPAIR,
                       script->OnBeforeDurabilityRepair(
                           player, npcGUID, itemGUID, discountMod, guildBank));
}
...
void ScriptMgr::OnBattlegroundDesertion(
    Player* player, BattlegroundDesertionType const desertionType)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                                     ...
                                     SpellInfo const* bySpell)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnCanPlayerFlyInZone(player, mapId, zoneId, bySpell));
}
                                             ...
                                             bool           increased)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnReputationRankChange(
                           player, factionID, newRank, oldRank, increased));
}
...
void ScriptMgr::OnPlayerChat(
    Player* player, uint32 type, uint32 lang, std::string& msg, Group* group)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_EMOTE, script->OnEmote(player, emote));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_LOGIN, script->OnLogin(player));
}

void ScriptMgr::OnPlayerLoadFromDB(Player* player)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_LOAD_FROM_DB, script->OnLoadFromDB(player));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_LOGOUT, script->OnLogout(player));
}

void ScriptMgr::OnPlayerCreate(Player* player)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_CREATE, script->OnCreate(player));
}

void ScriptMgr::OnPlayerSave(Player* player)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_SAVE, script->OnSave(player));
}

void ScriptMgr::OnPlayerDelete(ObjectGuid guid, uint32 accountId)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_DELETE, script->OnDelete(guid, accountId));
}
                                       ...
                                       Unit*   target)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnBeforeTeleport(
            player, mapid, x, y, z, orientation, options, target));
}
                                            ...
                                            AchievementEntry const* achievement)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnBeforeAchiComplete(player, achievement));
}
...
bool ScriptMgr::OnBeforeCriteriaProgress(
    Player* player, AchievementCriteriaEntry const* criteria)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript,
        PLAYERHOOK_ON_BEFORE_CRITERIA_PROGRESS,
        !script->OnBeforeCriteriaProgress(player, criteria));
}
...
void ScriptMgr::OnAfterPlayerMoveItemFromInventory(
...
void ScriptMgr::OnGetMaxPersonalArenaRatingRequirement(
    Player const* player, uint32 minSlot, uint32& maxArenaRating) const
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       PLAYERHOOK_ON_GET_MAX_PERSONAL_ARENA_RATING_REQUIREMENT,
                       script->OnGetMaxPersonalArenaRatingRequirement(
                           player, minSlot, maxArenaRating));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               PLAYERHOOK_CAN_PLACE_AUCTION_BID,
                               !script->CanPlaceAuctionBid(player, auction));
}

void ScriptMgr::OnGroupRollRewardItem(
    Player* player, Item* item, uint32 count, RollVote voteType, Roll* roll)
{
    CALL_ENABLED_HOOKS(
        PlayerScript,
...
bool ScriptMgr::OnBeforeOpenItem(Player* player, Item* item)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->OnBeforeOpenItem(player, item));
}

void ScriptMgr::OnFirstLogin(Player* player)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_FIRST_LOGIN, script->OnFirstLogin(player));
}
                                           ...
                                           GroupJoinBattlegroundResult& err)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanJoinInBattlegroundQueue(
            player, BattlemasterGuid, BGTypeID, joinAsGroup, err));
}
...
bool ScriptMgr::ShouldBeRewardedWithMoneyInsteadOfExp(Player* player)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        ...
        script->ShouldBeRewardedWithMoneyInsteadOfExp(player));
}
                                                  ...
                                                  PetType& petType)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnBeforeGuardianInitStatsForLevel(
                           player, guardian, cinfo, petType));
}
                                      ...
                                      bool&   forceLoadFromDB)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnBeforeLoadPetFromDB(
            player, petentry, petnumber, current, forceLoadFromDB));
}
                                          ...
                                          uint8      slot)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnBeforeBuyItemFromVendor(
            player, vendorguid, vendorslot, item, count, bag, slot));
}
                                           ...
                                           bool                bStore)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnAfterStoreOrEquipNewItem(player,
                                                          vendorslot,
                                                          item,
                                                          count,
                                                          bag,
                                                   ...
                                                   bool    ranged)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnBeforeUpdateAttackPowerAndDamage(
                           player, level, val2, ranged));
}
                                                  ...
                                                  bool    ranged)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnAfterUpdateAttackPowerAndDamage(player,
                                                  level,
                                                  base_attPower,
                                                  attPowerMod,
                                                  attPowerMultiplier,
                                           ...
                                           uint32& talentPointsForLevel)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnBeforeInitTalentForLevel(
                           player, level, talentPointsForLevel));
}
...
bool ScriptMgr::OnBeforePlayerQuestComplete(Player* player, uint32 quest_id)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnBeforeQuestComplete(player, quest_id));
}
                                            ...
                                            bool                bStore)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnBeforeStoreOrEquipNewItem(player,
                                                           vendorslot,
                                                           item,
                                                           count,
                                                           bag,
                                    ...
                                    GroupJoinBattlegroundResult& err)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanJoinInArenaQueue(player,
                                                            BattlemasterGuid,
                                                            arenaslot,
                                                            BGTypeID,
                                                            joinAsGroup,
                                   ...
                                   uint8              action)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanBattleFieldPort(player, arenaType, BGTypeID, action));
}
...
bool ScriptMgr::CanGroupInvite(Player* player, std::string& membername)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanGroupInvite(player, membername));
}
...
bool ScriptMgr::CanGroupAccept(Player* player, Group* group)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanGroupAccept(player, group));
}
...
bool ScriptMgr::CanSellItem(Player* player, Item* item, Creature* creature)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanSellItem(player, item, creature));
}
                            ...
                            Item*        item)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanSendMail(
            player, receiverGuid, mailbox, subject, body, money, COD, item));
}
...
bool ScriptMgr::CanSendErrorAlreadyLooted(Player* player)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanSendErrorAlreadyLooted(player));
}
                                 ...
                                 uint32&   CharterCost)
{
    CALL_ENABLED_HOOKS(
        ...
        script->PetitionShowList(
            player, creature, CharterEntry, CharterDispayID, CharterCost));
}
...
bool ScriptMgr::CanGiveMailRewardAtGiveLevel(Player* player, uint8 level)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanGiveMailRewardAtGiveLevel(player, level));
}
...
bool ScriptMgr::CanRepopAtGraveyard(Player* player)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanRepopAtGraveyard(player));
}
...
{
    if (ScriptRegistry<
            PlayerScript>::EnabledHooks[PLAYERHOOK_ON_PLAYER_IS_CLASS]
            .empty())
        return {};

    for (auto const& script : ScriptRegistry<
             PlayerScript>::EnabledHooks[PLAYERHOOK_ON_PLAYER_IS_CLASS]) {
        Optional<bool> scriptResult =
            script->OnPlayerIsClass(player, unitClass, context);
        if (scriptResult)
            return scriptResult;
    }
...
bool ScriptMgr::OnPlayerHasActivePowerType(Player const* player, Powers power)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        ...
        script->OnPlayerHasActivePowerType(player, power));
}
                                       ...
                                       uint32& gain)
{
    CALL_ENABLED_HOOKS(
        ...
        script->OnUpdateGatheringSkill(
            player, skillId, currentLevel, gray, green, yellow, gain));
}
...
bool ScriptMgr::OnUpdateFishingSkill(
    Player* player, int32 skill, int32 zone_skill, int32 chance, int32 roll)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript,
        PLAYERHOOK_ON_UPDATE_FISHING_SKILL,
        !script->OnUpdateFishingSkill(player, skill, zone_skill, chance, roll));
}
...
bool ScriptMgr::CanAreaExploreAndOutdoor(Player* player)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanAreaExploreAndOutdoor(player));
}
                                     ...
                                     uint32& victim_title)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnVictimRewardBefore(
                           player, victim, killer_title, victim_title));
}
                                    ...
                                    float&  honor_f)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnVictimRewardAfter(
                           player, victim, killer_title, victim_rank, honor_f));
}
                                         ...
                                         ScalingStatValuesEntry const* ssv)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnCustomScalingStatValue(player,
                                                        proto,
                                                        statType,
                                                        val,
                                                        itemProtoStatNumber,
...
bool ScriptMgr::CanArmorDamageModifier(Player* player)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanArmorDamageModifier(player));
}
...
bool ScriptMgr::CanApplyWeaponDependentAuraDamageMod(
    Player*           player,
    Item*             item,
    ...
    bool              apply)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanApplyWeaponDependentAuraDamageMod(
            player, item, attackType, aura, apply));
}
                                   ...
                                   bool             form_change)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanApplyEquipSpell(
            player, spellInfo, item, apply, form_change));
}
...
bool ScriptMgr::CanApplyEquipSpellsItemSet(Player* player, ItemSetEffect* eff)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanApplyEquipSpellsItemSet(player, eff));
}
                                       ...
                                       ItemTemplate const* proto)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanCastItemCombatSpell(
            player, target, attType, procVictim, procEx, item, proto));
}
                                    ...
                                    uint32                  glyphIndex)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanCastItemUseSpell(
            player, item, targets, cast_count, glyphIndex));
}
                             ...
                             bool    not_loading)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanEquipItem(player, slot, dest, pItem, swap, not_loading));
}
...
bool ScriptMgr::CanUnequipItem(Player* player, uint16 pos, bool swap)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanUnequipItem(player, pos, swap));
}
                           ...
                           InventoryResult&    result)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanUseItem(player, proto, result));
}
                                    ...
                                    bool    update)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanSaveEquipNewItem(player, item, pos, update));
}
                                    ...
                                    bool            ignore_condition)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanApplyEnchantment(
            player, item, slot, apply, apply_dur, ignore_condition));
}
                                               ...
                                               ObjectGuid   guid)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->PassedQuestKilledMonsterCredit(
                                   player, qinfo, entry, real_entry, guid));
}

bool ScriptMgr::CheckItemInSlotAtLoadInventory(
    Player* player, Item* item, uint8 slot, uint8& err, uint16& dest)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript,
        PLAYERHOOK_CHECK_ITEM_IN_SLOT_AT_LOAD_INVENTORY,
        !script->CheckItemInSlotAtLoadInventory(player, item, slot, err, dest));
}
                                ...
                                bool   report)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->NotAvoidSatisfy(player, ar, target_map, report));
}
...
bool ScriptMgr::NotVisibleGloballyFor(Player* player, Player const* u)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->NotVisibleGloballyFor(player, u));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_IS_PVP, script->OnIsPvP(player, result));
}
                                         ...
                                         uint32            value)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->NotSetArenaTeamInfoField(player, slot, type, value));
}
                           ...
                           const std::string&  comment)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanJoinLfg(player, roles, dungeons, comment));
}
                            ...
                            bool                    loginCheck)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanEnterMap(player, entry, instance, mapDiff, loginCheck));
}
...
bool ScriptMgr::CanInitTrade(Player* player, Player* target)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
                               ...
                               !script->CanInitTrade(player, target));
}
                                ...
                                uint8   tradeSlot)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanSetTradeItem(player, tradedItem, tradeSlot));
}
                                        ...
                                        uint32& graveyardOverride)
{
    CALL_ENABLED_HOOKS(PlayerScript,
                       ...
                       script->OnBeforeChooseGraveyard(
                           player, teamId, nearCorpse, graveyardOverride));
}
                                 ...
                                 std::string& msg)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanPlayerUseChat(player, type, language, msg));
}
                                 ...
                                 Player*      receiver)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanPlayerUseChat(player, type, language, msg, receiver));
}
                                 ...
                                 Group*       group)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanPlayerUseChat(player, type, language, msg, group));
}
                                 ...
                                 Guild*       guild)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanPlayerUseChat(player, type, language, msg, guild));
}
                                 ...
                                 Channel*     channel)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanPlayerUseChat(player, type, language, msg, channel));
}
