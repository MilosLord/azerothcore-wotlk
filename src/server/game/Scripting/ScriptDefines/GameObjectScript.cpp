...
bool ScriptMgr::OnGossipHello(Player* player, GameObject* go)
{
    ...
    auto tempScript =
        ScriptRegistry<GameObjectScript>::GetScriptById(go->GetScriptId());
                               ...
                               uint32      action)
{
    ...
    auto ret = IsValidBoolScript<AllGameObjectScript>(
        [&](AllGameObjectScript* script) {
            return script->CanGameObjectGossipSelect(
                player, go, sender, action);
        });
    ...
    auto tempScript =
        ScriptRegistry<GameObjectScript>::GetScriptById(go->GetScriptId());
    return tempScript ? tempScript->OnGossipSelect(player, go, sender, action)
                      : false;
}
                                   ...
                                   const char* code)
{
    ...
    auto ret = IsValidBoolScript<AllGameObjectScript>(
        [&](AllGameObjectScript* script) {
            return script->CanGameObjectGossipSelectCode(
                              ...
                              uint32       opt)
{
    ...
    auto tempScript =
        ScriptRegistry<GameObjectScript>::GetScriptById(go->GetScriptId());
    ClearGossipMenuFor(player);
    return tempScript ? tempScript->OnQuestReward(player, go, quest, opt)
                      : false;
}
...
uint32 ScriptMgr::GetDialogStatus(Player* player, GameObject* go)
{
    ...
    auto tempScript =
        ScriptRegistry<GameObjectScript>::GetScriptById(go->GetScriptId());
    return tempScript ? tempScript->GetDialogStatus(player, go)
...
void ScriptMgr::OnGameObjectDestroyed(GameObject* go, Player* player)
{
    ...
    if (auto tempScript = ScriptRegistry<GameObjectScript>::GetScriptById(
            go->GetScriptId())) {
                                         ...
                                         SpellInfo const* spellInfo)
{
    ...
    ExecuteScript<AllGameObjectScript>([&](AllGameObjectScript* script) {
        script->OnGameObjectModifyHealth(
            go, attackerOrHealer, change, spellInfo);
    });
