...
{
    CALL_ENABLED_BOOLEAN_HOOKS(PetScript,
                               PETHOOK_CAN_UNLEARN_SPELL_SET,
                               !script->CanUnlearnSpellSet(pet, level, spell));
}
...
bool ScriptMgr::CanUnlearnSpellDefault(Pet* pet, SpellInfo const* spellInfo)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PetScript,
                               ...
                               !script->CanUnlearnSpellDefault(pet, spellInfo));
}

bool ScriptMgr::CanResetTalents(Pet* pet)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PetScript, PETHOOK_CAN_RESET_TALENTS, !script->CanResetTalents(pet));
}
...
void ScriptMgr::OnPetAddToWorld(Pet* pet)
{
    ...
    CALL_ENABLED_HOOKS(
        PetScript, PETHOOK_ON_PET_ADD_TO_WORLD, script->OnPetAddToWorld(pet));
}
