...
{
    CALL_ENABLED_HOOKS(AllBattlegroundScript,
                                   ...
                                   BattlegroundBracketId bracket_id)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanFillPlayersToBG(queue, bg, bracket_id));
}
                                   ...
                                   uint32                maxPlayers)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        ...
        script->IsCheckNormalMatch(
            queue, bgTemplate, bracket_id, minPlayers, maxPlayers));
}
                                      ...
                                      PvPDifficultyEntry const* bracketEntry)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->CanSendMessageBGQueue(queue, leader, bg, bracketEntry));
}

bool ScriptMgr::OnBeforeSendJoinMessageArenaQueue(
    ...
    bool                      isRated)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnBeforeSendJoinMessageArenaQueue(
            queue, leader, ginfo, bracketEntry, isRated));
}
                                                  ...
                                                  GroupQueueInfo*    ginfo)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        ...
        !script->OnBeforeSendExitMessageArenaQueue(queue, ginfo));
}
