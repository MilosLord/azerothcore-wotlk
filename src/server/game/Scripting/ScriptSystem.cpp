...
void SystemMgr::LoadScriptWaypoints()
{
    ...
    LOG_INFO("server.loading",
             "Loading Script Waypoints For {} Creature(s)...",
             uiCreatureCount);
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 Script Waypoints. DB table `script_waypoint` is "
                 "empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        CreatureTemplate const* pCInfo =
            sObjectMgr->GetCreatureTemplate(temp.uiCreatureEntry);

        if (!pCInfo) {
            LOG_ERROR("sql.sql",
                      "DB table script_waypoint has waypoint for non-existant "
                      "creature entry {}",
        ...
        if (!pCInfo->ScriptID)
            LOG_ERROR("sql.sql",
                      ...
                      "entry {}, but creature does not have ScriptName defined "
                      "and then useless.",
                      temp.uiCreatureEntry);
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Script Waypoint nodes in {} ms",
             count,
