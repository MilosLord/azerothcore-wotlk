...
inline float hk_honor_at_level_f(uint8 level, float multiplier = 1.0f)
{
    ...
    // sScriptMgr->OnHonorCalculation(honor, level, multiplier); // pussywizard:
    // optimization
    return honor;
}
...
inline uint8 GetGrayLevel(uint8 pl_level)
{
    ...
    // sScriptMgr->OnGrayLevelCalculation(level, pl_level); // pussywizard:
    // optimization
    return level;
}
...
inline uint8 GetZeroDifference(uint8 pl_level)
{
    ...
    // sScriptMgr->OnZeroDifferenceCalculation(diff, pl_level); // pussywizard:
    // optimization
    return diff;
}
...
inline float xp_in_group_rate(uint32 count, bool isRaid)
{
    ...
    // sScriptMgr->OnGroupRateCalculation(rate, count, isRaid); // pussywizard:
    // optimization
    return rate;
}
} // namespace Acore::XP
