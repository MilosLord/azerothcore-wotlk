...
uint32
Acore::XP::BaseGain(uint8 pl_level, uint8 mob_level, ContentLevels content)
{
    uint32 baseGain;
    uint32 nBaseExp;
    ...
    if (mob_level >= pl_level) {
        ...
        if (mob_level > gray_level) {
            ...
            baseGain =
                (pl_level * 5 + nBaseExp) * (ZD + mob_level - pl_level) / ZD;
        }
        else
            baseGain = 0;
    }
    ...
    // content); // pussywizard: optimization
    return baseGain;
}

uint32
Acore::XP::Gain(Player* player, Unit* unit, bool isBattleGround /*= false*/)
{
    Creature* creature = unit->ToCreature();
    uint32    gain     = 0;

    if (!creature ||
        (!creature->IsTotem() && !creature->IsPet() && !creature->IsCritter() &&
         !(creature->GetCreatureTemplate()->flags_extra &
        ...
        if (gain && creature) {
            if (creature->isElite()) {
                // Elites in instances have a 2.75x XP bonus instead of the
                // regular 2x world bonus.
        ...
        // if players dealt less than 50% of the damage and were credited anyway
        // (due to CREATURE_FLAG_EXTRA_NO_PLAYER_DAMAGE_REQ), scale XP gained
        // appropriately (linear scaling)
        if (creature && creature->GetPlayerDamageReq()) {
            xpMod *= 1.0f - 2.0f * creature->GetPlayerDamageReq() /
    ...
    // sScriptMgr->OnGainCalculation(gain, player, u); // pussywizard:
    // optimization
    return gain;
}
