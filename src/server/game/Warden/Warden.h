...
enum WardenOpcodes {
    ...
    WARDEN_SMSG_MEM_CHECKS_REQUEST =
        4, // byte len; while(!EOF) { byte unk(1); byte index(++); string
           // module(can be 0); int offset; byte len; byte[]
           // bytes_to_compare[len]; }
    WARDEN_SMSG_HASH_REQUEST = 5
};

enum WardenCheckType {
    MEM_CHECK = 0xF3,    // 243: byte moduleNameIndex + uint Offset + byte Len
                         // (check to ensure memory isn't modified)
    PAGE_CHECK_A = 0xB2, // 178: uint Seed + byte[20] SHA1 + uint Addr + byte
                         // Len (scans all pages for specified hash)
    PAGE_CHECK_B =
        0xBF, // 191: uint Seed + byte[20] SHA1 + uint Addr + byte Len (scans
              // only pages starts with MZ+PE headers for specified hash)
    MPQ_CHECK = 0x98, // 152: byte fileNameIndex (check to ensure MPQ file isn't
                      // modified)
    LUA_EVAL_CHECK = 139, // evaluate arbitrary Lua check
    DRIVER_CHECK =
        0x71, // 113: uint Seed + byte[20] SHA1 + byte driverNameIndex (check to
              // ensure driver isn't loaded)
    TIMING_CHECK =
        0x57, //  87: empty (check to ensure GetTickCount() isn't detoured)
    PROC_CHECK = 0x7E, // 126: uint Seed + byte[20] SHA1 + byte moluleNameIndex
                       // + byte procNameIndex + uint Offset + byte Len (check
                       // to ensure proc isn't detoured)
    MODULE_CHECK = 0xD9, // 217: uint Seed + byte[20] SHA1 (check to ensure
...
public:
    ...
    static bool
    IsValidCheckSum(uint32 checksum, const uint8* data, const uint16 length);
