...
// GUILD is the shortest string that has no client validation (RAID only sends
// if in a raid group)
...
static_assert((sizeof(_luaEvalPrefix) - 1 + sizeof(_luaEvalMidfix) - 1 +
               sizeof(_luaEvalPostfix) - 1 + WARDEN_MAX_LUA_CHECK_LENGTH) ==
              255);
...
static constexpr uint8 GetCheckPacketBaseSize(uint8 type)
{
    ...
    case MPQ_CHECK:
        return 1;
    case LUA_EVAL_CHECK:
        return 1 + sizeof(_luaEvalPrefix) - 1 + sizeof(_luaEvalMidfix) - 1 + 4 +
               sizeof(_luaEvalPostfix) - 1;
    case PAGE_CHECK_A:
        return (4 + 1);
    case PAGE_CHECK_B:
        return (4 + 1);
    case MODULE_CHECK:
        return (4 + Acore::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES);
    case MEM_CHECK:
        return (1 + 4 + 1);
    default:
        return 0;
...
static uint16 GetCheckPacketSize(WardenCheck const* check)
{
    ...
    if (!check->Str.empty()) {
        size += (static_cast<uint16>(check->Str.length()) +
                 1); // 1 byte string length
    }
...
void WardenWin::Init(WorldSession* session, SessionKey const& k)
{
    ...
    LOG_DEBUG("warden",
              "Server side warden for client {} initializing...",
              session->GetAccountId());
    LOG_DEBUG("warden",
              "C->S Key: {}",
              Acore::Impl::ByteArrayToHexStr(_inputKey, 16));
    LOG_DEBUG("warden",
              "S->C Key: {}",
              Acore::Impl::ByteArrayToHexStr(_outputKey, 16));
    LOG_DEBUG(
        "warden", "  Seed: {}", Acore::Impl::ByteArrayToHexStr(_seed, 16));
...
ClientWardenModule* WardenWin::GetModuleForClient()
{
    ...
    // md5 hash
    mod->Id = Acore::Crypto::MD5::GetDigestOf(mod->CompressedData,
                                              mod->CompressedSize);
...
void WardenWin::InitializeModule()
{
    ...
    Request.CheckSumm1   = BuildChecksum(
        &Request.Unk1, Acore::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES);
    ...
    Request.Function2 =
        0x00419210; // 0x00400000 + 0x00419210 FrameScript::Execute
    ...
    pkt.append(reinterpret_cast<uint8*>(&Request),
               sizeof(WardenInitModuleRequest));
...
void WardenWin::HandleHashResult(ByteBuffer& buff)
{
    ...
    // Verify key
    if (memcmp(buff.contents() + 1,
               Module.ClientKeySeedHash,
 ...
 */
bool WardenWin::IsCheckInProgress() { return _checkInProgress; }

/**
 * @brief Force call RequestChecks() so they are sent immediately, this
 * interrupts warden and breaks result.
 */
...
void WardenWin::RequestChecks()
{
    ...
    // Erase any nullptrs.
    Acore::Containers::EraseIf(_PendingChecks, [this](uint16 id) {
        ...
        if (!check && id >= WardenPayloadMgr::WardenPayloadOffsetMin) {
            if (_payloadMgr.CachedChecks.find(id) !=
                _payloadMgr.CachedChecks.end()) {
    ...
    if (_PendingChecks.empty()) {
        for (uint8 checkType = 0; checkType < MAX_WARDEN_CHECK_TYPES;
             ++checkType) {
            for (uint32 y = 0;
                 y < sWorld->getIntConfig(GetMaxWardenChecksForType(checkType));
                 ++y) {
                // If todo list is done break loop (will be filled on next
                // Update() run)
                ...
                if (checkType == WARDEN_CHECK_LUA_TYPE &&
                    !_payloadMgr.QueuedPayloads.empty()) {
                    uint16 payloadId = _payloadMgr.QueuedPayloads.front();

                    LOG_DEBUG(
                        "warden",
                        "Adding custom warden payload '{}' to CurrentChecks.",
                        payloadId);
        ...
        for (uint16 const checkId : _PendingChecks) {
            WardenCheck const* check =
                sWardenCheckMgr->GetWardenDataById(checkId);
        ...
        if (!hasLuaChecks) {
            for (uint32 i = 0;
                 i < sWorld->getIntConfig(
                         GetMaxWardenChecksForType(WARDEN_CHECK_LUA_TYPE));
                 ++i) {
                // If todo list is done break loop (will be filled on next
                // Update() run)
    ...
    Acore::Containers::EraseIf(
        _CurrentChecks, [this, &expectedSize](uint16 id) {
            WardenCheck const* check = sWardenCheckMgr->GetWardenDataById(id);
            ...
            if ((expectedSize + thisSize) >
                500) // warden packets are truncated to 512 bytes clientside
            {
    ...
    for (uint16 const checkId : _CurrentChecks) {
        ...
        case MODULE_CHECK: {
            ...
            buff.append(seed);
            buff.append(
                Acore::Crypto::HMAC_SHA1::GetDigestOf(seed, check->Str));
            break;
        }
            /*case PROC_CHECK:
            {
                buff.append(wd->i.AsByteArray(0, false).get(),
            wd->i.GetNumBytes()); buff << uint8(index++); buff <<
            uint8(index++); buff << uint32(wd->Address); buff <<
            uint8(wd->Length); break;
            }*/
        }
    }
    buff << uint8(xorByte);
...
void WardenWin::HandleData(ByteBuffer& buff)
{
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00) {
            ...
            // ApplyPenalty(0, "TIMING CHECK FAIL result"); Commented out
            // because of too many false postives. Mostly caused by client
            // stutter.
            return;
        }
    ...
    for (uint16 const checkId : _CurrentChecks) {
        ...
        case MEM_CHECK: {
            ...
            if (Mem_Result != 0) {
                LOG_DEBUG("warden",
                          "RESULT MEM_CHECK not 0x00, CheckId {} account Id {}",
                          checkId,
            ...
            WardenCheckResult const* rs =
                sWardenCheckMgr->GetWardenResultById(checkId);
            ...
            if (memcmp(buff.contents() + buff.rpos(),
                       ...
                       rd->Length) != 0) {
                LOG_DEBUG("warden",
                          "RESULT MEM_CHECK fail CheckId {} account Id {}",
                          checkId,
                          _session->GetAccountId());
                checkFailed = checkId;
            ...
            buff.rpos(buff.rpos() + rd->Length);
            LOG_DEBUG("warden",
                      "RESULT MEM_CHECK passed CheckId {} account Id {}",
                      checkId,
                      _session->GetAccountId());
            break;
        }
        ...
        case MODULE_CHECK: {
            ...
            if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) !=
                ...
                if (type == PAGE_CHECK_A || type == PAGE_CHECK_B) {
                    LOG_DEBUG(
                        "warden",
                        "RESULT PAGE_CHECK fail, CheckId {} account Id {}",
                        checkId,
                        _session->GetAccountId());
                }
                ...
                if (type == MODULE_CHECK) {
                    LOG_DEBUG(
                        "warden",
                        "RESULT MODULE_CHECK fail, CheckId {} account Id {}",
                        checkId,
                        _session->GetAccountId());
                }
                ...
                if (type == DRIVER_CHECK) {
                    LOG_DEBUG(
                        "warden",
                        "RESULT DRIVER_CHECK fail, CheckId {} account Id {}",
                        checkId,
                        _session->GetAccountId());
                }
            ...
            if (type == PAGE_CHECK_A || type == PAGE_CHECK_B) {
                LOG_DEBUG("warden",
                          "RESULT PAGE_CHECK passed CheckId {} account Id {}",
                          checkId,
                          _session->GetAccountId());
            }
            else if (type == MODULE_CHECK) {
                LOG_DEBUG("warden",
                          "RESULT MODULE_CHECK passed CheckId {} account Id {}",
                          checkId,
                          _session->GetAccountId());
            }
            else if (type == DRIVER_CHECK) {
                LOG_DEBUG("warden",
                          "RESULT DRIVER_CHECK passed CheckId {} account Id {}",
                          checkId,
                          _session->GetAccountId());
            }
            break;
        ...
        case LUA_EVAL_CHECK: {
            ...
            LOG_DEBUG("warden",
                      "LUA_EVAL_CHECK CheckId {} account Id {} got in-warden "
                      "dummy response",
                      checkId,
                      _session->GetAccountId() /* , result */);
            break;
        }
        case MPQ_CHECK: {
            uint8 Mpq_Result;
            ...
            if (Mpq_Result != 0) {
                LOG_DEBUG("warden",
                          "RESULT MPQ_CHECK not 0x00 account id {}",
                          _session->GetAccountId());
                checkFailed = checkId;
                continue;
            }
            ...
            {
                LOG_DEBUG("warden",
                          "RESULT MPQ_CHECK fail, CheckId {} account Id {}",
                          checkId,
                          _session->GetAccountId());
                checkFailed = checkId;
                buff.rpos(buff.rpos() +
                          Acore::Crypto::Constants::
                              SHA1_DIGEST_LENGTH_BYTES); // 20 bytes SHA1
                continue;
            }
            ...
            buff.rpos(
                ...
                Acore::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES); // 20 bytes
                                                                     // SHA1
            LOG_DEBUG("warden",
                      "RESULT MPQ_CHECK passed, CheckId {} account Id {}",
                      checkId,
                      _session->GetAccountId());
            break;
        }
    ...
    uint32 const holdOff =
        sWorld->getIntConfig(CONFIG_WARDEN_CLIENT_CHECK_HOLDOFF);
