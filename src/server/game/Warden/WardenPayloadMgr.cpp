...
uint16 WardenPayloadMgr::GetFreePayloadId()
{
    ...
    while (CachedChecks.find(payloadId) != CachedChecks.end()) {
        ...
        if (payloadId > WardenPayloadMgr::WardenPayloadOffsetMax) {
            LOG_ERROR("warden",
                      "Max warden payload id of '{}' passed!",
                      WardenPayloadMgr::WardenPayloadOffsetMax);
            return 0;
        }
    }
                                       ...
                                       bool               replace)
{
    ...
    if (payloadId < WardenPayloadMgr::WardenPayloadOffsetMin) {
        LOG_ERROR("warden",
                  "Tried to register payloadId lower than '{}'.",
                  WardenPayloadMgr::WardenPayloadOffsetMin);
        return false;
    }
    ...
    if (it != CachedChecks.end() && !replace) {
        LOG_ERROR("warden",
                  "Payload Id '{}' already exists in CachedChecks.",
                  payloadId);
        return false;
    }
...
void WardenPayloadMgr::QueuePayload(uint16 payloadId, bool pushToFront)
{
    ...
    if (it == CachedChecks.end()) {
        LOG_ERROR("warden",
                  "Failed to queue payload id '{}' as it does not exist in "
                  "CachedChecks.",
                  payloadId);
        return;
    }
...
void WardenPayloadMgr::ClearQueuedPayloads() { QueuedPayloads.clear(); }
