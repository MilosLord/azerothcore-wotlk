 ...
 * @brief The WardenPayloadMgr is responsible for maintaining custom payloads
 * used by modules.
 * @details This allows users to send custom lua payloads up to a size of 512
 * bytes to the game client. Some of the things you can achieve with this is:
...
public:
     ...
     * @param payload The payload to be stored in
     * WardenPayloadMgr::CachedChecks.
     * @return uint16 The payload id for use with
     * WardenPayloadMgr::QueuePayload. Returns 0 if it failed to register.
     * @note
     * - Payloads are truncated to 512 bytes on the client, you may have to
     * register your payloads in chunks if they are larger than this.
     */
    uint16 RegisterPayload(const std::string& payload);

    /**
     * @brief Register a payload into cache with a custom id and returns the
     * result.
     * @param payload The payload to be stored in
     * WardenPayloadMgr::CachedChecks.
     * @param payloadId The payload id to be stored as the key in
     * WardenPayloadMgr::CachedChecks.
     * @param replace Whether the key should replace an existing entry value.
     * @return bool The payload insertion result. If exists it will return
     * false, otherwise true.
     * @note
     * - Payloads are truncated to 512 bytes on the client, you may have to
     * register your payloads in chunks if they are larger than this.
     * - It's a good idea to keep the value for payloadId between 9000-9999 for
     * self defined payloads as they're the least likely occupied ids.
     */
    bool RegisterPayload(std::string const& payload,
                         uint16             payloadId,
                         bool               replace = false);

    /**
     * @brief Unregister a payload from cache and return if successful.
     * @param payloadId The payload to removed from
     * WardenPayloadMgr::CachedChecks.
     ...
     * @param payloadId The payload to fetched from
     * WardenPayloadMgr::CachedChecks.
    ...
    /**
     * @brief Dequeue the payload from the WardenPayloadMgr::QueuedPayloads
     * queue.
    ...
    /**
     * @brief Clear the payloads from the WardenPayloadMgr::QueuedPayloads
     * queue.
     */
    void ClearQueuedPayloads();

    /**
     * @brief Get the amount of payloads waiting in
     * WardenPayloadMgr::QueuedPayloads.
    ...
    /**
     * @brief The list of currently queued payload ids to be sent through
     * Warden.
     */
