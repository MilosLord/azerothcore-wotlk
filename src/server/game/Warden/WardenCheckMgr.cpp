...
void WardenCheckMgr::LoadWardenChecks()
{
    ...
    QueryResult result =
        WorldDatabase.Query("SELECT MAX(id) FROM warden_checks");
    ...
    //                                    7
    result =
        WorldDatabase.Query("SELECT id, type, data, result, address, length, "
                            "str, comment FROM warden_checks ORDER BY id ASC");
    ...
    do {
        ...
        if (checkType == LUA_EVAL_CHECK && id > 9999) {
            LOG_ERROR("warden",
                      "sql.sql: Warden Lua check with id {} found in "
                      "`warden_checks`. Lua checks may have four-digit IDs at "
                      "most. Skipped.",
                      id);
            continue;
        }
        ...
        wardenCheck.Action =
            sWorld->getIntConfig(CONFIG_WARDEN_CLIENT_FAIL_ACTION);
        if (wardenCheck.Action > MAX_WARDEN_ACTION) {
            wardenCheck.Action = WARDEN_ACTION_BAN;
        }

        if (checkType == MEM_CHECK || checkType == PAGE_CHECK_A ||
            checkType == PAGE_CHECK_B || checkType == PROC_CHECK) {
        ...
        if (checkType == MEM_CHECK || checkType == MPQ_CHECK ||
            checkType == LUA_EVAL_CHECK || checkType == DRIVER_CHECK ||
            checkType == MODULE_CHECK) {
            wardenCheck.Str = str;
        }
        ...
        case LUA_EVAL_CHECK: {
            if (wardenCheck.Length > WARDEN_MAX_LUA_CHECK_LENGTH) {
                LOG_ERROR(
                    "warden",
                    "sql.sql: Found over-long Lua check for Warden check with "
                    "id {} in `warden_checks`. Max length is {}. Skipped.",
                    id,
        ...
        default: {
            if (checkType == PAGE_CHECK_A || checkType == PAGE_CHECK_B ||
                checkType == DRIVER_CHECK)
...
void WardenCheckMgr::LoadWardenOverrides()
{
    ...
    //                                                      0        1
    QueryResult result =
        CharacterDatabase.Query("SELECT wardenId, action FROM warden_action");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 Warden action overrides. DB table "
                 "`warden_action` is empty!");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (action > WARDEN_ACTION_BAN)
            LOG_ERROR("warden",
                      "Warden check override action out of range (ID: {}, "
                      "action: {})",
                      checkId,
                      action);
