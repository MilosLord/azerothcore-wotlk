...
BattlegroundMgr::BattlegroundMgr()
    : m_ArenaTesting(false), m_Testing(false), m_NextAutoDistributionTime(0),
...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    for (auto& [_, bgData] : bgDataStore) {
        ...
        for (BattlegroundContainer::iterator itr = ++itrDelete;
             itr != bgList.end();) {
            itrDelete        = itr++;
            ...
            if (bg->ToBeDeleted()) {
                ...
                BattlegroundClientIdsContainer& clients =
                    bgData._ClientBattlegroundIds[bg->GetBracketId()];
                if (!clients.empty())
    ...
    // update events
    for (uint8 qtype = BATTLEGROUND_QUEUE_NONE;
         qtype < MAX_BATTLEGROUND_QUEUE_TYPES;
         ++qtype)
        m_BattlegroundQueues[qtype].UpdateEvents(diff);

    // update using scheduled tasks (used only for rated arenas, initial
    // opponent search works differently than periodic queue update)
    if (!m_QueueUpdateScheduler.empty()) {
        ...
        for (uint8 i = 0; i < scheduled.size(); i++) {
            ...
            BattlegroundQueueTypeId bgQueueTypeId =
                BattlegroundQueueTypeId(scheduled[i] >> 16 & 255);
            BattlegroundTypeId bgTypeId =
                BattlegroundTypeId((scheduled[i] >> 8) & 255);
            BattlegroundBracketId bracket_id =
                BattlegroundBracketId(scheduled[i] & 255);
            m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(
                diff,
                bgTypeId,
                bracket_id,
                arenaType,
                arenaMMRating > 0,
                arenaMMRating);
            m_BattlegroundQueues[bgQueueTypeId]
                .BattlegroundQueueAnnouncerUpdate(
                    diff, bgQueueTypeId, bracket_id);
        }
    }
    ...
    if (m_NextPeriodicQueueUpdateTime < diff) {
        ...
        // for rated arenas
        for (uint32 qtype = BATTLEGROUND_QUEUE_2v2;
             qtype < MAX_BATTLEGROUND_QUEUE_TYPES;
             ++qtype) {
            for (uint32 bracket = BG_BRACKET_ID_FIRST;
                 bracket < MAX_BATTLEGROUND_BRACKETS;
                 ++bracket) {
                m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(
                    m_NextPeriodicQueueUpdateTime,
                    BATTLEGROUND_AA,
                    BattlegroundBracketId(bracket),
                    BattlegroundMgr::BGArenaType(
        ...
        for (uint32 qtype = BATTLEGROUND_QUEUE_AV;
             qtype < MAX_BATTLEGROUND_QUEUE_TYPES;
             ++qtype) {
            for (uint32 bracket = BG_BRACKET_ID_FIRST;
                 bracket < MAX_BATTLEGROUND_BRACKETS;
                 ++bracket) {
                m_BattlegroundQueues[qtype].BattlegroundQueueAnnouncerUpdate(
                    m_NextPeriodicQueueUpdateTime,
                    BattlegroundQueueTypeId(qtype),
                    BattlegroundBracketId(bracket));
            }
    ...
    if (sWorld->getBoolConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS)) {
        if (m_AutoDistributionTimeChecker < diff) {
            if (GameTime::GetGameTime() > m_NextAutoDistributionTime) {
                ...
                sWorld->setWorldState(WS_ARENA_DISTRIBUTION_TIME,
                                      m_NextAutoDistributionTime.count());
            }
...
void BattlegroundMgr::BuildBattlegroundStatusPacket(
    ...
    TeamId             teamId,
    bool               isRated,
    BattlegroundTypeId forceBgTypeId)
{
    // pussywizard:
    ...
    data->Initialize(SMSG_BATTLEFIELD_STATUS,
                     (4 + 8 + 1 + 1 + 4 + 1 + 4 + 4 + 4));
    *data << uint32(QueueSlot);
    // The following segment is read as uint64 in client but can be appended as
    // their original type.
    *data << uint8(arenatype);
    *data << uint8(bg->isArena() ? 0xE : 0x0);
    *data << uint32(forceBgTypeId != BATTLEGROUND_TYPE_NONE
                        ? forceBgTypeId
                        : bg->GetBgTypeID());
    *data << uint16(0x1F90);
    ...
    *data << uint8(bg->isRated() ||
                   isRated); // 1 for rated match, 0 for bg or non rated match
    ...
    *data << uint32(StatusID); // status
    ...
    case STATUS_WAIT_QUEUE:     // status_in_queue
        ...
        *data << uint32(
            Time2); // time in queue, updated every minute!, milliseconds
        break;
    ...
    case STATUS_IN_PROGRESS:             // status_in_progress
        *data << uint32(bg->GetMapId()); // map id
        *data << uint64(0);              // 3.3.5, unknown
        *data << uint32(Time1); // time to bg auto leave, 0 at bg start, 120000
                                // after bg end, milliseconds
        *data << uint32(Time2); // time from bg start, milliseconds
        *data << uint8(teamId == TEAM_ALLIANCE
                           ? 1
                           : 0); // arenafaction (0 for horde, 1 for alliance)
        break;
    default:
        break;
    }
...
void BattlegroundMgr::BuildGroupJoinedBattlegroundPacket(
    WorldPacket* data, GroupJoinBattlegroundResult result)
{
    data->Initialize(SMSG_GROUP_JOINED_BATTLEGROUND, 4);
    *data << int32(result);
    if (result == ERR_BATTLEGROUND_JOIN_TIMED_OUT ||
        result == ERR_BATTLEGROUND_JOIN_FAILED)
...
Battleground* BattlegroundMgr::GetBattlegroundThroughClientInstance(
    uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    // cause at HandleBattlegroundJoinOpcode the clients sends the instanceid he
    // gets from SMSG_BATTLEFIELD_LIST we need to find the battleground with
    // this clientinstance-id
                                               ...
                                               BattlegroundTypeId bgTypeId)
{
    ...
    auto GetBgWithInstanceID =
        [instanceId](BattlegroundData const* bgData) -> Battleground* {
...
BattlegroundMgr::GetBattlegroundTemplate(BattlegroundTypeId bgTypeId)
{
    ...
    // map is sorted and we can be sure that lowest instance id has only BG
    // template
                                               ...
                                               BattlegroundBracketId bracket_id)
{
    ...
    // the optimalization would be to use as bitmask std::vector<uint32> - but
    // that would only make code unreadable

    BattlegroundClientIdsContainer& clientIds =
        bgDataStore[bgTypeId]._ClientBattlegroundIds[bracket_id];
    uint32 lastId = 0;

    for (BattlegroundClientIdsContainer::const_iterator itr = clientIds.begin();
         itr != clientIds.end();) {
        if ((++lastId) !=
            *itr) // if there is a gap between the ids, we will break..
            break;
...
Battleground*
BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId,
                                       ...
                                       bool                      isRated)
{
    ...
    if (!bg_template) {
        LOG_ERROR("bg.battleground",
                  "Battleground: CreateNewBattleground - bg template not found "
                  "for {}",
                  bgTypeId);
        return nullptr;
    }
    ...
    if (BattlegroundMgr::bgTypeToTemplate.find(bgTypeId) ==
        BattlegroundMgr::bgTypeToTemplate.end()) {
        return nullptr;
    }
    ...
    bg->SetClientInstanceID(CreateClientVisibleInstanceId(
        originalBgTypeId, bracketEntry->GetBracketId()));
    bg->Init();
    ...
    if (bg->isArena()) {
        uint32 maxPlayersPerTeam =
            ArenaTeam::GetReqPlayersForType(arenaType) / 2;
...
bool BattlegroundMgr::CreateBattleground(BattlegroundTemplate const* bgTemplate)
{
    ...
    bg->SetName(
        bgTemplate->BattlemasterEntry->name[sWorld->GetDefaultDbcLocale()]);
    ...
    bg->SetTeamStartPosition(TEAM_ALLIANCE,
                             bgTemplate->StartLocation[TEAM_ALLIANCE]);
...
void BattlegroundMgr::LoadBattlegroundTemplates()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT ID, MinPlayersPerTeam, MaxPlayersPerTeam, MinLvl, MaxLvl, "
        "AllianceStartLoc, AllianceStartO, HordeStartLoc, HordeStartO, "
        "StartMaxDist, Weight, ScriptName FROM battleground_template");

    if (!result) {
        LOG_ERROR("bg.battleground",
                  ">> Loaded 0 battlegrounds. DB table `battleground_template` "
                  "is empty.");
        return;
    }
    ...
    do {
        ...
        BattlegroundTypeId bgTypeId =
            static_cast<BattlegroundTypeId>(fields[0].Get<uint32>());

        if (DisableMgr::IsDisabledFor(
                DISABLE_TYPE_BATTLEGROUND, bgTypeId, nullptr))
            continue;
        ...
        if (!bl) {
            LOG_ERROR("bg.battleground",
                      "Battleground ID {} not found in BattlemasterList.dbc. "
        ...
        bgTemplate.ScriptId =
            sObjectMgr->GetScriptId(fields[11].Get<std::string>());
        bgTemplate.BattlemasterEntry = bl;

        if (bgTemplate.MaxPlayersPerTeam == 0 ||
            bgTemplate.MinPlayersPerTeam > bgTemplate.MaxPlayersPerTeam) {
            LOG_ERROR(
                "sql.sql",
                "Table `battleground_template` for id {} contains bad values "
                "for MinPlayersPerTeam ({}) and MaxPlayersPerTeam({}).",
                bgTemplate.Id,
                bgTemplate.MinPlayersPerTeam,
                bgTemplate.MaxPlayersPerTeam);

            continue;
        }

        if (bgTemplate.MinLevel == 0 || bgTemplate.MaxLevel == 0 ||
            bgTemplate.MinLevel > bgTemplate.MaxLevel) {
            LOG_ERROR("sql.sql",
                      "Table `battleground_template` for id {} has bad values "
                      "for LevelMin ({}) and LevelMax({})",
                      bgTemplate.Id,
                      bgTemplate.MinLevel,
                      bgTemplate.MaxLevel);
            continue;
        ...
        if (bgTemplate.Id != BATTLEGROUND_AA &&
            ...
            if (GraveyardStruct const* start =
                    sGraveyard->GetGraveyard(startId)) {
                bgTemplate.StartLocation[TEAM_ALLIANCE].Relocate(
                    start->x, start->y, start->z, fields[6].Get<float>());
            }
            else {
                LOG_ERROR("sql.sql",
                          "Table `battleground_template` for id {} contains a "
                          "non-existing WorldSafeLocs.dbc id {} in field "
                          "`AllianceStartLoc`. BG not created.",
                          bgTemplate.Id,
                          startId);
                continue;
            }
            ...
            if (GraveyardStruct const* start =
                    sGraveyard->GetGraveyard(startId)) {
                bgTemplate.StartLocation[TEAM_HORDE].Relocate(
                    start->x, start->y, start->z, fields[8].Get<float>());
            }
            else {
                LOG_ERROR("sql.sql",
                          "Table `battleground_template` for id {} contains a "
                          "non-existing WorldSafeLocs.dbc id {} in field "
                          "`HordeStartLoc`. BG not created.",
                          bgTemplate.Id,
                          startId);
                continue;
            }
        ...
        if (bgTemplate.BattlemasterEntry->mapid[1] ==
            -1) // in this case we have only one mapId
            _battlegroundMapTemplates[bgTemplate.BattlemasterEntry->mapid[0]] =
                &_battlegroundTemplates[bgTypeId];

    } while (result->NextRow());

    LOG_INFO("server.loading",
             ">> Loaded {} battlegrounds in {} ms",
             _battlegroundTemplates.size(),
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    if (wstime < curtime) {
        m_NextAutoDistributionTime =
            curtime; // reset will be called in the next update
        LOG_INFO(
            "server.loading",
            "Next arena point distribution time in the past, reseting it now.");
    }
    else {
                                                  ...
                                                  uint8              fromWhere)
{
    ...
    winner_kills = Acore::Honor::hk_honor_at_level(player->GetLevel(),
                                                   float(winner_kills));
    loser_kills =
        Acore::Honor::hk_honor_at_level(player->GetLevel(), float(loser_kills));
        ...
        if (it != bgDataStore.end()) {
            ...
            if (PvPDifficultyEntry const* bracketEntry =
                    GetBattlegroundBracketByLevel(
                        it->second._Battlegrounds.begin()->second->GetMapId(),
                        player->GetLevel())) {
                uint32                count     = 0;
                BattlegroundBracketId bracketId = bracketEntry->GetBracketId();
                BattlegroundClientIdsContainer& clientIds =
                    it->second._ClientBattlegroundIds[bracketId];
                                         ...
                                         BattlegroundTypeId bgTypeId)
{
    if (Battleground* bg = GetBattleground(instanceId, bgTypeId)) {
        ...
        LOG_DEBUG("bg.battleground",
                  "BattlegroundMgr::SendToBattleground: Sending {} to map {}, "
                  "{} (bgType {})",
                  player->GetName(),
                  mapid,
                  pos->ToString(),
        ...
        LOG_ERROR("bg.battleground",
                  "BattlegroundMgr::SendToBattleground: Instance {} (bgType "
                  "{}) not found while trying to teleport player {}",
                  instanceId,
                  bgTypeId,
                  player->GetName());
    }
                                                      ...
                                                      ObjectGuid    guid)
{
    ...
    uint32      time_ = RESURRECTION_INTERVAL -
                   bg->GetLastResurrectTime(); // resurrect every X seconds
    if (time_ == uint32(-1))
        time_ = 0;
    data << guid << time_;
...
{
    return bgTypeId == BATTLEGROUND_AA || bgTypeId == BATTLEGROUND_BE ||
           bgTypeId == BATTLEGROUND_NA || bgTypeId == BATTLEGROUND_DS ||
           bgTypeId == BATTLEGROUND_RV || bgTypeId == BATTLEGROUND_RL;
}

BattlegroundQueueTypeId
BattlegroundMgr::BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType)
{
    uint32 queueTypeID = BATTLEGROUND_QUEUE_NONE;

    if (arenaType) {
        if (BattlegroundMgr::ArenaTypeToQueue.find(arenaType) !=
            BattlegroundMgr::ArenaTypeToQueue.end()) {
    ...
    if (BattlegroundMgr::bgToQueue.find(bgTypeId) !=
        BattlegroundMgr::bgToQueue.end()) {
...
{
    if (BattlegroundMgr::queueToBg.find(bgQueueTypeId) ==
        BattlegroundMgr::queueToBg.end()) {
...
uint8 BattlegroundMgr::BGArenaType(BattlegroundQueueTypeId bgQueueTypeId)
{
    ...
    if (BattlegroundMgr::QueueToArenaType.find(bgQueueTypeId) !=
        BattlegroundMgr::QueueToArenaType.end()) {
...
void BattlegroundMgr::ToggleArenaTesting()
{
    if (sWorld->getBoolConfig(CONFIG_DEBUG_ARENA)) {
        ...
        sWorld->SendWorldText(m_ArenaTesting ? LANG_DEBUG_ARENA_ON
                                          ...
                                          BattlegroundBracketId   bracket_id)
{
    ...
    uint64 const scheduleId = ((uint64)arenaMatchmakerRating << 32) |
                              ((uint64)arenaType << 24) |
                              ((uint64)bgQueueTypeId << 16) |
                              ((uint64)bgTypeId << 8) | (uint64)bracket_id;
    if (std::find(m_QueueUpdateScheduler.begin(),
                  m_QueueUpdateScheduler.end(),
                  scheduleId) == m_QueueUpdateScheduler.end())
...
void BattlegroundMgr::LoadBattleMastersEntry()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 battlemaster entries. DB table "
                 "`battlemaster_entry` is empty!");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (CreatureTemplate const* cInfo =
            ...
            if ((cInfo->npcflag & UNIT_NPC_FLAG_BATTLEMASTER) == 0)
                LOG_ERROR("sql.sql",
                          "Creature (Entry: {}) listed in `battlemaster_entry` "
        ...
        if (!sBattlemasterListStore.LookupEntry(bgTypeId)) {
            LOG_ERROR("sql.sql",
                      "Table `battlemaster_entry` contain entry {} for not "
                      "existed battleground type {}, ignored.",
                      entry,
                      bgTypeId);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} battlemaster entries in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void BattlegroundMgr::CheckBattleMasters()
{
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
         itr != ctc->end();
         ++itr) {
        if ((itr->second.npcflag & UNIT_NPC_FLAG_BATTLEMASTER) &&
            mBattleMastersMap.find(itr->second.Entry) ==
                mBattleMastersMap.end()) {
            LOG_ERROR(
                "sql.sql",
                "CreatureTemplate (Entry: {}) has UNIT_NPC_FLAG_BATTLEMASTER "
...
HolidayIds
BattlegroundMgr::BGTypeToWeekendHolidayId(BattlegroundTypeId bgTypeId)
{
    switch (bgTypeId) {
...
BattlegroundTypeId BattlegroundMgr::GetRandomBG(BattlegroundTypeId bgTypeId,
                                                uint32             minLevel)
{
    if (BattlegroundTemplate const* bgTemplate =
            GetBattlegroundTemplateByTypeId(bgTypeId)) {
        ...
        for (int32 mapId : bgTemplate->BattlemasterEntry->mapid) {
            ...
            if (BattlegroundTemplate const* bg =
                    GetBattlegroundTemplateByMapId(mapId)) {
        ...
        return *Acore::Containers::SelectRandomWeightedContainerElement(
            ids, weights);
    }
...
BattlegroundMgr::GetBGFreeSlotQueueStore(BattlegroundTypeId bgTypeId)
{
                                                ...
                                                uint32             instanceId)
{
    ...
    for (BGFreeSlotQueueContainer::iterator itr = queues.begin();
         itr != queues.end();
         ++itr)
...
std::unordered_map<int, bgRef> BattlegroundMgr::bgTypeToTemplate = {
    {BATTLEGROUND_AV,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundAV(*(BattlegroundAV*)bg_t);
     }},
    {BATTLEGROUND_WS,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundWS(*(BattlegroundWS*)bg_t);
     }},
    {BATTLEGROUND_AB,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundAB(*(BattlegroundAB*)bg_t);
     }},
    {BATTLEGROUND_NA,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundNA(*(BattlegroundNA*)bg_t);
     }},
    {BATTLEGROUND_BE,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundBE(*(BattlegroundBE*)bg_t);
     }},
    {BATTLEGROUND_EY,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundEY(*(BattlegroundEY*)bg_t);
     }},
    {BATTLEGROUND_RL,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundRL(*(BattlegroundRL*)bg_t);
     }},
    {BATTLEGROUND_SA,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundSA(*(BattlegroundSA*)bg_t);
     }},
    {BATTLEGROUND_DS,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundDS(*(BattlegroundDS*)bg_t);
     }},
    {BATTLEGROUND_RV,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundRV(*(BattlegroundRV*)bg_t);
     }},
    {BATTLEGROUND_IC,
     [](Battleground* bg_t) -> Battleground* {
         return new BattlegroundIC(*(BattlegroundIC*)bg_t);
     }},

    {
        BATTLEGROUND_RB,
        [](Battleground* bg_t) -> Battleground* {
            return new Battleground(*bg_t);
        },
    },
    {
        BATTLEGROUND_AA,
...
std::unordered_map<int, bgTypeRef> BattlegroundMgr::getBgFromTypeID = {
    {BATTLEGROUND_RB,
     [](WorldPacket*                                       data,
        Battleground::BattlegroundScoreMap::const_iterator itr2,
        Battleground*                                      bg) {
         if (BattlegroundMgr::getBgFromMap.find(bg->GetMapId()) ==
             BattlegroundMgr::getBgFromMap.end()) // this should not happen
         {
             *data << uint32(0);
         }
         else {
...
std::unordered_map<uint32, BattlegroundQueueTypeId>
    BattlegroundMgr::ArenaTypeToQueue = {
