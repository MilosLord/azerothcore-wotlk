...
enum BattlegroundSpells {
    ...
    SPELL_HONORABLE_DEFENDER_25Y =
        68652, // +50% honor when standing at a capture point that you control,
               // 25yards radius (added in 3.2)
    SPELL_HONORABLE_DEFENDER_60Y =
        66157 // +50% honor when standing at a capture point that you control,
              // 60yards radius (added in 3.2), probably for 40+ player
              // battlegrounds
};
...
#define RESURRECTION_INTERVAL                                                  \
    (sWorld->getIntConfig(CONFIG_BATTLEGROUND_PLAYER_RESPAWN) * IN_MILLISECONDS)
#define RESTORATION_BUFF_RESPAWN_TIME                                          \
    (sWorld->getIntConfig(CONFIG_BATTLEGROUND_RESTORATION_BUFF_RESPAWN))
#define BERSERKING_BUFF_RESPAWN_TIME                                           \
    (sWorld->getIntConfig(CONFIG_BATTLEGROUND_BERSERKING_BUFF_RESPAWN))
#define SPEED_BUFF_RESPAWN_TIME                                                \
    (sWorld->getIntConfig(CONFIG_BATTLEGROUND_SPEED_BUFF_RESPAWN))
...
const uint32 Buff_Entries[3] = {BG_OBJECTID_SPEEDBUFF_ENTRY,
...
enum BattlegroundStatus {
    ...
    STATUS_WAIT_JOIN  = 2,  // this means, that BG has already started and it is
...
public:
    ...
    void Fill(std::string_view    name,
...
public:
    ...
    /* achievement req. */
    [[nodiscard]] virtual bool AllNodesConrolledByTeam(TeamId /*teamId*/) const
    {
        return false;
    }
    ...
    [[nodiscard]] BattlegroundTypeId GetBgTypeID(bool GetRandom = false) const
    {
        return GetRandom ? m_RandomTypeID : m_RealTypeID;
    }
    ...
    [[nodiscard]] uint32             GetClientInstanceID() const
    {
    ...
    [[nodiscard]] uint32 GetLastResurrectTime() const
    {
    ...
    [[nodiscard]] uint32 GetMaxPlayersPerTeam() const
    {
    ...
    void AddToBGFreeSlotQueue(); // this queue will be useful when more
                                 // battlegrounds instances will be available
    void RemoveFromBGFreeSlotQueue(); // this method could delete whole BG
                                      // instance, if another free is available

    void DecreaseInvitedCount(TeamId teamId)
    {
        if (m_BgInvitedPlayers[teamId])
            --m_BgInvitedPlayers[teamId];
    }
    void IncreaseInvitedCount(TeamId teamId) { ++m_BgInvitedPlayers[teamId]; }
    [[nodiscard]] uint32 GetInvitedCount(TeamId teamId) const
    {
        return m_BgInvitedPlayers[teamId];
    }
    ...
    [[nodiscard]] const SpectatorList& GetSpectators() const
    {
        return m_Spectators;
    }
    void AddToBeTeleported(ObjectGuid spectator, ObjectGuid participant)
    {
        m_ToBeTeleported[spectator] = participant;
    }
    void RemoveToBeTeleported(ObjectGuid spectator)
    {
        ToBeTeleportedMap::iterator itr = m_ToBeTeleported.find(spectator);
        if (itr != m_ToBeTeleported.end())
            m_ToBeTeleported.erase(itr);
    }
    ...
    [[nodiscard]] BattlegroundPlayerMap const& GetPlayers() const
    {
        return m_Players;
    }
    ...
    typedef std::unordered_map<ObjectGuid::LowType, BattlegroundScore*>
        BattlegroundScoreMap;
    typedef std::unordered_map<ObjectGuid, ArenaLogEntryData>
                         ArenaLogEntryDataMap; // pussywizard
    ArenaLogEntryDataMap ArenaLogEntries;      // pussywizard
    [[nodiscard]] BattlegroundScoreMap const* GetPlayerScores() const
    {
        return &PlayerScores;
    }
    [[nodiscard]] std::size_t GetPlayerScoresSize() const
    {
        return PlayerScores.size();
    }

    [[nodiscard]] uint32 GetReviveQueueSize() const
    {
    ...
    [[nodiscard]] BattlegroundMap* GetBgMap() const
    {
        ASSERT(m_Map);
        return m_Map;
    }
    ...
    // Packet Transfer
    // method that should fill worldpacket with actual world states (not yet
    // implemented for all battlegrounds!)
    virtual void FillInitialWorldStates(WorldPacket& /*data*/) {}
    void         SendPacketToTeam(TeamId             teamId,
    ...
    void SendChatMessage(Creature*    source,
                         uint8        textId,
                         WorldObject* target = nullptr);
    void SendBroadcastText(uint32             id,
                           ChatMsg            msgType,
    ...
    void
    RewardReputationToTeam(uint32 factionId, uint32 reputation, TeamId teamId);
    ...
    void
    PSendMessageToAll(uint32 entry, ChatMsg type, Player const* source, ...);

    // specialized version with 2 string id args
    void SendMessage2ToAll(uint32        entry,
                           ChatMsg       type,
                           Player const* source,
                           uint32        strId1 = 0,
                           uint32        strId2 = 0);
    ...
    // Raid Group
    [[nodiscard]] Group* GetBgRaid(TeamId teamId) const
    {
        return m_BgRaids[teamId];
    }
    void SetBgRaid(TeamId teamId, Group* bg_raid);

    void         BuildPvPLogDataPacket(WorldPacket& data);
    virtual bool UpdatePlayerScore(Player* player,
                                   uint32  type,
                                   uint32  value,
                                   bool    doAddHonor = true);

    [[nodiscard]] uint32 GetPlayersCountByTeam(TeamId teamId) const
    {
        return m_PlayersCount[teamId];
    }
    [[nodiscard]] uint32 GetAlivePlayersCountByTeam(TeamId teamId)
        const; // used in arenas to correctly handle death in spirit of
               // redemption / last stand etc. (killer = killed) cases
    ...
    {
        m_ArenaTeamIds[teamId] = ArenaTeamId;
    }
    [[nodiscard]] uint32 GetArenaTeamIdForTeam(TeamId teamId) const
    {
    ...
    [[nodiscard]] uint32 GetArenaMatchmakerRating(TeamId teamId) const
    {
    ...
    // must be implemented in BG subclass if need AND call base class generic
    // code
    ...
    virtual void EventPlayerClickedOnFlag(Player* /*player*/,
                                          GameObject* /*gameObject*/)
    {
    }
    virtual void EventPlayerDamagedGO(Player* /*player*/,
                                      GameObject* /*go*/,
                                      uint32 /*eventType*/)
    {
    }
    ...
    virtual void
    AddPlayer(Player* player); // must be implemented in BG subclass
    ...
    void               SpawnBGObject(uint32 type,
                                     uint32 respawntime,
                                     uint32 forceRespawnDelay = 0);
    bool               AddObject(uint32  type,
                                 uint32  entry,
                                 float   x,
                                 float   y,
                                 float   z,
    ...
    Creature*          AddCreature(uint32           entry,
                                   uint32           type,
                                   float            x,
                                   float            y,
                                   float            z,
    ...
    // since arenas can be AvA or Hvh, we have to get the "temporary" team of a
    // player
    ...
    [[nodiscard]] virtual ObjectGuid
    GetFlagPickerGUID(TeamId /*teamId*/ = TEAM_NEUTRAL) const
    {
        return ObjectGuid::Empty;
    }
    virtual void SetDroppedFlagGUID(ObjectGuid /*guid*/,
    ...
    // because BattleGrounds with different types and same level range has
    // different m_BracketId
    [[nodiscard]] uint8 GetUniqueBracketId() const;

    BattlegroundAV* ToBattlegroundAV()
    {
        if (GetBgTypeID(true) == BATTLEGROUND_AV)
            return reinterpret_cast<BattlegroundAV*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundAV const* ToBattlegroundAV() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_AV)
            return reinterpret_cast<const BattlegroundAV*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_WS)
            return reinterpret_cast<BattlegroundWS*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundWS const* ToBattlegroundWS() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_WS)
            return reinterpret_cast<const BattlegroundWS*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_AB)
            return reinterpret_cast<BattlegroundAB*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundAB const* ToBattlegroundAB() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_AB)
            return reinterpret_cast<const BattlegroundAB*>(this);
        else
            return nullptr;
    }
    ...
    BattlegroundNA* ToBattlegroundNA()
    {
        if (GetBgTypeID(true) == BATTLEGROUND_NA)
            return reinterpret_cast<BattlegroundNA*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundNA const* ToBattlegroundNA() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_NA)
            return reinterpret_cast<const BattlegroundNA*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_BE)
            return reinterpret_cast<BattlegroundBE*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundBE const* ToBattlegroundBE() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_BE)
            return reinterpret_cast<const BattlegroundBE*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_EY)
            return reinterpret_cast<BattlegroundEY*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundEY const* ToBattlegroundEY() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_EY)
            return reinterpret_cast<const BattlegroundEY*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_RL)
            return reinterpret_cast<BattlegroundRL*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundRL const* ToBattlegroundRL() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_RL)
            return reinterpret_cast<const BattlegroundRL*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_SA)
            return reinterpret_cast<BattlegroundSA*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundSA const* ToBattlegroundSA() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_SA)
            return reinterpret_cast<const BattlegroundSA*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_DS)
            return reinterpret_cast<BattlegroundDS*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundDS const* ToBattlegroundDS() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_DS)
            return reinterpret_cast<const BattlegroundDS*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_RV)
            return reinterpret_cast<BattlegroundRV*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundRV const* ToBattlegroundRV() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_RV)
            return reinterpret_cast<const BattlegroundRV*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetBgTypeID(true) == BATTLEGROUND_IC)
            return reinterpret_cast<BattlegroundIC*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] BattlegroundIC const* ToBattlegroundIC() const
    {
        if (GetBgTypeID(true) == BATTLEGROUND_IC)
            return reinterpret_cast<const BattlegroundIC*>(this);
        else
            return nullptr;
    }

protected:
    // this method is called, when BG cannot spawn its own spirit guide, or
...
private:
    ...
    BattlegroundTypeId
           m_RandomTypeID; // TypeID created from Random Battleground list
    uint32 m_InstanceID;   // Battleground Instance's GUID!
    BattlegroundStatus m_Status;
    uint32 m_ClientInstanceID; // the instance-id which is sent to the client
                               // and without any other internal use
    uint32 m_StartTime;
    uint32 m_ResetStatTimer;
    uint32 m_ValidStartPositionTimer;
    int32  m_EndTime; // it is set to 120000 when bg is ending and it decreases
                      // itself
    uint32                m_LastResurrectTime;
    BattlegroundBracketId m_BracketId{BG_BRACKET_ID_FIRST};
    uint8                 m_ArenaType; // 2=2v2, 3=3v3, 5=5v5
    bool                  _InBGFreeSlotQueue{
        false}; // used to make sure that BG is only once inserted into the
                // BattlegroundMgr.BGFreeSlotQueue[bgTypeId] deque
    bool m_SetDeleteThis; // used for safe deletion of the bg after end / all
                          // players leave
    bool        m_IsArena;
    ...
    // Invited counters are useful for player invitation to BG - do not allow,
    // if BG is started to one faction to have 2 more players than another
    // faction Invited counters will be changed only when removing already
    // invited player from queue, removing player from battleground and inviting
    // player to BG Invited players counters
