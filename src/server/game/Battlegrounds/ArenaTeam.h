...
enum ArenaSlot { ARENA_SLOT_2v2, ARENA_SLOT_3v3, ARENA_SLOT_5v5 };
...
public:
    ...
    bool Create(ObjectGuid         captainGuid,
                uint8              type,
    ...
    // Shouldn't be ObjectGuid, because than can reference guid from members on
    // Disband and this method removes given record from list. So invalid
    // reference can happen.
    ...
    void BroadcastEvent(ArenaTeamEvents    event,
                        ObjectGuid         guid,
                        uint8              strCount,
    ...
    int32
    GetMatchmakerRatingMod(uint32 ownRating, uint32 opponentRating, bool won);
    int32 GetRatingMod(uint32 ownRating, uint32 opponentRating, bool won);
    float GetChanceAgainst(uint32 ownRating, uint32 opponentRating);
    int32 WonAgainst(uint32     Own_MMRating,
                     uint32     Opponent_MMRating,
                     int32&     rating_change,
                     const Map* bgMap);
    void  MemberWon(Player* player,
                    uint32  againstMatchmakerRating,
                    int32   MatchmakerRatingChange);
    int32 LostAgainst(uint32     Own_MMRating,
                      uint32     Opponent_MMRating,
                      int32&     rating_change,
                      const Map* bgMap);
    void  MemberLost(Player* player,
                     uint32  againstMatchmakerRating,
    ...
    {
        PreviousOpponents = arenaTeamId;
    }
    uint32 GetPreviousOpponents() { return PreviousOpponents; }

    void CreateTempArenaTeam(std::vector<Player*> playerList,
                             uint8                type,
                             std::string const&   teamName);

    // Containers
    static std::unordered_map<uint32, uint8> ArenaSlotByType; // Slot -> Type
    static std::unordered_map<uint8, uint8>
        ArenaReqPlayersForType; // Type -> Players count
