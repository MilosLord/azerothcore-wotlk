...
public:
    BattlegroundChatBuilder(ChatMsg       msgtype,
                            uint32        textId,
                            Player const* source,
...
private:
    void do_helper(WorldPacket& data, char const* text)
    {
        ChatHandler::BuildChatPacket(
            data, _msgtype, LANG_UNIVERSAL, _source, _source, text);
    }
...
public:
    Battleground2ChatBuilder(ChatMsg       msgtype,
                             uint32        textId,
                             Player const* source,
                             int32         arg1,
                             int32         arg2)
        : _msgtype(msgtype), _textId(textId), _source(source), _arg1(arg1),
          _arg2(arg2)
    {
    }
    ...
    void operator()(WorldPacket& data, LocaleConstant loc_idx)
    {
        char const* text = sObjectMgr->GetAcoreString(_textId, loc_idx);
        char const* arg1str =
            _arg1 ? sObjectMgr->GetAcoreString(_arg1, loc_idx) : "";
        char const* arg2str =
            _arg2 ? sObjectMgr->GetAcoreString(_arg2, loc_idx) : "";

        char str[2048];
        snprintf(str, 2048, text, arg1str, arg2str);

        ChatHandler::BuildChatPacket(
            data, _msgtype, LANG_UNIVERSAL, _source, _source, str);
    }
...
void Battleground::Update(uint32 diff)
{
    ...
    diff = BATTLEGROUND_UPDATE_INTERVAL; // just change diff value, no need to
                                         // replace variable name in many places
    ...
    if (!GetPlayersSize()) {
        ...
        //  [[   but if you use battleground object again (more battles possible
        //  to be played on 1 instance)
        //       then this condition should be removed and code:
        //       if (!GetInvitedCount(TEAM_HORDE) &&
        //       !GetInvitedCount(TEAM_ALLIANCE))
        //           AddToFreeBGObjectsQueue(); // not yet implemented
        //       should be used instead of current
        //  ]]
        //  Battleground Template instance cannot be updated, because it would
        //  be deleted
    ...
    case STATUS_IN_PROGRESS:
        if (isArena()) {
            if (GetStartTime() >=
                46 * MINUTE * IN_MILLISECONDS) // pussywizard: 1min startup +
                                               // 45min allowed duration
            {
            ...
            if (sBattlegroundMgr->GetPrematureFinishTime() &&
                (GetPlayersCountByTeam(TEAM_ALLIANCE) <
...
inline void Battleground::_CheckSafePositions(uint32 diff)
{
    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL) {
        ...
        for (auto const& [playerGuid, player] : GetPlayers()) {
            ...
            Position const* startPos =
                GetTeamStartPosition(player->GetBgTeamId());

            if (pos.GetExactDistSq(startPos) > maxDist) {
                LOG_DEBUG("bg.battleground",
                          "BATTLEGROUND: Sending {} back to start location "
                          "(map: {}) (possible exploit)",
                          player->GetName(),
                          GetMapId());
                player->TeleportTo(GetMapId(),
                                   startPos->GetPositionX(),
                                   startPos->GetPositionY(),
                                   startPos->GetPositionZ(),
                                   startPos->GetOrientation());
            }
        }
    }
}
...
inline void Battleground::_ProcessResurrect(uint32 diff)
{
    ...
    if (m_LastResurrectTime >= RESURRECTION_INTERVAL) {
        if (GetReviveQueueSize()) {
            for (std::map<ObjectGuid, GuidVector>::iterator itr =
                     m_ReviveQueue.begin();
                 itr != m_ReviveQueue.end();
                 ++itr) {
                Creature* sh = nullptr;
            ...
            // queue is clear and time passed, just update last resurrection
            // time
            m_LastResurrectTime = 0;
    }
    else if (m_LastResurrectTime >
             500) // Resurrect players only half a second later, to see spirit
                  // heal effect on NPC
    {
    ...
    if (!m_PrematureCountDown) {
        ...
        if (newtime > (MINUTE * IN_MILLISECONDS)) {
            if (newtime / (MINUTE * IN_MILLISECONDS) !=
                ...
                PSendMessageToAll(LANG_BATTLEGROUND_PREMATURE_FINISH_WARNING,
                                  CHAT_MSG_SYSTEM,
                                  nullptr,
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    if (m_ResetStatTimer > 5000) {
        m_ResetStatTimer = 0;
        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin();
             itr != GetPlayers().end();
             ++itr)
    ...
    if (!(m_Events & BG_STARTING_EVENT_1)) {
        ...
        if (!FindBgMap()) {
            LOG_ERROR("bg.battleground",
                      "Battleground::_ProcessJoin: map (map id: {}, instance "
                      "id: {}) is not created!",
                      m_MapId,
                      m_InstanceID);
            EndNow();
        ...
        if (StartMessageIds[BG_STARTING_EVENT_FIRST])
            SendBroadcastText(StartMessageIds[BG_STARTING_EVENT_FIRST],
                              CHAT_MSG_BG_SYSTEM_NEUTRAL);
    }
    // After 1 minute or 30 seconds, warning is signaled
    else if (GetStartDelayTime() <= StartDelayTimes[BG_STARTING_EVENT_SECOND] &&
             !(m_Events & BG_STARTING_EVENT_2)) {
        m_Events |= BG_STARTING_EVENT_2;

        if (StartMessageIds[BG_STARTING_EVENT_SECOND])
            SendBroadcastText(StartMessageIds[BG_STARTING_EVENT_SECOND],
                              CHAT_MSG_BG_SYSTEM_NEUTRAL);
    }
    // After 30 or 15 seconds, warning is signaled
    else if (GetStartDelayTime() <= StartDelayTimes[BG_STARTING_EVENT_THIRD] &&
    ...
    // Delay expired (after 2 or 1 minute)
        ...
        if (StartMessageIds[BG_STARTING_EVENT_FOURTH])
            SendBroadcastText(StartMessageIds[BG_STARTING_EVENT_FOURTH],
                              CHAT_MSG_BG_SYSTEM_NEUTRAL);
        ...
        // Remove preparation
        if (isArena()) {
            // pussywizard: initial visibility range is 30yd, set it to a proper
            // value now:
            if (BattlegroundMap* map = GetBgMap())
                map->SetVisibilityRange(
            ...
            for (BattlegroundPlayerMap::const_iterator itr =
                ...
                if (Player* player = itr->second) {
                    ...
                    Unit::AuraApplicationMap& auraMap =
                        player->GetAppliedAuras();
                    for (Unit::AuraApplicationMap::iterator iter =
                        ...
                        if (!aura->IsPermanent() &&
                            ...
                            // Xinef: condition, ALL buffs should be removed
                            &&
            ...
            for (SpectatorList::const_iterator itr = m_Spectators.begin();
                 itr != m_Spectators.end();
                 ++itr)
            ...
            if (GetStatus() == STATUS_IN_PROGRESS) {
                for (ToBeTeleportedMap::const_iterator itr =
                         m_ToBeTeleported.begin();
                     itr != m_ToBeTeleported.end();
                     ++itr)
                    if (Player* p =
                            ObjectAccessor::FindConnectedPlayer(itr->first))
                        if (Player* t =
                                ObjectAccessor::FindPlayer(itr->second)) {
                            if (!t->FindMap() || t->FindMap() != GetBgMap())
                                continue;

                            p->SetSummonPoint(t->GetMapId(),
                                              t->GetPositionX(),
                                              t->GetPositionY(),
                                              t->GetPositionZ(),
                                              15,
            ...
            if (sWorld->getBoolConfig(
                ...
                sWorld->SendWorldText(LANG_BG_STARTED_ANNOUNCE_WORLD,
                                      GetName().c_str(),
                                      std::min(GetMinLevel(), (uint32)80),
                                      std::min(GetMaxLevel(), (uint32)80));
                                     ...
                                     WorldObject const* target)
{
    if (!sObjectMgr->GetBroadcastText(id)) {
        LOG_ERROR("bg.battleground",
                  "Battleground::SendBroadcastText: `broadcast_text` (ID: {}) "
                  "was not found",
                  id);
        return;
    }
    ...
    Acore::BroadcastTextBuilder builder(
        nullptr, msgType, id, GENDER_MALE, target);
                                          ...
                                          TeamId teamId)
{
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
         ...
         ++itr)
        if (itr->second->GetBgTeamId() == teamId) {
            uint32 realFactionId =
                GetRealRepFactionForPlayer(factionId, itr->second);

            float repGain = static_cast<float>(reputation);
            AddPct(repGain,
                   itr->second->GetTotalAuraModifier(
                       SPELL_AURA_MOD_REPUTATION_GAIN));
            AddPct(repGain,
                   itr->second->GetTotalAuraModifierByMiscValue(
                       SPELL_AURA_MOD_FACTION_REPUTATION_GAIN, realFactionId));
            if (FactionEntry const* factionEntry =
                    sFactionStore.LookupEntry(realFactionId))
                itr->second->GetReputationMgr().ModifyReputation(factionEntry,
                                                                 repGain);
        }
}
...
{
    // xinef: if this is true, it means that endbattleground is called second
    // time skip to avoid double rating reduce / add can bug out due to
    // multithreading ? set as fast as possible
    ...
    if (winnerTeamId == PVP_TEAM_ALLIANCE) {
        if (isBattleground())
            SendBroadcastText(BG_TEXT_ALLIANCE_WINS,
                              CHAT_MSG_BG_SYSTEM_NEUTRAL);
    ...
    if (isBattleground() &&
        sWorld->getBoolConfig(CONFIG_BATTLEGROUND_STORE_STATISTICS_ENABLE)) {
    ...
    for (auto const& [playerGuid, player] : m_Players) {
        ...
        if (bgTeamId == GetTeamId(winnerTeamId)) {
            if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetBgTypeID(true))) {
                UpdatePlayerScore(player,
                                  SCORE_BONUS_HONOR,
                                  GetBonusHonorFromKill(winner_kills));

                // Xinef: check player level and not bracket level if
                // (CanAwardArenaPoints())
                if (player->GetLevel() >=
                    sWorld->getIntConfig(CONFIG_DAILY_RBG_MIN_LEVEL_AP_REWARD))
            ...
            player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_WIN_BG,
                                              player->GetMapId());
        }
        else {
            if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetBgTypeID(true)))
                UpdatePlayerScore(player,
                                  SCORE_BONUS_HONOR,
                                  GetBonusHonorFromKill(loser_kills));
        }

        sScriptMgr->OnBattlegroundEndReward(
            this, player, GetTeamId(winnerTeamId));
        ...
        if (isBattleground() &&
            ...
            auto const& score =
                PlayerScores.find(player->GetGUID().GetCounter());
...
bool Battleground::SpiritofCompetitionEvent(PvPTeamId winnerTeamId)
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
        ...
        bool    questStatus = player->GetQuestStatus(QUEST_FLAG_PARTICIPANT) !=
                           QUEST_STATUS_REWARDED;

        if (player && questStatus)
            player->CastSpell(
                player, SPELL_SPIRIT_OF_COMPETITION_PARTICIPANT, true);
    }
    ...
    if (filteredPlayers.size()) {
        if (Player* wPlayer = filteredPlayers[rand() % filteredPlayers.size()])
            wPlayer->CastSpell(
                wPlayer, SPELL_SPIRIT_OF_COMPETITION_WINNER, true);
    }
...
void Battleground::BlockMovement(Player* player)
{
    player->SetClientControl(
        player,
        0); // movement disabled NOTE: the effect will be automatically removed
            // by client when the player is teleported from the battleground, so
            // no need to send with uint8(1) in RemovePlayerAtLeave()
}
...
void Battleground::RemovePlayerAtLeave(Player* player)
{
    ...
    if (participant) {
        ...
        BattlegroundQueueTypeId bgQueueTypeId =
            BattlegroundMgr::BGQueueTypeId(GetBgTypeID(), GetArenaType());

        // this call is important, because player, when joins to battleground,
        // this method is not called, so it must be called when leaving bg
        ...
        if (Group* group = GetBgRaid(teamId))
            if (group->IsMember(player->GetGUID()))
                if (!group->RemoveMember(
                        player->GetGUID())) // group was disbanded
                    SetBgRaid(teamId, nullptr);

        // let others know
        sBattlegroundMgr->BuildPlayerLeftBattlegroundPacket(&data,
                                                            player->GetGUID());
        SendPacketToTeam(teamId, &data, player, false);

        // cast deserter
        if (isBattleground() && !player->IsGameMaster() &&
        ...
        if (isBattleground() && GetStatus() < STATUS_WAIT_LEAVE) {
            ...
            // a player has left the battleground, so there are free slots ->
            // add to queue
            AddToBGFreeSlotQueue();
            sBattlegroundMgr->ScheduleQueueUpdate(
                0, 0, bgQueueTypeId, bgTypeId, GetBracketId());
        }
    }
    ...
    player->ResetAchievementCriteria(
        ACHIEVEMENT_CRITERIA_CONDITION_BG_MAP, GetMapId(), true);
...
void Battleground::Init()
{
    ...
    if (m_BgInvitedPlayers[TEAM_ALLIANCE] > 0 ||
        m_BgInvitedPlayers[TEAM_HORDE] > 0) {
        LOG_ERROR(
            "bg.battleground",
            "Battleground::Reset: one of the counters is not 0 (alliance: {}, "
            "horde: {}) for BG (map: {}, instance id: {})!",
            m_BgInvitedPlayers[TEAM_ALLIANCE],
            m_BgInvitedPlayers[TEAM_HORDE],
            m_MapId,
            m_InstanceID);
        ABORT();
    }
...
void Battleground::StartBattleground()
{
    ...
    // this must be done here, because we need to have already invited some
    // players when first Battleground::Update() method is executed
    sBattlegroundMgr->AddBattleground(this);

    if (m_IsRated)
        LOG_DEBUG("bg.arena",
                  "Arena match type: {} for Team1Id: {} - Team2Id: {} started.",
                  m_ArenaType,
                  m_ArenaTeamIds[TEAM_ALLIANCE],
                  m_ArenaTeamIds[TEAM_HORDE]);
}
...
void Battleground::AddPlayer(Player* player)
{
    ...
    // Log
    LOG_DEBUG("bg.battleground",
              "BATTLEGROUND: Player {} joined the battle.",
              player->GetName());
}
...
{
    if (player->GetGroup() &&
        (player->GetGroup()->isBGGroup() || player->GetGroup()->isBFGroup())) {
        LOG_INFO("misc",
                 "Battleground::AddOrSetPlayerToCorrectBgGroup - player is "
                 "already in {} group!",
                 (player->GetGroup()->isBGGroup() ? "BG" : "BF"));
        return;
    }
...
// This method removes this battleground from free queue - it must be called
// when deleting battleground
...
uint32 Battleground::GetFreeSlotsForTeam(TeamId teamId) const
{
    ...
    // BG_QUEUE_INVITATION_TYPE_NO_BALANCE, invite everyone unless the BG is
    // full
    if (sWorld->getIntConfig(CONFIG_BATTLEGROUND_INVITATION_TYPE) ==
        BG_QUEUE_INVITATION_TYPE_NO_BALANCE)
        return (GetInvitedCount(teamId) < GetMaxPlayersPerTeam())
                   ? GetMaxPlayersPerTeam() - GetInvitedCount(teamId)
                   : 0;

    // if BG is already started or CONFIG_BATTLEGROUND_INVITATION_TYPE !=
    // BG_QUEUE_INVITATION_TYPE_NO_BALANCE, do not allow to join too many
    // players of one faction
    uint32 thisTeamInvitedCount  = teamId == TEAM_ALLIANCE
                                       ? GetInvitedCount(TEAM_ALLIANCE)
                                       : GetInvitedCount(TEAM_HORDE);
    uint32 thisTeamPlayersCount  = teamId == TEAM_ALLIANCE
                                       ? GetPlayersCountByTeam(TEAM_ALLIANCE)
                                       : GetPlayersCountByTeam(TEAM_HORDE);
    uint32 otherTeamInvitedCount = teamId == TEAM_ALLIANCE
                                       ? GetInvitedCount(TEAM_HORDE)
                                       : GetInvitedCount(TEAM_ALLIANCE);
    uint32 otherTeamPlayersCount = teamId == TEAM_ALLIANCE
                                       ? GetPlayersCountByTeam(TEAM_HORDE)
                                       : GetPlayersCountByTeam(TEAM_ALLIANCE);
    ...
    // allow join one person if the sides are equal (to fill up bg to
    // minPlayerPerTeam)
    if (otherTeamInvitedCount == thisTeamInvitedCount)
        diff = 1;
    else if (otherTeamInvitedCount >
             thisTeamInvitedCount) // allow join more ppl if the other side has
                                   // more players
        diff = otherTeamInvitedCount - thisTeamInvitedCount;

    // difference based on max players per team (don't allow inviting more)
    uint32 diff2 = (thisTeamInvitedCount < maxPlayersPerTeam)
                       ? maxPlayersPerTeam - thisTeamInvitedCount
                       : 0;
    ...
    // allow join one person if the sides are equal (to fill up bg
    // minPlayerPerTeam)
    if (otherTeamPlayersCount == thisTeamPlayersCount)
        diff3 = 1;
    else if (otherTeamPlayersCount >
             thisTeamPlayersCount) // allow join more ppl if the other side has
                                   // more players
        diff3 = otherTeamPlayersCount - thisTeamPlayersCount;
    else if (thisTeamInvitedCount <=
             minPlayersPerTeam) // or other side has less than minPlayersPerTeam
...
{
    return std::max<uint32>(GetFreeSlotsForTeam(TEAM_ALLIANCE),
                            GetFreeSlotsForTeam(TEAM_HORDE));
}
...
void Battleground::ReadyMarkerClicked(Player* p)
{
    if (!isArena() || GetStatus() >= STATUS_IN_PROGRESS ||
        GetStartDelayTime() <= BG_START_DELAY_15S ||
        (m_Events & BG_STARTING_EVENT_3) || p->IsSpectator())
        return;
    ...
    p->GetSession()->SendNotification(
        "You are marked as ready %u/%u", count, req);
    if (count == req) {
        m_Events |= BG_STARTING_EVENT_2;
                                     ...
                                     bool    doAddHonor)
{
    ...
    if (type == SCORE_BONUS_HONOR && doAddHonor && isBattleground())
        player->RewardHonor(
            nullptr, 1, value); // RewardHonor calls UpdatePlayerScore with
                                // doAddHonor = false
    else
...
{
    for (std::map<ObjectGuid, GuidVector>::iterator itr = m_ReviveQueue.begin();
         itr != m_ReviveQueue.end();
         ++itr)
        for (GuidVector::iterator itr2 = itr->second.begin();
             itr2 != itr->second.end();
             ++itr2)
...
{
    // Those who are waiting to resurrect at this node are taken to the closest
    // own node's graveyard
                             ...
                             GOState goState)
{
    ...
    // map of the instance and when loading it (in go::LoadFromDB()), a new guid
    // would be assigned to the object, and a new object would be created So we
    // must create it specific for this instance
    ...
    /*
       ...
       checking if it existed GameObjectData& data =
       sObjectMgr->NewGOData(spawnId);
...
// gameobject::update() It would be nice to correctly implement GO_ACTIVATED
// state and open/close doors in gameobject code
void Battleground::DoorClose(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type])) {
        ...
        LOG_ERROR("bg.battleground",
                  "Battleground::DoorClose: door gameobject (type: {}, {}) not "
                  "found for BG (map: {}, instance id: {})!",
                  type,
                  BgObjects[type].ToString(),
                  m_MapId,
                  m_InstanceID);
}
...
void Battleground::DoorOpen(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type])) {
        ...
        LOG_ERROR("bg.battleground",
                  "Battleground::DoorOpen: door gameobject (type: {}, {}) not "
                  "found for BG (map: {}, instance id: {})!",
                  type,
                  BgObjects[type].ToString(),
                  m_MapId,
...
GameObject* Battleground::GetBGObject(uint32 type)
{
    ...
    if (!obj)
        LOG_ERROR("bg.battleground",
                  "Battleground::GetBGObject: gameobject (type: {}, {}) not "
                  "found for BG (map: {}, instance id: {})!",
                  type,
                  BgObjects[type].ToString(),
                  m_MapId,
...
Creature* Battleground::GetBGCreature(uint32 type)
{
    ...
    if (!creature)
        LOG_ERROR("bg.battleground",
                  "Battleground::GetBGCreature: creature (type: {}, {}) not "
                  "found for BG (map: {}, instance id: {})!",
                  type,
                  BgCreatures[type].ToString(),
                  m_MapId,
                  m_InstanceID);
    return creature;
}
    ...
    if (Map* map = FindBgMap())
        if (GameObject* obj = map->GetGameObject(BgObjects[type])) {
            if (respawntime)
                ...
                // Change state from GO_JUST_DEACTIVATED to GO_READY in case
                                    ...
                                    MotionTransport* transport)
{
    ...
    if (transport) {
        ...
        if (Creature* creature = transport->SummonCreature(
                entry, x, y, z, o, TEMPSUMMON_MANUAL_DESPAWN)) {
    ...
    if (!creature->Create(map->GenerateLowGuid<HighGuid::Unit>(),
                          ...
                          o)) {
        LOG_ERROR("bg.battleground",
                  "Battleground::AddCreature: cannot create creature (entry: "
                  "{}) for BG (map: {}, instance id: {})!",
                  entry,
                  m_MapId,
                  m_InstanceID);
    ...
    if (!cinfo) {
        LOG_ERROR("bg.battleground",
                  "Battleground::AddCreature: creature template (entry: {}) "
                  "does not exist for BG (map: {}, instance id: {})!",
                  entry,
                  m_MapId,
                  m_InstanceID);
        delete creature;
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    LOG_ERROR("bg.battleground",
              "Battleground::DelCreature: creature (type: {}, {}) not found "
              "for BG (map: {}, instance id: {})!",
              type,
              BgCreatures[type].ToString(),
              m_MapId,
...
bool Battleground::DelObject(uint32 type)
{
    ...
    LOG_ERROR("bg.battleground",
              "Battleground::DelObject: gameobject (type: {}, {}) not found "
              "for BG (map: {}, instance id: {})!",
              type,
              BgObjects[type].ToString(),
              m_MapId,
...
bool Battleground::AddSpiritGuide(
    uint32 type, float x, float y, float z, float o, TeamId teamId)
{
    uint32 entry = (teamId == TEAM_ALLIANCE) ? BG_CREATURE_ENTRY_A_SPIRITGUIDE
    ...
    LOG_ERROR("bg.battleground",
              "Battleground::AddSpiritGuide: cannot create spirit guide (type: "
              "{}, entry: {}) for BG (map: {}, instance id: {})!",
              type,
              entry,
              m_MapId,
              m_InstanceID);
...
void Battleground::SendWarningToAll(uint32 entry, ...)
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
         itr != m_Players.end();
         ++itr) {
        if (localizedPackets.find(
                itr->second->GetSession()->GetSessionDbLocaleIndex()) ==
            localizedPackets.end()) {
            char const* format = sObjectMgr->GetAcoreString(
            ...
            ChatHandler::BuildChatPacket(
                localizedPackets[itr->second->GetSession()
...
void Battleground::SendMessage2ToAll(
    uint32 entry, ChatMsg type, Player const* source, uint32 arg1, uint32 arg2)
{
...
{
    // FIXME: now we have different DBC locales and need localized message for
    // each target client
...
{
    // Xinef: crash fix?
    if (GetStatus() != STATUS_IN_PROGRESS || !GetPlayersSize() ||
        BgObjects.empty())
        return;

    uint32 index = 0;
    for (;
         index < BgObjects.size() && BgObjects[index] != gameObject->GetGUID();
         ++index)
        ;
...
void Battleground::HandleKillPlayer(Player* victim, Player* killer)
{
    ...
    if (killer) {
        // Don't reward credit for killing ourselves, like fall damage of
        // hellfire (warlock)
        if (killer == victim)
            return;
        ...
        for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
            ...
            if (creditedPlayer->GetBgTeamId() == killer->GetBgTeamId() &&
...
uint32 Battleground::GetAlivePlayersCountByTeam(TeamId teamId) const
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
        ...
        if (itr->second->IsAlive() &&
            !itr->second->HasByteFlag(
                UNIT_FIELD_BYTES_2, 3, FORM_SPIRITOFREDEMPTION) &&
            itr->second->GetBgTeamId() == teamId)
            ++count;
...
int32 Battleground::GetObjectType(ObjectGuid guid)
{
    ...
    LOG_ERROR(
        "bg.battleground",
        "Battleground::GetObjectType: player used gameobject ({}) which is not "
        "in internal data for BG (map: {}, instance id: {}), cheating?",
        guid.ToString(),
        m_MapId,
        m_InstanceID);
...
uint32 Battleground::GetTeamScore(TeamId teamId) const
{
    ...
    LOG_ERROR("bg.battleground",
              "GetTeamScore with wrong Team {} for BG {}",
              teamId,
              GetBgTypeID());
    return 0;
}
...
uint8 Battleground::GetUniqueBracketId() const { return GetMaxLevel() / 10; }
