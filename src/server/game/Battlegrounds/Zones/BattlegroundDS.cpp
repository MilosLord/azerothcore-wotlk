...
void BattlegroundDS::PostUpdateImpl(uint32 diff)
{
    ...
    while (uint32 eventId = _events.ExecuteEvent()) {
        ...
        case BG_DS_EVENT_WATERFALL_WARNING:
            ...
            _events.ScheduleEvent(BG_DS_EVENT_WATERFALL_ON,
        ...
        case BG_DS_EVENT_WATERFALL_ON:
            ...
            _events.ScheduleEvent(BG_DS_EVENT_WATERFALL_KNOCKBACK,
        ...
        case BG_DS_EVENT_WATERFALL_OFF:
            ...
            _events.ScheduleEvent(BG_DS_EVENT_WATERFALL_WARNING,
        ...
        case BG_DS_EVENT_WATERFALL_KNOCKBACK:
            ...
            if (Creature* waterSpout =
                    GetBGCreature(BG_DS_NPC_WATERFALL_KNOCKBACK))
                waterSpout->CastSpell(
                    waterSpout, BG_DS_SPELL_WATER_SPOUT, true);
            _events.ScheduleEvent(eventId, BG_DS_WATERFALL_KNOCKBACK_TIMER);
            break;
        case BG_DS_EVENT_PIPE_KNOCKBACK:
            for (uint32 i = BG_DS_NPC_PIPE_KNOCKBACK_1;
                 i <= BG_DS_NPC_PIPE_KNOCKBACK_2;
                 ++i)
...
void BattlegroundDS::StartingEventOpenDoors()
{
    ...
    //     _events.ScheduleEvent(BG_DS_EVENT_PIPE_KNOCKBACK,
    //     BG_DS_PIPE_KNOCKBACK_FIRST_DELAY + i * BG_DS_PIPE_KNOCKBACK_DELAY);
    ...
    // Turn off collision
    if (GameObject* gob =
            GetBgMap()->GetGameObject(BgObjects[BG_DS_OBJECT_WATER_1]))
...
void BattlegroundDS::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case 5348:
        ...
        // Someone has get back into the pipes and the knockback has already
        // been performed, so we reset the knockback count for kicking the
        // player again into the arena.
        if (_pipeKnockBackCount >= BG_DS_PIPE_KNOCKBACK_TOTAL_COUNT)
            _pipeKnockBackCount = 0;
        break;
    ...
    case 5344: // -60
        player->NearTeleportTo(
            1330.0f, 800.0f, 3.16f, player->GetOrientation());
        break;
        /*default:
