...
enum BG_WS_SpellId {
    ...
    BG_WS_SPELL_WARSONG_FLAG_PICKED =
        61266, // fake spell, does not exist but used as timer start event
    BG_WS_SPELL_SILVERWING_FLAG         = 23335,
    BG_WS_SPELL_SILVERWING_FLAG_DROPPED = 23336,
    BG_WS_SPELL_SILVERWING_FLAG_PICKED =
        61265, // fake spell, does not exist but used as timer start event
...
public:
    ...
    {
        _droppedFlagGUID[teamId] = guid;
    }
    ObjectGuid GetDroppedFlagGUID(TeamId teamId) const
    {
        return _droppedFlagGUID[teamId];
    }
    ...
    {
        return 1 + (BG_WS_TOTAL_GAME_TIME - GetStartTime()) /
