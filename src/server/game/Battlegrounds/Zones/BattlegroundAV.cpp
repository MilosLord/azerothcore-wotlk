...
BattlegroundAV::BattlegroundAV()
{
    ...
    BgCreatures.resize(static_cast<uint16>(AV_CPLACE_MAX) +
                       AV_STATICCPLACE_MAX);
...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS]) {
        CastSpellOnTeam(23658,
                        TEAM_HORDE); // this is a spell which finishes a quest
                                     // where a player has to kill the boss
        ...
        CastSpellOnTeam(
            23658, TEAM_ALLIANCE); // this is a spell which finishes a quest
                                   // where a player has to kill the boss
        RewardReputationToTeam(730, BG_AV_REP_BOSS, TEAM_ALLIANCE);
        RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_BOSS),
                          TEAM_ALLIANCE);
        ...
        if (!m_CaptainAlive[0]) {
            LOG_ERROR("bg.battleground",
                      "Killed a Captain twice, please report this bug, if you "
                      "haven't done \".respawn\"");
            return;
        }
        m_CaptainAlive[0] = false;
        RewardReputationToTeam(729, BG_AV_REP_CAPTAIN, TEAM_HORDE);
        RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_CAPTAIN),
                          TEAM_HORDE);
        UpdateScore(TEAM_ALLIANCE, (-1) * BG_AV_RES_CAPTAIN);
        // spawn destroyed aura
        for (uint8 i = 0; i <= 9; i++)
            SpawnBGObject(BG_AV_OBJECT_BURN_BUILDING_ALLIANCE + i,
                          RESPAWN_IMMEDIATELY);
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if (creature)
            YellToAll(creature,
                      GetAcoreString(LANG_BG_AV_A_CAPTAIN_DEAD),
                      LANG_UNIVERSAL);
        ...
        // spawn destroyed aura
        for (uint8 i = 0; i <= 9; i++)
            SpawnBGObject(BG_AV_OBJECT_BURN_BUILDING_HORDE + i,
                          RESPAWN_IMMEDIATELY);
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if (creature)
            YellToAll(creature,
                      GetAcoreString(LANG_BG_AV_H_CAPTAIN_DEAD),
                      LANG_UNIVERSAL);
        DelCreature(AV_CPLACE_TRIGGER18);
    }
    else if (entry == BG_AV_CreatureInfo[AV_NPC_N_MINE_N_4] ||
             entry == BG_AV_CreatureInfo[AV_NPC_N_MINE_A_4] ||
             entry == BG_AV_CreatureInfo[AV_NPC_N_MINE_H_4]) {
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_S_MINE_N_4] ||
             entry == BG_AV_CreatureInfo[AV_NPC_S_MINE_A_4] ||
             entry == BG_AV_CreatureInfo[AV_NPC_S_MINE_H_4]) {
...
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return; // maybe we should log this, cause this must be a cheater or a
                // big bug
    TeamId teamId = player->GetTeamId();
    // TODO add reputation, events (including quest not available anymore, next
    // quest availabe, go/npc de/spawning)and maybe honor
    ...
    case AV_QUEST_H_SCRAPS2:
        ...
        if (m_Team_QuestStatus[teamId][0] == 500 ||
            m_Team_QuestStatus[teamId][0] == 1000 ||
            m_Team_QuestStatus[teamId][0] == 1500) // 25, 50, 75 turn ins
        {
            LOG_DEBUG("bg.battleground",
                      "BG_AV Quest {} completed starting with unit upgrading..",
                      questid);
            for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION;
                 i <= BG_AV_NODES_FROSTWOLF_HUT;
                 ++i)
                if (m_Nodes[i].OwnerId == player->GetTeamId() &&
                    m_Nodes[i].State == POINT_CONTROLED) {
                    DePopulateNode(i);
                    PopulateNode(i);
                    // maybe this is bad, because it will instantly respawn all
                    // creatures on every grave..
                }
        }
        break;
    ...
    case AV_QUEST_H_COMMANDER1:
        ...
        if (m_Team_QuestStatus[teamId][1] == 30)
            LOG_DEBUG(
                "bg.battleground",
                "BG_AV Quest {} completed (need to implement some events here",
                questid);
        break;
    ...
    case AV_QUEST_H_NEAR_MINE:
        ...
        if (m_Team_QuestStatus[teamId][5] == 28) {
            ...
            if (m_Team_QuestStatus[teamId][6] == 7)
                LOG_DEBUG("bg.battleground",
                          "BG_AV Quest {} completed (need to implement some "
                          "events here - ground assault ready",
                          questid);
        }
        break;
    ...
    case AV_QUEST_H_RIDER_HIDE:
        ...
        if (m_Team_QuestStatus[teamId][7] == 25) {
            ...
            if (m_Team_QuestStatus[teamId][8] == 25)
                LOG_DEBUG("bg.battleground",
                          "BG_AV Quest {} completed (need to implement some "
                          "events here - rider assault ready",
                          questid);
        }
        break;
    ...
    default:
        LOG_DEBUG("bg.battleground",
                  "BG_AV Quest {} completed but is not interesting at all",
                  questid);
...
{
    // note: to remove reinforcementpoints points must be negative, for adding
    // reinforcements points must be positive
    m_Team_Scores[teamId] += points;

    UpdateWorldState(
        ((teamId == TEAM_HORDE) ? AV_Horde_Score : AV_Alliance_Score),
        m_Team_Scores[teamId]);
    if (points < 0) {
        if (m_Team_Scores[teamId] < 1) {
            m_Team_Scores[teamId] = 0;
            EndBattleground(GetOtherTeamId(teamId));
        }
        else if (!m_IsInformedNearVictory[teamId] &&
                 m_Team_Scores[teamId] < SEND_MSG_NEAR_LOSE) {
            if (teamId == TEAM_ALLIANCE) {
                SendBroadcastText(BG_AV_TEXT_ALLIANCE_NEAR_LOSE,
                                  CHAT_MSG_BG_SYSTEM_ALLIANCE);
            }
            else {
                SendBroadcastText(BG_AV_TEXT_HORDE_NEAR_LOSE,
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type)
{
    ...
    if (!creature)
        return nullptr;
    if (creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN] ||
        creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN])
        creature->SetRespawnDelay(
            RESPAWN_ONE_DAY); /// @todo: look if this can be done by database +
                              /// also add this for the wingcommanders

    if ((isStatic && cinfoid >= 10 && cinfoid <= 14) ||
        (!isStatic && ((cinfoid >= AV_NPC_A_GRAVEDEFENSE0 &&
                        cinfoid <= AV_NPC_A_GRAVEDEFENSE3) ||
                       (cinfoid >= AV_NPC_H_GRAVEDEFENSE0 &&
                        cinfoid <= AV_NPC_H_GRAVEDEFENSE3)))) {
        if (!isStatic && ((cinfoid >= AV_NPC_A_GRAVEDEFENSE0 &&
                           cinfoid <= AV_NPC_A_GRAVEDEFENSE3) ||
                          (cinfoid >= AV_NPC_H_GRAVEDEFENSE0 &&
                           cinfoid <= AV_NPC_H_GRAVEDEFENSE3))) {
            CreatureData& data =
                sObjectMgr->NewOrExistCreatureData(creature->GetSpawnId());
        ...
        // (i just copied this code from a gm-command
    }
...
void BattlegroundAV::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS) {
        for (uint8 i = 0; i <= 1; i++) // 0=alliance, 1=horde
        {
            ...
            if (m_CaptainBuffTimer[i] > diff) {
                ...
                if (i == 0) {
                    ...
                    Creature* creature = GetBGCreature(
                        AV_CPLACE_MAX +
                        59); // TODO: make the captains a dynamic creature
                    if (creature) {
                        std::string creatureText =
                            sCreatureTextMgr->GetLocalizedChatString(
                                creature->GetEntry(),
                ...
                m_CaptainBuffTimer[i] =
                    120000 +
                    urand(0, 4) * 60000; // as far as i could see, the buff is
                                         // randomly so i make 2minutes (thats
                                         // the duration of the buff itself) +
                                         // 0-4minutes TODO get the right times
            }
        }
        // add points from mine owning, and look if he neutral team wanrts to
        // reclaim the mine
        m_Mine_Timer -= diff;
        for (uint8 mine = 0; mine < 2; mine++) {
            if (m_Mine_Owner[mine] == TEAM_ALLIANCE ||
                m_Mine_Owner[mine] == TEAM_HORDE) {
                if (m_Mine_Timer <= 0)
                    UpdateScore(m_Mine_Owner[mine], 1);

                if (m_Mine_Reclaim_Timer[mine] > diff)
                    m_Mine_Reclaim_Timer[mine] -= diff;
                else // we don't need to set this timer to 0 cause this codepart
                     // wont get called when this thing is 0
                {
        ...
        if (m_Mine_Timer <= 0)
            m_Mine_Timer = AV_MINE_TICK_TIMER; // this is at the end, cause we
                                               // need to update both mines
        ...
        // team
        for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX;
             ++i)
...
void BattlegroundAV::StartingEventOpenDoors()
{
    ...
    for (uint16 i = BG_AV_OBJECT_MINE_SUPPLY_N_MIN;
         i <= BG_AV_OBJECT_MINE_SUPPLY_N_MAX;
         i++)
        SpawnBGObject(i, RESPAWN_IMMEDIATELY, 5 * MINUTE);
    for (uint16 i = BG_AV_OBJECT_MINE_SUPPLY_S_MIN;
         i <= BG_AV_OBJECT_MINE_SUPPLY_S_MAX;
         i++)
        SpawnBGObject(i, RESPAWN_IMMEDIATELY, 5 * MINUTE);
    for (uint8 mine = AV_NORTH_MINE; mine <= AV_SOUTH_MINE;
         mine++) // mine population
...
void BattlegroundAV::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(player->GetGUID().GetCounter(),
                         new BattlegroundAVScore(player->GetGUID()));
}
...
void BattlegroundAV::EndBattleground(TeamId winnerTeamId)
{
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_DUNBALDAR_SOUTH;
         i <= BG_AV_NODES_FROSTWOLF_WTOWER;
         ++i) {
    ...
    for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE;
        ...
        if (rep[iTeamId] != 0)
            RewardReputationToTeam(
                iTeamId == TEAM_ALLIANCE ? 730 : 729, rep[iTeamId], iTeamId);
        if (kills[iTeamId] != 0)
...
void BattlegroundAV::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case 2608:
        if (player->GetTeamId() != TEAM_ALLIANCE)
            player->GetSession()->SendAreaTriggerMessage(
                "Only The Alliance can use that portal");
        else
    ...
    case 2606:
        if (player->GetTeamId() != TEAM_HORDE)
            player->GetSession()->SendAreaTriggerMessage(
                "Only The Horde can use that portal");
        else
            player->LeaveBattleground();
        break;
    case 3326:
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    LOG_DEBUG("bg.battleground",
              "bg_av: player destroyed point node {} object {}",
              node,
              object);
    ...
    if (IsTower(node)) {
        ...
        if (BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
            ...
            LOG_ERROR("bg.battleground",
                      "BG_AV: playerdestroyedpoint: marshal {} doesn't exist",
                      AV_CPLACE_A_MARSHAL_SOUTH + tmp);
        // spawn destroyed aura
        for (uint8 i = 0; i <= 9; i++)
            SpawnBGObject(BG_AV_OBJECT_BURN_DUNBALDAR_SOUTH + i + (tmp * 10),
                          RESPAWN_IMMEDIATELY);

        UpdateScore((ownerId == TEAM_ALLIANCE) ? TEAM_HORDE : TEAM_ALLIANCE,
                    -1 * BG_AV_RES_TOWER);
        RewardReputationToTeam(
            ownerId == TEAM_ALLIANCE ? 730 : 729, BG_AV_REP_TOWER, ownerId);
        RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_TOWER), ownerId);

        SpawnBGObject(static_cast<uint8>(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH) +
                          ownerId + (2 * tmp),
                      RESPAWN_ONE_DAY);
        SpawnBGObject(static_cast<uint8>(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH) +
                          ownerId + (2 * tmp),
                      RESPAWN_ONE_DAY);
    }
    else {
        if (ownerId == TEAM_ALLIANCE)
            ...
            SpawnBGObject(object + 11, RESPAWN_IMMEDIATELY);
        SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                      RESPAWN_ONE_DAY);
        SpawnBGObject(static_cast<uint8>(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION) +
                          ownerId + 3 * node,
                      RESPAWN_IMMEDIATELY);
        PopulateNode(node);
        ...
        {
            for (uint8 i = 0; i < 4; i++) {
                SpawnBGObject(((ownerId == TEAM_ALLIANCE)
    ...
    if (IsTower(node))
        snprintf(buf,
                 sizeof(buf),
                 GetAcoreString(LANG_BG_AV_TOWER_TAKEN),
                 GetNodeName(node),
                 (ownerId == TEAM_ALLIANCE) ? GetAcoreString(LANG_BG_AV_ALLY)
                                            : GetAcoreString(LANG_BG_AV_HORDE));
    else
        snprintf(buf,
                 sizeof(buf),
                 GetAcoreString(LANG_BG_AV_GRAVE_TAKEN),
                 GetNodeName(node),
                 (ownerId == TEAM_ALLIANCE) ? GetAcoreString(LANG_BG_AV_ALLY)
                                            : GetAcoreString(LANG_BG_AV_HORDE));
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, TeamId teamId, bool initial)
{
    ...
    // changing the owner results in setting respawntim to infinite for current
    // creatures, spawning new mine owners creatures and changing the
    // chest-objects so that the current owning team can use them

    ASSERT(mine == AV_NORTH_MINE || mine == AV_SOUTH_MINE);
    if (teamId == TEAM_ALLIANCE || teamId == TEAM_HORDE)
        PlaySoundToAll((teamId == TEAM_ALLIANCE) ? AV_SOUND_ALLIANCE_GOOD
                                                 : AV_SOUND_HORDE_GOOD);
    ...
    if (!initial) {
        LOG_DEBUG("bg.battleground",
                  "bg_av depopulating mine {} (0=north, 1=south)",
                  mine);
        if (mine == AV_SOUTH_MINE)
            for (uint16 i = AV_CPLACE_MINE_S_S_MIN; i <= AV_CPLACE_MINE_S_S_MAX;
                 i++)
                if (BgCreatures[i])
                    DelCreature(i); // TODO just set the respawntime to 999999
        for (uint16 i = ((mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_1_MIN
                                                 : AV_CPLACE_MINE_S_1_MIN);
             i <= ((mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_3
                                           : AV_CPLACE_MINE_S_3);
             i++)
            if (BgCreatures[i])
    ...
    LOG_DEBUG(
        "bg.battleground", "bg_av populating mine {} (0=north, 1=south)", mine);
    uint16 miner;
    // also neutral team exists.. after a big time, the neutral team tries to
    // conquer the mine
    ...
    for (uint16 i = ((mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_1_MIN
                                             : AV_CPLACE_MINE_S_1_MIN);
         i <= ((mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_1_MAX
                                       : AV_CPLACE_MINE_S_1_MAX);
         i++)
        AddAVCreature(miner, i);
    // the next chooses randomly between 2 cretures
    for (uint16 i = ((mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_2_MIN
                                             : AV_CPLACE_MINE_S_2_MIN);
         i <= ((mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_2_MAX
                                       : AV_CPLACE_MINE_S_2_MAX);
         i++)
        AddAVCreature(miner + (urand(1, 2)), i);
    AddAVCreature(miner + 3,
                  (mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_3
                                          : AV_CPLACE_MINE_S_3);
    ...
    if (teamId == TEAM_ALLIANCE || teamId == TEAM_HORDE) {
        ...
        snprintf(buf,
                 sizeof(buf),
                 GetAcoreString(LANG_BG_AV_MINE_TAKEN),
                 (teamId == TEAM_ALLIANCE) ? GetAcoreString(LANG_BG_AV_ALLY)
                                           : GetAcoreString(LANG_BG_AV_HORDE),
                 GetAcoreString((mine == AV_NORTH_MINE)
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    uint32 creatureid;
    if (IsTower(node))
        creatureid = (ownerId == TEAM_ALLIANCE) ? AV_NPC_A_TOWERDEFENSE
                                                : AV_NPC_H_TOWERDEFENSE;
    else {
        if (m_Team_QuestStatus[ownerId][0] < 500)
            creatureid = (ownerId == TEAM_ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE0
                                                    : AV_NPC_H_GRAVEDEFENSE0;
        else if (m_Team_QuestStatus[ownerId][0] < 1000)
            creatureid = (ownerId == TEAM_ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE1
                                                    : AV_NPC_H_GRAVEDEFENSE1;
        else if (m_Team_QuestStatus[ownerId][0] < 1500)
            creatureid = (ownerId == TEAM_ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE2
                                                    : AV_NPC_H_GRAVEDEFENSE2;
        else
            creatureid = (ownerId == TEAM_ALLIANCE) ? AV_NPC_A_GRAVEDEFENSE3
                                                    : AV_NPC_H_GRAVEDEFENSE3;
        // spiritguide
        if (BgCreatures[node])
            DelCreature(node);
        if (!AddSpiritGuide(node,
                            BG_AV_CreaturePos[node][0],
                            BG_AV_CreaturePos[node][1],
                            BG_AV_CreaturePos[node][2],
                            BG_AV_CreaturePos[node][3],
                            ownerId))
            LOG_ERROR("bg.battleground",
                      "AV: couldn't spawn spiritguide at node {}",
                      node);
    }
    for (uint8 i = 0; i < 4; i++)
    ...
    if (node >= BG_AV_NODES_MAX) // fail safe
        return;
    Creature* trigger = GetBgMap()->GetCreature(
        BgCreatures[node + 302]); // 0-302 other creatures
    if (!trigger) {
    ...
    // aura should only apply to players who have accupied the node, set correct
    // faction for trigger
    if (trigger) {
        if (ownerId != TEAM_ALLIANCE &&
            ownerId != TEAM_HORDE) // node can be neutral, remove trigger
        {
            DelCreature(node + 302);
            return;
        }
        trigger->SetFaction(ownerId == TEAM_ALLIANCE ? FACTION_ALLIANCE_GENERIC
...
void BattlegroundAV::DePopulateNode(BG_AV_Nodes node, bool ignoreSpiritGuide)
{
    ...
    if (node < BG_AV_NODES_MAX) // fail safe
        DelCreature(
            node +
            302); // nullptr checks are in DelCreature! 0-302 spirit guides
}
...
uint32 BattlegroundAV::GetObjectThroughNode(BG_AV_Nodes node)
{
    ...
    if (m_Nodes[node].OwnerId == TEAM_ALLIANCE) {
        if (m_Nodes[node].State == POINT_ASSAULTED) {
            if (node <= BG_AV_NODES_FROSTWOLF_HUT)
                return node + 11;
            if (node >= BG_AV_NODES_ICEBLOOD_TOWER &&
                node <= BG_AV_NODES_FROSTWOLF_WTOWER)
                return node + 7;
        }
    ...
    LOG_ERROR("bg.battleground",
              "BattlegroundAV: Error! GetPlaceNode couldn't resolve node {}",
              node);
    ABORT();
    return 0;
}
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    if (m_Nodes[node].PrevOwnerId != teamId) {
        LOG_ERROR(
            "bg.battleground",
            "BG_AV: player defends point which doesn't belong to his team {}",
            node);
        return;
    }
    ...
    if (m_Nodes[node].OwnerId == TEAM_ALLIANCE)
        ...
        SpawnBGObject(object - 22, RESPAWN_IMMEDIATELY); // spawn alliance
                                                         // banner
    ...
    if (IsTower(node)) {
        ...
        SpawnBGObject(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH +
                      ...
                      (teamId == TEAM_ALLIANCE) ? RESPAWN_IMMEDIATELY
                                                : RESPAWN_ONE_DAY);
        SpawnBGObject(BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH +
                          (2 * (node - BG_AV_NODES_DUNBALDAR_SOUTH)),
                      (teamId == TEAM_HORDE) ? RESPAWN_IMMEDIATELY
                                             : RESPAWN_ONE_DAY);
        SpawnBGObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH +
                          (2 * (node - BG_AV_NODES_DUNBALDAR_SOUTH)),
                      (teamId == TEAM_ALLIANCE) ? RESPAWN_IMMEDIATELY
                                                : RESPAWN_ONE_DAY);
    ...
    else if (node == BG_AV_NODES_SNOWFALL_GRAVE) // snowfall eyecandy
    {
        for (uint8 i = 0; i < 4; i++) {
            SpawnBGObject(((ownerId == TEAM_ALLIANCE)
                              ...
                              i,
                          RESPAWN_ONE_DAY);
            SpawnBGObject(((teamId == TEAM_ALLIANCE)
                               ? BG_AV_OBJECT_SNOW_EYECANDY_A
                               : BG_AV_OBJECT_SNOW_EYECANDY_H) +
                              i,
                          RESPAWN_IMMEDIATELY);
    ...
    snprintf(buf,
             sizeof(buf),
             GetAcoreString((IsTower(node)) ? LANG_BG_AV_TOWER_DEFENDED
    ...
    UpdatePlayerScore(player,
                      (IsTower(node)) ? SCORE_TOWERS_DEFENDED
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    if (node == BG_AV_NODES_SNOWFALL_GRAVE) // snowfall is a bit special in
                                            // capping + it gets eyecandy stuff
    {
        if (object == BG_AV_OBJECT_FLAG_N_SNOWFALL_GRAVE) // initial capping
        {
            if (!(prevOwnerId == TEAM_NEUTRAL &&
                  m_Nodes[node].TotalOwnerId == TEAM_NEUTRAL))
                return;

            if (teamId == TEAM_ALLIANCE)
                SpawnBGObject(BG_AV_OBJECT_FLAG_C_A_SNOWFALL_GRAVE,
                              RESPAWN_IMMEDIATELY);
            else
                SpawnBGObject(BG_AV_OBJECT_FLAG_C_H_SNOWFALL_GRAVE,
                              RESPAWN_IMMEDIATELY);
            SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                          RESPAWN_IMMEDIATELY); // neutral aura spawn
        }
        else if (m_Nodes[node].TotalOwnerId ==
                 TEAM_NEUTRAL) // recapping, when no team owns this node realy
        {
        ...
        if (teamId == TEAM_ALLIANCE) {
            despawn = (m_Nodes[node].State == POINT_ASSAULTED)
                          ? BG_AV_OBJECT_SNOW_EYECANDY_PH
                          : BG_AV_OBJECT_SNOW_EYECANDY_H;
    ...
    // xinef: moved here, assure that no call to m_Nodes is used in IF statement
    // bellow as it is modified
    ...
    if (m_Nodes[node].TotalOwnerId != TEAM_NEUTRAL) {
        ...
        if (IsTower(node)) {
            ...
            SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                          RESPAWN_IMMEDIATELY); // neutral aura spawn
            SpawnBGObject(
                static_cast<uint8>(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION) +
                    prevOwnerId + 3 * node,
                RESPAWN_ONE_DAY); // teeamaura despawn
    ...
    snprintf(buf,
             sizeof(buf),
             (IsTower(node)) ? GetAcoreString(LANG_BG_AV_TOWER_ASSAULTED)
                             : GetAcoreString(LANG_BG_AV_GRAVE_ASSAULTED),
             GetNodeName(node),
             (teamId == TEAM_ALLIANCE) ? GetAcoreString(LANG_BG_AV_ALLY)
                                       : GetAcoreString(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    ...
    UpdatePlayerScore(player,
                      (IsTower(node)) ? SCORE_TOWERS_ASSAULTED
                                      : SCORE_GRAVEYARDS_ASSAULTED,
                      1);
    PlaySoundToAll((teamId == TEAM_ALLIANCE) ? AV_SOUND_ALLIANCE_ASSAULTS
                                             : AV_SOUND_HORDE_ASSAULTS);

    player->KilledMonsterCredit((IsTower(node)) ? BG_AV_QUEST_CREDIT_TOWER
...
void BattlegroundAV::FillInitialWorldStates(WorldPacket& data)
{
    bool stateok;
    // graveyards
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT;
         i++) {
        for (uint8 j = 1; j <= 3; j += 2) {
            // j=1=assaulted j=3=controled
            stateok = (m_Nodes[i].State == j);
            data << uint32(BG_AV_NodeWorldStates[i][GetWorldStateType(
                        j, TEAM_ALLIANCE)])
                 << uint32((m_Nodes[i].OwnerId == TEAM_ALLIANCE && stateok)
                               ? 1
                               : 0);
            data << uint32(BG_AV_NodeWorldStates[i][GetWorldStateType(
                        j, TEAM_HORDE)])
                 << uint32((m_Nodes[i].OwnerId == TEAM_HORDE && stateok) ? 1
                                                                         : 0);
        }
    }
    ...
    for (uint8 i = BG_AV_NODES_DUNBALDAR_SOUTH; i < BG_AV_NODES_MAX; ++i)
        for (uint8 j = 1; j <= 3; j += 2) {
            // j=1=assaulted j=3=controled //i dont have j=2=destroyed cause
            // destroyed is the same like enemy-team controll
            stateok = (m_Nodes[i].State == j ||
                       (m_Nodes[i].State == POINT_DESTROYED && j == 3));
            data << uint32(BG_AV_NodeWorldStates[i][GetWorldStateType(
                        j, TEAM_ALLIANCE)])
                 << uint32((m_Nodes[i].OwnerId == TEAM_ALLIANCE && stateok)
                               ? 1
    ...
    if (m_Nodes[BG_AV_NODES_SNOWFALL_GRAVE].OwnerId ==
        TEAM_NEUTRAL) // cause neutral teams aren't handled generic
    ...
    if (GetStatus() ==
        STATUS_IN_PROGRESS) // only if game started the teamscores are displayed
    {
...
uint8 BattlegroundAV::GetWorldStateType(
    uint8 state,
    TeamId teamId) // this is used for node worldstates and returns values which
                   // fit good into the worldstatesarray
{
    ...
    LOG_ERROR("bg.battleground",
              "BG_AV: should update a strange worldstate state:{} team:{}",
              state,
              teamId);
    return 5; // this will crash the game, but i want to know if something is
              // wrong here
}
...
void BattlegroundAV::UpdateNodeWorldState(BG_AV_Nodes node)
{
    UpdateWorldState(BG_AV_NodeWorldStates[node][GetWorldStateType(
                         m_Nodes[node].State, m_Nodes[node].OwnerId)],
                     1);
    if (m_Nodes[node].PrevOwnerId ==
        TEAM_NEUTRAL) // currently only snowfall is supported as neutral node (i
                      // don't want to make an extra row (neutral states) in
                      // worldstatesarray just for one node
        UpdateWorldState(AV_SNOWFALL_N, 0);
    else
        UpdateWorldState(
            BG_AV_NodeWorldStates[node][GetWorldStateType(
                m_Nodes[node].PrevState, m_Nodes[node].PrevOwnerId)],
            0);
}
...
void BattlegroundAV::SendMineWorldStates(uint32 mine)
{
    ...
    for (uint8 i = 0; i < 3; ++i)
        UpdateWorldState(
            BG_AV_MineWorldStates[mine][i],
            0); // Xinef: Clear data for consistency and buglessness
...
GraveyardStruct const* BattlegroundAV::GetClosestGraveyard(Player* player)
{
    ...
    GraveyardStruct const* pGraveyard =
        sGraveyard->GetGraveyard(BG_AV_GraveyardIds[player->GetTeamId() + 7]);
    minDist = (pGraveyard->x - x) * (pGraveyard->x - x) +
              (pGraveyard->y - y) * (pGraveyard->y - y);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i) {
        if (i <= BG_AV_NODES_FROSTWOLF_HUT) {
            if (!AddObject(i,
                           ...
                           0,
                           0,
                           std::sin(BG_AV_ObjectPos[i][3] / 2),
                           cos(BG_AV_ObjectPos[i][3] / 2),
                           RESPAWN_ONE_DAY) ||
                !AddObject(i + 11,
                ...
                !AddObject(i + 22,
                           ...
                           RESPAWN_ONE_DAY)
                // aura
                || !AddObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + i * 3,
                              ...
                              cos(BG_AV_ObjectPos[i][3] / 2),
                              RESPAWN_ONE_DAY) ||
                !AddObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + i * 3,
                           ...
                           cos(BG_AV_ObjectPos[i][3] / 2),
                           RESPAWN_ONE_DAY) ||
                !AddObject(BG_AV_OBJECT_AURA_H_FIRSTAID_STATION + i * 3,
                           ...
                           RESPAWN_ONE_DAY)) {
                LOG_ERROR("bg.battleground",
                          "BatteGroundAV: Failed to spawn some object "
                          "Battleground not created!2");
                return false;
            }
        }
        else // towers
            ...
            if (i <= BG_AV_NODES_STONEHEART_BUNKER) // alliance towers
            {
                if (!AddObject(i,
                    ...
                    !AddObject(i + 22,
                               ...
                               cos(BG_AV_ObjectPos[i][3] / 2),
                               RESPAWN_ONE_DAY) ||
                    !AddObject(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH +
                                   (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                               BG_AV_OBJECTID_AURA_A,
                               BG_AV_ObjectPos[i + 8][0],
                               BG_AV_ObjectPos[i + 8][1],
                               BG_AV_ObjectPos[i + 8][2],
                               BG_AV_ObjectPos[i + 8][3],
                               0,
                               0,
                               std::sin(BG_AV_ObjectPos[i + 8][3] / 2),
                               cos(BG_AV_ObjectPos[i + 8][3] / 2),
                               RESPAWN_ONE_DAY) ||
                    !AddObject(BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH +
                                   (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                               BG_AV_OBJECTID_AURA_N,
                               BG_AV_ObjectPos[i + 8][0],
                               BG_AV_ObjectPos[i + 8][1],
                               ...
                               cos(BG_AV_ObjectPos[i + 8][3] / 2),
                               RESPAWN_ONE_DAY) ||
                    !AddObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH +
                                   (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                               ...
                               cos(BG_AV_ObjectPos[i + 8][3] / 2),
                               RESPAWN_ONE_DAY) ||
                    !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH +
                                   (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
            ...
            for (uint8 j = 0; j <= 9; j++) // burning aura
            {
                if (!AddObject(
                        BG_AV_OBJECT_BURN_DUNBALDAR_SOUTH +
                            ((i - BG_AV_NODES_DUNBALDAR_SOUTH) * 10) + j,
                        BG_AV_OBJECTID_FIRE,
                        BG_AV_ObjectPos
                            [AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                             ((i - BG_AV_NODES_DUNBALDAR_SOUTH) * 10) + j][0],
                        BG_AV_ObjectPos
                            [AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                             ((i - BG_AV_NODES_DUNBALDAR_SOUTH) * 10) + j][1],
                        BG_AV_ObjectPos
                            [AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                             ((i - BG_AV_NODES_DUNBALDAR_SOUTH) * 10) + j][2],
                        BG_AV_ObjectPos
                            [AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                             ((i - BG_AV_NODES_DUNBALDAR_SOUTH) * 10) + j][3],
                        0,
                        0,
                        std::sin(
                            BG_AV_ObjectPos[AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                                            ((i - BG_AV_NODES_DUNBALDAR_SOUTH) *
                                             10) +
                                            j][3] /
                            2),
                        cos(BG_AV_ObjectPos[AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                                            ((i - BG_AV_NODES_DUNBALDAR_SOUTH) *
                                             10) +
                                            j][3] /
                            2),
    ...
    for (uint8 i = 0; i < 2; i++) // burning aura for buildings
    {
        for (uint8 j = 0; j <= 9; j++) {
            if (j < 5) {
                if (!AddObject(
                        BG_AV_OBJECT_BURN_BUILDING_ALLIANCE + (i * 10) + j,
                        BG_AV_OBJECTID_SMOKE,
                        BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) +
                                        j][0],
                        BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) +
                                        j][1],
                        BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) +
                        ...
                        0,
                        0,
                        std::sin(BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A +
                                                 (i * 10) + j][3] /
                                 2),
                        cos(BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A +
                                            (i * 10) + j][3] /
                            2),
                        RESPAWN_ONE_DAY)) {
                    LOG_ERROR("bg.battleground",
                              "BatteGroundAV: Failed to spawn some object "
                              "Battleground not created!6.{}",
                              i);
                    return false;
                }
            }
    ...
    for (uint16 i = 0;
         i <= (BG_AV_OBJECT_MINE_SUPPLY_N_MAX - BG_AV_OBJECT_MINE_SUPPLY_N_MIN);
         i++) {
        if (!AddObject(
                BG_AV_OBJECT_MINE_SUPPLY_N_MIN + i,
                ...
                0,
                0,
                std::sin(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i][3] /
                         2),
                cos(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i][3] / 2),
                RESPAWN_ONE_DAY)) {
            LOG_ERROR("bg.battleground",
                      "BatteGroundAV: Failed to spawn some mine supplies "
                      "Battleground not created!7.5.{}",
                      i);
            return false;
        }
    }
    for (uint16 i = 0;
         i <= (BG_AV_OBJECT_MINE_SUPPLY_S_MAX - BG_AV_OBJECT_MINE_SUPPLY_S_MIN);
         i++) {
        if (!AddObject(
                BG_AV_OBJECT_MINE_SUPPLY_S_MIN + i,
                ...
                0,
                0,
                std::sin(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i][3] /
                         2),
                cos(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i][3] / 2),
                RESPAWN_ONE_DAY)) {
            LOG_ERROR("bg.battleground",
                      "BatteGroundAV: Failed to spawn some mine supplies "
                      "Battleground not created!7.6.{}",
                      i);
            return false;
        }
    }
    ...
    if (!AddObject(BG_AV_OBJECT_FLAG_N_SNOWFALL_GRAVE,
                   ...
                   cos(BG_AV_ObjectPos[BG_AV_NODES_SNOWFALL_GRAVE][3] / 2),
                   RESPAWN_ONE_DAY)) {
        LOG_ERROR("bg.battleground",
                  "BatteGroundAV: Failed to spawn some object Battleground not "
                  "created!8");
        return false;
    }
    for (uint8 i = 0; i < 4; i++) {
        if (!AddObject(BG_AV_OBJECT_SNOW_EYECANDY_A + i,
                       BG_AV_OBJECTID_SNOWFALL_CANDY_A,
                       BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][0],
                       BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][1],
                       BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][2],
                       BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3],
                       0,
                       0,
                       std::sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3] / 2),
                       cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3] / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_SNOW_EYECANDY_PA + i,
                       ...
                       cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3] / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_SNOW_EYECANDY_H + i,
                       ...
                       cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3] / 2),
                       RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_SNOW_EYECANDY_PH + i,
                       ...
                       RESPAWN_ONE_DAY)) {
            LOG_ERROR("bg.battleground",
                      "BatteGroundAV: Failed to spawn some object Battleground "
                      "not created!9.{}",
                      i);
            return false;
        }
    ...
    if (IsHolidayActive(HOLIDAY_FEAST_OF_WINTER_VEIL))
        for (uint16 i = 0; i <= (BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MAX -
                                 BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MIN);
             i++) {
            if (!AddObject(
                    BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MIN + i,
                    BG_AV_OBJECTID_HARDPACKED_SNOWDRIFT,
                    BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][0],
                    BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][1],
                    BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][2],
                    BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][3],
                    0,
                    0,
                    std::sin(
                        BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i]
                                       [3] /
                        2),
                    cos(BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i]
                                       [3] /
                        2),
                    RESPAWN_ONE_DAY))
                return false;
    ...
    // Quest banners
    if (!AddObject(BG_AV_OBJECT_FROSTWOLF_BANNER,
                   BG_AV_OBJECTID_FROSTWOLF_BANNER,
                   BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][0],
                   BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][1],
                   BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][2],
                   BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][3],
                   0,
                   0,
                   std::sin(BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][3] / 2),
                   cos(BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][3] / 2),
                   RESPAWN_ONE_DAY)) {
        LOG_ERROR("bg.battleground",
                  "BatteGroundAV: Failed to spawn some object Battleground not "
                  "created!8");
    ...
    if (!AddObject(BG_AV_OBJECT_STORMPIKE_BANNER,
                   ...
                   BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][0],
                   BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][1],
                   BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][2],
                   BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][3],
                   0,
                   0,
                   std::sin(BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][3] / 2),
                   cos(BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][3] / 2),
                   RESPAWN_ONE_DAY)) {
        LOG_ERROR("bg.battleground",
                  "BatteGroundAV: Failed to spawn some object Battleground not "
                  "created!8");
    ...
    for (i = BG_AV_OBJECT_FLAG_A_FIRSTAID_STATION;
         i <= BG_AV_OBJECT_FLAG_A_STONEHEART_GRAVE;
         i++) {
        SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + 3 * i,
                      RESPAWN_IMMEDIATELY);
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);
    }

    for (i = BG_AV_OBJECT_FLAG_A_DUNBALDAR_SOUTH;
         i <= BG_AV_OBJECT_FLAG_A_STONEHEART_BUNKER;
         i++)
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);

    for (i = BG_AV_OBJECT_FLAG_H_ICEBLOOD_GRAVE;
         i <= BG_AV_OBJECT_FLAG_H_FROSTWOLF_WTOWER;
         i++) {
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);
        if (i <= BG_AV_OBJECT_FLAG_H_FROSTWOLF_HUT)
            SpawnBGObject(BG_AV_OBJECT_AURA_H_FIRSTAID_STATION +
                              3 * GetNodeThroughObject(i),
                          RESPAWN_IMMEDIATELY);
    }

    for (i = BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH;
         i <= BG_AV_OBJECT_TFLAG_A_STONEHEART_BUNKER;
         i += 2) {
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);      // flag
        SpawnBGObject(i + 16, RESPAWN_IMMEDIATELY); // aura
    }

    for (i = BG_AV_OBJECT_TFLAG_H_ICEBLOOD_TOWER;
         i <= BG_AV_OBJECT_TFLAG_H_FROSTWOLF_WTOWER;
         i += 2) {
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);      // flag
        SpawnBGObject(i + 16, RESPAWN_IMMEDIATELY); // aura
    }
    ...
    // Handpacked snowdrift, only during holiday
    if (IsHolidayActive(HOLIDAY_FEAST_OF_WINTER_VEIL))
        for (i = BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MIN;
             i <= BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MAX;
             i++)
            SpawnBGObject(i, RESPAWN_IMMEDIATELY);
    ...
    AddSpiritGuide(7,
                   BG_AV_CreaturePos[7][0],
                   BG_AV_CreaturePos[7][1],
                   BG_AV_CreaturePos[7][2],
                   BG_AV_CreaturePos[7][3],
                   TEAM_ALLIANCE);
    AddSpiritGuide(8,
                   BG_AV_CreaturePos[8][0],
                   BG_AV_CreaturePos[8][1],
                   BG_AV_CreaturePos[8][2],
                   BG_AV_CreaturePos[8][3],
                   TEAM_HORDE);
    ...
    for (i = AV_NPC_A_MARSHAL_SOUTH; i <= AV_NPC_H_MARSHAL_WTOWER; i++)
        AddAVCreature(i,
                      AV_CPLACE_A_MARSHAL_SOUTH + (i - AV_NPC_A_MARSHAL_SOUTH));
    AddAVCreature(AV_NPC_HERALD, AV_CPLACE_HERALD);

    if (
        // alliance gates
        !AddObject(BG_AV_OBJECT_DOOR_A,
                   BG_AV_OBJECTID_GATE_A,
                   BG_AV_DoorPositons[0][0],
                   BG_AV_DoorPositons[0][1],
                   BG_AV_DoorPositons[0][2],
                   BG_AV_DoorPositons[0][3],
                   0,
                   0,
                   std::sin(BG_AV_DoorPositons[0][3] / 2),
                   cos(BG_AV_DoorPositons[0][3] / 2),
                   RESPAWN_IMMEDIATELY)
        // horde gates
        || !AddObject(BG_AV_OBJECT_DOOR_H,
                      BG_AV_OBJECTID_GATE_H,
                      BG_AV_DoorPositons[1][0],
                      BG_AV_DoorPositons[1][1],
                      BG_AV_DoorPositons[1][2],
                      BG_AV_DoorPositons[1][3],
                      0,
                      0,
                      std::sin(BG_AV_DoorPositons[1][3] / 2),
                      cos(BG_AV_DoorPositons[1][3] / 2),
                      RESPAWN_IMMEDIATELY)) {
        LOG_ERROR("sql.sql",
                  "BatteGroundAV: Failed to spawn some object Battleground not "
                  "created!1");
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, TeamId teamId)
{
    ...
    if (m_Nodes[node].State == POINT_ASSAULTED &&
        m_Nodes[node].TotalOwnerId !=
            TEAM_NEUTRAL) // only assault an assaulted node if no totalowner
                          // exists
    {
        LOG_FATAL("bg.battleground",
                  "Assault on an not assaulted node with total owner");
        ABORT();
    }
    // the timer gets another time, if the previous owner was 0 == Neutral
    m_Nodes[node].Timer       = (m_Nodes[node].PrevOwnerId != TEAM_NEUTRAL)
                                    ? BG_AV_CAPTIME
...
void BattlegroundAV::DestroyNode(BG_AV_Nodes node)
{
    ...
    m_Nodes[node].PrevState    = m_Nodes[node].State;
    m_Nodes[node].State =
        (m_Nodes[node].Tower) ? POINT_DESTROYED : POINT_CONTROLED;
...
void BattlegroundAV::DefendNode(BG_AV_Nodes node, TeamId teamId)
{
    ...
    ASSERT(m_Nodes[node].State != POINT_CONTROLED &&
           m_Nodes[node].State != POINT_DESTROYED);
    m_Nodes[node].PrevOwnerId = m_Nodes[node].OwnerId;
    m_Nodes[node].OwnerId     = teamId;
    m_Nodes[node].PrevState   = m_Nodes[node].State;
    m_Nodes[node].State       = POINT_CONTROLED;
...
{
    for (uint8 i = 0; i < 2;
         i++) // forloop for both teams (it just make 0 == alliance and 1 ==
              // horde also for both mines 0=north 1=south
    {
        for (uint8 j = 0; j < 9; j++)
            m_Team_QuestStatus[i][j] = 0;
        ...
        m_CaptainBuffTimer[i] =
            120000 +
            urand(0, 4) * 60; // as far as i could see, the buff is randomly so
                              // i make 2minutes (thats the duration of the buff
                              // itself) + 0-4minutes TODO get the right times
        m_Mine_Owner[i] = TEAM_NEUTRAL;
    }
    for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION;
         i <= BG_AV_NODES_STONEHEART_GRAVE;
         ++i) // alliance graves
        InitNode(i, TEAM_ALLIANCE, false);
    for (BG_AV_Nodes i = BG_AV_NODES_DUNBALDAR_SOUTH;
         i <= BG_AV_NODES_STONEHEART_BUNKER;
         ++i) // alliance towers
    ...
    InitNode(BG_AV_NODES_SNOWFALL_GRAVE,
             TEAM_NEUTRAL,
             false); // give snowfall neutral owner

    m_Mine_Timer = AV_MINE_TICK_TIMER;
    for (uint16 i = 0;
         i < static_cast<uint16>(AV_CPLACE_MAX) + AV_STATICCPLACE_MAX;
         i++)
        if (BgCreatures[i])
            DelCreature(i);
}
...
TeamId BattlegroundAV::GetPrematureWinner()
{
    ...
    return GetTeamScore(TEAM_HORDE) > GetTeamScore(TEAM_ALLIANCE)
               ? TEAM_HORDE
               : Battleground::GetPrematureWinner();
}
