    ...
    if (GetStatus() == STATUS_IN_PROGRESS) {
        ...
        case BG_WS_EVENT_UPDATE_GAME_TIME:
            ...
            _bgEvents.ScheduleEvent(BG_WS_EVENT_UPDATE_GAME_TIME,
                                    ((BG_WS_TOTAL_GAME_TIME - GetStartTime()) %
        ...
        case BG_WS_EVENT_NO_TIME_LEFT:
            if (GetTeamScore(TEAM_ALLIANCE) == GetTeamScore(TEAM_HORDE))
                ...
                EndBattleground(GetTeamScore(TEAM_HORDE) >
                                        GetTeamScore(TEAM_ALLIANCE)
                                    ? TEAM_HORDE
                                    : TEAM_ALLIANCE);
            break;
        case BG_WS_EVENT_RESPAWN_BOTH_FLAGS:
            ...
            SendBroadcastText(BG_WS_TEXT_FLAGS_PLACED,
        ...
        case BG_WS_EVENT_BOTH_FLAGS_KEPT10:
            if (Player* player = ObjectAccessor::GetPlayer(
                    FindBgMap(), GetFlagPickerGUID(TEAM_ALLIANCE)))
                player->CastSpell(player, BG_WS_SPELL_FOCUSED_ASSAULT, true);
            if (Player* player = ObjectAccessor::GetPlayer(
                    FindBgMap(), GetFlagPickerGUID(TEAM_HORDE)))
                player->CastSpell(player, BG_WS_SPELL_FOCUSED_ASSAULT, true);
            break;
...
void BattlegroundWS::StartingEventOpenDoors()
{
    ...
    _bgEvents.ScheduleEvent(BG_WS_EVENT_NO_TIME_LEFT,
                            BG_WS_TOTAL_GAME_TIME -
                                2 * MINUTE *
                                    IN_MILLISECONDS); // 27 - 2 = 25 minutes
...
void BattlegroundWS::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(player->GetGUID().GetCounter(),
                         new BattlegroundWGScore(player->GetGUID()));
}

void BattlegroundWS::RespawnFlagAfterDrop(TeamId teamId)
{
    if (GetStatus() != STATUS_IN_PROGRESS ||
        GetFlagState(teamId) != BG_WS_FLAG_STATE_ON_GROUND)
        return;

    UpdateFlagState(teamId, BG_WS_FLAG_STATE_ON_BASE);
    SpawnBGObject(teamId == TEAM_ALLIANCE ? BG_WS_OBJECT_A_FLAG
                                          : BG_WS_OBJECT_H_FLAG,
                  RESPAWN_IMMEDIATELY);
    ...
    if (GameObject* flag =
            GetBgMap()->GetGameObject(GetDroppedFlagGUID(teamId)))
        flag->Delete();
...
{
    if (GetStatus() != STATUS_IN_PROGRESS ||
        GetFlagState(teamId) != BG_WS_FLAG_STATE_ON_PLAYER) {
        return;
    }

    uint32             triggerId   = teamId == TEAM_ALLIANCE
    ...
    if (Player* player =
            ObjectAccessor::GetPlayer(FindBgMap(), GetFlagPickerGUID(teamId))) {
...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    UpdateFlagState(GetOtherTeamId(player->GetTeamId()),
                    BG_WS_FLAG_STATE_ON_BASE);

    if (player->GetTeamId() == TEAM_ALLIANCE) {
        player->RemoveAurasDueToSpell(BG_WS_SPELL_WARSONG_FLAG);
        PlaySoundToAll(BG_WS_SOUND_FLAG_CAPTURED_ALLIANCE);
        SendBroadcastText(BG_WS_TEXT_CAPTURED_HORDE_FLAG,
        ...
        SendBroadcastText(BG_WS_TEXT_CAPTURED_ALLIANCE_FLAG,
    ...
    if (GetTeamScore(TEAM_ALLIANCE) == BG_WS_MAX_TEAM_SCORE ||
        GetTeamScore(TEAM_HORDE) == BG_WS_MAX_TEAM_SCORE) {
        UpdateWorldState(BG_WS_STATE_TIMER_ACTIVE, 0);
        EndBattleground(GetTeamScore(TEAM_HORDE) == BG_WS_MAX_TEAM_SCORE
                            ? TEAM_HORDE
                            : TEAM_ALLIANCE);
    }
    else
        _bgEvents.ScheduleEvent(BG_WS_EVENT_RESPAWN_BOTH_FLAGS,
...
void BattlegroundWS::EventPlayerDroppedFlag(Player* player)
{
    ...
    if (player->GetTeamId() == TEAM_ALLIANCE) {
        ...
        SendBroadcastText(
            BG_WS_TEXT_HORDE_FLAG_DROPPED, CHAT_MSG_BG_SYSTEM_HORDE, player);
        _bgEvents.RescheduleEvent(BG_WS_EVENT_HORDE_DROP_FLAG,
        ...
        SendBroadcastText(BG_WS_TEXT_ALLIANCE_FLAG_DROPPED,
                          CHAT_MSG_BG_SYSTEM_ALLIANCE,
                          player);
        _bgEvents.RescheduleEvent(BG_WS_EVENT_ALLIANCE_DROP_FLAG,
                                  BG_WS_FLAG_DROP_TIME);
    }
}
                                              ...
                                              GameObject* gameObject)
{
    ...
    if (player->GetTeamId() == TEAM_HORDE &&
        GetFlagState(TEAM_ALLIANCE) == BG_WS_FLAG_STATE_ON_BASE &&
        ...
        player->StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET,
                                      BG_WS_SPELL_SILVERWING_FLAG_PICKED);

        PlaySoundToAll(BG_WS_SOUND_ALLIANCE_FLAG_PICKED_UP);
        SendBroadcastText(BG_WS_TEXT_ALLIANCE_FLAG_PICKED_UP,
                          CHAT_MSG_BG_SYSTEM_HORDE,
                          player);

        if (GetFlagState(TEAM_HORDE) != BG_WS_FLAG_STATE_ON_BASE) {
            _bgEvents.RescheduleEvent(BG_WS_EVENT_BOTH_FLAGS_KEPT10,
                                      BG_WS_SPELL_FORCE_TIME);
            _bgEvents.RescheduleEvent(BG_WS_EVENT_BOTH_FLAGS_KEPT15,
    ...
    if (player->GetTeamId() == TEAM_ALLIANCE &&
        GetFlagState(TEAM_HORDE) == BG_WS_FLAG_STATE_ON_BASE &&
        ...
        SendBroadcastText(BG_WS_TEXT_HORDE_FLAG_PICKED_UP,
                          CHAT_MSG_BG_SYSTEM_ALLIANCE,
                          player);
    ...
    if (GetFlagState(TEAM_ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND &&
        ...
        if (player->GetTeamId() == TEAM_ALLIANCE) {
            ...
            SendBroadcastText(BG_WS_TEXT_ALLIANCE_FLAG_RETURNED,
                              CHAT_MSG_BG_SYSTEM_ALLIANCE,
                              player);
            _bgEvents.CancelEvent(BG_WS_EVENT_BOTH_FLAGS_KEPT10);
            _bgEvents.CancelEvent(BG_WS_EVENT_BOTH_FLAGS_KEPT15);
    ...
    if (GetFlagState(TEAM_HORDE) == BG_WS_FLAG_STATE_ON_GROUND &&
        player->IsWithinDistInMap(gameObject, 10.0f) &&
        ...
        if (player->GetTeamId() == TEAM_HORDE) {
            ...
            PlaySoundToAll(BG_WS_SOUND_FLAG_RETURNED);
            SendBroadcastText(BG_WS_TEXT_HORDE_FLAG_RETURNED,
                              CHAT_MSG_BG_SYSTEM_HORDE,
                              player);
            _bgEvents.CancelEvent(BG_WS_EVENT_BOTH_FLAGS_KEPT10);
            _bgEvents.CancelEvent(BG_WS_EVENT_BOTH_FLAGS_KEPT15);
...
void BattlegroundWS::UpdateFlagState(TeamId teamId, uint32 value)
{
    ...
    UpdateWorldState(teamId == TEAM_ALLIANCE ? BG_WS_FLAG_STATE_HORDE
...
void BattlegroundWS::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case 4628:                                      // Not used
    case 4629:                                      // Not used
    case BG_WS_TRIGGER_ALLIANCE_ELIXIR_SPEED_SPAWN: // Alliance elixir of speed
                                                    // spawn
    case BG_WS_TRIGGER_HORDE_ELIXIR_SPEED_SPAWN: // Horde elixir of speed spawn
    case BG_WS_TRIGGER_ALLIANCE_ELIXIR_REGEN_SPAWN: // Alliance elixir of
                                                    // regeneration spawn
    case BG_WS_TRIGGER_HORDE_ELIXIR_REGEN_SPAWN: // Horde elixir of regeneration
                                                 // spawn
    case BG_WS_TRIGGER_ALLIANCE_ELIXIR_BERSERK_SPAWN: // Alliance elixir of
                                                      // berserk spawn
    case BG_WS_TRIGGER_HORDE_ELIXIR_BERSERK_SPAWN:    // Horde elixir of berserk
                                                      // spawn
        break;
    }
}
...
bool BattlegroundWS::SetupBattleground()
{
    ...
    GraveyardStruct const* sg =
        sGraveyard->GetGraveyard(WS_GRAVEYARD_MAIN_ALLIANCE);
    AddSpiritGuide(
        WS_SPIRIT_MAIN_ALLIANCE, sg->x, sg->y, sg->z, 3.124139f, TEAM_ALLIANCE);

    sg = sGraveyard->GetGraveyard(WS_GRAVEYARD_MAIN_HORDE);
    AddSpiritGuide(
        WS_SPIRIT_MAIN_HORDE, sg->x, sg->y, sg->z, 3.193953f, TEAM_HORDE);

    for (uint32 i = BG_WS_OBJECT_DOOR_A_1; i < BG_WS_OBJECT_MAX; ++i)
        if (!BgObjects[i]) {
            LOG_ERROR("sql.sql",
                      "BatteGroundWS: Failed to spawn some object Battleground "
                      "not created!");
            return false;
        }
    ...
    for (uint32 i = WS_SPIRIT_MAIN_ALLIANCE; i < BG_CREATURES_MAX_WS; ++i)
        if (!BgCreatures[i]) {
            LOG_ERROR("sql.sql",
                      "BatteGroundWS: Failed to spawn spirit guides "
...
{
    data << uint32(BG_WS_FLAG_CAPTURES_ALLIANCE)
         << uint32(GetTeamScore(TEAM_ALLIANCE));
    data << uint32(BG_WS_FLAG_CAPTURES_HORDE)
         << uint32(GetTeamScore(TEAM_HORDE));
    data << uint32(BG_WS_FLAG_CAPTURES_MAX) << uint32(BG_WS_MAX_TEAM_SCORE);

    data << uint32(BG_WS_STATE_TIMER_ACTIVE)
         << uint32(GetStatus() == STATUS_IN_PROGRESS);
    ...
    data << uint32(BG_WS_FLAG_STATE_ALLIANCE)
         << uint32(GetFlagState(TEAM_ALLIANCE));
}
...
{
    if ((!GetFlagPickerGUID(TEAM_ALLIANCE) &&
         GetFlagState(TEAM_ALLIANCE) != BG_WS_FLAG_STATE_ON_GROUND) ||
        (!GetFlagPickerGUID(TEAM_HORDE) &&
         GetFlagState(TEAM_HORDE) != BG_WS_FLAG_STATE_ON_GROUND) ||
        _bgEvents.GetNextEventTime(BG_WS_EVENT_BOTH_FLAGS_KEPT10) > 0)
        return 0;

    return _bgEvents.GetNextEventTime(BG_WS_EVENT_BOTH_FLAGS_KEPT15) > 0
