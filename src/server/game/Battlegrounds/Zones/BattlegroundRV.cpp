...
static constexpr Milliseconds BG_RV_FIRST_TIMER =
    20500ms; // elevators rise in 20133ms
...
void BattlegroundRV::TeleportUnitToNewZ(Unit* unit, float newZ, bool casting)
{
    ...
    unit->NearTeleportTo(unit->GetPositionX(),
                         unit->GetPositionY(),
                         newZ,
                         unit->GetOrientation(),
                         casting);
...
{
    // get height at current pos, if something is wrong (below or high above) -
    // teleport
    if (!unit->IsFalling() && unit->IsAlive()) {
        float groundZ_vmap = unit->GetMap()->GetHeight(
            unit->GetPositionX(), unit->GetPositionY(), 37.0f, true, 50.0f);
        float groundZ_dyntree =
            unit->GetMap()->GetDynamicMapTree().getHeight(unit->GetPositionX(),
                                                          unit->GetPositionY(),
                                                          37.0f,
                                                          50.0f,
                                                          unit->GetPhaseMask());
        ...
        if ((groundZ_vmap > 28.0f && groundZ_vmap < 29.0f) ||
            ...
            if (unit->GetPositionZ() < groundZ - 0.2f ||
                unit->GetPositionZ() > groundZ + 3.5f)
...
void BattlegroundRV::PostUpdateImpl(uint32 diff)
{
    ...
    if (_timer < Milliseconds(diff)) {
        ...
        case BG_RV_STATE_OPEN_FENCES: {
            for (uint8 i = BG_RV_OBJECT_FIRE_1; i <= BG_RV_OBJECT_FIREDOOR_2;
                 ++i)
                DoorOpen(i);
            ...
            for (auto const& [playerGuid, player] : m_Players) {
                ...
                if (GameObject* gObj = player->GetGameObject(48018)) {
                    gObj->Relocate(
                        gObj->GetPositionX(), gObj->GetPositionY(), 28.28f);
                ...
                for (uint8 i = SUMMON_SLOT_TOTEM; i < MAX_TOTEM_SLOT; ++i)
                    if (player->m_SummonSlot[i])
                        if (Creature* totem = GetBgMap()->GetCreature(
                                player->m_SummonSlot[i]))
            ...
            {
                uint32 objects[2] = {BG_RV_OBJECT_ELEVATOR_1,
        ...
        case BG_RV_STATE_CLOSE_FIRE:
            for (uint8 i = BG_RV_OBJECT_FIRE_1; i <= BG_RV_OBJECT_FIREDOOR_2;
                 ++i)
                DoorClose(i);
            // Fire got closed after five seconds, leaves twenty seconds before
            // toggling pillars
...
void BattlegroundRV::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case 5224:
        player->NearTeleportTo(
            765.0f, -294.0f, 28.3f, player->GetOrientation());
        break;
    case 5226:
        player->NearTeleportTo(
            765.0f, -272.0f, 28.3f, player->GetOrientation());
        break;
    case 5447:
        player->NearTeleportTo(763.5f, -284, 28.276f, 2.422f);
        break;
...
{
    GameObject* test =
        GetBgMap()->GetGameObject(BgObjects[BG_RV_OBJECT_PILAR_1]);
    if (!test)
        return;
