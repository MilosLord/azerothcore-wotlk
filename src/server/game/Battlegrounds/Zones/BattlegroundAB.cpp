...
BattlegroundAB::BattlegroundAB()
{
    ...
    BgCreatures.resize(
        ...
        BG_AB_DYNAMIC_NODES_COUNT); // xinef: +BG_AB_DYNAMIC_NODES_COUNT buff
                                    // triggers
    ...
    if (GetStatus() == STATUS_IN_PROGRESS) {
        ...
        while (uint32 eventId = _bgEvents.ExecuteEvent())
            ...
            case BG_AB_EVENT_CAPTURE_GOLDMINE: {
                ...
                TeamId teamId = _capturePointInfo[node]._state ==
                                        BG_AB_NODE_STATE_ALLY_CONTESTED
                                    ? TEAM_ALLIANCE
                                    : TEAM_HORDE;
                DeleteBanner(node);
                _capturePointInfo[node]._ownerTeamId = teamId;
                _capturePointInfo[node]._state =
                    teamId == TEAM_ALLIANCE ? BG_AB_NODE_STATE_ALLY_OCCUPIED
                ...
                if (teamId == TEAM_ALLIANCE) {
                    SendBroadcastText(ABNodes[node].TextAllianceTaken,
                                      CHAT_MSG_BG_SYSTEM_ALLIANCE);
                    PlaySoundToAll(BG_AB_SOUND_NODE_CAPTURED_ALLIANCE);
                }
                else {
                    SendBroadcastText(ABNodes[node].TextHordeTaken,
            ...
            case BG_AB_EVENT_HORDE_TICK: {
                ...
                auto reputationRewards =
                    uint8(m_TeamScores[teamId] / _reputationTics);
                auto information = uint8(m_TeamScores[teamId] /
                ...
                if (reputationRewards <
                    ...
                    RewardReputationToTeam(
                        teamId == TEAM_ALLIANCE ? 509 : 510, 10, teamId);
                if (information < uint8(m_TeamScores[teamId] /
                                        BG_AB_WARNING_NEAR_VICTORY_SCORE)) {
                    if (teamId == TEAM_ALLIANCE) {
                        SendBroadcastText(BG_AB_TEXT_ALLIANCE_NEAR_VICTORY,
                                          CHAT_MSG_BG_SYSTEM_NEUTRAL);
                        PlaySoundToAll(BG_AB_SOUND_NEAR_VICTORY_ALLIANCE);
                    }
                    else {
                        SendBroadcastText(BG_AB_TEXT_HORDE_NEAR_VICTORY,
                                          CHAT_MSG_BG_SYSTEM_NEUTRAL);
                        PlaySoundToAll(BG_AB_SOUND_NEAR_VICTORY_HORDE);
                    }
                }

                UpdateWorldState(teamId == TEAM_ALLIANCE
                                     ? BG_AB_OP_RESOURCES_ALLY
                                     : BG_AB_OP_RESOURCES_HORDE,
                                 m_TeamScores[teamId]);
                if (m_TeamScores[teamId] >
                    m_TeamScores[GetOtherTeamId(teamId)] + 500)
                    _teamScores500Disadvantage[GetOtherTeamId(teamId)] = true;
                if (m_TeamScores[teamId] >= BG_AB_MAX_TEAM_SCORE)
                    EndBattleground(teamId);

                _bgEvents.ScheduleEvent(eventId,
                                        BG_AB_TickIntervals[controlledPoints]);
                break;
            }
            default:
                break;
...
{
    for (uint32 obj = BG_AB_OBJECT_BANNER_NEUTRAL;
         obj <
         static_cast<uint8>(BG_AB_DYNAMIC_NODES_COUNT) * BG_AB_OBJECTS_PER_NODE;
         ++obj)
...
{
    for (uint32 banner = BG_AB_OBJECT_BANNER_NEUTRAL, i = 0;
         i < BG_AB_DYNAMIC_NODES_COUNT;
         banner += BG_AB_OBJECTS_PER_NODE, ++i)
        SpawnBGObject(banner, RESPAWN_IMMEDIATELY);

    for (uint32 i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
        SpawnBGObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + urand(0, 2) + i * 3,
                      RESPAWN_IMMEDIATELY);

    DoorOpen(BG_AB_OBJECT_GATE_A);
    DoorOpen(BG_AB_OBJECT_GATE_H);
    StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_EVENT,
...
void BattlegroundAB::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(player->GetGUID().GetCounter(),
                         new BattlegroundABScore(player->GetGUID()));
}
...
void BattlegroundAB::CreateBanner(uint8 node, bool delay)
{
    ...
    if (delay) {
        _bgEvents.RescheduleEvent(BG_AB_EVENT_UPDATE_BANNER_STABLE + node,
                                  BG_AB_BANNER_UPDATE_TIME);
        return;
    }

    SpawnBGObject(node * BG_AB_OBJECTS_PER_NODE +
                      _capturePointInfo[node]._state,
                  RESPAWN_IMMEDIATELY);
    SpawnBGObject(node * BG_AB_OBJECTS_PER_NODE + BG_AB_OBJECT_AURA_ALLY +
                      _capturePointInfo[node]._ownerTeamId,
                  RESPAWN_IMMEDIATELY);
}
...
void BattlegroundAB::FillInitialWorldStates(WorldPacket& data)
{
    for (auto& node : _capturePointInfo) {
        ...
        for (uint8 i = BG_AB_NODE_STATE_ALLY_OCCUPIED;
             i <= BG_AB_NODE_STATE_HORDE_CONTESTED;
             ++i)
            data << uint32(node._iconCapture + i - 1)
                 << uint32(node._state == i);
    }

    data << uint32(BG_AB_OP_OCCUPIED_BASES_ALLY)
         << uint32(_controlledPoints[TEAM_ALLIANCE]);
    data << uint32(BG_AB_OP_OCCUPIED_BASES_HORDE)
         << uint32(_controlledPoints[TEAM_HORDE]);
    data << uint32(BG_AB_OP_RESOURCES_MAX) << uint32(BG_AB_MAX_TEAM_SCORE);
    data << uint32(BG_AB_OP_RESOURCES_WARNING)
         << uint32(BG_AB_WARNING_NEAR_VICTORY_SCORE);
    data << uint32(BG_AB_OP_RESOURCES_ALLY)
         << uint32(m_TeamScores[TEAM_ALLIANCE]);
    data << uint32(BG_AB_OP_RESOURCES_HORDE)
         << uint32(m_TeamScores[TEAM_HORDE]);
...
void BattlegroundAB::SendNodeUpdate(uint8 node)
{
    ...
    for (uint8 i = BG_AB_NODE_STATE_ALLY_OCCUPIED;
         i <= BG_AB_NODE_STATE_HORDE_CONTESTED;
         ++i)
        UpdateWorldState(_capturePointInfo[node]._iconCapture + i - 1,
                         _capturePointInfo[node]._state == i);

    UpdateWorldState(BG_AB_OP_OCCUPIED_BASES_ALLY,
                     _controlledPoints[TEAM_ALLIANCE]);
    UpdateWorldState(BG_AB_OP_OCCUPIED_BASES_HORDE,
                     _controlledPoints[TEAM_HORDE]);
}
...
void BattlegroundAB::NodeOccupied(uint8 node)
{
    ApplyPhaseMask();
    AddSpiritGuide(node,
                   BG_AB_SpiritGuidePos[node][0],
                   BG_AB_SpiritGuidePos[node][1],
                   BG_AB_SpiritGuidePos[node][2],
                   BG_AB_SpiritGuidePos[node][3],
                   _capturePointInfo[node]._ownerTeamId);
    ...
    if (_controlledPoints[_capturePointInfo[node]._ownerTeamId] >= 4)
        CastSpellOnTeam(SPELL_AB_QUEST_REWARD_4_BASES,
                        _capturePointInfo[node]._ownerTeamId);

    Creature* trigger =
        GetBgMap()->GetCreature(BgCreatures[BG_AB_ALL_NODES_COUNT + node]);
    if (!trigger)
        trigger = AddCreature(WORLD_TRIGGER,
                              BG_AB_ALL_NODES_COUNT + node,
                              BG_AB_NodePositions[node][0],
                              BG_AB_NodePositions[node][1],
                              BG_AB_NodePositions[node][2],
                              BG_AB_NodePositions[node][3]);

    if (trigger) {
        trigger->SetFaction(_capturePointInfo[node]._ownerTeamId ==
                                    TEAM_ALLIANCE
...
void BattlegroundAB::EventPlayerClickedOnFlag(Player*     player,
                                              GameObject* gameObject)
{
    if (GetStatus() != STATUS_IN_PROGRESS ||
        !player->IsWithinDistInMap(gameObject, 10.0f))
        return;
    ...
    if (node == BG_AB_DYNAMIC_NODES_COUNT ||
        ...
        (_capturePointInfo[node]._state == BG_AB_NODE_STATE_ALLY_CONTESTED &&
         player->GetTeamId() == TEAM_ALLIANCE) ||
        (_capturePointInfo[node]._state == BG_AB_NODE_STATE_HORDE_CONTESTED &&
    ...
    if (_capturePointInfo[node]._state == BG_AB_NODE_STATE_NEUTRAL) {
        ...
        _capturePointInfo[node]._state =
            static_cast<uint8>(BG_AB_NODE_STATE_ALLY_CONTESTED) +
            player->GetTeamId();
        _capturePointInfo[node]._ownerTeamId = TEAM_NEUTRAL;
        _bgEvents.RescheduleEvent(BG_AB_EVENT_CAPTURE_STABLE + node,
                                  BG_AB_FLAG_CAPTURING_TIME);
        sound = BG_AB_SOUND_NODE_CLAIMED;

        if (teamid == TEAM_ALLIANCE) {
            SendBroadcastText(ABNodes[node].TextAllianceClaims,
                              CHAT_MSG_BG_SYSTEM_ALLIANCE,
                              player);
        }
        else {
            SendBroadcastText(ABNodes[node].TextHordeClaims,
                              CHAT_MSG_BG_SYSTEM_HORDE,
                              player);
        }
    }
    else if (_capturePointInfo[node]._state ==
                 BG_AB_NODE_STATE_ALLY_CONTESTED ||
             _capturePointInfo[node]._state ==
        ...
        if (!_capturePointInfo[node]._captured) {
            ...
            if (teamid == TEAM_ALLIANCE) {
                SendBroadcastText(ABNodes[node].TextAllianceAssaulted,
                                  CHAT_MSG_BG_SYSTEM_ALLIANCE,
                                  player);
            }
            else {
                SendBroadcastText(ABNodes[node].TextHordeAssaulted,
                                  CHAT_MSG_BG_SYSTEM_HORDE,
                                  player);
            }
        }
            ...
            _capturePointInfo[node]._state =
                static_cast<uint8>(BG_AB_NODE_STATE_ALLY_OCCUPIED) +
                player->GetTeamId();
            _capturePointInfo[node]._ownerTeamId = player->GetTeamId();
            ...
            if (teamid == TEAM_ALLIANCE) {
                SendBroadcastText(ABNodes[node].TextAllianceDefended,
                                  CHAT_MSG_BG_SYSTEM_ALLIANCE,
                                  player);
            }
            else {
                SendBroadcastText(ABNodes[node].TextHordeDefended,
                                  CHAT_MSG_BG_SYSTEM_HORDE,
                                  player);
            }
        }

        sound = player->GetTeamId() == TEAM_ALLIANCE
...
TeamId BattlegroundAB::GetPrematureWinner()
{
    ...
    return _controlledPoints[TEAM_HORDE] > _controlledPoints[TEAM_ALLIANCE]
               ? TEAM_HORDE
...
{
    for (uint32 i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i) {
        AddObject(BG_AB_OBJECT_BANNER_NEUTRAL + BG_AB_OBJECTS_PER_NODE * i,
                  BG_AB_OBJECTID_NODE_BANNER_0 + i,
                  BG_AB_NodePositions[i][0],
                  BG_AB_NodePositions[i][1],
                  BG_AB_NodePositions[i][2],
                  BG_AB_NodePositions[i][3],
                  0,
                  0,
                  std::sin(BG_AB_NodePositions[i][3] / 2),
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_BANNER_ALLY + BG_AB_OBJECTS_PER_NODE * i,
                  ...
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_BANNER_HORDE + BG_AB_OBJECTS_PER_NODE * i,
                  ...
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_BANNER_CONT_A + BG_AB_OBJECTS_PER_NODE * i,
                  ...
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_BANNER_CONT_H + BG_AB_OBJECTS_PER_NODE * i,
                  ...
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_AURA_ALLY + BG_AB_OBJECTS_PER_NODE * i,
                  ...
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_AURA_HORDE + BG_AB_OBJECTS_PER_NODE * i,
                  ...
                  cos(BG_AB_NodePositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_AURA_CONTESTED + BG_AB_OBJECTS_PER_NODE * i,
    ...
    for (uint32 i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i) {
        AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i,
                  Buff_Entries[0],
                  BG_AB_BuffPositions[i][0],
                  BG_AB_BuffPositions[i][1],
                  BG_AB_BuffPositions[i][2],
                  BG_AB_BuffPositions[i][3],
                  0,
                  0,
                  std::sin(BG_AB_BuffPositions[i][3] / 2),
                  cos(BG_AB_BuffPositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i + 1,
                  Buff_Entries[1],
                  BG_AB_BuffPositions[i][0],
                  BG_AB_BuffPositions[i][1],
                  BG_AB_BuffPositions[i][2],
                  ...
                  cos(BG_AB_BuffPositions[i][3] / 2),
                  RESPAWN_ONE_DAY);
        AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i + 2,
                  Buff_Entries[2],
                  BG_AB_BuffPositions[i][0],
                  BG_AB_BuffPositions[i][1],
                  BG_AB_BuffPositions[i][2],
    ...
    AddSpiritGuide(BG_AB_SPIRIT_ALIANCE,
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_ALIANCE][0],
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_ALIANCE][1],
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_ALIANCE][2],
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_ALIANCE][3],
                   TEAM_ALLIANCE);
    AddSpiritGuide(BG_AB_SPIRIT_HORDE,
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_HORDE][0],
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_HORDE][1],
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_HORDE][2],
                   BG_AB_SpiritGuidePos[BG_AB_SPIRIT_HORDE][3],
                   TEAM_HORDE);

    for (uint32 i = BG_AB_OBJECT_BANNER_NEUTRAL; i < BG_AB_OBJECT_MAX; ++i)
        if (!BgObjects[i]) {
            LOG_ERROR("sql.sql",
                      "BatteGroundAB: Failed to spawn some object Battleground "
                      "not created!");
            return false;
        }
    ...
    for (uint32 i = BG_AB_SPIRIT_ALIANCE; i <= BG_AB_SPIRIT_HORDE; ++i)
        if (!BgCreatures[i]) {
            LOG_ERROR("sql.sql",
                      "BatteGroundAB: Failed to spawn spirit guides "
...
void BattlegroundAB::Init()
{
    ...
    _honorTics      = BattlegroundMgr::IsBGWeekend(GetBgTypeID(true))
                          ? BG_AB_HONOR_TICK_WEEKEND
                          : BG_AB_HONOR_TICK_NORMAL;
    _reputationTics = BattlegroundMgr::IsBGWeekend(GetBgTypeID(true))
    ...
    _capturePointInfo[BG_AB_NODE_BLACKSMITH]._iconNone =
        BG_AB_OP_BLACKSMITH_ICON;
    _capturePointInfo[BG_AB_NODE_LUMBER_MILL]._iconNone =
        BG_AB_OP_LUMBERMILL_ICON;
    _capturePointInfo[BG_AB_NODE_GOLD_MINE]._iconNone = BG_AB_OP_GOLDMINE_ICON;
    _capturePointInfo[BG_AB_NODE_STABLES]._iconCapture =
        BG_AB_OP_STABLE_STATE_ALIENCE;
    _capturePointInfo[BG_AB_NODE_FARM]._iconCapture =
        BG_AB_OP_FARM_STATE_ALIENCE;
    _capturePointInfo[BG_AB_NODE_BLACKSMITH]._iconCapture =
        BG_AB_OP_BLACKSMITH_STATE_ALIENCE;
    _capturePointInfo[BG_AB_NODE_LUMBER_MILL]._iconCapture =
        BG_AB_OP_LUMBERMILL_STATE_ALIENCE;
    _capturePointInfo[BG_AB_NODE_GOLD_MINE]._iconCapture =
...
{
    GraveyardStruct const* entry = sGraveyard->GetGraveyard(
        BG_AB_GraveyardIds[static_cast<uint8>(BG_AB_SPIRIT_ALIANCE) +
                           player->GetTeamId()]);
    ...
    float dist =
        (entry->x - pX) * (entry->x - pX) + (entry->y - pY) * (entry->y - pY);
    float minDist = dist;
    ...
    for (uint8 i = BG_AB_NODE_STABLES; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
        if (_capturePointInfo[i]._ownerTeamId == player->GetTeamId()) {
            ...
            dist  = (entry->x - pX) * (entry->x - pX) +
                   (entry->y - pY) * (entry->y - pY);
            if (dist < minDist) {
                minDist      = dist;
                nearestEntry = entry;
            }
