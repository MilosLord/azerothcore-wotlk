...
{
    BgObjects.resize(MAX_NORMAL_GAMEOBJECTS_SPAWNS + MAX_AIRSHIPS_SPAWNS +
    ...
    BgCreatures.resize(MAX_NORMAL_NPCS_SPAWNS + MAX_WORKSHOP_SPAWNS +
                       MAX_DOCKS_SPAWNS + MAX_SPIRIT_GUIDES_SPAWNS +
    ...
    closeFortressDoorsTimer =
        CLOSE_DOORS_TIME; // the doors are closed again... in a special way
    doorsClosed   = false;
...
void BattlegroundIC::DoAction(uint32 action, ObjectGuid guid)
{
    ...
    MotionTransport* transport =
        player->GetTeamId() == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde;
...
{
    if (nodePoint[NODE_TYPE_QUARRY].nodeState ==
        (player->GetTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A
                                              : NODE_STATE_CONTROLLED_H))
        player->CastSpell(player, SPELL_QUARRY, true);

    if (nodePoint[NODE_TYPE_REFINERY].nodeState ==
        (player->GetTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A
                                              : NODE_STATE_CONTROLLED_H))
...
void BattlegroundIC::PostUpdateImpl(uint32 diff)
{
    ...
    if (!doorsClosed) {
        if (closeFortressDoorsTimer <= diff) {
            GetBGObject(BG_IC_GO_DOODAD_ND_HUMAN_GATE_CLOSEDFX_DOOR01)
                ->RemoveFromWorld();
            GetBGObject(BG_IC_GO_DOODAD_ND_WINTERORC_WALL_GATEFX_DOOR01)
                ->RemoveFromWorld();
            GetBGObject(BG_IC_GO_DOODAD_ND_HUMAN_GATE_CLOSEDFX_DOOR02)
                ->RemoveFromWorld();
            GetBGObject(BG_IC_GO_DOODAD_ND_WINTERORC_WALL_GATEFX_DOOR02)
                ->RemoveFromWorld();
            GetBGObject(BG_IC_GO_DOODAD_ND_HUMAN_GATE_CLOSEDFX_DOOR03)
                ->RemoveFromWorld();
            GetBGObject(BG_IC_GO_DOODAD_ND_WINTERORC_WALL_GATEFX_DOOR03)
                ->RemoveFromWorld();

            GetBGObject(BG_IC_GO_ALLIANCE_GATE_1)
                ->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
            GetBGObject(BG_IC_GO_HORDE_GATE_1)
                ->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
            GetBGObject(BG_IC_GO_ALLIANCE_GATE_2)
                ->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
            GetBGObject(BG_IC_GO_HORDE_GATE_2)
                ->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
            GetBGObject(BG_IC_GO_ALLIANCE_GATE_3)
                ->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
            GetBGObject(BG_IC_GO_HORDE_GATE_3)
                ->SetDestructibleState(GO_DESTRUCTIBLE_DAMAGED);
    ...
    for (uint8 i = NODE_TYPE_REFINERY; i < MAX_NODE_TYPES; ++i) {
        if (nodePoint[i].nodeType == NODE_TYPE_DOCKS) {
            if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
                ...
                if (nodePoint[i].timer <= diff) {
                    // we need to confirm this, i am not sure if this every 3
                    // minutes
                    for (uint8 j = 0; j < MAX_CATAPULTS_SPAWNS_PER_FACTION;
                         ++j) {
                        uint8 type = (nodePoint[i].faction == TEAM_ALLIANCE
                                          ? BG_IC_NPC_CATAPULT_1_A
                                          : BG_IC_NPC_CATAPULT_1_H) +
                                     j;
                        if (Creature* catapult =
                                GetBgMap()->GetCreature(BgCreatures[type]))
                            if (!catapult->IsAlive()) {
                                // Check if creature respawn time is properly
                                // saved
                                RespawnMap::iterator itr =
                                    respawnMap.find(catapult->GetGUID());
                                if (itr == respawnMap.end() ||
                                    GameTime::GetGameTime().count() <
                                        itr->second)
                                    continue;

                                catapult->Relocate(
                                    BG_IC_DocksVehiclesCatapults[j]
                                        .GetPositionX(),
                                    BG_IC_DocksVehiclesCatapults[j]
                                        .GetPositionY(),
                                    BG_IC_DocksVehiclesCatapults[j]
                    ...
                    // we need to confirm this is blizzlike, not sure if it is
                    // every 3 minutes
                    for (uint8 j = 0;
                         j < MAX_GLAIVE_THROWERS_SPAWNS_PER_FACTION;
                         ++j) {
                        uint8 type = (nodePoint[i].faction == TEAM_ALLIANCE
                                          ? BG_IC_NPC_GLAIVE_THROWER_1_A
                                          : BG_IC_NPC_GLAIVE_THROWER_1_H) +
                                     j;
                        if (Creature* glaiveThrower =
                                GetBgMap()->GetCreature(BgCreatures[type]))
                            if (!glaiveThrower->IsAlive()) {
                                ...
                                glaiveThrower->Relocate(
                                    BG_IC_DocksVehiclesGlaives[j]
                                        .GetPositionX(),
                                    BG_IC_DocksVehiclesGlaives[j]
                                        .GetPositionY(),
                                    BG_IC_DocksVehiclesGlaives[j]
        ...
        if (nodePoint[i].nodeType == NODE_TYPE_WORKSHOP) {
            if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
                ...
                if (siegeEngineWorkshopTimer <= diff) {
                    uint8 siegeType = (nodePoint[i].faction == TEAM_ALLIANCE
                                           ? BG_IC_NPC_SIEGE_ENGINE_A
                                           : BG_IC_NPC_SIEGE_ENGINE_H);
                    if (Creature* siege = GetBGCreature(
                            siegeType)) // this always should be true
                        if (!siege->IsAlive()) {
                            // Check if creature respawn time is properly saved
                            RespawnMap::iterator itr =
                                respawnMap.find(siege->GetGUID());
                            if (itr == respawnMap.end() ||
                                GameTime::GetGameTime().count() < itr->second)
                                continue;

                            siege->Relocate(
                                BG_IC_WorkshopVehicles[4].GetPositionX(),
                                BG_IC_WorkshopVehicles[4].GetPositionY(),
                                BG_IC_WorkshopVehicles[4].GetPositionZ(),
                                BG_IC_WorkshopVehicles[4].GetOrientation());
                            siege->Respawn(true);
                            respawnMap.erase(itr);
                        }
                    ...
                    // we need to confirm this, i am not sure if this every 3
                    // minutes
                    for (uint8 u = 0; u < MAX_DEMOLISHERS_SPAWNS_PER_FACTION;
                         ++u) {
                        uint8 type = (nodePoint[i].faction == TEAM_ALLIANCE
                                          ? BG_IC_NPC_DEMOLISHER_1_A
                                          : BG_IC_NPC_DEMOLISHER_1_H) +
                                     u;
                        if (Creature* demolisher =
                                GetBgMap()->GetCreature(BgCreatures[type]))
                            if (!demolisher->IsAlive()) {
                                ...
                                demolisher->Relocate(
                                    BG_IC_WorkshopVehicles[u].GetPositionX(),
                                    BG_IC_WorkshopVehicles[u].GetPositionY(),
                                    BG_IC_WorkshopVehicles[u].GetPositionZ(),
                                    BG_IC_WorkshopVehicles[u].GetOrientation());
        ...
        if (nodePoint[i].needChange) {
            if (nodePoint[i].timer <= diff) {
                uint32 nextBanner =
                    GetNextBanner(&nodePoint[i], nodePoint[i].faction, true);
                ...
                float cords[4] = {banner->GetPositionX(),
                                  banner->GetPositionY(),
                                  banner->GetPositionZ(),
                                  banner->GetOrientation()};

                DelObject(nodePoint[i].gameobject_type);
                AddObject(nodePoint[i].gameobject_type,
                ...
                GetBGObject(nodePoint[i].gameobject_type)
                    ->SetUInt32Value(GAMEOBJECT_FACTION,
                                     nodePoint[i].faction == TEAM_ALLIANCE
                                         ? BG_IC_Factions[1]
                                         : BG_IC_Factions[0]);
                ...
                if (nodePoint[i].faction == TEAM_ALLIANCE) {
                    SendBroadcastText(ICNodes[i].TextAllianceTaken,
                                      CHAT_MSG_BG_SYSTEM_ALLIANCE);
                }
                else {
                    SendBroadcastText(ICNodes[i].TextHordeTaken,
    ...
    if (resourceTimer <= diff) {
        for (uint8 i = 0; i < NODE_TYPE_DOCKS; ++i) {
            if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
                ...
                UpdateWorldState((nodePoint[i].faction == TEAM_ALLIANCE
                                      ? BG_IC_ALLIANCE_RENFORT
                                      : BG_IC_HORDE_RENFORT),
                                 factionReinforcements[nodePoint[i].faction]);
            }
        }
        resourceTimer = IC_RESOURCE_TIME;
    }
...
void BattlegroundIC::StartingEventOpenDoors()
{
    ...
    for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTERS_SPAWNS; ++i)
        GetBGObject(BG_IC_Teleporters[i].type)
            ->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE);

    for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTER_EFFECTS_SPAWNS; ++i)
        GetBGObject(BG_IC_TeleporterEffects[i].type)
...
{
    uint32      count           = 0;
    ICNodeState controlledState = teamId == TEAM_ALLIANCE
...
bool BattlegroundIC::IsResourceGlutAllowed(TeamId teamId) const
{
    ICNodeState controlledState = teamId == TEAM_ALLIANCE
                                      ? NODE_STATE_CONTROLLED_A
                                      : NODE_STATE_CONTROLLED_H;
    return nodePoint[NODE_TYPE_QUARRY].nodeState == controlledState &&
           nodePoint[NODE_TYPE_REFINERY].nodeState == controlledState;
}
...
void BattlegroundIC::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(player->GetGUID().GetCounter(),
                         new BattlegroundICScore(player->GetGUID()));
...
void BattlegroundIC::FillInitialWorldStates(WorldPacket& data)
{
    ...
    data << uint32(BG_IC_ALLIANCE_RENFORT)
         << uint32(factionReinforcements[TEAM_ALLIANCE]);
    data << uint32(BG_IC_HORDE_RENFORT)
         << uint32(factionReinforcements[TEAM_HORDE]);

    for (uint8 i = 0; i < MAX_FORTRESS_GATES_SPAWNS; ++i) {
        uint32 uws = GetWorldStateFromGateEntry(
            BG_IC_ObjSpawnlocs[i].entry,
            (GateStatus[GetGateIDFromEntry(BG_IC_ObjSpawnlocs[i].entry)] ==
             BG_IC_GATE_DESTROYED));
        data << uint32(uws) << uint32(1);
    }

    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
        data << uint32(nodePoint[i].worldStates[nodePoint[i].nodeState])
             << uint32(1);
}
...
bool BattlegroundIC::SetupBattleground()
{
    for (uint8 i = 0; i < MAX_NORMAL_GAMEOBJECTS_SPAWNS; ++i) {
        if (!AddObject(BG_IC_ObjSpawnlocs[i].type,
            ...
            LOG_ERROR(
                "bg.battleground",
                "Isle of Conquest: There was an error spawning gameobject {}",
                BG_IC_ObjSpawnlocs[i].entry);
            return false;
        }
    ...
    for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTERS_SPAWNS; ++i) {
        if (!AddObject(BG_IC_Teleporters[i].type,
            ...
            LOG_ERROR("bg.battleground",
                      "Isle of Conquest | Starting Event Open Doors: There was "
                      "an error spawning gameobject {}",
                      BG_IC_Teleporters[i].entry);
            return false;
        }
    }
    ...
    for (uint8 i = 0; i < MAX_NORMAL_NPCS_SPAWNS; ++i) {
        if (!AddCreature(BG_IC_NpcSpawnlocs[i].entry,
                         BG_IC_NpcSpawnlocs[i].type,
                         BG_IC_NpcSpawnlocs[i].x,
                         BG_IC_NpcSpawnlocs[i].y,
                         BG_IC_NpcSpawnlocs[i].z,
                         BG_IC_NpcSpawnlocs[i].o,
                         RESPAWN_ONE_DAY)) {
            LOG_ERROR(
                "bg.battleground",
                "Isle of Conquest: There was an error spawning creature {}",
                BG_IC_NpcSpawnlocs[i].entry);
            return false;
        }
    }

    if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 3,
                        BG_IC_SpiritGuidePos[5][0],
                        BG_IC_SpiritGuidePos[5][1],
                        BG_IC_SpiritGuidePos[5][2],
                        BG_IC_SpiritGuidePos[5][3],
                        TEAM_ALLIANCE) ||
        !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 4,
                        BG_IC_SpiritGuidePos[6][0],
                        BG_IC_SpiritGuidePos[6][1],
                        BG_IC_SpiritGuidePos[6][2],
                        BG_IC_SpiritGuidePos[6][3],
                        TEAM_HORDE) ||
        !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 5,
                        BG_IC_SpiritGuidePos[7][0],
                        BG_IC_SpiritGuidePos[7][1],
                        BG_IC_SpiritGuidePos[7][2],
                        BG_IC_SpiritGuidePos[7][3],
                        TEAM_ALLIANCE) ||
        !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 6,
                        BG_IC_SpiritGuidePos[8][0],
                        BG_IC_SpiritGuidePos[8][1],
                        BG_IC_SpiritGuidePos[8][2],
                        BG_IC_SpiritGuidePos[8][3],
                        TEAM_HORDE)) {
        LOG_ERROR("bg.battleground",
    ...
    gunshipHorde =
        sTransportMgr->CreateTransport(GO_HORDE_GUNSHIP, 0, GetBgMap());
    gunshipAlliance =
        sTransportMgr->CreateTransport(GO_ALLIANCE_GUNSHIP, 0, GetBgMap());
    ...
    for (uint8 i = BG_IC_GO_HUGE_SEAFORIUM_BOMBS_A_1;
         i < BG_IC_GO_HUGE_SEAFORIUM_BOMBS_H_4;
         ++i)
...
void BattlegroundIC::TurnBosses(bool on)
{
    ...
    if (Creature* boss =
            GetBGCreature(BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE)) {
        boss->SetVisible(on);
        boss->SetReactState(on ? REACT_AGGRESSIVE : REACT_PASSIVE);
    }
}
...
void BattlegroundIC::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    if (unit->IsVehicle()) {
        ...
        if (entry == NPC_DEMOLISHER || entry == NPC_SIEGE_ENGINE_H ||
            entry == NPC_SIEGE_ENGINE_A || entry == NPC_GLAIVE_THROWER_A ||
            entry == NPC_GLAIVE_THROWER_H || entry == NPC_CATAPULT)
            respawnMap[unit->GetGUID()] =
                GameTime::GetGameTime().count() + VEHICLE_RESPAWN_TIME;
    }
}
...
void BattlegroundIC::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    UpdateWorldState((player->GetTeamId() == TEAM_ALLIANCE
                          ? BG_IC_ALLIANCE_RENFORT
                          : BG_IC_HORDE_RENFORT),
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player*     player,
                                              GameObject* gameObject)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i) {
        ...
        if (point.gameobject_entry == gameObject->GetEntry()) {
            ...
            if (point.banners[BANNER_A_CONTESTED] == nextBanner ||
                point.banners[BANNER_H_CONTESTED] == nextBanner) {
                point.timer =
                    BANNER_STATE_CHANGE_TIME; // 1 minute for last change (real
                                              // faction banner)
                ...
                _reviveEvents.AddEventAtOffset(
                    [this, point]() {
                        RelocateDeadPlayers(
                            BgCreatures[static_cast<uint16>(
                                            BG_IC_NPC_SPIRIT_GUIDE_1) +
                                        point.nodeType - 2]);

                        // if we are here means that the point has been lost, or
                        // it is the first capture
                        if (point.nodeType != NODE_TYPE_REFINERY &&
                            point.nodeType != NODE_TYPE_QUARRY) {
                            if (BgCreatures[static_cast<uint16>(
                                                BG_IC_NPC_SPIRIT_GUIDE_1) +
                                            (point.nodeType) - 2]) {
                                DelCreature(static_cast<uint16>(
                                                BG_IC_NPC_SPIRIT_GUIDE_1) +
                                            (point.nodeType) - 2);
                            }
                        }
                ...
                if (nodePoint[i].faction == TEAM_ALLIANCE) {
                    SendBroadcastText(ICNodes[i].TextAssaulted,
                                      CHAT_MSG_BG_SYSTEM_ALLIANCE,
                                      player);
                }
                else {
            ...
            else if (nextBanner == point.banners[BANNER_A_CONTROLLED] ||
                     nextBanner == point.banners[BANNER_H_CONTROLLED])
            // if we are going to spawn the definitve faction banner, we dont
            // need the timer anymore
            {
                point.timer      = BANNER_STATE_CHANGE_TIME;
                point.needChange = false;

                if (nodePoint[i].faction == TEAM_ALLIANCE) {
                    SendBroadcastText(ICNodes[i].TextDefended,
                                      CHAT_MSG_BG_SYSTEM_ALLIANCE,
                                      player);
                }
                else {
            ...
            AddObject(point.gameobject_type,
                      point.gameobject_entry,
                      cords[0],
                      cords[1],
                      cords[2],
                      cords[3],
            ...
            GetBGObject(point.gameobject_type)
                ->SetUInt32Value(GAMEOBJECT_FACTION,
                                 point.faction == TEAM_ALLIANCE
                                     ? BG_IC_Factions[1]
                                     : BG_IC_Factions[0]);

            UpdateNodeWorldState(&point);
            // we dont need iterating if we are here
            // If the needChange bool was set true, we will handle the rest in
            // the Update Map function.
            return;
        }
    }
}
...
void BattlegroundIC::UpdateNodeWorldState(ICNodePoint* nodePoint)
{
    ...
    else if (nodePoint->gameobject_entry ==
             nodePoint->banners[BANNER_A_CONTESTED])
        nodePoint->nodeState = NODE_STATE_CONFLICT_A;
    else if (nodePoint->gameobject_entry ==
             nodePoint->banners[BANNER_H_CONTROLLED])
        nodePoint->nodeState = NODE_STATE_CONTROLLED_H;
    else if (nodePoint->gameobject_entry ==
             nodePoint->banners[BANNER_H_CONTESTED])
                                     ...
                                     bool         returnDefinitve)
{
    ...
    if (returnDefinitve)
        // here is a special case, here we must return the definitve faction
        // banner after the grey banner was spawned 1 minute
        return nodePoint->banners[(
            team == TEAM_ALLIANCE ? BANNER_A_CONTROLLED : BANNER_H_CONTROLLED)];

    // there were no changes, this point has never been captured by any faction
    // or at least clicked
    if (nodePoint->last_entry == 0)
        // 1 returns the CONTESTED ALLIANCE BANNER, 3 returns the HORDE one
        return nodePoint->banners[(team == TEAM_ALLIANCE ? BANNER_A_CONTESTED
                                                         : BANNER_H_CONTESTED)];

    // If the actual banner is the definitive faction banner, we must return the
    // grey banner of the player's faction
    if (nodePoint->gameobject_entry ==
            nodePoint->banners[BANNER_A_CONTROLLED] ||
        nodePoint->gameobject_entry == nodePoint->banners[BANNER_H_CONTROLLED])
        return nodePoint->banners[(team == TEAM_ALLIANCE ? BANNER_A_CONTESTED
                                                         : BANNER_H_CONTESTED)];

    // If the actual banner is the grey faction banner, we must return the
    // previous banner
    if (nodePoint->gameobject_entry == nodePoint->banners[BANNER_A_CONTESTED] ||
        nodePoint->banners[BANNER_H_CONTESTED])
...
void BattlegroundIC::HandleContestedNodes(ICNodePoint* nodePoint)
{
    if (nodePoint->nodeType == NODE_TYPE_HANGAR) {
        if (gunshipAlliance && gunshipHorde)
            (nodePoint->faction == TEAM_ALLIANCE ? gunshipHorde
                                                 : gunshipAlliance)
                ->EnableMovement(false);

        for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_1;
             u <= BG_IC_GO_HANGAR_TELEPORTER_3;
             ++u)
            DelObject(u);

        for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_EFFECT_1;
             u <= BG_IC_GO_HANGAR_TELEPORTER_EFFECT_3;
             ++u)
            DelObject(u);
        ...
        if (nodePoint->faction == TEAM_HORDE)
            gunshipAlliance->GetCreatureListWithEntryInGrid(
                cannons, NPC_ALLIANCE_GUNSHIP_CANNON, 150.0f);
        else
            gunshipHorde->GetCreatureListWithEntryInGrid(
                cannons, NPC_HORDE_GUNSHIP_CANNON, 150.0f);

        for (std::list<Creature*>::const_iterator itr = cannons.begin();
             itr != cannons.end();
             ++itr) {
...
{
    if (nodePoint->nodeType != NODE_TYPE_REFINERY &&
        nodePoint->nodeType != NODE_TYPE_QUARRY) {
        if (!AddSpiritGuide(static_cast<uint16>(BG_IC_NPC_SPIRIT_GUIDE_1) +
                                nodePoint->nodeType - 2,
                            BG_IC_SpiritGuidePos[nodePoint->nodeType][0],
                            BG_IC_SpiritGuidePos[nodePoint->nodeType][1],
                            BG_IC_SpiritGuidePos[nodePoint->nodeType][2],
                            BG_IC_SpiritGuidePos[nodePoint->nodeType][3],
                            nodePoint->faction))
            LOG_ERROR("bg.battleground",
                      "Isle of Conquest: Failed to spawn spirit guide! point: "
                      "{}, team: {}, ",
                      nodePoint->nodeType,
                      nodePoint->faction);
    }
    ...
    case BG_IC_GO_HANGAR_BANNER: {
        ...
        for (uint8 u = 0; u < MAX_HANGAR_TELEPORTERS_SPAWNS; ++u) {
            ...
            if (!AddObject(type,
                           (nodePoint->faction == TEAM_ALLIANCE
                                ? GO_ALLIANCE_GUNSHIP_PORTAL
                                : GO_HORDE_GUNSHIP_PORTAL),
                           BG_IC_HangarTeleporters[u].GetPositionX(),
                           BG_IC_HangarTeleporters[u].GetPositionY(),
                           BG_IC_HangarTeleporters[u].GetPositionZ(),
                           BG_IC_HangarTeleporters[u].GetOrientation(),
                           0,
                           0,
                           0,
                           0,
                           RESPAWN_ONE_DAY))
                LOG_ERROR("bg.battleground",
                          "Isle of Conquest: There was an error spawning a "
                          "gunship portal. Type: {}",
                          BG_IC_GO_HANGAR_TELEPORTER_1 + u);
        }
        ...
        for (uint8 u = 0; u < MAX_HANGAR_TELEPORTER_EFFECTS_SPAWNS; ++u) {
            ...
            if (!AddObject(type,
                           ...
                           BG_IC_HangarTeleporterEffects[u].GetPositionX(),
                           BG_IC_HangarTeleporterEffects[u].GetPositionY(),
                           BG_IC_HangarTeleporterEffects[u].GetPositionZ(),
                           BG_IC_HangarTeleporterEffects[u].GetOrientation(),
                           0,
                           0,
                           0,
                           0,
        ...
        for (uint8 u = 0; u < MAX_TRIGGER_SPAWNS_PER_FACTION; ++u) {
            if (!AddCreature(
                    ...
                    BG_IC_HangarTrigger[nodePoint->faction].GetPositionX(),
                    BG_IC_HangarTrigger[nodePoint->faction].GetPositionY(),
                    BG_IC_HangarTrigger[nodePoint->faction].GetPositionZ(),
                    BG_IC_HangarTrigger[nodePoint->faction].GetOrientation(),
                    RESPAWN_ONE_DAY,
        ...
        for (uint8 u = 0; u < MAX_CAPTAIN_SPAWNS_PER_FACTION; ++u) {
            ...
            if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_1)
                if (AddCreature(
                        ...
                        type,
                        BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE
                                                 ? 2
                                                 : 0]
                            .GetPositionX(),
                        BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE
                        ...
                        BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE
                            ...
                            .GetOrientation(),
                        RESPAWN_ONE_DAY))
                    GetBGCreature(BG_IC_NPC_GUNSHIP_CAPTAIN_1)
                        ->GetAI()
                        ->DoAction(ACTION_GUNSHIP_READY);
        ...
        (nodePoint->faction == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde)
            ->EnableMovement(true);
        break;
    }
    ...
    case BG_IC_GO_REFINERY_BANNER:
        RemoveAuraOnTeam(SPELL_OIL_REFINERY,
                         GetOtherTeamId(nodePoint->faction));
    ...
    case BG_IC_GO_DOCKS_BANNER:
        ...
        for (uint8 i = 0; i < MAX_GLAIVE_THROWERS_SPAWNS_PER_FACTION; ++i) {
            uint8 type = (nodePoint->faction == TEAM_ALLIANCE
                              ? BG_IC_NPC_GLAIVE_THROWER_1_A
                              : BG_IC_NPC_GLAIVE_THROWER_1_H) +
                         i;
            ...
            if (AddCreature(nodePoint->faction == TEAM_ALLIANCE
                                ? NPC_GLAIVE_THROWER_A
                                : NPC_GLAIVE_THROWER_H,
                            type,
                            BG_IC_DocksVehiclesGlaives[i].GetPositionX(),
                            BG_IC_DocksVehiclesGlaives[i].GetPositionY(),
                            BG_IC_DocksVehiclesGlaives[i].GetPositionZ(),
                            BG_IC_DocksVehiclesGlaives[i].GetOrientation(),
                            RESPAWN_ONE_DAY))
                GetBGCreature(type)->SetFaction(
                    BG_IC_Factions[nodePoint->faction]);
        }

        // spawning catapults
        for (uint8 i = 0; i < MAX_CATAPULTS_SPAWNS_PER_FACTION; ++i) {
            uint8 type =
                (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_CATAPULT_1_A
                                                     : BG_IC_NPC_CATAPULT_1_H) +
                i;
            ...
            if (AddCreature(NPC_CATAPULT,
                            type,
                            BG_IC_DocksVehiclesCatapults[i].GetPositionX(),
                            BG_IC_DocksVehiclesCatapults[i].GetPositionY(),
                            BG_IC_DocksVehiclesCatapults[i].GetPositionZ(),
                            BG_IC_DocksVehiclesCatapults[i].GetOrientation(),
                            RESPAWN_ONE_DAY))
                GetBGCreature(type)->SetFaction(BG_IC_Factions[(
                    nodePoint->faction == TEAM_ALLIANCE ? TEAM_ALLIANCE
                                                        : TEAM_HORDE)]);
        }
        break;
    case BG_IC_GO_WORKSHOP_BANNER: {
        ...
        if (!recapture) {
            for (uint8 i = 0; i < MAX_DEMOLISHERS_SPAWNS_PER_FACTION; ++i) {
                ...
                if (AddCreature(NPC_DEMOLISHER,
                                type,
                                BG_IC_WorkshopVehicles[i].GetPositionX(),
                                BG_IC_WorkshopVehicles[i].GetPositionY(),
                                BG_IC_WorkshopVehicles[i].GetPositionZ(),
                                BG_IC_WorkshopVehicles[i].GetOrientation(),
                                RESPAWN_ONE_DAY))
                    GetBGCreature(type)->SetFaction(BG_IC_Factions[(
                        nodePoint->faction == TEAM_ALLIANCE ? TEAM_ALLIANCE
                                                            : TEAM_HORDE)]);
            ...
            uint8 siegeType = (nodePoint->faction == TEAM_ALLIANCE
                                   ? BG_IC_NPC_SIEGE_ENGINE_A
                                   : BG_IC_NPC_SIEGE_ENGINE_H);
            if (!GetBgMap()->GetCreature(BgCreatures[siegeType])) {
                AddCreature((nodePoint->faction == TEAM_ALLIANCE
                                 ? NPC_SIEGE_ENGINE_A
                                 : NPC_SIEGE_ENGINE_H),
                            siegeType,
                            BG_IC_WorkshopVehicles[4].GetPositionX(),
            ...
            if (Creature* siegeEngine =
                    GetBgMap()->GetCreature(BgCreatures[siegeType])) {
                siegeEngine->SetFaction(BG_IC_Factions[(
                    nodePoint->faction == TEAM_ALLIANCE ? TEAM_ALLIANCE
                                                        : TEAM_HORDE)]);
                ...
                respawnMap[siegeEngine->GetGUID()] =
                    GameTime::GetGameTime().count() + VEHICLE_RESPAWN_TIME;
            }
        }

        for (uint8 i = 0; i < MAX_WORKSHOP_BOMBS_SPAWNS_PER_FACTION; ++i) {
            AddObject(BG_IC_GO_SEAFORIUM_BOMBS_1 + i,
                      GO_SEAFORIUM_BOMBS,
                      workshopBombs[i].GetPositionX(),
                      workshopBombs[i].GetPositionY(),
                      workshopBombs[i].GetPositionZ(),
                      workshopBombs[i].GetOrientation(),
                      0,
                      0,
                      0,
                      0,
            ...
            if (GameObject* seaforiumBombs =
                    GetBGObject(BG_IC_GO_SEAFORIUM_BOMBS_1 + i)) {
                seaforiumBombs->SetRespawnTime(10);
                seaforiumBombs->SetUInt32Value(
                    GAMEOBJECT_FACTION,
                    BG_IC_Factions[(nodePoint->faction == TEAM_ALLIANCE ? 0
                                                                        : 1)]);
            }
        }
        break;
...
GraveyardStruct const* BattlegroundIC::GetClosestGraveyard(Player* player)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
        if (nodePoint[i].faction == player->GetTeamId() &&
            !nodePoint[i]
                 .needChange) // xinef: controlled by faction and not contested!
            nodes.push_back(i);
    ...
    if (!nodes.empty()) {
        ...
        for (uint8 i = 0; i < nodes.size(); ++i) {
            GraveyardStruct const* entry =
                sGraveyard->GetGraveyard(BG_IC_GraveyardIds[nodes[i]]);
            if (!entry)
                continue;
            float dist = (entry->x - plr_x) * (entry->x - plr_x) +
                         (entry->y - plr_y) * (entry->y - plr_y);
            if (mindist > dist) {
                mindist    = dist;
                good_entry = entry;
            }
    ...
    if (!good_entry)
        good_entry = sGraveyard->GetGraveyard(
            BG_IC_GraveyardIds[player->GetTeamId() +
                               static_cast<uint16>(MAX_NODE_TYPES)]);
