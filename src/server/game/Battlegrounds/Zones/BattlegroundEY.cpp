        ...
        while (uint32 eventId = _bgEvents.ExecuteEvent())
            ...
            case BG_EY_EVENT_ADD_POINTS:
                if (_ownedPointsCount[TEAM_ALLIANCE] > 0)
                    AddPoints(
                        TEAM_ALLIANCE,
                        BG_EY_TickPoints[_ownedPointsCount[TEAM_ALLIANCE] - 1]);
                if (_ownedPointsCount[TEAM_HORDE] > 0)
                    AddPoints(
                        TEAM_HORDE,
                        BG_EY_TickPoints[_ownedPointsCount[TEAM_HORDE] - 1]);
...
void BattlegroundEY::StartingEventCloseDoors()
{
    ...
    for (uint32 i = BG_EY_OBJECT_A_BANNER_FEL_REAVER_CENTER;
         i < BG_EY_OBJECT_MAX;
         ++i)
...
void BattlegroundEY::StartingEventOpenDoors()
{
    ...
    for (uint32 i = BG_EY_OBJECT_N_BANNER_FEL_REAVER_CENTER;
         i <= BG_EY_OBJECT_FLAG_NETHERSTORM;
         ++i)
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);

    for (uint32 i = 0; i < EY_POINTS_MAX; ++i)
        SpawnBGObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3 + urand(0, 2),
                      RESPAWN_IMMEDIATELY);
...
void BattlegroundEY::AddPoints(TeamId teamId, uint32 points)
{
    ...
    for (; honorRewards < uint8(m_TeamScores[teamId] / _honorTics);
         ++honorRewards)
        RewardHonorToTeam(GetBonusHonorFromKill(1), teamId);

    UpdateWorldState(
        teamId == TEAM_ALLIANCE ? EY_ALLIANCE_RESOURCES : EY_HORDE_RESOURCES,
...
void BattlegroundEY::UpdatePointsState()
{
    ...
    for (uint8 point = 0; point < EY_POINTS_MAX; ++point) {
        pointsVec.push_back(
            GetBGObject(BG_EY_OBJECT_TOWER_CAP_FEL_REAVER + point));
    ...
    for (BattlegroundPlayerMap::const_iterator itr = bgPlayerMap.begin();
         itr != bgPlayerMap.end();
         ++itr) {
        itr->second->SendUpdateWorldState(PROGRESS_BAR_SHOW,
                                          BG_EY_PROGRESS_BAR_DONT_SHOW);
        for (uint8 point = 0; point < EY_POINTS_MAX; ++point)
            if (GameObject* pointObject = pointsVec[point])
                if (itr->second->CanCaptureTowerPoint() &&
                    itr->second->IsWithinDistInMap(pointObject,
                                                   BG_EY_POINT_RADIUS)) {
                    itr->second->SendUpdateWorldState(PROGRESS_BAR_SHOW,
                                                      BG_EY_PROGRESS_BAR_SHOW);
                    itr->second->SendUpdateWorldState(
                        PROGRESS_BAR_PERCENT_GREY,
                        BG_EY_PROGRESS_BAR_PERCENT_GREY);
                    itr->second->SendUpdateWorldState(
                        PROGRESS_BAR_STATUS,
                        _capturePointInfo[point]._barStatus);
                    ++_capturePointInfo[point]
                          ._playersCount[itr->second->GetTeamId()];
                    _capturePointInfo[point].player = itr->second;

                    // Xinef: ugly hax... area trigger is no longer called by
                    // client...
                    if (pointObject->GetEntry() ==
        ...
        if (pointOwnerTeamId != _capturePointInfo[point]._ownerTeamId) {
            if (_capturePointInfo[point].IsUncontrolled())
                EventTeamCapturedPoint(
                    _capturePointInfo[point].player, pointOwnerTeamId, point);

            if (pointOwnerTeamId == TEAM_NEUTRAL &&
...
void BattlegroundEY::UpdatePointsIcons(uint32 point)
{
    if (_capturePointInfo[point].IsUnderControl()) {
        ...
        UpdateWorldState(
            m_PointsIconStruct[point].WorldStateHordeControlledIndex,
            _capturePointInfo[point].IsUnderControl(TEAM_HORDE));
    }
    else {
        UpdateWorldState(
            m_PointsIconStruct[point].WorldStateAllianceControlledIndex, 0);
        UpdateWorldState(
            m_PointsIconStruct[point].WorldStateHordeControlledIndex, 0);
...
void BattlegroundEY::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(player->GetGUID().GetCounter(),
                         new BattlegroundEYScore(player->GetGUID()));
}
...
void BattlegroundEY::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case AT_BLOOD_ELF_POINT:
        if (_capturePointInfo[POINT_BLOOD_ELF].IsUnderControl(
                player->GetTeamId()))
    ...
    case AT_FEL_REAVER_POINT:
        if (_capturePointInfo[POINT_FEL_REAVER].IsUnderControl(
                player->GetTeamId()))
            if (_flagState == BG_EY_FLAG_STATE_ON_PLAYER &&
                GetFlagPickerGUID() == player->GetGUID())
    ...
    case AT_MAGE_TOWER_POINT:
        if (_capturePointInfo[POINT_MAGE_TOWER].IsUnderControl(
                player->GetTeamId()))
            if (_flagState == BG_EY_FLAG_STATE_ON_PLAYER &&
                GetFlagPickerGUID() == player->GetGUID())
    ...
    case AT_DRAENEI_RUINS_POINT:
        if (_capturePointInfo[POINT_DRAENEI_RUINS].IsUnderControl(
                player->GetTeamId()))
            if (_flagState == BG_EY_FLAG_STATE_ON_PLAYER &&
                GetFlagPickerGUID() == player->GetGUID())
...
bool BattlegroundEY::SetupBattleground()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i) {
        AreaTrigger const* at =
            sObjectMgr->GetAreaTrigger(_capturePointInfo[i]._areaTrigger);
    ...
    AddSpiritGuide(BG_EY_SPIRIT_MAIN_ALLIANCE,
                   sg->x,
                   sg->y,
                   sg->z,
                   3.124139f,
    ...
    AddSpiritGuide(
        BG_EY_SPIRIT_MAIN_HORDE, sg->x, sg->y, sg->z, 3.193953f, TEAM_HORDE);

    for (uint32 i = BG_EY_OBJECT_DOOR_A; i < BG_EY_OBJECT_MAX; ++i)
        if (!BgObjects[i]) {
            LOG_ERROR("sql.sql",
                      "BatteGroundEY: Failed to spawn some object Battleground "
                      "not created!");
            return false;
        }

    for (uint32 i = BG_EY_SPIRIT_MAIN_ALLIANCE; i <= BG_EY_SPIRIT_MAIN_HORDE;
         ++i)
        if (!BgCreatures[i]) {
            LOG_ERROR("sql.sql",
                      "BatteGroundEY: Failed to spawn spirit guides "
...
void BattlegroundEY::EventPlayerDroppedFlag(Player* player)
{
    ...
    _bgEvents.RescheduleEvent(BG_EY_EVENT_FLAG_ON_GROUND,
                                              ...
                                              GameObject* gameObject)
{
    ...
    PlaySoundToAll(player->GetTeamId() == TEAM_ALLIANCE
    ...
    if (player->GetTeamId() == TEAM_ALLIANCE)
        SendBroadcastText(
            BG_EY_TEXT_TAKEN_FLAG, CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
    else
        SendBroadcastText(
            BG_EY_TEXT_TAKEN_FLAG, CHAT_MSG_BG_SYSTEM_HORDE, player);
}
...
void BattlegroundEY::EventTeamLostPoint(Player* player, uint32 point)
{
    ...
    if (oldTeamId == TEAM_ALLIANCE) {
        ...
        SpawnBGObject(m_LosingPointTypes[point].DespawnObjectTypeAlliance,
                      RESPAWN_ONE_DAY);
        SpawnBGObject(m_LosingPointTypes[point].DespawnObjectTypeAlliance + 1,
                      RESPAWN_ONE_DAY);
        SpawnBGObject(m_LosingPointTypes[point].DespawnObjectTypeAlliance + 2,
                      RESPAWN_ONE_DAY);
        SendBroadcastText(m_LosingPointTypes[point].MessageIdAlliance,
                          CHAT_MSG_BG_SYSTEM_ALLIANCE,
                          player);
    }
    else {
        _ownedPointsCount[TEAM_HORDE]--;
        SpawnBGObject(m_LosingPointTypes[point].DespawnObjectTypeHorde,
                      RESPAWN_ONE_DAY);
        SpawnBGObject(m_LosingPointTypes[point].DespawnObjectTypeHorde + 1,
                      RESPAWN_ONE_DAY);
        SpawnBGObject(m_LosingPointTypes[point].DespawnObjectTypeHorde + 2,
                      RESPAWN_ONE_DAY);
    ...
    SpawnBGObject(m_LosingPointTypes[point].SpawnNeutralObjectType,
                  RESPAWN_IMMEDIATELY);
    SpawnBGObject(m_LosingPointTypes[point].SpawnNeutralObjectType + 1,
                  RESPAWN_IMMEDIATELY);
    SpawnBGObject(m_LosingPointTypes[point].SpawnNeutralObjectType + 2,
                  RESPAWN_IMMEDIATELY);
                                            ...
                                            uint32  point)
{
    ...
    SpawnBGObject(m_CapturingPointTypes[point].DespawnNeutralObjectType + 1,
                  RESPAWN_ONE_DAY);
    SpawnBGObject(m_CapturingPointTypes[point].DespawnNeutralObjectType + 2,
                  RESPAWN_ONE_DAY);

    if (teamId == TEAM_ALLIANCE) {
        _ownedPointsCount[TEAM_ALLIANCE]++;
        SpawnBGObject(m_CapturingPointTypes[point].SpawnObjectTypeAlliance,
                      RESPAWN_IMMEDIATELY);
        SpawnBGObject(m_CapturingPointTypes[point].SpawnObjectTypeAlliance + 1,
                      RESPAWN_IMMEDIATELY);
        SpawnBGObject(m_CapturingPointTypes[point].SpawnObjectTypeAlliance + 2,
                      RESPAWN_IMMEDIATELY);
        SendBroadcastText(m_CapturingPointTypes[point].MessageIdAlliance,
                          CHAT_MSG_BG_SYSTEM_ALLIANCE,
                          player);
    }
    else {
        _ownedPointsCount[TEAM_HORDE]++;
        SpawnBGObject(m_CapturingPointTypes[point].SpawnObjectTypeHorde,
                      RESPAWN_IMMEDIATELY);
        SpawnBGObject(m_CapturingPointTypes[point].SpawnObjectTypeHorde + 1,
                      RESPAWN_IMMEDIATELY);
        SpawnBGObject(m_CapturingPointTypes[point].SpawnObjectTypeHorde + 2,
                      RESPAWN_IMMEDIATELY);
    ...
    GraveyardStruct const* sg =
        sGraveyard->GetGraveyard(m_CapturingPointTypes[point].GraveYardId);
    ...
    Creature* trigger =
        GetBgMap()->GetCreature(BgCreatures[BG_EY_TRIGGER_FEL_REAVER + point]);
    if (!trigger)
    ...
    if (trigger) {
        trigger->SetFaction(teamId == TEAM_ALLIANCE ? FACTION_ALLIANCE_GENERIC
                                                    : FACTION_HORDE_GENERIC);
        trigger->CastSpell(trigger, SPELL_HONORABLE_DEFENDER_25Y, true);
    }
}

void BattlegroundEY::EventPlayerCapturedFlag(Player* player,
                                             uint32  BgObjectType)
{
    SetFlagPicker(ObjectGuid::Empty);
    _flagState = BG_EY_FLAG_STATE_ON_BASE;
    ...
    _bgEvents.RescheduleEvent(BG_EY_EVENT_RESPAWN_FLAG,
                              BG_EY_FLAG_RESPAWN_TIME);
    _flagCapturedObject = BgObjectType;

    if (player->GetTeamId() == TEAM_ALLIANCE) {
        PlaySoundToAll(BG_EY_SOUND_FLAG_CAPTURED_ALLIANCE);
        SendBroadcastText(BG_EY_TEXT_ALLIANCE_CAPTURED_FLAG,
                          CHAT_MSG_BG_SYSTEM_ALLIANCE,
                          player);
    }
    else {
        ...
        SendBroadcastText(
            BG_EY_TEXT_HORDE_CAPTURED_FLAG, CHAT_MSG_BG_SYSTEM_HORDE, player);
    }

    UpdatePlayerScore(player, SCORE_FLAG_CAPTURES, 1);
    if (_ownedPointsCount[player->GetTeamId()] > 0)
        AddPoints(player->GetTeamId(),
                  BG_EY_FlagPoints[_ownedPointsCount[player->GetTeamId()] - 1]);
}
...
void BattlegroundEY::FillInitialWorldStates(WorldPacket& data)
{
    ...
    data << uint32(EY_ALLIANCE_BASE)
         << uint32(_ownedPointsCount[TEAM_ALLIANCE]);
    data << uint32(DRAENEI_RUINS_HORDE_CONTROL)
         << uint32(_capturePointInfo[POINT_DRAENEI_RUINS].IsUnderControl(
                TEAM_HORDE));
    data << uint32(DRAENEI_RUINS_ALLIANCE_CONTROL)
         << uint32(_capturePointInfo[POINT_DRAENEI_RUINS].IsUnderControl(
                TEAM_ALLIANCE));
    data << uint32(DRAENEI_RUINS_UNCONTROL)
         << uint32(_capturePointInfo[POINT_DRAENEI_RUINS].IsUncontrolled());
    data << uint32(MAGE_TOWER_ALLIANCE_CONTROL)
         << uint32(
                _capturePointInfo[POINT_MAGE_TOWER].IsUnderControl(TEAM_HORDE));
    data << uint32(MAGE_TOWER_HORDE_CONTROL)
         << uint32(_capturePointInfo[POINT_MAGE_TOWER].IsUnderControl(
                TEAM_ALLIANCE));
    data << uint32(MAGE_TOWER_UNCONTROL)
         << uint32(_capturePointInfo[POINT_MAGE_TOWER].IsUncontrolled());
    data << uint32(FEL_REAVER_HORDE_CONTROL)
         << uint32(
                _capturePointInfo[POINT_FEL_REAVER].IsUnderControl(TEAM_HORDE));
    data << uint32(FEL_REAVER_ALLIANCE_CONTROL)
         << uint32(_capturePointInfo[POINT_FEL_REAVER].IsUnderControl(
                TEAM_ALLIANCE));
    data << uint32(FEL_REAVER_UNCONTROL)
         << uint32(_capturePointInfo[POINT_FEL_REAVER].IsUncontrolled());
    data << uint32(BLOOD_ELF_HORDE_CONTROL)
         << uint32(
                _capturePointInfo[POINT_BLOOD_ELF].IsUnderControl(TEAM_HORDE));
    data << uint32(BLOOD_ELF_ALLIANCE_CONTROL)
         << uint32(_capturePointInfo[POINT_BLOOD_ELF].IsUnderControl(
                TEAM_ALLIANCE));
    data << uint32(BLOOD_ELF_UNCONTROL)
         << uint32(_capturePointInfo[POINT_BLOOD_ELF].IsUncontrolled());
    data << uint32(NETHERSTORM_FLAG)
         << uint32(_flagState == BG_EY_FLAG_STATE_ON_BASE);
    ...
    data << uint32(EY_ALLIANCE_RESOURCES)
         << uint32(GetTeamScore(TEAM_ALLIANCE));
...
GraveyardStruct const* BattlegroundEY::GetClosestGraveyard(Player* player)
{
    GraveyardStruct const* entry = sGraveyard->GetGraveyard(
        static_cast<uint16>(BG_EY_GRAVEYARD_MAIN_ALLIANCE) +
        player->GetTeamId());
    ...
    float dist = (entry->x - pX) * (entry->x - pX) +
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
        if (_capturePointInfo[i].IsUnderControl(player->GetTeamId())) {
            entry =
                sGraveyard->GetGraveyard(m_CapturingPointTypes[i].GraveYardId);
