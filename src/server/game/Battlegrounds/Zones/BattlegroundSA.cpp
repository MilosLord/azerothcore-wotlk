...
BattlegroundSA::BattlegroundSA()
{
    ...
    //! In the future this function should be called BEFORE sending initial
    //! worldstates.
...
bool BattlegroundSA::SetupBattleground() { return ResetObjs(); }
...
bool BattlegroundSA::ResetObjs()
{
    ...
    for (uint8 i = BG_SA_MAXNPC;
         i < static_cast<uint16>(BG_SA_MAXNPC) + BG_SA_MAX_GY;
         i++)
        DelCreature(i);
    ...
    for (uint8 i = 0; i < BG_SA_BOAT_ONE; i++) {
        if (!AddObject(i,
                       BG_SA_ObjEntries[i],
                       BG_SA_ObjSpawnlocs[i][0],
                       BG_SA_ObjSpawnlocs[i][1],
                       BG_SA_ObjSpawnlocs[i][2],
                       BG_SA_ObjSpawnlocs[i][3],
                       0,
                       0,
                       0,
                       0,
    ...
    GetBGObject(BG_SA_BOAT_ONE)
        ->SetTransportPathRotation(0.0f, 0.0f, 1.0f, 0.0002f);
    GetBGObject(BG_SA_BOAT_TWO)
        ->SetTransportPathRotation(0.0f, 0.0f, 1.0f, 0.00001f);
    ...
    for (uint8 i = 0; i < BG_SA_DEMOLISHER_5; i++) {
        if (!AddCreature(BG_SA_NpcEntries[i],
                         i,
                         BG_SA_NpcSpawnlocs[i][0],
                         BG_SA_NpcSpawnlocs[i][1],
                         BG_SA_NpcSpawnlocs[i][2],
                         BG_SA_NpcSpawnlocs[i][3],
                         600))
            return false;
    }
    ...
    for (uint8 i = 0; i < BG_SA_MAX_GY; i++) {
        ...
        if (i == BG_SA_BEACH_GY) {
            ...
            AddSpiritGuide(i + BG_SA_MAXNPC,
                           sg->x,
                           sg->y,
                           sg->z,
                           BG_SA_GYOrientation[i],
                           Attackers);
        }
        else {
            GraveyardStatus[i] = GetOtherTeamId(Attackers);
            if (!AddSpiritGuide(i + BG_SA_MAXNPC,
                                sg->x,
                                sg->y,
                                sg->z,
                                BG_SA_GYOrientation[i],
                                GetOtherTeamId(Attackers)))
    ...
    for (uint8 i = BG_SA_CENTRAL_FLAG; i < BG_SA_PORTAL_DEFFENDER_BLUE; i++) {
        AddObject(i,
                  (BG_SA_ObjEntries[i] - (Attackers == TEAM_ALLIANCE ? 1 : 0)),
                  BG_SA_ObjSpawnlocs[i][0],
                  BG_SA_ObjSpawnlocs[i][1],
                  BG_SA_ObjSpawnlocs[i][2],
                  BG_SA_ObjSpawnlocs[i][3],
    ...
    // Player may enter BEFORE we set up bG - lets update his worldstates
    // anyway...
    UpdateWorldState(
        BG_SA_RIGHT_GY_HORDE,
        GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_HORDE ? 1 : 0);
    UpdateWorldState(
        BG_SA_LEFT_GY_HORDE,
        GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_HORDE ? 1 : 0);
    UpdateWorldState(
        BG_SA_CENTER_GY_HORDE,
        GraveyardStatus[BG_SA_CENTRAL_CAPTURABLE_GY] == TEAM_HORDE ? 1 : 0);

    UpdateWorldState(
        BG_SA_RIGHT_GY_ALLIANCE,
        GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_ALLIANCE ? 1 : 0);
    UpdateWorldState(
        BG_SA_LEFT_GY_ALLIANCE,
        GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_ALLIANCE ? 1 : 0);
    UpdateWorldState(
        BG_SA_CENTER_GY_ALLIANCE,
        GraveyardStatus[BG_SA_CENTRAL_CAPTURABLE_GY] == TEAM_ALLIANCE ? 1 : 0);
    ...
    if (InitSecondRound) {
        if (UpdateWaitTimer < diff) {
            if (!SignaledRoundTwo) {
                ...
                InitSecondRound  = false;
                SendBroadcastText(BG_SA_TEXT_ROUND_TWO_START_ONE_MINUTE,
    ...
    if (Status == BG_SA_WARMUP) {
        ...
        if (TotalTime >= BG_SA_WARMUPLENGTH) {
            ...
            StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_EVENT,
                                  (Attackers == TEAM_ALLIANCE) ? 23748 : 21702);
        }
        ...
        if (TotalTime >= 1min) {
            ...
            // status was set to STATUS_WAIT_JOIN manually for Preparation, set
            // it back now
                ...
                if (_relicClicked) {
                    ...
                    for (BattlegroundPlayerMap::const_iterator itr =
                        ...
                        if (itr->second->GetTeamId() == Attackers)
                            itr->second->UpdateAchievementCriteria(
                                ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET,
                                65246);
                }
                else {
                    // cast this before Attackers variable is switched
                    // cast this spell only upon timer end, no other ability for
                    // defenders to win :)
                    for (BattlegroundPlayerMap::const_iterator itr =
                             GetPlayers().begin();
                         itr != GetPlayers().end();
                         ++itr)
                        itr->second->CastSpell(
                            itr->second, SPELL_SA_END_OF_ROUND, true);
...
void BattlegroundSA::FillInitialWorldStates(WorldPacket& data)
{
    ...
    data << uint32(BG_SA_ANCIENT_GATEWS)
         << uint32(GateStatus[BG_SA_ANCIENT_GATE]);
    data << uint32(BG_SA_YELLOW_GATEWS)
         << uint32(GateStatus[BG_SA_YELLOW_GATE]);
    ...
    data << uint32(BG_SA_PURPLE_GATEWS)
         << uint32(GateStatus[BG_SA_PURPLE_GATE]);
    ...
    data << uint32(BG_SA_ENABLE_TIMER)
         << ((TimerEnabled) ? uint32(1) : uint32(0));
    data << uint32(BG_SA_TIMER_MINS) << uint32(0);
    data << uint32(BG_SA_TIMER_SEC_TENS) << uint32(0);
    data << uint32(BG_SA_TIMER_SEC_DECS) << uint32(0);

    data << uint32(BG_SA_RIGHT_GY_HORDE)
         << uint32(GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_HORDE
                       ? 1
                       : 0);
    data << uint32(BG_SA_LEFT_GY_HORDE)
         << uint32(GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_HORDE ? 1
                                                                           : 0);
    data << uint32(BG_SA_CENTER_GY_HORDE)
         << uint32(GraveyardStatus[BG_SA_CENTRAL_CAPTURABLE_GY] == TEAM_HORDE
                       ? 1
                       : 0);

    data << uint32(BG_SA_RIGHT_GY_ALLIANCE)
         << uint32(GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_ALLIANCE
                       ? 1
                       : 0);
    data << uint32(BG_SA_LEFT_GY_ALLIANCE)
         << uint32(GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_ALLIANCE
                       ? 1
                       : 0);
    data << uint32(BG_SA_CENTER_GY_ALLIANCE)
         << uint32(GraveyardStatus[BG_SA_CENTRAL_CAPTURABLE_GY] == TEAM_ALLIANCE
                       ? 1
                       : 0);
...
void BattlegroundSA::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(player->GetGUID().GetCounter(),
                         new BattlegroundSAScore(player->GetGUID()));
    ...
    if (player->GetTeamId() != Attackers) {
        ...
        if (!ShipsStarted) {
            player->CastSpell(
                player,
                12438,
                true); // Without this player falls before boat loads...
            if (urand(0, 1))
...
void BattlegroundSA::DefendersPortalTeleport(GameObject* portal, Player* plr)
{
    ...
    plr->TeleportTo(plr->GetMapId(),
                    SOTADefPortalDest[portal_num][0],
                    SOTADefPortalDest[portal_num][1],
                    SOTADefPortalDest[portal_num][2],
                    SOTADefPortalDest[portal_num][3],
                    TELE_TO_SPELL);
}
                                          ...
                                          uint32      eventType)
{
    ...
    if (eventType == go->GetGOInfo()->building.destroyedEvent) {
        if (go->GetGOInfo()->building.destroyedEvent == 19837)
            ...
            SendWarningToAll(LANG_BG_SA_WAS_DESTROYED,
    ...
    if (eventType == go->GetGOInfo()->building.damageEvent)
        SendWarningToAll(LANG_BG_SA_IS_UNDER_ATTACK,
                         go->GetGOInfo()->name.c_str());
}
...
void BattlegroundSA::OverrideGunFaction()
{
    ...
    for (uint8 i = BG_SA_GUN_1; i <= BG_SA_GUN_10; i++) {
        if (Creature* gun = GetBGCreature(i))
            gun->SetFaction(
                BG_SA_Factions[Attackers ? TEAM_ALLIANCE : TEAM_HORDE]);
    }
...
void BattlegroundSA::DemolisherStartState(bool start)
{
    ...
    for (uint8 i = BG_SA_GUN_1; i <= BG_SA_GUN_10; i++)
        if (Creature* gun = GetBGCreature(i)) {
            if (start)
                gun->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
                                 UNIT_FLAG_NOT_SELECTABLE);
            else
                gun->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
                                    UNIT_FLAG_NOT_SELECTABLE);
        }
    ...
    if (GameObject* g = GetBGObject(i)) {
        if (g->GetGOValue()->Building.Health == 0) {
            ...
            if (player) {
                ...
                if (rewardHonor)
                    UpdatePlayerScore(
                        player, SCORE_BONUS_HONOR, GetBonusHonorFromKill(1));
            }
        }
    }
}
...
GraveyardStruct const* BattlegroundSA::GetClosestGraveyard(Player* player)
{
    ...
    for (uint8 i = BG_SA_BEACH_GY; i < BG_SA_MAX_GY; i++) {
        ...
        GraveyardStruct const* ret =
            sGraveyard->GetGraveyard(BG_SA_GYEntries[i]);
    ...
    if (GameObject* go = GetBGObject(objectId)) {
        if (CanInteractWithObject(objectId))
            go->RemoveGameObjectFlag(GO_FLAG_NOT_SELECTABLE |
                                     GO_FLAG_INTERACT_COND | GO_FLAG_IN_USE);
        else
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player* Source)
{
    ...
    GuidVector& ghost_list =
        m_ReviveQueue[BgCreatures[static_cast<uint16>(BG_SA_MAXNPC) + i]];
    if (!ghost_list.empty()) {
    ...
    if (!sg) {
        LOG_ERROR("bg.battleground",
                  "BattlegroundSA::CaptureGraveyard: non-existant GY entry: {}",
                  BG_SA_GYEntries[i]);
        return;
    }

    AddSpiritGuide(i + static_cast<uint16>(BG_SA_MAXNPC),
                   sg->x,
                   sg->y,
                   sg->z,
                   BG_SA_GYOrientation[i],
    ...
    case BG_SA_LEFT_CAPTURABLE_GY:
        ...
        DelObject(flag);
        AddObject(flag,
                  (BG_SA_ObjEntries[flag] -
                   (Source->GetTeamId() == TEAM_ALLIANCE ? 0 : 1)),
                  BG_SA_ObjSpawnlocs[flag][0],
                  BG_SA_ObjSpawnlocs[flag][1],
                  BG_SA_ObjSpawnlocs[flag][2],
                  BG_SA_ObjSpawnlocs[flag][3],
                  0,
                  0,
                  0,
                  0,
        ...
        AddCreature(BG_SA_NpcEntries[npc],
                    npc,
                    Attackers,
                    BG_SA_NpcSpawnlocs[npc][0],
                    BG_SA_NpcSpawnlocs[npc][1],
                    BG_SA_NpcSpawnlocs[npc][2],
                    BG_SA_NpcSpawnlocs[npc][3]);

        for (uint8 j = BG_SA_DEMOLISHER_7; j <= BG_SA_DEMOLISHER_8; j++) {
            AddCreature(BG_SA_NpcEntries[j],
                        j,
                        BG_SA_NpcSpawnlocs[j][0],
                        BG_SA_NpcSpawnlocs[j][1],
                        BG_SA_NpcSpawnlocs[j][2],
                        BG_SA_NpcSpawnlocs[j][3],
                        600);
        ...
        UpdateWorldState(BG_SA_LEFT_GY_ALLIANCE,
                         (GraveyardStatus[i] == TEAM_ALLIANCE ? 1 : 0));
        UpdateWorldState(BG_SA_LEFT_GY_HORDE,
                         (GraveyardStatus[i] == TEAM_ALLIANCE ? 0 : 1));
    ...
    case BG_SA_RIGHT_CAPTURABLE_GY:
        ...
        UpdateWorldState(BG_SA_RIGHT_GY_ALLIANCE,
                         (GraveyardStatus[i] == TEAM_ALLIANCE ? 1 : 0));
        UpdateWorldState(BG_SA_RIGHT_GY_HORDE,
                         (GraveyardStatus[i] == TEAM_ALLIANCE ? 0 : 1));
        if (Source->GetTeamId() == TEAM_ALLIANCE)
    ...
    case BG_SA_CENTRAL_CAPTURABLE_GY:
        ...
        UpdateWorldState(BG_SA_CENTER_GY_ALLIANCE,
                         (GraveyardStatus[i] == TEAM_ALLIANCE ? 1 : 0));
        UpdateWorldState(BG_SA_CENTER_GY_HORDE,
                         (GraveyardStatus[i] == TEAM_ALLIANCE ? 0 : 1));
        if (Source->GetTeamId() == TEAM_ALLIANCE)
...
void BattlegroundSA::EventPlayerUsedGO(Player* Source, GameObject* object)
{
    if (object->GetEntry() == BG_SA_ObjEntries[BG_SA_TITAN_RELIC] &&
        CanInteractWithObject(BG_SA_TITAN_RELIC)) {
        if (Source->GetTeamId() == Attackers) {
            if (Source->GetTeamId() == TEAM_ALLIANCE)
                SendMessageToAll(LANG_BG_SA_ALLIANCE_CAPTURED_RELIC,
                                 CHAT_MSG_BG_SYSTEM_NEUTRAL);
            else
                SendMessageToAll(LANG_BG_SA_HORDE_CAPTURED_RELIC,
                                 CHAT_MSG_BG_SYSTEM_NEUTRAL);
            ...
            if (Status == BG_SA_ROUND_ONE) {
                ...
                for (BattlegroundPlayerMap::const_iterator itr =
                    ...
                    if (itr->second->GetTeamId() == Attackers &&
                        RoundScores[1].winner == Attackers)
                        itr->second->UpdateAchievementCriteria(
                            ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 65246);
        ...
        if (BgCreatures[i]) {
            if (Creature* Demolisher = GetBGCreature(i)) {
                if (Demolisher->isDead()) {
                    ...
                    if (DemoliserRespawnList.find(i) ==
                        DemoliserRespawnList.end()) {
                        DemoliserRespawnList[i] =
                            GameTime::GetGameTimeMS().count() + 30000;
                    }
                    else {
                        if (DemoliserRespawnList[i] <
                            GameTime::GetGameTimeMS().count()) {
                            Demolisher->Relocate(BG_SA_NpcSpawnlocs[i][0],
                                                 BG_SA_NpcSpawnlocs[i][1],
                                                 BG_SA_NpcSpawnlocs[i][2],
                                                 BG_SA_NpcSpawnlocs[i][3]);
