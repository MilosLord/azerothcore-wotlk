                                ...
                                uint32        queueTotal)
{
    ...
    if (bg) {
        // When limited, it announces only if there are at least
        // CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_LIMIT_MIN_PLAYERS in queue
        auto limitQueueMinLevel = sWorld->getIntConfig(
        ...
        if (limitQueueMinLevel && minLevel >= limitQueueMinLevel) {
            ...
            auto bgTypeToLimit =
                minLevel == 80 ? BATTLEGROUND_RB : BATTLEGROUND_WS;
