...
void ArenaTeamScore::BuildRatingInfoBlock(WorldPacket& data)
{
    ...
    // should be old rating, new rating, and client will calculate rating change
    // itself
...
void ArenaTeamScore::BuildTeamInfoBlock(WorldPacket& data) { data << TeamName; }
...
Arena::Arena()
{
    ...
    StartMessageIds[BG_STARTING_EVENT_FOURTH] =
...
void Arena::AddPlayer(Player* player)
{
    ...
    PlayerScores.emplace(
        ...
        new ArenaScore(player->GetGUID(), player->GetBgTeamId()));
    ...
    if (group) {
        ...
        // Without this, players in a crossfaction arena group would not be able
        // to cast beneficial spells on their teammates

        std::vector<Player*> members;
        bool                 isCrossfaction = false;
        for (Group::member_citerator mitr = group->GetMemberSlots().begin();
             mitr != group->GetMemberSlots().end();
             ++mitr) {
...
{
    data << uint32(ARENA_WORLD_STATE_ALIVE_PLAYERS_GREEN)
         << uint32(GetAlivePlayersCountByTeam(TEAM_HORDE));
    data << uint32(ARENA_WORLD_STATE_ALIVE_PLAYERS_GOLD)
         << uint32(GetAlivePlayersCountByTeam(TEAM_ALLIANCE));
}

void Arena::UpdateArenaWorldState()
{
    UpdateWorldState(ARENA_WORLD_STATE_ALIVE_PLAYERS_GREEN,
                     GetAlivePlayersCountByTeam(TEAM_HORDE));
    UpdateWorldState(ARENA_WORLD_STATE_ALIVE_PLAYERS_GOLD,
                     GetAlivePlayersCountByTeam(TEAM_ALLIANCE));
}
...
void Arena::RemovePlayerAtLeave(Player* player)
{
    if (isRated() && GetStatus() == STATUS_IN_PROGRESS) {
        if (auto const& member = Acore::Containers::MapGetValuePtr(
                m_Players, player->GetGUID())) {
            // if the player was a match participant, calculate rating
            auto teamId = member->GetBgTeamId();

            ArenaTeam* winnerArenaTeam = sArenaTeamMgr->GetArenaTeamById(
                GetArenaTeamIdForTeam(GetOtherTeamId(teamId)));
            ArenaTeam* loserArenaTeam =
                sArenaTeamMgr->GetArenaTeamById(GetArenaTeamIdForTeam(teamId));

            if (winnerArenaTeam && loserArenaTeam &&
                winnerArenaTeam != loserArenaTeam)
                loserArenaTeam->MemberLost(
                    player, GetArenaMatchmakerRating(GetOtherTeamId(teamId)));
        }
    }
...
void Arena::CheckWinConditions()
{
    ...
    else if (GetPlayersCountByTeam(TEAM_ALLIANCE) &&
             !GetAlivePlayersCountByTeam(TEAM_HORDE))
...
void Arena::EndBattleground(TeamId winnerTeamId)
{
    if (isRated()) {
        ...
        bool   bValidArena =
            ...
            GetStartTime() >=
                startDelay +
                    15000; // pussywizard: only if arena lasted at least 15 secs
        ...
        ArenaTeam* winnerArenaTeam =
            sArenaTeamMgr->GetArenaTeamById(GetArenaTeamIdForTeam(
                winnerTeamId == TEAM_NEUTRAL ? TEAM_HORDE : winnerTeamId));
        ArenaTeam* loserArenaTeam =
            sArenaTeamMgr->GetArenaTeamById(GetArenaTeamIdForTeam(
                winnerTeamId == TEAM_NEUTRAL ? TEAM_ALLIANCE
                                             : GetOtherTeamId(winnerTeamId)));
        ...
        auto SaveArenaLogs = [&]() {
            ...
            stmt2->SetData(
                2,
                ((GetStartTime() <= startDelay ? 0
                                               : GetStartTime() - startDelay) /
                 1000));
            ...
            uint8 memberId = 0;
            for (auto const& [playerGuid, arenaLogEntryData] :
                 ArenaLogEntries) {
                auto const& score = PlayerScores.find(playerGuid.GetCounter());
                stmt2             = CharacterDatabase.GetPreparedStatement(
        ...
        if (winnerArenaTeam && loserArenaTeam &&
            ...
            loserMatchmakerRating =
                GetArenaMatchmakerRating(GetOtherTeamId(winnerTeamId));
            ...
            if (winnerTeamId != TEAM_NEUTRAL) {
                winnerMatchmakerChange =
                    bValidArena
                        ? winnerArenaTeam->WonAgainst(winnerMatchmakerRating,
                                                      loserMatchmakerRating,
                                                      winnerChange,
                                                      GetBgMap())
                        : 0;
                loserMatchmakerChange =
                    loserArenaTeam->LostAgainst(loserMatchmakerRating,
                                                winnerMatchmakerRating,
                                                loserChange,
                                                GetBgMap());

                sScriptMgr->OnAfterArenaRatingCalculation(
                    this,
                    winnerMatchmakerChange,
                    loserMatchmakerChange,
                    winnerChange,
                ...
                LOG_DEBUG(
                    "bg.arena",
                    "match Type: {} --- Winner: old rating: {}, rating gain: "
                    "{}, old MMR: {}, MMR gain: {} --- Loser: old rating: {}, "
                    "rating loss: {}, old MMR: {}, MMR loss: {} ---",
                    GetArenaType(),
                    winnerTeamRating,
                    winnerChange,
                    winnerMatchmakerRating,
                ...
                SetArenaMatchmakerRating(winnerTeamId,
                                         winnerMatchmakerRating +
                                             winnerMatchmakerChange);
                SetArenaMatchmakerRating(GetOtherTeamId(winnerTeamId),
                                         loserMatchmakerRating +
                                             loserMatchmakerChange);

                // bg team that the client expects is different to TeamId
                // alliance 1, horde 0
                uint8 winnerTeam = winnerTeamId == TEAM_ALLIANCE
                                       ? PVP_TEAM_ALLIANCE
                                       : PVP_TEAM_HORDE;
                uint8 loserTeam  = winnerTeamId == TEAM_ALLIANCE
                                       ? PVP_TEAM_HORDE
                                       : PVP_TEAM_ALLIANCE;

                _arenaTeamScores[winnerTeam].Assign(winnerChange,
                                                    winnerMatchmakerRating,
                                                    winnerArenaTeam->GetName());
                _arenaTeamScores[loserTeam].Assign(loserChange,
                                                   loserMatchmakerRating,
                                                   loserArenaTeam->GetName());

                LOG_DEBUG(
                    "bg.arena",
                    "Arena match Type: {} for Team1Id: {} - Team2Id: {} ended. "
                    "WinnerTeamId: {}. Winner rating: +{}, Loser rating: {}",
                    GetArenaType(),
                    GetArenaTeamIdForTeam(TEAM_ALLIANCE),
                    GetArenaTeamIdForTeam(TEAM_HORDE),
                    winnerArenaTeam->GetId(),
                    winnerChange,
                    loserChange);
            }
            else // Deduct 16 points from each teams arena-rating if there are
                 // no winners after 45+2 minutes
            {
                // pussywizard: in case of a draw, the following is always true:
                // winnerArenaTeam => TEAM_HORDE, loserArenaTeam =>
                // TEAM_ALLIANCE
                ...
                _arenaTeamScores[PVP_TEAM_ALLIANCE].Assign(
                    ARENA_TIMELIMIT_POINTS_LOSS,
                    winnerMatchmakerRating,
                    winnerArenaTeam->GetName());
                _arenaTeamScores[PVP_TEAM_HORDE].Assign(
                    ARENA_TIMELIMIT_POINTS_LOSS,
                    loserMatchmakerRating,
                    loserArenaTeam->GetName());
            ...
            if (winnerArenaTeam && loserArenaTeam &&
                ...
                if (bgTeamId == winnerTeamId) {
                    if (bValidArena) {
                        ...
                        uint32 rating = player->GetArenaPersonalRating(
                            winnerArenaTeam->GetSlot());
                        player->UpdateAchievementCriteria(
                            ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA,
                            rating ? rating : 1);
                        player->UpdateAchievementCriteria(
                            ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA, GetMapId());

                        // Last standing - Rated 5v5 arena & be solely alive
                        // player
                        if (GetArenaType() == ARENA_TYPE_5v5 &&
                            aliveWinners == 1 && player->IsAlive()) {
                            player->CastSpell(
                                player, SPELL_LAST_MAN_STANDING, true);
                        }

                        winnerArenaTeam->MemberWon(player,
                                                   loserMatchmakerRating,
                                                   winnerMatchmakerChange);
                    }
                }
                    ...
                    loserArenaTeam->MemberLost(
                        player, winnerMatchmakerRating, loserMatchmakerChange);

                    // Arena lost => reset the win_rated_arena having the
                    // "no_lose" condition
                    player->ResetAchievementCriteria(
                        ACHIEVEMENT_CRITERIA_CONDITION_NO_LOSE, 0);
                }

                player->UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_PLAY_ARENA, GetMapId());
            }
        }
