...
ArenaTeam::ArenaTeam()
    : TeamId(0), Type(0), TeamName(), BackgroundColor(0), EmblemStyle(0),
      EmblemColor(0), BorderStyle(0), BorderColor(0)
{
    Stats.WeekGames   = 0;
    Stats.SeasonGames = 0;
    Stats.Rank        = 0;
                       ...
                       uint32             borderColor)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_ARENA_TEAM);
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    // xinef: Get player name and class from player storage or global data
    // storage
    ...
    if ((player && player->GetArenaTeamId(GetSlot())) ||
        sCharacterCache->GetCharacterArenaTeamIdByGuid(playerGuid, GetSlot()) !=
            0) {
        LOG_ERROR("bg.arena",
    ...
    // xinef: sync query
    // Try to get player's match maker rating from db and fall back to config
    // setting if not found
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_MATCH_MAKER_RATING);
    ...
    // This will prevent player from joining too many arena teams and corrupt
    // arena team data integrity
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
    ...
    do {
        ...
        // Prevent crash if db records are broken when all members in result are
        // already processed and current team doesn't have any members
        if (!fields)
            break;
        ...
        newMember.MatchMakerRating =
            fields[9].Get<uint16>() > 0
                ? fields[9].Get<uint16>()
                : sWorld->getIntConfig(CONFIG_ARENA_START_MATCHMAKER_RATING);
        newMember.MaxMMR =
            std::max(fields[10].Get<uint16>(), newMember.MatchMakerRating);

        // Delete member if character information is missing
        if (fields[6].Get<std::string>().empty()) {
            LOG_ERROR("sql.sql",
                      "ArenaTeam {} has member with empty name - probably "
                      "player {} doesn't exist, deleting him from memberlist!",
                      arenaTeamId,
        ...
        sCharacterCache->UpdateCharacterArenaTeamId(
            newMember.Guid, GetSlot(), GetId());
    } while (result->NextRow());

    if (Empty() || !captainPresentInTeam) {
        // Arena team is empty or captain is not in team, delete from db
        LOG_DEBUG("bg.battleground",
                  "ArenaTeam {} does not have any members or its captain is "
                  "not in team, disbanding it...",
                  TeamId);
        return false;
    }
...
{
    if (TeamName == name || name.empty() || name.length() > 24 ||
    ...
    TeamName = name;
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_ARENA_TEAM_NAME);
...
void ArenaTeam::SetCaptain(ObjectGuid guid)
{
    ...
    // Update database
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_ARENA_TEAM_CAPTAIN);
    ...
    if (Player* newCaptain = ObjectAccessor::FindConnectedPlayer(guid)) {
        ...
        /*if (oldCaptain)
        {
            LOG_DEBUG("bg.battleground", "Player: {} [{}] promoted player: {}
        [{}] to leader of arena team [Id: {}] [Type: {}].",
                oldCaptain->GetName(), oldCaptain->GetGUID().ToString(),
        newCaptain->GetName(), newCaptain->GetGUID().ToString(), GetId(),
        GetType());
        }*/
    }
}
...
void ArenaTeam::DelMember(ObjectGuid guid, bool cleanDb)
{
    ...
    Group*  group =
        (player && player->GetGroup()) ? player->GetGroup() : nullptr;

    // Remove member from team
    for (MemberList::iterator itr = Members.begin(); itr != Members.end();
         ++itr) {
        // Remove queues of members
        if (Player* playerMember =
                ObjectAccessor::FindConnectedPlayer(itr->Guid)) {
            if (group && playerMember->GetGroup() &&
                group->GetGUID() == playerMember->GetGroup()->GetGUID()) {
                if (BattlegroundQueueTypeId bgQueue =
                        BattlegroundMgr::BGQueueTypeId(BATTLEGROUND_AA,
                                                       GetType())) {
                    GroupQueueInfo     ginfo;
                    BattlegroundQueue& queue =
                        sBattlegroundMgr->GetBattlegroundQueue(bgQueue);
                    if (queue.GetPlayerGroupInfoData(playerMember->GetGUID(),
                                                     &ginfo)) {
    ...
    if (player) {
        player->GetSession()->SendArenaTeamCommandResult(
            ERR_ARENA_TEAM_QUIT_S, GetName(), "", 0);
        // delete all info regarding this team
        for (uint32 i = 0; i < ARENA_TEAM_END; ++i)
            player->SetArenaTeamInfoField(GetSlot(), ArenaTeamInfoType(i), 0);
    }

    // Only used for single member deletion, for arena team disband we use a
    // single query for more efficiency
    if (cleanDb) {
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_DEL_ARENA_TEAM_MEMBER);
...
void ArenaTeam::Disband(WorldSession* session)
{
    ...
    // Broadcast update
    if (session) {
        BroadcastEvent(ERR_ARENA_TEAM_DISBANDED_S,
                       ObjectGuid::Empty,
                       2,
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_ARENA_TEAM);
    stmt->SetData(0, TeamId);
    trans->Append(stmt);
...
void ArenaTeam::Roster(WorldSession* session)
{
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
        ...
        data << uint32((itr->Guid == GetCaptain()
                            ? 0
                            : 1)); // captain flag 0 captain 1 member
...
{
    WorldPacket data(SMSG_ARENA_TEAM_QUERY_RESPONSE,
                     4 * 7 + GetName().size() + 1);
    data << uint32(GetId());         // team id
...
void ArenaTeam::NotifyStatsChanged()
{
    ...
    // Updates arena team stats for every member of the team (not only the ones
    // who participated!)
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
         ++itr)
...
void ArenaTeam::Inspect(WorldSession* session, ObjectGuid guid)
{
    ...
    data << uint32(member->SeasonGames); // played (count of all games, that the
                                         // inspected member participated...)
                                           ...
                                           uint32  type)
{
    ...
    if (player && ArenaTeam::GetSlotByType(type) < 3) {
        player->SetArenaTeamInfoField(ArenaTeam::GetSlotByType(type),
...
void ArenaTeamMember::ModifyMatchmakerRating(int32 mod, uint32 /*slot*/)
{
    if (mod < 0) {
        ...
        int32 maxAllowedDrop =
            (int32)sWorld->getIntConfig(CONFIG_MAX_ALLOWED_MMR_DROP);
        mod = std::min<int32>(std::max<int32>(-((int32)MatchMakerRating -
                               ...
                               std::string const& str3)
{
    ...
    default:
        LOG_ERROR("bg.arena",
                  "Unhandled strCount {} in ArenaTeam::BroadcastEvent",
                  strCount);
        return;
    }
...
uint8 ArenaTeam::GetSlotByType(uint32 type)
{
    ...
    if (itr == ArenaSlotByType.end()) {
        LOG_ERROR(
            "bg.arena", "Unknown arena team type {} for some arena team", type);
        return slot;
    }
...
{
    // Returns how many points would be awarded with this team type with this
    // rating
    float points;

    uint32 rating =
        memberRating + 150 < Stats.Rating ? memberRating : Stats.Rating;

    if (rating <= 1500) {
        if (sWorld->getIntConfig(CONFIG_ARENA_SEASON_ID) < 6 &&
            !sWorld->getIntConfig(CONFIG_LEGACY_ARENA_POINTS_CALC))
        ...
        points =
            1511.26f / (1.0f + 1639.28f * std::exp(-0.00412f * (float)rating));
...
{
    // Returns the chance to win against a team with the given rating, used in
    // the rating adjustment calculation ELO system
    return 1.0f /
           (1.0f + (std::exp(std::log(10.0f) *
                             (float)((float)opponentRating - (float)ownRating) /
                             650.0f)));
}
                                        ...
                                        bool won /*, float& confidence_factor*/)
{
    ...
    confidence_factor -= ((confidence_factor - 1.0f) * confidence_mod) /
    confidence_factor;
    */
                              ...
                              bool   won /*, float confidence_factor*/)
{
    ...
    /// @todo: Replace this hack with using the confidence factor (limiting the
    /// factor to 2.0f)
    if (won) {
        if (ownRating < 1300) {
            float win_rating_modifier1 =
                sWorld->getFloatConfig(CONFIG_ARENA_WIN_RATING_MODIFIER_1);

            if (ownRating < 1000)
                mod = win_rating_modifier1 * (1.0f - chance);
            else
                mod = ((win_rating_modifier1 / 2.0f) +
                       ((win_rating_modifier1 / 2.0f) *
                        (1300.0f - float(ownRating)) / 300.0f)) *
                      (1.0f - chance);
        }
        else
            mod = sWorld->getFloatConfig(CONFIG_ARENA_WIN_RATING_MODIFIER_2) *
                  (1.0f - chance);
    }
    else
        mod = sWorld->getFloatConfig(CONFIG_ARENA_LOSE_RATING_MODIFIER) *
              (-chance);
...
void ArenaTeam::FinishGame(int32 mod, const Map* bgMap)
{
    ...
    // Update team's rank, start with rank 1 and increase until no team with
    // more rating was found
    Stats.Rank = 1;
    ArenaTeamMgr::ArenaTeamContainer::const_iterator i =
        sArenaTeamMgr->GetArenaTeamMapBegin();
    for (; i != sArenaTeamMgr->GetArenaTeamMapEnd(); ++i) {
        if (i->second->GetType() == Type &&
            i->second->GetStats().Rating > Stats.Rating)
            ++Stats.Rank;
    }
}
                           ...
                           int32   MatchmakerRatingChange)
{
    ...
    for (MemberList::iterator itr = Members.begin(); itr != Members.end();
        ...
        if (itr->Guid == player->GetGUID()) {
            ...
            int32 mod = GetRatingMod(
                itr->PersonalRating, againstMatchmakerRating, false);
            ...
            player->SetArenaTeamInfoField(
                GetSlot(), ARENA_TEAM_GAMES_WEEK, itr->WeekGames);
            player->SetArenaTeamInfoField(
                GetSlot(), ARENA_TEAM_GAMES_SEASON, itr->SeasonGames);
            return;
        }
    }
}
                          ...
                          int32   MatchmakerRatingChange)
{
    ...
    for (MemberList::iterator itr = Members.begin(); itr != Members.end();
        ...
        if (itr->Guid == player->GetGUID()) {
            // update personal rating
            int32 mod = GetRatingMod(
                itr->PersonalRating, againstMatchmakerRating, true);
            sScriptMgr->OnBeforeUpdatingPersonalRating(mod, GetType());
            itr->ModifyPersonalRating(player, mod, GetType());

            // update matchmaker rating (pussywizard: but don't allow it to go
            // over team rating)
            if (itr->MatchMakerRating < Stats.Rating) {
                mod = std::min(MatchmakerRatingChange,
                               Stats.Rating - itr->MatchMakerRating);
                sScriptMgr->OnBeforeUpdatingPersonalRating(mod, GetType());
    ...
    std::map<ObjectGuid, uint32>& playerPoints)
{
    ...
    // Helper function for arena point distribution (this way, when
    // distributing, no actual calculation is required, just a few comparisons)
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
        ...
        std::map<ObjectGuid, uint32>::iterator plr_itr =
            playerPoints.find(itr->Guid);
...
void ArenaTeam::SaveToDB(bool forceMemberSave)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_ARENA_TEAM_STATS);
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
        ...
        stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_UPD_ARENA_TEAM_MEMBER);
                                    ...
                                    std::string const&   teamName)
{
    ...
    for (auto const& _player : playerList) {
        ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById(
            _player->GetArenaTeamId(GetSlotByType(type)));
        if (!team)
            continue;
...
std::unordered_map<uint8, uint8> ArenaTeam::ArenaReqPlayersForType = {
    {ARENA_TYPE_2v2, 4}, {ARENA_TYPE_3v3, 6}, {ARENA_TYPE_5v5, 10}};
