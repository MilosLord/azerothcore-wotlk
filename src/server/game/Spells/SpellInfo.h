...
enum SpellCastTargetFlags {
    ...
    TARGET_FLAG_UNIT_RAID =
        0x00000004, // not sent, used to validate target (if raid member)
    TARGET_FLAG_UNIT_PARTY =
        0x00000008, // not sent, used to validate target (if party member)
    TARGET_FLAG_ITEM            = 0x00000010, // pguid
    TARGET_FLAG_SOURCE_LOCATION = 0x00000020, // pguid, 3 float
    TARGET_FLAG_DEST_LOCATION   = 0x00000040, // pguid, 3 float
    TARGET_FLAG_UNIT_ENEMY =
        0x00000080, // not sent, used to validate target (if enemy)
    TARGET_FLAG_UNIT_ALLY =
        0x00000100, // not sent, used to validate target (if ally)
    TARGET_FLAG_CORPSE_ENEMY = 0x00000200, // pguid
    TARGET_FLAG_UNIT_DEAD =
        0x00000400, // not sent, used to validate target (if dead creature)
    TARGET_FLAG_GAMEOBJECT =
        0x00000800, // pguid, used with TARGET_GAMEOBJECT_TARGET
    TARGET_FLAG_TRADE_ITEM = 0x00001000, // pguid
    TARGET_FLAG_STRING     = 0x00002000, // string
    TARGET_FLAG_GAMEOBJECT_ITEM =
        0x00004000, // not sent, used with TARGET_GAMEOBJECT_ITEM_TARGET
    TARGET_FLAG_CORPSE_ALLY = 0x00008000, // pguid
    TARGET_FLAG_UNIT_MINIPET =
        0x00010000, // pguid, used to validate target (if non combat pet)
    TARGET_FLAG_GLYPH_SLOT = 0x00020000, // used in glyph spells
    TARGET_FLAG_DEST_TARGET =
        0x00040000, // sometimes appears with DEST_TARGET spells (may appear or
                    // not for a given spell)
    TARGET_FLAG_UNUSED20 = 0x00080000, // uint32 counter, loop { vec3 - screen
                                       // position (?), guid }, not used so far
    TARGET_FLAG_UNIT_PASSENGER =
        0x00100000, // guessed, used to validate target (if vehicle passenger)

    TARGET_FLAG_UNIT_MASK =
        TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_RAID | TARGET_FLAG_UNIT_PARTY |
    ...
    TARGET_FLAG_ITEM_MASK =
        TARGET_FLAG_TRADE_ITEM | TARGET_FLAG_ITEM | TARGET_FLAG_GAMEOBJECT_ITEM,
};
...
enum SpellCustomAttributes {
    ...
    SPELL_ATTR0_CU_NEGATIVE = SPELL_ATTR0_CU_NEGATIVE_EFF0 |
                              SPELL_ATTR0_CU_NEGATIVE_EFF1 |
                              SPELL_ATTR0_CU_NEGATIVE_EFF2,
    SPELL_ATTR0_CU_POSITIVE = SPELL_ATTR0_CU_POSITIVE_EFF0 |
...
private:
    struct StaticData {
        SpellTargetObjectTypes
            ObjectType; // type of object returned by target type
        SpellTargetReferenceTypes
            ReferenceType; // defines which object is used as a reference when
                           // selecting target
        ...
        SpellTargetDirectionTypes
            DirectionType; // direction for cone and dest targets
    };
...
public:
    ...
    SpellEffectInfo()
        : _spellInfo(nullptr), _effIndex(0), Effect(0), ApplyAuraName(0),
          Amplitude(0), DieSides(0), RealPointsPerLevel(0), BasePoints(0),
          PointsPerComboPoint(0), ValueMultiplier(0), DamageMultiplier(0),
          BonusMultiplier(0), MiscValue(0), MiscValueB(0),
          Mechanic(MECHANIC_NONE), RadiusEntry(nullptr), ChainTarget(0),
          ItemType(0), TriggerSpell(0), ImplicitTargetConditions(nullptr)
    {
    }
    SpellEffectInfo(SpellEntry const* spellEntry,
    ...
    int32 CalcValue(Unit const*  caster     = nullptr,
    ...
    uint32 GetMissingTargetMask(bool   srcSet  = false,
                                bool   destSet = false,
                                uint32 mask    = 0) const;
...
private:
    struct StaticData {
        SpellEffectImplicitTargetTypes
            ImplicitTargetType; // defines what target can be added to effect
                                // target list if there's no valid target type
                                // provided for effect
        SpellTargetObjectTypes
            UsedTargetObjectType; // defines valid target object type for spell
                                  // effect
    };
...
public:
    ...
    inline bool HasAttribute(SpellAttr0 attribute) const
    {
        return (Attributes & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr1 attribute) const
    {
        return (AttributesEx & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr2 attribute) const
    {
        return (AttributesEx2 & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr3 attribute) const
    {
        return (AttributesEx3 & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr4 attribute) const
    {
        return (AttributesEx4 & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr5 attribute) const
    {
        return (AttributesEx5 & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr6 attribute) const
    {
        return (AttributesEx6 & attribute) != 0;
    }
    inline bool HasAttribute(SpellAttr7 attribute) const
    {
    ...
    bool NeedsToBeTriggeredByCaster(SpellInfo const* triggeringSpell,
                                    uint8 effIndex = MAX_SPELL_EFFECTS) const;
    ...
    bool
    IsAuraExclusiveBySpecificPerCasterWith(SpellInfo const* spellInfo) const;

    SpellCastResult CheckShapeshift(uint32 form) const;
    SpellCastResult CheckLocation(uint32  map_id,
                                  uint32  zone_id,
                                  uint32  area_id,
                                  Player* player = nullptr,
                                  bool    strict = true) const;
    SpellCastResult CheckTarget(Unit const*        caster,
                                WorldObject const* target,
                                bool               implicit = true) const;
    SpellCastResult CheckExplicitTarget(Unit const*        caster,
                                        WorldObject const* target,
                                        Item const* itemTarget = nullptr) const;
    ...
    bool ValidateAttribute6SpellDamageMods(Unit const*       caster,
    ...
    float GetMaxRange(bool   positive = false,
                      Unit*  caster   = nullptr,
    ...
    int32 CalcPowerCost(Unit const*     caster,
