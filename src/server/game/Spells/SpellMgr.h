...
enum SpellFamilyFlag {
    ...
    SPELLFAMILYFLAG_ROGUE_VAN_EVAS_SPRINT =
        0x00000860, // Vanish, Evasion, Sprint
    SPELLFAMILYFLAG1_ROGUE_COLDB_SHADOWSTEP =
        0x00000240,                                   // Cold Blood, Shadowstep
    ...
    SPELLFAMILYFLAG_SHAMAN_TOTEM_EFFECTS =
        0x04000000, // Seems to be linked to most totems and some totem effects
};
...
enum ProcFlags {
    ...
    PROC_FLAG_KILLED =
        0x00000001, // 00 Killed by agressor - not sure about this flag
    PROC_FLAG_KILL =
        0x00000002, // 01 Kill target (in most cases need XP/Honor reward)

    PROC_FLAG_DONE_MELEE_AUTO_ATTACK = 0x00000004, // 02 Done melee auto attack
    PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK =
        0x00000008, // 03 Taken melee auto attack

    PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS =
        0x00000010, // 04 Done attack by Spell that has dmg class melee
    PROC_FLAG_TAKEN_SPELL_MELEE_DMG_CLASS =
        0x00000020, // 05 Taken attack by Spell that has dmg class melee

    PROC_FLAG_DONE_RANGED_AUTO_ATTACK =
        0x00000040, // 06 Done ranged auto attack
    PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK =
        0x00000080, // 07 Taken ranged auto attack

    PROC_FLAG_DONE_SPELL_RANGED_DMG_CLASS =
        0x00000100, // 08 Done attack by Spell that has dmg class ranged
    PROC_FLAG_TAKEN_SPELL_RANGED_DMG_CLASS =
        0x00000200, // 09 Taken attack by Spell that has dmg class ranged

    PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS =
        0x00000400, // 10 Done positive spell that has dmg class none
    PROC_FLAG_TAKEN_SPELL_NONE_DMG_CLASS_POS =
        0x00000800, // 11 Taken positive spell that has dmg class none

    PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_NEG =
        0x00001000, // 12 Done negative spell that has dmg class none
    PROC_FLAG_TAKEN_SPELL_NONE_DMG_CLASS_NEG =
        0x00002000, // 13 Taken negative spell that has dmg class none

    PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS =
        0x00004000, // 14 Done positive spell that has dmg class magic
    PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS =
        0x00008000, // 15 Taken positive spell that has dmg class magic

    PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG =
        0x00010000, // 16 Done negative spell that has dmg class magic
    PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG =
        0x00020000, // 17 Taken negative spell that has dmg class magic

    PROC_FLAG_DONE_PERIODIC =
        0x00040000, // 18 Successful do periodic (damage / healing)
    PROC_FLAG_TAKEN_PERIODIC =
        0x00080000, // 19 Taken spell periodic (damage / healing)

    PROC_FLAG_TAKEN_DAMAGE = 0x00100000, // 20 Taken any damage
    PROC_FLAG_DONE_TRAP_ACTIVATION =
        0x00200000, // 21 On trap activation (possibly needs name change to
                    // ON_GAMEOBJECT_CAST or USE)

    PROC_FLAG_DONE_MAINHAND_ATTACK =
        0x00400000, // 22 Done main-hand melee attacks (spell and autoattack)
    PROC_FLAG_DONE_OFFHAND_ATTACK =
        0x00800000, // 23 Done off-hand melee attacks (spell and autoattack)
    ...
    MELEE_PROC_FLAG_MASK =
        PROC_FLAG_DONE_MELEE_AUTO_ATTACK | PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK |
        PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS |
        PROC_FLAG_TAKEN_SPELL_MELEE_DMG_CLASS | PROC_FLAG_DONE_MAINHAND_ATTACK |
        PROC_FLAG_DONE_OFFHAND_ATTACK,

    RANGED_PROC_FLAG_MASK = PROC_FLAG_DONE_RANGED_AUTO_ATTACK |
                            PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK |
                            PROC_FLAG_DONE_SPELL_RANGED_DMG_CLASS |
                            PROC_FLAG_TAKEN_SPELL_RANGED_DMG_CLASS,
    ...
    SPELL_CAST_PROC_FLAG_MASK = SPELL_PROC_FLAG_MASK |
                                PROC_FLAG_DONE_TRAP_ACTIVATION |
    ...
    DONE_HIT_PROC_FLAG_MASK =
        PROC_FLAG_DONE_MELEE_AUTO_ATTACK | PROC_FLAG_DONE_RANGED_AUTO_ATTACK |
        PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS |
        PROC_FLAG_DONE_SPELL_RANGED_DMG_CLASS |
        PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS |
        PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_NEG |
    ...
    TAKEN_HIT_PROC_FLAG_MASK =
        PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK | PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK |
        PROC_FLAG_TAKEN_SPELL_MELEE_DMG_CLASS |
        PROC_FLAG_TAKEN_SPELL_RANGED_DMG_CLASS |
        PROC_FLAG_TAKEN_SPELL_NONE_DMG_CLASS_POS |
        PROC_FLAG_TAKEN_SPELL_NONE_DMG_CLASS_NEG |
        PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS |
        PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG | PROC_FLAG_TAKEN_PERIODIC |
        PROC_FLAG_TAKEN_DAMAGE,

    REQ_SPELL_PHASE_PROC_FLAG_MASK =
        SPELL_PROC_FLAG_MASK & DONE_HIT_PROC_FLAG_MASK,
};
...
enum ProcFlagsExLegacy {
    ...
    PROC_EX_NOT_ACTIVE_SPELL =
        0x0008000, // Spell mustn't do damage/heal to proc
    PROC_EX_EX_TRIGGER_ALWAYS =
        0x0010000, // If set trigger always no matter of hit result
    PROC_EX_EX_ONE_TIME_TRIGGER = 0x0020000, // If set trigger always but only
                                             // one time (not implemented yet)
    PROC_EX_ONLY_ACTIVE_SPELL =
        0x0040000,                       // Spell has to do damage/heal to proc
    ...
    PROC_EX_ONLY_FIRST_TICK =
        0x0200000, // Proc only on first tick (in case of periodic spells)
...
#define AURA_SPELL_PROC_EX_MASK                                                \
    (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT | PROC_EX_MISS |                \
     ...
     PROC_EX_EVADE | PROC_EX_IMMUNE | PROC_EX_DEFLECT | PROC_EX_ABSORB |       \
     PROC_EX_REFLECT | PROC_EX_INTERRUPT)
...
enum ProcFlagsSpellType {
    ...
    PROC_SPELL_TYPE_MASK_ALL = PROC_SPELL_TYPE_DAMAGE | PROC_SPELL_TYPE_HEAL |
...
enum ProcFlagsSpellPhase {
    ...
    PROC_SPELL_PHASE_MASK_ALL =
        PROC_SPELL_PHASE_CAST | PROC_SPELL_PHASE_HIT | PROC_SPELL_PHASE_FINISH
};

enum ProcFlagsHit {
    PROC_HIT_NONE = 0x0000000, // no value - PROC_HIT_NORMAL | PROC_HIT_CRITICAL
                               // for TAKEN proc type, PROC_HIT_NORMAL |
                               // PROC_HIT_CRITICAL | PROC_HIT_ABSORB for DONE
...
struct SpellProcEventEntry {
    uint32
        schoolMask; // if nonzero - bit mask for matching proc condition based
                    // on spell candidate's school: Fire=2, Mask=1<<(2-1)=2
    uint32 spellFamilyName; // if nonzero - for matching proc condition based on
                            // candidate spell's SpellFamilyNamer value
    flag96 spellFamilyMask; // if nonzero - for matching proc condition based on
                            // candidate spell's SpellFamilyFlags  (like auras
                            // 107 and 108 do)
...
struct SpellProcEntry {
    uint32 SchoolMask; // if nonzero - bitmask for matching proc condition based
                       // on spell's school
    uint32 SpellFamilyName; // if nonzero - for matching proc condition based on
                            // candidate spell's SpellFamilyName
    flag96 SpellFamilyMask; // if nonzero - bitmask for matching proc condition
                            // based on candidate spell's SpellFamilyFlags
    uint32 ProcFlags;       // if nonzero - owerwrite procFlags field for given
                      // Spell.dbc entry, bitmask for matching proc condition,
                      // see enum ProcFlags
    uint32 SpellTypeMask; // if nonzero - bitmask for matching proc condition
                          // based on candidate spell's damage/heal effects, see
                          // enum ProcFlagsSpellType
    uint32 SpellPhaseMask; // if nonzero - bitmask for matching phase of a
                           // spellcast on which proc occurs, see enum
                           // ProcFlagsSpellPhase
    uint32 HitMask; // if nonzero - bitmask for matching proc condition based on
                    // hit result, see enum ProcFlagsHit
    uint32 AttributesMask; // bitmask, see ProcAttributes
    float  ProcsPerMinute; // if nonzero - chance to proc is equal to value *
                           // aura caster's weapon speed / 60
    float Chance; // if nonzero - owerwrite procChance field for given Spell.dbc
                  // entry, defines chance of proc to occur, not used if
                  // ProcsPerMinute set
    Milliseconds Cooldown; // if nonzero - cooldown in secs for aura proc,
                           // applied to aura
    uint32 Charges;        // if nonzero - owerwrite procCharges field for given
                    // Spell.dbc entry, defines how many times proc can occur
                    // before aura remove, 0 - infinite
};
...
enum EnchantProcAttributes {
    ENCHANT_PROC_ATTR_EXCLUSIVE =
        0x1, // Only one instance of that effect can be active
    ENCHANT_PROC_ATTR_WHITE_HIT =
        0x2 // Enchant shall only proc off white hits (not abilities)
};
...
typedef std::unordered_map<uint32, SpellEnchantProcEntry>
...
enum SpellGroupSpecialFlags {
    ...
    SPELL_GROUP_SPECIAL_FLAG_FORCED_STRONGEST =
        0x010, // xinef: specially helpful flag if some spells have different
               // auras, but only one should be present
    ...
    SPELL_GROUP_SPECIAL_FLAG_FLASK = SPELL_GROUP_SPECIAL_FLAG_ELIXIR_BATTLE |
...
typedef std::map<std::pair<uint32 /*spell_id*/, SpellEffIndex /*effIndex*/>,
                 SpellTargetPosition>
...
public:
    ...
    [[nodiscard]] int32 GetDamage() const { return damage; }
...
struct SpellArea {
    ...
    int32 auraSpell; // spell aura must be applied for spell apply)if possitive)
                     // and it must not be applied in other case
    uint32 raceMask; // can be applied only to races
    Gender gender;   // can be applied only to gender
    uint32 questStartStatus; // QuestStatus that quest_start must have in order
                             // to keep the spell
    uint32 questEndStatus; // QuestStatus that the quest_end must have in order
                           // to keep the spell (if the quest_end's status is
                           // different than this, the spell will be dropped)
...
public:
    ...
    static bool ComputeIsSpellValid(SpellInfo const* spellInfo,
                                    bool             msg = true);
    static bool IsSpellValid(SpellInfo const* spellInfo);
    static bool
    CheckSpellValid(SpellInfo const* spellInfo, uint32 spellId, bool isTalent);
    ...
    SpellInfo const* GetSpellForDifficultyFromSpell(SpellInfo const* spell,
                                                    Unit const* caster) const;

    // Spell Ranks table
    [[nodiscard]] SpellChainNode const*
                         GetSpellChainNode(uint32 spell_id) const;
    ...
    [[nodiscard]] uint32
    GetSpellWithRank(uint32 spell_id, uint32 rank, bool strict = false) const;

    // Spell Required table
    [[nodiscard]] SpellRequiredMapBounds
    GetSpellsRequiredForSpellBounds(uint32 spell_id) const;
    [[nodiscard]] SpellsRequiringSpellMapBounds
                       GetSpellsRequiringSpellBounds(uint32 spell_id) const;
    [[nodiscard]] bool IsSpellRequiringSpell(uint32 spellid,
                                             uint32 req_spellid) const;

    // Spell learning
    [[nodiscard]] SpellLearnSkillNode const*
    GetSpellLearnSkill(uint32 spell_id) const;

    // Spell target coordinates
    [[nodiscard]] SpellTargetPosition const*
    GetSpellTargetPosition(uint32 spell_id, SpellEffIndex effIndex) const;

    // Spell Groups
    [[nodiscard]] uint32 GetSpellGroup(uint32 spellid) const;
    [[nodiscard]] SpellGroupSpecialFlags
    GetSpellGroupSpecialFlags(uint32 spell_id) const;
    [[nodiscard]] SpellGroupStackFlags GetGroupStackFlags(uint32 groupid) const;
    SpellGroupStackFlags CheckSpellGroupStackRules(SpellInfo const* spellInfo1,
                                                   SpellInfo const* spellInfo2,
                                                   bool             remove,
                                                   bool areaAura) const;
    void                 GetSetOfSpellsInSpellGroupWithFlag(
                        uint32                 group_id,
                        SpellGroupSpecialFlags flag,
                        std::set<uint32>&      availableElixirs) const;

    // Spell proc event table
    [[nodiscard]] SpellProcEventEntry const*
    GetSpellProcEvent(uint32 spellId) const;
    bool
    IsSpellProcEventCanTriggeredBy(SpellInfo const*           spellProto,
    ...
    bool CanSpellTriggerProcOnEvent(SpellProcEntry const& procEntry,
                                    ProcEventInfo&        eventInfo) const;

    // Spell bonus data table
    [[nodiscard]] SpellBonusEntry const*
    GetSpellBonusData(uint32 spellId) const;

    // Spell threat table
    [[nodiscard]] SpellThreatEntry const*
    GetSpellThreatEntry(uint32 spellID) const;

    // Spell mixology table
    [[nodiscard]] float GetSpellMixologyBonus(uint32 spellId) const;

    [[nodiscard]] SkillLineAbilityMapBounds
    GetSkillLineAbilityMapBounds(uint32 spell_id) const;

    [[nodiscard]] PetAura const* GetPetAura(uint32 spell_id, uint8 eff) const;

    [[nodiscard]] SpellEnchantProcEntry const*
                       GetSpellEnchantProcEvent(uint32 enchId) const;
    [[nodiscard]] bool IsArenaAllowedEnchancment(uint32 ench_id) const;

    [[nodiscard]] const std::vector<int32>*
    GetSpellLinked(int32 spell_id) const;

    [[nodiscard]] PetLevelupSpellSet const*
    GetPetLevelupSpellList(uint32 petFamily) const;
    [[nodiscard]] PetDefaultSpellsEntry const*
    GetPetDefaultSpellsEntry(int32 id) const;

    // Spell area
    [[nodiscard]] SpellAreaMapBounds
    GetSpellAreaMapBounds(uint32 spell_id) const;
    [[nodiscard]] SpellAreaForQuestMapBounds
    GetSpellAreaForQuestMapBounds(uint32 quest_id) const;
    [[nodiscard]] SpellAreaForQuestMapBounds
    GetSpellAreaForQuestEndMapBounds(uint32 quest_id) const;
    [[nodiscard]] SpellAreaForAuraMapBounds
    GetSpellAreaForAuraMapBounds(uint32 spell_id) const;
    [[nodiscard]] SpellAreaForAreaMapBounds
    GetSpellAreaForAreaMapBounds(uint32 area_id) const;

    // SpellInfo object management
    [[nodiscard]] SpellInfo const* GetSpellInfo(uint32 spellId) const
    {
        return spellId < GetSpellInfoStoreSize() ? mSpellInfoMap[spellId]
                                                 : nullptr;
    }
    ...
    // use this instead of AssertSpellInfo to have the problem logged instead of
    // crashing the server
    [[nodiscard]] SpellInfo const* CheckSpellInfo(uint32 spellId) const
    {
        if (spellId >= GetSpellInfoStoreSize()) {
            LOG_ERROR(
                "server",
                "spellId {} is not lower than GetSpellInfoStoreSize() ({})",
                spellId,
    ...
    [[nodiscard]] uint32 GetSpellInfoStoreSize() const
    {
    ...
    [[nodiscard]] SpellCooldownOverride
    GetSpellCooldownOverride(uint32 spellId) const;
...
private:
    ...
    PetDefaultSpellsMap
        mPetDefaultSpellsMap; // only spells not listed in related
                              // mPetLevelupSpellMap entry
