...
// update aura target map every 500 ms instead of every update - reduce amount
// of grid searcher calls
...
AuraApplication::AuraApplication(Unit* target,
    ...
    : _target(target), _base(aura), _removeMode(AURA_REMOVE_NONE),
      _slot(MAX_AURAS), _flags(AFLAG_NONE), _effectsToApply(effMask),
      _needClientUpdate(false), _disableMask(0)
{
    ...
    if (GetBase()->CanBeSentToClient()) {
        ...
        if (AuraApplication* foundAura =
                GetTarget()->GetAuraApplication(GetBase()->GetId(),
                                                GetBase()->GetCasterGUID(),
                                                GetBase()->GetCastItemGUID())) {
        ...
        if (slot < MAX_AURAS) {
            ...
            LOG_DEBUG("spells.aura",
                      "Aura: {} Effect: {} put to unit visible auras slot: {}",
                      GetBase()->GetId(),
                      GetEffectMask(),
                      slot);
        }
            ...
            LOG_ERROR(
                "spells.aura",
                "Aura: {} Effect: {} could not find empty unit visible slot",
                GetBase()->GetId(),
                GetEffectMask());
        }
    }
...
void AuraApplication::_Remove()
{
    ...
    if (AuraApplication* foundAura =
            _target->GetAuraApplication(GetBase()->GetId(),
                                        GetBase()->GetCasterGUID(),
                                        GetBase()->GetCastItemGUID())) {
        // Reuse visible aura slot by aura which is still applied - prevent
        // storing dead pointers
...
void AuraApplication::_InitFlags(Unit* caster, uint8 effMask)
{
    ...
    _flags |= (GetBase()->GetCasterGUID() == GetTarget()->GetGUID())
                  ? AFLAG_CASTER
                  : AFLAG_NONE;
    ...
    if (IsSelfcasted() || !caster || !caster->IsFriendlyTo(GetTarget())) {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            if (((1 << i) & effMask) &&
                !GetBase()->GetSpellInfo()->IsPositiveEffect(i)) {
                negativeFound = true;
                break;
            }
        }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    LOG_DEBUG("spells.aura",
              "AuraApplication::_HandleEffect: {}, apply: {}: amount: {}",
              aurEff->GetAuraType(),
              apply,
              aurEff->GetAmount());
    ...
    if (uint32 groupId = aurEff->GetAuraGroup()) {
        ...
        if (!aurEff->IsPeriodic() &&
            (sFlag & SPELL_GROUP_STACK_FLAG_EFFECT_EXCLUSIVE)) {
            ...
            Unit::AuraEffectList const& auraList =
                target->GetAuraEffectsByType(aurEff->GetAuraType());
            for (Unit::AuraEffectList::const_iterator iter = auraList.begin();
                 iter != auraList.end();
                 ++iter) {
                if ((*iter)->GetAuraGroup() != groupId ||
                    (*iter) == strongestEff || (*iter)->GetBase()->IsRemoved())
                    continue;

                // xinef: skip different misc values
                if (aurEff->GetAuraType() !=
                        SPELL_AURA_230 /*SPELL_AURA_MOD_INCREASE_HEALTH_2*/
                    &&
                    aurEff->GetAuraType() !=
                ...
                AuraApplication* aurApp =
                    (*iter)->GetBase()->GetApplicationOfTarget(
                        target->GetGUID());
                if (!aurApp)
                    continue;

                if (amount < std::abs((*iter)->GetForcedAmount())) {
                    // xinef: if we have strongest aura and it is active, turn
                    // it off xinef: otherwise just save new aura;
                    if (strongestApp && strongestEff &&
                        strongestApp->IsActive(strongestEff->GetEffIndex())) {
                        strongestEff->HandleEffect(
                            strongestApp,
                            AURA_EFFECT_HANDLE_CHANGE_AMOUNT,
                            false);
                        if (!strongestEff->GetSpellInfo()->HasAreaAuraEffect())
                            strongestEff->SetEnabled(false);
                        strongestApp->SetDisableMask(
                            strongestEff->GetEffIndex());
                    }
                    strongestApp = aurApp;
                    strongestEff = (*iter);
                ...
                // xinef: itered aura is weaker, deactivate if active
                    ...
                    (*iter)->HandleEffect(
                        aurApp, AURA_EFFECT_HANDLE_CHANGE_AMOUNT, false);
            ...
            if (strongestApp && strongestEff &&
                ...
                strongestEff->HandleEffect(
                    strongestApp, AURA_EFFECT_HANDLE_CHANGE_AMOUNT, true);
            }
        }
    }
    SetNeedClientUpdate();
...
void AuraApplication::BuildUpdatePacket(ByteBuffer& data, bool remove) const
{
    ...
    uint32 flags = _flags;
    if (aura->GetMaxDuration() > 0 && !aura->GetSpellInfo()->HasAttribute(
                                          SPELL_ATTR5_DO_NOT_DISPLAY_DURATION))
        flags |= AFLAG_DURATION;
    data << uint8(flags);
    data << uint8(aura->GetCasterLevel());
    // send stack amount for aura which could be stacked (never 0 - causes
    // incorrect display) or charges stack amount has priority over charges
    // (checked on retail with spell 50262)
    data << uint8(aura->GetSpellInfo()->StackAmount ? aura->GetStackAmount()
                                                    : aura->GetCharges());
    ...
    if (GetSlot() < MAX_AURAS)
        if (Player const* plr = GetTarget()->ToPlayer())
            if (Aura* aura = GetBase())
                if (plr->NeedSendSpectatorData() &&
                    ArenaSpectator::ShouldSendAura(
                        aura, GetEffectMask(), GetTarget()->GetGUID(), remove))
                    ArenaSpectator::SendCommand_Aura(
                        plr->FindMap(),
                        plr->GetGUID(),
                        "AUR",
                        aura->GetCasterGUID(),
                        aura->GetSpellInfo()->Id,
                        aura->GetSpellInfo()->IsPositive(),
                        aura->GetSpellInfo()->Dispel,
                        aura->GetDuration(),
                        aura->GetMaxDuration(),
                        (aura->GetCharges() > 1 ? aura->GetCharges()
                                                : aura->GetStackAmount()),
                        remove);
    ...
    bool             periodicReset /*= false*/)
{
    ...
    if (refresh)
        *refresh = false;
    uint8 effMask =
        Aura::BuildEffectMaskForOwner(spellproto, tryEffMask, owner);
    if (!effMask)
        return nullptr;
    if (Aura* foundAura = owner->ToUnit()->_TryStackingOrRefreshingExistingAura(
            spellproto,
            effMask,
            caster,
            baseAmount,
            ...
            casterGUID,
            periodicReset)) {
        // we've here aura, which script triggered removal after modding stack
        // amount check the state here, so we won't create new Aura object
                   ...
                   ObjectGuid       itemGUID /*= ObjectGuid::Empty*/)
{
    ...
    if (owner->isType(TYPEMASK_UNIT))
        if (!owner->IsInWorld() || ((Unit*)owner)->IsDuringRemoveFromWorld())
            // owner not in world so don't allow to own not self casted single
            // target auras
    ...
    case TYPEID_PLAYER:
        ...
        break;
    case TYPEID_DYNAMICOBJECT:
        aura = new DynObjAura(spellproto,
                              effMask,
                              owner,
                              caster,
                              baseAmount,
...
Aura::Aura(SpellInfo const* spellproto,
    ...
    : m_spellInfo(spellproto),
      ...
      m_castItemGuid(itemGUID   ? itemGUID
                     : castItem ? castItem->GetGUID()
                                : ObjectGuid::Empty),
      m_castItemEntry(castItem ? castItem->GetEntry() : 0),
      m_applyTime(GameTime::GetGameTime().count()), m_owner(owner),
      m_timeCla(0), m_updateTargetMapInterval(0),
      m_casterLevel(caster ? caster->GetLevel() : m_spellInfo->SpellLevel),
      m_procCharges(0), m_stackAmount(1), m_isRemoved(false),
      m_isSingleTarget(false), m_isUsingCharges(false),
      m_triggeredByAuraSpellInfo(nullptr)
{
    if ((m_spellInfo->ManaPerSecond || m_spellInfo->ManaPerSecondPerLevel) &&
        !m_spellInfo->HasAttribute(SPELL_ATTR2_NO_TARGET_PER_SECOND_COST))
    ...
    // m_casterLevel = cast item level/caster level, caster level should be
    // saved to db, confirmed with sniffs
}

AuraScript* Aura::GetScriptByName(std::string const& scriptName) const
{
    for (std::list<AuraScript*>::const_iterator itr = m_loadedScripts.begin();
...
{
    // shouldn't be in constructor - functions in AuraEffect::AuraEffect use
    // polymorphism
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        if (effMask & (uint8(1) << i))
            m_effects[i] = new AuraEffect(
                this, i, baseAmount ? baseAmount + i : nullptr, caster);
        else
            m_effects[i] = nullptr;
    }
}
...
uint32 Aura::GetId() const { return GetSpellInfo()->Id; }
...
AuraObjectType Aura::GetType() const
{
    ...
    return (m_owner->GetTypeId() == TYPEID_DYNAMICOBJECT) ? DYNOBJ_AURA_TYPE
                                                          : UNIT_AURA_TYPE;
}
...
void Aura::_ApplyForTarget(Unit* target, Unit* caster, AuraApplication* auraApp)
{
    ...
    ASSERT(!IsAppliedOnTarget(target->GetGUID()) &&
           "Aura::_ApplyForTarget: aura musn't be already applied on target");
    ...
    if (m_spellInfo->IsCooldownStartedOnEvent() && !m_castItemGuid && caster) {
        if (caster->IsPlayer()) {
            caster->ToPlayer()->AddSpellAndCategoryCooldowns(
                m_spellInfo, 0, nullptr, true);
        }
        else {
            caster->ToCreature()->AddSpellCooldown(
                m_spellInfo->Id, 0, infinityCooldownDelay);
        }
    }
}
                             ...
                             AuraApplication* auraApp)
{
    ...
    if (itr == m_applications.end()) {
        LOG_ERROR("spells.aura",
                  "Aura::_UnapplyForTarget, target:{}, caster:{}, spell:{} was "
                  "not found in owners application map!",
...
void Aura::UpdateTargetMap(Unit* caster, bool apply)
{
    ...
    for (ApplicationMap::iterator appIter = m_applications.begin();
         appIter != m_applications.end();
         ++appIter) {
        std::map<Unit*, uint8>::iterator existing =
            targets.find(appIter->second->GetTarget());
        // not found in current area - remove the aura
        if (existing == targets.end())
            targetsToRemove.push_back(appIter->second->GetTarget());
        else {
            // xinef: check immunities here, so aura wont get removed on every
            // tick and then reapplied
            if (IsArea())
                for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS;
                     ++effIndex) {
                    if ((existing->second & (1 << effIndex)) &&
                        existing->first->IsImmunedToSpellEffect(GetSpellInfo(),
                                                                effIndex))
                        existing->second &= ~(1 << effIndex);
                }

            // needs readding - remove now, will be applied in next update cycle
            // (dbcs do not have auras which apply on same type of targets but
            // have different radius, so this is not really needed)
    ...
    for (std::map<Unit*, uint8>::iterator itr = targets.begin();
        ...
        if (AuraApplication* aurApp =
                GetApplicationOfTarget(itr->first->GetGUID())) {
            ...
            // this may cause area aura "bouncing" between 2 units after each
            // update but because we know the reason of a crash we can remove
        ...
        if (!itr->second || itr->first->IsImmunedToSpell(GetSpellInfo()) ||
        ...
        if (addUnit) {
            ...
            {
                // Allow to remove by stack when aura is going to be applied on
                // owner
                if (itr->first != GetOwner()) {
                    // check if not stacking aura already on target
                    // this one prevents unwanted usefull buff loss because of
                    // stacking and prevents overriding auras periodicaly by 2
                    // near area aura owners
        ...
        if (!addUnit)
            ...
            if (!GetOwner()->IsSelfOrInSameMap(itr->first)) {
                ...
                LOG_FATAL(
                    "spells.aura",
                    "Aura {}: Owner {} (map {}) is not in the same map as "
                    "target {} (map {}).",
                    GetSpellInfo()->Id,
                    GetOwner()->GetName(),
                    GetOwner()->IsInWorld() ? GetOwner()->GetMap()->GetId()
                                            : uint32(-1),
                    itr->first->GetName(),
                    itr->first->IsInWorld() ? itr->first->GetMap()->GetId()
                                            : uint32(-1));
                ABORT();
            }
    ...
    for (std::map<Unit*, uint8>::iterator itr = targets.begin();
        ...
        if (AuraApplication* aurApp =
                GetApplicationOfTarget(itr->first->GetGUID())) {
            // owner has to be in world, or effect has to be applied to self
            ASSERT((!GetOwner()->IsInWorld() && GetOwner() == itr->first) ||
                   GetOwner()->IsInMap(itr->first));
            itr->first->_ApplyAura(aurApp, itr->second);
        }
    }
}

// targets have to be registered and not have effect applied yet to use this
// function
void Aura::_ApplyEffectForTargets(uint8 effIndex)
{
    ...
    for (UnitList::iterator itr = targetList.begin(); itr != targetList.end();
        ...
        if (GetApplicationOfTarget((*itr)->GetGUID())) {
            // owner has to be in world, or effect has to be applied to self
            ASSERT((!GetOwner()->IsInWorld() && GetOwner() == *itr) ||
                   GetOwner()->IsInMap(*itr));
        ...
        if (m_timeCla) {
            if (m_timeCla > int32(diff))
                ...
                if (int32 ManaPerSecond = m_spellInfo->ManaPerSecond +
                                          m_spellInfo->ManaPerSecondPerLevel *
                                              caster->GetLevel()) {
                    m_timeCla += 1000 - diff;
...
int32 Aura::CalcMaxDuration(Unit* caster) const
{
    ...
    // IsPermanent() checks max duration (which we are supposed to calculate
    // here)
...
void Aura::RefreshTimersWithMods()
{
    Unit* caster  = GetCaster();
    m_maxDuration = CalcMaxDuration();
    if ((caster && caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE,
                                                     m_spellInfo)) ||
        m_spellInfo->HasAttribute(SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC)) {
        m_maxDuration =
            int32(m_maxDuration * caster->GetFloatValue(UNIT_MOD_CAST_SPEED));
    }

    // xinef: we should take ModSpellDuration into account, but none of the
    // spells using this function is affected by contents of ModSpellDuration
    RefreshDuration();
...
bool Aura::ModCharges(int32 num, AuraRemoveMode removeMode)
{
    if (IsUsingCharges()) {
        ...
        // limit charges (only on charges increase, charges may be changed
        // manually)
...
void Aura::SetStackAmount(uint8 stackAmount)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (HasEffect(i))
            m_effects[i]->ChangeAmount(
                m_effects[i]->CalculateAmount(caster), false, true);
                          ...
                          bool           periodicReset /*= false*/)
{
    ...
    // limit the stack amount (only on stack increase, stack amount may be
    // changed manually)
    ...
    bool refresh = stackAmount >= GetStackAmount() &&
                   (m_spellInfo->StackAmount ||
                    !m_spellInfo->HasAttribute(SPELL_ATTR1_AURA_UNIQUE));
    ...
    if (refresh) {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (AuraEffect* aurEff = GetEffect(i))
                if (aurEff->GetAuraType() == SPELL_AURA_ADD_FLAT_MODIFIER ||
                    aurEff->GetAuraType() == SPELL_AURA_ADD_PCT_MODIFIER)
...
{
    for (Aura::ApplicationMap::const_iterator appIter = m_applications.begin();
         appIter != m_applications.end();
         ++appIter)
...
{
    return (
        GetCasterGUID() == target->GetGUID() && m_spellInfo->Stances &&
        !m_spellInfo->HasAttribute(SPELL_ATTR2_ALLOW_WHILE_NOT_SHAPESHIFTED) &&
...
bool Aura::CanBeSaved() const
{
    ...
    if (IsPassive() || (spellInfo->HasAttribute(SPELL_ATTR0_DO_NOT_DISPLAY) &&
                        spellInfo->Stances)) {
        return false;
    }
    ...
    if (GetCasterGUID() != GetOwner()->GetGUID() &&
        (GetSpellInfo()->IsSingleTarget() || IsSingleTarget())) {
        return false;
    }
...
void Aura::UnregisterSingleTarget()
{
    ...
    if (!caster) {
        LOG_INFO("spells",
                 "Aura::UnregisterSingleTarget: (A1) - {}, {}, {}, {}",
                 GetId(),
                 GetOwner()->GetTypeId(),
                 GetOwner()->GetEntry(),
                 GetOwner()->GetName());
        LOG_ERROR(
            "spells",
            "Aura::UnregisterSingleTarget: No caster was found."); // ASSERT(caster);
    }
    else
    ...
    if (Unit* caster = GetCaster())
        if (Player* modOwner = caster->GetSpellModOwner())
            modOwner->ApplySpellMod(
                GetId(), SPELLMOD_RESIST_DISPEL_CHANCE, resistChance);

    // Dispel resistance from target SPELL_AURA_MOD_DISPEL_RESIST
    // Only affects offensive dispels
    if (offensive && auraTarget)
        resistChance +=
            auraTarget->GetTotalAuraModifier(SPELL_AURA_MOD_DISPEL_RESIST);
                                  ...
                                  bool                   onReapply)
{
    ...
    if (!onReapply) {
        ...
        if (apply) {
            if (std::vector<int32> const* spellTriggered =
                    sSpellMgr->GetSpellLinked(GetId() + SPELL_LINK_AURA)) {
                for (std::vector<int32>::const_iterator itr =
                         spellTriggered->begin();
                     itr != spellTriggered->end();
                     ++itr) {
                    if (*itr < 0)
                        target->ApplySpellImmune(
                            GetId(), IMMUNITY_ID, -(*itr), true);
                    else if (caster)
            ...
            // remove linked auras
            if (std::vector<int32> const* spellTriggered =
                    sSpellMgr->GetSpellLinked(-(int32)GetId())) {
                for (std::vector<int32>::const_iterator itr =
                         spellTriggered->begin();
                     itr != spellTriggered->end();
                     ++itr) {
            ...
            if (std::vector<int32> const* spellTriggered =
                ...
                for (std::vector<int32>::const_iterator itr =
                    ...
                    if (*itr < 0)
                        target->ApplySpellImmune(
                            GetId(), IMMUNITY_ID, -(*itr), false);
                    else
                        target->RemoveAura(
                            *itr, GetCasterGUID(), 0, removeMode);
                }
            }
        }
    }
        ...
        if (std::vector<int32> const* spellTriggered =
            ...
            for (std::vector<int32>::const_iterator itr =
                 ...
                 ++itr)
                if (*itr > 0)
                    if (Aura* triggeredAura =
                            target->GetAura(*itr, GetCasterGUID()))
                        triggeredAura->ModStackAmount(
    ...
    if (apply) {
        ...
        case SPELLFAMILY_MAGE:
            if (!caster)
                break;
            if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00000001 &&
                GetSpellInfo()->SpellFamilyFlags[2] & 0x00000008) {
                // Glyph of Fireball
                if (caster->HasAura(56368))
                    SetDuration(0);
            }
            else if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00000020 &&
                     GetSpellInfo()->SpellVisual[0] == 13) {
                // Glyph of Frostbolt
            ...
            /// @todo: This should be moved to similar function in spell::hit
                ...
                if (GetSpellInfo()->SpellIconID == 82 &&
            ...
            case 12043: // Presence of Mind
                ...
                if (AuraEffect const* aurEff = caster->GetAuraEffect(
                        SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0)) {
                    uint32 spellId = 0;
                    ...
                    default:
                        LOG_ERROR("spells.aura",
                                  "Aura::HandleAuraSpecificMods: Unknown rank "
                                  "of Arcane Potency ({}) found",
                                  aurEff->GetId());
                    }
                    if (spellId)
            ...
            {
                // See if we already have the indicator aura. If not, create
                // one.
                if (Aura* aur = target->GetAura(74396)) {
                    // Aura already there. Refresh duration and set original
                    // charges
                    aur->SetCharges(2);
                    aur->RefreshDuration();
                }
            ...
            case 12494: // Frostbite, synchronise with Fingers of Frost
            {
                ...
                if (AuraEffect* aurEff =
                        caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL,
                                              SPELLFAMILY_MAGE,
                                              2947,
                                              EFFECT_0)) {
                    if (SpellInfo const* triggeringSpellInfo =
                            GetTriggeredByAuraSpellInfo()) {
                        uint8 fbRank =
                            sSpellMgr->GetSpellRank(triggeringSpellInfo->Id);
                        uint8 fofRank =
                            sSpellMgr->GetSpellRank(aurEff->GetId());
                        uint8 chance =
                            uint8(std::ceil(fofRank * fbRank * 16.6f));
            ...
            if (GetSpellInfo()->SpellFamilyFlags[0] & 0x10 && GetEffect(0)) {
                ...
                if (caster->GetAuraEffectDummy(64760)) {
                    ...
                    damage        = target->SpellHealingBonusTaken(
                        caster, GetSpellInfo(), damage, DOT);
        ...
        case SPELLFAMILY_PRIEST:
            ...
            // Power Word: Shield
            else if (m_spellInfo->SpellFamilyFlags[0] & 0x1 &&
                     m_spellInfo->SpellFamilyFlags[2] & 0x400 && GetEffect(0)) {
                // Glyph of Power Word: Shield
                if (AuraEffect* glyph = caster->GetAuraEffect(55672, 0)) {
                    // instantly heal m_amount% of the absorb-value
                    int32 heal =
                        glyph->GetAmount() * GetEffect(0)->GetAmount() / 100;
                    caster->CastCustomSpell(GetUnitOwner(),
                                            56160,
                                            &heal,
                                            nullptr,
                                            nullptr,
        ...
        case SPELLFAMILY_ROGUE:
            ...
            if (GetSpellInfo()->SpellFamilyFlags[0] & 0x40 &&
                GetSpellInfo()->GetCategory() == 44)
                // in official maybe there is only one icon?
                if (target->HasAura(58039)) // Glyph of Blurred Speed
                    target->CastSpell(
                        target, 61922, true); // Sprint (waterwalk)
            break;
        case SPELLFAMILY_SHAMAN: {
            // Ghost Wolf Speed (PvP 58 lvl set)
            if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00000800 &&
                target->HasAura(22801) && target->GetLevel() <= 60) {
                int32 bp0 = 15;
            ...
            if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2) {
                ...
                if (aurEff) {
                    ...
                    default:
                        LOG_ERROR("spells.aura",
                                  "Aura::HandleAuraSpecificMods: Unknown rank "
                                  "of Crypt Fever/Ebon Plague ({}) found",
                                  aurEff->GetId());
                    }
        ...
        case SPELLFAMILY_POTION:
            // Alchemy: Mixology
            if (caster && caster->HasAura(53042) &&
                caster->GetTypeId() == TYPEID_PLAYER &&
                !caster->ToPlayer()->GetSession()->PlayerLoading()) {
                if (sSpellMgr->GetSpellGroup(GetId()) == 1) /*Elixirs*/
                {
                    if (caster->HasSpell(
                        ...
                        for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
                            if (GetEffect(i))
                                GetEffect(i)->SetAmount(CalculatePct(
                                    GetEffect(i)->GetAmount(),
                                    100 + sSpellMgr->GetSpellMixologyBonus(
                                              GetId())));

                        SetMaxDuration(
                            caster->CalcSpellDuration(GetSpellInfo()) * 2);
                        SetDuration(GetMaxDuration());
                    }
                }
            }
    ...
    // mods at aura remove
        ...
        case SPELLFAMILY_GENERIC:
            ...
            case 61987: // Avenging Wrath
                // Remove the immunity shield marker on Avenging Wrath removal
                // if Forbearance is not present
            ...
            case 72369:
                if (AuraEffect* aurEff = GetEffect(0)) {
                    int32 remainingDamage =
                        aurEff->GetAmount() *
                        (aurEff->GetTotalTicks() - aurEff->GetTickNumber());
        ...
        case SPELLFAMILY_MAGE: {
            ...
            if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL &&
                ...
                if (AuraEffect* absorb = GetEffect(EFFECT_0))
                    if (absorb->GetAmount() <=
                        0) // removed by damage, not dispel
                        if (AuraEffect* dummy = caster->GetDummyAuraEffect(
                                SPELLFAMILY_MAGE, 2945, 0))
                            if (roll_chance_i(
                                    dummy->GetSpellInfo()->ProcChance))
                                caster->CastSpell(
                                    target, 55080, true, nullptr, GetEffect(0));
            }
            break;
        }
        ...
        case SPELLFAMILY_WARLOCK:
            ...
            if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400) {
                if (AuraEffect* aurEff = caster->GetAuraEffect(
                        SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0)) {
                    uint32 spellId = 0;
                    switch (aurEff->GetId()) {
                    case 53759:
                        spellId = 60947;
        ...
        case SPELLFAMILY_PRIEST:
            ...
            if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL &&
                (GetSpellInfo()->SpellFamilyFlags[0] & 0x00008000 ||
                 GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)) {
                // Shadow Affinity
                if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(
                        SPELLFAMILY_PRIEST, 178, 1)) {
                    int32 basepoints0 =
                        aurEff->GetAmount() * caster->GetCreateMana() / 100;
                    caster->CastCustomSpell(caster,
                                            64103,
                                            &basepoints0,
                                            nullptr,
            ...
            // Power word: shield
                     ...
                     GetSpellInfo()->SpellFamilyFlags[0] & 0x00000001) {
                // Rapture
                if (Aura const* aura = caster->GetAuraOfRankedSpell(47535)) {
                    // check cooldown
                    if (caster->GetTypeId() == TYPEID_PLAYER) {
                        if (caster->ToPlayer()->HasSpellCooldown(
                                aura->GetId())) {
                            // This additional check is needed to add a minimal
                            // delay before cooldown in in effect to allow all
                            // bubbles broken by a single damage source proc
                            // mana return
                            if (caster->ToPlayer()->GetSpellCooldownDelay(
                                    aura->GetId()) <= 11500)
                                break;
                        }
                        else // and add if needed
                            caster->ToPlayer()->AddSpellCooldown(
                                aura->GetId(), 0, 12 * IN_MILLISECONDS);
                    }
                    ...
                    if (AuraEffect const* aurEff = aura->GetEffect(0)) {
                        ...
                        int32 basepoints0 = int32(CalculatePct(
                            caster->GetMaxPower(POWER_MANA), multiplier));
                        caster->CastCustomSpell(caster,
                                                47755,
                                                &basepoints0,
                                                nullptr,
                    ...
                    if (AuraEffect const* aurEff = aura->GetEffect(1)) {
                        ...
                        case POWER_MANA: {
                            int32 basepoints0 = int32(CalculatePct(
                                target->GetMaxPower(POWER_MANA), 2));
                            caster->CastCustomSpell(target,
                                                    63654,
                                                    &basepoints0,
                                                    nullptr,
            ...
            case 47788: // Guardian Spirit
            {
                ...
                if (AuraEffect* aurEff = player->GetAuraEffect(63231, 0)) {
                    ...
                    player->AddSpellCooldown(GetSpellInfo()->Id,
                                             0,
                                             aurEff->GetAmount() *
                                                 IN_MILLISECONDS);
        ...
        case SPELLFAMILY_ROGUE: {
            ...
            if (GetId() == 1784) {
                target->RemoveAurasWithFamily(
                    SPELLFAMILY_ROGUE, 0x800, 0, 0, ObjectGuid::Empty);
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if (GetSpellInfo()->SpellIconID == 3041 ||
                ...
                if (!GetEffect(0) ||
                    GetEffect(0)->GetAuraType() != SPELL_AURA_PERIODIC_DUMMY)
                    break;
                if (target->GetTypeId() != TYPEID_PLAYER)
                    break;
        ...
        case SPELLFAMILY_PALADIN:
            // Remove the immunity shield marker on Forbearance removal if AW
        ...
        case SPELLFAMILY_HUNTER:
            ...
            if ((GetSpellInfo()->SpellFamilyFlags[0] & 0x00000008) && caster &&
                caster->HasAura(56845)) {
    ...
    case SPELLFAMILY_PALADIN:
        ...
        case 31821:
            ...
            // If apply Concentration Aura -> trigger -> apply Aura Mastery
            // Immunity If remove Concentration Aura -> trigger -> remove Aura
            // Mastery Immunity If remove Aura Mastery -> trigger -> remove Aura
            // Mastery Immunity Do effects only on aura owner
            if (GetCasterGUID() != target->GetGUID())
                break;
            if (apply) {
        ...
        // Sanctified Retribution, Improved Devotion Aura, Swift Retribution,
        // Improved Concentration Aura
        if (caster == target &&
            GetSpellInfo()->GetSpellSpecific() == SPELL_SPECIFIC_AURA) {
            if (apply) {
...
bool Aura::IsAuraStronger(Aura const* newAura) const
{
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        for (uint8 j = EFFECT_0; j < MAX_SPELL_EFFECTS; ++j) {
            ...
            if (!newEffect ||
                thisEffect->GetAuraType() != newEffect->GetAuraType() ||
                thisEffect->GetMiscValue() != newEffect->GetMiscValue())
                continue;

            // xinef: assume that all spells are either positive or negative,
            // otherwise they should not be in one group
            int32 curValue = std::abs(thisEffect->GetAmount());
            if (curValue < std::abs(newEffect->GetAmount()))
                return true;

            if (curValue == std::abs(newEffect->GetAmount()))
                if (!IsPassive() && !IsPermanent() &&
                    GetDuration() < newAura->GetDuration())
                    return true;
        }
    }
...
bool Aura::CanStackWith(Aura const* existingAura, bool remove) const
{
    ...
    // xinef: dont allow different ranks (or the same rank) of same spell to
    // stack (many flamestrikes for example)
    if (existingAura->GetType() == DYNOBJ_AURA_TYPE) {
        // xinef: desecration lag fix - bound this condition to periodics
        if (sameCaster && GetSpellInfo()->Id == existingSpellInfo->Id &&
            GetSpellInfo()->HasAura(SPELL_AURA_PERIODIC_DAMAGE))
            return false;
        return true;
    }

    // passive auras don't stack with another rank of the spell cast by same
    // caster
    if (IsPassive() && sameCaster &&
        (m_spellInfo->IsDifferentRankOf(existingSpellInfo) ||
         (m_spellInfo->Id == existingSpellInfo->Id &&
          m_castItemGuid.IsEmpty())))
        return false;
    ...
    if (m_spellInfo->IsAuraExclusiveBySpecificWith(existingSpellInfo) ||
        (sameCaster && m_spellInfo->IsAuraExclusiveBySpecificPerCasterWith(
                           existingSpellInfo)))
        return false;

    // check spell group stack rules
    // xinef: this assures us that both spells are in same group!
    SpellGroupStackFlags stackFlags = sSpellMgr->CheckSpellGroupStackRules(
        m_spellInfo, existingSpellInfo, remove, IsArea());
    if (stackFlags) {
        // xinef: same caster rule is bounded by spellfamily
        if (sameCaster &&
            m_spellInfo->SpellFamilyName ==
                existingSpellInfo->SpellFamilyName &&
            (stackFlags & SPELL_GROUP_STACK_FLAG_NOT_SAME_CASTER))
            return false;

        // xinef: normal exclusive stacking, remove if auras are equal by
        // effects
        if (stackFlags & SPELL_GROUP_STACK_FLAG_EXCLUSIVE) {
            if (GetSpellInfo()->IsAuraEffectEqual(existingSpellInfo) ||
                GetSpellInfo()->IsRankOf(existingSpellInfo)) {
                if (remove)
        ...
        SpellGroupSpecialFlags thisAuraFlag =
            sSpellMgr->GetSpellGroupSpecialFlags(GetId());
        SpellGroupSpecialFlags existingAuraFlag =
            sSpellMgr->GetSpellGroupSpecialFlags(existingSpellInfo->Id);
        if (thisAuraFlag >= SPELL_GROUP_SPECIAL_FLAG_PRIORITY1 &&
            thisAuraFlag <= SPELL_GROUP_SPECIAL_FLAG_PRIORITY4 &&
    ...
    if (!sameCaster) {
        ...
        for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            ...
            case SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE:
                // periodic auras which target areas are not allowed to stack
                // this way (replenishment for example)
                if (m_spellInfo->Effects[i].IsTargetingArea() ||
                    existingSpellInfo->Effects[i].IsTargetingArea())
                    break;
                return true;
            default:
                break;
    ...
    while (i < MAX_SPELL_EFFECTS && !(VehicleAura1 && VehicleAura2)) {
        if (m_spellInfo->Effects[i].ApplyAuraName == SPELL_AURA_CONTROL_VEHICLE)
            VehicleAura1 = i + 1;
        if (existingSpellInfo->Effects[i].ApplyAuraName ==
    ...
    if (VehicleAura1 && VehicleAura2) {
        ...
        if (!veh) // We should probably just let it stack. Vehicle system will
                  // prevent undefined behaviour later
            return true;

        // xinef: allow direct auras to stack if there is no passenger in this
        // slot
    ...
    if (m_spellInfo->IsRankOf(existingSpellInfo) &&
        !(m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
          m_spellInfo->SpellFamilyFlags[1] & 0x80000000)) {
        // don't allow passive area auras to stack
        if (m_spellInfo->IsMultiSlotAura() && !IsArea())
            return true;
        if (GetCastItemGUID() && existingAura->GetCastItemGUID())
            if (GetCastItemGUID() != existingAura->GetCastItemGUID() &&
                m_spellInfo->HasAttribute(SPELL_ATTR0_CU_ENCHANT_PROC))
                return true;
                                ...
                                ProcEventInfo&   eventInfo)
{
    ...
    // take one charge, aura expiration will be handled in
    // Aura::TriggerProcOnEvent (if needed)
    if (IsUsingCharges() && (!eventInfo.GetSpellInfo() ||
                             !eventInfo.GetSpellInfo()->HasAttribute(
                                  ...
                                  ProcEventInfo&   eventInfo) const
{
    ...
    ConditionList conditions = sConditionMgr->GetConditionsForNotGroupedEntry(
        CONDITION_SOURCE_TYPE_SPELL_PROC, GetId());
    ConditionSourceInfo condInfo =
        ConditionSourceInfo(eventInfo.GetActor(), eventInfo.GetActionTarget());
    if (!sConditionMgr->IsObjectMeetToConditions(condInfo, conditions))
        return false;

    // AuraScript Hook
    bool check =
        const_cast<Aura*>(this)->CallScriptCheckProcHandlers(aurApp, eventInfo);
    if (!check)
        return false;

    /// @todo:
    // do allow additional requirements for procs
    // this is needed because this is the last moment in which you can prevent
    // aura charge drop on proc and possibly a way to prevent default checks (if
    // there're going to be any)
        ...
        if (!GetSpellInfo()->HasAttribute(
            ...
            if (GetSpellInfo()->EquippedItemClass == ITEM_CLASS_WEAPON) {
                ...
                if (DamageInfo const* damageInfo = eventInfo.GetDamageInfo()) {
                    ...
                    case BASE_ATTACK:
                        item = target->ToPlayer()->GetUseableItemByPos(
                           ...
                           ProcEventInfo&        eventInfo) const
{
    ...
    // so talents modifying chances and judgements will have properly calculated
    // proc chance
    if (Unit* caster = GetCaster()) {
        // calculate ppm chance if present and we're using weapon
        if (eventInfo.GetDamageInfo() && procEntry.ProcsPerMinute != 0) {
            uint32 WeaponSpeed = caster->GetAttackTime(
                eventInfo.GetDamageInfo()->GetAttackType());
            chance = caster->GetPPMProcChance(
                WeaponSpeed, procEntry.ProcsPerMinute, GetSpellInfo());
        }
        // apply chance modifer aura, applies also to ppm chance (see improved
        // judgement of light spell)
        if (Player* modOwner = caster->GetSpellModOwner())
            modOwner->ApplySpellMod(
                GetId(), SPELLMOD_CHANCE_OF_SUCCESS, chance);
    }
    return chance;
}
...
void Aura::TriggerProcOnEvent(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (aurApp->HasEffect(i))
            // OnEffectProc / AfterEffectProc hooks handled in
...
void Aura::LoadScripts()
{
    ...
    for (std::list<AuraScript*>::iterator itr = m_loadedScripts.begin();
        ...
        LOG_DEBUG("spells.aura",
                  "Aura::LoadScripts: Script `{}` for aura `{}` is loaded now",
...
bool Aura::CallScriptCheckAreaTargetHandlers(Unit* target)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::CheckAreaTargetHandler>::iterator
            hookItrEnd = (*scritr)->DoCheckAreaTarget.end(),
            hookItr    = (*scritr)->DoCheckAreaTarget.begin();
...
void Aura::CallScriptDispel(DispelInfo* dispelInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::AuraDispelHandler>::iterator
            hookItrEnd = (*scritr)->OnDispel.end(),
            hookItr    = (*scritr)->OnDispel.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
...
void Aura::CallScriptAfterDispel(DispelInfo* dispelInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::AuraDispelHandler>::iterator
            hookItrEnd = (*scritr)->AfterDispel.end(),
            hookItr    = (*scritr)->AfterDispel.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            hookItr->Call(*scritr, dispelInfo);
                                         ...
                                         AuraEffectHandleModes  mode)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectApplyHandler>::iterator
            effEndItr = (*scritr)->OnEffectApply.end(),
            effItr    = (*scritr)->OnEffectApply.begin();
                                          ...
                                          AuraEffectHandleModes  mode)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectApplyHandler>::iterator
            effEndItr = (*scritr)->OnEffectRemove.end(),
            effItr    = (*scritr)->OnEffectRemove.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, mode);
                                              ...
                                              AuraEffectHandleModes  mode)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
         scritr != m_loadedScripts.end();
         ++scritr) {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_APPLY,
                                      aurApp);
        std::list<AuraScript::EffectApplyHandler>::iterator
            effEndItr = (*scritr)->AfterEffectApply.end(),
            effItr    = (*scritr)->AfterEffectApply.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, mode);
                                               ...
                                               AuraEffectHandleModes  mode)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
         scritr != m_loadedScripts.end();
         ++scritr) {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_REMOVE,
                                      aurApp);
        std::list<AuraScript::EffectApplyHandler>::iterator
            effEndItr = (*scritr)->AfterEffectRemove.end(),
            effItr    = (*scritr)->AfterEffectRemove.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, mode);
                                            ...
                                            AuraApplication const* aurApp)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectPeriodicHandler>::iterator
            effEndItr = (*scritr)->OnEffectPeriodic.end(),
            effItr    = (*scritr)->OnEffectPeriodic.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
...
void Aura::CallScriptEffectUpdatePeriodicHandlers(AuraEffect* aurEff)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectUpdatePeriodicHandler>::iterator
            effEndItr = (*scritr)->OnEffectUpdatePeriodic.end(),
            effItr    = (*scritr)->OnEffectUpdatePeriodic.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff);
                                              ...
                                              bool& canBeRecalculated)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectCalcAmountHandler>::iterator
            effEndItr = (*scritr)->DoEffectCalcAmount.end(),
            effItr    = (*scritr)->DoEffectCalcAmount.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                                                ...
                                                int32&            amplitude)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectCalcPeriodicHandler>::iterator
            effEndItr = (*scritr)->DoEffectCalcPeriodic.end(),
            effItr    = (*scritr)->DoEffectCalcPeriodic.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                                                ...
                                                SpellModifier*&   spellMod)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectCalcSpellModHandler>::iterator
            effEndItr = (*scritr)->DoEffectCalcSpellMod.end(),
            effItr    = (*scritr)->DoEffectCalcSpellMod.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                                          ...
                                          bool& defaultPrevented)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectAbsorbHandler>::iterator
            effEndItr = (*scritr)->OnEffectAbsorb.end(),
            effItr    = (*scritr)->OnEffectAbsorb.begin();
        for (; effItr != effEndItr; ++effItr)
                                               ...
                                               uint32& absorbAmount)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_ABSORB,
                                      aurApp);
        std::list<AuraScript::EffectAbsorbHandler>::iterator
            effEndItr = (*scritr)->AfterEffectAbsorb.end(),
            effItr    = (*scritr)->AfterEffectAbsorb.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, dmgInfo, absorbAmount);
                                              ...
                                              bool& /*defaultPrevented*/)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_MANASHIELD,
                                      aurApp);
        std::list<AuraScript::EffectManaShieldHandler>::iterator
            effEndItr = (*scritr)->OnEffectManaShield.end(),
            effItr    = (*scritr)->OnEffectManaShield.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, dmgInfo, absorbAmount);
...
void Aura::CallScriptEffectAfterManaShieldHandlers(
    AuraEffect*            aurEff,
    AuraApplication const* aurApp,
    DamageInfo&            dmgInfo,
    uint32&                absorbAmount)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_MANASHIELD,
                                      aurApp);
        std::list<AuraScript::EffectManaShieldHandler>::iterator
            effEndItr = (*scritr)->AfterEffectManaShield.end(),
            effItr    = (*scritr)->AfterEffectManaShield.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, dmgInfo, absorbAmount);
                                         ...
                                         uint32&                splitAmount)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectSplitHandler>::iterator
            effEndItr = (*scritr)->OnEffectSplit.end(),
            effItr    = (*scritr)->OnEffectSplit.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                                       ...
                                       ProcEventInfo&         eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::CheckProcHandler>::iterator
            hookItrEnd = (*scritr)->DoCheckProc.end(),
            hookItr    = (*scritr)->DoCheckProc.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
                                            ...
                                            bool isTriggeredAtSpellProcEvent)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
         scritr != m_loadedScripts.end();
         ++scritr) {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_AFTER_CHECK_PROC,
                                      aurApp);
        std::list<AuraScript::AfterCheckProcHandler>::iterator
            hookItrEnd = (*scritr)->DoAfterCheckProc.end(),
            hookItr    = (*scritr)->DoAfterCheckProc.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            result &=
                hookItr->Call(*scritr, eventInfo, isTriggeredAtSpellProcEvent);
                                         ...
                                         ProcEventInfo&         eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::AuraProcHandler>::iterator
            effEndItr = (*scritr)->DoPrepareProc.end(),
            effItr    = (*scritr)->DoPrepareProc.begin();
        for (; effItr != effEndItr; ++effItr)
                                  ...
                                  ProcEventInfo&         eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::AuraProcHandler>::iterator
            hookItrEnd = (*scritr)->OnProc.end(),
            hookItr    = (*scritr)->OnProc.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
                                       ...
                                       ProcEventInfo&         eventInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::AuraProcHandler>::iterator
            hookItrEnd = (*scritr)->AfterProc.end(),
            hookItr    = (*scritr)->AfterProc.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            hookItr->Call(*scritr, eventInfo);
                                        ...
                                        ProcEventInfo&         eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<AuraScript::EffectProcHandler>::iterator
            effEndItr = (*scritr)->OnEffectProc.end(),
            effItr    = (*scritr)->OnEffectProc.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                                             ...
                                             ProcEventInfo&         eventInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin();
         scritr != m_loadedScripts.end();
         ++scritr) {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_PROC,
                                      aurApp);
        std::list<AuraScript::EffectProcHandler>::iterator
            effEndItr = (*scritr)->AfterEffectProc.end(),
            effItr    = (*scritr)->AfterEffectProc.begin();
        for (; effItr != effEndItr; ++effItr)
            if (effItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                effItr->Call(*scritr, aurEff, eventInfo);
...
void Aura::SetTriggeredByAuraSpellInfo(
    SpellInfo const* triggeredByAuraSpellInfo)
{
...
void UnitAura::FillTargetMap(std::map<Unit*, uint8>& targets, Unit* caster)
{
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
        ...
        UnitList targetList;
        // non-area aura
        if (GetSpellInfo()->Effects[effIndex].Effect ==
            ...
            if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED)) {
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_RAID: {
                    targetList.push_back(GetUnitOwner());
                    Acore::AnyGroupedUnitInObjectRangeCheck u_check(
                        GetUnitOwner(),
                        GetUnitOwner(),
                        radius,
                    ...
                    Acore::UnitListSearcher<
                        Acore::AnyGroupedUnitInObjectRangeCheck>
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND: {
                    targetList.push_back(GetUnitOwner());
                    Acore::AnyFriendlyUnitInObjectRangeCheck u_check(
                        GetUnitOwner(), GetUnitOwner(), radius);
                    Acore::UnitListSearcher<
                        Acore::AnyFriendlyUnitInObjectRangeCheck>
                        searcher(GetUnitOwner(), targetList, u_check);
                    Cell::VisitAllObjects(GetUnitOwner(), searcher, radius);
                    break;
                }
                case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY: {
                    Acore::AnyAoETargetUnitInObjectRangeCheck u_check(
                        GetUnitOwner(),
                        GetUnitOwner(),
                        radius); // No GetCharmer in searcher
                    Acore::UnitListSearcher<
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_PET:
                    targetList.push_back(GetUnitOwner());
                    [[fallthrough]]; /// @todo: Not sure whether the fallthrough
                                     /// was a mistake (forgetting a break) or
                                     /// intended. This should be
                                     /// double-checked.
                               ...
                               Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
        ...
        SpellInfo const* spellInfo = GetSpellInfo();
        UnitList         targetList;
        if (spellInfo->Effects[effIndex].TargetB.GetTarget() ==
        ...
        // pussywizard: TARGET_DEST_DYNOBJ_NONE is supposed to search for both
        // friendly and unfriendly targets, so for any unit what about
        // EffectImplicitTargetA?
        else if (spellInfo->Effects[effIndex].TargetB.GetTarget() ==
                 TARGET_DEST_DYNOBJ_NONE) {
            Acore::AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck
                u_check(GetDynobjOwner(), dynObjOwnerCaster, radius);
            Acore::UnitListSearcher<
                Acore::
                    AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck>
                searcher(GetDynobjOwner(), targetList, u_check);
            Cell::VisitAllObjects(GetDynobjOwner(), searcher, radius);
        }
        else {
        ...
        for (UnitList::iterator itr = targetList.begin();
            ...
            if (!spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) &&
                ...
                !GetDynobjOwner()->IsWithinLOSInMap(
                    target,
