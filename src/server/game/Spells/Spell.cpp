...
SpellDestination::SpellDestination(
    float x, float y, float z, float orientation, uint32 mapId)
{
...
SpellDestination::SpellDestination(WorldObject const& wObj)
{
    ...
    _transportOffset.Relocate(wObj.GetTransOffsetX(),
                              wObj.GetTransOffsetY(),
                              wObj.GetTransOffsetZ(),
                              wObj.GetTransOffsetO());
...
void SpellCastTargets::Read(ByteBuffer& data, Unit* caster)
{
    ...
    if (m_targetMask &
        (TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_MINIPET | TARGET_FLAG_GAMEOBJECT |
    ...
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION) {
        ...
        if (m_src._transportGUID)
            m_src._transportOffset.Relocate(caster->GetTransOffsetX(),
                                            caster->GetTransOffsetY(),
                                            caster->GetTransOffsetZ(),
    ...
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION) {
        ...
        if (m_dst._transportGUID)
            m_dst._transportOffset.Relocate(caster->GetTransOffsetX(),
                                            caster->GetTransOffsetY(),
                                            caster->GetTransOffsetZ(),
                                            caster->GetTransOffsetO());
        else
...
void SpellCastTargets::Write(ByteBuffer& data)
{
    ...
    if (m_targetMask &
        (TARGET_FLAG_UNIT | TARGET_FLAG_CORPSE_ALLY | TARGET_FLAG_GAMEOBJECT |
    ...
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION) {
        data << m_src._transportGUID
                    .WriteAsPacked(); // relative position guid here - transport
                                      // for example
        if (m_src._transportGUID)
            data << m_src._transportOffset.PositionXYZStream();
        else
...
void SpellCastTargets::RemoveObjectTarget()
{
    ...
    m_targetMask &= ~(TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK |
...
{
    return m_objectTargetGUIDChannel
               ? ((m_objectTargetGUIDChannel == caster->GetGUID())
                      ? caster
                      : ObjectAccessor::GetWorldObject(
                            *caster, m_objectTargetGUIDChannel))
               : nullptr;
}
...
void SpellCastTargets::Update(Unit* caster)
{
    ...
    if (caster->GetTypeId() == TYPEID_PLAYER) {
        ...
        if (m_targetMask & TARGET_FLAG_ITEM)
            ...
            if (m_itemTargetGUID.GetRawValue() ==
                TRADE_SLOT_NONTRADED) // here it is not guid but slot. Also
                                      // prevents hacking slots
    ...
    if (HasSrc() && m_src._transportGUID) {
        if (WorldObject* transport =
                ObjectAccessor::GetWorldObject(*caster, m_src._transportGUID)) {
    ...
    if (HasDst() && m_dst._transportGUID) {
        if (WorldObject* transport =
                ObjectAccessor::GetWorldObject(*caster, m_dst._transportGUID)) {
...
void SpellCastTargets::OutDebug() const
{
    ...
    if (m_targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK |
                        TARGET_FLAG_GAMEOBJECT_MASK))
    ...
    if (m_targetMask & TARGET_FLAG_TRADE_ITEM)
        LOG_INFO(
            "spells", "Trade item target: {}", m_itemTargetGUID.ToString());
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
        LOG_INFO(
            "spells",
            "Source location: transport guid: {} trans offset: {} position: {}",
            m_src._transportGUID.ToString(),
            m_src._transportOffset.ToString(),
            m_src._position.ToString());
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
        LOG_INFO("spells",
                 "Destination location: transport guid: {} trans offset: {} "
                 "position: {}",
                 m_dst._transportGUID.ToString(),
                 m_dst._transportOffset.ToString(),
                 m_dst._position.ToString());
    if (m_targetMask & TARGET_FLAG_STRING)
...
Spell::Spell(Unit*            caster,
    ...
    : m_spellInfo(sSpellMgr->GetSpellForDifficultyFromSpell(info, caster)),
      m_caster((info->HasAttribute(SPELL_ATTR6_ORIGINATE_FROM_CONTROLLER) &&
      ...
      m_spellValue(new SpellValue(m_spellInfo)), _spellEvent(nullptr)
{
    ...
    case SPELL_DAMAGE_CLASS_RANGED:
        m_attackType =
            m_spellInfo->IsRangedWeaponSpell() ? RANGED_ATTACK : BASE_ATTACK;
        break;
    default:
        // Wands
    ...
    m_spellSchoolMask = info->GetSchoolMask(); // Can be override for some spell
                                               // (wand shoot for example)
    ...
    if (m_attackType == RANGED_ATTACK)
        ...
        if ((m_caster->getClassMask() & CLASSMASK_WAND_USERS) != 0 &&
            ...
            if (Item* pItem =
                    m_caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK))
                m_spellSchoolMask = SpellSchoolMask(
                    1 << pItem->GetTemplate()->Damage[0].DamageType);
    ...
    if (m_originalCasterGUID == m_caster->GetGUID())
        ...
        m_originalCaster =
            ObjectAccessor::GetUnit(*m_caster, m_originalCasterGUID);
    ...
    if (info->HasAttribute(SPELL_ATTR4_ALLOW_CAST_WHILE_CASTING))
        _triggeredCastFlags = TriggerCastFlags(
            uint32(_triggeredCastFlags) | TRIGGERED_IGNORE_CAST_IN_PROGRESS |
    ...
    m_runesState = 0;
    m_powerCost =
        0; // setup to correct value in Spell::prepare, must not be used before.
    m_casttime =
        0; // setup to correct value in Spell::prepare, must not be used before.
    ...
    m_canReflect = m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC &&
                   ...
                   !m_spellInfo->HasAttribute(SPELL_ATTR1_NO_REFLECTION) &&
                   !m_spellInfo->HasAttribute(SPELL_ATTR0_NO_IMMUNITIES) &&
...
Spell::~Spell()
{
    ...
    if (m_referencedFromCurrentSpell && m_selfContainer &&
        *m_selfContainer == this) {
        ...
        // If this error is repeating, we may have to add an ASSERT to better
        // track down how we get into this case.
        LOG_ERROR("spells",
                  "Spell::~Spell: deleting spell for spell ID {}. However, "
                  "spell still referenced.",
                  m_spellInfo->Id);
        *m_selfContainer = nullptr;
    }
...
void Spell::InitExplicitTargets(SpellCastTargets const& targets)
{
    ...
    // client doesn't send explicit targets correctly sometimes - we need to fix
    // such spells serverside this also makes sure that we correctly send
    // explicit targets to client (removes redundant data)
    uint32 neededTargets = m_spellInfo->GetExplicitTargetMask();

    if (WorldObject* target = m_targets.GetObjectTarget()) {
        // check if object target is valid with needed target flags
        // for unit case allow corpse target mask because player with not
        // released corpse is a unit target
        if ((target->ToUnit() &&
             !(neededTargets &
               (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK))) ||
            (target->ToGameObject() &&
             !(neededTargets & TARGET_FLAG_GAMEOBJECT_MASK)) ||
            (target->ToCorpse() && !(neededTargets & TARGET_FLAG_CORPSE_MASK)))
            m_targets.RemoveObjectTarget();
    }
    else {
        // try to select correct unit target if not provided by client or by
        // serverside cast
        if (neededTargets & (TARGET_FLAG_UNIT_MASK)) {
            Unit* unit = nullptr;
            // try to use player selection as a target
            if (Player* playerCaster = m_caster->ToPlayer()) {
                // selection has to be found and to be valid target for the
                // spell
                if (Unit* selectedUnit = ObjectAccessor::GetUnit(
                        *m_caster, playerCaster->GetTarget()))
                    if (m_spellInfo->CheckExplicitTarget(
                            m_caster, selectedUnit) == SPELL_CAST_OK)
                        unit = selectedUnit;
            }
            // try to use attacked unit as a target
            else if ((m_caster->GetTypeId() == TYPEID_UNIT) &&
                     neededTargets &
                         (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT))
                unit = m_caster->GetVictim();

            // didn't find anything - let's use self as target
            if (!unit && neededTargets &
                             (TARGET_FLAG_UNIT_RAID | TARGET_FLAG_UNIT_PARTY |
...
{
    // here go all explicit target changes made to explicit targets after spell
    // prepare phase is finished
    if (Unit* target = m_targets.GetUnitTarget()) {
        // check for explicit target redirection, for Grounding Totem for
        // example
        if (m_spellInfo->GetExplicitTargetMask() & TARGET_FLAG_UNIT_ENEMY ||
            ...
            case SPELL_DAMAGE_CLASS_MAGIC:
                redirect =
                    m_caster->GetMagicHitRedirectTarget(target, m_spellInfo);
                break;
            case SPELL_DAMAGE_CLASS_MELEE:
            case SPELL_DAMAGE_CLASS_RANGED:
                redirect =
                    m_caster->GetMeleeHitRedirectTarget(target, m_spellInfo);
                break;
            default:
                redirect = nullptr;
                break;
...
void Spell::SelectSpellTargets()
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        uint32 implicitTargetMask =
            GetTargetFlagMask(m_spellInfo->Effects[i].TargetA.GetObjectType()) |
            GetTargetFlagMask(m_spellInfo->Effects[i].TargetB.GetObjectType());
        if (implicitTargetMask & TARGET_FLAG_UNIT)
            m_targets.SetTargetFlag(TARGET_FLAG_UNIT);
        if (implicitTargetMask &
            (TARGET_FLAG_GAMEOBJECT | TARGET_FLAG_GAMEOBJECT_ITEM))
            m_targets.SetTargetFlag(TARGET_FLAG_GAMEOBJECT);

        SelectEffectImplicitTargets(SpellEffIndex(i),
                                    m_spellInfo->Effects[i].TargetA,
                                    processedAreaEffectsMask);
        SelectEffectImplicitTargets(SpellEffIndex(i),
                                    m_spellInfo->Effects[i].TargetB,
                                    processedAreaEffectsMask);

        // Select targets of effect based on effect type
        // those are used when no valid target could be added for spell effect
        // based on spell target type some spell effects use explicit target as
        // a default target added to target map (like SPELL_EFFECT_LEARN_SPELL)
        // some spell effects add target to target map only when target type
        // specified (like SPELL_EFFECT_WEAPON) some spell effects don't add
        // anything to target map (confirmed with sniffs) (like
        ...
        if (m_spellInfo->IsChanneled()) {
            ...
            if (!focusObject && m_UniqueTargetInfo.empty() &&
            ...
            uint8 mask = (1 << i);
            for (auto ihit = m_UniqueTargetInfo.begin();
            ...
            m_UniqueTargetInfo.erase(
                std::remove_if(
                    std::begin(m_UniqueTargetInfo),
                    std::end(m_UniqueTargetInfo),
                    [&](TargetInfo const& targetInfo) -> bool {
                        // remove targets which did not pass min level check
                        if (m_auraScaleMask &&
                            targetInfo.effectMask == m_auraScaleMask) {
                            if (!targetInfo.scaleAura &&
                                targetInfo.targetGUID != m_caster->GetGUID())
                                return true;
                        }
...
uint64 Spell::CalculateDelayMomentForDst() const
{
    if (m_targets.HasDst()) {
        if (m_targets.HasTraj()) {
            ...
            // We should not subtract caster size from dist calculation (fixes
            // execution time desync with animation on client, eg. Malleable Goo
            // cast by PP)
...
void Spell::RecalculateDelayMomentForDst()
{
    ...
    m_caster->m_Events.ModifyEventTime(
        _spellEvent, Milliseconds(GetDelayStart() + m_delayMoment));
}
    ...
    uint32&                        processedEffectMask)
{
    ...
    case TARGET_SELECT_CATEGORY_AREA: {
        ...
        for (uint32 j = effIndex + 1; j < MAX_SPELL_EFFECTS; ++j) {
            if (effects[j].IsEffect() &&
                effects[effIndex].TargetA.GetTarget() ==
                    effects[j].TargetA.GetTarget() &&
                effects[effIndex].TargetB.GetTarget() ==
                    effects[j].TargetB.GetTarget() &&
                effects[effIndex].ImplicitTargetConditions ==
                    effects[j].ImplicitTargetConditions &&
                effects[effIndex].CalcRadius(m_caster) ==
                    effects[j].CalcRadius(m_caster) &&
    ...
    case TARGET_SELECT_CATEGORY_TRAJ:
        // just in case there is no dest, explanation in
    ...
    case TARGET_SELECT_CATEGORY_DEFAULT:
        ...
        case TARGET_OBJECT_TYPE_SRC:
            ...
            case TARGET_REFERENCE_TYPE_CASTER:
                ...
                break;
            default:
                ASSERT(false && "Spell::SelectEffectImplicitTargets: received "
                                "not implemented select target reference type "
    ...
    case TARGET_SELECT_CATEGORY_NYI:
        LOG_DEBUG("spells.aura",
                  "SPELL: target type {}, found in spellID {}, effect {} is "
                  "not implemented yet!",
                  m_spellInfo->Id,
                  effIndex,
        ...
        break;
    default:
        ASSERT(false && "Spell::SelectEffectImplicitTargets: received not "
                        "implemented select target category");
        break;
    }
}

void Spell::SelectImplicitChannelTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    if (targetType.GetReferenceType() != TARGET_REFERENCE_TYPE_CASTER) {
        ASSERT(false && "Spell::SelectImplicitChannelTargets: received not "
                        "implemented target reference type");
        return;
    }

    switch (targetType.GetTarget()) {
    case TARGET_UNIT_CHANNEL_TARGET: {
        // Xinef: All channel selectors have needed data passed in m_targets
        // structure
        WorldObject* target = m_targets.GetObjectTargetChannel(m_caster);
        if (target) {
            CallScriptObjectTargetSelectHandlers(target, effIndex, targetType);
            // unit target may be no longer avalible - teleported out of map for
            // example
            ...
            LOG_DEBUG("spells.aura",
                      "SPELL: cannot find channel spell target for spell ID "
                      "{}, effect {}",
                      m_spellInfo->Id,
                      effIndex);
        }
    ...
    case TARGET_DEST_CHANNEL_TARGET:
        ...
        else if (WorldObject* target =
                     m_targets.GetObjectTargetChannel(m_caster)) {
            CallScriptObjectTargetSelectHandlers(target, effIndex, targetType);
            if (target)
                m_targets.SetDst(*target);
        }
        else if (Spell* channeledSpell = m_originalCaster->GetCurrentSpell(
        ...
        {
            LOG_DEBUG("spells.aura",
                      "SPELL: cannot find channel spell destination for spell "
                      "ID {}, effect {}",
                      m_spellInfo->Id,
                      effIndex);
        }
...
void Spell::SelectImplicitNearbyTargets(
    SpellEffIndex                  effIndex,
    SpellImplicitTargetInfo const& targetType,
    uint32                         effMask)
{
    if (targetType.GetReferenceType() != TARGET_REFERENCE_TYPE_CASTER) {
        ASSERT(false && "Spell::SelectImplicitNearbyTargets: received not "
                        "implemented target reference type");
        return;
    }
    ...
    case TARGET_CHECK_DEFAULT:
        range =
            m_spellInfo->GetMaxRange(m_spellInfo->IsPositive(), m_caster, this);
        break;
    default:
        ASSERT(false && "Spell::SelectImplicitNearbyTargets: received not "
                        "implemented selection check type");
        break;
    }
    ...
    // handle emergency case - try to use other provided targets if no
    // conditions provided
    if (targetType.GetCheckType() == TARGET_CHECK_ENTRY &&
        (!condList || condList->empty())) {
        LOG_DEBUG("spells.aura",
                  "Spell::SelectImplicitNearbyTargets: no conditions entry for "
                  "target with TARGET_CHECK_ENTRY of spell ID {}, effect {} - "
                  "selecting default targets",
                  m_spellInfo->Id,
                  effIndex);
        switch (targetType.GetObjectType()) {
    ...
    WorldObject* target = SearchNearbyTarget(
        range, targetType.GetObjectType(), targetType.GetCheckType(), condList);
    if (!target) {
        LOG_DEBUG("spells.aura",
                  "Spell::SelectImplicitNearbyTargets: cannot find nearby "
                  "target for spell ID {}, effect {}",
                  m_spellInfo->Id,
                  effIndex);
        return;
    }
    ...
    CallScriptObjectTargetSelectHandlers(target, effIndex, targetType);
    if (!target) {
        // LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets:
        // OnObjectTargetSelect script hook for spell Id {} set nullptr target,
        // effect {}", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    case TARGET_OBJECT_TYPE_UNIT: {
        if (Unit* unit = target->ToUnit()) {
            ...
            // xinef: important! if channeling spell have nearby entry, it has
            // no unitTarget by default and if channeled spell has target 77, it
            // requires unitTarget, set it here! xinef: if we have NO unit
            // target
            ...
            // LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets:
            // OnObjectTargetSelect script hook for spell Id {} set object of
            // wrong type, expected unit, got {}, effect {}", m_spellInfo->Id,
    ...
    case TARGET_OBJECT_TYPE_GOBJ:
        if (GameObject* gobjTarget = target->ToGameObject())
            ...
            // LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets:
            // OnObjectTargetSelect script hook for spell Id {} set object of
            // wrong type, expected gameobject, got {}, effect {}",
    ...
    default:
        ASSERT(false && "Spell::SelectImplicitNearbyTargets: received not "
                        "implemented target object type");
        break;
    }
...
{
    if (targetType.GetReferenceType() != TARGET_REFERENCE_TYPE_CASTER) {
        ASSERT(false && "Spell::SelectImplicitConeTargets: received not "
                        "implemented target reference type");
        return;
    }
    ...
    if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList)) {
        Acore::WorldObjectSpellConeTargetCheck check(
            coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
        Acore::WorldObjectListSearcher<Acore::WorldObjectSpellConeTargetCheck>
            searcher(m_caster, targets, check, containerTypeMask);
        SearchTargets<Acore::WorldObjectListSearcher<
            Acore::WorldObjectSpellConeTargetCheck>>(
            searcher, containerTypeMask, m_caster, m_caster, radius);
                                      ...
                                      uint32                         effMask)
{
    ...
    case TARGET_REFERENCE_TYPE_LAST: {
        ...
        for (std::list<TargetInfo>::reverse_iterator ihit =
                 m_UniqueTargetInfo.rbegin();
             ihit != m_UniqueTargetInfo.rend();
             ++ihit) {
    ...
    default:
        ASSERT(false && "Spell::SelectImplicitAreaTargets: received not "
                        "implemented target reference type");
        return;
    }
    if (!referer)
    ...
    // Xinef: the distance should be increased by caster size, it is neglected
    // in latter calculations
    std::list<WorldObject*> targets;
    float radius = m_spellInfo->Effects[effIndex].CalcRadius(m_caster) *
                   m_spellValue->RadiusMod;
    SearchAreaTargets(targets,
                      radius,
                      center,
                      referer,
                      targetType.GetObjectType(),
                      targetType.GetCheckType(),
                      m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
...
void Spell::SelectImplicitCasterDestTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    case TARGET_DEST_HOME:
        if (Player* playerCaster = m_caster->ToPlayer())
            dest = SpellDestination(playerCaster->m_homebindX,
                                    playerCaster->m_homebindY,
                                    playerCaster->m_homebindZ,
                                    playerCaster->m_homebindO,
                                    playerCaster->m_homebindMapId);
        break;
    case TARGET_DEST_DB:
        if (SpellTargetPosition const* st =
                sSpellMgr->GetSpellTargetPosition(m_spellInfo->Id, effIndex)) {
            /// @todo fix this check
            if (m_spellInfo->HasEffect(SPELL_EFFECT_TELEPORT_UNITS) ||
                m_spellInfo->HasEffect(SPELL_EFFECT_BIND))
                dest = SpellDestination(st->target_X,
                                        st->target_Y,
                                        st->target_Z,
            ...
            LOG_DEBUG("spells.aura",
                      "SPELL: unknown target coordinates for spell ID {}",
                      m_spellInfo->Id);
            if (WorldObject* target = m_targets.GetObjectTarget())
    ...
    case TARGET_DEST_CASTER_FISHING: {
        ...
        float x, y, z, angle;
        angle = (float)rand_norm() * static_cast<float>(M_PI * 35.0f / 180.0f) -
                static_cast<float>(M_PI * 17.5f / 180.0f);
        // m_caster->GetClosePoint(x, y, z, DEFAULT_WORLD_OBJECT_SIZE, dis,
        // angle); this contains extra code that breaks fishing
        m_caster->GetNearPoint(m_caster,
                               x,
                               y,
                               z,
                               DEFAULT_WORLD_OBJECT_SIZE,
                               dis,
                               m_caster->GetOrientation() + angle);

        float             ground      = m_caster->GetMapHeight(x, y, z, true);
        float             liquidLevel = VMAP_INVALID_HEIGHT_VALUE;
        LiquidData const& liquidData =
            m_caster->GetMap()->GetLiquidData(m_caster->GetPhaseMask(),
                                              x,
                                              y,
                                              z,
        ...
        if (liquidLevel <=
            ground) // When there is no liquid Map::GetWaterOrGroundLevel
    ...
    case TARGET_DEST_CASTER_FRONT_LEAP: {
        ...
        float  destz = 0.0f, startx = 0.0f, starty = 0.0f, startz = 0.0f,
              starto = 0.0f;
        ...
        float ground = map->GetHeight(phasemask,
                                      pos.GetPositionX(),
                                      pos.GetPositionY(),
                                      pos.GetPositionZ());
        ...
        if (!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) ||
            (pos.GetPositionZ() - ground < distance)) {
            float tstX = 0.0f, tstY = 0.0f, tstZ = 0.0f, prevX = 0.0f,
                  prevY = 0.0f, prevZ = 0.0f;
            float tstZ1 = 0.0f, tstZ2 = 0.0f, tstZ3 = 0.0f, destz1 = 0.0f,
                  destz2 = 0.0f, destz3 = 0.0f, srange = 0.0f, srange1 = 0.0f,
                  srange2 = 0.0f, srange3 = 0.0f;
            ...
            for (; j < (numChecks + 1); j++) {
                ...
                tstZ = map->GetHeight(
                    phasemask, tstX, tstY, prevZ + maxtravelDistZ, true);
                ground = tstZ;

                if (!isCasterInWater) {
                    if (map->IsInWater(
                            phasemask, tstX, tstY, tstZ, collisionHeight)) {
                        if (!(beforewaterz != 0.0f)) {
                            beforewaterz = prevZ;
                        }
                        tstZ   = beforewaterz;
                        srange = sqrt((tstY - prevY) * (tstY - prevY) +
                                      (tstX - prevX) * (tstX - prevX));
                        // LOG_ERROR("spells", "(start was from land) step in
                        // water , number of cycle = {} , distance of step = {},
                        // total path = {}, Z = {}", j, srange, totalpath,
                        // tstZ);
                    }
                }
                else if (map->IsInWater(
                             phasemask, tstX, tstY, tstZ, collisionHeight)) {
                    prevZ  = pos.GetPositionZ();
                    ...
                    if (inwater && (fabs(tstZ - ground) < 2.0f)) {
                        wcol = true;
                        // LOG_ERROR("spells", "step in water with collide and
                        // use standart check (for continue way after possible
                        // collide), number of cycle = {} ", j);
                    }

                    // if (j < 2)
                    //    LOG_ERROR("spells", "(start in water) step in water,
                    //    number of cycle = {} , distance of step = {}, total
                    //    path = {}", j, srange, totalpath);
                    // else
                    //    LOG_ERROR("spells", "step in water, number of cycle =
                    //    {} , distance of step = {}, total path = {}", j,
                    //    srange, totalpath);
                }

                bool IsInWater = map->IsInWater(
                    phasemask, tstX, tstY, tstZ, collisionHeight);
                if ((!IsInWater && tstZ != beforewaterz) ||
                    wcol) // second safety check z for blink way if on the
                          // ground
                {
                    ...
                    tstZ1 = map->GetHeight(phasemask,
                                           tstX,
                                           tstY,
                                           prevZ + maxtravelDistZ,
                                           true,
                                           25.0f);
                    // upper or floor
                    tstZ2 = map->GetHeight(
                        phasemask, tstX, tstY, prevZ, true, 25.0f);
                    // lower than floor
                    tstZ3 = map->GetHeight(phasemask,
                                           tstX,
                                           tstY,
                                           prevZ - maxtravelDistZ / 2,
                                           true,
                                           25.0f);

                    // distance of rays, will select the shortest in 3D
                    srange1 = sqrt((tstY - prevY) * (tstY - prevY) +
                                   (tstX - prevX) * (tstX - prevX) +
                                   (tstZ1 - prevZ) * (tstZ1 - prevZ));
                    // LOG_ERROR("spells", "step = {}, distance of ray1 = {}",
                    // j, srange1);
                    srange2 = sqrt((tstY - prevY) * (tstY - prevY) +
                                   (tstX - prevX) * (tstX - prevX) +
                                   (tstZ2 - prevZ) * (tstZ2 - prevZ));
                    // LOG_ERROR("spells", "step = {}, distance of ray2 = {}",
                    // j, srange2);
                    srange3 = sqrt((tstY - prevY) * (tstY - prevY) +
                                   (tstX - prevX) * (tstX - prevX) +
                                   (tstZ3 - prevZ) * (tstZ3 - prevZ));
                    // LOG_ERROR("spells", "step = {}, distance of ray3 = {}",
                    // j, srange3);
                    ...
                    // LOG_ERROR("spells", "step on ground, number of cycle = {}
                    // , distance of step = {}, total path = {}", j, srange,
                    // totalpath);
                }
                ...
                if (totalpath > distance) {
                    ...
                    // LOG_ERROR("spells", "total path > than distance in 3D ,
                    // need to move back a bit for save distance, total path =
                    // {}, overdistance = {}", totalpath, overdistance);
                }

                bool col =
                    VMAP::VMapFactory::createOrGetVMapMgr()->GetObjectHitPos(
                        mapid,
                        prevX,
                        prevY,
                        prevZ + 0.5f,
                ...
                bool dcol = m_caster->GetMap()->GetObjectHitPos(phasemask,
                                                                prevX,
                                                                prevY,
                                                                prevZ + 0.5f,
                                                                tstX,
                ...
                if (col || dcol ||
                    (overdistance > 0.0f &&
                     !map->IsInWater(
                         phasemask, tstX, tstY, ground, collisionHeight)) ||
                    (fabs(prevZ - tstZ) > maxtravelDistZ && (tstZ > prevZ))) {
                    if ((overdistance > 0.0f) && (overdistance < 1.f)) {
                        destx =
                            prevX + overdistance * cos(pos.GetOrientation());
                        desty =
                            prevY + overdistance * sin(pos.GetOrientation());
                        // LOG_ERROR("spells", "(collision) collision occured
                        // 1");
                    }
                    else {
                        // move back a bit
                    ...
                    // highest available point
                    destz1 = map->GetHeight(phasemask,
                                            destx,
                                            desty,
                                            prevZ + maxtravelDistZ,
                                            true,
                                            25.0f);
                    // upper or floor
                    destz2 = map->GetHeight(
                        phasemask, destx, desty, prevZ, true, 25.0f);
                    // lower than floor
                    destz3 = map->GetHeight(phasemask,
                                            destx,
                                            desty,
                                            prevZ - maxtravelDistZ / 2,
                                            true,
                    ...
                    // distance of rays, will select the shortest in 3D
                    srange1 = sqrt((desty - prevY) * (desty - prevY) +
                                   (destx - prevX) * (destx - prevX) +
                                   (destz1 - prevZ) * (destz1 - prevZ));
                    srange2 = sqrt((desty - prevY) * (desty - prevY) +
                                   (destx - prevX) * (destx - prevX) +
                                   (destz2 - prevZ) * (destz2 - prevZ));
                    srange3 = sqrt((desty - prevY) * (desty - prevY) +
                                   (destx - prevX) * (destx - prevX) +
                                   (destz3 - prevZ) * (destz3 - prevZ));
                    ...
                    if (inwater && destz < prevZ && !wcol)
                        destz = prevZ;
                    // LOG_ERROR("spells", "(collision) destZ rewrited in
                    // prevZ");

                    // Don't make the player move backward from the xy
                    // adjustments by collisions.
                    if ((DELTA_X > 0 && startx > destx) ||
                        (DELTA_X < 0 && startx < destx) ||
                        (DELTA_Y > 0 && starty > desty) ||
                        (DELTA_Y < 0 && starty < desty)) {
                        destx = startx;
                        desty = starty;
                        destz = startz;
            ...
            bool col = VMAP::VMapFactory::createOrGetVMapMgr()->GetObjectHitPos(
                mapid,
                pos.GetPositionX(),
                pos.GetPositionY(),
                z,
            ...
            dest = SpellDestination(lastpos);
            // float range = sqrt((desty - pos.GetPositionY())*(desty -
            // pos.GetPositionY()) + (destx - pos.GetPositionX())*(destx -
            // pos.GetPositionX())); LOG_ERROR("spells", "Blink number 2, in
            // falling but at a hight, distance of blink = {}", range);
        }
        break;
    }
    default: {
...
void Spell::SelectImplicitTargetDestTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    // can only happen if previous destination target could not be set for some
    // reason (not found nearby target, or channel target for example maybe we
    // should abort the spell in such case?
    CheckDst();
...
void Spell::SelectImplicitCasterObjectTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    case TARGET_UNIT_PASSENGER_7:
        if (m_caster->GetTypeId() == TYPEID_UNIT &&
            m_caster->ToCreature()->IsVehicle())
            target = m_caster->GetVehicleKit()->GetPassenger(
                targetType.GetTarget() - TARGET_UNIT_PASSENGER_0);
        break;
    default:
        break;
    }
...
void Spell::SelectImplicitTargetObjectTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ASSERT((m_targets.GetObjectTarget() || m_targets.GetItemTarget()) &&
...
void Spell::SelectImplicitChainTargets(
    SpellEffIndex                  effIndex,
    SpellImplicitTargetInfo const& targetType,
    WorldObject*                   target,
    uint32                         effMask)
{
    ...
    if (Player* modOwner = m_caster->GetSpellModOwner())
        modOwner->ApplySpellMod(
            m_spellInfo->Id, SPELLMOD_JUMP_TARGETS, maxTargets, this);
...
{
    x = tan(x);
    // if (x < std::numeric_limits<float>::max() && x >
    // -std::numeric_limits<float>::max()) return x; if (x >=
    // std::numeric_limits<float>::max()) return
    // std::numeric_limits<float>::max(); if (x <=
    // -std::numeric_limits<float>::max()) return
                                      ...
                                      SpellImplicitTargetInfo const& targetType)
{
    ...
    float srcToDestDelta =
        m_targets.GetDstPos()->m_positionZ - m_targets.GetSrcPos()->m_positionZ;

    // xinef: supply correct target type, DEST_DEST and similar are ALWAYS
    // undefined xinef: correct target is stored in TRIGGERED SPELL, however as
    // far as i noticed, all checks are ENTRY, ENEMY
    std::list<WorldObject*>                targets;
    Acore::WorldObjectSpellTrajTargetCheck check(
        dist2d,
        m_targets.GetSrcPos(),
        m_caster,
        m_spellInfo,
        TARGET_CHECK_ENEMY /*targetCheckType*/,
        m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
    Acore::WorldObjectListSearcher<Acore::WorldObjectSpellTrajTargetCheck>
        searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<
        Acore::WorldObjectListSearcher<Acore::WorldObjectSpellTrajTargetCheck>>(
        searcher,
    ...
    for (; itr != targets.end(); ++itr) {
        if (Unit* unitTarget = (*itr)->ToUnit())
            if (m_caster == *itr || m_caster->IsOnVehicle(unitTarget) ||
                (unitTarget)->GetVehicle()) //(*itr)->IsOnVehicle(m_caster))
                continue;

        const float size =
            std::max((*itr)->GetObjectSize() * 0.7f, 1.0f); // 1/sqrt(3)
        /// @todo: all calculation should be based on src instead of m_caster
        const float objDist2d =
            std::fabs(m_targets.GetSrcPos()->GetExactDist2d(*itr) *
                      cos(m_targets.GetSrcPos()->GetRelativeAngle(*itr)));
        const float dz = std::fabs((*itr)->GetPositionZ() -
                                   m_targets.GetSrcPos()->m_positionZ);

        LOG_DEBUG("spells",
                  "Spell::SelectTrajTargets: check {}, dist between {} {}, "
                  "height between {} {}.",
                  (*itr)->GetEntry(),
                  objDist2d - size,
                  objDist2d + size,
        ...
        LOG_DEBUG("spells",
                  "Spell::SelectTrajTargets: dist {}, height {}.",
                  dist,
                  height);
...
#define CHECK_DIST                                                             \
    {                                                                          \
        ...
        if (m_CastItem) {
            if (dist < bestDist &&
                std::fabs(dz) < 6.0f) // closes target, also check Z difference)
            {
                bestDist = dist;
                break;
            }
        ...
        if (!a) {
            ...
            if (height < dz + size * (b + 1) && height > dz - size * (b + 1) &&
                dist < bestDist) {
                bestDist = dist;
                break;
            }
    ...
    if (m_targets.GetSrcPos()->GetExactDist2d(m_targets.GetDstPos()) >
        bestDist) {
        float x = m_targets.GetSrcPos()->m_positionX +
                  cos(m_caster->GetOrientation()) * bestDist;
        float y = m_targets.GetSrcPos()->m_positionY +
                  std::sin(m_caster->GetOrientation()) * bestDist;
        float z =
        ...
        if (itr != targets.end()) {
            ...
            sizeSq *= sizeSq;
            LOG_DEBUG("spells",
                      "Spell::SelectTrajTargets: Initial {} {} {} {} {}",
                      x,
                      y,
                      z,
                      distSq,
...
{
    // special case for SPELL_EFFECT_SUMMON_RAF_FRIEND and
    // SPELL_EFFECT_SUMMON_PLAYER
    /// @todo: this is a workaround - target shouldn't be stored in target map
    /// for those spells
    switch (m_spellInfo->Effects[effIndex].Effect) {
    case SPELL_EFFECT_SUMMON_RAF_FRIEND:
    case SPELL_EFFECT_SUMMON_PLAYER:
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
            m_caster->ToPlayer()->GetTarget()) {
            WorldObject* target =
                ObjectAccessor::FindPlayer(m_caster->ToPlayer()->GetTarget());

            CallScriptObjectTargetSelectHandlers(
                target, SpellEffIndex(effIndex), SpellImplicitTargetInfo());
    ...
    case EFFECT_IMPLICIT_TARGET_EXPLICIT:
        // player which not released his spirit is Unit, but target flag for it
        // is TARGET_FLAG_CORPSE_MASK
        if (targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK)) {
            if (Unit* unitTarget = m_targets.GetUnitTarget())
                target = unitTarget;
            else if (targetMask & TARGET_FLAG_CORPSE_MASK) {
                if (Corpse* corpseTarget = m_targets.GetCorpseTarget()) {
                    /// @todo: this is a workaround - corpses should be added to
                    /// spell target map too, but we can't do that so we add
                    /// owner instead
                    if (Player* owner = ObjectAccessor::FindPlayer(
                            corpseTarget->GetOwnerGUID()))
                        target = owner;
                }
            }
...
uint32 Spell::GetSearcherTypeMask(SpellTargetObjectTypes objType,
                                  ConditionList*         condList)
{
    // this function selects which containers need to be searched for spell
    // target
                          ...
                          float           radius)
{
    ...
    bool searchInGrid = containerMask & (GRID_MAP_TYPE_MASK_CREATURE |
                                         GRID_MAP_TYPE_MASK_GAMEOBJECT);
    bool searchInWorld =
        containerMask & (GRID_MAP_TYPE_MASK_CREATURE |
                         GRID_MAP_TYPE_MASK_PLAYER | GRID_MAP_TYPE_MASK_CORPSE);
                                       ...
                                       ConditionList*         condList)
{
    ...
    if (!containerTypeMask)
        return nullptr;
    Acore::WorldObjectSpellNearbyTargetCheck check(
        range, m_caster, m_spellInfo, selectionType, condList);
    Acore::WorldObjectLastSearcher<Acore::WorldObjectSpellNearbyTargetCheck>
        searcher(m_caster, target, check, containerTypeMask);
    SearchTargets<Acore::WorldObjectLastSearcher<
        Acore::WorldObjectSpellNearbyTargetCheck>>(
        searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
                              ...
                              ConditionList*           condList)
{
    ...
    if (!containerTypeMask)
        return;
    Acore::WorldObjectSpellAreaTargetCheck check(range,
                                                 position,
                                                 m_caster,
                                                 referer,
                                                 m_spellInfo,
                                                 selectionType,
                                                 condList);
    Acore::WorldObjectListSearcher<Acore::WorldObjectSpellAreaTargetCheck>
        searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<
        Acore::WorldObjectListSearcher<Acore::WorldObjectSpellAreaTargetCheck>>(
        searcher, containerTypeMask, m_caster, position, range);
}
    ...
    bool           isChainHeal)
{
    ...
    // chain lightning/heal spells and similar - allow to jump at larger
    // distance and go out of los
    bool isBouncingFar =
        (m_spellInfo->HasAttribute(SPELL_ATTR4_BOUNCY_CHAIN_MISSILES) ||
    ...
    // for some spells allow only chain targets in front of caster (swipe for
    // example)
    if (!isBouncingFar) {
        for (std::list<WorldObject*>::iterator itr = tempTargets.begin();
             itr != tempTargets.end();) {
            ...
            for (std::list<WorldObject*>::iterator itr = tempTargets.begin();
                ...
                if (Unit* unit = (*itr)->ToUnit()) {
                    ...
                    if ((deficit > maxHPDeficit ||
                         foundItr == tempTargets.end()) &&
                        target->IsWithinDist(unit, jumpRadius) &&
        ...
        // get closest object
            ...
            for (std::list<WorldObject*>::iterator itr = tempTargets.begin();
                ...
                if (foundItr == tempTargets.end()) {
                    if ((!isBouncingFar ||
                         target->IsWithinDist(*itr, jumpRadius)) &&
                        target->IsWithinLOSInMap(*itr,
                                                 VMAP::ModelIgnoreFlags::M2))
                        foundItr = itr;
                }
...
void Spell::prepareDataForTriggerSystem(AuraEffect const* /*triggeredByAura*/)
{
    ...
    // Create base triggers flags for Attacker and Victim (m_procAttacker,
    // m_procVictim and m_procEx)
    //==========================================================================================
    ...
    default:
        ...
        // For other spells trigger procflags are set in
        // Spell::DoAllEffectOnTarget Because spell positivity is dependant on
        // target
    }
    m_procEx = PROC_EX_NONE;

    // Hunter trap spells - activation proc for Lock and Load, Entrapment and
    // Misdirection
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
        (m_spellInfo->SpellFamilyFlags[0] &
             0x18 || // Freezing and Frost Trap, Freezing Arrow
         m_spellInfo->Id == 57879 ||
         m_spellInfo->Id ==
             45145 || // Snake Trap - done this way to avoid double proc
         m_spellInfo->SpellFamilyFlags[2] &
             0x00064000)) // Explosive and Immolation Trap
    {
    ...
    if (m_originalCaster && m_caster != m_originalCaster &&
        ...
        m_caster->ToCreature()->IsTotem() && m_caster->IsControlledByPlayer())
                          ...
                          bool   implicit /*= true*/)
{
    for (uint32 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
        if (!m_spellInfo->Effects[effIndex].IsEffect() ||
            !CheckEffectTarget(target, effIndex))
    ...
    if (checkIfValid) {
        SpellCastResult res =
            m_spellInfo->CheckTarget(m_caster, target, implicit);
    ...
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
        ...
        if (targetGUID == ihit->targetGUID) // Found in list
        {
            ...
            sScriptMgr->OnScaleAuraUnitAdd(this,
                                           target,
                                           effectMask,
                                           checkIfValid,
                                           implicit,
    ...
    if (m_originalCaster) {
        targetInfo.missCondition =
            m_originalCaster->SpellHitResult(target, this, m_canReflect);
    ...
    if (m_spellInfo->Speed > 0.0f && m_caster != target) {
        ...
        /// @todo: this is a hack
        float dist = m_caster->GetDistance(target->GetPositionX(),
                                           target->GetPositionY(),
                                           target->GetPositionZ());
        ...
        targetInfo.timeDelay =
            (uint64)floor(dist / m_spellInfo->Speed * 1000.0f);
    ...
    if (targetInfo.missCondition == SPELL_MISS_REFLECT) {
        ...
        targetInfo.reflectResult =
            m_caster->SpellHitResult(m_caster, this, m_canReflect);

        if (targetInfo.reflectResult ==
            SPELL_MISS_REFLECT) // Impossible reflect again, so simply deflect
                                // spell
            targetInfo.reflectResult = SPELL_MISS_PARRY;

        // Increase time interval for reflected spells by 1.5
        m_caster->m_Events.AddEvent(
            new ReflectEvent(m_caster, targetInfo.targetGUID, m_spellInfo),
            m_caster->m_Events.CalculateTime(targetInfo.timeDelay));
        ...
        if (m_caster->IsPet()) {
            CreatureTemplate const* ci =
                sObjectMgr->GetCreatureTemplate(m_caster->GetEntry());
            switch (ci->family) {
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (!effectUnit &&
        !target->targetGUID
             .IsPlayer()) // only players may be targeted across maps
        return;
    ...
    if (!effectUnit || m_spellInfo->Id == 45927) {
        ...
        // can't use default call because of threading, do stuff as fast as
        // possible
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (farMask & (1 << i))
                HandleEffects(effectUnit,
                              nullptr,
                              nullptr,
                              i,
    ...
    if (getState() == SPELL_STATE_DELAYED && !m_spellInfo->IsTargetingArea() &&
        !m_spellInfo->IsPositive() &&
        (GameTime::GetGameTimeMS().count() - target->timeDelay) <=
            effectUnit->m_lastSanctuaryTime &&
        GameTime::GetGameTimeMS().count() <
            (effectUnit->m_lastSanctuaryTime + 500) &&
    ...
    // Or on missInfo != SPELL_MISS_NONE unitTarget undefined (but need in
    // trigger subsystem)
    ...
    m_spellAura = nullptr; // Set aura to null for every target-make sure that
                           // pointer is not used for unit without aura applied

    PrepareScriptHitHandlers();
    CallScriptBeforeHitHandlers(missInfo);

    // Spells with this flag cannot trigger if effect is casted on self
    bool canEffectTrigger =
        !m_spellInfo->HasAttribute(SPELL_ATTR3_SUPRESS_CASTER_PROCS) &&
        unitTarget->CanProc() &&
        (CanExecuteTriggersOnHit(mask) || missInfo == SPELL_MISS_IMMUNE2);
    ...
    if (missInfo == SPELL_MISS_NONE) // In case spell hit target, do all effect
        ...
        spellHitTarget = unitTarget;
             ...
             SPELL_MISS_REFLECT) // In case spell reflect from target, do all
                                 // effect on caster (if hit)
    {
        missInfo = target->reflectResult;
        if (missInfo == SPELL_MISS_NONE) // If reflected spell hit caster -> do
                                         // all effect on him
        {
    ...
    if (spellHitTarget) {
        SpellMissInfo missInfo2 =
            DoSpellHitOnUnit(spellHitTarget, mask, target->scaleAura);
        if (missInfo2 != SPELL_MISS_NONE) {
            if (missInfo2 != SPELL_MISS_MISS)
                m_caster->SendSpellMiss(
                    spellHitTarget, m_spellInfo->Id, missInfo2);
            m_damage       = 0;
    ...
    if (missInfo != SPELL_MISS_NONE && m_needComboPoints &&
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
            (missInfo == SPELL_MISS_MISS || missInfo == SPELL_MISS_DODGE ||
    ...
    // Trigger info was not filled in spell::preparedatafortriggersystem - we do
    // it now
    if (canEffectTrigger && !procAttacker && !procVictim) {
        ...
        if (m_damage > 0)
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                ...
                // Xinef: if missInfo is immune state check all effects to
                // properly determine positiveness of spell
                if ((missInfo == SPELL_MISS_IMMUNE2 || (mask & (1 << i))) &&
    ...
    if (m_healing > 0) {
        ...
        if (crit) {
            ...
            addhealth = Unit::SpellCriticalHealingBonus(
                caster, m_spellInfo, addhealth, nullptr);
        }
        else
            procEx |= PROC_EX_NORMAL_HIT;

        HealInfo healInfo(caster,
                          unitTarget,
                          addhealth,
                          m_spellInfo,
                          m_spellInfo->GetSchoolMask());
        ...
        unitTarget->getHostileRefMgr().threatAssist(
            caster, float(gain) * 0.5f, m_spellInfo);
        m_healing = gain;
        ...
        // correct drop charge)
        if (canEffectTrigger)
            Unit::ProcDamageAndSpell(caster,
                                     unitTarget,
                                     procAttacker,
                                     procVictim,
                                     procEx,
                                     ...
                                     m_triggeredByAuraSpell.effectIndex,
                                     this,
                                     nullptr,
                                     &healInfo);
    }
    // Do damage and triggers
        ...
        SpellNonMeleeDamage damageInfo(
            caster, unitTarget, m_spellInfo, m_spellSchoolMask);

        // Add bonuses and fill damageInfo struct
        // Dancing Rune Weapon...
        if (m_caster->GetEntry() == 27893) {
            if (Unit* owner = m_caster->GetOwner())
                owner->CalculateSpellDamageTaken(&damageInfo,
                                                 m_damage,
                                                 m_spellInfo,
                                                 m_attackType,
                                                 target->crit);
            ...
            caster->CalculateSpellDamageTaken(
                &damageInfo, m_damage, m_spellInfo, m_attackType, target->crit);
        ...
        if (m_spellInfo->HasEffect(SPELL_EFFECT_HEALTH_LEECH)) {
            ...
            for (; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
                if (m_spellInfo->Effects[effIndex].Effect ==
                    SPELL_EFFECT_HEALTH_LEECH)
                    break;

            float healMultiplier =
                m_spellInfo->Effects[effIndex].CalcValueMultiplier(
                    m_originalCaster, this);

            // get max possible damage, don't count overkill for heal
            uint32 healthGain =
                uint32(-unitTarget->GetHealthGain(-int32(damageInfo.damage)) *
                       healMultiplier);

            if (m_caster->IsAlive()) {
                healthGain = m_caster->SpellHealingBonusDone(
                    m_caster, m_spellInfo, healthGain, HEAL, effIndex);
                healthGain = m_caster->SpellHealingBonusTaken(
                    m_caster, m_spellInfo, healthGain, HEAL);

                HealInfo healInfo(m_caster,
                                  m_caster,
                                  healthGain,
                                  m_spellInfo,
                                  m_spellInfo->GetSchoolMask());
        ...
        if (canEffectTrigger) {
            ...
            if (caster->GetTypeId() == TYPEID_PLAYER &&
                m_spellInfo->HasAttribute(
                    SPELL_ATTR0_CANCELS_AUTO_ATTACK_COMBAT) == 0 &&
                m_spellInfo->HasAttribute(SPELL_ATTR4_SUPRESS_WEAPON_PROCS) ==
                    0 &&
                (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE ||
                 m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED))
                caster->ToPlayer()->CastItemCombatSpell(
    ...
    // Passive spell hits/misses or active spells only misses (only triggers)
        ...
        if (canEffectTrigger) {
            ...
            // Xinef: eg. rogue poisons can proc off cheap shot, etc. so this
            // block should be here also Xinef: ofc count only spells that HIT
            // the target, little hack used to fool the system
        ...
        if (missInfo == SPELL_MISS_RESIST &&
            m_spellInfo->HasAttribute(SPELL_ATTR0_CU_PICKPOCKET) &&
            unitTarget->GetTypeId() == TYPEID_UNIT && m_caster) {
    ...
    if (m_caster) {
        if (missInfo != SPELL_MISS_EVADE &&
            !m_caster->IsFriendlyTo(effectUnit) &&
            (!m_spellInfo->IsPositive() ||
             m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL))) {
            m_caster->CombatStart(effectUnit,
                                  !(m_spellInfo->AttributesEx3 &
                                    SPELL_ATTR3_SUPRESS_TARGET_PROCS));

            // Patch 3.0.8: All player spells which cause a creature to become
            // aggressive to you will now also immediately cause the creature to
            // be tapped.
            ...
            // Unsure if there are more spells that are not supposed to stop
            // enemy from regenerating HP from food, so for now it stays as an
            // ID.
    ...
    if (missInfo != SPELL_MISS_EVADE && effectUnit != m_caster &&
        m_caster->IsFriendlyTo(effectUnit) && m_spellInfo->IsPositive() &&
        effectUnit->IsInCombat() &&
        !m_spellInfo->HasAttribute(SPELL_ATTR1_NO_THREAT)) {
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR7_CAN_CAUSE_INTERRUPT) &&
        effectUnit->GetTypeId() != TYPEID_PLAYER)
    ...
    if (spellHitTarget) {
        ...
        if (spellHitTarget->GetTypeId() == TYPEID_UNIT) {
            if (spellHitTarget->ToCreature()->IsAIEnabled)
                spellHitTarget->ToCreature()->AI()->SpellHit(m_caster,
                                                             m_spellInfo);
        }

        if (m_caster->GetTypeId() == TYPEID_UNIT &&
            m_caster->ToCreature()->IsAIEnabled)
            m_caster->ToCreature()->AI()->SpellHitTarget(spellHitTarget,
                                                         m_spellInfo);

        // Needs to be called after dealing damage/healing to not remove
        // breaking on damage auras
        DoTriggersOnSpellHit(spellHitTarget, mask);

        // if target is fallged for pvp also flag caster if a player
        // xinef: do not flag spells with aura bind sight (no special attribute)
        if (effectUnit->IsPvP() && effectUnit != m_caster &&
            effectUnit->GetOwnerGUID() != m_caster->GetGUID() &&
            m_caster->GetTypeId() == TYPEID_PLAYER &&
            !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_NO_PVP_FLAG)) {
...
SpellMissInfo
Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    if (!unit || !effectMask)
        return SPELL_MISS_EVADE;

    // For delayed spells immunity may be applied between missile launch and hit
    // - check immunity for that case
    if (m_spellInfo->Speed &&
        ((m_damage > 0 && unit->IsImmunedToDamage(this)) ||
         unit->IsImmunedToSchool(this) ||
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
        ...
        if (effectMask & (1 << effectNumber)) {
            ...
            // Xinef: And this is checked in MagicSpellHitResult, why we check
            // resistance twice? Xinef: And why we check every spell effect
            // basing on rand and generic dispel info? some effects will be
            // appliend and some wont?
            /*else if (m_spellInfo->Effects[effectNumber].IsAura() &&
            !m_spellInfo->IsPositiveEffect(effectNumber))
            {
                int32 debuff_resist_chance =
            unit->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE,
            int32(m_spellInfo->Dispel)); debuff_resist_chance +=
            unit->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE,
            int32(m_spellInfo->Dispel));
    ...
    if (unit->GetTypeId() == TYPEID_PLAYER) {
        unit->ToPlayer()->StartTimedAchievement(
            ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET, m_spellInfo->Id);
        unit->ToPlayer()->UpdateAchievementCriteria(
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
        m_caster->ToPlayer()->StartTimedAchievement(
            ACHIEVEMENT_TIMED_TYPE_SPELL_CASTER, m_spellInfo->Id);
        m_caster->ToPlayer()->UpdateAchievementCriteria(
            ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL2, m_spellInfo->Id, 0, unit);
    }
    ...
    if (m_caster != unit) {
        ...
        if (m_caster->_IsValidAttackTarget(unit, m_spellInfo) &&
            /*Intervene Trigger*/ m_spellInfo->Id != 59667) {
            unit->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_HITBYSPELL);
        }
        else if (m_caster->IsFriendlyTo(unit)) {
            // for delayed spells ignore negative spells (after duel end) for
            // friendly targets
            /// @todo: this cause soul transfer bugged
            if (!IsTriggered() && m_spellInfo->Speed > 0.0f &&
            ...
            if (unit->IsInCombat() &&
                !m_spellInfo->HasAttribute(SPELL_ATTR3_SUPRESS_TARGET_PROCS) &&
    ...
    // auras, so this must be done on spell hit, not aura add Xinef: Do not
    // increase diminishing level for self cast
    m_diminishGroup = GetDiminishingReturnsGroupForSpell(
        m_spellInfo, m_triggeredByAuraSpell.spellInfo);
    // xinef: do not increase diminish level for bosses (eg. Void Reaver silence
    // is never diminished)
    if (((m_spellFlags & SPELL_FLAG_REFLECTED) &&
         !(unit->HasAuraType(SPELL_AURA_REFLECT_SPELLS))) ||
        (aura_effmask && m_diminishGroup && unit != m_caster &&
         (m_caster->GetTypeId() != TYPEID_UNIT ||
          !m_caster->ToCreature()->isWorldBoss()))) {
        m_diminishLevel = unit->GetDiminishing(m_diminishGroup);
        DiminishingReturnsType type =
            GetDiminishingReturnsGroupType(m_diminishGroup);

        uint32 flagsExtra =
            unit->GetTypeId() == TYPEID_UNIT
                ? unit->ToCreature()->GetCreatureTemplate()->flags_extra
                : 0;

        // Increase Diminishing on unit, current informations for actually casts
        // will use values above
        if ((type == DRTYPE_PLAYER &&
             (unit->IsCharmedOwnedByPlayerOrPlayer() ||
              flagsExtra & CREATURE_FLAG_EXTRA_ALL_DIMINISH ||
              (m_diminishGroup == DIMINISHING_TAUNT &&
               (flagsExtra &
    ...
    if (m_caster != unit && m_caster->IsHostileTo(unit) &&
        !m_spellInfo->IsPositive() && !m_triggeredByAuraSpell &&
        !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_DONT_BREAK_STEALTH)) {
        unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
    }

    if (aura_effmask) {
        // Select rank for aura with level requirements only in specific cases
        // Unit has to be target only of aura effect, both caster and target
        // have to be players, target has to be other than unit target
        SpellInfo const* aurSpellInfo = m_spellInfo;
        int32            basePoints[3];
        if (scaleAura) {
            aurSpellInfo =
                m_spellInfo->GetAuraRankForLevel(unitTarget->GetLevel());
            ASSERT(aurSpellInfo);
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
                basePoints[i] = aurSpellInfo->Effects[i].BasePoints;
                if (m_spellInfo->Effects[i].Effect !=
                    aurSpellInfo->Effects[i].Effect) {
        ...
        if (m_originalCaster) {
            bool refresh = false;
            bool refreshPeriodic =
                m_spellInfo->StackAmount < 2 &&
                !HasTriggeredCastFlag(TRIGGERED_NO_PERIODIC_RESET);
            m_spellAura = Aura::TryRefreshStackOrCreate(
                aurSpellInfo,
                effectMask,
                unit,
                m_originalCaster,
            ...
            if (m_spellAura) {
                ...
                if (m_spellValue->AuraStackAmount > 1) {
                    if (!refresh)
                        m_spellAura->SetStackAmount(
                            m_spellValue->AuraStackAmount);
                    else
                        m_spellAura->ModStackAmount(
                            m_spellValue->AuraStackAmount);
                }
                ...
                // Xinef: if unit == caster - test versus original unit if
                // available
                float diminishMod = 1.0f;
                if (unit == m_caster && m_targets.GetUnitTarget())
                    diminishMod =
                        m_targets.GetUnitTarget()->ApplyDiminishingToDuration(
                            m_diminishGroup,
                            duration,
                            m_originalCaster,
                            m_diminishLevel,
                    ...
                    diminishMod =
                        unit->ApplyDiminishingToDuration(m_diminishGroup,
                                                         duration,
                                                         m_originalCaster,
                                                         m_diminishLevel,
                                                         limitduration);
                ...
                if (diminishMod == 0.0f) {
                    ...
                    if (AuraApplication* aurApp =
                            m_spellAura->GetApplicationOfTarget(
                                m_originalCaster->GetGUID()))
                        positive = aurApp->IsPositive();

                    duration = m_originalCaster->ModSpellDuration(
                        aurSpellInfo, unit, duration, positive, effectMask);

                    // xinef: haste affects duration of those spells twice
                    if (m_originalCaster->HasAuraTypeWithAffectMask(
                            SPELL_AURA_PERIODIC_HASTE, aurSpellInfo) ||
                        m_spellInfo->HasAttribute(
                            SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC))
                        duration =
                            int32(duration * m_originalCaster->GetFloatValue(
                                                 UNIT_MOD_CAST_SPEED));

                    if (m_spellValue->AuraDuration != 0) {
                        if (m_spellAura->GetMaxDuration() != -1) {
                            m_spellAura->SetMaxDuration(
                    ...
                    if (m_CastItem &&
                        m_CastItem->GetTemplate()->InventoryType ==
                            INVTYPE_RELIC &&
                        m_triggeredByAuraSpell)
                        m_caster->AddSpellCooldown(SPELL_RELIC_COOLDOWN,
                                                   m_CastItem->GetEntry(),
                                                   duration);

                    m_spellAura->SetTriggeredByAuraSpellInfo(
                        m_triggeredByAuraSpell.spellInfo);
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
        ...
        if (m_spellInfo->Effects[effectNumber].Effect ==
...
void Spell::DoTriggersOnSpellHit(Unit* unit, uint8 effMask)
{
    ...
    if (!m_hitTriggerSpells.empty()) {
        int _duration = 0;
        for (HitTriggerSpellList::const_iterator i = m_hitTriggerSpells.begin();
             i != m_hitTriggerSpells.end();
             ++i) {
            if (CanExecuteTriggersOnHit(effMask, i->triggeredByAura) &&
                roll_chance_i(i->chance)) {
                m_caster->CastSpell(unit, i->triggeredSpell->Id, true);
                LOG_DEBUG("spells.aura",
                          "Spell {} triggered spell {} by "
                          "SPELL_AURA_ADD_TARGET_TRIGGER aura",
                          m_spellInfo->Id,
                          i->triggeredSpell->Id);

                // SPELL_AURA_ADD_TARGET_TRIGGER auras shouldn't trigger auras
                // without duration set duration of current aura to the
                // triggered spell
                if (i->triggeredSpell->GetDuration() == -1) {
                    if (Aura* triggeredAur = unit->GetAura(
                            i->triggeredSpell->Id, m_caster->GetGUID())) {
    ...
    /// @todo: remove/cleanup this, as this table is not documented and people
    /// are doing stupid things with it
    if (std::vector<int32> const* spellTriggered =
            sSpellMgr->GetSpellLinked(m_spellInfo->Id + SPELL_LINK_HIT)) {
        for (std::vector<int32>::const_iterator i = spellTriggered->begin();
             i != spellTriggered->end();
             ++i)
            if (*i < 0) {
...
bool Spell::UpdateChanneledTargetList()
{
    ...
    if (channelAuraMask) {
        ...
        if (range == 0)
            for (int i = EFFECT_0; i <= EFFECT_2; ++i)
                if (channelAuraMask & (1 << i) &&
                    m_spellInfo->Effects[i].RadiusEntry) {
                    range =
                        m_spellInfo->Effects[i].CalcRadius(nullptr, nullptr);
                    break;
                }
        ...
        if (ihit->missCondition == SPELL_MISS_NONE &&
            ...
            if (IsValidDeadOrAliveTarget(unit)) {
                if (channelAuraMask & ihit->effectMask) {
                    if (AuraApplication* aurApp = unit->GetAuraApplication(
                            m_spellInfo->Id, m_originalCasterGUID)) {
                        if (m_caster != unit) {
                            ...
                            // Xinef: Update Orientation server side (non
                            // players wont sent appropriate packets)
                            else if (m_spellInfo->HasAttribute(
                                         SPELL_ATTR1_TRACK_TARGET_IN_CHANNEL))
                                m_caster->UpdateOrientation(
                                    m_caster->GetAngle(unit));
                        }
                    }
    ...
    if (channelTargetEffectMask != 0) {
        for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
             ...
             ++ihit)
            if (ihit->missCondition == SPELL_MISS_NONE &&
                (channelAuraMask & ihit->effectMask))
                if (Unit* unit = m_caster->GetGUID() == ihit->targetGUID
                                     ? m_caster
                                     : ObjectAccessor::GetUnit(
                                           *m_caster, ihit->targetGUID))
                    if (IsValidDeadOrAliveTarget(unit))
                        if (AuraApplication* aurApp = unit->GetAuraApplication(
                                m_spellInfo->Id, m_originalCasterGUID)) {
                            ihit->effectMask &= ~aurApp->GetEffectMask();
                               ...
                               AuraEffect const*       triggeredByAura)
{
    ...
    if (sScriptMgr->CanScalingEverything(this) || m_caster->IsTotem() ||
        (m_caster->IsControlledByPlayer() && !m_spellInfo->IsPassive() &&
         m_spellInfo->SpellLevel && !m_spellInfo->IsChanneled() &&
         !HasTriggeredCastFlag(TRIGGERED_IGNORE_AURA_SCALING))) {
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_APPLY_AURA ||
                m_spellInfo->Effects[i].Effect ==
                ...
                // Change aura with ranks only if basepoints are taken from
                // spellInfo and aura is positive
                if (m_spellInfo->IsPositiveEffect(i)) {
                    m_auraScaleMask |= (1 << i);
                    if (m_spellValue->EffectBasePoints[i] !=
                        m_spellInfo->Effects[i].BasePoints) {
                        m_auraScaleMask = 0;
                        break;
                    }
                }
    ...
    if (DisableMgr::IsDisabledFor(
            DISABLE_TYPE_SPELL, m_spellInfo->Id, m_caster)) {
    ...
    if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_CAST_IN_PROGRESS) &&
        m_caster->IsNonMeleeSpellCast(
            false, true, true, m_spellInfo->Id == 75) &&
        m_cast_count) {
    ...
    m_powerCost =
        m_CastItem
            ? 0
            : m_spellInfo->CalcPowerCost(m_caster, m_spellSchoolMask, this);
    ...
    {
        // Periodic auras should be interrupted when aura triggers a spell which
        // can't be cast for example bladestorm aura should be removed on disarm
        // as of patch 3.3.5 channeled periodic spells should be affected by
        // this (arcane missiles, penance, etc) a possible alternative sollution
        // for those would be validating aura target on unit state change
        if (m_caster->GetTypeId() == TYPEID_PLAYER && triggeredByAura &&
        ...
        // Allows to cast melee attack spell if result is
    ...
    m_casttime = HasTriggeredCastFlag(TRIGGERED_CAST_DIRECTLY)
                     ? 0
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) &&
        ...
        !IsTriggered()) {
        // 1. Has casttime, 2. Or doesn't have flag to allow action during
        // channel
    ...
    // xinef: if spell have nearby target entry only, do not allow to cast if no
    // targets are found
    if (m_CastItem) {
        ...
        for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i) {
            ...
            if (m_spellInfo->Effects[i].TargetA.GetSelectionCategory() !=
                    TARGET_SELECT_CATEGORY_NEARBY ||
                m_spellInfo->Effects[i].TargetA.GetCheckType() !=
                    TARGET_CHECK_ENTRY) {
                selectTargets = false;
                break;
            }

            if (m_spellInfo->Effects[i].TargetA.GetObjectType() ==
                TARGET_OBJECT_TYPE_DEST) {
                nearbyDest = true;
            }

            // xinef: by default set it to false, and to true if any valid
            // target is found
            selectTargets = true;
        }
        ...
        if (selectTargets) {
            ...
            if (m_UniqueTargetInfo.empty() && m_UniqueGOTargetInfo.empty()) {
                // no valid nearby target unit or game object found; check if
                // nearby destination type
                if (nearbyDest) {
                    if (!m_targets.HasDst()) {
    ...
    LOG_DEBUG("spells.aura",
              "Spell::prepare: spell id {} source {} caster {} customCastFlags "
              "{} mask {}",
              m_spellInfo->Id,
              m_caster->GetEntry(),
              m_originalCaster ? m_originalCaster->GetEntry() : -1,
              _triggeredCastFlags,
              m_targets.GetTargetMask());

    if (!(m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED) &&
        !(m_spellInfo->Attributes & SPELL_ATTR0_ALLOW_WHILE_SITTING) &&
        !m_triggeredByAuraSpell && m_caster->IsSitState()) {
    ...
    if (HasTriggeredCastFlag(TRIGGERED_CAST_DIRECTLY) &&
        (!m_spellInfo->IsChanneled() || !m_spellInfo->GetMaxDuration()))
        cast(true);
    else {
        // stealth must be removed at cast starting (at show channel bar)
        // skip triggered spell (item equip spell casting and other not explicit
        // character casts/item uses)
        if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS) &&
            ...
            m_caster->RemoveAurasWithInterruptFlags(
                AURA_INTERRUPT_FLAG_CAST, exceptSpellId, m_spellInfo->Id == 75);
            m_caster->RemoveAurasWithInterruptFlags(
        ...
        if ((m_casttime || m_spellInfo->IsChanneled()) &&
            !HasTriggeredCastFlag(TRIGGERED_IGNORE_SET_FACING)) {
            if (m_caster->GetTypeId() == TYPEID_UNIT &&
                !m_caster->ToCreature()->IsInEvadeMode() &&
                ((m_targets.GetObjectTarget() &&
                  m_caster != m_targets.GetObjectTarget()) ||
                 m_spellInfo->IsPositive())) {
                // Xinef: Creature should focus to cast target if there is
                // explicit target or self if casting positive spell Xinef:
                // Creature should not rotate when casting spell... based on
                // halion behavior
                m_caster->ToCreature()->FocusTarget(
                    this,
                    m_targets.GetObjectTarget() != nullptr
                        ? m_targets.GetObjectTarget()
                        : m_caster);
            }
        }
        ...
        //  xinef: removed itemguid check, currently there is no such item in
        //  database
        if (!m_casttime && /*!m_castItemGUID &&*/ GetCurrentContainer() ==
...
void Spell::cancel(bool bySelf)
{
    ...
    case SPELL_STATE_PREPARING:
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER) {
            if (m_caster->ToPlayer()->NeedSendSpectatorData())
                ArenaSpectator::SendCommand_Spell(m_caster->FindMap(),
                                                  m_caster->GetGUID(),
                                                  "SPE",
                                                  m_spellInfo->Id,
                                                  bySelf ? 99998 : 99999);
    ...
    case SPELL_STATE_CASTING:
            ...
            for (std::list<TargetInfo>::const_iterator ihit =
                ...
                if ((*ihit).missCondition == SPELL_MISS_NONE)
                    if (Unit* unit = m_caster->GetGUID() == ihit->targetGUID
                                         ...
                                         : ObjectAccessor::GetUnit(
                                               *m_caster, ihit->targetGUID))
                        unit->RemoveOwnedAura(m_spellInfo->Id,
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
            m_caster->ToPlayer()->NeedSendSpectatorData())
            ArenaSpectator::SendCommand_Spell(m_caster->FindMap(),
                                              m_caster->GetGUID(),
                                              "SPE",
                                              m_spellInfo->Id,
    ...
    // Do not remove current far sight object (already done in
    // Spell::EffectAddFarsight) to prevent from reset viewpoint to player
    if (!(bySelf && m_spellInfo->HasEffect(SPELL_EFFECT_ADD_FARSIGHT))) {
        m_caster->RemoveDynObject(m_spellInfo->Id);
    }

    if (m_spellInfo->IsChanneled()) // if not channeled then the object for the
                                    // current cast wasn't summoned yet
...
{
    // update pointers base at GUIDs to prevent access to non-existed already
    // object
    if (!UpdatePointers()) {
        // cancel the spell if UpdatePointers() returned false, something wrong
        // happened there
        cancel();
        return;
    }
    ...
    // Xinef: implement attribute SPELL_ATTR1_DISMISS_PET_FIRST, on spell cast
    // current pet is dismissed and charms are removed
    if (m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET_FIRST)) {
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
            !m_spellInfo->HasEffect(SPELL_EFFECT_SUMMON_PET))
    ...
    if (Player* playerCaster = m_caster->ToPlayer()) {
        ...
        // Let any pets know we've attacked something. Check DmgClass for
        // harmful spells only This prevents spells such as Hunter's Mark from
        // triggering pet attack xinef: take into account
        // SPELL_ATTR3_SUPRESS_TARGET_PROCS
        if ((m_targets.GetTargetMask() & TARGET_FLAG_UNIT) &&
            GetSpellInfo()->DmgClass != SPELL_DAMAGE_CLASS_NONE &&
            !GetSpellInfo()->HasAttribute(SPELL_ATTR3_SUPRESS_TARGET_PROCS))
            if (!playerCaster->m_Controlled.empty())
                for (Unit::ControlSet::iterator itr =
                         playerCaster->m_Controlled.begin();
                     itr != playerCaster->m_Controlled.end();
                     ++itr)
                    if (Unit* pet = *itr)
                        if (pet->IsAlive() && pet->GetTypeId() == TYPEID_UNIT)
                            pet->ToCreature()->AI()->OwnerAttacked(
    ...
    if (!skipCheck) {
        ...
        if (m_targets.GetTargetMask() & TARGET_FLAG_TRADE_ITEM) {
            if (m_caster->GetTypeId() == TYPEID_PLAYER) {
                if (TradeData* my_trade =
                        m_caster->ToPlayer()->GetTradeData()) {
                    if (!my_trade->IsInAcceptProcess()) {
                        // Spell will be casted at completing the trade.
                        // Silently ignore at this place
    ...
    if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_POWER_AND_REAGENT_COST)) {
        ...
        /// Not own traded item (in trader trade slot) req. reagents including
        /// triggered spell case
    ...
    // we must send smsg_spell_go packet before m_castItem delete in
    // TakeCastItem()...
    SendSpellGo();
    ...
    if (m_originalCaster) {
        ...
        Unit::ProcDamageAndSpell(m_originalCaster,
                                 m_originalCaster,
                                 procAttacker,
                                 PROC_FLAG_NONE,
                                 procEx,
    ...
    bool resetAttackTimers =
        IsAutoActionResetSpell() &&
        !m_spellInfo->HasAttribute(SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS);
    ...
    {
        // Remove used for cast item if need (it can be already nullptr after
        // TakeReagents call in case delayed spell remove item at cast delay
        // start
        TakeCastItem();
        ...
        if (m_caster->HasUnitState(UNIT_STATE_CASTING) &&
            !m_caster->IsNonMeleeSpellCast(false, false, true))
            m_caster->ClearUnitState(UNIT_STATE_CASTING);

        // remove all applied mods at this point
        // dont allow user to use them twice in case spell did not reach current
        // target
        if (modOwner)
            modOwner->RemoveSpellMods(this);

        // Xinef: why do we keep focus after spell is sent to air?
        // Xinef: Because of this, in the middle of some animation after setting
        // targetguid to 0 etc Xinef: we get focused to it out of nowhere...
    ...
    if (const std::vector<int32>* spell_triggered =
            sSpellMgr->GetSpellLinked(m_spellInfo->Id)) {
        for (std::vector<int32>::const_iterator i = spell_triggered->begin();
             i != spell_triggered->end();
             ++i)
            if (*i < 0)
                m_caster->RemoveAurasDueToSpell(-(*i));
            else
                m_caster->CastSpell(m_targets.GetUnitTarget()
                                        ? m_targets.GetUnitTarget()
                                        : m_caster,
                                    *i,
                                    true);
    ...
    // handle this here, in other places SpellHitTarget can be set to nullptr,
    // if there is an error in this function
    if (m_spellInfo->HasAttribute(SPELL_ATTR7_CAN_CAUSE_INTERRUPT))
        if (Unit* target = m_targets.GetUnitTarget())
    ...
    if (Unit* target = m_targets.GetUnitTarget())
        if (m_caster->GetTypeId() == TYPEID_PLAYER ||
            (m_caster->IsPet() && m_caster->IsControlledByPlayer()))
            if (GetDelayMoment() > 0 && !m_caster->IsFriendlyTo(target) &&
                !m_spellInfo->HasAura(SPELL_AURA_BIND_SIGHT) &&
                (!m_spellInfo->IsPositive() ||
                 m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
                m_caster->CombatStartOnCast(
                    target,
                    !m_spellInfo->HasAttribute(
                        SPELL_ATTR3_SUPRESS_TARGET_PROCS),
                    GetDelayMoment() +
                        500); // xinef: increase this time so we dont leave and
                              // enter combat in a moment
...
void Spell::handle_immediate()
{
    ...
    if (m_spellInfo->IsChanneled()) {
        ...
        if (duration > 0) {
            ...
            // Apply haste mods
            if (m_caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE,
                                                    m_spellInfo) ||
                m_spellInfo->HasAttribute(
                    SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC))
...
{
    if (!UpdatePointers()) {
        // finish the spell if UpdatePointers() returned false, something wrong
        // happened there
        finish(false);
        return 0;
    }
    ...
    // now recheck units targeting correctness (need before any effects apply to
    // prevent adding immunity at first effect not allow apply second spell
    // effect and similar cases)
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
         ihit != m_UniqueTargetInfo.end();
         ++ihit) {
    ...
    for (std::list<GOTargetInfo>::iterator ighit = m_UniqueGOTargetInfo.begin();
         ighit != m_UniqueGOTargetInfo.end();
         ++ighit) {
...
void Spell::_handle_finish_phase()
{
    ...
    if (m_comboTarget && m_comboPointGain) {
        ...
        // (this Aura removes the already-added CP when it expires from duration
        // - now that we've added CP, this shouldn't happen anymore!)
        ...
        if (m_caster->GetCharmerOrOwnerPlayerOrPlayerItself())
            for (std::list<TargetInfo>::iterator ihit =
                ...
                if (ihit->targetGUID == m_caster->GetGUID() &&
                    ...
                    if (m_caster->GetTypeId() == TYPEID_PLAYER &&
                        m_spellInfo->IsCooldownStartedOnEvent())
...
void Spell::SendSpellCooldown()
{
    ...
    if (m_caster->GetTypeId() != TYPEID_PLAYER) {
        if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD)) {
            ...
            // xinef: this adds cooldowns to vehicle spells which misses them
            // client-side (when we overwrote dbc info in eg.)
    ...
    // mana/health/etc potions, disabled by client (until combat out as
    // declarate)
    if (m_CastItem &&
    ...
    // have infinity cooldown but set at aura apply                  // do not
    // set cooldown for triggered spells (needed by reincarnation)
    if (m_spellInfo->IsCooldownStartedOnEvent() || m_spellInfo->IsPassive() ||
...
void Spell::update(uint32 difftime)
{
    ...
    if (m_targets.GetUnitTargetGUID() && !m_targets.GetUnitTarget()) {
        LOG_DEBUG("spells.aura",
                  "Spell {} is cancelled due to removal of target.",
                  m_spellInfo->Id);
        cancel();
        return;
    }
    ...
    // xinef: added preparing state (real cast, skip channels as they have other
    // flags for this)
    if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_timer != 0) &&
        m_caster->isMoving() &&
        (m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT) &&
        m_spellState == SPELL_STATE_PREPARING &&
        (m_spellInfo->Effects[0].Effect != SPELL_EFFECT_STUCK ||
         !m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR))) {
    ...
    case SPELL_STATE_PREPARING: {
        ...
        if (m_timer == 0 && !IsNextMeleeSwingSpell() && !IsAutoRepeat())
            // don't CheckCast for instant spells - done in spell::prepare, skip
            // duplicate checks, needed for range checks for example
            cast(!m_casttime);
        break;
    }
    case SPELL_STATE_CASTING: {
        ...
        // Xinef: Dont update channeled target list on last tick, allow auras to
        // update duration properly Xinef: Added this strange check because of
        // diffrent update routines for players / creatures Xinef: If creature
        // gets new aura in 800ms and in 840ms its updated with diff 270, not 40
        // is removed from duration but 270 Xinef: so the aura can be removed in
        // different updates for all units
        else if ((m_timer < 0 || m_timer > 300) &&
                 !UpdateChanneledTargetList()) {
            LOG_DEBUG("spells.aura",
                      "Channeled spell {} is removed due to lack of targets",
                      m_spellInfo->Id);
            SendChannelUpdate(0);
            finish();
        }
...
void Spell::finish(bool ok)
{
    ...
    if (!ok) {
        if (m_caster->GetTypeId() == TYPEID_PLAYER) {
            ...
            if (m_spellInfo->IsCooldownStartedOnEvent())
                m_caster->ToPlayer()->SendCooldownEvent(
                    m_spellInfo, 0, 0, false);
        }
        return;
    }

    // pussywizard:
    if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_ENCOUNTER_REWARD) &&
        m_caster->FindMap())
        m_caster->FindMap()->UpdateEncounterState(
            ENCOUNTER_CREDIT_CAST_SPELL, m_spellInfo->Id, m_caster);
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT &&
        ...
        if (spellInfo && spellInfo->SpellIconID == 2056) {
            LOG_DEBUG("spells.aura",
                      "Statue {} is unsummoned in spell {} finish",
                      m_caster->GetGUID().ToString(),
                      m_spellInfo->Id);
    ...
    // mods are taken only on succesfull cast and independantly from targets of
    // the spell
    if (Player* player = m_caster->GetSpellModOwner())
        player->RemoveSpellMods(this);

    // xinef: clear reactive auras states after spell cast
    if (m_spellInfo->CasterAuraState == AURA_STATE_DEFENSE ||
        m_spellInfo->CasterAuraState == AURA_STATE_HUNTER_PARRY)
        m_caster->ModifyAuraState(AuraStateType(m_spellInfo->CasterAuraState),
                                  false);
                                ...
                                SpellCustomErrors customError)
{
    ...
    case SPELL_FAILED_REQUIRES_SPELL_FOCUS:
        data << uint32(
            spellInfo->RequiresSpellFocus); // SpellFocusObject.dbc id
        break;
    ...
    SpellCustomErrors customError /*= SPELL_CUSTOM_ERROR_NONE*/)
{
    ...
    WriteCastResultInfo(
        data, caster, spellInfo, castCount, result, customError);
...
void Spell::SendCastResult(SpellCastResult result)
{
    ...
    // Xinef: override every possible result, except for gm fail result...
    // breaks many things and goes unnoticed because of this and makes me rage
    // when i find this out
    if (HasTriggeredCastFlag(TRIGGERED_DONT_REPORT_CAST_ERROR) &&
        result != SPELL_FAILED_BM_OR_INVISGOD)
        result = SPELL_FAILED_DONT_REPORT;

    SendCastResult(
        m_caster->ToPlayer(), m_spellInfo, m_cast_count, result, m_customError);
}
...
void Spell::SendPetCastResult(SpellCastResult result)
{
    ...
    WriteCastResultInfo(
        data, player, m_spellInfo, m_cast_count, result, m_customError);
...
void Spell::SendSpellStart()
{
    ...
    // LOG_DEBUG("spells.aura", "Sending SMSG_SPELL_START id={}",
    // m_spellInfo->Id);

    uint32 castFlags = CAST_FLAG_HAS_TRAJECTORY;

    if (((IsTriggered() && !m_spellInfo->IsAutoRepeatRangedSpell()) ||
         m_triggeredByAuraSpell) &&
        !m_cast_count &&
        !(m_spellInfo->IsChanneled() ||
          (m_spellInfo->CastTimeEntry &&
           m_spellInfo->CastTimeEntry->CastTime > 0)))
    ...
    if (TempSummon const* tempSummon = m_caster->ToTempSummon()) {
        if (tempSummon->GetEntry() == WORLD_TRIGGER) {
            if (GameObject* casterGameobject =
                    tempSummon->GetSummonerGameObject()) {
...
void Spell::SendSpellGo()
{
    ...
    // should only be sent to self, but the current messaging doesn't make that
    // possible
    if (m_caster->GetTypeId() == TYPEID_PLAYER || m_caster->IsPet()) {
        switch (m_spellInfo->PowerType) {
        case POWER_HEALTH:
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER) &&
        ...
        m_spellInfo->RuneCostID && m_spellInfo->PowerType == POWER_RUNE) {
        castFlags |= CAST_FLAG_NO_GCD;    // not needed, but Blizzard sends it
    ...
    {
        // TODO: There is a crash caused by a spell with CAST_FLAG_RUNE_LIST
        // casted by a creature The creature is the mover of a player, so
        // HandleCastSpellOpcode uses it as the caster
        if (Player* player = m_caster->ToPlayer()) {
            ...
            for (uint8 i = 0; i < MAX_RUNES; ++i) {
                uint8 mask = (1 << i);
                if (mask & runeMaskInitial &&
                    !(mask &
                      runeMaskAfterCast)) // usable before andon cooldown now...
                {
                    // float casts ensure the division is performed on floats as
                    // we need float result
                    float baseCd = float(player->GetRuneBaseCooldown(i, true));
                    data << uint8((baseCd - float(player->GetRuneCooldown(i))) /
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
        ...
        if (pItem) {
            ...
            if (ammoInventoryType == INVTYPE_THROWN)
                ...
                uint32 ammoID =
                    m_caster->ToPlayer()->GetUInt32Value(PLAYER_AMMO_ID);
                if (ammoID) {
                    ItemTemplate const* pProto =
                        sObjectMgr->GetItemTemplate(ammoID);
                    if (pProto) {
        ...
        for (uint8 i = 0; i < 3; ++i) {
            if (uint32 item_id =
                    m_caster->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i)) {
                if (ItemEntry const* itemEntry =
                        sItemStore.LookupEntry(item_id)) {
                    if (itemEntry->ClassID == ITEM_CLASS_WEAPON) {
                        ...
                        default:
                            ...
                            nonRangedAmmoInventoryType =
                                itemEntry->InventoryType;
                            break;
                        }
...
void Spell::WriteSpellGoTargets(WorldPacket* data)
{
    ...
    // Hit and miss target counts are both uint8, that limits us to 255 targets
    // for each sending more than 255 targets crashes the client (since count
    // sent would be wrong) Spells like 40647 (with a huge radius) can easily
    // reach this limit (spell might need target conditions but we still need to
    // limit the number of targets sent and keeping correct count for both hit
    // and miss).
    ...
    *data << (uint8)0; // placeholder
    for (std::list<TargetInfo>::const_iterator ihit =
             m_UniqueTargetInfo.begin();
         ihit != m_UniqueTargetInfo.end() && hit < 255;
         ++ihit) {
    ...
    for (std::list<GOTargetInfo>::const_iterator ighit =
         ...
         ighit != m_UniqueGOTargetInfo.end() && hit < 255;
         ++ighit) {
    ...
    *data << (uint8)0; // placeholder
    for (std::list<TargetInfo>::const_iterator ihit =
             m_UniqueTargetInfo.begin();
         ihit != m_UniqueTargetInfo.end() && miss < 255;
         ++ihit) {
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    if (!channelTarget && !m_spellInfo->NeedsExplicitUnitTarget())
        if (m_UniqueTargetInfo.size() + m_UniqueGOTargetInfo.size() ==
            1) // this is for TARGET_SELECT_CATEGORY_NEARBY
            channelTarget = !m_UniqueTargetInfo.empty()
                                ? m_UniqueTargetInfo.front().targetGUID
                                : m_UniqueGOTargetInfo.front().targetGUID;
...
{
    // get resurrector name for creature resurrections, otherwise packet will be
    // not accepted for player resurrections the name is looked up by guid
    std::string const sentName(
        m_caster->GetTypeId() == TYPEID_PLAYER
            ? ""
    ...
    data << uint8(m_caster->GetTypeId() == TYPEID_PLAYER
                      ? 0
                      : 1); // "you'll be afflicted with resurrection sickness"
    // override delay sent with SMSG_CORPSE_RECLAIM_DELAY, set instant
    // resurrection for spells with this attribute
...
void Spell::TakeCastItem()
{
    ...
    if (!proto) {
        ...
        // needs a prototype
        LOG_ERROR("spells",
                  "Cast item has no item prototype {}",
                  m_CastItem->GetGUID().ToString());
        return;
    }
        ...
        if (proto->Spells[i].SpellId) {
            ...
            if (proto->Spells[i].SpellCharges) {
                ...
                if (charges) {
                    (charges > 0)
                        ? --charges
                        : ++charges; // std::abs(charges) less at 1 after use
...
void Spell::TakePower()
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
        if (PowerType == POWER_RAGE || PowerType == POWER_ENERGY ||
            PowerType == POWER_RUNE || PowerType == POWER_RUNIC_POWER)
            if (ObjectGuid targetGUID = m_targets.GetUnitTargetGUID())
                for (std::list<TargetInfo>::iterator ihit =
                    ...
                    if (ihit->targetGUID == targetGUID) {
                        if (ihit->missCondition != SPELL_MISS_NONE &&
                            ihit->missCondition != SPELL_MISS_BLOCK &&
                            ihit->missCondition != SPELL_MISS_ABSORB &&
                            ihit->missCondition != SPELL_MISS_REFLECT) {
                            hit = false;
                            // lower spell cost on fail (by talent aura)
                            if (Player* modOwner =
                                    m_caster->ToPlayer()->GetSpellModOwner())
                                modOwner->ApplySpellMod(
                                    m_spellInfo->Id,
    ...
    if (PowerType >= MAX_POWERS) {
        LOG_ERROR(
            "spells", "Spell::TakePower: Unknown power type '{}'", PowerType);
        return;
    }
...
SpellCastResult Spell::CheckRuneCost(uint32 RuneCostID)
{
    ...
    // If we are in .cheat power mode we dont need to check the cost as we are
    // expected to be able to use it anyways (infinite power)
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    SpellRuneCostEntry const* runeCostData =
        sSpellRuneCostStore.LookupEntry(m_spellInfo->RuneCostID);
    if (!runeCostData ||
        (runeCostData->NoRuneCost() && runeCostData->NoRunicPowerGain()))
        return;
    ...
    for (uint32 i = 0; i < MAX_RUNES; ++i) {
        ...
        if (!player->GetRuneCooldown(i) && runeCost[rune] > 0) {
            player->SetRuneCooldown(i,
                                    didHit
                                        ? player->GetRuneBaseCooldown(i, false)
                                        : uint32(RUNE_MISS_COOLDOWN));
    ...
    for (uint8 loop = 0; loop < 2; ++loop) {
        runeCost[RUNE_DEATH] =
            runeCost[RUNE_BLOOD] + runeCost[RUNE_UNHOLY] + runeCost[RUNE_FROST];
        if (runeCost[RUNE_DEATH] > 0) {
            for (uint8 i = 0; i < MAX_RUNES; ++i) {
                RuneType rune = player->GetCurrentRune(i);
                if (!player->GetRuneCooldown(i) && rune == RUNE_DEATH &&
                    (loop ? true : (runeCost[player->GetBaseRune(i)] > 0))) {
                    player->SetRuneCooldown(
                        i,
                        didHit ? player->GetRuneBaseCooldown(i, false)
                               : uint32(RUNE_MISS_COOLDOWN));
                    player->SetLastUsedRune(rune);
                    runeCost[rune]--;
                    if (!loop)
    ...
    if (didHit)
        if (int32 rp = int32(runeCostData->runePowerGain *
                             sWorld->getRate(RATE_POWER_RUNICPOWER_INCOME)))
...
void Spell::TakeReagents()
{
    ...
    ItemTemplate const* castItemTemplate =
        m_CastItem ? m_CastItem->GetTemplate() : nullptr;
    ...
    for (uint32 x = 0; x < MAX_SPELL_REAGENTS; ++x) {
        ...
        if (castItemTemplate && castItemTemplate->ItemId == itemid) {
            for (int s = 0; s < MAX_ITEM_PROTO_SPELLS; ++s) {
                ...
                if (castItemTemplate->Spells[s].SpellCharges < 0 &&
                    std::abs(charges) < 2) {
                    ++itemcount;
                    break;
                }
...
void Spell::HandleThreatSpells()
{
    ...
    float threat = 0.0f;
    if (SpellThreatEntry const* threatEntry =
            sSpellMgr->GetSpellThreatEntry(m_spellInfo->Id)) {
        if (threatEntry->apPctMod != 0.0f)
            threat += threatEntry->apPctMod *
    ...
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
        ...
        if (m_spellInfo->_IsPositiveSpell() && IsFriendly)
            target->getHostileRefMgr().threatAssist(
                m_caster, threatToAdd, m_spellInfo);
        // for negative spells threat gets distributed among affected targets
        else if (!m_spellInfo->_IsPositiveSpell() && !IsFriendly &&
                 target->CanHaveThreatList())
            target->AddThreat(m_caster,
                              threatToAdd,
                              m_spellInfo->GetSchoolMask(),
                              m_spellInfo);
    }
    LOG_DEBUG("spells.aura",
              "Spell {}, added an additional {} threat for {} {} target(s)",
              m_spellInfo->Id,
              threat,
              m_spellInfo->_IsPositiveSpell() ? "assisting" : "harming",
...
SpellCastResult Spell::CheckCast(bool strict)
{
    // check death state
    if (!m_caster->IsAlive() &&
        !m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE) &&
        !(m_spellInfo->HasAttribute(SPELL_ATTR0_ALLOW_CAST_WHILE_DEAD) ||
          (IsTriggered() && !m_triggeredByAuraSpell)))
        return SPELL_FAILED_CASTER_DEAD;

    // Spectator check
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
        if (((Player const*)m_caster)->IsSpectator() &&
            m_spellInfo->Id != SPECTATOR_SPELL_BINDSIGHT)
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE)) {
        if (m_caster->GetTypeId() == TYPEID_PLAYER) {
            ...
            if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD) &&
                m_caster->ToPlayer()->HasSpellCooldown(m_spellInfo->Id)) {
            ...
            if (m_caster->ToPlayer()->GetLastPotionId() && m_CastItem &&
                (m_CastItem->IsPotion() ||
                 m_spellInfo->IsCooldownStartedOnEvent()))
                return SPELL_FAILED_NOT_READY;
        }
        else if (!IsTriggered() && m_caster->GetTypeId() == TYPEID_UNIT &&
                 m_caster->ToCreature()->IsSpellProhibited(
                     m_spellInfo->GetSchoolMask()))
            return SPELL_FAILED_NOT_READY;
    }

    if (m_spellInfo->HasAttribute(SPELL_ATTR7_DEBUG_SPELL) &&
        !m_caster->HasUnitFlag2(UNIT_FLAG2_ALLOW_CHEAT_SPELLS)) {
        m_customError = SPELL_CUSTOM_ERROR_GM_ONLY;
        return SPELL_FAILED_CUSTOM_ERROR;
    }

    // Check global cooldown
    if (strict && !HasTriggeredCastFlag(TRIGGERED_IGNORE_GCD) &&
        HasGlobalCooldown())
        return SPELL_FAILED_NOT_READY;
    ...
    // call for now, ignore triggered spells
    if (strict && !HasTriggeredCastFlag(TRIGGERED_IGNORE_SHAPESHIFT)) {
        bool checkForm = true;
        // Ignore form req aura
        Unit::AuraEffectList const& ignore =
            m_caster->GetAuraEffectsByType(SPELL_AURA_MOD_IGNORE_SHAPESHIFT);
        ...
        if (checkForm) {
            ...
            SpellCastResult shapeError =
                m_spellInfo->CheckShapeshift(m_caster->GetShapeshiftForm());
            if (shapeError != SPELL_CAST_OK)
                return shapeError;

            if (m_spellInfo->HasAttribute(SPELL_ATTR0_ONLY_STEALTHED) &&
                !(m_caster->HasStealthAura()))
                return SPELL_FAILED_ONLY_STEALTHED;
        }
    }

    Unit::AuraEffectList const& blockSpells =
        m_caster->GetAuraEffectsByType(SPELL_AURA_BLOCK_SPELL_FAMILY);
    for (Unit::AuraEffectList::const_iterator blockItr = blockSpells.begin();
         blockItr != blockSpells.end();
         ++blockItr)
        if (uint32((*blockItr)->GetMiscValue()) == m_spellInfo->SpellFamilyName)
            return SPELL_FAILED_SPELL_UNAVAILABLE;

    bool                        reqCombat = true;
    Unit::AuraEffectList const& stateAuras =
        m_caster->GetAuraEffectsByType(SPELL_AURA_ABILITY_IGNORE_AURASTATE);
    ...
    if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_CASTER_AURASTATE)) {
        if (m_spellInfo->CasterAuraState &&
            !m_caster->HasAuraState(AuraStateType(m_spellInfo->CasterAuraState),
                                    m_spellInfo,
                                    m_caster))
            return SPELL_FAILED_CASTER_AURASTATE;
        if (m_spellInfo->CasterAuraStateNot &&
            m_caster->HasAuraState(
                AuraStateType(m_spellInfo->CasterAuraStateNot),
                m_spellInfo,
                m_caster))
            return SPELL_FAILED_CASTER_AURASTATE;
        ...
        if (m_spellInfo->CasterAuraSpell &&
            !m_caster->HasAura(sSpellMgr->GetSpellIdForDifficulty(
                m_spellInfo->CasterAuraSpell, m_caster)))
            return SPELL_FAILED_CASTER_AURASTATE;
        if (m_spellInfo->ExcludeCasterAuraSpell &&
            m_caster->HasAura(sSpellMgr->GetSpellIdForDifficulty(
                m_spellInfo->ExcludeCasterAuraSpell, m_caster)))
            return SPELL_FAILED_CASTER_AURASTATE;
    ...
    if (reqCombat && !m_spellInfo->CanBeUsedInCombat() &&
        (m_spellInfo->HasEffect(SPELL_EFFECT_RESURRECT) ||
         m_spellInfo->HasEffect(SPELL_EFFECT_RESURRECT_NEW))) {
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
            m_caster->GetMap()->IsDungeon())
    ...
    // spell update code)
    if (m_caster->GetTypeId() == TYPEID_PLAYER &&
        m_caster->ToPlayer()->isMoving() && !IsTriggered()) {
        // skip stuck spell to allow use it in falling case and apply spell
        // limitations at movement
        if ((!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR) ||
             m_spellInfo->Effects[0].Effect != SPELL_EFFECT_STUCK) &&
            (IsAutoRepeat() || (m_spellInfo->AuraInterruptFlags &
    ...
    if (vehicle &&
        !HasTriggeredCastFlag(TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE)) {
        uint16 checkMask = 0;
        for (uint8 effIndex = EFFECT_0; effIndex < MAX_SPELL_EFFECTS;
             ++effIndex) {
            SpellEffectInfo const* effInfo = &m_spellInfo->Effects[effIndex];
            if (effInfo->ApplyAuraName == SPELL_AURA_MOD_SHAPESHIFT) {
                SpellShapeshiftEntry const* shapeShiftEntry =
                    sSpellShapeshiftStore.LookupEntry(effInfo->MiscValue);
                if (shapeShiftEntry &&
                    (shapeShiftEntry->flags1 & 1) == 0) // unk flag
        ...
        // All creatures should be able to cast as passengers freely,
        // restriction and attribute are only for players
        VehicleSeatEntry const* vehicleSeat =
            vehicle->GetSeatForPassenger(m_caster);
    ...
    // check spell cast conditions from database
    {
        ...
        if (!conditions.empty() &&
            ...
            // mLastFailedCondition can be nullptr if there was an error
            // processing the condition in Condition::Meets (i.e. wrong data for
            // ConditionTarget or others)
            if (condInfo.mLastFailedCondition &&
                condInfo.mLastFailedCondition->ErrorType) {
                if (condInfo.mLastFailedCondition->ErrorType ==
                    SPELL_FAILED_CUSTOM_ERROR)
                    m_customError = SpellCustomErrors(
                        condInfo.mLastFailedCondition->ErrorTextId);
                return SpellCastResult(
                    condInfo.mLastFailedCondition->ErrorType);
            }
            if (!condInfo.mLastFailedCondition ||
                !condInfo.mLastFailedCondition->ConditionTarget)
                return SPELL_FAILED_CASTER_AURASTATE;
            return SPELL_FAILED_BAD_TARGETS;
        }
    }

    // Don't check explicit target for passive spells (workaround) (check should
    // be skipped only for learn case) those spells may have incorrect target
    // entries or not filled at all (for example 15332) such spells when learned
    // are not targeting anyone using targeting system, they should apply
    // directly to caster instead also, such casts shouldn't be sent to client
    // Xinef: do not check explicit casts for self cast of triggered spells (eg.
    // reflect case)
    if (!(m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE) &&
          (!m_targets.GetUnitTarget() ||
           m_targets.GetUnitTarget() == m_caster))) {
        // Check explicit target for m_originalCaster - todo: get rid of such
        // workarounds Xinef: do not check explicit target for triggered spell
        // casted on self with targetflag enemy
        if (!m_triggeredByAuraSpell || m_targets.GetUnitTarget() != m_caster ||
            !(m_spellInfo->GetExplicitTargetMask() & TARGET_FLAG_UNIT_ENEMY)) {
            SpellCastResult castResult = m_spellInfo->CheckExplicitTarget(
                (m_originalCaster && m_caster->GetEntry() != WORLD_TRIGGER)
                    ? m_originalCaster
                    : m_caster,
                m_targets.GetObjectTarget(),
    ...
    if (Unit* target = m_targets.GetUnitTarget()) {
        SpellCastResult castResult =
            m_spellInfo->CheckTarget(m_caster, target, false);
        if (castResult != SPELL_CAST_OK)
            return castResult;
        ...
        if (target != m_caster) {
            ...
            if ((!m_caster->IsTotem() || !m_spellInfo->IsPositive()) &&
                !m_spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) &&
                !m_spellInfo->HasAttribute(
                ...
                if (m_originalCasterGUID.IsGameObject()) {
                    castedByGameobject = m_caster->GetMap()->GetGameObject(
                                             m_originalCasterGUID) != nullptr;
                }
                else if (m_caster->GetEntry() == WORLD_TRIGGER) {
                    if (TempSummon* tempSummon = m_caster->ToTempSummon()) {
                        castedByGameobject =
                            tempSummon->GetSummonerGameObject() != nullptr;
                    }
                }
    ...
    for (int j = 0; j < MAX_SPELL_EFFECTS; ++j) {
        if (m_spellInfo->Effects[j].TargetA.GetTarget() == TARGET_UNIT_PET) {
            if (!m_caster->GetGuardianPet() && !m_caster->GetCharm()) {
                if (m_triggeredByAuraSpell
                        .spellInfo) // not report pet not existence for
                                    // triggered spells
                    return SPELL_FAILED_DONT_REPORT;
                else
    ...
    // - with greater than 10 min CD without
    // SPELL_ATTR4_IGNORE_DEFAULT_ARENA_RESTRICTIONS flag
    // - with SPELL_ATTR4_NOT_IN_ARENA_OR_RATED_BATTLEGROUND flag
    if (m_spellInfo->HasAttribute(
            SPELL_ATTR4_NOT_IN_ARENA_OR_RATED_BATTLEGROUND) ||
        (m_spellInfo->GetRecoveryTime() >= 10 * MINUTE * IN_MILLISECONDS &&
         !m_spellInfo->HasAttribute(
             SPELL_ATTR4_IGNORE_DEFAULT_ARENA_RESTRICTIONS)))
        if (MapEntry const* mapEntry =
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT ||
        ...
        SpellCastResult locRes = m_spellInfo->CheckLocation(
            m_caster->GetMapId(),
            zone,
            area,
            m_caster->GetTypeId() == TYPEID_PLAYER ? m_caster->ToPlayer()
                                                   : nullptr);
    ...
    if (!m_spellInfo->IsPassive()) {
        // spell focus needs to be checked not only for players! there are
        // vehicle spells that require spell focus
    ...
    // xinef: do not skip triggered spells if they posses prevention type (eg.
    // Bladestorm vs Hand of Protection)
    if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_CASTER_AURAS) ||
        (m_spellInfo->PreventionType > SPELL_PREVENTION_TYPE_NONE &&
         m_triggeredByAuraSpell &&
         m_triggeredByAuraSpell.spellInfo->IsPositive())) {
        castResult = CheckCasterAuras(
            HasTriggeredCastFlag(TRIGGERED_IGNORE_CASTER_AURAS));
        if (castResult != SPELL_CAST_OK)
            return castResult;

        // xinef: Enraged Regeneration: While this is active, the warrior is
        // blocked from using abilities that trigger being enraged (which would
        // do nothing and waste the cooldowns).
        if (m_spellInfo->Mechanic && m_spellInfo->IsSelfCast()) {
            SpellImmuneList const& mechanicList =
                m_caster->m_spellImmune[IMMUNITY_MECHANIC];
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_DISPEL) {
            if (m_spellInfo->Effects[i].IsTargetingArea() ||
                m_spellInfo->HasAttribute(SPELL_ATTR1_INITIATE_COMBAT)) {
                hasDispellableAura = true;
                break;
            }

            dispelMask |= SpellInfo::GetDispelMask(
                DispelType(m_spellInfo->Effects[i].MiscValue));
        }
    ...
    if (!hasNonDispelEffect && !hasDispellableAura && dispelMask &&
        !IsTriggered()) {
        if (Unit* target = m_targets.GetUnitTarget()) {
            ...
            target->GetDispellableAuraList(
                m_caster, dispelMask, dispelList, m_spellInfo);
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        case SPELL_EFFECT_LEARN_SPELL: {
            ...
            SpellInfo const* learn_spellproto =
                sSpellMgr->GetSpellInfo(m_spellInfo->Effects[i].TriggerSpell);
        ...
        case SPELL_EFFECT_LEARN_PET_SPELL: {
            ...
            if (Unit* unitTarget = m_targets.GetUnitTarget()) {
                ...
                SpellInfo const* learn_spellproto = sSpellMgr->GetSpellInfo(
                    m_spellInfo->Effects[i].TriggerSpell);
        ...
        case SPELL_EFFECT_FEED_PET: {
            ...
            if (!pet->GetCurrentFoodBenefitLevel(
                    foodItem->GetTemplate()->ItemLevel))
        ...
        case SPELL_EFFECT_POWER_DRAIN: {
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                if (Unit* target = m_targets.GetUnitTarget())
                    if (target != m_caster &&
                        !target->HasActivePowerType(
                            Powers(m_spellInfo->Effects[i].MiscValue)))
                        return SPELL_FAILED_BAD_TARGETS;
            break;
        }
        case SPELL_EFFECT_CHARGE: {
            ...
            if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR) {
                // Warbringer - can't be handled in proc system - should be done
                // before checkcast root check and charge effect process
            ...
            if (DisableMgr::IsPathfindingEnabled(m_caster->FindMap()) &&
                ...
                // first we must check to see if the target is in LoS. A path
                // can usually be built but LoS matters for charge spells
                if (!target->IsWithinLOSInMap(
                        m_caster)) // Do full LoS/Path check. Don't exclude m2
                    return SPELL_FAILED_LINE_OF_SIGHT;

                float objSize = target->GetCombatReach();
                float range =
                    m_spellInfo->GetMaxRange(true, m_caster, this) * 1.5f +
                    objSize; // can't be overly strict
                ...
                bool result =
                    m_preGeneratedPath->CalculatePath(target->GetPositionX(),
                                                      target->GetPositionY(),
                                                      target->GetPositionZ(),
                                                      false);
                if (m_preGeneratedPath->GetPathType() & PATHFIND_SHORT)
                    return SPELL_FAILED_NOPATH;
                else if (!result || m_preGeneratedPath->GetPathType() &
                                        (PATHFIND_NOPATH | PATHFIND_INCOMPLETE))
                    return SPELL_FAILED_NOPATH;
                else if (m_preGeneratedPath->IsInvalidDestinationZ(
                             target)) // Check position z, if not in a straight
                                      // line
                    return SPELL_FAILED_NOPATH;

                m_preGeneratedPath->ShortenPathUntilDist(
                    G3D::Vector3(target->GetPositionX(),
                                 target->GetPositionY(),
                                 target->GetPositionZ()),
                    objSize); // move back
            }
        ...
        case SPELL_EFFECT_SKINNING: {
            ...
            if (!(m_targets.GetUnitTarget()->GetUnitFlags() &
                  UNIT_FLAG_SKINNABLE))
            ...
            uint32 skill =
                creature->GetCreatureTemplate()->GetRequiredLootSkill();
        ...
        case SPELL_EFFECT_OPEN_LOCK: {
            ...
            if (m_targets.GetTargetMask() & TARGET_FLAG_TRADE_ITEM) {
                if (TradeData* pTrade = m_caster->ToPlayer()->GetTradeData())
                    pTempItem = pTrade->GetTraderData()->GetItem(TradeSlots(
                        m_targets.GetItemTargetGUID().GetRawValue()));
            }
            else if (m_targets.GetTargetMask() & TARGET_FLAG_ITEM)
                pTempItem = m_caster->ToPlayer()->GetItemByGuid(
                    m_targets.GetItemTargetGUID());

            // we need a go target, or an openable item target in case of
            // TARGET_GAMEOBJECT_ITEM_TARGET
            if (m_spellInfo->Effects[i].TargetA.GetTarget() ==
                    TARGET_GAMEOBJECT_ITEM_TARGET &&
                !m_targets.GetGOTarget() &&
                (!pTempItem || !pTempItem->GetTemplate()->LockID ||
                 !pTempItem->IsLocked()))
                return SPELL_FAILED_BAD_TARGETS;

            // We must also ensure the gameobject we are opening is still closed
            // by the time the spell finishes.
            ...
            if (m_spellInfo->Id != 1842 ||
                ...
                if (m_targets.GetGOTarget() &&
                    m_targets.GetGOTarget()->GetEntry() == 179697) {
                    if (!m_caster->ToPlayer()->CanUseBattlegroundObject(
                            nullptr))
                        return SPELL_FAILED_TRY_AGAIN;
                }
                else if (m_caster->ToPlayer()
                             ->InBattleground() && // In Battleground players
                                                   // can use only flags and
                                                   // banners, or Gurubashi
                                                   // chest
            ...
            SpellCastResult res =
                CanOpenLock(i, lockId, skillId, reqSkillValue, skillValue);
            if (res != SPELL_CAST_OK)
                return res;
            ...
            if (skillId != SKILL_NONE && skillId != SKILL_HERBALISM &&
        ...
        case SPELL_EFFECT_SUMMON: {
            SummonPropertiesEntry const* SummonProperties =
                sSummonPropertiesStore.LookupEntry(
                    m_spellInfo->Effects[i].MiscValueB);
            if (!SummonProperties ||
                m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET_FIRST))
                break;
        ...
        case SPELL_EFFECT_SUMMON_PET: {
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER &&
                m_caster->IsClass(CLASS_WARLOCK, CLASS_CONTEXT_PET) && strict)
                if (Pet* pet = m_caster->ToPlayer()->GetPet())
                    pet->CastSpell(pet,
                                   32752,
                                   true,
                                   nullptr,
                                   nullptr,
                                   pet->GetGUID()); // starting cast, trigger
                                                    // pet stun (cast by pet so
                                                    // it doesn't attack player)

            Player* playerCaster = unitCaster->ToPlayer();
            if (playerCaster && playerCaster->GetPetStable()) {
                std::pair<PetStable::PetInfo const*, PetSaveMode> info =
                    Pet::GetLoadPetInfo(*playerCaster->GetPetStable(),
                                        m_spellInfo->Effects[i].MiscValue,
                                        0,
                                        false);
                if (info.first) {
                    if (info.first->Type == HUNTER_PET) {
                        ...
                        CreatureTemplate const* creatureInfo =
                            sObjectMgr->GetCreatureTemplate(
                                info.first->CreatureId);
                        if (!creatureInfo ||
                            !creatureInfo->IsTameable(
                                playerCaster->CanTameExoticPets())) {
                ...
                else if (!m_spellInfo->Effects[i]
                              .MiscValue) // when miscvalue is present it is
                                          // allowed to create new pets
                {
        ...
        case SPELL_EFFECT_SUMMON_PLAYER: {
            ...
            if (!target || m_caster->ToPlayer() == target ||
                (!target->IsInSameRaidWith(m_caster->ToPlayer()) &&
                 m_spellInfo->Id != 48955)) // refer-a-friend spell
                return SPELL_FAILED_BAD_TARGETS;

            // Xinef: Implement summon pending error
            if (target->GetSummonExpireTimer() >
            ...
            if (map->IsDungeon()) {
                ...
                /*if (map->IsRaid())
                    if (InstancePlayerBind* targetBind =
                   target->GetBoundInstance(mapId, difficulty)) if
                   (targetBind->perm && targetBind !=
                   m_caster->ToPlayer()->GetBoundInstance(mapId, difficulty))
                            return
        ...
        case SPELL_EFFECT_SUMMON_RAF_FRIEND: {
            ...
            if (!target || !(target->GetSession()->GetRecruiterId() ==
                                 playerCaster->GetSession()->GetAccountId() ||
                             target->GetSession()->GetAccountId() ==
                                 playerCaster->GetSession()->GetRecruiterId()))
                return SPELL_FAILED_BAD_TARGETS;
        ...
        case SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER: {
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                if (Battleground const* bg =
                        m_caster->ToPlayer()->GetBattleground())
        ...
        case SPELL_EFFECT_STEAL_BENEFICIAL_BUFF: {
            ...
            Unit::VisibleAuraMap const* visibleAuras =
                m_targets.GetUnitTarget()->GetVisibleAuras();
            for (Unit::VisibleAuraMap::const_iterator itr =
                     visibleAuras->begin();
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        case SPELL_AURA_AOE_CHARM: {
            ...
            // Xinef: allow SPELL_AURA_MOD_POSSESS to posses target if caster
            // has some pet
            ...
            else if (m_spellInfo->Effects[i].ApplyAuraName ==
            ...
            if (Unit* target = m_targets.GetUnitTarget()) {
                ...
                if (target->IsPet() &&
                    (!target->GetOwner() || target->GetOwner()->ToPlayer()))
                    return SPELL_FAILED_CANT_BE_CHARMED;
        ...
        case SPELL_AURA_MOUNTED: {
            ...
            InstanceTemplate const* it =
                sObjectMgr->GetInstanceTemplate(m_caster->GetMapId());
            if (it)
            ...
            if (m_caster->getTransForm())
                if (SpellInfo const* transformSpellInfo =
                        sSpellMgr->GetSpellInfo(m_caster->getTransForm()))
                    if (transformSpellInfo->HasAttribute(
                            SPELL_ATTR0_NO_IMMUNITIES) &&
                        !transformSpellInfo->HasAttribute(
                            SpellAttr0(SPELL_ATTR0_ALLOW_WHILE_MOUNTED |
        ...
        case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED: {
            ...
            // not allow cast fly spells if not have req. skills  (all spells is
            // self target) allow always ghost flight spells
            if (m_originalCaster &&
                m_originalCaster->GetTypeId() == TYPEID_PLAYER &&
                m_originalCaster->IsAlive()) {
                Battlefield* Bf = sBattlefieldMgr->GetBattlefieldToZoneId(
                    m_originalCaster->GetZoneId());
                if (AreaTableEntry const* pArea = sAreaTableStore.LookupEntry(
        ...
        case SPELL_AURA_HOVER: {
            if ((m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_MOUNT) !=
                    0 &&
                m_targets.GetUnitTarget() &&
                m_targets.GetUnitTarget()->IsMounted()) {
        ...
        case SPELL_AURA_MOD_SHAPESHIFT: {
            if (m_caster &&
                m_caster->HasAura(
                    23397)) // Nefarian Class Call (Warrior): Berserk --
                            // Nefertum: I don't really like this but I didn't
                            // find another way.
            {
                return SPELL_FAILED_NOT_SHAPESHIFT;
            }
    ...
    if (m_targets.GetTargetMask() & TARGET_FLAG_TRADE_ITEM) {
        ...
        TradeSlots slot =
            TradeSlots(m_targets.GetItemTargetGUID().GetRawValue());
    ...
    // check if caster has at least 1 combo point on target for spells that
    // require combo points
    if (m_needComboPoints) {
    ...
    if (m_CastItem &&
        m_CastItem->GetTemplate()->InventoryType == INVTYPE_RELIC &&
        m_triggeredByAuraSpell)
        if (m_caster->HasSpellCooldown(SPELL_RELIC_COOLDOWN) &&
            !m_caster->HasSpellItemCooldown(SPELL_RELIC_COOLDOWN,
                                            m_CastItem->GetEntry()))
            return SPELL_FAILED_NOT_READY;
...
SpellCastResult Spell::CheckPetCast(Unit* target)
{
    if (m_caster->HasUnitState(UNIT_STATE_CASTING) &&
        !HasTriggeredCastFlag(
            TRIGGERED_IGNORE_CAST_IN_PROGRESS)) // prevent spellcast
    ...
    // xinef: Calculate power cost here, so funciton checking power can work
    // properly and dont return bad results
    ...
    if (m_spellInfo->StartRecoveryCategory > 0)
        if (m_caster->GetCharmInfo() &&
            m_caster->GetCharmInfo()->GetGlobalCooldownMgr().HasGlobalCooldown(
                m_spellInfo))
            return SPELL_FAILED_NOT_READY;
...
SpellCastResult Spell::CheckCasterAuras(bool preventionOnly) const
{
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR1_IMMUNITY_PURGES_EFFECT)) {
        for (int i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            if (m_spellInfo->Effects[i].ApplyAuraName ==
                SPELL_AURA_SCHOOL_IMMUNITY)
                school_immune |= uint32(m_spellInfo->Effects[i].MiscValue);
            else if (m_spellInfo->Effects[i].ApplyAuraName ==
                     SPELL_AURA_MECHANIC_IMMUNITY)
                mechanic_immune |= 1
                                   << uint32(m_spellInfo->Effects[i].MiscValue);
            else if (m_spellInfo->Effects[i].ApplyAuraName ==
                     SPELL_AURA_DISPEL_IMMUNITY)
                dispel_immune |= SpellInfo::GetDispelMask(
                    DispelType(m_spellInfo->Effects[i].MiscValue));
        }
        // immune movement impairment and loss of control
        //         PVP trinket                   EMFH                   TOC PVP
        //         trinket               Bullheaded                  Bestial
        //         Wrath             // Beath Within         // Medalion of
        //         Immunity
        if (m_spellInfo->Id == 42292 || m_spellInfo->Id == 59752 ||
            m_spellInfo->Id == 65547 || m_spellInfo->Id == 53490 ||
            m_spellInfo->Id == 19574 || m_spellInfo->Id == 34471 ||
            m_spellInfo->Id == 46227)
            mechanic_immune =
                IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK;
    }

    bool usableInStun =
        m_spellInfo->HasAttribute(SPELL_ATTR5_ALLOW_WHILE_STUNNED);
    ...
    if (!preventionOnly) {
        if (unitflag & UNIT_FLAG_STUNNED) {
            // spell is usable while stunned, check if caster has only mechanic
            // stun auras, another stun types must prevent cast spell
            if (usableInStun) {
                bool   foundNotStun = false;
                uint32 mask = (1 << MECHANIC_STUN) | (1 << MECHANIC_FREEZE) |
                              (1 << MECHANIC_HORROR);
                // Barkskin should skip sleep effects, sap and fears
                if (m_spellInfo->Id == 22812)
                    mask |= 1 << MECHANIC_SAPPED | 1 << MECHANIC_HORROR |
                            1 << MECHANIC_SLEEP;
                ...
                for (Unit::AuraEffectList::const_iterator i = stunAuras.begin();
                    ...
                    if ((*i)->GetSpellInfo()->GetAllEffectsMechanicMask() &&
                        !((*i)->GetSpellInfo()->GetAllEffectsMechanicMask() &
                          mask)) {
                        foundNotStun = true;
                        break;
                    }
                ...
                prevented_reason = SPELL_FAILED_STUNNED;
                 ...
                 !m_spellInfo->HasAttribute(SPELL_ATTR5_ALLOW_WHILE_CONFUSED))
            prevented_reason = SPELL_FAILED_CONFUSED;
        else if (unitflag & UNIT_FLAG_FLEEING &&
                 !m_spellInfo->HasAttribute(SPELL_ATTR5_ALLOW_WHILE_FLEEING))
    ...
    if (prevented_reason != SPELL_CAST_OK) {
        if (school_immune || mechanic_immune || dispel_immune) {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = auras.begin();
                ...
                // SPELL_FAILED message is returned. That is needed when your
                // casting is prevented by multiple states and you are only
                // immune to some of them.
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
                    if (AuraEffect* part = aura->GetEffect(i)) {
                        ...
                        case SPELL_AURA_MOD_STUN: {
                            ...
                            if (!usableInStun ||
                                !(auraInfo->GetAllEffectsMechanicMask() & mask))
...
bool Spell::CanAutoCast(Unit* target)
{
        ...
        if (m_spellInfo->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA) {
            if (m_spellInfo->StackAmount <= 1) {
                ...
                if (AuraEffect* aureff =
                        target->GetAuraEffect(m_spellInfo->Id, j))
                    if (aureff->GetBase()->GetStackAmount() >=
    ...
    if (result == SPELL_CAST_OK || result == SPELL_FAILED_UNIT_NOT_INFRONT) {
        SelectSpellTargets();
        // check if among target units, our WANTED target is as well (->only
        // self cast spells return false)
        for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
             ihit != m_UniqueTargetInfo.end();
             ++ihit)
            if (ihit->targetGUID == targetguid)
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (m_spellInfo->RangeEntry) {
        // check needed by 68766 51693 - both spells are cast on enemies and
        // have 0 max range these are triggered by other spells - possibly we
        // should omit range check in that case?
    ...
    if (min_range && GetCaster()->GetTypeId() == TYPEID_UNIT &&
        !GetCaster()->GetOwnerGUID().IsPlayer() && min_range <= 6.0f)
    ...
    if (range_type != SPELL_RANGE_MELEE && !strict)
        max_range +=
            std::min(3.0f, max_range * 0.1f); // 10% but no more than 3yd
    ...
    if (target) {
        if (target != m_caster) {
            ...
            if (range_type == SPELL_RANGE_MELEE) {
                ...
                if (m_caster->GetTypeId() != TYPEID_UNIT &&
                    m_caster->isMoving() && target->isMoving() &&
                    !m_caster->IsWalking() && !target->IsWalking())
                    real_max_range -=
                        MIN_MELEE_REACH; // Because of lag, we can not check too
                                         // strictly here (is only used if both
                                         // caster and target are moving)
                else
                    real_max_range -= 2 * MIN_MELEE_REACH;

                if (!m_caster->IsWithinMeleeRange(
                        target, std::max(real_max_range, 0.0f)))
        ...
        if (min_range && range_type != SPELL_RANGE_RANGED &&
            m_caster->IsWithinCombatRange(
                target, min_range)) // skip this check if min_range = 0
            return SPELL_FAILED_TOO_CLOSE;
    }

    if (GameObject* goTarget = m_targets.GetGOTarget()) {
        if (!goTarget->IsAtInteractDistance(m_caster->ToPlayer(),
                                            m_spellInfo)) {
            return SPELL_FAILED_OUT_OF_RANGE;
        }
    }
    ...
    if (m_targets.HasDst() && !m_targets.HasTraj()) {
        if (!m_caster->IsWithinDist3d(m_targets.GetDstPos(), max_range))
            return SPELL_FAILED_OUT_OF_RANGE;
        if (min_range &&
            m_caster->IsWithinDist3d(m_targets.GetDstPos(), min_range))
            return SPELL_FAILED_TOO_CLOSE;
    }
...
SpellCastResult Spell::CheckPower()
{
    ...
    // While .cheat power is enabled dont check if we need power to cast the
    // spell
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
        if (m_caster->ToPlayer()->GetCommandStatus(CHEAT_POWER)) {
            return SPELL_CAST_OK;
    ...
    if (m_spellInfo->PowerType >= MAX_POWERS) {
        LOG_ERROR("spells",
                  "Spell::CheckPower: Unknown power type '{}'",
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (!player) {
        ...
        if (!m_caster->CanUseAttackType(m_attackType) &&
            (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE ||
             m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED)) {
    ...
    if (!m_CastItem) {
        ...
        if (proto->Class == ITEM_CLASS_CONSUMABLE &&
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++) {
                // skip check, pet not required like checks, and for
                // TARGET_UNIT_PET m_targets.GetUnitTarget() is not the real
                // target but the caster
                if (m_spellInfo->Effects[i].TargetA.GetTarget() ==
                    TARGET_UNIT_PET)
                    continue;
                ...
                if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_ENERGIZE) {
                    if (m_spellInfo->Effects[i].MiscValue < 0 ||
                        m_spellInfo->Effects[i].MiscValue >= int8(MAX_POWERS)) {
    ...
    // if not item target then required item must be equipped
        ...
        // Xinef: this is not true in my opinion, in eg bladestorm will not be
        // canceled after disarm
        // if
        // (!HasTriggeredCastFlag(TRIGGERED_IGNORE_EQUIPPED_ITEM_REQUIREMENT))
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
    ...
    // do not take reagents for these item casts
    if (!(m_CastItem &&
          m_CastItem->GetTemplate()->Flags & ITEM_FLAG_NO_REAGENT_COST)) {
        bool checkReagents =
            !HasTriggeredCastFlag(TRIGGERED_IGNORE_POWER_AND_REAGENT_COST) &&
            !player->CanNoReagentCast(m_spellInfo);
        // Not own traded item (in trader trade slot) requires reagents even if
        // triggered spell
        if (!checkReagents)
        ...
        for (int i = 0; i < 2; ++i) {
            if (m_spellInfo->TotemCategory[i] != 0) {
                if (player->HasItemTotemCategory(
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++) {
        ...
        case SPELL_EFFECT_CREATE_ITEM_2: {
            // m_targets.GetUnitTarget() means explicit cast, otherwise we dont
            // check for possible equip error
            Unit* target =
                m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : player;
            if (target->GetTypeId() == TYPEID_PLAYER && !IsTriggered()) {
                // SPELL_EFFECT_CREATE_ITEM_2 differs from
                // SPELL_EFFECT_CREATE_ITEM in that it picks the random item to
                // create from a pool of potential items, so we need to make
                // sure there is at least one free space in the player's
                // inventory
                if (m_spellInfo->Effects[i].Effect ==
                    ...
                    if (target->ToPlayer()->GetFreeInventorySpace() == 0) {
                        player->SendEquipError(
                            ...
                            nullptr,
                            nullptr,
                            m_spellInfo->Effects[i].ItemType);
                        return SPELL_FAILED_DONT_REPORT;
                    }
                ...
                if (m_spellInfo->Effects[i].ItemType) {
                    ...
                    uint32 createCount =
                        std::clamp<uint32>(m_spellInfo->Effects[i].CalcValue(),
                                           1u,
                                           itemTemplate->GetMaxStackSize());
                    ItemPosCountVec dest;
                    InventoryResult msg = target->ToPlayer()->CanStoreNewItem(
                        NULL_BAG,
                        NULL_SLOT,
                        dest,
                        m_spellInfo->Effects[i].ItemType,
                    ...
                    if (msg != EQUIP_ERR_OK) {
                        ...
                        if (!itemTemplate->ItemLimitCategory) {
                            ...
                            if (m_spellInfo->SpellFamilyName !=
                            ...
                            else if (!(target->ToPlayer()->HasItemCount(
                                         m_spellInfo->Effects[i].ItemType))) {
                                player->SendEquipError(
                                    msg,
                                    nullptr,
                                ...
                                player->CastSpell(
                                    player,
                                    m_spellInfo->Effects[EFFECT_1].CalcValue(),
                                    false); // move this to anywhere
        ...
        case SPELL_EFFECT_ENCHANT_ITEM:
            if (m_spellInfo->Effects[i].ItemType && m_targets.GetItemTarget() &&
                ...
                // do not allow to enchant vellum from scroll made by
                // vellum-prevent exploit
                if (m_CastItem && m_CastItem->GetTemplate()->Flags &
                                      ITEM_FLAG_NO_REAGENT_COST)
                    return SPELL_FAILED_TOTEM_CATEGORY;
                ItemPosCountVec dest;
                InventoryResult msg =
                    player->CanStoreNewItem(NULL_BAG,
                                            NULL_SLOT,
                                            dest,
                                            m_spellInfo->Effects[i].ItemType,
                                            1);
        ...
        case SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC: {
            ...
            if (targetItem->GetTemplate()->ItemLevel < m_spellInfo->BaseLevel ||
                (targetItem->GetTemplate()->RequiredLevel &&
                 targetItem->GetTemplate()->RequiredLevel <
                     m_spellInfo->BaseLevel))
                return SPELL_FAILED_LOWLEVEL;
            ...
            SpellItemEnchantmentEntry const* enchantEntry =
                sSpellItemEnchantmentStore.LookupEntry(
                    m_spellInfo->Effects[i].MiscValue);
            // do not allow adding usable enchantments to items that have use
            // effect already
            if (enchantEntry) {
                for (uint8 s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; ++s) {
                    ...
                    case ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET: {
                        uint32 numSockets = 0;
                        for (uint32 socket = 0; socket < MAX_ITEM_PROTO_SOCKETS;
                             ++socket)
                            if (targetItem->GetTemplate()->Socket[socket].Color)
                                ++numSockets;

                        if (numSockets == MAX_ITEM_PROTO_SOCKETS ||
                            targetItem->GetEnchantmentId(
        ...
        case SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY: {
            ...
            // max level restrition set
            if (m_CastItem && m_spellInfo->MaxLevel > 0) {
                if (item->GetTemplate()->ItemLevel <
                    m_CastItem->GetTemplate()->RequiredLevel)
                    return SPELL_FAILED_LOWLEVEL;
        ...
        case SPELL_EFFECT_DISENCHANT: {
            ...
            ItemTemplate const* itemProto =
                m_targets.GetItemTarget()->GetTemplate();
            if (!itemProto)
                return SPELL_FAILED_CANT_BE_DISENCHANTED;
            ...
            uint32 item_disenchantskilllevel =
                itemProto->RequiredDisenchantSkill;
            if (item_disenchantskilllevel == uint32(-1))
                return SPELL_FAILED_CANT_BE_DISENCHANTED;
            if (item_disenchantskilllevel >
                player->GetSkillValue(SKILL_ENCHANTING))
                return SPELL_FAILED_LOW_CASTLEVEL;
        ...
        case SPELL_EFFECT_PROSPECTING: {
            ...
            if (!(m_targets.GetItemTarget()->GetTemplate()->Flags &
        ...
        case SPELL_EFFECT_MILLING: {
            ...
            if (item_millingskilllevel >
                player->GetSkillValue(SKILL_INSCRIPTION))
                return SPELL_FAILED_LOW_CASTLEVEL;
        ...
        case SPELL_EFFECT_WEAPON_DAMAGE_NOSCHOOL: {
            ...
            Item* pItem =
                m_caster->ToPlayer()->GetWeaponForAttack(m_attackType);
            if (!pItem || pItem->IsBroken())
            ...
            case ITEM_SUBCLASS_WEAPON_CROSSBOW: {
                ...
                ItemTemplate const* ammoProto =
                    sObjectMgr->GetItemTemplate(ammo);
                if (!ammoProto)
                    return SPELL_FAILED_NO_AMMO;
    ...
    if (/*never skip those checks
        ...
        m_weaponItem =
            m_caster->ToPlayer()->GetWeaponForAttack(m_attackType, true);
    }
...
SpellCastResult Spell::CheckSpellFocus()
{
    ...
    if (m_spellInfo->RequiresSpellFocus) {
        CellCoord p(Acore::ComputeCellCoord(m_caster->GetPositionX(),
                                            m_caster->GetPositionY()));
        Cell      cell(p);

        GameObject*                 ok = nullptr;
        Acore::GameObjectFocusCheck go_check(m_caster,
                                             m_spellInfo->RequiresSpellFocus);
        Acore::GameObjectSearcher<Acore::GameObjectFocusCheck> checker(
            m_caster, ok, go_check);

        TypeContainerVisitor<
            Acore::GameObjectSearcher<Acore::GameObjectFocusCheck>,
            GridTypeMapContainer>
             object_checker(checker);
        Map& map = *m_caster->GetMap();
        cell.Visit(
            p, object_checker, map, *m_caster, m_caster->GetVisibilityRange());
...
void Spell::Delayed() // only called in DealDamage()
{
    ...
    //     return;                                             // spell is
    //     active and can't be time-backed
    ...
    m_caster->ToPlayer()->ApplySpellMod(
        m_spellInfo->Id, SPELLMOD_NOT_LOSE_CASTING_TIME, delayReduce, this);
    delayReduce +=
        m_caster->GetTotalAuraModifier(SPELL_AURA_REDUCE_PUSHBACK) - 100;
    if (delayReduce >= 100)
        return;
    ...
    LOG_DEBUG("spells",
              "Spell {} partially interrupted for ({}) ms at damage",
              m_spellInfo->Id,
              delaytime);
...
void Spell::DelayedChannel()
{
    ...
    int32 duration = ((m_channeledDuration > 0) ? m_channeledDuration
                                                : m_spellInfo->GetDuration());

    int32 delaytime = CalculatePct(
        duration, 25); // channeling delay is normally 25% of its time per hit
    int32 delayReduce = 100; // must be initialized to 100 for percent modifiers
    m_caster->ToPlayer()->ApplySpellMod(
        m_spellInfo->Id, SPELLMOD_NOT_LOSE_CASTING_TIME, delayReduce, this);
    delayReduce +=
    ...
    LOG_DEBUG("spells.aura",
              "Spell {} partially interrupted for {} ms, new duration: {} ms",
              m_spellInfo->Id,
              delaytime,
              m_timer);
...
bool Spell::UpdatePointers()
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
        ...
        if (!transport || transport->GetGUID() != dest._transportGUID)
            transport =
                ObjectAccessor::GetWorldObject(*m_caster, dest._transportGUID);
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
    ...
    // xinef: skip los checking if spell has appropriate attribute, or target
    // requires specific entry this is only for target addition and target has
    // to have unselectable flag, this is valid for FLAG_EXTRA_TRIGGER and quest
    // triggers however there are some without this flag, used not_selectable
    if (m_spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) ||
        (target->GetTypeId() == TYPEID_UNIT &&
         target->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE) &&
         (m_spellInfo->Effects[eff].TargetA.GetCheckType() ==
              TARGET_CHECK_ENTRY ||
          m_spellInfo->Effects[eff].TargetB.GetCheckType() ==
              TARGET_CHECK_ENTRY)))
        return true;

    // if spell is triggered, need to check for LOS disable on the aura
    // triggering it and inherit that behaviour
    if (IsTriggered() && m_triggeredByAuraSpell &&
        (m_triggeredByAuraSpell.spellInfo->HasAttribute(
             SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) ||
         DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL,
                                   m_triggeredByAuraSpell.spellInfo->Id,
                                   nullptr,
                                   SPELL_DISABLE_LOS))) {
        return true;
    }
    ...
    // Check targets for LOS visibility (except spells without range
    // limitations)
    ...
    case SPELL_EFFECT_RESURRECT_NEW:
        ...
        if (target != m_caster && !target->IsWithinLOSInMap(m_caster)) {
            ...
            Corpse* corpse = ObjectAccessor::GetCorpse(
                *m_caster, m_targets.GetCorpseTargetGUID());
            if (!corpse)
                return false;
    ...
    case SPELL_EFFECT_SKIN_PLAYER_CORPSE: {
        if (!m_targets.GetCorpseTargetGUID()) {
            if (target->IsWithinLOSInMap(m_caster,
                                         VMAP::ModelIgnoreFlags::M2) &&
                target->HasUnitFlag(UNIT_FLAG_SKINNABLE))
                return true;
    ...
    case SPELL_EFFECT_SUMMON_RAF_FRIEND:
        if (m_caster->GetTypeId() != TYPEID_PLAYER ||
            ...
            return false;
        if (m_caster->ToPlayer()->GetSession()->IsARecruiter() &&
            target->ToPlayer()->GetSession()->GetRecruiterId() !=
                m_caster->ToPlayer()->GetSession()->GetAccountId())
            return false;
        if (m_caster->ToPlayer()->GetSession()->GetRecruiterId() !=
                target->ToPlayer()->GetSession()->GetAccountId() &&
            target->ToPlayer()->GetSession()->IsARecruiter())
            return false;
        if (target->ToPlayer()->GetLevel() >=
            sWorld->getIntConfig(
    ...
    default: // normal case
    {
        ...
        if (target != m_caster) {
            ...
            else if (!m_caster->IsWithinLOSInMap(
                         target,
                         VMAP::ModelIgnoreFlags::M2,
                         LineOfSightChecks(losChecks))) {
                return false;
...
bool Spell::IsAutoActionResetSpell() const
{
    ...
    /// SPELL_INTERRUPT_FLAG_INTERRUPT to fix compile - is this check correct at
    /// all?
    if (IsTriggered() ||
        !(m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT)) {
        return false;
    }

    if (!m_casttime && m_spellInfo->HasAttribute(
...
bool Spell::IsNeedSendToClient(bool go) const
{
    return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] ||
           m_spellInfo->IsChanneled() || m_spellInfo->Speed > 0.0f ||
           (!m_triggeredByAuraSpell && !IsTriggered()) ||
           (go && m_triggeredByAuraSpell &&
            m_triggeredByAuraSpell.spellInfo->IsChanneled());
}
...
bool Spell::HaveTargetsForEffect(uint8 effect) const
{
    ...
    for (std::list<GOTargetInfo>::const_iterator itr =
             m_UniqueGOTargetInfo.begin();
         itr != m_UniqueGOTargetInfo.end();
         ++itr)
        if (itr->effectMask & (1 << effect))
            return true;

    for (std::list<ItemTargetInfo>::const_iterator itr =
             m_UniqueItemInfo.begin();
         itr != m_UniqueItemInfo.end();
         ++itr)
        if (itr->effectMask & (1 << effect))
            return true;
...
SpellEvent::~SpellEvent()
{
    ...
    if (m_Spell->IsDeletable()) {
        ...
        LOG_ERROR("spells",
                  "~SpellEvent: {} {} tried to delete non-deletable spell {}. "
                  "Was not deleted, causes memory leak.",
                  (m_Spell->GetCaster()->GetTypeId() == TYPEID_PLAYER
                       ? "Player"
                       : "Creature"),
                  m_Spell->GetCaster()->GetGUID().ToString(),
                  m_Spell->m_spellInfo->Id);
        ABORT();
    }
}
...
bool ReflectEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (target && _caster->IsInMap(target))
        Unit::ProcDamageAndSpell(_caster,
                                 target,
                                 PROC_FLAG_NONE,
...
void Spell::HandleLaunchPhase()
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (m_applyMultiplierMask & (1 << i))
            multiplier[i] = m_spellInfo->Effects[i].CalcDamageMultiplier(
                m_originalCaster, this);

    bool usesAmmo = m_spellInfo->HasAttribute(SPELL_ATTR0_CU_DIRECT_DAMAGE);
    Unit::AuraEffectList const& Auras =
        m_caster->GetAuraEffectsByType(SPELL_AURA_ABILITY_CONSUME_NO_AMMO);
    for (Unit::AuraEffectList::const_iterator j = Auras.begin();
         j != Auras.end();
         ++j) {
        if ((*j)->IsAffectedOnSpell(m_spellInfo))
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo& targetInfo, float* multiplier)
{
    ...
    if (targetInfo.missCondition == SPELL_MISS_NONE)
        unit = m_caster->GetGUID() == targetInfo.targetGUID
                   ? m_caster
                   : ObjectAccessor::GetUnit(*m_caster, targetInfo.targetGUID);
    // In case spell reflect from target, do all effect on caster (if hit)
    else if (targetInfo.missCondition == SPELL_MISS_REFLECT &&
             targetInfo.reflectResult == SPELL_MISS_NONE)
        unit = m_caster;
    if (!unit)
        return;
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        if (targetInfo.effectMask & (1 << i)) {
            ...
            HandleEffects(
                unit, nullptr, nullptr, i, SPELL_EFFECT_HANDLE_LAUNCH_TARGET);

            if (m_damage > 0) {
                // Xinef: Area Auras, AoE Targetting spells AND Chain Target
                // spells (cleave etc.)
                if (m_spellInfo->Effects[i].IsAreaAuraEffect() ||
                    m_spellInfo->Effects[i].IsTargetingArea() ||
                    (m_spellInfo->Effects[i].ChainTarget > 1 &&
                     m_spellInfo->DmgClass != SPELL_DAMAGE_CLASS_MAGIC)) {
                    m_damage = unit->CalculateAOEDamageReduction(
                        m_damage, m_spellInfo->SchoolMask, m_caster);
                    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    float critChance = caster->SpellDoneCritChance(
        unit, m_spellInfo, m_spellSchoolMask, m_attackType, false);
    critChance      = unit->SpellTakenCritChance(caster,
                                            m_spellInfo,
                                            m_spellSchoolMask,
                                            critChance,
                                            m_attackType,
                                   ...
                                   int32&     skillValue)
{
    ...
    for (int j = 0; j < MAX_LOCK_CASE; ++j) {
        ...
        case LOCK_KEY_SKILL: {
            ...
            // wrong locktype, skip
            if (uint32(m_spellInfo->Effects[effIndex].MiscValue) !=
                lockInfo->Index[j])
                continue;
            ...
            if (skillId != SKILL_NONE) {
                ...
                // add the effect base points modifier from the spell casted
                // (cheat lock / skeleton key etc.)
                if ((m_spellInfo->Effects[effIndex].TargetA.GetTarget() ==
                         TARGET_GAMEOBJECT_ITEM_TARGET ||
                     m_spellInfo->Effects[effIndex].TargetB.GetTarget() ==
                         TARGET_GAMEOBJECT_ITEM_TARGET) &&
...
void Spell::SetSpellValue(SpellValueMod mod, int32 value)
{
    ...
    case SPELLVALUE_BASE_POINT0:
        m_spellValue->EffectBasePoints[0] =
            m_spellInfo->Effects[EFFECT_0].CalcBaseValue(value);
        break;
    case SPELLVALUE_BASE_POINT1:
        m_spellValue->EffectBasePoints[1] =
            m_spellInfo->Effects[EFFECT_1].CalcBaseValue(value);
        break;
    case SPELLVALUE_BASE_POINT2:
        m_spellValue->EffectBasePoints[2] =
            m_spellInfo->Effects[EFFECT_2].CalcBaseValue(value);
        break;
...
void Spell::PrepareTargetProcessing() { CheckEffectExecuteData(); }
...
void Spell::LoadScripts()
{
    ...
    for (std::list<SpellScript*>::iterator itr = m_loadedScripts.begin();
        ...
        LOG_DEBUG(
            "spells.aura",
            "Spell::LoadScripts: Script `{}` for spell `{}` is loaded now",
            (*itr)->_GetScriptName()->c_str(),
            m_spellInfo->Id);
        (*itr)->Register();
        ++itr;
    }
...
void Spell::CallScriptBeforeCastHandlers()
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
         scritr != m_loadedScripts.end();
         ++scritr) {
        (*scritr)->_PrepareScriptCall(SPELL_SCRIPT_HOOK_BEFORE_CAST);
        std::list<SpellScript::CastHandler>::iterator
            hookItrEnd = (*scritr)->BeforeCast.end(),
            hookItr    = (*scritr)->BeforeCast.begin();
...
void Spell::CallScriptOnCastHandlers()
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::CastHandler>::iterator
            hookItrEnd = (*scritr)->OnCast.end(),
            hookItr    = (*scritr)->OnCast.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            (*hookItr).Call(*scritr);
...
void Spell::CallScriptAfterCastHandlers()
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::CastHandler>::iterator
            hookItrEnd = (*scritr)->AfterCast.end(),
            hookItr    = (*scritr)->AfterCast.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            (*hookItr).Call(*scritr);
...
SpellCastResult Spell::CallScriptCheckCastHandlers()
{
    ...
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::CheckCastHandler>::iterator
            hookItrEnd = (*scritr)->OnCheckCast.end(),
            hookItr    = (*scritr)->OnCheckCast.begin();
        for (; hookItr != hookItrEnd; ++hookItr) {
...
void Spell::CallScriptBeforeHitHandlers(SpellMissInfo missInfo)
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::BeforeHitHandler>::iterator
            hookItrEnd = (*scritr)->BeforeHit.end(),
            hookItr    = (*scritr)->BeforeHit.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
...
void Spell::CallScriptOnHitHandlers()
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::HitHandler>::iterator
            hookItrEnd = (*scritr)->OnHit.end(),
            hookItr    = (*scritr)->OnHit.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            (*hookItr).Call(*scritr);
...
void Spell::CallScriptAfterHitHandlers()
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::HitHandler>::iterator
            hookItrEnd = (*scritr)->AfterHit.end(),
            hookItr    = (*scritr)->AfterHit.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            (*hookItr).Call(*scritr);
...
void Spell::CallScriptObjectAreaTargetSelectHandlers(
    std::list<WorldObject*>&       targets,
    SpellEffIndex                  effIndex,
    SpellImplicitTargetInfo const& targetType)
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::ObjectAreaTargetSelectHandler>::iterator
            hookItrEnd = (*scritr)->OnObjectAreaTargetSelect.end(),
            hookItr    = (*scritr)->OnObjectAreaTargetSelect.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            if (hookItr->IsEffectAffected(m_spellInfo, effIndex) &&
                targetType.GetTarget() == hookItr->GetTarget())
...
void Spell::CallScriptObjectTargetSelectHandlers(
    WorldObject*&                  target,
    SpellEffIndex                  effIndex,
    SpellImplicitTargetInfo const& targetType)
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::ObjectTargetSelectHandler>::iterator
            hookItrEnd = (*scritr)->OnObjectTargetSelect.end(),
            hookItr    = (*scritr)->OnObjectTargetSelect.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            if (hookItr->IsEffectAffected(m_spellInfo, effIndex) &&
                targetType.GetTarget() == hookItr->GetTarget())
    ...
    SpellImplicitTargetInfo const& targetType)
{
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin();
        ...
        std::list<SpellScript::DestinationTargetSelectHandler>::iterator
            hookItrEnd = (*scritr)->OnDestinationTargetSelect.end(),
            hookItr    = (*scritr)->OnDestinationTargetSelect.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            if (hookItr->IsEffectAffected(m_spellInfo, effIndex) &&
                targetType.GetTarget() == hookItr->GetTarget())
                hookItr->Call(*scritr, target);
                                             ...
                                             uint32 effIndexToCheck)
{
    ...
    for (std::list<SpellScript*>::iterator itr = m_loadedScripts.begin();
         ...
         ++itr) {
        std::list<SpellScript::ObjectTargetSelectHandler>::iterator
            targetSelectHookEnd = (*itr)->OnObjectTargetSelect.end(),
            targetSelectHookItr = (*itr)->OnObjectTargetSelect.begin();
        for (; targetSelectHookItr != targetSelectHookEnd;
             ++targetSelectHookItr)
            if (((*targetSelectHookItr)
                     .IsEffectAffected(m_spellInfo, effIndex) &&
                 !(*targetSelectHookItr)
        ...
        std::list<SpellScript::ObjectAreaTargetSelectHandler>::iterator
            ...
            areaTargetSelectHookItr = (*itr)->OnObjectAreaTargetSelect.begin();
        for (; areaTargetSelectHookItr != areaTargetSelectHookEnd;
             ++areaTargetSelectHookItr)
            if (((*areaTargetSelectHookItr)
                     .IsEffectAffected(m_spellInfo, effIndex) &&
                 !(*areaTargetSelectHookItr)
                      .IsEffectAffected(m_spellInfo, effIndexToCheck)) ||
                (!(*areaTargetSelectHookItr)
...
bool Spell::CanExecuteTriggersOnHit(uint8            effMask,
                                    SpellInfo const* triggeredByAura) const
{
    ...
    bool only_on_caster =
        (triggeredByAura && triggeredByAura->HasAttribute(
                                SPELL_ATTR4_CLASS_TRIGGER_ONLY_ON_TARGET));
    // If triggeredByAura has SPELL_ATTR4_CLASS_TRIGGER_ONLY_ON_TARGET then it
    // can only proc on a casted spell with TARGET_UNIT_CASTER
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        if ((effMask & (1 << i)) &&
            (!only_on_caster || (m_spellInfo->Effects[i].TargetA.GetTarget() ==
                                 TARGET_UNIT_CASTER)))
            return true;
    }
    return effMask;
...
void Spell::PrepareTriggersExecutedOnHit()
{
    ...
    if (m_spellInfo->SpellFamilyName) {
        SpellInfo const* excludeCasterSpellInfo =
            sSpellMgr->GetSpellInfo(m_spellInfo->ExcludeCasterAuraSpell);
        if (excludeCasterSpellInfo && !excludeCasterSpellInfo->IsPositive())
            m_preCastSpell = m_spellInfo->ExcludeCasterAuraSpell;
        SpellInfo const* excludeTargetSpellInfo =
            sSpellMgr->GetSpellInfo(m_spellInfo->ExcludeTargetAuraSpell);
    ...
    case SPELLFAMILY_PALADIN: {
        if (m_spellInfo->SpellFamilyFlags[1] & 0x40000000) {
            Unit::AuraEffectList const& mVindication =
                m_caster->GetAuraEffectsByType(SPELL_AURA_PROC_TRIGGER_SPELL);
            for (Unit::AuraEffectList::const_iterator itr =
                     mVindication.begin();
                 itr != mVindication.end();
                 ++itr) {
                if ((*itr)->GetSpellInfo()->Effects[EFFECT_0].TriggerSpell ==
                    26017) {
    ...
    case SPELLFAMILY_DRUID: {
        ...
        if (m_spellInfo->Id == 16857 &&
            (m_caster->GetShapeshiftForm() == FORM_BEAR ||
             m_caster->GetShapeshiftForm() == FORM_DIREBEAR))
    ...
    // handle SPELL_AURA_ADD_TARGET_TRIGGER auras:
    // save auras which were present on spell caster on cast, to prevent
    // triggered auras from affecting caster and to correctly calculate proc
    // chance when combopoints are present
    Unit::AuraEffectList const& targetTriggers =
        m_caster->GetAuraEffectsByType(SPELL_AURA_ADD_TARGET_TRIGGER);
    for (Unit::AuraEffectList::const_iterator i = targetTriggers.begin();
         i != targetTriggers.end();
         ++i) {
        if (!(*i)->IsAffectedOnSpell(m_spellInfo))
            continue;
        ...
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(
                auraSpellInfo->Effects[auraSpellIdx].TriggerSpell)) {
            // calculate the chance using spell base amount, because aura amount
            // is not updated on combo-points change this possibly needs fixing
            int32 auraBaseAmount = (*i)->GetBaseAmount();
            // proc chance is stored in effect amount
            int32 chance = m_caster->CalculateSpellDamage(
                nullptr, auraSpellInfo, auraSpellIdx, &auraBaseAmount);
...
bool Spell::HasGlobalCooldown() const
{
    ...
    if (m_caster->GetCharmInfo())
        ...
        return m_caster->ToPlayer()->GetGlobalCooldownMgr().HasGlobalCooldown(
            m_spellInfo);
    else
        return false;
}
...
void Spell::TriggerGlobalCooldown()
{
    ...
    if (!gcd) {
        ...
        if (m_spellInfo->RecoveryTime == 0 &&
            m_spellInfo->CategoryRecoveryTime == 0 && m_caster->GetCharmInfo())
            gcd = MIN_GCD;
        else
            return;
    }
    ...
    // There are some spells (mostly not casted directly by player) that have <
    // 1 sec and > 1.5 sec global cooldowns but as tests show are not affected
    // by any spell mods.
    if (m_spellInfo->StartRecoveryTime >= MIN_GCD &&
        m_spellInfo->StartRecoveryTime <= MAX_GCD) {
        // gcd modifier auras are applied only to own spells and only players
        // have such mods
        if (m_caster->GetTypeId() == TYPEID_PLAYER)
            m_caster->ToPlayer()->ApplySpellMod(
                m_spellInfo->Id, SPELLMOD_GLOBAL_COOLDOWN, gcd, this);
    ...
    if (m_caster->GetCharmInfo())
        ...
        m_caster->ToPlayer()->GetGlobalCooldownMgr().AddGlobalCooldown(
            m_spellInfo, gcd);
}
...
void Spell::CancelGlobalCooldown()
{
    ...
    if (m_caster->GetCharmInfo())
        ...
        m_caster->ToPlayer()->GetGlobalCooldownMgr().CancelGlobalCooldown(
            m_spellInfo);
}
    ...
    if (m_spellInfo->Id == 21651) {
        if (GameObject* go = m_targets.GetGOTarget()) {
            LockEntry const* lockInfo =
                sLockStore.LookupEntry(go->GetGOInfo()->GetLockId());
...
std::string Spell::GetDebugInfo() const
{
    ...
    sstr << std::boolalpha << "Id: " << GetSpellInfo()->Id
         << " OriginalCaster: " << m_originalCasterGUID.ToString()
         << " State: " << getState();
    return sstr.str();
}
...
WorldObjectSpellTargetCheck::WorldObjectSpellTargetCheck(
    ...
    SpellTargetCheckTypes selectionType,
    ConditionList*        condList)
    : _caster(caster), _referer(referer), _spellInfo(spellInfo),
...
bool WorldObjectSpellTargetCheck::operator()(WorldObject* target)
{
    ...
    if (Corpse* corpseTarget = target->ToCorpse()) {
        ...
        if (Player* owner =
                ObjectAccessor::FindPlayer(corpseTarget->GetOwnerGUID()))
            unitTarget = owner;
        else
            return false;
    }
...
WorldObjectSpellNearbyTargetCheck::WorldObjectSpellNearbyTargetCheck(
    float                 range,
    Unit*                 caster,
    SpellInfo const*      spellInfo,
    SpellTargetCheckTypes selectionType,
    ...
    : WorldObjectSpellTargetCheck(
          caster, caster, spellInfo, selectionType, condList),
      _range(range), _position(caster)
{
}
...
WorldObjectSpellAreaTargetCheck::WorldObjectSpellAreaTargetCheck(
    ...
    ConditionList*        condList)
    : WorldObjectSpellTargetCheck(
          caster, referer, spellInfo, selectionType, condList),
...
bool WorldObjectSpellAreaTargetCheck::operator()(WorldObject* target)
{
    if (target->GetTypeId() == TYPEID_GAMEOBJECT) {
        if (!target->ToGameObject()->IsInRange(_position->GetPositionX(),
                                               _position->GetPositionY(),
                                               _position->GetPositionZ(),
                                               _range))
            return false;
    }
        ...
        return false;
             ...
             target->ToCreature()->IsAvoidingAOE()) // pussywizard
        return false;
...
WorldObjectSpellConeTargetCheck::WorldObjectSpellConeTargetCheck(
    ...
    : WorldObjectSpellAreaTargetCheck(
          range, caster, caster, caster, spellInfo, selectionType, condList),
...
bool WorldObjectSpellConeTargetCheck::operator()(WorldObject* target)
{
    if (_spellInfo->HasAttribute(SPELL_ATTR0_CU_CONE_BACK)) {
        ...
        if (!_caster->HasInLine(
                target, _caster->GetObjectSize() + target->GetObjectSize()))
            return false;
    }
    else {
...
WorldObjectSpellTrajTargetCheck::WorldObjectSpellTrajTargetCheck(
    ...
    ConditionList*        condList)
    : WorldObjectSpellAreaTargetCheck(
          range, position, caster, caster, spellInfo, selectionType, condList)
{
}
