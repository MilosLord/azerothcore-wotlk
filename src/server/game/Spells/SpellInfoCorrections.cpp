...
inline void ApplySpellFix(std::initializer_list<uint32> spellIds,
                          void (*fix)(SpellInfo*))
{
    for (uint32 spellId : spellIds) {
        ...
        if (!spellInfo) {
            LOG_ERROR(
                "sql.sql",
                "Spell info correction specified for non-existing spell {}",
                spellId);
            continue;
        }
...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    ApplySpellFix({42533}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(347); // 15 min
    });
    ...
    ApplySpellFix({38776}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(4); // 120 seconds
    });

    ApplySpellFix(
        {
            63026, // Force Cast (HACK: Target shouldn't be changed)
            63137  // Force Cast (HACK: Target shouldn't be changed; summon
                   // position should be untied from spell destination)
        },
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].TargetA =
                SpellImplicitTargetInfo(TARGET_DEST_DB);
        });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = 0; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].TargetA =
                SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
            spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo(0);
        });

    // Howl of Azgalor
    ApplySpellFix({31344}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_100_YARDS); // 100yards instead of 50000?!
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->RangeEntry =
                sSpellRangeStore.LookupEntry(6); // 100 yards
        });
    ...
    ApplySpellFix({36350}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TriggerSpell =
            36325; // They Must Burn Bomb Drop (DND)
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->AttributesEx3 |=
                SPELL_ATTR3_CAN_PROC_FROM_PROCS; // Entries were not updated
                                                 // after spell effect change,
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 3; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 10; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 1; });
    ...
    // Tidal Wave
    ApplySpellFix({53390},
                  [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 2; });
    ...
    ApplySpellFix({28200},
                  [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 6; });
    ...
    ApplySpellFix({29809}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_7_YARDS);
    });

    // Sic'em
    ApplySpellFix({42767}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_NEARBY_ENTRY);
    });

    // Master Shapeshifter: missing stance data for forms other than bear - bear
    // version has correct data To prevent aura staying on target after talent
    // unlearned
    ...
    ApplySpellFix({51466, 51470}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].SpellClassMask =
    ...
    ApplySpellFix({47569}, [](SpellInfo* spellInfo) {
        spellInfo->Attributes &=
            ~SPELL_ATTR0_NOT_SHAPESHIFTED; // with this spell atrribute aura can
                                           // be stacked several times
    });
    ...
    ApplySpellFix({41013}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].ApplyAuraName =
            SPELL_AURA_DUMMY; // proc debuff, and summon infinite fiends
    });
    ...
    ApplySpellFix({29214, 54836}, [](SpellInfo* spellInfo) {
        // target allys instead of enemies, target A is src_caster, spells with
        // effect like that have ally target this is the only known exception,
        // probably just wrong data
        spellInfo->Effects[EFFECT_0].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_SRC_AREA_ALLY);
        spellInfo->Effects[EFFECT_1].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_SRC_AREA_ALLY);
    });
    ...
    ApplySpellFix({57994}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].Effect =
    ...
    // Marked for Death
    ApplySpellFix({53241, 53243, 53244, 53245, 53246},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_0].SpellClassMask =
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].TargetA =
                SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
            spellInfo->Effects[EFFECT_0].TargetB =
                SpellImplicitTargetInfo(TARGET_UNIT_PET);
        });
    ...
    ApplySpellFix({70893}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
        spellInfo->Effects[EFFECT_0].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_MASTER);
    });
    ...
    ApplySpellFix({54800}, [](SpellInfo* spellInfo) {
        ...
        // this is done because another spell also uses the same
        // SpellFamilyFlags as Icy Touch SpellFamilyFlags[0] & 0x00000040 in
        // SPELLFAMILY_DEATHKNIGHT is currently unused (3.3.5a) this needs
        // research on modifier applying rules, does not seem to be in
        // Attributes fields
        spellInfo->Effects[EFFECT_0].SpellClassMask =
    ...
    ApplySpellFix({45602}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].BasePoints =
            0; // force seat 0, vehicle doesn't have the required seat flags for
               // "no seat specified (-1)"
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].BasePoints =
                100; // 100% chance of procc'ing, not -10% (chance calculated in
    ...
    ApplySpellFix({61719}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->AuraInterruptFlags =
            AURA_INTERRUPT_FLAG_HITBYSPELL | AURA_INTERRUPT_FLAG_TAKE_DAMAGE;
    });

    // Bleh, need to change FamilyFlags :/ (have the same as original aura -
    // bad!)
    ...
    ApplySpellFix({20138, // Improved Devotion Aura
                   20139,
                   20140},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_1].SpellClassMask[0] = 0;
                      spellInfo->Effects[EFFECT_1].SpellClassMask[2] =
                          0x2000000;
                  });

    ApplySpellFix({20254, // Improved concentration aura
                   20255,
                   20256},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_1].SpellClassMask[0] = 0;
                      ...
                      spellInfo->Effects[EFFECT_2].SpellClassMask[2] =
                          0x4000000;
                  });

    ApplySpellFix({53379, // Swift Retribution
                   53484,
                   53648},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_0].SpellClassMask[0] = 0;
                      spellInfo->Effects[EFFECT_0].SpellClassMask[2] =
                          0x8000000;
                  });
    ...
    // Redemption
    ApplySpellFix({7328, 10322, 10324, 20772, 20773, 48949, 48950},
                  [](SpellInfo* spellInfo) {
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            // hack for seal of light and few spells, judgement consists of few
            // single casts and each of them can proc some spell, base one has
            // disabled proc flag but those dont have this flag
    ...
    ApplySpellFix({67480}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].ApplyAuraName =
            SPELL_AURA_DUMMY; // just a marker
    });
    ...
    ApplySpellFix({54968, // Glyph of Holy Light, Damage Class should be magic
    ...
    ApplySpellFix({53480}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ALLY);
    ...
    // Silencing Shot
    ApplySpellFix({34490, 41084, 42671},
                  [](SpellInfo* spellInfo) { spellInfo->Speed = 0.0f; });

    // Monstrous Bite
    ApplySpellFix({54680, 55495, 55496, 55497, 55498, 55499},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_1].TargetA =
                          SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
                  });
    ...
    ApplySpellFix({34027, 37529},
                  [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 0; });
    ...
    ApplySpellFix({19185, 64803, 64804}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_DEST_TARGET_ENEMY);
        spellInfo->Effects[EFFECT_0].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_DEST_AREA_ENEMY);
    ...
    ApplySpellFix({19465}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_2].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
    });
    ...
    // Impurity
    ApplySpellFix(
        {49220, 49633, 49635, 49636, 49638}, [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_APPLY_AURA;
            spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_DUMMY;
            spellInfo->Effects[EFFECT_0].TargetA =
                SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
    ...
    // Magic Suppression
    ApplySpellFix({49224, 49610, 49611},
                  [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 0; });
    ...
    // Blood Gorged
    ApplySpellFix({61274, 61275, 61276, 61277, 61278},
                  [](SpellInfo* spellInfo) {
    ...
    ApplySpellFix({49576, 49560}, [](SpellInfo* spellInfo) {
        ...
        //  should fix taunt on bosses and not break the pull protection at the
        //  same time (no aura provides immunity to grip mechanic)
    ...
    // Frost Strike
    ApplySpellFix(
        {49143, 51416, 51417, 51418, 51419, 55268}, [](SpellInfo* spellInfo) {
    ...
    ApplySpellFix({8076, 8162, 8163, 10441, 25362, 25527, 57621, 58646},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_1].RadiusEntry =
                          spellInfo->Effects[EFFECT_0].RadiusEntry;
                      spellInfo->Effects[EFFECT_2].RadiusEntry =
                          spellInfo->Effects[EFFECT_0].RadiusEntry;
                  });

    // Flametongue Totem effect
    ApplySpellFix({52109, 52110, 52111, 52112, 52113, 58651, 58654, 58655},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_2].TargetB =
                          spellInfo->Effects[EFFECT_1].TargetB =
    ...
    ApplySpellFix({6495}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(0);
    });

    // Bind Sight (PT)
    ApplySpellFix({6277}, [](SpellInfo* spellInfo) {
        // because it is passive, needs this to be properly removed at death in
    ...
    ApplySpellFix({47201, 47202, 47203, 47204, 47205},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_1].SpellClassMask[0] |=
                          2; // add corruption to affected spells
                  });

    // Death's Embrace
    ApplySpellFix({47198, 47199, 47200}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_1].SpellClassMask[0] |=
            0x4000; // include Drain Soul
    });
    ...
    ApplySpellFix(
        {11129}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });

    // Magic Absorption
    ApplySpellFix({29441, 29444},
                  [](SpellInfo* spellInfo) { spellInfo->SpellLevel = 0; });
    ...
    ApplySpellFix({7405, 8380, 11596, 11597, 25225, 47467},
                  [](SpellInfo* spellInfo) {
    ...
    ApplySpellFix({64904}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_2].DieSides =
            spellInfo->Effects[EFFECT_0].DieSides;
        spellInfo->Effects[EFFECT_2].TargetA =
            spellInfo->Effects[EFFECT_0].TargetB;
        spellInfo->Effects[EFFECT_2].RadiusEntry =
            spellInfo->Effects[EFFECT_0].RadiusEntry;
    ...
    ApplySpellFix({14898, // Spiritual Healing affects prayer of mending
                   15349,
                   15354,
                   15355,
                   15356,
                   ...
                   52803},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_0].SpellClassMask[1] |=
                          0x20; // prayer of mending
                  });
    ...
    ApplySpellFix({10060}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_2].ApplyAuraName =
            SPELL_AURA_ADD_PCT_MODIFIER;
        spellInfo->Effects[EFFECT_2].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ALLY);
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->AttributesEx &=
                ~SPELL_ATTR1_IMMUNITY_TO_HOSTILE_AND_FRIENDLY_EFFECTS;
        });

    ApplySpellFix({49376}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS); // 3yd
        spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPRESS_TARGET_PROCS;
    });
    ...
    ApplySpellFix({3286}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
    });
    ...
    ApplySpellFix({39953}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_SRC_CASTER);
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_SRC_CASTER);
        spellInfo->Effects[EFFECT_2].TargetA =
            SpellImplicitTargetInfo(TARGET_SRC_CASTER);
        ...
        spellInfo->Effects[EFFECT_2].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_SRC_AREA_ALLY);
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(367); // 2 Hours
    });

    ApplySpellFix(
        {
            57607, // WintergraspCatapult - Spell Plague Barrel -
                   // EffectRadiusIndex
            57619, // WintergraspDemolisher - Spell Hourl Boulder -
                   // EffectRadiusIndex
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_1].RadiusEntry =
                sSpellRadiusStore.LookupEntry(
                    EFFECT_RADIUS_25_YARDS); // SPELL_EFFECT_WMO_DAMAGE
        });
    ...
    ApplySpellFix({51422}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_10_YARDS); // SPELL_EFFECT_SCHOOL_DAMAGE
    });
    ...
    ApplySpellFix({54107}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_3_YARDS); // SPELL_EFFECT_KNOCK_BACK
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_3_YARDS); // SPELL_EFFECT_SCHOOL_DAMAGE
        spellInfo->Effects[EFFECT_2].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_3_YARDS); // SPELL_EFFECT_WEAPON_DAMAGE
    });
    ...
    ApplySpellFix({51678}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_10_YARDS); // SPELL_EFFECT_KNOCK_BACK
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_10_YARDS); // SPELL_EFFECT_SCHOOL_DAMAGE
        spellInfo->Effects[EFFECT_2].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_20_YARDS); // SPELL_EFFECT_WEAPON_DAMAGE
    });

    // WintergraspCatapult - Spell Plague Barrell - Range
    ApplySpellFix({57606}, [](SpellInfo* spellInfo) {
        spellInfo->RangeEntry =
            sSpellRangeStore.LookupEntry(164); // "Catapult Range"
    });

    // Boulder (Demolisher)
    ApplySpellFix({50999}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS); // 10yd
    });

    // Flame Breath (Catapult)
    ApplySpellFix({50990}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_18_YARDS); // 18yd
    });
    ...
    ApplySpellFix({36173}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TriggerSpell =
            36174; // Currently triggers heroic version...
    });
    ...
    ApplySpellFix({46638}, [](SpellInfo* spellInfo) {
        spellInfo->AttributesEx3 &=
            ~SPELL_ATTR3_ONLY_ON_PLAYER; // Obvious fail, it targets
                                         // gameobject...
    });
    ...
    // Armageddon
    ApplySpellFix({45909},
                  [](SpellInfo* spellInfo) { spellInfo->Speed = 8.0f; });
    ...
    ApplySpellFix({32111}, [](SpellInfo* spellInfo) {
        spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(
            0); // We only need the animation, no damage
    });
    ...
    ApplySpellFix({66690}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_100_YARDS); // 100yd
        spellInfo->MaxAffectedTargets = 1;
    });
    ...
    ApplySpellFix({57697}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_DEST_DEST);
    });
    ...
    ApplySpellFix(
        {
            ...
            56438, // Arcane Overload (-50% dmg taken) - this is to prevent
                   // apply -> unapply -> apply ... dunno whether it's correct
        },
        [](SpellInfo* spellInfo) {
            spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE;
        });
    ...
    ApplySpellFix({57060}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_2].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_VEHICLE);
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            ...
            spellInfo->Effects[EFFECT_0].RadiusEntry =
                sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50000_YARDS);
            spellInfo->AttributesEx4 |= SPELL_ATTR4_ALLOW_CAST_WHILE_CASTING;
            spellInfo->Effects[EFFECT_0].TargetA =
                SpellImplicitTargetInfo(TARGET_SRC_CASTER);
            spellInfo->Effects[EFFECT_0].TargetB =
                SpellImplicitTargetInfo(TARGET_UNIT_SRC_AREA_ENEMY);
            spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT;
        });
    ...
    ApplySpellFix({62807}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_1_YARD); // 1yd
    });
    ...
    ApplySpellFix({62186}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].Amplitude =
            5000; // Duration 5 secs, amplitude 8 secs...
    });
    ...
    ApplySpellFix({62976}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(28); // 5 seconds, wrong DBC data?
    });

    // Sif's Blizzard
    ApplySpellFix({62576, 62602}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_8_YARDS); // 8yd
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_8_YARDS); // 8yd
    });
    ...
    // Protective Gaze
    ApplySpellFix(
        {64175}, [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 25000; });

    // Shadow Beacon
    ApplySpellFix({64465}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TriggerSpell =
            64467; // why do they need two script effects :/ (this one has
                   // visual effect)
    });
    ...
    // Onyxia's Lair, Onyxia, Flame Breath (TriggerSpell = 0 and spamming errors
    // in console)
    ...
    ApplySpellFix({17731, 69294}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_18_YARDS); // 18yd instead of 13yd to make sure
                                         // all cracks erupt
    });

    // Onyxia's Lair, Onyxia, Breath
    ApplySpellFix(
        {18576, 18578, 18579, 18580, 18581, 18582, 18583, 18609, 18611, 18612,
         18613, 18614, 18615, 18616, 18584, 18585, 18586, 18587, 18588, 18589,
         18590, 18591, 18592, 18593, 18594, 18595, 18564, 18565, 18566, 18567,
         18568, 18569, 18570, 18571, 18572, 18573, 18574, 18575, 18596, 18597,
         18598, 18599, 18600, 18601, 18602, 18603, 18604, 18605, 18606, 18607,
         18617, 18619, 18620, 18621, 18622, 18623, 18624, 18625, 18626, 18627,
         18628, 18618, 18351, 18352, 18353, 18354, 18355, 18356, 18357, 18358,
         18359, 18360, 18361, 17086, 17087, 17088, 17089, 17090, 17091, 17092,
         17093, 17094, 17095, 17097, 22267, 22268, 21132, 21133, 21135, 21136,
         21137, 21138, 21139},
        [](SpellInfo* spellInfo) {
            spellInfo->DurationEntry =
                sSpellDurationStore.LookupEntry(328); // 250ms
            ...
            if (spellInfo->Effects[EFFECT_1].Effect) {
                ...
                spellInfo->Effects[EFFECT_1].Amplitude =
                    ((spellInfo->CastTimeEntry ==
                      sSpellCastTimesStore.LookupEntry(170))
                         ? 50
                         : 215);
            }
        });
    ...
    ApplySpellFix({49838}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_150_YARDS);
    });

    // Oculus, Varos Cloudstrider, Energize Cores
    ApplySpellFix({61407, 62136, 56251, 54069}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_CONE_ENTRY);
        spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo();
    });
    ...
    ApplySpellFix({67530}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].ApplyAuraName =
            SPELL_AURA_PROC_TRIGGER_SPELL; // will trigger 67537
    });
    ...
    // Trial of the Crusader, Gormok, player vehicle spell, CUSTOM! (default
    // jump to hand, not used)
    ...
    ApplySpellFix({66313}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->Effects[EFFECT_0].TargetB =
            SpellImplicitTargetInfo(TARGET_DEST_TARGET_ANY);
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->Effects[EFFECT_1].TargetB =
            SpellImplicitTargetInfo(TARGET_DEST_TARGET_ANY);
    ...
    ApplySpellFix({66320, 67472, 67473, 67475}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_2_YARDS);
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_2_YARDS);
    });
    ...
    ApplySpellFix({66258, 67901}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(
            85); // summon for 18 seconds, 15 not enough
    });

    // Trial of the Crusader, Jaraxxus, Spinning Pain Spike
    ApplySpellFix({66281}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_4_YARDS);
    });
    ...
    ApplySpellFix({66287}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_2].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(35); // 4 secs
    });
    ...
    // Trial of the Crusader, Twin Valkyr, Touch of Light/Darkness, Light/Dark
    // Surge
    ApplySpellFix(
        {
            65950 // light 0
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            // spellInfo->Effects[EFFECT_0].ApplyAuraNames =
    ...
    ApplySpellFix({65875, 67303, 67304, 67305, 65876, 67306, 67307, 67308},
                  [](SpellInfo* spellInfo) {
                      spellInfo->Effects[EFFECT_1].Effect = 0;
                      spellInfo->Effects[EFFECT_2].Effect = 0;
                  });
    ...
    ApplySpellFix({70464}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_0].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_SRC_AREA_ENTRY);
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_70_YARDS);
    });
    ...
    ApplySpellFix({70525, 70639}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_2].TargetB =
            SpellImplicitTargetInfo(TARGET_UNIT_SRC_AREA_ENTRY);
        spellInfo->Effects[EFFECT_2].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_500_YARDS); // 500yd
    });
    ...
    ApplySpellFix({69604, 70286}, [](SpellInfo* spellInfo) {
        spellInfo->MaxAffectedTargets = 1;
        spellInfo->AttributesEx3 |=
            (SPELL_ATTR3_ALWAYS_HIT | SPELL_ATTR3_ONLY_ON_PLAYER);
    });
    ...
    ApplySpellFix({69172}, [](SpellInfo* spellInfo) {
        spellInfo->ProcFlags =
            DONE_HIT_PROC_FLAG_MASK & ~PROC_FLAG_DONE_PERIODIC;
    ...
    ApplySpellFix({69238, 69628}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_DEST_DEST);
        spellInfo->Effects[EFFECT_0].TargetB =
            SpellImplicitTargetInfo(TARGET_DEST_DYNOBJ_NONE);
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_DEST_DEST);
        spellInfo->Effects[EFFECT_1].TargetB =
            SpellImplicitTargetInfo(TARGET_DEST_DYNOBJ_NONE);
    ...
    ApplySpellFix({69503}, [](SpellInfo* spellInfo) {
        spellInfo->ChannelInterruptFlags |= 0;
        spellInfo->AuraInterruptFlags =
            AURA_INTERRUPT_FLAG_MOVE | AURA_INTERRUPT_FLAG_TURNING;
    });

    // Falric: Defiling Horror
    ApplySpellFix({72435, 72452}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_200_YARDS);
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_200_YARDS);
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            ...
            spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo();
            spellInfo->Effects[EFFECT_1].TargetA = SpellImplicitTargetInfo(
                TARGET_DEST_DB); // this target is for
    ...
    ApplySpellFix({69055, 70814}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_5_YARDS); // 5yd
    });

    // Impaled (Lord Marrowgar)
    ApplySpellFix({69065}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].Effect =
            0; // remove stun so Dispersion can be used
    });
    ...
    ApplySpellFix({72701, 72702, 72703, 72704}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].TargetB = SpellImplicitTargetInfo();
        spellInfo->DurationEntry             = sSpellDurationStore.LookupEntry(
            9); // 30 secs instead of 12, need him for longer, but will stop his
                            // actions after 12 secs
    });
    ...
    ApplySpellFix({69146, 70823, 70824, 70825}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(
                EFFECT_RADIUS_3_YARDS); // 3yd instead of 5yd
        spellInfo->AttributesEx4 &= ~SPELL_ATTR4_NO_CAST_LOG;
    });
    ...
    ApplySpellFix({72723}, [](SpellInfo* spellInfo) {
        // this spell initially granted Shadow damage immunity, however it was
        // removed but the data was left in client
        spellInfo->Effects[EFFECT_2].Effect = 0;
    });

    // Mark of the Fallen Champion (Deathbringer Saurfang)
    ApplySpellFix({72255, 72444, 72445, 72446}, [](SpellInfo* spellInfo) {
        // Patch 3.3.2 (2010-01-02): Deathbringer Saurfang will no longer gain
        // blood power from Mark of the Fallen Champion. prevented in script,
        // effect needed for Prayer of Mending
        spellInfo->AttributesEx3 &= ~SPELL_ATTR3_SUPRESS_CASTER_PROCS;
    });
    ...
    ApplySpellFix({70460}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(1); // 10 seconds
    });
    ...
    ApplySpellFix({70530}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].Effect =
            SPELL_EFFECT_APPLY_AURA; // blizzard typo, 65 instead of 6, aura
                                     // itself is defined (dummy)
    });
    ...
    ApplySpellFix({71621, // Create Concoction (Professor Putricide)
                   72850,
                   72851,
                   72852,
                  ...
                  [](SpellInfo* spellInfo) {
                      spellInfo->CastTimeEntry =
                          sSpellCastTimesStore.LookupEntry(15); // 4 sec
                  });
    ...
    ApplySpellFix({72454, 72464, 72506, 72507}, [](SpellInfo* spellInfo) {
        spellInfo->AttributesEx4 |= SPELL_ATTR4_NO_CAST_LOG;
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50000_YARDS); // 50000yd
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].RadiusEntry =
                sSpellRadiusStore.LookupEntry(
                    EFFECT_RADIUS_200_YARDS); // another missing radius
        });

    // Swarming Shadows
    ApplySpellFix({71266, 72890}, [](SpellInfo* spellInfo) {
        spellInfo->AreaGroupId = 0; // originally, these require area 4522,
                                    // which is... outside of Icecrown Citadel
    });
    ...
    ApplySpellFix({70715}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(32); // 6 seconds (missing)
    });

    // Mana Void (periodic aura)
    ApplySpellFix({71085}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(9); // 30 seconds (missing)
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].RadiusEntry =
                sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_200_YARDS); // 200yd
        });
    ...
    ApplySpellFix({70127, 72528, 72529, 72530}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_1].Effect =
            0; // remove obsolete spell effect with no targets
    });
    ...
    ApplySpellFix({70337, 73912, 73913, 73914, 70338, 73785, 73786, 73787},
                  [](SpellInfo* spellInfo) {
                      spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT;
                  });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) {
            spellInfo->Effects[EFFECT_0].RadiusEntry =
                sSpellRadiusStore.LookupEntry(
                    EFFECT_RADIUS_50000_YARDS); // 50000yd
        });
    ...
    ApplySpellFix({73540}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(3); // 60 seconds
    });

    // Shadow Trap (visual)
    ApplySpellFix({73530}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(28); // 5 seconds
    });
    ...
    ApplySpellFix({72762}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(559); // 53 seconds
    ...
    ApplySpellFix({72743}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(22); // 45 seconds
    });
    ...
    ApplySpellFix({73579}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_25_YARDS); // 25yd
    });
    ...
    ApplySpellFix({70485}, [](SpellInfo* spellInfo) {
        spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(6); // 100yd
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS);
    ...
    ApplySpellFix({70227, 70232}, [](SpellInfo* spellInfo) {
        spellInfo->AreaGroupId =
            2452; // Whole icc instead of Crimson Halls only, remove when area
                  // calculation is fixed
    });

    ApplySpellFix({74509}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_20_YARDS); // 20yd
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_20_YARDS); // 20yd
    });
    ...
    ApplySpellFix({76221}, [](SpellInfo* spellInfo) {
        spellInfo->ChannelInterruptFlags &=
            ~(AURA_INTERRUPT_FLAG_TURNING | AURA_INTERRUPT_FLAG_MOVE);
        spellInfo->Effects[EFFECT_0].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_NEARBY_ENTRY);
    });
    ...
    ApplySpellFix({75883, 75884}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS); // 6yd
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_6_YARDS); // 6yd
    });
    ...
    ApplySpellFix({74799}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_12_YARDS); // 12yd
    });
    ...
    // Meteor Strike
    ApplySpellFix({74637}, [](SpellInfo* spellInfo) { spellInfo->Speed = 0; });
    ...
    ApplySpellFix({54897}, [](SpellInfo* spellInfo) {
        ...
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            spellInfo->Effects[EFFECT_0].RadiusEntry;
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_UNIT_DEST_AREA_ENTRY);
    });
    ...
    ApplySpellFix({40856}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(27); // 3000ms
    });
    ...
    ApplySpellFix({34888}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(5); // 300 secs
    });
    ...
    ApplySpellFix({49177}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].BasePoints =
            1; // corrects seat id (points - 1 = seatId)
    });

    // Jormungar Strike
    ApplySpellFix({56513},
                  [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 2000; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 3000; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 6000; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 10000; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 12000; });

    // Signal Helmet to Attack
    ApplySpellFix(
        {51748}, [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 15000; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 20000; });
    ...
    // Savory Deviate Delight (transformations), allow to mount while
    // transformed
    ...
    ApplySpellFix({72623}, [](SpellInfo* spellInfo) {
        spellInfo->Effects[EFFECT_0].BasePoints =
            spellInfo->Effects[EFFECT_1].BasePoints;
    });

    // Compact Harvest Reaper
    ApplySpellFix({4078}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(6); // 10 minutes
    });
    ...
    ApplySpellFix({61994}, [](SpellInfo* spellInfo) {
        spellInfo->ManaCostPercentage =
            0; // Clicking on Warlock Summoning portal should not require mana
    });
    ...
    ApplySpellFix({20540, 32199}, [](SpellInfo* spellInfo) {
        spellInfo->AuraInterruptFlags |=
            (AURA_INTERRUPT_FLAG_MELEE_ATTACK | AURA_INTERRUPT_FLAG_CAST);
    });
    ...
    // Mortal Shots
    ApplySpellFix(
        {19485, 19487, 19488, 19489, 19490}, [](SpellInfo* spellInfo) {
    ...
    // Reflection
    ApplySpellFix(
        {22067}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });
    ...
    ApplySpellFix({26157, 26272, 26273, 26274},
                  [](SpellInfo* spellInfo) { spellInfo->Mechanic = 0; });
    ...
    ApplySpellFix(
        {24327}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });
    ...
    ApplySpellFix({28772}, [](SpellInfo* spellInfo) { spellInfo->Speed = 1; });
    ...
    // WotLK Prologue Frozen Shade Visual, temp used to restore visual after
    // Dispersion
    ...
    ApplySpellFix({32474}, [](SpellInfo* spellInfo) {
        spellInfo->DurationEntry =
            sSpellDurationStore.LookupEntry(556); // 28 seconds
    });
    ...
    ApplySpellFix({30567},
                  [](SpellInfo* spellInfo) { spellInfo->ProcChance = 3; });
    ...
    ApplySpellFix({45406}, [](SpellInfo* spellInfo) {
        spellInfo->AuraInterruptFlags |=
            (AURA_INTERRUPT_FLAG_MOUNT | AURA_INTERRUPT_FLAG_CAST);
    });
    ...
    // Battle Shout
    ApplySpellFix({2048, 5242, 6192, 6673, 11549, 11550, 11551, 25289, 47436},
                  [](SpellInfo* spellInfo) {
                      spellInfo->AttributesEx2 |=
                          SPELL_ATTR2_IGNORE_LINE_OF_SIGHT;
                  });
    ...
    // Gift of the Wild
    ApplySpellFix(
        {21849, 21850, 26991, 48470, 69381}, [](SpellInfo* spellInfo) {
            spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT;
        });
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
            ...
            // Xinef: i hope this will fix the problem with not working
            // resurrection
            if (spellInfo->Effects[j].Effect == SPELL_EFFECT_SELF_RESURRECT) {
                spellInfo->Effects[j].TargetA =
                    SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
            }
        }
        ...
        for (SpellEffectInfo const& spellEffectInfo : spellInfo->GetEffects()) {
            if (spellEffectInfo.IsEffect() &&
                (spellEffectInfo.TargetA.GetTarget() == TARGET_DEST_TRAJ ||
                 spellEffectInfo.TargetB.GetTarget() == TARGET_DEST_TRAJ)) {
                // Get triggered spell if any
                if (SpellInfo* spellInfoTrigger = const_cast<SpellInfo*>(
                        GetSpellInfo(spellEffectInfo.TriggerSpell))) {
                    float maxRangeMain =
                        spellInfo->RangeEntry
                            ? spellInfo->RangeEntry->RangeMax[0]
                            : 0.0f;
                    float maxRangeTrigger =
                        spellInfoTrigger->RangeEntry
                            ? spellInfoTrigger->RangeEntry->RangeMax[0]
                            : 0.0f;

                    // check if triggered spell has enough max range to cover
                    // trajectory
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            // Icy Touch - extend FamilyFlags (unused value) for Sigil of the
            // Frozen Conscience to use
            if (spellInfo->SpellIconID == 2721 &&
        ...
        if (spellInfo->CategoryEntry == sSpellCategoryStore.LookupEntry(132) &&
            spellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR) {
            spellInfo->AttributesEx6 |= SPELL_ATTR6_NO_CATEGORY_COOLDOWN_MODS;
        }
    }

    // Xinef: The Veiled Sea area in outlands (Draenei zone), client blocks
    // casting flying mounts
    for (uint32 i = 0; i < sAreaTableStore.GetNumRows(); ++i)
        if (AreaTableEntry* areaEntry =
                const_cast<AreaTableEntry*>(sAreaTableStore.LookupEntry(i))) {
    ...
    SummonPropertiesEntry* properties = const_cast<SummonPropertiesEntry*>(
        sSummonPropertiesStore.LookupEntry(121));
    properties->Type = SUMMON_TYPE_TOTEM;
    properties       = const_cast<SummonPropertiesEntry*>(
        sSummonPropertiesStore.LookupEntry(647)); // 52893
    properties->Type = SUMMON_TYPE_TOTEM;
    if ((properties = const_cast<SummonPropertiesEntry*>(
             sSummonPropertiesStore.LookupEntry(628)))) // Hungry Plaguehound
    {
    ...
    // Correct Pet Size
    CreatureDisplayInfoEntry* displayEntry =
        const_cast<CreatureDisplayInfoEntry*>(
            sCreatureDisplayInfoStore.LookupEntry(17028)); // Kurken
    displayEntry->scale = 2.5f;

    // Oracles and Frenzyheart faction
    FactionEntry* factionEntry =
        const_cast<FactionEntry*>(sFactionStore.LookupEntry(1104));
    ...
    FactionTemplateEntry* factionTemplateEntry =
        const_cast<FactionTemplateEntry*>(
            sFactionTemplateStore.LookupEntry(1978)); // Warsong Offensive
    factionTemplateEntry->hostileMask |= 8;
    factionTemplateEntry = const_cast<FactionTemplateEntry*>(
        sFactionTemplateStore.LookupEntry(1921)); // The Taunka
    factionTemplateEntry->hostileMask |= 8;

    // Remove vehicles attr, making accessories selectable
    VehicleSeatEntry* vse = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(4689)); // Siege Engine, Accessory
    vse->m_flags &= ~VEHICLE_SEAT_FLAG_PASSENGER_NOT_SELECTABLE;
    vse = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(4692)); // Siege Engine, Accessory
    vse->m_flags &= ~VEHICLE_SEAT_FLAG_PASSENGER_NOT_SELECTABLE;
    vse = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(4693)); // Siege Engine, Accessory
    vse->m_flags &= ~VEHICLE_SEAT_FLAG_PASSENGER_NOT_SELECTABLE;
    vse = const_cast<VehicleSeatEntry*>(sVehicleSeatStore.LookupEntry(
        3011)); // Salvaged Demolisher, Ulduar - not allow to change seats
    vse->m_flags &= ~VEHICLE_SEAT_FLAG_CAN_SWITCH;
    vse = const_cast<VehicleSeatEntry*>(sVehicleSeatStore.LookupEntry(
        3077)); // Salvaged Demolisher Seat, Ulduar - not allow to change seats
    vse->m_flags &= ~VEHICLE_SEAT_FLAG_CAN_SWITCH;

    // pussywizard: fix z offset for some vehicles:
    vse = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(6206)); // Marrowgar - Bone Spike
    vse->m_attachmentOffsetZ = 4.0f;
    vse = const_cast<VehicleSeatEntry*>(sVehicleSeatStore.LookupEntry(
        3806)); // Mimiron - seat on VX-001 for ACU during last phase
    vse->m_attachmentOffsetZ = 15.0f;
    vse                      = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(3566)); // Mimiron - Working seat
    vse->m_attachmentOffsetX = -3.5f;
    vse->m_attachmentOffsetY = 0.0f;
    vse                      = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(3567)); // Mimiron - Working seat
    ...
    AchievementEntry* achievement =
        const_cast<AchievementEntry*>(sAchievementStore.LookupEntry(4539));
    ...
    LockEntry* key = const_cast<LockEntry*>(sLockStore.LookupEntry(
        36)); // 3366 Opening, allows to open without proper key
    key->Type[2]   = LOCK_KEY_NONE;

    LOG_INFO("server.loading",
             ">> Loading spell dbc data corrections  in {} ms",
