...
{
    SkillLineAbilityMapBounds skillBounds =
        sSpellMgr->GetSkillLineAbilityMapBounds(spellId);
    for (SkillLineAbilityMap::const_iterator itr = skillBounds.first;
         itr != skillBounds.second;
         ++itr)
                                                    ...
                                                    bool             triggered)
{
    ...
    case SPELLFAMILY_MAGE: {
        ...
        // Shattered Barrier
            ...
            return DIMINISHING_ROOT;
        // Deep Freeze
        else if (spellproto->SpellIconID == 2939 &&
                 spellproto->SpellVisual[0] == 9963)
            return DIMINISHING_CONTROLLED_STUN;
    ...
    case SPELLFAMILY_HUNTER: {
        ...
        if ((spellproto->SpellFamilyFlags[0] & 0x400) &&
            spellproto->SpellIconID == 538)
            return DIMINISHING_LIMITONLY;
        // Scatter Shot (own diminishing)
        else if ((spellproto->SpellFamilyFlags[0] & 0x40000) &&
                 spellproto->SpellIconID == 132)
            return DIMINISHING_SCATTER_SHOT;
        // Entrapment (own diminishing)
        else if (spellproto->SpellVisual[0] == 7484 &&
                 spellproto->SpellIconID == 20)
            return DIMINISHING_ENTRAPMENT;
        // Wyvern Sting mechanic is MECHANIC_SLEEP but the diminishing is
        // DIMINISHING_DISORIENT
        else if ((spellproto->SpellFamilyFlags[1] & 0x1000) &&
                 spellproto->SpellIconID == 1721)
            return DIMINISHING_DISORIENT;
        // Freezing Arrow
    ...
    case SPELLFAMILY_PALADIN: {
        ...
        if (spellproto->SpellFamilyFlags[0] & 0x100000)
            return DIMINISHING_LIMITONLY;
        // Turn Evil
        else if ((spellproto->SpellFamilyFlags[1] & 0x804000) &&
                 spellproto->SpellIconID == 309)
            return DIMINISHING_FEAR;
        break;
    }
    ...
    if (mechanic & ((1 << MECHANIC_SAPPED) | (1 << MECHANIC_POLYMORPH) |
                    (1 << MECHANIC_SHACKLE)))
        return DIMINISHING_DISORIENT;
...
bool SpellMgr::ComputeIsSpellValid(SpellInfo const* spellInfo, bool msg)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        // craft spell for crafting non-existed item (break client recipes list
        // show)
        case SPELL_EFFECT_CREATE_ITEM:
        case SPELL_EFFECT_CREATE_ITEM_2: {
            if (spellInfo->Effects[i].ItemType == 0) {
                // skip auto-loot crafting spells, its not need explicit item
                // info (but have special fake items sometime)
                if (!spellInfo->IsLootCrafting()) {
                    if (msg)
                        LOG_ERROR("sql.sql",
                                  "Craft spell {} not have create item entry.",
                                  spellInfo->Id);
                    return false;
                }
            }
            // also possible IsLootCrafting case but fake item must exist anyway
            else if (!sObjectMgr->GetItemTemplate(
                         spellInfo->Effects[i].ItemType)) {
                if (msg)
                    LOG_ERROR("sql.sql",
                              "Craft spell {} create not-exist in DB item "
                              "(Entry: {}) and then...",
                              spellInfo->Id,
                              spellInfo->Effects[i].ItemType);
                return false;
        ...
        case SPELL_EFFECT_LEARN_SPELL: {
            ...
            if (!ComputeIsSpellValid(spellInfo2, msg)) {
                if (msg)
                    LOG_ERROR("sql.sql",
                              "Spell {} learn to invalid spell {}, and then...",
                              spellInfo->Id,
    ...
    if (need_check_reagents) {
        for (uint8 j = 0; j < MAX_SPELL_REAGENTS; ++j) {
            if (spellInfo->Reagent[j] > 0 &&
                !sObjectMgr->GetItemTemplate(spellInfo->Reagent[j])) {
                if (msg)
                    LOG_ERROR("sql.sql",
                              "Craft spell {} have not-exist reagent in DB "
                              "item (Entry: {}) and then...",
                              spellInfo->Id,
                              spellInfo->Reagent[j]);
                return false;
                               ...
                               bool             isTalent)
{
    if (!spellInfo) {
        ...
        LOG_ERROR("spells",
                  "Player::{}: Non-existed in SpellStore spell #{} request.",
                  (isTalent ? "AddTalent" : "addSpell"),
                  spellId);
        return false;
    }
    ...
    if (!IsSpellValid(spellInfo)) {
        DeleteSpellFromAllPlayers(spellId);
        LOG_ERROR("spells",
                  "Player::{}: Broken spell #{} learning not allowed.",
                  (isTalent ? "AddTalent" : "addSpell"),
                  spellId);
        return false;
    }
...
{
    SpellDifficultySearcherMap::const_iterator i =
        mSpellDifficultySearcherMap.find(spellId);
                                         ...
                                         Unit const* caster) const
{
    ...
    SpellDifficultyEntry const* difficultyEntry =
        sSpellDifficultyStore.LookupEntry(difficultyId);
    if (!difficultyEntry) {
        LOG_DEBUG("spells.aura",
                  "SpellMgr::GetSpellIdForDifficulty: SpellDifficultyEntry not "
                  "found for spell {}. This should never happen.",
                  spellId);
        return spellId; // return source spell
    }
    ...
    if (difficultyEntry->SpellID[mode] <= 0 &&
        ...
        LOG_DEBUG("spells.aura",
                  "SpellMgr::GetSpellIdForDifficulty: spell {} mode {} spell "
                  "is nullptr, using mode {}",
                  spellId,
                  mode,
                  mode - 2);
        mode -= 2;
    ...
    if (difficultyEntry->SpellID[mode] <= 0) {
        LOG_ERROR("sql.sql",
                  "SpellMgr::GetSpellIdForDifficulty: spell {} mode {} spell "
                  "is 0. Check spelldifficulty_dbc!",
                  spellId,
                  mode);
        return spellId;
    }
    ...
    LOG_DEBUG("spells.aura",
              "SpellMgr::GetSpellIdForDifficulty: spellid for spell {} in mode "
              "{} is {}",
              spellId,
              mode,
              difficultyEntry->SpellID[mode]);
    return uint32(difficultyEntry->SpellID[mode]);
}

SpellInfo const*
SpellMgr::GetSpellForDifficultyFromSpell(SpellInfo const* spell,
                                         Unit const*      caster) const
{
    ...
    if (!newSpell) {
        LOG_DEBUG("spells.aura",
                  "SpellMgr::GetSpellForDifficultyFromSpell: spell {} not "
                  "found. Check spelldifficulty_dbc!",
                  newSpellId);
        return spell;
    }

    LOG_DEBUG("spells.aura",
              "SpellMgr::GetSpellForDifficultyFromSpell: Spell id for instance "
              "mode is {} (original {})",
              newSpell->Id,
              spell->Id);
    return newSpell;
...
uint32
SpellMgr::GetSpellWithRank(uint32 spell_id, uint32 rank, bool strict) const
{
    if (SpellChainNode const* node = GetSpellChainNode(spell_id)) {
        if (rank != node->rank)
            return GetSpellWithRank(node->rank < rank ? node->next->Id
                                                      : node->prev->Id,
                                    rank,
                                    strict);
    }
...
SpellMgr::GetSpellsRequiredForSpellBounds(uint32 spell_id) const
{
    return mSpellReq.equal_range(spell_id);
}

SpellsRequiringSpellMapBounds
SpellMgr::GetSpellsRequiringSpellBounds(uint32 spell_id) const
{
...
bool SpellMgr::IsSpellRequiringSpell(uint32 spellid, uint32 req_spellid) const
{
    SpellsRequiringSpellMapBounds spellsRequiringSpell =
        GetSpellsRequiringSpellBounds(req_spellid);
...
{
    return mTalentSpellAdditionalSet.find(spellId) !=
           mTalentSpellAdditionalSet.end();
}
...
SpellTargetPosition const*
SpellMgr::GetSpellTargetPosition(uint32 spell_id, SpellEffIndex effIndex) const
{
    SpellTargetPositionMap::const_iterator itr =
...
SpellMgr::GetSpellGroupSpecialFlags(uint32 spell_id) const
{
    uint32                        first_rank = GetFirstSpellInChain(spell_id);
    SpellGroupMap::const_iterator itr        = mSpellGroupMap.find(first_rank);
    if (itr != mSpellGroupMap.end())
...
SpellMgr::CheckSpellGroupStackRules(SpellInfo const* spellInfo1,
                                    SpellInfo const* spellInfo2,
                                    bool             remove,
                                    bool             areaAura) const
{
    ...
    if (groupId > 0 && groupId == GetSpellGroup(spellid_2)) {
        ...
        if (flag1 & SPELL_GROUP_SPECIAL_FLAG_STACK_EXCLUSIVE_MAX &&
            flag2 & SPELL_GROUP_SPECIAL_FLAG_STACK_EXCLUSIVE_MAX) {
            if (flag1 & flag2)
...
void SpellMgr::GetSetOfSpellsInSpellGroupWithFlag(
    uint32                 group_id,
    SpellGroupSpecialFlags flag,
    std::set<uint32>&      availableElixirs) const
{
    for (SpellGroupMap::const_iterator itr = mSpellGroupMap.begin();
         itr != mSpellGroupMap.end();
         ++itr)
    ...
    bool                       active) const
{
    ...
    if (EventProcFlag & (PROC_FLAG_KILLED | PROC_FLAG_KILL | PROC_FLAG_DEATH |
    ...
    if (procFlags & PROC_FLAG_DONE_PERIODIC) {
        if (procExtra & PROC_EX_INTERNAL_HOT) {
            if (EventProcFlag == PROC_FLAG_DONE_PERIODIC) {
                /// no aura with only PROC_FLAG_DONE_PERIODIC and
                /// spellFamilyName == 0 can proc from a HOT.
        ...
        /// Aura must have negative or neutral(PROC_FLAG_DONE_PERIODIC only)
        /// procflags for a DOT to proc
    ...
    if (procFlags & PROC_FLAG_TAKEN_PERIODIC) {
        if (procExtra & PROC_EX_INTERNAL_HOT) {
            /// No aura that only has PROC_FLAG_TAKEN_PERIODIC can proc from a
            /// HOT.
        ...
        /// Aura must have negative or neutral(PROC_FLAG_TAKEN_PERIODIC only)
        /// procflags for a DOT to proc
    ...
    if (spellProcEvent) // Exist event data
    {
        ...
        else // For spells need check school/spell family/family mask
        {
            ...
            if (spellProcEvent->schoolMask &&
                (spellProcEvent->schoolMask & procSpellInfo->SchoolMask) == 0)
                return false;

            // Check (if set) for spellFamilyName
            if (spellProcEvent->spellFamilyName &&
                (spellProcEvent->spellFamilyName !=
                 procSpellInfo->SpellFamilyName))
                return false;

            // spellFamilyName is Ok need check for spellFamilyMask if present
            if (spellProcEvent->spellFamilyMask) {
                if (!(spellProcEvent->spellFamilyMask &
                      procSpellInfo->SpellFamilyFlags))
                    return false;
                hasFamilyMask = true;
                // Some spells are not considered as active even with have
                // spellfamilyflags
    ...
    if (procEvent_procEx == PROC_EX_NONE) {
        // No extra req, so can trigger only for hit/crit - spell has to be
        // active
    ...
    else // Passive spells hits here only if resist/reflect/immune/evade
    {
        if (procExtra & AURA_SPELL_PROC_EX_MASK) {
            ...
            if (procEvent_procEx & PROC_EX_EX_TRIGGER_ALWAYS)
                return true;
            // PROC_EX_NOT_ACTIVE_SPELL and PROC_EX_ONLY_ACTIVE_SPELL flags
            // handle: if passed checks before
            if ((procExtra & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT)) &&
                ((procEvent_procEx & (AURA_SPELL_PROC_EX_MASK)) == 0))
                return true;
        }
                                          ...
                                          ProcEventInfo&        eventInfo) const
{
    ...
    if (procEntry.AttributesMask & PROC_ATTR_REQ_EXP_OR_HONOR)
        if (Player* actor = eventInfo.GetActor()->ToPlayer())
            if (eventInfo.GetActionTarget() &&
                !actor->isHonorOrXPTarget(eventInfo.GetActionTarget()))
                return false;

    // always trigger for these types
    if (eventInfo.GetTypeMask() &
        (PROC_FLAG_KILLED | PROC_FLAG_KILL | PROC_FLAG_DEATH))
        return true;
    ...
    if (eventInfo.GetTypeMask() &
        (PERIODIC_PROC_FLAG_MASK | SPELL_PROC_FLAG_MASK |
         PROC_FLAG_DONE_TRAP_ACTIVATION)) {
        if (procEntry.SpellFamilyName &&
            (procEntry.SpellFamilyName !=
             eventInfo.GetSpellInfo()->SpellFamilyName))
            return false;

        if (procEntry.SpellFamilyMask &&
            !(procEntry.SpellFamilyMask &
              eventInfo.GetSpellInfo()->SpellFamilyFlags))
            return false;
    }

    // check spell type mask (if set)
    if (eventInfo.GetTypeMask() &
        (SPELL_PROC_FLAG_MASK | PERIODIC_PROC_FLAG_MASK)) {
        if (procEntry.SpellTypeMask &&
            !(eventInfo.GetSpellTypeMask() & procEntry.SpellTypeMask))
            return false;
    }
...
SpellMgr::GetSkillLineAbilityMapBounds(uint32 spell_id) const
{
...
{
    SpellPetAuraMap::const_iterator itr =
...
SpellEnchantProcEntry const*
SpellMgr::GetSpellEnchantProcEvent(uint32 enchId) const
{
    SpellEnchantProcEventMap::const_iterator itr =
        mSpellEnchantProcEventMap.find(enchId);
...
PetLevelupSpellSet const*
SpellMgr::GetPetLevelupSpellList(uint32 petFamily) const
{
    PetLevelupSpellMap::const_iterator itr =
        mPetLevelupSpellMap.find(petFamily);
...
SpellMgr::GetSpellAreaForQuestMapBounds(uint32 quest_id) const
{
    return mSpellAreaForQuestMap.equal_range(quest_id);
}

SpellAreaForQuestMapBounds
SpellMgr::GetSpellAreaForQuestEndMapBounds(uint32 quest_id) const
{
    return mSpellAreaForQuestEndMap.equal_range(quest_id);
}

SpellAreaForAuraMapBounds
SpellMgr::GetSpellAreaForAuraMapBounds(uint32 spell_id) const
{
    return mSpellAreaForAuraMap.equal_range(spell_id);
}

SpellAreaForAreaMapBounds
SpellMgr::GetSpellAreaForAreaMapBounds(uint32 area_id) const
{
                                    ...
                                    uint32        newArea) const
{
    ...
    if (questStart) // not in expected required quest state
        if (!player || (((1 << player->GetQuestStatus(questStart)) &
                         questStartStatus) == 0))
            return false;

    if (questEnd) // not in expected forbidden quest state
        if (!player ||
            (((1 << player->GetQuestStatus(questEnd)) & questEndStatus) == 0))
            return false;

    if (auraSpell) // not have expected aura
        if (!player || (auraSpell > 0 && !player->HasAura(auraSpell)) ||
    ...
    case 68720: // Quarry - Isle of Conquest.
    {
        ...
        uint8 nodeType =
            spellId == 68719 ? NODE_TYPE_REFINERY : NODE_TYPE_QUARRY;
        uint8 nodeState = player->GetTeamId() == TEAM_ALLIANCE
    ...
    case 73828:
        if (player)
            if (InstanceScript* s =
                    const_cast<Player*>(player)->GetInstanceScript())
...
{
    for (SpellChainMap::iterator itr = mSpellChains.begin();
         itr != mSpellChains.end();
         ++itr)
...
{
    // cleanup core data before reload - remove reference to ChainNode from
    // SpellInfo
    ...
    for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i) {
        ...
        if (!firstSpell) {
            LOG_ERROR("sql.sql",
                      "SpellMgr::LoadSpellTalentRanks: First Rank Spell {} for "
                      "TalentEntry {} does not exist.",
                      talentInfo->RankID[0],
                      i);
            continue;
        }
        ...
        for (uint8 rank = 0; rank < MAX_TALENT_RANK; ++rank) {
            ...
            if (!currentSpell) {
                LOG_ERROR("sql.sql",
                          "SpellMgr::LoadSpellTalentRanks: Spell {} (Rank: {}) "
                          "for TalentEntry {} does not exist.",
                          spellId,
                          rank + 1,
                          i);
                break;
            ...
            node.prev = prevSpell;
            node.next = node.rank < MAX_TALENT_RANK
                            ? GetSpellInfo(talentInfo->RankID[node.rank])
                            : nullptr;
...
void SpellMgr::LoadSpellRanks()
{
    ...
    //                                               0               1 2
    QueryResult result =
        WorldDatabase.Query("SELECT first_spell_id, spell_id, `rank` from "
                            "spell_ranks ORDER BY first_spell_id, `rank`");
    ...
    do {
        ...
        if (!first) {
            LOG_ERROR("sql.sql",
                      "Spell rank identifier(first_spell_id) {} listed in "
                      "`spell_ranks` does not exist!",
                      lastSpell);
            continue;
        }
        ...
        if (rankChain.size() < 2) {
            LOG_ERROR(
                "sql.sql",
                "There is only 1 spell rank for identifier(first_spell_id) {} "
                "in `spell_ranks`, entry is not needed!",
                lastSpell);
            continue;
        }
        int32 curRank = 0;
        ...
        for (std::list<std::pair<int32, int32>>::iterator itr =
                 rankChain.begin();
             itr != rankChain.end();
             ++itr) {
            ...
            if (!spell) {
                LOG_ERROR("sql.sql",
                          "Spell {} (rank {}) listed in `spell_ranks` for "
            ...
            if (itr->second != curRank) {
                LOG_ERROR(
                    "sql.sql",
                    "Spell {} (rank {}) listed in `spell_ranks` for chain {} "
                    "does not have proper rank value(should be {})!",
                    itr->first,
                    itr->second,
                    lastSpell,
                    curRank);
        ...
        do {
            ...
            mSpellChains[addedSpell].last =
                GetSpellInfo(rankChain.back().first);
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} spell rank records in {} ms",
             count,
...
void SpellMgr::LoadSpellRequired()
{
    ...
    //                                                   0        1
    QueryResult result =
        WorldDatabase.Query("SELECT spell_id, req_spell from spell_required");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell required records. DB table "
                 "`spell_required` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "spell_id {} in `spell_required` table is not found in "
                      "dbcs, skipped",
                      spellId);
            continue;
        }
        ...
        if (!reqSpellInfo) {
            LOG_ERROR("sql.sql",
                      "req_spell {} in `spell_required` table is not found in "
                      "dbcs, skipped",
                      spellReq);
            continue;
        }
        ...
        if (GetFirstSpellInChain(spellId) == GetFirstSpellInChain(spellReq)) {
            LOG_ERROR("sql.sql",
                      "req_spell {} and spell_id {} in `spell_required` table "
                      "are ranks of the same spell, entry not needed, skipped",
                      spellReq,
                      spellId);
            continue;
        }
        ...
        if (IsSpellRequiringSpell(spellId, spellReq)) {
            LOG_ERROR("sql.sql",
                      "duplicated entry of req_spell {} and spell_id {} in "
                      "`spell_required`, skipped",
                      spellReq,
                      spellId);
            continue;
        }
        ...
        // xinef: fill additionalTalentInfo data, currently Blessing of
        // Sanctuary only
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Spell Required Records in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellLearnSkills()
{
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Spell Learn Skills From DBC in {} ms",
             dbc_count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT ID, EffectIndex, MapID, PositionX, PositionY, PositionZ, "
        "Orientation FROM spell_target_position");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell target coordinates. DB table "
                 "`spell_target_position` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!mapEntry) {
            LOG_ERROR("sql.sql",
                      "Spell (Id: {}, effIndex: {}) target map (ID: {}) does "
                      "not exist in `Map.dbc`.",
                      Spell_ID,
                      effIndex,
                      st.target_mapId);
            continue;
        ...
        if (st.target_X == 0 && st.target_Y == 0 && st.target_Z == 0) {
            LOG_ERROR(
                "sql.sql",
                "Spell (Id: {}, effIndex: {}) target coordinates not provided.",
                Spell_ID,
                effIndex);
            continue;
        }
        ...
        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "Spell (ID:{}) listed in `spell_target_position` does "
                      "not exist.",
                      Spell_ID);
            continue;
        }

        if (spellInfo->Effects[effIndex].TargetA.GetTarget() ==
                TARGET_DEST_DB ||
            spellInfo->Effects[effIndex].TargetB.GetTarget() ==
                TARGET_DEST_DB) {
            std::pair<uint32, SpellEffIndex> key =
                std::make_pair(Spell_ID, effIndex);
            mSpellTargetPositions[key] = st;
            ++count;
        }
        else {
            LOG_ERROR("sql.sql",
                      "Spell (Id: {}, effIndex: {}) listed in "
                      "`spell_target_position` does not have target "
                      "TARGET_DEST_DB (17).",
                      Spell_ID,
                      effIndex);
            continue;
    ...
    for (uint32 i = 1; i < GetSpellInfoStoreSize; ++i)
    {
        ...
        if (found)
        {
            if (!sSpellMgr->GetSpellTargetPosition(i))
                LOG_DEBUG("spells.aura", "Spell (ID: {}) does not have record in
    `spell_target_position`", i);
        }
    }*/

    LOG_INFO("server.loading",
             ">> Loaded {} Spell Teleport Coordinates in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellGroups()
{
    ...
    //                                                0     1            2
    QueryResult result = WorldDatabase.Query(
        "SELECT id, spell_id, special_flag FROM spell_group");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell group definitions. DB table `spell_group` "
                 "is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        SpellGroupSpecialFlags specialFlag =
            (SpellGroupSpecialFlags)fields[2].Get<uint32>();
        SpellInfo const* spellInfo = GetSpellInfo(spell_id);

        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_group` does not exist",
                      spell_id);
            continue;
        }
        ...
        if (mSpellGroupMap.find(spell_id) != mSpellGroupMap.end()) {
            LOG_ERROR(
                "sql.sql",
                "Spell {} listed in `spell_group` has more than one group",
                spell_id);
            continue;
        }
        ...
        if (specialFlag >= SPELL_GROUP_SPECIAL_FLAG_MAX) {
            LOG_ERROR(
                "sql.sql",
                "Spell {} listed in `spell_group` has invalid special flag!",
                spell_id);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Spell Group Definitions in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellGroupStackRules()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell group stack rules. DB table "
                 "`spell_group_stack_rules` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (stack_rule >= SPELL_GROUP_STACK_FLAG_MAX) {
            LOG_ERROR("sql.sql",
                      "SpellGroupStackRule {} listed in "
                      "`spell_group_stack_rules` does not exist",
                      stack_rule);
            continue;
        }
        ...
        if (!present) {
            LOG_ERROR("sql.sql",
                      "SpellGroup id {} listed in `spell_group_stack_rules` "
                      "does not exist",
                      group_id);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Spell Group Stack Rules in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "SpellFamilyMask1, SpellFamilyMask2, procFlags, procEx, procPhase, "
        "ppmRate, CustomChance, Cooldown FROM spell_proc_event");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell proc event conditions. DB table "
                 "`spell_proc_event` is empty.");
        return;
    }
    ...
    do {
        ...
        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_proc_event` does not exist",
                      spellId);
            continue;
        }
        ...
        if (allRanks) {
            if (!spellInfo->IsRanked())
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_proc_event` with all "
                          "ranks, but spell has no ranks.",
                          spellId);

            if (spellInfo->GetFirstRankSpell()->Id != uint32(spellId)) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_proc_event` is not first "
                          "rank of spell.",
                          spellId);
                continue;
            }
        ...
        while (spellInfo) {
            if (mSpellProcEventMap.find(spellInfo->Id) !=
                mSpellProcEventMap.end()) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_proc_event` already has "
                          "its first rank in table.",
                          spellInfo->Id);
                break;
            }
            ...
            if (!spellInfo->ProcFlags && !spellProcEvent.procFlags)
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_proc_event` probally not "
                          "triggered spell",
                          spellInfo->Id);
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Extra Spell Proc Event Conditions in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellProcs()
{
    ...
    //                                                 13        14
    QueryResult result = WorldDatabase.Query(
        "SELECT SpellId, SchoolMask, SpellFamilyName, SpellFamilyMask0, "
        "SpellFamilyMask1, SpellFamilyMask2, ProcFlags, SpellTypeMask, "
        "SpellPhaseMask, HitMask, AttributesMask, ProcsPerMinute, Chance, "
        "Cooldown, Charges FROM spell_proc");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 Spell Proc Conditions And Data. DB table "
                 "`spell_proc` Is Empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_proc` does not exist",
                      spellId);
            continue;
        }
        ...
        if (allRanks) {
            if (spellInfo->GetFirstRankSpell()->Id != uint32(spellId)) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_proc` is not first rank "
                          "of spell.",
                          fields[0].Get<int32>());
                continue;
            }
        ...
        while (spellInfo) {
            if (mSpellProcMap.find(spellInfo->Id) != mSpellProcMap.end()) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_proc` has duplicate entry "
                          "in the table",
                          spellId);
                break;
            }
            ...
            if (procEntry.SchoolMask & ~SPELL_SCHOOL_MASK_ALL)
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has wrong "
                          "`SchoolMask` set: {}",
                          spellId,
                          procEntry.SchoolMask);
            if (procEntry.SpellFamilyName && (procEntry.SpellFamilyName < 3 ||
                                              procEntry.SpellFamilyName > 17 ||
                                              procEntry.SpellFamilyName == 14 ||
                                              procEntry.SpellFamilyName == 16))
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has wrong "
                          "`SpellFamilyName` set: {}",
                          spellId,
            ...
            if (procEntry.Chance < 0) {
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has "
                          "negative value in `Chance` field",
                          spellId);
                procEntry.Chance = 0;
            }
            if (procEntry.ProcsPerMinute < 0) {
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has "
                          "negative value in `ProcsPerMinute` field",
                          spellId);
                procEntry.ProcsPerMinute = 0;
            }
            if (procEntry.Chance == 0 && procEntry.ProcsPerMinute == 0)
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} doesn't "
                          "have `Chance` and `ProcsPerMinute` values defined, "
                          "proc will not be triggered",
                          spellId);
            if (procEntry.Charges > 99) {
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has too big "
                          "value in `Charges` field",
                          spellId);
            ...
            if (!procEntry.ProcFlags)
                LOG_ERROR(
                    "sql.sql",
                    "`spell_proc` table entry for SpellId {} doesn't have "
            ...
            if (procEntry.SpellTypeMask &&
                ...
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has "
                          "`SpellTypeMask` value defined, but it won't be used "
                          "for defined `ProcFlags` value",
                          spellId);
            if (!procEntry.SpellPhaseMask &&
                procEntry.ProcFlags & REQ_SPELL_PHASE_PROC_FLAG_MASK)
                LOG_ERROR(
                    "sql.sql",
                    "`spell_proc` table entry for SpellId {} doesn't have "
                    "`SpellPhaseMask` value defined, but it's required for "
                    "defined `ProcFlags` value, proc will not be triggered",
                    spellId);
            if (procEntry.SpellPhaseMask & ~PROC_SPELL_PHASE_MASK_ALL)
                LOG_ERROR("sql.sql",
                          ...
                          "`SpellPhaseMask` set: {}",
                          spellId,
                          procEntry.SpellPhaseMask);
            if (procEntry.SpellPhaseMask &&
                !(procEntry.ProcFlags & REQ_SPELL_PHASE_PROC_FLAG_MASK))
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has "
                          "`SpellPhaseMask` value defined, but it won't be "
                          "used for defined `ProcFlags` value",
                          spellId);
            ...
            if (procEntry.HitMask &&
                !(procEntry.ProcFlags & TAKEN_HIT_PROC_FLAG_MASK ||
                  (procEntry.ProcFlags & DONE_HIT_PROC_FLAG_MASK &&
                   (!procEntry.SpellPhaseMask ||
                    procEntry.SpellPhaseMask &
                        (PROC_SPELL_PHASE_HIT | PROC_SPELL_PHASE_FINISH)))))
                LOG_ERROR("sql.sql",
                          "`spell_proc` table entry for SpellId {} has "
                          "`HitMask` value defined, but it won't be used for "
                          "defined `ProcFlags` and `SpellPhaseMask` values",
                          spellId);
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} spell proc conditions and data in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellBonuses()
{
    ...
    //                                                0      1             2 3 4
    QueryResult result =
        WorldDatabase.Query("SELECT entry, direct_bonus, dot_bonus, ap_bonus, "
                            "ap_dot_bonus FROM spell_bonus_data");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell bonus data. DB table `spell_bonus_data` is "
                 "empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!spell) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_bonus_data` does not exist",
                      entry);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Extra Spell Bonus Data in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellThreats()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 aggro generating spells. DB table `spell_threat` "
                 "is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!GetSpellInfo(entry)) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_threat` does not exist",
                      entry);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} SpellThreatEntries in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellMixology()
{
    ...
    //                                                0      1
    QueryResult result =
        WorldDatabase.Query("SELECT entry, pctMod FROM spell_mixology");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 mixology bonuses. DB table `spell_mixology` is "
                 "empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!GetSpellInfo(entry)) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_mixology` does not exist",
                      entry);
            continue;
        }
...
void SpellMgr::LoadSkillLineAbilityMap()
{
    ...
    for (uint32 i = 0; i < sSkillLineAbilityStore.GetNumRows(); ++i) {
        ...
        mSkillLineAbilityMap.insert(
            SkillLineAbilityMap::value_type(SkillInfo->Spell, SkillInfo));
        ++count;
    }
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell pet auras. DB table `spell_pet_auras` is "
                 "empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        SpellPetAuraMap::iterator itr =
            mSpellPetAuraMap.find((spell << 8) + eff);
        if (itr != mSpellPetAuraMap.end())
            ...
            if (!spellInfo) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_pet_auras` does not exist",
                          spell);
                continue;
            }
            if (spellInfo->Effects[eff].Effect != SPELL_EFFECT_DUMMY &&
                ...
                LOG_ERROR("spells",
                          "Spell {} listed in `spell_pet_auras` does not have "
                          "dummy aura or dummy effect",
                          spell);
                continue;
            }
            ...
            if (!spellInfo2) {
                LOG_ERROR("sql.sql",
                          "Aura {} listed in `spell_pet_auras` does not exist",
                          aura);
                continue;
            }
            ...
            PetAura pa(pet,
                       aura,
                       spellInfo->Effects[eff].TargetA.GetTarget() ==
                           TARGET_UNIT_PET,
...
void SpellMgr::LoadEnchantCustomAttr()
{
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
        ...
        // Xinef: commented second part, fixes warlock enchants like firestone,
        // spellstone
        ...
        for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
            if (spellInfo->Effects[j].Effect ==
                ...
                SpellItemEnchantmentEntry const* ench =
                    sSpellItemEnchantmentStore.LookupEntry(enchId);
                if (!ench)
                    continue;
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Custom Enchant Attributes in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellEnchantProcData()
{
    ...
    //                                                  3          4
    QueryResult result =
        WorldDatabase.Query("SELECT entry, customChance, PPMChance, procEx, "
                            "attributeMask FROM spell_enchant_proc_data");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell enchant proc event conditions. DB table "
                 "`spell_enchant_proc_data` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!ench) {
            LOG_ERROR("sql.sql",
                      "Enchancment {} listed in `spell_enchant_proc_data` does "
                      "not exist",
                      enchantId);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Enchant Proc Data Definitions in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellLinked()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 linked spells. DB table `spell_linked_spell` is "
                 "empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_linked_spell` does not exist",
                      std::abs(trigger));
            continue;
        }
...
void SpellMgr::LoadPetLevelupSpellMap()
{
        ...
        for (uint8 j = 0; j < 2; ++j) {
            ...
            for (SkillLineAbilityEntry const* skillLine :
                 GetSkillLineAbilitiesBySkillLine(
                     creatureFamily->skillLine[j])) {
                if (skillLine->AcquireMethod !=
                ...
                PetLevelupSpellSet& spellSet =
                    mPetLevelupSpellMap[creatureFamily->ID];
                if (spellSet.empty())
                    ++family_count;

                spellSet.insert(PetLevelupSpellSet::value_type(
                    spell->SpellLevel, spell->Id));
                ++count;
            }
        }
    }
    ...
    LOG_INFO(
        "server.loading",
        ">> Loaded {} Pet Levelup And Default Spells For {} Families in {} ms",
        count,
        family_count,
        GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
                                 ...
                                 PetDefaultSpellsEntry&  petDefSpells)
{
    ...
    if (PetLevelupSpellSet const* levelupSpells =
            cInfo->family ? sSpellMgr->GetPetLevelupSpellList(cInfo->family)
                          : nullptr) {
        for (uint8 j = 0; j < MAX_CREATURE_SPELL_DATA_SLOT; ++j) {
            if (!petDefSpells.spellid[j])
                continue;

            for (PetLevelupSpellSet::const_iterator itr =
                     levelupSpells->begin();
                 itr != levelupSpells->end();
                 ++itr) {
...
void SpellMgr::LoadPetDefaultSpells()
{
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
        ...
        CreatureSpellDataEntry const* spellDataEntry =
            sCreatureSpellDataStore.LookupEntry(itr->second.PetSpellDataId);
        if (!spellDataEntry)
            continue;
    ...
    LOG_INFO("server.loading",
             ">> Loaded Addition Spells For {} Pet Spell Data Entries in {} ms",
             countData,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
        ...
        for (uint8 k = 0; k < MAX_SPELL_EFFECTS; ++k) {
            if (spellInfo->Effects[k].Effect == SPELL_EFFECT_SUMMON ||
                spellInfo->Effects[k].Effect == SPELL_EFFECT_SUMMON_PET) {
                ...
                // for creature without PetSpellDataId get default pet spells
                // from creature_template
                int32 petSpellsId = cInfo->Entry;
                if (mPetDefaultSpellsMap.find(cInfo->Entry) !=
                    mPetDefaultSpellsMap.end())
                    continue;
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Summonable Creature emplates in {} ms",
             countCreature,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellAreas()
{
    ...
    //                                                  6          7       8 9
    QueryResult result = WorldDatabase.Query(
        "SELECT spell, area, quest_start, quest_start_status, "
        "quest_end_status, quest_end, aura_spell, racemask, gender, autocast "
        "FROM spell_area");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell area requirements. DB table `spell_area` "
                 "is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (SpellInfo const* spellInfo = GetSpellInfo(spell)) {
            if (spellArea.autocast)
                const_cast<SpellInfo*>(spellInfo)->Attributes |=
                    SPELL_ATTR0_NO_AURA_CANCEL;
        }
        else {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_area` does not exist",
                      spell);
            continue;
        }

        {
            ...
            if (!ok) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_area` already listed with "
        ...
        if (spellArea.areaId &&
            !sAreaTableStore.LookupEntry(spellArea.areaId)) {
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_area` have wrong area ({}) "
                      "requirement",
                      spell,
                      spellArea.areaId);
            continue;
        ...
        if (spellArea.questStart &&
            ...
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_area` have wrong start quest "
                      "({}) requirement",
                      spell,
        ...
        if (spellArea.questEnd) {
            if (!sObjectMgr->GetQuestTemplate(spellArea.questEnd)) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_area` have wrong end "
                          "quest ({}) requirement",
                          spell,
                          spellArea.questEnd);
                continue;
        ...
        if (spellArea.auraSpell) {
            SpellInfo const* spellInfo =
                GetSpellInfo(std::abs(spellArea.auraSpell));
            if (!spellInfo) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_area` have wrong aura "
                          "spell ({}) requirement",
                          spell,
                          std::abs(spellArea.auraSpell));
                continue;
            }

            if (uint32(std::abs(spellArea.auraSpell)) == spellArea.spellId) {
                LOG_ERROR("sql.sql",
                          "Spell {} listed in `spell_area` have aura spell "
        ...
        if (spellArea.raceMask &&
            ...
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_area` have wrong race mask "
                      "({}) requirement",
                      spell,
                      spellArea.raceMask);
            continue;
        ...
        if (spellArea.gender != GENDER_NONE &&
            ...
            LOG_ERROR("sql.sql",
                      "Spell {} listed in `spell_area` have wrong gender ({}) "
                      "requirement",
                      spell,
                      spellArea.gender);
            continue;
        ...
        if (spellArea.auraSpell)
            mSpellAreaForAuraMap.insert(SpellAreaForAuraMap::value_type(
                std::abs(spellArea.auraSpell), sa));
    ...
    if (sWorld->getIntConfig(CONFIG_ICC_BUFF_HORDE) > 0) {
        ...
        SpellArea spellAreaICCBuffHorde = {
            ...
            11,
            1};
        SpellArea const* saICCBuffHorde =
            &mSpellAreaMap
                 .insert(SpellAreaMap::value_type(
                     sWorld->getIntConfig(CONFIG_ICC_BUFF_HORDE),
                     spellAreaICCBuffHorde))
                 ->second;
        mSpellAreaForAreaMap.insert(
            SpellAreaForAreaMap::value_type(ICC_AREA, saICCBuffHorde));
        ++count;
    }
    else
    ...
    if (sWorld->getIntConfig(CONFIG_ICC_BUFF_ALLIANCE) > 0) {
        LOG_INFO("server.loading",
                 ">> Using ICC Buff Alliance: {}",
                 sWorld->getIntConfig(CONFIG_ICC_BUFF_ALLIANCE));
        SpellArea spellAreaICCBuffAlliance = {
            sWorld->getIntConfig(CONFIG_ICC_BUFF_ALLIANCE),
            ICC_AREA,
            0,
            0,
            ...
            11,
            1};
        SpellArea const* saICCBuffAlliance =
            &mSpellAreaMap
                 .insert(SpellAreaMap::value_type(
                     sWorld->getIntConfig(CONFIG_ICC_BUFF_ALLIANCE),
                     spellAreaICCBuffAlliance))
                 ->second;
        mSpellAreaForAreaMap.insert(
            SpellAreaForAreaMap::value_type(ICC_AREA, saICCBuffAlliance));
        ++count;
    }
    else
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Spell Area Requirements in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellInfoStore()
{
    ...
    for (uint32 spellIndex = 0; spellIndex < GetSpellInfoStoreSize();
         ++spellIndex) {
        if (!mSpellInfoMap[spellIndex])
            continue;

        for (SpellEffectInfo const& spellEffectInfo :
             mSpellInfoMap[spellIndex]->GetEffects()) {
            // ASSERT(effect.EffectIndex < MAX_SPELL_EFFECTS, "MAX_SPELL_EFFECTS
            // must be at least {}", effect.EffectIndex + 1);
            ASSERT(spellEffectInfo.Effect < TOTAL_SPELL_EFFECTS,
                   "TOTAL_SPELL_EFFECTS must be at least {}",
                   spellEffectInfo.Effect + 1);
            ASSERT(spellEffectInfo.ApplyAuraName < TOTAL_AURAS,
                   "TOTAL_AURAS must be at least {}",
                   spellEffectInfo.ApplyAuraName + 1);
            ASSERT(spellEffectInfo.TargetA.GetTarget() < TOTAL_SPELL_TARGETS,
                   "TOTAL_SPELL_TARGETS must be at least {}",
                   spellEffectInfo.TargetA.GetTarget() + 1);
            ASSERT(spellEffectInfo.TargetB.GetTarget() < TOTAL_SPELL_TARGETS,
                   "TOTAL_SPELL_TARGETS must be at least {}",
                   spellEffectInfo.TargetB.GetTarget() + 1);
        }
    }

    LOG_INFO("server.loading",
             ">> Loaded Spell Custom Attributes in {} ms",
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellCooldownOverrides()
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "StartRecoveryCategory FROM spell_cooldown_overrides");
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Spell Cooldown Overrides entries in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
{
    return mSpellCooldownOverrideMap.find(spellId) !=
           mSpellCooldownOverrideMap.end();
}
...
void SpellMgr::LoadSpellSpecificAndAuraState()
{
    ...
    LOG_INFO("server.loading",
             ">> Loaded Spell Specific And Aura State in {} ms",
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void SpellMgr::LoadSpellInfoCustomAttributes()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT spell_id, attributes FROM spell_custom_attr");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 Spell Custom Attributes From DB. DB table "
                 "`spell_custom_attr` Is Empty.");
    }
    else {
        for (count = 0; result->NextRow(); ++count) {
            ...
            if (!spellInfo) {
                LOG_INFO("sql.sql",
                         "Table `spell_custom_attr` has wrong spell (spell_id: "
                         "{}), ignored.",
                         spellId);
                continue;
            }
            ...
            if (attributes & SPELL_ATTR0_CU_NEGATIVE) {
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
                    if (spellInfo->Effects[i].IsEffect()) {
                        if ((attributes &
                             (SPELL_ATTR0_CU_NEGATIVE_EFF0 << i)) &&
                            (attributes &
                             (SPELL_ATTR0_CU_POSITIVE_EFF0 << i))) {
                            LOG_ERROR(
                                "sql.sql",
                                "Table `spell_custom_attr` has attribute "
                                "SPELL_ATTR0_CU_NEGATIVE_EFF{} and "
                                "SPELL_ATTR0_CU_POSITIVE_EFF{} attributes for "
                                "spell {} which cannot stack together. "
                                "Attributes will not get applied",
                    ...
                    if (attributes & (SPELL_ATTR0_CU_NEGATIVE_EFF0 << i)) {
                        LOG_ERROR("sql.sql",
                                  "Table `spell_custom_attr` has attribute "
                                  "SPELL_ATTR0_CU_NEGATIVE_EFF{} for spell {} "
                                  "with no EFFECT_{}",
                                  static_cast<uint32>(i),
                                  spellId,
                                  static_cast<uint32>(i));
            ...
            if (attributes & SPELL_ATTR0_CU_POSITIVE) {
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
                    ...
                    if ((attributes & (SPELL_ATTR0_CU_POSITIVE_EFF0 << i))) {
                        LOG_ERROR("sql.sql",
                                  "Table `spell_custom_attr` has attribute "
                                  "SPELL_ATTR0_CU_POSITIVE_EFF{} for spell {} "
                                  "with no EFFECT_{}",
                                  uint32(i),
                                  spellId,
                                  uint32(i));
            ...
            if ((attributes & SPELL_ATTR0_CU_FORCE_AURA_SAVING) &&
                (attributes & SPELL_ATTR0_CU_AURA_CANNOT_BE_SAVED)) {
                LOG_ERROR(
                    "sql.sql",
                    "Table `spell_custom_attr` attribute1 field has attributes "
                    "SPELL_ATTR1_CU_FORCE_AURA_SAVING and "
                    "SPELL_ATTR0_CU_AURA_CANNOT_BE_SAVED which cannot stack "
                    "for spell {}. Both attributes will be ignored.",
                    spellId);
                attributes &= ~(SPELL_ATTR0_CU_FORCE_AURA_SAVING |
        ...
        LOG_INFO("server.loading",
                 ">> Loaded {} spell custom attributes from DB in {} ms",
                 count,
                 GetMSTimeDiffToNow(customAttrTime));
    }
        ...
        for (uint8 j = 0; j < MAX_TALENT_RANK; j++)
            if (uint32 spellId = talentInfo->RankID[j])
                if (SpellInfo const* spellInfo = GetSpellInfo(spellId))
                    for (uint8 k = 0; k < MAX_SPELL_EFFECTS; ++k)
                        if (spellInfo->Effects[k].Effect ==
                            SPELL_EFFECT_LEARN_SPELL)
                            if (SpellInfo const* learnSpell = GetSpellInfo(
                                    spellInfo->Effects[k].TriggerSpell))
                                if (learnSpell->IsRanked() &&
                                    !learnSpell->HasAttribute(
                                        SpellAttr0(SPELL_ATTR0_PASSIVE |
                                                   SPELL_ATTR0_DO_NOT_DISPLAY)))
                                    mTalentSpellAdditionalSet.insert(
                                        learnSpell->Id);
    }
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
            ...
            switch (spellInfo->Effects[j].ApplyAuraName) {
            case SPELL_AURA_CONVERT_RUNE: // Can't be saved - aura handler
                                          // relies on calculated amount and
                                          // changes it
            case SPELL_AURA_OPEN_STABLE:  // No point in saving this, since the
                                          // stable dialog can't be open on aura
                                          // load anyway.
            // Auras that require both caster & target to be in world cannot be
            // saved
            ...
            case SPELL_EFFECT_ENCHANT_HELD_ITEM: {
                ...
                if (IsPartOfSkillLine(SKILL_ENCHANTING, i)) {
                    ...
                    SpellItemEnchantmentEntry const* enchant =
                        sSpellItemEnchantmentStore.LookupEntry(enchantId);
                    for (uint8 s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS;
                         ++s) {
                        if (enchant->type[s] !=
                        ...
                        SpellInfo* procInfo =
                            _GetSpellInfo(enchant->spellid[s]);
                        if (!procInfo)
                            continue;

                        // if proced directly from enchantment, not via proc
                        // aura NOTE: Enchant Weapon - Blade Ward also has proc
                        // aura spell and is proced directly however its not
                        // expected to stack so this check is good
            ...
            for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
                if (spellInfo->Effects[j].Effect) {
                    ...
                    case SPELL_EFFECT_APPLY_AREA_AURA_OWNER:
                        if (spellInfo->Effects[j].ApplyAuraName ==
                                SPELL_AURA_PERIODIC_DAMAGE ||
                            spellInfo->Effects[j].ApplyAuraName ==
                            ...
                            continue;
                        [[fallthrough]]; /// @todo: Not sure whether the
                                         /// fallthrough was a mistake
                                         /// (forgetting a break) or intended.
        ...
        // ONLY SPELLS WITH SPELLFAMILY_GENERIC and EFFECT_SCHOOL_DAMAGE, OR
        // WEAPON_DMG_X
        ...
        case 65775: // Anub'arak, Swarm Scarab, Acid-Drenched Mandibles (10
                    // normal)
        case 67861: // Anub'arak, Swarm Scarab, Acid-Drenched Mandibles (25
                    // normal)
        case 67862: // Anub'arak, Swarm Scarab, Acid-Drenched Mandibles (10
                    // heroic)
        case 67863: // Anub'arak, Swarm Scarab, Acid-Drenched Mandibles (25
                    // heroic)
        ...
        case 72465: // Icecrown Citadel, Sindragosa, Respite for a Tormented
                    // Soul (weekly quest)
        ...
        // Xinef: NOT CUSTOM, cant add in DBC CORRECTION because i need to swap
        // effects, too much work to do there Envenom
        case 32645:
        case 32684:
        case 57992:
        case 57993: {
        ...
        if (spellInfo->Speed > 0.0f) {
            if (SpellVisualEntry const* spellVisual =
                    sSpellVisualStore.LookupEntry(spellInfo->SpellVisual[0])) {
                if (spellVisual->HasMissile) {
                    if (spellVisual->MissileModel == -4 ||
                        spellVisual->MissileModel == -5) {
                        spellInfo->AttributesCu |=
        ...
        if (sSpellMgr->HasSpellCooldownOverride(spellInfo->Id)) {
            SpellCooldownOverride spellOverride =
                sSpellMgr->GetSpellCooldownOverride(spellInfo->Id);
            ...
            if (spellInfo->CategoryRecoveryTime !=
                spellOverride.CategoryRecoveryTime) {
                spellInfo->CategoryRecoveryTime =
                    spellOverride.CategoryRecoveryTime;
            }

            if (spellInfo->StartRecoveryTime !=
                spellOverride.StartRecoveryTime) {
                spellInfo->RecoveryTime = spellOverride.RecoveryTime;
            }

            if (spellInfo->StartRecoveryCategory !=
                spellOverride.StartRecoveryCategory) {
                spellInfo->RecoveryTime = spellOverride.RecoveryTime;
            }
        }
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
            if (spellInfo->Effects[j].ApplyAuraName &&
                spellInfo->Effects[j].TriggerSpell) {
                switch (spellInfo->Effects[j].ApplyAuraName) {
                case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
                case SPELL_AURA_PERIODIC_TRIGGER_SPELL_FROM_CLIENT:
                case SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE:
                    if (SpellInfo const* triggerSpell = sSpellMgr->GetSpellInfo(
                            spellInfo->Effects[j].TriggerSpell)) {
                        overrideAttr = true;
                        if (triggerSpell->AttributesCu &
                            SPELL_ATTR0_CU_BINARY_SPELL)
                            allNonBinary = false;
                    }
                }
    ...
    LOG_INFO("server.loading",
             ">> Loaded SpellInfo Custom Attributes in {} ms",
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
