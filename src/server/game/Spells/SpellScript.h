...
// helper class from which SpellScript and SpellAura derive, use these classes
// instead
class _SpellScript {
...
public:
    _SpellScript()
        : m_currentScriptState(SPELL_SCRIPT_STATE_NONE), m_scriptName(nullptr),
          m_scriptSpellId(0)
    {
    }
...
protected:
    ...
    public:
        ...
        virtual bool CheckEffect(SpellInfo const* spellInfo,
                                 uint8            effIndex) = 0;
...
#define SPELLSCRIPT_FUNCTION_TYPE_DEFINES(CLASSNAME)                           \
    ...
    class EffectHandler : public _SpellScript::EffectNameCheck,
                          public _SpellScript::EffectHook {
    public:
        EffectHandler(SpellEffectFnType _pEffectHandlerScript,
                      uint8             _effIndex,
                      uint16            _effName);
        std::string ToString();
    ...
    public:
        ObjectAreaTargetSelectHandler(SpellObjectAreaTargetSelectFnType
...
#define SPELLSCRIPT_FUNCTION_CAST_DEFINES(CLASSNAME)                           \
    ...
    class EffectHandlerFunction : public SpellScript::EffectHandler {          \
    public:                                                                    \
        EffectHandlerFunction(SpellEffectFnType _pEffectHandlerScript,         \
                              uint8             _effIndex,                     \
                              uint16            _effName)                      \
            : SpellScript::EffectHandler(                                      \
                  (SpellScript::SpellEffectFnType)_pEffectHandlerScript,       \
                  _effIndex,                                                   \
                  _effName)                                                    \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class BeforeHitHandlerFunction : public SpellScript::BeforeHitHandler {    \
    public:                                                                    \
        explicit BeforeHitHandlerFunction(                                     \
            SpellBeforeHitFnType pBeforeHitHandlerScript)                      \
            : SpellScript::BeforeHitHandler(                                   \
                  (SpellScript::SpellBeforeHitFnType)pBeforeHitHandlerScript)  \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    ...
    public:                                                                    \
        ObjectAreaTargetSelectHandlerFunction(                                 \
            SpellObjectAreaTargetSelectFnType                                  \
                   _pObjectAreaTargetSelectHandlerScript,                      \
            uint8  _effIndex,                                                  \
            uint16 _targetType)                                                \
            : SpellScript::ObjectAreaTargetSelectHandler(                      \
                  (SpellScript::SpellObjectAreaTargetSelectFnType)             \
                      _pObjectAreaTargetSelectHandlerScript,                   \
                  _effIndex,                                                   \
                  _targetType)                                                 \
        {                                                                      \
    ...
    class ObjectTargetSelectHandlerFunction                                    \
        : public SpellScript::ObjectTargetSelectHandler {                      \
    public:                                                                    \
        ObjectTargetSelectHandlerFunction(                                     \
            ...
            : SpellScript::ObjectTargetSelectHandler(                          \
                  (SpellScript::SpellObjectTargetSelectFnType)                 \
                      _pObjectTargetSelectHandlerScript,                       \
                  _effIndex,                                                   \
                  _targetType)                                                 \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class DestinationTargetSelectHandlerFunction                               \
        : public SpellScript::DestinationTargetSelectHandler {                 \
    public:                                                                    \
        DestinationTargetSelectHandlerFunction(                                \
            SpellDestinationTargetSelectFnType                                 \
                   _DestinationTargetSelectHandlerScript,                      \
            uint8  _effIndex,                                                  \
            uint16 _targetType)                                                \
            : SpellScript::DestinationTargetSelectHandler(                     \
                  (SpellScript::SpellDestinationTargetSelectFnType)            \
                      _DestinationTargetSelectHandlerScript,                   \
                  _effIndex,                                                   \
                  _targetType)                                                 \
        {                                                                      \
...
#define SpellCheckCastFn(F) CheckCastHandlerFunction(&F)
    ...
    // example: OnEffect**** += SpellEffectFn(class::function,
    // EffectIndexSpecifier, EffectNameSpecifier); where function is void
    // function(SpellEffIndex effIndex)
...
#define SpellHitFn(F) HitHandlerFunction(&F)
    ...
    // example: OnObjectAreaTargetSelect +=
    // SpellObjectAreaTargetSelectFn(class::function, EffectIndexSpecifier,
    // TargetsNameSpecifier); where function is void
    // function(std::list<WorldObject*>& targets)
    HookList<ObjectAreaTargetSelectHandler> OnObjectAreaTargetSelect;
#define SpellObjectAreaTargetSelectFn(F, I, N)                                 \
    ObjectAreaTargetSelectHandlerFunction(&F, I, N)

    // example: OnObjectTargetSelect +=
    // SpellObjectTargetSelectFn(class::function, EffectIndexSpecifier,
    // TargetsNameSpecifier); where function is void function(WorldObject*&
    // target)
    HookList<ObjectTargetSelectHandler> OnObjectTargetSelect;
#define SpellObjectTargetSelectFn(F, I, N)                                     \
    ObjectTargetSelectHandlerFunction(&F, I, N)

    // example: OnDestinationTargetSelect +=
    // SpellDestinationTargetSelectFn(class::function, EffectIndexSpecifier,
    // TargetsNameSpecifier); where function is void function(SpellDestination&
    // target)
...
#define AURASCRIPT_FUNCTION_TYPE_DEFINES(CLASSNAME)                            \
    ...
    typedef void (CLASSNAME::*AuraEffectApplicationModeFnType)(                \
    ...
    typedef void (CLASSNAME::*AuraEffectCalcAmountFnType)(                     \
        AuraEffect const*, int32&, bool&);                                     \
    typedef void (CLASSNAME::*AuraEffectCalcPeriodicFnType)(                   \
        AuraEffect const*, bool&, int32&);                                     \
    typedef void (CLASSNAME::*AuraEffectCalcSpellModFnType)(AuraEffect const*, \
                                                            SpellModifier*&);  \
    typedef void (CLASSNAME::*AuraEffectAbsorbFnType)(                         \
        AuraEffect*, DamageInfo&, uint32&);                                    \
    typedef void (CLASSNAME::*AuraEffectSplitFnType)(                          \
        AuraEffect*, DamageInfo&, uint32&);                                    \
    ...
    class EffectBase : public _SpellScript::EffectAuraNameCheck,
                       public _SpellScript::EffectHook {
    public:
    ...
    public:
        EffectPeriodicHandler(AuraEffectPeriodicFnType _pEffectHandlerScript,
                              uint8                    _effIndex,
                              uint16                   _effName);
    ...
    public:
        EffectAbsorbHandler(AuraEffectAbsorbFnType _pEffectHandlerScript,
                            uint8                  _effIndex);
        void Call(AuraScript* auraScript,
                  AuraEffect* aurEff,
                  DamageInfo& dmgInfo,
    ...
    public:
        EffectManaShieldHandler(AuraEffectAbsorbFnType _pEffectHandlerScript,
                                uint8                  _effIndex);
        void Call(AuraScript* auraScript,
                  AuraEffect* aurEff,
                  DamageInfo& dmgInfo,
    ...
    public:
        EffectSplitHandler(AuraEffectSplitFnType _pEffectHandlerScript,
                           uint8                 _effIndex);
        void Call(AuraScript* auraScript,
                  AuraEffect* aurEff,
                  DamageInfo& dmgInfo,
    ...
    public:
        EffectProcHandler(AuraEffectProcFnType effectHandlerScript,
                          uint8                effIndex,
                          uint16               effName);
        void Call(AuraScript*       auraScript,
                  AuraEffect const* aurEff,
...
#define AURASCRIPT_FUNCTION_CAST_DEFINES(CLASSNAME)                            \
    class CheckAreaTargetFunction                                              \
        : public AuraScript::CheckAreaTargetHandler {                          \
    public:                                                                    \
        CheckAreaTargetFunction(AuraCheckAreaTargetFnType _pHandlerScript)     \
            : AuraScript::CheckAreaTargetHandler(                              \
                  (AuraScript::AuraCheckAreaTargetFnType)_pHandlerScript)      \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class AuraDispelFunction : public AuraScript::AuraDispelHandler {          \
    public:                                                                    \
        AuraDispelFunction(AuraDispelFnType _pHandlerScript)                   \
            : AuraScript::AuraDispelHandler(                                   \
                  (AuraScript::AuraDispelFnType)_pHandlerScript)               \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    ...
    public:                                                                    \
        EffectPeriodicHandlerFunction(                                         \
            AuraEffectPeriodicFnType _pEffectHandlerScript,                    \
            uint8                    _effIndex,                                \
            uint16                   _effName)                                 \
            : AuraScript::EffectPeriodicHandler(                               \
                  (AuraScript::AuraEffectPeriodicFnType)_pEffectHandlerScript, \
                  _effIndex,                                                   \
                  _effName)                                                    \
        {                                                                      \
        }                                                                      \
    ...
    public:                                                                    \
        EffectUpdatePeriodicHandlerFunction(                                   \
            AuraEffectUpdatePeriodicFnType _pEffectHandlerScript,              \
            uint8                          _effIndex,                          \
            uint16                         _effName)                           \
            : AuraScript::EffectUpdatePeriodicHandler(                         \
                  (AuraScript::AuraEffectUpdatePeriodicFnType)                 \
    ...
    public:                                                                    \
        EffectCalcAmountHandlerFunction(                                       \
            AuraEffectCalcAmountFnType _pEffectHandlerScript,                  \
            uint8                      _effIndex,                              \
            uint16                     _effName)                               \
            : AuraScript::EffectCalcAmountHandler(                             \
                  (AuraScript::AuraEffectCalcAmountFnType)                     \
                      _pEffectHandlerScript,                                   \
                  _effIndex,                                                   \
                  _effName)                                                    \
        {                                                                      \
    ...
    public:                                                                    \
        EffectCalcPeriodicHandlerFunction(                                     \
            AuraEffectCalcPeriodicFnType _pEffectHandlerScript,                \
            uint8                        _effIndex,                            \
            uint16                       _effName)                             \
            : AuraScript::EffectCalcPeriodicHandler(                           \
                  (AuraScript::AuraEffectCalcPeriodicFnType)                   \
                      _pEffectHandlerScript,                                   \
                  _effIndex,                                                   \
                  _effName)                                                    \
        {                                                                      \
    ...
    public:                                                                    \
        EffectCalcSpellModHandlerFunction(                                     \
            AuraEffectCalcSpellModFnType _pEffectHandlerScript,                \
            uint8                        _effIndex,                            \
            uint16                       _effName)                             \
            : AuraScript::EffectCalcSpellModHandler(                           \
                  (AuraScript::AuraEffectCalcSpellModFnType)                   \
                      _pEffectHandlerScript,                                   \
                  _effIndex,                                                   \
                  _effName)                                                    \
        {                                                                      \
    ...
    class EffectApplyHandlerFunction : public AuraScript::EffectApplyHandler { \
    public:                                                                    \
        EffectApplyHandlerFunction(                                            \
            ...
            uint16                          _effName,                          \
            AuraEffectHandleModes           _mode)                             \
            : AuraScript::EffectApplyHandler(                                  \
                  (AuraScript::AuraEffectApplicationModeFnType)                \
                      _pEffectHandlerScript,                                   \
                  _effIndex,                                                   \
                  _effName,                                                    \
                  _mode)                                                       \
    ...
    class EffectAbsorbFunction : public AuraScript::EffectAbsorbHandler {      \
    public:                                                                    \
        EffectAbsorbFunction(AuraEffectAbsorbFnType _pEffectHandlerScript,     \
                             uint8                  _effIndex)                 \
            : AuraScript::EffectAbsorbHandler(                                 \
                  (AuraScript::AuraEffectAbsorbFnType)_pEffectHandlerScript,   \
                  _effIndex)                                                   \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class EffectManaShieldFunction                                             \
        : public AuraScript::EffectManaShieldHandler {                         \
    public:                                                                    \
        EffectManaShieldFunction(AuraEffectAbsorbFnType _pEffectHandlerScript, \
                                 uint8                  _effIndex)             \
    ...
    class EffectSplitFunction : public AuraScript::EffectSplitHandler {        \
    public:                                                                    \
        EffectSplitFunction(AuraEffectSplitFnType _pEffectHandlerScript,       \
                            uint8                 _effIndex)                   \
            : AuraScript::EffectSplitHandler(                                  \
                  (AuraScript::AuraEffectSplitFnType)_pEffectHandlerScript,    \
                  _effIndex)                                                   \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class CheckProcHandlerFunction : public AuraScript::CheckProcHandler {     \
    public:                                                                    \
        CheckProcHandlerFunction(AuraCheckProcFnType handlerScript)            \
            : AuraScript::CheckProcHandler(                                    \
                  (AuraScript::AuraCheckProcFnType)handlerScript)              \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class AfterCheckProcHandlerFunction                                        \
        : public AuraScript::AfterCheckProcHandler {                           \
    public:                                                                    \
    ...
    class AuraProcHandlerFunction : public AuraScript::AuraProcHandler {       \
    public:                                                                    \
        AuraProcHandlerFunction(AuraProcFnType handlerScript)                  \
            : AuraScript::AuraProcHandler(                                     \
                  (AuraScript::AuraProcFnType)handlerScript)                   \
        {                                                                      \
        }                                                                      \
    };                                                                         \
    class EffectProcHandlerFunction : public AuraScript::EffectProcHandler {   \
    public:                                                                    \
        EffectProcHandlerFunction(AuraEffectProcFnType effectHandlerScript,    \
                                  uint8                effIndex,               \
                                  uint16               effName)                \
            : AuraScript::EffectProcHandler(                                   \
                  (AuraScript::AuraEffectProcFnType)effectHandlerScript,       \
                  effIndex,                                                    \
                  effName)                                                     \
        {                                                                      \
        }                                                                      \
...
#define PrepareAuraScript(CLASSNAME)                                           \
    AURASCRIPT_FUNCTION_TYPE_DEFINES(CLASSNAME)                                \
    AURASCRIPT_FUNCTION_CAST_DEFINES(CLASSNAME)

public:
    AuraScript()
        : _SpellScript(), m_aura(nullptr), m_auraApplication(nullptr),
...
#define AuraDispelFn(F) AuraDispelFunction(&F)
    ...
    // be unsafe example: OnEffectApply += AuraEffectApplyFn(class::function,
    // EffectIndexSpecifier, EffectAuraNameSpecifier, AuraEffectHandleModes);
    ...
    // example: AfterEffectApply += AuraEffectApplyFn(class::function,
    // EffectIndexSpecifier, EffectAuraNameSpecifier, AuraEffectHandleModes);
    // where function is: void function (AuraEffect const* aurEff,
    // AuraEffectHandleModes mode);
...
#define AuraEffectApplyFn(F, I, N, M) EffectApplyHandlerFunction(&F, I, N, M)
    ...
    // should be used when when effect handler preventing/replacing is needed,
    // do not use this hook for triggering spellcasts/removing auras etc - may
    // be unsafe example: OnEffectRemove += AuraEffectRemoveFn(class::function,
    // EffectIndexSpecifier, EffectAuraNameSpecifier, AuraEffectHandleModes);
    // where function is: void function (AuraEffect const* aurEff,
    // AuraEffectHandleModes mode);
    HookList<EffectApplyHandler> OnEffectRemove;
    // executed when aura effect is removed with specified mode from target
    // example: AfterEffectRemove += AuraEffectRemoveFn(class::function,
    // EffectIndexSpecifier, EffectAuraNameSpecifier, AuraEffectHandleModes);
    // where function is: void function (AuraEffect const* aurEff,
    // AuraEffectHandleModes mode);
...
#define AuraEffectRemoveFn(F, I, N, M) EffectApplyHandlerFunction(&F, I, N, M)
    ...
    // example: OnEffectPeriodic += AuraEffectPeriodicFn(class::function,
...
#define AuraEffectPeriodicFn(F, I, N) EffectPeriodicHandlerFunction(&F, I, N)
    ...
    // example: OnEffectUpdatePeriodic +=
    // AuraEffectUpdatePeriodicFn(class::function, EffectIndexSpecifier,
    // EffectAuraNameSpecifier); where function is: void function (AuraEffect*
    // aurEff);
...
#define AuraEffectUpdatePeriodicFn(F, I, N)                                    \
    ...
    // example: DoEffectCalcAmount += AuraEffectCalcAmounFn(class::function,
    // EffectIndexSpecifier, EffectAuraNameSpecifier); where function is: void
...
#define AuraEffectCalcAmountFn(F, I, N)                                        \
    ...
    // example: DoEffectCalcPeriodic +=
    // AuraEffectCalcPeriodicFn(class::function, EffectIndexSpecifier,
    // EffectAuraNameSpecifier); where function is: void function (AuraEffect
    // const* aurEff, bool& isPeriodic, int32& amplitude);
...
#define AuraEffectCalcPeriodicFn(F, I, N)                                      \
    ...
    // example: DoEffectCalcSpellMod +=
    // AuraEffectCalcSpellModFn(class::function, EffectIndexSpecifier,
    // EffectAuraNameSpecifier); where function is: void function (AuraEffect
    // const* aurEff, SpellModifier*& spellMod);
    HookList<EffectCalcSpellModHandler> DoEffectCalcSpellMod;
#define AuraEffectCalcSpellModFn(F, I, N)                                      \
    EffectCalcSpellModHandlerFunction(&F, I, N)

    // executed when absorb aura effect is going to reduce damage
    // example: OnEffectAbsorb += AuraEffectAbsorbFn(class::function,
...
#define AuraEffectAbsorbFn(F, I) EffectAbsorbFunction(&F, I)
    ...
    // is real amount absorbed by aura example: AfterEffectAbsorb +=
    // AuraEffectAbsorbFn(class::function, EffectIndexSpecifier); where function
    // is: void function (AuraEffect* aurEff, DamageInfo& dmgInfo, uint32&
    // absorbAmount);
    HookList<EffectAbsorbHandler> AfterEffectAbsorb;

    // executed when mana shield aura effect is going to reduce damage
    // example: OnEffectManaShield += AuraEffectAbsorbFn(class::function,
    // EffectIndexSpecifier); where function is: void function (AuraEffect*
...
#define AuraEffectManaShieldFn(F, I) EffectManaShieldFunction(&F, I)
    ...
    // AfterEffectManaShield += AuraEffectAbsorbFn(class::function,
    // EffectIndexSpecifier); where function is: void function (AuraEffect*
    // aurEff, DamageInfo& dmgInfo, uint32& absorbAmount);
    ...
    // through it example: OnEffectSplit += AuraEffectSplitFn(class::function,
    // EffectIndexSpecifier); where function is: void function (AuraEffect*
...
#define AuraProcFn(F) AuraProcHandlerFunction(&F)
    ...
    // example: AfterEffectProc += AuraEffectProcFn(class::function,
    // EffectIndexSpecifier, EffectAuraNameSpecifier); where function is: void
    // function (AuraEffect const* aurEff, ProcEventInfo& procInfo);
...
#define AuraEffectProcFn(F, I, N) EffectProcHandlerFunction(&F, I, N)
    ...
    // returns guid of object which casted the aura (m_originalCaster of the
    // Spell class)
    ObjectGuid GetCasterGUID() const;
    // returns unit which casted the aura or nullptr if not avalible (caster
    // logged out for example)
    Unit* GetCaster() const;
    // returns object on which aura was casted, target for non-area auras, area
