...
enum SpellCastFlags {
    ...
    CAST_FLAG_NO_GCD = 0x00040000, // no GCD for spell casts from charm/summon
                                   // (vehicle spells is an example)
    ...
    CAST_FLAG_HEAL_PREDICTION =
        0x40000000, //@todo: Unused on TC 3.3.5a. Defined from TC Master.
...
struct TriggeredByAuraSpellData {
    TriggeredByAuraSpellData()
        : spellInfo(nullptr), effectIndex(-1), tickNumber(0)
    {
    }
...
public:
    ...
    void SelectEffectImplicitTargets(SpellEffIndex                  effIndex,
                                     SpellImplicitTargetInfo const& targetType,
                                     uint32& processedEffectMask);
    void
         SelectImplicitChannelTargets(SpellEffIndex                  effIndex,
                                      SpellImplicitTargetInfo const& targetType);
    void SelectImplicitNearbyTargets(SpellEffIndex                  effIndex,
                                     SpellImplicitTargetInfo const& targetType,
                                     uint32                         effMask);
    void SelectImplicitConeTargets(SpellEffIndex                  effIndex,
                                   SpellImplicitTargetInfo const& targetType,
                                   uint32                         effMask);
    void SelectImplicitAreaTargets(SpellEffIndex                  effIndex,
                                   SpellImplicitTargetInfo const& targetType,
                                   uint32                         effMask);
    void
    SelectImplicitCasterDestTargets(SpellEffIndex                  effIndex,
                                    SpellImplicitTargetInfo const& targetType);
    void
    SelectImplicitTargetDestTargets(SpellEffIndex                  effIndex,
                                    SpellImplicitTargetInfo const& targetType);
    void
         SelectImplicitDestDestTargets(SpellEffIndex                  effIndex,
                                       SpellImplicitTargetInfo const& targetType);
    void SelectImplicitCasterObjectTargets(
        SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType);
    void SelectImplicitTargetObjectTargets(
        SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType);
    void SelectImplicitChainTargets(SpellEffIndex                  effIndex,
                                    SpellImplicitTargetInfo const& targetType,
                                    WorldObject*                   target,
                                    uint32                         effMask);
    ...
    WorldObject* SearchNearbyTarget(float                  range,
                                    SpellTargetObjectTypes objectType,
                                    SpellTargetCheckTypes  selectionType,
                                    ConditionList*         condList = nullptr);
    void         SearchAreaTargets(std::list<WorldObject*>& targets,
                                   float                    range,
    ...
    int32 CalculateSpellDamage(uint8 i, Unit const* target) const
    {
        return m_caster->CalculateSpellDamage(
            target, m_spellInfo, i, &m_spellValue->EffectBasePoints[i]);
    }
    ...
    static void WriteCastResultInfo(WorldPacket&      data,
                                    Player*           caster,
                                    SpellInfo const*  spellInfo,
                                    uint8             castCount,
    ...
    void SendLogExecute();
    void ExecuteLogEffectTakeTargetPower(uint8  effIndex,
                                         Unit*  target,
                                         uint32 PowerType,
                                         uint32 powerTaken,
                                         float  gainMultiplier);
    void
    ExecuteLogEffectExtraAttacks(uint8 effIndex, Unit* victim, uint32 attCount);
    void
    ExecuteLogEffectInterruptCast(uint8 effIndex, Unit* victim, uint32 spellId);
    void ExecuteLogEffectDurabilityDamage(uint8 effIndex,
                                          Unit* victim,
                                          int32 itemId,
                                          int32 slot);
    ...
    void HandleEffects(Unit*                 pUnitTarget,
                       Item*                 pItemTarget,
    ...
    {
        return HasTriggeredCastFlag(TRIGGERED_FULL_MASK);
    };
    bool HasTriggeredCastFlag(TriggerCastFlags flag) const
    {
        return _triggeredCastFlags & flag;
    };
    ...
    bool UpdatePointers(); // must be used at call Spell code after time delay
    ...
    [[nodiscard]] uint32 GetTriggeredByAuraTickNumber() const
    {
        return m_triggeredByAuraSpell.tickNumber;
    }

    [[nodiscard]] TriggerCastFlags GetTriggeredCastFlags() const
    {
        return _triggeredCastFlags;
    }

    [[nodiscard]] SpellSchoolMask GetSpellSchoolMask() const
    {
        return m_spellSchoolMask;
    }
...
protected:
    ...
    // e.g. damage around area spell trigered by victim aura and damage enemies
    // of aura caster
    ...
    int32 m_powerCost;         // Calculated spell cost     initialized only in
                               // Spell::prepare
    int32 m_casttime;          // Calculated spell cast time initialized only in
                               // Spell::prepare
    int32 m_channeledDuration; // Calculated channeled spell duration in order
                               // to calculate correct pushback.
    bool m_canReflect;         // can reflect this spell?

    uint8 m_spellFlags; // for spells whose target was changed in cast i.e. due
                        // to reflect
    ...
    void AddUnitTarget(Unit*  target,
                       uint32 effectMask,
    ...
    SpellMissInfo
         DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura);
    ...
    void CallScriptDestinationTargetSelectHandlers(
        ...
        SpellEffIndex                  effIndex,
        SpellImplicitTargetInfo const& targetType);
    bool                    CheckScriptEffectImplicitTargets(uint32 effIndex,
    ...
    bool
                                       CanExecuteTriggersOnHit(uint8            effMask,
                                                               SpellInfo const* triggeredByAura = nullptr) const;
    ...
    void SummonGuardian(uint32                       i,
                        ...
                        bool                         personalSpawn);
    void
    CalculateJumpSpeeds(uint8 i, float dist, float& speedxy, float& speedz);

    SpellCastResult CanOpenLock(uint32     effIndex,
                                uint32     lockid,
                                SkillType& skillid,
                                int32&     reqSkillValue,
                                int32&     skillValue);
    ...
    {
        return int32(m_caster->GetMap()->mtRand.randInt(max - min)) + min;
    }
    uint32 urand(uint32 min, uint32 max)
    {
        return m_caster->GetMap()->mtRand.randInt(max - min) + min;
    }
...
struct WorldObjectSpellTargetCheck {
    ...
    WorldObjectSpellTargetCheck(Unit*                 caster,
                                Unit*                 referer,
                                SpellInfo const*      spellInfo,
                                SpellTargetCheckTypes selectionType,
                                ConditionList*        condList);
...
struct WorldObjectSpellNearbyTargetCheck : public WorldObjectSpellTargetCheck {
    ...
    WorldObjectSpellNearbyTargetCheck(float                 range,
                                      Unit*                 caster,
                                      SpellInfo const*      spellInfo,
                                      SpellTargetCheckTypes selectionType,
                                      ConditionList*        condList);
...
struct WorldObjectSpellAreaTargetCheck : public WorldObjectSpellTargetCheck {
    float           _range;
    Position const* _position;
    WorldObjectSpellAreaTargetCheck(float                 range,
                                    Position const*       position,
                                    Unit*                 caster,
                                    Unit*                 referer,
                                    SpellInfo const*      spellInfo,
...
struct WorldObjectSpellConeTargetCheck
    : public WorldObjectSpellAreaTargetCheck {
    float _coneAngle;
    WorldObjectSpellConeTargetCheck(float                 coneAngle,
                                    float                 range,
                                    Unit*                 caster,
                                    SpellInfo const*      spellInfo,
                                    SpellTargetCheckTypes selectionType,
...
struct WorldObjectSpellTrajTargetCheck
    : public WorldObjectSpellAreaTargetCheck {
    WorldObjectSpellTrajTargetCheck(float                 range,
                                    Position const*       position,
                                    Unit*                 caster,
                                    SpellInfo const*      spellInfo,
                                    SpellTargetCheckTypes selectionType,
...
public:
    ReflectEvent(Unit*            caster,
                 ObjectGuid       targetGUID,
                 SpellInfo const* spellInfo)
        : _caster(caster), _targetGUID(targetGUID), _spellInfo(spellInfo)
    {
    }
