...
CalendarInvite::CalendarInvite()
    : _inviteId(1), _eventId(0), _statusTime(GameTime::GetGameTime().count()),
...
CalendarInvite::~CalendarInvite()
{
    // Free _inviteId only if it's a real invite and not just a pre-invite or
    // guild announcement
    if (_inviteId != 0 && _eventId != 0)
        sCalendarMgr->FreeInviteId(_inviteId);
}

CalendarEvent::~CalendarEvent() { sCalendarMgr->FreeEventId(_eventId); }

CalendarMgr::CalendarMgr() : _maxEventId(0), _maxInviteId(0) {}

CalendarMgr::~CalendarMgr()
{
    for (CalendarEventStore::iterator itr = _events.begin();
         itr != _events.end();
         ++itr)
        delete *itr;

    for (CalendarEventInviteStore::iterator itr = _invites.begin();
         itr != _invites.end();
         ++itr)
        for (CalendarInviteStore::iterator itr2 = itr->second.begin();
             itr2 != itr->second.end();
             ++itr2)
            delete *itr2;
}
...
void CalendarMgr::LoadFromDB()
{
    ...
    if (QueryResult result = CharacterDatabase.Query(
            "SELECT id, creator, title, description, type, dungeon, eventtime, "
            "flags, time2 FROM calendar_events"))
        do {
            ...
            if (flags & CALENDAR_FLAG_GUILD_EVENT ||
                flags & CALENDAR_FLAG_WITHOUT_INVITES) {
                guildId =
                    sCharacterCache->GetCharacterGuildIdByGuid(creatorGUID);
            }

            CalendarEvent* calendarEvent =
                new CalendarEvent(eventId,
                                  creatorGUID,
                                  guildId,
                                  type,
                                  dungeonId,
    ...
    //                                                       6      7
    if (QueryResult result = CharacterDatabase.Query(
            "SELECT id, event, invitee, sender, status, statustime, `rank`, "
            "text FROM calendar_invites"))
        do {
            Field* fields = result->Fetch();
            ...
            ObjectGuid senderGUID =
                ObjectGuid::Create<HighGuid::Player>(fields[3].Get<uint32>());
            CalendarInviteStatus status =
                CalendarInviteStatus(fields[4].Get<uint8>());
            uint32                 statusTime = fields[5].Get<uint32>();
            CalendarModerationRank rank =
                CalendarModerationRank(fields[6].Get<uint8>());
                           ...
                           CalendarSendEventType sendType)
{
    ...
    SendCalendarEvent(
        calendarEvent->GetCreatorGUID(), *calendarEvent, sendType);
}
                            ...
                            CharacterDatabaseTransaction trans)
{
    ...
    if (!calendarEvent->IsGuildEvent() ||
        invite->GetInviteeGUID() == calendarEvent->GetCreatorGUID())
...
void CalendarMgr::RemoveEvent(CalendarEvent* calendarEvent, ObjectGuid remover)
{
    ...
    MailDraft mail(calendarEvent->BuildCalendarMailSubject(remover),
                   calendarEvent->BuildCalendarMailBody());
    ...
    for (size_t i = 0; i < eventInvites.size(); ++i) {
        ...
        // When an event is deleted, all invited (accepted/declined? - verify)
        // guildies are notified via in-game mail. (wowwiki)
        if (remover && invite->GetInviteeGUID() != remover)
            mail.SendMailTo(trans,
                            MailReceiver(invite->GetInviteeGUID().GetCounter()),
                            calendarEvent,
                               ...
                               ObjectGuid /*remover*/)
{
    ...
    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_CALENDAR_INVITE);
    ...
    if (!calendarEvent->IsGuildEvent())
        SendCalendarEventInviteRemoveAlert(
            (*itr)->GetInviteeGUID(), *calendarEvent, CALENDAR_STATUS_REMOVED);

    SendCalendarEventInviteRemove(
        *calendarEvent, **itr, calendarEvent->GetFlags());

    // we need to find out how to use CALENDAR_INVITE_REMOVED_MAIL_SUBJECT to
    // force client to display different mail
    // if ((*itr)->GetInviteeGUID() != remover)
    //    MailDraft(calendarEvent->BuildCalendarMailSubject(remover),
    //    calendarEvent->BuildCalendarMailBody())
    //        .SendMailTo(trans, MailReceiver((*itr)->GetInvitee()),
    //        calendarEvent, MAIL_CHECK_MASK_COPIED);
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_REP_CALENDAR_EVENT);
    stmt->SetData(0, calendarEvent->GetEventId());
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_REP_CALENDAR_INVITE);
    stmt->SetData(0, invite->GetInviteId());
...
{
    for (CalendarEventStore::const_iterator itr = _events.begin();
         itr != _events.end();) {
    ...
    for (CalendarInviteStore::const_iterator itr = playerInvites.begin();
         itr != playerInvites.end();
         ++itr)
                                                    ...
                                                    uint32     guildId)
{
    for (CalendarEventStore::const_iterator itr = _events.begin();
        ...
        if ((*itr)->GetCreatorGUID() == guid &&
            ((*itr)->IsGuildEvent() || (*itr)->IsGuildAnnouncement()))
    ...
    for (CalendarInviteStore::const_iterator itr = playerInvites.begin();
        ...
        if (CalendarEvent* calendarEvent = GetEvent((*itr)->GetEventId()))
            if (calendarEvent->IsGuildEvent() &&
                calendarEvent->GetGuildId() == guildId)
                RemoveInvite((*itr)->GetInviteId(), (*itr)->GetEventId(), guid);
}
...
{
    for (CalendarEventInviteStore::const_iterator itr = _invites.begin();
         itr != _invites.end();
         ++itr)
        for (CalendarInviteStore::const_iterator itr2 = itr->second.begin();
             itr2 != itr->second.end();
             ++itr2)
    ...
    LOG_DEBUG(
        "entities.unit", "CalendarMgr::GetInvite: [{}] not found!", inviteId);
    return nullptr;
}
...
{
    time_t oldEventsTime =
        GameTime::GetGameTime().count() - CALENDAR_OLD_EVENTS_DELETION_TIME;
        ...
        for (CalendarInviteStore::const_iterator itr2 = itr->second.begin();
            ...
            if ((*itr2)->GetInviteeGUID() == guid)
                if (CalendarEvent* event =
                        GetEvent(itr->first)) // nullptr check added as attempt
                                              // to fix #11512
                    events.insert(event);
...
void CalendarMgr::SendCalendarEventInvite(CalendarInvite const& invite)
{
    ...
    uint8 level = player ? player->GetLevel()
                         : sCharacterCache->GetCharacterLevelByGuid(invitee);

    WorldPacket data(SMSG_CALENDAR_EVENT_INVITE,
                     8 + 8 + 8 + 1 + 1 + 1 + (statusTime ? 4 : 0) + 1);
    ...
    data << uint8(
        invite.GetSenderGUID() !=
        invite.GetInviteeGUID()); // false only if the invite is sign-up

    if (!calendarEvent) // Pre-invite
    {
        if (Player* player =
                ObjectAccessor::FindConnectedPlayer(invite.GetSenderGUID()))
            player->SendDirectMessage(&data);
    }
    else {
        if (calendarEvent->GetCreatorGUID() !=
            invite.GetInviteeGUID()) // correct?
            SendPacketToAllEventRelatives(data, *calendarEvent);
    }
}

void CalendarMgr::SendCalendarEventUpdateAlert(
    CalendarEvent const& calendarEvent, time_t oldEventTime)
{
    WorldPacket data(SMSG_CALENDAR_EVENT_UPDATED_ALERT,
                     1 + 8 + 4 + 4 + 4 + 1 + 4 +
                         calendarEvent.GetTitle().size() +
                         calendarEvent.GetDescription().size() + 1 + 4 + 4);
    data << uint8(1); // unk
    ...
    CalendarEvent const& calendarEvent)
{
    ...
    data << uint8(1); // FIXME: If true does not
                      // SignalEvent(EVENT_CALENDAR_ACTION_PENDING)
    data << uint64(calendarEvent.GetEventId());
    data.AppendPackedTime(calendarEvent.GetEventTime());
    ...
    if (calendarEvent.IsGuildEvent() || calendarEvent.IsGuildAnnouncement()) {
        if (Guild* guild = sGuildMgr->GetGuildById(calendarEvent.GetGuildId()))
            guild->BroadcastPacket(&data);
                 ...
                 ObjectAccessor::FindConnectedPlayer(invite.GetInviteeGUID()))
        player->SendDirectMessage(&data);
}
                                    ...
                                    CalendarSendEventType sendType)
{
    ...
    CalendarInviteStore const& eventInviteeList =
        _invites[calendarEvent.GetEventId()];

    WorldPacket data(SMSG_CALENDAR_SEND_EVENT,
                     60 + eventInviteeList.size() * 32);
    data << uint8(sendType);
    data << calendarEvent.GetCreatorGUID().WriteAsPacked();
    data << uint64(calendarEvent.GetEventId());
    data << calendarEvent.GetTitle();
    ...
    for (CalendarInviteStore::const_iterator itr = eventInviteeList.begin();
         itr != eventInviteeList.end();
         ++itr) {
        ...
        uint8 inviteeLevel =
            invitee ? invitee->GetLevel()
                    : sCharacterCache->GetCharacterLevelByGuid(inviteeGuid);
        uint32 inviteeGuildId =
            invitee ? invitee->GetGuildId()
                    : sCharacterCache->GetCharacterGuildIdByGuid(inviteeGuid);
...
void CalendarMgr::SendPacketToAllEventRelatives(
    WorldPacket packet, CalendarEvent const& calendarEvent)
{
    ...
    // Send packet to all invitees if event is non-guild, in other case only to
    // non-guild invitees (packet was broadcasted for them)
    CalendarInviteStore invites = _invites[calendarEvent.GetEventId()];
    for (CalendarInviteStore::iterator itr = invites.begin();
         itr != invites.end();
         ++itr)
        if (Player* player =
                ObjectAccessor::FindConnectedPlayer((*itr)->GetInviteeGUID()))
            if (!calendarEvent.IsGuildEvent() ||
                player->GetGuildId() != calendarEvent.GetGuildId())
