...
public:
    CalendarInvite(CalendarInvite const& calendarInvite,
                   uint64                inviteId,
                   uint64                eventId)
    {
        _inviteId   = inviteId;
    ...
    CalendarInvite(uint64                 inviteId,
                   uint64                 eventId,
                   ObjectGuid             invitee,
                   ...
                   std::string            text)
        : _inviteId(inviteId), _eventId(eventId), _invitee(invitee),
          _senderGUID(senderGUID), _statusTime(statusTime), _status(status),
          _rank(rank), _text(text)
    {
    }
...
public:
    ...
    CalendarEvent(uint64            eventId,
        ...
        : _eventId(eventId), _creatorGUID(creatorGUID), _guildId(guildId),
          _type(type), _dungeonId(dungeonId), _eventTime(eventTime),
          _flags(flags), _timezoneTime(timezoneTime), _title(title),
          _description(description)
    {
    }

    CalendarEvent()
        : _eventId(1), _guildId(0), _type(CALENDAR_TYPE_OTHER), _dungeonId(-1),
          _eventTime(0), _flags(0), _timezoneTime(0), _title(""),
          _description("")
    {
    }
    ...
    {
        return _flags & CALENDAR_FLAG_WITHOUT_INVITES;
    }

    static bool IsGuildEvent(uint32 flags)
    {
        return (flags & CALENDAR_FLAG_GUILD_EVENT) != 0;
    }
    static bool IsGuildAnnouncement(uint32 flags)
    {
        return (flags & CALENDAR_FLAG_WITHOUT_INVITES) != 0;
    }
...
typedef std::unordered_map<uint64 /* eventId */, CalendarInviteStore>
...
public:
    ...
    CalendarEventStore        GetEventsCreatedBy(ObjectGuid guid,
    ...
    void AddInvite(CalendarEvent*               calendarEvent,
    ...
    void SendCalendarEvent(ObjectGuid            guid,
                           CalendarEvent const&  calendarEvent,
                           CalendarSendEventType sendType);
    void SendCalendarEventInvite(CalendarInvite const& invite);
    void SendCalendarEventInviteAlert(CalendarEvent const&  calendarEvent,
                                      CalendarInvite const& invite);
    void SendCalendarEventInviteRemove(CalendarEvent const&  calendarEvent,
                                       CalendarInvite const& invite,
                                       uint32                flags);
    void SendCalendarEventInviteRemoveAlert(ObjectGuid           guid,
                                            CalendarEvent const& calendarEvent,
                                            CalendarInviteStatus status);
    void SendCalendarEventUpdateAlert(CalendarEvent const& calendarEvent,
                                      time_t               oldEventTime);
    void SendCalendarEventStatus(CalendarEvent const&  calendarEvent,
                                 CalendarInvite const& invite);
    void SendCalendarEventRemovedAlert(CalendarEvent const& calendarEvent);
    void
    SendCalendarEventModeratorStatusAlert(CalendarEvent const&  calendarEvent,
                                          CalendarInvite const& invite);
    void SendCalendarClearPendingAction(ObjectGuid guid);
    void SendCalendarCommandResult(ObjectGuid    guid,
                                   CalendarError err,
                                   char const*   param = nullptr);

    void SendPacketToAllEventRelatives(WorldPacket          packet,
                                       CalendarEvent const& calendarEvent);
};
