...
enum WorldStates {
    ...
    WS_DAILY_CALENDAR_DELETION_OLD_EVENTS_TIME =
        20008 // Next daily calendar deletions of old events time
};
...
public:
    ...
    [[nodiscard]] WorldSession* FindOfflineSessionForCharacterGUID(
        ObjectGuid::LowType guidLow) const override;
    void AddSession(WorldSession* s) override;
    bool KickSession(uint32 id) override;
    /// Get the number of current active sessions
    void                            UpdateMaxSessionCounters() override;
    [[nodiscard]] const SessionMap& GetAllSessions() const override
    {
        return _sessions;
    }
    [[nodiscard]] uint32 GetActiveAndQueuedSessionCount() const override
    {
        return _sessions.size();
    }
    [[nodiscard]] uint32 GetActiveSessionCount() const override
    {
        return _sessions.size() - _queuedPlayer.size();
    }
    [[nodiscard]] uint32 GetQueuedSessionCount() const override
    {
        return _queuedPlayer.size();
    }
    /// Get the maximum number of parallel sessions on the server since last
    /// reboot
    [[nodiscard]] uint32 GetMaxQueuedSessionCount() const override
    {
        return _maxQueuedSessionCount;
    }
    [[nodiscard]] uint32 GetMaxActiveSessionCount() const override
    {
        return _maxActiveSessionCount;
    }
    /// Get number of players
    [[nodiscard]] inline uint32 GetPlayerCount() const override
    {
        return _playerCount;
    }
    [[nodiscard]] inline uint32 GetMaxPlayerCount() const override
    {
    ...
    [[nodiscard]] AccountTypes GetPlayerSecurityLimit() const override
    {
    ...
    [[nodiscard]] uint32 GetPlayerAmountLimit() const override
    {
        return _playerLimit;
    }
    ...
    /// Is movement allowed?
    [[nodiscard]] bool getAllowMovement() const override
    {
    ...
    [[nodiscard]] std::string const& GetNewCharString() const override
    {
        return _newCharString;
    }

    [[nodiscard]] LocaleConstant GetDefaultDbcLocale() const override
    {
        return _defaultDbcLocale;
    }

    /// Get the path where data (dbc, maps) are stored on disk
    [[nodiscard]] std::string const& GetDataPath() const override
    {
        return _dataPath;
    }

    /// Next daily quests and random bg reset time
    [[nodiscard]] Seconds GetNextDailyQuestsResetTime() const override
    {
        return _nextDailyQuestReset;
    }
    [[nodiscard]] Seconds GetNextWeeklyQuestsResetTime() const override
    {
        return _nextWeeklyQuestReset;
    }
    [[nodiscard]] Seconds GetNextRandomBGResetTime() const override
    {
    ...
    void SendGlobalMessage(WorldPacket const* packet,
                           ...
                           TeamId             teamId = TEAM_NEUTRAL) override;
    void SendGlobalGMMessage(WorldPacket const* packet,
                             WorldSession*      self   = nullptr,
                             TeamId             teamId = TEAM_NEUTRAL) override;
    bool SendZoneMessage(uint32             zone,
                         WorldPacket const* packet,
                         WorldSession*      self   = nullptr,
                         TeamId             teamId = TEAM_NEUTRAL) override;
    void SendZoneText(uint32        zone,
                      const char*   text,
                      WorldSession* self   = nullptr,
                      TeamId        teamId = TEAM_NEUTRAL) override;
    void SendServerMessage(ServerMessageType messageID,
    ...
    [[nodiscard]] uint32 GetShutDownTimeLeft() const override
    {
    ...
    void         ShutdownMsg(bool               show   = false,
    ...
    {
        _rate_values[rate] = value;
    }
    /// Get a server rate (see #Rates)
    [[nodiscard]] float getRate(Rates rate) const override
    {
        return _rate_values[rate];
    }
    ...
    {
        return _maxVisibleDistanceInBGArenas;
    }
    ...
    {
        _cliCmdQueue.add(commandHolder);
    }
    ...
    [[nodiscard]] LocaleConstant
    GetAvailableDbcLocale(LocaleConstant locale) const override
    {
    ...
    [[nodiscard]] char const* GetDBVersion() const override
    {
        return _dbVersion.c_str();
    }

    void UpdateAreaDependentAuras() override;

    [[nodiscard]] uint32 GetCleaningFlags() const override
    {
    ...
    [[nodiscard]] std::string const& GetRealmName() const override
    {
        return _realmName;
    } // pussywizard
...
private:
    ...
    LocaleConstant
           _defaultDbcLocale; // from config for one from loaded DBC locales
    ...
    /**
     * @brief Executed when a World Session is being finalized. Be it from a
     * normal login or via queue popping.
     *
