...
enum MovementGeneratorType {
    ...
    ANIMAL_RANDOM_MOTION_TYPE =
        MAX_DB_MOTION_TYPE,    // AnimalRandomMovementGenerator.h
    ...
    ASSISTANCE_MOTION_TYPE =
        11, // PointMovementGenerator.h (first part of flee for assistance)
    ASSISTANCE_DISTRACT_MOTION_TYPE =
        12, // IdleMovementGenerator.h (second part of flee for assistance)
    TIMED_FLEEING_MOTION_TYPE = 13, // FleeingMovementGenerator.h (alt.second
                                    // part of flee for assistance)
...
struct ChaseRange {
    ...
    ChaseRange(float _minRange,
               float _minTolerance,
               float _maxTolerance,
               float _maxRange);

    // this contains info that informs how we should path!
    float MinRange; // we have to move if we are within this range...    (min.
                    // attack range)
    float MinTolerance; // ...and if we are, we will move this far away
    float MaxRange; // we have to move if we are outside this range...   (max.
                    // attack range)
...
struct ChaseAngle {
    ...
    float RelativeAngle; // we want to be at this angle relative to the target
                         // (0 = front, M_PI = back)
...
public:
    explicit MotionMaster(Unit* unit)
        : _expList(nullptr), _top(-1), _owner(unit), _cleanFlag(MMCF_NONE)
    {
    ...
    {
        MoveChase(target, ChaseRange(dist), ChaseAngle(angle));
    }
    void MoveChase(Unit* target, float dist)
    {
        MoveChase(target, ChaseRange(dist));
    }
    ...
    // These two movement types should only be used with creatures having
    // landing/takeoff animations
    void MoveLand(uint32 id, Position const& pos, float speed = 0.0f);
    void MoveLand(uint32 id,
                  float  x,
                  float  y,
                  float  z,
                  float speed = 0.0f); // pussywizard: added for easy calling by
                                       // passing 3 floats x, y, z
    void MoveTakeoff(uint32 id, Position const& pos, float speed = 0.0f);
    void MoveTakeoff(uint32 id,
                     float  x,
                     float  y,
                     float  z,
                     float speed = 0.0f); // pussywizard: added for easy calling
                                          // by passing 3 floats x, y, z

    void MoveCharge(float                        x,
                    float                        y,
                    float                        z,
    ...
    void
    MoveJump(Position const& pos, float speedXY, float speedZ, uint32 id = 0)
    {
        MoveJump(pos.m_positionX,
                 pos.m_positionY,
                 pos.m_positionZ,
                 speedXY,
                 speedZ,
...
private:
    void Mutate(MovementGenerator* m,
                MovementSlot       slot); // use Move* functions instead
