...
enum PathType {
    ...
    PATHFIND_FARFROMPOLY_START =
        0x40, // start position is far from the mmap poligon
    PATHFIND_FARFROMPOLY_END =
        0x80, // end positions is far from the mmap poligon
    PATHFIND_FARFROMPOLY =
        PATHFIND_FARFROMPOLY_START |
        PATHFIND_FARFROMPOLY_END, // start or end positions are far from the
                                  // mmap poligon
};
...
public:
    ...
    // needed)
    bool               CalculatePath(float destX,
                                     float destY,
                                     float destZ,
                                     bool  forceDest = false);
    bool               CalculatePath(float x,
                                     float y,
                                     float z,
                                     float destX,
                                     float destY,
    ...
    [[nodiscard]] bool IsWalkableClimb(
        float x, float y, float z, float destX, float destY, float destZ) const;
    [[nodiscard]] static bool IsWalkableClimb(float x,
                                              float y,
                                              float z,
                                              float destX,
                                              float destY,
    ...
    [[nodiscard]] bool IsSwimmableSegment(float const* v1,
                                          float const* v2,
                                          bool         checkSwim = true) const;
    [[nodiscard]] bool IsSwimmableSegment(float x,
                                          float y,
                                          float z,
                                          float destX,
                                          float destY,
                                          float destZ,
                                          bool  checkSwim = true) const;
    [[nodiscard]] static float GetRequiredHeightToClimb(float x,
                                                        float y,
                                                        float z,
                                                        float destX,
                                                        float destY,
    ...
    // when set, it skips paths with too high slopes (doesn't work with
    // StraightPath enabled)
    ...
    // result getters
    [[nodiscard]] G3D::Vector3 const& GetStartPosition() const
    {
        return _startPosition;
    }
    [[nodiscard]] G3D::Vector3 const& GetEndPosition() const
    {
        return _endPosition;
    }
    [[nodiscard]] G3D::Vector3 const& GetActualEndPosition() const
    {
        return _actualEndPosition;
    }

    [[nodiscard]] Movement::PointsArray const& GetPath() const
    {
        return _pathPoints;
    }
    ...
    // shortens the path until the destination is the specified distance from
    // the target point
...
private:
    ...
    bool _useStraightPath;  // type of path will be generated (do not use it for
                            // movement paths)
    bool _forceDestination; // when set, we will always arrive at given point
    bool _slopeCheck; // when set, it skips paths with too high slopes (doesn't
                      // work with _useStraightPath)
    ...
    [[nodiscard]] bool  InRange(G3D::Vector3 const& p1,
                                G3D::Vector3 const& p2,
                                float               r,
                                float               h) const;
    [[nodiscard]] float Dist3DSqr(G3D::Vector3 const& p1,
    ...
    void BuildPolyPath(G3D::Vector3 const& startPos,
