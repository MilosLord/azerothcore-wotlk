...
public:
    RandomMovementGenerator(float wanderDistance = 0.0f)
        : _nextMoveTime(0), _moveCount(0), _wanderDistance(wanderDistance),
          _pathGenerator(nullptr), _currentPoint(RANDOM_POINTS_NUMBER)
    {
        ...
        for (uint8 i = 0; i < RANDOM_POINTS_NUMBER; ++i) {
            ...
            for (uint8 j = 0; j < RANDOM_LINKS_COUNT; ++j)
                _validPointsVector[i].push_back((i + j +
                                                 RANDOM_POINTS_NUMBER / 2 -
                                                 RANDOM_LINKS_COUNT / 2) %
