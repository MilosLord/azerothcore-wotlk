...
void FleeingMovementGenerator<T>::SetTargetLocation(T* owner)
{
    ...
    if (!owner->IsWithinLOS(destination.GetPositionX(),
    ...
    bool result = _path->CalculatePath(destination.GetPositionX(),
                                       destination.GetPositionY(),
                                       destination.GetPositionZ());
    if (!result ||
        (_path->GetPathType() & PathType(PATHFIND_NOPATH | PATHFIND_SHORTCUT |
...
void FleeingMovementGenerator<T>::GetPoint(T* owner, Position& position)
{
    ...
    if (casterDistance < MIN_QUIET_DISTANCE) {
        ...
        distance =
            frand(0.4f, 1.0f) * (MAX_QUIET_DISTANCE - MIN_QUIET_DISTANCE);
        angle = -casterAngle + frand(-float(M_PI) / 4.0f, float(M_PI) / 4.0f);
    }
    else // we are inside quiet range
    {
        distance =
            frand(0.6f, 1.2f) * (MAX_QUIET_DISTANCE - MIN_QUIET_DISTANCE);
...
template void FleeingMovementGenerator<Creature>::GetPoint(Creature*,
                                                           Position&);
...
bool TimedFleeingMovementGenerator::Update(Unit* owner, uint32 time_diff)
{
    ...
    // This calls grant-parent Update method hiden by
    // FleeingMovementGenerator::Update(Creature &, uint32) version This is done
    // instead of casting Unit& to Creature& and call parent method, then we can
    // use Unit directly
    return MovementGeneratorMedium<
        Creature,
        FleeingMovementGenerator<Creature>>::Update(owner, time_diff);
}
