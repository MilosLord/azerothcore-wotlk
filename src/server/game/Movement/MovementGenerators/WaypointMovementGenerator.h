...
public:
    WaypointMovementGenerator(uint32 _path_id = 0, bool _repeating = true)
        : PathMovementBase((WaypointPath const*)nullptr), i_nextMoveTime(0),
          m_isArrivalDone(false), path_id(_path_id), repeating(_repeating)
    {
    }
...
public:
    ...
    void
    DoEventIfAny(Player* player, TaxiPathNodeEntry const* node, bool departure);
...
private:
    ...
    std::deque<TaxiNodeChangeInfo>
        _pointsForPathSwitch; //! node indexes and costs where TaxiPath changes
};
