...
PathGenerator::PathGenerator(WorldObject const* owner)
    : _polyLength(0), _type(PATHFIND_BLANK), _useStraightPath(false),
      _forceDestination(false), _slopeCheck(false),
      _pointPathLimit(MAX_POINT_PATH_LENGTH), _useRaycast(false),
...
bool PathGenerator::CalculatePath(float x,
                                  ...
                                  bool  forceDest)
{
    if (!Acore::IsValidMapCoord(destX, destY, destZ) ||
                                  ...
                                  G3D::Vector3 const& endPos)
{
    ...
    // make shortcut path and mark it as NOPATH ( with flying and swimming
    // exception ) its up to caller how he will use this info
    ...
    if (startFarFromPoly || endFarFromPoly) {
        ...
        auto liquidDataStart =
            _source->GetMap()->GetLiquidData(_source->GetPhaseMask(),
                                             startPos.x,
                                             startPos.y,
                                             startPos.z,
                                             _source->GetCollisionHeight(),
                                             MAP_ALL_LIQUIDS);
        auto liquidDataEnd =
            _source->GetMap()->GetLiquidData(_source->GetPhaseMask(),
                                             endPos.x,
        ...
        bool startUnderWaterEndInWater =
            liquidDataStart.Status == LIQUID_MAP_UNDER_WATER &&
            (liquidDataEnd.Status & MAP_LIQUID_STATUS_IN_CONTACT) != 0;
        bool startInWaterEndUnderWater =
            (liquidDataStart.Status & MAP_LIQUID_STATUS_IN_CONTACT) != 0 &&
        ...
        if (_sourceUnit) {
            ...
            if (isWater || _sourceUnit->CanFly() ||
                (_sourceUnit->IsFalling() && endPos.z < startPos.z)) {
                buildShortcut = true;
            }
        }
        ...
        if (buildShortcut) {
            ...
            if (dtStatusSucceed(_navMeshQuery->closestPointOnPoly(
                    endPoly, endPoint, closestPoint, nullptr))) {
                dtVcopy(endPoint, closestPoint);
                SetActualEndPosition(
                    G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]));
            }
    ...
    if (startPolyFound && endPolyFound) {
        ...
        // poly-path our path is a simple subpath case, we have all the data we
        // need just "cut" it out

        _polyLength = pathEndIndex - pathStartIndex + 1;
        memmove(_pathPolyRefs,
                _pathPolyRefs + pathStartIndex,
        ...
        if (dtStatusFailed(_navMeshQuery->closestPointOnPoly(
            ...
            // don't like that try to recover by using prev polyref
            --prefixPolyLength;
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult)) {
            ...
            LOG_ERROR("movement",
                      "PathGenerator::BuildPolyPath: Path Build failed {}",
        ...
        // the target just generate new path
        ...
        if (_useRaycast) {
            ...
            dtResult = _navMeshQuery->raycast(startPoly,
                                              startPoint,
                                              endPoint,
                                              &_filter,
                                              &hit,
            ...
            if (hit != FLT_MAX) {
                ...
                // Walk back a bit from the hit point to make sure it's in the
                // mesh (sometimes the point is actually outside of the polygons
                ...
                if (dtStatusFailed(_navMeshQuery->getPolyHeight(
                        _pathPolyRefs[_polyLength - 1], hitPos, &hitPos[1])))
                    _navMeshQuery->closestPointOnPolyBoundary(
                        _pathPolyRefs[_polyLength - 1], hitPos, hitPos);
                ...
                if (dtStatusFailed(_navMeshQuery->getPolyHeight(
                        _pathPolyRefs[_polyLength - 1],
                        endPoint,
                        &endPoint[1])))
                    _navMeshQuery->closestPointOnPolyBoundary(
                        _pathPolyRefs[_polyLength - 1], endPoint, endPoint);
        ...
        if (!_polyLength || dtStatusFailed(dtResult)) {
            ...
            // messed up
            LOG_ERROR("movement",
                      "PathGenerator::BuildPolyPath: {} Path Build failed: 0 "
                      "length path",
                      _source->GetGUID().ToString());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
    ...
    if (!_polyLength) {
        LOG_ERROR(
            "movement",
            "PathGenerator::BuildPolyPath: {} Path Build failed: 0 length path",
            _source->GetGUID().ToString());
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
...
void PathGenerator::BuildPointPath(const float* startPoint,
                                   const float* endPoint)
{
    ...
    if (_useRaycast) {
        // _straightLine uses raycast and it currently doesn't support building
        // a point path, only a 2-point path with start and hitpoint/end is
        // returned
        LOG_ERROR("movement",
                  "PathGenerator::BuildPointPath() called with _useRaycast for "
                  "unit {}",
                  _source->GetGUID().ToString());
        BuildShortcut();
        _type = PATHFIND_NOPATH;
    ...
    if (_polyLength == 1 && pointCount == 1) {
        ...
        // broken single point paths can be generated here
...
void PathGenerator::NormalizePath()
{
    for (uint32 i = 0; i < _pathPoints.size(); ++i) {
        _source->UpdateAllowedPositionZ(
...
void PathGenerator::UpdateFilter()
{
    ...
    // moved forcefully into terrain they can't normally move in
    if (Unit const* _sourceUnit = _source->ToUnit()) {
        ...
        /*if (Creature const* _sourceCreature = _source->ToCreature())
            if (_sourceCreature->IsInCombat() ||
           _sourceCreature->IsInEvadeMode())
                _filter.setIncludeFlags(_filter.getIncludeFlags() |
           NAV_GROUND_STEEP);*/
    }
}
                                   ...
                                   dtPolyRef&       steerPosRef)
{
    ...
    uint32              nsteerPath = 0;
    dtStatus            dtResult   = _navMeshQuery->findStraightPath(startPos,
                                                        endPos,
                                                        path,
                                                        pathSize,
                                                        steerPath,
                                       ...
                                       uint32           maxSmoothPathSize)
{
    *smoothPathSize    = 0;
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize) {
        ...
        uint32 nvisited = 0;
        if (dtStatusFailed(_navMeshQuery->moveAlongSurface(polys[0],
                                                           iterPos,
                                                           moveTgt,
                                                           &_filter,
                                                           result,
        ...
        if (dtStatusFailed(
            ...
            LOG_DEBUG("maps",
                      "PathGenerator::FindSmoothPath: Cannot find height at "
                      "position X: {} Y: {} Z: {} for {}",
                      result[2],
                      result[0],
                      result[1],
        ...
        bool canCheckSlope =
            _slopeCheck && (GetPathType() & ~(PATHFIND_NOT_USING_PATH));

        if (canCheckSlope && !IsSwimmableSegment(iterPos, steerPos) &&
            !IsWalkableClimb(iterPos, steerPos)) {
            return DT_FAILURE;
        }
...
bool PathGenerator::IsWalkableClimb(
    float x, float y, float z, float destX, float destY, float destZ) const
{
    return IsWalkableClimb(
        x, y, z, destX, destY, destZ, _source->GetCollisionHeight());
}
                                    ...
                                    float sourceHeight)
{
    ...
    float reqHeight =
        GetRequiredHeightToClimb(x, y, z, destX, destY, destZ, sourceHeight);
    // check walkable slopes, based on unit height
    return diffHeight <= reqHeight;
}

/**
 * @brief Return the height of a slope that can be climbed based on source
 * height This method is meant for short distances or linear paths
 *
 * @param x start x coord
 * @param y start y coord
 * @param z start z coord
                                              ...
                                              float sourceHeight)
{
    ...
    float climbableHeight =
        sourceHeight - (sourceHeight * (slopeAngleDegree / 100));
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
{
    if (GetPathType() == PATHFIND_BLANK || _pathPoints.size() < 2) {
        LOG_ERROR("movement",
                  "PathGenerator::ReducePathLengthByDist called before path "
    ...
    while (1) {
        // we know that pathPoints[i] is too close already (from the previous
        // iteration)
        ...
        // check if the shortened path is still in LoS with the target and it is
        // walkable
        _source->GetHitSpherePointFor({_pathPoints[i - 1].x,
                                       _pathPoints[i - 1].y,
                                       _pathPoints[i - 1].z + collisionHeight},
                                      x,
                                      y,
                                      z);
        if (!_source->GetMap()->isInLineOfSight(
                ...
                _pathPoints[i - 1].y,
                _pathPoints[i - 1].z + collisionHeight,
                _source->GetPhaseMask(),
                LINEOFSIGHT_ALL_CHECKS,
                VMAP::ModelIgnoreFlags::Nothing) ||
            (canCheckSlope &&
             !IsSwimmableSegment(_source->GetPositionX(),
                                 _source->GetPositionY(),
                                 _source->GetPositionZ(),
                                 _pathPoints[i - 1].x,
                                 _pathPoints[i - 1].y,
                                 _pathPoints[i - 1].z) &&
             !IsWalkableClimb(_source->GetPositionX(),
                              _source->GetPositionY(),
                              _source->GetPositionZ(),
                              _pathPoints[i - 1].x,
                              _pathPoints[i - 1].y,
                              _pathPoints[i - 1].z))) {
            // whenver we find a point that is not valid anymore, simply use
            // last valid path
    ...
    // ok, _pathPoints[i] is too close, _pathPoints[i-1] is not, so our target
    // point is somewhere between the two...
    //   ... settle for a guesstimate since i'm not confident in doing trig on
    //   every chase motion tick...
    // (@todo review this)
    _pathPoints[i] += (_pathPoints[i - 1] - _pathPoints[i]).direction() *
                      (dist - (_pathPoints[i] - target).length());
    _pathPoints.resize(i + 1);
}
...
bool PathGenerator::IsSwimmableSegment(float const* v1,
                                       float const* v2,
                                       bool         checkSwim) const
{
    return IsSwimmableSegment(
        v1[2], v1[0], v1[1], v2[2], v2[0], v2[1], checkSwim);
}
                                       ...
                                       bool  checkSwim) const
{
    ...
    return _source->GetMap()->IsInWater(_source->GetPhaseMask(),
                                        x,
                                        y,
                                        z,
                                        _source->GetCollisionHeight()) &&
           _source->GetMap()->IsInWater(_source->GetPhaseMask(),
                                        destX,
                                        destY,
                                        destZ,
                                        _source->GetCollisionHeight()) &&
...
{
    bool waterPath = true;
    // Check both start and end points, if they're both in water, then we can
    // *safely* let the creature move
    for (uint32 i = 0; i < pathPoints.size(); ++i) {
        NavTerrain terrain =
            GetNavTerrain(pathPoints[i].x, pathPoints[i].y, pathPoints[i].z);
