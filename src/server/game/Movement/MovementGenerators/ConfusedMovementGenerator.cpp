...
void ConfusedMovementGenerator<T>::DoInitialize(T* unit)
{
    ...
    for (uint8 idx = 0; idx < MAX_CONF_WAYPOINTS + 1; ++idx) {
        ...
        if (new_z <= INVALID_HEIGHT ||
            std::fabs(z - new_z) > 3.0f) // pussywizard
        {
            ...
            bool is_water = map->IsInWater(unit->GetPhaseMask(),
                                           wanderX,
                                           wanderY,
                                           z,
            ...
            if ((is_water && !is_water_ok) || (!is_water && !is_land_ok)) {
                //! Cannot use coordinates outside our InhabitType. Use the
            ...
            //! Trying to access path outside line of sight. Skip this by using
            //! the current or previous position.
            i_waypoints[idx][0] = idx > 0 ? i_waypoints[idx - 1][0] : x;
            i_waypoints[idx][1] = idx > 0 ? i_waypoints[idx - 1][1] : y;
            i_waypoints[idx][2] = idx > 0 ? i_waypoints[idx - 1][2] : z;
            continue;
...
template bool ConfusedMovementGenerator<Creature>::DoUpdate(Creature*,
                                                            uint32 diff);
