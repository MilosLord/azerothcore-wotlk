...
void PointMovementGenerator<T>::DoInitialize(T* unit)
{
    if (unit->HasUnitState(UNIT_STATE_NOT_MOVE) ||
        ...
        // the next line is to ensure that a new spline is created in DoUpdate()
        // once the unit is no longer rooted/stunned
        /// @todo: rename this flag to something more appropriate since it is
        /// set to true even without speed change now.
    ...
    if (m_precomputedPath.size() > 2) // pussywizard: for charge
        ...
        if (result && !(path.GetPathType() & PATHFIND_NOPATH) &&
            ...
            // Xinef: fix strange client visual bug, moving on z coordinate only
            // switches orientation by 180 degrees (visual only)
            if (G3D::fuzzyEq(unit->GetPositionX(), i_x) &&
                G3D::fuzzyEq(unit->GetPositionY(), i_y)) {
...
bool PointMovementGenerator<T>::DoUpdate(T* unit, uint32 /*diff*/)
{
    ...
    if (id != EVENT_CHARGE_PREPATH && i_recalculateSpeed &&
        !unit->movespline->Finalized()) {
        i_recalculateSpeed = false;
        Movement::MoveSplineInit init(unit);

        // xinef: speed changed during path execution, calculate remaining path
        // and launch it once more
        if (m_precomputedPath.size()) {
            uint32 offset =
                std::min(uint32(unit->movespline->_currentSplineIdx()),
                         uint32(m_precomputedPath.size()));
            Movement::PointsArray::iterator offsetItr =
                m_precomputedPath.begin();
            ...
            if (m_precomputedPath.size() > 2)
                ...
                init.MoveTo(m_precomputedPath[1].x,
                            m_precomputedPath[1].y,
                            m_precomputedPath[1].z,
                            true);
        }
        else
            init.MoveTo(i_x, i_y, i_z, true);
        if (speed > 0.0f) // Default value for point motion type is 0.0, if 0.0
                          // spline will use GetSpeed on unit
            init.SetVelocity(speed);
...
void PointMovementGenerator<T>::DoFinalize(T* unit)
{
    ...
    if (id == EVENT_CHARGE || id == EVENT_CHARGE_PREPATH) {
        ...
        if (_chargeTargetGUID && _chargeTargetGUID == unit->GetTarget()) {
            if (Unit* target =
                    ObjectAccessor::GetUnit(*unit, _chargeTargetGUID)) {
...
void EffectMovementGenerator::Finalize(Unit* unit)
{
    ...
    if (unit->GetTypeId() == TYPEID_UNIT &&
        unit->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) &&
        unit->movespline->isFalling()) // pussywizard
        unit->RemoveUnitMovementFlag(MOVEMENTFLAG_FALLING);

    // Need restore previous movement since we have no proper states system
    // if (unit->IsAlive() && !unit->HasUnitState(UNIT_STATE_CONFUSED |
    // UNIT_STATE_FLEEING))
    //{
