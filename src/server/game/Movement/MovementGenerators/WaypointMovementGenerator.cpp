...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    if (!i_path) {
        ...
        LOG_ERROR("sql.sql",
                  "WaypointMovementGenerator::LoadPath: creature {} ({}) "
...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* creature)
{
    ...
    if (i_path->at(i_currentNode)->event_id &&
        urand(0, 99) < i_path->at(i_currentNode)->event_chance) {
        LOG_DEBUG("maps.script",
                  "Creature movement start script {} at point {} for {}.",
                  i_path->at(i_currentNode)->event_id,
                  i_currentNode,
                  creature->GetGUID().ToString());
        creature->ClearUnitState(UNIT_STATE_ROAMING_MOVE);
        creature->GetMap()->ScriptsStart(sWaypointScripts,
                                         i_path->at(i_currentNode)->event_id,
                                         creature,
                                         nullptr);
    }
...
bool WaypointMovementGenerator<Creature>::StartMove(Creature* creature)
{
    ...
    bool transportPath =
        creature->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) &&
        creature->GetTransGUID();

    if (m_isArrivalDone) {
        // Xinef: not true... update this at every waypoint!
        // if ((i_currentNode == i_path->size() - 1) && !repeating) // If that's
        // our last waypoint
        {
            ...
            if (!transportPath)
                ...
                if (Transport* trans =
                        (creature->GetTransport()
                             ? creature->GetTransport()->ToMotionTransport()
                             : nullptr)) {
                    ...
                    transportPath = false;
                // else if (vehicle) - this should never happen, vehicle offsets
                // are const
            }
        }
        ...
        if ((i_currentNode == i_path->size() - 1) &&
            !repeating) // If that's our last waypoint
        {
    ...
    //! If creature is on transport, we assume waypoints set in DB are already
    //! transport offsets
    if (transportPath) {
        init.DisableTransportPathTransformations();
        if (TransportBase* trans = creature->GetDirectTransport())
            trans->CalculatePassengerPosition(formationDest.x,
                                              formationDest.y,
                                              formationDest.z,
                                              &formationDest.orientation);
    }

    float z = node->z;
    creature->UpdateAllowedPositionZ(node->x, node->y, z);
    //! Do not use formationDest here, MoveTo requires transport offsets due to
    //! DisableTransportPathTransformations() call but formationDest contains
    //! global coordinates
    ...
    if (creature->GetFormation() &&
        ...
        creature->GetFormation()->LeaderMoveTo(
            formationDest.x, formationDest.y, formationDest.z, node->move_type);
                                                   ...
                                                   uint32    diff)
{
    ...
    if (Stopped()) {
        ...
        if (creature->IsStopped())
            Stop(sWorld->getIntConfig(
                     CONFIG_WAYPOINT_MOVEMENT_STOP_TIME_FOR_PLAYER) *
                 IN_MILLISECONDS);
        else {
            bool finished = creature->movespline->Finalized();
            // xinef: code to detect pre-empetively if we should start movement
            // to next waypoint xinef: do not start pre-empetive movement if
            // current node has delay or we are ending waypoint movement
            // if (!finished && !i_path->at(i_currentNode)->delay &&
            // ((i_currentNode != i_path->size() - 1) || repeating))
            //    finished =
            //    (creature->movespline->_Spline().length(creature->movespline->_currentSplineIdx()
            //    + 1) - creature->movespline->timePassed()) < 200;
...
void WaypointMovementGenerator<Creature>::MovementInform(Creature* creature)
{
    ...
    if (Unit* owner = creature->GetCharmerOrOwner()) {
        if (UnitAI* AI = owner->GetAI()) {
            AI->SummonMovementInform(
                creature, WAYPOINT_MOTION_TYPE, i_currentNode);
        }
    }
    else {
...
void FlightPathMovementGenerator::LoadPath(Player* player)
{
    ...
    float                     discount = player->GetReputationPriceDiscount(
        player->m_taxi.GetFlightMasterFactionTemplate());
    for (uint32 src = 0, dst = 1; dst < taxi.size(); src = dst++) {
        ...
        if (!nodes.empty()) {
            ...
            for (uint32 i = 0; i < nodes.size(); ++i) {
                if (passedPreviousSegmentProximityCheck || !src ||
                    i_path.empty() ||
                    IsNodeIncludedInShortenedPath(i_path[i_path.size() - 1],
                                                  nodes[i])) {
                    if ((!src ||
                         (IsNodeIncludedInShortenedPath(start, nodes[i]) &&
                          i >= 2)) &&
                        (dst == taxi.size() - 1 ||
                         (IsNodeIncludedInShortenedPath(end, nodes[i]) &&
                          i < nodes.size() - 1))) {
        ...
        _pointsForPathSwitch.push_back(
            {uint32(i_path.size() - 1), int32(ceil(cost * discount))});
    }

    // TODO: fixes crash, but can be handled in a better way once we will know
    // how to reproduce it.
    if (GetCurrentNode() >= i_path.size()) {
        ...
        LOG_ERROR("movement.flightpath",
                  "Failed to build correct path for player: {}. Current node: "
                  "{}, max nodes: {}. Paths: {}. Player pos: {}.",
        ...
        // Lets choose the second last element so that a player would still have
        // some flight.
...
void FlightPathMovementGenerator::DoFinalize(Player* player)
{
    ...
    if (player->m_taxi.empty()) {
        ...
        // When the player reaches the last flight point, teleport to
        // destination taxi node location
...
void FlightPathMovementGenerator::DoReset(Player* player)
{
    ...
    if (currentNodeId == end) {
        LOG_DEBUG("movement.flightpath",
                  "FlightPathMovementGenerator::DoReset: trying to start a "
                  "flypath from the end point. {}",
                  player->GetGUID().ToString());
        return;
    }
...
{
    // skipping the first spline path point because it's our starting point and
    // not a taxi path point
    uint32 pointId = player->movespline->currentPathIdx() <= 0
                         ? 0
                         : player->movespline->currentPathIdx() - 1;
    if (pointId > i_currentNode && i_currentNode < i_path.size() - 1) {
        ...
        do {
            ...
            while (!_pointsForPathSwitch.empty() &&
                   _pointsForPathSwitch.front().PathIndex <= i_currentNode) {
                ...
                if (!_pointsForPathSwitch.empty()) {
                    player->UpdateAchievementCriteria(
                        ...
                        _pointsForPathSwitch.front().Cost);
                                               ...
                                               bool departure)
{
    if (uint32 eventid =
        ...
        LOG_DEBUG("maps.script",
                  "Taxi {} event {} of node {} of path {} for player {}",
...
{
    /*! Storage to preload flightmaster grid at end of flight. For multi-stop
       flights, this will be reinitialized for each flightmaster at the end of
       each spline (or stop) in the flight. */
    ...
    // pussywizard:
    {
        ...
        //_preloadTargetNode = nodeCount - 3; // pussywizard: this can be on
        //other map
    }
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
    ...
    if (endMap) {
        LOG_DEBUG("misc",
                  "Preloading grid ({}, {}) for map {} at node index {}/{}",
                  _endGridX,
                  _endGridY,
                  _endMapId,
                  _preloadTargetNode,
                  (uint32)(i_path.size() - 1));
