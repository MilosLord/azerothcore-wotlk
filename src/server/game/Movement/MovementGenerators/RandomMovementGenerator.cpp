...
void RandomMovementGenerator<Creature>::_setRandomLocation(Creature* creature)
{
    ...
    if (_validPointsVector[_currentPoint].empty()) {
        if (_currentPoint ==
            RANDOM_POINTS_NUMBER) // cant go anywhere from initial position,
                                  // lets stay
            return;
        ...
        init.MoveTo(_currDestPosition.GetPositionX(),
        ...
        if (creature->GetFormation() &&
            ...
            creature->GetFormation()->LeaderMoveTo(
                _currDestPosition.GetPositionX(),
                _currDestPosition.GetPositionY(),
                _currDestPosition.GetPositionZ(),
                0);
        return;
    ...
    std::vector<uint8>::iterator randomIter =
        _validPointsVector[_currentPoint].begin() + random;
    ...
    if (finalPath.empty()) {
        ...
        float x   = _destinationPoints[newPoint].x,
              y   = _destinationPoints[newPoint].y,
        ...
        // flying creature
        if (creature->CanFly())
            newZ = std::max<float>(levelZ,
                                   z + rand_norm() * _wanderDistance / 2.0f);
        // point underwater
            ...
            if (!creature->CanEnterWater()) {
                ...
                if (levelZ > INVALID_HEIGHT)
                    newZ = std::min<float>(levelZ - 2.0f,
                                           z + rand_norm() * _wanderDistance /
                                                   2.0f);
        ...
        if (newZ > INVALID_HEIGHT) {
            ...
            finalPath.push_back(G3D::Vector3(creature->GetPositionX(),
        ...
        else // ground
        {
            ...
            if (result && !(_pathGenerator->GetPathType() & PATHFIND_NOPATH)) {
                ...
                if (pathLen * pathLen > creature->GetExactDistSq(x, y, levelZ) *
                                            MAX_PATH_LENGHT_FACTOR *
                                            MAX_PATH_LENGHT_FACTOR) {
                    _validPointsVector[_currentPoint].erase(randomIter);
                    _preComputedPaths.erase(pathIdx);
                    return;
    ...
    if (creature->GetFormation() &&
        creature->GetFormation()->GetLeader() == creature)
        creature->GetFormation()->LeaderMoveTo(
            finalPoint.x, finalPoint.y, finalPoint.z, 0);
}
...
void RandomMovementGenerator<Creature>::DoInitialize(Creature* creature)
{
    ...
    _nextMoveTime.Reset(creature->GetSpawnId() &&
                                creature->GetWanderDistance() == _wanderDistance
                            ? urand(1, 5000)
                            : 0);
    _wanderDistance =
        std::max((creature->GetWanderDistance() == _wanderDistance &&
                  creature->GetInstanceId() == 0)
                     ? (creature->CanFly() ? MIN_WANDER_DISTANCE_AIR
                                           : MIN_WANDER_DISTANCE_GROUND)
                     : 0.0f,
                 _wanderDistance);
    ...
    if (G3D::fuzzyEq(_initialPosition.GetExactDist2d(0.0f, 0.0f), 0.0f)) {
        ...
        for (uint8 i = 0; i < RANDOM_POINTS_NUMBER; ++i) {
            ...
            _destinationPoints.push_back(
                G3D::Vector3(_initialPosition.GetPositionX() +
                                 _wanderDistance * cos(angle) * factor,
                             _initialPosition.GetPositionY() +
                                 _wanderDistance * std::sin(angle) * factor,
                                                 ...
                                                 const uint32 diff)
{
    ...
    // xinef: if we got disable move flag, do not remove default generator -
                                                         ...
                                                         float& z)
{
    ...
    else if (G3D::fuzzyNe(_initialPosition.GetExactDist2d(0.0f, 0.0f),
                          0.0f)) // if initial position is not 0.0f, 0.0f
