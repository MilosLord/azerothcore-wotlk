...
const float terminal_length =
    float(terminalVelocity * terminalVelocity) / (2.0f * gravity);
const float terminal_safeFall_length =
    (terminalSafefallVelocity * terminalSafefallVelocity) / (2.0f * gravity);
const float terminal_fallTime =
    float(terminalVelocity /
          gravity); // the time that needed to reach terminalVelocity
const float terminal_safeFall_fallTime = float(
    terminalSafefallVelocity /
    gravity); // the time that needed to reach terminalVelocity with safefall
...
float computeFallTime(float path_length, bool isSafeFall)
{
    ...
    if (isSafeFall) {
        if (path_length >= terminal_safeFall_length)
            time = (path_length - terminal_safeFall_length) /
                       terminalSafefallVelocity +
        ...
        if (path_length >= terminal_length)
            time = (path_length - terminal_length) / terminalVelocity +
                   terminal_fallTime;
        else
            time = sqrtf(2.0f * path_length / gravity);
    }
                           ...
                           float start_velocity /*= 0.0f*/)
{
    ...
    float terminal_time =
        (isSafeFall ? terminal_safeFall_fallTime : terminal_fallTime) -
        start_velocity /
            gravity; // the time that needed to reach terminalVelocity
...
char const* g_MovementFlag_names[] = {
    ...
    STR(Waterwalking), // 0x10000000,               // Prevent Unit From Falling
                       // Through Water
    STR(Safe_Fall), // 0x20000000,               // Active Rogue Safe Fall Spell
                    // (Passive)
...
char const* g_SplineFlag_names[32] = {
    ...
    STR(Done),       // 0x00000100,
    STR(Falling),    // 0x00000200,           // Not Compartible With Trajectory
                     // Movement
    STR(No_Spline),  // 0x00000400,
    STR(Trajectory), // 0x00000800,           // Not Compartible With Fall
                     // Movement
    STR(Walkmode),   // 0x00001000,
    STR(Flying),     // 0x00002000,           // Smooth Movement(Catmullrom
    ...
    STR(Catmullrom),   // 0x00040000,           // Used Catmullrom Interpolation
                       // Mode
    STR(Cyclic),       // 0x00080000,           // Movement By Cycled Spline
    STR(Enter_Cycle),  // 0x00100000,           // Everytime Appears With Cyclic
                       // Flag In Monster Move Packet
    STR(Animation),    // 0x00200000,           // Animationid (0...3), Uint32
                    // Time, Not Compartible With Trajectory And Fall Movement
    ...
    STR(OrientationInversed), // 0x08000000,           // Appears With Runmode
                              // Flag, Nodes ), // 1, Handles Orientation
