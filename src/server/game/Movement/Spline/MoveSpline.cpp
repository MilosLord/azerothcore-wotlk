...
Location MoveSpline::ComputePosition() const
{
    ...
    if (splineflags.done && splineflags.isFacing()) {
        ...
        if (!splineflags.hasFlag(MoveSplineFlag::OrientationFixed |
...
void MoveSpline::computeParabolicElevation(float& el) const
{
    if (time_passed > effect_start_time) {
        ...
        float t_durationf =
            MSToSec(Duration() -
                    effect_start_time); // client use not modified duration here

        // -a*x*x + bx + c:
        //(dur * v3->z_acceleration * dt)/2 - (v3->z_acceleration * dt * dt)/2 +
        //Z;
        el += (t_durationf - t_passedf) * 0.5f * vertical_acceleration *
              t_passedf;
    }
}
...
{
    float z_now = spline.getPoint(spline.first()).z -
                  Movement::computeFallElevation(MSToSec(time_passed), false);
...
struct FallInitializer {
    ...
    {
        return Movement::computeFallTime(start_elevation - s.getPoint(i + 1).z,
                                         false) *
               1000.f;
    }
};
...
struct CommonInitializer {
    CommonInitializer(float _velocity)
        : velocityInv(1000.f / _velocity), _time(minimal_duration)
    {
    }
...
{
    const SplineBase::EvaluationMode modes[2] = {SplineBase::ModeLinear,
    ...
    if (args.flags.cyclic) {
        ...
        spline.init_cyclic_spline(&args.path[0],
                                  args.path.size(),
                                  modes[args.flags.isSmooth()],
                                  cyclic_point);
    }
    else {
        spline.init_spline(
            &args.path[0], args.path.size(), modes[args.flags.isSmooth()]);
    }
    ...
    /// @todo: what to do in such cases? problem is in input data (all points
    /// are at same coords)
...
void MoveSpline::Initialize(MoveSplineInitArgs const& args)
{
    ...
    // spline initialized, duration known and i able to compute parabolic
    // acceleration
    if (args.flags & (MoveSplineFlag::Parabolic | MoveSplineFlag::Animation)) {
        ...
        if (args.flags.parabolic && effect_start_time < Duration()) {
            ...
            vertical_acceleration =
                args.parabolic_amplitude * 8.f / (f_duration * f_duration);
        }
    }
}

MoveSpline::MoveSpline()
    : m_Id(0), time_passed(0), vertical_acceleration(0.f),
      initialOrientation(0.f), effect_start_time(0), point_Idx(0),
      point_Idx_offset(0), onTransport(false)
{
    ...
    if (!(exp)) {                                                              \
        if (unit)                                                              \
            LOG_ERROR(                                                         \
                ...
                "MoveSplineInitArgs::Validate: expression '{}' failed for {}", \
                #exp,                                                          \
                unit->GetGUID().ToString());                                   \
        else                                                                   \
            LOG_ERROR("misc.movesplineinitargs",                               \
                      "MoveSplineInitArgs::Validate: expression '{}' failed "  \
                      "for cyclic spline continuation",                        \
                      #exp);                                                   \
        return false;                                                          \
    }
    CHECK(path.size() > 1);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2) {
        ...
        for (uint32 i = 1; i < path.size() - 1; ++i) {
            ...
            if (std::fabs(offset.x) >= MAX_OFFSET ||
                std::fabs(offset.y) >= MAX_OFFSET ||
    ...
    if (time_passed >= next_timestamp()) {
        ...
        if (point_Idx < spline.last()) {
            ...
            if (spline.isCyclic()) {
                ...
                result = Movement::MoveSpline::UpdateResult(Result_NextCycle |
                                                            Result_JustArrived);
            }
            else {
                _Finalize();
                ms_time_diff = 0;
                result = Movement::MoveSpline::UpdateResult(Result_Arrived |
                                                            Result_JustArrived);
            }
        }
    }
...
std::string MoveSpline::ToString() const
{
    ...
    if (splineflags.final_angle)
        ...
        str << "facing  point: " << facing.f.x << " " << facing.f.y << " "
            << facing.f.z;
    str << std::endl;
...
{
    int32 point =
        point_Idx_offset + point_Idx - spline.first() + (int)Finalized();
    if (isCyclic())
