...
public:
     ...
     * @param amplitude  - the maximum height of parabola, value could be
     * negative and positive
     * @param start_time - delay between movement starting time and beginning to
     * move by parabolic trajectory can't be combined with final animation
     */
     ...
     * @param pointId - Id of fisrt point of the path. Example: when third path
     * point will be done it will notify that pointId + 3 done
     */
    void MovebyPath(const PointsArray& path, int32 pointId = 0);

    /* Initializes simple A to B motion, A is current unit's position, B is
     * destination
     */
    ...
    /* Sets Id of fisrt point of the path. When N-th path point will be done
     * ILisener will notify that pointId + N done Needed for waypoint movement
     * where path splitten into parts
     */
     ...
     * if no set, speed will be selected based on unit's speeds and current
     * movement mode Has no effect if falling mode enabled velocity shouldn't be
     * negative
     */
    ...
    /* Disables transport coordinate transformations for cases where raw offsets
     * are available
     */
...
inline void MoveSplineInit::SetOrientationInversed()
{
...
inline void MoveSplineInit::SetOrientationFixed(bool enable)
{
    args.flags.orientationFixed = enable;
}

inline void MoveSplineInit::MovebyPath(const PointsArray& controls,
                                       int32              path_offset)
{
    args.path_Idx_offset = path_offset;
    args.path.resize(controls.size());
    std::transform(controls.begin(),
                   controls.end(),
                   args.path.begin(),
                   TransportPathTransform(unit, args.TransformForTransport));
}

inline void MoveSplineInit::MoveTo(
    float x, float y, float z, bool generatePath, bool forceDestination)
{
...
inline void MoveSplineInit::DisableTransportPathTransformations()
{
