...
UnitMoveType SelectSpeedType(uint32 moveFlags)
{
    if (moveFlags & MOVEMENTFLAG_FLYING) {
        ...
        if (moveFlags &
            MOVEMENTFLAG_BACKWARD /*&& speed_obj.swim >= speed_obj.swim_back*/)
        ...
        return MOVE_WALK;
             ...
             MOVEMENTFLAG_BACKWARD /*&& speed_obj.run >= speed_obj.run_back*/)
...
int32 MoveSplineInit::Launch()
{
    ...
    bool transport = unit->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) &&
                     unit->GetTransGUID();
    Location real_position;
    // there is a big chance that current position is unknown if current state
    // is not finalized, need compute it this also allows CalculatePath spline
    // position and update map position in much greater intervals Don't compute
    // for transport movement if the unit is in a motion between two transports
    ...
    if (!args.flags.orientationInversed) {
        moveFlags =
            (moveFlags & ~(MOVEMENTFLAG_BACKWARD)) | MOVEMENTFLAG_FORWARD;
    }
    else {
        moveFlags =
            (moveFlags & ~(MOVEMENTFLAG_FORWARD)) | MOVEMENTFLAG_BACKWARD;
    }
    ...
    if (!args.HasVelocity) {
        // If spline is initialized with SetWalk method it only means we need to
        // select walk move speed for it but not add walk flag to unit
    ...
    args.velocity =
        std::min(args.velocity,
                 args.flags.catmullrom || args.flags.flying
                     ? 50.0f
...
MoveSplineInit::MoveSplineInit(Unit* m) : unit(m)
{
    ...
    args.TransformForTransport =
        unit->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) &&
        unit->GetTransGUID();
    // mix existing state into new
    args.flags.walkmode =
        unit->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_WALKING);
