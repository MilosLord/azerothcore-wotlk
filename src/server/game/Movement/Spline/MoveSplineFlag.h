...
public:
    enum eFlags {
        None = 0x00000000,
        // x00-xFF(first byte) used as animation Ids storage in pair with
        // Animation flag
        Done    = 0x00000100,
        Falling = 0x00000200, // Affects elevation computation, can't be
                              // combined with Parabolic flag
        No_Spline = 0x00000400,
        Parabolic = 0x00000800, // Affects elevation computation, can't be
                                // combined with Falling flag
        Walkmode = 0x00001000,
        Flying = 0x00002000, // Smooth movement(Catmullrom interpolation mode),
                             // flying animation
        ...
        Enter_Cycle      = 0x00100000, // Everytimes appears with cyclic flag in
                                  // monster move packet, erases first spline
                                  // vertex after first cycle done
        ...
        // animation ids stored here, see AnimType enum, used with Animation
        // flag
        Mask_Animations = 0xFF,
        // flags that shouldn't be appended into
        // SMSG_MONSTER_MOVE\SMSG_MONSTER_MOVE_TRANSPORT packet, should be more
        // probably
        Mask_No_Monster_Move = Mask_Final_Facing | Mask_Animations | Done,
        // CatmullRom interpolation mode used
        Mask_CatmullRom = Flying | Catmullrom,
        // Unused, not suported flags
        Mask_Unused = No_Spline | Enter_Cycle | Frozen | Unknown7 | Unknown8 |
                      Unknown10 | Unknown11 | Unknown12 | Unknown13
    };

    inline uint32&                     raw() { return (uint32&)*this; }
    [[nodiscard]] inline const uint32& raw() const
    {
    ...
    {
        raw() = (raw() & ~(Mask_Animations | Falling | Parabolic)) | Animation |
                anim;
    }
    void EnableParabolic()
    {
        raw() = (raw() & ~(Mask_Animations | Falling | Animation)) | Parabolic;
    }
    void EnableFalling()
    {
        raw() = (raw() & ~(Mask_Animations | Parabolic | Flying | Animation)) |
                Falling;
    }
    ...
    {
        raw() = (raw() & ~TransportExit) | TransportEnter;
    }
    void EnableTransportExit()
    {
        raw() = (raw() & ~TransportEnter) | TransportExit;
    }
