...
// MoveSpline represents smooth catmullrom or linear curve and point that moves
// belong it curve can be cyclic - in this case movement will be cyclic point
// can have vertical acceleration motion componemt(used in fall, parabolic
// movement)
class MoveSpline {
public:
...
protected:
    [[nodiscard]] const MySpline::ControlArray& getPath() const
    {
    ...
    [[nodiscard]] int32 next_timestamp() const
    {
        return spline.length(point_Idx + 1);
    }
    [[nodiscard]] int32 segment_time_elapsed() const
    {
        return next_timestamp() - time_passed;
    }

public:
    [[nodiscard]] int32 timeElapsed() const
    {
        return Duration() - time_passed;
    } // xinef: moved to public for waypoint movegen
    [[nodiscard]] int32 timePassed() const
    {
        return time_passed;
    } // xinef: moved to public for waypoint movegen
...
public:
    ...
    [[nodiscard]] Vector3 FinalDestination() const
    {
        return Initialized() ? spline.getPoint(spline.last()) : Vector3();
    }
    [[nodiscard]] Vector3 CurrentDestination() const
    {
        return Initialized() ? spline.getPoint(point_Idx + 1) : Vector3();
    }
    ...
    [[nodiscard]] bool        HasStarted() const { return time_passed > 0; }
};
} // namespace Movement
