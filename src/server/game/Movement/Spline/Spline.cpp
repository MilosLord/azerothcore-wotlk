...
SplineBase::EvaluationMethtod
    SplineBase::derivative_evaluators[SplineBase::ModesEnd] = {
...
SplineBase::InitMethtod SplineBase::initializers[SplineBase::ModesEnd] = {
    ...
    &SplineBase::InitCatmullRom, // we should use catmullrom initializer even
                                 // for linear mode! (client's internal
...
using G3D::Matrix4;
static const Matrix4 s_catmullRomCoeffs(-0.5f,
                                        1.5f,
                                        -1.5f,
                                        0.5f,
                                        1.f,
...
static const Matrix4 s_Bezier3Coeffs(-1.f,
                                     3.f,
                                     -3.f,
                                     1.f,
                                     3.f,
...
/*  classic view:
inline void C_Evaluate(const Vector3 *vertice, float t, const float
(&matrix)[4][4], Vector3 &position)
{
    ...
    while ( i < 4 )
    {
        c = matrix[0][i]*tvec.x + matrix[1][i]*tvec.y + matrix[2][i]*tvec.z +
matrix[3][i];
...
inline void C_Evaluate(const Vector3* vertice,
                       float          t,
                       const Matrix4& matr,
                       Vector3&       result)
{
...
inline void C_Evaluate_Derivative(const Vector3* vertice,
                                  float          t,
                                  const Matrix4& matr,
                                  Vector3&       result)
{
...
void SplineBase::EvaluateLinear(index_type index,
                                float      u,
...
void SplineBase::EvaluateCatmullRom(index_type index,
                                    float      t,
                                    Vector3&   result) const
{
    ASSERT(index >= index_lo && index < index_hi);
...
void SplineBase::EvaluateBezier3(index_type index,
                                 float      t,
                                 Vector3&   result) const
{
    index *= 3u;
...
void SplineBase::EvaluateDerivativeLinear(index_type index,
                                          float,
                                          Vector3& result) const
{
    ASSERT(index >= index_lo && index < index_hi);
...
void SplineBase::EvaluateDerivativeCatmullRom(index_type index,
                                              float      t,
                                              Vector3&   result) const
{
    ASSERT(index >= index_lo && index < index_hi);
...
void SplineBase::EvaluateDerivativeBezier3(index_type index,
                                           float      t,
                                           Vector3&   result) const
{
    index *= 3u;
...
float SplineBase::SegLengthCatmullRom(index_type index) const
{
    ...
    while (i <= STEPS_PER_SEGMENT) {
        C_Evaluate(p,
                   float(i) / float(STEPS_PER_SEGMENT),
                   s_catmullRomCoeffs,
                   nextPos);
...
float SplineBase::SegLengthBezier3(index_type index) const
{
    ...
    while (i <= STEPS_PER_SEGMENT) {
        C_Evaluate(
            p, float(i) / float(STEPS_PER_SEGMENT), s_Bezier3Coeffs, nextPos);
        length += (nextPos - curPos).length();
        curPos = nextPos;
        ++i;
    }
...
void SplineBase::init_spline(const Vector3* controls,
                             index_type     count,
                             EvaluationMode m)
{
    m_mode = m;
...
void SplineBase::init_cyclic_spline(const Vector3* controls,
                                    index_type     count,
                                    EvaluationMode m,
...
void SplineBase::InitLinear(const Vector3* controls,
                            index_type     count,
                            bool           cyclic,
                            index_type     cyclic_point)
{
    ...
    // these points are required for proper C_Evaluate and C_Evaluate_Derivative
    // methtod work
    if (cyclic)
...
void SplineBase::InitCatmullRom(const Vector3* controls,
                                index_type     count,
                                bool           cyclic,
                                index_type     cyclic_point)
{
...
void SplineBase::InitBezier3(const Vector3* controls,
                             index_type     count,
                             bool /*cyclic*/,
                             index_type /*cyclic_point*/)
{
...
std::string SplineBase::ToString() const
{
    ...
    const char*       mode_str[ModesEnd] = {
        "Linear", "CatmullRom", "Bezier3", "Uninitialized"};
