...
protected:
    ...
    typedef void (SplineBase::*EvaluationMethtod)(index_type,
                                                  float,
                                                  Vector3&) const;
    ...
    typedef void (SplineBase::*InitMethtod)(const Vector3*,
                                            index_type,
                                            bool,
                                            index_type);
    ...
    [[nodiscard]] float UninitializedSplineSegLenghtMethod(index_type) const
    {
        ABORT();
    }
    void
    UninitializedSplineInitMethod(Vector3 const*, index_type, bool, index_type)
    {
        ABORT();
    }
...
public:
    ...
    /** Caclulates the position for given segment Idx, and percent of segment
       length t
    ...
    /** Caclulates derivation in index Idx, and percent of segment length t
        @param Idx - spline segment index, should be in range [first, last)
        @param t  - percent of spline segment length, assumes that t in range
       [0, 1]
     */
    ...
    /**  Bounds for spline indexes. All indexes should be in range [first,
     * last). */
    ...
    [[nodiscard]] const Vector3& getPoint(index_type i) const
    {
        return points[i];
    }

    /** Initializes spline. Don't call other methods while spline not
     * initialized. */
    void
    init_spline(const Vector3* controls, index_type count, EvaluationMode m);
    void init_cyclic_spline(const Vector3* controls,
                            index_type     count,
                            EvaluationMode m,
    ...
    /** Calculates distance between [i; i+1] points, assumes that index i is in
     * bounds. */
    [[nodiscard]] float SegLength(index_type i) const
    {
...
public:
    ...
    /** Calculates the position for given segment Idx, and percent of segment
       length t
    ...
    /** Initializes lengths in some custom way
        Note that value returned by cacher must be greater or equal to previous
       value. */
    template <class T>
    ...
    [[nodiscard]] length_type length(index_type first, index_type last) const
    {
        return lengths[last] - lengths[first];
    }
    [[nodiscard]] length_type length(index_type Idx) const
    {
        return lengths[Idx];
    }
