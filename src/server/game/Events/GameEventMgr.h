...
enum GameEventState {
    ...
    GAMEEVENT_WORLD_NEXTPHASE =
        3, // conditions are met, now 'length' timer to start next event
...
typedef std::map<uint32 /*condition id*/, GameEventFinishCondition>
...
struct GameEventData {
    ...
    time_t nextstart{
        0}; // after this time the follow-up events count this phase completed
    uint32 occurence{0}; // time between end and start
    uint32 length{
        0}; // length of the event (minutes) after finishing all conditions
    HolidayIds     holiday_id{HOLIDAY_NONE};
    uint8          holidayStage;
    GameEventState state{
        GAMEEVENT_NORMAL}; // state of the game event, these are saved into the
                           // game_event table on change!
    GameEventConditionMap conditions; // conditions to finish
    std::set<uint16 /*gameevent id*/>
        prerequisite_events; // events that must be completed before starting
                             // this event
...
public:
    ...
    [[nodiscard]] ActiveEvents const&  GetActiveEventList() const
    {
        return m_ActiveEvents;
    }
    [[nodiscard]] GameEventDataMap const& GetEventMap() const
    {
        return mGameEvent;
    }
    ...
    void   HandleQuestComplete(
          uint32 quest_id); // called on world event type quest completions
...
private:
    ...
    void RunSmartAIScripts(
        uint16 event_id,
        bool   activate); //! Runs SMART_EVENT_GAME_EVENT_START/_END SAI
    bool CheckOneGameEventConditions(uint16 event_id);
    void SaveWorldEventStateToDB(uint16 event_id);
    bool hasCreatureQuestActiveEventExcept(uint32 quest_id, uint16 event_id);
    bool hasGameObjectQuestActiveEventExcept(uint32 quest_id, uint16 event_id);
    bool hasCreatureActiveEventExcept(ObjectGuid::LowType creature_guid,
                                      uint16              event_id);
    bool hasGameObjectActiveEventExcept(ObjectGuid::LowType go_guid,
                                        uint16              event_id);
    ...
    typedef std::map<uint32 /*quest id*/, GameEventQuestToEventConditionNum>
        QuestIdToEventConditionMap;
    typedef std::pair<ObjectGuid::LowType /*guid*/, uint32 /*npcflag*/>
                                       GuidNPCFlagPair;
    typedef std::list<GuidNPCFlagPair> NPCFlagList;
    typedef std::vector<NPCFlagList>   GameEventNPCFlagMap;
    typedef std::vector<uint32>        GameEventBitmask;
    typedef std::unordered_map<uint32, std::vector<uint32>>
