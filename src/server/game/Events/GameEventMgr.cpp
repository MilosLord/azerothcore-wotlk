...
bool GameEventMgr::CheckOneGameEvent(uint16 entry) const
{
    ...
    case GAMEEVENT_NORMAL: {
        ...
        return mGameEvent[entry].start < currenttime &&
               ...
               (currenttime - mGameEvent[entry].start) %
                       (mGameEvent[entry].occurence * MINUTE) <
                   mGameEvent[entry].length * MINUTE;
    }
    ...
    case GAMEEVENT_WORLD_INACTIVE: {
        time_t currenttime = GameTime::GetGameTime().count();
        for (std::set<uint16>::const_iterator itr =
                 mGameEvent[entry].prerequisite_events.begin();
             itr != mGameEvent[entry].prerequisite_events.end();
             ++itr) {
            if ((mGameEvent[*itr].state != GAMEEVENT_WORLD_NEXTPHASE &&
                 mGameEvent[*itr].state !=
                     GAMEEVENT_WORLD_FINISHED) || // if prereq not in nextphase
                                                  // or finished state, then
                                                  // can't start this one
                mGameEvent[*itr].nextstart >
                    currenttime) // if not in nextphase state for long enough,
                                 // can't start this one
                return false;
        }
        // all prerequisite events are met
        // but if there are no prerequisites, this can be only activated through
        // gm command
...
uint32 GameEventMgr::NextCheck(uint16 entry) const
{
    ...
    // for NEXTPHASE state world events, return the delay to start the next
    // event, so the followup event will be checked correctly
    if ((mGameEvent[entry].state == GAMEEVENT_WORLD_NEXTPHASE ||
         mGameEvent[entry].state == GAMEEVENT_WORLD_FINISHED) &&
        mGameEvent[entry].nextstart >= currenttime)
        return uint32(mGameEvent[entry].nextstart - currenttime);

    // for CONDITIONS state world events, return the length of the wait period,
    // so if the conditions are met, this check will be called again to set the
    // timer as NEXTPHASE event
    ...
    if ((((currenttime - mGameEvent[entry].start) %
          (mGameEvent[entry].occurence * 60)) <
         (mGameEvent[entry].length * 60)))
        // we return the delay before it ends
        delay = (mGameEvent[entry].length * MINUTE) -
                ((currenttime - mGameEvent[entry].start) %
                 (mGameEvent[entry].occurence * MINUTE));
    else // not in window, we return the delay before next start
        delay = (mGameEvent[entry].occurence * MINUTE) -
                ((currenttime - mGameEvent[entry].start) %
                 (mGameEvent[entry].occurence * MINUTE));
...
{
    if (DisableMgr::IsDisabledFor(DISABLE_TYPE_GAME_EVENT, event_id, nullptr) &&
        !overwrite) {
        return false;
    }
    ...
    if (data.state == GAMEEVENT_NORMAL || data.state == GAMEEVENT_INTERNAL) {
        ...
        // force game event update to set the update timer if conditions were
        // met from a command this update is needed to possibly start events
        // dependent on the started one or to scedule another update where the
        // next event will be started
...
void GameEventMgr::StopEvent(uint16 event_id, bool overwrite)
{
    GameEventData& data = mGameEvent[event_id];
    bool           serverwide_evt =
        data.state != GAMEEVENT_NORMAL && data.state != GAMEEVENT_INTERNAL;
    ...
    if (overwrite && !serverwide_evt) {
        ...
        if (overwrite || data.state != GAMEEVENT_WORLD_FINISHED) {
            ...
            for (itr = data.conditions.begin(); itr != data.conditions.end();
                 ++itr)
                itr->second.done = 0;
...
{
    {
        ...
        QueryResult result = WorldDatabase.Query(
            ...
            "UNIX_TIMESTAMP(end_time), occurence, length, holiday, "
            "holidayStage, description, world_event, announce FROM game_event");
        if (!result) {
            mGameEvent.clear();
            LOG_WARN(
                "server.loading",
        ...
        do {
            ...
            if (event_id == 0) {
                LOG_ERROR("sql.sql",
                          "`game_event` game event entry 0 is reserved and "
                          "can't be used.");
                continue;
            }
            ...
            if (fields[2].IsNull())
                endtime = GameTime::GetGameTime().count() +
                          63072000; // add 2 years to current date
            ...
            if (pGameEvent.length == 0 &&
                pGameEvent.state ==
                    GAMEEVENT_NORMAL) // length>0 is validity check
            {
                LOG_ERROR("sql.sql",
                          "`game_event` game event id ({}) isn't a world event "
                          "and has length = 0, thus it can't be used.",
                          event_id);
                continue;
            }
            ...
            if (pGameEvent.holiday_id != HOLIDAY_NONE) {
                if (!sHolidaysStore.LookupEntry(pGameEvent.holiday_id)) {
                    LOG_ERROR("sql.sql",
                              "`game_event` game event id ({}) have not "
                              "existed holiday id {}.",
                              event_id,
                              pGameEvent.holiday_id);
    ...
    LOG_INFO("server.loading", "Loading Game Event Saves Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Game Event Saves In Game Events. DB Table "
                     "`game_event_save` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_save` game event entry ({}) is out of "
                        "range compared to max event entry in `game_event`",
                        event_id);
                    continue;
                }
                ...
                if (mGameEvent[event_id].state != GAMEEVENT_NORMAL &&
                    ...
                    mGameEvent[event_id].state =
                        (GameEventState)(fields[1].Get<uint8>());
                    mGameEvent[event_id].nextstart =
                        time_t(fields[2].Get<uint32>());
                }
                else {
                    LOG_ERROR("sql.sql",
                              "game_event_save includes event save for "
                              "non-worldevent id {}",
                              event_id);
                    continue;
                }
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Game Event Saves In Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Prerequisite Data...");
    {
        ...
        //                                                   0             1
        QueryResult result =
            WorldDatabase.Query("SELECT eventEntry, prerequisite_event FROM "
                                "game_event_prerequisite");
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Game Rvent Prerequisites in Game Events. DB "
            ...
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_prerequisite` game event id ({}) is out "
                        "of range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
                ...
                if (mGameEvent[event_id].state != GAMEEVENT_NORMAL &&
                    ...
                    if (prerequisite_event >= mGameEvent.size()) {
                        LOG_ERROR("sql.sql",
                                  "`game_event_prerequisite` game event "
                                  "prerequisite id ({}) is out of range "
                                  "compared to max event id in `game_event`",
                                  prerequisite_event);
                        continue;
                    }
                    mGameEvent[event_id].prerequisite_events.insert(
                        prerequisite_event);
                }
                else {
                    LOG_ERROR("sql.sql",
                              "game_event_prerequisiste includes event entry "
                              "for non-worldevent id {}",
                              event_id);
                    continue;
                }
            ...
            LOG_INFO(
                "server.loading",
                ">> Loaded {} game event prerequisites in Game Events in {} ms",
                count,
                GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Creature Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 creatures in game events. DB table "
                     "`game_event_creature` is empty");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (!data) {
                    LOG_ERROR("sql.sql",
                              "`game_event_creature` contains creature (GUID: "
                              "{}) not found in `creature` table.",
                              guid);
                    continue;
                }
                ...
                if (internal_event_id < 0 ||
                    internal_event_id >=
                        int32(mGameEventCreatureGuids.size())) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_creature` game event id ({}) is out of "
                        "range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }

                GuidLowList& crelist =
                    mGameEventCreatureGuids[internal_event_id];
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Creatures In Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event GO Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 gameobjects in game events. DB table "
                     "`game_event_gameobject` is empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                GameObjectData const* data =
                    sObjectMgr->GetGameObjectData(guid);
                if (!data) {
                    LOG_ERROR("sql.sql",
                              "`game_event_gameobject` contains gameobject "
                              "(GUID: {}) not found in `gameobject` table.",
                              guid);
                    continue;
                }
                ...
                if (internal_event_id < 0 ||
                    ...
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_gameobject` game event id ({}) is out of "
                        "range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
                ...
                GuidLowList& golist =
                    mGameEventGameobjectGuids[internal_event_id];
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Gameobjects In Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
             ...
             "Loading Game Event Model/Equipment Change Data...");
    {
        ...
        //                                                     5 6
        QueryResult result = WorldDatabase.Query(
            "SELECT creature.guid, creature.id1, creature.id2, creature.id3, "
            "game_event_model_equip.eventEntry, "
            "game_event_model_equip.modelid, "
            "game_event_model_equip.equipment_id "
            "FROM creature JOIN game_event_model_equip ON "
            "creature.guid=game_event_model_equip.guid");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Model/Equipment Changes In Game Events. DB "
            ...
            do {
                ...
                if (event_id >= mGameEventModelEquip.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_model_equip` game event id ({}) is out of "
                        "range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
                ...
                if (newModelEquipSet.equipment_id > 0) {
                    int8 equipId =
                        static_cast<int8>(newModelEquipSet.equipment_id);
                    if ((!sObjectMgr->GetEquipmentInfo(entry, equipId)) ||
                        (entry2 &&
                         !sObjectMgr->GetEquipmentInfo(entry2, equipId)) ||
                        (entry3 &&
                         !sObjectMgr->GetEquipmentInfo(entry3, equipId))) {
                        LOG_ERROR(
                            "sql.sql",
                            "Table `game_event_model_equip` have creature "
                            "(Guid: {}) with equipment_id {} not found in "
                            "table `creature_equip_template`, set to no "
                            "equipment.",
                            guid,
                ...
                equiplist.push_back(std::pair<ObjectGuid::LowType, ModelEquip>(
                    guid, newModelEquipSet));
            ...
            LOG_INFO(
                "server.loading",
                ">> Loaded {} Model/Equipment Changes In Game Events in {} ms",
                count,
                GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Quest Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Quests Additions In Game Events. DB Table "
                     "`game_event_creature_quest` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEventCreatureQuests.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_creature_quest` game event id ({}) is out "
                        "of range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Quests Additions In Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event GO Quest Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 go Quests Additions In Game Events. DB Table "
                     "`game_event_gameobject_quest` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEventGameObjectQuests.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_gameobject_quest` game event id ({}) is "
                        "out of range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
    ...
    LOG_INFO("server.loading", "Loading Game Event Quest Condition Data...");
    {
        ...
        //                                                 0       1         2 3
        QueryResult result =
            WorldDatabase.Query("SELECT quest, eventEntry, condition_id, num "
                                "FROM game_event_quest_condition");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 quest event Conditions In Game Events. DB "
            ...
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_quest_condition` game event id ({}) is "
                        "out of range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
            ...
            LOG_INFO(
                "server.loading",
                ">> Loaded {} quest event conditions in Game Events in {} ms",
                count,
                GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Condition Data...");
    {
        ...
        //                                                  0          1 2 3 4
        QueryResult result = WorldDatabase.Query(
            "SELECT eventEntry, condition_id, req_num, max_world_state_field, "
            "done_world_state_field FROM game_event_condition");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Conditions In Game Events. DB table "
                     "`game_event_condition` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_condition` game event id ({}) is out of "
                        "range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }

                mGameEvent[event_id].conditions[condition].reqNum =
                    fields[2].Get<float>();
                mGameEvent[event_id].conditions[condition].done = 0;
                mGameEvent[event_id].conditions[condition].max_world_state =
                    fields[3].Get<uint16>();
                mGameEvent[event_id].conditions[condition].done_world_state =
                    fields[4].Get<uint16>();
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} conditions in Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Condition Save Data...");
    {
        ...
        //                                                      0           1 2
        QueryResult result =
            CharacterDatabase.Query("SELECT eventEntry, condition_id, done "
                                    "FROM game_event_condition_save");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Condition Saves In Game Events. DB Table "
                     "`game_event_condition_save` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_condition_save` game event id ({}) is out "
                        "of range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
                ...
                if (itr != mGameEvent[event_id].conditions.end()) {
                    ...
                    LOG_ERROR("sql.sql",
                              "game_event_condition_save contains not present "
                              "condition evt id {} cond id {}",
                              event_id,
                              condition);
                    continue;
                }
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Condition Saves In Game Events In {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event NPCflag Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Npcflags In Game Events. DB Table "
                     "`game_event_npcflag` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_npcflag` game event id ({}) is out of "
                        "range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }

                mGameEventNPCFlags[event_id].push_back(
                    GuidNPCFlagPair(guid, npcflag));

                ++count;
            } while (result->NextRow());

            LOG_INFO("server.loading",
                     ">> Loaded {} Npcflags In Game Events In {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
             ...
             "Loading Game Event Seasonal Quest Relations...");
    {
        ...
        //                                                  0          1
        QueryResult result =
            WorldDatabase.Query("SELECT questId, eventEntry FROM "
                                "game_event_seasonal_questrelation");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Seasonal Quests Additions In Game Events. DB "
                     "Table `game_event_seasonal_questrelation` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                uint32 eventEntry =
                    fields[1].Get<uint32>(); /// @todo: Change to uint8

                Quest* questTemplate =
                    const_cast<Quest*>(sObjectMgr->GetQuestTemplate(questId));

                if (!questTemplate) {
                    LOG_ERROR("sql.sql",
                              "`game_event_seasonal_questrelation` quest id "
                ...
                if (eventEntry >= mGameEvent.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_seasonal_questrelation` event id ({}) is "
                        "out of range compared to max event in `game_event`",
                        eventEntry);
                    continue;
                }
    ...
    LOG_INFO("server.loading", "Loading Game Event Vendor Additions Data...");
    {
        ...
        QueryResult result = WorldDatabase.Query(
            ...
            "FROM game_event_npc_vendor ORDER BY guid, slot ASC");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Vendor Additions In Game Events. DB Table "
                     "`game_event_npc_vendor` Is Empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEventVendors.size()) {
                    LOG_ERROR(
                        "sql.sql",
                        "`game_event_npc_vendor` game event id ({}) is out of "
                        "range compared to max event id in `game_event`",
                        event_id);
                    continue;
                }
                ...
                // get the event npc flag for checking if the npc will be vendor
                // during the event or not
                uint32       event_npc_flag = 0;
                NPCFlagList& flist          = mGameEventNPCFlags[event_id];
                for (NPCFlagList::const_iterator itr = flist.begin();
                     itr != flist.end();
                     ++itr) {
                    if (itr->first == guid) {
                ...
                if (CreatureData const* data =
                        sObjectMgr->GetCreatureData(guid))
                    newEntry.entry = data->id1;

                // check validity with event's npcflag
                if (!sObjectMgr->IsVendorItemValid(newEntry.entry,
                                                   newEntry.item,
                                                   newEntry.maxcount,
                                                   newEntry.incrtime,
                                                   newEntry.ExtendedCost,
                                                   nullptr,
                                                   nullptr,
                                                   event_npc_flag))
                    continue;
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Vendor Additions In Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Battleground Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Battleground Holidays In Game Events. DB "
                     "table `game_event_battleground_holiday` is empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (event_id >= mGameEvent.size()) {
                    LOG_ERROR("sql.sql",
                              "`game_event_battleground_holiday` game event id "
                              "({}) is out of range compared to max event id "
                              "in `game_event`",
                              event_id);
                    continue;
                }

                mGameEventBattlegroundHolidays[event_id] =
                    fields[1].Get<uint32>();
            ...
            LOG_INFO(
                "server.loading",
                ">> Loaded {} Battleground Holidays In Game Events in {} ms",
                count,
                GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Game Event Pool Data...");
    {
        ...
        //                                                               0 1
        QueryResult result =
            WorldDatabase.Query("SELECT pool_template.entry, "
                                "game_event_pool.eventEntry FROM pool_template"
                                " JOIN game_event_pool ON pool_template.entry "
                                "= game_event_pool.pool_entry");

        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 Pools For Game Events. DB Table "
            ...
            do {
                ...
                if (internal_event_id < 0 ||
                    internal_event_id >= int32(mGameEventPoolIds.size())) {
                    LOG_ERROR("sql.sql",
                              "`game_event_pool` game event id ({}) is out of "
                              "range compared to max event id in `game_event`",
                              event_id);
                    continue;
                }
                ...
                if (!sPoolMgr->CheckPool(entry)) {
                    LOG_ERROR(
                        "sql.sql",
                        "Pool Id ({}) has all creatures or gameobjects with "
                        "explicit chance sum <>100 and no equal chance "
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Pools For Game Events in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
...
void GameEventMgr::LoadHolidayDates()
{
    ...
    do {
        ...
        HolidaysEntry* entry =
            const_cast<HolidaysEntry*>(sHolidaysStore.LookupEntry(holidayId));
        if (!entry) {
            LOG_ERROR("sql.sql",
                      "holiday_dates entry has invalid holiday id {}.",
                      holidayId);
            continue;
        }
        ...
        if (dateId >= MAX_HOLIDAY_DATES) {
            LOG_ERROR("sql.sql",
                      "holiday_dates entry has out of range date_id {}.",
                      dateId);
            continue;
        }
        ...
        auto itr = std::lower_bound(
            modifiedHolidays.begin(), modifiedHolidays.end(), entry->Id);
...
uint32 GameEventMgr::GetNPCFlag(Creature* cr)
{
    ...
    for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
         e_itr != m_ActiveEvents.end();
         ++e_itr) {
...
{
    QueryResult result =
        WorldDatabase.Query("SELECT MAX(eventEntry) FROM game_event");
    if (result) {
        Field* fields = result->Fetch();
...
void GameEventMgr::StartArenaSeason()
{
    ...
    if (!result) {
        LOG_ERROR("gameevent",
                  "ArenaSeason ({}) must be an existant Arena Season",
                  season);
        return;
    }
    ...
    if (eventId >= mGameEvent.size()) {
        LOG_ERROR("gameevent",
                  "EventEntry {} for ArenaSeason ({}) does not exists",
                  eventId,
                  season);
        return;
    }
...
uint32 GameEventMgr::Update() // return the next event delay in ms
{
    ...
    for (uint16 itr = 1; itr < mGameEvent.size(); ++itr) {
        ...
        if (CheckOneGameEvent(itr)) {
            // if the world event is in NEXTPHASE state, and the time has passed
            // to finish this event, then do so
            if (mGameEvent[itr].state == GAMEEVENT_WORLD_NEXTPHASE &&
                mGameEvent[itr].nextstart <= currenttime) {
            ...
            else if (mGameEvent[itr].state == GAMEEVENT_WORLD_CONDITIONS &&
                     CheckOneGameEventConditions(itr))
                // changed, save to DB the gameevent state, will be updated in
                // next update cycle
                SaveWorldEventStateToDB(itr);
    ...
    // following the activate - deactivate order, deactivating the first event
    // later will leave the spawn in (wont disappear then reappear clientside)
    for (std::set<uint16>::iterator itr = activate.begin();
         itr != activate.end();
         ++itr)
        // start the event
        ...
        if (StartEvent(*itr))
            nextEventDelay = 0;
    for (std::set<uint16>::iterator itr = deactivate.begin();
         itr != deactivate.end();
         ++itr)
        StopEvent(*itr);

    LOG_DEBUG("gameevent",
              "Next game event check in {} seconds.",
              nextEventDelay + 1);
    return (nextEventDelay + 1) *
           IN_MILLISECONDS; // Add 1 second to be sure event has started/stopped
                            // at next call
}
...
void GameEventMgr::ApplyNewEvent(uint16 event_id)
{
    ...
    if (announce == 1 ||
        (announce == 2 && sWorld->getIntConfig(CONFIG_EVENT_ANNOUNCE)))
        sWorld->SendWorldText(LANG_EVENTMESSAGE,
    ...
    // If event's worldstate is 0, it means the event hasn't been started yet.
    // In that case, reset seasonal quests. When event ends (if it expires or if
    // it's stopped via commands) worldstate will be set to 0 again, ready for
    // another seasonal quest reset.
...
{
    std::unordered_map<uint32, std::unordered_set<ObjectGuid::LowType>>
        creaturesByMap;

    // go through the creatures whose npcflags are changed in the event
    for (NPCFlagList::iterator itr = mGameEventNPCFlags[event_id].begin();
         itr != mGameEventNPCFlags[event_id].end();
         ++itr) {
        // get the creature data from the low guid to get the entry, to be able
        // to find out the whole guid
    ...
    for (auto const& p : creaturesByMap) {
        sMapMgr->DoForAllMapsWithMapId(p.first, [this, &p](Map* map) {
            for (auto& spawnId : p.second) {
                ...
                for (auto itr = creatureBounds.first;
                     itr != creatureBounds.second;
                     ++itr) {
                    ...
                    // reset gossip options, since the flag change might have
                    // added / removed some
...
{
    uint32 mask = 0;
    for (ActiveEvents::const_iterator itr = m_ActiveEvents.begin();
         itr != m_ActiveEvents.end();
         ++itr)
...
{
    for (NPCVendorList::iterator itr = mGameEventVendors[event_id].begin();
         itr != mGameEventVendors[event_id].end();
         ++itr) {
        if (activate)
            sObjectMgr->AddVendorItem(itr->entry,
                                      itr->item,
                                      itr->maxcount,
                                      itr->incrtime,
                                      itr->ExtendedCost,
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 ||
        internal_event_id >= int32(mGameEventCreatureGuids.size())) {
        LOG_ERROR("gameevent",
                  "GameEventMgr::GameEventSpawn attempt access to out of range "
                  "mGameEventCreatureGuids element {} (size: {})",
                  internal_event_id,
                  mGameEventCreatureGuids.size());
        return;
    }

    for (GuidLowList::iterator itr =
             mGameEventCreatureGuids[internal_event_id].begin();
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr)) {
            ...
            if (!map->Instanceable() &&
                map->IsGridLoaded(data->posX, data->posY)) {
    ...
    if (internal_event_id >= int32(mGameEventGameobjectGuids.size())) {
        LOG_ERROR("gameevent",
                  "GameEventMgr::GameEventSpawn attempt access to out of range "
                  "mGameEventGameobjectGuids element {} (size: {})",
                  internal_event_id,
                  mGameEventGameobjectGuids.size());
        return;
    }

    for (GuidLowList::iterator itr =
             mGameEventGameobjectGuids[internal_event_id].begin();
        ...
        if (GameObjectData const* data = sObjectMgr->GetGameObjectData(*itr)) {
            ...
            Map* map = sMapMgr->CreateBaseMap(data->mapid);
            // We use current coords to unspawn, not spawn coords since creature
            // can have changed grid
            if (!map->Instanceable() &&
                map->IsGridLoaded(data->posX, data->posY)) {
                GameObject* pGameobject =
                    sObjectMgr->IsGameObjectStaticTransport(data->id)
                        ? new StaticTransport()
                        : new GameObject();
    ...
    if (internal_event_id >= int32(mGameEventPoolIds.size())) {
        LOG_ERROR("gameevent",
                  "GameEventMgr::GameEventSpawn attempt access to out of range "
                  "mGameEventPoolIds element {} (size: {})",
                  internal_event_id,
                  mGameEventPoolIds.size());
        return;
    }

    for (IdList::iterator itr = mGameEventPoolIds[internal_event_id].begin();
         itr != mGameEventPoolIds[internal_event_id].end();
         ++itr)
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 ||
        internal_event_id >= int32(mGameEventCreatureGuids.size())) {
        LOG_ERROR("gameevent",
                  "GameEventMgr::GameEventUnspawn attempt access to out of "
                  "range mGameEventCreatureGuids element {} (size: {})",
                  internal_event_id,
                  mGameEventCreatureGuids.size());
        return;
    }
    ...
    for (GuidLowList::iterator itr =
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr)) {
            ...
            sMapMgr->DoForAllMapsWithMapId(data->mapid, [&itr](Map* map) {
                auto creatureBounds =
                    map->GetCreatureBySpawnIdStore().equal_range(*itr);
                for (auto itr2 = creatureBounds.first;
                     itr2 != creatureBounds.second;) {
    ...
    if (internal_event_id >= int32(mGameEventGameobjectGuids.size())) {
        LOG_ERROR("gameevent",
                  "GameEventMgr::GameEventUnspawn attempt access to out of "
                  "range mGameEventGameobjectGuids element {} (size: {})",
                  internal_event_id,
                  mGameEventGameobjectGuids.size());
        return;
    }
    ...
    for (GuidLowList::iterator itr =
        ...
        if (GameObjectData const* data = sObjectMgr->GetGameObjectData(*itr)) {
            ...
            sMapMgr->DoForAllMapsWithMapId(data->mapid, [&itr](Map* map) {
                auto gameobjectBounds =
                    map->GetGameObjectBySpawnIdStore().equal_range(*itr);
                for (auto itr2 = gameobjectBounds.first;
                     itr2 != gameobjectBounds.second;) {
    ...
    if (internal_event_id >= int32(mGameEventPoolIds.size())) {
        LOG_ERROR("gameevent",
                  "GameEventMgr::GameEventUnspawn attempt access to out of "
                  "range mGameEventPoolIds element {} (size: {})",
                  internal_event_id,
                  mGameEventPoolIds.size());
        return;
    }
...
{
    for (ModelEquipList::iterator itr = mGameEventModelEquip[event_id].begin();
         itr != mGameEventModelEquip[event_id].end();
         ++itr) {
        // Remove the creature from grid
        ...
        sMapMgr->DoForAllMapsWithMapId(data->mapid, [&itr, activate](Map* map) {
            auto creatureBounds =
                map->GetCreatureBySpawnIdStore().equal_range(itr->first);
            for (auto itr2 = creatureBounds.first;
                 itr2 != creatureBounds.second;
                 ++itr2) {
                Creature* creature = itr2->second;
                if (activate) {
                    ...
                    if (itr->second.modelid > 0 &&
                        itr->second.modelid_prev != itr->second.modelid &&
                    ...
                    creature->LoadEquipment(itr->second.equipement_id_prev,
                                            true);
                    if (itr->second.modelid_prev > 0 &&
                        itr->second.modelid_prev != itr->second.modelid &&
                        sObjectMgr->GetCreatureModelInfo(
                                                     ...
                                                     uint16 event_id)
{
    for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
         ...
         ++e_itr) {
        if ((*e_itr) != event_id)
            for (QuestRelList::iterator itr =
                     mGameEventCreatureQuests[*e_itr].begin();
                                                       ...
                                                       uint16 event_id)
{
    for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
        ...
        if ((*e_itr) != event_id)
            for (QuestRelList::iterator itr =
                     mGameEventGameObjectQuests[*e_itr].begin();
...
bool GameEventMgr::hasCreatureActiveEventExcept(
    ObjectGuid::LowType creature_guid, uint16 event_id)
{
    for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
         e_itr != m_ActiveEvents.end();
         ++e_itr) {
...
void GameEventMgr::UpdateEventQuests(uint16 event_id, bool activate)
{
    ...
    for (itr = mGameEventCreatureQuests[event_id].begin();
         itr != mGameEventCreatureQuests[event_id].end();
         ++itr) {
        QuestRelations* CreatureQuestMap =
            sObjectMgr->GetCreatureQuestRelationMap();
        if (activate) // Add the pair(id, quest) to the multimap
            CreatureQuestMap->insert(
                QuestRelations::value_type(itr->first, itr->second));
        else {
            if (!hasCreatureQuestActiveEventExcept(itr->second, event_id)) {
                // Remove the pair(id, quest) from the multimap
                QuestRelations::iterator qitr =
                    CreatureQuestMap->find(itr->first);
                if (qitr == CreatureQuestMap->end())
                    continue;
                QuestRelations::iterator lastElement =
                    CreatureQuestMap->upper_bound(itr->first);
                for (; qitr != lastElement; ++qitr) {
                    if (qitr->second == itr->second) {
                        CreatureQuestMap->erase(
                            qitr); // iterator is now no more valid
                        break;     // but we can exit loop since the element is
                                   // found
                    }
                }
            }
    ...
    for (itr = mGameEventGameObjectQuests[event_id].begin();
         itr != mGameEventGameObjectQuests[event_id].end();
         ++itr) {
        QuestRelations* GameObjectQuestMap =
            sObjectMgr->GetGOQuestRelationMap();
        if (activate) // Add the pair(id, quest) to the multimap
            GameObjectQuestMap->insert(
                QuestRelations::value_type(itr->first, itr->second));
        else {
            if (!hasGameObjectQuestActiveEventExcept(itr->second, event_id)) {
                // Remove the pair(id, quest) from the multimap
                QuestRelations::iterator qitr =
                    GameObjectQuestMap->find(itr->first);
                if (qitr == GameObjectQuestMap->end())
                    continue;
                QuestRelations::iterator lastElement =
                    GameObjectQuestMap->upper_bound(itr->first);
                for (; qitr != lastElement; ++qitr) {
                    if (qitr->second == itr->second) {
                        GameObjectQuestMap->erase(
                            qitr); // iterator is now no more valid
                        break;     // but we can exit loop since the element is
                                   // found
                    }
...
void GameEventMgr::UpdateWorldStates(uint16 event_id, bool Activate)
{
    ...
    if (event.holiday_id != HOLIDAY_NONE) {
        BattlegroundTypeId bgTypeId =
            BattlegroundMgr::WeekendHolidayIdToBGType(event.holiday_id);
...
void GameEventMgr::HandleQuestComplete(uint32 quest_id)
{
    ...
    QuestIdToEventConditionMap::iterator itr =
        mQuestToEventConditions.find(quest_id);
    // quest is registered
    if (itr != mQuestToEventConditions.end()) {
        ...
        if (mGameEvent[event_id].state != GAMEEVENT_WORLD_CONDITIONS)
            return;
        GameEventConditionMap::iterator citr =
            mGameEvent[event_id].conditions.find(condition);
        ...
        if (citr != mGameEvent[event_id].conditions.end()) {
            ...
            if (citr->second.done < citr->second.reqNum) {
                ...
                trans->Append(stmt);
                CharacterDatabase.CommitTransaction(trans);
                // check if all conditions are met, if so, update the event
                // state
...
{
    for (GameEventConditionMap::const_iterator itr =
             mGameEvent[event_id].conditions.begin();
         itr != mGameEvent[event_id].conditions.end();
         ++itr)
    ...
    if (!mGameEvent[event_id].nextstart) {
        time_t currenttime = GameTime::GetGameTime().count();
        mGameEvent[event_id].nextstart =
            currenttime + mGameEvent[event_id].length * 60;
    }
    return true;
}
...
void GameEventMgr::SaveWorldEventStateToDB(uint16 event_id)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GAME_EVENT_SAVE);
    stmt->SetData(0, uint8(event_id));
    trans->Append(stmt);
    ...
    stmt->SetData(2,
                  mGameEvent[event_id].nextstart
                      ? uint32(mGameEvent[event_id].nextstart)
                      : 0);
    trans->Append(stmt);
    CharacterDatabase.CommitTransaction(trans);
}
...
void GameEventMgr::SendWorldStateUpdate(Player* player, uint16 event_id)
{
    ...
    for (itr = mGameEvent[event_id].conditions.begin();
         itr != mGameEvent[event_id].conditions.end();
         ++itr) {
        if (itr->second.done_world_state)
            player->SendUpdateWorldState(itr->second.done_world_state,
                                         (uint32)(itr->second.done));
        if (itr->second.max_world_state)
            player->SendUpdateWorldState(itr->second.max_world_state,
                                         (uint32)(itr->second.reqNum));
    }
}
...
public:
    GameEventAIHookWorker(uint16 eventId, bool activate)
        : _eventId(eventId), _activate(activate)
    {
    }
    ...
    void Visit(std::unordered_map<ObjectGuid, Creature*>& creatureMap)
    {
        for (auto const& p : creatureMap)
            if (p.second->IsInWorld() && !p.second->IsDuringRemoveFromWorld() &&
                p.second->FindMap() && p.second->IsAIEnabled && p.second->AI())
...
void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate)
{
    ...
    sMapMgr->DoForAllMaps([event_id, activate](Map* map) {
        ...
        TypeContainerVisitor<GameEventAIHookWorker,
...
void GameEventMgr::SetHolidayEventTime(GameEventData& event)
{
    ...
    {
        LOG_ERROR("sql.sql",
                  "Missing date or duration for holiday {}.",
                  event.holiday_id);
        return;
    }
    ...
    if (holiday->Looping) {
        event.occurence = 0;
        for (uint8 i = 0; i < MAX_HOLIDAY_DURATIONS && holiday->Duration[i];
             ++i) {
            event.occurence += holiday->Duration[i] * HOUR / MINUTE;
        }
    }

    bool singleDate = ((holiday->Date[0] >> 24) & 0x1F) ==
                      31; // Events with fixed date within year have - 1
...

    {
        ...
        if (singleDate) {
            ...
            timeInfo.tm_year -=
                1; // First try last year (event active through New Year)
        }
        else {
        ...
        if (curTime < startTime + event.length * MINUTE) {
            ...
            // date is due and not a singleDate event, try with next DBC date
            // (modified by holiday_dates) if none is found we don't modify
            // start date and use the one in game_event
        }
    }
}
...
bool IsHolidayActive(HolidayIds id)
{
    ...
    for (GameEventMgr::ActiveEvents::const_iterator itr = ae.begin();
         itr != ae.end();
         ++itr)
