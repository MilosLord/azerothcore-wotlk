...
/// Ban an account, duration will be parsed using TimeStringToSecs if it is
/// positive, otherwise permban
                             ...
                             std::string const& Author)
{
    ...
    // pussywizard: check existing ban to prevent overriding by a shorter one!
    // >_>
    LoginDatabasePreparedStatement* stmtAccountBanned =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BANNED);
    ...
    if (banresult &&
        ((*banresult)[0].Get<uint32>() == (*banresult)[1].Get<uint32>() ||
         ((*banresult)[1].Get<uint32>() >
              GameTime::GetGameTime().count() + DurationSecs &&
          DurationSecs)))
        return BAN_LONGER_EXISTS;

    // make sure there is only one active ban
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_NOT_BANNED);
    ...
    if (WorldSession* session = sWorld->FindSession(AccountID))
        if (session->GetPlayerName() != Author)
            session->KickPlayer(
                "Ban Account at condition "
                "'FindSession(account)->GetPlayerName() != author'");

    if (WorldSession* session = sWorld->FindOfflineSession(AccountID))
        if (session->GetPlayerName() != Author)
            session->KickPlayer(
                "Ban Account at condition "
                "'FindOfflineSession(account)->GetPlayerName() != author'");
    ...
    if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD)) {
        ...
        if (!IsPermanetly)
            sWorld->SendWorldText(
                ...
                Author.c_str(),
                AccountName.c_str(),
                secsToTimeString(TimeStringToSecs(Duration), true).c_str(),
                Reason.c_str());
        else
            sWorld->SendWorldText(LANG_BAN_ACCOUNT_YOUPERMBANNEDMESSAGE_WORLD,
                                  Author.c_str(),
                                  AccountName.c_str(),
                                  Reason.c_str());
    }
...
/// Ban an account by player name, duration will be parsed using
/// TimeStringToSecs if it is positive, otherwise permban
                                         ...
                                         std::string const& Author)
{
    ...
    uint32 AccountID =
        sCharacterCache->GetCharacterAccountIdByName(CharacterName);
    if (!AccountID)
        return BAN_NOTFOUND;
...
/// Ban an IP address, duration will be parsed using TimeStringToSecs if it is
/// positive, otherwise permban
                        ...
                        std::string const& Author)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD)) {
        ...
        if (IsPermanetly)
            sWorld->SendWorldText(LANG_BAN_IP_YOUPERMBANNEDMESSAGE_WORLD,
                                  Author.c_str(),
                                  IP.c_str(),
                                  Reason.c_str());
        else
...
/// Ban an character, duration will be parsed using TimeStringToSecs if it is
/// positive, otherwise permban
                               ...
                               std::string const& Author)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHARACTER_BAN);
    ...
    if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD)) {
        ...
        if (!IsPermanetly)
            ...
            sWorld->SendWorldText(LANG_BAN_CHARACTER_YOUPERMBANNEDMESSAGE_WORLD,
                                  Author.c_str(),
                                  CharacterName.c_str(),
                                  Reason.c_str());
    }
...
{
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_DEL_IP_NOT_BANNED);
    stmt->SetData(0, IP);
    LoginDatabase.Execute(stmt);
