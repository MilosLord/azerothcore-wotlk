...
void Graveyard::LoadGraveyardFromDB()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT ID, Map, x, y, z, Comment FROM game_graveyard");
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 graveyard. Table `game_graveyard` is empty!");
    ...
    do {
        ...
        if (!Utf8toWStr(Graveyard.name, Graveyard.wnameLow)) {
            LOG_ERROR("sql.sql",
                      "Wrong UTF8 name for id {} in `game_graveyard` table, "
                      "ignoring.",
                      ID);
            continue;
        }
...
GraveyardStruct const* Graveyard::GetDefaultGraveyard(TeamId teamId)
{
    ...
    return sGraveyard->GetGraveyard(teamId == TEAM_HORDE ? HORDE_GRAVEYARD
                                                         : ALLIANCE_GRAVEYARD);
}

GraveyardStruct const*
Graveyard::GetClosestGraveyard(Player* player, TeamId teamId, bool nearCorpse)
{
    uint32 graveyardOverride = 0;
    sScriptMgr->OnBeforeChooseGraveyard(
    ...
    if (!zoneId && !areaId) {
        if (z > -500) {
            LOG_ERROR("sql.sql",
                      "GetClosestGraveyard: unable to find zoneId and areaId "
                      "for map {} coords ({}, {}, {})",
                      mapId,
                      x,
                      y,
    ...
    //     then check faction
    //   if mapId != graveyard.mapId (ghost in instance) and search any
    //   graveyard associated
    //     then check faction

    // Fetch the graveyards linked to the areaId first, presumably the closer
    // ones.
    ...
    if (range.first == range.second && !map->IsBattlegroundOrArena()) {
        LOG_ERROR("sql.sql",
                  "Table `graveyard_zone` incomplete: Zone {} Team {} does not "
    ...
    for (; range.first != range.second; ++range.first) {
        ...
        GraveyardStruct const* entry =
            sGraveyard->GetGraveyard(graveyardLink.safeLocId);
        if (!entry) {
            LOG_ERROR("sql.sql",
                      "Table `graveyard_zone` has record for not existing "
                      "`game_graveyard` table {}, skipped.",
                      graveyardLink.safeLocId);
            continue;
        }
        ...
        if (!player->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_GRAVEYARD) &&
            (graveyardLink.safeLocId == GRAVEYARD_EBON_HOLD ||
             graveyardLink.safeLocId == GRAVEYARD_ARCHERUS)) {
            continue;
        }
                                 ...
                                 bool   persist /*= true*/)
{
    ...
    if (persist) {
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_INS_GRAVEYARD_ZONE);
                                    ...
                                    bool   persist /*= false*/)
{
    ...
    for (; range.first != range.second; ++range.first) {
        ...
        // skip enemy faction graveyard at same map (normal area, city, or
        // battleground) team == 0 case can be at call from .neargrave
        if (data.teamId != TEAM_NEUTRAL && teamId != TEAM_NEUTRAL &&
            data.teamId != teamId)
            continue;
    ...
    if (persist) {
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_DEL_GRAVEYARD_ZONE);
...
void Graveyard::LoadGraveyardZones()
{
    ...
    do {
        ...
        TeamId teamId    = team == 0
                               ? TEAM_NEUTRAL
                               : (team == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);

        GraveyardStruct const* entry = sGraveyard->GetGraveyard(safeLocId);
        if (!entry) {
            LOG_ERROR("sql.sql",
                      "Table `graveyard_zone` has a record for not existing "
                      "`game_graveyard` table {}, skipped.",
                      safeLocId);
            continue;
        }
        ...
        if (team != 0 && team != HORDE && team != ALLIANCE) {
            LOG_ERROR("sql.sql",
                      "Table `graveyard_zone` has a record for non player "
                      "faction ({}), skipped.",
                      team);
            continue;
        }
        ...
        if (!AddGraveyardLink(safeLocId, zoneId, teamId, false))
            LOG_ERROR("sql.sql",
                      "Table `graveyard_zone` has a duplicate record for "
                      "Graveyard (ID: {}) and Zone (ID: {}), skipped.",
                      safeLocId,
                      zoneId);
    } while (result->NextRow());

    LOG_INFO("server.loading",
             ">> Loaded {} Graveyard-Zone Links in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
GraveyardStruct const* Graveyard::GetGraveyard(const std::string& name) const
{
    ...
    for (GraveyardContainer::const_iterator itr = _graveyardStore.begin();
         itr != _graveyardStore.end();
         ++itr) {
