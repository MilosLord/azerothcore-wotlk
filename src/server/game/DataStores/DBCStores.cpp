...
typedef std::map<WMOAreaTableKey, WMOAreaTableEntry const*>
...
DBCStorage<AchievementCategoryEntry>
    sAchievementCategoryStore(AchievementCategoryfmt);
DBCStorage<AchievementCriteriaEntry>
                              sAchievementCriteriaStore(AchievementCriteriafmt);
DBCStorage<AuctionHouseEntry> sAuctionHouseStore(AuctionHouseEntryfmt);
DBCStorage<BankBagSlotPricesEntry>
    sBankBagSlotPricesStore(BankBagSlotPricesEntryfmt);
DBCStorage<BattlemasterListEntry>
...
DBCStorage<CinematicSequencesEntry>
    sCinematicSequencesStore(CinematicSequencesEntryfmt);
DBCStorage<CreatureDisplayInfoEntry>
    sCreatureDisplayInfoStore(CreatureDisplayInfofmt);
DBCStorage<CreatureDisplayInfoExtraEntry>
    sCreatureDisplayInfoExtraStore(CreatureDisplayInfoExtrafmt);
DBCStorage<CreatureFamilyEntry> sCreatureFamilyStore(CreatureFamilyfmt);
DBCStorage<CreatureModelDataEntry>
    sCreatureModelDataStore(CreatureModelDatafmt);
DBCStorage<CreatureSpellDataEntry>
                               sCreatureSpellDataStore(CreatureSpellDatafmt);
DBCStorage<CreatureTypeEntry>  sCreatureTypeStore(CreatureTypefmt);
DBCStorage<CurrencyTypesEntry> sCurrencyTypesStore(CurrencyTypesfmt);

DBCStorage<DestructibleModelDataEntry>
    sDestructibleModelDataStore(DestructibleModelDatafmt);
DBCStorage<DungeonEncounterEntry> sDungeonEncounterStore(DungeonEncounterfmt);
DBCStorage<DurabilityQualityEntry>
...
DBCStorage<GtChanceToSpellCritBaseEntry>
    sGtChanceToSpellCritBaseStore(GtChanceToSpellCritBasefmt);
DBCStorage<GtChanceToSpellCritEntry>
    sGtChanceToSpellCritStore(GtChanceToSpellCritfmt);
DBCStorage<GtNPCManaCostScalerEntry>
    sGtNPCManaCostScalerStore(GtNPCManaCostScalerfmt);
DBCStorage<GtOCTClassCombatRatingScalarEntry>
    sGtOCTClassCombatRatingScalarStore(GtOCTClassCombatRatingScalarfmt);
DBCStorage<GtOCTRegenHPEntry> sGtOCTRegenHPStore(GtOCTRegenHPfmt);
// DBCStorage <GtOCTRegenMPEntry> sGtOCTRegenMPStore(GtOCTRegenMPfmt);  -- not
// used currently
...
DBCStorage<ItemDisplayInfoEntry>
    sItemDisplayInfoStore(ItemDisplayTemplateEntryfmt);
DBCStorage<ItemExtendedCostEntry>
    sItemExtendedCostStore(ItemExtendedCostEntryfmt);
DBCStorage<ItemLimitCategoryEntry>
    sItemLimitCategoryStore(ItemLimitCategoryEntryfmt);
DBCStorage<ItemRandomPropertiesEntry>
...
DBCStorage<MapDifficultyEntry>
                 sMapDifficultyStore(MapDifficultyEntryfmt); // only for loading
...
DBCStorage<OverrideSpellDataEntry>
    sOverrideSpellDataStore(OverrideSpellDatafmt);
...
DBCStorage<QuestFactionRewEntry>
    sQuestFactionRewardStore(QuestFactionRewardfmt);
DBCStorage<RandomPropertiesPointsEntry>
    sRandomPropertiesPointsStore(RandomPropertiesPointsfmt);
DBCStorage<ScalingStatDistributionEntry>
    sScalingStatDistributionStore(ScalingStatDistributionfmt);
DBCStorage<ScalingStatValuesEntry>
    sScalingStatValuesStore(ScalingStatValuesfmt);

DBCStorage<SkillLineEntry>        sSkillLineStore(SkillLinefmt);
DBCStorage<SkillLineAbilityEntry> sSkillLineAbilityStore(SkillLineAbilityfmt);
SkillLineAbilityIndexBySkillLine  sSkillLineAbilityIndexBySkillLine;
DBCStorage<SkillRaceClassInfoEntry>
...
DBCStorage<SpellItemEnchantmentEntry>
...
DBCStorage<TeamContributionPointsEntry>
    sTeamContributionPointsStore(TeamContributionPointsfmt);
DBCStorage<TotemCategoryEntry> sTotemCategoryStore(TotemCategoryEntryfmt);
DBCStorage<TransportAnimationEntry>
    sTransportAnimationStore(TransportAnimationfmt);
DBCStorage<TransportRotationEntry>
                                 sTransportRotationStore(TransportRotationfmt);
...
static bool
LoadDBC_assert_print(uint32 fsize, uint32 rsize, const std::string& filename)
{
    LOG_ERROR("dbc",
              "Size of '{}' set by format string ({}) not equal size of C++ "
              "structure ({}).",
              filename,
              fsize,
              rsize);
                    ...
                    char const*        dbTable = nullptr)
{
    ...
    if (!existDBData) {
        ...
        if (FILE* f = fopen(dbcFilename.c_str(), "rb")) {
            ...
            stream << dbcFilename << " exists, and has "
                   << storage.GetFieldCount() << " field(s) (expected "
                   << strlen(storage.GetFormat())
                   << "). Extracted file might be from wrong client version or "
                      "a database-update has been forgotten.";
...
#define LOAD_DBC(store, file, dbtable)                                         \
    ...
    LOAD_DBC(sAchievementCategoryStore,
             "Achievement_Category.dbc",
             "achievement_category_dbc");
    LOAD_DBC(sAchievementCriteriaStore,
    ...
    LOAD_DBC(sBankBagSlotPricesStore,
             "BankBagSlotPrices.dbc",
             "bankbagslotprices_dbc");
    LOAD_DBC(
        sBattlemasterListStore, "BattlemasterList.dbc", "battlemasterlist_dbc");
    LOAD_DBC(
        sBarberShopStyleStore, "BarberShopStyle.dbc", "barbershopstyle_dbc");
    LOAD_DBC(
        sCharStartOutfitStore, "CharStartOutfit.dbc", "charstartoutfit_dbc");
    ...
    LOAD_DBC(
        sCinematicCameraStore, "CinematicCamera.dbc", "cinematiccamera_dbc");
    LOAD_DBC(sCinematicSequencesStore,
             "CinematicSequences.dbc",
             "cinematicsequences_dbc");
    LOAD_DBC(sCreatureDisplayInfoStore,
             "CreatureDisplayInfo.dbc",
             "creaturedisplayinfo_dbc");
    LOAD_DBC(sCreatureDisplayInfoExtraStore,
             "CreatureDisplayInfoExtra.dbc",
             "creaturedisplayinfoextra_dbc");
    LOAD_DBC(sCreatureFamilyStore, "CreatureFamily.dbc", "creaturefamily_dbc");
    LOAD_DBC(sCreatureModelDataStore,
             "CreatureModelData.dbc",
             "creaturemodeldata_dbc");
    LOAD_DBC(sCreatureSpellDataStore,
    ...
    LOAD_DBC(sDestructibleModelDataStore,
             "DestructibleModelData.dbc",
             "destructiblemodeldata_dbc");
    LOAD_DBC(
        sDungeonEncounterStore, "DungeonEncounter.dbc", "dungeonencounter_dbc");
    LOAD_DBC(
        sDurabilityCostsStore, "DurabilityCosts.dbc", "durabilitycosts_dbc");
    LOAD_DBC(sDurabilityQualityStore,
    ...
    LOAD_DBC(
        sFactionTemplateStore, "FactionTemplate.dbc", "factiontemplate_dbc");
    LOAD_DBC(
        sGameObjectArtKitStore, "GameObjectArtKit.dbc", "gameobjectartkit_dbc");
    LOAD_DBC(sGameObjectDisplayInfoStore,
             "GameObjectDisplayInfo.dbc",
             "gameobjectdisplayinfo_dbc");
    LOAD_DBC(sGemPropertiesStore, "GemProperties.dbc", "gemproperties_dbc");
    LOAD_DBC(
        sGlyphPropertiesStore, "GlyphProperties.dbc", "glyphproperties_dbc");
    LOAD_DBC(sGlyphSlotStore, "GlyphSlot.dbc", "glyphslot_dbc");
    LOAD_DBC(sGtBarberShopCostBaseStore,
             "gtBarberShopCostBase.dbc",
             "gtbarbershopcostbase_dbc");
    LOAD_DBC(
        sGtCombatRatingsStore, "gtCombatRatings.dbc", "gtcombatratings_dbc");
    LOAD_DBC(sGtChanceToMeleeCritBaseStore,
             "gtChanceToMeleeCritBase.dbc",
             "gtchancetomeleecritbase_dbc");
    LOAD_DBC(sGtChanceToMeleeCritStore,
             "gtChanceToMeleeCrit.dbc",
             "gtchancetomeleecrit_dbc");
    LOAD_DBC(sGtChanceToSpellCritBaseStore,
             "gtChanceToSpellCritBase.dbc",
             "gtchancetospellcritbase_dbc");
    LOAD_DBC(sGtChanceToSpellCritStore,
             "gtChanceToSpellCrit.dbc",
             "gtchancetospellcrit_dbc");
    LOAD_DBC(sGtNPCManaCostScalerStore,
             "gtNPCManaCostScaler.dbc",
             "gtnpcmanacostscaler_dbc");
    LOAD_DBC(sGtOCTClassCombatRatingScalarStore,
    ...
    // LOAD_DBC(sGtOCTRegenMPStore,                  "gtOCTRegenMP.dbc",
    // "gtoctregenmp_dbc");       -- not used currently
    LOAD_DBC(
        sGtRegenHPPerSptStore, "gtRegenHPPerSpt.dbc", "gtregenhpperspt_dbc");
    LOAD_DBC(
        sGtRegenMPPerSptStore, "gtRegenMPPerSpt.dbc", "gtregenmpperspt_dbc");
    ...
    LOAD_DBC(
        sItemDisplayInfoStore, "ItemDisplayInfo.dbc", "itemdisplayinfo_dbc");
    // LOAD_DBC(sItemCondExtCostsStore,              "ItemCondExtCosts.dbc",
    // "itemcondextcosts_dbc");
    LOAD_DBC(
        sItemExtendedCostStore, "ItemExtendedCost.dbc", "itemextendedcost_dbc");
    LOAD_DBC(sItemLimitCategoryStore,
             "ItemLimitCategory.dbc",
             "itemlimitcategory_dbc");
    LOAD_DBC(sItemRandomPropertiesStore,
             "ItemRandomProperties.dbc",
             "itemrandomproperties_dbc");
    LOAD_DBC(
        sItemRandomSuffixStore, "ItemRandomSuffix.dbc", "itemrandomsuffix_dbc");
    ...
    LOAD_DBC(sOverrideSpellDataStore,
    ...
    LOAD_DBC(
        sSpellDifficultyStore, "SpellDifficulty.dbc", "spelldifficulty_dbc");
    LOAD_DBC(sSpellDurationStore, "SpellDuration.dbc", "spellduration_dbc");
    LOAD_DBC(
        sSpellFocusObjectStore, "SpellFocusObject.dbc", "spellfocusobject_dbc");
    LOAD_DBC(sSpellItemEnchantmentStore,
             "SpellItemEnchantment.dbc",
             "spellitemenchantment_dbc");
    LOAD_DBC(sSpellItemEnchantmentConditionStore,
    ...
    LOAD_DBC(sSpellShapeshiftStore,
             "SpellShapeshiftForm.dbc",
             "spellshapeshiftform_dbc");
    LOAD_DBC(sSpellVisualStore, "SpellVisual.dbc", "spellvisual_dbc");
    LOAD_DBC(
        sStableSlotPricesStore, "StableSlotPrices.dbc", "stableslotprices_dbc");
    LOAD_DBC(
        sSummonPropertiesStore, "SummonProperties.dbc", "summonproperties_dbc");
    ...
    LOAD_DBC(
        sWorldMapOverlayStore, "WorldMapOverlay.dbc", "worldmapoverlay_dbc");

#undef LOAD_DBC

    for (CharStartOutfitEntry const* outfit : sCharStartOutfitStore)
        sCharStartOutfitMap[outfit->Race | (outfit->Class << 8) |
                            (outfit->Gender << 16)] = outfit;
    ...
    for (MapDifficultyEntry const* entry : sMapDifficultyStore)
        sMapDifficultyMap[MAKE_PAIR32(entry->MapId, entry->Difficulty)] =
            MapDifficulty(entry->resetTime,
                          entry->maxPlayers,
                          entry->areaTriggerText[0] != '\0');
    ...
    for (SkillLineAbilityEntry const* skillLine : sSkillLineAbilityStore) {
        SpellEntry const* spellEntry =
            sSpellStore.LookupEntry(skillLine->Spell);
        if (spellEntry && spellEntry->Attributes & SPELL_ATTR0_PASSIVE) {
            for (CreatureFamilyEntry const* cFamily : sCreatureFamilyStore) {
                if (skillLine->SkillLine != cFamily->skillLine[0] &&
                    skillLine->SkillLine != cFamily->skillLine[1]) {
                    continue;
                }
    ...
    for (SkillLineAbilityEntry const* skillLine : sSkillLineAbilityStore)
        sSkillLineAbilityIndexBySkillLine[skillLine->SkillLine].push_back(
            skillLine);
    ...
    for (SpellDifficultyEntry const* spellDiff : sSpellDifficultyStore) {
        ...
        for (uint8 x = 0; x < MAX_DIFFICULTY; ++x) {
            if (spellDiff->SpellID[x] <= 0 ||
                !sSpellStore.LookupEntry(spellDiff->SpellID[x])) {
                if (spellDiff->SpellID[x] >
                    0) // don't show error if spell is <= 0, not all modes have
                       // spells and there are unknown negative values
                    LOG_ERROR("sql.sql",
                              "spelldifficulty_dbc: spell {} at field id: {} "
                              "at spellid {} does not exist in SpellStore "
                              "(spell.dbc), loaded as 0",
                              spellDiff->SpellID[x],
                              spellDiff->ID,
                              x);
        ...
        if (newEntry.SpellID[0] <= 0 ||
            newEntry.SpellID[1] <= 0) // id0-1 must be always set!
            continue;

        for (uint8 x = 0; x < MAX_DIFFICULTY; ++x)
            if (newEntry.SpellID[x])
                sSpellMgr->SetSpellDifficultyId(uint32(newEntry.SpellID[x]),
                                                spellDiff->ID);
    }

    // create talent spells set
    for (TalentEntry const* talentInfo : sTalentStore) {
        TalentTabEntry const* talentTab =
            sTalentTabStore.LookupEntry(talentInfo->TalentTab);

        for (uint8 j = 0; j < MAX_TALENT_RANK; ++j) {
            if (talentInfo->RankID[j]) {
                sTalentSpellPosMap[talentInfo->RankID[j]] =
                    TalentSpellPos(talentInfo->TalentID, j);
    ...
    // include existed nodes that have at least single not spell base (scripted)
    // path
    {
        ...
        for (uint32 i = 1; i < sTaxiNodesStore.GetNumRows(); ++i) {
            ...
            if (src_i != sTaxiPathSetBySource.end() && !src_i->second.empty()) {
                bool ok = false;
                for (TaxiPathSetForSource::const_iterator dest_i =
                         src_i->second.begin();
                     dest_i != src_i->second.end();
                     ++dest_i) {
                    // not spell path
                    if (dest_i->second->price ||
                        spellPaths.find(dest_i->second->ID) ==
                            spellPaths.end()) {
                        ok = true;
                        break;
                    }
            ...
            // old continent node (+ nodes virtually at old continents, check
            // explicitly to avoid loading map files for zone info)
    ...
    for (TransportAnimationEntry const* anim : sTransportAnimationStore)
        sTransportMgr->AddPathNodeToTransport(
            anim->TransportEntry, anim->TimeSeg, anim);

    for (TransportRotationEntry const* rot : sTransportRotationStore)
        sTransportMgr->AddPathRotationToTransport(
            rot->TransportEntry, rot->TimeSeg, rot);

    for (WMOAreaTableEntry const* entry : sWMOAreaTableStore)
        sWMOAreaInfoByTripple[WMOAreaTableKey(
            entry->rootId, entry->adtId, entry->groupId)] = entry;

    // error checks
    if (bad_dbc_files.size() >= DBCFileCount) {
        LOG_ERROR("dbc",
                  "Incorrect DataDir value in worldserver.conf or ALL required "
                  "*.dbc files ({}) not found by path: {}dbc",
                  DBCFileCount,
                  dataPath);
        exit(1);
    }
        ...
        std::string str;
        for (StoreProblemList::iterator i = bad_dbc_files.begin();
             i != bad_dbc_files.end();
             ++i)
            str += *i + "\n";

        LOG_ERROR("dbc",
                  "Some required *.dbc files ({} from {}) not found or not "
                  "compatible:\n{}",
    ...
    if (!sAreaTableStore.LookupEntry(4987) || // last area added in 3.3.5a
        ...
        !sItemStore.LookupEntry(
            56806) || // last client known item added in 3.3.5a
        !sItemExtendedCostStore.LookupEntry(
            2997) || // last item extended cost added in 3.3.5a
        !sMapStore.LookupEntry(724) || // last map added in 3.3.5a
        !sSpellStore.LookupEntry(
            80864)) // last client known item added in 3.3.5a
    {
        LOG_ERROR("dbc",
                  "You have _outdated_ DBC data. Please extract correct "
                  "versions from current using client.");
        exit(1);
    }

    LOG_INFO("server.loading",
             ">> Initialized {} Data Stores in {} ms",
             DBCFileCount,
...
{
    auto i = sWMOAreaInfoByTripple.find(
...
{
    MapDifficultyMap::const_iterator itr =
        sMapDifficultyMap.find(MAKE_PAIR32(mapId, difficulty));
                                                    ...
                                                    Difficulty& difficulty)
{
    ...
    MapDifficulty const* mapDiff =
        GetMapDifficultyData(mapId, Difficulty(tmpDiff));
    if (!mapDiff) {
        if (tmpDiff >
            RAID_DIFFICULTY_25MAN_NORMAL) // heroic, downscale to normal
            tmpDiff -= 2;
        else
            tmpDiff -=
                1; // any non-normal mode for raids like tbc (only one mode)

        // pull new data
        mapDiff = GetMapDifficultyData(
            mapId, Difficulty(tmpDiff)); // we are 10 normal or 25 normal
        if (!mapDiff) {
            tmpDiff -= 1;
            mapDiff =
                GetMapDifficultyData(mapId, Difficulty(tmpDiff)); // 10 normal
        }
    }
...
PvPDifficultyEntry const* GetBattlegroundBracketByLevel(uint32 mapid,
                                                        uint32 level)
{
    PvPDifficultyEntry const* maxEntry =
        nullptr; // used for level > max listed level case
...
uint32 const* GetTalentTabPages(uint8 cls) { return sTalentTabPages[cls]; }
...
CharStartOutfitEntry const*
GetCharStartOutfitEntry(uint8 race, uint8 class_, uint8 gender)
{
    std::map<uint32, CharStartOutfitEntry const*>::const_iterator itr =
...
/// Returns LFGDungeonEntry for a specific map and difficulty. Will return first
/// found entry if multiple dungeons use the same map (such as Scarlet
/// Monastery)
LFGDungeonEntry const* GetLFGDungeon(uint32 mapId, Difficulty difficulty)
{
    for (uint32 i = 0; i < sLFGDungeonStore.GetNumRows(); ++i) {
        ...
        if (dungeon->MapID == uint32(mapId) &&
            Difficulty(dungeon->Difficulty) == difficulty)
            return dungeon;
    }
                                              ...
                                              LocaleConstant     locale)
{
    for (LFGDungeonEntry const* dungeon : sLFGDungeonStore) {
        if (dungeon->TypeID == lfg::LFG_TYPE_ZONE &&
            zoneName.find(dungeon->Name[locale]) != std::string::npos) {
            return dungeon;
        }
    }
...
uint32 GetDefaultMapLight(uint32 mapId)
{
    for (int32 i = sLightStore.GetNumRows(); i >= 0; --i) {
        ...
        if (light->MapId == mapId && light->X == 0.0f && light->Y == 0.0f &&
            light->Z == 0.0f)
            return light->Id;
    }
...
{
    SkillRaceClassInfoBounds bounds =
        SkillRaceClassInfoBySkill.equal_range(skill);
    for (SkillRaceClassInfoMap::iterator itr = bounds.first;
         itr != bounds.second;
         ++itr) {
        if (itr->second->RaceMask &&
            !(itr->second->RaceMask & (1 << (race - 1)))) {
            continue;
        }

        if (itr->second->ClassMask &&
            !(itr->second->ClassMask & (1 << (class_ - 1)))) {
            continue;
        }
...
const std::vector<SkillLineAbilityEntry const*>&
GetSkillLineAbilitiesBySkillLine(uint32 skillLine)
{
