...
struct M2Header {
    ...
    uint32 ofsName; // Offset to the name, it seems like models can get reloaded
                    // by this name.should be unique, i guess.
    uint32 GlobalModelFlags; // 0x0001: tilt x, 0x0002: tilt y, 0x0008: add 2
                             // fields in header, 0x0020: load .phys data
                             // (MoP+), 0x0080: has _lod .skin files (MoP?+),
                             // 0x0100: is camera related.
    uint32 nGlobalSequences;
    uint32 ofsGlobalSequences; // A list of timestamps.
    uint32 nAnimations;
    uint32 ofsAnimations; // Information about the animations in the model.
    uint32 nAnimationLookup;
    uint32 ofsAnimationLookup; // Mapping of global IDs to the entries in the
    ...
    uint32
        ofsSubmeshAnimations; // Submesh color and alpha animations definitions.
    uint32 nTextures;
    ...
    uint32 nTransLookup;
    uint32 ofsTransLookup; // Everything needs its lookup. Here are the
                           // transparencies.
    uint32     nUVAnimLookup;
    uint32     ofsUVAnimLookup;
    G3D::AABox BoundingBox; // min/max( [1].z, 2.0277779f ) - 0.16f seems to be
                            // the maximum camera height
    float      BoundingSphereRadius;
    G3D::AABox CollisionBox;
    float      CollisionSphereRadius;
    uint32     nBoundingTriangles;
    uint32 ofsBoundingTriangles; // Our bounding volumes. Similar structure like
                                 // in the old ofsViews.
    ...
    uint32 nLights;
    uint32 ofsLights; // Lights are mainly used in loginscreens but in wands and
                      // some doodads too.
    uint32 nCameras;  // Format of Cameras changed with version 271!
    uint32 ofsCameras; // The cameras are present in most models for having a
                       // model in the Character-Tab.
    uint32 nCameraLookup;
    uint32 ofsCameraLookup; // And lookup-time again.
    uint32 nRibbonEmitters;
    uint32 ofsRibbonEmitters; // Things swirling around. See the CoT-entrance
                              // for light-trails.
    uint32 nParticleEmitters;
    uint32
        ofsParticleEmitters; // Spells and weapons, doodads and loginscreens use
                             // them. Blood dripping of a blade? Particles.
    uint32
        nBlendMaps; // This has to deal with blending. Exists IFF (flags & 0x8)
                    // != 0. When set, textures blending is overriden by the
                    // associated array. See M2/WotLK#Blend_mode_overrides
    uint32 ofsBlendMaps; // Same as above. Points to an array of uint16 of
                         // nBlendMaps entries -- From WoD information.};
};
...
struct M2Camera {
    uint32_t type; // 0: portrait, 1: characterinfo; -1: else (flyby etc.);
                   // referenced backwards in the lookup table.
