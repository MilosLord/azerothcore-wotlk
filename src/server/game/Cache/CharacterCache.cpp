...
std::unordered_map<std::string, CharacterCacheEntry*>
    _characterCacheByNameStore;
} // namespace
 ...
 *sCharacterCache->GetCharacterCacheByGuid(GUID); if (!characterInfo) return;
 *
...
void CharacterCache::LoadCharacterCacheStorage()
{
    ...
    QueryResult result =
        CharacterDatabase.Query("SELECT guid, name, account, race, gender, "
                                "class, level FROM characters");
    if (!result) {
    ...
    do {
        ...
        AddCharacterCacheEntry(ObjectGuid::Create<HighGuid::Player>(
                                   fields[0].Get<uint32>()) /*guid*/,
                               fields[2].Get<uint32>() /*account*/,
                               fields[1].Get<std::string>() /*name*/,
                               fields[4].Get<uint8>() /*gender*/,
                               fields[3].Get<uint8>() /*race*/,
                               fields[5].Get<uint8>() /*class*/,
                               fields[6].Get<uint8>() /*level*/);
    } while (result->NextRow());

    QueryResult mailCountResult = CharacterDatabase.Query(
        "SELECT receiver, COUNT(receiver) FROM mail GROUP BY receiver");
    if (mailCountResult) {
        do {
            Field* fields = mailCountResult->Fetch();
            UpdateCharacterMailCount(
                ObjectGuid(HighGuid::Player, fields[0].Get<uint32>()),
                static_cast<int8>(fields[1].Get<uint64>()),
                true);
        } while (mailCountResult->NextRow());
    }

    LOG_INFO("server.loading",
             ">> Loaded Character Infos For {} Characters in {} ms",
             _characterCacheStore.size(),
...
void CharacterCache::RefreshCacheEntry(uint32 lowGuid)
{
    QueryResult result =
        CharacterDatabase.Query("SELECT guid, name, account, race, gender, "
                                "class, level FROM characters WHERE guid = {}",
                                lowGuid);
    if (!result) {
        return;
    }
    ...
    do {
        Field* fields = result->Fetch();
        DeleteCharacterCacheEntry(ObjectGuid::Create<HighGuid::Player>(lowGuid),
    ...
    QueryResult mailCountResult =
        CharacterDatabase.Query("SELECT receiver, COUNT(receiver) FROM mail "
                                "WHERE receiver = {} GROUP BY receiver",
                                lowGuid);
    if (mailCountResult) {
        do {
            Field* fields = mailCountResult->Fetch();
...
CharacterCacheEntry const*
CharacterCache::GetCharacterCacheByGuid(ObjectGuid const& guid) const
{
    auto itr = _characterCacheStore.find(guid);
    if (itr != _characterCacheStore.end()) {
        return &itr->second;
...
CharacterCacheEntry const*
CharacterCache::GetCharacterCacheByName(std::string const& name) const
{
...
uint32
CharacterCache::GetCharacterAccountIdByName(std::string const& name) const
{
    auto itr = _characterCacheByNameStore.find(name);
    if (itr != _characterCacheByNameStore.end()) {
        return itr->second->AccountId;
...
ObjectGuid::LowType
CharacterCache::GetCharacterGuildIdByGuid(ObjectGuid guid) const
{
    auto itr = _characterCacheStore.find(guid);
    if (itr == _characterCacheStore.end()) {
        return 0;
