...
public:
    ...
    void AddCharacterCacheEntry(ObjectGuid const&  guid,
                                uint32             accountId,
    ...
    void UpdateCharacterData(ObjectGuid const&  guid,
                             std::string const& name,
    ...
    void UpdateCharacterGuildId(ObjectGuid const&   guid,
                                ObjectGuid::LowType guildId);
    void UpdateCharacterArenaTeamId(ObjectGuid const& guid,
                                    uint8             slot,
                                    uint32            arenaTeamId);

    void UpdateCharacterMailCount(ObjectGuid const& guid,
                                  int8              count,
                                  bool              update = false);
    void DecreaseCharacterMailCount(ObjectGuid const& guid)
    {
        UpdateCharacterMailCount(guid, -1);
    };
    ...
    [[nodiscard]] CharacterCacheEntry const*
    GetCharacterCacheByGuid(ObjectGuid const& guid) const;
    [[nodiscard]] CharacterCacheEntry const*
    GetCharacterCacheByName(std::string const& name) const;

    void UpdateCharacterGroup(ObjectGuid const& guid, ObjectGuid groupGUID);
    void ClearCharacterGroup(ObjectGuid const& guid)
    {
        UpdateCharacterGroup(guid, ObjectGuid::Empty);
    };

    [[nodiscard]] ObjectGuid
         GetCharacterGuidByName(std::string const& name) const;
    ...
    [[nodiscard]] uint32
    GetCharacterAccountIdByName(std::string const& name) const;
    [[nodiscard]] uint8 GetCharacterLevelByGuid(ObjectGuid guid) const;
    [[nodiscard]] ObjectGuid::LowType
                             GetCharacterGuildIdByGuid(ObjectGuid guid) const;
    [[nodiscard]] uint32     GetCharacterArenaTeamIdByGuid(ObjectGuid guid,
                                                           uint8      type) const;
