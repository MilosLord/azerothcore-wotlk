...
void PoolGroup<Creature>::Despawn1Object(ObjectGuid::LowType guid)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(guid)) {
        ...
        if (!map->Instanceable()) {
            auto creatureBounds =
                map->GetCreatureBySpawnIdStore().equal_range(guid);
    ...
    if (GameObjectData const* data = sObjectMgr->GetGameObjectData(guid)) {
        ...
        if (!map->Instanceable()) {
            auto gameobjectBounds =
                map->GetGameObjectBySpawnIdStore().equal_range(guid);
            for (auto itr = gameobjectBounds.first;
                 itr != gameobjectBounds.second;) {
...
void PoolGroup<Quest>::Despawn1Object(uint32 quest_id)
{
    ...
    PooledQuestRelationBoundsNC qr =
        sPoolMgr->mQuestCreatureRelation.equal_range(quest_id);
    for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second;
         ++itr) {
        QuestRelations::iterator qitr = questMap->find(itr->second);
        if (qitr == questMap->end())
            continue;
        QuestRelations::iterator lastElement =
            questMap->upper_bound(itr->second);
...
{
    for (PoolObjectList::iterator itr = ExplicitlyChanced.begin();
         itr != ExplicitlyChanced.end();
         ++itr) {
    ...
    for (PoolObjectList::iterator itr = EqualChanced.begin();
         itr != EqualChanced.end();
         ++itr) {
        if (itr->guid == child_pool_id) {
                               ...
                               uint32          triggerFrom)
{
    ...
    // If triggered from some object respawn this object is still marked as
    // spawned and also counted into m_SpawnedPoolAmount so we need increase
    // count to be spawned by 1
    if (triggerFrom) {
        ++count;
    }
    ...
    if (count > 0) {
        ...
        if (!ExplicitlyChanced.empty()) {
            ...
            for (PoolObject& obj : ExplicitlyChanced) {
                ...
                // Triggering object is marked as spawned at this time and can
                // be also rolled (respawn case) so this need explicit check for
                // this case
                if (roll < 0 &&
                    (/*obj.guid == triggerFrom ||*/ !spawns.IsActiveObject<T>(
                        obj.guid))) {
        ...
        if (!EqualChanced.empty() && rolledObjects.empty()) {
            std::copy_if(EqualChanced.begin(),
                         EqualChanced.end(),
                         std::back_inserter(rolledObjects),
                         [/*triggerFrom, */ &spawns](PoolObject const& object) {
                             return /*object.guid == triggerFrom ||*/ !spawns
                                 .IsActiveObject<T>(object.guid);
                         });
...
void PoolGroup<GameObject>::Spawn1Object(PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr->GetGameObjectData(obj->guid)) {
        ...
        Map* map = sMapMgr->CreateBaseMap(data->mapid);
        // We use current coords to unspawn, not spawn coords since creature can
        // have changed grid
        if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY)) {
...
void PoolGroup<Quest>::Spawn1Object(PoolObject* obj)
{
    ...
    QuestRelations* questMap = sObjectMgr->GetCreatureQuestRelationMap();
    PooledQuestRelationBoundsNC qr =
        sPoolMgr->mQuestCreatureRelation.equal_range(obj->guid);
    for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second;
         ++itr) {
        LOG_DEBUG("pool",
                  "PoolGroup<Quest>: Adding quest {} to creature {}",
                  itr->first,
                  itr->second);
    ...
    for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second;
         ++itr) {
        LOG_DEBUG("pool",
                  "PoolGroup<Quest>: Adding quest {} to GO {}",
                  itr->first,
                  itr->second);
        questMap->insert(QuestRelations::value_type(itr->second, itr->first));
    }
                                   ...
                                   uint32          triggerFrom)
{
    ...
    if (!triggerFrom) {
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_SEL_POOL_QUEST_SAVE);
    ...
    if (limit > newQuests.size() && !currentQuests.empty()) {
        do {
            uint32 questId =
                Acore::Containers::SelectRandomContainerElement(currentQuests);
            newQuests.insert(questId);
            currentQuests.erase(questId);
        } while (newQuests.size() < limit &&
                 !currentQuests.empty()); // failsafe
    }
    ...
    do {
        uint32 questId =
            Acore::Containers::SelectRandomContainerElement(newQuests);
        spawns.ActivateObject<Quest>(questId, poolId);
        PoolObject tempObj(questId, 0.0f);
        Spawn1Object(&tempObj);
    ...
    // if we are here it means the pool is initialized at startup and did not
    // have previous saved state
    if (!triggerFrom)
...
void PoolMgr::LoadFromDB()
{
    // Pool templates
    {
        ...
        QueryResult result =
            WorldDatabase.Query("SELECT entry, max_limit FROM pool_template");
        if (!result) {
            mPoolTemplate.clear();
            LOG_WARN(
                "server.loading",
    ...
    LOG_INFO("server.loading", "Loading Creatures Pooling Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 creatures in  pools. DB table "
                     "`pool_creature` is empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                if (!data) {
                    LOG_ERROR(
                        ...
                        "(GUID: {}) defined for pool id ({}), skipped.",
                        guid,
                        pool_id);
                    continue;
                }
                ...
                if (it == mPoolTemplate.end()) {
                    LOG_ERROR("sql.sql",
                              "`pool_creature` pool id ({}) is not in "
                              "`pool_template`, skipped.",
                              pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100) {
                    LOG_ERROR("sql.sql",
                              "`pool_creature` has an invalid chance ({}) for "
                              "creature guid ({}) in pool id ({}), skipped.",
                              chance,
                              guid,
                              pool_id);
                    continue;
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Creatures In Pools in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Gameobjects Pooling Data...");
    {
        ...
        if (!result) {
            LOG_WARN("server.loading",
                     ">> Loaded 0 gameobjects in  pools. DB table "
                     "`pool_gameobject` is empty.");
            LOG_INFO("server.loading", " ");
        }
        else {
            uint32 count = 0;
            do {
                ...
                GameObjectTemplate const* goinfo =
                    sObjectMgr->GetGameObjectTemplate(data->id);
                if (goinfo->type != GAMEOBJECT_TYPE_CHEST &&
                    goinfo->type != GAMEOBJECT_TYPE_GOOBER &&
                    goinfo->type != GAMEOBJECT_TYPE_FISHINGHOLE) {
                    LOG_ERROR("sql.sql",
                              "`pool_gameobject` has a not lootable gameobject "
                              "spawn (GUID: {}, type: {}) defined for pool id "
                              "({}), skipped.",
                              guid,
                              goinfo->type,
                              pool_id);
                ...
                if (it == mPoolTemplate.end()) {
                    LOG_ERROR("sql.sql",
                              "`pool_gameobject` pool id ({}) is not in "
                              "`pool_template`, skipped.",
                              pool_id);
                    continue;
                }
                ...
                if (chance < 0 || chance > 100) {
                    LOG_ERROR(
                        "sql.sql",
                        "`pool_gameobject` has an invalid chance ({}) for "
                        "gameobject guid ({}) in pool id ({}), skipped.",
                        chance,
                        guid,
                        pool_id);
                    continue;
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Gameobjects In Pools in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
        ...
        if (!result) {
            ...
            do {
...

                {
                    ...
                    if (it == mPoolTemplate.end()) {
                        LOG_ERROR("sql.sql",
                                  "`pool_pool` mother_pool id ({}) is not in "
                                  "`pool_template`, skipped.",
                                  mother_pool_id);
                        continue;
                    }
                }
                {
                    ...
                    if (it == mPoolTemplate.end()) {
                        LOG_ERROR("sql.sql",
                                  "`pool_pool` included pool_id ({}) is not in "
                                  "`pool_template`, skipped.",
                                  child_pool_id);
                        continue;
                    }
                ...
                if (mother_pool_id == child_pool_id) {
                    LOG_ERROR("sql.sql",
                              "`pool_pool` pool_id ({}) includes itself, "
                ...
                if (chance < 0 || chance > 100) {
                    LOG_ERROR("sql.sql",
                              "`pool_pool` has an invalid chance ({}) for pool "
                              "id ({}) in mother pool id ({}), skipped.",
                              chance,
                              child_pool_id,
                              mother_pool_id);
                    continue;
            ...
            for (auto const& it : mPoolTemplate) {
                ...
                for (SearchMap::iterator poolItr =
                         mPoolSearchMap.find(it.first);
                     poolItr != mPoolSearchMap.end();
                     poolItr = mPoolSearchMap.find(poolItr->second)) {
                    checkedPools.insert(poolItr->first);
                    if (checkedPools.find(poolItr->second) !=
                        checkedPools.end()) {
                        std::ostringstream ss;
                        ss << "The pool(s) ";
                        for (std::set<uint32>::const_iterator itr =
                                 checkedPools.begin();
                             itr != checkedPools.end();
                             ++itr)
                            ss << *itr << ' ';
                        ss << "create(s) a circular reference, which can cause "
                              "the server to freeze.\nRemoving the last link "
                              "between mother pool "
                           << poolItr->first << " and child pool "
                           << poolItr->second;
            ...
            LOG_INFO("server.loading",
                     ">> Loaded {} Pools In Mother Pools in {} ms",
                     count,
                     GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    ...
    LOG_INFO("server.loading", "Loading Quest Pooling Data...");
    {
        ...
        WorldDatabasePreparedStatement* stmt =
            WorldDatabase.GetPreparedStatement(WORLD_SEL_QUEST_POOLS);
        ...
        if (!result) {
            ...
            do {
                ...
                if (!quest) {
                    LOG_ERROR("sql.sql",
                              "`pool_quest` has a non existing quest template "
                              "(Entry: {}) defined for pool id ({}), skipped.",
                              entry,
                              pool_id);
                    continue;
                }
                ...
                if (!quest->IsDailyOrWeekly()) {
                    LOG_ERROR("sql.sql",
                              "`pool_quest` has an quest ({}) which is not "
                              "daily or weekly in pool id ({}), use "
                              "ExclusiveGroup instead, skipped.",
                              entry,
                              pool_id);
                    continue;
                }
                ...
                if (poolTypeMap[pool_id] == QUEST_NONE)
                    poolTypeMap[pool_id] =
                        quest->IsDaily() ? QUEST_DAILY : QUEST_WEEKLY;

                int32 currType = quest->IsDaily() ? QUEST_DAILY : QUEST_WEEKLY;

                if (poolTypeMap[pool_id] != currType) {
                    LOG_ERROR("sql.sql",
                              "`pool_quest` quest {} is {} but pool ({}) is "
                              "specified for {}, mixing not allowed, skipped.",
                              entry,
                              currType == QUEST_DAILY ? "QUEST_DAILY"
                                                      : "QUEST_WEEKLY",
                              pool_id,
                              poolTypeMap[pool_id] == QUEST_DAILY
                                  ? "QUEST_DAILY"
                                  : "QUEST_WEEKLY");
                    continue;
                }
                ...
                if (creBounds.first == creBounds.second &&
                    goBounds.first == goBounds.second) {
                    LOG_ERROR("sql.sql",
                              "`pool_quest` lists entry ({}) as member of pool "
                              "({}) but is not started anywhere, skipped.",
                              entry,
                              pool_id);
                    continue;
                }
    ...
    // The initialize method will spawn all pools not in an event and not in
    // another pool, this is why there is 2 left joins with 2 null checks
    LOG_INFO("server.loading", "Starting Objects Pooling System...");
    {
        uint32 oldMSTime = getMSTime();

        QueryResult result = WorldDatabase.Query(
            "SELECT DISTINCT pool_template.entry, pool_pool.pool_id, "
            "pool_pool.mother_pool FROM pool_template"
            " LEFT JOIN game_event_pool ON "
            "pool_template.entry=game_event_pool.pool_entry"
            " LEFT JOIN pool_pool ON pool_template.entry=pool_pool.pool_id "
            "WHERE game_event_pool.pool_entry IS NULL");
        ...
        if (!result) {
            ...
            do {
                ...
                if (!CheckPool(pool_entry)) {
                    if (pool_pool_id)
                        // The pool is a child pool in pool_pool table. Ideally
                        // we should remove it from the pool handler to ensure
                        // it never gets spawned, however that could recursively
                        // invalidate entire chain of mother pools. It can be
                        // done in the future but for now we'll do nothing.
                        LOG_ERROR("sql.sql",
                                  "Pool Id {} has no equal chance pooled "
                                  "entites defined and explicit chance sum is "
                                  "not 100. This broken pool is a child pool "
                                  "of Id {} and cannot be safely removed.",
                                  pool_entry,
                                  fields[2].Get<uint32>());
                    else
                        LOG_ERROR("sql.sql",
                                  "Pool Id {} has no equal chance pooled "
                                  "entites defined and explicit chance sum is "
                                  "not 100. The pool will not be spawned.",
                                  pool_entry);
                    continue;
                }

                // Don't spawn child pools, they are spawned recursively by
                // their parent pools
                if (!pool_pool_id) {
            ...
            LOG_INFO(
                "pool",
                "Pool handling system initialized, {} pools spawned in {} ms",
                count,
                GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
...
void PoolMgr::SaveQuestsToDB(bool daily, bool weekly, bool other)
{
    ...
    for (PoolGroupQuestMap::iterator itr = mPoolQuestGroups.begin();
         itr != mPoolQuestGroups.end();
         ++itr) {
        if (itr->second.IsEmpty())
            continue;
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(
                itr->second.GetFirstEqualChancedObjectId())) {
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_DEL_QUEST_POOL_SAVE);
        stmt->SetData(0, itr->second.GetPoolId());
        trans->Append(stmt);
        deletedPools.insert(itr->second.GetPoolId());
    }

    for (SearchMap::iterator itr = mQuestSearchMap.begin();
         itr != mQuestSearchMap.end();
         ++itr)
...
void PoolMgr::ChangeDailyQuests()
{
    for (PoolGroupQuestMap::iterator itr = mPoolQuestGroups.begin();
        ...
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(
            ...
            UpdatePool<Quest>(itr->second.GetPoolId(),
                              1); // anything non-zero means don't load from db
        }
    }
...
// entry is different If it's same, the creature is respawned only (added back
// to map)
template <>
void PoolMgr::SpawnPool<Creature>(uint32 pool_id, uint32 db_guid)
{
    ...
    if (it != mPoolCreatureGroups.end() && !it->second.IsEmpty())
        it->second.SpawnObject(
            mSpawnedData, mPoolTemplate[pool_id].MaxLimit, db_guid);
}

// Call to spawn a pool, if cache if true the method will spawn only if cached
// entry is different If it's same, the gameobject is respawned only (added back
// to map)
template <>
...
// Call to spawn a pool, if cache if true the method will spawn only if cached
// entry is different If it's same, the pool is respawned only
template <>
void PoolMgr::SpawnPool<Pool>(uint32 pool_id, uint32 sub_pool_id)
{
    ...
    if (it != mPoolPoolGroups.end() && !it->second.IsEmpty())
        it->second.SpawnObject(
            mSpawnedData, mPoolTemplate[pool_id].MaxLimit, sub_pool_id);
}
...
void PoolMgr::SpawnPool<Quest>(uint32 pool_id, uint32 quest_id)
{
    ...
    if (it != mPoolQuestGroups.end() && !it->second.IsEmpty())
        it->second.SpawnObject(
            mSpawnedData, mPoolTemplate[pool_id].MaxLimit, quest_id);
}
...
// Call to update the pool when a gameobject/creature part of pool [pool_id] is
// ready to respawn Here we cache only the creature/gameobject whose guid is
// passed as parameter Then the spawn pool call will use this cache to decide
template <typename T>
...
template void PoolMgr::UpdatePool<Pool>(uint32 pool_id,
                                        uint32 db_guid_or_pool_id);
template void PoolMgr::UpdatePool<GameObject>(uint32 pool_id,
                                              uint32 db_guid_or_pool_id);
