...
struct PoolObject {
    ...
    PoolObject(uint32 _guid, float _chance)
        : guid(_guid), chance(std::fabs(_chance))
    {
    }
};
...
typedef std::pair<PooledQuestRelation::const_iterator,
                  PooledQuestRelation::const_iterator>
    PooledQuestRelationBounds;
typedef std::pair<PooledQuestRelation::iterator, PooledQuestRelation::iterator>
...
private:
    ...
    typedef std::unordered_map<uint32, PoolGroup<Creature>>
        PoolGroupCreatureMap;
    typedef std::unordered_map<uint32, PoolGroup<GameObject>>
...
// Method that tell if the creature is part of a pool and return the pool id if
// yes
template <>
...
// Method that tell if the gameobject is part of a pool and return the pool id
// if yes
template <>
...
// Method that tell if the quest is part of another pool and return the pool id
// if yes
template <>
...
// Method that tell if the pool is part of another pool and return the pool id
// if yes
template <>
