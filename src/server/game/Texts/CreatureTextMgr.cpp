...
public:
    CreatureTextBuilder(WorldObject*       obj,
                        uint8              gender,
                        ChatMsg            msgtype,
                        uint8              textGroup,
                        uint32             id,
        ...
        : _source(obj), _gender(gender), _msgType(msgtype),
          _textGroup(textGroup), _textId(id), _language(language),
          _target(target)
    {
    }
    ...
    size_t operator()(WorldPacket* data, LocaleConstant locale) const
    {
        std::string const& text = sCreatureTextMgr->GetLocalizedChatString(
            _source->GetEntry(), _gender, _textGroup, _textId, locale);

        return ChatHandler::BuildChatPacket(*data,
                                            _msgType,
                                            Language(_language),
                                            _source,
                                            _target,
...
public:
    PlayerTextBuilder(WorldObject*       obj,
                      WorldObject*       speaker,
                      uint8              gender,
                      ChatMsg            msgtype,
                      uint8              textGroup,
                      ...
                      uint32             language,
                      WorldObject const* target)
        : _source(obj), _talker(speaker), _gender(gender), _msgType(msgtype),
          _textGroup(textGroup), _textId(id), _language(language),
          _target(target)
    {
    }
...
void CreatureTextMgr::LoadCreatureTexts()
{
    ...
    WorldDatabasePreparedStatement* stmt =
        WorldDatabase.GetPreparedStatement(WORLD_SEL_CREATURE_TEXT);
    ...
    do {
        ...
        if (temp.sound) {
            if (!sSoundEntriesStore.LookupEntry(temp.sound)) {
                LOG_ERROR(
                    "sql.sql",
                    "CreatureTextMgr: Entry {}, Group {} in table "
                    "`creature_texts` has Sound {} but sound does not exist.",
                    temp.entry,
                    temp.group,
                    temp.sound);
                temp.sound = 0;
        ...
        if (!GetLanguageDescByID(temp.lang)) {
            LOG_ERROR(
                "sql.sql",
                "CreatureTextMgr: Entry {}, Group {} in table `creature_texts` "
                "using Language {} but Language does not exist.",
                temp.entry,
                temp.group,
                uint32(temp.lang));
        ...
        if (temp.type >= MAX_CHAT_MSG_TYPE) {
            LOG_ERROR(
                "sql.sql",
                "CreatureTextMgr: Entry {}, Group {} in table `creature_texts` "
                "has Type {} but this Chat Type does not exist.",
                temp.entry,
                temp.group,
                uint32(temp.type));
        ...
        if (temp.emote) {
            if (!sEmotesStore.LookupEntry(temp.emote)) {
                LOG_ERROR(
                    "sql.sql",
                    "CreatureTextMgr: Entry {}, Group {} in table "
                    "`creature_texts` has Emote {} but emote does not exist.",
                    temp.entry,
                    temp.group,
                    uint32(temp.emote));
        ...
        if (temp.BroadcastTextId) {
            if (!sObjectMgr->GetBroadcastText(temp.BroadcastTextId)) {
                LOG_ERROR("sql.sql",
                          "CreatureTextMgr: Entry {}, Group {}, Id {} in table "
                          "`creature_text` has non-existing or incompatible "
                          "BroadcastTextId {}.",
                          temp.entry,
                          temp.group,
                          temp.id,
                          temp.BroadcastTextId);
        ...
        if (temp.TextRange > TEXT_RANGE_WORLD) {
            LOG_ERROR("sql.sql",
                      "CreatureTextMgr: Entry {}, Group {}, Id {} in table "
                      "`creature_text` has incorrect TextRange {}.",
                      temp.entry,
                      temp.group,
                      temp.id,
                      temp.TextRange);
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Creature Texts For {} Creatures in {} ms",
             textCount,
             mTextMap.size(),
...
void CreatureTextMgr::LoadCreatureTextLocales()
{
    ...
    QueryResult result =
        WorldDatabase.Query("SELECT CreatureId, GroupId, ID, Locale, Text FROM "
    ...
    do {
        ...
        CreatureTextLocale& data =
            mLocaleTextMap[CreatureTextId(CreatureId, GroupId, ID)];
        ObjectMgr::AddLocaleString(
            fields[4].Get<std::string>(), locale, data.Text);
    } while (result->NextRow());

    LOG_INFO("server.loading",
             ">> Loaded {} Creature Text Locale in {} ms",
             uint32(mLocaleTextMap.size()),
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
                          ...
                          Player*            srcPlr /*= nullptr*/)
{
    ...
    if (sList == mTextMap.end()) {
        LOG_ERROR("sql.sql",
                  "CreatureTextMgr: Could not find Text for Creature({}) Entry "
    ...
    if (itr == textHolder.end()) {
        LOG_ERROR("sql.sql",
                  "CreatureTextMgr: Could not find TextGroup {} for Creature "
                  "{} ({}). Ignoring.",
                  uint32(textGroup),
                  source->GetName(),
    ...
    CreatureTextRepeatIds repeatGroup = GetRepeatGroup(
        source,
        textGroup); // has all textIDs from the group that were already said
    CreatureTextGroup
        tempGroup; // will use this to talk after sorting repeatGroup

    for (CreatureTextGroup::const_iterator giter = textGroupContainer.begin();
         giter != textGroupContainer.end();
         ++giter)
        if (std::find(repeatGroup.begin(), repeatGroup.end(), giter->id) ==
            repeatGroup.end())
            tempGroup.push_back(*giter);

    if (tempGroup.empty()) {
        CreatureTextRepeatMap::iterator mapItr =
            mTextRepeatMap.find(source->GetGUID());
        if (mapItr != mTextRepeatMap.end()) {
            CreatureTextRepeatGroup::iterator groupItr =
    ...
    if (srcPlr) {
        PlayerTextBuilder builder(source,
                                  finalSource,
                                  finalSource->getGender(),
                                  finalType,
                                  iter->group,
                                  iter->id,
                                  finalLang,
                                  target);
        SendChatPacket(
            finalSource, builder, finalType, target, range, teamId, gmOnly);
    }
    else {
        CreatureTextBuilder builder(finalSource,
                                    finalSource->getGender(),
                                    finalType,
                                    iter->group,
                                    iter->id,
                                ...
                                bool               gmOnly)
{
    ...
    SendNonChatPacket(source,
                      WorldPackets::Misc::Playsound(sound).Write(),
                      msgType,
                      target,
                      range,
                      teamId,
                                        ...
                                        bool               gmOnly) const
{
    ...
    case TEXT_RANGE_AREA: {
        ...
        for (Map::PlayerList::const_iterator itr = players.begin();
            ...
            if (itr->GetSource()->GetAreaId() == areaId &&
                (teamId == TEAM_NEUTRAL ||
                 itr->GetSource()->GetTeamId() == teamId) &&
                (!gmOnly || itr->GetSource()->IsGameMaster()))
    ...
    case TEXT_RANGE_WORLD: {
        ...
        for (SessionMap::const_iterator itr = smap.begin(); itr != smap.end();
             ++itr)
...
void CreatureTextMgr::SetRepeatId(Creature* source, uint8 textGroup, uint8 id)
{
    ...
    if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
        ...
        LOG_ERROR("sql.sql",
                  "CreatureTextMgr: TextGroup {} for Creature {} ({}), id {} "
                  "already added",
                  uint32(textGroup),
                  source->GetName(),
                  source->GetGUID().ToString(),
...
bool CreatureTextMgr::TextExist(uint32 sourceEntry, uint8 textGroup)
{
    ...
    if (sList == mTextMap.end()) {
        LOG_DEBUG("entities.unit",
                  "CreatureTextMgr::TextExist: Could not find Text for "
                  "Creature (entry {}) in 'creature_text' table.",
                  sourceEntry);
        return false;
    }
    ...
    if (itr == textHolder.end()) {
        LOG_DEBUG("entities.unit",
                  "CreatureTextMgr::TextExist: Could not find TextGroup {} for "
                  "Creature (entry {}).",
                  uint32(textGroup),
                  sourceEntry);
        return false;
                                                    ...
                                                    LocaleConstant locale) const
{
    ...
    CreatureTextHolder::const_iterator holderItr =
        mapitr->second.find(textGroup);
    ...
    BroadcastText const* bct =
        sObjectMgr->GetBroadcastText(groupItr->BroadcastTextId);
    if (bct)
    ...
    if (locale != DEFAULT_LOCALE && !bct) {
        LocaleCreatureTextMap::const_iterator locItr =
