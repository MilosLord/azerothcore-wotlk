...
void AutobroadcastMgr::LoadAutobroadcasts()
{
    ...
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_AUTOBROADCAST);
    stmt->SetData(0, realmId);
    PreparedQueryResult result = LoginDatabase.Query(stmt);

    if (!result) {
        LOG_WARN("autobroadcast",
                 ">> Loaded 0 autobroadcasts definitions. DB table "
                 "`autobroadcast` is empty for this realm!");
        LOG_INFO("autobroadcast", " ");
        return;
    }

    _announceType = static_cast<AnnounceType>(
        sWorld->getIntConfig(CONFIG_AUTOBROADCAST_CENTER));

    if (_announceType < AnnounceType::World ||
        _announceType > AnnounceType::Both) {
        LOG_ERROR("autobroadcast",
                  "AutobroadcastMgr::LoadAutobroadcasts: Config option "
                  "AutoBroadcast.Center set to not allowed value {}. Set to "
                  "default value 0",
                  (int8)_announceType);
    ...
    LOG_INFO("autobroadcast",
             ">> Loaded {} Autobroadcast Definitions in {} ms",
             count,
...
void AutobroadcastMgr::SendAutobroadcasts()
{
    ...
    for (AutobroadcastsWeightMap::const_iterator it =
             _autobroadcastsWeights.begin();
    ...
    LOG_DEBUG(
        "autobroadcast", "AutobroadcastMgr::SendAutobroadcasts: '{}'", msg);
}

void AutobroadcastMgr::SendWorldAnnouncement(std::string msg)
{
    sWorld->SendWorldTextOptional(
        LANG_AUTO_BROADCAST, ANNOUNCER_FLAG_DISABLE_AUTOBROADCAST, msg.data());
}
