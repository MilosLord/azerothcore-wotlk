...
enum UNIT_EVENT_TYPE {
    ...
    // Player/Pet entered/left  water or some other place where it is/was not
    // accessible for the creature
...
#define UEV_THREAT_REF_EVENT_MASK                                              \
    (UEV_THREAT_REF_ONLINE_STATUS | UEV_THREAT_REF_THREAT_CHANGE |             \
     UEV_THREAT_REF_REMOVE_FROM_LIST | UEV_THREAT_REF_ASSECCIBLE_STATUS)
#define UEV_THREAT_MANAGER_EVENT_MASK                                          \
    (UEV_THREAT_SORT_LIST | UEV_THREAT_SET_NEXT_TARGET |                       \
...
public:
    ...
    [[nodiscard]] bool   matchesTypeMask(uint32 pMask) const
    {
        return iType & pMask;
    }
...
public:
    ThreatRefStatusChangeEvent(uint32 pType)
        : UnitBaseEvent(pType), iThreatMgr(nullptr)
    {
    ...
    [[nodiscard]] HostileReference* getReference() const
    {
...
public:
    ThreatMgrEvent(uint32 pType)
        : ThreatRefStatusChangeEvent(pType), iThreatContainer(nullptr)
    {
    }
    ...
    [[nodiscard]] ThreatContainer* getThreatContainer() const
    {
