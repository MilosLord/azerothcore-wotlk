...
// default predicate function to select target based on distance, player and/or
// aura criteria
struct DefaultTargetSelector : public Acore::unary_function<Unit*, bool> {
    ...
    // dist: if 0: ignored, if > 0: maximum distance to the reference unit, if <
    // 0: minimum distance to the reference unit playerOnly: self explaining
    // withMainTank: allow current tank to be selected
    // aura: if 0: ignored, if > 0: the target shall have the aura, if < 0, the
    // target shall NOT have the aura
    DefaultTargetSelector(Unit const* unit,
                          float       dist,
                          bool        playerOnly,
                          bool        withMainTank,
                          int32       aura)
        : me(unit), m_dist(dist),
          except(!withMainTank ? me->GetThreatMgr().GetCurrentVictim()
                               : nullptr),
          m_playerOnly(playerOnly), m_aura(aura)
    {
    }
...
// NOTE: When passing to UnitAI::SelectTarget remember to use 0 as position for
// random selection
...
struct PowerUsersSelector : public Acore::unary_function<Unit*, bool> {
    ...
    PowerUsersSelector(Unit const* unit,
                       Powers      power,
                       float       dist,
                       bool        playerOnly)
...
struct FarthestTargetSelector : public Acore::unary_function<Unit*, bool> {
    FarthestTargetSelector(Unit const* unit,
                           float       maxDist,
                           bool        playerOnly,
                           bool        inLos,
                           float       minDist = 0.f)
        : _me(unit), _minDist(minDist), _maxDist(maxDist),
          _playerOnly(playerOnly), _inLos(inLos)
    {
    }
...
public:
    ...
    // Select the best target (in <targetType> order) from the threat list that
    // fulfill the following:
    // - Not among the first <offset> entries in <targetType> order (or
    // SelectTargetMethod::MaxThreat order,
    ...
    // Select the best target (in <targetType> order) satisfying <predicate>
    // from the threat list. If <offset> is nonzero, the first <offset> entries
    // in <targetType> order (or SelectTargetMethod::MaxThreat order, if
    // <targetType> is SelectTargetMethod::Random) are skipped.
                       ...
                       PREDICATE const&   predicate)
    {
        ...
        // shortcut: if we ignore the first <offset> elements, and there are at
        // most <offset> elements, then we ignore ALL elements
        if (mgr.GetThreatListSize() <= position)
            return nullptr;

        std::list<Unit*> targetList;
        SelectTargetList(targetList,
                         mgr.GetThreatListSize(),
                         targetType,
                         position,
                         predicate);
    ...
    // Select the best (up to) <num> targets (in <targetType> order) from the
    // threat list that fulfill the following:
    // - Not among the first <offset> entries in <targetType> order (or
    // SelectTargetMethod::MaxThreat order,
    //   if <targetType> is SelectTargetMethod::Random).
    // - Within at most <dist> yards (if dist > 0.0f)
    ...
    // - Has aura with ID <aura> (if aura > 0)
    // - Does not have aura with ID -<aura> (if aura < 0)
    // The resulting targets are stored in <targetList> (which is cleared
    // first).
    void SelectTargetList(std::list<Unit*>&  targetList,
                          uint32             num,
    ...
    // Select the best (up to) <num> targets (in <targetType> order) satisfying
    // <predicate> from the threat list and stores them in <targetList> (which
    // is cleared first). If <offset> is nonzero, the first <offset> entries in
    // <targetType> order (or SelectTargetMethod::MaxThreat order, if
    // <targetType> is SelectTargetMethod::Random) are skipped.
    template <class PREDICATE>
    void SelectTargetList(std::list<Unit*>&  targetList,
                          uint32             num,
                          ...
                          PREDICATE const&   predicate)
    {
        targetList.clear();
        ThreatMgr& mgr = GetThreatMgr();
        // shortcut: we're gonna ignore the first <offset> elements, and there's
        // at most <offset> elements, so we ignore them all - nothing to do here
        if (mgr.GetThreatListSize() <= position)
            return;
        ...
        // right now, list is unsorted for DISTANCE types - re-sort by
        // SelectTargetMethod::MaxDistance
        if (targetType == SelectTargetMethod::MaxDistance ||
            targetType == SelectTargetMethod::MinDistance)
            SortByDistance(targetList,
                           targetType == SelectTargetMethod::MinDistance);
        ...
        targetList.remove_if(
            [&predicate](Unit* target) { return !predicate(target); });
     ...
     * (NOTE: Creature engage logic should NOT be here, but in JustEngagedWith,
     * which happens once threat is established!)
     *
    ...
    virtual void DamageDealt(Unit* /*victim*/,
                             uint32& /*damage*/,
    ...
    virtual void DamageTaken(Unit* /*attacker*/,
                             uint32& /*damage*/,
                             DamageEffectType /*damagetype*/,
    ...
    virtual void OnPowerUpdate(Powers /*power*/,
                               int32 /*updateVal*/,
                               int32 /*gain*/,
                               uint32 /*currPower*/)
    {
    ...
    virtual void OnCalculateMeleeDamageReceived(uint32& /*damage*/,
                                                Unit* /*attacker*/)
    {
    }
    virtual void OnCalculateSpellDamageReceived(int32& /*damage*/,
                                                Unit* /*attacker*/)
    {
    }

    // Called during calculation when receiving periodic healing or damage (DoT
    // or HoT)
    virtual void OnCalculatePeriodicTickReceived(uint32& /*damage*/,
                                                 Unit* /*attacker*/)
    {
    }
    ...
    SpellCastResult
    DoCast(Unit* victim, uint32 spellId, bool triggered = false);
    SpellCastResult DoCastSelf(uint32 spellId, bool triggered = false)
    {
        return DoCast(me, spellId, triggered);
    }
    SpellCastResult DoCastToAllHostilePlayers(uint32 spellid,
    ...
    virtual void SummonMovementInform(Creature* /*creature*/,
                                      uint32 /*motionType*/,
                                      uint32 /*point*/)
    {
    }
    ...
    virtual void sGossipSelectCode(Player* /*player*/,
                                   uint32 /*sender*/,
                                   uint32 /*action*/,
                                   char const* /*code*/)
    {
    ...
    virtual void
    sQuestReward(Player* /*player*/, Quest const* /*quest*/, uint32 /*opt*/)
    {
    }
