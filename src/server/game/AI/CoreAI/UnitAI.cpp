...
void UnitAI::DoMeleeAttackIfReady()
{
    ...
    // checking distance
    if (me->isAttackReady()) {
        // xinef: prevent base and off attack in same time, delay attack at 0.2
        // sec
...
bool UnitAI::DoSpellAttackIfReady(uint32 spell)
{
    ...
    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell)) {
        if (me->IsWithinCombatRange(me->GetVictim(),
...
void UnitAI::DoSpellAttackToRandomTargetIfReady(
    uint32 spell,
    ...
    bool   playerOnly /*= true*/)
{
    ...
    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell)) {
        if (Unit* target = SelectTarget(SelectTargetMethod::Random,
                                        ...
                                        dist,
                                        playerOnly)) {
            if (me->IsWithinCombatRange(target,
                                        spellInfo->GetMaxRange(false))) {
                           ...
                           int32              aura)
{
    return SelectTarget(
        targetType,
        position,
        DefaultTargetSelector(me, dist, playerOnly, withTank, aura));
}
...
std::string UnitAI::GetDebugInfo() const
{
    ...
    sstr << std::boolalpha << "Me: " << (me ? me->GetDebugInfo() : "NULL");
    return sstr.str();
}
...
SpellCastResult UnitAI::DoCast(uint32 spellId)
{
    ...
    case AITARGET_ENEMY: {
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId)) {
            bool playerOnly =
                spellInfo->HasAttribute(SPELL_ATTR3_ONLY_ON_PLAYER);
            target = SelectTarget(SelectTargetMethod::Random,
                                  0,
                                  spellInfo->GetMaxRange(false),
                                  playerOnly);
        }
    ...
    case AITARGET_DEBUFF: {
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId)) {
            ...
            DefaultTargetSelector targetSelector(
                me, range, playerOnly, true, -(int32)spellId);
            if (!(spellInfo->AuraInterruptFlags &
                ...
                target =
                    SelectTarget(SelectTargetMethod::Random, 0, targetSelector);
        }
        break;
    }
    }
...
/**
 * @brief Cast spell on the max threat target, which may not always be the
 * current victim.
 *
...
ThreatMgr& UnitAI::GetThreatMgr() { return me->GetThreatMgr(); }
...
void SimpleCharmedAI::UpdateAI(uint32 /*diff*/)
{
    ...
    if (charmer->IsInEvadeMode()) {
        Unit::AuraEffectList const& auras =
            me->GetAuraEffectsByType(SPELL_AURA_MOD_CHARM);
        for (Unit::AuraEffectList::const_iterator iter = auras.begin();
             iter != auras.end();
             ++iter)
    ...
    if (!charmer->IsInCombat())
        me->GetMotionMaster()->MoveFollow(
            charmer, PET_FOLLOW_DIST, me->GetFollowAngle());

    Unit* target = me->GetVictim();
    if (!target || !charmer->IsValidAttackTarget(target))
        AttackStart(
            charmer->SelectNearestTargetInAttackDistance(ATTACK_DISTANCE));
}

SpellTargetSelector::SpellTargetSelector(Unit* caster, uint32 spellId)
    : _caster(caster), _spellInfo(sSpellMgr->GetSpellForDifficultyFromSpell(
...
bool SpellTargetSelector::operator()(Unit const* target) const
{
    ...
    uint32 range_type =
        _spellInfo->RangeEntry ? _spellInfo->RangeEntry->Flags : 0;
