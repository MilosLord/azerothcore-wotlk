...
int32 PetAI::Permissible(Creature const* creature)
{
    if (creature->HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN)) {
        if (reinterpret_cast<Guardian const*>(creature)
                ->GetOwner()
...
{
    // This is needed for charmed creatures, as once their target was reset
    // other effects can trigger threat
...
void PetAI::PetStopAttack() { _stopAttack(); }

void PetAI::_stopAttack()
{
    if (!me->IsAlive()) {
        LOG_DEBUG("entities.unit.ai",
                  "Creature stoped attacking cuz his dead [{}]",
                  me->GetGUID().ToString());
...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetVictim() && me->GetVictim()->IsAlive()) {
        ...
        if (me->GetCharmInfo()->HasCommandState(COMMAND_STAY)) {
            ...
            _doMeleeAttack();
             ...
             (!me->GetCharmInfo()->GetForcedSpell() &&
              !(me->IsPet() && me->ToPet()->HasTempSpell()) &&
              !me->HasUnitState(UNIT_STATE_CASTING))) {
        if (me->HasReactState(REACT_AGGRESSIVE) ||
            me->GetCharmInfo()->IsAtStay()) {
            // Every update we need to check targets only in certain cases
            // Aggressive - Allow auto select if owner or pet don't have a
            // target Stay - Only pick from pet or owner targets / attackers so
            // targets won't run by
            //   while chasing our owner. Don't do auto select.
            // All other cases (ie: defensive) - Targets are assigned by
            // AttackedBy(), OwnerAttackedBy(), OwnerAttacked(), etc.
            Unit* nextTarget =
                SelectNextTarget(me->HasReactState(REACT_AGGRESSIVE));
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING)) {
        if (owner && owner->GetTypeId() == TYPEID_PLAYER &&
            ...
            owner->ToPlayer()->GetSession()->HandlePetActionHelper(
                me,
                me->GetGUID(),
            ...
            // xinef: if spell was casted properly and we are in passive mode,
            // handle return
            if (!me->GetCharmInfo()->GetForcedSpell() &&
                me->HasReactState(REACT_PASSIVE)) {
                if (me->HasUnitState(UNIT_STATE_CASTING)) {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i) {
            ...
            if (me->GetCharmInfo()->GetGlobalCooldownMgr().HasGlobalCooldown(
                    spellInfo))
                continue;
            ...
            if (spellInfo->IsPositive()) {
                if (spellInfo->CanBeUsedInCombat()) {
                    // Check if we're in combat or commanded to attack (exlude
                    // auras with infinity duration)
                    if (!me->IsInCombat() &&
                ...
                spell->LoadScripts(); // xinef: load for CanAutoCast (calling
                                      // CheckPetCast)
                ...
                if (!spellUsed) {
                    for (ObjectGuid const& guid : m_AllySet) {
                        ...
                        // only buff targets that are in combat, unless the
                        // spell can only be cast while out of combat
                        if (!ally)
                            continue;

                        if (spell->CanAutoCast(ally)) {
                            targetSpellStore.push_back(
                                std::make_pair(ally, spell));
                            spellUsed = true;
                            break;
                        }
                    }
            ...
            else if (me->GetVictim() && CanAttack(me->GetVictim(), spellInfo) &&
                     spellInfo->CanBeUsedInCombat()) {
                Spell* spell = new Spell(me, spellInfo, TRIGGERED_NONE);
                if (spell->CanAutoCast(me->GetVictim()))
                    targetSpellStore.push_back(
                        std::make_pair(me->GetVictim(), spell));
                else
                    delete spell;
            }
        }
...
void PetAI::UpdateAllies()
{
    ...
    m_updateAlliesTimer =
        10 * IN_MILLISECONDS; // update friendly targets every 10 seconds,
                              // lesser checks increase performance
    ...
    if (m_AllySet.size() == 2 && !group)
        return;
    // owner is in group; group members filled in already (no raid ->
    // subgroupcount = whole count)
    if (group && !group->isRaidGroup() &&
        m_AllySet.size() == (group->GetMembersCount() + 2))
        return;
    ...
    if (group) // add group
    {
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
            ...
            if (!Target || !Target->IsInMap(owner) ||
                !group->SameSubGroup(owner->ToPlayer(), Target))
                continue;
...
void PetAI::KilledUnit(Unit* victim)
{
    ...
    // Xinef: if pet is channeling a spell and owner killed something different,
    // dont interrupt it
    if (me->HasUnitState(UNIT_STATE_CASTING) &&
    ...
    // Can't use _stopAttack() because that activates movement handlers and
    // ignores next target selection
    me->AttackStop();
    me->InterruptNonMeleeSpells(false);
...
{
    // Called when owner takes damage. This function helps keep pets from
    // running off
...
Unit* PetAI::SelectNextTarget(bool allowAutoSelect) const
{
    ...
    // Targets are not evaluated here for being valid targets, that is done in
    // _CanAttack() The parameter: allowAutoSelect lets us disable aggressive
    // pet auto targeting for certain situations

    // Passive pets don't do next target selection
    if (me->HasReactState(REACT_PASSIVE))
        return nullptr;

    // Check pet attackers first so we don't drag a bunch of targets to the
    // owner
    if (Unit* myAttacker = me->getAttackerForHelper())
        if (!myAttacker->HasBreakableByDamageCrowdControlAura() &&
            me->CanCreatureAttack(myAttacker))
            return myAttacker;

    // Check pet's attackers first to prevent dragging mobs back to owner
    if (me->HasAuraType(SPELL_AURA_MOD_TAUNT)) {
        const Unit::AuraEffectList& tauntAuras =
    ...
    if (Unit* ownerAttacker = owner->getAttackerForHelper())
        if (!ownerAttacker->HasBreakableByDamageCrowdControlAura() &&
            me->CanCreatureAttack(ownerAttacker))
            return ownerAttacker;

    // Check owner victim
    // 3.0.2 - Pets now start attacking their owners victim in defensive mode as
    // soon as the hunter does
    if (Unit* ownerVictim = owner->GetVictim())
        if (me->CanCreatureAttack(ownerVictim))
            return ownerVictim;

    // Neither pet or owner had a target and aggressive pets can pick any target
    // To prevent aggressive pets from chain selecting targets and running off,
    // we only select a random target if certain conditions are met.
    if (allowAutoSelect)
        if (!me->GetCharmInfo()->IsReturning() ||
            me->GetCharmInfo()->IsFollowing() || me->GetCharmInfo()->IsAtStay())
            if (Unit* nearTarget =
                    me->ToCreature()->SelectNearestTargetInAttackDistance(
                        MAX_AGGRO_RADIUS))
                return nearTarget;
...
void PetAI::SpellHit(Unit* caster, SpellInfo const* spellInfo)
{
    ...
    if (spellInfo->HasAura(SPELL_AURA_MOD_TAUNT) &&
        ...
        if (CanAttack(caster, spellInfo)) {
            // Only chase if not commanded to stay or if stay but commanded to
            // attack
            DoAttack(caster,
                     (!me->GetCharmInfo()->HasCommandState(COMMAND_STAY) ||
                      me->GetCharmInfo()->IsCommandAttack()));
...
void PetAI::DoAttack(Unit* target, bool chase)
{
    ...
    if (me->Attack(target, true)) {
        ...
        // Play sound to let the player know the pet is attacking something it
        // picked on its own
        if (me->HasReactState(REACT_AGGRESSIVE) &&
            !me->GetCharmInfo()->IsCommandAttack())
        ...
        if (chase) {
            bool oldCmdAttack =
                me->GetCharmInfo()
                    ->IsCommandAttack(); // This needs to be reset after other
                                         // flags are cleared
            ClearCharmInfoFlags();
            me->GetCharmInfo()->SetIsCommandAttack(
                oldCmdAttack); // For passive pets commanded to attack so they
                               // will use spells

            if (_canMeleeAttack()) {
                float angle = combatRange == 0.f &&
                                      target->GetTypeId() != TYPEID_PLAYER &&
                                      !target->IsPet()
                                  ? float(M_PI)
                                  : 0.f;
                float tolerance =
                    combatRange == 0.f ? float(M_PI_4) : float(M_PI * 2);
                me->GetMotionMaster()->MoveChase(target,
                                                 ChaseRange(0.f, combatRange),
                                                 ChaseAngle(angle, tolerance));
            }
        }
...
void PetAI::MovementInform(uint32 moveType, uint32 data)
{
    ...
    case FOLLOW_MOTION_TYPE: {
        ...
        if (me->GetCharmerOrOwner() && me->GetCharmInfo() &&
            data == me->GetCharmerOrOwner()->GetGUID().GetCounter() &&
            me->GetCharmInfo()->IsReturning()) {
            ClearCharmInfoFlags();
...
{
    // Evaluates wether a pet can attack a specific target based on
    // CommandState, ReactState and other flags IMPORTANT: The order in which
    // things are checked is important, be careful if you add or remove checks
    ...
    if (target->HasBreakableByDamageCrowdControlAura() &&
        (!spellInfo ||
         !spellInfo->HasAttribute(SPELL_ATTR4_REACTIVE_DAMAGE_PROC)))
        return me->GetCharmInfo()->IsCommandAttack();
    ...
    //  Pets attacking something (or chasing) should only switch targets if
    //  owner tells them to
...
{
    // Called when pet takes damage. This function helps keep pets from running
    // off
