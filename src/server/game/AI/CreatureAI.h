...
public:
    ...
    enum EvadeReason {
        ...
        EVADE_REASON_BOUNDARY,       // the creature has moved outside its evade
                                     // boundary
        EVADE_REASON_SEQUENCE_BREAK, // this is a boss and the pre-requisite
                                     // encounters for engaging it are not
                                     // defeated yet
        EVADE_REASON_NO_PATH, // the creature was unable to reach its target for
                              // over 5 seconds
        EVADE_REASON_OTHER
    };
    ...
    explicit CreatureAI(Creature* creature)
        : UnitAI(creature), me(creature), _boundary(nullptr),
          _negateBoundary(false), m_MoveInLineOfSight_locked(false)
    {
    }
    ...
    // Called in Creature::Update when deathstate = DEAD. Inherited classes may
    // maniuplate the ability to respawn based on scripted events.
    ...
    // victim) Note: for reaction at hostile action must be called AttackedBy
    // function.
    ...
    /// == Fields =======================================
    virtual void
    PassengerBoarded(Unit* /*passenger*/, int8 /*seatId*/, bool /*apply*/)
    {
    }
    ...
    virtual bool CanAlwaysBeDetectable(WorldObject const* /*seer*/)
    {
        return false;
    }
    ...
    static bool IsInBounds(CreatureBoundary const& boundary,
                           Position const*         who);
    bool        IsInBoundary(Position const* who = nullptr) const;

    virtual void CalculateThreat(Unit* /*hatedUnit*/,
                                 float& /*threat*/,
                                 SpellInfo const* /*threatSpell*/)
    {
    }

    virtual bool OnTeleportUnreacheablePlayer(Player* /*player*/)
    {
        return false;
    }
