...
SmartAI::SmartAI(Creature* c) : CreatureAI(c)
{
    ...
    // spawn in run mode
    // Xinef: spawn in run mode and set mRun to run... this overrides SetWalk
    // EVERYWHERE
    mRun           = true;
...
bool SmartAI::IsAIControlled() const { return !mIsCharmed; }
    ...
    if (itr != mWayPoints->end()) {
        ...
        if (mLastWP->id != mCurrentWPID)
            LOG_ERROR("scripts.ai.sai",
                      "SmartAI::GetNextWayPoint: Got not expected waypoint id "
                      "{}, expected {}",
                      mLastWP->id,
                      mCurrentWPID);
...
void SmartAI::GenerateWayPointArray(Movement::PointsArray* points)
{
    ...
    if (me->m_movementInfo.HasMovementFlag(
            (MovementFlags)(MOVEMENTFLAG_CAN_FLY |
                            MOVEMENTFLAG_DISABLE_GRAVITY))) {
        // xinef: first point in vector is unit real position
        points->clear();
        points->push_back(G3D::Vector3(
        ...
        for (float size = 1.0f; size; size *= 0.5f) {
            ...
            {
                G3D::Vector3 middle =
                    (pVector[0] + pVector[pVector.size() - 1]) / 2.f;
                G3D::Vector3 offset;

                bool continueLoop = false;
                for (uint32 i = 1; i < pVector.size() - 1; ++i) {
                    offset = middle - pVector[i];
                    if (std::fabs(offset.x) >= 0xFF ||
                        std::fabs(offset.y) >= 0xFF ||
                        std::fabs(offset.z) >= 0x7F) {
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    if (HasEscortState(SMART_ESCORT_PAUSED)) {
        LOG_ERROR("scripts.ai.sai",
                  "SmartAI::StartPath: Creature entry {} wanted to pause "
                  "waypoint movement while already paused, ignoring.",
                  me->GetEntry());
        return;
    }
    ...
    if (forced && !mWPReached) {
        ...
        SetRun(mRun);
        if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) ==
            ESCORT_MOTION_TYPE)
    ...
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_PAUSED,
                                  nullptr,
                                  mCurrentWPID,
                                  GetScript()->GetPathId());
}
...
void SmartAI::StopPath(uint32 DespawnTime, uint32 quest, bool fail)
{
    ...
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_STOPPED,
                                  nullptr,
                                  mCurrentWPID,
                                  GetScript()->GetPathId());
    EndPath(fail);
...
{
    RemoveEscortState(SMART_ESCORT_ESCORTING | SMART_ESCORT_PAUSED |
    ...
    ObjectVector const* targets =
        GetScript()->GetStoredTargetVector(SMART_ESCORT_TARGETS, *me);
        ...
        if (targets->size() == 1 &&
            ...
            if (Group* group = player->GetGroup()) {
                for (GroupReference* groupRef = group->GetFirstMember();
                    ...
                    if (!fail && groupGuy->IsAtGroupRewardDistance(me) &&
                        !groupGuy->HasCorpse())
                        groupGuy->AreaExploredOrEventHappens(mEscortQuestID);
                    else if (fail && groupGuy->GetQuestStatus(mEscortQuestID) ==
                                         QUEST_STATUS_INCOMPLETE)
                        groupGuy->FailQuest(mEscortQuestID);
                }
            }
            else {
                if (!fail && player->IsAtGroupRewardDistance(me) &&
                    !player->HasCorpse())
                    player->GroupEventHappens(mEscortQuestID, me);
                else if (fail && player->GetQuestStatus(mEscortQuestID) ==
                                     QUEST_STATUS_INCOMPLETE)
    ...
    // Xinef: End Path events should be only processed if it was SUCCESSFUL stop
    // or stop called by SMART_ACTION_WAYPOINT_STOP
    if (fail) {
        mCurrentWPID = 0;
        return;
    }

    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_ENDED,
                                  nullptr,
                                  mCurrentWPID,
                                  GetScript()->GetPathId());
    mCurrentWPID = 0;
...
void SmartAI::UpdatePath(const uint32 diff)
{
    ...
    if (mEscortInvokerCheckTimer < diff) {
        ...
        if (!IsEscortInvokerInRange()) {
            ...
            // Xinef: allow to properly hook out of range despawn action, which
            // in most cases should perform the same operation as dying
    ...
    if (!me->HasUnitState(UNIT_STATE_NOT_MOVE) &&
        me->movespline->Finalized()) // reached WP
    {
        if (!mWPReached) {
            ResumePath();
            return;
        ...
        if (mCurrentWPID == GetWPCount())
            ...
            SetRun(mRun);
            // xinef: if we have reached waypoint, and there is no working
            // spline movement it means our splitted array has ended, make new
            // one
...
void SmartAI::CheckConditions(const uint32 diff)
{
    ...
    if (m_ConditionsTimer < diff) {
        if (!conditions.empty()) {
            for (SeatMap::iterator itr = vehicle->Seats.begin();
                 itr != vehicle->Seats.end();
                 ++itr)
                if (Unit* passenger = ObjectAccessor::GetUnit(
                        *me, itr->second.Passenger.Guid)) {
                    if (Player* player = passenger->ToPlayer()) {
                        if (!sConditionMgr->IsObjectMeetToConditions(
                                player, me, conditions)) {
                            player->ExitVehicle();
...
void SmartAI::UpdateAI(uint32 diff)
{
    ...
    if (mFollowGuid) {
        if (mFollowArrivedTimer < diff) {
            if (me->FindNearestCreature(mFollowArrivedEntry,
...
{
    if (ObjectVector const* targets =
            GetScript()->GetStoredTargetVector(SMART_ESCORT_TARGETS, *me)) {
        float checkDist = me->GetInstanceScript()
                              ? SMART_ESCORT_MAX_PLAYER_DIST * 2
        ...
        if (targets->size() == 1 &&
            ...
            if (Group* group = player->GetGroup()) {
                for (GroupReference* groupRef = group->GetFirstMember();
                    ...
                    if (groupGuy && me->IsInMap(groupGuy) &&
...
void SmartAI::MovepointReached(uint32 id)
{
    ...
    GetScript()->ProcessEventsFor(
        SMART_EVENT_WAYPOINT_REACHED, nullptr, mCurrentWPID);

    if (mLastWP) {
        me->SetPosition(
            mLastWP->x, mLastWP->y, mLastWP->z, me->GetOrientation());
...
void SmartAI::MovementInform(uint32 MovementType, uint32 Data)
{
    if (MovementType == POINT_MOTION_TYPE &&
        Data == SMART_ESCORT_LAST_OOC_POINT)
        me->ClearUnitState(UNIT_STATE_EVADE);

    GetScript()->ProcessEventsFor(
        SMART_EVENT_MOVEMENTINFORM, nullptr, MovementType, Data);
    if (!HasEscortState(SMART_ESCORT_ESCORTING))
        return;

    if (MovementType == ESCORT_MOTION_TYPE ||
        (MovementType == POINT_MOTION_TYPE &&
         Data == SMART_ESCORT_LAST_OOC_POINT))
...
void SmartAI::EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    if (me->GetCharmerGUID().IsPlayer() ||
        me->HasUnitFlag(UNIT_FLAG_POSSESSED)) {
        me->AttackStop();
        return;
    }
    ...
    GetScript()->ProcessEventsFor(
        SMART_EVENT_EVADE); // must be after aura clear so we can cast spells
                            // from db
    ...
    if (HasEscortState(SMART_ESCORT_ESCORTING)) {
        ...
        if (Unit* target = ObjectAccessor::GetUnit(*me, mFollowGuid))
            me->GetMotionMaster()->MoveFollow(
                target, mFollowDist, mFollowAngle);
        me->ClearUnitState(UNIT_STATE_EVADE);
...
bool SmartAI::AssistPlayerInCombatAgainst(Unit* who)
{
    ...
    if (!(me->GetCreatureTemplate()->type_flags &
...
void SmartAI::JustRespawned()
{
    ...
    mFollowGuid.Clear(); // do not reset follower on Reset(), we need it after
                         // combat evade
    mFollowDist         = 0;
    mFollowAngle        = 0;
    mFollowCredit       = 0;
    ...
    if (who && me->Attack(who, me->IsWithinMeleeRange(who))) {
        if (mCanCombatMove) {
            ...
            MovementGeneratorType type =
                me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE);
                          ...
                          SpellSchoolMask /*damageSchoolMask*/)
{
    ...
    if (damagetype != NODAMAGE && mInvincibilityHpLevel &&
        (damage >= me->GetHealth() - mInvincibilityHpLevel))
        damage = me->GetHealth() -
                 mInvincibilityHpLevel; // damage should not be nullified,
                                        // because of player damage req.
}
...
{
    GetScript()->ProcessEventsFor(SMART_EVENT_JUST_SUMMONED,
                                  summoner->ToUnit(),
                                  0,
                                  0,
                                  false,
                                  nullptr,
                                  summoner->ToGameObject());
}
...
void SmartAI::CorpseRemoved(uint32& respawnDelay)
{
    ...
    // xinef: end escort upon corpse remove, safe check in case of immediate
    // despawn
    if (IsEscorted())
        EndPath(true);
}
...
{
    GetScript()->ProcessEventsFor(apply ? SMART_EVENT_PASSENGER_BOARDED
...
void SmartAI::OnCharmed(bool /* apply */)
{
    ...
    if (charmed) // do this before we change charmed state, as charmed state
                 // might prevent these things from processing
    {
...
uint32 SmartAI::GetData(uint32 /*id*/) const { return 0; }
...
ObjectGuid SmartAI::GetGUID(int32 /*id*/) const { return ObjectGuid::Empty; }
...
void SmartAI::SetSwim(bool swim) { me->SetSwim(swim); }

void SmartAI::SetEvadeDisabled(bool disable) { mEvadeDisabled = disable; }
...
{
    GetScript()->ProcessEventsFor(
        SMART_EVENT_GOSSIP_SELECT, player, sender, action);
}
...
{
    GetScript()->ProcessEventsFor(
        SMART_EVENT_REWARD_QUEST, player, quest->GetQuestId(), opt);
}
...
{
    GetScript()->ProcessEventsFor(start ? SMART_EVENT_GAME_EVENT_START
...
bool SmartGameObjectAI::GossipHello(Player* player, bool reportUse)
{
    ...
    GetScript()->ProcessEventsFor(SMART_EVENT_GOSSIP_HELLO,
                                  player,
                                  (uint32)reportUse,
                                  0,
                                  false,
...
{
    GetScript()->ProcessEventsFor(
        SMART_EVENT_GOSSIP_SELECT, player, sender, action, false, nullptr, me);
    return false;
}

// Called when a player selects a gossip with a code in the gameobject's gossip
// menu.
bool SmartGameObjectAI::GossipSelectCode(Player* /*player*/,
                                         uint32 /*sender*/,
                                         uint32 /*action*/,
                                         const char* /*code*/)
{
...
{
    GetScript()->ProcessEventsFor(SMART_EVENT_ACCEPTED_QUEST,
                                  player,
                                  quest->GetQuestId(),
                                  0,
                                  false,
...
bool SmartGameObjectAI::QuestReward(Player*      player,
                                    ...
                                    uint32       opt)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_REWARD_QUEST,
                                  player,
                                  quest->GetQuestId(),
                                  opt,
                                  false,
...
{
    GetScript()->ProcessEventsFor(
        SMART_EVENT_DEATH, player, eventId, 0, false, nullptr, me);
}
...
{
    GetScript()->ProcessEventsFor(
        SMART_EVENT_GO_EVENT_INFORM, nullptr, eventId);
}
...
{
    GetScript()->ProcessEventsFor(
        SMART_EVENT_SUMMON_DESPAWNED, unit, unit->GetEntry());
}
...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* trigger) override
    {
        ...
        LOG_DEBUG("sql.sql",
                  "AreaTrigger {} is using SmartTrigger script",
                  trigger->entry);
        SmartScript script;
        script.OnInitialize(nullptr, trigger);
        script.ProcessEventsFor(
            SMART_EVENT_AREATRIGGER_ONTRIGGER, player, trigger->entry);
        return true;
    }
};

void AddSC_SmartScripts() { new SmartTrigger(); }
