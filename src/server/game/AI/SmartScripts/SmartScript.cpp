...
void SmartScript::OnReset()
{
    ...
    ResetBaseObject();
    for (SmartAIEventList::iterator i = mEvents.begin(); i != mEvents.end();
         ++i) {
                                   ...
                                   GameObject*      gob)
{
    for (SmartAIEventList::iterator i = mEvents.begin(); i != mEvents.end();
        ...
        if (eventType == e) {
            ConditionList conds = sConditionMgr->GetConditionsForSmartEvent(
                (*i).entryOrGuid, (*i).event_id, (*i).source_type);
            ConditionSourceInfo info = ConditionSourceInfo(
                unit, GetBaseObject(), me ? me->GetVictim() : nullptr);
                                ...
                                GameObject*        gob)
{
    ...
    // Remove SMART_EVENT_FLAG_TEMP_IGNORE_CHANCE_ROLL flag after processing
    // roll chances as it's not needed anymore
    e.event.event_flags &= ~SMART_EVENT_FLAG_TEMP_IGNORE_CHANCE_ROLL;
    ...
    if (Unit* tempInvoker = GetLastInvoker())
        LOG_DEBUG("sql.sql",
                  "SmartScript::ProcessAction: Invoker: {} ({})",
                  tempInvoker->GetName(),
                  tempInvoker->GetGUID().ToString());
    ...
    case SMART_ACTION_TALK: {
        ...
        for (WorldObject* target : targets) {
            if (IsCreature((target)) &&
                !target->ToCreature()
                     ->IsPet()) // Prevented sending text to pets.
            {
        ...
        if (!sCreatureTextMgr->TextExist(talker->GetEntry(),
                                         uint8(e.action.talk.textGroupID))) {
            LOG_ERROR("sql.sql",
                      "SmartScript::ProcessAction: SMART_ACTION_TALK: "
                      "EntryOrGuid {} SourceType {} EventType {} TargetType {} "
                      "using non-existent Text id {} for talker {}, ignored.",
                      e.entryOrGuid,
                      e.GetScriptType(),
                      e.GetEventType(),
                      e.GetTargetType(),
                      e.action.talk.textGroupID,
                      talker->GetEntry());
            break;
        }
        ...
        sCreatureTextMgr->SendChat(
            talker, uint8(e.action.talk.textGroupID), talkTarget);
        LOG_DEBUG("sql.sql",
                  "SmartScript::ProcessAction: SMART_ACTION_TALK: talker: {} "
                  "({}), textId: {}",
                  talker->GetName(),
                  talker->GetGUID().ToString(),
                  mLastTextID);
        break;
    }
    case SMART_ACTION_SIMPLE_TALK: {
        for (WorldObject* target : targets) {
            if (IsCreature(target))
                sCreatureTextMgr->SendChat(
                    target->ToCreature(),
                    uint8(e.action.simpleTalk.textGroupID),
                    IsPlayer(GetLastInvoker()) ? GetLastInvoker() : 0);
            else if (IsPlayer(target) && me) {
                Unit* templastInvoker = GetLastInvoker();
                sCreatureTextMgr->SendChat(
                    me,
                    uint8(e.action.simpleTalk.textGroupID),
                    IsPlayer(templastInvoker) ? templastInvoker : 0,
                    CHAT_MSG_ADDON,
                    LANG_ADDON,
                    TEXT_RANGE_NORMAL,
                    0,
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                ...
                LOG_DEBUG("sql.sql",
                          ...
                          "SMART_ACTION_PLAY_EMOTE: target: {} ({}), emote: {}",
                          target->GetName(),
                          target->GetGUID().ToString(),
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                if (e.action.sound.distance == 1)
                    target->PlayDistanceSound(
                        e.action.sound.sound,
                        e.action.sound.onlySelf ? target->ToPlayer() : nullptr);
                else
                    target->PlayDirectSound(
                        e.action.sound.sound,
                        e.action.sound.onlySelf ? target->ToPlayer() : nullptr);
                LOG_DEBUG("sql.sql",
                          "SmartScript::ProcessAction:: SMART_ACTION_SOUND: "
                          "target: {} ({}), sound: {}, onlyself: {}",
                          target->GetName(),
                          target->GetGUID().ToString(),
                          e.action.sound.sound,
    ...
    case SMART_ACTION_RANDOM_SOUND: {
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                ...
                target->PlayDirectSound(sound,
                                        e.action.randomSound.onlySelf
                                            ? target->ToPlayer()
                                            : nullptr);
                LOG_DEBUG(
                    "sql.sql",
    ...
    case SMART_ACTION_MUSIC: {
        ...
        if (e.action.music.type > 0) {
            if (me && me->FindMap()) {
                ...
                if (!players.IsEmpty()) {
                    for (Map::PlayerList::const_iterator i = players.begin();
                         i != players.end();
                         ++i)
        ...
        if (!targets.empty()) {
            for (WorldObject* target : targets) {
                if (IsUnit(target)) {
                    target->SendPlayMusic(e.action.music.sound,
                                          e.action.music.onlySelf > 0);
                    LOG_DEBUG(
                        "sql.sql",
                        "SmartScript::ProcessAction:: SMART_ACTION_MUSIC: "
                        "target: {} ({}), sound: {}, onlySelf: {}, type: {}",
                        target->GetName(),
                        target->GetGUID().ToString(),
                        e.action.music.sound,
                        e.action.music.onlySelf,
                        e.action.music.type);
                }
            }
        }
    ...
    case SMART_ACTION_RANDOM_MUSIC: {
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                ...
                LOG_DEBUG(
                    "sql.sql",
                    "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_MUSIC: "
                    "target: {} ({}), sound: {}, onlyself: {}, type: {}",
                    target->GetName(),
                    target->GetGUID().ToString(),
                    sound,
                    e.action.randomMusic.onlySelf,
                    e.action.randomMusic.type);
            }
        }
        ...
        for (WorldObject* target : targets) {
            if (IsCreature(target)) {
                if (e.action.faction.factionID) {
                    target->ToCreature()->SetFaction(
                        e.action.faction.factionID);
                    LOG_DEBUG("scripts.ai",
                              "SmartScript::ProcessAction:: "
                              "SMART_ACTION_SET_FACTION: Creature entry {}, "
                              "GuidLow {} set faction to {}",
                              target->GetEntry(),
                              target->GetGUID().ToString(),
                              e.action.faction.factionID);
                }
                    ...
                    if (CreatureTemplate const* ci =
                        ...
                        if (target->ToCreature()->GetFaction() != ci->faction) {
                            ...
                            LOG_DEBUG("scripts.ai",
                                      "SmartScript::ProcessAction:: "
                                      "SMART_ACTION_SET_FACTION: Creature "
                                      "entry {}, GuidLow {} set faction to {}",
                                      target->GetEntry(),
                                      target->GetGUID().ToString(),
                                      ci->faction);
                        }
        ...
        for (WorldObject* target : targets) {
            ...
            if (e.action.morphOrMount.creature || e.action.morphOrMount.model) {
                ...
                if (e.action.morphOrMount.creature) {
                    if (CreatureTemplate const* ci =
                        ...
                        LOG_DEBUG(
                            ...
                            "entry {}, GuidLow {} set displayid to {}",
                            target->GetEntry(),
                            target->GetGUID().ToString(),
                            displayId);
                    }
                ...
                // if no param1, then use value from param2 (modelId)
                    ...
                    target->ToCreature()->SetDisplayId(
                    ...
                    if (me && e.action.questOffer.directAdd == 0) {
                        if (player->CanTakeQuest(q, true)) {
                            if (WorldSession* session = player->GetSession()) {
                                ...
                                menu.SendQuestGiverQuestDetails(
                                    q, me->GetGUID(), true);
                                LOG_DEBUG("scripts.ai",
                                          "SmartScript::ProcessAction:: "
                                          "SMART_ACTION_OFFER_QUEST: Player "
                                          "guidLow {} - offering quest {}",
                        ...
                        LOG_DEBUG("scripts.ai",
                                  "SmartScript::ProcessAction:: "
                                  "SMART_ACTION_OFFER_QUEST: Player guidLow {} "
                                  "- quest {} added",
                                  player->GetGUID().ToString(),
                                  e.action.questOffer.questID);
                    }
    ...
    case SMART_ACTION_SET_REACT_STATE: {
        for (WorldObject* target : targets) {
            ...
            target->ToCreature()->SetReactState(
                ReactStates(e.action.react.state));
        }
        break;
    }
    case SMART_ACTION_RANDOM_EMOTE: {
        std::vector<uint32> emotes;
        std::copy_if(e.action.randomEmote.emotes.begin(),
                     e.action.randomEmote.emotes.end(),
                     std::back_inserter(emotes),
                     [](uint32 emote) { return emote != 0; });

        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                uint32 emote =
                    Acore::Containers::SelectRandomContainerElement(emotes);
                target->ToUnit()->HandleEmoteCommand(emote);
                LOG_DEBUG(
                    "scripts.ai",
                    "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_EMOTE: "
                    "Creature guidLow {} handle random emote {}",
                    target->GetGUID().ToString(),
                    emote);
            }
        }
    ...
    case SMART_ACTION_THREAT_ALL_PCT: {
        ...
        ThreatContainer::StorageType threatList =
            me->GetThreatMgr().GetThreatList();
        for (ThreatContainer::StorageType::const_iterator i =
                 threatList.begin();
             i != threatList.end();
             ++i) {
            if (Unit* target =
                    ObjectAccessor::GetUnit(*me, (*i)->getUnitGuid())) {
                me->GetThreatMgr().ModifyThreatByPercent(
                    target,
                ...
                LOG_DEBUG(
                    ...
                    "Creature {} modify threat for unit {}, value {}",
                    me->GetGUID().ToString(),
                    target->GetGUID().ToString(),
                    e.action.threatPCT.threatINC
                        ? (int32)e.action.threatPCT.threatINC
    ...
    case SMART_ACTION_THREAT_SINGLE_PCT: {
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                ...
                LOG_DEBUG("scripts.ai",
                          "SmartScript::ProcessAction:: "
                          "SMART_ACTION_THREAT_SINGLE_PCT: Creature guidLow {} "
                          "modify threat for unit {}, value {}",
                          me->GetGUID().ToString(),
                          target->GetGUID().ToString(),
                          e.action.threatPCT.threatINC
                              ? (int32)e.action.threatPCT.threatINC
            ...
            if (IsUnit(target))
                if (Vehicle* vehicle = target->ToUnit()->GetVehicleKit())
                    for (auto& Seat : vehicle->Seats)
                        if (Player* player = ObjectAccessor::GetPlayer(
                                *target, Seat.second.Passenger.Guid))
                            player->AreaExploredOrEventHappens(
                                e.action.quest.quest);

            if (IsPlayer(target)) {
                target->ToPlayer()->AreaExploredOrEventHappens(
                    e.action.quest.quest);

                LOG_DEBUG("scripts.ai",
                          "SmartScript::ProcessAction:: "
                          "SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS: "
                          "Player guidLow {} credited quest {}",
                          target->GetGUID().ToString(),
                          e.action.quest.quest);
            }
        }
    ...
    case SMART_ACTION_CAST: {
        ...
        if (e.GetScriptType() == SMART_SCRIPT_TYPE_AREATRIGGER)
            caster = unit->SummonTrigger(unit->GetPositionX(),
                                         unit->GetPositionY(),
                                         unit->GetPositionZ(),
                                         unit->GetOrientation(),
                                         5000);
        ...
        if (e.action.cast.targetsLimit)
            Acore::Containers::RandomResize(targets,
                                            e.action.cast.targetsLimit);
        ...
        for (WorldObject* target : targets) {
            ...
            if (caster && caster != me) // Areatrigger cast
            {
                caster->CastSpell(
                    target->ToUnit(),
                    e.action.cast.spell,
                    (e.action.cast.castFlags & SMARTCAST_TRIGGERED));
            }
            else if (me) {
                // If target has the aura, skip
                if ((e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) &&
                ...
                if ((e.action.cast.castFlags & SMARTCAST_TARGET_POWER_MANA) &&
                    !target->ToUnit()->GetPower(POWER_MANA))
                    continue;
                ...
                SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(e.action.cast.spell);
                float distanceToTarget = me->GetDistance(target->ToUnit());
                float spellMaxRange =
                    me->GetSpellMaxRangeForTarget(target->ToUnit(), spellInfo);
                float spellMinRange =
                    me->GetSpellMinRangeForTarget(target->ToUnit(), spellInfo);
                ...
                bool isTargetRooted =
                    target->ToUnit()->HasUnitState(UNIT_STATE_ROOT);
                // To prevent running back and forth when OOM, we must have more
                // than 10% mana.
                bool canCastSpell =
                    me->GetPowerPct(POWER_MANA) > 10.0f &&
                    spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask()) <
                        (int32)me->GetPower(POWER_MANA) &&
                    !me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SILENCED);
                bool isSpellIgnoreLOS =
                    spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT);

                // If target is rooted we move out of melee range before
                // casting, but not further than spell max range.
                if (isWithinLOSInMap && isWithinMeleeRange && isRangedAttack &&
                    isTargetRooted && canCastSpell) {
                    failedSpellCast = true; // Mark spellcast as failed so we
                                            // can retry it later
                    float minDistance = std::max(meleeRange, spellMinRange) -
                                        distanceToTarget + NOMINAL_MELEE_RANGE;
                    CAST_AI(SmartAI, me->AI())
                        ->MoveAway(std::min(minDistance, spellMaxRange));
                    continue;
                }

                // Let us not try to cast spell if we know it is going to fail
                // anyway. Stick to chasing and continue.
                if (distanceToTarget > spellMaxRange && isWithinLOSInMap) {
                    failedSpellCast = true;
                    CAST_AI(SmartAI, me->AI())
                        ->SetCombatMove(
                            true,
                            std::max(spellMaxRange - NOMINAL_MELEE_RANGE,
                                     0.0f));
                    continue;
                }
                else if (distanceToTarget < spellMinRange ||
                         !(isWithinLOSInMap || isSpellIgnoreLOS)) {
                    failedSpellCast = true;
                ...
                if (e.action.cast.castFlags & SMARTCAST_TRIGGERED) {
                    if (e.action.cast.triggerFlags)
                        triggerFlags =
                            TriggerCastFlags(e.action.cast.triggerFlags);
                    else
                        triggerFlags = TRIGGERED_FULL_MASK;
                }

                SpellCastResult result = me->CastSpell(
                    target->ToUnit(), e.action.cast.spell, triggerFlags);
                bool spellCastFailed =
                    (result != SPELL_CAST_OK &&
                     result != SPELL_FAILED_SPELL_IN_PROGRESS);

                if (e.action.cast.castFlags & SMARTCAST_COMBAT_MOVE) {
                    // If cast flag SMARTCAST_COMBAT_MOVE is set combat movement
                    // will not be allowed unless target is outside spell range,
                    // out of mana, or LOS.
                    if (result == SPELL_FAILED_OUT_OF_RANGE)
                        // if we are just out of range, we only chase until we
                        // are back in spell range.
                        CAST_AI(SmartAI, me->AI())
                            ->SetCombatMove(
                                true,
                                std::max(spellMaxRange - NOMINAL_MELEE_RANGE,
                        ...
                        // if spell fail for any other reason, we chase to melee
                        // range, or stay where we are if spellcast was
                        // successful.
                        CAST_AI(SmartAI, me->AI())
                            ->SetCombatMove(spellCastFailed);
                }
                ...
                LOG_DEBUG(
                    "scripts.ai",
                    "SmartScript::ProcessAction:: SMART_ACTION_CAST: Unit {} "
                    "casts spell {} on target {} with castflags {}",
                    me->GetGUID().ToString(),
                    e.action.cast.spell,
                    target->GetGUID().ToString(),
        ...
        // If there is at least 1 failed cast and no successful casts at all,
    ...
    case SMART_ACTION_INVOKER_CAST: {
        ...
        for (WorldObject* target : targets) {
            ...
            if (!(e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) ||
                ...
                tempLastInvoker->CastSpell(
                    target->ToUnit(), e.action.cast.spell, triggerFlags);
                LOG_DEBUG(
                    "scripts.ai",
                    "SmartScript::ProcessAction:: SMART_ACTION_INVOKER_CAST: "
                    "Invoker {} casts spell {} on target {} with castflags {}",
                    tempLastInvoker->GetGUID().ToString(),
                    e.action.cast.spell,
                    target->GetGUID().ToString(),
                    e.action.cast.castFlags);
            }
                ...
                LOG_DEBUG("scripts.ai",
                          "Spell {} not cast because it has flag "
    ...
    case SMART_ACTION_ACTIVATE_GOBJECT: {
        for (WorldObject* target : targets) {
            if (IsGameObject(target)) {
                ...
                go->UseDoorOrButton(
                    0, !!e.action.activateObject.alternative, unit);
                LOG_DEBUG(
                    "sql.sql",
                    "SmartScript::ProcessAction:: "
                    "SMART_ACTION_ACTIVATE_GOBJECT. Gameobject {} activated",
                    go->GetGUID().ToString());
            }
        }
    ...
    case SMART_ACTION_SET_EMOTE_STATE: {
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                target->ToUnit()->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                                                 e.action.emote.emote);
                LOG_DEBUG("scripts.ai",
                          "SmartScript::ProcessAction:: "
                          "SMART_ACTION_SET_EMOTE_STATE. Unit {} set "
                          "emotestate to {}",
                          target->GetGUID().ToString(),
                          e.action.emote.emote);
            }
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                if (!e.action.unitFlag.type) {
                    target->ToUnit()->SetFlag(UNIT_FIELD_FLAGS,
                                              e.action.unitFlag.flag);
                    LOG_DEBUG("scripts.ai",
                              "SmartScript::ProcessAction:: "
                              "SMART_ACTION_SET_UNIT_FLAG. Unit {} added flag "
                              "{} to UNIT_FIELD_FLAGS",
                              target->GetGUID().ToString(),
                              e.action.unitFlag.flag);
                }
                else {
                    target->ToUnit()->SetFlag(UNIT_FIELD_FLAGS_2,
                                              e.action.unitFlag.flag);
                    LOG_DEBUG("scripts.ai",
                              "SmartScript::ProcessAction:: "
                              "SMART_ACTION_SET_UNIT_FLAG. Unit {} added flag "
                              "{} to UNIT_FIELD_FLAGS_2",
                              target->GetGUID().ToString(),
                              e.action.unitFlag.flag);
                }
            }
    ...
    case SMART_ACTION_REMOVE_UNIT_FLAG: {
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                if (!e.action.unitFlag.type) {
                    target->ToUnit()->RemoveFlag(UNIT_FIELD_FLAGS,
                                                 e.action.unitFlag.flag);
                    LOG_DEBUG("scripts.ai",
                              "SmartScript::ProcessAction:: "
                              "SMART_ACTION_REMOVE_UNIT_FLAG. Unit {} removed "
                              "flag {} to UNIT_FIELD_FLAGS",
                              target->GetGUID().ToString(),
                              e.action.unitFlag.flag);
                }
                else {
                    target->ToUnit()->RemoveFlag(UNIT_FIELD_FLAGS_2,
                                                 e.action.unitFlag.flag);
                    LOG_DEBUG("scripts.ai",
                              "SmartScript::ProcessAction:: "
                              "SMART_ACTION_REMOVE_UNIT_FLAG. Unit {} removed "
                              "flag {} to UNIT_FIELD_FLAGS_2",
                              target->GetGUID().ToString(),
                              e.action.unitFlag.flag);
                }
            }
    ...
    case SMART_ACTION_SET_EVENT_PHASE: {
        ...
        LOG_DEBUG("sql.sql",
                  ...
                  "Creature {} set event phase {}",
                  GetBaseObject()->GetGUID().ToString(),
    ...
    case SMART_ACTION_FLEE_FOR_ASSIST: {
        ...
        if (!me || me->HasUnitState(UNIT_STATE_LOST_CONTROL) ||
            me->GetSpeed(MOVE_RUN) < 0.1f)
            break;

        me->DoFleeToGetAssistance();
        if (e.action.flee.withEmote) {
            Acore::BroadcastTextBuilder builder(me,
                                                CHAT_MSG_MONSTER_EMOTE,
                                                BROADCAST_TEXT_FLEE_FOR_ASSIST,
                                                me->getGender());
            sCreatureTextMgr->SendChatPacket(
                me, builder, CHAT_MSG_MONSTER_EMOTE);
        }
        LOG_DEBUG("sql.sql",
    ...
    case SMART_ACTION_CALL_GROUPEVENTHAPPENS: {
        for (WorldObject* target : targets) {
            ...
            if (player && GetBaseObject()) {
                player->GroupEventHappens(e.action.quest.quest,
                                          GetBaseObject());
                LOG_DEBUG("scripts.ai",
                          "SmartScript::ProcessAction: "
                          "SMART_ACTION_CALL_GROUPEVENTHAPPENS: Player {}, "
                          "group credit for quest {}",
                          player->GetGUID().ToString(),
                          e.action.quest.quest);
            }
            ...
            if (Vehicle* vehicle = unitTarget->GetVehicleKit()) {
                for (auto& Seat : vehicle->Seats) {
                    if (Player* player = ObjectAccessor::GetPlayer(
                            *unitTarget, Seat.second.Passenger.Guid)) {
                        player->GroupEventHappens(e.action.quest.quest,
                                                  GetBaseObject());
                    }
                }
        ...
        for (WorldObject* target : targets) {
            ...
            if (e.action.removeAura.spell) {
                if (e.action.removeAura.charges) {
                    if (Aura* aur = target->ToUnit()->GetAura(
                            e.action.removeAura.spell))
                        aur->ModCharges(
                            -static_cast<int32>(e.action.removeAura.charges),
                            AURA_REMOVE_BY_EXPIRE);
                }
                else
                    target->ToUnit()->RemoveAurasDueToSpell(
                        e.action.removeAura.spell);
            }
            else
            ...
            LOG_DEBUG("scripts.ai",
                      "SmartScript::ProcessAction: "
                      "SMART_ACTION_REMOVEAURASFROMSPELL: Unit {}, spell {}",
                      target->GetGUID().ToString(),
                      e.action.removeAura.spell);
        }
        break;
    ...
    case SMART_ACTION_FOLLOW: {
        ...
        if (e.target.type == SMART_TARGET_NONE ||
            e.target.type == SMART_TARGET_SELF) {
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                float angle = e.action.follow.angle > 6
                                  ? (e.action.follow.angle * M_PI / 180.0f)
                                  : e.action.follow.angle;
                CAST_AI(SmartAI, me->AI())
                    ->SetFollow(target->ToUnit(),
                                float(e.action.follow.dist) + 0.1f,
                                angle,
                                e.action.follow.credit,
                                e.action.follow.entry,
                                e.action.follow.creditType,
                                e.action.follow.aliveState);
                LOG_DEBUG("scripts.ai",
                          "SmartScript::ProcessAction: SMART_ACTION_FOLLOW: "
                          "Creature {} following target {}",
                          me->GetGUID().ToString(),
                          target->GetGUID().ToString());
                break;
            }
    ...
    case SMART_ACTION_RANDOM_PHASE: {
        ...
        std::copy_if(e.action.randomPhase.phases.begin(),
                     e.action.randomPhase.phases.end(),
    ...
    case SMART_ACTION_CALL_KILLEDMONSTER: {
        if (trigger && IsPlayer(unit)) {
            unit->ToPlayer()->RewardPlayerAndGroupAtEvent(
                e.action.killedMonster.creature, unit);
            LOG_DEBUG(
                "sql.sql",
                "SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: "
                "(trigger == true) Player {}, Killcredit: {}",
                unit->GetGUID().ToString(),
                e.action.killedMonster.creature);
        }
        else if (e.target.type == SMART_TARGET_NONE ||
                 e.target.type ==
                     SMART_TARGET_SELF) // Loot recipient and his group members
        {
            if (!me)
                break;
            ...
            if (Player* player = me->GetLootRecipient()) {
                ...
                LOG_DEBUG("sql.sql",
                          "SmartScript::ProcessAction: "
                          "SMART_ACTION_CALL_KILLEDMONSTER: Player {}, "
                          "Killcredit: {}",
                          player->GetGUID().ToString(),
                          e.action.killedMonster.creature);
            }
        ...
        else // Specific target type
        {
            for (WorldObject* target : targets) {
                ...
                Player* player =
                    target->ToUnit()->GetCharmerOrOwnerPlayerOrPlayerItself();
                if (!player)
                    continue;
    ...
    case SMART_ACTION_SET_INST_DATA: {
        ...
        if (!instance) {
            LOG_ERROR("scripts.ai.sai",
                      "SmartScript: Event {} attempt to set instance data "
                      "without instance script. EntryOrGuid {}",
                      e.GetEventType(),
                      e.entryOrGuid);
            break;
        }
        ...
        case 1: {
            instance->SetBossState(
                e.action.setInstanceData.field,
                static_cast<EncounterState>(e.action.setInstanceData.data));
            LOG_DEBUG("scripts.ai.sai",
                      "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: "
    ...
    case SMART_ACTION_SET_INST_DATA64: {
        ...
        instance->SetGuidData(e.action.setInstanceData64.field,
                              targets.front()->GetGUID());
        LOG_DEBUG("scripts.ai",
                  "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA64: "
                  "Field: {}, data: {}",
                  e.action.setInstanceData64.field,
                  targets.front()->GetGUID().ToString());
        break;
    }
    case SMART_ACTION_UPDATE_TEMPLATE: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToCreature()->UpdateEntry(
                    e.action.updateTemplate.creature,
                    target->ToCreature()->GetCreatureData(),
                    e.action.updateTemplate.updateLevel != 0);
        break;
    }
        ...
        if (e.action.die.milliseconds) {
            if (me && !me->isDead()) {
                me->m_Events.AddEventAtOffset(
                    [&] {
                        // We need to check again to see if we didn't die in the
                        // process.
                        if (me && !me->isDead()) {
                            me->KillSelf();
                            LOG_DEBUG("sql.sql",
                                      "SmartScript::ProcessAction: "
                                      "SMART_ACTION_DIE: Creature {}",
                                      me->GetGUID().ToString());
                        }
                    },
    ...
    case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: {
        ...
        if (!me->GetMap()->IsDungeon()) {
            ...
            GetWorldObjectsInDist(
                units, static_cast<float>(e.target.unitRange.maxDist));
            ...
            for (WorldObject* target : targets) {
                if (IsCreature(target)) {
                    ...
                    LOG_DEBUG("scripts.ai",
                              "SmartScript::ProcessAction: "
                              "SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: Creature "
                              "{}, target: {}",
                              me->GetGUID().ToString(),
                              target->GetGUID().ToString());
                }
    ...
    case SMART_ACTION_CALL_FOR_HELP: {
        for (WorldObject* target : targets) {
            if (IsCreature(target)) {
                target->ToCreature()->CallForHelp(
                    float(e.action.callHelp.range));
                if (e.action.callHelp.withEmote) {
                    Acore::BroadcastTextBuilder builder(
                        ...
                        nullptr);
                    sCreatureTextMgr->SendChatPacket(
                        target, builder, CHAT_MSG_MONSTER_EMOTE);
                }
                LOG_DEBUG("scripts.ai",
                          "SmartScript::ProcessAction: "
                          "SMART_ACTION_CALL_FOR_HELP: Creature {}, target: {}",
                          me->GetGUID().ToString(),
                          target->GetGUID().ToString());
            }
        }
    ...
    case SMART_ACTION_FORCE_DESPAWN: {
        for (WorldObject* target : targets) {
            ...
            // Wait at least one world update tick before despawn, so it doesn't
            // break linked actions.
            if (despawnDelay <= 0ms) {
                despawnDelay = 1ms;
            }
    ...
    case SMART_ACTION_SET_INGAME_PHASE_MASK: {
        for (WorldObject* target : targets) {
            if (IsUnit(target))
                target->ToUnit()->SetPhaseMask(e.action.ingamePhaseMask.mask,
                                               true);
            else if (IsGameObject(target))
                target->ToGameObject()->SetPhaseMask(
                    e.action.ingamePhaseMask.mask, true);
        }
        break;
    }
                ...
                if (e.action.morphOrMount.creature > 0) {
                    if (CreatureTemplate const* cInfo =
                            sObjectMgr->GetCreatureTemplate(
                                e.action.morphOrMount.creature))
                        target->ToUnit()->Mount(
                            ObjectMgr::ChooseDisplayId(cInfo));
                }
                else
        ...
        for (WorldObject* target : targets) {
            if (IsCreature(target)) {
                ...
                if (e.action.invincHP.percent)
                    ai->SetInvincibilityHpLevel(
                        target->ToCreature()->CountPctFromMaxHealth(
    ...
    case SMART_ACTION_SET_DATA: {
        for (WorldObject* target : targets) {
            if (IsCreature(target))
                target->ToCreature()->AI()->SetData(e.action.setData.field,
                                                    e.action.setData.data);
            else if (IsGameObject(target))
                target->ToGameObject()->AI()->SetData(e.action.setData.field,
                                                      e.action.setData.data);
        }
        break;
    }
    case SMART_ACTION_MOVE_FORWARD: {
        ...
        me->GetClosePoint(x,
                          ...
                          z,
                          me->GetObjectSize() / 3,
                          (float)e.action.moveRandom.distance);
    ...
    case SMART_ACTION_RISE_UP: {
        ...
        me->GetMotionMaster()->MovePoint(
            ...
            me->GetPositionX(),
            me->GetPositionY(),
            me->GetPositionZ() + (float)e.action.moveRandom.distance);
        break;
    }
    ...
    case SMART_ACTION_ATTACK_START: {
        ...
        if (Unit* target =
                Acore::Containers::SelectRandomContainerElement(targets)
                    ->ToUnit())
    ...
    case SMART_ACTION_SUMMON_CREATURE: {
        EnumFlag<SmartActionSummonCreatureFlags> flags(
            static_cast<SmartActionSummonCreatureFlags>(
                e.action.summonCreature.flags));
        bool preferUnit =
            flags.HasFlag(SmartActionSummonCreatureFlags::PreferUnit);
        WorldObject* summoner =
            preferUnit ? unit : Coalesce<WorldObject>(GetBaseObject(), unit);
        if (!summoner)
            break;

        bool personalSpawn =
            flags.HasFlag(SmartActionSummonCreatureFlags::PersonalSpawn);
        ...
        if (e.GetTargetType() == SMART_TARGET_RANDOM_POINT) {
            ...
            for (uint32 i = 0; i < e.target.randomPoint.amount; i++) {
                ...
                if (Creature* summon = summoner->SummonCreature(
                        e.action.summonCreature.creature,
                        randomPoint,
        ...
        for (WorldObject* target : targets) {
            ...
            if (Creature* summon = summoner->SummonCreature(
                ...
                if (e.action.summonCreature.attackInvoker ==
                    2) // pussywizard: proper attackInvoker implementation
                    summon->AI()->AttackStart(unit);
    ...
    case SMART_ACTION_SUMMON_GO: {
            ...
            for (WorldObject* target : targets) {
                ...
                if (!e.action.summonGO.targetsummon)
                    GetBaseObject()->SummonGameObject(
                        ...
                        0,
                        0,
                        e.action.summonGO.despawnTime);
                else
                    target->SummonGameObject(e.action.summonGO.entry,
                                             GetBaseObject()->GetPositionX(),
                                             GetBaseObject()->GetPositionY(),
                                             GetBaseObject()->GetPositionZ(),
                                             GetBaseObject()->GetOrientation(),
                                             0,
                                             0,
                                             0,
                                             0,
        ...
        GetBaseObject()->SummonGameObject(e.action.summonGO.entry,
                                          e.target.x,
                                          e.target.y,
                                          e.target.z,
                                          e.target.o,
    ...
    case SMART_ACTION_ADD_ITEM: {
        for (WorldObject* target : targets) {
            ...
            target->ToPlayer()->AddItem(e.action.item.entry,
                                        e.action.item.count);
        }
        break;
    }
    case SMART_ACTION_REMOVE_ITEM: {
        for (WorldObject* target : targets) {
            ...
            target->ToPlayer()->DestroyItemCount(
                e.action.item.entry, e.action.item.count, true);
        }
        break;
    }
    ...
    case SMART_ACTION_TELEPORT: {
        for (WorldObject* target : targets) {
            if (IsPlayer(target))
                target->ToPlayer()->TeleportTo(e.action.teleport.mapID,
                                               e.target.x,
                                               e.target.y,
                                               e.target.z,
                                               e.target.o);
            else if (IsCreature(target))
                target->ToCreature()->NearTeleportTo(
        ...
        for (WorldObject* target : targets) {
            if (IsCreature(target)) {
                if (IsSmart(target->ToCreature()))
                    CAST_AI(SmartAI, target->ToCreature()->AI())
                        ->SetRun(e.action.setRun.run);
                else
                    target->ToCreature()->SetWalk(
                        e.action.setRun.run ? false : true); // Xinef: reversed
            }
        }
        ...
        if (!targets.empty()) {
            for (WorldObject* target : targets) {
                if (IsCreature(target)) {
                    if (SmartAI* ai =
                            CAST_AI(SmartAI, target->ToCreature()->AI()))
                        ai->GetScript()->StoreCounter(
                            e.action.setCounter.counterId,
                            e.action.setCounter.value,
                            e.action.setCounter.reset,
                            e.action.setCounter.subtract);
                    else
                        LOG_ERROR("sql.sql",
                                  "SmartScript: Action target for "
                                  "SMART_ACTION_SET_COUNTER is not using "
                                  "SmartAI, skipping");
                }
                else if (IsGameObject(target)) {
                    if (SmartGameObjectAI* ai = CAST_AI(
                            SmartGameObjectAI, target->ToGameObject()->AI()))
                        ai->GetScript()->StoreCounter(
                            e.action.setCounter.counterId,
                            e.action.setCounter.value,
                            e.action.setCounter.reset,
                        ...
                        LOG_ERROR("sql.sql",
                                  "SmartScript: Action target for "
                                  "SMART_ACTION_SET_COUNTER is not using "
                                  "SmartGameObjectAI, skipping");
                }
            }
        }
        else
            StoreCounter(e.action.setCounter.counterId,
                         e.action.setCounter.value,
                         e.action.setCounter.reset,
                         e.action.setCounter.subtract);
        break;
    ...
    case SMART_ACTION_WP_PAUSE: {
        ...
        CAST_AI(SmartAI, me->AI())
            ->PausePath(delay,
                        e.GetEventType() == SMART_EVENT_WAYPOINT_REACHED
                            ? false
                            : true);
        break;
    }
    ...
    case SMART_ACTION_SET_ORIENTATION: {
        ...
        if (e.action.orientation.turnAngle) {
            float turnOri =
                me->GetOrientation() +
                (static_cast<float>(e.action.orientation.turnAngle) * M_PI /
                 180.0f);
        ...
        if (e.GetTargetType() == SMART_TARGET_SELF) {
            me->SetFacingTo(
                (me->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) &&
                         me->GetTransGUID()
    ...
    case SMART_ACTION_MOVE_TO_POS: {
        ...
        /*if (e.GetTargetType() == SMART_TARGET_CREATURE_RANGE ||
        e.GetTargetType() == SMART_TARGET_CREATURE_GUID || e.GetTargetType() ==
        SMART_TARGET_CREATURE_DISTANCE || e.GetTargetType() ==
        SMART_TARGET_GAMEOBJECT_RANGE || e.GetTargetType() ==
        SMART_TARGET_GAMEOBJECT_GUID || e.GetTargetType() ==
        SMART_TARGET_GAMEOBJECT_DISTANCE || e.GetTargetType() ==
        SMART_TARGET_CLOSEST_CREATURE || e.GetTargetType() ==
        SMART_TARGET_CLOSEST_GAMEOBJECT || e.GetTargetType() ==
        SMART_TARGET_OWNER_OR_SUMMONER || e.GetTargetType() ==
        SMART_TARGET_ACTION_INVOKER || e.GetTargetType() ==
        SMART_TARGET_CLOSEST_ENEMY || e.GetTargetType() ==
        SMART_TARGET_CLOSEST_FRIENDLY || e.GetTargetType() == SMART_TARGET_SELF
        || e.GetTargetType() == SMART_TARGET_STORED)) */
        {
        ...
        else // Xinef: we can use dest.x, dest.y, dest.z to make offset
        {
            ...
            if (e.action.moveToPos.ContactDistance > 0) {
                target->GetNearPoint(me,
                                     x,
                                     y,
                                     z,
                                     e.action.moveToPos.ContactDistance,
                                     0,
                                     target->GetAngle(me));
            }
            me->GetMotionMaster()->MovePoint(
    ...
    case SMART_ACTION_MOVE_TO_POS_TARGET: {
        for (WorldObject* target : targets) {
            if (IsCreature(target)) {
                ...
                ctarget->GetMotionMaster()->MovePoint(
                    e.action.moveToPos.pointId,
                    e.target.x,
                    e.target.y,
                    e.target.z,
        ...
        for (WorldObject* target : targets) {
            if (IsCreature(target))
                ...
                if (target->ToGameObject()->isSpawnedByDefault())
                    ...
                    target->ToGameObject()->SetRespawnTime(
                        e.action.RespawnTarget.goRespawnTime);
            }
        }
        break;
    }
        ...
        for (WorldObject* target : targets) {
            if (Creature* npc = target->ToCreature()) {
                ...
                if (int8 equipId = static_cast<int8>(e.action.equip.entry)) {
                    EquipmentInfo const* eInfo =
                        sObjectMgr->GetEquipmentInfo(npc->GetEntry(), equipId);
                    if (!eInfo) {
                        LOG_ERROR(
                            "sql.sql",
                            "SmartScript: SMART_ACTION_EQUIP uses non-existent "
                    ...
                    std::copy(std::begin(eInfo->ItemEntry),
                              std::end(eInfo->ItemEntry),
                              std::begin(slot));
                }
                else
                    std::copy(std::begin(e.action.equip.slots),
                              std::end(e.action.equip.slots),
                              std::begin(slot));

                for (uint32 i = 0; i < MAX_EQUIPMENT_ITEMS; ++i)
                    if (!e.action.equip.mask ||
                        (e.action.equip.mask & (1 << i)))
                        npc->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i,
                                            slot[i]);
            }
        }
        break;
    ...
    case SMART_ACTION_SET_RANGED_MOVEMENT: {
        ...
        float attackAngle =
            float(e.action.setRangedMovement.angle) / 180.0f * float(M_PI);

        for (WorldObject* target : targets)
            if (Creature* creature = target->ToCreature())
                if (IsSmart(creature) && creature->GetVictim())
                    if (CAST_AI(SmartAI, creature->AI())->CanCombatMove())
                        creature->GetMotionMaster()->MoveChase(
                            creature->GetVictim(), attackDistance, attackAngle);
    ...
    case SMART_ACTION_CALL_TIMED_ACTIONLIST: {
        if (e.GetTargetType() == SMART_TARGET_NONE) {
            LOG_ERROR("sql.sql",
                      "SmartScript: Entry {} SourceType {} Event {} Action {} "
                      "is using TARGET_NONE(0) for Script9 target. Please "
                      "correct target_type in database.",
                      e.entryOrGuid,
                      e.GetScriptType(),
                      e.GetEventType(),
                      e.GetActionType());
        ...
        for (WorldObject* target : targets) {
            if (Creature* creature = target->ToCreature()) {
                if (IsSmart(creature))
                    CAST_AI(SmartAI, creature->AI())
                        ->SetScript9(
                            e, e.action.timedActionList.id, GetLastInvoker());
            }
            else if (GameObject* go = target->ToGameObject()) {
                if (IsSmartGO(go))
                    CAST_AI(SmartGameObjectAI, go->AI())
                        ->SetScript9(
                            e, e.action.timedActionList.id, GetLastInvoker());
            }
        }
    ...
    case SMART_ACTION_SET_NPC_FLAG: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToUnit()->SetUInt32Value(UNIT_NPC_FLAGS,
                                                 e.action.unitFlag.flag);
        break;
    }
    case SMART_ACTION_ADD_NPC_FLAG: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToUnit()->SetFlag(UNIT_NPC_FLAGS,
                                          e.action.unitFlag.flag);
        break;
    }
    case SMART_ACTION_REMOVE_NPC_FLAG: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToUnit()->RemoveFlag(UNIT_NPC_FLAGS,
                                             e.action.unitFlag.flag);
        break;
    }
    case SMART_ACTION_CROSS_CAST: {
        ...
        GetTargets(
            casters,
            CreateSmartEvent(SMART_EVENT_UPDATE_IC,
                             0,
                             0,
                             0,
                             0,
                             ...
                             0,
                             0,
                             (SMARTAI_TARGETS)e.action.crossCast.targetType,
                             e.action.crossCast.targetParam1,
                             e.action.crossCast.targetParam2,
                             e.action.crossCast.targetParam3,
                             0,
                             0),
            unit);
        ...
        for (WorldObject* caster : casters) {
            ...
            for (WorldObject* target : targets) {
                ...
                if (!(e.action.crossCast.flags & SMARTCAST_AURA_NOT_PRESENT) ||
                    !target->ToUnit()->HasAura(e.action.crossCast.spell)) {
                    if (!interruptedSpell && e.action.crossCast.flags &
                                                 SMARTCAST_INTERRUPT_PREVIOUS) {
                        casterUnit->InterruptNonMeleeSpells(false);
                        interruptedSpell = true;
                    }

                    casterUnit->CastSpell(
                        target->ToUnit(),
                        e.action.crossCast.spell,
                        (e.action.crossCast.flags & SMARTCAST_TRIGGERED) != 0);
                }
                else
                    LOG_DEBUG("scripts.ai",
                              "Spell {} not cast because it has flag "
    ...
    case SMART_ACTION_CALL_RANDOM_TIMED_ACTIONLIST: {
        ...
        std::copy_if(e.action.randTimedActionList.actionLists.begin(),
                     e.action.randTimedActionList.actionLists.end(),
                     std::back_inserter(actionLists),
                     [](uint32 actionList) { return actionList != 0; });

        uint32 id =
            Acore::Containers::SelectRandomContainerElement(actionLists);
        if (e.GetTargetType() == SMART_TARGET_NONE) {
            LOG_ERROR("sql.sql",
                      "SmartScript: Entry {} SourceType {} Event {} Action {} "
                      "is using TARGET_NONE(0) for Script9 target. Please "
    ...
    case SMART_ACTION_CALL_RANDOM_RANGE_TIMED_ACTIONLIST: {
        uint32 id = urand(e.action.randTimedActionList.actionLists[0],
                          e.action.randTimedActionList.actionLists[1]);
        if (e.GetTargetType() == SMART_TARGET_NONE) {
            LOG_ERROR("sql.sql",
                      "SmartScript: Entry {} SourceType {} Event {} Action {} "
                      "is using TARGET_NONE(0) for Script9 target. Please "
        ...
        for (WorldObject* target : targets) {
            if (IsCreature((target))) {
                ...
                if (e.action.moveRandom.distance)
                    target->ToCreature()->GetMotionMaster()->MoveRandom(
                        float(e.action.moveRandom.distance));
                else
    ...
    case SMART_ACTION_SET_UNIT_FIELD_BYTES_1: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->SetByteFlag(UNIT_FIELD_BYTES_1,
                                              e.action.setunitByte.type,
    ...
    case SMART_ACTION_REMOVE_UNIT_FIELD_BYTES_1: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->RemoveByteFlag(UNIT_FIELD_BYTES_1,
                                                 e.action.delunitByte.type,
                                                 e.action.delunitByte.byte1);
        break;
    }
        ...
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->InterruptNonMeleeSpells(
                    e.action.interruptSpellCasting.withDelayed != 0,
                    e.action.interruptSpellCasting.spell_id,
    ...
    case SMART_ACTION_SEND_GO_CUSTOM_ANIM: {
        for (WorldObject* target : targets)
            if (IsGameObject(target))
                target->ToGameObject()->SendCustomAnim(
                    e.action.sendGoCustomAnim.anim);
        break;
    }
    case SMART_ACTION_SET_DYNAMIC_FLAG: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->SetUInt32Value(UNIT_DYNAMIC_FLAGS,
                                                 e.action.unitFlag.flag);
        break;
    }
    case SMART_ACTION_ADD_DYNAMIC_FLAG: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->SetFlag(UNIT_DYNAMIC_FLAGS,
                                          e.action.unitFlag.flag);
        break;
    }
    case SMART_ACTION_REMOVE_DYNAMIC_FLAG: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->RemoveFlag(UNIT_DYNAMIC_FLAGS,
                                             e.action.unitFlag.flag);
        break;
    }
        ...
        if (e.GetTargetType() == SMART_TARGET_RANDOM_POINT) {
            if (me) {
                ...
                Position srcPos = {
                    e.target.x, e.target.y, e.target.z, e.target.o};
                Position randomPoint = me->GetRandomPoint(srcPos, range);
                me->GetMotionMaster()->MoveJump(randomPoint,
                                                (float)e.action.jump.speedxy,
                                                (float)e.action.jump.speedz);
            }
            ...
            if (WorldObject* target =
                ...
                if (me)
                    me->GetMotionMaster()->MoveJump(
                        target->GetPositionX() + e.target.x,
                        target->GetPositionY() + e.target.y,
                        target->GetPositionZ() + e.target.z,
                        (float)e.action.jump.speedxy,
                        (float)e.action.jump.speedz);
        }
        else {
            for (WorldObject* target : targets)
                if (WorldObject* obj = (target)) {
                    if (Creature* creature = obj->ToCreature())
                        creature->GetMotionMaster()->MoveJump(
                            e.target.x,
                            e.target.y,
                            e.target.z,
                            (float)e.action.jump.speedxy,
    ...
    case SMART_ACTION_GO_SET_LOOT_STATE: {
        for (WorldObject* target : targets)
            if (IsGameObject(target))
                target->ToGameObject()->SetLootState(
    ...
    case SMART_ACTION_GO_SET_GO_STATE: {
        for (WorldObject* target : targets)
            if (IsGameObject(target))
                target->ToGameObject()->SetGoState(
    ...
    case SMART_ACTION_SEND_TARGET_TO_TARGET: {
        ...
        ObjectVector const* storedTargets =
            GetStoredTargetVector(e.action.sendTargetToTarget.id, *ref);
        if (!storedTargets)
            break;
        ...
        for (WorldObject* target : targets) {
            if (IsCreature(target)) {
                if (SmartAI* ai = CAST_AI(SmartAI, target->ToCreature()->AI()))
                    ai->GetScript()->StoreTargetList(
                        ObjectVector(*storedTargets),
                        e.action.sendTargetToTarget
                            .id); // store a copy of target list
                else
                    LOG_ERROR("sql.sql",
                              "SmartScript: Action target for "
                              "SMART_ACTION_SEND_TARGET_TO_TARGET is not using "
                              "SmartAI, skipping");
            }
            else if (IsGameObject(target)) {
                if (SmartGameObjectAI* ai = CAST_AI(
            ...
            for (WorldObject* target : targets)
                if (IsCreature(target)) {
                    if (e.action.setHomePos.spawnPos) {
                        ...
                        target->ToCreature()->SetHomePosition(
                            target->GetPositionX(),
                            target->GetPositionY(),
                            target->GetPositionZ(),
    ...
    case SMART_ACTION_SET_HEALTH_REGEN: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToCreature()->SetRegeneratingHealth(
                    e.action.setHealthRegen.regenHealth);
    ...
    case SMART_ACTION_SET_ROOT: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToCreature()->SetControlled(e.action.setRoot.root != 0,
                                                    UNIT_STATE_ROOT);
        break;
    }
    case SMART_ACTION_SET_GO_FLAG: {
        for (WorldObject* target : targets)
            if (IsGameObject(target))
                target->ToGameObject()->SetUInt32Value(GAMEOBJECT_FLAGS,
                                                       e.action.goFlag.flag);
        break;
    }
    case SMART_ACTION_ADD_GO_FLAG: {
        for (WorldObject* target : targets)
            if (IsGameObject(target))
                target->ToGameObject()->SetFlag(GAMEOBJECT_FLAGS,
                                                e.action.goFlag.flag);
        break;
    }
    case SMART_ACTION_REMOVE_GO_FLAG: {
        for (WorldObject* target : targets)
            if (IsGameObject(target))
                target->ToGameObject()->RemoveFlag(GAMEOBJECT_FLAGS,
                                                   e.action.goFlag.flag);
        break;
    }
    case SMART_ACTION_SUMMON_CREATURE_GROUP: {
        std::list<TempSummon*> summonList;
        GetBaseObject()->SummonCreatureGroup(e.action.creatureGroup.group,
                                             &summonList);

        for (std::list<TempSummon*>::const_iterator itr = summonList.begin();
             itr != summonList.end();
             ++itr) {
    ...
    case SMART_ACTION_SET_POWER: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->SetPower(Powers(e.action.power.powerType),
                                           e.action.power.newPower);
        break;
    }
    ...
    case SMART_ACTION_GAME_EVENT_STOP: {
        ...
        if (!sGameEventMgr->IsActiveEvent(eventId)) {
            LOG_ERROR("scripts.ai.sai",
                      "SmartScript::ProcessAction: At case "
                      "SMART_ACTION_GAME_EVENT_STOP, inactive event (id: {})",
                      eventId);
            break;
        }
    ...
    case SMART_ACTION_GAME_EVENT_START: {
        ...
        if (sGameEventMgr->IsActiveEvent(eventId)) {
            LOG_ERROR("scripts.ai.sai",
                      "SmartScript::ProcessAction: At case "
                      "SMART_ACTION_GAME_EVENT_START, already activated event "
                      "(id: {})",
                      eventId);
            break;
        }
    ...
    case SMART_ACTION_START_CLOSEST_WAYPOINT: {
        ...
        std::copy_if(e.action.closestWaypointFromList.wps.begin(),
                     e.action.closestWaypointFromList.wps.end(),
        ...
        for (WorldObject* target : targets) {
            if (Creature* creature = target->ToCreature()) {
                if (IsSmart(creature)) {
                    for (uint32 wp : waypoints) {
                        ...
                        if (itrWp != path->end()) {
                            if (WayPoint* wp = itrWp->second) {
                                float distToThisPath =
                                    creature->GetDistance(wp->x, wp->y, wp->z);
                    ...
                    if (closestWp)
                        CAST_AI(SmartAI, creature->AI())
                            ->StartPath(false, closestWp->id, true);
                }
            }
        }
        break;
    ...
    case SMART_ACTION_SET_UNIT_MOVEMENT_FLAGS: {
        for (WorldObject* target : targets)
            if (IsUnit(target)) {
                target->ToUnit()->SetUnitMovementFlags(
                    e.action.movementFlag.flag);
    ...
    case SMART_ACTION_SET_COMBAT_DISTANCE: {
        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToCreature()->m_CombatDistance =
                    e.action.combatDistance.dist;
    ...
    case SMART_ACTION_SET_SIGHT_DIST: {
        for (WorldObject* const target : targets)
            if (IsCreature(target))
                target->ToCreature()->m_SightDistance =
                    e.action.sightDistance.dist;
        break;
    }
    case SMART_ACTION_FLEE: {
        for (WorldObject* const target : targets)
            if (IsCreature(target))
                target->ToCreature()->GetMotionMaster()->MoveFleeing(
    ...
    case SMART_ACTION_ADD_THREAT: {
        for (WorldObject* const target : targets)
            if (IsUnit(target))
                me->AddThreat(target->ToUnit(),
                              float(e.action.threatPCT.threatINC) -
                                  float(e.action.threatPCT.threatDEC));
        break;
    }
    case SMART_ACTION_LOAD_EQUIPMENT: {
        for (WorldObject* const target : targets)
            if (IsCreature(target))
                target->ToCreature()->LoadEquipment(
                    e.action.loadEquipment.id,
                    e.action.loadEquipment.force != 0);
        break;
    }
    case SMART_ACTION_TRIGGER_RANDOM_TIMED_EVENT: {
        uint32 eventId = urand(e.action.randomTimedEvent.minId,
                               e.action.randomTimedEvent.maxId);
        ProcessEventsFor(
            (SMART_EVENT)SMART_EVENT_TIMED_EVENT_TRIGGERED, nullptr, eventId);
        break;
    }
    ...
    case SMART_ACTION_ADD_IMMUNITY: {
        for (WorldObject* target : targets)
            if (IsUnit(target))
                target->ToUnit()->ApplySpellImmune(e.action.immunity.id,
                                                   e.action.immunity.type,
                                                   e.action.immunity.value,
                                                   true);
    ...
    case SMART_ACTION_PLAYER_TALK: {
        char const* text = sObjectMgr->GetAcoreString(
            e.action.playerTalk.textId, DEFAULT_LOCALE);

        if (!targets.empty())
            for (WorldObject* target : targets)
                if (IsPlayer(target))
                    !e.action.playerTalk.flag
                        ? target->ToPlayer()->Say(text, LANG_UNIVERSAL)
                        : target->ToPlayer()->Yell(text, LANG_UNIVERSAL);
            ...
            if (IsUnit(target)) {
                ...
                if (e.action.castCustom.flags & SMARTCAST_COMBAT_MOVE) {
                    ...
                    SpellInfo const* spellInfo  = sSpellMgr->GetSpellInfo(
                        e.action.castCustom.spell); // AssertSpellInfo?
                    int32 mana = me->GetPower(POWER_MANA);

                    if (me->GetDistance(target->ToUnit()) >
                            spellInfo->GetMaxRange(true) ||
                        me->GetDistance(target->ToUnit()) <
                            spellInfo->GetMinRange(true) ||
                        !me->IsWithinLOSInMap(target->ToUnit()) ||
                        mana < spellInfo->CalcPowerCost(
                                   me, spellInfo->GetSchoolMask()))
                        _allowMove = true;

                    CAST_AI(SmartAI, me->AI())->SetCombatMove(_allowMove);
                }

                if (!(e.action.castCustom.flags & SMARTCAST_AURA_NOT_PRESENT) ||
                    !target->ToUnit()->HasAura(e.action.castCustom.spell)) {
                    CustomSpellValues values;
                    if (e.action.castCustom.bp1)
                        values.AddSpellMod(SPELLVALUE_BASE_POINT0,
                                           e.action.castCustom.bp1);
                    if (e.action.castCustom.bp2)
                        values.AddSpellMod(SPELLVALUE_BASE_POINT1,
                                           e.action.castCustom.bp2);
                    if (e.action.castCustom.bp3)
                        values.AddSpellMod(SPELLVALUE_BASE_POINT2,
                                           e.action.castCustom.bp3);
                    me->CastCustomSpell(
                        e.action.castCustom.spell,
                        values,
                        target->ToUnit(),
                        (e.action.castCustom.flags & SMARTCAST_TRIGGERED)
                            ? TRIGGERED_FULL_MASK
                            : TRIGGERED_NONE);
                }
            }
    ...
    case SMART_ACTION_VORTEX_SUMMON: {
        ...
        TempSummonType summon_type = (e.action.summonVortex.summonDuration > 0)
        ...
        float delta_phi =
            M_PI * static_cast<float>(e.action.summonVortex.phi_delta) / 180.0f;
        ...
        for (WorldObject* target : targets) {
            ...
            do {
                ...
                me->SummonCreature(e.action.summonVortex.summonEntry,
                                   summonPosition,
                                   summon_type,
                                   e.action.summonVortex.summonDuration);
    ...
    case SMART_ACTION_CONE_SUMMON: {
        ...
        TempSummonType spawnType = (e.action.coneSummon.summonDuration > 0)
                                       ? TEMPSUMMON_TIMED_DESPAWN
                                       : TEMPSUMMON_CORPSE_DESPAWN;

        float distInARow =
            static_cast<float>(e.action.coneSummon.distanceBetweenSummons);
        float coneAngle =
            static_cast<float>(e.action.coneSummon.coneAngle) * M_PI / 180.0f;

        for (uint32 radius = 0; radius <= e.action.coneSummon.coneLength;
             radius += e.action.coneSummon.distanceBetweenRings) {
            ...
            if (e.GetTargetType() == SMART_TARGET_SELF ||
                e.GetTargetType() == SMART_TARGET_NONE)
                currentAngle += G3D::fuzzyGt(e.target.o, 0.0f)
                                    ? (e.target.o - me->GetOrientation())
                                    : 0.0f;
            else if (!targets.empty()) {
                currentAngle +=
            ...
            for (uint32 index = 0; index < count; ++index) {
                ...
                me->SummonCreature(e.action.coneSummon.summonEntry,
                                   spawnPosition,
                                   spawnType,
                                   e.action.coneSummon.summonDuration);
            }
        }
    ...
    case SMART_ACTION_CU_ENCOUNTER_START: {
        for (WorldObject* target : targets) {
            if (Player* playerTarget = target->ToPlayer()) {
                ...
                playerTarget->RemoveAurasDueToSpell(
                    57724); // Spell Shaman Debuff - Sated (Heroism)
                playerTarget->RemoveAurasDueToSpell(
                    57723); // Spell Shaman Debuff - Exhaustion (Bloodlust)
    ...
    case SMART_ACTION_DO_ACTION: {
        int32 const actionId = e.action.doAction.isNegative
                                   ? -e.action.doAction.actionId
                                   : e.action.doAction.actionId;
        if (!e.action.doAction.instanceTarget) {
            ...
            for (WorldObject* objTarget : targets) {
                ...
                else if (GameObject const* gobjTarget =
                             objTarget->ToGameObject()) {
    ...
    case SMART_ACTION_DISABLE_EVADE: {
        ...
        CAST_AI(SmartAI, me->AI())
            ->SetEvadeDisabled(e.action.disableEvade.disable != 0);
        break;
    }
    case SMART_ACTION_SET_CORPSE_DELAY: {
        for (WorldObject* const target : targets) {
            if (IsCreature(target))
                target->ToCreature()->SetCorpseDelay(
                    e.action.corpseDelay.timer);
        }
        break;
    }
    case SMART_ACTION_SET_HEALTH_PCT: {
        for (WorldObject* target : targets)
            if (Unit* targetUnit = target->ToUnit())
                targetUnit->SetHealth(targetUnit->CountPctFromMaxHealth(
    ...
    case SMART_ACTION_SET_MOVEMENT_SPEED: {
        ...
        float  speed =
            ...
            float(speedFraction) /
                std::pow(10,
                         std::floor(std::log10(float(
                                        speedFraction ? speedFraction : 1)) +
                                    1));

        for (WorldObject* target : targets)
            if (IsCreature(target))
                target->ToCreature()->SetSpeed(
    ...
    case SMART_ACTION_SUMMON_RADIAL: {
        ...
        TempSummonType spawnType = (e.action.radialSummon.summonDuration > 0)
                                       ? TEMPSUMMON_TIMED_DESPAWN
                                       : TEMPSUMMON_CORPSE_DESPAWN;

        float startAngle =
            me->GetOrientation() +
            (static_cast<float>(e.action.radialSummon.startAngle) * M_PI /
             180.0f);
        float stepAngle =
            static_cast<float>(e.action.radialSummon.stepAngle) * M_PI / 180.0f;

        if (e.action.radialSummon.dist) {
            for (uint32 itr = 0; itr < e.action.radialSummon.repetitions;
                 itr++) {
                Position summonPos = me->GetPosition();
                summonPos.RelocatePolarOffset(
                    itr * stepAngle,
                    static_cast<float>(e.action.radialSummon.dist));
                me->SummonCreature(e.action.radialSummon.summonEntry,
                                   summonPos,
                                   spawnType,
                                   e.action.radialSummon.summonDuration);
            }
            break;
        }
        ...
        for (WorldObject* target : targets) {
            if (IsUnit(target)) {
                if (e.action.spellVisual.visualId)
                    target->ToUnit()->SendPlaySpellVisual(
                        e.action.spellVisual.visualId);
            }
        }
        break;
    }
    case SMART_ACTION_FOLLOW_GROUP: {
        ...
        case FOLLOW_TYPE_CIRCLE: {
            float angle =
                (membCount > 4 ? (M_PI * 2) / membCount
                               : (M_PI / 2)); // 90 degrees is the maximum angle
            for (WorldObject* target : targets) {
                if (IsCreature(target)) {
                    target->ToCreature()->GetMotionMaster()->MoveFollow(
                        me, dist, angle * itr);
                    itr++;
                }
            }
            ...
            for (WorldObject* target : targets) {
                if (IsCreature(target)) {
                    target->ToCreature()->GetMotionMaster()->MoveFollow(
                        me,
                        dist,
                        (M_PI / 2.0f) + (M_PI / membCount) * (itr - 1));
                    itr++;
                }
            }
            break;
        ...
        case FOLLOW_TYPE_LINE: {
            for (WorldObject* target : targets) {
                if (IsCreature(target)) {
                    target->ToCreature()->GetMotionMaster()->MoveFollow(
                        me, dist * (((itr - 1) / 2) + 1), itr % 2 ? 0.f : M_PI);
                    itr++;
                }
            }
            break;
    ...
    case SMART_ACTION_SET_ORIENTATION_TARGET: {
        ...
        {
            for (WorldObject* target : targets)
                target->ToCreature()->SetFacingTo(
                    (target->ToCreature()->HasUnitMovementFlag(
                         MOVEMENTFLAG_ONTRANSPORT) &&
                             target->ToCreature()->GetTransGUID()
        ...
        case 3: // Target parameters
        {
            ...
            GetTargets(
                ...
                CreateSmartEvent(
                    ...
                    0,
                    0,
                    (SMARTAI_TARGETS)e.action.orientationTarget.targetType,
                    e.action.orientationTarget.targetParam1,
                    e.action.orientationTarget.targetParam2,
                    e.action.orientationTarget.targetParam3,
                    e.action.orientationTarget.targetParam4,
                    0),
                unit);
    ...
    default:
        LOG_ERROR("sql.sql",
                  "SmartScript::ProcessAction: Entry {} SourceType {}, Event "
    ...
    if (e.link && e.link != e.event_id) {
        ...
        if (linked.GetActionType() && linked.GetEventType() == SMART_EVENT_LINK)
            ...
            LOG_ERROR("sql.sql",
                      "SmartScript::ProcessAction: Entry {} SourceType {}, "
                      "Event {}, Link Event {} not found or invalid, skipped.",
                      e.entryOrGuid,
                      e.GetScriptType(),
                      e.event_id,
                      e.link);
                                     ...
                                     GameObject*        gob)
{
    ...
    ConditionList const conds = sConditionMgr->GetConditionsForSmartEvent(
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    if (mTemplate != SMARTAI_TEMPLATE_BASIC) {
        LOG_ERROR("sql.sql",
                  "SmartScript::InstallTemplate: Entry {} SourceType {} AI "
                  "Template can not be set more then once, skipped.",
                  e.entryOrGuid,
                  e.GetScriptType());
        return;
    }
    ...
    case SMARTAI_TEMPLATE_CASTER: {
        AddEvent(SMART_EVENT_UPDATE_IC,
                 0,
                 0,
                 0,
                 e.action.installTtemplate.param2,
                 e.action.installTtemplate.param3,
                 0,
                 0,
                 SMART_ACTION_CAST,
                 e.action.installTtemplate.param1,
                 e.target.raw.param1,
                 0,
                 0,
                 0,
                 ...
                 0,
                 1);
        AddEvent(SMART_EVENT_RANGE,
                 0,
                 e.action.installTtemplate.param4,
                 300,
                 0,
                 0,
                 0,
        ...
        AddEvent(SMART_EVENT_RANGE,
                 0,
                 0,
                 e.action.installTtemplate.param4 > 10
                     ? e.action.installTtemplate.param4 - 10
                     : 0,
                 0,
                 0,
                 ...
                 0,
                 1);
        AddEvent(SMART_EVENT_MANA_PCT,
                 0,
                 e.action.installTtemplate.param5 - 15 > 100
                     ? 100
                     : e.action.installTtemplate.param5 + 15,
                 100,
                 1000,
                 1000,
                 0,
        ...
        AddEvent(SMART_EVENT_MANA_PCT,
                 0,
                 0,
                 e.action.installTtemplate.param5,
                 1000,
                 1000,
                 0,
                 0,
    ...
    case SMARTAI_TEMPLATE_TURRET: {
        AddEvent(SMART_EVENT_UPDATE_IC,
                 ...
                 0,
                 0);
        AddEvent(SMART_EVENT_JUST_CREATED,
                 0,
                 0,
                 0,
                 0,
    ...
    case SMARTAI_TEMPLATE_CAGED_NPC_PART: {
        ...
        AddEvent(SMART_EVENT_MOVEMENTINFORM,
                 0,
                 POINT_MOTION_TYPE,
                 SMART_RANDOM_POINT,
                 0,
        ...
        if (sCreatureTextMgr->TextExist(
                me->GetEntry(), (uint8)e.action.installTtemplate.param5))
            AddEvent(SMART_EVENT_MOVEMENTINFORM,
                     0,
                     POINT_MOTION_TYPE,
                     SMART_RANDOM_POINT,
    ...
    case SMARTAI_TEMPLATE_CAGED_GO_PART: {
        ...
        AddEvent(SMART_EVENT_GO_STATE_CHANGED,
                 0,
                 2,
                 0,
                 0,
...
void SmartScript::AddEvent(SMART_EVENT     e,
                           ...
                           uint32          phaseMask)
{
    mInstallEvents.push_back(CreateSmartEvent(e,
                                              event_flags,
                                              event_param1,
                                              event_param2,
                                              event_param3,
                             ...
                             Unit* invoker /*= nullptr*/) const
{
        ...
        if (me) {
            if (e.target.hostileRandom.powerType) {
                if (Unit* u = me->AI()->SelectTarget(
                        ...
                        PowerUsersSelector(
                            me,
                            Powers(e.target.hostileRandom.powerType - 1),
                            (float)e.target.hostileRandom.maxDist,
                            e.target.hostileRandom.playerOnly)))
                    targets.push_back(u);
            }
            else if (Unit* u = me->AI()->SelectTarget(
                         SelectTargetMethod::MaxThreat,
                         1,
                         (float)e.target.hostileRandom.maxDist,
                         e.target.hostileRandom.playerOnly,
                         true,
                         -e.target.hostileRandom.aura))
                targets.push_back(u);
        }
        break;
    ...
    case SMART_TARGET_FARTHEST:
        if (me) {
            if (Unit* u = me->AI()->SelectTarget(
                    ...
                    0,
                    FarthestTargetSelector(me,
                                           e.target.farthest.maxDist,
                                           e.target.farthest.playerOnly,
                                           e.target.farthest.isInLos,
                                           e.target.farthest.minDist)))
                targets.push_back(u);
        }
        break;
    ...
    case SMART_TARGET_ACTION_INVOKER_VEHICLE:
        if (scriptTrigger && scriptTrigger->GetVehicle() &&
            scriptTrigger->GetVehicle()->GetBase())
    ...
    case SMART_TARGET_CREATURE_RANGE: {
        ...
        if (!ref) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_CREATURE_RANGE: Entry {} SourceType {} Event {} "
                "Action {} Target {} is missing base object or invoker",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
                e.GetActionType(),
        ...
        ObjectVector units;
        GetWorldObjectsInDist(units,
                              static_cast<float>(e.target.unitRange.maxDist));
        ...
        for (WorldObject* unit : units) {
            ...
            if (((e.target.unitRange.creature &&
                  unit->ToCreature()->GetEntry() ==
    ...
    case SMART_TARGET_CREATURE_DISTANCE: {
        ObjectVector units;
        GetWorldObjectsInDist(units,
                              static_cast<float>(e.target.unitDistance.dist));
        ...
        for (WorldObject* unit : units) {
            ...
            if ((e.target.unitDistance.creature &&
                 unit->ToCreature()->GetEntry() ==
                     e.target.unitDistance.creature) ||
                !e.target.unitDistance.creature)
                targets.push_back(unit);
        }
    ...
    case SMART_TARGET_GAMEOBJECT_DISTANCE: {
        ObjectVector units;
        GetWorldObjectsInDist(units,
                              static_cast<float>(e.target.goDistance.dist));
        ...
        for (WorldObject* unit : units) {
            ...
            if ((e.target.goDistance.entry &&
                 unit->ToGameObject()->GetEntry() ==
                     e.target.goDistance.entry) ||
                !e.target.goDistance.entry)
                targets.push_back(unit);
        }
    ...
    case SMART_TARGET_GAMEOBJECT_RANGE: {
        ...
        if (!ref) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_GAMEOBJECT_RANGE: Entry: {} SourceType {} Event "
                "{} Action {} Target {} is missing base object or invoker.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
        ...
        ObjectVector units;
        GetWorldObjectsInDist(units,
                              static_cast<float>(e.target.goRange.maxDist));
        ...
        for (WorldObject* unit : units) {
            ...
            if (((e.target.goRange.entry && IsGameObject(unit) &&
                  unit->ToGameObject()->GetEntry() == e.target.goRange.entry) ||
                 !e.target.goRange.entry) &&
                ref->IsInRange((unit),
    ...
    case SMART_TARGET_CREATURE_GUID: {
        if (!scriptTrigger && !baseObject) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_CREATURE_GUID: Entry {} SourceType {} Event {} "
                "Action {} Target {} is missing base object or invoker.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
        ...
        Creature* target =
            FindCreatureNear(scriptTrigger ? scriptTrigger : GetBaseObject(),
                             e.target.unitGUID.dbGuid);
        if (target && (!e.target.unitGUID.entry ||
                       target->GetEntry() == e.target.unitGUID.entry))
    ...
    case SMART_TARGET_GAMEOBJECT_GUID: {
        if (!scriptTrigger && !GetBaseObject()) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_GAMEOBJECT_GUID: Entry {} SourceType {} Event {} "
                "Action {} Target {} is missing base object or invoker.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
        ...
        GameObject* target =
            FindGameObjectNear(scriptTrigger ? scriptTrigger : GetBaseObject(),
                               e.target.goGUID.dbGuid);
        if (target && (!e.target.goGUID.entry ||
                       target->GetEntry() == e.target.goGUID.entry))
            targets.push_back(target);
        break;
    }
    case SMART_TARGET_PLAYER_RANGE: {
        ObjectVector units;
        GetWorldObjectsInDist(units,
                              static_cast<float>(e.target.playerRange.maxDist));

        if (!units.empty() && baseObject)
            for (WorldObject* unit : units)
                if (IsPlayer(unit) &&
                    baseObject->IsInRange(unit,
                                          float(e.target.playerRange.minDist),
                                          float(e.target.playerRange.maxDist)))
                    targets.push_back(unit);
        if (e.target.playerRange.maxCount)
            Acore::Containers::RandomResize(targets,
                                            e.target.playerRange.maxCount);
        break;
    }
    case SMART_TARGET_PLAYER_DISTANCE: {
        ObjectVector units;
        GetWorldObjectsInDist(units,
                              static_cast<float>(e.target.playerDistance.dist));
    ...
    case SMART_TARGET_STORED: {
        ...
        if (!ref) {
            LOG_ERROR("scripts.ai.sai",
                      "SMART_TARGET_STORED: Entry {} SourceType {} Event {} "
                      "Action {} Target {} is missing base object or invoker.",
                      e.entryOrGuid,
                      e.GetScriptType(),
                      e.event_id,
        ...
        if (ObjectVector const* stored =
                GetStoredTargetVector(e.target.stored.id, *ref))
    ...
    case SMART_TARGET_CLOSEST_CREATURE: {
        ...
        if (!ref) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_CLOSEST_CREATURE: Entry {} SourceType {} Event "
                "{} Action {} Target {} is missing base object or invoker.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
        ...
        Creature* target = GetClosestCreatureWithEntry(
            ref,
            e.target.unitClosest.entry,
            (float)(e.target.unitClosest.dist ? e.target.unitClosest.dist
                                              : 100),
    ...
    case SMART_TARGET_CLOSEST_GAMEOBJECT: {
        ...
        if (!ref) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_CLOSEST_GAMEOBJECT: Entry {} SourceType {} Event "
                "{} Action {} Target {} is missing base object or invoker.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
        ...
        GameObject* target = GetClosestGameObjectWithEntry(
            ref,
            e.target.goClosest.entry,
            (float)(e.target.goClosest.dist ? e.target.goClosest.dist : 100),
            e.target.goClosest.onlySpawned);
        if (target)
            targets.push_back(target);
        break;
    }
    case SMART_TARGET_CLOSEST_PLAYER: {
        ...
        if (!ref) {
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_CLOSEST_PLAYER: Entry {} SourceType {} Event {} "
                "Action {} Target {} is missing base object or invoker.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
        ...
        if (Player* target =
                ref->SelectNearestPlayer((float)e.target.playerDistance.dist))
            targets.push_back(target);
        break;
    }
    case SMART_TARGET_OWNER_OR_SUMMONER:
        /*
         * Owners/Summoners should be WorldObjects. This allows to have other
         * objects such as gameobjects to execute SmartScripts using this type
         * of target. Otherwise, only Units like creatures can summon other
         * creatures.
         */
        {
            if (me) {
                if (WorldObject* owner = ObjectAccessor::GetWorldObject(
                        *me, me->GetCharmerOrOwnerGUID())) {
                    targets.push_back(owner);
                }
                else if (me->IsSummon() &&
                         me->ToTempSummon()->GetSummonerUnit()) {
            ...
            if (e.target.owner.useCharmerOrOwner && !targets.empty()) {
                if (WorldObject* owner = targets.front()) {
                    ...
                    if (owner->ToCreature()) {
                        if (Unit* base = ObjectAccessor::GetUnit(
                                *owner,
                                owner->ToCreature()->GetCharmerOrOwnerGUID())) {
                        ...
                        if (Unit* base = ObjectAccessor::GetUnit(
                                *owner,
                                owner->ToGameObject()->GetOwnerGUID())) {
                            targets.push_back(base);
                        }
                    }
                }
        ...
        if (me) {
            ...
            for (ThreatContainer::StorageType::const_iterator i =
                ...
                if (Unit* temp =
                    ...
                    if (e.target.threatList.maxDist == 0 ||
                        me->IsWithinCombatRange(
                            temp, (float)e.target.threatList.maxDist))
    ...
    case SMART_TARGET_CLOSEST_ENEMY: {
        if (me)
            if (Unit* target = me->SelectNearestTarget(
                    e.target.closestAttackable.maxDist,
                    e.target.closestAttackable.playerOnly))
                targets.push_back(target);
    ...
    case SMART_TARGET_CLOSEST_FRIENDLY: {
        if (me)
            if (Unit* target = DoFindClosestFriendlyInRange(
                    e.target.closestFriendly.maxDist,
                    e.target.closestFriendly.playerOnly))
                targets.push_back(target);
    ...
    case SMART_TARGET_PLAYER_WITH_AURA: {
        ...
        for (WorldObject* unit : units)
            if (IsPlayer(unit) && unit->ToPlayer()->IsAlive() &&
                !unit->ToPlayer()->IsGameMaster())
                if (GetBaseObject()->IsInRange(
                        unit,
                        (float)e.target.playerWithAura.distMin,
                        (float)e.target.playerWithAura.distMax))
                    if (bool(e.target.playerWithAura.negation) !=
                        unit->ToPlayer()->HasAura(
                            e.target.playerWithAura.spellId))
                        targets.push_back(unit);
    ...
    case SMART_TARGET_ROLE_SELECTION: {
            ...
            if (Player* targetPlayer = unit->ToPlayer())
                if (targetPlayer->IsAlive() && !targetPlayer->IsGameMaster()) {
                    ...
                    if (roleMask & SMART_TARGET_ROLE_FLAG_DAMAGERS) {
                        if (targetPlayer->HasCasterSpec() ||
                            targetPlayer->HasMeleeSpec()) {
                            targets.push_back(unit);
                            continue;
                        }
                    }
        ...
        if (e.target.roleSelection.resize > 0)
            Acore::Containers::RandomResize(targets,
                                            e.target.roleSelection.resize);

        break;
    }
    case SMART_TARGET_VEHICLE_PASSENGER: {
        if (me && me->IsVehicle()) {
            if (Unit* target = me->GetVehicleKit()->GetPassenger(
                    e.target.vehicle.seatMask)) {
                targets.push_back(target);
            }
        }
        break;
    ...
    case SMART_TARGET_LOOT_RECIPIENTS: {
        if (me) {
            if (Group* lootGroup = me->GetLootRecipientGroup()) {
                for (GroupReference* it = lootGroup->GetFirstMember();
                     it != nullptr;
                     it = it->next()) {
    ...
    case SMART_TARGET_SUMMONED_CREATURES: {
        if (me) {
            for (ObjectGuid const& guid : _summonList) {
                if (!e.target.summonedCreatures.entry ||
                    guid.GetEntry() == e.target.summonedCreatures.entry) {
        ...
        if (InstanceScript* instance = GetBaseObject()->GetInstanceScript()) {
            if (e.target.instanceStorage.type == 1) {
                if (Creature* creature =
                        instance->GetCreature(e.target.instanceStorage.index)) {
                    targets.push_back(creature);
                }
            }
            else if (e.target.instanceStorage.type == 2) {
                if (GameObject* go = instance->GetGameObject(
                        e.target.instanceStorage.index)) {
            ...
            LOG_ERROR(
                "scripts.ai.sai",
                "SMART_TARGET_INSTANCE_STORAGE: Entry {} SourceType {} Event "
                "{} Action {} Target {} called outside an instance map.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
                e.GetActionType(),
                               ...
                               GameObject*        gob)
{
    ...
    case SMART_EVENT_TARGET_HEALTH_PCT: {
        ...
        if (perc > e.event.minMaxRepeat.max || perc < e.event.minMaxRepeat.min)
            return;
        ProcessTimedAction(e,
                           e.event.minMaxRepeat.repeatMin,
    ...
    case SMART_EVENT_RANGE: {
        ...
        if (me->IsInRange(me->GetVictim(),
                          (float)e.event.minMaxRepeat.rangeMin,
                          (float)e.event.minMaxRepeat.rangeMax))
            ProcessTimedAction(e,
                               e.event.minMaxRepeat.repeatMin,
                               e.event.minMaxRepeat.repeatMax,
                               me->GetVictim());
    ...
    case SMART_EVENT_VICTIM_CASTING: {
        ...
        if (e.event.targetCasting.spellId > 0)
            if (Spell* currSpell =
                    victim->GetCurrentSpell(CURRENT_GENERIC_SPELL))
                if (currSpell->m_spellInfo->Id != e.event.targetCasting.spellId)
                    return;

        ProcessTimedAction(e,
                           e.event.targetCasting.repeatMin,
                           e.event.targetCasting.repeatMax,
                           me->GetVictim());
        break;
    }
    case SMART_EVENT_FRIENDLY_HEALTH: {
        ...
        Unit* target =
            DoSelectLowestHpFriendly((float)e.event.friendlyHealth.radius,
                                     e.event.friendlyHealth.hpDeficit);
        if (!target || !target->IsInCombat()) {
            // Xinef: if there are at least two same npcs, they will perform the
            // same action immediately even if this is useless...
            RecalcTimer(e, 1000, 3000);
            return;
        }
        ProcessTimedAction(e,
                           e.event.friendlyHealth.repeatMin,
    ...
    case SMART_EVENT_FRIENDLY_IS_CC: {
        ...
        ProcessTimedAction(
            e,
            e.event.friendlyCC.repeatMin,
            e.event.friendlyCC.repeatMax,
            Acore::Containers::SelectRandomContainerElement(creatures));
        break;
    }
    case SMART_EVENT_FRIENDLY_MISSING_BUFF: {
        ...
        std::vector<Creature*> creatures;
        DoFindFriendlyMissingBuff(creatures,
                                  float(e.event.missingBuff.radius),
                                  e.event.missingBuff.spell);
    ...
    case SMART_EVENT_TARGET_BUFFED: {
        ...
        if (count < e.event.aura.count)
            return;
        ProcessTimedAction(
            e, e.event.aura.repeatMin, e.event.aura.repeatMax, me->GetVictim());
        break;
    }
    ...
    case SMART_EVENT_IS_BEHIND_TARGET: {
        ...
        if (Unit* victim = me->GetVictim()) {
            if (e.event.minMaxRepeat.rangeMax &&
                (me->IsInRange(victim,
                               (float)e.event.minMaxRepeat.rangeMin,
                               (float)e.event.minMaxRepeat.rangeMax)))
    ...
    case SMART_EVENT_OOC_LOS: {
        ...
        if (me->IsWithinDistInMap(unit, range) && me->IsWithinLOSInMap(unit)) {
            SmartEvent::LOSHostilityMode hostilityMode =
                static_cast<SmartEvent::LOSHostilityMode>(
                    e.event.los.hostilityMode);
            // if friendly event&&who is not hostile OR hostile event&&who is
            // hostile
            if ((hostilityMode == SmartEvent::LOSHostilityMode::Any) ||
                (hostilityMode == SmartEvent::LOSHostilityMode::NotHostile &&
                 !me->IsHostileTo(unit)) ||
                (hostilityMode == SmartEvent::LOSHostilityMode::Hostile &&
                 me->IsHostileTo(unit))) {
    ...
    case SMART_EVENT_INSTANCE_PLAYER_ENTER: {
        if (e.event.instancePlayerEnter.team &&
            ...
            return;
        RecalcTimer(e,
                    e.event.instancePlayerEnter.cooldownMin,
    ...
    case SMART_EVENT_GOSSIP_SELECT: {
        LOG_DEBUG("sql.sql",
                  "SmartScript: Gossip Select:  menu {} action {}",
                  var0,
                  var1); // little help for scripters
    ...
    case SMART_EVENT_FRIENDLY_HEALTH_PCT: {
        ...
        case SMART_TARGET_PLAYER_DISTANCE: {
            ...
            GetTargets(targets, e);
            for (WorldObject* target : targets) {
                if (IsUnit(target) && me->IsFriendlyTo(target->ToUnit()) &&
                    target->ToUnit()->IsAlive() &&
                    target->ToUnit()->IsInCombat()) {
        ...
        case SMART_TARGET_SELF:
        case SMART_TARGET_ACTION_INVOKER:
            unitTarget = DoSelectLowestHpPercentFriendly(
                (float)e.event.friendlyHealthPct.radius,
                0,
                e.event.friendlyHealthPct.hpPct);
            break;
        default:
            return;
        }
        ...
        ProcessTimedAction(e,
                           e.event.friendlyHealthPct.repeatMin,
                           e.event.friendlyHealthPct.repeatMax,
                           unitTarget);
        break;
    }
    case SMART_EVENT_DISTANCE_CREATURE: {
        ...
        if (e.event.distance.guid != 0) {
            ...
            me->GetCreatureListWithEntryInGrid(
                list, e.event.distance.entry, (float)e.event.distance.dist);
        ...
        if (creature)
            ProcessTimedAction(
                e, e.event.distance.repeat, e.event.distance.repeat);
    ...
    case SMART_EVENT_COUNTER_SET:
        if (e.event.counter.id != var0 ||
            GetCounterValue(e.event.counter.id) != e.event.counter.value)
            return;

        ProcessTimedAction(
            e, e.event.counter.cooldownMin, e.event.counter.cooldownMax);
        break;
    case SMART_EVENT_NEAR_PLAYERS: {
        uint32       playerCount = 0;
        ObjectVector targets;
        GetWorldObjectsInDist(targets,
                              static_cast<float>(e.event.nearPlayer.radius));
    ...
    case SMART_EVENT_NEAR_PLAYERS_NEGATION: {
        ...
        ObjectVector targets;
        GetWorldObjectsInDist(
            targets, static_cast<float>(e.event.nearPlayerNegation.radius));
        ...
        RecalcTimer(e,
                    e.event.nearPlayerNegation.repeatMin,
                    e.event.nearPlayerNegation.repeatMax);
        break;
    }
    case SMART_EVENT_NEAR_UNIT: {
        ...
        ObjectVector targets;
        GetWorldObjectsInDist(targets,
                              static_cast<float>(e.event.nearUnit.range));

        if (!targets.empty()) {
            if (e.event.nearUnit.type) {
                for (WorldObject* target : targets) {
                    if (IsGameObject(target) &&
                        target->GetEntry() == e.event.nearUnit.entry)
                        unitCount++;
                }
            }
            else {
    ...
    case SMART_EVENT_NEAR_UNIT_NEGATION: {
        ...
        ObjectVector targets;
        GetWorldObjectsInDist(
            targets, static_cast<float>(e.event.nearUnitNegation.range));

        if (!targets.empty()) {
            if (e.event.nearUnitNegation.type) {
                for (WorldObject* target : targets) {
                    if (IsGameObject(target) &&
                        target->GetEntry() == e.event.nearUnitNegation.entry)
                        unitCount++;
                }
            }
            else {
        ...
        RecalcTimer(
            e, e.event.nearUnitNegation.timer, e.event.nearUnitNegation.timer);
        break;
    }
        ...
        for (ThreatContainer::StorageType::const_iterator i =
            ...
            if (Unit* target =
                ...
                if (!target || !IsPlayer(target) ||
                    !target->IsNonMeleeSpellCast(false, false, true))
                    continue;

                if (!(me->IsInRange(target,
                                    (float)e.event.minMaxRepeat.rangeMin,
                                    (float)e.event.minMaxRepeat.rangeMax)))
                    continue;
            ...
            // If no targets are found and it's off cooldown, check again in
            // 1200ms
    ...
    default:
        LOG_ERROR("sql.sql",
                  "SmartScript::ProcessEvent: Unhandled Event type {}",
                  e.GetEventType());
        break;
    }
}
...
void SmartScript::InitTimer(SmartScriptHolder& e)
{
    ...
    case SMART_EVENT_NEAR_PLAYERS_NEGATION:
        RecalcTimer(
            e, e.event.nearPlayer.firstTimer, e.event.nearPlayer.firstTimer);
        break;
    case SMART_EVENT_UPDATE:
    case SMART_EVENT_UPDATE_IC:
    case SMART_EVENT_UPDATE_OOC:
...
void SmartScript::UpdateTimer(SmartScriptHolder& e, uint32 const diff)
{
    ...
    if (e.GetEventType() == SMART_EVENT_UPDATE_OOC &&
        (me && me->IsEngaged())) // can be used with me=nullptr (go script)
        return;
    ...
    if (e.timer < diff) {
        ...
        if (e.GetActionType() == SMART_ACTION_FLEE_FOR_ASSIST && me &&
            me->HasUnitState(UNIT_STATE_CASTING)) {
            e.timer = 1200;
            return;
        } // @TODO: Can't these be handled by the action themselves instead?
          // Less expensive
        ...
        case SMART_EVENT_DISTANCE_GAMEOBJECT: {
            ...
            if (e.GetScriptType() == SMART_SCRIPT_TYPE_TIMED_ACTIONLIST) {
                ...
                for (SmartAIEventList::iterator i = mTimedActionList.begin();
                     i != mTimedActionList.end();
                     ++i) {
        ...
        if (e.priority != SmartScriptHolder::DEFAULT_PRIORITY) {
            // Reset priority to default one only if the event hasn't been
            // rescheduled again to next loop
            if (e.timer > 1) {
...
void SmartScript::InstallEvents()
{
    if (!mInstallEvents.empty()) {
        for (SmartAIEventList::iterator i = mInstallEvents.begin();
             i != mInstallEvents.end();
             ++i)
...
{
    e.timer = 1200;
    // Change priority only if it's set to default, otherwise keep the current
    // order of events
    if (e.priority == SmartScriptHolder::DEFAULT_PRIORITY) {
...
void SmartScript::RetryLater(SmartScriptHolder& e, bool ignoreChanceRoll)
{
    ...
    // This allows to retry the action later without rolling again the chance
    // roll (which might fail and end up not executing the action)
    if (ignoreChanceRoll)
        e.event.event_flags |= SMART_EVENT_FLAG_TEMP_IGNORE_CHANCE_ROLL;
...
{
    (void)at; // ensure that the variable is referenced even if extra logs are
              // disabled in order to pass compiler checks

    if (e.empty()) {
        if (obj)
            LOG_DEBUG("sql.sql",
                      "SmartScript: EventMap for Entry {} is empty but is "
                      "using SmartScript.",
                      obj->GetEntry());

        if (at)
            LOG_DEBUG("sql.sql",
                      "SmartScript: EventMap for AreaTrigger {} is empty but "
                      "is using SmartScript.",
                      at->entry);
        return;
    }
    for (SmartAIEventList::iterator i = e.begin(); i != e.end(); ++i) {
        ...
        if ((*i).event.event_flags &
            SMART_EVENT_FLAG_DIFFICULTY_ALL) // if has instance flag add only if
                                             // in it
        {
        ...
        mEvents.push_back((*i)); // NOTE: 'world(0)' events still get processed
    ...
    if (me) {
        ...
        if (CreatureTemplate const* cInfo = me->GetCreatureTemplate()) {
            if (cInfo->HasFlagsExtra(CREATURE_FLAG_DONT_OVERRIDE_ENTRY_SAI)) {
                e = sSmartScriptMgr->GetScript((int32)me->GetEntry(),
                                               mScriptType);
                FillScript(e, me, nullptr);
            }
        }
...
void SmartScript::OnInitialize(WorldObject* obj, AreaTrigger const* at)
{
    if (obj) // handle object based scripts
    {
        ...
        case TYPEID_UNIT:
            ...
            me          = obj->ToCreature();
            LOG_DEBUG("sql.sql",
                      "SmartScript::OnInitialize: source is Creature {}",
                      me->GetEntry());
            break;
        case TYPEID_GAMEOBJECT:
            ...
            go          = obj->ToGameObject();
            LOG_DEBUG("sql.sql",
                      "SmartScript::OnInitialize: source is GameObject {}",
                      go->GetEntry());
            break;
        default:
            LOG_ERROR("scripts.ai.sai",
        ...
        trigger     = at;
        LOG_DEBUG("sql.sql",
                  "SmartScript::OnInitialize: source is AreaTrigger {}",
                  trigger->entry);
    }
    else {
        LOG_ERROR("scripts.ai.sai",
                  "SmartScript::OnInitialize: !WARNING! Initialized objects "
                  "are nullptr.");
        return;
    }
...
void SmartScript::OnMoveInLineOfSight(Unit* who)
{
    ...
    ProcessEventsFor(me->IsEngaged() ? SMART_EVENT_IC_LOS : SMART_EVENT_OOC_LOS,
                     who);
}
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff) const
{
    ...
    Acore::UnitLastSearcher<Acore::MostHPMissingInRange> searcher(
        me, unit, u_check);
                                                   ...
                                                   uint32 maxHpPct) const
{
    ...
    Acore::UnitLastSearcher<Acore::MostHPPercentMissingInRange> searcher(
        me, unit, u_check);
    Cell::VisitGridObjects(me, searcher, range);
    return unit;
}
                                   ...
                                   float                   range) const
{
    ...
    Acore::CreatureListSearcher<Acore::FriendlyCCedInRange> searcher(
        me, creatures, u_check);
    Cell::VisitGridObjects(me, searcher, range);
}
                                            ...
                                            uint32 spellid) const
{
    ...
    Acore::CreatureListSearcher<Acore::FriendlyMissingBuffInRange> searcher(
        me, creatures, u_check);
    Cell::VisitGridObjects(me, searcher, range);
}
                                                ...
                                                bool  playerOnly) const
{
    ...
    Unit*                                           unit = nullptr;
    Acore::AnyFriendlyNotSelfUnitInObjectRangeCheck u_check(
        me, me, range, playerOnly);
    Acore::UnitLastSearcher<Acore::AnyFriendlyNotSelfUnitInObjectRangeCheck>
        searcher(me, unit, u_check);
...
{
    // do NOT clear mTimedActionList if it's being iterated because it will
    // invalidate the iterator and delete
    //  any SmartScriptHolder contained like the "e" parameter passed to this
    //  function
    if (isProcessingTimedActionList) {
        LOG_ERROR(
            "scripts.ai.sai",
            "Entry {} SourceType {} Event {} Action {} is trying to overwrite "
            "timed action list from a timed action, this is not allowed!.",
            e.entryOrGuid,
            e.GetScriptType(),
            e.GetEventType(),
            e.GetActionType());
    ...
    // Do NOT allow to start a new actionlist if a previous one is already
    // running, unless explicitly allowed. We need to always finish the current
    // actionlist
    ...
    mTimedActionList.clear();
    mTimedActionList =
        sSmartScriptMgr->GetScript(entry, SMART_SCRIPT_TYPE_TIMED_ACTIONLIST);
    if (mTimedActionList.empty())
        return;
    for (SmartAIEventList::iterator i = mTimedActionList.begin();
         i != mTimedActionList.end();
        ...
        i->enableTimed =
            i == mTimedActionList
                     .begin(); // enable processing only for the first action
...
{
    // Xinef: Look for invoker only on map of base object... Prevents
    // multithreaded crashes
    if (GetBaseObject())
