...
{
    for (StorageType::const_iterator i = storage_.begin(); i != storage_.end();
         ++i) {
        Creature* summon = ObjectAccessor::GetCreature(*me, *i);
...
ScriptedAI::ScriptedAI(Creature* creature)
    : CreatureAI(creature), me(creature), IsFleeing(false)
{
                             ...
                             bool             triggered)
{
    ...
    me->StopMoving();
    me->CastSpell(
        target, spellInfo, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE);
}
                                    ...
                                    uint32                uniqueId)
{
    ...
    scheduler.Schedule(
        timerMin == 0s ? timerMax : timerMin,
        timerMax,
        [exec, repeatMin, repeatMax, uniqueId](TaskContext context) {
            exec();

            if (!uniqueId) {
                repeatMax > 0s ? context.Repeat(repeatMin, repeatMax)
                               : context.Repeat(repeatMin);
            }
        });
                                         ...
                                         SelectEffect     effects)
{
    ...
    for (uint32 i = 0; i < MAX_CREATURE_SPELLS; i++) {
        ...
        if (targets &&
            !(SpellSummary[me->m_spells[i]].Targets & (1 << (targets - 1))))
            continue;

        // Check the type of spell if we are looking for a specific spell type
        if (effects &&
            !(SpellSummary[me->m_spells[i]].Effects & (1 << (effects - 1))))
            continue;
        ...
        if (me->IsWithinDistInMap(
                target,
                float(me->GetSpellMinRangeForTarget(target, tempSpell))) ||
            !me->IsWithinDistInMap(
                target,
                float(me->GetSpellMaxRangeForTarget(target, tempSpell))))
            continue;
...
void ScriptedAI::DoResetThreatList()
{
    if (!me->CanHaveThreatList() || me->GetThreatMgr().isThreatListEmpty()) {
        LOG_ERROR("entities.unit.ai",
                  "DoResetThreatList called for creature that either cannot "
                  "have threat list or has empty threat list (me entry = {})",
                  me->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(
    Unit* unit, float x, float y, float z, float o)
{
    if (!unit)
        return;
    ...
    if (Player* player = unit->ToPlayer())
        ...
        LOG_ERROR("entities.unit.ai",
                  "Creature {} Tried to teleport non-player unit {} to x: {} "
                  "y:{} z: {} o: {}. Aborted.",
                  me->GetGUID().ToString(),
...
void ScriptedAI::DoTeleportAll(float x, float y, float z, float o)
{
    ...
    for (Map::PlayerList::const_iterator itr = PlayerList.begin();
        ...
        if (Player* player = itr->GetSource())
            if (player->IsAlive())
                player->TeleportTo(
                    me->GetMapId(), x, y, z, o, TELE_TO_NOT_LEAVE_COMBAT);
}
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float minimumRange)
{
    ...
    Acore::PlayerSearcher<Acore::PlayerAtMinimumRangeAway> searcher(
        me, player, check);
                                               ...
                                               bool   mustBeInLOS) const
{
    ...
    for (Map::PlayerList::const_iterator itr = pList.begin();
         itr != pList.end();
         ++itr) {
        if (!me->IsWithinDistInMap(itr->GetSource(), maxdist, true, false) ||
            !itr->GetSource()->IsAlive() || itr->GetSource()->IsGameMaster())
            continue;
...
BossAI::BossAI(Creature* creature, uint32 bossId)
    : ScriptedAI(creature), instance(creature->GetInstanceScript()),
      summons(creature), _bossId(bossId)
{
    ...
    // Clear it in the script if you need it to update while the creature is
    // casting.
...
void BossAI::_JustEngagedWith()
{
    ...
    if (callForHelpRange) {
        ScheduleTimedEvent(
            0s, [&] { me->CallForHelp(callForHelpRange); }, 2s);
    }
    if (instance) {
...
void BossAI::SummonedCreatureDespawnAll() { summons.DespawnAll(); }
                         ...
                         SpellSchoolMask /*damageSchoolMask*/)
{
    if (!_healthCheckEvents.empty()) {
        _healthCheckEvents.remove_if([&](HealthCheckEventData data) -> bool {
            return _ProccessHealthCheckEvent(
                data._healthPct, damage, data._exec);
        });
    }
}

/**
 * @brief Executes a function once the creature reaches the defined health point
 * percent.
 *
