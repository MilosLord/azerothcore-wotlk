...
enum ePoints { POINT_LAST_POINT = 0xFFFFFF, POINT_HOME = 0xFFFFFE };

npc_escortAI::npc_escortAI(Creature* creature)
    : ScriptedAI(creature), m_uiWPWaitTimer(1000), m_uiPlayerCheckTimer(0),
      m_uiEscortState(STATE_ESCORT_NONE),
      MaxPlayerDistance(DEFAULT_MAX_PLAYER_DISTANCE),
      m_pQuestForEscort(nullptr), m_bIsActiveAttacker(true),
      m_bIsRunning(false), m_bCanInstantRespawn(false),
      m_bCanReturnToStart(false), DespawnAtEnd(true), DespawnAtFar(true),
      ScriptWP(false), HasImmuneToNPCFlags(false)
{
}
...
void npc_escortAI::MoveInLineOfSight(Unit* who)
{
    ...
    if (!me->HasUnitState(UNIT_STATE_STUNNED) &&
        who->isTargetableForAttack(true, me) && who->isInAccessiblePlaceFor(me))
...
{
    if (!HasEscortState(STATE_ESCORT_ESCORTING) || !m_uiPlayerGUID ||
        !m_pQuestForEscort)
        return;
                ...
                if (Player* member = groupRef->GetSource())
                    if (member->IsInMap(player) &&
                        member->GetQuestStatus(
                            m_pQuestForEscort->GetQuestId()) ==
                            QUEST_STATUS_INCOMPLETE)
                        member->FailQuest(m_pQuestForEscort->GetQuestId());
        }
        else {
            if (player->GetQuestStatus(m_pQuestForEscort->GetQuestId()) ==
                QUEST_STATUS_INCOMPLETE)
...
void npc_escortAI::EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING)) {
        ...
        LOG_DEBUG(
            "scripts.ai",
            "EscortAI has left combat and is now returning to last point");
    }
    else {
    ...
    if (Player* player = GetPlayerForEscort()) {
        if (Group* group = player->GetGroup()) {
            for (GroupReference* groupRef = group->GetFirstMember();
                 ...
                 groupRef = groupRef->next())
                if (Player* member = groupRef->GetSource())
                    if (me->IsWithinDistInMap(
                            member, GetMaxPlayerDistance(), true, false))
                        return true;
        }
        else if (me->IsWithinDistInMap(
                     player, GetMaxPlayerDistance(), true, false))
            return true;
    }
...
{
    if (HasEscortState(STATE_ESCORT_ESCORTING) && !me->GetVictim() &&
        ...
        if (m_uiWPWaitTimer <= diff) {
            if (CurrentWP == WaypointList.end()) {
                ...
                // xinef: remove escort state, escort was finished (lack of this
                // line resulted in skipping UpdateEscortAI calls after finished
                // escort)
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_uiPlayerGUID &&
        !me->GetVictim() && !HasEscortState(STATE_ESCORT_RETURNING)) {
...
void npc_escortAI::MovementInform(uint32 moveType, uint32 pointId)
{
    ...
    if (moveType == POINT_MOTION_TYPE) {
        ...
        if (pointId == POINT_LAST_POINT) {
            LOG_DEBUG(
                "scripts.ai",
                "EscortAI has returned to original position before combat");
            ...
            LOG_DEBUG("scripts.ai",
                      "EscortAI has returned to original home location and "
                      "will continue from beginning of waypoint list.");
        ...
        if (m_uiWPWaitTimer <= 1 && !HasEscortState(STATE_ESCORT_PAUSED) &&
            ...
            if (m_uiWPWaitTimer > 1 || HasEscortState(STATE_ESCORT_PAUSED)) {
                if (me->GetMotionMaster()->GetMotionSlotType(
                        MOTION_SLOT_ACTIVE) == ESCORT_MOTION_TYPE)
                    me->GetMotionMaster()->MovementExpired();
...
void npc_escortAI::OnPossess(bool apply)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        else
        {
            Returning = true;
            me->GetMotionMaster()->MovementExpired();
            me->GetMotionMaster()->MovePoint(WP_LAST_POINT, LastPos.x,
LastPos.y, LastPos.z);
        }
    }
}
...
void npc_escortAI::AddWaypoint(
    uint32 id, float x, float y, float z, uint32 waitTime)
{
...
{
    ScriptPointVector const& movePoints =
        sScriptSystemMgr->GetPointMoveList(me->GetEntry());
    if (movePoints.empty())
        return;

    ScriptPointVector::const_iterator itrEnd = movePoints.end();
    for (ScriptPointVector::const_iterator itr = movePoints.begin();
         itr != itrEnd;
         ++itr) {
        Escort_Waypoint point(
...
void npc_escortAI::SetRun(bool on)
{
    if (on) {
        if (!m_bIsRunning)
            ...
            LOG_DEBUG(
                "scripts.ai",
                "EscortAI attempt to set run mode, but is already running.");
    }
    else {
        if (m_bIsRunning)
            me->SetWalk(true);
            ...
            LOG_DEBUG(
                "scripts.ai",
                "EscortAI attempt to set walk mode, but is already walking.");
    }
                         ...
                         bool         resetWaypoints /* = true */)
{
    if (me->GetVictim()) {
        LOG_ERROR("entities.unit.ai",
                  "ERROR: EscortAI (script: {}, creature entry: {}) attempts "
                  "to Start while in combat",
                  me->GetScriptName(),
                  me->GetEntry());
        return;
    }
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING)) {
        LOG_ERROR("entities.unit.ai",
                  "EscortAI (script: {}, creature entry: {}) attempts to Start "
                  "while already escorting",
                  me->GetScriptName(),
                  me->GetEntry());
        return;
    }
    ...
    if (WaypointList.empty()) {
        LOG_ERROR(
            "sql.sql",
            "EscortAI (script: {}, creature entry: {}) starts with 0 waypoints "
            "(possible missing entry in script_waypoint. Quest: {}).",
            me->GetScriptName(),
            me->GetEntry(),
            quest ? quest->GetQuestId() : 0);
        return;
    }
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        LOG_DEBUG(
            "scripts.ai",
            "EscortAI is set to return home after waypoint end and instant "
            "respawn at waypoint end. Creature will never despawn.");
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() ==
        ...
        LOG_DEBUG(
            "scripts.ai",
            "EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }
    ...
    LOG_DEBUG("scripts.ai",
              "EscortAI started with {} waypoints. ActiveAttacker = {}, Run = "
              "{}, PlayerGUID = {}",
              uint64(WaypointList.size()),
              m_bIsActiveAttacker,
              m_bIsRunning,
              m_uiPlayerGUID.ToString());
...
bool npc_escortAI::SetNextWaypoint(
    uint32 pointId, float x, float y, float z, float orientation)
{
...
bool npc_escortAI::SetNextWaypoint(uint32 pointId, bool setPosition)
{
    ...
    for (CurrentWP = WaypointList.begin(); CurrentWP != WaypointList.end();
         ++CurrentWP) {
        if (CurrentWP->id == pointId) {
            if (setPosition)
                me->UpdatePosition(CurrentWP->x,
                                   CurrentWP->y,
                                   CurrentWP->z,
                                   me->GetOrientation());
            return true;
...
bool npc_escortAI::GetWaypointPosition(uint32 pointId,
                                       ...
                                       float& z)
{
    ScriptPointVector const& waypoints =
        sScriptSystemMgr->GetPointMoveList(me->GetEntry());
    if (waypoints.empty())
        return false;

    for (ScriptPointVector::const_iterator itr = waypoints.begin();
         itr != waypoints.end();
         ++itr) {
...
void npc_escortAI::GenerateWaypointArray(Movement::PointsArray* points)
{
    ...
    if (me->m_movementInfo.HasMovementFlag(
        ...
        for (std::list<Escort_Waypoint>::const_iterator itr = CurrentWP;
             itr != WaypointList.end();
             ++itr)
        ...
        for (float size = 1.0f; size; size *= 0.5f) {
            ...
            uint32 cnt = 0;
            for (std::list<Escort_Waypoint>::const_iterator itr = CurrentWP;
                 itr != WaypointList.end() && cnt <= length;
                 ++itr, ++cnt)
