...
enum ePoints { POINT_COMBAT_START = 0xFFFFFF };

FollowerAI::FollowerAI(Creature* creature)
    : ScriptedAI(creature), m_uiUpdateFollowTimer(2500),
...
void FollowerAI::AttackStart(Unit* who)
{
    ...
    if (me->Attack(who, true)) {
        // This is done in Unit::Attack function which wont bug npcs by not
        // adding threat upon combat start...
...
// This part provides assistance to a player that are attacked by who, even if
// out of normal aggro range It will cause me to attack who that are attacking
// _any_ player (which has been confirmed may happen also on offi) The flag
// (type_flag) is unconfirmed, but used here for further research and is a good
// candidate.
bool FollowerAI::AssistPlayerInCombatAgainst(Unit* who)
{
    ...
    if (me->IsWithinDistInMap(who, MAX_PLAYER_DISTANCE) &&
...
{
    if (!HasFollowState(STATE_FOLLOW_INPROGRESS) || !m_uiLeaderGUID ||
        !m_pQuestForFollow)
        return;
    ...
    if (HasFollowState(STATE_FOLLOW_INPROGRESS)) {
        ...
        if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() ==
            ...
            me->GetMotionMaster()->MovePoint(
                POINT_COMBAT_START, fPosX, fPosY, fPosZ);
        }
    }
    else {
        if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() ==
    ...
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->GetVictim()) {
        if (m_uiUpdateFollowTimer <= uiDiff) {
            ...
            if (Player* player = GetLeaderForFollower()) {
                ...
                if (Group* group = player->GetGroup()) {
                    for (GroupReference* groupRef = group->GetFirstMember();
                        ...
                        if (member &&
                            me->IsWithinDistInMap(
                                member, MAX_PLAYER_DISTANCE, true, false)) {
                    ...
                    if (me->IsWithinDistInMap(
                            player, MAX_PLAYER_DISTANCE, true, false))
                        bIsMaxRangeExceeded = false;
                }
            }
            ...
            if (bIsMaxRangeExceeded) {
                LOG_DEBUG("scripts.ai",
                          "FollowerAI failed because player/group was to far "
                          "away or not found");
                me->DespawnOrUnsummon();
                return;
            }
...
void FollowerAI::MovementInform(uint32 motionType, uint32 pointId)
{
    if (motionType != POINT_MOTION_TYPE ||
        !HasFollowState(STATE_FOLLOW_INPROGRESS))
        return;
                             ...
                             const Quest* quest)
{
    ...
    LOG_DEBUG("scripts.ai",
              "FollowerAI start follow {} ({})",
              player->GetName(),
              m_uiLeaderGUID.ToString());
}
        ...
        if (player->IsAlive())
            ...
            if (Group* group = player->GetGroup()) {
                for (GroupReference* groupRef = group->GetFirstMember();
                    ...
                    if (member &&
                        me->IsWithinDistInMap(member, MAX_PLAYER_DISTANCE) &&
                        member->IsAlive()) {
                        LOG_DEBUG("scripts.ai",
                                  "FollowerAI GetLeader changed and returned "
                                  "new leader.");
...
void FollowerAI::SetFollowPaused(bool paused)
{
    ...
    if (paused) {
        ...
        if (Player* leader = GetLeaderForFollower())
            me->GetMotionMaster()->MoveFollow(
                leader, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
    }
}
