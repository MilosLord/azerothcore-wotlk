...
/**
 * @brief Causes the creature to talk/say the text assigned to their entry in
 * the `creature_text` database table.
 *
 * @param uint8 id Text ID from `creature_text`.
 * @param WorldObject target The target of the speech, in case it has elements
 * such as $n, where the target's name will be referrenced.
 * @param Milliseconds delay Delay until the creature says the text line.
 * Creatures will talk immediately by default.
 */
...
inline bool IsValidCombatTarget(Creature* source, Player* target)
{
    ...
    if (source->HasUnitState(UNIT_STATE_IN_FLIGHT) ||
                                ...
                                float     maxRangeToNearestTarget /*= 250.0f*/)
{
    ...
    if (!map->IsDungeon()) // use IsDungeon instead of Instanceable, in case
                           // battlegrounds will be instantiated
    {
        LOG_ERROR("entities.unit.ai",
                  "DoZoneInCombat call for map {} that isn't a dungeon "
                  "(creature entry = {})",
                  map->GetId(),
                  creature->GetTypeId() == TYPEID_UNIT
                      ? creature->ToCreature()->GetEntry()
                      : 0);
        return;
    }
...
// MoveInLineOfSight can be called inside another MoveInLineOfSight and cause
// stack overflow
...
void CreatureAI::MoveInLineOfSight(Unit* who)
{
    ...
    if (me->IsMoveInLineOfSightDisabled())
        if (me->GetCreatureType() ==
                CREATURE_TYPE_NON_COMBAT_PET || // nothing more to do, return
            !who->IsInCombat() || // if not in combat, nothing more to do
            !me->IsWithinDist(who,
                              ATTACK_DISTANCE,
                              true,
                              false)) // if in combat and in dist - neutral to
                                      // all can actually assist other creatures
            return;
...
void CreatureAI::TriggerAlert(Unit const* who) const
{
    ...
    if (!who || who->GetTypeId() != TYPEID_PLAYER)
        return;
    // If this unit isn't an NPC, is already distracted, is in combat, is
    // confused, stunned or fleeing, do nothing
    if (me->GetTypeId() != TYPEID_UNIT || me->IsEngaged() ||
        me->HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_STUNNED |
                         UNIT_STATE_FLEEING | UNIT_STATE_DISTRACTED))
        return;
    // Only alert for hostiles!
    if (me->IsCivilian() || me->HasReactState(REACT_PASSIVE) ||
        !me->IsHostileTo(who) || !me->_IsTargetAcceptable(who))
        return;
...
void CreatureAI::EnterEvadeMode(EvadeReason why)
{
    ...
    LOG_DEBUG(
        "entities.unit", "Creature {} enters evade mode.", me->GetEntry());
    ...
    if (!me->GetVehicle()) // otherwise me will be in evade mode forever
    {
        if (Unit* owner = me->GetCharmerOrOwner()) {
            ...
            // Required to prevent attacking creatures that are evading and
            // cause them to reenter combat Does not apply to MoveFollow
    ...
    Reset();
    if (me->IsVehicle()) // use the same sequence of addtoworld, aireset may
                         // remove all summons!
    {
        me->GetVehicleKit()->Reset(true);
    }

    // despawn bosses at reset - only verified tbc/woltk bosses with this reset
    // type
...
bool CreatureAI::_EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    // don't remove vehicle auras, passengers aren't supposed to drop off the
    // vehicle don't remove clone caster on evade (to be verified)
    me->RemoveEvadeAuras();
    ...
    if (ZoneScript* zoneScript = me->GetZoneScript()
                                     ? me->GetZoneScript()
                                     : (ZoneScript*)me->GetInstanceScript())
...
void CreatureAI::MoveCircleChecks()
{
    ...
    if (!victim || !me->IsFreeToMove() ||
        me->HasUnitMovementFlag(MOVEMENTFLAG_ROOT) ||
        !me->IsWithinMeleeRange(victim) || me == victim->GetVictim() ||
        (!victim->IsPlayer() &&
         !victim->IsPet()) // only player & pets to save CPU
    ) {
        return;
    }
