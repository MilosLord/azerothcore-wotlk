...
typedef std::unordered_map<uint32, AchievementCriteriaEntryList>
...
enum AchievementCriteriaDataType {
    ...
    ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_DEAD =
        4, // own_team       0             not corpse (not released body),
           // own_team == false if enemy team expected
    ...
    ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE =
        8, // minvalue                     value provided with achievement
           // update must be not less that limit
    ...
    ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY =
        12, // difficulty                   normal/heroic difficulty for current
            // event map
    ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT =
        13, // count                        "with less than %u people in the
            // zone"
    ...
    ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_LOSS_TEAM_SCORE =
        17, // min_score      max_score     player's team win bg and opposition
            // team have team score in range
    ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT =
        18, // 0              0             maker instance script call for check
            // current criteria requirements fit
    ACHIEVEMENT_CRITERIA_DATA_TYPE_S_EQUIPED_ITEM =
        19, // item_level     item_quality  for equipped item in slot to check
            // item level and quality
    ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_ID = 20, // map_id         0 player must
                                                // be on map with id in map_id
    ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE = 21, // class_id race_id
    ACHIEVEMENT_CRITERIA_DATA_TYPE_NTH_BIRTHDAY =
        22, // N                            login on day of N-th Birthday
    ACHIEVEMENT_CRITERIA_DATA_TYPE_S_KNOWN_TITLE =
        23, // title_id                     known (pvp) title, values from dbc
    ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_TEAMS_SCORES =
        24, // winner_score   loser score   player's team win bg and their teams
            // have exact scores
    ACHIEVEMENT_CRITERIA_DATA_TYPE_S_ITEM_QUALITY = 25 // item_quality
};
#define MAX_ACHIEVEMENT_CRITERIA_DATA_TYPE                                     \
    26 // maximum value in AchievementCriteriaDataType enum
...
struct AchievementCriteriaData {
    ...
    AchievementCriteriaData(uint32 _dataType,
                            uint32 _value1,
                            uint32 _value2,
                            uint32 _scriptId)
...
struct AchievementCriteriaDataSet {
    ...
    bool
    Meets(Player const* source, Unit const* target, uint32 miscvalue = 0) const;
...
typedef std::unordered_map<uint32, CompletedAchievementData>
...
public:
    ...
    void        LoadFromDB(PreparedQueryResult achievementResult,
    ...
    void RemoveTimedAchievement(
        ...
        uint32 entry); // used for quest and scripted timed achievements

    void RemoveCriteriaProgress(AchievementCriteriaEntry const* entry);
    CriteriaProgress*
    GetCriteriaProgress(AchievementCriteriaEntry const* entry);
...
private:
    ...
    void SendCriteriaUpdate(AchievementCriteriaEntry const* entry,
                            CriteriaProgress const*         progress,
                            uint32                          timeElapsed,
                            bool timedCompleted) const;
    void SetCriteriaProgress(AchievementCriteriaEntry const* entry,
                             uint32                          changeValue,
    ...
    bool CanUpdateCriteria(AchievementCriteriaEntry const* criteria,
                           AchievementEntry const*         achievement);
...
public:
    ...
    [[nodiscard]] AchievementCriteriaEntryList const*
    GetAchievementCriteriaByType(AchievementCriteriaTypes type) const
    {
        return &_achievementCriteriasByType[type];
    }

    AchievementCriteriaEntryList const*
    GetSpecialAchievementCriteriaByType(AchievementCriteriaTypes type,
                                        uint32                   val)
    {
    ...
    AchievementCriteriaEntryList const*
    GetAchievementCriteriaByCondition(AchievementCriteriaCondition condition,
                                      uint32                       val)
    {
        if (_achievementCriteriasByCondition[condition].find(val) !=
            _achievementCriteriasByCondition[condition].end())
            return &_achievementCriteriasByCondition[condition][val];
        return nullptr;
    }

    [[nodiscard]] AchievementCriteriaEntryList const&
    GetTimedAchievementCriteriaByType(AchievementCriteriaTimedTypes type) const
    {
        return _achievementCriteriasByTimedType[type];
    }

    [[nodiscard]] AchievementCriteriaEntryList const*
    GetAchievementCriteriaByAchievement(uint32 id) const
    {
        AchievementCriteriaListByAchievement::const_iterator itr =
            _achievementCriteriaListByAchievement.find(id);
        return itr != _achievementCriteriaListByAchievement.end() ? &itr->second
                                                                  : nullptr;
    }

    [[nodiscard]] AchievementEntryList const*
    GetAchievementByReferencedId(uint32 id) const
    {
        AchievementListByReferencedId::const_iterator itr =
            _achievementListByReferencedId.find(id);
        return itr != _achievementListByReferencedId.end() ? &itr->second
                                                           : nullptr;
    }

    AchievementReward const*
    GetAchievementReward(AchievementEntry const* achievement) const
    {
        AchievementRewards::const_iterator iter =
            _achievementRewards.find(achievement->ID);
        return iter != _achievementRewards.end() ? &iter->second : nullptr;
    }

    AchievementRewardLocale const*
    GetAchievementRewardLocale(AchievementEntry const* achievement) const
    {
        AchievementRewardLocales::const_iterator iter =
            _achievementRewardLocales.find(achievement->ID);
        return iter != _achievementRewardLocales.end() ? &iter->second
                                                       : nullptr;
    }

    AchievementCriteriaDataSet const* GetCriteriaDataSet(
        AchievementCriteriaEntry const* achievementCriteria) const
    {
        AchievementCriteriaDataMap::const_iterator iter =
            _criteriaDataMap.find(achievementCriteria->ID);
    ...
    [[nodiscard]] AchievementEntry const*
    GetAchievement(uint32 achievementId) const;
...
private:
    ...
    AchievementCriteriaEntryList
        _achievementCriteriasByType[ACHIEVEMENT_CRITERIA_TYPE_TOTAL];
    AchievementCriteriaEntryList
        _achievementCriteriasByTimedType[ACHIEVEMENT_TIMED_TYPE_MAX];
    ...
    typedef std::unordered_map<uint32 /*achievementId*/,
    ...
    // pussywizard:
    std::map<uint32, AchievementCriteriaEntryList>
        _specialList[ACHIEVEMENT_CRITERIA_TYPE_TOTAL];
    std::map<uint32, AchievementCriteriaEntryList>
        _achievementCriteriasByCondition[ACHIEVEMENT_CRITERIA_CONDITION_TOTAL];
};
