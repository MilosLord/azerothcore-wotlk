...
bool AchievementCriteriaData::IsValid(AchievementCriteriaEntry const* criteria)
{
    if (dataType >= MAX_ACHIEVEMENT_CRITERIA_DATA_TYPE) {
        LOG_ERROR("sql.sql",
                  "Table `achievement_criteria_data` for criteria (Entry: {}) "
                  "has wrong data type ({}), ignored.",
                  criteria->ID,
                  dataType);
        return false;
    }
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST: // only Children's Week
                                                         // achievements
    case ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM:             // only Children's Week
                                                         // achievements
    ...
    default:
        if (dataType != ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` has data for non-supported "
                "criteria type (Entry: {} Type: {}), ignored.",
                criteria->ID,
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_CREATURE:
        if (!creature.id || !sObjectMgr->GetCreatureTemplate(creature.id)) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_CREATURE ({}) has "
                "non-existing creature id in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
                creature.id);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE:
        if (classRace.class_id &&
            ((1 << (classRace.class_id - 1)) & CLASSMASK_ALL_PLAYABLE) == 0) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE "
                "({}) has non-existing class in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
                classRace.class_id);
        ...
        if (classRace.race_id &&
            ...
            LOG_ERROR(
                ...
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE "
                "({}) has non-existing race in value2 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
                classRace.race_id);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_LESS_HEALTH:
        if (health.percent < 1 || health.percent > 100) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_PLAYER_LESS_HEALTH "
                "({}) has wrong percent value in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
                health.percent);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_DEAD:
        if (player_dead.own_team_flag > 1) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_DEAD ({}) "
                "has wrong boolean value1 ({}).",
                criteria->ID,
                criteria->requiredType,
                dataType,
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA: {
        ...
        if (!spellInfo) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type {} ({}) has wrong spell id in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
        ...
        if (aura.effect_idx >= 3) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type {} ({}) has wrong spell effect index in "
                      "value2 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      (dataType == ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA
        ...
        if (!spellInfo->Effects[aura.effect_idx].ApplyAuraName) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type {} ({}) has non-aura spell effect (ID: {} "
                      "Effect: {}), ignores.",
                      criteria->ID,
                      criteria->requiredType,
                      (dataType == ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AREA:
        if (!sAreaTableStore.LookupEntry(area.id)) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AREA "
                      "({}) has wrong area id in value1 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      dataType,
                      area.id);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE:
        if (value.compType >= COMP_TYPE_MAX) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE ({}) "
                      "has wrong ComparisionType in value2 ({}), ignored.",
                      value.compType,
                      criteria->requiredType,
                      dataType,
                      value.value);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL:
        if (level.minlevel > STRONG_MAX_LEVEL) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL "
                      "({}) has wrong minlevel in value1 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      dataType,
                      level.minlevel);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_GENDER:
        if (gender.gender > GENDER_NONE) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_GENDER "
                      "({}) has wrong gender in value1 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      dataType,
                      gender.gender);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT:
        if (!ScriptId) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT "
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY:
        if (difficulty.difficulty >= MAX_DIFFICULTY) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY ({}) "
                "has wrong difficulty in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT:
        if (map_players.maxcount <= 0) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT "
                "({}) has wrong max players count in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_TEAM:
        if (team.team != ALLIANCE && team.team != HORDE) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_TEAM "
                      "({}) has unknown team in value1 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      dataType,
                      team.team);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_DRUNK:
        if (drunk.state >= MAX_DRUNKEN) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_DRUNK "
                      "({}) has unknown drunken state in value1 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      dataType,
                      drunk.state);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_HOLIDAY:
        if (!sHolidaysStore.LookupEntry(holiday.id)) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` (Entry: {} Type: {}) "
                      "for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_HOLIDAY "
                      "({}) has unknown holiday in value1 ({}), ignored.",
                      criteria->ID,
                      criteria->requiredType,
                      dataType,
                      holiday.id);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_ITEM_QUALITY:
        if (equipped_item.item_quality >= MAX_ITEM_QUALITY) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_requirement` (Entry: {} Type: {}) "
                "for requirement ACHIEVEMENT_CRITERIA_REQUIRE_S_EQUIPED_ITEM "
                "({}) has unknown quality state in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_ID:
        if (!sMapStore.LookupEntry(map_id.mapId)) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_requirement` (Entry: {} Type: {}) "
                "for requirement ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_ID ({}) "
                "has unknown map id in value1 ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType,
                map_id.mapId);
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE:
        if (!classRace.class_id && !classRace.race_id) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE "
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_KNOWN_TITLE: {
        if (!sCharTitlesStore.LookupEntry(known_title.title_id)) {
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_requirement` (Entry: {} Type: {}) "
                "for requirement ACHIEVEMENT_CRITERIA_DATA_TYPE_S_KNOWN_TITLE "
                "({}) have unknown title_id in value1 ({}), ignore.",
                criteria->ID,
                criteria->requiredType,
                dataType,
                known_title.title_id);
            return false;
        }
        return true;
    }
    default:
        LOG_ERROR("sql.sql",
                  "Table `achievement_criteria_data` (Entry: {} Type: {}) has "
                  "data for non-supported data type ({}), ignored.",
                  criteria->ID,
                  criteria->requiredType,
                  dataType);
        return false;
                                    ...
                                    uint32        miscvalue1 /*= 0*/) const
{
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE:
        if (!source || source->GetTypeId() != TYPEID_PLAYER)
            return false;
        if (classRace.class_id &&
            classRace.class_id != source->ToPlayer()->getClass())
            return false;
        if (classRace.race_id &&
            classRace.race_id != source->ToPlayer()->getRace())
            return false;
        return true;
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_LESS_HEALTH:
        if (!target || target->GetTypeId() != TYPEID_PLAYER)
        ...
        if (target && !target->IsAlive())
            if (Player const* player = target->ToPlayer())
                if (player->GetDeathTimer() != 0)
                    ...
                    return (player->GetTeamId() == source->GetTeamId()) ==
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE:
        return CompareValues(
            ComparisionType(value.compType), miscvalue1, value.value);
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL:
        if (!target)
            return false;
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT:
        return sScriptMgr->OnCriteriaCheck(ScriptId,
                                           const_cast<Player*>(source),
                                           const_cast<Unit*>(target),
                                           criteria_id);
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY: {
        if (source->GetMap()->IsRaid())
            if (source->GetMap()->Is25ManRaid() !=
                ((difficulty.difficulty & RAID_DIFFICULTY_MASK_25MAN) != 0))
                return false;
        ...
        // Dungeons completed on heroic mode count towards both in general
        // achievement, but not in statistics.
        return sAchievementMgr->IsStatisticCriteria(criteria)
                   ? spawnMode == difficulty.difficulty
                   : spawnMode >= difficulty.difficulty;
    }
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT:
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_LOSS_TEAM_SCORE: {
        ...
        uint32 score = bg->GetTeamScore(
            source->GetTeamId() == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);
        return score >= bg_loss_team_score.min_score &&
               score <= bg_loss_team_score.max_score;
    }
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT: {
        if (!source->IsInWorld())
            return false;
        ...
        if (!map->IsDungeon()) {
            LOG_ERROR("sql.sql",
                      "Achievement system call "
                      "ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT ({}) for "
                      "achievement criteria {} for non-dungeon/non-raid map {}",
        ...
        if (!instance) {
            LOG_ERROR("sql.sql",
                      "Achievement system call "
                      "ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT ({}) for "
                      "achievement criteria {} for map {} but map does not "
                      "have a instance script",
                      ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT,
                      criteria_id,
                      map->GetId());
            return false;
        }
        return instance->CheckAchievementCriteriaMeet(
            criteria_id, source, target, miscvalue1);
    }
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_EQUIPED_ITEM: {
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_NTH_BIRTHDAY: {
        tm birthday_tm = Acore::Time::TimeBreakdown(
            sWorld->getIntConfig(CONFIG_BIRTHDAY_TIME));
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_KNOWN_TITLE: {
        if (CharTitlesEntry const* titleInfo =
                sCharTitlesStore.LookupEntry(known_title.title_id))
    ...
    case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_TEAMS_SCORES: {
        ...
        return source->GetTeamId() == winnerTeam &&
               winnnerScore == teams_scores.winner_score &&
               loserScore == teams_scores.loser_score;
    }
    default:
        break;
    }
...
{
    for (Storage::const_iterator itr = _storage.begin(); itr != _storage.end();
         ++itr)
...
AchievementMgr::AchievementMgr(Player* player) { _player = player; }
...
void AchievementMgr::Reset()
{
    for (CompletedAchievementMap::const_iterator iter =
             _completedAchievements.begin();
         iter != _completedAchievements.end();
         ++iter) {
    ...
    for (CriteriaProgressMap::const_iterator iter = _criteriaProgress.begin();
         iter != _criteriaProgress.end();
         ++iter) {
...
void AchievementMgr::ResetAchievementCriteria(
    ...
    bool                         evenIfCriteriaComplete)
{
    ...
    LOG_DEBUG("achievement",
              "AchievementMgr::ResetAchievementCriteria({}, {}, {})",
              condition,
              value,
              evenIfCriteriaComplete);

    AchievementCriteriaEntryList const* achievementCriteriaList =
        sAchievementMgr->GetAchievementCriteriaByCondition(condition, value);
    if (!achievementCriteriaList)
        return;

    for (AchievementCriteriaEntryList::const_iterator i =
             achievementCriteriaList->begin();
         i != achievementCriteriaList->end();
         ++i) {
        ...
        // don't update already completed criteria if not forced or achievement
        // already complete
...
void AchievementMgr::DeleteFromDB(ObjectGuid::LowType lowguid)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT);
...
void AchievementMgr::SaveToDB(CharacterDatabaseTransaction trans)
{
    if (!_completedAchievements.empty()) {
        for (CompletedAchievementMap::iterator iter =
            ...
            sScriptMgr->OnAchievementSave(
                trans, GetPlayer(), iter->first, iter->second);
        }
    }

    if (!_criteriaProgress.empty()) {
        for (CriteriaProgressMap::iterator iter = _criteriaProgress.begin();
             iter != _criteriaProgress.end();
             ++iter) {
            if (!iter->second.changed)
                continue;
            ...
            // pussywizard: insert only for (counter != 0) is very important!
            // this is how criteria of completed achievements gets deleted from
            // db (by setting counter to 0); if conflicted during merge -
            // contact me
                                ...
                                PreparedQueryResult criteriaResult)
{
    if (achievementResult) {
        do {
            ...
            // must not happen: cleanup at server startup in
            ...
            CompletedAchievementData& ca =
                _completedAchievements[achievementid];
            ca.date    = time_t(fields[1].Get<uint32>());
            ca.changed = false;

            // title achievement rewards are retroactive
            if (AchievementReward const* reward =
                    sAchievementMgr->GetAchievementReward(achievement))
                if (uint32 titleId = reward->titleId[Player::TeamIdForRace(
                        GetPlayer()->getRace())])
    ...
    if (criteriaResult) {
        do {
            ...
            if (!criteria) {
                ...
                LOG_ERROR("achievement",
                          "Non-existing achievement criteria {} data removed "
                          "from table `character_achievement_progress`.",
                          id);
            ...
            if (criteria->timeLimit && time_t(date + criteria->timeLimit) <
...
void AchievementMgr::SendAchievementEarned(
    AchievementEntry const* achievement) const
{
    ...
    if (guild) {
        Acore::BroadcastTextBuilder _builder(GetPlayer(),
                                             CHAT_MSG_GUILD_ACHIEVEMENT,
                                             BROADCAST_TEXT_ACHIEVEMENT_EARNED,
                                             GetPlayer()->getGender(),
                                             GetPlayer(),
                                             achievement->ID);
        Acore::LocalizedPacketDo<Acore::BroadcastTextBuilder> _localizer(
            _builder);
    ...
    if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_KILL |
        ...
        if (achievement->flags & ACHIEVEMENT_FLAG_REALM_FIRST_KILL && guild) {
            WorldPacket data(SMSG_SERVER_FIRST_ACHIEVEMENT,
                             guild->GetName().size() + 1 + 8 + 4 + 4);
            data << guild->GetName();
            data << GetPlayer()->GetGUID();
            data << uint32(achievement->ID);
            data << uint32(
                0); // display name as plain string in chat (always 0 for guild)
            ...
            WorldPacket data(SMSG_SERVER_FIRST_ACHIEVEMENT,
                             GetPlayer()->GetName().size() + 1 + 8 + 4 + 4);
            ...
            data.put<uint32>(linkTypePos,
                             0); // display name as plain string in chat
            sWorld->SendGlobalMessage(&data,
                                      nullptr,
                                      teamId == TEAM_ALLIANCE ? TEAM_HORDE
                                                              : TEAM_ALLIANCE);
        }
    }
    // if player is in world he can tell his friends about new achievement
    else if (GetPlayer()->IsInWorld()) {
        CellCoord p = Acore::ComputeCellCoord(GetPlayer()->GetPositionX(),
        ...
        Acore::PlayerDistWorker<
            ...
            _worker(GetPlayer(),
                    ...
                    _localizer);
        TypeContainerVisitor<Acore::PlayerDistWorker<Acore::LocalizedPacketDo<
                                 Acore::BroadcastTextBuilder>>,
    ...
    data << uint32(0);
    GetPlayer()->SendMessageToSetInRange(
                                        ...
                                        bool timedCompleted) const
{
    ...
    if (!entry->timeLimit)
        ...
        data << uint32(timedCompleted
                           ? 0
                           : 1); // 1 is for keeping the counter at 0 in client
    ...
    if (sAchievementMgr->IsAverageCriteria(entry))
        data << uint32(
            GameTime::GetGameTime().count() -
            GetPlayer()->GetCreationTime().count()); // for average achievements
    else
        data << uint32(timeElapsed); // time elapsed in seconds
...
/**
 * called at player login. The player might have fulfilled some achievements
 * when the achievement system wasn't working yet.
 */
...
/**
 * this function will be called whenever the user might have done a criteria
 * relevant action
 */
                                               ...
                                               Unit*  unit /*= nullptr*/)
{
    ...
    if (type >= ACHIEVEMENT_CRITERIA_TYPE_TOTAL) {
        LOG_DEBUG("achievement",
                  "UpdateAchievementCriteria: Wrong criteria type {}",
                  type);
        return;
    }
    ...
    for (AchievementCriteriaEntryList::const_iterator i =
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_WON_AUCTIONS: /* FIXME: for online player
                                                        only currently */
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_ACCEPTED_SUMMONINGS:
            // AchievementMgr::UpdateAchievementCriteria might also be called on
            // login - skip in this case
            if (!miscValue1)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_GOLD_EARNED_BY_AUCTIONS: /* FIXME: for
                                                                   online player
                                                                   only
                                                                   currently */
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_USE_LFD_TO_GROUP_WITH_PLAYERS:
            ...
            if (!miscValue1)
                continue;
            SetCriteriaProgress(
                achievementCriteria, miscValue1, PROGRESS_ACCUMULATE);
            break;
        // std case: high value at miscvalue1
        case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_AUCTION_BID:
        case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_AUCTION_SOLD: /* FIXME: for
                                                                online player
                                                                only currently
                                                              */
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEALING_RECEIVED:
            ...
            if (!miscValue1)
                continue;
            SetCriteriaProgress(
                achievementCriteria, miscValue1, PROGRESS_HIGHEST);
            break;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE: {
            ...
            SetCriteriaProgress(
                achievementCriteria, miscValue2, PROGRESS_ACCUMULATE);
            break;
        }
        case ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE_TYPE: {
            // AchievementMgr::UpdateAchievementCriteria might also be called on
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_REACH_LEVEL:
            if (AchievementCriteriaDataSet const* data =
                    sAchievementMgr->GetCriteriaDataSet(achievementCriteria))
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_REACH_SKILL_LEVEL:
            ...
            if (miscValue1 &&
                miscValue1 != achievementCriteria->reach_skill_level.skillID)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LEVEL:
            // update at loading or specific skill update
            if (miscValue1 &&
                miscValue1 != achievementCriteria->learn_skill_level.skillID)
                continue;
            if (uint32 maxSkillvalue = GetPlayer()->GetPureMaxSkillValue(
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ACHIEVEMENT:
            if ((miscValue1 &&
                 achievementCriteria->complete_achievement.linkedAchievement ==
                     miscValue1) ||
                (!miscValue1 && GetPlayer()->HasAchieved(
                                    achievementCriteria->complete_achievement
                                        .linkedAchievement)))
                SetCriteriaProgress(achievementCriteria, 1);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUEST_COUNT: {
            SetCriteriaProgress(achievementCriteria,
                                GetPlayer()->GetRewardedQuestCount());
            break;
        }
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST_DAILY: {
            Seconds nextDailyResetTime = sWorld->GetNextDailyQuestsResetTime();
            CriteriaProgress* progress =
                GetCriteriaProgress(achievementCriteria);

            if (!miscValue1) // Login case.
            {
                // reset if player missed one day.
                if (progress &&
                    Seconds(progress->date) < (nextDailyResetTime - 2_days))
            ...
            if (!progress)
                ...
                // last progress is older than 2 days. Player missed 1 day =>
                // Retart count.
                progressType = PROGRESS_RESET;
            else if (Seconds(progress->date) < (nextDailyResetTime - 1_days))
                // last progress is between 1 and 2 days. => 1st time of the
                // day.
                progressType = PROGRESS_ACCUMULATE;
            else
                // last progress is within the day before the reset => Already
                // counted today.
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUESTS_IN_ZONE: {
            ...
            const RewardedQuestSet& rewQuests =
                GetPlayer()->getRewardedQuests();
            for (RewardedQuestSet::const_iterator itr = rewQuests.begin();
                 itr != rewQuests.end();
                 ++itr) {
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_DEATH: {
            ...
            if (!miscValue1)
                continue;
            // skip wrong arena achievements, if not achievIdByArenaSlot then
            // normal total death counter
            bool notfit = false;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_DEATH_IN_DUNGEON: {
            ...
            if (map->ToInstanceMap()->GetMaxPlayers() !=
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_RAID: {
            ...
            if (map->ToInstanceMap()->GetMaxPlayers() !=
                achievementCriteria->complete_raid.groupSize)
                continue;
            SetCriteriaProgress(achievementCriteria, 1, PROGRESS_ACCUMULATE);
            break;
        }
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_PLAYER:
            ...
            if (achievement->ID == 318 &&
                miscValue2 == GetPlayer()->GetTeamId())
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2: {
            if (!miscValue1 ||
                miscValue1 != achievementCriteria->be_spell_target.spellID)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA:
            ...
            if (achievementCriteria->additionalRequirements[0]
                ...
                AchievementCriteriaDataSet const* data =
                    sAchievementMgr->GetCriteriaDataSet(achievementCriteria);
                if (!data || !data->Meets(GetPlayer(), unit, miscValue1)) {
                    // reset the progress as we have a win without the
                    // requirement.
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM:
            ...
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_LOOT_ITEM:
            // You _have_ to loot that item, just owning it when logging in does
            // _not_ count!
            if (!miscValue1)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_EXPLORE_AREA: {
            ...
            for (int j = 0; j < MAX_WORLD_MAP_OVERLAY_AREA_IDX; ++j) {
                AreaTableEntry const* area = sAreaTableStore.LookupEntry(
                    worldOverlayEntry->areatableID[j]);
                if (!area)
                    break;
                ...
                if (GetPlayer()->GetUInt32Value(PLAYER_EXPLORED_ZONES_1 +
                                                playerIndexOffset) &
                    mask) {
                    matchFound = true;
                    break;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION: {
            ...
            if (miscValue1 &&
                miscValue1 != achievementCriteria->gain_reputation.factionID)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED_ON_LOOT: {
            ...
            ItemTemplate const* pProto =
                sObjectMgr->GetItemTemplate(miscValue1);
            if (!pProto)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_HEALING_DONE: {
            ...
            if (achievementCriteria->additionalRequirements[0]
                ...
                if (GetPlayer()->GetMapId() !=
                    achievementCriteria->additionalRequirements[0]
                        .additionalRequirement_value)
                    continue;

                // map specific case (BG in fact) expected player targeted
                // damage/heal
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS: {
            ...
            for (PlayerSpellMap::const_iterator spellIter =
                ...
                SkillLineAbilityMapBounds bounds =
                    sSpellMgr->GetSkillLineAbilityMapBounds(spellIter->first);
                for (SkillLineAbilityMap::const_iterator skillIter =
                         bounds.first;
                     skillIter != bounds.second;
                     ++skillIter) {
                    if (skillIter->second->SkillLine ==
                        achievementCriteria->learn_skillline_spell.skillLine) {
                        // xinef: do not add couter twice if by any chance skill
                        // is listed twice in dbc (eg. skill 777 and spell
                        // 22717)
                        ++spellCount;
                        break;
                    }
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LINE: {
            if (miscValue1 &&
                miscValue1 != achievementCriteria->learn_skill_line.skillLine)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_GOLD_VALUE_OWNED:
            SetCriteriaProgress(
                achievementCriteria, GetPlayer()->GetMoney(), PROGRESS_HIGHEST);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_EARN_ACHIEVEMENT_POINTS: {
            if (!miscValue1) {
                ...
                for (CompletedAchievementMap::iterator itr =
                         _completedAchievements.begin();
                     itr != _completedAchievements.end();
                     ++itr)
                    if (AchievementEntry const* pAchievement =
                            sAchievementStore.LookupEntry(itr->first))
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE: {
            if (!miscValue1 ||
                miscValue1 != achievementCriteria->bg_objective.objectiveId)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_TEAM_RATING: {
            ...
            else // login case
            {
                for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT;
                     ++arena_slot) {
                    uint32 arenaTeamId =
                        GetPlayer()->GetArenaTeamId(arena_slot);
                    if (!arenaTeamId)
                        continue;

                    ArenaTeam* team =
                        sArenaTeamMgr->GetArenaTeamById(arenaTeamId);
                    if (!team || team->GetType() != reqTeamType)
                        continue;

                    SetCriteriaProgress(achievementCriteria,
                                        team->GetStats().Rating,
                                        PROGRESS_HIGHEST);
                    break;
                }
            }
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_PERSONAL_RATING: {
            uint32 reqTeamType =
                achievementCriteria->highest_personal_rating.teamtype;
            ...
            else // login case
            {
                for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT;
                    ...
                    if (ArenaTeamMember const* member =
                            team->GetMember(GetPlayer()->GetGUID())) {
                        SetCriteriaProgress(achievementCriteria,
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_ON_LOGIN: {
            // This criteria is only called directly after login - with expected
            // miscvalue1 == 1
            if (!miscValue1)
                continue;
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA: // This also behaves like
                                                  // ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA
        {
            ...
            if (miscValue1 == achievementCriteria->win_arena.mapID)
                SetCriteriaProgress(
                    achievementCriteria, 1, PROGRESS_ACCUMULATE);
            break;
        }
        case ACHIEVEMENT_CRITERIA_TYPE_OWN_RANK: {
            // those requirements couldn't be found in the dbc
        ...
        // as they don't depend on the completed criteria but on the sum of the
        // progress of each individual criteria
        if (achievement->flags & ACHIEVEMENT_FLAG_SUMM)
            if (IsCompletedAchievement(achievement))
                CompletedAchievement(achievement);

        if (AchievementEntryList const* achRefList =
                sAchievementMgr->GetAchievementByReferencedId(achievement->ID))
    ...
    AchievementEntry const*         achievement)
{
    ...
    // pussywizard: progress will be deleted after getting the achievement
    // (optimization) finished achievement should indicate criteria completed,
    // since not finding progress would start some timed achievements and
    ...
    if (!sScriptMgr->IsCompletedCriteria(
            this, achievementCriteria, achievement, progress))
        return false;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_PERSONAL_RATING:
        return progress->counter >=
               achievementCriteria->highest_personal_rating.PersonalRating;
    case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LEVEL:
        return progress->counter >=
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_GAIN_EXALTED_REPUTATION:
        return progress->counter >= achievementCriteria->gain_exalted_reputation
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_LOOT_MONEY:
        return progress->counter >=
               achievementCriteria->loot_money.goldInCopper;
    case ACHIEVEMENT_CRITERIA_TYPE_USE_GAMEOBJECT:
        return progress->counter >=
               achievementCriteria->use_gameobject.useCount;
    case ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL:
        return progress->counter >=
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_LOOT_TYPE:
        return progress->counter >=
               achievementCriteria->loot_type.lootTypeCount;
    case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LINE:
        return progress->counter >=
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS:
        return progress->counter >=
               achievementCriteria->get_killing_blow.killCount;
    case ACHIEVEMENT_CRITERIA_TYPE_ON_LOGIN:
        return true;
    case ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA:
        return achievementCriteria->win_arena.count &&
               progress->counter >= achievementCriteria->win_arena.count;
    case ACHIEVEMENT_CRITERIA_TYPE_OWN_RANK:
        return true;
...
bool AchievementMgr::IsCompletedAchievement(AchievementEntry const* entry)
{
    ...
    // for achievement with referenced achievement criterias get from referenced
    // and counter from self
    ...
    // For SUMM achievements, we have to count the progress of each criteria of
    // the achievement. Oddly, the target count is NOT countained in the
    // achievement, but in each individual criteria
    ...
    for (AchievementCriteriaEntryList::const_iterator itr = cList->begin();
        ...
        // found an uncompleted criteria, but DONT return false yet - there
        // might be a completed criteria with
...
CriteriaProgress*
AchievementMgr::GetCriteriaProgress(AchievementCriteriaEntry const* entry)
{
                                         ...
                                         ProgressType ptype)
{
    ...
    TimedAchievementMap::iterator timedIter =
        _timedAchievements.find(entry->ID);
    ...
    LOG_DEBUG("achievement",
              "AchievementMgr::SetCriteriaProgress({}, {}) for {}",
              entry->ID,
              changeValue,
    ...
    if (!progress) {
        ...
        case PROGRESS_ACCUMULATE: {
            ...
            newValue         = max_value - progress->counter > changeValue
                                   ? progress->counter + changeValue
                                   : max_value;
            break;
        }
        case PROGRESS_HIGHEST:
            newValue = progress->counter < changeValue ? changeValue
                                                       : progress->counter;
            break;
        }

        // not update (not mark as changed) if counter will have same value
        if (ptype != PROGRESS_RESET && progress->counter == newValue &&
            !entry->timeLimit)
            return;
    ...
    progress->date =
        GameTime::GetGameTime().count(); // set the date to the latest update.
    ...
    if (entry->timeLimit) {
        ...
        timedCompleted = IsCompletedCriteria(
            entry, sAchievementStore.LookupEntry(entry->referredAchievement));
...
{
    CriteriaProgressMap::iterator criteriaProgress =
        _criteriaProgress.find(entry->ID);
...
void AchievementMgr::UpdateTimedAchievements(uint32 timeDiff)
{
    if (!_timedAchievements.empty()) {
        for (TimedAchievementMap::iterator itr = _timedAchievements.begin();
             itr != _timedAchievements.end();) {
                                           ...
                                           uint32 timeLost /*= 0*/)
{
    ...
    for (AchievementCriteriaEntryList::const_iterator i =
        ...
        AchievementEntry const* achievement =
            sAchievementStore.LookupEntry((*i)->referredAchievement);
        if (_timedAchievements.find((*i)->ID) == _timedAchievements.end() &&
            !IsCompletedCriteria(*i, achievement)) {
            // Start the timer
            if ((*i)->timeLimit * IN_MILLISECONDS > timeLost) {
                _timedAchievements[(*i)->ID] =
                    (*i)->timeLimit * IN_MILLISECONDS - timeLost;
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    if (_player->IsGameMaster()) {
        ...
        ChatHandler(_player->GetSession())
            .PSendSysMessage("Not available in GM mode");
        return;
    }
    ...
    // pussywizard: set all progress counters to 0, so progress will be deleted
    // from db during save
    {
        bool   allRefsCompleted = true;
        uint32 achiCheckId      = achievement->refAchievement
                                      ? achievement->refAchievement
                                      : achievement->ID;

        if (AchievementEntryList const* achRefList =
                sAchievementMgr->GetAchievementByReferencedId(achiCheckId))
            for (AchievementEntryList::const_iterator itr = achRefList->begin();
                 itr != achRefList->end();
                 ++itr)
                if (!IsCompletedAchievement(*itr)) {
        ...
        if (allRefsCompleted)
            if (AchievementCriteriaEntryList const* cList =
                    sAchievementMgr->GetAchievementCriteriaByAchievement(
                        achiCheckId))
                for (AchievementCriteriaEntryList::const_iterator itr =
                         cList->begin();
                     itr != cList->end();
                     ++itr)
                    if (CriteriaProgress* progress =
                            GetCriteriaProgress(*itr)) {
                        progress->changed = true;
                        progress->counter = 0;
                    }
    }
    ...
    if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_REACH |
                              ACHIEVEMENT_FLAG_REALM_FIRST_KILL) &&
        AccountMgr::IsPlayerAccount(_player->GetSession()->GetSecurity()))
    ...
    AchievementReward const* reward =
        sAchievementMgr->GetAchievementReward(achievement);
    ...
    // titles
    //! Currently there's only one achievement that deals with gender-specific
    //! titles. Since no common attributes were found, (not even in
    //! titleRewardFlags field) we explicitly check by ID. Maybe in the future
    //! we could move the achievement_reward condition fields to the condition
    //! system.
    if (uint32 titleId = reward->titleId[achievement->ID == 1793
                                             ? GetPlayer()->getGender()
    ...
    if (reward->sender) {
        ...
        if (!reward->mailTemplate) {
            ...
            if (localeConstant != LOCALE_enUS) {
                if (AchievementRewardLocale const* loc =
                        sAchievementMgr->GetAchievementRewardLocale(
                            achievement)) {
                    ObjectMgr::GetLocaleString(
                        loc->Subject, localeConstant, subject);
        ...
        Item* item = reward->itemId
                         ? Item::CreateItem(reward->itemId, 1, GetPlayer())
                         : nullptr;
        if (item) {
            // save new item before send
            item->SaveToDB(trans); // save for prevent lost at next mail load,
                                   // if send fail then item will deleted
        ...
        draft.SendMailTo(
            trans, GetPlayer(), MailSender(MAIL_CREATURE, reward->sender));
        CharacterDatabase.CommitTransaction(trans);
    }
}
...
void AchievementMgr::SendAllAchievementData() const
{
    WorldPacket data(SMSG_ALL_ACHIEVEMENT_DATA,
                     _completedAchievements.size() * 8 + 4 +
                         _criteriaProgress.size() * 38 + 4);
...
void AchievementMgr::SendRespondInspectAchievements(Player* player) const
{
    WorldPacket data(SMSG_RESPOND_INSPECT_ACHIEVEMENTS,
                     9 + _completedAchievements.size() * 8 + 4 +
                         _criteriaProgress.size() * 38 + 4);
    data << GetPlayer()->GetPackGUID();
    BuildAllDataPacket(&data);
...
void AchievementMgr::BuildAllDataPacket(WorldPacket* data) const
{
    for (CompletedAchievementMap::const_iterator iter =
        ...
        AchievementEntry const* achievement =
            sAchievementStore.LookupEntry(iter->first);
    ...
    *data << int32(-1);
    ...
    for (CriteriaProgressMap::const_iterator iter = _criteriaProgress.begin();
        ...
        *data << GetPlayer()->GetPackGUID();
        *data << uint32(
            0); /// @todo: This should be 1 if it is a failed timed criteria
        data->AppendPackedTime(iter->second.date);
        *data << uint32(now - iter->second.date);

        if (sAchievementMgr->IsAverageCriteria(
                sAchievementCriteriaStore.LookupEntry(iter->first)))
            *data << uint32(now - GetPlayer()
                                      ->GetCreationTime()
                                      .count()); // for average achievements
        else
            *data << uint32(now - iter->second.date);
    }
...
{
    return _completedAchievements.find(achievementId) !=
           _completedAchievements.end();
}
                                       ...
                                       AchievementEntry const* achievement)
{
    if (DisableMgr::IsDisabledFor(
            DISABLE_TYPE_ACHIEVEMENT_CRITERIA, criteria->ID, nullptr))
        return false;

    if (achievement->mapID != -1 &&
        GetPlayer()->GetMapId() != uint32(achievement->mapID))
        return false;

    if ((achievement->requiredFaction == ACHIEVEMENT_FACTION_HORDE &&
         GetPlayer()->GetTeamId(true) != TEAM_HORDE) ||
        (achievement->requiredFaction == ACHIEVEMENT_FACTION_ALLIANCE &&
         GetPlayer()->GetTeamId(true) != TEAM_ALLIANCE))
        return false;
    ...
    for (uint32 i = 0; i < MAX_CRITERIA_REQUIREMENTS; ++i) {
        ...
        switch (
            criteria->additionalRequirements[i].additionalRequirement_type) {
        case ACHIEVEMENT_CRITERIA_CONDITION_BG_MAP:
            if (GetPlayer()->GetMapId() !=
                criteria->additionalRequirements[i].additionalRequirement_value)
                return false;
            break;
...
bool AchievementGlobalMgr::IsStatisticCriteria(
    AchievementCriteriaEntry const* achievementCriteria) const
{
    return IsStatisticAchievement(sAchievementStore.LookupEntry(
        achievementCriteria->referredAchievement));
}

bool AchievementGlobalMgr::IsStatisticAchievement(
    AchievementEntry const* achievement) const
{
    if (!achievement)
        return false;

    AchievementCategoryEntry const* cat =
        sAchievementCategoryStore.LookupEntry(achievement->categoryId);
    do {
        switch (cat->ID) {
...
bool AchievementGlobalMgr::IsAverageCriteria(
    AchievementCriteriaEntry const* criteria) const
{
    if ((sAchievementStore.LookupEntry(criteria->referredAchievement))->flags &
        ACHIEVEMENT_FLAG_AVERAGE)
        return true;

    if (AchievementEntryList const* achRefList =
            GetAchievementByReferencedId(criteria->referredAchievement))
        for (AchievementEntryList::const_iterator itr = achRefList->begin();
             itr != achRefList->end();
             ++itr)
...
bool AchievementGlobalMgr::IsRealmCompleted(
    AchievementEntry const* achievement) const
{
    ...
    // Allow completing the realm first kill for entire minute after first
    // person did it it may allow more than one group to achieve it (highly
    // unlikely) but apparently this is how blizz handles it as well
    if (achievement->flags & ACHIEVEMENT_FLAG_REALM_FIRST_KILL)
...
void AchievementGlobalMgr::SetRealmCompleted(
    AchievementEntry const* achievement)
{
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows();
        ...
        if (!GetAchievement(criteria->referredAchievement)) {
            LOG_DEBUG("server.loading",
                      "Achievement {} referenced by criteria {} doesn't exist, "
                      "criteria not loaded.",
        ...
        _achievementCriteriaListByAchievement[criteria->referredAchievement]
            .push_back(criteria);

        if (criteria->additionalRequirements[0].additionalRequirement_type !=
            ACHIEVEMENT_CRITERIA_CONDITION_NONE)
            _achievementCriteriasByCondition[criteria->additionalRequirements[0]
                                                 .additionalRequirement_type]
                                            [criteria->additionalRequirements[0]
                                                 .additionalRequirement_value]
                                                .push_back(criteria);
        if (criteria->additionalRequirements[1].additionalRequirement_type !=
                ACHIEVEMENT_CRITERIA_CONDITION_NONE &&
            criteria->additionalRequirements[1].additionalRequirement_type !=
                criteria->additionalRequirements[0].additionalRequirement_type)
            _achievementCriteriasByCondition[criteria->additionalRequirements[1]
                                                 .additionalRequirement_type]
                                            [criteria->additionalRequirements[1]
                                                 .additionalRequirement_value]
                                                .push_back(criteria);

        switch (criteria->requiredType) {
        case ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE:
            _specialList[criteria->requiredType]
                        [criteria->kill_creature.creatureID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_WIN_BG:
            _specialList[criteria->requiredType][criteria->win_bg.bgMapID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_REACH_SKILL_LEVEL:
            _specialList[criteria->requiredType]
                        [criteria->reach_skill_level.skillID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ACHIEVEMENT:
            _specialList[criteria->requiredType]
                        [criteria->complete_achievement.linkedAchievement]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUESTS_IN_ZONE:
            _specialList[criteria->requiredType]
                        [criteria->complete_quests_in_zone.zoneID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_BATTLEGROUND:
            _specialList[criteria->requiredType]
                        [criteria->complete_battleground.mapID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_CREATURE:
            _specialList[criteria->requiredType]
                        [criteria->killed_by_creature.creatureEntry]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUEST:
            _specialList[criteria->requiredType]
                        [criteria->complete_quest.questID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET:
            _specialList[criteria->requiredType]
                        [criteria->be_spell_target.spellID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL:
            _specialList[criteria->requiredType][criteria->cast_spell.spellID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE:
            _specialList[criteria->requiredType]
                        [criteria->bg_objective.objectiveId]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL_AT_AREA:
            _specialList[criteria->requiredType]
                        [criteria->honorable_kill_at_area.areaID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SPELL:
            _specialList[criteria->requiredType][criteria->learn_spell.spellID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM:
            _specialList[criteria->requiredType][criteria->own_item.itemID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LEVEL:
            _specialList[criteria->requiredType]
                        [criteria->learn_skill_level.skillID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM:
            _specialList[criteria->requiredType][criteria->use_item.itemID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_LOOT_ITEM:
            _specialList[criteria->requiredType][criteria->own_item.itemID]
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_EXPLORE_AREA: {
            WorldMapOverlayEntry const* worldOverlayEntry =
                sWorldMapOverlayStore.LookupEntry(
                    criteria->explore_area.areaReference);
            if (!worldOverlayEntry)
                break;
            ...
            for (uint8 j = 0; j < MAX_WORLD_MAP_OVERLAY_AREA_IDX; ++j)
                if (worldOverlayEntry->areatableID[j]) {
                    ...
                    for (uint8 i = 0; i < j; ++i)
                        if (worldOverlayEntry->areatableID[j] ==
                            worldOverlayEntry->areatableID[i])
                            valid = false;
                    if (valid)
                        _specialList[criteria->requiredType]
                                    [worldOverlayEntry->areatableID[j]]
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION:
            _specialList[criteria->requiredType]
                        [criteria->gain_reputation.factionID]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_EQUIP_EPIC_ITEM:
            _specialList[criteria->requiredType]
                        [criteria->equip_epic_item.itemSlot]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_HK_CLASS:
            _specialList[criteria->requiredType][criteria->hk_class.classID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_HK_RACE:
            _specialList[criteria->requiredType][criteria->hk_race.raceID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_DO_EMOTE:
            _specialList[criteria->requiredType][criteria->do_emote.emoteID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_EQUIP_ITEM:
            _specialList[criteria->requiredType][criteria->equip_item.itemID]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_USE_GAMEOBJECT:
            _specialList[criteria->requiredType]
                        [criteria->use_gameobject.goEntry]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2:
            _specialList[criteria->requiredType]
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_FISH_IN_GAMEOBJECT:
            _specialList[criteria->requiredType]
                        [criteria->fish_in_gameobject.goEntry]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS:
            _specialList[criteria->requiredType]
                        [criteria->learn_skillline_spell.skillLine]
                            .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_LOOT_TYPE:
            _specialList[criteria->requiredType][criteria->loot_type.lootType]
                .push_back(criteria);
            break;
        case ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL2:
            _specialList[criteria->requiredType][criteria->cast_spell.spellID]
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LINE:
            _specialList[criteria->requiredType]
                        [criteria->learn_skill_line.skillLine]
                            .push_back(criteria);
            break;
        }

        if (criteria->timeLimit)
            _achievementCriteriasByTimedType[criteria->timedType].push_back(
                criteria);
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} achievement criteria in {} ms",
             loaded,
...
void AchievementGlobalMgr::LoadAchievementReferenceList()
{
    ...
    for (uint32 entryId = 0; entryId < sAchievementStore.GetNumRows();
         ++entryId) {
        AchievementEntry const* achievement =
            sAchievementStore.LookupEntry(entryId);
        if (!achievement || !achievement->refAchievement)
            continue;

        _achievementListByReferencedId[achievement->refAchievement].push_back(
            achievement);
        ++count;
    }

    LOG_INFO("server.loading",
             ">> Loaded {} achievement references in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void AchievementGlobalMgr::LoadAchievementCriteriaData()
{
    ...
    QueryResult result =
        WorldDatabase.Query("SELECT criteria_id, type, value1, value2, "
                            "ScriptName FROM achievement_criteria_data");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 additional achievement criteria data. DB table "
                 "`achievement_criteria_data` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!criteria) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_criteria_data` has data for "
                      "non-existing criteria (Entry: {}), ignore.",
                      criteria_id);
            continue;
        }
        ...
        if (scriptName.length()) // not empty
        {
            if (dataType != ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT)
                LOG_ERROR("sql.sql",
                          "Table `achievement_criteria_data` has ScriptName "
                          "set for non-scripted data type (Entry: {}, type "
                          "{}), useless data.",
                          criteria_id,
                          dataType);
            else
        ...
        AchievementCriteriaData data(dataType,
                                     fields[2].Get<uint32>(),
                                     fields[3].Get<uint32>(),
                                     scriptId);
    ...
    for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows();
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUEST: {
            AchievementEntry const* achievement =
                sAchievementStore.LookupEntry(criteria->referredAchievement);
            if (!achievement)
                continue;

            // exist many achievements with this criteria, use at this moment
            // hardcoded check to skil simple case
            if (achievement->ID == 1282)
                break;

            continue;
        }
        case ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA: // need skip generic
                                                        // cases
            if (criteria->additionalRequirements[0]
                    .additionalRequirement_type !=
                ACHIEVEMENT_CRITERIA_CONDITION_NO_LOSE)
                continue;
            break;
        ...
        if (!GetCriteriaDataSet(criteria) &&
            !DisableMgr::IsDisabledFor(
                DISABLE_TYPE_ACHIEVEMENT_CRITERIA, entryId, nullptr))
            LOG_ERROR(
                "sql.sql",
                "Table `achievement_criteria_data` does not have expected data "
                "for criteria (Entry: {} Type: {}) for achievement {}.",
                criteria->ID,
                criteria->requiredType,
                criteria->referredAchievement);
    }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} additional achievement criteria data in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    // Populate _allCompletedAchievements with all realm first achievement ids
    // to make multithreaded access safer while it will not prevent races, it
    // will prevent crashes that happen because std::unordered_map key was added
    // instead the only potential race will happen on value associated with the
    // key
    for (uint32 i = 0; i < sAchievementStore.GetNumRows(); ++i)
        if (AchievementEntry const* achievement =
                sAchievementStore.LookupEntry(i))
            if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_REACH |
                                      ACHIEVEMENT_FLAG_REALM_FIRST_KILL))
                _allCompletedAchievements[achievement->ID] =
                    SystemTimePoint::min();

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 completed achievements. DB table "
                 "`character_achievement` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (!achievement) {
            ...
            LOG_ERROR("achievement",
                      "Non-existing achievement {} data removed from table "
        ...
        else if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_REACH |
                                       ACHIEVEMENT_FLAG_REALM_FIRST_KILL))
            _allCompletedAchievements[achievementId] = SystemTimePoint::max();
    } while (result->NextRow());

    LOG_INFO("server.loading",
             ">> Loaded {} completed achievements in {} ms",
             (unsigned long)_allCompletedAchievements.size(),
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT ID, TitleA, TitleH, ItemID, Sender, Subject, Body, "
        "MailTemplateID FROM achievement_reward");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 achievement rewards. DB table "
                 "`achievement_reward` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        AchievementEntry const* achievement =
            sAchievementStore.LookupEntry(entry);
        if (!achievement) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_reward` has wrong achievement "
                      "(Entry: {}). Ignoring.",
                      entry);
            continue;
        }
        ...
        reward.sender =
            fields[4].Get<uint32>(); // The sender of the mail (a creature from
                                     // creature_template)
        ...
        if (!reward.titleId[0] && !reward.titleId[1] && !reward.sender) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_reward` (Entry: {}) does not have "
                      "any title or item reward data. Ignoring.",
                      entry);
            continue;
        }

        if (achievement->requiredFaction == ACHIEVEMENT_FACTION_ANY &&
            (!reward.titleId[0] ^ !reward.titleId[1]))
            LOG_DEBUG("achievement",
                      "Table `achievement_reward` (Entry: {}) has title (A: {} "
                      "H: {}) set for only one team.",
                      entry,
                      reward.titleId[0],
                      reward.titleId[1]);
        ...
        if (reward.titleId[0]) {
            CharTitlesEntry const* titleEntry =
                sCharTitlesStore.LookupEntry(reward.titleId[0]);
            if (!titleEntry) {
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has invalid "
                          "title id ({}) in `title_A`. Setting it to 0.",
                          entry,
                          reward.titleId[0]);
                reward.titleId[0] = 0;
            }
        ...
        if (reward.titleId[1]) {
            CharTitlesEntry const* titleEntry =
                sCharTitlesStore.LookupEntry(reward.titleId[1]);
            if (!titleEntry) {
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has invalid "
                          "title id ({}) in `title_H`. Setting it to 0.",
                          entry,
                          reward.titleId[1]);
                reward.titleId[1] = 0;
            }
        ...
        if (reward.sender) {
            if (!sObjectMgr->GetCreatureTemplate(reward.sender)) {
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has invalid "
                          "creature_template entry {} as Sender. Will not send "
                          "the mail reward.",
                          entry,
                          reward.sender);
                reward.sender = 0;
            ...
            if (reward.itemId)
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has itemId "
                          "reward set but does not have Sender data set. Item "
                          "will not be sent.",
                          entry);

            if (!reward.subject.empty())
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has mail "
                          "Subject but does not have Sender data set.",
                          entry); // Maybe add "Mail will not be sent." ?

            if (!reward.text.empty())
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has mail "
                          "text (Body) set but does not have Sender data set.",
                          entry); // Maybe add "Mail will not be sent." ?

            if (reward.mailTemplate)
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has "
                          "mailTemplate set does not have Sender data set.",
                          entry); // Maybe add "Mail will not be sent." ?
        }
        ...
        if (reward.mailTemplate) {
            if (!sMailTemplateStore.LookupEntry(reward.mailTemplate)) {
                ...
                LOG_ERROR(
                    "sql.sql",
                    "Table `achievement_reward` (Entry: {}) has mailTemplate "
                    "({}) and mail Subject/Body. To use the column "
                    "mailTemplate, Subject and Body must be empty.",
                    entry,
                    reward.mailTemplate);
        }
        ...
        if (reward.itemId) {
            if (!sObjectMgr->GetItemTemplate(reward.itemId)) {
                // Not sure it's an error, it's probably an outDebug instead,
                // because we can simply send a mail with no reward, right?
                LOG_ERROR("sql.sql",
                          "Table `achievement_reward` (Entry: {}) has invalid "
                          "item_template id {}. Reward mail will not contain "
                          "any item.",
                          entry,
                          reward.itemId);
                reward.itemId = 0;
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} achievement rewards in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 achievement reward locale strings.  DB table "
                 "`achievement_reward_locale` is empty");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        if (_achievementRewards.find(ID) == _achievementRewards.end()) {
            LOG_ERROR("sql.sql",
                      "Table `achievement_reward_locale` (Entry: {}) has "
                      "locale strings for non-existing achievement reward.",
                      ID);
            continue;
        }
        ...
        ObjectMgr::AddLocaleString(
            fields[2].Get<std::string>(), locale, data.Subject);
        ObjectMgr::AddLocaleString(
            fields[3].Get<std::string>(), locale, data.Text);
    } while (result->NextRow());

    LOG_INFO("server.loading",
             ">> Loaded {} Achievement Reward Locale Strings in {} ms",
             (unsigned long)_achievementRewardLocales.size(),
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}

AchievementEntry const*
AchievementGlobalMgr::GetAchievement(uint32 achievementId) const
{
