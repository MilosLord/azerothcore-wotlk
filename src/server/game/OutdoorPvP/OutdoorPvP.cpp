...
bool OPvPCapturePoint::HandlePlayerEnter(Player* player)
{
    if (_capturePoint) {
        player->SendUpdateWorldState(
            _capturePoint->GetGOInfo()->capturePoint.worldState1, 1);
        player->SendUpdateWorldState(
            _capturePoint->GetGOInfo()->capturePoint.worldstate2,
            (uint32)std::ceil((_value + _maxValue) / (2 * _maxValue) * 100.0f));
        player->SendUpdateWorldState(
            _capturePoint->GetGOInfo()->capturePoint.worldstate3,
            _neutralValuePct);
    }
...
void OPvPCapturePoint::HandlePlayerLeave(Player* player)
{
    if (_capturePoint) {
        player->SendUpdateWorldState(
            _capturePoint->GetGOInfo()->capturePoint.worldState1, 0);
    }
...
void OPvPCapturePoint::SendChangePhase()
{
    ...
    SendUpdateWorldState(_capturePoint->GetGOInfo()->capturePoint.worldState1,
                         1);
    ...
    SendUpdateWorldState(_capturePoint->GetGOInfo()->capturePoint.worldstate3,
                         _neutralValuePct);
}
...
bool OPvPCapturePoint::AddObject(uint32 type,
                                 ...
                                 float  rotation3)
{
    if (ObjectGuid::LowType guid = sObjectMgr->AddGOData(entry,
                                                         map,
                                                         x,
                                                         y,
                                                         z,
                                   ...
                                   uint32 spawntimeDelay)
{
    if (ObjectGuid::LowType guid =
            sObjectMgr->AddCreData(entry, map, x, y, z, o, spawntimeDelay)) {
                                           ...
                                           float  rotation3)
{
    ...
    if (!goinfo || goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT) {
        LOG_ERROR(
            "outdoorpvp", "OutdoorPvP: GO {} is not capture point!", entry);
        return false;
    }

    m_capturePointSpawnId = sObjectMgr->AddGOData(
        entry, map, x, y, z, o, 0, rotation0, rotation1, rotation2, rotation3);
    ...
    _maxSpeed =
        _maxValue /
        float(goinfo->capturePoint.minTime ? goinfo->capturePoint.minTime : 60);
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    ...
    if (!spawnId) {
        LOG_DEBUG("outdoorpvp",
                  "OutdoorPvP: Creature type {} was already deleted",
                  type);
        return false;
    }
    ...
    // beats me why this is needed, but with the recent removal "cleanup" some
    // creatures stay in the map if "properly" deleted so this is a big fat
    // workaround, if AddObjectToRemoveList and DoDelayedMovesAndRemoves worked
    // correctly, this wouldn't be needed
...
bool OPvPCapturePoint::DelObject(uint32 type)
{
    ...
    auto bounds =
        _pvp->GetMap()->GetGameObjectBySpawnIdStore().equal_range(spawnId);
    for (auto itr = bounds.first; itr != bounds.second;) {
...
void OutdoorPvP::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    ...
    // remove the world state information from the player (we can't keep
    // everyone up to date, so leave out those who are not in the concerning
    // zones)
    ...
    LOG_DEBUG("outdoorpvp",
              "OutdoorPvP: Player {} left an outdoorpvp zone",
              player->GetName());
}
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    float factDiff =
        ((float)_activePlayers[0].size() - (float)_activePlayers[1].size()) *
    ...
    else if (oldValue * _value <= 0) // grey, go through mid-point
    {
        ...
        if (ChallengerId == TEAM_ALLIANCE) {
            _state = OBJECTIVESTATE_NEUTRAL_ALLIANCE_CHALLENGE;
                 ...
                 TEAM_HORDE) // if challenger is horde, then n->h challenge
        {
