...
void OutdoorPvPMgr::InitOutdoorPvP()
{
    ...
    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 outdoor PvP definitions. DB table "
                 "`outdoorpvp_template` is empty.");
    ...
    for (auto const& fields : *result) {
        ...
        if (typeId >= MAX_OUTDOORPVP_TYPES) {
            LOG_ERROR("sql.sql",
                      "Invalid OutdoorPvPTypes value {} in "
                      "outdoorpvp_template; skipped.",
                      typeId);
            continue;
        }
    ...
    for (uint8 i = 1; i < MAX_OUTDOORPVP_TYPES; ++i) {
        ...
        if (iter == m_OutdoorPvPDatas.end()) {
            LOG_ERROR("sql.sql",
                      "Could not initialize OutdoorPvP object for type ID {}; "
                      "no entry in database.",
                      uint32(i));
            continue;
        }

        auto pvp = std::unique_ptr<OutdoorPvP>(
            sScriptMgr->CreateOutdoorPvP(iter->second.get()));
        if (!pvp) {
            LOG_ERROR("outdoorpvp",
                      "Could not initialize OutdoorPvP object for type ID {}; "
                      "got nullptr pointer from script.",
                      uint32(i));
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} outdoor PvP definitions in {} ms",
             count,
