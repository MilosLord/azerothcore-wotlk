...
enum ChatNotify : uint8 {
    ...
    // CHAT_SUSPENDED_NOTICE             = 0x01,           // "%s left
    // channel.";
    CHAT_YOU_JOINED_NOTICE = 0x02, //+ "Joined Channel: [%s]"; -- You joined
    // CHAT_YOU_CHANGED_NOTICE           = 0x02,           // "Changed Channel:
    // [%s]";
    ...
    CHAT_ANNOUNCEMENTS_ON_NOTICE =
        0x0D, //+ "[%s] Channel announcements enabled by %s.";
    CHAT_ANNOUNCEMENTS_OFF_NOTICE =
        0x0E, //+ "[%s] Channel announcements disabled by %s.";
    CHAT_MODERATION_ON_NOTICE =
        0x0F, //+ "[%s] Channel moderation enabled by %s.";
    CHAT_MODERATION_OFF_NOTICE =
        0x10,                 //+ "[%s] Channel moderation disabled by %s.";
    CHAT_MUTED_NOTICE = 0x11, //+ "[%s] You do not have permission to speak.";
    CHAT_PLAYER_KICKED_NOTICE = 0x12, //? "[%s] Player %s kicked by %s.";
    CHAT_BANNED_NOTICE =
        0x13, //+ "[%s] You are bannedStore from that channel.";
    ...
    CHAT_PLAYER_NOT_BANNED_NOTICE =
        0x16, //+ "[%s] Player %s is not bannedStore.";
    CHAT_PLAYER_ALREADY_MEMBER_NOTICE =
        0x17, //+ "[%s] Player %s is already on the channel.";
    CHAT_INVITE_NOTICE =
        0x18, //+ "%2$s has invited you to join the channel '%1$s'.";
    CHAT_INVITE_WRONG_FACTION_NOTICE =
        0x19, //+ "Target is in the wrong alliance for %s.";
    ...
    CHAT_PLAYER_INVITED_NOTICE =
        0x1D, //+ "[%s] You invited %s to join the channel";
    CHAT_PLAYER_INVITE_BANNED_NOTICE =
        0x1E, //+ "[%s] %s has been bannedStore.";
    CHAT_THROTTLED_NOTICE =
        0x1F, //+ "[%s] The number of messages that can be sent to this channel
              //is limited, please wait to send another message.";
    CHAT_NOT_IN_AREA_NOTICE =
        0x20, //+ "[%s] You are not in the correct area for this channel."; --
              //The user is trying to send a chat to a zone specific channel,
              //and they're not physically in that zone.
    CHAT_NOT_IN_LFG_NOTICE =
        0x21, //+ "[%s] You must be queued in looking for group before joining
              //this channel."; -- The user must be in the looking for group
              //system to join LFG chat channels.
...
enum ChannelDBCFlags {
    ...
    CHANNEL_DBC_FLAG_ZONE_DEP =
        0x00002, // General, Trade, LocalDefense, GuildRecruitment
...
public:
    ...
    ChannelRights(const uint32&    f,
                  const uint32&    d,
                  std::string      jm,
                  std::string      sm,
                  std::set<uint32> ml)
        : flags(f), speakDelay(d), joinMessage(std::move(jm)),
          speakMessage(std::move(sm)), moderators(std::move(ml))
    {
    }
    uint32           flags{0};
    uint32           speakDelay{0};
...
public:
    ...
    void
    SetMode(Player const* player, std::string const& p2n, bool mod, bool set);
...
private:
    ...
    void MakePlayerNotFound(WorldPacket*       data,
                            std::string const& name); //+ 0x09
    void MakeNotOwner(WorldPacket* data);             //? 0x0A
    void MakeChannelOwner(WorldPacket* data);         //? 0x0B
    void
    MakeModeChange(WorldPacket* data, ObjectGuid guid, uint8 oldflags); //+ 0x0C
    ...
    void MakePlayerKicked(WorldPacket* data,
                          ObjectGuid   bad,
                          ObjectGuid   good); //? 0x12
    void MakeBanned(WorldPacket* data);     //? 0x13
    void MakePlayerBanned(WorldPacket* data,
                          ObjectGuid   bad,
                          ObjectGuid   good); //? 0x14
    void MakePlayerUnbanned(WorldPacket* data,
                            ObjectGuid   bad,
                            ObjectGuid   good); //? 0x15
    void MakePlayerNotBanned(WorldPacket*       data,
                             std::string const& name);                  //? 0x16
    ...
    void MakePlayerInviteBanned(WorldPacket*       data,
                                std::string const& name);  //? 0x1E
    ...
    [[nodiscard]] bool IsOn(ObjectGuid who) const
    {
        return playersStore.find(who) != playersStore.end();
    }
