...
void ChannelMgr::LoadChannels()
{
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT channelId, name, team, announce, ownership, password FROM "
        "channels ORDER BY channelId ASC");
    if (!result) {
    ...
    do {
        ...
        if (!Utf8toWStr(channelName, channelWName)) {
            LOG_ERROR("server.loading",
                      "Failed to load channel '{}' from database - invalid "
                      "utf8 sequence? Deleted.",
                      channelName);
        ...
        if (!mgr) {
            LOG_ERROR("server.loading",
                      "Failed to load custom chat channel '{}' from database - "
                      "invalid team {}. Deleted.",
                      channelName,
                      team);
            toDelete.push_back({channelName, team});
            continue;
        ...
        if (QueryResult banResult =
                CharacterDatabase.Query("SELECT playerGUID, banTime FROM "
                                        "channels_bans WHERE channelId = {}",
                                        channelDBId)) {
            do {
                Field* banFields = banResult->Fetch();
                if (!banFields)
                    break;
                newChannel->AddBan(ObjectGuid::Create<HighGuid::Player>(
    ...
    for (auto& pair : toDelete) {
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHANNEL);
...
Channel*
ChannelMgr::GetChannel(std::string const& name, Player* player, bool pkt)
{
    std::wstring wname;
    Utf8toWStr(name, wname);
    wstrToLower(wname);
...
void ChannelMgr::LoadChannelRights()
{
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT name, flags, speakdelay, joinmessage, delaymessage, moderators "
                                     ...
                                     const std::set<uint32>& moderators)
{
    std::string nameStr = name;
    std::transform(nameStr.begin(), nameStr.end(), nameStr.begin(), ::tolower);
    channels_rights[nameStr] =
        ChannelRights(flags, speakDelay, joinmessage, speakmessage, moderators);
}
