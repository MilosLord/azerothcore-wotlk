...
struct LinkValidator<LinkTags::achievement> {
    static bool IsTextValid(AchievementLinkData const& data,
                            std::string_view           text)
    {
        if (text.empty())
            return false;
...
struct LinkValidator<LinkTags::item> {
    static bool IsTextValid(ItemLinkData const& data, std::string_view text)
    {
        ...
        if (data.IsBuggedInspectLink) /* DBC lookup will have failed on the
                                         client, so the link should've arrived
                                         without suffix */
        ...
        for (uint8 i = 0; i < TOTAL_LOCALES; ++i) {
            ...
            std::string_view name =
                (i == DEFAULT_LOCALE)
                    ? data.Item->Name1
                    : ObjectMgr::GetLocaleString(locale->Name, i);
            if (name.empty())
                continue;
            ...
            if (randomSuffixes) {
                ...
                if ((!randomSuffix.empty()) &&
                    (text.length() ==
                     (name.length() + 1 + randomSuffix.length())) &&
...
struct LinkValidator<LinkTags::quest> {
    static bool IsTextValid(QuestLinkData const& data, std::string_view text)
    {
        ...
        QuestLocale const* locale =
            sObjectMgr->GetQuestLocale(data.Quest->GetQuestId());
        if (!locale)
            return false;
        ...
        for (uint8 i = 0; i < TOTAL_LOCALES; ++i) {
            ...
            std::string_view name =
                ObjectMgr::GetLocaleString(locale->Title, i);
...
struct LinkValidator<LinkTags::enchant> {
    static bool IsTextValid(SpellInfo const* info, std::string_view text)
    {
        ...
        SkillLineAbilityMapBounds bounds =
            sSpellMgr->GetSkillLineAbilityMapBounds(info->Id);
        if (bounds.first == bounds.second)
            return false;

        for (auto pair = bounds.first; pair != bounds.second; ++pair) {
            SkillLineEntry const* skill =
                sSkillLineStore.LookupEntry(pair->second->SkillLine);
            if (!skill)
                return false;
            ...
            for (uint8 i = 0; i < TOTAL_LOCALES; ++i) {
                ...
                if ((text.length() ==
                     (skillName.length() + 2 + spellName.length())) &&
...
struct LinkValidator<LinkTags::glyph> {
    static bool IsTextValid(GlyphLinkData const& data, std::string_view text)
    {
        if (SpellInfo const* info =
                sSpellMgr->GetSpellInfo(data.Glyph->SpellId))
...
struct LinkValidator<LinkTags::trade> {
    static bool IsTextValid(TradeskillLinkData const& data,
                            std::string_view          text)
    {
...
static bool ValidateAs(HyperlinkInfo const& info)
{
    ...
    int32 const severity = static_cast<int32>(
        sWorld->getIntConfig(CONFIG_CHAT_STRICT_LINK_CHECKING_SEVERITY));
    if (severity >= 0) {
