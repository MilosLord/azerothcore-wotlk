...
using ChatSubCommandMap = std::map<std::string_view,
                                   Acore::Impl::ChatCommands::ChatCommandNode,
                                   StringCompareLessI_T>;

void Acore::Impl::ChatCommands::ChatCommandNode::LoadFromBuilder(
    ChatCommandBuilder const& builder)
{
    if (std::holds_alternative<ChatCommandBuilder::InvokerEntry>(
            builder._data)) {
        ASSERT(!_invoker, "Duplicate blank sub-command.");
        AcoreStrings help;
        std::tie(_invoker, help, _permission) =
            *(std::get<ChatCommandBuilder::InvokerEntry>(builder._data));
        if (help)
        ...
        LoadCommandsIntoMap(
            this,
            _subCommands,
            std::get<ChatCommandBuilder::SubCommandEntry>(builder._data));
}

/*static*/ void Acore::Impl::ChatCommands::ChatCommandNode::LoadCommandsIntoMap(
    ChatCommandNode*                             blank,
    std::map<std::string_view,
             Acore::Impl::ChatCommands::ChatCommandNode,
             StringCompareLessI_T>&              map,
    ...
    for (ChatCommandBuilder const& builder : commands) {
        if (builder._name.empty()) {
            ...
            std::vector<std::string_view> const tokens =
                Acore::Tokenize(builder._name, COMMAND_DELIMITER, false);
            ASSERT(
                !tokens.empty(), "Invalid command name '{}'.", builder._name);
...
/*static*/ ChatSubCommandMap const&
Acore::Impl::ChatCommands::ChatCommandNode::GetTopLevelMap()
{
...
/*static*/ void
Acore::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap()
{
    COMMAND_MAP.clear();
}
    ...
    if (PreparedQueryResult result = WorldDatabase.Query(
        ...
        do {
            ...
            for (std::string_view key :
                 Acore::Tokenize(name, COMMAND_DELIMITER, false)) {
                ...
                if (it != map->end()) {
                    ...
                    LOG_ERROR("sql.sql",
                              "Table `command` contains data for non-existant "
                              "command '{}'. Skipped.",
                              name);
                    cmd = nullptr;
                    break;
            ...
            if (cmd->_invoker && (cmd->_permission.RequiredLevel != secLevel)) {
                LOG_WARN("sql.sql",
                         "Table `command` has permission {} for '{}' which "
                         "does not match the core ({}). Overriding.",
                         secLevel,
                         name,
                         cmd->_permission.RequiredLevel);
            ...
            if (std::holds_alternative<std::string>(cmd->_help))
                LOG_ERROR("sql.sql",
                          "Table `command` contains duplicate data for command "
                          "'{}'. Skipped.",
                          name);
            ...
            if (std::holds_alternative<std::monostate>(cmd->_help))
                ...
                LOG_ERROR("sql.sql",
                          "Table `command` contains legacy help text for "
                          "command '{}', which uses `acore_string`. Skipped.",
                          name);
...
void Acore::Impl::ChatCommands::ChatCommandNode::ResolveNames(std::string name)
{
    if (_invoker && std::holds_alternative<std::monostate>(_help))
        LOG_WARN("sql.sql",
                 "Table `command` is missing help text for command '{}'.",
                 name);
                            ...
                            std::string_view    cmdStr)
{
    ...
    std::string logMessage = Acore::StringFormatFmt(
        "Command: {} [Player: {} ({}) (Account: {}) X: {} Y: {} Z: {} Map: {} "
        "({}) Area: {} ({}) Zone: {} ({}) Selected: {} ({})]",
        cmdStr,
        player->GetName(),
...
void Acore::Impl::ChatCommands::ChatCommandNode::SendCommandHelp(
    ChatHandler& handler) const
{
    ...
    if (hasInvoker) {
        if (std::holds_alternative<AcoreStrings>(_help))
            ...
            handler.PSendSysMessage(LANG_CMD_HELP_GENERIC,
                                    STRING_VIEW_FMT_ARG(_name));
            handler.PSendSysMessage(LANG_CMD_NO_HELP_AVAILABLE,
                                    STRING_VIEW_FMT_ARG(_name));
        }
    }
    ...
    for (auto it = _subCommands.begin(); it != _subCommands.end(); ++it) {
        bool const subCommandHasSubCommand =
            it->second.HasVisibleSubCommands(handler);
        ...
        handler.PSendSysMessage(subCommandHasSubCommand
                                    ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS
                                    : LANG_SUBCMDS_LIST_ENTRY,
                                STRING_VIEW_FMT_ARG(it->second._name));
    }
}
...
public:
    FilteredCommandListIterator(ChatSubCommandMap const& map,
                                ChatHandler const&       handler,
                                std::string_view         token)
        : _handler{handler}, _token{token}, _it{map.lower_bound(token)},
          _end{map.end()}
    {
        _skip();
    }
...
/*static*/ bool Acore::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(
    ChatHandler& handler, std::string_view cmdStr)
{
    ...
    while (!oldTail.empty()) {
        ...
        if (!StringEqualI(
                it1->first,
                token)) { /* ok, so it1 points at a partially matching
                             subcommand - let's see if there are others */
            auto it2 = it1;
            ++it2;

            if (it2) { /* there are multiple matching subcommands - print
                          possibilities and return */
                if (cmd)
                    handler.PSendSysMessage(LANG_SUBCMD_AMBIGUOUS,
                                            STRING_VIEW_FMT_ARG(cmd->_name),
                                            COMMAND_DELIMITER,
                                            STRING_VIEW_FMT_ARG(token));
                else
                    handler.PSendSysMessage(LANG_CMD_AMBIGUOUS,
                                            STRING_VIEW_FMT_ARG(token));

                handler.PSendSysMessage(
                    it1->second.HasVisibleSubCommands(handler)
                        ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS
                        : LANG_SUBCMDS_LIST_ENTRY,
                    STRING_VIEW_FMT_ARG(it1->first));
                do {
                    handler.PSendSysMessage(
                        it2->second.HasVisibleSubCommands(handler)
                            ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS
                            : LANG_SUBCMDS_LIST_ENTRY,
                        STRING_VIEW_FMT_ARG(it2->first));
                } while (++it2);
        ...
        /* now we matched exactly one subcommand, and it1 points to it; go down
         * the rabbit hole */
        cmd = &it1->second;
        map = &cmd->_subCommands;
    ...
    if (cmd) { /* if we matched a command at some point, invoke it */
        handler.SetSentErrorMessage(false);
        if (cmd->IsInvokerVisible(handler) &&
            cmd->_invoker(&handler,
                          oldTail)) { /* invocation succeeded, log this */
            if (!handler.IsConsole())
                LogCommandUsage(*handler.GetSession(), cmdStr);
        }
        else if (!handler.HasSentErrorMessage()) /* invocation failed, we should
                                                    show usage */
        {
...
/*static*/ void Acore::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(
    ChatHandler& handler, std::string_view cmdStr)
{
    ChatCommandNode const*   cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    ...
    for (std::string_view token :
         Acore::Tokenize(cmdStr, COMMAND_DELIMITER, false)) {
        FilteredCommandListIterator it1(*map, handler, token);
        if (!it1) { /* no matching subcommands found */
            if (cmd) {
                cmd->SendCommandHelp(handler);
                handler.PSendSysMessage(LANG_SUBCMD_INVALID,
                                        STRING_VIEW_FMT_ARG(cmd->_name),
                                        COMMAND_DELIMITER,
                                        STRING_VIEW_FMT_ARG(token));
            }
            else
                handler.PSendSysMessage(LANG_CMD_INVALID,
                                        STRING_VIEW_FMT_ARG(token));
            return;
        }
    ...
    if (cmd)
        ...
        do {
            handler.PSendSysMessage(it->second.HasVisibleSubCommands(handler)
                                        ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS
                                        : LANG_SUBCMDS_LIST_ENTRY,
                                    STRING_VIEW_FMT_ARG(it->second._name));
        } while (++it);
...
Acore::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(
    ChatHandler const& handler, std::string_view cmdStr)
{
    std::string              path;
    ChatCommandNode const*   cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    ...
    while (!oldTail.empty()) {
        ...
        if (!StringEqualI(
            ...
            if (it2) { /* there are multiple matching subcommands - terminate
                          here and show possibilities */
                std::vector<std::string> vec;
                auto possibility = ([prefix = std::string_view(path),
                                     suffix = std::string_view(newTail)](
                                        std::string_view match) {
                    if (prefix.empty()) {
                        return Acore::StringFormatFmt(
                            "{}{}{}", match, COMMAND_DELIMITER, suffix);
                    }
                    else {
                        return Acore::StringFormatFmt("{}{}{}{}{}",
                                                      prefix,
                ...
                do
                    vec.emplace_back(possibility(it2->first));
                while (++it2);
    ...
    if (!oldTail.empty()) { /* there is some trailing text, leave it as is */
        if (cmd) { /* if we matched a command at some point, auto-complete it */
            return {Acore::StringFormatFmt(
                "{}{}{}", path, COMMAND_DELIMITER, oldTail)};
        }
        else
            return {};
    }
    else { /* offer all subcommands */
        auto possibility =
            ([prefix = std::string_view(path)](std::string_view match) {
                if (prefix.empty())
                    return std::string(match);
                else {
                    return Acore::StringFormatFmt(
                        "{}{}{}", prefix, COMMAND_DELIMITER, match);
                }
            });
...
bool Acore::Impl::ChatCommands::ChatCommandNode::IsInvokerVisible(
    ChatHandler const& who) const
{
    if (!_invoker)
        return false;

    if (who.IsConsole() &&
        (_permission.AllowConsole == Acore::ChatCommands::Console::No))
        return false;

    if (who.IsConsole() &&
        (_permission.AllowConsole == Acore::ChatCommands::Console::Yes))
        return true;

    return !who.IsConsole() && who.IsAvailable(_permission.RequiredLevel);
}

bool Acore::Impl::ChatCommands::ChatCommandNode::HasVisibleSubCommands(
    ChatHandler const& who) const
{
    for (auto it = _subCommands.begin(); it != _subCommands.end(); ++it)
...
{
    Acore::Impl::ChatCommands::ChatCommandNode::LoadCommandMap();
}
...
bool Acore::ChatCommands::TryExecuteCommand(ChatHandler&     handler,
                                            std::string_view cmd)
{
    return Acore::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(
        handler, cmd);
}
void Acore::ChatCommands::SendCommandHelpFor(ChatHandler&     handler,
                                             std::string_view cmd)
{
    Acore::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(handler,
                                                                   cmd);
}
std::vector<std::string>
Acore::ChatCommands::GetAutoCompletionsFor(ChatHandler const& handler,
                                           std::string_view   cmd)
{
    return Acore::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(
        handler, cmd);
}
