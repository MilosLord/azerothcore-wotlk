...
struct AchievementVisitor {
    ...
    {
        return achData->Achievement;
    }
    value_type operator()(uint32 achId) const
    {
        return sAchievementMgr->GetAchievement(achId);
    }
};

ChatCommandResult
Acore::Impl::ChatCommands::ArgInfo<AchievementEntry const*>::TryConsume(
    AchievementEntry const*& data,
    ...
    std::string_view         args)
{
    ...
    if (uint32* id = std::get_if<uint32>(&val))
        return FormatAcoreString(
            handler, LANG_CMDPARSER_ACHIEVEMENT_NO_EXIST, *id);
...
struct GameTeleVisitor {
    ...
    {
        return sObjectMgr->GetGameTele(tele);
    }
...
ChatCommandResult
Acore::Impl::ChatCommands::ArgInfo<GameTele const*>::TryConsume(
    GameTele const*& data, ChatHandler const* handler, std::string_view args)
{
    ...
    if (val.holds_alternative<Hyperlink<tele>>())
        return FormatAcoreString(
            ...
            static_cast<uint32>(std::get<Hyperlink<tele>>(val)));
    else
        return FormatAcoreString(
            handler,
            LANG_CMDPARSER_GAME_TELE_NO_EXIST,
            STRING_VIEW_FMT_ARG(std::get<std::string_view>(val)));
}
...
struct ItemTemplateVisitor {
    ...
    {
        return sObjectMgr->GetItemTemplate(item);
    }
};

ChatCommandResult
Acore::Impl::ChatCommands::ArgInfo<ItemTemplate const*>::TryConsume(
    ItemTemplate const*& data,
    ChatHandler const*   handler,
    std::string_view     args)
{
...
struct SpellInfoVisitor {
    ...
    {
        return operator()(glyph->Glyph->SpellId);
    };
...
ChatCommandResult
Acore::Impl::ChatCommands::ArgInfo<SpellInfo const*>::TryConsume(
    SpellInfo const*& data, ChatHandler const* handler, std::string_view args)
{
    Variant<Hyperlink<enchant>,
            Hyperlink<glyph>,
            Hyperlink<spell>,
            Hyperlink<talent>,
            Hyperlink<trade>,
...
struct QuestVisitor {
    ...
    value_type operator()(Hyperlink<quest> quest_template) const
    {
        return quest_template->Quest;
    };
...
ChatCommandResult Acore::Impl::ChatCommands::ArgInfo<Quest const*>::TryConsume(
    Quest const*& data, ChatHandler const* handler, std::string_view args)
{
