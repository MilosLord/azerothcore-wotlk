...
{
    static_assert(N <= 25,
                  "The EXACT_SEQUENCE macro can only be used with up to 25 "
                  "character long literals. Specify them char-by-char (null "
                  "terminated) as parameters to ExactSequence<> instead.");
    return i >= N ? '\0' : s[i];
}
...
#define CHATCOMMANDS_IMPL_SPLIT_LITERAL_CONSTRAINED(maxlen, strliteral)        \
    BOOST_PP_REPEAT(                                                           \
        maxlen, CHATCOMMANDS_IMPL_SPLIT_LITERAL_EXTRACT_CHAR, strliteral)

// this creates always 25 elements - "abc" -> 'a', 'b', 'c', '\0', '\0', ... up
// to 25
#define CHATCOMMANDS_IMPL_SPLIT_LITERAL(strliteral)                            \
    CHATCOMMANDS_IMPL_SPLIT_LITERAL_CONSTRAINED(25, strliteral)
} // namespace Acore::Impl::ChatCommands
...
struct ExactSequence : Acore::Impl::ChatCommands::ContainerTag {
    ...
    ChatCommandResult TryConsume(ChatHandler const* handler,
                                 std::string_view   args) const
    {
        ...
        if (StringEqualI(start, _string)) {
            auto [remainingToken, tail] = Acore::Impl::ChatCommands::tokenize(
                args.substr(_string.length()));
        ...
        return Acore::Impl::ChatCommands::FormatAcoreString(
            handler,
            LANG_CMDPARSER_EXACT_SEQ_MISMATCH,
            STRING_VIEW_FMT_ARG(_string),
            STRING_VIEW_FMT_ARG(start));
    }

private:
    static constexpr std::array<char, sizeof...(chars)> _storage = {chars...};
    static_assert(!_storage.empty() && (_storage.back() == '\0'),
                  "ExactSequence parameters must be null terminated! Use the "
                  "EXACT_SEQUENCE macro to make this easier!");
    static constexpr std::string_view _string = {
        _storage.data(),
        std::string_view::traits_type::length(_storage.data())};
};

#define EXACT_SEQUENCE(str)                                                    \
    Acore::ChatCommands::ExactSequence<CHATCOMMANDS_IMPL_SPLIT_LITERAL(str)>
...
struct WTail : std::wstring, Acore::Impl::ChatCommands::ContainerTag {
                                 ...
                                 std::string_view   args)
    {
        if (Utf8toWStr(args, *this))
            ...
            return Acore::Impl::ChatCommands::GetAcoreString(
                handler, LANG_CMDPARSER_INVALID_UTF8);
    }
};
...
struct QuotedString : std::string, Acore::Impl::ChatCommands::ContainerTag {
    ...
    AC_GAME_API ChatCommandResult TryConsume(ChatHandler const* handler,
                                             std::string_view   args);
};
...
struct Hyperlink : Acore::Impl::ChatCommands::ContainerTag {
    ...
    ChatCommandResult TryConsume(ChatHandler const* handler,
                                 std::string_view   args)
    {
        Acore::Hyperlinks::HyperlinkInfo info =
            Acore::Hyperlinks::ParseSingleHyperlink(args);
        ...
        if (!linktag::StoreTo(val, info.data))
            return Acore::Impl::ChatCommands::GetAcoreString(
                handler, LANG_CMDPARSER_LINKDATA_INVALID);

        // finally, skip any potential delimiters
        auto [token, next] = Acore::Impl::ChatCommands::tokenize(info.tail);
        if (token.empty()) /* empty token = first character is delimiter, skip
                              past it */
            return next;
        else
            return info.tail;
...
struct Variant : public std::variant<T1, Ts...> {
    ...
    static constexpr bool have_operators =
        Acore::Impl::ChatCommands::are_all_assignable<
            first_type,
            Acore::Impl::ChatCommands::tag_base_t<Ts>...>::value;
    ...
    template <bool C = have_operators>
    operator std::enable_if_t<C && !std::is_same_v<first_type, size_t> &&
                                  std::is_convertible_v<first_type, size_t>,
                              size_t>() const
    {
        return operator*();
    }
    ...
    {
        return std::get<index>(static_cast<base const&>(*this));
    }
    template <typename type>
    constexpr decltype(auto) get()
    {
        return std::get<type>(static_cast<base&>(*this));
    }
    template <typename type>
    constexpr decltype(auto) get() const
    {
        return std::get<type>(static_cast<base const&>(*this));
    }

    template <typename T>
    constexpr decltype(auto) visit(T&& arg)
    {
        return std::visit(std::forward<T>(arg), static_cast<base&>(*this));
    }
    template <typename T>
    ...
    {
        return std::holds_alternative<T>(static_cast<base const&>(*this));
    }

    template <bool C = have_operators>
    friend std::enable_if_t<C, std::ostream&>
