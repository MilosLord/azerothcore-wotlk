...
// forward declaration
// ConsumeFromOffset contains the bounds check for offset, then hands off to
// MultiConsumer the call stack is MultiConsumer -> ConsumeFromOffset ->
// MultiConsumer -> ConsumeFromOffset etc MultiConsumer goes into ArgInfo for
// parsing on each iteration
template <typename Tuple, size_t offset>
ChatCommandResult
ConsumeFromOffset(Tuple&, ChatHandler const* handler, std::string_view args);

template <typename Tuple, typename NextType, size_t offset>
struct MultiConsumer {
    static ChatCommandResult TryConsumeTo(Tuple&             tuple,
                                          ChatHandler const* handler,
                                          std::string_view   args)
    {
        ChatCommandResult next = ArgInfo<NextType>::TryConsume(
            std::get<offset>(tuple), handler, args);
        if (next)
...
struct MultiConsumer<Tuple, Optional<NestedNextType>, offset> {
                                          ...
                                          std::string_view   args)
    {
        ...
        ChatCommandResult result1 =
            ArgInfo<NestedNextType>::TryConsume(myArg.value(), handler, args);
        if (result1)
            if ((result1 = ConsumeFromOffset<Tuple, offset + 1>(
                     tuple, handler, *result1)))
                return result1;
        // try again omitting the argument
        myArg = std::nullopt;
        ChatCommandResult result2 =
            ConsumeFromOffset<Tuple, offset + 1>(tuple, handler, args);
        if (result2)
            return result2;
        if (result1.HasErrorMessage() && result2.HasErrorMessage()) {
            return Acore::StringFormatFmt(
                "{} \"{}\"\n{} \"{}\"",
                GetAcoreString(handler, LANG_CMDPARSER_EITHER),
                result2.GetErrorMessage(),
                GetAcoreString(handler, LANG_CMDPARSER_OR),
                result1.GetErrorMessage());
        }
...
template <typename Tuple, size_t offset>
ChatCommandResult ConsumeFromOffset([[maybe_unused]] Tuple&             tuple,
                                    [[maybe_unused]] ChatHandler const* handler,
                                    std::string_view                    args)
{
    if constexpr (offset < std::tuple_size_v<Tuple>)
        return MultiConsumer<Tuple,
                             std::tuple_element_t<offset, Tuple>,
                             offset>::TryConsumeTo(tuple, handler, args);
...
struct HandlerToTuple {
    static_assert(Acore::dependant_false_v<T>,
...
struct CommandInvoker {
    ...
    CommandInvoker(TypedHandler& handler)
    {
        _wrapper = [](void*            handler,
            ...
            ChatCommandResult result =
                ConsumeFromOffset<Tuple, 1>(arguments, chatHandler, argsStr);
            if (result)
                return std::apply(reinterpret_cast<TypedHandler*>(handler),
                                  std::move(arguments));
            else {
                if (result.HasErrorMessage())
                    SendErrorMessageToHandler(chatHandler,
    ...
    CommandInvoker(bool (&handler)(ChatHandler*, char const*))
    {
        _wrapper = [](void*            handler,
            ...
            return reinterpret_cast<bool (*)(ChatHandler*, char const*)>(
...
struct CommandPermissions {
    ...
    CommandPermissions(uint32                       securityLevel,
                       Acore::ChatCommands::Console console)
        : RequiredLevel{securityLevel}, AllowConsole{console}
    {
    }
...
public:
    ...
    GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd);

    ChatCommandNode()
        : _name{}, _invoker{}, _permission{}, _help{}, _subCommands{}
    {
    }

private:
    static std::
        map<std::string_view, ChatCommandNode, StringCompareLessI_T> const&
        GetTopLevelMap();
    static void
    LoadCommandsIntoMap(ChatCommandNode*                             blank,
    ...
    std::map<std::string_view, ChatCommandNode, StringCompareLessI_T>
        _subCommands;
};
} // namespace Acore::Impl::ChatCommands
...
struct ChatCommandBuilder {
    ...
    struct InvokerEntry {
        template <typename T>
        InvokerEntry(T&                           handler,
                     AcoreStrings                 help,
                     uint32                       securityLevel,
                     Acore::ChatCommands::Console allowConsole)
            : _invoker{handler}, _help{help},
              _permissions{securityLevel, allowConsole}
        {
        }
        ...
        auto operator*() const
        {
            return std::tie(_invoker, _help, _permissions);
        }
    };

    using SubCommandEntry =
        std::reference_wrapper<std::vector<ChatCommandBuilder> const>;
    ...
    ChatCommandBuilder(char const*                  name,
                       ...
                       uint32                       securityLevel,
                       Acore::ChatCommands::Console allowConsole)
        : _name{ASSERT_NOTNULL(name)}, _data{std::in_place_type<InvokerEntry>,
                                             handler,
                                             help,
                                             securityLevel,
                                             allowConsole}
    ...
    ChatCommandBuilder(char const*                  name,
                       ...
                       Acore::ChatCommands::Console allowConsole)
        : ChatCommandBuilder(
              name, handler, AcoreStrings(), securityLevel, allowConsole)
    {
    }

    ChatCommandBuilder(char const*                            name,
                       std::vector<ChatCommandBuilder> const& subCommands)
        : _name{ASSERT_NOTNULL(name)},
          _data{std::in_place_type<SubCommandEntry>, subCommands}
    {
    }

    [[deprecated("char const* parameters to command handlers are deprecated; "
                 "convert this to a typed argument handler "
                 "instead")]] ChatCommandBuilder(
        char const* name,
    ...
    template <typename TypedHandler>
    [[deprecated("you are using the old-style command format; convert this to "
                 "the new format ({ name, handler (not a pointer!), "
                 "permission, Console::(Yes/No) })")]] ChatCommandBuilder(
        char const*   name,
        uint32        securityLevel,
        bool          console,
        TypedHandler* handler,
        char const*)
        : ChatCommandBuilder(name,
                             *handler,
                             AcoreStrings(),
                             securityLevel,
                             static_cast<Acore::ChatCommands::Console>(console))
    {
    }

    [[deprecated("you are using the old-style command format; convert this to "
                 "the new format ({ name, subCommands })")]] ChatCommandBuilder(
        char const* name,
        uint32,
        bool,
        std::nullptr_t,
...
AC_GAME_API      std::vector<std::string>
GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd);
} // namespace Acore::ChatCommands

// backwards compatibility with old patches
using ChatCommand
    [[deprecated("std::vector<ChatCommand> should be ChatCommandTable! (using "
                 "namespace Acore::ChatCommands)")]] =
        Acore::ChatCommands::ChatCommandBuilder;
