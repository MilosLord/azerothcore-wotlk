...
*|
|*   - on match, returns tail of the provided argument string (as
std::string_view)     *|
|*   - on specific error, returns error message (as std::string&& or char
const*)       *|
...
struct ArgInfo {
    static_assert(Acore::dependant_false_v<T>,
                  "Invalid command parameter type - see ChatCommandArgs.h for "
                  "possible types");
};
...
struct ArgInfo<
    ...
    static ChatCommandResult
    TryConsume(T& val, ChatHandler const* handler, std::string_view args)
    {
...
struct ArgInfo<std::string_view, void> {
    static ChatCommandResult
    TryConsume(std::string_view& val, ChatHandler const*, std::string_view args)
    {
...
struct ArgInfo<std::string, void> {
    static ChatCommandResult TryConsume(std::string&       val,
                                        ChatHandler const* handler,
                                        std::string_view   args)
    {
        std::string_view  view;
        ChatCommandResult next =
            ArgInfo<std::string_view>::TryConsume(view, handler, args);
        if (next)
            val.assign(view);
        return next;
    }
...
struct ArgInfo<std::wstring, void> {
    static ChatCommandResult TryConsume(std::wstring&      val,
                                        ChatHandler const* handler,
                                        std::string_view   args)
    {
        std::string_view  utf8view;
        ChatCommandResult next =
            ArgInfo<std::string_view>::TryConsume(utf8view, handler, args);
...
struct ArgInfo<T, std::enable_if_t<std::is_enum_v<T>>> {
    using SearchMap =
        std::map<std::string_view, Optional<T>, StringCompareLessI_T>;
    ...
    static T const* Match(std::string_view s)
    {
        ...
        if (!StringEqualI(
                it->first,
                s)) // we don't have an exact match - check if it is unique
        {
            auto it2 = it;
            ++it2;
            if ((it2 != _map.end()) &&
                StringStartsWithI(it2->first, s)) // not unique
                return nullptr;
        }
    ...
    TryConsume(T& val, ChatHandler const* handler, std::string_view args)
    {
        ...
        ChatCommandResult next1 =
            ArgInfo<std::string_view>::TryConsume(strVal, handler, args);
        if (next1) {
        ...
        // Value not found. Try to parse arg as underlying type and cast it to
        // enum type
        using U = std::underlying_type_t<T>;
        U uVal  = 0;
        if (ChatCommandResult next2 =
                ArgInfo<U>::TryConsume(uVal, handler, args)) {
...
struct ArgInfo<T, std::enable_if_t<std::is_base_of_v<ContainerTag, T>>> {
    static ChatCommandResult
    TryConsume(T& tag, ChatHandler const* handler, std::string_view args)
    {
...
struct ArgInfo<std::vector<T>, void> {
    static ChatCommandResult TryConsume(std::vector<T>&    val,
                                        ChatHandler const* handler,
                                        std::string_view   args)
    {
        val.clear();
        ChatCommandResult next =
            ArgInfo<T>::TryConsume(val.emplace_back(), handler, args);

        if (!next)
            return next;

        while (ChatCommandResult next2 =
                   ArgInfo<T>::TryConsume(val.emplace_back(), handler, *next))
            next = std::move(next2);
...
struct ArgInfo<std::array<T, N>, void> {
    static ChatCommandResult TryConsume(std::array<T, N>&  val,
                                        ChatHandler const* handler,
                                        std::string_view   args)
    {
...
struct ArgInfo<Acore::ChatCommands::Variant<Ts...>> {
    ...
    template <size_t I>
    static ChatCommandResult
    TryAtIndex([[maybe_unused]] Acore::ChatCommands::Variant<Ts...>& val,
               [[maybe_unused]] ChatHandler const*                   handler,
               [[maybe_unused]] std::string_view                     args)
    {
        if constexpr (I < N) {
            ChatCommandResult thisResult =
                ArgInfo<std::variant_alternative_t<I, V>>::TryConsume(
                    val.template emplace<I>(), handler, args);
            if (thisResult)
                return thisResult;
            else {
                ChatCommandResult nestedResult =
                    TryAtIndex<I + 1>(val, handler, args);
                ...
                if (StringStartsWith(nestedResult.GetErrorMessage(), "\""))
                    return Acore::StringFormat(
                        "\"%s\"\n%s %s",
                        thisResult.GetErrorMessage().c_str(),
                        GetAcoreString(handler, LANG_CMDPARSER_OR),
                        nestedResult.GetErrorMessage().c_str());
                else
                    return Acore::StringFormat(
                        "\"%s\"\n%s \"%s\"",
                        thisResult.GetErrorMessage().c_str(),
    ...
    static ChatCommandResult
    TryConsume(Acore::ChatCommands::Variant<Ts...>& val,
               ChatHandler const*                   handler,
               std::string_view                     args)
    {
        ChatCommandResult result = TryAtIndex<0>(val, handler, args);
        if (result.HasErrorMessage() &&
            (result.GetErrorMessage().find('\n') != std::string::npos))
            return Acore::StringFormat(
                "%s %s",
...
struct AC_GAME_API ArgInfo<AchievementEntry const*> {
    static ChatCommandResult
    TryConsume(AchievementEntry const*&, ChatHandler const*, std::string_view);
};
...
struct AC_GAME_API ArgInfo<GameTele const*> {
    static ChatCommandResult
    TryConsume(GameTele const*&, ChatHandler const*, std::string_view);
};
...
struct AC_GAME_API ArgInfo<ItemTemplate const*> {
    static ChatCommandResult
    TryConsume(ItemTemplate const*&, ChatHandler const*, std::string_view);
};
...
struct AC_GAME_API ArgInfo<SpellInfo const*> {
    static ChatCommandResult
    TryConsume(SpellInfo const*&, ChatHandler const*, std::string_view);
};
...
struct AC_GAME_API ArgInfo<Quest const*> {
    static ChatCommandResult
    TryConsume(Quest const*&, ChatHandler const*, std::string_view);
};
} // namespace Acore::Impl::ChatCommands
