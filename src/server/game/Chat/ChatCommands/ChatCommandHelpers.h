...
inline TokenizeResult tokenize(std::string_view args)
{
    ...
    if (size_t delimPos = args.find(COMMAND_DELIMITER);
        delimPos != std::string_view::npos) {
        result.token = args.substr(0, delimPos);
        if (size_t tailPos =
                args.find_first_not_of(COMMAND_DELIMITER, delimPos);
            tailPos != std::string_view::npos)
...
// this essentially models std::optional<std::string_view>, except it can also
// hold an error message it has std::string_view's bool conversion and
// dereference operators
//
// monostate <-> unspecified error, typically end-of-string reached or parsing
// failed std::string <-> specified error, typically character-not-found or
// invalid item link std::string_view <-> success, string_view is remaining
// argument string
struct ChatCommandResult {
    ...
    ChatCommandResult(std::string&& s)
        : _storage(std::in_place_type<std::string>,
    ...
    {
        return std::get<std::string_view>(_storage);
    }
    bool IsSuccessful() const
    {
        return std::holds_alternative<std::string_view>(_storage);
    }
...
AC_GAME_API void        SendErrorMessageToHandler(ChatHandler*     handler,
                                                  std::string_view str);
AC_GAME_API char const* GetAcoreString(ChatHandler const* handler,
                                       AcoreStrings       which);
template <typename... Ts>
std::string
FormatAcoreString(ChatHandler const* handler, AcoreStrings which, Ts&&... args)
{
    return Acore::StringFormat(GetAcoreString(handler, which),
                               std::forward<Ts>(args)...);
}
} // namespace Acore::Impl::ChatCommands
