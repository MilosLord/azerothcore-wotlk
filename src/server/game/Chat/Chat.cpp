...
bool ChatHandler::IsAvailable(uint32 securityLevel) const
{
    ...
    return IsConsole()
               ? true
               : m_session->GetSecurity() >= AccountTypes(securityLevel);
}
                                          ...
                                          bool          strong)
{
    ...
    if (target)
        ...
        return true; // caller must report error for (target == nullptr &&
                     // target_account == 0)

    AccountTypes target_ac_sec = AccountTypes(target_sec);
    if (m_session->GetSecurity() < target_ac_sec ||
        (strong && m_session->GetSecurity() <= target_ac_sec)) {
                                    ...
                                    std::string const& channelName /*= ""*/)
{
    ...
    if (chatType == CHAT_MSG_ACHIEVEMENT ||
        chatType == CHAT_MSG_GUILD_ACHIEVEMENT)
...
Creature* ChatHandler::getSelectedCreature() const
{
    ...
    return ObjectAccessor::GetCreatureOrPetOrVehicle(
        *m_session->GetPlayer(), m_session->GetPlayer()->GetTarget());
}
                                      ...
                                      char**      something1)
{
    ...
    if (strcmp(cLinkType, linkType) != 0) {
        strtok(nullptr,
               " "); // skip link tail (to allow continue strtok(nullptr, s) use
                     // after retturn from function
    ...
    strtok(nullptr, " "); // skip link tail (to allow continue strtok(nullptr,
                          // s) use after return from function
    return cKey;
}
                                      ...
                                      char**             something1)
{
    ...
    // or
    // [name] Shift-click form
    // |color|linkType:key:something1:...:somethingN|h[name]|h|r or [name]
    // Shift-click form |linkType:key|h[name]|h|r
    ...
    for (int i = 0; linkTypes[i]; ++i) {
        if (strcmp(cLinkType, linkTypes[i]) == 0) {
            ...
            strtok(cKeysTail,
                   "]"); // restart scan tail and skip name with possible spaces
            strtok(nullptr,
                   " "); // skip link tail (to allow continue strtok(nullptr, s)
                         // use after return from function
            if (found_idx)
                *found_idx = i;
            return cKey;
        }
...
GameObject* ChatHandler::GetNearbyGameObject() const
{
    ...
    Acore::GameObjectLastSearcher<Acore::NearestGameObjectCheck> searcher(
        pl, obj, check);
...
Creature*
ChatHandler::GetCreatureFromPlayerMapByDbGuid(ObjectGuid::LowType lowguid)
{
    if (!m_session)
        return nullptr;
    ...
    auto bounds = m_session->GetPlayer()
                      ->GetMap()
...
GameObject*
ChatHandler::GetObjectFromPlayerMapByDbGuid(ObjectGuid::LowType lowguid)
{
    if (!m_session)
        return nullptr;
...
uint32 ChatHandler::extractSpellIdFromLink(char* text)
{
    ...
    // |color|Henchant:recipe_spell_id|h[prof_name: recipe_name]|h|r number or
    // [name] Shift-click form
    // |color|Hglyph:glyph_slot_id:glyph_prop_id|h[%s]|h|r number or [name]
    // Shift-click form |color|Hspell:spell_id|h[name]|h|r number or [name]
    // Shift-click form |color|Htalent:talent_id, rank|h[name]|h|r number or
    // [name] Shift-click form |color|Htrade:spell_id, skill_id, max_value,
    // cur_value|h[name]|h|r
    int   type       = 0;
    ...
    case SPELL_LINK_GLYPH: {
        ...
        GlyphPropertiesEntry const* glyphPropEntry =
            sGlyphPropertiesStore.LookupEntry(glyph_prop_id);
        if (!glyphPropEntry)
            return 0;
...
static char const* const guidKeys[] = {
    "Hplayer", "Hcreature", "Hgameobject", 0};
                                      ...
                                      std::string* player_name /*= nullptr*/)
{
    if (args && *args) {
        ...
        ObjectGuid guid = !pl && (player_guid || player_name)
...
bool CliHandler::needReportToTarget(Player* /*chr*/) const { return true; }
