...
public:
    ...
    // Builds chat packet and returns receiver guid position in the packet to
    // substitute in whisper builders
    static size_t BuildChatPacket(WorldPacket&       data,
                                  ChatMsg            chatType,
                                  Language           language,
    ...
    virtual void        SendSysMessage(std::string_view str,
    ...
    {
        SendSysMessage(
            Acore::StringFormat(fmt, std::forward<Args>(args)...).c_str());
    }
    ...
    {
        SendSysMessage(
            PGetParseString(entry, std::forward<Args>(args)...).c_str());
    }

    template <typename... Args>
    std::string PGetParseString(uint32 entry, Args&&... args) const
    {
        return Acore::StringFormat(GetAcoreString(entry),
    ...
    bool HasLowerSecurity(Player*    target,
                          ObjectGuid guid   = ObjectGuid::Empty,
                          bool       strong = false);
    bool HasLowerSecurityAccount(WorldSession* target,
                                 uint32        account,
                                 bool          strong = false);
    ...
    char* extractKeyFromLink(char*       text,
                             char const* linkType,
    ...
    bool                GetPlayerGroupAndGUIDByName(const char* cname,
                                                    Player*&    player,
                                                    Group*&     group,
                                                    ObjectGuid& guid,
                                                    bool        offline = false);
    ...
    bool extractPlayerTarget(char*        args,
                             Player**     player,
    ...
    {
        return m_session ? "|cffffffff|Hplayer:" + name + "|h[" + name + "]|h|r"
                         : name;
    }
...
protected:
    explicit ChatHandler()
        : m_session(nullptr), sentErrorMessage(false) {} // for CLI subclass

private:
    WorldSession* m_session; // != nullptr for chat command call and nullptr for
                             // CLI command
