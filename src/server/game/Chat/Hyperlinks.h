...
struct ItemLinkData {
    ...
    uint32 RandomSuffixBaseAmount; /* ITEM_FIELD_PROPERTY_SEED - only nonzero
                                      for RandomSuffix items, AllocationPct from
                                      DBC are multiplied with this, then
                                      floored, to get stat value */
    uint8 RenderLevel;
...
|*   - returns identifier string for the link ("creature", "creature_entry",
"item")    *|
...
#define make_base_tag(ltag, type)                                              \
    struct ltag : public base_tag {                                            \
        using value_type = type;                                               \
        static constexpr std::string_view tag() { return #ltag; }              \
    }
...
struct HyperlinkInfo {
    ...
    HyperlinkInfo(std::string_view t,
                  uint32           c,
                  std::string_view ta,
                  std::string_view d,
                  std::string_view te)
