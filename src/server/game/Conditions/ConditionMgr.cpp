...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
    ...
    if (ConditionTarget >= MAX_CONDITION_TARGETS) {
        LOG_ERROR("condition",
                  "ConditionTarget {} for for condition (Entry: {} Type: {} "
                  "Group: {}) is greater or equal than MAX_CONDITION_TARGETS",
                  ConditionTarget,
                  SourceEntry,
                  SourceType,
                  SourceGroup);
    ...
    if (!object) {
        LOG_DEBUG("condition",
                  "Condition object not found for condition (Entry: {} Type: "
                  "{} Group: {})",
                  SourceEntry,
                  SourceType,
                  SourceGroup);
    ...
    case CONDITION_ITEM: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                ...
                condMeets      = player->HasItemCount(
                    ConditionValue1, ConditionValue2, checkBank);
            }
        }
        break;
    }
    case CONDITION_ITEM_EQUIPPED: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                    unit->GetCharmerOrOwnerPlayerOrPlayerItself()) {
                condMeets =
                    player->HasItemOrGemWithIdEquipped(ConditionValue1, 1);
            }
        }
        break;
    }
        ...
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                ...
                if (FactionEntry const* faction =
                    ...
                    condMeets =
                        (ConditionValue2 &
                         (1 << player->GetReputationMgr().GetRank(faction)));
                }
            }
        }
        break;
    ...
    case CONDITION_SKILL: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                    unit->GetCharmerOrOwnerPlayerOrPlayerItself()) {
                condMeets = player->HasSkill(ConditionValue1) &&
                            player->GetBaseSkillValue(ConditionValue1) >=
                                ConditionValue2;
            }
        }
        break;
        ...
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                ...
                condMeets          = (status == QUEST_STATUS_COMPLETE &&
                             !player->GetQuestRewardStatus(ConditionValue1));
            }
        }
        break;
    }
    ...
    case CONDITION_QUEST_SATISFY_EXCLUSIVE: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                ...
                const Quest* quest =
                    sObjectMgr->GetQuestTemplate(ConditionValue1);
    ...
    case CONDITION_INSTANCE_INFO: {
        ...
        if (map->IsDungeon()) {
            if (InstanceScript const* instance =
                    map->ToInstanceMap()->GetInstanceScript()) {
                switch (ConditionValue3) {
                case INSTANCE_INFO_DATA:
                    condMeets =
                        instance->GetData(ConditionValue1) == ConditionValue2;
                    break;
                case INSTANCE_INFO_GUID_DATA:
                    condMeets = instance->GetGuidData(ConditionValue1) ==
                                ObjectGuid(uint64(ConditionValue2));
                    break;
                ...
                case INSTANCE_INFO_DATA64:
                    condMeets =
                        instance->GetData64(ConditionValue1) == ConditionValue2;
                    break;
                }
            }
        }
        ...
        if (Unit* unit = object->ToUnit())
            condMeets =
                CompareValues(static_cast<ComparisionType>(ConditionValue2),
                              static_cast<uint32>(unit->GetLevel()),
                              ConditionValue1);
        break;
    }
    case CONDITION_DRUNKENSTATE: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                    unit->GetCharmerOrOwnerPlayerOrPlayerItself()) {
                condMeets = (uint32)Player::GetDrunkenstateByValue(
                                player->GetDrunkValue()) >= ConditionValue1;
            }
        }
        break;
    }
    case CONDITION_NEAR_CREATURE: {
        condMeets = static_cast<bool>(
            GetClosestCreatureWithEntry(object,
                                        ConditionValue1,
    ...
    case CONDITION_NEAR_GAMEOBJECT: {
        if (!ConditionValue3) {
            condMeets = static_cast<bool>(GetClosestGameObjectWithEntry(
                object, ConditionValue1, static_cast<float>(ConditionValue2)));
            break;
        }
        else {
            if (GameObject* go = GetClosestGameObjectWithEntry(
                    object,
                    ConditionValue1,
                    static_cast<float>(ConditionValue2))) {
    ...
    case CONDITION_OBJECT_ENTRY_GUID: {
        if (ConditionValue3 == 1 &&
            object->ToUnit()) // pussywizard: if == 1, ignore not
                              // attackable/selectable targets
            if (object->ToUnit()->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
        ...
        if (uint32(object->GetTypeId()) == ConditionValue1) {
            ...
            if (ConditionValue3 > 1) {
                ...
                case TYPEID_UNIT:
                    condMeets &=
                        object->ToCreature()->GetSpawnId() == ConditionValue3;
                    break;
                case TYPEID_GAMEOBJECT:
                    condMeets &=
                        object->ToGameObject()->GetSpawnId() == ConditionValue3;
                    break;
                default:
                    break;
                }
    ...
    case CONDITION_CREATURE_TYPE: {
        if (Creature* creature = object->ToCreature())
            condMeets =
                creature->GetCreatureTemplate()->type == ConditionValue1;
        break;
    }
    case CONDITION_REALM_ACHIEVEMENT: {
        AchievementEntry const* achievement =
            sAchievementStore.LookupEntry(ConditionValue1);
    ...
    case CONDITION_QUESTSTATE: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                    unit->GetCharmerOrOwnerPlayerOrPlayerItself()) {
                uint32 queststateConditionValue1 =
                    player->GetQuestStatus(ConditionValue1);
                if (((ConditionValue2 & (1 << QUEST_STATUS_NONE)) &&
                     (queststateConditionValue1 == QUEST_STATUS_NONE)) ||
                    ((ConditionValue2 & (1 << QUEST_STATUS_COMPLETE)) &&
                     (queststateConditionValue1 == QUEST_STATUS_COMPLETE)) ||
                    ((ConditionValue2 & (1 << QUEST_STATUS_INCOMPLETE)) &&
                     (queststateConditionValue1 == QUEST_STATUS_INCOMPLETE)) ||
                    ((ConditionValue2 & (1 << QUEST_STATUS_FAILED)) &&
                     (queststateConditionValue1 == QUEST_STATUS_FAILED)) ||
                    ((ConditionValue2 & (1 << QUEST_STATUS_REWARDED)) &&
                     player->GetQuestRewardStatus(ConditionValue1))) {
                    condMeets = true;
                }
            }
    ...
    case CONDITION_QUEST_OBJECTIVE_PROGRESS: {
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                    unit->GetCharmerOrOwnerPlayerOrPlayerItself()) {
                Quest const* quest = ASSERT_NOTNULL(
                    sObjectMgr->GetQuestTemplate(ConditionValue1));
                ...
                if (player->GetQuestSlotCounter(log_slot, ConditionValue2) ==
                    ConditionValue3) {
                    condMeets = true;
                }
            }
        ...
        if (Unit* unit = object->ToUnit()) {
            if (Player* player =
                ...
                if (Pet* pet = player->GetPet()) {
                    condMeets =
                        (((1 << pet->getPetType()) & ConditionValue1) != 0);
                }
            }
        }
        break;
    ...
    // bool script = sScriptMgr->OnConditionCheck(this, sourceInfo); // Returns
    // true by default. // pussywizard: optimization
...
uint32 Condition::GetSearcherTypeMaskForCondition()
{
    ...
    case CONDITION_CHARMED:
        ...
        break;
    default:
        ASSERT(false && "Condition::GetSearcherTypeMaskForCondition - missing "
                        "condition handling!");
        break;
    }
    return mask;
...
ConditionMgr::~ConditionMgr() { Clean(); }
...
ConditionList ConditionMgr::GetConditionReferences(uint32 refId)
{
    ...
    ConditionReferenceContainer::const_iterator ref =
        ConditionReferenceStore.find(refId);
    ...
    ConditionList const& conditions)
{
    ...
    for (ConditionList::const_iterator i = conditions.begin();
         i != conditions.end();
         ++i) {
        // no point of having not loaded conditions in list
        ASSERT((*i)->isLoaded() &&
               "ConditionMgr::GetSearcherTypeMaskForConditionList - not yet "
               "loaded condition found in list");
        std::map<uint32, uint32>::const_iterator itr =
            ElseGroupStore.find((*i)->ElseGroup);
        ...
        {
            ConditionReferenceContainer::const_iterator ref =
                ConditionReferenceStore.find((*i)->ReferenceId);
            ASSERT(ref != ConditionReferenceStore.end() &&
                   "ConditionMgr::GetSearcherTypeMaskForConditionList - "
                   "incorrect reference");
            ElseGroupStore[(*i)->ElseGroup] &=
                GetSearcherTypeMaskForConditionList((*ref).second);
        }
        else // handle normal condition
        {
            // object will match conditions in one ElseGroupStore only when it
            // matches all of them so, let's find a smallest possible mask which
            // satisfies all conditions
            ElseGroupStore[(*i)->ElseGroup] &=
                (*i)->GetSearcherTypeMaskForCondition();
        }
    }
    // object will match condition when one of the checks in ElseGroupStore is
    // matching so, let's include all possible masks
    uint32 mask = 0;
    for (std::map<uint32, uint32>::const_iterator i = ElseGroupStore.begin();
         i != ElseGroupStore.end();
         ++i)
        mask |= i->second;
                                               ...
                                               ConditionList const& conditions)
{
    ...
    for (ConditionList::const_iterator i = conditions.begin();
        ...
        LOG_DEBUG(
            "condition",
            "ConditionMgr::IsPlayerMeetToConditionList condType: {} val1: {}",
            (*i)->ConditionType,
            (*i)->ConditionValue1);
        if ((*i)->isLoaded()) {
            ...
            if ((*i)->ReferenceId) // handle reference
            {
                ...
                if (ref != ConditionReferenceStore.end()) {
                    ...
                    LOG_DEBUG("condition",
                              "IsPlayerMeetToConditionList: Reference template "
                              "-{} not found",
                              (*i)->ReferenceId);
                }
            }
...
bool ConditionMgr::CanHaveSourceGroupSet(ConditionSourceType sourceType) const
{
    return (sourceType == CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE ||
            ...
            sourceType == CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE);
}
...
ConditionList
ConditionMgr::GetConditionsForNotGroupedEntry(ConditionSourceType sourceType,
                                              uint32              entry)
{
    ConditionList spellCond;
    if (sourceType > CONDITION_SOURCE_TYPE_NONE &&
        sourceType < CONDITION_SOURCE_TYPE_MAX) {
        ConditionContainer::const_iterator itr =
            ConditionStore.find(sourceType);
        if (itr != ConditionStore.end()) {
            ConditionTypeContainer::const_iterator i =
                (*itr).second.find(entry);
            if (i != (*itr).second.end()) {
                spellCond = (*i).second;
                LOG_DEBUG("condition",
                          "GetConditionsForNotGroupedEntry: found conditions "
                          "for type {} and entry {}",
                          uint32(sourceType),
                          entry);
            }
...
{
    ConditionList                                   cond;
    CreatureSpellConditionContainer::const_iterator itr =
        SpellClickEventConditionStore.find(creatureId);
    if (itr != SpellClickEventConditionStore.end()) {
        ConditionTypeContainer::const_iterator i = (*itr).second.find(spellId);
        if (i != (*itr).second.end()) {
            cond = (*i).second;
            LOG_DEBUG("condition",
                      "GetConditionsForSpellClickEvent: found conditions for "
                      "Vehicle entry {} spell {}",
                      creatureId,
                      spellId);
        }
                                                         ...
                                                         uint32 spellId)
{
    ConditionList                                   cond;
    CreatureSpellConditionContainer::const_iterator itr =
        VehicleSpellConditionStore.find(creatureId);
    if (itr != VehicleSpellConditionStore.end()) {
        ConditionTypeContainer::const_iterator i = (*itr).second.find(spellId);
        if (i != (*itr).second.end()) {
            cond = (*i).second;
            LOG_DEBUG("condition",
                      "GetConditionsForVehicleSpell: found conditions for "
                      "Vehicle entry {} spell {}",
                      creatureId,
                      spellId);
        }
...
{
    ConditionList                                cond;
    SmartEventConditionContainer::const_iterator itr =
        SmartEventConditionStore.find(std::make_pair(entryOrGuid, sourceType));
    if (itr != SmartEventConditionStore.end()) {
        ConditionTypeContainer::const_iterator i =
            (*itr).second.find(eventId + 1);
        if (i != (*itr).second.end()) {
            cond = (*i).second;
            LOG_DEBUG("condition",
                      "GetConditionsForSmartEvent: found conditions for Smart "
                      "Event entry or guid {} event_id {}",
                      entryOrGuid,
                      eventId);
        }
    }
...
{
    ConditionList                               cond;
    NpcVendorConditionContainer::const_iterator itr =
        NpcVendorConditionContainerStore.find(creatureId);
    if (itr != NpcVendorConditionContainerStore.end()) {
        ...
        if (i != (*itr).second.end()) {
            ...
            if (itemId) {
                LOG_DEBUG("condition",
                          "GetConditionsForNpcVendorEvent: found conditions "
                          "for creature entry {} item {}",
                          creatureId,
                          itemId);
            }
            else {
...
void ConditionMgr::LoadConditions(bool isReload)
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "NegativeCondition, ErrorType, ErrorTextId, ScriptName FROM "
        "conditions");
    ...
    do {
        ...
        if (iConditionTypeOrReference < 0) // it has a reference
        {
            ...
            {
                LOG_ERROR("sql.sql",
                          "Condition reference {} is referencing self, skipped",
            ...
            if (cond->ConditionTarget)
                LOG_ERROR(
                    "sql.sql",
                    "Condition {} {} has useless data in ConditionTarget ({})!",
                    rowType,
                    iSourceTypeOrReferenceId,
                    cond->ConditionTarget);
            if (cond->ConditionValue1)
                LOG_ERROR("sql.sql",
                          "Condition {} {} has useless data in value1 ({})!",
                          rowType,
                          iSourceTypeOrReferenceId,
                          cond->ConditionValue1);
            if (cond->ConditionValue2)
                LOG_ERROR("sql.sql",
                          "Condition {} {} has useless data in value2 ({})!",
                          rowType,
                          iSourceTypeOrReferenceId,
                          cond->ConditionValue2);
            if (cond->ConditionValue3)
                LOG_ERROR("sql.sql",
                          "Condition {} {} has useless data in value3 ({})!",
                          rowType,
                          iSourceTypeOrReferenceId,
                          cond->ConditionValue3);
            if (cond->NegativeCondition)
                LOG_ERROR("sql.sql",
                          "Condition {} {} has useless data in "
                          "NegativeCondition ({})!",
                          rowType,
                          iSourceTypeOrReferenceId,
                          cond->NegativeCondition);
            if (cond->SourceGroup && iSourceTypeOrReferenceId < 0)
                LOG_ERROR(
                    "sql.sql",
                    "Condition {} {} has useless data in SourceGroup ({})!",
                    rowType,
                    iSourceTypeOrReferenceId,
                    cond->SourceGroup);
            if (cond->SourceEntry && iSourceTypeOrReferenceId < 0)
                LOG_ERROR(
                    "sql.sql",
                    "Condition {} {} has useless data in SourceEntry ({})!",
                    rowType,
                    iSourceTypeOrReferenceId,
                    cond->SourceEntry);
        }
        ...
        if (iSourceTypeOrReferenceId < 0) // it is a reference template
        {
            ...
            if (ConditionReferenceStore.find(uRefId) ==
                ConditionReferenceStore
                    .end()) // make sure we have a list for our conditions,
        ...
        if (cond->SourceGroup && !CanHaveSourceGroupSet(cond->SourceType)) {
            LOG_ERROR(
                "sql.sql",
                "Condition type {} has not allowed value of SourceGroup = {}!",
        ...
        if (cond->SourceId && !CanHaveSourceIdSet(cond->SourceType)) {
            LOG_ERROR(
                "sql.sql",
                "Condition type {} has not allowed value of SourceId = {}!",
                uint32(cond->SourceType),
                cond->SourceId);
            delete cond;
            continue;
        ...
        if (cond->ErrorType &&
            cond->SourceType != CONDITION_SOURCE_TYPE_SPELL) {
            LOG_ERROR("condition",
                      "Condition type {} entry {} can't have ErrorType ({}), "
                      "set to 0!",
                      uint32(cond->SourceType),
                      cond->SourceEntry,
                      cond->ErrorType);
        ...
        if (cond->ErrorTextId && !cond->ErrorType) {
            LOG_ERROR("condition",
                      "Condition type {} entry {} has any ErrorType, "
                      "ErrorTextId ({}) is set, set to 0!",
                      uint32(cond->SourceType),
                      cond->SourceEntry,
                      cond->ErrorTextId);
            cond->ErrorTextId = 0;
        ...
        if (cond->SourceGroup ||
            cond->SourceType == CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE) {
            bool valid = false;
            // handle grouped conditions
            switch (cond->SourceType) {
            case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Creature.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Disenchant.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_FISHING_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Fishing.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Gameobject.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_ITEM_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Item.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_MAIL_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Mail.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_MILLING_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Milling.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Pickpocketing.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Prospecting.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_REFERENCE_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Reference.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_SKINNING_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Skinning.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE:
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Spell.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            case CONDITION_SOURCE_TYPE_GOSSIP_MENU:
            ...
            case CONDITION_SOURCE_TYPE_GOSSIP_MENU_OPTION:
                ...
                break;
            case CONDITION_SOURCE_TYPE_SPELL_CLICK_EVENT: {
                SpellClickEventConditionStore[cond->SourceGroup]
                                             [cond->SourceEntry]
                                                 .push_back(cond);
                valid = true;
                ++count;
                continue; // do not add to m_AllocatedMemory to avoid double
                          // deleting
            }
            case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
                ...
                break;
            case CONDITION_SOURCE_TYPE_VEHICLE_SPELL: {
                VehicleSpellConditionStore[cond->SourceGroup][cond->SourceEntry]
                    .push_back(cond);
                valid = true;
                ++count;
                continue; // do not add to m_AllocatedMemory to avoid double
            ...
            case CONDITION_SOURCE_TYPE_SMART_EVENT: {
                ...
                SmartEventConditionStore[key][cond->SourceGroup].push_back(
                    cond);
                valid = true;
                ++count;
                continue;
            ...
            case CONDITION_SOURCE_TYPE_NPC_VENDOR: {
                NpcVendorConditionContainerStore[cond->SourceGroup]
                                                [cond->SourceEntry]
                                                    .push_back(cond);
                valid = true;
                ++count;
            ...
            case CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE: {
                valid = addToLootTemplate(
                    cond,
                    LootTemplates_Player.GetLootForConditionFill(
                        cond->SourceGroup));
                break;
            }
            default:
            ...
            if (!valid) {
                LOG_ERROR("sql.sql",
                          "Not handled grouped condition, SourceGroup {}",
                          cond->SourceGroup);
                delete cond;
            }
            else {
...
bool ConditionMgr::addToLootTemplate(Condition* cond, LootTemplate* loot)
{
    if (!loot) {
        LOG_ERROR("sql.sql",
                  "ConditionMgr: LootTemplate {} not found",
                  cond->SourceGroup);
        return false;
    }
    ...
    LOG_ERROR("sql.sql",
              "ConditionMgr: Item {} not found in LootTemplate {}",
              cond->SourceEntry,
              cond->SourceGroup);
    return false;
}
...
{
    GossipMenusMapBoundsNonConst pMenuBounds =
        sObjectMgr->GetGossipMenusMapBoundsNonConst(cond->SourceGroup);

    if (pMenuBounds.first != pMenuBounds.second) {
        for (GossipMenusContainer::iterator itr = pMenuBounds.first;
             itr != pMenuBounds.second;
             ++itr) {
            if ((*itr).second.MenuID == cond->SourceGroup &&
                (*itr).second.TextID == uint32(cond->SourceEntry)) {
    ...
    LOG_ERROR("sql.sql",
              "addToGossipMenus: GossipMenu {} not found",
              cond->SourceGroup);
    return false;
}
...
{
    GossipMenuItemsMapBoundsNonConst pMenuItemBounds =
        sObjectMgr->GetGossipMenuItemsMapBoundsNonConst(cond->SourceGroup);
    if (pMenuItemBounds.first != pMenuItemBounds.second) {
        for (GossipMenuItemsContainer::iterator itr = pMenuItemBounds.first;
             itr != pMenuItemBounds.second;
             ++itr) {
            if ((*itr).second.MenuID == cond->SourceGroup &&
                (*itr).second.OptionID == uint32(cond->SourceEntry)) {
                (*itr).second.Conditions.push_back(cond);
                return true;
            }
        }
    ...
    LOG_ERROR("sql.sql",
              "addToGossipMenuItems: GossipMenuId {} Item {} not found",
              cond->SourceGroup,
              cond->SourceEntry);
    return false;
}
...
bool ConditionMgr::addToSpellImplicitTargetConditions(Condition* cond)
{
    ...
    SpellInfo* spellInfo =
        const_cast<SpellInfo*>(sSpellMgr->AssertSpellInfo(cond->SourceEntry));
    std::list<uint32> sharedMasks;
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        // check if effect is already a part of some shared mask
        bool found = false;
        for (std::list<uint32>::iterator itr = sharedMasks.begin();
             itr != sharedMasks.end();
             ++itr) {
            if ((1 << i) & *itr) {
                found = true;
    ...
    for (std::list<uint32>::iterator itr = sharedMasks.begin();
        ...
        if (uint32 commonMask = *itr & conditionEffMask) {
            ...
            // get shared data
            ConditionList* sharedList =
                spellInfo->Effects[firstEffIndex].ImplicitTargetConditions;
            ...
            if (sharedList) {
                ...
                if (conditionEffMask != *itr) {
                    LOG_ERROR(
                        "sql.sql",
                        "SourceEntry {} in `condition` table, has incorrect "
                        "SourceGroup {} (spell effectMask) set - "
                        "effect masks are overlapping (all SourceGroup values "
                        "having given bit set must be equal) - ignoring.",
                        cond->SourceEntry,
                        cond->SourceGroup);
                    return false;
                }
            ...
            // no data for shared mask, we can create new submask
                ...
                for (uint8 i = firstEffIndex; i < MAX_SPELL_EFFECTS; ++i) {
                    if ((1 << i) & commonMask) {
                        spellInfo->Effects[i].ImplicitTargetConditions =
                            sharedList;
                        assigned = true;
                    }
                }
...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
    if (cond->SourceType == CONDITION_SOURCE_TYPE_NONE ||
        ...
        LOG_ERROR(
            "sql.sql",
            "Invalid ConditionSourceType {} in `condition` table, ignoring.",
            uint32(cond->SourceType));
        return false;
    }
    ...
    case CONDITION_SOURCE_TYPE_GRAVEYARD: {
        LOG_ERROR("sql.sql",
                  "ConditionSourceType {} in `condition` table is not "
                  "supported on 3.3.5a, ignoring.",
                  uint32(cond->SourceType));
        return false;
    }
    case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE: {
        if (!LootTemplates_Creature.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`creature_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Creature.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
                      "SourceType {}, SourceEntry {} in `condition` table, "
                      "does not exist in `item_template`, ignoring.",
                      cond->SourceType,
                      cond->SourceEntry);
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE: {
        if (!LootTemplates_Disenchant.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`disenchant_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Disenchant.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_FISHING_LOOT_TEMPLATE: {
        if (!LootTemplates_Fishing.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`fishing_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Fishing.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE: {
        if (!LootTemplates_Gameobject.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`gameobject_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Gameobject.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_ITEM_LOOT_TEMPLATE: {
        if (!LootTemplates_Item.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`item_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Item.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_MAIL_LOOT_TEMPLATE: {
        if (!LootTemplates_Mail.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`mail_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Mail.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_MILLING_LOOT_TEMPLATE: {
        if (!LootTemplates_Milling.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`milling_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Milling.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE: {
        if (!LootTemplates_Pickpocketing.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`pickpocketing_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE: {
        if (!LootTemplates_Prospecting.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`prospecting_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot = LootTemplates_Prospecting.GetLootForConditionFill(
            cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
    ...
    case CONDITION_SOURCE_TYPE_REFERENCE_LOOT_TEMPLATE: {
        if (!LootTemplates_Reference.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`reference_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Reference.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_SKINNING_LOOT_TEMPLATE: {
        if (!LootTemplates_Skinning.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`skinning_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Skinning.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE: {
        if (!LootTemplates_Spell.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`spell_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Spell.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
    ...
    case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET: {
        ...
        if (!spellInfo) {
            LOG_ERROR("sql.sql",
                      "SourceEntry {} in `condition` table, does not exist in "
                      "`spell.dbc`, ignoring.",
                      cond->SourceEntry);
            return false;
        }
        ...
        if ((cond->SourceGroup > MAX_EFFECT_MASK) || !cond->SourceGroup) {
            LOG_ERROR("sql.sql",
                      "SourceEntry {} in `condition` table, has incorrect "
                      "SourceGroup {} (spell effectMask) set , ignoring.",
                      cond->SourceEntry,
                      cond->SourceGroup);
            return false;
        }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            ...
            // Xinef: chain targets are treated as area targets! Apply
            // conditions!
            if (spellInfo->Effects[i].ChainTarget > 1)
                continue;

            LOG_ERROR("sql.sql",
                      "SourceEntry {} SourceGroup {} in `condition` table - "
                      "spell {} does not have implicit targets of types: "
                      "_AREA_, _CONE_, _NEARBY_ for effect {}, SourceGroup "
                      "needs correction, ignoring.",
                      cond->SourceEntry,
                      origGroup,
                      cond->SourceEntry,
                      uint32(i));
    ...
    case CONDITION_SOURCE_TYPE_SPELL_PROC: {
        SpellInfo const* spellProto =
            sSpellMgr->GetSpellInfo(cond->SourceEntry);
        if (!spellProto) {
            LOG_ERROR("sql.sql",
                      "SourceEntry {} in `condition` table, does not exist in "
                      "`spell.dbc`, ignoring.",
    ...
    case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
        if (!sObjectMgr->GetQuestTemplate(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
                      "CONDITION_SOURCE_TYPE_QUEST_AVAILABLE specifies "
                      "non-existing quest ({}), skipped",
                      cond->SourceEntry);
            return false;
        }
        break;
    case CONDITION_SOURCE_TYPE_UNUSED_20:
        LOG_ERROR("sql.sql",
                  "CONDITION_SOURCE_TYPE_UNUSED_20 is not in use. SourceEntry "
                  "= ({}), skipped",
                  cond->SourceEntry);
        break;
    case CONDITION_SOURCE_TYPE_VEHICLE_SPELL:
    ...
    case CONDITION_SOURCE_TYPE_NPC_VENDOR: {
        ...
        if (cond->SourceEntry) {
            ...
            if (!itemTemplate) {
                LOG_ERROR("condition",
                          "SourceEntry {} in `condition` table, does not exist "
    ...
    case CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE: {
        if (!LootTemplates_Player.HaveLootFor(cond->SourceGroup)) {
            LOG_ERROR("sql.sql",
                      "SourceGroup {} in `condition` table, does not exist in "
                      "`player_loot_template`, ignoring.",
                      cond->SourceGroup);
            return false;
        }

        LootTemplate* loot =
            LootTemplates_Player.GetLootForConditionFill(cond->SourceGroup);
        ItemTemplate const* pItemProto =
            sObjectMgr->GetItemTemplate(cond->SourceEntry);
        if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
            LOG_ERROR("sql.sql",
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    if (cond->ConditionType == CONDITION_NONE ||
        (cond->ConditionType >= CONDITION_TC_END &&
         cond->ConditionType <= CONDITION_AC_START) ||
        (cond->ConditionType >= CONDITION_AC_END)) {
        LOG_ERROR("sql.sql",
                  "SourceEntry {} in `condition` table has an invalid "
                  "ConditionType ({}), ignoring.",
                  cond->SourceEntry,
                  uint32(cond->ConditionType));
        return false;
    }
    switch (cond->ConditionType) {
    case CONDITION_TERRAIN_SWAP:
        LOG_ERROR("sql.sql",
                  "SourceEntry {} in `condition` table has a ConditionType "
                  "that is not supported on 3.3.5a ({}), ignoring.",
                  cond->SourceEntry,
                  uint32(cond->ConditionType));
        return false;
    default:
    ...
    if (cond->ConditionTarget >= cond->GetMaxAvailableConditionTargets()) {
        LOG_ERROR("sql.sql",
                  "SourceType {}, SourceEntry {} in `condition` table, has "
    ...
    case CONDITION_AURA: {
        if (!sSpellMgr->GetSpellInfo(cond->ConditionValue1)) {
            LOG_ERROR("sql.sql",
                      "Aura condition has non existing spell (Id: {}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2 > EFFECT_2) {
            LOG_ERROR("sql.sql",
                      "Aura condition has non existing effect index ({}) (must "
                      "be 0..2), skipped",
                      cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Aura condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_ITEM: {
        ItemTemplate const* proto =
            sObjectMgr->GetItemTemplate(cond->ConditionValue1);
        if (!proto) {
            LOG_ERROR("sql.sql",
                      "Item condition has non existing item ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (!cond->ConditionValue2) {
            LOG_ERROR("sql.sql",
                      "Item condition has 0 set for item count in value2 ({}), "
                      "skipped",
                      cond->ConditionValue2);
            return false;
        }
    ...
    case CONDITION_ITEM_EQUIPPED: {
        ...
        if (!proto) {
            LOG_ERROR(
                "sql.sql",
                "ItemEquipped condition has non existing item ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "ItemEquipped condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "ItemEquipped condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_ZONEID: {
        AreaTableEntry const* areaEntry =
            sAreaTableStore.LookupEntry(cond->ConditionValue1);
        if (!areaEntry) {
            LOG_ERROR("sql.sql",
                      "ZoneID condition has non existing area ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (areaEntry->zone != 0) {
            LOG_ERROR("sql.sql",
                      "ZoneID condition requires to be in area ({}) which is a "
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "ZoneID condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "ZoneID condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_REPUTATION_RANK: {
        ...
        if (!factionEntry) {
            LOG_ERROR(
                "sql.sql",
                "Reputation condition has non existing faction ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Reputation condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_TEAM: {
        if (cond->ConditionValue1 != ALLIANCE &&
            cond->ConditionValue1 != HORDE) {
            LOG_ERROR("sql.sql",
                      "Team condition specifies unknown team ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Team condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Team condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_SKILL: {
        ...
        if (!pSkill) {
            LOG_ERROR(
                "sql.sql",
                "Skill condition specifies non-existing skill ({}), skipped",
                cond->ConditionValue1);
            return false;
        }

        if (cond->ConditionValue2 < 1 ||
            cond->ConditionValue2 > sWorld->GetConfigMaxSkillValue()) {
            LOG_ERROR(
                "sql.sql",
                "Skill condition specifies invalid skill value ({}), skipped",
                cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Skill condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_QUESTREWARDED:
    ...
    case CONDITION_QUEST_SATISFY_EXCLUSIVE: {
        if (!sObjectMgr->GetQuestTemplate(cond->ConditionValue1)) {
            LOG_ERROR(
                "sql.sql",
                "Quest condition specifies non-existing quest ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2 > 1) {
            LOG_ERROR("sql.sql",
                      "Quest condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        }
        if (cond->ConditionValue3) {
            LOG_ERROR("sql.sql",
                      "Quest condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        }
        break;
    }
    case CONDITION_QUESTSTATE:
        if (cond->ConditionValue2 >= (1 << MAX_QUEST_STATUS)) {
            LOG_ERROR(
                "sql.sql",
                "ConditionType ({}) has invalid state mask ({}), skipped.",
                cond->ConditionType,
                cond->ConditionValue2);
            return false;
        }
    ...
    case CONDITION_ACTIVE_EVENT: {
        ...
        if (cond->ConditionValue1 >= events.size() ||
            ...
            LOG_ERROR(
                "sql.sql",
                "ActiveEvent condition has non existing event id ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "ActiveEvent condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "ActiveEvent condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_ACHIEVEMENT: {
        ...
        if (!achievement) {
            LOG_ERROR("sql.sql",
                      "Achivement condition has non existing achivement id "
                      "({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Achivement condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Achivement condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_CLASS: {
        if (!(cond->ConditionValue1 & CLASSMASK_ALL_PLAYABLE)) {
            LOG_ERROR(
                "sql.sql",
                "Class condition has non existing classmask ({}), skipped",
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Class condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Class condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_RACE: {
        if (!(cond->ConditionValue1 & RACEMASK_ALL_PLAYABLE)) {
            LOG_ERROR("sql.sql",
                      "Race condition has non existing racemask ({}), skipped",
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Race condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Race condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_GENDER: {
        if (!Player::IsValidGender(uint8(cond->ConditionValue1))) {
            LOG_ERROR("condition",
                      "Gender condition has invalid gender ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("condition",
                      "Gender condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("condition",
                      "Gender condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_MAPID: {
        ...
        if (!mapId) {
            LOG_ERROR("sql.sql",
                      "Map condition has non existing map ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Map condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Map condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_SPELL: {
        if (!sSpellMgr->GetSpellInfo(cond->ConditionValue1)) {
            LOG_ERROR(
                "sql.sql",
                "Spell condition has non existing spell (Id: {}), skipped",
                cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Spell condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Spell condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_LEVEL: {
        if (cond->ConditionValue2 >= COMP_TYPE_MAX) {
            LOG_ERROR("sql.sql",
                      "Level condition has invalid option ({}), skipped",
                      cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Level condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_DRUNKENSTATE: {
        if (cond->ConditionValue1 > DRUNKEN_SMASHED) {
            LOG_ERROR("sql.sql",
                      "DrunkState condition has invalid state ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue2) {
            LOG_ERROR("sql.sql",
                      "DrunkState condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "DrunkState condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_NEAR_CREATURE: {
        if (!sObjectMgr->GetCreatureTemplate(cond->ConditionValue1)) {
            LOG_ERROR("sql.sql",
                      "NearCreature condition has non existing creature "
                      "template entry ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        break;
    ...
    case CONDITION_NEAR_GAMEOBJECT: {
        if (!sObjectMgr->GetGameObjectTemplate(cond->ConditionValue1)) {
            LOG_ERROR("sql.sql",
                      "NearGameObject condition has non existing gameobject "
                      "template entry ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue3 > 2)
            LOG_ERROR("sql.sql",
                      "NearGameObject condition for gameobject ID ({}) has "
                      "data over 2 for value3 ({})!",
                      cond->ConditionValue1,
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_OBJECT_ENTRY_GUID: {
        ...
        case TYPEID_UNIT:
            if (cond->ConditionValue2 &&
                !sObjectMgr->GetCreatureTemplate(cond->ConditionValue2)) {
                LOG_ERROR("sql.sql",
                          "ObjectEntryGuid condition has non existing creature "
                          "template entry ({}), skipped",
                          cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3 > 1) {
                if (CreatureData const* creatureData =
                        sObjectMgr->GetCreatureData(cond->ConditionValue3)) {
                    if (cond->ConditionValue2 &&
                        creatureData->id1 != cond->ConditionValue2) {
                        LOG_ERROR(
                            "sql.sql",
                            "ObjectEntryGuid condition has guid {} set but "
                            "does not match creature entry ({}), skipped",
                            cond->ConditionValue3,
                            cond->ConditionValue2);
                        return false;
                    }
                    ...
                    LOG_ERROR("sql.sql",
                              "ObjectEntryGuid condition has non existing "
        ...
        case TYPEID_GAMEOBJECT:
            if (cond->ConditionValue2 &&
                !sObjectMgr->GetGameObjectTemplate(cond->ConditionValue2)) {
                LOG_ERROR("sql.sql",
                          "ObjectEntryGuid condition has non existing "
                          "gameobject template entry ({}), skipped",
                          cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3) {
                if (GameObjectData const* goData =
                        sObjectMgr->GetGameObjectData(cond->ConditionValue3)) {
                    if (cond->ConditionValue2 &&
                        goData->id != cond->ConditionValue2) {
                        LOG_ERROR(
                            "sql.sql",
                            "ObjectEntryGuid condition has guid {} set but "
                            "does not match gameobject entry ({}), skipped",
                            cond->ConditionValue3,
                            cond->ConditionValue2);
                        return false;
                    }
        ...
        case TYPEID_CORPSE:
            if (cond->ConditionValue2)
                LOG_ERROR("sql.sql",
                          "ObjectEntryGuid condition has useless data in "
                          "value2 ({})!",
                          cond->ConditionValue2);
            if (cond->ConditionValue3)
                LOG_ERROR("sql.sql",
        ...
        default:
            LOG_ERROR(
                "sql.sql",
                "ObjectEntryGuid condition has wrong typeid set ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        break;
    ...
    case CONDITION_TYPE_MASK: {
        if (!cond->ConditionValue1 ||
            ...
            LOG_ERROR(
                "sql.sql",
                "TypeMask condition has invalid typemask set ({}), skipped",
                cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "TypeMask condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "TypeMask condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_RELATION_TO: {
        if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets()) {
            LOG_ERROR(
                ...
                "ConditionValue1(ConditionTarget selection) ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2 >= RELATION_MAX) {
            LOG_ERROR("sql.sql",
                      "RelationTo condition has invalid "
                      "ConditionValue2(RelationType) ({}), skipped",
                      cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "RelationTo condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_REACTION_TO: {
        ...
        if (!cond->ConditionValue2) {
            LOG_ERROR("sql.sql",
                      ...
                      "ConditionValue2(rankMask) ({}), skipped",
                      cond->ConditionValue2);
            return false;
        }
        break;
    ...
    case CONDITION_DISTANCE_TO: {
        ...
        if (cond->ConditionValue3 >= COMP_TYPE_MAX) {
            LOG_ERROR("sql.sql",
                      "DistanceTo condition has invalid ComparisionType ({}), "
                      "skipped",
                      cond->ConditionValue3);
            return false;
        }
    ...
    case CONDITION_ALIVE: {
        if (cond->ConditionValue1)
            LOG_ERROR("sql.sql",
                      "Alive condition has useless data in value1 ({})!",
                      cond->ConditionValue1);
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Alive condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Alive condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_HP_VAL: {
        if (cond->ConditionValue2 >= COMP_TYPE_MAX) {
            LOG_ERROR(
                "sql.sql",
                "HpVal condition has invalid ComparisionType ({}), skipped",
                cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "HpVal condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_HP_PCT: {
        if (cond->ConditionValue1 > 100) {
            LOG_ERROR("sql.sql",
                      "HpPct condition has too big percent value ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue2 >= COMP_TYPE_MAX) {
            LOG_ERROR(
                "sql.sql",
                "HpPct condition has invalid ComparisionType ({}), skipped",
                cond->ConditionValue2);
            return false;
        }
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "HpPct condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_AREAID:
    ...
    case CONDITION_WORLD_STATE: {
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "World state condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_PHASEMASK: {
        if (cond->ConditionValue2)
            LOG_ERROR("sql.sql",
                      "Phasemask condition has useless data in value2 ({})!",
                      cond->ConditionValue2);
        if (cond->ConditionValue3)
            LOG_ERROR("sql.sql",
                      "Phasemask condition has useless data in value3 ({})!",
                      cond->ConditionValue3);
        break;
    }
    case CONDITION_TITLE: {
        ...
        if (!titleEntry) {
            LOG_ERROR("sql.sql",
                      "Title condition has non existing title in value1 ({}), "
                      "skipped",
                      cond->ConditionValue1);
            return false;
        }
    ...
    case CONDITION_SPAWNMASK: {
        if (cond->ConditionValue1 > SPAWNMASK_RAID_ALL) {
            LOG_ERROR("condition",
                      "SpawnMask condition has non existing SpawnMask in "
                      "value1 ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
    ...
    case CONDITION_UNIT_STATE: {
        if (!(cond->ConditionValue1 & UNIT_STATE_ALL_STATE_SUPPORTED)) {
            LOG_ERROR("condition",
                      "UnitState condition has non existing UnitState in "
                      "value1 ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
    ...
    case CONDITION_CREATURE_TYPE: {
        if (!cond->ConditionValue1 ||
            cond->ConditionValue1 > CREATURE_TYPE_GAS_CLOUD) {
            LOG_ERROR("condition",
                      "CreatureType condition has non existing CreatureType in "
                      "value1 ({}), skipped",
                      cond->ConditionValue1);
            return false;
        }
    ...
    case CONDITION_REALM_ACHIEVEMENT: {
        ...
        if (!achievement) {
            LOG_ERROR("condition",
                      "CONDITION_REALM_ACHIEVEMENT has non existing realm "
                      "first achivement id ({}), skipped.",
                      cond->ConditionValue1);
            return false;
        }
        break;
    ...
    case CONDITION_QUEST_OBJECTIVE_PROGRESS: {
        const Quest* quest =
            sObjectMgr->GetQuestTemplate(cond->ConditionValue1);
        if (!quest) {
            LOG_ERROR("sql.sql",
                      "CONDITION_QUEST_OBJECTIVE_PROGRESS points to "
                      "non-existing quest ({}), skipped.",
                      cond->ConditionValue1);
            return false;
        }
        ...
        if (cond->ConditionValue2 > 3) {
            LOG_ERROR("sql.sql",
                      "CONDITION_QUEST_OBJECTIVE_PROGRESS has out-of-range "
                      "quest objective index specified ({}), it must be a "
                      "number between 0 and 3. skipped.",
                      cond->ConditionValue2);
            return false;
        }
        ...
        if (quest->RequiredNpcOrGo[cond->ConditionValue2] == 0) {
            LOG_ERROR(
                "sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective {} for "
                "quest {}, but the field RequiredNPCOrGo{} is 0, skipped.",
                cond->ConditionValue2,
                cond->ConditionValue1,
                cond->ConditionValue2);
            return false;
        ...
        if (cond->ConditionValue3 >
            quest->RequiredNpcOrGoCount[cond->ConditionValue2]) {
            LOG_ERROR(
                "sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective count "
                "{} in value3, but quest {} has a maximum objective count of "
                "{} in RequiredNPCOrGOCount{}, skipped.",
                cond->ConditionValue3,
                cond->ConditionValue2,
                quest->RequiredNpcOrGoCount[cond->ConditionValue2],
                cond->ConditionValue2);
    ...
    case CONDITION_HAS_AURA_TYPE: {
        if (cond->ConditionValue1 == SPELL_AURA_NONE ||
            cond->ConditionValue1 >= TOTAL_AURAS) {
            LOG_ERROR(
                "sql.sql",
                "Has Aura Effect condition has non existing aura ({}), skipped",
                cond->ConditionValue1);
            return false;
        }
        break;
    ...
    case CONDITION_STAND_STATE: {
        ...
        if (!valid) {
            LOG_ERROR("sql.sql",
                      "CONDITION_STAND_STATE has non-existing stand state "
                      "({},{}), skipped.",
                      cond->ConditionValue1,
                      cond->ConditionValue2);
            return false;
    ...
    case CONDITION_DIFFICULTY_ID:
        if (cond->ConditionValue1 >= MAX_DIFFICULTY) {
            LOG_ERROR("sql.sql",
                      "CONDITION_DIFFICULTY_ID has non existing difficulty in "
                      "value1 ({}), skipped.",
                      cond->ConditionValue1);
            return false;
        }
    ...
    case CONDITION_PET_TYPE:
        if (cond->ConditionValue1 >= (1 << MAX_PET_TYPE)) {
            LOG_ERROR(
                "sql.sql",
                "CONDITION_PET_TYPE has non-existing pet type {}, skipped.",
                cond->ConditionValue1);
            return false;
        }
        break;
...
{
    for (ConditionReferenceContainer::iterator itr =
             ConditionReferenceStore.begin();
         itr != ConditionReferenceStore.end();
         ++itr) {
        for (ConditionList::const_iterator it = itr->second.begin();
             it != itr->second.end();
             ++it)
            delete *it;
        itr->second.clear();
    ...
    for (ConditionContainer::iterator itr = ConditionStore.begin();
         itr != ConditionStore.end();
         ++itr) {
        for (ConditionTypeContainer::iterator it = itr->second.begin();
             it != itr->second.end();
             ++it) {
            for (ConditionList::const_iterator i = it->second.begin();
                 i != it->second.end();
                 ++i)
                delete *i;
            it->second.clear();
    ...
    for (CreatureSpellConditionContainer::iterator itr =
             VehicleSpellConditionStore.begin();
         itr != VehicleSpellConditionStore.end();
         ++itr) {
        for (ConditionTypeContainer::iterator it = itr->second.begin();
             it != itr->second.end();
             ++it) {
    ...
    for (SmartEventConditionContainer::iterator itr =
             SmartEventConditionStore.begin();
         itr != SmartEventConditionStore.end();
         ++itr) {
        for (ConditionTypeContainer::iterator it = itr->second.begin();
             it != itr->second.end();
             ++it) {
    ...
    for (CreatureSpellConditionContainer::iterator itr =
             SpellClickEventConditionStore.begin();
         itr != SpellClickEventConditionStore.end();
         ++itr) {
        for (ConditionTypeContainer::iterator it = itr->second.begin();
             it != itr->second.end();
             ++it) {
    ...
    for (NpcVendorConditionContainer::iterator itr =
             NpcVendorConditionContainerStore.begin();
         itr != NpcVendorConditionContainerStore.end();
         ++itr) {
        for (ConditionTypeContainer::iterator it = itr->second.begin();
             it != itr->second.end();
             ++it) {
    ...
    // this is a BIG hack, feel free to fix it if you can figure out the
    // ConditionMgr ;)
    for (std::list<Condition*>::const_iterator itr =
             AllocatedMemoryStore.begin();
         itr != AllocatedMemoryStore.end();
         ++itr)
        delete *itr;
