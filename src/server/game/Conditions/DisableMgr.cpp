...
void LoadDisables()
{
    ...
    // reload case
    for (DisableMap::iterator itr = m_DisableMap.begin();
         itr != m_DisableMap.end();
         ++itr)
        itr->second.clear();
    ...
    do {
        ...
        if (type >= MAX_DISABLE_TYPES) {
            LOG_ERROR("sql.sql",
                      "Invalid type {} specified in `disables` table, skipped.",
                      type);
            continue;
        }
        ...
        case DISABLE_TYPE_GO_LOS:
            if (!sObjectMgr->GetGameObjectTemplate(entry)) {
                LOG_ERROR("sql.sql",
                          "Gameobject entry {} from `disables` doesn't exist "
                          "in dbc, skipped.",
                          entry);
                continue;
            }
            if (flags)
                LOG_ERROR(
                    "sql.sql",
                    "Disable flags specified for gameobject {}, useless data.",
                    entry);
            break;
        case DISABLE_TYPE_SPELL:
            if (!(sSpellMgr->GetSpellInfo(entry) ||
                  flags & SPELL_DISABLE_DEPRECATED_SPELL)) {
                LOG_ERROR("sql.sql",
                          "Spell entry {} from `disables` doesn't exist in "
                          "dbc, skipped.",
                          entry);
                continue;
            }
            ...
            if (!flags || flags > MAX_SPELL_DISABLE_TYPE) {
                LOG_ERROR("sql.sql",
                          "Disable flags for spell {} are invalid, skipped.",
                          entry);
                continue;
            }

            if (flags & SPELL_DISABLE_MAP) {
                for (std::string_view mapStr :
                     Acore::Tokenize(params_0, ',', true)) {
                    if (Optional<uint32> mapId =
                            Acore::StringTo<uint32>(mapStr))
                        data.params[0].insert(*mapId);
                    else
                        LOG_ERROR("sql.sql",
                                  "Disable map '{}' for spell {} is invalid, "
                                  "skipped.",
                                  mapStr,
                                  entry);
                }
            ...
            if (flags & SPELL_DISABLE_AREA) {
                for (std::string_view areaStr :
                     Acore::Tokenize(params_1, ',', true)) {
                    if (Optional<uint32> areaId =
                            Acore::StringTo<uint32>(areaStr))
                        data.params[1].insert(*areaId);
                    else
                        LOG_ERROR("sql.sql",
                                  "Disable area '{}' for spell {} is invalid, "
                                  "skipped.",
                                  areaStr,
                                  entry);
                }
            ...
            if (flags & SPELL_DISABLE_LOS) {
                SpellInfo* spellInfo =
                    const_cast<SpellInfo*>(sSpellMgr->GetSpellInfo(entry));
        ...
        case DISABLE_TYPE_LFG_MAP: {
            ...
            if (!mapEntry) {
                LOG_ERROR("sql.sql",
                          "Map entry {} from `disables` doesn't exist in dbc, "
                          "skipped.",
                          entry);
                continue;
            }
            ...
            case MAP_INSTANCE:
            case MAP_RAID:
                if (flags & DUNGEON_STATUSFLAG_HEROIC &&
                    !GetMapDifficultyData(entry, DUNGEON_DIFFICULTY_HEROIC))
                    isFlagInvalid = true;
                else if (flags & RAID_STATUSFLAG_10MAN_HEROIC &&
                         !GetMapDifficultyData(entry,
                                               RAID_DIFFICULTY_10MAN_HEROIC))
                    isFlagInvalid = true;
                else if (flags & RAID_STATUSFLAG_25MAN_HEROIC &&
                         !GetMapDifficultyData(entry,
            ...
            case MAP_ARENA:
                LOG_ERROR("sql.sql",
                          "Battleground map {} specified to be disabled in map "
                          "case, skipped.",
                          entry);
                continue;
            }
        ...
        case DISABLE_TYPE_BATTLEGROUND:
            if (!sBattlemasterListStore.LookupEntry(entry)) {
                LOG_ERROR("sql.sql",
                          "Battleground entry {} from `disables` doesn't exist "
                          "in dbc, skipped.",
                          entry);
                continue;
            }
            if (flags)
                LOG_ERROR("sql.sql",
                          "Disable flags specified for battleground {}, "
                          "useless data.",
                          entry);
            break;
        case DISABLE_TYPE_OUTDOORPVP:
            if (entry > MAX_OUTDOORPVP_TYPES) {
                LOG_ERROR("sql.sql",
                          "OutdoorPvPTypes value {} from `disables` is "
                          "invalid, skipped.",
                          entry);
                continue;
            }
        ...
        case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
            if (!sAchievementCriteriaStore.LookupEntry(entry)) {
                LOG_ERROR("sql.sql",
                          "Achievement Criteria entry {} from `disables` "
                          "doesn't exist in dbc, skipped.",
                          entry);
                continue;
            }
            if (flags)
                LOG_ERROR("sql.sql",
                          "Disable flags specified for Achievement Criteria "
                          "{}, useless data.",
                          entry);
            break;
        case DISABLE_TYPE_VMAP: {
            ...
            case MAP_COMMON:
                if (flags & VMAP::VMAP_DISABLE_AREAFLAG)
                    LOG_INFO("disable",
                             "Areaflag disabled for world map {}.",
                             entry);
                if (flags & VMAP::VMAP_DISABLE_LIQUIDSTATUS)
                    LOG_INFO("disable",
                             "Liquid status disabled for world map {}.",
                             entry);
                break;
            case MAP_INSTANCE:
            case MAP_RAID:
                if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                    LOG_INFO("disable",
                             "Height disabled for instance map {}.",
                             entry);
                if (flags & VMAP::VMAP_DISABLE_LOS)
                    LOG_INFO(
                        "disable", "LoS disabled for instance map {}.", entry);
                break;
            case MAP_BATTLEGROUND:
                if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                    LOG_INFO("disable",
                             "Height disabled for battleground map {}.",
                             entry);
                if (flags & VMAP::VMAP_DISABLE_LOS)
                    LOG_INFO("disable",
                             "LoS disabled for battleground map {}.",
                             entry);
                break;
            case MAP_ARENA:
                if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                    LOG_INFO(
                        "disable", "Height disabled for arena map {}.", entry);
                if (flags & VMAP::VMAP_DISABLE_LOS)
                    LOG_INFO(
                        "disable", "LoS disabled for arena map {}.", entry);
                break;
            default:
                break;
            }
        ...
        case DISABLE_TYPE_GAME_EVENT: {
            ...
            if (activeEvents.find(entry) != activeEvents.end()) {
                ...
                LOG_INFO(
                    "disable",
                    "Event entry {} was stopped because it has been disabled.",
                    entry);
            }
            break;
        }
...
void CheckQuestDisables()
{
    ...
    for (DisableTypeMap::iterator itr =
             m_DisableMap[DISABLE_TYPE_QUEST].begin();
         itr != m_DisableMap[DISABLE_TYPE_QUEST].end();) {
        const uint32 entry = itr->first;
        if (!sObjectMgr->GetQuestTemplate(entry)) {
            LOG_ERROR("sql.sql",
                      "Quest entry {} from `disables` doesn't exist, skipped.",
                      entry);
    ...
    LOG_INFO("server.loading",
             ">> Checked {} Quest Disables in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}

bool IsDisabledFor(DisableType type,
                   uint32      entry,
                   Unit const* unit,
                   uint8       flags)
{
    if (type > MAX_DISABLE_TYPES) {
        LOG_ERROR("server",
                  "Disables::IsDisabledFor() called with unknown disable type "
                  "{}!  (entry {}, flags {}).",
                  type,
                  entry,
                  flags);
    ...
    case DISABLE_TYPE_SPELL: {
        ...
        if (unit) {
            if ((spellFlags & SPELL_DISABLE_PLAYER &&
                 unit->GetTypeId() == TYPEID_PLAYER) ||
                (unit->GetTypeId() == TYPEID_UNIT &&
                 ((unit->IsPet() && spellFlags & SPELL_DISABLE_PET) ||
                  spellFlags & SPELL_DISABLE_CREATURE))) {
                if (spellFlags & SPELL_DISABLE_MAP) {
                    ...
                    if (!(spellFlags & SPELL_DISABLE_AREA))
                        return false; // Spell is disabled on another map, but
                                      // not this one, return false

                    // Spell is disabled in an area, but not explicitly our
                    // current mapId. Continue processing.
                }
                ...
                if (spellFlags & SPELL_DISABLE_AREA) {
                    ...
                    return false; // Spell is disabled in another area, but not
            ...
            return false;
                 ...
                 SPELL_DISABLE_DEPRECATED_SPELL) // call not from spellcast
            return true;
        ...
        if (Player const* player = unit->ToPlayer()) {
            ...
            if (mapEntry->IsDungeon()) {
                ...
                Difficulty targetDifficulty =
                    player->GetDifficulty(mapEntry->IsRaid());
...
bool IsPathfindingEnabled(const Map* map)
{
    ...
    return !MMAP::MMapFactory::forbiddenMaps[map->GetId()] &&
           (sWorld->getBoolConfig(CONFIG_ENABLE_MMAPS)
                ? true
