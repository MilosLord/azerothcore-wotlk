...
#define OUT_SAVE_INST_DATA                                                     \
    LOG_DEBUG("scripts.ai",                                                    \
              "Saving Instance Data for Instance {} (Map {}, Instance Id {})", \
              instance->GetMapName(),                                          \
              instance->GetId(),                                               \
...
#define OUT_LOAD_INST_DATA(a)                                                  \
    LOG_DEBUG("scripts.ai",                                                    \
              "Loading Instance Data for Instance {} (Map {}, Instance Id "    \
              "{}). Input is '{}'",                                            \
              instance->GetMapName(),                                          \
              instance->GetId(),                                               \
              instance->GetInstanceId(),                                       \
...
#define OUT_LOAD_INST_DATA_COMPLETE                                            \
    LOG_DEBUG("scripts.ai",                                                    \
              "Instance Data Load for Instance {} (Map {}, Instance Id: {}) "  \
              "is complete.",                                                  \
              instance->GetMapName(),                                          \
              instance->GetId(),                                               \
              instance->GetInstanceId())
#define OUT_LOAD_INST_DATA_FAIL                                                \
    LOG_ERROR("scripts.ai",                                                    \
              "Unable to load Instance Data for Instance {} (Map {}, "         \
              "Instance Id: {}).",                                             \
              instance->GetMapName(),                                          \
              instance->GetId(),                                               \
              instance->GetInstanceId())
...
enum EncounterFrameType {
    ...
    ENCOUNTER_FRAME_REFRESH_FRAMES =
        7, // Xinef: can be used to refresh frames after unit was destroyed from
           // client and send back (phase changes)
};
...
enum DoorType {
    ...
    DOOR_TYPE_SPAWN_HOLE = 2, // Door can open if encounter is in progress,
                              // typically used for spawning places
    MAX_DOOR_TYPES,
};
...
typedef std::pair<DoorInfoMap::const_iterator, DoorInfoMap::const_iterator>
    DoorInfoMapBounds;
...
typedef std::map<ObjectGuid::LowType /*spawnId*/, uint8 /*state*/>
    ObjectStateMap;
...
public:
    ...
    virtual void OnPlayerAreaUpdate(Player* /*player*/,
                                    uint32 /*oldArea*/,
                                    uint32 /*newArea*/)
    {
    }
    ...
    // use HandleGameObject(ObjectGuid::Empty, boolen, GO); in OnObjectCreate in
    // instance scripts use HandleGameObject(GUID, boolen, nullptr); in any
    // other script
    void HandleGameObject(ObjectGuid guid, bool open, GameObject* go = nullptr);

    // change active state of doors or buttons
    void DoUseDoorOrButton(ObjectGuid guid,
    ...
    void DoUpdateAchievementCriteria(AchievementCriteriaTypes type,
                                     uint32                   miscValue1 = 0,
                                     uint32                   miscValue2 = 0,
                                     Unit*                    unit = nullptr);

    // Start/Stop Timed Achievement Criteria for all players in instance
    void DoStartTimedAchievement(AchievementCriteriaTimedTypes type,
                                 uint32                        entry);
    void DoStopTimedAchievement(AchievementCriteriaTimedTypes type,
                                uint32                        entry);
    ...
    void DoCastSpellOnPlayer(Player* player,
                             uint32  spell,
    ...
    {
        return index < persistentData.size() ? persistentData[index] : 0;
    };
    void StorePersistentData(uint32 index, uint32 data);

    // Achievement criteria additional requirements check
    // NOTE: not use this if same can be checked existed requirement types from
    // AchievementCriteriaRequirementType
    virtual bool CheckAchievementCriteriaMeet(uint32 /*criteria_id*/,
                                              Player const* /*source*/,
                                              Unit const* /*target*/ = nullptr,
                                              uint32 /*miscvalue1*/  = 0);

    // Checks boss requirements (one boss required to kill other)
    virtual bool CheckRequiredBosses(uint32 /*bossId*/,
                                     Player const* /*player*/ = nullptr) const
    {
        return true;
    }
    ...
    void SendEncounterUnit(uint32 type,
                           Unit*  unit   = nullptr,
                           uint8  param1 = 0,
                           uint8  param2 = 0);
    ...
    {
        return _activatedAreaTriggers.find(id) != _activatedAreaTriggers.end();
    }

    // Allows to perform particular actions
    virtual void DoAction(int32 /*action*/) {}

    // Allows executing code using all creatures registered in the instance
    // script as minions
    ...
    {
        _objectStateMap[spawnId] = state;
    };
    [[nodiscard]] uint8
    GetStoredGameObjectState(ObjectGuid::LowType spawnId) const;
...
protected:
    ...
    {
        persistentData.resize(number);
    }
    void LoadBossBoundaries(BossBoundaryData const& data);
    void LoadDoorData(DoorData const* data);
    void LoadMinionData(MinionData const* data);
    void LoadObjectData(ObjectData const* creatureData,
...
private:
    static void LoadObjectData(ObjectData const* creatureData,
    ...
    uint32 completedEncounters; // completed encounter mask, bit indexes are
                                // DungeonEncounter.dbc boss numbers, used for
                                // packets
