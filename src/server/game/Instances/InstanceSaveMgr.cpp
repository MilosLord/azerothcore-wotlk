...
{
    lock_instLists = true;
    // pussywizard: crashes on calling function in destructor
    // (PlayerUnbindInstance), completely not needed anyway
    /*for (InstanceSaveHashMap::iterator itr = m_instanceSaveById.begin(); itr
    != m_instanceSaveById.end(); ++itr)
    {
        InstanceSave* save = itr->second;

        InstanceSave::PlayerListType &pList = save->m_playerList;
        while (!pList.empty())
            PlayerUnbindInstance(*(pList.begin()), save->GetMapId(),
    save->GetDifficulty(), false);
                                               ...
                                               bool       startup /*=false*/)
{
    ...
    if (!entry) {
        LOG_ERROR("instance.save",
                  "InstanceSaveMgr::AddInstanceSave: wrong mapid = {}, "
                  "instanceid = {}!",
                  mapId,
                  instanceId);
        return nullptr;
    ...
    if (instanceId == 0) {
        LOG_ERROR("instance.save",
                  "InstanceSaveMgr::AddInstanceSave: mapid = {}, wrong "
                  "instanceid = {}!",
                  mapId,
                  instanceId);
        return nullptr;
    ...
    if (difficulty >=
        ...
        LOG_ERROR("instance.save",
                  "InstanceSaveMgr::AddInstanceSave: mapid = {}, instanceid = "
                  "{}, wrong difficulty {}!",
                  mapId,
                  instanceId,
                  difficulty);
    ...
    if (entry->map_type == MAP_RAID || difficulty > DUNGEON_DIFFICULTY_NORMAL) {
        ...
        resetTime = GameTime::GetGameTime().count() +
                    static_cast<long long>(3) *
                        DAY; // normals expire after 3 days even if someone is
                             // still bound to them, cleared on startup
        extendedResetTime = 0;
    }
    InstanceSave* save = new InstanceSave(
        mapId, instanceId, difficulty, resetTime, extendedResetTime);
    if (!startup)
        save->InsertToDB();
...
bool InstanceSaveMgr::DeleteInstanceSaveIfNeeded(uint32 InstanceId,
                                                 bool   skipMapCheck)
{
    return DeleteInstanceSaveIfNeeded(GetInstanceSave(InstanceId),
                                      skipMapCheck);
}
...
{
    // pussywizard: save is removed only when there are no more players bound
    // AND the map doesn't exist pussywizard: this function is called when
    // unbinding a player and when unloading a map
    if (!lock_instLists && save && save->m_playerList.empty() &&
        (skipMapCheck ||
         !sMapMgr->FindMap(save->GetMapId(), save->GetInstanceId()))) {
        // delete save from storage:
        InstanceSaveHashMap::iterator itr =
            m_instanceSaveById.find(save->GetInstanceId());
        ...
        // clear respawn times (if map is loaded do it just to be sure, if
        // already unloaded it won't do it by itself)
...
InstanceSave::InstanceSave(uint16     MapId,
    ...
    : m_resetTime(resetTime), m_extendedResetTime(extendedResetTime),
      m_instanceid(InstanceId), m_mapid(MapId),
      m_difficulty(IsSharedDifficultyMap(MapId) ? Difficulty(difficulty % 2)
                                                : difficulty),
      m_canReset(true), m_instanceData(""), m_completedEncounterMask(0)
{
...
void InstanceSave::InsertToDB()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_INSTANCE_SAVE);
...
bool InstanceSave::RemovePlayer(ObjectGuid guid, InstanceSaveMgr* ism)
{
    bool deleteSave = false;
    {
        ...
        // ism passed as an argument to avoid calling via singleton (might
...
void InstanceSaveMgr::LoadInstances()
{
    ...
    CharacterDatabase.DirectExecute(
        "DELETE ci.* FROM character_instance AS ci LEFT JOIN characters AS c "
        "ON ci.guid = c.guid WHERE c.guid IS NULL");

    // Delete expired normal instances (normals expire after 3 days even if
    // someone is still bound to them, cleared on startup)
    CharacterDatabase.DirectExecute("DELETE FROM instance WHERE resettime > 0 "
                                    "AND resettime < UNIX_TIMESTAMP()");

    // Delete instance with no binds
    CharacterDatabase.DirectExecute(
        "DELETE i.* FROM instance AS i LEFT JOIN character_instance AS ci ON "
        "i.id = ci.instance WHERE ci.guid IS NULL");

    // Delete creature_respawn, gameobject_respawn and creature_instance for
    // non-existent instance
    CharacterDatabase.DirectExecute(
        "DELETE FROM creature_respawn WHERE instanceId > 0 AND instanceId NOT "
        "IN (SELECT id FROM instance)");
    CharacterDatabase.DirectExecute(
        "DELETE FROM gameobject_respawn WHERE instanceId > 0 AND instanceId "
        "NOT IN (SELECT id FROM instance)");
    CharacterDatabase.DirectExecute(
        "DELETE tmp.* FROM character_instance AS tmp LEFT JOIN instance ON "
        "tmp.instance = instance.id WHERE tmp.instance > 0 AND instance.id IS "
        "NULL");

    // Clean invalid references to instance
    CharacterDatabase.DirectExecute(
        "UPDATE corpse SET instanceId = 0 WHERE instanceId > 0 AND instanceId "
        "NOT IN (SELECT id FROM instance)");
    CharacterDatabase.DirectExecute(
        "UPDATE characters AS tmp LEFT JOIN instance ON tmp.instance_id = "
        "instance.id SET tmp.instance_id = 0 WHERE tmp.instance_id > 0 AND "
        "instance.id IS NULL");

    // Initialize instance id storage (Needs to be done after the trash has been
    // clean out)
    ...
    // Sanitize pending rows on Instance_saved_data for data that wasn't deleted
    // properly
    SanitizeInstanceSavedData();

    LOG_INFO("server.loading",
             ">> Loaded Instances And Binds in {} ms",
...
void InstanceSaveMgr::LoadResetTimes()
{
    ...
    if (result) {
        do {
            ...
            if (!mapDiff) {
                LOG_ERROR("instance.save",
                          "InstanceSaveMgr::LoadResetTimes: invalid "
                          "mapid({})/difficulty({}) pair in instance_reset!",
                          mapid,
                          difficulty);
                CharacterDatabase.DirectExecute(
                    "DELETE FROM instance_reset WHERE mapid = '{}' AND "
                    "difficulty = '{}'",
                    mapid,
                    difficulty);
                continue;
    ...
    // have never been reset yet add the global reset times to the priority
    // queue
    for (MapDifficultyMap::const_iterator itr = sMapDifficultyMap.begin();
         itr != sMapDifficultyMap.end();
         ++itr) {
        ...
        uint32 period = uint32(
            ((mapDiff->resetTime * sWorld->getRate(RATE_INSTANCE_RESET_TIME)) /
             DAY) *
            DAY);
        if (period < DAY)
            period = DAY;
        ...
        ScheduleReset(t - ResetTimeDelay[type - 1],
                      InstResetEvent(type, mapid, difficulty));
    }
}
...
{
    QueryResult result = CharacterDatabase.Query(
        "SELECT id, map, resettime, difficulty, completedEncounters, data FROM "
        "instance ORDER BY id ASC");
    if (result) {
        do {
            Field* fields = result->Fetch();
            ...
            InstanceSave* save = AddInstanceSave(
                mapId, instanceId, Difficulty(difficulty), true);
            if (save) {
...
void InstanceSaveMgr::LoadCharacterBinds()
{
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT guid, instance, permanent, extended FROM character_instance");
    if (result) {
        do {
            Field* fields = result->Fetch();
            ...
            if (InstanceSave* save = GetInstanceSave(instanceId)) {
                ...
                InstancePlayerBind& bind =
                    playerBindStorage[guid]
                        ->m[save->GetDifficulty()][save->GetMapId()];
                if (bind.save) // pussywizard: another bind for the same map and
                               // difficulty! may happen because of mysql thread
                               // races
                {
                    if (bind.perm) // already loaded perm -> delete currently
                                   // checked one from db
                    {
                        CharacterDatabasePreparedStatement* stmt =
                            CharacterDatabase.GetPreparedStatement(
...
void InstanceSaveMgr::Update()
{
    ...
    while (!m_resetTimeQueue.empty()) {
        ...
        if (event.type) {
            ...
            if (warn) {
                ...
                ++event.type;
                ScheduleReset(resetTime - ResetTimeDelay[event.type - 1],
                              event);
            }
            else
                resetOccurred = true;
    ...
    if (resetOccurred) {
        LOG_INFO("instance.save",
                 "Instance ID reset occurred, sending updated calendar and "
                 "raid info to all players!");
        WorldPacket dummy;
        for (SessionMap::const_iterator itr = sWorld->GetAllSessions().begin();
...
void InstanceSaveMgr::_ResetSave(InstanceSaveHashMap::iterator& itr)
{
    ...
    for (GuidList::iterator iter = pList.begin(), iter2; iter != pList.end();) {
        ...
        PlayerUnbindInstanceNotExtended(
            *iter2,
            itr->second->GetMapId(),
            itr->second->GetDifficulty(),
            ObjectAccessor::FindConnectedPlayer(*iter2));
    }
    ...
    if (pList.empty()) {
        ...
        // clear respawn times if the map is already unloaded and won't do it by
        // itself
        if (!sMapMgr->FindMap(itr->second->GetMapId(),
                              itr->second->GetInstanceId()))
            Map::DeleteRespawnTimesInDB(itr->second->GetMapId(),
                                        itr->second->GetInstanceId());
        ...
        // delete character_instance per id where extended = 0, transtaction
        // with set extended = 0, transaction is used to avoid mysql thread
        // races
        ...
        itr->second->SetResetTime(GetResetTimeFor(
            itr->second->GetMapId(), itr->second->GetDifficulty()));
        itr->second->SetExtendedResetTime(GetExtendedResetTimeFor(
            itr->second->GetMapId(), itr->second->GetDifficulty()));
    }
                                      ...
                                      time_t     resetTime)
{
    ...
    if (!warn) {
        ...
        if (!mapDiff || !mapDiff->resetTime) {
            LOG_ERROR("instance.save",
                      "InstanceSaveMgr::ResetOrWarnAll: not valid difficulty "
                      "or no reset delay for map {}",
                      mapid);
            return;
        }
        ...
        // calculate the next reset time
        uint32 diff =
            sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_HOUR) * HOUR;
        ...
        uint32 next_reset =
            uint32(((resetTime + MINUTE) / DAY * DAY) + period + diff);
        SetResetTimeFor(mapid, difficulty, next_reset);
        SetExtendedResetTimeFor(mapid, difficulty, next_reset + period);
        ScheduleReset(time_t(next_reset - 3600),
                      InstResetEvent(1, mapid, difficulty));
        ...
        // remove all binds to instances of the given map and delete from db
        // (delete per instance id, no mass deletion!) do this after new reset
        // time is calculated
        for (InstanceSaveHashMap::iterator itr = m_instanceSaveById.begin(),
                                           itr2;
             itr != m_instanceSaveById.end();) {
            itr2 = itr++;
            if (itr2->second->GetMapId() == mapid &&
                itr2->second->GetDifficulty() == difficulty)
                _ResetSave(itr2);
        }
    }
    ...
    for (mitr = instMaps.begin(); mitr != instMaps.end(); ++mitr) {
        ...
        if (warn) {
            ...
            map2->ToInstanceMap()->Reset(
                INSTANCE_RESET_GLOBAL,
                (save ? &(save->m_playerList) : nullptr));
        }
    }
}

InstancePlayerBind*
InstanceSaveMgr::PlayerBindToInstance(ObjectGuid    guid,
                                      InstanceSave* save,
                                      bool          permanent,
                                      Player*       player /*= nullptr*/)
{
    InstancePlayerBind& bind =
        playerBindStorage[guid]->m[save->GetDifficulty()][save->GetMapId()];
    ASSERT(!bind.perm ||
           permanent); // ensure there's no changing permanent to temporary,
                       // this can be done only by unbinding
    ...
    if (bind.save) {
        ...
        // pussywizard: CHANGED MY MIND! DON'T SLOW DOWN THIS QUERY! HANDLE ONLY
        // DURING LOADING FROM DB! example: enter instance -> bind -> update old
        // id to new id -> exit -> delete new id if delete by new id is executed
        // before update, then we end up with in db
        /*CharacterDatabaseTransaction trans =
        CharacterDatabase.BeginTransaction();
        // ensure any for that map+difficulty is deleted!
        CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_GUID_MAP_DIFF);
        // DELETE ci FROM character_instance ci JOIN instance i ON ci.instance =
        i.id WHERE ci.guid = ? AND i.map = ? AND i.difficulty = ?
        ...
        if (player)
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_RAID, 1);
    }
    ...
    if (player)
        sScriptMgr->OnPlayerBindToInstance(
            player, save->GetDifficulty(), save->GetMapId(), permanent);
                                           ...
                                           Player*    player /*= nullptr*/)
{
    ...
    if (itr != w->m[difficulty].end()) {
        ...
        if (itr->second.perm && player)
            player->GetSession()->SendCalendarRaidLockout(itr->second.save,
                                                          false);
...
InstancePlayerBind* InstanceSaveMgr::PlayerGetBoundInstance(
    ObjectGuid guid, uint32 mapid, Difficulty difficulty)
{
    Difficulty difficulty_fixed =
        (IsSharedDifficultyMap(mapid) ? Difficulty(difficulty % 2)
                                      : difficulty);

    MapDifficulty const* mapDiff =
        GetDownscaledMapDifficultyData(mapid, difficulty_fixed);
    if (!mapDiff)
        return nullptr;
                                                  ...
                                                  Difficulty difficulty)
{
    if (InstancePlayerBind* bind =
            PlayerGetBoundInstance(guid, mapid, difficulty))
        if (bind->perm)
            return true;
    return false;
}

BoundInstancesMap const&
InstanceSaveMgr::PlayerGetBoundInstances(ObjectGuid guid, Difficulty difficulty)
{
                                                       ...
                                                       Difficulty difficulty)
{
    ...
    InstancePlayerBind* ipb =
        PlayerGetBoundInstance(player->GetGUID(), mapid, difficulty);
    if (ipb && ipb->perm) // 1. self perm
        return ipb->save->GetInstanceId();
    if (Group* g = player->GetGroup()) {
        if (InstancePlayerBind* ilb = PlayerGetBoundInstance(
                g->GetLeaderGUID(), mapid, difficulty)) // 2. leader temp/perm
...
void InstanceSaveMgr::CopyBinds(ObjectGuid from, ObjectGuid to, Player* toPlr)
{
    ...
    for (uint8 d = 0; d < MAX_DIFFICULTY; ++d) {
        BoundInstancesMap const& bi =
            PlayerGetBoundInstances(from, Difficulty(d));
        for (BoundInstancesMap::const_iterator itr = bi.begin();
             itr != bi.end();
             ++itr)
