...
const int32 ReputationMgr::PointsInRank[MAX_REPUTATION_RANK] = {
...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
    ...
    if (!factionEntry) {
        LOG_ERROR("reputation",
                  "ReputationMgr::IsAtWar: Can't get AtWar flag of {} for "
                  "unknown faction (faction id) #{}.",
                  _player->GetName(),
                  faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    if (!factionEntry) {
        LOG_ERROR("reputation",
                  "ReputationMgr::GetReputation: Can't get reputation of {} "
                  "for unknown faction (faction id) #{}.",
                  _player->GetName(),
                  faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetBaseReputation(FactionEntry const* factionEntry) const
{
    ...
    // in faction.dbc exist factions with (RepListId >=0, listed in character
    // reputation list) with all BaseRepRaceMask[i] == 0
    return 0;
}
...
ReputationRank
ReputationMgr::GetBaseRank(FactionEntry const* factionEntry) const
{
...
uint32
ReputationMgr::GetDefaultStateFlags(FactionEntry const* factionEntry) const
{
    if (!factionEntry)
        return 0;
...
{
    WorldPacket data;
    data.Initialize(SMSG_SET_FORCED_REACTIONS,
                    4 + _forcedReactions.size() * (4 + 4));
    data << uint32(_forcedReactions.size());
    for (ForcedReactions::const_iterator itr = _forcedReactions.begin();
         itr != _forcedReactions.end();
         ++itr) {
...
void ReputationMgr::SendState(FactionState const* faction)
{
    ...
    for (FactionStateList::iterator itr = _factions.begin();
         itr != _factions.end();
         ++itr) {
                                  ...
                                  Optional<ReputationRank> repMaxCap)
{
    ...
    if (!noSpillOver) {
        ...
        if (const RepSpilloverTemplate* repTemplate =
                sObjectMgr->GetRepSpilloverTemplate(factionEntry->ID)) {
            for (uint32 i = 0; i < MAX_SPILLOVER_FACTIONS; ++i) {
                if (repTemplate->faction[i]) {
                    if (_player->GetReputationRank(repTemplate->faction[i]) <=
                        ReputationRank(repTemplate->faction_rank[i])) {
                        // bonuses are already given, so just modify standing by
                        // rate
                        float spilloverRep =
                            standing * repTemplate->faction_rate[i];
            ...
            SimpleFactionsList const* flist =
                GetFactionTeamList(factionEntry->ID);
            // if has no sub-factions, check for factions with same parent
            if (!flist && factionEntry->team &&
                factionEntry->spilloverRateOut != 0.0f) {
                spillOverRepOut *= factionEntry->spilloverRateOut;
                if (FactionEntry const* parent =
                        sFactionStore.LookupEntry(factionEntry->team)) {
                    FactionStateList::iterator parentState =
                        _factions.find(parent->reputationListID);
                    // some team factions have own reputation standing, in this
                    // case do not spill to other sub-factions
                    if (parentState != _factions.end() &&
                        (parentState->second.Flags & FACTION_FLAG_SPECIAL)) {
                        SetOneFactionReputation(
                            parent, spillOverRepOut, incremental);
                    }
            ...
            if (flist) {
                ...
                for (SimpleFactionsList::const_iterator itr = flist->begin();
                     itr != flist->end();
                     ++itr) {
                    if (FactionEntry const* factionEntryCalc =
                            sFactionStore.LookupEntry(*itr)) {
                        if (factionEntryCalc == factionEntry ||
                            GetRank(factionEntryCalc) >
                                ReputationRank(
                                    factionEntryCalc->spilloverMaxRankIn))
                            continue;
                        float spilloverRep =
                            spillOverRepOut * factionEntryCalc->spilloverRateIn;
                        if (spilloverRep != 0 || !incremental)
                            res = SetOneFactionReputation(
                                factionEntryCalc, spilloverRep, incremental);
                    }
                }
            }
        }
    ...
    FactionStateList::iterator faction =
        _factions.find(factionEntry->reputationListID);
    if (faction != _factions.end()) {
        // Xinef: if we update spillover only, do not update main reputation
        // (rank exceeds creature reward rate)
        if (!spillOverOnly) {
            res = SetOneFactionReputation(
                factionEntry, standing, incremental, repMaxCap);
        }

        // only this faction gets reported to client, even if it has no own
        // visible standing
                                            ...
                                            Optional<ReputationRank> repMaxCap)
{
    ...
    if (itr != _factions.end()) {
        ...
        ReputationRank old_rank =
            ReputationToRank(itr->second.Standing + BaseRep);
        ...
        if (sScriptMgr->OnPlayerReputationChange(
            ...
            if (new_rank != old_rank) {
                sScriptMgr->OnPlayerReputationRankChange(_player,
                                                         factionEntry->ID,
                                                         new_rank,
                                                         old_rank,
...
void ReputationMgr::SetVisible(FactionTemplateEntry const* factionTemplateEntry)
{
    ...
    if (FactionEntry const* factionEntry =
        ...
        if (!(factionEntry->BaseRepRaceMask[1] & _player->getRaceMask() &&
              factionEntry->BaseRepValue[1] == Reputation_Bottom))
...
void ReputationMgr::SetVisible(FactionState* faction)
{
    ...
    if (faction->Flags &
            (FACTION_FLAG_INVISIBLE_FORCED | FACTION_FLAG_HIDDEN) &&
...
void ReputationMgr::LoadFromDB(PreparedQueryResult result)
{
    ...
    // QueryResult* result = CharacterDatabase.Query("SELECT faction, standing,
    // flags FROM character_reputation WHERE guid = '{}'",
    // GetGUID().GetCounter());
    ...
    if (result) {
        do {
            ...
            FactionEntry const* factionEntry =
                sFactionStore.LookupEntry(fields[0].Get<uint16>());
            if (factionEntry && (factionEntry->reputationListID >= 0)) {
                FactionState* faction =
                    &_factions[factionEntry->reputationListID];
                ...
                ReputationRank new_rank =
                    ReputationToRank(BaseRep + faction->Standing);
                UpdateRankCounters(old_rank, new_rank);

                uint32 dbFactionFlags = fields[2].Get<uint16>();

                if (dbFactionFlags & FACTION_FLAG_VISIBLE)
                    SetVisible(faction); // have internal checks for forced
                                         // invisibility

                if (dbFactionFlags & FACTION_FLAG_INACTIVE)
                    SetInactive(faction, true); // have internal checks for
                ...
                if (dbFactionFlags & FACTION_FLAG_AT_WAR) // DB at war
                {
                    ...
                    SetAtWar(
                        faction,
                        bool(!(dbFactionFlags & FACTION_FLAG_HIDDEN) ||
                             GetRank(factionEntry) <
                                 REP_NEUTRAL)); // have internal checks for
                                                // FACTION_FLAG_PEACE_FORCED
                }
                else // DB not at war
                {
                    // allow remove if visible (and then not
                    // FACTION_FLAG_INVISIBLE_FORCED or FACTION_FLAG_HIDDEN)
                    if (faction->Flags & FACTION_FLAG_VISIBLE)
                        SetAtWar(faction, false); // have internal checks for
                                                  // FACTION_FLAG_PEACE_FORCED
                }
