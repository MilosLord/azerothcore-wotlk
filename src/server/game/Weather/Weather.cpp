...
bool Weather::Update(uint32 diff)
{
    ...
    if (m_timer.Passed()) {
        ...
        if (ReGenerate()) {
            ///- Weather will be removed if not updated (no players in zone
            ///anymore)
            if (!UpdateWeather())
                return false;
        }
...
bool Weather::ReGenerate()
{
    ...
    LOG_DEBUG("weather",
              "Generating a change in {} weather for zone {}.",
              seasonName[season],
              m_zone);
    ...
    // At this point, only weather that isn't doing anything remains but that
    // have weather data
...
bool Weather::UpdateWeather()
{
    ...
    LOG_DEBUG(
        "weather", "Change the weather of zone {} to {}.", m_zone, wthstr);
