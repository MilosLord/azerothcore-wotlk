...
void LoadWeatherData()
{
    ...
    do {
        ...
        for (uint8 season = 0; season < WEATHER_SEASONS; ++season) {
            wzc.data[season].rainChance =
                fields[season * (MAX_WEATHER_TYPE - 1) + 1].Get<uint8>();
            wzc.data[season].snowChance =
                fields[season * (MAX_WEATHER_TYPE - 1) + 2].Get<uint8>();
            wzc.data[season].stormChance =
                fields[season * (MAX_WEATHER_TYPE - 1) + 3].Get<uint8>();

            if (wzc.data[season].rainChance > 100) {
                wzc.data[season].rainChance = 25;
                LOG_ERROR("sql.sql",
                          "Weather for zone {} season {} has wrong rain chance "
                          "> 100%",
                          zone_id,
                          season);
            }
            ...
            if (wzc.data[season].snowChance > 100) {
                ...
                LOG_ERROR("sql.sql",
                          "Weather for zone {} season {} has wrong snow chance "
                          "> 100%",
                          zone_id,
                          season);
            }
            ...
            if (wzc.data[season].stormChance > 100) {
                ...
                LOG_ERROR("sql.sql",
                          "Weather for zone {} season {} has wrong storm "
                          "chance > 100%",
                          zone_id,
                          season);
            }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Weather Definitions in {} ms",
             count,
