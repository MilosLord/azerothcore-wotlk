...
struct VisibleNotifier {
    ...
    VisibleNotifier(Player& player, bool gobjOnly, bool largeOnly)
        : i_player(player), vis_guids(player.m_clientGUIDs),
          i_visibleNow(player.m_newVisible), i_gobjOnly(gobjOnly),
...
struct MessageDistDeliverer {
    ...
    MessageDistDeliverer(WorldObject const* src,
                         WorldPacket const* msg,
                         float              dist,
                         bool               own_team_only = false,
                         Player const*      skipped       = nullptr)
        : i_source(src), i_message(msg), i_phaseMask(src->GetPhaseMask()),
          i_distSq(dist * dist),
          teamId((own_team_only && src->GetTypeId() == TYPEID_PLAYER)
                     ? src->ToPlayer()->GetTeamId()
                     : TEAM_NEUTRAL),
...
struct MessageDistDelivererToHostile {
    ...
    {
        // never send packet to self
        if (player == i_source || !player->HaveAtClient(i_source) ||
...
struct ObjectUpdater {
    uint32 i_timeDiff;
    bool   i_largeOnly;
    explicit ObjectUpdater(const uint32 diff, bool largeOnly)
        : i_timeDiff(diff), i_largeOnly(largeOnly)
    {
    }
    template <class T>
    void Visit(GridRefMgr<T>& m);
...
// Generic base class to insert elements into arbitrary containers using
// push_back
...
protected:
    ...
    void Insert(Type type) { inserter(ref, std::move(type)); }
};
...
struct WorldObjectSearcher {
    ...
    WorldObjectSearcher(WorldObject const* searcher,
                        WorldObject*&      result,
                        Check&             check,
                        uint32             mapTypeMask = GRID_MAP_TYPE_MASK_ALL)
        : i_mapTypeMask(mapTypeMask), i_phaseMask(searcher->GetPhaseMask()),
...
struct WorldObjectLastSearcher {
    ...
    WorldObjectLastSearcher(WorldObject const* searcher,
                            WorldObject*&      result,
                            Check&             check,
                            uint32 mapTypeMask = GRID_MAP_TYPE_MASK_ALL)
        : i_mapTypeMask(mapTypeMask), i_phaseMask(searcher->GetPhaseMask()),
...
struct WorldObjectWorker {
    ...
    WorldObjectWorker(WorldObject const* searcher,
                      Do const&          _do,
                      uint32             mapTypeMask = GRID_MAP_TYPE_MASK_ALL)
        : i_mapTypeMask(mapTypeMask), i_phaseMask(searcher->GetPhaseMask()),
          i_do(_do)
...
struct GameObjectSearcher {
    ...
    GameObjectSearcher(WorldObject const* searcher,
                       GameObject*&       result,
                       Check&             check)
        : i_phaseMask(searcher->GetPhaseMask()), i_object(result),
          i_check(check)
    {
    }
...
struct GameObjectLastSearcher {
    ...
    GameObjectLastSearcher(WorldObject const* searcher,
                           GameObject*&       result,
                           Check&             check)
        : i_phaseMask(searcher->GetPhaseMask()), i_object(result),
          i_check(check)
...
// Last accepted by Check Unit if any (Check can change requirements at each
// call)
template <class Check>
...
struct UnitListSearcher : ContainerInserter<Unit*> {
    ...
    template <typename Container>
    UnitListSearcher(WorldObject const* searcher,
                     Container&         container,
                     Check&             check)
...
struct CreatureSearcher {
    ...
    CreatureSearcher(WorldObject const* searcher,
                     Creature*&         result,
                     Check&             check)
        : i_phaseMask(searcher->GetPhaseMask()), i_object(result),
          i_check(check)
...
// Last accepted by Check Creature if any (Check can change requirements at each
// call)
template <class Check>
struct CreatureLastSearcher {
    ...
    CreatureLastSearcher(WorldObject const* searcher,
                         Creature*&         result,
                         Check&             check)
        : i_phaseMask(searcher->GetPhaseMask()), i_object(result),
          i_check(check)
...
struct PlayerListSearcher : ContainerInserter<Player*> {
    ...
    template <typename Container>
    PlayerListSearcher(WorldObject const* searcher,
                       Container&         container,
                       Check&             check)
...
struct PlayerListSearcherWithSharedVision {
    ...
    PlayerListSearcherWithSharedVision(WorldObject const*  searcher,
                                       std::list<Player*>& objects,
                                       Check&              check)
        : i_phaseMask(searcher->GetPhaseMask()), i_objects(objects),
          i_check(check)
    {
    }
...
struct PlayerLastSearcher {
    ...
    PlayerLastSearcher(WorldObject const* searcher,
                       Player*&           result,
                       Check&             check)
        : i_phaseMask(searcher->GetPhaseMask()), i_object(result),
          i_check(check)
...
struct PlayerDistWorker {
    ...
    {
        for (PlayerMapType::iterator itr = m.begin(); itr != m.end(); ++itr)
            if (itr->GetSource()->HaveAtClient(i_searcher) &&
...
public:
    AnyDeadUnitObjectInRangeCheck(Unit* searchObj, float range)
        : i_searchObj(searchObj), i_range(range)
    {
    }
...
class AnyDeadUnitSpellTargetInRangeCheck
    : public AnyDeadUnitObjectInRangeCheck {
public:
    AnyDeadUnitSpellTargetInRangeCheck(Unit*                 searchObj,
                                       float                 range,
...
// Find the nearest Fishing hole and return true only if source object is in
// range of hole
...
public:
    ...
    bool operator()(GameObject* go)
    {
        if (i_obj.IsWithinDistInMap(go, i_range)) {
            i_range = i_obj.GetDistance(
                go); // use found GO range as new range limit for next check
            return true;
        }
        return false;
    }
...
// Success at unit in range, range update for next check (this can be use with
// GameobjectLastSearcher to find nearest GO)
...
// Success at unit in range, range update for next check (this can be use with
// GameobjectLastSearcher to find nearest GO with a certain type)
class NearestGameObjectTypeInObjectRangeCheck {
public:
    NearestGameObjectTypeInObjectRangeCheck(WorldObject const& obj,
                                            GameobjectTypes    type,
                                            float              range)
        : i_obj(obj), i_type(type), i_range(range)
    {
    }
    bool operator()(GameObject* go)
    {
...
private:
    ...
    NearestGameObjectTypeInObjectRangeCheck(
        NearestGameObjectTypeInObjectRangeCheck const&);
};
...
public:
    MostHPPercentMissingInRange(Unit const* obj,
                                float       range,
                                uint32      minHpPct,
                                uint32      maxHpPct)
        : i_obj(obj), i_range(range), i_minHpPct(minHpPct),
          i_maxHpPct(maxHpPct), i_hpPct(101.f)
    {
    }
    ...
    bool operator()(Unit* u)
    {
        if (u->IsAlive() && u->IsInCombat() && !i_obj->IsHostileTo(u) &&
            i_obj->IsWithinDistInMap(u, i_range) &&
            i_minHpPct <= u->GetHealthPct() &&
            u->GetHealthPct() <= i_maxHpPct && u->GetHealthPct() < i_hpPct) {
    ...
    bool operator()(Unit* u)
    {
        if (u->IsAlive() && u->IsInCombat() && !i_obj->IsHostileTo(u) &&
            ...
            (u->isFeared() || u->IsCharmed() || u->isFrozen() ||
             u->HasUnitState(UNIT_STATE_STUNNED) ||
             u->HasUnitState(UNIT_STATE_CONFUSED))) {
            return true;
        }
        return false;
    }
...
public:
        ...
        : i_obj(obj), i_range(range)
    {
        ...
        if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(spellid))
            if (SpellInfo const* newSpell =
                    sSpellMgr->GetSpellForDifficultyFromSpell(
                        spell, const_cast<Unit*>(obj)))
                i_spell = newSpell->Id;
    }
    bool operator()(Unit* u)
    {
        if (u->IsAlive() && u->IsInCombat() && !i_obj->IsHostileTo(u) &&
            i_obj->IsWithinDistInMap(u, i_range) && !(u->HasAura(i_spell))) {
            return true;
        }
        return false;
    }
...
public:
    ...
    bool operator()(Unit* u)
    {
        if (u->IsAlive() && !u->IsCritter() &&
            i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) &&
            (i_funit->GetTypeId() != TYPEID_UNIT ||
             !i_funit->ToCreature()->IsAvoidingAOE())) // pussywizard
            return true;
        else
            return false;
    }
...
public:
    ...
    bool operator()(Unit* u)
    {
        ...
        if (u->GetTypeId() == TYPEID_UNIT &&
            (u->ToCreature()->IsTotem() || u->ToCreature()->IsTrigger() ||
             u->ToCreature()
                 ->IsAvoidingAOE())) // pussywizard: added IsAvoidingAOE()
            return false;
...
public:
    NearestAttackableNoTotemUnitInObjectRangeCheck(WorldObject const* obj,
                                                   Unit const*        owner,
                                                   float              range)
        : i_obj(obj), i_owner(owner), i_range(range)
    {
    }
    ...
    bool operator()(Unit* u)
    {
        ...
        if (!i_obj->IsWithinDistInMap(u, i_range) ||
            !i_owner->IsValidAttackTarget(u) ||
            !i_obj->IsWithinLOSInMap(u,
...
public:
    AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck(Unit const* funit,
                                                         float       range)
    ...
    {
        return u->IsAlive() && i_funit->IsWithinDistInMap(u, i_range) &&
               !i_funit->IsFriendlyTo(u) && i_funit->IsValidAttackTarget(u) &&
               !u->IsCritter() && !u->IsTotem() // xinef: dont attack totems
            /*&& i_funit->CanSeeOrDetect(u)*/;  // pussywizard: already checked
                                                // in IsValidAttackTarget(u)
    }
...
public:
    AnyFriendlyUnitInObjectRangeCheck(WorldObject const* obj,
                                      Unit const*        funit,
                                      float              range,
                                      bool               playerOnly = false)
        : i_obj(obj), i_funit(funit), i_range(range), i_playerOnly(playerOnly)
    {
    }
    bool operator()(Unit* u)
    {
...
public:
    AnyFriendlyNotSelfUnitInObjectRangeCheck(WorldObject const* obj,
                                             Unit const*        funit,
                                             float              range,
                                             bool playerOnly = false)
        : i_obj(obj), i_funit(funit), i_range(range), i_playerOnly(playerOnly)
...
public:
    AnyGroupedUnitInObjectRangeCheck(WorldObject const* obj,
                                     Unit const*        funit,
                                     float              range,
                                     bool               raid)
        : _source(obj), _refUnit(funit), _range(range), _raid(raid)
    {
    }
    bool operator()(Unit* u)
    {
        ...
        return !_refUnit->IsHostileTo(u) && u->IsAlive() &&
               _source->IsWithinDistInMap(u, _range);
    }
...
// Success at unit in range, range update for next check (this can be use with
// UnitLastSearcher to find nearest unit)
...
public:
    ...
    bool operator()(Unit* u)
    {
        if (u->isTargetableForAttack(true, i_funit) &&
            i_obj->IsWithinDistInMap(u, i_range) &&
            (i_funit->IsInCombatWith(u) || u->IsHostileTo(i_funit)) &&
            i_obj->CanSeeOrDetect(u)) {
            i_range = i_obj->GetDistance(
                u); // use found unit range as new range limit for next check
            return true;
        }
...
public:
        ...
        : i_obj(obj), i_funit(funit), _spellInfo(nullptr), i_range(range)
    {
        ...
        if (i_obj->GetTypeId() == TYPEID_DYNAMICOBJECT)
            _spellInfo =
                sSpellMgr->GetSpellInfo(((DynamicObject*)i_obj)->GetSpellId());
    }
    bool operator()(Unit* u)
    {
        // Check contains checks for: live, non-selectable, non-attackable
        ...
        if (i_funit->_IsValidAttackTarget(
                u,
                _spellInfo,
                i_obj->GetTypeId() == TYPEID_DYNAMICOBJECT ? i_obj : nullptr) &&
            i_obj->IsWithinDistInMap(u, i_range))
            return true;
...
public:
    AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck(
        WorldObject const* obj, Unit const* funit, float range)
        : i_obj(obj), i_funit(funit), i_range(range)
    {
    }
    bool operator()(Unit* u)
    {
        if (!u->IsAlive() ||
            u->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE |
                           UNIT_FLAG_NOT_SELECTABLE) ||
            (u->IsImmuneToPC() && !u->IsInCombat()))
            return false;
...
{
    bool operator()(Unit* u) { return u->GetVisibility() ==
...
public:
    explicit NearestHostileUnitCheck(Creature const* creature,
                                     float           dist       = 0,
                                     bool            playerOnly = false)
        : me(creature), i_playerOnly(playerOnly)
    {
...
public:
    explicit NearestVisibleDetectableContestedGuardUnitCheck(Unit const* unit)
        : me(unit)
    {
    }
    bool operator()(Unit* u)
...
private:
    ...
    NearestVisibleDetectableContestedGuardUnitCheck(
        NearestVisibleDetectableContestedGuardUnitCheck const&);
};
...
public:
    NearestAssistCreatureInCreatureRangeCheck(Creature* obj,
                                              Unit*     enemy,
                                              float     range)
...
// Success at unit in range, range update for next check (this can be use with
// CreatureLastSearcher to find nearest creature)
...
public:
    ...
    bool operator()(Creature* u)
    {
        if (u->GetEntry() == i_entry && u->IsAlive() == i_alive &&
            i_obj.IsWithinDist(u, i_range) && i_obj.InSamePhase(u)) {
            i_range = i_obj.GetDistance(
                u); // use found unit range as new range limit for next check
            return true;
        }
...
public:
    AnyPlayerInObjectRangeCheck(WorldObject const* obj,
                                float              range,
                                bool               reqAlive   = true,
                                bool               disallowGM = false)
        : _obj(obj), _range(range), _reqAlive(reqAlive), _disallowGM(disallowGM)
    {
    }
    bool operator()(Player* u)
    {
...
public:
    AnyPlayerExactPositionInGameObjectRangeCheck(GameObject const* go,
                                                 float             range)
        : _go(go), _range(range)
    {
    }
    bool operator()(Player* u)
    {
        if (!_go->IsInRange(u->GetPositionX(),
                            u->GetPositionY(),
                            u->GetPositionZ(),
                            _range))
            return false;
...
public:
    AllGameObjectsWithEntryInRange(WorldObject const* object,
                                   uint32             entry,
                                   float              maxRange)
        : m_pObject(object), m_uiEntry(entry), m_fRange(maxRange)
    {
    }
    bool operator()(GameObject* go)
    {
        if (go->GetEntry() == m_uiEntry &&
            m_pObject->IsWithinDist(go, m_fRange, false))
            return true;
...
public:
    AllCreaturesOfEntryInRange(WorldObject const* object,
                               uint32             entry,
                               float              maxRange)
        : m_pObject(object), m_uiEntry(entry), m_fRange(maxRange)
    {
...
public:
    ...
    bool operator()(Unit* u)
    {
        ...
        if (!group || !group->IsMember(i_obj->IsPet() ? i_obj->GetOwnerGUID()
                                                      : i_obj->GetGUID())) {
            return false;
        }

        if (u->IsAlive() && !i_obj->IsHostileTo(u) &&
            i_obj->IsWithinDistInMap(u, i_range) &&
            u->GetMaxHealth() - u->GetHealth() > i_hp) {
            i_hp = u->GetMaxHealth() - u->GetHealth();
            return true;
...
public:
    AllDeadCreaturesInRange(WorldObject const* obj,
                            float              range,
                            bool               reqAlive = true)
...
public:
    PlayerAtMinimumRangeAway(Unit const* unit, float fMinRange)
        : unit(unit), fRange(fMinRange)
    {
    }
    bool operator()(Player* player)
    {
        // No threat list check, must be done explicit if expected to be in
...
public:
    GameObjectInRangeCheck(
        float _x, float _y, float _z, float _range, uint32 _entry = 0)
...
class UnitAuraCheck {
public:
    UnitAuraCheck(bool       present,
                  uint32     spellId,
                  ObjectGuid casterGUID = ObjectGuid::Empty)
        : _present(present), _spellId(spellId), _casterGUID(casterGUID)
    {
    }
    ...
    bool operator()(WorldObject const* object) const
    {
        return object->ToUnit() &&
               object->ToUnit()->HasAura(_spellId, _casterGUID) == _present;
    }
...
public:
    AllWorldObjectsInExactRange(WorldObject const* object,
                                float              range,
                                bool               equals)
        : _object(object), _range(range), _equals(equals)
    {
    }
    bool operator()(WorldObject const* object)
    {
...
public:
    explicit PowerCheck(Powers const power, bool equals)
        : _power(power), _equals(equals)
    {
    }
    bool operator()(WorldObject const* object) const
    {
        return object->ToUnit() &&
               (object->ToUnit()->getPowerType() == _power) == _equals;
    }
...
public:
    explicit RaidCheck(Unit const* compare, bool equals)
        : _compare(compare), _equals(equals)
    {
    }
    bool operator()(WorldObject const* object) const
    {
        return object->ToUnit() &&
               object->ToUnit()->IsInRaidWith(_compare) == _equals;
    }
...
private:
    Builder& i_builder;
    std::vector<WorldPacket*>
        i_data_cache; // 0 = default, i => i-1 locale index
};
