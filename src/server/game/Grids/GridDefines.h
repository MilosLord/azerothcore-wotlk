...
// Creature used instead pet to simplify *::Visit templates (not required
// duplicate code for Creature->Pet case)
typedef TYPELIST_5(GameObject,
                   Player,
                   Creature /*pets*/,
                   Corpse /*resurrectable*/,
                   DynamicObject /*farsight target*/) AllWorldObjectTypes;
typedef TYPELIST_4(GameObject,
                   Creature /*except pets*/,
                   DynamicObject,
                   Corpse /*Bones*/) AllGridObjectTypes;
typedef TYPELIST_5(Creature, GameObject, DynamicObject, Pet, Corpse)
...
typedef NGrid<MAX_NUMBER_OF_CELLS,
              Player,
              AllWorldObjectTypes,
              AllGridObjectTypes>
    NGridType;
...
typedef TypeUnorderedMapContainer<AllMapStoredObjectTypes, ObjectGuid>
...
{
    // calculate and store temporary values in double format for having same
    // result as same mySQL calculations
...
{
    return Compute<GridCoord, CENTER_GRID_ID>(
        x, y, CENTER_GRID_OFFSET, SIZE_OF_GRIDS);
}

inline CellCoord ComputeCellCoord(float x, float y)
{
    return Compute<CellCoord, CENTER_GRID_CELL_ID>(
        x, y, CENTER_GRID_CELL_OFFSET, SIZE_OF_GRID_CELL);
}
