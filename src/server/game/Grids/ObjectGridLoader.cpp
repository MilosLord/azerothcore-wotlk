...
template <class T>
void AddObjectHelper(
    CellCoord& cell, GridRefMgr<T>& m, uint32& count, Map* /*map*/, T* obj)
{
    obj->AddToGrid(m);
...
template <>
void AddObjectHelper(
    CellCoord& cell, CreatureMapType& m, uint32& count, Map* map, Creature* obj)
{
    obj->AddToGrid(m);
    ObjectGridLoader::SetObjectCell(obj, cell);
    obj->AddToWorld();
                ...
                Map*                  map)
{
    for (CellGuidSet::const_iterator i_guid = guid_set.begin();
        ...
        if (!obj->IsMoveInLineOfSightDisabled() &&
            obj->GetDefaultMovementType() == IDLE_MOTION_TYPE &&
            !obj->isNeedNotify(NOTIFY_VISIBILITY_CHANGED |
                               NOTIFY_AI_RELOCATION)) {
            if (obj->IsAlive() && !obj->HasUnitState(UNIT_STATE_SIGHTLESS) &&
                obj->HasReactState(REACT_AGGRESSIVE) && !obj->IsImmuneToNPC()) {
                ...
                Map*                    map)
{
    for (CellGuidSet::const_iterator i_guid = guid_set.begin();
        ...
        GameObject*           obj =
            data && sObjectMgr->IsGameObjectStaticTransport(data->id)
                          ? new StaticTransport()
                          : new GameObject();
...
void ObjectGridLoader::Visit(GameObjectMapType& m)
{
    ...
    CellObjectGuids const& cell_guids = sObjectMgr->GetCellObjectGuids(
        i_map->GetId(), i_map->GetSpawnMode(), cellCoord.GetId());
...
{
    CellCoord cellCoord = i_cell.GetCellCoord();
    if (std::unordered_set<Corpse*> const* corpses =
            i_map->GetCorpsesInCell(cellCoord.GetId())) {
...
void ObjectGridLoader::LoadN(void)
{
    ...
    LOG_DEBUG("maps",
              "{} GameObjects, {} Creatures, and {} Corpses/Bones loaded for "
              "grid {} on map {}",
              i_gameObjects,
              i_creatures,
              i_corpses,
...
void ObjectGridUnloader::Visit(GridRefMgr<T>& m)
{
    while (!m.IsEmpty()) {
        ...
        //    obj->SaveRespawnTime();
        // Some creatures may summon other temp summons in
        // CleanupsBeforeDelete() So we need this even after cleaner (maybe we
        // can remove cleaner) Example: Flame Leviathan Turret 33139 is summoned
        // when a creature is deleted
        // TODO: Check if that script has the correct logic. Do we really need
        // to summons something before deleting?
