...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    ...
    CellCoord centerX =
        Acore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY =
        Acore::ComputeCellCoord(x + radius, y + radius).normalize();
...
inline void Cell::Visit(CellCoord const&                    standing_cell,
                        ...
                        float                               radius) const
{
    ...
    // we could have problems with huge creatures, which won't attack nearest
    // players etc
    Visit(standing_cell,
          visitor,
          map,
                        ...
                        float                               radius) const
{
    ...
    // issue?) maybe it is better to just return when radius <= 0.0f?
    if (radius <= 0.0f) {
    ...
    // if radius is known to reach cell area more than 4x4 then we should call
    // optimized VisitCircle currently this technique works with
    // MAX_NUMBER_OF_CELLS 16 and higher, with lower values there are nothing to
    // optimize because SIZE_OF_GRID_CELL is too big...
...
template <class T, class CONTAINER>
inline void Cell::VisitCircle(TypeContainerVisitor<T, CONTAINER>& visitor,
                              Map&                                map,
                              ...
                              CellCoord const& end_cell) const
{
    ...
    for (uint32 step = 1; step <= (x_start - begin_cell.x_coord); ++step) {
        ...
        for (uint32 y = y_start; y >= y_end; --y) {
            // we visit cells symmetrically from both sides, heading from center
            // to sides and from up to bottom e.g. filling 2 trapezoids after
            // filling central cell strip...
...
template <class T>
inline void Cell::VisitGridObjects(WorldObject const* center_obj,
                                   T&                 visitor,
                                   float              radius,
                                   bool               dont_load /*= true*/)
{
    CellCoord p(Acore::ComputeCellCoord(center_obj->GetPositionX(),
                                        center_obj->GetPositionY()));
    Cell      cell(p);
    if (dont_load) {
        cell.SetNoCreate();
    }
...
template <class T>
inline void Cell::VisitWorldObjects(WorldObject const* center_obj,
                                    T&                 visitor,
                                    float              radius,
                                    bool               dont_load /*= true*/)
{
...
template <class T>
inline void Cell::VisitAllObjects(WorldObject const* center_obj,
                                  T&                 visitor,
                                  float              radius,
                                  bool               dont_load /*= true*/)
{
...
template <class T>
inline void Cell::VisitGridObjects(float x,
                                   float y,
                                   Map*  map,
                                   T&    visitor,
                                   float radius,
...
template <class T>
inline void Cell::VisitWorldObjects(float x,
                                    float y,
                                    Map*  map,
                                    T&    visitor,
                                    float radius,
...
template <class T>
inline void Cell::VisitAllObjects(float x,
                                  float y,
                                  Map*  map,
                                  T&    visitor,
                                  float radius,
