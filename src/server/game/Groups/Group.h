...
enum GroupType {
    ...
    GROUPTYPE_LFG            = 0x08,
    // 0x10, leave/change group?, I saw this flag when leaving group and after
    // leaving BG while in group GROUPTYPE_ONE_PERSON_PARTY   = 0x20, 4.x
    // Script_IsOnePersonParty() GROUPTYPE_EVERYONE_ASSISTANT = 0x40  4.x
...
enum GroupUpdateFlags {
    ...
    GROUP_UPDATE_FLAG_PET_NAME =
        0x00000800, // pet name, nullptr terminated string
    ...
    GROUP_UPDATE_FLAG_PET_AURAS =
        0x00040000, // uint64 mask, for each bit set uint32 spellid + uint8 unk,
                    // pet auras...
...
public:
    ...
    PlayerVote
        playerVote; // vote position correspond with player position (in group)
...
public:
    ...
    void LoadMemberFromDB(ObjectGuid::LowType guidLow,
                          uint8               memberFlags,
                          uint8               subgroup,
                          uint8               roles);
    ...
    CanJoinBattlegroundQueue(Battleground const*     bgTemplate,
    ...
    void SendLootStartRollToPlayer(
        uint32 countDown, uint32 mapId, Player* p, bool canNeed, Roll const& r);
    void            SendLootRoll(ObjectGuid  SourceGuid,
                                 ObjectGuid  TargetGuid,
                                 uint8       RollNumber,
                                 uint8       RollType,
                                 const Roll& r);
    void            SendLootRollWon(ObjectGuid  SourceGuid,
                                    ObjectGuid  TargetGuid,
                                    uint8       RollNumber,
                                    uint8       RollType,
                                    const Roll& r);
    ...
    void AddLfgBuffFlag()
    {
        m_lfgGroupFlags |= GROUP_LFG_FLAG_APPLY_RANDOM_BUFF;
    }
    void AddLfgRandomInstanceFlag()
    {
        m_lfgGroupFlags |= GROUP_LFG_FLAG_IS_RANDOM_INSTANCE;
    }
    ...
    {
        return isLFGGroup() && (m_lfgGroupFlags & GROUP_LFG_FLAG_IS_HEROIC);
    }
    ...
    {
        return _difficultyChangePreventionType;
    }
