...
// Proof of concept, we should shift the info we're obtaining in here into
// AuctionEntry probably
                        ...
                        bool                    checkMinBidBuyout)
{
    for (auto& thisOrder : sortOrder) {
            ...
            if (left->bid == right->bid) {
                if (checkMinBidBuyout) {
                    if (left->buyout == right->buyout) {
                        ...
                        return thisOrder.isDesc
                                   ? left->startbid > right->startbid
                                   : left->startbid < right->startbid;
                    }

                    return thisOrder.isDesc ? left->buyout > right->buyout
                                            : left->buyout < right->buyout;
                }

                continue;
            }

            return thisOrder.isDesc ? left->bid > right->bid
                                    : left->bid < right->bid;
        }
        ...
        case AUCTION_SORT_ITEM: {
            ItemTemplate const* protoLeft =
                sObjectMgr->GetItemTemplate(left->item_template);
            ItemTemplate const* protoRight =
                sObjectMgr->GetItemTemplate(right->item_template);
            ...
            if (locale > LOCALE_enUS) {
                if (ItemLocale const* leftIl =
                        sObjectMgr->GetItemLocale(protoLeft->ItemId)) {
                    ObjectMgr::GetLocaleString(leftIl->Name, locale, leftName);
                }

                if (ItemLocale const* rightIl =
                        sObjectMgr->GetItemLocale(protoRight->ItemId)) {
                    ObjectMgr::GetLocaleString(
                        rightIl->Name, locale, rightName);
                }
            }
        ...
        case AUCTION_SORT_MINLEVEL: {
            ...
            return thisOrder.isDesc
                       ? protoLeft->RequiredLevel > protoRight->RequiredLevel
                       : protoLeft->RequiredLevel < protoRight->RequiredLevel;
        }
        ...
        case AUCTION_SORT_RARITY: {
            ...
            return thisOrder.isDesc ? protoLeft->Quality > protoRight->Quality
                                    : protoLeft->Quality < protoRight->Quality;
        }
        ...
        case AUCTION_SORT_TIMELEFT: {
            ...
            return thisOrder.isDesc ? left->expire_time > right->expire_time
        ...
        case AUCTION_SORT_MINBIDBUY: {
            if (left->buyout == right->buyout) {
                ...
                return thisOrder.isDesc ? left->startbid > right->startbid
                                        : left->startbid < right->startbid;
            }
...
AuctionHouseObject* AuctionHouseMgr::GetAuctionsMap(uint32 factionTemplateId)
{
    ...
    FactionTemplateEntry const* u_entry =
        sFactionTemplateStore.LookupEntry(factionTemplateId);
    if (!u_entry)
        return &_neutralAuctions;
...
AuctionHouseObject*
AuctionHouseMgr::GetAuctionsMapByHouseId(uint8 auctionHouseId)
{
    if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
        return &_neutralAuctions;
                                          ...
                                          uint32                   count)
{
    ...
    uint32 deposit    = uint32(((multiplier * MSV * count / 3) * timeHr * 3) *
                            sWorld->getRate(RATE_AUCTION_DEPOSIT));
                                         ...
                                         bool sendMail)
{
    ...
    if (bidder) {
        ...
        bidder_accId =
            sCharacterCache->GetCharacterAccountIdByGuid(auction->bidder);
    }

    // receiver exist
    if (bidder || bidder_accId) {
        sScriptMgr->OnBeforeAuctionHouseMgrSendAuctionWonMail(
            this,
            auction,
            bidder,
            bidder_accId,
            ...
            sendMail);
        // set owner to bidder (to prevent delete item with sender char
        // deleting) owner in `data` will set at mail receive and item
        // extracting
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_UPD_ITEM_OWNER);
        ...
        if (bidder) {
            if (sendNotification) // can be changed in the hook
                bidder->GetSession()->SendAuctionBidderNotification(
                    auction->GetHouseId(),
                    auction->Id,
                    auction->bidder,
                    0,
            ...
            if (updateAchievementCriteria) // can be changed in the hook
                bidder->UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_WON_AUCTIONS, 1);
        }

        if (sendMail) // can be changed in the hook
            MailDraft(auction->BuildAuctionMailSubject(AUCTION_WON),
                      AuctionEntry::BuildAuctionMailBody(
                          auction->owner, auction->bid, auction->buyout))
                .AddItem(pItem)
                .SendMailTo(trans,
                            MailReceiver(bidder, auction->bidder.GetCounter()),
                            auction,
...
void AuctionHouseMgr::SendAuctionSalePendingMail(
    AuctionEntry* auction, CharacterDatabaseTransaction trans, bool sendMail)
{
    ...
    if (owner || owner_accId) {
        sScriptMgr->OnBeforeAuctionHouseMgrSendAuctionSalePendingMail(
            this, auction, owner, owner_accId, sendMail);

        uint32 deliveryDelay = sWorld->getIntConfig(CONFIG_MAIL_DELIVERY_DELAY);

        ByteBuffer timePacker;
        timePacker.AppendPackedTime(GameTime::GetGameTime().count() +
                                    time_t(deliveryDelay));

        if (sendMail) // can be changed in the hook
            MailDraft(
                auction->BuildAuctionMailSubject(AUCTION_SALE_PENDING),
                AuctionEntry::BuildAuctionMailBody(auction->bidder,
                                                   auction->bid,
                                                   auction->buyout,
                                                   auction->deposit,
                                                   auction->GetAuctionCut(),
                                                   deliveryDelay,
                                                   timePacker.read<uint32>()))
                .SendMailTo(trans,
                            MailReceiver(owner, auction->owner.GetCounter()),
                            auction,
                            MAIL_CHECK_MASK_COPIED);
    }
}

// call this method to send mail to auction owner, when auction is successful,
    ...
    bool                         sendMail)
{
    ...
    if (owner || owner_accId) {
        uint32 profit =
            auction->bid + auction->deposit - auction->GetAuctionCut();
        sScriptMgr->OnBeforeAuctionHouseMgrSendAuctionSuccessfulMail(
            this,
            auction,
            owner,
            owner_accId,
        ...
        if (owner) {
            if (updateAchievementCriteria) // can be changed in the hook
            {
                owner->UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_GOLD_EARNED_BY_AUCTIONS, profit);
                owner->UpdateAchievementCriteria(
        ...
        if (sendMail) // can be changed in the hook
            MailDraft(
                auction->BuildAuctionMailSubject(AUCTION_SUCCESSFUL),
                AuctionEntry::BuildAuctionMailBody(auction->bidder,
                                                   auction->bid,
                                                   auction->buyout,
                                                   auction->deposit,
        ...
        if (auction->bid >= 500 * GOLD)
            if (CharacterCacheEntry const* gpd =
                    sCharacterCache->GetCharacterCacheByGuid(auction->bidder)) {
                Player* bidder =
                    ObjectAccessor::FindConnectedPlayer(auction->bidder);
                std::string owner_name  = "";
                uint8       owner_level = 0;
                if (CharacterCacheEntry const* gpd_owner =
                ...
                CharacterDatabase.Execute(
                    ...
                    "\"{}\", {}, \"profit: {}g, bidder: {} {} lvl (guid: {}), "
                    "seller: {} {} lvl (guid: {}), item {} ({})\", NOW(), {})",
                    gpd->AccountId,
                    auction->bidder.GetCounter(),
                    gpd->Name,
                    bidder ? bidder->GetSession()->GetRemoteAddress() : "",
                    owner_accId,
                    owner_name,
                    auction->bid,
                    (profit / GOLD),
                                             ...
                                             bool sendMail)
{
    ...
    // owner exist
    if (owner || owner_accId) {
        sScriptMgr->OnBeforeAuctionHouseMgrSendAuctionExpiredMail(
        ...
        if (sendMail) // can be changed in the hook
            MailDraft(
                auction->BuildAuctionMailSubject(AUCTION_EXPIRED),
                AuctionEntry::BuildAuctionMailBody(
                    ObjectGuid::Empty, 0, auction->buyout, auction->deposit))
                .AddItem(pItem)
                .SendMailTo(trans,
                            MailReceiver(owner, auction->owner.GetCounter()),
                            auction,
...
void AuctionHouseMgr::SendAuctionOutbiddedMail(
    AuctionEntry*                auction,
    uint32                       newPrice,
    Player*                      newBidder,
    CharacterDatabaseTransaction trans,
    ...
    bool                         sendMail)
{
    ...
    if (oldBidder || oldBidder_accId) {
        sScriptMgr->OnBeforeAuctionHouseMgrSendAuctionOutbiddedMail(
            this,
            auction,
            oldBidder,
            oldBidder_accId,
        ...
        if (oldBidder && newBidder &&
            sendNotification) // can be changed in the hook
        ...
        if (sendMail) // can be changed in the hook
            MailDraft(
                auction->BuildAuctionMailSubject(AUCTION_OUTBIDDED),
                AuctionEntry::BuildAuctionMailBody(auction->owner,
                                                   auction->bid,
                                                   auction->buyout,
                                                   auction->deposit,
                                                   auction->GetAuctionCut()))
                ...
                .SendMailTo(
                    trans,
                    MailReceiver(oldBidder, auction->bidder.GetCounter()),
                    auction,
                    MAIL_CHECK_MASK_COPIED);
    }
}
...
void AuctionHouseMgr::SendAuctionCancelledToBidderMail(
    AuctionEntry* auction, CharacterDatabaseTransaction trans, bool sendMail)
{
    Player* bidder = ObjectAccessor::FindConnectedPlayer(auction->bidder);
    ...
    // bidder exist
    if (bidder || bidder_accId) {
        sScriptMgr->OnBeforeAuctionHouseMgrSendAuctionCancelledToBidderMail(
            this, auction, bidder, bidder_accId, sendMail);
        if (sendMail) // can be changed in the hook
            MailDraft(
                auction->BuildAuctionMailSubject(AUCTION_CANCELLED_TO_BIDDER),
                AuctionEntry::BuildAuctionMailBody(auction->owner,
                                                   auction->bid,
                                                   auction->buyout,
                                                   auction->deposit))
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!_mAitems.empty()) {
        for (ItemMap::iterator itr = _mAitems.begin(); itr != _mAitems.end();
             ++itr)
            delete itr->second;
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_AUCTION_ITEMS);
    PreparedQueryResult result = CharacterDatabase.Query(stmt);

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 auction items. DB table `auctionhouse` or "
                 "`item_instance` is empty!");
    ...
    do {
        ...
        if (!proto) {
            LOG_ERROR("auctionHouse",
                      "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: "
                      "{} id: #{}) in auction, skipped.",
                      item_guid,
                      item_template);
            continue;
        }
        ...
        if (!item->LoadFromDB(
                item_guid, ObjectGuid::Empty, fields, item_template)) {
            delete item;
            continue;
        }
        AddAItem(item);
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_AUCTIONS);
    PreparedQueryResult result = CharacterDatabase.Query(stmt);
...
bool AuctionHouseMgr::RemoveAItem(
    ObjectGuid                    itemGuid,
    bool                          deleteFromDB,
...
AuctionHouseEntry const*
AuctionHouseMgr::GetAuctionHouseEntry(uint32 factionTemplateId)
{
    uint32 houseid = AUCTIONHOUSE_NEUTRAL; // goblin auction house

    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION)) {
        // FIXME: found way for proper auctionhouse selection by another way
        //  AuctionHouse.dbc have faction field with _player_ factions
        //  associated with auction house races. but no easy way convert
        //  creature faction to player race faction for specific city
        FactionTemplateEntry const* u_entry =
            sFactionTemplateStore.LookupEntry(factionTemplateId);
        if (!u_entry)
...
AuctionHouseEntry const*
AuctionHouseMgr::GetAuctionHouseEntryFromHouse(uint8 houseId)
{
    return (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
               ? sAuctionHouseStore.LookupEntry(AUCTIONHOUSE_NEUTRAL)
...
void AuctionHouseObject::Update()
{
    ...
    for (AuctionEntryMap::iterator itr, iter = _auctionsMap.begin();
         iter != _auctionsMap.end();) {
        itr                   = iter++;
...
void AuctionHouseObject::BuildListBidderItems(WorldPacket& data,
                                              ...
                                              uint32&      totalcount)
{
    for (AuctionEntryMap::const_iterator itr = _auctionsMap.begin();
         itr != _auctionsMap.end();
         ++itr) {
...
bool AuctionHouseObject::BuildListAuctionItems(
    WorldPacket&        data,
    Player*             player,
    std::wstring const& wsearchedname,
    uint32              listfrom,
    uint8               levelmin,
    uint8               levelmax,
    uint8               usable,
    ...
    Milliseconds                  searchTimeout)
{
    ...
    if (itemClass == 0xffffffff && itemSubClass == 0xffffffff &&
        inventoryType == 0xffffffff && quality == 0xffffffff &&
        levelmin == 0x00 && levelmax == 0x00 && usable == 0x00 &&
        wsearchedname.empty()) {
        ...
        for (AuctionEntryMap::const_iterator itr = _auctionsMap.begin();
             ...
             ++itr) {
            if ((itrcounter++) % 100 ==
                0) // check condition every 100 iterations
            {
                if (GetMSTimeDiff(GameTime::GetGameTimeMS(), GetTimeMS()) >=
                    searchTimeout) // pussywizard: stop immediately if diff is
                                   // high or waiting too long
                {
                    return false;
                }
            }
            ...
            if (levelmin != 0x00 &&
                (proto->RequiredLevel < levelmin ||
                 (levelmax != 0x00 && proto->RequiredLevel > levelmax))) {
                continue;
            }
            ...
            // Allow search by suffix (ie: of the Monkey) or partial name (ie:
            // Monkey) No need to do any of this if no search term was entered
            if (!wsearchedname.empty()) {
                ...
                if (loc_idx >= 0)
                    if (ItemLocale const* il =
                            sObjectMgr->GetItemLocale(proto->ItemId))
                        ObjectMgr::GetLocaleString(il->Name, loc_idx, name);

                // DO NOT use GetItemEnchantMod(proto->RandomProperty) as it may
                // return a result
                //  that matches the search but it may not equal
                //  item->GetItemRandomPropertyId() used in BuildAuctionInfo()
                //  which then causes wrong items to be listed
                int32 propRefID = item->GetItemRandomPropertyId();

                if (propRefID) {
                    // Append the suffix to the name (ie: of the Monkey) if one
                    // exists These are found in ItemRandomSuffix.dbc and
                    // ItemRandomProperties.dbc even though the DBC name seems
                    // misleading
                    std::array<char const*, 16> const* suffix = nullptr;

                    if (propRefID < 0) {
                        ItemRandomSuffixEntry const* itemRandEntry =
                        ...
                        ItemRandomPropertiesEntry const* itemRandEntry =
                    ...
                    if (suffix) {
                        ...
                        // using localization or default enUS if localization is
                        // invalid
                        name += ' ';
                        name += (*suffix)[locdbc_idx >= 0 ? locdbc_idx
                                                          : LOCALE_enUS];
                    }
                }
    ...
    if (!sortOrder.empty()) {
        ...
        if (sortInfo.sortOrder >= AUCTION_SORT_MINLEVEL &&
            sortInfo.sortOrder < AUCTION_SORT_MAX &&
            sortInfo.sortOrder != AUCTION_SORT_UNK4) {
            // Partial sort to improve performance a bit, but the last pages
            // will burn
            if (listfrom + 50 <= auctionShortlist.size()) {
                std::partial_sort(
                    auctionShortlist.begin(),
                    auctionShortlist.begin() + listfrom + 50,
                    auctionShortlist.end(),
                    std::bind(SortAuction,
                              std::placeholders::_1,
                              std::placeholders::_2,
                              sortOrder,
                              player,
                              sortInfo.sortOrder == AUCTION_SORT_BID));
            }
            else {
                std::sort(auctionShortlist.begin(),
                          auctionShortlist.end(),
                          std::bind(SortAuction,
                                    std::placeholders::_1,
                                    std::placeholders::_2,
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item) {
        LOG_ERROR("auctionHouse",
                  "AuctionEntry::BuildAuctionInfo: Auction {} has a "
                  "non-existent item: {}",
                  Id,
                  item_guid.ToString());
        return false;
    ...
    data << uint32((expire_time - GameTime::GetGameTime().count()) *
                   IN_MILLISECONDS); // time left
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_AUCTION);
    stmt->SetData(0, Id);
    trans->Append(stmt);
}
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_AUCTION);
    stmt->SetData(0, Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctionHouseEntry) {
        LOG_ERROR(
            "auctionHouse", "Auction {} has invalid house id {}", Id, houseId);
        return false;
    }

    // check if sold item exists for guid
    // and item_template in fact (GetAItem will fail if problematic in result
    // check in AuctionHouseMgr::LoadAuctionItems)
    if (!sAuctionMgr->GetAItem(item_guid)) {
        LOG_ERROR("auctionHouse",
                  "Auction {} has not a existing item : {}",
                  Id,
                  item_guid.ToString());
        return false;
    }
...
std::string
AuctionEntry::BuildAuctionMailSubject(MailAuctionAnswers response) const
{
