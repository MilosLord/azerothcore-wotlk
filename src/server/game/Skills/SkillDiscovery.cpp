...
struct SkillDiscoveryEntry {
    ...
    SkillDiscoveryEntry() : spellId(0), reqSkillValue(0), chance(0) {}
...
void LoadSkillDiscoveryTable()
{
    ...
    //                                                0        1         2 3
    QueryResult result =
        WorldDatabase.Query("SELECT spellId, reqSpell, reqSkillValue, chance "
    ...
    do {
        ...
        if (reqSkillOrSpell > 0) // spell case
        {
            ...
            SpellInfo const* reqSpellInfo =
                sSpellMgr->GetSpellInfo(absReqSkillOrSpell);
            if (!reqSpellInfo) {
                if (reportedReqSpells.find(absReqSkillOrSpell) ==
                    reportedReqSpells.end()) {
                    LOG_ERROR(
                        "sql.sql",
                        "Spell (ID: {}) have not existed spell (ID: {}) in "
            ...
            if (reqSpellInfo->Mechanic != MECHANIC_DISCOVERY &&
                ...
                if (reportedReqSpells.find(absReqSkillOrSpell) ==
                    reportedReqSpells.end()) {
                    LOG_ERROR("sql.sql",
                              "Spell (ID: {}) not have MECHANIC_DISCOVERY (28) "
                              "value in Mechanic field in spell.dbc"
                              " and not 100% chance random discovery ability "
                              "but listed for spellId {} (and maybe more) in "
            ...
            SkillDiscoveryStore[reqSkillOrSpell].push_back(
                SkillDiscoveryEntry(spellId, reqSkillValue, chance));
        }
        else if (reqSkillOrSpell == 0) // skill case
        {
            ...
            if (bounds.first == bounds.second) {
                LOG_ERROR("sql.sql",
                          "Spell (ID: {}) not listed in `SkillLineAbility.dbc` "
                          "but listed with `reqSpell`=0 in "
                          "`skill_discovery_template` table",
                          spellId);
                continue;
            }
            ...
            for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first;
                 ...
                 ++_spell_idx)
                SkillDiscoveryStore[-int32(_spell_idx->second->SkillLine)]
                    .push_back(
                        SkillDiscoveryEntry(spellId, reqSkillValue, chance));
        }
        else {
            LOG_ERROR("sql.sql",
                      "Spell (ID: {}) have negative value in `reqSpell` field "
    ...
    if (!ssNonDiscoverableEntries.str().empty())
        LOG_ERROR("sql.sql",
                  "Some items can't be successfully discovered: have in chance "
                  "field value < 0.000001 in `skill_discovery_template` DB "
                  "table . List:\n{}",
    ...
    for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize();
        ...
        if (SkillDiscoveryStore.find(int32(spell_id)) ==
            SkillDiscoveryStore.end())
            LOG_ERROR("sql.sql",
                      "Spell (ID: {}) is 100% chance random discovery ability "
                      "but not have data in `skill_discovery_template` table",
                      spell_id);
    }

    LOG_INFO("server.loading",
             ">> Loaded {} skill discovery definitions in {} ms",
             count,
...
uint32 GetExplicitDiscoverySpell(uint32 spellId, Player* player)
{
    ...
    SkillDiscoveryMap::const_iterator tab =
        SkillDiscoveryStore.find(int32(spellId));
    ...
    uint32 skillvalue =
        bounds.first != bounds.second
            ? player->GetSkillValue(bounds.first->second->SkillLine)
            : uint32(0);

    float full_chance = 0;
    for (SkillDiscoveryList::const_iterator item_iter = tab->second.begin();
         item_iter != tab->second.end();
         ++item_iter)
    ...
    float roll =
        (float)rand_chance() * rate; // roll now in range 0..full_chance
        ...
        if (item_iter->chance > roll) {
            ...
            if (spellId != 64323)
                player->UpdateGatherSkill(
                    SKILL_INSCRIPTION,
                    player->GetPureSkillValue(SKILL_INSCRIPTION),
                    item_iter->reqSkillValue);
...
uint32 GetSkillDiscoverySpell(uint32 skillId, uint32 spellId, Player* player)
{
    ...
    if (tab != SkillDiscoveryStore.end()) {
        for (SkillDiscoveryList::const_iterator item_iter = tab->second.begin();
             item_iter != tab->second.end();
             ++item_iter) {
            if (roll_chance_f(item_iter->chance *
                              sWorld->getRate(RATE_SKILL_DISCOVERY)) &&
