...
void HashMapHolder<T>::Insert(T* o)
{
    static_assert(std::is_same<Player, T>::value ||
                  ...
                  "Only Player and Motion Transport can be registered in "
                  "global HashMapHolder");
...
void Insert(Player* p) { PlayerNameMap[p->GetName()] = p; }

void Remove(Player* p) { PlayerNameMap.erase(p->GetName()); }

void RemoveByName(std::string const& name) { PlayerNameMap.erase(name); }
...
/**
 * @brief Get a spawned creature by DB `guid` column. MODULE USAGE ONLY - USE IT
 * FOR CUSTOM CONTENT.
 *
...
/**
 * @brief Get a spawned gameobject by DB `guid` column. MODULE USAGE ONLY - USE
