...
void LFGPlayerScript::OnLogout(Player* player)
{
    ...
    if (!player->GetGroup() || !player->GetGroup()->isLFGGroup()) {
        ...
        sLFGMgr->LeaveAllLfgQueues(player->GetGUID(),
                                   true,
                                   player->GetGroup()
                                       ? player->GetGroup()->GetGUID()
                                       : ObjectGuid::Empty);
...
void LFGPlayerScript::OnLogin(Player* player)
{
    ...
    /// @todo - Restore LfgPlayerData and send proper status to player if it was
    /// in a group
}

void LFGPlayerScript::OnBindToInstance(Player*    player,
...
void LFGPlayerScript::OnMapChanged(Player* player)
{
    ...
    if (sLFGMgr->inLfgDungeonMap(
            player->GetGUID(), map->GetId(), map->GetDifficulty())) {
        Group* group = player->GetGroup();
        // This function is also called when players log in
        // if for some reason the LFG system recognises the player as being in a
        // LFG dungeon, but the player was loaded without a valid group, we'll
        // teleport to homebind to prevent crashes or other undefined behaviour
        if (!group) {
            sLFGMgr->LeaveLfg(player->GetGUID());
            ...
            player->TeleportTo(player->m_homebindMapId,
                               ...
                               player->m_homebindO);
            LOG_DEBUG("lfg",
                      "LFGPlayerScript::OnMapChanged, Player {} ({}) is in LFG "
                      "dungeon map but does not have a valid group! "
                      "Teleporting to homebind.",
                      player->GetName(),
...
void LFGGroupScript::OnAddMember(Group* group, ObjectGuid guid)
{
    ...
    if (!group->isLFGGroup()) {
        sLFGMgr->LeaveAllLfgQueues(
            leader,
            true,
            gguid); // pussywizard: invited, queued, party formed, neither party
                    // nor new member are queued, but leader is in queue solo!
...
void LFGGroupScript::OnRemoveMember(Group*       group,
                                    ObjectGuid   guid,
                                    RemoveMethod method,
                                    ObjectGuid   kicker,
                                    char const*  reason)
{
    ...
    ObjectGuid gguid = group->GetGUID();
    LOG_DEBUG("lfg",
              "LFGScripts::OnRemoveMember [{}]: remove [{}] Method: {} Kicker: "
              "[{}] Reason: {}",
              gguid.ToString(),
              guid.ToString(),
              method,
    ...
    if (state != LFG_STATE_FINISHED_DUNGEON &&
        group) // Need more players to finish the dungeon
    {
        if (Player* leader = ObjectAccessor::FindConnectedPlayer(
                sLFGMgr->GetLeader(gguid))) {
            sLFGMgr->SetDungeon(gguid, dungeonId);
            leader->GetSession()->SendLfgOfferContinue(
    ...
    if (Player* player = ObjectAccessor::FindConnectedPlayer(guid)) {
        ...
        player->GetSession()->SendLfgUpdateParty(
            LfgUpdateData(LFG_UPDATETYPE_LEADER_UNK1));
        if (player->GetMap()->IsDungeon()) // Teleport player out the dungeon
        {
            // Xinef: no longer valid sLFGMgr->TeleportPlayer(player, true);
            if (!player->IsBeingTeleportedFar() &&
                player->GetMapId() == sLFGMgr->GetDungeonMapId(gguid))
...
void LFGGroupScript::OnChangeLeader(Group*     group,
...
void LFGGroupScript::OnInviteMember(Group* group, ObjectGuid guid)
{
    ...
    // leader and no gguid == first invite after leader is added to new group
    // (this is the real invite)
    if (leader && !gguid) {
