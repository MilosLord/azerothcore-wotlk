...
LFGMgr::~LFGMgr()
{
    for (LfgRewardContainer::iterator itr = RewardMapStore.begin();
         itr != RewardMapStore.end();
         ++itr)
        delete itr->second;
}
...
void LFGMgr::_SaveToDB(ObjectGuid guid)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_REP_LFG_DATA);
...
void LFGMgr::LoadRewards()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT dungeonId, maxLevel, firstQuestId, otherQuestId FROM "
        "lfg_dungeon_rewards ORDER BY dungeonId, maxLevel ASC");

    if (!result) {
        LOG_ERROR("lfg",
                  ">> Loaded 0 lfg dungeon rewards. DB table "
                  "`lfg_dungeon_rewards` is empty!");
        return;
    }
    ...
    do {
        ...
        if (!GetLFGDungeonEntry(dungeonId)) {
            LOG_ERROR("lfg",
                      "Dungeon {} specified in table `lfg_dungeon_rewards` "
                      "does not exist!",
                      dungeonId);
            continue;
        }
        ...
        if (!maxLevel ||
            ...
            LOG_ERROR("lfg",
                      "Level {} specified for dungeon {} in table "
                      "`lfg_dungeon_rewards` can never be reached!",
                      maxLevel,
                      dungeonId);
            maxLevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
        }
        ...
        if (!firstQuestId || !sObjectMgr->GetQuestTemplate(firstQuestId)) {
            LOG_ERROR("lfg",
                      "First quest {} specified for dungeon {} in table "
                      "`lfg_dungeon_rewards` does not exist!",
                      firstQuestId,
                      dungeonId);
            continue;
        }
        ...
        if (otherQuestId && !sObjectMgr->GetQuestTemplate(otherQuestId)) {
            LOG_ERROR("lfg",
                      "Other quest {} specified for dungeon {} in table "
                      "`lfg_dungeon_rewards` does not exist!",
                      otherQuestId,
                      dungeonId);
            otherQuestId = 0;
        ...
        RewardMapStore.insert(LfgRewardContainer::value_type(
            dungeonId, new LfgReward(maxLevel, firstQuestId, otherQuestId)));
        ++count;
    } while (result->NextRow());

    LOG_INFO("server.loading",
             ">> Loaded {} LFG Dungeon Rewards in {} ms",
             count,
...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    //                                                   0          1 2 3 4
    QueryResult result = WorldDatabase.Query(
        "SELECT dungeonId, position_x, position_y, position_z, orientation "
        "FROM lfg_dungeon_template");

    if (!result) {
        LOG_ERROR("lfg",
                  ">> Loaded 0 LFG Entrance Positions. DB Table "
                  "`lfg_dungeon_template` Is Empty!");
        LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do {
        ...
        uint32                        dungeonId = fields[0].Get<uint32>();
        LFGDungeonContainer::iterator dungeonItr =
            LfgDungeonStore.find(dungeonId);
        if (dungeonItr == LfgDungeonStore.end()) {
            LOG_ERROR("lfg",
                      "table `lfg_dungeon_template` contains coordinates for "
                      "wrong dungeon {}",
                      dungeonId);
            continue;
        }
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} LFG Entrance Positions in {} ms",
             count,
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");

    // Fill all other teleport coords from areatriggers
    for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin();
         itr != LfgDungeonStore.end();
         ++itr) {
        LFGDungeonData& dungeon = itr->second;

        // No teleport coords in database, load from areatriggers
        if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f &&
            dungeon.y == 0.0f && dungeon.z == 0.0f) {
            AreaTriggerTeleport const* at =
                sObjectMgr->GetMapEntranceTrigger(dungeon.map);
            if (!at) {
                LOG_ERROR("lfg",
                          "LFGMgr::LoadLFGDungeons: Failed to load dungeon {}, "
                          "cant find areatrigger for map {}",
                          dungeon.name,
                          dungeon.map);
                continue;
            }
    ...
    if (reload) {
        ...
        for (LfgPlayerDataContainer::const_iterator it = PlayersStore.begin();
             it != PlayersStore.end();
             ++it)
...
void LFGMgr::Update(uint32 tdiff, uint8 task)
{
    ...
    if (task == 0) {
        ...
        for (LfgRoleCheckContainer::iterator it = RoleChecksStore.begin();
             it != RoleChecksStore.end();) {
            ...
            for (LfgRolesMap::const_iterator itRoles = roleCheck.roles.begin();
                 itRoles != roleCheck.roles.end();
                 ++itRoles) {
                ...
                if (guid == roleCheck.leader)
                    SendLfgJoinResult(
                        guid,
                        LfgJoinResultData(LFG_JOIN_FAILED,
        ...
        for (LfgProposalContainer::iterator it = ProposalsStore.begin();
             it != ProposalsStore.end();) {
        ...
        for (LfgPlayerBootContainer::iterator it = BootsStore.begin();
             it != BootsStore.end();) {
            LfgPlayerBootContainer::iterator itBoot = it++;
            LfgPlayerBoot&                   boot   = itBoot->second;
            if (boot.cancelTime < currTime) {
                boot.inProgress = false;
                for (LfgAnswerContainer::const_iterator itVotes =
                         boot.votes.begin();
                     itVotes != boot.votes.end();
                     ++itVotes) {
        ...
        this->lastProposalId =
            m_lfgProposalId; // pussywizard: task 2 is done independantly, store
                             // previous value in LFGMgr for future use
        uint8 newGroupsProcessed = 0;
        // Check if a proposal can be formed with the new groups being added
        for (LfgQueueContainer::iterator it = QueuesStore.begin();
             it != QueuesStore.end();
             ++it) {
        ...
        if (!newGroupsProcessed) // don't do this on updates that precessed
                                 // groups (performance)
            for (LfgQueueContainer::iterator it = QueuesStore.begin();
                 it != QueuesStore.end();
                 ++it)
        ...
        if (lastProposalId != m_lfgProposalId) {
            // pussywizard: only one proposal can be created in World::Update
            // (during maps update), and it has id == m_lfgProposalId, so try to
            // find only that one, dunno why for loop here xD
            for (LfgProposalContainer::const_iterator itProposal =
                     ProposalsStore.find(m_lfgProposalId);
                 itProposal != ProposalsStore.end();
                 ++itProposal) {
                uint32       proposalId = itProposal->first;
                LfgProposal& proposal   = ProposalsStore[proposalId];

                ObjectGuid guid;
                for (LfgProposalPlayerContainer::const_iterator itPlayers =
                         proposal.players.begin();
                     itPlayers != proposal.players.end();
                     ++itPlayers) {
                    ...
                    if (ObjectGuid gguid = GetGroup(guid)) {
                        ...
                        SendLfgUpdateParty(
                            guid,
                            LfgUpdateData(LFG_UPDATETYPE_PROPOSAL_BEGIN,
                ...
                if (proposal.state ==
                    LFG_PROPOSAL_SUCCESS) // pussywizard: no idea what's the
                                          // purpose of this xD
...
void LFGMgr::InitializeLockedDungeons(Player* player, Group const* group)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin();
         it != dungeons.end();
         ++it) {
        LFGDungeonData const* dungeon = GetLFGDungeon(*it);
        if (!dungeon) // should never happen - We provide a list from
                      // sLFGDungeonStore
            continue;
        MapEntry const* mapEntry = sMapStore.LookupEntry(dungeon->map);
        DungeonProgressionRequirements const* ar =
            sObjectMgr->GetAccessRequirement(dungeon->map,
                                             Difficulty(dungeon->difficulty));

        uint32 lockData = 0;
        if (dungeon->expansion > expansion ||
            (onlySeasonalBosses && !dungeon->seasonal))
            lockData = LFG_LOCKSTATUS_INSUFFICIENT_EXPANSION;
        else if (DisableMgr::IsDisabledFor(
                     DISABLE_TYPE_MAP, dungeon->map, player))
            lockData = LFG_LOCKSTATUS_RAID_LOCKED;
        else if (DisableMgr::IsDisabledFor(
                     DISABLE_TYPE_LFG_MAP, dungeon->map, player))
            lockData = LFG_LOCKSTATUS_RAID_LOCKED;
        else if (dungeon->difficulty > DUNGEON_DIFFICULTY_NORMAL &&
                 (!mapEntry || !mapEntry->IsRaid()) &&
                 sInstanceSaveMgr->PlayerIsPermBoundToInstance(
                     player->GetGUID(),
                     dungeon->map,
                     Difficulty(dungeon->difficulty)))
            lockData = LFG_LOCKSTATUS_RAID_LOCKED;
            ...
            (sWorld->getBoolConfig(
                 CONFIG_DUNGEON_ACCESS_REQUIREMENTS_LFG_DBC_LEVEL_OVERRIDE) &&
             ar && ar->levelMin > 0 && ar->levelMin > level))
            ...
            (sWorld->getBoolConfig(
                 CONFIG_DUNGEON_ACCESS_REQUIREMENTS_LFG_DBC_LEVEL_OVERRIDE) &&
             ar && ar->levelMax > 0 && ar->levelMax < level))
            ...
            for (const ProgressionRequirement* itemRequirement : ar->items) {
                if (!itemRequirement->checkLeaderOnly || !group ||
                    group->GetLeaderGUID() == player->GetGUID()) {
                    if (itemRequirement->faction == TEAM_NEUTRAL ||
                        itemRequirement->faction == player->GetTeamId(true)) {
            ...
            for (const ProgressionRequirement* questRequirement : ar->quests) {
                if (!questRequirement->checkLeaderOnly || !group ||
                    group->GetLeaderGUID() == player->GetGUID()) {
                    if (questRequirement->faction == TEAM_NEUTRAL ||
                        questRequirement->faction == player->GetTeamId(true)) {
            ...
            for (const ProgressionRequirement* achievementRequirement :
                 ar->achievements) {
                if (!achievementRequirement->checkLeaderOnly || !group ||
                    group->GetLeaderGUID() == player->GetGUID()) {
                    if (achievementRequirement->faction == TEAM_NEUTRAL ||
                        achievementRequirement->faction ==
        ...
        sScriptMgr->OnInitializeLockedDungeons(
            player, level, lockData, dungeon);

        /* TODO VoA closed if WG is not under team control
           (LFG_LOCKSTATUS_RAID_LOCKED) lockData =
           LFG_LOCKSTATUS_TOO_LOW_GEAR_SCORE; lockData =
           LFG_LOCKSTATUS_TOO_HIGH_GEAR_SCORE; lockData =
           LFG_LOCKSTATUS_ATTUNEMENT_TOO_LOW_LEVEL; lockData =
...
/**
    Adds the player/group to lfg queue. If player is in a group then it is the
   leader of the group tying to join the group. Join conditions are checked
   before adding to the new queue.

   @param[in]     player Player trying to join (or leader of group trying to
   join)
                     ...
                     const std::string& comment)
{
    ...
    LfgGuidSet        players;
    uint32            rDungeonId = 0;
    bool              isContinue = grp && grp->isLFGGroup() &&
                      GetState(gguid) != LFG_STATE_FINISHED_DUNGEON;
    ...
    if (GetState(player->GetGUID()) == LFG_STATE_RAIDBROWSER) {
        ...
        SendRaidBrowserJoinedPacket(
            player,
            tmp,
            ""); // the df "eye" can disappear in various case, resend if needed
        return;
    }
    ...
    switch (state) {
    case LFG_STATE_ROLECHECK: // if joining again during rolecheck (eg. many
                              // players clicked continue inside instance)
        if (gguid.IsGroup())
            UpdateRoleCheck(
                gguid); // abort role check and remove from RoleChecksStore
        break;
    ...
    */
    ...
    if (joinData.result == LFG_JOIN_OK) {
        ...
        for (LfgDungeonSet::const_iterator it = dungeons.begin();
             it != dungeons.end() && joinData.result == LFG_JOIN_OK;
             ++it) {
            ...
            case LFG_TYPE_RANDOM:
                ...
                [[fallthrough]]; // On purpose (Random can only be dungeon or
                                 // heroic dungeon)
            case LFG_TYPE_HEROIC:
            case LFG_TYPE_DUNGEON:
                if (isRaid) {
            ...
            default:
                LOG_ERROR(
                    "lfg", "Wrong dungeon type {} for dungeon {}", type, *it);
                joinData.result = LFG_JOIN_DUNGEON_INVALID;
                break;
            }
        }
    ...
    if (!isRaid && joinData.result == LFG_JOIN_OK) {
        ...
        if (player->InBattleground() ||
            (player->InBattlegroundQueue() &&
             !sWorld->getBoolConfig(CONFIG_ALLOW_JOIN_BG_AND_LFG))) {
            ...
            if (grp->GetMembersCount() > MAXGROUPSIZE)
                ...
                uint8 memberCount = 0;
                for (GroupReference* itr = grp->GetFirstMember();
                     itr != nullptr && joinData.result == LFG_JOIN_OK;
                     itr = itr->next()) {
        ...
        // Xinef: Moreover check this only if dungeon is not started, afterwards
        // its obvious that players will have the cooldown
        if (joinData.result == LFG_JOIN_OK && !isContinue && rDungeonId) {
            if (player->HasAura(
                    LFG_SPELL_DUNGEON_COOLDOWN)) // xinef: added !isContinue
                ...
                for (GroupReference* itr = grp->GetFirstMember();
                    ...
                    if (Player* plrg = itr->GetSource())
                        if (plrg->HasAura(
                                LFG_SPELL_DUNGEON_COOLDOWN)) // xinef: added
                                                             // !isContinue
    ...
    if (joinData.result == LFG_JOIN_OK) {
        ...
        // xinef: dont check compatibile dungeons for already running group
        // (bind problems)
        if (!isContinue) {
            GetCompatibleDungeons(dungeons, players, joinData.lockmap);
            if (dungeons.empty())
                joinData.result =
                    grp ? LFG_JOIN_PARTY_NOT_MEET_REQS : LFG_JOIN_NOT_MEET_REQS;
        }
    }
    ...
    if (joinData.result != LFG_JOIN_OK) {
        LOG_DEBUG("lfg",
                  "LFGMgr::Join: [{}] joining with {} members. result: {}",
                  guid.ToString(),
                  grp ? grp->GetMembersCount() : 1,
                  joinData.result);
        if (!dungeons.empty()) // Only should show lockmap when have no dungeons
                               // available
    ...
    if (isRaid) {
        if (grp)
            ...
            roles &=
                (PLAYER_ROLE_TANK | PLAYER_ROLE_HEALER | PLAYER_ROLE_DAMAGE);
        if (!roles)
            return;
    ...
    if (grp) // Begin rolecheck
    {
        ...
        roleCheck.roles.clear(); // pussywizard: NEW rolecheck, not old one with
                                 // trash data >_>
        roleCheck.cancelTime =
            time_t(GameTime::GetGameTime().count()) + LFG_TIME_ROLECHECK;
        ...
        LfgUpdateData updateData =
            LfgUpdateData(LFG_UPDATETYPE_JOIN_QUEUE, dungeons, comment);
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr;
             itr                 = itr->next()) {
            if (Player* plrg = itr->GetSource()) {
    ...
    else // Add player to queue
    {
        ...
        queue.AddQueueData(
            guid, GameTime::GetGameTime().count(), dungeons, rolesMap);
        ...
        // Send update to player
        player->GetSession()->SendLfgJoinResult(joinData);
        player->GetSession()->SendLfgUpdatePlayer(
            LfgUpdateData(LFG_UPDATETYPE_JOIN_QUEUE, dungeons, comment));
    ...
    {
        std::ostringstream o;
        o << "LFGMgr::Join: [" << guid << "] joined (" << (grp ? "group" :
    "player") << ") Members: " << debugNames.c_str()
          << ". Dungeons (" << uint32(dungeons.size()) << "): " <<
    ConcatenateDungeons(dungeons); LOG_DEBUG("lfg", "{}", o.str());
    }*/
}
...
/**
    Leaves Dungeon System. Player/Group is removed from queue, rolechecks,
   proposals or votekicks. Player or group needs to be not nullptr and using
   Dungeon System
...
void LFGMgr::LeaveLfg(ObjectGuid guid)
{
    ...
    case LFG_STATE_QUEUED:
        if (gguid) {
            ...
            for (LfgGuidSet::const_iterator it = players.begin();
                 it != players.end();
                 ++it) {
            ...
            SendLfgUpdatePlayer(
                guid, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE));
    ...
    case LFG_STATE_ROLECHECK:
        if (gguid)
            UpdateRoleCheck(
                gguid); // No player to update role = LFG_ROLECHECK_ABORTED
        break;
    case LFG_STATE_PROPOSAL: {
        ...
        while (it != ProposalsStore.end()) {
            LfgProposalPlayerContainer::iterator itPlayer =
                it->second.players.find(pguid);
            if (itPlayer != it->second.players.end()) {
                // Mark the player/leader of group who left as didn't accept the
                // proposal
        ...
        // Remove from queue - if proposal is found, RemoveProposal will call
        // RemoveFromQueue
    ...
    case LFG_STATE_RAIDBROWSER:
        ...
        SendLfgUpdatePlayer(guid,
                            LfgUpdateData(LFG_UPDATETYPE_LEAVE_RAIDBROWSER));
        SendLfgUpdateParty(guid,
                           LfgUpdateData(LFG_UPDATETYPE_LEAVE_RAIDBROWSER));
        break;
    }
}

void LFGMgr::JoinRaidBrowser(Player*        player,
                             uint8          roles,
                             LfgDungeonSet& dungeons,
                             std::string    comment)
{
    ...
    for (LfgDungeonSet::const_iterator itr = dungeons.begin();
         itr != dungeons.end();
         ++itr)
        if (GetLFGDungeon(
                *itr)) // ensure dungeon data exists for such dungeon id
        {
            RaidBrowserStore[player->GetTeamId()][*itr][player->GetGUID()] =
                entry;
...
{
    for (uint8 team = 0; team < 2; ++team)
        for (RBStoreMap::iterator itr = RaidBrowserStore[team].begin();
             itr != RaidBrowserStore[team].end();
             ++itr)
            itr->second.erase(guid);
}

void LFGMgr::SendRaidBrowserJoinedPacket(Player*        p,
                                         LfgDungeonSet& dungeons,
                                         std::string    comment)
{
    if (dungeons.empty()) {
    ...
    LfgUpdateData updateData =
        LfgUpdateData(LFG_UPDATETYPE_JOIN_RAIDBROWSER, dungeons, comment);
    if (p->GetGroup())
...
{
    RBCacheMap::iterator itr =
        RBCacheStore[player->GetTeamId()].find(dungeonId);
...
void LFGMgr::UpdateRaidBrowser(uint32 diff)
{
    ...
    if (GetMSTimeDiff(GameTime::GetGameTimeMS(), GetTimeMS()) >
        98ms) // prevent lagging
    {
        return;
    }
    ...
    for (uint8 team = 0; team < 2; ++team) {
        ...
        {
            if (m_raidBrowserUpdateTimer[team] >
                0) // allowed only with some time interval
                continue;
            else // reset timer
        ...
        for (neitr = RBUsedDungeonsStore[team].begin();
             neitr != RBUsedDungeonsStore[team].end();) {
            titr      = neitr++;
            dungeonId = (*titr);

            // go to next dungeon than previously (one dungeon updated in one
            ...
            LFGDungeonData const* dungeonData =
                GetLFGDungeon(dungeonId); // checked if exists before inserting
                                          // to the container
            RBInternalInfoMap& currInternalInfoMap =
                RBInternalInfoStoreCurr[team][dungeonId];
            for (RBEntryInfoMap::const_iterator sitr = entryInfoMap.begin();
                 sitr != entryInfoMap.end();
                 ++sitr) {
                guid = sitr->first;
                groupGuid.Clear();
                ...
                encounterMask = 0;
                instanceGuid.Clear();
                if (InstancePlayerBind* bind =
                        sInstanceSaveMgr->PlayerGetBoundInstance(
                            guid, dungeonData->map, dungeonData->difficulty))
                    if (bind->perm) {
                        instanceGuid = ObjectGuid::Create<HighGuid::Instance>(
                ...
                if (p->IsClass(CLASS_DRUID))
                    ...
                    maxPower = (p->getPowerType() == POWER_RAGE ||
                                p->getPowerType() == POWER_RUNIC_POWER)
                                   ? p->GetMaxPower(p->getPowerType()) / 10
                                   : p->GetMaxPower(p->getPowerType());

                currInternalInfoMap[sitr->first] = RBInternalInfo(
                    guid,
                    sitr->second.comment,
                    !groupGuid.IsEmpty(),
                    groupGuid,
                    ...
                    p->getClass(),
                    p->getRace(),
                    p->GetAverageItemLevel(),
                    talents,
                    p->GetAreaId(),
                    p->GetArmor(),
                    (uint32)std::max<int32>(0, spellDamage),
                    (uint32)std::max<int32>(0, spellHeal),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_CRIT_MELEE)),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_CRIT_RANGED)),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_CRIT_SPELL)),
                    std::max<float>(0.0f, mp5),
                    std::max<float>(0.0f, mp5combat),
                    std::max<uint32>(baseAP, rangedAP),
                    (uint32)p->GetStat(STAT_AGILITY),
                    p->GetMaxHealth(),
                    maxPower,
                    p->GetDefenseSkillValue(),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_DODGE)),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_BLOCK)),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_PARRY)),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_HASTE_SPELL)),
                    p->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                                      static_cast<uint16>(CR_EXPERTISE)));

                if (!groupGuid)
                    continue;
                for (Group::member_citerator mitr =
                         p->GetGroup()->GetMemberSlots().begin();
                     mitr != p->GetGroup()->GetMemberSlots().end();
                     ++mitr) {
                    ...
                    talents[1] = 0;
                    talents[2] = 0;
                    if (CharacterCacheEntry const* gpd =
                            sCharacterCache->GetCharacterCacheByGuid(
                                mitr->guid)) {
                        level = gpd->Level;
                        Class = gpd->Class;
                        race  = gpd->Race;
            ...
            RBInternalInfoMap& prevInternalInfoMap =
                RBInternalInfoStorePrev[team][dungeonId];
            RBInternalInfoMap::iterator iter, iterTmp;
            for (RBInternalInfoMap::const_iterator sitr =
                     prevInternalInfoMap.begin();
                 sitr != prevInternalInfoMap.end();
                 ++sitr) {
                ...
                else // was -> is
                {
                    ...
                    {
                        if (!iter->second
                                 .isGroupLeader) // leader -> no longer a leader
                            deletedGroups.insert(sitr->second.groupGuid);
                        else if (sitr->second.groupGuid !=
                                 iter->second.groupGuid) // leader -> leader of
                                                         // another group
                        {
                            deletedGroups.insert(sitr->second.groupGuid);
                        ...
                        else if (sitr->second.comment != iter->second.comment ||
                                 sitr->second.encounterMask !=
                                     iter->second.encounterMask ||
                                 sitr->second.instanceGuid !=
                                     iter->second
                                         .instanceGuid) // leader -> nothing
                                                        // changed
                        {
                            ++groupCounter;
                            RBPacketAppendGroup(iter->second, buffer_groups);
                    ...
                    else if (iter->second.isGroupLeader) // wasn't a leader ->
                                                         // is a leader
                    {
                        deletedGroupsToErase.insert(iter->second.groupGuid);
                        ++groupCounter;
                    ...
                    if (!iter->second
                             ._online) // if offline, copy previous stats
                                       // (itemLevel, talents, area, etc.)
                    {
                        iterTmp = copy.find(
                            sitr->first); // copied container is for building a
                                          // full packet, so modify it there
                                          // (currInternalInfoMap is erased)
                        iterTmp->second.CopyStats(sitr->second);
                        if (!sitr->second.PlayerSameAs(
                                iterTmp->second)) // player info changed
                        {
                            ++playerCounter;
                            RBPacketAppendPlayer(iterTmp->second,
                                                 buffer_players);
                        }
                    }
                    else if (!sitr->second.PlayerSameAs(
                                 iter->second)) // player info changed
                    {
                        ++playerCounter;
            ...
            // left entries (new)
            for (RBInternalInfoMap::const_iterator sitr =
                     currInternalInfoMap.begin();
                 sitr != currInternalInfoMap.end();
                 ++sitr) {
                if (sitr->second.isGroupLeader) {
            ...
            RBPacketBuildDifference(differencePacket,
                                    dungeonId,
                                    deletedCounter,
                                    buffer_deleted,
                                    groupCounter,
            ...
            for (RBSearchersMap::const_iterator sitr =
                     RBSearchersStore[team].begin();
                 sitr != RBSearchersStore[team].end();
                 ++sitr)
                if (sitr->second == dungeonId)
                    if (Player* p =
                            ObjectAccessor::FindConnectedPlayer(sitr->first))
...
void LFGMgr::RBPacketAppendGroup(const RBInternalInfo& info, ByteBuffer& buffer)
{
    ...
    uint32 flags = LFG_UPDATE_FLAG_COMMENT | LFG_UPDATE_FLAG_ROLES |
...
void LFGMgr::RBPacketAppendPlayer(const RBInternalInfo& info,
                                  ByteBuffer&           buffer)
{
    buffer << info.guid;
    uint32 flags =
        LFG_UPDATE_FLAG_CHARACTERINFO | LFG_UPDATE_FLAG_ROLES |
        LFG_UPDATE_FLAG_COMMENT |
        (info.groupGuid ? LFG_UPDATE_FLAG_GROUPGUID : LFG_UPDATE_FLAG_BINDED) |
        (info.isGroupLeader ? LFG_UPDATE_FLAG_GROUPLEADER : 0) |
        (!info.groupGuid || info.isGroupLeader ? LFG_UPDATE_FLAG_AREA : 0);
    buffer << (uint32)flags;
    ...
    if (flags & LFG_UPDATE_FLAG_CHARACTERINFO) {
        ...
        buffer << (uint32)info._online; // talentpoints, used as online/offline
                                        // marker :D
    ...
    if (flags & LFG_UPDATE_FLAG_GROUPGUID)
        buffer << info.groupGuid;
    if (flags & LFG_UPDATE_FLAG_ROLES)
        buffer << (uint8)(info.groupGuid
                              ? (info.isGroupLeader ? PLAYER_ROLE_LEADER : 0)
                              : info.roles);
...
void LFGMgr::RBPacketBuildDifference(WorldPacket& differencePacket,
                                     uint32       dungeonId,
...
void LFGMgr::RBPacketBuildFull(WorldPacket&       fullPacket,
                               uint32             dungeonId,
                               RBInternalInfoMap& infoMap)
{
    ...
    for (RBInternalInfoMap::const_iterator itr = infoMap.begin();
         itr != infoMap.end();
         ++itr) {
...
// pussywizard:
void LFGMgr::LeaveAllLfgQueues(ObjectGuid guid,
                               bool       allowgroup,
                               ObjectGuid groupguid)
{
    ...
    if (pguid)
        for (lfg::LfgQueueContainer::iterator itr = QueuesStore.begin();
             itr != QueuesStore.end();
             ++itr)
    ...
    if (pguid && !gguid) {
        if (GetState(pguid) == LFG_STATE_QUEUED) {
            SendLfgUpdatePlayer(
                pguid, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE));
...
/**
   ...
   @param[in]     roles Player selected roles
*/
void LFGMgr::UpdateRoleCheck(ObjectGuid gguid,
                             ObjectGuid guid /* = 0 */,
                             uint8      roles /* = PLAYER_ROLE_NONE */)
{
    ...
    if (!guid)
        ...
        while (itRoles != roleCheck.roles.end() &&
               itRoles->second != PLAYER_ROLE_NONE)
            ++itRoles;

        if (itRoles == roleCheck.roles.end()) {
            // use temporal var to check roles, CheckGroupRoles modifies the
            // roles
            check_roles     = roleCheck.roles;
            roleCheck.state = CheckGroupRoles(check_roles)
    ...
    LfgJoinResultData joinData =
        LfgJoinResultData(LFG_JOIN_FAILED, roleCheck.state);
    for (LfgRolesMap::const_iterator it = roleCheck.roles.begin();
         it != roleCheck.roles.end();
         ++it) {
        ...
        case LFG_ROLECHECK_FINISHED:
            ...
            SendLfgUpdateParty(pguid,
                               LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE,
                                             dungeons,
                                             GetComment(pguid)));
            break;
        default:
            ...
            SendLfgUpdateParty(pguid,
                               LfgUpdateData(LFG_UPDATETYPE_ROLECHECK_FAILED));
...
/**
   ...
   @param[out]    lockMap Map of players Lock status info of given dungeons
   (Empty if dungeons is not empty)
*/
void LFGMgr::GetCompatibleDungeons(LfgDungeonSet&    dungeons,
                                   LfgGuidSet const& players,
                                   LfgLockPartyMap&  lockMap)
{
    lockMap.clear();
    for (LfgGuidSet::const_iterator it = players.begin();
         it != players.end() && !dungeons.empty();
         ++it) {
        ObjectGuid        guid          = (*it);
        LfgLockMap const& cachedLockMap = GetLockedDungeons(guid);
        for (LfgLockMap::const_iterator it2 = cachedLockMap.begin();
             it2 != cachedLockMap.end() && !dungeons.empty();
             ++it2) {
                              ...
                              bool         removeLeaderFlag /*= true*/)
{
    ...
    if (removeLeaderFlag)
        for (LfgRolesMap::iterator it = groles.begin(); it != groles.end();
             ++it)
...
void LFGMgr::MakeNewGroup(LfgProposal const& proposal)
{
    ...
    for (LfgProposalPlayerContainer::const_iterator it =
             proposal.players.begin();
         it != proposal.players.end();
         ++it) {
    ...
    Group* grp            = proposal.group
                                ? sGroupMgr->GetGroupByGUID(proposal.group.GetCounter())
                                : nullptr;
    if (!grp) {
        ObjectGuid groupGUID;
    ...
    for (LfgGuidList::const_iterator it = players.begin(); it != players.end();
         ++it) {
        ObjectGuid pguid  = (*it);
        ...
        if (group) {
            SetRandomPlayersCount(pguid,
                                  group->GetMembersCount() >= MAXGROUPSIZE
                                      ? 0
                                      : MAXGROUPSIZE -
    ...
    // pussywizard: crashfix, group wasn't created when iterating players (no
    // player found by guid), proposal is deleted by the calling function
    if (!grp)
        return;
    ...
    for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr;
        ...
        if (Player* plr = itr->GetSource()) {
            if (grp->IsLeader(plr->GetGUID()) &&
                playersToTeleport.find(plr->GetGUID()) !=
    ...
    // Teleport Player
    for (GuidUnorderedSet::const_iterator it = playersToTeleport.begin();
         it != playersToTeleport.end();
         ++it) {
        if (Player* player = ObjectAccessor::FindPlayer(*it)) {
            if (player->GetGroup() !=
                grp) // pussywizard: could not add because group was full
                continue;

            // Add the cooldown spell if queued for a random dungeon
            // xinef: add aura
            if ((randomDungeon ||
                 selectedRandomLfgDungeon(player->GetGUID())) &&
                ...
                if (!leaderTeleportIncluded) {
                    if (InstancePlayerBind* ilb =
                        ...
                        if (player->GetInstanceId() ==
                            ilb->save->GetInstanceId()) {
                            // Do not teleport if in the same map and instance
                            // as leader
                            continue;
                        }
                    }
...
void LFGMgr::UpdateProposal(uint32 proposalId, ObjectGuid guid, bool accept)
{
    ...
    LfgProposalPlayerContainer::iterator itProposalPlayer =
        proposal.players.find(guid);
    ...
    LOG_DEBUG("lfg",
              "LFGMgr::UpdateProposal: Player [{}] of proposal {} selected: {}",
              guid.ToString(),
              proposalId,
              accept);
    if (!accept) {
    ...
    for (LfgProposalPlayerContainer::const_iterator itPlayers =
         ...
         itPlayers != proposal.players.end();
         ++itPlayers)
        if (itPlayers->second.accept !=
            LFG_ANSWER_AGREE) // No answer (-1) or not accepted (0)
            allAnswered = false;
    ...
    for (LfgProposalPlayerContainer::const_iterator it =
        ...
        if (gguid) {
            waitTime =
                int32((joinTime - queue.GetJoinTime(gguid)) / IN_MILLISECONDS);
            SendLfgUpdateParty(pguid, updateData);
        }
        else {
            waitTime =
                int32((joinTime - queue.GetJoinTime(pguid)) / IN_MILLISECONDS);
...
/**
   Remove a proposal from the pool, remove the group that didn't accept (if
   needed) and readd the other members to the queue

   @param[in]     itProposal Iterator to the proposal to remove
   @param[in]     type Type of removal (LFG_UPDATETYPE_PROPOSAL_FAILED,
   LFG_UPDATETYPE_PROPOSAL_DECLINED)
*/
void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal,
                            LfgUpdateType                  type)
{
    LfgProposal& proposal = itProposal->second;
    ...
    LOG_DEBUG(
        "lfg",
        "LFGMgr::RemoveProposal: Proposal {}, state FAILED, UpdateType {}",
        itProposal->first,
        type);
    // Mark all people that didn't answered as no accept
    if (type == LFG_UPDATETYPE_PROPOSAL_FAILED)
        for (LfgProposalPlayerContainer::iterator it = proposal.players.begin();
             it != proposal.players.end();
             ++it)
    ...
    for (LfgProposalPlayerContainer::iterator it = proposal.players.begin();
        ...
        if (it->second.accept == LFG_ANSWER_DENY ||
            type == LFG_UPDATETYPE_PROPOSAL_FAILED) {
            it->second.accept = LFG_ANSWER_DENY;
    ...
    for (LfgProposalPlayerContainer::const_iterator it =
        ...
        if (toRemove.find(gguid) !=
            toRemove.end()) // Didn't accept or in same group that someone that
                            // didn't accept
        {
            LfgUpdateData updateData;
            if (it->second.accept == LFG_ANSWER_DENY) {
                ...
                LOG_DEBUG("lfg",
                          "LFGMgr::RemoveProposal: [{}] didn't accept. "
                ...
                updateData.updateType = LFG_UPDATETYPE_REMOVED_FROM_QUEUE;
                LOG_DEBUG("lfg",
                          "LFGMgr::RemoveProposal: [{}] in same group that "
                          "someone that didn't accept. Removing from queue and "
                          "compatible cache",
                          guid.ToString());
            }

            RestoreState(guid,
                         "Proposal Fail (didn't accepted or in group with "
                         "someone that didn't accept");
            if (gguid != guid) {
                RestoreState(
                    it->second.group,
            ...
            LOG_DEBUG("lfg",
                      "LFGMgr::RemoveProposal: Readding [{}] to queue.",
                      guid.ToString());
            SetState(guid, LFG_STATE_QUEUED);
            if (gguid != guid) {
                SetState(gguid, LFG_STATE_QUEUED);
    ...
    // Remove players/groups from queue
    for (LfgGuidSet::const_iterator it = toRemove.begin(); it != toRemove.end();
         ++it) {
        ObjectGuid guid = *it;
        queue.RemoveFromQueue(guid);
    ...
    // Readd to queue
    for (uint8 i = 0; i < 5 && proposal.queues.guids[i]; ++i) {
        // xinef: this will work as data is not deleted, only references to this
        // data are cleared xinef: when new proposal is created xinef:
        // successful proposal is also taken into account is similar manner
...
*/
void LFGMgr::InitBoot(ObjectGuid         gguid,
                      ObjectGuid         kicker,
                      ObjectGuid         victim,
                      std::string const& reason)
{
    ...
    // Set votes
    for (LfgGuidSet::const_iterator itr = players.begin(); itr != players.end();
         ++itr) {
        ObjectGuid guid = (*itr);
    ...
    // Notify players
    for (LfgGuidSet::const_iterator it = players.begin(); it != players.end();
         ++it)
...
void LFGMgr::UpdateBoot(ObjectGuid guid, bool accept)
{
    ...
    if (boot.votes[guid] !=
        LFG_ANSWER_PENDING) // Cheat check: Player can't vote twice
        return;
    ...
    uint8 votesNum = 0;
    uint8 agreeNum = 0;
    for (LfgAnswerContainer::const_iterator itVotes = boot.votes.begin();
         itVotes != boot.votes.end();
         ++itVotes) {
    ...
    if (agreeNum < LFG_GROUP_KICK_VOTES_NEEDED &&
        (votesNum - agreeNum) < LFG_GROUP_KICK_VOTES_NEEDED)
        return;
    ...
    if (agreeNum == LFG_GROUP_KICK_VOTES_NEEDED) // Vote passed - Kick player
    {
        if (Group* group = sGroupMgr->GetGroupByGUID(gguid.GetCounter()))
            Player::RemoveFromGroup(
                group, boot.victim, GROUP_REMOVEMETHOD_KICK_LFG);
...
/**
   ...
   @param[in]     fromOpcode Function called from opcode handlers? (Default
   false)
*/
void LFGMgr::TeleportPlayer(Player*              player,
                            bool                 out,
                            WorldLocation const* teleportLocation /*= nullptr*/)
{
    ...
    if (!dungeon) {
        player->GetSession()->SendLfgTeleportError(
            uint8(LFG_TELEPORTERROR_INVALID_LOCATION));
        return;
    }
    ...
    if (!player->IsAlive()) {
        ...
        if (!player->GetMap()->IsDungeon() ||
            player->GetEntryPoint().GetMapId() == MAPID_INVALID) {
            player->SetEntryPoint();
        }

        if (!player->TeleportTo(mapid,
                                x,
                                y,
                                z,
                                orientation,
    ...
    if (error != LFG_TELEPORTERROR_OK) {
        ...
        LOG_DEBUG("lfg",
                  "Player [{}] could NOT be teleported in to map [{}] (x: {}, "
                  "y: {}, z: {}) Error: {}",
                  player->GetName(),
                  dungeon->map,
                  dungeon->x,
        ...
        LOG_DEBUG("lfg",
                  "Player [{}] is being teleported in to map [{}] (x: {}, y: "
                  "{}, z: {})",
                  player->GetName(),
                  dungeon->map,
                  dungeon->x,
...
*/
void LFGMgr::FinishDungeon(ObjectGuid   gguid,
                           const uint32 dungeonId,
                           const Map*   currMap)
{
    uint32 gDungeonId = GetDungeon(gguid);
    if (gDungeonId != dungeonId) {
        LOG_DEBUG("lfg",
                  "LFGMgr::FinishDungeon: [{}] Finished dungeon {} but group "
                  "queued for {}. Ignoring",
                  gguid.ToString(),
                  dungeonId,
                  gDungeonId);
    ...
    if (GetState(gguid) ==
        LFG_STATE_FINISHED_DUNGEON) // Shouldn't happen. Do not reward multiple
                                    // times
    {
        LOG_DEBUG(
            "lfg",
            "LFGMgr::FinishDungeon: [{}] Already rewarded group. Ignoring",
            gguid.ToString());
        return;
    }
    ...
    for (LfgGuidSet::const_iterator it = players.begin(); it != players.end();
        ...
        if (GetState(guid) == LFG_STATE_FINISHED_DUNGEON) {
            LOG_DEBUG(
                "lfg",
                "LFGMgr::FinishDungeon: [{}] Already rewarded player. Ignoring",
                guid.ToString());
            continue;
        }
        ...
        if (!dungeon ||
            (dungeon->type != LFG_TYPE_RANDOM && !dungeon->seasonal)) {
            LOG_DEBUG("lfg",
                      "LFGMgr::FinishDungeon: [{}] dungeon {} is not random or "
                      "seasonal",
                      guid.ToString(),
                      rDungeonId);
            continue;
        ...
        if (!player ||
            player->FindMap() !=
                currMap) // pussywizard: currMap - multithreading crash if on
                         // other map (map id check is not enough, binding
                         // system is not reliable)
        {
            LOG_DEBUG("lfg",
                      "LFGMgr::FinishDungeon: [{}] not found in world",
                      guid.ToString());
            continue;
        }
        ...
        if (player->GetMapId() != mapId) {
            LOG_DEBUG("lfg",
                      "LFGMgr::FinishDungeon: [{}] is in map {} and should be "
                      "in {} to get reward",
                      guid.ToString(),
                      player->GetMapId(),
                      mapId);
        ...
        // Xinef: Update achievements, set correct amount of randomly grouped
        // players
        ...
        LfgReward const* reward =
            GetRandomDungeonReward(rDungeonId, player->GetLevel());
        if (!reward)
            continue;
        ...
        // if we can take the quest, means that we haven't done this kind of
        // "run", IE: First Heroic Random of Day.
        ...
        LOG_DEBUG(
            "lfg",
            "LFGMgr::FinishDungeon: [{}] done dungeon {}, {} previously done.",
            player->GetGUID().ToString(),
            GetDungeon(gguid),
            done ? " " : " not");
        LfgPlayerRewardData data = LfgPlayerRewardData(
            dungeon->Entry(), GetDungeon(gguid, false), done, quest);
...
/**
   ...
   @param[in]     randomdungeon Random dungeon id (if value = 0 will return all
   dungeons)
...
LfgReward const* LFGMgr::GetRandomDungeonReward(uint32 dungeon, uint8 level)
{
    ...
    LfgRewardContainerBounds bounds =
        RewardMapStore.equal_range(dungeon & 0x00FFFFFF);
    for (LfgRewardContainer::const_iterator itr = bounds.first;
         itr != bounds.second;
         ++itr) {
        rew = itr->second;
...
uint32 LFGMgr::GetDungeonMapId(ObjectGuid guid)
{
    ...
    LOG_DEBUG("lfg",
              "LFGMgr::GetDungeonMapId: [{}] = {} (DungeonId = {})",
              guid.ToString(),
              mapId,
              dungeonId);
    return mapId;
...
void LFGMgr::RestoreState(ObjectGuid guid, char const* /*debugMsg*/)
{
    if (guid.IsGroup()) {
        ...
        /*if (sLog->ShouldLog(LOG_FILTER_LFG, LOG_LEVEL_DEBUG))
        {
            ...
            LOG_TRACE("lfg", "LFGMgr::RestoreState: Group: [{}] ({}) State: {},
        oldState: {}", guid.ToString(), debugMsg, ps, os);
        }*/
        ...
        /*if (sLog->ShouldLog(LOG_FILTER_LFG, LOG_LEVEL_DEBUG))
        {
            std::string const& ps = GetStateString(data.GetState());
            std::string const& os = GetStateString(data.GetOldState());
            LOG_TRACE("lfg", "LFGMgr::RestoreState: Player: [{}] ({}) State: {},
        oldState: {}", guid.ToString(), debugMsg, ps, os);
        }*/
        data.RestoreState();
    }
...
void LFGMgr::SetState(ObjectGuid guid, LfgState state)
{
    if (guid.IsGroup()) {
        ...
        LOG_DEBUG("lfg",
                  "LFGMgr::SetState: Group: [{}] newState: {}, previous: {}, "
                  "oldState: {}",
                  guid.ToString(),
                  ns,
                  ps,
        ...
        std::string    os   = GetStateString(data.GetOldState());
        LOG_DEBUG("lfg",
                  "LFGMgr::SetState: Player: [{}] newState: {}, previous: {}, "
                  "oldState: {}",
                  guid.ToString(),
                  ns,
                  ps,
...
{
    LOG_DEBUG(
        "lfg", "LFGMgr::SetDungeon: [{}] dungeon {}", guid.ToString(), dungeon);
...
{
    LOG_DEBUG(
        "lfg", "LFGMgr::SetRoles: [{}] roles: {}", guid.ToString(), roles);
...
void LFGMgr::LfrSetComment(Player* p, std::string comment)
{
    ...
    RBEntryInfoMap::iterator iter;
    for (RBStoreMap::iterator itr = RaidBrowserStore[teamId].begin();
         itr != RaidBrowserStore[teamId].end();
         ++itr)
        if ((iter = itr->second.find(p->GetGUID())) != itr->second.end())
...
void LFGMgr::RemoveGroupData(ObjectGuid guid)
{
    ...
    for (auto iterator = players.begin(); iterator != players.end();
         ++iterator) {
        ...
        if (state != LFG_STATE_PROPOSAL) {
            ...
            SendLfgUpdateParty(
                objectGuid, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE));
        }
    }
    GroupsStore.erase(it);
}

TeamId LFGMgr::GetTeam(ObjectGuid guid) { return PlayersStore[guid].GetTeam(); }
...
void LFGMgr::SetTeam(ObjectGuid guid, TeamId teamId)
{
    if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP))
        teamId = TEAM_ALLIANCE; // @Not Sure About That TeamId is supposed to be
                                // uint8 Team = 0(@TrinityCore)
...
bool LFGMgr::HasIgnore(ObjectGuid guid1, ObjectGuid guid2)
{
    ...
    return plr1 && plr2 &&
           (plr1->GetSocial()->HasIgnore(guid2) ||
            plr2->GetSocial()->HasIgnore(guid1));
}
...
void LFGMgr::SendLfgRoleCheckUpdate(ObjectGuid          guid,
...
void LFGMgr::SendLfgBootProposalUpdate(ObjectGuid           guid,
...
LFGQueue& LFGMgr::GetQueue(ObjectGuid guid)
{
    ...
    if (guid.IsGroup()) {
        ...
        ObjectGuid        pguid =
            players.empty() ? ObjectGuid::Empty : (*players.begin());
        if (pguid)
...
void LFGMgr::Clean() { QueuesStore.clear(); }

bool LFGMgr::isOptionEnabled(uint32 option) { return m_options & option; }

uint32 LFGMgr::GetOptions() { return m_options; }

void LFGMgr::SetOptions(uint32 options) { m_options = options; }
...
LfgUpdateData LFGMgr::GetLfgStatus(ObjectGuid guid)
{
    ...
    return LfgUpdateData(LFG_UPDATETYPE_UPDATE_STATUS,
                         playerData.GetState(),
                         playerData.GetSelectedDungeons());
}
...
LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(uint8 level, uint8 expansion)
{
    ...
    for (lfg::LFGDungeonContainer::const_iterator itr = LfgDungeonStore.begin();
         itr != LfgDungeonStore.end();
         ++itr) {
