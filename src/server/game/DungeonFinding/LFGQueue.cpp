...
LfgQueueData::LfgQueueData()
    : joinTime(time_t(GameTime::GetGameTime().count())),
      lastRefreshTime(joinTime), tanks(LFG_TANKS_NEEDED),
...
{
    LOG_DEBUG("lfg",
              "ADD AddToQueue: {}, failed proposal: {}",
              guid.ToString(),
              failedProposal ? 1 : 0);
    LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
    if (itQueue == QueueDataStore.end()) {
        LOG_ERROR("lfg",
                  "LFGQueue::AddToQueue: Queue data not found for [{}]",
                  guid.ToString());
        return;
    }
...
{
    LOG_DEBUG("lfg",
              "REMOVE RemoveFromQueue: {}, partial: {}",
              guid.ToString(),
              partial ? 1 : 0);
    ...
    for (LfgQueueDataContainer::iterator itr = QueueDataStore.begin();
         itr != QueueDataStore.end();
         ++itr) {
        if (itr->first != guid) {
            if (itr->second.bestCompatible.hasGuid(guid)) {
                LOG_DEBUG("lfg",
                          "CLEAR bestCompatible: {}, because of: {}",
                          itr->second.bestCompatible.toString(),
                          guid.ToString());
            ...
            LOG_DEBUG("lfg",
                      "CLEAR bestCompatible SELF: {}, because of: {}",
                      itr->second.bestCompatible.toString(),
                      guid.ToString());
            // itr->second.bestCompatible.clear(); // don't clear here, because
            // UpdateQueueTimers will try to find with every diff update
            itDelete = itr;
        }
    }
    ...
    if (!partial && itDelete != QueueDataStore.end()) {
        ...
        LOG_DEBUG("lfg",
                  "ERASE QueueDataStore for: {}, itDelete: {},{},{}",
                  guid.ToString(),
                  itDelete->second.dps,
                  itDelete->second.healers,
                  itDelete->second.tanks);
        QueueDataStore.erase(itDelete);
        LOG_DEBUG(
            "lfg", "ERASE QueueDataStore for: {} SUCCESS", guid.ToString());
    }
}
...
void LFGQueue::AddQueueData(ObjectGuid           guid,
                            time_t               joinTime,
...
void LFGQueue::RemoveFromCompatibles(ObjectGuid guid)
{
    ...
    for (LfgCompatibleContainer::iterator it = CompatibleList.begin();
         it != CompatibleList.end();
         ++it)
        if (it->hasGuid(guid)) {
            LOG_DEBUG("lfg",
                      "Removed Compatible: {}, because of: {}",
                      it->toString(),
                      guid.ToString());
            it->clear(); // set to 0, this will be removed while iterating in
                         // FindNewGroups
        }
    for (LfgCompatibleContainer::iterator itr = CompatibleTempList.begin();
         itr != CompatibleTempList.end();) {
        LfgCompatibleContainer::iterator it = itr++;
        if (it->hasGuid(guid)) {
            LOG_DEBUG("lfg",
                      "Erased Temp Compatible: {}, because of: {}",
                      it->toString(),
                      guid.ToString());
...
uint8 LFGQueue::FindGroups()
{
    ...
    if (!newToQueueStore.empty()) {
        ...
        bool       pushCompatiblesToFront =
            (std::find(restoredAfterProposal.begin(),
                       restoredAfterProposal.end(),
                       newGuid) != restoredAfterProposal.end());
        LOG_DEBUG("lfg",
                  "newToQueueStore: {}, front: {}",
                  newGuid.ToString(),
                  pushCompatiblesToFront ? 1 : 0);
        RemoveFromNewQueue(newGuid);

        FindNewGroups(newGuid);

        CompatibleList.splice((pushCompatiblesToFront ? CompatibleList.begin()
                                                      : CompatibleList.end()),
                              CompatibleTempList);
        CompatibleTempList.clear();

        return newGroupsProcessed; // pussywizard: only one per update,
...
{
    // each combination of dps+heal+tank (tank*8 + heal+4 + dps) has a value
    // assigned 0..15 first 16 bits of the mask are for marking if such
    // combination was found once, second 16 bits for marking second occurence
    ...
    // we have to take into account that FindNewGroups is called every X minutes
    // if number of compatibles is low! build set of already present compatibles
    // for this guid
    std::set<Lfg5Guids> currentCompatibles;
    for (Lfg5GuidsList::iterator it = CompatibleList.begin();
         it != CompatibleList.end();
         ++it)
    ...
    if (currentCompatibles.empty()) {
        ...
        if (selfCompatibility !=
            LFG_COMPATIBLES_WITH_LESS_PLAYERS) // group is already compatible (a
                                               // party of 5 players)
    ...
    for (Lfg5GuidsList::iterator it = CompatibleList.begin();
        ...
        LfgCompatibility compatibility = CheckCompatibility(
            *itr, newGuid, foundMask, foundCount, currentCompatibles);
        if (compatibility == LFG_COMPATIBLES_MATCH)
            return LFG_COMPATIBLES_MATCH;
        if ((foundMask & 0x3FFF3FFF3FFF3FFF) ==
            0x3FFF3FFF3FFF3FFF) // each combination of dps+heal+tank already
                                // found 4 times
            break;
    }
...
LfgCompatibility
LFGQueue::CheckCompatibility(Lfg5Guids const&           checkWith,
...
{
    LOG_DEBUG("lfg",
              "CHECK CheckCompatibility: {}, new guid: {}",
              checkWith.toString(),
              newGuid.ToString());
    ...
    if (!currentCompatibles.empty() &&
        currentCompatibles.find(strGuids) != currentCompatibles.end())
    ...
    for (uint8 i = 0; i < 5 && !(guid = check.guids[i]).IsEmpty() &&
                      numLfgGroups < 2 && numPlayers <= MAXGROUPSIZE;
         ++i) {
        LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
        if (itQueue == QueueDataStore.end()) {
            LOG_ERROR("lfg",
                      "LFGQueue::CheckCompatibility: [{}] is not queued but "
                      "listed as queued!",
                      guid.ToString());
        ...
        // Store group so we don't need to call Mgr to get it later (if it's
        // player group will be 0 otherwise would have joined as group)
        for (LfgRolesMap::const_iterator it2 = itQueue->second.roles.begin();
             it2 != itQueue->second.roles.end();
             ++it2)
            proposalGroups[it2->first] =
                itQueue->first.IsGroup() ? itQueue->first : ObjectGuid::Empty;
    ...
    if (!sLFGMgr->IsTesting() && check.size() == 1 &&
        ...
        LfgQueueDataContainer::iterator itQueue =
        ...
        itQueue->second.bestCompatible
            .clear(); // this may be left after a failed proposal (not cleared,
                      // because UpdateQueueTimers would try to generate it with
                      // every update)
        // UpdateBestCompatibleInQueue(itQueue, strGuids);
        AddToCompatibles(strGuids);
        if (roleCheckResult && roleCheckResult <= 15)
            foundMask |= ((((uint64)1) << (roleCheckResult - 1)) |
                          (((uint64)1) << (16 + roleCheckResult - 1)) |
                          (((uint64)1) << (32 + roleCheckResult - 1)) |
                          (((uint64)1) << (48 + roleCheckResult - 1)));
    ...
    // If it's single group no need to check for duplicate players, ignores, bad
    // roles or bad dungeons as it's been checked before joining
    if (check.size() > 1) {
        for (uint8 i = 0; i < 5 && check.guids[i]; ++i) {
            const LfgRolesMap& roles = QueueDataStore[check.guids[i]].roles;
            for (LfgRolesMap::const_iterator itRoles = roles.begin();
                 itRoles != roles.end();
                 ++itRoles) {
                LfgRolesMap::const_iterator itPlayer;
                for (itPlayer = proposalRoles.begin();
                     itPlayer != proposalRoles.end();
                     ++itPlayer) {
                    if (itRoles->first == itPlayer->first) {
                        // pussywizard: LFG this means that this player was in
                        // two different LfgQueueData (in QueueDataStore), and
                        // at least one of them is a group guid, because we do
                        // checks so there aren't 2 same guids in current CHECK
                        // LOG_ERROR("lfg", "LFGQueue::CheckCompatibility:
                        // ERROR! Player multiple times in queue! [{}]",
                        // itRoles->first.ToString());
                        break;
                    }
        ...
        // now, every combination can occur only 4 times (explained in
        // FindNewGroups)
        if (foundMask & (((uint64)1) << (roleCheckResult - 1))) {
            if (foundMask & (((uint64)1) << (16 + roleCheckResult - 1))) {
                if (foundMask & (((uint64)1) << (32 + roleCheckResult - 1))) {
                    if (foundMask &
                        (((uint64)1) << (48 + roleCheckResult - 1))) {
                        if (foundCount >= 10) // but only after finding at least
                                              // 10 compatibles (this helps when
        ...
        for (uint8 i = 1; i < 5 && check.guids[i]; ++i) {
            ...
            std::set_intersection(proposalDungeons.begin(),
                                  proposalDungeons.end(),
                                  dungeons.begin(),
                                  dungeons.end(),
                                  std::inserter(temporal, temporal.begin()));
    ...
    if (!sLFGMgr->IsTesting() && numPlayers != MAXGROUPSIZE) {
        ...
        for (uint8 i = 0; i < 5 && check.guids[i]; ++i) {
            LfgQueueDataContainer::iterator itr =
                QueueDataStore.find(check.guids[i]);
            if (!itr->second.bestCompatible
                     .empty()) // update if groups don't have it empty (for
                               // empty it will be generated in
                               // UpdateQueueTimers)
    ...
    proposal.dungeonId =
        Acore::Containers::SelectRandomContainerElement(proposalDungeons);

    uint32 completedEncounters = 0;
    bool   leader              = false;
    for (LfgRolesMap::const_iterator itRoles = proposalRoles.begin();
         itRoles != proposalRoles.end();
         ++itRoles) {
        // Assing new leader
        ...
        if (!proposal.isNew && data.group &&
            data.group ==
                proposal.group) // Player from existing group, autoaccept
            data.accept = LFG_ANSWER_AGREE;

        if (!completedEncounters && !proposal.isNew) {
            if (LFGDungeonEntry const* dungeon =
                    sLFGDungeonStore.LookupEntry(proposal.dungeonId)) {
                if (Player* player =
                        ObjectAccessor::FindConnectedPlayer(itRoles->first)) {
                    if (player->GetMapId() ==
                        static_cast<uint32>(dungeon->MapID)) {
                        if (InstanceScript* instance =
                                player->GetInstanceScript()) {
                            completedEncounters =
...
void LFGQueue::UpdateQueueTimers(uint32 diff)
{
    ...
    if (!sendQueueStatus) {
        for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin();
             itQueue != QueueDataStore.end();) {
            ...
            if (itQueue->second.bestCompatible.empty()) {
                ...
                if (numOfCompatibles /*must be positive, because proposals don't
                                        delete QueueQueueData*/
                    && currTime - itQueue->second.lastRefreshTime >= 60 &&
                    numOfCompatibles <
                        (5 - itQueue->second.bestCompatible.roles->size()) *
                            25) {
    ...
    for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin();
        ...
        for (LfgRolesMap::const_iterator itPlayer = queueinfo.roles.begin();
             itPlayer != queueinfo.roles.end();
             ++itPlayer)
        ...
        LfgQueueStatusData queueData(dungeonId,
                                     waitTime,
                                     wtAvg,
                                     wtTank,
                                     wtHealer,
...
uint32
LFGQueue::FindBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue)
{
    uint32 numOfCompatibles = 0;
    for (LfgCompatibleContainer::const_iterator itr = CompatibleList.begin();
         itr != CompatibleList.end();
         ++itr)
...
void LFGQueue::UpdateBestCompatibleInQueue(
    LfgQueueDataContainer::iterator itrQueue, Lfg5Guids const& key)
{
    ...
    for (LfgRolesMap::const_iterator it = key.roles->begin();
         it != key.roles->end();
         ++it) {
