...
enum LfgCompatibility {
    ...
    LFG_COMPATIBLES_WITH_LESS_PLAYERS, // Values under this = not compatible (do
                                       // not modify order)
...
struct LfgQueueData {
    ...
    LfgQueueData(time_t _joinTime, LfgDungeonSet _dungeons, LfgRolesMap _roles)
        : joinTime(_joinTime), lastRefreshTime(_joinTime),
          tanks(LFG_TANKS_NEEDED), healers(LFG_HEALERS_NEEDED),
          dps(LFG_DPS_NEEDED), dungeons(std::move(_dungeons)),
          roles(std::move(_roles))
    {
    }
...
public:
    ...
    void RemoveFromQueue(ObjectGuid guid,
                         bool partial = false); // xinef: partial remove, dont
                                                // delete data from list!
    void AddQueueData(ObjectGuid           guid,
                      time_t               joinTime,
...
private:
    void SetQueueUpdateData(std::string const& strGuids,
    ...
    void   UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue,
                                       Lfg5Guids const&                key);

    LfgCompatibility FindNewGroups(const ObjectGuid& newGuid);
    LfgCompatibility
    CheckCompatibility(Lfg5Guids const&           checkWith,
    ...
    LfgCompatibleContainer
        CompatibleTempList; // new compatibles are added to this container while
                            // main one is being iterated

    LfgWaitTimesContainer waitTimesAvgStore; // Average wait time to find a
                                             // group queuing as multiple roles
    LfgWaitTimesContainer
        waitTimesTankStore; // Average wait time to find a group queuing as tank
    LfgWaitTimesContainer waitTimesHealerStore; // Average wait time to find a
                                                // group queuing as healer
    LfgWaitTimesContainer
        waitTimesDpsStore; // Average wait time to find a group queuing as dps
