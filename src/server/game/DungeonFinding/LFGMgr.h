...
enum LfgJoinResult {
    ...
    LFG_JOIN_NOT_MEET_REQS =
        5, // You do not meet the requirements for the chosen dungeons
    LFG_JOIN_PARTY_NOT_MEET_REQS =
        6, // One or more party members do not meet the requirements for the
           // chosen dungeons
    LFG_JOIN_MIXED_RAID_DUNGEON =
        7, // You cannot mix dungeons, raids, and random when picking dungeons
    LFG_JOIN_MULTI_REALM = 8, // The dungeon you chose does not support players
                              // from multiple realms
    LFG_JOIN_DISCONNECTED =
        9, // One or more party members are pending invites or disconnected
    LFG_JOIN_PARTY_INFO_FAILED =
        10, // Could not retrieve information about some party members
    LFG_JOIN_DUNGEON_INVALID = 11, // One or more dungeons was not valid
    LFG_JOIN_DESERTER        = 12, // You can not queue for dungeons until your
                                   // deserter debuff wears off
    LFG_JOIN_PARTY_DESERTER =
        13, // One or more party members has a deserter debuff
    LFG_JOIN_RANDOM_COOLDOWN = 14, // You can not queue for random dungeons
                                   // while on random dungeon cooldown
    LFG_JOIN_PARTY_RANDOM_COOLDOWN =
        15, // One or more party members are on random dungeon cooldown
    LFG_JOIN_TOO_MUCH_MEMBERS =
        16, // You can not enter dungeons with more that 5 party members
    LFG_JOIN_USING_BG_SYSTEM =
        17 // You can not use the dungeon system while in BG or arenas
};
...
enum LfgRoleCheckState {
    ...
    LFG_ROLECHECK_MISSING_ROLE =
        3, // Someone didn't selected a role after 2 mins
    LFG_ROLECHECK_WRONG_ROLES =
        4,                     // Can't form a group with that role selection
...
struct RBEntryInfo {
    RBEntryInfo() = default;
    RBEntryInfo(uint8 _roles, std::string _comment)
        : roles(_roles), comment(std::move(_comment))
    {
    }
    uint8       roles;
    std::string comment;
...
struct RBInternalInfo {
    ...
    RBInternalInfo(ObjectGuid  guid,
                   std::string comment,
                   bool        isGroupLeader,
                   ObjectGuid  groupGuid,
                   uint8       roles,
                   ...
                   uint32 _hasteRating,
                   uint32 _expertiseRating)
        : guid(guid), comment(std::move(comment)), isGroupLeader(isGroupLeader),
          groupGuid(groupGuid), roles(roles), encounterMask(encounterMask),
          instanceGuid(instanceGuid), _online(_online), _level(_level),
          _class(_class), _race(_race), _avgItemLevel(_avgItemLevel),
          _talents0(_talents[0]), _talents1(_talents[1]),
          _talents2(_talents[2]), _area(_area), _armor(_armor),
          _spellDamage(_spellDamage), _spellHeal(_spellHeal),
          _critRatingMelee(_critRatingMelee),
          _critRatingRanged(_critRatingRanged),
          _critRatingSpell(_critRatingSpell), _mp5(_mp5),
          _mp5combat(_mp5combat), _attackPower(_attackPower),
          _agility(_agility), _health(_health), _mana(_mana),
          _defenseSkill(_defenseSkill), _dodgeRating(_dodgeRating),
          _blockRating(_blockRating), _parryRating(_parryRating),
          _hasteRating(_hasteRating), _expertiseRating(_expertiseRating)
    {
    }
    [[nodiscard]] bool PlayerSameAs(RBInternalInfo const& i) const
    {
        return isGroupLeader == i.isGroupLeader && groupGuid == i.groupGuid &&
               roles == i.roles &&
               (isGroupLeader ||
                (comment == i.comment && encounterMask == i.encounterMask &&
                 instanceGuid == i.instanceGuid)) &&
               _online == i._online && _level == i._level &&
               _class == i._class && _race == i._race &&
               std::fabs(_avgItemLevel - i._avgItemLevel) < 0.01f &&
               _talents0 == i._talents0 && _talents1 == i._talents1 &&
               _talents2 == i._talents2 && _area == i._area &&
               _armor == i._armor && _spellDamage == i._spellDamage &&
               ...
               std::fabs(_mp5 - i._mp5) < 0.01f &&
               std::fabs(_mp5combat - i._mp5combat) < 0.01f &&
               _attackPower == i._attackPower && _agility == i._agility &&
               _health == i._health && _mana == i._mana &&
...
typedef std::pair<LfgRewardContainer::const_iterator,
...
struct LfgJoinResultData {
    LfgJoinResultData(LfgJoinResult     _result = LFG_JOIN_OK,
                      LfgRoleCheckState _state  = LFG_ROLECHECK_DEFAULT)
...
struct LfgUpdateData {
    LfgUpdateData(LfgUpdateType _type = LFG_UPDATETYPE_DEFAULT)
        : updateType(_type), comment("")
    {
    }
    LfgUpdateData(LfgUpdateType _type,
                  LfgDungeonSet _dungeons,
                  std::string   _comment)
        : updateType(_type), state(LFG_STATE_NONE),
          dungeons(std::move(_dungeons)), comment(std::move(_comment))
    {
    }
    LfgUpdateData(LfgUpdateType _type,
                  LfgState      _state,
                  LfgDungeonSet _dungeons,
                  std::string   _comment = "")
        : updateType(_type), state(_state), dungeons(std::move(_dungeons)),
...
struct LfgQueueStatusData {
    LfgQueueStatusData(uint32 _dungeonId      = 0,
                       int32  _waitTime       = -1,
                       ...
                       uint8  _healers        = 0,
                       uint8  _dps            = 0)
        : dungeonId(_dungeonId), waitTime(_waitTime), waitTimeAvg(_waitTimeAvg),
          waitTimeTank(_waitTimeTank), waitTimeHealer(_waitTimeHealer),
          waitTimeDps(_waitTimeDps), queuedTime(_queuedTime), tanks(_tanks),
          healers(_healers), dps(_dps)
    {
    }
...
struct LfgPlayerRewardData {
    LfgPlayerRewardData(uint32       random,
                        uint32       current,
                        bool         _done,
                        Quest const* _quest)
        : rdungeonEntry(random), sdungeonEntry(current), done(_done),
          quest(_quest)
    {
    }
...
struct LfgProposalPlayer {
    ...
    LfgAnswer accept{LFG_ANSWER_PENDING}; ///< Accept status (-1 not answer | 0
                                          ///< Not agree | 1 agree)
...
struct LfgRoleCheck {
    ...
    LfgDungeonSet
        dungeons; ///< Dungeons group is applying for (expanded random dungeons)
...
struct LfgPlayerBoot {
    ...
    LfgAnswerContainer
                votes; ///< Player votes (-1 not answer | 0 Not agree | 1 agree)
...
struct LFGDungeonData {
    ...
    LFGDungeonData(LFGDungeonEntry const* dbc)
        : id(dbc->ID), name(dbc->Name[0]), map(dbc->MapID), type(dbc->TypeID),
          expansion(uint8(dbc->ExpansionLevel)), group(uint8(dbc->GroupID)),
          minlevel(uint8(dbc->MinLevel)), maxlevel(uint8(dbc->MaxLevel)),
          difficulty(Difficulty(dbc->Difficulty)),
          seasonal((dbc->Flags & LFG_FLAG_SEASONAL) != 0), x(0.0f), y(0.0f),
          z(0.0f), o(0.0f)
    {
    }
...
private:
    ...
    typedef std::unordered_map<ObjectGuid /*playerGuid*/, RBEntryInfo>
        RBEntryInfoMap;
    typedef std::unordered_map<uint32 /*dungeonId*/, RBEntryInfoMap> RBStoreMap;
    RBStoreMap RaidBrowserStore[2]; // for 2 factions
    typedef std::unordered_map<ObjectGuid /*playerGuid*/, uint32 /*dungeonId*/>
                   RBSearchersMap;
    ...
    typedef std::unordered_map<ObjectGuid /*guid*/, RBInternalInfo>
        RBInternalInfoMap;
    typedef std::unordered_map<uint32 /*dungeonId*/, RBInternalInfoMap>
    ...
    typedef std::set<uint32 /*dungeonId*/>
                      RBUsedDungeonsSet;      // needs to be ordered
...
public:
    ...
    // World.cpp
    /// Finish the dungeon for the given group. All check are performed using
    /// internal lfg data
    ...
    /// Initializes locked dungeons for given player (called at login or level
    /// change)
    ...
    /// Gets the random dungeon reward corresponding to given dungeon and player
    /// level
    ...
    void InitBoot(ObjectGuid         gguid,
                  ObjectGuid         kicker,
                  ObjectGuid         victim,
    ...
    void UpdateRoleCheck(ObjectGuid gguid,
    ...
    void LeaveAllLfgQueues(ObjectGuid guid,
                           bool       allowgroup,
                           ObjectGuid groupguid = ObjectGuid::Empty);
    /// pussywizard: Raid Browser
    void JoinRaidBrowser(Player*        player,
                         uint8          roles,
                         LfgDungeonSet& dungeons,
                         std::string    comment);
    ...
    void SendRaidBrowserJoinedPacket(Player*        p,
                                     LfgDungeonSet& dungeons,
                                     std::string    comment);
    ...
    void RBPacketBuildFull(WorldPacket&       fullPacket,
                           uint32             dungeonId,
    ...
    static void SendLfgQueueStatus(ObjectGuid                guid,
...
private:
    ...
    void   GetCompatibleDungeons(LfgDungeonSet&    dungeons,
    ...
    // Proposals
    void RemoveProposal(LfgProposalContainer::iterator itProposal,
                        LfgUpdateType                  type);
    ...
    uint32 lastProposalId;  ///< pussywizard, store it here because of splitting
                            ///< LFGMgr update into tasks
    uint32 m_raidBrowserUpdateTimer[2]; ///< pussywizard
    uint32
        m_raidBrowserLastUpdatedDungeonId[2]; ///< pussywizard: for 2 factions

    LfgQueueContainer QueuesStore; ///< Queues
    LfgCachedDungeonContainer
        CachedDungeonMapStore; ///< Stores all dungeons by groupType
    // Reward System
