...
enum LfgUpdateType {
    ...
    LFG_UPDATETYPE_GROUP_DISBAND_UNK16 =
        16, // FIXME: Sometimes at group disband
};
...
public:
    ...
    {
        return g && (guids[0] == g || guids[1] == g || guids[2] == g ||
                     guids[3] == g || guids[4] == g);
    }
    ...
    bool operator==(const Lfg5Guids& x) const
    {
        return guids[0] == x.guids[0] && guids[1] == x.guids[1] &&
               guids[2] == x.guids[2] && guids[3] == x.guids[3] &&
               guids[4] == x.guids[4];
    }
    ...
    {
        std::ostringstream o;
        o << guids[0].ToString() << "," << guids[1].ToString() << ","
          << guids[2].ToString() << "," << guids[3].ToString() << ","
          << guids[4].ToString() << ":" << (roles ? 1 : 0);
        return o.str();
    }
};
