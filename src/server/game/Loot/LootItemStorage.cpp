...
void LootItemStorage::LoadStorageFromDB()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_ITEMCONTAINER_ITEMS);
    ...
    do {
        ...
        StoredLootItemList& itemList =
            lootItemStore[ObjectGuid::Create<HighGuid::Item>(
                fields[0].Get<uint32>())];
        itemList.push_back(StoredLootItem(fields[1].Get<uint32>(),
                                          fields[2].Get<uint32>(),
                                          fields[3].Get<uint32>(),
                                          fields[4].Get<int32>(),
                                          fields[5].Get<uint32>(),
...
void LootItemStorage::AddNewStoredLoot(Loot* loot, Player* /*player*/)
{
    if (lootItemStore.find(loot->containerGUID) != lootItemStore.end()) {
        LOG_INFO("misc",
                 "LootItemStorage::AddNewStoredLoot (A1) - {}!",
                 loot->containerGUID.ToString());
        return;
    }
    ...
    // Gold at first
    if (loot->gold) {
        itemList.push_back(StoredLootItem(0,
                                          0,
                                          loot->gold,
                                          0,
                                          0,
    ...
    // And normal items
    if (!loot->isLooted())
        for (LootItemList::const_iterator li = loot->items.begin();
             li != loot->items.end();
             li++) {
            // Even if an item is not available for a specific player, it
            // doesn't mean that we are not able to trade this container to
            // another player that is able to loot that item if we don't save it
            // then the item will be lost at player re-login.
            // if (!li->AllowedForPlayer(player))
            //    continue;

            ItemTemplate const* itemTemplate =
                sObjectMgr->GetItemTemplate(li->itemid);
            ...
            itemList.push_back(StoredLootItem(li->itemid,
                                              li->itemIndex,
                                              li->count,
                                              li->randomPropertyId,
                                              li->randomSuffix,
...
bool LootItemStorage::LoadStoredLoot(Item* item, Player* player)
{
    ...
    for (StoredLootItemList::iterator it2 = itemList.begin();
         it2 != itemList.end();
         ++it2) {
        if (it2->itemid == 0) {
            loot->gold = it2->count;
        ...
        if (LootTemplate const* lt =
                LootTemplates_Item.GetLootFor(item->GetEntry())) {
            LootItem li;
            ...
            if ((!li.needs_quest && li.conditions.empty() &&
