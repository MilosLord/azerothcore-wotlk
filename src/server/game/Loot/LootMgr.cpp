...
LootStore LootTemplates_Skinning("skinning_loot_template",
                                 ...
                                 true);
LootStore LootTemplates_Spell("spell_loot_template",
                              "spell id (random item creating)",
                              false);
LootStore LootTemplates_Player("player_loot_template", "team id", true);

// Selects invalid loot items to be removed from group possible entries (before
// rolling)
struct LootGroupInvalidSelector
    : public Acore::unary_function<LootStoreItem*, bool> {
    explicit LootGroupInvalidSelector(Loot const& loot, uint16 lootMode)
        : _loot(loot), _lootMode(lootMode)
    {
    }
    ...
    bool operator()(LootStoreItem* item) const
    {
        ...
        if (!item->reference) {
            ItemTemplate const* _proto =
                sObjectMgr->GetItemTemplate(item->itemid);
            if (!_proto)
                return true;

            uint8 foundDuplicates = 0;
            for (std::vector<LootItem>::const_iterator itr =
                     _loot.items.begin();
                 itr != _loot.items.end();
                 ++itr)
                if (itr->itemid == item->itemid &&
                    itr->groupid == item->groupid) {
                    ++foundDuplicates;
                    if (_proto->InventoryType == 0 && foundDuplicates == 3 &&
                        _proto->ItemId !=
                            47242 /*Trophy of the Crusade*/) // Non-equippable
                                                             // items are
                                                             // limited to 3
                                                             // drops
                        return true;
                    else if (_proto->InventoryType != 0 &&
                             foundDuplicates ==
                                 1) // Equippable item are limited to 1 drop
                        return true;
                }
        }
...
public:
    ...
    bool HasQuestDrop(LootTemplateMap const& store)
        const; // True if group includes at least 1 quest drop entry
    bool HasQuestDropForPlayer(Player const*          player,
                               LootTemplateMap const& store) const;
    ...
    void Process(Loot&            loot,
        ...
        const; // Rolls an item from the group (if any) and adds the item to the
               // loot
...
private:
    ...
    Roll(Loot&            loot,
         ...
         uint16 lootMode) const; // Rolls an item from the group, returns
...
{
    for (LootTemplateMap::const_iterator i = m_LootTemplates.begin();
         i != m_LootTemplates.end();
         ++i)
...
uint32 LootStore::LoadLootTable()
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "GroupId, MinCount, MaxCount FROM {}",
        GetName());
    ...
    do {
        ...
        if (maxcount > std::numeric_limits<uint8>::max()) {
            LOG_ERROR("sql.sql",
                      "Table '{}' Entry {} Item {}: MaxCount value ({}) to "
                      "large. must be less {} - skipped",
                      GetName(),
                      entry,
                      item,
                      maxcount,
        ...
        if (lootmode == 0) {
            LOG_ERROR("sql.sql",
                      "Table '{}' Entry {} Item {}: LootMode is equal to 0, "
                      "item will never drop - setting mode 1",
                      GetName(),
                      entry,
                      item);
            lootmode = 1;
        ...
        LootStoreItem* storeitem = new LootStoreItem(item,
                                                     reference,
                                                     chance,
                                                     needsquest,
                                                     lootmode,
        ...
        if (m_LootTemplates.empty() || tab->first != entry) {
            ...
            if (tab == m_LootTemplates.end()) {
                std::pair<LootTemplateMap::iterator, bool> pr =
                    m_LootTemplates.insert(
                        LootTemplateMap::value_type(entry, new LootTemplate()));
                tab = pr.first;
            }
        }
        // else is empty - template Id and iter are the same
        // finally iter refers to already existed or just created <entry,
        // LootTemplate>
...
{
    for (LootTemplateMap::iterator itr = m_LootTemplates.begin();
         itr != m_LootTemplates.end();
         ++itr) {
        ConditionList empty;
...
{
    for (LootTemplateMap::const_iterator ltItr = m_LootTemplates.begin();
         ltItr != m_LootTemplates.end();
         ++ltItr)
...
void LootStore::ReportUnusedIds(LootIdSet const& lootIdSet) const
{
    ...
    for (LootIdSet::const_iterator itr = lootIdSet.begin();
         itr != lootIdSet.end();
         ++itr)
        LOG_ERROR("sql.sql",
                  "Table '{}' Entry {} isn't {} and not referenced from loot, "
                  "and thus useless.",
                  GetName(),
                  *itr,
                  GetEntryName());
...
{
    LOG_ERROR(
        "sql.sql", "Table '{}' Entry {} does not exist", GetName(), lootId);
}
                         ...
                         LootStore const& store) const
{
    ...
    if (reference) // reference case
        return roll_chance_f(
            _chance *
            (rate ? sWorld->getRate(RATE_DROP_ITEM_REFERENCED) : 1.0f));

    ItemTemplate const* pProto = sObjectMgr->GetItemTemplate(itemid);

    float qualityModifier =
        pProto && rate ? sWorld->getRate(qualityToRate[pProto->Quality]) : 1.0f;
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        LOG_ERROR("sql.sql",
                  "Table '{}' Entry {} Item {}: GroupId ({}) must be less {} - "
                  "skipped",
                  store.GetName(),
                  entry,
                  itemid,
    ...
    if (mincount == 0) {
        LOG_ERROR("sql.sql",
                  "Table '{}' Entry {} Item {}: wrong MinCount ({}) - skipped",
                  store.GetName(),
                  entry,
                  itemid,
                  mincount);
    ...
    if (!reference) // item (quest or non-quest) entry, maybe grouped
    {
        ...
        if (!proto) {
            LOG_ERROR("sql.sql",
                      "Table '{}' Entry {} Item {}: item entry not listed in "
        ...
        {
            LOG_ERROR("sql.sql",
                      "Table '{}' Entry {} Item {}: equal-chanced grouped "
        ...
        {
            LOG_ERROR("sql.sql",
                      "Table '{}' Entry {} Item {}: MaxCount ({}) less that "
                      "MinCount ({}) - skipped",
                      store.GetName(),
                      entry,
                      itemid,
    ...
    else // if reference loot
    {
        if (needs_quest)
            LOG_ERROR(
                "sql.sql",
                "Table '{}' Entry {} Item {}: quest required will be ignored",
                store.GetName(),
                entry,
                itemid);
        ...
        {
            LOG_ERROR("sql.sql",
                      "Table '{}' Entry {} Item {}: zero chance is specified "
...
LootItem::LootItem(LootStoreItem const& li)
{
    ...
    follow_loot_rules =
        proto && (proto->FlagsCu & ITEM_FLAGS_CU_FOLLOW_LOOT_RULES);
...
bool LootItem::AllowedForPlayer(Player const* player, ObjectGuid source) const
{
    ...
    bool isMasterLooter =
        player->GetGroup() &&
        player->GetGroup()->GetMasterLooterGuid() == player->GetGUID();
    bool itemVisibleForMasterLooter =
        !needs_quest && (!follow_loot_rules || !is_underthreshold);

    // DB conditions check
    if (!sConditionMgr->IsObjectMeetToConditions(const_cast<Player*>(player),
                                                 conditions)) {
        // Master Looter can see conditioned recipes
        if (isMasterLooter && itemVisibleForMasterLooter) {
            if ((pProto->Flags & ITEM_FLAG_HIDE_UNUSABLE_RECIPE) ||
                (pProto->Class == ITEM_CLASS_RECIPE &&
                 pProto->Bonding == BIND_WHEN_PICKED_UP &&
                 pProto->Spells[1].SpellId != 0)) {
                return true;
            }
        }
    ...
    if ((pProto->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY) &&
        player->GetTeamId(true) != TEAM_HORDE) {
        return false;
    }

    if ((pProto->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY) &&
    ...
    // Don't allow loot for players without profession or those who already know
    // the recipe
    if ((pProto->Flags & ITEM_FLAG_HIDE_UNUSABLE_RECIPE) &&
        (!player->HasSkill(pProto->RequiredSkill) ||
         player->HasSpell(pProto->Spells[1].SpellId))) {
        return false;
    }
    ...
    if (!(pProto->FlagsCu & ITEM_FLAGS_CU_IGNORE_QUEST_STATUS) &&
        ((needs_quest ||
          (pProto->StartQuest &&
           player->GetQuestStatus(pProto->StartQuest) != QUEST_STATUS_NONE)) &&
...
void Loot::AddItem(LootStoreItem const& item)
{
    ...
    uint32 stacks = count / proto->GetMaxStackSize() +
                    (count % proto->GetMaxStackSize() ? 1 : 0);
    ...
    for (uint32 i = 0; i < stacks && lootItems.size() < limit; ++i) {
        ...
        // In some cases, a dropped item should be visible/lootable only for
        // some players in group
        bool canSeeItemInLootWindow = false;
        if (auto player = ObjectAccessor::FindPlayer(lootOwnerGUID)) {
            if (auto group = player->GetGroup()) {
                for (auto itr = group->GetFirstMember(); itr != nullptr;
                     itr      = itr->next()) {
                    if (auto member = itr->GetSource()) {
                        if (generatedLoot.AllowedForPlayer(
                                member, sourceWorldObjectGUID)) {
        ...
        if (!canSeeItemInLootWindow) {
            LOG_DEBUG("loot",
                      "Skipping ++unlootedCount for unlootable item: {}",
                      item.itemid);
            continue;
        }
                    ...
                    WorldObject*     lootSource /*= nullptr*/)
{
    ...
    if (!tab) {
        if (!noEmptyError)
            LOG_ERROR(
                "sql.sql",
                "Table '{}' loot id #{} used but it doesn't have records.",
                store.GetName(),
                lootId);
        return false;
    }
    ...
    tab->Process(
        *this,
        ...
        lootOwner,
        0,
        true); // Processing is done there, callback via Loot::AddItem()

    sScriptMgr->OnAfterLootTemplateProcess(
        this, tab, store, lootOwner, personal, noEmptyError, lootMode);
    ...
    if (!personal && group) {
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
             itr                 = itr->next()) {
            if (Player* player =
                    itr->GetSource()) // should actually be looted object
                                      // instead of lootOwner but looter has to
                                      // be really close so doesnt really matter
            {
                if (player->IsAtLootRewardDistance(lootSource ? lootSource
                                                              : lootOwner)) {
        ...
        for (uint8 i = 0; i < items.size(); ++i) {
            if (ItemTemplate const* proto =
                    sObjectMgr->GetItemTemplate(items[i].itemid))
...
void Loot::FillNotNormalLootFor(Player* player)
{
    ...
    for (uint32 i = 0; i < max_slot; ++i) {
        ...
        if (!item->is_looted && item->freeforall &&
            item->AllowedForPlayer(player, sourceWorldObjectGUID))
            if (ItemTemplate const* proto =
                    sObjectMgr->GetItemTemplate(item->itemid))
...
QuestItemList* Loot::FillFFALoot(Player* player)
{
    ...
    for (uint8 i = 0; i < items.size(); ++i) {
        ...
        if (!item.is_looted && item.freeforall &&
            item.AllowedForPlayer(player, containerGUID)) {
...
QuestItemList* Loot::FillQuestLoot(Player* player)
{
    ...
    Player* lootOwner = (roundRobinPlayer)
                            ? ObjectAccessor::FindPlayer(roundRobinPlayer)
                            : player;
    ...
    for (uint8 i = 0; i < quest_items.size(); ++i) {
        ...
        // Quest item is not free for all and is already assigned to another
        // player or player doesn't need it
        if (item.is_blocked ||
            !item.AllowedForPlayer(player, sourceWorldObjectGUID)) {
            continue;
        }

        // Player is not the loot owner, and loot owner still needs this quest
        // item
        if (!item.freeforall && lootOwner != player &&
...
void Loot::NotifyItemRemoved(uint8 lootIndex)
{
    ...
    for (PlayersLootingSet::iterator i = PlayersLooting.begin();
         i != PlayersLooting.end();
         i = i_next) {
        i_next = i;
        ++i_next;
...
void Loot::NotifyQuestItemRemoved(uint8 questIndex)
{
    ...
    for (PlayersLootingSet::iterator i = PlayersLooting.begin();
        ...
        ++i_next;
        if (Player* player = ObjectAccessor::FindPlayer(*i)) {
            QuestItemMap::const_iterator pq =
...
void Loot::generateMoneyLoot(uint32 minAmount, uint32 maxAmount)
{
    if (maxAmount > 0) {
        if (maxAmount <= minAmount)
            ...
            gold = uint32(urand(minAmount, maxAmount) *
                          sWorld->getRate(RATE_DROP_MONEY));
        else
            gold = uint32(urand(minAmount >> 8, maxAmount >> 8) *
                          sWorld->getRate(RATE_DROP_MONEY))
                   << 8;
    }
}
    ...
    if (lootSlot >= items.size()) {
        ...
        if (itr != PlayerQuestItems.end() && questSlot < itr->second->size()) {
            ...
            if (item->follow_loot_rules &&
                !item->AllowedForPlayer(
                    player,
                    sourceWorldObjectGUID)) // pussywizard: such items
                                            // (follow_loot_rules) are added to
                                            // every player, but not everyone is
                                            // allowed, check it here
                return nullptr;
        ...
        if (item->freeforall) {
            QuestItemMap::const_iterator itr =
                PlayerFFAItems.find(player->GetGUID());
            if (itr != PlayerFFAItems.end()) {
                for (QuestItemList::const_iterator iter = itr->second->begin();
                     iter != itr->second->end();
                     ++iter)
            ...
            QuestItemMap::const_iterator itr =
                PlayerNonQuestNonFFAConditionalItems.find(player->GetGUID());
...
uint32 Loot::GetMaxSlotInLootFor(Player* player) const
{
    ...
    return items.size() +
           (itr != PlayerQuestItems.end() ? itr->second->size() : 0);
}
...
bool Loot::hasItemFor(Player* player) const
{
    ...
    QuestItemMap::const_iterator q_itr =
        lootPlayerQuestItems.find(player->GetGUID());
    if (q_itr != lootPlayerQuestItems.end()) {
        QuestItemList* q_list = q_itr->second;
        for (QuestItemList::const_iterator qi = q_list->begin();
             qi != q_list->end();
             ++qi) {
    ...
    QuestItemMap::const_iterator ffa_itr =
        lootPlayerFFAItems.find(player->GetGUID());
    if (ffa_itr != lootPlayerFFAItems.end()) {
        QuestItemList* ffa_list = ffa_itr->second;
        for (QuestItemList::const_iterator fi = ffa_list->begin();
             fi != ffa_list->end();
             ++fi) {
    ...
    QuestItemMap const& lootPlayerNonQuestNonFFAConditionalItems =
        GetPlayerNonQuestNonFFAConditionalItems();
    QuestItemMap::const_iterator nn_itr =
        lootPlayerNonQuestNonFFAConditionalItems.find(player->GetGUID());
    if (nn_itr != lootPlayerNonQuestNonFFAConditionalItems.end()) {
        QuestItemList* conditional_list = nn_itr->second;
        for (QuestItemList::const_iterator ci = conditional_list->begin();
             ci != conditional_list->end();
             ++ci) {
...
// return true if there is any item over the group threshold (i.e. not
// underthreshold).
bool Loot::hasOverThresholdItem() const
{
    for (uint8 i = 0; i < items.size(); ++i) {
        if (!items[i].is_looted && !items[i].is_underthreshold &&
            !items[i].freeforall)
            return true;
    }
...
ByteBuffer& operator<<(ByteBuffer& b, LootItem const& li)
{
    ...
    // b << uint8(0);                                        // slot type - will
    // send after this function call
    return b;
}
...
ByteBuffer& operator<<(ByteBuffer& b, LootView const& lv)
{
    ...
    case RESTRICTED_PERMISSION: {
        bool isMasterLooter = lv.viewer->GetGroup() &&
                              lv.viewer->GetGroup()->GetMasterLooterGuid() ==
                                  lv.viewer->GetGUID();
        ...
        for (uint8 i = 0; i < l.items.size(); ++i) {
            if (!l.items[i].is_looted && !l.items[i].freeforall &&
                (l.items[i].conditions.empty() || isMasterLooter) &&
                l.items[i].AllowedForPlayer(lv.viewer,
                                            l.sourceWorldObjectGUID)) {
                uint8 slot_type = 0;
                                           ...
                                           // !is_underthreshold
                {
                    ...
                    case MASTER_PERMISSION: {
                        if (lv.viewer->GetGroup()) {
                            if (lv.viewer->GetGroup()->GetMasterLooterGuid() ==
                                lv.viewer->GetGUID())
    ...
    case OWNER_PERMISSION: {
        uint8 slot_type = lv.permission == OWNER_PERMISSION
    ...
    LootSlotType slotType = lv.permission == OWNER_PERMISSION
                                ? LOOT_SLOT_TYPE_OWNER
                                : LOOT_SLOT_TYPE_ALLOW_LOOT;

    // Xinef: items that do not follow loot rules need this
    LootSlotType partySlotType =
        lv.permission == MASTER_PERMISSION ? LOOT_SLOT_TYPE_MASTER : slotType;

    QuestItemMap const&          lootPlayerQuestItems = l.GetPlayerQuestItems();
    QuestItemMap::const_iterator q_itr =
        lootPlayerQuestItems.find(lv.viewer->GetGUID());
    if (q_itr != lootPlayerQuestItems.end()) {
        QuestItemList* q_list = q_itr->second;
        for (QuestItemList::const_iterator qi = q_list->begin();
             qi != q_list->end();
            ...
            if (!qi->is_looted && !item.is_looted) {
                ...
                bool hasQuestForItem = lv.viewer->HasQuestForItem(
                    item.itemid, 0, false, &showInLoot);
                if (!hasQuestForItem) {
                    if (!showInLoot) {
                    ...
                    b << item;
                    b << uint8(lv.permission == MASTER_PERMISSION
                    ...
                    if (item.follow_loot_rules) {
                        ...
                        case MASTER_PERMISSION:
                            ...
                            break;
                        case RESTRICTED_PERMISSION:
                            b << (item.is_blocked ? uint8(LOOT_SLOT_TYPE_LOCKED)
                                                  : uint8(slotType));
                            break;
                        case GROUP_PERMISSION:
                        case ROUND_ROBIN_PERMISSION:
    ...
    QuestItemMap::const_iterator ffa_itr =
        lootPlayerFFAItems.find(lv.viewer->GetGUID());
    if (ffa_itr != lootPlayerFFAItems.end()) {
        QuestItemList* ffa_list = ffa_itr->second;
        for (QuestItemList::const_iterator fi = ffa_list->begin();
             fi != ffa_list->end();
    ...
    QuestItemMap const& lootPlayerNonQuestNonFFAConditionalItems =
        l.GetPlayerNonQuestNonFFAConditionalItems();
    QuestItemMap::const_iterator nn_itr =
        lootPlayerNonQuestNonFFAConditionalItems.find(lv.viewer->GetGUID());
    if (nn_itr != lootPlayerNonQuestNonFFAConditionalItems.end()) {
        QuestItemList* conditional_list = nn_itr->second;
        for (QuestItemList::const_iterator ci = conditional_list->begin();
             ci != conditional_list->end();
                                                   ...
                                                   uint16 lootMode) const
{
    ...
    if (!possibleLoot.empty()) // First explicitly chanced entries are checked
    {
        ...
        for (LootStoreItemList::const_iterator itr = possibleLoot.begin();
             itr != possibleLoot.end();
             ++itr) // check each explicitly chanced entry in the template and
    ...
    if (!sScriptMgr->OnBeforeLootEqualChanced(
            player, EqualChanced, loot, store))
        return nullptr;

    possibleLoot = EqualChanced;
    possibleLoot.remove_if(LootGroupInvalidSelector(loot, lootMode));
    if (!possibleLoot.empty()) // If nothing selected yet - an item is taken
...
{
    for (LootStoreItemList::const_iterator i = ExplicitlyChanced.begin();
         i != ExplicitlyChanced.end();
         ++i) {
        LootStoreItem* item = *i;
        if (item->reference) // References
        {
            LootTemplateMap::const_iterator Referenced =
                store.find(std::abs(item->reference));
    ...
    for (LootStoreItemList::const_iterator i = EqualChanced.begin();
         i != EqualChanced.end();
         ++i) {
        LootStoreItem* item = *i;
        if (item->reference) // References
...
bool LootTemplate::LootGroup::HasQuestDropForPlayer(
    Player const* player, LootTemplateMap const& store) const
{
    for (LootStoreItemList::const_iterator i = ExplicitlyChanced.begin();
         i != ExplicitlyChanced.end();
         ++i) {
...
{
    for (LootStoreItemList::iterator i = ExplicitlyChanced.begin();
         i != ExplicitlyChanced.end();
         ++i)
        (*i)->conditions.clear();

    for (LootStoreItemList::iterator i = EqualChanced.begin();
         i != EqualChanced.end();
         ++i)
        (*i)->conditions.clear();
}
...
void LootTemplate::LootGroup::Process(Loot&            loot,
                                      ...
                                      LootStore const& store,
                                      uint16           lootMode,
                                      uint16 nonRefIterationsLeft) const
{
    if (LootStoreItem const* item = Roll(loot, player, store, lootMode)) {
        ...
        if (item->reference) // References processing
        {
            if (LootTemplate const* Referenced =
                    LootTemplates_Reference.GetLootFor(
                        std::abs(item->reference))) {
                uint32 maxcount =
                    uint32(float(item->maxcount) *
                           sWorld->getRate(RATE_DROP_ITEM_REFERENCED_AMOUNT));
                sScriptMgr->OnAfterRefCount(player,
                                            loot,
                                            rate,
                                            lootMode,
                                            const_cast<LootStoreItem*>(item),
                                            maxcount,
                                            store);
                for (uint32 loop = 0; loop < maxcount;
                     ++loop) // Ref multiplicator
                    // This reference needs to be processed further, but it is
                    // marked isTopLevel=false so that any groups inside the
                    // reference are not multiplied by
                    // Rate.Drop.Item.GroupAmount
                    Referenced->Process(
                        loot, store, lootMode, player, item->groupid, false);
            }
        }
        else {
            // Plain entries (not a reference, not grouped)
            sScriptMgr->OnBeforeDropAddItem(player,
                                            loot,
                                            rate,
                                            lootMode,
                                            const_cast<LootStoreItem*>(item),
            ...
            // If we still have non-ref runs to do for this group AND this item
            // wasn't a reference, recursively call this function to produce
            // more items for this group. However, if this is a quest item we
            // shouldn't multiply this group.
            if (nonRefIterationsLeft > 1 && !item->needs_quest) {
                this->Process(
                    loot, player, store, lootMode, nonRefIterationsLeft - 1);
            }
        }
    }
}
                                     ...
                                     uint8            group_id) const
{
    ...
    {
        LOG_ERROR("sql.sql",
                  "Table '{}' entry {} group {} has total chance > 100% ({})",
                  lootstore.GetName(),
                  id,
                  group_id,
                  chance);
    ...
    if (chance >= 100.0f && !EqualChanced.empty()) {
        LOG_ERROR("sql.sql",
                  "Table '{}' entry {} group {} has items with chance=0% but "
                  "group total chance >= 100% ({})",
                  lootstore.GetName(),
                  id,
                  group_id,
                  chance);
...
{
    for (LootStoreItemList::const_iterator ieItr = ExplicitlyChanced.begin();
         ieItr != ExplicitlyChanced.end();
         ++ieItr) {
        LootStoreItem* item = *ieItr;
        if (item->reference) {
            if (!LootTemplates_Reference.GetLootFor(
                    std::abs(item->reference))) {
                LootTemplates_Reference.ReportNonExistingId(
                    std::abs(item->reference), "Reference", item->itemid);
            }
            else if (ref_set) {
    ...
    for (LootStoreItemList::const_iterator ieItr = EqualChanced.begin();
         ieItr != EqualChanced.end();
         ++ieItr) {
        LootStoreItem* item = *ieItr;
        if (item->reference) {
            if (!LootTemplates_Reference.GetLootFor(
...
{
    // `item->reference` > 0 --> Reference is counted as a normal and non
    // grouped entry `item->reference` < 0 --> Reference is counted as grouped
    // entry within shared groupid
    if (item->groupid > 0 &&
        item->reference <= 0) // Group and grouped reference
    {
        if (item->groupid >= Groups.size()) {
            Groups.resize(
                item->groupid,
                nullptr); // Adds new group the the loot template if needed
        }
        ...
        Groups[item->groupid - 1]->AddEntry(
            item); // Adds new entry to the group
    }
...
{
    for (LootStoreItemList::iterator i = Entries.begin(); i != Entries.end();
         ++i)
        (*i)->conditions.clear();
...
{
    for (LootStoreItemList::const_iterator _iter = Entries.begin();
         _iter != Entries.end();
         ++_iter) {
        ...
        if (item->reference) {
            ...
            if (!item->conditions.empty() && conditionLootId &&
                conditionLootId != item->conditions.front()->SourceGroup) {
                continue;
            }
    ...
    for (LootGroups::const_iterator groupItr = Groups.begin();
         groupItr != Groups.end();
         ++groupItr) {
        ...
        for (LootStoreItemList::iterator i = itemList->begin();
             i != itemList->end();
             ++i) {
            LootStoreItem* item = *i;
            if (item->reference) {
                           ...
                           bool             isTopLevel) const
{
    ...
    if (groupId) // Group reference uses own processing of the group
    {
        ...
        // Rate.Drop.Item.GroupAmount is only in effect for the top loot
        // template level
        if (isTopLevel) {
            Groups[groupId - 1]->Process(
                loot,
                player,
                store,
                lootMode,
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin();
         i != Entries.end();
         ++i) {
        LootStoreItem* item = *i;
        ...
        if (item->reference) // References processing
        {
            LootTemplate const* Referenced =
                LootTemplates_Reference.GetLootFor(std::abs(item->reference));
            if (!Referenced)
                continue; // Error message already printed at loading stage
            ...
            uint32 maxcount =
                uint32(float(item->maxcount) *
                       sWorld->getRate(RATE_DROP_ITEM_REFERENCED_AMOUNT));
            sScriptMgr->OnAfterRefCount(
                player, loot, rate, lootMode, item, maxcount, store);
            ...
            sScriptMgr->OnBeforeDropAddItem(
                player, loot, rate, lootMode, item, store);
            loot.AddItem(*item); // Chance is already checked, just add
        }
    }
        ...
        if (LootGroup* group = *i) {
            ...
            if (isTopLevel) {
                ...
                sScriptMgr->OnAfterCalculateLootGroupAmount(
                    player, loot, lootMode, groupAmount, store);
                                ...
                                uint8                  groupId) const
{
    ...
    // Now processing groups
    for (LootGroups::const_iterator i = Groups.begin(); i != Groups.end();
         ++i) {
        if (LootGroup* group = *i) {
...
void LootTemplate::CheckLootRefs(LootTemplateMap const& store,
                                 LootIdSet*             ref_set) const
{
    for (LootStoreItemList::const_iterator ieItr = Entries.begin();
         ieItr != Entries.end();
         ++ieItr) {
        LootStoreItem* item = *ieItr;
        if (item->reference) {
    ...
    for (LootGroups::const_iterator grItr = Groups.begin();
         grItr != Groups.end();
         ++grItr)
...
bool LootTemplate::addConditionItem(Condition* cond)
{
    ...
    if (!Entries.empty()) {
        for (LootStoreItemList::iterator i = Entries.begin();
             i != Entries.end();
             ++i) {
    ...
    if (!Groups.empty()) {
        for (LootGroups::iterator groupItr = Groups.begin();
             groupItr != Groups.end();
             ++groupItr) {
            LootGroup* group = *groupItr;
            if (!group)
...
void LoadLootTemplates_Creature()
{
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
        ...
        if (uint32 lootid = itr->second.lootid) {
            if (lootIdSet.find(lootid) == lootIdSet.end())
                LootTemplates_Creature.ReportNonExistingId(
                    lootid, "Creature", itr->second.Entry);
            else
                lootIdSetUsed.insert(lootid);
        }
    }

    for (LootIdSet::const_iterator itr = lootIdSetUsed.begin();
         itr != lootIdSetUsed.end();
         ++itr)
        lootIdSet.erase(*itr);
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} Creature Loot Templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 creature loot templates. DB table "
                 "`creature_loot_template` is empty");
...
void LoadLootTemplates_Disenchant()
{
    ...
    for (ItemTemplateContainer::const_iterator itr = its->begin();
         itr != its->end();
         ++itr) {
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} disenchanting loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 disenchanting loot templates. DB table "
                 "`disenchant_loot_template` is empty");
    LOG_INFO("server.loading", " ");
}
...
void LoadLootTemplates_Fishing()
{
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} Fishing Loot Templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 fishing loot templates. DB table "
                 "`fishing_loot_template` is empty");
...
void LoadLootTemplates_Gameobject()
{
    ...
    for (GameObjectTemplateContainer::const_iterator itr = gotc->begin();
        ...
        if (uint32 lootid = itr->second.GetLootId()) {
            if (lootIdSet.find(lootid) == lootIdSet.end())
                LootTemplates_Gameobject.ReportNonExistingId(
                    lootid, "Gameobject", itr->second.entry);
            else
                lootIdSetUsed.insert(lootid);
        }
    }
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} Gameobject Loot Templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 gameobject loot templates. DB table "
                 "`gameobject_loot_template` is empty");
...
void LoadLootTemplates_Item()
{
    ...
    for (ItemTemplateContainer::const_iterator itr = its->begin();
        ...
        if (lootIdSet.find(itr->second.ItemId) != lootIdSet.end() &&
            itr->second.Flags & ITEM_FLAG_HAS_LOOT)
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} item loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 item loot templates. DB table "
                 "`item_loot_template` is empty");
...
void LoadLootTemplates_Milling()
{
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} milling loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 milling loot templates. DB table "
                 "`milling_loot_template` is empty");
...
void LoadLootTemplates_Pickpocketing()
{
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
        ...
        if (uint32 lootid = itr->second.pickpocketLootId) {
            if (lootIdSet.find(lootid) == lootIdSet.end())
                LootTemplates_Pickpocketing.ReportNonExistingId(
                    lootid, "Creature", itr->second.Entry);
            else
                lootIdSetUsed.insert(lootid);
        }
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} pickpocketing loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 pickpocketing loot templates. DB table "
                 "`pickpocketing_loot_template` is empty");
...
void LoadLootTemplates_Prospecting()
{
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} prospecting loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 prospecting loot templates. DB table "
                 "`prospecting_loot_template` is empty");
...
void LoadLootTemplates_Mail()
{
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} mail loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 mail loot templates. DB table "
                 "`mail_loot_template` is empty");
...
void LoadLootTemplates_Skinning()
{
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
        ...
        if (uint32 lootid = itr->second.SkinLootId) {
            if (lootIdSet.find(lootid) == lootIdSet.end())
                LootTemplates_Skinning.ReportNonExistingId(
                    lootid, "Creature", itr->second.Entry);
            else
                lootIdSetUsed.insert(lootid);
        }
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} skinning loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 skinning loot templates. DB table "
                 "`skinning_loot_template` is empty");
...
void LoadLootTemplates_Spell()
{
    ...
    // remove real entries and check existence loot
    for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize();
         ++spell_id) {
        ...
        if (lootIdSet.find(spell_id) == lootIdSet.end()) {
            // not report about not trainable spells (optionally supported by
            // DB) ignore 61756 (Northrend Inscription Research (FAST QA
            // VERSION) for example
            if (!spellInfo->HasAttribute(SPELL_ATTR0_NOT_SHAPESHIFTED) ||
                spellInfo->HasAttribute(SPELL_ATTR0_IS_TRADESKILL)) {
                LootTemplates_Spell.ReportNonExistingId(
                    spell_id, "Spell", spellInfo->Id);
            }
        }
        else
    ...
    if (count)
        LOG_INFO("server.loading",
                 ">> Loaded {} spell loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    else
        LOG_WARN("server.loading",
                 ">> Loaded 0 spell loot templates. DB table "
                 "`spell_loot_template` is empty");
    LOG_INFO("server.loading", " ");
}
...
void LoadLootTemplates_Player()
{
    ...
    if (count) {
        LOG_INFO("server.loading",
                 ">> Loaded {} player loot templates in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    }
    else {
        LOG_WARN("server.loading",
                 ">> Loaded 0 player loot templates. DB table "
                 "`player_loot_template` is empty");
    }
...
void LoadLootTemplates_Reference()
{
    ...
    LOG_INFO("server.loading",
             ">> Loaded refence loot templates in {} ms",
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
