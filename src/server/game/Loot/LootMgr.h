...
enum LootType {
    ...
    LOOT_FISHING_JUNK =
        22 // unsupported by client, sending LOOT_FISHING instead
};
...
enum LootError {
    ...
    LOOT_ERROR_NOT_WHILE_SHAPESHIFTED =
        16 // You can't do that while shapeshifted.
};
...
enum LootSlotType {
    ...
    LOOT_SLOT_TYPE_MASTER =
        2, // item can only be distributed by group loot master.
...
struct LootStoreItem {
    ...
    float  chance; // chance to drop for both quest and non-quest items, chance
                   // to be used for refs
    ...
    LootStoreItem(uint32 _itemid,
                  ...
                  int32  _mincount,
                  uint8  _maxcount)
        : itemid(_itemid), reference(_reference), chance(_chance),
          needs_quest(_needs_quest), lootmode(_lootmode), groupid(_groupid),
          mincount(_mincount), maxcount(_maxcount)
    {
    }

    bool
    Roll(bool rate, Player const* player, Loot& loot, LootStore const& store)
...
struct LootItem {
    ...
    ObjectGuid
        rollWinnerGUID; // Stores the guid of person who won loot, if his bags
                        // are full only he can see the item in loot list!
    uint8 count : 8;
    bool  is_looted : 1;
    bool  is_blocked : 1;
    ...
    // count and random suffixes/properties Should be called for non-reference
    // LootStoreItem entries only (reference = 0)
    explicit LootItem(LootStoreItem const& li);

    LootItem() = default;

    // Basic checks for player/item compatibility - if false no chance to see
    // the item in the loot
    bool AllowedForPlayer(Player const* player, ObjectGuid source) const;
    void AddAllowedLooter(Player const* player);
    [[nodiscard]] const AllowedLooterSet& GetAllowedLooters() const
    {
        return allowedGUIDs;
    }
};
...
class LootStore {
public:
    explicit LootStore(char const* name,
    ...
    void ReportNonExistingId(uint32      lootId,
                             const char* ownerType,
                             uint32      ownerId) const;

    [[nodiscard]] bool HaveLootFor(uint32 loot_id) const
    {
...
public:
    ...
    void Process(Loot&            loot,
                 ...
                 Player const*    player,
                 uint8            groupId    = 0,
                 bool             isTopLevel = true) const;
    void CopyConditions(ConditionList conditions);
    bool CopyConditions(LootItem* li, uint32 conditionLootId = 0) const;

    // True if template includes at least 1 quest drop entry
    [[nodiscard]] bool HasQuestDrop(LootTemplateMap const& store,
...
private:
    ...
    // Objects of this class must never be copied, we are storing pointers in
    // container
...
public:
    ...
    {
        return (LootValidatorRef*)RefMgr<Loot, LootValidatorRef>::getFirst();
    }
    LootValidatorRef* getLast()
    {
        return (LootValidatorRef*)RefMgr<Loot, LootValidatorRef>::getLast();
    }
...
struct Loot {
    ...
    [[nodiscard]] QuestItemMap const& GetPlayerQuestItems() const
    {
        return PlayerQuestItems;
    }
    [[nodiscard]] QuestItemMap const& GetPlayerFFAItems() const
    {
        return PlayerFFAItems;
    }
    [[nodiscard]] QuestItemMap const&
    GetPlayerNonQuestNonFFAConditionalItems() const
    {
    ...
    ObjectGuid
        roundRobinPlayer; // GUID of the player having the Round-Robin ownership
                          // for the loot. If 0, round robin owner has released.
    ObjectGuid lootOwnerGUID;
    LootType   loot_type{LOOT_NONE}; // required for achievement system

    // GUID of container that holds this loot (item_instance.entry), set for
    ...
    void clear()
    {
        for (QuestItemMap::const_iterator itr = PlayerQuestItems.begin();
             itr != PlayerQuestItems.end();
             ++itr)
            delete itr->second;
        PlayerQuestItems.clear();

        for (QuestItemMap::const_iterator itr = PlayerFFAItems.begin();
             itr != PlayerFFAItems.end();
             ++itr)
            delete itr->second;
        PlayerFFAItems.clear();

        for (QuestItemMap::const_iterator itr =
                 PlayerNonQuestNonFFAConditionalItems.begin();
             itr != PlayerNonQuestNonFFAConditionalItems.end();
             ++itr)
            delete itr->second;
...
private:
    ...
    // All rolls are registered here. They need to know, when the loot is not
    // valid anymore
