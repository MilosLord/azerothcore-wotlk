...
struct StoredLootItem {
    StoredLootItem(uint32 i,
        ...
        : itemid(i), itemIndex(idx), count(c), randomPropertyId(ri),
          randomSuffix(rs), follow_loot_rules(follow_loot_rules),
          freeforall(freeforall), is_blocked(is_blocked),
          is_counted(is_counted), is_underthreshold(is_underthreshold),
          needs_quest(needs_quest), conditionLootId(conditionLootId)
    {
    }
...
public:
    ...
    void RemoveEntryFromDB(ObjectGuid containerGUID,
                           uint32     itemid,
                           uint32     count,
                           uint32     itemIndex);
    ...
    void RemoveStoredLootItem(ObjectGuid containerGUID,
                              uint32     itemid,
                              uint32     count,
                              Loot*      loot,
                              uint32     itemIndex);
