...
#define MAIL_BODY_ITEM_TEMPLATE                                                \
    9311 // - plain letter, A Dusty Unsent Letter: 889
...
enum MailCheckMask : uint8 {
    ...
    MAIL_CHECK_MASK_RETURNED = 0x02, /// This mail was returned. Do not allow
                                     /// returning mail back again.
    MAIL_CHECK_MASK_COPIED = 0x04, /// This mail was copied. Do not allow making
                                   /// a copy of items in mail.
...
enum MailShowFlags {
    ...
    MAIL_SHOW_DELETE =
        0x0002, // forced show delete button instead return button
...
public: // Constructors
    MailSender(MailMessageType messageType,
        ...
        : m_messageType(messageType), m_senderId(sender_guidlow_or_entry),
          m_stationery(stationery)
    {
    }
    MailSender(Object*        sender,
               MailStationery stationery = MAIL_STATIONERY_DEFAULT);
...
public: // Accessors
    [[nodiscard]] MailMessageType GetMailMessageType() const
    {
...
class MailReceiver {
public: // Constructors
    explicit MailReceiver(ObjectGuid::LowType receiver_lowguid)
        : m_receiver(nullptr), m_receiver_lowguid(receiver_lowguid)
    {
    }
...
public: // Accessors
    ...
    [[nodiscard]] ObjectGuid::LowType GetPlayerGUIDLow() const
    {
...
public: // Constructors
    ...
    MailDraft(std::string subject, std::string body)
        : m_mailTemplateId(0), m_mailTemplateItemsNeed(false),
          m_subject(std::move(subject)), m_body(std::move(body)), m_money(0),
          m_COD(0)
    {
    }
...
public: // finishers
    void SendReturnToSender(uint32                       sender_acc,
                            ObjectGuid::LowType          sender_guid,
                            ObjectGuid::LowType          receiver_guid,
                            CharacterDatabaseTransaction trans);
    void SendMailTo(CharacterDatabaseTransaction trans,
...
private:
    void deleteIncludedItems(CharacterDatabaseTransaction trans,
                             bool                         inDB = false);
    void prepareItems(
        Player*                      receiver,
        CharacterDatabaseTransaction trans); // called from SendMailTo for
                                             // generate mailTemplateBase items
    ...
    MailItemMap m_items; // Keep the items in a map to avoid duplicate guids
                         // (which can happen), store only low part of guid
...
struct Mail {
    ...
    {
        for (MailItemInfoVec::iterator itr = items.begin(); itr != items.end();
             ++itr) {
    ...
    [[nodiscard]] bool IsSentByPlayer() const
    {
        return messageType == MAIL_NORMAL;
    }
    [[nodiscard]] bool IsSentByGM() const
    {
        return stationery == MAIL_STATIONERY_GM;
    }
    [[nodiscard]] bool IsCODPayment() const
    {
        return checked & MAIL_CHECK_MASK_COD_PAYMENT;
    }
    [[nodiscard]] bool IsReturnedMail() const
    {
        return checked & MAIL_CHECK_MASK_RETURNED;
    }
};
