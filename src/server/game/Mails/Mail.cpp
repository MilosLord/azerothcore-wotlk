    ...
    : m_stationery(stationery)
{
    ...
    default:
        ...
        LOG_ERROR(
            "mail",
            "MailSender::MailSender - Mail have unexpected sender typeid ({})",
            sender->GetTypeId());
        break;
    }
}
...
MailSender::MailSender(CalendarEvent* sender)
    : m_messageType(MAIL_CALENDAR), m_senderId(sender->GetEventId()),
      m_stationery(
          MAIL_STATIONERY_DEFAULT) // what stationery we should use here?
{
}

MailSender::MailSender(AuctionEntry* sender)
    : m_messageType(MAIL_AUCTION), m_senderId(sender->GetHouseId()),
...
MailSender::MailSender(Player* sender)
{
    m_messageType = MAIL_NORMAL;
    m_stationery =
        sender->IsGameMaster() ? MAIL_STATIONERY_GM : MAIL_STATIONERY_DEFAULT;
    m_senderId = sender->GetGUID().GetCounter();
}
...
MailReceiver::MailReceiver(Player* receiver)
    : m_receiver(receiver), m_receiver_lowguid(receiver->GetGUID().GetCounter())
{
}

MailReceiver::MailReceiver(Player*             receiver,
                           ObjectGuid::LowType receiver_lowguid)
    : m_receiver(receiver), m_receiver_lowguid(receiver_lowguid)
{
                             ...
                             CharacterDatabaseTransaction trans)
{
    ...
    // The mail sent after turning in the quest The Wrath of Neptulon contains
    // 100g Only quest in the game up to BFA which sends raw gold through mail.
    // So would just be overkill to introduce a new column in the database.
    ...
    // can be empty
    mailLoot.FillLoot(
        m_mailTemplateId, LootTemplates_Mail, receiver, true, true);

    uint32 max_slot = mailLoot.GetMaxSlotInLootFor(receiver);
    for (uint32 i = 0; m_items.size() < MAX_MAIL_ITEMS && i < max_slot; ++i) {
        if (LootItem* lootitem = mailLoot.LootItemInSlot(i, receiver)) {
            if (Item* item = Item::CreateItem(
                    lootitem->itemid, lootitem->count, receiver)) {
                item->SaveToDB(
                    trans); // save for prevent lost at next mail load, if send
                                   ...
                                   CharacterDatabaseTransaction trans)
{
    ...
    if (!receiver && !sCharacterCache->GetCharacterAccountIdByGuid(
                         ObjectGuid(HighGuid::Player, receiver_guid))) {
    ...
    if (!m_items.empty()) {
        ...
        for (MailItemMap::iterator mailItemIter = m_items.begin();
            ...
            Item* item = mailItemIter->second;
            item->SaveToDB(
                trans); // item not in inventory and can be save standalone
...
void MailDraft::SendMailTo(CharacterDatabaseTransaction trans,
                           ...
                           bool                         sendMail)
{
    sScriptMgr->OnBeforeMailDraftSendMailTo(this,
                                            receiver,
                                            sender,
                                            checked,
                                            deliver_delay,
    ...
    // mail from battlemaster (rewardmarks) should last only one day
             ...
             sBattlegroundMgr->GetBattleMasterBG(sender.GetSenderId()) !=
                 BATTLEGROUND_TYPE_NONE)
        expire_delay = DAY;
    // default case: expire time if COD 3 days, if no COD 30 days (or 90 days if
    // sender is a game master)
    else if (m_COD)
        expire_delay = 3 * DAY;
        ...
        Player* pSender =
            ObjectAccessor::FindPlayerByLowGUID(sender.GetSenderId());
        expire_delay = pSender && pSender->GetSession()->GetSecurity()
                           ? 90 * DAY
                           : 30 * DAY;
    }
    ...
    uint8                               index = 0;
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_MAIL);
    ...
    for (MailItemMap::const_iterator mailItemIter = m_items.begin();
         mailItemIter != m_items.end();
         ++mailItemIter) {
    ...
    sCharacterCache->IncreaseCharacterMailCount(
        ObjectGuid(HighGuid::Player, receiver.GetPlayerGUIDLow()));
