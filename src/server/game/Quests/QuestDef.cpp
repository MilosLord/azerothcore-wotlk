...
void Quest::LoadQuestTemplateAddon(Field* fields)
{
    ...
    if ((SpecialFlags & QUEST_SPECIAL_FLAGS_AUTO_ACCEPT) &&
        !sWorld->getBoolConfig(CONFIG_QUEST_IGNORE_AUTO_ACCEPT)) {
        Flags |= QUEST_FLAGS_AUTO_ACCEPT;
    }
}
...
int32 Quest::GetRewOrReqMoney(uint8 playerLevel) const
{
    ...
    if (playerLevel && RewardMoneyDifficulty) {
        if (uint32 questRewardedMoney = sObjectMgr->GetQuestMoneyReward(
                playerLevel, RewardMoneyDifficulty)) {
            rewardedMoney = questRewardedMoney;
        }
    }

    return static_cast<int32>(rewardedMoney *
                              sWorld->getRate(RATE_REWARD_QUEST_MONEY));
}
...
uint32 Quest::GetRewMoneyMaxLevel() const
{
    ...
    rewMoney =
        (XPValue(sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) * (6 * COPPER));
    // https://wowpedia.fandom.com/wiki/Quest?oldid=1035002 Formula is XP gained
    // * 6c
    return static_cast<int32>(rewMoney *
                              sWorld->getRate(RATE_REWARD_BONUS_MONEY));
}

bool Quest::IsAutoAccept() const { return HasFlag(QUEST_FLAGS_AUTO_ACCEPT); }

bool Quest::IsAutoComplete() const { return HasFlag(QUEST_FLAGS_AUTOCOMPLETE); }
...
uint32 Quest::CalculateHonorGain(uint8 level) const
{
    ...
    if (GetRewHonorAddition() > 0 || GetRewHonorMultiplier() > 0.0f) {
        ...
        TeamContributionPointsEntry const* tc =
            sTeamContributionPointsStore.LookupEntry(level);
        if (!tc)
            return 0;
        honor =
            uint32(tc->value * GetRewHonorMultiplier() * 0.1000000014901161);

        // Xinef: exactly this is calculated above, however with higher
        // precision...
        // honor += Acore::Honor::hk_honor_at_level(level,
...
void Quest::InitializeQueryData()
{
    ...
    queryData << uint32(
        GetQuestMethod()); // Accepted values: 0, 1 or 2. 0 == IsAutoComplete()
                           // (skip objectives/details)
    queryData << uint32(
        GetQuestLevel()); // may be -1, static data, in other cases must be used
                          // dynamic level: Player::GetQuestLevel (0 is not
                          // known, but assuming this is no longer valid for
                          // quest intended for client)
    ...
    queryData << uint32(GetRepObjectiveFaction()); // shown in quest log as part
                                                   // of quest objective
    queryData << uint32(GetRepObjectiveValue());   // shown in quest log as part
                                                   // of quest objective

    queryData << uint32(
        GetRepObjectiveFaction2()); // shown in quest log as part of quest
                                    // objective OPPOSITE faction
    queryData << uint32(
        GetRepObjectiveValue2()); // shown in quest log as part of quest
                                  // objective OPPOSITE faction

    queryData << uint32(GetNextQuestInChain()); // client will request this
    ...
    queryData << uint32(GetRewSpell()); // reward spell, this spell will display
                                        // (icon) (casted if RewSpellCast == 0)
    ...
    queryData << uint32(
        GetCharTitleId()); // CharTitleId, new 2.4.0, player gets this title (id
                           // from CharTitles)
    ...
    queryData << GetCompletedText(); // display in quest objectives window once
    ...
    for (uint32 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i) {
        if (RequiredNpcOrGo[i] < 0)
            queryData << uint32(
                (RequiredNpcOrGo[i] * (-1)) |
                0x80000000); // client expects gameobject template id in form
                             // (id|0x80000000)
        else
