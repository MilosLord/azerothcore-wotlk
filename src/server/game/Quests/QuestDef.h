...
enum QuestFailedReason : uint32 {
    ...
    INVALIDREASON_QUEST_FAILED_LOW_LEVEL =
        1, // DESCRIPTION You are not high enough level for that quest.
    INVALIDREASON_QUEST_FAILED_WRONG_RACE =
        6, // DESCRIPTION That quest is not available to your race.
    INVALIDREASON_QUEST_ALREADY_DONE =
        7, // DESCRIPTION You have completed that quest.
    INVALIDREASON_QUEST_ONLY_ONE_TIMED =
        12, // DESCRIPTION You can only be on one timed quest at a time.
    INVALIDREASON_QUEST_ALREADY_ON =
        13, // DESCRIPTION You are already on that quest.
    INVALIDREASON_QUEST_FAILED_EXPANSION =
        16, // DESCRIPTION This quest requires an expansion enabled account.
    INVALIDREASON_QUEST_ALREADY_ON2 =
        18, // DESCRIPTION You are already on that quest.
    INVALIDREASON_QUEST_FAILED_MISSING_ITEMS =
        21, // DESCRIPTION You don't have the required items with you. Check
            // storage.
    INVALIDREASON_QUEST_FAILED_NOT_ENOUGH_MONEY =
        23, // DESCRIPTION You don't have enough money for that quest.
    INVALIDREASON_DAILY_QUESTS_REMAINING =
        26, // DESCRIPTION You have already completed 25 daily quests today.
    INVALIDREASON_QUEST_FAILED_CAIS =
        27, // DESCRIPTION You cannot complete quests once you have reached
            // tired time.
    INVALIDREASON_DAILY_QUEST_COMPLETED_TODAY =
        29 // DESCRIPTION You have completed that daily quest today.
};
...
enum QuestGiverStatus : uint32 {
    ...
    // Custom value meaning that script call did not return any valid quest
    // status
...
enum QuestFlags {
    ...
    QUEST_FLAGS_PARTY_ACCEPT =
        0x00000002, // Not used currently. If player in party, all players that
                    // can accept this quest will receive confirmation box to
                    // accept quest
                    // CMSG_QUEST_CONFIRM_ACCEPT/SMSG_QUEST_CONFIRM_ACCEPT
    QUEST_FLAGS_EXPLORATION = 0x00000004, // Not used currently
    QUEST_FLAGS_SHARABLE = 0x00000008, // Can be shared: Player::CanShareQuest()
    QUEST_FLAGS_HAS_CONDITION = 0x00000010, // Not used currently
    QUEST_FLAGS_HIDE_REWARD_POI =
        0x00000020,                // Not used currently: Unsure of content
    QUEST_FLAGS_RAID = 0x00000040, // Not used currently
    QUEST_FLAGS_TBC  = 0x00000080, // Not used currently: Available if TBC
                                   // expansion enabled only
    QUEST_FLAGS_NO_MONEY_FROM_XP =
        0x00000100, // Not used currently: Experience is not converted to gold
                    // at max level
    QUEST_FLAGS_HIDDEN_REWARDS =
        0x00000200, // Items and money rewarded only sent in
                    // SMSG_QUESTGIVER_OFFER_REWARD (not in
                    // SMSG_QUESTGIVER_QUEST_DETAILS or in client quest
                    // log(SMSG_QUEST_QUERY_RESPONSE))
    QUEST_FLAGS_TRACKING =
        0x00000400, // These quests are automatically rewarded on quest complete
                    // and they will never appear in quest log client side.
    QUEST_FLAGS_DEPRECATE_REPUTATION = 0x00000800, // Not used currently
    QUEST_FLAGS_DAILY = 0x00001000, // Used to know quest is Daily one
    QUEST_FLAGS_FLAGS_PVP =
        0x00002000, // Having this quest in log forces PvP flag
    QUEST_FLAGS_UNAVAILABLE =
        0x00004000, // Used on quests that are not generically available
    QUEST_FLAGS_WEEKLY       = 0x00008000,
    QUEST_FLAGS_AUTOCOMPLETE = 0x00010000, // auto complete
    QUEST_FLAGS_DISPLAY_ITEM_IN_TRACKER =
        0x00020000,                    // Displays usable item in quest tracker
    QUEST_FLAGS_OBJ_TEXT = 0x00040000, // use Objective text as Complete text
    QUEST_FLAGS_AUTO_ACCEPT =
        0x00080000, // The client recognizes this flag as auto-accept. However,
                    // NONE of the current quests (3.3.5a) have this flag. Maybe
                    // blizz used to use it, or will use it in the future.
...
enum QuestSpecialFlags {
    ...
    // Trinity flags for set SpecialFlags in DB if required but used only at
    // server
    QUEST_SPECIAL_FLAGS_REPEATABLE = 0x001, // Set by 1 in SpecialFlags from DB
    QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT =
        0x002, // Set by 2 in SpecialFlags from DB (if required area explore,
               // spell SPELL_EFFECT_QUEST_COMPLETE casting, table
               // `FECT_QUEST_COMPLETE casting, table `*_script` command
               // SCRIPT_COMMAND_QUEST_EXPLORED use, set from script)
    QUEST_SPECIAL_FLAGS_AUTO_ACCEPT =
        0x004, // Set by 4 in SpecialFlags in DB if the quest is to be
               // auto-accepted.
    QUEST_SPECIAL_FLAGS_DF_QUEST =
        0x008, // Set by 8 in SpecialFlags in DB if the quest is used by Dungeon
               // Finder.
    QUEST_SPECIAL_FLAGS_MONTHLY =
        0x010, // Set by 16 in SpecialFlags in DB if the quest is reset at the
               // begining of the month
    QUEST_SPECIAL_FLAGS_CAST =
        0x020, // Set by 32 in SpecialFlags in DB if the quest requires
               // RequiredOrNpcGo killcredit but NOT kill (a spell cast)
    QUEST_SPECIAL_FLAGS_NO_REP_SPILLOVER =
        0x040, // Set by 64 in SpecialFlags in DB if the quest does not share
               // rewarded reputation with other allied factions
...
// This Quest class provides a convenient way to access a few pretotaled
// (cached) quest details, all base quest information, and any utility functions
// such as generating the amount of xp to give
class Quest {
...
public:
    ...
    [[nodiscard]] bool HasFlag(uint32 flag) const
    {
        return (Flags & flag) != 0;
    }
    void SetFlag(uint32 flag) { Flags |= flag; }

    [[nodiscard]] bool HasSpecialFlag(uint32 flag) const
    {
    ...
    [[nodiscard]] uint32 GetRequiredSkillValue() const
    {
        return RequiredSkillPoints;
    }
    [[nodiscard]] uint32 GetRepObjectiveFaction() const
    {
        return RequiredFactionId1;
    }
    [[nodiscard]] int32 GetRepObjectiveValue() const
    {
        return RequiredFactionValue1;
    }
    [[nodiscard]] uint32 GetRepObjectiveFaction2() const
    {
        return RequiredFactionId2;
    }
    [[nodiscard]] int32 GetRepObjectiveValue2() const
    {
        return RequiredFactionValue2;
    }
    [[nodiscard]] uint32 GetRequiredMinRepFaction() const
    {
        return RequiredMinRepFaction;
    }
    [[nodiscard]] int32 GetRequiredMinRepValue() const
    {
        return RequiredMinRepValue;
    }
    [[nodiscard]] uint32 GetRequiredMaxRepFaction() const
    {
        return RequiredMaxRepFaction;
    }
    [[nodiscard]] int32 GetRequiredMaxRepValue() const
    {
        return RequiredMaxRepValue;
    }
    [[nodiscard]] uint32 GetSuggestedPlayers() const
    {
    ...
    [[nodiscard]] std::string const& GetObjectives() const
    {
        return Objectives;
    }
    [[nodiscard]] std::string const& GetOfferRewardText() const
    {
        return OfferRewardText;
    }
    [[nodiscard]] std::string const& GetRequestItemsText() const
    {
        return RequestItemsText;
    }
    [[nodiscard]] std::string const& GetAreaDescription() const
    {
        return AreaDescription;
    }
    [[nodiscard]] std::string const& GetCompletedText() const
    {
    ...
    [[nodiscard]] float  GetRewHonorMultiplier() const
    {
        return RewardKillHonor;
    }
    [[nodiscard]] uint32
    GetRewMoneyMaxLevel() const; // use in XP calculation at client
    [[nodiscard]] uint32 GetRewSpell() const { return RewardDisplaySpell; }
    [[nodiscard]] int32  GetRewSpellCast() const { return RewardSpell; }
    [[nodiscard]] uint32 GetRewMailTemplateId() const
    {
        return RewardMailTemplateId;
    }
    [[nodiscard]] uint32 GetRewMailDelaySecs() const { return RewardMailDelay; }
    [[nodiscard]] uint32 GetRewMailSenderEntry() const
    {
    ...
    [[nodiscard]] uint32 GetIncompleteEmote() const
    {
        return EmoteOnIncomplete;
    }
    [[nodiscard]] uint32 GetCompleteEmote() const { return EmoteOnComplete; }
    [[nodiscard]] bool   IsRepeatable() const
    {
        return SpecialFlags & QUEST_SPECIAL_FLAGS_REPEATABLE;
    }
    ...
    [[nodiscard]] bool   IsMonthly() const
    {
        return SpecialFlags & QUEST_SPECIAL_FLAGS_MONTHLY;
    }
    [[nodiscard]] bool IsSeasonal() const
    {
        return (ZoneOrSort == -QUEST_SORT_SEASONAL ||
                ZoneOrSort == -QUEST_SORT_SPECIAL ||
                ZoneOrSort == -QUEST_SORT_LUNAR_FESTIVAL ||
                ZoneOrSort == -QUEST_SORT_MIDSUMMER ||
                ZoneOrSort == -QUEST_SORT_BREWFEST ||
                ZoneOrSort == -QUEST_SORT_LOVE_IS_IN_THE_AIR ||
                ZoneOrSort == -QUEST_SORT_NOBLEGARDEN) &&
               !IsRepeatable();
    }
    [[nodiscard]] bool IsDailyOrWeekly() const
    {
        return Flags & (QUEST_FLAGS_DAILY | QUEST_FLAGS_WEEKLY);
    }
    [[nodiscard]] bool IsRaidQuest(Difficulty difficulty) const;
    [[nodiscard]] bool IsAllowedInRaid(Difficulty difficulty) const;
    [[nodiscard]] bool IsDFQuest() const
    {
        return SpecialFlags & QUEST_SPECIAL_FLAGS_DF_QUEST;
    }
    ...
    [[nodiscard]] uint32 GetReqCreatureOrGOcount() const
    {
        return _reqCreatureOrGOcount;
    }
    [[nodiscard]] uint32 GetRewChoiceItemsCount() const
    {
        return _rewChoiceItemsCount;
    }
