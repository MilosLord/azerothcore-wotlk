...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    if (type >= MAX_CHAT_MSG_TYPE) {
        LOG_ERROR(
            "network.opcode", "CHAT: Wrong message type received: {}", type);
        recvData.rfinish();
        return;
    }

    if (lang == LANG_UNIVERSAL && type != CHAT_MSG_AFK &&
        type != CHAT_MSG_DND) {
        LOG_ERROR("entities.player.cheat",
                  "CMSG_MESSAGECHAT: Possible hacking-attempt: {} tried to "
                  "send a message in universal language",
                  GetPlayerInfo());
    ...
    if (langDesc->skill_id != 0 && !sender->HasSkill(langDesc->skill_id)) {
        ...
        for (auto const& auraEff :
             sender->GetAuraEffectsByType(SPELL_AURA_COMPREHEND_LANGUAGE)) {
    ...
    // pussywizard: chatting on most chat types requires 2 hours played to
    // prevent spam/abuse
    if (AccountMgr::IsPlayerAccount(GetSecurity())) {
        ...
        default: {
            if (sWorld->getBoolConfig(CONFIG_CHAT_MUTE_FIRST_LOGIN)) {
                uint32 minutes =
                    sWorld->getIntConfig(CONFIG_CHAT_TIME_MUTE_FIRST_LOGIN);
    ...
    if (sender->HasAura(1852) && type != CHAT_MSG_WHISPER) {
        SendNotification(GetAcoreString(LANG_GM_SILENCE),
    ...
    if (lang == LANG_ADDON) {
        ...
        default:
            LOG_ERROR("network",
                      "Player {} ({}) sent a chatmessage with an invalid "
                      "language/message type combination",
                      GetPlayer()->GetName(),
        ...
        if (sender->IsGameMaster())
            ...
            // Overwritten by SPELL_AURA_MOD_LANGUAGE auras (Affects only Say
            // and Yell)
            Unit::AuraEffectList const& ModLangAuras =
                sender->GetAuraEffectsByType(SPELL_AURA_MOD_LANGUAGE);
        ...
        if (type != CHAT_MSG_AFK && type != CHAT_MSG_DND)
            sender->UpdateSpeakTime(lang == LANG_ADDON
                                        ? Player::ChatFloodThrottle::ADDON
                                        : Player::ChatFloodThrottle::REGULAR);
    }
    ...
    // Our Warden module also uses SendAddonMessage as a way to communicate Lua
    // check results to the server, see if this is that
    if (type == CHAT_MSG_GUILD && lang == LANG_ADDON && _warden &&
        _warden->ProcessLuaCheckResponse(msg)) {
        return;
    }

    // pussywizard:
    if (msg.length() > 255 ||
        (lang != LANG_ADDON && msg.find("|0") != std::string::npos))
        return;
    ...
    if (!ignoreChecks) {
        ...
        if (!_player->CanSpeak()) {
            std::string timeStr =
                secsToTimeString(m_muteTime - GameTime::GetGameTime().count());
            SendNotification(GetAcoreString(LANG_WAIT_BEFORE_SPEAKING),
                             timeStr.c_str());
            return;
        }
    }
    ...
    if (lang != LANG_ADDON) {
        ...
        for (uint8 c : msg) {
            if (isNasty(c)) {
                LOG_ERROR("network",
                          "Player {} {} sent a message containing invalid "
                          "character {} - blocked",
                          GetPlayer()->GetName(),
                          GetPlayer()->GetGUID().ToString(),
                          uint8(c));
        ...
        if (sWorld->getBoolConfig(CONFIG_CHAT_FAKE_MESSAGE_PREVENTING)) {
            auto end =
                std::unique(msg.begin(), msg.end(), [](char c1, char c2) {
                    return (c1 == ' ') && (c2 == ' ');
                });
    ...
    case CHAT_MSG_YELL: {
        ...
        if (sender->GetLevel() <
            ...
            SendNotification(GetAcoreString(LANG_SAY_REQ),
                             sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
            return;
        }
    ...
    case CHAT_MSG_WHISPER: {
        ...
        bool    receiverIsPlayer = AccountMgr::IsPlayerAccount(
            receiver ? receiver->GetSession()->GetSecurity() : SEC_PLAYER);

        if (sender->GetLevel() <
                sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ) &&
            receiver != sender) {
            SendNotification(
                GetAcoreString(LANG_WHISPER_REQ),
                sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
            return;
        }

        if (!receiver || (senderIsPlayer && !receiverIsPlayer &&
                          !receiver->isAcceptWhispers() &&
        ...
        if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHAT) &&
            senderIsPlayer && receiverIsPlayer)
        ...
        // If player is a Gamemaster and doesn't accept whisper, we
        // auto-whitelist every player that the Gamemaster is talking to
        if (!senderIsPlayer && !sender->isAcceptWhispers() &&
    ...
    case CHAT_MSG_PARTY_LEADER: {
        ...
        if (type == CHAT_MSG_PARTY_LEADER &&
            !group->IsLeader(sender->GetGUID()))
            return;

        if (!sScriptMgr->CanPlayerUseChat(
                GetPlayer(), type, lang, msg, group)) {
            return;
        }
        ...
        ChatHandler::BuildChatPacket(
            data, ChatMsg(type), Language(lang), sender, nullptr, msg);
        group->BroadcastPacket(
            &data, false, group->GetMemberGroup(GetPlayer()->GetGUID()));
    } break;
    case CHAT_MSG_GUILD: {
        if (GetPlayer()->GetGuildId()) {
            if (Guild* guild =
                ...
                if (!sScriptMgr->CanPlayerUseChat(
                        GetPlayer(), type, lang, msg, guild)) {
                    return;
                }

                sScriptMgr->OnPlayerChat(GetPlayer(), type, lang, msg, guild);

                guild->BroadcastToGuild(this,
                                        false,
                                        msg,
                                        lang == LANG_ADDON ? LANG_ADDON
                                                           : LANG_UNIVERSAL);
            }
        }
    } break;
    ...
    case CHAT_MSG_RAID: {
        ...
        WorldPacket data;
        ChatHandler::BuildChatPacket(
            data, CHAT_MSG_RAID, Language(lang), sender, nullptr, msg);
    ...
    case CHAT_MSG_RAID_LEADER: {
        ...
        WorldPacket data;
        ChatHandler::BuildChatPacket(
            data, CHAT_MSG_RAID_LEADER, Language(lang), sender, nullptr, msg);
        group->BroadcastPacket(&data, false);
    } break;
    case CHAT_MSG_RAID_WARNING: {
        ...
        WorldPacket data;
        ChatHandler::BuildChatPacket(
            data, CHAT_MSG_RAID_WARNING, Language(lang), sender, nullptr, msg);
        group->BroadcastPacket(&data, false);
    } break;
    case CHAT_MSG_BATTLEGROUND: {
        ...
        WorldPacket data;
        ChatHandler::BuildChatPacket(
            data, CHAT_MSG_BATTLEGROUND, Language(lang), sender, nullptr, msg);
        group->BroadcastPacket(&data, false);
    } break;
    case CHAT_MSG_BATTLEGROUND_LEADER: {
        // battleground raid is always in Player->GetGroup(), never in
    ...
    case CHAT_MSG_CHANNEL: {
        if (AccountMgr::IsPlayerAccount(GetSecurity())) {
            if (sender->GetLevel() <
                ...
                SendNotification(
                    GetAcoreString(LANG_CHANNEL_REQ),
                    sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                return;
            }
        }
    ...
    case CHAT_MSG_AFK: {
        if (!sender->IsInCombat()) {
            ...
            {
                sender->autoReplyMsg =
                    msg.empty() ? GetAcoreString(LANG_PLAYER_AFK_DEFAULT) : msg;
    ...
    case CHAT_MSG_DND: {
        ...
        {
            sender->autoReplyMsg =
                msg.empty() ? GetAcoreString(LANG_PLAYER_DND_DEFAULT) : msg;
    ...
    default:
        LOG_ERROR("network.opcode",
                  "CHAT: unknown message type {}, lang: {}",
                  type,
                  lang);
        break;
    }
...
public:
    EmoteChatBuilder(Player const& player,
                     uint32        text_emote,
                     uint32        emote_num,
                     Unit const*   target)
        : i_player(player), i_text_emote(text_emote), i_emote_num(emote_num),
          i_target(target)
    {
    }
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
    ...
    Acore::EmoteChatBuilder emote_builder(
        *GetPlayer(), text_emote, emoteNum, unit);
    Acore::LocalizedPacketDo<Acore::EmoteChatBuilder> emote_do(emote_builder);
    Acore::PlayerDistWorker<Acore::LocalizedPacketDo<Acore::EmoteChatBuilder>>
        emote_worker(GetPlayer(),
                     sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE),
                     emote_do);
    TypeContainerVisitor<Acore::PlayerDistWorker<
                             Acore::LocalizedPacketDo<Acore::EmoteChatBuilder>>,
    ...
    GetPlayer()->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_DO_EMOTE, text_emote, 0, unit);
