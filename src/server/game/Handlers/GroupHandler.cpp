                                   ...
                                   uint32             val /* = 0 */)
{
    ...
    data << uint32(res);
    data << uint32(
        val); // LFD cooldown related (used with ERR_PARTY_LFG_BOOT_COOLDOWN_S
              // and ERR_PARTY_LFG_BOOT_NOT_ELIGIBLE_S)
...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recvData)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_GM_GROUP) &&
        !invitingPlayer->IsGameMaster() && invitedPlayer->IsGameMaster()) {
        SendPartyResult(PARTY_OP_INVITE, membername, ERR_BAD_PLAYER_NAME_S);
        return;
    }
    ...
    if (invitingPlayer->GetInstanceId() != 0 &&
        invitedPlayer->GetInstanceId() != 0 &&
        invitingPlayer->GetInstanceId() != invitedPlayer->GetInstanceId() &&
        invitingPlayer->GetMapId() == invitedPlayer->GetMapId()) {
        SendPartyResult(
            PARTY_OP_INVITE, membername, ERR_TARGET_NOT_IN_INSTANCE_S);
        return;
    }
    // just ignore us
    if (invitedPlayer->GetInstanceId() != 0 &&
        invitedPlayer->GetDungeonDifficulty() !=
            invitingPlayer->GetDungeonDifficulty()) {
    ...
    if (!invitedPlayer->GetSocial()->HasFriend(invitingPlayer->GetGUID()) &&
        invitingPlayer->GetLevel() <
            sWorld->getIntConfig(CONFIG_PARTY_LEVEL_REQ)) {
        SendPartyResult(
            PARTY_OP_INVITE, invitedPlayer->GetName(), ERR_INVITE_RESTRICTED);
        return;
    }
    ...
    if (group2 || invitedPlayer->GetGroupInvite()) {
        ...
        if (group2) {
            // tell the player that they were invited but it failed as they were
            // already in a group
    ...
    if (group) {
        ...
        if (!group->IsLeader(invitingPlayer->GetGUID()) &&
            !group->IsAssistant(invitingPlayer->GetGUID())) {
    ...
    if (!group && invitingPlayer->GetGroupInvite() &&
        invitingPlayer->GetGroupInvite()->GetLeaderGUID() ==
            invitingPlayer->GetGUID())
        group = invitingPlayer->GetGroupInvite();
...
void WorldSession::HandleGroupAcceptOpcode(WorldPacket& recvData)
{
    ...
    if (group->GetLeaderGUID() == GetPlayer()->GetGUID()) {
        LOG_ERROR("network.opcode",
                  "HandleGroupAcceptOpcode: player {} ({}) tried to accept an "
                  "invite to his own group",
                  GetPlayer()->GetName(),
    ...
    if (!group->IsCreated()) {
        // This can happen if the leader is zoning. To be removed once delayed
        // actions for zoning are implemented
        if (!leader || leader->IsSpectator()) {
            group->RemoveAllInvites();
            return;
        }

        // If we're about to create a group there really should be a leader
        // present
        ASSERT(leader);
...
void WorldSession::HandleGroupDeclineOpcode(WorldPacket& /*recvData*/)
{
    ...
    // Remember leader if online (group pointer will be invalid if group gets
    // disbanded)
    Player* leader =
        ObjectAccessor::FindConnectedPlayer(group->GetLeaderGUID());
...
void WorldSession::HandleGroupUninviteGuidOpcode(WorldPacket& recvData)
{
    ...
    if (res != ERR_PARTY_RESULT_OK) {
        if (res == ERR_PARTY_LFG_BOOT_NOT_ELIGIBLE_S) {
            if (Player* kickTarget =
                    ObjectAccessor::FindConnectedPlayer(guid)) {
                if (Aura* dungeonCooldownAura =
                        kickTarget->GetAura(lfg::LFG_SPELL_DUNGEON_COOLDOWN)) {
                    ...
                    if (static_cast<int32>(sWorld->getIntConfig(
                        ...
                        SendPartyResult(PARTY_OP_UNINVITE,
                                        name,
                                        res,
                                        (sWorld->getIntConfig(
    ...
    // Xinef: do not allow to kick with empty reason, this will resend packet
    // with given reason
...
void WorldSession::HandleGroupUninviteOpcode(WorldPacket& recvData)
{
    ...
    if (GetPlayer()->GetName() == membername) {
        LOG_ERROR("network.opcode",
                  "WorldSession::HandleGroupUninviteOpcode: leader {} ({}) "
                  "tried to uninvite himself from the group.",
                  GetPlayer()->GetName(),
                  GetPlayer()->GetGUID().ToString());
        return;
    ...
    PartyResult res =
        GetPlayer()->CanUninviteFromGroup(grp->GetMemberGUID(membername));
    if (res != ERR_PARTY_RESULT_OK) {
    ...
    if (ObjectGuid guid = grp->GetMemberGUID(membername)) {
        Player::RemoveFromGroup(
            grp, guid, GROUP_REMOVEMETHOD_KICK, GetPlayer()->GetGUID());
        return;
    }
...
void WorldSession::HandleGroupSetLeaderOpcode(WorldPacket& recvData)
{
    ...
    if (!group->IsLeader(GetPlayer()->GetGUID()) ||
        player->GetGroup() != group || guid == GetPlayer()->GetGUID())
        return;
...
void WorldSession::HandleGroupDisbandOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (grp) {
        SendPartyResult(
            PARTY_OP_LEAVE, GetPlayer()->GetName(), ERR_PARTY_RESULT_OK);
        GetPlayer()->RemoveFromGroup(GROUP_REMOVEMETHOD_LEAVE);
    }
    else if (grpInvite &&
             grpInvite->GetLeaderGUID() ==
                 GetPlayer()
                     ->GetGUID()) { // pending group creation being cancelled
        SendPartyResult(
            PARTY_OP_LEAVE, GetPlayer()->GetName(), ERR_PARTY_RESULT_OK);
        grpInvite->Disband();
    }
...
void WorldSession::HandleLootMethodOpcode(WorldPacket& recvData)
{
    ...
    if (lootThreshold < ITEM_QUALITY_UNCOMMON ||
        lootThreshold > ITEM_QUALITY_ARTIFACT)
        return;
...
void WorldSession::HandleLootRoll(WorldPacket& recvData)
{
    ...
    case ROLL_GREED:
        GetPlayer()->UpdateAchievementCriteria(
            ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED, 1);
        break;
    case ROLL_DISENCHANT:
        GetPlayer()->UpdateAchievementCriteria(
            ACHIEVEMENT_CRITERIA_TYPE_ROLL_DISENCHANT, 1);
        break;
    }
}
...
void WorldSession::HandleMinimapPingOpcode(WorldPacket& recvData)
{
    ...
    data << float(x);
    data << float(y);
    GetPlayer()->GetGroup()->BroadcastPacket(
        &data, true, -1, GetPlayer()->GetGUID());
}

void WorldSession::HandleRandomRollOpcode(
    WorldPackets::Misc::RandomRollClient& packet)
{
...
void WorldSession::HandleGroupRaidConvertOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (!group->IsLeader(GetPlayer()->GetGUID()) ||
        ...
        group->isLFGGroup()) // pussywizard: not allowed for lfg groups, it is
                             // either raid from the beginning or not!
        return;
    /********************/
...
{
    // we will get correct pointer for group here, so we don't have to check if
    // group is BG raid
    Group* group = GetPlayer()->GetGroup();
    if (!group)
        return;
...
void WorldSession::HandlePartyAssignmentOpcode(WorldPacket& recvData)
{
    ...
    case GROUP_ASSIGN_MAINTANK:
        group->RemoveUniqueGroupMemberFlag(
            MEMBER_FLAG_MAINTANK); // Remove main assist flag from current if
                                   // any.
                                                      ...
                                                      WorldPacket* data)
{
    ...
    if (mask & GROUP_UPDATE_FLAG_POWER_TYPE) // if update power type, update
                                             // current/max power also
    ...
    *data << uint32(mask);
    ...
    if (mask & GROUP_UPDATE_FLAG_VEHICLE_SEAT) {
        if (Vehicle* veh = player->GetVehicle())
            *data << uint32(
                veh->GetVehicleInfo()
                    ->m_seatID[player->m_movementInfo.transport.seat]);
        else
            *data << uint32(0);
    }
}
...
void WorldSession::HandleRequestPartyMemberStatsOpcode(WorldPacket& recvData)
{
    ...
    if (!player) {
        ...
        data << uint8(0); // only for SMSG_PARTY_MEMBER_STATS_FULL, probably
                          // arena/bg related
    ...
    WorldPacket data(SMSG_PARTY_MEMBER_STATS_FULL,
                     4 + 2 + 2 + 2 + 1 + 2 * 6 + 8 + 1 + 8);
    data << uint8(
        0); // only for SMSG_PARTY_MEMBER_STATS_FULL, probably arena/bg related
    data << player->GetPackGUID();

    uint32 updateFlags =
        GROUP_UPDATE_FLAG_STATUS | GROUP_UPDATE_FLAG_CUR_HP |
        GROUP_UPDATE_FLAG_MAX_HP | GROUP_UPDATE_FLAG_CUR_POWER |
        GROUP_UPDATE_FLAG_MAX_POWER | GROUP_UPDATE_FLAG_LEVEL |
        GROUP_UPDATE_FLAG_ZONE | GROUP_UPDATE_FLAG_POSITION |
        GROUP_UPDATE_FLAG_AURAS | GROUP_UPDATE_FLAG_PET_NAME |
        GROUP_UPDATE_FLAG_PET_MODEL_ID | GROUP_UPDATE_FLAG_PET_AURAS;
    ...
    if (pet)
        updateFlags |=
            GROUP_UPDATE_FLAG_PET_GUID | GROUP_UPDATE_FLAG_PET_CUR_HP |
    ...
    data << uint16(
        player->GetMaxPower(powerType));    // GROUP_UPDATE_FLAG_MAX_POWER
    ...
    data << uint16(pet ? pet->GetDisplayId()
                       : 0); // GROUP_UPDATE_FLAG_PET_MODEL_ID
...
void WorldSession::HandleGroupSwapSubGroupOpcode(WorldPacket& recv_data)
{
    ...
    auto getGuid = [&group](std::string const& playerName) {
        ...
        if (Player* player =
                ObjectAccessor::FindPlayerByName(playerName.c_str())) {
            return player->GetGUID();
        }
        else {
            if (ObjectGuid guid =
                    sCharacterCache->GetCharacterGuidByName(playerName)) {
                return guid;
            }
            else {
    ...
    if (groupId1 == MAX_RAID_SUBGROUPS + 1 ||
        groupId2 == MAX_RAID_SUBGROUPS + 1) {
        return;
    }
