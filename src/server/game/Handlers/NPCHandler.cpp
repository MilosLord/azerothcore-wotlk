...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleTabardVendorActivateOpcode - Unit ({}) not "
                  "found or you can not interact with him.",
                  guid.ToString());
        return;
    }
...
void WorldSession::SendTrainerList(ObjectGuid guid, const std::string& strTitle)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: SendTrainerList - Unit ({}) not found or you can not "
                  "interact with him.",
                  guid.ToString());
        return;
    }
    ...
    if (!ci) {
        LOG_DEBUG("network",
                  "WORLD: SendTrainerList - ({}) NO CREATUREINFO!",
                  guid.ToString());
        return;
    }
    ...
    if (!trainer_spells) {
        LOG_DEBUG("network",
                  "WORLD: SendTrainerList - Training spells not found for "
                  "creature ({})",
                  guid.ToString());
        return;
    }
    ...
    WorldPacket data(SMSG_TRAINER_LIST,
                     8 + 4 + 4 + trainer_spells->spellList.size() * 38 +
                         strTitle.size() + 1);
    data << guid;
    ...
    uint32 count = 0;
    for (TrainerSpellMap::const_iterator itr =
             trainer_spells->spellList.begin();
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            ...
            SpellInfo const* learnedSpellInfo =
                sSpellMgr->GetSpellInfo(tSpell->learnedSpell[i]);
            if (learnedSpellInfo &&
        ...
        data << uint32(
            tSpell->spell); // learned spell (or cast-spell in profession case)
        data << uint8(state == TRAINER_SPELL_GREEN_DISABLED
                          ? TRAINER_SPELL_GREEN
                          : state);
        data << uint32(floor(tSpell->spellCost * fDiscountMod));

        data << uint32(primary_prof_first_rank && can_learn_primary_prof ? 1
                                                                         : 0);
        // primary prof. learn confirmation dialog
        data << uint32(primary_prof_first_rank
                           ? 1
                           : 0); // must be equal prev. field to have learn
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            if (!tSpell->learnedSpell[i])
                continue;
            if (uint32 prevSpellId =
                    sSpellMgr->GetPrevSpellInChain(tSpell->learnedSpell[i])) {
            ...
            SpellsRequiringSpellMapBounds spellsRequired =
                sSpellMgr->GetSpellsRequiredForSpellBounds(
                    tSpell->learnedSpell[i]);
            for (SpellsRequiringSpellMap::const_iterator itr2 =
                     spellsRequired.first;
                 itr2 != spellsRequired.second && maxReq < 3;
                 ++itr2) {
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleTrainerBuySpellOpcode - Unit ({}) not found or "
                  "you can not interact with him.",
                  guid.ToString());
        return;
    }
    ...
    if (trainer_spell->reqSpell &&
        !_player->HasSpell(trainer_spell->reqSpell)) {
        return;
    }
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleGossipHelloOpcode - Unit ({}) not found or you "
                  "can not interact with him.",
                  guid.ToString());
        return;
    }
    ...
    // If spiritguide, no need for gossip menu, just put player into resurrect
    // queue
    if (unit->IsSpiritGuide()) {
        Battleground* bg = _player->GetBattleground();
        if (bg) {
            bg->AddPlayerToResurrectQueue(unit->GetGUID(), _player->GetGUID());
            sBattlegroundMgr->SendAreaSpiritHealerQueryOpcode(
                _player, bg, unit->GetGUID());
            return;
        }
    }
    ...
    if (!sScriptMgr->OnGossipHello(_player, unit)) {
        ...
        _player->PrepareGossipMenu(
            unit, unit->GetCreatureTemplate()->GossipMenuId, true);
...
UNIT_NPC_FLAG_NONE); if (!unit)
    {
        LOG_DEBUG("network.opcode", "WORLD: HandleGossipSelectOptionOpcode -
Unit ({}) not found or you can't interact with him.", guid.ToString()); return;
    }
    ...
    if (!code.empty())
    {
        if (!Script->GossipSelectWithCode(_player, unit,
_player->PlayerTalkClass->GossipOptionSender (option),
_player->PlayerTalkClass->GossipOptionAction(option), code.c_str()))
            unit->OnGossipSelect (_player, option);
    }
    else
    {
        if (!Script->OnGossipSelect (_player, unit,
_player->PlayerTalkClass->GossipOptionSender (option),
_player->PlayerTalkClass->GossipOptionAction (option))) unit->OnGossipSelect
(_player, option);
    }
}*/
...
void WorldSession::HandleSpiritHealerActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleSpiritHealerActivateOpcode - Unit ({}) not "
                  "found or you can not interact with him.",
                  guid.ToString());
        return;
    }
...
void WorldSession::SendSpiritResurrect()
{
    ...
    corpseGrave = sGraveyard->GetClosestGraveyard(
        _player, _player->GetTeamId(), _player->HasCorpse());
    ...
    // player ghost
    if (corpseGrave) {
        GraveyardStruct const* ghostGrave =
            sGraveyard->GetClosestGraveyard(_player, _player->GetTeamId());

        if (corpseGrave != ghostGrave)
            _player->TeleportTo(corpseGrave->Map,
                                corpseGrave->x,
                                corpseGrave->y,
                                corpseGrave->z,
        ...
        // else
        //    _player->UpdateObjectVisibility(); // xinef: not needed, called in
        //    ResurrectPlayer
    }
    // or update at original position
    // else
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleBinderActivateOpcode - Unit ({}) not found or "
                  "you can not interact with him.",
                  npcGUID.ToString());
        return;
    }
...
void WorldSession::HandleListStabledPetsOpcode(WorldPacket& recvData)
{
    ...
    // remove mounts this fix bug where getting pet from stable while mounted
    // deletes pet.
...
void WorldSession::SendStablePet(ObjectGuid guid)
{
    ...
    if (petStable->CurrentPet) {
        ...
        if (PetStable::PetInfo const* pet =
    ...
    for (Optional<PetStable::PetInfo> const& stabledSlot :
...
void WorldSession::HandleStablePet(WorldPacket& recvData)
{
    ...
    if ((pet && (!pet->IsAlive() || pet->getPetType() != HUNTER_PET)) ||
        (!pet && (petStable->UnslottedPets.size() != 1 ||
                  !petStable->UnslottedPets[0].Health ||
                  petStable->UnslottedPets[0].Type != HUNTER_PET))) {
        SendStableResult(STABLE_ERR_STABLE);
        return;
    }

    for (uint32 freeSlot = 0; freeSlot < petStable->MaxStabledPets;
         ++freeSlot) {
        if (!petStable->StabledPets[freeSlot]) {
            if (pet) {
                // stable summoned pet
                _player->RemovePet(
                    pet, PetSaveMode(PET_SAVE_FIRST_STABLE_SLOT + freeSlot));
                std::swap(petStable->StabledPets[freeSlot],
...
void WorldSession::HandleUnstablePet(WorldPacket& recvData)
{
    ...
    CreatureTemplate const* creatureInfo =
        sObjectMgr->GetCreatureTemplate((*stabledPet)->CreatureId);
    if (!creatureInfo ||
        !creatureInfo->IsTameable(_player->CanTameExoticPets())) {
    ...
    if (oldPet) {
        // try performing a swap, client sends this packet instead of swap when
        // starting from stabled slot
        ...
        _player->RemovePet(
            ...
            PetSaveMode(
                ...
                std::distance(petStable->StabledPets.begin(), stabledPet)));
    }
    else if (petStable->UnslottedPets.size() == 1) {
        if (petStable->CurrentPet || !petStable->UnslottedPets[0].Health ||
            petStable->UnslottedPets[0].Type != HUNTER_PET) {
            SendStableResult(STABLE_ERR_STABLE);
            return;
        }
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(
                CHAR_UPD_CHAR_PET_SLOT_BY_ID);
        stmt->SetData(0,
                      PetSaveMode(PET_SAVE_FIRST_STABLE_SLOT +
                                  std::distance(petStable->StabledPets.begin(),
                                                stabledPet)));
        stmt->SetData(1, _player->GetGUID().GetCounter());
        stmt->SetData(2, petStable->UnslottedPets[0].PetNumber);
        CharacterDatabase.Execute(stmt);

        // move unsummoned pet into CurrentPet slot so that it gets moved into
        // stable slot later
    ...
    if (!newPet->LoadPetFromDB(_player, 0, petnumber, false)) {
        ...
        // update current pet slot in db immediately to maintain slot
        // consistency, dismissed pet was already saved
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(
                CHAR_UPD_CHAR_PET_SLOT_BY_ID);
...
void WorldSession::HandleBuyStableSlot(WorldPacket& recvData)
{
    ...
    if (petStable.MaxStabledPets < MAX_PET_STABLES) {
        StableSlotPricesEntry const* SlotPrice =
            sStableSlotPricesStore.LookupEntry(petStable.MaxStabledPets + 1);
...
void WorldSession::HandleStableSwapPet(WorldPacket& recvData)
{
    ...
    auto stabledPet =
        std::find_if(petStable->StabledPets.begin(),
                     petStable->StabledPets.end(),
                     [petId](Optional<PetStable::PetInfo> const& pet) {
...
void WorldSession::HandleRepairItemOpcode(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleRepairItemOpcode - Unit ({}) not found or you "
                  "can not interact with him.",
                  npcGUID.ToString());
        return;
    }
    ...
    sScriptMgr->OnBeforePlayerDurabilityRepair(
        _player, npcGUID, itemGUID, discountMod, guildBank);

    if (itemGUID) {
        LOG_DEBUG("network",
                  "ITEM: Repair item, item {}, npc {}",
                  itemGUID.ToString(),
                  npcGUID.ToString());

        Item* item = _player->GetItemByGuid(itemGUID);
        if (item)
            _player->DurabilityRepair(
                item->GetPos(), true, discountMod, guildBank);
    }
    else {
        LOG_DEBUG(
            "network", "ITEM: Repair all items, npc {}", npcGUID.ToString());
