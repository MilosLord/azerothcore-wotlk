...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPacket& recvData)
{
    ...
    if (lguid.IsGameObject()) {
        ...
        // if (player->GetGroup() && player->GetGroup()->GetLootMethod() ==
        // MASTER_LOOT && player->GetGUID() !=
        // player->GetGroup()->GetMasterLooterGuid())
        //    go = nullptr;

        // not check distance for GO in case owned GO (fishing bobber case, for
        // example) or Fishing hole GO
        if (!go || ((go->GetOwnerGUID() != _player->GetGUID() &&
                     go->GetGoType() != GAMEOBJECT_TYPE_FISHINGHOLE) &&
                    !go->IsWithinDistInMap(_player))) {
        ...
        if (!lootAllowed ||
            !creature->IsWithinDistInMap(_player, INTERACTION_DISTANCE)) {
            player->SendLootError(lguid,
                                  lootAllowed ? LOOT_ERROR_TOO_FAR
    ...
    if (msg != EQUIP_ERR_OK && lguid.IsItem() &&
...
void WorldSession::HandleLootMoneyOpcode(WorldPacket& /*recvData*/)
{
    ...
    case HighGuid::GameObject: {
        ...
        // do not check distance for GO if player is the owner of it (ex.
        // fishing bobber)
        if (go && ((go->GetOwnerGUID() == player->GetGUID() ||
                    go->IsWithinDistInMap(player)))) {
            loot = &go->loot;
        }
    ...
    case HighGuid::Vehicle: {
        ...
        if (lootAllowed &&
            ...
            player->SendLootError(
                guid, lootAllowed ? LOOT_ERROR_TOO_FAR : LOOT_ERROR_DIDNT_KILL);
        break;
    }
    default:
    ...
    if (loot) {
        ...
        if (shareMoney &&
            player->GetGroup()) // item, pickpocket and players can be looted
                                // only single player
        {
            ...
            for (std::vector<Player*>::const_iterator i = playersNear.begin();
                 i != playersNear.end();
                 ++i) {
                (*i)->ModifyMoney(goldPerPlayer);
                (*i)->UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_LOOT_MONEY, goldPerPlayer);
                ...
                data << uint8(
                    playersNear.size() > 1
                        ? 0
                        : 1); // Controls the text displayed in chat. 0 is "Your
                              // share is..." and 1 is "You loot..."
            ...
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_LOOT_MONEY, loot->gold);
...
void WorldSession::HandleLootReleaseOpcode(WorldPacket& recvData)
{
    ...
    // re-loot use internal stored guid
    ObjectGuid guid;
    recvData >> guid;
...
void WorldSession::DoLootRelease(ObjectGuid lguid)
{
    ...
    if (lguid.IsGameObject()) {
        ...
        if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR) {
            ...
            if (go->GetGoType() == GAMEOBJECT_TYPE_FISHINGHOLE) {
                ...
                // Xinef: moved event execution to loot release (after
                // everything is looted) Xinef: 99% sure that this worked like
                // this on blizz Xinef: prevents exploits with just opening GO
                // and spawning bilions of npcs, which can crash core if you
                // know what you're doin ;)
                if (go->GetGoType() == GAMEOBJECT_TYPE_CHEST &&
                    go->GetGOInfo()->chest.eventId) {
                    LOG_DEBUG("spells.aura",
                              "Chest ScriptStart id {} for GO {}",
                              go->GetGOInfo()->chest.eventId,
                              go->GetSpawnId());
                    player->GetMap()->ScriptsStart(
                        sEventScripts,
                        go->GetGOInfo()->chest.eventId,
                        player,
                        go);
    ...
    else if (lguid.IsCorpse()) // ONLY remove insignia at BG
    {
        ...
        if (!corpse ||
            !corpse->IsWithinDistInMap(_player, INTERACTION_DISTANCE))
            return;
        ...
        {
            loot->clear();
            corpse->RemoveFlag(CORPSE_FIELD_DYNAMIC_FLAGS,
        ...
        if (proto->Flags &
            (ITEM_FLAG_IS_PROSPECTABLE | ITEM_FLAG_IS_MILLABLE)) {
            pItem->m_lootGenerated = false;
            pItem->loot.clear();

            uint32 count = pItem->GetCount();

            // >=5 checked in spell code, but will work for cheating cases also
            // with removing from another stacks.
            if (count > 5)
                count = 5;

            player->DestroyItemCount(pItem, count, true);
        }
        else if (pItem->loot.isLooted() ||
                 !(proto->Flags & ITEM_FLAG_HAS_LOOT)) {
        ...
        loot = &creature->loot;
        if (loot->isLooted()) {
            // skip pickpocketing loot for speed, skinning timer reduction is
            // no-op in fact
            if (!creature->IsAlive())
    ...
    // Player is not looking at loot list, he doesn't need to see updates on the
    // loot list
    if (!lguid.IsItem()) {
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
    ...
    if (!_player->GetGroup() ||
        _player->GetGroup()->GetMasterLooterGuid() != _player->GetGUID() ||
        _player->GetGroup()->GetLootMethod() != MASTER_LOOT) {
    ...
    LOG_DEBUG("network",
              ...
              "(CMSG_LOOT_MASTER_GIVE, 0x02A3) Target = [{}].",
              target->GetName());
    ...
    if (!_player->IsInRaidWith(target)) {
        ...
        // LOG_DEBUG("network", "MasterLootItem: Player {} tried to give an item
        // to ineligible player {} !", GetPlayer()->GetName(),
        // target->GetName());
        return;
    }
    ...
    if (slotid >= loot->items.size() + loot->quest_items.size()) {
        LOG_DEBUG("loot",
                  "MasterLootItem: Player {} might be using a hack! (slot {}, "
                  "size {})",
                  GetPlayer()->GetName(),
                  slotid,
                  (unsigned long)loot->items.size());
        return;
    }

    LootItem& item = slotid >= loot->items.size()
                         ? loot->quest_items[slotid - loot->items.size()]
                         : loot->items[slotid];

    ItemPosCountVec dest;
    InventoryResult msg = target->CanStoreNewItem(
    ...
    Item* newitem = target->StoreNewItem(
