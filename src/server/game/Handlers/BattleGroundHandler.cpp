...
void WorldSession::HandleBattlemasterHelloOpcode(WorldPacket& recvData)
{
    ...
    recvData >> guid;
    LOG_DEBUG("network",
              "WORLD: Recvd CMSG_BATTLEMASTER_HELLO Message from ({})",
              guid.ToString());
    ...
    BattlegroundTypeId bgTypeId =
        sBattlegroundMgr->GetBattleMasterBG(unit->GetEntry());
...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recvData)
{
    ...
    if (!sBattlemasterListStore.LookupEntry(bgTypeId_)) {
        LOG_ERROR("network",
                  "Battleground: invalid bgtype ({}) received. possible "
                  "cheater? player {}",
                  bgTypeId_,
    ...
    if (DisableMgr::IsDisabledFor(
            DISABLE_TYPE_BATTLEGROUND, bgTypeId_, nullptr)) {
        ChatHandler(this).PSendSysMessage(LANG_BG_DISABLED);
        return;
    }

    LOG_DEBUG("network",
              "WORLD: Recvd CMSG_BATTLEMASTER_JOIN Message from {}",
              guid.ToString());

    // get queue typeid and random typeid to check if already queued for them
    BattlegroundTypeId      bgTypeId = BattlegroundTypeId(bgTypeId_);
    BattlegroundQueueTypeId bgQueueTypeId =
        BattlegroundMgr::BGQueueTypeId(bgTypeId, 0);
    BattlegroundQueueTypeId bgQueueTypeIdRandom =
        BattlegroundMgr::BGQueueTypeId(BATTLEGROUND_RB, 0);

    // safety check - bgQueueTypeId == BATTLEGROUND_QUEUE_NONE if tried to queue
    // for arena using this function
    ...
    if (instanceId)
        bg = sBattlegroundMgr->GetBattlegroundThroughClientInstance(instanceId,
                                                                    bgTypeId);
    ...
    PvPDifficultyEntry const* bracketEntry =
        GetBattlegroundBracketByLevel(bg->GetMapId(), _player->GetLevel());
    if (!bracketEntry)
        return;
    ...
    if (!_player->HasFreeBattlegroundQueueId()) {
        ...
        sBattlegroundMgr->BuildGroupJoinedBattlegroundPacket(
            &data, ERR_BATTLEGROUND_TOO_MANY_QUEUES);
        SendPacket(&data);
        return;
    }
    ...
    GroupJoinBattlegroundResult err =
        GroupJoinBattlegroundResult(bg->GetBgTypeID());

    if (!sScriptMgr->CanJoinInBattlegroundQueue(
            _player, guid, bgTypeId, joinAsGroup, err) &&
        err <= 0) {
        WorldPacket data;
    ...
    if (!joinAsGroup) {
        ...
        else if (lfgState > lfg::LFG_STATE_NONE &&
                 (lfgState != lfg::LFG_STATE_QUEUED ||
                  !sWorld->getBoolConfig(
                      CONFIG_ALLOW_JOIN_BG_AND_LFG))) // using lfg system
        {
        ...
        else if (_player->InBattlegroundQueueForBattlegroundQueueType(
                     bgQueueTypeIdRandom)) // queued for random bg, so can't
                                           // queue for anything else
        {
            err = ERR_IN_RANDOM_BG;
        }
        else if (_player->InBattlegroundQueueForBattlegroundQueueType(
                     bgQueueTypeId)) // queued for this bg
        {
            err = ERR_BATTLEGROUND_NONE;
        }
        else if (_player->InBattlegroundQueue() &&
                 bgTypeId == BATTLEGROUND_RB) // already in queue, so can't
                                              // queue for random
        {
            err = ERR_IN_NON_RANDOM_BG;
                 ...
                 _player->InBattlegroundQueueForBattlegroundQueueType(
                     BATTLEGROUND_QUEUE_3v3) ||
                 _player->InBattlegroundQueueForBattlegroundQueueType(
                     BATTLEGROUND_QUEUE_5v5)) // can't be already queued for
                                              // arenas
        {
        ...
        // be teleported yet
        else if (_player->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_TELEPORT) &&
                 _player->GetMapId() == 609 && !_player->IsGameMaster() &&
                 !_player->HasSpell(50977)) {
            err = ERR_BATTLEGROUND_NONE;
        }
        ...
        GroupQueueInfo* ginfo       = bgQueue.AddGroup(_player,
                                                 nullptr,
                                                 bgTypeId,
                                                 bracketEntry,
                                                 0,
    ...
    // check if group can queue:
        ...
        grp->DoForAllMembers(
            [&err, bgQueueTypeId, bgQueueTypeIdRandom, bgTypeId](
                Player* member) {
                                              ...
                                              // queue for anything else
                {
                    err = ERR_IN_RANDOM_BG;
                         ...
                         bgTypeId == BATTLEGROUND_RB) // already in queue, so
                                                      // can't queue for random
                {
                    err = ERR_IN_NON_RANDOM_BG;
                }
                else if (member->InBattlegroundQueueForBattlegroundQueueType(
                             BATTLEGROUND_QUEUE_2v2) ||
                         member->InBattlegroundQueueForBattlegroundQueueType(
                             BATTLEGROUND_QUEUE_3v3) ||
                         member->InBattlegroundQueueForBattlegroundQueueType(
                             BATTLEGROUND_QUEUE_5v5)) // can't be already queued
                                                      // for arenas
                {
                    err = ERR_BATTLEGROUND_QUEUED_FOR_RATED;
                }
        ...
        if (err) {
            err = grp->CanJoinBattlegroundQueue(
                bg, bgQueueTypeId, 0, bg->GetMaxPlayersPerTeam(), false, 0);
        }
        ...
        GroupQueueInfo* ginfo = bgQueue.AddGroup(
            _player, grp, bgTypeId, bracketEntry, 0, false, isPremade, 0, 0);
        avgWaitTime = bgQueue.GetAverageQueueWaitTime(ginfo);

        grp->DoForAllMembers([bg, err, bgQueueTypeId, avgWaitTime](
                                 Player* member) {
            WorldPacket data;
    ...
    sBattlegroundMgr->ScheduleQueueUpdate(
        0, 0, bgQueueTypeId, bgTypeId, bracketEntry->GetBracketId());
}
...
void WorldSession::HandlePVPLogDataOpcode(WorldPacket& /*recvData*/)
{
    ...
    // Prevent players from sending BuildPvpLogDataPacket in an arena except for
    // when sent in BattleGround::EndBattleGround.
    if (bg->isArena())
        return;
...
void WorldSession::HandleBattlefieldListOpcode(WorldPacket& recvData)
{
    ...
    uint8 fromWhere;
    recvData >> fromWhere; // 0 - battlemaster (lua: ShowBattlefieldList), 1 -
                           // UI (lua: RequestBattlegroundInstanceInfo)

    uint8 canGainXP;
    recvData >>
        canGainXP; // players with locked xp have their own bg queue on retail

    BattlemasterListEntry const* bl =
        sBattlemasterListStore.LookupEntry(bgTypeId);
    if (!bl) {
        LOG_DEBUG("bg.battleground",
                  "BattlegroundHandler: invalid bgtype ({}) with player (Name: "
                  "{}, {}) received.",
                  bgTypeId,
                  _player->GetName(),
                  _player->GetGUID().ToString());
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundListPacket(&data,
                                                  ObjectGuid::Empty,
                                                  _player,
                                                  BattlegroundTypeId(bgTypeId),
                                                  fromWhere);
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket& recvData)
{
    ...
    if (!sBattlemasterListStore.LookupEntry(bgTypeId_)) {
        LOG_DEBUG("bg.battleground",
                  "CMSG_BATTLEFIELD_PORT {} ArenaType: {}, Unk: {}, BgType: "
                  "{}, Action: {}. Invalid BgType!",
                  GetPlayerInfo(),
                  arenaType,
                  unk2,
                  bgTypeId_,
    ...
    BattlegroundTypeId      bgTypeId = BattlegroundTypeId(bgTypeId_);
    BattlegroundQueueTypeId bgQueueTypeId =
        BattlegroundMgr::BGQueueTypeId(bgTypeId, arenaType);
    BattlegroundQueue& bgQueue =
        sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId);
    ...
    if (!bgQueue.GetPlayerGroupInfoData(_player->GetGUID(), &ginfo)) {
        LOG_DEBUG("bg.battleground",
                  "CMSG_BATTLEFIELD_PORT {} ArenaType: {}, Unk: {}, BgType: "
                  "{}, Action: {}. Player not in queue (No player Group Info)!",
                  GetPlayerInfo(),
                  arenaType,
                  unk2,
                  bgTypeId_,
    ...
    if (!ginfo.IsInvitedToBGInstanceGUID && action == 1) {
        LOG_DEBUG("bg.battleground",
                  "CMSG_BATTLEFIELD_PORT {} ArenaType: {}, Unk: {}, BgType: "
                  "{}, Action: {}. Player is not invited to any bg!",
                  GetPlayerInfo(),
                  arenaType,
                  unk2,
                  bgTypeId_,
    ...
    Battleground* bg = sBattlegroundMgr->GetBattleground(
        ginfo.IsInvitedToBGInstanceGUID, bgTypeId);
    if (!bg) {
        if (action) {
            LOG_DEBUG("bg.battleground",
                      "CMSG_BATTLEFIELD_PORT {} ArenaType: {}, Unk: {}, "
                      "BgType: {}, Action: {}. Cant find BG with id {}!",
                      GetPlayerInfo(),
                      arenaType,
                      unk2,
                      bgTypeId_,
        ...
        if (!bg) {
            LOG_ERROR(
                "network",
                "BattlegroundHandler: bg_template not found for type id {}.",
                bgTypeId);
            return;
        }
    }
    ...
    if (action == 1 && ginfo.ArenaType == 0) {
        // can't join with deserter, check it here right before joining to be
        // sure
        if (!_player->CanJoinToBattleground()) {
            ...
            action = 0;
            LOG_DEBUG("bg.battleground",
                      "Player {} {} has a deserter debuff, do not port him to "
                      "battleground!",
                      _player->GetName(),
                      _player->GetGUID().ToString());
        }
        ...
        if (_player->GetLevel() > bg->GetMaxLevel()) {
            LOG_ERROR("network",
                      "Player {} {} has level ({}) higher than maxlevel ({}) "
                      "of battleground ({})! Do not port him to battleground!",
                      _player->GetName(),
                      _player->GetGUID().ToString(),
                      _player->GetLevel(),
                      bg->GetMaxLevel(),
    ...
    // get player queue slot index for this bg (can be in up to 2 queues at the
    // same time)
    ...
    if (action) // accept
    {
        ...
        // this is still needed here if battleground "jumping" shouldn't add
        // deserter debuff also this is required to prevent stuck at old
        // battleground after SetBattlegroundId set to new
        ...
        for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i) {
            ...
            if (playerBgQueueTypeId != BATTLEGROUND_QUEUE_NONE &&
                playerBgQueueTypeId != bgQueueTypeId) {
                _player->RemoveBattlegroundQueueId(playerBgQueueTypeId);
                sBattlegroundMgr->GetBattlegroundQueue(playerBgQueueTypeId)
        ...
        sBattlegroundMgr->SendToBattleground(
            _player, ginfo.IsInvitedToBGInstanceGUID, bgTypeId);

        LOG_DEBUG("bg.battleground",
                  "Battleground: player {} {} joined battle for bg {}, bgtype "
                  "{}, queue type {}.",
                  _player->GetName(),
                  _player->GetGUID().ToString(),
                  bg->GetInstanceID(),
    ...
    else // leave queue
    {
        for (auto const& playerGuid : ginfo.Players) {
            ...
            LOG_DEBUG("bg.battleground",
                      "Battleground: player {} {} left queue for bgtype {}, "
                      "queue type {}.",
                      player->GetName(),
                      playerGuid.ToString(),
                      bg->GetBgTypeID(),
        ...
        if (bg->isBattleground() && (bg->GetStatus() == STATUS_IN_PROGRESS ||
            ...
            sScriptMgr->OnBattlegroundDesertion(_player,
...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (Battleground* bg = _player->GetBattleground())
        if (bg->GetPlayers().count(_player->GetGUID())) {
            sBattlegroundMgr->BuildBattlegroundStatusPacket(
                &data,
                bg,
                _player->GetCurrentBattlegroundQueueSlot(),
                STATUS_IN_PROGRESS,
                bg->GetEndTime(),
                bg->GetStartTime(),
                bg->GetArenaType(),
    ...
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i) {
        ...
        if (ginfo.IsInvitedToBGInstanceGUID) {
            ...
            uint32 remainingTime =
                (GameTime::GetGameTimeMS().count() < ginfo.RemoveInviteTime
                     ? getMSTimeDiff(GameTime::GetGameTimeMS().count(),
        ...
        // if not invited - send STATUS_WAIT_QUEUE
            ...
            // expected bracket entry
            PvPDifficultyEntry const* bracketEntry =
                GetBattlegroundBracketByLevel(bgt->GetMapId(),
                                              _player->GetLevel());
            if (!bracketEntry)
                continue;
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
    ...
    default:
        LOG_ERROR("network",
                  "Unknown arena slot {} at HandleBattlemasterJoinArena()",
                  arenaslot);
        return;
    }
    ...
    // expected bracket entry
    PvPDifficultyEntry const* bracketEntry =
        GetBattlegroundBracketByLevel(bgt->GetMapId(), _player->GetLevel());
    if (!bracketEntry)
        return;

    // must have free queue slot
    // pussywizard: allow being queued only in one arena queue, and it even
    // cannot be together with bg queues
    if (_player->InBattlegroundQueue()) {
        WorldPacket data;
        sBattlegroundMgr->BuildGroupJoinedBattlegroundPacket(
            &data, ERR_BATTLEGROUND_CANNOT_QUEUE_FOR_RATED);
        SendPacket(&data);
        return;
    }
    ...
    // queue result (default ok)
    GroupJoinBattlegroundResult err =
        GroupJoinBattlegroundResult(bgt->GetBgTypeID());

    if (!sScriptMgr->CanJoinInArenaQueue(
            _player, guid, arenaslot, bgTypeId, asGroup, isRated, err) &&
        err <= 0) {
        WorldPacket data;
        sBattlegroundMgr->BuildGroupJoinedBattlegroundPacket(&data, err);
        SendPacket(&data);
    ...
    if (!asGroup) {
        ...
        if (_player->GetBattlegroundQueueIndex(bgQueueTypeId) <
        ...
        LOG_DEBUG("bg.battleground",
                  "Battleground: player joined queue for arena, skirmish, bg "
                  "queue type {} bg type {}: {}, NAME {}",
                  bgQueueTypeId,
                  bgTypeId,
                  _player->GetGUID().ToString(),
                  _player->GetName());
    ...
    // check if group can queue:
        ...
        err = grp->CanJoinBattlegroundQueue(
            bgt, bgQueueTypeId, arenatype, arenatype, (bool)isRated, arenaslot);
        ...
        if (err > 0) {
            ...
            if (isRated) {
                LOG_DEBUG("bg.battleground",
                          "Battleground: arena team id {}, leader {} queued "
                          "with matchmaker rating {} for type {}",
                          _player->GetArenaTeamId(arenaslot),
                          _player->GetName(),
                          matchmakerRating,
                          arenatype);
                bgt->SetRated(true);
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr;
            ...
            LOG_DEBUG("bg.battleground",
                      "Battleground: player joined queue for arena as group bg "
                      "queue type {} bg type {}: {}, NAME {}",
                      bgQueueTypeId,
                      bgTypeId,
                      member->GetGUID().ToString(),
                      member->GetName());
    ...
    sBattlegroundMgr->ScheduleQueueUpdate(matchmakerRating,
                                          arenatype,
                                          bgQueueTypeId,
                                          bgTypeId,
                                          bracketEntry->GetBracketId());
...
void WorldSession::HandleReportPvPAFK(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("bg.battleground",
              "WorldSession::HandleReportPvPAFK: {} reported {}",
              _player->GetName(),
