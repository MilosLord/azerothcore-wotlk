...
bool LoginQueryHolder::Initialize()
{
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_QUESTSTATUS);
    stmt->SetData(0, lowGuid);
    ...
    stmt->SetData(0, lowGuid);
    res &=
        SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_SEASONAL_QUEST_STATUS, stmt);

    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_REPUTATION);
    stmt->SetData(0, lowGuid);
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_SOCIALLIST);
    stmt->SetData(0, lowGuid);
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_ACHIEVEMENTS);
    stmt->SetData(0, lowGuid);
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_ENTRY_POINT);
    stmt->SetData(0, lowGuid);
...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result) {
        do {
            ObjectGuid guid = ObjectGuid::Create<HighGuid::Player>(
                (*result)[0].Get<uint32>());
            LOG_DEBUG("network.opcode",
                      "Loading char {} from account {}.",
                      guid.ToString(),
                      GetAccountId());
...
void WorldSession::HandleCharEnumOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_DECLINED_NAMES_USED))
        stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_SEL_ENUM_DECLINED_NAME);
    else
    ...
    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(std::bind(
            &WorldSession::HandleCharEnum, this, std::placeholders::_1)));
}

void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    std::shared_ptr<CharacterCreateInfo> createInfo =
        std::make_shared<CharacterCreateInfo>();

    recvData >> createInfo->Name >> createInfo->Race >> createInfo->Class >>
        createInfo->Gender >> createInfo->Skin >> createInfo->Face >>
    ...
    if (!classEntry) {
        ...
        LOG_ERROR("network.opcode",
                  "Class ({}) not found in DBC while creating new char for "
                  "account (ID: {}): wrong DBC files or cheater?",
                  createInfo->Class,
                  GetAccountId());
        return;
    }

    ChrRacesEntry const* raceEntry =
        sChrRacesStore.LookupEntry(createInfo->Race);
    if (!raceEntry) {
        SendCharCreate(CHAR_CREATE_FAILED);
        LOG_ERROR("network.opcode",
                  "Race ({}) not found in DBC while creating new char for "
                  "account (ID: {}): wrong DBC files or cheater?",
                  createInfo->Race,
                  GetAccountId());
        return;
    ...
    if (raceEntry->expansion > Expansion()) {
        ...
        LOG_ERROR("network.opcode",
                  "Expansion {} account:[{}] tried to Create character with "
                  "expansion {} race ({})",
                  Expansion(),
                  GetAccountId(),
                  raceEntry->expansion,
    ...
    uint32 heroic_free_slots =
        sWorld->getIntConfig(CONFIG_HEROIC_CHARACTERS_PER_REALM);
    ...
    // speedup check for heroic class disabled case
    uint32 req_level_for_heroic = sWorld->getIntConfig(
        CONFIG_CHARACTER_CREATING_MIN_LEVEL_FOR_HEROIC_CHARACTER);
    if (AccountMgr::IsPlayerAccount(GetSecurity()) &&
        createInfo->Class == CLASS_DEATH_KNIGHT &&
        req_level_for_heroic > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
    ...
    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt)
            .WithChainingPreparedCallback([this](QueryCallback& queryCallback,
            ...
            .WithChainingPreparedCallback([this](QueryCallback& queryCallback,
                ...
                if (acctCharCount >= static_cast<uint64>(sWorld->getIntConfig(
            ...
            .WithChainingPreparedCallback([this, createInfo](
                                              QueryCallback&      queryCallback,
                                              PreparedQueryResult result) {
                if (result) {
                    Field* fields         = result->Fetch();
                    createInfo->CharCount = uint8(
                        fields[0].Get<uint64>()); // SQL's COUNT() returns
                                                  // uint64 but it will always
                                                  // be less than uint8.Max

                    if (createInfo->CharCount >=
                        sWorld->getIntConfig(CONFIG_CHARACTERS_PER_REALM)) {
                ...
                bool allowTwoSideAccounts =
                    !sWorld->IsPvPRealm() ||
                    sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_ACCOUNTS) ||
                    !AccountMgr::IsPlayerAccount(GetSecurity());
                uint32 skipCinematics =
                    sWorld->getIntConfig(CONFIG_SKIP_CINEMATICS);

                std::function<void(PreparedQueryResult)>
                    finalizeCharacterCreation = [this,
                                                 createInfo](PreparedQueryResult
                                                                 result) {
                        if (!sScriptMgr->CanAccountCreateCharacter(
                                GetAccountId(),
                                createInfo->Race,
                                createInfo->Class)) {
                            SendCharCreate(CHAR_CREATE_DISABLED);
                        ...
                        uint32 heroicReqLevel = sWorld->getIntConfig(
                            CONFIG_CHARACTER_CREATING_MIN_LEVEL_FOR_HEROIC_CHARACTER);
                        ...
                        if (result) {
                            TeamId teamId =
                                Player::TeamIdForRace(createInfo->Race);
                            uint32 freeDeathKnightSlots = sWorld->getIntConfig(
                            ...
                            // search same race for cinematic or same class if
                            // need
                            /// @todo check if cinematic already shown? (already
                            /// logged in?; cinematic field)
                        ...
                        // database
                        if (sCharacterCache->GetCharacterGuidByName(
                                createInfo->Name)) {
                            SendCharCreate(CHAR_CREATE_NAME_IN_USE);
                            return;
                        }

                        std::shared_ptr<Player> newChar(
                            new Player(this), [](Player* ptr) {
                                // Only when player is created correctly do
                                // clean
                        ...
                        if ((haveSameRace && skipCinematics == 1) ||
                            skipCinematics == 2)
                            newChar->setCinematic(1); // not show intro

                        newChar->SetAtLoginFlag(AT_LOGIN_FIRST); // First login

                        CharacterDatabaseTransaction characterTransaction =
                            CharacterDatabase.BeginTransaction();
                        LoginDatabaseTransaction trans =
                        ...
                        AddTransactionCallback(
                            CharacterDatabase.AsyncCommitTransaction(
                                characterTransaction))
                            .AfterComplete([this, newChar = std::move(newChar)](
                                               bool success) {
                                if (success) {
                ...
                queryCallback
                    ...
                    .SetNextQuery(CharacterDatabase.AsyncQuery(stmt));
            }));
}
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    if (ObjectAccessor::FindConnectedPlayer(guid) ||
        sWorld->FindOfflineSessionForCharacterGUID(guid.GetCounter())) {
    ...
    if (CharacterCacheEntry const* playerData =
    ...
    LOG_INFO("entities.player.character",
             "Account: {}, IP: {} deleted character: {}, {}, Level: {}",
             accountId,
             GetRemoteAddress(),
             name,
             guid.ToString(),
    ...
    sScriptMgr->OnPlayerDelete(
        guid, initAccountId); // To prevent race conditioning, but as it also
                              // makes sense, we hand the accountId over for
                              // successful delete.
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    if (!playerGuid.IsPlayer() || !IsLegitCharacterForAccount(playerGuid)) {
        LOG_ERROR("network",
                  "Account ({}) can't login with that character ({}).",
                  GetAccountId(),
                  playerGuid.ToString());
    ...
    // pussywizard:
    if (WorldSession* sess =
            sWorld->FindOfflineSessionForCharacterGUID(playerGuid.GetCounter()))
    ...
    if (WorldSession* sess = sWorld->FindOfflineSession(GetAccountId())) {
        ...
        if (p->GetGUID() != playerGuid)
            sess->KickPlayer(
                "No return, go to normal loading"); // no return, go to normal
                                                    // loading
        else {
            // pussywizard: players stay ingame no matter what (prevent abuse),
            // but allow to turn it off to stop crashing
            ...
            uint8 limitA = 10, limitB = 10,
                  limitC = 10; // pussywizard: this somehow froze (probably, ahh
                               // crash logs ...), and while (far) have never
                               // frozen in LogoutPlayer o_O maybe it's the
                               // combination of while(far); while(near);
            while (sess->GetPlayer() &&
                   (sess->GetPlayer()->IsBeingTeleportedFar() ||
                    (sess->GetPlayer()->IsInWorld() &&
                     sess->GetPlayer()->IsBeingTeleportedNear()))) {
                ...
                while (sess->GetPlayer() && sess->GetPlayer()->IsInWorld() &&
                       sess->GetPlayer()->IsBeingTeleportedNear()) {
    ...
    std::shared_ptr<LoginQueryHolder> holder =
        std::make_shared<LoginQueryHolder>(GetAccountId(), playerGuid);
    ...
    AddQueryHolderCallback(CharacterDatabase.DelayQueryHolder(holder))
        .AfterComplete([this](SQLQueryHolderBase const& holder) {
            HandlePlayerLoginFromDB(
                static_cast<LoginQueryHolder const&>(holder));
        });
}
...
void WorldSession::HandlePlayerLoginFromDB(LoginQueryHolder const& holder)
{
    ...
    if (!pCurrChar->LoadFromDB(playerGuid, holder)) {
        SetPlayer(nullptr);
        KickPlayer(
            "WorldSession::HandlePlayerLogin Player::LoadFromDB failed"); // disconnect
                                                                          // client,
                                                                          // player
                                                                          // no
                                                                          // set
    ...
    LoadAccountData(
        holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_ACCOUNT_DATA),
    ...
    if (uint32 guildId =
            sCharacterCache->GetCharacterGuildIdByGuid(pCurrChar->GetGUID())) {
        Guild*               guild = sGuildMgr->GetGuildById(guildId);
        Guild::Member const* member =
            guild ? guild->GetMember(pCurrChar->GetGUID()) : nullptr;
        if (member) {
            ...
            LOG_ERROR("network.opcode",
                      "Player {} ({}) marked as member of not existing guild "
                      "(id: {}), removing guild membership for player.",
                      pCurrChar->GetName(),
    ...
    if (!pCurrChar->getCinematic()) {
        ...
        if (ChrClassesEntry const* cEntry =
                sChrClassesStore.LookupEntry(pCurrChar->getClass())) {
            if (cEntry->CinematicSequence)
                pCurrChar->SendCinematicStart(cEntry->CinematicSequence);
            else if (ChrRacesEntry const* rEntry =
    ...
    if (!pCurrChar->GetMap()->AddPlayerToMap(pCurrChar) ||
        !pCurrChar->CheckInstanceLoginValid()) {
        AreaTriggerTeleport const* at =
            sObjectMgr->GetGoBackTrigger(pCurrChar->GetMapId());
        if (at)
            pCurrChar->TeleportTo(at->target_mapId,
                                  at->target_X,
                                  at->target_Y,
                                  at->target_Z,
                                  pCurrChar->GetOrientation());
        else
            pCurrChar->TeleportTo(pCurrChar->m_homebindMapId,
                                  pCurrChar->m_homebindX,
                                  pCurrChar->m_homebindY,
                                  pCurrChar->m_homebindZ,
                                  pCurrChar->m_homebindO);
    }

    pCurrChar->SendInitialPacketsAfterAddToMap();

    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_ONLINE);
    ...
    // pussywizard: send instance welcome message as when entering the instance
    // through a portal
    if (MapDifficulty const* mapDiff = GetMapDifficultyData(
            pCurrChar->GetMap()->GetId(), pCurrChar->GetMap()->GetDifficulty()))
        if (mapDiff->resetTime)
            if (time_t timeReset = sInstanceSaveMgr->GetResetTimeFor(
                    pCurrChar->GetMap()->GetId(),
                    pCurrChar->GetMap()->GetDifficulty())) {
                uint32 timeleft =
                    uint32(timeReset - GameTime::GetGameTime().count());
                pCurrChar->SendInstanceResetWarning(
                    pCurrChar->GetMap()->GetId(),
                    pCurrChar->GetMap()->GetDifficulty(),
                    timeleft,
                    true);
    ...
    if (Transport* t = pCurrChar->GetTransport())
        if (!t->IsInMap(pCurrChar)) {
            ...
            pCurrChar->m_movementInfo.RemoveMovementFlag(
                MOVEMENTFLAG_ONTRANSPORT);
        }

    // friend status
    sSocialMgr->SendFriendStatus(
        pCurrChar, FRIEND_ONLINE, pCurrChar->GetGUID(), true);

    // Place character in world (and load zone) before some object loading
    pCurrChar->LoadCorpse(
        holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_CORPSE_LOCATION));
    ...
    if (pCurrChar->m_deathState != DeathState::Alive) {
        ...
        if (pCurrChar->getRace() == RACE_NIGHTELF)
            pCurrChar->CastSpell(
                ...
                20584,
                true,
                0); // auras SPELL_AURA_INCREASE_SPEED(+speed in wisp form),
                    // SPELL_AURA_INCREASE_SWIM_SPEED(+swim speed in wisp form),
        ...
        pCurrChar->CastSpell(
            ...
            8326,
            true,
            0); // auras SPELL_AURA_GHOST, SPELL_AURA_INCREASE_SPEED(why?),
                // SPELL_AURA_INCREASE_SWIM_SPEED(why?)
        pCurrChar->SetMovement(MOVE_WATER_WALK);
    }

    // Set FFA PvP for non GM in non-rest mode
    if (sWorld->IsFFAPvPRealm() && !pCurrChar->IsGameMaster() &&
        !pCurrChar->HasPlayerFlag(PLAYER_FLAGS_RESTING))
        if (!pCurrChar->HasByteFlag(
                UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP)) {
            sScriptMgr->OnFfaPvpStateUpdate(pCurrChar, true);
            pCurrChar->SetByteFlag(
                UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP);
        }
    ...
    if (pCurrChar->HasAtLoginFlag(AT_LOGIN_RESET_TALENTS)) {
        ...
        pCurrChar->SendTalentsInfoData(
            false); // original talents send already in to
    ...
    if (firstLogin) {
        PlayerInfo const* info = sObjectMgr->GetPlayerInfo(
            pCurrChar->getRace(), pCurrChar->getClass());
        ...
        // Reputations if "StartAllReputation" is enabled, -- TODO: Fix this in
        // a better way
        if (sWorld->getBoolConfig(CONFIG_START_ALL_REP)) {
            ReputationMgr& repMgr = pCurrChar->GetReputationMgr();

            auto SendFullReputation =
                [&repMgr](std::initializer_list<uint32> factionsList) {
                    for (auto const& itr : factionsList) {
                        repMgr.SetOneFactionReputation(
                            sFactionStore.LookupEntry(itr), 42999.f, false);
                    }
                };

            SendFullReputation({942,  935,  936,  1011, 970,  967,  989,
                                932,  934,  1038, 1077, 1106, 1104, 1090,
                                1098, 1156, 1073, 1105, 1119, 1091});

            switch (pCurrChar->GetFaction()) {
            case ALLIANCE:
                SendFullReputation({72,
                                    47,
                                    69,
                                    930,
                                    730,
    ...
    LOG_INFO("entities.player",
             "Account: {} (IP: {}) Login Character:[{}] ({}) Level: {}",
             GetAccountId(),
             IP_str,
             pCurrChar->GetName(),
             pCurrChar->GetGUID().ToString(),
    ...
    if (!pCurrChar->IsStandState() &&
        !pCurrChar->HasUnitState(UNIT_STATE_STUNNED))
    ...
    if (pCurrChar->IsInWorld()) {
        if (pCurrChar->GetMountBlockId() &&
            !pCurrChar->HasAuraType(SPELL_AURA_MOUNTED)) {
            pCurrChar->CastSpell(pCurrChar, pCurrChar->GetMountBlockId(), true);
            pCurrChar->SetMountBlockId(0);

            // Xinef: refresh this in case mount aura changes anything (eg no
            // fly zone)
    ...
    // pussywizard: on login it's not possible to go back to arena as a
    // spectator, HandleMoveWorldportAckOpcode is not sent, so call it here
    ...
    if (pCurrChar->GetSession()->GetRecruiterId() != 0 ||
        pCurrChar->GetSession()->IsARecruiter()) {
        ...
        for (auto const& [accID, session] : sWorld->GetAllSessions()) {
            ...
            if ((isReferrer && pCurrChar->GetSession()->GetAccountId() ==
                                   session->GetRecruiterId()) ||
                (!isReferrer && pCurrChar->GetSession()->GetRecruiterId() ==
...
void WorldSession::HandlePlayerLoginToCharInWorld(Player* pCurrChar)
{
    ...
    // Xinef: fix possible problem with flag UNIT_FLAG_STUNNED added during
    // logout
    ...
    for (uint16 Opcode = SMSG_SET_FLAT_SPELL_MODIFIER;
         Opcode <= SMSG_SET_PCT_SPELL_MODIFIER;
         ++Opcode) // PCT = FLAT+1
    {
        uint32 modType = (Opcode == SMSG_SET_FLAT_SPELL_MODIFIER)
                             ? SPELLMOD_FLAT
                             : SPELLMOD_PCT;
    ...
    if (MapDifficulty const* mapDiff = GetMapDifficultyData(
        ...
        if (mapDiff->resetTime)
            if (time_t timeReset = sInstanceSaveMgr->GetResetTimeFor(
                ...
                uint32 timeleft =
                    uint32(timeReset - GameTime::GetGameTime().count());
                GetPlayer()->SendInstanceResetWarning(
                    pCurrChar->GetMap()->GetId(),
                    pCurrChar->GetMap()->GetDifficulty(),
                    timeleft,
                    true);
    ...
    // this shouldn't do anything, becaues offline can't be on taxi, but just in
    // case
    pCurrChar->ContinueTaxiFlight();
...
{
    LOG_ERROR("network.opcode",
              "WORLD SESSION: HandleSetFactionCheat, not expected call, please "
              "report.");
...
void WorldSession::HandleShowingHelmOpcode(
    WorldPackets::Character::ShowingHelm& packet)
{
    if (packet.ShowHelm)
...
void WorldSession::HandleShowingCloakOpcode(
    WorldPackets::Character::ShowingCloak& packet)
{
    if (packet.ShowCloak)
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recvData)
{
    std::shared_ptr<CharacterRenameInfo> renameInfo =
        std::make_shared<CharacterRenameInfo>();
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_FREE_NAME);
    ...
    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
            std::bind(&WorldSession::HandleCharRenameCallBack,
                      this,
                      renameInfo,
                      std::placeholders::_1)));
}

void WorldSession::HandleCharRenameCallBack(
    std::shared_ptr<CharacterRenameInfo> renameInfo, PreparedQueryResult result)
{
    if (!result) {
    ...
    if (ObjectAccessor::FindConnectedPlayer(
        ...
        sWorld->FindOfflineSessionForCharacterGUID(guidLow)) {
        SendCharRename(CHAR_CREATE_ERROR, renameInfo.get());
        return;
    }
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_NAME_AT_LOGIN);
    ...
    LOG_INFO(
        ...
        "Account: {} (IP: {}), Character [{}] (guid: {}) Changed name to: {}",
        GetAccountId(),
        GetRemoteAddress(),
        oldName,
        guidLow,
...
void WorldSession::HandleAlterAppearance(WorldPacket& recvData)
{
    ...
    BarberShopStyleEntry const* bs_hair =
        sBarberShopStyleStore.LookupEntry(Hair);

    if (!bs_hair || bs_hair->type != 0 || bs_hair->race != _player->getRace() ||
        bs_hair->gender != _player->getGender())
        return;

    BarberShopStyleEntry const* bs_facialHair =
        sBarberShopStyleStore.LookupEntry(FacialHair);

    if (!bs_facialHair || bs_facialHair->type != 2 ||
        bs_facialHair->race != _player->getRace() ||
        bs_facialHair->gender != _player->getGender())
        return;

    BarberShopStyleEntry const* bs_skinColor =
        sBarberShopStyleStore.LookupEntry(SkinColor);

    if (bs_skinColor &&
        (bs_skinColor->type != 3 || bs_skinColor->race != _player->getRace() ||
         bs_skinColor->gender != _player->getGender()))
        return;

    GameObject* go = _player->FindNearestGameObjectOfType(
        GAMEOBJECT_TYPE_BARBER_CHAIR, 5.0f);
    if (!go) {
    ...
    if (_player->getStandState() !=
        UNIT_STAND_STATE_SIT_LOW_CHAIR +
            go->GetGOInfo()->barberChair.chairheight) {
        WorldPacket data(SMSG_BARBER_SHOP_RESULT, 4);
        data << uint32(2);
        SendPacket(&data);
        return;
    ...
    uint32 cost = _player->GetBarberShopCost(
        bs_hair->hair_id, Color, bs_facialHair->hair_id, bs_skinColor);
    ...
    _player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_GOLD_SPENT_AT_BARBER, cost);
    ...
    _player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_VISIT_BARBER_SHOP, 1);
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    if (slot >= MAX_GLYPH_SLOT_INDEX) {
        LOG_DEBUG("network",
                  "Client sent wrong glyph slot number in opcode "
                  "CMSG_REMOVE_GLYPH {}",
                  slot);
        return;
    }
    ...
    if (uint32 glyph = _player->GetGlyph(slot)) {
        if (GlyphPropertiesEntry const* glyphEntry =
            ...
            _player->SendLearnPacket(glyphEntry->SpellId,
                                     false); // Send packet to properly handle
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    std::shared_ptr<CharacterCustomizeInfo> customizeInfo =
        std::make_shared<CharacterCustomizeInfo>();

    recvData >> customizeInfo->Guid;

    if (!IsLegitCharacterForAccount(customizeInfo->Guid)) {
        LOG_ERROR("entities.player.cheat",
                  "Account {}, IP: {} tried to customise {}, but it does not "
                  "belong to their account!",
                  GetAccountId(),
                  GetRemoteAddress(),
                  customizeInfo->Guid.ToString());
        recvData.rfinish();
        KickPlayer("WorldSession::HandleCharCustomize Trying to customise "
                   "character of another account");
        return;
    }

    // pussywizard:
    if (ObjectAccessor::FindConnectedPlayer(customizeInfo->Guid) ||
        sWorld->FindOfflineSessionForCharacterGUID(
            customizeInfo->Guid.GetCounter())) {
        recvData.rfinish();
    ...
    recvData >> customizeInfo->Name >> customizeInfo->Gender >>
        customizeInfo->Skin >> customizeInfo->HairColor >>
        customizeInfo->HairStyle >> customizeInfo->FacialHair >>
        customizeInfo->Face;

    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_CUSTOMIZE_INFO);
    stmt->SetData(0, customizeInfo->Guid.GetCounter());

    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
            std::bind(&WorldSession::HandleCharCustomizeCallback,
                      this,
                      customizeInfo,
                      std::placeholders::_1)));
}
    ...
    PreparedQueryResult                     result)
{
    ...
    CharacterCacheEntry const* playerData =
        sCharacterCache->GetCharacterCacheByGuid(customizeInfo->Guid);
    if (!playerData) {
        SendCharCustomize(CHAR_CREATE_ERROR, customizeInfo.get());
        return;
    }
    ...
    ResponseCodes res = static_cast<ResponseCodes>(
        ObjectMgr::CheckPlayerName(customizeInfo->Name, true));
    if (res != CHAR_NAME_SUCCESS) {
    ...
    if (ObjectGuid newguid =
            sCharacterCache->GetCharacterGuidByName(customizeInfo->Name)) {
    ...
    sCharacterCache->UpdateCharacterData(
        customizeInfo->Guid, customizeInfo->Name, customizeInfo->Gender);
...
void WorldSession::HandleEquipmentSetSave(WorldPacket& recvData)
{
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i) {
        ...
        if (itemGuid.GetRawValue() == 1) {
            // ignored slots saved as bit mask because we have no free special
            // values for Items[i]
...
void WorldSession::HandleEquipmentSetUse(WorldPacket& recvData)
{
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i) {
        ...
        if (_player->IsInCombat() && i != EQUIPMENT_SLOT_MAINHAND &&
            i != EQUIPMENT_SLOT_OFFHAND && i != EQUIPMENT_SLOT_RANGED)
            continue;
        ...
        if (uItem) {
            ...
            if (!item) {
                ...
                msg = _player->CanStoreItem(
                    NULL_BAG, NULL_SLOT, sDest, uItem, false);
                if (msg == EQUIP_ERR_OK) {
                    savedItems.emplace_back(
                        std::make_unique<SavedItem>(uItem, dstpos));
                    ...
                    for (uint8_t j = 0; j < savedItems.size(); ++j) {
                        _player->SwapItem(savedItems[j].get()->item->GetPos(),
                                          savedItems[j].get()->dstpos);
                    }
                    break;
                }
        ...
        if (item) {
            ...
            if (!item->IsEquipped()) {
                ...
                msg =
                    _player->CanEquipItem(NULL_SLOT, _candidatePos, item, true);
                if (msg != EQUIP_ERR_OK) {
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo =
        std::make_shared<CharacterFactionChangeInfo>();

    recvData >> factionChangeInfo->Guid;

    if (!IsLegitCharacterForAccount(factionChangeInfo->Guid)) {
        LOG_ERROR("entities.player.cheat",
                  "Account {}, IP: {} tried to factionchange character {}, but "
                  "it does not belong to their account!",
                  GetAccountId(),
                  GetRemoteAddress(),
                  factionChangeInfo->Guid.ToString());
        recvData.rfinish();
        KickPlayer("WorldSession::HandleCharFactionOrRaceChange Trying to "
                   "change faction of character of another account");
        return;
    }

    recvData >> factionChangeInfo->Name >> factionChangeInfo->Gender >>
        factionChangeInfo->Skin >> factionChangeInfo->HairColor >>
        factionChangeInfo->HairStyle >> factionChangeInfo->FacialHair >>
        factionChangeInfo->Face >> factionChangeInfo->Race;

    // pussywizard:
    if (ObjectAccessor::FindConnectedPlayer(factionChangeInfo->Guid) ||
        sWorld->FindOfflineSessionForCharacterGUID(
            factionChangeInfo->Guid.GetCounter())) {
        SendCharFactionChange(CHAR_CREATE_ERROR, factionChangeInfo.get());
        return;
    }

    factionChangeInfo->FactionChange =
        (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE);
    ...
    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
            std::bind(&WorldSession::HandleCharFactionOrRaceChangeCallback,
                      this,
                      factionChangeInfo,
                      std::placeholders::_1)));
}

void WorldSession::HandleCharFactionOrRaceChangeCallback(
    std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo,
    PreparedQueryResult                         result)
{
    if (!result) {
    ...
    // get the players old (at this moment current) race
    CharacterCacheEntry const* playerData =
        sCharacterCache->GetCharacterCacheByGuid(factionChangeInfo->Guid);
    if (!playerData) {
        SendCharFactionChange(CHAR_CREATE_ERROR, factionChangeInfo.get());
        return;
    }
    ...
    uint32 usedLoginFlag =
        (factionChangeInfo->FactionChange ? AT_LOGIN_CHANGE_FACTION
    ...
    if (factionChangeInfo->FactionChange) {
        ...
        if (playerData->GuildId &&
            ...
            SendCharFactionChange(CHAR_CREATE_CHARACTER_IN_GUILD,
        ...
        if (sArenaTeamMgr->GetArenaTeamByCaptain(factionChangeInfo->Guid)) {
            SendCharFactionChange(CHAR_CREATE_CHARACTER_ARENA_LEADER,
                                  factionChangeInfo.get());
            return;
        }
        ...
        // check mailbox
        if (playerData->MailCount) {
            SendCharFactionChange(CHAR_CREATE_CHARACTER_DELETE_MAIL,
                                  factionChangeInfo.get());
            return;
        }

        // check auctions, current packet is processed single-threaded way, so
        // not a problem
        bool has_auctions = false;

        for (uint8 i = 0; i < 2;
             ++i) // check both neutral and faction-specific AH
        {
            AuctionHouseObject* auctionHouse = sAuctionMgr->GetAuctionsMap(
                i == 0 ? 0
                       : (((1 << (playerData->Race - 1)) & RACEMASK_ALLIANCE)
                              ? 12
                              : 29));

            for (auto const& [auID, Aentry] : auctionHouse->GetAuctions()) {
                if (Aentry && (Aentry->owner == factionChangeInfo->Guid ||
                               Aentry->bidder == factionChangeInfo->Guid)) {
                    has_auctions = true;
                    break;
                }
            }
    ...
    if (factionChangeInfo->FactionChange ==
        (Player::TeamIdForRace(oldRace) == newTeam)) {
        SendCharFactionChange(factionChangeInfo->FactionChange
    ...
    if (maxMoney && money > maxMoney) {
        SendCharFactionChange(CHAR_CREATE_CHARACTER_GOLD_LIMIT,
                              factionChangeInfo.get());
        return;
    }
    ...
    std::vector<std::string_view> tokens =
        Acore::Tokenize(knownTitlesStr, ' ', false);
    ...
    ResponseCodes res = static_cast<ResponseCodes>(
        ObjectMgr::CheckPlayerName(factionChangeInfo->Name, true));
    if (res != CHAR_NAME_SUCCESS) {
    ...
    // character with this name already exist
    if (ObjectGuid newguid =
            sCharacterCache->GetCharacterGuidByName(factionChangeInfo->Name)) {
        if (newguid != factionChangeInfo->Guid) {
            SendCharFactionChange(CHAR_CREATE_NAME_IN_USE,
                                  factionChangeInfo.get());
            return;
        }
    }
    ...
    // Name Change and update atLogin flags
    {
        ...
        stmt->SetData(
            1, uint16((atLoginFlags | AT_LOGIN_RESURRECT) & ~usedLoginFlag));
        stmt->SetData(2, lowGuid);
        trans->Append(stmt);
    ...
    LOG_INFO("entities.player.character",
             "Account: {} (IP: {}), Character [{}] (guid: {}) Changed "
             "Race/Faction to: {}",
             GetAccountId(),
             GetRemoteAddress(),
             playerData->Name,
    ...
    // xinef: update global data
    sCharacterCache->UpdateCharacterData(factionChangeInfo->Guid,
                                         factionChangeInfo->Name,
    ...
    if (oldRace != factionChangeInfo->Race) {
        ...
        if (factionChangeInfo->Race != RACE_ORC &&
            factionChangeInfo->Race != RACE_HUMAN) {
            stmt = CharacterDatabase.GetPreparedStatement(
                CHAR_INS_CHAR_SKILL_LANGUAGE);
            stmt->SetData(0, lowGuid);
        ...
        if (factionChangeInfo->FactionChange) {
            {
                ...
                for (auto const& itr : sTaxiPathSetBySource) {
                    ...
                    TaxiPathNodeList const& taxiNodePaths =
                        sTaxiPathNodesByPath[taxiPath->ID];
                    ...
                    AreaTableEntry const* zone = sAreaTableStore.LookupEntry(
                        sMapMgr->GetZoneId(PHASEMASK_NORMAL,
                                           pathNode->mapid,
                                           pathNode->x,
                                           pathNode->y,
                                           pathNode->z));
                    ...
                    LFGDungeonEntry const* lfgDungeon = GetZoneLFGDungeonEntry(
                        zone->area_name[GetSessionDbLocaleIndex()],
                    ...
                    // Get level from LFGDungeonEntry because the one from
                    // AreaTableEntry is not valid If area level is too big, do
                    // not add new taxi
            ...
            if (!sWorld->getBoolConfig(
                ...
                if (uint32 guildId = playerData->GuildId)
                    if (Guild* guild = sGuildMgr->GetGuildById(guildId))
                        guild->DeleteMember(
                            factionChangeInfo->Guid, false, false, true);
            }
            ...
            Player::SavePositionInDB(
                loc, zoneId, factionChangeInfo->Guid, trans);

            // Achievement conversion
            for (auto const& [achiev_alliance, achiev_horde] :
                 sObjectMgr->FactionChangeAchievements) {
                stmt = CharacterDatabase.GetPreparedStatement(
                    CHAR_DEL_CHAR_ACHIEVEMENT_BY_ACHIEVEMENT);
                stmt->SetData(0,
                              uint16(newTeam == TEAM_ALLIANCE ? achiev_alliance
                                                              : achiev_horde));
                stmt->SetData(1, lowGuid);
                trans->Append(stmt);
            ...
            // Item conversion
            for (auto const& [item_alliance, item_horde] :
                 sObjectMgr->FactionChangeItems) {
                uint32 new_entry =
                    (newTeam == TEAM_ALLIANCE ? item_alliance : item_horde);
                uint32 old_entry =
                    (newTeam == TEAM_ALLIANCE ? item_horde : item_alliance);

                if (old_entry == 45978 /*Solid Gold Coin*/ ||
                    old_entry == 2589 /*Linen Cloth*/ ||
                    old_entry == 5976 /*Guild Tabard*/)
                    continue;
            ...
            // Quest conversion
            for (auto const& [quest_alliance, quest_horde] :
                ...
                stmt->SetData(
                    1,
                    (newTeam == TEAM_ALLIANCE ? quest_alliance : quest_horde));
                trans->Append(stmt);
                ...
                stmt->SetData(
                    1,
                    (newTeam == TEAM_ALLIANCE ? quest_horde : quest_alliance));
                stmt->SetData(2, lowGuid);
                trans->Append(stmt);
            }

            // Mark all rewarded quests as "active" (will count for completed
            // quests achievements)
            stmt = CharacterDatabase.GetPreparedStatement(
            ...
            for (auto const& [questID, quest] :
                 sObjectMgr->GetQuestTemplates()) {
                uint32 newRaceMask = (newTeam == TEAM_ALLIANCE)
                                         ? RACEMASK_ALLIANCE
                                         : RACEMASK_HORDE;

                if (quest->GetAllowableRaces() &&
                    !(quest->GetAllowableRaces() & newRaceMask)) {
                    stmt = CharacterDatabase.GetPreparedStatement(
            ...
            // Spell conversion
            for (auto const& [spell_alliance, spell_horde] :
                ...
                stmt->SetData(
                    1,
                    (newTeam == TEAM_ALLIANCE ? spell_alliance : spell_horde));
                trans->Append(stmt);
                ...
                stmt->SetData(
                    1,
                    (newTeam == TEAM_ALLIANCE ? spell_horde : spell_alliance));
                stmt->SetData(2, lowGuid);
                trans->Append(stmt);
            }

            // Reputation conversion
            for (auto const& [reputation_alliance, reputation_horde] :
                 sObjectMgr->FactionChangeReputation) {
                uint32 newReputation = (newTeam == TEAM_ALLIANCE)
                                           ? reputation_alliance
                                           : reputation_horde;
                uint32 oldReputation = (newTeam == TEAM_ALLIANCE)
                                           ? reputation_horde
                                           : reputation_alliance;
                ...
                FactionEntry const* factionEntry =
                    sFactionStore.LookupEntry(oldReputation);

                // old base reputation
                int32 oldBaseRep = sObjectMgr->GetBaseReputationOf(
                    factionEntry, oldRace, playerClass);

                // new base reputation
                int32 newBaseRep = sObjectMgr->GetBaseReputationOf(
                    sFactionStore.LookupEntry(newReputation),
                    factionChangeInfo->Race,
                    playerClass);
            ...
            if (!knownTitlesStr.empty()) {
                ...
                for (uint32 index = 0; index < knownTitles.size(); ++index) {
                    ...
                    if (thisMask)
                        ...
                        LOG_WARN(
                            "entities.player",
                            "{} has invalid title data '{}' at index {} - "
                            "skipped, this may result in titles being lost",
                            GetPlayerInfo(),
                            (index < tokens.size()) ? std::string(tokens[index])
                                                    : "<none>",
                            index);
                ...
                for (auto const& [title_alliance, title_horde] :
                     sObjectMgr->FactionChangeTitles) {
                    CharTitlesEntry const* atitleInfo =
                        sCharTitlesStore.LookupEntry(title_alliance);
                    CharTitlesEntry const* htitleInfo =
                        sCharTitlesStore.LookupEntry(title_horde);
...
void WorldSession::SendCharFactionChange(
    ResponseCodes result, CharacterFactionChangeInfo const* factionChangeInfo)
{
    WorldPacket data(SMSG_CHAR_FACTION_CHANGE,
                     1 + 8 + factionChangeInfo->Name.size() + 1 + 7);
    data << uint8(result);
    if (result == RESPONSE_SUCCESS) {
...
void WorldSession::SendCharCustomize(
    ResponseCodes result, CharacterCustomizeInfo const* customizeInfo)
{
    WorldPacket data(SMSG_CHAR_CUSTOMIZE,
                     1 + 8 + customizeInfo->Name.size() + 1 + 6);
    data << uint8(result);
    if (result == RESPONSE_SUCCESS) {
