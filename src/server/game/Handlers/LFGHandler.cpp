...
void BuildPlayerLockDungeonBlock(WorldPacket& data, lfg::LfgLockMap const& lock)
{
    ...
    for (lfg::LfgLockMap::const_iterator it = lock.begin(); it != lock.end();
         ++it) {
                                ...
                                const lfg::LfgLockPartyMap& lockMap)
{
    ...
    for (lfg::LfgLockPartyMap::const_iterator it = lockMap.begin();
         it != lockMap.end();
         ++it) {
...
void WorldSession::HandleLfgJoinOpcode(WorldPackets::LFG::LFGJoin& packet)
{
    if (!sLFGMgr->isOptionEnabled(lfg::LFG_OPTION_ENABLE_DUNGEON_FINDER |
                                  lfg::LFG_OPTION_ENABLE_RAID_BROWSER |
                                  lfg::LFG_OPTION_ENABLE_SEASONAL_BOSSES) ||
        (GetPlayer()->GetGroup() &&
         GetPlayer()->GetGroup()->GetLeaderGUID() != GetPlayer()->GetGUID() &&
         (GetPlayer()->GetGroup()->GetMembersCount() == MAXGROUPSIZE ||
          !GetPlayer()->GetGroup()->isLFGGroup())))
        return;

    if (packet.Slots.empty()) {
        LOG_DEBUG(
            "lfg", "CMSG_LFG_JOIN {} no dungeons selected", GetPlayerInfo());
        return;
    }
    ...
    for (uint32 slot : packet.Slots) {
        uint32 dungeon =
            slot & 0x00FFFFFF; // remove the type from the dungeon entry
    ...
    sLFGMgr->JoinLfg(
        GetPlayer(), uint8(packet.Roles), newDungeons, packet.Comment);
}
...
void WorldSession::HandleLfgLeaveOpcode(WorldPackets::LFG::LFGLeave& /*packet*/)
{
    ...
    LOG_DEBUG("network",
              "CMSG_LFG_LEAVE [{}] in group: {}",
              guid.ToString(),
              group ? 1 : 0);

    // Check cheating - only leader can leave the queue
    if (!group || group->GetLeaderGUID() == guid) {
        sLFGMgr->LeaveLfg(sLFGMgr->GetState(guid) == lfg::LFG_STATE_RAIDBROWSER
                              ? guid
                              : gguid);
        sLFGMgr->LeaveAllLfgQueues(
            guid, true, group ? group->GetGUID() : ObjectGuid::Empty);
    }
}
...
void WorldSession::HandleLfgSetRolesOpcode(WorldPacket& recvData)
{
    ...
    if (!group) {
        LOG_DEBUG(
            "network", "CMSG_LFG_SET_ROLES [{}] Not in group", guid.ToString());
        return;
    }
    ...
    WorldPacket& /*recvData*/)
{
    ObjectGuid guid = GetPlayer()->GetGUID();
    LOG_DEBUG(
        "network", "CMSG_LFG_PLAYER_LOCK_INFO_REQUEST [{}]", guid.ToString());

    // Get Random dungeons that can be done at a certain level and expansion
    uint8                     level = GetPlayer()->GetLevel();
    lfg::LfgDungeonSet const& randomDungeons =
        sLFGMgr->GetRandomAndSeasonalDungeons(
            level, GetPlayer()->GetSession()->Expansion());

    // Get player locked Dungeons
    sLFGMgr->InitializeLockedDungeons(GetPlayer(),
                                      GetPlayer()->GetGroup()); // pussywizard
    ...
    for (lfg::LfgDungeonSet::const_iterator it = randomDungeons.begin();
         it != randomDungeons.end();
         ++it) {
        data << uint32(*it); // Dungeon Entry (id + type)
        lfg::LfgReward const* reward =
            sLFGMgr->GetRandomDungeonReward(*it, level);
    ...
    WorldPacket& /*recvData*/)
{
    ObjectGuid guid = GetPlayer()->GetGUID();
    LOG_DEBUG(
        "network", "CMSG_LFG_PARTY_LOCK_INFO_REQUEST [{}]", guid.ToString());
...
void WorldSession::HandleLfrSearchJoinOpcode(WorldPacket& recvData)
{
    ...
    recvData >> dungeonId;
    dungeonId =
        (dungeonId & 0x00FFFFFF); // remove the type from the dungeon entry
...
void WorldSession::SendLfgUpdatePlayer(lfg::LfgUpdateData const& updateData)
{
    ...
    if (size) {
        ...
        data << uint8(size);
        for (lfg::LfgDungeonSet::const_iterator it =
                 updateData.dungeons.begin();
             it != updateData.dungeons.end();
             ++it)
            data << uint32(*it);
...
void WorldSession::SendLfgUpdateParty(lfg::LfgUpdateData const& updateData)
{
    ...
    case lfg::LFG_UPDATETYPE_PROPOSAL_BEGIN:
        ...
        break;
    case lfg::LFG_UPDATETYPE_UPDATE_STATUS:
        join = updateData.state != lfg::LFG_STATE_ROLECHECK &&
               updateData.state != lfg::LFG_STATE_NONE;
        queued = updateData.state == lfg::LFG_STATE_QUEUED;
        break;
    default:
        break;
...
void WorldSession::SendLfgRoleCheckUpdate(lfg::LfgRoleCheck const& roleCheck)
{
    ...
    if (!roleCheck.roles.empty()) {
        ...
        for (lfg::LfgRolesMap::const_iterator it = roleCheck.roles.begin();
...
{
    uint32 size = 0;
    for (lfg::LfgLockPartyMap::const_iterator it = joinData.lockmap.begin();
         it != joinData.lockmap.end();
         ++it)
        size += 8 + 4 + uint32(it->second.size()) * (4 + 4);
...
void WorldSession::SendLfgQueueStatus(lfg::LfgQueueStatusData const& queueData)
{
    LOG_DEBUG(
        "network",
        "SMSG_LFG_QUEUE_STATUS [{}] dungeon: {} - waitTime: {} - avgWaitTime: "
        "{} - waitTimeTanks: {} - waitTimeHealer: {} - waitTimeDps: {} - "
...
void WorldSession::SendLfgPlayerReward(
    lfg::LfgPlayerRewardData const& rewardData)
{
    ...
    LOG_DEBUG("lfg",
              "SMSG_LFG_PLAYER_REWARD {} rdungeonEntry: {}, sdungeonEntry: {}, "
              "done: {}",
              GetPlayerInfo(),
...
void WorldSession::SendLfgBootProposalUpdate(lfg::LfgPlayerBoot const& boot)
{
    ...
    for (lfg::LfgAnswerContainer::const_iterator it = boot.votes.begin();
         it != boot.votes.end();
         ++it) {
    ...
    LOG_DEBUG("network",
              "SMSG_LFG_BOOT_PROPOSAL_UPDATE [{}] inProgress: {} - didVote: {} "
              "- agree: {} - victim: [{}] votes: {} - agrees: {} - left: {} - "
              "needed: {} - reason {}",
              guid.ToString(),
              uint8(boot.inProgress),
              uint8(playerVote != lfg::LFG_ANSWER_PENDING),
              uint8(playerVote == lfg::LFG_ANSWER_AGREE),
              boot.victim.ToString(),
              votesNum,
              agreeNum,
              secsleft,
              lfg::LFG_GROUP_KICK_VOTES_NEEDED,
              boot.reason);
    WorldPacket data(SMSG_LFG_BOOT_PROPOSAL_UPDATE,
                     1 + 1 + 1 + 8 + 4 + 4 + 4 + 4 + boot.reason.length());
...
void WorldSession::SendLfgUpdateProposal(lfg::LfgProposal const& proposal)
{
    ...
    // show random dungeon if player selected random dungeon and it's not lfg
    // group
    if (!silent) {
        lfg::LfgDungeonSet const& playerDungeons =
            sLFGMgr->GetSelectedDungeons(guid);
    ...
    WorldPacket data(SMSG_LFG_PROPOSAL_UPDATE,
                     4 + 1 + 4 + 4 + 1 + 1 +
                         proposal.players.size() * (4 + 1 + 1 + 1 + 1 + 1));
    ...
    for (lfg::LfgProposalPlayerContainer::const_iterator it =
        ...
        if (!player.group)                // Player not it a group
        {
            ...
            data << uint8(player.group ==
                          proposal.group);        // In dungeon (silent)
...
{
    LOG_DEBUG(
        "network", "SMSG_LFG_DISABLED [{}]", GetPlayer()->GetGUID().ToString());
