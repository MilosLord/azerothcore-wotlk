...
void WorldSession::SendNameQueryOpcode(ObjectGuid guid)
{
    ...
    data << uint8(0); // realm name - only set for cross realm interaction (such
                      // as Battlegrounds)
    ...
    /*Player* player = ObjectAccessor::FindConnectedPlayer(guid);
    if (DeclinedName const* names = (player ? player->GetDeclinedNames() :
    nullptr))
    {
...
{
    auto timeResponse =
        sWorld->GetNextDailyQuestsResetTime() - GameTime::GetGameTime();
    ...
    if (ci) {
        ...
        if (loc_idx >= 0) {
            if (CreatureLocale const* cl =
                    sObjectMgr->GetCreatureLocale(entry)) {
        ...
        CreatureQuestItemList const* items =
            sObjectMgr->GetCreatureQuestItemList(entry);
        if (items)
        ...
        LOG_DEBUG("network",
                  "WORLD: CMSG_CREATURE_QUERY - NO CREATURE INFO! ({})",
                  guid.ToString());
...
void WorldSession::HandleGameObjectQueryOpcode(WorldPacket& recvData)
{
    ...
    if (info) {
        ...
        if (localeConstant >= LOCALE_enUS)
            if (GameObjectLocale const* gameObjectLocale =
                    sObjectMgr->GetGameObjectLocale(entry)) {
                ObjectMgr::GetLocaleString(
                    gameObjectLocale->Name, localeConstant, Name);
                ObjectMgr::GetLocaleString(gameObjectLocale->CastBarCaption,
                                           localeConstant,
                                           CastBarCaption);
            }

        LOG_DEBUG("network",
                  "WORLD: CMSG_GAMEOBJECT_QUERY '{}' - Entry: {}. ",
                  info->name,
                  entry);
        ...
        data << IconName; // 2.0.3, string. Icon name to use instead of default
                          // icon for go's (ex: "Attack" makes sword)
        data << CastBarCaption; // 2.0.3, string. Text will appear in Cast Bar
                                // when using GO (ex: "Collecting")
        ...
        LOG_DEBUG(
            "network",
            "WORLD: CMSG_GAMEOBJECT_QUERY - Missing gameobject info for ({})",
            guid.ToString());
    ...
    if (mapID != _player->GetMapId()) {
        ...
        if (MapEntry const* corpseMapEntry = sMapStore.LookupEntry(mapID)) {
            if (corpseMapEntry->IsDungeon() &&
                corpseMapEntry->entrance_map >= 0) {
                // if corpse map have entrance
                if (Map const* entranceMap =
...
void WorldSession::HandleNpcTextQueryOpcode(WorldPacket& recvData)
{
    ...
    if (!gossip) {
        ...
        std::string text0[MAX_GOSSIP_TEXT_OPTIONS],
            text1[MAX_GOSSIP_TEXT_OPTIONS];
        LocaleConstant locale = GetSessionDbLocaleIndex();

        for (uint8 i = 0; i < MAX_GOSSIP_TEXT_OPTIONS; ++i) {
            BroadcastText const* bct = sObjectMgr->GetBroadcastText(
                gossip->Options[i].BroadcastTextID);
            if (bct) {
            ...
            if (locale != DEFAULT_LOCALE && !bct) {
                if (NpcTextLocale const* npcTextLocale =
                    ...
                    ObjectMgr::GetLocaleString(
                        npcTextLocale->Text_0[i], locale, text0[i]);
                    ObjectMgr::GetLocaleString(
                        npcTextLocale->Text_1[i], locale, text1[i]);
                }
            }
...
void WorldSession::HandlePageTextQueryOpcode(WorldPacket& recvData)
{
    ...
    while (pageID) {
        ...
        if (!pageText) {
            ...
            if (loc_idx >= 0)
                if (PageTextLocale const* player =
...
void WorldSession::HandleQuestPOIQuery(WorldPacket& recvData)
{
    ...
    // Read quest ids and add the in a unordered_set so we don't send POIs for
    // the same quest multiple times
    ...
    for (std::unordered_set<uint32>::const_iterator itr = questIds.begin();
         itr != questIds.end();
         ++itr) {
        uint32 questId = *itr;
        bool   questOk = false;
        ...
        if (questOk) {
            ...
            if (POI) {
                ...
                for (QuestPOIVector::const_iterator itr = POI->begin();
                     itr != POI->end();
                     ++itr) {
                    ...
                    for (std::vector<QuestPOIPoint>::const_iterator itr2 =
                             itr->points.begin();
                         itr2 != itr->points.end();
                         ++itr2) {
