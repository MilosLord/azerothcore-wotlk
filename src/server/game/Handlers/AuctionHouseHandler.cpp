...
void WorldSession::HandleAuctionHelloOpcode(WorldPacket& recvData)
{
    ...
    Creature* unit =
        GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_AUCTIONEER);
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionHelloOpcode - Unit ({}) not found or "
...
void WorldSession::SendAuctionHello(ObjectGuid guid, Creature* unit)
{
    if (GetPlayer()->GetLevel() <
        ...
        SendNotification(GetAcoreString(LANG_AUCTION_REQ),
                         sWorld->getIntConfig(CONFIG_AUCTION_LEVEL_REQ));
        return;
    }
    ...
    AuctionHouseEntry const* ahEntry =
        AuctionHouseMgr::GetAuctionHouseEntry(unit->GetFaction());
    if (!ahEntry)
        return;
...
void WorldSession::HandleAuctionSellItem(WorldPacket& recvData)
{
    ...
    ObjectGuid itemGUIDs[MAX_AUCTION_ITEMS]; // 160 slot = 4x 36 slot bag +
                                             // backpack 16 slot
    ...
    if (itemsCount > MAX_AUCTION_ITEMS) {
        SendAuctionCommandResult(
            0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
        recvData.rfinish();
        return;
    }
    ...
    if (bid > MAX_MONEY_AMOUNT || buyout > MAX_MONEY_AMOUNT) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionSellItem - Player {} ({}) attempted to "
                  "sell item with higher price than max gold amount.",
                  _player->GetName(),
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionSellItem - Unit ({}) not found or you "
    ...
    AuctionHouseEntry const* auctionHouseEntry =
        AuctionHouseMgr::GetAuctionHouseEntry(creature->GetFaction());
    if (!auctionHouseEntry) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionSellItem - Unit ({}) has wrong faction.",
                  auctioneer.ToString());
        return;
    }
    ...
    for (uint32 i = 0; i < itemsCount; ++i) {
        ...
        if (!item) {
            SendAuctionCommandResult(
                0, AUCTION_SELL_ITEM, ERR_AUCTION_ITEM_NOT_FOUND);
            return;
        }
        ...
        if (sAuctionMgr->GetAItem(item->GetGUID()) || !item->CanBeTraded() ||
            item->IsNotEmptyBag() ||
            item->GetTemplate()->Flags & ITEM_FLAG_CONJURED ||
    ...
    // check if there are 2 identical guids, in this case user is most likely
    // cheating
    ...
    for (uint32 i = 0; i < itemsCount; ++i) {
        ...
        AuctionHouseObject* auctionHouse =
            sAuctionMgr->GetAuctionsMap(creature->GetFaction());

        uint32 deposit = sAuctionMgr->GetAuctionDeposit(
            auctionHouseEntry, etime, item, finalCount);
        if (!_player->HasEnoughMoney(deposit)) {
            SendAuctionCommandResult(
                0, AUCTION_SELL_ITEM, ERR_AUCTION_NOT_ENOUGHT_MONEY);
            return;
        }
        ...
        if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
            ...
            if (!auctioneerData) {
                LOG_ERROR("network.opcode",
                          "Data for auctioneer not found ({})",
                          auctioneer.ToString());
                return;
            }

            CreatureTemplate const* auctioneerInfo =
                sObjectMgr->GetCreatureTemplate(auctioneerData->id1);
            if (!auctioneerInfo) {
                LOG_ERROR("network.opcode",
                          "Non existing auctioneer ({})",
                          auctioneer.ToString());
                return;
            }

            const AuctionHouseEntry* AHEntry =
                sAuctionMgr->GetAuctionHouseEntry(auctioneerInfo->faction);
            AH->houseId = AHEntry->houseId;
        }

        // Required stack size of auction matches to current item stack size,
        // just move item to auctionhouse
        if (itemsCount == 1 && item->GetCount() == count[i]) {
            ...
            LOG_DEBUG("network.opcode",
                      "CMSG_AUCTION_SELL_ITEM: Player {} ({}) is selling item "
                      "{} entry {} ({}) with count {} with initial bid {} with "
                      "buyout {} and with time {} (in sec) in auctionhouse {}",
                      _player->GetName(),
                      _player->GetGUID().ToString(),
        ...
        else // Required stack size of auction does not match to current item
             // stack size, clone item and set correct stack size
        {
            Item* newItem = item->CloneItem(finalCount, _player);
            if (!newItem) {
                LOG_ERROR(
                    "network.opcode",
                    "CMSG_AUCTION_SELL_ITEM: Could not create clone of item {}",
                    item->GetEntry());
                SendAuctionCommandResult(
                    0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
                return;
            ...
            LOG_DEBUG("network.opcode",
                      ...
                      _player->GetName(),
                      _player->GetGUID().ToString(),
                      newItem->GetTemplate()->Name1,
                      newItem->GetEntry(),
                      newItem->GetGUID().ToString(),
                      newItem->GetCount(),
                      bid,
                      buyout,
                      auctionTime,
            ...
            for (uint32 j = 0; j < itemsCount; ++j) {
                ...
                // Item stack count equals required count, ready to delete item
                // - cloned item will be used for auction
                if (item2->GetCount() == count[j]) {
                    _player->MoveItemFromInventory(
                        item2->GetBagSlot(), item2->GetSlot(), true);
                ...
                else // Item stack count is bigger than required count, update
                     // item stack count and save to database - cloned item will
                     // be used for auction
                {
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket& recvData)
{
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionPlaceBid - Unit ({}) not found or you "
                  "can't interact with him.",
                  auctioneer.ToString());
        return;
    }
    ...
    if (!sScriptMgr->CanPlaceAuctionBid(player, auction)) {
        SendAuctionCommandResult(
            0, AUCTION_PLACE_BID, ERR_AUCTION_RESTRICTED_ACCOUNT);
        return;
    }
    ...
    // impossible have online own another character (use this for speedup check
    // in case online owner)
    Player* auction_owner = ObjectAccessor::FindConnectedPlayer(auction->owner);
    if (!auction_owner && sCharacterCache->GetCharacterAccountIdByGuid(
                              auction->owner) == GetAccountId()) {
    ...
    if (price < auction->buyout || auction->buyout == 0) {
        if (auction->bidder) {
            if (auction->bidder == player->GetGUID())
                ...
                sAuctionMgr->SendAuctionOutbiddedMail(
                    auction, price, GetPlayer(), trans);
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_UPD_AUCTION_BID);
        ...
        SendAuctionCommandResult(
            auction->Id, AUCTION_PLACE_BID, ERR_AUCTION_OK, 0);
    }
    else {
        // buyout:
        if (player->GetGUID() == auction->bidder)
            ...
            if (auction->bidder) // buyout for bidded auction ..
                sAuctionMgr->SendAuctionOutbiddedMail(
                    auction, auction->buyout, GetPlayer(), trans);
        }
        auction->bidder = player->GetGUID();
        ...
        SendAuctionCommandResult(
            auction->Id, AUCTION_PLACE_BID, ERR_AUCTION_OK);
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket& recvData)
{
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionRemoveItem - Unit ({}) not found or you "
                  "can't interact with him.",
                  auctioneer.ToString());
        return;
    }
    ...
    if (auction && auction->owner == player->GetGUID()) {
        ...
        if (pItem) {
            if (auction->bidder) // If we have a bidder, we have to send him the
                                 // money he paid
            {
                uint32 auctionCut = auction->GetAuctionCut();
                if (!player->HasEnoughMoney(
                        auctionCut)) // player doesn't have enough money, maybe
                                     // message needed
                    return;
                // some auctionBidderNotification would be needed, but don't
                // know that parts..
            ...
            MailDraft(
                auction->BuildAuctionMailSubject(AUCTION_CANCELED),
            ...
            LOG_ERROR("network.opcode",
                      "Auction id: {} has non-existed item (item: {})!!!",
                      auction->Id,
                      auction->item_guid.ToString());
            SendAuctionCommandResult(
        ...
        LOG_ERROR("network.opcode",
                  "CHEATER : {}, he tried to cancel auction (id: {}) of "
                  "another player, or auction is nullptr",
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket& recvData)
{
    ...
    recvData >> guid;
    recvData >> listfrom; // not used in fact (this list not have page control
                          // in client)
    recvData >> outbiddedCount;
    if (recvData.size() != (16 + outbiddedCount * 4)) {
        LOG_ERROR("network.opcode",
                  "Client sent bad opcode!!! with count: {} and size : {} "
                  "(must be: {})",
                  outbiddedCount,
                  (unsigned long)recvData.size(),
                  (16 + outbiddedCount * 4));
        outbiddedCount = 0;
    }
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionListBidderItems - Unit ({}) not found "
    ...
    WorldPacket data(SMSG_AUCTION_BIDDER_LIST_RESULT,
                     (4 + 4 + 4) +
                         30000); // pussywizard: ensure there is enough memory
    Player*     player = GetPlayer();
...
void WorldSession::HandleAuctionListOwnerItems(WorldPacket& recvData)
{
    ...
    recvData >> guid;
    recvData >> listfrom; // not used in fact (this list does not have page
                          // control in client)
    ...
    _lastAuctionListOwnerItemsMSTime =
        now + delay; // set longest possible here, actual executing will change
                     // this to getMSTime of that moment
    _player->m_Events.AddEvent(
        new AuctionListOwnerItemsDelayEvent(guid, _player->GetGUID()),
        _player->m_Events.CalculateTime(delay.count() - diff.count()));
}
...
void WorldSession::HandleAuctionListOwnerItemsEvent(ObjectGuid creatureGuid)
{
    ...
    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(
        creatureGuid, UNIT_NPC_FLAG_AUCTIONEER);
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleAuctionListOwnerItems - Unit ({}) not found or "
                  "you can't interact with him.",
    ...
    WorldPacket data(SMSG_AUCTION_OWNER_LIST_RESULT,
                     (4 + 4 + 4) +
                         60000); // pussywizard: ensure there is enough memory
...
void WorldSession::HandleAuctionListItems(WorldPacket& recvData)
{
    ...
    AsyncAuctionListingMgr::GetTempList().emplace_back(delay - diff,
                                                       _player->GetGUID(),
                                                       guid,
                                                       searchedname,
                                                       listfrom,
