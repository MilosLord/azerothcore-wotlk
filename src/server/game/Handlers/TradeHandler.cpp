...
{
    LOG_DEBUG(
        "network", "WORLD: Ignore Trade {}", _player->GetGUID().ToString());
...
{
    TradeData* view_trade = trader_data
                                ? _player->GetTradeData()->GetTraderData()
                                : _player->GetTradeData();

    WorldPacket data(SMSG_TRADE_STATUS_EXTENDED,
                     1 + 4 + 4 + 4 + 4 + 4 +
                         7 * (1 + 4 + 4 + 4 + 4 + 8 + 4 + 4 + 4 + 4 + 8 + 4 +
                              4 + 4 + 4 + 4 + 4));
    data << uint8(trader_data); // 1 means traders data, 0 means own
    data << uint32(0); // added in 2.4.0, this value must be equal to value from
                       // TRADE_STATUS_OPEN_WINDOW status packet (different
                       // value for different players to block multiple trades?)
    data << uint32(TRADE_SLOT_COUNT); // trade slots count/number?, = next field
                                      // in most cases
    data << uint32(TRADE_SLOT_COUNT); // trade slots count/number?, = prev field
                                      // in most cases
    ...
    for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i) {
        data << uint8(
            i); // trade slot number, if not specified, then end of packet
        ...
        if (Item* item = view_trade->GetItem(TradeSlots(i))) {
            ...
            data << uint32(
                item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED) ? 1
                                                                         : 0);
            ...
            for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT;
                 ...
                 ++enchant_slot)
                data << uint32(
                    item->GetEnchantmentId(EnchantmentSlot(enchant_slot)));
            // creator
            data << item->GetGuidValue(ITEM_FIELD_CREATOR);
            data << uint32(item->GetSpellCharges());     // charges
            data << uint32(item->GetItemSuffixFactor()); // SuffixFactor
            data << int32(
                item->GetItemRandomPropertyId());        // random properties id
...
void WorldSession::moveItems(Item* myItems[], Item* hisItems[])
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i) {
        ...
        bool            traderCanTrade =
            (!myItems[i] ||
             trader->CanStoreItem(
                 NULL_BAG, NULL_SLOT, traderDst, myItems[i], false) ==
                 EQUIP_ERR_OK);
        bool playerCanTrade =
            (!hisItems[i] ||
             _player->CanStoreItem(
                 NULL_BAG, NULL_SLOT, playerDst, hisItems[i], false) ==
                 EQUIP_ERR_OK);
        if (traderCanTrade && playerCanTrade) {
            // Ok, if trade item exists and can be stored
            // If we trade in both directions we had to check, if the trade will
            // work before we actually do it A roll back is not possible after
            // we stored it
            if (myItems[i]) {
                // logging
                LOG_DEBUG("network",
                          "partner storing: {}",
                          myItems[i]->GetGUID().ToString());
                        ...
                        _player->GetTotalPlayedTime() -
                            (trader->GetTotalPlayedTime() -
                             hisItems[i]->GetUInt32Value(
                                 ITEM_FIELD_CREATE_PLAYED_TIME)));
                // store
                _player->MoveItemToInventory(
                    playerDst, hisItems[i], true, true);
            }
        }
        else {
            ...
            if (myItems[i]) {
                if (!traderCanTrade)
                    LOG_ERROR("network.opcode",
                              "trader can't store item: {}",
                              myItems[i]->GetGUID().ToString());
                if (_player->CanStoreItem(
                        NULL_BAG, NULL_SLOT, playerDst, myItems[i], false) ==
                    EQUIP_ERR_OK)
                    _player->MoveItemToInventory(
                        playerDst, myItems[i], true, true);
                else
                    LOG_ERROR("network.opcode",
            ...
            if (hisItems[i]) {
                if (!playerCanTrade)
                    LOG_ERROR("network.opcode",
                              "player can't store item: {}",
                              hisItems[i]->GetGUID().ToString());
                if (trader->CanStoreItem(
                        NULL_BAG, NULL_SLOT, traderDst, hisItems[i], false) ==
                    EQUIP_ERR_OK)
                    trader->MoveItemToInventory(
                        traderDst, hisItems[i], true, true);
                else
                    LOG_ERROR("network.opcode",
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    // set before checks for propertly undo at problems (it already set in to
    // client)
    ...
    if (_player->GetMoney() >=
        uint32(MAX_MONEY_AMOUNT) - his_trade->GetMoney()) {
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i) {
        ...
        if (Item* item = his_trade->GetItem(TradeSlots(i))) {
            ...
            // if (item->IsBindedNotWith(_player))   // dont mark as invalid
            // when his item isnt good (not exploitable because if item is
            // invalid trade will fail anyway later on the same check)
            //{
            //     SendTradeStatus(TRADE_STATUS_NOT_ELIGIBLE);
            //     his_trade->SetAccepted(false, true);
            //     return;
    ...
    if (his_trade->IsAccepted()) {
        ...
        if (uint32 his_spell_id = his_trade->GetSpell()) {
            ...
            if (!spellInfo || !my_trade->GetItem(TRADE_SLOT_NONTRADED) ||
        ...
        hisCanCompleteTrade =
            (trader->CanStoreItems(myItems, TRADE_SLOT_TRADED_COUNT) ==
             EQUIP_ERR_OK);
        myCanCompleteTrade =
            (_player->CanStoreItems(hisItems, TRADE_SLOT_TRADED_COUNT) ==
             EQUIP_ERR_OK);
        ...
        for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i) {
            if (myItems[i]) {
                myItems[i]->SetGuidValue(ITEM_FIELD_GIFTCREATOR,
                                         _player->GetGUID());
                _player->MoveItemFromInventory(
                    myItems[i]->GetBagSlot(), myItems[i]->GetSlot(), true);
            }
            if (hisItems[i]) {
                hisItems[i]->SetGuidValue(ITEM_FIELD_GIFTCREATOR,
                                          trader->GetGUID());
                trader->MoveItemFromInventory(
                    hisItems[i]->GetBagSlot(), hisItems[i]->GetSlot(), true);
            }
        }
        ...
        if (his_trade->GetMoney() >= 10 * GOLD) {
            CharacterDatabase.Execute(
                ...
                trader->GetName(),
                trader->GetSession()->GetRemoteAddress(),
                GetAccountId(),
                _player->GetName(),
                his_trade->GetMoney(),
                6);
        ...
        // desynchronized with the other saves here (SaveInventoryAndGoldToDB()
        // not have own transaction guards)
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    if (GetPlayer()->GetLevel() <
        ...
        SendNotification(GetAcoreString(LANG_TRADE_REQ),
                         sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ));
        return;
    }
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_TRADE) &&
        pOther->GetTeamId() != _player->GetTeamId()) {
    ...
    if (pOther->GetLevel() < sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ)) {
        SendNotification(GetAcoreString(LANG_TRADE_OTHER_REQ),
                         sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ));
        return;
    }
...
void WorldSession::HandleSetTradeItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (!item || (tradeSlot != TRADE_SLOT_NONTRADED &&
    ...
    // prevent place single item into many trade slots using cheating and client
    // bugs
    ...
    // PlayerScript Hook for checking traded items if we want to filter them in
    // a custom module
    if (!sScriptMgr->CanSetTradeItem(_player, item, tradeSlot)) {
        // Do not send TRADE_STATUS_TRADE_CANCELED because it will cause double
        // display of "Transaction canceled" notification On the trade initiator
        // screen
