...
SMSG_CALENDAR_EVENT_INVITE_NOTES        [ packguid(Invitee), uint64(inviteId),
string(Text), Boolean(Unk) ] ?CMSG_CALENDAR_EVENT_INVITE_NOTES       [
uint32(unk1), uint32(unk2), uint32(unk3), uint32(unk4), uint32(unk5) ]
SMSG_CALENDAR_EVENT_INVITE_NOTES_ALERT  [ uint64(inviteId), string(Text) ]
SMSG_CALENDAR_EVENT_INVITE_STATUS_ALERT [ uint64(eventId), uint32(eventTime),
uint32(unkFlag), uint8(deletePending) ]

----- TODO -----

Finish complains' handling - what to do with received complains and how to
respond? Find out what to do with all "not used yet" opcodes Correct errors
sending (event/invite not found, invites exceeded, event already passed,
permissions etc.) Fix locked events to be displayed properly and response time
shouldn't be shown for people that haven't respond yet Copied events should
probably have a new owner
...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*recvData*/)
{
    ...
    WorldPacket data(SMSG_CALENDAR_SEND_CALENDAR,
                     1000); // Average size if no instance

    CalendarInviteStore invites = sCalendarMgr->GetPlayerInvites(guid);
    data << uint32(invites.size());
    for (CalendarInviteStore::const_iterator itr = invites.begin();
         itr != invites.end();
         ++itr) {
        ...
        if (CalendarEvent* calendarEvent =
                sCalendarMgr->GetEvent((*itr)->GetEventId())) {
    ...
    for (CalendarEventStore::const_iterator itr = playerEvents.begin();
         itr != playerEvents.end();
         ++itr) {
    ...
    for (uint8 i = 0; i < MAX_DIFFICULTY; ++i) {
        BoundInstancesMap const& m_boundInstances =
            sInstanceSaveMgr->PlayerGetBoundInstances(_player->GetGUID(),
                                                      Difficulty(i));
        for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
            ...
            if (itr->second.perm) {
                ...
                dataBuffer << uint32(
                    resetTime >= currTime ? resetTime - currTime : 0);
                dataBuffer << ObjectGuid::Create<HighGuid::Instance>(
                    save->GetInstanceId()); // instance save id as unique
                                            // instance copy id
                ++boundCounter;
            }
        }
    ...
    // pussywizard
    uint32 relationTime =
        sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_RELATIVE_TIMESTAMP) +
        sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_HOUR) *
            HOUR; // set point in time (default 29.12.2005) + X hours
    ...
    ResetTimeByMapDifficultyMap const& resets =
        sInstanceSaveMgr->GetResetTimeMap();
    for (ResetTimeByMapDifficultyMap::const_iterator itr = resets.begin();
         itr != resets.end();
         ++itr) {
        ...
        time_t period = sInstanceSaveMgr->GetExtendedResetTimeFor(
                            mapId, (Difficulty)PAIR32_HIPART(itr->first)) -
                        itr->second;
        dataBuffer << int32(period); // pussywizard: reset time period
        dataBuffer << int32(
            0); // pussywizard: reset time offset, needed for other than 7-day
                // periods if not aligned with relationTime
        ++boundCounter;
    }
    ...
    for (uint32 entry : sGameEventMgr->modifiedHolidays) {
        ...
        if (DisableMgr::IsDisabledFor(
                ...
                sGameEventMgr->GetHolidayEventId(holiday->Id),
                nullptr)) {
            continue;
        }
        ...
        for (uint8 j = 0; j < MAX_HOLIDAY_DATES; ++j)
            data << uint32(
                holiday->Date[j]); // 26 * m_date -- WritePackedTime ?
...
void WorldSession::HandleCalendarGetEvent(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_GET_EVENT. Player [{}] Event [{}]",
              _player->GetGUID().ToString(),
              eventId);

    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId))
        sCalendarMgr->SendCalendarEvent(
            _player->GetGUID(), *calendarEvent, CALENDAR_SENDTYPE_GET);
    else
        sCalendarMgr->SendCalendarCommandResult(_player->GetGUID(),
...
void WorldSession::HandleCalendarGuildFilter(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_GUILD_FILTER: Min level [{}], Max level [{}], Min "
              "rank [{}]",
              minLevel,
              maxLevel,
              minRank);
                     ...
                     ObjectGuid   playerGUID)
{
    if (!utf8::is_valid(s.begin(), s.end())) {
        LOG_INFO("network.opcode",
                 "CalendarHandler: Player ({}) attempt to {} an event with "
                 "invalid name or description (packet modification)",
                 playerGUID.ToString(),
                 action);
        recvData.rfinish();
        return false;
...
void WorldSession::HandleCalendarAddEvent(WorldPacket& recvData)
{
    ...
    // prevent attacks with non-utf8 chars -> with multiple packets it will hang
    // up the db due to errors.
    if (!validUtf8String(recvData, title, "create", guid) ||
        title.size() > 31 ||
        !validUtf8String(recvData, description, "create", guid) ||
        description.size() > 255)
        return;

    // prevent events in the past
    // To Do: properly handle timezones and remove the "- time_t(86400L)" hack
    if (time_t(eventPackedTime) <
        (GameTime::GetGameTime().count() - time_t(86400L))) {
        recvData.rfinish();
        sCalendarMgr->SendCalendarCommandResult(guid,
                                                CALENDAR_ERROR_EVENT_PASSED);
        return;
    }

    // If the event is a guild event, check if the player is in a guild
    if (CalendarEvent::IsGuildEvent(flags) ||
        CalendarEvent::IsGuildAnnouncement(flags)) {
        if (!_player->GetGuildId()) {
            recvData.rfinish();
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_GUILD_PLAYER_NOT_IN_GUILD);
            return;
        }
    }
    ...
    if (CalendarEvent::IsGuildEvent(flags) ||
        CalendarEvent::IsGuildAnnouncement(flags)) {
        if (sCalendarMgr->GetGuildEvents(_player->GetGuildId()).size() >=
            CALENDAR_MAX_GUILD_EVENTS) {
            recvData.rfinish();
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_GUILD_EVENTS_EXCEEDED);
            return;
        }
    }
    else {
        if (sCalendarMgr->GetEventsCreatedBy(guid).size() >=
            CALENDAR_MAX_EVENTS) {
            recvData.rfinish();
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_EVENTS_EXCEEDED);
            return;
        }
    }
    ...
    SetCalendarEventCreationCooldown(GameTime::GetGameTime().count() +
                                     CALENDAR_CREATE_EVENT_COOLDOWN);

    CalendarEvent* calendarEvent =
        new CalendarEvent(sCalendarMgr->GetFreeEventId(),
                          guid,
                          0,
    ...
    if (calendarEvent->IsGuildAnnouncement()) {
        ...
        try {
            ...
            for (uint32 i = 0; i < inviteCount && i < CALENDAR_MAX_INVITES;
                 ++i) {
        ...
        for (uint32 i = 0; i < inviteCount && i < CALENDAR_MAX_INVITES; ++i) {
            // 946684800 is 01/01/2000 00:00:00 - default response time
            CalendarInvite* invite =
                new CalendarInvite(sCalendarMgr->GetFreeInviteId(),
                                   calendarEvent->GetEventId(),
                                   invitee[i],
                                   guid,
                                   946684800,
                                   CalendarInviteStatus(status[i]),
                                   CalendarModerationRank(rank[i]),
                                   "");
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket& recvData)
{
    ...
    recvData >> eventId >> inviteId >> title >> description >> type >>
        repetitionType >> maxInvites >> dungeonId;
    recvData.ReadPackedTime(eventPackedTime);
    ...
    // prevent attacks with non-utf8 chars -> with multiple packets it will hang
    // up the db due to errors.
    if (!validUtf8String(recvData, title, "update", guid) ||
        title.size() > 31 ||
        !validUtf8String(recvData, description, "update", guid) ||
        description.size() > 255)
        return;
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_UPDATE_EVENT [{}] EventId [{}], InviteId [{}] "
              "Title {}, Description {}, type {} "
              "Repeatable {}, MaxInvites {}, Dungeon ID {}, Time {} Time2 {}, "
              "Flags {}",
              guid.ToString(),
              eventId,
              inviteId,
    ...
    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId)) {
        ...
        calendarEvent->SetTimeZoneTime(
            time_t(timeZoneTime)); // Not sure, seems constant from the little
                                   // sniffs we have
        ...
        sCalendarMgr->SendCalendarEventUpdateAlert(*calendarEvent,
                                                   oldEventTime);
    }
    else
        sCalendarMgr->SendCalendarCommandResult(guid,
    ...
    if (CalendarEvent* oldEvent = sCalendarMgr->GetEvent(eventId)) {
        ...
        if (oldEvent->IsGuildEvent() || oldEvent->IsGuildAnnouncement()) {
            if (oldEvent->GetGuildId() != _player->GetGuildId()) {
                sCalendarMgr->SendCalendarCommandResult(
                    guid, CALENDAR_ERROR_EVENT_INVALID);
                return;
            }
        }
        else {
        ...
        // Check if the player reached the max number of events allowed to
        // create
        if (oldEvent->IsGuildEvent() || oldEvent->IsGuildAnnouncement()) {
            if (sCalendarMgr->GetGuildEvents(_player->GetGuildId()).size() >=
                CALENDAR_MAX_GUILD_EVENTS) {
        ...
        if (GetCalendarEventCreationCooldown() >
        ...
        CalendarEvent* newEvent =
            new CalendarEvent(*oldEvent, sCalendarMgr->GetFreeEventId());
        ...
        for (CalendarInviteStore::const_iterator itr = invites.begin();
             itr != invites.end();
             ++itr)
            sCalendarMgr->AddInvite(
                newEvent,
                new CalendarInvite(**itr,
                                   sCalendarMgr->GetFreeInviteId(),
                                   newEvent->GetEventId()),
                trans);

        if (invites.size() > 1)
            CharacterDatabase.CommitTransaction(trans);
        // should we change owner when somebody makes a copy of event owned by
        // another person?
    }
    else
        sCalendarMgr->SendCalendarCommandResult(guid,
...
void WorldSession::HandleCalendarEventInvite(WorldPacket& recvData)
{
    ...
    if (Player* player =
            ObjectAccessor::FindPlayerByName(name.c_str(), false)) {
        // Invitee is online
    ...
    if (!inviteeGuid) {
        sCalendarMgr->SendCalendarCommandResult(
            playerGuid, CALENDAR_ERROR_PLAYER_NOT_FOUND);
        return;
    }

    if (_player->GetTeamId() != inviteeTeamId &&
        !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CALENDAR)) {
    ...
    if (QueryResult result =
            CharacterDatabase.Query("SELECT flags FROM character_social WHERE "
                                    "guid = {} AND friend = {}",
        ...
        if (fields[0].Get<uint8>() & SOCIAL_FLAG_IGNORED) {
            sCalendarMgr->SendCalendarCommandResult(
                playerGuid, CALENDAR_ERROR_IGNORING_YOU_S, name.c_str());
            return;
        }
    }
    ...
    if (!isPreInvite) {
        if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId)) {
            if (calendarEvent->IsGuildEvent() &&
                calendarEvent->GetGuildId() == inviteeGuildId) {
                // we can't invite guild members to guild events
                sCalendarMgr->SendCalendarCommandResult(
                    playerGuid, CALENDAR_ERROR_NO_GUILD_INVITES);
                return;
            }
            ...
            sCalendarMgr->SendCalendarCommandResult(
                playerGuid, CALENDAR_ERROR_EVENT_INVALID);
    }
    else {
        ...
        // 946684800 is 01/01/2000 00:00:00 - default response time
        CalendarInvite* invite = new CalendarInvite(inviteId,
                                                    0,
                                                    inviteeGuid,
                                                    playerGuid,
                                                    946684800,
...
void WorldSession::HandleCalendarEventSignup(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_EVENT_SIGNUP [{}] EventId [{}] Tentative {}",
              guid.ToString(),
              eventId,
              tentative);
    ...
    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId)) {
        if (calendarEvent->IsGuildEvent() &&
            calendarEvent->GetGuildId() != _player->GetGuildId()) {
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_GUILD_PLAYER_NOT_IN_GUILD);
            return;
        }
...
void WorldSession::HandleCalendarEventRsvp(WorldPacket& recvData)
{
    ...
    LOG_DEBUG(
        "network",
        "CMSG_CALENDAR_EVENT_RSVP [{}] EventId [{}], InviteId [{}], status {}",
        guid.ToString(),
        eventId,
        inviteId,
        status);
    ...
    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId)) {
        ...
        if (status != CALENDAR_STATUS_REMOVED &&
            calendarEvent->GetFlags() & CALENDAR_FLAG_INVITES_LOCKED) {
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_EVENT_LOCKED);
            return;
        }
        ...
        if (CalendarInvite* invite = sCalendarMgr->GetInvite(inviteId)) {
            ...
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_NO_INVITE); // correct?
    }
    else
        sCalendarMgr->SendCalendarCommandResult(guid,
                                                CALENDAR_ERROR_EVENT_INVALID);
...
void WorldSession::HandleCalendarEventRemoveInvite(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_EVENT_REMOVE_INVITE [{}] EventId [{}], "
              "ownerInviteId [{}], Invitee ([{}] id: [{}])",
              guid.ToString(),
              eventId,
              ownerInviteId,
              invitee.ToString(),
    ...
    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId)) {
        if (calendarEvent->GetCreatorGUID() == invitee) {
            sCalendarMgr->SendCalendarCommandResult(
                guid, CALENDAR_ERROR_DELETE_CREATOR_FAILED);
            return;
        }
...
void WorldSession::HandleCalendarEventStatus(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_EVENT_STATUS [{}] EventId [{}] ownerInviteId "
              "[{}], Invitee ({}) id: [{}], status {}",
              guid.ToString(),
              eventId,
              ownerInviteId,
...
void WorldSession::HandleCalendarEventModeratorStatus(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_EVENT_MODERATOR_STATUS [{}] EventId [{}] "
              "ownerInviteId [{}], Invitee ([{}] id: [{}], rank {}",
              guid.ToString(),
              eventId,
              ownerInviteId,
              invitee.ToString(),
    ...
    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId)) {
        if (CalendarInvite* invite = sCalendarMgr->GetInvite(inviteId)) {
            ...
            sCalendarMgr->UpdateInvite(invite);
            sCalendarMgr->SendCalendarEventModeratorStatusAlert(*calendarEvent,
                                                                *invite);
        }
        else
            sCalendarMgr->SendCalendarCommandResult(
...
void WorldSession::HandleCalendarComplain(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_COMPLAIN [{}] EventId [{}] guid [{}]",
              guid.ToString(),
              eventId,
              complainGUID.ToString());
...
void WorldSession::HandleCalendarGetNumPending(WorldPacket& /*recvData*/)
{
    ...
    LOG_DEBUG("network",
              "CMSG_CALENDAR_GET_NUM_PENDING: [{}] Pending: {}",
              guid.ToString(),
              pending);
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket& recvData)
{
    ...
    InstancePlayerBind* instanceBind = sInstanceSaveMgr->PlayerGetBoundInstance(
        GetPlayer()->GetGUID(), mapId, Difficulty(difficulty));
    if (!instanceBind || !instanceBind->perm ||
        (bool)toggleExtendOn == instanceBind->extended)
        return;
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_INSTANCE_EXTENDED);
...
void WorldSession::SendCalendarRaidLockout(InstanceSave const* save, bool add)
{
    LOG_DEBUG("network",
              "{}",
              add ? "SMSG_CALENDAR_RAID_LOCKOUT_ADDED"
    ...
    data << uint32(
        save->GetResetTime() >= currTime ? save->GetResetTime() - currTime : 0);
                                                  ...
                                                  bool isExtended)
{
    time_t currTime = GameTime::GetGameTime().count();
    time_t resetTime =
        isExtended ? save->GetExtendedResetTime() : save->GetResetTime();
    time_t resetTimeOp =
        isExtended ? save->GetResetTime() : save->GetExtendedResetTime();
    WorldPacket data(SMSG_CALENDAR_RAID_LOCKOUT_UPDATED, 4 + 4 + 4 + 4 + 8);
    data.AppendPackedTime(currTime);
    data << uint32(save->GetMapId());
    data << uint32(save->GetDifficulty());
    data << uint32(resetTimeOp >= currTime
                       ? resetTimeOp - currTime
                       : resetTimeOp); // pussywizard: old time in secs to reset
    data << uint32(resetTime >= currTime
                       ? resetTime - currTime
                       : 0); // pussywizard: new time in secs to reset
    SendPacket(&data);
}
