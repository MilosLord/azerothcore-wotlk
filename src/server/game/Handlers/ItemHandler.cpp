...
void WorldSession::HandleSplitItemOpcode(WorldPacket& recvData)
{
    ...
    if (!_player->IsValidPos(dstbag, dstslot, false)) // can be autostore pos
    {
        _player->SendEquipError(
            EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT, nullptr, nullptr);
        return;
    }
...
void WorldSession::HandleSwapInvItemOpcode(WorldPacket& recvData)
{
    ...
    // prevent attempt swap same item to current position generated by client at
    // special checting sequence
    if (srcslot == dstslot)
        return;
    ...
    if (_player->IsBankPos(INVENTORY_SLOT_BAG_0, srcslot) && !CanUseBank()) {
        // LOG_DEBUG("network", "WORLD: HandleSwapInvItemOpcode - Unit ({}) not
...
void WorldSession::HandleSwapItem(WorldPacket& recvData)
{
    ...
    if (_player->IsBankPos(srcbag, srcslot) && !CanUseBank()) {
        // LOG_DEBUG("network", "WORLD: HandleSwapItem - Unit ({}) not found or
        // you can't interact with him.", m_currentBankerGUID.ToString());
        return;
    }
...
void WorldSession::HandleAutoEquipItemOpcode(WorldPacket& recvData)
{
    ...
    if (!pProto) {
        _player->SendEquipError(pSrcItem->IsBag()
    ...
    InventoryResult msg =
        _player->CanEquipItem(NULL_SLOT, dest, pSrcItem, !pSrcItem->IsBag());
    if (msg != EQUIP_ERR_OK) {
        ...
        if (_player->IsInventoryPos(src)) {
            ...
            if (msg != EQUIP_ERR_OK)
                msg = _player->CanStoreItem(
                    srcbag, NULL_SLOT, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = _player->CanStoreItem(
                    NULL_BAG, NULL_SLOT, sSrc, pDstItem, true);
        }
        else if (_player->IsBankPos(src)) {
            msg = _player->CanBankItem(srcbag, srcslot, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = _player->CanBankItem(
                    srcbag, NULL_SLOT, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = _player->CanBankItem(
                    NULL_BAG, NULL_SLOT, sSrc, pDstItem, true);
...
void WorldSession::HandleDestroyItemOpcode(WorldPacket& recvData)
{
    ...
    // prevent drop unequipable items (in combat, for example) and non-empty
    // bags
    ...
    if (pItem->GetTemplate()->Flags & ITEM_FLAG_NO_USER_DESTROY) {
        _player->SendEquipError(
            EQUIP_ERR_CANT_DROP_SOULBOUND, nullptr, nullptr);
        return;
    }
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i) {
        ...
        for (uint8 j = EFFECT_0; j <= EFFECT_2; ++j) {
            ...
            case SPELL_AURA_MOD_DAMAGE_TAKEN:
                if (!(spellInfo->Effects[j].MiscValue &
...
void ItemTemplate::InitializeQueryData()
{
    ...
    queryData << Name1;
    queryData << uint8(
        0x00); // pProto->Name2; // blizz not send name there, just uint8(0x00);
               // <-- \0 = empty string = empty name...
    queryData << uint8(
        0x00); // pProto->Name3; // blizz not send name there, just uint8(0x00);
    queryData << uint8(
        0x00); // pProto->Name4; // blizz not send name there, just uint8(0x00);
    ...
    queryData << ScalingStatValue; // some kind of flags used to determine stat
                                   // values column
    ...
    for (int s = 0; s < MAX_ITEM_PROTO_SPELLS; ++s) {
        ...
        // Spell::SendSpellCooldown use `item_template` or if not set then only
        // use spell cooldowns
        SpellInfo const* spell = sSpellMgr->GetSpellInfo(Spells[s].SpellId);
        if (spell) {
            bool db_data = Spells[s].SpellCooldown >= 0 ||
                           Spells[s].SpellCategoryCooldown >= 0;
    ...
    if (pProto) {
        ...
        if (loc_idx >= 0) {
            if (ItemLocale const* il =
                    sObjectMgr->GetItemLocale(pProto->ItemId)) {
                ObjectMgr::GetLocaleString(il->Name, loc_idx, Name);
                ObjectMgr::GetLocaleString(
                    il->Description, loc_idx, Description);
            }
        }
        // guess size
        ...
        queryData << Name;
        queryData << uint8(
            0x00); // pProto->Name2; // blizz not send name there, just
                   // uint8(0x00); <-- \0 = empty string = empty name...
        queryData << uint8(0x00); // pProto->Name3; // blizz not send name
                                  // there, just uint8(0x00);
        queryData << uint8(0x00); // pProto->Name4; // blizz not send name
                                  // there, just uint8(0x00);
        ...
        queryData
            << pProto->ScalingStatDistribution; // scaling stats distribution
        queryData << pProto->ScalingStatValue;  // some kind of flags used to
                                                // determine stat values column
        for (int i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        ...
        for (int s = 0; s < MAX_ITEM_PROTO_SPELLS; ++s) {
            // send DBC data for cooldowns in same way as it used in
            // Spell::SendSpellCooldown use `item_template` or if not set then
            // only use spell cooldowns
            SpellInfo const* spell =
                sSpellMgr->GetSpellInfo(pProto->Spells[s].SpellId);
            if (spell) {
                ...
                if (db_data) {
                    ...
                    queryData
                        << uint32(pProto->Spells[s].SpellCategoryCooldown);
                }
                else {
                    queryData << uint32(spell->RecoveryTime);
                    queryData << uint32(spell->GetCategory());
        ...
        LOG_DEBUG("network",
                  "WORLD: CMSG_ITEM_QUERY_SINGLE - NO item INFO! (ENTRY: {})",
                  item);
...
void WorldSession::HandleReadItem(WorldPacket& recvData)
{
    ...
    // LOG_DEBUG("network.opcode", "STORAGE: Read bag = {}, slot = {}", bag,
    // slot);
    Item* pItem = _player->GetItemByPos(bag, slot);
...
void WorldSession::HandleSellItemOpcode(WorldPacket& recvData)
{
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleSellItemOpcode - Unit ({}) not found or you "
    ...
    if ((creature->GetCreatureTemplate()->flags_extra &
         CREATURE_FLAG_EXTRA_NO_SELL_VENDOR) != 0) {
        _player->SendSellError(
            SELL_ERR_CANT_SELL_TO_THIS_MERCHANT, creature, itemguid, 0);
        return;
    }
    ...
    if (pItem) {
        ...
        if (_player->GetGUID() != pItem->GetOwnerGUID()) {
            _player->SendSellError(
                SELL_ERR_CANT_SELL_ITEM, creature, itemguid, 0);
            return;
        }
        ...
        // this probably happens when right clicking a refundable item, the
        // client sends both CMSG_SELL_ITEM and CMSG_REFUND_ITEM (unverified)
        ...
        if (count == 0) {
            ...
            // prevent sell more items that exist in stack (possible only not
            // from client)
        ...
        if (pProto) {
            if (pProto->SellPrice > 0) {
                ...
                if (_player->GetMoney() >=
                    MAX_MONEY_AMOUNT - money) // prevent exceeding gold limit
                {
                    _player->SendEquipError(
                        EQUIP_ERR_TOO_MUCH_GOLD, nullptr, nullptr);
                ...
                uint32 maxDurability =
                    pItem->GetUInt32Value(ITEM_FIELD_MAXDURABILITY);
                if (maxDurability) {
                    ...
                    if (LostDurability > 0) {
                        DurabilityCostsEntry const* dcost =
                            sDurabilityCostsStore.LookupEntry(
                                pProto->ItemLevel);
                        if (!dcost) {
                            _player->SendSellError(
                                SELL_ERR_CANT_SELL_ITEM, creature, itemguid, 0);
                            LOG_ERROR("network.opcode",
                                      "WORLD: HandleSellItemOpcode - Wrong "
                                      "item lvl {} for item {} count = {}",
                                      pProto->ItemLevel,
                                      pItem->GetEntry(),
                                      count);
                            return;
                        ...
                        DurabilityQualityEntry const* dQualitymodEntry =
                            sDurabilityQualityStore.LookupEntry(
                                dQualitymodEntryId);
                        if (!dQualitymodEntry) {
                            _player->SendSellError(
                                SELL_ERR_CANT_SELL_ITEM, creature, itemguid, 0);
                            LOG_ERROR(
                                "network.opcode",
                                "WORLD: HandleSellItemOpcode - Wrong "
                                "dQualityModEntry {} for item {} count = {}",
                                dQualitymodEntryId,
                                pItem->GetEntry(),
                                count);
                            return;
                        ...
                        uint32 dmultiplier =
                            dcost->multiplier
                                [ItemSubClassToDurabilityMultiplierId(
                                    pProto->Class, pProto->SubClass)];
                        uint32 refund = uint32(
                            std::ceil(LostDurability * dmultiplier *
                        ...
                        // starter items can cost more to refund than
                        // vendorprice
                        if (refund > money) {
                            money = 1;
                        }
                ...
                if (count < pItem->GetCount()) // need split items
                {
                    ...
                    if (!pNewItem) {
                        LOG_ERROR("network.opcode",
                                  "WORLD: HandleSellItemOpcode - could not "
                                  "create clone of item {}; count = {}",
                                  pItem->GetEntry(),
                                  count);
                        _player->SendSellError(
                            SELL_ERR_CANT_SELL_ITEM, creature, itemguid, 0);
                    ...
                    _player->ItemRemovedQuestCheck(pItem->GetEntry(),
                                                   pItem->GetCount());
                    _player->RemoveItem(
                        pItem->GetBagSlot(), pItem->GetSlot(), true);
...
void WorldSession::HandleBuybackItem(WorldPacket& recvData)
{
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleBuybackItem - Unit ({}) not found or you can "
                  "not interact with him.",
                  vendorguid.ToString());
        _player->SendSellError(
            SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid::Empty, 0);
        return;
    }
    ...
    if (pItem) {
        uint32 price = _player->GetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE_1 +
                                               slot - BUYBACK_SLOT_START);
        if (!_player->HasEnoughMoney(price)) {
            _player->SendBuyError(
                BUY_ERR_NOT_ENOUGHT_MONEY, creature, pItem->GetEntry(), 0);
            return;
        }

        ItemPosCountVec dest;
        InventoryResult msg =
            _player->CanStoreItem(NULL_BAG, NULL_SLOT, dest, pItem, false);
        if (msg == EQUIP_ERR_OK) {
            if (sWorld->getBoolConfig(CONFIG_ITEMDELETE_VENDOR)) {
...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPacket& recvData)
{
    ...
    // client expects count starting at 1, and we send vendorslot+1 to client
    // already
    if (slot > 0)
        --slot;
    else
    ...
    GetPlayer()->BuyItemFromVendorSlot(
        vendorguid, slot, item, count, bag, bagslot);
}
...
void WorldSession::HandleBuyItemOpcode(WorldPacket& recvData)
{
    ...
    GetPlayer()->BuyItemFromVendorSlot(
        vendorguid, slot, item, count, NULL_BAG, NULL_SLOT);
}
...
void WorldSession::SendListInventory(ObjectGuid vendorGuid, uint32 vendorEntry)
{
    ...
    if (!vendor) {
        LOG_DEBUG("network",
                  "WORLD: SendListInventory - Unit ({}) not found or you can "
                  "not interact with him.",
                  vendorGuid.ToString());
        _player->SendSellError(
            SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid::Empty, 0);
    ...
    VendorItemData const* items =
        vendorEntry ? sObjectMgr->GetNpcVendorItemList(vendorEntry)
                    : vendor->GetVendorItems();
    if (!items) {
            ...
            if (ItemTemplate const* itemTemplate =
                ...
                if (!_player->IsGameMaster() &&
                    ((itemTemplate->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY &&
                      _player->GetTeamId() == TEAM_ALLIANCE) ||
                     (itemTemplate->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY &&
                      _player->GetTeamId() == TEAM_HORDE))) {
                    continue;
                }

                // Items sold out are not displayed in list
                uint32 leftInStock =
                    !item->maxcount ? 0xFFFFFFFF
                                    : vendor->GetVendorItemCurrentCount(item);
                ...
                // reputation discount
                int32 price =
                    item->IsGoldRequired(itemTemplate)
                        ? uint32(floor(itemTemplate->BuyPrice * discountMod))
                        : 0;

                data << uint32(slot +
                               1); // client expects counting to start at 1
...
void WorldSession::HandleAutoStoreBagItemOpcode(WorldPacket& recvData)
{
    ...
    if (_player->IsEquipmentPos(src) || _player->IsBagPos(src)) {
        InventoryResult msg =
            _player->CanUnequipItem(src, !_player->IsBagPos(src));
        if (msg != EQUIP_ERR_OK) {
            _player->SendEquipError(msg, pItem, nullptr);
            return;
        }
    ...
    ItemPosCountVec dest;
    InventoryResult msg =
        _player->CanStoreItem(dstbag, NULL_SLOT, dest, pItem, false);
    if (msg != EQUIP_ERR_OK) {
        _player->SendEquipError(msg, pItem, nullptr);
        return;
    }
...
void WorldSession::HandleItemNameQueryOpcode(WorldPacket& recvData)
{
    ...
    if (pName) {
        ...
        if (loc_idx >= 0)
            if (ItemSetNameLocale const* isnl =
                    sObjectMgr->GetItemSetNameLocale(itemid))
...
void WorldSession::HandleWrapItemOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WRAP: receive gift_bag = {}, gift_slot = {}, item_bag = {}, "
              "item_slot = {}",
              gift_bag,
              gift_slot,
              item_bag,
    ...
    if (!(gift->GetTemplate()->Flags &
          ITEM_FLAG_IS_WRAPPER)) // cheating: non-wrapper wrapper
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, gift, nullptr);
        return;
    }
    ...
    {
        _player->SendEquipError(
            EQUIP_ERR_WRAPPED_CANT_BE_WRAPPED, item, nullptr);
        return;
    }

    if (item->IsEquipped()) {
        _player->SendEquipError(
            EQUIP_ERR_EQUIPPED_CANT_BE_WRAPPED, item, nullptr);
        return;
    }

    if (item->GetGuidValue(ITEM_FIELD_GIFTCREATOR)) // HasFlag(ITEM_FIELD_FLAGS,
                                                    // ITEM_FLAGS_WRAPPED);
    {
        _player->SendEquipError(
            EQUIP_ERR_WRAPPED_CANT_BE_WRAPPED, item, nullptr);
    ...
    if (item->GetMaxStackCount() != 1) {
        _player->SendEquipError(
            EQUIP_ERR_STACKABLE_CANT_BE_WRAPPED, item, nullptr);
        return;
    }
    ...
    if (item->GetTemplate()->MaxCount > 0) {
        _player->SendEquipError(
            EQUIP_ERR_UNIQUE_CANT_BE_WRAPPED, item, nullptr);
        return;
    }
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_GIFT);
...
void WorldSession::HandleSocketOpcode(WorldPacket& recvData)
{
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i) // get geminfo from dbc storage
        GemProps[i] = (Gems[i]) ? sGemPropertiesStore.LookupEntry(
                                      Gems[i]->GetTemplate()->GemProperties)
                                : nullptr;

    // Find first prismatic socket
    int32 firstPrismatic = 0;
    while (firstPrismatic < MAX_GEM_SOCKETS &&
           itemProto->Socket[firstPrismatic].Color)
        ++firstPrismatic;
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i) // check for hack maybe
    {
        ...
        // tried to put gem in socket where no socket exists (take care about
        // prismatic sockets)
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i) {
        ...
        // continue check for case when attempt add 2 similar unique equipped
        // gems in one item.
        ...
        if (iGemProto->ItemLimitCategory) {
            if (ItemLimitCategoryEntry const* limitEntry =
                    sItemLimitCategoryStore.LookupEntry(
                        iGemProto->ItemLimitCategory)) {
                // NOTE: limitEntry->mode is not checked because if item has
                // limit then it is applied in equip case
                for (int j = 0; j < MAX_GEM_SOCKETS; ++j) {
                    if (Gems[j]) {
                        // new gem
                        if (iGemProto->ItemLimitCategory ==
                            Gems[j]->GetTemplate()->ItemLimitCategory)
                            ++limit_newcount;
                    }
                    else if (OldEnchants[j]) {
                        // existing gem
        ...
        if (itemTarget->IsEquipped()) {
            if (InventoryResult res = _player->CanEquipUniqueItem(
                    Gems[i], slot, std::max(limit_newcount, 0))) {
    ...
    bool SocketBonusActivated =
        itemTarget->GemsFitSockets(); // save state of socketbonus
    _player->ToggleMetaGemsActive(
        slot, false); // turn off all metagems (except for the target item)

    // if a meta gem is being equipped, all information has to be written to the
    // item before testing if the conditions for the gem are met
    ...
    for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT;
        ...
        _player->ApplyEnchantment(
            itemTarget, EnchantmentSlot(enchant_slot), false);
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i) {
        if (GemEnchants[i]) {
            ...
            if (Item* guidItem = _player->GetItemByGuid(gem_guids[i]))
                _player->DestroyItem(
                    guidItem->GetBagSlot(), guidItem->GetSlot(), true);
        }
    }
    ...
    for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT;
         ...
         ++enchant_slot)
        _player->ApplyEnchantment(
            itemTarget, EnchantmentSlot(enchant_slot), true);

    bool SocketBonusToBeActivated =
        itemTarget->GemsFitSockets(); // current socketbonus state
    if (SocketBonusActivated ^
        SocketBonusToBeActivated) // if there was a change...
    {
        _player->ApplyEnchantment(itemTarget, BONUS_ENCHANTMENT_SLOT, false);
        itemTarget->SetEnchantment(BONUS_ENCHANTMENT_SLOT,
                                   (SocketBonusToBeActivated
                                        ? itemTarget->GetTemplate()->socketBonus
                                        : 0),
                                   0,
                                   0,
                                   _player->GetGUID());
        _player->ApplyEnchantment(itemTarget, BONUS_ENCHANTMENT_SLOT, true);
        // it is not displayed, client has an inbuilt system to determine if the
        // bonus is activated
    }

    _player->ToggleMetaGemsActive(
        slot, true); // turn on all metagems (except for target item)
...
/**
 * Handles the packet sent by the client when requesting information about item
 * text.
 *
...
bool WorldSession::recoveryItem(Item* pItem)
{
    if (sWorld->getBoolConfig(CONFIG_ITEMDELETE_METHOD) &&
        pItem->GetTemplate()->Quality >=
            sWorld->getIntConfig(CONFIG_ITEMDELETE_QUALITY) &&
        pItem->GetTemplate()->ItemLevel >=
            sWorld->getIntConfig(CONFIG_ITEMDELETE_ITEM_LEVEL)) {
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_INS_RECOVERY_ITEM);
