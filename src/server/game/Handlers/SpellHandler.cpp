...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("network",
              "WORLD: CMSG_USE_ITEM packet, bagIndex: {}, slot: {}, castCount: "
              "{}, spellId: {}, Item: {}, glyphIndex: {}, data length = {}",
              bagIndex,
              slot,
              castCount,
              spellId,
    ...
    // only allow conjured consumable, bandage, poisons (all should have the
    // 2^21 item flag set in DB)
    ...
    if (pUser->IsInCombat()) {
        for (int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i) {
            if (SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(proto->Spells[i].SpellId)) {
                if (!spellInfo->CanBeUsedInCombat()) {
                    pUser->SendEquipError(
                        EQUIP_ERR_NOT_IN_COMBAT, pItem, nullptr);
                    return;
                }
            }
        }
    ...
    if (pItem->GetTemplate()->Bonding == BIND_WHEN_USE ||
    ...
    // Note: If script stop casting it must send appropriate data to client to
    // prevent stuck item in gray state.
...
{
    LOG_DEBUG("network",
              "WORLD: CMSG_OPEN_ITEM packet, data length = {}",
              (uint32)recvPacket.size());
    ...
    // Verify that the bag is an actual bag or wrapped item that can be used
    // "normally"
    if (!(proto->Flags & ITEM_FLAG_HAS_LOOT) &&
        !item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED)) {
        pUser->SendEquipError(EQUIP_ERR_CANT_DO_RIGHT_NOW, item, nullptr);
        LOG_ERROR("network.opcode",
                  "Possible hacking attempt: Player {} [{}] tried to open item "
                  "[{}, entry: {}] which is not openable!",
                  pUser->GetName(),
                  pUser->GetGUID().ToString(),
    ...
    if (lockId) {
        ...
        if (!lockInfo) {
            ...
            LOG_ERROR("network.opcode",
                      "WORLD::OpenItem: item [{}] has an unknown lockId: {}!",
                      item->GetGUID().ToString(),
                      lockId);
            return;
        }
    ...
    if (sScriptMgr->OnBeforeOpenItem(pUser, item)) {
        if (item->HasFlag(ITEM_FIELD_FLAGS,
                          ITEM_FIELD_FLAG_WRAPPED)) // wrapped?
        {
            ...
            _queryProcessor.AddCallback(
                CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
                    std::bind(&WorldSession::HandleOpenWrappedItemCallback,
                              this,
                              bagIndex,
                              slot,
...
void WorldSession::HandleOpenWrappedItemCallback(
    uint8               bagIndex,
    uint8               slot,
    ...
    PreparedQueryResult result)
{
    ...
    if (item->GetGUID().GetCounter() != itemLowGUID ||
        !item->HasFlag(
            ITEM_FIELD_FLAGS,
            ITEM_FIELD_FLAG_WRAPPED)) // during getting result, gift was swapped
                                      // with another item
        return;

    if (!result) {
        LOG_ERROR("network",
                  "Wrapped item {} don't have record in character_gifts table "
                  "and will deleted",
                  item->GetGUID().ToString());
...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Recvd CMSG_GAMEOBJ_USE Message [{}]",
              guid.ToString());
    ...
    if (GameObject* obj = GetPlayer()->GetMap()->GetGameObject(guid)) {
        ...
        if (GetPlayer()->m_mover != GetPlayer())
            if (!(GetPlayer()->IsOnVehicle(GetPlayer()->m_mover) ||
                  GetPlayer()->IsMounted()) &&
...
void WorldSession::HandleGameobjectReportUse(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Recvd CMSG_GAMEOBJ_REPORT_USE Message [{}]",
              guid.ToString());
    ...
    _player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_USE_GAMEOBJECT,
                                       go->GetEntry());
}
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("network",
              "WORLD: got cast spell packet, castCount: {}, spellId: {}, "
              "castFlags: {}, data length = {}",
              castCount,
              spellId,
              castFlags,
              (uint32)recvPacket.size());
    ...
    if (mover->GetTypeId() == TYPEID_PLAYER) {
        ...
        if (!(spellInfo->Targets & TARGET_FLAG_GAMEOBJECT_ITEM) &&
            (!mover->ToPlayer()->HasActiveSpell(spellId) ||
             spellInfo->IsPassive())) {
            bool allow = false;
        ...
        if (Vehicle* veh = mover->GetVehicleKit())
            if (const VehicleSeatEntry* seat =
                    veh->GetSeatForPassenger(_player))
                if (seat->m_flags & VEHICLE_SEAT_FLAG_CAN_ATTACK ||
                    spellInfo->Effects[EFFECT_0].Effect == SPELL_EFFECT_OPEN_LOCK /*allow looting from vehicle, but only if player has required spell (all necessary opening spells are in playercreateinfo_spell)*/)
                    if ((mover->GetTypeId() == TYPEID_UNIT &&
                         !mover->ToCreature()->HasSpell(spellId)) ||
                        spellInfo->IsPassive()) // the creature can't cast that
                                                // spell, check player instead
                    {
                        if (!(spellInfo->Targets &
                              TARGET_FLAG_GAMEOBJECT_ITEM) &&
                            (!_player->HasActiveSpell(spellId) ||
                             spellInfo->IsPassive())) {
                            // cheater? kick? ban?
                            recvPacket
                                .rfinish(); // prevent spam at ignore packet
                            return;
                        }

                        // at this point, player is a valid caster
                        // swapping the mover will stop the check below at ==
                        // TYPEID_UNIT, so everything works fine
                        mover = _player;
                    }
    ...
    sScriptMgr->ValidateSpellAtCastSpell(
        _player, oldSpellId, spellId, castCount, castFlags);

    if (oldSpellId != spellId)
        spellInfo = sSpellMgr->GetSpellInfo(spellId);

    // Client is resending autoshot cast opcode when other spell is casted
    // during shoot rotation Skip it to prevent "interrupt" message
    ...
    // pussywizard: HandleClientCastFlags calls HandleMovementOpcodes, which can
    // result in pretty much anything. Caster not in map will crash at GetMap()
    // for spell difficulty in Spell constructor.
    if (!mover->FindMap()) {
        recvPacket.rfinish(); // prevent spam at ignore packet
        return;
    }
    ...
    if (targets.GetUnitTarget()) {
        SpellInfo const* actualSpellInfo =
            spellInfo->GetAuraRankForLevel(targets.GetUnitTarget()->GetLevel());

        // if rank not found then function return nullptr but in explicit cast
        // case original spell can be casted and later failed with appropriate
        // error message
        if (actualSpellInfo)
    ...
    Spell* spell =
        new Spell(mover, spellInfo, triggerFlag, ObjectGuid::Empty, false);

    sScriptMgr->ValidateSpellAtCastSpellResult(
        _player, mover, spell, oldSpellId, spellId);
...
void WorldSession::HandleCancelCastOpcode(WorldPacket& recvPacket)
{
    ...
    recvPacket.read_skip<uint8>(); // counter, increments with every CANCEL
                                   // packet, don't use for now
    recvPacket >> spellId;
...
void WorldSession::HandleCancelAuraOpcode(WorldPacket& recvPacket)
{
    ...
    _player->RemoveOwnedAura(
        spellId, ObjectGuid::Empty, 0, AURA_REMOVE_BY_CANCEL);
}
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket& recvPacket)
{
    ...
    if (!pet) {
        LOG_ERROR("network.opcode",
                  "HandlePetCancelAura: Attempt to cancel an aura for "
                  "non-existant pet {} by player {}",
                  guid.ToString(),
                  GetPlayer()->GetName());
        return;
    }
    ...
    if (pet != GetPlayer()->GetGuardianPet() &&
        pet != GetPlayer()->GetCharm()) {
        LOG_ERROR("network.opcode",
                  "HandlePetCancelAura: Pet {} is not a pet of player {}",
                  guid.ToString(),
                  GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandleTotemDestroyed(
    WorldPackets::Totem::TotemDestroyed& totemDestroyed)
{
    ...
    Creature* totem =
        GetPlayer()->GetMap()->GetCreature(_player->m_SummonSlot[slotId]);
...
void WorldSession::HandleSelfResOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(
            _player->GetUInt32Value(PLAYER_SELF_RES_SPELL))) {
        if (_player->HasAuraType(SPELL_AURA_PREVENT_RESURRECTION) &&
            !spell->HasAttribute(SPELL_ATTR7_BYPASS_NO_RESURRECTION_AURA)) {
            return; // silent return, client should display error by itself and
                    // not send this opcode
        }
...
void WorldSession::HandleSpellClick(WorldPacket& recvData)
{
    ...
    /// @todo: Unit::SetCharmedBy: 28782 is not in world but 0 is trying to
    /// charm it! -> crash
    if (!unit->IsInWorld())
        return;
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    Unit* creator = unit->GetAuraEffectsByType(SPELL_AURA_CLONE_CASTER)
                        .front()
                        ->GetCaster();
    if (!creator)
        return;
    ...
    if (creator->GetTypeId() == TYPEID_PLAYER) {
        ...
        static EquipmentSlots const itemSlots[] = {EQUIPMENT_SLOT_HEAD,
