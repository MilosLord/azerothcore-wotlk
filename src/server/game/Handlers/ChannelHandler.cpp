...
void WorldSession::HandleJoinChannel(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system",
              "CMSG_JOIN_CHANNEL {} Channel: {}, unk1: {}, unk2: {}, channel: "
              "{}, password: {}",
              GetPlayerInfo(),
              channelId,
              unknown1,
    ...
    if (channelId) {
        ...
        AreaTableEntry const* zone =
            sAreaTableStore.LookupEntry(GetPlayer()->GetZoneId());
    ...
    if (channelName.size() >= 100 ||
        !DisallowHyperlinksAndMaybeKick(channelName)) {
        return;
    }
...
void WorldSession::HandleLeaveChannel(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system",
              "CMSG_LEAVE_CHANNEL {} Channel: {}, unk1: {}",
              GetPlayerInfo(),
              channelName,
              unk);
    if (channelName.empty())
...
void WorldSession::HandleChannelList(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system",
              "{} {} Channel: {}",
              recvPacket.GetOpcode() == CMSG_CHANNEL_DISPLAY_LIST
...
void WorldSession::HandleGetChannelMemberCount(WorldPacket& recvPacket)
{
    ...
    if (ChannelMgr* cMgr = ChannelMgr::forTeam(GetPlayer()->GetTeamId())) {
        if (Channel* channel = cMgr->GetChannel(channelName, GetPlayer())) {
            ...
            WorldPacket data(SMSG_CHANNEL_MEMBER_COUNT,
                             channel->GetName().size() + 1 + 4);
