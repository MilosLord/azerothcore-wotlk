...
void WorldSession::HandleAddFriendOpcode(WorldPacket& recv_data)
{
    ...
    if (!AccountMgr::IsPlayerAccount(GetSecurity()) ||
        sWorld->getBoolConfig(CONFIG_ALLOW_GM_FRIEND) ||
        AccountMgr::IsPlayerAccount(
            AccountMgr::GetSecurity(friendAccountId, realm.Id.Realm))) {
        if (friendGuid) {
            if (friendGuid == GetPlayer()->GetGUID())
                friendResult = FRIEND_SELF;
            else if (GetPlayer()->GetTeamId() != teamId &&
                ...
                Player* pFriend =
                    ObjectAccessor::FindConnectedPlayer(friendGuid);
                if (pFriend && pFriend->IsVisibleGloballyFor(GetPlayer()) &&
                    !pFriend->GetSession()->IsGMAccount())
                    friendResult = FRIEND_ADDED_ONLINE;
                else
                    friendResult = FRIEND_ADDED_OFFLINE;
                if (GetPlayer()->GetSocial()->AddToSocialList(
...
void WorldSession::HandleAddIgnoreOpcode(WorldPacket& recv_data)
{
    ...
    if (ignoreGuid == GetPlayer()->GetGUID()) // not add yourself
        ...
        // ignore list full
        if (!GetPlayer()->GetSocial()->AddToSocialList(ignoreGuid,
                                                       SOCIAL_FLAG_IGNORED))
