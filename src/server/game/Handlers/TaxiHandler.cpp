...
void WorldSession::SendTaxiStatus(ObjectGuid guid)
{
    ...
    if (!unit || unit->IsHostileTo(player) ||
        !unit->HasNpcFlag(UNIT_NPC_FLAG_FLIGHTMASTER)) {
        LOG_DEBUG("network",
                  "WorldSession::SendTaxiStatus - Unit ({}) not found.",
                  guid.ToString());
        return;
    }

    // find taxi node
    uint32 nearest = sObjectMgr->GetNearestTaxiNode(unit->GetPositionX(),
                                                    unit->GetPositionY(),
                                                    unit->GetPositionZ(),
                                                    unit->GetMapId(),
                                                    player->GetTeamId());
...
void WorldSession::HandleTaxiQueryAvailableNodes(WorldPacket& recvData)
{
    ...
    if (!unit) {
        LOG_DEBUG("network",
                  "WORLD: HandleTaxiQueryAvailableNodes - Unit ({}) not found "
...
void WorldSession::SendTaxiMenu(Creature* unit)
{
    ...
    if (unit->GetEntry() == 29480)
        GetPlayer()->SetTaxiCheater(
            true); // Grimwing in Ebon Hold, special case. NOTE: Not perfect,
                   // Zul'Aman should not be included according to WoWhead, and
                   // I think taxicheat includes it.
                                ...
                                uint32 pathNode)
{
    ...
    if (Creature* critter = ObjectAccessor::GetCreature(
            *GetPlayer(), GetPlayer()->GetCritterGUID()))
...
void WorldSession::HandleActivateTaxiExpressOpcode(WorldPacket& recvData)
{
    ...
    if (!npc) {
        LOG_DEBUG("network",
                  "WORLD: HandleActivateTaxiExpressOpcode - Unit ({}) not "
                  "found or you can't interact with it.",
                  guid.ToString());
    ...
    for (uint32 i = 0; i < node_count; ++i) {
        ...
        if (!GetPlayer()->m_taxi.IsTaximaskNodeKnown(node) &&
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_ACTIVATETAXIEXPRESS from {} to {}",
              nodes.front(),
              nodes.back());
...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& recvData)
{
    ...
    if (curDest) {
        TaxiNodesEntry const* curDestNode =
            sTaxiNodesStore.LookupEntry(curDest);
        ...
        if (curDestNode && curDestNode->map_id != GetPlayer()->GetMapId() &&
            ...
            if (FlightPathMovementGenerator* flight =
                ...
                GetPlayer()->TeleportTo(curDestNode->map_id,
                                        node->x,
                                        node->y,
                                        node->z,
    ...
    GetPlayer()->SetFallInformation(GameTime::GetGameTime().count(),
...
void WorldSession::HandleActivateTaxiOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_ACTIVATETAXI from {} to {}",
              nodes[0],
              nodes[1]);
    Creature* npc =
        GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_FLIGHTMASTER);
    if (!npc) {
        LOG_DEBUG("network",
                  "WORLD: HandleActivateTaxiOpcode - Unit ({}) not found or "
    ...
    if (!GetPlayer()->isTaxiCheater()) {
        if (!GetPlayer()->m_taxi.IsTaximaskNodeKnown(nodes[0]) ||
