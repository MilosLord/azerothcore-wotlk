...
void WorldSession::HandleDismissCritter(
    WorldPackets::Pet::DismissCritter& packet)
{
    Unit* pet =
        ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, packet.CritterGUID);

    if (!pet) {
        LOG_DEBUG("network",
                  "Vanitypet ({}) does not exist - player {} ({} / account: "
                  "{}) attempted to dismiss it (possibly lagged out)",
...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network.opcode",
              "HandlePetAction: Pet {} - flag: {}, spellId: {}, target: {}.",
              guid1.ToString(),
              uint32(flag),
              spellId,
              guid2.ToString());
    ...
    if (!pet) {
        LOG_ERROR("network.opcode",
                  "HandlePetAction: Pet ({}) doesn't exist for player {}",
                  guid1.ToString(),
                  GetPlayer()->GetName());
        return;
    }
    ...
    if (pet != GetPlayer()->GetFirstControlled()) {
        LOG_ERROR("network.opcode",
                  "HandlePetAction: Pet ({}) does not belong to player {}",
                  guid1.ToString(),
                  GetPlayer()->GetName());
        return;
    }
    ...
    if (!pet->IsAlive()) {
        ...
        SpellInfo const* spell = (flag == ACT_ENABLED || flag == ACT_PASSIVE)
                                     ? sSpellMgr->GetSpellInfo(spellId)
                                     : nullptr;
        if ((flag != ACT_COMMAND || spellId != COMMAND_ABANDON) &&
            (!spell || !spell->HasAttribute(SPELL_ATTR0_ALLOW_CAST_WHILE_DEAD)))
            return;
    }

    // Xinef: allow to controll players
    if (pet->GetTypeId() == TYPEID_PLAYER && flag != ACT_COMMAND &&
        flag != ACT_REACTION)
        return;
    ...
    if (GetPlayer()->m_Controlled.size() == 1)
        ...
        for (Unit::ControlSet::iterator itr = GetPlayer()->m_Controlled.begin();
             itr != GetPlayer()->m_Controlled.end();
             ++itr) {
            // xinef: allow to dissmis dead pets
            if ((*itr)->GetEntry() == pet->GetEntry() &&
                ((*itr)->IsAlive() ||
                 (flag == ACT_COMMAND && spellId == COMMAND_ABANDON)))
                controlled.push_back(*itr);
            // xinef: mirror image blizzard
            else if ((*itr)->GetEntry() == NPC_MIRROR_IMAGE &&
                     flag == ACT_COMMAND && spellId == COMMAND_FOLLOW) {
...
{
    Unit* pet =
        ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, packet.PetGUID);

    if (!pet) {
        LOG_ERROR("network.opcode",
                  "HandlePetStopAttack: Pet {} does not exist",
    ...
    if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharm()) {
        LOG_ERROR("network.opcode",
                  "HandlePetStopAttack: Pet {} isn't a pet or charmed creature "
                  "of player {}",
                  packet.PetGUID.ToString(),
                  GetPlayer()->GetName());
        return;
...
void WorldSession::HandlePetActionHelper(
    Unit* pet, ObjectGuid guid1, uint32 spellId, uint16 flag, ObjectGuid guid2)
{
    CharmInfo* charmInfo = pet->GetCharmInfo();
    if (!charmInfo) {
        LOG_ERROR("network.opcode",
                  "WorldSession::HandlePetAction(petGuid: {}, tagGuid: {}, "
                  "spellId: {}, flag: {}): object ({}) is considered pet-like "
    ...
    case ACT_COMMAND: // 0x07
        ...
        case COMMAND_STAY: // flat=1792  //STAY
        {
            bool controlledMotion =
                pet->GetMotionMaster()->GetMotionSlotType(
        ...
        case COMMAND_FOLLOW: // spellId=1792  //FOLLOW
        {
            ...
            pet->GetMotionMaster()->MoveFollow(
                _player, PET_FOLLOW_DIST, pet->GetFollowAngle());
            if (pet->ToPet())
                pet->ToPet()->ClearCastWhenWillAvailable();
        ...
        case COMMAND_ATTACK: // spellId=1792  //ATTACK
        {
            ...
            bool checkLos = !DisableMgr::IsPathfindingEnabled(pet->GetMap()) ||
                            (TargetUnit->GetTypeId() == TYPEID_UNIT &&
                             (TargetUnit->ToCreature()->isWorldBoss() ||
                              TargetUnit->ToCreature()->IsDungeonBoss()));
            ...
            if (pet->GetVictim() != TargetUnit ||
                (pet->GetVictim() == TargetUnit &&
                 !pet->GetCharmInfo()->IsCommandAttack())) {
                pet->AttackStop();

                if (pet->GetTypeId() != TYPEID_PLAYER &&
                    pet->ToCreature()->IsAIEnabled) {
                    ...
                    if (pet->IsPet() &&
                        ((Pet*)pet)->getPetType() == SUMMON_PET &&
                        pet != TargetUnit && urand(0, 100) < 10)
                        pet->SendPetTalk((uint32)PET_TALK_ATTACK);
                    else {
                        // 90% chance for pet and 100% chance for charmed
                        // creature
    ...
    case ACT_ENABLED:  // 0xC1    spell
    {
        ...
        if (!spellInfo) {
            LOG_ERROR(
                "network.opcode", "WORLD: unknown PET spell id {}", spellId);
            return;
        }
        ...
        if (!pet->HasSpell(spellId)) {
            ...
            // allow casting of spells triggered by clientside periodic trigger
            // auras
            if (pet->HasAuraTypeWithTriggerSpell(
                    SPELL_AURA_PERIODIC_TRIGGER_SPELL_FROM_CLIENT, spellId)) {
                allow            = true;
        ...
        if (result == SPELL_CAST_OK) {
            ...
            // 10% chance to play special pet attack talk, else growl
            // actually this only seems to happen on special spells, fire shield
            // for imp, torment for voidwalker, but it's stupid to check every
            // spell
            if (pet->IsPet() && (((Pet*)pet)->getPetType() == SUMMON_PET) &&
                (pet != unit_target) && (urand(0, 100) < 10))
        ...
        else if (pet->ToPet() && (result == SPELL_FAILED_LINE_OF_SIGHT ||
            ...
            else if (GetPlayer()->IsFriendlyTo(unit_target) &&
                     !haspositiveeffect)
            ...
            if (!GetPlayer()->IsFriendlyTo(unit_target)) {
                ...
                if (pet->GetVictim() != TargetUnit ||
                    ...
                    if (pet->GetTypeId() != TYPEID_PLAYER &&
                        pet->ToCreature() && pet->ToCreature()->IsAIEnabled) {
                        charmInfo->SetIsCommandAttack(true);
                        charmInfo->SetIsAtStay(false);
                        charmInfo->SetIsFollowing(false);
                        charmInfo->SetIsCommandFollow(false);
                    ...
                    pet->ToPet()->CastWhenWillAvailable(spellId,
                                                        unit_target,
                                                        ObjectGuid::Empty,
                                                        tempspellIsPositive);
                }
                ...
                if (pet->GetTypeId() != TYPEID_PLAYER && pet->ToCreature() &&
                    pet->ToCreature()->IsAIEnabled) {
                    pet->StopMoving();
                    ...
                    pet->GetMotionMaster()->MoveFollow(
                        unit_target, PET_FOLLOW_DIST, rand_norm() * 2 * M_PI);
                    ...
                    pet->ToPet()->CastWhenWillAvailable(
                        spellId, unit_target, oldTarget, tmpSpellIsPositive);
                }
            }
        }
        else {
            ...
            // reset specific flags in case of spell fail. AI will reset other
            // flags
...
{
    Creature* pet =
        ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, petguid);
    if (!pet) {
...
bool WorldSession::CheckStableMaster(ObjectGuid guid)
{
    ...
    if (guid == GetPlayer()->GetGUID()) {
        if (!GetPlayer()->IsGameMaster() &&
            !GetPlayer()->HasAuraType(SPELL_AURA_OPEN_STABLE)) {
            LOG_DEBUG("network.opcode",
                      "Player ({}) attempt open stable in cheating way.",
                      guid.ToString());
            return false;
        }
    }
    // stable master case
        ...
        if (!GetPlayer()->GetNPCIfCanInteractWith(guid,
                                                  UNIT_NPC_FLAG_STABLEMASTER)) {
            LOG_DEBUG(
                "network.opcode",
                "Stablemaster ({}) not found or you can't interact with him.",
                guid.ToString());
            return false;
        }
    }
...
void WorldSession::HandlePetSetAction(WorldPacket& recvData)
{
    ...
    if (!checkPet || checkPet != _player->GetFirstControlled()) {
        LOG_ERROR("network.opcode",
                  "HandlePetSetAction: Unknown pet ({}) or pet owner ({})",
                  petguid.ToString(),
    ...
    for (uint8 i = 0; i < count; ++i) {
        ...
        // not removed at moving count == 2, at removing count == 1 ignore
        // attempt to remove command|reaction buttons (not possible at normal
        // case)
    ...
    // Xinef: loop all pets with same entry (fixes partial state change for
    // feral spirits)
    for (Unit::ControlSet::const_iterator itr = petsSet.begin();
         itr != petsSet.end();
         ++itr) {
        Unit* pet = *itr;
        if (checkPet->GetEntry() == petguid.GetEntry() &&
            pet->GetEntry() != petguid.GetEntry())
            continue;
        ...
        if (!charmInfo) {
            LOG_ERROR(
                "network.opcode",
                "WorldSession::HandlePetSetAction: object ({} TypeId: {}) is "
        ...
        // check swap (at command->spell swap client remove spell first in
        // another packet, so check only command move correctness)
        if (move_command) {
            ...
            if (act_state_0 == ACT_COMMAND || act_state_0 == ACT_REACTION) {
                ...
                if (!actionEntry_1 ||
                    spell_id_0 != actionEntry_1->GetAction() ||
            ...
            if (act_state_1 == ACT_COMMAND || act_state_1 == ACT_REACTION) {
                ...
                if (!actionEntry_0 ||
                    spell_id_1 != actionEntry_0->GetAction() ||
        ...
        for (uint8 i = 0; i < count; ++i) {
            ...
            // if it's act for spell (en/disable/cast) and there is a spell
            // given (0 = remove spell) which pet doesn't know, don't add
            if (!((act_state == ACT_ENABLED || act_state == ACT_DISABLED ||
                ...
                if (SpellInfo const* spellInfo =
                        sSpellMgr->GetSpellInfo(spell_id)) {
                    // sign for autocast
                    if (act_state == ACT_ENABLED) {
                        if (pet->GetTypeId() == TYPEID_UNIT && pet->IsPet()) {
                            ...
                            for (auto iterator =
                                 ...
                                 iterator != GetPlayer()->m_Controlled.end();
                                 ++iterator) {
                                if ((*iterator)->GetEntry() ==
                                    pet->GetEntry()) {
                                    (*iterator)
                                        ->GetCharmInfo()
                                        ->ToggleCreatureAutocast(spellInfo,
                                                                 true);
                                }
                            }
                        }
...
void WorldSession::HandlePetRename(WorldPacket& recvData)
{
    ...
    if (!pet || !pet->IsPet() || ((Pet*)pet)->getPetType() != HUNTER_PET ||
        ...
        petStable->CurrentPet->PetNumber !=
    ...
    if (isdeclined) {
        ...
        if (!ObjectMgr::CheckDeclinedNames(wname, declinedname)) {
            SendPetNameInvalid(PET_NAME_DECLENSION_DOESNT_MATCH_BASE_NAME,
                               name,
                               &declinedname);
            return;
        }
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_PET_NAME);
    stmt->SetData(0, name);
    ...
    pet->SetUInt32Value(
        ...
        uint32(GameTime::GetGameTime().count())); // cast can't be helped
}
    ...
    if (pet && pet->ToPet() && pet->ToPet()->getPetType() == HUNTER_PET) {
        if (pet->IsPet()) {
            if (pet->GetGUID() == _player->GetPetGUID()) {
                ...
                pet->SetPower(POWER_HAPPINESS,
                              feelty > 50000 ? (feelty - 50000) : 0);
            }
...
void WorldSession::HandlePetSpellAutocastOpcode(
    WorldPackets::Pet::PetSpellAutocast& packet)
{
    Creature* checkPet =
        ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, packet.PetGUID);
    if (!checkPet) {
        LOG_ERROR(
            "entities.pet",
            "WorldSession::HandlePetSpellAutocastOpcode: Pet {} not found.",
            packet.PetGUID.ToString());
        return;
    }
    ...
    if (!spellInfo) {
        LOG_ERROR("spells.pet",
                  "WorldSession::HandlePetSpellAutocastOpcode: Unknown spell "
                  "id {} used by {}.",
                  packet.SpellID,
                  packet.PetGUID.ToString());
        return;
    ...
    if (checkPet != _player->GetGuardianPet() &&
        checkPet != _player->GetCharm()) {
        LOG_ERROR("entities.pet",
                  "WorldSession::HandlePetSpellAutocastOpcode: {} isn't pet of "
                  "player {} ({}).",
                  packet.PetGUID.ToString(),
                  GetPlayer()->GetName(),
                  GetPlayer()->GetGUID().ToString());
    ...
    for (Unit* pet : petsSet) {
        if (checkPet->GetEntry() == packet.PetGUID.GetEntry() &&
            pet->GetEntry() != packet.PetGUID.GetEntry())
            continue;
        ...
        if (!charmInfo) {
            LOG_ERROR("network.opcode",
                      "WorldSession::HandlePetSpellAutocastOpcode: object ({} "
                      "TypeId: {}) is considered pet-like but doesn't have a "
                      "charminfo!",
                      pet->GetGUID().ToString(),
                      pet->GetTypeId());
            continue;
        ...
        if (Pet* summon = pet->ToPet())
            ...
            charmInfo->ToggleCreatureAutocast(spellInfo,
                                              packet.AutocastEnabled);
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("network",
              "WORLD: CMSG_PET_CAST_SPELL, guid: {}, castCount: {}, spellId "
              "{}, castFlags {}",
              guid.ToString(),
              castCount,
              spellId,
    ...
    // This opcode is also sent from charmed and possessed units (players and
    // creatures)
    ...
    if (!caster || (caster != _player->GetGuardianPet() &&
                    caster != _player->GetCharm())) {
        LOG_ERROR("network.opcode",
                  "HandlePetCastSpellOpcode: Pet {} isn't pet of player {} .",
                  guid.ToString(),
                  GetPlayer()->GetName());
        return;
    }
    ...
    if (result == SPELL_CAST_OK) {
        if (Creature* creature = caster->ToCreature()) {
            ...
            if (Pet* pet = creature->ToPet()) {
                // 10% chance to play special pet attack talk, else growl
                // actually this only seems to happen on special spells, fire
                // shield for imp, torment for voidwalker, but it's stupid to
                // check every spell
        ...
        if (!caster->GetCharmInfo() ||
            !caster->GetCharmInfo()->GetForcedSpell())
            spell->SendPetCastResult(result);
        ...
        if (caster->GetTypeId() == TYPEID_PLAYER) {
            ...
            if (caster->IsPet())
                caster->PetSpellFail(
                    spellInfo, targets.GetUnitTarget(), result);
        }
