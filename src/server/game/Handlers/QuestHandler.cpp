...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket& recvData)
{
    ...
    // Did we already get a gossip menu with that NPC? if so no need to status
    // query
    if (gossipMenu.GetSenderGUID() == guid) {
        return;
    }

    Object* questGiver = ObjectAccessor::GetObjectByTypeMask(
        *_player, guid, TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT);
    if (!questGiver) {
    ...
    case TYPEID_UNIT: {
        LOG_DEBUG("network",
                  "WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for npc {}",
                  guid.ToString());
        if (!questGiver->ToCreature()->IsHostileTo(
                _player)) // do not show quest status to enemies
    ...
    case TYPEID_GAMEOBJECT: {
        LOG_DEBUG(
            "network",
            "WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for GameObject {}",
            guid.ToString());
    ...
    default:
        LOG_ERROR("network.opcode",
                  "QuestGiver called for unexpected type {}",
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_QUESTGIVER_HELLO npc {}",
              guid.ToString());
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandleQuestgiverHelloOpcode - Unit ({}) not found or "
    ...
    if (uint32 pause =
            creature->GetMovementTemplate().GetInteractionPauseTimer())
    ...
    _player->PrepareGossipMenu(
        creature, creature->GetCreatureTemplate()->GossipMenuId, true);
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST npc {}, quest = "
              "{}, unk1 = {}",
              guid.ToString(),
              questId,
              unk1);
    ...
    if (!object || object == _player ||
        (object->GetTypeId() != TYPEID_PLAYER && !object->hasQuest(questId)) ||
        (object->GetTypeId() == TYPEID_PLAYER &&
         !object->ToPlayer()->CanShareQuest(questId))) {
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId)) {
        // pussywizard: exploit fix, can't share quests that give items to be
        // sold
        if (object->GetTypeId() == TYPEID_PLAYER)
            if (uint32 itemId = quest->GetSrcItemId())
                if (ItemTemplate const* srcItem =
        ...
        if (_player->GetDivider()) {
            Player* player =
                ObjectAccessor::GetPlayer(*_player, _player->GetDivider());
            if (player) {
                player->SendPushToPartyResponse(_player,
            ...
            if (quest->HasFlag(QUEST_FLAGS_PARTY_ACCEPT)) {
                if (Group* group = _player->GetGroup()) {
                    for (GroupReference* itr = group->GetFirstMember();
                        ...
                        if (!itrPlayer || itrPlayer == _player ||
                            !itrPlayer->IsAtGroupRewardDistance(_player) ||
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    recvData >> guid >> questId >> unk1;
    LOG_DEBUG("network",
              "WORLD: Received CMSG_QUESTGIVER_QUERY_QUEST npc {}, quest = {}, "
              "unk1 = {}",
              guid.ToString(),
              questId,
              unk1);
    ...
    // requested quest
    Object* object = ObjectAccessor::GetObjectByTypeMask(
        *_player, guid, TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT | TYPEMASK_ITEM);
    if (!object ||
        (!object->hasQuest(questId) && !object->hasInvolvedQuest(questId))) {
        _player->PlayerTalkClass->SendCloseGossip();
        return;
    }

    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId)) {
        // not sure here what should happen to quests with
        // QUEST_FLAGS_AUTOCOMPLETE if this breaks them, add &&
        // object->GetTypeId() == TYPEID_ITEM to this check item-started quests
        // never have that flag
...
void WorldSession::HandleQuestQueryOpcode(WorldPacket& recvData)
{
    ...
    recvData >> questId;
    LOG_DEBUG(
        "network", "WORLD: Received CMSG_QUEST_QUERY quest = {}", questId);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& recvData)
{
    ...
    if (reward >= QUEST_REWARD_CHOICES_COUNT) {
        LOG_ERROR("network.opcode",
                  "Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player {} ({}) "
                  "tried to get invalid reward ({}) (probably packet hacking)",
                  _player->GetName(),
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc {}, quest = "
              "{}, reward = {}",
              guid.ToString(),
              questId,
              reward);
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId)) {
        if ((!_player->CanSeeStartQuest(quest) &&
             _player->GetQuestStatus(questId) == QUEST_STATUS_NONE) ||
            (_player->GetQuestStatus(questId) != QUEST_STATUS_COMPLETE &&
             !quest->IsAutoComplete() && quest->GetQuestMethod())) {
            LOG_ERROR("network.opcode",
                      "HACK ALERT: Player {} ({}) is trying to complete quest "
                      "(id: {}) but he has no right to do it!",
                      _player->GetName(),
                      _player->GetGUID().ToString(),
                      questId);
            return;
        ...
        if (_player->CanRewardQuest(quest, reward, true)) {
            ...
            if (!quest->GetQuestMethod()) {
                _player->PlayerTalkClass->SendQuestGiverStatus(
                    uint8(_player->GetQuestDialogStatus(object)), guid);
            }

            switch (object->GetTypeId()) {
            case TYPEID_UNIT: {
                Creature* questgiver = object->ToCreature();
                if (!sScriptMgr->OnQuestReward(
                        _player, questgiver, quest, reward)) {
                    // Send next quest
                    if (Quest const* nextQuest =
                            _player->GetNextQuest(guid, quest)) {
                        if (_player->CanTakeQuest(nextQuest, false)) {
                            if (nextQuest->IsAutoAccept()) {
                                // QUEST_FLAGS_AUTO_ACCEPT was not used by
                                // Blizzard.
                                if (_player->CanAddQuest(nextQuest, false)) {
                                    ...
                                    // Auto accept is set for a custom quest and
                                    // there is no inventory space
                                    _player->PlayerTalkClass->SendCloseGossip();
                                    break;
                                }
                            }
                            _player->PlayerTalkClass
            ...
            case TYPEID_GAMEOBJECT: {
                ...
                if (!sScriptMgr->OnQuestReward(
                    ...
                    if (Quest const* nextQuest =
                            _player->GetNextQuest(guid, quest)) {
                        if (_player->CanAddQuest(nextQuest, false) &&
                            _player->CanTakeQuest(nextQuest, false)) {
                            if (nextQuest->IsAutoAccept())
                                _player->AddQuestAndCheckCompletion(nextQuest,
                                                                    object);
                            _player->PlayerTalkClass
                                ->SendQuestGiverQuestDetails(
                                    nextQuest, guid, true);
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG(
        "network",
        "WORLD: Received CMSG_QUESTGIVER_REQUEST_REWARD npc {}, quest = {}",
        guid.ToString(),
        questId);
...
void WorldSession::HandleQuestLogSwapQuest(WorldPacket& recvData)
{
    ...
    LOG_DEBUG(
        "network",
        "WORLD: Received CMSG_QUESTLOG_SWAP_QUEST slot 1 = {}, slot 2 = {}",
        slot1,
        slot2);
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_QUESTLOG_REMOVE_QUEST slot = {}",
              slot);
        ...
        if (uint32 questId = _player->GetQuestSlotQuestId(slot)) {
            ...
            if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId)) {
                ...
                if (quest->HasFlag(QUEST_FLAGS_FLAGS_PVP)) {
                    _player->pvpInfo.IsHostile =
                        _player->pvpInfo.IsInHostileArea ||
                        _player->HasPvPForcingQuest();
                    _player->UpdatePvPState();
                }
            }

            _player->TakeQuestSourceItem(
                questId, true);             // remove quest src item from player
            _player->AbandonQuest(questId); // remove all quest items player
                                            // received before abandoning quest.
            _player->RemoveActiveQuest(questId);
            _player->RemoveTimedAchievement(ACHIEVEMENT_TIMED_TYPE_QUEST,
                                            questId);
...
void WorldSession::HandleQuestConfirmAccept(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Received CMSG_QUEST_CONFIRM_ACCEPT quest = {}",
              questId);
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId)) {
        ...
        if (!_player->IsInSameRaidWith(originalPlayer) ||
            !_player->IsAtGroupRewardDistance(originalPlayer))
            return;
        ...
        if (_player->CanAddQuest(quest, true))
            _player->AddQuestAndCheckCompletion(
                quest, nullptr); // nullptr, this prevent DB script from
                                 // duplicate running
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recvData)
{
    ...
    LOG_DEBUG(
        "network",
        "WORLD: Received CMSG_QUESTGIVER_COMPLETE_QUEST npc {}, quest = {}",
        guid.ToString(),
        questId);
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId)) {
        if (!_player->CanSeeStartQuest(quest) &&
            ...
            LOG_ERROR(
                "network.opcode",
                "Possible hacking attempt: Player {} [{}] tried to complete "
                "quest [entry: {}] without being in possession of the quest!",
                _player->GetName(),
                _player->GetGUID().ToString(),
                questId);
            return;
        ...
        if (_player->GetQuestStatus(questId) != QUEST_STATUS_COMPLETE) {
            if (quest->IsRepeatable())
                _player->PlayerTalkClass->SendQuestGiverRequestItems(
                    quest,
                    guid,
                    _player->CanCompleteRepeatableQuest(quest),
                    false);
            else
                _player->PlayerTalkClass->SendQuestGiverRequestItems(
...
void WorldSession::HandlePushQuestToParty(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "network", "WORLD: Received CMSG_PUSHQUESTTOPARTY quest = {}", questId);
        ...
        if (Group* group = _player->GetGroup()) {
            for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
                ...
                if (!player || player == _player ||
                    !player->IsInMap(_player)) // skip self
                    continue;

                if (!player->SatisfyQuestStatus(quest, false)) {
                    _player->SendPushToPartyResponse(
                        player, QUEST_PARTY_MSG_HAVE_QUEST);
                    continue;
                }

                if (player->GetQuestStatus(questId) == QUEST_STATUS_COMPLETE) {
                    _player->SendPushToPartyResponse(
                        player, QUEST_PARTY_MSG_FINISH_QUEST);
                    continue;
                }

                if (!player->CanTakeQuest(quest, false)) {
                    _player->SendPushToPartyResponse(
                        player, QUEST_PARTY_MSG_CANT_TAKE_QUEST);
                    continue;
                }

                if (!player->SatisfyQuestLog(false)) {
                    _player->SendPushToPartyResponse(player,
                ...
                if (quest->IsAutoAccept() && player->CanAddQuest(quest, true) &&
                    player->CanTakeQuest(quest, true))
                ...
                if (quest->IsAutoComplete() || !quest->GetQuestMethod())
                    player->PlayerTalkClass->SendQuestGiverRequestItems(
                        quest,
                        _player->GetGUID(),
                        player->CanCompleteRepeatableQuest(quest),
                        true);
                else {
