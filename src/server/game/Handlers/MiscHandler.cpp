...
void WorldSession::HandleRepopRequestOpcode(WorldPacket& recv_data)
{
    ...
    if (GetPlayer()->IsAlive() ||
        GetPlayer()->HasPlayerFlag(PLAYER_FLAGS_GHOST))
        return;
    ...
    if (GetPlayer()->getDeathState() == DeathState::JustDied) {
        LOG_DEBUG("network",
                  "HandleRepopRequestOpcode: got request after player {} ({}) "
...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recv_data)
{
    ...
    if (guid.IsCreatureOrVehicle()) {
        ...
        if (!unit) {
            LOG_DEBUG("network",
                      "WORLD: HandleGossipSelectOptionOpcode - Unit ({}) not "
        ...
        if (!go) {
            LOG_DEBUG("network",
                      "WORLD: HandleGossipSelectOptionOpcode - GameObject ({}) "
                      "not found.",
                      guid.ToString());
            return;
        }
        ...
        if (!item || _player->IsBankPos(item->GetPos())) {
            // LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - {}
            // not found.", guid.ToString());
            return;
        }
    }
    else if (guid.IsPlayer()) {
        if (guid != _player->GetGUID() ||
            menuId != _player->PlayerTalkClass->GetGossipMenu().GetMenuId()) {
            // LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - {}
            // not found.", guid.ToString());
            return;
        }
    ...
    if ((unit && unit->GetScriptId() != unit->LastUsedScriptID) ||
        ...
        LOG_DEBUG("network",
                  "WORLD: HandleGossipSelectOptionOpcode - Script reloaded "
                  "while in use, ignoring and set new scipt id");
        if (unit)
    ...
    if (!code.empty()) {
        if (unit) {
            unit->AI()->sGossipSelectCode(
                _player, menuId, gossipListId, code.c_str());
            if (!sScriptMgr->OnGossipSelectCode(
                    _player,
                    unit,
                    _player->PlayerTalkClass->GetGossipOptionSender(
                        gossipListId),
                    _player->PlayerTalkClass->GetGossipOptionAction(
                        gossipListId),
                    code.c_str()))
                _player->OnGossipSelect(unit, gossipListId, menuId);
        }
        else if (go) {
            go->AI()->GossipSelectCode(
                _player, menuId, gossipListId, code.c_str());
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    recvData >> strCount; // user entered strings count, client limit=4 (checked
                          // on 2.0.10)

    if (strCount > 4)
        return; // can't be received from real client or broken packet

    LOG_DEBUG("network.who",
              "Minlvl {}, maxlvl {}, name {}, guild {}, racemask {}, classmask "
              "{}, zones {}, strings {}",
              levelMin,
              levelMax,
              packetPlayerName,
    ...
    for (uint32 i = 0; i < strCount; ++i) {
        std::string temp;
        recvData >> temp; // user entered string, it used as universal search
                          // pattern(guild+player name)?
    ...
    if (!(Utf8toWStr(packetPlayerName, wpacketPlayerName) &&
    ...
    // client send in case not set max level value 100 but Acore supports 255
    // max level, update it to show GMs with characters after 100 level
    ...
    data << uint32(
        matchCount); // placeholder, count of players matching criteria
    ...
    for (auto const& target : sWhoListCacheMgr->GetWhoList()) {
        ...
        if ((_player->GetGUID() != target.GetGuid() && !target.IsVisible()) &&
            (AccountMgr::IsPlayerAccount(
                 _player->GetSession()->GetSecurity()) ||
             target.GetSecurity() > _player->GetSession()->GetSecurity())) {
            continue;
        }
        ...
        if (!(wpacketPlayerName.empty() ||
              wideplayername.find(wpacketPlayerName) != std::wstring::npos)) {
            continue;
        }

        std::wstring const& wideguildname = target.GetWideGuildName();
        if (!(wpacketGuildName.empty() ||
              wideguildname.find(wpacketGuildName) != std::wstring::npos)) {
            continue;
        }

        std::string aname;
        if (AreaTableEntry const* areaEntry =
                sAreaTableStore.LookupEntry(playerZoneId)) {
        ...
        if ((matchCount++) >=
            sWorld->getIntConfig(CONFIG_MAX_WHO_LIST_RETURN)) {
            continue;
        }
...
void WorldSession::HandleLogoutRequestOpcode(
    WorldPackets::Character::LogoutRequest& /*logoutRequest*/)
{
    LOG_DEBUG("network",
              "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - {}",
              GetSecurity());

    if (ObjectGuid lguid = GetPlayer()->GetLootGUID())
        DoLootRelease(lguid);

    bool instantLogout = ((GetSecurity() >= 0 &&
                           uint32(GetSecurity()) >=
                               sWorld->getIntConfig(CONFIG_INSTANT_LOGOUT)) ||
                          (GetPlayer()->HasPlayerFlag(PLAYER_FLAGS_RESTING) &&
                           !GetPlayer()->IsInCombat())) ||
                         GetPlayer()->IsInFlight();

    bool preventAfkSanctuaryLogout =
        sWorld->getIntConfig(CONFIG_AFK_PREVENT_LOGOUT) == 1 &&
        GetPlayer()->isAFK() &&
        sAreaTableStore.LookupEntry(GetPlayer()->GetAreaId())->IsSanctuary();

    bool preventAfkLogout =
        sWorld->getIntConfig(CONFIG_AFK_PREVENT_LOGOUT) == 2 &&
        GetPlayer()->isAFK();
    ...
    if (GetPlayer()->IsInCombat() && !canLogoutInCombat)
        reason = 1;
    else if (GetPlayer()->m_movementInfo.HasMovementFlag(
                 MOVEMENTFLAG_FALLING | MOVEMENTFLAG_FALLING_FAR))
        reason = 3; // is jumping or falling
    else if (preventAfkSanctuaryLogout || preventAfkLogout ||
             GetPlayer()->duel ||
             GetPlayer()->HasAura(
                 9454)) // is dueling or frozen by GM via freeze command
    ...
    // its enabled in worldserver.conf
    if (instantLogout) {
        LogoutPlayer(true);
        return;
    }
...
void WorldSession::HandlePlayerLogoutOpcode(
    WorldPackets::Character::PlayerLogout& /*playerLogout*/)
{
}

void WorldSession::HandleLogoutCancelOpcode(
    WorldPackets::Character::LogoutCancel& /*logoutCancel*/)
{
...
void WorldSession::HandleTogglePvP(WorldPacket& recv_data)
{
    ...
    else if (!GetPlayer()->pvpInfo.IsHostile &&
             GetPlayer()
                 ->IsPvP()) // pussywizard: in pvp mode, but doesn't want to be
                            // and not in hostile territory, so start timer
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket& recv_data)
{
    ...
    // use server side data, but only after update the player position. See
        ...
        if (Spell* autoReapeatSpell =
            ...
            if (autoReapeatSpell->m_targets.GetUnitTargetGUID() != guid) {
                if (Unit* unit = ObjectAccessor::GetUnit(*_player, guid)) {
                    if (unit->IsAlive() && !_player->IsFriendlyTo(unit) &&
                        unit->isTargetableForAttack(true, _player)) {
...
void WorldSession::HandleBugOpcode(WorldPacket& recv_data)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_BUG_REPORT);
...
void WorldSession::HandleReclaimCorpseOpcode(WorldPacket& recv_data)
{
    ...
    if (time_t(corpse->GetGhostTime() +
               _player->GetCorpseReclaimDelay(corpse->GetType() ==
...
void WorldSession::HandleResurrectResponseOpcode(WorldPacket& recv_data)
{
    ...
    if (GetPlayer()->IsAlive() ||
        GetPlayer()->HasAuraType(SPELL_AURA_PREVENT_RESURRECTION))
        return;
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
    ...
    if (player->IsInFlight()) {
        LOG_DEBUG("network",
                  "HandleAreaTriggerOpcode: Player '{}' ({}) in flight, ignore "
                  "Area Trigger ID:{}",
                  player->GetName(),
    ...
    if (!atEntry) {
        LOG_DEBUG("network",
                  "HandleAreaTriggerOpcode: Player '{}' ({}) send unknown (by "
                  "DBC) Area Trigger ID:{}",
                  player->GetName(),
                  player->GetGUID().ToString(),
                  triggerId);
    ...
    uint32 teamFaction = player->GetTeamId(true) == TEAM_ALLIANCE
                             ? FACTION_MASK_ALLIANCE
                             : FACTION_MASK_HORDE;
    bool   isTavernAreatrigger =
        sObjectMgr->IsTavernAreaTrigger(triggerId, teamFaction);
    if (!player->IsInAreaTriggerRadius(atEntry,
                                       isTavernAreatrigger ? 5.f : 0.f)) {
        LOG_DEBUG("network",
                  "HandleAreaTriggerOpcode: Player {} ({}) too far (trigger "
                  "map: {} player map: {}), ignore Area Trigger ID: {}",
                  player->GetName(),
                  player->GetGUID().ToString(),
                  atEntry->map,
                  player->GetMapId(),
    ...
    if (player->isDebugAreaTriggers)
        ChatHandler(this).PSendSysMessage(LANG_DEBUG_AREATRIGGER_REACHED,
                                          triggerId);
    ...
    AreaTriggerTeleport const* at =
        sObjectMgr->GetAreaTriggerTeleport(triggerId);
    if (!at)
        return;
    ...
    if (player->GetMapId() != at->target_mapId) {
        if (Map::EnterState denyReason =
                sMapMgr->PlayerCannotEnter(at->target_mapId, player, false)) {
            bool reviveAtTrigger =
                false; // should we revive the player if he is trying to enter
            ...
            if (reviveAtTrigger) // check if the player is touching the
                                 // areatrigger leading to the map his corpse is
                                 // on
            {
                if (!player->IsAlive() && player->HasCorpse()) {
                    if (player->GetCorpseLocation().GetMapId() ==
                        at->target_mapId) {
...
void WorldSession::HandleUpdateAccountData(WorldPacket& recv_data)
{
    ...
    LOG_DEBUG("network",
              "UAD: type {}, time {}, decompressedSize {}",
              type,
              timestamp,
              decompressedSize);
    ...
    if (decompressedSize > 0xFFFF) {
        ...
        LOG_ERROR("network.opcode",
                  "UAD: Account data packet too big, size {}",
                  decompressedSize);
        return;
    }
    ...
    if (uncompress(dest.contents(),
                   &realSize,
                   recv_data.contents() + recv_data.rpos(),
                   recv_data.size() - recv_data.rpos()) != Z_OK) {
        recv_data.rfinish(); // unnneded warning spam in this case
    ...
    recv_data
        .rfinish(); // uncompress read (recv_data.size() - recv_data.rpos())
...
void WorldSession::HandleRequestAccountData(WorldPacket& recv_data)
{
    ...
    if (size && compress(dest.contents(),
                         &destSize,
                         (uint8 const*)adata->Data.c_str(),
                         size) != Z_OK) {
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recv_data)
{
    ...
    if (!packetData) {
        LOG_DEBUG(
            "network.opcode", "MISC: Remove action from button {}", button);
        ...
        case ACTION_BUTTON_CMACRO:
            LOG_DEBUG("network.opcode",
                      "MISC: Added Macro {} into button {}",
                      action,
                      button);
            break;
        case ACTION_BUTTON_EQSET:
            LOG_DEBUG("network.opcode",
                      "MISC: Added EquipmentSet {} into button {}",
                      action,
                      button);
            break;
        case ACTION_BUTTON_SPELL:
            LOG_DEBUG("network.opcode",
                      "MISC: Added Spell {} into button {}",
                      action,
                      button);
            break;
        case ACTION_BUTTON_ITEM:
            LOG_DEBUG("network.opcode",
                      "MISC: Added Item {} into button {}",
                      action,
                      button);
            break;
        default:
            LOG_ERROR("network.opcode",
                      "MISC: Unknown action button type {} for action {} into "
                      "button {} for player {} ({})",
                      type,
                      action,
                      button,
...
{
    // Sent by client when cinematic actually begun. So we begin the server side
    // process
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recv_data)
{
                      ...
                      // STATUS_AUTHED)
    {
        if (ActionBar != 0)
            LOG_ERROR("network.opcode",
                      "WorldSession::HandleSetActionBarToggles in not logged "
                      "state with value: {}, ignored",
                      uint32(ActionBar));
        return;
    }
...
void WorldSession::HandlePlayedTime(
    WorldPackets::Character::PlayedTimeClient& packet)
{
    ...
    playedTime.TriggerScriptEvent =
        packet.TriggerScriptEvent; // 0-1 - will not show in chat frame
...
void WorldSession::HandleInspectOpcode(WorldPacket& recv_data)
{
    ...
    if (!player) {
        LOG_DEBUG("network",
                  "CMSG_INSPECT: No player found from {}",
                  guid.ToString());
        return;
    }
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recv_data)
{
    ...
    if (!player) {
        LOG_DEBUG("network",
                  "MSG_INSPECT_HONOR_STATS: No player found from {}",
                  guid.ToString());
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recv_data)
{
    ...
    LOG_DEBUG("network",
              "CMSG_WORLD_TELEPORT: Player = {}, Time = {}, map = {}, x = {}, "
              "y = {}, z = {}, o = {}",
              GetPlayer()->GetName(),
              time,
              mapid,
...
void WorldSession::HandleWhoisOpcode(WorldPacket& recv_data)
{
    ...
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_WHOIS);
    ...
    std::string msg = charname + "'s " + "account is " + acc +
                      ", e-mail: " + email + ", last ip: " + lastip;
...
void WorldSession::HandleComplainOpcode(WorldPacket& recv_data)
{
    ...
    // reporter until logout in case chat spam. if it's mail spam - ALL mails
    // from this spammer automatically removed by client
    ...
    LOG_DEBUG("network",
              "REPORT SPAM: type {}, {}, unk1 {}, unk2 {}, unk3 {}, unk4 {}, "
              "message {}",
              spam_type,
              spammer_guid.ToString(),
              unk1,
              unk2,
              unk3,
              unk4,
...
void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
    ...
    if (apply) {
        LOG_DEBUG("network",
                  "Added FarSight {} to player {}",
                  _player->GetGuidValue(PLAYER_FARSIGHT).ToString(),
                  _player->GetGUID().ToString());
        if (WorldObject* target = _player->GetViewpoint())
            _player->SetSeer(target);
        else
            LOG_DEBUG("network.opcode",
                      "Player {} requests non-existing seer {}",
                      _player->GetName(),
                      _player->GetGuidValue(PLAYER_FARSIGHT).ToString());
    }
    else {
        ...
        if (WorldObject* viewpoint = _player->GetViewpoint()) {
            if (DynamicObject* viewpointDynamicObject =
                    viewpoint->ToDynObject()) {
                newFarsightobject = ObjectAccessor::GetUnit(
                    *viewpointDynamicObject,
                    viewpointDynamicObject->GetOldFarsightGUID());
            }
            else if (DynamicObject* viewpointDynamicObject =
                         _player->GetDynObject(
                             _player->GetUInt32Value(UNIT_CHANNEL_SPELL))) {
                if (viewpointDynamicObject->IsViewpoint() &&
                    viewpointDynamicObject->GetCasterGUID() ==
                        _player->GetGUID()) {
        ...
        if (newFarsightobject) {
            LOG_DEBUG("network",
                      "Player {} set vision to old farsight {}",
                      _player->GetGUID().ToString(),
                      newFarsightobject->GetGUID().ToString());
            _player->SetViewpoint(_player->GetViewpoint(), false);
            _player->SetViewpoint(newFarsightobject, true);
        }
        else {
            LOG_DEBUG("network",
                      "Player {} set vision to self",
                      _player->GetGUID().ToString());
    ...
    if (group) {
        if (group->IsLeader(_player->GetGUID())) {
            for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
                ...
                if (groupGuy->GetGUID() == _player->GetGUID()
                        ? groupGuy->GetMap()->IsDungeon()
                        : groupGuy->GetMap()->IsNonRaidDungeon()) {
                    _player->SendDungeonDifficulty(group != nullptr);
                    return;
                }
            }
            ...
            group->ResetInstances(
                INSTANCE_RESET_CHANGE_DIFFICULTY, false, _player);
        ...
        Player::ResetInstances(
            _player->GetGUID(), INSTANCE_RESET_CHANGE_DIFFICULTY, false);
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket& recv_data)
{
    ...
    if (group) {
        if (group->IsLeader(_player->GetGUID())) {
            ...
            if (uint32 preventionTime =
                    group->GetDifficultyChangePreventionTime()) {
                ...
                case DIFFICULTY_PREVENTION_CHANGE_BOSS_KILLED:
                    ChatHandler(this).PSendSysMessage(
                        "Raid was in combat recently and may not change "
                        "difficulty again for %u sec.",
                        preventionTime);
                    break;
                case DIFFICULTY_PREVENTION_CHANGE_RECENTLY_CHANGED:
                default:
                    ChatHandler(this).PSendSysMessage(
                        "Raid difficulty has changed recently, and may not "
                        "change again for %u sec.",
                        preventionTime);
                    break;
                }
            ...
            for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
                ...
                if (IsSharedDifficultyMap(groupGuy->GetMap()->GetId()) &&
                    (uint32(mode % 2) ==
                     uint32(_player->GetRaidDifficulty() % 2)) &&
                    group->isRaidGroup()) {
                    if (!currMap)
                    ...
                    if (!groupGuy->IsAlive() || groupGuy->IsInCombat() ||
                        groupGuy->GetVictim() ||
                        groupGuy->m_mover != groupGuy ||
                        groupGuy->IsNonMeleeSpellCast(true) ||
                        (!groupGuy->GetMotionMaster()->empty() &&
                         groupGuy->GetMotionMaster()
                        ...
                        !groupGuy->Satisfy(
                            sObjectMgr->GetAccessRequirement(
                                groupGuy->GetMap()->GetId(), Difficulty(mode)),
                            groupGuy->GetMap()->GetId(),
                            false)) {
                        _player->SendRaidDifficulty(group != nullptr);
                        return;
                    }
                }
                else if (groupGuy->GetGUID() == _player->GetGUID()
                             ? groupGuy->GetMap()->IsDungeon()
                             : groupGuy->GetMap()->IsRaid()) {
                    _player->SendRaidDifficulty(group != nullptr);
                    return;
                }
            }
            ...
            if (currMap)
                for (Map::PlayerList::const_iterator itr =
                         currMap->GetPlayers().begin();
                     itr != currMap->GetPlayers().end();
                     ++itr)
                    if (Player* p = itr->GetSource())
                        if (p->GetGroup() != group) {
                            if (!p->IsInWorld() || !p->IsAlive() ||
                                p->IsInCombat() || p->GetVictim() ||
                                p->m_mover != p ||
                                p->IsNonMeleeSpellCast(true) ||
                                (!p->GetMotionMaster()->empty() &&
                                 p->GetMotionMaster()
                                         ->GetCurrentMovementGeneratorType() !=
                                     IDLE_MOTION_TYPE) ||
            ...
            for (std::map<Player*, Position>::iterator itr =
                     playerTeleport.begin();
                 itr != playerTeleport.end();
                 ++itr) {
                Player* p      = itr->first;
                Map*    oldMap = p->GetMap();
                ...
                if (!p->TeleportTo(571, 5790.20f, 2071.36f, 636.07f, 3.60f))
                    p->GetSession()->KickPlayer(
                        "HandleSetRaidDifficultyOpcode 1");
            }
            ...
            if (!anyoneInside) // pussywizard: don't reset if changing ICC/RS
                               // difficulty while inside
                group->ResetInstances(
                    INSTANCE_RESET_CHANGE_DIFFICULTY, true, _player);
            ...
            for (std::map<Player*, Position>::iterator itr =
                 ...
                 itr != playerTeleport.end();
                 ++itr) {
                itr->first->SetRaidDifficulty(
                    Difficulty(mode)); // needed for teleport not to fail
                if (!itr->first->TeleportTo(*(foundMaps.begin()),
                                            itr->second.GetPositionX(),
                                            itr->second.GetPositionY(),
                                            itr->second.GetPositionZ(),
                                            itr->second.GetOrientation()))
                    itr->first->GetSession()->KickPlayer(
                        "HandleSetRaidDifficultyOpcode 2");
            }
        }
    }
    else {
        ...
        Player::ResetInstances(
            _player->GetGUID(), INSTANCE_RESET_CHANGE_DIFFICULTY, true);
...
void WorldSession::HandleMoveSetCanFlyAckOpcode(WorldPacket& recv_data)
{
    ...
    sScriptMgr->AnticheatSetCanFlybyServer(
        _player, movementInfo.HasMovementFlag(MOVEMENTFLAG_CAN_FLY));

    _player->m_mover->m_movementInfo.flags = movementInfo.GetMovementFlags();
}

void WorldSession::HandleRequestPetInfo(
    WorldPackets::Pet::RequestPetInfo& /*packet*/)
{
    /*
...
void WorldSession::HandleSetTaxiBenchmarkOpcode(WorldPacket& recv_data)
{
    ...
    mode ? _player->SetPlayerFlag(PLAYER_FLAGS_TAXI_BENCHMARK)
...
void WorldSession::HandleAreaSpiritHealerQueryOpcode(WorldPacket& recv_data)
{
    ...
    if (Battlefield* bf =
            sBattlefieldMgr->GetBattlefieldToZoneId(_player->GetZoneId()))
...
void WorldSession::HandleHearthAndResurrect(WorldPacket& /*recv_data*/)
{
    ...
    AreaTableEntry const* atEntry =
        sAreaTableStore.LookupEntry(_player->GetAreaId());
    ...
    _player->SpawnCorpseBones();
    _player->TeleportTo(_player->m_homebindMapId,
                        _player->m_homebindX,
                        _player->m_homebindY,
                        _player->m_homebindZ,
                        _player->m_homebindO);
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket)
{
    ...
    if (!_player->HasPendingBind() ||
        _player->GetPendingBind() != _player->GetInstanceId() ||
        (_player->GetGroup() && _player->GetGroup()->isLFGGroup() &&
         _player->GetGroup()->IsLfgRandomInstance())) {
        LOG_DEBUG("network.opcode",
                  "InstanceLockResponse: Player {} ({}) tried to bind "
                  "himself/teleport to graveyard without a pending bind!",
                  _player->GetName(),
                  _player->GetGUID().ToString());
        return;
    }
...
void WorldSession::HandleUpdateMissileTrajectory(WorldPacket& recvPacket)
{
    ...
    Spell* spell =
        caster ? caster->GetCurrentSpell(CURRENT_GENERIC_SPELL) : nullptr;
