...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network",
              "Petitioner ({}) tried sell petition: name {}",
              guidNPC.ToString(),
              name);
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandlePetitionBuyOpcode - Unit ({}) not found or you "
    ...
    if (creature->IsTabardDesigner()) {
        ...
        default:
            LOG_DEBUG("network",
                      "unknown selection at buy arena petition: {}",
                      clientIndex);
            return;
        }

        if (_player->GetArenaTeamId(clientIndex -
    ...
    if (type == GUILD_CHARTER_TYPE) {
        if (sGuildMgr->GetGuildByName(name)) {
            Guild::SendCommandResult(
                this, GUILD_COMMAND_CREATE, ERR_GUILD_NAME_EXISTS_S, name);
            return;
        }

        if (sObjectMgr->IsReservedName(name) ||
            sObjectMgr->IsProfanityName(name) ||
            !ObjectMgr::IsValidCharterName(name)) {
            Guild::SendCommandResult(
                this, GUILD_COMMAND_CREATE, ERR_GUILD_NAME_INVALID, name);
            return;
        }
    }
    else {
    ...
    if (!_player->HasEnoughMoney(cost)) {
        ...
        _player->SendBuyError(
            BUY_ERR_NOT_ENOUGHT_MONEY, creature, charterid, 0);
        return;
    }

    ItemPosCountVec dest;
    InventoryResult msg = _player->CanStoreNewItem(
        NULL_BAG, NULL_SLOT, dest, charterid, pProto->BuyCount);
    if (msg != EQUIP_ERR_OK) {
    ...
    charter->SetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1,
                            charter->GetGUID().GetCounter());
    ...
    Petition const* petition =
        sPetitionMgr->GetPetitionByOwnerWithType(_player->GetGUID(), type);
    ...
    if (petition) {
        LOG_DEBUG("network",
                  "Invalid petition: {}",
                  petition->petitionGuid.ToString());

        trans->Append("DELETE FROM petition WHERE petitionguid = {}",
                      petition->petitionGuid.GetCounter());
        trans->Append("DELETE FROM petition_sign WHERE petitionguid = {}",
                      petition->petitionGuid.GetCounter());
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_PETITION);
    ...
    sPetitionMgr->AddPetition(
        charter->GetGUID(), _player->GetGUID(), name, uint8(type));
}
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recvData)
{
    ...
    // solve (possible) some strange compile problems with explicit use petition
    // low guid at some GCC versions (wrong code optimization in compiler?)
    ...
    LOG_DEBUG("network",
              "CMSG_PETITION_SHOW_SIGNATURES petition {}",
    ...
    if (signs)
        for (SignatureMap::const_iterator itr =
                 signatures->signatureMap.begin();
             itr != signatures->signatureMap.end();
             ++itr) {
...
void WorldSession::HandlePetitionQueryOpcode(WorldPacket& recvData)
{
    ...
    recvData >> petitionguid; // petition guid
    LOG_DEBUG("network",
              "CMSG_PETITION_QUERY Petition ({}) Guild GUID {}",
              petitionguid.ToString(),
              guildguid);
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    if (!petition) {
        LOG_DEBUG("network",
                  "CMSG_PETITION_QUERY failed for petition ({})",
                  petitionguid.ToString());
        return;
    }
    ...
    WorldPacket data(
        ...
        (4 + 8 + petition->petitionName.size() + 1 + 1 + 4 * 12 + 2 + 10));
    data << uint32(
        petitionguid.GetCounter()); // guild/team guid (in Trinity always same
                                    // as petition low guid
...
void WorldSession::HandlePetitionRenameOpcode(WorldPacket& recvData)
{
    ...
    if (petition->petitionType == GUILD_CHARTER_TYPE) {
        if (sGuildMgr->GetGuildByName(newName)) {
            Guild::SendCommandResult(
                this, GUILD_COMMAND_CREATE, ERR_GUILD_NAME_EXISTS_S, newName);
            return;
        }
        if (sObjectMgr->IsReservedName(newName) ||
            sObjectMgr->IsProfanityName(newName) ||
            !ObjectMgr::IsValidCharterName(newName)) {
            Guild::SendCommandResult(
                this, GUILD_COMMAND_CREATE, ERR_GUILD_NAME_INVALID, newName);
            return;
        }
    }
    else {
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_PETITION_NAME);
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recvData)
{
    ...
    if (!petition) {
        LOG_ERROR("network.opcode",
                  "Petition {} is not found for player {} (Name: {})",
                  petitionGuid.ToString(),
    ...
    if (type != GUILD_CHARTER_TYPE) {
        if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_ARENA) &&
            GetPlayer()->GetTeamId() !=
                sCharacterCache->GetCharacterTeamByGuid(petition->ownerGuid)) {
            SendArenaTeamCommandResult(
        ...
        if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
            GetPlayer()->GetTeamId() !=
                sCharacterCache->GetCharacterTeamByGuid(petition->ownerGuid)) {
            Guild::SendCommandResult(
                this, GUILD_COMMAND_CREATE, ERR_GUILD_NOT_ALLIED);
            return;
        }

        if (_player->GetGuildId()) {
            Guild::SendCommandResult(this,
    ...
    // check sign by another character from same account not allow sign another
    // player from already sign player account

    bool found = false;
    for (SignatureMap::const_iterator itr = signatures->signatureMap.begin();
         itr != signatures->signatureMap.end();
         ++itr)
    ...
    LOG_DEBUG("network",
              "PETITION SIGN: {} by player: {} ({}, Account: {})",
              petitionGuid.ToString(),
              _player->GetName(),
              playerGuid.ToString(),
              GetAccountId());
...
void WorldSession::HandlePetitionDeclineOpcode(WorldPacket& recvData)
{
    ...
    if (Player* owner = ObjectAccessor::FindConnectedPlayer(
            ownerguid)) // petition owner online
    {
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recvData)
{
    ...
    if (petition->petitionType != GUILD_CHARTER_TYPE) {
        if (GetPlayer()->GetTeamId() != player->GetTeamId() &&
            !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_ARENA)) {
            SendArenaTeamCommandResult(
                ERR_ARENA_TEAM_INVITE_SS, "", "", ERR_ARENA_TEAM_NOT_ALLIED);
            return;
        }
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& recvData)
{
    ...
    if (!petition) {
        LOG_ERROR("network.opcode",
                  "Player {} ({}) tried to turn in petition ({}) that is not "
    ...
    if (type == GUILD_CHARTER_TYPE) {
        ...
        Guild::SendCommandResult(
            this, GUILD_COMMAND_CREATE, ERR_GUILD_COMMAND_SUCCESS, name);

        // Add members from signatures
        if (signs)
            for (SignatureMap::const_iterator itr = signatureCopy.begin();
                 itr != signatureCopy.end();
                 ++itr)
        ...
        if (!arenaTeam->Create(_player->GetGUID(),
                               type,
                               name,
                               background,
                               icon,
        ...
        LOG_DEBUG("network",
                  "PetitonsHandler: Arena team (guid: {}) added to ObjectMgr",
                  arenaTeam->GetId());
        ...
        if (signs)
            for (SignatureMap::const_iterator itr = signatureCopy.begin();
                ...
                LOG_DEBUG(
                    "network",
                    "PetitionsHandler: Adding arena team (guid: {}) member {}",
                    arenaTeam->GetId(),
                    itr->first.ToString());
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_PETITION_BY_GUID);
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
    ...
    if (!creature) {
        LOG_DEBUG("network",
                  "WORLD: HandlePetitionShowListOpcode - Unit ({}) not found "
    ...
    if (creature->IsTabardDesigner()) {
        sScriptMgr->PetitionShowList(
            _player, creature, CharterEntry, CharterDispayID, CharterCost);
        ...
        data << uint32(
            sWorld->getIntConfig(CONFIG_MIN_PETITION_SIGNS)); // required signs
    }
    else {
        // For 2v2 default
