    ...
    if (guid == _player->GetGUID()) {
        if (_player->GetSession()->GetSecurity() < SEC_MODERATOR) {
            LOG_ERROR("network.opcode",
                      "{} attempt open mailbox in cheating way.",
                      _player->GetName());
            return false;
        }
    }
    else if (guid.IsGameObject()) {
        if (!_player->GetGameObjectIfCanInteractWith(guid,
...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    {
        GetPlayer()->SendMailResult(
            0, MAIL_SEND, MAIL_ERR_TOO_MANY_ATTACHMENTS);
    ...
    if (player->GetLevel() < sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ)) {
        SendNotification(GetAcoreString(LANG_MAIL_SENDER_REQ),
                         sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    if (!receiverGuid) {
        LOG_DEBUG("network.opcode",
                  "Player {} is sending mail to {} (GUID: not existed!) with "
                  "subject {} and body {} includes {} items, {} copper and {} "
                  "COD copper with unk1 = {}, unk2 = {}",
    ...
    LOG_DEBUG("network.opcode",
              ...
              "{} includes {} items, {} copper and {} COD copper with unk1 = "
              "{}, unk2 = {}",
              player->GetGUID().ToString(),
              receiver,
    ...
    uint32 cost =
        items_count ? 30 * items_count : 30; // price hardcoded in client
    ...
    if (receive) {
        ...
        if (CharacterCacheEntry const* playerData =
                sCharacterCache->GetCharacterCacheByGuid(receiverGuid)) {
    ...
    // opcode uint8!!! - so max can be 255..
    if (mails_count > 100) {
    ...
    uint32 rc_account =
        receive ? receive->GetSession()->GetAccountId()
                : sCharacterCache->GetCharacterAccountIdByGuid(receiverGuid);

    if (/*!accountBound*/ GetAccountId() != rc_account &&
        !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_MAIL) &&
        player->GetTeamId() != rc_teamId &&
    ...
    for (uint8 i = 0; i < items_count; ++i) {
        if (!itemGUIDs[i]) {
            player->SendMailResult(
                0, MAIL_SEND, MAIL_ERR_MAIL_ATTACHMENT_INVALID);
            return;
        }

        Item* item = player->GetItemByGuid(itemGUIDs[i]);

        // prevent sending bag with items (cheat: can be placed in bag after
        // adding equipped empty bag to mail)
        if (!item) {
            player->SendMailResult(
                0, MAIL_SEND, MAIL_ERR_MAIL_ATTACHMENT_INVALID);
            return;
        ...
        // handle empty bag before CanBeTraded, since that func already has that
        // check
        if (item->IsNotEmptyBag()) {
            player->SendMailResult(0,
                                   MAIL_SEND,
        ...
        if (!item->CanBeTraded(true)) {
            player->SendMailResult(
                0, MAIL_SEND, MAIL_ERR_EQUIP_ERROR, EQUIP_ERR_MAIL_BOUND_ITEM);
            return;
        }

        if (item->IsBoundAccountWide() && item->IsSoulBound() &&
            GetAccountId() != rc_account) {
            player->SendMailResult(0,
                                   MAIL_SEND,
                                   MAIL_ERR_EQUIP_ERROR,
        ...
        if (item->GetTemplate()->Flags & ITEM_FLAG_CONJURED ||
        ...
        if (COD && item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED)) {
            player->SendMailResult(
                0, MAIL_SEND, MAIL_ERR_CANT_SEND_WRAPPED_COD);
            return;
        }

        if (!sScriptMgr->CanSendMail(player,
                                     receiverGuid,
                                     mailbox,
                                     subject,
                                     body,
    ...
    if (!items_count && !sScriptMgr->CanSendMail(player,
                                                 receiverGuid,
                                                 mailbox,
                                                 subject,
                                                 body,
    ...
    if (items_count > 0 || money > 0) {
        if (items_count > 0) {
            for (uint8 i = 0; i < items_count; ++i) {
                ...
                item->SetNotRefundable(
                    GetPlayer()); // makes the item no longer refundable
                player->MoveItemFromInventory(
                    items[i]->GetBagSlot(), item->GetSlot(), true);

                item->DeleteFromInventoryDB(
                    trans); // deletes item from character's inventory
                if (item->GetState() == ITEM_UNCHANGED)
                    item->FSetState(
                        ITEM_CHANGED); // pussywizard: so the item will be saved
                                       // and owner will be updated in database
                item->SetOwnerGUID(receiverGuid);
                item->SaveToDB(trans); // recursive and not have transaction
                                       // guard into self, item not in inventory
                                       // and can be save standalone

                draft.AddItem(item);
            }

            // if item send to character at another account, then apply item
            // delivery delay
    ...
    // If theres is an item, there is a one hour delivery delay if sent to
    // another account's character.
    uint32 deliver_delay =
        needItemDelay ? sWorld->getIntConfig(CONFIG_MAIL_DELIVERY_DELAY) : 0;
    ...
    draft.AddMoney(money).AddCOD(COD).SendMailTo(
        trans,
        MailReceiver(receive, receiverGuid.GetCounter()),
        MailSender(player),
        body.empty() ? MAIL_CHECK_MASK_COPIED : MAIL_CHECK_MASK_HAS_BODY,
        deliver_delay);
...
void WorldSession::HandleMailDelete(WorldPacket& recvData)
{
    ...
    if (m) {
        ...
        if (m->COD) {
            player->SendMailResult(
                mailId, MAIL_DELETED, MAIL_ERR_INTERNAL_ERROR);
            return;
        }
...
void WorldSession::HandleMailReturnToSender(WorldPacket& recvData)
{
    ...
    recvData >> mailId;
    recvData
        .read_skip<uint64>(); // original sender GUID for return to, not used
    ...
    Mail*   m      = player->GetMail(mailId);
    if (!m || m->state == MAIL_STATE_DELETED ||
        m->deliver_time > GameTime::GetGameTime().count()) {
        player->SendMailResult(
            mailId, MAIL_RETURNED_TO_SENDER, MAIL_ERR_INTERNAL_ERROR);
        return;
    }

    if (m->HasItems()) {
        for (MailItemInfoVec::iterator itr = m->items.begin();
             itr != m->items.end();
             ++itr) {
            Item* item = player->GetMItem(itr->item_guid);
            if (item && !sScriptMgr->CanSendMail(
                            player,
    ...
    else if (!sScriptMgr->CanSendMail(player,
                                      ObjectGuid(HighGuid::Player, m->sender),
                                      mailbox,
                                      m->subject,
                                      m->body,
    ...
    if (m->messageType == MAIL_NORMAL && m->sender) {
        ...
        if (m->mailTemplateId)
            draft =
                MailDraft(m->mailTemplateId, false); // items already included
        ...
        draft.AddMoney(m->money).SendReturnToSender(
            GetAccountId(), m->receiver, m->sender, trans);
    }
...
void WorldSession::HandleMailTakeItem(WorldPacket& recvData)
{
    ...
    if (!m || m->state == MAIL_STATE_DELETED ||
        m->deliver_time > GameTime::GetGameTime().count()) {
        player->SendMailResult(
            mailId, MAIL_ITEM_TAKEN, MAIL_ERR_INTERNAL_ERROR);
        return;
    }
    ...
    for (std::vector<MailItemInfo>::const_iterator itr = m->items.begin();
         itr != m->items.end();
         ++itr)
    ...
    if (!player->HasEnoughMoney(m->COD)) {
        player->SendMailResult(
            mailId, MAIL_ITEM_TAKEN, MAIL_ERR_NOT_ENOUGH_MONEY);
        return;
    }
    ...
    if (msg == EQUIP_ERR_OK) {
        ...
        if (m->COD > 0) // if there is COD, take COD money from player and send
                        // them to sender by mail
        {
            ...
            if (sender) {
                ...
                sender_accId = sCharacterCache->GetCharacterAccountIdByGuid(
                    ObjectGuid(HighGuid::Player, m->sender));
            }
            ...
            if (sender || sender_accId) {
                ...
                if (m->COD >= 10 * GOLD) {
                    ...
                    if (!sCharacterCache->GetCharacterNameByGuid(
                            ObjectGuid(HighGuid::Player, m->sender),
                            senderName)) {
                        senderName = sObjectMgr->GetAcoreStringForDBCLocale(
                            LANG_UNKNOWN);
                    }
        ...
        uint32 count = it->GetCount(); // save counts before store and possible
                                       // merge with deleting
        it->SetState(ITEM_UNCHANGED);  // need to set this state, otherwise item
                                       // cannot be removed later, if neccessary
        ...
        player->SendMailResult(
            mailId, MAIL_ITEM_TAKEN, MAIL_OK, 0, itemLowGuid, count);
    }
    else
        player->SendMailResult(
            mailId, MAIL_ITEM_TAKEN, MAIL_ERR_EQUIP_ERROR, msg);
}
...
void WorldSession::HandleMailTakeMoney(WorldPacket& recvData)
{
    ...
    if (!m || m->state == MAIL_STATE_DELETED ||
        m->deliver_time > GameTime::GetGameTime().count()) {
        player->SendMailResult(
            mailId, MAIL_MONEY_TAKEN, MAIL_ERR_INTERNAL_ERROR);
        return;
    }

    if (!player->ModifyMoney(m->money, false)) {
        player->SendMailResult(mailId,
                               MAIL_MONEY_TAKEN,
                               MAIL_ERR_EQUIP_ERROR,
...
void WorldSession::HandleGetMailList(WorldPacket& recvData)
{
    ...
    for (Mail const* mail : player->GetMails()) {
        ...
        if (mail->state == MAIL_STATE_DELETED ||
            cur_time < mail->deliver_time || cur_time > mail->expire_time) {
            continue;
        }

        uint8 item_count =
            uint8(mail->items.size()); // max count is MAX_MAIL_ITEMS (12)

        size_t next_mail_size =
            2 + 4 + 1 + (mail->messageType == MAIL_NORMAL ? 8 : 4) + 4 * 8 +
            (mail->subject.size() + 1) + (mail->body.size() + 1) + 1 +
            item_count * (1 + 4 + 4 + MAX_INSPECTED_ENCHANTMENT_SLOT * 3 * 4 +
                          4 + 4 + 4 + 4 + 4 + 4 + 1);
        ...
        case MAIL_AUCTION:
        case MAIL_CALENDAR:
            data << uint32(mail->sender); // creature/gameobject entry, auction
        ...
        data << float(
            float(mail->expire_time - GameTime::GetGameTime().count()) /
            DAY); // Time
        data << uint32(
            mail->mailTemplateId); // mail template (MailTemplate.dbc)
        data
            << subject; // Subject string - once 00, when mail type = 3, max 256
        ...
        for (uint8 i = 0; i < item_count; ++i) {
            ...
            for (uint8 j = 0; j < MAX_INSPECTED_ENCHANTMENT_SLOT; ++j) {
                data << uint32(item ? item->GetEnchantmentId(EnchantmentSlot(j))
                                    : 0);
                data << uint32(
                    item ? item->GetEnchantmentDuration(EnchantmentSlot(j))
                         : 0);
                data << uint32(
                    item ? item->GetEnchantmentCharges(EnchantmentSlot(j)) : 0);
            }
            // can be negative
            ...
            // durability
            data << uint32(
                (item ? item->GetUInt32Value(ITEM_FIELD_MAXDURABILITY) : 0));
            // durability
            data << uint32(
                (item ? item->GetUInt32Value(ITEM_FIELD_DURABILITY) : 0));
            // unknown wotlk
            data << uint8(0);
        }
    ...
    data.put<uint32>(0,
                     realCount); //  this will display warning about undelivered
                                 //  mail to player if realCount > mailsCount
...
void WorldSession::HandleMailCreateTextItem(WorldPacket& recvData)
{
    ...
    if (!m || (m->body.empty() && !m->mailTemplateId) ||
        ...
        player->SendMailResult(
            mailId, MAIL_MADE_PERMANENT, MAIL_ERR_INTERNAL_ERROR);
        return;
    }

    Item* bodyItem = new Item; // This is not bag and then can be used new Item.
    if (!bodyItem->Create(sObjectMgr->GetGenerator<HighGuid::Item>().Generate(),
    ...
    if (m->mailTemplateId) {
        MailTemplateEntry const* mailTemplateEntry =
    ...
    ItemPosCountVec dest;
    uint8           msg =
        _player->CanStoreItem(NULL_BAG, NULL_SLOT, dest, bodyItem, false);
    if (msg == EQUIP_ERR_OK) {
        ...
        player->SendMailResult(
            mailId, MAIL_MADE_PERMANENT, MAIL_ERR_EQUIP_ERROR, msg);
        delete bodyItem;
    }
}
...
void WorldSession::HandleQueryNextMailTime(WorldPacket& /*recvData*/)
{
    ...
    if (_player->unReadMails > 0) {
        ...
        for (Mail const* mail : _player->GetMails()) {
            ...
            data << (mail->messageType == MAIL_NORMAL
                         ? ObjectGuid::Create<HighGuid::Player>(mail->sender)
                         : ObjectGuid::Empty); // player guid
            data << uint32(mail->messageType != MAIL_NORMAL
                               ? mail->sender
                               : 0); // non-player entries
