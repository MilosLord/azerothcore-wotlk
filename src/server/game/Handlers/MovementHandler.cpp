...
void WorldSession::HandleMoveWorldportAck()
{
    ...
    if (GetPlayer()->IsInWorld()) {
        LOG_ERROR("network.opcode",
                  "Player (Name {}) is still in world when teleported from map "
                  "{} to new map {}",
                  GetPlayer()->GetName(),
                  oldMap->GetId(),
                  loc.GetMapId());
    ...
    if (!GetPlayer()->m_InstanceValid && !mInstance) {
        ...
        // pussywizard: m_InstanceValid can be false only by leaving a group in
        // an instance => so remove temp binds that could not be removed because
        // player was still on the map!
        if (!sInstanceSaveMgr->PlayerIsPermBoundToInstance(
                GetPlayer()->GetGUID(),
                oldMap->GetId(),
                oldMap->GetDifficulty()))
            sInstanceSaveMgr->PlayerUnbindInstance(GetPlayer()->GetGUID(),
                                                   oldMap->GetId(),
                                                   oldMap->GetDifficulty(),
                                                   true);
    }
    ...
    if (!newMap || newMap->CannotEnter(GetPlayer(), false)) {
        LOG_ERROR("network.opcode",
                  "Map {} could not be created for player {}, porting player "
                  "to homebind",
                  loc.GetMapId(),
                  GetPlayer()->GetGUID().ToString());
        GetPlayer()->TeleportTo(GetPlayer()->m_homebindMapId,
                                GetPlayer()->m_homebindX,
                                GetPlayer()->m_homebindY,
                                GetPlayer()->m_homebindZ,
                                GetPlayer()->m_homebindO);
        return;
    }

    float z = loc.GetPositionZ() + GetPlayer()->GetHoverHeight();
    GetPlayer()->Relocate(
        loc.GetPositionX(), loc.GetPositionY(), z, loc.GetOrientation());
    ...
    if (!GetPlayer()->GetMap()->AddPlayerToMap(GetPlayer())) {
        LOG_ERROR("network.opcode",
                  "WORLD: failed to teleport player {} ({}) to map {} because "
                  "of unknown reason!",
                  GetPlayer()->GetName(),
                  GetPlayer()->GetGUID().ToString(),
                  loc.GetMapId());
    ...
    // pussywizard: transport teleport couldn't teleport us to the same map
    // (some other teleport pending, reqs not met, etc.), but we still have
    // transport set until player moves! clear it if map differs (crashfix)
    if (Transport* t = _player->GetTransport())
        if (!t->IsInMap(_player)) {
            ...
            _player->m_movementInfo.RemoveMovementFlag(
    ...
    CellCoord pair(Acore::ComputeCellCoord(GetPlayer()->GetPositionX(),
    ...
    if (_player->InBattleground()) {
        ...
        // everything ok
            ...
            if (_player->IsInvitedForBattlegroundInstance()) // GMs are not
                                                             // invited, so they
                                                             // are not added to
                                                             // participants
    ...
    {
        if (newMap->IsBattleArena() && ((BattlegroundMap*)newMap)->GetBG() &&
            _player->HasPendingSpectatorForBG(
                ((BattlegroundMap*)newMap)->GetInstanceId())) {
            _player->ClearReceivedSpectatorResetFor();
            _player->SetIsSpectator(true);
            ArenaSpectator::SendCommand(
                _player, "%sENABLE", SPECTATOR_ADDON_PREFIX);
        ...
        if (uint32 inviteInstanceId =
                _player->GetPendingSpectatorInviteInstanceId()) {
            if (Battleground* tbg = sBattlegroundMgr->GetBattleground(
                    inviteInstanceId, BATTLEGROUND_TYPE_NONE))
                tbg->RemoveToBeTeleported(_player->GetGUID());
            _player->SetPendingSpectatorInviteInstanceId(0);
        }
    }

    // xinef: do this again, player can be teleported inside
    // bg->AddPlayer(_player)!!!!
    CellCoord pair2(Acore::ComputeCellCoord(GetPlayer()->GetPositionX(),
                                            GetPlayer()->GetPositionY()));
    Cell      cell2(pair2);
    ...
    // resurrect character at enter into instance where his corpse exist after
    // add to map
    Corpse* corpse =
        GetPlayer()->GetMap()->GetCorpseByPlayer(GetPlayer()->GetGUID());
    ...
    if (!corpse && mEntry->IsDungeon()) {
        // resurrect character upon entering instance when the corpse is not
        // available anymore
    ...
    if (mInstance) {
        ...
        if (MapDifficulty const* mapDiff =
                GetMapDifficultyData(mEntry->MapID, diff))
            if (mapDiff->resetTime)
                if (time_t timeReset = sInstanceSaveMgr->GetResetTimeFor(
                    ...
                    GetPlayer()->SendInstanceResetWarning(
                        mEntry->MapID, diff, timeleft, true);
                }
    ...
    // update zone immediately, otherwise leave channel will cause crash in
    // mtmap
    ...
    // in friendly area
    else if (GetPlayer()->IsPvP() &&
             !GetPlayer()->HasPlayerFlag(PLAYER_FLAGS_IN_PVP))
...
void WorldSession::HandleMoveTeleportAck(WorldPacket& recvData)
{
    ...
    LOG_DEBUG(
        "network.opcode", "Flags {}, time {}", flags, time / IN_MILLISECONDS);
    ...
    if (Pet* pet = plMover->GetPet()) {
        if (!pet->IsWithinDist3d(
                plMover, plMover->GetMap()->GetVisibilityRange() - 5.0f))
            pet->NearTeleportTo(plMover->GetPositionX(),
    ...
    if (oldPos.GetExactDist2d(plMover) > 100.0f) {
        ...
        if (old_zone != newzone) {
            ...
            // in friendly area
                     ...
                     !plMover->HasPlayerFlag(PLAYER_FLAGS_IN_PVP))
    ...
    if (plMover->HasUnitState(UNIT_STATE_FLEEING | UNIT_STATE_CONFUSED) ||
        plMover->IsCharmed()) // only in such cases SetClientControl(self,
                              // false) is sent
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    // pussywizard: typical check for incomming movement packets | prevent
    // tampered movement data
    ...
    if (Player* plrMover = mover->ToPlayer()) {
        if (plrMover->GetUInt32Value(UNIT_NPC_EMOTESTATE) !=
            EMOTE_ONESHOT_NONE) {
    ...
    if (mover->HasUnitFlag(UNIT_FLAG_DISABLE_MOVE)) {
        ...
        movementInfo.pos.Relocate(mover->GetPositionX(),
                                  mover->GetPositionY(),
                                  mover->GetPositionZ());
    ...
    if (movementInfo.HasMovementFlag(MOVEMENTFLAG_ONTRANSPORT)) {
        ...
        if (movementInfo.pos.GetExactDist2d(mover) > SIZE_OF_GRIDS) {
            if (plrMover) {
                sScriptMgr->AnticheatUpdateMovementInfo(plrMover, movementInfo);
                // LOG_INFO("anticheat", "MovementHandler:: 2 We were
                // teleported, skip packets that were broadcast before
                // teleport");
            }
            recvData.rfinish(); // prevent warnings spam
            return;
        ...
        if (!Acore::IsValidMapCoord(
                movementInfo.pos.GetPositionX() +
                    movementInfo.transport.pos.GetPositionX(),
                movementInfo.pos.GetPositionY() +
                    movementInfo.transport.pos.GetPositionY(),
                movementInfo.pos.GetPositionZ() +
                    movementInfo.transport.pos.GetPositionZ(),
                movementInfo.pos.GetOrientation() +
                    movementInfo.transport.pos.GetOrientation())) {
            if (plrMover) {
                sScriptMgr->AnticheatUpdateMovementInfo(plrMover, movementInfo);
            }
        ...
        if (plrMover) {
            if (!plrMover->GetTransport()) {
                if (Transport* transport = plrMover->GetMap()->GetTransport(
                        movementInfo.transport.guid)) {
                    plrMover->m_transport = transport;
                    transport->AddPassenger(plrMover);
                }
            }
            else if (plrMover->GetTransport()->GetGUID() !=
                     movementInfo.transport.guid) {
        ...
        if (!mover->GetTransport() && !mover->GetVehicle()) {
            GameObject* go =
                mover->GetMap()->GetGameObject(movementInfo.transport.guid);
            if (!go || go->GetGoType() != GAMEOBJECT_TYPE_TRANSPORT) {
             ...
             plrMover->GetTransport()) // if we were on a transport, leave
    {
    ...
    if (opcode == MSG_MOVE_FALL_LAND || opcode == MSG_MOVE_START_SWIM) {
        mover->RemoveAurasWithInterruptFlags(
            AURA_INTERRUPT_FLAG_LANDING); // Parachutes
    ...
    if (plrMover && ((movementInfo.flags & MOVEMENTFLAG_SWIMMING) != 0) !=
                        plrMover->IsInWater()) {
        // now client not include swimming flag in case jumping under water
        plrMover->SetInWater(
            !plrMover->IsInWater() ||
            plrMover->GetMap()->IsUnderWater(plrMover->GetPhaseMask(),
                                             movementInfo.pos.GetPositionX(),
                                             movementInfo.pos.GetPositionY(),
                                             movementInfo.pos.GetPositionZ(),
                                             plrMover->GetCollisionHeight()));
    }
    ...
    if (opcode == MSG_MOVE_JUMP) {
        jumpopcode = true;
        if (plrMover &&
            !sScriptMgr->AnticheatHandleDoubleJump(plrMover, mover)) {
    ...
    if (plrMover && !sScriptMgr->AnticheatCheckMovementInfo(
                        plrMover, movementInfo, mover, jumpopcode)) {
        plrMover->GetSession()->KickPlayer();
        return;
    }
    ...
    if (_timeSyncClockDelta == 0 || movementTime < 0 ||
        movementTime > 0xFFFFFFFF) {
        LOG_INFO("misc",
                 "The computed movement time using clockDelta is erronous. "
    ...
    if (Vehicle* vehicle = mover->GetVehicle()) {
        if (VehicleSeatEntry const* seat =
                vehicle->GetSeatForPassenger(mover)) {
    ...
    if (plrMover) // nothing is charmed, or player charmed
    {
        if (plrMover->IsSitState() &&
            (movementInfo.flags &
             (MOVEMENTFLAG_MASK_MOVING | MOVEMENTFLAG_MASK_TURNING)))
            plrMover->SetStandState(UNIT_STAND_STATE_STAND);

        plrMover->UpdateFallInformationIfNeed(movementInfo, opcode);

        if (movementInfo.pos.GetPositionZ() <
            plrMover->GetMap()->GetMinHeight(movementInfo.pos.GetPositionX(),
                                             movementInfo.pos.GetPositionY()))
            if (!plrMover->GetBattleground() ||
                !plrMover->GetBattleground()->HandlePlayerUnderMap(_player)) {
                if (plrMover->IsAlive()) {
                    plrMover->SetPlayerFlag(PLAYER_FLAGS_IS_OUT_OF_BOUNDS);
                    plrMover->EnvironmentalDamage(DAMAGE_FALL_TO_VOID,
                                                  GetPlayer()->GetMaxHealth());
                    // player can be alive if GM
                    if (plrMover->IsAlive())
                        plrMover->KillPlayer();
                }
                else if (!plrMover->HasPlayerFlag(
                             PLAYER_FLAGS_IS_OUT_OF_BOUNDS)) {
                    GraveyardStruct const* grave =
                        sGraveyard->GetClosestGraveyard(plrMover,
                                                        plrMover->GetTeamId());
                    if (grave) {
                        plrMover->TeleportTo(grave->Map,
                                             grave->x,
                                             grave->y,
                                             grave->z,
                                             plrMover->GetOrientation());
                        plrMover->Relocate(grave->x,
                                           grave->y,
                                           grave->z,
                                           plrMover->GetOrientation());
                    }
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket& recvData)
{
    ...
    // last from its in other cases anti-cheat check can be fail in false case
    UnitMoveType move_type;
    UnitMoveType force_move_type;

    static char const* move_type_name[MAX_MOVE_TYPE] = {"Walk",
                                                        "Run",
                                                        "RunBack",
                                                        "Swim",
                                                        "SwimBack",
    ...
    if (!_player->GetTransport() &&
        std::fabs(_player->GetSpeed(move_type) - newspeed) > 0.01f) {
        if (_player->GetSpeed(move_type) >
            newspeed) // must be greater - just correct
        {
            LOG_ERROR("network.opcode",
                      "{}SpeedChange player {} is NOT correct (must be {} "
                      "instead {}), force set to correct value",
                      move_type_name[move_type],
                      _player->GetName(),
        ...
        else // must be lesser - cheating
        {
            LOG_INFO("network.opcode",
                     "Player {} from account id {} kicked for incorrect speed "
                     "(must be {} instead {})",
                     _player->GetName(),
                     GetAccountId(),
                     _player->GetSpeed(move_type),
...
void WorldSession::HandleSetActiveMoverOpcode(WorldPacket& recvData)
{
    ...
    if (GetPlayer()->IsInWorld() && _player->m_mover &&
        _player->m_mover->IsInWorld()) {
        if (_player->m_mover->GetGUID() != guid)
            LOG_ERROR("network.opcode",
                      "HandleSetActiveMoverOpcode: incorrect mover guid: mover "
                      "is {} and should be {}",
                      guid.ToString(),
                      _player->m_mover->GetGUID().ToString());
    }
}
...
void WorldSession::HandleMoveNotActiveMover(WorldPacket& recvData)
{
    ...
    if (!_player->m_mover || !_player->m_mover->IsInWorld() ||
        _player->m_mover->IsDuringRemoveFromWorld() ||
        old_mover_guid != _player->m_mover->GetGUID()) {
        recvData.rfinish(); // prevent warnings spam
        return;
    }
...
void WorldSession::HandleSummonResponseOpcode(WorldPacket& recvData)
{
    ...
    if (agree && _player->IsSummonAsSpectator()) {
        ...
        if (Player* summoner = ObjectAccessor::FindPlayer(summoner_guid))
            ArenaSpectator::HandleSpectatorSpectateCommand(
                &chc, summoner->GetName().c_str());
        else
...
void WorldSession::HandleMoveTimeSkippedOpcode(WorldPacket& recvData)
{
    ...
    if (!mover) {
        LOG_ERROR("network.opcode",
                  "WorldSession::HandleMoveTimeSkippedOpcode wrong mover state "
                  "from the unit moved by the player [{}]",
                  GetPlayer()->GetGUID().ToString());
        return;
    }
    ...
    if (guid != mover->GetGUID()) {
        LOG_ERROR("network.opcode",
                  "WorldSession::HandleMoveTimeSkippedOpcode wrong guid from "
                  "the unit moved by the player [{}]",
                  GetPlayer()->GetGUID().ToString());
        return;
    }
...
void WorldSession::HandleTimeSyncResp(WorldPacket& recvData)
{
    ...
    // time it took for the request to travel to the client, for the client to
    // process it and reply and for response to travel back to the server. we
    // are going to make 2 assumptions: 1) we assume that the request processing
    // time equals 0. 2) we assume that the packet took as much time to travel
    // from server to client than it took to travel from client to server.
    uint32 roundTripDuration =
        getMSTimeDiff(serverTimeAtSent, recvData.GetReceivedTime());
    uint32 lagDelay = roundTripDuration / 2;

    // clockDelta = serverTime - clientTime
    // where
    // serverTime: time that was displayed on the clock of the SERVER at the
    // moment when the client processed the SMSG_TIME_SYNC_REQUEST packet.
    // clientTime:  time that was displayed on the clock of the CLIENT at the
    // moment when the client processed the SMSG_TIME_SYNC_REQUEST packet.

    // Once clockDelta has been computed, we can compute the time of an event on
    // server clock when we know the time of that same event on the client
    // clock, using the following relation: serverTime = clockDelta + clientTime

    int64 clockDelta =
        (int64)serverTimeAtSent + (int64)lagDelay - (int64)clientTimestamp;
    _timeSyncClockDeltaQueue.put(
        std::pair<int64, uint32>(clockDelta, roundTripDuration));
    ComputeNewClockDelta();
}

void WorldSession::ComputeNewClockDelta()
{
    // implementation of the technique described here:
    // https://web.archive.org/web/20180430214420/http://www.mine-control.com/zack/timesync/timesync.html
    ...
    if (sampleSizeAfterFiltering != 0) {
        int64 meanClockDelta =
            static_cast<int64>(mean(clockDeltasAfterFiltering));
