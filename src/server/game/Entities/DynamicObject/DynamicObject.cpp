...
DynamicObject::DynamicObject(bool isWorldObject)
    : WorldObject(isWorldObject), MovableMapObject(), _aura(nullptr),
      _removedAura(nullptr), _caster(nullptr), _duration(0),
      _isViewpoint(false), _updateViewerVisibilityTimer(0)
{
...
{
    ///- Remove the dynamicObject from the accessor and from all lists of
    ///objects in world
    if (IsInWorld()) {
        if (_isViewpoint)
                                        ...
                                        DynamicObjectType   type)
{
    ...
    if (!IsPositionValid()) {
        LOG_ERROR("dyobject",
                  "DynamicObject (spell {}) not created. Suggested coordinates "
                  "isn't valid (X: {} Y: {})",
                  spellId,
                  GetPositionX(),
                  GetPositionY());
    ...
    WorldObject::_Create(
        guidlow, HighGuid::DynamicObject, caster->GetPhaseMask());
    ...
    // The lower word of DYNAMICOBJECT_BYTES must be 0x0001. This value means
    // that the visual radius will be overriden by client for most of the
    // "ground patch" visual effect spells and a few "skyfall" ones like
    // Hurricane. If any other value is used, the client will _always_ use the
    // radius provided in DYNAMICOBJECT_RADIUS, but precompensation is necessary
    // (eg radius *= 2) for many spells. Anyway, blizz sends 0x0001 for all the
    // spells I saw sniffed...
    ...
    if (!GetMap()->AddToMap(this, true)) {
        // Returning false will cause the object to be deleted - remove from
        // transport
        return false;
    }
...
void DynamicObject::SetCasterViewpoint(bool updateViewerVisibility)
{
    if (Player* caster = _caster->ToPlayer()) {
        ...
        if (Unit* farsightObject = ObjectAccessor::GetUnit(
                *caster, caster->GetGuidValue(PLAYER_FARSIGHT))) {
...
void DynamicObject::RemoveCasterViewpoint()
{
    if (Player* caster = _caster->ToPlayer()) {
        ...
        if (Unit* farsightObject =
                ObjectAccessor::GetUnit(*caster, _oldFarsightGUID)) {
