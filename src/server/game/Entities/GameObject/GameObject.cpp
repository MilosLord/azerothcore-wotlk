...
GameObject::GameObject()
    : WorldObject(false), MovableMapObject(), m_model(nullptr), m_goValue(),
      m_AI(nullptr)
{
    m_objectType |= TYPEMASK_GAMEOBJECT;
    m_objectTypeId = TYPEID_GAMEOBJECT;

    m_updateFlag = (UPDATEFLAG_LOWGUID | UPDATEFLAG_STATIONARY_POSITION |
                    UPDATEFLAG_POSITION | UPDATEFLAG_ROTATION);
...
GameObject::~GameObject()
{
    delete m_AI;
    delete m_model;
    // if (m_uint32Values)                                      // field array
    // can be not exist if GameOBject not loaded
...
void GameObject::RemoveFromOwner()
{
    ...
    LOG_DEBUG("entities.gameobject",
              "Delete GameObject ({} Entry: {} SpellId {} LinkedGO {}) that "
              "lost references to owner {} GO list.",
              GetGUID().ToString(),
              GetGOInfo()->entry,
              m_spellId,
...
void GameObject::AddToWorld()
{
    ...
    if (!IsInWorld()) {
        ...
        if (m_spawnId)
            GetMap()->GetGameObjectBySpawnIdStore().insert(
        ...
        EnableCollision(
            ...
            IsTransport()); // pussywizard: this startOpen is unneeded here,
                            // collision depends entirely on GOState
    ...
    if (IsInWorld()) {
        ...
        if (m_spawnId)
            Acore::Containers::MultimapErasePair(
                GetMap()->GetGameObjectBySpawnIdStore(), m_spawnId, this);
...
void GameObject::CheckRitualList()
{
    ...
    for (GuidSet::iterator itr = m_unique_users.begin();
         itr != m_unique_users.end();) {
        if (*itr == GetOwnerGUID()) {
            ++itr;
            continue;
        }
        ...
        if (Player* channeler = ObjectAccessor::GetPlayer(*this, *itr))
            if (Spell* spell =
                    channeler->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
                if (spell->m_spellInfo->Id ==
                    GetGOInfo()->summoningRitual.animSpell)
                    erase = false;
                        ...
                        uint32              artKit)
{
    ...
    if (!IsPositionValid()) {
        LOG_ERROR("entities.gameobject",
                  "Gameobject (GUID: {} Entry: {}) not created. Suggested "
                  "coordinates isn't valid (X: {} Y: {})",
                  guidlow,
                  name_id,
                  x,
                  y);
    ...
    if (!goinfo) {
        LOG_ERROR("sql.sql",
                  "Gameobject (GUID: {} Entry: {}) not created: non-existing "
                  "entry in `gameobject_template`. Map: {} (X: {} Y: {} Z: {})",
                  guidlow,
                  name_id,
                  map->GetId(),
                  x,
    ...
    if (goinfo->type >= MAX_GAMEOBJECT_TYPE) {
        LOG_ERROR(
            "sql.sql",
            "Gameobject (GUID: {} Entry: {}) not created: non-existing GO type "
            "'{}' in `gameobject_template`. It will crash client if created.",
            guidlow,
            name_id,
            goinfo->type);
        return false;
    ...
    case 21042:  // theramore guard badge
        ...
        break;
    default:
        // xinef: hackfix - but make it possible to use original WorldRotation
        // (using special gameobject addon data) pussywizard: temporarily
        // calculate WorldRotation from orientation, do so until values in db
        // are correct
        if (addon && addon->invisibilityType == INVISIBILITY_GENERAL &&
            addon->InvisibilityValue == 0) {
            SetLocalRotation(rotation);
        }
        else {
            SetLocalRotationAngles(
                NormalizeOrientation(GetOrientation()), 0.0f, 0.0f);
        }
        break;
    }
    ...
    if (IsInstanceGameobject()) {
        if (InstanceScript* instance = GetInstanceScript()) {
            switch (uint8 state =
                        instance->GetStoredGameObjectState(GetSpawnId())) {
            case 0:
    ...
    case GAMEOBJECT_TYPE_FISHINGHOLE:
        ...
        m_goValue.FishingHole.MaxOpens =
            urand(GetGOInfo()->fishinghole.minSuccessOpens,
                  GetGOInfo()->fishinghole.maxSuccessOpens);
        break;
    case GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING:
        m_goValue.Building.Health =
            goinfo->building.intactNumHits + goinfo->building.damagedNumHits;
    ...
    if (addon) {
        if (addon->InvisibilityValue) {
            ...
            m_invisibility.AddValue(addon->invisibilityType,
                                    addon->InvisibilityValue);
        }
    }
    ...
    if (uint32 linkedEntry = GetGOInfo()->GetLinkedGameObjectEntry()) {
        ...
        if (linkedGO->Create(map->GenerateLowGuid<HighGuid::GameObject>(),
                             linkedEntry,
                             map,
                             phaseMask,
                             x,
...
void GameObject::Update(uint32 diff)
{
    ...
    for (std::unordered_map<ObjectGuid, int32>::iterator itr =
             m_SkillupList.begin();
         itr != m_SkillupList.end();) {
        if (itr->second > 0) {
    ...
    case GO_NOT_READY: {
        ...
        case GAMEOBJECT_TYPE_TRAP: {
            ...
            if (goInfo->trap.type == 2)
                m_cooldownTime =
                    ...
                    10 * IN_MILLISECONDS; // Hardcoded tooltip value
            else if (GetOwner())
                m_cooldownTime = GameTime::GetGameTimeMS().count() +
                                 goInfo->trap.startDelay * IN_MILLISECONDS;
        ...
        case GAMEOBJECT_TYPE_SUMMONING_RITUAL: {
            ...
            if (info->summoningRitual.animSpell) {
                // xinef: if ritual requires animation, ensure that all users
                // performs channel
                CheckRitualList();
            }
            ...
            Unit* spellCaster =
                owner ? owner
                      : ObjectAccessor::GetPlayer(*this, m_ritualOwnerGUID);
            if (!spellCaster) {
            ...
            // on the current DB there is only one gameobject that uses this
            // (Ritual of Doom) and its required target number is 1 (outter for
            // loop will run once)
            if (info->summoningRitual.casterTargetSpell &&
                info->summoningRitual.casterTargetSpell !=
                    1) // No idea why this field is a bool in some cases
                for (uint32 i = 0;
                     i < info->summoningRitual.casterTargetSpellTargets;
                     i++)
                    // m_unique_users can contain only player GUIDs
                    if (Player* target = ObjectAccessor::GetPlayer(
                            *this,
            ...
            if (owner) {
                if (Spell* spell =
                        owner->GetCurrentSpell(CURRENT_CHANNELED_SPELL)) {
                    spell->SendChannelUpdate(0);
                    spell->finish(false);
                }
            }
        ...
        case GAMEOBJECT_TYPE_CHEST:
            ...
            // If there is no restock timer, or if the restock timer passed, the
            // chest becomes ready to loot
            m_restockTime = 0s;
            m_lootState   = GO_READY;
            AddToObjectUpdateIfNeeded();
            break;
        default:
            m_lootState = GO_READY; // for other GOis same switched without
                                    // delay to GO_READY
            break;
        }
        [[fallthrough]];
    ...
    case GO_READY: {
        if (m_respawnTime > 0) // timer on
        {
            ...
            if (m_respawnTime <= now) // timer expired
            {
                ObjectGuid dbtableHighGuid =
                    ObjectGuid::Create<HighGuid::GameObject>(GetEntry(),
                                                             m_spawnId);
                time_t linkedRespawntime =
                    ...
                    if (targetGuid ==
                        ...
                        m_respawnTime =
                            (now > linkedRespawntime ? now
                                                     : linkedRespawntime) +
                            urand(5, MINUTE); // else copy time from master and
                                              // add a little
                ...
                // Xinef: Call AI Reset (required for example in SmartAI to
                ...
                // respawn timer
                uint32 poolid =
                    m_spawnId ? sPoolMgr->IsPartOfAPool<GameObject>(m_spawnId)
                              : 0;
                if (poolid)
        ...
        if (isSpawned()) {
            ...
            if (goInfo->type == GAMEOBJECT_TYPE_TRAP) {
                ...
                /// @todo This is activation radius. Casting radius must be
                /// selected from spell data.
                /// @todo Move activated state code to GO_ACTIVATED, in this
                /// place just check for activation and set state.
                float radius = float(goInfo->trap.diameter) * 0.5f;
                if (!goInfo->trap.diameter) {
                    // Cast in other case (at some triggering/linked go/etc
                    // explicit call)
                ...
                // Type 0 and 1 - trap (type 0 will not get removed after
                // casting a spell)
                Unit* owner = GetOwner();
                Unit* target =
                    nullptr; // pointer to appropriate target if found any

                // Note: this hack with search required until GO casting not
                // implemented search unfriendly creature
                if (owner && goInfo->trap.autoCloseTime != -1) // hunter trap
                {
                    Acore::NearestAttackableNoTotemUnitInObjectRangeCheck
                        checker(this, owner, radius);
                    Acore::UnitSearcher<
                        Acore::NearestAttackableNoTotemUnitInObjectRangeCheck>
                        searcher(this, target, checker);
                    Cell::VisitAllObjects(this, searcher, radius);
                }
                else // environmental trap
                {
                    // environmental damage spells already have around enemies
                    // targeting but this not help in case not existed GO
                    // casting support affect only players
                    Player*                            player = nullptr;
                    Acore::AnyPlayerInObjectRangeCheck checker(
                        this, radius, true, true);
                    Acore::PlayerSearcher<Acore::AnyPlayerInObjectRangeCheck>
                        searcher(this, player, checker);
    ...
    case GO_ACTIVATED: {
        ...
        case GAMEOBJECT_TYPE_BUTTON:
            if (GetGOInfo()->GetAutoCloseTime() &&
                GameTime::GetGameTimeMS().count() >= m_cooldownTime)
                ResetDoorOrButton();
            break;
        ...
        case GAMEOBJECT_TYPE_CHEST:
            if (m_groupLootTimer) {
                if (m_groupLootTimer <= diff) {
                    Group* group =
                        sGroupMgr->GetGroupByGUID(lootingGroupLowGUID);
                    if (group)
            ...
            // Non-consumable chest was partially looted and restock time
            // passed, restock all loot now
            if (GetGOInfo()->chest.consumable == 0 &&
                GameTime::GetGameTime() >= m_restockTime) {
                m_restockTime = 0s;
                m_lootState   = GO_READY;
                AddToObjectUpdateIfNeeded();
            }
        ...
        case GAMEOBJECT_TYPE_TRAP: {
            ...
            if (goInfo->trap.type == 2) {
                if (goInfo->trap.spellId)
                    CastSpell(nullptr,
                              goInfo->trap.spellId); // FIXME: null target won't
                                                     // work for target type 1
                SetLootState(GO_JUST_DEACTIVATED);
            }
            else if (Unit* target =
                         ObjectAccessor::GetUnit(*this, _lootStateUnitGUID)) {
                if (goInfo->trap.spellId)
                    CastSpell(target, goInfo->trap.spellId);

                m_cooldownTime = GameTime::GetGameTimeMS().count() +
                                 (goInfo->trap.cooldown ? goInfo->trap.cooldown
                                                        : uint32(4)) *
                                     IN_MILLISECONDS; // template or 4 seconds
    ...
    case GO_JUST_DEACTIVATED: {
        ...
        if (GetGoType() == GAMEOBJECT_TYPE_GOOBER) {
            ...
            //  Xinef: Do not return here for summoned gos that should be
            //  deleted few lines below Xinef: Battleground objects are treated
            //  as spawned by default
        ...
        // Do not delete chests or goobers that are not consumed on loot, while
        // still allowing them to despawn when they expire if summoned
        bool isSummonedAndExpired =
            (GetOwner() || GetSpellId()) && m_respawnTime == 0;
        if ((GetGoType() == GAMEOBJECT_TYPE_CHEST ||
             GetGoType() == GAMEOBJECT_TYPE_GOOBER) &&
            !GetGOInfo()->IsDespawnAtAction() && !isSummonedAndExpired) {
            if (GetGoType() == GAMEOBJECT_TYPE_CHEST &&
                GetGOInfo()->chest.chestRestockTime > 0) {
                // Start restock timer when the chest is fully looted
                m_restockTime = GameTime::GetGameTime() +
                                Seconds(GetGOInfo()->chest.chestRestockTime);
...
void GameObject::DespawnOrUnsummon(Milliseconds delay, Seconds forceRespawnTime)
{
    if (delay > 0ms) {
        ...
        if (m_goData) {
            int32 const respawnDelay = (forceRespawnTime > 0s)
                                           ? forceRespawnTime.count()
                                           : m_goData->spawntimesecs;
        ...
        // Uncomment this and remove the following lines if dynamic spawn is
        // implemented. Delete();
        {
            SetLootState(GO_JUST_DEACTIVATED);
            SendObjectDeSpawnAnim(GetGUID());
            ...
            uint32 poolid =
                m_spawnId ? sPoolMgr->IsPartOfAPool<GameObject>(m_spawnId) : 0;
            if (poolid) {
                sPoolMgr->UpdatePool<GameObject>(poolid, m_spawnId);
            }
        }
...
void GameObject::GetFishLoot(Loot* fishloot, Player* loot_owner)
{
    ...
    if (fishloot->empty()) // use this becase if zone or subzone has set
                           // LOOT_MODE_JUNK_FISH,Even if no normal drop,
                           // fishloot->FillLoot return true. it wrong.
    {
        // subzone no result,use zone loot
        fishloot->FillLoot(zone, LootTemplates_Fishing, loot_owner, true, true);
        // use zone 1 as default, somewhere fishing got nothing,becase subzone
        // and zone not set, like Off the coast of Storm Peaks.
        if (fishloot->empty())
            fishloot->FillLoot(
                defaultzone, LootTemplates_Fishing, loot_owner, true, true);
    }
}
...
void GameObject::GetFishLootJunk(Loot* fishloot, Player* loot_owner)
{
    ...
    // if subzone loot exist use it
    fishloot->FillLoot(subzone,
                       LootTemplates_Fishing,
                       loot_owner,
                       true,
                       true,
                       LOOT_MODE_JUNK_FISH);
    if (fishloot->empty()) // use this becase if zone or subzone has normal mask
                           // drop, then fishloot->FillLoot return true.
    {
        // use zone loot
        fishloot->FillLoot(zone,
                           LootTemplates_Fishing,
                           loot_owner,
                           true,
                           true,
                          ...
                          bool   saveAddon /*= false*/)
{
    ...
    data.spawntimesecs =
        m_spawnedByDefault ? m_respawnDelayTime : -(int32)m_respawnDelayTime;
    ...
    WorldDatabasePreparedStatement* stmt =
        WorldDatabase.GetPreparedStatement(WORLD_DEL_GAMEOBJECT);
                                      ...
                                      bool                addToMap)
{
    ...
    if (!data) {
        LOG_ERROR("sql.sql",
                  "Gameobject (GUID: {}) not found in table `gameobject`, "
                  "can't load. ",
                  spawnId);
        return false;
    }
    ...
    if (!Create(map->GenerateLowGuid<HighGuid::GameObject>(),
                entry,
                map,
                phaseMask,
                x,
...
bool GameObject::hasInvolvedQuest(uint32 quest_id) const
{
    QuestRelationBounds qir =
        sObjectMgr->GetGOQuestInvolvedRelationBounds(GetEntry());
...
{
    return GetGOInfo() && (GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT ||
                           GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT);
}
...
{
    if (m_goData && m_goData->dbData &&
        (forceDelay || m_respawnTime > GameTime::GetGameTime().count()) &&
        m_spawnedByDefault) {
        time_t respawnTime = forceDelay
                                 ? GameTime::GetGameTime().count() + forceDelay
                                 : m_respawnTime;
...
bool GameObject::IsNeverVisible() const
{
    ...
    if (GetGoType() == GAMEOBJECT_TYPE_SPELL_FOCUS &&
        GetGOInfo()->spellFocus.serverOnly == 1)
        return true;
...
bool GameObject::ActivateToQuest(Player* target) const
{
    ...
    case GAMEOBJECT_TYPE_CHEST: {
        ...
        if (target->GetQuestStatus(GetGOInfo()->chest.questId) ==
            ...
            LootTemplates_Gameobject.HaveQuestLootForPlayer(
                GetGOInfo()->GetLootId(), target)) {
            // TODO: fix this hack
            ...
            if (GetEntry() == BG_AV_OBJECTID_MINE_N ||
                ...
                if (Battleground* bg = target->GetBattleground())
                    if (bg->GetBgTypeID(true) == BATTLEGROUND_AV &&
                        !bg->ToBattlegroundAV()->PlayerCanDoMineQuest(
                            GetEntry(), target->GetTeamId()))
                        return false;
            return true;
        }
        break;
    ...
    case GAMEOBJECT_TYPE_GENERIC: {
        if (GetGOInfo()->_generic.questID == -1 ||
            target->GetQuestStatus(GetGOInfo()->_generic.questID) ==
                QUEST_STATUS_INCOMPLETE)
            return true;
        break;
    }
    case GAMEOBJECT_TYPE_SPELL_FOCUS: {
        if (GetGOInfo()->spellFocus.questID > 0 &&
            target->GetQuestStatus(GetGOInfo()->spellFocus.questID) ==
                QUEST_STATUS_INCOMPLETE)
            return true;
        break;
    }
    case GAMEOBJECT_TYPE_GOOBER: {
        if (GetGOInfo()->goober.questId == -1 ||
            target->GetQuestStatus(GetGOInfo()->goober.questId) ==
                QUEST_STATUS_INCOMPLETE)
            return true;
        break;
    }
...
{
    GameObjectTemplate const* trapInfo =
        sObjectMgr->GetGameObjectTemplate(trapEntry);
    if (!trapInfo || trapInfo->type != GAMEOBJECT_TYPE_TRAP)
        return;

    SpellInfo const* trapSpell =
        sSpellMgr->GetSpellInfo(trapInfo->trap.spellId);
    if (!trapSpell) // checked at load already
        return;

    // xinef: many spells have range 0 but radius > 0
    float range =
        float(target->GetSpellMaxRangeForTarget(GetOwner(), trapSpell));
    if (range < 1.0f)
        range = 5.0f;
    ...
    if (GameObject* trapGO = GetLinkedTrap()) {
        trapGO->Use(
            target); // trapGO->CastSpell(target, trapInfo->trap.spellId);
    }
}
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    Acore::GameObjectSearcher<Acore::NearestGameObjectFishingHole> checker(
        this, ok, u_check);
...
void GameObject::SetGoArtKit(uint8 kit)
{
    ...
    GameObjectData* data =
        const_cast<GameObjectData*>(sObjectMgr->GetGameObjectData(m_spawnId));
    if (data)
        data->artKit = kit;
}
...
void GameObject::Use(Unit* user)
{
    ...
    if (uint32 cooldown = GetGOInfo()->GetCooldown()) {
        ...
        m_cooldownTime =
            GameTime::GetGameTimeMS().count() + cooldown * IN_MILLISECONDS;
    }
    ...
    case GAMEOBJECT_TYPE_TRAP: // 6
    {
        ...
        m_cooldownTime =
            GameTime::GetGameTimeMS().count() +
            (goInfo->trap.cooldown ? goInfo->trap.cooldown : uint32(4)) *
                IN_MILLISECONDS; // template or 4 seconds
    ...
    case GAMEOBJECT_TYPE_CHAIR: // 7
    {
        ...
        if (ChairListSlots.empty()) // this is called once at first chair use to
                                    // make list of available slots
        {
            if (info->chair.slots >
                0) // sometimes chairs in DB have error in fields and we dont
                   // know number of slots
                for (uint32 i = 0; i < info->chair.slots; ++i)
                    ChairListSlots[i].Clear(); // Last user of current slot set
                                               // to 0 (none sit here yet)
            else
        ...
        for (ChairSlotAndUser::iterator itr = ChairListSlots.begin();
             itr != ChairListSlots.end();
             ++itr) {
            // the distance between this slot and the center of the go - imagine
            // a 1D space
            ...
            float x_i =
                GetPositionX() + relativeDistance * cos(orthogonalOrientation);
            float y_i = GetPositionY() +
                        relativeDistance * std::sin(orthogonalOrientation);

            if (itr->second) {
                if (Player* ChairUser =
                        ObjectAccessor::GetPlayer(*this, itr->second)) {
                    if (ChairUser->IsSitState() &&
                        ChairUser->getStandState() != UNIT_STAND_STATE_SIT &&
                        ChairUser->GetExactDist2d(x_i, y_i) < 0.1f)
                        continue; // This seat is already occupied by ChairUser.
                    ...
                    itr->second.Clear(); // The seat may of had an occupant, but
                                         // they're offline.
            }
        ...
        if (found_free_slot) {
            ...
            if (itr != ChairListSlots.end()) {
                itr->second =
                    player->GetGUID(); // this slot in now used by player
                player->TeleportTo(GetMapId(),
                                   x_lowest,
                                   y_lowest,
                                   GetPositionZ(),
                                   GetOrientation(),
                ...
                player->SetStandState(UNIT_STAND_STATE_SIT_LOW_CHAIR +
                                      info->chair.height);
                return;
            }
        }
    ...
    case GAMEOBJECT_TYPE_GOOBER: // 10
    {
        ...
        if (user->GetTypeId() == TYPEID_PLAYER) {
            ...
            if (info->goober.eventId) {
                LOG_DEBUG(
                    "maps.script",
                    "Goober ScriptStart id {} for GO entry {} (spawnId {}).",
                    info->goober.eventId,
                    GetEntry(),
                    m_spawnId);
                GetMap()->ScriptsStart(
        ...
        m_cooldownTime =
            GameTime::GetGameTimeMS().count() + info->GetAutoCloseTime();
    ...
    case GAMEOBJECT_TYPE_FISHINGNODE: // 17
    {
        ...
        case GO_READY: // ready for loot
        {
            ...
            if (!zone_skill)
                LOG_ERROR("sql.sql",
                          "Fishable areaId {} are not properly defined in "
            ...
            LOG_DEBUG("entities.gameobject",
                      "Fishing check (skill: {} zone min skill: {} chance {} "
                      "roll: {}",
                      skill,
                      zone_skill,
                      chance,
            ...
            if (sScriptMgr->OnUpdateFishingSkill(
                    player, skill, zone_skill, chance, roll)) {
                player->UpdateFishingSkill();
            }
            // but you will likely cause junk in areas that require a high
            // fishing skill (not yet implemented)
            if (chance >= roll) {
                // TODO: I do not understand this hack. Need some explanation.
                //  prevent removing GO at spell cancel
                RemoveFromOwner();
                SetOwnerGUID(player->GetGUID());
                SetSpellId(0); // prevent removing unintended auras at
    ...
    case GAMEOBJECT_TYPE_SUMMONING_RITUAL: // 18
    {
        ...
        if (owner) {
            ...
            if (!ritualOwner)
                return;
            if (player != ritualOwner &&
                (info->summoningRitual.castersGrouped &&
                 !player->IsInSameRaidWith(ritualOwner)))
                return;
        }
        ...
        if (GetUniqueUseCount() == info->summoningRitual.reqParticipants) {
            ...
            if (!info->summoningRitual.animSpell)
                ...
                m_cooldownTime =
                    GameTime::GetGameTimeMS().count() + 5 * IN_MILLISECONDS;
        }
    ...
    case GAMEOBJECT_TYPE_SPELLCASTER: // 22
    {
        ...
        if (info->spellcaster.partyOnly) {
            Player const* caster =
                ObjectAccessor::FindConnectedPlayer(GetOwnerGUID());
            if (!caster || user->GetTypeId() != TYPEID_PLAYER ||
                !user->ToPlayer()->IsInSameRaidWith(caster))
                return;
        }
    ...
    case GAMEOBJECT_TYPE_MEETINGSTONE: // 23
    {
        ...
        // accept only use by player from same raid as caster, except caster
        // itself
    ...
    case GAMEOBJECT_TYPE_FISHINGHOLE: // 25
    {
        ...
        player->UpdateAchievementCriteria(
            ACHIEVEMENT_CRITERIA_TYPE_FISH_IN_GAMEOBJECT, GetGOInfo()->entry);
        return;
    }
    ...
    case GAMEOBJECT_TYPE_FLAGDROP: // 26
    {
        ...
        if (player->CanUseBattlegroundObject(this)) {
            ...
            GameObjectTemplate const* info = GetGOInfo();
            if (info) {
                if (GameObject::gameObjectToEventFlag.find(info->entry) !=
                    GameObject::gameObjectToEventFlag.end()) {
                    GameObject::gameObjectToEventFlag[info->entry](
                        player, this, bg);
                }
                else {
                    switch (info->entry) {
    ...
    case GAMEOBJECT_TYPE_BARBER_CHAIR: // 32
    {
        ...
        player->SetStandState(UNIT_STAND_STATE_SIT_LOW_CHAIR +
                              info->barberChair.chairheight);
        return;
    }
    default:
        if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
            LOG_ERROR("entities.gameobject",
                      "GameObject::Use(): unit ({}, name: {}) tries to use "
                      "object ({}, name: {}) of unknown type ({})",
    ...
    if (!spellInfo) {
        if (user->GetTypeId() != TYPEID_PLAYER ||
            ...
            LOG_ERROR("entities.gameobject",
                      "WORLD: unknown spell id {} at use action for gameobject "
                      "(Entry: {} GoType: {})",
                      spellId,
                      GetEntry(),
                      GetGoType());
        else
            LOG_DEBUG("outdoorpvp",
                      "WORLD: {} non-dbc spell was handled by OutdoorPvP",
                      spellId);
        return;
    }
...
void GameObject::CastSpell(Unit* target, uint32 spellId)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        if (spellInfo->Effects[i].TargetA.GetReferenceType() !=
    ...
    Creature* trigger = SummonTrigger(GetPositionX(),
                                      GetPositionY(),
                                      GetPositionZ(),
                                      0,
                                      spellInfo->CalcCastTime() + 2000,
                                      true);
    if (!trigger)
        return;
    ...
    if (Unit* owner = GetOwner()) {
        ...
        trigger->CastSpell(
            target ? target : trigger, spellInfo, true, 0, 0, owner->GetGUID());
    }
    else {
        // xinef: set faction of gameobject, if no faction - assume hostile
        trigger->SetFaction(GetTemplateAddon() && GetTemplateAddon()->faction
                                ? GetTemplateAddon()->faction
                                : 14);
        // Set owner guid for target if no owner availble - needed by trigger
        // auras
        // - trigger gets despawned and there's no caster avalible (see
        // AuraEffect::TriggerSpell()) xinef: set proper orientation, fixes cast
        // against stealthed targets
        if (target)
            trigger->SetInFront(target);
        trigger->CastSpell(target ? target : trigger,
                           spellInfo,
                           true,
                           0,
                           0,
...
bool GameObject::IsInRange(float x, float y, float z, float radius) const
{
    GameObjectDisplayInfoEntry const* info =
        sGameObjectDisplayInfoStore.LookupEntry(m_goInfo->displayId);
    if (!info)
    ...
    //! Check if the distance between the 2 objects is 0, can happen if both
    //! objects are on the same position. The code below this check wont crash
    //! if dist is 0 because 0/0 in float operations is valid, and returns
    //! infinite
    ...
    return dx < (info->maxX * scale) + radius &&
                                         ...
                                         Player const* skipped_rcvr) const
{
    ...
    if (includeMargin)
        dist +=
            VISIBILITY_COMPENSATION * 2.0f; // pussywizard: to ensure everyone
...
std::string const& GameObject::GetNameForLocaleIdx(LocaleConstant loc_idx) const
{
    if (loc_idx != DEFAULT_LOCALE) {
        ...
        if (GameObjectLocale const* cl =
                sObjectMgr->GetGameObjectLocale(GetEntry()))
...
void GameObject::SetLocalRotation(G3D::Quat const& rot)
{
    ...
    if (G3D::fuzzyEq(rot.z, 0.f) && G3D::fuzzyEq(rot.w, 0.f))
        rotation = G3D::Quat::fromAxisAngleRotation(G3D::Vector3::unitZ(),
                                                    GetOrientation());
    else
        rotation = rot;
...
{
    SetLocalRotation(
        G3D::Quat(G3D::Matrix3::fromEulerAnglesZYX(z_rot, y_rot, x_rot)));
}
                              ...
                              uint32 spellId /*= 0*/)
{
    ...
    sScriptMgr->OnGameObjectModifyHealth(
        this, attackerOrHealer, change, sSpellMgr->GetSpellInfo(spellId));
    ...
    else if (int32(m_goValue.Building.Health) + change >=
             int32(m_goValue.Building.MaxHealth))
        m_goValue.Building.Health = m_goValue.Building.MaxHealth;
    else
        m_goValue.Building.Health += change;

    // Set the health bar, value = 255 * healthPct;
    SetGoAnimProgress(m_goValue.Building.Health * 255 /
                      m_goValue.Building.MaxHealth);
    ...
    if (player) {
        ...
        data << uint32(-change); // change  < 0 triggers SPELL_BUILDING_HEAL
                                 // combat log event
...
{
    // the user calling this must know he is already operating on destructible
    // gameobject
    ...
    case GO_DESTRUCTIBLE_DAMAGED: {
        ...
        if (BattlegroundMap* bgMap = GetMap()->ToBattlegroundMap())
            if (Battleground* bg = bgMap->GetBG())
                bg->EventPlayerDamagedGO(
                    eventInvoker, this, m_goInfo->building.damagedEvent);

        RemoveGameObjectFlag(GO_FLAG_DESTROYED);
        SetGameObjectFlag(GO_FLAG_DAMAGED);

        uint32 modelId = m_goInfo->building.damagedDisplayId;
        if (DestructibleModelDataEntry const* modelData =
                sDestructibleModelDataStore.LookupEntry(
                    m_goInfo->building.destructibleData))
        ...
        if (setHealth) {
            ...
            // in this case current health is 0 anyway so just prevent crashing
            // here
            if (!maxHealth)
                maxHealth = 1;
    ...
    case GO_DESTRUCTIBLE_DESTROYED: {
        ...
        if (BattlegroundMap* bgMap = GetMap()->ToBattlegroundMap()) {
            if (Battleground* bg = bgMap->GetBG()) {
                bg->EventPlayerDamagedGO(
                    eventInvoker, this, m_goInfo->building.destroyedEvent);
...
void GameObject::SetLootState(LootState state, Unit* unit)
{
    ...
    if (GetGoType() == GAMEOBJECT_TYPE_CHEST && state == GO_ACTIVATED &&
        GetGOInfo()->chest.chestRestockTime > 0 && m_restockTime == 0s) {
        m_restockTime = GameTime::GetGameTime() +
                        Seconds(GetGOInfo()->chest.chestRestockTime);
    }

    // pussywizard: lootState has nothing to do with collision, it depends
    // entirely on GOState. Loot state is for timed close/open door and
    // respawning, which then sets GOState
    /*if (m_model)
    {
        // startOpen determines whether we are going to add or remove the LoS on
    activation bool startOpen = (GetGoType() == GAMEOBJECT_TYPE_DOOR ||
    GetGoType() == GAMEOBJECT_TYPE_BUTTON ? GetGOInfo()->door.startOpen :
    false);
...
void GameObject::SetGoState(GOState state)
{
    ...
    if (m_model) {
        ...
        // pussywizard: this startOpen is unneeded here, collision depends
        // entirely on current GOState
        EnableCollision(state == GO_STATE_READY || IsTransport());
        // pussywizard: commented out everything below

        // startOpen determines whether we are going to add or remove the LoS on
        // activation
        /*bool startOpen = (GetGoType() == GAMEOBJECT_TYPE_DOOR || GetGoType()
        == GAMEOBJECT_TYPE_BUTTON ? GetGOInfo()->door.startOpen : false);

        if (GetGameObjectData() && GetGameObjectData()->go_state ==
        GO_STATE_READY) startOpen = !startOpen;
...
void GameObject::SaveStateToDB()
{
    if (IsInstanceGameobject()) {
        if (InstanceScript* instance = GetInstanceScript()) {
            ...
            instance->StoreGameObjectState(GetSpawnId(),
                                           GameobjectStateToInt(&param));
...
void GameObject::EnableCollision(bool enable)
{
    ...
    uint32 phaseMask = 0;
    if (enable &&
        !DisableMgr::IsDisabledFor(DISABLE_TYPE_GO_LOS, GetEntry(), nullptr))
                                   ...
                                   Player*     target)
{
    ...
    bool forcedFlags = GetGoType() == GAMEOBJECT_TYPE_CHEST &&
                       GetGOInfo()->chest.groupLootRules && HasLootRecipient();
    bool targetIsGM =
        target->IsGameMaster() && target->GetSession()->IsGMAccount();
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index) {
        if (_fieldNotifyFlags & flags[index] ||
            ...
            if (index == GAMEOBJECT_DYNAMIC) {
                ...
                case GAMEOBJECT_TYPE_GENERIC:
                    if (ActivateToQuest(target) &&
                        sWorld->getBoolConfig(CONFIG_OBJECT_SPARKLES))
                        dynFlags |= GO_DYNFLAG_LO_SPARKLE;
                    break;
                case GAMEOBJECT_TYPE_TRANSPORT:
                    if (const StaticTransport* t = ToStaticTransport())
                        if (t->GetPauseTime()) {
                            if (GetGoState() == GO_STATE_READY) {
                                if (t->GetPathProgress() >=
                                    t->GetPauseTime()) // if not, send 100%
                                                       // progress
                                    pathProgress =
                                        int16(float(t->GetPathProgress() -
                                                    t->GetPauseTime()) /
                                              float(t->GetPeriod() -
                                                    t->GetPauseTime()) *
                                              65535.0f);
                                ...
                                if (t->GetPathProgress() <=
                                    t->GetPauseTime()) // if not, send 100%
                                                       // progress
                                    pathProgress = int16(
                                        float(t->GetPathProgress()) /
                                        float(t->GetPauseTime()) * 65535.0f);
                            }
                        }
                    // else it's ignored
                    break;
                case GAMEOBJECT_TYPE_MO_TRANSPORT:
                    if (const MotionTransport* t = ToMotionTransport())
                        pathProgress = int16(float(t->GetPathProgress()) /
                                             float(t->GetPeriod()) * 65535.0f);
                    break;
                default:
                    break;
                }
                ...
                if (GetGoType() == GAMEOBJECT_TYPE_CHEST && GetGOInfo() &&
                    GetGOInfo()->chest.groupLootRules &&
...
void GameObject::GetRespawnPosition(float& x,
                                    float& y,
                                    float& z,
                                    float* ori /* = nullptr*/) const
{
    if (m_spawnId) {
        if (GameObjectData const* data =
                sObjectMgr->GetGameObjectData(m_spawnId)) {
            x = data->posX;
            y = data->posY;
            z = data->posZ;
            if (ori)
...
{
    // pussywizard: do not call for MotionTransport and other gobjects not in
    // grid
...
float GameObject::GetInteractionDistance() const
{
    ...
    case GAMEOBJECT_TYPE_MAILBOX:
        // Successful mailbox interaction is rather critical to the client,
        // failing it will start a minute-long cooldown until the next mail
        // query may be executed. And since movement info update is not sent
        // with mailbox interaction query, server may find the player outside of
        // interaction range. Thus we increase it.
...
public:
    ...
    {
        return (_owner->GetGoState() == GO_STATE_READY || _owner->IsTransport())
                   ? _owner->GetPhaseMask()
                   : 0;
    }
    G3D::Vector3 GetPosition() const override
    {
        return G3D::Vector3(_owner->GetPositionX(),
                            _owner->GetPositionY(),
                            _owner->GetPositionZ());
    }
    ...
    {
        const_cast<GameObject*>(_owner)->SummonCreature(
            1,
            corner.x,
            corner.y,
            corner.z,
...
{
    return GameObjectModel::Create(
        std::make_unique<GameObjectModelOwnerImpl>(this),
        sWorld->GetDataPath());
}
...
bool GameObject::IsAtInteractDistance(Position const& pos, float radius) const
{
    if (GameObjectDisplayInfoEntry const* displayInfo =
            sGameObjectDisplayInfoStore.LookupEntry(GetGOInfo()->displayId)) {
        float scale = GetObjectScale();
...
{
    return IsInMap(player) && InSamePhase(player) &&
           IsAtInteractDistance(player);
}
...
SpellInfo const* GameObject::GetSpellForLock(Player const* player) const
{
    ...
    for (uint8 i = 0; i < MAX_LOCK_CASE; ++i) {
        ...
        if (lock->Type[i] == LOCK_KEY_SPELL) {
            if (SpellInfo const* spell =
                    sSpellMgr->GetSpellInfo(lock->Index[i])) {
                return spell;
            }
        }
        ...
        for (auto&& playerSpell : player->GetSpellMap()) {
            if (SpellInfo const* spell =
                    sSpellMgr->GetSpellInfo(playerSpell.first)) {
                for (auto&& effect : spell->Effects) {
                    if (effect.Effect == SPELL_EFFECT_OPEN_LOCK &&
                        ((uint32)effect.MiscValue) == lock->Index[i]) {
...
std::string GameObject::GetDebugInfo() const
{
    ...
    sstr << WorldObject::GetDebugInfo() << "\n"
         << "SpawnId: " << GetSpawnId()
         << " GoState: " << std::to_string(GetGoState())
