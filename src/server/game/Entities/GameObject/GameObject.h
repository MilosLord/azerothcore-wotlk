...
typedef std::unordered_map<uint32, GameObjectTemplate>
    GameObjectTemplateContainer;
typedef std::unordered_map<uint32, GameObjectTemplateAddon>
...
typedef std::unordered_map<uint32, GameObjectQuestItemList>
...
// For containers:  [GO_NOT_READY]->GO_READY (close)->GO_ACTIVATED (open)
// ->GO_JUST_DEACTIVATED->GO_READY        -> ... For bobber:      GO_NOT_READY
// ->GO_READY (close)->GO_ACTIVATED (open) ->GO_JUST_DEACTIVATED-><deleted> For
// door(closed):[GO_NOT_READY]->GO_READY (close)->GO_ACTIVATED (open)
// ->GO_JUST_DEACTIVATED->GO_READY(close) -> ... For door(open):
// [GO_NOT_READY]->GO_READY (open) ->GO_ACTIVATED
// (close)->GO_JUST_DEACTIVATED->GO_READY(open)  -> ...
enum LootState {
    GO_NOT_READY,
    GO_READY, // can be ready but despawned, and then not possible activate
              // until spawn
    GO_ACTIVATED,
    GO_JUST_DEACTIVATED
};
...
public:
    ...
    {
        return GetUInt32Value(GAMEOBJECT_DYNAMIC);
    }
    void ReplaceAllDynamicFlags(uint32 flag) override
    {
        SetUInt32Value(GAMEOBJECT_DYNAMIC, flag);
    }

    virtual bool                            Create(ObjectGuid::LowType guidlow,
                                                   uint32              name_id,
                                                   Map*                map,
                                                   uint32              phaseMask,
                                                   float               x,
    ...
    [[nodiscard]] GameObjectTemplate const* GetGOInfo() const
    {
        return m_goInfo;
    }
    [[nodiscard]] GameObjectTemplateAddon const* GetTemplateAddon() const;
    [[nodiscard]] GameObjectData const*          GetGameObjectData() const
    {
        return m_goData;
    }
    [[nodiscard]] GameObjectValue const* GetGOValue() const
    {
        return &m_goValue;
    }
    ...
    [[nodiscard]] G3D::Quat const& GetLocalRotation() const
    {
        return m_localRotation;
    }
    [[nodiscard]] int64 GetPackedLocalRotation() const
    {
        return m_packedRotation;
    }
    [[nodiscard]] G3D::Quat GetWorldRotation() const;

    // overwrite WorldObject function for proper name localization
    [[nodiscard]] std::string const&
    GetNameForLocaleIdx(LocaleConstant locale_idx) const override;
    ...
    bool LoadGameObjectFromDB(ObjectGuid::LowType guid,
                              Map*                map,
                              bool                addToMap = true);
    void DeleteFromDB();
    ...
    {
        // Owner already found and different than expected owner - remove object
        // from old owner
    ...
    {
        return GetGuidValue(OBJECT_FIELD_CREATED_BY);
    }
    ...
    void                 Refresh();
    void                 DespawnOrUnsummon(Milliseconds delay             = 0ms,
                                           Seconds      forcedRespawnTime = 0s);
    void                 Delete();
    void                 GetFishLoot(Loot* loot, Player* loot_owner);
    void                 GetFishLootJunk(Loot* loot, Player* loot_owner);
    [[nodiscard]] GameobjectTypes GetGoType() const
    {
        return GameobjectTypes(GetByteValue(GAMEOBJECT_BYTES_1, 1));
    }
    ...
    {
        return GOState(GetByteValue(GAMEOBJECT_BYTES_1, 0));
    }
    void                SetGoState(GOState state);
    [[nodiscard]] uint8 GetGoArtKit() const
    {
        return GetByteValue(GAMEOBJECT_BYTES_1, 2);
    }
    void                SetGoArtKit(uint8 artkit);
    [[nodiscard]] uint8 GetGoAnimProgress() const
    {
        return GetByteValue(GAMEOBJECT_BYTES_1, 3);
    }
    ...
    static void
    SetGoArtKit(uint8 artkit, GameObject* go, ObjectGuid::LowType lowguid = 0);
    ...
    {
        return GameObjectFlags(GetUInt32Value(GAMEOBJECT_FLAGS));
    }
    bool HasGameObjectFlag(GameObjectFlags flags) const
    {
        return HasFlag(GAMEOBJECT_FLAGS, flags) != 0;
    }
    void SetGameObjectFlag(GameObjectFlags flags)
    {
        SetFlag(GAMEOBJECT_FLAGS, flags);
    }
    void RemoveGameObjectFlag(GameObjectFlags flags)
    {
        RemoveFlag(GAMEOBJECT_FLAGS, flags);
    }
    ...
    [[nodiscard]] uint32 GetUniqueUseCount() const
    {
    ...
    [[nodiscard]] uint32 GetLootGenerationTime() const
    {
        return m_lootGenerationTime;
    }
    ...
    void               UseDoorOrButton(uint32 time_to_restore = 0,
    ...
    void ModifyHealth(int32  change,
                      Unit*  attackerOrHealer = nullptr,
                      uint32 spellId          = 0);
    void SetDestructibleBuildingModifyState(bool allow)
    {
        m_allowModifyDestructibleBuilding = allow;
    }
    // sets GameObject type 33 destruction flags and optionally default health
    // for that state
    void SetDestructibleState(GameObjectDestructibleState state,
    ...
    {
        return GetUInt32Value(GAMEOBJECT_DISPLAYID);
    }

    GameObjectModel* m_model;
    void             GetRespawnPosition(float& x,
                                        float& y,
                                        float& z,
    ...
    {
        SetPosition(pos.GetPositionX(),
                    pos.GetPositionY(),
                    pos.GetPositionZ(),
                    pos.GetOrientation());
    }

    [[nodiscard]] bool IsStaticTransport() const
    {
        return GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT;
    }
    [[nodiscard]] bool IsMotionTransport() const
    {
        return GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT;
    }

    Transport* ToTransport()
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT ||
            GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT)
            return reinterpret_cast<Transport*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] Transport const* ToTransport() const
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT ||
            GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT)
    ...
    StaticTransport* ToStaticTransport()
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT)
            return reinterpret_cast<StaticTransport*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] StaticTransport const* ToStaticTransport() const
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT)
            return reinterpret_cast<StaticTransport const*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT)
            return reinterpret_cast<MotionTransport*>(this);
        else
            return nullptr;
    }
    [[nodiscard]] MotionTransport const* ToMotionTransport() const
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT)
            return reinterpret_cast<MotionTransport const*>(this);
        else
            return nullptr;
    }

    [[nodiscard]] float GetStationaryX() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
            return m_stationaryPosition.GetPositionX();
        return GetPositionX();
    }
    [[nodiscard]] float GetStationaryY() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
            return m_stationaryPosition.GetPositionY();
        return GetPositionY();
    }
    [[nodiscard]] float GetStationaryZ() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
            return m_stationaryPosition.GetPositionZ();
        return GetPositionZ();
    }
    [[nodiscard]] float GetStationaryO() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
    ...
    [[nodiscard]] bool IsAtInteractDistance(Position const& pos,
                                            float           radius) const;
    [[nodiscard]] bool
    IsAtInteractDistance(Player const*    player,
                         SpellInfo const* spell = nullptr) const;
    ...
    static std::unordered_map<int, goEventFlag>
        gameObjectToEventFlag; // Gameobject -> event flag
    ...
    /* Enable or Disable the ability to save on the database this gameobject's
...
protected:
    ...
    uint32 m_spellId;
    time_t m_respawnTime; // (secs) time of next respawn (or despawn if GO have
                          // owner()),
    uint32 m_respawnDelayTime; // (secs) if 0 then current GO state no dependent
                               // from timer
    ...
    uint32    m_cooldownTime; // used as internal reaction delay time store (not
                              // state change reaction).
    // For traps this: spell casting cooldown, for doors/buttons: reset time.
    std::unordered_map<ObjectGuid, int32> m_SkillupList;

    ObjectGuid m_ritualOwnerGUID; // used for GAMEOBJECT_TYPE_SUMMONING_RITUAL
                                  // where GO is not summoned (no owner)
    ...
    ObjectGuid::LowType m_spawnId; ///< For new or temporary gameobjects is 0
                                   ///< for saved it is lowguid
    ...
    uint16 m_LootMode; // bitmask, default LOOT_MODE_DEFAULT, determines what
                       // loot will be lootable
...
private:
    ...
    //! Object distance/size - overridden from Object::_IsWithinDist. Needs to
    //! take in account proper GO size.
    bool _IsWithinDist(WorldObject const* obj,
                       float              dist2compare,
                       bool /*is3D*/,
                       bool /*useBoundingRadius = true*/) const override
    {
        //! Following check does check 3d distance
        dist2compare += obj->GetObjectSize();
        return IsInRange(obj->GetPositionX(),
                         obj->GetPositionY(),
                         obj->GetPositionZ(),
                         dist2compare);
    }
