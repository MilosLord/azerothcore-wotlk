...
struct GameObjectTemplate {
    ...
    union // different GO types have different data field
    {
        ...
        struct {
            uint32 startOpen; // 0 used client side to determine GO_ACTIVATED
                              // means open/closed
            uint32 lockId;        // 1 -> Lock.dbc
            uint32 autoCloseTime; // 2 secs till autoclose = autoCloseTime /
                                  // 0x10000
            uint32
                noDamageImmune; // 3 break opening whenever you recieve damage?
        ...
        struct {
            ...
            uint32 consumable;       // 3
            uint32 minSuccessOpens; // 4 Deprecated, pre 3.0 was used for mining
                                    // nodes but since WotLK all mining nodes
                                    // are usable once and grant all loot with a
                                    // single use
            uint32 maxSuccessOpens; // 5 Deprecated, pre 3.0 was used for mining
                                    // nodes but since WotLK all mining nodes
                                    // are usable once and grant all loot with a
                                    // single use
            uint32 eventId;      // 6 lootedEvent
            uint32 linkedTrapId; // 7
            uint32 questId; // 8 not used currently but store quest required for
                            // GO activation for player
            uint32 level;           // 9
            uint32 losOK;           // 10
            uint32 leaveLoot;       // 11
        ...
        struct {
            ...
            uint32 spellId;  // 3
            uint32 type; // 4 0 trap with no despawn after cast. 1 trap despawns
                         // after cast. 2 bomb casts on spawn.
        ...
        struct {
            ...
            uint32 autoCloseTime; // 5 secs till autoclose = autoCloseTime /
                                  // 0x10000
            uint32 openTextID;  // 6
            uint32 closeTextID; // 7
        } areadamage;
        ...
        // not use for specific field access (only for output with loop by all
        // filed), also this determinate max union size
        struct {
    ...
    [[nodiscard]] bool
    GetDespawnPossibility() const // despawn at targeting of cast?
    {
        switch (type) {
        case GAMEOBJECT_TYPE_DOOR:
    ...
    [[nodiscard]] uint32 GetAutoCloseTime() const
    {
        ...
        return autoCloseTime /* xinef: changed to milliseconds/
                                IN_MILLISECONDS*/
    ...
    [[nodiscard]] uint32
    GetCooldown() const // Cooldown preventing goober and traps to cast spell
    {
        switch (type) {
        case GAMEOBJECT_TYPE_TRAP:
    ...
    [[nodiscard]] bool IsGameObjectForQuests() const { return IsForQuests; }
...
enum GOState {
    ...
    GO_STATE_ACTIVE_ALTERNATIVE = 2 // show in world as used in alt way and not
                                    // reset (closed door open by cannon fire)
};
