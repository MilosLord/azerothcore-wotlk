...
Totem::Totem(SummonPropertiesEntry const* properties, ObjectGuid owner)
...
{
    // client requires SMSG_TOTEM_CREATED to be sent before adding to world and
    // before removing old totem Xinef: Set level for Unit totems
    if (Unit* owner = ObjectAccessor::GetUnit(*this, m_owner)) {
        uint32 slot = m_Properties->Slot;
        if (owner->GetTypeId() == TYPEID_PLAYER && slot >= SUMMON_SLOT_TOTEM &&
            slot < MAX_TOTEM_SLOT) {
            ...
            SetDisplayId(
                owner->GetModelForTotem(PlayerTotemType(m_Properties->Id)));
        }
    ...
    if (SpellInfo const* totemSpell = sSpellMgr->GetSpellInfo(GetSpell()))
        if (totemSpell->CalcCastTime()) // If spell has cast time -> its an
                                        // active totem
            m_type = TOTEM_ACTIVE;
...
void Totem::InitSummon()
{
    ...
    if (m_type == TOTEM_PASSIVE && GetSpell()) {
        if (TotemSpellIds(GetUInt32Value(UNIT_CREATED_BY_SPELL)) ==
            TotemSpellIds::FireTotemSpell) {
            m_Events.AddEventAtOffset(
                [this]() { CastSpell(this, GetSpell(), true); }, 4s);
        }
        else {
...
{
    if (msTime) {
        m_Events.AddEvent(new ForcedUnsummonDelayEvent(*this),
                          m_Events.CalculateTime(msTime));
        return;
    }
    ...
    if (Unit* owner = GetOwner()) {
        ...
        if (GetEntry() == SENTRY_TOTEM_ENTRY)
            owner->RemoveAurasDueToSpell(
                static_cast<uint32>(TotemSpellIds::SentryTotemSpell));
        ...
        if (Player* player = owner->ToPlayer()) {
            ...
            if (Group* group = player->GetGroup()) {
                for (GroupReference* itr = group->GetFirstMember();
                    ...
                    if (target && target->IsInMap(player) &&
                        group->SameSubGroup(player, target))
...
bool Totem::IsImmunedToSpellEffect(SpellInfo const* spellInfo,
                                   uint32           index) const
{
    // xinef: immune to all positive spells, except of stoneclaw totem absorb
    // and sentry totem bind sight totems positive spells have unit_caster
    // target
    if (spellInfo->Effects[index].Effect != SPELL_EFFECT_DUMMY &&
        ...
        spellInfo->Effects[index].TargetA.GetCheckType() !=
            TARGET_CHECK_ENTRY &&
        spellInfo->Id != 55277 && spellInfo->Id != 6277)
        return true;
