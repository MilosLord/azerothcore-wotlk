...
#define GUID_TRAIT_GLOBAL(highguid)                                            \
    template <>                                                                \
    struct ObjectGuidTraits<highguid> {                                        \
...
public:
    ...
    static typename std::enable_if<ObjectGuidTraits<type>::Global,
                                   ObjectGuid>::type
    ...
    template <HighGuid type>
    static typename std::enable_if<ObjectGuidTraits<type>::MapSpecific,
                                   ObjectGuid>::type
    Create(uint32 entry, LowType counter)
    {
        return MapSpecific(type, entry, counter);
    }

    ObjectGuid() = default;
    explicit ObjectGuid(uint64 guid) : _guid(guid) {}
    ObjectGuid(HighGuid hi, uint32 entry, LowType counter)
        : _guid(counter ? uint64(counter) | (uint64(entry) << 24) |
                              (uint64(hi) << 48)
                        : 0)
    {
    }
    ...
    [[nodiscard]] HighGuid GetHigh() const
    {
        return HighGuid((_guid >> 48) & 0x0000FFFF);
    }
    [[nodiscard]] uint32 GetEntry() const
    {
        return HasEntry() ? uint32((_guid >> 24) & UI64LIT(0x0000000000FFFFFF))
                          : 0;
    }
    [[nodiscard]] LowType GetCounter() const
    {
        return HasEntry() ? LowType(_guid & UI64LIT(0x0000000000FFFFFF))
                          : LowType(_guid & UI64LIT(0x00000000FFFFFFFF));
    }

    static LowType GetMaxCounter(HighGuid high)
    {
        return HasEntry(high) ? LowType(0x00FFFFFF) : LowType(0xFFFFFFFF);
    }

    [[nodiscard]] ObjectGuid::LowType GetMaxCounter() const
    {
    ...
    [[nodiscard]] bool IsCreatureOrPet() const
    {
        return IsCreature() || IsPet();
    }
    [[nodiscard]] bool IsCreatureOrVehicle() const
    {
        return IsCreature() || IsVehicle();
    }
    [[nodiscard]] bool IsAnyTypeCreature() const
    {
    ...
    static TypeID GetTypeId(HighGuid high)
    {
        ...
        case HighGuid::Item:
            return TYPEID_ITEM;
        // case HighGuid::Container:    return TYPEID_CONTAINER;
        // HighGuid::Container == HighGuid::Item currently
        case HighGuid::Unit:
            return TYPEID_UNIT;
        case HighGuid::Pet:
            return TYPEID_UNIT;
...
private:
    ...
    explicit ObjectGuid(uint32 const&) =
        delete; // no implementation, used to catch wrong type assignment
    ObjectGuid(HighGuid, uint32, uint64 counter) =
        delete; // no implementation, used to catch wrong type assignment
    ObjectGuid(HighGuid, uint64 counter) =
        delete; // no implementation, used to catch wrong type assignment
...
public:
    explicit PackedGuid() : _packedGuid(PACKED_GUID_MIN_BUFFER_SIZE)
    {
        _packedGuid.appendPackGUID(0);
    }
    explicit PackedGuid(uint64 guid) : _packedGuid(PACKED_GUID_MIN_BUFFER_SIZE)
    {
        _packedGuid.appendPackGUID(guid);
    }
    explicit PackedGuid(ObjectGuid guid)
        : _packedGuid(PACKED_GUID_MIN_BUFFER_SIZE)
    {
        _packedGuid.appendPackGUID(guid.GetRawValue());
    }
...
public:
    ...
    [[nodiscard]] ObjectGuid::LowType GetNextAfterMaxUsed() const
    {
        return _nextGuid;
    }
...
public:
    explicit ObjectGuidGenerator(ObjectGuid::LowType start = 1)
