...
enum TempSummonType {
    ...
    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT =
        4, // despawns after a specified time after the creature is out of
           // combat
    ...
    TEMPSUMMON_TIMED_DESPAWN_OOC_ALIVE =
        10, // despawns after a specified time after the creature is out of
            // combat and alive
};
...
enum GOSummonType {
    GO_SUMMON_TIMED_OR_CORPSE_DESPAWN =
        0, // despawns after a specified time OR when the summoner dies
...
public:
    ...
    [[nodiscard]] static ObjectGuid GetGUID(Object const* o)
    {
        return o ? o->GetGUID() : ObjectGuid::Empty;
    }
    ...
    [[nodiscard]] float GetObjectScale() const
    {
    ...
    {
        return (GetDynamicFlags() & flag) != 0;
    }
    virtual void SetDynamicFlag(uint32 flag)
    {
        ReplaceAllDynamicFlags(GetDynamicFlags() | flag);
    }
    ...
    [[nodiscard]] bool   isType(uint16 mask) const
    {
    ...
    [[nodiscard]] bool
    HasByteFlag(uint16 index, uint8 offset, uint8 flag) const;
    ...
    [[nodiscard]] virtual bool hasQuest(uint32 /* quest_id */) const
    {
        return false;
    }
    [[nodiscard]] virtual bool hasInvolvedQuest(uint32 /* quest_id */) const
    {
        return false;
    }
    ...
    [[nodiscard]] inline bool IsPlayer() const
    {
    ...
    [[nodiscard]] Player const* ToPlayer() const
    {
        if (GetTypeId() == TYPEID_PLAYER)
            return (Player const*)((Player*)this);
        else
            return nullptr;
    }
    Creature* ToCreature()
    ...
    [[nodiscard]] Creature const* ToCreature() const
    {
        if (GetTypeId() == TYPEID_UNIT)
            return (Creature const*)((Creature*)this);
        else
            return nullptr;
    }
    ...
    [[nodiscard]] Unit const* ToUnit() const
    {
        if (GetTypeId() == TYPEID_UNIT || GetTypeId() == TYPEID_PLAYER)
            return (Unit const*)((Unit*)this);
        else
            return nullptr;
    }
    ...
    [[nodiscard]] GameObject const* ToGameObject() const
    {
        if (GetTypeId() == TYPEID_GAMEOBJECT)
            return (GameObject const*)((GameObject*)this);
        else
            return nullptr;
    }
    ...
    [[nodiscard]] Corpse const* ToCorpse() const
    {
        if (GetTypeId() == TYPEID_CORPSE)
    ...
    [[nodiscard]] DynamicObject const* ToDynObject() const
    {
        if (GetTypeId() == TYPEID_DYNAMICOBJECT)
...
protected:
    ...
    [[nodiscard]] std::string _ConcatFields(uint16 startIndex,
                                            uint16 size) const;
    bool                      _LoadIntoDataField(std::string const& data,
                                                 uint32             startOffset,
                                                 uint32             count);
    ...
    virtual void
    BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target);
...
struct MovementInfo {
    ...
    [[nodiscard]] bool   HasMovementFlag(uint32 flag) const
    {
        return flags & flag;
    }
    ...
    [[nodiscard]] bool   HasExtraMovementFlag(uint16 flag) const
    {
        return flags2 & flag;
    }
...
public:
    ...
    [[nodiscard]] bool    HasFlag(FLAG_TYPE flag) const
    {
    ...
    [[nodiscard]] T_VALUES GetValue(FLAG_TYPE flag) const
    {
...
protected:
    explicit WorldObject(
        bool isWorldObject); // note: here it means if it is in grid object list
                             // or world object list
public:
    ...
    void
    _Create(ObjectGuid::LowType guidlow, HighGuid guidhigh, uint32 phaseMask);

    void AddToWorld() override;
    void RemoveFromWorld() override;

    void     GetNearPoint2D(WorldObject const* searcher,
                            float&             x,
                            float&             y,
                            float              distance,
                            float              absAngle,
                            Position const*    startPos = nullptr) const;
    void     GetNearPoint2D(float&          x,
                            float&          y,
                            float           distance,
                            float           absAngle,
                            Position const* startPos = nullptr) const;
    void     GetNearPoint(WorldObject const* searcher,
                          float&             x,
                          float&             y,
                          float&             z,
                          float              searcher_size,
                          ...
                          float              controlZ = 0,
                          Position const*    startPos = nullptr) const;
    void     GetVoidClosePoint(float& x,
                               float& y,
                               float& z,
                               float  size,
                               float  distance2d = 0,
    ...
    [[nodiscard]] virtual float GetCombatReach() const
    {
        return 0.0f;
    } // overridden (only) in Unit
    void UpdateGroundPositionZ(float x, float y, float& z) const;
    void UpdateAllowedPositionZ(float  x,
                                float  y,
                                float& z,
                                float* groundZ = nullptr) const;

    void                   GetRandomPoint(const Position& srcPos,
                                          float           distance,
                                          float&          rand_x,
                                          float&          rand_y,
                                          float&          rand_z) const;
    [[nodiscard]] Position GetRandomPoint(const Position& srcPos,
    ...
    {
        return m_useCombinedPhases ? GetPhaseMask() & phasemask
    ...
    GetNameForLocaleIdx(LocaleConstant /*locale_idx*/) const
    {
        return m_name;
    }
    ...
    bool               IsWithinDist(WorldObject const* obj,
                                    float              dist2compare,
                                    bool               is3D = true,
                                    bool               useBoundingRadius = true) const;
    bool               IsWithinDistInMap(WorldObject const* obj,
                                         float              dist2compare,
                                         bool               is3D = true,
                                         bool               useBoundingRadius = true) const;
    [[nodiscard]] bool IsWithinLOS(
        float                  x,
        float                  y,
        float                  z,
        VMAP::ModelIgnoreFlags ignoreFlags = VMAP::ModelIgnoreFlags::Nothing,
        LineOfSightChecks      checks      = LINEOFSIGHT_ALL_CHECKS) const;
    [[nodiscard]] bool IsWithinLOSInMap(
    ...
    [[nodiscard]] Position
         GetHitSpherePointFor(Position const& dest,
                              Optional<float> collisionHeight = {},
                              Optional<float> combatReach     = {}) const;
    void GetHitSpherePointFor(Position const& dest,
                              float&          x,
                              float&          y,
                              float&          z,
                              Optional<float> collisionHeight = {},
                              Optional<float> combatReach     = {}) const;
    bool GetDistanceOrder(WorldObject const* obj1,
                          WorldObject const* obj2,
                          bool               is3D = true) const;
    bool IsInRange(WorldObject const* obj,
                   float              minRange,
                   float              maxRange,
                   bool               is3D = true) const;
    [[nodiscard]] bool
    IsInRange2d(float x, float y, float minRange, float maxRange) const;
    [[nodiscard]] bool IsInRange3d(
        float x, float y, float z, float minRange, float maxRange) const;
    bool isInFront(WorldObject const* target, float arc = M_PI) const;
    bool isInBack(WorldObject const* target, float arc = M_PI) const;

    bool IsInBetween(WorldObject const* obj1,
                     WorldObject const* obj2,
                     float              size = 0) const;

    virtual void CleanupsBeforeDelete(
        bool finalCleanup =
            true); // used in destructor or explicitly before mass creature
                   // delete to remove cross-references to already deleted units
    ...
    virtual void SendMessageToSetInRange(
        WorldPacket const* data,
        float              dist,
        bool /*self*/,
        bool          includeMargin = false,
        Player const* skipped_rcvr  = nullptr) const; // pussywizard!
    virtual void SendMessageToSet(WorldPacket const* data,
                                  Player const*      skipped_rcvr) const
    {
        if (IsInWorld())
            SendMessageToSetInRange(
                data, GetVisibilityRange(), false, true, skipped_rcvr);
    } // pussywizard!
    ...
    bool CanSeeOrDetect(WorldObject const* obj,
                        bool               ignoreStealth = false,
                        bool               distanceCheck = false,
                        bool               checkAlert    = false) const;

    FlaggedValuesArray32<int32, uint32, StealthType, TOTAL_STEALTH_TYPES>
        m_stealth;
    FlaggedValuesArray32<int32, uint32, StealthType, TOTAL_STEALTH_TYPES>
        m_stealthDetect;
    ...
    Creature* SummonTrigger(float  x,
                            float  y,
                            float  z,
                            float  ang,
                            uint32 dur,
    ...
    [[nodiscard]] Creature*
    FindNearestCreature(uint32 entry, float range, bool alive = true) const;
    [[nodiscard]] GameObject* FindNearestGameObject(
        uint32 entry, float range, bool onlySpawned = false) const;
    [[nodiscard]] GameObject* FindNearestGameObjectOfType(GameobjectTypes type,
                                                          float range) const;
    ...
    void GetCreatureListWithEntryInGrid(std::list<Creature*>& lList,
                                        uint32                uiEntry,
                                        float fMaxSearchRange) const;
    void GetDeadCreatureListInGrid(std::list<Creature*>& lList,
    ...
    void         BuildUpdate(UpdateDataMapType& data_map,
                             UpdatePlayerSet&   player_set) override;
    void GetCreaturesWithEntryInRange(std::list<Creature*>& creatureList,
                                      float                 radius,
                                      uint32                entry);
    ...
    [[nodiscard]] bool isNeedNotify(uint16 f) const
    {
        return m_notifyflags & f;
    }
    [[nodiscard]] uint16 GetNotifyFlags() const { return m_notifyflags; }
    [[nodiscard]] bool   NotifyExecuted(uint16 f) const
    {
    ...
    [[nodiscard]] bool IsVisibilityOverridden() const
    {
        return m_visibilityDistanceOverride.has_value();
    }
    void SetVisibilityDistanceOverride(VisibilityDistanceType type);
    void SetWorldObject(bool apply);
    [[nodiscard]] bool IsPermanentWorldObject() const
    {
    ...
    {
        return GetMapId() == 571 && GetPositionX() > 3733.33331f &&
               GetPositionX() < 5866.66663f && GetPositionY() > 1599.99999f &&
               GetPositionY() < 4799.99997f;
    }
    ...
    [[nodiscard]] virtual float GetStationaryX() const
    {
        return GetPositionX();
    }
    [[nodiscard]] virtual float GetStationaryY() const
    {
        return GetPositionY();
    }
    [[nodiscard]] virtual float GetStationaryZ() const
    {
        return GetPositionZ();
    }
    [[nodiscard]] virtual float GetStationaryO() const
    {
        return GetOrientation();
    }

    [[nodiscard]] float GetMapWaterOrGroundLevel(float  x,
                                                 float  y,
                                                 float  z,
                                                 float* ground = nullptr) const;
    [[nodiscard]] float GetMapHeight(
        float x,
        float y,
        float z,
        bool  vmap             = true,
        float distanceToSearch = 50.0f) const; // DEFAULT_HEIGHT_SEARCH in map.h
    ...
    [[nodiscard]] virtual float GetCollisionWidth() const
    {
        return GetObjectSize();
    }
    [[nodiscard]] virtual float GetCollisionRadius() const
    {
...
protected:
    ...
    virtual void
           ProcessPositionDataChanged(PositionFullTerrainStatus const& data);
    uint32 _zoneId;
    uint32 _areaId;
    float  _floorZ;
    bool   _outdoors;
    ...
    // stuff... use them ONLY in LoadFromDB()/Create() funcs and nowhere else!
    ...
    // difference from IsAlwaysVisibleFor: 1. after distance check; 2. use owner
    // or charmer as seer
...
private:
    ...
    bool   m_useCombinedPhases; // true (default): use phaseMask as bit mask
                                // combining up to 32 phases
    // false: use phaseMask to represent single phases only (up to 4294967295
    // phases)

    uint16 m_notifyflags;
    uint16 m_executed_notifies;

    virtual bool _IsWithinDist(WorldObject const* obj,
                               float              dist2compare,
                               bool               is3D,
                               bool useBoundingRadius = true) const;

    bool         CanNeverSee(WorldObject const* obj) const;
    virtual bool CanAlwaysSee(WorldObject const* /*obj*/) const
    {
        return false;
    }
    // bool CanDetect(WorldObject const* obj, bool ignoreStealth, bool
    // checkClient) const;
    bool CanDetect(WorldObject const* obj,
                   bool               ignoreStealth,
                   bool               checkClient,
                   bool               checkAlert = false) const;
    bool CanDetectInvisibilityOf(WorldObject const* obj) const;
    // bool CanDetectStealthOf(WorldObject const* obj) const;
    bool CanDetectStealthOf(WorldObject const* obj,
                            bool               checkAlert = false) const;
