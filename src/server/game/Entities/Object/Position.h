...
struct Position {
    Position(float x = 0, float y = 0, float z = 0, float o = 0)
        : m_positionX(x), m_positionY(y), m_positionZ(z),
          m_orientation(NormalizeOrientation(o))
    {
    }
    ...
    float m_positionY   = 0;
    float m_positionZ   = 0;
    float m_orientation = 0; // Better to limit access to _orientation field, to
                             // guarantee the value is normalized
    ...
    [[nodiscard]] float GetExactDist2dSq(Position const& pos) const
    {
    ...
    [[nodiscard]] float GetExactDist2d(Position const& pos) const
    {
    ...
    [[nodiscard]] float GetExactDistSq(Position const& pos) const
    {
        return GetExactDistSq(
    ...
    [[nodiscard]] float GetExactDist(Position const& pos) const
    {
    ...
    [[nodiscard]] float GetAbsoluteAngle(float x, float y) const
    {
        return NormalizeOrientation(
            std::atan2(static_cast<float>(y - m_positionY),
                       static_cast<float>(x - m_positionX)));
    }
    [[nodiscard]] float GetAbsoluteAngle(Position const& pos) const
    {
    ...
    [[nodiscard]] float GetRelativeAngle(float x, float y) const
    {
        return NormalizeOrientation(GetAngle(x, y) - m_orientation);
    }
    [[nodiscard]] float ToAbsoluteAngle(float relAngle) const
    {
    ...
    [[nodiscard]] bool        IsWithinBox(const Position& center,
                                          float           xradius,
                                          float           yradius,
                                          float           zradius) const;
    bool                      HasInArc(float           arcangle,
                                       const Position* pos,
                                       float           targetRadius = 0.0f) const;
...
public:
    explicit WorldLocation(uint32 _mapId = MAPID_INVALID,
                           float  x      = 0.f,
                           float  y      = 0.f,
                           float  z      = 0.f,
                           float  o      = 0.f)
    ...
    void WorldRelocate(uint32 mapId = MAPID_INVALID,
                       float  x     = 0.f,
                       float  y     = 0.f,
                       float  z     = 0.f,
                       float  o     = 0.f)
    ...
    void GetWorldLocation(
        uint32& mapId, float& x, float& y, float& z, float& o) const
    {
        mapId = m_mapId;
        x     = m_positionX;
        y     = m_positionY;
    ...
    void GetWorldLocation(WorldLocation* location) const
    {
        if (location) {
            location->Relocate(
                m_positionX, m_positionY, m_positionZ, GetOrientation());
            location->SetMapId(m_mapId);
        }
    }

    [[nodiscard]] WorldLocation GetWorldLocation() const { return *this; }
