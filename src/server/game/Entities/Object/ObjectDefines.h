...
#define SPELL_SEARCHER_COMPENSATION                                            \
    30.0f // increase searchers size in case we have large npc near cell border
#define TRADE_DISTANCE 11.11f
#define MAX_VISIBILITY_DISTANCE                                                \
    250.0f // max distance for visible objects, experimental
#define SIGHT_RANGE_UNIT 50.0f
#define MAX_SEARCHER_DISTANCE                                                  \
    150.0f // pussywizard: replace the use of MAX_VISIBILITY_DISTANCE in
           // searchers, because MAX_VISIBILITY_DISTANCE is quite too big for
           // this purpose
...
#define DEFAULT_VISIBILITY_DISTANCE                                            \
    100.0f // default visible distance, 100 yards on continents
#define DEFAULT_VISIBILITY_INSTANCE                                            \
    170.0f // default visible distance in instances, 170 yards
#define VISIBILITY_DIST_WINTERGRASP 175.0f
#define DEFAULT_VISIBILITY_BGARENAS                                            \
    533.0f // default visible distance in BG/Arenas, roughly 533 yards

#define DEFAULT_WORLD_OBJECT_SIZE                                              \
    0.388999998569489f // player size, also currently used (correctly?) for any
                       // non Unit world objects
#define DEFAULT_COMBAT_REACH 1.5f
#define MIN_MELEE_REACH 2.0f
#define NOMINAL_MELEE_RANGE 5.0f
#define MELEE_RANGE                                                            \
    (NOMINAL_MELEE_RANGE - MIN_MELEE_REACH * 2) // center to center for players
...
uint16 MAKE_PAIR16(uint8 l, uint8 h) { return uint16(l | (uint16(h) << 8)); }

uint32 MAKE_PAIR32(uint16 l, uint16 h) { return uint32(l | (uint32(h) << 16)); }
