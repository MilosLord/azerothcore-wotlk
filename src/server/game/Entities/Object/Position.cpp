...
bool Position::HasInLine(Position const* pos, float width) const
{
    ...
    return std::fabs(std::sin(angle)) *
               GetExactDist2d(pos->GetPositionX(), pos->GetPositionY()) <
           width;
}
...
std::string Position::ToString() const
{
    ...
    sstr << "X: " << m_positionX << " Y: " << m_positionY
         << " Z: " << m_positionZ << " O: " << m_orientation;
    return sstr.str();
}

void Position::RelocateOffset(const Position& offset)
{
    m_positionX = GetPositionX() +
                  (offset.GetPositionX() * std::cos(GetOrientation()) +
                   offset.GetPositionY() * std::sin(GetOrientation() + M_PI));
    m_positionY =
        GetPositionY() + (offset.GetPositionY() * std::cos(GetOrientation()) +
                                   ...
                                   Position&       retOffset) const
{
    ...
    retOffset.m_positionX =
        dx * cos(GetOrientation()) + dy * std::sin(GetOrientation());
    retOffset.m_positionY =
        dy * cos(GetOrientation()) - dx * std::sin(GetOrientation());
...
void Position::GetSinCos(const float x,
                         const float y,
                         float&      vsin,
                         float&      vcos) const
{
...
bool Position::IsWithinBox(const Position& center,
                           ...
                           float           zradius) const
{
    // rotate the WorldObject position instead of rotating the whole cube, that
    // way we can make a simplified is-in-cube check and we have to calculate
    // only one point instead of 4
    ...
    float rotX =
        float(center.GetPositionX() + BoxDistX * cosVal - BoxDistY * sinVal);
    float rotY =
        float(center.GetPositionY() + BoxDistY * cosVal + BoxDistX * sinVal);
    ...
    if ((std::fabs(dx) > xradius) || (std::fabs(dy) > yradius) ||
        (std::fabs(dz) > zradius)) {
        return false;
    }
