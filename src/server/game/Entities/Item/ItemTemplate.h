...
enum ItemSpelltriggerType {
    ...
    ITEM_SPELLTRIGGER_LEARN_SPELL_ID =
        6 // used in item_template.spell_2 with spell_id with
...
/* /// @todo: Requiring actual cases in which using (an) item isn't allowed
   while shapeshifted. Else, this flag would need an implementation.
    ITEM_FLAG_USE_WHEN_SHAPESHIFTED    = 0x00800000, // Item can be used in
   shapeshift forms */

enum ItemFieldFlags : uint32 {
    ITEM_FIELD_FLAG_SOULBOUND =
        0x00000001, // Item is soulbound and cannot be traded <<--
    ITEM_FIELD_FLAG_UNK1 = 0x00000002, // ?
    ITEM_FIELD_FLAG_UNLOCKED =
        0x00000004, // Item had lock but can be opened now
    ITEM_FIELD_FLAG_WRAPPED =
        0x00000008, // Item is wrapped and contains another item
    ...
    ITEM_FIELD_FLAG_BOP_TRADEABLE =
        0x00000100,                        // Allows trading soulbound items
    ...
    ITEM_FIELD_FLAG_REFUNDABLE =
        0x00001000, // Item can be returned to vendor for its original cost
                    // (extended cost)
    ...
    ITEM_FLAG_MAIL_TEXT_MASK =
        ITEM_FIELD_FLAG_READABLE | ITEM_FIELD_FLAG_UNK13 | ITEM_FIELD_FLAG_UNK14
};
...
enum ItemFlags : uint32 {
    ...
    ITEM_FLAG_HAS_LOOT =
        0x00000004, // Item can be right clicked to open for loot
    ITEM_FLAG_HEROIC_TOOLTIP =
        0x00000008, // Makes green "Heroic" text appear on item
    ITEM_FLAG_DEPRECATED = 0x00000010, // Cannot equip or use
    ITEM_FLAG_NO_USER_DESTROY =
        0x00000020, // Item can not be destroyed, except by using spell (item
                    // can be reagent for spell)
    ITEM_FLAG_PLAYERCAST = 0x00000040, // Item's spells are castable by players
    ITEM_FLAG_NO_EQUIP_COOLDOWN =
        0x00000080, // No default 30 seconds cooldown when equipped
    ...
    ITEM_FLAG_MULTI_DROP =
        0x00000800, // Looting this item does not remove it from available loot
    ITEM_FLAG_ITEM_PURCHASE_RECORD =
        0x00001000, // Item can be returned to vendor for its original cost
                    // (extended cost)
    ITEM_FLAG_PETITION = 0x00002000, // Item is guild or arena charter
    ITEM_FLAG_HAS_TEXT =
        0x00004000, // Only readable items have this (but not all)
    ...
    ITEM_FLAG_IGNORE_DEFAULT_ARENA_RESTRICTIONS =
        0x00200000, // Item can be used during arena match
    ITEM_FLAG_NO_DURABILITY_LOSS =
        0x00400000, // Some Thrown weapons have it (and only Thrown) but not all
    ITEM_FLAG_USE_WHEN_SHAPESHIFTED =
        0x00800000, // Item can be used in shapeshift forms
    ITEM_FLAG_HAS_QUEST_GLOW = 0x01000000,
    ITEM_FLAG_HIDE_UNUSABLE_RECIPE =
        0x02000000, // Profession recipes: can only be looted if you meet
                    // requirements and don't already know it
    ITEM_FLAG_NOT_USEABLE_IN_ARENA = 0x04000000, // Item cannot be used in arena
    ITEM_FLAG_IS_BOUND_TO_ACCOUNT =
        0x08000000, // Item binds to account and can be sent only to your own
                    // characters
...
enum ItemFlags2 : uint32 {
    ...
    ITEM_FLAG2_DONT_IGNORE_BUY_PRICE =
        0x00000004, // when item uses extended cost, gold is also required
...
enum ItemFlagsCustom {
    ITEM_FLAGS_CU_DURATION_REAL_TIME =
        0x0001, // Item duration will tick even if player is offline
    ITEM_FLAGS_CU_IGNORE_QUEST_STATUS =
        0x0002, // No quest status will be checked when this item drops
    ITEM_FLAGS_CU_FOLLOW_LOOT_RULES =
        0x0004, // Item will always follow group/master/need before greed
                // looting rules
};
...
#define SOCKET_COLOR_ALL                                                       \
    (SOCKET_COLOR_META | SOCKET_COLOR_RED | SOCKET_COLOR_YELLOW |              \
     SOCKET_COLOR_BLUE)
...
inline uint8 ItemSubClassToDurabilityMultiplierId(uint32 ItemClass,
                                                  uint32 ItemSubClass)
{
    switch (ItemClass) {
...
struct ItemTemplate {
    ...
    int32  SoundOverrideSubclass; // < 0: id from ItemSubClass.dbc, used to
                                  // override weapon sound from actual SubClass
    std::string Name1;
    ...
    int32 Stackable;      // 0: not allowed, -1: put in player coin info tab and
                          // don't limit stacking (so 1 slot)
    uint32    ContainerSlots;
    uint32    StatsCount;
    _ItemStat ItemStat[MAX_ITEM_PROTO_STATS];
    uint32    ScalingStatDistribution; // id from ScalingStatDistribution.dbc
    uint32
        ScalingStatValue; // mask for selecting column in ScalingStatValues.dbc
    ...
    {
        return GetMaxStackSize() == 1 && Class != ITEM_CLASS_CONSUMABLE &&
    ...
    [[nodiscard]] bool IsCurrencyToken() const
    {
    ...
    {
        return (Stackable == 2147483647 || Stackable <= 0)
                   ? uint32(0x7FFFFFFF - 1)
                   : uint32(Stackable);
    }
    ...
    {
        constexpr uint32 feralApEnabledInventoryTypeMaks =
            1 << INVTYPE_WEAPON | 1 << INVTYPE_2HWEAPON |
            1 << INVTYPE_WEAPONMAINHAND | 1 << INVTYPE_WEAPONOFFHAND;

        // 0x02A5F3 - is mask for Melee weapon from ItemSubClassMask.dbc
        if (Class == ITEM_CLASS_WEAPON &&
            (1 << InventoryType) & feralApEnabledInventoryTypeMaks) {
    ...
    [[nodiscard]] bool IsPotion() const
    {
        return Class == ITEM_CLASS_CONSUMABLE &&
               SubClass == ITEM_SUBCLASS_POTION;
    }
    [[nodiscard]] bool IsWeaponVellum() const
    {
        return Class == ITEM_CLASS_TRADE_GOODS &&
               SubClass == ITEM_SUBCLASS_WEAPON_ENCHANTMENT;
    }
    [[nodiscard]] bool IsArmorVellum() const
    {
        return Class == ITEM_CLASS_TRADE_GOODS &&
               SubClass == ITEM_SUBCLASS_ARMOR_ENCHANTMENT;
    }
    [[nodiscard]] bool IsConjuredConsumable() const
    {
