...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    if (!set) {
        LOG_ERROR("sql.sql",
                  "Item set {} for item (id {}) not found, mods not applied.",
                  setid,
                  proto->ItemId);
        return;
    }
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y) {
            if (!eff->spells[y]) // free slot
            {
                SpellInfo const* spellInfo =
                    sSpellMgr->GetSpellInfo(set->spells[x]);
                if (!spellInfo) {
                    LOG_ERROR(
                        "entities.item",
                        "WORLD: unknown spell id {} in items set {} effects",
                        set->spells[x],
                        setid);
                    break;
                }
...
void RemoveItemsSetItem(Player* player, ItemTemplate const* proto)
{
    ...
    if (!set) {
        LOG_ERROR("sql.sql",
                  "Item set #{} for item #{} not found, mods not removed.",
                  setid,
                  proto->ItemId);
        return;
    }
                  ...
                  Player const*       owner)
{
    ...
    SetGuidValue(ITEM_FIELD_OWNER,
                 owner ? owner->GetGUID() : ObjectGuid::Empty);
    ...
    case ITEM_CHANGED: {
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(
                uState == ITEM_NEW ? CHAR_REP_ITEM_INSTANCE
        ...
        stmt->SetData(++index,
                      GetGuidValue(ITEM_FIELD_GIFTCREATOR).GetCounter());
                      ...
                      uint32              entry)
{
    ...
    //                                                    3        4      5 6 7
    //                                                    8           9    10
    // result = CharacterDatabase.Query("SELECT creatorGuid, giftCreatorGuid,
    // count, duration, charges, flags, enchantments, randomPropertyId,
    // durability, playedTime, text FROM item_instance WHERE guid = '{}'",
    // guid);

    // create item before any checks for store correct guid
    // and allow use "FSetState(ITEM_REMOVED); SaveToDB();" for deleting item
    // from DB
    ...
    if (!proto) {
        LOG_ERROR("entities.item",
                  "Invalid entry {} for item {}. Refusing to load.",
                  GetEntry(),
                  GetGUID().ToString());
        return false;
    }
    ...
    SetGuidValue(ITEM_FIELD_CREATOR,
    ...
    std::vector<std::string_view> tokens =
        Acore::Tokenize(fields[4].Get<std::string_view>(), ' ', false);
    if (tokens.size() == MAX_ITEM_PROTO_SPELLS) {
        for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i) {
            if (Optional<int32> charges = Acore::StringTo<int32>(tokens[i]))
                ...
                LOG_ERROR("entities.item",
                          "Invalid charge info '{}' for item {}, charge data "
                          "not loaded.",
                          tokens.at(i),
                          GetGUID().ToString());
        }
    ...
    if (IsSoulBound() && proto->Bonding == NO_BIND &&
        sScriptMgr->CanApplySoulboundFlag(this, proto)) {
    ...
    if (!_LoadIntoDataField(fields[6].Get<std::string>(),
                            ITEM_FIELD_ENCHANTMENT_1_1,
                            MAX_ENCHANTMENT_SLOT * MAX_ENCHANTMENT_OFFSET)) {
        LOG_WARN(
            "entities.item",
            "Invalid enchantment data '{}' for item {}. Forcing partial load.",
...
// Legacy / Shortcut
uint32 Item::GetSkill() { return GetTemplate()->GetSkill(); }
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id)
{
    ...
    if ((itemProto->RandomProperty) && (itemProto->RandomSuffix)) {
        LOG_ERROR("sql.sql",
                  "Item template {} have RandomProperty == {} and RandomSuffix "
                  "== {}, but must have one from field =0",
                  itemProto->ItemId,
                  itemProto->RandomProperty,
                  itemProto->RandomSuffix);
        return 0;
    }
    ...
    if (itemProto->RandomProperty) {
        ...
        if (!random_id) {
            LOG_ERROR("sql.sql",
                      "Enchantment id #{} used but it doesn't have records in "
...
void Item::SetItemRandomProperties(int32 randomPropId)
{
    ...
    if (randomPropId > 0) {
        ...
        if (item_rand) {
            ...
            for (uint32 i = PROP_ENCHANTMENT_SLOT_0; i < MAX_ENCHANTMENT_SLOT;
                 ++i)
                SetEnchantment(
                    EnchantmentSlot(i),
                    item_rand->Enchantment[i - PROP_ENCHANTMENT_SLOT_0],
                    0,
                    0);
        }
    }
...
void Item::AddToUpdateQueueOf(Player* player)
{
    ...
    if (player->GetGUID() != GetOwnerGUID()) {
        LOG_DEBUG("entities.player.items",
                  "Item::AddToUpdateQueueOf - Owner's guid ({}) and player's "
                  "guid ({}) don't match!",
...
void Item::RemoveFromUpdateQueueOf(Player* player)
{
    ...
    if (player->GetGUID() != GetOwnerGUID()) {
        LOG_DEBUG("entities.player.items",
                  "Item::RemoveFromUpdateQueueOf - Owner's guid ({}) and "
                  "player's guid ({}) don't match!",
                  GetOwnerGUID().ToString(),
                  player->GetGUID().ToString());
        return;
    }
...
bool Item::CanBeTraded(bool mail, bool trade) const
{
    if ((!mail || !IsBoundAccountWide()) &&
        (IsSoulBound() &&
         (!HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_BOP_TRADEABLE) || !trade)))
        return false;

    if (IsBag() &&
        (Player::IsBagPos(GetPos()) || !((Bag const*)this)->IsEmpty()))
        return false;
...
bool Item::HasEnchantRequiredSkill(Player const* player) const
{
    ...
    for (uint32 enchant_slot = PERM_ENCHANTMENT_SLOT;
         enchant_slot < MAX_ENCHANTMENT_SLOT;
         ++enchant_slot)
        if (uint32 enchant_id = GetEnchantmentId(EnchantmentSlot(enchant_slot)))
            if (SpellItemEnchantmentEntry const* enchantEntry =
                    sSpellItemEnchantmentStore.LookupEntry(enchant_id))
                if (enchantEntry->requiredSkill &&
                    player->GetSkillValue(enchantEntry->requiredSkill) <
                        enchantEntry->requiredSkillValue)
                    return false;
...
bool Item::IsFitToSpellRequirements(SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo->EquippedItemClass != -1) // -1 == any item class
    {
        ...
        if ((spellInfo->EquippedItemClass == ITEM_CLASS_ARMOR &&
            ...
            (spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
             proto->IsWeaponVellum()))
            if (spellInfo->IsAbilityOfSkillType(
                    SKILL_ENCHANTING)) // only for enchanting spells
                return true;
        ...
        0) // 0 == any inventory type
    {
        ...
        if (proto->InventoryType == INVTYPE_WEAPON &&
            ...
            return true;
        else if ((spellInfo->EquippedItemInventoryTypeMask &
                          ...
                          ObjectGuid      caster /*= ObjectGuid::Empty*/)
{
    ...
    if (slot < MAX_INSPECTED_ENCHANTMENT_SLOT) {
        if (uint32 oldEnchant = GetEnchantmentId(slot))
            owner->GetSession()->SendEnchantmentLog(
                GetOwnerGUID(), ObjectGuid::Empty, GetEntry(), oldEnchant);

        if (id)
            owner->GetSession()->SendEnchantmentLog(
                GetOwnerGUID(), caster, GetEntry(), id);
    }
                                  ...
                                  Player*         owner)
{
    ...
    SetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 + slot * MAX_ENCHANTMENT_OFFSET +
                   ...
                   duration);
    SetState(ITEM_CHANGED, owner);
    // Cannot use GetOwner() here, has to be passed as an argument to avoid
    // freeze due to hashtable locking
}
...
bool Item::GemsFitSockets() const
{
    for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT;
         ...
         ++enchant_slot) {
        uint8 SocketColor =
            GetTemplate()->Socket[enchant_slot - SOCK_ENCHANTMENT_SLOT].Color;
...
{
    // There can only be one socket added, and it's always in slot
    // `PRISMATIC_ENCHANTMENT_SLOT`.
    //     Built-in sockets                        Socket from upgrade
    return this->GetTemplate()->Socket[0].Color ||
                       ...
                       uint32        randomPropertyId)
{
    ...
    if (pProto) {
        ...
        if (pItem->Create(sObjectMgr->GetGenerator<HighGuid::Item>().Generate(),
                          item,
                          player)) {
...
{
    // player CAN be nullptr in which case we must not update random properties
    // because that accesses player's item update queue
    Item* newItem = CreateItem(GetEntry(),
                               count,
                               player,
                               true,
                               player ? GetItemRandomPropertyId() : 0);
    if (!newItem)
        return nullptr;

    newItem->SetUInt32Value(ITEM_FIELD_CREATOR,
                            GetUInt32Value(ITEM_FIELD_CREATOR));
    newItem->SetUInt32Value(ITEM_FIELD_GIFTCREATOR,
                            GetUInt32Value(ITEM_FIELD_GIFTCREATOR));
    newItem->SetUInt32Value(
        ITEM_FIELD_FLAGS,
        GetUInt32Value(ITEM_FIELD_FLAGS) &
            ~(ITEM_FIELD_FLAG_REFUNDABLE | ITEM_FIELD_FLAG_BOP_TRADEABLE));
    newItem->SetUInt32Value(ITEM_FIELD_DURATION,
                            GetUInt32Value(ITEM_FIELD_DURATION));
    return newItem;
}
...
void Item::SaveRefundDataToDB()
{
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_ITEM_REFUND_INSTANCE);
    stmt->SetData(0, GetGUID().GetCounter());
...
void Item::ClearSoulboundTradeable(Player* currentOwner)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEM_BOP_TRADE);
    stmt->SetData(0, GetGUID().GetCounter());
    CharacterDatabase.Execute(stmt);
}
...
std::string Item::GetDebugInfo() const
{
    ...
    sstr << Object::GetDebugInfo() << "\n"
         << std::boolalpha << "Owner: " << GetOwnerGUID().ToString()
         << " Count: " << GetCount()
         << " BagSlot: " << std::to_string(GetBagSlot())
         << " Slot: " << std::to_string(GetSlot())
         << " Equipped: " << IsEquipped();
    return sstr.str();
}
