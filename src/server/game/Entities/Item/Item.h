...
enum SellResult {
    ...
    SELL_ERR_CANT_SELL_TO_THIS_MERCHANT =
        7, // You cannot sell items to this merchant.
    SELL_ERR_MUST_REPAIR_ITEM_DURABILITY_TO_USE =
        8, // You must repair that item's durability to use it.
...
#define MAX_VISIBLE_ITEM_OFFSET                                                \
    2 // 2 fields per visible item (entry+enchantment)
...
public:
    static Item* CreateItem(uint32        item,
                            uint32        count,
    ...
    virtual bool
    Create(ObjectGuid::LowType guidlow, uint32 itemid, Player const* owner);
    ...
    [[nodiscard]] bool IsBoundAccountWide() const
    {
        return (GetTemplate()->Flags & ITEM_FLAG_IS_BOUND_TO_ACCOUNT) != 0;
    }
    bool               IsBindedNotWith(Player const* player) const;
    [[nodiscard]] bool IsBoundByEnchant() const;
    [[nodiscard]] bool IsBoundByTempEnchant() const;
    virtual void       SaveToDB(CharacterDatabaseTransaction trans);
    virtual bool       LoadFromDB(ObjectGuid::LowType guid,
                                  ObjectGuid          owner_guid,
                                  Field*              fields,
                                  uint32              entry);
    static void        DeleteFromDB(CharacterDatabaseTransaction trans,
                                    ObjectGuid::LowType          itemGuid);
    virtual void       DeleteFromDB(CharacterDatabaseTransaction trans);
    static void        DeleteFromInventoryDB(CharacterDatabaseTransaction trans,
                                             ObjectGuid::LowType          itemGuid);
    ...
    [[nodiscard]] const Bag* ToBag() const
    {
        if (IsBag())
    ...
    {
        return !HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_UNLOCKED);
    }
    ...
    [[nodiscard]] bool IsLimitedToAnotherMapOrZone(uint32 cur_mapId,
    ...
    [[nodiscard]] uint32 GetMaxStackCount() const
    {
    ...
    [[nodiscard]] uint8
                    GetGemCountWithLimitCategory(uint32 limitCategory) const;
    ...
    [[nodiscard]] int32 GetItemRandomPropertyId() const
    {
        return GetInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID);
    }
    [[nodiscard]] uint32 GetItemSuffixFactor() const
    {
    ...
    void         SetEnchantment(EnchantmentSlot slot,
                                uint32          id,
                                uint32          duration,
                                uint32          charges,
                                ObjectGuid      caster = ObjectGuid::Empty);
    void         SetEnchantmentDuration(EnchantmentSlot slot,
                                        uint32          duration,
                                        Player*         owner);
    void         SetEnchantmentCharges(EnchantmentSlot slot, uint32 charges);
    void         ClearEnchantment(EnchantmentSlot slot);
    [[nodiscard]] uint32 GetEnchantmentId(EnchantmentSlot slot) const
    {
        return GetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 +
                              slot * MAX_ENCHANTMENT_OFFSET +
                              ENCHANTMENT_ID_OFFSET);
    }
    [[nodiscard]] uint32 GetEnchantmentDuration(EnchantmentSlot slot) const
    {
        return GetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 +
                              slot * MAX_ENCHANTMENT_OFFSET +
                              ENCHANTMENT_DURATION_OFFSET);
    }
    [[nodiscard]] uint32 GetEnchantmentCharges(EnchantmentSlot slot) const
    {
        return GetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 +
                              slot * MAX_ENCHANTMENT_OFFSET +
                              ENCHANTMENT_CHARGES_OFFSET);
    ...
    [[nodiscard]] int32 GetSpellCharges(uint8 index /*0..5*/ = 0) const
    {
    ...
    {
        return GetTemplate()->StartQuest == quest_id;
    }
    ...
    {
        return GetTemplate()->IsWeaponVellum();
    }
    [[nodiscard]] bool IsArmorVellum() const
    {
        return GetTemplate()->IsArmorVellum();
    }
