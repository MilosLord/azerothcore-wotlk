...
DamageInfo::DamageInfo(Unit*            _attacker,
                       ...
                       uint32           cleanDamage)
    : m_attacker(_attacker), m_victim(_victim), m_damage(_damage),
      m_spellInfo(_spellInfo), m_schoolMask(_schoolMask),
      m_damageType(_damageType), m_attackType(BASE_ATTACK),
      m_cleanDamage(cleanDamage)
{
    m_absorb = 0;
    m_resist = 0;
    m_block  = 0;
...
DamageInfo::DamageInfo(CalcDamageInfo const& dmgInfo)
    : DamageInfo(DamageInfo(dmgInfo, 0), DamageInfo(dmgInfo, 1))
{
}

DamageInfo::DamageInfo(DamageInfo const& dmg1, DamageInfo const& dmg2)
    : m_attacker(dmg1.m_attacker), m_victim(dmg1.m_victim),
      m_damage(dmg1.m_damage + dmg2.m_damage), m_spellInfo(dmg1.m_spellInfo),
      m_schoolMask(SpellSchoolMask(dmg1.m_schoolMask | dmg2.m_schoolMask)),
      m_damageType(dmg1.m_damageType), m_attackType(dmg1.m_attackType),
      m_absorb(dmg1.m_absorb + dmg2.m_absorb),
      m_resist(dmg1.m_resist + dmg2.m_resist), m_block(dmg1.m_block),
      m_cleanDamage(dmg1.m_cleanDamage + dmg1.m_cleanDamage)
{
}

DamageInfo::DamageInfo(CalcDamageInfo const& dmgInfo, uint8 damageIndex)
    : m_attacker(dmgInfo.attacker), m_victim(dmgInfo.target),
      m_damage(dmgInfo.damages[damageIndex].damage), m_spellInfo(nullptr),
      m_schoolMask(
          SpellSchoolMask(dmgInfo.damages[damageIndex].damageSchoolMask)),
      m_damageType(DIRECT_DAMAGE), m_attackType(dmgInfo.attackType),
      m_absorb(dmgInfo.damages[damageIndex].absorb),
      m_resist(dmgInfo.damages[damageIndex].resist),
      m_block(dmgInfo.blocked_amount), m_cleanDamage(dmgInfo.cleanDamage)
{
}
...
DamageInfo::DamageInfo(SpellNonMeleeDamage const& spellNonMeleeDamage,
    ...
    : m_attacker(spellNonMeleeDamage.attacker),
      ...
      m_spellInfo(spellNonMeleeDamage.spellInfo),
      m_schoolMask(SpellSchoolMask(spellNonMeleeDamage.schoolMask)),
      m_damageType(damageType), m_absorb(spellNonMeleeDamage.absorb),
...
ProcEventInfo::ProcEventInfo(Unit*            actor,
    ...
    : _actor(actor), _actionTarget(actionTarget), _procTarget(procTarget),
      ...
      _triggeredByAuraSpell(triggeredByAuraSpell),
      _procAuraEffectIndex(procAuraEffectIndex)
{
    _chance.reset();
}
...
Unit::Unit(bool isWorldObject)
    : WorldObject(isWorldObject), m_movedByPlayer(nullptr),
      m_lastSanctuaryTime(0), IsAIEnabled(false), NeedChangeAI(false),
      m_ControlledByPlayer(false), m_CreatedByPlayer(false),
      movespline(new Movement::MoveSpline()), i_AI(nullptr),
      i_disabledAI(nullptr), m_realRace(0), m_race(0),
      m_AutoRepeatFirstCast(false), m_procDeep(0), m_removedAurasCount(0),
      i_motionMaster(new MotionMaster(this)), m_regenTimer(0),
      m_ThreatMgr(this), m_vehicle(nullptr), m_vehicleKit(nullptr),
      m_unitTypeMask(UNIT_MASK_NONE), m_HostileRefMgr(this),
      m_comboTarget(nullptr), m_comboPoints(0)
{
#ifdef _MSC_VER
...
{
    GlobalCooldownList::const_iterator itr =
        m_GlobalCooldowns.find(spellInfo->StartRecoveryCategory);
    return itr != m_GlobalCooldowns.end() && itr->second.duration &&
           getMSTimeDiff(itr->second.cast_time,
                                          ...
                                          uint32           gcd)
{
    m_GlobalCooldowns[spellInfo->StartRecoveryCategory] =
        GlobalCooldown(gcd, GameTime::GetGameTimeMS().count());
}
...
void Unit::Update(uint32 p_time)
{
    ...
    // _UpdateSpells. Or else we may have some SPELL_STATE_FINISHED spells
    // stalled in pointers, that is bad.
    ...
    if (GetTypeId() != TYPEID_PLAYER ||
        (!ToPlayer()->IsBeingTeleported() && !bRequestForcedVisibilityUpdate)) {
    ...
    if (IsInCombat() &&
        (GetTypeId() == TYPEID_PLAYER ||
         ((IsPet() || HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN)) &&
          IsControlledByPlayer()))) {
        // Check UNIT_STATE_MELEE_ATTACKING or UNIT_STATE_CHASE (without
        // UNIT_STATE_FOLLOW in this case) so pets can reach far away targets
        // without stopping half way there and running off. These flags are
        // reset after target dies or another command is given.
        if (m_HostileRefMgr.IsEmpty()) {
            // m_CombatTimer set at aura start and it will be freeze until aura
            // removing
    ...
    if (_lastExtraAttackSpell) {
        while (!extraAttacksTargets.empty()) {
            ...
            if (Unit* victim = ObjectAccessor::GetUnit(*this, targetGuid)) {
                if (_lastExtraAttackSpell == SPELL_SWORD_SPECIALIZATION ||
                    _lastExtraAttackSpell == SPELL_HACK_AND_SLASH ||
    ...
    if (IsPlayer() && HasUnitState(UNIT_STATE_CASTING)) {
        for (Spell* spell : m_currentSpells) {
            if (spell) {
                if (spell->GetSpellInfo()->HasAttribute(
    ...
    if (!suspendAttackTimer) {
        if (int32 base_attack = getAttackTimer(BASE_ATTACK)) {
            setAttackTimer(BASE_ATTACK,
                           base_attack > 0 ? base_attack - (int32)p_time : 0);
        }

        if (int32 off_attack = getAttackTimer(OFF_ATTACK)) {
            setAttackTimer(OFF_ATTACK,
                           off_attack > 0 ? off_attack - (int32)p_time : 0);
        }
    }
    ...
    if (!suspendRangedAttackTimer) {
        if (int32 ranged_attack = getAttackTimer(RANGED_ATTACK)) {
            setAttackTimer(RANGED_ATTACK,
                           ranged_attack > 0 ? ranged_attack - (int32)p_time
                                             : 0);
        }
    }
    ...
    ModifyAuraState(AURA_STATE_HEALTHLESS_20_PERCENT,
                    IsAlive() ? HealthBelowPct(20) : false);
    ModifyAuraState(AURA_STATE_HEALTHLESS_35_PERCENT,
                    IsAlive() ? HealthBelowPct(35) : false);
    ModifyAuraState(AURA_STATE_HEALTH_ABOVE_75_PERCENT,
                    IsAlive() ? HealthAbovePct(75) : false);
...
public:
    ...
    bool operator()(Movement::MoveSpline::UpdateResult result)
    {
        if ((result & (Movement::MoveSpline::Result_NextSegment |
                       Movement::MoveSpline::Result_JustArrived)) &&
            _unit->GetTypeId() == TYPEID_UNIT &&
            _unit->GetMotionMaster()->GetCurrentMovementGeneratorType() ==
                ESCORT_MOTION_TYPE &&
            _unit->movespline->GetId() ==
                _unit->GetMotionMaster()->GetCurrentSplineId()) {
            _unit->ToCreature()->AI()->MovementInform(
                ESCORT_MOTION_TYPE, _unit->movespline->currentPathIdx() - 1);
        }
...
void Unit::UpdateSplineMovement(uint32 t_diff)
{
    ...
    // this code cant be placed inside EscortMovementGenerator, because we cant
    // delete active MoveGen while it is updated
    ...
    if (arrived) {
        ...
        if (movespline->HasAnimation() && GetTypeId() == TYPEID_UNIT &&
            IsAlive())
            SetByteValue(UNIT_FIELD_BYTES_1,
                         UNIT_BYTES_1_OFFSET_ANIM_TIER,
                         movespline->GetAnimationType());
    }

    // pussywizard: update always! not every 400ms, because movement generators
...
void Unit::DisableSpline()
{
    m_movementInfo.RemoveMovementFlag(
        MovementFlags(MOVEMENTFLAG_SPLINE_ENABLED | MOVEMENTFLAG_FORWARD |
...
bool Unit::GetRandomContactPoint(
    Unit const* obj, float& x, float& y, float& z, bool force) const
{
    ...
    if (c)
        if (c->isWorldBoss() || c->IsDungeonBoss() ||
            (obj->IsPet() && const_cast<Unit*>(obj)->ToPet()->isControlled()))
            attacker_number = 0; // pussywizard: pets and bosses just come to
                                 // target from their angle
    ...
    GetNearPoint(
        ...
        z,
        isMoving()
            ? (obj->GetCombatReach() > 7.75f ? obj->GetCombatReach() - 7.5f
                                             : 0.25f)
            : obj->GetCombatReach(),
        0.0f,
        GetAngle(obj) + (attacker_number
                             ? (static_cast<float>(M_PI / 2) -
                                static_cast<float>(M_PI) * (float)rand_norm()) *
                                   float(attacker_number) / combat_reach * 0.3f
                             : 0));

    // pussywizard
    if (std::fabs(this->GetPositionZ() - z) > this->GetCollisionHeight() ||
        !IsWithinLOS(x, y, z)) {
        x = this->GetPositionX();
        y = this->GetPositionY();
        z = this->GetPositionZ();
...
bool Unit::HasBreakableByDamageAuraType(AuraType type, uint32 excludeAura) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end();
        ...
        if ((!excludeAura ||
             excludeAura != (*itr)
                                ->GetSpellInfo()
                                ->Id) && // Avoid self interrupt of channeled
                                         // Crowd Control spells like Seduction
            ((*itr)->GetSpellInfo()->AuraInterruptFlags &
             AURA_INTERRUPT_FLAG_TAKE_DAMAGE))
            return true;
    return false;
}

bool Unit::HasBreakableByDamageCrowdControlAura(
    Unit* excludeCasterChannel) const
{
    uint32 excludeAura = 0;
    if (Spell* currentChanneledSpell =
            excludeCasterChannel
                ? excludeCasterChannel->GetCurrentSpell(CURRENT_CHANNELED_SPELL)
                : nullptr)
        excludeAura = currentChanneledSpell->GetSpellInfo()
                          ->Id; // Avoid self interrupt of channeled Crowd
                                // Control spells like Seduction
...
{
    if (!victim || !victim->IsAlive() || victim->IsInFlight() ||
        (victim->GetTypeId() == TYPEID_UNIT &&
         victim->ToCreature()->IsEvadingAttacks())) {
        if (absorb)
            *absorb += damage;
        damage = 0;
    }
                        ...
                        Spell const* damageSpell /*= nullptr*/)
{
    ...
    // Xinef: its rare to modify damage in hooks, however training dummy's sets
    // damage to 0
    uint32 rage_damage =
        damage + ((cleanDamage != nullptr) ? cleanDamage->absorbed_damage : 0);

    // if (attacker)
    {
        if (victim->IsAIEnabled)
            victim->GetAI()->DamageTaken(
                attacker, damage, damagetype, damageSchoolMask);
    ...
    if (victim->GetTypeId() == TYPEID_UNIT && attacker != victim &&
        victim->IsPet() && victim->IsAlive())
        victim->ToPet()->AI()->AttackedBy(attacker);

    if (damagetype != NODAMAGE) {
        // interrupting auras with AURA_INTERRUPT_FLAG_DAMAGE before checking
        // !damage (absorbed damage breaks that type of auras)
        if (spellProto) {
            if (!spellProto->HasAttribute(SPELL_ATTR4_REACTIVE_DAMAGE_PROC))
                victim->RemoveAurasWithInterruptFlags(
                    AURA_INTERRUPT_FLAG_TAKE_DAMAGE, spellProto->Id);
        }
        else
            victim->RemoveAurasWithInterruptFlags(
                AURA_INTERRUPT_FLAG_TAKE_DAMAGE, 0);

        // interrupt spells with SPELL_INTERRUPT_FLAG_ABORT_ON_DMG on absorbed
        // damage (no dots)
        if (!damage && damagetype != DOT && cleanDamage &&
            cleanDamage->absorbed_damage) {
            if (victim != attacker && victim->GetTypeId() == TYPEID_PLAYER) {
                if (Spell* spell =
                        victim->m_currentSpells[CURRENT_GENERIC_SPELL]) {
        ...
        // We're going to call functions which can modify content of the list
        // during iteration over it's elements Let's copy the list so we can
        ...
        for (AuraEffectList::iterator i = vCopyDamageCopy.begin();
             i != vCopyDamageCopy.end();
             ++i) {
            // Check if aura was removed during iteration - we don't need to
            // work on such auras
            ...
            if (shareDamageTarget->IsImmunedToDamageOrSchool(
                    damageSchoolMask)) {
                shareAbsorb = shareDamage;
                shareDamage = 0;
            }
            else {
                DamageInfo sharedDamageInfo(attacker,
                                            shareDamageTarget,
                                            shareDamage,
                                            spellProto,
                                            damageSchoolMask,
                ...
                Unit::DealDamageMods(
                    shareDamageTarget, shareDamage, &shareAbsorb);
            }

            if (attacker && shareDamageTarget->GetTypeId() == TYPEID_PLAYER) {
                attacker->SendSpellNonMeleeDamageLog(shareDamageTarget,
                                                     spell,
                                                     shareDamage,
                                                     damageSchoolMask,
                                                     shareAbsorb,
    ...
    if (attacker && cleanDamage && damagetype == DIRECT_DAMAGE &&
        attacker != victim && attacker->HasActivePowerType(POWER_RAGE)) {
        ...
        case OFF_ATTACK: {
            weaponSpeedHitFactor = uint32(
                attacker->GetAttackTime(cleanDamage->attackType) / 1000.0f *
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->ToPlayer()->duel &&
        damage >= (health - 1)) {
        ...
        if (victim->ToPlayer()->duel->Opponent == attacker ||
            victim->ToPlayer()->duel->Opponent->GetGUID() ==
                attacker->GetOwnerGUID())
            damage = health - 1;

        duel_hasEnded = true;
    }
    else if (victim->IsVehicle() && damage >= (health - 1) &&
             victim->GetCharmer() &&
             victim->GetCharmer()->GetTypeId() == TYPEID_PLAYER) {
        ...
        if (victimRider && victimRider->duel && victimRider->duel->IsMounted) {
            ...
            // prevent kill only if killed in duel and killed by opponent or
            // opponent controlled creature
            if (victimRider->duel->Opponent == attacker ||
                victimRider->duel->Opponent->GetGUID() ==
                    attacker->GetCharmerGUID())
                damage = health - 1;
    ...
    if (attacker && attacker != victim)
        if (Player* killer =
            ...
            // pussywizard: don't allow GMs to deal damage in normal way (this
            // leaves no evidence in logs!), they have commands to do so
            // if (!allowGM && killer->GetSession()->GetSecurity() &&
            // killer->GetSession()->GetSecurity() <= SEC_ADMINISTRATOR)
            //  return 0;
            ...
            if (Battleground* bg = killer->GetBattleground()) {
                ...
                killer->UpdateAchievementCriteria(
                    ...
                    damage,
                    0,
                    victim); // pussywizard: InBattleground() optimization
            }
            // killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_DEALT,
            // damage); // pussywizard: optimization
        }

    if (victim->GetTypeId() == TYPEID_PLAYER)
        ; // victim->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_RECEIVED,
          // damage); // pussywizard: optimization
        ...
        if (!attacker || attacker->IsControlledByPlayer() ||
            attacker->IsCreatedByPlayer()) {
            uint32 unDamage = health < damage ? health : damage;
            bool   damagedByPlayer =
                unDamage && attacker &&
                (attacker->IsPlayer() || attacker->m_movedByPlayer != nullptr);
            victim->ToCreature()->LowerPlayerDamageReq(unDamage,
                                                       damagedByPlayer);
        }
    }
    ...
    if (health <= damage) {
        ...
        // if (attacker && victim->GetTypeId() == TYPEID_PLAYER && victim !=
        // attacker)
        // victim->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_DAMAGE_RECEIVED,
        // health); // pussywizard: optimization
        Unit::Kill(attacker,
                   victim,
                   durabilityLoss,
        ...
        if (victim->GetTypeId() != TYPEID_PLAYER) {
            // Part of Evade mechanics. DoT's and Thorns / Retribution Aura do
            // not contribute to this
            if (damagetype != DOT && damage > 0 &&
                !victim->GetOwnerGUID().IsPlayer() &&
                (!spellProto || !spellProto->HasAura(SPELL_AURA_DAMAGE_SHIELD)))
                victim->ToCreature()->SetLastDamagedTime(
                    GameTime::GetGameTime().count() + MAX_AGGRO_RESET_TIME);

            if (attacker) {
                if (spellProto && victim->CanHaveThreatList() &&
                    !victim->HasUnitState(UNIT_STATE_EVADE) &&
                    !victim->IsInCombatWith(attacker)) {
                    victim->CombatStart(attacker,
                                        !(spellProto->AttributesEx3 &
                                          SPELL_ATTR3_SUPRESS_TARGET_PROCS));
                }

                victim->AddThreat(
                    attacker, float(damage), damageSchoolMask, spellProto);
            }
        }
        else // victim is a player
        {
            ...
            if (roll_chance_f(sWorld->getRate(RATE_DURABILITY_LOSS_DAMAGE))) {
                EquipmentSlots slot =
                    EquipmentSlots(urand(0, EQUIPMENT_SLOT_END - 1));
        ...
        if (attacker != victim && victim->HasActivePowerType(POWER_RAGE)) {
            uint32 rageDamage =
                damage + (cleanDamage ? cleanDamage->absorbed_damage : 0);
        ...
        if (damagetype != NODAMAGE && damage &&
            (!spellProto ||
             !(spellProto->HasAttribute(SPELL_ATTR3_TREAT_AS_PERIODIC) ||
               spellProto->HasAttribute(
                   SPELL_ATTR7_DONT_CAUSE_SPELL_PUSHBACK)))) {
            if (victim != attacker &&
                victim->GetTypeId() ==
                    TYPEID_PLAYER) // does not support creature push_back
            {
                if (damagetype != DOT &&
                    !(damageSpell && damageSpell->m_targets.HasDstChannel())) {
                    if (Spell* spell =
                            victim->m_currentSpells[CURRENT_GENERIC_SPELL]) {
                        if (spell->getState() == SPELL_STATE_PREPARING) {
                            uint32 interruptFlags =
                    ...
                    if (Spell* spell =
                            victim->m_currentSpells[CURRENT_CHANNELED_SPELL])
                        if (spell->getState() == SPELL_STATE_CASTING) {
                            if ((spell->m_spellInfo->ChannelInterruptFlags &
                                 CHANNEL_FLAG_DELAY) != 0) {
        ...
        if (duel_hasEnded) {
            Player* he = duel_wasMounted ? victim->GetCharmer()->ToPlayer()
                                         : victim->ToPlayer();
                                ...
                                ObjectGuid               originalCaster)
{
    if (!spellInfo) {
        LOG_ERROR("entities.unit",
                  "CastSpell: unknown spell by caster {}",
                  GetGUID().ToString());
        return SPELL_FAILED_SPELL_UNAVAILABLE;
    }

    /// @todo: this is a workaround - not needed anymore, but required for some
    /// scripts :(
...
SpellCastResult
Unit::CastSpell(Unit*             victim,
                uint32            spellId,
                TriggerCastFlags  triggerFlags /*= TRIGGER_NONE*/,
...
SpellCastResult Unit::CastCustomSpell(Unit*             target,
                                      uint32            spellId,
                                      int32 const*      bp0,
                                      int32 const*      bp1,
                                      int32 const*      bp2,
                                      ...
                                      ObjectGuid        originalCaster)
{
    ...
    return CastCustomSpell(spellId,
                           values,
                           target,
                           triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE,
                           castItem,
                                     ...
                                     bool                 crit)
{
    ...
    // Script Hook For CalculateSpellDamageTaken -- Allow scripts to change the
    // Damage post class mitigation calculations
    sScriptMgr->ModifySpellDamageTaken(
        damageInfo->target, damageInfo->attacker, damage, spellInfo);
    ...
    if (Unit::IsDamageReducedByArmor(damageSchoolMask, spellInfo)) {
        ...
        damage          = Unit::CalcArmorReducedDamage(
            this, victim, damage, spellInfo, 0, attackType);
        ...
        if (crit) {
            ...
            if (Player* modOwner = GetSpellModOwner())
                modOwner->ApplySpellMod(
                    spellInfo->Id, SPELLMOD_CRIT_DAMAGE_BONUS, crit_bonus);
            damage += crit_bonus;
            ...
            if (attackType == RANGED_ATTACK)
                critPctDamageMod += victim->GetTotalAuraModifier(
            ...
            critPctDamageMod += GetTotalAuraModifierByMiscMask(
                SPELL_AURA_MOD_CRIT_DAMAGE_BONUS, spellInfo->GetSchoolMask());
    ...
    case SPELL_DAMAGE_CLASS_MAGIC: {
        ...
        if (crit) {
            damageInfo->HitInfo |= SPELL_HIT_TYPE_CRIT;
            damage =
                Unit::SpellCriticalDamageBonus(this, spellInfo, damage, victim);
        }
                           ...
                           Spell const*         spell /*= nullptr*/)
{
    ...
    if (!victim->IsAlive() || victim->IsInFlight() ||
        (victim->GetTypeId() == TYPEID_UNIT &&
         victim->ToCreature()->IsEvadingAttacks()))
        return;
    ...
    CleanDamage cleanDamage(damageInfo->cleanDamage,
                            damageInfo->absorb,
                            BASE_ATTACK,
                            MELEE_HIT_NORMAL);
...
void Unit::CalculateMeleeDamage(Unit*            victim,
                                ...
                                WeaponAttackType attackType,
                                const bool       sittingVictim)
{
    damageInfo->attacker = this;
    damageInfo->target   = victim;

    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        damageInfo->damages[i].damageSchoolMask =
            GetMeleeDamageSchoolMask(attackType, i);
    ...
    case BASE_ATTACK:
        damageInfo->procAttacker =
            PROC_FLAG_DONE_MELEE_AUTO_ATTACK | PROC_FLAG_DONE_MAINHAND_ATTACK;
        damageInfo->procVictim = PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK;
        break;
    case OFF_ATTACK:
        damageInfo->procAttacker =
            PROC_FLAG_DONE_MELEE_AUTO_ATTACK | PROC_FLAG_DONE_OFFHAND_ATTACK;
        damageInfo->procVictim = PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK;
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        if (damageInfo->target->IsImmunedToDamageOrSchool(
                SpellSchoolMask(damageInfo->damages[i].damageSchoolMask))) {
            immunedMask |= (1 << i);
            if (damageInfo->damages[i].damageSchoolMask !=
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        ...
        damage += CalculateDamage(
            damageInfo->attackType, false, addPctMods, itemDamagesMask);
        // Add melee damage bonus
        damage = MeleeDamageBonusDone(damageInfo->target,
                                      damage,
                                      damageInfo->attackType,
                                      nullptr,
                                      schoolMask);
        damage = damageInfo->target->MeleeDamageBonusTaken(
            this, damage, damageInfo->attackType, nullptr, schoolMask);

        // Script Hook For CalculateMeleeDamage -- Allow scripts to change the
        // Damage pre class mitigation calculations
        sScriptMgr->ModifyMeleeDamage(
            damageInfo->target, damageInfo->attacker, damage);
        ...
        if (IsDamageReducedByArmor(
                (SpellSchoolMask)(damageInfo->damages[i].damageSchoolMask))) {
            damageInfo->damages[i].damage =
                Unit::CalcArmorReducedDamage(this,
                                             damageInfo->target,
                                             damage,
                                             nullptr,
                                             0,
    ...
    case MELEE_HIT_CRIT: {
        ...
        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
            ...
            if (damageInfo->attackType == RANGED_ATTACK) {
                ...
                // Increase crit damage from SPELL_AURA_MOD_CRIT_DAMAGE_BONUS
                mod += (GetTotalAuraMultiplierByMiscMask(
    ...
    case MELEE_HIT_GLANCING: {
        ...
        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
            uint32 reducedDamage =
                uint32(reducePercent * damageInfo->damages[i].damage);
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        ...
        // attackType is checked already for BASE_ATTACK or OFF_ATTACK so it
        // can't be RANGED_ATTACK here
        if (CanApplyResilience()) {
        ...
        if (damageInfo->damages[i].damage > 0) {
            ...
            if (damageInfo->damages[i].absorb) {
                tmpHitInfo[i] |= (damageInfo->damages[i].damage -
                                              damageInfo->damages[i].absorb ==
                                          0
                                      ? HITINFO_FULL_ABSORB
    ...
    if ((tmpHitInfo[0] & HITINFO_FULL_ABSORB) != 0) {
        ...
        damageInfo->HitInfo |= ((tmpHitInfo[1] & HITINFO_PARTIAL_ABSORB) != 0)
                                   ? HITINFO_PARTIAL_ABSORB
                                   : HITINFO_FULL_ABSORB;
    }
    else {
    ...
    if ((tmpHitInfo[0] & HITINFO_FULL_RESIST) != 0) {
        ...
        damageInfo->HitInfo |= ((tmpHitInfo[1] & HITINFO_PARTIAL_RESIST) != 0)
                                   ? HITINFO_PARTIAL_RESIST
                                   : HITINFO_FULL_RESIST;
    }
    else {
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    auto canTakeMeleeDamage = [&]() {
        return victim->IsAlive() &&
               !victim->HasUnitState(UNIT_STATE_IN_FLIGHT) &&
               (victim->GetTypeId() != TYPEID_UNIT ||
                !victim->ToCreature()->IsEvadingAttacks());
    };
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        if (!canTakeMeleeDamage() ||
            (!damageInfo->damages[i].damage && !damageInfo->damages[i].absorb &&
             !damageInfo->damages[i].resist)) {
            continue;
        }
    ...
    if ((damageInfo->damages[0].damage + damageInfo->damages[1].damage) &&
        ((damageInfo->hitOutCome == MELEE_HIT_CRIT ||
         ...
         !victim->HasInArc(M_PI, this) &&
         (victim->GetTypeId() == TYPEID_PLAYER ||
          !victim->ToCreature()->isWorldBoss()) &&
         !victim->IsVehicle())) {
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->CastItemCombatSpell(victim,
                                        damageInfo->attackType,
    ...
    if (damageInfo->damages[0].damage + damageInfo->damages[1].damage) {
        ...
        AuraEffectList vDamageShieldsCopy(
            victim->GetAuraEffectsByType(SPELL_AURA_DAMAGE_SHIELD));
        for (AuraEffectList::const_iterator dmgShieldItr =
                 vDamageShieldsCopy.begin();
             dmgShieldItr != vDamageShieldsCopy.end();
             ++dmgShieldItr) {
            SpellInfo const* i_spellProto = (*dmgShieldItr)->GetSpellInfo();
            // Damage shield can be resisted...
            if (SpellMissInfo missInfo =
                    victim->SpellHitResult(this, i_spellProto, false)) {
            ...
            uint32 damage = uint32(std::max(
                0, (*dmgShieldItr)->GetAmount())); // xinef: done calculated at
                                                   // amount calculation
            ...
            if (Unit* caster = (*dmgShieldItr)->GetCaster()) {
                damage = caster->SpellDamageBonusDone(
                    ...
                    damage,
                    SPELL_DIRECT_DAMAGE,
                    (*dmgShieldItr)->GetEffIndex());
                damage = this->SpellDamageBonusTaken(
                    caster, i_spellProto, damage, SPELL_DIRECT_DAMAGE);
            }

            uint32 absorb = 0;

            DamageInfo dmgInfo(victim,
                               this,
                               damage,
                               i_spellProto,
                               i_spellProto->GetSchoolMask(),
                               SPELL_DIRECT_DAMAGE);
            Unit::CalcAbsorbResist(dmgInfo);
                                  ...
                                  uint8            effIndex)
{
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0)
        return false;
    if (spellInfo) {
        // there are spells with no specific attribute but they have "ignores
        // armor" in tooltip
        ...
        if (effIndex != MAX_SPELL_EFFECTS) {
            if (spellInfo->Effects[effIndex].ApplyAuraName ==
                ...
                spellInfo->Effects[effIndex].Effect ==
                    SPELL_EFFECT_SCHOOL_DAMAGE)
                if (spellInfo->GetEffectMechanicMask(effIndex) &
                    (1 << MECHANIC_BLEED))
                    return false;
        }
    }
                                    ...
                                    WeaponAttackType /*attackType*/)
{
    ...
    if (attacker) {
        ...
        AuraEffectList const& ResIgnoreAuras =
            attacker->GetAuraEffectsByType(SPELL_AURA_MOD_IGNORE_TARGET_RESIST);
        for (AuraEffectList::const_iterator j = ResIgnoreAuras.begin();
             j != ResIgnoreAuras.end();
             ++j) {
            if ((*j)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL)
                armor = floor(AddPct(armor, -(*j)->GetAmount()));
        }

        // Apply Player CR_ARMOR_PENETRATION rating and buffs from
        // stances\specializations etc.
        if (attacker->GetTypeId() == TYPEID_PLAYER) {
            float                 bonusPct = 0;
            AuraEffectList const& armorPenAuras =
                attacker->GetAuraEffectsByType(
                    SPELL_AURA_MOD_ARMOR_PENETRATION_PCT);
            for (AuraEffectList::const_iterator itr = armorPenAuras.begin();
                 itr != armorPenAuras.end();
                 ++itr) {
                if ((*itr)->GetSpellInfo()->EquippedItemClass == -1) {
                    if (!spellInfo || (*itr)->IsAffectedOnSpell(spellInfo) ||
                        (*itr)->GetMiscValue() & spellInfo->GetSchoolMask())
                        bonusPct += (*itr)->GetAmount();
                    else if (!(*itr)->GetMiscValue() &&
                             !(*itr)->HasSpellClassMask())
                        bonusPct += (*itr)->GetAmount();
                }
                else {
                    if (attacker->ToPlayer()->HasItemFitToSpellRequirements(
                            (*itr)->GetSpellInfo()))
                        bonusPct += (*itr)->GetAmount();
                }
            }
            ...
            if (victim->GetLevel() < 60)
                ...
                maxArmorPen = 400 + 85 * victim->GetLevel() +
                              4.5f * 85 * (victim->GetLevel() - 59);

            // Cap armor penetration to this number
            maxArmorPen = std::min((armor + maxArmorPen) / 3, armor);
            // Figure out how much armor do we ignore
            float armorPen = CalculatePct(
                maxArmorPen,
                bonusPct + attacker->ToPlayer()->GetRatingBonusValue(
                                     ...
                                     Unit const*     victim)
{
    ...
    if (owner) {
        ...
        if (player && owner->GetEntry() != WORLD_TRIGGER) {
            victimResistance += float(player->GetTotalAuraModifierByMiscMask(
                SPELL_AURA_MOD_TARGET_RESISTANCE, schoolMask));
            victimResistance -= float(player->GetSpellPenetrationItemMod());
        }
        else
            victimResistance += float(owner->GetTotalAuraModifierByMiscMask(
                SPELL_AURA_MOD_TARGET_RESISTANCE, schoolMask));
    }

    victimResistance = std::max(victimResistance, 0.0f);
    if (owner)
        victimResistance += std::max(
            (float(victim->GetLevel()) - float(owner->GetLevel())) * 5.0f,
            0.0f);
...
void Unit::CalcAbsorbResist(DamageInfo& dmgInfo, bool Splited)
{
    ...
    if (!(schoolMask & SPELL_SCHOOL_MASK_NORMAL) &&
        ...
        (!spellInfo || (!spellInfo->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) &&
                        !spellInfo->HasAttribute(SPELL_ATTR4_NO_CAST_LOG)))) {
        float averageResist =
            Unit::GetEffectiveResistChance(attacker, schoolMask, victim);

        float discreteResistProbability[11];
        for (uint32 i = 0; i < 11; ++i) {
            discreteResistProbability[i] =
                0.5f - 2.5f * std::fabs(0.1f * i - averageResist);
        ...
        {
            if (attacker) {
                AuraEffectList const& ResIgnoreAurasAb =
                    attacker->GetAuraEffectsByType(
                        SPELL_AURA_MOD_ABILITY_IGNORE_TARGET_RESIST);
                for (AuraEffectList::const_iterator j =
                         ResIgnoreAurasAb.begin();
                     j != ResIgnoreAurasAb.end();
                     ++j)
            ...
            // pussywizard:
            if (spellInfo && spellInfo->HasAttribute(
                                 SPELL_ATTR0_CU_SCHOOLMASK_NORMAL_WITH_MAGIC)) {
                uint32 damageAfterArmor = Unit::CalcArmorReducedDamage(
                    attacker, victim, damage, spellInfo, 0, BASE_ATTACK);
                uint32 armorReduction = damage - damageAfterArmor;
                if (armorReduction <
                    damageResisted) // pick the lower one, the weakest
                                    // resistance counts
    ...
    if (attacker) {
        AuraEffectList const& AbsIgnoreAurasA =
            attacker->GetAuraEffectsByType(SPELL_AURA_MOD_TARGET_ABSORB_SCHOOL);
        for (AuraEffectList::const_iterator itr = AbsIgnoreAurasA.begin();
             itr != AbsIgnoreAurasA.end();
             ++itr) {
    ...
    // We're going to call functions which can modify content of the list during
    // iteration over it's elements Let's copy the list so we can prevent
    // iterator invalidation
    AuraEffectList vSchoolAbsorbCopy(
        victim->GetAuraEffectsByType(SPELL_AURA_SCHOOL_ABSORB));
    ...
    for (AuraEffectList::iterator itr = vSchoolAbsorbCopy.begin();
        ...
        // Check if aura was removed during iteration - we don't need to work on
        // such auras
        AuraApplication const* aurApp =
            absorbAurEff->GetBase()->GetApplicationOfTarget(victim->GetGUID());
        if (!aurApp)
            continue;
        ...
        // aura with infinite absorb amount - let the scripts handle absorbtion
        // amount, set here to 0 for safety
        if (currentAbsorb < 0)
            currentAbsorb = 0;
        ...
        absorbAurEff->GetBase()->CallScriptEffectAbsorbHandlers(
            absorbAurEff, aurApp, dmgInfo, tempAbsorb, defaultPrevented);
        ...
        currentAbsorb =
            RoundToInterval(currentAbsorb, 0, int32(dmgInfo.GetDamage()));
        ...
        absorbAurEff->GetBase()->CallScriptEffectAfterAbsorbHandlers(
            absorbAurEff, aurApp, dmgInfo, tempAbsorb);
    ...
    AuraEffectList vManaShieldCopy(
        victim->GetAuraEffectsByType(SPELL_AURA_MANA_SHIELD));
    for (AuraEffectList::const_iterator itr = vManaShieldCopy.begin();
        ...
        if (float manaMultiplier =
                absorbAurEff->GetSpellInfo()
                    ->Effects[absorbAurEff->GetEffIndex()]
                    .CalcValueMultiplier(absorbAurEff->GetCaster()))
            manaReduction = int32(float(manaReduction) * manaMultiplier);

        int32 manaTaken = -victim->ModifyPower(POWER_MANA, -manaReduction);

        // take case when mana has ended up into account
        currentAbsorb = currentAbsorb
                            ? int32(float(currentAbsorb) *
                                    (float(manaTaken) / float(manaReduction)))
                            : 0;
    ...
    if (/*victim != attacker &&*/ !Splited) {
        ...
        AuraEffectList vSplitDamageFlatCopy(
            victim->GetAuraEffectsByType(SPELL_AURA_SPLIT_DAMAGE_FLAT));
        for (AuraEffectList::iterator itr = vSplitDamageFlatCopy.begin();
            ...
            if (!caster || (caster == victim) || !caster->IsInWorld() ||
                !caster->IsAlive())
                continue;
            ...
            // absorb must be smaller than the damage itself
            splitDamage =
                RoundToInterval(splitDamage, 0, int32(dmgInfo.GetDamage()));
            ...
            DamageInfo splittedDmgInfo(attacker,
                                       caster,
                                       splitted,
                                       spellInfo,
                                       schoolMask,
            ...
            // create procs
            createProcFlags(
                spellInfo, BASE_ATTACK, false, procAttacker, procVictim);
            caster->ProcDamageAndSpellFor(true,
                                          attacker,
                                          procVictim,
                                          procEx,
                                          BASE_ATTACK,
            ...
            if (attacker) {
                attacker->SendSpellNonMeleeDamageLog(caster,
                                                     (*itr)->GetSpellInfo(),
                                                     splitted,
                                                     schoolMask,
                                                     splitted_absorb,
            ...
            CleanDamage cleanDamage =
                CleanDamage(splitted, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
            Unit::DealDamage(attacker,
                             caster,
                             splitted,
                             &cleanDamage,
        ...
        AuraEffectList vSplitDamagePctCopy(
            victim->GetAuraEffectsByType(SPELL_AURA_SPLIT_DAMAGE_PCT));
        for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(), next;
            ...
            // work on such auras
            AuraApplication const* aurApp =
                (*itr)->GetBase()->GetApplicationOfTarget(victim->GetGUID());
            if (!aurApp)
                continue;
            ...
            if (!splitSpellInfo->Effects[(*itr)->GetEffIndex()]
                     .IsAreaAuraEffect() &&
                !splitSpellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT))
                if (!caster->IsWithinLOSInMap(victim) ||
                    !caster->IsWithinDist(
                        victim,
                        splitSpellInfo->GetMaxRange(
                            splitSpellInfo->IsPositive(), caster)))
                    continue;

            uint32 splitDamage =
                CalculatePct(dmgInfo.GetDamage(), (*itr)->GetAmount());
            SpellSchoolMask splitSchoolMask = schoolMask;

            (*itr)->GetBase()->CallScriptEffectSplitHandlers(
                *itr, aurApp, dmgInfo, splitDamage);

            // absorb must be smaller than the damage itself
            splitDamage = RoundToInterval(
                splitDamage, uint32(0), uint32(dmgInfo.GetDamage()));
...
void Unit::CalcHealAbsorb(HealInfo& healInfo)
{
    ...
    // absorb without mana cost
    AuraEffectList const& vHealAbsorb =
        healInfo.GetTarget()->GetAuraEffectsByType(
            SPELL_AURA_SCHOOL_HEAL_ABSORB);
    for (AuraEffectList::const_iterator i = vHealAbsorb.begin();
         i != vHealAbsorb.end() && absorbAmount <= healing;
         ++i) {
    ...
    if (existExpired) {
        for (AuraEffectList::const_iterator i = vHealAbsorb.begin();
            ...
            if (auraEff->GetAmount() <= 0) {
                ...
                if (removedAuras + 1 <
                    healInfo.GetTarget()->m_removedAurasCount)
                    i = vHealAbsorb.begin();
            }
        }
    }
                               ...
                               bool             ignoreCasting /*= false*/)
{
    ...
    if ((attType == BASE_ATTACK || attType == OFF_ATTACK) &&
        !IsWithinLOSInMap(victim))
        return;

    // CombatStart puts the target into stand state, so we need to cache sit
    // state here to know if we should crit later
    const bool sittingVictim =
        victim->GetTypeId() == TYPEID_PLAYER &&
        (victim->IsSitState() ||
         victim->getStandState() == UNIT_STAND_STATE_SLEEP);
    ...
    // melee attack spell casted at main hand attack only - no normal melee dmg
    // dealt
    ...
    if (meleeAttack) {
        ...
        Unit::ProcDamageAndSpell(damageInfo.attacker,
                                 damageInfo.target,
                                 damageInfo.procAttacker,
                                 damageInfo.procVictim,
                                 damageInfo.procEx,
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            LOG_DEBUG("entities.unit",
                      "AttackerStateUpdate: (Player) {} attacked {} for {} "
                      "dmg, absorbed {}, blocked {}, resisted {}.",
                      GetGUID().ToString(),
            ...
            LOG_DEBUG("entities.unit",
                      "AttackerStateUpdate: (NPC) {} attacked {} for {} dmg, "
                      "absorbed {}, blocked {}, resisted {}.",
                      GetGUID().ToString(),
                      victim->GetGUID().ToString(),
                      dmgInfo.GetDamage(),
                      dmgInfo.GetAbsorb(),
        ...
        // Let the pet know we've started attacking someting. Handles melee
        // attacks only Spells such as auto-shot and others handled in
        // WorldSession::HandleCastSpellOpcode
        if (GetTypeId() == TYPEID_PLAYER && !m_Controlled.empty())
            for (Unit::ControlSet::iterator itr = m_Controlled.begin();
...
bool Unit::GetMeleeAttackPoint(Unit* attacker, Position& pos)
{
    ...
    for (const auto& otherAttacker : attackers) {
        ...
        float curretAngle =
            atan(attacker->GetExactDist2d(otherAttacker) / meleeReach);
    ...
    // in instance: the more attacker there are, the higher will be the
    // tollerance outside: creatures should not intersecate
    float angleTollerance =
        attacker->GetMap()->IsDungeon()
            ? requiredAngle - requiredAngle * tanh(validAttackers / 5.0f)
            : requiredAngle;
    ...
    if (!GetMap()->CanReachPositionAndGetValidCoords(
            this, x, y, z, true, true)) {
        GetNearPoint(attacker,
                     x,
                     y,
                     z,
                     distance,
                     0.0f,
                     absAngle * -1); // try the other side
...
void Unit::AddExtraAttacks(uint32 count)
{
    ...
    if (!targetGUID) {
        if (ObjectGuid selection = GetTarget()) {
            targetGUID =
                selection; // Spell was cast directly (not triggered by aura)
        }
        else
            return;
    }
                                              ...
                                              WeaponAttackType attType) const
{
    ...
    float miss_chance =
        MeleeSpellMissChance(victim,
                             attType,
                             int32(GetWeaponSkillValue(attType, victim)) -
                                 int32(victim->GetMaxSkillValueForLevel(this)),
                             0);
    ...
    // Useful if want to specify crit & miss chances for melee, else it could be
    // removed
    // LOG_DEBUG("entities.unit", "MELEE OUTCOME: miss {} crit {} dodge {} parry
    // {} block {}", miss_chance, crit_chance, dodge_chance, parry_chance,
    // block_chance);

    return RollMeleeOutcomeAgainst(victim,
                                   attType,
                                   int32(crit_chance * 100),
                                   int32(miss_chance * 100),
                                   int32(dodge_chance * 100),
                                   int32(parry_chance * 100),
                                   int32(block_chance * 100));
}
                                              ...
                                              int32 block_chance) const
{
    ...
    sScriptMgr->OnBeforeRollMeleeOutcomeAgainst(this,
                                                victim,
                                                attType,
    ...
    LOG_DEBUG("entities.unit",
              "RollMeleeOutcomeAgainst: skill bonus of {} for attacker",
              skillBonus);
    // LOG_DEBUG("entities.unit", "RollMeleeOutcomeAgainst: rolled {}, miss {},
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && !victim->HasInArc(M_PI, this) &&
        ...
        // LOG_DEBUG("entities.unit", "RollMeleeOutcomeAgainst: attack came from
        // behind and victim was a player.");
    }
    ...
    // Max 40% chance to score a glancing blow against mobs that are higher
    // level (can do only players and pets and not with ranged weapon)
    if (attType != RANGED_ATTACK && (GetTypeId() == TYPEID_PLAYER || IsPet()) &&
        ...
        if (roll < (sum += tmp)) {
            LOG_DEBUG("entities.unit",
                      "RollMeleeOutcomeAgainst: GLANCING <{}, {})",
                      sum - 4000,
                      sum);
    ...
    if (getLevelForTarget(victim) >= victim->getLevelForTarget(this) + 4 &&
        // can be from by creature (if can) or from controlled player that
        // considered as creature
        !IsControlledByPlayer() &&
        !(GetTypeId() == TYPEID_UNIT &&
          ToCreature()->GetCreatureTemplate()->flags_extra &
              CREATURE_FLAG_EXTRA_NO_CRUSHING_BLOWS)) {
        // when their weapon skill is 15 or more above victim's defense skill
        tmp          = victimDefenseSkill;
        int32 tmpmax = victimMaxSkillValueForLevel;
        // having defense above your maximum (from items, talents etc.) has no
        // effect
        ...
        if (tmp >= 15) {
            ...
            if (roll < (sum += tmp)) {
                LOG_DEBUG("entities.unit",
                          "RollMeleeOutcomeAgainst: CRUSHING <{}, {})",
                          sum - tmp,
                          sum);
        ...
        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
            if (itemDamagesMask & (1 << i)) {
                ...
                CalculateMinMaxDamage(
                    attType, normalized, addTotalPct, minTmp, maxTmp, i);
                minDamage += minTmp;
                maxDamage += maxTmp;
            }
        }
...
float Unit::CalculateLevelPenalty(SpellInfo const* spellProto) const
{
    ...
    float LvlFactor =
        (float(spellProto->SpellLevel) + 6.0f) / float(GetLevel());
    if (LvlFactor > 1.0f)
        LvlFactor = 1.0f;
...
{
    // pussywizard: calling SendMeleeAttackStop without clearing
    // UNIT_STATE_MELEE_ATTACKING and then AttackStart the same player may spoil
    // npc rotating! pussywizard: this happens in some boss scripts, just add
    // clearing here ClearUnitState(UNIT_STATE_MELEE_ATTACKING); // commented
    // out for now

    WorldPacket data(SMSG_ATTACKSTOP, (8 + 8 + 4));
    data << GetPackGUID();
    data << (victim ? victim->GetPackGUID() : PackedGuid());
    data << uint32(0); //! Can also take the value 0x01, which seems related to
                       //! updating rotation
    SendMessageToSet(&data, true);
                          ...
                          WeaponAttackType attackType)
{
    ...
    if (victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION) ||
        ...
        blockChance += (int32(GetWeaponSkillValue(attackType)) -
                        int32(victim->GetMaxSkillValueForLevel())) *
                       0.04f;
                                        ...
                                        SpellInfo const* spellInfo)
{
    ...
    // - they are meele, but can't be dodged/parried/deflected because of ranged
    // dmg class
    ...
    int32 skillDiff =
        attackerWeaponSkill - int32(victim->GetMaxSkillValueForLevel(this));

    uint32 roll = urand(0, 10000);

    uint32 missChance =
        uint32(MeleeSpellMissChance(victim, attType, skillDiff, spellInfo->Id) *
               100.0f);
    // Roll miss
    ...
    bool canBlock = spellInfo->HasAttribute(SPELL_ATTR3_COMPLETELY_BLOCKED) &&
                    !spellInfo->HasAttribute(SPELL_ATTR0_CU_DIRECT_DAMAGE);
    ...
    if (attType == RANGED_ATTACK) {
        ...
        if (!victim->HasUnitState(UNIT_STATE_STUNNED) &&
            ...
            int32 deflect_chance =
                victim->GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS) * 100;
            tmp += deflect_chance;
            if (roll < tmp)
    ...
    AuraEffectList const& ignore =
        GetAuraEffectsByType(SPELL_AURA_IGNORE_COMBAT_RESULT);
    for (AuraEffectList::const_iterator i = ignore.begin(); i != ignore.end();
        ...
        default:
            LOG_DEBUG("entities.unit",
                      "Spell {} SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled "
                      "state {}",
                      (*i)->GetId(),
                      (*i)->GetMiscValue());
            break;
    ...
    if (canDodge) {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            dodgeChance -=
                int32(ToPlayer()->GetExpertiseDodgeOrParryReduction(attType) *
                      100.0f);
        else
                                        ...
                                        SpellInfo const* spellInfo)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_MISS_CHANCE_MULTIPLIER_ONLY_FOR_PLAYERS) &&
        GetTypeId() != TYPEID_PLAYER) // keep it as it was originally (7 and 11)
    {
        MISS_CHANCE_MULTIPLIER = victim->GetTypeId() == TYPEID_PLAYER ? 7 : 11;
    }
    else {
        MISS_CHANCE_MULTIPLIER =
            sWorld->getRate(victim->GetTypeId() == TYPEID_PLAYER
    ...
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(
            spellInfo->Id, SPELLMOD_RESIST_MISS_CHANCE, modHitChance);
    ...
    // xinef: imo it should completly ignore all calculations, eg: 14792. Hits
    // 80 level players on blizz without any problems
    // if (!spell->HasAttribute(SPELL_ATTR3_ALWAYS_HIT))
    {
        ...
        if (spellInfo->IsAffectingArea())
            modHitChance -=
                victim->GetTotalAuraModifier(SPELL_AURA_MOD_AOE_AVOIDANCE);

        // Decrease hit chance from victim rating bonus
        if (victim->GetTypeId() == TYPEID_PLAYER)
            modHitChance -= int32(
                victim->ToPlayer()->GetRatingBonusValue(CR_HIT_TAKEN_SPELL));
    }

    int32 HitChance = modHitChance * 100;
    // Increase hit chance from attacker SPELL_AURA_MOD_SPELL_HIT_CHANCE and
    // attacker ratings Xinef: Totems should inherit casters ratings?
    if (IsTotem()) {
    ...
    int32 rand = irand(1, 10000); // Needs to be  1 to 10000 to avoid the
                                  // 1/10000 chance to miss on 100% hit rating
    ...
    if (!spellInfo->IsPositive() &&
        ...
        if (bNegativeAura) {
            tmp += victim->GetMaxPositiveAuraModifierByMiscValue(
                       SPELL_AURA_MOD_DEBUFF_RESISTANCE,
                       int32(spellInfo->Dispel)) *
                   100;
            tmp += victim->GetMaxNegativeAuraModifierByMiscValue(
                       SPELL_AURA_MOD_DEBUFF_RESISTANCE,
                       int32(spellInfo->Dispel)) *
                   100;
        }
        ...
        if (spellInfo->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) &&
            (spellInfo->GetSchoolMask() &
             (SPELL_SCHOOL_MASK_NORMAL | SPELL_SCHOOL_MASK_HOLY)) == 0)
            tmp += int32(Unit::GetEffectiveResistChance(
                             this, spellInfo->GetSchoolMask(), victim) *
                         10000.0f); // 100 for spell calculations, and 100 for
...
//   Resist
SpellMissInfo
Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect)
{
    // Check for immune
    ...
    /// @todo: client not show miss log for this spells - so need find info for
    /// this in dbc and use it!
    if ((spell->IsPositive() || spell->HasEffect(SPELL_EFFECT_DISPEL)) &&
        (!IsHostileTo(
            victim))) // prevent from affecting enemy by "positive" spell
        return SPELL_MISS_NONE;
    ...
    if (victim->GetTypeId() == TYPEID_UNIT &&
        victim->ToCreature()->IsEvadingAttacks() &&
        !spell->HasAura(SPELL_AURA_CONTROL_VEHICLE) &&
        !spell->HasAttribute(SPELL_ATTR0_CU_IGNORE_EVADE))
        return SPELL_MISS_EVADE;

    // Try victim reflect spell
    if (CanReflect) {
        int32 reflectchance =
            victim->GetTotalAuraModifier(SPELL_AURA_REFLECT_SPELLS);
        Unit::AuraEffectList const& mReflectSpellsSchool =
            victim->GetAuraEffectsByType(SPELL_AURA_REFLECT_SPELLS_SCHOOL);
        for (Unit::AuraEffectList::const_iterator i =
                 mReflectSpellsSchool.begin();
             i != mReflectSpellsSchool.end();
             ++i)
            if ((*i)->GetMiscValue() & spell->GetSchoolMask())
                reflectchance += (*i)->GetAmount();
        if (reflectchance > 0 && roll_chance_i(reflectchance)) {
            // Start triggers for remove charges if need (trigger only for
            // victim, and mark as active spell)
            // ProcDamageAndSpell(victim, PROC_FLAG_NONE,
            // PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1,
            // BASE_ATTACK, spell);
    ...
    case SPELL_DAMAGE_CLASS_NONE: {
        ...
        // Xinef: apply DAMAGE_CLASS_MAGIC conditions to damaging
        // DAMAGE_CLASS_NONE spells
        for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
            if (spell->Effects[i].Effect &&
                spell->Effects[i].Effect != SPELL_EFFECT_SCHOOL_DAMAGE)
                if (spell->Effects[i].ApplyAuraName !=
                    SPELL_AURA_PERIODIC_DAMAGE)
                    return SPELL_MISS_NONE;
        [[fallthrough]];
    }
...
SpellMissInfo
Unit::SpellHitResult(Unit* victim, Spell const* spell, bool CanReflect)
{
...
float Unit::GetUnitDodgeChance() const
{
    if (GetTypeId() == TYPEID_PLAYER)
        return ToPlayer()
            ->GetRealDodge(); // GetFloatValue(PLAYER_DODGE_PERCENTAGE);
    else {
        if (ToCreature()->IsTotem())
            return 0.0f;
        else {
            float dodge =
                ToCreature()->isWorldBoss()
                    ? 5.85f
                    : 5.0f; // Xinef: bosses should have 6.5% dodge (5.9 + 0.6
                            // from defense skill difference)
    ...
    if (Player const* player = ToPlayer()) {
        if (player->CanParry()) {
            ...
            if (tmpitem)
                chance =
                    player
                        ->GetRealParry(); // GetFloatValue(PLAYER_PARRY_PERCENTAGE);
        }
    }
                                  ...
                                  Unit const*      victim) const
{
    ...
    AuraEffectList const& mTotalAuraList =
    ...
    if (attackType != RANGED_ATTACK)
        Unit::ApplyResilience(
            victim, &crit, nullptr, false, CR_CRIT_TAKEN_MELEE);
    else
        Unit::ApplyResilience(
            victim, &crit, nullptr, false, CR_CRIT_TAKEN_RANGED);

    // Apply crit chance from defence skill
    crit += (int32(GetMaxSkillValueForLevel(victim)) -
             int32(victim->GetDefenseSkillValue(this))) *
            0.04f;

    // xinef: SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE should be
                                 ...
                                 Unit const*      target) const
{
    ...
    if (Player const* player = ToPlayer()) {
        ...
        case BASE_ATTACK:
            value +=
                uint32(player->GetRatingBonusValue(CR_WEAPON_SKILL_MAINHAND));
            break;
        case OFF_ATTACK:
            value +=
                uint32(player->GetRatingBonusValue(CR_WEAPON_SKILL_OFFHAND));
            break;
        case RANGED_ATTACK:
            value +=
                uint32(player->GetRatingBonusValue(CR_WEAPON_SKILL_RANGED));
            break;
        default:
            break;
        }
...
void Unit::_UpdateSpells(uint32 time)
{
    ...
    // m_auraUpdateIterator can be updated in indirect called code at aura
    // remove to skip next planned to update but removed auras
    for (m_auraUpdateIterator = m_ownedAuras.begin();
         m_auraUpdateIterator != m_ownedAuras.end();) {
        Aura* i_aura = m_auraUpdateIterator->second;
        ++m_auraUpdateIterator; // need shift to next for allow update if need
                                // into aura update
    ...
    for (AuraMap::iterator i = m_ownedAuras.begin(); i != m_ownedAuras.end();) {
        ...
        else if (i->second->GetSpellInfo()->IsChanneled() &&
                 i->second->GetCasterGUID() != GetGUID() &&
                 !ObjectAccessor::GetWorldObject(*this,
                                                 i->second->GetCasterGUID()))
            RemoveOwnedAura(
                i, AURA_REMOVE_BY_CANCEL); // remove channeled auras when caster
                                           // is not on the same map
        else
            ++i;
    }

    for (VisibleAuraMap::iterator itr = m_visibleAuras.begin();
         itr != m_visibleAuras.end();
         ++itr)
    ...
    if (!m_gameObj.empty()) {
        for (GameObjectList::iterator itr = m_gameObj.begin();
             itr != m_gameObj.end();) {
...
void Unit::_UpdateAutoRepeatSpell()
{
    ...
    if ((GetTypeId() == TYPEID_PLAYER && ToPlayer()->isMoving() &&
         spellProto->Id != HUNTER_AUTOSHOOT) ||
        IsNonMeleeSpellCast(
            false, false, true, spellProto->Id == HUNTER_AUTOSHOOT)) {
        // cancel wand shoot
...
{
    ASSERT(pSpell); // nullptr may be never passed here, use InterruptSpell or
    ...
    bool bySelf =
        m_currentSpells[CSpellType] &&
        m_currentSpells[CSpellType]->m_spellInfo->Id == pSpell->m_spellInfo->Id;
                          ...
                          bool              bySelf)
{
    ...
    if (spell && (withDelayed || spell->getState() != SPELL_STATE_DELAYED) &&
        (withInstant || spell->GetCastTime() > 0 ||
         spell->getState() ==
             SPELL_STATE_CASTING)) // xinef: or spell is in casting state
                                   // (channeled spells only)
    {
                               ...
                               bool skipInstant) const
{
    ...
    if (m_currentSpells[CURRENT_GENERIC_SPELL] &&
        ...
        (withDelayed || m_currentSpells[CURRENT_GENERIC_SPELL]->getState() !=
                            SPELL_STATE_DELAYED)) {
        if (!skipInstant ||
    ...
    if (!skipChanneled && m_currentSpells[CURRENT_CHANNELED_SPELL] &&
        (m_currentSpells[CURRENT_CHANNELED_SPELL]->getState() !=
         SPELL_STATE_FINISHED)) {
        if (!isAutoshoot || !m_currentSpells[CURRENT_CHANNELED_SPELL]
                                 ->m_spellInfo->HasAttribute(
                                     SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS))
            return true;
    }
    ...
    if (m_currentSpells[CURRENT_AUTOREPEAT_SPELL] &&
        (!spell_id ||
         m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id ==
             spell_id))
        InterruptSpell(
            CURRENT_AUTOREPEAT_SPELL, withDelayed, withInstant, bySelf);

    // channeled spells are interrupted if they are not finished, even if they
    // are delayed
...
bool Unit::IsMovementPreventedByCasting() const
{
    ...
    // channeled spells during channel stage (after the initial cast timer)
    // allow movement with a specific spell attribute
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL]) {
        if (spell->getState() != SPELL_STATE_FINISHED &&
...
bool Unit::isInAccessiblePlaceFor(Creature const* c) const
{
    if (c->GetMapId() == 618) // Ring of Valor
    {
        ...
        if (BattlegroundMap* bgMap = c->GetMap()->ToBattlegroundMap())
            if (Battleground* bg = bgMap->GetBG())
                if (bg->GetStartTime() < 80133) // 60000ms preparation time +
                                                // 20133ms elevator rise time
                    return false;
    }
    else if (c->GetMapId() == 631) // Icecrown Citadel
    {
        ...
        if (c->GetTransport() != this->GetTransport() &&
            ((c->GetTransport() && c->GetTransport()->IsStaticTransport()) ||
             (this->GetTransport() &&
              this->GetTransport()->IsStaticTransport())))
            return false;

        // special handling for ICC (map 631), for non-flying pets in Gunship
        // Battle, for trash npcs this is done via CanAIAttack
        ...
        // pussywizard: prevent any bugs by passengers exiting transports or
        // normal creatures flying away
        if (c->GetTransport() != this->GetTransport())
            return false;
    }
    ...
    if (isInWater ||
        (liquidStatus == LIQUID_MAP_ABOVE_WATER &&
         (IsFalling() || (ToPlayer() && ToPlayer()->IsFalling())))) {
...
void Unit::HandleSafeUnitPointersOnDelete(Unit* thisUnit)
{
    if (thisUnit->SafeUnitPointerSet.empty())
        return;
    for (std::set<SafeUnitPointer*>::iterator itr =
             thisUnit->SafeUnitPointerSet.begin();
         itr != thisUnit->SafeUnitPointerSet.end();
         ++itr)
        (*itr)->UnitDeleted();
    ...
    bool             periodicReset /*= false*/)
{
    ...
    // passive and Incanter's Absorption and auras with different type can stack
    // with themselves any number of times
    if (!newAura->IsMultiSlotAura()) {
        ...
        // find current aura from spell and change it's stackamount, or refresh
        // it's duration
        if (Aura* foundAura = GetOwnedAura(
                newAura->Id,
                newAura->HasAttribute(SPELL_ATTR0_CU_SINGLE_AURA_STACK)
                    ? ObjectGuid::Empty
                    : casterGUID,
                newAura->HasAttribute(SPELL_ATTR0_CU_ENCHANT_PROC)
                    ? castItemGUID
                    : ObjectGuid::Empty,
                0)) {
            ...
            // update basepoints with new values - effect amount will be
            // recalculated in ModStackAmount
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            ...
            if (castItemGUID != foundAura->GetCastItemGUID()) {
                ObjectGuid* oldGUID =
                    const_cast<ObjectGuid*>(&foundAura->m_castItemGuid);
...
void Unit::_AddAura(UnitAura* aura, Unit* caster)
{
    ...
    aura->SetIsSingleTarget(caster &&
                            (aura->GetSpellInfo()->IsSingleTarget() ||
    ...
    if (aura->IsSingleTarget()) {
         ...
         *        but may be created as a result of aura links (player mounts
         * with passengers)
         */
...
// aura application effects are handled separately to prevent aura list
// corruption
AuraApplication* Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
    // can't apply aura on unit which is going to be deleted - to not create a
    // memory leak
    ASSERT(!m_cleanupDone);
    ...
    ASSERT(!aura->IsAppliedOnTarget(GetGUID()) &&
           "Unit::_CreateAuraApplication: aura musn't be applied on target");
    ...
    // (will be cleanup after load) Xinef: Added IsAllowingDeadTarget check
    if (!IsAlive() && !aurSpellInfo->IsDeathPersistent() &&
        !aurSpellInfo->IsAllowingDeadTarget() &&
        (GetTypeId() != TYPEID_PLAYER ||
    ...
    // xinef: do not insert our application to interruptible list if application
    // target is not the owner (area auras) xinef: even if it gets removed, it
    // will be reapplied in a second
...
void Unit::_ApplyAura(AuraApplication* aurApp, uint8 effMask)
{
    ...
    if (AuraStateType aState = spellInfo->GetAuraState()) {
        ...
        if ((aStateMask & PER_CASTER_AURA_STATE_MASK) &&
            HasFlag(UNIT_FIELD_AURASTATE, aStateMask))
                        ...
                        AuraRemoveMode                removeMode)
{
    ...
    // Remove all pointers from lists here to prevent possible pointer
    // invalidation on spellcast/auraapply/auraremove
    m_appliedAuras.erase(i);

    // xinef: do not insert our application to interruptible list if application
    // target is not the owner (area auras) xinef: event if it gets removed, it
    // will be reapplied in a second
    ...
    if (auraState) {
        ...
        for (AuraStateAurasMap::iterator itr =
                 m_auraStateAuras.lower_bound(auraState);
    ...
    // remove effects of the spell - needs to be done after removing aura from
    // lists
    ...
    if (aurApp->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE && IsTotem() &&
        GetGUID() == aura->GetCasterGUID()) {
        if (ToTotem()->GetSpell() == aura->GetId() &&
            ToTotem()->GetTotemType() == TOTEM_PASSIVE)
...
{
    // aura can be removed from unit only if it's applied on it, shouldn't
    // happen
    ASSERT(aurApp->GetBase()->GetApplicationOfTarget(GetGUID()) == aurApp);

    uint32                           spellId = aurApp->GetBase()->GetId();
    AuraApplicationMapBoundsNonConst range =
        m_appliedAuras.equal_range(spellId);

    for (AuraApplicationMap::iterator iter = range.first;
         iter != range.second;) {
...
void Unit::RemoveOwnedAura(AuraMap::iterator& i, AuraRemoveMode removeMode)
{
    ...
    // if unit currently update aura list then make safe update iterator shift
    // to next
...
Aura* Unit::GetOwnedAura(uint32     spellId,
                         ObjectGuid casterGUID,
                         ObjectGuid itemCasterGUID,
                         uint8      reqEffMask,
                         Aura*      except) const
{
    AuraMapBounds range = m_ownedAuras.equal_range(spellId);
    for (AuraMap::const_iterator itr = range.first; itr != range.second;
         ++itr) {
        if (((itr->second->GetEffectMask() & reqEffMask) == reqEffMask) &&
            (!casterGUID || itr->second->GetCasterGUID() == casterGUID) &&
            (!itemCasterGUID ||
...
void Unit::RemoveAura(AuraApplication* aurApp, AuraRemoveMode mode)
{
    ...
    for (AuraApplicationMap::iterator iter = range.first;
        ...
        if (aurApp == iter->second) {
            // Prevent Arena Preparation aura from being removed by player
            // actions It's an invisibility spell so any interaction/spell cast
            // etc. removes it. Should only be removed by the arena script, once
            // the match starts.
            if (aurApp->GetBase()->HasEffectType(
...
void Unit::RemoveAppliedAuras(
    uint32 spellId, std::function<bool(AuraApplication const*)> const& check)
{
    for (AuraApplicationMap::iterator iter =
             m_appliedAuras.lower_bound(spellId);
         iter != m_appliedAuras.upper_bound(spellId);) {
        if (check(iter->second)) {
            RemoveAura(iter);
            continue;
        }
                                         ...
                                         uint8      chargesRemoved /*= 1*/)
{
    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;) {
        ...
        if (aura->GetCasterGUID() == casterGUID) {
            ...
            if (aura->GetSpellInfo()->HasAttribute(
                    SPELL_ATTR7_DISPEL_REMOVES_CHARGES))
                aura->ModCharges(-dispelInfo.GetRemovedCharges(),
                                 AURA_REMOVE_BY_ENEMY_SPELL);
            else
                aura->ModStackAmount(-dispelInfo.GetRemovedCharges(),
                                     AURA_REMOVE_BY_ENEMY_SPELL);
                ...
                if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x1000) {
                    if (Unit* caster = aura->GetCaster()) {
                        if (AuraEffect* aureff = caster->GetAuraEffect(
                                ...
                                1)) {
                            if (Aura* noxious =
                                    Aura::TryCreate(aura->GetSpellInfo(),
                                                    aura->GetEffectMask(),
                                                    dispeller,
                                                    caster)) {
                                noxious->SetDuration(aura->GetDuration() *
                                                     aureff->GetAmount() / 100);
                                if (aura->GetUnitOwner())
                                    if (const std::vector<int32>*
                                            spell_triggered =
                                                sSpellMgr->GetSpellLinked(
                                                    -int32(aura->GetId())))
                                        for (std::vector<int32>::const_iterator
                                                 itr = spell_triggered->begin();
                                             itr != spell_triggered->end();
                                             ++itr)
                                            aura->GetUnitOwner()
            ...
            case SPELLFAMILY_DEATHKNIGHT: {
                ...
                if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x4000000) {
                    if (AuraEffect* aureff =
                            GetAuraEffect(SPELL_AURA_MOD_DECREASE_SPEED,
                                        ...
                                        Unit*      stealer)
{
    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;) {
        ...
        if (aura->GetCasterGUID() == casterGUID) {
            ...
            bool stealCharge = aura->GetSpellInfo()->HasAttribute(
                SPELL_ATTR7_DISPEL_REMOVES_CHARGES);
            // Cast duration to unsigned to prevent permanent aura's such as
            // Righteous Fury being permanently added to caster
            uint32 dur = std::min(2u * MINUTE * IN_MILLISECONDS,
                                  uint32(aura->GetDuration()));

            if (Aura* oldAura =
                    stealer->GetAura(aura->GetId(), aura->GetCasterGUID())) {
                if (stealCharge)
                ...
                // single target state must be removed before aura creation to
                // preserve existing single target aura
                if (aura->IsSingleTarget())
                    aura->UnregisterSingleTarget();

                // Xinef: if stealer has same aura
                Aura* curAura = stealer->GetAura(aura->GetId());
                if (!curAura || (!curAura->IsPermanent() &&
                                 curAura->GetDuration() < (int32)dur))
                    if (Aura* newAura = Aura::TryRefreshStackOrCreate(
                            aura->GetSpellInfo(),
                            effMask,
                            stealer,
                            nullptr,
                            ...
                            nullptr,
                            aura->GetCasterGUID())) {
                        // created aura must not be single target aura,, so
                        // stealer won't loose it on recast
                        if (newAura->IsSingleTarget()) {
                            newAura->UnregisterSingleTarget();
                            // bring back single target aura status to the old
                            // aura
                            aura->SetIsSingleTarget(true);
                            caster->GetSingleCastAuras().push_back(aura);
                        }
                        // FIXME: using aura->GetMaxDuration() maybe not
                        // blizzlike but it fixes stealing of spells like
                        // Innervate
                        newAura->SetLoadedState(
                            aura->GetMaxDuration(),
                            int32(dur),
                            stealCharge ? 1 : aura->GetCharges(),
                            1,
                            recalculateMask,
                            &damage[0]);
...
void Unit::RemoveNotOwnSingleTargetAuras()
{
    ...
    // Iterate m_ownedAuras - aura is marked as single target in Unit::AddAura
    // (and pushed to m_ownedAuras). m_appliedAuras will NOT contain the aura
    // before first Unit::Update after adding it to m_ownedAuras. Quickly
    // removing such an aura will lead to it not being unregistered from
    // caster's single cast auras container leading to assertion failures if the
    // aura was cast on a player that can (and is changing map at the point
    // where this function is called). Such situation occurs when player is
    // logging in inside an instance and fails the entry check for any reason.
    // The aura that was loaded from db (indirectly, via linked casts) gets
    // removed before it has a chance to register in m_appliedAuras
    for (AuraMap::iterator iter = m_ownedAuras.begin();
         iter != m_ownedAuras.end();) {
                                         ...
                                         bool   isAutoshot /*= false*/)
{
    ...
    for (AuraApplicationList::iterator iter = m_interruptableAuras.begin();
        ...
        Aura* aura = (*iter)->GetBase();
        ++iter;
        if ((aura->GetSpellInfo()->AuraInterruptFlags & flag) &&
    ...
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL]) {
        if (spell->getState() == SPELL_STATE_CASTING &&
            ...
            if (!(isAutoshot && spell->m_spellInfo->HasAttribute(
                                    SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS))) {
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
        ...
        if (!casterGUID || aura->GetCasterGUID() == casterGUID) {
            ...
            if (spell->SpellFamilyName == uint32(family) &&
                spell->SpellFamilyFlags.HasFlag(
                    familyFlag1, familyFlag2, familyFlag3)) {
                RemoveAura(iter);
                continue;
            }
        }
...
void Unit::RemoveMovementImpairingAuras(bool withRoot)
{
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (((1 << i) & iter->second->GetEffectMask()) &&
                aura->GetSpellInfo()->Effects[i].Mechanic == MECHANIC_SNARE)
                                   ...
                                   uint32         except)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
        ...
        if (!except || aura->GetId() != except) {
            if (aura->GetSpellInfo()->GetAllEffectsMechanicMask() &
                mechanic_mask) {
                RemoveAura(iter, removemode);
                continue;
            }
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
        ...
        if ((aura->GetSpellInfo()->GetAllEffectsMechanicMask() &
            ...
            (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_CU_AURA_CC) ||
             (aura->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_WARRIOR &&
              (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x20)))) {
            RemoveAura(iter);
            continue;
        }
        ++iter;
...
{
    // make sure that all area auras not applied on self are removed - prevent
    // access to deleted pointer later
    for (AuraMap::iterator iter = m_ownedAuras.begin();
         iter != m_ownedAuras.end();) {
        Aura* aura = iter->second;
        ++iter;
        ...
        for (Aura::ApplicationMap::const_iterator itr = appMap.begin();
            ...
            // things linked on aura remove may apply new area aura - so start
            // from the beginning
...
{
    // this may be a dead loop if some events on aura remove will continiously
    // apply aura on remove we want to have all auras removed, so use your brain
    // when linking events
...
void Unit::RemoveArenaAuras()
{
    ...
    RemoveAppliedAuras([](AuraApplication const* aurApp) {
        ...
        return (!aura->GetSpellInfo()->HasAttribute(
                    SPELL_ATTR4_ALLOW_ENETRING_ARENA) // don't remove stances,
                                                      // shadowform,
                                                      // pally/hunter auras
                && !aura->IsPassive() // don't remove passive auras
                &&
                (aurApp->IsPositive() ||
                 !aura->GetSpellInfo()->HasAttribute(
                     SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD))) || // not negative
                                                             // death persistent
                                                             // auras
               aura->GetSpellInfo()->HasAttribute(
                   SPELL_ATTR5_REMOVE_ENTERING_ARENA); // special marker, always
                                                       // remove
    });
}
...
void Unit::RemoveAllAurasOnDeath()
{
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
         iter != m_appliedAuras.end();) {
        Aura const* aura = iter->second->GetBase();
        if ((!aura->IsPassive() || aura->GetSpellInfo()->HasAttribute(
...
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter !=
m_appliedAuras.end();)
    {
        Aura const* aura = iter->second->GetBase();
        if (aura->GetSpellInfo()->HasAura(type1) ||
aura->GetSpellInfo()->HasAura(type2))
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }

    for (AuraMap::iterator iter = m_ownedAuras.begin(); iter !=
m_ownedAuras.end();)
    {
        Aura* aura = iter->second;
        if (aura->GetSpellInfo()->HasAura(type1) ||
...
// Xinef: We should not remove passive auras on evade, if npc has player owner
// (scripted one cast auras)
void Unit::RemoveEvadeAuras()
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
        ...
        Aura const*      aura      = iter->second->GetBase();
        SpellInfo const* spellInfo = aura->GetSpellInfo();
        if (spellInfo->HasAttribute(SPELL_ATTR0_CU_IGNORE_EVADE) ||
            spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE) ||
            spellInfo->HasAura(SPELL_AURA_CLONE_CASTER) ||
            (aura->IsPassive() && GetOwnerGUID().IsPlayer()))
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
...
void Unit::DelayOwnedAuras(uint32 spellId, ObjectGuid caster, int32 delaytime)
{
    ...
    for (; range.first != range.second; ++range.first) {
        ...
        if (!caster || aura->GetCasterGUID() == caster) {
            ...
            LOG_DEBUG(
                "spells.aura",
                "Aura {} partially interrupted on unit {}, new duration: {} ms",
                aura->GetId(),
                GetGUID().ToString(),
                aura->GetDuration());
        }
...
void Unit::_RemoveAllAuraStatMods()
{
    for (AuraApplicationMap::iterator i = m_appliedAuras.begin();
         i != m_appliedAuras.end();
         ++i)
        (*i).second->GetBase()->HandleAllEffects(
            i->second, AURA_EFFECT_HANDLE_STAT, false);
}
...
void Unit::_ApplyAllAuraStatMods()
{
    for (AuraApplicationMap::iterator i = m_appliedAuras.begin();
         ...
         ++i)
        (*i).second->GetBase()->HandleAllEffects(
            i->second, AURA_EFFECT_HANDLE_STAT, true);
}

AuraEffect*
Unit::GetAuraEffect(uint32 spellId, uint8 effIndex, ObjectGuid caster) const
{
                    ...
                    uint8      reqEffMask) const
{
    AuraApplication* aurApp =
        GetAuraApplication(spellId, casterGUID, itemCasterGUID, reqEffMask);
    return aurApp ? aurApp->GetBase() : nullptr;
}

AuraApplication*
Unit::GetAuraApplicationOfRankedSpell(uint32           spellId,
                                      ObjectGuid       casterGUID,
                                      ObjectGuid       itemCasterGUID,
                                      uint8            reqEffMask,
                                      AuraApplication* except) const
{
    uint32 rankSpell = sSpellMgr->GetFirstSpellInChain(spellId);
    while (rankSpell) {
        if (AuraApplication* aurApp = GetAuraApplication(
                rankSpell, casterGUID, itemCasterGUID, reqEffMask, except))
            return aurApp;
        rankSpell = sSpellMgr->GetNextSpellInChain(rankSpell);
    }
    return nullptr;
...
Aura* Unit::GetAuraOfRankedSpell(uint32     spellId,
                                 ...
                                 uint8      reqEffMask) const
{
    AuraApplication* aurApp = GetAuraApplicationOfRankedSpell(
        spellId, casterGUID, itemCasterGUID, reqEffMask);
    return aurApp ? aurApp->GetBase() : nullptr;
}
...
{
    // we should not be able to dispel diseases if the target is affected by
    // unholy blight
            ...
            if (FactionTemplateEntry const* factionTemplateEntry =
                ...
                if (FactionEntry const* factionEntry =
                    ...
                    if (factionEntry->CanBeSetAtWar()) {
                        positive = !casterPlayer->GetReputationMgr().IsAtWar(
                            factionEntry);
                    }
                }
            }
    ...
    for (Unit::VisibleAuraMap::const_iterator itr = visibleAuras->begin();
        ...
        if (aura->GetSpellInfo()->GetDispelMask() & dispelMask) {
            ...
            if (aura->GetSpellInfo()->Mechanic == MECHANIC_BANISH &&
                !dispelSpell->HasAttribute(SPELL_ATTR0_NO_IMMUNITIES)) {
                continue;
            }

            // The charges / stack amounts don't count towards the total number
            // of auras that can be dispelled. Ie: A dispel on a target with 5
            // stacks of Winters Chill and a Polymorph has 1 / (1 + 1) -> 50%
            // chance to dispell Polymorph instead of 1 / (5 + 1) -> 16%.
            bool dispel_charges = aura->GetSpellInfo()->HasAttribute(
                SPELL_ATTR7_DISPEL_REMOVES_CHARGES);
            uint8 charges =
                dispel_charges ? aura->GetCharges() : aura->GetStackAmount();
            if (charges > 0)
...
bool Unit::HasVisibleAuraType(AuraType auraType) const
{
    ...
    for (AuraEffectList::const_iterator i = mAuraList.begin();
         i != mAuraList.end();
         ++i)
...
bool Unit::HasAuraTypeWithTriggerSpell(AuraType auratype,
                                       uint32   triggerSpell) const
{
    for (AuraEffect const* aura : GetAuraEffectsByType(auratype)) {
        if (aura->GetSpellInfo()->Effects[aura->GetEffIndex()].TriggerSpell ==
            triggerSpell) {
            return true;
        }
    }
...
bool Unit::HasNegativeAuraWithInterruptFlag(uint32 flag, ObjectGuid guid)
{
    ...
    for (AuraApplicationList::iterator iter = m_interruptableAuras.begin();
         ...
         ++iter) {
        if (!(*iter)->IsPositive() &&
            (*iter)->GetBase()->GetSpellInfo()->AuraInterruptFlags & flag &&
...
{
    for (AuraApplicationMap::const_iterator iter = m_appliedAuras.begin();
         iter != m_appliedAuras.end();
         ++iter) {
        Aura const* aura = iter->second->GetBase();
        if (!iter->second->IsPositive() &&
            aura->GetSpellInfo()->Attributes & flag &&
...
bool Unit::HasAuraWithMechanic(uint32 mechanicMask) const
{
    for (AuraApplicationMap::const_iterator iter = m_appliedAuras.begin();
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (iter->second->HasEffect(i) && spellInfo->Effects[i].Effect &&
                spellInfo->Effects[i].Mechanic)
...
uint32 Unit::GetDiseasesByCaster(ObjectGuid casterGUID, uint8 mode)
{
    ...
    for (uint8 index = 0; diseaseAuraTypes[index] != SPELL_AURA_NONE; ++index) {
        for (AuraEffectList::iterator i =
                 m_modAuras[diseaseAuraTypes[index]].begin();
             i != m_modAuras[diseaseAuraTypes[index]].end();) {
            // Get auras with disease dispel type by caster
            if ((*i)->GetSpellInfo()->Dispel == DISPEL_DISEASE &&
                ((*i)->GetCasterGUID() == casterGUID ||
                 (*i)->GetCasterGUID() ==
                     drwGUID)) // if its caster or his dancing rune weapon
            {
                ++diseases;
                ...
                // used for glyph of scourge strike
                    ...
                    if (aura && !aura->IsRemoved() && aura->GetDuration() > 0)
                        if ((aura->GetApplyTime() +
                             aura->GetMaxDuration() / 1000 + 8) >
...
int32 Unit::GetTotalAuraModifierByAffectMask(
    AuraType auratype, SpellInfo const* affectedSpell) const
{
    int32 modifier = 0;
...
float Unit::GetTotalAuraMultiplierByAffectMask(
    AuraType auratype, SpellInfo const* affectedSpell) const
{
    float multiplier = 1.0f;
...
int32 Unit::GetMaxPositiveAuraModifierByAffectMask(
    AuraType auratype, SpellInfo const* affectedSpell) const
{
    int32 modifier = 0;
...
{
    for (GameObjectList::const_iterator itr = m_gameObj.begin();
         itr != m_gameObj.end();
         ++itr)
        if (GameObject* go = ObjectAccessor::GetGameObject(*this, *itr))
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && gameObj->GetSpellId()) {
        SpellInfo const* createBySpell =
            sSpellMgr->GetSpellInfo(gameObj->GetSpellId());
        // Need disable spell use for owner
        if (createBySpell && createBySpell->IsCooldownStartedOnEvent())
            // note: item based cooldowns and cooldown spell mods with charges
            // ignored (unknown existing cases)
            ToPlayer()->AddSpellAndCategoryCooldowns(
                createBySpell, 0, nullptr, true);
    }
}
...
void Unit::RemoveAllGameObjects()
{
    while (!m_gameObj.empty()) {
        GameObject* go =
            ObjectAccessor::GetGameObject(*this, *m_gameObj.begin());
        if (go) {
            go->SetOwnerGUID(ObjectGuid::Empty);
            go->SetRespawnTime(0);
            go->Delete();
...
void Unit::SendSpellNonMeleeReflectLog(SpellNonMeleeDamage* log, Unit* attacker)
{
    ...
    // IF we are in cheat mode we swap absorb with damage and set damage to 0,
    // this way we can still debug damage but our hp bar will not drop
    uint32 damage = log->damage;
    uint32 absorb = log->absorb;
    if (log->target->GetTypeId() == TYPEID_PLAYER &&
        log->target->ToPlayer()->GetCommandStatus(CHEAT_GOD)) {
        absorb = damage;
        damage = 0;
    }
    ...
    data << uint8(
        log->physicalLog); // if 1, then client show spell name (example: %s's
                           // ranged shot hit %s for %u school or %s suffers %u
                           // school damage from %s's spell_name
...
void Unit::SendSpellNonMeleeDamageLog(SpellNonMeleeDamage* log)
{
    ...
    data << uint32(log->HitInfo);
    data << uint32(log->HitInfo);
    data << uint8(log->HitInfo &
                  (SPELL_HIT_TYPE_CRIT_DEBUG | SPELL_HIT_TYPE_HIT_DEBUG |
                              ...
                              uint32           procPhase)
{
    ...
    if (procAttacker && actor)
        actor->ProcDamageAndSpellFor(false,
                                     victim,
                                     procAttacker,
                                     procExtra,
                                     attType,
    ...
    if (victim && victim->IsAlive() && procVictim)
        victim->ProcDamageAndSpellFor(true,
                                      actor,
                                      procVictim,
                                      procExtra,
                                      attType,
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    case SPELL_AURA_PERIODIC_DAMAGE_PERCENT: {
        // IF we are in cheat mode we swap absorb with damage and set damage to
        // 0, this way we can still debug damage but our hp bar will not drop
        uint32 damage = pInfo->damage;
        uint32 absorb = pInfo->absorb;
        if (GetTypeId() == TYPEID_PLAYER &&
            ToPlayer()->GetCommandStatus(CHEAT_GOD)) {
            absorb = damage;
            damage = 0;
        }
    ...
    default:
        LOG_ERROR("entities.unit",
                  "Unit::SendPeriodicAuraLog: unknown aura {}",
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i) {
        ...
        if (damageInfo->target->GetTypeId() == TYPEID_PLAYER &&
            damageInfo->target->ToPlayer()->GetCommandStatus(CHEAT_GOD)) {
    ...
    size_t const maxsize = 4 + 5 + 5 + 4 + 4 + 1 + count * (4 + 4 + 4 + 4 + 4) +
                           1 + 4 + 4 + 4 + 4 + 4 * 12;
    ...
    int32 overkill =
        tmpDamage[0] + tmpDamage[1] - damageInfo->target->GetHealth();
    data << uint32(overkill < 0 ? 0 : overkill); // Overkill
    data << uint8(count);                        // Sub damage count

    for (uint32 i = 0; i < count; ++i) {
        data << uint32(
            damageInfo->damages[i].damageSchoolMask); // School of sub damage
    ...
    //! Probably used for debugging purposes, as it is not known to appear on
    //! retail servers
                               ...
                               ProcEventInfo const& eventInfo)
{
    ...
    Item* castItem = triggeredByAura->GetBase()->GetCastItemGUID() &&
                             GetTypeId() == TYPEID_PLAYER
                         ? ToPlayer()->GetItemByGuid(
                               triggeredByAura->GetBase()->GetCastItemGUID())
                         : nullptr;

    uint32 triggered_spell_id = 0;
    uint32 cooldown_spell_id =
        0; // for random trigger, will be one of the triggered spell to avoid
           // repeatable triggers
    ...
    case SPELLFAMILY_GENERIC: {
        ...
        // Druid) 40998 Kleptomania: +35 agility (Warrior, Rogue, Paladin,
        // Hunter, Druid) 40999 Megalomania: +41 damage/healing (Druid, Shaman,
        // Priest, Warlock, Mage, Paladin) 41002 Paranoia: +35
        // spell/melee/ranged crit strike rating (All classes) 41005 Manic: +35
        // haste (spell, melee and ranged) (All classes) 41009 Narcissism: +35
        // intellect (Druid, Shaman, Priest, Warlock, Mage, Paladin, Hunter)
        // 41011 Martyr Complex: +35 stamina (All classes)
        // 41406 Dementia: Every 5 seconds either gives you +5% damage/healing.
        // (Druid, Shaman, Priest, Warlock, Mage, Paladin) 41409 Dementia: Every
        // 5 seconds either gives you -5% damage/healing. (Druid, Shaman,
        // Priest, Warlock, Mage, Paladin)
        case 39446: {
            ...
            switch (getClass()) {
            case CLASS_PALADIN: // 39511, 40997, 40998, 40999, 41002, 41005,
                                // 41009, 41011, 41409
            case CLASS_DRUID:   // 39511, 40997, 40998, 40999, 41002, 41005,
                                // 41009, 41011, 41409
                triggered_spell_id = RAND(39511,
                                          40997,
                                          40998,
                                          40999,
                                          41002,
            ...
            case CLASS_DEATH_KNIGHT:
                ...
                cooldown_spell_id = 39511;
                break;
            case CLASS_PRIEST: // 40999, 41002, 41005, 41009, 41011, 41406,
                               // 41409
            case CLASS_SHAMAN: // 40999, 41002, 41005, 41009, 41011, 41406,
                               // 41409
            case CLASS_MAGE: // 40999, 41002, 41005, 41009, 41011, 41406, 41409
            case CLASS_WARLOCK: // 40999, 41002, 41005, 41009, 41011, 41406,
                                // 41409
                triggered_spell_id =
                    RAND(40999, 41002, 41005, 41009, 41011, 41406, 41409);
                cooldown_spell_id = 40999;
                break;
            case CLASS_HUNTER: // 40997, 40999, 41002, 41005, 41009, 41011,
                               // 41406, 41409
                triggered_spell_id = RAND(
            ...
            target = this;
            if (roll_chance_i(10))
                ToPlayer()->Say("This is Madness!",
                                LANG_UNIVERSAL); /// @todo: It should be moved
                                                 /// to database, shouldn't it?
            break;
        }
        ...
        // neck) cast 45478 Light's Salvation if Exalted by Aldor cast 45430
        // Arcane Surge if Exalted by Scryers
        case 45484: {
            if (GetTypeId() != TYPEID_PLAYER)
                return false;
        ...
        case 31872: {
            ...
            victim->RemoveAurasWithMechanic(1 << MECHANIC_STUN,
                                            AURA_REMOVE_BY_ENEMY_SPELL);
            return true;
        }
        // Glyph of Life Tap
        ...
        case 69172: // everything except for DoTs
        {
            ...
            if (Unit* caster = triggeredByAura->GetCaster()) {
                if (procFlag & (PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS |
                    ...
                    if (caster->GetVictim()) {
                        ...
                        target->CastCustomSpell(
                            caster->GetVictim(), 69189, &dmg, 0, 0, true);
                    }
                }
            }
            return true;
        ...
        case 69755: {
            triggered_spell_id =
                ((procFlag & PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS) ? 69733
                                                                       : 69729);
            break;
        }
        ...
        case 69739: {
            triggered_spell_id =
                ((procFlag & PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS) ? 69734
                                                                       : 69730);
            break;
        }
    ...
    case SPELLFAMILY_MAGE: {
        ...
        if (dummySpell->SpellIconID ==
            459) // only this spell has SpellIconID == 459 and dummy aura
        {
            ...
            // mana reward
            basepoints0 =
                CalculatePct(int32(GetMaxPower(POWER_MANA)), triggerAmount);
            target             = this;
        ...
        if (dummySpell->SpellIconID == 2999) {
            if (effIndex != 0)
                return false;
            AuraEffect* counter =
                triggeredByAura->GetBase()->GetEffect(EFFECT_1);
            if (!counter)
                return true;
        ...
        case 56374: {
            RemoveAurasByType(
                SPELL_AURA_HASTE_SPELLS, ObjectGuid::Empty, 0, true, false);
    ...
    case SPELLFAMILY_WARRIOR: {
        ...
        if (dummySpell->SpellIconID == 1697) {
            // only for spells and hit/crit (trigger start always) and not start
            // from self casted spells (5530 Mace Stun Effect for example)
            if (procSpell == 0 ||
                !(procEx & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT)) ||
                this == victim)
                return false;
            // Need stun or root mechanic
            if (!(procSpell->GetAllEffectsMechanicMask() &
            ...
            default:
                LOG_ERROR(
                    "entities.unit",
                    "Unit::HandleDummyAuraProc: non handled spell id: {} (SW)",
                    dummySpell->Id);
                return false;
            }
    ...
    case SPELLFAMILY_WARLOCK: {
        ...
        if (dummySpell->SpellFamilyFlags[1] & 0x00000010) {
            if (procSpell && procSpell->SpellFamilyFlags[1] & 0x8000)
                return false;
            // if damage is more than need or target die from damage deal finish
            // spell
            if (triggeredByAura->GetAmount() <= int32(damage) ||
                ...
                uint32 spell =
                    sSpellMgr->GetSpellWithRank(27285, dummySpell->GetRank());
        ...
        case 30296: {
            // Improved Soul Leech
            AuraEffectList const& SoulLeechAuras =
                GetAuraEffectsByType(SPELL_AURA_DUMMY);
            for (Unit::AuraEffectList::const_iterator i =
                     SoulLeechAuras.begin();
                 i != SoulLeechAuras.end();
                 ++i) {
                if ((*i)->GetId() == 54117 || (*i)->GetId() == 54118) {
                    ...
                    CastCustomSpell(this,
                                    ...
                                    castItem,
                                    triggeredByAura);
                    // Get second aura of spell for replenishment effect on
                    // party
                    if (AuraEffect const* aurEff =
                            (*i)->GetBase()->GetEffect(EFFECT_1)) {
                        // Replenishment - roll chance
                        if (roll_chance_i(aurEff->GetAmount())) {
                            CastSpell(
                                this, 57669, true, castItem, triggeredByAura);
                        }
                    }
                    break;
                }
    ...
    case SPELLFAMILY_PRIEST: {
        ...
        if (dummySpell->SpellIconID == 2218) {
            ...
            if (procSpell->Id != 552 || victim != this ||
        ...
        case 70770:
            ...
            if (procSpell->SpellFamilyFlags[0] & 0x800) {
                ...
                SpellInfo const* blessHealing =
                    sSpellMgr->GetSpellInfo(triggered_spell_id);
                if (!blessHealing || !victim)
                    return false;
                basepoints0 = int32(CalculatePct(damage, triggerAmount) /
                                    (blessHealing->GetMaxDuration() /
                                     blessHealing->Effects[0].Amplitude));
    ...
    case SPELLFAMILY_DRUID: {
        ...
        case 54832: {
            ...
            int32 mana_perc = triggeredByAura->GetSpellInfo()
                                  ->Effects[triggeredByAura->GetEffIndex()]
                                  .CalcValue();
            basepoints0 = int32(
                CalculatePct(GetCreatePowers(POWER_MANA), mana_perc) / 10);
        ...
        case 54815: {
            ...
            if (AuraEffect const* AurEff =
                    target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                          ...
                                          0x0,
                                          GetGUID())) {
                // Rip's max duration, note: spells which modifies Rip's
                // duration also counted like Glyph of Rip
                ...
                CountMax +=
                    3 * triggerAmount * IN_MILLISECONDS; // Glyph of Shred -> +6
                                                         // seconds
                CountMax += HasAura(54818) ? 4 * IN_MILLISECONDS
                                           : 0; // Glyph of Rip -> +4 seconds
                CountMax +=
                    HasAura(60141)
                        ? 4 * IN_MILLISECONDS
                        : 0; // Rip Duration/Lacerate Damage -> +4 seconds
                ...
                if (CountMin < CountMax) {
                    AurEff->GetBase()->SetDuration(
                        ...
                        triggerAmount * IN_MILLISECONDS);
                    AurEff->GetBase()->SetMaxDuration(
                        CountMin + triggerAmount * IN_MILLISECONDS);
                    return true;
                }
            }
            // if not found Rip
        ...
        case 54821: {
            if (procSpell->SpellVisual[0] == 750 &&
                procSpell->Effects[1].ApplyAuraName == 3) {
        ...
        case 24932: {
            ...
            if (triggeredByAura->GetCasterGUID() != GetGUID())
                break;
            int32 basepoints1 = CalculatePct(GetMaxPower(Powers(POWER_MANA)),
                                             triggerAmount * 2);
            ...
            if (GetTypeId() == TYPEID_PLAYER &&
                ...
                CastCustomSpell(
                    this, 68285, &basepoints1, 0, 0, true, 0, triggeredByAura);
            break;
        }
        ...
        case 70664: {
            ...
            CastCustomSpell(
                70691, SPELLVALUE_BASE_POINT0, damage, victim, true);
            return true;
        }
        }
        // Eclipse
        if (dummySpell->SpellIconID == 2856 && GetTypeId() == TYPEID_PLAYER) {
            ...
            if (!roll_chance_f(dummySpell->ProcChance *
                               (isWrathSpell ? 0.6f : 1.0f)))
                return false;
        ...
        [[fallthrough]]; /// @todo: Not sure whether the fallthrough was a
                         /// mistake (forgetting a break) or intended. This
    ...
    case SPELLFAMILY_ROGUE: {
        ...
        case 32748: {
            // Prevent cast Deadly Throw Interrupt on self from last effect
            // (apply dummy) of Deadly Throw
            if (this == victim)
                return false;
        ...
        if (dummySpell->SpellIconID == 2909) {
            // "refresh your Slice and Dice duration to its 5 combo point
            // maximum" lookup Slice and Dice
            if (AuraEffect const* aur =
                    GetAuraEffect(SPELL_AURA_MOD_MELEE_HASTE,
                                  SPELLFAMILY_ROGUE,
                                  0x40000,
                                  0,
                                  0)) {
                aur->GetBase()->SetDuration(
                    aur->GetSpellInfo()->GetMaxDuration(), true);
                return true;
            }
            return false;
        }
        ...
        // Quick Recovery
            ...
            // energy cost save
            basepoints0 =
                CalculatePct(int32(procSpell->ManaCost), triggerAmount);
            if (basepoints0 <= 0)
                return false;
    ...
    case SPELLFAMILY_HUNTER: {
        ...
        case 2236: // Thrill of the Hunt
        {
            ...
            if (procSpell->SpellFamilyFlags[2] & 0x200) {
                if (!victim)
                    return false;
                if (AuraEffect const* pEff =
                        victim->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY,
                                              SPELLFAMILY_HUNTER,
                                              0x0,
                                              0x80000000,
                                              0x0,
                                              GetGUID()))
                    basepoints0 =
                        pEff->GetSpellInfo()->CalcPowerCost(
                            this,
                            SpellSchoolMask(pEff->GetSpellInfo()->SchoolMask)) *
                        4 / 10 / 3;
            }
            else
                basepoints0 =
                    procSpell->CalcPowerCost(
                        this, SpellSchoolMask(procSpell->SchoolMask)) *
                    4 / 10;
    ...
    case SPELLFAMILY_PALADIN: {
        ...
        if (dummySpell->Id == 53651) {
            ...
            if (!beaconTarget || beaconTarget == this ||
                !beaconTarget->GetAura(53563, victim->GetGUID()))
                return false;

            basepoints0 = int32(damage);
            triggered_spell_id =
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(635)) ? 53652
                                                                  : 53654;

            victim->CastCustomSpell(beaconTarget,
                                    triggered_spell_id,
                                    &basepoints0,
                                    nullptr,
                                    nullptr,
        ...
        case 20185: {
            if (!victim || !victim->IsAlive() ||
                victim->HasSpellCooldown(20267))
                return false;
            // 2% of base mana
            basepoints0 = int32(victim->CountPctFromMaxHealth(2));
            victim->CastCustomSpell(
                victim, 20267, &basepoints0, 0, 0, true, 0, triggeredByAura);
        ...
        case 28789: {
            ...
            case CLASS_DRUID:
                triggered_spell_id =
                    28795; // Increases the friendly target's mana regeneration
                           // by $s1 per 5 sec. for $d.
                break;
            case CLASS_MAGE:
            case CLASS_WARLOCK:
                triggered_spell_id =
                    28793; // Increases the friendly target's spell damage and
                           // healing by up to $s1 for $d.
                break;
            case CLASS_HUNTER:
            case CLASS_ROGUE:
        ...
        case 31801: {
            if (effIndex != 0 ||
                !victim) // effect 1, 2 used by seal unleashing code
                return false;

            // At melee attack or Hammer of the Righteous spell damage
            // considered as melee attack
            ...
            bool damager = procSpell && (procSpell->EquippedItemClass != -1 ||
                                         (procSpell->SpellIconID == 243 &&
                                          procSpell->SpellVisual[0] == 39));
        ...
        case 71545: // Tiny Abomination in a Jar (Heroic)
        {
            ...
            if (!dummy ||
                dummy->GetStackAmount() < (dummySpell->Id == 71406 ? 8 : 7))
                return false;
    ...
    case SPELLFAMILY_SHAMAN: {
        ...
        case 28823: {
            ...
            case CLASS_DRUID:
                triggered_spell_id =
                    28824; // Increases the friendly target's mana regeneration
                           // by $s1 per 5 sec. for $d.
                break;
            case CLASS_MAGE:
            case CLASS_WARLOCK:
                triggered_spell_id =
                    28825; // Increases the friendly target's spell damage and
                           // healing by up to $s1 for $d.
                break;
            case CLASS_HUNTER:
            case CLASS_ROGUE:
        ...
        case 33757: {
            Player* player = ToPlayer();
            if (!player || !castItem || !castItem->IsEquipped() || !victim ||
                !victim->IsAlive())
                return false;
            ...
            WeaponAttackType attType =
                WeaponAttackType(player->GetAttackBySlot(castItem->GetSlot()));
            if ((attType != BASE_ATTACK && attType != OFF_ATTACK) ||
                (attType == BASE_ATTACK &&
                 procFlag & PROC_FLAG_DONE_OFFHAND_ATTACK) ||
                (attType == OFF_ATTACK &&
                 procFlag & PROC_FLAG_DONE_MAINHAND_ATTACK))
                return false;
            ...
            default: {
                LOG_ERROR("entities.unit",
                          "Unit::HandleDummyAuraProc: non handled item "
                          "enchantment (rank?) {} for spell id: {} (Windfury)",
            ...
            SpellInfo const* windfurySpellInfo =
                sSpellMgr->GetSpellInfo(spellId);
            if (!windfurySpellInfo) {
                LOG_ERROR("entities.unit",
                          "Unit::HandleDummyAuraProc: non-existing spell id: "
                          "{} (Windfury)",
                          spellId);
                return false;
            }
            ...
            int32 extra_attack_power =
                CalculateSpellDamage(victim, windfurySpellInfo, 1);

            // Value gained from additional AP
            basepoints0 = int32(extra_attack_power / 14.0f *
                                GetAttackTime(attType) / 1000);
        ...
        case 67228: {
            ...
            if (procSpell->SpellFamilyFlags[1] & 0x1000) {
                ...
                if (!triggeredSpell)
                    return false;
                basepoints0 = CalculatePct(int32(damage), triggerAmount) /
                              (triggeredSpell->GetMaxDuration() /
                               triggeredSpell->Effects[0].Amplitude);
            }
            break;
        }
        ...
        case 70817: {
            ...
            if (AuraEffect const* aurEff =
                ...
                flameShock->SetMaxDuration(flameShock->GetMaxDuration() +
                                           extraTime);
        ...
        if (dummySpell->SpellFamilyFlags[0] & 0x200000) {
            ...
            float fire_onhit = float(
                CalculatePct(dummySpell->Effects[EFFECT_0].CalcValue(), 1.0f));

            float add_spellpower =
                (float)(SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE) +
                        victim->SpellBaseDamageBonusTaken(
                            SPELL_SCHOOL_MASK_FIRE));

            // 1.3speed = 5%, 2.6speed = 10%, 4.0 speed = 15%, so, 1.0speed
            // = 3.84%
            ...
            if (castItem->GetSlot() == EQUIPMENT_SLOT_OFFHAND &&
                ...
                // Value1: add the tooltip damage by swingspeed + Value2: add
                // spelldmg by swingspeed
        ...
        if (dummySpell->SpellIconID == 2287) {
            ...
            // Water Shield
            if (AuraEffect const* aurEff =
                    GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL,
                                  SPELLFAMILY_SHAMAN,
                                  0,
                                  0x00000020,
                                  0)) {
                uint32 spell = aurEff->GetSpellInfo()
                                   ->Effects[aurEff->GetEffIndex()]
                                   .TriggerSpell;
        ...
        // Lightning Overload
        if (dummySpell->SpellIconID ==
            2018) // only this spell have SpellFamily Shaman SpellIconID == 2018
                  // and dummy aura
        {
            ...
            do {
                ...
                // Cast
                CastCustomSpell(
                    victim, spell, &dmg, 0, 0, true, castItem, triggeredByAura);
        ...
        if (dummySpell->SpellIconID == 3059) {
            ...
            if (AuraEffect const* aurEff =
                    GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL,
                                  ...
                                  0,
                                  0)) {
                uint32 spell = sSpellMgr->GetSpellWithRank(
                    26364, aurEff->GetSpellInfo()->GetRank());
    ...
    case SPELLFAMILY_DEATHKNIGHT: {
        ...
        if (dummySpell->Id == 49194) {
            ...
            basepoints0 = basepoints0 / (unholyBlight->GetMaxDuration() /
                                         unholyBlight->Effects[0].Amplitude);
            victim->CastDelayedSpellWithPeriodicAmount(
                this,
                triggered_spell_id,
                SPELL_AURA_PERIODIC_DAMAGE,
        ...
        if (dummySpell->SpellIconID == 2023) {
            ...
            // This should do, restore spell mod so next attack can also use
            // this! crit chance for first strike is already computed
            ToPlayer()->RestoreSpellMods(m_currentSpells[CURRENT_GENERIC_SPELL],
                                         51124,
                                         nullptr); // Killing Machine
            ToPlayer()->RestoreSpellMods(m_currentSpells[CURRENT_GENERIC_SPELL],
                                         49796,
                                         nullptr); // Deathchill

            // Xinef: Somehow basepoints are divided by 2 which is later divided
            // by 2 (offhand multiplier)
            SpellInfo const* triggerEntry =
                sSpellMgr->GetSpellInfo(triggered_spell_id);
        ...
        if (dummySpell->SpellIconID == 1939 && GetTypeId() == TYPEID_PLAYER) {
            ...
            for (PlayerSpellMap::const_iterator itr = sp_list.begin();
                ...
                if (!itr->second->Active ||
                    !itr->second->IsInSpec(ToPlayer()->GetActiveSpec()) ||
                    itr->second->State == PLAYERSPELL_REMOVED)
                    continue;

                SpellInfo const* spellProto =
                    sSpellMgr->GetSpellInfo(itr->first);
                if (!spellProto)
                    continue;
        ...
        if (dummySpell->Id == 17619) {
            if (procSpell->SpellFamilyName == SPELLFAMILY_POTION) {
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++) {
                    ...
                    else if (procSpell->Effects[i].Effect ==
                    ...
                    basepoints0 =
                        int32(CalculateSpellDamage(this, procSpell, i) * 0.4f);
                    CastCustomSpell(this,
                                    triggered_spell_id,
                                    &basepoints0,
                                    nullptr,
    ...
    case SPELLFAMILY_PET: {
        ...
        case 201: {
            ...
            target             = this;
            float addThreat    = float(CalculatePct(
                procSpell->Effects[0].CalcValue(this), triggerAmount));
    ...
    if (!triggered_spell_id)
        triggered_spell_id =
            dummySpell->Effects[triggeredByAura->GetEffIndex()].TriggerSpell;
    ...
    if (!triggerEntry) {
        LOG_ERROR("entities.unit",
                  "Unit::HandleDummyAuraProc: Spell {} has non-existing "
                  "triggered spell {}",
                  dummySpell->Id,
                  triggered_spell_id);
        return false;
                          ...
                          bool*  handled)
{
    ...
    case SPELLFAMILY_GENERIC:
            ...
            if (Unit* caster = triggeredByAura->GetCaster())
                if (victim && caster->GetGUID() == victim->GetGUID()) {
                    *handled          = true;
                    ...
                    int32 const mod =
                        (GetMap()->GetSpawnMode() & 1) ? 1500 : 1250;
                    int32 dmg = 0;
        ...
        case 59915: {
            CastSpell(
                this, 59914, true); // 59914 already has correct basepoints in
                                    // DBC, no need for custom bp
            *handled = true;
            break;
        }
        // Swift Hand of Justice
        case 59906: {
            int32 bp0 = CalculatePct(GetMaxHealth(),
                                     dummySpell->Effects[EFFECT_0].CalcValue());
    ...
    case SPELLFAMILY_MAGE: {
        ...
        case 31658: {
            *handled = true;
            ...
            int32 bp0 = int32(CalculatePct(GetMaxPower(POWER_MANA),
                                           spInfo->Effects[0].CalcValue()));
            CastCustomSpell(this,
                            67545,
                            &bp0,
                            nullptr,
                            ...
                            true,
                            nullptr,
                            triggeredByAura->GetEffect(EFFECT_0),
                            GetGUID());
            return true;
        }
        }
    ...
    case SPELLFAMILY_DEATHKNIGHT: {
        ...
        if (dummySpell->SpellIconID == 3041 ||
            (dummySpell->SpellIconID == 22 && dummySpell->Id != 62459) ||
            ...
            if (Player* player = ToPlayer()) {
                ...
                for (uint8 i = 0; i < MAX_RUNES && runesLeft; ++i) {
                    ...
                    if (player->GetRuneCooldown(i) !=
                        player->GetRuneBaseCooldown(i, false))
                        continue;
    ...
    case SPELLFAMILY_WARRIOR: {
        ...
        case 70844: {
            int32 basepoints0 = CalculatePct(
                GetMaxHealth(), dummySpell->Effects[EFFECT_1].CalcValue());
                                  ...
                                  ProcEventInfo&   eventInfo)
{
    ...
    uint32 trigger_spell_id =
        auraSpellInfo->Effects[triggeredByAura->GetEffIndex()].TriggerSpell;
    ...
    // if (sSpellMgr->GetSpellInfo(trigger_spell_id) == nullptr)
    {
        ...
        case SPELLFAMILY_GENERIC:
            ...
            case 43820: // Charm of the Witch Doctor (Amani Charm of the Witch
                ...
                if (!victim)
                    return false;
                basepoints0 = victim->GetCreateHealth() *
                              auraSpellInfo->Effects[1].CalcValue() / 100;
                target = victim;
                break;
            ...
            case 67702: // Death's Choice, Item - Coliseum 25 Normal Melee
                        // Trinket
            {
                if (!damage)
                    return false;
            ...
            case 67771: // Death's Choice (heroic), Item - Coliseum 25 Heroic
                        // Melee Trinket
            {
                if (!damage)
                    return false;
            ...
            case 27522:
            case 40336: {
                // On successful melee or ranged attack gain $29471s1 mana and
                // if possible drain $27526s1 mana from the target.
                if (IsAlive())
            ...
            case 50051: {
                ...
                if (Creature* cr = ObjectAccessor::GetCreature(
                        *this, m_SummonSlot[SUMMON_SLOT_MINIPET]))
        ...
        case SPELLFAMILY_MAGE:
            if (auraSpellInfo->SpellIconID == 2127) // Blazing Speed
            {
                ...
                default:
                    LOG_ERROR("entities.unit",
                              "Unit::HandleProcTriggerSpell: Spell {} miss "
                              "posibly Blazing Speed",
                              auraSpellInfo->Id);
                    return false;
                }
            }
            else if (auraSpellInfo->Id ==
                     71761) // Deep Freeze Immunity State (only permanent)
            {
                Creature* creature = victim->ToCreature();
                if (!creature || !creature->HasMechanicTemplateImmunity(
                                     1 << (MECHANIC_STUN - 1)))
                    return false;
            }
            break;
        ...
        case SPELLFAMILY_PRIEST: {
            ...
            if (auraSpellInfo->SpellIconID == 1875) {
                ...
                default:
                    LOG_ERROR("entities.unit",
                              "Unit::HandleProcTriggerSpell: Spell {} not "
                              "handled in BR",
                              auraSpellInfo->Id);
                    return false;
                }
                ...
                CastDelayedSpellWithPeriodicAmount(this,
                                                   trigger_spell_id,
                                                   SPELL_AURA_PERIODIC_HEAL,
                                                   basepoints0);
                return true;
        ...
        case SPELLFAMILY_HUNTER: {
            if (auraSpellInfo->SpellIconID == 3247) // Piercing Shots
            {
                ...
                default:
                    LOG_ERROR("entities.unit",
                              "Unit::HandleProcTriggerSpell: Spell {} miss "
                              "posibly Piercing Shots",
                              auraSpellInfo->Id);
                    return false;
                }
                SpellInfo const* TriggerPS =
                    sSpellMgr->GetSpellInfo(trigger_spell_id);
                if (!TriggerPS)
                    return false;

                basepoints0 = CalculatePct(int32(damage), triggerAmount) /
                              (TriggerPS->GetMaxDuration() /
                               TriggerPS->Effects[0].Amplitude);
                victim->CastDelayedSpellWithPeriodicAmount(
                    this,
                    trigger_spell_id,
                    SPELL_AURA_PERIODIC_DAMAGE,
        ...
        case SPELLFAMILY_PALADIN: {
            ...
            case 67758: // Item - Coliseum 25 Heroic Caster Trinket
            {
                ...
                CastSpell(
                    victim, trigger_spell_id, true, nullptr, triggeredByAura);
                return true;
            }
            default:
                // Illumination
                if (auraSpellInfo->SpellIconID == 241) {
                    if (!procSpell)
                        return false;
                    // procspell is triggered spell but we need mana cost of
                    // original casted spell
                    ...
                    if (procSpell->SpellFamilyFlags[1] & 0x00010000) {
                        ...
                        default:
                            LOG_ERROR("entities.unit",
                                      "Unit::HandleProcTriggerSpell: Spell {} "
                                      "not handled in HShock",
                                      procSpell->Id);
                            return false;
                        }
                    }
                    SpellInfo const* originalSpell =
                        sSpellMgr->GetSpellInfo(originalSpellId);
                    if (!originalSpell) {
                        LOG_ERROR("entities.unit",
                                  "Unit::HandleProcTriggerSpell: Spell {} "
                                  "unknown but selected as original in Illu",
                                  originalSpellId);
                        return false;
                    }
                    // percent stored in effect 1 (class scripts) base points
                    int32 cost =
                        int32(originalSpell->ManaCost +
                              CalculatePct(GetCreateMana(),
                                           originalSpell->ManaCostPercentage));
                    basepoints0 = CalculatePct(
                        cost, auraSpellInfo->Effects[1].CalcValue());
        ...
        case SPELLFAMILY_SHAMAN: {
            ...
            if (auraSpellInfo->SpellFamilyFlags[0] & 0x400 &&
                auraSpellInfo->HasAttribute(SPELL_ATTR1_NO_THREAT)) {
                // Do not proc off from self-casted items
                if (Spell const* spell = eventInfo.GetProcSpell()) {
                    if (spell->m_castItemGUID &&
                        victim->GetGUID() == GetGUID()) {
                        return false;
                    }
                }

                trigger_spell_id = sSpellMgr->GetSpellWithRank(
                    26364, auraSpellInfo->GetRank());
            }
            // Nature's Guardian
            else if (auraSpellInfo->SpellIconID == 2013) {
                // Check health condition - should drop to less 30% (damage deal
                // after this!)
    ...
    if (!triggerEntry) {
        ...
        LOG_ERROR("entities.unit",
                  "Unit::HandleProcTriggerSpell: Spell {} (effIndex: {}) has "
                  "unknown TriggerSpell {}. Unhandled custom case?",
                  auraSpellInfo->Id,
    ...
    if (triggerEntry->HasEffect(SPELL_EFFECT_ADD_EXTRA_ATTACKS)) {
        uint32 lastExtraAttackSpell =
            eventInfo.GetActor()->GetLastExtraAttackSpell();

        // Patch 1.12.0(?) extra attack abilities can no longer chain proc
        // themselves
        if (lastExtraAttackSpell == trigger_spell_id) {
            return false;
        }

        // Patch 2.2.0 Sword Specialization (Warrior, Rogue) extra attack can no
        // longer proc additional extra attacks 3.3.5 Sword Specialization
        // (Warrior), Hack and Slash (Rogue)
        if (lastExtraAttackSpell == SPELL_SWORD_SPECIALIZATION ||
            lastExtraAttackSpell == SPELL_HACK_AND_SLASH) {
            return false;
        }
    }

    // Custom requirements (not listed in procEx) Warning! damage dealing after
    // this Custom triggered spells
    switch (auraSpellInfo->Id) {
    // Deep Wounds
    case 12834:
    case 12849:
    case 12867: {
        ...
        if (procFlags & PROC_FLAG_DONE_OFFHAND_ATTACK)
            basepoints0 = int32((GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE) +
                                 GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE)) /
                                2.0f);
        else
            basepoints0 = int32((GetFloatValue(UNIT_FIELD_MAXDAMAGE) +
    ...
    case 38164: {
        if (!victim ||
            victim->GetEntry() != 19457) // Proc only if your target is Grillok
            return false;
        break;
    }
    // Deflection
    ...
    case 40971: {
        ...
        if (!victim || !victim->IsAlive() ||
            victim->HealthAbovePct(triggerAmount))
            return false;
        break;
    }
    // Rapid Recuperation
    ...
    case 63158:
        ...
        if (!victim || !victim->HasAuraState(
                           AURA_STATE_HEALTHLESS_35_PERCENT, procSpell, this))
            return false;
        break;
    // Ulduar, Hodir, Toasty Fire
    case 62821:
        if (this->GetTypeId() !=
            TYPEID_PLAYER) // spell has Attribute, but persistent area auras
                           // ignore it
            return false;
        break;
    ...
    case 71770:
        if (victim)
            victim->CastSpell(
                victim, trigger_spell_id, true); // EffectImplicitTarget is self
        return true;
    ...
    if (auraSpellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT) {
        ...
        if (auraSpellInfo->Id == 70656) {
            if (GetTypeId() != TYPEID_PLAYER ||
                !IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_ABILITY))
                return false;
        ...
        // Blade Barrier
            ...
            Player* plr = ToPlayer();
            if (!plr ||
                !plr->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_ABILITY) ||
                !procSpell)
                return false;
    ...
    case 15250: // Rogue Setup
    {
        ...
        if (!victim || (GetTypeId() == TYPEID_PLAYER &&
                        victim != ToPlayer()->GetSelectedUnit()))
            return false;
    ...
    {
        victim = nullptr;
        // Need add combopoint AFTER finish movie (or they dropped in finish
        // phase)
        break;
    }
    ...
    case 30824: {
        basepoints0 = int32(
            CalculatePct(GetTotalAttackPowerValue(BASE_ATTACK), triggerAmount));
        break;
    }
    ...
    case 48090: {
        ...
        if (IsPet())
            if (Unit* owner = GetOwner()) {
                ...
                if (AuraEffect* aurEff = owner->GetDummyAuraEffect(
                        SPELLFAMILY_WARLOCK, 3220, 0)) {
                    int32 spellPower = owner->SpellBaseDamageBonusDone(
                        SpellSchoolMask(SPELL_SCHOOL_MASK_MAGIC));
                    if (AuraEffect const* demonicAuraEffect =
                            GetAuraEffect(trigger_spell_id, EFFECT_0))
                        spellPower -= demonicAuraEffect->GetAmount();

                    basepoints0 = int32(
                        (aurEff->GetAmount() * spellPower + 100.0f) / 100.0f);
                    CastCustomSpell(this,
                                    trigger_spell_id,
                                    &basepoints0,
                                    &basepoints0,
    ...
    case 52437: // Sudden Death
    {
        ...
        if (AuraEffect const* aurEff = GetAuraEffect(70847, 0)) {
            ...
            CastSpell(
                this, 70849, true, castItem, triggeredByAura); // Extra Charge!
            if (trigger_spell_id == 46916)
                CastSpell(this,
                          71072,
                          true,
                          castItem,
                          triggeredByAura); // Slam GCD Reduced
            else
                CastSpell(this,
                          71069,
                          true,
                          castItem,
                          triggeredByAura); // Execute GCD Reduced
        }
        break;
    }
    // Sword and Board
    ...
    case 53817: {
        ...
        if (AuraEffect const* aurEff = GetAuraEffect(70832, 0))
            if (Aura const* maelstrom = GetAura(53817))
                // xinef: we have 4 charges and all proc conditions are met -
                // aura reaches 5 charges
    ...
    case 58679: {
        // Proc only from healing part of Death Coil. Check is essential as all
        // Death Coil spells have 0x2000 mask in SpellFamilyFlags
        if (!procSpell ||
    ...
    case 62606: {
        basepoints0 =
            CalculatePct(triggerAmount, GetTotalAttackPowerValue(BASE_ATTACK));
        break;
    }
    // Body and Soul
    case 64128:
    ...
    case 70893: {
        ...
        if (!(procSpell->SpellFamilyFlags[1] & 0x10000000) &&
            (procEx != PROC_EX_CRITICAL_HIT))
            return false;
        // check if we're procced by Claw, Bite or Smack (need to use the spell
        // icon ID to detect it)
    ...
    case 44544: {
        if (procPhase == PROC_SPELL_PHASE_HIT) {
            // Find Frostbite
            if (AuraEffect* aurEff =
                    this->GetAuraEffect(SPELL_AURA_ADD_TARGET_TRIGGER,
                                        SPELLFAMILY_MAGE,
                                        119,
                                        EFFECT_0)) {
                if (!victim)
                ...
                uint8 fofRank =
                    sSpellMgr->GetSpellRank(triggeredByAura->GetId());
                ...
                if (roll_chance_i(chance))
                    CastSpell(
                        victim,
                        aurEff->GetSpellInfo()->Effects[EFFECT_0].TriggerSpell,
                        true);
            }
        }
        break;
    ...
    if (!target)
        target = !(procFlags & (PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS |
                                PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS)) &&
                         triggerEntry->IsPositive()
                     ? this
                     : victim;
                                             ...
                                             uint32           cooldown)
{
    ...
    if (!triggerEntry) {
        LOG_ERROR("entities.unit",
                  "Unit::HandleOverrideClassScriptAuraProc: Spell {} "
                  "triggering for class script id {}",
                  triggered_spell_id,
                  scriptId);
        return false;
    }
...
void Unit::setPowerType(Powers new_powertype)
{
    ...
    case POWER_RAGE:
        SetMaxPower(
            POWER_RAGE,
            uint32(std::ceil(GetCreatePowers(POWER_RAGE) * powerMultiplier)));
        SetPower(POWER_RAGE, 0);
        break;
    case POWER_FOCUS:
        SetMaxPower(
            POWER_FOCUS,
            uint32(std::ceil(GetCreatePowers(POWER_FOCUS) * powerMultiplier)));
        SetPower(
            POWER_FOCUS,
            uint32(std::ceil(GetCreatePowers(POWER_FOCUS) * powerMultiplier)));
        break;
    case POWER_ENERGY:
        SetMaxPower(
            POWER_ENERGY,
            uint32(std::ceil(GetCreatePowers(POWER_ENERGY) * powerMultiplier)));
        break;
    case POWER_HAPPINESS:
        SetMaxPower(POWER_HAPPINESS,
                    uint32(std::ceil(GetCreatePowers(POWER_HAPPINESS) *
                                     powerMultiplier)));
    ...
    if (Player const* player = ToPlayer())
        if (player->NeedSendSpectatorData()) {
            ArenaSpectator::SendCommand_UInt32Value(
                FindMap(), GetGUID(), "PWT", new_powertype);
            ArenaSpectator::SendCommand_UInt32Value(
                FindMap(),
                GetGUID(),
                "MPW",
                new_powertype == POWER_RAGE ||
                        new_powertype == POWER_RUNIC_POWER
                    ? GetMaxPower(new_powertype) / 10
                    : GetMaxPower(new_powertype));
            ArenaSpectator::SendCommand_UInt32Value(
                FindMap(),
                GetGUID(),
                "CPW",
    ...
    if (!entry) {
        ...
        if (GetGUID() != guid) {
            if (Player const* player = ToPlayer())
                LOG_ERROR(
                    "entities.unit",
                    "Player {} has invalid faction (faction template id) #{}",
                    player->GetName(),
                    GetFaction());
            else if (Creature const* creature = ToCreature())
                LOG_ERROR("entities.unit",
                          "Creature (template id: {}) has invalid faction "
                          "(faction template id) #{}",
                          creature->GetCreatureTemplate()->Entry,
                          GetFaction());
            else
                LOG_ERROR("entities.unit",
                          "Unit (name={}, type={}) has invalid faction "
                          "(faction template id) #{}",
                          GetName(),
                          uint32(GetTypeId()),
                          GetFaction());
...
ReputationRank Unit::GetReactionTo(Unit const* target,
                                   bool checkOriginalFaction /*= false*/) const
{
    ...
    if (selfPlayerOwner) {
        if (FactionTemplateEntry const* targetFactionTemplateEntry =
            ...
            if (ReputationRank const* repRank =
                    selfPlayerOwner->GetReputationMgr().GetForcedRankIfAny(
                        targetFactionTemplateEntry))
                return *repRank;
    }
    else if (targetPlayerOwner) {
        if (FactionTemplateEntry const* selfFactionTemplateEntry =
                GetFactionTemplateEntry())
            if (ReputationRank const* repRank =
                    targetPlayerOwner->GetReputationMgr().GetForcedRankIfAny(
                        selfFactionTemplateEntry))
                return *repRank;
    }
    ...
    if (HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED)) {
        if (target->HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED)) {
            if (selfPlayerOwner && targetPlayerOwner) {
                // always friendly to other unit controlled by player, or to the
                // player himself
                if (selfPlayerOwner == targetPlayerOwner)
                    return REP_FRIENDLY;
                ...
                if (selfPlayerOwner->IsInRaidWith(targetPlayerOwner))
                    return REP_FRIENDLY; // return true to allow config option
                                         // AllowTwoSide.Interaction.Group to
                                         // work
                // however client seems to allow mixed group parties, because in
                // 13850 client it works like: return
                // GetFactionReactionTo(GetFactionTemplateEntry(), target);
            }
            ...
            if (selfPlayerOwner) {
                if (FactionTemplateEntry const* targetFactionTemplateEntry =
                    ...
                    if (ReputationRank const* repRank =
                        ...
                        return *repRank;
                    if (!selfPlayerOwner->HasUnitFlag2(
    ...
    if (checkOriginalFaction) {
        if (GetTypeId() == TYPEID_PLAYER) {
            if (ChrRacesEntry const* rEntry =
                    sChrRacesStore.LookupEntry(getRace())) {
                factionTemplateEntry =
                    sFactionTemplateStore.LookupEntry(rEntry->FactionID);
            }
        }
        else {
            Unit* owner = GetOwner();
            if (HasUnitTypeMask(UNIT_MASK_MINION) && owner) {
                factionTemplateEntry =
                    sFactionTemplateStore.LookupEntry(owner->GetFaction());
            }
            else if (CreatureTemplate const* cinfo =
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    // if (HasUnitFlag(UNIT_FLAG_PACIFIED)) // pussywizard: why having this flag
    // prevents from entering combat? it should just prevent melee attack
    //     return false;
    ...
    // remove SPELL_AURA_MOD_UNATTACKABLE at attack (in case non-interruptible
    // spells stun aura applied also that not let attack)
    ...
    // if (GetTypeId() == TYPEID_UNIT)
    //    ToCreature()->SetCombatStartPosition(GetPositionX(), GetPositionY(),
    //    GetPositionZ());
    if (creature && !(IsControllableGuardian() && IsControlledByPlayer())) {
        // should not let player enter combat by right clicking target - doesn't
        // helps
        ...
        /// @todo: Implement aggro range, detection range and assistance range
        /// templates
        if (!(creature->GetCreatureTemplate()->flags_extra &
              CREATURE_FLAG_EXTRA_DONT_CALL_ASSISTANCE)) {
            creature->CallAssistance();
        }
        creature->SetAssistanceTimer(
            sWorld->getIntConfig(CONFIG_CREATURE_FAMILY_ASSISTANCE_PERIOD));
...
void Unit::CombatStop(bool includingCast)
{
    ...
    RemoveAllAttackers();
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->SendAttackSwingCancelAttack(); // melee and ranged forced
                                                   // attack cancel
    ClearInCombat();
...
void Unit::CombatStopWithPets(bool includingCast)
{
    ...
    for (ControlSet::const_iterator itr = m_Controlled.begin();
         itr != m_Controlled.end();
         ++itr)
...
void Unit::ModifyAuraState(AuraStateType flag, bool apply)
{
    if (apply) {
        if (!HasFlag(UNIT_FIELD_AURASTATE, 1 << (flag - 1))) {
            ...
            for (Unit::AuraMap::iterator itr = tAuras.begin();
                 itr != tAuras.end();
                 ++itr) {
                ...
                if ((*itr).second->GetSpellInfo()->CasterAuraState == flag)
                    if (AuraApplication* aurApp =
                            (*itr).second->GetApplicationOfTarget(GetGUID()))
                        (*itr).second->HandleAllEffects(
                            aurApp, AURA_EFFECT_HANDLE_REAL, true);
            }
        }
    }
    else {
        if (HasFlag(UNIT_FIELD_AURASTATE, 1 << (flag - 1))) {
            ...
            if (flag != AURA_STATE_ENRAGE) // enrage aura state triggering
                                           // continues auras
            {
                Unit::AuraMap& tAuras = GetOwnedAuras();
                for (Unit::AuraMap::iterator itr = tAuras.begin();
                    ...
                    if ((*itr).second->GetSpellInfo()->CasterAuraState == flag)
                        if (AuraApplication* aurApp =
                                (*itr).second->GetApplicationOfTarget(
                                    GetGUID()))
                            (*itr).second->HandleAllEffects(
                                aurApp, AURA_EFFECT_HANDLE_REAL, false);
                }
            }
        }
    }
...
{
    uint32 auraStates =
        GetUInt32Value(UNIT_FIELD_AURASTATE) & ~(PER_CASTER_AURA_STATE_MASK);
    for (AuraStateAurasMap::const_iterator itr = m_auraStateAuras.begin();
         itr != m_auraStateAuras.end();
         ++itr)
                        ...
                        Unit const*      Caster) const
{
    if (Caster) {
        ...
        if ((1 << (flag - 1)) & PER_CASTER_AURA_STATE_MASK) {
            ...
            for (AuraStateAurasMap::const_iterator itr = range.first;
                 itr != range.second;
                 ++itr)
...
void Unit::SetOwnerGUID(ObjectGuid owner)
{
    ...
    if (!player || !player->HaveAtClient(
                       this)) // if player cannot see this unit yet, he will
                              // receive needed data with create object
        return;
...
Minion* Unit::GetFirstMinion() const
{
    if (ObjectGuid pet_guid = GetMinionGUID()) {
        if (Creature* pet =
                ObjectAccessor::GetCreatureOrPetOrVehicle(*this, pet_guid))
            if (pet->HasUnitTypeMask(UNIT_MASK_MINION))
                return (Minion*)pet;

        LOG_ERROR("entities.unit",
                  "Unit::GetFirstMinion: Minion {} not exist.",
                  pet_guid.ToString());
...
Guardian* Unit::GetGuardianPet() const
{
    if (ObjectGuid pet_guid = GetPetGUID()) {
        ...
        LOG_FATAL("entities.unit",
                  "Unit::GetGuardianPet: Guardian {} not exist.",
                  pet_guid.ToString());
...
Unit* Unit::GetCharm() const
{
    if (ObjectGuid charm_guid = GetCharmGUID()) {
        ...
        LOG_ERROR("entities.unit",
                  "Unit::GetCharm: Charmed creature {} not exist.",
                  charm_guid.ToString());
        const_cast<Unit*>(this)->SetGuidValue(UNIT_FIELD_CHARM,
                                              ObjectGuid::Empty);
    }
...
void Unit::SetMinion(Minion* minion, bool apply)
{
    ...
    if (apply) {
        if (minion->GetOwnerGUID()) {
            LOG_FATAL("entities.unit",
                      "SetMinion: Minion {} is not the minion of owner {}",
                      minion->GetEntry(),
                      GetEntry());
            return;
        }
        ...
        // PvP, FFAPvP
        minion->SetByteValue(
            UNIT_FIELD_BYTES_2, 1, GetByteValue(UNIT_FIELD_BYTES_2, 1));

        // Ghoul pets have energy instead of mana (is anywhere better place for
        // this code?)
        if (minion->IsPetGhoul() ||
            minion->GetEntry() == 24207 /*ENTRY_ARMY_OF_THE_DEAD*/)
            minion->setPowerType(POWER_ENERGY);

        if (GetTypeId() == TYPEID_PLAYER) {
            // Send infinity cooldown - client does that automatically but after
            // relog cooldown needs to be set again
        ...
        if (minion->IsGuardianPet()) {
            ...
            if (SpellInfo const* spInfo =
                    sSpellMgr->GetSpellInfo(minion->ToTotem()->GetSpell())) {
...
void Unit::RemoveAllMinionsByEntry(uint32 entry)
{
    for (Unit::ControlSet::iterator itr = m_Controlled.begin();
        ...
        // i think this is safe because i have never heard that a despawned
        // minion will trigger a same minion
    }
}
    ...
    if (apply) {
        if (GetTypeId() == TYPEID_PLAYER) {
            if (!AddGuidValue(UNIT_FIELD_CHARM, charm->GetGUID()))
                LOG_FATAL("entities.unit",
                          "Player {} is trying to charm unit {}, but it "
                          "already has a charmed unit {}",
                          GetName(),
                          charm->GetEntry(),
                          GetCharmGUID().ToString());

            charm->m_ControlledByPlayer = true;
            /// @todo: maybe we can use this flag to check if controlled by
            /// player
        ...
        if (!charm->AddGuidValue(UNIT_FIELD_CHARMEDBY, GetGUID()))
            LOG_FATAL(
                "entities.unit",
                "Unit {} is being charmed, but it already has a charmer {}",
                charm->GetEntry(),
                charm->GetCharmerGUID().ToString());
        ...
        if (GetTypeId() == TYPEID_PLAYER) {
            if (!RemoveGuidValue(UNIT_FIELD_CHARM, charm->GetGUID()))
                LOG_FATAL("entities.unit",
                          "Player {} is trying to uncharm unit {}, but it has "
                          "another charmed unit {}",
                          GetName(),
                          charm->GetEntry(),
                          GetCharmGUID().ToString());
        }
        ...
        if (!charm->RemoveGuidValue(UNIT_FIELD_CHARMEDBY, GetGUID()))
            LOG_FATAL(
                "entities.unit",
                "Unit {} is being uncharmed, but it has another charmer {}",
                charm->GetEntry(),
                charm->GetCharmerGUID().ToString());
        ...
        if (charm->GetTypeId() == TYPEID_PLAYER) {
            ...
            charm->ToPlayer()->UpdatePvPState();
                     ...
                     charm->GetCharmerOrOwnerPlayerOrPlayerItself()) {
            charm->m_ControlledByPlayer = true;
            charm->SetUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);
            charm->SetByteValue(UNIT_FIELD_BYTES_2,
                                1,
                                player->GetByteValue(UNIT_FIELD_BYTES_2, 1));

            // Xinef: skip controlled erase if charmed unit is owned by charmer
            if (charm->IsInWorld() && !charm->IsDuringRemoveFromWorld() &&
                player->GetGUID() == this->GetGUID() &&
                (charm->IsPet() || charm->HasUnitTypeMask(UNIT_MASK_MINION)))
                return;
        }
        else {
            charm->m_ControlledByPlayer = false;
        ...
        if (charm->IsWalking() != _isWalkingBeforeCharm) {
            ...
            charm->SendMovementFlagUpdate(
                true); // send packet to self, to update movement state on
                       // player.
        }
...
int32 Unit::DealHeal(Unit* healer, Unit* victim, uint32 addhealth)
{
    ...
    if (healer && healer->GetTypeId() == TYPEID_UNIT &&
        healer->ToCreature()->IsTotem())
    ...
    if (Player* player = unit->ToPlayer()) {
        ...
        // use the actual gain, as the overheal shall not be counted, skip gain
        // 0 (it ignored anyway in to criteria)
        if (gain && player->InBattleground()) // pussywizard: InBattleground()
                                              // optimization
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_HEALING_DONE, gain, 0, victim);

        // player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEAL_CASTED,
        // addhealth); // pussywizard: optimization
    }

    /*if (Player* player = victim->ToPlayer())
    {
        //player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_HEALING_RECEIVED,
    gain); // pussywizard: optimization
        //player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEALING_RECEIVED,
    addhealth); // pussywizard: optimization
    }*/
...
bool RedirectSpellEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    if (Unit* auraOwner = ObjectAccessor::GetUnit(_self, _auraOwnerGUID)) {
        ...
        Unit::AuraEffectList const& magnetAuras =
            auraOwner->GetAuraEffectsByType(SPELL_AURA_SPELL_MAGNET);
...
Unit* Unit::GetMagicHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    if (spellInfo->HasAttribute(SPELL_ATTR0_IS_ABILITY) ||
        spellInfo->HasAttribute(SPELL_ATTR1_NO_REDIRECTION) ||
        spellInfo->HasAttribute(SPELL_ATTR0_NO_IMMUNITIES))
        return victim;

    Unit::AuraEffectList const& magnetAuras =
        victim->GetAuraEffectsByType(SPELL_AURA_SPELL_MAGNET);
    for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin();
         itr != magnetAuras.end();
         ++itr) {
        if (Unit* magnet = (*itr)->GetBase()->GetUnitOwner())
            if (spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK
                //&& spellInfo->CheckTarget(this, magnet, false) ==
                //SPELL_CAST_OK
                && _IsValidAttackTarget(magnet, spellInfo)
                /*&& IsWithinLOSInMap(magnet)*/) {
                // Xinef: We should choose minimum between flight time and queue
                // time as in reflect, however we dont know flight time at this
                // point, use arbitrary small number
                magnet->m_Events.AddEvent(
                    new RedirectSpellEvent(*magnet, victim->GetGUID(), *itr),
                    magnet->m_Events.CalculateQueueTime(100));

                if (magnet->IsTotem()) {
                    uint64 queueTime = magnet->m_Events.CalculateQueueTime(100);
                    if (spellInfo->Speed > 0.0f) {
                        float dist = GetDistance(magnet->GetPositionX(),
                                                 magnet->GetPositionY(),
                                                 magnet->GetPositionZ());
                        if (dist < 5.0f)
                            dist = 5.0f;
                        queueTime = magnet->m_Events.CalculateTime(
                            (uint64)floor(dist / spellInfo->Speed * 1000.0f));
                    }

                    magnet->m_Events.AddEvent(new KillMagnetEvent(*magnet),
                                              queueTime);
                }
...
{
    AuraEffectList const& hitTriggerAuras =
        victim->GetAuraEffectsByType(SPELL_AURA_ADD_CASTER_HIT_TRIGGER);
    for (AuraEffectList::const_iterator i = hitTriggerAuras.begin();
         i != hitTriggerAuras.end();
         ++i) {
        if (Unit* magnet = (*i)->GetBase()->GetCaster())
            if (_IsValidAttackTarget(magnet, spellInfo) &&
                magnet->IsWithinLOSInMap(this) &&
                (!spellInfo || (spellInfo->CheckExplicitTarget(this, magnet) ==
                                    SPELL_CAST_OK &&
                                spellInfo->CheckTarget(this, magnet, false) ==
                                    SPELL_CAST_OK)))
...
void Unit::RemoveAllControlled(bool onDeath /*= false*/)
{
    ...
    while (!m_Controlled.empty()) {
        ...
        if (target->GetCharmerGUID() == GetGUID()) {
            ...
            LOG_ERROR("entities.unit",
                      "Unit {} is trying to release unit {} which is neither "
                      "charmed nor owned by it",
                      GetEntry(),
                      target->GetEntry());
        }
    }
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    // reserve place for players and pets because resizing vector every unit
    // push is unefficient (vector is reallocated then)
...
int32 Unit::HealBySpell(HealInfo& healInfo, bool critical)
{
    ...
    sScriptMgr->ModifyHealReceived(
        this, healInfo.GetTarget(), heal, healInfo.GetSpellInfo());
    healInfo.SetHeal(heal);

    // calculate heal absorb and reduce healing
    CalcHealAbsorb(healInfo);

    int32 gain = Unit::DealHeal(
        healInfo.GetHealer(), healInfo.GetTarget(), healInfo.GetHeal());
                           ...
                           Powers powerType)
{
    ...
    if (powerType != POWER_HAPPINESS && gainedPower) {
        SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellID);
        victim->getHostileRefMgr().threatAssist(
            this, float(gainedPower) * 0.5f, spellInfo);
    }
                                   ...
                                   DamageEffectType damagetype)
{
    ...
    if (GetTypeId() == TYPEID_UNIT) {
        if (IsTotem()) {
            if (Unit* owner = GetOwner())
                return owner->SpellPctDamageModsDone(
                    victim, spellProto, damagetype);
        }
        // Dancing Rune Weapon...
    ...
    AuraEffectList const& mModDamagePercentDone =
        GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
    for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin();
         i != mModDamagePercentDone.end();
         ++i) {
        // prevent apply mods from weapon specific case to non weapon specific
        // spells (Example: thunder clap and two-handed weapon specialization)
        if (spellProto->EquippedItemClass == -1 &&
            (*i)->GetSpellInfo()->EquippedItemClass != -1 &&
            !(*i)->GetSpellInfo()->HasAttribute(
                SPELL_ATTR5_AURA_AFFECTS_NOT_JUST_REQ_EQUIPED_ITEM) &&
            (*i)->GetMiscValue() == SPELL_SCHOOL_MASK_NORMAL) {
            continue;
        }

        if (!spellProto->ValidateAttribute6SpellDamageMods(
                this, *i, damagetype == DOT))
            continue;

        if (!sScriptMgr->IsNeedModSpellDamagePercent(
        ...
        if ((*i)->GetMiscValue() & spellProto->GetSchoolMask()) {
            ...
            else if (!(*i)->GetSpellInfo()->HasAttribute(
                         SPELL_ATTR5_AURA_AFFECTS_NOT_JUST_REQ_EQUIPED_ITEM) &&
                     ((*i)->GetSpellInfo()->EquippedItemSubClassMask == 0))
                AddPct(DoneTotalMod, (*i)->GetAmount());
            else if (ToPlayer() && ToPlayer()->HasItemFitToSpellRequirements(
                                       (*i)->GetSpellInfo()))
                AddPct(DoneTotalMod, (*i)->GetAmount());
        }
    }
    ...
    AuraEffectList const& mDamageDoneVersus =
        GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE_VERSUS);
    for (AuraEffectList::const_iterator i = mDamageDoneVersus.begin();
         i != mDamageDoneVersus.end();
         ++i)
        if ((creatureTypeMask & uint32((*i)->GetMiscValue())) &&
            spellProto->ValidateAttribute6SpellDamageMods(
                this, *i, damagetype == DOT))
            AddPct(DoneTotalMod, (*i)->GetAmount());

    // bonus against aurastate
    AuraEffectList const& mDamageDoneVersusAurastate =
        GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE_VERSUS_AURASTATE);
    for (AuraEffectList::const_iterator i = mDamageDoneVersusAurastate.begin();
         i != mDamageDoneVersusAurastate.end();
         ++i)
    ...
    AuraEffectList const& mOverrideClassScript =
        owner->GetAuraEffectsByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
         i != mOverrideClassScript.end();
         ++i) {
        ...
        case 4919:
            if (victim->HasAuraState(
                    AURA_STATE_HEALTHLESS_35_PERCENT, spellProto, this))
                AddPct(DoneTotalMod, (*i)->GetAmount());
            break;
        }
        ...
        case 4993: {
            ...
            // effect 0 m_amount
            int32 stepPercent =
                CalculateSpellDamage(this, (*i)->GetSpellInfo(), 0);
        ...
        case 6927:
            if (HasAuraState(
                    AURA_STATE_HEALTHLESS_20_PERCENT, spellProto, this))
                AddPct(DoneTotalMod, (*i)->GetAmount());
            break;
        ...
        case 7277: {
            ...
            if ((*i)->GetSpellInfo()->SpellIconID == 2656) {
                if ((spellProto && spellProto->SpellFamilyFlags[0] & 0x2) ||
                    spellProto->SpellFamilyFlags[1] & 0x2)
        ...
        case 7601:
        case 7602: {
            if (victim->GetAuraEffect(
                    SPELL_AURA_MOD_STALKED, SPELLFAMILY_HUNTER, 0x400, 0, 0))
                AddPct(DoneTotalMod, (*i)->GetAmount());
            break;
        }
        // Dirty Deeds
    ...
    case SPELLFAMILY_MAGE:
        ...
        if (spellProto->SpellIconID == 186) {
            if (victim->HasAuraState(AURA_STATE_FROZEN, spellProto, this)) {
                ...
                if (owner->HasAura(56377) &&
                    victim->GetLevel() > owner->GetLevel())
                    DoneTotalMod *= 4.0f;
                else
                    DoneTotalMod *= 3.0f;
            }
        ...
        // Torment the weak
        if (spellProto->SpellFamilyFlags[0] & 0x20600021 ||
            spellProto->SpellFamilyFlags[1] & 0x9000)
            if (victim->HasAuraWithMechanic((1 << MECHANIC_SNARE) |
                                            (1 << MECHANIC_SLOW_ATTACK)))
                if (AuraEffect* aurEff = GetAuraEffect(
                        SPELL_AURA_DUMMY, SPELLFAMILY_GENERIC, 3263, EFFECT_0))
    ...
    case SPELLFAMILY_PRIEST:
        ...
        if (spellProto->SpellFamilyFlags[0] & 0x800000) {
            ...
            if (AuraEffect* aurEff = GetAuraEffect(55687, 0))
                // Increase Mind Flay damage if Shadow Word: Pain present on
                // target
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                          SPELLFAMILY_PRIEST,
                                          0x8000,
            ...
            if (AuraEffect* aurEff =
                    GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS,
                                  SPELLFAMILY_PRIEST,
                                  2848,
                                  1))
                // Increase Mind Flay damage if Shadow Word: Pain present on
    ...
    case SPELLFAMILY_PALADIN:
        ...
        if ((spellProto->SpellFamilyFlags[1] & 0x400000) &&
            ...
            // Get stack of Holy Vengeance/Blood Corruption on the target added
            // by caster
            uint32                      stacks = 0;
            Unit::AuraEffectList const& auras =
                victim->GetAuraEffectsByType(SPELL_AURA_PERIODIC_DAMAGE);
            for (Unit::AuraEffectList::const_iterator itr = auras.begin();
                 itr != auras.end();
                 ++itr)
            ...
            // + 10% for each application of Holy Vengeance/Blood Corruption on
            // the target
            if (stacks)
    ...
    case SPELLFAMILY_WARLOCK:
        ...
        if (spellProto->SpellFamilyFlags[1] & 0x00020040)
            if (victim->HasAuraState(AURA_STATE_CONFLAGRATE)) {
                AuraEffectList const& mDumyAuras =
                    GetAuraEffectsByType(SPELL_AURA_DUMMY);
    ...
    case SPELLFAMILY_HUNTER:
        ...
        if (spellProto->SpellFamilyFlags[1] & 0x1)
            if (AuraEffect* aurEff =
                    GetAuraEffect(56826, 0)) // Glyph of Steady Shot
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                          SPELLFAMILY_HUNTER,
                                          0x00004000,
                                          0,
    ...
    case SPELLFAMILY_DEATHKNIGHT:
        ...
        if (spellProto->SpellFamilyFlags[0] & 0x2)
            if (AuraEffect* aurEff =
                    GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 2721, 0))
                AddPct(DoneTotalMod, aurEff->GetAmount());
        ...
        if (spellProto->SpellFamilyFlags[0] & 0x2 ||
            ...
            if (AuraEffect* aurEff =
                    GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 196, 0))
                                  ...
                                  uint32           stack)
{
    ...
    if (GetTypeId() == TYPEID_UNIT) {
        if (IsTotem()) {
            if (Unit* owner = GetOwner())
                return owner->SpellDamageBonusDone(victim,
                                                   spellProto,
                                                   pdamage,
                                                   damagetype,
                                                   effIndex,
    ...
    float DoneTotalMod =
        TotalMod ? TotalMod
                 : SpellPctDamageModsDone(victim, spellProto, damagetype);

    // Config : RATE_CREATURE_X_SPELLDAMAGE & Do Not Modify Pet/Guardian/Mind
    // Controled Damage
    if (GetTypeId() == TYPEID_UNIT &&
        (!ToCreature()->IsPet() || !ToCreature()->IsGuardian() ||
         !ToCreature()->IsControlledByPlayer()))
        DoneTotalMod *= ToCreature()->GetSpellDamageMod(
            ToCreature()->GetCreatureTemplate()->rank);
    ...
    case SPELLFAMILY_DRUID: {
        ...
        if (spellProto->SpellFamilyFlags[0] & 0x00200000) {
            if (AuraEffect const* relicAurEff =
                    GetAuraEffect(64950, EFFECT_0)) {
    ...
    case SPELLFAMILY_DEATHKNIGHT: {
        ...
        // Impurity
        if (AuraEffect* aurEff =
                GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 1986, 0)) {
    ...
    DoneAdvertisedBenefit +=
        SpellBaseDamageBonusDone(spellProto->GetSchoolMask());
    ...
    if (bonus) {
        if (damagetype == DOT) {
            ...
            if (bonus->ap_dot_bonus > 0) {
                WeaponAttackType attType =
                    (spellProto->IsRangedWeaponSpell() &&
                     spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE)
                        ? RANGED_ATTACK
                        : BASE_ATTACK;
                float APbonus = float(victim->GetTotalAuraModifier(
                    attType == BASE_ATTACK
    ...
    if (coeff && DoneAdvertisedBenefit) {
        ...
        if (Player* modOwner = GetSpellModOwner()) {
            coeff *= 100.0f;
            modOwner->ApplySpellMod(
                spellProto->Id, SPELLMOD_BONUS_MULTIPLIER, coeff);
            coeff /= 100.0f;
        }
                                   ...
                                   uint32           stack)
{
    ...
    // multiplicative bonus, for example Dispersion + Shadowform
    // (0.10*0.85=0.085)
    AuraEffectList const& mTotalAuraList =
    ...
    // From caster spells
    if (caster) {
        AuraEffectList const& mOwnerTaken =
            GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_FROM_CASTER);
    ...
    if (uint32 mechanicMask = spellProto->GetAllEffectsMechanicMask()) {
        int32                 modifierMax = 0;
        int32                 modifierMin = 0;
        AuraEffectList const& auraEffectList =
            GetAuraEffectsByType(SPELL_AURA_MOD_MECHANIC_DAMAGE_TAKEN_PERCENT);
        for (AuraEffectList::const_iterator i = auraEffectList.begin();
             i != auraEffectList.end();
             ++i) {
            if (!spellProto->ValidateAttribute6SpellDamageMods(
                    caster, *i, damagetype == DOT))
                continue;
            ...
            if ((*i)->GetSpellInfo()->SpellFamilyName ==
                SPELLFAMILY_DEATHKNIGHT)
    ...
    int32 TakenAdvertisedBenefit = SpellBaseDamageBonusTaken(
        spellProto->GetSchoolMask(), damagetype == DOT);
    ...
    if (TakenAdvertisedBenefit) {
        if (coeff <= 0.0f) {
            if (caster)
                coeff = caster->CalculateDefaultCoefficient(spellProto,
                                                            damagetype) *
                        int32(stack);
            else
    ...
    if (spellProto->HasAttribute(SPELL_ATTR0_CU_NO_POSITIVE_TAKEN_BONUS) &&
        TakenTotalMod > 1.0f) {
        TakenTotal    = 0;
        TakenTotalMod = 1.0f;
    }
    ...
    if (caster && TakenTotalMod < 1.0f &&
        caster->HasAuraType(SPELL_AURA_MOD_IGNORE_TARGET_RESIST)) {
...
{
    // note: old code coming from TC, just extracted here to remove the code
    // duplication + solve potential crash see:
    // https://github.com/TrinityCore/TrinityCore/commit/c85710e148d75450baedf6632b9ca6fd40b4148e
        ...
        if (auto spellIconId = (*i)->GetSpellInfo()->SpellIconID) {
            ...
            case 2109:
                if ((*i)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) {
                    // Patch 2.4.3: The resilience required to reach the 90%
                    // damage reduction cap
                    //  is 22.5% critical strike damage reduction, or 444
                    //  resilience.
                    // To calculate for 90%, we multiply the 100% by 4 (22.5% *
                    // 4 = 90%)
                    float mod = -1.0f * GetMeleeCritDamageReduction(400);
                    AddPct(TakenTotalMod,
                           std::max(mod, float((*i)->GetAmount())));
                }
                break;
            }
        }
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask)
{
    ...
    AuraEffectList const& mDamageDone =
        GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE);
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
        ...
        for (AuraEffectList::const_iterator i =
             ...
             i != mDamageDoneOfStatPercent.end();
             ++i) {
        ...
        // ... and attack power
        AuraEffectList const& mDamageDonebyAP =
            GetAuraEffectsByType(SPELL_AURA_MOD_SPELL_DAMAGE_OF_ATTACK_POWER);
        for (AuraEffectList::const_iterator i = mDamageDonebyAP.begin();
             i != mDamageDonebyAP.end();
             ++i)
            if ((*i)->GetMiscValue() & schoolMask)
                DoneAdvertisedBenefit += int32(CalculatePct(
                    GetTotalAttackPowerValue(BASE_ATTACK), (*i)->GetAmount()));
    }
...
int32 Unit::SpellBaseDamageBonusTaken(SpellSchoolMask schoolMask, bool isDoT)
{
    ...
    AuraEffectList const& mDamageTaken =
        GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_TAKEN);
    for (AuraEffectList::const_iterator i = mDamageTaken.begin();
         i != mDamageTaken.end();
         ++i)
        if (((*i)->GetMiscValue() & schoolMask) != 0) {
            // Xinef: if we have DoT damage type and aura has charges, check if
            // it affects DoTs Xinef: required for hemorrhage & rupture /
            // garrote
            if (isDoT && (*i)->GetBase()->IsUsingCharges() &&
                !((*i)->GetSpellInfo()->ProcFlags & PROC_FLAG_TAKEN_PERIODIC))
                continue;
...
float Unit::SpellDoneCritChance(Unit const* /*victim*/,
                                SpellInfo const* spellProto,
                                SpellSchoolMask  schoolMask,
                                WeaponAttackType attackType,
                                bool             skipEffectCheck) const
{
    ...
    // Xinef: check if spell is capable of critting, auras requires special aura
    // to crit so they can be skipped
    ...
    case SPELL_DAMAGE_CLASS_MAGIC: {
        ...
        // For other schools
            ...
            crit_chance = GetFloatValue(
                static_cast<uint16>(PLAYER_SPELL_CRIT_PERCENTAGE1) +
                                 ...
                                 bool             skipEffectCheck) const
{
    ...
    case SPELL_DAMAGE_CLASS_MAGIC: {
        if (!spellProto->IsPositive()) {
            ...
            // SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE xinef: apply max and
            // min only
            if (HasAuraType(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE)) {
                crit_chance += GetMaxNegativeAuraModifierByMiscMask(
                    SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
                crit_chance += GetMaxPositiveAuraModifierByMiscMask(
                    SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
            }
        ...
        if (caster) {
            AuraEffectList const& mOverrideClassScript =
                caster->GetAuraEffectsByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
            for (AuraEffectList::const_iterator i =
            ...
            case SPELLFAMILY_MAGE:
                // Glyph of Fire Blast
                if (spellProto->SpellFamilyFlags[0] == 0x2 &&
                    spellProto->SpellIconID == 12)
                    if (HasAuraWithMechanic((1 << MECHANIC_STUN) |
                                            (1 << MECHANIC_KNOCKOUT)))
                        if (AuraEffect const* aurEff =
                                caster->GetAuraEffect(56369, EFFECT_0))
                            crit_chance += aurEff->GetAmount();
                break;
            case SPELLFAMILY_DRUID:
                // Improved Faerie Fire
                if (HasAuraState(AURA_STATE_FAERIE_FIRE))
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(
                            SPELLFAMILY_DRUID, 109, 0))
                        crit_chance += aurEff->GetAmount();
            ...
            case SPELLFAMILY_PALADIN:
                ...
                if (spellProto->SpellFamilyFlags[0] & 0x40000000) {
                    // Sacred Shield
                    if (AuraEffect const* aura =
                            GetAuraEffect(58597, 1, GetGUID()))
                        crit_chance += aura->GetAmount();
                    break;
                }
                // Exorcism
                else if (spellProto->GetCategory() == 19) {
                    if (GetCreatureTypeMask() &
    ...
    case SPELL_DAMAGE_CLASS_MELEE:
        ...
        if (caster) {
            switch (spellProto->SpellFamilyName) {
            case SPELLFAMILY_DRUID:
                // Rend and Tear - bonus crit chance for Ferocious Bite on
                // bleeding targets
                if (spellProto->SpellFamilyFlags[0] & 0x00800000 &&
                    spellProto->SpellIconID == 1680 &&
                    HasAuraState(AURA_STATE_BLEEDING)) {
                    if (AuraEffect const* rendAndTear =
                            caster->GetDummyAuraEffect(
                                SPELLFAMILY_DRUID, 2859, 1))
        ...
        if (GetTypeId() == TYPEID_PLAYER &&
            (IsSitState() || getStandState() == UNIT_STAND_STATE_SLEEP)) {
            return 100.0f;
        }
        [[fallthrough]]; /// @todo: Not sure whether the fallthrough was a
                         /// mistake (forgetting a break) or intended. This
    ...
    case SPELL_DAMAGE_CLASS_RANGED: {
        ...
        // reduce crit chance from Rating for players
        if (attackType != RANGED_ATTACK) {
            // xinef: little hack, crit chance dont require caster to calculate,
            // pass victim
            Unit::ApplyResilience(
                this, &crit_chance, nullptr, false, CR_CRIT_TAKEN_MELEE);
        }
        else
            Unit::ApplyResilience(
                this, &crit_chance, nullptr, false, CR_CRIT_TAKEN_RANGED);

        // Apply crit chance from defence skill
        if (caster)
            crit_chance += (int32(caster->GetMaxSkillValueForLevel(this)) -
                            int32(GetDefenseSkillValue(caster))) *
                           0.04f;
    ...
    // Modify critical chance by victim
    // SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE xinef: should be
    // calculated at the end
    if (!spellProto->IsPositive())
        crit_chance += GetTotalAuraModifier(
            SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);
                                      ...
                                      Unit const*      victim)
{
    ...
    if (caster) {
        crit_mod += caster->GetTotalAuraModifierByMiscMask(
            SPELL_AURA_MOD_CRIT_DAMAGE_BONUS, spellProto->GetSchoolMask());

        if (victim)
            crit_mod += caster->GetTotalAuraModifierByMiscMask(
                SPELL_AURA_MOD_CRIT_PERCENT_VERSUS,
                victim->GetCreatureTypeMask());
                                       ...
                                       Unit const*      victim)
{
    ...
    if (caster)
        damage =
            int32(float(damage) * caster->GetTotalAuraMultiplier(
                                    ...
                                    DamageEffectType damagetype)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && IsTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellPctHealingModsDone(
                victim, spellProto, damagetype);
    ...
    AuraEffectList const& mHealingDonePct =
        GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_DONE_PERCENT);
    for (auto const& auraEff : mHealingDonePct) {
        if (!sScriptMgr->IsNeedModHealPercent(
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
        ...
        {
            if (victim->GetAuraEffect(
                    SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_DRUID, 0x40, 0, 0))
                AddPct(DoneTotalMod, (*i)->GetAmount());
            break;
        }
        ...
        case 7871: // Glyph of Lesser Healing Wave
        {
            ...
            if (victim->GetAuraEffect(
                    SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, 0, 0x00000400, 0))
                AddPct(DoneTotalMod, (*i)->GetAmount());
            break;
        }
        default:
    ...
    case SPELLFAMILY_GENERIC:
        ...
        if (spellProto->Id == 52042) {
            ...
            if (AuraEffect* aurEff =
                    GetDummyAuraEffect(SPELLFAMILY_SHAMAN, 338, 1))
                AddPct(DoneTotalMod, aurEff->GetAmount());
        }
        break;
    case SPELLFAMILY_PRIEST:
                                   ...
                                   uint32           stack)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && IsTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellHealingBonusDone(victim,
                                                spellProto,
                                                healamount,
                                                damagetype,
                                                effIndex,
    ...
    float DoneTotalMod =
        TotalMod ? TotalMod
                 : SpellPctHealingModsDone(victim, spellProto, damagetype);
    int32 DoneTotal = 0;
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
        ...
        case 4953:
            DoneAdvertisedBenefit += (*i)->GetAmount();
            break;
        case 3736: // Hateful Totem of the Third Wind / Increased Lesser Healing
    ...
    // Done fixed damage bonus auras
    DoneAdvertisedBenefit +=
        SpellBaseHealingBonusDone(spellProto->GetSchoolMask());
    float coeff = spellProto->Effects[effIndex].BonusMultiplier;
    ...
    if (bonus) {
        if (damagetype == DOT) {
            ...
            if (bonus->ap_dot_bonus > 0)
                DoneTotal +=
                    int32(bonus->ap_dot_bonus * ApCoeffMod * stack *
            ...
            if (bonus->ap_bonus > 0)
                DoneTotal +=
                    int32(bonus->ap_bonus * ApCoeffMod * stack *
                          GetTotalAttackPowerValue(
                              (spellProto->IsRangedWeaponSpell() &&
                               spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE)
                                  ? RANGED_ATTACK
                                    ...
                                    uint32           stack)
{
    ...
    if (!sScriptMgr->OnSpellHealingBonusTakenNegativeModifiers(
            this, caster, spellProto, minval)) {
    ...
    int32 TakenAdvertisedBenefit =
        SpellBaseHealingBonusTaken(spellProto->GetSchoolMask());

    // Nourish cast, glyph of nourish
    if (spellProto->SpellFamilyName == SPELLFAMILY_DRUID &&
        spellProto->SpellFamilyFlags[1] & 0x2000000 && caster) {
        bool                  any      = false;
        bool                  hasglyph = caster->GetAuraEffectDummy(62971);
        AuraEffectList const& auras =
            GetAuraEffectsByType(SPELL_AURA_PERIODIC_HEAL);
        for (AuraEffectList::const_iterator i = auras.begin(); i != auras.end();
             ++i) {
            if (((*i)->GetCasterGUID() == caster->GetGUID())) {
                SpellInfo const* spell = (*i)->GetSpellInfo();
                ...
                if (!any &&
                    spell->SpellFamilyFlags.HasFlag(0x50, 0x4000010, 0)) {
                    TakenTotalMod *= 1.2f;
                    any = true;
                }
    ...
    if (damagetype == DOT) {
        ...
        float minval_hot =
            float(GetMaxNegativeAuraModifier(SPELL_AURA_MOD_HOT_PCT));
        if (minval_hot)
            AddPct(TakenTotalMod, minval_hot);

        float maxval_hot =
            float(GetMaxPositiveAuraModifier(SPELL_AURA_MOD_HOT_PCT));
        if (maxval_hot)
    ...
    if (bonus)
        ...
        if (spellProto->DmgClass == SPELL_DAMAGE_CLASS_NONE) {
            healamount =
                uint32(std::max((float(healamount) * TakenTotalMod), 0.0f));
            return healamount;
        }
    }
    ...
    if (TakenAdvertisedBenefit) {
        ...
        if (coeff <= 0)
            coeff = CalculateDefaultCoefficient(spellProto, damagetype) *
                    int32(stack) * 1.88f; // As wowwiki says: C = (Cast Time
                                          // / 3.5) * 1.88 (for healing spells)
        ...
        TakenTotal += int32(TakenAdvertisedBenefit *
                            (coeff > 0 ? coeff : TakenCoeff) * factorMod);
    }

    if (caster) {
        AuraEffectList const& mHealingGet =
            GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_RECEIVED);
    ...
    // No positive taken bonus, custom attr
    if ((spellProto->HasAttribute(SPELL_ATTR6_IGNORE_HEALTH_MODIFIERS) ||
         spellProto->HasAttribute(SPELL_ATTR0_CU_NO_POSITIVE_TAKEN_BONUS)) &&
        TakenTotalMod > 1.0f) {
        TakenTotal    = 0;
        TakenTotalMod = 1.0f;
    }
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask)
{
    ...
    AuraEffectList const& mHealingDone =
        GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_DONE);
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
        ...
        AuraEffectList const& mHealingDoneOfStatPercent =
            GetAuraEffectsByType(SPELL_AURA_MOD_SPELL_HEALING_OF_STAT_PERCENT);
        for (AuraEffectList::const_iterator i =
                 mHealingDoneOfStatPercent.begin();
             i != mHealingDoneOfStatPercent.end();
             ++i) {
            // stat used dependent from misc value (stat index)
            Stats usedStat = Stats(
                (*i)->GetSpellInfo()->Effects[(*i)->GetEffIndex()].MiscValue);
            AdvertisedBenefit +=
                int32(CalculatePct(GetStat(usedStat), (*i)->GetAmount()));
        }

        // ... and attack power
        AuraEffectList const& mHealingDonebyAP =
            GetAuraEffectsByType(SPELL_AURA_MOD_SPELL_HEALING_OF_ATTACK_POWER);
        for (AuraEffectList::const_iterator i = mHealingDonebyAP.begin();
             i != mHealingDonebyAP.end();
             ++i)
            if ((*i)->GetMiscValue() & schoolMask)
                AdvertisedBenefit += int32(CalculatePct(
                    GetTotalAttackPowerValue(BASE_ATTACK), (*i)->GetAmount()));
    }
...
int32 Unit::SpellBaseHealingBonusTaken(SpellSchoolMask schoolMask)
{
    ...
    AuraEffectList const& mDamageTaken =
        GetAuraEffectsByType(SPELL_AURA_MOD_HEALING);
    for (AuraEffectList::const_iterator i = mDamageTaken.begin();
         i != mDamageTaken.end();
         ++i)
        if (((*i)->GetMiscValue() & schoolMask) != 0)
...
bool Unit::IsImmunedToDamage(SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo->HasAttribute(
        ...
        spellInfo->HasAttribute(SPELL_ATTR2_NO_SCHOOL_IMMUNITIES)) {
        return false;
    }
...
bool Unit::IsImmunedToSchool(SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo->Id != 42292 && spellInfo->Id != 59752 &&
        spellInfo->Id != 19574 && spellInfo->Id != 34471) {
        // If m_immuneToSchool type contain this school type, IMMUNE damage.
        SpellImmuneList const& schoolList = m_spellImmune[IMMUNITY_SCHOOL];
        for (SpellImmuneList::const_iterator itr = schoolList.begin();
             itr != schoolList.end();
             ++itr)
            if ((itr->type & schoolMask) == schoolMask &&
                !spellInfo->CanPierceImmuneAura(
                    sSpellMgr->GetSpellInfo(itr->spellId)))
                return true;
    }
...
bool Unit::IsImmunedToDamageOrSchool(SpellSchoolMask meleeSchoolMask) const
{
    ...
    return IsImmunedToDamage(meleeSchoolMask) ||
           IsImmunedToSchool(meleeSchoolMask);
}
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo, Spell const* spell)
{
    ...
    // xinef: my special immunity, if spellid is not on this list it means npc
    // is immune
    SpellImmuneList const& allowIdList = m_spellImmune[IMMUNITY_ALLOW_ID];
    if (!allowIdList.empty()) {
        for (SpellImmuneList::const_iterator itr = allowIdList.begin();
             itr != allowIdList.end();
             ++itr)
            if (itr->type == spellInfo->Id)
                return false;
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        // Ignore effects with mechanic, they are supposed to be checked
        // separately
        if (!spellInfo->Effects[i].IsEffect())
            continue;

        // Xinef: if target is immune to one effect, and the spell has transform
        // aura - it is immune to whole spell
    ...
    if (immuneToAllEffects) // Return immune only if the target is immune to all
                            // spell effects.
        return true;
    ...
    if (spellInfo->Id != 42292 && spellInfo->Id != 59752 &&
        ...
        if (spellSchoolMask != SPELL_SCHOOL_MASK_NONE) {
            ...
            for (SpellImmuneList::const_iterator itr = schoolList.begin();
                 itr != schoolList.end();
                 ++itr) {
                SpellInfo const* immuneSpellInfo =
                    sSpellMgr->GetSpellInfo(itr->spellId);
                if (((itr->type & spellSchoolMask) == spellSchoolMask) &&
                    (!immuneSpellInfo || immuneSpellInfo->IsPositive()) &&
                                  ...
                                  uint32           index) const
{
    ...
    for (SpellImmuneList::const_iterator itr = effectList.begin();
        ...
        if (itr->type == effect &&
            (itr->spellId != 62692 ||
             (spellInfo->Effects[index].MiscValue == POWER_MANA &&
              !CanRestoreMana(spellInfo)))) {
            return true;
        }
    }
    ...
    if (uint32 aura = spellInfo->Effects[index].ApplyAuraName) {
            ...
            if (itr->type == aura &&
                (itr->spellId != 64848 ||
                 (spellInfo->Effects[index].MiscValue == POWER_MANA &&
                  !CanRestoreMana(spellInfo)))) {
                if (!spellInfo->HasAttribute(SPELL_ATTR3_ALWAYS_HIT)) {
                    if (itr->blockType == SPELL_BLOCK_TYPE_ALL ||
                        spellInfo->IsPositive()) // xinef: added for pet scaling
                    {
                        return true;
                    }
                }
        ...
        if (!spellInfo->HasAttribute(SPELL_ATTR2_NO_SCHOOL_IMMUNITIES)) {
            ...
            AuraEffectList const& immuneAuraApply =
                GetAuraEffectsByType(SPELL_AURA_MOD_IMMUNE_AURA_APPLY_SCHOOL);
            for (AuraEffectList::const_iterator iter = immuneAuraApply.begin();
                 iter != immuneAuraApply.end();
                 ++iter) {
...
uint32 Unit::MeleeDamageBonusDone(
    Unit*            victim,
    uint32           pdamage,
    WeaponAttackType attType,
    SpellInfo const* spellProto,
    SpellSchoolMask  damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    if (GetTypeId() == TYPEID_UNIT) {
        ...
        if (GetEntry() == 27893) {
            if (Unit* owner = GetOwner())
                return owner->MeleeDamageBonusDone(victim,
                                                   pdamage,
                                                   attType,
                                                   spellProto,
                                                   damageSchoolMask) /
    ...
    // ..done
    AuraEffectList const& mDamageDoneCreature =
        GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE_CREATURE);
    for (AuraEffectList::const_iterator i = mDamageDoneCreature.begin();
         i != mDamageDoneCreature.end();
         ++i)
        if (creatureTypeMask & uint32((*i)->GetMiscValue()))
    ...
    // ..done (base at attack power for marked target and base at attack power
    // for creature type)
    int32 APbonus = 0;

    if (attType == RANGED_ATTACK) {
        APbonus += victim->GetTotalAuraModifier(
            SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS);

        // ..done (base at attack power and creature type)
        AuraEffectList const& mCreatureAttackPower =
            GetAuraEffectsByType(SPELL_AURA_MOD_RANGED_ATTACK_POWER_VERSUS);
        for (AuraEffectList::const_iterator i = mCreatureAttackPower.begin();
             i != mCreatureAttackPower.end();
             ++i)
            if (creatureTypeMask & uint32((*i)->GetMiscValue()))
                APbonus += (*i)->GetAmount();
    }
        ...
        // ..done (base at attack power and creature type)
        AuraEffectList const& mCreatureAttackPower =
            GetAuraEffectsByType(SPELL_AURA_MOD_MELEE_ATTACK_POWER_VERSUS);
        for (AuraEffectList::const_iterator i = mCreatureAttackPower.begin();
             i != mCreatureAttackPower.end();
             ++i)
            if (creatureTypeMask & uint32((*i)->GetMiscValue()))
    ...
    // mods for SPELL_SCHOOL_MASK_NORMAL are already factored in base melee
    // damage calculation
    if (!(damageSchoolMask & SPELL_SCHOOL_MASK_NORMAL)) {
        // Some spells don't benefit from pct done mods
        AuraEffectList const& mModDamagePercentDone =
        ...
        for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin();
             i != mModDamagePercentDone.end();
             ++i) {
            if (!spellProto || (spellProto->ValidateAttribute6SpellDamageMods(
                                    this, *i, false) &&
                ...
                if (((*i)->GetMiscValue() & damageSchoolMask)) {
                    if ((*i)->GetSpellInfo()->EquippedItemClass == -1)
                        ...
                        AddPct(DoneTotalMod, (*i)->GetAmount());
                             ...
                             ToPlayer()->HasItemFitToSpellRequirements(
                                 (*i)->GetSpellInfo()))
                        AddPct(DoneTotalMod, (*i)->GetAmount());
                }
            }
    ...
    for (AuraEffectList::const_iterator i = mDamageDoneVersus.begin();
        ...
        if (creatureTypeMask & uint32((*i)->GetMiscValue()))
            if (!spellProto ||
                spellProto->ValidateAttribute6SpellDamageMods(this, *i, false))
                AddPct(DoneTotalMod, (*i)->GetAmount());
...
uint32 Unit::MeleeDamageBonusTaken(
    Unit*            attacker,
    uint32           pdamage,
    WeaponAttackType attType,
    SpellInfo const* spellProto /*= nullptr*/,
    SpellSchoolMask  damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    TakenTotalMod *= GetTotalAuraMultiplierByMiscMask(
        SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, damageSchoolMask);
    ...
    if (spellProto) {
        ...
        // Shred, Maul - "Effects which increase Bleed damage also increase
        // Shred damage"
        if (spellProto->SpellFamilyName == SPELLFAMILY_DRUID &&
    ...
    /*AuraEffectList const& mclassScritAuras =
    GetAuraEffectsByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS); for
    (AuraEffectList::const_iterator i = mclassScritAuras.begin(); i !=
    mclassScritAuras.end(); ++i)
    {
        switch ((*i)->GetMiscValue())
        {
        }
    ...
    if (attType != RANGED_ATTACK) {
        ...
        for (AuraEffectList::const_iterator i =
             ...
             i != mModMeleeDamageTakenPercent.end();
             ++i)
            AddPct(TakenTotalMod, (*i)->GetAmount());
    }
    else {
        AuraEffectList const& mModRangedDamageTakenPercent =
            GetAuraEffectsByType(SPELL_AURA_MOD_RANGED_DAMAGE_TAKEN_PCT);
        for (AuraEffectList::const_iterator i =
                 mModRangedDamageTakenPercent.begin();
             i != mModRangedDamageTakenPercent.end();
             ++i)
            AddPct(TakenTotalMod, (*i)->GetAmount());
    }
    ...
    if (TakenTotalMod < 1.0f &&
        attacker->HasAuraType(SPELL_AURA_MOD_IGNORE_TARGET_RESIST)) {
        float                 ignoreModifier = 1.0f - TakenTotalMod;
        bool                  addModifier    = false;
        AuraEffectList const& ResIgnoreAuras =
            attacker->GetAuraEffectsByType(SPELL_AURA_MOD_IGNORE_TARGET_RESIST);
...
public:
    ...
    {
        return spellImmune.spellId == 0 && spellImmune.type == _type;
    }
...
void Unit::ApplySpellImmune(uint32               spellId,
                            ...
                            uint32               type,
                            bool                 apply,
                            SpellImmuneBlockType blockType)
{
    if (apply) {
        // xinef: immunities with spellId 0 are intended to be applied only once
        // (script purposes mosty)
        if (spellId == 0 && std::find_if(m_spellImmune[op].begin(),
                                         m_spellImmune[op].end(),
                                         spellIdImmunityPredicate(type)) !=
                                m_spellImmune[op].end())
            return;
...
float Unit::GetPPMProcChance(uint32           WeaponSpeed,
                             float            PPM,
                             SpellInfo const* spellProto) const
{
    ...
    return floor((WeaponSpeed * PPM) /
                 600.0f); // result is chance in percents (probability =
                          // Speed_in_sec * (PPM / 60))
}
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    if (Player* player = ToPlayer()) {
        ...
        if (pet) {
            ...
            // don't unsummon pet in arena but SetFlag UNIT_FLAG_STUNNED to
            // disable pet's interface
            if (bg && bg->isArena())
        ...
        WorldPacket data(SMSG_MOVE_SET_COLLISION_HGT,
                         GetPackGUID().size() + 4 + 4);
        data << GetPackGUID();
...
void Unit::Dismount()
{
    ...
    // this prevents adding a pet to a not created map which would otherwise
    // cause a crash (it could probably happen when logging in after a previous
    // crash)
    if (Player* player = ToPlayer()) {
        sScriptMgr->AnticheatSetUnderACKmount(player);

        if (Pet* pPet = player->GetPet()) {
            if (pPet->HasUnitFlag(UNIT_FLAG_STUNNED) &&
                !pPet->HasUnitState(UNIT_STATE_STUNNED))
        ...
        if (Unit* charm = player->GetCharm())
            if (charm->GetTypeId() == TYPEID_UNIT &&
                !charm->HasUnitState(UNIT_STATE_STUNNED))
...
void Unit::CombatStart(Unit* victim, bool initialAggro)
{
    ...
    if (initialAggro) {
        ...
        if (!victim->IsInCombat() && victim->GetTypeId() != TYPEID_PLAYER &&
            !victim->ToCreature()->HasReactState(REACT_PASSIVE) &&
            victim->ToCreature()->IsAIEnabled) {
            if (victim->IsPet())
                victim->ToCreature()->AI()->AttackedBy(
                    this); // PetAI has special handler before AttackStart()
            else {
                victim->ToCreature()->AI()->AttackStart(this);
                // if the target is an NPC with a pet or minion, pet should
                // react.
            ...
            if (Unit* victimOwner = victim->GetOwner()) {
                if (!(victimOwner->IsInCombatWith(this))) {
                    ...
                    if (!victimOwner->IsInCombat() &&
                        victimOwner->IsAIEnabled) {
    ...
    Player* player = GetCharmerOrOwnerPlayerOrPlayerItself();
    if (player && who->IsPvP() &&
        (who->GetTypeId() != TYPEID_PLAYER || !player->duel ||
         player->duel->Opponent != who)) {
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, uint32 duration)
{
    ...
    if (PvP)
        m_CombatTimer = std::max<uint32>(GetCombatTimer(),
                                         std::max<uint32>(5500, duration));
    else if (duration)
        m_CombatTimer = std::max<uint32>(GetCombatTimer(), duration);

    if (HasUnitState(UNIT_STATE_EVADE) ||
        GetCreatureType() == CREATURE_TYPE_NON_COMBAT_PET)
        return;

    // xinef: if we somehow engage in combat (scripts, dunno) with player,
    // remove this flag so he can fight back
    if (GetTypeId() == TYPEID_UNIT && enemy && IsImmuneToPC() &&
        enemy->GetCharmerOrOwnerPlayerOrPlayerItself())
        SetImmuneToPC(false); // unit has engaged in combat, remove immunity so
    ...
    if (Creature* creature = ToCreature()) {
        ...
        if ((IsAIEnabled && creature->AI()->IsEscorted()) ||
            ...
            GetMotionMaster()->GetCurrentMovementGeneratorType() ==
                ESCORT_MOTION_TYPE)
            creature->SetHomePosition(GetPositionX(),
                                      GetPositionY(),
                                      GetPositionZ(),
                                      GetOrientation());
        ...
        if (!(creature->GetCreatureTemplate()->type_flags &
              CREATURE_TYPE_FLAG_ALLOW_MOUNTED_COMBAT))
            Dismount();
        if (!IsStandState()) // pussywizard: already done in CombatStart(target,
                             // initialAggro) for the target, but when aggro'ing
                             // from MoveInLOS CombatStart is not called!
    ...
    for (Unit::ControlSet::iterator itr = m_Controlled.begin();
        ...
        // Xinef: Dont set combat for passive units, they will evade in next
        // update...
        if (controlled->GetTypeId() == TYPEID_UNIT &&
            controlled->ToCreature()->HasReactState(REACT_PASSIVE))
            continue;
...
void Unit::ClearInCombat()
{
    ...
    if (Creature* creature = ToCreature()) {
        if (creature->GetCreatureTemplate() &&
            creature->GetCreatureTemplate()->unit_flags &
                UNIT_FLAG_IMMUNE_TO_PC)
            SetImmuneToPC(
                true); // set immunity state to the one from db on evade

        ClearUnitState(UNIT_STATE_ATTACK_PLAYER);
        if (HasDynamicFlag(UNIT_DYNFLAG_TAPPED))
            ReplaceAllDynamicFlags(
                creature->GetCreatureTemplate()->dynamicflags);

        creature->SetAssistanceTimer(0);

        // Xinef: will be recalculated at follow movement generator
        // initialization
        if (!IsPet() && !IsCharmed())
            return;
    }
...
bool Unit::isTargetableForAttack(bool checkFakeDeath, Unit const* byWho) const
{
    ...
    if (IsImmuneToPC() && byWho &&
        byWho->GetCharmerOrOwnerPlayerOrPlayerItself())
        return false;
    ...
    return !HasUnitState(UNIT_STATE_UNATTACKABLE) &&
           (!checkFakeDeath || !HasUnitState(UNIT_STATE_DIED));
}

bool Unit::IsValidAttackTarget(Unit const*      target,
                               SpellInfo const* bySpell) const
{
                                ...
                                WorldObject const* obj) const
{
    ...
    if (m_vehicle)
        if (IsOnVehicle(target) || m_vehicle->GetBase()->IsOnVehicle(target))
            if (!IsHostileTo(
                    target)) // pussywizard: actually can attack own vehicle or
                             // passenger if it's hostile to us - needed for
                             // snobold in Gormok encounter
                return false;

    // can't attack invisible (ignore stealth for aoe spells) also if the area
    // being looked at is from a spell use the dynamic object created instead of
    // the casting unit.
    ...
    if ((!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_IGNORE_PHASE_SHIFT)) &&
        (obj ? !obj->CanSeeOrDetect(target,
                                    bySpell && bySpell->IsAffectingArea())
             : !CanSeeOrDetect(target,
                               (bySpell && bySpell->IsAffectingArea()) ||
                                   (target->GetTypeId() == TYPEID_PLAYER &&
                                    target->HasStealthAura() &&
                                    target->IsInCombat() &&
                                    IsInCombatWith(target)))))
        return false;
    ...
    if ((!bySpell ||
         !bySpell->HasAttribute(SPELL_ATTR6_CAN_TARGET_UNTARGETABLE)) &&
    ...
    // check flags
    if (target->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT |
                            UNIT_FLAG_NOT_ATTACKABLE_1 |
                            UNIT_FLAG_NON_ATTACKABLE_2) ||
        (!HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) &&
        ...
        // check if this is a world trigger cast - GOs are using world triggers
        // to cast their spells, so we need to ignore their immunity flag here,
        // this is a temp workaround, needs removal when go cast is implemented
        // properly
        || ((GetEntry() != WORLD_TRIGGER &&
             (!obj ||
              !obj->isType(TYPEMASK_GAMEOBJECT | TYPEMASK_DYNAMICOBJECT))) &&
            target->HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) && IsImmuneToPC()))
        return false;
    ...
    if (!HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) &&
        ...
        return GetReactionTo(target) <= REP_HOSTILE ||
               target->GetReactionTo(this) <= REP_HOSTILE;
    ...
    // Not all neutral creatures can be attacked (even some unfriendly faction
    // does not react aggresive to you, like Sporaggar)
    if (repThisToTarget == REP_NEUTRAL && repTargetToThis <= REP_NEUTRAL) {
        ...
        if (!(target->GetTypeId() == TYPEID_PLAYER &&
              thisUnit->GetTypeId() == TYPEID_PLAYER) &&
            !(target->GetTypeId() == TYPEID_UNIT &&
              thisUnit->GetTypeId() == TYPEID_UNIT)) {
            Player const* player = target->GetTypeId() == TYPEID_PLAYER
                                       ? target->ToPlayer()
                                       : thisUnit->ToPlayer();
            Unit const*   creature =
                target->GetTypeId() == TYPEID_UNIT ? target : thisUnit;

            if (FactionTemplateEntry const* factionTemplate =
                    creature->GetFactionTemplateEntry()) {
                if (!(player->GetReputationMgr().GetForcedRankIfAny(
                        factionTemplate)))
                    if (FactionEntry const* factionEntry =
                            sFactionStore.LookupEntry(factionTemplate->faction))
                        if (FactionState const* repState =
                                player->GetReputationMgr().GetState(
                                    factionEntry))
    ...
    if (creatureAttacker &&
        creatureAttacker->GetCreatureTemplate()->type_flags &
            CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT)
        return false;

    Player const* playerAffectingAttacker =
        HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) ? GetAffectingPlayer()
                                                 : nullptr;
    Player const* playerAffectingTarget =
        target->HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED)
            ? target->GetAffectingPlayer()
            : nullptr;

    // check duel - before sanctuary checks
    if (playerAffectingAttacker && playerAffectingTarget)
        if (playerAffectingAttacker->duel &&
            playerAffectingAttacker->duel->Opponent == playerAffectingTarget &&
            playerAffectingAttacker->duel->State == DUEL_STATE_IN_PROGRESS)
            return true;

    // PvP case - can't attack when attacker or target are in sanctuary
    // however, 13850 client doesn't allow to attack when one of the unit's has
    // sanctuary flag and is pvp
    if (target->HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) &&
        HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) &&
    ...
    if (playerAffectingAttacker && playerAffectingTarget) {
        if (!IsPvP() && bySpell && bySpell->IsAffectingArea() &&
            !bySpell->HasAttribute(SPELL_ATTR5_IGNORE_AREA_EFFECT_PVP_CHECK))
            return false;
                                ...
                                SpellInfo const* bySpell) const
{
    ...
    if (GetReactionTo(target) < REP_NEUTRAL &&
        ...
        (!ToCreature() || !(ToCreature()->GetCreatureTemplate()->type_flags &
                            CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT)))
        return false;
    ...
    if (target->HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED)) {
        ...
        if (HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED)) {
            ...
            // can't assist player out of sanctuary from sanctuary if has pvp
            // enabled
            if (target->IsPvP())
    ...
    // !target->HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) &&
        ...
        if (Creature const* creatureTarget = target->ToCreature())
            return creatureTarget->GetCreatureTemplate()->type_flags &
                       CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT ||
                   creatureTarget->GetCreatureTemplate()->type_flags &
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    case MOVE_RUN: {
        if (IsMounted()) // Use on mount auras
        {
            ...
            non_stack_bonus += GetMaxPositiveAuraModifier(
    ...
    case MOVE_SWIM: {
        ...
        Unit::AuraEffectList const& swimAuras =
            GetAuraEffectsByType(SPELL_AURA_MOD_INCREASE_SWIM_SPEED);
        for (Unit::AuraEffectList::const_iterator itr = swimAuras.begin();
             itr != swimAuras.end();
             ++itr) {
            // xinef: sea turtle only, it is not affected by any increasing /
            // decreasing effects
            if ((*itr)->GetId() == 64731 /*SPELL_SEA_TURTLE*/) {
                SetSpeed(mtype,
                         AddPct(non_stack_bonus, (*itr)->GetAmount()),
                         forced);
                return;
            }
            else if (
    ...
    case MOVE_FLIGHT: {
            ...
            IsControlledByPlayer()) // not sure if good for pet
        {
            ...
            stack_bonus =
                GetTotalAuraMultiplier(SPELL_AURA_MOD_VEHICLE_SPEED_ALWAYS);
            ...
            main_speed_mod =
                GetTotalAuraModifier(SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED) +
                GetTotalAuraModifier(
                    SPELL_AURA_MOD_INCREASE_VEHICLE_FLIGHT_SPEED);

        non_stack_bonus +=
            GetMaxPositiveAuraModifier(SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACK) /
            100.0f;
    ...
    default:
        LOG_ERROR("entities.unit",
                  "Unit::UpdateSpeed: Unsupported move type ({})",
                  mtype);
        return;
    }
    ...
    case MOVE_FLIGHT: {
        ...
        if (GetTypeId() == TYPEID_UNIT) {
            if (IsPet() && ToPet()->isControlled() && IsControlledByPlayer()) {
                ...
                speed *= ToCreature()
                             ->GetCreatureTemplate()
                             ->speed_run; // at this point, MOVE_WALK is never
                                          // reached
            }
        }

        // Normalize speed by 191 aura SPELL_AURA_USE_NORMAL_MOVEMENT_SPEED if
        // need
        ...
        if (int32 normalization = GetMaxPositiveAuraModifier(
            ...
            if (Creature* creature = ToCreature()) {
                uint32 immuneMask =
                    creature->GetCreatureTemplate()->MechanicImmuneMask;
                if (immuneMask & (1 << (MECHANIC_SNARE - 1)) ||
                    immuneMask & (1 << (MECHANIC_DAZE - 1)))
                    break;
            }

            // Use speed from aura
            float max_speed = normalization / (IsControlledByPlayer()
    ...
    if (creature && !IsPet() && !(IsControlledByPlayer() && IsVehicle()) &&
        ...
        slowFromHealth =
            (int32)std::min(0.0f, (1.66f * (GetHealthPct() - 30.0f)));
    }
    ...
    if (float minSpeedMod =
            (float)GetMaxPositiveAuraModifier(SPELL_AURA_MOD_MINIMUM_SPEED)) {
        float base_speed = (GetTypeId() == TYPEID_UNIT
                                ? ToCreature()->GetCreatureTemplate()->speed_run
                                : 1.0f);
...
{
    return m_speed_rate[mtype] * (IsControlledByPlayer()
                                      ? playerBaseMoveSpeed[mtype]
                                      : baseMoveSpeed[mtype]);
}
...
void Unit::SetSpeed(UnitMoveType mtype, float rate, bool forced)
{
    ...
    if (!forced) {
        ...
        default:
            LOG_ERROR("entities.unit",
                      "Unit::SetSpeed: Unsupported move type ({}), data not "
                      "sent to client.",
                      mtype);
            return;
        }
        ...
        if (GetTypeId() == TYPEID_PLAYER) {
            ...
            // WorldSession::HandleForceSpeedChangeAck and do it only for real
            // sent packets and use run for run/mounted as client expected
            ...
            if (!IsInCombat()) {
                ...
                if (pet &&
                    (pet->HasUnitFlag(UNIT_FLAG_POSSESSED) || pet->IsVehicle()))
                    pet = nullptr;

                if (pet && pet->GetTypeId() == TYPEID_UNIT &&
                    !pet->IsInCombat() &&
                    pet->GetMotionMaster()->GetCurrentMovementGeneratorType() ==
                        FOLLOW_MOTION_TYPE)
                    pet->UpdateSpeed(mtype, forced);
                if (Unit* critter =
                        ObjectAccessor::GetUnit(*this, GetCritterGUID()))
...
{
    // death state needs to be updated before RemoveAllAurasOnDeath() calls
    // HandleChannelDeathItem(..) so that it can be used to check creation of
    // death items (such as soul shards).

    if (s != DeathState::Alive && s != DeathState::JustRespawned) {
        CombatStop();
        GetThreatMgr().ClearAllThreat();
        getHostileRefMgr().deleteReferences();
        ClearComboPointHolders(); // any combo points pointed to unit lost at it
                                  // death
    ...
    if (s == DeathState::JustDied) {
        ...
        // without this when removing IncreaseMaxHealth aura player may stuck
        // with 1 hp do not why since in IncreaseMaxHealth currenthealth is
        // checked
        SetHealth(0);
        ...
        // players in instance don't have ZoneScript, but they have
        // InstanceScript
        if (ZoneScript* zoneScript = GetZoneScript()
                                         ? GetZoneScript()
                                         : (ZoneScript*)GetInstanceScript())
            zoneScript->OnUnitDeath(this);
    }
    else if (s == DeathState::JustRespawned) {
        RemoveFlag(UNIT_FIELD_FLAGS,
                   UNIT_FLAG_SKINNABLE); // clear skinnable for creature and
                                         // player (at battleground)
    }
...
bool Unit::CanHaveThreatList() const
{
    ...
    // summons can not have a threat list, unless they are controlled by a
    // creature
    if (HasUnitTypeMask(UNIT_MASK_MINION | UNIT_MASK_GUARDIAN |
                        UNIT_MASK_CONTROLABLE_GUARDIAN) &&
        ((Pet*)this)->GetOwnerGUID().IsPlayer())
        return false;
    ...
    if (m_ThreatMgr.isThreatListEmpty()) {
        if (creature->IsAIEnabled)
            creature->AI()->EnterEvadeMode(
                CreatureAI::EVADE_REASON_NO_HOSTILES);
        return;
    }
...
Unit* Creature::SelectVictim()
{
    ...
    AuraEffectList const& tauntAuras =
        GetAuraEffectsByType(SPELL_AURA_MOD_TAUNT);
    if (!tauntAuras.empty())
        for (Unit::AuraEffectList::const_reverse_iterator itr =
                 tauntAuras.rbegin();
             itr != tauntAuras.rend();
             ++itr)
            if (Unit* caster = (*itr)->GetCaster())
                if (CanCreatureAttack(caster) &&
                    !caster->HasAuraTypeWithCaster(SPELL_AURA_IGNORED,
                                                   GetGUID())) {
                    target = caster;
                    break;
                }
    ...
    if (CanHaveThreatList()) {
        ...
        if (!target && IsSummon())
            if (Unit* owner = ToTempSummon()->GetOwner()) {
                ...
                if (!target)
                    for (ControlSet::const_iterator itr =
    ...
    // it in combat but attacker not make any damage and not enter to aggro
    // radius to have record in threat list Note: creature does not have
    // targeted movement generator but has attacker in this case
    ...
    Unit::AuraEffectList const& iAuras =
        GetAuraEffectsByType(SPELL_AURA_MOD_INVISIBILITY);
    if (!iAuras.empty()) {
...
{
    return spellProto->Effects[effect_index].CalcValue(
...
int32 Unit::CalcSpellDuration(SpellInfo const* spellProto)
{
    ...
    if (comboPoints && minduration != -1 && minduration != maxduration)
        duration =
            minduration + int32((maxduration - minduration) * comboPoints / 5);
    else
                             ...
                             uint32           effectMask)
{
    ...
    if (!positive) {
        int32 mechanic =
            spellProto->GetSpellMechanicMaskByEffectMask(effectMask);
        ...
        for (uint8 i = 1; i <= MECHANIC_ENRAGED; ++i) {
            ...
            // Xinef: spells affecting movement imparing effects should not
            // reduce duration if disoriented mechanic is present
        ...
        // when there will be, change GetTotalAuraModifierByMiscValue to
        // GetTotalPositiveAuraModifierByMiscValue
    }
...
void Unit::ModSpellCastTime(SpellInfo const* spellInfo,
                            int32&           castTime,
                            Spell*           spell)
{
    ...
    if (Player* modOwner = GetSpellModOwner())
        ...
        modOwner->ApplySpellMod(spellInfo->Id,
                                ...
                                castTime,
                                spell,
                                bool(modOwner != this && !IsPet()));

    switch (spellInfo->DmgClass) {
    case SPELL_DAMAGE_CLASS_NONE:
        if (spellInfo->AttributesEx5 &
            SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC) // required double check
            castTime =
                int32(float(castTime) * GetFloatValue(UNIT_MOD_CAST_SPEED));
        else if (spellInfo->SpellVisual[0] == 3881 &&
                 HasAura(67556)) // cooking with Chef Hat.
            castTime = 500;
        break;
    case SPELL_DAMAGE_CLASS_MELEE:
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group)
{
    for (Diminishing::iterator i = m_Diminishing.begin();
        ...
        if (i->stack == 0 &&
            getMSTimeDiff(i->hitTime, GameTime::GetGameTimeMS().count()) >
                15000) {
...
void Unit::IncrDiminishing(DiminishingGroup group)
{
    ...
    m_Diminishing.push_back(DiminishingReturn(
        group, GameTime::GetGameTimeMS().count(), DIMINISHING_LEVEL_2));
}
                                       ...
                                       int32             limitduration)
{
    ...
    // Duration of crowd control abilities on pvp target is limited by 10 sec.
    // (2.2.0)
    if (limitduration > 0 && duration > limitduration) {
        ...
        if ((target->GetTypeId() == TYPEID_PLAYER ||
             target->ToCreature()->GetCreatureTemplate()->flags_extra &
    ...
    // Some diminishings applies to mobs too (for example, Stun)
              ...
              ((targetOwner ? (targetOwner->GetTypeId() == TYPEID_PLAYER)
                            : (GetTypeId() == TYPEID_PLAYER)) ||
               (GetTypeId() == TYPEID_UNIT &&
                ToCreature()->GetCreatureTemplate()->flags_extra &
                    CREATURE_FLAG_EXTRA_ALL_DIMINISH))) ||
                                      ...
                                      SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo->RangeEntry->RangeMax[1] ==
        spellInfo->RangeEntry->RangeMax[0]) {
                                      ...
                                      SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo->RangeEntry->RangeMin[1] ==
        spellInfo->RangeEntry->RangeMin[0]) {
...
uint32 Unit::GetCreatureType() const
{
    if (GetTypeId() == TYPEID_PLAYER) {
        ...
        SpellShapeshiftEntry const* ssEntry =
            sSpellShapeshiftStore.LookupEntry(form);
                              ...
                              bool             apply)
{
    if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_END) {
        LOG_ERROR("entities.unit",
                  "ERROR in HandleStatModifier(): non-existing UnitMods or "
                  "wrong UnitModifierType!");
        return false;
    }
...
{
    UnitMods unitMod =
        UnitMods(static_cast<uint16>(UNIT_MOD_STAT_START) + stat);
...
float Unit::GetTotalAuraModValue(UnitMods unitMod) const
{
    if (unitMod >= UNIT_MOD_END) {
        LOG_ERROR("entities.unit",
                  "attempt to access non-existing UnitMods in "
...
void Unit::ApplyStatPercentBuffMod(Stats stat, float val, bool apply)
{
    ...
    ApplyModSignedFloatValue(
        (var > 0 ? static_cast<uint16>(UNIT_FIELD_POSSTAT0) + stat
                 : static_cast<uint16>(UNIT_FIELD_NEGSTAT0) + stat),
        var,
        apply);
}
                                     ...
                                     Unit*            victim) const
{
    if (attType == RANGED_ATTACK) {
        ...
        if (victim)
            ap += victim->GetTotalAuraModifier(
                SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS);
...
void Unit::SetHealth(uint32 val)
{
    ...
    if (GetTypeId() == TYPEID_UNIT &&
        !(IsPet() && ToPet()->isControlled() && IsControlledByPlayer()) &&
        (prevHealthPct < 30.0 || HealthBelowPct(30))) {
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
        ...
        if (getPowerType() == power && player->NeedSendSpectatorData()) {
            ArenaSpectator::SendCommand_UInt32Value(
                ...
                GetGUID(),
                "CPW",
                power == POWER_RAGE || power == POWER_RUNIC_POWER ? val / 10
                                                                  : val);
        }
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld()) {
        ...
        if (GetCharmerGUID()) {
            LOG_FATAL("entities.unit",
                      "Unit {} has charmer guid when removed from world",
                      GetEntry());
            ABORT();
        }
        ...
        if (Unit* owner = GetOwner()) {
            if (owner->m_Controlled.find(this) != owner->m_Controlled.end()) {
                ...
                LOG_INFO("entities.unit",
                         "Unit {} is in controlled list of {} when removed "
                         "from world",
                         GetEntry(),
                         owner->GetEntry());
                // ABORT();
...
void Unit::CleanupBeforeRemoveFromMap(bool finalCleanup)
{
    ...
    // This needs to be before RemoveFromWorld to make GetCaster() return a
    // valid pointer on aura removal
    ...
    m_Events.KillAllEvents(
        false); // non-delatable (currently casted spells) will not deleted now
                // but it will deleted at call in
    ...
    GetMotionMaster()->Clear(
        false); // remove different non-standard movement generators.
}
...
CharmInfo::CharmInfo(Unit* unit)
    : _unit(unit), _CommandState(COMMAND_FOLLOW), _petnumber(0),
      _oldReactState(REACT_PASSIVE), _isCommandAttack(false),
      _isCommandFollow(false), _isAtStay(false), _isFollowing(false),
      _isReturning(false), _forcedSpellId(0), _stayX(0.0f), _stayY(0.0f),
      _stayZ(0.0f)
{
...
void CharmInfo::InitPetActionBar()
{
    ...
    for (uint32 i = 0;
         i < ACTION_BAR_INDEX_PET_SPELL_START - ACTION_BAR_INDEX_START;
         ++i)
        SetActionBar(
    ...
    for (uint32 i = 0;
         i < ACTION_BAR_INDEX_END - ACTION_BAR_INDEX_PET_SPELL_END;
         ++i)
        SetActionBar(ACTION_BAR_INDEX_PET_SPELL_END + i,
                     COMMAND_ATTACK - i,
                     ACT_REACTION);
}
...
void CharmInfo::InitPossessCreateSpells()
{
    if (_unit->GetTypeId() == TYPEID_UNIT) {
        ...
        // Adding entrys to this switch will prevent COMMAND_ATTACK being added
        // to pet bar.
                                    ...
                                    ActiveStates     newstate)
{
    ...
    for (uint8 i = 0; i < MAX_UNIT_ACTION_BAR_INDEX; ++i) {
        if (!PetActionBar[i].GetAction() &&
            ...
            SetActionBar(i,
                         spell_id,
                         newstate == ACT_DECIDE ? spellInfo->IsAutocastable()
                                                      ? ACT_DISABLED
                                                      : ACT_PASSIVE
                                                : newstate);
            ...
            if (_unit->GetCharmer() && _unit->GetCharmer()->IsPlayer()) {
                if (Creature* creature = _unit->ToCreature()) {
                    ...
                    _unit->m_Events.AddEventAtOffset(
                        [creature, spell_id]() {
                            if (uint32 cooldown =
                                    creature->GetSpellCooldown(spell_id)) {
                                WorldPacket data;
                                creature->BuildCooldownPacket(
                                    data,
                                    SPELL_COOLDOWN_FLAG_NONE,
                                    spell_id,
                                    cooldown);
                                if (creature->GetCharmer() &&
                                    creature->GetCharmer()->IsPlayer()) {
                                    creature->GetCharmer()
                                        ->ToPlayer()
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    for (uint8 index = ACTION_BAR_INDEX_START; index < ACTION_BAR_INDEX_END;
         ++index) {
        ...
        PetActionBar[index].SetActionAndType(*action,
                                             static_cast<ActiveStates>(*type));

        // check correctness
        if (PetActionBar[index].IsActionBarForSpell()) {
            SpellInfo const* spellInfo =
                sSpellMgr->GetSpellInfo(PetActionBar[index].GetAction());
            if (!spellInfo) {
...
bool InitTriggerAuraData()
{
    ...
    isTriggerAura[SPELL_AURA_MOD_STUN] =
        true; // Aura does not have charges but needs to be removed on trigger
    isTriggerAura[SPELL_AURA_MOD_DAMAGE_DONE]  = true;
    isTriggerAura[SPELL_AURA_MOD_DAMAGE_TAKEN] = true;
    isTriggerAura[SPELL_AURA_MOD_RESISTANCE]   = true;
    isTriggerAura[SPELL_AURA_MOD_STEALTH]      = true;
    isTriggerAura[SPELL_AURA_MOD_FEAR] =
        true; // Aura does not have charges but needs to be removed on trigger
        ...
        default:
            if (spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
                ...
                spellInfo->HasAttribute(
                    SPELL_ATTR2_AUTO_REPEAT)) // Wands auto attack
            {
                procAttacker = PROC_FLAG_DONE_RANGED_AUTO_ATTACK;
                procVictim   = PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK;
            }
                                 ...
                                 uint32           procPhase)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && target &&
        ...
        // only when you are not fighting other players or their pets/totems
        // (pvp)
        if (IsPlayer() && !target->IsCharmedOwnedByPlayerOrPlayer()) {
            // On melee based hit/miss/resist/parry/dodge need to update skill
            // (for victim and attacker)
            if (procExtra & (PROC_EX_NORMAL_HIT | PROC_EX_MISS |
                             PROC_EX_RESIST | PROC_EX_PARRY | PROC_EX_DODGE)) {
                ToPlayer()->UpdateCombatSkills(
                    target,
                    attType,
                    isVictim,
                    procSpell ? procSpell->m_weaponItem : nullptr);
            }
            // Update defence if player is victim and we block - TODO: confirm
            // that blocked attacks only have a chance to increase defence skill
            else if (isVictim && procExtra & (PROC_EX_BLOCK)) {
                ToPlayer()->UpdateCombatSkills(target, attType, true);
            }
        }
        // If exist crit/parry/dodge/block need update aura state (for victim
        // and attacker)
        if (procExtra & (PROC_EX_CRITICAL_HIT | PROC_EX_PARRY | PROC_EX_DODGE |
            ...
            if (isVictim) {
                ...
                if (procExtra & PROC_EX_DODGE) {
                    ...
                    if (!IsClass(CLASS_ROGUE,
                                 CLASS_CONTEXT_ABILITY_REACTIVE)) // skip Rogue
                                                                  // Riposte
                    {
    ...
    ProcEventInfo eventInfo = ProcEventInfo(actor,
                                            actionTarget,
                                            target,
                                            procFlag,
                                            0,
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin();
        ...
        if (itr->second->GetBase()->GetCastItemGUID() &&
            HasSpellItemCooldown(itr->first, uint32(-1)))
            continue;

        ProcTriggeredData triggerData(itr->second->GetBase());
        // Defensive procs are active on absorbs (so absorption effects are not
        // a hindrance)
        ...
        // only auras that have trigger spell should proc from fully absorbed
        // damage
        if (procExtra & PROC_EX_ABSORB && isVictim)
            if (damage || spellProto->Effects[EFFECT_0].TriggerSpell ||
                spellProto->Effects[EFFECT_1].TriggerSpell ||
                spellProto->Effects[EFFECT_2].TriggerSpell)
                active = true;

        // xinef: fix spell procing from damaging / healing casts if spell has
        // DoT / HoT effect only only player spells are taken into account
        if (!active && !isVictim && !(procFlag & PROC_FLAG_DONE_PERIODIC) &&
            procSpellInfo && procSpellInfo->SpellFamilyName &&
            (procSpellInfo->HasAura(SPELL_AURA_PERIODIC_DAMAGE) ||
             procSpellInfo->HasAura(SPELL_AURA_PERIODIC_HEAL)))
            active = true;

        // AuraScript Hook
        if (!triggerData.aura->CallScriptCheckProcHandlers(itr->second,
                                                           eventInfo)) {
            continue;
        }

        bool isTriggeredAtSpellProcEvent =
            IsTriggeredAtSpellProcEvent(target,
                                        triggerData.aura,
                                        attType,
                                        isVictim,
                                        active,
        ...
        // AuraScript Hook
        if (!triggerData.aura->CallScriptAfterCheckProcHandlers(
                itr->second, eventInfo, isTriggeredAtSpellProcEvent)) {
            continue;
        }
        ...
        ConditionList conditions =
            sConditionMgr->GetConditionsForNotGroupedEntry(
                CONDITION_SOURCE_TYPE_SPELL_PROC, spellProto->Id);
        ConditionSourceInfo condInfo = ConditionSourceInfo(
            eventInfo.GetActor(), eventInfo.GetActionTarget());
        ...
        // bool triggered =
        // !spellProto->HasAttribute(SPELL_ATTR3_CAN_PROC_FROM_PROCS) ?
        //    (procExtra & PROC_EX_INTERNAL_TRIGGERED && !(procFlag &
        //    PROC_FLAG_DONE_TRAP_ACTIVATION)) : false;
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
            if (itr->second->HasEffect(i)) {
                ...
                // If not trigger by default and spellProcEvent == nullptr -
                // skip
                ...
                case SPELL_AURA_PROC_TRIGGER_SPELL_WITH_VALUE:
                    if (uint32 triggerSpellId =
                            aurEff->GetSpellInfo()->Effects[i].TriggerSpell) {
        ...
        if (triggerData.effMask) {
            // If there is aura that triggers another proc aura, make sure that
            // the triggered one is going to be proccessed on top of it
            if (hasTriggeredProc) {
                bool proccessed = false;
                for (uint8 i = 0; i < EFFECT_ALL; ++i) {
                    if (uint32 triggeredSpellId =
                            triggerData.triggerSpelId[i]) {
                        auto iter = std::find(procTriggered.begin(),
                                              procTriggered.end(),
                                              triggeredSpellId);
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin();
         i != procTriggered.end();
         ++i) {
        // look for aura in auras list, it may be removed while proc event
        // processing
        if (i->aura->IsRemoved())
            continue;
        ...
        bool prepare =
            i->aura->CallScriptPrepareProcHandlers(aurApp, eventInfo);
        ...
        if (!handled && HandleAuraProc(target,
                                       damage,
                                       i->aura,
                                       procSpellInfo,
                                       procFlag,
            ...
            LOG_DEBUG("spells.aura",
                      "ProcDamageAndSpell: casting spell {} (triggered with "
                      "value by {} aura of spell {})",
                      spellInfo->Id,
        ...
        if (!handled)
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
                ...
                bool prevented = i->aura->CallScriptEffectProcHandlers(
                    triggeredByAura, aurApp, eventInfo);
                if (prevented) {
                    takeCharges = true;
                    continue;
                }
                ...
                case SPELL_AURA_PROC_TRIGGER_SPELL: {
                    LOG_DEBUG("spells.aura",
                              "ProcDamageAndSpell: casting spell {} (triggered "
                              "by {} aura of spell {})",
                              spellInfo->Id,
                              (isVictim ? "a victim's" : "an attacker's"),
                              triggeredByAura->GetId());
                    // Don`t drop charge or add cooldown for not started trigger
                    if (HandleProcTriggerSpell(target,
                                               damage,
                                               triggeredByAura,
                                               procSpellInfo,
                                               procFlag,
                ...
                case SPELL_AURA_PROC_TRIGGER_DAMAGE: {
                    ...
                    triggeredByAura->HandleProcTriggerDamageAuraProc(
                        aurApp, eventInfo); // this function is part of the new
                                            // proc system
                    takeCharges = true;
                    break;
                }
                ...
                case SPELL_AURA_DUMMY: {
                    LOG_DEBUG("spells.aura",
                              "ProcDamageAndSpell: casting spell id {} "
                              "(triggered by {} dummy aura of spell {})",
                              spellInfo->Id,
                              (isVictim ? "a victim's" : "an attacker's"),
                              triggeredByAura->GetId());
                    if (HandleDummyAuraProc(target,
                                            damage,
                                            triggeredByAura,
                                            procSpellInfo,
                                            procFlag,
                ...
                case SPELL_AURA_OVERRIDE_CLASS_SCRIPTS: {
                    LOG_DEBUG("spells.aura",
                              ...
                              (isVictim ? "a victim's" : "an attacker's"),
                              triggeredByAura->GetId());
                    if (HandleOverrideClassScriptAuraProc(target,
                                                          damage,
                                                          triggeredByAura,
                                                          procSpellInfo,
                                                          cooldown))
                ...
                case SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE: {
                    LOG_DEBUG("spells.aura",
                              "ProcDamageAndSpell: casting mending (triggered "
                              "by {} dummy aura of spell {})",
                              (isVictim ? "a victim's" : "an attacker's"),
                              triggeredByAura->GetId());
                    if (damage > 0) {
                ...
                case SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK:
                    ...
                    // xinef: check channeled spells which are affected by haste
                    // also
                ...
                case SPELL_AURA_REFLECT_SPELLS_SCHOOL:
                    ...
                    if (procSpellInfo &&
                        (triggeredByAura->GetMiscValue() &
                         procSpellInfo->SchoolMask)) // School check
                        takeCharges = true;
                    break;
                case SPELL_AURA_SPELL_MAGNET:
                    // Skip Melee hits and targets with magnet aura
                    if (procSpellInfo &&
                        (triggeredByAura->GetBase()->GetUnitOwner()->ToUnit() ==
                         ToUnit())) // Magnet
                        takeCharges = true;
                    break;
                ...
                case SPELL_AURA_MOD_POWER_COST_SCHOOL:
                    ...
                    if (procSpellInfo &&
                        (procSpellInfo->ManaCost != 0 ||
                         procSpellInfo->ManaCostPercentage != 0 ||
                         (procSpellInfo->SpellFamilyFlags[1] &
                          0x2)) && // Cost check, mutilate include
                        (triggeredByAura->GetMiscValue() &
                         procSpellInfo->SchoolMask)) // School check
                        takeCharges = true;
                    break;
                ...
                case SPELL_AURA_MOD_DAMAGE_FROM_CASTER:
                    // Compare casters
                    if (target &&
                        triggeredByAura->GetCasterGUID() == target->GetGUID())
                        takeCharges = true;
                    break;
                ...
                case SPELL_AURA_TRANSFORM: {
                    ...
                    // Xinef: Or when the aura is at full duration (assume that
                    // such auras should be added at the end, skipping all
                    // damage procs etc.)
                    if (procSpellInfo)
                        if ((!i->aura->IsPermanent() &&
                             i->aura->GetDuration() ==
                                 i->aura->GetMaxDuration()) ||
                            procSpellInfo->Id == triggeredByAura->GetId() ||
                            procSpellInfo->HasAttribute(
                    ...
                    if (useCharges)
                        takeCharges = true;
                                 ...
                                 ->GetAmount()) // aura must have amount
                    {
                ...
                case SPELL_AURA_ABILITY_IGNORE_AURASTATE:
                    if (procSpellInfo &&
                        procSpellInfo->Id ==
                            20647) // hack for warriors execute, both dummy and
                                   // damage spell are affected by ignore
                                   // aurastate aura
                        break;
                    takeCharges = true;
                    break;
                ...
                case SPELL_AURA_ADD_PCT_MODIFIER: {
                    if (triggeredByAura->GetSpellModifier()) {
                        ...
                        if (!procSpell ||
                            procSpell->m_appliedMods.find(i->aura) !=
                                procSpell->m_appliedMods.end()) {
                            takeCharges = true;
                        }
                    }
                    break;
...
void Unit::GetProcAurasTriggeredOnEvent(
    std::list<AuraApplication*>& aurasTriggeringProc,
    std::list<AuraApplication*>* procAuras,
    ProcEventInfo                eventInfo)
{
    // use provided list of auras which can proc
    if (procAuras) {
        for (std::list<AuraApplication*>::iterator itr = procAuras->begin();
             itr != procAuras->end();
             ++itr) {
            ASSERT((*itr)->GetTarget() == this);
            if (!(*itr)->GetRemoveMode())
                if ((*itr)->GetBase()->IsProcTriggeredOnEvent(*itr,
                                                              eventInfo)) {
    ...
    // or generate one on our own
        ...
        for (AuraApplicationMap::iterator itr = GetAppliedAuras().begin();
             itr != GetAppliedAuras().end();
             ++itr) {
            if (itr->second->GetBase()->IsProcTriggeredOnEvent(itr->second,
                                                               eventInfo)) {
                itr->second->GetBase()->PrepareProcToTrigger(itr->second,
                                                             eventInfo);
...
void Unit::TriggerAurasProcOnEvent(CalcDamageInfo& damageInfo)
{
    ...
    TriggerAurasProcOnEvent(nullptr,
                            nullptr,
                            damageInfo.target,
                            damageInfo.procAttacker,
                            damageInfo.procVictim,
                                   ...
                                   HealInfo*   healInfo)
{
    ...
    ProcEventInfo               myProcEventInfo = ProcEventInfo(this,
                                                  actionTarget,
                                                  actionTarget,
                                                  typeMaskActor,
                                                  spellTypeMask,
    ...
    GetProcAurasTriggeredOnEvent(
        myAurasTriggeringProc, myProcAuras, myProcEventInfo);

    // prepare data for target trigger
    ProcEventInfo               targetProcEventInfo = ProcEventInfo(this,
                                                      actionTarget,
                                                      this,
                                                      typeMaskActionTarget,
                                                      spellTypeMask,
                                                      spellPhaseMask,
                                                      hitMask,
                                                      spell,
    ...
    if (typeMaskActionTarget)
        GetProcAurasTriggeredOnEvent(
            targetAurasTriggeringProc, targetProcAuras, targetProcEventInfo);
...
void Unit::TriggerAurasProcOnEvent(
    ProcEventInfo& eventInfo, std::list<AuraApplication*>& aurasTriggeringProc)
{
    for (std::list<AuraApplication*>::iterator itr =
             aurasTriggeringProc.begin();
         itr != aurasTriggeringProc.end();
         ++itr) {
        if (!(*itr)->GetRemoveMode())
...
void Unit::StopMoving()
{
    ...
    // Update position now since Stop does not start a new movement that can be
    // updated later
...
void Unit::PauseMovement(uint32 timer /* = 0*/, uint8 slot /* = 0*/)
{
    ...
    if (MovementGenerator* movementGenerator =
            GetMotionMaster()->GetMotionSlot(slot))
...
void Unit::StopMovingOnCurrentPos() // pussywizard
{
    ...
    DisableSpline(); // pussywizard: required so Launch() won't recalculate
                     // position from previous spline
    Movement::MoveSplineInit init(this);
...
{
    int32 scaleAuras = GetTotalAuraModifier(SPELL_AURA_MOD_SCALE) +
                       GetTotalAuraModifier(SPELL_AURA_MOD_SCALE_2);
...
void Unit::SetDisplayId(uint32 modelId)
{
    ...
    if (CreatureModelInfo const* minfo =
            sObjectMgr->GetCreatureModelInfo(modelId))
...
void Unit::RestoreDisplayId()
{
    ...
    AuraEffectList const& transforms =
        GetAuraEffectsByType(SPELL_AURA_TRANSFORM);
    if (!transforms.empty()) {
        // iterate over already applied transform auras - from newest to oldest
        for (auto i = transforms.rbegin(); i != transforms.rend(); ++i) {
            if (AuraApplication const* aurApp =
                    (*i)->GetBase()->GetApplicationOfTarget(GetGUID())) {
                if (!handledAura)
                    handledAura = (*i);
                // xinef: prefer negative/forced auras
                if ((*i)->GetSpellInfo()->HasAttribute(
    ...
    if (!handledAuraForced && !handledAura) {
        Unit::AuraEffectList const& cloneAuras =
            GetAuraEffectsByType(SPELL_AURA_CLONE_CASTER);
        if (!cloneAuras.empty())
            for (Unit::AuraEffectList::const_iterator i = cloneAuras.begin();
                 i != cloneAuras.end();
                 ++i)
                handledAura = *i;
    }

    AuraEffectList const& shapeshiftAura =
    ...
    if (handledAuraForced) {
        handledAuraForced->HandleEffect(
            this, AURA_EFFECT_HANDLE_SEND_FOR_CLIENT, true);
        return;
    }
    else if (!shapeshiftAura.empty()) // we've found shapeshift
    {
        // only one such aura possible at a time
        if (uint32 modelId = GetModelForForm(GetShapeshiftForm(),
                                             shapeshiftAura.front()->GetId())) {
            SetDisplayId(modelId);
            return;
        }
    }
    else if (handledAura) {
        handledAura->HandleEffect(
            this, AURA_EFFECT_HANDLE_SEND_FOR_CLIENT, true);
        return;
    }
...
void Unit::AddComboPoints(Unit* target, int8 count)
{
    ...
    if (target && target != m_comboTarget) {
        ...
        m_comboPoints =
            std::max<int8>(std::min<int8>(m_comboPoints + count, 5), 0);
    }
...
void Unit::ClearComboPoints()
{
    ...
    // (NB: this Aura retains the CP while it's active - now that CP have reset,
...
void Unit::SendComboPoints()
{
    ...
    if (m_movedByPlayer || owner) {
        WorldPacket data(SMSG_PET_UPDATE_COMBO_POINTS,
                         GetPackGUID().size() + packGUID.size() + 1);
        data << GetPackGUID();
        data << packGUID;
        data << uint8(m_comboPoints);
...
void Unit::ClearComboPointHolders()
{
    while (!m_ComboPointHolders.empty()) {
        (*m_ComboPointHolders.begin())
            ->ClearComboPoints(); // this also removes it from
                                  // m_comboPointHolders
    }
}
...
Unit* Unit::SelectNearbyNoTotemTarget(Unit* exclude, float dist) const
{
    ...
    Acore::UnitListSearcher<Acore::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>
        searcher(this, targets, u_check);
    Cell::VisitAllObjects(this, searcher, dist);
...
void Unit::ApplyAttackTimePercentMod(WeaponAttackType att,
                                     ...
                                     bool             apply)
{
    float remainingTimePct = std::max((float)m_attackTimer[att], 0.0f) /
                             (GetAttackTime(att) * m_modAttackSpeedPct[att]);
    if (val > 0) {
        ApplyPercentModFloatVar(m_modAttackSpeedPct[att], val, !apply);
        ApplyPercentModFloatValue(
            static_cast<uint16>(UNIT_FIELD_BASEATTACKTIME) + att, val, !apply);
    }
    else {
        ApplyPercentModFloatVar(m_modAttackSpeedPct[att], -val, apply);
        ApplyPercentModFloatValue(
    ...
    m_attackTimer[att] = uint32(GetAttackTime(att) * m_modAttackSpeedPct[att] *
                                remainingTimePct);
}
                                    ...
                                    uint32           CastingTime) const
{
    ...
    for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
        if (spellProto->Effects[j].Effect == SPELL_EFFECT_HEALTH_LEECH ||
            (spellProto->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA &&
             spellProto->Effects[j].ApplyAuraName ==
                                        ...
                                        DamageEffectType damagetype) const
{
    ...
    int32 CastingTime = spellInfo->IsChanneled() ? spellInfo->GetDuration()
...
float Unit::GetAPMultiplier(WeaponAttackType attType, bool normalized)
{
    ...
    default:
        return Weapon->GetTemplate()->SubClass == ITEM_SUBCLASS_WEAPON_DAGGER
                   ? 1.7f
                   : 2.4f;
    }
}
...
{
    return getMSTimeDiff(m_lastManaUse, GameTime::GetGameTimeMS().count()) <
           5000;
}

void Unit::SetContestedPvP(Player* attackedPlayer,
                           bool    lookForNearContestedGuards)
{
    Player* player = GetCharmerOrOwnerPlayerOrPlayerItself();
    ...
    if (!player ||
        ((attackedPlayer &&
          (attackedPlayer == player ||
           (player->duel && player->duel->Opponent == attackedPlayer))) ||
         player->InBattleground()))
        return;

    // check if there any guards that should care about the contested flag on
    // player
    if (lookForNearContestedGuards) {
        std::list<Unit*>                                       targets;
        Acore::NearestVisibleDetectableContestedGuardUnitCheck u_check(this);
        Acore::UnitListSearcher<
            Acore::NearestVisibleDetectableContestedGuardUnitCheck>
            searcher(this, targets, u_check);
...
Pet* Unit::CreateTamedPetFrom(Creature* creatureTarget, uint32 spell_id)
{
    ...
    uint8 level = creatureTarget->GetLevel() + 5 < GetLevel()
                      ? (GetLevel() - 5)
                      : creatureTarget->GetLevel();
...
bool Unit::InitTamedPet(Pet* pet, uint8 level, uint32 spell_id)
{
    ...
    if (!pet->InitStatsForLevel(level)) {
        LOG_ERROR("entities.unit",
                  "Pet::InitStatsForLevel() failed for creature (Entry: {})!",
                  pet->GetEntry());
        return false;
    }
...
bool Unit::IsTriggeredAtSpellProcEvent(
    Unit*                       victim,
    Aura*                       aura,
    WeaponAttackType            attType,
    bool                        isVictim,
    ...
    ProcEventInfo const&        eventInfo)
{
    ...
    // if (procExtra & PROC_EX_INTERNAL_TRIGGERED && !(procFlag &
    ...
    // Xinef: additional check for player auras - only player spells can trigger
    // player proc auras Xinef: skip victim auras Excluded player shoot spells
    // Excluded player item spells
    if (!isVictim && IsPlayer() &&
        !(EventProcFlag & (PROC_FLAG_KILL | PROC_FLAG_DEATH))) {
        if (procSpell && procSpell->SpellFamilyName == SPELLFAMILY_GENERIC &&
            procSpell->GetCategory() != 76 &&
            (!eventInfo.GetProcSpell() ||
             !eventInfo.GetProcSpell()->m_CastItem) &&
            (!eventInfo.GetTriggerAuraSpell() ||
             eventInfo.GetTriggerAuraSpell()->SpellFamilyName ==
                 SPELLFAMILY_GENERIC)) {
            return false;
        }
    }
    ...
    if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(
            spellProto, spellProcEvent, EventProcFlag, eventInfo, active))
        return false;
    ...
    if (EventProcFlag & PROC_FLAG_KILL && GetTypeId() == TYPEID_PLAYER) {
        ...
        if (aura->GetId() == 32409 || aura->GetId() == 18372 ||
            aura->GetId() == 18213)
            allow = true;
        if (!allow)
            return false;
    ...
    // triggers) But except periodic and kill triggers (can triggered from self)
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER &&
        !spellProto->HasAttribute(SPELL_ATTR3_NO_PROC_EQUIP_REQUIREMENT)) {
        Player* player = ToPlayer();
        if (spellProto->EquippedItemClass == ITEM_CLASS_WEAPON) {
            Item* item = nullptr;
            if (attType == BASE_ATTACK)
                item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0,
            ...
            Item* item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0,
                                                     EQUIPMENT_SLOT_OFFHAND);
            if (!item || item->IsBroken() ||
    ...
    if (spellProcEvent && spellProcEvent->ppmRate != 0) {
        if (!isVictim) {
            ...
            chance             = GetPPMProcChance(
                WeaponSpeed, spellProcEvent->ppmRate, spellProto);
        }
        else if (victim) {
            uint32 WeaponSpeed = victim->GetAttackTime(attType);
            chance             = victim->GetPPMProcChance(
                WeaponSpeed, spellProcEvent->ppmRate, spellProto);
        }
    }
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
    ...
    if (!(spellProto->SpellFamilyName == SPELLFAMILY_PRIEST &&
        ...
        LOG_DEBUG("spells.aura",
                  "Unit::HandleAuraRaidProcFromChargeWithValue, received not "
                  "handled spell: {}",
                  spellProto->Id);
        return false;
    }
    ...
    // current aura expire
    triggeredByAura->GetBase()->SetCharges(
    ...
    if (jumps > 0) {
        if (Unit* caster = triggeredByAura->GetCaster()) {
            // smart healing
            float radius = triggeredByAura->GetSpellInfo()
                               ->Effects[triggeredByAura->GetEffIndex()]
                               .CalcRadius(caster);
            ...
            if (player) {
                ...
                if (!nearMembers.empty()) {
                    ...
                    if (Unit* target = nearMembers.front()) {
                        CastSpell(target,
                                  41637 /*Dummy visual effect triggered by main
                                           spell cast*/
                                  ,
                                  true);
                        CastCustomSpell(target,
                                        ...
                                        triggeredByAura,
                                        caster_guid);
                        if (Aura* aura = target->GetAura(spellProto->Id,
                                                         caster->GetGUID()))
...
bool Unit::HandleAuraRaidProcFromCharge(AuraEffect* triggeredByAura)
{
    ...
    case 57949: // shiver
        damageSpellId = 57952;
        // animationSpellId = 57951; dummy effects for jump spell have unknown
        // use (see also 41637)
        break;
    case 59978: // shiver
        damageSpellId = 59979;
    ...
    default:
        LOG_ERROR(
            "entities.unit",
            "Unit::HandleAuraRaidProcFromCharge, received unhandled spell: {}",
            spellProto->Id);
        return false;
    }
        ...
        if (Unit* caster = triggeredByAura->GetCaster()) {
            ...
            if (Unit* target = GetNextRandomRaidMemberOrPet(radius)) {
                CastSpell(target,
                          ...
                          caster_guid);
                if (Aura* aura =
                        target->GetAura(spellProto->Id, caster->GetGUID()))
                    aura->SetCharges(jumps);
            }
        }
    }
                ...
                Spell const*     spell /*= nullptr*/)
{
    ...
    Player* player =
        killer ? killer->GetCharmerOrOwnerPlayerOrPlayerItself() : nullptr;
    Creature* creature = victim->ToCreature();
    ...
    if (isRewardAllowed && creature) {
        ...
        if (lr && lr->IsInMap(creature))
            ...
            for (GroupReference* itr = lrg->GetFirstMember(); itr != nullptr;
                 itr                 = itr->next())
    ...
    // call kill spell proc event (before real die and combat stop to triggering
    // auras removed at death/combat stop)
    if (isRewardAllowed && player && player != victim) {
        ...
        if (group) {
            ...
            if (creature) {
                ...
                if (group->GetLooterGuid() &&
                    group->GetLootMethod() != FREE_FOR_ALL) {
                    looter = ObjectAccessor::FindPlayer(group->GetLooterGuid());
                    if (looter) {
                        hasLooterGuid = true;
                        creature->SetLootRecipient(
                            looter); // update creature loot recipient to the
                                     // allowed looter.
                    }
                }
            }
        ...
        if (creature) {
            ...
            if (uint32 lootid = creature->GetCreatureTemplate()->lootid)
                loot->FillLoot(lootid,
                               LootTemplates_Creature,
                               looter,
                               false,
                               false,
                               creature->GetLootMode(),
                               creature);
    ...
    // Do KILL and KILLED procs. KILL proc is called only for the unit who
    // landed the killing blow (and its owner - for pets and totems) regardless
    // of who tapped the victim
    if (killer && (killer->IsPet() || killer->IsTotem()))
        if (Unit* owner = killer->GetOwner()) {
            Unit::ProcDamageAndSpell(owner,
                                     victim,
                                     PROC_FLAG_KILL,
                                     PROC_FLAG_NONE,
                                     PROC_EX_NONE,
                                     ...
                                     -1,
                                     spell);
            sScriptMgr->OnCreatureKilledByPet(
                killer->GetCharmerOrOwnerPlayerOrPlayerItself(),
                victim->ToCreature());
        }

    if (killer != victim) {
        Unit::ProcDamageAndSpell(killer,
                                 victim,
                                 killer ? PROC_FLAG_KILL : 0,
                                 PROC_FLAG_KILLED,
                                 PROC_EX_NONE,
                                 0,
                                 attackType,
    ...
    Unit::ProcDamageAndSpell(victim,
                             nullptr,
                             PROC_FLAG_DEATH,
                             PROC_FLAG_NONE,
                             PROC_EX_NONE,
    ...
    // update get killing blow achievements, must be done before setDeathState
    // to be able to require auras on target and before Spirit of Redemption as
    // it also removes auras
    if (killer)
        if (Player* killerPlayer =
                killer->GetCharmerOrOwnerPlayerOrPlayerItself())
            killerPlayer->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS, 1, 0, victim);

    // Spirit of Redemption
    // if talent known but not triggered (check priest class for speedup check)
    bool spiritOfRedemption = false;
    if (victim->GetTypeId() == TYPEID_PLAYER &&
        victim->IsClass(CLASS_PRIEST, CLASS_CONTEXT_ABILITY) &&
        !victim->ToPlayer()->HasPlayerFlag(PLAYER_FLAGS_IS_OUT_OF_BOUNDS)) {
        if (AuraEffect* aurEff = victim->GetAuraEffectDummy(20711)) {
            ...
            if (victim->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION) ||
                victim->HasAura(27827)) {
                /*LOG_INFO("misc", "Player ({}) died with spirit of redemption.
                Killer (Entry: {}, Name: {}), Map: {}, x: {}, y: {}, z: {}",
                    victim->GetGUID().ToString(), killer ? killer->GetEntry() :
                1, killer ? killer->GetName() : "", victim->GetMapId(),
                victim->GetPositionX(), victim->GetPositionY(),
                victim->GetPositionZ());
                ...
                uint32 ressSpellId =
                    victim->GetUInt32Value(PLAYER_SELF_RES_SPELL);
                if (!ressSpellId)
                    ressSpellId = victim->ToPlayer()->GetResurrectionSpellId();

                // Remove all expected to remove at death auras (most important
                // negative case like DoT or periodic triggers)
    ...
    // MUST come after victim->setDeathState(DeathState::JustDied); or pet next
    // target selection will get stuck on same target and break pet react state
    if (player) {
        Pet* pet = player->GetPet();
    ...
    if (Player* plrVictim = victim->ToPlayer()) {
        ...
        if ((durabilityLoss && !player && !plrVictim->InBattleground()) ||
            (player && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP))) {
            LOG_DEBUG("entities.unit",
                      "We are dead, losing {} percent durability",
                      sWorld->getRate(RATE_DURABILITY_LOSS_ON_DEATH));
            plrVictim->DurabilityLossAll(
    ...
    else // creature died
    {
        ...
        // Call KilledUnit for creatures, this needs to be called after the
        // lootable flag is set
        if (killer && killer->GetTypeId() == TYPEID_UNIT && killer->IsAIEnabled)
            killer->ToCreature()->AI()->KilledUnit(victim);
        ...
        if (TempSummon* summon = creature->ToTempSummon()) {
            if (WorldObject* summoner = summon->GetSummoner()) {
                if (summoner->ToCreature() &&
                    summoner->ToCreature()->IsAIEnabled) {
                    summoner->ToCreature()->AI()->SummonedCreatureDies(creature,
                                                                       killer);
                }
                else if (summoner->ToGameObject() &&
                         summoner->ToGameObject()->AI()) {
                    summoner->ToGameObject()->AI()->SummonedCreatureDies(
                        creature, killer);
                }
            }
        }
        ...
        if (creature->GetInstanceId()) {
            ...
            /// @todo: do instance binding anyway if the charmer/owner is
            /// offline
    ...
    // outdoor pvp things, do these after setting the death state, else the
    // player activity notify won't work... doh... handle player kill only if
    // not suicide (spirit of redemption for example)
    if (player && killer != victim) {
        if (OutdoorPvP* pvp = player->GetOutdoorPvP())
            pvp->HandleKill(player, victim);

        if (Battlefield* bf =
                sBattlefieldMgr->GetBattlefieldToZoneId(player->GetZoneId()))
    ...
    if (killer && victim->GetTypeId() == TYPEID_PLAYER) {
        if (killer->GetTypeId() == TYPEID_UNIT)
            victim->ToPlayer()->UpdateAchievementCriteria(
                         ...
                         bool      isFear /*= false*/)
{
    if (apply) {
        ...
        // xinef: moved from below, checked all SetX functions, no calls to
        // currently modified state xinef: added to each case because of return
        ...
        case UNIT_STATE_STUNNED:
            ...
            break;
        case UNIT_STATE_ROOT:
            // Prevent creature_template_movement rooted flag from being removed
            // on aura expiration.
            if (GetTypeId() == TYPEID_UNIT) {
        ...
        if (HasUnitState(UNIT_STATE_STUNNED) ||
            ...
            if (HasUnitState(UNIT_STATE_CONFUSED) ||
                ...
                if (HasAuraType(SPELL_AURA_MOD_FEAR)) {
                    isFear = true;
                    source = ObjectAccessor::GetUnit(
                        *this,
                        GetAuraEffectsByType(SPELL_AURA_MOD_FEAR)
                            .front()
                            ->GetCasterGUID());
                }
...
void Unit::SetStunned(bool apply)
{
    ...
    if (apply) {
        ...
        if (GetTypeId() == TYPEID_UNIT) {
            // don't remove UNIT_FLAG_STUNNED for pet when owner is mounted
            // (disabled pet's interface)
            Unit* owner = GetOwner();
            if (!owner || owner->GetTypeId() != TYPEID_PLAYER ||
        ...
        if (!HasUnitState(
                UNIT_STATE_ROOT)) // prevent moving if it also has root effect
        {
...
void Unit::SetRooted(bool apply, bool isStun)
{
    if (apply) {
        ...
        // MOVEMENTFLAG_MASK_MOVING (tested 3.3.5a) this will freeze clients.
        // That's why we remove MOVEMENTFLAG_MASK_MOVING before setting
        // MOVEMENTFLAG_ROOT
        ...
        if (!HasUnitState(UNIT_STATE_STUNNED)) // prevent moving if it also has
                                               // stun effect
        {
            if (m_movedByPlayer) {
                     ...
                     bool  isFear /*= false*/)
{
    if (apply) {
        ...
        GetMotionMaster()->MoveFleeing(
            fearedBy,
            isFear ? 0
                   : sWorld->getIntConfig(CONFIG_CREATURE_FAMILY_FLEE_DELAY));
    }
    else {
        if (IsAlive()) {
            if (GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_CONTROLLED) ==
                FLEEING_MOTION_TYPE) {
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
        if (m_movedByPlayer)
            m_movedByPlayer->ToPlayer()->SetClientControl(this,
                                                          !apply); // verified
        // else
                        ...
                        AuraApplication const* aurApp)
{
    ...
    LOG_DEBUG("entities.unit",
              "SetCharmedBy: charmer {} ({}), charmed {} ({}), type {}.",
              charmer->GetEntry(),
              charmer->GetGUID().ToString(),
              GetEntry(),
              GetGUID().ToString(),
              uint32(type));
    ...
    if (this == charmer) {
        LOG_FATAL("entities.unit",
                  "Unit::SetCharmedBy: Unit {} ({}) is trying to charm itself!",
                  GetEntry(),
                  GetGUID().ToString());
        return false;
    }
    ...
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetTransport()) {
        LOG_FATAL("entities.unit",
                  "Unit::SetCharmedBy: Player on transport is trying to charm "
                  "{} ({})",
                  GetEntry(),
                  GetGUID().ToString());
        return false;
    ...
    if (GetCharmerGUID()) {
        LOG_FATAL("entities.unit",
                  "Unit::SetCharmedBy: {} ({}) has already been charmed but {} "
                  "({}) is trying to charm it!",
                  GetEntry(),
                  GetGUID().ToString(),
                  charmer->GetEntry(),
                  charmer->GetGUID().ToString());
    ...
    // Xinef: dont reset threat and combat, put them on offline list, moved down
    // after faction changes
    //  CombatStop(); /// @todo: CombatStop(true) may cause crash (interrupt
    //  spells) DeleteThreatList();
    ...
    if (playerCharmer) {
        ...
        playerCharmer->StopCastingBindSight(aurApp ? aurApp->GetBase()
                                                   : nullptr);
    }
    ...
    if (!IsInWorld()) {
        LOG_FATAL("entities.unit",
                  "Unit::SetCharmedBy: {} ({}) is not in world but {} ({}) is "
                  "trying to charm it!",
                  GetEntry(),
                  GetGUID().ToString(),
                  charmer->GetEntry(),
    ...
    // charm is set by aura, and aura effect remove handler was called during
    // apply handler execution prevent undefined behaviour
    ...
    if (GetTypeId() == TYPEID_UNIT) {
        ...
        if (charmer->GetTypeId() == TYPEID_PLAYER &&
            ...
            ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON) {
            // Disable CreatureAI/SmartAI and switch to CharmAI when charmed by
            // warlock
        ...
        // Xinef: If creature can fly, add normal player flying flag (fixes
        // speed)
    ...
    if (/*type != CHARM_TYPE_VEHICLE &&*/ !GetCharmInfo()) {
        ...
        if (type == CHARM_TYPE_POSSESS)
            ...
            // Xinef: convert charm npcs dont have pet bar so initialize them as
            // defensive helpers
                ...
                if (cinfo && cinfo->type == CREATURE_TYPE_DEMON) {
                    ...
                    SetUInt32Value(
                        ...
                        uint32(GameTime::GetGameTime()
                                   .count())); // cast can't be helped
                }
            }
            if (playerCharmer->m_seer != this) {
                GetMotionMaster()->MoveFollow(
                    charmer, PET_FOLLOW_DIST, GetFollowAngle());
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        sScriptMgr->OnPlayerBeingCharmed(
            ToPlayer(), charmer, _oldFactionId, charmer->GetFaction());
...
void Unit::RemoveCharmedBy(Unit* charmer)
{
    ...
    {
        //        LOG_FATAL("entities.unit", "Unit::RemoveCharmedBy: this: {}
        //        true charmer: {} false charmer: {}",
    ...
    ASSERT(type != CHARM_TYPE_POSSESS || charmer->GetTypeId() == TYPEID_PLAYER);
    ASSERT(type != CHARM_TYPE_VEHICLE ||
           (GetTypeId() == TYPEID_UNIT && IsVehicle()));
    ...
    if (playerCharmer) {
        ...
        case CHARM_TYPE_CHARM:
            if (GetTypeId() == TYPEID_UNIT &&
                ...
                if (cinfo && cinfo->type == CREATURE_TYPE_DEMON) {
                    SetByteValue(
                        UNIT_FIELD_BYTES_0, 1, uint8(cinfo->unit_class));
                    if (GetCharmInfo())
                        GetCharmInfo()->SetPetNumber(0, true);
                    else
                        LOG_ERROR("entities.unit",
                                  "Aura::HandleModCharm: target={} has a charm "
                                  "aura but no charm info!",
                                  GetGUID().ToString());
                }
            }
            break;
    ...
    for (CharmThreatMap::const_iterator itr = _charmThreatInfo.begin();
         itr != _charmThreatInfo.end();
         ++itr) {
    ...
    if (Creature* creature = ToCreature()) {
        ...
        if (type != CHARM_TYPE_VEHICLE && charmer->IsAlive() &&
            !charmer->IsFriendlyTo(creature))
    ...
    if (GetTypeId() == TYPEID_PLAYER ||
        (GetTypeId() == TYPEID_UNIT && !ToCreature()->IsGuardian()))
        DeleteCharmInfo();
}
...
void Unit::RestoreFaction()
{
    if (GetTypeId() == TYPEID_PLAYER)
        ...
        if (CreatureTemplate const* cinfo =
                ToCreature()->GetCreatureTemplate()) // normal creature
...
bool Unit::IsInPartyWith(Unit const* unit) const
{
    ...
    else if (u1->GetTypeId() == TYPEID_UNIT && u2->GetTypeId() == TYPEID_UNIT &&
             !u1->IsControlledByPlayer() && !u2->IsControlledByPlayer())
        return u1->GetFaction() == u2->GetFaction();
    // Xinef: creature type_flag should work for party check only if player
    // group is not a raid
    else if ((u2->GetTypeId() == TYPEID_PLAYER &&
              u1->GetTypeId() == TYPEID_UNIT &&
              (u1->ToCreature()->GetCreatureTemplate()->type_flags &
               CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT) &&
              u2->ToPlayer()->GetGroup() &&
              !u2->ToPlayer()->GetGroup()->isRaidGroup()) ||
             (u1->GetTypeId() == TYPEID_PLAYER &&
              u2->GetTypeId() == TYPEID_UNIT &&
              (u2->ToCreature()->GetCreatureTemplate()->type_flags &
               CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT) &&
              u1->ToPlayer()->GetGroup() &&
              !u1->ToPlayer()->GetGroup()->isRaidGroup()))
        return true;
    else
        return false;
}
...
void Unit::GetPartyMembers(std::list<Unit*>& TagUnitMap)
{
    ...
    if (group) {
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
            ...
            // IsHostileTo check duel and controlled by enemy
            if (Target && Target->IsInMap(owner) &&
                Target->GetSubGroup() == subgroup && !IsHostileTo(Target)) {
                if (Target->IsAlive())
                    TagUnitMap.push_back(Target);

                for (Unit::ControlSet::iterator iterator =
                         Target->m_Controlled.begin();
                     iterator != Target->m_Controlled.end();
                     ++iterator) {
        ...
        for (Unit::ControlSet::iterator itr = owner->m_Controlled.begin();
             itr != owner->m_Controlled.end();
             ++itr) {
            if (Unit* pet = *itr)
                if (pet->IsGuardian() && pet->IsAlive())
...
Aura* Unit::AddAura(SpellInfo const* spellInfo, uint8 effMask, Unit* target)
{
    ...
    if (Aura* aura =
            Aura::TryRefreshStackOrCreate(spellInfo, effMask, target, this)) {
...
// Crit or block - determined on damage calculation phase! (and can be both in
// some time)
...
{
    SpellInfo const* spellInfo =
        spellId ? sSpellMgr->GetSpellInfo(spellId) : nullptr;
    ...
    // Check if dual wielding, add additional miss penalty - when mainhand has
    // on next swing spell, offhand doesnt suffer penalty
    if (!spellId && (attType != RANGED_ATTACK) && haveOffhandWeapon() &&
        (!m_currentSpells[CURRENT_MELEE_SPELL] ||
         !m_currentSpells[CURRENT_MELEE_SPELL]->IsNextMeleeSwingSpell())) {
        missChance += 19;
    }
...
void Unit::SetPhaseMask(uint32 newPhaseMask, bool update)
{
    ...
    if (IsInWorld()) {
        ...
        // pussywizard: goign to other phase (valithria, algalon) should not
        // remove such auras
        // RemoveNotOwnSingleTargetAuras(newPhaseMask, true);            // we
        // can lost access to caster or target

        if (!sScriptMgr->CanSetPhaseMask(this, newPhaseMask, update))
            return;

        // modify hostile references for new phasemask, some special cases deal
        // with hostile references themselves
        if (GetTypeId() == TYPEID_UNIT ||
            (!ToPlayer()->IsGameMaster() &&
             !ToPlayer()->GetSession()->PlayerLogout())) {
            ...
            while (ref) {
                if (Unit* unit = ref->GetSource()->GetOwner())
                    if (Creature* creature = unit->ToCreature())
                        refMgr.setOnlineOfflineState(
                            creature, creature->InSamePhase(newPhaseMask));
            ...
            if (GetTypeId() != TYPEID_PLAYER) {
                ...
                ThreatContainer::StorageType offlineThreatList =
                    GetThreatMgr().GetOfflineThreatList();
                ...
                for (ThreatContainer::StorageType::const_iterator itr =
                         threatList.begin();
                     itr != threatList.end();
                     ++itr)
                    if (Unit* unit = (*itr)->getTarget())
                        unit->getHostileRefMgr().setOnlineOfflineState(
                            ToCreature(), unit->InSamePhase(newPhaseMask));
            }
        }
    }
...
uint32 Unit::GetModelForForm(ShapeshiftForm form, uint32 spellId) const
{
    ...
    uint32                      modelid = 0;
    SpellShapeshiftEntry const* formEntry =
        sSpellShapeshiftStore.LookupEntry(form);
    if (formEntry && formEntry->modelID_A) {
        ...
        if (GetTypeId() != TYPEID_PLAYER)
            ...
            // If the player is horde but there are no values for the horde
            // modelid - take the alliance modelid
...
{
    return _redirectThreatInfo.GetTargetGUID()
               ? ObjectAccessor::GetUnit(*this,
                                         _redirectThreatInfo.GetTargetGUID())
               : nullptr;
}
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    bool   result = false;
    uint32 spellClickEntry =
        GetVehicleKit() ? GetVehicleKit()->GetCreatureEntry() : GetEntry();
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first;
        ...
        ConditionList conds = sConditionMgr->GetConditionsForSpellClickEvent(
            spellClickEntry, itr->second.spellId);
        ConditionSourceInfo info = ConditionSourceInfo(clicker, this);
        if (!sConditionMgr->IsObjectMeetToConditions(info, conds))
            continue;

        Unit* caster = (itr->second.castFlags & NPC_CLICK_CAST_CASTER_CLICKER)
                           ? clicker
                           : this;
        Unit* target = (itr->second.castFlags & NPC_CLICK_CAST_TARGET_CLICKER)
                           ? clicker
                           : this;
        ObjectGuid origCasterGUID =
            (itr->second.castFlags & NPC_CLICK_CAST_ORIG_CASTER_OWNER)
                ? GetOwnerGUID()
                : clicker->GetGUID();

        SpellInfo const* spellInfo =
            sSpellMgr->GetSpellInfo(itr->second.spellId);

        // xinef: dont allow players to enter vehicles on arena
        if (spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE) &&
            caster->GetTypeId() == TYPEID_PLAYER && caster->FindMap() &&
            caster->FindMap()->IsBattleArena())
            continue;
        ...
        if (seatId > -1) {
            ...
            if (!valid) {
                LOG_ERROR("sql.sql",
                          "Spell {} specified in npc_spellclick_spells is not "
                          "a valid vehicle enter aura!",
                          itr->second.spellId);
                continue;
            }
            ...
            if (IsInMap(caster))
                caster->CastCustomSpell(
                    itr->second.spellId,
                    SpellValueMod(SPELLVALUE_BASE_POINT0 + i),
                    seatId + 1,
                    target,
                    GetVehicleKit()
            ...
            else // This can happen during Player::_LoadAuras
            {
                ...
                bp0[i] = seatId;
                Aura::TryRefreshStackOrCreate(spellInfo,
                                              MAX_EFFECT_MASK,
                                              this,
                                              clicker,
                                              bp0,
...
{
    CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED,
    ...
    if (m_vehicle) {
        if (m_vehicle == vehicle) {
            if (seatId >= 0 && seatId != GetTransSeat()) {
                LOG_DEBUG(
                    "vehicles",
                    "EnterVehicle: {} leave vehicle {} seat {} and enter {}.",
                    GetEntry(),
...
{
    //! This function can be called at upper level code to initialize an exit
    ...
    //! without specifying an exitposition. The subsequent call below would
    //! return on if (!m_vehicle).
    /*_ExitVehicle(exitPosition);*/
    //! To do:
    //! We need to allow SPELL_AURA_CONTROL_VEHICLE unapply handlers in
    //! spellscripts to specify exit coordinates and either store those per
    //! passenger, or we need to init spline movement based on those coordinates
    //! in unapply handlers, and relocate exiting passengers based on
    //! Unit::moveSpline data. Either way, Coming Soon(TM)
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    // If player is on mouted duel and exits the mount should immediatly lose
    // the duel
    if (player && player->duel && player->duel->IsMounted)
        player->DuelComplete(DUEL_FLED);

    // This should be done before dismiss, because there may be some aura
    // removal
    ...
    SetControlled(
        false, UNIT_STATE_ROOT); // SMSG_MOVE_FORCE_UNROOT, ~MOVEMENTFLAG_ROOT

    Position pos;
    if (!exitPosition)                    // Exit position not specified
        pos = vehicleBase->GetPosition(); // This should use passenger's current
                                          // position, leaving it as it is now
    ...
    if (vehicleInfo) {
        ...
        else if (vehicleInfo->m_ID == 349) // AT Mounts, dismount to the right
        {
            float x = pos.GetPositionX() +
                      2.0f * cos(pos.GetOrientation() - M_PI / 2.0f);
            float y = pos.GetPositionY() +
                      2.0f * std::sin(pos.GetOrientation() - M_PI / 2.0f);
    ...
    if (player) {
        player->SetFallInformation(GameTime::GetGameTime().count(),
                                   GetPositionZ());
    ...
    if (!vehicleInfo || vehicleInfo->m_ID != 341) {
        ...
        Movement::MoveSplineInit init(this);
        init.MoveTo(pos.GetPositionX() + 8 * cos(o),
                    pos.GetPositionY() + 8 * std::sin(o),
                    pos.GetPositionZ() + 16.0f);
        init.SetFacing(GetOrientation());
        init.SetTransportExit();
        init.Launch();
        DisableSpline();
    ...
    // xinef: move fall, should we support all creatures that exited vehicle in
    // air? Currently Quest Drag and Drop only, Air Assault quest
    if (GetTypeId() == TYPEID_UNIT && !CanFly() && vehicleInfo &&
        (vehicleInfo->m_ID == 113 || vehicleInfo->m_ID == 8 ||
         vehicleInfo->m_ID == 290 || vehicleInfo->m_ID == 298)) {
        GetMotionMaster()->MoveFall();
    }

    if ((!player ||
         !(player->GetDelayedOperations() & DELAYED_VEHICLE_TELEPORT)) &&
        vehicle->GetBase()->HasUnitTypeMask(UNIT_MASK_MINION))
        if (((Minion*)vehicleBase)->GetOwner() == this) {
            if (!vehicleInfo || vehicleInfo->m_ID != 349) {
                ...
                vehicle->Uninstall();
                vehicleBase->m_Events.AddEvent(
                    new VehicleDespawnEvent(*vehicleBase, 2000),
                    vehicleBase->m_Events.CalculateTime(2000));
            }
            ...
            if (player) {
                if (vehicleBase->GetEntry() == NPC_EIDOLON_WATCHER)
                    player->CastSpell(
                        player, VEHICLE_SPELL_SHADE_CONTROL_END, true);
                else if (vehicleBase->GetEntry() == NPC_LITHE_STALKER)
                    player->CastSpell(
                        player, VEHICLE_SPELL_GEIST_CONTROL_END, true);
            }
        }
    ...
    if (HasUnitTypeMask(UNIT_MASK_ACCESSORY)) {
        ...
        // If for other reason we as minion are exiting the vehicle (ejected,
        // master dismounted) - unsummon
        else {
            SetVisible(false);
...
{
    return m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING |
                                          MOVEMENTFLAG_FALLING_FAR) ||
           (!movespline->Finalized() && movespline->Initialized() &&
            movespline->isFalling());
}
...
{
    // Mirror client behavior, if this method returns false then client will not
    // use swimming animation and for players will apply gravity as if there was
    // no water
    if (HasUnitFlag(UNIT_FLAG_CANNOT_SWIM))
        return false;
    if (HasUnitFlag(UNIT_FLAG_POSSESSED) ||
        HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED)) // is player
        return true;
...
{
    NearTeleportTo(pos.GetPositionX(),
                   pos.GetPositionY(),
                   pos.GetPositionZ(),
                   pos.GetOrientation(),
                   casting,
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->TeleportTo(
            ...
            TELE_TO_NOT_LEAVE_COMBAT |
                (removeTransport ? 0 : TELE_TO_NOT_LEAVE_TRANSPORT) |
                TELE_TO_NOT_UNSUMMON_PET | (casting ? TELE_TO_SPELL : 0) |
                (vehicleTeleport ? TELE_TO_NOT_LEAVE_VEHICLE : 0) |
                (withPet ? TELE_TO_WITH_PET : 0));
    else {
    ...
    float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!GetVehicle()) {
        uint32 mask = 0;
        if (turn)
            mask |= AURA_INTERRUPT_FLAG_TURNING;
        if (relocated)
            mask |= AURA_INTERRUPT_FLAG_MOVE;
        if (mask)
...
void Unit::SendThreatListUpdate()
{
    if (!GetThreatMgr().isThreatListEmpty()) {
        ...
        data << GetPackGUID();
        data << uint32(count);
        ThreatContainer::StorageType const& tlist =
            GetThreatMgr().GetThreatList();
        for (ThreatContainer::StorageType::const_iterator itr = tlist.begin();
             itr != tlist.end();
             ++itr) {
    ...
    for (AttackerSet::const_iterator itr = attackers.begin();
        ...
        if (!attacker->IsValidAttackTarget(this)) {
            ...
            for (Unit* controled : attacker->m_Controlled) {
                if (controled->GetVictim() == this &&
                    !controled->IsValidAttackTarget(this)) {
...
void Unit::OutDebugInfo() const
{
    ...
    LOG_INFO(
        "entities.unit", "GUID {}, name {}", GetGUID().ToString(), GetName());
    LOG_INFO("entities.unit",
             "OwnerGUID {}, MinionGUID {}, CharmerGUID {}, CharmedGUID {}",
             GetOwnerGUID().ToString(),
             GetMinionGUID().ToString(),
             GetCharmerGUID().ToString(),
             GetCharmGUID().ToString());
    LOG_INFO("entities.unit",
             "In world {}, unit type mask {}",
             (uint32)(IsInWorld() ? 1 : 0),
             m_unitTypeMask);
    if (IsInWorld())
    ...
    if (IsVehicle()) {
        ...
        for (SeatMap::iterator itr = GetVehicleKit()->Seats.begin();
             itr != GetVehicleKit()->Seats.end();
             ++itr)
            if (Unit* passenger = ObjectAccessor::GetUnit(
                    *GetVehicleBase(), itr->second.Passenger.Guid))
                LOG_INFO(
                    "entities.unit", "{}, ", passenger->GetGUID().ToString());
        LOG_INFO("server.loading", " ");
    }

    if (GetVehicle())
        LOG_INFO(
            "entities.unit", "On vehicle {}.", GetVehicleBase()->GetEntry());
}

class AuraMunchingQueue : public BasicEvent {
public:
    AuraMunchingQueue(Unit&      owner,
                      ObjectGuid targetGUID,
                      int32      basePoints,
                      uint32     spellId)
        : _owner(owner), _targetGUID(targetGUID), _basePoints(basePoints),
          _spellId(spellId)
    {
    }
...
void Unit::CastDelayedSpellWithPeriodicAmount(Unit*    caster,
                                              ...
                                              uint8    effectIndex)
{
    for (AuraEffectList::iterator i = m_modAuras[auraType].begin();
         i != m_modAuras[auraType].end();
         ++i) {
        AuraEffect* aurEff = *i;
        if (aurEff->GetCasterGUID() != caster->GetGUID() ||
            aurEff->GetId() != spellId ||
            aurEff->GetEffIndex() != effectIndex || !aurEff->GetTotalTicks())
            continue;

        addAmount += ((aurEff->GetOldAmount() *
                       std::max<int32>(aurEff->GetTotalTicks() -
                                           int32(aurEff->GetTickNumber()),
                                       0)) /
    ...
    if (this == caster)
        caster->CastCustomSpell(spellId,
                                SPELLVALUE_BASE_POINT0,
                                addAmount,
                                this,
                                TriggerCastFlags(TRIGGERED_FULL_MASK &
        ...
        caster->m_Events.AddEvent(
            new AuraMunchingQueue(*caster, GetGUID(), addAmount, spellId),
            caster->m_Events.CalculateQueueTime(400));
}
...
uint32 Unit::GetResistance(SpellSchoolMask mask) const
{
    ...
    for (int i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; ++i)
        if (mask & (1 << i) &&
            (resist < 0 || resist > int32(GetResistance(SpellSchools(i)))))
...
{
    //! At this point a new spline destination is enabled because of
    //! Unit::StopMoving()
    G3D::Vector3 stayPos = G3D::Vector3();

    if (atCurrentPos) {
        float z = INVALID_HEIGHT;
        _unit->UpdateAllowedPositionZ(
            _unit->GetPositionX(), _unit->GetPositionY(), z);
        stayPos = G3D::Vector3(_unit->GetPositionX(),
                               _unit->GetPositionY(),
                               z != INVALID_HEIGHT ? z : _unit->GetPositionZ());
    }
    else
...
bool CharmInfo::HasStayPosition() { return _stayX && _stayY && _stayZ; }
...
void Unit::PetSpellFail(SpellInfo const* spellInfo, Unit* target, uint32 result)
{
    ...
    if ((DisableMgr::IsPathfindingEnabled(GetMap()) ||
        ...
        if ((result == SPELL_FAILED_LINE_OF_SIGHT ||
            ...
            if (Unit* owner = GetOwner()) {
                if (spellInfo->IsPositive() && IsFriendlyTo(target)) {
                    ...
                    GetMotionMaster()->MoveFollow(
                        target, PET_FOLLOW_DIST, rand_norm() * 2 * M_PI);
                }
        ...
        if (result == SPELL_FAILED_LINE_OF_SIGHT ||
            result == SPELL_FAILED_OUT_OF_RANGE) {
            charmInfo->SetForcedSpell(spellInfo->IsPositive()
                                          ? -int32(spellInfo->Id)
                                          : spellInfo->Id);
                                        ...
                                        Unit*  caster) const
{
    damage = int32(float(damage) *
                   GetTotalAuraMultiplierByMiscMask(
                       SPELL_AURA_MOD_AOE_DAMAGE_AVOIDANCE, schoolMask));
    if (caster && caster->GetTypeId() == TYPEID_UNIT)
        damage = int32(
            float(damage) *
            GetTotalAuraMultiplierByMiscMask(
                SPELL_AURA_MOD_CREATURE_AOE_DAMAGE_AVOIDANCE, schoolMask));
...
void Unit::ExecuteDelayedUnitRelocationEvent()
{
    ...
    if (this->HasSharedVision())
        for (SharedVisionList::const_iterator itr =
                 this->GetSharedVisionList().begin();
             itr != this->GetSharedVisionList().end();
             ++itr)
            if (Player* player = (*itr)) {
                if (player->IsOnVehicle(this) || !player->IsInWorld() ||
                    player->IsDuringRemoveFromWorld()) // players on vehicles
                ...
                if (Unit* active = viewPoint->ToUnit()) {
                    // if (active->IsVehicle()) // always check original unit
                    // here, last notify position is not relocated
                    //   active = player;

                    float dx = active->m_last_notify_position.GetPositionX() -
                               active->GetPositionX();
                    float dy = active->m_last_notify_position.GetPositionY() -
                               active->GetPositionY();
                    float dz = active->m_last_notify_position.GetPositionZ() -
                               active->GetPositionZ();
                    float distsq    = dx * dx + dy * dy + dz * dz;
                    float mindistsq = DynamicVisibilityMgr::GetReqMoveDistSq(
                        active->FindMap()->GetEntry()->map_type);
                    if (distsq < mindistsq)
                        continue;

                    // this will be relocated below sharedvision!
                    // active->m_last_notify_position.Relocate(active->GetPositionX(),
                    // active->GetPositionY(), active->GetPositionZ());
                }

                Acore::PlayerRelocationNotifier relocateNoLarge(
                    *player, false); // visit only objects which are not large;
                                     // default distance
                Cell::VisitAllObjects(viewPoint,
                                      relocateNoLarge,
                                      player->GetSightRange() +
                                          VISIBILITY_INC_FOR_GOBJECTS);
                relocateNoLarge.SendToSelf();
                Acore::PlayerRelocationNotifier relocateLarge(
                    *player,
                    true); // visit only large objects; maximum distance
                Cell::VisitAllObjects(
                    viewPoint, relocateLarge, MAX_VISIBILITY_DISTANCE);
    ...
    if (Player* player = this->ToPlayer()) {
        ...
        if (viewPoint->GetMapId() != player->GetMapId() ||
            !viewPoint->IsPositionValid() || !player->IsPositionValid())
            return;
        ...
        if (!player->GetFarSightDistance()) {
            Acore::PlayerRelocationNotifier relocateLarge(
                *player, true); // visit only large objects; maximum distance
            Cell::VisitAllObjects(
                viewPoint, relocateLarge, MAX_VISIBILITY_DISTANCE);
            relocateLarge.SendToSelf();
        }
        ...
        float dx =
            unit->m_last_notify_position.GetPositionX() - unit->GetPositionX();
        float dy =
            unit->m_last_notify_position.GetPositionY() - unit->GetPositionY();
        float dz =
            unit->m_last_notify_position.GetPositionZ() - unit->GetPositionZ();
        float distsq    = dx * dx + dy * dy + dz * dz;
        float mindistsq = DynamicVisibilityMgr::GetReqMoveDistSq(
            unit->FindMap()->GetEntry()->map_type);
        if (distsq < mindistsq)
            return;

        unit->m_last_notify_position.Relocate(
            unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ());

        Acore::CreatureRelocationNotifier relocate(*unit);
        Cell::VisitAllObjects(unit,
                              relocate,
...
void Unit::SetFacingTo(float ori)
{
    ...
    if (HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && GetTransGUID())
        init.DisableTransportPathTransformations(); // It makes no sense to
                                                    // target global orientation
    init.SetFacing(ori);
    init.Launch();
}
...
void Unit::SetFacingToObject(WorldObject* object)
{
    ...
    /// @todo figure out under what conditions creature will move towards object
    /// instead of facing it where it currently is.
    Movement::MoveSplineInit init(this);
    init.MoveTo(GetPositionX(), GetPositionY(), GetPositionZ());
    init.SetFacing(GetAngle(
        object)); // when on transport, GetAngle will still return global
                  // coordinates (and angle) that needs transforming
    init.Launch();
}
                    ...
                    bool /*updateAnimationTier = true*/)
{
    ...
    if (enable) {
        ...
        if (hoverHeight && (!isDying() || GetTypeId() != TYPEID_UNIT)) {
            float newZ =
                std::max<float>(GetFloorZ(), GetPositionZ() - hoverHeight);
            UpdateAllowedPositionZ(GetPositionX(), GetPositionY(), newZ);
            UpdateHeight(newZ);
        }
        SendMovementFlagUpdate(); // pussywizard: needed for falling after death
                                  // (instead of falling onto air at hover
                                  // height)
    }
...
void Unit::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target)
{
    ...
    if (cacheIt != _valuesUpdateCache.end()) {
        ...
        BuildValuesCachePosPointers dataAdjustedPos =
            cacheIt->second.posPointers;
        if (cachePos)
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index) {
        if (_fieldNotifyFlags & flags[index] ||
            ...
            ((updateType == UPDATETYPE_VALUES ? _changesMask.GetBit(index)
                                              : m_uint32Values[index]) &&
             (flags[index] & visibleFlag)) ||
            (index == UNIT_FIELD_AURASTATE &&
             HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK))) {
            updateMask.SetBit(index);

            if (index == UNIT_NPC_FLAGS) {
                cacheValue.posPointers.UnitNPCFlagsPos =
                    int32(fieldBuffer.wpos());
                fieldBuffer << m_uint32Values[UNIT_NPC_FLAGS];
            }
            else if (index == UNIT_FIELD_AURASTATE) {
                cacheValue.posPointers.UnitFieldAuraStatePos =
                    int32(fieldBuffer.wpos());
                fieldBuffer << uint32(0); // Fill in later.
            }
            // FIXME: Some values at server stored in float format but must be
            // sent to client in uint32 format
            ...
            // there are some float values which may be negative or can't get
            // negative due to other checks
            else if ((index >= UNIT_FIELD_NEGSTAT0 &&
                      index <= UNIT_FIELD_NEGSTAT4) ||
                     (index >= UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE &&
                      index <= (UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE + 6)) ||
                     (index >= UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE &&
                      index <= (UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE + 6)) ||
                     (index >= UNIT_FIELD_POSSTAT0 &&
                      index <= UNIT_FIELD_POSSTAT4)) {
                fieldBuffer << uint32(m_floatValues[index]);
            }
            // Gamemasters should be always able to select units - remove not
            // selectable flag
            else if (index == UNIT_FIELD_FLAGS) {
                cacheValue.posPointers.UnitFieldFlagsPos =
                    int32(fieldBuffer.wpos());
            ...
            // creatures
                ...
                cacheValue.posPointers.UnitFieldDisplayPos =
                    int32(fieldBuffer.wpos());
                fieldBuffer << m_uint32Values[UNIT_FIELD_DISPLAYID];
            }
            else if (index == UNIT_DYNAMIC_FLAGS) {
                cacheValue.posPointers.UnitDynamicFlagsPos =
                    int32(fieldBuffer.wpos());
                uint32 dynamicFlags =
                    m_uint32Values[UNIT_DYNAMIC_FLAGS] &
                    ~(UNIT_DYNFLAG_TAPPED | UNIT_DYNFLAG_TAPPED_BY_PLAYER);
                fieldBuffer << dynamicFlags;
            }
            else if (index == UNIT_FIELD_BYTES_2) {
                cacheValue.posPointers.UnitFieldBytes2Pos =
                    int32(fieldBuffer.wpos());
                ...
                if (sScriptMgr->ShouldTrackValuesUpdatePosByIndex(
                        this, updateType, index))
                    cacheValue.posPointers.other[index] =
                        static_cast<uint32>(fieldBuffer.wpos());
    ...
    _valuesUpdateCache.insert(std::pair<uint64, BuildValuesCachedBuffer>(
        cacheKey, std::move(cacheValue)));
}
                             ...
                             Player*                      target)
{
    ...
    if (creature && posPointers.UnitNPCFlagsPos >= 0) {
        ...
        if (sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 2 &&
            ...
            appendValue |=
                UNIT_NPC_FLAG_GOSSIP; // flight masters need NPC gossip flag to
                                      // show instant flight toggle option
    ...
    if (posPointers.UnitFieldAuraStatePos >= 0)
        valuesUpdateBuf.put(posPointers.UnitFieldAuraStatePos,
                            uint32(BuildAuraStateUpdateForTarget(target)));
    ...
    if (posPointers.UnitFieldDisplayPos >= 0) {
        ...
        if (creature) {
            ...
            if (SpellInfo const* transform =
                    sSpellMgr->GetSpellInfo(getTransForm()))
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    if (transform->Effects[i].IsAura(SPELL_AURA_TRANSFORM))
                        if (CreatureTemplate const* transformInfo =
                                sObjectMgr->GetCreatureTemplate(
                                    transform->Effects[i].MiscValue)) {
                            cinfo = transformInfo;
                            break;
                        }
            ...
            if (cinfo->flags_extra & CREATURE_FLAG_EXTRA_TRIGGER) {
                if (target->IsGameMaster() &&
                    target->GetSession()->IsGMAccount()) {
                    if (cinfo->Modelid1)
                        displayId = cinfo->Modelid1; // Modelid1 is a visible
                                                     // model for gms
                    else
                    ...
                    if (cinfo->Modelid2)
                        displayId = cinfo->Modelid2; // Modelid2 is an invisible
                                                     // model for players
                    else
    ...
    if (posPointers.UnitDynamicFlagsPos >= 0) {
        ...
        // unit UNIT_DYNFLAG_TRACK_UNIT should only be sent to caster of
        // SPELL_AURA_MOD_STALKED auras
        if (dynamicFlags & UNIT_DYNFLAG_TRACK_UNIT)
            if (!HasAuraTypeWithCaster(SPELL_AURA_MOD_STALKED,
                                       target->GetGUID()))
    ...
    if (posPointers.UnitFieldBytes2Pos >= 0) {
        if (IsControlledByPlayer() && target != this &&
            sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) &&
            IsInRaidWith(target)) {
            FactionTemplateEntry const* ft1 = GetFactionTemplateEntry();
            FactionTemplateEntry const* ft2 = target->GetFactionTemplateEntry();
            if (ft1 && ft2 && !ft1->IsFriendlyTo(*ft2))
                // Allow targetting opposite faction in party when enabled in
                // config
                valuesUpdateBuf.put(
                    posPointers.UnitFieldBytes2Pos,
                    (m_uint32Values[UNIT_FIELD_BYTES_2] & ((UNIT_BYTE2_FLAG_SANCTUARY /*| UNIT_BYTE2_FLAG_AURAS | UNIT_BYTE2_FLAG_UNK5*/)
                                                           << 8))); // this flag
                                                                    // is at
                                                                    // uint8
                                                                    // offset 1
        ...
        else if (target->IsSpectator() && target->FindMap() &&
                 target->FindMap()->IsBattleArena() &&
                 (this->GetTypeId() == TYPEID_PLAYER ||
                  this->GetTypeId() == TYPEID_UNIT ||
                  this->GetTypeId() == TYPEID_DYNAMICOBJECT)) {
            valuesUpdateBuf.put(posPointers.UnitFieldBytes2Pos,
                                (m_uint32Values[UNIT_FIELD_BYTES_2] &
                                 0xFFFFF2FF)); // clear UNIT_BYTE2_FLAG_PVP,
    ...
    if (posPointers.UnitFieldFactionTemplatePos >= 0) {
        if (IsControlledByPlayer() && target != this &&
            ...
            FactionTemplateEntry const* ft2 = target->GetFactionTemplateEntry();
            if (ft1 && ft2 && !ft1->IsFriendlyTo(*ft2))
                // pretend that all other HOSTILE players have own faction, to
                // allow follow, heal, rezz (trade wont work)
                               ...
                               PacketCooldowns const& cooldowns)
{
    ...
    data << GetGUID();
    data << uint8(flags);
    for (std::unordered_map<uint32, uint32>::const_iterator itr =
             cooldowns.begin();
         itr != cooldowns.end();
         ++itr) {
...
{
    if (CreatureDisplayInfoEntry const* display =
            sCreatureDisplayInfoStore.LookupEntry(modelId)) {
        if (CreatureDisplayInfoExtraEntry const* displayExtra =
...
bool Unit::IsInCombatWith(Unit const* who) const
{
    ...
    for (ThreatContainer::StorageType::const_iterator i =
             m_ThreatMgr.GetThreatList().begin();
         i != m_ThreatMgr.GetThreatList().end();
         ++i) {
...
/**
 * @brief this method gets the diameter of a Unit by DB if any value is defined,
 * otherwise it gets the value by the DBC
 *
 * If the player is mounted the diameter also takes in consideration the mount
 * size
 *
...
float Unit::GetCollisionWidth() const
{
    ...
    CreatureDisplayInfoEntry const* displayInfo =
        sCreatureDisplayInfoStore.AssertEntry(GetNativeDisplayId());
    CreatureModelDataEntry const* modelData =
        sCreatureModelDataStore.AssertEntry(displayInfo->ModelId);

    if (IsMounted()) {
        if (CreatureDisplayInfoEntry const* mountDisplayInfo =
                sCreatureDisplayInfoStore.LookupEntry(
                    GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID))) {
            if (CreatureModelDataEntry const* mountModelData =
                    sCreatureModelDataStore.LookupEntry(
                        mountDisplayInfo->ModelId)) {
                if (G3D::fuzzyGt(mountModelData->CollisionWidth,
                                 modelData->CollisionWidth))
                    modelData = mountModelData;
            }
        }
    }

    float collisionWidth = scaleMod * modelData->CollisionWidth *
                           modelData->Scale * displayInfo->scale * 2;
    // if the objectSize is the default value or the creature is mounted and we
    // have a DBC value, then we can retrieve DBC value instead
    return G3D::fuzzyGt(collisionWidth, 0.0f) &&
                   (G3D::fuzzyEq(objectSize, defaultSize) || IsMounted())
               ? collisionWidth
               : objectSize;
}

/**
 * @brief this method gets the radius of a Unit by DB if any value is defined,
 * otherwise it gets the value by the DBC
 *
 * If the player is mounted the radius also takes in consideration the mount
 * size
 *
 * @return float The radius of a unit
 */
float Unit::GetCollisionRadius() const { return GetCollisionWidth() / 2; }
        ...
        if (CreatureDisplayInfoEntry const* mountDisplayInfo =
            ...
            if (CreatureModelDataEntry const* mountModelData =
                    sCreatureModelDataStore.LookupEntry(
                        mountDisplayInfo->ModelId)) {
                collisionHeight =
                    scaleMod * (mountModelData->MountHeight +
                                modelData->CollisionHeight * modelData->Scale *
                                    displayInfo->scale * 0.5f);
            }
        }
    }
    else
        collisionHeight = scaleMod * modelData->CollisionHeight *
                          modelData->Scale * displayInfo->scale;
                ...
                WorldObject const* target)
{
    ...
    Acore::PlayerDistWorker<
        Acore::LocalizedPacketDo<Acore::CustomChatTextBuilder>>
        worker(this, textRange, localizer);
...
{
    Talk(text,
         isBossEmote ? CHAT_MSG_RAID_BOSS_EMOTE : CHAT_MSG_MONSTER_EMOTE,
         LANG_UNIVERSAL,
                   ...
                   bool             isBossWhisper /*= false*/)
{
    ...
    ChatHandler::BuildChatPacket(data,
                                 isBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER
                ...
                WorldObject const* target)
{
    if (!sObjectMgr->GetBroadcastText(textId)) {
        LOG_ERROR("entities.unit",
                  "Unit::Talk: `broadcast_text` (ID: {}) was not found",
                  textId);
        return;
    }

    Acore::BroadcastTextBuilder builder(
        this, msgType, textId, getGender(), target);
    Acore::LocalizedPacketDo<Acore::BroadcastTextBuilder> localizer(builder);
    Acore::PlayerDistWorker<
        Acore::LocalizedPacketDo<Acore::BroadcastTextBuilder>>
        worker(this, textRange, localizer);
    Cell::VisitWorldObjects(this, worker, textRange);
}
                   ...
                   bool    isBossWhisper /*= false*/)
{
    ...
    if (!bct) {
        LOG_ERROR("entities.unit",
                  "Unit::Whisper: `broadcast_text` was not {} found",
                  textId);
        return;
    }
...
{
    if (SpellInfo const* transformSpellInfo =
            sSpellMgr->GetSpellInfo(getTransForm())) {
    ...
    CreatureDisplayInfoEntry const* display =
        sCreatureDisplayInfoStore.LookupEntry(GetDisplayId());
    if (!display) {
        return true;
    }

    CreatureDisplayInfoExtraEntry const* displayExtra =
        sCreatureDisplayInfoExtraStore.LookupEntry(
            display->ExtendedDisplayInfoID);
    if (!displayExtra) {
        return true;
    ...
    CreatureModelDataEntry const* model =
        sCreatureModelDataStore.LookupEntry(display->ModelId);
    ChrRacesEntry const* race =
        sChrRacesStore.LookupEntry(displayExtra->DisplayRaceID);
...
std::string Unit::GetDebugInfo() const
{
    ...
    sstr << WorldObject::GetDebugInfo() << "\n"
         << std::boolalpha << "AliveState: " << IsAlive()
         << " UnitMovementFlags: " << GetUnitMovementFlags()
         << " ExtraUnitMovementFlags: " << GetExtraUnitMovementFlags()
