...
bool Player::UpdateStats(Stats stat)
{
    ...
    if (stat == STAT_STRENGTH) {
        ...
        if (HasAuraTypeWithMiscvalue(
                SPELL_AURA_MOD_RANGED_ATTACK_POWER_OF_STAT_PERCENT, stat))
        ...
        if (HasAuraTypeWithMiscvalue(
                SPELL_AURA_MOD_ATTACK_POWER_OF_STAT_PERCENT, stat))
            UpdateAttackPowerAndDamage(false);
        if (HasAuraTypeWithMiscvalue(
                SPELL_AURA_MOD_RANGED_ATTACK_POWER_OF_STAT_PERCENT, stat))
            UpdateAttackPowerAndDamage(true);
    ...
    // Update ratings in exist SPELL_AURA_MOD_RATING_FROM_STAT and only depends
    // from stat
    uint32                mask = 0;
...
bool Player::UpdateAllStats()
{
    ...
    UpdateArmor();
    // calls UpdateAttackPowerAndDamage() in UpdateArmor for
...
void Player::UpdateResistances(uint32 school)
{
    if (school > SPELL_SCHOOL_NORMAL) {
        ...
        AuraEffectList const& mResbyIntellect =
            GetAuraEffectsByType(SPELL_AURA_MOD_RESISTANCE_OF_STAT_PERCENT);
        for (AuraEffectList::const_iterator i = mResbyIntellect.begin();
             i != mResbyIntellect.end();
             ++i) {
            if ((*i)->GetMiscValue() & (1 << (school - 1)))
                value += int32(GetStat(Stats((*i)->GetMiscValueB())) *
                               (*i)->GetAmount() / 100.0f);
        }
...
void Player::UpdateArmor()
{
    ...
    float value =
        GetModifierValue(unitMod, BASE_VALUE);    // base armor (from items)
    ...
    for (AuraEffectList::const_iterator i = mResbyIntellect.begin();
        ...
        if ((*i)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL)
            value += CalculatePct(GetStat(Stats((*i)->GetMiscValueB())),
                                  (*i)->GetAmount());
    }
    ...
    UpdateAttackPowerAndDamage(); // armor dependent auras update for
                                  // SPELL_AURA_MOD_ATTACK_POWER_OF_ARMOR
}
...
{
    UnitMods unitMod =
        UnitMods(static_cast<uint16>(UNIT_MOD_POWER_START) + power);

    float bonusPower = (power == POWER_MANA && GetCreatePowers(power) > 0)
...
void Player::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (ranged) {
        ...
        else if (IsClass(CLASS_ROGUE, CLASS_CONTEXT_STATS) ||
                 IsClass(CLASS_WARRIOR, CLASS_CONTEXT_STATS)) {
        ...
        if (IsClass(CLASS_PALADIN, CLASS_CONTEXT_STATS) ||
            IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_STATS) ||
            IsClass(CLASS_WARRIOR, CLASS_CONTEXT_STATS)) {
            val2 = level * 3.0f + GetStat(STAT_STRENGTH) * 2.0f - 20.0f;
        }
        else if (IsClass(CLASS_HUNTER, CLASS_CONTEXT_STATS) ||
                 IsClass(CLASS_SHAMAN, CLASS_CONTEXT_STATS) ||
                 IsClass(CLASS_ROGUE, CLASS_CONTEXT_STATS)) {
            ...
            if (IsInFeralForm()) {
                Unit::AuraEffectList const& mDummy =
                    GetAuraEffectsByType(SPELL_AURA_DUMMY);
                for (Unit::AuraEffectList::const_iterator itr = mDummy.begin();
                     itr != mDummy.end();
                     ++itr) {
                    ...
                    if (aurEff->GetSpellInfo()->SpellIconID == 1563) {
                        ...
                        case 0: // Predatory Strikes (effect 0)
                            mLevelMult =
                                CalculatePct(1.0f, aurEff->GetAmount());
                            break;
                        case 1: // Predatory Strikes (effect 1)
                            if (Item* mainHand =
                                    m_items[EQUIPMENT_SLOT_MAINHAND]) {
                                // also gains % attack power from equipped
                                // weapon
                                ...
                                for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS;
                                     ++i) {
                                ...
                                for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS;
                                     ++i) {
                                    // no spell
                                    ...
                                    SpellInfo const* spellproto =
                                        sSpellMgr->GetSpellInfo(
                                            proto->Spells[i].SpellId);
                                    if (!spellproto)
                                        continue;

                                    for (uint8 j = 0; j < MAX_SPELL_EFFECTS;
                                         ++j)
                                        if (spellproto->Effects[j]
                                                .ApplyAuraName ==
                                            SPELL_AURA_MOD_ATTACK_POWER)
                                            ap += spellproto->Effects[j]
                                                      .CalcValue();
                                }

                                weapon_bonus = CalculatePct(
                                    float(ap), aurEff->GetAmount());
                            }
                            break;
                        default:
                            break;
        ...
        else if (IsClass(CLASS_MAGE, CLASS_CONTEXT_STATS) ||
                 IsClass(CLASS_PRIEST, CLASS_CONTEXT_STATS) ||
    ...
    if (ranged) {
        if ((getClassMask() & CLASSMASK_WAND_USERS) == 0) {
            AuraEffectList const& mRAPbyStat = GetAuraEffectsByType(
                SPELL_AURA_MOD_RANGED_ATTACK_POWER_OF_STAT_PERCENT);
            for (AuraEffectList::const_iterator i = mRAPbyStat.begin();
                 i != mRAPbyStat.end();
                 ++i)
                attPowerMod += CalculatePct(
                    GetStat(Stats((*i)->GetMiscValue())), (*i)->GetAmount());
        }
    }
    else {
        AuraEffectList const& mAPbyStat =
            GetAuraEffectsByType(SPELL_AURA_MOD_ATTACK_POWER_OF_STAT_PERCENT);
        for (AuraEffectList::const_iterator i = mAPbyStat.begin();
             i != mAPbyStat.end();
             ++i)
            attPowerMod += CalculatePct(GetStat(Stats((*i)->GetMiscValue())),
                                        (*i)->GetAmount());

        AuraEffectList const& mAPbyArmor =
            GetAuraEffectsByType(SPELL_AURA_MOD_ATTACK_POWER_OF_ARMOR);
        for (AuraEffectList::const_iterator iter = mAPbyArmor.begin();
             iter != mAPbyArmor.end();
             ++iter)
            // always: ((*i)->GetModifier()->m_miscvalue == 1 ==
    ...
    sScriptMgr->OnAfterUpdateAttackPowerAndDamage(
        this, level, base_attPower, attPowerMod, attPowerMultiplier, ranged);
    SetInt32Value(
        index, (uint32)base_attPower); // UNIT_FIELD_(RANGED)_ATTACK_POWER field
    SetInt32Value(
        index_mod,
        (uint32)attPowerMod); // UNIT_FIELD_(RANGED)_ATTACK_POWER_MODS field
    SetFloatValue(index_mult, attPowerMultiplier); // UNIT_FIELD_(RANGED)_ATTACK_POWER_MULTIPLIER
                                                   // field
    ...
    if (ranged) {
        ...
        if (CanDualWield() &&
            haveOffhandWeapon()) // allow update offhand damage only if player
                                 // knows DualWield Spec and has equipped
                                 // offhand weapon
            UpdateDamagePhysical(OFF_ATTACK);
        if (IsClass(CLASS_SHAMAN, CLASS_CONTEXT_STATS) ||
            IsClass(CLASS_PALADIN, CLASS_CONTEXT_STATS)) // mental quickness
            UpdateSpellDamageAndHealingBonus();
    }
}
                                   ...
                                   uint8            damageIndex)
{
    ...
    float baseValue =
        ...
        GetTotalAttackPowerValue(attType) / 14.0f * attackSpeedMod;
    ...
    else if (!CanUseAttackType(attType)) // check if player not in form but
                                         // still can't use (disarm case)
    {
    ...
    minDamage =
        ((weaponMinDamage + baseValue) * basePct + totalValue) * totalPct;
    maxDamage =
        ((weaponMaxDamage + baseValue) * basePct + totalValue) * totalPct;

    // pussywizard: crashfix (casting negative to uint => min > max => assertion
    // in urand)
...
void Player::UpdateBlockPercentage()
{
    ...
    if (CanBlock()) {
        ...
        if (sConfigMgr->GetOption<bool>("Stats.Limits.Enable", false)) {
            value =
                value > sConfigMgr->GetOption<float>("Stats.Limits.Block",
                                                     95.0f)
                    ? sConfigMgr->GetOption<float>("Stats.Limits.Block", 95.0f)
                    : value;
        }
...
void Player::UpdateCritPercentage(WeaponAttackType attType)
{
    ...
    if (sConfigMgr->GetOption<bool>("Stats.Limits.Enable", false)) {
        value = value > sConfigMgr->GetOption<float>("Stats.Limits.Crit", 95.0f)
                    ? sConfigMgr->GetOption<float>("Stats.Limits.Crit", 95.0f)
                    : value;
    }
...
float Player::GetMissPercentageFromDefence() const
{
    ...
    // Modify value from defense skill (only bonus from defense rating
    // diminishes)
    nondiminishing +=
        (GetSkillValue(SKILL_DEFENSE) - GetMaxSkillValueForLevel()) * 0.04f;
    diminishing += (int32(GetRatingBonusValue(CR_DEFENSE_SKILL))) * 0.04f;

    // apply diminishing formula to diminishing miss chance
    uint32 pclass = getClass() - 1;
    return nondiminishing +
           (diminishing * miss_cap[pclass] /
            (diminishing + miss_cap[pclass] * m_diminishing_k[pclass]));
}
...
void Player::UpdateParryPercentage()
{
    ...
    if (CanParry() && parry_cap[pclass] > 0.0f) {
        ...
        if (sConfigMgr->GetOption<bool>("Stats.Limits.Enable", false)) {
            value =
                value > sConfigMgr->GetOption<float>("Stats.Limits.Parry",
                                                     95.0f)
...
void Player::UpdateDodgePercentage()
{
    ...
    uint32 pclass = getClass() - 1;
    m_realDodge   = nondiminishing +
                  (diminishing * dodge_cap[pclass] /
    ...
    if (sConfigMgr->GetOption<bool>("Stats.Limits.Enable", false)) {
        value =
            value > sConfigMgr->GetOption<float>("Stats.Limits.Dodge", 95.0f)
...
{
    // Store Rating Value
    SetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 +
                       static_cast<uint16>(CR_ARMOR_PENETRATION),
                   amount);
}
...
{
    m_modMeleeHitChance =
        (float)GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE);
...
{
    m_modSpellHitChance =
        (float)GetTotalAuraModifier(SPELL_AURA_MOD_SPELL_HIT_CHANCE);
...
void Player::UpdateExpertise(WeaponAttackType attack)
{
    ...
    AuraEffectList const& expAuras =
        GetAuraEffectsByType(SPELL_AURA_MOD_EXPERTISE);
    for (AuraEffectList::const_iterator itr = expAuras.begin();
         itr != expAuras.end();
         ++itr) {
        // item neutral spell
        ...
        // item dependent spell
                 ...
                 weapon->IsFitToSpellRequirements((*itr)->GetSpellInfo()))
            expertise += (*itr)->GetAmount();
    }
...
void Player::UpdateManaRegen()
{
    ...
    AuraEffectList const& regenAura =
        GetAuraEffectsByType(SPELL_AURA_MOD_MANA_REGEN_FROM_STAT);
    for (AuraEffectList::const_iterator i = regenAura.begin();
         i != regenAura.end();
         ++i) {
        power_regen_mp5 +=
            GetStat(Stats((*i)->GetMiscValue())) * (*i)->GetAmount() / 500.0f;
    }
...
void Creature::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    float baseAttackPower = GetModifierValue(unitMod, BASE_VALUE) *
                            GetModifierValue(unitMod, BASE_PCT);
    ...
    SetInt32Value(index,
                  uint32(baseAttackPower)); // UNIT_FIELD_(RANGED)_ATTACK_POWER
    SetInt32Value(
        indexMod,
        uint32(attackPowerMod)); // UNIT_FIELD_(RANGED)_ATTACK_POWER_MODS
    SetFloatValue(
        indexMulti,
        attackPowerMultiplier); // UNIT_FIELD_(RANGED)_ATTACK_POWER_MULTIPLIER
                                     ...
                                     uint8            damageIndex /*= 0*/)
{
    ...
    float baseValue        = GetModifierValue(unitMod, BASE_VALUE) +
                      (attackPower / 14.0f) * variance;
    float basePct    = GetModifierValue(unitMod, BASE_PCT) * attackSpeedMulti;
    float totalValue = GetModifierValue(unitMod, TOTAL_VALUE);
    float totalPct = addTotalPct ? GetModifierValue(unitMod, TOTAL_PCT) : 1.0f;
    float dmgMultiplier =
        GetCreatureTemplate()
            ->DamageModifier; // = DamageModifier * _GetDamageMod(rank);

    minDamage =
        ((weaponMinDamage + baseValue) * dmgMultiplier * basePct + totalValue) *
        totalPct;
    maxDamage =
...
void Guardian::UpdateArmor()
{
    ...
    value += std::max<float>(
                 GetStat(STAT_AGILITY) - GetCreateStat(STAT_AGILITY), 0.0f) *
             2.0f;
...
{
    UnitMods unitMod = UNIT_MOD_HEALTH;
    float    stamina = std::max<float>(
        GetStat(STAT_STAMINA) - GetCreateStat(STAT_STAMINA), 0.0f);
    ...
    float value = GetModifierValue(
        unitMod,
        BASE_VALUE); // xinef: Do NOT add base health TWICE + GetCreateHealth();
    value *= GetModifierValue(unitMod, BASE_PCT);
...
void Guardian::UpdateMaxPower(Powers power)
{
    ...
    float addValue =
        (power == POWER_MANA)
            ? std::max<float>(
                  GetStat(STAT_INTELLECT) - GetCreateStat(STAT_INTELLECT), 0.0f)
            : 0.0f;
    ...
    float value = GetModifierValue(unitMod, BASE_VALUE) +
                  (power != POWER_MANA ? GetCreatePowers(power) : 0);
    value *= GetModifierValue(unitMod, BASE_PCT);
...
void Guardian::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (GetEntry() == NPC_IMP) // imp's attack power
        ...
        val = 589 /*xinef: base ap!*/ + GetStat(STAT_STRENGTH) +
