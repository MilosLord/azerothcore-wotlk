...
void Player::ResetInstances(ObjectGuid guid, uint8 method, bool isRaid)
{
    ...
    case INSTANCE_RESET_ALL: {
        ...
        BoundInstancesMap const&   m_boundInstances =
            sInstanceSaveMgr->PlayerGetBoundInstances(
                p->GetGUID(), Difficulty(DUNGEON_DIFFICULTY_NORMAL));
    ...
    case INSTANCE_RESET_CHANGE_DIFFICULTY: {
        ...
        std::vector<InstanceSave*> toUnbind;
        BoundInstancesMap const&   m_boundInstances =
            sInstanceSaveMgr->PlayerGetBoundInstances(p->GetGUID(),
                                                      p->GetDifficulty(isRaid));
        for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
             itr != m_boundInstances.end();
             ++itr) {
            InstanceSave*   instanceSave = itr->second.save;
    ...
    case INSTANCE_RESET_GROUP_JOIN: {
        ...
        for (uint8 d = 0; d < MAX_DIFFICULTY; ++d) {
            ...
            BoundInstancesMap const&   m_boundInstances =
                sInstanceSaveMgr->PlayerGetBoundInstances(p->GetGUID(),
                                                          Difficulty(d));
            for (BoundInstancesMap::const_iterator itr =
                     m_boundInstances.begin();
                 itr != m_boundInstances.end();
                 ++itr) {
                if (itr->second.perm)
                    continue;
            ...
            for (std::vector<InstanceSave*>::const_iterator itr =
                     toUnbind.begin();
                 itr != toUnbind.end();
                 ++itr)
                sInstanceSaveMgr->PlayerUnbindInstance(p->GetGUID(),
                                                       (*itr)->GetMapId(),
                                                       (*itr)->GetDifficulty(),
                                                       true,
                                                       p);
        ...
        for (uint8 d = 0; d < MAX_DIFFICULTY; ++d) {
            ...
            for (std::vector<InstanceSave*>::const_iterator itr =
                ...
                sInstanceSaveMgr->PlayerUnbindInstance(
                    guid, (*itr)->GetMapId(), (*itr)->GetDifficulty(), true, p);
        }
    } break;
    }
}
...
void Player::UpdatePvPFlag(time_t currTime)
{
    ...
    if (currTime < (pvpInfo.EndTimer + 300 + 5)) {
        if (currTime > (pvpInfo.EndTimer + 4) &&
            !HasPlayerFlag(PLAYER_FLAGS_PVP_TIMER))
...
{
    if (!IsFFAPvP() || sWorld->IsFFAPvPRealm() || !pvpInfo.FFAPvPEndTimer ||
        currTime < pvpInfo.FFAPvPEndTimer + 30) {
        return;
    }
...
{
    if (duel && duel->State == DUEL_STATE_COUNTDOWN &&
    ...
    std::vector<std::pair<uint32, uint32>> mailItems)
{
    if (mailItems.empty()) {
        ...
        LOG_ERROR("entities.player.items",
                  "> SendItemRetrievalMail: Attempt to send almost with items "
                  "without items. Player {}",
                  GetGUID().ToString());
        return;
    }
    ...
    auto AddMailItem = [&allItems](uint32 itemEntry, uint32 itemCount) {
        ...
        if (!itemTemplate) {
            LOG_ERROR("entities.player.items",
                      "> SendItemRetrievalMail: Item id {} is invalid",
                      itemEntry);
            return;
        }

        if (itemCount < 1 ||
            (itemTemplate->MaxCount > 0 &&
             itemCount > static_cast<uint32>(itemTemplate->MaxCount))) {
            LOG_ERROR("entities.player.items",
                      "> SendItemRetrievalMail: Incorrect item count ({}) for "
                      "item id {}",
                      itemEntry,
                      itemCount);
            return;
        ...
        while (itemCount > itemTemplate->GetMaxStackSize()) {
            if (toSendItems.size() <= MAX_MAIL_ITEMS) {
                toSendItems.emplace_back(itemEntry,
    ...
    for (auto const& items : allItems) {
        ...
        MailDraft  draft("Recovered Item",
                        "We recovered a lost item in the twisting nether and "
                         "noted that it was yours.$B$BPlease find said object "
                         "enclosed."); // This is the text used in Cataclysm, it
                                       // probably wasn't changed.
