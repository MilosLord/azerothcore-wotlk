...
// 2^n values, Player::m_isunderwater is a bitmask. These are Trinity internal
...
struct PlayerSpell {
    PlayerSpellState
         State : 7;  // UPPER CASE TO CAUSE CONSOLE ERRORS (CHECK EVERY USAGE)!
    bool Active : 1; // UPPER CASE TO CAUSE CONSOLE ERRORS (CHECK EVERY USAGE)!
                     // lower rank of a spell are not useable, but learnt
    uint8 specMask : 8;
...
struct PlayerTalent {
    PlayerSpellState
           State : 8; // UPPER CASE TO CAUSE CONSOLE ERRORS (CHECK EVERY USAGE)!
    uint8  specMask : 8;
    uint32 talentID;
    bool   inSpellBook;
    bool   IsInSpec(uint8 spec) { return (specMask & (1 << spec)); }
...
struct SpellModifier {
    SpellModifier(Aura* _ownerAura = nullptr)
        : op(SPELLMOD_DAMAGE), type(SPELLMOD_FLAT), charges(0), mask(),
...
struct ActionButton {
    ...
    // helpers
    [[nodiscard]] ActionButtonType GetType() const
    {
...
struct PvPInfo {
    ...
    bool IsInHostileArea{
        false}; ///> Marks if player is in an area which forces PvP flag
    bool IsInNoPvPArea{
        false}; ///> Marks if player is in a sanctuary or friendly capital city
    bool IsInFFAPvPArea{false}; ///> Marks if player is in an FFAPvP area (such
                                ///as Gurubashi Arena)
    time_t EndTimer{0}; ///> Time when player unflags himself for PvP (flag
                        ///removed after 5 minutes)
    time_t FFAPvPEndTimer{0}; ///> Time when player unflags himself for FFA PvP
                              ///(flag removed after 30 sec)
};
...
struct DuelInfo {
    DuelInfo(Player* opponent, Player* initiator, bool isMounted)
        : Opponent(opponent), Initiator(initiator), IsMounted(isMounted)
    {
    }
...
enum PlayerFlags : uint32 {
    ...
    PLAYER_ALLOW_ONLY_ABILITY =
        0x00800000, // used by bladestorm and killing spree, allowed only spells
                    // with SPELL_ATTR0_USES_RANGED_SLOT, SPELL_EFFECT_ATTACK,
                    // checked only for active player
    PLAYER_FLAGS_UNK24 =
        0x01000000, // disabled all melee ability on tab include autoattack
...
static_assert((PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID & 1) == 0,
              "PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID must be aligned "
              "to 2 byte boundary");
...
enum PlayerExtraFlags {
    ...
    PLAYER_EXTRA_HAS_310_FLYER =
        0x0040, // Marks if player already has 310% speed flying mount
    ...
    PLAYER_EXTRA_SHOW_DK_PET =
        0x0400, // Marks if player should see ghoul on login screen
};
...
struct ItemPosCount {
    ...
    [[nodiscard]] bool
           isContainedIn(std::vector<ItemPosCount> const& vec) const;
    uint16 pos;
    uint32 count;
};
...
enum TransferAbortReason {
    ...
    TRANSFER_ABORT_DIFFICULTY =
        0x08, // <Normal, Heroic, Epic> difficulty mode is not available for %s.
...
struct EntryPointData {
    ...
    [[nodiscard]] bool HasTaxiPath() const
    {
...
public:
    ...
    void SetObjectScale(float scale) override
    {
        ...
        SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS,
                      scale * DEFAULT_WORLD_OBJECT_SIZE);
        SetFloatValue(UNIT_FIELD_COMBATREACH, scale * DEFAULT_COMBAT_REACH);
    }

    [[nodiscard]] bool hasSpanishClient()
    {
        return GetSession()->GetSessionDbLocaleIndex() == LOCALE_esES ||
               GetSession()->GetSessionDbLocaleIndex() == LOCALE_esMX;
    }

    bool TeleportTo(uint32 mapid,
                    float  x,
                    float  y,
                    float  z,
                    float  orientation,
    ...
    bool TeleportTo(WorldLocation const& loc,
                    uint32               options = 0,
                    Unit*                target  = nullptr)
    {
    ...
    void               SetSummonPoint(uint32 mapid,
                                      float  x,
                                      float  y,
                                      float  z,
                                      uint32 delay       = 0,
    ...
    [[nodiscard]] time_t GetSummonExpireTimer() const
    {
        return m_summon_expire;
    }
    ...
    {
        return PlayerFlags(GetUInt32Value(PLAYER_FLAGS));
    }
    bool HasPlayerFlag(PlayerFlags flags) const
    {
        return HasFlag(PLAYER_FLAGS, flags) != 0;
    }
    void SetPlayerFlag(PlayerFlags flags) { SetFlag(PLAYER_FLAGS, flags); }
    void RemovePlayerFlag(PlayerFlags flags)
    {
        RemoveFlag(PLAYER_FLAGS, flags);
    }
    ...
    [[nodiscard]] bool
    IsClass(Classes      playerClass,
            ClassContext context = CLASS_CONTEXT_NONE) const override;

    void SetInWater(bool apply);

    [[nodiscard]] bool IsInWater() const override { return m_isInWater; }
    [[nodiscard]] bool IsFalling() const;
    bool               IsInAreaTriggerRadius(AreaTrigger const* trigger,
                                             float              delta = 0.f) const;

    void SendInitialPacketsBeforeAddToMap();
    void SendInitialPacketsAfterAddToMap();
    void SendTransferAborted(uint32              mapid,
                             TransferAbortReason reason,
                             uint8               arg = 0);
    void SendInstanceResetWarning(uint32     mapid,
    ...
    GetGameObjectIfCanInteractWith(ObjectGuid guid, GameobjectTypes type) const;
    ...
    uint32 GetBarberShopCost(uint8                       newhairstyle,
                             uint8                       newhaircolor,
                             uint8                       newfacialhair,
    ...
    [[nodiscard]] bool IsCommentator() const
    {
        return HasPlayerFlag(PLAYER_FLAGS_COMMENTATOR2);
    }
    void SetCommentator(bool on)
    {
        ApplyModFlag(PLAYER_FLAGS, PLAYER_FLAGS_COMMENTATOR2, on);
    }
    [[nodiscard]] bool IsDeveloper() const
    {
        return HasPlayerFlag(PLAYER_FLAGS_DEVELOPER);
    }
    void SetDeveloper(bool on)
    {
        ApplyModFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER, on);
    }
    [[nodiscard]] bool isAcceptWhispers() const
    {
    ...
    [[nodiscard]] bool IsGameMaster() const
    {
        return m_ExtraFlags & PLAYER_EXTRA_GM_ON;
    }
    void               SetGameMaster(bool on);
    [[nodiscard]] bool isGMChat() const
    {
    ...
    [[nodiscard]] bool isTaxiCheater() const
    {
    ...
    [[nodiscard]] bool isGMVisible() const
    {
    ...
    {
        if (on)
            m_ExtraFlags |= PLAYER_EXTRA_HAS_310_FLYER;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_HAS_310_FLYER;
    }
    ...
    [[nodiscard]] bool GetCommandStatus(uint32 command) const
    {
    ...
    void setDeathState(DeathState s, bool despawn = false)
        override; // overwrite Unit::setDeathState
    ...
    [[nodiscard]] bool HasRestFlag(RestFlag restFlag) const
    {
    ...
    [[nodiscard]] PetStable const* GetPetStable() const
    {
        return m_petStable.get();
    }

    [[nodiscard]] Pet* GetPet() const;
    Pet*               SummonPet(uint32       entry,
                                 float        x,
                                 float        y,
                                 float        z,
                                 float        ang,
    ...
    [[nodiscard]] uint32 GetPhaseMaskForSpawn()
        const; // used for proper set phase for DB at GM-mode creature/GO spawn
    ...
    uint8
    FindEquipSlot(ItemTemplate const* proto, uint32 slot, bool swap) const;
    uint32               GetItemCount(uint32 item,
                                      bool   inBankAlso = false,
                                      Item*  skipItem   = nullptr) const;
    uint32               GetItemCountWithLimitCategory(uint32 limitCategory,
                                                       Item*  skipItem = nullptr) const;
    ...
    [[nodiscard]] inline Item* GetUseableItemByPos(uint8 bag, uint8 slot)
        const // Does additional check for disarmed weapons
    {
        if (!CanUseAttackType(GetAttackBySlot(slot)))
            return nullptr;
        return GetItemByPos(bag, slot);
    }
    [[nodiscard]] Item* GetWeaponForAttack(WeaponAttackType attackType,
    ...
    [[nodiscard]] uint8 GetBankBagSlotCount() const
    {
    ...
    [[nodiscard]] bool
    HasItemCount(uint32 item, uint32 count = 1, bool inBankAlso = false) const;
    bool               HasItemFitToSpellRequirements(SpellInfo const* spellInfo,
                                                     Item const* ignoreItem = nullptr) const;
    bool               CanNoReagentCast(SpellInfo const* spellInfo) const;
    [[nodiscard]] bool HasItemOrGemWithIdEquipped(
        uint32 item, uint32 count, uint8 except_slot = NULL_SLOT) const;
    [[nodiscard]] bool
                    HasItemOrGemWithLimitCategoryEquipped(uint32 limitCategory,
                                                          uint32 count,
                                                          uint8  except_slot = NULL_SLOT) const;
    InventoryResult CanTakeMoreSimilarItems(Item* pItem) const
    {
        return CanTakeMoreSimilarItems(
            pItem->GetEntry(), pItem->GetCount(), pItem);
    }
    [[nodiscard]] InventoryResult CanTakeMoreSimilarItems(uint32 entry,
                                                          uint32 count) const
    {
        return CanTakeMoreSimilarItems(entry, count, nullptr);
    }
    InventoryResult CanStoreNewItem(uint8            bag,
                                    uint8            slot,
    ...
    {
        return CanStoreItem(
            bag, slot, dest, item, count, nullptr, false, no_space_count);
    }
    InventoryResult CanStoreItem(uint8            bag,
                                 uint8            slot,
                                 ItemPosCountVec& dest,
                                 Item*            pItem,
                                 bool             swap = false) const
    {
        ...
        return CanStoreItem(
            bag, slot, dest, pItem->GetEntry(), count, pItem, swap, nullptr);
    }
    InventoryResult CanStoreItems(Item** pItem, int32 count) const;
    InventoryResult
    CanEquipNewItem(uint8 slot, uint16& dest, uint32 item, bool swap) const;
    InventoryResult CanEquipItem(uint8   slot,
                                 uint16& dest,
                                 Item*   pItem,
                                 bool    swap,
    ...
    InventoryResult CanEquipUniqueItem(ItemTemplate const* itemProto,
                                       uint8  except_slot = NULL_SLOT,
                                       uint32 limit_count = 1) const;
    [[nodiscard]] InventoryResult CanUnequipItems(uint32 item,
                                                  uint32 count) const;
    [[nodiscard]] InventoryResult CanUnequipItem(uint16 src, bool swap) const;
    InventoryResult               CanBankItem(uint8            bag,
                                              uint8            slot,
                                              ItemPosCountVec& dest,
                                              Item*            pItem,
                                              bool             swap,
    ...
    bool            IsTotemCategoryCompatiableWith(ItemTemplate const* pProto,
                                                   uint32 requiredTotemCategoryId) const;
    InventoryResult CanUseItem(ItemTemplate const* pItem) const;
    [[nodiscard]] InventoryResult CanUseAmmo(uint32 item) const;
    InventoryResult               CanRollForItemInLFG(ItemTemplate const* item,
                                                      WorldObject const*  lootedObject) const;
    Item*                         StoreNewItem(ItemPosCountVec const& pos,
                                               uint32                 item,
                                               bool                   update,
    ...
    void  AutoStoreLoot(uint8            bag,
                        uint8            slot,
                        uint32           loot_id,
                        LootStore const& store,
                        bool             broadcast = false);
    void  AutoStoreLoot(uint32           loot_id,
                        LootStore const& store,
                        bool             broadcast = false)
    {
    ...
    void MoveItemToInventory(ItemPosCountVec const& dest,
                             Item*                  pItem,
                             bool                   update,
    ...
    void  DestroyItemCount(uint32 item,
                           uint32 count,
                           bool   update,
    ...
    [[nodiscard]] uint32 GetMaxKeyringSize() const
    {
    ...
    void SendEquipError(InventoryResult msg,
                        ...
                        uint32          itemid = 0);
    void
    SendBuyError(BuyResult msg, Creature* creature, uint32 item, uint32 param);
    void SendSellError(SellResult msg,
                       Creature*  creature,
                       ObjectGuid guid,
                       uint32     param);
    ...
    [[nodiscard]] uint32 GetWeaponProficiency() const
    {
        return m_WeaponProficiency;
    }
    [[nodiscard]] uint32 GetArmorProficiency() const
    {
    ...
    bool BuyItemFromVendorSlot(ObjectGuid vendorguid,
                               uint32     vendorslot,
                               uint32     item,
                               uint8      count,
                               uint8      bag,
                               uint8      slot);
    bool _StoreOrEquipNewItem(uint32              vendorslot,
                              uint32              item,
                              uint8               count,
                              uint8               bag,
                              uint8               slot,
    ...
    void
    AddEnchantmentDuration(Item* item, EnchantmentSlot slot, uint32 duration);
    void ApplyEnchantment(Item*           item,
                          EnchantmentSlot slot,
                          bool            apply,
                          bool            apply_dur        = true,
                          bool            ignore_condition = false);
    void ApplyEnchantment(Item* item, bool apply);
    void UpdateSkillEnchantments(uint16 skill_id,
                                 uint16 curr_value,
                                 uint16 new_value);
    ...
    void PrepareGossipMenu(WorldObject* source,
                           uint32       menuId     = 0,
                           bool         showQuests = false);
    void SendPreparedGossip(WorldObject* source);
    void
    OnGossipSelect(WorldObject* source, uint32 gossipListId, uint32 menuId);
    ...
    void RewardQuest(Quest const* quest,
                     uint32       reward,
                     Object*      questGiver,
                     bool         announce    = true,
                     bool         isLFGReward = false);
    ...
    [[nodiscard]] uint32 GetQuestSlotQuestId(uint16 slot) const
    {
        return GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET +
                              QUEST_ID_OFFSET);
    }
    [[nodiscard]] uint32 GetQuestSlotState(uint16 slot) const
    {
        return GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET +
                              QUEST_STATE_OFFSET);
    }
    [[nodiscard]] uint16 GetQuestSlotCounter(uint16 slot, uint8 counter) const
    {
    ...
    [[nodiscard]] uint32 GetQuestSlotTime(uint16 slot) const
    {
        return GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET +
                              QUEST_TIME_OFFSET);
    }
    ...
    {
        uint64 val =
            GetUInt64Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET +
                           QUEST_COUNTS_OFFSET);
    ...
    {
        SetFlag(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET +
                    QUEST_STATE_OFFSET,
                state);
    }
    ...
    void SwapQuestSlot(uint16 slot1, uint16 slot2)
    {
        for (int i = 0; i < MAX_QUEST_OFFSET; ++i) {
            uint32 temp1 = GetUInt32Value(PLAYER_QUEST_LOG_1_1 +
                                          MAX_QUEST_OFFSET * slot1 + i);
            uint32 temp2 = GetUInt32Value(PLAYER_QUEST_LOG_1_1 +
                                          MAX_QUEST_OFFSET * slot2 + i);

            SetUInt32Value(PLAYER_QUEST_LOG_1_1 + MAX_QUEST_OFFSET * slot1 + i,
                           temp2);
            SetUInt32Value(PLAYER_QUEST_LOG_1_1 + MAX_QUEST_OFFSET * slot2 + i,
                           temp1);
        }
    }
    ...
    [[nodiscard]] bool HasQuestForItem(uint32 itemId,
                                       uint32 excludeQuestId = 0,
                                       bool   turnIn         = false,
                                       bool*  showInLoot     = nullptr) const;
    ...
    void
    SendQuestUpdateAddItem(Quest const* quest, uint32 item_idx, uint16 count);
    void SendQuestUpdateAddCreatureOrGo(Quest const* quest,
                                        ObjectGuid   guid,
                                        uint32       creatureOrGO_idx,
                                        uint16       old_count,
                                        uint16       add_count);
    void SendQuestUpdateAddPlayer(Quest const* quest,
                                  uint16       old_count,
                                  uint16       add_count);
    ...
    static void SavePositionInDB(uint32     mapid,
                                 float      x,
                                 float      y,
                                 float      z,
                                 float      o,
                                 uint32     zone,
                                 ObjectGuid guid);
    static void SavePositionInDB(WorldLocation const&         loc,
                                 uint16                       zoneId,
                                 ObjectGuid                   guid,
                                 CharacterDatabaseTransaction trans);
    ...
    [[nodiscard]] uint32 GetMoney() const
    {
        return GetUInt32Value(PLAYER_FIELD_COINAGE);
    }
    bool               ModifyMoney(int32 amount, bool sendError = true);
    [[nodiscard]] bool HasEnoughMoney(uint32 amount) const
    {
    ...
    [[nodiscard]] RewardedQuestSet const& getRewardedQuests() const
    {
    ...
    [[nodiscard]] size_t GetRewardedQuestCount() const
    {
    ...
    void SendItemRetrievalMail(
        std::vector<std::pair<uint32, uint32>>
            mailItems); // Item retrieval mails sent by The Postmaster (34337)
    ...
    TrainerSpellState
    GetTrainerSpellState(TrainerSpell const* trainer_spell) const;
    ...
    [[nodiscard]] uint32 GetFreeTalentPoints() const
    {
    ...
    [[nodiscard]] uint8 GetActiveSpecMask() const
    {
        return (1 << m_activeSpec);
    }
    ...
    [[nodiscard]] uint32 GetGlyphSlot(uint8 slot) const
    {
    ...
    [[nodiscard]] uint32 GetGlyph(uint8 slot) const
    {
        return m_Glyphs[m_activeSpec][slot];
    }

    [[nodiscard]] uint32 GetFreePrimaryProfessionPoints() const
    {
    ...
    [[nodiscard]] SpellCooldowns const& GetSpellCooldownMap() const
    {
    ...
    bool IsAffectedBySpellmod(SpellInfo const* spellInfo,
                              SpellModifier*   mod,
                              Spell*           spell = nullptr);
    bool HasSpellMod(SpellModifier* mod, Spell* spell);
    template <class T>
    void ApplySpellMod(uint32     spellId,
                       SpellModOp op,
                       T&         basevalue,
                       Spell*     spell        = nullptr,
    ...
    [[nodiscard]] bool   HasSpellItemCooldown(uint32 spell_id,
                                              uint32 itemid) const override;
    [[nodiscard]] uint32 GetSpellCooldownDelay(uint32 spell_id) const;
    void AddSpellAndCategoryCooldowns(SpellInfo const* spellInfo,
                                      uint32           itemId,
                                      Spell*           spell = nullptr,
                                      bool infinityCooldown  = false);
    void AddSpellCooldown(uint32 spell_id,
                          uint32 itemid,
                          uint32 end_time,
                          bool   needSendToClient     = false,
                          bool   forceSendToSpectator = false) override;
    void _AddSpellCooldown(uint32 spell_id,
                           uint16 categoryId,
                           uint32 itemid,
                           uint32 end_time,
                           bool   needSendToClient     = false,
    ...
    void SendCooldownEvent(SpellInfo const* spellInfo,
                           uint32           itemId      = 0,
                           Spell*           spell       = nullptr,
                           bool             setCooldown = true);
    void ProhibitSpellSchool(SpellSchoolMask idSchoolMask,
                             uint32          unTimeMs) override;
    ...
    void setResurrectRequestData(ObjectGuid guid,
                                 uint32     mapId,
                                 float      X,
                                 float      Y,
                                 float      Z,
    ...
    [[nodiscard]] bool isResurrectRequestedBy(ObjectGuid guid) const
    {
    ...
    [[nodiscard]] uint8 getCinematic() const { return m_cinematic; }
    ...
    [[nodiscard]] uint8 GetRank() const
    {
        return uint8(GetUInt32Value(PLAYER_GUILDRANK));
    }
    void SetGuildIdInvited(uint32 GuildId) { m_GuildIdInvited = GuildId; }
    [[nodiscard]] uint32 GetGuildId() const
    {
    ...
    void
    SetArenaTeamInfoField(uint8 slot, ArenaTeamInfoType type, uint32 value);
    ...
    [[nodiscard]] Difficulty GetDifficulty(bool isRaid) const
    {
        return isRaid ? m_raidDifficulty : m_dungeonDifficulty;
    }
    [[nodiscard]] Difficulty GetDungeonDifficulty() const
    {
        return m_dungeonDifficulty;
    }
    [[nodiscard]] Difficulty GetRaidDifficulty() const
    {
        return m_raidDifficulty;
    }
    [[nodiscard]] Difficulty GetStoredRaidDifficulty() const
    {
    ...
    bool UpdateGatherSkill(uint32 SkillId,
                           uint32 SkillValue,
                           uint32 RedLevel,
                           uint32 Multiplicator = 1);
    bool UpdateFishingSkill();

    [[nodiscard]] uint32 GetBaseDefenseSkillValue() const
    {
        return GetBaseSkillValue(SKILL_DEFENSE);
    }
    [[nodiscard]] uint32
    GetBaseWeaponSkillValue(WeaponAttackType attType) const;
    ...
    void CalculateMinMaxDamage(WeaponAttackType attType,
                               bool             normalized,
                               bool             addTotalPct,
                               float&           minDamage,
                               float&           maxDamage,
    ...
    [[nodiscard]] int32 GetSpellPenetrationItemMod() const
    {
        return m_spellPenetrationItemMod;
    }

    [[nodiscard]] float
         GetExpertiseDodgeOrParryReduction(WeaponAttackType attType) const;
    ...
    void SendLogXPGain(uint32 GivenXP,
                       Unit*  victim,
                       uint32 BonusXP,
    ...
    void SendMessageToSetInRange(
        ...
        Player const* skipped_rcvr = nullptr) const override; // pussywizard!
    void SendMessageToSetInRange_OwnTeam(
        WorldPacket const* data,
        float              dist,
        bool self) const; // pussywizard! param includeMargin not needed here
    void SendMessageToSet(WorldPacket const* data,
                          Player const*      skipped_rcvr) const override
    {
    ...
    void                  RemoveCorpse();
    void                  KillPlayer();
    static void           OfflineResurrect(ObjectGuid const             guid,
                                           CharacterDatabaseTransaction trans);
    [[nodiscard]] bool    HasCorpse() const
    {
        return _corpseLocation.GetMapId() != MAPID_INVALID;
    }
    [[nodiscard]] WorldLocation GetCorpseLocation() const
    {
    ...
    uint32
    DurabilityRepair(uint16 pos, bool cost, float discountMod, bool guildBank);
    ...
    bool CanJoinConstantChannelInZone(ChatChannelsEntry const* channel,
    ...
    void UpdateDefense();
    void UpdateWeaponSkill(Unit*            victim,
                           WeaponAttackType attType,
                           Item*            item = nullptr);
    void UpdateCombatSkills(Unit*            victim,
                            WeaponAttackType attType,
                            bool             defence,
                            Item*            item = nullptr);
    ...
    [[nodiscard]] uint16
    GetMaxSkillValue(uint32 skill) const; // max + perm. bonus + temp bonus
    [[nodiscard]] uint16 GetPureMaxSkillValue(uint32 skill) const; // max
    [[nodiscard]] uint16
    GetSkillValue(uint32 skill) const; // skill value + perm. bonus + temp bonus
    [[nodiscard]] uint16
    GetBaseSkillValue(uint32 skill) const; // skill value + perm. bonus
    ...
    [[nodiscard]] bool IsBeingTeleported() const
    {
        return mSemaphoreTeleport_Near != 0 || mSemaphoreTeleport_Far != 0;
    }
    [[nodiscard]] bool IsBeingTeleportedNear() const
    {
        return mSemaphoreTeleport_Near != 0;
    }
    [[nodiscard]] bool IsBeingTeleportedFar() const
    {
        return mSemaphoreTeleport_Far != 0;
    }
    ...
    [[nodiscard]] time_t GetSemaphoreTeleportNear() const
    {
        return mSemaphoreTeleport_Near;
    }
    [[nodiscard]] time_t GetSemaphoreTeleportFar() const
    {
        return mSemaphoreTeleport_Far;
    }
    void                 ProcessDelayedOperations();
    [[nodiscard]] uint32 GetDelayedOperations() const
    {
        return m_DelayedOperations;
    }
    ...
    [[nodiscard]] TeamId GetTeamId(bool original = false) const
    {
        return original ? TeamIdForRace(getRace(true)) : m_team;
    };
    ...
    void RewardPlayerAndGroupAtEvent(uint32       creature_id,
    ...
    [[nodiscard]] ReputationMgr const& GetReputationMgr() const
    {
    ...
    float CalculateReputationGain(ReputationSource source,
     ...
     * Usage:
     * 1). Hot update situation (occurred when character is online, like
     * PlayerScript:OnAchiComplete): Right after calling this function,
     * character can reward talent points by calling function
    ...
    bool                 RewardHonor(Unit*  victim,
                                     uint32 groupsize,
                                     int32  honor   = -1,
                                     bool   awardXP = true);
    [[nodiscard]] uint32 GetHonorPoints() const
    {
        return GetUInt32Value(PLAYER_FIELD_HONOR_CURRENCY);
    }
    [[nodiscard]] uint32 GetArenaPoints() const
    {
    ...
    void ModifyHonorPoints(
        ...
        CharacterDatabaseTransaction trans = CharacterDatabaseTransaction(
            nullptr)); //! If trans is specified, honor save query will be added
                       //! to trans
    void ModifyArenaPoints(
        int32                        value,
        CharacterDatabaseTransaction trans = CharacterDatabaseTransaction(
            nullptr)); //! If trans is specified, arena point save query will be
                       //! added to trans
    [[nodiscard]] uint32
         GetMaxPersonalArenaRatingRequirement(uint32 minarenaslot) const;
    ...
    [[nodiscard]] inline SpellCooldowns GetSpellCooldowns() const
    {
        return m_spellCooldowns;
    }

    void                SetDrunkValue(uint8 newDrunkValue, uint32 itemId = 0);
    [[nodiscard]] uint8 GetDrunkValue() const
    {
        return GetByteValue(PLAYER_BYTES_3, 1);
    }
    [[nodiscard]] int32 GetFakeDrunkValue() const
    {
        return GetInt32Value(PLAYER_FAKE_INEBRIATION);
    }
    ...
    [[nodiscard]] bool CanTameExoticPets() const
    {
    ...
    void
    SetBaseModValue(BaseModGroup modGroup, BaseModType modType, float value)
    {
        m_auraBaseMod[modGroup][modType] = value;
    }
    void                HandleBaseModValue(BaseModGroup modGroup,
                                           BaseModType  modType,
                                           float        amount,
                                           bool         apply);
    [[nodiscard]] float GetBaseModValue(BaseModGroup modGroup,
                                        BaseModType  modType) const;
    [[nodiscard]] float GetTotalBaseModValue(BaseModGroup modGroup) const;
    [[nodiscard]] float GetTotalPercentageModValue(BaseModGroup modGroup) const
    {
    ...
    GetMeleeDamageSchoolMask(WeaponAttackType attackType  = BASE_ATTACK,
                             uint8            damageIndex = 0) const override;

    void _ApplyWeaponDependentAuraMods(Item*            item,
                                       WeaponAttackType attackType,
                                       bool             apply);
    void _ApplyWeaponDependentAuraCritMod(Item*             item,
                                          WeaponAttackType  attackType,
                                          AuraEffect const* aura,
                                          bool              apply);
    void _ApplyWeaponDependentAuraDamageMod(Item*             item,
                                            WeaponAttackType  attackType,
                                            AuraEffect const* aura,
                                            bool              apply);
    ...
    void _ApplyItemBonuses(ItemTemplate const* proto,
                           uint8               slot,
                           bool                apply,
                           bool                only_level_scale = false);
    void _ApplyWeaponDamage(uint8                         slot,
    ...
    void ApplyEquipSpell(SpellInfo const* spellInfo,
                         Item*            item,
                         bool             apply,
                         bool             form_change = false);
    void UpdateEquipSpellsAtFormChange();
    void CastItemCombatSpell(Unit*            target,
                             WeaponAttackType attType,
                             uint32           procVictim,
                             uint32           procEx);
    void CastItemUseSpell(Item*                   item,
    ...
    [[nodiscard]] bool InBattleground() const
    {
        return m_bgData.bgInstanceID != 0;
    }
    [[nodiscard]] bool   InArena() const;
    [[nodiscard]] uint32 GetBattlegroundId() const
    {
        return m_bgData.bgInstanceID;
    }
    [[nodiscard]] BattlegroundTypeId GetBattlegroundTypeId() const
    {
        return m_bgData.bgTypeID;
    }
    [[nodiscard]] uint32 GetCurrentBattlegroundQueueSlot() const
    {
        return m_bgData.bgQueueSlot;
    }
    [[nodiscard]] bool IsInvitedForBattlegroundInstance() const
    {
        return m_bgData.isInvited;
    }
    [[nodiscard]] bool IsCurrentBattlegroundRandom() const
    {
        return m_bgData.bgIsRandom;
    }
    ...
    [[nodiscard]] BattlegroundQueueTypeId
    GetBattlegroundQueueTypeId(uint32 index) const;
    [[nodiscard]] uint32
    GetBattlegroundQueueIndex(BattlegroundQueueTypeId bgQueueTypeId) const;
    [[nodiscard]] bool IsInvitedForBattlegroundQueueType(
        BattlegroundQueueTypeId bgQueueTypeId) const;
    [[nodiscard]] bool InBattlegroundQueueForBattlegroundQueueType(
        BattlegroundQueueTypeId bgQueueTypeId) const;

    void   SetBattlegroundId(uint32             id,
    ...
    void
    SetInviteForBattlegroundQueueType(BattlegroundQueueTypeId bgQueueTypeId,
                                      uint32                  instanceId);
    bool IsInvitedForBattlegroundInstance(uint32 instanceId) const;

    [[nodiscard]] TeamId GetBgTeamId() const
    {
        return m_bgData.bgTeamId != TEAM_NEUTRAL ? m_bgData.bgTeamId
                                                 : GetTeamId();
    }
    ...
    [[nodiscard]] bool
    canFlyInZone(uint32 mapid, uint32 zone, SpellInfo const* bySpell);

    void
    SetClientControl(Unit* target, bool allowMove, bool packetOnly = false);
    ...
    [[nodiscard]] WorldLocation const& GetEntryPoint() const
    {
        return m_entryPointData.joinPos;
    }
    void SetEntryPoint();
    ...
    template <class T>
    void UpdateVisibilityOf(T*                  target,
                            UpdateData&         data,
                            std::vector<Unit*>& visibleNow);

    uint8 m_forced_speed_changes[MAX_MOVE_TYPE];

    [[nodiscard]] bool HasAtLoginFlag(AtLoginFlags f) const
    {
    ...
    [[nodiscard]] uint32 GetTemporaryUnsummonedPetNumber() const
    {
    ...
    [[nodiscard]] bool IsPetNeedBeTemporaryUnsummoned() const
    {
        return GetSession()->PlayerLogout() || !IsInWorld() || !IsAlive() ||
               IsMounted() /*+in flight*/ || GetVehicle() ||
               IsBeingTeleported();
    }
    ...
    void                 PrettyPrintRequirementsQuestList(
                        const std::vector<const ProgressionRequirement*>& missingQuests) const;
    void PrettyPrintRequirementsAchievementsList(
        const std::vector<const ProgressionRequirement*>& missingAchievements)
        const;
    void PrettyPrintRequirementsItemsList(
        const std::vector<const ProgressionRequirement*>& missingItems) const;
    bool               Satisfy(DungeonProgressionRequirements const* ar,
                               uint32                                target_map,
                               bool                                  report = false);
    ...
    [[nodiscard]] uint32 GetGroupUpdateFlag() const
    {
        return m_groupUpdateMask;
    }
    void SetGroupUpdateFlag(uint32 flag) { m_groupUpdateMask |= flag; }
    [[nodiscard]] uint64 GetAuraUpdateMaskForRaid() const
    {
    ...
    CanUninviteFromGroup(ObjectGuid targetPlayerGUID = ObjectGuid::Empty) const;
    ...
    [[nodiscard]] uint8 GetOriginalSubGroup() const
    {
    ...
    [[nodiscard]] DeclinedName const* GetDeclinedNames() const
    {
        return m_declinedname;
    }
    [[nodiscard]] uint8    GetRunesState() const { return m_runes->runeState; }
    [[nodiscard]] RuneType GetBaseRune(uint8 index) const
    {
        return RuneType(m_runes->runes[index].BaseRune);
    }
    [[nodiscard]] RuneType GetCurrentRune(uint8 index) const
    {
    ...
    void
    AddRuneByAuraEffect(uint8 index, RuneType newType, AuraEffect const* aura)
    {
    ...
    void UpdateAchievementCriteria(AchievementCriteriaTypes type,
                                   ...
                                   uint32                   miscValue2 = 0,
                                   Unit*                    unit = nullptr);
    void StartTimedAchievement(AchievementCriteriaTimedTypes type,
                               uint32                        entry,
                               uint32                        timeLost = 0);
    void RemoveTimedAchievement(AchievementCriteriaTimedTypes type,
                                uint32                        entry);
    void CompletedAchievement(AchievementEntry const* entry);
    [[nodiscard]] AchievementMgr* GetAchievementMgr() const
    {
    ...
    {
        SetUInt32Value(PLAYER_CHOSEN_TITLE, clear ? 0 : title->bit_index);
    };
    ...
    [[nodiscard]] uint32 GetChampioningFaction() const
    {
    ...
    {
        return m_ExtraFlags & PLAYER_EXTRA_SPECTATOR_ON;
    }
    void               SetIsSpectator(bool on);
    [[nodiscard]] bool NeedSendSpectatorData() const;
    void               SetPendingSpectatorForBG(uint32 bgInstanceId)
    {
        m_pendingSpectatorForBG = bgInstanceId;
    }
    [[nodiscard]] bool HasPendingSpectatorForBG(uint32 bgInstanceId) const
    {
        return m_pendingSpectatorForBG == bgInstanceId;
    }
    void SetPendingSpectatorInviteInstanceId(uint32 bgInstanceId)
    {
        m_pendingSpectatorInviteInstanceId = bgInstanceId;
    }
    [[nodiscard]] uint32 GetPendingSpectatorInviteInstanceId() const
    {
        return m_pendingSpectatorInviteInstanceId;
    }
    bool HasReceivedSpectatorResetFor(ObjectGuid guid)
    {
        return m_receivedSpectatorResetFor.find(guid) !=
               m_receivedSpectatorResetFor.end();
    }
    void ClearReceivedSpectatorResetFor()
    {
        m_receivedSpectatorResetFor.clear();
    }
    void AddReceivedSpectatorResetFor(ObjectGuid guid)
    {
        m_receivedSpectatorResetFor.insert(guid);
    }
    void RemoveReceivedSpectatorResetFor(ObjectGuid guid)
    {
        m_receivedSpectatorResetFor.erase(guid);
    }
    ...
    [[nodiscard]] bool CanSeeDKPet() const
    {
        return m_ExtraFlags & PLAYER_EXTRA_SHOW_DK_PET;
    }
    void SetShowDKPet(bool on)
    {
        if (on)
            m_ExtraFlags |= PLAYER_EXTRA_SHOW_DK_PET;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_SHOW_DK_PET;
    };
    ...
    // mt maps
    [[nodiscard]] const PlayerTalentMap& GetTalentMap() const
    {
        return m_talents;
    }
    [[nodiscard]] uint32              GetNextSave() const { return m_nextSave; }
    [[nodiscard]] SpellModList const& GetSpellModList(uint32 type) const
    {
        return m_spellMods[type];
    }

    void SetServerSideVisibility(ServerSideVisibilityType type,
                                 AccountTypes             sec);
    void SetServerSideVisibilityDetect(ServerSideVisibilityType type,
                                       AccountTypes             sec);

    static std::unordered_map<int, bgZoneRef>
        bgZoneIdToFillWorldStates; // zoneId -> FillInitialWorldStates
    ...
    // Settings
    [[nodiscard]] PlayerSetting GetPlayerSetting(std::string source,
                                                 uint8       index);
...
protected:
    ...
    std::array<BgBattlegroundQueueID_Rec, PLAYER_MAX_BATTLEGROUND_QUEUES>
    ...
    void         _LoadMail(PreparedQueryResult mailsResult,
                           PreparedQueryResult mailItemsResult);
    static Item* _LoadMailedItem(ObjectGuid const& playerGuid,
                                 Player*           player,
                                 uint32            mailId,
                                 Mail*             mail,
                                 Field*            fields);
    ...
    /*********************************************************/
    void  HandleSobering();
    void  SendMirrorTimer(MirrorTimerType Type,
                          uint32          MaxValue,
                          uint32          CurrentValue,
                          int32           Regen);
    ...
    uint32 m_lastPotionId; // last used health/mana potion in combat, that block
                           // next potion use
    ...
    // uint32 m_pad;
    //         Spell* m_spellModTakingSpell;  // Spell for which charges are
    //         dropped in spell::finish
...
private:
    ...
    InventoryResult CanStoreItem_InSpecificSlot(uint8               bag,
                                                uint8               slot,
                                                ItemPosCountVec&    dest,
                                                ItemTemplate const* pProto,
                                                uint32&             count,
                                                bool                swap,
                                                Item* pSrcItem) const;
    InventoryResult CanStoreItem_InBag(uint8               bag,
                                       ItemPosCountVec&    dest,
                                       ItemTemplate const* pProto,
                                       uint32&             count,
                                       bool                merge,
    ...
    InventoryResult CanStoreItem_InInventorySlots(uint8            slot_begin,
                                                  ...
                                                  uint8 skip_slot) const;
    Item*
    _StoreItem(uint16 pos, Item* pItem, uint32 count, bool clone, bool update);
    Item* _LoadItem(CharacterDatabaseTransaction trans,
                    uint32                       zoneId,
                    uint32                       timeDiff,
                    Field*                       fields);
    ...
    [[nodiscard]] bool MustDelayTeleport() const
    {
        return m_bMustDelayTeleport;
    } // pussywizard: must delay teleports during player update to the very end
    void SetMustDelayTeleport(bool setting) { m_bMustDelayTeleport = setting; }
    [[nodiscard]] bool HasDelayedTeleport() const
    {
