...
public: // constructors
    TradeData(Player* player, Player* trader)
        : m_player(player), m_trader(trader), m_accepted(false),
          m_acceptProccess(false), m_money(0), m_spell(0)
    {
    }
...
private:              // fields
    ...
    bool m_acceptProccess; // one from player/trader press accept and this
                           // processed
    ...
    ObjectGuid m_items[TRADE_SLOT_COUNT]; // traded itmes from m_player side
                                          // including non-traded slot
};
