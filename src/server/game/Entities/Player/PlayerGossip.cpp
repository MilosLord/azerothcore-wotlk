                               ...
                               bool         showQuests /*= false*/)
{
    ...
    for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first;
         itr != menuItemBounds.second;
         ++itr) {
        bool canTalk = true;
        if (!sConditionMgr->IsObjectMeetToConditions(
                this, source, itr->second.Conditions))
            continue;
        ...
        if (Creature* creature = source->ToCreature()) {
                ...
                if (!creature->isVendorWithIconSpeak()) {
                    VendorItemData const* vendorItems =
                        itr->second.ActionMenuID
                            ? sObjectMgr->GetNpcVendorItemList(
                                  itr->second.ActionMenuID)
                            : creature->GetVendorItems();
                    if (!vendorItems || vendorItems->Empty()) {
                        LOG_ERROR("sql.sql",
                                  "Creature {} have UNIT_NPC_FLAG_VENDOR but "
            ...
            case GOSSIP_OPTION_DUALSPEC_INFO:
                if (!(GetSpecsCount() == 1 &&
                      creature->isCanTrainingAndResetTalentsOf(this) &&
                      !(GetLevel() <
            ...
            case GOSSIP_OPTION_UNLEARNPETTALENTS:
                if (!GetPet() || GetPet()->getPetType() != HUNTER_PET ||
                    ...
                    creature->GetCreatureTemplate()->trainer_type !=
                        TRAINER_TYPE_PETS ||
                    creature->GetCreatureTemplate()->trainer_class !=
                        CLASS_HUNTER)
                    canTalk = false;
                break;
            ...
            if (optionBroadcastText)
                ObjectMgr::GetLocaleString(
                    getGender() == GENDER_MALE
                        ? optionBroadcastText->MaleText
                        : optionBroadcastText->FemaleText,
                    locale,
                    strOptionText);

            if (boxBroadcastText)
                ObjectMgr::GetLocaleString(getGender() == GENDER_MALE
                                               ? boxBroadcastText->MaleText
                                               : boxBroadcastText->FemaleText,
                                           locale,
                                           strBoxText);

            // if the language is not default and the texts weren't found, maybe
            // they're in gossip_menu_option_locale table
            if (locale != DEFAULT_LOCALE) {
                if (!optionBroadcastText) {
                    /// Find localizations from database.
                    if (GossipMenuItemsLocale const* gossipMenuLocale =
                            sObjectMgr->GetGossipMenuItemsLocale(
                                MAKE_PAIR32(menuId, itr->second.OptionID)))
                ...
                if (!boxBroadcastText) {
                    ...
                    if (GossipMenuItemsLocale const* gossipMenuLocale =
                            sObjectMgr->GetGossipMenuItemsLocale(
                                MAKE_PAIR32(menuId, itr->second.OptionID)))
                        ObjectMgr::GetLocaleString(
                            gossipMenuLocale->BoxText, locale, strBoxText);
                }
            }
    ...
    if (sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 2 &&
        npcflags & UNIT_NPC_FLAG_FLIGHTMASTER)
...
void Player::SendPreparedGossip(WorldObject* source)
{
    ...
    if (source->GetTypeId() == TYPEID_UNIT) {
        ...
        if (!source->ToCreature()->HasNpcFlag(UNIT_NPC_FLAG_GOSSIP) &&
    ...
    // it (quest entries from quest menu will be included in list)
                            ...
                            uint32       menuId)
{
    ...
    if (source->GetTypeId() == TYPEID_GAMEOBJECT) {
        if (gossipOptionId > GOSSIP_OPTION_QUESTGIVER) {
            LOG_ERROR("entities.player",
                      "Player guid {} request invalid gossip option for "
                      "GameObject entry {}",
                      GetGUID().ToString(),
                      source->GetEntry());
            return;
    ...
    GossipMenuItemData const* menuItemData =
        gossipMenu.GetItemData(gossipListId);
    if (!menuItemData)
        return;
    ...
    case GOSSIP_OPTION_OUTDOORPVP:
        sOutdoorPvPMgr->HandleGossipOption(
            this, source->ToCreature(), gossipListId);
        break;
    case GOSSIP_OPTION_SPIRITHEALER:
        if (isDead())
            source->ToCreature()->CastSpell(
                source->ToCreature(), 17251, true, nullptr, nullptr, GetGUID());
        break;
    case GOSSIP_OPTION_QUESTGIVER:
    ...
    case GOSSIP_OPTION_BATTLEFIELD: {
        ...
        if (bgTypeId == BATTLEGROUND_TYPE_NONE) {
            LOG_ERROR("entities.player",
                      "A user ({}) requested battlegroundlist from a npc who "
                      "is no battlemaster",
                      GetGUID().ToString());
            return;
        }
