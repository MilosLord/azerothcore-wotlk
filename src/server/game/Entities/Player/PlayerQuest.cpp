...
void Player::PrepareQuestMenu(ObjectGuid guid)
{
    ...
    if (creature) {
        ...
        objectQIR = sObjectMgr->GetCreatureQuestInvolvedRelationBounds(
            creature->GetEntry());
    }
    else {
        // we should obtain map pointer from GetMap() in 99% of cases. Special
        // case only for quests which cast teleport spells on player
        Map* _map = IsInWorld() ? GetMap()
                                : sMapMgr->FindMap(GetMapId(), GetInstanceId());
        ASSERT(_map);
        GameObject* pGameObject = _map->GetGameObject(guid);
        if (pGameObject) {
            objectQR =
                sObjectMgr->GetGOQuestRelationBounds(pGameObject->GetEntry());
            objectQIR = sObjectMgr->GetGOQuestInvolvedRelationBounds(
    ...
    for (QuestRelations::const_iterator i = objectQIR.first;
         i != objectQIR.second;
         ++i) {
    ...
    for (QuestRelations::const_iterator i = objectQR.first;
         i != objectQR.second;
         ++i) {
        uint32       quest_id = i->second;
        ...
        if (creature) {
            ...
            if (!gossiptext) {
                ...
                if (!gossiptext->Options[0].Text_0.empty()) {
                    ...
                    if (loc_idx >= 0)
                        if (NpcTextLocale const* npcTextLocale =
                                sObjectMgr->GetNpcTextLocale(textid))
                            ObjectMgr::GetLocaleString(
                                npcTextLocale->Text_0[0], loc_idx, title);
                }
                else {
                    ...
                    if (loc_idx >= 0)
                        if (NpcTextLocale const* npcTextLocale =
                                sObjectMgr->GetNpcTextLocale(textid))
                            ObjectMgr::GetLocaleString(
                                npcTextLocale->Text_1[0], loc_idx, title);
                }
            }
        }
...
Quest const* Player::GetNextQuest(ObjectGuid guid, Quest const* quest)
{
    ...
    for (QuestRelations::const_iterator itr = objectQR.first;
         itr != objectQR.second;
         ++itr) {
...
bool Player::CanSeeStartQuest(Quest const* quest)
{
    if (!DisableMgr::IsDisabledFor(
            DISABLE_TYPE_QUEST, quest->GetQuestId(), this) &&
        ...
        SatisfyQuestPreviousQuest(quest, false) &&
        SatisfyQuestNextChain(quest, false) &&
        SatisfyQuestPrevChain(quest, false) && SatisfyQuestDay(quest, false) &&
        SatisfyQuestWeek(quest, false) && SatisfyQuestMonth(quest, false) &&
...
{
    return !DisableMgr::IsDisabledFor(
               DISABLE_TYPE_QUEST, quest->GetQuestId(), this) &&
           ...
           SatisfyQuestPreviousQuest(quest, msg) &&
           SatisfyQuestTimed(quest, msg) && SatisfyQuestNextChain(quest, msg) &&
           SatisfyQuestPrevChain(quest, msg) && SatisfyQuestDay(quest, msg) &&
...
bool Player::CanCompleteQuest(uint32                 quest_id,
                              const QuestStatusData* q_savedStatus)
{
    if (quest_id) {
        ...
        if (!qInfo->IsRepeatable() && !qInfo->IsSeasonal() &&
            IsQuestRewarded(quest_id))
            return false; // not allow re-complete quest

        // auto complete quest
        if ((qInfo->IsAutoComplete() || !qInfo->GetQuestMethod()) &&
            CanTakeQuest(qInfo, false))
            return true;
            ...
            if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_KILL |
                ...
                for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; i++) {
                    ...
                    if (qInfo->RequiredNpcOrGoCount[i] != 0 &&
                        q_status.CreatureOrGOCount[i] <
                            qInfo->RequiredNpcOrGoCount[i])
                        return false;
                }
            }
    ...
    if (quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_DELIVER))
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
            if (quest->RequiredItemId[i] && quest->RequiredItemCount[i] &&
                !HasItemCount(quest->RequiredItemId[i],
...
bool Player::CanRewardQuest(Quest const* quest, bool msg)
{
    ...
    if (quest->GetRewOrReqMoney() < 0 &&
        !HasEnoughMoney(-quest->GetRewOrReqMoney()))
        return false;
...
{
    // prevent receive reward with quest items in bank or for not completed
    // quest
        ...
        if (quest->RewardChoiceItemId[reward]) {
            InventoryResult res =
                CanStoreNewItem(NULL_BAG,
                                NULL_SLOT,
                                dest,
                                quest->RewardChoiceItemId[reward],
                                quest->RewardChoiceItemCount[reward]);
            if (res != EQUIP_ERR_OK) {
                SendEquipError(
                    res, nullptr, nullptr, quest->RewardChoiceItemId[reward]);
                return false;
            }
        }
    }
            ...
            if (quest->RewardItemId[i]) {
                InventoryResult res =
                    CanStoreNewItem(NULL_BAG,
                                    NULL_SLOT,
                                    dest,
                                    quest->RewardItemId[i],
                                    quest->RewardItemIdCount[i]);
                if (res != EQUIP_ERR_OK) {
                    SendEquipError(
                        res, nullptr, nullptr, quest->RewardItemId[i]);
                    return false;
                }
            }
        }
...
void Player::AddQuest(Quest const* quest, Object* questGiver)
{
    ...
    if (log_slot >= MAX_QUEST_LOG_SIZE) // Player does not have any free slot in
                                        // the quest log
        return;
    ...
    if (quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_TIMED)) {
        ...
        qtime =
            static_cast<uint32>(GameTime::GetGameTime().count()) + timeAllowed;
    }
    else
...
void Player::RewardQuest(Quest const* quest,
                         ...
                         bool         isLFGReward)
{
    // this THING should be here to protect code from quest, which cast on
    // player far teleport as a reward should work fine, cause far teleport will
    // be executed in Player::Update()
    ...
    for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; ++i) {
        if (ItemTemplate const* itemTemplate =
            ...
            if (quest->RequiredItemCount[i] > 0 &&
                ...
                DestroyItemCount(quest->RequiredItemId[i],
                                 quest->RequiredItemCount[i],
                                 true);
        }
    }
    for (uint8 i = 0; i < QUEST_SOURCE_ITEM_IDS_COUNT; ++i) {
        if (ItemTemplate const* itemTemplate =
            ...
            if (quest->ItemDropQuantity[i] > 0 &&
                ...
                DestroyItemCount(
                    quest->ItemDrop[i], quest->ItemDropQuantity[i], true);
        }
    }
    ...
    if (quest->GetRewChoiceItemsCount()) {
        if (uint32 itemId = quest->RewardChoiceItemId[reward]) {
            ...
            if (CanStoreNewItem(NULL_BAG,
                ...
                sScriptMgr->OnQuestRewardItem(
                    this, item, quest->RewardChoiceItemCount[reward]);
            }
            else {
                problematicItems.emplace_back(
                    itemId, quest->RewardChoiceItemCount[reward]);
            }
        }
    }
        ...
        for (uint32 i = 0; i < quest->GetRewItemsCount(); ++i) {
            if (uint32 itemId = quest->RewardItemId[i]) {
                ...
                if (CanStoreNewItem(NULL_BAG,
                    ...
                    sScriptMgr->OnQuestRewardItem(
                        this, item, quest->RewardItemIdCount[i]);
                }
                else
                    problematicItems.emplace_back(itemId,
                                                  quest->RewardItemIdCount[i]);
            }
        }
    }
    ...
    uint32 XP = rewarded ? 0
                         : uint32(quest->XPValue(GetLevel()) *
                                  GetQuestRate(quest->IsDFQuest()));
    ...
    if (GetLevel() >= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL) ||
        sScriptMgr->ShouldBeRewardedWithMoneyInsteadOfExp(this)) {
        moneyRew = quest->GetRewMoneyMaxLevel();
    }
    else {
        sScriptMgr->OnGivePlayerXP(this,
                                   XP,
                                   nullptr,
                                   isLFGReward
                                       ? PlayerXPSource::XPSOURCE_QUEST_DF
                                       : PlayerXPSource::XPSOURCE_QUEST);
    ...
    if (quest->IsDaily() || quest->IsDFQuest()) {
        ...
        if (quest->IsDaily()) {
            UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST, quest_id);
            UpdateAchievementCriteria(
    ...
    if (quest->GetRewSpellCast() > 0) {
        SpellInfo const* spellInfo =
            sSpellMgr->GetSpellInfo(quest->GetRewSpellCast());
        if (questGiver->isType(TYPEMASK_UNIT) &&
            ...
            if (Creature* creature =
                    GetMap()->GetCreature(questGiver->GetGUID()))
        ...
        SpellInfo const* spellInfo =
            sSpellMgr->GetSpellInfo(quest->GetRewSpell());
        if (questGiver->isType(TYPEMASK_UNIT) &&
            !spellInfo->HasEffect(SPELL_EFFECT_LEARN_SPELL) &&
            !spellInfo->HasEffect(SPELL_EFFECT_CREATE_ITEM) &&
            !spellInfo->IsSelfCast()) {
    ...
    if (GetLevel() < qInfo->GetMinLevel()) {
        ...
        if (msg)
            SendCanTakeQuestResponse(
                INVALIDREASON_DONT_HAVE_REQ); // There doesn't seem to be a
                                              // specific response for too high
                                              // player level
        return false;
    }
    return true;
...
bool Player::SatisfyQuestPreviousQuest(Quest const* qInfo, bool msg) const
{
    ...
    for (Quest::PrevQuests::const_iterator iter = qInfo->prevQuests.begin();
         iter != qInfo->prevQuests.end();
         ++iter) {
        ...
        if (qPrevInfo) {
            ...
            if (*iter > 0 && IsQuestRewarded(prevId) &&
                (!qPrevInfo->IsSeasonal() ||
                 !SatisfyQuestSeasonal(qPrevInfo, false))) {
                ...
                ObjectMgr::ExclusiveQuestGroupsBounds range(
                    sObjectMgr->mExclusiveQuestGroups.equal_range(
                ...
                for (; range.first != range.second; ++range.first) {
                    ...
                    Quest const* qExcludeInfo =
                        sObjectMgr->GetQuestTemplate(exclude_Id);
                    if (!IsQuestRewarded(exclude_Id) ||
                        (qExcludeInfo->IsSeasonal() &&
                         SatisfyQuestSeasonal(qExcludeInfo, false))) {
                        if (msg)
                            SendCanTakeQuestResponse(
...
bool Player::SatisfyQuestConditions(Quest const* qInfo, bool msg)
{
    ...
    if (!sConditionMgr->IsObjectMeetToConditions(this, conditions)) {
        ...
        LOG_DEBUG(
            "condition",
            "Player::SatisfyQuestConditions: conditions not met for quest {}",
            qInfo->GetQuestId());
        return false;
    }
    return true;
...
bool Player::SatisfyQuestExclusiveGroup(Quest const* qInfo, bool msg) const
{
    ...
    for (; range.first != range.second; ++range.first) {
        ...
        // alternative quest already started or completed - but don't check
        // rewarded states if both are repeatable
        if (GetQuestStatus(exclude_Id) != QUEST_STATUS_NONE ||
            (!(qInfo->IsRepeatable() && Nquest->IsRepeatable()) &&
             !Nquest->IsSeasonal() &&
             IsQuestRewarded(
                 exclude_Id))) // pussywizard: added !Nquest->IsSeasonal()
                               // because seasonal quests are considered
                               // rewarded only if finished this year, this is
                               // checked above in SatisfyQuestSeasonal
        {
            if (msg)
                SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            return false;
...
bool Player::SatisfyQuestNextChain(Quest const* qInfo, bool msg) const
{
    ...
    if (GetQuestStatus(nextQuest) !=
        QUEST_STATUS_NONE) // GetQuestStatus returns QUEST_STATUS_COMPLETED for
                           // rewarded quests
    {
        if (msg)
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
...
bool Player::SatisfyQuestPrevChain(Quest const* qInfo, bool msg) const
{
    ...
    for (Quest::PrevChainQuests::const_iterator iter =
             qInfo->prevChainQuests.begin();
         iter != qInfo->prevChainQuests.end();
         ++iter) {
...
bool Player::SatisfyQuestSeasonal(Quest const* qInfo, bool /*msg*/) const
{
    ...
    Player::SeasonalEventQuestMap::iterator itr =
        ((Player*)this)->m_seasonalquests.find(qInfo->GetEventIdForQuest());
        ...
        if (srcItemId > 0) {
            ...
            for (uint8 n = 0; n < QUEST_ITEM_OBJECTIVES_COUNT; ++n)
                if (item->StartQuest == questId &&
                    srcItemId == quest->RequiredItemId[n])
                    destroyItem = false;
...
bool Player::CanShareQuest(uint32 quest_id) const
{
    ...
    if (qInfo && qInfo->HasFlag(QUEST_FLAGS_SHARABLE)) {
        ...
        if (itr != m_QuestStatus.end()) {
            ...
            // weekly) - can't share
            if (sPoolMgr->IsPartOfAPool<Quest>(quest_id) &&
                !sPoolMgr->IsSpawnedObject<Quest>(quest_id)) {
                SendPushToPartyResponse(this,
...
void Player::SendQuestUpdate(uint32 questId)
{
    ...
    if (saBounds.first != saBounds.second) {
        ...
        for (SpellAreaForAreaMap::const_iterator itr = saBounds.first;
             itr != saBounds.second;
             ++itr) {
            // xinef: spell was already casted, skip different areas with same
            // spell
    ...
    if (saBounds.first != saBounds.second) {
        ...
        for (SpellAreaForAreaMap::const_iterator itr = saBounds.first;
            ...
            // xinef: spells are sorted, if no condition is fulfilled remove
            // aura
...
QuestGiverStatus Player::GetQuestDialogStatus(Object* questgiver)
{
    ...
    switch (questgiver->GetTypeId()) {
    case TYPEID_GAMEOBJECT: {
        QuestGiverStatus questStatus = QuestGiverStatus(
            sScriptMgr->GetDialogStatus(this, questgiver->ToGameObject()));
        if (questStatus != DIALOG_STATUS_SCRIPTED_NO_STATUS)
            return questStatus;
        qr  = sObjectMgr->GetGOQuestRelationBounds(questgiver->GetEntry());
        qir = sObjectMgr->GetGOQuestInvolvedRelationBounds(
            questgiver->GetEntry());
        break;
    }
    case TYPEID_UNIT: {
        QuestGiverStatus questStatus = QuestGiverStatus(
            sScriptMgr->GetDialogStatus(this, questgiver->ToCreature()));
        if (questStatus != DIALOG_STATUS_SCRIPTED_NO_STATUS)
            return questStatus;
        qr = sObjectMgr->GetCreatureQuestRelationBounds(questgiver->GetEntry());
        qir = sObjectMgr->GetCreatureQuestInvolvedRelationBounds(
            questgiver->GetEntry());
        break;
    }
    default:
        // it's impossible, but check
        // LOG_ERROR("entities.player.quest", "GetQuestDialogStatus called for
        // unexpected type {}", questgiver->GetTypeId());
    ...
    for (QuestRelations::const_iterator i = qir.first; i != qir.second; ++i) {
        ...
        ConditionList conditions =
            sConditionMgr->GetConditionsForNotGroupedEntry(
                CONDITION_SOURCE_TYPE_QUEST_AVAILABLE, quest->GetQuestId());
        if (!sConditionMgr->IsObjectMeetToConditions(this, conditions))
            continue;
                                     ...
                                     QuestStatusData& questStatusData)
{
    if (quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_DELIVER)) {
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; ++i) {
            ...
            if (reqitemcount != 0) {
                uint32 curitemcount =
                    GetItemCount(quest->RequiredItemId[i], true);

                questStatusData.ItemCount[i] =
    ...
    if (questId) {
        ...
        if (log_slot < MAX_QUEST_LOG_SIZE) {
            ...
            if (!q_status->Explored &&
                q_status->Status != QUEST_STATUS_FAILED) {
        ...
        for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j) {
            ...
            if (reqitem == entry) {
                ...
                if (curitemcount < reqitemcount) {
                    q_status.ItemCount[j] = std::min<uint16>(
                        q_status.ItemCount[j] + count, reqitemcount);
                    m_QuestStatusSave[questid] = true;
                }
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i) {
        ...
        for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j) {
            ...
            if (reqitem == entry) {
                ...
                if (q_status.ItemCount[j] >=
                    reqitemcount) // we may have more than what the status shows
                    curitemcount = GetItemCount(entry, false);

                uint16 newItemCount =
                    (count > curitemcount) ? 0 : curitemcount - count;
...
void Player::KilledMonsterCredit(uint32 entry, ObjectGuid guid)
{
    ...
    UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE,
                              real_entry,
                              addkillcount,
                              guid ? GetMap()->GetCreature(guid) : nullptr);
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i) {
        ...
        if (q_status.Status == QUEST_STATUS_INCOMPLETE &&
            (!GetGroup() || !GetGroup()->isRaidGroup() ||
             ...
             (qInfo->IsPVPQuest() &&
              (GetGroup()->isBFGroup() || GetGroup()->isBGGroup())))) {
            if (!sScriptMgr->PassedQuestKilledMonsterCredit(
                    this, qInfo, entry, real_entry, guid))
                continue;
...
void Player::KilledPlayerCredit(uint16 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i) {
        ...
        if (q_status.Status == QUEST_STATUS_INCOMPLETE &&
            (!GetGroup() || !GetGroup()->isRaidGroup() ||
             qInfo->IsAllowedInRaid(GetMap()->GetDifficulty()))) {
            // Xinef: PvP Killing quest require player to be in same zone as
            // quest zone (only 2 quests so no doubt, can be extended to
            // conditions in cata ;s)
            if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_PLAYER_KILL) &&
                (qInfo->GetZoneOrSort() >= 0 &&
                 GetZoneId() == uint32(qInfo->GetZoneOrSort()))) {
            ...
            if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_CAST) /*&& !qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_KILL)*/)
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j) {
                    ...
                    if (curCastCount < reqCastCount) {
                        ...
                        SendQuestUpdateAddCreatureOrGo(
                            qInfo, guid, j, curCastCount, addCastCount);
                    }
                    ...
                    // not in 2 objectives for single quest (code optimization).
                    break;
                }
            }
        }
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i) {
        ...
        if ((q_status.Status == QUEST_STATUS_INCOMPLETE) ||
            ...
            if (GetGroup() && GetGroup()->isRaidGroup() &&
                ...
                if (!InBattleground() &&
                    !GetGroup()
                         ->isBFGroup()) // there are two ways.. we can make
                                        // every bg-quest a raidquest, or add
                                        // this code here.. i don't know if this
                                        // can be exploited by other quests, but
                                        // i think all other quests depend on a
                                        // specific area.. but keep this in
                                        // mind, if something strange happens
                                        // later
                {
                    continue;
                }
            ...
            for (uint8 j = 0; j < QUEST_SOURCE_ITEM_IDS_COUNT; ++j) {
                ...
                if (qinfo->ItemDrop[j] == itemid) {
                    ...
                    // 'unique' item
                    if ((pProto->MaxCount &&
                         int32(ownedCount) < pProto->MaxCount) ||
                        (turnIn && int32(ownedCount) >= pProto->MaxCount))
                        return true;

                    // allows custom amount drop when not 0
                    if (qinfo->ItemDropQuantity[j]) {
                        if ((ownedCount < qinfo->ItemDropQuantity[j]) ||
                            (turnIn &&
                             ownedCount >= qinfo->ItemDropQuantity[j]))
                            return true;
                    }
...
void Player::SendQuestComplete(uint32 quest_id)
{
    if (quest_id) {
        ...
        GetSession()->SendPacket(&data);
        LOG_DEBUG("network",
                  "WORLD: Sent SMSG_QUESTUPDATE_COMPLETE quest = {}",
                  quest_id);
    }
}
...
void Player::SendQuestReward(Quest const* quest, uint32 XP)
{
    ...
    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUESTGIVER_QUEST_COMPLETE quest = {}",
              questid);
...
void Player::SendQuestFailed(uint32 questId, InventoryResult reason)
{
    if (questId) {
        ...
        data << uint32(questId);
        data << uint32(reason); // failed reason (valid reasons: 4, 16, 50, 17,
                                // 74, other values show default message)
        GetSession()->SendPacket(&data);
...
void Player::SendQuestUpdateAddCreatureOrGo(Quest const* quest,
                                            ...
                                            uint16       add_count)
{
    ASSERT(old_count + add_count < 65536 &&
           "mob/GO count store in 16 bits 2^16 = 65536 (0..65536)");
    ...
    if (log_slot < MAX_QUEST_LOG_SIZE)
        SetQuestSlotCounter(log_slot,
                            creatureOrGO_idx,
                            GetQuestSlotCounter(log_slot, creatureOrGO_idx) +
                                add_count);
}
                                      ...
                                      uint16       add_count)
{
    ...
    if (log_slot < MAX_QUEST_LOG_SIZE)
        SetQuestSlotCounter(log_slot,
                            QUEST_PVP_KILL_SLOT,
                            GetQuestSlotCounter(log_slot, QUEST_PVP_KILL_SLOT) +
                                add_count);
}
