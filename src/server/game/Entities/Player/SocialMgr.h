...
enum SocialFlag {
    ...
    SOCIAL_FLAG_ALL =
        SOCIAL_FLAG_FRIEND | SOCIAL_FLAG_IGNORED | SOCIAL_FLAG_MUTED
};
...
struct FriendInfo {
    ...
    FriendInfo()
        : Status(FRIEND_STATUS_OFFLINE), Flags(0), Area(0), Level(0), Class(0),
          Note()
    {
    }

    FriendInfo(uint8 flags, std::string const& note)
        : Status(FRIEND_STATUS_OFFLINE), Flags(flags), Area(0), Level(0),
          Class(0), Note(note)
    {
    }
};
...
public:
    ...
    static void GetFriendInfo(Player*     player,
                              ObjectGuid  friendGUID,
                              FriendInfo& friendInfo);
    // Packet management
    void MakeFriendStatusPacket(FriendsResult result,
                                ObjectGuid    friend_guid,
                                WorldPacket*  data);
    void SendFriendStatus(Player*       player,
