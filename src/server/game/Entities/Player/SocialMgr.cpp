...
{
    // check client limits
    if (GetNumberOfSocialsWithFlag(flag) >= (((flag & SOCIAL_FLAG_FRIEND) != 0)
    ...
    if (itr != m_playerSocialMap.end()) {
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_INS_CHARACTER_SOCIAL);
...
void PlayerSocial::RemoveFromSocialList(ObjectGuid friendGuid, SocialFlag flag)
{
    ...
    if (itr->second.Flags == 0) {
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHARACTER_SOCIAL);
...
void PlayerSocial::SetFriendNote(ObjectGuid friendGuid, std::string note)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHARACTER_SOCIAL_NOTE);
...
void PlayerSocial::SendSocialList(Player* player, uint32 flags)
{
    ...
    WorldPacket data(
        SMSG_CONTACT_LIST,
        (4 + 4 + m_playerSocialMap.size() * 25)); // just can guess size
    data << uint32(flags); // 0x1 = Friendlist update. 0x2 = Ignorelist update.
                           // 0x4 = Mutelist update.
    ...
    for (auto& itr : m_playerSocialMap) {
        ...
        data << uint32(contactFlags); // player flag (0x1 = Friend, 0x2 =
                                      // Ignored, 0x4 = Muted)
                              ...
                              FriendInfo& friendInfo)
{
    ...
    AccountTypes gmLevelInWhoList =
        AccountTypes(sWorld->getIntConfig(CONFIG_GM_LEVEL_IN_WHO_LIST));

    auto const& itr = player->GetSocial()->m_playerSocialMap.find(friendGUID);
    if (itr != player->GetSocial()->m_playerSocialMap.end())
        friendInfo.Note = itr->second.Note;

    // PLAYER see his team only and PLAYER can't see MODERATOR, GAME MASTER,
    // ADMINISTRATOR characters MODERATOR, GAME MASTER, ADMINISTRATOR can see
    // all
    if ((!AccountMgr::IsPlayerAccount(security) ||
         ((pFriend->GetTeamId() == teamId || allowTwoSideWhoList) &&
...
void SocialMgr::BroadcastToFriendListers(Player* player, WorldPacket* packet)
{
    ...
    for (auto const& itr : m_socialMap) {
        ...
        if (itr2 != itr.second.m_playerSocialMap.end() &&
            (itr2->second.Flags & SOCIAL_FLAG_FRIEND)) {
            Player* pFriend = ObjectAccessor::FindPlayer(itr.first);

            // PLAYER see his team only and PLAYER can't see MODERATOR, GAME
            // MASTER, ADMINISTRATOR characters MODERATOR, GAME MASTER,
            // ADMINISTRATOR can see all
            if (pFriend &&
                (!AccountMgr::IsPlayerAccount(
                     pFriend->GetSession()->GetSecurity()) ||
                 ((pFriend->GetTeamId() == teamId || allowTwoSideWhoList) &&
                  security <= gmLevelInWhoList)) &&
