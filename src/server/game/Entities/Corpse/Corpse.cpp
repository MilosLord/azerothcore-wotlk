...
Corpse::Corpse(CorpseType type)
    : WorldObject(type != CORPSE_BONES), m_type(type)
{
...
bool Corpse::Create(ObjectGuid::LowType guidlow, Player* owner)
{
    ...
    if (!IsPositionValid()) {
        LOG_ERROR("entities.player",
                  "Corpse (guidlow {}, owner {}) not created. Suggested "
...
void Corpse::SaveToDB()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_CORPSE);
    ...
    stmt->SetData(
        7, _ConcatFields(CORPSE_FIELD_ITEM, EQUIPMENT_SLOT_END));  // itemCache
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_CORPSE);
...
bool Corpse::LoadCorpseFromDB(ObjectGuid::LowType guid, Field* fields)
{
    ...
    if (!_LoadIntoDataField(fields[6].Get<std::string>(),
                            ...
                            EQUIPMENT_SLOT_END)) {
        LOG_ERROR("entities.player",
                  "Corpse ({}, owner: {}) is not created, given equipment info "
                  "is not valid ('{}')",
                  GetGUID().ToString(),
    ...
    SetGuidValue(CORPSE_FIELD_OWNER,
    ...
    if (!IsPositionValid()) {
        LOG_ERROR("entities.player",
                  "Corpse ( {}, owner: {}) is not created, given coordinates "
                  "are not valid (X: {}, Y: {}, Z: {})",
                  GetGUID().ToString(),
                  GetOwnerGUID().ToString(),
                  posX,
...
void Corpse::ResetGhostTime() { m_time = GameTime::GetGameTime().count(); }
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index) {
        if (_fieldNotifyFlags & flags[index] ||
            ...
            if (index == CORPSE_FIELD_BYTES_1 ||
                index == CORPSE_FIELD_BYTES_2) {
                Player* owner =
                    ObjectAccessor::GetPlayer(*this, GetOwnerGUID());
                if (owner && owner != target &&
                    sWorld->getBoolConfig(
                        CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) &&
                    owner->IsInRaidWith(target) &&
                    owner->GetTeamId() != target->GetTeamId()) {
                    uint32 playerBytes = target->GetUInt32Value(PLAYER_BYTES);
                    uint32 playerBytes2 =
                        target->GetUInt32Value(PLAYER_BYTES_2);
                    ...
                    uint32 corpseBytes1 =
                        ((0x00) | (race << 8) |
                         (target->GetByteValue(PLAYER_BYTES_3, 0) << 16) |
                         (skin << 24));
                    uint32 corpseBytes2 =
                        ((face) | (hairstyle << 8) | (haircolor << 16) |
                         (facialhair << 24));
