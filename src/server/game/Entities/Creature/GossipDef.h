...
enum Gossip_Option {
    ...
    GOSSIP_OPTION_UNLEARNTALENTS =
        16, // UNIT_NPC_FLAG_TRAINER             (16) (bonus option for
            // GOSSIP_OPTION_TRAINER)
    GOSSIP_OPTION_UNLEARNPETTALENTS = 17, // UNIT_NPC_FLAG_TRAINER (16) (bonus
                                          // option for GOSSIP_OPTION_TRAINER)
    GOSSIP_OPTION_LEARNDUALSPEC =
        18, // UNIT_NPC_FLAG_TRAINER             (16) (bonus option for
            // GOSSIP_OPTION_TRAINER)
    GOSSIP_OPTION_OUTDOORPVP =
        19, // added by code (option for outdoor pvp creatures)
    GOSSIP_OPTION_DUALSPEC_INFO =
        20, // UNIT_NPC_FLAG_TRAINER             (16) (bonus option for
            // GOSSIP_OPTION_TRAINER)
    GOSSIP_OPTION_MAX
};
...
public:
    ...
    void AddMenuItem(int32              menuItemId,
                     uint8              icon,
                     ...
                     uint32             boxMoney,
                     bool               coded = false);
    void AddMenuItem(uint32 menuId,
                     uint32 menuItemId,
                     uint32 sender,
                     uint32 action,
                     uint32 boxMoney);
    ...
    void AddGossipMenuItemData(uint32 menuItemId,
                               uint32 gossipActionMenuId,
                               uint32 gossipActionPoi);

    [[nodiscard]] uint32 GetMenuItemCount() const { return _menuItems.size(); }

    [[nodiscard]] bool Empty() const { return _menuItems.empty(); }
    ...
    {
        GossipMenuItemDataContainer::const_iterator itr =
            _menuItemData.find(indexId);
...
public:
    ...
    [[nodiscard]] bool Empty() const { return _questMenuItems.empty(); }
...
public:
    ...
    void                 ClearMenus();
    [[nodiscard]] uint32 GetGossipOptionSender(uint32 selection) const
    {
        return _gossipMenu.GetMenuItemSender(selection);
    }
    [[nodiscard]] uint32 GetGossipOptionAction(uint32 selection) const
    {
        return _gossipMenu.GetMenuItemAction(selection);
    }
    [[nodiscard]] bool IsGossipOptionCoded(uint32 selection) const
    {
    ...
    void SendQuestGiverQuestList(QEmote const&      eEmote,
                                 std::string const& Title,
                                 ObjectGuid         guid);

    void SendQuestQueryResponse(Quest const* quest) const;
    void SendQuestGiverQuestDetails(Quest const* quest,
                                    ObjectGuid   npcGUID,
                                    bool         activateAccept) const;

    void SendQuestGiverOfferReward(Quest const* quest,
                                   ObjectGuid   npcGUID,
                                   bool         enableNext) const;
    void SendQuestGiverRequestItems(Quest const* quest,
                                    ObjectGuid   npcGUID,
                                    bool         canComplete,
