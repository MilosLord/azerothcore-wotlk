                             ...
                             bool               coded /*= false*/)
{
    ...
    if (menuItemId == -1) {
        menuItemId = 0;
        if (!_menuItems.empty()) {
            for (GossipMenuItemContainer::const_iterator itr =
                     _menuItems.begin();
                 itr != _menuItems.end();
                 ++itr) {
 ...
 * @param boxMoney Custom price for pop-up box. If > 0, it will replace DB
 * value.
 */
                             ...
                             uint32 boxMoney)
{
    ...
    for (GossipMenuItemsContainer::const_iterator itr = bounds.first;
         itr != bounds.second;
         ++itr) {
        ...
        if (optionBroadcastText)
            ObjectMgr::GetLocaleString(
                optionBroadcastText->MaleText, GetLocale(), strOptionText);
        else
        ...
        if (boxBroadcastText)
            ObjectMgr::GetLocaleString(
                boxBroadcastText->MaleText, GetLocale(), strBoxText);
        else
        ...
        if (GetLocale() != DEFAULT_LOCALE) {
            if (!optionBroadcastText) {
                ...
                if (GossipMenuItemsLocale const* gossipMenuLocale =
                        sObjectMgr->GetGossipMenuItemsLocale(
                            MAKE_PAIR32(menuId, menuItemId)))
                    ObjectMgr::GetLocaleString(gossipMenuLocale->OptionText,
                                               GetLocale(),
                                               strOptionText);
            }
            ...
            if (!boxBroadcastText) {
                ...
                if (GossipMenuItemsLocale const* gossipMenuLocale =
                        sObjectMgr->GetGossipMenuItemsLocale(
                            MAKE_PAIR32(menuId, menuItemId)))
                    ObjectMgr::GetLocaleString(
                        gossipMenuLocale->BoxText, GetLocale(), strBoxText);
            }
        }

        /// Add menu item with existing method. Menu item id -1 is also used in
        /// ADD_GOSSIP_ITEM macro.
        AddMenuItem(-1,
                    ...
                    action,
                    strBoxText,
                    boxMoney ? boxMoney : itr->second.BoxMoney,
                    itr->second.BoxCoded);
    }
}
...
void PlayerMenu::SendGossipMenu(uint32 titleTextId, ObjectGuid objectGUID)
{
    ...
    WorldPacket data(SMSG_GOSSIP_MESSAGE,
                     24 + _gossipMenu.GetMenuItemCount() * 100 +
                         _questMenu.GetMenuItemCount() * 75); // guess size
    data << objectGUID;
    ...
    for (GossipMenuItemContainer::const_iterator itr =
             _gossipMenu.GetMenuItems().begin();
         itr != _gossipMenu.GetMenuItems().end();
         ++itr) {
        ...
        data << item.BoxMessage;       // accept text (related to money) pop up
                                       // box, 2.0.3
    }
    ...
    for (uint32 iI = 0; iI < _questMenu.GetMenuItemCount(); ++iI) {
        ...
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(questID)) {
            ...
            data << uint8(
                ...
                !quest->IsMonthly()); // 3.3.3 icon changes: blue question mark
                                      // or yellow exclamation mark
            std::string title = quest->GetTitle();

            int32 locale = _session->GetSessionDbLocaleIndex();
            if (QuestLocale const* localeData =
                    sObjectMgr->GetQuestLocale(questID))
...
void PlayerMenu::SendPointOfInterest(uint32 poiId) const
{
    ...
    int32       locale = _session->GetSessionDbLocaleIndex();
    if (PointOfInterestLocale const* localeData =
            sObjectMgr->GetPointOfInterestLocale(poiId))
...
QuestMenu::QuestMenu()
{
    _questMenuItems.reserve(16); // can be set for max from most often sizes to
                                 // speedup push_back and less memory use
}
...
{
    for (QuestMenuItemList::const_iterator i = _questMenuItems.begin();
         i != _questMenuItems.end();
         ++i)
...
void QuestMenu::ClearMenu() { _questMenuItems.clear(); }
                                         ...
                                         ObjectGuid         guid)
{
    ...
    if (QuestGreeting const* questGreeting =
            sObjectMgr->GetQuestGreeting(guid.GetTypeId(), guid.GetEntry())) {
        std::string strGreeting = questGreeting->Text;

        LocaleConstant localeConstant = _session->GetSessionDbLocaleIndex();
        if (localeConstant != LOCALE_enUS)
            if (QuestGreetingLocale const* questGreetingLocale =
                    sObjectMgr->GetQuestGreetingLocale(guid.GetTypeId(),
                                                       guid.GetEntry()))
                ObjectMgr::GetLocaleString(
                    questGreetingLocale->Greeting, localeConstant, strGreeting);
    ...
    for (uint32 i = 0; i < _questMenu.GetMenuItemCount(); ++i) {
        ...
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(questID)) {
            ...
            LocaleConstant localeConstant = _session->GetSessionDbLocaleIndex();
            if (localeConstant != LOCALE_enUS)
                if (QuestLocale const* questTemplateLocale =
                        sObjectMgr->GetQuestLocale(questID))
                    ObjectMgr::GetLocaleString(
                        questTemplateLocale->Title, localeConstant, title);
            ...
            data << uint8(
                quest->IsRepeatable() && !quest->IsDailyOrWeekly() &&
                !quest->IsMonthly()); // 3.3.3 changes icon: blue question or
                                      // yellow exclamation
            data << title;
        }
    }
    ...
    _session->SendPacket(&data);
    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUESTGIVER_QUEST_LIST (QuestGiver: {})",
              guid.ToString());
}

void PlayerMenu::SendQuestGiverStatus(uint8      questStatus,
                                      ObjectGuid npcGUID) const
{
    ...
    _session->SendPacket(&data);
    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUESTGIVER_STATUS NPC {}, status={}",
              npcGUID.ToString(),
              questStatus);
}
                                            ...
                                            bool         activateAccept) const
{
    ...
    if (QuestLocale const* localeData =
        ...
        ObjectMgr::GetLocaleString(
            localeData->Objectives, locale, questObjectives);
        ObjectMgr::GetLocaleString(
            localeData->AreaDescription, locale, questAreaDescription);
    }
    ...
    data << uint8(
        0); // IsFinished? value is sent back to server in quest accept packet
    ...
    if (quest->HasFlag(QUEST_FLAGS_HIDDEN_REWARDS)) {
        ...
        for (uint32 i = 0; i < QUEST_REWARDS_COUNT; ++i) {
            ...
            if (ItemTemplate const* itemTemplate =
                    sObjectMgr->GetItemTemplate(quest->RewardItemId[i]))
                data << uint32(itemTemplate->DisplayInfoID);
            else
                data << uint32(0);
        }
        ...
        if (player &&
            (player->getLevel() >=
             ...
             sScriptMgr->ShouldBeRewardedWithMoneyInsteadOfExp(player))) {
            moneyRew = quest->GetRewMoneyMaxLevel();
        }
        moneyRew += quest->GetRewOrReqMoney(
            player ? player->GetLevel() : 0); // reward money (below max lvl)
        data << moneyRew;
        uint32 questXp;
        if (player &&
            !sScriptMgr->ShouldBeRewardedWithMoneyInsteadOfExp(player)) {
            questXp =
                uint32(quest->XPValue(playerLevel) * player->GetQuestRate());
        }
        else {
            questXp = 0;
        }
    ...
    data << uint32(10 * quest->CalculateHonorGain(
                            _session->GetPlayer()->GetQuestLevel(quest)));
    data << float(0.0f); // unk, honor multiplier?
    data << uint32(
        quest->GetRewSpell()); // reward spell, this spell will display (icon)
                               // (casted if RewSpellCast == 0)
    data << int32(quest->GetRewSpellCast()); // casted spell
    data << uint32(
        quest->GetCharTitleId()); // CharTitleId, new 2.4.0, player gets this
                                  // title (id from CharTitles)
    ...
    for (uint32 i = 0; i < QUEST_EMOTE_COUNT; ++i) {
        ...
        data << uint32(
            quest->DetailsEmoteDelay[i]); // DetailsEmoteDelay (in ms)
    }
    _session->SendPacket(&data);

    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUESTGIVER_QUEST_DETAILS {}, questid={}",
              npcGUID.ToString(),
              quest->GetQuestId());
}
...
void PlayerMenu::SendQuestQueryResponse(Quest const* quest) const
{
    ...
    if (QuestLocale const* localeData =
        ...
        ObjectMgr::GetLocaleString(
            localeData->AreaDescription, locale, questAreaDescription);
        ObjectMgr::GetLocaleString(
            localeData->CompletedText, locale, questCompletedText);

        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
            ObjectMgr::GetLocaleString(
                localeData->ObjectiveText[i], locale, questObjectiveText[i]);
    }
    ...
    data << uint32(
        quest->GetQuestMethod()); // Accepted values: 0, 1 or 2. 0 ==
                                  // IsAutoComplete() (skip objectives/details)
    data << uint32(
        quest->GetQuestLevel()); // may be -1, static data, in other cases must
                                 // be used dynamic level: Player::GetQuestLevel
                                 // (0 is not known, but assuming this is no
                                 // longer valid for quest intended for client)
    data << uint32(quest->GetMinLevel()); // min level
    data << uint32(
        quest->GetZoneOrSort()); // zone or sort to display in quest log

    data << uint32(quest->GetType());             // quest type
    data << uint32(quest->GetSuggestedPlayers()); // suggested players count

    data << uint32(quest->GetRepObjectiveFaction()); // shown in quest log as
                                                     // part of quest objective
    data << uint32(quest->GetRepObjectiveValue()); // shown in quest log as part
                                                   // of quest objective

    data << uint32(
        quest->GetRepObjectiveFaction2()); // shown in quest log as part of
                                           // quest objective OPPOSITE faction
    data << uint32(
        quest->GetRepObjectiveValue2()); // shown in quest log as part of quest
                                         // objective OPPOSITE faction

    data << uint32(quest->GetNextQuestInChain()); // client will request this
                                                  // quest from NPC, if not 0
    data << uint32(
        quest->GetXPId()); // used for calculating rewarded experience
    ...
    data << uint32(
        quest->GetRewMoneyMaxLevel()); // used in XP calculation at client
    data << uint32(
        quest->GetRewSpell()); // reward spell, this spell will display (icon)
                               // (cast if RewSpellCast == 0)
    ...
    for (uint8 i = 0; i < QUEST_REPUTATIONS_COUNT;
         ++i) // columnid+1 QuestFactionReward.dbc?
        data << int32(quest->RewardFactionValueId[i]);
    ...
    data << questCompletedText; // display in quest objectives window once all
                                // objectives are completed

    for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i) {
        if (quest->RequiredNpcOrGo[i] < 0)
            data << uint32((quest->RequiredNpcOrGo[i] * (-1)) |
                           0x80000000); // client expects gameobject template id
                                        // in form (id|0x80000000)
        else
    ...
    _session->SendPacket(&data);
    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUEST_QUERY_RESPONSE questid={}",
              quest->GetQuestId());
}

void PlayerMenu::SendQuestGiverOfferReward(Quest const* quest,
                                           ObjectGuid   npcGUID,
                                           bool         enableNext) const
{
    std::string questTitle = quest->GetTitle();
    ...
    if (QuestOfferRewardLocale const* questOfferRewardLocale =
            sObjectMgr->GetQuestOfferRewardLocale(quest->GetQuestId()))
        ObjectMgr::GetLocaleString(
            questOfferRewardLocale->RewardText, locale, RewardText);
    ...
    for (uint32 i = 0; i < QUEST_REPUTATIONS_COUNT;
         ++i) // columnid in QuestFactionReward.dbc (zero based)?
        data << int32(quest->RewardFactionValueId[i]);

    for (uint32 i = 0; i < QUEST_REPUTATIONS_COUNT;
         ++i) // reward reputation override?
        data << uint32(quest->RewardFactionValueIdOverride[i]);

    _session->SendPacket(&data);
    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUESTGIVER_OFFER_REWARD {}, questid={}",
              npcGUID.ToString(),
              quest->GetQuestId());
}
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const* quest,
                                            ObjectGuid   npcGUID,
                                            bool         canComplete,
                                            bool         closeOnCancel) const
{
    // We can always call to RequestItems, but this packet only goes out if
    // there are actually items.  Otherwise, we'll skip straight to the
    // OfferReward
    ...
    if (QuestRequestItemsLocale const* questRequestItemsLocale =
            sObjectMgr->GetQuestRequestItemsLocale(quest->GetQuestId()))
        ObjectMgr::GetLocaleString(
            questRequestItemsLocale->CompletionText, locale, requestItemsText);
    ...
    QuestStatusMap::iterator qsitr =
        _player->getQuestStatusMap().find(quest->GetQuestId());
    if (qsitr != _player->getQuestStatusMap().end() &&
        qsitr->second.Status == QUEST_STATUS_INCOMPLETE) {
        for (uint8 i = 0; i < 6; ++i)
            if (quest->RequiredItemId[i] &&
                qsitr->second.ItemCount[i] < quest->RequiredItemCount[i])
                if (_player->GetItemCount(quest->RequiredItemId[i], false) >=
    ...
    // Required Money
    data << uint32(quest->GetRewOrReqMoney() < 0 ? -quest->GetRewOrReqMoney()
                                                 : 0);
    ...
    _session->SendPacket(&data);
    LOG_DEBUG("network",
              "WORLD: Sent SMSG_QUESTGIVER_REQUEST_ITEMS {}, questid={}",
              npcGUID.ToString(),
              quest->GetQuestId());
}
