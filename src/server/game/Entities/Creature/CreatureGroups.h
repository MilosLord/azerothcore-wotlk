...
enum class GroupAIFlags : uint16 {
    ...
    GROUP_AI_FLAG_EVADE_MASK =
        GROUP_AI_FLAG_EVADE_TOGETHER | GROUP_AI_FLAG_RESPAWN_ON_EVADE,

    // Used to verify valid and usable flags
    GROUP_AI_FLAG_SUPPORTED =
        GROUP_AI_FLAG_ASSIST_MASK | GROUP_AI_FLAG_EVADE_MASK |
...
struct FormationInfo {
    FormationInfo()
        : leaderGUID(0), follow_dist(0.0f), follow_angle(0.0f), groupAI(0),
          point_1(0), point_2(0)
    {
    }
...
typedef std::unordered_map<ObjectGuid::LowType /*memberDBGUID*/,
...
public:
    ...
    explicit CreatureGroup(uint32 id)
        : m_leader(nullptr), m_groupID(id), m_Formed(false)
    {
    }
    ~CreatureGroup() {}
...
private:
    Creature* m_leader; // Important do not forget sometimes to work with
                        // pointers instead synonims :D:D
