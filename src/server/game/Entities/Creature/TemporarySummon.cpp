...
TempSummon::TempSummon(SummonPropertiesEntry const* properties,
                       ObjectGuid                   owner,
                       bool                         isWorldObject)
    : Creature(isWorldObject), m_Properties(properties),
      m_type(TEMPSUMMON_MANUAL_DESPAWN), m_timer(0), m_lifetime(0),
...
WorldObject* TempSummon::GetSummoner() const
{
    return m_summonerGUID
               ? ObjectAccessor::GetWorldObject(*this, m_summonerGUID)
               : nullptr;
}
...
{
    return m_summonerGUID ? ObjectAccessor::GetCreature(*this, m_summonerGUID)
                          : nullptr;
}
...
void TempSummon::InitStats(uint32 duration)
{
    ...
    if (m_type == TEMPSUMMON_MANUAL_DESPAWN)
        m_type = (duration == 0) ? TEMPSUMMON_DEAD_DESPAWN
    ...
    if (owner) {
        if (uint32 slot = m_Properties->Slot) {
            if (owner->m_SummonSlot[slot] &&
                owner->m_SummonSlot[slot] != GetGUID()) {
                Creature* oldSummon =
                    GetMap()->GetCreature(owner->m_SummonSlot[slot]);
...
void TempSummon::SetTempSummonType(TempSummonType type) { m_type = type; }
...
void TempSummon::RemoveFromWorld()
{
    ...
    // if (GetOwnerGUID())
    //     LOG_ERROR("entities.unit", "Unit {} has owner guid when removed from
    //     world", GetEntry());
...
std::string TempSummon::GetDebugInfo() const
{
    ...
    sstr << Creature::GetDebugInfo() << "\n"
         << std::boolalpha
         << "TempSummonType : " << std::to_string(GetSummonType())
         << " Summoner: " << GetSummonerGUID().ToString();
    return sstr.str();
}

Minion::Minion(SummonPropertiesEntry const* properties,
               ObjectGuid                   owner,
               bool                         isWorldObject)
...
{
    return IsPet() ||
           (m_Properties && m_Properties->Category == SUMMON_CATEGORY_PET);
}
...
void Guardian::InitSummon()
{
    ...
    if (Unit* m_owner = GetOwner()) {
        if (m_owner->GetTypeId() == TYPEID_PLAYER &&
            m_owner->GetMinionGUID() == GetGUID() && !m_owner->GetCharmGUID()) {
...
Puppet::Puppet(SummonPropertiesEntry const* properties, ObjectGuid owner)
    : Minion(properties, owner, false), m_owner(owner) // maybe true?
{
