...
CreatureMovementData::CreatureMovementData()
    : Ground(CreatureGroundMovementType::Run),
      Flight(CreatureFlightMovementType::None), Swim(true), Rooted(false),
...
{
    constexpr std::array<char const*, 3> GroundStates = {
        "None", "Run", "Hover"};
    constexpr std::array<char const*, 3> FlightStates = {
        "None", "DisableGravity", "CanFly"};
    constexpr std::array<char const*, 3> ChaseStates = {
        "Run", "CanWalk", "AlwaysWalk"};
    constexpr std::array<char const*, 3> RandomStates = {
        "Walk", "CanRun", "AlwaysRun"};
...
VendorItemCount::VendorItemCount(uint32 _item, uint32 _count)
    : itemId(_item), count(_count),
      lastIncrementTime(GameTime::GetGameTime().count())
{
}
...
void CreatureTemplate::InitializeQueryData()
{
    ...
    queryData << Name;
    queryData << uint8(0) << uint8(0)
              << uint8(0); // name2, name3, name4, always empty
    queryData << SubName;
...
bool AssistDelayEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    if (Unit* victim = ObjectAccessor::GetUnit(*m_owner, m_victim)) {
        ...
        m_owner->SetLastDamagedTime(GameTime::GetGameTime().count() +
                                    MAX_AGGRO_RESET_TIME);

        while (!m_assistants.empty()) {
            Creature* assistant =
                ObjectAccessor::GetCreature(*m_owner, *m_assistants.begin());
            ...
            if (assistant && assistant->CanAssistTo(m_owner, victim)) {
                ...
                assistant->SetLastDamagedTimePtr(
                    m_owner->GetLastDamagedTimePtr());
            }
        }
    }
    return true;
...
bool ForcedDespawnDelayEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    m_owner.DespawnOrUnsummon(
        0s, m_respawnTimer); // since we are here, we are not TempSummon as
                             // object type cannot change during runtime
    return true;
}
...
bool TemporaryThreatModifierEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    if (Unit* victim = ObjectAccessor::GetUnit(m_owner, m_threatVictimGUID)) {
        if (m_owner.IsInCombatWith(victim)) {
            m_owner.GetThreatMgr().ModifyThreatByPercent(
                victim, -100); // Reset threat to zero.
            m_owner.GetThreatMgr().AddThreat(
                victim, m_threatValue); // Set to the previous value it had,
                                        // first before modification.
        }
    }
...
Creature::Creature(bool isWorldObject)
    : Unit(isWorldObject), MovableMapObject(), m_groupLootTimer(0),
      lootingGroupLowGUID(0), m_lootRecipientGroup(0), m_corpseRemoveTime(0),
      m_respawnTime(0), m_respawnDelay(300), m_corpseDelay(60),
      m_wanderDistance(0.0f), m_boundaryCheckTime(2500),
      m_transportCheckTimer(1000), lootPickPocketRestoreTime(0),
      m_combatPulseTime(0), m_combatPulseDelay(0),
      m_reactState(REACT_AGGRESSIVE), m_defaultMovementType(IDLE_MOTION_TYPE),
      m_spawnId(0), m_equipmentId(0), m_originalEquipmentId(0),
      m_AlreadyCallAssistance(false), m_AlreadySearchedAssistance(false),
      m_regenHealth(true), m_regenPower(true), m_AI_locked(false),
      m_meleeDamageSchoolMask(SPELL_SCHOOL_MASK_NORMAL), m_originalEntry(0),
      m_moveInLineOfSightDisabled(false),
      m_moveInLineOfSightStrictlyDisabled(false), m_homePosition(),
      m_transportHomePosition(), m_creatureInfo(nullptr),
      m_creatureData(nullptr), m_detectionDistance(20.0f), m_waypointID(0),
      m_path_id(0), m_formation(nullptr), _lastDamagedTime(nullptr),
      m_cannotReachTimer(0), _isMissingSwimmingFlagOutOfCombat(false),
      m_assistanceTimer(0), _playerDamageReq(0), _damagedByPlayer(false),
...
void Creature::AddToWorld()
{
    ...
    if (!IsInWorld()) {
        // pussywizard: motion master needs to be initialized before
        // OnCreatureCreate, which may set death state to JUST_DIED, to prevent
        // crash it's also initialized in AIM_Initialize(), few lines below, but
        // it's not a problem
        Motion_Initialize();

        GetMap()->GetObjectsStore().Insert<Creature>(GetGUID(), this);
        if (m_spawnId) {
            GetMap()->GetCreatureBySpawnIdStore().insert(
                std::make_pair(m_spawnId, this));
        }
        Unit::AddToWorld();
...
void Creature::RemoveFromWorld()
{
    if (IsInWorld()) {
        ...
        if (m_spawnId)
            Acore::Containers::MultimapErasePair(
                GetMap()->GetCreatureBySpawnIdStore(), m_spawnId, this);
...
void Creature::SearchFormation()
{
    ...
    CreatureGroupInfoType::const_iterator frmdata =
        sFormationMgr->CreatureGroupMap.find(spawnId);
...
void Creature::RemoveCorpse(bool setSpawnTime, bool skipVisibility)
{
    ...
    // pussywizard: if corpse was removed during falling then the falling will
    // continue after respawn, so stop falling is such case
    if (IsFalling())
        StopMoving();
}
...
bool Creature::InitEntry(uint32 Entry, const CreatureData* data)
{
    ...
    if (!normalInfo) {
        LOG_ERROR("sql.sql",
                  "Creature::InitEntry creature entry {} does not exist.",
                  Entry);
        return false;
    }
    ...
    for (uint8 diff = uint8(GetMap()->GetSpawnMode()); diff > 0 && !IsPet();) {
        ...
        if (normalInfo->DifficultyEntry[diff - 1]) {
            cinfo = sObjectMgr->GetCreatureTemplate(
                normalInfo->DifficultyEntry[diff - 1]);
            if (cinfo)
                break; // template found

            // check and reported at startup, so just ignore (restore
            // normalInfo)
            cinfo = normalInfo;
        }

        // for instances heroic to normal, other cases attempt to retrieve
        // previous difficulty
    ...
    if (!(cinfo->GetFirstValidModelId())) {
        LOG_ERROR("sql.sql",
                  "Creature (Entry: {}) has no model defined in table "
    ...
    if (!sObjectMgr->GetCreatureModelRandomGender(
            &displayID)) // Cancel load if no model defined
    {
        LOG_ERROR("sql.sql",
                  "Creature (Entry: {}) has no model defined in table "
                  "`creature_template`, can't load. ",
                           ...
                           bool                updateAI)
{
    ...
    // creatures always have melee weapon ready if any unless specified
    // otherwise
    if (!GetCreatureAddon())
    ...
    ObjectMgr::ChooseCreatureFlags(
        cInfo, npcflag, unit_flags, dynamicflags, data);
    ...
    if (previousHealth > 0) {
        ...
        if (previousMaxHealth && previousMaxHealth > GetMaxHealth()) {
            _playerDamageReq = (uint32)(previousPlayerDamageReq *
                                        GetMaxHealth() / previousMaxHealth);
        }
        else {
    ...
    CreatureBaseStats const* stats =
        sObjectMgr->GetCreatureBaseStats(GetLevel(), cInfo->unit_class);
    float armor = stats->GenerateArmor(cInfo);
    SetModifierValue(UNIT_MOD_ARMOR, BASE_VALUE, armor);
    SetModifierValue(UNIT_MOD_RESISTANCE_HOLY,
                     BASE_VALUE,
                     float(cInfo->resistance[SPELL_SCHOOL_HOLY]));
    SetModifierValue(UNIT_MOD_RESISTANCE_FIRE,
                     BASE_VALUE,
                     float(cInfo->resistance[SPELL_SCHOOL_FIRE]));
    SetModifierValue(UNIT_MOD_RESISTANCE_NATURE,
                     BASE_VALUE,
                     float(cInfo->resistance[SPELL_SCHOOL_NATURE]));
    SetModifierValue(UNIT_MOD_RESISTANCE_FROST,
                     BASE_VALUE,
                     float(cInfo->resistance[SPELL_SCHOOL_FROST]));
    SetModifierValue(UNIT_MOD_RESISTANCE_SHADOW,
                     BASE_VALUE,
                     float(cInfo->resistance[SPELL_SCHOOL_SHADOW]));
    SetModifierValue(UNIT_MOD_RESISTANCE_ARCANE,
                     BASE_VALUE,
                     float(cInfo->resistance[SPELL_SCHOOL_ARCANE]));
    ...
    if (FactionTemplateEntry const* factionTemplate =
        ...
        if (factionTemplate->factionFlags &
    ...
    // updates spell bars for vehicles and set player's faction - should be
    // called here, to overwrite faction that is set from the new template
    if (IsVehicle()) {
        if (Player* owner = Creature::
                GetCharmerOrOwnerPlayerOrPlayerItself()) // this check comes in
                                                         // case we don't have a
                                                         // player
        {
            SetFaction(owner->GetFaction()); // vehicles should have same as
                                             // owner faction
...
void Creature::Update(uint32 diff)
{
    ...
    case DeathState::JustRespawned:
        // Must not be called, see Creature::setDeathState JUST_RESPAWNED ->
        // ALIVE promoting.
        LOG_ERROR("entities.unit",
                  "Creature ({}) in wrong state: DeathState::JustRespawned (4)",
                  GetGUID().ToString());
        break;
    case DeathState::JustDied:
        // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE
        // promoting.
        LOG_ERROR("entities.unit",
                  "Creature ({}) in wrong state: DeathState::JustDead (1)",
                  GetGUID().ToString());
        break;
    ...
    case DeathState::Alive: {
        ...
        if (NeedChangeAI) {
            ...
            // xinef: update combat state, if npc is not in combat - return to
            // spawn correctly by calling EnterEvadeMode
            SelectVictim();
        }

        // if periodic combat pulse is enabled and we are both in combat and in
        // a dungeon, do this now
        ...
        if (IsCharmed() &&
            !IsWithinDistInMap(
                owner, GetMap()->GetVisibilityRange(), true, false)) {
            RemoveCharmAuras();
        }

        if (Unit* victim = GetVictim()) {
            // If we are closer than 50% of the combat reach we are going to
            // reposition the victim
            if (diff >= m_moveBackwardsMovementTime) {
                float MaxRange =
                    GetCollisionRadius() + GetVictim()->GetCollisionRadius();

                if (IsInDist(victim, MaxRange))
                    AI()->MoveBackwardsChecks();

                m_moveBackwardsMovementTime =
                    urand(MOVE_BACKWARDS_CHECK_INTERVAL,
                          MOVE_BACKWARDS_CHECK_INTERVAL * 3);
            }
            else {
            ...
            if (diff >= m_moveCircleMovementTime) {
                AI()->MoveCircleChecks();
                m_moveCircleMovementTime = urand(
                    MOVE_CIRCLE_CHECK_INTERVAL, MOVE_CIRCLE_CHECK_INTERVAL * 2);
            }
            else {
        ...
        if (m_assistanceTimer) {
            if (m_assistanceTimer <= diff) {
                ...
                m_assistanceTimer = sWorld->getIntConfig(
        ...
        if (m_regenTimer <= 0) {
            if (!IsInEvadeMode()) {
                ...
                if (!IsInCombat() || IsPolymorphed())
                    ...
                    // regenerate health if cannot reach the target and the
                    // setting is set to do so. this allows to disable the
                    // health regen of raid bosses if pathfinding has issues for
                    // whatever reason
                    if (sWorld->getBoolConfig(
                        ...
                        RegenerateHealth();
                        LOG_DEBUG("entities.unit",
                                  "RegenerateHealth() enabled because Creature "
                                  "cannot reach the target. Detail: {}",
                                  GetDebugInfo());
                    }
                    else
                        LOG_DEBUG(
                            "entities.unit",
                            "RegenerateHealth() disabled even if the Creature "
                            "cannot reach the target. Detail: {}",
                            GetDebugInfo());
                }
            }
        ...
        if (CanNotReachTarget() && !IsInEvadeMode()) {
            ...
            if (m_cannotReachTimer >=
                (sWorld->getIntConfig(CONFIG_NPC_EVADE_IF_NOT_REACHABLE) *
                 IN_MILLISECONDS)) {
                Player* cannotReachPlayer =
                    ObjectAccessor::GetPlayer(*this, m_cannotReachTarget);
                if (cannotReachPlayer && IsEngagedBy(cannotReachPlayer) &&
                    IsAIEnabled &&
                    AI()->OnTeleportUnreacheablePlayer(cannotReachPlayer)) {
                    ...
                    auto EnterEvade = [&]() {
                        if (CreatureAI* ai = AI()) {
                            ai->EnterEvadeMode(
                                CreatureAI::EvadeReason::EVADE_REASON_NO_PATH);
                        }
                    };
        ...
        if (GetOwnerGUID().IsPlayer()) {
            if (m_transportCheckTimer <= diff) {
                ...
                Transport* newTransport =
                    GetMap()->GetTransportForPos(GetPhaseMask(),
                                                 GetPositionX(),
                                                 GetPositionY(),
                                                 GetPositionZ(),
                                                 this);
...
void Creature::Regenerate(Powers power)
{
    ...
    case POWER_MANA: {
        ...
        if (IsInCombat() || GetCharmerOrOwnerGUID()) {
            if (GetEntry() == NPC_IMP ||
                GetEntry() == NPC_WATER_ELEMENTAL_TEMP ||
                GetEntry() == NPC_WATER_ELEMENTAL_PERM) {
                addvalue =
                    uint32((GetStat(STAT_SPIRIT) /
                                (IsUnderLastManaUseEffect() ? 8.0f : 5.0f) +
                            17.0f));
            }
    ...
    addvalue +=
        GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_POWER_REGEN, power) *
        (power == POWER_FOCUS ? PET_FOCUS_REGEN_INTERVAL.count()
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius > 0) {
        ...
        Acore::NearestAssistCreatureInCreatureRangeCheck u_check(
            this, GetVictim(), radius);
        Acore::CreatureLastSearcher<
            Acore::NearestAssistCreatureInCreatureRangeCheck>
            searcher(this, creature, u_check);
                      ...
                      const CreatureData* data)
{
    ...
    if (!cinfo) {
        LOG_ERROR("sql.sql",
                  "Creature::Create(): creature template (guidlow: {}, entry: "
                  "{}) does not exist.",
                  guidlow,
                  Entry);
        return false;
    ...
    if (!IsPositionValid()) {
        LOG_ERROR("entities.unit",
                  "Creature::Create(): given coordinates for creature (guidlow "
                  "{}, entry {}) are not valid (X: {}, Y: {}, Z: {}, O: {})",
                  guidlow,
                  Entry,
                  x,
                  y,
    ...
    // area/zone id is needed immediately for ZoneScript::GetCreatureEntry hook
    // before it is known which creature template to load (no model/scale
    // available yet)
    PositionFullTerrainStatus terrainData;
    GetMap()->GetFullTerrainStatusForPosition(GetPhaseMask(),
                                              GetPositionX(),
                                              GetPositionY(),
                                              GetPositionZ(),
    ...
    // oX = x;     oY = y;    dX = x;    dY = y;    m_moveTime = 0; m_startMove
    // = 0;
    ...
    if (sObjectMgr->GetCreatureModelRandomGender(&displayID) &&
        !IsTotem()) // Cancel load if no model defined or if totem
    {
        SetDisplayId(displayID);
        SetNativeDisplayId(displayID);
    }
    ...
    //! Need to be called after LoadCreaturesAddon - MOVEMENTFLAG_HOVER is set
    //! there
    ...
    if (IsSpiritHealer() || IsSpiritGuide() ||
        (GetCreatureTemplate()->flags_extra &
    ...
    else if (cinfo->type_flags &
             CREATURE_TYPE_FLAG_VISIBLE_TO_GHOSTS) // Xinef: Add ghost
                                                   // visibility for ghost units
        m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GHOST,
    ...
    if (GetCreatureTemplate()->flags_extra &
...
bool Creature::isCanInteractWithBattleMaster(Player* player, bool msg) const
{
    ...
    BattlegroundTypeId bgTypeId =
        sBattlegroundMgr->GetBattleMasterBG(GetEntry());
    if (!msg)
...
bool Creature::isCanTrainingAndResetTalentsOf(Player* player) const
{
    return player->GetLevel() >= 10 &&
           ...
           player->IsClass((Classes)GetCreatureTemplate()->trainer_class,
                           CLASS_CONTEXT_CLASS_TRAINER);
}

bool Creature::IsValidTrainerForPlayer(Player* player,
                                       uint32* npcFlags /*= nullptr*/) const
{
    if (!IsTrainer()) {
        return false;
    }
    ...
    case TRAINER_TYPE_PETS:
        if (m_creatureInfo->trainer_class &&
    ...
    case TRAINER_TYPE_MOUNTS:
        if (m_creatureInfo->trainer_race &&
            m_creatureInfo->trainer_race != player->getRace()) {
            return false;
        }
        break;
    case TRAINER_TYPE_TRADESKILLS:
        if (m_creatureInfo->trainer_spell &&
            !player->HasSpell(m_creatureInfo->trainer_spell)) {
            if (npcFlags)
...
// return true if this creature is tapped by the player or by a member of his
// group.
bool Creature::isTappedBy(Player const* player) const
{
    ...
    if (!playerGroup ||
        playerGroup != GetLootRecipientGroup()) // if we dont have a group we
                                                // arent the recipient
        return false; // if creature doesnt have group bound it means it was
                      // solo killed by someone else
...
void Creature::SaveToDB()
{
    ...
    uint32 mapId = GetTransport() && GetTransport()->ToMotionTransport()
                       ? GetTransport()->GetGOInfo()->moTransport.mapID
                       : GetMapId();
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    data.wander_distance =
        GetDefaultMovementType() == IDLE_MOTION_TYPE ? 0.0f : m_wanderDistance;
    data.currentwaypoint = 0;
    data.curhealth       = GetHealth();
    data.curmana         = GetPower(POWER_MANA);
    // prevent add data integrity problems
    data.movementType =
        !m_wanderDistance && GetDefaultMovementType() == RANDOM_MOTION_TYPE
            ? IDLE_MOTION_TYPE
    ...
    WorldDatabasePreparedStatement* stmt =
        WorldDatabase.GetPreparedStatement(WORLD_DEL_CREATURE);
                               ...
                               const CreatureData* data)
{
    ...
    if (!normalInfo) {
        LOG_ERROR("sql.sql",
                  "Creature::CreateFromProto(): creature template (guidlow: "
    ...
    Object::_Create(guidlow,
                    Entry,
                    (vehId || normalInfo->VehicleId) ? HighGuid::Vehicle
                                                     : HighGuid::Unit);

    // Xinef: select proper vehicle id
    if (!vehId) {
        CreatureTemplate const* cinfo = normalInfo;
        for (uint8 diff = uint8(GetMap()->GetSpawnMode());
             diff > 0 && !IsPet();) {
            // we already have valid Map pointer for current creature!
        ...
        if (cinfo->VehicleId)
            CreateVehicleKit(cinfo->VehicleId,
                             (cinfo->VehicleId != normalInfo->VehicleId
                                  ? cinfo->Entry
                                  : normalInfo->Entry));
    }
    else
...
{
    return m_creatureInfo->IconName == "Speak" &&
           m_creatureData->npcflag & UNIT_NPC_FLAG_VENDOR;
}
                                  ...
                                  bool allowDuplicate /*= false*/)
{
    if (!allowDuplicate) {
        ...
        // If only dead instance(s) exist, despawn them and spawn a new (maybe
        // also dead) version
        const auto creatureBounds =
            map->GetCreatureBySpawnIdStore().equal_range(spawnId);
        ...
        if (creatureBounds.first != creatureBounds.second) {
            for (auto itr = creatureBounds.first; itr != creatureBounds.second;
                ...
                if (itr->second->IsAlive()) {
                    LOG_DEBUG(
                        "maps",
                        "Would have spawned {} but {} already exists",
                        spawnId,
                        creatureBounds.first->second->GetGUID().ToString());
                    return false;
                }
                else {
                    despawnList.push_back(itr->second);
                    LOG_DEBUG("maps",
                              "Despawned dead instance of spawn {} ({})",
                              spawnId,
                              itr->second->GetGUID().ToString());
                }
            }
    ...
    if (!data) {
        LOG_ERROR("sql.sql",
                  "Creature (SpawnId: {}) not found in table `creature`, can't "
                  "load. ",
                  spawnId);
        return false;
    }

    // xinef: this has to be assigned before Create function, properly loads
    // equipment id from DB
    m_creatureData = data;
    m_spawnId      = spawnId;
    ...
    if (!Create(map->GenerateLowGuid<HighGuid::Unit>(),
                map,
                data->phaseMask,
                entry,
                0,
    ...
    if (m_respawnTime) // respawn on Update
    {
        ...
        if (CanFly()) {
            float tz = map->GetHeight(GetPhaseMask(),
                                      data->posX,
                                      data->posY,
                                      data->posZ,
                                      true,
    ...
    if (!m_regenHealth) {
        ...
        if (curhealth) {
            curhealth =
                uint32(curhealth * _GetHealthMod(GetCreatureTemplate()->rank));
            if (curhealth < 1)
                curhealth = 1;
        }
...
bool Creature::hasQuest(uint32 quest_id) const
{
    QuestRelationBounds qr =
        sObjectMgr->GetCreatureQuestRelationBounds(GetEntry());
...
void Creature::DeleteFromDB()
{
    if (!m_spawnId) {
        LOG_ERROR("entities.unit",
                  "Trying to delete not saved creature: {}",
                  GetGUID().ToString());
        return;
    }
...
bool Creature::CanStartAttack(Unit const* who) const
{
    ...
    if ((IsImmuneToNPC() &&
         who->GetTypeId() !=
             TYPEID_PLAYER) || // flag is valid only for non player characters
        (IsImmuneToPC() &&
         who->GetTypeId() == TYPEID_PLAYER)) // immune to PC and target is a
                                             // player, return false
    {
        return false;
    }
    ...
    if (Unit* owner = who->GetOwner())
        if (owner->GetTypeId() == TYPEID_PLAYER &&
            IsImmuneToPC()) // immune to PC and target has player owner
            return false;

    // Do not attack non-combat pets
    if (who->GetTypeId() == TYPEID_UNIT &&
        who->GetCreatureType() == CREATURE_TYPE_NON_COMBAT_PET)
        return false;

    if (!CanFly() && (GetDistanceZ(who) >
                      CREATURE_Z_ATTACK_RANGE +
                          m_CombatDistance)) // too much Z difference, skip very
                                             // costy vmap calculations here
        return false;

    if (!_IsTargetAcceptable(who))
        return false;

    // pussywizard: at this point we are either hostile to who or friendly to
    // who->getAttackerForHelper() pussywizard: if who is in combat and has an
    // attacker, help him if the distance is right (help because who is hostile
    // or help because attacker is friendly)
    bool assist = false;
    ...
    if (!assist)
        if (IsNeutralToAll() ||
            !IsWithinDistInMap(who,
                               GetAggroRange(who) + m_CombatDistance,
                               true,
                               false)) // pussywizard: +m_combatDistance for
                                       // turrets and similar
            return false;
...
void Creature::setDeathState(DeathState s, bool despawn)
{
    ...
    if (s == DeathState::JustDied) {
        ...
        m_respawnTime =
            GameTime::GetGameTime().count() + m_respawnDelay + m_corpseDelay;

        // always save boss respawn time at death to prevent crash cheating
        if (GetMap()->IsDungeon() || isWorldBoss() ||
            GetCreatureTemplate()->rank >= CREATURE_ELITE_ELITE)
            SaveRespawnTime();

        SetTarget(); // remove target selection in any cases (can be set at aura
                     // remove in Unit::setDeathState)
        ReplaceAllNpcFlags(UNIT_NPC_FLAG_NONE);

        Dismount(); // if creature is mounted on a virtual mount, remove it at
                    // death
        ...
        bool needsFalling =
            !despawn && (IsFlying() || IsHovering()) && !IsUnderWater();
        ...
        ClearUnitState(
            uint32(UNIT_STATE_ALL_STATE & ~(UNIT_STATE_IGNORE_PATHFINDING |
...
void Creature::Respawn(bool force)
{
    ...
    ConditionList conditions = sConditionMgr->GetConditionsForNotGroupedEntry(
        CONDITION_SOURCE_TYPE_CREATURE_RESPAWN, GetEntry());

    if (!sConditionMgr->IsObjectMeetToConditions(this, conditions) && !force) {
        // Creature should not respawn, reset respawn timer. Conditions will be
        // checked again the next time it tries to respawn.
        m_respawnTime = GameTime::GetGameTime().count() + m_respawnDelay;
        return;
    }

    bool allowed = !IsAIEnabled ||
                   AI()->CanRespawn(); // First check if there are any scripts
                                       // that prevent us respawning
    if (!allowed && !force)            // Will be rechecked on next Update call
        return;

    ObjectGuid dbtableHighGuid = ObjectGuid::Create<HighGuid::Unit>(
        m_creatureData ? m_creatureData->id1 : GetEntry(), m_spawnId);
        ...
        force) // Should respawn
    {
        ...
        if (getDeathState() == DeathState::Dead) {
            if (m_spawnId) {
                ...
                CreatureData const* data =
                    sObjectMgr->GetCreatureData(m_spawnId);
                // Respawn check if spawn has 2 entries
                if (data->id2) {
                    uint32 entry = GetRandomId(data->id1, data->id2, data->id3);
                    UpdateEntry(entry, data, true); // Select Random Entry
                    m_defaultMovementType = MovementGeneratorType(
                        data->movementType);          // Reload Movement Type
            ...
            if (IsAIEnabled) {
                // reset the AI to be sure no dirty or uninitialized values will
                // be used till next tick
                AI()->Reset();
                TriggerJustRespawned =
                    true; // delay event to next tick so all creatures are
                          // created on the map before processing
            }

            uint32 poolid =
                m_spawnId ? sPoolMgr->IsPartOfAPool<Creature>(m_spawnId) : 0;
            if (poolid)
                sPoolMgr->UpdatePool<Creature>(poolid, m_spawnId);

            // Re-initialize reactstate that could be altered by
            // movementgenerators
            InitializeReactState();
        ...
        m_respawnedTime = GameTime::GetGameTime().count();
        // xinef: relocate notifier, fixes npc appearing in corpse position
        // after forced respawn (instead of spawn)
        m_last_notify_position.Relocate(-5000.0f, -5000.0f, -5000.0f, 0.0f);
    ...
    else // the master is dead
    {
        ObjectGuid targetGuid =
            sObjectMgr->GetLinkedRespawnGuid(dbtableHighGuid);
        if (targetGuid == dbtableHighGuid) // if linking self, never respawn
                                           // (check delayed to next day)
        {
            SetRespawnTime(DAY);
        }
        else {
...
{
    if (timeMSToDespawn) {
        ForcedDespawnDelayEvent* pEvent =
            new ForcedDespawnDelayEvent(*this, forceRespawnTimer);
...
void Creature::DespawnOrUnsummon(Milliseconds msTimeToDespawn /*= 0*/,
...
void Creature::DespawnOnEvade(Seconds respawnDelay)
{
    ...
    if (respawnDelay < 2s) {
        LOG_WARN(
            "entities.unit",
            "DespawnOnEvade called with delay of {} seconds, defaulting to 2.",
            respawnDelay.count());
        respawnDelay = 2s;
    }

    if (TempSummon* whoSummon = ToTempSummon()) {
        LOG_WARN("entities.unit",
                 "DespawnOnEvade called on a temporary summon.");
        whoSummon->UnSummon();
        return;
    }
...
bool Creature::HasMechanicTemplateImmunity(uint32 mask) const
{
    return !GetOwnerGUID().IsPlayer() &&
           (GetCreatureTemplate()->MechanicImmuneMask & mask);
}

void Creature::LoadSpellTemplateImmunity()
{
    // uint32 max used for "spell id", the immunity system will not perform
    // SpellInfo checks against invalid spells used so we know which immunities
    // were loaded from template
    ...
    if (uint8 mask = GetCreatureTemplate()->SpellSchoolImmuneMask) {
        for (uint8 i = SPELL_SCHOOL_NORMAL; i <= SPELL_SCHOOL_ARCANE; ++i) {
            if (mask & (1 << i)) {
                ApplySpellImmune(
                    placeholderSpellId, IMMUNITY_SCHOOL, 1 << i, true);
            }
        }
    }
}
...
bool Creature::IsImmunedToSpell(SpellInfo const* spellInfo, Spell const* spell)
{
    ...
    if (spellInfo->Mechanic > MECHANIC_NONE &&
        HasMechanicTemplateImmunity(1 << (spellInfo->Mechanic - 1)))
        return true;

    // This check must be done instead of 'if
    // (GetCreatureTemplate()->MechanicImmuneMask & (1 << (spellInfo->Mechanic -
    // 1)))' for not break the check of mechanic immunity on DB (tested) because
    // GetCreatureTemplate()->MechanicImmuneMask and
    // m_spellImmune[IMMUNITY_MECHANIC] don't have same data.
    bool immunedToAllEffects = true;
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (spellInfo->Effects[i].IsEffect() &&
            !IsImmunedToSpellEffect(spellInfo, i)) {
                                      ...
                                      uint32           index) const
{
    ...
    if (spellInfo->Effects[index].Mechanic > MECHANIC_NONE &&
        HasMechanicTemplateImmunity(
            1 << (spellInfo->Effects[index].Mechanic - 1)))
        return true;

    if (GetCreatureTemplate()->type == CREATURE_TYPE_MECHANICAL &&
        spellInfo->Effects[index].Effect == SPELL_EFFECT_HEAL)
        return true;
...
SpellInfo const* Creature::reachWithSpellAttack(Unit* victim)
{
    ...
    for (uint32 i = 0; i < MAX_CREATURE_SPELLS; ++i) {
        ...
        if (!spellInfo) {
            LOG_ERROR(
                "entities.unit", "WORLD: unknown spell id {}", m_spells[i]);
            continue;
        }
        ...
        if (spellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE &&
            HasUnitFlag(UNIT_FLAG_SILENCED))
            continue;
        if (spellInfo->PreventionType == SPELL_PREVENTION_TYPE_PACIFY &&
            HasUnitFlag(UNIT_FLAG_PACIFIED))
            continue;
        return spellInfo;
    }
    return nullptr;
...
// select nearest hostile unit within the given distance (regardless of threat
// list).
Unit* Creature::SelectNearestTarget(float dist,
                                    bool  playerOnly /* = false */) const
{
    ...
    Acore::UnitLastSearcher<Acore::NearestHostileUnitCheck> searcher(
        this, target, u_check);
    Cell::VisitAllObjects(this, searcher, dist);
    return target;
}

// select nearest hostile unit within the given attack distance (i.e. distance
// is ignored if > than ATTACK_DISTANCE), regardless of threat list.
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    Acore::UnitLastSearcher<Acore::NearestHostileUnitInAttackDistanceCheck>
        searcher(this, target, u_check);
...
void Creature::SendAIReaction(AiReaction reactionType)
{
    ...
    LOG_DEBUG(
        "network", "WORLD: Sent SMSG_AI_REACTION, type {}.", reactionType);
}
...
void Creature::CallAssistance(Unit* target /*= nullptr*/)
{
    ...
    if (!m_AlreadyCallAssistance && target && !IsPet() && !IsCharmed()) {
        ...
        if (radius > 0) {
            ...
            Acore::CreatureListSearcher<Acore::AnyAssistCreatureInRangeCheck>
                searcher(this, assistList, u_check);
            Cell::VisitGridObjects(this, searcher, radius);

            if (!assistList.empty()) {
                AssistDelayEvent* e =
                    new AssistDelayEvent(target->GetGUID(), this);
                while (!assistList.empty()) {
                    // Pushing guids because in delay can happen some creature
                    // gets despawned => invalid pointer
                    e->AddAssistant((*assistList.begin())->GetGUID());
                    assistList.pop_front();
                }
                m_Events.AddEvent(
                    e,
                    m_Events.CalculateTime(sWorld->getIntConfig(
...
void Creature::CallForHelp(float radius, Unit* target /*= nullptr*/)
{
    ...
    Acore::CreatureWorker<Acore::CallOfHelpCreatureInRangeDo> worker(this,
                                                                     u_do);
                           ...
                           bool        checkfaction /*= true*/) const
{
    ...
    /// templates
    if (m_creatureInfo->HasFlagsExtra(
...
bool Creature::_IsTargetAcceptable(Unit const* target) const
{
    ...
    if (IsFriendlyTo(target) || !target->isTargetableForAttack(false, this) ||
        (m_vehicle &&
         (IsOnVehicle(target) || m_vehicle->GetBase()->IsOnVehicle(target))))
        return false;

    if (target->HasUnitState(UNIT_STATE_DIED)) {
        // some creatures can detect fake death
        if (CanIgnoreFeignDeath() &&
            target->HasUnitFlag2(UNIT_FLAG2_FEIGN_DEATH))
            return true;
        else
            return false;
    }
    ...
    // if I'm already fighting target, or I'm hostile towards the target, the
    // target is acceptable
    if (IsEngagedBy(target) || IsHostileTo(target))
        return true;

    // if the target's victim is friendly, and the target is neutral, the target
    // is acceptable
    if (targetVictim && !IsNeutralToAll() && IsFriendlyTo(targetVictim))
        return true;

    // if the target's victim is not friendly, or the target is friendly, the
...
void Creature::UpdateMoveInLineOfSightState()
{
    ...
    if (IsPet() ||
        HasUnitTypeMask(UNIT_MASK_MINION | UNIT_MASK_SUMMON |
                        UNIT_MASK_GUARDIAN | UNIT_MASK_CONTROLABLE_GUARDIAN) ||
    ...
    if (IsTrigger() || IsCivilian() ||
        GetCreatureType() == CREATURE_TYPE_NON_COMBAT_PET || IsCritter() ||
        GetAIName() == "NullCreatureAI") {
    ...
    if (FactionTemplateEntry const* factionTemplate =
        ...
        if (factionTemplate->hostileMask || factionTemplate->enemyFaction[0] ||
            factionTemplate->enemyFaction[1] ||
            factionTemplate->enemyFaction[2] ||
            factionTemplate->enemyFaction[3])
            nonHostile = false;
...
bool Creature::CanCreatureAttack(Unit const* victim, bool skipDistCheck) const
{
    ...
    if (m_respawnedTime &&
        (GameTime::GetGameTime().count() - m_respawnedTime) < 5 &&
        victim->getAttackers().empty()) {
        return false;
    }

    // if victim is in FD and we can't see that
    if (victim->HasUnitFlag2(UNIT_FLAG2_FEIGN_DEATH) &&
        !CanIgnoreFeignDeath()) {
        return false;
    }
    ...
    if (!GetCharmerOrOwnerGUID().IsPlayer()) {
        ...
        // pussywizard: don't check distance to home position if recently
        // damaged (allow kiting away from spawnpoint!) xinef: this should
        // include taunt auras
        if (!isWorldBoss() &&
            (GetLastDamagedTime() > GameTime::GetGameTime().count() ||
    ...
    float dist = std::min<float>(
        GetMap()->GetVisibilityRange() + GetObjectSize() * 2, 150.0f);

    if (Unit* unit = GetCharmerOrOwner())
        return victim->IsWithinDist(unit, dist);
    else {
        // to prevent creatures in air ignore attacks because distance is
        // already too high...
...
{
    if (m_spawnId) {
        if (CreatureAddon const* addon =
                sObjectMgr->GetCreatureAddon(m_spawnId))
            return addon;
    }
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    ...
    if (cainfo->bytes1 != 0) {
        ...
        SetByteValue(UNIT_FIELD_BYTES_1,
                     ...
                     uint8(cainfo->bytes1 & 0xFF));
        // SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_PET_TALENTS,
        // uint8((cainfo->bytes1 >> 8) & 0xFF));
        ...
        //! Suspected correlation between UNIT_FIELD_BYTES_1, offset 3, value
        //! 0x2: If no inhabittype_fly (if no MovementFlag_DisableGravity or
        //! MovementFlag_CanFly flag found in sniffs) Check using InhabitType as
        //! movement flags are assigned dynamically basing on whether the
        //! creature is in air or not Set MovementFlag_Hover. Otherwise do
        //! nothing.
        if (CanHover())
    ...
    if (cainfo->bytes2 != 0) {
        ...
        // SetByteValue(UNIT_FIELD_BYTES_2, 1, uint8((cainfo->bytes2 >> 8) &
        // 0xFF)); SetByteValue(UNIT_FIELD_BYTES_2, 2, uint8((cainfo->bytes2 >>
        // 16) & 0xFF));
        SetByteValue(UNIT_FIELD_BYTES_2, 2, 0);
        // SetByteValue(UNIT_FIELD_BYTES_2, 3, uint8((cainfo->bytes2 >> 24) &
        // 0xFF));
    ...
    if (!cainfo->auras.empty()) {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin();
             itr != cainfo->auras.end();
             ++itr) {
            SpellInfo const* AdditionalSpellInfo =
                sSpellMgr->GetSpellInfo(*itr);
            if (!AdditionalSpellInfo) {
                LOG_ERROR("sql.sql",
                          "Creature ({}) has wrong spell {} defined in `auras` "
                          "field.",
                          GetGUID().ToString(),
                          *itr);
                continue;
            ...
            if (HasAura(*itr)) {
                if (!reload)
                    LOG_ERROR("sql.sql",
                              "Creature ({}) has duplicate aura (spell {}) in "
                              "`auras` field.",
                              GetGUID().ToString(),
                              *itr);
...
void Creature::SendZoneUnderAttackMessage(Player* attacker)
{
    ...
    sWorld->SendGlobalMessage(
        &data,
        nullptr,
        (attacker->GetTeamId() == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE));
}
                                   ...
                                   uint32          unTimeMs)
{
    for (uint8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; ++i) {
        if (idSchoolMask & (1 << i)) {
            m_ProhibitSchoolTime[i] =
                GameTime::GetGameTimeMS().count() + unTimeMs;
        }
    }
}
                                ...
                                bool /*forceSendToSpectator*/)
{
    ...
    if (Player* modOwner = GetSpellModOwner()) {
        modOwner->ApplySpellMod(
            spellInfo->Id, SPELLMOD_COOLDOWN, spellcooldown);
        modOwner->ApplySpellMod(
            spellInfo->Id, SPELLMOD_COOLDOWN, categorycooldown);
    }

    SpellCategoryStore::const_iterator i_scstore =
        sSpellsByCategoryStore.find(categoryId);
    if (categorycooldown && i_scstore != sSpellsByCategoryStore.end()) {
        for (SpellCategorySet::const_iterator i_scset =
            ...
            _AddCreatureSpellCooldown(
                i_scset->second, categoryId, categorycooldown);
        }
    }
    ...
    if (sSpellMgr->HasSpellCooldownOverride(spellInfo->Id)) {
        if (IsCharmed() && GetCharmer()->IsPlayer()) {
            WorldPacket data;
            BuildCooldownPacket(
                data, SPELL_COOLDOWN_FLAG_NONE, spellInfo->Id, spellcooldown);
...
uint32 Creature::GetSpellCooldown(uint32 spell_id) const
{
    CreatureSpellCooldowns::const_iterator itr =
        m_CreatureSpellCooldowns.find(spell_id);
    if (itr == m_CreatureSpellCooldowns.end())
        return 0;

    return itr->second.end > GameTime::GetGameTimeMS().count()
               ? itr->second.end - GameTime::GetGameTimeMS().count()
               : 0;
}
...
bool Creature::HasSpell(uint32 spellID) const
{
    ...
    for (i = 0; i < MAX_CREATURE_SPELLS; ++i)
        if (spellID == m_spells[i])
            break;
    return i <
           MAX_CREATURE_SPELLS; // broke before end of iteration of known spells
}
...
void Creature::GetRespawnPosition(
    float& x, float& y, float& z, float* ori, float* dist) const
{
    ...
    // xinef: changed this from current position to home position, fixes world
    // summons with infinite duration
    if (GetTransport()) {
        x = GetPositionX();
        y = GetPositionY();
        z = GetPositionZ();
...
{
    if (CreatureMovementData const* movementOverride =
            sObjectMgr->GetCreatureMovementOverride(m_spawnId))
...
{
    if (loot.loot_type != LOOT_SKINNING && !IsPet() &&
        GetCreatureTemplate()->SkinLootId && hasLootRecipient()) {
    ...
    // corpse skinnable, but without skinning flag, and then skinned, corpse
    // will despawn next update
...
bool Creature::IsImmuneToKnockback() const
{
    ...
    CreatureTemplate const* cinfo = sObjectMgr->GetCreatureTemplate(GetEntry());
    return cinfo &&
           (cinfo->flags_extra & CREATURE_FLAG_EXTRA_IMMUNITY_KNOCKBACK);
}
...
bool Creature::SetWalk(bool enable)
{
    ...
    WorldPacket data(enable ? SMSG_SPLINE_MOVE_SET_WALK_MODE
                                 ...
                                 bool updateAnimationTier /*= true*/)
{
    ...
    WorldPacket data(disable ? SMSG_SPLINE_MOVE_GRAVITY_DISABLE
...
bool Creature::SetSwim(bool enable)
{
    ...
    WorldPacket data(enable ? SMSG_SPLINE_MOVE_START_SWIM
...
/**
 * @brief This method check the current flag/status of a creature and its
 * inhabit type
 *
 * Pets should swim by default to properly follow the player
 * NOTE: You can set the UNIT_FLAG_CANNOT_SWIM temporary to deny a creature to
 * swim
 *
 */
...
bool Creature::SetCanFly(bool enable, bool /*packetOnly*/ /* = false */)
{
    ...
    if (m_movedByPlayer) {
        sScriptMgr->AnticheatSetCanFlybyServer(m_movedByPlayer->ToPlayer(),
                                               enable);

        if (!enable)
            m_movedByPlayer->ToPlayer()->SetFallInformation(
                GameTime::GetGameTime().count(),
                m_movedByPlayer->ToPlayer()->GetPositionZ());

        WorldPacket data(
            enable ? SMSG_MOVE_SET_CAN_FLY : SMSG_MOVE_UNSET_CAN_FLY, 12);
        data << GetPackGUID();
        data << uint32(0); //! movement counter
        m_movedByPlayer->ToPlayer()->SendDirectMessage(&data);
    ...
    WorldPacket data(enable ? SMSG_SPLINE_MOVE_SET_FLYING
...
bool Creature::SetWaterWalking(bool enable, bool packetOnly /* = false */)
{
    ...
    WorldPacket data(
        enable ? SMSG_SPLINE_MOVE_WATER_WALK : SMSG_SPLINE_MOVE_LAND_WALK, 9);
    data << GetPackGUID();
    SendMessageToSet(&data, true);
    return true;
}
...
bool Creature::SetFeatherFall(bool enable, bool packetOnly /* = false */)
{
    ...
    if (m_movedByPlayer) {
        WorldPacket data(
            enable ? SMSG_MOVE_FEATHER_FALL : SMSG_MOVE_NORMAL_FALL, 12);
        data << GetPackGUID();
        data << uint32(0); //! movement counter
        m_movedByPlayer->ToPlayer()->SendDirectMessage(&data);
    ...
    WorldPacket data(enable ? SMSG_SPLINE_MOVE_FEATHER_FALL
                        ...
                        bool updateAnimationTier /*= true*/)
{
    ...
    WorldPacket data(
        enable ? SMSG_SPLINE_MOVE_SET_HOVER : SMSG_SPLINE_MOVE_UNSET_HOVER, 9);
    data << GetPackGUID();
    SendMessageToSet(&data, false);
    return true;
}
...
float Creature::GetAggroRange(Unit const* target) const
{
    ...
    // Aggro Radius varies with level difference at a rate of roughly 1
    // yard/level
    ...
    // hunter pets seem to ignore minimum aggro radius so we'll default it a
    // little higher
...
void Creature::UpdateMovementFlags()
{
    ...
    // Creatures with CREATURE_FLAG_EXTRA_NO_MOVE_FLAGS_UPDATE should control
    // MovementFlags in your own scripts
    ...
    bool isInAir =
        (G3D::fuzzyGt(
             GetPositionZ(),
             ground +
                 (canHover ? GetFloatValue(UNIT_FIELD_HOVERHEIGHT) : 0.0f) +
                 GROUND_HEIGHT_TOLERANCE) ||
         G3D::fuzzyLt(GetPositionZ(),
                      ground -
                          GROUND_HEIGHT_TOLERANCE)); // Can be underground too,
                                                     // prevent the falling
    ...
    case LIQUID_MAP_IN_WATER:
        Swim = GetPositionZ() - liquidData.DepthLevel >
               GetCollisionHeight() *
                   0.75f; // Shallow water at ~75% of collision height
        break;
...
void Creature::SetObjectScale(float scale)
{
    ...
    if (CreatureModelInfo const* minfo =
            sObjectMgr->GetCreatureModelInfo(GetDisplayId())) {
        SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS,
                      (IsPet() ? 1.0f : minfo->bounding_radius) * scale);
...
void Creature::SetDisplayId(uint32 modelId)
{
    ...
    if (CreatureModelInfo const* minfo =
        ...
        SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS,
                      (IsPet() ? 1.0f : minfo->bounding_radius) *
                          GetObjectScale());
        if (minfo->combat_reach > 0)
            combatReach = minfo->combat_reach;
    }
...
void Creature::FocusTarget(Spell const* focusSpell, WorldObject const* target)
{
    ...
    SetGuidValue(UNIT_FIELD_TARGET,
                 this == target ? ObjectGuid::Empty : target->GetGUID());
    if (focusSpell->GetSpellInfo()->HasAttribute(
...
bool Creature::HasSpellFocus(Spell const* focusSpell) const
{
    ...
    return focusSpell ? (focusSpell == _spellFocusInfo.Spell)
                      : (_spellFocusInfo.Spell || _spellFocusInfo.Delay);
}
...
float Creature::GetAttackDistance(Unit const* player) const
{
    ...
    int32 levelDiff =
        static_cast<int32>(playerLevel) - static_cast<int32>(creatureLevel);

    // "The maximum Aggro Radius has a cap of 25 levels under. Example: A level
    // 30 char has the same Aggro Radius of a level 5 char on a level 60 mob."
    if (levelDiff < -25)
        levelDiff = -25;

    // "The aggro radius of a mob having the same level as the player is roughly
    // 20 yards"
    float retDistance = 20.0f;

    // "Aggro Radius varies with level difference at a rate of roughly 1
    // yard/level" radius grow if playlevel < creaturelevel
    retDistance -= static_cast<float>(levelDiff);

    if (creatureLevel + 5 <= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
        // detect range auras
        retDistance += static_cast<float>(
            GetTotalAuraModifier(SPELL_AURA_MOD_DETECT_RANGE));

        // detected range auras
        retDistance += static_cast<float>(
            player->GetTotalAuraModifier(SPELL_AURA_MOD_DETECTED_RANGE));
    }
...
bool Creature::IsMovementPreventedByCasting() const
{
    ...
    // first check if currently a movement allowed channel is active and we're
    // not casting
    if (spell && spell->getState() != SPELL_STATE_FINISHED &&
        spell->IsChannelActive() &&
        spell->GetSpellInfo()->IsActionAllowedChannel()) {
        return false;
    }
...
bool Creature::CanNotReachTarget() const { return m_cannotReachTarget; }

bool Creature::IsNotReachableAndNeedRegen() const
{
    if (CanNotReachTarget()) {
        return m_cannotReachTimer >=
               (sWorld->getIntConfig(
...
{
    lootPickPocketRestoreTime = GameTime::GetGameTime().count() + MINUTE +
                                GetCorpseDelay() + GetRespawnTime();
}

bool Creature::CanGeneratePickPocketLoot() const
{
    return (lootPickPocketRestoreTime == 0 ||
            lootPickPocketRestoreTime < GameTime::GetGameTime().count());
}
                                       ...
                                       Milliseconds duration)
{
    if (victim) {
        ...
        TemporaryThreatModifierEvent* pEvent = new TemporaryThreatModifierEvent(
            *this, victim->GetGUID(), currentThreat);
...
{
    return (m_creatureInfo->flags_extra &
            CREATURE_FLAG_EXTRA_NO_PLAYER_DAMAGE_REQ) ||
           (_playerDamageReq == 0 && _damagedByPlayer);
}
...
{
    if (_playerDamageReq)
        _playerDamageReq > unDamage ? _playerDamageReq -= unDamage
                                    : _playerDamageReq = 0;
...
uint32 Creature::GetPlayerDamageReq() const { return _playerDamageReq; }
...
bool Creature::CanCastSpell(uint32 spellID) const
{
    ...
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SILENCED) ||
        IsSpellProhibited(spellInfo->GetSchoolMask())) {
        return false;
    }

    if (spellInfo && (currentPower < spellInfo->CalcPowerCost(
                                         this, spellInfo->GetSchoolMask()))) {
        return false;
    }
...
ObjectGuid Creature::GetSummonerGUID() const
{
    ...
    LOG_DEBUG("entities.unit",
              "Creature::GetSummonerGUID() called by creature that is not a "
              "summon. Creature: {} ({})",
              GetEntry(),
              GetName());
