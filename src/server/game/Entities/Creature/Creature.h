...
#define MAX_VENDOR_ITEMS                                                       \
    150 // Limitation in 3.x.x item count in SMSG_LIST_INVENTORY

class Creature : public Unit,
                 public GridObject<Creature>,
...
public:
    ...
    {
        return GetCreatureTemplate()->flags_extra &
    ...
    {
        return GetMovementTemplate().Ground ==
                   CreatureGroundMovementType::Hover ||
               IsHovering();
    }
    [[nodiscard]] bool IsRooted() const
    {
        return GetMovementTemplate().IsRooted();
    }
    ...
    [[nodiscard]] bool
    IsValidTrainerForPlayer(Player* player, uint32* npcFlags = nullptr) const;
    bool CanCreatureAttack(Unit const* victim,
                           bool        skipDistCheck = false) const;
    void LoadSpellTemplateImmunity();
    bool IsImmunedToSpell(SpellInfo const* spellInfo,
                          Spell const*     spell = nullptr) override;

    [[nodiscard]] bool HasMechanicTemplateImmunity(uint32 mask) const;
    // redefine Unit::IsImmunedToSpell
    bool IsImmunedToSpellEffect(SpellInfo const* spellInfo,
    ...
    struct {
        ...
        uint32 Delay = 0;  // ms until the creature's target should snap back (0
                           // = no snapback scheduled)
        ObjectGuid Target; // the creature's "real" target while casting
        float      Orientation =
            0.0f; // the creature's "real" orientation while casting
    } _spellFocusInfo;
    ...
    void                 _AddCreatureSpellCooldown(uint32 spell_id,
                                                   uint16 categoryId,
                                                   uint32 end_time);
    ...
    {
        return UpdateEntry(entry, nullptr, true, updateAI);
    }
    ...
    uint32 UpdateVendorItemCurrentCount(VendorItem const* vItem,
                                        uint32            used_count);
    ...
    void setDeathState(DeathState s, bool despawn = false)
        override; // override virtual Unit::setDeathState

    bool
    LoadFromDB(ObjectGuid::LowType guid, Map* map, bool allowDuplicate = false)
    {
        return LoadCreatureFromDB(guid, map, false, allowDuplicate);
    }
    bool LoadCreatureFromDB(ObjectGuid::LowType guid,
                            Map*                map,
                            bool                addToMap       = true,
    ...
    Loot                     loot;
    [[nodiscard]] ObjectGuid GetLootRecipientGUID() const
    {
        return m_lootRecipient;
    }
    [[nodiscard]] Player*             GetLootRecipient() const;
    [[nodiscard]] ObjectGuid::LowType GetLootRecipientGroupGUID() const
    {
        return m_lootRecipientGroup;
    }
    ...
    bool isTappedBy(
        Player const* player) const; // return true if the creature is tapped by
                                     // the player or a member of his party.
    ...
    [[nodiscard]] Unit* SelectNearestTarget(float dist       = 0,
    ...
    bool CanAssistTo(Unit const* u,
                     Unit const* enemy,
                     bool        checkfaction = true) const;
    bool _IsTargetAcceptable(Unit const* target) const;
    [[nodiscard]] bool CanIgnoreFeignDeath() const
    {
        return (GetCreatureTemplate()->flags_extra &
                CREATURE_FLAG_EXTRA_IGNORE_FEIGN_DEATH) != 0;
    }

    // pussywizard: updated at faction change, disable move in line of sight if
    // actual faction is not hostile to anyone
    ...
    {
        DespawnOrUnsummon(Milliseconds(msTimeToDespawn), 0s);
    };
    ...
    void   SetCombatPulseDelay(
          uint32 delay) // (secs) interval at which the creature pulses the entire
                        // zone into combat (only works in dungeons)
    {
    ...
    void
    GetTransportHomePosition(float& x, float& y, float& z, float& ori) const
    {
    ...
    [[nodiscard]] CreatureGroup const* GetFormation() const
    {
        return m_formation;
    }
    ...
    void
    ModifyThreatPercentTemp(Unit* victim, int32 percent, Milliseconds duration);
    ...
    // Used to control if MoveChase() is to be used or not in AttackStart().
    // Some creatures does not chase victims NOTE: If you use SetCombatMovement
    // while the creature is in combat, it will do NOTHING - This only affects
    // AttackStart
    //       You should make the necessary to make it happen so.
    //       Remember that if you modified _isCombatMovementAllowed (e.g: using
    //       SetCombatMovement) it will not be reset at Reset(). It will keep
...
protected:
    bool CreateFromProto(ObjectGuid::LowType guidlow,
                         uint32              Entry,
                         uint32              vehId,
                         const CreatureData* data = nullptr);
    ...
    uint32 m_boundaryCheckTime; // (msecs) remaining time for next evade
                                // boundary check
    uint16 m_transportCheckTimer;
    uint32 lootPickPocketRestoreTime;
    uint32 m_combatPulseTime; // (msecs) remaining time for next zone-in-combat
                              // pulse
    ...
    ObjectGuid::LowType m_spawnId; ///< For new or temporary creatures is 0 for
                                   ///< saved it is lowguid
    uint8 m_equipmentId;
    ...
    CreatureTemplate const*
        m_creatureInfo; // in difficulty mode > 0 can different from
...
private:
    ...
    Spell const*
        _focusSpell; ///> Locks the target during spell cast for proper facing
...
public:
    AssistDelayEvent(ObjectGuid victim, Creature* owner)
        : BasicEvent(), m_victim(victim), m_owner(owner)
    {
    }
...
public:
    ForcedDespawnDelayEvent(Creature& owner, Seconds respawnTimer)
        : BasicEvent(), m_owner(owner), m_respawnTimer(respawnTimer)
    {
    }
    bool Execute(uint64 e_time, uint32 p_time) override;
...
public:
    TemporaryThreatModifierEvent(Creature&  owner,
                                 ObjectGuid threatVictimGUID,
                                 float      threatValue)
        : BasicEvent(), m_owner(owner), m_threatVictimGUID(threatVictimGUID),
          m_threatValue(threatValue)
    {
    }
    bool Execute(uint64 e_time, uint32 p_time) override;
