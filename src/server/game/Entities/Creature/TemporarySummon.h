...
struct TempSummonData {
    ...
    TempSummonType
           type; ///< Summon type, see TempSummonType for available types
    uint32 time; ///< Despawn time, usable only with certain temp summon types
};

class TempSummon : public Creature {
public:
    explicit TempSummon(SummonPropertiesEntry const* properties,
                        ObjectGuid                   owner,
                        bool                         isWorldObject);
    ...
    {
        _visibleBySummonerOnly = visibleBySummonerOnly;
    }
    [[nodiscard]] bool IsVisibleBySummonerOnly() const
    {
        return _visibleBySummonerOnly;
    }
...
public:
    Minion(SummonPropertiesEntry const* properties,
           ObjectGuid                   owner,
           bool                         isWorldObject);
    void                InitStats(uint32 duration) override;
    void                RemoveFromWorld() override;
    [[nodiscard]] Unit* GetOwner() const;
    [[nodiscard]] float GetFollowAngle() const override
    {
        return m_followAngle;
    }
    void               SetFollowAngle(float angle) { m_followAngle = angle; }
    [[nodiscard]] bool IsPetGhoul() const
    {
        return GetEntry() == 26125 /*normal ghoul*/ ||
               GetEntry() == 30230 /*Raise Ally ghoul*/;
...
public:
    Guardian(SummonPropertiesEntry const* properties,
             ObjectGuid                   owner,
             bool                         isWorldObject);
    void InitStats(uint32 duration) override;
