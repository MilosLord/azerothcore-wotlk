...
/// @todo: Implement missing flags from TC in places that custom flags from
/// xinef&pussywizzard use flag values.
// EnumUtils: DESCRIBE THIS
enum CreatureFlagsExtra : uint32 {
    CREATURE_FLAG_EXTRA_INSTANCE_BIND =
        0x00000001, // creature kill bind instance with killer and killer's
                    // group
    ...
    CREATURE_FLAG_EXTRA_NO_TAUNT =
        0x00000100, // creature is immune to taunt auras and 'attack me' effects
    CREATURE_FLAG_EXTRA_NO_MOVE_FLAGS_UPDATE =
        0x00000200, // creature won't update movement flags
    CREATURE_FLAG_EXTRA_GHOST_VISIBILITY =
        0x00000400, // creature will only be visible to dead players
    CREATURE_FLAG_EXTRA_UNUSED_12 =
        0x00000800, /// @todo: Implement CREATURE_FLAG_EXTRA_USE_OFFHAND_ATTACK
    ...
    CREATURE_FLAG_EXTRA_WORLDEVENT =
        0x00004000, // custom flag for world event creatures (left room for
                    // merging)
    CREATURE_FLAG_EXTRA_GUARD = 0x00008000, // Creature is guard
    CREATURE_FLAG_EXTRA_IGNORE_FEIGN_DEATH =
        0x00010000, // creature ignores feign death
    CREATURE_FLAG_EXTRA_NO_CRIT =
        0x00020000, // creature can't do critical strikes
    CREATURE_FLAG_EXTRA_NO_SKILL_GAINS =
        0x00040000, // creature won't increase weapon skills
    CREATURE_FLAG_EXTRA_OBEYS_TAUNT_DIMINISHING_RETURNS =
        0x00080000, // Taunt is subject to diminishing returns on this creature
    CREATURE_FLAG_EXTRA_ALL_DIMINISH =
        0x00100000, // creature is subject to all diminishing returns as players
                    // are
    CREATURE_FLAG_EXTRA_NO_PLAYER_DAMAGE_REQ =
        0x00200000, // creature does not need to take player damage for kill
                    // credit
    CREATURE_FLAG_EXTRA_AVOID_AOE =
        0x00400000, // pussywizard: ignored by aoe attacks (for icc blood prince
    ...
    CREATURE_FLAG_EXTRA_DONT_CALL_ASSISTANCE =
        0x02000000, // Prevent creatures from calling for assistance on initial
                    // aggro
    CREATURE_FLAG_EXTRA_IGNORE_ALL_ASSISTANCE_CALLS =
        0x04000000, // Prevents creature from responding to assistance calls
    CREATURE_FLAG_DONT_OVERRIDE_ENTRY_SAI =
        0x08000000, // Load both ENTRY and GUID specific SAI
    CREATURE_FLAG_EXTRA_DUNGEON_BOSS =
        0x10000000, // creature is a dungeon boss (SET DYNAMICALLY, DO NOT ADD
                    // IN DB)
    CREATURE_FLAG_EXTRA_IGNORE_PATHFINDING =
        0x20000000, // creature ignore pathfinding
    CREATURE_FLAG_EXTRA_IMMUNITY_KNOCKBACK =
        0x40000000, // creature is immune to knockback effects
    ...
    CREATURE_FLAG_EXTRA_DB_ALLOWED =
        (0xFFFFFFFF & ~(CREATURE_FLAG_EXTRA_UNUSED |
                        CREATURE_FLAG_EXTRA_DUNGEON_BOSS)) // SKIP
};
...
struct CreatureTemplate {
    ...
    [[nodiscard]] bool IsTameable(bool exotic) const
    {
        if (type != CREATURE_TYPE_BEAST || family == 0 ||
            (type_flags & CREATURE_TYPE_FLAG_TAMEABLE) == 0)
            return false;
    ...
    [[nodiscard]] bool HasFlagsExtra(uint32 flag) const
    {
        return (flags_extra & flag) != 0;
    }
...
// Defines base stats for creatures (used to calculate
// HP/mana/armor/attackpower/rangedattackpower/all damage).
...
typedef std::unordered_map<uint16, CreatureBaseStats>
...
typedef std::unordered_map<uint32, EquipmentInfoContainerInternal>
...
enum InhabitTypeValues {
    ...
    INHABIT_ROOT   = 8,
    INHABIT_ANYWHERE =
        INHABIT_GROUND | INHABIT_WATER | INHABIT_AIR | INHABIT_ROOT
};
...
struct VendorItem {
    ...
    {
        return pProto->Flags2 & ITEM_FLAGS_EXTRA_EXT_COST_REQUIRES_GOLD ||
               !ExtendedCost;
    }
};
...
struct VendorItemData {
    ...
    [[nodiscard]] VendorItem const* FindItemCostPair(uint32 item_id,
                                                     uint32 extendedCost) const;
    void                            Clear()
    {
        for (VendorItemList::const_iterator itr = m_items.begin();
             itr != m_items.end();
             ++itr)
            delete (*itr);
        m_items.clear();
...
struct TrainerSpellData {
    ...
    uint32 trainerType{0}; // trainer type based at trainer spells, can be
...
struct CreatureSpellCooldown {
    ...
    CreatureSpellCooldown(uint16 categoryId, uint32 endTime)
        : category(categoryId), end(endTime)
    {
    }
