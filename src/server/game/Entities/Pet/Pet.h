...
public:
    ...
    bool IsPermanentPetFor(Player* owner)
        const; // pet have tab in character windows and set UNIT_FIELD_PETNUMBER
    ...
    bool CreateBaseAtTamed(CreatureTemplate const* cinfo,
                           Map*                    map,
                           uint32                  phaseMask);
    static std::pair<PetStable::PetInfo const*, PetSaveMode>
                GetLoadPetInfo(PetStable const& stable,
                               uint32           petEntry,
                               uint32           petnumber,
                               bool             current);
    bool        LoadPetFromDB(Player* owner,
                              uint32  petEntry,
                              uint32  petnumber,
                              bool    current,
                              uint32  healthPct = 0);
    ...
    void setDeathState(DeathState s, bool despawn = false)
        override; // overwrite virtual Creature::setDeathState and
                  // Unit::setDeathState
    void Update(uint32 diff)
        override; // overwrite virtual Creature::Update and Unit::Update
    ...
    void CastWhenWillAvailable(uint32     spellid,
                               Unit*      spellTarget,
    ...
    bool        resetTalents();
    static void resetTalentsForAllPetsOf(Player* owner,
    ...
    {
        SetByteValue(
            UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_PET_TALENTS, points);
    }
    ...
    {
        return m_declinedname.get();
    }

    bool m_removed; // prevent overwrite pet state in DB at next Pet::Update if
                    // pet already removed(saved)
...
protected:
    ...
    PetType      m_petType;
    Milliseconds m_duration; // time until unsummon (used mostly for summoned
                             // guardians and not used for controlled pets)
...
private:
    void SaveToDB(uint32, uint8, uint32)
        override // override of Creature::SaveToDB     - must not be called
    {
        ABORT();
    }
    void DeleteFromDB()
        override // override of Creature::DeleteFromDB - must not be called
    {
        ABORT();
    }
};
