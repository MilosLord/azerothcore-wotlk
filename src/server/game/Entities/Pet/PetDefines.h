...
enum PetSaveMode : int8 {
    ...
    PET_SAVE_LAST_STABLE_SLOT =
        MAX_PET_STABLES, // last in DB stable slot index (including), all higher
                         // have same meaning as PET_SAVE_NOT_IN_SLOT
...
public:
    ...
    std::array<Optional<PetInfo>, MAX_PET_STABLES>
        StabledPets; // PET_SAVE_FIRST_STABLE_SLOT - PET_SAVE_LAST_STABLE_SLOT
    ...
    {
        return UnslottedPets.size() == 1 && UnslottedPets[0].Type == HUNTER_PET
                   ? &UnslottedPets[0]
                   : nullptr;
    }
};
