...
AccountOpResult CreateAccount(std::string username, std::string password)
{
    ...
    auto [salt, verifier] =
        Acore::Crypto::SRP6::MakeRegistrationData(username, password);
    stmt->SetData(1, salt);
    ...
    if (result) {
        do {
            ...
            if (Player* p = ObjectAccessor::FindPlayer(guid)) {
                ...
                s->KickPlayer("Delete account"); // mark session to remove at
                                                 // next session list update
                s->LogoutPlayer(false); // logout player without waiting next
                                        // session list update
            }

            Player::DeleteFromDB(guid.GetCounter(),
                                 accountId,
                                 false,
                                 true); // no need to update realm characters
...
AccountOpResult ChangeUsername(uint32      accountId,
                               ...
                               std::string newPassword)
{
    ...
    auto [salt, verifier] =
        Acore::Crypto::SRP6::MakeRegistrationData(newUsername, newPassword);
...
AccountOpResult ChangePassword(uint32 accountId, std::string newPassword)
{
    ...
    auto [salt, verifier] =
        Acore::Crypto::SRP6::MakeRegistrationData(username, newPassword);

    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_UPD_LOGON);
    stmt->SetData(0, salt);
    stmt->SetData(1, verifier);
    stmt->SetData(2, accountId);
    LoginDatabase.Execute(stmt);
...
{
    LoginDatabasePreparedStatement* stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_GET_ACCOUNT_ACCESS_GMLEVEL);
    stmt->SetData(0, accountId);
    PreparedQueryResult result = LoginDatabase.Query(stmt);
...
bool IsPlayerAccount(uint32 gmlevel) { return gmlevel == SEC_PLAYER; }
...
bool IsConsoleAccount(uint32 gmlevel) { return gmlevel == SEC_CONSOLE; }
