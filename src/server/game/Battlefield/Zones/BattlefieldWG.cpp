...
/// @todo: Use spell victory/defeat in wg instead of RewardMarkOfHonor() &&
/// RewardHonor
...
{
    for (Workshop::const_iterator itr = WorkshopsList.begin();
         itr != WorkshopsList.end();
         ++itr)
        delete *itr;

    for (GameObjectBuilding::const_iterator itr = BuildingsInZone.begin();
         itr != BuildingsInZone.end();
         ++itr)
        delete *itr;
}
...
bool BattlefieldWG::SetupBattlefield()
{
    ...
    // init stalker AFTER setting map id... we spawn it at map=random memory
    // value?...
    InitStalker(BATTLEFIELD_WG_NPC_STALKER,
                WintergraspStalkerPos[0],
                WintergraspStalkerPos[1],
                WintergraspStalkerPos[2],
                WintergraspStalkerPos[3]);
    ...
    m_BattleTime = sWorld->getIntConfig(CONFIG_WINTERGRASP_BATTLETIME) *
                   MINUTE * IN_MILLISECONDS;
    m_NoWarBattleTime = sWorld->getIntConfig(CONFIG_WINTERGRASP_NOBATTLETIME) *
                        MINUTE * IN_MILLISECONDS;
    m_RestartAfterCrash =
        sWorld->getIntConfig(CONFIG_WINTERGRASP_RESTART_AFTER_CRASH) * MINUTE *
        IN_MILLISECONDS;
    ...
    if (!sWorld->getWorldState(BATTLEFIELD_WG_WORLD_STATE_ACTIVE) &&
        ...
        sWorld->setWorldState(BATTLEFIELD_WG_WORLD_STATE_DEFENDER,
                              uint64(urand(0, 1)));
    ...
    m_DefenderTeam =
        TeamId(sWorld->getWorldState(BATTLEFIELD_WG_WORLD_STATE_DEFENDER));
    ...
    for (uint8 i = 0; i < BATTLEFIELD_WG_GRAVEYARD_MAX; i++) {
        ...
        if (WGGraveYard[i].startcontrol == TEAM_NEUTRAL)
            graveyard->Initialize(
                WGGraveYard[i].gyid == BATTLEFIELD_WG_GY_WORKSHOP_SE ||
                        WGGraveYard[i].gyid == BATTLEFIELD_WG_GY_WORKSHOP_SW
                    ? GetAttackerTeam()
                    : m_DefenderTeam,
                WGGraveYard[i].gyid);
        else
            graveyard->Initialize(WGGraveYard[i].startcontrol,
                                  WGGraveYard[i].gyid);
    ...
    for (uint8 i = 0; i < WG_MAX_KEEP_NPC; i++) {
        // Horde npc
        if (Creature* creature = SpawnCreature(WGKeepNPC[i].entryHorde,
                                               WGKeepNPC[i].x,
                                               WGKeepNPC[i].y,
                                               WGKeepNPC[i].z,
                                               WGKeepNPC[i].o,
        ...
        // Alliance npc
        if (Creature* creature = SpawnCreature(WGKeepNPC[i].entryAlliance,
                                               WGKeepNPC[i].x,
                                               WGKeepNPC[i].y,
                                               WGKeepNPC[i].z,
                                               WGKeepNPC[i].o,
    ...
    for (GuidUnorderedSet::const_iterator itr =
             KeepCreature[GetAttackerTeam()].begin();
         itr != KeepCreature[GetAttackerTeam()].end();
         ++itr)
    ...
    for (uint8 i = 0; i < WG_OUTSIDE_ALLIANCE_NPC; i++)
        if (Creature* creature = SpawnCreature(WGOutsideNPC[i].entryHorde,
                                               WGOutsideNPC[i].x,
                                               WGOutsideNPC[i].y,
                                               WGOutsideNPC[i].z,
                                               WGOutsideNPC[i].o,
    ...
    for (uint8 i = WG_OUTSIDE_ALLIANCE_NPC; i < WG_MAX_OUTSIDE_NPC; i++)
        if (Creature* creature = SpawnCreature(WGOutsideNPC[i].entryAlliance,
                                               WGOutsideNPC[i].x,
                                               WGOutsideNPC[i].y,
                                               WGOutsideNPC[i].z,
                                               WGOutsideNPC[i].o,
    ...
    for (GuidUnorderedSet::const_iterator itr =
             OutsideCreature[GetDefenderTeam()].begin();
         itr != OutsideCreature[GetDefenderTeam()].end();
         ++itr)
        if (Creature* creature = GetCreature(*itr))
            HideNpc(creature);
    ...
    for (uint8 i = 0; i < WG_MAX_TURRET; i++) {
        ...
        if (Creature* creature = SpawnCreature(
                NPC_WINTERGRASP_TOWER_CANNON, towerCannonPos, TEAM_ALLIANCE)) {
    ...
    for (uint8 i = 0; i < WG_MAX_OBJ; i++) {
        GameObject* go = SpawnGameObject(WGGameObjectBuilding[i].entry,
                                         WGGameObjectBuilding[i].x,
                                         WGGameObjectBuilding[i].y,
                                         WGGameObjectBuilding[i].z,
                                         WGGameObjectBuilding[i].o);
        BfWGGameObjectBuilding* b = new BfWGGameObjectBuilding(this);
        b->Init(go,
                WGGameObjectBuilding[i].type,
                WGGameObjectBuilding[i].WorldState,
                WGGameObjectBuilding[i].damageText,
                WGGameObjectBuilding[i].destroyText);
        BuildingsInZone.insert(b);
    }

    // Spawning portal defender
    for (uint8 i = 0; i < WG_MAX_TELEPORTER; i++) {
        GameObject* go = SpawnGameObject(WGPortalDefenderData[i].entry,
                                         WGPortalDefenderData[i].x,
                                         WGPortalDefenderData[i].y,
                                         WGPortalDefenderData[i].z,
                                         WGPortalDefenderData[i].o);
        DefenderPortalList.insert(go);
        go->SetUInt32Value(GAMEOBJECT_FACTION,
                           WintergraspFaction[GetDefenderTeam()]);
    }
...
void BattlefieldWG::OnBattleStart()
{
    ...
    GameObject* go = SpawnGameObject(
        GO_WINTERGRASP_TITAN_S_RELIC, 5440.0f, 2840.8f, 430.43f, 0);
    if (go) {
        // Update faction of relic, only attacker can click on
        go->SetUInt32Value(GAMEOBJECT_FACTION,
                           WintergraspFaction[GetAttackerTeam()]);
    ...
    for (GuidUnorderedSet::const_iterator itr = CanonList.begin();
         itr != CanonList.end();
         ++itr) {
        if (Creature* creature = GetCreature(*itr)) {
    ...
    // Update graveyard (in no war time all graveyard is to deffender, in war
    // time, depend of base)
    for (Workshop::const_iterator itr = WorkshopsList.begin();
         itr != WorkshopsList.end();
         ++itr)
    ...
    for (BfCapturePointVector::const_iterator itr = m_capturePoints.begin();
         itr != m_capturePoints.end();
         ++itr)
        (*itr)->SetCapturePointData(
            (*itr)->GetCapturePointGo(),
            (*itr)->GetCapturePointGo()->GetEntry() ==
    ...
    for (uint8 team = 0; team < 2; ++team)
        for (GuidUnorderedSet::const_iterator itr = m_players[team].begin();
             itr != m_players[team].end();
             ++itr) {
            ...
            if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
                ...
                if (5500 > x && x > 5392 && y < 2880 && y > 2800 && z < 480)
                    player->TeleportTo(
                        571, 5349.8686f, 2838.481f, 409.240f, 0.046328f);
...
void BattlefieldWG::UpdateVehicleCountWG()
{
    for (uint8 i = 0; i < PVP_TEAMS_COUNT; ++i)
        for (GuidUnorderedSet::iterator itr = m_players[i].begin();
             itr != m_players[i].end();
             ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
                ...
                player->SendUpdateWorldState(
                    ...
                    GetData(BATTLEFIELD_WG_DATA_MAX_VEHICLE_H));
                player->SendUpdateWorldState(
                    BATTLEFIELD_WG_WORLD_STATE_VEHICLE_A,
                    GetData(BATTLEFIELD_WG_DATA_VEHICLE_A));
                player->SendUpdateWorldState(
...
void BattlefieldWG::OnBattleEnd(bool endByTimer)
{
    ...
    for (GuidUnorderedSet::const_iterator itr =
             KeepCreature[GetDefenderTeam()].begin();
         itr != KeepCreature[GetDefenderTeam()].end();
         ++itr)
        if (Creature* creature = GetCreature(*itr))
            ShowNpc(creature, true);
    ...
    for (GuidUnorderedSet::const_iterator itr =
             OutsideCreature[GetAttackerTeam()].begin();
         itr != OutsideCreature[GetAttackerTeam()].end();
         ++itr)
        if (Creature* creature = GetCreature(*itr))
            ShowNpc(creature, true);
    ...
    for (uint8 i = 0; i < BATTLEFIELD_WG_GY_HORDE; i++)
        if (BfGraveyard* graveyard = GetGraveyardById(i))
            graveyard->GiveControlTo(i == BATTLEFIELD_WG_GY_WORKSHOP_SE ||
                                             i == BATTLEFIELD_WG_GY_WORKSHOP_SW
                                         ? GetAttackerTeam()
                                         : GetDefenderTeam());

    for (GameObjectSet::const_iterator itr =
             m_KeepGameObject[GetDefenderTeam()].begin();
         itr != m_KeepGameObject[GetDefenderTeam()].end();
         ++itr)
        (*itr)->SetRespawnTime(RESPAWN_IMMEDIATELY);

    for (GameObjectSet::const_iterator itr =
             m_KeepGameObject[GetAttackerTeam()].begin();
         itr != m_KeepGameObject[GetAttackerTeam()].end();
         ++itr)
        (*itr)->SetRespawnTime(RESPAWN_ONE_DAY);

    // Update portal defender faction
    for (GameObjectSet::const_iterator itr = DefenderPortalList.begin();
         itr != DefenderPortalList.end();
         ++itr)
        (*itr)->SetUInt32Value(GAMEOBJECT_FACTION,
                               WintergraspFaction[GetDefenderTeam()]);
    ...
    for (Workshop::const_iterator itr = WorkshopsList.begin();
         ...
         ++itr) {
        (*itr)->GiveControlTo(
            (*itr)->workshopId == BATTLEFIELD_WG_WORKSHOP_SE ||
                    (*itr)->workshopId == BATTLEFIELD_WG_WORKSHOP_SW
                ? GetAttackerTeam()
                : GetDefenderTeam(),
            true);
        (*itr)->Save();
    ...
    for (uint8 team = 0; team < 2; ++team) {
        for (GuidUnorderedSet::const_iterator itr = m_vehicles[team].begin();
             itr != m_vehicles[team].end();
             ++itr)
            if (Creature* creature = GetCreature(*itr))
    ...
    for (GuidUnorderedSet::const_iterator itr =
             m_PlayersInWar[GetDefenderTeam()].begin();
         itr != m_PlayersInWar[GetDefenderTeam()].end();
         ++itr) {
        if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
            // Victory in Wintergrasp
            player->AreaExploredOrEventHappens(
                GetDefenderTeam() ? 13183
                                  : 13181); // HORDE / ALLY win wg quest id
    ...
    for (GuidUnorderedSet::const_iterator itr =
             m_PlayersInWar[GetAttackerTeam()].begin();
         itr != m_PlayersInWar[GetAttackerTeam()].end();
         ++itr)
        if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
            ...
            for (GuidUnorderedSet::const_iterator itr = m_players[team].begin();
                ...
                if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
                    player->RemoveAurasDueToSpell(
                        ...
                        player->GetGUID());
                    player->AddAura(m_DefenderTeam == TEAM_HORDE
                                        ? SPELL_HORDE_CONTROL_PHASE_SHIFT
                                        : SPELL_ALLIANCE_CONTROL_PHASE_SHIFT,
                                    player);
                }
    ...
    {
        uint32 const worldStateId = GetDefenderTeam() == TEAM_ALLIANCE
                                        ? WORLDSTATE_ALLIANCE_KEEP_CAPTURED
                                        : WORLDSTATE_HORDE_KEEP_CAPTURED;
        sWorld->setWorldState(worldStateId,
                              sWorld->getWorldState(worldStateId) + 1);

        SendWarning((GetDefenderTeam() == TEAM_ALLIANCE)
                        ? BATTLEFIELD_WG_TEXT_WIN_KEEP
                        : (BATTLEFIELD_WG_TEXT_WIN_KEEP + 2));
    }
    else // defend alli/horde
    {
        ...
        SendWarning((GetDefenderTeam() == TEAM_ALLIANCE)
                        ...
                        : (BATTLEFIELD_WG_TEXT_DEFEND_KEEP + 2));
    }
}
...
uint8 BattlefieldWG::GetSpiritGraveyardId(uint32 areaId) const
{
    ...
    default:
        LOG_ERROR("bg.battlefield",
                  "BattlefieldWG::GetSpiritGraveyardId: Unexpected Area Id {}",
                  areaId);
        break;
    }
...
void BattlefieldWG::OnCreatureCreate(Creature* creature)
{
    ...
    case NPC_TAUNKA_SPIRIT_GUIDE: {
        TeamId teamId =
            (creature->GetEntry() == NPC_DWARVEN_SPIRIT_GUIDE ? TEAM_ALLIANCE
                                                              : TEAM_HORDE);
    ...
    if (IsWarTime()) {
        ...
        case NPC_WINTERGRASP_DEMOLISHER: {
            if (!creature->IsSummon() ||
                !creature->ToTempSummon()->GetSummonerGUID())
                return;

            Player* creator = ObjectAccessor::FindPlayer(
                creature->ToTempSummon()->GetSummonerGUID());
            if (!creator)
                return;
            ...
            if (team == TEAM_HORDE) {
                ...
                if (GetData(BATTLEFIELD_WG_DATA_VEHICLE_A) <
                    GetData(BATTLEFIELD_WG_DATA_MAX_VEHICLE_A)) {
...
void BattlefieldWG::OnCreatureRemove(Creature* /*creature*/)
{
    /* possibly can be used later
       ...
       WintergraspFaction[TEAM_ALLIANCE]) team = TEAM_ALLIANCE; else if
       (creature->GetFaction() == WintergraspFaction[TEAM_HORDE]) team =
       TEAM_HORDE; else return;
    ...
    for (Workshop::const_iterator itr = WorkshopsList.begin();
        ...
        if (WGWorkshop* workshop = (*itr)) {
            if (workshop->workshopId == workshopId) {
                WintergraspCapturePoint* capturePoint =
                    new WintergraspCapturePoint(this, workshop->teamControl);
                // Sending neutral team at start to set normal capture points by
                // workshop->teamControl, TEAM_NEUTRAL is ignored at first call
...
void BattlefieldWG::HandleKill(Player* killer, Unit* victim)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER || IsKeepNpc(victim->GetEntry()) ||
        victim->GetEntry() == NPC_WINTERGRASP_TOWER_CANNON) {
        if (victim->GetTypeId() == TYPEID_PLAYER &&
            victim->HasAura(SPELL_LIEUTENANT)) {
...
void BattlefieldWG::OnPlayerJoinWar(Player* player)
{
    ...
    if (player->GetTeamId() == GetDefenderTeam())
        ...
        if (player->GetTeamId() == TEAM_HORDE)
            player->TeleportTo(
                571, 5025.857422f, 3674.628906f, 362.737122f, 4.135169f);
        else
    ...
    if (player->GetTeamId() == GetAttackerTeam()) {
        if (GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT) < 3)
            player->SetAuraStack(
                SPELL_TOWER_CONTROL,
                player,
                3 - GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT));
    }
    else {
        if (GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT) > 0)
            player->SetAuraStack(SPELL_TOWER_CONTROL,
                                 player,
                                 GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT));
    }
    SendInitWorldStatesTo(player);
}
...
{
    // Remove all aura from WG /// @todo: false we can go out of this zone on
    // retail and keep Rank buff, remove on end of WG
...
void BattlefieldWG::OnPlayerEnterZone(Player* player)
{
    ...
    if (player->GetTeamId() == GetAttackerTeam())
        if (player->GetPositionX() > 5400.0f &&
            player->GetPositionX() < 5490.0f &&
            player->GetPositionY() > 2803.0f &&
            player->GetPositionY() < 2878.0f)
...
uint32 BattlefieldWG::GetData(uint32 data) const
{
    ...
    case AREA_EASTPARK_WORKSHOP:
        ...
        if (BfGraveyard const* graveyard =
                GetGraveyardById(GetSpiritGraveyardId(data)))
...
{
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_ATTACKER)
         << uint32(GetAttackerTeam());
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_DEFENDER)
         << uint32(GetDefenderTeam());
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_ACTIVE)
         << uint32(
                IsWarTime()
                    ? 0
                    : 1); // Note: cleanup these two, their names look awkward
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_SHOW_WORLDSTATE)
         << uint32(IsWarTime() ? 1 : 0);

    for (uint32 i = 0; i < 2; ++i)
        data << ClockWorldState[i]
             << uint32(GameTime::GetGameTime().count() + (m_Timer / 1000));

    data << uint32(BATTLEFIELD_WG_WORLD_STATE_VEHICLE_H)
         << uint32(GetData(BATTLEFIELD_WG_DATA_VEHICLE_H));
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_MAX_VEHICLE_H)
         << GetData(BATTLEFIELD_WG_DATA_MAX_VEHICLE_H);
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_VEHICLE_A)
         << uint32(GetData(BATTLEFIELD_WG_DATA_VEHICLE_A));
    data << uint32(BATTLEFIELD_WG_WORLD_STATE_MAX_VEHICLE_A)
         << GetData(BATTLEFIELD_WG_DATA_MAX_VEHICLE_A);

    data << uint32(WORLDSTATE_HORDE_KEEP_CAPTURED)
         << uint32(sWorld->getWorldState(WORLDSTATE_HORDE_KEEP_CAPTURED));
    data << uint32(WORLDSTATE_HORDE_KEEP_DEFENDED)
         << uint32(sWorld->getWorldState(WORLDSTATE_HORDE_KEEP_DEFENDED));
    data << uint32(WORLDSTATE_ALLIANCE_KEEP_CAPTURED)
         << uint32(sWorld->getWorldState(WORLDSTATE_ALLIANCE_KEEP_CAPTURED));
    data << uint32(WORLDSTATE_ALLIANCE_KEEP_DEFENDED)
         << uint32(sWorld->getWorldState(WORLDSTATE_ALLIANCE_KEEP_DEFENDED));
    ...
    for (Workshop::const_iterator itr = WorkshopsList.begin();
         ...
         ++itr)
        if (*itr)
            data << WorkshopsData[(*itr)->workshopId].worldstate
                 << (*itr)->state;
}
...
void BattlefieldWG::SendInitWorldStatesTo(Player* player)
{
    WorldPacket data(SMSG_INIT_WORLD_STATES,
                     (4 + 4 + 4 + 2 + (BuildingsInZone.size() * 8) +
                      (WorkshopsList.size() * 8)));

    data << uint32(m_MapId);
    data << uint32(m_ZoneId);
    data << uint32(0);
    data << uint16(14 + BuildingsInZone.size() +
                   WorkshopsList.size()); // Number of fields
...
{
    for (uint8 team = 0; team < 2; team++)
        for (GuidUnorderedSet::iterator itr = m_players[team].begin();
             itr != m_players[team].end();
             ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
                SendInitWorldStatesTo(player);
...
{
    // might be some use for this in the future. old code commented out below.
    // KL
    /*    if (team == GetDefenderTeam())
        {
            for (GuidUnorderedSet::const_iterator itr =
       m_PlayersInWar[GetAttackerTeam()].begin(); itr !=
       m_PlayersInWar[GetAttackerTeam()].end(); ++itr)
            {
                if (Player* player = ObjectAccessor::FindPlayer(*itr))
                    IncrementQuest(player, WGQuest[player->GetTeamId()][2],
       true);
            }
        }*/
}
...
void BattlefieldWG::UpdatedDestroyedTowerCount(TeamId team, GameObject* go)
{
    ...
    if (team == GetAttackerTeam()) {
        ...
        // If all three south towers are destroyed (ie. all attack towers),
        // remove ten minutes from battle time
        if (GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT) == 3) {
...
void BattlefieldWG::ProcessEvent(WorldObject* obj, uint32 eventId)
{
    ...
    // if destroy or damage event, search the wall/tower and update
    // worldstate/send warning message
    for (GameObjectBuilding::const_iterator itr = BuildingsInZone.begin();
         itr != BuildingsInZone.end();
         ++itr) {
        if (GameObject* build =
                ObjectAccessor::GetGameObject(*obj, (*itr)->m_Build)) {
...
void BattlefieldWG::UpdateTenacity()
{
    ...
    if (alliancePlayers && hordePlayers) {
        if (alliancePlayers < hordePlayers)
            newStack = int32((((float)hordePlayers / alliancePlayers) - 1.0f) *
                             4.0f); // positive, should cast on alliance
        else if (alliancePlayers > hordePlayers)
            newStack = int32((1.0f - ((float)alliancePlayers / hordePlayers)) *
                             4.0f); // negative, should cast on horde
    }

    // Return if no change in stack and apply tenacity to new player
    if (newStack == m_tenacityStack) {
        for (GuidUnorderedSet::const_iterator itr =
                 m_updateTenacityList.begin();
             itr != m_updateTenacityList.end();
             ++itr)
            if (Player* newPlayer = ObjectAccessor::FindPlayer(*itr))
                if ((newPlayer->GetTeamId() == TEAM_ALLIANCE &&
                     m_tenacityStack > 0) ||
                    (newPlayer->GetTeamId() == TEAM_HORDE &&
                     m_tenacityStack < 0)) {
                    newStack          = std::min(std::abs(newStack), 20);
                    uint32 buff_honor = GetHonorBuff(newStack);
                    newPlayer->SetAuraStack(
                        SPELL_TENACITY, newPlayer, newStack);
                    if (buff_honor)
    ...
    if (team != TEAM_NEUTRAL) {
        for (GuidUnorderedSet::const_iterator itr =
                 m_PlayersInWar[team].begin();
             itr != m_PlayersInWar[team].end();
             ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
                player->RemoveAurasDueToSpell(SPELL_TENACITY);
    ...
    if (newStack) {
        ...
        for (GuidUnorderedSet::const_iterator itr = m_vehicles[team].begin();
             ...
             ++itr)
            if (Creature* creature = GetCreature(*itr)) {
                creature->SetAuraStack(
                    SPELL_TENACITY_VEHICLE, creature, newStack);
                if (buff_honor)
...
BfGraveyardWG::BfGraveyardWG(BattlefieldWG* battlefield)
    : BfGraveyard(battlefield)
{
    m_Bf           = battlefield;
    m_GossipTextId = 0;
}
