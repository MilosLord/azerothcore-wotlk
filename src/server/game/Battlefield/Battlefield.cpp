...
Battlefield::~Battlefield()
{
    for (BfCapturePointVector::iterator itr = m_capturePoints.begin();
         itr != m_capturePoints.end();
         ++itr)
        delete *itr;

    for (GraveyardVect::const_iterator itr = m_GraveyardList.begin();
         itr != m_GraveyardList.end();
         ++itr)
        delete *itr;
...
void Battlefield::HandlePlayerEnterZone(Player* player, uint32 /*zone*/)
{
    ...
    if (!player->IsInFlight()) {
        ...
        // If full of players > announce to player that BF is full and kick him
        // after a few second if he desn't leave
        if (IsWarTime()) {
            if (m_PlayersInWar[player->GetTeamId()].size() +
                    m_InvitedPlayers[player->GetTeamId()].size() <
                m_MaxPlayer) // Vacant spaces
                InvitePlayerToWar(player);
            else // No more vacant places
            {
                /// @todo: Send a packet to announce it to player
                m_PlayersWillBeKick[player->GetTeamId()][player->GetGUID()] =
                    GameTime::GetGameTime().count() +
                    (player->IsGameMaster() ? 30 * MINUTE : 10);
...
void Battlefield::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    if (IsWarTime()) {
        ...
        if (m_PlayersInWar[player->GetTeamId()].find(player->GetGUID()) !=
            m_PlayersInWar[player->GetTeamId()].end()) {
            m_PlayersInWar[player->GetTeamId()].erase(player->GetGUID());
            player->GetSession()->SendBfLeaveMessage(m_BattleId);
            if (Group* group =
                    player->GetGroup()) // Remove the player from the raid group
                if (group->isBFGroup())
        ...
        if (!IsEnabled() ||
            (!IsWarTime() &&
             sWorld->GetActiveSessionCount() >
                 3500)) // if WG is disabled or there is more than 3500
                        // connections, switch automaticly
        {
            m_isActive = true;
            EndBattle(false);
            return false;
    ...
    if (IsWarTime()) {
        ...
        if (m_uiKickDontAcceptTimer <= diff) {
            ...
            for (int team = 0; team < 2; team++)
                for (PlayerTimerMap::iterator itr =
                         m_InvitedPlayers[team].begin();
                     itr != m_InvitedPlayers[team].end();
                     ++itr)
                    if (itr->second <= now)
            ...
            for (int team = 0; team < 2; team++)
                for (PlayerTimerMap::iterator itr =
                         m_PlayersWillBeKick[team].begin();
                     itr != m_PlayersWillBeKick[team].end();
                     ++itr)
                    if (itr->second <= now)
                        KickPlayerFromBattlefield(itr->first);
...
void Battlefield::InvitePlayerToQueue(Player* player)
{
    ...
    if (m_PlayersInQueue[player->GetTeamId()].size() <= m_MinPlayer ||
        m_PlayersInQueue[GetOtherTeam(player->GetTeamId())].size() >=
            m_MinPlayer)
...
void Battlefield::InvitePlayersInQueueToWar()
{
    for (uint8 team = 0; team < PVP_TEAMS_COUNT; ++team) {
        ...
        for (GuidUnorderedSet::const_iterator itr = copy.begin();
             itr != copy.end();
             ++itr) {
            if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
                if (m_PlayersInWar[player->GetTeamId()].size() +
...
void Battlefield::InvitePlayersInZoneToWar()
{
    for (uint8 team = 0; team < PVP_TEAMS_COUNT; ++team)
        for (GuidUnorderedSet::const_iterator itr = m_players[team].begin();
             ...
             ++itr) {
            if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
                if (m_PlayersInWar[player->GetTeamId()].count(
                        player->GetGUID()) ||
                    m_InvitedPlayers[player->GetTeamId()].count(
                        player->GetGUID()))
                    continue;
                if (m_PlayersInWar[player->GetTeamId()].size() +
                        m_InvitedPlayers[player->GetTeamId()].size() <
                    m_MaxPlayer)
                    InvitePlayerToWar(player);
                else if (m_PlayersWillBeKick[player->GetTeamId()].count(
                             player->GetGUID()) ==
                         0) // Battlefield is full of players
                    m_PlayersWillBeKick[player->GetTeamId()]
                                       [player->GetGUID()] =
                                           GameTime::GetGameTime().count() + 10;
            }
        }
}
...
void Battlefield::InvitePlayerToWar(Player* player)
{
    ...
    // If the player does not match minimal level requirements for the
    // battlefield, kick him
    ...
    if (m_PlayersInWar[player->GetTeamId()].count(player->GetGUID()) ||
        m_InvitedPlayers[player->GetTeamId()].count(player->GetGUID()))
        return;

    m_PlayersWillBeKick[player->GetTeamId()].erase(player->GetGUID());
    m_InvitedPlayers[player->GetTeamId()][player->GetGUID()] =
        GameTime::GetGameTime().count() + m_TimeForAcceptInvite;
    player->GetSession()->SendBfInvitePlayerToWar(
        m_BattleId, m_ZoneId, m_TimeForAcceptInvite);
}
...
void Battlefield::InitStalker(uint32 entry, float x, float y, float z, float o)
{
    if (Creature* creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
        ...
        LOG_ERROR("bg.battlefield",
                  "Battlefield::InitStalker: could not spawn Stalker (Creature "
                  "entry {}), zone messeges will be un-available",
                  entry);
}
...
void Battlefield::KickAfkPlayers()
{
    ...
    for (uint8 team = 0; team < PVP_TEAMS_COUNT; ++team)
        for (GuidUnorderedSet::const_iterator itr =
             ...
             ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
                if (player->isAFK() && player->GetZoneId() == GetZoneId() &&
                    !player->IsGameMaster())
...
{
    return m_players[player->GetTeamId()].find(player->GetGUID()) !=
           m_players[player->GetTeamId()].end();
}
...
void Battlefield::BroadcastPacketToQueue(WorldPacket const* data) const
{
    for (uint8 team = 0; team < PVP_TEAMS_COUNT; ++team)
        for (GuidUnorderedSet::const_iterator itr =
                 m_PlayersInQueue[team].begin();
             itr != m_PlayersInQueue[team].end();
             ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
                player->GetSession()->SendPacket(data);
}
...
void Battlefield::HideNpc(Creature* creature)
{
    ...
    creature->SetPhaseMask(
        2, false); // pussywizard: false because UpdateObjectVisibility(true) is
                   // called below in SetVisible(), no need to have it here
...
{
    creature->SetPhaseMask(
        1, false); // pussywizard: false because UpdateObjectVisibility(true) is
                   // called below in SetVisible(), no need to have it here
...
{
    for (GuidUnorderedSet::const_iterator itr = m_Groups[TeamId].begin();
         itr != m_Groups[TeamId].end();
         ++itr)
...
bool Battlefield::AddOrSetPlayerToCorrectBfGroup(Player* player)
{
    ...
    if (player->GetGroup() &&
        ...
        LOG_INFO("misc",
                 "Battlefield::AddOrSetPlayerToCorrectBfGroup - player is "
                 "already in {} group!",
...
BfGraveyard* Battlefield::GetGraveyardById(uint32 id) const
{
    if (id < m_GraveyardList.size()) {
        if (m_GraveyardList[id])
            ...
            LOG_ERROR("bg.battlefield",
                      "Battlefield::GetGraveyardById Id:{} not existed",
                      id);
    }
    else
        LOG_ERROR("bg.battlefield",
                  "Battlefield::GetGraveyardById Id:{} cant be found",
                  id);
...
void BfGraveyard::Resurrect()
{
    ...
    for (GuidUnorderedSet::const_iterator itr = m_ResurrectQueue.begin();
         itr != m_ResurrectQueue.end();
         ++itr) {
        ...
        if (player->IsInWorld())
            if (Unit* spirit = ObjectAccessor::GetCreature(
                    *player, m_SpiritGuide[m_ControlTeam]))
...
void BfGraveyard::RelocateDeadPlayers()
{
    ...
    for (GuidUnorderedSet::const_iterator itr = m_ResurrectQueue.begin();
        ...
        if (closestGrave)
            player->TeleportTo(player->GetMapId(),
                               closestGrave->x,
                               closestGrave->y,
                               closestGrave->z,
...
Creature* Battlefield::SpawnCreature(
    uint32 entry, float x, float y, float z, float o, TeamId teamId)
{
    // Get map object
    Map* map = sMapMgr->CreateBaseMap(m_MapId);
    if (!map) {
        LOG_ERROR("bg.battlefield",
                  "Battlefield::SpawnCreature: Can't create creature entry: {} "
                  "map not found",
                  entry);
        return nullptr;
    }
    ...
    if (!cinfo) {
        LOG_ERROR("sql.sql",
                  "Battlefield::SpawnCreature: entry {} does not exist.",
                  entry);
        return nullptr;
    }
...
GameObject*
Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
    // Get map object
    Map* map = sMapMgr->CreateBaseMap(m_MapId);
    if (!map)
    ...
    if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(),
                    ...
                    GO_STATE_READY)) {
        LOG_ERROR("sql.sql",
                  "Battlefield::SpawnGameObject: Gameobject template {} not "
                  "found in database! Battlefield not created!",
                  entry);
        LOG_ERROR("bg.battlefield",
                  "Battlefield::SpawnGameObject: Cannot create gameobject "
                  "template {}! Battlefield not created!",
                  entry);
        delete go;
        return nullptr;
    }
...
bool BfCapturePoint::HandlePlayerEnter(Player* player)
{
    if (GameObject* go = GetCapturePointGo(player)) {
        player->SendUpdateWorldState(go->GetGOInfo()->capturePoint.worldState1,
                                     1);
        player->SendUpdateWorldState(
            go->GetGOInfo()->capturePoint.worldstate2,
            uint32(
                std::ceil((m_value + m_maxValue) / (2 * m_maxValue) * 100.0f)));
        player->SendUpdateWorldState(go->GetGOInfo()->capturePoint.worldstate3,
                                     m_neutralValuePct);
    }
    return m_activePlayers[player->GetTeamId()]
...
GuidUnorderedSet::iterator BfCapturePoint::HandlePlayerLeave(Player* player)
{
    ...
    GuidUnorderedSet::iterator current =
        m_activePlayers[player->GetTeamId()].find(player->GetGUID());
...
void BfCapturePoint::SendChangePhase()
{
    ...
    for (uint8 team = 0; team < 2; ++team)
        for (GuidUnorderedSet::iterator itr = m_activePlayers[team].begin();
             itr != m_activePlayers[team].end();
             ++itr) // send to all players present in the area
            if (Player* player = ObjectAccessor::FindPlayer(*itr)) {
                // send this too, sometimes the slider disappears, dunno why :(
                player->SendUpdateWorldState(
                    capturePoint->GetGOInfo()->capturePoint.worldState1, 1);
                // send these updates to only the ones in this objective
                player->SendUpdateWorldState(
                    capturePoint->GetGOInfo()->capturePoint.worldstate2,
                    (uint32)std::ceil((m_value + m_maxValue) /
                                      (2 * m_maxValue) * 100.0f));
                // send this too, sometimes it resets :S
                player->SendUpdateWorldState(
                    capturePoint->GetGOInfo()->capturePoint.worldstate3,
                    m_neutralValuePct);
            }
}
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint, TeamId team)
{
    ...
    // At first call using TEAM_NEUTRAL as a checker but never using it, after
    // first call we reset the capturepoints to the new winner of the last WG
    // war
    if (team == TEAM_NEUTRAL)
        team = m_team;
    LOG_DEBUG("bg.battlefield",
    ...
    if (goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT) {
        LOG_ERROR("bg.battlefield",
                  "OutdoorPvP: GO {} is not capture point!",
                  capturePoint->GetEntry());
        return false;
    }
    ...
    m_maxSpeed =
        m_maxValue /
        (goinfo->capturePoint.minTime ? goinfo->capturePoint.minTime : 60);
...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    for (std::list<Player*>::iterator itr = players.begin();
        ...
        if ((*itr)->IsOutdoorPvPActive())
            if (m_activePlayers[(*itr)->GetTeamId()]
                    .insert((*itr)->GetGUID())
                    .second)
    ...
    float fact_diff =
        ((float)m_activePlayers[0].size() - (float)m_activePlayers[1].size()) *
        diff / BATTLEFIELD_OBJECTIVE_UPDATE_INTERVAL;
    ...
    if (fact_diff < 0) {
        ...
        if (m_State == BF_CAPTUREPOINT_OBJECTIVESTATE_HORDE &&
            m_value <= -m_maxValue)
            return false;
        ...
        if (m_State == BF_CAPTUREPOINT_OBJECTIVESTATE_ALLIANCE &&
            m_value >= m_maxValue)
            return false;
    ...
    {
        // old phase and current are on the same side, so one team challenges
        // the other
        if (ChallengerId == TEAM_ALLIANCE &&
            (m_OldState == BF_CAPTUREPOINT_OBJECTIVESTATE_HORDE ||
             m_OldState ==
                 BF_CAPTUREPOINT_OBJECTIVESTATE_NEUTRAL_HORDE_CHALLENGE))
            m_State = BF_CAPTUREPOINT_OBJECTIVESTATE_HORDE_ALLIANCE_CHALLENGE;
        else if (
            ChallengerId == TEAM_HORDE &&
            (m_OldState == BF_CAPTUREPOINT_OBJECTIVESTATE_ALLIANCE ||
             m_OldState ==
...
{
    return m_activePlayers[player->GetTeamId()].find(player->GetGUID()) !=
           m_activePlayers[player->GetTeamId()].end();
}
