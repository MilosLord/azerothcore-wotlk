                                             ...
                                             bool   Full)
{
    ...
    data << uint32(BattleId);
    data << uint32(ZoneId);
    data << uint8((CanQueue ? 1 : 0)); // Accepted          //0 you cannot queue
                                       // wg     //1 you are queued
    data << uint8((Full ? 0 : 1)); // Logging In        //0 wg full //1 queue
                                   // for upcoming
    data << uint8(1); // Warmup
    SendPacket(&data);
}
...
// Send by client on clicking in accept or refuse of invitation windows for join
// game
