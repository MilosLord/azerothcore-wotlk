...
enum GuildEvents {
    ...
    GE_GUILDBANKBAGSLOTS_CHANGED =
        14, /// @todo: Sent when items are moved in gbank - all players with
            /// bank open will send tab query
...
public:
    EmblemInfo()
        : m_style(0), m_color(0), m_borderStyle(0), m_borderColor(0),
          m_backgroundColor(0)
    {
    }
...
public:
    ...
    GuildBankRightsAndSlots(uint8 _tabId, uint8 _rights, uint32 _slots)
        : tabId(_tabId), rights(_rights), slots(_slots)
    {
    }
...
public: // pussywizard: public class Member
    ...
    public:
        Member(uint32 guildId, ObjectGuid guid, uint8 rankId)
            : m_guildId(guildId), m_guid(guid), m_zoneId(0), m_level(0),
              m_class(0), m_flags(GUILDMEMBER_STATUS_NONE), m_accountId(0),
              m_rankId(rankId), receiveGuildBankUpdatePackets(false)
        {
        }

        void SetStats(Player* player);
        void SetStats(std::string_view name,
                      uint8            level,
                      uint8            _class,
                      uint8            gender,
                      uint32           zoneId,
        ...
        inline bool IsRankNotLower(uint8 rankId) const
        {
            return m_rankId <= rankId;
        }
        inline bool IsSamePlayer(ObjectGuid guid) const
        {
            return m_guid == guid;
        }

        void  UpdateBankWithdrawValue(CharacterDatabaseTransaction trans,
                                      uint8                        tabId,
                                      uint32                       amount);
        int32 GetBankWithdrawValue(uint8 tabId) const;
        void  ResetValues();

        inline Player* FindPlayer() const
        {
        ...
        {
            receiveGuildBankUpdatePackets = true;
        }
        void UnsubscribeFromGuildBankUpdatePackets()
        {
            receiveGuildBankUpdatePackets = false;
        }
        [[nodiscard]] bool ShouldReceiveBankPartialUpdatePackets() const
        {
            return receiveGuildBankUpdatePackets;
        }
...
private:
    ...
    public:
        ...
        LogEntry(uint32 guildId, ObjectGuid::LowType guid, time_t timestamp)
            : m_guildId(guildId), m_guid(guid), m_timestamp(timestamp)
        {
        }
        virtual ~LogEntry() {}
    ...
    public:
        EventLogEntry(uint32              guildId,
                      ObjectGuid::LowType guid,
                      GuildEventLogTypes  eventType,
                      ObjectGuid          playerGuid1,
                      ObjectGuid          playerGuid2,
                      uint8               newRank)
            : LogEntry(guildId, guid), m_eventType(eventType),
              m_playerGuid1(playerGuid1), m_playerGuid2(playerGuid2),
              m_newRank(newRank)
        {
        }
        ...
        EventLogEntry(uint32              guildId,
                      ...
                      ObjectGuid          playerGuid2,
                      uint8               newRank)
            : LogEntry(guildId, guid, timestamp), m_eventType(eventType),
              m_playerGuid1(playerGuid1), m_playerGuid2(playerGuid2),
              m_newRank(newRank)
        {
        }
    ...
    public:
        ...
        bool IsMoneyEvent() const { return IsMoneyEvent(m_eventType); }

        BankEventLogEntry(uint32                 guildId,
                          ObjectGuid::LowType    guid,
                          ...
                          uint8                  destTabId)
            : LogEntry(guildId, guid), m_eventType(eventType),
              m_bankTabId(tabId), m_playerGuid(playerGuid),
              m_itemOrMoney(itemOrMoney), m_itemStackCount(itemStackCount),
    ...
    public:
        RankInfo()
            : m_guildId(0), m_rankId(GUILD_RANK_NONE), m_rights(GR_RIGHT_EMPTY),
              m_bankMoneyPerDay(0)
        {
        }
        RankInfo(uint32 guildId)
            : m_guildId(guildId), m_rankId(GUILD_RANK_NONE),
              m_rights(GR_RIGHT_EMPTY), m_bankMoneyPerDay(0)
        {
        }
        RankInfo(uint32           guildId,
                 uint8            rankId,
                 ...
                 uint32           rights,
                 uint32           money)
            : m_guildId(guildId), m_rankId(rankId), m_name(name),
              m_rights(rights),
              m_bankMoneyPerDay(rankId != GR_GUILDMASTER
                                    ? money
        ...
        {
            return tabId < GUILD_BANK_MAX_TABS
                       ? m_bankTabRightsAndSlots[tabId].GetRights()
                       : 0;
        }
        ...
        {
            return tabId < GUILD_BANK_MAX_TABS
                       ? m_bankTabRightsAndSlots[tabId].GetSlots()
                       : 0;
        }

        void SetBankTabSlotsAndRights(GuildBankRightsAndSlots rightsAndSlots,
                                      bool                    saveToDB);
        void CreateMissingTabsIfNeeded(uint8                        ranks,
    ...
    private:
        ...
        std::array<GuildBankRightsAndSlots, GUILD_BANK_MAX_TABS>
    ...
    public:
        ...
        void Delete(CharacterDatabaseTransaction trans,
        ...
        inline Item* GetItem(uint8 slotId) const
        {
            return slotId < GUILD_BANK_MAX_SLOTS ? m_items[slotId] : nullptr;
        }
        bool
        SetItem(CharacterDatabaseTransaction trans, uint8 slotId, Item* pItem);
    ...
    class MoveItemData {
    public:
        MoveItemData(Guild*  guild,
                     Player* player,
                     uint8   container,
                     uint8   slotId)
            : m_pGuild(guild), m_pPlayer(player), m_container(container),
              m_slotId(slotId), m_pItem(nullptr), m_pClonedItem(nullptr)
        {
        }
        virtual ~MoveItemData() {}

        virtual bool IsBank() const = 0;
        // Initializes item pointer. Returns true, if item exists, false
        // otherwise.
        virtual bool InitItem() = 0;
        // Checks splited amount against item. Splited amount cannot be more
        // that number of items in stack.
        virtual bool CheckItem(uint32& splitedAmount);
        // Defines if player has rights to save item in container
        virtual bool HasStoreRights(MoveItemData* /*pOther*/) const
        {
            return true;
        }
        // Defines if player has rights to withdraw item from container
        virtual bool HasWithdrawRights(MoveItemData* /*pOther*/) const
        {
            return true;
        }
        ...
        virtual void RemoveItem(CharacterDatabaseTransaction trans,
                                MoveItemData*                pOther,
                                uint32 splitedAmount = 0) = 0;
        // Saves item to container
        virtual Item* StoreItem(CharacterDatabaseTransaction trans,
                                Item*                        pItem) = 0;
        // Log bank event
        virtual void LogBankEvent(CharacterDatabaseTransaction trans,
                                  MoveItemData*                pFrom,
    ...
    public:
        PlayerMoveItemData(Guild*  guild,
                           Player* player,
                           uint8   container,
                           uint8   slotId)
        ...
        void  RemoveItem(CharacterDatabaseTransaction trans,
                         MoveItemData*                pOther,
                         uint32 splitedAmount = 0) override;
        Item* StoreItem(CharacterDatabaseTransaction trans,
                        Item*                        pItem) override;
        void  LogBankEvent(CharacterDatabaseTransaction trans,
                           MoveItemData*                pFrom,
    ...
    private:
        Item* _StoreItem(CharacterDatabaseTransaction trans,
                         BankTab*                     pTab,
                         Item*                        pItem,
                         ItemPosCount&                pos,
                         bool                         clone) const;
        bool  _ReserveSpace(uint8   slotId,
                            Item*   pItem,
                            Item*   pItemDest,
                            uint32& count);
        void  CanStoreItemInTab(Item*   pItem,
                                uint8   skipSlotId,
                                bool    merge,
                                uint32& count);
    };
...
public:
    static void SendCommandResult(WorldSession*     session,
                                  GuildCommandType  type,
                                  GuildCommandError errCode,
                                  std::string_view  param = {});
    static void SendSaveEmblemResult(WorldSession*    session,
    ...
    void HandleSetBankTabInfo(WorldSession*    session,
                              uint8            tabId,
                              std::string_view name,
                              std::string_view icon);
    void HandleSetMemberNote(WorldSession*    session,
                             std::string_view name,
                             std::string_view note,
                             bool             officer);
    void
         HandleSetRankInfo(WorldSession*                          session,
                           uint8                                  rankId,
                           std::string_view                       name,
                           uint32                                 rights,
                           uint32                                 moneyPerDay,
                           std::array<GuildBankRightsAndSlots,
                                 GUILD_BANK_MAX_TABS> const& rightsAndSlots);
    void HandleBuyBankTab(WorldSession* session, uint8 tabId);
    void HandleInviteMember(WorldSession* session, std::string const& name);
    void HandleAcceptMember(WorldSession* session);
    void HandleLeaveMember(WorldSession* session);
    void HandleRemoveMember(WorldSession* session, std::string_view name);
    void HandleUpdateMemberRank(WorldSession*    session,
                                std::string_view name,
                                bool             demote);
    ...
    bool HandleMemberWithdrawMoney(WorldSession* session,
                                   uint32        amount,
                                   bool          repair = false);
    ...
    void SendBankTabData(WorldSession* session,
                         uint8         tabId,
    ...
    // Broadcasts
    void BroadcastToGuild(WorldSession*    session,
                          bool             officerOnly,
                          std::string_view msg,
                          uint32           language = LANG_UNIVERSAL) const;
    ...
    // Members
    // Adds member to guild. If rankId == GUILD_RANK_NONE, lowest rank is
    // assigned.
    bool AddMember(ObjectGuid guid, uint8 rankId = GUILD_RANK_NONE);
    void DeleteMember(ObjectGuid guid,
    ...
    // Bank
    void SwapItems(Player* player,
                   uint8   tabId,
                   uint8   slotId,
                   uint8   destTabId,
                   uint8   destSlotId,
                   uint32  splitedAmount);
    void SwapItemsWithInventory(Player* player,
                                bool    toChar,
                                uint8   tabId,
                                uint8   slotId,
                                uint8   playerBag,
    ...
    [[nodiscard]] bool ModifyBankMoney(CharacterDatabaseTransaction trans,
                                       const uint64&                amount,
                                       bool                         add)
    {
...
protected:
    ...
    std::array<LogHolder<BankEventLogEntry>, GUILD_BANK_MAX_TABS + 1>
        m_bankEventLog = {};

private:
    inline uint8 _GetRanksSize() const { return uint8(m_ranks.size()); }
    inline const RankInfo* GetRankInfo(uint8 rankId) const
    {
        return rankId < _GetRanksSize() ? &m_ranks[rankId] : nullptr;
    }
    inline RankInfo* GetRankInfo(uint8 rankId)
    {
        return rankId < _GetRanksSize() ? &m_ranks[rankId] : nullptr;
    }
    inline bool _HasRankRight(Player* player, uint32 right) const
    {
        if (player)
            if (Member const* member = GetMember(player->GetGUID()))
                return (_GetRankRights(member->GetRankId()) & right) !=
                       GR_RIGHT_EMPTY;
        return false;
    }
    ...
    inline uint8 _GetPurchasedTabsSize() const
    {
        return uint8(m_bankTabs.size());
    }
    inline BankTab* GetBankTab(uint8 tabId)
    {
        return tabId < m_bankTabs.size() ? &m_bankTabs[tabId] : nullptr;
    }
    inline BankTab const* GetBankTab(uint8 tabId) const
    {
        return tabId < m_bankTabs.size() ? &m_bankTabs[tabId] : nullptr;
    }
    ...
    inline void _DeleteMemberFromDB(ObjectGuid::LowType lowguid) const
    {
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_MEMBER);
    ...
    void _DeleteBankItems(CharacterDatabaseTransaction trans,
                          bool removeItemsFromDB = false);
    bool _ModifyBankMoney(CharacterDatabaseTransaction trans,
                          uint64                       amount,
                          bool                         add);
    void _SetLeaderGUID(Member& pLeader);

    void        _SetRankBankMoneyPerDay(uint8 rankId, uint32 moneyPerDay);
    void        _SetRankBankTabRightsAndSlots(uint8                   rankId,
    ...
    void  _UpdateMemberWithdrawSlots(CharacterDatabaseTransaction trans,
                                     ObjectGuid                   guid,
                                     uint8                        tabId);
    ...
    void _LogEvent(GuildEventLogTypes eventType,
                   ...
                   uint8              newRank     = 0);
    void _LogBankEvent(CharacterDatabaseTransaction trans,
                       GuildBankEventLogTypes       eventType,
                       uint8                        tabId,
                       ObjectGuid                   playerGuid,
                       uint32                       itemOrMoney,
    ...
    void
    _RemoveItem(CharacterDatabaseTransaction trans, uint8 tabId, uint8 slotId);
    void
    _MoveItems(MoveItemData* pSrc, MoveItemData* pDest, uint32 splitedAmount);
    bool _DoItemsMove(MoveItemData* pSrc,
                      MoveItemData* pDest,
                      bool          sendError,
                      uint32        splitedAmount = 0);

    void _SendBankContent(WorldSession* session,
                          uint8         tabId,
                          bool          sendAllSlots) const;
    void _SendBankMoneyUpdate(WorldSession* session) const;
    void _SendBankContentUpdate(MoveItemData* pSrc, MoveItemData* pDest) const;
    void _SendBankContentUpdate(uint8 tabId, SlotIds slots) const;
    void _SendBankList(WorldSession* session       = nullptr,
                       uint8         tabId         = 0,
                       bool          sendFullSlots = false,
                       SlotIds*      slots         = nullptr) const;

    void _BroadcastEvent(GuildEvents                guildEvent,
