...
Guild::LogHolder<Entry>::LogHolder()
    : m_maxRecords(sWorld->getIntConfig(std::is_same_v<Entry, BankEventLogEntry>
...
{
    // Next guid was not initialized. It means there are no records for this
    // holder in DB yet. Start from the beginning.
    if (m_nextGUID == uint32(GUILD_EVENT_LOG_GUID_UNDEFINED))
        m_nextGUID = 0;
    else
...
Guild::LogEntry::LogEntry(uint32 guildId, ObjectGuid::LowType guid)
    : m_guildId(guildId), m_guid(guid),
      m_timestamp(GameTime::GetGameTime().count())
{
}
...
void Guild::EventLogEntry::SaveToDB(CharacterDatabaseTransaction trans) const
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_EVENTLOG);
...
void Guild::EventLogEntry::WritePacket(
    WorldPackets::Guild::GuildEventLogQueryResults& packet) const
{
    ObjectGuid playerGUID =
        ObjectGuid::Create<HighGuid::Player>(m_playerGuid1.GetCounter());
    ObjectGuid otherGUID =
        ObjectGuid::Create<HighGuid::Player>(m_playerGuid2.GetCounter());
    ...
    eventEntry.TransactionDate =
        uint32(GameTime::GetGameTime().count() - m_timestamp);
    eventEntry.RankID = uint8(m_newRank);
    packet.Entry.push_back(eventEntry);
}

// BankEventLogEntry
void Guild::BankEventLogEntry::SaveToDB(
    CharacterDatabaseTransaction trans) const
{
    uint8 index = 0;

    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_EVENTLOG);
    stmt->SetData(index, m_guildId);
    stmt->SetData(++index, m_guid);
...
void Guild::BankEventLogEntry::WritePacket(
    WorldPackets::Guild::GuildBankLogQueryResults& packet) const
{
    WorldPackets::Guild::GuildBankLogEntry bankLogEntry;
    bankLogEntry.PlayerGUID =
        ObjectGuid::Create<HighGuid::Player>(m_playerGuid.GetCounter());
    bankLogEntry.TimeOffset =
        int32(GameTime::GetGameTime().count() - m_timestamp);
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_RANK);
    stmt->SetData(0, m_guildId);
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(
    uint8                        tabs,
    ...
    bool                         logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i) {
        ...
        if (logOnCreate)
            LOG_ERROR(
                "guild",
                "Guild {} has broken Tab {} for rank {}. Created default tab.",
                m_guildId,
                i,
                m_rankId);

        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_BANK_RIGHT);
        stmt->SetData(0, m_guildId);
        stmt->SetData(1, i);
...
void Guild::RankInfo::SetName(std::string_view name)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RANK_NAME);
...
void Guild::RankInfo::SetRights(uint32 rights)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RANK_RIGHTS);
...
void Guild::RankInfo::SetBankMoneyPerDay(uint32 money)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RANK_BANK_MONEY);
...
void Guild::RankInfo::SetBankTabSlotsAndRights(
    GuildBankRightsAndSlots rightsAndSlots, bool saveToDB)
{
    if (m_rankId == GR_GUILDMASTER) // Prevent loss of leader rights
        rightsAndSlots.SetGuildMasterValues();

    GuildBankRightsAndSlots& guildBR =
        m_bankTabRightsAndSlots[rightsAndSlots.GetTabId()];
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    if (slotId >= GUILD_BANK_MAX_SLOTS) {
        LOG_ERROR(
            "guild",
            "Invalid slot for item (GUID: {}, id: {}) in guild bank, skipped.",
            itemGuid,
            itemEntry);
        return false;
    }
    ...
    if (!proto) {
        LOG_ERROR("guild",
                  "Unknown item (GUID: {}, id: {}) in guild bank, skipped.",
                  itemGuid,
                  itemEntry);
        return false;
    }
    ...
    if (!pItem->LoadFromDB(itemGuid, ObjectGuid::Empty, fields, itemEntry)) {
        LOG_ERROR("guild",
                  "Item (GUID {}, id: {}) not found in item_instance, deleting "
                  "from guild bank!",
                  itemGuid,
                  itemEntry);
...
void Guild::BankTab::SetInfo(std::string_view name, std::string_view icon)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_BANK_TAB_INFO);
    stmt->SetData(0, m_name);
...
void Guild::BankTab::SetText(std::string_view text)
{
    ...
    m_text = text;
    utf8truncate(m_text,
                 MAX_GUILD_BANK_TAB_TEXT_LEN); // DB and client size limitation

    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_BANK_TAB_TEXT);
                             ...
                             Item*                        item)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_ITEM);
    stmt->SetData(0, m_guildId);
    stmt->SetData(1, m_tabId);
    stmt->SetData(2, slotId);
    CharacterDatabase.ExecuteOrAppend(trans, stmt);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    if (session) {
        ...
        LOG_DEBUG("guild",
                  "MSG_QUERY_GUILD_BANK_TEXT [Broadcast]: Tabid: {}, Text: {}",
                  m_tabId,
                  m_text);
...
void Guild::Member::SetPublicNote(std::string_view publicNote)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_PNOTE);
...
void Guild::Member::SetOfficerNote(std::string_view officerNote)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_OFFNOTE);
...
void Guild::Member::ChangeRank(uint8 newRank)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_RANK);
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_MEMBER);
    stmt->SetData(0, m_guildId);
    stmt->SetData(1, m_guid.GetCounter());
    stmt->SetData(2, m_rankId);
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId) {
        LOG_ERROR(
            "guild", "Player ({}) has broken zone-data", m_guid.ToString());
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1) {
        LOG_ERROR("guild",
                  "Player ({}) has a broken data in field "
                  "`characters`.`level`, deleting him from guild!",
                  m_guid.ToString());
        return false;
    }
    ...
    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES) {
        LOG_ERROR("guild",
                  "Player ({}) has a broken data in field "
                  "`characters`.`class`, deleting him from guild!",
                  m_guid.ToString());
        return false;
    }
    return true;
                                            ...
                                            uint32                       amount)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_MEMBER_WITHDRAW);
...
int32 Guild::Member::GetBankWithdrawValue(uint8 tabId) const
{
    ...
    if (IsRank(GR_GUILDMASTER))
        return tabId == GUILD_BANK_MAX_TABS ? GUILD_WITHDRAW_MONEY_UNLIMITED
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_EMBLEM_INFO);
...
void Guild::MoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    sScriptMgr->OnGuildItemMove(m_pGuild,
                                m_pPlayer,
                                pFrom->GetItem(),
                                pFrom->IsBank(),
                                pFrom->GetContainer(),
...
{
    for (ItemPosCountVec::const_iterator itr = m_vec.begin();
         itr != m_vec.end();
         ++itr)
...
bool Guild::PlayerMoveItemData::InitItem()
{
    ...
    if (m_pItem) {
        ...
        if (m_pItem->IsNotEmptyBag()) {
            m_pPlayer->SendEquipError(EQUIP_ERR_CAN_ONLY_DO_WITH_EMPTY_BAGS,
                                      m_pItem);
            m_pItem = nullptr;
        }
...
inline InventoryResult Guild::PlayerMoveItemData::CanStore(Item* pItem,
                                                           bool  swap)
{
...
bool Guild::BankMoveItemData::HasStoreRights(MoveItemData* pOther) const
{
    ...
    if (pOther->IsBank() && pOther->GetContainer() == m_container)
        return true;
    return m_pGuild->_MemberHasTabRights(
        m_pPlayer->GetGUID(), m_container, GUILD_BANK_RIGHT_DEPOSIT_ITEM);
}
                                         ...
                                         uint32 splitedAmount)
{
    ...
    // Decrease amount of player's remaining items (if item is moved to
    // different tab or to player)
    if (!pOther->IsBank() || pOther->GetContainer() != m_container)
        m_pGuild->_UpdateMemberWithdrawSlots(
            trans, m_pPlayer->GetGUID(), m_container);
}
                                         ...
                                         Item*                        pItem)
{
    ...
    for (ItemPosCountVec::const_iterator itr = m_vec.begin();
        ...
        LOG_DEBUG("guild",
                  "GUILD STORAGE: StoreItem tab = {}, slot = {}, item = {}, "
                  "count = {}",
                  m_container,
                  m_slotId,
                  pItem->GetEntry(),
...
// Returns false if destination item specified and it cannot be used to reserve
// space.
                                            ...
                                            uint32& count)
{
    ...
    if (pItemDest) {
        // Make sure source and destination items match and destination item has
...
void Guild::BankMoveItemData::CanStoreItemInTab(Item*   pItem,
                                                ...
                                                uint32& count)
{
    for (uint8 slotId = 0; (slotId < GUILD_BANK_MAX_SLOTS) && (count > 0);
         ++slotId) {
        // Skip slot already processed in CanStore (when destination slot was
        // specified)
...
InventoryResult Guild::BankMoveItemData::CanStore(Item* pItem, bool swap)
{
    ...
    // Slot was not specified or it has not enough space for all the items in
    // stack Search for stacks to merge with
...
// Guild
Guild::Guild() : m_id(0), m_createdDate(0), m_accountsNumber(0), m_bankMoney(0)
{
}
...
bool Guild::Create(Player* pLeader, std::string_view name)
{
    ...
    LOG_DEBUG("guild",
              ...
              m_name,
              pLeader->GetName(),
              m_leaderGuid.ToString());

    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();

    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_MEMBERS);
    stmt->SetData(0, m_id);
    trans->Append(stmt);
    ...
    _CreateDefaultGuildRanks(
        pLeaderSession->GetSessionDbLocaleIndex());     // Create default ranks
    bool ret = AddMember(m_leaderGuid, GR_GUILDMASTER); // Add guildmaster

    for (short i = 0; i < static_cast<short>(sWorld->getIntConfig(
...
void Guild::Disband()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD);
    stmt->SetData(0, m_id);
    trans->Append(stmt);
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_EVENTLOGS);
    stmt->SetData(0, m_id);
    trans->Append(stmt);
...
void Guild::UpdateMemberData(Player* player, uint8 dataid, uint32 value)
{
    if (Member* member = GetMember(player->GetGUID())) {
        ...
        default:
            LOG_ERROR("guild",
                      "Guild::UpdateMemberData: Called with incorrect DATAID "
                      "{} (value {})",
                      dataid,
                      value);
            return;
...
{
    if (m_name == name || name.empty() || name.length() > 24 ||
        sObjectMgr->IsReservedName(name) || sObjectMgr->IsProfanityName(name) ||
    ...
    m_name = name;
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_NAME);
    stmt->SetData(0, m_name);
...
void Guild::HandleRoster(WorldSession* session)
{
    ...
    bool sendOfficerNote =
        _HasRankRight(session->GetPlayer(), GR_RIGHT_VIEWOFFNOTE);
    ...
    for (auto const& [guid, member] : m_members) {
        ...
        memberData.LastSave = float(
            float(GameTime::GetGameTime().count() - member.GetLogoutTime()) /
            DAY);
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    LOG_DEBUG(
        "guild", "SMSG_GUILD_QUERY_RESPONSE [{}]", session->GetPlayerInfo());
}
...
void Guild::HandleSetMOTD(WorldSession* session, std::string_view motd)
{
    ...
    if (!_HasRankRight(session->GetPlayer(), GR_RIGHT_SETMOTD))
        SendCommandResult(
            session, GUILD_COMMAND_EDIT_MOTD, ERR_GUILD_PERMISSIONS);
    else {
        m_motd = motd;
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MOTD);
...
void Guild::HandleSetInfo(WorldSession* session, std::string_view info)
{
    ...
    if (_HasRankRight(session->GetPlayer(), GR_RIGHT_MODIFY_GUILD_INFO)) {
        ...
        CharacterDatabasePreparedStatement* stmt =
            CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_INFO);
...
void Guild::HandleSetEmblem(WorldSession* session, const EmblemInfo& emblemInfo)
{
    ...
    if (!_IsLeader(player))
        SendSaveEmblemResult(
            session, ERR_GUILDEMBLEM_NOTGUILDMASTER); // "Only guild leaders can
                                                      // create emblems."
...
void Guild::HandleSetLeader(WorldSession* session, std::string_view name)
{
    ...
    if (!_IsLeader(player))
        SendCommandResult(
            session, GUILD_COMMAND_CHANGE_LEADER, ERR_GUILD_PERMISSIONS);
                                 ...
                                 std::string_view icon)
{
    ...
    if (!tab) {
        LOG_ERROR("guild",
                  "Guild::HandleSetBankTabInfo: Player {} trying to change "
                  "bank tab info from unexisting tab {}.",
                  session->GetPlayerInfo(),
                  tabId);
        return;
    }
    ...
    _BroadcastEvent(GE_BANK_TAB_UPDATED,
                    ObjectGuid::Empty,
                    std::to_string(tabId),
                    tab->GetName(),
                    tab->GetIcon());
                                ...
                                bool             isPublic)
{
    ...
    if (!_HasRankRight(session->GetPlayer(),
                       isPublic ? GR_RIGHT_EPNOTE : GR_RIGHT_EOFFNOTE))
        SendCommandResult(
            session, GUILD_COMMAND_PUBLIC_NOTE, ERR_GUILD_PERMISSIONS);
...
void Guild::HandleSetRankInfo(
    WorldSession*    session,
    uint8            rankId,
    std::string_view name,
    uint32           rights,
    uint32           moneyPerDay,
    std::array<GuildBankRightsAndSlots, GUILD_BANK_MAX_TABS> const&
        rightsAndSlots)
{
    // Only leader can modify ranks
    if (!_IsLeader(session->GetPlayer()))
        SendCommandResult(
            session, GUILD_COMMAND_CHANGE_RANK, ERR_GUILD_PERMISSIONS);
        ...
        _BroadcastEvent(GE_RANK_UPDATED,
                        ObjectGuid::Empty,
        ...
        LOG_DEBUG("guild",
                  "Changed RankName to '{}', rights to 0x{:08X}",
                  rankInfo->GetName(),
                  rights);
    }
}
...
void Guild::HandleBuyBankTab(WorldSession* session, uint8 tabId)
{
    ...
    if (!player->HasEnoughMoney(
            tabCost)) // Should not happen, this is checked by client
        return;
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    if (!pInvitee) {
        SendCommandResult(
            session, GUILD_COMMAND_INVITE, ERR_GUILD_PLAYER_NOT_FOUND_S, name);
        return;
    }
    ...
    if (memberLimit > 0 &&
        player->GetGuild()->GetMemberCount() >= memberLimit) {
        ChatHandler(player->GetSession())
            .PSendSysMessage("Your guild has reached the maximum amount of "
                             "members (%u). You cannot send another invite "
                             "until the guild member count is lower.",
                             memberLimit);
        SendCommandResult(
            session, GUILD_COMMAND_INVITE, ERR_GUILD_INTERNAL, name);
        return;
    }

    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        pInvitee->GetTeamId(true) != player->GetTeamId(true)) {
        SendCommandResult(
            session, GUILD_COMMAND_INVITE, ERR_GUILD_NOT_ALLIED, name);
        return;
    }
    // Invited player cannot be in another guild
    if (pInvitee->GetGuildId()) {
        SendCommandResult(
            session, GUILD_COMMAND_INVITE, ERR_ALREADY_IN_GUILD_S, name);
        return;
    }
    // Invited player cannot be invited
    if (pInvitee->GetGuildIdInvited()) {
        SendCommandResult(session,
    ...
    SendCommandResult(
        session, GUILD_COMMAND_INVITE, ERR_GUILD_COMMAND_SUCCESS, name);

    LOG_DEBUG("guild",
              "Player {} invited {} to join his Guild",
              player->GetName(),
              pInvitee->GetName());

    pInvitee->SetGuildIdInvited(m_id);
    _LogEvent(
        GUILD_EVENT_LOG_INVITE_PLAYER, player->GetGUID(), pInvitee->GetGUID());
...
void Guild::HandleAcceptMember(WorldSession* session)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        player->GetTeamId() !=
            sCharacterCache->GetCharacterTeamByGuid(GetLeaderGUID())) {
        return;
    }
...
void Guild::HandleLeaveMember(WorldSession* session)
{
    ...
    if (_IsLeader(player)) {
        if (m_members.size() > 1)
            ...
            SendCommandResult(
                session, GUILD_COMMAND_QUIT, ERR_GUILD_LEADER_LEAVE);
        else {
        ...
        SendCommandResult(
            session, GUILD_COMMAND_QUIT, ERR_GUILD_COMMAND_SUCCESS, m_name);
    }
...
void Guild::HandleRemoveMember(WorldSession* session, std::string_view name)
{
    ...
    if (!_HasRankRight(player, GR_RIGHT_REMOVE))
        ...
        if (member->IsRank(GR_GUILDMASTER))
            SendCommandResult(
                session, GUILD_COMMAND_REMOVE, ERR_GUILD_LEADER_LEAVE);
        // Do not allow to remove player with the same rank or higher
            ...
            if (!memberMe || member->IsRankNotLower(memberMe->GetRankId()))
                ...
                _LogEvent(
                    GUILD_EVENT_LOG_UNINVITE_PLAYER, player->GetGUID(), guid);
                _BroadcastEvent(GE_REMOVED,
                                ObjectGuid::Empty,
                                member->GetName(),
                                player->GetName());
            }
                                   ...
                                   bool             demote)
{
    Player*          player = session->GetPlayer();
    GuildCommandType type =
        demote ? GUILD_COMMAND_DEMOTE : GUILD_COMMAND_PROMOTE;
    ...
    // Promoted player must be a member of guild
        ...
        if (demote) {
            ...
            if (member->IsRankNotLower(rankId)) {
                SendCommandResult(
                    session, type, ERR_GUILD_RANK_TOO_HIGH_S, name);
                return;
            }
            // Lowest rank cannot be demoted
            if (member->GetRankId() >= _GetLowestRankId()) {
                SendCommandResult(
                    session, type, ERR_GUILD_RANK_TOO_LOW_S, name);
                return;
            }
        }
        else {
            // Allow to promote only to lower rank than member's rank
            // member->GetRankId() + 1 is the highest rank that current player
            // can promote to
        ...
        _LogEvent(demote ? GUILD_EVENT_LOG_DEMOTE_PLAYER
                  ...
                  member->GetGUID(),
                  newRankId);
        _BroadcastEvent(demote ? GE_DEMOTION : GE_PROMOTION,
                        ObjectGuid::Empty,
                        player->GetName(),
                        member->GetName(),
                        _GetRankName(newRankId));
    }
}
...
{
    // Cannot remove rank if total count is minimum allowed by the client or is
    // not leader
    ...
    _BroadcastEvent(
        GE_RANK_DELETED, ObjectGuid::Empty, std::to_string(m_ranks.size()));
}
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    if (m_bankMoney > GUILD_BANK_MONEY_LIMIT - amount) {
        SendCommandResult(
            session, GUILD_COMMAND_MOVE_ITEM, ERR_GUILD_BANK_FULL);
        return;
    }
    ...
    std::string aux = Acore::Impl::ByteArrayToHexStr(
        reinterpret_cast<uint8*>(&m_bankMoney), 8, true);
    _BroadcastEvent(GE_BANK_MONEY_SET, ObjectGuid::Empty, aux.c_str());

    if (amount >
        10 * GOLD) // receiver_acc = Guild id, receiver_name = Guild name
        CharacterDatabase.Execute(
            "INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", "
            "{}, \"(guild members: {}, new amount: {}, leader guid low: {}, "
            "sender level: {})\", NOW(), {})",
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session,
                                      ...
                                      bool          repair)
{
    // clamp amount to MAX_MONEY_AMOUNT, Players can't hold more than that
    // anyway
    ...
    if (uint32(_GetMemberRemainingMoney(*member)) <
        amount) // Check if we have enough slot/money today
        return false;
    ...
    _LogBankEvent(trans,
                  repair ? GUILD_BANK_LOG_REPAIR_MONEY
    ...
    if (amount > 10 * GOLD) // sender_acc = 0 (guild has no account),
                            // sender_guid = Guild id, sender_name = Guild name
        CharacterDatabase.Execute(
            "INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", "
            "{}, \"(guild, members: {}, new amount: {}, leader guid low: {}, "
            "withdrawer level: {})\", NOW(), {})",
            0,
            GetId(),
            GetName(),
            session->GetRemoteAddress(),
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    LOG_DEBUG(
        "guild", "MSG_GUILD_EVENT_LOG_QUERY [{}]", session->GetPlayerInfo());
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS) {
        std::list<BankEventLogEntry> const& bankEventLog =
        ...
        session->SendPacket(packet.Write());
        LOG_DEBUG(
            "guild", "MSG_GUILD_BANK_LOG_QUERY [{}]", session->GetPlayerInfo());
    }
}

void Guild::SendBankTabData(WorldSession* session,
                            uint8         tabId,
                            bool          sendAllSlots) const
{
    if (tabId < _GetPurchasedTabsSize())
...
void Guild::SendPermissions(WorldSession* session)
{
    ...
    // We are unsubscribing here since it is the only reliable way to handle
    // /reload from player as GuildPermissionsQuery is sent on each reload, and
    // we don't want to send partial changes while client doesn't know the full
    // state
    ...
    for (uint8 tabId = 0; tabId < GUILD_BANK_MAX_TABS; ++tabId) {
        ...
        queryResult.Tab[tabId].WithdrawItemLimit =
            _GetMemberRemainingSlots(*member, tabId);
    }

    session->SendPacket(queryResult.Write());
    LOG_DEBUG("guild",
              "MSG_GUILD_PERMISSIONS [{}] Rank: {}",
              session->GetPlayerInfo(),
              rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    LOG_DEBUG("guild",
              "MSG_GUILD_BANK_MONEY_WITHDRAWN [{}] Money: {}",
              session->GetPlayerInfo(),
              amount);
}
...
bool Guild::LoadFromDB(Field* fields)
{
    ...
    m_leaderGuid =
        ObjectGuid::Create<HighGuid::Player>(fields[2].Get<uint32>());
...
bool Guild::LoadMemberFromDB(Field* fields)
{
    ...
    auto [memberIt, isNew] = m_members.try_emplace(
        lowguid, m_id, playerGuid, fields[2].Get<uint8>());
    if (!isNew) {
        LOG_ERROR("guild",
                  "Tried to add {} to guild '{}'. Member already exists.",
                  playerGuid.ToString(),
                  m_name);
        return false;
    }
...
{
    // tabId              rights                slots
    GuildBankRightsAndSlots rightsAndSlots(fields[1].Get<uint8>(),
                                           fields[3].Get<uint8>(),
                                           fields[4].Get<uint32>());
    // rankId
    _SetRankBankTabRightsAndSlots(
        fields[2].Get<uint8>(), rightsAndSlots, false);
}
    ...
    if (m_eventLog.CanInsert()) {
        m_eventLog.LoadEvent(
            ...
            ObjectGuid::Create<HighGuid::Player>(
                fields[3].Get<uint32>()), // player guid 1
            ObjectGuid::Create<HighGuid::Player>(
                fields[4].Get<uint32>()), // player guid 2
    ...
    if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab) {
        ...
        if (bankLog.CanInsert()) {
            ...
            GuildBankEventLogTypes eventType =
                GuildBankEventLogTypes(fields[3].Get<uint8>());
            if (BankEventLogEntry::IsMoneyEvent(eventType)) {
                if (!isMoneyTab) {
                    LOG_ERROR("guild",
                              "GuildBankEventLog ERROR: MoneyEvent(LogGuid: "
                              "{}, Guild: {}) does not belong to money tab "
                              "({}), ignoring...",
                              guid,
                              m_id,
                              dbTabId);
                ...
                LOG_ERROR("guild",
                          "GuildBankEventLog ERROR: non-money event (LogGuid: "
                          "{}, Guild: {}) belongs to money tab, ignoring...",
                          guid,
                          m_id);
                return false;
            }
            bankLog.LoadEvent(m_id,                            // guild id
                              guid,                            // guid
                              time_t(fields[8].Get<uint32>()), // timestamp
                              dbTabId,                         // tab id
                              eventType,                       // event type
                              ObjectGuid::Create<HighGuid::Player>(
                                  fields[4].Get<uint32>()), // player guid
...
void Guild::LoadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
        LOG_ERROR(
            "guild", "Invalid tab (tabId: {}) in guild bank, skipped.", tabId);
    else
...
// Validates guild data loaded from database. Returns false if guild should be
// deleted.
bool Guild::Validate()
{
    // Validate ranks data
    // GUILD RANKS represent a sequence starting from 0 = GUILD_MASTER (ALL
    // PRIVILEGES) to max 9 (lowest privileges). The lower rank id is considered
    // higher rank - so promotion does rank-- and demotion does rank++ Between
    // ranks in sequence cannot be gaps - so 0, 1, 2, 4 is impossible Min ranks
    // count is 5 and max is 10.
    ...
    if (ranks < GUILD_RANKS_MIN_COUNT || ranks > GUILD_RANKS_MAX_COUNT) {
        ...
        for (uint8 rankId = 0; rankId < ranks; ++rankId) {
            ...
            if (rankInfo->GetId() != rankId) {
                LOG_ERROR("guild",
                          "Guild {} has broken rank id {}, creating default "
                          "set of ranks...",
                          m_id,
                          rankId);
                broken_ranks = true;
                ...
                rankInfo->CreateMissingTabsIfNeeded(
                    _GetPurchasedTabsSize(), trans, true);
                CharacterDatabase.CommitTransaction(trans);
            }
        }
    }
    ...
    if (!sConfigMgr->GetOption<bool>("Guild.AllowMultipleGuildMaster", 0))
        for (auto& [guid, member] : m_members)
            if ((member.GetRankId() == GR_GUILDMASTER) &&
                !member.IsSamePlayer(m_leaderGuid))
...
void Guild::BroadcastToGuild(WorldSession*    session,
                             bool             officerOnly,
                             std::string_view msg,
                             uint32           language) const
{
    if (session && session->GetPlayer() &&
        _HasRankRight(session->GetPlayer(),
                      officerOnly ? GR_RIGHT_OFFCHATSPEAK
                                  : GR_RIGHT_GCHATSPEAK)) {
        WorldPacket data;
        ChatHandler::BuildChatPacket(data,
                                     officerOnly ? CHAT_MSG_OFFICER
                                                 : CHAT_MSG_GUILD,
                                     Language(language),
                                     session->GetPlayer(),
                                     nullptr,
        ...
        for (auto const& [guid, member] : m_members)
            if (Player* player = member.FindPlayer())
                if (_HasRankRight(player,
                                  officerOnly ? GR_RIGHT_OFFCHATLISTEN
                                              : GR_RIGHT_GCHATLISTEN) &&
                    !player->GetSocial()->HasIgnore(
    ...
    for (auto const& [guid, member] : m_members) {
        ...
        if (count >= CALENDAR_MAX_INVITES) {
            if (Player* player = session->GetPlayer())
                sCalendarMgr->SendCalendarCommandResult(
                    player->GetGUID(), CALENDAR_ERROR_INVITES_EXCEEDED);
            return;
        }

        uint32 level =
            sCharacterCache->GetCharacterLevelByGuid(member.GetGUID());

        if (member.GetGUID() != session->GetPlayer()->GetGUID() &&
            level >= minLevel && level <= maxLevel &&
            member.IsRankNotLower(minRank)) {
...
bool Guild::AddMember(ObjectGuid guid, uint8 rankId)
{
    ...
    // This will be prevent attempt to join many guilds and corrupt guild data
    // integrity
                         ...
                         bool       canDeleteGuild)
{
    ...
    // characters table or when he is removed from guild by gm command
    if (m_leaderGuid == guid && !isDisbanding) {
        ...
        // If leader does not exist (at guild loading with deleted leader) do
        // not send broadcasts
        if (oldLeader) {
            _BroadcastEvent(GE_LEADER_CHANGED,
                            ObjectGuid::Empty,
    ...
    // Call script on remove before member is actually removed from guild (and
    // database)
...
{
    if (newRank <=
        _GetLowestRankId()) // Validate rank (allow only existing ranks)
                      ...
                      uint32  splitedAmount)
{
    if (tabId >= _GetPurchasedTabsSize() || slotId >= GUILD_BANK_MAX_SLOTS ||
        ...
        destSlotId >= GUILD_BANK_MAX_SLOTS)
        return;
...
void Guild::SwapItemsWithInventory(Player* player,
                                   ...
                                   uint32  splitedAmount)
{
    if ((slotId >= GUILD_BANK_MAX_SLOTS && slotId != NULL_SLOT) ||
...
void Guild::_CreateNewBankTab()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_TAB);
    stmt->SetData(0, m_id);
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_RANKS);
    stmt->SetData(0, m_id);
    CharacterDatabase.Execute(stmt);
    ...
    _CreateRank(sObjectMgr->GetAcoreString(LANG_GUILD_MASTER, loc),
                GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetAcoreString(LANG_GUILD_OFFICER, loc),
                GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetAcoreString(LANG_GUILD_VETERAN, loc),
                GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetAcoreString(LANG_GUILD_MEMBER, loc),
                GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetAcoreString(LANG_GUILD_INITIATE, loc),
                GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
                             ...
                             bool                         add)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_BANK_MONEY);
...
void Guild::_SetLeaderGUID(Member& pLeader)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_LEADER);
    stmt->SetData(0, m_leaderGuid.GetCounter());
    stmt->SetData(1, m_id);
    CharacterDatabase.Execute(stmt);
}
...
void Guild::_SetRankBankTabRightsAndSlots(
    uint8 rankId, GuildBankRightsAndSlots rightsAndSlots, bool saveToDB)
{
...
inline int32 Guild::_GetMemberRemainingSlots(Member const& member,
                                             uint8         tabId) const
{
    uint8 rankId = member.GetRankId();
    if (rankId == GR_GUILDMASTER)
        return static_cast<int32>(GUILD_WITHDRAW_SLOT_UNLIMITED);
    if ((_GetRankBankTabRights(rankId, tabId) & GUILD_BANK_RIGHT_VIEW_TAB) !=
        0) {
        int32 remaining = _GetRankBankTabSlotsPerDay(rankId, tabId) -
                          member.GetBankWithdrawValue(tabId);
        if (remaining > 0)
            return remaining;
    }
    return 0;
...
inline int32 Guild::_GetMemberRemainingMoney(Member const& member) const
{
    ...
    if ((_GetRankRights(rankId) &
         (GR_RIGHT_WITHDRAW_REPAIR | GR_RIGHT_WITHDRAW_GOLD)) != 0) {
        int32 remaining = _GetRankBankMoneyPerDay(rankId) -
                          member.GetBankWithdrawValue(GUILD_BANK_MAX_TABS);
        if (remaining > 0)
            return remaining;
    }
    return 0;
...
inline void Guild::_UpdateMemberWithdrawSlots(
    CharacterDatabaseTransaction trans, ObjectGuid guid, uint8 tabId)
{
    if (Member* member = GetMember(guid)) {
        uint8 rankId = member->GetRankId();
        if (rankId != GR_GUILDMASTER &&
            member->GetBankWithdrawValue(tabId) <
                _GetRankBankTabSlotsPerDay(rankId, tabId))
            member->UpdateBankWithdrawValue(trans, tabId, 1);
    }
}

inline bool
Guild::_MemberHasTabRights(ObjectGuid guid, uint8 tabId, uint32 rights) const
{
    if (const Member* member = GetMember(guid)) {
        // Leader always has full rights
        if (member->IsRank(GR_GUILDMASTER) || m_leaderGuid == guid)
            return true;
        return (_GetRankBankTabRights(member->GetRankId(), tabId) & rights) ==
               rights;
    }
    return false;
}
...
inline void Guild::_LogEvent(GuildEventLogTypes eventType,
                             ObjectGuid         playerGuid1,
                             ObjectGuid         playerGuid2,
                             uint8              newRank)
{
    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
    m_eventLog.AddEvent(trans,
                        m_id,
                        m_eventLog.GetNextGUID(),
                        eventType,
                        playerGuid1,
                        playerGuid2,
                        newRank);
    ...
    sScriptMgr->OnGuildEvent(this,
                             uint8(eventType),
                             playerGuid1.GetCounter(),
                             playerGuid2.GetCounter(),
                             newRank);
}
                          ...
                          uint8                        destTabId)
{
    ...
    sScriptMgr->OnGuildBankEvent(this,
                                 uint8(eventType),
                                 tabId,
                                 guid.GetCounter(),
                                 itemOrMoney,
...
inline void Guild::_RemoveItem(CharacterDatabaseTransaction trans,
                               uint8                        tabId,
                               uint8                        slotId)
{
    if (BankTab* pTab = GetBankTab(tabId))
    ...
    else // 6. No split
    {
        ...
        if (!_DoItemsMove(pSrc, pDest, false)) // Item could not be merged
        {
            ...
            if (!pSrc->HasStoreRights(pDest))
                return; // Player has no rights to store item in source
                        // (opposite direction)

            if (!pDest->HasWithdrawRights(pSrc))
                return; // Player has no rights to withdraw item from
...
void Guild::_BroadcastEvent(GuildEvents                guildEvent,
                            ...
                            Optional<std::string_view> param3 /*= {}*/) const
{
                          ...
                          SlotIds*      slots /*= nullptr*/) const
{
    ...
    if (BankTab const* tab = GetBankTab(tabId)) {
        auto fillItems = [&](auto begin, auto end, bool skipEmpty) {
            for (auto itr = begin; itr != end; ++itr) {
                if (Item* tabItem = tab->GetItem(*itr)) {
                    ...
                    itemInfo.Charges =
                        int32(std::abs(tabItem->GetSpellCharges()));
                    itemInfo.EnchantmentID =
                        int32(tabItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT));
                    itemInfo.Flags = tabItem->GetInt32Value(ITEM_FIELD_FLAGS);
                    itemInfo.RandomPropertiesID =
                        tabItem->GetItemRandomPropertyId();
                    itemInfo.RandomPropertiesSeed =
                        int32(tabItem->GetItemSuffixFactor());

                    for (uint32 socketSlot = 0; socketSlot < MAX_GEM_SOCKETS;
                         ++socketSlot) {
                        if (uint32 enchId =
                                tabItem->GetEnchantmentId(EnchantmentSlot(
                                    SOCK_ENCHANTMENT_SLOT + socketSlot))) {
        ...
        if (sendAllSlots)
            fillItems(
                boost::make_counting_iterator(uint8(0)),
                boost::make_counting_iterator(uint8(GUILD_BANK_MAX_SLOTS)),
                true);
    ...
    if (session) {
        ...
        for (auto const& [guid, member] : m_members) {
            ...
            if (!_MemberHasTabRights(
                    member.GetGUID(), tabId, GUILD_BANK_RIGHT_VIEW_TAB))
                continue;
            ...
            packet.SetWithdrawalsRemaining(
                _GetMemberRemainingSlots(member, tabId));
            player->SendDirectMessage(packet.GetRawPacket());
            LOG_DEBUG("guild",
                      "SMSG_GUILD_BANK_LIST [{}]: TabId: {}, FullSlots: {}, "
                      "slots: {}",
                      player->GetName(),
                      tabId,
                      sendAllSlots,
