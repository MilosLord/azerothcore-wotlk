...
bool MapSessionFilter::Process(WorldPacket* packet)
{
    ...
    // let's check if our has an anxiety disorder can be really processed in
    // Map::Update()
...
WorldSession::WorldSession(uint32                       id,
                           ...
                           bool                         skipQueue,
                           uint32                       TotalTime)
    : m_muteTime(mute_time), m_timeOutTime(0), _lastAuctionListItemsMSTime(0),
      _lastAuctionListOwnerItemsMSTime(0), AntiDOS(this), m_GUIDLow(0),
      _player(nullptr), m_Socket(sock), _security(sec), _skipQueue(skipQueue),
      _accountId(id), _accountName(std::move(name)), m_expansion(expansion),
      m_total_time(TotalTime), _logoutTime(0), m_inQueue(false),
      m_playerLoading(false), m_playerLogout(false),
      m_playerRecentlyLogout(false), m_playerSave(false),
      m_sessionDbcLocale(sWorld->GetDefaultDbcLocale()),
      m_sessionDbLocaleIndex(locale), m_latency(0), m_TutorialsChanged(false),
      recruiterId(recruiter), isRecruiter(isARecruiter),
      m_currentVendorEntry(0), _calendarEventCreationCooldown(0),
      _addonMessageReceiveCount(0), _timeSyncClockDeltaQueue(6),
      _timeSyncClockDelta(0), _pendingTimeSyncRequests()
{
    ...
    if (sock) {
        ...
        LoginDatabase.Execute("UPDATE account SET online = 1 WHERE id = {};",
...
{
    LoginDatabase.Execute("UPDATE account SET totaltime = {} WHERE id = {}",
                          GetTotalTime(),
                          GetAccountId());
    ...
    LoginDatabase.Execute("UPDATE account SET online = 0 WHERE id = {};",
                          GetAccountId()); // One-time query
}
...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    if ((cur_time - lastTime) < 60) {
        ...
        LOG_DEBUG("network",
                  "Send all time packets count: {} bytes: {} avr.count/sec: {} "
                  "avr.bytes/sec: {} time: {}",
                  sendPacketCount,
                  sendPacketBytes,
        ...
        LOG_DEBUG("network",
                  "Send last min packets count: {} bytes: {} avr.count/sec: {} "
                  "avr.bytes/sec: {}",
                  sendLastPacketCount,
                  sendLastPacketBytes,
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
    ...
    /// If necessary, kick the player because the client didn't send anything
    /// for too long (or they've been idling in character select)
    if (sWorld->getBoolConfig(CONFIG_CLOSE_IDLE_CONNECTIONS) &&
        IsConnectionIdle() && m_Socket)
        m_Socket->CloseSocket();
    ...
    while (m_Socket && _recvQueue.next(packet, updater)) {
        ...
        METRIC_DETAILED_TIMER("worldsession_update_opcode_time",
                              METRIC_TAG("opcode", opHandle->Name));
        LOG_DEBUG(
            "network", "message id {} ({}) under READ", opcode, opHandle->Name);
        ...
        try {
            ...
            case STATUS_LOGGEDIN:
                if (!_player) {
                    ...
                    // packets
                    //! If player didn't log out a while ago, it means packets
                    //! are being sent while the server does not recognize the
                    //! client to be in world yet. We will re-add the packets to
                    //! the bottom of the queue and process them later.
                    if (!m_playerRecentlyLogout) {
                        requeuePackets.push_back(packet);
                        deletePacket = false;

                        LOG_DEBUG("network",
                                  "Delaying processing of message with status "
                                  "STATUS_LOGGEDIN: No players in the world "
                                  "for account id {}",
                                  GetAccountId());
                    }
                }
                    ...
                    if (AntiDOS.EvaluateOpcode(*packet, currentTime)) {
                        ...
                        processedPackets =
                            MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE; // break out of packet processing loop
                }
            ...
            case STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT:
                if (!_player &&
                    !m_playerRecentlyLogout) // There's a short delay between
                                             // _player = null and
                                             ...
                                             // during logout
                {
                    LogUnexpectedOpcode(packet,
                                        ...
                                        "the player has not logged in yet and "
                                        "not recently logout");
                }
                    ...
                    processedPackets =
                        MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE; // break
                                                                           // out
                                                                           // of
                                                                           // packet
                                                                           // processing
            ...
            case STATUS_AUTHED:
                ...
                // STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT opcodes however when we
                // recieve CMSG_CHAR_ENUM we are surely no longer during the
                // logout process.
            ...
            case STATUS_UNHANDLED:
                LOG_DEBUG("network.opcode",
                          "Received not handled opcode {} from {}",
                          GetOpcodeNameForLogging(
                              static_cast<OpcodeClient>(packet->GetOpcode())),
                          GetPlayerInfo());
                break;
            ...
            LOG_ERROR("network",
                      "{} sent {} which illegally contained a hyperlink:\n{}",
                      GetPlayerInfo(),
                      GetOpcodeNameForLogging(
                          static_cast<OpcodeClient>(packet->GetOpcode())),
                      ihe.GetInvalidValue());
            ...
            LOG_ERROR("network",
                      ...
                      "parsing a packet (opcode: {}) from client {}, "
                      "accountid={}. Skipped packet.",
                      packet->GetOpcode(),
                      GetRemoteAddress(),
                      GetAccountId());
        ...
        if (processedPackets >
            MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE)
            break;
    }
...
{
    if (m_Socket && !m_Socket->IsOpen() && !IsKicked() && GetPlayer() &&
        !PlayerLogout() && GetPlayer()->m_taxi.empty() &&
        GetPlayer()->IsInWorld() && !World::IsStopped()) {
        m_Socket = nullptr;
...
void WorldSession::HandleTeleportTimeout(bool updateInSessions)
{
    ...
    if (m_Socket && m_Socket->IsOpen() && GetPlayer() &&
        ...
        if (updateInSessions) // session update from World::UpdateSessions
        {
            if (GetPlayer()->IsBeingTeleportedFar() &&
                GetPlayer()->GetSemaphoreTeleportFar() +
                        sWorld->getIntConfig(CONFIG_TELEPORT_TIMEOUT_FAR) <
                    currTime)
        ...
        {
            if (GetPlayer()->IsBeingTeleportedNear() &&
                GetPlayer()->GetSemaphoreTeleportNear() +
                        sWorld->getIntConfig(CONFIG_TELEPORT_TIMEOUT_NEAR) <
                    currTime)
                while (GetPlayer() && GetPlayer()->IsInWorld() &&
                       GetPlayer()->IsBeingTeleportedNear()) {
...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player) {
        ...
        ///- If the player just died before logging out, make him appear as a
        ///ghost
        // FIXME: logout must be delayed in case lost connection with client in
        // time of combat
        ...
        // pussywizard: leave whole bg on logout (character stays ingame when
        // necessary) pussywizard: GetBattleground() checked inside
        _player->LeaveBattleground();

        // pussywizard: checked first time
        if (!_player->IsBeingTeleportedFar() && !_player->m_InstanceValid &&
            ...
            if (BattlegroundQueueTypeId bgQueueTypeId =
                ...
                if (_player->IsInvitedForBattlegroundInstance()) {
                    ...
                    sScriptMgr->OnBattlegroundDesertion(
                        _player, BG_DESERTION_TYPE_INVITE_LOGOUT);
                }

                _player->RemoveBattlegroundQueueId(bgQueueTypeId);
                sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId)
                    .RemovePlayer(_player->GetGUID(), true);
            }
        ...
        // pussywizard: on logout remove auras that are removed at map change
        // (before saving to db) there are some positive auras from boss
        // encounters that can be kept by logging out and logging in after boss
        // is dead, and may be used on next bosses
        _player->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_CHANGE_MAP);

        ///- If the player is in a group and LeaveGroupOnLogout is enabled or if
        ///the player is invited to a group, remove him. If the group is then
        ///only 1 person, disband the group.
        if (!_player->GetGroup() ||
        ...
        // kicked or disconnected) d) LeaveGroupOnLogout is enabled
        ...
        // player because of not present map Teleport player immediately for
        // correct player save
        while (_player && _player->IsBeingTeleportedFar())
            HandleMoveWorldportAck();
        ...
        if (save) {
            ...
            for (int j = BUYBACK_SLOT_START; j < BUYBACK_SLOT_END; ++j) {
                ...
                _player->SetGuidValue(PLAYER_FIELD_VENDORBUYBACK_SLOT_1 +
                                          (eslot * 2),
                                      ObjectGuid::Empty);
                _player->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE_1 + eslot,
                                        0);
                _player->SetUInt32Value(
                    PLAYER_FIELD_BUYBACK_TIMESTAMP_1 + eslot, 0);
            }
        ...
        if (_player->GetGroup()) {
            ...
            if (_player->GetMap()->IsDungeon() ||
                _player->GetMap()->IsRaidOrHeroicDungeon()) {
                Map::PlayerList const& playerList =
                    _player->GetMap()->GetPlayers();
        ...
        sSocialMgr->SendFriendStatus(
            _player, FRIEND_OFFLINE, _player->GetGUID(), true);
        ...
        LOG_INFO("entities.player",
                 "Account: {} (IP: {}) Logout Character:[{}] ({}) Level: {}",
                 GetAccountId(),
                 GetRemoteAddress(),
                 _player->GetName(),
        ...
        //! Client will respond by sending 3x CMSG_CANCEL_TRADE, which we
        //! currently dont handle
...
void WorldSession::KickPlayer(std::string const& reason, bool setKicked)
{
    if (m_Socket) {
        LOG_INFO("network.kick",
                 "Account: {} Character: '{}' {} kicked with reason: {}",
                 GetAccountId(),
                 _player ? _player->GetName() : "<none>",
                 _player ? _player->GetGUID().ToString() : "",
                 reason);
    ...
    if (setKicked)
        SetKicked(true); // pussywizard: the session won't be left ingame for 60
                         // seconds and to also kick offline session
}
...
bool WorldSession::DisallowHyperlinksAndMaybeKick(std::string_view str)
{
    ...
    LOG_ERROR("network",
              "Player {} {} sent a message which illegally contained a "
              "hyperlink:\n%.*s",
              GetPlayer()->GetName(),
              GetPlayer()->GetGUID().ToString(),
              STRING_VIEW_FMT_ARG(str));
...
void WorldSession::Handle_NULL(WorldPacket& null)
{
    LOG_ERROR(
        "network.opcode",
        "Received unhandled opcode {} from {}",
...
{
    LOG_ERROR("network.opcode",
              "Received opcode {} that must be processed in "
              "WorldSocket::ReadDataHandler from {}",
              GetOpcodeNameForLogging(
                  static_cast<OpcodeClient>(recvPacket.GetOpcode())),
              GetPlayerInfo());
}
...
void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    LOG_ERROR("network.opcode",
              ...
              GetOpcodeNameForLogging(
                  static_cast<OpcodeServer>(recvPacket.GetOpcode())),
              GetPlayerInfo());
}
    ...
    do {
        ...
        if (type >= NUM_ACCOUNT_DATA_TYPES) {
            LOG_ERROR("network",
                      ...
                      mask == GLOBAL_CACHE_MASK ? "account_data"
        ...
        if ((mask & (1 << type)) == 0) {
            LOG_ERROR("network",
                      "Table `{}` have non appropriate for table  account data "
                      "type ({}), ignore.",
                      mask == GLOBAL_CACHE_MASK ? "account_data"
                                                : "character_account_data",
                      type);
                                  ...
                                  std::string const& data)
{
    ...
    if ((1 << type) & GLOBAL_CACHE_MASK) {
        ...
        // _player can be nullptr and packet received after logout but m_GUID
        // still store correct guid
        if (!m_GUIDLow)
            return;
...
void WorldSession::SaveTutorialsData(CharacterDatabaseTransaction trans)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_HAS_TUTORIALS);
    stmt->SetData(0, GetAccountId());
    bool hasTutorials = bool(CharacterDatabase.Query(stmt));

    stmt = CharacterDatabase.GetPreparedStatement(
        hasTutorials ? CHAR_UPD_TUTORIALS : CHAR_INS_TUTORIALS);
...
void WorldSession::ReadMovementInfo(WorldPacket& data, MovementInfo* mi)
{
    ...
    if (mi->HasMovementFlag(
            MovementFlags(MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) ||
    ...
    if (mi->HasMovementFlag(MOVEMENTFLAG_SPLINE_ELEVATION))
        ...
        //! Anti-cheat checks. Please keep them in seperate if() blocks to
        //! maintain a clear overview. Might be subject to latency, so just
        //! remove improper flags.
#ifdef ACORE_DEBUG
#define REMOVE_VIOLATING_FLAGS(check, maskToRemove)                            \
    {                                                                          \
        if (check) {                                                           \
            LOG_DEBUG("entities.unit",                                         \
                      "WorldSession::ReadMovementInfo: Violation of "          \
                      "MovementFlags found ({}). "                             \
                      "MovementFlags: {}, MovementFlags2: {} for player {}. "  \
...
#define REMOVE_VIOLATING_FLAGS(check, maskToRemove)                            \
    ...
    */
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_HOVER) &&
                               !GetPlayer()->HasAuraType(SPELL_AURA_HOVER),
                           MOVEMENTFLAG_HOVER);
    ...
    REMOVE_VIOLATING_FLAGS(
        mi->HasMovementFlag(MOVEMENTFLAG_WATERWALKING) &&
    ...
    */
    ...
    REMOVE_VIOLATING_FLAGS(
        mi->HasMovementFlag(MOVEMENTFLAG_FLYING | MOVEMENTFLAG_CAN_FLY) &&
            ...
            !GetPlayer()->m_mover->HasAuraType(
                SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED),
        MOVEMENTFLAG_FLYING | MOVEMENTFLAG_CAN_FLY);

    //! Cannot fly and fall at the same time
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_CAN_FLY |
                                               MOVEMENTFLAG_DISABLE_GRAVITY) &&
                               mi->HasMovementFlag(MOVEMENTFLAG_FALLING),
                           MOVEMENTFLAG_FALLING);

    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_SPLINE_ENABLED) &&
                               (!GetPlayer()->movespline->Initialized() ||
                                GetPlayer()->movespline->Finalized()),
...
void WorldSession::ReadAddonsInfo(ByteBuffer& data)
{
    ...
    if (size > 0xFFFFF) {
        LOG_ERROR("network",
                  "WorldSession::ReadAddonsInfo addon info too big, size {}",
                  size);
        return;
    }
    ...
    if (uncompress(addonInfo.contents(),
                   &uSize,
                   data.contents() + pos,
                   data.size() - pos) == Z_OK) {
        uint32 addonsCount;
        ...
        for (uint32 i = 0; i < addonsCount; ++i) {
            ...
            LOG_DEBUG("network",
                      "ADDON: Name: {}, Enabled: 0x{:x}, CRC: 0x{:x}, "
                      "Unknown2: 0x{:x}",
                      addonName,
                      enabled,
                      crc,
            ...
            if (savedAddon) {
                ...
                if (!match)
                    LOG_DEBUG("network",
                              "ADDON: {} was known, but didn't match known CRC "
                              "(0x{:x})!",
                              addon.Name,
                              savedAddon->CRC);
                else
                    LOG_DEBUG("network",
                              "ADDON: {} was known, CRC is correct (0x{:x})",
                              addon.Name,
                              savedAddon->CRC);
            }
            else {
                ...
                LOG_DEBUG("network",
                          "ADDON: {} (0x{:x}) was not known, saving...",
                          addon.Name,
                          addon.CRC);
            }

            /// @todo: Find out when to not use CRC/pubkey, and other possible
            /// states.
...
void WorldSession::SendAddonsInfo()
{
    ...
    for (AddonsList::iterator itr = m_addonsList.begin();
         itr != m_addonsList.end();
         ++itr) {
        ...
        if (crcpub) {
            uint8 usepk =
                (itr->CRC !=
                 STANDARD_ADDON_CRC); // If addon is Standard addon CRC
            data << uint8(usepk);
            if (usepk) // if CRC is wrong, add public key (client need it)
            {
                LOG_DEBUG("network",
                          "ADDON: CRC (0x{:x}) for addon {} is wrong (does not "
                          "match expected 0x{:x}), sending pubkey",
                          itr->CRC,
                          itr->Name,
                          STANDARD_ADDON_CRC);
    ...
    for (AddonMgr::BannedAddonList::const_iterator itr = bannedAddons->begin();
         itr != bannedAddons->end();
         ++itr) {
        data << uint32(itr->Id);
...
TransactionCallback&
WorldSession::AddTransactionCallback(TransactionCallback&& callback)
{
    return _transactionCallbacks.AddCallback(std::move(callback));
}

SQLQueryHolderCallback&
WorldSession::AddQueryHolderCallback(SQLQueryHolderCallback&& callback)
{
...
Warden* WorldSession::GetWarden() { return &(*_warden); }
                                                 ...
                                                 time_t       time) const
{
    ...
    LOG_WARN("network",
             "AntiDOS: Account {}, IP: {}, Ping: {}, Character: {}, flooding "
             "packet (opc: {} (0x{:X}), count: {})",
    ...
    case POLICY_KICK: {
        LOG_INFO(
            "network", "AntiDOS: Player {} kicked!", Session->GetPlayerName());
        Session->KickPlayer();
        return false;
    }
    case POLICY_BAN: {
        ...
        LOG_INFO("network",
                 "AntiDOS: Player automatically banned for {} seconds.",
                 duration);
        return false;
    }
...
uint32
WorldSession::DosProtection::GetMaxPacketCounterAllowed(uint16 opcode) const
{
    ...
    case CMSG_GUILD_SET_PUBLIC_NOTE:  //   1               2         1 async db
                                      //   query
    case CMSG_GUILD_SET_OFFICER_NOTE: //   1               2         1 async db
                                      //   query
    case CMSG_SET_CONTACT_NOTES: //   1               2.5       1 async db query
    case CMSG_CALENDAR_GET_CALENDAR: //   0               1.5       medium
                                     //   upload bandwidth usage
    case CMSG_GUILD_BANK_QUERY_TAB: //   0               3.5       medium upload
                                    //   bandwidth usage
...
void WorldSession::SendTimeSync()
{
    ...
    // Schedule next sync in 10 sec (except for the 2 first packets, which are
    // spaced by only 5s)
...
void WorldSession::InitializeSession()
{
    ...
    AddQueryHolderCallback(CharacterDatabase.DelayQueryHolder(realmHolder))
        .AfterComplete([this, cacheVersion](SQLQueryHolderBase const& holder) {
...
void WorldSession::InitializeSessionCallback(
    CharacterDatabaseQueryHolder const& realmHolder, uint32 clientCacheVersion)
{
    LoadAccountData(realmHolder.GetPreparedResult(
                        AccountInfoQueryHolderPerRealm::GLOBAL_ACCOUNT_DATA),
                    GLOBAL_CACHE_MASK);
    LoadTutorialsData(realmHolder.GetPreparedResult(
        AccountInfoQueryHolderPerRealm::TUTORIALS));
