...
enum Opcodes : uint16 {
    ...
    SMSG_LFG_PLAYER_REWARD =
        0x1FF, // uint32, uint8, uint32, uint32, uint32, uint32, uint32, uint8,
               // for (uint8) {uint32, uint32, uint32}
    ...
    SMSG_CHAT_NOT_IN_PARTY = 0x299, // uint32, errors: ERR_NOT_IN_GROUP (2, 51)
                                    // and ERR_NOT_IN_RAID (3, 39, 40)
    ...
    SMSG_UPDATE_LFG_LIST =
        0x360, // uint32, uint32, if (uint8) { uint32 count, for (count) {
               // uint64} }, uint32 count2, uint32, for (count2) { uint64,
               // uint32 flags, if (flags & 0x2) {string}, if (flags & 0x10)
               // {for (3) uint8}, if (flags & 0x80) {uint64, uint32}}, uint32
               // count3, uint32, for (count3) {uint64, uint32 flags, if (flags
               // & 0x1) {uint8, uint8, uint8, for (3) uint8, uint32, uint32,
               // uint32, uint32, uint32, uint32, float, float, uint32, uint32,
               // uint32, uint32, uint32, float, uint32, uint32, uint32, uint32,
               // uint32, uint32}, if (flags&0x2) string, if (flags&0x4) uint8,
               // if (flags&0x8) uint64, if (flags&0x10) uint8, if (flags&0x20)
               // uint32, if (flags&0x40) uint8, if (flags& 0x80) {uint64,
               // uint32}}
    SMSG_LFG_PROPOSAL_UPDATE =
        0x361, // uint32, uint8, uint32, uint32, uint8, for (uint8) {uint32,
               // uint8, uint8, uint8, uint8}
    CMSG_LFG_PROPOSAL_RESULT = 0x362,
    SMSG_LFG_ROLE_CHECK_UPDATE =
        0x363, // uint32, uint8, for (uint8) uint32, uint8, for (uint8) {
               // uint64, uint8, uint32, uint8, }
    SMSG_LFG_JOIN_RESULT = 0x364, // uint32 unk, uint32, if (unk == 6) { uint8
                                  // count, for (count) uint64 }
    SMSG_LFG_QUEUE_STATUS =
        0x365, // uint32 dungeon, uint32 lfgtype, uint32, uint32, uint32,
               // uint32, uint8, uint8, uint8, uint8
    CMSG_SET_LFG_COMMENT = 0x366,
    SMSG_LFG_UPDATE_PLAYER =
        0x367, // uint8, if (uint8) { uint8, uint8, uint8, uint8, if (uint8) for
               // (uint8) uint32, string}
    SMSG_LFG_UPDATE_PARTY =
        0x368, // uint8, if (uint8) { uint8, uint8, uint8, for (3) uint8, uint8,
               // if (uint8) for (uint8) uint32, string}
    ...
    SMSG_LFG_BOOT_PROPOSAL_UPDATE =
        0x36D, // uint8, uint8, uint8, uint64, uint32, uint32, uint32, uint32
    CMSG_LFD_PLAYER_LOCK_INFO_REQUEST = 0x36E,
    SMSG_LFG_PLAYER_INFO =
        0x36F, // uint8, for (uint8) { uint32, uint8, uint32, uint32, uint32,
               // uint32, uint8, for (uint8) {uint32, uint32, uint32}}, uint32,
               // for (uint32) {uint32, uint32}
    ...
    SMSG_OVERRIDE_LIGHT = 0x412, // uint32 defaultMapLight, uint32
                                 // overrideLight, uint32 transitionTimeMs
    ...
    SMSG_CAMERA_SHAKE =
        0x50A, // uint32 SpellEffectCameraShakes.dbc index, uint32
    SMSG_SOCKET_GEMS_RESULT  = 0x50B,
    CMSG_SET_CHARACTER_MODEL = 0x50C,
    SMSG_REDIRECT_CLIENT     = 0x50D, // uint32 ip, uint16 port, uint32 unk,
                                  // uint8[20] hash (ip + port, seed=sessionkey)
    ...
    SMSG_MULTIPLE_MOVES =
        0x51E, // uncompressed version of SMSG_COMPRESSED_MOVES
    NUM_MSG_TYPES = 0x51F
};
...
enum SessionStatus {
    STATUS_AUTHED =
        0, // Player authenticated (_player == nullptr, m_playerRecentlyLogout =
           // false or will be reset before handler call, m_GUID have garbage)
    STATUS_LOGGEDIN, // Player in game (_player != nullptr, m_GUID ==
                     // _player->GetGUID(), inWorld())
    STATUS_TRANSFER, // Player transferring to another map (_player != nullptr,
                     // m_GUID == _player->GetGUID(), !inWorld())
    STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT, // _player != nullptr or _player ==
                                         // nullptr && m_playerRecentlyLogout &&
                                         // m_playerLogout, m_GUID store last
                                         // _player guid)
    STATUS_NEVER, // Opcode not accepted from client (deprecated or server side
                  // only)
    STATUS_UNHANDLED, // Opcode not handled yet
};

enum PacketProcessing {
    PROCESS_INPLACE = 0, // process packet whenever we receive it - mostly for
                         // non-handled or non-implemented packets
    PROCESS_THREADUNSAFE, // packet is not thread-safe - process it in
...
public:
    ClientOpcodeHandler(char const*      name,
                        SessionStatus    status,
...
private:
    ...
    void ValidateAndSetClientOpcode(OpcodeClient     opcode,
                                    char const*      name,
                                    SessionStatus    status,
                                    PacketProcessing processing);

    void ValidateAndSetServerOpcode(OpcodeServer  opcode,
                                    char const*   name,
                                    SessionStatus status);
