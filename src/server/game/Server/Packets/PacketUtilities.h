...
namespace Strings {
struct RawBytes {
    static bool Validate(std::string const& /*value*/) { return true; }
};
...
class String {
    using ValidatorList = std::conditional_t<
        !Acore::has_type<Strings::RawBytes, std::tuple<Validators...>>::value,
        std::tuple<Strings::ByteSize<MaxBytesWithoutNullTerminator>,
                   Strings::Utf8,
                   Validators...>,
        std::tuple<Strings::ByteSize<MaxBytesWithoutNullTerminator>,
                   Validators...>>;
...
private:
    ...
    {
        return ValidateNth(
            std::make_index_sequence<std::tuple_size_v<ValidatorList>>{});
    }
    ...
    {
        return (
            std::tuple_element_t<indexes, ValidatorList>::Validate(_storage) &&
            ...);
    }
...
public:
    PacketArrayMaxCapacityException(std::size_t requestedSize,
                                    std::size_t sizeLimit);
};

/**
 * Utility class for automated prevention of loop counter spoofing in client
 * packets
 */
...
struct fmt::formatter<
    WorldPackets::String<MaxBytesWithoutNullTerminator, Validators...>>
    : fmt::formatter<std::string_view> {
    auto format(WorldPackets::String<MaxBytesWithoutNullTerminator,
                                     Validators...> const& str,
                format_context&                            ctx) const
    {
        return fmt::formatter<std::string_view>::format(std::string_view(str),
                                                        ctx);
    }
};
