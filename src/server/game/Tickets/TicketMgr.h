...
// from Blizzard LUA:
// GMTICKET_ASSIGNEDTOGM_STATUS_NOT_ASSIGNED      = 0;    -- ticket is not
// currently assigned to a gm GMTICKET_ASSIGNEDTOGM_STATUS_ASSIGNED          =
// 1;    -- ticket is assigned to a normal gm
// GMTICKET_ASSIGNEDTOGM_STATUS_ESCALATED         = 2;    -- ticket is in the
// escalation queue 3 is a custom value and should never actually be sent
...
private:
    ...
    ObjectGuid  _closedBy;  // 0 = Open or Closed by Console (if type = 1),
                            // playerGuid = GM who closed it or player abandoned
                            // ticket or read the GM response message.
    ObjectGuid _resolvedBy; // 0 = Open, -1 = Resolved by Console, GM who
                            // resolved it by closing or completing the ticket.
    ObjectGuid               _assignedTo;
    std::string              _comment;
    bool                     _completed;
    GMTicketEscalationStatus _escalatedStatus;
    bool                     _viewed;
    bool _needResponse; /// @todo: find out the use of this, and then store it
                        /// in DB
    bool        _needMoreHelp;
    std::string _response;
    std::string _chatLog; // No need to store in db, will be refreshed every
                          // session client side
};
...
public:
    ...
    GmTicket* GetTicketByPlayer(ObjectGuid playerGuid)
    {
        for (GmTicketList::const_iterator itr = _ticketList.begin();
             itr != _ticketList.end();
             ++itr)
            if (itr->second && itr->second->IsFromPlayer(playerGuid) &&
                !itr->second->IsClosed())
                return itr->second;
    ...
    GmTicket* GetOldestOpenTicket()
    {
        for (GmTicketList::const_iterator itr = _ticketList.begin();
             itr != _ticketList.end();
             ++itr)
            if (itr->second && !itr->second->IsClosed() &&
                !itr->second->IsCompleted())
                return itr->second;
    ...
    void
    ResolveAndCloseTicket(uint32     ticketId,
                          ObjectGuid source); // used when GM resolves a ticket
...
protected:
    void
    _RemoveTicket(uint32 ticketId, int64 source = -1, bool permanently = false);
