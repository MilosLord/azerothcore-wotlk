...
{
    return float(GameTime::GetGameTime().count() - t) / DAY;
}

///////////////////////////////////////////////////////////////////////////////////////////////////
// GM ticket
GmTicket::GmTicket()
    : _id(0), _type(TICKET_TYPE_OPEN), _posX(0), _posY(0), _posZ(0), _mapId(0),
      _createTime(0), _lastModifiedTime(0), _completed(false),
      _escalatedStatus(TICKET_UNASSIGNED), _viewed(false), _needResponse(false),
      _needMoreHelp(false)
{
}

GmTicket::GmTicket(Player* player)
    : _type(TICKET_TYPE_OPEN), _createTime(GameTime::GetGameTime().count()),
      _lastModifiedTime(GameTime::GetGameTime().count()), _completed(false),
      _escalatedStatus(TICKET_UNASSIGNED), _viewed(false), _needMoreHelp(false)
{
...
bool GmTicket::LoadFromDB(Field* fields)
{
    ...
    //  18         19
    // id, type, playerGuid, name, message, createTime, mapId, posX, posY, posZ,
    // lastModifiedTime, closedBy, assignedTo, comment, response, completed,
    // escalated, viewed, haveTicket, resolvedBy
    uint8 index = 0;
    _id         = fields[index].Get<uint32>();
    _type       = TicketType(fields[++index].Get<uint8>());
    _playerGuid =
        ObjectGuid::Create<HighGuid::Player>(fields[++index].Get<uint32>());
    ...
    _closedBy =
        ObjectGuid::Create<HighGuid::Player>(fields[++index].Get<int32>());
    _assignedTo =
        ObjectGuid::Create<HighGuid::Player>(fields[++index].Get<uint32>());
...
void GmTicket::SaveToDB(CharacterDatabaseTransaction trans) const
{
    ...
    // id, type, playerGuid, name, description, createTime, mapId, posX, posY,
    // posZ, lastModifiedTime, closedBy, assignedTo, comment, response,
    // completed, escalated, viewed, needMoreHelp, resolvedBy
    uint8                               index = 0;
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_REP_GM_TICKET);
...
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_GM_TICKET);
    stmt->SetData(0, _id);
...
void GmTicket::WritePacket(WorldPacket& data) const
{
    ...
    // I am not sure how blizzlike this is, and we don't really have a way to
    // find out
    data << GetAge(sTicketMgr->GetLastChange());

    data << uint8(std::min(_escalatedStatus,
                           TICKET_IN_ESCALATION_QUEUE)); // escalated data
    data << uint8(
        _viewed ? GMTICKET_OPENEDBYGM_STATUS_OPENED
                : GMTICKET_OPENEDBYGM_STATUS_NOT_OPENED); // whether or not it
                                                          // has been viewed
}
                                          ...
                                          bool         detailed) const
{
    ...
    ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTNAME,
                                  _playerName.c_str());
    ss << handler.PGetParseString(
        LANG_COMMAND_TICKETLISTAGECREATE,
        (secsToTimeString(curTime - _createTime, true)).c_str());
    ss << handler.PGetParseString(
        LANG_COMMAND_TICKETLISTAGE,
        (secsToTimeString(curTime - _lastModifiedTime, true)).c_str());

    std::string name;
    if (sCharacterCache->GetCharacterNameByGuid(_assignedTo, name)) {
        ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTASSIGNEDTO,
                                      name.c_str());
    }

    if (detailed) {
        ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTMESSAGE,
                                      _message.c_str());
        if (!_comment.empty())
            ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTCOMMENT,
                                          _comment.c_str());
        if (!_response.empty())
            ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTRESPONSE,
                                          _response.c_str());
    }
    return ss.str();
}
                                          ...
                                          const char*  szDeletedName) const
{
    ...
    if (szAssignedToName)
        ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTASSIGNEDTO,
                                      szAssignedToName);
    if (szUnassignedName)
        ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTUNASSIGNED,
                                      szUnassignedName);
    if (szDeletedName)
        ss << handler.PGetParseString(LANG_COMMAND_TICKETDELETED,
                                      szDeletedName);
    return ss.str();
}
...
void GmTicket::SetGmAction(uint32 needResponse, bool needMoreHelp)
{
    _needResponse =
        (needResponse ==
         17); // Requires GM response. 17 = true, 1 = false (17 is default)
    _needMoreHelp = needMoreHelp; // Requests further GM interaction on a ticket
                                  // to which a GM has already responded.
                                  // Basically means "has a new ticket"
}
...
// Ticket manager
TicketMgr::TicketMgr()
    : _status(true), _lastTicketId(0), _lastSurveyId(0), _openTicketCount(0),
...
TicketMgr::~TicketMgr()
{
    for (GmTicketList::const_iterator itr = _ticketList.begin();
         itr != _ticketList.end();
         ++itr)
        delete itr->second;
}
...
void TicketMgr::ResetTickets()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_ALL_GM_TICKETS);
...
void TicketMgr::LoadTickets()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_GM_TICKETS);
...
void TicketMgr::LoadSurveys()
{
    ...
    if (QueryResult result =
            CharacterDatabase.Query("SELECT MAX(surveyId) FROM gm_survey"))
        _lastSurveyId = (*result)[0].Get<uint32>();

    LOG_INFO("server.loading",
             ">> Loaded GM Survey count from database in {} ms",
             GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
{
    handler.SendSysMessage(onlineOnly ? LANG_COMMAND_TICKETSHOWONLINELIST
        ...
        if (!itr->second->IsClosed() && !itr->second->IsCompleted())
            if (!onlineOnly || itr->second->GetPlayer())
                handler.SendSysMessage(
                    itr->second->FormatMessageString(handler).c_str());
}
...
void TicketMgr::ShowEscalatedList(ChatHandler& handler) const
{
    ...
    for (GmTicketList::const_iterator itr = _ticketList.begin();
         ...
         ++itr)
        if (!itr->second->IsClosed() &&
            itr->second->GetEscalatedStatus() == TICKET_IN_ESCALATION_QUEUE)
            handler.SendSysMessage(
                itr->second->FormatMessageString(handler).c_str());
}
...
{
    WorldPacket data(SMSG_GMTICKET_GETTICKET,
                     (ticket ? (4 + 4 + 1 + 4 + 4 + 4 + 1 + 1) : 4));
