...
#define MAP_LIQUID_STATUS_SWIMMING                                             \
    (LIQUID_MAP_IN_WATER | LIQUID_MAP_UNDER_WATER)
#define MAP_LIQUID_STATUS_IN_CONTACT                                           \
    (MAP_LIQUID_STATUS_SWIMMING | LIQUID_MAP_WATER_WALK)
...
#define INVALID_HEIGHT                                                         \
    -100000.0f // for check, must be equal to VMAP_INVALID_HEIGHT, real value
               // for unknown height is VMAP_INVALID_HEIGHT_VALUE
#define MAX_FALL_DISTANCE                                                      \
    250000.0f // "unlimited fall" to find VMap ground if it is available, just
              // larger than MAX_HEIGHT - INVALID_HEIGHT
#define DEFAULT_HEIGHT_SEARCH                                                  \
    50.0f // default search distance to find height at nearby locations
...
enum LineOfSightChecks {
    ...
    LINEOFSIGHT_CHECK_GOBJECT_WMO =
        0x2, // check dynamic game object data (wmo models)
    LINEOFSIGHT_CHECK_GOBJECT_M2 =
        0x4, // check dynamic game object data (m2 models)

    LINEOFSIGHT_CHECK_GOBJECT_ALL =
        LINEOFSIGHT_CHECK_GOBJECT_WMO | LINEOFSIGHT_CHECK_GOBJECT_M2,

    LINEOFSIGHT_ALL_CHECKS =
        LINEOFSIGHT_CHECK_VMAP | LINEOFSIGHT_CHECK_GOBJECT_ALL
};
...
public:
    ...
    [[nodiscard]] inline float getHeight(float x, float y) const
    {
    ...
    [[nodiscard]] LiquidData const GetLiquidData(float x,
                                                 float y,
                                                 float z,
                                                 float collisionHeight,
                                                 uint8 ReqLiquidType) const;
};
...
typedef std::map<uint32 /*leaderDBGUID*/, CreatureGroup*>
    CreatureGroupHolderType;
typedef std::unordered_map<uint32 /*zoneId*/, ZoneDynamicInfo>
                                   ZoneDynamicInfoMap;
...
public:
    ...
    // function for setting up visibility distance for maps on per-type/per-Id
    // basis
    virtual void InitVisibilityDistance();

    void PlayerRelocation(Player*, float x, float y, float z, float o);
    void
    CreatureRelocation(Creature* creature, float x, float y, float z, float o);
    void
    GameObjectRelocation(GameObject* go, float x, float y, float z, float o);
    void DynamicObjectRelocation(
        DynamicObject* go, float x, float y, float z, float o);
    ...
    [[nodiscard]] std::shared_mutex& GetMMapLock() const
    {
        return *(const_cast<std::shared_mutex*>(&MMapLock));
    }
    // pussywizard:
    ...
    [[nodiscard]] float GetMinHeight(float x, float y) const;
    Transport*          GetTransportForPos(uint32       phase,
                                           float        x,
                                           float        y,
                                           float        z,
                                           WorldObject* worldobject = nullptr);

    void             GetFullTerrainStatusForPosition(uint32 phaseMask,
                                                     float  x,
                                                     float  y,
                                                     float  z,
                                                     float  collisionHeight,
    ...
    LiquidData const GetLiquidData(uint32 phaseMask,
                                   float  x,
                                   float  y,
                                   float  z,
                                   float  collisionHeight,
    ...
    [[nodiscard]] bool GetAreaInfo(uint32  phaseMask,
                                   float   x,
                                   float   y,
                                   float   z,
                                   uint32& mogpflags,
                                   ...
                                   int32&  groupId) const;
    [[nodiscard]] uint32
    GetAreaId(uint32 phaseMask, float x, float y, float z) const;
    [[nodiscard]] uint32
         GetZoneId(uint32 phaseMask, float x, float y, float z) const;
    void GetZoneAndAreaId(uint32  phaseMask,
                          uint32& zoneid,
                          uint32& areaid,
                          float   x,
                          float   y,
    ...
    [[nodiscard]] bool  IsInWater(uint32 phaseMask,
                                  float  x,
                                  float  y,
                                  float  z,
                                  float  collisionHeight) const;
    [[nodiscard]] bool  IsUnderWater(uint32 phaseMask,
                                     float  x,
                                     float  y,
                                     float  z,
                                     float  collisionHeight) const;
    [[nodiscard]] bool  HasEnoughWater(WorldObject const* searcher,
                                       float              x,
                                       float              y,
                                       float              z) const;
    [[nodiscard]] bool  HasEnoughWater(WorldObject const* searcher,
    ...
    enum EnterState {
        ...
        CANNOT_ENTER_UNINSTANCED_DUNGEON, // No instance template was found for
                                          // dungeon map
        CANNOT_ENTER_DIFFICULTY_UNAVAILABLE, // Requested instance difficulty is
                                             // not available for target map
        CANNOT_ENTER_NOT_IN_RAID, // Target instance is a raid instance and the
                                  // player is not in a raid group
        CANNOT_ENTER_CORPSE_IN_DIFFERENT_INSTANCE, // Player is dead and their
                                                   // corpse is not in target
                                                   // instance
        CANNOT_ENTER_INSTANCE_BIND_MISMATCH, // Player's permanent instance save
                                             // is not compatible with their
                                             // group's current instance bind
        CANNOT_ENTER_TOO_MANY_INSTANCES,     // Player has entered too many
                                             // instances recently
        CANNOT_ENTER_MAX_PLAYERS, // Target map already has the maximum number
                                  // of players allowed
        CANNOT_ENTER_ZONE_IN_COMBAT, // A boss encounter is currently in
                                     // progress on the target map
        CANNOT_ENTER_UNSPECIFIED_REASON
    };

    virtual EnterState CannotEnter(Player* /*player*/,
    ...
    [[nodiscard]] bool IsRegularDifficulty() const
    {
    ...
    {
        return IsRaid() ? i_spawnMode >= RAID_DIFFICULTY_10MAN_HEROIC
                        : i_spawnMode >= DUNGEON_DIFFICULTY_HEROIC;
    }
    [[nodiscard]] bool Is25ManRaid() const
    {
    ...
    {
        return marked_cells_large.test(pCellId);
    }
    ...
    void ScriptsStart(
        std::map<uint32, std::multimap<uint32, ScriptInfo>> const& scripts,
        uint32                                                     id,
        Object*                                                    source,
        Object*                                                    target);
    void ScriptCommandStart(ScriptInfo const& script,
                            uint32            delay,
                            Object*           source,
                            Object*           target);
    ...
    GameObject* SummonGameObject(uint32 entry,
                                 float  x,
                                 float  y,
                                 float  z,
                                 float  ang,
    ...
    GameObject* SummonGameObject(uint32          entry,
                                 ...
                                 bool            checkTransport = true);
    void        SummonCreatureGroup(uint8                   group,
    ...
    typedef std::unordered_multimap<ObjectGuid::LowType, Creature*>
    ...
    typedef std::unordered_multimap<ObjectGuid::LowType, GameObject*>
    ...
    [[nodiscard]] std::unordered_set<Corpse*> const*
    ...
    float GetWaterOrGroundLevel(
        ...
        bool   swim            = false,
        float  collisionHeight = DEFAULT_COLLISION_HEIGHT) const;
    [[nodiscard]] float
    GetHeight(uint32 phasemask,
              float  x,
              float  y,
              float  z,
              bool   vmap          = true,
    ...
    bool CanReachPositionAndGetValidCoords(WorldObject const* source,
                                           PathGenerator*     path,
                                           float&             destX,
                                           float&             destY,
                                           float&             destZ,
                                           bool failOnCollision = true,
                                           bool failOnSlopes    = true) const;
    bool CanReachPositionAndGetValidCoords(WorldObject const* source,
                                           float&             destX,
                                           float&             destY,
                                           float&             destZ,
    ...
    bool CanReachPositionAndGetValidCoords(WorldObject const* source,
                                           ...
                                           bool failOnCollision = true,
                                           bool failOnSlopes    = true) const;
    bool CheckCollisionAndGetValidCoords(WorldObject const* source,
                                         float              startX,
                                         float              startY,
                                         float              startZ,
                                         float&             destX,
                                         float&             destY,
                                         float&             destZ,
                                         bool failOnCollision = true) const;
    ...
    [[nodiscard]] bool
    ContainsGameObjectModel(const GameObjectModel& model) const
    {
        return _dynamicTree.contains(model);
    }
    [[nodiscard]] DynamicMapTree const& GetDynamicMapTree() const
    {
        return _dynamicTree;
    }
    bool GetObjectHitPos(uint32 phasemask,
                         float  x1,
                         float  y1,
                         float  z1,
                         float  x2,
                         ...
                         float  modifyDist);
    [[nodiscard]] float
    GetGameObjectFloor(uint32 phasemask,
                       float  x,
                       float  y,
                       float  z,
                       float  maxSearchDist = DEFAULT_HEIGHT_SEARCH) const
    ...
    */
    ...
    [[nodiscard]] time_t
    GetCreatureRespawnTime(ObjectGuid::LowType dbGuid) const
    {
        std::unordered_map<ObjectGuid::LowType /*dbGUID*/,
                           time_t>::const_iterator itr =
            _creatureRespawnTimes.find(dbGuid);
    ...
    void SaveCreatureRespawnTime(ObjectGuid::LowType dbGuid,
                                 time_t&             respawnTime);
    ...
    [[nodiscard]] time_t GetInstanceResetPeriod() const
    {
        return _instanceResetPeriod;
    }

    TaskScheduler _creatureRespawnScheduler;

    void ScheduleCreatureRespawn(ObjectGuid /*creatureGuid*/,
    ...
    Corpse* ConvertCorpseToBones(ObjectGuid const ownerGuid,
                                 bool             insignia = false);
    ...
    void
    SetZoneWeather(uint32 zoneId, WeatherState weatherId, float weatherGrade);
    void SetZoneOverrideLight(uint32       zoneId,
                              uint32       lightId,
                              Milliseconds fadeInTime);

    // Checks encounter state at kill/spellcast, originally in InstanceScript
    // however not every map has instance script :(
    void UpdateEncounterState(EncounterCreditType type,
                              uint32              creditEntry,
                              Unit*               source);
    void LogEncounterFinished(EncounterCreditType type, uint32 creditEntry);

    // Do whatever you want to all the players in map [including GameMasters],
    // i.e.: param exec = [&](Player* p) { p->Whatever(); }
    ...
    [[nodiscard]] TransportsContainer const& GetAllTransports() const
    {
        return _transports;
    }
    ...
    {
        static_assert(ObjectGuidTraits<high>::MapSpecific,
...
private:
    ...
    [[nodiscard]] bool isGridObjectDataLoaded(uint32 x, uint32 y) const
    {
...
private:
    Player*      _GetScriptPlayerSourceOrTarget(Object*           source,
                                                Object*           target,
                                                const ScriptInfo* scriptInfo) const;
    Creature*    _GetScriptCreatureSourceOrTarget(Object*           source,
                                                  Object*           target,
                                                  const ScriptInfo* scriptInfo,
                                                  bool bReverse = false) const;
    Unit*        _GetScriptUnit(Object*           obj,
                                bool              isSource,
                                const ScriptInfo* scriptInfo) const;
    Player*      _GetScriptPlayer(Object*           obj,
                                  bool              isSource,
                                  const ScriptInfo* scriptInfo) const;
    Creature*    _GetScriptCreature(Object*           obj,
                                    bool              isSource,
                                    const ScriptInfo* scriptInfo) const;
    WorldObject* _GetScriptWorldObject(Object*           obj,
                                       bool              isSource,
                                       const ScriptInfo* scriptInfo) const;
    void         _ScriptProcessDoor(Object*           source,
                                    Object*           target,
                                    const ScriptInfo* scriptInfo) const;
    GameObject*  _FindGameObject(WorldObject*        pWorldObject,
    ...
    std::bitset<TOTAL_NUMBER_OF_CELLS_PER_MAP * TOTAL_NUMBER_OF_CELLS_PER_MAP>
        marked_cells;
    std::bitset<TOTAL_NUMBER_OF_CELLS_PER_MAP * TOTAL_NUMBER_OF_CELLS_PER_MAP>
        marked_cells_large;
    ...
    std::unordered_map<ObjectGuid::LowType /*dbGUID*/, time_t>
    ...
    std::unordered_map<uint32 /*cellId*/, std::unordered_set<Corpse*>>
                                            _corpsesByCell;
...
enum InstanceResetMethod {
    INSTANCE_RESET_ALL, // reset all option under portrait, resets only normal
                        // 5-mans
...
public:
    ...
    [[nodiscard]] InstanceScript const* GetInstanceScript() const
    {
...
template <class T, class CONTAINER>
inline void Map::Visit(Cell const&                         cell,
