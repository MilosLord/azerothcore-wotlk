...
public:
    ...
    {
        return pos && (IsWithinBoundaryArea(pos) != _isInvertedBoundary);
    }
    bool IsWithinBoundary(Position const& pos) const
    {
        return IsWithinBoundary(&pos);
    }
...
protected:
    ...
    struct DoublePosition : Position {
        DoublePosition(double x = 0.0,
                       double y = 0.0,
                       double z = 0.0,
                       float  o = 0.0f)
            : Position(float(x), float(y), float(z), o), DoublePosX(x),
              DoublePosY(y), DoublePosZ(z)
        {
        }
        ...
        DoublePosition(Position const& pos)
            : Position(pos), DoublePosX(pos.m_positionX),
              DoublePosY(pos.m_positionY), DoublePosZ(pos.m_positionZ)
        {
        }
...
public:
    ...
    RectangleBoundary(float southX,
                      float northX,
                      float eastY,
                      float westY,
...
public:
    CircleBoundary(Position const& center,
                   double          radius,
                   bool            isInverted = false);
    CircleBoundary(Position const& center,
...
public:
    EllipseBoundary(Position const& center,
                    double          radiusX,
                    double          radiusY,
                    bool            isInverted = false);
...
public:
    TriangleBoundary(Position const& pointA,
...
public:
    ...
    ParallelogramBoundary(Position const& cornerA,
...
public:
    BoundaryUnionBoundary(AreaBoundary const* b1,
