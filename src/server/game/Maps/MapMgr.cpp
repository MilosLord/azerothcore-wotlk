...
Map::EnterState
MapMgr::PlayerCannotEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    // Get the highest available difficulty if current setting is higher than
    // the instance allows
    MapDifficulty const* mapDiff =
        GetDownscaledMapDifficultyData(entry->MapID, targetDifficulty);
    if (!mapDiff) {
        player->SendTransferAborted(
            mapid, TRANSFER_ABORT_DIFFICULTY, requestedDifficulty);
    ...
    char const* mapName =
        entry->name[player->GetSession()->GetSessionDbcLocale()];
    ...
    if (entry->IsRaid()) {
        ...
        if ((!group || !group->isRaidGroup()) &&
            ...
            // probably there must be special opcode, because client has this
            // string constant in GlobalStrings.lua
            /// @todo: this is not a good place to send the message
            player->GetSession()->SendAreaTriggerMessage(
                player->GetSession()->GetAcoreString(
                    LANG_INSTANCE_RAID_GROUP_ONLY),
                mapName);
            LOG_DEBUG("maps",
                      "MAP: Player '{}' must be in a raid group to enter "
                      "instance '{}'",
                      player->GetName(),
                      mapName);
    ...
    if (group)
        if (group->isLFGGroup())
            if (!sLFGMgr->inLfgDungeonMap(
                    group->GetGUID(), mapid, targetDifficulty)) {
    ...
    if (!player->IsAlive()) {
        if (player->HasCorpse()) {
            // let enter in ghost mode in instance that connected to inner
            // instance with corpse
            uint32 corpseMap = player->GetCorpseLocation().GetMapId();
            do {
                if (corpseMap == mapid)
                    break;

                InstanceTemplate const* corpseInstance =
                    sObjectMgr->GetInstanceTemplate(corpseMap);
            ...
            if (!corpseMap) {
                ...
                LOG_DEBUG("maps",
                          "MAP: Player '{}' does not have a corpse in instance "
                          "'{}' and cannot enter.",
                          player->GetName(),
                          mapName);
            ...
            LOG_DEBUG(
                "maps",
                "MAP: Player '{}' has corpse in instance '{}' and can enter.",
                player->GetName(),
                mapName);
        }
        else {
            LOG_DEBUG("maps",
                      "Map::PlayerCannotEnter - player '{}' is dead but does "
                      "not have a corpse!",
                      player->GetName());
        }
    }
    ...
    {
        uint32 destInstId = sInstanceSaveMgr->PlayerGetDestinationInstanceId(
            player, mapid, targetDifficulty);
        if (destInstId)
            if (Map* boundMap = sMapMgr->FindMap(mapid, destInstId))
                if (Map::EnterState denyReason =
                        boundMap->CannotEnter(player, loginCheck))
                    return denyReason;
    }

    // players are only allowed to enter 5 instances per hour
    if (entry->IsDungeon() &&
        (!group || !group->isLFGGroup() || !group->IsLfgRandomInstance())) {
        ...
        // instaceIdToCheck can be 0 if save not found - means no bind so the
        // instance is new
    ...
    return player->Satisfy(
               sObjectMgr->GetAccessRequirement(mapid, targetDifficulty),
               mapid,
               true)
               ? Map::CAN_ENTER
...
void MapMgr::Update(uint32 diff)
{
    ...
    // pussywizard: lfg compatibles update, schedule before maps so it is
    // processed from the very beginning
    // if (mapUpdateStep == 0)
    {
    ...
    for (; iter != i_maps.end(); ++iter) {
        bool full =
            mapUpdateStep < 3 &&
            ((mapUpdateStep == 0 && !iter->second->IsBattlegroundOrArena() &&
              !iter->second->IsDungeon()) ||
             (mapUpdateStep == 1 && iter->second->IsBattlegroundOrArena()) ||
             (mapUpdateStep == 2 && iter->second->IsDungeon()));
        if (m_updater.activated())
            m_updater.schedule_update(
                *iter->second,
                uint32(full ? i_timer[mapUpdateStep].GetCurrent() : 0),
                diff);
        else
            iter->second->Update(
                uint32(full ? i_timer[mapUpdateStep].GetCurrent() : 0), diff);
    }
    ...
    if (mapUpdateStep < 3) {
        for (iter = i_maps.begin(); iter != i_maps.end(); ++iter) {
            bool full = ((mapUpdateStep == 0 &&
                          !iter->second->IsBattlegroundOrArena() &&
                          !iter->second->IsDungeon()) ||
                         (mapUpdateStep == 1 &&
                          iter->second->IsBattlegroundOrArena()) ||
                         (mapUpdateStep == 2 && iter->second->IsDungeon()));
            if (full)
                iter->second->DelayedUpdate(
                    uint32(i_timer[mapUpdateStep].GetCurrent()));
        }
...
bool MapMgr::IsValidMAP(uint32 mapid, bool startUp)
{
    ...
    if (startUp) {
        ...
        return mEntry &&
               (!mEntry->IsDungeon() || sObjectMgr->GetInstanceTemplate(mapid));
    }
...
void MapMgr::InitInstanceIds()
{
    ...
    QueryResult result =
        CharacterDatabase.Query("SELECT MAX(id) FROM instance");
    if (result) {
...
void MapMgr::RegisterInstanceId(uint32 instanceId)
{
    ...
    // Instances are pulled in ascending order from db and _nextInstanceId is
    // initialized with 1, so if the instance id is used, increment
...
uint32 MapMgr::GenerateInstanceId()
{
    ...
    while (_nextInstanceId < 0xFFFFFFFF &&
           ++_nextInstanceId < _instanceIds.size() &&
           _instanceIds[_nextInstanceId])
        ;
