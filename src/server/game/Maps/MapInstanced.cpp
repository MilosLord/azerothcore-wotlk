...
void MapInstanced::Update(const uint32 t, const uint32 s_diff, bool /*thread*/)
{
    ...
    while (i != m_InstancedMaps.end()) {
        if (i->second->CanUnload(t)) {
            ...
            // update only here, because it may schedule some bad things before
            // delete
            if (sMapMgr->GetMapUpdater()->activated())
                sMapMgr->GetMapUpdater()->schedule_update(
                    *i->second, t, s_diff);
            else
...
{
    for (InstancedMaps::iterator i = m_InstancedMaps.begin(); i !=
m_InstancedMaps.end(); ++i) i->second->RelocationNotify();
}
*/
...
void MapInstanced::UnloadAll()
{
    ...
    // Unload own grids (just dummy(placeholder) grids, neccesary to unload
    // GridMaps!)
    Map::UnloadAll();
}
...
Map* MapInstanced::CreateInstanceForPlayer(const uint32 mapId, Player* player)
{
    ...
    if (IsBattlegroundOrArena()) {
        ...
        if (destInstId) {
            ...
            else if (IsSharedDifficultyMap(mapId) && !map->HavePlayers() &&
                     map->GetDifficulty() != realdiff) {
                if (player
                        ->isBeingLoaded()) // pussywizard: crashfix
                                           // (assert(passengers.empty) fail in
                                           // ~transport), could be added to a
                                           // transport during loading from db
                    return nullptr;

                if (!map->AllTransportsEmpty())
                    map->AllTransportsRemovePassengers(); // pussywizard:
                                                          // gameobjects /
                                                          // summons
            ...
            ASSERT(!FindInstanceMap(newInstanceId)); // pussywizard: instance
                                                     // with new id can't exist
                                          ...
                                          Difficulty    difficulty)
{
    ...
    if (!iTemplate) {
        LOG_ERROR(
            "maps", "CreateInstance: no instance template for map {}", GetId());
        ABORT();
    }
    ...
    LOG_DEBUG("maps",
              "MapInstanced::CreateInstance: {} map instance {} for {} created "
              "with difficulty {}",
              save ? "" : "new ",
              InstanceId,
              GetId(),
    ...
    if (save)
        map->CreateInstanceScript(
            true, save->GetInstanceData(), save->GetCompletedEncounterMask());
    else
        map->CreateInstanceScript(false, "", 0);

    if (!save) // this is for sure a dungeon (assert above), no need to check
               // here
                                                  ...
                                                  Battleground* bg)
{
    ...
    LOG_DEBUG("maps",
              "MapInstanced::CreateBattleground: map bg {} for {} created.",
              InstanceId,
              GetId());

    PvPDifficultyEntry const* bracketEntry =
        GetBattlegroundBracketByLevel(bg->GetMapId(), bg->GetMinLevel());
    ...
    BattlegroundMap* map =
        new BattlegroundMap(GetId(), InstanceId, this, spawnMode);
