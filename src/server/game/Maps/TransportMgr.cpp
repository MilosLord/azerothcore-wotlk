...
TransportTemplate::~TransportTemplate()
{
    ...
    for (std::set<TransportSpline*>::iterator itr = splines.begin();
         itr != splines.end();
         ++itr)
        delete *itr;
}
...
void TransportMgr::Unload() { _transportTemplates.clear(); }
...
void TransportMgr::LoadTransportTemplates()
{
    ...
    QueryResult result =
        WorldDatabase.Query("SELECT entry FROM gameobject_template WHERE type "
                            "= 15 ORDER BY entry ASC");

    if (!result) {
        LOG_WARN("server.loading",
                 ">> Loaded 0 transport templates. DB table "
                 "`gameobject_template` has no transports!");
        return;
    }
    ...
    do {
        ...
        if (!goInfo) {
            LOG_ERROR("entities.transport",
                      "Transport {} has no associated GameObjectTemplate from "
        ...
        if (goInfo->moTransport.taxiPathId >= sTaxiPathNodesByPath.size()) {
            LOG_ERROR("entities.transport",
                      "Transport {} (name: {}) has an invalid path specified "
                      "in `gameobject_template`.`data0` ({}) field, skipped.",
                      entry,
                      goInfo->name,
        ...
        // paths are generated per template, saves us from generating it again
        // in case of instanced transports
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} Transport Templates in {} ms",
             count,
                                ...
                                TransportTemplate*        transport)
{
    ...
    allPoints.insert(allPoints.begin(),
                     allPoints.front().lerp(allPoints[1], -0.2f));
    allPoints.push_back(
    ...
    for (size_t i = 0; i < path.size(); ++i) {
        if (!mapChange) {
            ...
            if (i != path.size() - 1 && (node_i->actionFlag & 1 ||
                                         node_i->mapid != path[i + 1]->mapid)) {
                ...
                k.InitialOrientation =
                    Position::NormalizeOrientation(std::atan2(h.y, h.x) + M_PI);
    ...
    if (transport->mapsUsed.size() > 1) {
        for (std::set<uint32>::const_iterator itr = transport->mapsUsed.begin();
             itr != transport->mapsUsed.end();
             ++itr)
    ...
    for (size_t i = 1; i < keyFrames.size(); ++i) {
        if (keyFrames[i - 1].Teleport || i + 1 == keyFrames.size()) {
            ...
            spline->init_spline(&splinePath[start],
                                i - start + extra,
            ...
            for (size_t j = start; j < i + extra; ++j) {
                ...
                keyFrames[j].DistFromPrev =
                    spline->length(j - start, j + 1 - start);
                if (j > 0)
    ...
    for (size_t i = 0; i < keyFrames.size(); ++i) {
        float total_dist =
            keyFrames[i].DistSinceStop + keyFrames[i].DistUntilStop;
        if (total_dist < 2 * accel_dist) // won't reach full speed
        {
            if (keyFrames[i].DistSinceStop <
                keyFrames[i].DistUntilStop) // is still accelerating
            {
                // calculate accel+brake time for this short segment
                float segment_time =
                    2.0f * std::sqrt((keyFrames[i].DistUntilStop +
                                      keyFrames[i].DistSinceStop) /
                                     accel);
                // substract acceleration time
                keyFrames[i].TimeTo =
                    segment_time -
                    std::sqrt(2 * keyFrames[i].DistSinceStop / accel);
            }
            else // slowing down
                keyFrames[i].TimeTo =
                    std::sqrt(2 * keyFrames[i].DistUntilStop / accel);
        }
        else if (keyFrames[i].DistSinceStop <
                 accel_dist) // still accelerating (but will reach full speed)
        {
            // calculate accel + cruise + brake time for this long segment
            float segment_time =
                (keyFrames[i].DistUntilStop + keyFrames[i].DistSinceStop) /
                    speed +
                (speed / accel);
            // substract acceleration time
            keyFrames[i].TimeTo =
        ...
        else if (keyFrames[i].DistUntilStop <
                 accel_dist) // already slowing down (but reached full speed)
            keyFrames[i].TimeTo =
                std::sqrt(2 * keyFrames[i].DistUntilStop / accel);
        else // at full speed
            keyFrames[i].TimeTo =
                (keyFrames[i].DistUntilStop / speed) + (0.5f * speed / accel);
    }
                                               ...
                                               Map* map /*= nullptr*/)
{
    ...
    if (map) {
        // SetZoneScript() is called after adding to map, so fetch the script
        // using map
        if (map->IsDungeon())
            if (InstanceScript* instance =
                    static_cast<InstanceMap*>(map)->GetInstanceScript())
    ...
    if (!tInfo) {
        LOG_ERROR(
            "entities.transport",
            "Transport {} will not be loaded, `transport_template` missing",
            entry);
        return nullptr;
    }
    ...
    ObjectGuid::LowType guidLow =
        guid ? guid
             : sObjectMgr->GetGenerator<HighGuid::Mo_Transport>().Generate();
    ...
    if (MapEntry const* mapEntry = sMapStore.LookupEntry(mapId)) {
        if (mapEntry->Instanceable() != tInfo->inInstance) {
            LOG_ERROR("entities.transport",
                      "Transport {} (name: {}) attempted creation in instance "
                      "map (id: {}) but it is not an instanced transport!",
                      entry,
                      trans->GetName(),
                      mapId);
            delete trans;
    ...
    // xinef: transports are active so passengers can be relocated (grids must
    // be loaded)
...
void TransportMgr::SpawnContinentTransports()
{
    ...
    uint32      oldMSTime = getMSTime();
    QueryResult result =
        WorldDatabase.Query("SELECT guid, entry FROM transports");
    ...
    if (sWorld->getBoolConfig(CONFIG_ENABLE_CONTINENT_TRANSPORT)) {
        ...
        if (result) {
            do {
                ...
                if (TransportTemplate const* tInfo =
                        GetTransportTemplate(entry))
                    if (!tInfo->inInstance)
        ...
        LOG_INFO("server.loading",
                 ">> Spawned {} continent motion transports in {} ms",
                 count,
                 GetMSTimeDiffToNow(oldMSTime));
    }
    ...
    if (sWorld->getBoolConfig(CONFIG_ENABLE_CONTINENT_TRANSPORT_PRELOADING)) {
        ...
        QueryResult result2 = WorldDatabase.Query(
            "SELECT map, position_x, position_y FROM gameobject g JOIN "
            "gameobject_template t ON g.id = t.entry WHERE t.type = 11");
        ...
        LOG_INFO(
            "server.loading",
            ">> Preloaded grids for {} continent static transports in {} ms",
            count,
            GetMSTimeDiffToNow(oldMSTime));
    }
}
...
{
    TransportInstanceMap::const_iterator mapTransports =
        _instanceTransports.find(map->GetId());

    // no transports here
    if (mapTransports == _instanceTransports.end() ||
        mapTransports->second.empty())
        return;

    // create transports
    for (std::set<uint32>::const_iterator itr = mapTransports->second.begin();
         itr != mapTransports->second.end();
         ++itr)
                                     ...
                                     float& percPos) const
{
    ...
    for (TransportPathContainer::const_reverse_iterator itr = Path.rbegin();
         itr != Path.rend();
         ++itr)
        if (time >= itr->first) {
            curr = itr->second;
            ...
            --itr;
            next    = itr->second;
            percPos = float(time - curr->TimeSeg) /
                      float(next->TimeSeg - curr->TimeSeg);
            return true;
        }
                                         ...
                                         float&     percRot) const
{
    ...
    for (TransportPathRotationContainer::const_reverse_iterator itr =
             Rotations.rbegin();
         itr != Rotations.rend();
         ++itr)
        if (time >= itr->first) {
            ...
            curr           = G3D::Quat(
                itr->second->X, itr->second->Y, itr->second->Z, itr->second->W);
            if (itr != Rotations.rbegin()) {
                --itr;
                next    = G3D::Quat(itr->second->X,
                                 itr->second->Y,
                                 itr->second->Z,
                                 itr->second->W);
                ...
                next    = G3D::Quat(Rotations.begin()->second->X,
                                 Rotations.begin()->second->Y,
                                 Rotations.begin()->second->Z,
                                 Rotations.begin()->second->W);
