...
struct AC_DATABASE_API UpdateResult {
    ...
    UpdateResult(size_t const updated_,
...
public:
    ...
    UpdateResult Update(bool const  redundancyChecks,
...
private:
    ...
    enum State { RELEASED, CUSTOM, MODULE, ARCHIVED };

    struct AppliedFileEntry {
        AppliedFileEntry(std::string const& name_,
        ...
        std::string GetStateAsString() const { return StateConvert(state); }
    };
    ...
    struct PathCompare {
        bool operator()(LocaleFileEntry const& left,
    ...
    typedef std::unordered_map<std::string, AppliedFileEntry>
                                                       AppliedFileStorage;
    typedef std::vector<UpdateFetcher::DirectoryEntry> DirectoryStorage;

    LocaleFileStorage GetFileList() const;
    void              FillFileListRecursively(Path const&        path,
    ...
    void UpdateEntry(AppliedFileEntry const& entry,
                     uint32 const            speed = 0) const;
