...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!is_regular_file(exe)) {
        ...
        LOG_FATAL("sql.updates",
                  "Didn't find any executable MySQL binary at \'{}\' or in "
                  "path, correct the path in the *.conf (\"MySQLExecutable\").",
...
bool DBUpdater<T>::Create(DatabaseWorkerPool<T>& pool)
{
    ...
    if (!sConfigMgr->isDryRun() &&
        (disableInteractive == nullptr ||
         std::strcmp(disableInteractive, "1") != 0)) {
        std::cout << "Do you want to create it? [yes (default) / no]:"
                  << std::endl;
        std::string answer;
    ...
    if (!file.is_open()) {
        LOG_FATAL("sql.updates",
                  "Failed to create temporary query file \"{}\"!",
                  temp.generic_string());
        return false;
    }
    ...
    try {
        DBUpdater<T>::ApplyFile(pool,
                                pool.GetConnectionInfo()->host,
                                pool.GetConnectionInfo()->user,
                                pool.GetConnectionInfo()->password,
                                pool.GetConnectionInfo()->port_or_socket,
                                "",
                                pool.GetConnectionInfo()->ssl,
                                temp);
    }
    catch (UpdateException&) {
        LOG_FATAL("sql.updates",
                  "Failed to create database {}! Does the user (named in "
                  "*.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and "
                  "`DELETE` privileges on the MySQL server?",
                  pool.GetConnectionInfo()->database);
                          ...
                          std::string_view       modulesList /*= {}*/)
{
    ...
    LOG_INFO(
        "sql.updates", "Updating {} database...", DBUpdater<T>::GetTableName());
    ...
    if (!is_directory(sourceDirectory)) {
        LOG_ERROR(
            "sql.updates",
            "DBUpdater: The given source directory {} does not exist, change "
            "the path to the directory where your sql directory exists (for "
            "example c:\\source\\azerothcore). Shutting down.",
            sourceDirectory.generic_string());
        return false;
    }

    auto CheckUpdateTable = [&](std::string const& tableName) {
        auto checkTable = DBUpdater<T>::Retrieve(
            pool, Acore::StringFormatFmt("SHOW TABLES LIKE '{}'", tableName));
        if (!checkTable) {
            LOG_WARN("sql.updates",
                     "> Table '{}' not exist! Try add based table",
                     tableName);
            ...
            try {
                ...
                LOG_FATAL("sql.updates",
                          "Failed apply file to database {}! Does the user "
                          "(named in *.conf) have `INSERT` and `DELETE` "
    ...
    std::string const info =
        Acore::StringFormatFmt("Containing {} new and {} archived updates.",
                               result.recent,
                               result.archived);
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    LOG_INFO("sql.updates",
             "Database {} is empty, auto populating it...",
             DBUpdater<T>::GetTableName());
    ...
    for (std::filesystem::directory_iterator itr(DirPath); itr != DirItr;
         ++itr) {
    ...
    if (!FilesCount) {
        LOG_ERROR("sql.updates",
                  ">> In directory \"{}\" not exist '*.sql' files",
                  DirPath.generic_string());
        return false;
    }
...
void DBUpdater<T>::ApplyFile(DatabaseWorkerPool<T>& pool,
                             ...
                             Path const&            path)
{
    std::string configTempDir =
        sConfigMgr->GetOption<std::string>("TempDir", "");

    auto tempDir = configTempDir.empty()
                       ? std::filesystem::temp_directory_path().string()
                       : configTempDir;

    tempDir = Acore::String::AddSuffixIfNotExists(
        tempDir, std::filesystem::path::preferred_separator);
    ...
    if (!std::isdigit(port_or_socket[0])) {
        // We can't check if host == "." here, because it is named localhost if
    ...
    // Execute sql file
    args.emplace_back("-e");
    args.emplace_back(Acore::StringFormat("BEGIN; SOURCE %s; COMMIT;",
    ...
    int const ret =
        Acore::StartProcess(DBUpdaterUtil::GetCorrectedMySQLExecutable(),
                            args,
                            "sql.updates",
                            "",
                            true);
    ...
    if (ret != EXIT_SUCCESS) {
        LOG_FATAL("sql.updates",
                  ...
                  " If you are a user, please pull the latest revision from "
                  "the repository. "
                  "Also make sure you have not applied any of the databases "
                  "with your sql client. "
                  "You cannot use auto-update system and import sql files from "
