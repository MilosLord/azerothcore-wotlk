...
UpdateFetcher::UpdateFetcher(
    ...
    : _sourceDirectory(std::make_unique<Path>(sourceDirectory)), _apply(apply),
      _applyFile(applyFile), _retrieve(retrieve), _dbModuleName(dbModuleName),
      _setDirectories(setDirectories)
{
}
...
UpdateFetcher::UpdateFetcher(
    ...
    : _sourceDirectory(std::make_unique<Path>(sourceDirectory)), _apply(apply),
      _applyFile(applyFile), _retrieve(retrieve), _dbModuleName(dbModuleName),
      _setDirectories(nullptr), _modulesList(modulesList)
{
}
                                            ...
                                            uint32 const       depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr) {
        if (is_directory(itr->path())) {
            ...
            LOG_TRACE("sql.updates",
                      "Added locale file \"{}\" state '{}'.",
                      itr->path().filename().generic_string(),
                      AppliedFileEntry::StateConvert(state));
            ...
            if (storage.find(entry) != storage.end()) {
                LOG_FATAL("sql.updates",
                          "Duplicate filename \"{}\" occurred. Because updates "
                          "are ordered "
                          "by their filenames, every name needs to be unique!",
                          itr->path().generic_string());

                throw UpdateException(
                    "Updating failed, see the log for details.");
            }
...
UpdateFetcher::DirectoryStorage
UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    if (_setDirectories) {
        for (auto const& itr : *_setDirectories) {
            ...
            DirectoryEntry const entry = {
                p, AppliedFileEntry::StateConvert("MODULE")};
            directories.push_back(entry);

            LOG_TRACE("sql.updates",
                      "Added applied extra file \"{}\" from remote.",
                      p.filename().generic_string());
        }
    }
    else {
        QueryResult const result =
            _retrieve("SELECT `path`, `state` FROM `updates_include`");
        if (!result)
            return directories;
        ...
        do {
            ...
            if (!is_directory(p)) {
                LOG_WARN("sql.updates",
                         "DBUpdater: Given update include directory \"{}\" "
            ...
            DirectoryEntry const entry = {
                p, AppliedFileEntry::StateConvert(state)};
            directories.push_back(entry);

            LOG_TRACE("sql.updates",
                      "Added applied file \"{}\" '{}' state from remote.",
                      p.filename().generic_string(),
                      state);
        ...
        for (auto const& itr : moduleList) {
            std::string path =
                _sourceDirectory->generic_string() + "/modules/" + itr +
                "/data/sql/" +
                _dbModuleName; // modules/mod-name/data/sql/db-world
            ...
            LOG_TRACE("sql.updates",
                      "Added applied modules file \"{}\" from remote.",
                      p.filename().generic_string());
        }
    }
...
UpdateFetcher::AppliedFileStorage UpdateFetcher::ReceiveAppliedFiles() const
{
    ...
    QueryResult result =
        _retrieve("SELECT `name`, `hash`, `state`, UNIX_TIMESTAMP(`timestamp`) "
                  "FROM `updates` ORDER BY `name` ASC");
    if (!result)
        return map;
    ...
    do {
        ...
        AppliedFileEntry const entry = {
            ...
            AppliedFileEntry::StateConvert(fields[2].Get<std::string>()),
            fields[3].Get<uint64>()};
...
std::string UpdateFetcher::ReadSQLUpdate(Path const& file) const
{
    ...
    if (!in.is_open()) {
        LOG_FATAL("sql.updates",
                  ...
                  "try to identify and solve the issue or disable the database "
                  "updater.",
                  file.generic_string());
...
UpdateResult
UpdateFetcher::Update(bool const  redundancyChecks,
                      ...
                      int32 const cleanDeadReferencesMaxCount) const
{
    ...
    auto ApplyUpdateFile = [&](LocaleFileEntry const& sqlFile) {
        ...
        AppliedFileStorage::const_iterator iter =
            applied.find(filePath.filename().string());
        if (iter != applied.end()) {
            // If redundancy is disabled, skip it, because the update is already
            // applied.
            if (!redundancyChecks) {
                LOG_DEBUG("sql.updates",
                          ">> Update is already applied, skipping redundancy "
                          "checks.");
                applied.erase(iter);
                return;
            }

            // If the update is in an archived directory and is marked as
            // archived in our database, skip redundancy checks (archived
            // updates never change).
            if (!archivedRedundancy && (iter->second.state == ARCHIVED) &&
                (sqlFile.second == ARCHIVED)) {
                LOG_DEBUG("sql.updates",
                          ">> Update is archived and marked as archived in "
                          "database, skipping redundancy checks.");
                applied.erase(iter);
                return;
            }
        }

        std::string const hash = ByteArrayToHexStr(
            Acore::Crypto::SHA1::GetDigestOf(ReadSQLUpdate(filePath)));
        ...
        if (iter == applied.end()) {
            ...
            if (hashIter != hashToName.end()) {
                // Check if the original file was removed. If not, we've got a
                // problem.
                LocaleFileStorage::const_iterator localeIter;

                // Push localeIter forward
                for (localeIter = available.begin();
                     (localeIter != available.end()) &&
                ...
                if (localeIter != available.end()) {
                    LOG_WARN("sql.updates",
                             ">> It seems like the update \"{}\" \'{}\' was "
                             "renamed, but the old file is still there! "
                             "Treating it as a new file! (It is probably an "
        ...
        // Rehash the update entry if it exists in our database with an empty
        // hash.
            ...
            // If the hash of the files differs from the one stored in our
            // database, reapply the update (because it changed).
            if (iter->second.hash != hash) {
                LOG_INFO("sql.updates",
                         ">> Reapplying update \"{}\" \'{}\' -> \'{}\' (it "
                         "changed)...",
                         filePath.filename().string(),
                         iter->second.hash.substr(0, 7),
                         hash.substr(0, 7));
            }
            else {
                // If the file wasn't changed and just moved, update its state
                // (if necessary).
                if (iter->second.state != fileState) {
                    LOG_DEBUG("sql.updates",
                              ">> Updating the state of \"{}\" to \'{}\'...",
                              filePath.filename().string(),
                              AppliedFileEntry::StateConvert(fileState));
                ...
                LOG_DEBUG(
                    "sql.updates",
                    ">> Update is already applied and matches the hash \'{}\'.",
                    hash.substr(0, 7));
        ...
        uint32                 speed = 0;
        AppliedFileEntry const file  = {
            filePath.filename().string(), hash, fileState, 0};
    ...
    if (!applied.empty() && !_setDirectories) {
        bool const doCleanup =
            (cleanDeadReferencesMaxCount < 0) ||
            (applied.size() <=
        ...
        for (auto const& entry : applied) {
            if (entry.second.state != MODULE) {
                LOG_WARN("sql.updates",
                         ">> The file \'{}\' was applied to the database, but "
                         "is missing in"
        ...
        if (!toCleanup.empty()) {
            if (doCleanup)
                ...
                LOG_ERROR("sql.updates",
                          "Cleanup is disabled! There were {} dirty files "
    ...
    return UpdateResult(
        importedUpdates, countRecentUpdates, countArchivedUpdates);
}
                                ...
                                std::string const& to) const
{
    ...
    // Rename
    {
        std::string const update = "UPDATE `updates` SET `name`=\"" + to +
                                   "\" WHERE `name`=\"" + from + "\"";
...
{
    std::string const update = "UPDATE `updates` SET `state`=\'" +
                               AppliedFileEntry::StateConvert(state) +
                               "\' WHERE `name`=\"" + name + "\"";
...
bool UpdateFetcher::PathCompare::operator()(LocaleFileEntry const& left,
                                            LocaleFileEntry const& right) const
{
