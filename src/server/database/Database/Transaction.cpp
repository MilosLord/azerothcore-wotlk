...
{
    // This might be called by explicit calls to Cleanup or by the
    // auto-destructor
    if (_cleanedUp)
        return;
        ...
        case SQL_ELEMENT_PREPARED: {
            try {
                PreparedStatementBase* stmt =
                    std::get<PreparedStatementBase*>(data.element);
                ASSERT(stmt);
...
bool TransactionTask::Execute()
{
    ...
    if (errorCode == ER_LOCK_DEADLOCK) {
...

        {
            ...
            // keep dead-locking each other
            std::lock_guard<std::mutex> lock(_deadlockLock);

            for (Milliseconds loopDuration{}, startMSTime = GetTimeMS();
                 loopDuration <= DEADLOCK_MAX_RETRY_TIME_MS;
                 loopDuration = GetMSTimeDiffToNow(startMSTime)) {
                if (!TryExecute())
                    return true;

                LOG_WARN("sql.sql",
                         "Deadlocked SQL Transaction, retrying. Loop timer: {} "
                         "ms, Thread Id: {}",
                         loopDuration.count(),
                         threadId);
            }
        ...
        LOG_ERROR("sql.sql",
                  "Fatal deadlocked SQL Transaction, it will not be retried "
                  "anymore. Thread Id: {}",
                  threadId);
    }
...
{
    if (m_future.valid() &&
        m_future.wait_for(0s) == std::future_status::ready) {
