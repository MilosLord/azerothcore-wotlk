...
ResultSet::ResultSet(MySQLResult* result,
                     ...
                     uint64       rowCount,
                     uint32       fieldCount)
    : _rowCount(rowCount), _fieldCount(fieldCount), _result(result),
      _fields(fields)
{
...
PreparedResultSet::PreparedResultSet(MySQLStmt*   stmt,
                                     ...
                                     uint64       rowCount,
                                     uint32       fieldCount)
    : m_rowCount(rowCount), m_rowPosition(0), m_fieldCount(fieldCount),
      m_rBind(nullptr), m_stmt(stmt), m_metadataResult(result)
{
    if (!m_metadataResult)
        return;
    ...
    //- for future readers wondering where this is freed -
    //mysql_stmt_bind_result moves pointers to these
    // from m_rBind to m_stmt->bind and it is later freed by the `if
    // (m_stmt->bind_result_done)` block just above here MYSQL_STMT lifetime is
    // equal to connection lifetime
    ...
    if (mysql_stmt_store_result(m_stmt)) {
        LOG_WARN("sql.sql",
                 "{}:mysql_stmt_store_result, cannot bind result from MySQL "
                 "server. Error: {}",
                 __FUNCTION__,
    ...
    MySQLField* field =
        reinterpret_cast<MySQLField*>(mysql_fetch_fields(m_metadataResult));
    ...
    if (mysql_stmt_bind_result(m_stmt, m_rBind)) {
        LOG_WARN("sql.sql",
                 "{}:mysql_stmt_bind_result, cannot bind result from MySQL "
                 "server. Error: {}",
                 __FUNCTION__,
                 mysql_stmt_error(m_stmt));
    ...
    while (_NextRow()) {
        for (uint32 fIndex = 0; fIndex < m_fieldCount; ++fIndex) {
            m_rows[uint32(m_rowPosition) * m_fieldCount + fIndex].SetMetadata(
                &m_fieldMetadata[fIndex]);
            ...
            if (!*m_rBind[fIndex].is_null) {
                ...
                case MYSQL_TYPE_STRING:
                case MYSQL_TYPE_VAR_STRING:
                    // warning - the string will not be null-terminated if there
                    // is no space for it in the buffer when mysql_stmt_fetch
                    // returned MYSQL_DATA_TRUNCATED we cannot blindly
                    // null-terminate the data either as it may be retrieved as
                    // binary blob and not specifically a string in this case
                    // using Field::GetCString will result in garbage
                    /// @todo: remove Field::GetCString and use std::string_view
                    /// in C++17
                ...
                m_rows[uint32(m_rowPosition) * m_fieldCount + fIndex]
                    .SetByteValue((char const*)buffer, fetched_length);
                ...
                m_rows[uint32(m_rowPosition) * m_fieldCount + fIndex]
                    .SetByteValue(nullptr, *m_rBind[fIndex].length);
            }
        }
...
PreparedResultSet::~PreparedResultSet() { CleanUp(); }

bool PreparedResultSet::NextRow()
{
    /// Only updates the m_rowPosition so upper level code knows in which
    /// element of the rows vector to look
