...
DatabaseWorkerPool<T>::DatabaseWorkerPool()
    : _queue(new ProducerConsumerQueue<SQLOperation*>()), _async_threads(0),
      _synch_threads(0)
{
    ...
    WPFatal(isSupportClientDB,
            "AzerothCore does not support MySQL versions below 5.7 or MariaDB "
            "versions below 10.5.\n\nFound version: {} / {}. Server compiled "
            "with: {}.\nSearch the wiki for ACE00043 in Common Errors "
            "(https://www.azerothcore.org/wiki/common-errors#ace00043).",
            mysql_get_client_info(),
            mysql_get_client_version(),
            MYSQL_VERSION_ID);
    WPFatal(isSameClientDB,
            "Used MySQL library version ({} id {}) does not match the version "
            "id used to compile AzerothCore (id {}).\nSearch the wiki for "
            "ACE00046 in Common Errors "
...
uint32 DatabaseWorkerPool<T>::Open()
{
    ...
    LOG_INFO("sql.driver",
             "Opening DatabasePool '{}'. Asynchronous connections: {}, "
    ...
    if (!error) {
        LOG_INFO(
            "sql.driver",
            "DatabasePool '{}' opened successfully. {} total connections "
            "running.",
            GetDatabaseName(),
...
{
    LOG_INFO(
        "sql.driver", "Closing down DatabasePool '{}'.", GetDatabaseName());
    ...
    //! There's no need for locking the connection, because
    //! DatabaseWorkerPool<>::Close should only be called after any other thread
    //! tasks in the core have exited, meaning there can be no concurrent access
    //! at this point.
    ...
    for (auto const& connections : _connections) {
        for (auto const& connection : connections) {
            ...
            for (size_t i = 0; i < preparedSize; ++i) {
                ...
                // (each connection only has prepared statements of it's own
                // type sync/async)
                if (_preparedStatementSize[i] > 0)
                    continue;

                if (MySQLPreparedStatement* stmt =
                        connection->m_stmts[i].get()) {
...
QueryCallback DatabaseWorkerPool<T>::AsyncQuery(std::string_view sql)
{
    ...
    // Store future result before enqueueing - task might get already processed
    // and deleted before returning from this method
...
template <class T>
SQLQueryHolderCallback DatabaseWorkerPool<T>::DelayQueryHolder(
    std::shared_ptr<SQLQueryHolder<T>> holder)
{
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction<T> transaction)
{
    ...
    case 1:
        LOG_DEBUG("sql.driver",
                  "Warning: Transaction only holds 1 query, consider removing "
                  "Transaction context in code.");
        break;
    default:
        break;
    }
...
template <class T>
TransactionCallback
DatabaseWorkerPool<T>::AsyncCommitTransaction(SQLTransaction<T> transaction)
{
#ifdef ACORE_DEBUG
    //! Only analyze transaction weaknesses in Debug mode.
    //! Ideally we catch the faults in Debug mode and then correct them,
    ...
    TransactionWithResultTask* task =
        new TransactionWithResultTask(transaction);
...
template <class T>
void DatabaseWorkerPool<T>::DirectCommitTransaction(
    SQLTransaction<T>& transaction)
{
    ...
    if (errorCode == ER_LOCK_DEADLOCK) {
        // todo: handle multiple sync threads deadlocking in a similar way as
        // async threads
        uint8 loopBreaker = 5;
...
template <class T>
PreparedStatement<T>*
DatabaseWorkerPool<T>::GetPreparedStatement(PreparedStatementIndex index)
{
...
void DatabaseWorkerPool<T>::KeepAlive()
{
    ...
    //! Assuming all worker threads are free, every worker thread will receive 1
    //! ping operation request If one or more worker threads are busy, the ping
    //! operations will not be split evenly, but this doesn't matter as the sole
    //! purpose is to prevent connections from idling.
...
bool DatabaseIncompatibleVersion(std::string const mysqlVersion)
{
    ...
    return std::lexicographical_compare(parsedMySQLVersion.begin(),
                                        parsedMySQLVersion.end(),
                                        parsedMinVersion.begin(),
                                        parsedMinVersion.end());
}
    ...
    for (uint8 i = 0; i < numConnections; ++i) {
        ...
        if (uint32 error = connection->Open()) {
            ...
            LOG_ERROR("sql.driver",
                      ...
                      "or MariaDB versions below 10.5.\n\nFound server "
                      "version: {}. Server compiled with: {}.",
