...
MySQLConnectionInfo::MySQLConnectionInfo(std::string_view infoString)
{
    std::vector<std::string_view> tokens =
        Acore::Tokenize(infoString, ';', true);
...
MySQLConnection::MySQLConnection(MySQLConnectionInfo& connInfo)
    : m_reconnecting(false), m_prepareError(false), m_Mysql(nullptr),
      m_queue(nullptr), m_connectionInfo(connInfo),
...
MySQLConnection::MySQLConnection(ProducerConsumerQueue<SQLOperation*>* queue,
    ...
    : m_reconnecting(false), m_prepareError(false), m_Mysql(nullptr),
      m_queue(queue), m_connectionInfo(connInfo),
...
uint32 MySQLConnection::Open()
{
    ...
    if (!mysqlInit) {
        LOG_ERROR("sql.driver",
                  "Could not initialize Mysql connection to database `{}`",
                  m_connectionInfo.database);
    ...
    if (m_connectionInfo.ssl != "") {
        ...
        mysql_options(
            mysqlInit, MYSQL_OPT_SSL_ENFORCE, (char const*)&opt_use_ssl);
#endif
    }

    m_Mysql = reinterpret_cast<MySQLHandle*>(
    ...
    if (m_Mysql) {
        if (!m_reconnecting) {
            LOG_INFO(
                "sql.sql", "MySQL client library: {}", mysql_get_client_info());
            LOG_INFO("sql.sql",
                     "MySQL server ver: {} ",
        ...
        // set connection properties to UTF8 to properly handle locales for
        // different server configs - core sends data in UTF8, so MySQL must
        // expect UTF8 too
...
bool MySQLConnection::Execute(std::string_view sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, std::string(sql).c_str())) {
            ...
            if (_HandleMySQLErrno(
                    lErrno))         // If it returns true, an error was handled
                                     // successfully (i.e. reconnection)
...
bool MySQLConnection::Execute(PreparedStatementBase* stmt)
{
    ...
    ASSERT(m_mStmt); // Can only be null if preparation failed, server side
                     // error or bad query
    ...
    if (mysql_stmt_bind_named_param(
            msql_STMT, msql_BIND, m_mStmt->GetParameterCount(), nullptr))
#else
                             ...
                             uint32*                  pFieldCount)
{
...
#endif
    {
        ...
        if (_HandleMySQLErrno(
                lErrno)) // If it returns true, an error was handled
                         // successfully (i.e. reconnection)
            return _Query(
                stmt, mysqlStmt, pResult, pRowCount, pFieldCount); // Try again
    ...
    *pResult =
        reinterpret_cast<MySQLResult*>(mysql_stmt_result_metadata(msql_STMT));
...
void MySQLConnection::BeginTransaction() { Execute("START TRANSACTION"); }

void MySQLConnection::RollbackTransaction() { Execute("ROLLBACK"); }

void MySQLConnection::CommitTransaction() { Execute("COMMIT"); }

int MySQLConnection::ExecuteTransaction(
    std::shared_ptr<TransactionBase> transaction)
{
    ...
    for (auto const& data : queries) {
        ...
        case SQL_ELEMENT_PREPARED: {
            ...
            try {
                ...
                LOG_FATAL(
                    "sql.sql",
                    "> PreparedStatementBase not found in SQLElementData. {}",
                    ex.what());
                ABORT();
            }
        ...
        case SQL_ELEMENT_RAW: {
            ...
            try {
                ...
                LOG_FATAL("sql.sql",
                          "> std::string not found in SQLElementData. {}",
                          ex.what());
                ABORT();
            }
    ...
    // we might encounter errors during certain queries, and depending on the
    // kind of error we might want to restart the transaction. So to prevent
    // data loss, we only clean up when it's all done. This is done in calling
    // functions DatabaseWorkerPool<T>::DirectCommitTransaction and
    // TransactionTask::Execute, and not while iterating over every element.
...
void MySQLConnection::Ping() { mysql_ping(m_Mysql); }

uint32 MySQLConnection::GetLastError() { return mysql_errno(m_Mysql); }

bool MySQLConnection::LockIfReady() { return m_Mutex.try_lock(); }

void MySQLConnection::Unlock() { m_Mutex.unlock(); }
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ASSERT(index < m_stmts.size(),
           "Tried to access invalid prepared statement index {} (max index {}) "
           "on database `{}`, connection type: {}",
           index,
           m_stmts.size(),
           m_connectionInfo.database,
           (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous"
                                                  : "synchronous");
    ...
    if (!ret)
        LOG_ERROR("sql.sql",
                  "Could not fetch prepared statement {} on database `{}`, "
                  "connection type: {}.",
                  index,
                  m_connectionInfo.database,
                  (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous"
                                       ...
                                       ConnectionFlags  flags)
{
    ...
    if (!stmt) {
        LOG_ERROR(
            "sql.sql", "In mysql_stmt_init() id: {}, sql: \"{}\"", index, sql);
        ...
        if (mysql_stmt_prepare(stmt,
            ...
            m_stmts[index] = std::make_unique<MySQLPreparedStatement>(
...
PreparedResultSet* MySQLConnection::Query(PreparedStatementBase* stmt)
{
    ...
    return new PreparedResultSet(
        mysqlStmt->GetSTMT(), result, rowCount, fieldCount);
}
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, uint8 attempts /*= 5*/)
{
    ...
    case CR_CONN_HOST_ERROR: {
        ...
        if (!lErrno) {
            // Don't remove 'this' pointer unless you want to skip loading all
            // prepared statements...
            ...
            LOG_INFO("sql.sql",
                     ...
                     m_connectionInfo.database,
                     m_connectionInfo.host,
                     m_connectionInfo.port_or_socket,
                     (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous"
                                                            : "synchronous");
        ...
        if ((--attempts) == 0) {
            ...
            LOG_FATAL("sql.sql",
                      "Failed to reconnect to the MySQL server, terminating "
                      "the server to prevent data corruption!");

            // We could also initiate a shutdown through using
            // std::raise(SIGTERM)
            std::this_thread::sleep_for(10s);
            ABORT();
        }
    ...
    case ER_LOCK_DEADLOCK:
        return false; // Implemented in TransactionTask::Execute and
                      // DatabaseWorkerPool<T>::DirectCommitTransaction
    ...
    case ER_NO_SUCH_TABLE:
        LOG_ERROR("sql.sql",
                  "Your database structure is not up to date. Please make sure "
                  "you've executed all queries in the sql/updates folders.");
        std::this_thread::sleep_for(10s);
        ABORT();
        return false;
    case ER_PARSE_ERROR:
    ...
    default:
        LOG_ERROR("sql.sql",
                  "Unhandled MySQL errno {}. Unexpected behaviour possible.",
                  errNo);
        return false;
    }
}
