...
{
    // Int8
    if constexpr (std::is_same_v<T, bool> || std::is_same_v<T, int8> ||
    ...
    if constexpr (std::is_same_v<T, double>) {
        if (type == DatabaseFieldTypes::Double ||
            type == DatabaseFieldTypes::Decimal)
            return true;
    }
...
{
    if constexpr (std::is_same_v<T, double>) {
        if ((StringEqualI(alias, "sum") || StringEqualI(alias, "avg")) &&
            type == DatabaseFieldTypes::Decimal)
            return true;
    ...
    if ((StringEqualI(alias, "min") || StringEqualI(alias, "max")) &&
        IsCorrectFieldType<T>(type)) {
        return true;
    }
...
{
    // This value stores somewhat structured data that needs function style
    // casting
    data.value  = newValue;
    data.length = length;
    data.raw    = false;
...
bool Field::IsType(DatabaseFieldTypes type) const { return meta->Type == type; }
...
T Field::GetData() const
{
    static_assert(std::is_arithmetic_v<T>,
                  "Unsurropt type for Field::GetData()");
    ...
    if constexpr (std::is_same_v<T, uint32>) {
        ...
        if (!tableName.empty() && tableName.size() > 4) {
            ...
            if (signedResult && !result &&
                tableName.substr(tableName.length() - 4) == "_dbc") {
                LOG_DEBUG(
                    "sql.sql",
                    ...
                    typeid(T).name());
                LOG_DEBUG("sql.sql",
                          "> Table name '{}'. Field name '{}'. Try return "
                          "int32 value",
                          meta->TableName,
                          meta->Name);
    ...
    if (auto alias = GetCleanAliasName(meta->Alias)) {
        if ((StringEqualI(*alias, "min") || StringEqualI(*alias, "max")) &&
            !IsCorrectAlias<T>(meta->Type, *alias)) {
            LogWrongType(__FUNCTION__, typeid(T).name());
        }
