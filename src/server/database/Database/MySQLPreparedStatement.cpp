...
struct MySQLType<float>
    : std::integral_constant<enum_field_types, MYSQL_TYPE_FLOAT> {};
template <>
      ...
      m_queryString(std::string(queryString))
{
    ...
    /// "If set to 1, causes mysql_stmt_store_result() to update the metadata
...
void MySQLPreparedStatement::BindParameters(PreparedStatementBase* stmt)
{
    ...
    if (pos < m_paramCount)
        LOG_WARN("sql.sql",
                 "[WARNING]: BindParameters() for statement {} did not bind "
                 "all allocated parameters",
                 stmt->GetIndex());
#endif
}
...
static bool
ParamenterIndexAssertFail(uint32 stmtIndex, uint8 index, uint32 paramCount)
{
    LOG_ERROR(
        "sql.driver",
        "Attempted to bind parameter {}{} on a PreparedStatement {} (statement "
        "has only {} parameters)",
        uint32(index) + 1,
...
{
    ASSERT(index < m_paramCount ||
           ParamenterIndexAssertFail(m_stmt->GetIndex(), index, m_paramCount));

    if (m_paramsSet[index])
        LOG_ERROR("sql.sql",
                  "[ERROR] Prepared Statement (id: {}) trying to bind value on "
                  "already bound index ({}).",
                  m_stmt->GetIndex(),
                  index);
}
...
std::string MySQLPreparedStatement::getQueryString() const
{
    ...
    for (PreparedStatementData const& data : m_stmt->GetParameters()) {
        ...
        std::string replaceStr = std::visit(
            [&](auto&& data) { return PreparedStatementData::ToString(data); },
            data.data);
