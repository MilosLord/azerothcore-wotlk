...
template <typename T>
using is_default = std::enable_if_t<std::is_arithmetic_v<T> ||
                                    std::is_same_v<std::vector<uint8>, T>>;

template <typename T>
using is_enum_v = std::enable_if_t<std::is_enum_v<T>>;

template <typename T>
using is_non_string_view_v =
    std::enable_if_t<!std::is_base_of_v<std::string_view, T>>;
} // namespace Acore::Types
...
public:
    ...
    inline void SetData(const uint8                                 index,
                        std::chrono::duration<_Rep, _Period> const& value,
                        bool convertToUin32 = true)
    {
        SetValidData(index,
                     convertToUin32 ? static_cast<uint32>(value.count())
                                    : value.count());
    }
    ...
    [[nodiscard]] std::vector<PreparedStatementData> const&
    GetParameters() const
    {
...
protected:
    template <typename T>
    Acore::Types::is_non_string_view_v<T> SetValidData(const uint8 index,
                                                       T const&    value);
    ...
    operator=(PreparedStatementBase const& right) = delete;
};
...
public:
    explicit PreparedStatement(uint32 index, uint8 capacity)
        : PreparedStatementBase(index, capacity)
    {
    }
