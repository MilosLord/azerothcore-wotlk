...
using is_chrono_v = std::enable_if_t<
    std::is_same_v<Milliseconds, T> || std::is_same_v<Seconds, T> ||
        std::is_same_v<Minutes, T> || std::is_same_v<Hours, T> ||
        std::is_same_v<Days, T> || std::is_same_v<Weeks, T> ||
...
public:
    ...
    inline std::enable_if_t<std::is_same_v<Binary, T>, std::array<uint8, S>>
    Get() const
    {
