...
uint32 GenerateUniqueObjectId(uint32 clientId, uint16 clientDoodadId)
{
    return uniqueObjectIds
        .emplace(std::make_pair(clientId, clientDoodadId),
                 uint32(uniqueObjectIds.size() + 1))
        .first->second;
}
...
bool ExtractSingleWmo(std::string& fname)
{
    ...
    if (froot.nGroups != 0) {
        for (uint32 i = 0; i < froot.nGroups; ++i) {
            ...
            Wmo_nVertices +=
                fgroup.ConvertToVMAPGroupWmo(output, preciseVectorData);
            for (uint16 groupReference : fgroup.DoodadReferences) {
                if (groupReference >= doodads.Spawns.size())
                    continue;

                uint32 doodadNameIndex =
                    doodads.Spawns[groupReference].NameIndex;
                if (froot.ValidDoodadNames.find(doodadNameIndex) ==
                    froot.ValidDoodadNames.end())
                    continue;
...
void ParsMapFiles()
{
    ...
    for (unsigned int i = 0; i < map_count; ++i) {
        sprintf(
            fn, "World\\Maps\\%s\\%s.wdt", map_ids[i].name, map_ids[i].name);
            ...
            for (int x = 0; x < 64; ++x) {
                for (int y = 0; y < 64; ++y) {
                    if (ADTFile* ADT = WDT.GetMap(x, y)) {
                        // sprintf(id_filename,"%02u %02u
                        // %03u",x,y,map_ids[i].id);//!!!!!!!!!
...
bool fillArchiveNameVector(std::vector<std::string>& pArchiveNames)
{
    ...
    for (auto& locale : locales) {
        pArchiveNames.push_back(in_path + locale + "/locale-" + locale +
                                ".MPQ");
        pArchiveNames.push_back(in_path + locale + "/expansion-locale-" +
                                locale + ".MPQ");
        pArchiveNames.push_back(in_path + locale + "/lichking-locale-" +
                                locale + ".MPQ");
    }
...
bool processArgv(int argc, char** argv, const char* versionString)
{
        ...
        if (strcmp("-s", argv[i]) == 0) {
            ...
            if ((i + 1) < argc) {
                ...
                if (input_path[strlen(input_path) - 1] != '\\' &&
                    input_path[strlen(input_path) - 1] != '/')
                    strcat(input_path, "/");
                ++i;
            }
            else {
    ...
    if (!result) {
        printf("Extract %s.\n", versionString);
        printf("%s [-?][-s][-l][-d <path>]\n", argv[0]);
        printf("   -s : (default) small size (data size optimization), ~500MB "
               "less vmap data.\n");
        printf("   -l : large size, ~500MB more vmap data. (might contain more "
               "details)\n");
...
int main(int argc, char** argv)
{
    ...
    // some simple check if working dir is dirty
        ...
        if (!stat(sdir.c_str(), &status) || !stat(sdir_bin.c_str(), &status)) {
            printf("Your output directory seems to be polluted, please use an "
                   "empty directory!\n");
    ...
    if (gOpenArchives.empty()) {
        printf("FATAL ERROR: None MPQ archive found by path '%s'. Use -d "
               "option with proper path.\n",
               input_path);
        return 1;
    }
