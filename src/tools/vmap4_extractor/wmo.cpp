...
WMORoot::WMORoot(std::string const& filename)
    : filename(filename), color(0), nTextures(0), nGroups(0), nPortals(0),
      nLights(0), nDoodadNames(0), nDoodadDefs(0), nDoodadSets(0), RootWMOID(0),
      flags(0)
{
...
WMOGroup::WMOGroup(std::string const& filename)
    : filename(std::move(filename)), MOPY(nullptr), MOVI(nullptr),
      MoviEx(nullptr), MOVT(nullptr), MOBA(nullptr), MobaEx(nullptr),
      hlq(nullptr), LiquEx(nullptr), LiquBytes(nullptr), groupName(0),
      descGroupName(0), mogpFlags(0), moprIdx(0), moprNItems(0), nBatchA(0),
      nBatchB(0), nBatchC(0), fogIdx(0), groupLiquid(0), groupWMOID(0),
...
bool WMOGroup::open(WMORoot* rootWMO)
{
    ...
    while (!f.isEof()) {
        ...
        if (!strcmp(fourcc, "MOGP")) // header
        {
            ...
            /* std::ofstream llog("Buildings/liquid.log", ios_base::out |
            ios_base::app); llog << filename; llog << "\nbbox: " << bbcorn1[0]
            << ", " << bbcorn1[1] << ", " << bbcorn1[2] << " | " << bbcorn2[0]
            << ", " << bbcorn2[1] << ", " << bbcorn2[2]; llog << "\nlpos: " <<
            hlq->pos_x << ", " << hlq->pos_y << ", " << hlq->pos_z; llog <<
            "\nx-/yvert: " << hlq->xverts << "/" << hlq->yverts << " size: " <<
            size << " expected size: " << 30 + hlq->xverts*hlq->yverts*8 +
            hlq->xtiles*hlq->ytiles << std::endl; llog.close(); */
        }
        f.seek((int)nextpos);
    }
    f.close();
...
int WMOGroup::ConvertToVMAPGroupWmo(FILE* output, bool preciseVectorData)
{
    ...
    if (preciseVectorData) {
        ...
        if (nIdexes > 0) {
            if (fwrite(MOVI, sizeof(unsigned short), nIdexes, output) !=
                nIdexes) {
        ...
        if (nVertices > 0) {
            if (fwrite(MOVT, sizeof(float) * 3, nVertices, output) !=
                nVertices) {
        ...
        for (int i = 0; i < nTriangles; ++i) {
            ...
            bool isCollision =
                MOPY[2 * i] & WMO_MATERIAL_COLLISION || isRenderFace;
            if (!isCollision)
                continue;
            // Use this triangle
            for (int j = 0; j < 3; ++j) {
        ...
        // write vertices
        int VERT[] = {0x54524556,
                      nColVertices * 3 * static_cast<int>(sizeof(float)) + 4,
                      nColVertices}; // "VERT"
    ...
    if (liquflags & 3) {
        ...
        if (liquflags & 1) {
            ...
            LIQU_totalSize +=
                LiquEx_size / sizeof(WMOLiquidVert) * sizeof(float);
        ...
        /* std::ofstream llog("Buildings/liquid.log", ios_base::out |
        ios_base::app); llog << filename; llog << ":\nliquidEntry: " <<
        liquidEntry << " type: " << hlq->type << " (root:" <<
        rootWMO->liquidType << " group:" << liquidType << ")\n"; llog.close();
      */
                        ...
                        FILE*            pDirfile)
{
    ...
    // write mapID, tileX, tileY, Flags, NameSet, UniqueId, Pos, Rot, Scale,
