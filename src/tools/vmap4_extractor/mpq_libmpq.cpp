...
{
    return std::find(gOpenArchives.begin(), gOpenArchives.end(), this) !=
           gOpenArchives.end();
}
...
MPQFile::MPQFile(const char* filename)
    : eof(false), buffer(nullptr), pointer(0), size(0)
{
    for (auto& gOpenArchive : gOpenArchives) {
        ...
        // HACK: in patch.mpq some files don't want to open and give 1 for
        // filesize
        if (size <= 1) {
