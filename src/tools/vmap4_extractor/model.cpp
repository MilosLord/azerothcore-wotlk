...
Model::Model(std::string& filename)
    : filename(filename), header(), vertices(nullptr), indices(nullptr)
{
}
...
bool Model::open()
{
    ...
    if (f.isEof()) {
        f.close();
        // Do not show this error on console to avoid confusion, the extractor
        // can continue working even if some models fail to load
...
bool Model::ConvertToVMAPModel(const char* outfilename)
{
    ...
    fwrite(
        N, sizeof(float), 3 * 2, output); // bbox, only needed for WMO currently
...
Vec3D fixCoordSystem(Vec3D const& v) { return Vec3D(v.x, v.z, -v.y); }
                        ...
                        FILE*                pDirfile)
{
    ...
    G3D::Matrix3 wmoRotation =
        G3D::Matrix3::fromEulerAnglesZYX(G3D::toRadians(wmo.Rotation.y),
                                         G3D::toRadians(wmo.Rotation.x),
                                         G3D::toRadians(wmo.Rotation.z));
    ...
    for (uint16 doodadIndex : doodadData.References) {
        ...
        G3D::Vector3 position =
            wmoPosition + (wmoRotation * G3D::Vector3(doodad.Position.x,
                                                      doodad.Position.y,
                                                      doodad.Position.z));

        Vec3D rotation;
        (G3D::Quat(doodad.Rotation.X,
                   doodad.Rotation.Y,
                   doodad.Rotation.Z,
                   doodad.Rotation.W)
             .toRotationMatrix() *
