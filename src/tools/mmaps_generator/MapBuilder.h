...
struct MapTiles {
    ...
    bool operator==(uint32 id) { return m_mapId == id; }
};
...
struct TileConfig {
    TileConfig(bool bigBaseUnit)
    {
        ...
        // value have to divide GRID_SIZE(533.3333f) ( aka: 0.5333, 0.2666,
        // 0.3333, 0.1333, etc )
...
/// @todo: move this to its own file. For now it will stay here to keep the
/// changes to a minimum, especially in the cpp file
class MapBuilder;
class TileBuilder {
public:
    ...
    void
    buildTile(uint32 mapID, uint32 tileX, uint32 tileY, dtNavMesh* navMesh);
    // move map building
...
private:
    ...
    void getTileBounds(uint32 tileX,
                       uint32 tileY,
                       float* verts,
                       int    vertCount,
                       float* bmin,
                       float* bmax) const;
    void getGridBounds(uint32  mapID,
                       uint32& minX,
                       uint32& minY,
                       uint32& maxX,
                       uint32& maxY) const;
    ...
    rcConfig GetMapSpecificConfig(uint32            mapID,
                                  float             bmin[3],
                                  float             bmax[3],
