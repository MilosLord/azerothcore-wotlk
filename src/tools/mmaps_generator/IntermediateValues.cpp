...
#define DEBUG_WRITE(fileExtension, data)                                       \
    do {                                                                       \
        sprintf(                                                               \
            fileName, (name + fileExtension).c_str(), mapID, tileY, tileX);    \
        FILE* file = fopen(fileName, "wb");                                    \
        if (!file) {                                                           \
            char message[1024];                                                \
            sprintf(message,                                                   \
                    "%sFailed to open %s for writing!\n",                      \
                    tileString,                                                \
                    fileName);                                                 \
            perror(message);                                                   \
        }                                                                      \
        ...
        if (file)                                                              \
            fclose(file);                                                      \
        printf("%sWriting debug output...                       \r",           \
               tileString);                                                    \
    } while (false)
...
void IntermediateValues::debugWrite(FILE* file, const rcCompactHeightfield* chf)
{
    ...
    if (chf->cells)
        fwrite(
            chf->cells, sizeof(rcCompactCell), chf->width * chf->height, file);
    if (chf->spans)
...
void IntermediateValues::debugWrite(FILE* file, const rcContourSet* cs)
{
    ...
    for (int i = 0; i < cs->nconts; ++i) {
        ...
        fwrite(
            cs->conts[i].rverts, sizeof(int), cs->conts[i].nrverts * 4, file);
    }
}
...
void IntermediateValues::generateObjFile(uint32    mapID,
                                         uint32    tileX,
                                         uint32    tileY,
                                         MeshData& meshData)
{
    ...
    TerrainBuilder::copyIndices(
        meshData.solidTris, allTris, allVerts.size() / 3);
