...
/**************************************************************************/
void TerrainBuilder::getLoopVars(Spot portion,
                                 int& loopStart,
                                 int& loopEnd,
                                 int& loopInc)
{
...
/**************************************************************************/
void TerrainBuilder::loadMap(uint32    mapID,
                             uint32    tileX,
                             uint32    tileY,
                             MeshData& meshData)
{
    ...
    uint32 mapID, uint32 tileX, uint32 tileY, MeshData& meshData, Spot portion)
{
    ...
    if (haveTerrain) {
        ...
        if (hheader.flags & MAP_HEIGHT_AS_INT8) {
            ...
            if (count != expected)
                printf("TerrainBuilder::loadMap: Failed to read some data "
                       "expected %d, read %d\n",
                       expected,
                       count);

            heightMultiplier =
                (hheader.gridMaxHeight - hheader.gridHeight) / 255;
            ...
            heightMultiplier =
                (hheader.gridMaxHeight - hheader.gridHeight) / 65535;
    ...
    if (haveLiquid) {
        ...
        if (fread(&lheader, sizeof(map_liquidHeader), 1, mapFile) != 1)
            printf("TerrainBuilder::loadMap: Failed to read some data expected "
                   "1, read 0\n");
        ...
        if (!(lheader.flags & MAP_LIQUID_NO_HEIGHT)) {
            ...
            for (int i = 0; i < V9_SIZE_SQ; ++i) {
                ...
                if (row < lheader.offsetY ||
                    row >= lheader.offsetY + lheader.height ||
                    col < lheader.offsetX ||
    ...
    // now that we have gathered the data, we can figure out which parts to
    // keep: liquid above ground, ground above liquid
    ...
    if (meshData.liquidVerts.size()) {
        ...
        memcpy(
            lverts_copy, lverts, sizeof(float) * meshData.liquidVerts.size());
    }
    ...
    for (int i = loopStart; i < loopEnd; i += loopInc) {
        for (int j = 0; j < 2; ++j) {
            ...
            // FIXME: "warning: the address of ‘liquid_type’ will always
            // evaluate as ‘true’"
            ...
            if (!meshData.liquidVerts.size() || !ltriangles.size()) {
                ...
                case MAP_LIQUID_TYPE_DARK_WATER:
                    // players should not be here, so logically neither should
                    // creatures
                    useTerrain = false;
                    useLiquid  = false;
                    break;
            ...
            if (useLiquid) {
                ...
                for (uint32 idx = 0; idx < 3; idx++) {
                    ...
                    if (h != INVALID_MAP_LIQ_HEIGHT &&
                        h < INVALID_MAP_LIQ_HEIGHT_MAX) {
                        quadHeight += h;
                        validCount++;
                    }
                }
...
/**************************************************************************/
void TerrainBuilder::getHeightCoord(
    int index, Grid grid, float xOffset, float yOffset, float* coord, float* v)
{
    ...
    case GRID_V9:
        ...
        break;
    case GRID_V8:
        coord[0] = (xOffset + index % (V8_SIZE)*GRID_PART_SIZE +
                    GRID_PART_SIZE / 2.f) *
                   -1.f;
        coord[1] = (yOffset + (int)(index / (V8_SIZE)) * GRID_PART_SIZE +
                    GRID_PART_SIZE / 2.f) *
                   -1.f;
        coord[2] = v[index];
        break;
...
/**************************************************************************/
void TerrainBuilder::getHeightTriangle(int  square,
                                       Spot triangle,
                                       int* indices,
                                       bool liquid /* = false*/)
{
...
/**************************************************************************/
void TerrainBuilder::getLiquidCoord(
    int index, int index2, float xOffset, float yOffset, float* coord, float* v)
{
    // wow coords: x, y, height
    // coord is mirroed about the horizontal axes
    coord[0] = (xOffset + index % (V9_SIZE)*GRID_PART_SIZE) * -1.f;
...
/**************************************************************************/
bool TerrainBuilder::loadVMap(uint32    mapID,
                              uint32    tileX,
                              uint32    tileY,
                              MeshData& meshData)
{
    ...
    do {
        ...
        for (uint32 i = 0; i < count; ++i) {
            ...
            // model instances exist in tree even though there are instances of
            // that model in this tile
            ...
            bool isM2 = instance.name.find(".m2") != std::string::npos ||
                        instance.name.find(".M2") != std::string::npos;
                ...
                if (liquid && liquid->GetFlagsStorage()) {
                    ...
                    for (uint32 x = 0; x < vertsX; ++x) {
                        for (uint32 y = 0; y < vertsY; ++y) {
                            vert = G3D::Vector3(corner.x + x * GRID_PART_SIZE,
                                                corner.y + y * GRID_PART_SIZE,
                                                data[y * vertsX + x]);
                    ...
                    for (auto& liqVert : liqVerts) {
                        meshData.liquidVerts.append(
                            liqVert.y, liqVert.z, liqVert.x);
                    }

                    for (uint32 j = 0; j < liqTris.size() / 3; ++j) {
                        meshData.liquidTris.append(
                            liqTris[j * 3 + 1] + liqOffset,
                            liqTris[j * 3 + 2] + liqOffset,
...
/**************************************************************************/
void TerrainBuilder::transform(std::vector<G3D::Vector3>& source,
                               std::vector<G3D::Vector3>& transformedVertices,
                               float                      scale,
...
/**************************************************************************/
void TerrainBuilder::copyVertices(std::vector<G3D::Vector3>& source,
...
/**************************************************************************/
void TerrainBuilder::copyIndices(std::vector<MeshTriangle>& source,
                                 G3D::Array<int>&           dest,
                                 int                        offset,
                                 bool                       flip)
{
...
/**************************************************************************/
void TerrainBuilder::copyIndices(G3D::Array<int>& source,
                                 G3D::Array<int>& dest,
                                 int              offset)
{
...
/**************************************************************************/
void TerrainBuilder::cleanVertices(G3D::Array<float>& verts,
                                   G3D::Array<int>&   tris)
{
...
/**************************************************************************/
void TerrainBuilder::loadOffMeshConnections(uint32      mapID,
                                            uint32      tileX,
                                            uint32      tileY,
                                            MeshData&   meshData,
                                            const char* offMeshFilePath)
{
    ...
    // pretty silly thing, as we parse entire file and load only the tile we
    // need but we don't expect this file to be too large
    ...
    while (fgets(buf, 512, fp)) {
        ...
        if (mapID == mid && tileX == tx && tileY == ty) {
            ...
            meshData.offMeshConnectionDirs.append(
                1); // 1 - both direction, 0 - one sided
            meshData.offMeshConnectionRads.append(
                size); // agent size equivalent
            // can be used same way as polygon flags
            meshData.offMeshConnectionsAreas.append((unsigned char)0xFF);
            meshData.offMeshConnectionsFlags.append(
                (unsigned short)0xFF); // all movement masks can make this path
        }
    }
