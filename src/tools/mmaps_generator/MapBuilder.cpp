...
TileBuilder::TileBuilder(MapBuilder* mapBuilder,
                         ...
                         bool        bigBaseUnit,
                         bool        debugOutput)
    : m_bigBaseUnit(bigBaseUnit), m_debugOutput(debugOutput),
      m_mapBuilder(mapBuilder), m_terrainBuilder(nullptr),
      m_workerThread(&TileBuilder::WorkerThread, this), m_rcContext(nullptr)
{
...
MapBuilder::MapBuilder(float        maxWalkableAngle,
    ...
    :
      ...
      m_debugOutput(debugOutput), m_offMeshFilePath(offMeshFilePath),
      m_threads(threads), m_skipContinents(skipContinents),
      m_skipJunkMaps(skipJunkMaps), m_skipBattlegrounds(skipBattlegrounds),
      m_skipLiquid(skipLiquid), m_maxWalkableAngle(maxWalkableAngle),
      m_bigBaseUnit(bigBaseUnit), m_mapid(mapid), m_totalTiles(0u),
...
void MapBuilder::buildMaps(Optional<uint32> mapID)
{
    ...
    for (unsigned int i = 0; i < m_threads; ++i) {
        m_tileBuilders.push_back(
            new TileBuilder(this, m_skipLiquid, m_bigBaseUnit, m_debugOutput));
    }
    ...
    if (mapID) {
        ...
        for (TileList::iterator it = m_tiles.begin(); it != m_tiles.end();
             ++it) {
...
void MapBuilder::getGridBounds(
    uint32 mapID, uint32& minX, uint32& minY, uint32& maxX, uint32& maxY) const
{
    // min and max are initialized to invalid values so the caller iterating the
    // [min, max] range will never enter the loop unless valid min/max values
    // are found
    maxX = 0;
    maxY = 0;
    ...
    if (meshData.solidVerts.size() && meshData.liquidVerts.size()) {
        rcCalcBounds(meshData.solidVerts.getCArray(),
                     meshData.solidVerts.size() / 3,
                     bmin,
                     bmax);
        rcCalcBounds(meshData.liquidVerts.getCArray(),
                     meshData.liquidVerts.size() / 3,
                     lmin,
                     lmax);
        rcVmin(bmin, lmin);
        rcVmax(bmax, lmax);
...
void MapBuilder::buildMeshFromFile(char* name)
{
    ...
    getTileBounds(tileX,
                  tileY,
                  data.solidVerts.getCArray(),
                  data.solidVerts.size() / 3,
                  bmin,
                  bmax);

    // build navmesh tile
    TileBuilder tileBuilder =
        TileBuilder(this, m_skipLiquid, m_bigBaseUnit, m_debugOutput);
    tileBuilder.buildMoveMapTile(
        mapId, tileX, tileY, data, bmin, bmax, navMesh);
    fclose(file);
}
...
void TileBuilder::WorkerThread()
{
    while (true) {
        ...
        buildTile(
            tileInfo.m_mapId, tileInfo.m_tileX, tileInfo.m_tileY, navMesh);
...
void MapBuilder::buildMap(uint32 mapID)
{
    ...
    if (!tiles->empty()) {
        ...
        for (unsigned int tile : *tiles) {
            ...
            memcpy(&tileInfo.m_navMeshParams,
                   navMesh->getParams(),
...
/**************************************************************************/
void TileBuilder::buildTile(uint32     mapID,
                            uint32     tileX,
                            uint32     tileY,
                            dtNavMesh* navMesh)
{
    ...
    printf("%u%% [Map %04i] Building tile [%02u,%02u]\n",
           m_mapBuilder->currentPercentageDone(),
           mapID,
           tileX,
           tileY);
...
void MapBuilder::buildNavMesh(uint32 mapID, dtNavMesh*& navMesh)
{
    ...
    // need at least one bit! int polyBits = sizeof(dtPolyRef)*8 - SALT_MIN_BITS
    // - tileBits;
    ...
    if (!file) {
        ...
        char message[1024];
        sprintf(message,
                "[Map %03i] Failed to open %s for writing!\n",
                mapID,
                fileName);
        perror(message);
        return;
...
/**************************************************************************/
void TileBuilder::buildMoveMapTile(uint32     mapID,
                                   uint32     tileX,
                                   uint32     tileY,
                                   MeshData&  meshData,
                                   float      bmin[3],
                                   ...
                                   dtNavMesh* navMesh)
{
    ...
    rcConfig         config =
        m_mapBuilder->GetMapSpecificConfig(mapID, bmin, bmax, tileConfig);
    ...
    for (int y = 0; y < TILES_PER_MAP; ++y) {
        for (int x = 0; x < TILES_PER_MAP; ++x) {
            ...
            tileCfg.bmin[0] =
                config.bmin[0] + x * float(config.tileSize * config.cs);
            tileCfg.bmin[2] =
                config.bmin[2] + y * float(config.tileSize * config.cs);
            tileCfg.bmax[0] =
                config.bmin[0] + (x + 1) * float(config.tileSize * config.cs);
            tileCfg.bmax[2] =
                config.bmin[2] + (y + 1) * float(config.tileSize * config.cs);
            ...
            if (!tile.solid || !rcCreateHeightfield(m_rcContext,
                                                    *tile.solid,
                                                    tileCfg.width,
                                                    tileCfg.height,
                                                    tileCfg.bmin,
            ...
            rcClearUnwalkableTriangles(m_rcContext,
                                       ...
                                       tTriCount,
                                       triFlags);
            rcRasterizeTriangles(m_rcContext,
                                 tVerts,
                                 tVertCount,
                                 tTris,
                                 triFlags,
            ...
            rcFilterLedgeSpans(m_rcContext,
            ...
            if (!tile.chf || !rcBuildCompactHeightfield(m_rcContext,
                                                        tileCfg.walkableHeight,
                                                        tileCfg.walkableClimb,
                                                        *tile.solid,
                                                        *tile.chf)) {
            ...
            if (!rcBuildRegions(m_rcContext,
                                *tile.chf,
                                tileCfg.borderSize,
                                tileCfg.minRegionArea,
            ...
            if (!tile.cset || !rcBuildContours(m_rcContext,
                                               *tile.chf,
            ...
            if (!tile.pmesh || !rcBuildPolyMesh(m_rcContext,
                                                *tile.cset,
    ...
    /// @todo: special flags for DYNAMIC polygons, ie surfaces that can be
    /// turned on and off
    ...
    do {
        ...
        // here so we have a clear error message
...
/**************************************************************************/
void MapBuilder::getTileBounds(uint32 tileX,
                               uint32 tileY,
                               float* verts,
                               int    vertCount,
                               float* bmin,
...
rcConfig MapBuilder::GetMapSpecificConfig(uint32            mapID,
                                          float             bmin[3],
                                          float             bmax[3],
                                          const TileConfig& tileConfig) const
{
    ...
    config.maxEdgeLen =
        tileConfig.VERTEX_PER_TILE + 1; // anything bigger than tileSize
...
/**************************************************************************/
uint32 MapBuilder::percentageDone(uint32 totalTiles,
                                  uint32 totalTilesBuilt) const
{
    if (totalTiles)
