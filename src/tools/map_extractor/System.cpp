...
float CONF_flat_height_delta_limit =
    0.005f; // If max - min less this value - surface is flat
float CONF_flat_liquid_delta_limit =
    0.001f; // If max - min less this value - liquid surface is flat
...
static const char* const langs[] = {"enGB",
                                    "enUS",
                                    "deDE",
                                    "esES",
                                    "frFR",
...
void CreateDir(const std::string& Path)
{
    ...
    if (ret != 0) {
        printf(
            "Fatal Error: Could not create directory %s check your permissions",
            Path.c_str());
        exit(1);
    }
}
...
void Usage(char* prg)
{
    printf("Usage:\n"
           ...
           "-f height stored as int (less map size but lost some accuracy) 1 "
           "by default\n"
...
uint32 ReadBuild(int locale)
{
    ...
    std::string filename =
        std::string("component.wow-") + langs[locale] + ".txt";
                ...
                uint32 build)
{
        ...
        for (int j = 0; j < ADT_CELLS_PER_GRID; j++) {
            ...
            for (int y = 0; y <= ADT_CELL_SIZE; y++) {
                ...
                for (int x = 0; x <= ADT_CELL_SIZE; x++) {
                    int cx = j * ADT_CELL_SIZE + x;
                    V9[cy][cx] +=
                        v->height_map[y * (ADT_CELL_SIZE * 2 + 1) + x];
                }
            }
            // get V8 height map
            for (int y = 0; y < ADT_CELL_SIZE; y++) {
                ...
                for (int x = 0; x < ADT_CELL_SIZE; x++) {
                    int cx = j * ADT_CELL_SIZE + x;
                    V8[cy][cx] += v->height_map[y * (ADT_CELL_SIZE * 2 + 1) +
                                                ADT_CELL_SIZE + 1 + x];
                }
            }
        }
    ...
    // Check for allow limit minimum height (not store height in deep ochean -
    // allow save some memory)
    ...
    if (CONF_allow_float_to_int &&
        (maxHeight - minHeight) < CONF_flat_height_delta_limit)
        heightHeader.flags |= MAP_HEIGHT_NO_HEIGHT;
    ...
    if (!(heightHeader.flags & MAP_HEIGHT_NO_HEIGHT)) {
        ...
        if (CONF_allow_float_to_int) {
            ...
            if (diff <
                CONF_float_to_int8_limit) // As uint8 (max accuracy =
                                          // CONF_float_to_int8_limit/256)
            {
                ...
                diff <
                CONF_float_to_int16_limit) // As uint16 (max accuracy =
                                           // CONF_float_to_int16_limit/65536)
            {
    ...
    for (int y = 0; y < ADT_CELLS_PER_GRID; y++) {
        for (int x = 0; x < ADT_CELLS_PER_GRID; x++) {
            if (liquid_entry[y][x] != firstLiquidType ||
                liquid_flags[y][x] != firstLiquidFlag) {
                fullType = true;
    ...
    if (firstLiquidFlag == 0 && !fullType) {
        ...
        // Not need store if flat surface
        if (CONF_allow_float_to_int &&
            (maxHeight - minHeight) < CONF_flat_liquid_delta_limit)
            liquidHeader.flags |= MAP_LIQUID_NO_HEIGHT;
        ...
        if (!(liquidHeader.flags & MAP_LIQUID_NO_HEIGHT))
            map.liquidMapSize +=
                sizeof(float) * liquidHeader.width * liquidHeader.height;
    }
    ...
    if (map.liquidMapOffset) {
        ...
        if (!(liquidHeader.flags & MAP_LIQUID_NO_HEIGHT)) {
            for (int y = 0; y < liquidHeader.height; y++)
                fwrite(&liquid_height[y + liquidHeader.offsetY]
                                     [liquidHeader.offsetX],
                       sizeof(float),
                       liquidHeader.width,
                       output);
        }
...
void ExtractMapsFromMpq(uint32 build)
{
    ...
    for (uint32 z = 0; z < map_count; ++z) {
        ...
        mpqMapName = Acore::StringFormat(
            R"(World\Maps\%s\%s.wdt)", map_ids[z].name, map_ids[z].name);
        WDT_file wdt;
        ...
        for (uint32 y = 0; y < WDT_MAP_SIZE; ++y) {
            for (uint32 x = 0; x < WDT_MAP_SIZE; ++x) {
                if (!wdt.main->adt_list[y][x].exist)
                    continue;
                mpqFileName =
                    Acore::StringFormat(R"(World\Maps\%s\%s_%u_%u.adt)",
                                        map_ids[z].name,
                                        map_ids[z].name,
                                        x,
                                        y);
                outputFileName = Acore::StringFormat("%s/maps/%03u%02u%02u.map",
                                                     output_path,
                                                     map_ids[z].id,
                                                     y,
                                                     x);
...
int main(int argc, char* arg[])
{
    ...
    for (int i = 0; i < LANG_COUNT; i++) {
        char tmp1[512];
        sprintf(
            tmp1, "%s/Data/%s/locale-%s.MPQ", input_path, langs[i], langs[i]);
        if (FileExists(tmp1)) {
