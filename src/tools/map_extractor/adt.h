...
public:
    float height_map[(ADT_CELL_SIZE + 1) * (ADT_CELL_SIZE + 1) +
                     ADT_CELL_SIZE * ADT_CELL_SIZE];
...
public:
    ...
    adt_liquid_instance const* GetLiquidInstance(int32 x, int32 y) const
    {
        if (liquid[x][y].used && liquid[x][y].OffsetInstances)
            return (adt_liquid_instance*)((uint8*)this + 8 +
    ...
    adt_liquid_attributes GetLiquidAttributes(int32 x, int32 y) const
    {
        if (liquid[x][y].used) {
            if (liquid[x][y].OffsetAttributes)
                return *(
                    (adt_liquid_attributes*)((uint8*)this + 8 +
    ...
    float GetLiquidHeight(adt_liquid_instance const* h, int32 pos) const
    {
        ...
        case LiquidVertexFormatType::HeightTextureCoord:
            return (
                (float const*)((uint8*)this + 8 + h->OffsetVertexData))[pos];
    ...
    int8 GetLiquidDepth(adt_liquid_instance const* h, int32 pos) const
    {
        ...
        switch (h->LiquidVertexFormat) {
        case LiquidVertexFormatType::HeightDepth:
            return ((int8 const*)((int8 const*)this + 8 + h->OffsetVertexData +
                                  (h->GetWidth() + 1) * (h->GetHeight() + 1) *
                                      4))[pos];
        case LiquidVertexFormatType::HeightTextureCoord:
            return 0;
        case LiquidVertexFormatType::Depth:
    ...
    uint16 const* GetLiquidTextureCoordMap(adt_liquid_instance const* h,
                                           int32                      pos) const
    {
        if (!h->OffsetVertexData)
            return nullptr;
        ...
        case LiquidVertexFormatType::HeightTextureCoord:
            return (
                uint16 const*)((uint8 const*)this + 8 + h->OffsetVertexData +
                               4 * ((h->GetWidth() + 1) * (h->GetHeight() + 1) +
                                    pos));
        default:
            break;
        }
...
public:
    ...
    adt_MCIN* getMCIN()
    {
        return reinterpret_cast<adt_MCIN*>(reinterpret_cast<uint8*>(&flags) +
                                           offsMCIN);
    }
    adt_MH2O* getMH2O()
    {
        if (offsMH2O)
            return reinterpret_cast<adt_MH2O*>(
                reinterpret_cast<uint8*>(&flags) + offsMH2O);
        return nullptr;
    }
    adt_MFBO* getMFBO()
    {
        if (flags & 1 && offsMFBO)
            return reinterpret_cast<adt_MFBO*>(
                reinterpret_cast<uint8*>(&flags) + offsMFBO);
        return nullptr;
    }
};
