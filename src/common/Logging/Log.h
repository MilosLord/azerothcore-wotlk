...
public:
    ...
    bool
    SetLogLevel(std::string const& name, int32 level, bool isLogger = true);

    template <typename... Args>
    inline void outMessage(std::string const&           filter,
                           ...
                           Args&&... args)
    {
        _outMessage(filter,
                    level,
                    Acore::StringFormatFmt(fmt, std::forward<Args>(args)...));
    }

    template <typename... Args>
    void
    outCommand(uint32 account, Acore::FormatString<Args...> fmt, Args&&... args)
    {
        if (!ShouldLog("commands.gm", LOG_LEVEL_INFO)) {
            return;
        }

        _outCommand(Acore::StringFormatFmt(fmt, std::forward<Args>(args)...),
    ...
    [[nodiscard]] std::string const& GetLogsTimestamp() const
    {
        return m_logsTimestamp;
    }
...
private:
    ...
    void _outMessage(std::string const& filter,
                     LogLevel           level,
...
#define LOG_EXCEPTION_FREE(filterType__, level__, ...)                         \
    {                                                                          \
        try {                                                                  \
            ...
            sLog->outMessage("server",                                         \
                             LogLevel::LOG_LEVEL_ERROR,                        \
                             "Wrong format occurred ({}) at '{}:{}'",          \
                             e.what(),                                         \
                             __FILE__,                                         \
                             __LINE__);                                        \
        }                                                                      \
