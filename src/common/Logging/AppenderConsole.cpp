...
AppenderConsole::AppenderConsole(uint8                                id,
...
void AppenderConsole::InitColors(std::string const& name, std::string_view str)
{
    ...
    if (colorStrs.size() != NUM_ENABLED_LOG_LEVELS) {
        throw InvalidAppenderArgsException(Acore::StringFormatFmt(
            "Log::CreateAppenderFromConfig: Invalid color data '{}' for "
            "console appender {} (expected {} entries, got {})",
            str,
            name,
    ...
    for (uint8 i = 0; i < NUM_ENABLED_LOG_LEVELS; ++i) {
        if (Optional<uint8> color = Acore::StringTo<uint8>(colorStrs[i]);
            color && EnumUtils::IsValid<ColorTypes>(*color)) {
            _colors[i] = static_cast<ColorTypes>(*color);
        }
        else {
            throw InvalidAppenderArgsException(Acore::StringFormatFmt(
                "Log::CreateAppenderFromConfig: Invalid color '{}' for log "
                "level {} on console appender {}",
                colorStrs[i],
                EnumUtils::ToTitle(static_cast<LogLevel>(i)),
                name));
        }
    }
...
void AppenderConsole::SetColor(bool stdout_stream, ColorTypes color)
{
    ...
    fprintf((stdout_stream ? stdout : stderr),
            "\x1b[%d%sm",
            UnixColorFG[color],
            (color >= YELLOW && color < NUM_COLOR_TYPES ? ";1" : ""));
#endif
}
...
void AppenderConsole::ResetColor(bool stdout_stream)
{
    ...
    SetConsoleTextAttribute(
        hConsole, FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED);
#else
...
{
    bool stdout_stream = !(message->level == LOG_LEVEL_ERROR ||
