...
LogMessage::LogMessage(LogLevel           _level,
    ...
    : level(_level), type(_type), text(std::string(_text)),
      mtime(GetEpochTime())
{
}
...
LogMessage::LogMessage(LogLevel           _level,
    ...
    : level(_level), type(_type), text(std::string(_text)),
      param1(std::string(_param1)), mtime(GetEpochTime())
{
}
...
std::string LogMessage::getTimeStr() const { return getTimeStr(mtime); }
