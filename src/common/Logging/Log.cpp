...
uint8 Log::NextAppenderId() { return AppenderId++; }

Appender* Log::GetAppenderByName(std::string_view name)
{
    auto it = appenders.begin();
    while (it != appenders.end() && it->second &&
...
void Log::CreateAppenderFromConfig(std::string const& appenderName)
{
    ...
    if (size < 2) {
        fmt::print(stderr,
                   "Log::CreateAppenderFromConfig: Wrong configuration for "
                   "appender {}. Config line: {}\n",
                   name,
                   options);
        return;
    }
    ...
    AppenderType  type  = AppenderType(
        Acore::StringTo<uint8>(tokens[0]).value_or(APPENDER_INVALID));
    LogLevel level =
        LogLevel(Acore::StringTo<uint8>(tokens[1]).value_or(LOG_LEVEL_INVALID));

    auto factoryFunction = appenderFactory.find(type);
    if (factoryFunction == appenderFactory.end()) {
        fmt::print(stderr,
                   "Log::CreateAppenderFromConfig: Unknown type '{}' for "
                   "appender {}\n",
                   tokens[0],
                   name);
        return;
    ...
    if (level > NUM_ENABLED_LOG_LEVELS) {
        fmt::print(stderr,
                   "Log::CreateAppenderFromConfig: Wrong Log Level '{}' for "
                   "appender {}\n",
                   tokens[1],
                   name);
        return;
    ...
    if (size > 2) {
        if (Optional<uint8> flagsVal = Acore::StringTo<uint8>(tokens[2])) {
            ...
            fmt::print(stderr,
                       "Log::CreateAppenderFromConfig: Unknown flags '{}' for "
                       "appender {}\n",
                       tokens[2],
                       name);
            return;
    ...
    try {
        Appender* appender = factoryFunction->second(
            NextAppenderId(), name, level, flags, tokens);
...
void Log::CreateLoggerFromConfig(std::string const& appenderName)
{
    ...
    if (options.empty()) {
        fmt::print(
            stderr,
            "Log::CreateLoggerFromConfig: Missing config option Logger.{}\n",
            name);
        return;
    }
    ...
    if (logger) {
        fmt::print(stderr,
                   "Error while configuring Logger {}. Already defined\n",
                   name);
        return;
    }

    level =
        LogLevel(Acore::StringTo<uint8>(tokens[0]).value_or(LOG_LEVEL_INVALID));
    if (level > NUM_ENABLED_LOG_LEVELS) {
        fmt::print(
            stderr,
            "Log::CreateLoggerFromConfig: Wrong Log Level '{}' for logger {}\n",
            tokens[0],
            name);
        return;
    }
    ...
    for (std::string_view appendName : Acore::Tokenize(tokens[1], ' ', false)) {
        if (Appender* appender = GetAppenderByName(appendName)) {
            ...
            fmt::print(stderr,
                       "Error while configuring Appender {} in Logger {}. "
...
void Log::ReadLoggersFromConfig()
{
    ...
    if (loggers.find(LOGGER_ROOT) == loggers.end()) {
        fmt::print(
            ...
            "Creating default loggers [root (Error), server (Info)] to "
            "console\n");

        Close(); // Clean any Logger or Appender created

        AppenderConsole* appender = new AppenderConsole(NextAppenderId(),
                                                        "Console",
                                                        LOG_LEVEL_DEBUG,
                                                        APPENDER_FLAGS_NONE,
                                                        {});
...
{
    write(std::make_unique<LogMessage>(
        LOG_LEVEL_INFO, "commands.gm", message, param1));
}
...
void Log::write(std::unique_ptr<LogMessage>&& msg) const
{
    ...
    if (_ioContext) {
        ...
        Acore::Asio::post(
            *_ioContext, Acore::Asio::bind_executor(*_strand, [logOperation]() {
                logOperation->call();
            }));
    }
    else
...
{
    for (std::pair<uint8 const, std::unique_ptr<Appender>>& appender :
         appenders) {
...
bool Log::ShouldLog(std::string const& type, LogLevel level) const
{
    ...
    // Don't even look for a logger if the LogLevel is higher than the highest
    // log levels across all loggers
    if (level > highestLogLevel) {
        return false;
    }
