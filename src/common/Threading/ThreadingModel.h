...
class GeneralLock {
public:
    GeneralLock(MUTEX& m) : i_mutex(m) { i_mutex.lock(); }

    ~GeneralLock() { i_mutex.unlock(); }
...
private:
    ...
    ObjectLevelLockable<T, MUTEX>&
    operator=(const ObjectLevelLockable<T, MUTEX>&);
...
public:
    ...
    class Lock {
    public:
        Lock(const T& /*host*/) { ClassLevelLockable<T, MUTEX>::si_mtx.lock(); }
...
template <class T, class MUTEX>
MUTEX Acore::ClassLevelLockable<T, MUTEX>::si_mtx;
