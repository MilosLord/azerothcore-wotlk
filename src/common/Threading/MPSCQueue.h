...
public:
    ...
    {
        // _dummy is constructed from aligned_storage and is intentionally left
        // uninitialized (it might not be default constructible) so we init only
        // its IntrusiveLink here
        std::atomic<T*>* dummyNext =
            new (&(_dummyPtr->*IntrusiveLink)) std::atomic<T*>();
...
using MPSCQueue =
    std::conditional_t<IntrusiveLink != nullptr,
                       Acore::Impl::MPSCQueueIntrusive<T, IntrusiveLink>,
                       Acore::Impl::MPSCQueueNonIntrusive<T>>;
