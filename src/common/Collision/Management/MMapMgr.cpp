...
MMapMgr::~MMapMgr()
{
    for (MMapDataSet::iterator i = loadedMMaps.begin(); i != loadedMMaps.end();
         ++i) {
        delete i->second;
    }
...
{
    // the caller must pass the list of all mapIds that will be used in the
    // VMapMgr2 lifetime
...
bool MMapMgr::loadMapData(uint32 mapId)
{
    ...
    if (itr != loadedMMaps.end()) {
        ...
        if (thread_safe_environment) {
            itr = loadedMMaps.insert(MMapDataSet::value_type(mapId, nullptr))
                      .first;
        }
        else {
            ABORT("Invalid mapId {} passed to MMapMgr after startup in thread "
                  "unsafe environment",
                  mapId);
        }
    }
    ...
    std::string fileName = Acore::StringFormat(
        MAP_FILE_NAME_FORMAT,
        sConfigMgr->GetOption<std::string>("DataDir", ".").c_str(),
        mapId);

    FILE* file = fopen(fileName.c_str(), "rb");
    if (!file) {
        LOG_DEBUG("maps",
                  "MMAP:loadMapData: Error: Could not open mmap file '{}'",
                  fileName);
        return false;
    }
    ...
    if (count != 1) {
        LOG_DEBUG(
            "maps",
            "MMAP:loadMapData: Error: Could not read params from file '{}'",
            fileName);
        return false;
    }
    ...
    if (DT_SUCCESS != mesh->init(&params)) {
        dtFreeNavMesh(mesh);
        LOG_ERROR("maps",
                  "MMAP:loadMapData: Failed to initialize dtNavMesh for mmap "
                  "{:03} from file {}",
                  mapId,
                  fileName);
        return false;
...
uint32 MMapMgr::packTileID(int32 x, int32 y) { return uint32(x << 16 | y); }
...
bool MMapMgr::loadMap(uint32 mapId, int32 x, int32 y)
{
    ...
    if (mmap->loadedTileRefs.find(packedGridPos) !=
        mmap->loadedTileRefs.end()) {
        LOG_ERROR("maps",
                  "MMAP:loadMap: Asked to load already loaded navmesh tile. "
                  "{:03}{:02}{:02}.mmtile",
                  mapId,
                  x,
                  y);
    ...
    if (!file) {
        LOG_DEBUG(
            "maps", "MMAP:loadMap: Could not open mmtile file '{}'", fileName);
        return false;
    }
    ...
    if (fread(&fileHeader, sizeof(MmapTileHeader), 1, file) != 1 ||
        ...
        LOG_ERROR("maps",
                  "MMAP:loadMap: Bad header in mmap {:03}{:02}{:02}.mmtile",
                  mapId,
                  x,
                  y);
        fclose(file);
    ...
    if (fileHeader.mmapVersion != MMAP_VERSION) {
        LOG_ERROR("maps",
                  "MMAP:loadMap: {:03}{:02}{:02}.mmtile was built with "
                  "generator v{}, expected v{}",
                  mapId,
                  x,
                  y,
    ...
    if (!result) {
        LOG_ERROR(
            "maps",
            "MMAP:loadMap: Bad header or data in mmap {:03}{:02}{:02}.mmtile",
            mapId,
            x,
            y);
        fclose(file);
    ...
    if (dtStatusSucceed(mmap->navMesh->addTile(
        ...
        mmap->loadedTileRefs.insert(
            std::pair<uint32, dtTileRef>(packedGridPos, tileRef));
        ++loadedTiles;
        dtMeshHeader* header = (dtMeshHeader*)data;
        LOG_DEBUG("maps",
                  "MMAP:loadMap: Loaded mmtile {:03}[{:02},{:02}] into "
                  "{:03}[{:02},{:02}]",
                  mapId,
                  x,
                  y,
    ...
    LOG_ERROR(
        "maps",
        "MMAP:loadMap: Could not load {:03}{:02}{:02}.mmtile into navmesh",
        mapId,
        x,
        y);
    dtFree(data);
...
bool MMapMgr::unloadMap(uint32 mapId, int32 x, int32 y)
{
    ...
    if (dtStatusFailed(mmap->navMesh->removeTile(tileRef, nullptr, nullptr))) {
        ...
        // but no extra memory is used we cannot recover from this error -
        // assert out
        LOG_ERROR("maps",
                  "MMAP:unloadMap: Could not unload {:03}{:02}{:02}.mmtile "
                  "from navmesh",
                  mapId,
                  x,
                  y);
    ...
    --loadedTiles;
    LOG_DEBUG("maps",
              "MMAP:unloadMap: Unloaded mmtile {:03}[{:02},{:02}] from {:03}",
              mapId,
              x,
              y,
              mapId);
...
bool MMapMgr::unloadMap(uint32 mapId)
{
    ...
    if (itr == loadedMMaps.end() || !itr->second) {
        ...
        LOG_DEBUG(
            "maps",
            "MMAP:unloadMap: Asked to unload not loaded navmesh map {:03}",
            mapId);
        return false;
    }
...
bool MMapMgr::unloadMapInstance(uint32 mapId, uint32 instanceId)
{
    ...
    if (itr == loadedMMaps.end()) {
        // file may not exist, therefore not loaded
        LOG_DEBUG("maps",
                  "MMAP:unloadMapInstance: Asked to unload not loaded navmesh "
                  "map {:03}",
                  mapId);
        return false;
    }
    ...
    if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end()) {
        LOG_DEBUG("maps",
                  "MMAP:unloadMapInstance: Asked to unload not loaded "
                  "dtNavMeshQuery mapId {:03} instanceId {}",
                  mapId,
                  instanceId);
        return false;
    }
    ...
    LOG_DEBUG("maps",
              "MMAP:unloadMapInstance: Unloaded mapId {:03} instanceId {}",
              mapId,
              instanceId);
...
dtNavMeshQuery const* MMapMgr::GetNavMeshQuery(uint32 mapId, uint32 instanceId)
{
    ...
    if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end()) {
        ...
        if (mmap->navMeshQueries.find(instanceId) ==
            ...
            if (dtStatusFailed(query->init(mmap->navMesh, 1024))) {
                dtFreeNavMeshQuery(query);
                LOG_ERROR("maps",
                          "MMAP:GetNavMeshQuery: Failed to initialize "
                          "dtNavMeshQuery for mapId {:03} instanceId {}",
                          mapId,
                          instanceId);
                return nullptr;
            }
            ...
            LOG_DEBUG("maps",
                      "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId "
                      "{:03} instanceId {}",
                      mapId,
                      instanceId);
            mmap->navMeshQueries.insert(
                std::pair<uint32, dtNavMeshQuery*>(instanceId, query));
        }
    }
