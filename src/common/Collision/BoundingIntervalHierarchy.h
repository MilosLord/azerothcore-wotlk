...
public:
    ...
    void build(const PrimArray& primitives,
    ...
    void intersectRay(const G3D::Ray& r,
                      ...
                      bool            stopAtFirstHit) const
    {
        ...
        while (true) {
            while (true) {
                uint32 tn = tree[node];
                uint32 axis =
                    (tn & (3 << 30)) >> 30; // cppcheck-suppress integerOverflow
                bool BVH2 = tn & (1 << 29); // cppcheck-suppress integerOverflow
                int  offset =
                    tn & ~(7 << 29); // cppcheck-suppress integerOverflow
                if (!BVH2) {
                    if (axis < 3) {
                        // "normal" interior node
                        float tf =
                            (intBitsToFloat(tree[node + offsetFront[axis]]) -
                             org[axis]) *
                            invDir[axis];
                        float tb =
                            (intBitsToFloat(tree[node + offsetBack[axis]]) -
                             org[axis]) *
                            invDir[axis];
                        ...
                        if (tb > intervalMax) {
                            intervalMax =
                                (tf <= intervalMax) ? tf : intervalMax;
                            continue;
                        }
                        // ray passes through both nodes
                        // push back node
                        stack[stackPos].node = back;
                        stack[stackPos].tnear =
                            (tb >= intervalMin) ? tb : intervalMin;
                        ...
                        int n = tree[node + 1];
                        while (n > 0) {
                            bool hit = intersectCallback(
                                r, objects[offset], maxDist, stopAtFirstHit);
                    ...
                    float tf = (intBitsToFloat(tree[node + offsetFront[axis]]) -
                                org[axis]) *
                               invDir[axis];
                    float tb = (intBitsToFloat(tree[node + offsetBack[axis]]) -
                                org[axis]) *
                               invDir[axis];
                    node        = offset;
    ...
    void intersectPoint(const G3D::Vector3& p,
...
protected:
    ...
    void buildHierarchy(std::vector<uint32>& tempTree,
                        buildData&           dat,
                        BuildStats&          stats);

    void createNode(std::vector<uint32>& tempTree,
                    int                  nodeIndex,
                    uint32               left,
                    uint32               right) const
    {
        // write leaf node
        tempTree[nodeIndex + 0] =
            (3 << 30) | left; // cppcheck-suppress integerOverflow
        tempTree[nodeIndex + 1] = right - left + 1;
    }

    void subdivide(int                  left,
                   int                  right,
                   std::vector<uint32>& tempTree,
                   buildData&           dat,
                   AABound&             gridBox,
