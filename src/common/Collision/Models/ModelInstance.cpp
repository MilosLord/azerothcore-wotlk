                                 ...
                                 ModelIgnoreFlags ignoreFlags) const
{
    ...
    /*        std::cout << "ray from:" << pRay.origin().x << ", " <<
       pRay.origin().y << ", " << pRay.origin().z
                      << " dir:" << pRay.direction().x << ", " <<
       pRay.direction().y << ", " << pRay.direction().z
                      << " t/tmax:" << time << '/' << pMaxDist;
            std::cout << "\nBound lo:" << iBound.low().x << ", " <<
       iBound.low().y << ", " << iBound.low().z << " hi: "
                      << iBound.high().x << ", " << iBound.high().y << ", " <<
       iBound.high().z << std::endl; */
                                    ...
                                    LocationInfo&       info) const
{
    ...
    if (iModel->GetLocationInfo(pModel, zDirModel, zDist, info)) {
        ...
        // and for rotation matrices: Mat.inverse() == Mat.transpose()
        float world_Z = ((modelGround * iInvRot) * iScale + iPos).z;
        if (info.ground_Z < world_Z) // hm...could it be handled automatically
                                     // with zDist at intersection?
        {
            info.ground_Z    = world_Z;
...
bool ModelSpawn::readFromFile(FILE* rf, ModelSpawn& spawn)
{
    ...
    if (has_bound) // only WMOs have bound in MPQ, only available after
                   // computation
    {
        Vector3 bLow, bHigh;
    ...
    char nameBuff[500];
    if (nameLen >
        500) // file names should never be that long, must be file error
    {
