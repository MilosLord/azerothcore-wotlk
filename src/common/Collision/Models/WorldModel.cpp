...
namespace VMAP {
bool IntersectTriangle(const MeshTriangle&                  tri,
...
WmoLiquid::WmoLiquid(uint32         width,
                     uint32         height,
...
bool WmoLiquid::GetLiquidHeight(const Vector3& pos, float& liqHeight) const
{
    ...
    /* Tesselate tile to two triangles (not sure if client does it exactly like
      this)
    ...
    */
    ...
    {
        float sx =
            iHeight[tx + 1 + ty * rowOffset] - iHeight[tx + ty * rowOffset];
        float sy = iHeight[tx + 1 + (ty + 1) * rowOffset] -
                   iHeight[tx + 1 + ty * rowOffset];
        liqHeight = iHeight[tx + ty * rowOffset] + dx * sx + dy * sy;
    }
    else // case (b)
    {
        float sx = iHeight[tx + 1 + (ty + 1) * rowOffset] -
                   iHeight[tx + (ty + 1) * rowOffset];
        float sy =
            iHeight[tx + (ty + 1) * rowOffset] - iHeight[tx + ty * rowOffset];
        liqHeight = iHeight[tx + ty * rowOffset] + dx * sx + dy * sy;
    }
    return true;
}
...
uint32 WmoLiquid::GetFileSize()
{
    return 2 * sizeof(uint32) + sizeof(Vector3) + sizeof(uint32) +
           (iFlags ? ((iTilesX + 1) * (iTilesY + 1) * sizeof(float) +
                      iTilesX * iTilesY)
                   : sizeof(float));
}
...
void WmoLiquid::GetPosInfo(uint32&       tilesX,
                           uint32&       tilesY,
                           G3D::Vector3& corner) const
{
    tilesX = iTilesX;
    tilesY = iTilesY;
    corner = iCorner;
...
GroupModel::GroupModel(const GroupModel& other)
    : iBound(other.iBound), iMogpFlags(other.iMogpFlags),
      iGroupWMOID(other.iGroupWMOID), vertices(other.vertices),
      triangles(other.triangles), meshTree(other.meshTree), iLiquid(0)
{
    if (other.iLiquid) {
...
void GroupModel::setMeshData(std::vector<Vector3>&      vert,
...
bool GroupModel::writeToFile(FILE* wf)
{
    ...
    if (fwrite(&iBound, sizeof(G3D::AABox), 1, wf) != 1) {
        result = false;
    }
    ...
    if (!count) // models without (collision) geometry end here, unsure if they
                // are useful
    {
        return result;
    }
    ...
    if (result &&
        fwrite(&triangles[0], sizeof(MeshTriangle), count, wf) != count) {
        result = false;
    }
...
bool GroupModel::readFromFile(FILE* rf)
{
    ...
    if (result &&
        fread(&triangles[0], sizeof(MeshTriangle), count, rf) != count) {
        result = false;
    }
...
struct GModelRayCallback {
    GModelRayCallback(const std::vector<MeshTriangle>& tris,
    ...
    {
        bool result =
            IntersectTriangle(triangles[entry], vertices, ray, distance);
        if (result) {
            hit = true;
        }
        return hit;
...
bool GroupModel::IntersectRay(const G3D::Ray& ray,
                              float&          distance,
...
bool GroupModel::IsInsideObject(const Vector3& pos,
                                const Vector3& down,
                                float&         z_dist) const
{
...
void GroupModel::GetMeshData(std::vector<G3D::Vector3>& outVertices,
...
bool WorldModel::IntersectRay(const G3D::Ray&  ray,
                              float&           distance,
                              bool             stopAtFirstHit,
                              ModelIgnoreFlags ignoreFlags) const
{
    ...
    if ((ignoreFlags & ModelIgnoreFlags::M2) != ModelIgnoreFlags::Nothing) {
        // M2 models are not taken into account for LoS calculation if caller
        // requested their ignoring.
        if (Flags & MOD_M2) {
            return false;
        }
    }

    // small M2 workaround, maybe better make separate class with virtual
    // intersection funcs in any case, there's no need to use a bound tree if we
    // only have one submodel
...
public:
    WModelAreaCallback(const std::vector<GroupModel>& vals, const Vector3& down)
        : prims(vals.begin()), hit(vals.end()), minVol(G3D::inf()),
          zDist(G3D::inf()), zVec(down)
    {
    }
    ...
    void operator()(const Vector3& point, uint32 entry)
    {
        ...
        if (prims[entry].IsInsideObject(point, zVec, group_Z)) {
            ...
            printf("%10u %8X %7.3f, %7.3f, %7.3f | %7.3f, %7.3f, %7.3f | z=%f, "
                   "p_z=%f\n",
                   gm.GetWmoID(),
                   gm.GetMogpFlags(),
                   gm.GetBound().low().x,
...
bool WorldModel::IntersectPoint(const G3D::Vector3& p,
...
bool WorldModel::GetLocationInfo(const G3D::Vector3& p,
                                 const G3D::Vector3& down,
                                 float&              dist,
...
bool WorldModel::writeFile(const std::string& filename)
{
    ...
    if (count) {
        ...
        // if (result && fwrite(&chunkSize, sizeof(uint32), 1, wf) != 1) result
        // = false;
        if (result && fwrite(&count, sizeof(uint32), 1, wf) != 1) {
            result = false;
        }
...
bool WorldModel::readFile(const std::string& filename)
{
    ...
    if (result && readChunk(rf, chunk, "GMOD", 4)) {
        ...
        if (fread(&count, sizeof(uint32), 1, rf) != 1) {
            result = false;
        }
        if (result) {
            groupModels.resize(count);
        }
        // if (result && fread(&groupModels[0], sizeof(GroupModel), count, rf)
        // != count) result = false;
