...
struct GameobjectModelData {
    GameobjectModelData(char const*    name_,
                        uint32         nameLength,
...
void LoadGameObjectModelList(std::string const& dataPath)
{
    ...
    FILE* model_list_file =
        fopen((dataPath + "vmaps/" + VMAP::GAMEOBJECT_MODELS).c_str(), "rb");
    if (!model_list_file) {
    ...
    if (fread(magic, 1, 8, model_list_file) != 8 ||
        ...
        LOG_ERROR("maps",
                  "File '{}' has wrong header, expected {}.",
    ...
    LOG_INFO("server.loading",
             ">> Loaded {} GameObject Models in {} ms",
             uint32(model_list.size()),
...
bool GameObjectModel::initialize(
    std::unique_ptr<GameObjectModelOwnerBase> modelOwner,
    std::string const&                        dataPath)
{
    ...
    if (mdl_box == G3D::AABox::zero()) {
        LOG_ERROR("maps",
                  "GameObject model {} has zero bounds, loading skipped",
                  it->second.name);
        return false;
    }

    iModel = VMAP::VMapFactory::createOrGetVMapMgr()->acquireModelInstance(
        dataPath + "vmaps/",
        it->second.name,
        it->second.isWmo ? VMAP::ModelFlags::MOD_WORLDSPAWN
                         : VMAP::ModelFlags::MOD_M2);
    ...
    G3D::Matrix3 iRotation =
        G3D::Matrix3::fromEulerAnglesZYX(modelOwner->GetOrientation(), 0, 0);
...
GameObjectModel*
GameObjectModel::Create(std::unique_ptr<GameObjectModelOwnerBase> modelOwner,
                        std::string const&                        dataPath)
{
...
bool GameObjectModel::UpdatePosition()
{
    ...
    // ignore models with no bounds
    if (mdl_box == G3D::AABox::zero()) {
        // VMAP_ERROR_LOG("misc", "GameObject model %s has zero bounds, loading
        // skipped", it->second.name.c_str());
        return false;
    }

    iPos = owner->GetPosition();
    G3D::Matrix3 iRotation =
        G3D::Matrix3::fromEulerAnglesZYX(owner->GetOrientation(), 0, 0);
    iInvRot = iRotation.inverse();
