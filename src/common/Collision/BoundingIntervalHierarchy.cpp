                         ...
                         BuildStats&          stats)
{
    ...
    subdivide(
        0, dat.numPrims - 1, tempTree, dat, gridBox, nodeBox, 0, 1, stats);
}
                    ...
                    BuildStats&          stats)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < 3; i++) {
            if (nodeBox.hi[i] < gridBox.lo[i] ||
                nodeBox.lo[i] > gridBox.hi[i]) {
...
bool BIH::readFromFile(FILE* rf)
{
    ...
    return uint64(check) ==
           uint64(3 + 3 + 1 + 1 + uint64(treeSize) + uint64(count));
}
