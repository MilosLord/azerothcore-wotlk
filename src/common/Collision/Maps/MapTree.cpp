...
public:
    MapRayCallback(ModelInstance* val, ModelIgnoreFlags ignoreFlags)
        : prims(val), flags(ignoreFlags), hit(false)
    {
    }
    bool operator()(const G3D::Ray& ray,
                    uint32          entry,
                    float&          distance,
                    bool            StopAtFirstHit)
    {
        bool result =
            prims[entry].intersectRay(ray, distance, StopAtFirstHit, flags);
        if (result) {
            hit = true;
        }
        return result;
...
std::string
StaticMapTree::getTileFileName(uint32 mapID, uint32 tileX, uint32 tileY)
{
...
bool StaticMapTree::GetAreaInfo(Vector3& pos,
                                uint32&  flags,
                                int32&   adtId,
                                int32&   rootId,
                                int32&   groupId) const
...
bool StaticMapTree::GetLocationInfo(const Vector3& pos,
...
{
    if (iBasePath.length() > 0 && iBasePath[iBasePath.length() - 1] != '/' &&
...
and returns true. Else, pMaxDist is not modified and returns false;
*/

bool StaticMapTree::GetIntersectionTime(const G3D::Ray&  pRay,
                                        float&           pMaxDist,
                                        bool             StopAtFirstHit,
...
bool StaticMapTree::isInLineOfSight(const Vector3&   pos1,
                                    const Vector3&   pos2,
                                    ModelIgnoreFlags ignoreFlags) const
{
    float maxDist = (pos2 - pos1).magnitude();
    // return false if distance is over max float, in case of cheater
    // teleporting to the end of the universe
...
bool StaticMapTree::GetObjectHitPos(const Vector3& pPos1,
                                    const Vector3& pPos2,
                                    Vector3&       pResultHitPos,
...
LoadResult StaticMapTree::CanLoadMap(const std::string& vmapPath,
                                     uint32             mapID,
                                     uint32             tileX,
                                     uint32             tileY)
{
...
{
    // VMAP_DEBUG_LOG(LOG_FILTER_MAPS, "StaticMapTree::InitMap() : initializing
    // StaticMapTree '{}'", fname);
    bool        success  = false;
    ...
    ModelSpawn spawn;
#ifdef VMAP_DEBUG
    // LOG_DEBUG(LOG_FILTER_MAPS, "StaticMapTree::InitMap() : map isTiled: {}",
    // static_cast<uint32>(iIsTiled));
#endif
    if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn)) {
        WorldModel* model =
            vm->acquireModelInstance(iBasePath, spawn.name, spawn.flags);
        // VMAP_DEBUG_LOG(LOG_FILTER_MAPS, "StaticMapTree::InitMap() : loading
        // {}", spawn.name);
        if (model) {
            // assume that global model always is the first and only tree value
            // (could be improved...)
            ...
            success = false;
            // VMAP_ERROR_LOG(LOG_FILTER_GENERAL, "StaticMapTree::InitMap() :
            // could not acquire WorldModel pointer for '{}'", spawn.name);
        }
    }
...
{
    for (loadedSpawnMap::iterator i = iLoadedSpawns.begin();
         i != iLoadedSpawns.end();
         ++i) {
...
bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapMgr2* vm)
{
    if (!iIsTiled) {
        ...
        // tiles or not so we need "fake" tile loads to know when we can unload
        // map geometry
    ...
    if (!iTreeValues) {
        LOG_ERROR("maps",
                  "StaticMapTree::LoadMapTile() : tree has not been "
                  "initialized [{}, {}]",
                  tileX,
                  tileY);
        return false;
    ...
    if (tf) {
        ...
        for (uint32 i = 0; i < numSpawns && result; ++i) {
            ...
            if (result) {
                ...
                WorldModel* model = vm->acquireModelInstance(
                    iBasePath, spawn.name, spawn.flags);
                if (!model) {
                    LOG_ERROR("maps",
                              "StaticMapTree::LoadMapTile() : could not "
                              "acquire WorldModel pointer [{}, {}]",
                              tileX,
                              tileY);
                }
                ...
                if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1) {
                    if (!iLoadedSpawns.count(referencedVal)) {
                        ...
                        if (referencedVal > iNTreeValues) {
                            LOG_DEBUG("maps",
                                      "StaticMapTree::LoadMapTile() : invalid "
                                      "tree element ({}/{})",
                                      referencedVal,
                                      iNTreeValues);
                            continue;
...
#endif
                        iTreeValues[referencedVal] =
                            ModelInstance(spawn, model);
                        ...
                        if (iTreeValues[referencedVal].ID != spawn.ID) {
                            LOG_DEBUG("maps",
                                      "StaticMapTree::LoadMapTile() : trying "
                                      "to load wrong spawn in node");
                        }
                        else if (iTreeValues[referencedVal].name !=
                                 spawn.name) {
                            LOG_DEBUG("maps",
                                      "StaticMapTree::LoadMapTile() : name "
                                      "collision on GUID={}",
                                      spawn.ID);
                        }
#endif
    ...
    METRIC_EVENT("map_events",
                 "LoadMapTile",
                 "Map: " + std::to_string(iMapID) +
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapMgr2* vm)
{
    ...
    if (tile == iLoadedTiles.end()) {
        LOG_ERROR("maps",
                  "StaticMapTree::UnloadMapTile() : trying to unload "
                  "non-loaded tile - Map:{} X:{} Y:{}",
                  iMapID,
                  tileX,
                  tileY);
        return;
                ...
                if (result) {
                    ...
                    if (fread(&referencedNode, sizeof(uint32), 1, tf) != 1) {
                        ...
                        if (!iLoadedSpawns.count(referencedNode)) {
                            LOG_ERROR(
                                "maps",
                                "StaticMapTree::UnloadMapTile() : trying to "
                                "unload non-referenced model '{}' (ID:{})",
                                spawn.name,
                                spawn.ID);
                        }
