...
#define CELL_SIZE float(HGRID_MAP_SIZE / (float)CELL_NUMBER)
    ...
    struct Cell {
        ...
        {
            Cell c = {int(fx * (1.f / CELL_SIZE) + (CELL_NUMBER / 2)),
                      int(fy * (1.f / CELL_SIZE) + (CELL_NUMBER / 2))};
            return c;
        }
    ...
    void intersectRay(const G3D::Ray& ray,
                      ...
                      bool            stopAtFirstHit)
    {
        intersectRay(ray,
                     intersectCallback,
                     max_dist,
                     ray.origin() + ray.direction() * max_dist,
                     stopAtFirstHit);
    }
                      ...
                      bool                stopAtFirstHit)
    {
        ...
        if (cell == last_cell) {
            if (Node* node = nodes[cell.x][cell.y]) {
                node->intersectRay(
                    ray, intersectCallback, max_dist, stopAtFirstHit);
            }
            return;
        }
        ...
        // int Cycles =
        // std::max((int)ceilf(max_dist/tMaxX),(int)ceilf(max_dist/tMaxY)); int
        // i = 0;
    ...
    void intersectPoint(const G3D::Vector3& point,
    ...
    // Optimized verson of intersectRay function for rays with vertical
    // directions
    template <typename RayCallback>
    void intersectZAllignedRay(const G3D::Ray& ray,
                               RayCallback&    intersectCallback,
                               float&          max_dist)
    {
        Cell cell = Cell::ComputeCell(ray.origin().x, ray.origin().y);
