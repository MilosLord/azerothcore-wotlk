...
public:
    ...
    static Digest
    GetDigestOf(Container const& seed, uint8 const* data, size_t len)
    {
    ...
    static auto GetDigestOf(Container const& seed, Ts&&... pack)
        -> std::enable_if_t<!(std::is_integral_v<std::decay_t<Ts>> || ...),
                            Digest>
    {
        GenericHMAC hash(seed);
    ...
    GenericHMAC(GenericHMAC&& right) noexcept { *this = std::move(right); }
    ...
    GenericHMAC& operator=(GenericHMAC const& right)
    {
        ...
        ASSERT(result == 1);
        _key = right._key; // EVP_PKEY uses reference counting internally, just
                           // copy the pointer
        EVP_PKEY_up_ref(_key); // Bump reference count for PKEY, as every
                               // instance of this class holds two references to
...
using HMAC_SHA1 =
    Acore::Impl::GenericHMAC<EVP_sha1, Constants::SHA1_DIGEST_LENGTH_BYTES>;
using HMAC_SHA256 =
    Acore::Impl::GenericHMAC<EVP_sha256, Constants::SHA256_DIGEST_LENGTH_BYTES>;
} // namespace Acore::Crypto
