...
/*static*/ std::array<uint8, 32> const SRP6::N = HexStrToByteArray<32>(
...
SRP6::MakeRegistrationData(std::string const& username,
...
/*static*/ SRP6::Verifier SRP6::CalculateVerifier(std::string const& username,
                                                  std::string const& password,
...
/*static*/ SessionKey SRP6::SHA1Interleave(SRP6::EphemeralKey const& S)
{
    ...
    SHA1::Digest const hash0 =
        SHA1::GetDigestOf(buf0.data() + p, EPHEMERAL_KEY_LENGTH / 2 - p);
    SHA1::Digest const hash1 =
        SHA1::GetDigestOf(buf1.data() + p, EPHEMERAL_KEY_LENGTH / 2 - p);
...
SRP6::SRP6(std::string const& username,
    ...
    : _I(SHA1::GetDigestOf(username)), _b(Crypto::GetRandomBytes<32>()),
      _v(verifier), s(salt), B(_B(_b, _v))
{
}

std::optional<SessionKey>
SRP6::VerifyChallengeResponse(EphemeralKey const& A,
                              SHA1::Digest const& clientM)
{
    ...
    EphemeralKey const S =
        (_A * (_v.ModExp(u, _N))).ModExp(_b, N).ToByteArray<32>();
