...
/*static*/ uint32 Acore::Crypto::TOTP::GenerateToken(Secret const& secret,
                                                     time_t        timestamp)
{
    ...
    uint32 truncated = (digest[offset] << 24) | (digest[offset + 1] << 16) |
                       (digest[offset + 2] << 8) | (digest[offset + 3]);
    truncated &= 0x7FFFFFFF;
    return (truncated % 1000000);
}

/*static*/ bool Acore::Crypto::TOTP::ValidateToken(Secret const& secret,
                                                   uint32        token)
{
    time_t now = GetEpochTime().count();
    return ((token == GenerateToken(secret, now - TOTP_INTERVAL)) ||
