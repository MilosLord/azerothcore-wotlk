...
class RefMgr : public LinkedListHead {
public:
    ...
    [[nodiscard]] Reference<TO, FROM> const* getFirst() const
    {
    ...
    [[nodiscard]] Reference<TO, FROM> const* getLast() const
    {
    ...
    void clearReferences()
    {
        ...
        while ((ref = getFirst()) != nullptr) {
            ...
            ref->delink(); // the delink might be already done by invalidate(),
                           // but doing it here again does not hurt and insures
                           // an empty list
        }
    }
};
