        ...
        if (GetModuleFileName(0, path, sizeof(path) / sizeof(path[0])) > 0) {
            ...
            if (service) {
                ...
                CSD_T ChangeService_Config2 =
                    (CSD_T)GetProcAddress(advapi32, "ChangeServiceConfig2A");
...
bool WinServiceUninstall()
{
    ...
    if (serviceControlMgr) {
        SC_HANDLE service = OpenService(
            serviceControlMgr, serviceName, SERVICE_QUERY_STATUS | DELETE);
        if (service) {
...
void WINAPI ServiceMain(DWORD argc, char* argv[])
{
    ...
    serviceStatusHandle =
        RegisterServiceCtrlHandler(serviceName, ServiceControlHandler);
...
bool WinServiceRun()
{
    ...
    if (!StartServiceCtrlDispatcher(serviceTable)) {
        LOG_ERROR(
            "server", "StartService Failed. Error [{}]", ::GetLastError());
        return false;
    }
    return true;
}
