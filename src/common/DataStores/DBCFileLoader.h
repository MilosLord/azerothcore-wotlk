...
public:
    ...
    public:
        [[nodiscard]] float getFloat(size_t field) const
        {
            ...
            float val =
                *reinterpret_cast<float*>(offset + file.GetOffset(field));
            EndianConvert(val);
            return val;
        }
        ...
        [[nodiscard]] uint32 getUInt(size_t field) const
        {
            ASSERT(field < file.fieldCount);
            uint32 val =
                *reinterpret_cast<uint32*>(offset + file.GetOffset(field));
            EndianConvert(val);
            return val;
        }
    ...
    [[nodiscard]] uint32 GetOffset(size_t id) const
    {
        return (fieldsOffset != nullptr && id < fieldCount) ? fieldsOffset[id]
                                                            : 0;
    }
