...
uint32 DBCFileLoader::GetFormatRecordSize(char const* format, int32* index_pos)
{
    ...
    for (uint32 x = 0; format[x]; ++x) {
        ...
        case FT_LOGIC:
            ASSERT(
                false &&
                "Attempted to load DBC files that do not have field types that "
                "match what is in the core. Check DBCfmt.h or your DBC files.");
            break;
        default:
...
char* DBCFileLoader::AutoProduceData(char const* format,
                                     uint32&     records,
                                     char**&     indexTable)
{
    /*
        ...
        for (uint32 x = 0; x < fieldCount; ++x) {
            ...
            case FT_STRING:
                *((char**)(&dataTable[offset])) =
                    nullptr; // will replace non-empty or "" strings in
                             // AutoProduceStrings
                offset += sizeof(char*);
                break;
            case FT_LOGIC:
                ASSERT(false && "Attempted to load DBC files that do not have "
                                "field types that match what is in the core. "
                                "Check DBCfmt.h or your DBC files.");
                break;
            case FT_NA:
            case FT_NA_BYTE:
            case FT_SORT:
...
char* DBCFileLoader::AutoProduceStrings(char const* format, char* dataTable)
{
    ...
    for (uint32 y = 0; y < recordCount; ++y) {
        for (uint32 x = 0; x < fieldCount; ++x) {
            ...
            case FT_LOGIC:
                ASSERT(false && "Attempted to load DBC files that does not "
                                "have field types that match what is in the "
                                "core. Check DBCfmt.h or your DBC files.");
                break;
            case FT_NA:
            case FT_NA_BYTE:
            case FT_SORT:
