...
AC_COMMON_API int StartProcess(std::string const&              executable,
...
/// returns an AsyncProcessResult immediately which is set, when the process
/// exits. When an input path is given, the file will be routed to the processes
/// stdin. When the process is marked as secure no arguments are leaked to logs.
/// Note that most executables expect it's name as the first argument.
