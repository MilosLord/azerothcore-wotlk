...
struct Finder {
    ...
    {
        return obj.second.*idMember_ == val_;
    }
};
...
AC_COMMON_API bool
Utf8toWStr(char const* utf8str, size_t csize, wchar_t* wstr, size_t& wsize);
...
AC_COMMON_API bool
WStrToUtf8(wchar_t const* wstr, size_t size, std::string& utf8str);
...
{
    if (wchar >= L'a' &&
        wchar <= L'z') // LATIN SMALL LETTER A - LATIN SMALL LETTER Z
    {
        return true;
    }
    if (wchar >= L'A' &&
        wchar <= L'Z') // LATIN CAPITAL LETTER A - LATIN CAPITAL LETTER Z
    {
        return true;
    }
    return false;
...
inline bool isExtendedLatinCharacter(wchar_t wchar)
{
    ...
    if (wchar >= 0x00C0 &&
        wchar <= 0x00D6) // LATIN CAPITAL LETTER A WITH GRAVE - LATIN CAPITAL
                         // LETTER O WITH DIAERESIS
    {
        return true;
    }
    if (wchar >= 0x00D8 &&
        wchar <= 0x00DE) // LATIN CAPITAL LETTER O WITH STROKE - LATIN CAPITAL
                         // LETTER THORN
    {
        return true;
    }
    ...
    if (wchar >= 0x00E0 &&
        wchar <= 0x00F6) // LATIN SMALL LETTER A WITH GRAVE - LATIN SMALL LETTER
                         // O WITH DIAERESIS
    {
        return true;
    }
    if (wchar >= 0x00F8 && wchar <= 0x00FE) // LATIN SMALL LETTER O WITH STROKE
                                            // - LATIN SMALL LETTER THORN
    {
        return true;
    }
    if (wchar >= 0x0100 &&
        wchar <= 0x012F) // LATIN CAPITAL LETTER A WITH MACRON - LATIN SMALL
...
{
    if (wchar >= 0x0410 &&
        wchar <= 0x044F) // CYRILLIC CAPITAL LETTER A - CYRILLIC SMALL LETTER YA
    {
        return true;
    }
    if (wchar == 0x0401 ||
        wchar == 0x0451) // CYRILLIC CAPITAL LETTER IO, CYRILLIC SMALL LETTER IO
    {
        return true;
    }
    return false;
...
inline bool IsEvenNumber(int32 n) { return n % 2 == 0; }
...
{
    for (wchar_t i : wstr)
        if (!isBasicLatinCharacter(i) &&
            (!numericOrSpace || !isNumericOrSpace(i))) {
            return false;
        }
    return true;
}
...
inline wchar_t wcharToUpper(wchar_t wchar)
{
    ...
    if (wchar >= 0x0101 &&
        wchar <= 0x012F) // LATIN SMALL LETTER A WITH MACRON - LATIN SMALL
                         // LETTER I WITH OGONEK (only %2=1)
    {
        if (wchar % 2 == 1) {
    ...
    if (wchar >= 0x0430 &&
        wchar <= 0x044F) // CYRILLIC SMALL LETTER A - CYRILLIC SMALL LETTER YA
    {
        return wchar_t(uint16(wchar) - 0x0020);
    }
...
inline wchar_t wcharToLower(wchar_t wchar)
{
    ...
    if (wchar >= 0x0100 &&
        wchar <= 0x012E) // LATIN CAPITAL LETTER A WITH MACRON - LATIN CAPITAL
                         // LETTER I WITH OGONEK (only %2=0)
    {
        if (wchar % 2 == 0) {
            return wchar_t(uint16(wchar) + 0x0001);
        }
    ...
    if (wchar >= 0x0410 &&
        wchar <=
            0x042F) // CYRILLIC CAPITAL LETTER A - CYRILLIC CAPITAL LETTER YA
    {
        return wchar_t(uint16(wchar) + 0x0020);
    }
...
AC_COMMON_API std::string
ByteArrayToHexStr(uint8 const* bytes, size_t length, bool reverse = false);
AC_COMMON_API void HexStrToByteArray(std::string_view str,
                                     uint8*           out,
                                     size_t           outlen,
                                     bool             reverse = false);
} // namespace Acore::Impl
...
inline bool StringStartsWithI(std::string_view haystack,
...
template <typename T>
inline bool ValueContainsStringI(std::pair<T, std::string_view> const& haystack,
                                 std::string_view                      needle)
{
...
public:
    ...
    IsEqual(uint32 p1 = 0, uint32 p2 = 0, uint32 p3 = 0) const
    {
        return (part[0] == p1 && part[1] == p2 && part[2] == p3);
    }

    [[nodiscard]] inline bool
    HasFlag(uint32 p1 = 0, uint32 p2 = 0, uint32 p3 = 0) const
    {
        return (part[0] & p1 || part[1] & p2 || part[2] & p3);
    }
    ...
    inline bool operator==(flag96 const& right) const
    {
        return (part[0] == right.part[0] && part[1] == right.part[1] &&
                part[2] == right.part[2]);
    }
    ...
    {
        return flag96(part[0] & right.part[0],
                      part[1] & right.part[1],
                      part[2] & right.part[2]);
    }
    ...
    inline uint32 const& operator[](uint8 el) const { return part[el]; }
};
...
template <typename T>
std::enable_if_t<!std::is_same_v<std::decay_t<T>, std::type_info>, std::string>
GetTypeName(T&& v)
{
    return GetTypeName(typeid(v));
}
