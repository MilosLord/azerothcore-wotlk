...
/* Return a random millisecond value between min and max seconds. Functionally
 * equivalent to urand(min*IN_MILLISECONDS, max*IN_MILLISECONDS). */
 ...
 * Only works for values where millisecond difference is a valid uint32. */
...
/* Return a random number in the range 0..count (exclusive) with each value
 * having a different chance of happening */
...
/*
 * Wrapper satisfying UniformRandomNumberGenerator concept for use in <random>
 * algorithms
 */
