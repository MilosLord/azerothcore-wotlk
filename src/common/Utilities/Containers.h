...
// if it has more than <requestedSize> elements, the elements to keep are
// selected randomly
template <class C>
void RandomResize(C& container, std::size_t requestedSize)
{
    static_assert(
        std::is_base_of<std::forward_iterator_tag,
                        typename std::iterator_traits<
                            typename C::iterator>::iterator_category>::value,
        "Invalid container passed to Acore::Containers::RandomResize");
    ...
    uint32 elementsToKeep    = requestedSize,
           elementsToProcess = std::size(container);

    while (elementsToProcess) {
        // this element has chance (elementsToKeep / elementsToProcess) of being
        // kept
                  ...
                  std::size_t requestedSize)
{
    ...
    std::copy_if(std::begin(container),
                 std::end(container),
                 std::inserter(containerCopy, std::end(containerCopy)),
                 predicate);
 ...
 */
template <class C>
inline auto SelectRandomContainerElement(C const& container) ->
 ...
 */
template <class C, class Predicate>
inline auto SelectRandomContainerElementIf(C const&    container,
                                           Predicate&& predicate) ->
    typename std::add_const<decltype(*std::begin(container))>::type&
{
    C containerCopy;
...
/*
 * Select a random element from a container where each element has a different
 * chance to be selected.
 *
 * @param container Container to select an element from
 * @param weights Chances of each element to be selected, must be in the same
 * order as elements in container. Caller is responsible for checking that sum
 * of all weights is greater than 0.
 *
 * Note: container cannot be empty
 */
template <class C>
inline auto SelectRandomWeightedContainerElement(C const&            container,
 ...
 *
 * @param container Container to select an element from
 * @param weightExtractor Function retrieving chance of each element in
 * container, expected to take an element of the container and returning a
 * double
 *
 * Note: container cannot be empty
 */
template <class C, class Fn>
auto SelectRandomWeightedContainerElement(C const& container,
                                          Fn       weightExtractor)
    -> decltype(std::begin(container))
{
    std::vector<double> weights;
...
/**
 * Returns a pointer to mapped value (or the value itself if map stores
 * pointers)
 */
template <class M>
...
{
    std::shuffle(
        std::begin(container), std::end(container), RandomEngine::Instance());
}
...
std::enable_if_t<
    std::is_move_assignable_v<decltype(*std::declval<Container>().begin())>,
    void>
EraseIf(Container& c, Predicate p)
{
    auto wpos = c.begin();
