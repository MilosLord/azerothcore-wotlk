...
public:
    ...
    ACLogSink(T callback) : callback_(std::move(callback)) {}
...
template <typename T>
static int CreateChildProcess(T                               waiter,
...
/*
  Silence warning with boost 1.83
    boost/process/pipe.hpp(132,5): warning C4297:
  'boost::process::basic_pipebuf<char,std::char_traits<char>>::~basic_pipebuf':
  function assumed not to throw an exception but does
    boost/process/pipe.hpp(132,5): message : destructor or deallocator has a
  (possibly implicit) non-throwing exception specification
    boost/process/pipe.hpp(124,6): message : while compiling class template
  member function
  'boost::process::basic_pipebuf<char,std::char_traits<char>>::~basic_pipebuf(void)'
    boost/process/pipe.hpp(304,42): message : see reference to class template
  instantiation 'boost::process::basic_pipebuf<char,std::char_traits<char>>'
  being compiled
*/
#endif
    ipstream outStream;
...
#pragma warning(pop)
    ...
    if (!secure) {
        LOG_TRACE(logger,
                  ...
                  executable,
                  boost::algorithm::join(argsVector, " "));
    }

    // prepare file with only read permission (boost process opens with
    // read_write)
    std::shared_ptr<FILE> inputFile(
        !input.empty() ? fopen(input.c_str(), "rb") : nullptr, [](FILE* ptr) {
            if (ptr != nullptr)
                fclose(ptr);
        });
    ...
    child c = [&]() {
        if (inputFile) {
            // With binding stdin
            return child{exe  = std::filesystem::absolute(executable).string(),
                         args = argsVector,
    ...
    auto outInfo = MakeACLogSink(
        [&](std::string const& msg) { LOG_INFO(logger, "{}", msg); });

    auto outError = MakeACLogSink(
        [&](std::string const& msg) { LOG_ERROR(logger, "{}", msg); });
...
int StartProcess(std::string const&              executable,
...
class AsyncProcessResultImplementation : public AsyncProcessResult {
    std::string const              executable;
    std::vector<std::string> const args;
    std::string const              logger;
...
public:
    explicit AsyncProcessResultImplementation(std::string executable_,
    ...
    AsyncProcessResultImplementation(AsyncProcessResultImplementation const&) =
        delete;
    AsyncProcessResultImplementation&
    operator=(AsyncProcessResultImplementation const&) = delete;
    AsyncProcessResultImplementation(AsyncProcessResultImplementation&&) =
        delete;
    AsyncProcessResultImplementation&
    operator=(AsyncProcessResultImplementation&&) = delete;
...
StartAsyncProcess(std::string              executable,
                  ...
                  bool                     secure)
{
    ...
    handle->SetFuture(std::async(std::launch::async,
                                 [handle] { return handle->StartProcess(); }));
    return handle;
}
