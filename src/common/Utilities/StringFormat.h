...
inline std::string StringFormatFmt(FormatString<Args...> fmt, Args&&... args)
{
    try {
        ...
        return fmt::format("Wrong format occurred ({}). Fmt string: '{}'",
                           e.what(),
                           fmt.get());
    }
}
...
inline bool IsFormatEmptyOrNull(std::string_view fmt) { return fmt.empty(); }
} // namespace Acore
...
AC_COMMON_API std::string AddSuffixIfNotExists(std::string str,
                                               const char  suffix);
} // namespace Acore::String
