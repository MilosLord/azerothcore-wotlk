...
public:
     ...
     */
    [[nodiscard]] uint32 GetTimer() const { return _time; }

    void SetTimer(uint32 time) { _time = time; }

    /**
     * @name GetPhaseMask
     * @return Active phases as mask.
     */
    [[nodiscard]] uint8 GetPhaseMask() const { return _phase; }

    /**
     * @name Empty
     * @return True, if there are no events scheduled.
     */
    [[nodiscard]] bool Empty() const { return _eventMap.empty(); }
     ...
     * @param group The group which the event is associated to. Has to be
     * between 1 and 8. 0 means it has no group.
     * @param phase The phase in which the event can occur. Has to be between 1
     * and 8. 0 means it can occur in all phases.
     */
    void ScheduleEvent(uint32 eventId,
                       uint32 time,
                       uint32 group = 0,
                       uint32 phase = 0);
     ...
     * @brief Schedules a new event. An existing event is not canceled.
     * @param eventId The id of the new event.
     * @param minTime The minimum time until the event occurs as std::chrono
     * type.
     * @param maxTime The maximum time until the event occurs as std::chrono
     * type.
     * @param group The group which the event is associated to. Has to be
     * between 1 and 8. 0 means it has no group.
     * @param phase The phase in which the event can occur. Has to be between 1
     ...
     * and 8. 0 means it can occur in all phases.
     */
    void RescheduleEvent(uint32 eventId,
                         uint32 time,
                         uint32 groupId = 0,
                         uint32 phase   = 0);
    ...
    */
    ...
    /**
     * @name DelayEvents
     * @brief Delays all events in the map. If delay is greater than or equal
     * internal timer, delay will be 0.
     ...
     * @return Time of next event. If event is not scheduled returns
     * Milliseconds::max()
     */
