...
void TaskScheduler::Dispatch(success_t const& callback)
{
    ...
    while (!_task_holder.IsEmpty()) {
        ...
        TaskContext context(_task_holder.Pop(),
...
void TaskScheduler::TaskQueue::Clear() { container.clear(); }

void TaskScheduler::TaskQueue::RemoveIf(
    std::function<bool(TaskContainer const&)> const& filter)
{
...
bool TaskScheduler::TaskQueue::IsEmpty() const { return container.empty(); }

TaskContext& TaskContext::Dispatch(
    std::function<TaskScheduler&(TaskScheduler&)> const& apply)
{
...
bool TaskContext::IsExpired() const { return _owner.expired(); }
...
{
    return Dispatch(
        std::bind(&TaskScheduler::Async, std::placeholders::_1, callable));
}
...
{
    return Dispatch(
        std::bind(&TaskScheduler::CancelGroup, std::placeholders::_1, group));
}

TaskContext&
TaskContext::CancelGroupsOf(std::vector<TaskScheduler::group_t> const& groups)
{
    return Dispatch(std::bind(&TaskScheduler::CancelGroupsOf,
                              std::placeholders::_1,
                              std::cref(groups)));
}
...
void TaskContext::AssertOnConsumed() const
{
    ...
    // If you encounter this assertion check if you repeat a TaskContext more
    // then 1 time!
    ASSERT(!(*_consumed) &&
           "Bad task logic, task context was consumed already!");
}

void TaskContext::Invoke() { _task->_task(*this); }
