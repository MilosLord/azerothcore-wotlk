...
/// The TaskScheduler class provides the ability to schedule std::function's in
/// the near future. Use TaskScheduler::Update to update the scheduler. Popular
/// methods are:
/// * Schedule (Schedules a std::function which will be executed in the near
/// future).
/// * Schedules an asynchronous function which will be executed at the next
/// update tick.
/// * Cancel, Delay & Reschedule (Methods to manipulate already scheduled
/// tasks). Tasks are organized in groups (uint), multiple tasks can have the
/// same group id, you can provide a group or not, but keep in mind that you can
/// only manipulate specific tasks through its group id! Tasks callbacks use the
/// function signature void(TaskContext) where TaskContext provides access to
/// the function schedule plan which makes it possible to repeat the task with
/// the same duration or a new one. It also provides access to the repeat
/// counter which is useful for task that repeat itself often but behave
...
class TaskScheduler {
    ...
    public:
        ...
        Task(timepoint_t const&    end,
             duration_t const&     duration,
             task_handler_t const& task)
            : _end(end), _duration(duration), _group(std::nullopt),
              _repeated(0), _task(task)
        {
        }
        ...
        inline bool operator==(Task const& other) { return _end == other._end; }
    ...
    struct Compare {
        bool operator()(TaskContainer const& left,
                        TaskContainer const& right) const
        {
    ...
    static bool EmptyValidator() { return true; }
...
public:
    ...
    /// Never call this from within a task context! Use TaskContext::Schedule
    /// instead!
    template <class _Rep, class _Period>
    ...
    /// Never call this from within a task context! Use TaskContext::Schedule
    /// instead!
    template <class _RepLeft,
              class _PeriodLeft,
              class _RepRight,
              class _PeriodRight>
    TaskScheduler&
    Schedule(std::chrono::duration<_RepLeft, _PeriodLeft> const&   min,
    ...
    /// Cancels all tasks.
    /// Never call this from within a task context! Use TaskContext::CancelAll
    /// instead!
    TaskScheduler& CancelAll();

    /// Cancel all tasks of a single group.
    /// Never call this from within a task context! Use TaskContext::CancelGroup
    /// instead!
    ...
    template <class _Rep, class _Period>
    TaskScheduler&
    DelayAll(std::chrono::duration<_Rep, _Period> const& duration)
    {
    ...
    template <class _RepLeft,
              ...
              class _PeriodRight>
    TaskScheduler&
    DelayAll(std::chrono::duration<_RepLeft, _PeriodLeft> const&   min,
             std::chrono::duration<_RepRight, _PeriodRight> const& max)
    {
    ...
    template <class _Rep, class _Period>
    TaskScheduler&
    DelayGroup(group_t const                               group,
               std::chrono::duration<_Rep, _Period> const& duration)
    {
        _task_holder.ModifyIf(
            [&duration, group](TaskContainer const& task) -> bool {
    ...
    template <class _Rep, class _Period>
    TaskScheduler&
    RescheduleAll(std::chrono::duration<_Rep, _Period> const& duration)
    {
    ...
    template <class _RepLeft,
              ...
              class _PeriodRight>
    TaskScheduler&
    RescheduleAll(std::chrono::duration<_RepLeft, _PeriodLeft> const&   min,
                  std::chrono::duration<_RepRight, _PeriodRight> const& max)
    {
    ...
    template <class _Rep, class _Period>
    TaskScheduler&
    RescheduleGroup(group_t const                               group,
                    std::chrono::duration<_Rep, _Period> const& duration)
    {
        auto const end = _now + duration;
    ...
    /// Reschedule all tasks of a group with a random duration between min and
    /// max.
    template <class _RepLeft,
              class _PeriodLeft,
              class _RepRight,
...
private:
                              ...
                              task_handler_t const&                       task)
    {
        ...
        return InsertTask(TaskContainer(
    ...
    template <class _RepLeft,
              ...
              class _RepRight,
              class _PeriodRight>
    static std::chrono::milliseconds RandomDurationBetween(
        std::chrono::duration<_RepLeft, _PeriodLeft> const&   min,
        std::chrono::duration<_RepRight, _PeriodRight> const& max)
    {
        auto const milli_min =
            std::chrono::duration_cast<std::chrono::milliseconds>(min);
        auto const milli_max =
            std::chrono::duration_cast<std::chrono::milliseconds>(max);

        // TC specific: use SFMT URandom
        return std::chrono::milliseconds(
            urand(uint32(milli_min.count()), uint32(milli_max.count())));
    }
...
class TaskContext {
    ...
    TaskContext&
    Dispatch(std::function<TaskScheduler&(TaskScheduler&)> const& apply);

public:
    // Empty constructor
    TaskContext() : _task(), _owner(), _consumed(std::make_shared<bool>(true))
    {
    }

    // Construct from task and owner
    explicit TaskContext(TaskScheduler::TaskContainer&& task,
    ...
    /// Returns the repeat counter which increases every time the task is
    /// repeated.
    TaskScheduler::repeated_t GetRepeatCounter() const;

    /// Repeats the event and sets a new duration.
    /// std::chrono::seconds(5) for example.
    /// This will consume the task context, its not possible to repeat the task
    /// again from the same task context!
    template <class _Rep, class _Period>
    TaskContext& Repeat(std::chrono::duration<_Rep, _Period> const& duration)
    {
        ...
        return Dispatch(std::bind(
            &TaskScheduler::InsertTask, std::placeholders::_1, _task));
    }

    /// Repeats the event with the same duration.
    /// This will consume the task context, its not possible to repeat the task
    /// again from the same task context!
    TaskContext& Repeat() { return Repeat(_task->_duration); }

    /// Repeats the event and set a new duration that is randomized between min
    /// and max. std::chrono::seconds(5) for example. This will consume the task
    /// context, its not possible to repeat the task again from the same task
    /// context!
    template <class _RepLeft,
              class _PeriodLeft,
              class _RepRight,
              class _PeriodRight>
    TaskContext&
    Repeat(std::chrono::duration<_RepLeft, _PeriodLeft> const&   min,
           std::chrono::duration<_RepRight, _PeriodRight> const& max)
    {
        return Repeat(TaskScheduler::RandomDurationBetween(min, max));
    }

    /// Schedule a callable function that is executed at the next update tick
    /// from within the context. Its safe to modify the TaskScheduler from
    /// within the callable.
                          ...
                          TaskScheduler::task_handler_t const&        task)
    {
        ...
        return Dispatch(
            [end, time, task](TaskScheduler& scheduler) -> TaskScheduler& {
    ...
    {
        auto const end = _task->_end;
        return Dispatch([end, time, group, task](
                            TaskScheduler& scheduler) -> TaskScheduler& {
            return scheduler.ScheduleAt<_Rep, _Period>(end, time, group, task);
        });
    }

    /// Schedule an event with a randomized rate between min and max rate from
    /// within the context. Its possible that the new event is executed
    /// immediately! Use TaskScheduler::Async to create a task which will be
    /// called at the next update tick.
    template <class _RepLeft,
              class _PeriodLeft,
              class _RepRight,
              class _PeriodRight>
    ...
    {
        return Schedule(
            TaskScheduler::RandomDurationBetween(min, max), group, task);
    }
    ...
    /// Hint: Use std::initializer_list for this: "{1, 2, 3, 4}"
    TaskContext&
    CancelGroupsOf(std::vector<TaskScheduler::group_t> const& groups);

    /// Delays all tasks with the given duration from within the context.
    template <class _Rep, class _Period>
    TaskContext& DelayAll(std::chrono::duration<_Rep, _Period> const& duration)
    {
        return Dispatch(std::bind(&TaskScheduler::DelayAll<_Rep, _Period>,
                                  std::placeholders::_1,
                                  duration));
    }

    /// Delays all tasks with a random duration between min and max from within
    /// the context.
    template <class _RepLeft,
              class _PeriodLeft,
              class _RepRight,
    ...
    /// Delays all tasks of a group with the given duration from within the
    /// context.
    template <class _Rep, class _Period>
    TaskContext&
    DelayGroup(TaskScheduler::group_t const                group,
               std::chrono::duration<_Rep, _Period> const& duration)
    {
        return Dispatch(std::bind(&TaskScheduler::DelayGroup<_Rep, _Period>,
                                  std::placeholders::_1,
                                  group,
                                  duration));
    }
    ...
    DelayGroup(TaskScheduler::group_t const                          group,
               ...
               std::chrono::duration<_RepRight, _PeriodRight> const& max)
    {
        return DelayGroup(group,
                          TaskScheduler::RandomDurationBetween(min, max));
    }
    ...
    {
        return Dispatch(std::bind(
            &TaskScheduler::RescheduleAll, std::placeholders::_1, duration));
    }
    ...
    template <class _Rep, class _Period>
    TaskContext&
    RescheduleGroup(TaskScheduler::group_t const                group,
                    std::chrono::duration<_Rep, _Period> const& duration)
    {
        return Dispatch(
            std::bind(&TaskScheduler::RescheduleGroup<_Rep, _Period>,
                      std::placeholders::_1,
                      group,
                      duration));
    }
