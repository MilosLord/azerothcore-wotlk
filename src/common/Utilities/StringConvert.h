...
struct For<
    ...
    {
        std::string buf(20, '\0'); /* 2^64 is 20 decimal characters, -(2^63) is
                                      20 including the sign */
...
struct For<bool, void> {
    ...
    FromString(std::string_view str,
               int strict = 0) /* this is int to match the signature for
                                  "proper" integral types */
    {
        if (strict) {
            if (str == "1") {
                return true;
            ...
            if ((str == "1") || StringEqualI(str, "y") ||
                StringEqualI(str, "on") || StringEqualI(str, "yes") ||
                StringEqualI(str, "true")) {
                return true;
            }
            if ((str == "0") || StringEqualI(str, "n") ||
                StringEqualI(str, "off") || StringEqualI(str, "no") ||
                StringEqualI(str, "false")) {
                return false;
            }
            return std::nullopt;
        }
    ...
    static std::string ToString(bool val) { return (val ? "1" : "0"); }
};
...
struct For<T, std::enable_if_t<std::is_floating_point_v<T>>> {
                                  ...
                                  std::chars_format fmt = std::chars_format())
    {
        ...
        T                            val;
        std::from_chars_result const res =
            std::from_chars(start, end, val, fmt);
        if ((res.ptr == end) && (res.ec == std::errc())) {
            return val;
        }
        else {
    ...
    static std::string ToString(T val) { return std::to_string(val); }
};
#else
...
{
    return Acore::Impl::StringConvertImpl::For<Result>::FromString(
        str, std::forward<Params>(params)...);
}

template <typename Type, typename... Params>
std::string ToString(Type&& val, Params&&... params)
{
    return Acore::Impl::StringConvertImpl::For<std::decay_t<Type>>::ToString(
