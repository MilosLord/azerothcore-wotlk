...
struct SymbolDetail {
    SymbolDetail()
        : Prefix(), Type(), Suffix(), Name(), Value(), Logged(false),
          HasChildren(false)
    {
    }
...
public:
    ...
    static LONG WINAPI
    WheatyUnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo);

    static void __cdecl WheatyCrtHandler(wchar_t const* expression,
...
private:
    ...
    static void DumpTypeIndex(
        DWORD64, DWORD, DWORD_PTR, bool&, char const*, char*, bool, bool);

    static void FormatOutputValue(char*     pszCurrBuffer,
                                  BasicType basicType,
                                  DWORD64   length,
                                  PVOID     pAddress,
                                  size_t    bufferSize,
...
extern WheatyExceptionReport
    g_WheatyExceptionReport; //  global instance of class
#endif                       // _WIN32
