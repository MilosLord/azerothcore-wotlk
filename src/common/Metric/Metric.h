...
public:
    ...
    template <class T>
    void
    LogValue(std::string const& category, T value, std::vector<MetricTag> tags)
    {
    ...
    void LogEvent(std::string const& category,
...
public:
    MetricStopWatch(LoggerType&& loggerFunc)
        : _logger(std::forward<LoggerType>(loggerFunc)),
          _startTime(sMetric->IsEnabled() ? std::chrono::steady_clock::now()
                                          : TimePoint())
    {
    }
...
#define METRIC_TIMER(category, ...)                                            \
    MetricStopWatch METRIC_UNIQUE_NAME(__ac_metric_stop_watch) =               \
        MakeMetricStopWatch([&](TimePoint start) {                             \
            sMetric->LogValue(category,                                        \
...
#define METRIC_DETAILED_TIMER(category, ...)                                   \
    MetricStopWatch METRIC_UNIQUE_NAME(__ac_metric_stop_watch) =               \
        MakeMetricStopWatch([&](TimePoint start) {                             \
            int64 duration =                                                   \
                int64(std::chrono::duration_cast<Milliseconds>(                \
                          std::chrono::steady_clock::now() - start)            \
                          .count());                                           \
...
#define METRIC_DETAILED_NO_THRESHOLD_TIMER(category, ...)                      \
    METRIC_TIMER(category, __VA_ARGS__)
