...
template <typename CodeUnit>
void write_codecvt(codecvt_result<CodeUnit>& out,
                   string_view               in_buf,
...
inline void
write_digit2_separated(char* buf, unsigned a, unsigned b, unsigned c, char sep)
{
...
{
    static constexpr const char* full_name_list[] = {"Sunday",
                                                     "Monday",
                                                     "Tuesday",
                                                     "Wednesday",
                                                     "Thursday",
...
public:
        ...
        -> decltype(ctx.begin())
    {
        ...
        if (*it == '.') {
            ...
            it = detail::parse_precision(
                it, end, specs_.precision, precision_ref_, ctx);
        }
        if (it != end && *it == 'L') {
            localized_ = true;
            ++it;
        ...
        -> decltype(ctx.out())
    {
        ...
        detail::handle_dynamic_spec<detail::width_checker>(
            specs.width, width_ref_, ctx);
        detail::handle_dynamic_spec<detail::precision_checker>(
            precision, precision_ref_, ctx);
        if (begin == end || *begin == '}') {
            out =
            ...
            using chrono_formatter = detail::
                chrono_formatter<FormatContext, decltype(out), Rep, Period>;
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>,
    ...
    template <typename FormatContext>
    auto
    format(std::chrono::time_point<std::chrono::system_clock, Duration> val,
