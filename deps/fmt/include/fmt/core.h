...
FMT_NORETURN FMT_API void
assert_fail(const char* file, int line, const char* message);
...
template <typename T>
using buffer_appender = conditional_t<std::is_same<T, char>::value,
                                      appender,
...
init_named_args(std::nullptr_t, int, int, const Args&...)
{
}
...
template <typename Char>
FMT_CONSTEXPR FMT_INLINE auto
parse_format_specs(const Char*                       begin,
                   const Char*                       end,
...
template <typename OutputIt,
          ...
          FMT_ENABLE_IF(detail::is_output_iterator<OutputIt, char>::value)>
FMT_INLINE auto
format_to_n(OutputIt out, size_t n, format_string<T...> fmt, T&&... args)
    -> format_to_n_result<OutputIt>
{
