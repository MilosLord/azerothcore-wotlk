...
BinaryInput::BinaryInput(const uint8* data,
                         int64        dataLen,
                         G3DEndian    dataEndian,
                         bool         compressed,
                         bool         copyMemory)
    : m_filename("<memory>"), m_bitPos(0), m_bitString(0), m_beginEndBits(0),
      m_alreadyRead(0), m_bufferLength(0), m_pos(0)
{
...
BinaryInput::BinaryInput(const std::string& filename,
                         G3DEndian          fileEndian,
                         bool               compressed)
    : m_filename(filename), m_bitPos(0), m_bitString(0), m_beginEndBits(0),
      m_alreadyRead(0), m_length(0), m_bufferLength(0), m_buffer(NULL),
      m_pos(0), m_freeBuffer(true)
{
    ...
    if (FileSystem::inZipfile(m_filename, zipfile)) {
        ...
        std::string internalFile =
            FilePath::canonicalize(m_filename.substr(zipfile.length() + 1));
        struct zip* z = zip_open(zipfile.c_str(), ZIP_CHECKCONS, NULL);
        {
            ...
            m_buffer =
                reinterpret_cast<uint8*>(System::alignedMalloc(m_length, 16));
            struct zip_file* zf =
                zip_fopen(z, internalFile.c_str(), ZIP_FL_NOCASE);
            if (zf == NULL) {
                throw std::string("\"") + internalFile + "\" inside \"" +
                    zipfile + "\" could not be opened.";
            }
            else {
                ...
                debugAssertM(
                    bytesRead == m_length,
                    internalFile +
                        " was corrupt because it unzipped to the wrong size.");
                (void)bytesRead;
                zip_fclose(zf);
            }
        }
...
void BinaryInput::decompress()
{
    ...
    alwaysAssertM(m_length < m_bufferLength * 500,
                  "Compressed file header is corrupted");
    ...
    unsigned long L = (unsigned long)m_length;
    int64         result =
        (int64)uncompress(m_buffer, &L, tempBuffer + 4, (uLong)tempLength - 4);
    m_length       = L;
    m_bufferLength = m_length;

    debugAssertM(result == Z_OK,
                 "BinaryInput/zlib detected corruption in " + m_filename);
    (void)result;
...
void BinaryInput::loadIntoMemory(int64 startPosition, int64 minLength)
{
    ...
    if (m_bufferLength < minLength) {
        // The current buffer isn't big enough to hold the chunk we want to
        // read. This happens if there was little memory available during the
        // initial constructor read but more memory has since been freed.
        m_bufferLength = minLength;
        debugAssert(m_freeBuffer);
        m_buffer = (uint8*)System::realloc(m_buffer, m_bufferLength);
        if (m_buffer == NULL) {
            throw "Tried to read a larger memory chunk than could fit in "
                  "memory. (2)";
        }
    }
    ...
    debugAssert(ret == 0);
    size_t toRead =
        (size_t)G3D::min<int64>(m_bufferLength, m_length - m_alreadyRead);
    ret = fread(m_buffer, 1, toRead, file);
...
void BinaryInput::prepareToRead(int64 nbytes)
{
    ...
    debugAssertM(m_pos + nbytes + m_alreadyRead <= m_length,
...
uint32 BinaryInput::readBits(int numBits)
{
    ...
    while (numBits > 0) {
        if (m_bitPos > 7) {
            ...
            // of the loop so that we don't try to read past the end of the
            // file.
            m_bitPos    = 0;
            m_bitString = readUInt8();
        }
