...
{
    return format(
        "Sphere(<%g, %g, %g>, %g)", center.x, center.y, center.z, radius);
}
...
bool Sphere::contains(const Sphere& other) const
{
    ...
    return (radius >= other.radius) &&
           (distance <= square(radius - other.radius));
}
...
bool Sphere::culledBy(const Array<Plane>& plane,
                      ...
                      uint32&             outMask) const
{
    ...
    return culledBy(
        plane.getCArray(), plane.size(), cullingPlaneIndex, inMask, outMask);
}
...
bool Sphere::culledBy(const class Plane* plane,
                      int                numPlanes,
                      int&               cullingPlane,
                      ...
                      uint32&            childMask) const
{
    ...
    for (int p = 0; p < numPlanes; ++p) {
        ...
        if ((inMask & 1) != 0) {
            ...
            bool culledLow = !plane[p].halfSpaceContainsFinite(
                center + plane[p].normal() * radius);
            bool culledHigh = !plane[p].halfSpaceContainsFinite(
                center - plane[p].normal() * radius);
                      ...
                      const uint32       _inMask) const
{
    ...
    for (int p = 0; p < numPlanes; ++p) {
        ...
        // Only test planes that are not masked
        if ((inMask & 1) != 0) {
            bool culled = !plane[p].halfSpaceContains(
                center + plane[p].normal() * radius);
            if (culled) {
                // Plane p culled the sphere
                cullingPlane = p;
...
Vector3 Sphere::randomInteriorPoint() const
{
    ...
    do {
        result = Vector3(
            uniformRandom(-1, 1), uniformRandom(-1, 1), uniformRandom(-1, 1));
