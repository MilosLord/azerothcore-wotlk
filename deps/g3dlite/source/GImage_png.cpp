...
void GImage::encodePNG(BinaryOutput& out) const
{
    ...
    png_structp png_ptr = png_create_write_struct(
        PNG_LIBPNG_VER_STRING, NULL, png_error, png_warning);
    if (!png_ptr) {
...
void GImage::decodePNG(BinaryInput& input)
{
    ...
    // now that the libpng structures are setup, change the error handlers and
    // read routines to use G3D functions so that BinaryInput can be used.

    png_set_read_fn(png_ptr, (png_voidp)&input, png_read_data);

    // read in sequentially so that three copies of the file are not in memory
    // at once
    ...
    if (color_type == PNG_COLOR_TYPE_GRAY_ALPHA) {
        ...
        throw GImage::Error(
            "Unsupported PNG color type - PNG_COLOR_TYPE_GRAY_ALPHA.",
            input.getFilename());
    }
