...
Stopwatch::Stopwatch(const std::string& myName)
    : myName(myName), inBetween(false), lastTockTime(-1), lastDuration(0),
      lastCycleCount(0), m_fps(0), emwaFPS(0), m_smoothFPS(0), emwaDuration(0)
{
    computeOverhead();
    reset();
}
...
void Stopwatch::tock()
{
    ...
    if (abs(emwaDuration - lastDuration) >
        max(emwaDuration, lastDuration) * 0.50) {
        // Off by more than 50%
    ...
    if (lastTockTime != -1.0) {
        ...
        if (abs(emwaFPS - m_fps) >
            max(emwaFPS, m_fps) * maxDiscrepancyPercentage) {
        ...
        if (m_smoothFPS == 0) {
            ...
            if (::fabs(m_smoothFPS - emwaFPS) > 0.75) {
                // Small number and display is off by more than 0.75; round to
                // the nearest 0.1
                m_smoothFPS = floor(emwaFPS * 10.0 + 0.5) / 10.0;
            }
        }
        else if (::fabs(m_smoothFPS - emwaFPS) > 1.25) {
            // Large number and display is off by more than 1.25; round to the
            // nearest 1.0
