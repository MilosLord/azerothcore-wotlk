    ...
    if (beginsWith(n, "vector3") || beginsWith(n, "point3")) {
        ...
        *this = UprightFrame(a).toCoordinateFrame();
        ...
        if (a.type() == Any::ARRAY) {
            ...
            for (Any::AnyTable::Iterator it = a.table().begin(); it.isValid();
                 ++it) {
        ...
        a.verify(false,
                 "Unrecognized class name where a PhysicsFrame or equivalent "
                 "was expected.");
    }
}
...
PhysicsFrame PhysicsFrame::operator*(const PhysicsFrame& other) const
{
    ...
    result.translation =
        translation + rotation.toRotationMatrix() * other.translation;
