...
std::string CoordinateFrame::toXYZYPRDegreesString() const
{
    ...
    return format("CFrame::fromXYZYPRDegrees(% 5.1ff, % 5.1ff, % 5.1ff, % "
                  "5.1ff, % 5.1ff, % 5.1ff)",
                  x,
                  y,
                  z,
    ...
    *this = CFrame();
    ...
    if (beginsWith(n, "VECTOR3") || beginsWith(n, "POINT3")) {
        ...
        *this = PhysicsFrame(any);
        //    } else if (beginsWith(n, "UPRIGHTFRAME") || beginsWith(n,
        //    "UFRAME")) {
        //        *this = UprightFrame(any);
    }
    else {
        any.verifyName("CFrame::fromXYZYPRDegrees",
                       "CoordinateFrame::fromXYZYPRDegrees");
        any.verifyType(Any::ARRAY);
        any.verifySize(3, 6);

        int s = any.size();

        *this = fromXYZYPRDegrees(any[0],
                                  any[1],
                                  any[2],
...
Any CoordinateFrame::toAny() const
{
    ...
    a.append(x, y, z);
    if (!G3D::fuzzyEq(yaw, 0.0f) || !G3D::fuzzyEq(pitch, 0.0f) ||
        !G3D::fuzzyEq(roll, 0.0f)) {
        a.append(yaw);
...
CoordinateFrame CoordinateFrame::fromXYZYPRRadians(
...
void CoordinateFrame::getXYZYPRRadians(
    float& x, float& y, float& z, float& yaw, float& pitch, float& roll) const
{
    x = translation.x;
    y = translation.y;
    z = translation.z;
...
void CoordinateFrame::getXYZYPRDegrees(
    float& x, float& y, float& z, float& yaw, float& pitch, float& roll) const
{
...
CoordinateFrame CoordinateFrame::fromXYZYPRDegrees(
    float x, float y, float z, float yaw, float pitch, float roll)
{
    return fromXYZYPRRadians(
        x, y, z, toRadians(yaw), toRadians(pitch), toRadians(roll));
}
...
{
    return Cylinder(pointToWorldSpace(c.point(0)),
...
{
    return Capsule(pointToWorldSpace(c.point(0)),
                   pointToWorldSpace(c.point(1)),
                   c.radius());
}
...
CoordinateFrame::CoordinateFrame(class BinaryInput& b)
...
{
    return Ray::fromOriginAndDirection(pointToWorldSpace(r.origin()),
                                       vectorToWorldSpace(r.direction()));
}

Ray CoordinateFrame::toObjectSpace(const Ray& r) const
{
    return Ray::fromOriginAndDirection(pointToObjectSpace(r.origin()),
                                       vectorToObjectSpace(r.direction()));
}
...
void CoordinateFrame::moveTowards(const CoordinateFrame& goal,
...
CoordinateFrame CoordinateFrame::lerp(const CoordinateFrame& other,
                                      float                  alpha) const
{
    ...
    if (alpha == 1.0f) {
        ...
        return CoordinateFrame(q1.slerp(q2, alpha).toRotationMatrix(),
                               translation * (1 - alpha) +
                                   other.translation * alpha);
    }
}

void CoordinateFrame::pointToWorldSpace(const Array<Vector3>& v,
...
void CoordinateFrame::normalToWorldSpace(const Array<Vector3>& v,
                                         Array<Vector3>&       vout) const
{
    vout.resize(v.size());
...
void CoordinateFrame::vectorToWorldSpace(const Array<Vector3>& v,
                                         Array<Vector3>&       vout) const
{
    vout.resize(v.size());
...
void CoordinateFrame::pointToObjectSpace(const Array<Vector3>& v,
                                         Array<Vector3>&       vout) const
{
    vout.resize(v.size());
...
void CoordinateFrame::normalToObjectSpace(const Array<Vector3>& v,
                                          Array<Vector3>&       vout) const
{
    vout.resize(v.size());
...
void CoordinateFrame::vectorToObjectSpace(const Array<Vector3>& v,
                                          Array<Vector3>&       vout) const
{
    vout.resize(v.size());
