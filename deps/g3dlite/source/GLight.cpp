...
GLight::GLight(const Any& any)
{
    ...
    if (any.type() == Any::TABLE) {
        *this = GLight();
        ...
        for (Any::AnyTable::Iterator it = any.table().begin(); it.hasMore();
             ++it) {
...
GLight::GLight()
    : position(0, 0, 0, 0), rightDirection(0, 0, 0), spotDirection(0, 0, -1),
      spotCutoff(180), spotSquare(false), color(Color3::white()),
      enabled(false), specular(true), diffuse(true)
{
...
GLight
GLight::directional(const Vector3& toLight, const Color3& color, bool s, bool d)
{
    GLight L;
...
GLight GLight::point(const Vector3& pos,
                     const Color3&  color,
                     float          constAtt,
                     float          linAtt,
                     float          quadAtt,
...
GLight GLight::spot(const Vector3& pos,
...
bool GLight::operator==(const GLight& other) const
{
    return (position == other.position) &&
           ...
           (attenuation[2] == other.attenuation[2]) && (color == other.color) &&
           (enabled == other.enabled) && (specular == other.specular) &&
           (diffuse == other.diffuse);
}

bool GLight::operator!=(const GLight& other) const { return !(*this == other); }
...
Sphere GLight::effectSphere(float cutoff) const
{
    if (position.w == 0) {
        ...
        if (attenuation[2] != 0) {
            ...
            float radius =
                (maxIntensity / cutoff - attenuation[0]) / attenuation[1];
