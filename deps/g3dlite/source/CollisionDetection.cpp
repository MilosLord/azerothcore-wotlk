/**
  ...
  and comments by Max McGuire.  Ray-sphere code by Eric Haines.
...
bool CollisionDetection::parallelAxisForSolidBoxSolidBox(const double* ca,
...
void CollisionDetection::fillSolidBoxSolidBoxInfo(const Box& box1,
                                                  const Box& box2,
                                                  Vector3&   a,
                                                  Vector3&   b,
                                                  Vector3&   D,
...
bool CollisionDetection::conservativeBoxBoxTest(const Vector3& a,
                                                const Vector3& b,
                                                const Vector3& D)
{
...
bool CollisionDetection::fixedSolidBoxIntersectsFixedSolidBox(
    const Box& box1, const Box& box2, const int lastSeparatingAxis)
{
    ...
    int  dummy1, dummy2;
    bool parallelAxes =
        parallelAxisForSolidBoxSolidBox(ca, 0.00001, dummy1, dummy2);
...
void CollisionDetection::closestPointsBetweenLineAndLine(const Line& line1,
                                                         const Line& line2,
                                                         Vector3&    closest1,
                                                         Vector3&    closest2)
{
    ...
    const int       lastSeparatingAxis)
{
    ...
    int  axis1, axis2;
    bool parallelAxes =
        parallelAxisForSolidBoxSolidBox(ca, 0.00001, axis1, axis2);
    ...
    if (!parallelAxes) {
        ...
        for (int i = 6; i < 15; ++i) {
            ...
            // magnitude by dividing by a number smaller than 1 and greater than
            // 0
    ...
    Vector3 L =
        separatingAxisForSolidBoxSolidBox(penetrationAxisIndex, box1, box2);
    ...
    Array<Vector3>& contactNormals)
{
    ...
    case 2: // Edge collision
    {
        ...
        contactNormals.append(
            boxFrame.normalToWorldSpace(X - center).direction());
    ...
    case 0: // Volume collision
        ...
        if (distOutsideBox.x > distOutsideBox.y) {
            if (distOutsideBox.x > distOutsideBox.z) {
                ...
                if (center.x > 0) {
                    contactNormals.append(
                        boxFrame.normalToWorldSpace(-Vector3::unitX()));
                }
                else {
                    contactNormals.append(
                        boxFrame.normalToWorldSpace(Vector3::unitX()));
            ...
            if (center.y > 0) {
                contactNormals.append(
                    boxFrame.normalToWorldSpace(-Vector3::unitY()));
            }
            else {
                contactNormals.append(
                    boxFrame.normalToWorldSpace(Vector3::unitY()));
        ...
        ZAXIS:
            ...
            if (center.z > 0) {
                contactNormals.append(
                    boxFrame.normalToWorldSpace(-Vector3::unitZ()));
            }
            else {
                contactNormals.append(
                    boxFrame.normalToWorldSpace(Vector3::unitZ()));
    ...
    Array<Vector3>& contactNormals)
{
    ...
    if (depth >= 0) {
        contactPoints.append(sphereA.center +
                             axis * (sphereA.radius - depth / 2));
...
bool __fastcall CollisionDetection::rayAABox(const Ray&     ray,
                                             ...
                                             bool&    inside)
{
    ...
    debugAssertM(fabs(ray.direction().squaredLength() - 1.0f) < 0.01f,
                 format("Length = %f", ray.direction().length()));
    {
        ...
        if (((d < 0) && (L2 > boundingRadiusSquared)) ||
            (M2 > boundingRadiusSquared)) {
            inside = false;
            return false;
        }
    ...
    for (int i = 0; i < 3; ++i) {
        if (i != WhichPlane) {
            location[i] =
                ray.origin()[i] + MaxT[WhichPlane] * ray.direction()[i];
    ...
    Vector3&       outNormal)
{
    ...
    if (time < finf()) {
        // Location is now the center of the moving sphere at the collision
        // time. Adjust for the size of the moving sphere.  Two spheres always
        // collide along a line between their centers.
        location += (location - fixedSphere.center) * movingSphere.radius /
                    fixedSphere.radius;
    }
...
float CollisionDetection::collisionTimeForMovingPointFixedTriangle(
    ...
    double time = collisionTimeForMovingPointFixedPlane(point, velocity,
triangle.plane(), outLocation, outNormal);
    ...
    if (isPointInsideTriangle(triangle.vertex(0), triangle.vertex(1),
triangle.vertex(2), triangle.normal(), outLocation, triangle.primaryAxis())) {
    ...
    Vector3&       outNormal)
{
    ...
    bestTime = collisionTimeForMovingPointFixedRectangle(
        point, velocity, v[0], v[1], v[2], v[3], location, normal);
    outNormal = normal;
    ...
    for (f = 1; f < 6; ++f) {
        Vector3 pos;
        box.getFaceCorners(f, v[0], v[1], v[2], v[3]);
        float time = collisionTimeForMovingPointFixedRectangle(
            point, velocity, v[0], v[1], v[2], v[3], pos, normal);
        if (time < bestTime) {
            bestTime  = time;
            outNormal = normal;
            location  = pos;
    ...
    Vector3&       normal)
{
    ...
    if (collisionLocationForMovingPointFixedAABox(
            origin, dir, box, location, Inside, normal)) {
    ...
    Vector3&       outNormal)
{
    ...
    float time = collisionTimeForMovingPointFixedPlane(
        point, velocity, plane, location, outNormal);
    ...
    if (isPointInsideRectangle(v0, v1, v2, v3, plane.normal(), location)) {
        // The intersection point is inside the rectangle; that is the location
        // where the point hits the rectangle.
        return time;
    }
    else {
        return finf();
...
/** Used by findRayCapsuleIntersection.
    @cite From magic software
   http://www.magic-software.com/Source/Intersection3D/MgcIntr3DLinCap.cpp */
static int findRayCapsuleIntersectionAux(const Vector3& rkOrigin,
...
/** Used by collisionTimeForMovingPointFixedCapsule.
    ...
    */
static bool findRayCapsuleIntersection(const Ray&     rkRay,
                                       const Capsule& rkCapsule,
                                       int&           riQuantity,
                                       Vector3        akPoint[2])
{
    ...
    double afT[2];
    riQuantity = findRayCapsuleIntersectionAux(
        rkRay.origin(), rkRay.direction(), rkCapsule, afT);
    ...
    for (i = 0; i < riQuantity; ++i) {
        if (afT[i] >= 0.0f) {
            akPoint[iClipQuantity] =
                rkRay.origin() + afT[i] * rkRay.direction();
            ++iClipQuantity;
        }
    }
    ...
    Vector3&       outNormal)
{
    ...
    findRayCapsuleIntersection(Ray::fromOriginAndDirection(_point, direction),
                               capsule,
                               numIntersections,
                               intersection);

    if (numIntersections == 2) {
        // A collision can only occur if there are two intersections.  If there
        // is one intersection, that one is exiting the capsule.
        ...
        if (&outNormal != &ignore) {
            Vector3 p2 =
                LineSegment::fromTwoPoints(capsule.point(0), capsule.point(1))
                    .closestPoint(_point);
    ...
    Vector3&       outNormal)
{
    ...
    if (sphere.radius == 0) {
        ...
        return collisionTimeForMovingPointFixedPlane(
            sphere.center, velocity, plane, location, outNormal);
    }

    // The world-space collision point, which lies on the surface of the sphere,
    // will be the point at center + velocity * time - (radius * planeNormal).
    // Collisions only occur when the sphere is travelling into the plane.
    ...
    if (fuzzyLe(G3D::abs(distance), sphere.radius)) {
        ...
        // The point on the sphere (in world space) that will eventually first
        // contact the plane
    ...
    float           b[3])
{
    ...
    Vector3     dummy;
    const float time = collisionTimeForMovingSphereFixedPlane(
        sphere, velocity, triangle.plane(), outLocation, dummy);
    ...
    if (isPointInsideTriangle(triangle.vertex(0),
                              triangle.vertex(1),
                              triangle.vertex(2),
                              triangle.normal(),
                              outLocation,
                              b,
                              triangle.primaryAxis())) {

        // The intersection point is inside the triangle; that is the location
        // where the sphere hits the triangle.

#ifdef G3D_DEBUG
        {
            // Internal consistency checks
            debugAssertM(b[0] >= 0.0 && b[0] <= 1.0f,
                         "Intersection is outside triangle.");
            debugAssertM(b[1] >= 0.0 && b[1] <= 1.0f,
                         "Intersection is outside triangle.");
            debugAssertM(b[2] >= 0.0 && b[2] <= 1.0f,
                         "Intersection is outside triangle.");
            const Vector3& blend = b[0] * triangle.vertex(0) +
                                   b[1] * triangle.vertex(1) +
                                   b[2] * triangle.vertex(2);
            debugAssertM(blend.fuzzyEq(outLocation),
                         "Barycentric coords don't match intersection.");
            // Call again so that we can debug the problem
            // isPointInsideTriangle(triangle.vertex(0), triangle.vertex(1),
            // triangle.vertex(2), triangle.normal(), outLocation, b,
            // triangle.primaryAxis());
        }
#endif
    ...
    // Switch over to moving the triangle towards a fixed sphere and see at what
    // time they will hit.

    // Closest point on the triangle to the sphere intersection with the plane.
    int            edgeIndex;
    const Vector3& point =
        closestPointOnTrianglePerimeter(triangle._vertex,
                                        triangle.edgeDirection,
                                        triangle.edgeMagnitude,
                                        outLocation,
                                        edgeIndex);
    ...
    if (!sphere.contains(point)) {
        ...
        t = collisionTimeForMovingPointFixedSphere(
            point, -velocity, sphere, dummy, dummy);
    }
    ...
    if (t < finf()) {
        ...
        // Avoid sqrt by taking advantage of the existing edgeDirection unit
        // vector.
        b[next[edgeIndex]] = (outLocation - triangle._vertex[edgeIndex])
                                 .dot(triangle.edgeDirection[edgeIndex]) /
                             triangle.edgeMagnitude[edgeIndex];
...
#ifdef G3D_DEBUG
        {
            // Internal consistency checks
            for (int i = 0; i < 3; ++i) {
                debugAssertM(fuzzyGe(b[i], 0.0f) && fuzzyLe(b[i], 1.0f),
                             "Intersection is outside triangle.");
            }
            Vector3 blend = b[0] * triangle.vertex(0) +
                            b[1] * triangle.vertex(1) +
                            b[2] * triangle.vertex(2);
            debugAssertM(
                blend.fuzzyEq(outLocation),
    ...
    Vector3&       outNormal)
{
    ...
    float time = collisionTimeForMovingSphereFixedPlane(
        sphere, velocity, plane, location, outNormal);
    ...
    if (isPointInsideRectangle(v0, v1, v2, v3, plane.normal(), location)) {
        // The intersection point is inside the rectangle; that is the location
        // where the sphere hits the rectangle.
        return time;
    }

    // Switch over to moving the rectangle towards a fixed sphere and see at
    // what time they will hit.

    Vector3 point =
        closestPointToRectanglePerimeter(v0, v1, v2, v3, sphere.center);

    Vector3 dummy;
    double  t = collisionTimeForMovingPointFixedSphere(
        point, -velocity, sphere, location, dummy);

    // Normal is the plane normal, location is the original location of the
    // point.
    location = point;
    ...
    Vector3&       outNormal)
{
    ...
    int     f = 0;
    box.getFaceCorners(f, v[0], v[1], v[2], v[3]);
    bestTime = collisionTimeForMovingSphereFixedRectangle(
        sphere, velocity, v[0], v[1], v[2], v[3], location, outNormal);

    for (f = 1; f < 6; ++f) {
        Vector3 pos, normal;
        box.getFaceCorners(f, v[0], v[1], v[2], v[3]);
        float time = collisionTimeForMovingSphereFixedRectangle(
            sphere, velocity, v[0], v[1], v[2], v[3], pos, normal);
        if (time < bestTime) {
            bestTime  = time;
            location  = pos;
            outNormal = normal;
    ...
    Vector3&       outNormal)
{
    ...
    Capsule _capsule(
        capsule.point(0), capsule.point(1), capsule.radius() + sphere.radius);

    Vector3 normal;
    double  time = collisionTimeForMovingPointFixedCapsule(
        sphere.center, velocity, _capsule, location, normal);

    if (time < finf()) {
        // Location is now the position of the center of the sphere at the time
        // of collision. We have to adjust the collision location for the size
        // of the sphere.
...
Vector3 CollisionDetection::bounceDirection(const Sphere&  sphere,
                                            const Vector3& velocity,
...
Vector3 CollisionDetection::slideDirection(const Sphere&  sphere,
                                           const Vector3& velocity,
                                           const float    collisionTime,
                                           const Vector3& collisionLocation)
{
...
Vector3 CollisionDetection::closestPointOnLineSegment(const Vector3& v0,
                                                      const Vector3& v1,
                                                      const Vector3& point)
{
    ...
    return closestPointOnLineSegment(
        v0, v1, edge / edgeLength, edgeLength, point);
}

Vector3
CollisionDetection::closestPointOnLineSegment(const Vector3& v0,
                                              const Vector3& v1,
...
Vector3
CollisionDetection::closestPointOnTrianglePerimeter(const Vector3& v0,
                                                    const Vector3& v1,
                                                    const Vector3& v2,
                                                    const Vector3& point)
{
    ...
    int edgeIndex;
    return closestPointOnTrianglePerimeter(
        v, edgeDirection, edgeLength, point, edgeIndex);
}
    ...
    int&           edgeIndex)
{
    ...
    for (int i = 0; i < 3; ++i) {
        r[i] = closestPointOnLineSegment(
            v[i], v[next[i]], edgeDirection[i], edgeLength[i], point);
...
#ifdef G3D_DEBUG
    {
        ...
        debugAssertM(
            fuzzyEq(diff.direction().dot(edgeDirection[edgeIndex]), 1.0f) ||
                diff.fuzzyEq(Vector3::zero()),
            "Point not on correct triangle edge");
...
bool CollisionDetection::isPointInsideTriangle(const Vector3& v0,
                                               const Vector3& v1,
                                               const Vector3& v2,
...
bool CollisionDetection::isPointInsideRectangle(const Vector3& v0,
                                                const Vector3& v1,
                                                const Vector3& v2,
                                                const Vector3& v3,
                                                const Vector3& normal,
...
Vector3
CollisionDetection::closestPointToRectanglePerimeter(const Vector3& v0,
                                                     const Vector3& v1,
                                                     const Vector3& v2,
                                                     const Vector3& v3,
                                                     const Vector3& point)
...
Vector3 CollisionDetection::closestPointToRectangle(const Vector3& v0,
                                                    const Vector3& v1,
                                                    const Vector3& v2,
                                                    const Vector3& v3,
                                                    const Vector3& point)
    ...
    const Sphere& sphere1, const Sphere& sphere2)
{
    ...
    return (sphere1.center - sphere2.center).squaredMagnitude() <
           square(sphere1.radius + sphere2.radius);
}

bool CollisionDetection::fixedSolidSphereIntersectsFixedSolidBox(
    const Sphere& sphere, const Box& box)
{
    ...
    for (f = 0; f < 6; ++f) {
        ...
        if ((closestPointToRectangle(v0, v1, v2, v3, sphere.center) -
             sphere.center)
                .squaredMagnitude() <= r2) {
            return true;
        }
    ...
    double         timeLimit)
{
    ...
    return (collisionTimeForMovingSphereFixedBox(
                sphere, velocity, box, dummy1, dummy2) < timeLimit);
}
    ...
    double         timeLimit)
{
    ...
    // If the sphere collides with the other sphere during the time limit, it
    // passes through
    return (collisionTimeForMovingPointFixedSphere(
                sphere.center, velocity, bigFixed, dummy1, dummy2) < timeLimit);
}
    ...
    const Sphere& sphere, const Triangle& triangle)
{
    ...
    v = closestPointOnTrianglePerimeter(triangle._vertex,
                                        triangle.edgeDirection,
                                        triangle.edgeMagnitude,
                                        sphere.center,
                                        edgeIndex);
...
static bool planeBoxOverlap(const Vector3& normal,
                            const Vector3& vert,
