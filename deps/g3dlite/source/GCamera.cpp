    ...
    while (it.hasMore()) {
        ...
        if (k == "FOVDIRECTION") {
            ...
            if (v == "HORIZONTAL") {
                ...
                any.verify(
                    false,
                    "fovDirection must be \"HORIZONTAL\" or \"VERTICAL\"");
            }
        }
...
GCamera::operator Any() const
{
    ...
    any.set(
        "fovDirection",
        std::string((m_direction == HORIZONTAL) ? "HORIZONTAL" : "VERTICAL"));
...
GCamera::GCamera(const Matrix4& proj, const CFrame& frame)
{
    ...
    proj.getPerspectiveProjectionParameters(
...
void GCamera::getCoordinateFrame(CoordinateFrame& c) const { c = m_cframe; }

void GCamera::setCoordinateFrame(const CoordinateFrame& c) { m_cframe = c; }
...
float GCamera::imagePlaneDepth() const { return -m_nearPlaneZ; }
...
void GCamera::getProjectPixelMatrix(const Rect2D& viewport, Matrix4& P) const
{
    ...
    P = Matrix4(sx,
                0,
                0,
                sx + viewport.x0() - m_pixelOffset.x,
                0,
                -sy,
                0,
                sy + viewport.y0() + m_pixelOffset.y,
                0,
                0,
                1,
                0,
...
Vector3 GCamera::unproject(const Vector3& v, const Rect2D& viewport) const
{
    ...
    const Ray& ray =
        worldRay(v.x - m_pixelOffset.x, v.y - m_pixelOffset.y, viewport);

    // Find out where the ray reaches the specified depth.
    const Vector3& out =
        ray.origin() +
        ray.direction() * -z / (ray.direction().dot(m_cframe.lookVector()));

    return out;
}

float GCamera::worldToScreenSpaceArea(float         area,
                                      float         z,
...
void GCamera::frustum(const Rect2D& viewport, Frustum& fr) const
{
    ...
    fr.vertexPos.append(Vector4(x, y, zn, 1),
                        Vector4(-x, y, zn, 1),
                        Vector4(-x, -y, zn, 1),
                        Vector4(x, -y, zn, 1));
    ...
    // Left plane
    face.plane          = Plane(Vector3(-fr.faceArray.last().plane.normal().x,
                               0,
                               fr.faceArray.last().plane.normal().z),
                       Vector3::zero());
    ...
    // Bottom plane
    face.plane          = Plane(Vector3(0,
                               -fr.faceArray.last().plane.normal().y,
                               fr.faceArray.last().plane.normal().z),
                       Vector3::zero());
    ...
    for (int p = 0; p < fr.faceArray.size(); ++p) {
        ...
        if (isFinite(d)) {
            ...
            // generating NaNs.
            fr.faceArray[p].plane =
                Plane::fromEquation(newNormal.x, newNormal.y, newNormal.z, d);
        }
    }
}

void GCamera::getNearViewportCorners(const Rect2D& viewport,
                                     Vector3&      outUR,
                                     Vector3&      outUL,
                                     Vector3&      outLL,
                                     Vector3&      outLR) const
...
void GCamera::getFarViewportCorners(const Rect2D& viewport,
                                    Vector3&      outUR,
                                    Vector3&      outUL,
                                    Vector3&      outLL,
                                    Vector3&      outLR) const
...
void GCamera::setPosition(const Vector3& t) { m_cframe.translation = t; }
...
Vector3 GCamera::convertFromUnitToNormal(const Vector3& in,
                                         const Rect2D&  viewport) const
{
    return (in + Vector3(1, 1, 1)) * 0.5 *
               Vector3(viewport.width(), -viewport.height(), 1) +
