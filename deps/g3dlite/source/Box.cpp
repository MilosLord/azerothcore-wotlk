...
void Box::init(const Point3& min, const Point3& max)
{
    ...
    for (int i = 0; i < 3; ++i) {
        if (!G3D::isFinite(extent(i))) {
            finiteExtent = false;
            // If the extent is infinite along an axis, make the center zero to
            // avoid NaNs
            _center[i] = 0.0f;
        }
    }
    ...
    _area =
        2 * (bounds.x * bounds.y + bounds.y * bounds.z + bounds.z * bounds.x);
}
...
Vector3 Box::corner(int i) const
{
    ...
    // add half of the corresponding edge vector from center, and a zero
    // indicates we should subtract it. Note: 1 = 001 2 = 010 4 = 100
    //
    ...
    // This method is implemented as a swtich statement due to being marginally
    // faster than the bit-hack method Also, the _center + 0.5f * (...) is
    // repeated every time for similarly speed-based reasons.
    switch (i) {
    case 0:
        return _center +
               (0.5f * (-_edgeVector[0] - _edgeVector[1] - _edgeVector[2]));
    case 1:
        return _center +
               (0.5f * (_edgeVector[0] - _edgeVector[1] - _edgeVector[2]));
    case 2:
    ...
    case 6:
        return _center +
               (0.5f * (-_edgeVector[0] + _edgeVector[1] + _edgeVector[2]));
    default:
        return _center + (0.5f * (_edgeVector[0] + _edgeVector[1] +
                                  _edgeVector[2])); // case 7
    }
}
...
void Box::getLocalFrame(CoordinateFrame& frame) const
{
    ...
    frame.rotation = Matrix3(axis0[0],
                             axis1[0],
                             axis2[0],
                             axis0[1],
                             axis1[1],
...
void Box::getFaceCorners(
    int f, Point3& v0, Point3& v1, Point3& v2, Point3& v3) const
{
    switch (f) {
    case 0:
        v0 = corner(0);
...
bool Box::culledBy(const Array<Plane>& plane,
                   int&                cullingPlane,
...
bool Box::contains(const Point3& point) const
{
    ...
    // TODO: Write in a more intuitive way. I left it as it was before after
    // figuring it out, but this should make no sense to someone who is just
    // starting to read this code.
...
void Box::getRandomSurfacePoint(Vector3& P, Vector3& N) const
{
    ...
    if (r < aXY) {
        P = _edgeVector[0] * (float)uniformRandom(-0.5, 0.5) +
            _edgeVector[1] * (float)uniformRandom(-0.5, 0.5) + _center +
            _edgeVector[2] * d * 0.5f;
        N = axis(2) * d;
    }
    else if (r < aYZ) {
        P = _edgeVector[1] * (float)uniformRandom(-0.5, 0.5) +
            _edgeVector[2] * (float)uniformRandom(-0.5, 0.5) + _center +
            _edgeVector[0] * d * 0.5f;
        N = axis(0) * d;
    }
    else {
        P = _edgeVector[2] * (float)uniformRandom(-0.5, 0.5) +
            _edgeVector[0] * (float)uniformRandom(-0.5, 0.5) + _center +
            _edgeVector[1] * d * 0.5f;
        N = axis(1) * d;
    }
}
