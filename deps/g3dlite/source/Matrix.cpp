...
{
    debugAssert(((impl->R == 4) && (impl->C == 1)) ||
                ((impl->R == 1) && (impl->C == 4)));

    if (impl->R > impl->C) {
        return Vector4(
            impl->get(0, 0), impl->get(1, 0), impl->get(2, 0), impl->get(3, 0));
    }
    else {
        return Vector4(
...
Matrix::Impl::Impl(const Matrix3& M)
...
Matrix::Impl::Impl(const Matrix4& M)
    : elt(NULL), data(NULL), R(0), C(0), dataSize(0)
{
    setSize(4, 4);
    for (int r = 0; r < 4; ++r) {
...
Matrix::T Matrix::Impl::determinant() const
{
    ...
    case 3: {
        ...
        return Matrix::T(elt[0][0] * cofactor00 + elt[0][1] * cofactor10 +
                         elt[0][2] * cofactor20);
    }
...
void Matrix::Impl::withoutRowAndCol(int   excludeRow,
                                    int   excludeCol,
                                    Impl& out) const
{
    ...
    for (int r = 0; r < out.R; ++r) {
        for (int c = 0; c < out.C; ++c) {
            out.elt[r][c] = elt[r + ((r >= excludeRow) ? 1 : 0)]
                               [c + ((c >= excludeCol) ? 1 : 0)];
        }
    }
}
...
/*
    Public function for testing purposes only. Use pseudoInverse(), as it
   contains optimizations for nonsingular matrices with at least one small (<5)
   dimension.
*/
...
Matrix Matrix::svdPseudoInverse(float tolerance) const
{
    ...
    if (r == 0) {
        ...
        */
        ...
        //
        // Avoid the explicit subMatrix call, and by storing A' instead of A,
        // avoid both the transpose and the memory incoherence of striding
        // across memory in big steps.
...
{
    // If vector A has nonzero elements: transpose A, then divide each elt. by
    // the squared norm If A is zero vector: transpose A
    double x = 0.0;
...
Matrix Matrix::rowPartPseudoInverse() const
{
    ...
    int n = cols();
    alwaysAssertM((m <= n),
                  "Row-partitioned block matrix pseudoinverse requires R<C");
    ...
    default:
        alwaysAssertM(
            false,
            "G3D internal error: Should have used the vector or general case!");
        return Matrix();
    }
}
...
Matrix Matrix::colPartPseudoInverse() const
{
    ...
    int n = cols();
    alwaysAssertM((m >= n),
                  "Column-partitioned block matrix pseudoinverse requires R>C");
...
Matrix Matrix::col2PseudoInverse(const Matrix& B) const
{
    ...
    const float B2[2][2] = {{B.get(0, 0), B.get(0, 1)},
                            {B.get(1, 0), B.get(1, 1)}};
...
// Uses the block matrix pseudoinverse to compute the pseudoinverse of a
// full-rank mxn matrix with m >= n
...
#define SWAP(x, y)                                                             \
    ...
    for (int c = 0; c < R; ++c) {
        ...
        // Interchange columns so that the pivot element is on the diagonal
        // (we'll have to undo this at the end)
        if (row != col) {
            for (int k = 0; k < R; ++k) {
