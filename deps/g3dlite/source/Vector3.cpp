/**
 ...
 \cite Portions based on Dave Eberly's Magic Software Library at
...
{
    static const Vector3 v(
        (float)G3D::finf(), (float)G3D::finf(), (float)G3D::finf());
    return v;
}
const Vector3& Vector3::nan()
{
    static const Vector3 v(
        (float)G3D::fnan(), (float)G3D::fnan(), (float)G3D::fnan());
    return v;
}
...
size_t Vector3::hashCode() const
{
    ...
    return HashTrait<uint32>::hashCode(u[0]) ^
...
Vector3
Vector3::cosPowHemiRandom(const Vector3& normal, const float k, Random& r)
{
    debugAssertM(G3D::fuzzyEq(normal.length(), 1.0f),
...
Vector3 Vector3::refractionDirection(const Vector3& normal,
                                     float          iInside,
