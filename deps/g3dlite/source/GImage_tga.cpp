...
void GImage::decodeTGA(BinaryInput& input)
{
    ...
    if (imageType != 2 && imageType != 10) {
        throw Error("TGA images must be type 2 (Uncompressed truecolor) or 10 "
                    "(Run-length truecolor)",
                    input.getFilename());
    }
    ...
    if (imageType == 2) {
        ...
        for (y = m_height - 1; y >= 0; --y) {
            for (int x = 0; x < m_width; /* intentionally no x increment */) {
                // The specification guarantees that no packet will wrap past
                // the end of a row
