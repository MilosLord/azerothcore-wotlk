...
void AnyVal::load(const std::string& filename) { *this = fromFile(filename); }
...
AnyVal::AnyVal(bool b)
    : m_type(BOOLEAN), m_value(new bool(b)), m_referenceCount(NULL)
{
}

AnyVal::AnyVal(G3D::TextInput& t)
    : m_type(NIL), m_value(NULL), m_referenceCount(NULL)
{
    deserialize(t);
}
...
AnyVal::AnyVal(const CoordinateFrame& v)
    : m_type(COORDINATEFRAME), m_referenceCount(NULL)
{
...
AnyVal::AnyVal(const AnyVal& c)
    : m_type(NIL), m_value(NULL), m_referenceCount(NULL)
{
    *this = c;
}

AnyVal::AnyVal(Type arrayOrTable)
    : m_type(NIL), m_value(NULL), m_referenceCount(new int(1))
{
    ...
    default:
        debugAssertM(
            false,
            "Cannot construct AnyVal from constants except ARRAY or TABLE.");
    }
}

AnyVal::~AnyVal() { deleteValue(); }
...
void* AnyVal::copyValue() const
{
    ...
    case TABLE:
        return new Table<std::string, AnyVal>(
            *(Table<std::string, AnyVal>*)m_value);

    default:
        debugAssertM(false,
                     "Internal error: no assignment operator for this type.");
        return NULL;
    }
}

AnyVal::Type AnyVal::type() const { return m_type; }
...
void AnyVal::serialize(G3D::TextOutput& t) const
{
    ...
    case RECT2D:
        t.printf("R(%g, %g, %g, %g)",
                 ((Rect2D*)m_value)->x0(),
                 ((Rect2D*)m_value)->y0(),
                 ((Rect2D*)m_value)->width(),
                 ((Rect2D*)m_value)->height());
        break;
    ...
    case MATRIX4: {
        ...
        t.printf("M4(\n");
        t.pushIndent();
        t.printf("%10.5f, %10.5f, %10.5f, %10.5f,\n"
    ...
    case COORDINATEFRAME: {
        ...
        t.printf(
            ...
            "%10.5f, %10.5f, %10.5f,   %10.5f,\n"
            "%10.5f, %10.5f, %10.5f,   %10.5f)",
            c.rotation[0][0], c.rotation[0][1], c.rotation[0][2],
        c.translation.x, c.rotation[1][0], c.rotation[1][1], c.rotation[1][2],
        c.translation.y, c.rotation[2][0], c.rotation[2][1], c.rotation[2][2],
        c.translation.z); t.popIndent();
        */
    } break;
    ...
    case TABLE: {
        const Table<std::string, AnyVal>& a =
            *(Table<std::string, AnyVal>*)m_value;
        t.printf("{\n");
        t.pushIndent();
    ...
    default:
        debugAssertM(false,
                     "Internal error: no serialize method for this type.");
    }
}
    ...
    case Token::SYMBOL: {
        ...
        if (s == "NIL") {
            ...
            if (t.peek().type() == Token::SYMBOL) {
                ...
                m = CoordinateFrame::fromXYZYPRDegrees(
            ...
            while ((peek.type() != Token::SYMBOL) || (peek.string() != "}")) {
                ...
                peek = t.peek();
                if ((peek.type() != Token::SYMBOL) &&
                    (peek.extendedType() != Token::SINGLE_QUOTED_TYPE)) {
...
const AnyVal& AnyVal::operator[](const std::string& key) const
{
    ...
    const Table<std::string, AnyVal>& t =
        *(const Table<std::string, AnyVal>*)m_value;
...
AnyVal::coordinateFrame(const CoordinateFrame& defaultVal) const
{
    if (m_type != COORDINATEFRAME) {
        return defaultVal;
    }
...
const AnyVal& AnyVal::get(const std::string& key,
                          const AnyVal&      defaultVal) const
{
    if (m_type != TABLE) {
        return defaultVal;
    }
