...
{
    return LineSegment::fromTwoPoints(p1, p2).distanceSquared(p) <=
           square(radius());
}
...
void Capsule::getRandomSurfacePoint(Vector3& p, Vector3& N) const
{
    ...
    // Create a random point on a standard capsule and then rotate to the global
    // frame.
...
void Capsule::getReferenceFrame(CoordinateFrame& cframe) const
{
    ...
    Vector3 X = (abs(Y.dot(Vector3::unitX())) > 0.9) ? Vector3::unitY()
                                                     : Vector3::unitX();
...
Vector3 Capsule::randomInteriorPoint() const
{
    ...
    // Create a random point in a standard capsule and then rotate to the global
    // frame.
    ...
    if (r1 < 2.0 * hemiVolume) {
        ...
        p = Vector3(
            cos(a) * r2, uniformRandom(-h / 2.0f, h / 2.0f), sin(a) * r2);
    }
