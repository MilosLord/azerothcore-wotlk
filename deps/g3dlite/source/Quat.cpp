...
Quat::Quat(const Matrix3& rot)
{
    ...
    //
    // Since we're going to pay the sqrt anyway, we perform a post
    // normalization, which also fixes any poorly normalized input.  Multiply
    // all elements by 2*c in the above, giving:
...
Quat Quat::slerp(const Quat& _quat1,
                 float       alpha,
                 float       threshold,
