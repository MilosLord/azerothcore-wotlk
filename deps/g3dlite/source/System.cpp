...
System::System()
    : m_initialized(false), m_cpuSpeed(0), m_hasCPUID(false), m_hasRDTSC(false),
      m_hasMMX(false), m_hasSSE(false), m_hasSSE2(false), m_hasSSE3(false),
      m_has3DNOW(false), m_has3DNOW2(false), m_hasAMDMMX(false),
      m_cpuVendor("Uninitialized"), m_numCores(1),
      m_machineEndian(G3D_LITTLE_ENDIAN), m_cpuArch("Uninitialized"),
      m_operatingSystem("Uninitialized"), m_version("Uninitialized"),
      m_outOfMemoryCallback(NULL), m_realWorldGetTickTime0(0),
...
void System::init()
{
    ...
    if (m_hasCPUID) {
        ...
        cpuid(CPUID_GET_HIGHEST_FUNCTION,
              m_highestCPUIDFunction,
              ebxreg,
              ecxreg,
              edxreg);
    }

    // Get the operating system name (also happens to read some other
    // information)
#ifdef G3D_WINDOWS
    ...
    uint64_t maxAddr =
        reinterpret_cast<uint64_t>(systemInfo.lpMaximumApplicationAddress);
    {
        char c[1024];
...
void getG3DVersion(std::string& s)
{
    ...
    if ((G3D_VER % 100) != 0) {
        sprintf(cstr,
                "G3D Innovation Engine %d.%02d beta %d, %s%s",
                G3D_VER / 10000,
                (G3D_VER / 100) % 100,
                G3D_VER % 100,
                build,
        ...
        sprintf(cstr,
                "G3D Innovation Engine %d.%02d, %s%s",
                G3D_VER / 10000,
                (G3D_VER / 100) % 100,
                build,
                debug);
...
std::string System::findDataFile(const std::string& _full,
                                 ...
                                 bool               caseSensitive)
{
    ...
    if (directoryArray.size() == 0) {
        ...
        if (g3dPath == NULL) {
            ...
            const char* paths[] = {"../data-files/",
                                   "../../data-files/",
                                   "../../../data-files/",
                                   NULL};
            for (int i = 0; paths[i]; ++i) {
                if (FileSystem::exists(
                        pathConcat(paths[i], "G3D-DATA-README.TXT"),
                        true,
                        caseSensitive)) {
                    g3dPath = paths[i];
                    break;
        ...
        static const std::string subdirs[] = {"font",
                                              "gui",
                                              "shader",
                                              "model",
                                              "cubemap",
    ...
    if (errorIfNotFound) {
        // Generate an error message.  Delay this operation until we know that
        // we need it; otherwise all of the string concatenation would run on
        ...
        if (g3dPath) {
            ...
            msg += "  G3D9DATA               = (environment variable is not "
                   "defined)\n";
        }
        MARK_LOG();
...
#undef checkBit
    ...
    !defined(__MINGW32__) /* G3DFIX: Don't check if on 64-bit Windows          \
                             platforms or using MinGW */
// 32-bit
...
void memcpyMMX(void* dst, const void* src, int nbytes)
{
    ...
    alwaysAssertM((int)dst % 16 == 0,
                  format("Must be on 16-byte boundary.  dst = 0x%x", dst));
    alwaysAssertM((int)src % 16 == 0,
                  format("Must be on 16-byte boundary.  src = 0x%x", src));
    ...
    if (remainingBytes > 0) {
        ...
        memcpy((uint8*)dst + (nbytes - remainingBytes),
               (const uint8*)src + (nbytes - remainingBytes),
               remainingBytes);
    }
}
#endif
...
void System::memcpy(void* dst, const void* src, size_t numBytes)
{
    ...
    if (((size_t)dst % 16 == 0) && ((size_t)src % 16 == 0) &&
        (numBytes > 3400000)) {
...
void memfill(void* dst, int n32, unsigned long i)
{
    ...
    if (bytesRemaining > 0) {
        ::memset(
            (uint8*)dst + (originalSize - bytesRemaining), n32, bytesRemaining);
    }
}
#endif
...
static std::string computeAppName(const std::string& start)
{
    ...
    if (start[start.size() - 1] == 'd') {
        ...
        if ((path.size() > 2) &&
            (toLower(path[path.size() - 2]) == toLower(shortName))) {
            return shortName;
        }
    }
...
{
    static std::string n =
        computeAppName(filenameBase(currentProgramFilename()));
    return n;
}
...
void System::sleep(RealTime t)
{
    ...
    // On Windows, a "time slice" is measured in quanta of 3-5 ms
    // (http://support.microsoft.com/kb/259025) Sleep(0) yields the remainder of
    // the time slice, which could be a long time. A 1 ms minimum time
    // experimentally kept the "Empty GApp" at nearly no CPU load at 100 fps,
    ...
    while (remainingTime > 0) {
        ...
        if (remainingTime > minRealSleepTime * 2.5) {
            // Safe to use Sleep with a time... sleep for half the remaining
            // time
...
void System::initTime()
{
    ...
    m_realWorldGetTickTime0 = (RealTime)t.time - t.timezone * G3D::MINUTE +
                              (t.dstflag ? G3D::HOUR : 0);
...
RealTime System::time()
{
    ...
    return ((RealTime)(now.QuadPart - instance().m_start.QuadPart) /
            instance().m_counterFrequency.QuadPart) +
...
#define REALSIZE_FROM_USERPTR(u)                                               \
    (*(size_t*)USERPTR_TO_REALPTR(ptr) + sizeof(size_t))
...
public:
    /** Only store buffers up to these sizes (in bytes) in each pool->
      ...
      */
#ifdef G3D_64BIT
    // 64-bit machines have larger pointers...and probably have more memory as
    // well
...
private:
    /** Pointer given to the program.  Unless in the tiny heap, the user size of
     * the block is stored right in front of the pointer as a uint32.*/
    ...
    inline UserPtr tinyMalloc(size_t bytes)
    {
        ...
        if (tinyPoolSize > 0) {
            ...
            if (tinyPoolSize > 0) {
                ...
                //   "the last two pointers on the freelist are identical
                //   (during tinyMalloc).");
            }
#endif
    ...
    void tinyFree(UserPtr ptr)
    {
        ...
        //           "Tried to free a tiny pool buffer when the tiny pool
        //           freelist is full.");
        ...
        if (tinyPoolSize > 0) {
            ...
            //                    "System::malloc heap corruption detected: "
            //                    "the last two pointers on the freelist are
            //                    identical (during tinyFree).");
        }
#endif
...
public:
    ...
    UserPtr malloc(size_t bytes)
    {
        ...
        if (ptr == NULL) {
            ...
            alwaysAssertM(_CrtCheckMemory() == TRUE,
        ...
        if (ptr == NULL) {
            if ((System::outOfMemoryCallback() != NULL) &&
                (System::outOfMemoryCallback()(USERSIZE_TO_REALSIZE(bytes),
                                               true) == true)) {
    ...
    std::string performance() const
    {
        if (totalMallocs > 0) {
            int pooled =
                mallocsFromTinyPool + mallocsFromSmallPool + mallocsFromMedPool;
...
void* System::calloc(size_t n, size_t x)
{
    ...
    debugAssertM(isValidHeapPointer(b),
                 "System::malloc returned an invalid pointer");
...
// Windows 64-bit
void System::cpuid(
    CPUIDFunction func, uint32& eax, uint32& ebx, uint32& ecx, uint32& edx)
{
...
// for a discussion of why the second version saves ebx; it allows 32-bit code
// to compile with the -fPIC option. On 64-bit x86, PIC code has a dedicated rip
// register for PIC so there is no ebx conflict.
void System::cpuid(
    CPUIDFunction func, uint32& eax, uint32& ebx, uint32& ecx, uint32& edx)
{
