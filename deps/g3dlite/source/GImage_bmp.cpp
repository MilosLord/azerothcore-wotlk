    ...
    if (bitCount == 1) {
        ...
        for (int h = hStart; h != hEnd; h += hDir) {
            ...
            for (int w = 0; w < BMScanWidth; ++w) {
                ...
                // Now we read the pixels. Usually there are eight pixels per
                // byte, since each pixel is represented by one bit, but if the
                // width is not a multiple of eight, the last byte will have
                // some bits set, with the others just being extra. Plus there's
                // the dword-alignment padding. So we keep checking to see if
                // we've already read "width" number of pixels.
                for (int i = 7; i >= 0; --i) {
        ...
        for (int h = hStart; h != hEnd; h += hDir) {
            ...
            for (int w = 0; w < BMScanWidth; w++) {
                ...
                // Now we read the pixels. Usually there are two pixels per
                // byte, since each pixel is represented by four bits, but if
                // the width is not a multiple of two, the last byte will have
                // only four bits set, with the others just being extra. Plus
                // there's the dword-alignment padding. So we keep checking to
                // see if we've already read "Width" number of pixels.
...
void GImage::decodeICO(BinaryInput& input)
{
    ...
    // of 0 represents the first color table entry, a pixel color of 255 (if
    // there are that many) represents the 256th entry.
    /*
       ...
       { int bit = ((byte << j) >> (8 - bitsPerPixel)) & mask; pixel4(x, y) =
       colorTable[bit];
                }
            }
        }
