...
// this is the signature for all conversion routines (same parameters as
// ImageFormat::convert)
typedef void (*ConvertFunc)(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
...
// this defines the conversion routines for converting between compatible
// formats
...
#define DECLARE_CONVERT_FUNC(name)                                             \
    static void name(const Array<const void*>&   srcBytes,                     \
                     int                         srcWidth,                     \
                     int                         srcHeight,                    \
                     const ImageFormat*          srcFormat,                    \
                     int                         srcRowPadBits,                \
...
static const ConvertAttributes sConvertMappings[] = {
    ...
    // L8 ->
    {l8_to_rgb8,
     {ImageFormat::CODE_L8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     false,
     false,
     true},
    ...
    // L32F ->
    {l32f_to_rgb8,
     {ImageFormat::CODE_L32F, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     false,
     false,
     true},
    ...
    {rgb8_to_rgba8,
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_RGBA8, ImageFormat::CODE_NONE},
     false,
     false,
     true},
    {rgb8_to_bgr8,
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_BGR8, ImageFormat::CODE_NONE},
     false,
     false,
     true},
    {rgb8_to_rgba32f,
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_RGBA32F, ImageFormat::CODE_NONE},
     true,
     false,
     true},
    ...
    // RGB32F ->
    {rgb32f_to_rgba32f,
     {ImageFormat::CODE_RGB32F, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_RGBA32F, ImageFormat::CODE_NONE},
     true,
     false,
     true},
    ...
    {rgba32f_to_bayer_rggb8,
     {ImageFormat::CODE_RGBA32F, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_BAYER_RGGB8, ImageFormat::CODE_NONE},
     false,
     true,
     true},
    {rgba32f_to_bayer_gbrg8,
     {ImageFormat::CODE_RGBA32F, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_BAYER_GBRG8, ImageFormat::CODE_NONE},
     false,
     true,
     true},
    {rgba32f_to_bayer_grbg8,
     {ImageFormat::CODE_RGBA32F, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_BAYER_GRBG8, ImageFormat::CODE_NONE},
     false,
     true,
     true},
    {rgba32f_to_bayer_bggr8,
     {ImageFormat::CODE_RGBA32F, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_BAYER_BGGR8, ImageFormat::CODE_NONE},
     false,
     true,
     true},
    ...
    {rgb8_to_yuv420p,
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_YUV420_PLANAR, ImageFormat::CODE_NONE},
     false,
     false,
     false},
    {rgb8_to_yuv422,
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_YUV422, ImageFormat::CODE_NONE},
     false,
     false,
     false},
    {rgb8_to_yuv444,
     {ImageFormat::CODE_RGB8, ImageFormat::CODE_NONE},
     {ImageFormat::CODE_YUV444, ImageFormat::CODE_NONE},
     false,
     false,
     false},
    {yuv420p_to_rgb8,
...
static ConvertFunc findConverter(TextureFormat::Code sourceCode,
    ...
    for (int routineIndex = 0; routineIndex < numRoutines; ++routineIndex) {
        ...
        while (routine.m_sourceFormats[sourceIndex] != ImageFormat::CODE_NONE) {
            ...
            if (routine.m_sourceFormats[sourceIndex] == sourceCode) {
                ...
                while (routine.m_destFormats[destIndex] !=
                       ImageFormat::CODE_NONE) {

                    // check if dest format matches and padding + invert rules
                    // match
                    if ((routine.m_destFormats[destIndex] == destCode) &&
                        (!needsSourcePadding ||
                         (routine.m_handlesSourcePadding ==
                          needsSourcePadding)) &&
                        (!needsDestPadding ||
                         (routine.m_handlesDestPadding == needsDestPadding)) &&
                        (!needsInvertY ||
                         (routine.m_handleInvertY == needsInvertY))) {
...
bool conversionAvailable(const ImageFormat* srcFormat,
                         int                srcRowPadBits,
                         const ImageFormat* dstFormat,
                         int                dstRowPadBits,
                         bool               invertY = false)
{
    ...
    if ((srcFormat->code == dstFormat->code) &&
        (srcRowPadBits == dstRowPadBits) && !invertY) {
        conversionAvailable = true;
    }
    else {
        ConvertFunc directConverter = findConverter(srcFormat->code,
                                                    dstFormat->code,
                                                    srcRowPadBits > 0,
                                                    dstRowPadBits > 0,
                                                    invertY);
...
bool ImageFormat::convert(const Array<const void*>& srcBytes,
                          int                       srcWidth,
                          int                       srcHeight,
                          const ImageFormat*        srcFormat,
                          int                       srcRowPadBits,
                          ...
                          BayerAlgorithm            bayerAlg)
{
    ...
    if ((srcFormat->code == dstFormat->code) &&
        ...
        System::memcpy(
            dstBytes[0],
            srcBytes[0],
            iCeil(((srcWidth * srcFormat->cpuBitsPerPixel + srcRowPadBits) *
                   srcHeight) /
                  8.0f));
        conversionAvailable = true;
    }
        ...
        // if we have a direct converter, use it, otherwise find intermdiate
        // path
        if (directConverter) {
            directConverter(srcBytes,
                            srcWidth,
                            srcHeight,
                            srcFormat,
                            srcRowPadBits,
            ...
            ConvertFunc toInterConverter =
                findConverter(srcFormat->code,
                              ImageFormat::CODE_RGBA32F,
                              srcRowPadBits > 0,
                              false,
                              false);
            ;
            ConvertFunc fromInterConverter =
                findConverter(ImageFormat::CODE_RGBA32F,
                              dstFormat->code,
                              false,
                              dstRowPadBits > 0,
                              invertY);
            ...
            if (toInterConverter && fromInterConverter) {
                Array<void*> tmp;
                tmp.append(System::malloc(
                    srcWidth * srcHeight *
                    ImageFormat::RGBA32F()->cpuBitsPerPixel * 8));

                toInterConverter(srcBytes,
                                 srcWidth,
                                 srcHeight,
                                 srcFormat,
                                 srcRowPadBits,
                                 ...
                                 false,
                                 bayerAlg);
                fromInterConverter(reinterpret_cast<Array<const void*>&>(tmp),
                                   srcWidth,
                                   srcHeight,
                                   ImageFormat::RGBA32F(),
                                   0,
...
// L8 ->
static void l8_to_rgb8(const Array<const void*>&   srcBytes,
                       int                         srcWidth,
                       int                         srcHeight,
                       const ImageFormat*          srcFormat,
                       int                         srcRowPadBits,
                       ...
                       ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        for (int x = 0; x < srcWidth; ++x) {
            int i  = (invertY) ? ((srcHeight - 1 - y) * srcWidth + x)
                               : (y * srcWidth + x);
            int i3 = i * 3;
...
// L32F ->
static void l32f_to_rgb8(const Array<const void*>&   srcBytes,
                         int                         srcWidth,
                         int                         srcHeight,
                         const ImageFormat*          srcFormat,
                         int                         srcRowPadBits,
                         ...
                         ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        ...
        for (int x = 0; x < srcWidth; ++x, ++srcIndex, dstByteOffset += 3) {
            Color3unorm8& d =
                *reinterpret_cast<Color3unorm8*>(dst + dstByteOffset);
...
// RGB8 ->
static void rgb8_to_rgba8(const Array<const void*>&   srcBytes,
                          int                         srcWidth,
                          int                         srcHeight,
                          const ImageFormat*          srcFormat,
                          int                         srcRowPadBits,
...
static void rgb8_to_bgr8(const Array<const void*>&   srcBytes,
                         int                         srcWidth,
                         int                         srcHeight,
                         const ImageFormat*          srcFormat,
                         int                         srcRowPadBits,
...
static void rgb8_to_rgba32f(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
                            const ImageFormat*          srcFormat,
                            int                         srcRowPadBits,
                            ...
                            ImageFormat::BayerAlgorithm bayerAlg)
{
    debugAssertM(
        srcRowPadBits % 8 == 0,
        "Source row padding must be a multiple of 8 bits for this format");
    ...
    for (int y = 0; y < srcHeight; ++y) {
        ...
        for (int x = 0; x < srcWidth; ++x, ++dstIndex, srcByteOffset += 3) {
            const Color3unorm8& s =
                *reinterpret_cast<const Color3unorm8*>(src + srcByteOffset);
...
// BGR8 ->
static void bgr8_to_rgb8(const Array<const void*>&   srcBytes,
                         int                         srcWidth,
                         int                         srcHeight,
                         const ImageFormat*          srcFormat,
                         int                         srcRowPadBits,
...
static void bgr8_to_rgba8(const Array<const void*>&   srcBytes,
                          int                         srcWidth,
                          int                         srcHeight,
                          const ImageFormat*          srcFormat,
                          int                         srcRowPadBits,
...
static void bgr8_to_rgba32f(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
                            const ImageFormat*          srcFormat,
                            int                         srcRowPadBits,
...
// RGBA8 ->
static void rgba8_to_rgb8(const Array<const void*>&   srcBytes,
                          int                         srcWidth,
                          int                         srcHeight,
                          const ImageFormat*          srcFormat,
                          int                         srcRowPadBits,
...
static void rgba8_to_bgr8(const Array<const void*>&   srcBytes,
                          int                         srcWidth,
                          int                         srcHeight,
                          const ImageFormat*          srcFormat,
                          int                         srcRowPadBits,
...
static void rgba8_to_rgba32f(const Array<const void*>&   srcBytes,
                             int                         srcWidth,
                             int                         srcHeight,
                             const ImageFormat*          srcFormat,
                             int                         srcRowPadBits,
                             ...
                             ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        ...
        for (int x = 0; x < srcWidth; ++x, ++dstIndex, srcByteOffset += 4) {
            const Color4unorm8& s =
                *reinterpret_cast<const Color4unorm8*>(src + srcByteOffset);
...
// RGB32F ->
static void rgb32f_to_rgba32f(const Array<const void*>&   srcBytes,
                              int                         srcWidth,
                              int                         srcHeight,
                              const ImageFormat*          srcFormat,
                              int                         srcRowPadBits,
                              ...
                              ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        ...
        for (int x = 0; x < srcWidth;
             ++x, ++dstIndex, srcByteOffset += 3 * sizeof(float)) {
            const Color3& s =
                *reinterpret_cast<const Color3*>(src + srcByteOffset);
            dst[dstIndex] = Color4(Color3(s), 1.0f);
        }
        srcByteOffset += srcRowPadBytes;
    }
...
// RGBA32F ->
static void rgba32f_to_rgb8(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
                            const ImageFormat*          srcFormat,
                            int                         srcRowPadBits,
                            ...
                            ImageFormat::BayerAlgorithm bayerAlg)
{
    debugAssertM(
        dstRowPadBits % 8 == 0,
        "Destination row padding must be a multiple of 8 bits for this format");
...
static void rgba32f_to_rgba8(const Array<const void*>&   srcBytes,
                             int                         srcWidth,
                             int                         srcHeight,
                             const ImageFormat*          srcFormat,
                             int                         srcRowPadBits,
                             ...
                             ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        ...
        for (int x = 0; x < srcWidth; ++x, ++srcIndex, dstByteOffset += 4) {
            Color4unorm8& d =
                *reinterpret_cast<Color4unorm8*>(dst + dstByteOffset);
            const Color4& s = src[srcIndex];
...
static void rgba32f_to_bgr8(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
                            const ImageFormat*          srcFormat,
                            int                         srcRowPadBits,
...
static void rgba32f_to_rgb32f(const Array<const void*>&   srcBytes,
                              int                         srcWidth,
                              int                         srcHeight,
                              const ImageFormat*          srcFormat,
                              int                         srcRowPadBits,
                              ...
                              ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        ...
        for (int x = 0; x < srcWidth;
             ++x, ++srcIndex, dstByteOffset += 3 * sizeof(float)) {
...
#define PIXEL_RGB8_TO_YUV_Y(r, g, b)                                           \
    unorm8::fromBits(iClamp(                                                   \
        ((66 * r.bits() + 129 * g.bits() + 25 * b.bits() + 128) >> 8) + 16,    \
        0,                                                                     \
        255))
#define PIXEL_RGB8_TO_YUV_U(r, g, b)                                           \
    unorm8::fromBits(iClamp(                                                   \
        ((-38 * r.bits() - 74 * g.bits() + 112 * b.bits() + 128) >> 8) + 128,  \
        0,                                                                     \
        255))
#define PIXEL_RGB8_TO_YUV_V(r, g, b)                                           \
    unorm8::fromBits(iClamp(                                                   \
        ((112 * r.bits() - 94 * g.bits() - 18 * b.bits() + 128) >> 8) + 128,   \
        0,                                                                     \
        255))

static void rgb8_to_yuv420p(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
                            const ImageFormat*          srcFormat,
                            int                         srcRowPadBits,
                            ...
                            ImageFormat::BayerAlgorithm bayerAlg)
{
    debugAssertM(srcRowPadBits == 0,
                 "Source row padding must be 0 for this format");
    debugAssertM((srcWidth % 2 == 0) && (srcHeight % 2 == 0),
                 "Source width and height must be a multiple of two");
    ...
    for (int y = 0; y < srcHeight; y += 2) {
        for (int x = 0; x < srcWidth; x += 2) {
            ...
            dstY[yIndex]     = PIXEL_RGB8_TO_YUV_Y(src[srcPixelOffset0].r,
                                               src[srcPixelOffset0].g,
                                               src[srcPixelOffset0].b);
            dstY[yIndex + 1] = PIXEL_RGB8_TO_YUV_Y(src[srcPixelOffset1].r,
                                                   src[srcPixelOffset1].g,
                                                   src[srcPixelOffset1].b);

            yIndex += srcWidth;
            dstY[yIndex]     = PIXEL_RGB8_TO_YUV_Y(src[srcPixelOffset2].r,
                                               src[srcPixelOffset2].g,
                                               src[srcPixelOffset2].b);
            dstY[yIndex + 1] = PIXEL_RGB8_TO_YUV_Y(src[srcPixelOffset3].r,
                                                   src[srcPixelOffset3].g,
                                                   src[srcPixelOffset3].b);

            uint32 blendedPixel = blendPixels(src[srcPixelOffset0].asUInt32(),
                                              src[srcPixelOffset2].asUInt32());
            Color3unorm8 uvSrcColor = Color3unorm8::fromARGB(blendedPixel);

            int uvIndex = y / 2 * srcWidth / 2 + x / 2;
            dstU[uvIndex] =
                PIXEL_RGB8_TO_YUV_U(uvSrcColor.r, uvSrcColor.g, uvSrcColor.b);
            dstV[uvIndex] =
                PIXEL_RGB8_TO_YUV_V(uvSrcColor.r, uvSrcColor.g, uvSrcColor.b);
        }
    }
}

static void rgb8_to_yuv422(const Array<const void*>&   srcBytes,
                           int                         srcWidth,
                           int                         srcHeight,
                           const ImageFormat*          srcFormat,
                           int                         srcRowPadBits,
                           ...
                           ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        for (int x = 0; x < srcWidth; x += 2) {
            ...
            uint32       blendedPixel = blendPixels(src[srcIndex].asUInt32(),
                                              src[srcIndex + 1].asUInt32());
            Color3unorm8 uvSrcColor   = Color3unorm8::fromARGB(blendedPixel);

            dst[dstIndex] = PIXEL_RGB8_TO_YUV_Y(
                src[srcIndex].r, src[srcIndex].g, src[srcIndex].b);

            dst[dstIndex + 1] =
                PIXEL_RGB8_TO_YUV_U(uvSrcColor.r, uvSrcColor.g, uvSrcColor.b);

            dst[dstIndex + 2] = PIXEL_RGB8_TO_YUV_Y(
                src[srcIndex + 1].r, src[srcIndex + 1].g, src[srcIndex + 1].b);
...
static void rgb8_to_yuv444(const Array<const void*>&   srcBytes,
                           int                         srcWidth,
                           int                         srcHeight,
                           const ImageFormat*          srcFormat,
                           int                         srcRowPadBits,
                           ...
                           ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        for (int x = 0; x < srcWidth; ++x) {
            ...
            unorm8 y =
                PIXEL_RGB8_TO_YUV_Y(src[index].r, src[index].g, src[index].b);
            unorm8 u =
                PIXEL_RGB8_TO_YUV_U(src[index].r, src[index].g, src[index].b);
            unorm8 v =
                PIXEL_RGB8_TO_YUV_V(src[index].r, src[index].g, src[index].b);
...
#define PIXEL_YUV_TO_RGB8_R(y, u, v)                                           \
    unorm8::fromBits(iClamp(                                                   \
        (298 * (y.bits() - 16) + 409 * (v.bits() - 128) + 128) >> 8, 0, 255))
#define PIXEL_YUV_TO_RGB8_G(y, u, v)                                           \
    unorm8::fromBits(iClamp((298 * (y.bits() - 16) - 100 * (u.bits() - 128) -  \
                             208 * (v.bits() - 128) + 128) >>                  \
                                8,                                             \
                            0,                                                 \
                            255))
#define PIXEL_YUV_TO_RGB8_B(y, u, v)                                           \
    unorm8::fromBits(iClamp(                                                   \
        (298 * (y.bits() - 16) + 516 * (u.bits() - 128) + 128) >> 8, 0, 255))

static void yuv420p_to_rgb8(const Array<const void*>&   srcBytes,
                            int                         srcWidth,
                            int                         srcHeight,
                            const ImageFormat*          srcFormat,
                            int                         srcRowPadBits,
                            ...
                            ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    for (int y = 0; y < srcHeight; ++y) {
        for (int x = 0; x < srcWidth; x += 2) {
            ...
            rgb->r = PIXEL_YUV_TO_RGB8_R(
                srcY[yOffset], srcU[uvOffset], srcV[uvOffset]);
            rgb->g = PIXEL_YUV_TO_RGB8_G(
                srcY[yOffset], srcU[uvOffset], srcV[uvOffset]);
            rgb->b = PIXEL_YUV_TO_RGB8_B(
                srcY[yOffset], srcU[uvOffset], srcV[uvOffset]);

            rgb += 1;
            rgb->r = PIXEL_YUV_TO_RGB8_R(
                srcY[yOffset + 1], srcU[uvOffset], srcV[uvOffset]);
            rgb->g = PIXEL_YUV_TO_RGB8_G(
                srcY[yOffset + 1], srcU[uvOffset], srcV[uvOffset]);
            rgb->b = PIXEL_YUV_TO_RGB8_B(
                srcY[yOffset + 1], srcU[uvOffset], srcV[uvOffset]);
...
static void yuv422_to_rgb8(const Array<const void*>&   srcBytes,
                           int                         srcWidth,
                           int                         srcHeight,
                           const ImageFormat*          srcFormat,
                           int                         srcRowPadBits,
...
static void yuv444_to_rgb8(const Array<const void*>&   srcBytes,
                           int                         srcWidth,
                           int                         srcHeight,
                           const ImageFormat*          srcFormat,
                           int                         srcRowPadBits,
...
static unorm8 applyFilter(
    const unorm8* I, int x, int y, int w, int h, const float filter[5][5])
{
...
// =====================================================================
static void
rgb8_to_bayer_rggb8(const int w, const int h, const unorm8* src, unorm8* dst)
{
...
static void
rgb8_to_bayer_grbg8(const int w, const int h, const unorm8* src, unorm8* dst)
{
    Color3unorm8* srcColor = (Color3unorm8*)src;
    Color1unorm8* dstColor = (Color1unorm8*)dst;
...
static void
rgb8_to_bayer_bggr8(const int w, const int h, const unorm8* src, unorm8* dst)
{
    Color3unorm8* srcColor = (Color3unorm8*)src;
    Color1unorm8* dstColor = (Color1unorm8*)dst;
...
static void
rgb8_to_bayer_gbrg8(const int w, const int h, const unorm8* src, unorm8* dst)
{
    Color3unorm8* srcColor = (Color3unorm8*)src;
    Color1unorm8* dstColor = (Color1unorm8*)dst;
...
// =====================================================================
static void rgba32f_to_bayer_rggb8(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
                                   ...
                                   ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    rgba32f_to_rgb8(srcBytes,
                    ...
                    invertY,
                    bayerAlg);
    rgb8_to_bayer_rggb8(srcWidth,
                        srcHeight,
                        static_cast<unorm8*>(tmp[0]),
                        static_cast<unorm8*>(dstBytes[0]));

    System::free(tmp[0]);
}

static void rgba32f_to_bayer_gbrg8(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
...
static void rgba32f_to_bayer_grbg8(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
...
static void rgba32f_to_bayer_bggr8(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
...
// =====================================================================
static void
bayer_rggb8_to_rgb8_mhc(int w, int h, const unorm8* in, unorm8* _out)
{
    debugAssert(in != _out);
...
static void
bayer_gbrg8_to_rgb8_mhc(int w, int h, const unorm8* in, unorm8* _out)
{
...
static void
bayer_grbg8_to_rgb8_mhc(int w, int h, const unorm8* in, unorm8* _out)
{
...
static void
bayer_bggr8_to_rgb8_mhc(int w, int h, const unorm8* in, unorm8* _out)
{
    // Run the equivalent function for red
...
// =====================================================================
static void bayer_rggb8_to_rgba32f(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
                                   ...
                                   ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    bayer_rggb8_to_rgb8_mhc(srcWidth,
                            srcHeight,
                            static_cast<const unorm8*>(srcBytes[0]),
                            static_cast<unorm8*>(tmp[0]));
    rgb8_to_rgba32f(reinterpret_cast<Array<const void*>&>(tmp),
                    srcWidth,
                    srcHeight,
                    ImageFormat::RGB8(),
                    0,
...
static void bayer_gbrg8_to_rgba32f(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
                                   ...
                                   ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    bayer_grbg8_to_rgb8_mhc(srcWidth,
                            srcHeight,
                            static_cast<const unorm8*>(srcBytes[0]),
                            static_cast<unorm8*>(tmp[0]));
    rgb8_to_rgba32f(reinterpret_cast<Array<const void*>&>(tmp),
...
static void bayer_grbg8_to_rgba32f(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
                                   ...
                                   ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    bayer_gbrg8_to_rgb8_mhc(srcWidth,
                            srcHeight,
                            static_cast<const unorm8*>(srcBytes[0]),
                            static_cast<unorm8*>(tmp[0]));
    rgb8_to_rgba32f(reinterpret_cast<Array<const void*>&>(tmp),
...
static void bayer_bggr8_to_rgba32f(const Array<const void*>&   srcBytes,
                                   int                         srcWidth,
                                   int                         srcHeight,
                                   const ImageFormat*          srcFormat,
                                   int                         srcRowPadBits,
                                   ...
                                   ImageFormat::BayerAlgorithm bayerAlg)
{
    ...
    bayer_bggr8_to_rgb8_mhc(srcWidth,
                            srcHeight,
                            static_cast<const unorm8*>(srcBytes[0]),
                            static_cast<unorm8*>(tmp[0]));
    rgb8_to_rgba32f(reinterpret_cast<Array<const void*>&>(tmp),
