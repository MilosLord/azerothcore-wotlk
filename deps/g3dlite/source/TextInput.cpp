...
double TextInput::parseNumber(const std::string& _string)
{
    ...
    if (s == "1.#inf00" || s == "1.#inf" || s == "inf" || s == "+inf" ||
        s == "Infinity") {
        return inf();
    }
...
TextInput::Settings::Settings()
    : cppBlockComments(true), cppLineComments(true), otherLineComments(true),
      escapeSequencesInStrings(true), otherCommentCharacter('\0'),
      otherCommentCharacter2('\0'), generateCommentTokens(false),
      generateNewlineTokens(false), signedNumbers(true),
      singleQuotedStrings(true), singleQuoteCharacter('\''), sourceFileName(),
      startingLineNumberOffset(0), msvcFloatSpecials(true),
      simpleFloatSpecials(true), proofSymbols(false), caseSensitive(true)
{
...
std::string TextInput::readUntilDelimiterAsString(const char delimiter1,
    ...
    while (!whitespaceDone) {
        ...
        if (isLineComment) {
            ...
            while (!((c == '*') && (c2 == '/')) && (c != EOF)) {
                ...
                // Eat input char may consume more than one character if there
                // is a newline
                eatInputChar();
    ...
    case '-': // negative number, -, --, -=, or ->
        ...
        if (options.signedNumbers) {
            if (isDigit(c) || (c == '.' && isDigit(peekInputChar(1)))) {
                ...
                if (options.simpleFloatSpecials && (c == 'i') &&
                    (peekInputChar(1) == 'n') && (peekInputChar(2) == 'f') &&
...
numLabel:
    if (isDigit(c) || (c == '.')) {
        ...
        if ((c == '0') && (peekInputChar(1) == 'x')) {
                ...
                if (options.msvcFloatSpecials && (c == '#')) {
                    ...
                    for (int j = 0; j < 2; ++j) {
                        ...
                        if (c == '0') {
                            c = eatAndPeekInputChar();
                            if (c != '0') {
                                throw BadMSVCSpecial("Incorrect floating-point "
                                                     "special (inf or nan) "
                                                     "format.",
                                                     t.line(),
                                                     charNumber);
            ...
            if (!isSpecial && (t._extendedType == Token::FLOATING_POINT_TYPE) &&
                (c == 'f')) {
        ...
        if ((options.trueSymbols.size() > 0) ||
            (options.falseSymbols.size() > 0)) {
        ...
        if (options.simpleFloatSpecials &&
            ((t._string == "nan") || (t._string == "inf"))) {
            t._type         = Token::NUMBER;
            t._extendedType = Token::FLOATING_POINT_TYPE;
        }
        return;
    ...
    // Some unknown token
    throw format(
        "Unrecognized token type beginning with character '%c' (ASCII %d)",
        c,
        c);
    return;
}
...
bool TextInput::readBoolean()
{
    ...
    push(t);
    throw WrongTokenType(options.sourceFileName,
                         t.line(),
                         t.character(),
                         Token::BOOLEAN,
                         t._type);
...
int TextInput::readInteger()
{
    ...
    if (t._extendedType == Token::INTEGER_TYPE) { // common case
        ...
        if (!options.signedNumbers && (t._type == Token::SYMBOL) &&
            ((t._string == "-") || (t._string == "+"))) {
...
void TextInput::readString(const std::string& s)
{
    ...
    push(t);
    throw WrongString(
        options.sourceFileName, t.line(), t.character(), s, t._string);
}
...
void TextInput::readSymbol(const std::string& symbol)
{
    ...
    push(t);
    throw WrongSymbol(
        options.sourceFileName, t.line(), t.character(), symbol, t._string);
}

TextInput::TextInput(const std::string& filename, const Settings& opt)
    : options(opt)
{
    init();
...
void TextInput::initFromString(const char*     str,
                               int             len,
...
TextInput::TextInput(FS fs, const char* str, size_t len, const Settings& opt)
    : options(opt)
{
    (void)fs;
...
TextInput::TokenException::TokenException(const std::string& src,
                                          int                ln,
                                          int                ch)
    : ParseError(src, ln, ch, format("%s(%d) : ", src.c_str(), ln)),
      sourceFile(src)
{
}
...
TextInput::WrongTokenType::WrongTokenType(
    const std::string& src, int ln, int ch, Token::Type e, Token::Type a)
...
TextInput::BadMSVCSpecial::BadMSVCSpecial(const std::string& src,
                                          int                ln,
                                          int                ch)
...
TextInput::WrongSymbol::WrongSymbol(const std::string& src,
                                    int                ln,
                                    int                ch,
                                    const std::string& e,
                                    const std::string& a)
    : TokenException(src, ln, ch), expected(e), actual(a)
{
    ...
    message += format(
        "Expected symbol '%s', found symbol '%s'.", e.c_str(), a.c_str());
}

TextInput::WrongString::WrongString(const std::string& src,
                                    int                ln,
                                    int                ch,
                                    const std::string& e,
                                    const std::string& a)
    : TokenException(src, ln, ch), expected(e), actual(a)
{
    ...
    message += format(
        "Expected string '%s', found string '%s'.", e.c_str(), a.c_str());
}
