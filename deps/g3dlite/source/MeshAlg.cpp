...
void MeshAlg::generateGrid(Array<Vector3>&        vertex,
...
MeshAlg::Geometry& MeshAlg::Geometry::operator=(const MeshAlg::Geometry& src)
{
    ...
    System::memcpy(vertexArray.getCArray(),
                   src.vertexArray.getCArray(),
                   sizeof(Vector3) * vertexArray.size());
    System::memcpy(normalArray.getCArray(),
                   src.normalArray.getCArray(),
                   sizeof(Vector3) * normalArray.size());
...
void MeshAlg::computeNormals(Geometry& geometry, const Array<int>& indexArray)
{
    ...
    computeAdjacency(
        geometry.vertexArray, indexArray, faceArray, edgeArray, vertexArray);

    computeNormals(geometry.vertexArray,
                   faceArray,
                   vertexArray,
...
void MeshAlg::computeNormals(const Array<Vector3>&    vertexGeometry,
...
void MeshAlg::computeFaceNormals(const Array<Vector3>&       vertexArray,
...
void MeshAlg::identifyBackfaces(const Array<Vector3>&       vertexArray,
                                const Array<MeshAlg::Face>& faceArray,
                                const Vector4&              HP,
                                Array<bool>&                backface)
{
...
void MeshAlg::createIndexArray(
    int n, Array<int>& array, int start, int run, int skip)
{
    debugAssert(skip >= 0);
    debugAssert(run >= 0);
...
void MeshAlg::computeAreaStatistics(const Array<Vector3>& vertexArray,
...
void MeshAlg::computeBounds(const Array<Vector3>& vertexArray,
                            const Array<int>&     indexArray,
                            AABox&                box,
                            Sphere&               sphere)
{
...
void MeshAlg::computeTangentSpaceBasis(const Array<Vector3>& vertexArray,
                                       ...
                                       Array<Vector3>&       binormal)
{
    ...
    debugAssertM(faceArray.size() != 0,
                 "Unable to calculate valid tangent space without faces.");
    ...
    for (int f = 0; f < faceArray.size(); ++f) {
        ...
        // See http://www.terathon.com/code/tangent.html for a derivation of the
        // following code
    ...
    for (int v = 0; v < vertexArray.size(); ++v) {
        ...
        debugAssertM(N.isUnit() || N.isZero(),
                     "Input normals must have unit length");
