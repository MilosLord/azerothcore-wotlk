...
bool RegistryUtil::keyExists(const std::string& key)
{
    ...
    HKEY  openKey;
    int32 result =
        RegOpenKeyExA(hkey, (key.c_str() + pos + 1), 0, KEY_READ, &openKey);
...
bool RegistryUtil::valueExists(const std::string& key, const std::string& value)
{
    ...
    if (result == ERROR_SUCCESS) {
        uint32 dataSize = 0;
        result          = RegQueryValueExA(openKey,
                                  value.c_str(),
                                  NULL,
                                  NULL,
                                  NULL,
...
bool RegistryUtil::readInt32(const std::string& key,
                             ...
                             int32&             data)
{
    ...
    if (result == ERROR_SUCCESS) {
        ...
        debugAssertM(result == ERROR_SUCCESS,
...
bool RegistryUtil::readBytes(const std::string& key,
                             const std::string& value,
                             uint8*             data,
                             uint32&            dataSize)
{
...
bool RegistryUtil::readString(const std::string& key,
                              const std::string& value,
                              std::string&       data)
{
    size_t pos = key.find('\\', 0);
    ...
    if (result == ERROR_SUCCESS) {
        ...
        // increment datasize to allow for non null-terminated strings in
        // registry
        dataSize += 1;
...
bool RegistryUtil::writeInt32(const std::string& key,
                              const std::string& value,
                              int32              data)
{
    size_t pos = key.find('\\', 0);
    ...
    HKEY  openKey;
    int32 result =
        RegOpenKeyExA(hkey, (key.c_str() + pos + 1), 0, KEY_WRITE, &openKey);
    debugAssert(result == ERROR_SUCCESS || result == ERROR_FILE_NOT_FOUND);

    if (result == ERROR_SUCCESS) {
        result = RegSetValueExA(openKey,
                                value.c_str(),
                                0,
                                REG_DWORD,
        ...
        debugAssertM(result == ERROR_SUCCESS,
                     "Could not write registry key value.");
...
bool RegistryUtil::writeBytes(const std::string& key,
                              const std::string& value,
                              const uint8*       data,
                              uint32             dataSize)
{
...
bool RegistryUtil::writeString(const std::string& key,
                               const std::string& value,
