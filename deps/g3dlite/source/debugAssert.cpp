 ...
 */
static void createErrorMessage(const char*        expression,
                               ...
                               std::string&       outMessage)
{
    ...
    outTitle               = outTitle + string(" - ") +
               string(moduleName ? (moduleName + 1) : modulePath);
    ...
    // Build the message.
    outMessage = G3D::format("%s%s%sExpression: %s%s%s:%d%s%s%s",
                             message.c_str(),
                             newline,
                             newline,
                             expression,
                         ...
                         bool               useGuiPrompt)
{
    ...
    createErrorMessage(
        expression, message, filename, lineNumber, dialogTitle, dialogText);
    ...
    int result = G3D::prompt(dialogTitle.c_str(),
                             dialogText.c_str(),
                             (const char**)choices,
                             3,
                             useGuiPrompt);
...
bool _handleErrorCheck_(const char*        expression,
                        const std::string& message,
                        const char*        filename,
                        int                lineNumber,
                        bool               useGuiPrompt)
{
    ...
    const std::string& m = std::string("An internal error has occured in this "
                                       "program and it will now close.  "
                                       "The specific error is below. More "
                                       "information has been saved in \"") +
                           Log::getCommonLogFilename() + "\".\n\n" + dialogText;

    int result =
        G3D::prompt("Error", m.c_str(), (const char**)choices, 1, useGuiPrompt);
    (void)result;
