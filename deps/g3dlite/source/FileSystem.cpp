...
void FileSystem::Dir::computeZipListing(const std::string& zipfile,
                                        const std::string& _pathInsideZipfile)
{
#if _HAVE_ZIP /* G3DFIX: Use ZIP-library only if defined */
    const std::string& pathInsideZipfile =
        FilePath::canonicalize(_pathInsideZipfile);
    struct zip* z = zip_open(
        FilePath::removeTrailingSlash(zipfile).c_str(), ZIP_CHECKCONS, NULL);
    debugAssert(z);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        zip_stat_init(
            &info); // TODO: Docs unclear if zip_stat_init is required.
        ...
        if (beginsWith(name, pathInsideZipfile)) {
            ...
            // For building the cached directory listing, extract only elements
            // that do not contain additional subdirectories.
...
FileSystem::Dir& FileSystem::getContents(const std::string& path,
                                         bool               forceUpdate)
{
    const std::string& key =
#if defined(G3D_WINDOWS)
        FilePath::canonicalize(FilePath::removeTrailingSlash(
            toLower(FilePath::canonicalize(resolve(path)))));
#else
        FilePath::canonicalize(FilePath::removeTrailingSlash(
            FilePath::canonicalize(resolve(path))));
#endif
    ...
    if ((now > dir.lastChecked + cacheLifetime()) || forceUpdate) {
        ...
        dir.lastChecked = now;
#ifdef G3D_WINDOWS
        // On windows, we have to use GetFileAttributes
        // (http://msdn.microsoft.com/en-us/library/aa364944(v=vs.85).aspx)
        // instead of stat in order to work with network shares
...
bool FileSystem::_isZipfile(const std::string& _filename)
{
    const std::string& filename =
        FilePath::canonicalize(FilePath::expandEnvironmentVariables(_filename));
...
#ifdef G3D_WINDOWS
            toLower(FilePath::canonicalize(
                FilePath::removeTrailingSlash(_resolve(path))));
#else
...
bool FileSystem::_exists(const std::string& _f,
                         bool               trustCache,
                         bool               caseSensitive)
{
    ...
    if (FilePath::containsWildcards(f)) {
        ...
        for (int i = 0; i < entry.nodeArray.size(); ++i) {
            if (FilePath::matches(
                    entry.nodeArray[i].name, pattern, caseSensitive)) {
                return true;
            }
        }
        ...
        return entry.exists &&
               entry.contains(FilePath::baseExt(path), caseSensitive);
    }
}
...
{
    const std::string& filename =
        FilePath::expandEnvironmentVariables(_filename);
    // TODO: work with zipfiles and cache
    struct _stat st;
    const bool   exists =
        _stat(FilePath::removeTrailingSlash(filename).c_str(), &st) != -1;
    return exists && ((st.st_mode & S_IFDIR) != 0);
}

void FileSystem::_removeFile(const std::string& path)
{
    alwaysAssertM(!inZipfile(path),
                  "Cannot invoke removeFile() on files inside zipfiles.");
...
std::string FileSystem::_resolve(const std::string& _filename,
    ...
    if (result == -1) {
        ...
        if (zipfileExists(filename, zip, contents)) {
            ...
            debugAssertM(z != NULL, zip + ": zip open failed.");
            {
                struct zip_stat info;
                zip_stat_init(
                    &info); // Docs unclear if zip_stat_init is required.
                int success =
                    zip_stat(z, contents.c_str(), ZIP_FL_NOCASE, &info);
                (void)success;
                debugAssertM(success == 0,
                             zip + ": " + contents + ": zip stat failed.");
...
void FileSystem::listHelper(const std::string&  shortSpec,
    ...
    for (int i = 0; i < dir.nodeArray.size(); ++i) {
        ...
        if (FilePath::matches(entry.name, shortSpec, settings.caseSensitive)) {
            ...
            if ((entry.type == UNKNOWN) &&
                (!(settings.files && settings.directories) ||
                 settings.recursive)) {
                // Update the type: it is unknown and we'll need to branch onit
                // below
                entry.type =
                    isDirectory(FilePath::concat(parentPath, entry.name))
                        ? DIR_TYPE
                        : FILE_TYPE;
            }
        ...
        if (settings.recursive) {
            ...
            if (entry.type == DIR_TYPE) {
                listHelper(shortSpec,
                           FilePath::concat(parentPath, entry.name),
                           result,
                           settings);
            }
        }
...
void FileSystem::_list(const std::string&  _spec,
                       Array<std::string>& result,
                       const ListSettings& settings)
{
...
const Array<std::string>& FileSystem::_drives()
{
    if (m_winDrive.length() == 0) {
        ...
        // Drive list is a series of NULL-terminated strings, itself terminated
        // with a NULL.
...
bool FilePath::isRoot(const std::string& f)
{
    ...
    // Windows shares are considered roots, but only if this does not include a
...
std::string FilePath::concat(const std::string& dirname,
                             const std::string& file)
{
    // Ensure that the directory ends in a slash
    if (!dirname.empty() && !isSlash(dirname[dirname.size() - 1]) &&
...
bool FilePath::matches(const std::string& path,
...
void FilePath::parse(const std::string&  filename,
                     std::string&        root,
                     Array<std::string>& path,
                     std::string&        base,
                     std::string&        ext)
...
std::string FilePath::expandEnvironmentVariables(const std::string& path)
{
    ...
    while (end != std::string::npos) {
        ...
        if (path[start] == '(') {
            ...
            if (end == std::string::npos) {
                throw std::string(
                    "Missing close paren in environment variable in \"") +
                    path + "\"";
            }
            ...
            if ((size_t(i) != std::string::npos) &&
                ((end == std::string::npos) || (size_t(i) < end))) {
                end = i;
            }
            if (end == std::string::npos) {
                // If the varible goes to the end of the string, it is the rest
                // of the string
                end = path.size();
            }
            else {
        ...
        if (!var.empty()) {
            ...
            if (value == NULL) {
                throw(std::string("LocalLightingEnvironment variable \"") +
                      var + "\" not defined for path \"" + path + "\"");
            }
            else {
                result += value;
            }
            ...
            // We just parsed an "empty" variable, which was probably a default
            // share on Windows, e.g.,
