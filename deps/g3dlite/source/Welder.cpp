...
struct HashTrait<G3D::_internal::VN> {
    static size_t hashCode(const G3D::_internal::VN& k)
    {
        return static_cast<size_t>(k.vertex.hashCode());
    }
};
template <>
struct HashTrait<G3D::_internal::VNTi> {
    static size_t hashCode(const G3D::_internal::VNTi& k)
    {
        return static_cast<size_t>(k.vertex.hashCode());
    }
};
...
struct EqualsTrait<G3D::_internal::VN> {
    static bool equals(const G3D::_internal::VN& a, const G3D::_internal::VN& b)
    {
...
struct EqualsTrait<G3D::_internal::VNTi> {
    static bool equals(const G3D::_internal::VNTi& a,
                       const G3D::_internal::VNTi& b)
    {
        return a.vertex == b.vertex;
    }
};
...
struct PositionTrait<G3D::_internal::VN> {
    static void getPosition(const G3D::_internal::VN& v, G3D::Vector3& p)
    {
        p = v.vertex;
    }
};
...
struct PositionTrait<G3D::_internal::VNTi> {
    static void getPosition(const G3D::_internal::VNTi& v, G3D::Vector3& p)
    {
        p = v.vertex;
    }
};
...
private:
    ...
    int getIndex(const Vector3& v, const Vector3& n, const Vector2& t)
    {
        ...
        if (n.isZero()) {
            // Don't bother trying to match the surface normal, since this
            // vertex has no surface normal.
            while (it.isValid()) {
        ...
        // Note that a sliver triangle processed before its neighbors may reach
        // here with a zero length normal.
    ...
    /**
     ...
     Called from process()
     */
    void updateTriLists(Array<Array<int>*>&   indexArrayArray,
        ...
        for (int t = 0; t < numTriLists; ++t) {
            if (indexArrayArray[t] != NULL) {
                ...
                for (int v = 0; v < triList.size(); ++v) {
                    ...
                    triList[v] = getIndex(
                        vertexArray[u], normalArray[u], texCoordArray[u]);
    ...
    /** Expands the indexed triangle lists into a triangle list.
        ...
        Called from process() */
    void unroll(const Array<Array<int>*>& indexArrayArray,
                const Array<Vector3>&     vertexArray,
                const Array<Vector2>&     texCoordArray,
    ...
    /** For every three vertices, compute the face normal and store it three
       times. Sliver triangles have a zero surface normal, which we will later
       take to match *any* surface normal. */
    void computeFaceNormals(const Array<Vector3>& vertexArray,
                            Array<Vector3>&       faceNormalArray)
    {
        ...
        debugAssertM(vertexArray.size() % 3 == 0,
                     "Input is not a triangle soup");
        ...
        for (int v = 0; v < vertexArray.size(); v += 3) {
            ...
            // Note that the length may be zero in the case of sliver polygons,
            // e.g., those correcting a T-junction.  Scale up by 256 to avoid
            // underflow when multiplying very small edges
    ...
    /**
     Computes @a smoothNormalArray, whose elements are those of normalArray
     averaged with neighbors within the angular cutoff.
     */
    void smoothNormals(const Array<Point3>&  vertexArray,
                       const Array<Vector3>& normalArray,
                       Array<Vector3>&       smoothNormalArray)
    {
        ...
        MemoryManager::Ref mm = AreaMemoryManager::create(
            iRound(sizeof(VN) * normalArray.size() * 1.5));
        ...
        if (vertexWeldRadius == 0) {
            ...
            for (int v = 0; v < vertexArray.size(); ++v) {
                bool                    ignore = false;
                SmallArray<Vector3, k>& list =
                    normalTable.getCreate(vertexArray[v], ignore);
            ...
            for (int v = 0; v < vertexArray.size(); ++v) {
                ...
                const SmallArray<Vector3, k>& list =
                    normalTable[vertexArray[v]];
                ...
                for (int i = 0; i < list.size(); ++i) {
                    ...
                    if (cosAngle > cosThresholdAngle) {
                        // This normal is close enough to consider.  Avoid
                        // underflow by scaling up
                        sum += (N * 256.0f);
                    }
                }
            ...
            // Compute a hash grid so that we can find neighbors quickly.
            alwaysAssertM(vertexWeldRadius > 0,
                          "Cannot smooth with zero vertex weld radius");
            ...
            for (int v = 0; v < normalArray.size(); ++v) {
                // Compute the sum of all nearby normals within the cutoff
                // angle. Search within the vertexWeldRadius, since those are
                // the vertices that will collapse to the same point.
...
public:
    ...
    4. Generate output indexArrayArray.  While doing so, merge all vertices
    where the distance between position, texCoord, and normal is within the
    thresholds.
     */
    void process(Array<Vector3>&     vertexArray,
                 ...
                 float               normRadius)
    {
        ...
        unroll(indexArrayArray,
               vertexArray,
               texCoordArray,
               unrolledVertexArray,
               unrolledTexCoordArray);
        ...
        if (unrolledFaceNormalArray.size() > 0) {
            smoothNormals(unrolledVertexArray,
                          unrolledFaceNormalArray,
                          unrolledSmoothNormalArray);
            unrolledFaceNormalArray.clear();
        }

        // Regenerate the triangle lists
        updateTriLists(indexArrayArray,
                       unrolledVertexArray,
                       unrolledSmoothNormalArray,
                       unrolledTexCoordArray);
...
void Welder::weld(Array<Vector3>&         vertexArray,
                  Array<Vector2>&         texCoordArray,
                  Array<Vector3>&         normalArray,
                  Array<Array<int>*>&     indexArrayArray,
                  const Welder::Settings& settings)
{
    ...
    _internal::WeldHelper(settings.vertexWeldRadius)
        .process(vertexArray,
                 texCoordArray,
                 normalArray,
                 indexArrayArray,
                 settings.normalSmoothingAngle,
                 settings.textureWeldRadius,
                 settings.normalWeldRadius);
}
