...
{
    return UprightFrame(
        translation + other.translation, pitch + other.pitch, yaw + other.yaw);
}
...
void UprightFrame::unwrapYaw(UprightFrame* a, int N)
{
    ...
    for (int i = 1; i < N; ++i) {
        ...
        if (abs(cur - prev) > G3D::pi()) {
            ...
            // Find the difference -pi < diff < pi between the current and
            // previous values
            float diff = c - p;
            if (diff < -G3D::pi()) {
                diff += (float)twoPi();
...
UprightSpline::UprightSpline(const Any& any)
{
    ...
    for (int controlIndex = 0; controlIndex < control.length();
         ++controlIndex) {
