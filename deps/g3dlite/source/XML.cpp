...
void XML::serialize(TextOutput& t) const
{
    if (m_type == VALUE) {
        ...
        for (AttributeTable::Iterator it = m_attribute.begin(); it.isValid();
             ++it) {
...
begin:
    ...
    if ((n.type() == Token::SYMBOL) && (n.string() == "<")) {
        ...
        if ((n.string() == "?") || ((n.string() == "!") && !isComment)) {
            ...
            while (t.hasMore() && !((n.type() == Token::SYMBOL) &&
                                    (n.string() == ">") && prevWasDash)) {
                prevWasDash =
                    (n.type() == Token::SYMBOL) && (n.string() == "--");
                n = t.read();
            }
        ...
        while (!done) {
            ...
            if (n.string() == "/") {
                ...
                // Read close tag (we wouldn't be here unless it parses
                // correctly)
        ...
        // Beginning embedded content.  Read until the end of file or the next
        // tag.
        m_type = VALUE;
        m_value += n.string();
