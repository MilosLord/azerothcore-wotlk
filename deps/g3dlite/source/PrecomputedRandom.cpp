...
PrecomputedRandom::PrecomputedRandom(int dataSize, uint32 seed)
    : Random((void*)NULL), m_hemiUniform(NULL), m_sphereBits(NULL),
      m_modMask(dataSize - 1), m_freeData(true)
{
    ...
    SphereBitsData*  s;
    m_hemiUniform = h =
        (HemiUniformData*)System::malloc(sizeof(HemiUniformData) * dataSize);
    m_sphereBits = s =
        (SphereBitsData*)System::malloc(sizeof(SphereBitsData) * dataSize);
...
PrecomputedRandom::PrecomputedRandom(const HemiUniformData* data1,
                                     const SphereBitsData*  data2,
                                     int                    dataSize,
                                     uint32                 seed)
    : Random((void*)NULL), m_hemiUniform(data1), m_sphereBits(data2),
      m_modMask(dataSize - 1), m_freeData(false)
{
...
void PrecomputedRandom::cosPowHemi(const float k, float& x, float& y, float& z)
{
    ...
    // Add a very tiny offset to handle the (almost impossibly unlikely) case
    // where z = 1 and x^2+y^2 = 0.
    static const float eps = 0.000001f;
    const float        s =
        sqrt((1.0f + eps - square(z)) / (square(x) + square(y) + eps));
