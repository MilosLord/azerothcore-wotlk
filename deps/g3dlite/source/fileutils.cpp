...
std::string pathConcat(const std::string& dirname, const std::string& file)
{
    ...
    if ((dirname.size() != 0) && (dirname[dirname.size() - 1] != '/') &&
...
std::string readWholeFile(const std::string& filename)
{
    ...
    if (!FileSystem::inZipfile(filename, zipfile)) {
        ...
        if (!FileSystem::exists(filename)) {
            throw FileNotFound(
                filename,
                std::string("File not found in readWholeFile: ") + filename);
        }
        ...
        std::string internalFile =
            FilePath::canonicalize(filename.substr(zipfile.length() + 1));
        struct zip* z = zip_open(zipfile.c_str(), ZIP_CHECKCONS, NULL);
        {
            ...
            char* buffer = reinterpret_cast<char*>(
                System::alignedMalloc(info.size + 1, 16));
            buffer[info.size] = '\0';
            ...
            if (zf == NULL) {
                ...
                debugAssertM(
                    (int64)bytesRead == (int64)info.size,
                    internalFile +
...
///////////////////////////////////////////////////////////////////////////////
void writeWholeFile(const std::string& filename,
...
static void _zip_resolveDirectory(std::string&              completeDir,
                                  const std::string&        drive,
                                  const Array<std::string>& path,
                                  const int                 length)
{
    completeDir = drive;
    int tempLength;
...
static bool _zip_zipContains(const std::string& zipDir,
                             const std::string& desiredFile)
{
 ...
 */
static void getFileOrDirListNormal(const std::string&  filespec,
...
/**
 ...
 @param prefix Directory inside the zipfile. No leading slash, must have
 trailing slash if non-empty.
 @param file   Name inside the zipfile that we are testing to see if it matches
 prefix + "*"
 */
...
static void determineFileOrDirList(const std::string&  filespec,
                                   Array<std::string>& files,
                                   bool                wantFiles,
                                   bool                includePath)
{
    ...
    if ((path == "") || FileSystem::exists(path)) {
        if ((path != "") && FileSystem::isZipfile(path)) {
            ...
            debugAssertM(filenameBaseExt(filespec) == "*",
                         "Can only call getFiles/getDirs on zipfiles using '*' "
                         "wildcard");
        ...
        debugAssertM(
            filenameBaseExt(filespec) == "*",
            "Can only call getFiles/getDirs on zipfiles using '*' wildcard");
        getFileOrDirListZip(path, prefix, files, wantFiles, includePath);
    }
}
...
void getDirs(const std::string&  filespec,
             Array<std::string>& files,
             bool                includePath)
{
