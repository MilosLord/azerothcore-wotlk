...
void BinaryOutput::reallocBuffer(size_t bytes, size_t oldBufferLen)
{
    ...
    if ((m_filename == "<memory>") ||
        (newBufferLen < MAX_BINARYOUTPUT_BUFFER_SIZE)) {
    ...
    if ((newBuffer == NULL) && (bytes > 0)) {
        ...
        alwaysAssertM(m_filename != "<memory>",
                      "Realloc failed while writing to memory.");
...
{
    if (m_filename == "<memory>") {
        throw "Out of memory while writing to memory in BinaryOutput (no RAM "
              "left).";
    }
    else if ((int)bytes > (int)m_maxBufferLen) {
        throw "Out of memory while writing to disk in BinaryOutput (could not "
...
void BinaryOutput::compress(int level)
{
    if (m_alreadyWritten > 0) {
        throw "Cannot compress huge files (part of this file has already been "
              "written to disk).";
    }
    debugAssertM(!m_committed, "Cannot compress after committing.");
    alwaysAssertM(m_bufferLen < 0xFFFFFFFF,
                  "Compress only works for 32-bit files.");
    ...
    // Write the header containing the old buffer size, which is needed for
    // decompression
    {
    ...
    int result = compress2(
        m_buffer + 4, &compressedSize, src, srcSize, iClamp(level, 0, 9));
...
void BinaryOutput::commit(bool flush)
{
    ...
    if (m_ok) {
        ...
        if (m_buffer != NULL) {
            ...
            (void)success;
            debugAssertM(success == 1,
                         std::string("Could not write to '") + m_filename +
                             "'");
        }
        if (flush) {
            fflush(file);
