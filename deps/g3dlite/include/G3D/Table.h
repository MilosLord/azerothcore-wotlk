...
/**
 ...
 There are two ways of definining custom hash functions (G3D provides built-in
 ones for most classes):
 ...
 public:
  ...
  template<> struct HashTrait<class Foo> {
       static size_t hashCode(const Foo& key) { return
 HashTrait<std::string>::hashCode(key.name) + key.index; }
  };
 ...
 Key must be a pointer, an int, a std::string or provide overloads for:
  ...
  <PRE>
    template<> struct HashTrait<class Key> {
        static size_t hashCode(const Key& key) { return
 ...
 G3D pre-defines HashTrait specializations for common types (like
 <CODE>int</CODE> and <CODE>std::string</CODE>). If you use a Table with a
 different type you must write those functions yourself.  For example, an enum
 would use:

  <PRE>
    template<> struct HashTrait<MyEnum> {
        static size_t hashCode(const MyEnum& key) const { return
...
private:
    ...
    private:
        ...
        Node(const Key& k, size_t h, Node* n) : entry(k), hashCode(h), next(n)
        {
            debugAssert((next == NULL) || isValidHeapPointer(next));
        }

    public:
        static Node* create(const Key&          k,
                            const Value&        v,
                            size_t              h,
                            Node*               n,
        ...
        static Node*
        create(const Key& k, size_t hashCode, Node* n, MemoryManager::Ref& mm)
        {
            Node* node = (Node*)mm->alloc(sizeof(Node));
        ...
        */
        ...
        {
            return create(this->entry.key,
                          this->entry.value,
                          hashCode,
                          (next == NULL) ? NULL : next->clone(mm),
                          mm);
        }
    };
...
public:
    ...
    /**
        Recommends that the table resize to anticipate at least this number of
       elements.
     */
    ...
    /**
       ...
       deallocate things that the table points at, use getKeys() and
       Array::deleteAll() to delete them.
    */
    ...
    */
    ...
    private:
        ...
        Iterator(size_t numBuckets, Node** m_bucket)
            : index(0), node(NULL), m_numBuckets(numBuckets), m_bucket(m_bucket)
        {
            ...
            for (unsigned int i = 0; i < m_numBuckets; ++i) {
                debugAssert((m_bucket[i] == NULL) ||
                            isValidHeapPointer(m_bucket[i]));
            }
#endif
        ...
        /**
         If node is NULL, then finds the next element by searching through the
         bucket array. Sets isDone if no more nodes are available.
         */
        void findNext()
        {
            while (node == NULL) {
...
private:
                ...
                bool       updateRemoved)
    {
        ...
        do {
            if ((code == n->hashCode) &&
                EqualsFunc::equals(n->entry.key, key)) {
    ...
    Entry* getEntryPointer(const Key& key) const
    {
        ...
        while (node != NULL) {
            if ((node->hashCode == code) &&
                EqualsFunc::equals(node->entry.key, key)) {
                return &(node->entry);
            }
            node = node->next;
        }
...
public:
    /** If a value that is EqualsFunc to @a member is present, returns a pointer
    ...
    /**
     If the key is present in the table, val is set to the associated value and
     returns true. If the key is not present, returns false.
     */
    ...
    */
    Entry& getCreateEntry(const Key& key, bool& created)
    {
        ...
        const int bucketsPerElement =
            (m_size > 50000)
                ? 3
                : ((m_size > 10000) ? 5 : ((m_size > 5000) ? 10 : 15));
    ...
    /** Will contain duplicate values if they exist in the table.  This array is
     * parallel to the one returned by getKeys() if the table has not been
     * modified. */
