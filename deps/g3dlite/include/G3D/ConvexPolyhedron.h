...
public:
    ...
    */
    ...
    /**
       ...
       @param newEdge  If a new edge was introduced, this is that edge (on the
       above portion; the below portion is the opposite winding.
    */
    ...
    /**
       When a cut plane grazes a vertex in the polygon, two near-identical
       vertices may be created. The closeness of these two points can cause a
       number of problems, such as ConvexPolygon::normal() returning an infinite
       vector.  It should be noted, however, that not all applications are
...
public:
    ...
    /**
       Cuts the polyhedron at the plane. If the polyhedron is entirely above or
       below the plane, one of the returned polyhedra will be empty.
     ...
     */
    void
    cut(const Plane& plane, ConvexPolyhedron& above, ConvexPolyhedron& below);
};
...
public:
    ...
    /**
     ...
     @param reverse If true, the points are reversed (i.e. winding direction is
     changed) before the polygon is created.
     */
    ...
    /** @param reverseWinding If true, the winding direction of the polygon is
     * reversed for this test.*/
