...
/**
 Thomas Wang's 64-to-32-bit mix hash based on Robert Jenkin's hash
 http://www.concentric.net/~ttwang/tech/inthash.htm

 Found by Morgan to produce the best net performance for building tables from
 Vector4int16
*/
...
struct HashTrait<G3D::uint64> {
    ...
    {
        return static_cast<size_t>(k) ^ static_cast<size_t>(k >> 32);
    }
};
...
struct HashTrait<G3D::int64> {
    ...
    {
        return HashTrait<G3D::uint64>::hashCode(G3D::uint64(k));
    }
};
...
struct HashTrait<G3D::uint128> {
    static size_t hashCode(G3D::uint128 key)
    {
        ...
        // return HashTrait<G3D::uint64>::hashCode(key.hi) ^
        // HashTrait<G3D::uint64>::hashCode(key.lo);
