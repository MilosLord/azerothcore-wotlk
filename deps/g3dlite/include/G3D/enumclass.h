...
\def G3D_DECLARE_ENUM_CLASS_METHODS
  ...
  \brief Creates a series of methods that turn a class into a scoped
enumeration.
  ...
  public:
    ...
    static const char* toString(int i, Value& v) {
        static const char* str[] = {"FUEL", "FOOD", "WATER", NULL}; // Whatever
your enum values are static const Value val[] = {FUEL, FOOD, WATER}; // Whatever
your enum values are const char* s = str[i]; if (s) { v = val[i];
        }
        return s;
    }
...
private:                                                                       \
    ...
    {                                                                          \
        ...
        do {                                                                   \
            s = toString(i, v);                                                \
            if (s == NULL) {                                                   \
                return; /** Needed to get correct compilation on gcc */        \
            }                                                                  \
            if (x == s) {                                                      \
                value = v;                                                     \
                return;                                                        \
...
public:                                                                        \
    ...
    {                                                                          \
        fromString(a.string());                                                \
                                                                               ...
                                                                               \
    G3D::Any toAny() const { return G3D::Any(toString()); }                    \
                                                                               \
    ...
    {                                                                          \
        return Classname((int)value - x);                                      \
                                                                               ...
                                                                               \
    unsigned int hashCode() const { return (unsigned int)value; }              \
                                                                               \
    void serialize(G3D::BinaryOutput& b) const { b.writeInt32(value); }        \
                                                                               \
    void deserialize(G3D::BinaryInput& b) { value = (Value)b.readInt32(); }

/** \def G3D_DECLARE_ENUM_CLASS_HASHCODE
   Must be used at top level (i.e., not inside a class or namespace), with a
...
\code
// Arguments may not have initializer expressions.  Arguments may contain
comments.
// Namespaces aren't *required*, this example just shows how to use them.
namespace Foo {
   G3D_DECLARE_ENUM_CLASS(Day, SUNDAY, MONDAY, TUESDAY, WEDNESDAY, THURSDAY,
FRIDAY, SATURDAY);
}
...
\sa G3D_DECLARE_ENUM_CLASS_METHODS, G3D_DECLARE_ENUM_CLASS_HASHCODE,
G3D::enumToJavaScriptDeclaration, G3D_BEGIN_ENUM_CLASS_DECLARATION
*/
...
\code
G3D_BEGIN_ENUM_CLASS_DECLARATION(Day, SUNDAY, MONDAY, TUESDAY, WEDNESDAY,
...
#define G3D_BEGIN_ENUM_CLASS_DECLARATION(ClassName, ...)                                                        \
    ...
    public:                                                                                                     \
        enum Value { __VA_ARGS__ } value;                                                                       \
                                                                                                                \
        /* The static variables here may be duplicated in different shared                                      \
         * object binaries (DLLs), but that's ok--we only depend on their                                       \
         * values, not their uniqueness.  See also                                                              \
         * http://stackoverflow.com/questions/11962918/local-static-variable-is-instantiated-multiple-times-why \
         */                                                                                                     \
        static const char* toString(int i, Value& v)                                                            \
        {                                                                                                       \
            static const char** str =                                                                           \
                G3D::_internal::smartEnumParseNames(#__VA_ARGS__);                                              \
...
{
    t.printf(
        "/* BEGIN GENERATED CODE. The following was automatically generated by "
        "G3D::enumToJavaScriptDeclaration().  Do not edit it manually. */\n\n");
    t.printf("var %s = (function (propList) {", EnumClass::classname());
    t.writeNewline();
    t.pushIndent();
    {
        t.printf("// Define immutable properties");
        t.writeNewline();
        t.printf("var en = {};");
        t.writeNewline();
        t.printf("for (var i = 0; i < propList.length; i += 2)");
        t.writeNewline();
        t.pushIndent();
        {
            t.printf("Object.defineProperty(en, propList[i], {enumerable: "
                     "true, value: propList[i + 1]});");
            t.writeNewline();
        }
        t.popIndent();
        t.printf("return en;");
