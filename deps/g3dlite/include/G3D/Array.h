...
/**
 ...
 the overhead of repeatedly allocating the array as it grows from 1, to 2, and
 so on. If you are creating a lot of small Arrays, however, you may want to set
 this smaller to reduce the memory cost. Once the array has been allocated, it
 will never deallocate the underlying array unless MIN_ELEMENTS is set to 0,
 MIN_BYTES is 0, and the array is empty.
...
private:
    /** Once the array has been allocated, it will never deallocate the
    underlying array unless MIN_ELEMENTS is set to 0, MIN_BYTES is 0, and the
    array is empty. */
    ...
    /**
     ...
     and then copies at most oldNum elements from the old array to it.
     Destructors are called for oldNum elements of the old array.
     */
    void realloc(size_t oldNum)
    {
        T* oldData = data;

        // The allocation is separate from the constructor invocation because we
        // don't want to pay for the cost of constructors until the newly
        // allocated elements are actually revealed to the application.  They
        ...
        // Call the copy constructors
        {
            ...
            for (T* ptr = data; ptr < end; ++ptr, ++oldPtr) {
                ...
                // that we determined.  Use the copy constructor to make the
                // assignment.
                const T* constructed = new (ptr) T(*oldPtr);

                (void)constructed;
                debugAssertM(constructed == ptr,
                             "new returned a different address than the one "
                             "provided by Array.");
            }
        }

        // Call destructors on the old array (if there is no destructor, this
        // will compile away)
        {
...
public:
    /**
     Assignment operator.  Will be private in a future release because this is
     slow and can be invoked by accident by novice C++ programmers. If you
     really want to copy an Array, use the explicit copy constructor.
     */
...
public:
    ...
    /** Exchanges all data between the two arrays, which are required to have a
       common MemoryManager. This is a convenient way to avoid large array
       copies when handing off data without involving reference counting or
       manual memory management. Beware that pointers or references into the
       arrays will access memory in the <i>other</i> array after the swap. */
    static void swap(Array<T, MIN_ELEMENTS>& a, Array<T, MIN_ELEMENTS>& b)
    {
        alwaysAssertM(
            a.memoryManager() == b.memoryManager(),
            "The arrays are required to have the same memory manager");
    ...
    /**
     Copy constructor.  Copying arrays is slow...perhaps you want to pass a
     reference or a pointer instead?
     */
    // TODO: patch rest of the API to prevent returning Arrays by value, then
    // make explicit
    ...
    /* Sets this to hold the same contents as other, with num = numAllocated (no
    ...
    /** Resizes this to match the size of \a other and then copies the data from
     * other using memcpy.  This is only safe for POD types */
    void copyPOD(const Array<T>& other)
    {
        static_assert(std::is_standard_layout<T>::value,
                      "copyPOD called on non-POD type");
    ...
    /** Resizes this to just barely match the size of \a other + itself and then
       copies the data to the end of the array from other using memcpy. This is
       only safe for POD types */
    void appendPOD(const Array<T>& other)
    {
        static_assert(std::is_standard_layout<T>::value,
                      "appendPOD called on non-POD type");
    ...
    /**
     ...
     the objects referenced.  Do not call deleteAll if <CODE>T</CODE> is not a
     pointer type (e.g. do call Array<Foo*>::deleteAll, do <B>not</B> call
     Array<Foo>::deleteAll).
     */
    ~Array()
    {
        ...
        m_memoryManager->free(data);
        // Set to 0 in case this Array is global and gets referenced during app
        // exit
        data         = NULL;
        num          = 0;
        numAllocated = 0;
    ...
    */
    void resize(size_t n, bool shrinkIfNecessary = true)
    {
        alwaysAssertM(
            n < 0xFFFFFFFF,
            "This implementation does not support arrays with more than 2^32 "
            "elements, although the size in memory may be larger.");
        if (num == n) {
            return;
        }
        ...
        // Once allocated, always maintain MIN_ELEMENTS elements or 32 bytes,
        // whichever is higher.
        const size_t minSize =
            G3D::max(MIN_ELEMENTS, (size_t)(MIN_BYTES / sizeof(T)));

        if ((MIN_ELEMENTS == 0) && (MIN_BYTES == 0) && (n == 0) &&
            shrinkIfNecessary) {
        ...
        if (num > numAllocated) {
            ...
            if (numAllocated == 0) {
                // First allocation; grow to exactly the size requested to avoid
                // wasting space.
                numAllocated = n;
                ...
                if (num < minSize) {
                    ...
                    // Increase the underlying size of the array.  Grow
                    // aggressively up to 64k, less aggressively up to 400k, and
                    // then grow relatively slowly (1.5x per resize) to avoid
        ...
        else if ((num <= numAllocated / 3) && shrinkIfNecessary &&
                 (num > minSize)) {
    ...
    /**
     Add an element to the end of the array.  Will not shrink the underlying
     array under any circumstances.  It is safe to append an element that is
    ...
    inline void append(const T& value)
    {
        ...
        if (num < numAllocated) {
            // This is a simple situation; just stick it in the next free slot
    ...
    inline void append(const T& v1, const T& v2)
    {
        if (inArray(&v1) || inArray(&v2)) {
            ...
            // Resize the array.  Note that neither value is already in the
            // array.
    ...
    /** "The member function removes the last element of the controlled
       sequence, which must be non-empty." For compatibility with std::vector.
     */
    inline void pop_back() { pop(); }

    /**
       "The member function returns the storage currently allocated to hold the
       controlled sequence, a value at least as large as size()" For
    ...
    */
    ...
    /**
       ...
       controlled sequence, which must be non-empty." For compatibility with
       std::vector.
    */
    ...
    */
    ...
    /**
     Removes the last element and returns it.  By default, shrinks the
     underlying array.
     */
    ...
    /** Pops the last element and discards it without returning anything. Faster
       than pop. By default, does not shrink the underlying array.*/
    ...
    inline T& operator[](int n)
    {
        debugAssertM((n >= 0) && (n < (int)num),
                     format("Array index out of bounds. n = %d, size() = %d",
                            (int)n,
                            (int)num));
        debugAssert(data != NULL);
        return data[n];
    ...
    */
    ...
    /** Returns element firstIndex(), performing a check in debug mode to ensure
    ...
    /** Returns iFloor(size() / 2), throws an assertion in debug mode if the
     * array is empty */
...
\endcode
    ...
    {
        // Using std::sort, which according to
        // http://www.open-std.org/JTC1/SC22/WG21/docs/D_4.cpp was 2x faster
        // than qsort for arrays around size 2000 on intel core2 with gcc
        std::sort(data, data + num, lessThan);
    }
     ...
     */
    void
    sortSubArray(int beginIndex, int endIndex, int direction = SORT_INCREASING)
    {
        if (direction == SORT_INCREASING) {
    ...
    void sortSubArray(int beginIndex,
                      int endIndex,
    ...
    /**
     The StrictWeakOrdering can be either a class that overloads the function
     call operator() or a function pointer of the form <code>bool (__cdecl
     *lessThan)(const T& elem1, const T& elem2)</code>
     */
    template <typename StrictWeakOrdering>
    void
    sortSubArray(int beginIndex, int endIndex, StrictWeakOrdering& lessThan)
    {
        std::sort(data + beginIndex, data + endIndex + 1, lessThan);
    }

    /** Uses < and == to evaluate operator(); this is the default comparator for
     * Array::partition. */
    ...
    /** The output arrays are resized with fastClear() so that if they are
       already of the same size as this array no memory is allocated during
       partitioning.

        @param comparator A function, or class instance with an overloaded
       operator() that compares two elements of type <code>T</code> and returns
       0 if they are equal, -1 if the second is smaller, and 1 if the first is
       smaller (i.e., following the conventions of std::string::compare).  For
       example:
                   ...
                   Array<T>& gtArray) const
    {
        ...
        partition(partitionElement,
                  ...
                  eqArray,
                  gtArray,
                  typename Array<T>::DefaultComparator());
    }

    /**
     Paritions the array into those below the median, those above the median,
     and those elements equal to the median in expected O(n) time using
     quickselect.  If the array has an even number of different elements, the
     median for partition purposes is the largest value less than the median.

     @param tempArray used for working scratch space
     @param comparator see parition() for a discussion.*/
    template <typename Comparator>
    void medianPartition(Array<T>&         ltMedian,
                         Array<T>&         eqMedian,
                         Array<T>&         gtMedian,
                         Array<T>&         tempArray,
                         const Comparator& comparator) const
    {
        ...
        // Maintain pointers to the arrays; we'll switch these around during
        // sorting to avoid copies.
        ...
        while (true) {
            // Choose a random element -- choose the middle element; this is
            // theoretically suboptimal, but for loosly sorted array is actually
            // the best strategy
            ...
            if ((L >= lowerHalfSize) && (U >= upperHalfSize)) {
                ...
                // x must be smaller than the median.  Recurse into the 'gt'
                // array.
                ...
                // first iteration)
                Array<T>* newGt =
                    (source == this) ? extra : const_cast<Array<T>*>(source);
                ...
                // x must be bigger than the median.  Recurse into the 'lt'
                // array.
        ...
        // Now that we know the median, make a copy of it (since we're about to
        // destroy the array that it points into).
        T median = *xPtr;
        xPtr     = NULL;
    ...
    /**
      Computes a median partition using the default comparator and a dynamically
      allocated temporary working array.  If the median is not in the array, it
      is chosen to be the largest value smaller than the true median.
     */
    void medianPartition(Array<T>& ltMedian,
                         Array<T>& eqMedian,
                         Array<T>& gtMedian) const
    {
        ...
        Array<T> temp;
        medianPartition(
            ltMedian, eqMedian, gtMedian, temp, DefaultComparator());
    }

    /** Redistributes the elements so that the new order is statistically
       independent of the original order. O(n) time.*/
    void randomize()
    {
        T temp;
    ...
    /** Ensures that future append() calls can grow up to size \a n without
     * allocating memory.*/
    void reserve(int n)
    {
        debugAssert(n >= size());
    ...
    /** Number of bytes used by the array object and the memory allocated for
     * it's data pointer. Does *not* include the memory of objects pointed to by
    ...
    /** Remove all NULL elements in linear time without affecting order of the
     * other elements. */
    void removeNulls()
    {
        int nextNull = 0;
