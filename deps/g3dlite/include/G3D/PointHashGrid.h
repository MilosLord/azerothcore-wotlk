...
/**
    ...
    Template argument \a PosFunc must provide a static <code>getPosition</code>
   method and \a EqualsFunc must provide a static <code>equals</code> method, as
   described below. You can either write classes that support these yourself,
   provide template specializations of G3D::PositionTrait and G3D::EqualsTrait,
   or rely on the default template specializations, which already exist for
   ...
   If the \a Value class defines <code>operator==</code>, then the \a Equalsfunc
   is optional, so you can just write:
...
public:
    /**
        ...
        beginBallIntersection and beginBoxIntersection. If two <i>Value</i>s are
       equal, their positions must be within this radius as well.  You can later
       change this value with clearAndSetRadiusHint().
    */
    PointHashGrid(float                     radiusHint,
    ...
    /**
       ...
       \sa clearAndSetRadiusHint()
    */
    PointHashGrid(const Array<Value>&       init,
        ...
        : m_size(0), m_memoryManager(m)
    {
        ...
        if (radiusHint <= 0) {
            ...
            float numPerCell = 5;
            radiusHint       = (float)pow(
                numPerCell * m_bounds.volume() / init.size(), 1.0 / 3.0);
    ...
    */
    ...
    private:
        ...
        /** End iterator. Note that the m_tableIterator is initialized to the
           end iterator of a temporary value!  This is ok because we'll never
           look at the value of the m_tableIterator, since we're initializing
           the "end" Iterator.*/
        Iterator()
            : m_isEnd(true), m_grid(NULL), m_tableIterator(CellTable().end()),
              m_arrayIndex(0), m_epoch(0)
        {
        }

        Iterator(const ThisType* grid)
            : m_isEnd(grid->size() == 0), m_grid(grid),
              m_tableIterator(grid->m_data.begin()), m_arrayIndex(0),
    ...
    /** Iterate through all members.  It is an error to mutate the HashGrid
        while iterating through it. Each member can be accessed by
    ...
    */
    ...
    // Forward declaration required by older gcc versions for friend declaration
    // in BoxIterator
    ...
    private:
        ...
        /** The iterator winds through the 3D grid between m_lo and (m_lo +
           m_extent) in Z,Y,X-major order.  This is the index keeping track of
           how far it has come */
        ...
        /** End iterator */
        BoxIterator()
            : m_isEnd(true), m_grid(NULL), m_exact(true), m_current(0, 0, 0),
              m_cell(NULL), m_arrayIndex(0), m_epoch(0)
        {
        }

        /** Begin iterator */
        BoxIterator(const ThisType* grid, bool exact, const AABox& box)
            : m_isEnd(false), m_grid(grid), m_exact(exact), m_box(box),
              m_current(-1, 0, 0), m_cell(NULL), m_arrayIndex(0),
              m_epoch(grid->m_epoch)
        {
    ...
    public:
        inline bool operator!=(const BoxIterator& other) const
        {
            if (other.m_isEnd && m_isEnd) {
                ...
                return (m_isEnd != other.m_isEnd) || (m_cell != other.m_cell) ||
                       (m_arrayIndex != other.m_arrayIndex);
            }
        }
    ...
    /**
        ...
        @param exact If false, the iterator will execute more quickly but will
       likely return some values that lie outside the box. Set exact = false if
       you are going to test the results against the yourself box anyway.
    */
    ...
    private:
        ...
        // before iterating through it, since there may be many boxes outside
        // the sphere.
    ...
    SphereIterator G3D_DEPRECATED
    beginSphereIntersection(const Sphere& sphere) const
    {
        return SphereIterator(this, sphere);
    }
    ...
    /** Appends results */
    void getIntersectingMembers(const Sphere& sphere,
                                Array<Value>& result) const
    {
        for (SphereIterator it = beginSphereIntersection(sphere); it.isValid();
             ++it) {
            result.append(*it);
        }
    }
    ...
    /**
       Dereference to access the bounds() and size() [element count] of the
       underlying cell object.

       Example:
       <pre>
       for(PointHashGrid<Vector3>::CellIterator iter = grid.beginCells(); iter
       != grid.endCells(); ++iter) { entriesFound += iter->size();
       }
       </pre>
    */
    class CellIterator {
        ...
        public:
            ...
            AABox bounds() const
            {
                ...
                return AABox(Vector3(k) * m_parent->m_grid->m_cellWidth,
                             Vector3(k + Vector3int32(1, 1, 1)) *
                                 m_parent->m_grid->m_cellWidth);
            }
    ...
    private:
        ...
        CellIterator(const ThisType* grid)
            : m_isEnd(false), m_grid(grid),
              m_tableIterator(grid->m_data.begin()), m_epoch(grid->m_epoch)
        {
    ...
    /** Returns true if there is a value that is exactly equal to @a v. This
       will check all neighboring cells to avoid roundoff error at cell
       boundaries.
    */
    ...
    /** Calls delete on all of the values, which are assumed to be pointers.
        ...
        structure, removing and deleting each one. Clears the PointHashGrid at
       the end.
