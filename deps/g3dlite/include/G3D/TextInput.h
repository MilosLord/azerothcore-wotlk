...
public:
    Token()
        : _string(""), _bool(false), _line(0), _character(0), _bytePosition(0),
          _type(END), _extendedType(END_TYPE)
    {
    }

    Token(
        Type t, ExtendedType e, const std::string& s, int L, int c, uint64 byte)
        : _string(s), _bool(false), _line(L), _character(c),
          _bytePosition(byte), _type(t), _extendedType(e)
    {
    }
    ...
    Token(Type               t,
          ...
          int                c,
          uint64             byte)
        : _string(s), _bool(b), _line(L), _character(c), _bytePosition(byte),
          _type(t), _extendedType(e)
    {
    }
    ...
    /** Number of bytes from the beginning of the buffer that this token was
      parsed from. Begins at 0 */
...
/**
 ...
 <ul>
  <li><CODE>Token::SINGLE_QUOTED_TYPE</CODE> string of characters surrounded by
 single quotes, e.g., 'x', '\\0', 'foo'.
  <li><CODE>Token::DOUBLE_QUOTED_TYPE</CODE> string of characters surrounded by
 double quotes, e.g., "x", "abc\txyz", "b o b".
  <li><CODE>Token::SYMBOL_TYPE</CODE> legal C++ operators, keywords, and
 identifiers.  e.g., >=, Foo, _X, class, { <li><CODE>Token::INTEGER_TYPE</CODE>
 numbers without decimal places or exponential notation. e.g., 10, 0x17F, 32, 0,
 -155 <li><CODE>Token::FLOATING_POINT_TYPE</CODE> numbers with decimal places or
 exponential notation. e.g., 1e3, -1.2, .4, 0.5
  <li><CODE>Token::BOOLEAN_TYPE</CODE> special symbols like "true" and "false";
 the exact details can be configured in TextInput::Settings
  <li><CODE>Token::LINE_COMMENT_TYPE</CODE> (disabled by default); generated for
 line comments as specified by TextInput::Settings
  <li><CODE>Token::BLOCK_COMMENT_TYPE</CODE> (disabled by default); generated
 for c-style block comments as specified by TextInput::Settings
  <li><CODE>Token::NEWLINE_TYPE</CODE> (disabled by default); generated for any
 of "\\r", "\\n" or "\\r\\n"
 </ul>
 ...
 Negative numbers are handled specially because of the ambiguity between unary
 minus and negative numbers-- see the note on TextInput::read.
...
public:
    ...
    public:
        ...
        /** The character to use as a single quote.  Defaults to "'"
           (backquote), occasionally useful to set to "`" (forward quote) or to
           "," (comma) for reading CSV files. */
        ...
        */
        ...
        /** All symbols that will become the 'true' boolean token.  See also
           caseSensitive. Clear this value to disable parsing of true booleans.
...
public:
    ...
    public:
        ...
        WrongTokenType(const std::string& src,
                       int                ln,
                       int                ch,
                       Token::Type        e,
                       Token::Type        a);
    ...
    public:
        ...
        WrongSymbol(const std::string& src,
                    int                ln,
                    int                ch,
                    const std::string& e,
                    const std::string& a);
    ...
    public:
        ...
        WrongString(const std::string& src,
                    int                ln,
                    int                ch,
                    const std::string& e,
                    const std::string& a);
    ...
    */
    ...
    /** Creates input directly from a fixed-length, non-NULL terminated string.
    ...
    */
    ...
    /** Temporarily switch parsing to use \a settings.  Note that this will
    override the currently recorded sourceFilename unless you explicitly set it
    back. \sa popSettings */
    ...
    /** Read the next token (which will be the END token if ! hasMore()).
        ...
        A '+' or '-' immediately before a number is prepended onto that number
       and if there is intervening whitespace, it is read as a separate symbol.

        If TextInput::Settings::signedNumbers is false,
        read() does not distinguish between a plus or minus symbol next
        to a number and a positive/negative number itself.  For example, "x - 1"
       and "x -1" will be parsed the same way by read().
    ...
    */
    ...
    /** Read from the beginning of the next token until the following newline
      and return the result as a string, ignoring all parsing in between. The
      newline is not returned in the string, and the following token read will
      be a newline or end of file token (if they are enabled for parsing).*/
    std::string readUntilNewlineAsString();

    /** Read from the beginning of the next token until the following delimiter
      character and return the result as a string, ignoring all parsing in
      between. The delimiter is not returned in the string, and the following
      token read will begin at the delimiter or end of file token (if they are
      enabled for parsing).*/
    std::string readUntilDelimiterAsString(const char delimiter1,
    ...
    */
    ...
    void readSymbols(const std::string& s1,
                     const std::string& s2,
                     const std::string& s3)
    {
        readSymbol(s1);
