...
/**
  A 4x4 matrix.  Do not subclass.  Data is initialized to 0 when default
  constructed.
...
public:
    ...
    */
    ...
    /**
        a is the upper left 3x3 submatrix and b is the upper right 3x1
       submatrix. The last row of the created matrix is (0,0,0,1).
    */
    Matrix4(const class Matrix3& upper3x3,
    ...
    {
        return Matrix4(e00, 0, 0, 0, 0, e11, 0, 0, 0, 0, e22, 0, 0, 0, 0, e33);
    }
    ...
    /** If this is a perspective projection matrix created by
        ...
        precision.
     */
    void getPerspectiveProjectionParameters(double& left,
                                            double& right,
                                            double& bottom,
                                            double& top,
                                            double& nearval,
    ...
    \param upDirection Use -1.0 for 2D Y increasing downwards (the G3D 8.x
    default convention), 1.0 for 2D Y increasing upwards (the G3D 7.x default
    and OpenGL convention)
     */
    static Matrix4 orthogonalProjection(float left,
                                        float right,
                                        float bottom,
                                        float top,
                                        float nearval,
    ...
    and OpenGL convention)
      */
    static Matrix4 orthogonalProjection(const class Rect2D& rect,
                                        float               nearval,
                                        float               farval,
                                        float upDirection = -1.0f);
    ...
    and OpenGL convention)
        ...
        projection involve divisions that can significantly impact
        precision.    */
    static Matrix4 perspectiveProjection(double left,
                                         double right,
                                         double bottom,
                                         double top,
                                         double nearval,
    ...
    */
    ...
    {
        return Matrix4(
            Matrix3::fromAxisAngle(Vector3::unitY(), toRadians(deg)));
    }

    inline static Matrix4 pitchDegrees(float deg)
    {
        return Matrix4(
            Matrix3::fromAxisAngle(Vector3::unitX(), toRadians(deg)));
    }

    inline static Matrix4 rollDegrees(float deg)
    {
        return Matrix4(
            Matrix3::fromAxisAngle(Vector3::unitZ(), toRadians(deg)));
    }
};
...
public:
    ...
    static Matrix4float64 perspectiveProjection(double left,
                                                double right,
                                                double bottom,
                                                double top,
                                                double nearval,
