...
<i>filename</i> for any single expression in an Any.  The filename is
interpreted relative to the file being parsed, and inside of the included file,
relative filenames are interpreted with respect to the included file (and so on,
recursively for nested inclusion).

Filenames are resolved with System::resolve and then System::findDataFile if not
found, so they may contain environment variables.
...
It is often convenient to iterate through the table portion using
G3D::AnyTableReader.
...
string      ::= <legal C double-quoted or unquoted string; backslashes must be
escaped> boolean     ::= "True" | "False" nil         ::= "Nil" <not case
sensitive> | "None" <case sensitive> array       ::= ("(" | "[" | "{") [value
(separator value)* [separator] ] (")" | "]" | "}") pair        ::= (identifier |
string) ("=" | ":") value table       ::= ("(" | "[" | "{") [ pair (separator
pair)* [separator] ] (")" | "]" | "}") named-array ::= identifier-exp array
named-table ::= identifier-exp table
include     ::= "#" "include" "(" string ")"

value       ::= [comment] (nil | number | boolean | string | array | table |
...
programming language, e.g., <code>"[ printf("hello world.");
clearScreen();]"</code> parses as an array containing two named arrays within
it. The deserializer also allows a trailing comma inside any array or table,
...
private:
    ...
    static void
    deserializeComment(TextInput& ti, Token& token, std::string& comment);
    ...
    public:
        ...
        */
        ...
        /** Two-character string of "{}", "[]", or "()"; to be used when
         * unparsing. */
        const char* bracket;
    ...
    private:
        /** Called by create() */
        inline Data(Type t, const char* b, char s)
            : type(t), referenceCount(1), bracket(b), separator(s)
        {
        }
...
public:
    ...
    explicit Any(int64 x)
        : m_type(NUMBER), m_simpleValue((double)x), m_data(NULL)
    {
    }
#endif
    ...
    /** \a t must be ARRAY or TABLE
        \param brackets must be "" (defaults to {} for table, () for array),
       "[]", "()", or "{}" \param separator must be ';', ',', or '\0' (defaults
       to ',' for array and ';' for table)
    */
    explicit Any(Type               t,
    ...
    /** Assigns from an array.  Assumes that T can be converted to Any.  Removes
     * the comment and name */
    template <class T>
    ...
    std::string
    unparse(const TextOutput::Settings& s = TextOutput::Settings()) const;

    /** \param allowCoercion If false, throws an error if the Any uses features
       that are not supported by JSON such as named arrays.  Otherwise, silently
       coerces to JSON. */
    std::string
    unparseJSON(const TextOutput::Settings& s = TextOutput::Settings(),
                bool                        allowCoercion = true) const;
    ...
    */
    ...
    /** Returns true if this key is in the TABLE.  Illegal to call on an object
     * that is not a TABLE. */
    ...
    /** The parent directory of the location from which this Any was loaded.
       This is useful for interpreting filenames relative to the Any's source
       location, which may not match the current directory if the Any was from
       an included file. */
    std::string sourceDirectory() const;

    /** True if the Anys are exactly equal, ignoring comments.  Applies deeply
    ...
    void
    serialize(TextOutput& to, bool json = false, bool coerce = false) const;
    ...
    */
    ...
    /** Verifies that the name is identifier \a n or \a m or \a p (case
       sensitive). It may contain identifier operators after this */
    void verifyName(const std::string& n,
                    const std::string& m,
                    const std::string& p) const;

    /** Verifies that the name is identifier \a n or \a m or \a p or \a q (case
       sensitive). It may contain identifier operators after this */
    void verifyName(const std::string& n,
                    const std::string& m,
                    const std::string& p,
    ...
    /** Verifies that the name <i>begins with</i> identifier \a n (case
       sensitive). It may contain identifier operators after this */
    void verifyNameBeginsWith(const std::string& n) const;

    /** Verifies that the name <i>begins with</i> identifier \a n or \a m (case
       sensitive). It may contain identifier operators after this */
    void verifyNameBeginsWith(const std::string& n, const std::string& m) const;

    /** Verifies that the name <i>begins with</i> identifier \a n or \a m or \a
       p (case sensitive). It may contain identifier operators after this */
    void verifyNameBeginsWith(const std::string& n,
                              const std::string& m,
                              const std::string& p) const;

    /** Verifies that the name <i>begins with</i> identifier \a n or \a m or \a
       p or \a q (case sensitive). It may contain identifier operators after
       this */
    void verifyNameBeginsWith(const std::string& n,
                              const std::string& m,
                              const std::string& p,
    ...
    /** Verifies that the name <i>begins with</i> identifier \a n or \a m or \a
       p or \a q or \a r (case sensitive). It may contain identifier operators
       after this */
    void verifyNameBeginsWith(const std::string& n,
                              const std::string& m,
                              const std::string& p,
    ...
    /** Verifies that the name <i>begins with</i> identifier \a n or \a m or \a
       p or \a q or \a r or \a s (case sensitive). It may contain identifier
       operators after this */
    void verifyNameBeginsWith(const std::string& n,
                              const std::string& m,
                              const std::string& p,
                              const std::string& q,
    ...
    /** Verifies that the name <i>begins with</i> identifier \a n or \a m or \a
       p or \a q or \a r or \a s or \a t(case sensitive). It may contain
    ...
    /** Assumes that Any(T) is well-defined, e.g., by T defining a
        T::toAny() method. */
    template <class T>
    explicit Any(const Array<T>& array, const std::string& name = "")
        : m_type(ARRAY), m_data(NULL)
    {
        setName(name);
        resize(array.size());
        for (int i = 0; i < array.size(); ++i) {
    ...
    {
        verifyType(TABLE);
        for (Table<std::string, Any>::Iterator it = this->table().begin();
             it.isValid();
             ++it) {
...
public:
    ...
    /** If key \a s appears in the any, reads its value into \a v and
        ...
        Assumes that if key \a s appears in the any it has not already been
       extracted by this iterator.  If it has been read before, an assertion
