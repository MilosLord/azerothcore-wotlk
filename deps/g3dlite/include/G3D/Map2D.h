...
/**
  Map of values across a discrete 2D plane.  Can be thought of as a generic
  class for 2D images, allowing flexibility as to pixel format and convenient
  methods. In fact, the "pixels" can be any values on a grid that can be
  sensibly interpolated--RGB colors, scalars, 4D vectors, and so on.

  Other "image" classes in G3D:

  G3D::GImage - Supports file formats, fast, Color3uint8 and Color4uint8
  formats.  No interpolation.

  G3D::shared_ptr<Texture> - Represents image on the graphics card (not directly
  readable on the CPU).  Supports 2D, 3D, and a variety of interpolation
  methods, loads file formats.

  G3D::Image3 - A subclass of Map2D<Color3> that supports image loading and
  saving and conversion to Texture.

  G3D::Image4 - A subclass of Map2D<Color4> that supports image loading and
  saving and conversion to Texture.

  G3D::Image3uint8 - A subclass of Map2D<Color3uint8> that supports image
  loading and saving and conversion to Texture.

  G3D::Image4uint8 -  A subclass of Map2D<Color4uint8> that supports image
  loading and saving and conversion to Texture.
  ...
  the type operated on by computation.  The Compute::Compute(Storage&)
  constructor is used to convert between storage and computation types.
...
template <typename Storage,
          typename Compute =
              typename G3D::_internal::_GetComputeType<Storage>::Type>
class Map2D : public ReferenceCountedObject {

    //
    // It doesn't make sense to automatically convert from Compute back to
    // Storage because the rounding rule (and scaling) is application dependent.
...
public:
    /** Unsafe access to the underlying data structure with no wrapping support;
     * requires that (x, y) is in bounds. */
...
protected:
    ...
    Compute bicubic(const Compute* ctrl, double s) const
    {
        ...
        static const double B[4][4] = {{0.0, -0.5, 1.0, -0.5},
                                       {1.0, 0.0, -2.5, 1.5},
                                       {0.0, 0.5, 2.0, -1.5},
                                       {0.0, 0.0, -0.5, 0.5}};
...
public:
    ...
    */
    ...
    /**
     Returns true if this map has been written to since the last call to
     setChanged(false). This is useful if you are caching a texture map other
     value that must be recomputed whenever this changes.
     */
    ...
    /** Returns a pointer to the underlying row-major data. There is no padding
       at the end of the row. Be careful--this will be reallocated during a
       resize.  You should call setChanged(true) if you mutate the array.*/
    Storage* getCArray() { return data.getCArray(); }

    const Storage* getCArray() const { return data.getCArray(); }

    /** Row-major array.  You should call setChanged(true) if you mutate the
     * array. */
    Array<Storage>& getArray() { return data; }

    const Array<Storage>& getArray() const { return data; }

    /** is (x, y) strictly within the image bounds, or will it trigger some kind
     * of wrap mode */
    ...
    /** Get the value at (x, y).
        ...
        (as, for example Color3(Color3uint8&) does), this will not match the
       value returned by Map2D::nearest.
      */
...
#ifndef G3D_WINDOWS
        // gcc gives a useless warning that the above code might reach the end
        // of the function; we use this line to supress the warning.
        return ZERO;
#endif
    }
    ...
    {
        return const_cast<Storage&>(
            const_cast<const Type*>(this)->get(x, y, wrap));
#ifndef G3D_WINDOWS
        // gcc gives a useless warning that the above code might reach the end
        // of the function; we use this line to supress the warning.
        return ZERO;
    ...
    /**
     Crops this map so that it only contains pixels between (x, y) and (x + w -
     1, y + h - 1) inclusive.
     */
    ...
    virtual void crop(const Rect2D& rect)
    {
        crop(iRound(rect.x0()),
             iRound(rect.y0()),
             iRound(rect.x1()) - iRound(rect.x0()),
             iRound(rect.y1()) - iRound(rect.y0()));
    }

    /** Returns the nearest neighbor.  Pixel values are considered
        to be at the upper left corner, so <code>image->nearest(x, y) ==
       image(x, y)</code>
      */
