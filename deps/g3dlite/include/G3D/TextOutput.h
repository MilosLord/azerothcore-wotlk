...
/**
  ...
  en-masse.  Use [de]serialize(bool, TextOutput) and other overloads to
  read/write primitive types in a standardized manner and
  ...
  the number of columns specified by Options::numColumns, <I>minus</I> the
  ...
  indents after the last newline of a file (if the indent level is non-zero at
  the end).

  <P><B>Serialization/Marshalling</B>
  <DT>Text serialization is accomplished using TextOutput by defining the pair
  of methods:
...
public:
    ...
    public:
        /**
          ...
          '); it will not occur after commas, punctuation, minus signs, or any
          other characters
        */
        ...
        Settings()
            : wordWrap(WRAP_WITHOUT_BREAKING),
              allowWordWrapInsideDoubleQuotes(false), numColumns(80),
              spacesPerIndent(4), convertNewlines(true), trueSymbol("true"),
              falseSymbol("false")
        {
#ifdef G3D_WINDOWS
...
private:
    ...
    /** Appends the character to data, indenting whenever a newline is
       encountered. Called from wordWrapIndentAppend */
    void indentAppend(char c);

public:
    explicit TextOutput(const std::string& filename,
                        const Settings&    options = Settings());

    /** Constructs a text output that can later be commited to a string instead
     * of a file.*/
    explicit TextOutput(const Settings& options = Settings());

    /** Returns one plus the number of newlines written since the output was
     * created. */
    ...
    /** Commit to the filename specified on the constructor.
     ...
     it yourself.
    @param flush If true (default) the file is ready for reading when the method
    returns, otherwise the method returns immediately and writes the file in the
    background.*/
    ...
    /** Writes a quoted string. Special characters in the string (e.g., \\, \\t,
       \\n) are escaped so that TextInput will produce the identical string on
       reading.*/
    ...
    /** If the most recently written character was a space, remove it and return
     * true. Can be called repeatedly to back up over multiple spaces. */
    bool deleteSpace();

    /** The symbol is written without quotes.  Symbols are required to begin
       with a letter or underscore and contain only letters, underscores, and
       numbers or be a C++ symbol (e.g. "{", "(", "++", etc.) so that they may
       be properly parsed by TextInput::readSymbol. Symbols are printed with a
       trailing space.*/
    ...
    /** Convenient idiom for writing multiple symbols in a row, e.g.
        ...
        */
    void writeSymbols(const std::string& a,
                      const std::string& b = "",
                      const std::string& c = "",
                      const std::string& d = "",
                      const std::string& e = "",
    ...
    void __cdecl vprintf(const char* fmt,
