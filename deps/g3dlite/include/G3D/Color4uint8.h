...
public:
    ...
    {
        return Color4uint8(G3D::max(r, x.r),
                           G3D::max(g, x.g),
                           G3D::max(b, x.b),
                           G3D::max(a, x.a));
    }
    ...
    {
        return Color4uint8(G3D::min(r, x.r),
                           G3D::min(g, x.g),
                           G3D::min(b, x.b),
                           G3D::min(a, x.a));
    }
    ...
    uint8& operator[](int i) const { return ((uint8*)this)[i]; }

    operator uint8*() { return (uint8*)this; }

    operator const uint8*() const { return (uint8*)this; }

    inline Color3uint8 bgr() const { return Color3uint8(b, g, r); }

    void serialize(class BinaryOutput& bo) const;

    void deserialize(class BinaryInput& bi);

    inline Color3uint8 rgb() const { return Color3uint8(r, g, b); }
