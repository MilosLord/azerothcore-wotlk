/**
  ...
  @cite JPEG compress/decompressor is the <A
  HREF="http://www.ijg.org/files/">IJG library</A>, used in accordance with
  their license.
  @cite JPG code by John Chisholm, using the IJG Library
  @cite TGA code by Morgan McGuire
  @cite BMP code by John Chisholm, based on code by Edward "CGameProgrammer"
  Resnick <A HREF="mailto:cgp@gdnmail.net">mailto:cgp@gdnmail.net</A> at <A
  HREF="ftp://ftp.flipcode.com/cotd/LoadPicture.txt">ftp://ftp.flipcode.com/cotd/LoadPicture.txt</A>
  @cite PCX format described in the ZSOFT PCX manual
  http://www.nist.fss.ru/hr/doc/spec/pcx.htm#2
  @cite PNG compress/decompressor is the <A
  HREF="http://www.libpng.org/pub/png/libpng.html">libpng library</A>, used in
  accordance with their license.
  @cite PPM code by Morgan McGuire based on
...
/**
  ...
  and PBM.  (Compressed) TGA 24, (Compressed) TGA 32, 8-bit paletted PCX, 24-bit
  PCX, and ICO are supported for decoding only.
  ...
  recommended over GImage (we don't include it directly in G3D because their
  license is more restrictive than the BSD one).

  \cite http://tfcduke.developpez.com/tutoriel/format/tga/fichiers/tga_specs.pdf

  \sa Image3, Image3uint8, Image4, Image4uint8, Image1, Image1uint8, Texture,
  Map2D
...
public:
    ...
    /** PGM, PPM, and PBM all come in two versions and are classified as PPM_*
     * files */
    enum Format {
        JPEG,
        BMP,
    ...
    inline const uint8* byte() const { return m_byte; }
    ...
    */
    ...
    inline const Color1uint8* pixel1() const
    {
        debugAssertM(
            m_channels == 1,
            format("Tried to call GImage::pixel1 on an image with %d channels",
                   m_channels));
    ...
    inline const Color4uint8* pixel4() const
    {
        debugAssertM(
            m_channels == 4,
            format("Tried to call GImage::pixel4 on an image with %d channels",
                   m_channels));
    ...
    inline const Color3uint8* pixel3() const
    {
        debugAssertM(
            m_channels == 3,
            format("Tried to call GImage::pixel3 on an image with %d channels",
                   m_channels));
    ...
    inline uint8* byte() { return m_byte; }
...
private:
    ...
    void encodePPM(BinaryOutput& out) const;

    void encodePPMASCII(BinaryOutput& out) const;
    ...
    void decodePPM(BinaryInput& input);

    void decodePPMASCII(BinaryInput& input);
...
public:
    ...
    */
    GImage(const unsigned char*      data,
           int                       length,
           Format                    format = AUTODETECT,
           const MemoryManager::Ref& m      = MemoryManager::create());
     ...
     */
    static bool pasteSubImage(GImage&       dest,
                              const GImage& src,
                              int           destX,
                              int           destY,
                              int           srcX,
    ...
    /**
     ...
     is not within src.
     */
    static bool copySubImage(GImage&       dest,
                             const GImage& src,
                             int           srcX,
                             int           srcY,
                             int           srcWidth,
    ...
    /** Ok for in == out */
    static void
    R8G8B8_to_Y8U8V8(int width, int height, const uint8* in, uint8* out);

    /** Ok for in == out */
    static void
    Y8U8V8_to_R8G8B8(int width, int height, const uint8* in, uint8* out);
    ...
    */
    static void RGBxRGBtoRGBA(const uint8* colorRGB,
    ...
    */
    static void
    flipRGBVertical(const uint8* in, uint8* out, int width, int height);

    static void
    flipRGBAVertical(const uint8* in, uint8* out, int width, int height);
    ...
    In the resulting image, x = red = tangent, y = green = binormal, and z =
    blue = normal.
    ...
    <PRE>
      ...
      */
    static void
    computeNormalMap(const class GImage&      bump,
                     class GImage&            normal,
                     const BumpMapPreprocess& preprocess = BumpMapPreprocess());

    static void
    computeNormalMap(int                      width,
                     int                      height,
                     int                      channels,
                     const uint8*             src,
                     GImage&                  normal,
    ...
    IMAGES Henrique S. Malvar, Li-wei He, and Ross Cutler
    ...
    */
    static void
    BAYER_G8B8_R8G8_to_R8G8B8_MHC(int w, int h, const uint8* in, uint8* _out);
    static void
    BAYER_G8R8_B8G8_to_R8G8B8_MHC(int w, int h, const uint8* in, uint8* _out);
    static void
    BAYER_R8G8_G8B8_to_R8G8B8_MHC(int w, int h, const uint8* in, uint8* _out);
    static void
    BAYER_B8G8_G8R8_to_R8G8B8_MHC(int w, int h, const uint8* in, uint8* _out);

    /** Fast conversion; the output has 1/2 the size of the input in each
    direction. Assumes in != out. See G3D::BAYER_G8B8_R8G8_to_R8G8B8_MHC for a
    much better result. */
    static void BAYER_G8B8_R8G8_to_Quarter_R8G8B8(int          inWidth,
                                                  int          inHeight,
                                                  const uint8* in,
                                                  uint8*       out);

    /** Attempt to undo fast conversion of
       G3D::BAYER_G8B8_R8G8_to_Quarter_R8G8B8; the green channel will lose data.
       Assumes in != out The input should have size 3 * inWidth * inHeight.  The
       output should have size 2 * inWidth * 2 * inHeight.
    */
    static void Quarter_R8G8B8_to_BAYER_G8B8_R8G8(int          inWidth,
                                                  int          inHeight,
                                                  const uint8* in,
                                                  uint8*       out);

    /** Overwrites every pixel with one of the two colors in a checkerboard
       pattern. The fields used from the two colors depend on the current number
       of channels in @a im.
        */
