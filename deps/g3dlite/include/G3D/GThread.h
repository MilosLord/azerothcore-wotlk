...
public:
    ...
    static GThreadRef
    create(const std::string& name, void (*proc)(void*), void* param = NULL);
    ...
    /** Starts the thread and executes threadMain().  Returns false if
       ...
       @param behavior If USE_CURRENT_THREAD, rather than spawning a new thread,
       this routine runs threadMain on the current thread.
       */
    ...
    /**
        ...
        public:
            ...
            void traceAll() {
               GThread::runConcurrently2D(Point2int32(0,0), Point2int32(w,h),
       this, &RayTracer::trace);
            }
        };
        \endcode
    */
    template <class Class>
    static void runConcurrently2D(const Vector2int32& start,
                                  ...
                                  int maxThreads = NUM_CORES)
    {
        _internal_runConcurrently2DHelper(
            ...
            object,
            method,
            static_cast<void (Class::*)(int, int, int)>(NULL),
            maxThreads);
    }
    ...
    */
    template <class Class>
    static void
    runConcurrently2D(const Vector2int32& start,
                      const Vector2int32& upTo,
                      Class*              object,
                      ...
                      int maxThreads = NUM_CORES)
    {
        _internal_runConcurrently2DHelper(
            ...
            upTo,
            object,
            static_cast<void (Class::*)(int, int)>(NULL),
            method,
            maxThreads);
    }
};

// Can't use an inherited class inside of its parent on g++ 4.2.1 if it is later
// a template parameter

/** For use by runConcurrently2D. Designed for arbitrary iteration, although
   only used for interlaced rows in the current implementation. */
template <class Class>
...
public:
    ...
    _internalGThreadWorker(int                 threadID,
                           ...
                           void (Class::*method2)(int x, int y, int threadID),
                           const Vector2int32& stride)
        : GThread("runConcurrently2D worker"), threadID(threadID), start(start),
          upTo(upTo), stride(stride), object(object), method1(method1),
          method2(method2)
    {
    }
...
template <class Class>
void _internal_runConcurrently2DHelper(const Vector2int32& start,
                                       const Vector2int32& upTo,
                                       Class*              object,
                                       void (Class::*method1)(int x, int y),
                                       void (Class::*method2)(int x,
                                                              int y,
                                                              int threadID),
                                       int maxThreads)
{
    ...
    for (int t = 0; t < numThreads; ++t) {
        threadSet.insert(shared_ptr<_internalGThreadWorker<Class>>(
            new _internalGThreadWorker<Class>(t,
                                              start + Vector2int32(0, t),
                                              upTo,
                                              object,
                                              method1,
                                              method2,
