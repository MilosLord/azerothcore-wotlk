/**
  ...
  \cite Benjamin Jurke
  http://www.flipcode.com/cgi-bin/msg.cgi?showThread=COTD-ProcessorDetectionClass&forum=cotd&id=-1
...
private:
    ...
    */
    static void cpuid(CPUIDFunction func,
                      uint32&       areg,
                      uint32&       breg,
                      uint32&       creg,
                      uint32&       dreg);
...
public:
    ...
    */
    ...
    /**
       Returns a string describing the current usage of the buffer pools used
       for optimizing System::malloc.
     */
    ...
    */
    ...
    /** An implementation of memcpy that may be up to 2x as fast as the C
       library one on some processors.  Guaranteed to have the same behavior as
       memcpy in all cases. */
    static void memcpy(void* dst, const void* src, size_t numBytes);

    /** An implementation of memset that may be up to 2x as fast as the C
       library one on some processors.  Guaranteed to have the same behavior as
       memset in all cases. */
    ...
    /**
       @brief The optimization status of the G3D library (not the program
       compiled against it)
    ...
    */
    ...
    /** Get an environment variable for the current process.  Returns NULL if
    ...
    /**
     ...
     Looks in:
         ...
         - Literal interpretation of full (i.e., if it contains a
     fully-qualified name)
         - Last directory in which a file was found
         - Current directory
         - System::appDataDir (which is usually GApp::Settings.dataDir, which
     defaults to the directory containing the program binary)
         - $G3D9DATA directory
         - System::appDataDir() + "data/"  (note that this may be a zipfile
     named "data" with no extension)
       ...
       Plus the following subdirectories of those:
        ...
        \param exceptionIfNotFound If true and the file is not found, throws
     G3D::FileNotFound.
     */
    static std::string findDataFile(const std::string& full,
...
inline void System::endCycleCount(uint64& cycleCount)
{
    ...
    cycleCount = (uint64)((double)(instance().m_OSXCPUSpeed) *
