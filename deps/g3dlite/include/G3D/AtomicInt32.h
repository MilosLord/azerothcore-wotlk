...
public:
    ...
    /** Returns zero if the result is zero after decrement, non-zero
     * otherwise.*/
    int32 decrement()
    {
#if defined(G3D_WINDOWS)
    ...
    /** Atomic test-and-set:  if <code>*this == comperand</code> then
       <code>*this := exchange</code> else do nothing. In both cases, returns
       the old value of <code>*this</code>.

        Performs an atomic comparison of this with the Comperand value.
        If this is equal to the Comperand value, the Exchange value is stored in
       this. Otherwise, no operation is performed.
    ...
    int32 compareAndSet(const int32 comperand, const int32 exchange)
    {
        ...
        // Note that OSAtomicCompareAndSwap32 does not return a useful value for
        // us so it can't satisfy the cmpxchgl contract.
#endif
#endif
    }
};
