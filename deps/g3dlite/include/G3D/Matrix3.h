...
public:
    ...
    */
    ...
    static Matrix3
    fromColumns(const Vector3& c0, const Vector3& c1, const Vector3& c2)
    {
        Matrix3 m;
        for (int r = 0; r < 3; ++r) {
            m.elt[r][0] = c0[r];
    ...
    static Matrix3
    fromRows(const Vector3& r0, const Vector3& r1, const Vector3& r2)
    {
        Matrix3 m;
        for (int c = 0; c < 3; ++c) {
            m.elt[0][c] = r0[c];
    ...
    {
        return Matrix3(e00, 0, 0, 0, e11, 0, 0, 0, e22);
    }
...
public:
    /** Optimized implementation of out = A.transpose().  It is safe (but slow)
       to call with A and out possibly pointer equal to one another.
    ...
    */
    ...
    /** Returns true if the rows and column L2 norms are 1.0 and the rows are
     * orthogonal. */
    ...
    */
     ...
     */
    bool
    toEulerAnglesXYZ(float& rfYAngle, float& rfPAngle, float& rfRAngle) const;
    bool
    toEulerAnglesXZY(float& rfYAngle, float& rfPAngle, float& rfRAngle) const;
    bool
    toEulerAnglesYXZ(float& rfYAngle, float& rfPAngle, float& rfRAngle) const;
    bool
    toEulerAnglesYZX(float& rfYAngle, float& rfPAngle, float& rfRAngle) const;
    bool
    toEulerAnglesZXY(float& rfYAngle, float& rfPAngle, float& rfRAngle) const;
    bool
    toEulerAnglesZYX(float& rfYAngle, float& rfPAngle, float& rfRAngle) const;
    static Matrix3
    fromEulerAnglesXYZ(float fYAngle, float fPAngle, float fRAngle);
    static Matrix3
    fromEulerAnglesXZY(float fYAngle, float fPAngle, float fRAngle);
    static Matrix3
    fromEulerAnglesYXZ(float fYAngle, float fPAngle, float fRAngle);
    static Matrix3
    fromEulerAnglesYZX(float fYAngle, float fPAngle, float fRAngle);
    static Matrix3
    fromEulerAnglesZXY(float fYAngle, float fPAngle, float fRAngle);
    static Matrix3
    fromEulerAnglesZYX(float fYAngle, float fPAngle, float fRAngle);
    ...
    static void
    tensorProduct(const Vector3& rkU, const Vector3& rkV, Matrix3& rkProduct);
    ...
    // according to Chapter 10 of "Thinking in C++, 2nd ed. Volume 1" by Bruce
    // Eckel, http://www.mindview.net/
...
inline Vector3 operator*(const Vector3& rkPoint, const Matrix3& rkMatrix)
{
    ...
    for (int r = 0; r < 3; ++r) {
        kProd[r] = rkPoint[0] * rkMatrix.elt[0][r] +
