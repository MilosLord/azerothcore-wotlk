...
public:
    ...
    /** The OpenGL format equivalent to this one, e.g, GL_RGB8  Zero if there is
     * no equivalent. This is actually a GLenum */
    int openGLFormat;
    ...
    /** Number of bits per pixel storage for alpha values; Zero for compressed
     * textures and non-RGB. */
    int alphaBits;

    /** Number of bits per pixel storage for red values; Zero for compressed
     * textures and non-RGB. */
    int redBits;

    /** Number of bits per pixel storage for green values; Zero for compressed
     * textures and non-RGB. */
    int greenBits;

    /** Number of bits per pixel storage for blue values; Zero for compressed
     * textures  and non-RGB. */
    int blueBits;
    ...
    /** Amount of CPU memory per pixel when packed into an array, discounting
     * any end-of-row padding. */
    int cpuBitsPerPixel;

    /**
      Amount of GPU memory per pixel on most graphics cards, for formats
     supported by OpenGL. This is only an estimate--the actual amount of memory
     may be different on your actual card.
    ...
    /** The OpenGL bytes (type) format of the data buffer used with this texture
     * format, e.g., GL_UNSIGNED_BYTE */
    ...
    /** Indicates whether this format treats numbers as integers, floating
     * point, or normalized fixed point */
    ...
    /** Returns ImageFormat representing the same channels as \a
        ...
        an alpha channel. */
    static const ImageFormat*
    getFormatWithAlpha(const ImageFormat* otherFormat);
...
public:
    ...
    /** Returns the matching ImageFormat* identified by the Code.  May return
      NULL if this format's code is reserved but not yet implemented by G3D. */
    ...
    private:
        static const char* toString(int i, Value& v)
        {
            static const char* str[] = {
                "NEAREST", "BILINEAR", "MHC", "BEST", NULL};
    ...
    /** Converts between arbitrary formats on the CPU.  Not all format
       conversions are supported or directly supported. Formats without direct
       conversions will attempt to convert through RGBA first.

        A conversion routine might only support source or destination padding or
       y inversion or none. If support is needed and not available in any of the
       direct conversion routines, then no conversion is done.

        YUV422 expects data in YUY2 format (Y, U, Y2, v).  Most YUV formats
       require width and heights that are multiples of 2.
       ...
       \sa G3D::ImageConvert
    */
    static bool convert(const Array<const void*>& srcBytes,
                        int                       srcWidth,
                        int                       srcHeight,
    ...
    /**
       ...
       \sa G3D::ImageConvert
     */
    static bool conversionAvailable(const ImageFormat* srcFormat,
                                    int                srcRowPadBits,
                                    const ImageFormat* dstFormat,
                                    int                dstRowPadBits,
                                    bool               invertY = false);
    ...
    /** Returns a Color4 that masks off unused components in the format, given
       in RGBA For example, the mask for R32F is (1,0,0,0), for A32F is
       (0,0,0,1), for RGB32F is (1,1,1,0). (Note that luminance is interpreted
       as using only the R channel, even though RGB would make more sense to
       me...)
      */
