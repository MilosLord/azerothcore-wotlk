...
/**
 ...
 <ul>
   ...
   <li> "zipfile" = a compressed file storing an archive of files and
 directories in the zip format
 </ul>

 In G3D, Zipfiles are transparently treated as if they were directories,
 provided: <ul> <li> The zipfile name contains an extension (e.g., map.pk3,
 files.zip) <li> There are no nested zipfiles
 </ul>

 All FileSystem routines invoke FilePath::expandEnvironmentVariables if the
 input contains a '$'.

 The extension requirement allows G3D to quickly identify whether a path could
 enter a zipfile without forcing it to open all parent directories for reading.
...
private:
    /** Drive letters.  Only used on windows, but defined on all platforms to
       help avoid breaking the Windows build when compiling on another platform.
     */
    ...
    class Dir {
    public:
        /** If false, this path did not exist (even inside a zipfile) when last
         * checked, or it is not a directory. */
        bool exists;
        ...
        void computeZipListing(const std::string& zipfile,
                               const std::string& pathInsideZipfile);

        Dir()
            : exists(false), isZipfile(false), inZipfile(false), lastChecked(0)
        {
        }
    };

    /** Maps path names (without trailing slashes, except for the file system
       root) to contents. On Windows, all paths are lowercase */
    ...
    /** \copydoc exists */
    bool _exists(const std::string& f,
    ...
    /** Called from list() */
    void listHelper(const std::string&  shortSpec,
                    const std::string&  parentPath,
                    Array<std::string>& result,
                    const ListSettings& settings);

    /** \copydoc list */
    void _list(const std::string&  spec,
               Array<std::string>& result,
    ...
    /** \copydoc getFiles */
    void _getFiles(const std::string&  spec,
                   Array<std::string>& result,
    ...
    void _getDirectories(const std::string&  spec,
                         Array<std::string>& result,
                         bool                includeParentPath = false)
    {
        ListSettings set;
...
public:
    ...
    /** Clears old cache entries so that exists() and list() will reflect recent
       changes to the file system. \param path Clear only \a path and its
       subdirectories ("" means clear the entire cache) */
    ...
    /** Returns true if \a path is a file that is a zipfile. Note that G3D
       requires zipfiles to have some extension, although it is not required to
       be "zip" */
    ...
    /** The current working directory (cwd).  Only ends in a slash if this is
    ...
    */
    ...
    /** Returns true if a node named \a f exists.
        ...
        \param f If \a f contains wildcards, the function returns true if any
       file matches those wildcards.  Wildcards may only appear in the base or
       ext, not the path.  Environment variables beginning with dollar signs
       (e.g., in "$G3DDATA/cubemap"), with optional parens ("$(G3DDATA)") are
        automatically expanded in \a f. Default share names on Windows (e.g.,
       "\\mycomputer\c$") are correctly distinguished from empty environment
       variables.

        \param trustCache If true, uses the cache for optimizing repeated calls
        in the same parent directory.

        \param caseSensitive If true, the match must have exactly the same case
       for the base and extension.  If false, case is ignored.  The default on
       Windows is false and the default on other operating systems is true.
     */
    ...
    /** Fully qualifies a filename.
        ...
        The filename may contain wildcards, in which case the wildcards will be
       preserved in the returned value.

        \param cwd The directory to treat as the "current" directory when
       resolving a relative path.  The default value is the actual current
       directory.  (G3D::Any::sourceDirectory is a common alternative)
    */
    ...
    */
    ...
    /** Returns the length of the file in bytes, or -1 if the file could not be
     * opened. */
     ...
     */
    static void list(const std::string&  spec,
                     Array<std::string>& result,
                     const ListSettings& listSettings = ListSettings())
    {
        mutex.lock();
    ...
    /** list() files */
    static void getFiles(const std::string&  spec,
                         Array<std::string>& result,
                         bool                includeParentPath = false)
    {
        mutex.lock();
    ...
    /** list() directories */
    static void getDirectories(const std::string&  spec,
                               Array<std::string>& result,
                               bool                includeParentPath = false)
    {
        mutex.lock();
    ...
    /** Adds \a filename to usedFiles().  This is called automatically by open()
      and all G3D routines that open files. */
    static void markFileUsed(const std::string& filename);

    /** All files that have been marked by markFileUsed().  GApp automatically
       prints this list to log.txt.  It is useful for finding the dependencies
       of your program automatically.*/
...
public:
    ...
    /** Returns true if \a f specifies a path that parses as root of the
       filesystem. On OS X and other Unix-based operating systems, "/" is the
       only root. On Windows, drive letters and shares are roots, e.g., "c:\",
       "\\foo\". Does not check on Windows to see if the root is actually
       mounted or a legal drive letter--this is a purely string based test. */
    ...
    /** Returns everything to the right of the last slash (or, on Windows, the
     * last ':') */
    ...
    /** \brief Replaces <code>$VAR</code> and <code>$(VAR)</code> patterns with
       the corresponding environment variable. Throws std::string if the
    ...
    /**
      Returns true if \a path matches \a pattern, with standard filesystem
      wildcards.
     */
    ...
    /** Replaces characters that are illegal in a filename with legal
     * equivalents.*/
    static std::string makeLegalFilename(const std::string& f,
