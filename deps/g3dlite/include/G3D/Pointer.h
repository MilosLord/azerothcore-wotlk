...
/**
   ...
   Because the accessors require values to be passed by value (instead of by
   reference) this is primarily useful for objects whose memory size is small.
   ...
   *p2 = *p1; \/\/ Value assignment
   ...
   <i>Note:</i> Because of the way that dereference is implemented, you cannot
   pass <code>*p</code> through a function that takes varargs (...), e.g.,
   <code>printf("%d", *p)</code> will produce a compile-time error.  Instead use
   <code>printf("%d",(bool)*p)</code> or <code>printf("%d",
   p.getValue())</code>.

   \cite McGuire, GUIs for Real-time Programs, using Universal Pointers,
...
private:
    ...
    public:
        Accessor(T* object, GetMethod getMethod, SetMethod setMethod)
            : object(object), getMethod(getMethod), setMethod(setMethod)
        {
    ...
    public:
        SharedAccessor(const shared_ptr<T>& object,
                       GetMethod            getMethod,
                       SetMethod            setMethod)
            : object(object), getMethod(getMethod), setMethod(setMethod)
        {
...
public:
    ...
    Pointer(const shared_ptr<Class>& object,
        ...
        : m_interface(new SharedAccessor<Class,
    ...
    Pointer(ValueType (*getMethod)(), void (*setMethod)(ValueType))
        : m_interface(new FcnAccessor<ValueType (*)(), void (*)(ValueType)>(
              getMethod, setMethod))
    {
    }
    ...
    Pointer(const ValueType& (*getMethod)(), void (*setMethod)(ValueType))
        : m_interface(
              new FcnAccessor<const ValueType& (*)(), void (*)(ValueType)>(
                  getMethod, setMethod))
    {
    }
    ...
    Pointer(Class* object,
            const ValueType& (Class::*getMethod)() const,
            void (Class::*setMethod)(const ValueType&))
        : m_interface(new Accessor<Class,
                                   const ValueType& (Class::*)() const,
                                   void (Class::*)(const ValueType&)>(
              object, getMethod, setMethod))
    {
    ...
    /** \brief Assign a value to the referenced element.
        If this Pointer was initialized with a NULL setMethod, the call is
       ignored */
