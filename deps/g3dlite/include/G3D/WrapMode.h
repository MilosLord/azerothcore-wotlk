...
/**
  ...
  Refer to these as scoped enums, e.g., <code>WrapMode m =
  WrapMode::CLAMP;</code>.
  ...
  WrapMode::CLAMP makes out of bounds pixels equal to the last in-range pixel
  along that dimension.

  WrapMode::TILE computes out of bounds pixels modulo the dimension

  WrapMode::ZERO treats out of bounds values as the zero value, which varies in
  definition according to the class used.  For example, with a G3D::Texture,
  ZERO = Color4(0,0,0,0).
...
public:
    ...
    enum Value { CLAMP, TILE, ZERO, IGNORE, ERROR } value;

    static const char* toString(int i, Value& v)
    {
        static const char* str[] = {
            "CLAMP", "TILE", "ZERO", "IGNORE", "ERROR", NULL};
