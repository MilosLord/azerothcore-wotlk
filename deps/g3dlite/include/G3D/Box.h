...
/**
 ...
 <CODE>Box box = cframe.toObjectSpace(Box(center - extent/2, center +
 extent/2));</CODE>
 */
class Box {
private:
...
public:
    ...
    /**
     ...
     this box, where the origin is the center of the box.
     localFrame().worldToObject(...) takes objects into the space where the box
     axes are (1,0,0), (0,1,0), (0,0,1).  Note that there is no scaling in this
     transformation.
     */
  ...
  z/
    ...
    {
        return Vector3(_edgeVector[0].length(),
                       _edgeVector[1].length(),
                       _edgeVector[2].length());
    }
...
z/
     ...
     */
    void getFaceCorners(
        int f, Vector3& v0, Vector3& v1, Vector3& v2, Vector3& v3) const;
    ...
    void getRandomSurfacePoint(Vector3& P,
