...
#ifndef _MSC_VER
/// Fast call is a register-based optimized calling convention supported only by
/// Visual C++
#define __fastcall
#endif
...
/** Define the g++ thread-local syntax on all platforms (since the MSVC version
 * would be hard to emulate with a macro) */
#if defined(_MSC_VER)
...
/** \def G3D_START_AT_MAIN()
    Makes Windows programs using the WINDOWS subsystem invoke main() at program
   start by defining a WinMain(). Does nothing on other operating systems.*/
#define G3D_START_AT_MAIN()                                                    \
    int WINAPI G3D_WinMain(                                                    \
        HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int sw);            \
    int WINAPI WinMain(                                                        \
        HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int sw)             \
    {                                                                          \
...
/** @def G3D_START_AT_MAIN()
    Defines necessary wrapper around WinMain on Windows to allow transfer of
   execution to main(). */
#define G3D_START_AT_MAIN()
...
/**
  ...
  <CODE>STR(this becomes a string)\verbatim<PRE>\endverbatim evaluates the same
  as \verbatim<CODE>\endverbatim"this becomes a string"</CODE>
 */
#define STR(x) #x
...
#include <ciso646> // Defines _LIBCC_VERSION if linking against libc++ or does nothing
    ...
    (!defined(_LIBCPP_VERSION) && defined(__linux__))
#include <tr1/memory>
#else
#include <memory>
#endif
...
public:
    /**
      ...
      If true, G3D will spawn a thread for network management on the first
      invocation of G3D::NetServer::create or
      G3D::NetConnection::connectToServer.

      If false and networking is used, the application must explicitly invoke
      G3D::serviceNetwork() regularly to allow the network code to run.
