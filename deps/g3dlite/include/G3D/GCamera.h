...
/**
  ...
  See
  http://bittermanandy.wordpress.com/2009/04/10/a-view-to-a-thrill-part-one-camera-concepts/
...
public:
    /** Must be of the format produced by the Any cast, e.g.,
        ...
        <pre>
        GCamera {
            coordinateFrame = CFrame::fromXYZYPRDegrees(-13.3f, 8.0f, -1.9f,
       246.6f, -3), nearPlaneZ = -0.5, farPlaneZ = -50, fovDirection =
       "HORIZONTAL", fovAngleDegrees = 90
        }</pre>
    ...
    */
    ...
    const CoordinateFrame& coordinateFrame() const { return m_cframe; }
    ...
    void setPixelOffset(const Vector2& p) { m_pixelOffset = p; }

    const Vector2& pixelOffset() const { return m_pixelOffset; }
    ...
    /** Sets \a P equal to the camera's projection matrix. This is the
        ...
        This is the matrix that a RenderDevice (or OpenGL) uses as the
       projection matrix.
        @sa RenderDevice::setProjectionAndCameraMatrix,
    ...
    */
    ...
    /** Converts projected points from OpenGL standards
        ...
        \deprecated
    */ // TODO: Remove
    Vector3 convertFromUnitToNormal(const Vector3& in,
    ...
    */
    ...
    /**
     ...
     at the near plane and 1 at the far plane and varies hyperbolically in
     between.
    ...
    /**
      ...
      transforms the point into a pixel location for the viewport, then calls
      unproject
    */
     ...
     */
    float worldToScreenSpaceArea(float               area,
                                 float               z,
                                 const class Rect2D& viewport) const;
     ...
     */
    inline float nearPlaneZ() const { return m_nearPlaneZ; }

    /**
     Returns a negative z-value.
     */
    inline float farPlaneZ() const { return m_farPlaneZ; }
    ...
    /**
       ...
       The viewport is used only to determine the aspect ratio of the screen;
       the absolute dimensions and xy values don't matter.
    */
    void getClipPlanes(const Rect2D& viewport, Array<Plane>& outClip) const;

    /**
      Returns the world space view frustum, which is a truncated pyramid
      describing the volume of space seen by this camera.
    */
