...
public:
    ...
    /** Amount of time between the most recent tick and tock calls.  0 if tick
    ...
    /** Time-smoothed value of fps that is stable to the nearest integer for fps
      > 10 and to the first decimal place for fps <= 10. This is useful if you
      are displaying the frame rate in real-time and want a stable (readable)
      number.*/
    double smoothFPS() const { return m_smoothFPS; }

    /** The elapsed cycle time between tick and tock.  An attempt is made to
       factor out all tick/tock overhead, so that back-to-back calls should
       return zero. Unreliable on non-x86 platforms.*/
    ...
    /** Call after an operation has completed, with the name of the operation,
       to print a debug message listing the time since the previous after()
       call.
