...
public:
    ...
    */
    ...
    inline bool halfSpaceContains(Point3 point) const
    {
        ...
        // We can get away with putting values *at* the limits of the float32
        // range into a dot product, since the dot product is carried out on
        // float64.
    ...
    /**
     Returns true if point is on the side the normal points to or
     is in the plane.  Only call on finite points.  Faster than
     halfSpaceContains.
     */
    ...
    /**
      Returns distance from point to plane. Distance is negative if point is
      behind (not in plane in direction opposite normal) the plane.
    */
