...
private:
     ...
     */
    template <class T>
    static bool clipSide2D(const float     p,
                           bool            clipGreater,
                           int             axis,
                           ...
                           Array<T>&       outPoly)
    {
        ...
        //     if the point is outside the side and the previous one was also
        //     outside, continue if the point is outside the side and the
        //     previous one was inside, cut the line if the point is inside the
        //     side and the previous one was also inside, append the points if
        //     the point is inside the side and the previous one was outside,
        //     cut the line
        for (int i = 1; i <= inPoly.length(); ++i) {
            ...
            if (c1 ^ c2) {
                ...
                if (!c1 && c2 && (i > 1)) {
                    // Unclipped to clipped trasition and not the first
                    // iteration
                    outPoly.append(pt1);
                }

                // only one point is clipped, find where the line crosses the
                // clipping plane
...
public:
    ...
    /** Returns true if this is the empty set, which is distinct from a
     * zero-area rectangle. */
    ...
    bool contains(const Point2& v) const
    {
        ...
        return (v.x >= min.x) && (v.y >= min.y) && (v.x <= max.x) &&
               (v.y <= max.y);
    }
    ...
    {
        // This will automatically return false if isEmpty()
        return (min.x <= r.min.x) && (min.y <= r.min.y) && (max.x >= r.max.x) &&
               (max.y >= r.max.y);
    }

    /** True if there is non-zero area to the intersection between @a this and
       @a r. Note that two rectangles that are adjacent do not intersect because
       there is zero area to the overlap, even though one of them "contains" the
       corners of the other.*/
    bool intersects(const Rect2D& r) const
    {
        // This will automatically return false if isEmpty()
        return (min.x < r.max.x) && (min.y < r.max.y) && (max.x > r.min.x) &&
               (max.y > r.min.y);
    }
    ...
    /** Returns the corners in the order: (min,min), (max,min), (max,max),
     * (min,max). */
    ...
    /**
     Clips so that the rightmost point of the outPoly is at rect.x1 (e.g. a
     800x600 window produces rightmost point 799, not 800).  The results are
     suitable for pixel rendering if iRounded. Templated so that it will work
     for Vector2,3,4 (the z and w components are interpolated linearly). The
     template parameter must define T.lerp and contain x and y components.

     If the entire polygon is clipped by a single side, the result will be
     empty. The result might also have zero area but not be empty.
     */
    template <class T>
