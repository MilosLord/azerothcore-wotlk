...
CoordinateFrame::rotation is the upper 3x3 submatrix,
CoordinateFrame::translation is the right 3x1 column.  The 4th row is always [0
0 0 1], so it isn't stored. So you don't have to remember which way the
multiplication and transformation work, it provides explicit toWorldSpace and
toObjectSpace methods.  Also, points, vectors (directions), and surface normals
transform differently, so they have separate methods.

Some helper functions transform whole primitives like boxes in and out of object
space.

Convert to Matrix4 using CoordinateFrame::toMatrix4.  You <I>can</I> construct a
CoordinateFrame from a Matrix4 using Matrix4::approxCoordinateFrame, however,
because a Matrix4 is more general than a CoordinateFrame, some information may
be lost.
...
public:
    ...
    /** The origin of this coordinate frame in world space (or its parent's
     * space, if nested). */
    Point3 translation;
    ...
    static CoordinateFrame fromXYZYPRRadians(float x,
                                             float y,
                                             float z,
                                             float yaw   = 0.0f,
                                             float pitch = 0.0f,
    ...
    static CoordinateFrame fromXYZYPRDegrees(float x,
                                             float y,
                                             float z,
                                             float yaw   = 0.0f,
                                             float pitch = 0.0f,
    ...
    void getXYZYPRRadians(float& x,
                          float& y,
                          float& z,
                          float& yaw,
                          float& pitch,
                          float& roll) const;
    void getXYZYPRDegrees(float& x,
                          float& y,
                          float& z,
                          float& yaw,
                          float& pitch,
    ...
    {
        return Vector4(rotation * Vector3(v.x, v.y, v.z) + translation * v.w,
                       v.w);
    }
    ...
    {
        return Point3(rotation[0][0] * v[0] + rotation[0][1] * v[1] +
                          rotation[0][2] * v[2] + translation[0],
                      rotation[1][0] * v[0] + rotation[1][1] * v[1] +
                          rotation[1][2] * v[2] + translation[1],
                      rotation[2][0] * v[0] + rotation[2][1] * v[1] +
                          rotation[2][2] * v[2] + translation[2]);
    }

    /**
     Transforms the point into object space.  Assumes that the rotation matrix
     is orthonormal.
     */
    inline Point3 pointToObjectSpace(const Point3& v) const
    {
        ...
        return Point3(rotation[0][0] * p[0] + rotation[1][0] * p[1] +
                          rotation[2][0] * p[2],
                      rotation[0][1] * p[0] + rotation[1][1] * p[1] +
                          rotation[2][1] * p[2],
                      rotation[0][2] * p[0] + rotation[1][2] * p[1] +
                          rotation[2][2] * p[2]);
    }
    ...
    void normalToWorldSpace(const Array<Vector3>& v,
                            Array<Vector3>&       vout) const;

    void vectorToWorldSpace(const Array<Vector3>& v,
                            Array<Vector3>&       vout) const;

    void pointToObjectSpace(const Array<Point3>& v, Array<Point3>& vout) const;

    void normalToObjectSpace(const Array<Vector3>& v,
                             Array<Vector3>&       vout) const;

    void vectorToObjectSpace(const Array<Vector3>& v,
                             Array<Vector3>&       vout) const;
    ...
    /**
    Transform this coordinate frame towards \a goal, but not past it, goverened
    by maximum rotation and translations.  This is a useful alternative to \a
    lerp, especially if the goal is expected to change every transformation step
    so that constant start and end positions will not be available.
    ...
    \sa lerp
    */
    void moveTowards(const CoordinateFrame& goal,
    ...
    /** Returns the ray starting at the camera origin travelling in direction
     * CoordinateFrame::lookVector. */
