...
public:
    ...
    Color3uint8 bgr() const { return Color3uint8(b, g, r); }

    Color3uint8 max(const Color3uint8 x) const
    {
        return Color3uint8(
            G3D::max(r, x.r), G3D::max(g, x.g), G3D::max(b, x.b));
    }

    Color3uint8 min(const Color3uint8 x) const
    {
        return Color3uint8(
            G3D::min(r, x.r), G3D::min(g, x.g), G3D::min(b, x.b));
    }
    ...
    operator uint8*() { return (G3D::uint8*)this; }
    ...
    bool operator!=(const Color3uint8 other) const { return !(*this == other); }
