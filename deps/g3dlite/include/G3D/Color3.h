...
public:
    ...
    /**
     Initialize from an HTML-style color (e.g. 0xFF0000 == RED) by converting
     from sRGB to RGB.
    ...
    {
        return Color3(
            G3D::max(r, other.r), G3D::max(g, other.g), G3D::max(b, other.b));
    }

    inline Color3 min(const Color3& other) const
    {
        return Color3(
            G3D::min(r, other.r), G3D::min(g, other.g), G3D::min(b, other.b));
    }
    ...
    /**
     *  Converts from HSV to RGB , note: toHSV(fromHSV(_hsv)) may not be _hsv,
     *if it is at a grey point or black point. The components of _hsv should lie
    ...
    /** Duplicates the matlab jet colormap maps [0,1] --> (r,g,b) where blue is
     * close to  0 and red is close to 1. */
...
{
    return Color3(g * rkVector.b - b * rkVector.g,
                  b * rkVector.r - r * rkVector.b,
...
{
    Color3 kCross(g * rkVector.b - b * rkVector.g,
                  b * rkVector.r - r * rkVector.b,
                  r * rkVector.g - g * rkVector.r);
    return kCross.direction();
}

/** Radiance * measure(Solid Angle) between two points, measured at the receiver
 * orthogonal to the axis between them; W/m^2 */
