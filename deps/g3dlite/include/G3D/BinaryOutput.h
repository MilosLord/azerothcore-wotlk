...
private:
    ...
    /** Number of bytes already written to the file.  Even on 32-bit OS, this
     * can be 64-bits*/
...
public:
    ...
    /** Compresses the data in the buffer in place,
        ...
        \param level Compression level.  0 = fast, low compression; 9 = slow,
       high compression
     */
    ...
    /**
     ...
     @param flush If true (default) the file is ready for reading when the
     method returns, otherwise the method returns immediately and writes the
    ...
    */
    ...
    /**
      ...
      without allocating new memory.  The underlying array will not be
      deallocated, but the reset structure will act like a newly intialized one.
     */
    void reset();
    ...
    /** Write a string that always consumes len bytes, truncating or padding as
     * necessary*/
    ...
    /** Call before a series of BinaryOutput::writeBits calls. Only writeBits
        can be called between beginBits and endBits without corrupting the
       stream.*/
    void beginBits();

    /** Write numBits from bitString to the output stream.  Bits are numbered
       from low to high.

        Can only be
        called between beginBits and endBits.  Bits written are semantically
        little-endian, regardless of the actual endian-ness of the system.  That
       is, <CODE>writeBits(0xABCD, 16)</CODE> writes 0xCD to the first byte and
        0xAB to the second byte.  However, if used with BinaryInput::readBits,
       the ordering is transparent to the caller.
      */
