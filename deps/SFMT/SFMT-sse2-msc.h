...
inline static __m128i __vectorcall mm_recursion(__m128i a,
                                                __m128i b,
                                                __m128i c,
                                                __m128i d);
...
void sfmt_gen_rand_all(sfmt_t* sfmt)
{
    ...
    for (i = 0; i < SFMT_N - SFMT_POS1; i++) {
        pstate[i].si =
            mm_recursion(pstate[i].si, pstate[i + SFMT_POS1].si, r1, r2);
        r1 = r2;
        r2 = pstate[i].si;
    }
    for (; i < SFMT_N; i++) {
        pstate[i].si = mm_recursion(
            pstate[i].si, pstate[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1 = r2;
        r2 = pstate[i].si;
    }
}
...
static void gen_rand_array(sfmt_t* sfmt, w128_t* array, int size)
{
    ...
    for (; i < SFMT_N; i++) {
        array[i].si = mm_recursion(
            pstate[i].si, array[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1 = r2;
        r2 = array[i].si;
    }
    for (; i < size - SFMT_N; i++) {
        array[i].si = mm_recursion(
            array[i - SFMT_N].si, array[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1 = r2;
        r2 = array[i].si;
    }
...
void sfmt_gen_rand_all(sfmt_t* sfmt)
{
    ...
    for (i = 0; i < SFMT_N - SFMT_POS1; i++) {
        mm_recursion(
            pstate[i].si, pstate[i].si, pstate[i + SFMT_POS1].si, r1, r2);
        r1 = r2;
        r2 = pstate[i].si;
    }
    for (; i < SFMT_N; i++) {
        mm_recursion(pstate[i].si,
                     pstate[i].si,
...
static void gen_rand_array(sfmt_t* sfmt, w128_t* array, int size)
{
    ...
    for (i = 0; i < SFMT_N - SFMT_POS1; i++) {
        mm_recursion(
            array[i].si, pstate[i].si, pstate[i + SFMT_POS1].si, r1, r2);
        r1 = r2;
        r2 = array[i].si;
    }
    for (; i < SFMT_N; i++) {
        mm_recursion(array[i].si,
                     pstate[i].si,
