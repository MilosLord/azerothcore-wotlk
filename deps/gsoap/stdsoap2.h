...
#ifndef SOAP_FMAC5
#define SOAP_FMAC5                                                             \
    SOAP_STD_API /* export soapClient.cpp and soapServer.cpp API */
#endif
#endif
#endif
...
#define HAVE_ISINF
#if !defined(__GNUC__) ||                                                      \
    __GNUC__ >= 4 /* gcc 3 and earlier often refuse to compile _l functions */
#define HAVE_STRTOD_L
#define HAVE_SSCANF_L
#define HAVE_LOCALE_H
#endif
#define HAVE_SOCKLEN_T
#else /* Default assumptions for supported library functions when not          \
         including config.h */
...
/* gSOAP 2.7.15 and higher: always use FAST and retain CDATA in literal XML,
 * unless WITH_LEAN or WITH_NOCDATA */
#ifndef WITH_LEAN
#ifndef WITH_FAST
#define WITH_FAST
#endif
...
/* if we have locale.h then we should use it WITH_C_LOCALE enabled to avoid
 * decimal point conversion issues */
#ifdef HAVE_LOCALE_H
#ifndef WITH_NO_C_LOCALE
#ifndef WITH_C_LOCALE
...
/* user can set WITH_NO_C_LOCALE to force removal of locale (e.g. in case of
 * compiler errors) */
...
/* force inclusion of xlocale.h when WITH_INCLUDE_XLOCALE_H is defined by the
 * user for systems that require xlocale.h */
...
#define SOAP_LOCALE(soap)                                                      \
    ((soap)->c_locale ? (soap)->c_locale                                       \
                      : ((soap)->c_locale = _create_locale(LC_ALL, "C")))
#define SOAP_FREELOCALE(soap)                                                  \
    (void)((soap)->c_locale &&                                                 \
...
#define SOAP_STRCMP                                                            \
    soap_tag_cmp /* case insensitive XML element/attribute names */
#else
...
/* #define DEBUG */ /* Uncomment to debug sending (in file SENT.log) receiving
                       (in file RECV.log) and internal operations (in file
                       TEST.log) */

/* #define DEBUG_STAMP */ /* Uncomment to debug sending (in file SENT.log)
                             receiving (in file RECV.log) and time-stamped
                             operations (in file TEST.log) */

#ifdef __cplusplus
extern "C" {
#endif

/* Portability (X/Open, BSD sockets etc): define SOAP_SOCKLEN_T as socklen_t or
 * int or ... */
...
#define SOAP_SOCKLEN_T SOCKLEN_T
    ...
    defined(__ANDROID__) || !defined(_GNU_SOURCE) ||                           \
    (!_GNU_SOURCE && !defined(_POSIX_C_SOURCE) && !defined(_XOPEN_SOURCE)) ||  \
    _POSIX_C_SOURCE >= 200112L || _XOPEN_SOURCE >= 600
#define SOAP_SOCKLEN_T socklen_t
#elif defined(IRIX) || defined(WIN32) || defined(SUN_OS) ||                    \
    defined(OPENSERVER) || defined(TRU64) || defined(VXWORKS)
#define SOAP_SOCKLEN_T int
#elif !defined(SOAP_SOCKLEN_T)
#define SOAP_SOCKLEN_T socklen_t
#endif

/* AIX DCE threads portability: define SOAP_FUNC_R_ERR gmtime_r and localtime_r
 * err ret val as -1 */
...
#define SOAP_ULONG_FORMAT "%lu"
    ...
    defined(__GNU__) || defined(__GNUC__)
#ifndef LONG64
...
#define SOAP_ULONG_FORMAT                                                      \
    "%llu" /* printf format for unsigned 64 bit long long ints */
#endif
...
#define SOAP_BUFLEN                                                            \
    (65536) /* buffer length for socket packets, so don't make this too small  \
             */
#else
...
#ifndef WITH_LEAN
#define SOAP_PTRHASH                                                           \
    (4096) /* size of pointer analysis hash table (must be power of 2) */
#else
#define SOAP_PTRHASH (32)
#endif
#endif
...
#ifndef WITH_LEAN
#define SOAP_IDHASH                                                            \
    (1999) /* 1999, 8191: prime size of hash table for parsed id/ref */
#else
...
#ifndef WITH_LEAN
#define SOAP_TMPLEN                                                            \
    (2048) /* maximum length of msgbuf and tmpbuf short message buffers, must  \
              be >=1024 */
#else
...
#ifndef WITH_LEAN
#define SOAP_MAXPTRS                                                           \
    (4) /* maximum depth + 1 of id-ref deserialized pointer types (int* has    \
           depth 0, int*** has depth 2) */
#else
...
#define SOAP_MAXALLOCSIZE                                                      \
    (0) /* max size that malloc() can handle, zero for no limit */
#endif
...
/* Max number of EINTR interrupts to ignore while poll/select for pending
...
/* SOAP_MAXKEEPALIVE: Max iterations in soap_serve() to keep server connection
 * alive */
...
/* SOAP_MINDEFLATERATIO: Trusted deflation ratio after SOAP_MAXINFLATESIZE is
   reached. Trust when compressed / deflated > SOAP_MINDEFLATERATIO Sets a ratio
   > 0.00096899224806 (1032:1) According to the zlib site: the limit (1032:1)
   comes from the fact that one length/distance pair can represent at most 258
   output bytes. A length requires at least one bit and a distance requires at
   least one bit, so two bits in can give 258 bytes out, or eight bits in give
   1032 bytes out. A dynamic block has no length restriction, so you could get
   arbitrarily close to the limit of 1032:1.
*/
#ifndef SOAP_MINDEFLATERATIO
#define SOAP_MINDEFLATERATIO (1.0 / 1032.0) /* ratio of deflated/inflated */
#endif

/* maximum XML nesting depth level allowed for inbound XML parsing, must be
 * greater than zero (0) */
#ifndef SOAP_MAXLEVEL
#define SOAP_MAXLEVEL (10000)
#endif

/* maximum string content length if not already constrained by XML schema
 * validation maxLength constraints, zero or negative means unlimited string
 * lengths are allowed unless restricted by XML schema maxLength */
...
/* use safer snprintf if possible or guard sprintf against overrun (assumes no
 * variadic macros) */
#ifdef HAVE_SNPRINTF
#if _MSC_VER >= 1400
...
#define soap_strcpy(buf, len, src)                                             \
    (void)strncpy_s((buf), (len), (src), _TRUNCATE)
...
#if _MSC_VER >= 1400
#define soap_strcat(buf, len, src)                                             \
    (void)strncat_s((buf), (len), (src), _TRUNCATE)
...
/* copy string up to num chars (sets string to empty on overrun and returns
 * nonzero, zero if OK) */
#if _MSC_VER >= 1400
#define soap_strncpy(buf, len, src, num)                                       \
    ((buf) == NULL ||                                                          \
     ((size_t)(len) > (size_t)(num) ? strncpy_s((buf), (len), (src), (num))    \
                                    : ((buf)[0] = '\0', 1)))
#else
#define soap_strncpy(buf, len, src, num)                                       \
    ((buf) == NULL ||                                                          \
     ((size_t)(len) > (size_t)(num)                                            \
          ? (strncpy((buf), (src), (num)), (buf)[(size_t)(num)] = '\0')        \
          : ((buf)[0] = '\0', 1)))
#endif

/* concat string up to n chars (leaves destination intact on overrun and returns
 * nonzero, zero if OK) */
#if _MSC_VER >= 1400
#define soap_strncat(buf, len, src, num)                                       \
    ((buf) == NULL || ((size_t)(len) > strlen((buf)) + (size_t)(num)           \
                           ? strncat_s((buf), (len), (src), (num))             \
                           : 1))
#else
...
/* copy memory (returns SOAP_ERANGE on overrun, zero if OK, pointers must not be
 * NULL) */
#if _MSC_VER >= 1400
#define soap_memcpy(buf, len, src, num)                                        \
    ((size_t)(len) >= (size_t)(num) ? memcpy_s((buf), (len), (src), (num))     \
                                    : SOAP_ERANGE)
#else
#define soap_memcpy(buf, len, src, num)                                        \
    ((size_t)(len) >= (size_t)(num) ? !memcpy((buf), (src), (num))             \
                                    : SOAP_ERANGE)
#endif

/* move memory (returns SOAP_ERANGE on overrun, zero if OK, pointers must not be
 * NULL) */
#if _MSC_VER >= 1400
#define soap_memmove(buf, len, src, num)                                       \
    ((size_t)(len) >= (size_t)(num) ? memmove_s((buf), (len), (src), (num))    \
                                    : SOAP_ERANGE)
#else
#define soap_memmove(buf, len, src, num)                                       \
    ((size_t)(len) >= (size_t)(num) ? !memmove((buf), (src), (num))            \
                                    : SOAP_ERANGE)
#endif
...
#define soap_xml_error_check(e)                                                \
    ((e) == SOAP_TAG_MISMATCH || (e) == SOAP_NO_TAG || (e) == SOAP_IOB ||      \
     (e) == SOAP_SYNTAX_ERROR || (e) == SOAP_NAMESPACE || (e) == SOAP_TYPE ||  \
     (e) == SOAP_DUPLICATE_ID || (e) == SOAP_MISSING_ID ||                     \
     (e) == SOAP_REQUIRED || (e) == SOAP_PROHIBITED || (e) == SOAP_OCCURS ||   \
     (e) == SOAP_LENGTH || (e) == SOAP_LEVEL || (e) == SOAP_PATTERN ||         \
     (e) == SOAP_NULL || (e) == SOAP_HREF || (e) == SOAP_FIXED ||              \
     (e) == SOAP_EMPTY || (e) == SOAP_END_TAG || (e) == SOAP_UTF_ERROR)
...
#define soap_http_error_check(e)                                               \
    ((e) == SOAP_HTTP_ERROR || (e) == SOAP_NO_DATA ||                          \
     ((e) >= SOAP_GET_METHOD && (e) <= SOAP_HTTP_METHOD) ||                    \
     ((e) >= 100 && (e) < 600))

#define soap_dime_error_check(e)                                               \
    ((e) == SOAP_DIME_ERROR || (e) == SOAP_DIME_HREF ||                        \
     (e) == SOAP_DIME_MISMATCH || (e) == SOAP_DIME_END)

#define soap_mime_error_check(e)                                               \
    ((e) == SOAP_MIME_ERROR || (e) == SOAP_MIME_HREF || (e) == SOAP_MIME_END)
...
#define SOAP_STOP                                                              \
    1000 /* No HTTP response should follow after SOAP_STOP is issued */
#define SOAP_FORM                                                              \
    1001 /* Request (form) data is present, no HTTP response should follow */
#define SOAP_HTML 1002 /* Custom HTML response */
#define SOAP_FILE                                                              \
    1200 /* Custom file-based response with soap::http_content and optional    \
            http status */
...
#define SOAP_IO_FLUSH                                                          \
    0x00000000 /* out: flush output immediately, no buffering */
#define SOAP_IO_BUFFER                                                         \
    0x00000001 /* out: buffer output in packets of size SOAP_BUFLEN */
#define SOAP_IO_STORE                                                          \
    0x00000002 /* out: store entire output to determine length for transport   \
                */
#define SOAP_IO_CHUNK                                                          \
    0x00000003 /* out: use HTTP chunked transfer AND buffer packets */
...
#define SOAP_ENC_PLAIN                                                         \
    0x00000040 /* out: plain (XML or other) body, no HTTP header */
...
#define SOAP_XML_CANONICAL                                                     \
    0x00010000                     /* out: exc-C14N exclusive canonical XML */
...
#define SOAP_C_NOIOB                                                           \
    0x01000000 /* don't fault on array index out of bounds (just ignore) */
#define SOAP_C_UTFSTRING                                                       \
    0x02000000 /* (de)serialize strings with UTF8 content */
#define SOAP_C_MBSTRING                                                        \
    0x04000000 /* (de)serialize strings with multi-byte content */
#define SOAP_C_NILSTRING                                                       \
    0x08000000 /* serialize empty strings as nil (omitted) */
...
#define SOAP_SSL_REQUIRE_SERVER_AUTHENTICATION                                 \
    (0x0001) /* client requires server to authenticate */
#define SOAP_SSL_REQUIRE_CLIENT_AUTHENTICATION                                 \
    (0x0002) /* server requires client to authenticate */
#define SOAP_SSL_SKIP_HOST_CHECK                                               \
    (0x0004) /* client skips common name check against host name */
#define SOAP_SSL_ALLOW_EXPIRED_CERTIFICATE                                     \
    (0x0008) /* allow self-signed and expired certificates and those w/o CRL   \
              */
#define SOAP_SSL_NO_DEFAULT_CA_PATH                                            \
    (0x0010)                  /* don't use SSL_CTX_set_default_verify_paths */
...
#define SOAP_FREE(soap, ptr)                                                   \
    soap_track_free((soap), __FILE__, __LINE__, (void*)(ptr))
#endif
#endif
...
#ifndef SOAP_MALLOC_UNMANAGED /* use libc malloc to alloc soap context with    \
                                 soap_new() */
#define SOAP_MALLOC_UNMANAGED(size) malloc((size))
#endif

#ifndef SOAP_FREE_UNMANAGED /* use libc free to free soap context with         \
                               soap_free() */
...
#endif
#else
/* new form with parenthesis for (type) but not type[n], soap context may be
 * NULL */
#ifndef SOAP_NEW
#define SOAP_NEW(soap, type) new SOAP_NOTHROW(type)
#endif
...
#ifndef SOAP_DELETE_ARRAY /* use C++ delete[] operator, soap context may be    \
                             NULL */
#define SOAP_DELETE_ARRAY(soap, obj, type) delete[] obj
#endif

#ifndef SOAP_NEW_UNMANAGED /* use C++ unmanaged new operator for soap_new()    \
                              and soap::copy() */
#define SOAP_NEW_UNMANAGED(soap) new SOAP_NOTHROW soap
#endif

#ifndef SOAP_DELETE_UNMANAGED /* use C++ unmanaged delete operator for         \
                                 soap_free() */
        ...
        if (soap) {                                                            \
            ...
            if (soap->fdebug[SOAP_INDEX_##DBGFILE]) {                          \
                ...
                fprintf(fdebug,                                                \
                        ...
                        (int)_localTime.wYear % 100,                           \
                        (int)_localTime.wMonth,                                \
                        (int)_localTime.wDay,                                  \
                        (int)_localTime.wHour,                                 \
                        (int)_localTime.wMinute,                               \
                        (int)_localTime.wSecond,                               \
                        (int)_localTime.wMilliseconds);                        \
                CMD;                                                           \
                fflush(fdebug);                                                \
            }                                                                  \
        }                                                                      \
        ...
        if (soap) {                                                            \
            ...
            if (soap->fdebug[SOAP_INDEX_##DBGFILE]) {                          \
                fwrite((void*)(MSG),                                           \
                       1,                                                      \
                       (size_t)(LEN),                                          \
                       soap->fdebug[SOAP_INDEX_##DBGFILE]);                    \
        ...
        if (soap) {                                                            \
            ...
            if (soap->fdebug[SOAP_INDEX_##DBGFILE]) {                          \
                ...
                for (s = (char*)(MSG), i = (LEN); i; i--)                      \
                    fprintf(soap->fdebug[SOAP_INDEX_##DBGFILE],                \
                            "%2.2X  ",                                         \
                            (int)*s++ & 0xFF);                                 \
                fflush(soap->fdebug[SOAP_INDEX_##DBGFILE]);                    \
            }                                                                  \
...
struct soap_nlist {
    ...
    const char*
         ns;    /* only set when parsed ns URI is not in the ns mapping table */
    char id[1]; /* the actual string value flows into the allocated region
                   id[0...] below this struct */
};
...
struct soap_plist {
    ...
    int                type;
    int                id;
    char mark1; /* 0=single-ref, 1=embedded-multi-ref (SOAP1.1), 2=multi-ref,
                   3=attachment */
    char mark2;
};
...
struct soap_attribute {
    ...
    const char* ns;
    short       visible;
    char name[1]; /* the actual name string flows into the allocated region
                     below this struct */
};
...
struct soap_cookie {
    ...
    char*               path;
    ULONG64
         expire; /* client-side: local time to expire (value cast to time_t) */
    ...
    short env; /* server-side: got cookie from client and should not be (re)send
                */
    short modified; /* server-side: client cookie was modified and should be
                       send */
};
#endif

SOAP_FMAC1 struct soap_multipart* SOAP_FMAC2
...
public:
    ...
    {
        soap_multipart_iterator iter(*this);
        content = soap_next_multipart(content);
        return iter;
    }
...
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_new(struct soap* soap,
                                                            const char*  ns,
                                                            const char*  tag);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
...
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_set_w(
    struct soap_dom_element* elt, const char* ns, const wchar_t* tag);

SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2
...
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_nth_elt(
    struct soap_dom_element* elt, const char* ns, const char* tag, size_t n);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_nth_elt_w(
    struct soap_dom_element* elt, const char* ns, const wchar_t* tag, size_t n);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_nth(struct soap_dom_element* elt, size_t n);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_add_att(
    struct soap_dom_element* elt, const struct soap_dom_attribute* node);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_add_elt(struct soap_dom_element* elt, const struct soap_dom_element* node);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_add_atts(
    struct soap_dom_element* elt, const struct soap_dom_attribute* atts);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_add_elts(
    struct soap_dom_element* elt, const struct soap_dom_element* elts);
...
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_copy(
    struct soap_dom_element* elt, const struct soap_dom_element* node);
...
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2 soap_att_set_w(
    struct soap_dom_attribute* att, const char* ns, const wchar_t* tag);
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2
soap_att_add(struct soap_dom_attribute* att, const char* ns, const char* tag);
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2 soap_att_add_w(
    struct soap_dom_attribute* att, const char* ns, const wchar_t* tag);
...
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2 soap_att_copy(
    struct soap_dom_attribute* att, const struct soap_dom_attribute* node);
...
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2 soap_att_get(
    const struct soap_dom_element* elt, const char* ns, const char* tag);
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2 soap_att_get_w(
    const struct soap_dom_element* elt, const char* ns, const wchar_t* tag);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_get(
    const struct soap_dom_element* elt, const char* ns, const char* tag);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_get_w(
    const struct soap_dom_element* elt, const char* ns, const wchar_t* tag);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
...
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2 soap_att_find_next(
    const struct soap_dom_attribute* att, const char* ns, const char* patt);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_elt_find(struct soap_dom_element* elt, const char* ns, const char* patt);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_find_next(
    const struct soap_dom_element* elt, const char* ns, const char* patt);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_elt_find_type(
    struct soap_dom_element* elt, const char* ns, const char* patt, int type);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_elt_find_next_type(const struct soap_dom_element* elt,
                        const char*                    ns,
                        const char*                    patt,
                        int                            type);
...
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2 soap_dom_next_element(
    const struct soap_dom_element* elt, const struct soap_dom_element* end);

/* DOM deep search */

SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_dom_find(struct soap_dom_element*       begin,
              const struct soap_dom_element* end,
              const char*                    ns,
              const char*                    patt,
              int                            type);
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_dom_find_next(const struct soap_dom_element* elt,
                   const struct soap_dom_element* end,
                   const char*                    ns,
                   const char*                    patt,
                   int                            type);
...
struct SOAP_CMAC soap_dom_attribute {
    ...
    {
        return *soap_att_set(this, ns, tag);
    }
    soap_dom_attribute& set(const char* ns, const wchar_t* tag)
    {
        return *soap_att_set_w(this, ns, tag);
    }
    ...
    soap_dom_attribute& set(const char* str)
    {
        return *soap_att_text(this, str);
    }
    soap_dom_attribute& set(const wchar_t* str)
    {
        return *soap_att_text_w(this, str);
    }
#ifndef WITH_COMPAT
    soap_dom_attribute& set(const std::string& str)
    {
        return *soap_att_text(this, str.c_str());
    }
    soap_dom_attribute& set(const std::wstring& str)
    {
        return *soap_att_text_w(this, str.c_str());
    }
#endif
    ...
    {
        return *soap_att_double(this, x);
    }
    soap_dom_attribute& operator=(const char* str)
    {
        return *soap_att_text(this, str);
    }
    soap_dom_attribute& operator=(const wchar_t* str)
    {
        return *soap_att_text_w(this, str);
    }
#ifndef WITH_COMPAT
    soap_dom_attribute& operator=(const std::string& str)
    {
        return *soap_att_text(this, str.c_str());
    }
    soap_dom_attribute& operator=(const std::wstring& str)
    {
        return *soap_att_text_w(this, str.c_str());
    }
#endif
    ...
    {
        return soap_dom_attribute_iterator(this);
    }
    soap_dom_attribute_iterator att_end()
    {
        return soap_dom_attribute_iterator(NULL);
    }
    soap_dom_attribute_iterator att_find(const char* patt)
    {
        return att_find(NULL, patt);
    }
...
struct SOAP_CMAC soap_dom_element {
    ...
    {
        return *soap_elt_set(this, ns, tag);
    }
    soap_dom_element& set(const char* ns, const wchar_t* tag)
    {
        return *soap_elt_set_w(this, ns, tag);
    }
    ...
    soap_dom_element& set(const wchar_t* str)
    {
        return *soap_elt_text_w(this, str);
    }
#ifndef WITH_COMPAT
    soap_dom_element& set(const std::string& str)
    {
        return *soap_elt_text(this, str.c_str());
    }
    soap_dom_element& set(const std::wstring& str)
    {
        return *soap_elt_text_w(this, str.c_str());
    }
#endif
    soap_dom_element& set(const void* nod, int typ)
    {
        return *soap_elt_node(this, nod, typ);
    }
    soap_dom_element& add(soap_dom_element& elt)
    {
        return *soap_add_elt(this, &elt);
    }
    soap_dom_element& add(soap_dom_element* elt)
    {
        return *soap_add_elt(this, elt);
    ...
    {
        return *soap_add_att(this, &att);
    }
    soap_dom_element& add(soap_dom_attribute* att)
    {
        return *soap_add_att(this, att);
    }
    soap_dom_element& adds(soap_dom_element& elt)
    {
        return *soap_add_elts(this, &elt);
    }
    soap_dom_element& adds(soap_dom_element* elt)
    {
        return *soap_add_elts(this, elt);
    }
    soap_dom_element& adds(soap_dom_attribute& att)
    {
        return *soap_add_atts(this, &att);
    }
    soap_dom_element& adds(soap_dom_attribute* att)
    {
        return *soap_add_atts(this, att);
    ...
    soap_dom_element& operator=(const char* str)
    {
        return *soap_elt_text(this, str);
    }
    soap_dom_element& operator=(const wchar_t* str)
    {
        return *soap_elt_text_w(this, str);
    }
#ifndef WITH_COMPAT
    soap_dom_element& operator=(const std::string& str)
    {
        return *soap_elt_text(this, str.c_str());
    }
    soap_dom_element& operator=(const std::wstring& str)
    {
        return *soap_elt_text_w(this, str.c_str());
    }
#endif
    ...
    {
        return *soap_elt_copy(this, &elt);
    }
    template <class T>
    soap_dom_element& operator=(const T& nod)
    {
        return this->set(&nod, nod.soap_type());
    }
    template <class T>
    soap_dom_element& operator=(const T* nod)
    {
    ...
    template <class T>
    soap_dom_element& operator=(T* nod)
    {
        return this->set(nod, nod->soap_type());
    }
    soap_dom_attribute& att(const char* tag)
    {
        return *soap_att(this, NULL, tag);
    }
    soap_dom_attribute& att(const wchar_t* tag)
    {
        return *soap_att_w(this, NULL, tag);
    }
    soap_dom_attribute& att(const char* ns, const char* tag)
    {
        return *soap_att(this, ns, tag);
    }
    soap_dom_attribute& att(const char* ns, const wchar_t* tag)
    {
        return *soap_att_w(this, ns, tag);
    }
    soap_dom_element& elt() { return *soap_elt(this, NULL, NULL); }
    soap_dom_element& elt(const char* tag)
    {
        return *soap_elt(this, NULL, tag);
    }
    soap_dom_element& elt(const wchar_t* tag)
    {
        return *soap_elt_w(this, NULL, tag);
    }
    soap_dom_element& elt(const char* ns, const char* tag)
    {
        return *soap_elt(this, ns, tag);
    }
    soap_dom_element& elt(const char* ns, const wchar_t* tag)
    {
        return *soap_elt_w(this, ns, tag);
    }
    ...
    {
        return soap_elt_match(this, NULL, pat) != 0;
    }
    bool match(const wchar_t* pat) const
    {
        return soap_elt_match_w(this, NULL, pat) != 0;
    }
    bool match(const char* ns, const char* pat) const
    {
        return soap_elt_match(this, ns, pat) != 0;
    }
    bool match(const char* ns, const wchar_t* pat) const
    {
        return soap_elt_match_w(this, ns, pat) != 0;
    }
    const char*       ns() const { return this->nstr; }
    const char*       tag() const { return this->name; }
    ...
    size_t            elt_size(const char* pat, int typ = 0)
    {
        return elt_size(NULL, pat, typ);
    }
    size_t elt_size(const char* ns, const char* pat, int typ = 0)
    {
        return soap_elt_size_type(this, ns, pat, typ);
    }
...
#endif
    size_t att_size(const char* ns, const char* pat)
    {
        return soap_att_size(this, ns, pat);
    }
#ifndef WITH_COMPAT
    size_t att_size(const char* ns, const std::string& pat)
    {
        return soap_att_size(this, ns, pat.c_str());
    }
#endif
    ...
    int         get_type(const void** nod) const
    {
        return soap_elt_get_type(this, nod);
    }
    ...
    {
        return soap_dom_element_iterator(this->elts);
    }
    soap_dom_element_iterator elt_end()
    {
        return soap_dom_element_iterator(NULL);
    }
    soap_dom_attribute_iterator att_begin()
    {
        return soap_dom_attribute_iterator(this->atts);
    }
    soap_dom_attribute_iterator att_end()
    {
        return soap_dom_attribute_iterator(NULL);
    }
    soap_dom_element_iterator find(const char* pat, int typ = 0)
    {
        return find(NULL, pat, typ);
    }
    soap_dom_element_iterator find(const wchar_t* pat, int typ = 0)
    {
        return find(NULL, pat, typ);
    }
    soap_dom_element_iterator
    find(const char* ns, const char* pat, int typ = 0);
    soap_dom_element_iterator
    find(const char* ns, const wchar_t* pat, int typ = 0);
    soap_dom_element_iterator find(int typ);
    soap_dom_element_iterator elt_find(const char* pat, int typ = 0)
    {
        return elt_find(NULL, pat, typ);
    }
    soap_dom_element_iterator elt_find(const wchar_t* pat, int typ = 0)
    {
        return elt_find(NULL, pat, typ);
    }
    soap_dom_element_iterator
    elt_find(const char* ns, const char* pat, int typ = 0);
    soap_dom_element_iterator
    elt_find(const char* ns, const wchar_t* pat, int typ = 0);
    soap_dom_element_iterator   elt_find(int typ);
    soap_dom_attribute_iterator att_find(const char* pat)
    {
        return att_find(NULL, pat);
    }
    soap_dom_attribute_iterator att_find(const wchar_t* pat)
    {
        return att_find(NULL, pat);
    }
...
#ifndef WITH_LEANER
SOAP_FMAC1 struct soap_dom_element* SOAP_FMAC2
soap_dup_xsd__anyType(struct soap*                   soap,
...
SOAP_FMAC1 struct soap_dom_attribute* SOAP_FMAC2
soap_dup_xsd__anyAttribute(struct soap*                     soap,
...
struct SOAP_CMAC soap {
    ...
    short version;   /* 1 = SOAP1.1 and 2 = SOAP1.2 (set automatically from
                        namespace URI in nsmap table), 0 indicates non-SOAP
                        content */
    soap_mode mode;  /* internal mode flag, combines imode/omode */
    soap_mode imode; /* input mode flag set with soap_init1(), soap_new1(), or
                        soap_set_imode() */
    soap_mode omode; /* ouput mode flag set with soap_init1(), soap_new1(), or
                        soap_set_omode() */
    const char* float_format;  /* user-definable format string for floats (<1024
                                  chars) */
    const char* double_format; /* user-definable format string for doubles
                                  (<1024 chars) */
    const char* long_double_format; /* user-definable format string for long
                                       doubles (<1024 chars) */
    const char* dime_id_format; /* user-definable format string for integer DIME
                                   id (<SOAP_TAGLEN chars) */
    ULONG64 recv_maxlength; /* user-definable, when > 0, sets max message size
                               that can be received */
    int recv_timeout;       /* user-definable, when > 0, sets socket recv stall
                               timeout in seconds, < 0 in usec */
    int send_timeout;       /* user-definable, when > 0, sets socket send stall
                               timeout in seconds, < 0 in usec */
    int transfer_timeout;   /* user-definable, when > 0, sets socket total
                               transfer timeout in seconds, < 0 in usec */
    int connect_timeout;    /* user-definable, when > 0, sets socket connect()
                               timeout in seconds, < 0 in usec */
    int accept_timeout;     /* user-definable, when > 0, sets socket accept()
                               timeout in seconds, < 0 in usec */
    int socket_flags; /* user-definable socket recv() and send() flags, e.g. set
                         to MSG_NOSIGNAL to disable sigpipe */
    int connect_flags; /* user-definable connect() SOL_SOCKET sockopt flags,
                          e.g. set to SO_DEBUG to debug socket */
    int connect_retry; /* number of times to retry connecting (exponential
                          backoff), zero by default */
    int bind_flags; /* user-definable bind() SOL_SOCKET sockopt flags, e.g. set
                       to SO_REUSEADDR to enable reuse */
    short bind_inet6;  /* user-definable, when > 0 use AF_INET6 instead of
                          PF_UNSPEC (only with -DWITH_IPV6) */
    short bind_v6only; /* user-definable, when > 0 use IPPROTO_IPV6 sockopt
                          IPV6_V6ONLY (only with -DWITH_IPV6) */
    int accept_flags;  /* user-definable accept() SOL_SOCKET sockopt flags */
#ifdef WITH_SELF_PIPE
    int pipe_fd[2]; /* self pipe trick file descriptors used to close the select
                       call from another thread */
#endif
    int sndbuf; /* user-definable SO_SNDBUF setsockopt value */
    int rcvbuf; /* user-definable SO_RCVBUF setsockopt value */
    unsigned short
        linger_time;       /* user-definable linger time for SO_LINGER option */
    unsigned int maxlevel; /* user-definable max XML nesting depth levels,
                              initialized to SOAP_MAXLEVEL */
    long maxlength;        /* user-definable max string length, initialized to
                              SOAP_MAXLENGTH, maxlength<=0 is unbounded */
    size_t maxoccurs; /* user-definable max array/container size, initialized to
                         SOAP_MAXOCCURS */
    const char* http_version;      /* HTTP version used "1.0" or "1.1" */
    const char* http_content;      /* optional custom HTTP content type (with
                                      SOAP_PUT, SOAP_POST_FILE, SOAP_FILE) */
    const char* http_extra_header; /* optional custom HTTP header of the form
                                      'key: val' (multiple headers should be
                                      separated in the string by \r\n - crlf) */
    const char*
        encodingStyle; /* default = "" which means that SOAP encoding is used */
    const char* actor; /* SOAP-ENV:actor or role attribute value */
    const char*
        lang; /* user-definable xml:lang attribute value of SOAP-ENV:Text */
    ...
    int (*fpatch)(
        struct soap*); /* HTTP PATCH hook (handled as POST by default) */
    ...
    void* (*fdimewriteopen)(struct soap*,
                            const char*,
                            const char*,
                            const char*);
    ...
    void* (*fmimereadopen)(
        struct soap*, void*, const char*, const char*, const char*);
    void* (*fmimewriteopen)(struct soap*,
                            void*,
                            const char*,
                            const char*,
                            const char*,
...
#else
    const char** os; /* C only: pointer to a const char*, will be set to point
                        to the string output */
    const char*
        is; /* C only: a const char* to read from (soap->is will advance) */
#endif
#ifndef UNDER_CE
    ...
    char msgbuf[SOAP_TMPLEN]; /* in/out buffer for HTTP/MIME headers and short
                                 messages, must be >=1024 bytes */
    char tmpbuf[SOAP_TMPLEN]; /* in/out buffer for HTTP/MIME headers, simpleType
                                 values, element and attribute tag names, and
                                 DIME must be >=1024 bytes */
    ...
    unsigned int ip6[4]; /* same for IPv6: upper in ip6[0] to lower in ip6[3]
                            requires WITH_IPV6 */
    int         port;    /* port number */
    const char* override_host; /* to override the client-side host name/IP when
                                  connecting */
    ...
    int max_keep_alive; /* maximum keep-alive session (default=100) 0 to always
                           keep open */
    ...
    const char* x_frame_options; /* "DENY", "SAMEORIGIN" (default), or
                                    "ALLOW-FROM uri" */
    ...
    const char*
        c14nexclude; /* space-separated string of prefixes for c14n exclusion */
    const char*
        c14ninclude; /* space-separated string of prefixes for c14n inclusion */
#endif
...
#ifndef WITH_NOIO
    unsigned int
        ipv6_multicast_if; /* in_addr_t in6addr->sin6_scope_id IPv6 value */
    char*
        ipv4_multicast_if; /* IP_MULTICAST_IF IPv4 setsockopt interface_addr */
    unsigned char ipv4_multicast_ttl; /* IP_MULTICAST_TTL value 0..255 */
    const char*   client_addr;    /* when non-NULL, client binds to this address
                                     before connect */
    const char* client_addr_ipv6; /* WITH_IPV6: when non-NULL and client_addr is
                                     non-NULL and when connecting to a IPv6
                                     server, client binds to this IPv6 address
                                     instead of client_addr */
    int client_port; /* when nonnegative, client binds to this port before
                        connect */
    const char* client_interface; /* when non-NULL, override client-side
                                     interface address using this address */
    union {
...
#ifdef SOAP_LOCALE_T
    SOAP_LOCALE_T
        c_locale; /* if this does not compile, use ./configure --enable-xlocale
                     or compile with -DWITH_INCLUDE_XLOCALE_H, or use
                     -DWITH_NO_C_LOCALE to disable locale support */
#else
    void* c_locale;
#endif
#ifdef WITH_ZLIB
    ...
    void         destroy();
    ~soap(); /* no virtual methods, so sizeof(soap) should be the same in C and
                C++ */
#endif
};
...
struct soap_plugin {
    ...
    const char*         id;
    void*               data;
    int (*fcopy)(struct soap*        soap,
                 struct soap_plugin* dst,
                 struct soap_plugin* src);
    void (*fdelete)(struct soap*        soap,
                    struct soap_plugin* p); /* should delete fields of plugin
                                               only and not free(p) */
};
...
#define soap_set_mode(soap, n)                                                 \
    ((soap)->mode |= (n), (soap)->imode |= (n), (soap)->omode |= (n))
#define soap_clr_mode(soap, n)                                                 \
    ((soap)->mode &= ~(n), (soap)->imode &= ~(n), (soap)->omode &= ~(n))
#define soap_destroy(soap) soap_delete((soap), NULL)

#define SOAP_NO_LINK_TO_DELETE                                                 \
    (-2) /* pass to soap_link() as size n: do not manage, smart pointers are   \
            self-managing */
...
SOAP_FMAC1 int SOAP_FMAC2  soap_PUT(struct soap*,
                                    const char*,
                                    const char*,
                                    const char*);
SOAP_FMAC1 int SOAP_FMAC2  soap_PATCH(struct soap*,
                                      const char*,
                                      const char*,
                                      const char*);
SOAP_FMAC1 int SOAP_FMAC2  soap_POST(struct soap*,
                                     const char*,
                                     const char*,
                                     const char*);
...
SOAP_FMAC1 const char* SOAP_FMAC2 soap_http_header_attribute(struct soap*,
                                                             const char*,
                                                             const char*);
...
#ifndef WITH_LEANER
SOAP_FMAC1 int SOAP_FMAC2
soap_xop_forward(struct soap*, unsigned char**, int*, char**, char**, char**);
SOAP_FMAC1 int SOAP_FMAC2 soap_attachment_forward(
...
SOAP_FMAC1 int SOAP_FMAC2
soap_array_reference(struct soap*, const void* p, const void* a, int n, int t);
...
SOAP_FMAC1 const struct soap_code_map* SOAP_FMAC2
soap_code(const struct soap_code_map*, const char*);
SOAP_FMAC1 LONG64 SOAP_FMAC2      soap_code_int(const struct soap_code_map*,
                                                const char*,
                                                LONG64);
SOAP_FMAC1 const char* SOAP_FMAC2 soap_code_str(const struct soap_code_map*,
                                                long);
SOAP_FMAC1 LONG64 SOAP_FMAC2      soap_code_bits(const struct soap_code_map*,
                                                 const char*);
SOAP_FMAC1 const char* SOAP_FMAC2 soap_code_list(struct soap*,
                                                 const struct soap_code_map*,
                                                 long);

SOAP_FMAC1 int SOAP_FMAC2 soap_binary_search_string(const char**,
                                                    int,
                                                    const char*);
...
SOAP_FMAC1 struct soap_clist* SOAP_FMAC2 soap_link(
    struct soap*, int, int, int (*fdelete)(struct soap*, struct soap_clist*));
...
SOAP_FMAC1 void* SOAP_FMAC2 soap_id_lookup(struct soap*,
                                           ...
                                           unsigned int k,
                                           int (*fbase)(int, int));
SOAP_FMAC1 void* SOAP_FMAC2 soap_id_forward(
    struct soap*,
    const char*  id,
    void*        p,
    size_t       i,
...
#endif
SOAP_FMAC1 void* SOAP_FMAC2 soap_id_enter(
    struct soap*,
    const char* id,
    void*       p,
    int         t,
...
SOAP_FMAC1 char* SOAP_FMAC2
soap_putsizesoffsets(struct soap*, const char*, const int*, const int*, int);
...
SOAP_FMAC1 struct soap* SOAP_FMAC2 soap_versioning(soap_new)(soap_mode,
                                                             soap_mode);
...
SOAP_FMAC1 void
    SOAP_FMAC2 soap_versioning(soap_init)(struct soap*, soap_mode, soap_mode);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_match_tag(struct soap*,
                                         const char*,
                                         const char*);
SOAP_FMAC1 int SOAP_FMAC2 soap_match_att(struct soap*,
                                         const char*,
                                         const char*);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_string_out(struct soap*,
                                          ...
                                          int         flag);
SOAP_FMAC1 char* SOAP_FMAC2
soap_string_in(struct soap*, int, long, long, const char*);

#ifndef WITH_LEANER
SOAP_FMAC1 int SOAP_FMAC2 soap_wstring_out(struct soap*,
                                           const wchar_t* s,
                                           int            flag);
SOAP_FMAC1 wchar_t* SOAP_FMAC2
soap_wstring_in(struct soap*, int, long, long, const char*);
#endif

SOAP_FMAC1 int SOAP_FMAC2 soap_match_namespace(
    struct soap*, const char*, const char*, size_t n1, size_t n2);
...
SOAP_FMAC1 struct soap_nlist* SOAP_FMAC2 soap_push_namespace(struct soap*,
                                                             const char*,
                                                             const char*);
...
SOAP_FMAC1 void* SOAP_FMAC2              soap_push_block(struct soap*,
                                                         struct soap_blist*,
                                                         size_t);
SOAP_FMAC1 void* SOAP_FMAC2              soap_push_block_max(struct soap*,
                                                             struct soap_blist*,
                                                             size_t);
...
SOAP_FMAC1 char* SOAP_FMAC2 soap_http_get_body_prefix(struct soap*,
                                                      size_t*     len,
                                                      const char* prefix);
...
SOAP_FMAC1 void SOAP_FMAC2 soap_stream_fault_location(struct soap*,
                                                      std::ostream&);
#endif
#endif
...
SOAP_FMAC1 int SOAP_FMAC2 soap_s2char(struct soap*,
                                      const char*,
                                      char**,
                                      int,
                                      long        minlen,
...
#ifndef WITH_COMPAT
#ifdef __cplusplus
SOAP_FMAC1 int SOAP_FMAC2 soap_s2stdQName(struct soap*,
                                          const char*,
                                          std::string*,
                                          long        minlen,
                                          long        maxlen,
                                          const char* pattern);
SOAP_FMAC1 int SOAP_FMAC2 soap_s2stdchar(struct soap*,
                                         const char*,
                                         std::string*,
                                         int,
                                         long        minlen,
                                         long        maxlen,
                                         const char* pattern);
SOAP_FMAC1 int SOAP_FMAC2 soap_s2stdwchar(struct soap*,
                                          const char*,
                                          std::wstring*,
                                          int,
                                          long        minlen,
...
SOAP_FMAC1 const char* SOAP_FMAC2
soap_base642s(struct soap*, const char*, char*, size_t, int*);
SOAP_FMAC1 const char* SOAP_FMAC2
soap_hex2s(struct soap*, const char*, char*, size_t, int*);

SOAP_FMAC1 int* SOAP_FMAC2
soap_inint(struct soap*, const char* tag, int* p, const char*, int);
SOAP_FMAC1 char* SOAP_FMAC2
soap_inbyte(struct soap*, const char* tag, char* p, const char*, int);
SOAP_FMAC1 long* SOAP_FMAC2
soap_inlong(struct soap*, const char* tag, long* p, const char*, int);
SOAP_FMAC1 LONG64* SOAP_FMAC2
soap_inLONG64(struct soap*, const char* tag, LONG64* p, const char*, int);
SOAP_FMAC1 short* SOAP_FMAC2
soap_inshort(struct soap*, const char* tag, short* p, const char*, int);
SOAP_FMAC1 float* SOAP_FMAC2
soap_infloat(struct soap*, const char* tag, float* p, const char*, int);
SOAP_FMAC1 double* SOAP_FMAC2
soap_indouble(struct soap*, const char* tag, double* p, const char*, int);
...
SOAP_FMAC1 ULONG64* SOAP_FMAC2
soap_inULONG64(struct soap*, const char* tag, ULONG64* p, const char*, int);
...
#ifndef WITH_LEAN
SOAP_FMAC1 time_t* SOAP_FMAC2
soap_indateTime(struct soap*, const char* tag, time_t* p, const char*, int);
#endif
...
SOAP_FMAC1 int SOAP_FMAC2  soap_set_dime_attachment(struct soap*,
                                                    ...
                                                    const char*    option);
SOAP_FMAC1 int SOAP_FMAC2
                           soap_set_mime_attachment(struct soap*,
                                                    const char*             ptr,
                                                    size_t                  size,
...
SOAP_FMAC1 int SOAP_FMAC2 soap_register_plugin_arg(
    struct soap*,
    int (*fcreate)(struct soap*, struct soap_plugin*, void*),
    void*);
...
SOAP_FMAC1 const char* SOAP_FMAC2 soap_encode_url_string(struct soap*,
                                                         const char*);
#ifdef WITH_COOKIES
SOAP_FMAC1 void SOAP_FMAC2 soap_getcookies(struct soap* soap, const char* val);
SOAP_FMAC1 struct soap_cookie* SOAP_FMAC2 soap_set_cookie(
    struct soap*, const char*, const char*, const char*, const char*);
SOAP_FMAC1 struct soap_cookie* SOAP_FMAC2 soap_cookie(struct soap*,
                                                      const char*,
                                                      const char*,
                                                      const char*);
SOAP_FMAC1 struct soap_cookie* SOAP_FMAC2
soap_cookie_env(struct soap*, const char*, const char*, const char*, short);
SOAP_FMAC1 const char* SOAP_FMAC2 soap_cookie_value(struct soap*,
                                                    const char*,
                                                    const char*,
                                                    const char*);
SOAP_FMAC1 const char* SOAP_FMAC2 soap_env_cookie_value(struct soap*,
                                                        const char*,
                                                        const char*,
                                                        const char*);
SOAP_FMAC1 time_t SOAP_FMAC2      soap_cookie_expire(struct soap*,
                                                     const char*,
                                                     const char*,
                                                     const char*);
SOAP_FMAC1 int SOAP_FMAC2         soap_set_cookie_expire(
            struct soap*, const char*, long, const char*, const char*);
SOAP_FMAC1 int SOAP_FMAC2                 soap_set_cookie_secure(struct soap*,
                                                                 const char*,
                                                                 const char*,
                                                                 const char*);
SOAP_FMAC1 int SOAP_FMAC2                 soap_set_cookie_session(struct soap*,
                                                                  const char*,
                                                                  const char*,
                                                                  const char*);
SOAP_FMAC1 int SOAP_FMAC2                 soap_clr_cookie_session(struct soap*,
                                                                  const char*,
                                                                  const char*,
                                                                  const char*);
SOAP_FMAC1 void SOAP_FMAC2                soap_clr_cookie(struct soap*,
                                                          const char*,
                                                          const char*,
                                                          const char*);
SOAP_FMAC1 int SOAP_FMAC2                 soap_getenv_cookies(struct soap*);
SOAP_FMAC1 void SOAP_FMAC2                soap_free_cookies(struct soap*);
SOAP_FMAC1 struct soap_cookie* SOAP_FMAC2 soap_copy_cookies(struct soap*,
                                                            const struct soap*);
SOAP_FMAC1 int SOAP_FMAC2                 soap_putsetcookies(struct soap* soap);
SOAP_FMAC1 int SOAP_FMAC2                 soap_putcookies(struct soap* soap,
                                                          const char*  domain,
                                                          const char*  path,
                                                          int          secure);
#endif
...
/* declare global SOAP header and fault processing functions, unless WITH_STATIC
 * is defined (generated in soapXYXLib code) */
#ifndef WITH_STATIC
...
/* C++ templates (for generated C++ only, not needed nor used in generated C
 * code) */
...
struct soap_block {
    ...
    static void save(struct soap* soap, struct soap_blist* b, T* p)
    {
        if (!b)
            b = soap->blist;
        for (T* q = (T*)soap_first_block(soap, b); q;
             q    = (T*)soap_next_block(soap, b)) {
            soap_update_pointers(
                soap, (const char*)p, (const char*)q, sizeof(T));
            *p++ = *q;
            q->~T();
        }
    ...
    static void end(struct soap* soap, struct soap_blist* b)
    {
        if (!b)
            b = soap->blist;
        for (T* p = (T*)soap_first_block(soap, b); p;
             p    = (T*)soap_next_block(soap, b))
            p->~T();
        soap_end_block(soap, b);
    }
};
