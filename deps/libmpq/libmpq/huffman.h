...
#define LIBMPQ_HUFF_DECOMPRESS                                                 \
    0 /* we want to decompress using huffman trees. */
...
struct huffman_tree_item_s {
    struct huffman_tree_item_s*
        next; /* 00 - pointer to next huffman tree item. */
    struct huffman_tree_item_s*
             prev; /* 04 - pointer to prev huffman tree item (< 0 if none). */
    uint32_t dcmp_byte;  /* 08 - index of this item in item pointer array,
                            decompressed byte value. */
    uint32_t byte_value; /* 0C - some byte value. */
    struct huffman_tree_item_s*
        parent; /* 10 - pointer to parent huffman tree item (NULL if none). */
    struct huffman_tree_item_s*
        child; /* 14 - pointer to child huffman tree item. */
};
...
struct huffman_decompress_s {
    ...
    union {
        uint32_t
            dcmp_byte; /* 08 - byte value for decompress (if bitCount <= 7). */
        struct huffman_tree_item_s* p_item; /* 08 - huffman tree item (if number
                                               of bits is greater than 7). */
    };
};
...
struct huffman_tree_s {
    ...
    struct huffman_tree_item_s
        items0008[0x203];                 /* 0008 - huffman tree items. */
    struct huffman_tree_item_s* item3050; /* 3050 - always NULL? */
    struct huffman_tree_item_s*
        item3054; /* 3054 - pointer to huffman tree item. */
    struct huffman_tree_item_s*
        item3058; /* 3058 - pointer to huffman tree item (< 0 if invalid). */
    struct huffman_tree_item_s* item305C; /* 305C - usually NULL. */
    struct huffman_tree_item_s*
        first; /* 3060 - pointer to top (first) huffman tree item. */
    struct huffman_tree_item_s* last; /* 3064 - pointer to bottom (last) huffman
                                         tree item (< 0 if invalid). */
    uint32_t items; /* 3068 - number of used huffman tree items. */
    struct huffman_tree_item_s*
        items306C[0x102]; /* 306C - huffman tree item pointer array. */
    struct huffman_decompress_s
            qd3474[0x80];     /* 3474 - array for quick decompression. */
    uint8_t table_1502A630[]; /* some table to make struct size flexible. */
};

/* insert a new item into huffman tree. */
void libmpq__huffman_insert_item(struct huffman_tree_item_s** p_item,
...
/* call 1500E820- */
void libmpq__huffman_call_1500E820(struct huffman_tree_s*      ht,
...
int32_t libmpq__do_decompress_huffman(struct huffman_tree_s*         ht,
