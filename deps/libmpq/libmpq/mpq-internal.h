...
#define LIBMPQ_ARCHIVE_VERSION_ONE                                             \
    0 /* version one used until world of warcraft. */
#define LIBMPQ_ARCHIVE_VERSION_TWO                                             \
    1 /* version two used from world of warcraft - the burning crusade. */

/* define values used by blizzard as flags. */
#define LIBMPQ_FLAG_EXISTS                                                     \
    0x80000000 /* set if file exists, reset when the file was deleted. */
#define LIBMPQ_FLAG_ENCRYPTED                                                  \
    0x00010000 /* indicates whether file is encrypted. */
#define LIBMPQ_FLAG_COMPRESSED 0x0000FF00 /* file is compressed. */
#define LIBMPQ_FLAG_COMPRESS_PKZIP                                             \
    0x00000100 /* compression made by pkware data compression library. */
#define LIBMPQ_FLAG_COMPRESS_MULTI 0x00000200 /* multiple compressions. */
#define LIBMPQ_FLAG_COMPRESS_NONE                                              \
    0x00000300 /* no compression (no blizzard flag used by myself). */
#define LIBMPQ_FLAG_SINGLE                                                     \
    0x01000000 /* file is stored in one single sector, first seen in world of  \
                  warcraft. */
#define LIBMPQ_FLAG_CRC                                                        \
    0x04000000 /* compressed block offset table has CRC checksum. */

/* define generic hash values. */
#define LIBMPQ_HASH_FREE                                                       \
...
typedef struct {
    ...
    uint32_t block_table_offset; /* file position of mpq_block, each entry has
                                    16 bytes. */
...
/* mpq extended archive header, used since world of warcraft - the burning
 * crusade. */
typedef struct {
    uint64_t
        extended_offset; /* offset to the beginning of the extended block table,
                            relative to the beginning of the archive. */
    uint16_t hash_table_offset_high;  /* upper 16 bits of the hash table offset
                                         for large archives. */
    uint16_t block_table_offset_high; /* upper 16 bits of the block table offset
                                         for large archives.*/
...
/* extended file description block contains information about the offset beyond
 * 2^32 (4GB). */
typedef struct {
...
typedef struct {
    ...
    uint32_t* packed_offset; /* position of each file block (only for packed
                                files). */
    uint32_t
        open_count; /* number of times it has been opened - used for freeing */
} PACK_STRUCT mpq_file_s;

/* map structure for valid blocks and hashes (first seen in warcraft 3
 * archives). */
typedef struct {
    uint32_t
        block_table_indices; /* real mapping for file number to block entry. */
    uint32_t block_table_diff; /* block table difference between valid blocks
                                  and invalid blocks before. */
