...
extern my_bool bitmap_init(MY_BITMAP*     map,
                           my_bitmap_map* buf,
                           uint           n_bits,
                           my_bool        thread_safe);
...
#define _bitmap_is_set(MAP, BIT)                                               \
    (uint)(((uchar*)(MAP)->bitmap)[(BIT) / 8] & (1 << ((BIT) & 7)))
/*
  WARNING!
...
static inline uint bitmap_is_set(const MY_BITMAP* map, uint bit)
{
    DBUG_ASSERT(bit < (map)->n_bits);
...
static inline my_bool bitmap_cmp(const MY_BITMAP* map1, const MY_BITMAP* map2)
{
    ...
    *(map2)->last_word_ptr |= (map2)->last_word_mask;
    return memcmp((map1)->bitmap,
                  (map2)->bitmap,
                  4 * no_words_in_map((map1))) == 0;
}
