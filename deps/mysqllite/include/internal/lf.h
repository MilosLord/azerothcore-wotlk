...
#define nolock_wrap(f, t, proto_args, args)                                    \
    t _##f            proto_args;                                              \
    static inline t f proto_args { return _##f args; }

#define nolock_wrap_void(f, proto_args, args)                                  \
    void _##f            proto_args;                                           \
    static inline void f proto_args { _##f args; }
...
nolock_wrap(lf_dynarray_value,
            ...
            (array, idx)) lock_wrap(lf_dynarray_lvalue,
                                    void*,
                                    (LF_DYNARRAY * array, uint idx),
                                    (array, idx),
                                    &array->lock)
    nolock_wrap(lf_dynarray_iterate,
                int,
...
lock_wrap(lf_pinbox_get_pins,
          ...
          (LF_PINBOX * pinbox),
          (pinbox),
          &pinbox->pinarray.lock) lock_wrap_void(lf_pinbox_put_pins,
                                                 (LF_PINS * pins),
                                                 (pins),
...
lock_wrap(
    lf_alloc_new, void*, (LF_PINS * pins), (pins), &pins->pinbox->pinarray.lock)
