...
extern void    _db_enter_(const char*              _func_,
                          const char*              _file_,
                          uint                     _line_,
...
extern void _db_dump_(uint                 _line_,
                      const char*          keyword,
...
#define DBUG_CHECK_CRASH(func, op)                                             \
    do {                                                                       \
        char _dbuf_[255];                                                      \
        strxnmov(_dbuf_, sizeof(_dbuf_) - 1, (func), (op));                    \
        DBUG_EXECUTE_IF(_dbuf_, DBUG_ABORT());                                 \
    } while (0)
#define DBUG_CRASH_ENTER(func)                                                 \
    DBUG_ENTER(func);                                                          \
    DBUG_CHECK_CRASH(func, "_crash_enter")
