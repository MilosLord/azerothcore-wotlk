...
#define mysql_file_create_with_symlink(K, P1, P2, P3, P4, P5)                  \
    inline_mysql_file_create_with_symlink(                                     \
        K, __FILE__, __LINE__, P1, P2, P3, P4, P5)
#else
#define mysql_file_create_with_symlink(K, P1, P2, P3, P4, P5)                  \
    ...
    MYSQL_FILE* file)
{
    ...
    if (likely(PSI_server && file->m_psi)) {
        locker = PSI_server->get_thread_file_stream_locker(
            &state, file->m_psi, PSI_FILE_READ);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(
                locker, (size_t)size, src_file, src_line);
    }
#endif
    ...
    if (likely(that != NULL)) {
        that->m_psi = NULL;
        {
            ...
            if (likely(PSI_server != NULL)) {
                ...
                if (likely(locker != NULL))
                    that->m_psi = PSI_server->start_file_open_wait(
    ...
    myf         flags)
{
    ...
    if (likely(file != NULL)) {
        ...
        if (likely(PSI_server && file->m_psi)) {
            ...
            if (likely(locker != NULL))
                PSI_server->start_file_wait(
                    locker, (size_t)0, src_file, src_line);
        }
#endif
...
#endif
    file = my_create_with_symlink(
        linkname, filename, create_flags, access_flags, flags);
#ifdef HAVE_PSI_INTERFACE
    if (likely(locker != NULL))
        PSI_server->end_file_open_wait_and_bind_to_descriptor(locker, file);
#endif
