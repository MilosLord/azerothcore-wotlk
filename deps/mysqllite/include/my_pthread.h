...
typedef union {
    ...
    struct {
        ...
        enum { SIGNAL = 0, BROADCAST = 1, MAX_EVENTS = 2 } EVENTS;
...
#define cmp_timespec(TS1, TS2)                                                 \
    ((TS1.tv.i64 > TS2.tv.i64) ? 1 : ((TS1.tv.i64 < TS2.tv.i64) ? -1 : 0))
...
#if !defined(HAVE_SIGWAIT) && !defined(HAVE_rts_threads) &&                    \
    !defined(sigwait) && !defined(alpha_linux_port) &&                         \
    !defined(HAVE_NONPOSIX_SIGWAIT) && !defined(HAVE_DEC_3_2_THREADS) &&       \
    !defined(_AIX)
...
#define pthread_detach_this_thread()                                           \
    {                                                                          \
     ...
     defined(HAVE_DEC_3_2_THREADS)) &&                                         \
    !defined(HAVE_CTHREADS_WRAPPER)
/* This is set on AIX_3_2 and Siemens unix (and DEC OSF/1 3.2 too) */
#define pthread_key_create(A, B)                                               \
    pthread_keycreate(A,                                                       \
                      (B) ? (pthread_destructor_t)(B)                          \
...
#if !defined(HAVE_PTHREAD_YIELD_ONE_ARG) &&                                    \
    !defined(HAVE_PTHREAD_YIELD_ZERO_ARG)
...
#define set_timespec_nsec(ABSTIME, NSEC)                                       \
    {                                                                          \
        ...
        (ABSTIME).tv_nsec =                                                    \
            (long)(now % ULL(10000000) * 100 + ((NSEC) % 100));                \
    }
#endif /* !set_timespec_nsec */
...
#define cmp_timespec(TS1, TS2)                                                 \
    ((TS1.ts_sec > TS2.ts_sec ||                                               \
      (TS1.ts_sec == TS2.ts_sec && TS1.ts_nsec > TS2.ts_nsec))                 \
         ? 1                                                                   \
         : ((TS1.ts_sec < TS2.ts_sec ||                                        \
             (TS1.ts_sec == TS2.ts_sec && TS1.ts_nsec < TS2.ts_nsec))          \
                ? -1                                                           \
...
#define pthread_cond_timedwait(A, B, C)                                        \
    safe_cond_timedwait((A), (B), (C), __FILE__, __LINE__)
#define pthread_mutex_trylock(A) safe_mutex_lock((A), TRUE, __FILE__, __LINE__)
#define pthread_mutex_t safe_mutex_t
#define safe_mutex_assert_owner(mp)                                            \
    DBUG_ASSERT((mp)->count > 0 && pthread_equal(pthread_self(), (mp)->thread))
#define safe_mutex_assert_not_owner(mp)                                        \
    DBUG_ASSERT(!(mp)->count || !pthread_equal(pthread_self(), (mp)->thread))
#else
#define safe_mutex_assert_owner(mp)
#define safe_mutex_assert_not_owner(mp)
#endif /* SAFE_MUTEX */
...
#define pthread_cond_timedwait(A, B, C)                                        \
    pthread_cond_timedwait((A), &(B)->mutex, (C))
...
#define rw_pr_lock_assert_not_write_owner(A)                                   \
    DBUG_ASSERT(!(A)->active_writer ||                                         \
                !pthread_equal(pthread_self(), (A)->writer_thread))
#else
#define rw_pr_lock_assert_write_owner(A)
#define rw_pr_lock_assert_not_write_owner(A)
#endif /* SAFE_MUTEX */
...
#define my_rw_lock_assert_write_owner(A)                                       \
    DBUG_ASSERT((A)->state == -1 &&                                            \
                pthread_equal(pthread_self(), (A)->write_thread))
#define my_rw_lock_assert_not_write_owner(A)                                   \
    DBUG_ASSERT((A)->state >= 0 ||                                             \
                !pthread_equal(pthread_self(), (A)->write_thread))
#else
#define my_rw_lock_assert_write_owner(A)
#define my_rw_lock_assert_not_write_owner(A)
...
#define thr_setconcurrency(A) pthread_dummy(0)
    ...
    !defined(pthread_attr_setstacksize)
#define pthread_attr_setstacksize(A, B) pthread_dummy(0)
#endif
