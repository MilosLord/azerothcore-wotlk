...
///
/// In order for A* searches to work properly, the cost should be proportional
/// to the travel distance. Implementing a cost modifier less than 1.0 is likely
...
{
    return (poly->flags & m_includeFlags) != 0 &&
           (poly->flags & m_excludeFlags) == 0;
}
...
inline float dtQueryFilter::getCost(const float* pa,
                                    const float* pb,
                                    const dtPolyRef /*prevRef*/,
                                    const dtMeshTile* /*prevTile*/,
                                    const dtPoly* /*prevPoly*/,
...
/// A portal may be treated as a wall based on the dtQueryFilter used for a
/// query.
///
/// @see dtNavMesh, dtQueryFilter, #dtAllocNavMeshQuery(),
/// #dtAllocNavMeshQuery()

dtNavMeshQuery::dtNavMeshQuery()
    : m_nav(0), m_tinyNodePool(0), m_nodePool(0), m_openList(0)
{
...
dtStatus dtNavMeshQuery::init(const dtNavMesh* nav, const int maxNodes)
{
    ...
    if (!m_nodePool || m_nodePool->getMaxNodes() < maxNodes) {
        ...
        m_nodePool = new (dtAlloc(sizeof(dtNodePool), DT_ALLOC_PERM))
            dtNodePool(maxNodes, dtNextPow2(maxNodes / 4));
        if (!m_nodePool)
    ...
    if (!m_tinyNodePool) {
        m_tinyNodePool =
            new (dtAlloc(sizeof(dtNodePool), DT_ALLOC_PERM)) dtNodePool(64, 32);
        if (!m_tinyNodePool)
            return DT_FAILURE | DT_OUT_OF_MEMORY;
    }
    else {
    ...
    if (!m_openList || m_openList->getCapacity() < maxNodes) {
        ...
        m_openList = new (dtAlloc(sizeof(dtNodeQueue), DT_ALLOC_PERM))
            dtNodeQueue(maxNodes);
        if (!m_openList)
            return DT_FAILURE | DT_OUT_OF_MEMORY;
    }
    else {
                                         ...
                                         float*     randomPt) const
{
    ...
    // Randomly pick one tile. Assume that all tiles cover roughly the same
    // area.
...
dtStatus
dtNavMeshQuery::findRandomPointAroundCircle(dtPolyRef            startRef,
                                            ...
                                            float*     randomPt) const
{
    ...
    // Validate input
    if (!m_nav->isValidPolyRef(startRef) || !centerPos ||
        !dtVisfinite(centerPos) || maxRadius < 0 ||
        !dtMathIsfinite(maxRadius) || !filter || !frand || !randomRef ||
        !randomPt) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    while (!m_openList->empty()) {
        ...
        for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK;
            ...
            const float total =
                bestNode->total + dtVdist(bestNode->pos, neighbourNode->pos);
...
///
/// @p pos does not have to be within the bounds of the polygon or navigation
/// mesh.
///
...
/// below), then @p pos and @p closest will be equal.
///
/// The height of @p closest will be the polygon boundary.  The height detail is
/// not used.
///
/// @p pos does not have to be within the bounds of the polybon or the
/// navigation mesh.
///
...
/// @par
///
/// Will return #DT_FAILURE | DT_INVALID_PARAM if the provided position is
/// outside the xz-bounds of the polygon.
///
...
public:
    dtFindNearestPolyQuery(const dtNavMeshQuery* query, const float* center)
        : m_query(query), m_center(center), m_nearestDistanceSqr(FLT_MAX),
          m_nearestRef(0), m_nearestPoint()
    {
    }
    ...
    void
    process(const dtMeshTile* tile, dtPoly** polys, dtPolyRef* refs, int count)
    {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            float     d;
            m_query->closestPointOnPoly(
                ref, m_center, closestPtPoly, &posOverPoly);
                                         ...
                                         dtPolyQuery*         query) const
{
    ...
    if (tile->bvTree) {
        ...
        while (node < end) {
            const bool overlap =
                dtOverlapQuantBounds(bmin, bmax, node->bmin, node->bmax);
...
public:
    dtCollectPolysQuery(dtPolyRef* polys, const int maxPolys)
        : m_polys(polys), m_maxPolys(maxPolys), m_numCollected(0),
          m_overflow(false)
    {
    }
    ...
    process(const dtMeshTile* tile, dtPoly** polys, dtPolyRef* refs, int count)
    {
        ...
        memcpy(
            m_polys + m_numCollected, refs, (size_t)toCopy * sizeof(dtPolyRef));
                                       ...
                                       const int            maxPolys) const
{
    ...
    *polyCount = collector.numCollected();
    return collector.overflowed() ? DT_SUCCESS | DT_BUFFER_TOO_SMALL
                                  : DT_SUCCESS;
}
...
/// to the query have bounding boxes that overlap with the center and
/// halfExtents passed to this function. The dtPolyQuery::process function is
/// invoked multiple times until all overlapping polygons have been processed.
///
dtStatus dtNavMeshQuery::queryPolygons(const float*         center,
                                       const float*         halfExtents,
                                       const dtQueryFilter* filter,
                                       dtPolyQuery*         query) const
{
    ...
    if (!center || !dtVisfinite(center) || !halfExtents ||
        !dtVisfinite(halfExtents) || !filter || !query) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
                                  ...
                                  const int            maxPath) const
{
    ...
    *pathCount = 0;
    ...
    if (!m_nav->isValidPolyRef(startRef) || !m_nav->isValidPolyRef(endRef) ||
        !startPos || !dtVisfinite(startPos) || !endPos ||
        !dtVisfinite(endPos) || !filter || !path || maxPath <= 0) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    while (!m_openList->empty()) {
        ...
        for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK;
            ...
            dtNode* neighbourNode =
                m_nodePool->getNode(neighbourRef, crossSide);
            if (!neighbourNode) {
                outOfNodes = true;
                continue;
            }
            ...
            if (neighbourRef == endRef) {
                // Cost
                const float curCost = filter->getCost(bestNode->pos,
                                                      neighbourNode->pos,
                                                      parentRef,
                                                      parentTile,
                                                      parentPoly,
                                                      ...
                                                      neighbourTile,
                                                      neighbourPoly);
                const float endCost = filter->getCost(neighbourNode->pos,
                                                      endPos,
                                                      bestRef,
                                                      bestTile,
                                                      bestPoly,
            ...
            if ((neighbourNode->flags & DT_NODE_OPEN) &&
                total >= neighbourNode->total)
                continue;
            // The node is already visited and process, and the new result is
            // worse, skip.
            if ((neighbourNode->flags & DT_NODE_CLOSED) &&
                total >= neighbourNode->total)
                continue;
...
dtStatus dtNavMeshQuery::getPathToNode(dtNode*    endNode,
                                       dtPolyRef* path,
                                       int*       pathCount,
                                       int        maxPath) const
{
    ...
    // If the path cannot be fully stored then advance to the last node we will
    // be able to store.
    curNode = endNode;
    int writeCount;
                                            ...
                                            const unsigned int   options)
{
    ...
    if (options & DT_FINDPATH_ANY_ANGLE) {
        ...
        // It is not sensitive so it is enough to compute it from the first
        // tile.
        const dtMeshTile* tile        = m_nav->getTileByRef(startRef);
        float             agentRadius = tile->header->walkableRadius;
        m_query.raycastLimitSqr =
            dtSqr(agentRadius * DT_RAY_CAST_LIMIT_PROPORTIONS);
    }
...
dtStatus dtNavMeshQuery::updateSlicedFindPath(const int maxIter, int* doneIters)
{
    ...
    if (!m_nav->isValidPolyRef(m_query.startRef) ||
        !m_nav->isValidPolyRef(m_query.endRef)) {
        m_query.status = DT_FAILURE;
        return DT_FAILURE;
    }
    ...
    while (iter < maxIter && !m_openList->empty()) {
        ...
        const dtPoly*     bestPoly = 0;
        if (dtStatusFailed(
                m_nav->getTileAndPolyByRef(bestRef, &bestTile, &bestPoly))) {
        ...
        if (parentRef) {
            bool invalidParent = dtStatusFailed(m_nav->getTileAndPolyByRef(
                parentRef, &parentTile, &parentPoly));
            if (invalidParent ||
                (grandpaRef && !m_nav->isValidPolyRef(grandpaRef))) {
                // The polygon has disappeared during the sliced query, fail.
                m_query.status = DT_FAILURE;
                if (doneIters)
                    *doneIters = iter;
        ...
        if (m_query.options & DT_FINDPATH_ANY_ANGLE) {
            if ((parentRef != 0) &&
                (dtVdistSqr(parentNode->pos, bestNode->pos) <
                 m_query.raycastLimitSqr))
                tryLOS = true;
        }
        ...
        for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK;
            ...
            // do not expand to nodes that were already visited from the same
            // parent
            if (neighbourNode->pidx != 0 &&
            ...
            if (foundShortCut) {
                ...
                // No shortcut found.
                const float curCost =
                    m_query.filter->getCost(bestNode->pos,
                                            neighbourNode->pos,
                                            parentRef,
                                            parentTile,
                                            parentPoly,
            ...
            if (neighbourRef == m_query.endRef) {
                const float endCost =
                    m_query.filter->getCost(neighbourNode->pos,
                                            m_query.endPos,
                                            bestRef,
                                            bestTile,
                                            bestPoly,
                ...
                heuristic =
                    dtVdist(neighbourNode->pos, m_query.endPos) * H_SCALE;
            }
            ...
            // Add or update the node.
            neighbourNode->pidx = foundShortCut
                                      ? bestNode->pidx
                                      : m_nodePool->getNodeIdx(bestNode);
            neighbourNode->id   = neighbourRef;
            neighbourNode->flags =
                (neighbourNode->flags &
                 ~(DT_NODE_CLOSED | DT_NODE_PARENT_DETACHED));
            neighbourNode->cost  = cost;
            neighbourNode->total = total;
            if (foundShortCut)
                neighbourNode->flags =
                    (neighbourNode->flags | DT_NODE_PARENT_DETACHED);
    ...
    *pathCount = 0;
    ...
    if (m_query.startRef == m_query.endRef) {
        ...
        do {
            ...
            int nextRay =
                node->flags &
                DT_NODE_PARENT_DETACHED; // keep track of whether parent is not
                                         // adjacent (i.e. due to raycast
                                         // shortcut)
            node->flags = (node->flags & ~DT_NODE_PARENT_DETACHED) |
        ...
        do {
            ...
            if (node->flags & DT_NODE_PARENT_DETACHED) {
                ...
                status = raycast(node->id,
                                 ...
                                 maxPath - n);
                n += m;
                // raycast ends on poly boundary and the path might include the
                // next poly boundary.
                if (path[n - 1] == next->id)
...
dtStatus
dtNavMeshQuery::finalizeSlicedFindPathPartial(const dtPolyRef* existing,
                                      ...
                                      const int           maxStraightPath) const
{
    if ((*straightPathCount) > 0 &&
        dtVequal(&straightPath[((*straightPathCount) - 1) * 3], pos)) {
                                       ...
                                       const int        options) const
{
    ...
    for (int i = startIdx; i < endIdx; i++) {
        ...
        if (dtStatusFailed(
                m_nav->getTileAndPolyByRef(from, &fromTile, &fromPoly)))
            return DT_FAILURE | DT_INVALID_PARAM;
        ...
        if (dtStatusFailed(getPortalPoints(
                from, fromPoly, fromTile, to, toPoly, toTile, left, right)))
            break;
                                          ...
                                          const int        options) const
{
    ...
    *straightPathCount = 0;
    ...
    if (!startPos || !dtVisfinite(startPos) || !endPos ||
        !dtVisfinite(endPos) || !path || pathSize <= 0 || !path[0] ||
        maxStraightPath <= 0) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    if (dtStatusFailed(closestPointOnPolyBoundary(
            path[pathSize - 1], endPos, closestEndPos)))
        return DT_FAILURE | DT_INVALID_PARAM;

    // Add start point.
    stat = appendVertex(closestStartPos,
        ...
        for (int i = 0; i < pathSize; ++i) {
            ...
            if (i + 1 < pathSize) {
                ...
                if (dtStatusFailed(getPortalPoints(
                    ...
                    // Failed to get portal points, in practice this means that
                    // path[i+1] is invalid polygon. Clamp the end point to
                    // path[i], and return the path so far.

                    if (dtStatusFailed(closestPointOnPolyBoundary(
                            path[i], endPos, closestEndPos))) {
                        // This should only happen when the first polygon is
                        // invalid.
                        return DT_FAILURE | DT_INVALID_PARAM;
                    }
                    ...
                    if (options & (DT_STRAIGHTPATH_AREA_CROSSINGS |
                        ...
                        // Ignore status return value as we're just about to
                        // return anyway.
                        appendPortals(apexIndex,
                                      i,
                                      closestEndPos,
                                      path,
                                      straightPath,
                ...
                if (i == 0) {
                    float t;
                    if (dtDistancePtSegSqr2D(portalApex, left, right, t) <
                        dtSqr(0.001f))
                        continue;
                }
            }
                                          ...
                                          const int maxVisitedSize) const
{
    ...
    *visitedCount = 0;
    ...
    if (!m_nav->isValidPolyRef(startRef) || !startPos ||
        !dtVisfinite(startPos) || !endPos || !dtVisfinite(endPos) || !filter ||
        !resultPos || !visited || maxVisitedSize <= 0) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    while (nstack) {
        ...
        for (int i = 0, j = (int)curPoly->vertCount - 1;
             i < (int)curPoly->vertCount;
             j = i++) {
            ...
            if (curPoly->neis[j] & DT_EXT_LINK) {
                // Tile border.
                for (unsigned int k = curPoly->firstLink; k != DT_NULL_LINK;
            ...
            if (!nneis) {
                ...
                float        tseg;
                const float  distSqr =
                    dtDistancePtSegSqr2D(endPos, vj, vi, tseg);
                if (distSqr < bestDist) {
                ...
                for (int k = 0; k < nneis; ++k) {
                    ...
                    float        tseg;
                    float        distSqr =
                        dtDistancePtSegSqr2D(searchPos, vj, vi, tseg);
                    ...
                    if (nstack < MAX_STACK) {
                        neighbourNode->pidx =
                            m_tinyNodePool->getNodeIdx(curNode);
                                         ...
                                         float*            right) const
{
    ...
    const dtLink* link = 0;
    for (unsigned int i = fromPoly->firstLink; i != DT_NULL_LINK;
    ...
    if (toPoly->getType() == DT_POLYTYPE_OFFMESH_CONNECTION) {
        for (unsigned int i = toPoly->firstLink; i != DT_NULL_LINK;
    ...
    if (link->side != 0xff) {
        ...
        if (link->bmin != 0 || link->bmax != 255) {
            ...
            dtVlerp(
                left, &fromTile->verts[v0 * 3], &fromTile->verts[v1 * 3], tmin);
            dtVlerp(right,
                    &fromTile->verts[v0 * 3],
                    &fromTile->verts[v1 * 3],
                    tmax);
...
dtStatus
dtNavMeshQuery::getEdgeMidPoint(dtPolyRef from, dtPolyRef to, float* mid) const
{
    float         left[3], right[3];
    unsigned char fromType, toType;
    if (dtStatusFailed(
            getPortalPoints(from, to, left, right, fromType, toType)))
        return DT_FAILURE | DT_INVALID_PARAM;
...
/// (no wall hit), meaning it reached the end position. This is one example of
/// why this method is meant for short distance checks.
///
                                 ...
                                 dtPolyRef            prevRef) const
{
    ...
    while (curRef) {
        ...
        float tmin, tmax;
        int   segMin, segMax;
        if (!dtIntersectSegmentPoly2D(
                startPos, endPos, verts, nv, tmin, tmax, segMin, segMax)) {
        ...
        if (segMax == -1) {
            ...
            if (options & DT_RAYCAST_USE_COSTS)
                hit->pathCost += filter->getCost(curPos,
                                                 endPos,
                                                 prevRef,
                                                 prevTile,
                                                 prevPoly,
        ...
        if (options & DT_RAYCAST_USE_COSTS) {
            ...
            float s   = dtSqr(eDir[0]) > dtSqr(eDir[2]) ? diff[0] / eDir[0]
                                                        : diff[2] / eDir[2];
            curPos[1] = e1[1] + eDir[1] * s;

            hit->pathCost += filter->getCost(lastPos,
                                             curPos,
                                             prevRef,
                                             prevTile,
                                             prevPoly,
...
///
/// If the result arrays are to small to hold the entire result set, they will
                                               ...
                                               const int  maxResult) const
{
    ...
    *resultCount = 0;
    ...
    if (!m_nav->isValidPolyRef(startRef) || !centerPos ||
        !dtVisfinite(centerPos) || radius < 0 || !dtMathIsfinite(radius) ||
        !filter || maxResult < 0) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    while (!m_openList->empty()) {
        ...
        for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK;
            ...
            float cost = filter->getCost(bestNode->pos,
                                         neighbourNode->pos,
                                         parentRef,
                                         parentTile,
                                         parentPoly,
                                              ...
                                              const int maxResult) const
{
    ...
    *resultCount = 0;
    ...
    if (!m_nav->isValidPolyRef(startRef) || !verts || nverts < 3 || !filter ||
        maxResult < 0) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    while (!m_openList->empty()) {
        ...
        for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK;
            ...
            // If the poly is not touching the edge to the next polygon, skip
            // the connection it.
            float tmin, tmax;
            int   segMin, segMax;
            if (!dtIntersectSegmentPoly2D(
                    va, vb, verts, nverts, tmin, tmax, segMin, segMax))
                continue;
...
/// @par
///
/// This method is optimized for a small search radius and small number of
/// result polygons.
///
                                                ...
                                                const int  maxResult) const
{
    ...
    *resultCount = 0;
    ...
    while (nstack) {
        ...
        for (unsigned int i = curPoly->firstLink; i != DT_NULL_LINK;
            ...
            for (int k = 0; k < npa; ++k)
                dtVcopy(&pa[k * 3],
                        &neighbourTile->verts[neighbourPoly->verts[k] * 3]);
            ...
            for (int j = 0; j < n; ++j) {
                ...
                for (int k = 0; k < npb; ++k)
                    dtVcopy(&pb[k * 3],
                            &pastTile->verts[pastPoly->verts[k] * 3]);
                           ...
                           const dtPolyRef ref)
{
    ...
    if (nints - idx)
        memmove(
            ints + idx + 1, ints + idx, sizeof(dtSegInterval) * (nints - idx));
    // Store
    ints[idx].ref  = ref;
    ints[idx].tmin = tmin;
    ints[idx].tmax = tmax;
...
/// @par
///
/// If the @p segmentRefs parameter is provided, then all polygon segments will
/// be returned. Otherwise only the wall segments are returned.
///
/// A segment that is normally a portal will be included in the result set as a
/// wall if the @p filter results in the neighbor polygon becoomming impassable.
///
/// The @p segmentVerts and @p segmentRefs buffers should normally be sized for
/// the maximum segments per polygon of the source navigation mesh.
///
                                             ...
                                             const int maxSegments) const
{
    ...
    *segmentCount = 0;
    ...
    for (int i = 0, j = (int)poly->vertCount - 1; i < (int)poly->vertCount;
         j = i++) {
        ...
        if (poly->neis[j] & DT_EXT_LINK) {
            ...
            // If the edge leads to another polygon and portals are not stored,
            // skip.
                                            ...
                                            float* hitNormal) const
{
    ...
    if (!m_nav->isValidPolyRef(startRef) || !centerPos ||
        !dtVisfinite(centerPos) || maxRadius < 0 ||
        !dtMathIsfinite(maxRadius) || !filter || !hitDist || !hitPos ||
        !hitNormal) {
        return DT_FAILURE | DT_INVALID_PARAM;
    }
    ...
    while (!m_openList->empty()) {
        ...
        // Hit test walls.
        for (int i = 0, j = (int)bestPoly->vertCount - 1;
             i < (int)bestPoly->vertCount;
             j = i++) {
            // Skip non-solid edges.
            if (bestPoly->neis[j] & DT_EXT_LINK) {
                // Tile border.
                bool solid = true;
                for (unsigned int k = bestPoly->firstLink; k != DT_NULL_LINK;
        ...
        for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK;
            ...
            const float* vb =
                &bestTile->verts[bestPoly->verts[(link->edge + 1) %
                                                 bestPoly->vertCount] *
                                 3];
            float tseg;
            float distSqr = dtDistancePtSegSqr2D(centerPos, va, vb, tseg);
