...
bool dtPointInPolygon(const float* pt, const float* verts, const int nverts)
{
    ...
    for (i = 0, j = nverts - 1; i < nverts; j = i++) {
        ...
        if (((vi[2] > pt[2]) != (vj[2] > pt[2])) &&
            (pt[0] <
             (vj[0] - vi[0]) * (pt[2] - vi[2]) / (vj[2] - vi[2]) + vi[0]))
            c = !c;
    }
    return c;
}
...
bool dtDistancePtPolyEdgesSqr(
    const float* pt, const float* verts, const int nverts, float* ed, float* et)
{
    // TODO: Replace pnpoly with triArea2D tests?
    int  i, j;
    bool c = false;
...
inline bool overlapRange(const float amin,
                         const float amax,
                         const float bmin,
                         const float bmax,
                         const float eps)
...
inline float vperpXZ(const float* a, const float* b)
{
