...
///	 @param[in]		t		The interpolation factor. [Limits: 0 <= value
///<= 1.0]
inline void
dtVlerp(float* dest, const float* v1, const float* v2, const float t)
{
...
/// Derives the square of the distance between the specified points on the
/// xz-plane.
///  @param[in]		v1	A point. [(x, y, z)]
///  @param[in]		v2	A point. [(x, y, z)]
...
{
    bool result =
        dtMathIsfinite(v[0]) && dtMathIsfinite(v[1]) && dtMathIsfinite(v[2]);
...
/// Derives the signed xz-plane area of the triangle ABC, or the relationship of
/// line AB to point C.
...
/// @see dtOverlapBounds
inline bool dtOverlapQuantBounds(const unsigned short amin[3],
...
inline bool dtOverlapBounds(const float* amin,
                            const float* amax,
                            const float* bmin,
                            const float* bmax)
{
...
bool dtClosestHeightPointTriangle(
    const float* p, const float* a, const float* b, const float* c, float& h);
...
{
    TypeToRetrieveAs* returnPointer =
        reinterpret_cast<TypeToRetrieveAs*>(buffer);
...
This is a low cost function than can be used for various purposes.  Its main
purpose is for point/line relationship testing.

In all cases: A value of zero indicates that all vertices are collinear or
represent the same point. (On the xz-plane.)
...
A positive value indicates that point C is to the left of line AB, looking from
A toward B.<br/> A negative value indicates that point C is to the right of
lineAB, looking from A toward B.

When used for evaluating a triangle:

The absolute value of the return value is two times the area of the triangle
when it is projected onto the xz-plane.
...
<ul>
<li>The vertices are reverse wrapped. (Wrapped opposite the normal Detour wrap
direction.)</li> <li>The triangle's 3D face normal is in the general down
direction.</li>
</ul>
