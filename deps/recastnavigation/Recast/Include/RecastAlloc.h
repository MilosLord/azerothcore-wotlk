...
//  @param[in]		rcAllocHint	A hint to the allocator on how long the memory
//  is expected to be in use.
//  @return A pointer to the beginning of the allocated memory block, or null if
//  the allocation failed.
///  @see rcAllocSetCustom
typedef void*(rcAllocFunc)(size_t size, rcAllocHint hint);

/// A memory deallocation function.
///  @param[in]		ptr		A pointer to a memory block previously allocated
///  using #rcAllocFunc.
/// @see rcAllocSetCustom
typedef void(rcFreeFunc)(void* ptr);

/// Sets the base custom allocation functions to be used by Recast.
///  @param[in]		allocFunc	The memory allocation function to be used by
///  #rcAlloc
///  @param[in]		freeFunc	The memory de-allocation function to be used by
///  #rcFree
void rcAllocSetCustom(rcAllocFunc* allocFunc, rcFreeFunc* freeFunc);

/// Allocates a memory block.
///  @param[in]		size	The size, in bytes of memory, to allocate.
///  @param[in]		hint	A hint to the allocator on how long the memory is
///  expected to be in use.
///  @return A pointer to the beginning of the allocated memory block, or null
...
/// An implementation of operator new usable for placement new. The default one
/// is part of STL (which we don't use). rcNewTag is a dummy type used to
/// differentiate our operator from the STL one, in case users import both
/// Recast and STL.
struct rcNewTag {};
inline void* operator new(size_t, const rcNewTag&, void* p) { return p; }
inline void  operator delete(void*, const rcNewTag&, void*) {}

/// Signed to avoid warnnings when comparing to int loop indexes, and common
/// error with comparing to zero. MSVC2010 has a bug where ssize_t is unsigned
/// (!!!).
typedef intptr_t rcSizeType;
#define RC_SIZE_MAX INTPTR_MAX

/// Macros to hint to the compiler about the likeliest branch. Please add a
/// benchmark that demonstrates a performance improvement before introducing use
/// cases.
...
/// Variable-sized storage type. Mimics the interface of std::vector<T> with
/// some notable differences:
///  * Uses rcAlloc()/rcFree() to handle storage.
///  * No support for a custom allocator.
///  * Uses signed size instead of size_t to avoid warnings in for loops: "for
///  (int i = 0; i < foo.size(); i++)"
///  * Omits methods of limited utility: insert/erase, (bad performance), at (we
///  don't use exceptions), operator=.
///  * assign() and the pre-sizing constructor follow C++11 semantics -- they
///  don't construct a temporary if no value is provided.
///  * push_back() and resize() support adding values from the current vector.
///  Range-based constructors and assign(begin, end) do not.
...
class rcVectorBase {
    ...
    rcSizeType m_cap;
    T*         m_data;
    // Constructs a T at the give address with either the copy constructor or
    // the default.
    ...
    // Creates an array of the given size, copies all of this vector's data into
    // it, and returns it.
...
public:
    ...
    // Unlike in std::vector, we return a bool to indicate whether the alloc was
    // successful.
    ...
    // Not implemented as resize(0) because resize requires T to be
