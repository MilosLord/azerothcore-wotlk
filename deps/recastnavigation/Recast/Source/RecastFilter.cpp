...
///
/// Two neighboring spans are walkable if: <tt>rcAbs(currentSpan.smax -
/// neighborSpan.smax) < waklableClimb</tt>
///
/// @warning Will override the effect of #rcFilterLedgeSpans.  So if both
/// filters are used, call #rcFilterLedgeSpans after calling this filter.
///
...
/// than @p walkableClimb from the current span's maximum. This method removes
...
///
/// A span is a ledge if: <tt>rcAbs(currentSpan.smax - neighborSpan.smax) >
/// walkableClimb</tt>
///
/// @see rcHeightfield, rcConfig
        ...
        for (int x = 0; x < w; ++x) {
            for (rcSpan* s = solid.spans[x + y * w]; s; s = s->next) {
                ...
                for (int dir = 0; dir < 4; ++dir) {
                    ...
                    // Skip neightbour if the gap between the spans is too
                    // small.
                    ...
                    for (ns = solid.spans[dx + dy * w]; ns; ns = ns->next) {
                        ...
                        // Skip neightbour if the gap between the spans is too
                        // small.
                        if (rcMin(top, ntop) - rcMax(bot, nbot) >
                            walkableHeight) {
                            minh = rcMin(minh, nbot - bot);
