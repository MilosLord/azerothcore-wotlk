...
static bool circumCircle(
    const float* p1, const float* p2, const float* p3, float* c, float& r)
{
    ...
    if (fabsf(cp) > EPS) {
        ...
        c[0]             = (v1Sq * (v2[2] - v3[2]) + v2Sq * (v3[2] - v1[2]) +
                v3Sq * (v1[2] - v2[2])) /
               (2 * cp);
        c[1] = 0;
        c[2] = (v1Sq * (v3[0] - v2[0]) + v2Sq * (v1[0] - v3[0]) +
                v3Sq * (v2[0] - v1[0])) /
               (2 * cp);
        r = vdist2(c, v1);
        rcVadd(c, c, p1);
...
static float
distPtTri(const float* p, const float* a, const float* b, const float* c)
{
                                ...
                                const rcHeightPatch& hp)
{
    ...
    if (h == RC_UNSET_HEIGHT) {
        ...
        for (int i = 0; i < maxIter; i++) {
            ...
            // entrance to the function. The next "ring" contains 8 cells
            // (marked 1 above). Those are all the neighbors to the center cell.
                   ...
                   int        r)
{
    if (nedges >= maxEdges) {
        ctx->log(
            RC_LOG_ERROR, "addEdge: Too many edges (%d/%d).", nedges, maxEdges);
        return EV_UNDEF;
    }
...
static int
overlapSegSeg2d(const float* a, const float* b, const float* c, const float* d)
{
...
static bool
overlapEdges(const float* pts, const int* edges, int nedges, int s1, int t1)
{
    for (int i = 0; i < nedges; ++i) {
        ...
        if (s0 == s1 || s0 == t1 || t0 == s1 || t0 == t1)
            continue;
        if (overlapSegSeg2d(
                &pts[s0 * 3], &pts[t0 * 3], &pts[s1 * 3], &pts[t1 * 3]))
            return true;
    }
    return false;
}
    ...
    for (int u = 0; u < npts; ++u) {
        ...
        if (vcross2(&pts[s * 3], &pts[t * 3], &pts[u * 3]) > EPS) {
            ...
            if (d > r * (1 + tol)) {
                ...
                // Inside epsilon circum circle, do extra tests to make sure the
                // edge is valid. s-u and t-u cannot overlap with s-pt nor t-pt
                // if they exists.
                            ...
                            rcIntArray&  tris)
{
    ...
    for (int i = 0; i < nhull; i++) {
        if (hull[i] >= nin)
            continue; // Ears are triangles with original vertices as middle
                      // vertex while others are actually line segments on edges
        int          pi = prev(i, nhull);
        int          ni = next(i, nhull);
                            ...
                            rcIntArray&                 samples)
{
    ...
    static const int MAX_TRIS =
        255; // Max tris for delaunay is 2n-2-k (n=num verts, k=num hull verts).
    ...
    // If the polygon minimum extent is small (sliver or small triangle), do not
    // try to add internal points.
    ...
    if (tris.size() == 0) {
        // Could not triangulate the poly, make sure there is some valid data
        // there.
        ctx->log(RC_LOG_WARNING,
        ...
        for (int iter = 0; iter < nsamples; ++iter) {
            ...
            for (int i = 0; i < nsamples; ++i) {
                ...
                if (s[3])
                    continue; // skip added.
                float pt[3];
                // The sample location is jittered to get rid of some bad
                // triangulations which are cause by symmetrical data from the
                // grid structure.
                pt[0] = s[0] * sampleDist + getJitterX(i) * cs * 0.1f;
                pt[1] = s[1] * chf.ch;
                pt[2] = s[2] * sampleDist + getJitterY(i) * cs * 0.1f;
                float d =
                    distToTriMesh(pt, verts, nverts, &tris[0], tris.size() / 4);
                if (d < 0)
    ...
    if (ntris > MAX_TRIS) {
        ...
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Shrinking triangle count from %d to "
                 "max %d.",
                 ntris,
                 MAX_TRIS);
    }
                                    ...
                                    rcIntArray&                 array)
{
    ...
    for (int j = 0; j < npoly && dmin > 0; ++j) {
        for (int k = 0; k < 9 && dmin > 0; ++k) {
            ...
            const rcCompactCell& c =
                chf.cells[(ax + bs) + (az + bs) * chf.width];
    ...
    // DFS to move to the center. Note that we need a DFS here and can not just
    // move directly towards the center without recording intermediate nodes,
    // even though the polygons are convex. In very rare we can get stuck due to
    // contour simplification if we do not record nodes.
    int cx = -1, cy = -1, ci = -1;
    while (true) {
        if (array.size() < 3) {
            ctx->log(RC_LOG_WARNING,
                     "Walk towards polygon center failed to reach center");
            break;
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            array.push(newY);
            array.push(
                (int)chf.cells[(newX + bs) + (newY + bs) * chf.width].index +
                rcGetCon(cs, dir));
        }
                    ...
                    if (s.reg == region) {
                        ...
                        for (int dir = 0; dir < 4; ++dir) {
                            if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
                                ...
                                const int ai =
                                    (int)chf.cells[ax + ay * chf.width].index +
                                    rcGetCon(s, dir);
    ...
    while (head * 3 < queue.size()) {
        ...
        if (head >= RETRACT_SIZE) {
            ...
            if (queue.size() > RETRACT_SIZE * 3)
                memmove(&queue[0],
                        &queue[RETRACT_SIZE * 3],
                        sizeof(int) * (queue.size() - RETRACT_SIZE * 3));
        ...
        for (int dir = 0; dir < 4; ++dir) {
            ...
            if ((unsigned int)hx >= (unsigned int)hp.width ||
                (unsigned int)hy >= (unsigned int)hp.height)
                continue;
            ...
            const int ai =
                (int)chf.cells[ax + ay * chf.width].index + rcGetCon(cs, dir);
            const rcCompactSpan& as = chf.spans[ai];
...
static unsigned char getEdgeFlags(const float* va,
                                  ...
                                  const int    npoly)
{
    // The flag returned by this function matches dtDetailTriEdgeFlags in
    // Detour. Figure out if edge (va,vb) is part of the polygon boundary.
...
/// parameters.
///
/// @see rcAllocPolyMeshDetail, rcPolyMesh, rcCompactHeightfield,
/// rcPolyMeshDetail, rcConfig
                           ...
                           rcPolyMeshDetail&           dmesh)
{
    ...
    rcScopedDelete<int> bounds(
        (int*)rcAlloc(sizeof(int) * mesh.npolys * 4, RC_ALLOC_TEMP));
    if (!bounds) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'bounds' (%d).",
                 mesh.npolys * 4);
        return false;
    }
    rcScopedDelete<float> poly(
        (float*)rcAlloc(sizeof(float) * nvp * 3, RC_ALLOC_TEMP));
    if (!poly) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'poly' (%d).",
                 nvp * 3);
        return false;
    }
    ...
    if (!hp.data) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'hp.data' (%d).",
                 maxhw * maxhh);
        return false;
    }
    ...
    dmesh.nverts  = 0;
    dmesh.ntris   = 0;
    dmesh.meshes  = (unsigned int*)rcAlloc(
        sizeof(unsigned int) * dmesh.nmeshes * 4, RC_ALLOC_PERM);
    if (!dmesh.meshes) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'dmesh.meshes' (%d).",
                 dmesh.nmeshes * 4);
        return false;
    }
    ...
    if (!dmesh.verts) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'dmesh.verts' (%d).",
                 vcap * 3);
        return false;
    }
    dmesh.ntris = 0;
    dmesh.tris  = (unsigned char*)rcAlloc(sizeof(unsigned char) * tcap * 4,
                                         RC_ALLOC_PERM);
    if (!dmesh.tris) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' (%d).",
                 tcap * 4);
        return false;
    }
    ...
    for (int i = 0; i < mesh.npolys; ++i) {
        ...
        getHeightData(
            ctx, chf, p, npoly, mesh.verts, borderSize, hp, arr, mesh.regs[i]);

        // Build detail mesh.
        int nverts = 0;
        if (!buildPolyDetail(ctx,
                             poly,
                             npoly,
                             sampleDist,
                             sampleMaxError,
        ...
        if (dmesh.nverts + nverts > vcap) {
            ...
            float* newv =
                (float*)rcAlloc(sizeof(float) * vcap * 3, RC_ALLOC_PERM);
            if (!newv) {
                ctx->log(RC_LOG_ERROR,
        ...
        if (dmesh.ntris + ntris > tcap) {
            while (dmesh.ntris + ntris > tcap)
                tcap += 256;
            unsigned char* newt = (unsigned char*)rcAlloc(
                sizeof(unsigned char) * tcap * 4, RC_ALLOC_PERM);
            if (!newt) {
                ctx->log(RC_LOG_ERROR,
            ...
            if (dmesh.ntris)
                memcpy(
                    newt, dmesh.tris, sizeof(unsigned char) * 4 * dmesh.ntris);
            rcFree(dmesh.tris);
            dmesh.tris = newt;
        }
        for (int j = 0; j < ntris; ++j) {
            ...
            dmesh.tris[dmesh.ntris * 4 + 3] = getTriFlags(&verts[t[0] * 3],
                                                          &verts[t[1] * 3],
                                                          &verts[t[2] * 3],
                                                          poly,
                                                          npoly);
                            ...
                            rcPolyMeshDetail&  mesh)
{
    ...
    mesh.nmeshes = 0;
    mesh.meshes  = (unsigned int*)rcAlloc(sizeof(unsigned int) * maxMeshes * 4,
                                         RC_ALLOC_PERM);
    if (!mesh.meshes) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'pmdtl.meshes' (%d).",
                 maxMeshes * 4);
        return false;
    }

    mesh.ntris = 0;
    mesh.tris  = (unsigned char*)rcAlloc(sizeof(unsigned char) * maxTris * 4,
                                        RC_ALLOC_PERM);
    if (!mesh.tris) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' (%d).",
