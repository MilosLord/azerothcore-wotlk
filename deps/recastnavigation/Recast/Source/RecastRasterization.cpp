...
inline bool overlapBounds(const float* amin,
                          const float* amax,
                          const float* bmin,
                          const float* bmax)
{
...
inline bool overlapInterval(unsigned short amin,
                            unsigned short amax,
                            unsigned short bmin,
                            unsigned short bmax)
{
...
static rcSpan* allocSpan(rcHeightfield& hf)
{
    ...
    if (!hf.freelist || !hf.freelist->next) {
        ...
        rcSpanPool* pool =
            (rcSpanPool*)rcAlloc(sizeof(rcSpanPool), RC_ALLOC_PERM);
        if (!pool)
            return 0;
                       ...
                       int          axis)
{
    ...
    for (int i = 0, j = nin - 1; i < nin; j = i, ++i) {
        ...
        {
            // add the i'th point to the right polygon. Addition is done even
            // for points on the dividing line
            if (d[i] >= 0) {
                rcVcopy(out1 + m * 3, in + i * 3);
                m++;
                if (d[i] != 0)
                         ...
                         const int           flagMergeThr)
{
    ...
    // If the triangle does not touch the bbox of the heightfield, skip the
    // triagle.
    ...
    float  buf[7 * 3 * 4];
    float *in = buf, *inrow = buf + 7 * 3, *p1 = inrow + 7 * 3,
          *p2 = p1 + 7 * 3;
    ...
    for (int y = y0; y <= y1; ++y) {
        ...
        for (int x = x0; x <= x1; ++x) {
            ...
            unsigned short ismin = (unsigned short)rcClamp(
                (int)floorf(smin * ich), 0, RC_SPAN_MAX_HEIGHT);
            unsigned short ismax = (unsigned short)rcClamp(
                (int)ceilf(smax * ich), (int)ismin + 1, RC_SPAN_MAX_HEIGHT);
