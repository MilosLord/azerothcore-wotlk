        ...
        for (int x = 0; x < w; ++x) {
            ...
            for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni;
                ...
                if (rcGetCon(s, 0) != RC_NOT_CONNECTED) {
                    ...
                    const int ai =
                        (int)chf.cells[ax + ay * w].index + rcGetCon(s, 0);
                    ...
                    if (rcGetCon(as, 3) != RC_NOT_CONNECTED) {
                        ...
                        const int aai = (int)chf.cells[aax + aay * w].index +
                                        rcGetCon(as, 3);
                        if (src[aai] + 3 < src[i])
                            src[i] = src[aai] + 3;
                    }
                ...
                if (rcGetCon(s, 3) != RC_NOT_CONNECTED) {
                    ...
                    const int ai =
                        (int)chf.cells[ax + ay * w].index + rcGetCon(s, 3);
                    const rcCompactSpan& as = chf.spans[ai];
                    if (src[ai] + 2 < src[i])
                        src[i] = src[ai] + 2;
        ...
        for (int x = w - 1; x >= 0; --x) {
            ...
            for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni;
                ...
                if (rcGetCon(s, 2) != RC_NOT_CONNECTED) {
                    ...
                    const int ai =
                        (int)chf.cells[ax + ay * w].index + rcGetCon(s, 2);
                    const rcCompactSpan& as = chf.spans[ai];
                    if (src[ai] + 2 < src[i])
                        src[i] = src[ai] + 2;
                ...
                if (rcGetCon(s, 1) != RC_NOT_CONNECTED) {
                    ...
                    const int ai =
                        (int)chf.cells[ax + ay * w].index + rcGetCon(s, 1);
                    const rcCompactSpan& as = chf.spans[ai];
                    if (src[ai] + 2 < src[i])
                        src[i] = src[ai] + 2;
    ...
    while (stack.size() > 0) {
        ...
        for (int dir = 0; dir < 4; ++dir) {
            ...
            if (rcGetCon(cs, dir) != RC_NOT_CONNECTED) {
                ...
                const int ai =
                    (int)chf.cells[ax + ay * w].index + rcGetCon(cs, dir);
                ...
                if (rcGetCon(as, dir2) != RC_NOT_CONNECTED) {
                    const int ax2 = ax + rcGetDirOffsetX(dir2);
                    const int ay2 = ay + rcGetDirOffsetY(dir2);
                    const int ai2 = (int)chf.cells[ax2 + ay2 * w].index +
                                    rcGetCon(as, dir2);
    ...
    while (stack.size() > 0) {
        ...
        for (int j = 0; j < stack.size(); j++) {
            ...
            for (int dir = 0; dir < 4; ++dir) {
                ...
                const int ay = y + rcGetDirOffsetY(dir);
                const int ai =
                    (int)chf.cells[ax + ay * w].index + rcGetCon(s, dir);
                if (chf.areas[ai] != area)
                    continue;
...
static void
sortCellsByLevel(unsigned short                 startLevel,
                 rcCompactHeightfield&          chf,
                 const unsigned short*          srcReg,
                 unsigned int                   nbStacks,
                 rcTempVector<LevelStackEntry>* stacks,
                 unsigned short loglevelsPerStack) // the levels per stack (2 in
                                                   // our case) as a bit shift
{
    const int w = chf.width;
    const int h = chf.height;
...
struct rcRegion {
    inline rcRegion(unsigned short i)
        : spanCount(0), id(i), areaType(0), remap(false), visited(false),
          overlap(false), connectsToBorder(false), ymin(0xffff), ymax(0)
    {
    }
                        ...
                        rcIntArray&           cont)
{
    ...
    if (rcGetCon(ss, dir) != RC_NOT_CONNECTED) {
        ...
        const int ay = y + rcGetDirOffsetY(dir);
        const int ai =
            (int)chf.cells[ax + ay * chf.width].index + rcGetCon(ss, dir);
        curReg = srcReg[ai];
    }
    cont.push(curReg);
                                  ...
                                  rcIntArray&           overlaps)
{
    ...
    if (!regions.reserve(nreg)) {
        ctx->log(RC_LOG_ERROR,
                 "mergeAndFilterRegions: Out of memory 'regions' (%d).",
                 nreg);
        return false;
    }
    ...
    do {
        ...
        for (int i = 0; i < nreg; ++i) {
            ...
            if (reg.spanCount > mergeRegionSize &&
                                       ...
                                       unsigned short*       srcReg)
{
    ...
    if (!regions.reserve(nreg)) {
        ctx->log(RC_LOG_ERROR,
                 "mergeAndFilterLayerRegions: Out of memory 'regions' (%d).",
                 nreg);
        return false;
    }
    for (int i = 0; i < nreg; ++i)
    ...
    for (int i = 1; i < nreg; ++i) {
        ...
        while (stack.size() > 0) {
            ...
            for (int j = 0; j < ncons; ++j) {
                ...
                regn.spanCount = 0;
                root.connectsToBorder =
                    root.connectsToBorder || regn.connectsToBorder;
            }
        }
    ...
    for (int i = 0; i < nreg; ++i) {
        if (regions[i].spanCount > 0 && regions[i].spanCount < minRegionArea &&
...
/// rcCompactHeightfield::maxDistance and rcCompactHeightfield::dist fields.
///
...
bool rcBuildDistanceField(rcContext* ctx, rcCompactHeightfield& chf)
{
    ...
    unsigned short* src = (unsigned short*)rcAlloc(
        sizeof(unsigned short) * chf.spanCount, RC_ALLOC_TEMP);
    if (!src) {
        ctx->log(RC_LOG_ERROR,
    ...
    unsigned short* dst = (unsigned short*)rcAlloc(
        sizeof(unsigned short) * chf.spanCount, RC_ALLOC_TEMP);
    if (!dst) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildDistanceField: Out of memory 'dst' (%d).",
                 chf.spanCount);
        rcFree(src);
        return false;
    }
...
///
/// Partitioning can result in smaller than necessary regions. @p
...
///
/// @see rcCompactHeightfield, rcCompactSpan, rcBuildDistanceField,
/// rcBuildRegionsMonotone, rcConfig
                            ...
                            const int             mergeRegionArea)
{
    ...
    rcScopedDelete<unsigned short> srcReg((unsigned short*)rcAlloc(
        sizeof(unsigned short) * chf.spanCount, RC_ALLOC_TEMP));
    if (!srcReg) {
        ctx->log(RC_LOG_ERROR,
    ...
    rcScopedDelete<rcSweepSpan> sweeps(
        (rcSweepSpan*)rcAlloc(sizeof(rcSweepSpan) * nsweeps, RC_ALLOC_TEMP));
    if (!sweeps) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildRegionsMonotone: Out of memory 'sweeps' (%d).",
                 nsweeps);
        return false;
    }
...
/// especially in diagonal corridors.
                    ...
                    const int             mergeRegionArea)
{
    ...
    rcScopedDelete<unsigned short> buf((unsigned short*)rcAlloc(
        sizeof(unsigned short) * chf.spanCount * 2, RC_ALLOC_TEMP));
    if (!buf) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildRegions: Out of memory 'tmp' (%d).",
                 chf.spanCount * 4);
        return false;
    }
    ...
    while (level > 0) {
        ...
        if (sId == 0)
            ...
            appendStacks(lvlStacks[sId - 1],
                         lvlStacks[sId],
                         srcReg); // copy left overs from last level
            ...
            for (int j = 0; j < lvlStacks[sId].size(); j++) {
                ...
                if (i >= 0 && srcReg[i] == 0) {
                    if (floodRegion(x,
                        ...
                        if (regionId == 0xFFFF) {
                            ctx->log(RC_LOG_ERROR,
                                     "rcBuildRegions: Region ID overflow");
                            return false;
                        }
...

    {
        ...
        // If overlapping regions were found during merging, split those
        // regions.
        if (overlaps.size() > 0) {
            ctx->log(RC_LOG_ERROR,
                     "rcBuildRegions: %d overlapping regions.",
                     overlaps.size());
        }
    }
                         ...
                         const int             minRegionArea)
{
    ...
    if (!srcReg) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildLayerRegions: Out of memory 'src' (%d).",
                 chf.spanCount);
        return false;
    }
    memset(srcReg, 0, sizeof(unsigned short) * chf.spanCount);
    ...
    if (!sweeps) {
        ctx->log(RC_LOG_ERROR,
                 "rcBuildLayerRegions: Out of memory 'sweeps' (%d).",
                 nsweeps);
        return false;
    }
...

    {
        ...
        if (!mergeAndFilterLayerRegions(
                ctx, minRegionArea, chf.maxRegions, chf, srcReg))
            return false;
    }
