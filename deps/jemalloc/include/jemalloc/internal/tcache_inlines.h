...
static inline void tcache_enabled_set(tsd_t* tsd, bool enabled)
{
...
JEMALLOC_ALWAYS_INLINE void tcache_event(tsd_t* tsd, tcache_t* tcache)
{
    if (TCACHE_GC_INCR == 0) {
        return;
    }
...
JEMALLOC_ALWAYS_INLINE void* tcache_alloc_large(tsd_t*    tsd,
                                                arena_t*  arena,
                                                tcache_t* tcache,
                                                size_t    size,
                                                szind_t   binind,
...
JEMALLOC_ALWAYS_INLINE void tcache_dalloc_small(
    tsd_t* tsd, tcache_t* tcache, void* ptr, szind_t binind, bool slow_path)
{
    ...
    if (unlikely(!cache_bin_dalloc_easy(bin, bin_info, ptr))) {
        tcache_bin_flush_small(
            tsd, tcache, bin, binind, (bin_info->ncached_max >> 1));
...
JEMALLOC_ALWAYS_INLINE void tcache_dalloc_large(
    tsd_t* tsd, tcache_t* tcache, void* ptr, szind_t binind, bool slow_path)
{
    cache_bin_t*      bin;
    cache_bin_info_t* bin_info;
    ...
    if (unlikely(bin->ncached == bin_info->ncached_max)) {
        tcache_bin_flush_large(
            tsd, bin, binind, (bin_info->ncached_max >> 1), tcache);
    }
...
JEMALLOC_ALWAYS_INLINE tcache_t* tcaches_get(tsd_t* tsd, unsigned ind)
{
