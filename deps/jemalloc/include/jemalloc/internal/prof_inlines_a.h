...
prof_accum_add(tsdn_t* tsdn, prof_accum_t* prof_accum, uint64_t accumbytes)
{
    ...
    do {
        ...
        &prof_accum->accumbytes, &a0, a1, ATOMIC_RELAXED, ATOMIC_RELAXED));
#else
...
prof_accum_cancel(tsdn_t* tsdn, prof_accum_t* prof_accum, size_t usize)
{
    ...
    malloc_mutex_lock(tsdn, &prof_accum->mtx);
    a0 = prof_accum->accumbytes;
    a1 = (a0 >= SC_LARGE_MINCLASS - usize) ? a0 - (SC_LARGE_MINCLASS - usize)
                                           : 0;
    prof_accum->accumbytes = a1;
    malloc_mutex_unlock(tsdn, &prof_accum->mtx);
#endif
...
JEMALLOC_ALWAYS_INLINE bool prof_active_get_unlocked(void)
{
    /*
