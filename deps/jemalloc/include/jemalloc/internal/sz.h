...
JEMALLOC_ALWAYS_INLINE pszind_t sz_psz2ind(size_t psz)
{
    ...
    pszind_t shift =
        (x < SC_LG_NGROUP + LG_PAGE) ? 0 : x - (SC_LG_NGROUP + LG_PAGE);
    pszind_t grp = shift << SC_LG_NGROUP;

    pszind_t lg_delta =
        (x < SC_LG_NGROUP + LG_PAGE + 1) ? LG_PAGE : x - SC_LG_NGROUP - 1;
...
static inline size_t sz_pind2sz_compute(pszind_t pind)
{
    ...
    size_t grp_size =
        ((ZU(1) << (LG_PAGE + (SC_LG_NGROUP - 1))) << grp) & grp_size_mask;
...
static inline size_t sz_pind2sz_lookup(pszind_t pind)
{
...
static inline size_t sz_pind2sz(pszind_t pind)
{
    assert(pind < SC_NPSIZES + 1);
    return sz_pind2sz_lookup(pind);
}

static inline size_t sz_psz2u(size_t psz)
{
    if (unlikely(psz > SC_LARGE_MAXCLASS)) {
        return SC_LARGE_MAXCLASS + PAGE;
    }
...
static inline szind_t sz_size2index_compute(size_t size)
{
...
JEMALLOC_ALWAYS_INLINE szind_t sz_size2index_lookup(size_t size)
{
...
static inline size_t sz_index2size_compute(szind_t index)
{
#if (SC_NTINY > 0)
    if (index < SC_NTINY) {
...
#endif
    {
        ...
        size_t grp_size_mask = ~((!!grp) - 1);
        size_t grp_size =
            ((ZU(1) << (LG_QUANTUM + (SC_LG_NGROUP - 1))) << grp) &
            grp_size_mask;
...
JEMALLOC_ALWAYS_INLINE size_t sz_index2size_lookup(szind_t index)
{
...
JEMALLOC_ALWAYS_INLINE size_t sz_index2size(szind_t index)
{
    assert(index < SC_NSIZES);
    return sz_index2size_lookup(index);
}

JEMALLOC_ALWAYS_INLINE size_t sz_s2u_compute(size_t size)
{
    if (unlikely(size > SC_LARGE_MAXCLASS)) {
        return 0;
    }
    ...
    if (size <= (ZU(1) << SC_LG_TINY_MAXCLASS)) {
        ...
        return (lg_ceil < lg_tmin ? (ZU(1) << lg_tmin) : (ZU(1) << lg_ceil));
    }
#endif
    {
...
JEMALLOC_ALWAYS_INLINE size_t sz_s2u_lookup(size_t size)
{
 ...
 * specified size.
 */
JEMALLOC_ALWAYS_INLINE size_t sz_s2u(size_t size)
{
    if (likely(size <= SC_LOOKUP_MAXCLASS)) {
 ...
 */
JEMALLOC_ALWAYS_INLINE size_t sz_sa2u(size_t size, size_t alignment)
{
    size_t usize;
