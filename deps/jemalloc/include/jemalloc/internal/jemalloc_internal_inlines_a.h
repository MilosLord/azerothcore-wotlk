...
JEMALLOC_ALWAYS_INLINE malloc_cpuid_t malloc_getcpu(void)
{
...
JEMALLOC_ALWAYS_INLINE unsigned percpu_arena_choose(void)
{
    ...
    unsigned arena_ind;
    if ((opt_percpu_arena == percpu_arena) || ((unsigned)cpuid < ncpus / 2)) {
        arena_ind = cpuid;
    }
    else {
...
JEMALLOC_ALWAYS_INLINE unsigned percpu_arena_ind_limit(percpu_arena_mode_t mode)
{
...
arena_tdata_get(tsd_t* tsd, unsigned ind, bool refresh_if_missing)
{
    ...
    if (unlikely(ind >= tsd_narenas_tdata_get(tsd))) {
         ...
         * initialized.
         */
        return (refresh_if_missing ? arena_tdata_get_hard(tsd, ind) : NULL);
    }
...
arena_get(tsdn_t* tsdn, unsigned ind, bool init_if_missing)
{
    ...
    if (unlikely(ret == NULL)) {
        if (init_if_missing) {
            ret = arena_init(tsdn, ind, (extent_hooks_t*)&extent_hooks_default);
        }
    }
    return ret;
}

static inline ticker_t* decay_ticker_get(tsd_t* tsd, unsigned ind)
{
    arena_tdata_t* tdata;
...
JEMALLOC_ALWAYS_INLINE cache_bin_t* tcache_small_bin_get(tcache_t* tcache,
                                                         szind_t   binind)
{
...
JEMALLOC_ALWAYS_INLINE cache_bin_t* tcache_large_bin_get(tcache_t* tcache,
                                                         szind_t   binind)
{
...
JEMALLOC_ALWAYS_INLINE bool tcache_available(tsd_t* tsd)
{
    /*
    ...
    if (likely(tsd_tcache_enabled_get(tsd))) {
        ...
        assert(tsd_tcachep_get(tsd)->arena == NULL ||
               tcache_small_bin_get(tsd_tcachep_get(tsd), 0)->avail != NULL);
        return true;
    }
...
static inline void pre_reentrancy(tsd_t* tsd, arena_t* arena)
{
...
static inline void post_reentrancy(tsd_t* tsd)
{
