    ...
    do {                                                                       \
        ...
        if (opt_prof_active == false) {                                        \
            ...
            if (prof_tdata->threshold == 0) {                                  \
                ...
                /* each thread.                               */               \
                prof_tdata->prng_state = (uint64_t)(uintptr_t) & size;         \
...
void        prof_realloc(const void*     ptr,
                         size_t          usize,
                         prof_thr_cnt_t* cnt,
                         size_t          old_usize,
                         prof_ctx_t*     old_ctx);
...
malloc_tsd_externs(prof_tdata, prof_tdata_t*) malloc_tsd_funcs(
    JEMALLOC_INLINE, prof_tdata, prof_tdata_t*, NULL, prof_tdata_cleanup)

    JEMALLOC_INLINE prof_tdata_t* prof_tdata_get(bool create)
{
    prof_tdata_t* prof_tdata;
...
JEMALLOC_INLINE void prof_sample_threshold_update(prof_tdata_t* prof_tdata)
{
    /*
    ...
    prof_tdata->threshold =
        (uint64_t)(log(u) /
                   log(1.0 -
                       (1.0 / (double)((uint64_t)1U << opt_lg_prof_sample)))) +
        (uint64_t)1U;
#endif
}

JEMALLOC_INLINE prof_ctx_t* prof_ctx_get(const void* ptr)
{
    prof_ctx_t*    ret;
    arena_chunk_t* chunk;
...
JEMALLOC_INLINE bool prof_sample_accum_update(size_t size)
{
    prof_tdata_t* prof_tdata;
...
JEMALLOC_INLINE void prof_realloc(const void*     ptr,
                                  size_t          usize,
                                  prof_thr_cnt_t* cnt,
                                  size_t          old_usize,
                                  prof_ctx_t*     old_ctx)
...
JEMALLOC_INLINE void prof_free(const void* ptr, size_t size)
{
