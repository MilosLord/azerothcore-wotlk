...
#define log_do_begin(log_var)                                                  \
    if (config_log) {                                                          \
        unsigned log_state = atomic_load_u(&(log_var).state, ATOMIC_RELAXED);  \
 ...
 */
static inline void log_impl_varargs(const char* name, ...)
{
...
#define LOG(log_var_str, ...)                                                  \
    do {                                                                       \
        ...
        log_do_begin(log_var) log_impl_varargs((log_var).name, __VA_ARGS__);   \
        log_do_end(log_var)                                                    \
    } while (0)
