...
#define JEMALLOC_GENERATE_ATOMICS(type, short_type, lg_size)                   \
    ...
    ATOMIC_INLINE type atomic_exchange_##short_type(                           \
        atomic_##short_type##_t* a, type val, atomic_memory_order_t mo)        \
    {                                                                          \
        return (type)ATOMIC_INTERLOCKED_NAME(_InterlockedExchange, lg_size)(   \
            &a->repr, (ATOMIC_INTERLOCKED_REPR(lg_size))val);                  \
    }                                                                          \
                                                                               \
    ...
    {                                                                          \
        ...
        ATOMIC_INTERLOCKED_REPR(lg_size)                                       \
        old = ATOMIC_INTERLOCKED_NAME(_InterlockedCompareExchange,             \
                                      lg_size)(&a->repr, d, e);                \
        if (old == e) {                                                        \
            return true;                                                       \
        }                                                                      \
...
#define JEMALLOC_GENERATE_INT_ATOMICS(type, short_type, lg_size)               \
    ...
    {                                                                          \
        ...
        __pragma(warning(push)) __pragma(warning(                              \
            disable : 4146)) return atomic_fetch_add_##short_type(a,           \
                                                                  -val,        \
                                                                  mo);         \
