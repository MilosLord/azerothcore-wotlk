...
#define SC_NPSIZES                                                             \
    (          /* Start with all the size classes. */                          \
     ...
     - (LG_PAGE - 1 - SC_LG_FIRST_REGULAR_BASE) *                              \
           SC_NGROUP /* And the pseudo-group. */                               \
     - SC_NPSEUDO    /* And the tiny group. */                                 \
     - SC_NTINY      /* Sizes where ndelta*delta is not a multiple of the page \
                        size. */                                               \
...
#define SC_NBINS                                                               \
    (                      /* Sub-regular size classes. */                     \
     SC_NTINY + SC_NPSEUDO /* Groups with lg_regular_min_base <= lg_base <=    \
                              lg_base_max */                                   \
     + SC_NGROUP *                                                             \
           (LG_PAGE + SC_LG_NGROUP -                                           \
            SC_LG_FIRST_REGULAR_BASE) /* Last SC of the last group hits the    \
                                         bound exactly; exclude it. */         \
     - 1)
 ...
 */
void sc_data_update_slab_size(sc_data_t* data,
                              size_t     begin,
                              size_t     end,
                              int        pgs);
