...
#define JEMALLOC_VALGRIND_REALLOC(                                             \
    ptr, usize, old_ptr, old_usize, old_rzsize, zero)                          \
    do {                                                                       \
        if (config_valgrind && opt_valgrind) {                                 \
            size_t rzsize = p2rz(ptr);                                         \
                                                                               \
            if (ptr == old_ptr) {                                              \
                VALGRIND_RESIZEINPLACE_BLOCK(ptr, old_usize, usize, rzsize);   \
                if (zero && old_usize < usize) {                               \
                    VALGRIND_MAKE_MEM_DEFINED(                                 \
                        (void*)((uintptr_t)ptr + old_usize),                   \
                        usize - old_usize);                                    \
                }                                                              \
            }                                                                  \
            else {                                                             \
                if (old_ptr != NULL) {                                         \
                    VALGRIND_FREELIKE_BLOCK(old_ptr, old_rzsize);              \
                }                                                              \
                if (ptr != NULL) {                                             \
                    size_t copy_size =                                         \
                        (old_usize < usize) ? old_usize : usize;               \
                    size_t tail_size = usize - copy_size;                      \
                    VALGRIND_MALLOCLIKE_BLOCK(ptr, usize, rzsize, false);      \
                    if (copy_size > 0) {                                       \
                        VALGRIND_MAKE_MEM_DEFINED(ptr, copy_size);             \
                    }                                                          \
                    if (zero && tail_size > 0) {                               \
                        VALGRIND_MAKE_MEM_DEFINED(                             \
                            (void*)((uintptr_t)ptr + copy_size), tail_size);   \
                    }                                                          \
                }                                                              \
            }                                                                  \
        }                                                                      \
 ...
 * for allocations.
 */
malloc_tsd_externs(arenas, arena_t*) malloc_tsd_funcs(
    JEMALLOC_ALWAYS_INLINE, arenas, arena_t*, NULL, arenas_cleanup)

    /*
     * Compute usable size that would result from allocating an object with the
     * specified size.
     */
    JEMALLOC_ALWAYS_INLINE size_t s2u(size_t size)
{
...
void* ipalloct(
    size_t usize, size_t alignment, bool zero, bool try_tcache, arena_t* arena);
...
void*  iralloct_realign(void*    ptr,
                        size_t   oldsize,
                        size_t   size,
                        size_t   extra,
                        size_t   alignment,
...
void*  iralloct(void*    ptr,
                ...
                arena_t* arena);
void*  iralloc(
     void* ptr, size_t size, size_t extra, size_t alignment, bool zero);
bool ixalloc(void* ptr, size_t size, size_t extra, size_t alignment, bool zero);
...
JEMALLOC_ALWAYS_INLINE void* ipalloct(
    size_t usize, size_t alignment, bool zero, bool try_tcache, arena_t* arena)
{
    void* ret;
    ...
    if (usize <= arena_maxclass && alignment <= PAGE)
        ...
        if (usize <= arena_maxclass) {
            ret = arena_palloc(choose_arena(arena), usize, alignment, zero);
        }
...
JEMALLOC_ALWAYS_INLINE void idalloc(void* ptr) { idalloct(ptr, true); }
...
JEMALLOC_ALWAYS_INLINE void iqalloc(void* ptr) { iqalloct(ptr, true); }
...
malloc_tsd_externs(thread_allocated, thread_allocated_t)
    malloc_tsd_funcs(JEMALLOC_ALWAYS_INLINE,
                     thread_allocated,
                     thread_allocated_t,
